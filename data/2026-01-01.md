...
Method: 提出NIRNL框架：1) 跨模态边界保持(CMP)调整正负样本对相对距离；2) 邻居感知实例精炼(NIR)通过跨模态邻域共识识别纯子集、硬子集和噪声子集；3) 为细粒度划分构建定制优化策略
...
<details>
  <summary>Details</summary>
Motivation: 现有视频语言模型在事件级感知的时间定位上存在困难，时间定位和文本响应这两个关键任务通常耦合处理，缺乏清晰的逻辑层次结构，导致次优目标

Method: 提出D²VLM框架，采用"先定位后回答并引用证据"范式，引入证据令牌进行证据定位；设计因子化偏好优化算法，将概率时间定位建模明确纳入优化目标；构建合成数据集支持因子化偏好学习
...
TL;DR: 提出空间提升全景拼接框架，将2D图像提升为3D点云表示，通过统一投影中心生成几何一致的360°全景图，有效解决传统方法在视差和遮挡场景中的失真问题。


<details>
  <summary>Details</summary>
Motivation: 传统图像拼接技术主要使用2D单应性变换和网格变形，适用于近似共面或视差较小的场景。但在真实3D场景中，存在多个深度层和遮挡时，这些方法会导致重影、结构弯曲和拉伸失真，特别是在多视角累积和360°闭环拼接场景中问题更加严重。

Method: 1. 将输入图像提升为统一坐标系下的密集3D点表示，通过置信度指标增强全局跨视图融合；2. 在3D空间中建立统一投影中心，使用等距圆柱投影将融合数据映射到单个全景流形；3. 在画布域进行空洞填充，处理视点转换中出现的未知区域，恢复连续纹理和语义一致性。

Result: 实验评估表明，该方法在显著视差和复杂遮挡场景中，大幅减少了几何失真和重影伪影，生成的全景结果更加自然和一致。

Conclusion: 该框架将拼接从2D变形范式重新概念化为3D一致性范式，能够灵活整合各种3D提升和补全模块，为解决传统拼接技术在复杂3D场景中的局限性提供了有效方案。
...
TL;DR: HaineiFRDM是一个基于扩散模型的电影修复框架，通过分块训练策略、全局提示模块和频率模块，在单GPU上实现高质量高分辨率电影修复，并构建了包含真实退化影片的数据集。


<details>
  <summary>Details</summary>
Motivation: 现有开源电影修复方法性能有限，主要原因是使用低质量合成数据训练和噪声光流，且未探索高分辨率影片。需要利用扩散模型强大的内容理解能力来帮助专家修复难以识别的影片缺陷。

Method: 采用分块训练测试策略使单24GB GPU能处理高分辨率影片；设计位置感知的全局提示和帧融合模块；引入全局-局部频率模块保持不同分块间纹理一致性；先修复低分辨率结果作为全局残差减少分块伪影；构建包含真实退化影片和合成数据的修复数据集。

Result: 综合实验结果表明，该模型在缺陷修复能力上明显优于现有开源方法，能够有效恢复高分辨率影片中的各种缺陷。

Conclusion: HaineiFRDM框架成功探索了扩散模型在电影修复中的应用，通过创新的分块策略和模块设计，在有限硬件条件下实现了高质量高分辨率电影修复，填补了开源方法在高分辨率影片修复领域的空白。
...
TL;DR: 提出一种基于Depth Anything V2架构的单目深度估计方法，通过DV-LORA适配技术将合成先验迁移到医疗领域，在手术内窥镜环境中显著提升深度估计精度，特别是在高反射区域。


<details>
  <summary>Details</summary>
Motivation: 传统自监督单目深度估计方法在手术内窥镜环境中表现脆弱，特别是在反射和充满液体的环境下，容易在薄手术工具和透明表面产生边界崩溃问题。现有方法通常依赖噪声真实世界伪标签训练的基础模型，无法有效处理这些挑战。

Method: 1. 利用Depth Anything V2架构的高保真合成先验，该架构能捕捉薄结构的精确几何细节；2. 采用动态向量低秩适配(DV-LORA)技术，以最小参数预算将合成先验高效适配到医疗领域，弥合合成到真实的差距；3. 在SCARED数据集上引入物理分层评估协议，严格量化高反射区域的性能。

Result: 在SCARED数据集上达到新的最先进水平：准确度(<1.25)达到98.1%，平方相对误差相比基线降低超过17%，在不利手术光照条件下表现出卓越的鲁棒性。

Conclusion: 通过结合Depth Anything V2的合成先验和DV-LORA适配技术，成功解决了手术内窥镜环境中单目深度估计的挑战，特别是在高反射区域，为机器人手术提供了更可靠的深度感知能力。
...
Method: 使用计算机视觉模型从训练视频中提取2D骨架、注视向量和运动轨迹，开发任务特定指标衡量心理运动流畅性、情境意识和团队协调，通过加权组合的认知任务分析层次生成总体性能评分。

Result: 通过真实世界ECR演练案例研究，展示了系统能提供可操作的领域特定指标，支持行动后审查和交互式仪表板，为Gamemaster和GIFT系统提供直观反馈。

Conclusion: 视频分析可实现合成训练环境中的可扩展评估，未来工作包括扩展到3D视频分析和解决跟踪困难、地面真实验证等限制。
...
TL;DR: PFP是一种神经网络结构，可将长视频压缩为短上下文，通过预训练目标保留任意时间位置单帧的高频细节，支持低上下文成本的长历史记忆


<details>
  <summary>Details</summary>
Motivation: 解决长视频处理中上下文长度限制问题，需要在压缩视频的同时保持单帧的视觉细节，为自回归视频模型提供高效的内存编码

Method: 提出PFP神经网络结构，设计显式预训练目标来保留任意时间位置单帧的高频细节，可将20秒视频压缩至约5k长度的上下文

Result: 预训练模型可直接微调作为自回归视频模型的内存编码器，实现低上下文成本的长历史记忆，相对保真度损失较低

Conclusion: PFP框架通过神经架构设计的权衡分析，为长视频压缩和记忆编码提供了有效解决方案，平衡了上下文成本与视觉保真度
...
<details>
  <summary>Details</summary>
Motivation: 现有3D人体姿态估计方法依赖受控环境标注数据，难以泛化到真实场景。传统领域自适应方法忽略了目标姿态数据集的非平稳性，需要解决连续适应新领域同时保留历史知识的问题。

Method: 提出创新的GAN框架，包含3D姿态生成器、2D姿态判别器和3D姿态估计器。构建新型3D姿态生成器范式，整合姿态感知、时序感知和领域感知知识，增强当前领域适应并缓解对先前领域的灾难性遗忘。

Result: 在多个领域自适应3D人体姿态估计数据集上进行广泛实验，证明了方法的优越性能。

Conclusion: 首次将终身领域自适应引入3D人体姿态估计任务，提出的GAN框架能有效缓解领域偏移和对齐原始与增强姿态，为解决非平稳目标数据集和灾难性遗忘问题提供了有效方案。
...
TL;DR: 提出深度学习管道，将儿童T1加权MRI转换为合成CT，实现颅骨分割、缝合线概率热图和直接缝合线分割，填补MRI无法显示骨骼和缝合线的空白。


<details>
  <summary>Details</summary>
Motivation: CT可用于评估儿童颅骨发育和缝合线骨化，但有电离辐射风险；MRI无辐射但无法显示颅骨缝合线和评估骨密度。需要非侵入性方法评估儿童颅骨发育。

Method: 使用深度学习驱动的管道，将0.2-2岁儿童的T1加权MRI转换为合成CT，预测详细的颅骨分割，生成缝合线概率热图，并从热图直接获得缝合线分割。采用领域特定的变分自编码器。

Result: 合成CT与真实CT的结构相似度达99%，Fréchet起始距离为1.01。颅骨分割在七个颅骨上平均Dice系数达85%，缝合线分割Dice系数达80%。通过TOST检验证实合成CT与真实CT在分割上的等效性。

Conclusion: 这是首个能从MRI生成合成CT并实现缝合线分割的儿科颅骨CT合成框架，填补了MRI无法显示骨骼和缝合线的关键空白，为非侵入性颅骨评估提供了新方法。
...
TL;DR: 该论文研究了在高分辨率遥感数据上训练基础模型的扩展行为，使用超过千万亿像素的商业卫星数据，发现即使在此规模下，性能仍受数据限制而非模型参数限制。


<details>
  <summary>Details</summary>
Motivation: 遥感领域缺乏像自然图像领域那样明确的扩展规律，而现代多模态AI应用需要强大的领域专用编码器。研究旨在为前沿规模的遥感基础模型开发提供实用指导。

Method: 使用超过千万亿像素的商业卫星EO数据和MITRE联邦AI沙盒，训练逐渐增大的视觉变换器（ViT）骨干网络，分析在petascale规模下的成功和失败模式。

Result: 即使在如此大规模下，性能仍与数据受限机制一致，而非模型参数受限。研究为数据收集策略、计算预算和优化计划提供了实用见解。

Conclusion: 该研究为遥感基础模型的未来发展提供了重要指导，揭示了在遥感领域扩展模型时数据规模的关键作用，有助于弥合不同遥感模态之间的领域差距。
...
TL;DR: 提出一种基于双层优化的无监督胎儿超声技能评估框架，通过任务执行质量来量化技能水平，无需人工标注的技能评级。


<details>
  <summary>Details</summary>
Motivation: 传统超声技能评估依赖专家监督和反馈，具有主观性强、耗时长的缺点。现有量化评估方法多采用监督学习，局限于预定义的技能影响因素分析。

Method: 提出双层优化框架，包含临床任务预测器和技能预测器，通过联合优化同时精炼两个网络，将优化的任务性能作为技能指标。

Result: 在真实临床胎儿头部超声扫描视频上验证，结果表明该框架能够有效预测超声技能，量化任务性能作为技能指标具有可行性。

Conclusion: 该无监督框架能够客观评估胎儿超声技能，通过任务执行质量量化技能水平，为超声技能评估提供了新的自动化解决方案。
...
### [12] [MGML: A Plug-and-Play Meta-Guided Multi-Modal Learning Framework for Incomplete Multimodal Brain Tumor Segmentation](https://arxiv.org/abs/2512.23936)
*Yulong Zou,Bo Liu,Cun-Jing Zheng,Yuan-ming Geng,Siyue Li,Qiankun Zuo,Shuihua Wang,Yudong Zhang,Jin Hong*

Main category: cs.CV

TL;DR: 提出MGML框架，通过元参数化自适应模态融合和一致性正则化，解决临床MRI多模态数据不完整时的脑肿瘤分割问题
...
Method: 提出元引导多模态学习框架，包含元参数化自适应模态融合和一致性正则化模块。Meta-AMF根据可用模态生成自适应软标签监督信号，促进更一致的多模态融合

Result: 在BraTS2020和BraTS2023数据集上优于多个SOTA方法。在BraTS2020的15种缺失模态组合平均Dice分数：WT 87.55、TC 79.36、ET 62.67

Conclusion: MGML框架能有效处理不完整多模态MRI数据，不改变原始模型架构，可方便集成到训练流程中进行端到端优化
...
### [13] [Learnable Query Aggregation with KV Routing for Cross-view Geo-localisation](https://arxiv.org/abs/2512.23938)
*Hualin Ye,Bingxi Liu,Jixiang Du,Yu Qin,Ziyi Chen,Hong Zhang*

Main category: cs.CV

TL;DR: 提出了一种新颖的跨视角地理定位系统，通过DINOv2骨干网络、多尺度通道重分配模块和改进的聚合模块，在减少训练参数的同时实现了竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 跨视角地理定位中存在的显著视角差异给特征聚合和对齐带来了巨大挑战，需要更有效的特征表示和匹配方法。

Method: 1) 使用带卷积适配器微调的DINOv2骨干网络增强模型适应性；2) 提出多尺度通道重分配模块增强空间表示多样性；3) 提出改进的聚合模块，在交叉注意力框架中集成MoE路由，动态选择专家子空间处理异构输入域。

Result: 在University-1652和SUES-200数据集上的大量实验表明，该方法以更少的训练参数实现了竞争性性能。

Conclusion: 提出的CVGL系统通过三个关键改进有效解决了跨视角差异问题，在减少参数量的同时保持了高性能，为跨视角地理定位提供了有效的解决方案。
...
### [14] [Kinematic-Based Assessment of Surgical Actions in Microanastomosis](https://arxiv.org/abs/2512.23942)
*Yan Meng,Daniel Donoho,Marcelle Altshuler,Omar Arnaout*

Main category: cs.CV

TL;DR: 提出AI驱动的微血管吻合术动作分割与性能评估框架，实现92.4%帧级分割准确率和85.5%技能分类准确率


<details>
  <summary>Details</summary>
Motivation: 微血管吻合术需要精细操作技能，传统评估依赖专家主观评分，存在评分者间变异性和时间成本高的问题，需要自动化、可扩展的解决方案

Method: 三模块框架：1) 基于YOLO和DeepSORT的器械尖端跟踪定位；2) 基于自相似矩阵的动作边界检测和无监督聚类的动作分割；3) 评估手术手势熟练度的监督分类模块

Result: 在58个专家评分的微血管吻合术视频数据集上验证，帧级动作分割准确率92.4%，技能分类准确率85.5%

Conclusion: 该方法能为显微外科教育提供客观、实时反馈，实现标准化、数据驱动的训练方案，推进高风险手术环境中的能力评估
...
### [15] [U-Net-Like Spiking Neural Networks for Single Image Dehazing](https://arxiv.org/abs/2512.23950)
*Huibin Li,Haoran Liu,Mingzhe Liu,Yulong Xiao,Peng Li,Guibin Zan*

Main category: cs.CV

TL;DR: DehazeSNN：一种基于脉冲神经网络的图像去雾方法，结合U-Net架构和OLIFBlock，在保持高性能的同时降低计算成本


<details>
  <summary>Details</summary>
Motivation: 传统去雾方法依赖大气散射模型，而深度学习方法中CNN难以捕捉长距离依赖，Transformer计算成本高。需要一种既能有效处理多尺度特征又能降低计算负担的新方法。

Method: 提出DehazeSNN架构，结合U-Net-like设计和脉冲神经网络(SNN)，引入正交泄漏积分发放块(OLIFBlock)增强跨通道通信，有效管理局部和长距离依赖关系。

Result: 在基准数据集上表现出与最先进方法相当的竞争力，能够生成高质量无雾图像，同时具有更小的模型尺寸和更少的乘累加运算。

Conclusion: DehazeSNN为图像去雾提供了一种高效且有效的解决方案，平衡了性能与计算效率，代码已开源供研究使用。
...
### [16] [T2VAttack: Adversarial Attack on Text-to-Video Diffusion Models](https://arxiv.org/abs/2512.23953)
...
<details>
  <summary>Details</summary>
Motivation: 尽管文本到视频（T2V）扩散模型在生成高质量视频方面取得了显著进展，但其对抗攻击的脆弱性尚未得到充分研究。考虑到视频数据的动态特性，需要从语义和时间两个维度全面评估模型的鲁棒性。

Method: 提出两种攻击方法：T2VAttack-S通过贪婪搜索识别提示中语义或时间关键词并用同义词替换；T2VAttack-I通过迭代插入优化词对提示进行最小扰动。同时定义语义目标（评估视频-文本对齐）和时间目标（评估时间动态）。

Result: 在ModelScope、CogVideoX、Open-Sora、HunyuanVideo等多个先进T2V模型上的实验表明，即使是单个词的替换或插入，也能导致语义保真度和时间动态的显著下降，揭示了当前T2V扩散模型的关键脆弱性。

Conclusion: 当前T2V扩散模型对对抗攻击高度脆弱，微小的提示修改就能破坏视频质量。这强调了在开发更鲁棒的T2V系统时需要考虑对抗鲁棒性的重要性。
...
### [17] [DriveExplorer: Images-Only Decoupled 4D Reconstruction with Progressive Restoration for Driving View Extrapolation](https://arxiv.org/abs/2512.23983)
*Yuang Jia,Jinlong Wang,Jiayi Zhao,Chunlam Li,Shunzhou Wang,Wei Gao*

Main category: cs.CV

TL;DR: 提出一种用于自动驾驶场景视图外推的有效方法，仅需图像和可选相机位姿，通过4D高斯模型与视频扩散模型的迭代优化，无需昂贵传感器或标注数据。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的方法依赖LiDAR点云、3D边界框和车道标注等先验信息，需要昂贵传感器或人工标注，限制了实际部署。本文旨在仅使用图像和可选相机位姿实现高质量视图外推。

Method: 首先从图像估计全局静态点云和逐帧动态点云，融合为统一表示。采用可变形4D高斯框架重建场景。初始训练的4D高斯模型生成降质伪图像训练视频扩散模型，然后通过扩散模型迭代优化逐步偏移的高斯渲染结果，并将增强结果反馈给4DGS训练，直到外推至目标视点。

Result: 与基线方法相比，该方法在新型外推视点上生成更高质量的图像。

Conclusion: 提出了一种仅需图像输入的自动驾驶视图外推方法，通过4D高斯与扩散模型的协同优化，无需昂贵传感器或标注，实现了高质量的外推结果。
...
### [18] [Anomaly detection in satellite imagery through temporal inpainting](https://arxiv.org/abs/2512.23986)
*Bertrand Rouet-Leduc,Claudia Hulbert*

Main category: cs.CV

TL;DR: 基于深度学习的时间序列卫星图像异常检测方法，通过预测无变化情况下的地表形态来检测地表变化，比传统方法灵敏度高3倍。


<details>
  <summary>Details</summary>
Motivation: 卫星图像地表变化检测对灾害响应和环境监测至关重要，但面临大气噪声、季节变化和传感器伪影等复杂因素的挑战，传统方法难以达到高灵敏度。

Method: 使用基于SATLAS基础模型的修复模型，利用全球分布的Sentinel-2时间序列数据训练，从先前图像预测最后一帧的无变化地表形态，通过预测与观测的差异检测异常。

Result: 在2023年土耳其-叙利亚地震序列中成功检测到Tepehan的断裂特征，检测灵敏度比时间中值或Reed-Xiaoli异常检测器高约3倍，具有更高的特异性和灵敏度。
...
### [19] [GCA-ResUNet: Medical Image Segmentation Using Grouped Coordinate Attention](https://arxiv.org/abs/2512.23990)
*Jun Ding,Shang Gao*

Main category: cs.CV

TL;DR: 提出GCA-ResUNet，一种用于医学图像分割的高效框架，通过轻量级分组坐标注意力模块增强全局表示能力，在保持CNN效率的同时提升分割精度。


<details>
  <summary>Details</summary>
Motivation: 现有U-Net架构在建模长距离上下文依赖方面存在局限，特别是在多器官场景和低对比度区域。Transformer架构虽然能缓解此问题，但计算资源需求高，不利于在资源受限的临床环境中部署。

Method: 提出分组坐标注意力模块，将通道上下文建模解耦为多个组以考虑通道间的语义异质性，并集成方向感知坐标编码来捕获水平和垂直轴的结构化空间依赖关系。
...
### [20] [Bridging Structure and Appearance: Topological Features for Robust Self-Supervised Segmentation](https://arxiv.org/abs/2512.23997)
*Haotang Li,Zhenyu Qi,Hao Qin,Huanrui Yang,Sen He,Kebin Peng*

Main category: cs.CV

TL;DR: GASeg通过引入拓扑信息解决自监督语义分割中的外观歧义问题，使用可微盒计数模块量化多尺度拓扑统计，结合拓扑增强和几何-外观对齐损失，在多个基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 自监督语义分割方法在面对外观歧义（如阴影、眩光、局部纹理）时经常失败，这是因为过度依赖不稳定的外观特征。需要引入稳定的拓扑信息来桥接几何和外观特征。

Method: 提出GASeg框架，核心是可微盒计数(DBC)模块，从几何特征流和外观特征流中量化多尺度拓扑统计。使用拓扑增强(TopoAug)通过形态学操作模拟真实世界歧义，并通过GALoss损失强制几何和外观特征的跨模态对齐。

Result: 在COCO-Stuff、Cityscapes、PASCAL等四个基准测试中取得了最先进的性能，验证了通过拓扑信息桥接几何和外观方法的有效性。

Conclusion: 通过引入稳定的拓扑信息，GASeg成功解决了自监督语义分割中的外观歧义问题，证明了桥接几何和外观特征对于提升分割鲁棒性的重要性。
...
### [21] [Improved 3D Gaussian Splatting of Unknown Spacecraft Structure Using Space Environment Illumination Knowledge](https://arxiv.org/abs/2512.23998)
*Tae Ha Park,Simone D'Amico*

Main category: cs.CV

TL;DR: 提出一种结合太阳位置先验知识的3D高斯溅射(3DGS)管道，用于从空间交会对接图像序列中重建未知航天器的3D结构，并改善动态光照条件下的渲染质量。


<details>
  <summary>Details</summary>
Motivation: 在空间交会对接(RPO)过程中，从图像序列重建目标航天器的3D结构面临挑战：传统3DGS需要静态场景假设，而空间图像存在动态光照变化；同时，重建模型的渲染质量直接影响后续相机姿态估计的精度。

Method: 提出将服务航天器估计的太阳位置先验知识整合到3DGS训练管道中，使模型能够适应空间快速变化的光照条件，学习全局阴影和自遮挡效应，从而提高渲染的光度精度。

Result: 实验研究表明，所提方法能有效处理空间动态光照，训练出的3DGS模型能够适应快速变化的光照条件，并准确反映全局阴影和自遮挡现象。

Conclusion: 通过整合太阳位置先验知识，提出的3DGS管道能够在动态光照条件下准确重建航天器的3D结构，提高渲染的光度精度，从而支持空间交会对接过程中的相机姿态估计任务。
...
### [22] [Bridging the Perception-Cognition Gap:Re-engineering SAM2 with Hilbert-Mamba for Robust VLM-based Medical Diagnosis](https://arxiv.org/abs/2512.24013)
*Hao Wu,Hui Li,Yiyun Su*

Main category: cs.CV

TL;DR: 提出Hilbert-VLM框架，通过Hilbert空间填充曲线改进SAM2架构，结合Mamba SSM和跨注意力机制，实现3D医学图像精准分割和疾病分类。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在医学诊断中面临处理复杂3D多模态医学图像的挑战，包括互补信息有效整合和关键病理特征的遗漏问题。

Method: 提出两阶段融合框架：1) HilbertMed-SAM模块进行精确病灶分割，使用Hilbert空间填充曲线改进SAM2的Mamba SSM扫描机制，引入Hilbert-Mamba跨注意力机制和尺度感知解码器；2) 提示增强模块将分割掩码和文本属性统一为信息密集提示，指导VLM进行疾病分类。

Result: 在BraTS2021分割基准测试中达到82.35%的Dice分数，诊断分类准确率为78.85%，显著提升了医学VLM分析的准确性和可靠性。

Conclusion: Hilbert-VLM模型通过创新的架构设计和融合策略，有效解决了3D医学图像分析中的关键挑战，为医学VLM应用提供了有前景的解决方案。
...
### [23] [On Exact Editing of Flow-Based Diffusion Models](https://arxiv.org/abs/2512.24015)
...
Method: 提出条件速度校正(CVC)框架，将流基编辑重新定义为由已知源先验驱动的分布转换问题。引入双视角速度转换机制，将潜在演化分解为保持源轨迹一致的结构保留分支和驱动向目标分布受控偏差的语义引导分支。基于经验贝叶斯推断和Tweedie校正进行后验一致更新，补偿量化偏差。

Result: CVC产生稳定可解释的潜在动态，实现忠实重建和平滑的局部语义转换。综合实验表明，CVC在不同任务中始终实现更优的保真度、更好的语义对齐和更可靠的编辑行为。

Conclusion: CVC通过双视角速度转换和后验一致更新，有效解决了流基扩散编辑中的速度误差累积问题，为分布转换提供了数学基础扎实的稳定框架。
...
### [24] [FitControler: Toward Fit-Aware Virtual Try-On](https://arxiv.org/abs/2512.24016)
*Lu Yang,Yicheng Liu,Yanan Li,Xiang Bai,Hao Lu*

Main category: cs.CV

TL;DR: FitControler是一个可学习的插件，能够集成到现代虚拟试穿模型中，实现定制化的服装合身度控制，解决了传统VTON忽视服装合身度这一关键风格因素的问题。


<details>
  <summary>Details</summary>
Motivation: 传统虚拟试穿方法主要关注服装细节的忠实渲染，但忽视了服装合身度这一塑造整体风格的关键因素。合身度定义了服装与穿着者身体的贴合方式，是时尚设计的基本要素。

Method: FitControler包含两个核心组件：1) 合身度感知布局生成器，基于精心处理的服装无关表示重新绘制身体-服装布局；2) 多尺度合身度注入器，将布局线索传递给VTON模型实现布局驱动的虚拟试穿。同时构建了Fit4Men数据集，包含13,000个不同合身度的身体-服装对。

Result: 实验表明FitControler能够与各种VTON模型协同工作，实现准确的合身度控制。引入了两种合身度一致性指标来评估生成效果。

Conclusion: 该工作首次将合身度控制引入虚拟试穿领域，通过FitControler插件和Fit4Men数据集，为VTON系统提供了定制化合身度控制能力，填补了现有研究的空白。
...
### [25] [Structure-Guided Allocation of 2D Gaussians for Image Representation and Compression](https://arxiv.org/abs/2512.24018)
...
<details>
  <summary>Details</summary>
Motivation: 现有2D高斯泼溅方法在分配表示能力和参数精度时忽视了图像结构信息，导致在低比特率下的率失真效率受限。

Method: 1) 结构引导初始化：根据自然图像的空间结构先验分配2D高斯分布；2) 自适应位宽量化：对协方差参数进行自适应精度量化，复杂区域小尺度高斯分配更高精度；3) 几何一致正则化：对齐高斯方向与局部梯度方向以保持结构细节。

Result: 在保持1000+FPS解码速度的同时，相比基线GSImage，在Kodak数据集上降低43.44% BD-rate，在DIV2K数据集上降低29.91% BD-rate。

Conclusion: 提出的结构引导分配方法显著提升了2D高斯泼溅的表示能力和率失真性能，同时保持了原生解码速度，为紧凑图像表示提供了高效解决方案。
...
### [26] [FUSE-RSVLM: Feature Fusion Vision-Language Model for Remote Sensing](https://arxiv.org/abs/2512.24022)
...
<details>
  <summary>Details</summary>
Motivation: 现有的大型视觉语言模型在遥感领域面临挑战，因为遥感图像与自然图像存在固有差异。现有遥感VLM难以提取细粒度视觉特征，且在深度语言处理过程中容易出现视觉遗忘问题。

Method: 提出MF-RSVLM模型，学习多尺度视觉表示，结合全局上下文和局部细节，改进对遥感场景中小型和复杂结构的捕捉。采用循环视觉特征注入方案，确保语言模型在生成过程中保持对视觉证据的感知，减少视觉遗忘。

Result: 在多个遥感基准测试上的广泛实验表明，MF-RSVLM在遥感分类、图像描述和视觉问答任务中实现了最先进或极具竞争力的性能。

Conclusion: MF-RSVLM通过有效的多特征融合和循环视觉特征注入，解决了遥感视觉语言模型中的细粒度特征提取和视觉遗忘问题，在遥感理解任务中表现出色。
...
### [27] [RSAgent: Learning to Reason and Act for Text-Guided Segmentation via Multi-Turn Tool Invocations](https://arxiv.org/abs/2512.24023)
*Xingqi He,Yujie Zhang,Shuyong Gao,Wenjie Li,Lingyi Hong,Mingxi Chen,Kaixun Jiang,Jiyuan Fu,Wenqiang Zhang*

Main category: cs.CV

TL;DR: RSAgent：一个通过多轮工具调用实现推理与行动交替的代理式多模态大语言模型，用于文本引导的对象分割，通过迭代推理和掩码优化提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本引导分割方法通常将问题视为一次性定位，模型在单次前向传播中预测像素提示来驱动外部分割器，当初始定位错误时缺乏验证、重新聚焦和细化的能力。

Method: 提出RSAgent代理式MLLM，通过多轮工具调用交替进行推理和分割行动：查询分割工具箱、观察视觉反馈、利用历史观察修订空间假设以重新定位目标并迭代优化掩码。构建多轮推理分割轨迹的数据管道，采用两阶段训练框架：冷启动监督微调+代理式强化学习（细粒度任务特定奖励）。

Result: 在ReasonSeg测试集上达到66.5% gIoU的零样本性能，比Seg-Zero-7B提升9%；在RefCOCOg上达到81.5% cIoU，在领域内和领域外基准测试中都展示了最先进的性能。

Conclusion: RSAgent通过代理式多轮推理和工具调用，有效解决了传统一次性定位方法的局限性，实现了更准确、可验证和可细化的文本引导对象分割。
...
### [28] [PipeFlow: Pipelined Processing and Motion-Aware Frame Selection for Long-Form Video Editing](https://arxiv.org/abs/2512.24026)
*Mustafa Munir,Md Mostafijur Rahman,Kartikeya Bhardwaj,Paul Whatmough,Radu Marculescu*

Main category: cs.CV

TL;DR: PipeFlow是一个可扩展的流水线视频编辑方法，通过跳过低运动帧、并行处理视频片段和神经网络插值技术，实现长视频编辑的线性时间增长，相比现有方法获得显著加速。


<details>
  <summary>Details</summary>
Motivation: 长视频编辑面临计算成本指数级增长的问题，主要源于联合编辑和DDIM反演在长序列上的计算开销。现有方法在处理长视频时效率低下，需要更可扩展的解决方案。

Method: 1) 基于SSIM和光流的运动分析，识别并跳过低运动帧的编辑；2) 流水线任务调度算法，将视频分割成多个片段，基于可用GPU内存并行执行DDIM反演和联合编辑；3) 神经网络插值技术平滑片段边界帧并插值跳过的帧。

Result: PipeFlow实现了线性时间增长，相比TokenFlow获得9.6倍加速，相比DMT获得31.7倍加速。理论上可以编辑无限长的视频，而不会遇到其他方法中每帧计算开销增长的问题。

Conclusion: PipeFlow通过创新的运动分析、流水线调度和插值技术，解决了长视频编辑的计算瓶颈，实现了可扩展的高效编辑，为处理超长视频提供了实用解决方案。
...
### [29] [Reinforced Diffusion: Learning to Push the Limits of Anisotropic Diffusion for Image Denoising](https://arxiv.org/abs/2512.24035)
*Xinran Qin,Yuhui Quan,Ruotao Xu,Hui Ji*

Main category: cs.CV

TL;DR: 本文提出了一种基于强化学习的可训练各向异性扩散框架，通过深度Q学习选择扩散动作，构建自适应图像结构的随机扩散过程，在多种噪声去除任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统各向异性扩散方法使用显式扩散算子，难以适应复杂图像结构，性能受限。而基于学习的方法（如深度CNN）虽然效果好，但缺乏扩散方法的理论优势。需要结合两者优点，开发自适应且性能优越的扩散去噪方法。

Method: 提出基于强化学习的可训练各向异性扩散框架：1）将去噪过程建模为一系列基础扩散动作；2）使用深度Q学习学习扩散动作的顺序选择；3）构建自适应图像结构的随机各向异性扩散过程。

Result: 实验表明：1）该方法在三种常见噪声去除任务中表现优异；2）超越了现有基于扩散的方法；3）与代表性的深度CNN方法性能相当。

Conclusion: 通过强化学习将各向异性扩散过程参数化，实现了对复杂图像结构的自适应，结合了传统扩散方法的理论优势与学习方法的性能优势，为图像去噪提供了新思路。
...
TL;DR: 本文提出PCRNet，通过病理学重校准模块和专家先验引导适配器，结合病理学上下文和专家经验先验来提升眼病识别性能和决策可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前深度神经网络在眼病识别中忽略了临床病理学上下文和专家经验先验，这限制了识别性能和决策可解释性的提升。

Method: 1) 病理学重校准模块(PRM)：结合像素级上下文压缩算子和病理分布集中算子；2) 专家先验引导适配器(EPGA)：挖掘专家经验先验突出重要像素区域；3) 集成损失(IL)：考虑样本损失分布和训练标签频率。

Result: 在三个眼病数据集上的实验表明，PCRNet结合IL优于最先进的注意力网络和先进损失方法，可视化分析解释了PRM和EPGA影响DNN决策的内在行为。

Conclusion: 通过有效整合病理学上下文和专家经验先验，PCRNet显著提升了眼病识别的性能和决策可解释性，为临床诊断提供了更可靠的辅助工具。
...
<details>
  <summary>Details</summary>
Motivation: 细粒度遥感数据集常使用层次标签结构，但将语义层次嵌入表示学习空间以提升检测性能仍具挑战。现有方法应用监督对比学习时忽视了两个关键问题：1) 标签层次中数据分布不平衡导致高频类别主导学习过程；2) 类别间语义关系学习干扰类别无关的定位任务。

Method: 提出平衡层次对比损失结合解耦学习策略，在DETR框架内实现。平衡层次对比损失引入可学习的类别原型，在每一层次平衡不同类别的梯度贡献，确保每个层次类别在每个小批次中对损失计算贡献相等。解耦策略将DETR的对象查询分为分类和定位两组，实现任务特定的特征提取和优化。

Result: 在三个具有层次标注的细粒度数据集上的实验表明，该方法优于现有最先进方法。

Conclusion: 通过平衡层次对比损失解决类别不平衡问题，通过解耦学习策略避免语义关系对定位的干扰，在DETR框架中有效提升了细粒度遥感目标检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器(DiT)模型在视频和图像生成任务中因注意力机制导致极高计算成本，限制了实际应用。现有稀疏注意力方法存在两个关键限制：稀疏模式预测的开销和缺乏硬件通用性（主要针对GPU设计）。

Method: 1) 利用块级均值作为代表性token进行稀疏掩码预测；2) 实现时空感知的token排列；3) 为视频生成场景引入首帧汇聚机制。这些技术共同构成了在线自适应、硬件高效、低开销的稀疏注意力机制。

Result: RainFusion2.0能够实现80%的稀疏度，同时获得1.5-1.8倍的端到端加速，且不损害视频质量。该方法在各种生成模型上有效，并在不同硬件平台上验证了其泛化能力。

Conclusion: RainFusion2.0成功解决了现有稀疏注意力方法的局限性，为视频和图像生成模型提供了一种高效、硬件通用的加速方案，具有实际应用价值。
...
TL;DR: 提出Latent Motion Reasoning (LMR)框架，通过两阶段"思考-行动"过程解决文本到动作生成中的语义-运动阻抗不匹配问题


<details>
  <summary>Details</summary>
Motivation: 当前文本到动作生成方法直接将符号语言映射到连续姿态，面临语义-运动阻抗不匹配的根本理论瓶颈：难以在单次映射中将语义密集的离散语言意图接地到运动密集的高频运动数据

Method: 提出Latent Motion Reasoning (LMR)框架，采用两阶段"思考-行动"决策过程。核心是双粒度分词器，将运动解耦为两个不同的流形：用于规划全局拓扑的压缩语义丰富的推理潜在空间，以及用于保持物理保真度的高频执行潜在空间

Result: 在T2M-GPT（离散）和MotionStreamer（连续）两个代表性基线上实现LMR，实验显示在语义对齐和物理合理性方面都有非平凡改进

Conclusion: 运动规划的最佳基底不是自然语言，而是学习的、运动对齐的概念空间。通过强制模型先推理（规划粗略轨迹）再行动（实例化帧），有效弥合了语言和物理之间的不可言说性鸿沟
...
TL;DR: 提出一种无需训练的生成式对抗攻击框架，通过扩散模型生成自然、场景一致的对抗物体，显著提升自动驾驶中单目深度估计攻击的有效性、隐蔽性和物理可部署性。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计(MDE)是自动驾驶系统的核心感知模块，但易受对抗攻击。现有物理攻击主要依赖纹理贴片，存在放置限制严格、真实性有限等问题，在复杂驾驶环境中效果不佳。
...
Conclusion: 该方法生成的物理可信对抗物体对自动驾驶安全评估具有重要实际意义，为复杂驾驶环境下的对抗攻击提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前几何推理评估存在三个主要问题：1) 基于教科书的基准测试存在测试数据污染风险；2) 过度强调最终答案而忽视推理过程；3) 诊断粒度不足。需要更全面、系统化的评估框架。

Method: 提出GeoBench层次化基准，包含四个推理级别：视觉感知、目标导向规划、严谨定理应用、自反思回溯。通过TrustGeoGen生成六个形式化验证的任务，系统评估几何问题解决能力。
...
TL;DR: 该论文提出了FSAP（Few-Shot Architecture Prompting）和Whitespace-Normalized Hash Validation两种方法，用于改进基于LLM的计算机视觉架构自动设计，通过大规模实验验证了n=3示例的最佳平衡效果，并提供了数据集平衡评估方法。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉中自动化神经网络架构设计面临任务多样性和计算约束的挑战。虽然大型语言模型（LLMs）为替代计算密集型的神经架构搜索（NAS）提供了有前景的替代方案，但在计算机视觉架构生成中的应用尚未得到系统研究，特别是在提示工程和验证策略方面。

Method: 1. 提出Few-Shot Architecture Prompting（FSAP）：系统研究支持示例数量（n=1-6）对LLM生成架构的影响；2. 引入Whitespace-Normalized Hash Validation：轻量级去重方法（<1ms），比AST解析快100倍；3. 采用数据集平衡评估方法，解决异构视觉任务间架构比较的挑战。

Result: 在7个计算机视觉基准测试（MNIST、CIFAR-10、CIFAR-100、CelebA、ImageNette、SVHN、Places365）上生成了1,900个独特架构。发现n=3示例在架构多样性和上下文关注之间达到最佳平衡。去重方法显著提高了效率。

Conclusion: 该研究为计算机视觉中基于LLM的架构搜索提供了实用指南，建立了严格的评估实践，使计算资源有限的研究者能够更便捷地进行自动化设计。FSAP和轻量级去重方法共同推进了高效架构搜索的发展。
...
Method: 提出方向性解耦对齐（D²-Align）框架：1）在冻结模型的情况下学习奖励模型嵌入空间中的方向性修正；2）在优化过程中应用修正后的奖励信号，防止模型崩溃到特定模式

Result: D²-Align在质量和多样性的定量指标上都表现出色，实现了与人类偏好的更好对齐，同时避免了偏好模式崩溃

Conclusion: 该工作首次量化和解决了文本到图像扩散模型对齐中的偏好模式崩溃问题，提出的D²-Align框架能有效保持生成多样性，为基于人类反馈的强化学习提供了更稳健的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏大规模、高质量的工业缺陷多模态数据集，限制了制造业质量检测中多模态学习的发展。需要构建专门针对工业场景的数据集和基础模型，以支持分类、分割、检索、生成等多种应用。

Method: 1) 构建IMDD-1M数据集：收集100万张高分辨率真实缺陷图像，覆盖60+材料类别和400+缺陷类型，每张图像配有专家验证的细粒度文本描述（包含缺陷位置、严重程度和上下文属性）；2) 基于该数据集从头训练扩散式视觉语言基础模型，专门针对工业场景设计；3) 通过轻量级微调实现模型在特定领域的适配。

Result: 1) 发布了首个大规模工业多模态缺陷数据集IMDD-1M；2) 训练的扩散式视觉语言基础模型具有良好泛化能力；3) 仅需不到5%的任务特定数据即可达到专用专家模型的性能水平；4) 模型支持分类、分割、检索、描述生成等多种工业应用。

Conclusion: IMDD-1M数据集和相应的基础模型为工业检测和生成任务提供了数据高效的基础设施，展示了数据高效的基础模型适配在工业领域的潜力，为可扩展、领域自适应、知识驱动的制造智能铺平了道路。
...
TL;DR: 提出贝叶斯自蒸馏（BSD），一种不依赖硬标签的样本特定目标分布构建方法，通过贝叶斯推理利用模型自身预测，提升精度、校准和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统深度神经网络分类训练依赖硬标签，导致过度自信、校准差、泛化能力弱和鲁棒性不足。现有自蒸馏方法虽利用模型预测信息，但仍依赖硬标签，效果有限。

Method: 提出贝叶斯自蒸馏（BSD），通过贝叶斯推理构建样本特定的目标分布，仅初始化时使用硬标签，后续训练完全依赖模型自身预测，无需硬标签。

Result: BSD在多种深度架构和数据集上优于现有方法：ResNet-50在CIFAR-100上测试精度提升1.4%，预期校准误差降低40%；提升对数据损坏、扰动和标签噪声的鲁棒性；结合对比损失在标签噪声下达到单阶段单网络方法的最先进鲁棒性。

Conclusion: BSD是一种有效的自蒸馏方法，通过贝叶斯推理构建样本特定目标分布，显著提升模型精度、校准性和鲁棒性，尤其在标签噪声场景下表现优异。
...
TL;DR: DiffThinker：一种基于扩散的生成式多模态推理框架，将多模态推理重新定义为原生图像到图像的生成任务，在视觉中心任务中显著优于现有MLLMs。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型（MLLMs）的推理过程主要基于文本，导致在复杂长视野、视觉中心任务中表现不佳。需要一种更符合视觉本质的推理范式。

Method: 提出生成式多模态推理范式，开发DiffThinker框架，将多模态推理重新定义为原生图像到图像的生成任务，利用扩散模型实现推理过程。
...
<details>
  <summary>Details</summary>
Motivation: 高光谱成像分析面临大数据量的计算瓶颈，现有预训练基础模型在特定领域（如近距离农业监测）的迁移效果不佳，需要开发内存高效且无需预训练的解决方案。

Method: Deep Global Clustering (DGC)框架：使用重叠区域的小补丁进行操作，通过一致性约束从局部观察学习全局聚类结构，保持恒定内存使用。
...
TL;DR: 提出内部引导(IG)策略，通过在训练时引入中间层辅助监督，采样时外推中间层和深层输出来提升扩散模型生成质量，显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型难以覆盖低概率区域，标准CFG会导致样本过简化或失真，而基于坏版本的引导方法需要精心设计的退化策略、额外训练和采样步骤。
...
Conclusion: IG策略通过简单的内部引导机制，显著提升了扩散模型的训练效率和生成质量，在各种基准上都有明显改进，且与CFG结合能达到最佳性能。
...
### [43] [PointRAFT: 3D deep learning for high-throughput prediction of potato tuber weight from partial point clouds](https://arxiv.org/abs/2512.24193)
*Pieter M. Blok,Haozhou Wang,Hyun Kwon Suh,Peicheng Wang,James Burridge,Wei Guo*

Main category: cs.CV

TL;DR: 提出PointRAFT网络，直接从部分点云预测马铃薯块茎重量，无需完整三维重建，满足收获机高通量需求


<details>
  <summary>Details</summary>
Motivation: 马铃薯收获机使用RGB-D相机估计产量时，由于自遮挡导致点云不完整，系统性地低估块茎重量，需要高精度、高通量的重量预测方法

Method: 提出PointRAFT点云回归网络，引入对象高度嵌入作为几何线索，直接从部分点云预测连续三维形状属性（如重量），避免完整三维重建

Result: 在172个块茎的5,254个点云测试集上，平均绝对误差12.0g，均方根误差17.2g，推理时间6.3ms/点云，支持每秒150个块茎的处理速度

Conclusion: PointRAFT为马铃薯重量估计提供了高精度、高通量的解决方案，并可扩展到其他三维表型分析和机器人感知任务
...
### [44] [CorGi: Contribution-Guided Block-Wise Interval Caching for Training-Free Acceleration of Diffusion Transformers](https://arxiv.org/abs/2512.24195)
*Yonglak Son,Suhyeok Kim,Seungryong Kim,Young Geun Kim*

Main category: cs.CV

TL;DR: CorGi是一个无需训练、基于贡献度引导的块级间隔缓存框架，通过选择性重用DiT模型中低贡献度transformer块的输出来减少去噪步骤间的冗余计算，实现推理加速。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器(DiT)在视觉生成方面表现优异，但其迭代去噪过程结合大模型容量导致推理成本高昂。现有研究表明DiT模型的迭代去噪过程在步骤间存在大量冗余计算。

Method: 提出CorGi框架：1）基于贡献度分析识别低贡献度transformer块；2）在间隔内缓存这些块的输出并在后续步骤中重用；3）对于文本到图像任务，提出CorGi+，利用跨注意力图识别重要token并应用部分注意力更新来保护对象细节。

Result: 在最先进的DiT模型上评估显示，CorGi和CorGi+平均可实现高达2.0倍的加速，同时保持高质量生成效果。

Conclusion: CorGi和CorGi+通过减少DiT模型中的冗余计算，实现了显著的推理加速而不影响生成质量，为扩散变换器的高效部署提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分类中数据不平衡问题突出，特别是在疫情期间，COVID-19检测面临数据严重不平衡的挑战，现有AI方法因缺乏足够平衡数据而受限。
...
Conclusion: 所提模型能有效解决疫情期间医学图像分类中的数据不平衡问题，生成合成数据结合优化算法的方法在COVID-19检测中表现出优越性能。
...
### [46] [ARM: A Learnable, Plug-and-Play Module for CLIP-based Open-vocabulary Semantic Segmentation](https://arxiv.org/abs/2512.24224)
*Ziquan Liu,Zhewei Zhu,Xuyang Shi*

Main category: cs.CV

TL;DR: ARM是一个轻量级可学习模块，通过自适应融合CLIP的层次特征来提升开放词汇语义分割性能，采用"一次训练，随处使用"的范式，可作为通用即插即用后处理器。


<details>
  <summary>Details</summary>
Motivation: 现有训练免费方法要么依赖昂贵的外部基础模型（如SAM、DINO），要么使用静态手工启发式方法处理CLIP内部特征，这些方法要么计算成本高，要么效果不理想。

Method: 提出注意力精炼模块（ARM），包含语义引导的交叉注意力块（使用深层特征K、V选择和精炼细节丰富的浅层特征Q）和自注意力块，采用"一次训练，随处使用"的范式。

Result: 在多个基准测试上一致提升基线性能，推理开销可忽略不计，为训练免费的开放词汇语义分割建立了高效有效的范式。

Conclusion: ARM有效解锁和精炼了CLIP的内部潜力，作为通用即插即用后处理器，为训练免费的开放词汇语义分割提供了高效解决方案。
...
### [47] [Mirage: One-Step Video Diffusion for Photorealistic and Coherent Asset Editing in Driving Scenes](https://arxiv.org/abs/2512.24227)
...
<details>
  <summary>Details</summary>
Motivation: 现有的视频对象编辑方法在保持高视觉保真度和时间一致性方面存在困难，而视觉中心自动驾驶系统需要多样化和可扩展的训练数据来实现鲁棒性能。

Method: 基于文本到视频扩散先验，注入时间无关的2D编码器潜在特征到3D解码器以恢复细节；采用两阶段数据对齐策略（粗粒度3D对齐和细粒度2D精炼）解决分布不匹配问题。

Result: Mirage在多种编辑场景中实现了高真实感和时间一致性，并能泛化到其他视频到视频转换任务，为未来研究提供了可靠的基线。

Conclusion: Mirage通过创新的架构设计和数据对齐策略，有效解决了驾驶场景中资产编辑的视觉保真度和时间一致性问题，为自动驾驶数据增强提供了有前景的解决方案。
...
### [48] [MotivNet: Evolving Meta-Sapiens into an Emotionally Intelligent Foundation Model](https://arxiv.org/abs/2512.24231)
*Rahul Medicharla,Alper Yilmaz*

Main category: cs.CV

TL;DR: MotivNet是一个基于Meta-Sapiens基础模型的可泛化面部情绪识别模型，无需跨域训练就能在不同数据集上取得竞争性性能，解决了现有FER模型在真实世界泛化能力弱的问题。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的面部情绪识别模型在多样化数据上测试时泛化能力较弱，导致在真实世界应用中性能下降，阻碍了FER作为研究领域的发展。虽然研究者提出了复杂架构来解决泛化问题，但它们需要跨域训练才能获得可泛化的结果，这与真实世界应用存在内在矛盾。
...
Result: MotivNet在不同领域具有可泛化性，能够与现有SOTA模型进行基准比较，并满足所列标准，验证了其作为Sapiens下游任务的可行性。

Conclusion: MotivNet验证了作为Sapiens下游任务的可行性，使FER在野外应用中更具激励性，推动了面部情绪识别在真实世界应用的发展。
...
### [49] [MambaSeg: Harnessing Mamba for Accurate and Efficient Image-Event Semantic Segmentation](https://arxiv.org/abs/2512.24243)
*Fuqiang Gu,Yuanke Li,Xianlei Long,Kangping Ji,Chao Chen,Qingyi Gu,Zhenliang Ni*

Main category: cs.CV

TL;DR: MambaSeg：一种新颖的双分支语义分割框架，使用并行Mamba编码器处理RGB和事件数据，通过双维交互模块实现时空融合，在降低计算成本的同时达到SOTA性能。
...
Result: 在DDD17和DSEC数据集上的实验表明，MambaSeg实现了最先进的分割性能，同时显著降低了计算成本，展示了高效、可扩展和鲁棒的多模态感知能力。

Conclusion: MambaSeg通过有效的时空多模态融合，解决了RGB和事件数据互补性的挑战，为高效、可扩展的语义分割提供了有前景的解决方案。
...
### [50] [Physically-Grounded Manifold Projection with Foundation Priors for Metal Artifact Reduction in Dental CBCT](https://arxiv.org/abs/2512.24260)
...
<details>
  <summary>Details</summary>
Motivation: 当前CBCT金属伪影严重影响诊断，现有深度学习方法存在局限：监督方法因"回归到均值"导致光谱模糊，无监督方法可能产生结构幻觉，而扩散模型虽能生成真实图像但采样速度慢不适合临床使用

Method: 提出物理基础流形投影(PGMP)框架：1) AAPS管道通过蒙特卡洛光谱建模和患者特异性数字孪生合成高保真训练对；2) DMP-Former采用直接x预测范式，将恢复重构为确定性流形投影；3) SSA模块利用医学基础模型先验确保临床合理性

Result: 在合成和多中心临床数据集上的实验表明，PGMP在未见解剖结构上优于现有最先进方法，在效率和诊断可靠性方面设定了新基准

Conclusion: PGMP框架有效解决了CBCT金属伪影问题，通过物理模拟、确定性投影和医学先验的结合，实现了临床适用的高效高质量去伪影
...
### [51] [Taming Hallucinations: Boosting MLLMs' Video Understanding via Counterfactual Video Generation](https://arxiv.org/abs/2512.24271)
*Zhe Huang,Hao Wen,Aiming Hao,Bingze Song,Meiqi Wu,Jiahong Wu,Xiangxiang Chu,Sheng Lu,Haoqian Wang*

Main category: cs.CV

TL;DR: 提出DualityForge框架，通过可控扩散视频编辑生成反事实视频数据，构建DualityVidQA数据集，结合DNA-Train训练方法，显著减少多模态大语言模型在反事实视频上的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视频理解中过度依赖语言先验，导致在处理违背常识的反事实视频时出现视觉未接地幻觉。由于文本与视频数据不平衡，收集和标注反事实数据成本高昂，需要新的解决方案。

Method: 1. DualityForge框架：使用可控扩散视频编辑将真实视频转换为反事实场景，自动生成高质量QA对和原始-编辑视频对；2. 构建DualityVidQA数据集；3. DNA-Train训练方法：两阶段SFT-RL训练，RL阶段应用成对ℓ₁优势归一化实现更稳定的策略优化。

Result: 在DualityVidQA-Test上，方法显著减少模型在反事实视频上的幻觉，相比Qwen2.5-VL-7B基线相对提升24.0%。在幻觉和通用基准测试上都取得显著增益，显示出强大的泛化能力。

Conclusion: 提出的框架有效解决了MLLM在反事实视频上的幻觉问题，通过合成高质量反事实数据和对比训练方法，显著提升了模型的视觉接地能力和泛化性能。将开源数据集和代码。
...
TL;DR: EndoRare：基于单张参考图像生成罕见胃肠道病变图像的免重训练框架，通过语言引导的概念解耦分离病理特征与非诊断属性，提升AI分类器性能和新手内镜医师诊断能力


<details>
  <summary>Details</summary>
Motivation: 罕见胃肠道病变在常规内镜检查中不常见，导致可用于开发可靠AI模型和培训新手临床医生的数据有限，需要一种数据高效的方法来弥合罕见疾病诊断和培训的差距

Method: 提出EndoRare框架，采用一次性、免重训练生成方法，通过语言引导的概念解耦将病理特征与非诊断属性分离，将病理特征编码为可学习的原型嵌入，同时变化非诊断属性以确保多样性

Result: 在四种罕见病理（钙化纤维瘤、幼年性息肉病综合征、家族性腺瘤性息肉病、Peutz-Jeghers综合征）上验证，专家认为合成图像临床可信；用于数据增强显著提升下游AI分类器性能；盲法读者研究显示新手内镜医师接触EndoRare生成病例后召回率提高0.400，精确率提高0.267

Conclusion: EndoRare建立了一种实用、数据高效的途径，弥合了计算机辅助诊断和临床教育中罕见疾病的差距，为罕见病变的AI模型开发和临床培训提供了有效解决方案
...
TL;DR: Virtual-Eyes预处理管道能提升通用基础模型在低剂量CT肺癌筛查中的性能，但会降低专门针对原始临床数据训练的专家模型性能。


<details>
  <summary>Details</summary>
Motivation: 在低剂量CT肺癌筛查的深度学习流程中，稳健的预处理方法很少被量化评估。本研究旨在开发和验证一个临床驱动的16位CT质量控制管道，并测量其对通用基础模型与专家模型的不同影响。

Method: 开发了Virtual-Eyes预处理管道：强制512x512平面分辨率，剔除短序列或非诊断序列，通过亨氏单位过滤和双侧肺覆盖评分提取连续肺块，同时保留原生16位网格。使用765名NLST患者数据，计算RAD-DINO和Merlin的切片级嵌入，训练无泄漏的患者级MLP头，并评估Sybil和2D ResNet-18基线模型。

Result: Virtual-Eyes将RAD-DINO切片级AUC从0.576提升至0.610，患者级AUC从0.646提升至0.683（平均池化）和从0.619提升至0.735（最大池化），校准性也得到改善。相反，Sybil和ResNet-18在Virtual-Eyes下性能下降，Merlin则显示有限的迁移能力。

Conclusion: 解剖学针对性的质量控制可以稳定和改进通用基础模型工作流程，但可能破坏已适应原始临床环境的专家模型，表明预处理策略需要根据模型类型进行定制。
...
Method: UniAct采用两阶段框架：1) 通过FSQ共享离散码本统一多模态输入，实现跨模态对齐；2) 结合微调的多模态大语言模型(MLLM)和因果流式处理管道，将指令映射到物理可行的动作流形上。

Result: 在UniMoCap（20小时人形动作基准）上验证，实现低于500毫秒的延迟，零样本跟踪不完美参考动作的成功率提升19%，在多样化真实场景中展示鲁棒泛化能力。
...
TL;DR: CERES是一个用于Ego-RVOS任务的因果推理框架，通过双模态因果干预解决语言偏见和视觉混淆问题，在基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有Ego-RVOS方法存在两个主要问题：1) 从训练数据中的偏斜对象-动作配对学习虚假相关性（语言表示偏见）；2) 第一人称视角固有的视觉混淆因素（快速运动、频繁遮挡）。这些限制导致模型在真实场景中表现不佳。

Method: CERES是一个插件式因果框架，对预训练的RVOS主干网络进行双模态因果干预：1) 使用后门调整原则对抗语言表示偏见；2) 利用前门调整概念，通过因果原则指导将语义视觉特征与几何深度信息智能集成，创建对第一人称扭曲更鲁棒的表征。

Result: 在Ego-RVOS基准测试中，CERES实现了最先进的性能，证明了因果推理在构建更可靠的自我中心视频理解模型方面的潜力。

Conclusion: CERES通过因果推理框架有效解决了Ego-RVOS中的语言偏见和视觉混淆问题，为更广泛的自我中心视频理解任务提供了可靠建模方法。
...
TL;DR: SenseNova-MARS是一个多模态代理推理与搜索框架，通过强化学习让视觉语言模型具备交错视觉推理和工具使用能力，在知识密集的视觉理解任务中实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在代理推理方面主要局限于文本导向的思维链或孤立工具调用，缺乏人类般熟练地在动态工具操作和连续推理之间无缝切换的能力，特别是在需要协调外部工具（如搜索和图像裁剪）的知识密集和视觉复杂场景中。

Method: 提出SenseNova-MARS多模态代理推理与搜索框架，通过强化学习赋予视觉语言模型交错视觉推理和工具使用能力。框架动态整合图像搜索、文本搜索和图像裁剪工具，并提出了Batch-Normalized Group Sequence Policy Optimization (BN-GSPO)算法来提升训练稳定性和工具调用与推理能力。同时创建了HR-MMSearch基准测试，这是首个由高分辨率图像组成的搜索导向基准。

Result: SenseNova-MARS在开源搜索和细粒度图像理解基准测试中实现了最先进的性能。在搜索导向基准上，SenseNova-MARS-8B在MMSearch上得分为67.84，在HR-MMSearch上得分为41.64，超越了Gemini-3-Flash和GPT-5等专有模型。

Conclusion: SenseNova-MARS通过提供有效且稳健的工具使用能力，向代理视觉语言模型迈出了有希望的一步。为了促进该领域的进一步研究，作者将发布所有代码、模型和数据集。
...
TL;DR: LVLDrive是一个将LiDAR点云作为额外输入模态来增强视觉语言模型3D空间理解能力的自动驾驶框架，通过渐进融合机制保持预训练模型稳定性，显著提升场景理解和决策可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉的VLM方法在自动驾驶中存在严重瓶颈：依赖2D图像进行复杂场景理解和决策，缺乏准确的度量空间推理和几何推断能力，导致驾驶策略不可靠。需要引入3D度量数据来构建可信的VLM自动驾驶系统。

Method: 提出LVLDrive框架，通过引入LiDAR点云作为额外输入模态来增强VLM的3D空间理解能力。关键创新包括：1）渐进融合Q-Former，逐步注入LiDAR特征以避免对预训练VLM造成灾难性干扰；2）空间感知问答数据集，专门训练模型的3D感知和推理能力。

Result: 在多个驾驶基准测试中，LVLDrive相比纯视觉方法在场景理解、度量空间感知和可靠驾驶决策方面都取得了显著优越的性能，证明了3D度量数据对构建可信VLM自动驾驶系统的必要性。

Conclusion: LVLDrive成功将LiDAR点云集成到VLM中，显著提升了自动驾驶系统的3D空间理解能力。这项工作强调了显式3D度量数据对于构建可信赖的VLM自动驾驶系统的重要性，为未来多模态自动驾驶研究提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 受狭义相对论和量子力学启发，建立卷积神经网络信息处理与物理能量-动量关系的理论联系，为理解CNN中信息传播提供新的物理类比框架。

Method: 将卷积核分解为正交的偶分量和奇分量，偶分量对应各向同性扩散（类似势能），奇分量对应方向性位移（类似动能）。通过离散余弦变换在频域分析，特别关注3×3小卷积核中DC和梯度分量构成的基本信息传播模式。

Result: 信息位移速度与奇分量占总核能量的比例呈线性关系，小卷积滤波器的结构主要由低频基（DC和梯度分量）主导，这些分量定义了信息传播的基本模式。
...
TL;DR: 提出DermaVQA-DAS数据集扩展，支持皮肤病变分割和问答任务，引入DAS专家评估框架，基准测试多模态模型表现


<details>
  <summary>Details</summary>
Motivation: 现有皮肤图像分析数据集主要关注皮肤镜图像，缺乏患者查询和临床背景，限制了在患者中心护理中的应用

Method: 开发Dermatology Assessment Schema (DAS)专家框架，包含36个高级和27个细粒度评估问题；扩展DermaVQA数据集支持分割和问答任务；评估多种提示策略和多模态模型

Result: 分割任务中，不同提示策略影响性能，增强提示在多数投票评估中表现最佳；问答任务中，o3模型整体准确率最高(0.798)，GPT-4.1接近(0.796)

Conclusion: DermaVQA-DAS填补了患者中心皮肤图像分析的空白，DAS框架提供了标准化评估方法，公开数据集将加速皮肤视觉语言建模研究
...
TL;DR: 本文提出一个多模态预训练综合框架，旨在解决自动驾驶等自主系统中不同传感器（相机、LiDAR）数据融合的挑战，建立统一的空间智能理解。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆和无人机等自主系统的快速发展，迫切需要从多模态车载传感器数据中构建真正的空间智能。虽然基础模型在单模态场景表现出色，但将相机、LiDAR等不同传感器的能力整合起来形成统一理解仍然是一个重大挑战。
...
Result: 框架研究了文本输入和占用表示的集成，以促进开放世界感知和规划。识别了计算效率和模型可扩展性等关键瓶颈。

Conclusion: 提出了实现通用多模态基础模型的路线图，这些模型能够为实际部署实现鲁棒的空间智能。
...
TL;DR: RedunCut是一个动态模型大小选择系统，通过测量驱动的规划器和轻量级性能模型，在固定精度下减少14-62%的计算成本


<details>
  <summary>Details</summary>
Motivation: 实时视频分析在大规模摄像头部署中计算成本高昂，现有动态模型大小选择方法无法适应多样化工作负载（特别是移动视频和低精度目标），存在采样成本过高和精度预测不准确的问题

Method: RedunCut系统采用测量驱动的规划器来估计采样的成本效益权衡，并使用轻量级数据驱动的性能模型来改进精度预测

Result: 在道路车辆、无人机和监控视频以及多种模型家族和任务中，RedunCut在固定精度下减少14-62%的计算成本，并对有限历史数据和漂移保持鲁棒性

Conclusion: RedunCut通过改进的采样策略和精度预测机制，解决了现有动态模型大小选择方法的局限性，为大规模实时视频分析提供了高效的计算成本优化方案
...
TL;DR: DyStream：基于流匹配的自回归模型，用于实时生成逼真的双向对话头部视频，延迟低于100毫秒，实现卓越的唇形同步质量。
...
Method: 采用流友好的自回归框架，结合流匹配头部进行概率建模；提出由前瞻模块增强的因果编码器，引入少量未来上下文（60毫秒）以提升质量同时保持低延迟。

Result: 每帧生成时间34毫秒，系统总延迟低于100毫秒；在HDTF数据集上获得离线8.13和在线7.61的LipSync Confidence分数，达到最先进的唇形同步质量。

Conclusion: DyStream通过创新的因果编码器和前瞻模块设计，实现了实时高质量的双向对话头部视频生成，显著优于其他因果策略，为实时交互应用提供了可行解决方案。
...
Method: 提出AI驱动框架：1) 使用改进的TimeSformer视频Transformer架构，加入分层时间注意力和加权空间注意力机制进行动作识别；2) 使用YOLO目标检测和跟踪方法提取细粒度运动特征；3) 从五个维度评估微血管吻合技能。

Result: 在58个专家标注视频数据集上验证：动作分割帧级准确率87.7%，后处理后提升至93.62%；在五个技能维度上平均分类准确率76%，能复现专家评估。
...
Method: 引入一个框架，类比先前表示学习中的组合性测量方法，使用离散小波变换获取输入依赖的视觉基元，通过检查组合表示重构原始图像表示的能力来测试组合性。

Result: 研究发现，单层DWT分解得到的基元产生的编码器表示在潜在空间中近似组合，这为理解ViT如何结构化信息提供了新视角。

Conclusion: Vision Transformer编码器在表示空间中表现出一定的组合性特征，离散小波变换是获取视觉基元的有效工具，这有助于深入理解ViT的信息处理机制。
...
Result: 在SDOML数据集上评估，相比强基线方法，MSID降低20.15%，PSNR提升最高1.09%，log变换MS-SSIM增益1.62%，在相同比特率下获得更清晰、光谱保真的重建。
...
TL;DR: 提出基于DINOv2特征的条件扩散模型F2IDiff，用于智能手机图像超分辨率，通过低级特征提供更严格的条件约束，避免生成伪影


<details>
  <summary>Details</summary>
Motivation: 现有基于文本到图像扩散模型的超分辨率方法在智能手机摄影中存在两个问题：1）文本特征无法准确描述图像中的细微纹理；2）智能手机图像分辨率高（≥12MP），而现有模型设计用于小图像（<1MP），需要分块处理时文本特征描述不准确，容易产生不希望的幻觉伪影

Method: 提出特征到图像扩散基础模型（F2IDiff），使用DINOv2的低级特征作为条件输入，这些特征提供更严格的条件约束，同时能丰富描述小图像块

Result: 未在摘要中明确说明具体实验结果，但暗示该方法能解决智能手机图像超分辨率中的幻觉问题，提供更精确的生成

Conclusion: 通过使用低级特征（DINOv2特征）替代文本特征作为扩散模型的条件，可以更好地控制超分辨率生成过程，避免不希望的幻觉，特别适用于高保真度的智能手机图像增强
...
<details>
  <summary>Details</summary>
Motivation: 虽然AI在医学影像分析中取得了先进性能，但现有系统通常只输出结构化预测结果，需要放射科医生将其转化为完整的叙述性报告。大型语言模型的出现为弥合这一差距提供了新机会。

Method: 开发了一个集成YOLOv5和YOLOv8进行胸部X光异常检测的管道，将检测结果（边界框和类别标签）输入大型语言模型（GPT-4）生成自然语言放射学报告。比较了两个YOLO模型在检测精度和推理延迟方面的性能。

Result: YOLO模型检测结果与人工报告具有强语义相似性。人类评估显示GPT-4在清晰度方面表现出色（4.88/5），但在自然写作流畅度方面得分较低（2.81/5），表明当前系统能达到临床准确性，但在风格上与放射科医生撰写的文本仍有区别。

Conclusion: 该研究证明了结合计算机视觉和大型语言模型生成放射学报告的可行性，系统在临床准确性方面表现良好，但在自然语言生成风格方面仍需改进，以实现与放射科医生报告的无缝融合。
...
TL;DR: 提出MS-VQ-VAE模型，为低分辨率视频生成紧凑高保真潜在表示，适用于边缘设备部署和带宽敏感场景。


<details>
  <summary>Details</summary>
Motivation: 视频流量激增对带宽和存储基础设施提出更高要求，传统视频编解码器主要针对像素域重建，缺乏对机器学习潜在表示的原生支持，限制了与深度学习管道的集成。

Method: 基于VQ-VAE-2框架扩展到时空设置，引入两级分层潜在结构，使用3D残差卷积构建。模型轻量（约1850万参数），针对64x64分辨率视频片段优化，结合预训练VGG16网络的感知损失提升重建质量。

Result: 在UCF101数据集上训练，使用2秒视频片段（32帧，16FPS），测试集达到25.96 dB PSNR和0.8375 SSIM。验证集上比单尺度基线提升1.41 dB PSNR和0.0248 SSIM。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成方法在遵循物理规律方面存在不足，缺乏包含丰富物理交互的训练数据，需要显式的物理推理机制

Method: 1) PhyAugPipe数据管道：利用视觉语言模型和思维链推理构建大规模物理视频数据集PhyVidGen-135K；2) PhyGDPO框架：基于Plackett-Luce概率模型的物理感知分组直接偏好优化，包含物理引导奖励和LoRA切换参考方案

Result: 在PhyGenBench和VideoPhy2基准测试中显著优于现有开源方法，生成视频具有更好的物理一致性

Conclusion: 通过物理增强数据和物理感知优化框架，有效提升了文本到视频生成的物理真实性，为物理一致的视频生成提供了新方法
...
<details>
  <summary>Details</summary>
Motivation: 针对深度学习中的大规模优化问题，传统优化算法在收敛速度、训练稳定性和噪声鲁棒性方面存在不足，需要开发更高效的二阶优化方法。
...
TL;DR: 提出了首个面向复杂现实场景的大规模视觉定位基准RGBT-Ground，包含对齐的RGB和热红外图像对，并提出了支持多模态输入的RGBT-VGNet基线模型，在夜间和远距离场景表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位基准大多基于COCO等清洁环境数据集，场景多样性有限，无法反映真实世界中光照、天气等复杂条件的变化，难以评估模型在安全关键应用中的鲁棒性和泛化能力。

Method: 1) 构建RGBT-Ground基准，包含空间对齐的RGB和热红外图像对，高质量的指代表达式、目标边界框以及场景、环境和目标级别的细粒度标注；2) 提出统一视觉定位框架，支持单模态(RGB或TIR)和多模态(RGB-TIR)输入；3) 提出RGBT-VGNet基线模型，有效融合互补视觉模态。

Result: 实验结果表明，提出的RGBT-VGNet显著优于现有方法的适配版本，特别是在夜间和远距离场景中表现突出。所有资源将公开以促进复杂现实环境中鲁棒视觉定位的研究。

Conclusion: RGBT-Ground基准填补了复杂现实场景视觉定位评估的空白，提出的多模态框架和基线模型为鲁棒视觉定位研究提供了新方向，特别是在光照变化和恶劣天气条件下具有重要应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 研究发现CDVQA模型中大量失败案例并非源于明显错误预测，而是决策模糊性导致模型在正确答案和强干扰项之间难以区分。决策模糊样本(DAS)的存在限制了模型判别能力和鲁棒性
...
Conclusion: 通过明确优化决策模糊样本，DARFT框架有效提升了CDVQA模型的判别能力和鲁棒性，为解决视觉问答中的决策模糊问题提供了新思路
...
TL;DR: SliceLens是一个假设驱动的框架，利用LLM和VLM通过视觉推理生成和验证多样化失败假设，实现细粒度可解释错误切片的可靠识别，并在新基准FeSD上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有切片发现方法主要针对图像分类设计，不适用于检测、分割、姿态估计等多实例任务；现有方法缺乏细粒度推理能力，难以处理复杂视觉关系导致的错误切片；当前基准测试偏向特定算法或图像分类，且使用人工标注而非真实模型失败。

Method: 提出SliceLens框架：1) 利用LLM和VLM生成多样化失败假设；2) 通过基于视觉的推理验证假设；3) 实现细粒度可解释错误切片的识别。同时创建FeSD基准，包含专家标注的细粒度切片，精确关联到局部错误区域。

Result: 在现有基准和FeSD上均达到SOTA性能：在FeSD上Precision@10提升0.42（0.73 vs 0.31）；识别出的可解释切片有助于模型改进，模型修复实验验证了其有效性。

Conclusion: SliceLens通过LLM/VLM驱动的假设生成和验证，有效解决了多实例视觉任务中细粒度错误切片发现的挑战；FeSD基准为评估细粒度切片发现提供了更真实、更全面的测试平台。
...
TL;DR: 该研究构建了首个专门用于灾后评估的3D语义分割数据集，基于无人机拍摄的飓风伊恩灾后影像，并评估了现有SOTA模型在该数据集上的表现，发现现有方法在灾后场景中存在显著局限性。
...
Method: 使用无人机拍摄飓风伊恩(2022)灾区的航空影像，采用SfM和MVS技术重建3D点云，构建专门的数据集，并评估了FPT、PTv3和OA-CNNs等SOTA 3D语义分割模型在该数据集上的表现。

Result: 现有SOTA模型在灾后3D数据集上表现出显著局限性，暴露了当前方法在灾后场景理解方面的不足。
...
### [75] [Renormalization Group Guided Tensor Network Structure Search](https://arxiv.org/abs/2512.24663)
...
<details>
  <summary>Details</summary>
Motivation: 现有张量网络结构搜索方法存在三个主要问题：单尺度优化无法捕捉多尺度结构、离散搜索空间阻碍平滑结构演化、结构和参数优化分离导致计算效率低下。需要一种更高效、自适应且鲁棒的搜索方法。

Method: 提出RGTN框架，基于重整化群思想进行多尺度变换。核心创新包括：可学习的边门用于优化阶段拓扑修改，基于物理量（如节点张力测量局部应力、边信息流量化连接重要性）的智能提议机制。从低复杂度粗尺度开始，逐步细化到细尺度。

Result: 在光场数据、高阶合成张量和视频补全任务上的实验表明，RGTN实现了最先进的压缩比，运行速度比现有方法快4-600倍。

Conclusion: RGTN通过重整化群引导的多尺度搜索框架，成功解决了传统张量网络结构搜索方法的局限性，验证了物理启发方法的有效性。
...
Method: CLoRA包含两个核心组件：1）基础空间共享 - 所有低秩模块共享一组下/上投影空间，通过共享空间获得的低秩矩阵协同构建每个LRM；2）样本无关多样性增强（SADE） - 正则化这些矩阵之间的相似性，鼓励训练过程中产生多样化的表示。

Result: 在广泛使用的图像和点云数据集上的实验表明，CLoRA在学习性能和参数效率之间取得了更好的平衡，同时在点云分析中需要最少的GFLOPs，优于现有最先进方法。

Conclusion: CLoRA通过基础空间共享和多样性增强机制，成功解决了现有LoRA方法在学习性能和参数效率之间的权衡问题，为视觉Transformer微调提供了一种更平衡有效的解决方案。
...
TL;DR: 本文提出了面向户外监控场景的3D视觉定位新任务，构建了首个真实世界大规模多模态数据集MoniRefer，并提出了端到端方法Moni3DVG，在复杂交通路口实现基础设施级的3D场景理解。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位研究主要关注室内和自动驾驶场景，而户外监控场景由于缺乏路边基础设施传感器采集的配对点云-文本数据，一直未被探索。基础设施系统需要理解自然语言并在复杂交通环境中定位目标物体。

Method: 提出了Moni3DVG方法，利用图像提供的丰富外观信息以及点云的几何和光学信息进行多模态特征学习和3D物体定位。构建了MoniRefer数据集，包含约136,018个物体和411,128个自然语言描述，来自真实世界复杂交通路口。

Result: 在提出的基准测试上进行了大量实验和消融研究，证明了方法的优越性和有效性。数据集包含高质量的手动验证的语言描述和3D标签。

Conclusion: 本文开创了户外监控场景的3D视觉定位新任务，提供了首个大规模真实世界数据集和有效的端到端方法，为基础设施级交通场景理解奠定了基础。
...
### [78] [Evolving, Not Training: Zero-Shot Reasoning Segmentation via Evolutionary Prompting](https://arxiv.org/abs/2512.24702)
*Kai Ye,Xiaotong You,Jianghang Lin,Jiayi Ji,Pingyang Dai,Liujuan Cao*

Main category: cs.CV

TL;DR: EVOL-SAM3：一个零样本推理分割框架，通过推理时的进化搜索过程，使用"生成-评估-进化"循环迭代优化提示假设，显著超越现有静态方法和有监督方法。


<details>
  <summary>Details</summary>
Motivation: 当前推理分割方法存在严重缺陷：有监督微调（SFT）会导致灾难性遗忘和领域依赖；强化学习（RL）训练不稳定且依赖预定义奖励函数；训练免费方法采用静态推理范式，缺乏深度推理和自我纠正能力。

Method: 提出EVOL-SAM3框架，将推理分割重新定义为推理时的进化搜索过程。采用"生成-评估-进化"循环：1）维护提示假设种群；2）通过无参考成对竞赛的视觉竞技场评估提示适应性；3）使用语义变异算子注入多样性和纠正语义错误；4）异构竞技场模块整合几何先验和语义推理进行最终选择。

Result: 在挑战性的ReasonSeg基准测试中，EVOL-SAM3不仅大幅超越静态基线方法，还在零样本设置下显著超过完全有监督的最先进方法。

Conclusion: EVOL-SAM3通过推理时的进化搜索范式，成功解决了现有推理分割方法的局限性，实现了更强大的零样本推理分割能力，为复杂语言查询的像素级定位提供了新思路。
...
TL;DR: 提出Layer-by-Layer Hierarchical Attention Network，通过分层注意力机制和阶段融合解决特征点匹配中的离群值问题，提高匹配精度和相机姿态估计性能。


<details>
  <summary>Details</summary>
Motivation: 特征点匹配是计算机视觉的基础任务，但大量离群值的存在严重影响匹配结果的准确性和鲁棒性。主要挑战在于如何在离群值比例较高的情况下，既能提取高质量信息，又能减少负样本带来的误差。

Method: 提出Layer-by-Layer Hierarchical Attention Network方法，包含：1）逐层通道融合模块，保留各阶段特征语义信息并实现整体融合；2）分层注意力模块，通过注意力机制自适应捕获和融合全局感知与结构语义信息；3）两种架构用于特征提取和集成，提高网络适应性。

Result: 在YFCC100M和SUN3D两个公开数据集上的实验表明，该方法在离群值去除和相机姿态估计方面均优于多种最先进技术。

Conclusion: 提出的分层注意力网络通过有效的特征融合和注意力机制，显著提升了特征点匹配的精度，特别是在处理高比例离群值场景时表现出色，为计算机视觉中的特征匹配问题提供了有效解决方案。
...
### [80] [FireRescue: A UAV-Based Dataset and Enhanced YOLO Model for Object Detection in Fire Rescue Scenes](https://arxiv.org/abs/2512.24622)
*Qingyu Xu,Runtong Zhang,Zihuan Qiu,Fanman Meng*

Main category: cs.CV

TL;DR: 本文针对消防救援场景目标检测的局限性，构建了FireRescue数据集并提出FRS-YOLO改进模型，通过注意力模块和动态特征采样器提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有消防救援目标检测研究存在两个主要问题：1）主要关注山地、森林环境，对更频繁且结构复杂的城市救援场景关注不足；2）现有检测系统类别有限（如火焰、烟雾），缺乏覆盖消防车、消防员等指挥决策关键目标的全面系统。

Method: 1）构建FireRescue新数据集，涵盖城市、山地、森林、水域多种救援场景，包含8个关键类别（如消防车、消防员），共15,980张图像和32,000个边界框；2）提出FRS-YOLO改进模型：引入即插即用的多维协同增强注意力模块，通过跨维度特征交互增强易混淆类别的判别表示；集成动态特征采样器，强化高响应前景特征以减轻烟雾遮挡和背景干扰影响。

Result: 实验结果表明，消防救援场景中的目标检测具有高度挑战性，所提出的方法有效提升了YOLO系列模型在该场景下的检测性能。

Conclusion: 本文通过构建全面数据集和改进检测模型，解决了消防救援场景目标检测的现有局限性，为消防指挥决策提供了更有效的技术支持。
...
### [81] [Video and Language Alignment in 2D Systems for 3D Multi-object Scenes with Multi-Information Derivative-Free Control](https://arxiv.org/abs/2512.24826)
...
Method: 通过无导数优化的遗憾最小化改进多元互信息估计，利用表达性度量和基于价值的优化来控制场景内相机，直接从视觉语言模型的噪声输出中学习

Result: 该方法使现成的2D视觉跨模态系统能够在线适应物体遮挡和区分特征，在多物体3D场景的跨模态任务中提高性能，无需预训练或微调

Conclusion: 提出的管道通过改进的互信息估计和相机控制，成功解决了2D到3D的维度转换问题，为跨模态系统在3D场景中的应用提供了有效解决方案
...
Method: 提出RadAR框架：1）采用径向拓扑组织生成过程：选择初始token作为起点，将所有其他token按空间距离分组为多个同心环；2）环状生成：从内环到外环逐环生成，同一环内所有token可并行预测；3）嵌套注意力机制：动态修正前向传播中的不合理输出，缓解错误累积和模型崩溃。

Result: 通过径向并行预测与动态输出修正的结合，RadAR在保持视觉场景结构局部性和空间连贯性的同时，显著提高了生成效率。

Conclusion: RadAR通过创新的径向拓扑设计和嵌套注意力机制，成功解决了传统自回归视觉生成的低效问题，实现了高效且高质量的并行生成。
...
<details>
  <summary>Details</summary>
Motivation: 研究发现模型容量在不同时间步的影响不同：在早期和晚期阶段至关重要，但在中间阶段基本可以忽略。这为混合使用不同容量模型提供了机会，以加速推理而不损失质量。

Method: 提出FlowBlending策略：1）使用大模型处理容量敏感阶段（早期和晚期），2）使用小模型处理中间阶段，3）引入简单标准选择阶段边界，4）提供速度-散度分析作为识别容量敏感区域的有效代理。

Result: 在LTX-Video（2B/13B）和WAN 2.1（1.3B/14B）上，FlowBlending实现高达1.65倍推理加速，减少57.35%的FLOPs，同时保持大模型的视觉保真度、时间连贯性和语义对齐。与现有采样加速技术兼容，可实现额外2倍加速。

Conclusion: FlowBlending通过阶段感知的多模型采样策略，有效利用模型容量在不同时间步的敏感性差异，在保持生成质量的同时显著加速扩散模型推理，为高效视频生成提供了实用解决方案。
...
Method: 1) 统一离散-连续动作：在共享模型中集成离散点击和连续拖拽；2) 基于流的动作生成：通过轻量级动作专家从连续视觉观察预测增量光标调整；3) 拖拽训练数据和基准：手动收集合成20K拖拽轨迹，创建ScreenDrag基准

Result: 在ScreenDrag基准上，ShowUI-π仅用4.5亿参数就达到26.98分，显著优于现有GUI代理（Operator 13.27分，最佳Gemini-2.5-CUA 22.18分），证明了方法的有效性

Conclusion: ShowUI-π通过统一离散-连续动作建模和基于流的生成方法，显著提升了GUI代理的灵巧操作能力，推动了数字世界中类人灵巧控制的发展
...
<details>
  <summary>Details</summary>
Motivation: 现有视频-文本到音频（VT2A）方法存在三个关键限制：1）视觉与文本条件不平衡导致视觉主导；2）缺乏细粒度可控生成的具体定义；3）指令理解能力弱，现有数据集依赖简短分类标签。

Method: 提出EchoFoley任务，使用符号化声音事件表示（指定何时、什么、如何产生声音），构建EchoFoley-6k数据集（6000+视频-指令-标注三元组），开发EchoVidia框架，采用慢-快思维策略的以声音事件为中心的智能生成方法。

Result: EchoVidia在可控性上比现有VT2A模型提升40.7%，在感知质量上提升12.5%，实现了细粒度的声音生成、插入和编辑控制。

Conclusion: EchoFoley任务和EchoVidia框架解决了现有VT2A方法的局限性，通过符号化事件表示和分层语义控制，实现了更精确、可控的视频声音生成。
...
TL;DR: 该论文全面评估了量化、剪枝和权重聚类等模型压缩技术对卷积神经网络鲁棒性的影响，发现某些压缩策略不仅能保持甚至能提高模型在自然损坏环境下的鲁棒性，特别是对于复杂架构的网络。


<details>
  <summary>Details</summary>
Motivation: 在资源受限设备上部署计算机视觉系统需要压缩深度学习模型，但模型压缩可能影响鲁棒性，尤其是在自然损坏环境下。因此，在验证计算机视觉系统时考虑鲁棒性评估非常重要。

Method: 对ResNet-50、VGG-19和MobileNetV2等卷积神经网络应用量化、剪枝和权重聚类等压缩技术（单独和组合使用），使用CIFAR-10-C和CIFAR-100-C数据集分析鲁棒性、准确性和压缩率之间的权衡关系。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术的快速发展导致了对标准化评估工具的迫切需求，特别是在压缩任务方面。现有基准测试往往缺乏全面评估不同方法独特特性（如渲染速度、率失真权衡、内存效率和几何精度）的特定指标。

Method: 开发了Splatwizard统一基准工具包，提供易于使用的框架来实施新的3DGS压缩模型并利用现有最先进技术。包含自动化计算关键性能指标的集成管道，包括图像质量指标、重建网格的倒角距离、渲染帧率和计算资源消耗。

Result: 提出了一个开源工具包，代码已在GitHub上公开。该框架能够全面评估3DGS压缩模型的多个维度性能，为研究社区提供了标准化的评估平台。

Conclusion: Splatwizard填补了3D高斯泼溅压缩模型评估领域的空白，为研究人员提供了统一、全面的基准测试工具，有助于推动该领域的发展。
...
TL;DR: DarkEQA是一个开源基准测试，用于评估视觉语言模型在低光照条件下的感知能力，通过模拟物理真实的视觉退化来测试EQA相关任务。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要在理想光照条件下评估视觉语言模型，但实际24/7运行的具身智能体需要在各种视觉退化条件下工作，特别是低光照环境，这一核心需求被严重忽视。

Method: DarkEQA通过模拟物理真实的视觉退化来评估EQA相关感知能力：在RAW线性空间模拟基于物理的照明下降和传感器噪声，然后通过ISP启发的渲染管道处理，从而隔离感知瓶颈。

Result: 评估了多种最先进的视觉语言模型和低光照图像增强模型，系统揭示了这些模型在挑战性视觉条件下的局限性。

Conclusion: DarkEQA填补了低光照条件下视觉语言模型评估的空白，为可归因的鲁棒性分析提供了基准，代码和数据集将在接受后发布。
...
TL;DR: 提出统一框架，将3D高斯分割中的特征学习和标签解码合并，通过可学习嵌入和"嵌入到标签"过程提升性能，并引入边界硬挖掘解决边界伪影问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D实例分割方法面临多视角2D标签不一致的问题，导致3D预测质量差。现有方法要么采用两阶段方法（对比学习+聚类），要么预处理标签，训练时间长且性能受限。

Method: 提出统一框架：1）为高斯基元引入可学习特征嵌入；2）通过新颖的"嵌入到标签"过程高效解码为实例标签；3）针对边界伪影问题，提出边界硬挖掘，在线性层后计算三元组损失以稳定训练。

Result: 在ScanNet、Replica3D和Messy-Rooms数据集上，方法在定性和定量评估中均优于基线方法。

Conclusion: 提出的统一框架将特征学习和标签解码合并，减少了训练时间并提升了性能，同时通过边界硬挖掘有效解决了边界伪影问题，为3D场景理解提供了更高效的解决方案。
...
TL;DR: 提出一种基于投影的对抗攻击方法，通过将扰动光线投影到目标物体上，使基于DNN的单目深度估计模型产生深度误判
...
Result: 实验证实该方法成功创建了导致深度误判的对抗样本，使目标场景中的部分物体消失

Conclusion: 验证了基于DNN的单目深度估计模型对物理对抗攻击的脆弱性，强调了在实际应用中增强鲁棒性的重要性
...
TL;DR: SpaceTimePilot是一个视频扩散模型，能够解耦空间和时间控制，实现可控的生成式渲染，允许独立改变摄像机视角和运动序列。
...
Method: 1. 引入动画时间嵌入机制，在扩散过程中显式控制输出视频的运动序列；2. 提出时间扭曲训练方案，利用多视角数据集模拟时间差异；3. 改进摄像机条件机制，允许从第一帧开始改变视角；4. 创建CamxTime合成数据集，提供完全自由的空间-时间视频轨迹。

Result: 在真实世界和合成数据上的评估表明，SpaceTimePilot能够实现清晰的空间-时间解耦，相比先前工作取得了更强的结果。

Conclusion: SpaceTimePilot通过创新的时间嵌入机制、训练方案和数据集，成功实现了视频生成中空间和时间的解耦控制，为可控生成式渲染提供了新的解决方案。
...
### [92] [Nonlinear Noise2Noise for Efficient Monte Carlo Denoiser Training](https://arxiv.org/abs/2512.24794)
...
<details>
  <summary>Details</summary>
Motivation: Noise2Noise方法虽然能使用噪声图像对进行训练，但非线性函数会引入偏差，限制了预处理操作。特别是在HDR图像去噪中，异常值会严重影响训练效果，而常用的色调映射非线性函数被认为与Noise2Noise训练不兼容。

Method: 建立理论框架分析非线性函数的影响，识别出一类引入最小偏差的非线性函数。针对HDR图像去噪，研究特定损失函数与色调映射函数的组合，在减少动态范围的同时最小化偏差。

Result: 方法应用于现有的基于机器学习的蒙特卡洛去噪器，仅使用噪声训练数据就能获得接近原始实现（使用高样本参考图像训练）的结果，成功解决了HDR图像训练中的异常值问题。

Conclusion: 证明了某些非线性函数可以与Noise2Noise训练兼容，扩展了该方法的应用范围，特别是在HDR图像处理中，为仅使用噪声数据训练高质量去噪器提供了新途径。
...
TL;DR: CropTrack是一个用于农业场景的多目标跟踪框架，通过结合外观和运动信息，解决重复模式、相似外观、光照变化和频繁遮挡等挑战，在身份保持方面优于传统运动跟踪方法。


<details>
  <summary>Details</summary>
Motivation: 农业环境中的多目标跟踪面临重复模式、相似物体外观、突然光照变化和频繁遮挡等挑战。现有跟踪器主要依赖运动信息，但在频繁强遮挡时难以保持目标身份，而外观相似性使得外观关联在农业场景中难以应用。

Method: CropTrack提出一个结合外观和运动信息的MOT框架，包含：1）重排序增强的外观关联；2）一对多关联与基于外观的冲突解决策略；3）指数移动平均原型特征库来改进外观关联。

Result: 在公开农业MOT数据集上评估，CropTrack展现出持续的身份保持能力，优于传统运动跟踪方法。与最先进方法相比，在识别F1分数和关联准确率上取得显著提升，同时身份切换次数更低。

Conclusion: CropTrack通过有效结合外观和运动信息，解决了农业场景中多目标跟踪的身份保持问题，特别是在频繁遮挡情况下表现出色，为农业MOT提供了更可靠的解决方案。
...
TL;DR: 本文提出了VLN-MME评估框架，用于系统评估多模态大语言模型在具身导航任务中的零样本表现，发现MLLMs在3D空间推理方面存在局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在各种视觉语言任务中表现出色，但其作为具身智能体（需要多轮对话、空间推理和序列动作预测）的性能仍需深入探索。本文旨在研究MLLMs在视觉语言导航任务中的潜力。

Method: 提出了VLN-MME评估框架，将传统导航数据集标准化为统一基准，采用高度模块化和易访问的设计简化评估。该框架支持对不同MLLM架构、智能体设计和导航任务进行结构化比较和组件级消融实验。

Result: 研究发现，增强基线智能体使用思维链推理和自反思反而导致性能下降，这表明MLLMs在具身导航任务中上下文感知能力较差。虽然MLLMs能够遵循指令并结构化输出，但其3D空间推理保真度较低。

Conclusion: VLN-MME为系统评估通用MLLMs在具身导航设置中的表现奠定了基础，并揭示了其在序列决策能力方面的局限性。这些发现为MLLMs作为具身智能体的后训练提供了重要指导。
...
TL;DR: OFL-SAM2是一个无需手动提示的SAM2框架，通过轻量级映射网络和在线学习，在有限标注数据下实现高效的医学图像分割。


<details>
  <summary>Details</summary>
Motivation: SAM2在医学图像分割中存在两个主要挑战：1）需要大量标注数据进行微调；2）需要高质量手动提示，这两者都需要医学专家参与且劳动密集。需要开发一种无需提示且标注高效的医学图像分割方法。

Method: 提出OFL-SAM2框架，包含两个核心组件：1）在线少样本学习器，训练轻量级映射网络生成目标特征；2）自适应融合模块，动态整合目标特征与SAM2的注意力特征。映射网络支持推理时在线参数更新，增强泛化能力。

Result: 在三个不同的医学图像分割数据集上进行广泛实验，OFL-SAM2在有限训练数据下实现了最先进的性能。

Conclusion: OFL-SAM2成功解决了SAM2在医学图像分割中的挑战，通过无需手动提示的框架和在线学习机制，在标注数据有限的情况下实现了高效准确的医学图像分割。
...
TL;DR: FinMMDocR是一个用于评估多模态大语言模型在真实世界金融数值推理任务上的双语多模态基准，包含1200个专家标注的问题，涵盖12种金融场景和837份中英文文档，要求平均11步推理，最佳模型准确率仅58.0%。


<details>
  <summary>Details</summary>
Motivation: 现有基准在金融领域的多模态推理评估存在不足，缺乏真实世界的金融场景、复杂的文档理解和多步推理要求，需要更贴近实际金融应用场景的评估基准。

Method: 构建包含1200个专家标注问题的双语多模态基准，涵盖12种金融场景，使用837份平均50.8页的中英文金融文档（9种类型），设计需要平均11步推理（5.3步信息提取+5.7步计算）的问题，其中65.0%需要跨页证据（平均2.4页）。

Result: 最佳性能的MLLM仅达到58.0%的准确率，不同的检索增强生成方法在该任务上表现出显著性能差异，表明当前模型在复杂金融多模态推理方面仍有很大提升空间。

Conclusion: FinMMDocR基准填补了金融多模态推理评估的空白，能够有效驱动MLLM和推理增强方法在真实世界复杂多模态推理任务上的改进。
...
TL;DR: 本文提出了一种基于神经元激活模式的激光雷达域自适应方法，仅需标注目标域中少量代表性样本即可实现SOTA性能，结合持续学习技术防止权重漂移。


<details>
  <summary>Details</summary>
Motivation: 3D物体检测器在自动驾驶感知系统中至关重要，但在不同地理区域（如美国、亚洲、欧洲）之间存在显著的域泛化问题。现有模型在一个区域训练后，在其他区域性能大幅下降，需要解决跨域适应问题。

Method: 提出基于神经元激活模式的激光雷达域自适应方法：1）从目标域中选取少量代表性、多样性的样本进行标注；2）结合持续学习启发的后训练技术，防止原始模型权重漂移；3）通过神经元激活模式指导样本选择，最大化标注效率。

Result: 实验评估表明，该方法在线性探测和现有最先进的域自适应技术中表现最优，仅需极小的标注预算即可实现卓越性能。

Conclusion: 通过神经元激活模式指导的样本选择和持续学习技术，可以在极低标注成本下有效解决激光雷达3D检测器的跨域适应问题，为自动驾驶系统在不同地理区域的部署提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有作物病害诊断方法依赖昂贵的监督微调，且在领域迁移时表现不佳，需要更准确、可解释且无需训练的解决方案。

Method: 提出Caption-Prompt-Judge框架：1) 使用大视觉语言模型生成多角度图像描述；2) 通过LLM-as-Judge模块迭代优化描述；3) 基于优化描述进行双答案VQA（识别和管理响应）。
...
<details>
  <summary>Details</summary>
Motivation: 非门控胸部CT中冠状动脉钙化评分常受心脏和呼吸运动伪影影响，而ECG门控心脏CT在人群筛查和常规成像中应用有限。需要一种方法从常规非门控CT中准确量化钙化病灶。

Method: 提出ProDM（属性感知渐进校正扩散模型），包含三个关键组件：1）CAC运动模拟数据引擎，从心脏门控CT合成真实非门控采集数据；2）属性感知学习策略，通过可微分钙化一致性损失保留病灶完整性；3）渐进校正方案，在扩散步骤中逐步减少伪影。

Result: 在真实患者数据集上，ProDM显著提高了CAC评分准确性、空间病灶保真度和风险分层性能。读者研究证实ProDM能抑制运动伪影并提高临床可用性。

Conclusion: ProDM展示了渐进、属性感知框架在从常规胸部CT成像中进行可靠CAC量化的潜力，为心血管疾病风险评估提供了更易获得的替代方案。
...
TL;DR: VIPER：首个面向生成式视频推理的过程感知评估基准，提出过程-结果一致性指标POC@r，发现当前视频模型存在约20%的POC@1.0得分和显著的结果欺骗问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型展现出链式帧推理能力，但现有评估框架多依赖单帧评估，容易导致结果欺骗——模型通过错误过程得到正确结论。需要过程感知的评估范式来准确衡量生成式视频推理能力。

Method: 提出VIPER基准，涵盖16个任务，覆盖时间、结构、符号、空间、物理和规划推理。提出POC@r指标，使用VLM-as-Judge配合分层评分标准，同时评估中间步骤有效性和最终结果。

Result: 实验显示最先进的视频模型仅达到约20%的POC@1.0得分，存在显著的结果欺骗现象。测试时缩放和采样鲁棒性分析揭示了当前视频生成与真正广义视觉推理之间的巨大差距。

Conclusion: 需要过程感知评估来准确衡量生成式视频推理能力，VIPER基准和POC@r指标为此提供了系统框架。当前视频模型在过程一致性方面表现有限，存在结果欺骗问题，距离真正的广义视觉推理仍有较大差距。
...
TL;DR: 提出RFL-ReID新任务：无需重新索引历史图库图像即可进行终身行人重识别，解决数据隐私和高成本问题，并提出Bi-C2R框架实现特征兼容性。


<details>
  <summary>Details</summary>
Motivation: 传统终身行人重识别方法需要每次更新后重新提取所有历史图库特征（重新索引），但面临数据隐私问题和计算成本高的挑战，导致新旧模型特征不兼容，严重影响重识别性能。

Method: 提出双向连续兼容表示（Bi-C2R）框架，通过持续更新旧模型提取的图库特征，实现新旧模型特征的兼容性，从而在无需重新索引的情况下进行高效终身行人重识别。

Result: 通过理论分析和多个基准测试的广泛实验验证，Bi-C2R方法在提出的RFL-ReID任务和传统L-ReID任务上均取得领先性能。

Conclusion: RFL-ReID是比传统L-ReID更具挑战性的新任务，Bi-C2R框架有效解决了无需重新索引的终身行人重识别问题，实现了新旧模型特征的兼容性，为实际应用提供了可行方案。
...
TL;DR: FoundationSLAM是一个基于学习的单目稠密SLAM系统，通过结合基础深度模型指导，将光流估计与几何推理相结合，实现实时高精度跟踪和建图。


<details>
  <summary>Details</summary>
Motivation: 解决先前基于光流的SLAM方法中缺乏几何一致性的问题，以实现更准确和鲁棒的跟踪与建图。
...
Conclusion: FoundationSLAM成功地将基础深度模型指导与几何推理相结合，为单目稠密SLAM提供了准确、鲁棒且实用的解决方案。
...
TL;DR: 提出自引导框架，将视觉配音从不适定修复任务重构为条件良好的视频编辑问题，使用DiT生成对齐训练数据，实现精准唇部同步和身份保持


<details>
  <summary>Details</summary>
Motivation: 现有基于掩码修复的方法因缺乏理想训练数据（仅唇部运动不同而其他视觉条件完全相同的配对视频）而面临挑战，导致视觉伪影、身份漂移和同步不良

Method: 1) 使用Diffusion Transformer作为数据生成器，为每个真实样本合成唇部对齐的伴生视频，形成视觉对齐的视频对；2) 在合成数据上端到端训练基于DiT的音频驱动编辑器；3) 引入时间步自适应多阶段学习策略；4) 提出ContextDubBench评估基准

Result: 该方法实现了高度准确的唇部同步、忠实的身份保持，并在具有挑战性的野外场景中表现出卓越的鲁棒性

Conclusion: 通过将视觉配音重构为条件良好的视频编辑问题，并利用自引导框架生成对齐训练数据，该方法克服了现有方法的局限性，实现了高质量的音频驱动视觉配音
...
Method: 1) 上下文感知补全恢复基础骨架序列；2) 空间分解模块将骨架分为5个语义区域，基于运动方差分为动态/静态子组，生成增强序列；3) 物理驱动估计模块使用拉格朗日动力学估计关节加速度；4) 融合骨架位置序列和加速度序列输入GCN识别头。
...
TL;DR: Edit3r是一个前馈框架，能够从无姿态、视角不一致的指令编辑图像中单次前向传播重建和编辑3D场景，无需逐场景优化或姿态估计，实现快速逼真渲染。


<details>
  <summary>Details</summary>
Motivation: 现有3D编辑方法通常需要逐场景优化，计算成本高且速度慢。同时，训练这类模型面临缺乏多视角一致编辑图像监督的挑战，需要解决跨视角一致性的监督问题。

Method: 1) 使用SAM2-based重新着色策略生成可靠的跨视角一致监督；2) 采用非对称输入策略，将重新着色的参考视图与原始辅助视图配对，鼓励网络融合和对齐不同的观察；3) 构建DL3DV-Edit-Bench基准进行大规模定量评估。

Result: Edit3r在语义对齐和3D一致性方面优于现有基线方法，同时推理速度显著更快。模型能够有效处理训练中未接触过的2D编辑方法（如InstructPix2Pix）生成的图像。

Conclusion: Edit3r实现了快速、逼真的3D场景编辑，无需优化或姿态估计，在语义对齐和3D一致性方面表现优异，推理速度快，有望应用于实时3D编辑应用。
...
TL;DR: GaMO通过多视角外推而非生成新视角来改进稀疏视图3D重建，保持几何一致性并实现25倍加速


<details>
  <summary>Details</summary>
Motivation: 现有扩散方法在稀疏视图3D重建中存在三个关键问题：已知视角外围覆盖不足、生成视图间几何不一致、计算流程昂贵

Method: 提出几何感知多视角外推框架GaMO，从现有相机位姿扩展视野而非生成新视角，采用多视角条件和几何感知去噪策略，无需训练

Result: 在Replica和ScanNet++数据集上，3、6、9个输入视图下均达到SOTA重建质量，PSNR和LPIPS指标优于先前方法，处理时间10分钟内，比SOTA扩散方法快25倍

Conclusion: GaMO通过多视角外推重新定义稀疏视图重建，在保持几何一致性的同时提供更广场景覆盖，显著提升效率和质量
...
TL;DR: 论文提出DDFT测试协议，用于评估语言模型在语义压缩和对抗性干扰下的认知鲁棒性，发现模型大小与鲁棒性无关，错误检测能力是关键瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型评估（如MMLU、TruthfulQA）只能测量理想条件下的知识，无法评估模型在现实压力下的鲁棒性。这些静态基准无法区分模型是缺乏知识，还是在信息退化或对抗性攻击时验证机制崩溃。

Method: 引入Drill-Down and Fabricate Test (DDFT)协议，测量认知鲁棒性：模型在渐进语义压缩和对抗性伪造下保持事实准确性的能力。提出两系统认知模型：语义系统生成流畅文本，认知验证器验证事实准确性。评估了9个前沿模型在8个知识领域和5个压缩级别（共1800次轮级评估）。

Result: 认知鲁棒性与传统设计范式正交。参数数量(r=0.083, p=0.832)和架构类型(r=0.153, p=0.695)均不能显著预测鲁棒性，表明鲁棒性源于训练方法和验证机制。错误检测能力强烈预测整体鲁棒性(rho=-0.817, p=0.007)，是关键瓶颈。旗舰模型尽管规模大但表现出脆弱性，而较小模型可以实现鲁棒性能。

Conclusion: DDFT框架为评估认知鲁棒性提供了理论基础和实用工具，有助于在关键应用部署前评估模型可靠性。挑战了模型大小与可靠性关系的假设，表明鲁棒性更多取决于训练方法和验证机制而非规模。
...
### [108] [CASCADE: Cumulative Agentic Skill Creation through Autonomous Development and Evolution](https://arxiv.org/abs/2512.23880)
*Xu Huang,Junwu Chen,Yuxing Fei,Zhuohan Li,Philippe Schwaller,Gerbrand Ceder*

Main category: cs.AI

TL;DR: CASCADE是一个自进化的LLM智能体框架，通过持续学习和自我反思等元技能掌握复杂外部工具，在材料科学和化学研究任务上达到93.3%的成功率，推动从"LLM+工具使用"向"LLM+技能获取"的转变。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体依赖预定义工具或脆弱的工具生成，限制了其在复杂科学任务中的能力和适应性。需要从"LLM+工具使用"向"LLM+技能获取"转变，使智能体能够自主掌握外部工具并积累可执行技能。

Method: CASCADE框架通过两种元技能实现自我进化：1) 持续学习（通过网页搜索和代码提取）；2) 自我反思（通过内省和知识图谱探索）。该框架支持人类-智能体协作和记忆巩固，能够积累可共享的可执行技能。

Result: 在SciSkillBench基准测试（116个材料科学和化学研究任务）中，CASCADE使用GPT-5达到93.3%的成功率，相比没有进化机制的35.4%有显著提升。框架在计算分析、自主实验室实验和论文选择性复现等实际应用中表现出色。

Conclusion: CASCADE通过自我进化机制使LLM智能体能够掌握复杂工具并积累可共享技能，显著提升了科学研究的成功率和适应性，为实现可扩展的AI辅助科学研究迈出了重要一步。
...
### [109] [A Proof-of-Concept for Explainable Disease Diagnosis Using Large Language Models and Answer Set Programming](https://arxiv.org/abs/2512.23932)
*Ioanna Gemou,Evangelos Lamprou*

Main category: cs.AI

TL;DR: McCoy框架结合大语言模型与答案集编程，通过LLM将医学文献转化为ASP代码，结合患者数据生成可解释的诊断预测系统


<details>
  <summary>Details</summary>
Motivation: 准确的疾病预测对及时干预和有效治疗至关重要。虽然符号AI已应用于医疗领域，但构建高质量知识库所需的大量工作限制了其广泛应用。需要一种方法能自动从医学文献中提取知识，同时保持系统的可解释性。

Method: McCoy框架结合大语言模型与答案集编程：1) 使用LLM将医学文献翻译成ASP代码；2) 将生成的ASP代码与患者数据结合；3) 使用ASP求解器处理数据得出最终诊断。这种集成利用了两种范式的优势。

Result: 初步结果显示，McCoy在小规模疾病诊断任务上表现出色，展示了该框架在医疗预测领域的潜力。

Conclusion: McCoy框架成功克服了符号AI在医疗领域应用的主要障碍，通过LLM自动构建知识库，同时保持了系统的可解释性和鲁棒性，为医疗诊断提供了新的解决方案。
...
### [110] [SPARK: Search Personalization via Agent-Driven Retrieval and Knowledge-sharing](https://arxiv.org/abs/2512.24008)
...
<details>
  <summary>Details</summary>
Motivation: 传统个性化搜索系统受限于静态用户画像和单一检索流程，难以捕捉用户动态、多维的信息需求。需要一种能够建模人类信息寻求行为复杂性、流动性和上下文敏感性的新一代搜索系统。

Method: 1. 定义由角色、专业知识、任务上下文和领域构成的人物空间；2. 引入人物协调器动态解析查询并激活最相关的专业智能体；3. 每个智能体执行独立的检索增强生成过程，配备长短期记忆存储和上下文感知推理模块；4. 通过共享内存库、迭代辩论和中继式知识转移等结构化通信协议促进智能体协作。

Result: 框架产生了关于协调效率、个性化质量和认知负载分布的可测试预测，同时包含自适应学习机制用于持续人物优化。通过分布式智能体行为和最小协调规则，实现了涌现的个性化特性。
...
TL;DR: ROAD是一种无需标注数据集的自动提示优化框架，通过多智能体架构将失败日志转化为决策树协议，在冷启动场景下实现高效优化。


<details>
  <summary>Details</summary>
Motivation: 现实软件工程中，LLM代理开发初期通常缺乏标注数据集，只有混乱的生产日志和不断变化的失败模式，传统基于标注数据的自动提示优化方法不适用。

Method: ROAD采用多智能体架构：分析器进行根因分析，优化器进行模式聚合，教练进行策略整合，将非结构化失败日志转化为结构化的决策树协议。

Result: 在学术基准和生产知识管理引擎上的实验显示，ROAD在3次自动迭代中成功率提升5.6%，搜索准确率提升3.8%，在零售领域复杂推理任务上性能提升约19%。

Conclusion: 模拟人类工程师的失败分析和修复循环为部署可靠LLM代理提供了数据高效、资源节约的替代方案，避免了资源密集的强化学习训练。
...
TL;DR: LoongFlow是一个自进化代理框架，通过认知性的"计划-执行-总结"范式将LLM集成到进化搜索中，显著提高了解决方案质量和计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统进化方法在从静态LLM向自改进代理过渡时存在结构化推理不足的问题，容易出现早熟收敛和高维代码空间探索效率低下的挑战。

Method: 1) 集成LLM到"计划-执行-总结"认知范式；2) 采用混合进化记忆系统，结合多岛模型、MAP-Elites和自适应玻尔兹曼选择；3) 实例化为通用代理和机器学习代理。

Result: 在AlphaEvolve基准测试和Kaggle竞赛中，LoongFlow比OpenEvolve、ShinkaEvolve等基线方法在进化效率上提升高达60%，并发现更优解决方案。

Conclusion: LoongFlow在自主科学发现领域迈出了重要一步，能够以更低的计算开销生成专家级解决方案，平衡了探索与利用的权衡。
...
### [113] [CogRec: A Cognitive Recommender Agent Fusing Large Language Models and Soar for Explainable Recommendation](https://arxiv.org/abs/2512.24113)
*Jiaxin Hu,Tao Wang,Bingsan Yang,Hongrun Wang*

Main category: cs.AI

TL;DR: CogRec是一个结合大语言模型和Soar认知架构的推荐系统，利用LLM进行知识初始化，通过Soar进行符号推理，在遇到瓶颈时动态查询LLM获取解决方案并转化为生产规则，实现可解释的在线学习推荐。
...
Method: 提出CogRec认知推荐代理，以Soar作为核心符号推理引擎，利用LLM进行知识初始化填充工作记忆中的生产规则。采用感知-认知-行动循环，遇到瓶颈时动态查询LLM获取推理解决方案，通过Soar的组块机制转化为新的符号生产规则。

Result: 在三个公共数据集上的广泛评估表明，CogRec在推荐准确性、可解释性以及解决长尾问题方面具有显著优势。

Conclusion: CogRec成功结合了LLM和Soar认知架构的优势，实现了可解释的在线学习推荐系统，解决了传统LLM推荐系统的黑盒、幻觉和学习能力限制问题。
...
### [114] [Graph-Based Exploration for ARC-AGI-3 Interactive Reasoning Tasks](https://arxiv.org/abs/2512.24156)
*Evgenii Rudakov,Jonathan Shock,Benjamin Ultan Cowley*

Main category: cs.AI

TL;DR: 提出一种无需训练、基于图的方法来解决ARC-AGI-3基准中的交互推理任务，通过视觉帧处理与系统化状态空间探索，在稀疏反馈环境中显著优于当前最先进的LLM
...
Method: 结合视觉帧处理与系统化状态空间探索：将视觉帧分割为有意义的组件，基于视觉显著性优先选择动作，维护探索状态和转换的有向图，通过跟踪访问状态和测试动作，优先选择到达未测试状态-动作对的最短路径

Result: 在ARC-AGI-3预览挑战中，该方法在6个游戏的52个关卡中解决了中位数30个关卡，在私有排行榜上排名第3，显著优于前沿的LLM智能体

Conclusion: 即使没有学习，显式的图结构探索可以作为交互推理的强大基线，突显了在稀疏反馈环境中系统化状态跟踪和动作优先级的重要性，这些环境是当前LLM难以捕捉任务动态的
...
### [115] [SCP: Accelerating Discovery with a Global Web of Autonomous Scientific Agents](https://arxiv.org/abs/2512.24189)
...
TL;DR: SCP（科学上下文协议）是一个开源标准，旨在通过建立全球自主科学代理网络来加速科学发现。它提供统一的资源集成规范和实验生命周期管理架构，构建了包含1600多个工具资源的大规模科学发现平台。


<details>
  <summary>Details</summary>
Motivation: 当前科学发现面临资源分散、平台异构、集成成本高、可重复性差等挑战。不同机构的研究工具、模型、数据集和物理仪器难以无缝协作，阻碍了大规模跨机构科学合作和自主AI代理的协同工作。

Method: SCP基于两大支柱：1）统一资源集成：提供描述和调用科学资源的通用规范，涵盖软件工具、模型、数据集和物理仪器；2）编排的实验生命周期管理：采用集中式SCP Hub和联邦式SCP Server的安全服务架构，管理实验注册、规划、执行、监控和归档全过程。

Result: 基于SCP构建的科学发现平台已集成超过1600个工具资源。该协议在不同用例中促进了异构AI系统与人类研究人员之间的安全大规模协作，显著降低了集成开销，增强了实验可重复性。
...
### [116] [Deep Reinforcement Learning for Solving the Fleet Size and Mix Vehicle Routing Problem](https://arxiv.org/abs/2512.24251)
*Pengfu Wan,Jiawei Chen,Gangyan Xu*

Main category: cs.AI

TL;DR: 本文提出了一种基于深度强化学习的方法来解决车队规模与混合车辆路径问题，能够在几秒内生成近似最优解，特别适用于大规模和时间受限的场景。


<details>
  <summary>Details</summary>
Motivation: FSMVRP问题在实际应用中非常重要（如短期车辆租赁和按需物流），但需要同时做出车队组成和路径规划决策，增加了问题复杂度，特别是在大规模和时间受限环境下存在显著挑战。

Method: 将问题建模为马尔可夫决策过程，开发了名为FRIPN的新型策略网络，无缝集成车队组成和路径决策，包含专门设计的输入嵌入（包括剩余图嵌入）以促进有效的车辆使用决策。
...
### [117] [Constrained Language Model Policy Optimization via Risk-aware Stepwise Alignment](https://arxiv.org/abs/2512.24263)
*Lijun Zhang,Lin Li,Wei Wei,Yajie Qi,Huizhong Song,Jun Wang,Yaodong Yang,Jiye Liang*

Main category: cs.AI

TL;DR: 提出RSA方法，通过嵌套风险度量在策略优化中显式纳入风险意识，解决现有安全对齐方法风险中性不足的问题，有效抑制低概率高危害行为。


<details>
  <summary>Details</summary>
Motivation: 现有安全对齐方法（如Safe RLHF和SACPO）通常采用风险中性范式，不足以处理参考策略偏差带来的风险，且对罕见但可能灾难性的有害行为鲁棒性有限。

Method: 提出风险感知逐步对齐（RSA）方法，将安全对齐表述为token级别的风险感知约束策略优化问题，通过基于嵌套风险度量的逐步对齐过程求解，实现token级策略更新。

Result: 实验结果表明，该方法在保持高帮助性的同时确保强安全性，显著抑制了尾部风险（低概率高危害的不安全响应）。

Conclusion: RSA方法通过显式纳入风险意识，有效解决了现有安全对齐方法的局限性，在风险控制和模型安全性方面表现出色。
...
### [118] [Align While Search: Belief-Guided Exploratory Inference for World-Grounded Embodied Agents](https://arxiv.org/abs/2512.24461)
...
<details>
  <summary>Details</summary>
Motivation: 解决部分可观测环境下LLM智能体需要与潜在世界状态对齐的问题，避免梯度更新或额外训练的开销

Method: 维护外部结构化环境状态信念，通过动作条件观察迭代更新，使用轻量级LLM代理估计信息增益，通过后验信念与真实环境配置一致性奖励评估世界对齐

Result: 方法优于推理时扩展基线（如提示增强或检索增强LLM），在潜在世界状态对齐方面表现更好，且集成开销显著降低

Conclusion: 提出的测试时自适应智能体通过后验引导信念细化，能够在部分可观测环境下有效对齐潜在世界状态，无需梯度更新或额外训练
...
### [119] [What Drives Success in Physical Planning with Joint-Embedding Predictive World Models?](https://arxiv.org/abs/2512.24497)
*Basile Terver,Tsung-Yen Yang,Jean Ponce,Adrien Bardes,Yann LeCun*

Main category: cs.AI

TL;DR: 本文研究了在AI中通过世界模型进行规划的方法，特别是JEPA-WM这类在表示空间进行规划的方法，通过系统实验找到了最优配置，提出的模型在导航和操作任务上超越了现有基线。


<details>
  <summary>Details</summary>
Motivation: AI领域长期面临开发能够解决广泛物理任务并在新任务和环境上泛化的智能体的挑战。现有方法通常在世界模型的状态-动作轨迹上训练，然后在输入空间规划。最近出现了在表示空间规划的方法，但需要系统研究其技术选择。

Method: 将这类方法定义为JEPA-WM，系统研究模型架构、训练目标和规划算法等关键组件。使用模拟环境和真实机器人数据进行实验，分析各组件对规划成功率的影响。

Result: 提出的模型在导航和操作任务上超越了DINO-WM和V-JEPA-2-AC两个基线方法。代码、数据和检查点已开源。

Conclusion: 通过系统研究JEPA-WM家族的技术选择，找到了最优配置，证明了在表示空间规划的有效性，为开发更通用的AI智能体提供了重要进展。
...
### [120] [Thinking on Maps: How Foundation Model Agents Explore, Remember, and Reason Map Environments](https://arxiv.org/abs/2512.24504)
*Zhiwei Wei,Yuxing Liu,Hua Liao,Wenjia Xu*

Main category: cs.AI

TL;DR: 提出交互式评估框架分析基础模型代理在符号地图环境中的探索、记忆和推理能力，发现结构化记忆对空间理解至关重要，单纯模型缩放无法持续提升空间推理性能


<details>
  <summary>Details</summary>
Motivation: 现有基础模型空间能力评估多依赖静态地图输入或文本查询，忽略了空间理解的交互性和经验驱动特性，需要新的评估框架来深入分析模型在动态地图环境中的表现

Method: 提出交互式评估框架，让代理在部分可观测的网格地图中逐步探索（包含道路、交叉口和兴趣点），使用六种空间任务评估空间理解能力，系统变化探索策略、记忆表示和推理方案

Result: 探索主要影响经验获取但对最终推理准确性影响有限；记忆表示在整合空间经验中起核心作用，结构化记忆（特别是序列和图表示）显著提升路径规划等结构密集型任务性能；推理方案影响存储知识的利用方式；空间推理性能在超过一定能力阈值后趋于饱和

Conclusion: 提升地图空间理解需要针对空间表示和推理的专门机制，而非单纯模型缩放；结构化记忆表示对空间推理至关重要，为开发更可靠的地图推理应用提供了重要见解
...
### [121] [Evaluating the Reasoning Abilities of LLMs on Underrepresented Mathematics Competition Problems](https://arxiv.org/abs/2512.24505)
*Samuel Golladay,Majid Bani-Yaghoub*

Main category: cs.AI

TL;DR: 该研究评估了三种主流LLM在代表性不足的数学竞赛问题上的表现，发现DeepSeek-V3在所有类别中表现最佳，但所有模型在几何问题上都表现较弱，并揭示了不同的错误模式。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多使用相同数据集评估LLM的数学推理能力，限制了发现的一般性，且未能充分捕捉数学任务中的多样化挑战。本研究旨在分析LLM在代表性不足的数学竞赛问题上的表现。

Method: 使用密苏里大学数学竞赛问题（微积分、解析几何、离散数学）测试GPT-4o-mini、Gemini-2.0-Flash和DeepSeek-V3三种LLM，比较模型回答与已知正确答案，并分析推理过程中的错误模式。

Result: DeepSeek-V3在微积分、解析几何和离散数学三个类别中表现最佳（推理和最终答案均最优）。所有三种LLM在几何问题上表现明显较弱。DeepSeek-V3主要错误是计算和逻辑错误，GPT-4o-mini常见逻辑和方法错误，Gemini则倾向于不完整推理和仓促结论。

Conclusion: 在代表性不足的数学竞赛数据集上评估LLM能提供更深入的错误模式洞察，突显结构化推理（特别是几何领域）的持续挑战，为改进LLM数学能力提供方向。
...
### [122] [From Building Blocks to Planning: Multi-Step Spatial Reasoning in LLMs with Reinforcement Learning](https://arxiv.org/abs/2512.24532)
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在通用语言能力上表现强劲，但在结构化环境中的空间变换和多步规划方面仍然存在困难，这限制了其在导航和规划等应用中的效果。

Method: 采用两阶段方法：1) 对基础空间变换（旋转、平移、缩放）进行监督微调，使模型具备基础空间物理知识；2) 冻结物理感知模型，在GRPO框架内训练轻量级LoRA适配器，以组合这些基础模块进行多步规划。同时创建ASCII-art数据集和相应的强化学习环境。

Result: 该方法在动态环境（有显式状态更新）和静态环境（依赖内部状态）中均优于基线模型，包括通用主干模型、物理感知模型和端到端RL模型。此外，该方法收敛更快，训练更稳定，注意力模式分析显示微调确实改善了空间理解。

Conclusion: 通过将空间推理分解为原子构建块及其组合的两阶段方法，可以有效提升大语言模型的空间推理能力，为导航和规划应用提供了更可靠的解决方案。
...
### [123] [MCPAgentBench: A Real-world Task Benchmark for Evaluating LLM Agent MCP Tool Use](https://arxiv.org/abs/2512.24565)
*Wenrui Liu,Zixiang Liu,Elsie Dai,Wenhan Yu,Lei Yu,Tong Yang*

Main category: cs.AI

TL;DR: 提出MCPAgentBench基准测试，用于评估基于MCP协议的LLM智能体工具使用能力，包含真实任务、模拟工具和动态沙箱环境。


<details>
  <summary>Details</summary>
Motivation: 当前MCP评估集存在依赖外部MCP服务、缺乏难度感知等问题，需要更好的基准来评估智能体的工具使用能力。

Method: 构建基于真实MCP定义的数据集，包含真实任务和模拟MCP工具；使用动态沙箱环境，提供包含干扰项的工具候选列表；引入综合指标衡量任务完成率和执行效率。

Result: 在多种主流大语言模型上的实验显示，在处理复杂多步骤工具调用时存在显著性能差异。

Conclusion: MCPAgentBench为评估智能体工具使用能力提供了有效的基准测试框架，所有代码已开源。
...
### [124] [Recursive Language Models](https://arxiv.org/abs/2512.24601)
*Alex L. Zhang,Tim Kraska,Omar Khattab*

Main category: cs.AI

TL;DR: 提出递归语言模型（RLMs），一种让LLM通过编程方式检查、分解并递归调用自身处理超长提示的推理策略，能处理超出模型上下文窗口两个数量级的输入，性能优于基础LLM和常见长上下文框架。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型受限于固定的上下文窗口长度，无法处理任意长度的提示。需要一种方法让LLM能够处理超出其原始设计限制的长文本输入。

Method: 提出递归语言模型（RLMs），这是一种通用的推理策略。它将长提示视为外部环境，允许LLM以编程方式检查、分解提示，并在提示片段上递归调用自身。RLMs不是修改模型架构，而是在推理时通过递归处理实现长上下文能力。

Result: RLMs成功处理了超出模型上下文窗口两个数量级的输入。即使在较短的提示上，RLMs在四个不同的长上下文任务中也显著优于基础LLM和常见的长上下文框架，同时每个查询的成本相当或更低。

Conclusion: 递归语言模型提供了一种有效的推理时扩展策略，使LLM能够处理任意长度的提示，在保持成本效益的同时显著提升长上下文处理能力。
...
### [125] [Reinforcement Learning-Augmented LLM Agents for Collaborative Decision Making and Performance Optimization](https://arxiv.org/abs/2512.24609)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在语言任务中表现良好，但在多智能体环境中缺乏协作意识，难以优化全局性能，需要解决多智能体协作优化问题。

Method: 1. 将协作建模为分散部分可观测马尔可夫决策过程(Dec-POMDP)；2. 采用集中训练分散执行(CTDE)框架；3. 引入组相对策略优化(GRPO)联合优化智能体策略；4. 设计简化联合奖励函数平衡任务质量、速度和协调成本。

Result: 1. 任务处理速度比单智能体基线提升3倍；2. 协作写作中结构/风格一致性达98.7%；3. 编程任务测试通过率达74.6%；4. 持续优于强大多智能体LLM基线。
...
### [126] [Group Deliberation Oriented Multi-Agent Conversational Model for Complex Reasoning](https://arxiv.org/abs/2512.24613)
*Zheyu Shi,Dong Qiu,Shanlong Yu*

Main category: cs.AI

TL;DR: 提出基于群体审议的多智能体对话模型，通过三层角色分工架构提升复杂推理任务的准确性和一致性，相比主流方法在多个数据集上显著提升性能。
...
Method: 采用三层角色分工架构：生成代理产生多样化推理视角，验证代理检索外部知识并量化事实支持，仲裁代理整合逻辑一致结论。引入自博弈机制扩展多路径推理轨迹，检索增强模块动态补充外部知识，设计结合事实一致性和逻辑连贯性的复合奖励函数，应用改进的近端策略优化进行协同训练。
...
### [127] [Youtu-Agent: Scaling Agent Productivity with Automated Generation and Hybrid Policy Optimization](https://arxiv.org/abs/2512.24615)
...
TL;DR: Youtu-Agent：一个模块化LLM智能体框架，支持自动化生成和持续进化，解决现有框架配置成本高和静态能力的问题


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体框架面临两大挑战：1）高配置成本（需要大量手动工具集成和提示工程）；2）静态能力（部署后难以适应动态环境，需要昂贵的微调）

Method: 提出模块化框架，包含：1）结构化配置系统（解耦执行环境、工具包和上下文管理）；2）两种生成范式（Workflow模式用于标准任务，Meta-Agent模式用于复杂需求）；3）混合策略优化系统（Agent Practice模块通过上下文优化积累经验，Agent RL模块集成分布式训练框架进行强化学习）

Result: 在WebWalkerQA（71.47%）和GAIA（72.8%）上达到SOTA性能；工具合成成功率超过81%；Practice模块在AIME 2024/2025上分别提升2.7%和5.4%；Agent RL训练在7B LLM上实现40%加速，在数学和通用/多跳QA基准上分别提升35%和21%

Conclusion: Youtu-Agent通过自动化生成和持续进化机制，有效解决了LLM智能体框架的高配置成本和静态能力问题，实现了灵活、可扩展的智能体开发
...
### [128] [Multi-modal cross-domain mixed fusion model with dual disentanglement for fault diagnosis under unseen working conditions](https://arxiv.org/abs/2512.24679)
*Pengcheng Xia,Yixiang Huang,Chengjin Qin,Chengliang Liu*

Main category: cs.AI

TL;DR: 提出多模态跨域混合融合模型，通过双重解耦分离模态不变/特定特征和域不变/特定表示，增强故障诊断在未见工况下的泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有故障诊断方法在未见工况下性能下降严重，域适应方法依赖目标域样本，且大多依赖单模态信号，忽略了多模态信息的互补性
...
Conclusion: 提出的多模态跨域混合融合模型通过双重解耦和跨域混合融合，有效提升了故障诊断在未见工况下的泛化性能
...
### [129] [BatteryAgent: Synergizing Physics-Informed Interpretation with LLM Reasoning for Intelligent Battery Fault Diagnosis](https://arxiv.org/abs/2512.24686)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法虽然检测精度高，但存在"黑箱"问题难以解释，且受限于二分类范式，无法提供根因分析和维护建议

Method: 提出三层框架：1)物理感知层提取10个基于电化学原理的特征；2)检测归因层使用梯度提升决策树和SHAP量化特征贡献；3)推理诊断层利用LLM作为核心，构建"数值-语义"桥梁，结合SHAP归因和机理知识库生成综合报告

Result: BatteryAgent有效纠正硬边界样本的误分类，AUROC达到0.986，显著优于当前最先进方法，并将传统二分类检测扩展到多类型可解释诊断

Conclusion: 该框架为电池安全管理提供了从"被动检测"到"智能诊断"的新范式转变，实现了故障类型、根因分析和维护建议的综合诊断
...
### [130] [Explaining Why Things Go Where They Go: Interpretable Constructs of Human Organizational Preferences](https://arxiv.org/abs/2512.24829)
*Emmanuel Fashae,Michael Burke,Leimin Tian,Lingheng Meng,Pamela Carreno-Medrano*

Main category: cs.AI

TL;DR: 论文提出了一个可解释的家庭物品摆放偏好模型，包含四个维度：空间实用性、习惯便利性、语义连贯性和常识适当性，并通过问卷验证和MCTS规划器应用。


<details>
  <summary>Details</summary>
Motivation: 现有机器人系统依赖潜在偏好模型进行物品重排，虽然预测有效但缺乏可解释性，无法理解指导人类决策的可解释因素。

Method: 设计了包含四个可解释构念的偏好模型，通过63名参与者的在线研究验证问卷，并将这些构念整合到蒙特卡洛树搜索规划器中。

Result: 研究证实了这四个构念的心理区分性和解释力，当规划器使用参与者衍生的偏好时，能够生成与参与者安排高度一致的合理布局。

Conclusion: 这项工作贡献了一个紧凑、可解释的物品摆放偏好模型，并展示了如何将其应用于机器人规划，为可解释的家庭机器人系统提供了新方法。
...
### [131] [GenZ: Foundational models as latent variable generators within traditional statistical models](https://arxiv.org/abs/2512.24834)
*Marko Jojic,Nebojsa Jojic*

Main category: cs.AI

TL;DR: GenZ是一个混合模型，通过可解释的语义特征桥接基础模型和统计建模，在房价预测和电影推荐任务中显著优于单纯依赖基础模型的方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然拥有广泛的领域知识，但往往无法捕捉对预测任务至关重要的数据集特定模式。需要一种方法既能利用基础模型的语义理解能力，又能发现数据集特定的统计模式。

Method: 通过迭代过程发现语义特征描述，对比统计建模错误识别的项目组。采用广义EM算法联合优化语义特征描述符和统计模型参数，将冻结的基础模型对项目的分类判断视为潜在二元特征的噪声观测。

Result: 在房价预测任务中，使用多模态列表数据发现的语义特征实现了12%的中位数相对误差，显著优于GPT-5基准的38%误差。在Netflix电影推荐任务中，仅从语义描述预测协同过滤表示达到0.59余弦相似度，相当于传统协同过滤需要约4000个用户评分的性能。

Conclusion: GenZ成功地将基础模型的语义理解与统计建模相结合，发现了数据集特定的模式（如建筑细节预测当地住房市场、系列电影成员预测用户偏好），这些模式超越了基础模型单独的知识范围。
...
### [132] [A study on constraint extraction and exception exclusion in care worker scheduling](https://arxiv.org/abs/2512.24853)
*Koki Suenaga,Tomohiro Furuta,Satoshi Ono*

Main category: cs.AI

TL;DR: 提出一种使用约束模板从养老机构管理者访谈中提取设施特定约束条件的方法，用于生成护工排班表


<details>
  <summary>Details</summary>
Motivation: 养老机构的排班条件因设施而异，需要通过与排班管理者访谈来设计设施特定的约束条件，但现有约束提取技术难以处理例外约束

Method: 使用约束模板提取各种组件的组合（如连续工作班次模式、员工组合），通过改变关注的天数和员工数量，以及将提取焦点调整为模式或频率来提取多样约束，并包含排除例外约束的机制

Result: 实验证明该方法成功创建了满足所有硬约束的排班表，并通过避免提取例外约束减少了软约束的违反次数

Conclusion: 提出的约束模板方法能够有效提取养老机构特定的约束条件，生成符合实际需求的护工排班表
...
### [133] [Let It Flow: Agentic Crafting on Rock and Roll, Building the ROME Model within an Open Agentic Learning Ecosystem](https://arxiv.org/abs/2512.24873)
...
Method: 提出ALE生态系统，包含三个组件：ROLL（权重优化）、ROCK（轨迹生成沙盒）、iFlow CLI（上下文工程）；开发数据合成协议和IPA算法（基于语义交互块的策略优化）；训练ROME智能体模型

Result: ROME在SWE-bench Verified和Terminal Bench等基准测试中表现优异，证明了ALE基础设施的有效性；发布了包含超过100万轨迹训练的开放源码智能体

Conclusion: ALE为智能体开发提供了系统化的基础设施，ROME的成功验证了该生态系统的有效性，为开源社区提供了完整的智能体开发解决方案
...
### [134] [Semi-Automated Data Annotation in Multisensor Datasets for Autonomous Vehicle Testing](https://arxiv.org/abs/2512.24896)
*Andrii Gamalii,Daniel Górniak,Robert Nowak,Bartłomiej Olber,Krystian Radlak,Jakub Winter*

Main category: cs.AI

TL;DR: 开发了一个用于波兰驾驶场景数据标注的半自动化流水线，采用人机协同方法，结合AI自动标注与人工审核，显著降低标注成本和时间


<details>
  <summary>Details</summary>
Motivation: DARTS项目需要创建大规模、多模态的波兰驾驶场景数据集，但手动标注异构数据成本高、耗时长，需要更高效的解决方案

Method: 采用人机协同方法，结合AI自动生成初始标注、迭代模型重训练、数据匿名化和领域适应技术，核心基于3D目标检测算法生成初步标注

Result: 开发的工具和方法实现了显著的时间节省，同时确保不同传感器模态间的一致高质量标注，直接支持DARTS项目加速大规模标注数据集准备

Conclusion: 该半自动化标注流水线有效解决了大规模驾驶场景数据标注的挑战，为波兰自动驾驶研究提供了技术基础，平衡了效率与标注质量
...
### [135] [Iterative Deployment Improves Planning Skills in LLMs](https://arxiv.org/abs/2512.24940)
*Augusto B. Corrêa,Yoav Gelberg,Luckeciano C. Melo,Ilia Shumailov,André G. Pereira,Yarin Gal*

Main category: cs.AI

TL;DR: 迭代部署LLM模型，通过用户从先前模型部署中精心筛选数据来微调后续模型，能显著改变模型特性，在规划任务中实现能力提升和泛化，且该过程在理论上等价于隐式强化学习。


<details>
  <summary>Details</summary>
Motivation: 探索迭代部署大型语言模型时，用户从先前模型输出中筛选数据来微调后续模型这一机制对模型特性的影响，特别是这种机制如何隐式地实现强化学习效果。

Method: 采用迭代部署方法：每个新模型都在用户从前一模型部署中精心筛选的数据上进行微调。在多个规划领域测试该机制，并进行理论分析，证明迭代部署在外部循环中实现了隐式强化学习训练。

Result: 在规划任务中观察到显著改进，后续模型展现出涌现的泛化能力，能够发现比初始模型长得多的规划方案。理论分析表明迭代部署实现了隐式奖励函数的强化学习。

Conclusion: 迭代部署机制可视为显式强化学习的替代训练方案，依赖数据筛选而非显式奖励。这对AI安全有重要启示，因为隐含的奖励函数未明确定义，可能对未来模型部署产生意外影响。
...
### [136] [AMAP Agentic Planning Technical Report](https://arxiv.org/abs/2512.24957)
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂时空任务（如受限兴趣点发现和行程规划）需要专门的智能体模型，能够与时空工具交互并在推理过程中探索、验证和优化中间步骤。

Method: 1) 构建支持10+领域特定工具的稳定工具环境；2) 分层数据筛选框架，以1:10,000的比例筛选高质量查询；3) 级联训练方法：种子SFT阶段评估查询难度，第二SFT阶段微调高确定性查询，最终RL阶段利用低确定性数据。

Result: STAgent在TravelBench上表现优异，同时在广泛的通用基准测试中保持其通用能力，证明了所提出的智能体模型的有效性。
...
### [137] [Context-aware LLM-based AI Agents for Human-centered Energy Management Systems in Smart Buildings](https://arxiv.org/abs/2512.25055)
...
<details>
  <summary>Details</summary>
Motivation: 现有能源管理系统存在局限性，需要更智能、上下文感知的解决方案。利用LLM的自主数据分析能力，通过自然语言交互实现更直观的能源管理

Method: 提出包含感知、中央控制和行动三个模块的框架，形成闭环反馈系统。使用120个用户查询在四个真实住宅能源数据集上评估原型性能，采用延迟、功能、能力、准确性和成本效益等指标

Result: 设备控制准确率86%，记忆相关任务97%，调度和自动化74%，能源分析77%，但成本估算任务准确率较低为49%。ANOVA测试证明了框架的普适性

Conclusion: LLM-based BEMS AI代理在多个任务上表现良好，但复杂任务需要改进。研究为评估此类系统提供了基准，并强调了响应准确性与计算效率之间的权衡
...
### [138] [Network Traffic Analysis with Process Mining: The UPSIDE Case Study](https://arxiv.org/abs/2512.23718)
...
<details>
  <summary>Details</summary>
Motivation: 在线游戏产生大量网络流量和市场收入，需要建模网络设备行为以评估带宽消耗、预测高负载和检测恶意活动。过程挖掘结合数据驱动分析和模型洞察的能力在此领域具有潜力。

Method: 基于过程挖掘的方法分析游戏网络流量：1) 无监督表征游戏网络数据的不同状态；2) 通过过程挖掘将这些状态编码为可解释的Petri网；3) 对游戏网络流量数据进行分类以识别不同游戏。

Result: 在UPSIDE案例研究中应用该方法，分析Clash Royale和Rocket League两款游戏的网络数据。结果显示：游戏网络行为可通过Petri网状态有效建模，具有足够的一致性（设备间相似度94.02%）和特异性（状态间分离度174.99%），同时保持较好的游戏分类准确率（AUC 73.84%）。

Conclusion: 过程挖掘方法能够有效且可解释地建模游戏网络行为，为网络设备行为分析、带宽管理和游戏识别提供了新工具。
...
### [139] [A Comprehensive Study of Deep Learning Model Fixing Approaches](https://arxiv.org/abs/2512.23745)
*Hanmo You,Zan Wang,Zishuo Dong,Luanqi Mo,Jianjun Zhao,Junjie Chen*

Main category: cs.LG

TL;DR: 对16种最先进的深度学习模型修复方法进行大规模实证研究，评估其修复效果及对其他关键属性（鲁棒性、公平性、向后兼容性）的影响，发现模型级方法修复效果最佳，但没有单一方法能在提高准确性的同时保持所有其他属性。


<details>
  <summary>Details</summary>
Motivation: 深度学习系统与传统软件一样容易出错，其故障可能给用户带来重大风险。虽然已有许多修复方法被提出，但缺乏对这些方法性能的全面评估，特别是对修复效果之外的其他关键属性的影响。

Method: 对16种最先进的深度学习模型修复方法进行大规模实证研究，涵盖模型级、层级和神经元级三个类别。在统一的实验设置下，使用多样化的数据集、模型架构和应用领域进行评估，不仅评估修复效果，还评估对鲁棒性、公平性和向后兼容性的影响。

Result: 模型级方法在修复效果上优于其他方法；没有单一方法能在实现最佳修复性能的同时提高准确性并保持所有其他属性；修复方法往往会对其他关键属性产生负面影响。
...
### [140] [A Review of Diffusion-based Simulation-Based Inference: Foundations and Applications in Non-Ideal Data Scenarios](https://arxiv.org/abs/2512.23748)
*Haley Rosso,Talea Mayo*

Main category: cs.LG

TL;DR: 本文综述了基于扩散模型的仿真推理方法，从数学基础到实际应用，重点探讨了扩散模型在科学数据非理想条件下的鲁棒性优势。


<details>
  <summary>Details</summary>
Motivation: 对于复杂仿真问题，由于似然函数难以处理，传统的基于似然的方法往往不可行。仿真推理方法通过直接利用仿真器样本来学习参数后验分布，而扩散模型作为一种灵活的生成模型框架，在SBI任务中展现出潜力，特别是在处理科学数据常见的非理想条件时具有鲁棒性优势。

Method: 回顾扩散模型在SBI中的应用：包括扩散模型的数学基础（前向加噪、反向时间SDE/ODE、概率流、去噪分数匹配），条件分数如何实现无似然后验采样，分析扩散模型相对于归一化流的优势与权衡，以及针对非理想条件（模型误设、非结构化/无限维观测、数据缺失）的鲁棒性方法。

Result: 扩散模型为SBI提供了灵活的框架，在非理想条件下表现出鲁棒性，能够处理传统方法难以应对的科学数据问题，如模型误设、非结构化观测和数据缺失等。

Conclusion: 基于扩散模型的SBI方法在处理科学数据非理想条件时具有显著优势，为概率地球物理模型等应用的不确定性量化提供了有前景的解决方案，但仍存在迭代采样成本等权衡，需要进一步研究开放性问题。
...
### [141] [Coordinate Matrix Machine: A Human-level Concept Learning to Classify Very Similar Documents](https://arxiv.org/abs/2512.23749)
...
<details>
  <summary>Details</summary>
Motivation: 人类通常能从单个示例中学习新概念，而机器学习算法需要大量样本。现有"红色AI"依赖大规模预训练和GPU基础设施，能耗高。需要一种更接近人类学习方式、更环保的AI解决方案。

Method: 提出Coordinate Matrix Machine (CM²)，这是一种小型专用模型。它学习文档结构信息进行分类，专注于人类会考虑的结构性"重要特征"，而不是穷尽的语义向量。模型设计为绿色AI解决方案，可在CPU环境中运行。

Result: CM²在单样本学习方面优于传统向量化方法和复杂的深度学习模型。它能够使用每个类别仅一个样本对非常相似的文档进行分类，同时提供高准确性、快速计算、低延迟和对抗不平衡类别的鲁棒性。

Conclusion: CM²实现了人类级概念学习，提供了一种环保、经济、可解释的AI解决方案。它展示了通过专注于结构特征而非语义向量，可以在小数据场景下实现高效学习，为绿色AI发展提供了新方向。
...
### [142] [Geometric Scaling of Bayesian Inference in LLMs](https://arxiv.org/abs/2512.23752)
...
Method: 在Pythia、Phi-2、Llama-3和Mistral等模型家族中分析最后一层值表示的组织结构，发现其沿单一主导轴排列。通过针对性干预实验，在Pythia-410M的上下文学习过程中操纵熵对齐轴，观察对局部不确定性几何的影响。

Result: 发现所有测试模型的值表示都沿单一主导轴组织，该轴位置与预测熵强相关；领域受限提示会将该结构压缩为与合成设置中观察到的相同低维流形。干预熵对齐轴会选择性破坏局部不确定性几何，但单层操纵不会导致贝叶斯行为的比例性退化。

Conclusion: 现代语言模型保留了支持贝叶斯推理的几何基底，并沿该基底组织其近似贝叶斯更新。该几何结构是特权不确定性读取机制，而非单一计算瓶颈。
...
### [143] [Generalized Regularized Evidential Deep Learning Models: Theory and Comprehensive Evaluation](https://arxiv.org/abs/2512.23753)
*Deep Shankar Pandey,Hyomin Choi,Qi Yu*

Main category: cs.LG

TL;DR: 本文分析了EDL模型中激活函数导致的梯度消失问题，提出了新的激活函数和正则化方法来解决学习冻结行为。


<details>
  <summary>Details</summary>
Motivation: EDL模型虽然能有效量化不确定性，但主观逻辑框架要求证据非负，这导致特定激活函数在低证据区域会产生极小的梯度，引发学习冻结问题。

Method: 理论分析了激活函数对学习动态的影响，设计了一族通用的激活函数和相应的证据正则化器，确保在不同激活区域都能进行一致的证据更新。

Result: 在四个基准分类任务、两个少样本分类任务和盲脸恢复问题上进行了广泛实验，验证了理论分析并证明了所提方法的有效性。

Conclusion: 通过理论分析和实验验证，提出的广义正则化证据模型解决了EDL中的学习冻结问题，提高了模型的学习效率和性能。
...
### [144] [HINTS: Extraction of Human Insights from Time-Series Without External Sources](https://arxiv.org/abs/2512.23755)
*Sheo Yon Jhin,Noseong Park*

Main category: cs.LG

TL;DR: HINTS是一个自监督学习框架，通过从时间序列残差中内生提取人类因素（社会影响、记忆和偏见），无需外部数据，显著提升预测准确性。


<details>
  <summary>Details</summary>
Motivation: 人类决策、情感和集体心理是影响金融经济系统时间动态的关键因素。现有方法依赖外部数据（新闻、社交媒体）来捕捉这些因素，但存在高昂的数据依赖成本（财务、计算、实际应用）。
...
Conclusion: HINTS提供了一种无需外部数据、成本效益高的方法来捕捉人类因素，在提升预测准确性的同时保持可解释性，具有实际应用价值。
...
### [145] [Learning Coupled System Dynamics under Incomplete Physical Constraints and Missing Data](https://arxiv.org/abs/2512.23761)
*Esha Saha,Hao Wang*
...
<details>
  <summary>Details</summary>
Motivation: 复杂系统通常由多个耦合变量描述，但通常只有一个变量的控制方程已知，其他变量只能通过数据获取。这种已知物理与观测数据之间的不匹配对现有的物理信息机器学习方法构成了根本挑战。
...
Conclusion: MUSIC为建模具有不完全物理知识的部分观测系统提供了一种灵活有效的方法。
...
### [146] [Drift-Based Dataset Stability Benchmark](https://arxiv.org/abs/2512.23762)
*Dominik Soukup,Richard Plný,Daniel Vašata,Tomáš Čejka*

Main category: cs.LG

TL;DR: 本文提出了一种评估数据集稳定性的新方法和基准工作流程，用于网络流量分类中的概念漂移检测和数据集质量评估。


<details>
  <summary>Details</summary>
Motivation: 网络流量分类中，由于网络快速演变和新协议出现，训练模型部署后可能很快性能下降。数据或概念漂移会导致模型性能大幅下降，而现有方法通常直接重新训练，缺乏对数据集质量的深入调查。

Method: 提出基于概念漂移检测方法的新框架，利用机器学习特征权重提升检测性能。建立数据集稳定性评估方法和基准工作流程，用于比较不同数据集。

Result: 在CESNET-TLS-Year22数据集上验证了方法的有效性，提供了初始数据集稳定性基准，识别了数据集弱点和优化方向，展示了优化对数据集变体的影响。

Conclusion: 提出的方法能够有效评估数据集稳定性，为网络流量分类中的概念漂移检测提供系统化的工作流程，有助于识别数据集质量问题并指导优化方向。
...
### [147] [Neural Optimal Design of Experiment for Inverse Problems](https://arxiv.org/abs/2512.23763)
...
<details>
  <summary>Details</summary>
Motivation: 传统逆问题中的实验最优设计通常采用双层优化和间接稀疏正则化方法，计算复杂且需要调参。需要一种更直接、高效的方法来优化测量位置

Method: 提出NODE框架，在单一优化循环中联合训练神经重建模型和固定预算的连续设计变量（传感器位置、采样时间、测量角度）。通过直接优化测量位置而非加权密集候选网格，实现设计层面的稀疏性

Result: 在指数增长基准测试、MNIST图像采样和稀疏视角X射线CT实验中，NODE均优于基线方法，展示了改进的重建精度和任务特定性能

Conclusion: NODE提供了一种计算高效、无需l1调参的实验最优设计方法，通过设计层面的稀疏性实现了更好的重建效果
...
### [148] [Exploring Cumulative Effects in Survival Data Using Deep Learning Networks](https://arxiv.org/abs/2512.23764)
*Kang-Chung Yang,Shinsheng Yuan*

Main category: cs.LG

TL;DR: CENNSurv是一个深度学习框架，用于分析时间依赖性暴露对生存结果的累积效应，在保持可解释性的同时提升大规模数据集的处理能力。


<details>
  <summary>Details</summary>
Motivation: 流行病学研究中，时间依赖性暴露对生存结果的累积效应建模存在挑战：传统样条方法需要重复数据转换且计算量大，而现有神经网络方法注重准确性但缺乏对累积暴露模式的可解释性。

Method: 提出CENNSurv深度学习框架，能够从时间依赖性数据中捕捉动态风险关系，在保持可解释性的同时处理复杂的时间模式。

Result: 在两个真实世界数据集上评估，CENNSurv揭示了慢性环境暴露与关键生存结果之间的多年滞后关联，以及订阅流失前的关键短期行为转变，展示了其建模复杂时间模式的能力和可扩展性改进。

Conclusion: CENNSurv为研究累积效应的研究者提供了一个实用工具，能够提供可解释的洞察，解决了传统方法在大规模数据处理和可解释性方面的局限性。
...
### [149] [A Granular Grassmannian Clustering Framework via the Schubert Variety of Best Fit](https://arxiv.org/abs/2512.23766)
*Karim Salta,Michael Kirby,Chris Peterson*

Main category: cs.LG

TL;DR: 提出SVBF-LBG算法，用可训练的舒伯特簇最佳拟合原型替代传统子空间均值，在LBG框架中实现更好的子空间聚类效果。


<details>
  <summary>Details</summary>
Motivation: 在子空间聚类任务中，需要为数据集或簇计算几何代表（如均值或中位数）。传统方法使用子空间均值，但可能无法充分捕捉子空间的几何结构。本文旨在开发一种能更好利用子空间几何特性的聚类算法。

Method: 提出SVBF-LBG算法：1）用舒伯特簇最佳拟合（SVBF）作为可训练原型，该原型定义为与每个簇成员在至少一个固定方向上尽可能接近相交的子空间；2）将SVBF集成到Linde-Buzo-Grey（LBG）聚类流程中。

Result: 在合成数据、图像数据、光谱数据和视频动作数据上的实验表明，SVBF-LBG相比传统方法能获得更高的聚类纯度，同时保留了后续分析所需的数学结构。

Conclusion: SVBF-LBG算法通过引入几何意义更强的舒伯特簇最佳拟合原型，在子空间聚类中取得了更好的性能，为下游分析提供了合适的数学框架。
...
### [150] [Enabling Physical AI at the Edge: Hardware-Accelerated Recovery of System Dynamics](https://arxiv.org/abs/2512.23767)
...
<details>
  <summary>Details</summary>
Motivation: 边缘物理AI需要硬件高效的学习和推理能力，而现有的模型恢复方法（如EMILY和PINN+SR）依赖需要迭代求解器的神经ODE公式，难以在边缘硬件上高效加速。

Method: MERINDA用硬件友好的公式替换昂贵的神经ODE组件，结合：(1)基于GRU的离散化动力学，(2)密集逆ODE层，(3)稀疏性驱动的dropout，(4)轻量级ODE求解器，并将计算结构化为流式并行。

Result: 在四个基准非线性动力系统中，MERINDA相比GPU实现：能耗降低114倍（434J vs 49,375J），内存占用减少28倍（214MB vs 6,118MB），训练速度提高1.68倍，同时保持最先进的模型恢复精度。

Conclusion: MERINDA能够将准确、可解释的模型恢复带到边缘，实现对自主系统的实时监控，使物理AI在资源受限设备上变得实用。
...
### [151] [Safety-Biased Policy Optimisation: Towards Hard-Constrained Reinforcement Learning via Trust Regions](https://arxiv.org/abs/2512.23770)
*Ankit Kanwar,Dominik Wagner,Luke Ong*

Main category: cs.LG

TL;DR: SB-TRPO是一种新的信任区域算法，通过自适应偏置策略更新来满足安全约束，同时寻求奖励改进，在安全关键RL中实现安全与任务完成的最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 安全关键领域中的强化学习需要在最大化奖励的同时严格遵守安全约束。现有方法（如拉格朗日法和投影法）要么无法确保接近零的安全违规，要么在面对硬约束时牺牲奖励性能。

Method: 提出安全偏置信任区域策略优化（SB-TRPO），使用成本和奖励的自然策略梯度的凸组合进行信任区域更新，确保每一步都有固定比例的最优成本减少，自适应地将策略更新偏置到约束满足方向。

Result: 在标准和安全Gymnasium挑战任务上的实验表明，SB-TRPO相比最先进方法，始终实现安全与有意义任务完成的最佳平衡。

Conclusion: SB-TRPO为硬约束强化学习提供了一种有效的信任区域算法，能够在确保安全的同时保持奖励性能，解决了现有方法在安全与性能之间的权衡问题。
...
### [152] [FineFT: Efficient and Risk-Aware Ensemble Reinforcement Learning for Futures Trading](https://arxiv.org/abs/2512.23773)
...
<details>
  <summary>Details</summary>
Motivation: 现有RL方法主要针对现货市场，无法直接应用于高杠杆的期货市场，面临两个挑战：1）高杠杆放大奖励波动导致训练不稳定；2）缺乏对能力边界的自我认知，遇到新市场状态（如黑天鹅事件）时可能造成重大损失

Method: 提出三阶段集成RL框架：阶段I通过集成TD误差选择性更新Q学习器以提高收敛性；阶段II基于盈利能力筛选Q学习器，并训练VAE识别学习器的能力边界；阶段III根据训练好的VAE指导，在筛选后的集成策略和保守策略之间选择，以维持盈利能力并降低新市场状态下的风险

Result: 在高频交易环境下对加密货币期货进行实验（5倍杠杆），FineFT在6个金融指标上优于12个SOTA基线，风险降低超过40%的同时获得比第二名更优的盈利能力；可视化显示不同智能体专门处理不同的市场动态，消融研究证实VAE路由有效降低最大回撤，选择性更新改善收敛和性能

Conclusion: FineFT通过创新的三阶段集成RL框架成功解决了高杠杆期货交易中的训练不稳定和风险管理问题，实现了在保持高盈利能力的同时显著降低风险，为量化交易提供了有效的风险感知解决方案
...
### [153] [A Survey on Graph Neural Networks for Fraud Detection in Ride Hailing Platforms](https://arxiv.org/abs/2512.23777)
...
<details>
  <summary>Details</summary>
Motivation: 网约车平台面临日益复杂的欺诈活动，需要有效的检测方法。现有欺诈检测研究在应对类别不平衡和欺诈伪装方面存在不足，图神经网络为这一领域提供了新的技术可能性。

Method: 采用图神经网络(GNNs)进行欺诈检测，分析比较不同GNN模型架构和方法论，重点关注处理类别不平衡和欺诈伪装的技术，并对现有相关工作进行系统综述。

Result: 研究识别了GNN在异常检测中的方法论进展和差距，比较了各种模型的有效性，为网约车平台欺诈检测提供了结构化框架，但指出在现实世界适用性方面仍需改进。
...
### [154] [TabMixNN: A Unified Deep Learning Framework for Structural Mixed Effects Modeling on Tabular Data](https://arxiv.org/abs/2512.23787)
*Deniz Akdemir*
...
Method: 采用模块化三层架构：1) 具有变分随机效应和灵活协方差结构的混合效应编码器；2) 包括广义结构方程模型和时空流形网络的骨干架构；3) 支持多种结果族的特定结果预测头。关键创新包括R风格公式接口、DAG约束、SPDE核和SHAP值等可解释性工具。

Result: 通过纵向数据分析、基因组预测和时空建模等应用展示了框架的灵活性，为研究人员提供了统一接口，既能利用深度学习优势，又能保持经典混合效应模型的可解释性和理论基础。

Conclusion: TabMixNN成功地将经典混合效应建模与现代神经网络架构相结合，为表格数据分析提供了一个灵活、可解释且功能强大的框架，特别适用于处理分层数据结构和复杂建模任务。
...
### [155] [Zero-Trust Agentic Federated Learning for Secure IIoT Defense Systems](https://arxiv.org/abs/2512.23809)
...
<details>
  <summary>Details</summary>
Motivation: 工业物联网(IIoT)关键基础设施面临严重安全威胁（如2021年Oldsmar水处理厂和2023年丹麦能源部门攻击），现有联邦学习(FL)框架存在拜占庭攻击漏洞和代理认证不足的问题。

Method: 提出零信任代理联邦学习(ZTA-FL)框架：1) TPM加密认证（错误接受率<0.0000001）；2) SHAP加权聚合算法（非独立同分布条件下可解释的拜占庭检测）；3) 隐私保护的设备端对抗训练。

Result: 在三个IDS基准测试中：检测准确率97.8%；30%拜占庭攻击下准确率93.2%（优于FLAME 3.1%，p<0.01）；对抗鲁棒性89.3%；通信开销降低34%。

Conclusion: ZTA-FL框架有效解决了工业物联网联邦学习的安全漏洞，提供了理论保证、故障模式分析和可复现代码，为关键基础设施安全提供了深度防御方案。
...
### [156] [Improved Bounds for Private and Robust Alignment](https://arxiv.org/abs/2512.23816)
*Wenqian Weng,Yi He,Xingyu Zhou*

Main category: cs.LG

TL;DR: 论文从理论角度研究语言模型的私有鲁棒对齐，在离线和在线设置中建立次优性差距的上界，分析隐私约束和对抗性腐败的两种交互模式。


<details>
  <summary>Details</summary>
Motivation: 研究在隐私约束和对抗性腐败同时存在的情况下，如何实现语言模型的对齐，解决现有方法在理论保证方面的不足。

Method: 使用对数损失和平方损失的均匀收敛保证，分析隐私优先和腐败优先两种交互模式，提出MLE风格算法，并在离线和在线设置中建立理论界限。

Result: 在仅隐私设置中，对数损失配合MLE风格算法达到接近最优的收敛率；在联合隐私腐败设置中，现有离线算法提供比已知更强的保证；首次提出私有鲁棒在线对齐的结果。

Conclusion: 论文建立了语言模型私有鲁棒对齐的理论框架，提供了新的均匀收敛保证，这些结果在学习和统计领域具有广泛适用性。
...
### [157] [MS-SSM: A Multi-Scale State Space Model for Efficient Sequence Modeling](https://arxiv.org/abs/2512.23824)
*Mahdi Karami,Ali Behrouz,Peilin Zhong,Razvan Pascanu,Vahab Mirrokni*

Main category: cs.LG

TL;DR: MS-SSM：一种多尺度状态空间模型框架，通过多分辨率处理提升序列建模能力，解决传统SSM内存有限和难以捕捉多尺度依赖的问题。


<details>
  <summary>Details</summary>
Motivation: 传统状态空间模型（SSMs）虽然计算效率高，但存在有效内存有限的问题，需要更大的状态尺寸来改善记忆能力。此外，现有SSMs难以捕捉多尺度依赖关系，而这对于建模时间序列、图像和自然语言中的复杂结构至关重要。

Method: 提出多尺度SSM框架，通过在不同分辨率上表示序列动态，并使用专门的状态空间动态处理每个分辨率。引入输入相关的尺度混合器，实现跨分辨率的动态信息融合，从而同时捕捉细粒度高频模式和粗粒度全局趋势。

Result: 在Long Range Arena、层次推理、时间序列分类和图像识别等多个基准测试中，MS-SSM持续优于先前的SSM模型，特别是在长距离和层次化任务中表现突出，同时保持了计算效率。

Conclusion: 多分辨率处理显著增强了状态空间架构的序列建模能力，MS-SSM框架通过有效捕捉多尺度依赖关系，在提升内存效率和长距离建模方面取得了重要进展。
...
### [158] [Exploiting the Prior of Generative Time Series Imputation](https://arxiv.org/abs/2512.23832)
...
Method: 提出Bridge-TS方法：1）专家先验：使用预训练的transformer模块作为专家进行确定性估计，将结果作为真实目标的先验；2）组合先验：利用多个预训练模型提供不同估计结果，在数据到数据生成过程中组合它们，实现组合先验到目标的插补过程。

Result: 在ETT、Exchange、Weather等多个基准数据集上的实验表明，Bridge-TS在均方误差和平均绝对误差方面达到了新的插补精度记录。

Conclusion: 通过改进先验设计，Bridge-TS在生成式时间序列插补中取得了优越性能，证明了为生成式时间序列插补改进先验的有效性。
...
### [159] [Trellis: Learning to Compress Key-Value Memory in Attention Models](https://arxiv.org/abs/2512.23852)
*Mahdi Karami,Ali Behrouz,Praneeth Kacham,Vahab Mirrokni*

Main category: cs.LG

TL;DR: Trellis是一种新型Transformer架构，通过固定大小内存和两阶段循环压缩机制解决传统Transformer的二次计算复杂度和KV缓存无限增长问题。
...
Method: Trellis用固定大小内存替换标准KV缓存，训练两阶段循环压缩机制来存储新的键值对。采用带有遗忘门的在线梯度下降程序，使压缩内存能够递归更新，同时学习在测试时保留来自输入令牌的重要上下文信息。

Result: 在语言建模、常识推理、回忆密集型任务和时间序列上的广泛实验表明，该架构优于强基线模型。随着序列长度增加，其性能优势更加明显，突显了其在长上下文应用中的潜力。

Conclusion: Trellis通过有界内存和动态压缩机制有效解决了Transformer的计算和内存瓶颈，特别适合长序列处理应用，为高效的长上下文建模提供了有前景的解决方案。
...
### [160] [Flow Matching Neural Processes](https://arxiv.org/abs/2512.23853)
...
<details>
  <summary>Details</summary>
Motivation: 神经过程模型能够直接从数据中学习随机过程，但现有方法在实现复杂度和采样效率方面存在局限。流匹配在多种数据模态上表现出色，将其应用于神经过程可以简化实现并提高性能。

Method: 基于流匹配构建神经过程模型，遵循NP训练框架，通过ODE求解器进行条件分布采样，无需辅助条件方法。模型提供通过ODE步数控制精度与运行时间的权衡。

Result: 在合成1D高斯过程数据、2D图像和真实世界天气数据等多个基准测试中，该模型超越了先前最先进的神经过程方法。

Conclusion: 基于流匹配的神经过程模型实现简单，采样高效，在准确性和计算效率之间提供可控权衡，为随机过程建模提供了有效的新方法。
...
### [161] [Yggdrasil: Bridging Dynamic Speculation and Static Runtime for Latency-Optimal Tree-Based LLM Decoding](https://arxiv.org/abs/2512.23858)
...
TL;DR: Yggdrasil是一个通过上下文感知树状草稿和编译器友好执行实现延迟最优推测解码的系统，相比现有基线最高可获得3.98倍加速。
...
Method: Yggdrasil采用协同设计方法，引入等增长树结构保持静态图兼容性，使用延迟感知优化目标进行草稿选择，并采用阶段化调度减少开销。

Result: Yggdrasil在多种硬件配置下实现了最高3.98倍的加速，优于现有最先进的基线方法，并且支持未经修改的LLM模型。
...
### [162] [Probing the Limits of Compressive Memory: A Study of Infini-Attention in Small-Scale Pretraining](https://arxiv.org/abs/2512.23862)
*Ruizhe Huang,Kexuan Zhang,Yihao Fang,Baifeng Yu*

Main category: cs.LG

TL;DR: Infini-attention在小语言模型(SLMs)中通过压缩记忆机制增强长上下文处理能力，在有限参数下实现比基线高31%的检索准确率


<details>
  <summary>Details</summary>
Motivation: 研究小规模预训练的小语言模型，旨在利用有限数据和计算资源，提高低资源环境下的可访问性并降低成本，同时解决紧凑模型的长上下文外推问题

Method: 使用Infini-attention架构，从过去片段构建压缩记忆同时保留局部注意力，对300M参数的LLaMA模型进行预训练实证研究

Result: 模型训练稳定，在长上下文检索中优于基线；平衡因子是关键性能因素；重复记忆压缩会降低检索准确率；在16,384标记上下文下，Infini-attention模型比基线准确率高31%

Conclusion: Infini-attention等架构记忆机制有助于小语言模型获得稳健的长上下文能力，有效补偿SLM的有限参数限制
...
### [163] [Max-Entropy Reinforcement Learning with Flow Matching and A Case Study on LQR](https://arxiv.org/abs/2512.23870)
*Yuyang Zhang,Yang Hu,Bo Dai,Na Li*

Main category: cs.LG

TL;DR: 提出一种基于流模型的SAC算法变体，使用重要性采样流匹配（ISFM）在线更新策略，在最大熵线性二次调节器问题上验证了学习最优动作分布的能力。


<details>
  <summary>Details</summary>
Motivation: 传统SAC算法为了效率通常使用简单的策略类别来近似基于能量的策略，牺牲了表达能力和鲁棒性。需要更强大的策略参数化方法来提升SAC算法的性能。

Method: 提出基于流模型的SAC算法变体：1）使用流模型参数化策略，利用其丰富的表达能力；2）采用瞬时变量变换技术评估流策略；3）提出重要性采样流匹配（ISFM）在线更新策略，仅需从用户指定的采样分布中采样而非未知的目标分布。

Result: 1）开发了ISFM的理论分析，表征不同采样分布选择对学习效率的影响；2）在最大熵线性二次调节器问题上进行案例研究，证明所提算法能够学习到最优动作分布。

Conclusion: 通过将流模型集成到SAC框架中，结合ISFM在线更新方法，实现了更强大的策略表达能力，在保持算法效率的同时提升了性能，为最大熵强化学习提供了新的有效方法。
...
### [164] [Efficient Deep Learning for Short-Term Solar Irradiance Time Series Forecasting: A Benchmark Study in Ho Chi Minh City](https://arxiv.org/abs/2512.23898)
*Tin Hoang*

Main category: cs.LG

TL;DR: 该研究对10种深度学习架构进行了全球水平辐照度(GHI)短期预测基准测试，发现Transformer表现最佳(R²=0.9696)，并通过知识蒸馏压缩模型23.5%同时降低误差，为边缘设备部署提供了可行方案。


<details>
  <summary>Details</summary>
Motivation: 太阳能发电的波动性对电网稳定性构成挑战，需要可靠的GHI预测来缓解这种变异性。现有研究缺乏对不同深度学习架构在GHI预测方面的系统性比较，特别是新兴架构如Transformer、Mamba等的表现尚未充分评估。

Method: 使用胡志明市2011-2020年高分辨率NSRDB卫星数据，对10种深度学习架构进行1小时前GHI时间序列预测基准测试。包括传统基线(LSTM、TCN)和新兴架构(Transformer、Informer、iTransformer、TSMixer、Mamba)。采用SHAP分析对比不同架构的时间推理模式，并使用知识蒸馏技术压缩高性能模型。

Result: Transformer架构表现最佳，R²达到0.9696。SHAP分析显示Transformer具有"近期偏向"，专注于即时大气条件，而Mamba则明确利用24小时周期性依赖。知识蒸馏成功将Transformer模型压缩23.5%，同时降低预测误差(MAE: 23.78 W/m²)。

Conclusion: Transformer在GHI短期预测中表现最优，知识蒸馏技术为在资源受限的边缘设备上部署复杂预测模型提供了有效途径。不同架构的时间推理模式存在显著差异，这为特定应用场景的模型选择提供了指导。
...
Method: 引入阶段式成对混合器(SPM)，通过稀疏成对混合阶段的组合实现全局线性变换，提供正交保范旋转和通用2×2混合两种参数化变体。

Result: 在结构化学习问题上显著降低计算成本并提高准确性，在真实世界基准测试中保持竞争力，同时提供显式的组合归纳偏置。

Conclusion: SPM作为密集线性层的有效替代方案，在计算效率、参数效率和泛化性能方面均有优势，特别适用于与任务结构对齐的场景。
...
### [166] [Constraint Breeds Generalization: Temporal Dynamics as an Inductive Bias](https://arxiv.org/abs/2512.23916)
*Xia Chen*

Main category: cs.LG

TL;DR: 论文提出物理约束（如代谢限制）可作为时间归纳偏置促进泛化，通过相空间分析揭示扩张动态放大噪声而耗散动态压缩相空间，强制提取不变特征，在临界"过渡"机制下最大化泛化能力


<details>
  <summary>Details</summary>
Motivation: 传统深度学习关注无约束优化，但生物系统在严格代谢约束下运行。作者认为这些物理约束不是限制，而是塑造动态的时间归纳偏置，能够促进泛化能力

Method: 通过信号传播的相空间分析，揭示扩张动态与耗散动态的基本不对称性；提出可通过外部输入编码或内在网络动态施加约束；需要具备时间整合能力的架构来解码诱导的不变特征

Result: 在监督分类、无监督重建和零样本强化学习的综合评估中，证明临界"过渡"机制能最大化泛化能力；静态架构无法利用时间结构

Conclusion: 动态约束构成一类独特的归纳偏置，稳健AI发展不仅需要扩展和消除限制，还需要计算性地掌握自然促进泛化的时间特性
...
<details>
  <summary>Details</summary>
Motivation: 传统机器学习依赖大量标注数据或在线交互，但在实践中获取高质量标签或通过试错决策成本高、耗时长或风险大，特别是在大规模或高风险场景中

Method: 研究交互式机器学习，让学习器主动影响信息收集或行动选择，基于过去观察指导未来交互。开发新算法原理并建立交互学习的基本限制，涵盖三个维度：带噪声数据和丰富模型类的主动学习、大动作空间的序列决策、部分反馈下的模型选择

Result: 实现了首个无需低噪声假设就能实现指数级标签节省的计算高效主动学习算法；首个保证与动作空间大小无关的高效通用上下文赌博机算法；以及序列决策中模型选择基本成本的首次严格刻画

Conclusion: 通过开发统计最优且计算高效的算法，推进了交互式机器学习的理论基础，同时为在大规模实际场景中部署交互学习方法提供了原则性指导
...
TL;DR: 论文提出两种处理类别不平衡的损失函数家族：GLA和GCA，理论分析显示GCA在高度不平衡场景下具有更好的理论保证，实验验证两者均优于传统方法。
...
Method: 提出两种损失函数家族：1) GLA损失：将Logit-Adjusted损失推广到一般交叉熵家族；2) GCA损失：扩展标准类别加权损失，加入类别相关的置信度边界并推广到一般交叉熵家族。

Result: 理论分析显示：GLA损失是贝叶斯一致的，但仅对完整假设集是H一致的，其边界与最小类别概率成反比；GCA损失对所有假设集都是H一致的，边界缩放为1/√p_min，在不平衡场景下提供更强理论保证。实验表明两者均优于传统方法，GLA在常见基准上略优，GCA在高度不平衡场景下略优。

Conclusion: GCA损失在高度不平衡设置中提供更优的理论保证，两种方法都能有效处理类别不平衡问题，为实际应用提供了理论指导和实用工具。
...
<details>
  <summary>Details</summary>
Motivation: 量化CNN容易受到提取攻击导致IP盗窃，现有防御方法存在局限：1) 仅作为训练后附加措施而非集成到模型设计中；2) 计算成本高；3) 对受害者模型有不切实际的假设；4) 不适用于量化模型。

Method: 提出DivQAT算法，基于量化感知训练修改量化过程，将模型提取防御直接集成到训练流程中，而不是作为后处理措施。

Result: 在基准视觉数据集上的实证验证表明，该技术能有效防御模型提取攻击，同时不损害模型准确性。与其他防御机制结合时，相比传统QAT能进一步提高防御效果。
...
### [170] [Physics-informed Graph Neural Networks for Operational Flood Modeling](https://arxiv.org/abs/2512.23964)
*Carlo Malapad Acosta,Herath Mudiyanselage Viraj Vidura Herath,Jia Yu Lim,Abhishek Saha,Sanka Rasnayaka,Lucy Marshall*

Main category: cs.LG

TL;DR: 提出DUALFloodGNN，一种结合全局和局部物理约束的图神经网络洪水模型，通过共享消息传递框架同时预测节点水量和边流量，采用多步损失和动态课程学习提升自回归推理性能。


<details>
  <summary>Details</summary>
Motivation: 传统物理洪水模型计算成本高，难以满足快速预测需求；图神经网络虽快但缺乏物理可解释性。需要开发既能保持计算效率又能嵌入物理约束的洪水预测模型。

Method: 设计DUALFloodGNN架构，在全局和局部尺度嵌入物理约束作为显式损失项；通过共享消息传递框架同时预测节点水量和边流量；采用多步损失和动态课程学习训练模型以提升自回归推理性能。

Result: 相比标准GNN架构和最先进的GNN洪水模型，DUALFloodGNN在预测多个水文变量方面取得显著改进，同时保持高计算效率。模型已在GitHub开源。

Conclusion: DUALFloodGNN成功结合了物理约束和神经网络优势，为快速准确的洪水预测提供了有效解决方案，在计算效率和预测精度之间取得了良好平衡。
...
TL;DR: DataFlow是一个用于在无限时间序列数据上构建、测试和部署高性能机器学习系统的计算框架，通过基于DAG的统一执行模型和点对点幂等性保证，解决了传统批处理到流式生产系统转换中的问题。


<details>
  <summary>Details</summary>
Motivation: 传统数据科学工作流假设有限数据集，从批处理原型迁移到流式生产系统时需要大量重新实现，这会导致因果关系违反、批处理边界伪影和实时故障难以复现等问题。

Method: 采用基于有向无环图（DAG）的统一执行模型，具有点对点幂等性：任何时间t的输出仅依赖于t之前的固定长度上下文窗口。框架通过自动跟踪所有转换中的知识时间来强制执行严格因果关系，并支持跨时间和特征维度的灵活分块。

Result: DataFlow确保在批处理模式下开发的模型无需代码更改即可在流式生产中相同执行，消除了未来窥视错误，并在金融交易、物联网、欺诈检测和实时分析等多个领域展示了有效性。
...
TL;DR: 论文提出一个基于运筹学的保障自主性框架，通过流式生成模型和对抗鲁棒性方法解决生成式AI在自主系统中的脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 生成式AI正从对话助手转向自主决策系统，这产生了自主性悖论：系统自主性越高，越需要正式结构、明确约束和尾部风险控制。随机生成模型在操作领域可能很脆弱，需要可验证的可行性、分布偏移鲁棒性和压力测试机制。

Method: 提出基于运筹学的保障自主性概念框架，包含两种互补方法：1) 流式生成模型将生成视为确定性传输，实现可审计性、约束感知生成；2) 通过对抗鲁棒性视角制定操作安全性，在不确定性集合内评估最坏情况扰动。

Result: 框架阐明了自主性增加如何改变运筹学的角色：从求解器到护栏再到系统架构师，负责控制逻辑、激励协议、监控机制和安全边界的设计。

Conclusion: 该框架为安全关键、可靠性敏感的操作领域中的保障自主性定义了研究议程，强调需要将运筹学方法整合到生成式AI自主系统中以确保可靠性和安全性。
...
TL;DR: 本文提出了一种基于信息论的嵌入质量评估新指标ERPM，通过分析邻域矩阵的奇异值谱熵来量化信息损失，相比传统距离或几何方法能更全面地评估降维效果。


<details>
  <summary>Details</summary>
Motivation: 现有嵌入评估指标（如应力、基于排名的邻域标准、局部Procrustes）主要关注距离或局部几何的失真，但未能直接评估高维数据投影到低维空间时信息保留的程度。需要一种从信息论角度直接量化信息损失的评估方法。

Method: 提出熵秩保持度量（ERPM），这是一种基于邻域矩阵奇异值谱的香农熵和稳定秩的局部度量。ERPM量化原始表示与其降维投影之间的不确定性变化，提供邻域级指标和全局汇总统计。

Result: 通过金融时间序列和文献中常见流形数据进行验证，发现基于距离的标准与几何和谱度量相关性很低，而ERPM与局部Procrustes显示出强平均相关性，但在局部区域存在显著差异。ERPM能识别信息损失严重的邻域。

Conclusion: ERPM通过识别信息损失严重的邻域，补充了现有评估指标，特别是在构建早期预警指标等对信息敏感的应用中，能够实现更全面的嵌入评估。
...
TL;DR: CLEAR-HUG是一个两阶段心电图自监督学习框架，通过捕捉导联间细微传导差异并遵循临床诊断流程，提升心电图表征学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有心电图自监督学习方法存在两个关键局限：1）只关注导联和心跳间的一致性模式，忽略了心脏传导过程中固有的心跳差异，而这些细微变化携带独特的生理特征；2）未能遵循心电图诊断指南的渐进逻辑（从单个心跳到单导联再到导联组合），导致预训练模型与下游任务不匹配。

Method: 提出两阶段框架CLEAR-HUG：第一阶段为CLEAR（Conduction-LEAd Reconstructor）自监督模型，通过稀疏注意力机制将每个心跳视为独立实体进行信号重建，捕捉心跳间的特定变化和一般共性；第二阶段为HUG（Hierarchical lead-Unified Group head）疾病诊断模块，模拟临床工作流程进行分层诊断。

Result: 在六个任务上的实验结果显示性能提升了6.84%，验证了CLEAR-HUG在增强心脏传导表征和使模式与专家诊断指南对齐方面的有效性。

Conclusion: CLEAR-HUG通过捕捉导联间细微传导差异并遵循临床诊断流程，成功提升了心电图表征学习效果，为心电图分析提供了更符合临床实践的解决方案。
...
### [175] [Hyperspherical Graph Representation Learning via Adaptive Neighbor-Mean Alignment and Uniformity](https://arxiv.org/abs/2512.24062)
*Rui Chen,Junjun Guo,Hongbin Wang,Yan Xiang,Yantuan Xian,Zhengtao Yu*

Main category: cs.LG

TL;DR: HyperGRL：一种通过自适应邻居均值对齐和无采样均匀性实现超球面图表示学习的统一框架，在节点分类、聚类和链接预测任务上优于现有方法。
...
Method: 提出HyperGRL框架，将节点嵌入到单位超球面上，通过两个对抗耦合的目标：邻居均值对齐和无采样均匀性。对齐目标使用节点局部邻域的均值表示构建语义基础稳定的目标；均匀性目标通过L2超球面正则化实现分散，鼓励全局均匀嵌入分布。还引入熵引导的自适应平衡机制动态调节对齐和均匀性之间的相互作用。

Result: 在节点分类、节点聚类和链接预测任务上的广泛实验表明，HyperGRL在表示质量和泛化能力方面优于现有方法，平均分别提升1.49%、0.86%和0.74%。

Conclusion: 基于几何基础的无采样对比目标对图表示学习是有效的，HyperGRL框架能够提供更优的表示质量和泛化能力。
...
### [176] [How and Why LLMs Generalize: A Fine-Grained Analysis of LLM Reasoning from Cognitive Behaviors to Low-Level Patterns](https://arxiv.org/abs/2512.24063)
*Haoyue Bai,Yiyou Sun,Wenjie Hu,Shi Qiu,Maggie Ziyu Huan,Peiyang Song,Robert Nowak,Dawn Song*

Main category: cs.LG

TL;DR: 该论文通过引入一个分解推理为原子核心技能的新基准，揭示了SFT和RL调优在LLM泛化行为上的差异：SFT容易导致技能崩溃和过拟合，而RL能保持更稳定的推理能力。


<details>
  <summary>Details</summary>
Motivation: LLMs在监督微调(SFT)和强化学习(RL)调优后表现出截然不同的泛化行为：SFT往往缩小能力范围，而RL倾向于保持能力。现有研究主要依赖粗粒度的准确率指标，无法深入理解这种差异的原因。

Method: 1. 引入新基准，将推理分解为原子核心技能（计算、事实检索、模拟、枚举、诊断）；2. 结合低层统计模式分析（分布差异、参数统计）；3. 开发元探测框架追踪不同训练阶段模型行为；4. 在数学、科学推理和非推理任务上对比SFT和RL的泛化演化。

Result: RL调优的模型保持更稳定的行为特征，抵抗推理技能崩溃；SFT模型表现出更尖锐的漂移，容易过拟合到表面模式。新基准提供了对LLM推理本质更细粒度的理解。

Conclusion: 该工作为LLM推理本质提供了新见解，并指出了设计促进广泛、稳健泛化的训练策略的原则。通过原子技能分解和细粒度分析，揭示了不同调优方法对模型认知能力演化的影响。
...
### [177] [Time-varying Mixing Matrix Design for Energy-efficient Decentralized Federated Learning](https://arxiv.org/abs/2512.24069)
*Xusheng Zhang,Tuan Nguyen,Ting He*

Main category: cs.LG

TL;DR: 设计去中心化联邦学习的混合矩阵以最小化无线网络中节点的最大能耗，通过时变通信拓扑和多阶段框架平衡能耗与收敛速度


<details>
  <summary>Details</summary>
Motivation: 现有混合矩阵设计主要关注最小化通信时间，而忽略了能量受限设备的关键需求——最小化节点能耗。无线通信的广播特性未被充分利用，需要设计能平衡能耗和收敛速度的混合矩阵

Method: 提出一个多阶段设计框架：基于允许任意时变混合矩阵的新收敛定理，激活时变通信拓扑并在优化预算下权衡每次迭代的能耗与收敛速度，同时平衡节点间的能耗分布

Result: 基于真实数据的评估验证了所提方案的有效性，能够结合稀疏混合矩阵的低能耗和密集混合矩阵的快速收敛优势

Conclusion: 该工作填补了去中心化联邦学习中混合矩阵设计的空白，通过理论支持的时变拓扑设计，在考虑无线通信广播特性的前提下，有效最小化了节点最大能耗并平衡了能耗与收敛速度
...
### [178] [Multi-Scenario Highway Lane-Change Intention Prediction: A Temporal Physics-Informed Multi-Modal Framework](https://arxiv.org/abs/2512.24075)
*Jiazhao Shi,Ziyu Wang,Yichen Lin,Shoufeng Lu*

Main category: cs.LG

TL;DR: 提出TPI-AI混合框架，融合深度时序表示与物理启发交互特征，用于车道变换意图预测，在高速公路和匝道丰富场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 车道变换意图预测对自动驾驶安全至关重要，但在自然交通中面临挑战：噪声运动学数据、严重类别不平衡、异构高速公路场景泛化能力有限。

Method: 提出TPI-AI混合框架：使用双向LSTM编码器学习轨迹历史时序表示，结合物理启发特征（车距、碰撞时间、安全间隙等），用LightGBM分类器进行三分类（不变道、左变道、右变道），采用不平衡感知优化（重采样/加权和阈值校准）。

Result: 在两个大规模无人机数据集（highD和exiD）上评估，TPI-AI优于单独LightGBM和Bi-LSTM基线，在1、2、3秒预测时域上分别达到宏平均F1分数：highD为0.9562、0.9124、0.8345，exiD为0.9247、0.8197、0.7605。

Conclusion: 物理启发交互特征与学习到的时序嵌入相结合，能够实现鲁棒的多场景车道变换意图预测。
...
### [179] [Autoregressivity in the Latent Space of a GP-VAE Language Model: An Empirical Ablation Study](https://arxiv.org/abs/2512.24102)
*Yves Ruffenach*

Main category: cs.LG

TL;DR: 对GP-VAE模型中潜在自回归的消融分析：潜在自回归能产生更符合高斯过程先验的轨迹和更好的长程稳定性，但并非新架构提案


<details>
  <summary>Details</summary>
Motivation: 传统语言模型在token层面进行自回归分解，而作者先前工作将序列结构转移到潜在空间。本文旨在通过系统消融研究，分析潜在自回归在GP-VAE模型中的作用和重要性。
...
Conclusion: 潜在自回归在GP-VAE模型中起着重要作用，能产生更结构化的潜在表示和稳定的长程行为。本文是表示结构的实证分析，而非新架构提案，强调了潜在自回归作为组织长程结构的有效机制。
...
### [180] [Enhancing LLM Planning Capabilities through Intrinsic Self-Critique](https://arxiv.org/abs/2512.24103)
...
Method: 采用few-shot学习技术并逐步扩展到many-shot方法作为基础，然后通过迭代的校正和精炼过程进行自我批判，实现内在自我改进

Result: 在Blocksworld、Logistics和Mini-grid数据集上取得显著性能提升，超越了强基线准确率，在2024年10月的LLM检查点模型中实现了新的SOTA

Conclusion: 自我批判方法能够显著提升LLM的规划性能，该方法具有内在自我改进能力，不依赖于特定模型版本，未来应用于更复杂的搜索技术和更强大的模型将带来更好性能
...
### [181] [OptRot: Mitigating Weight Outliers via Data-Free Rotations for Post-Training Quantization](https://arxiv.org/abs/2512.24124)
*Advait Gadhikar,Riccardo Grazzi,James Hensman*

Main category: cs.LG

TL;DR: 提出OptRot方法，通过学习可融合的旋转来减少LLM权重中的异常值，优化量化性能，在W4A8设置中表现优于现有方法


<details>
  <summary>Details</summary>
Motivation: 大型语言模型权重和激活中存在异常值，导致量化困难。现有方法使用旋转来缓解异常值，但需要更有效的方法

Method: 提出OptRot方法，通过最小化旋转权重的四阶幂来减少权重异常值；还提出数据依赖的OptRot⁺方法，结合激活协方差信息进一步优化

Result: OptRot在权重量化上优于Hadamard旋转、SpinQuant和OSTQuant等方法，在W4A8设置中也改善了激活量化。但在W4A4设置中表现较差，显示权重和激活量化之间的权衡

Conclusion: OptRot是一种有效的旋转方法，能显著改善LLM量化性能，特别是在W4A8设置中。权重和激活量化之间存在权衡关系，需要根据具体场景选择合适的方法
...
### [182] [GARDO: Reinforcing Diffusion Models without Reward Hacking](https://arxiv.org/abs/2512.24138)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型通过在线强化学习微调可以提升文本-图像对齐，但由于视觉任务难以精确指定真实目标，模型通常使用部分捕捉真实目标的代理奖励进行优化。这种不匹配导致奖励黑客问题（代理分数上升但真实图像质量下降、生成多样性崩溃），而现有正则化方法又会影响样本效率和探索能力。

Method: 提出GARDO框架：1）选择性正则化：仅对高不确定性样本进行惩罚；2）自适应正则化机制：定期更新参考模型以匹配在线策略能力；3）多样性感知优化：对高质量且高多样性的样本放大奖励，鼓励模式覆盖。

Result: 在多种代理奖励和未见指标上的实验表明，GARDO能有效缓解奖励黑客问题，增强生成多样性，同时不牺牲样本效率或探索能力，展现了其有效性和鲁棒性。

Conclusion: GARDO通过选择性正则化、自适应参考模型更新和多样性感知奖励放大，解决了扩散模型RL微调中的核心挑战，在保持样本效率和探索能力的同时，有效缓解了奖励黑客和模式崩溃问题。
...
### [183] [Colorful Pinball: Density-Weighted Quantile Regression for Conditional Guarantee of Conformal Prediction](https://arxiv.org/abs/2512.24139)
*Qianyi Chen,Bo Li*

Main category: cs.LG

TL;DR: 提出一种改进条件覆盖率的共形预测方法，通过优化分位数回归组件，使用密度加权的pinball损失函数，并设计三头分位数网络来估计权重，显著提升条件覆盖性能。


<details>
  <summary>Details</summary>
Motivation: 虽然共形预测能提供稳健的边缘覆盖保证，但为特定输入实现可靠的条件覆盖仍然具有挑战性。尽管在有限样本下无法实现精确的无分布条件覆盖，但现有研究致力于改进标准共形程序的条件覆盖性能。

Method: 通过泰勒展开推导出分位数回归的替代目标函数：密度加权的pinball损失，其中权重由在真实分位数处评估的符合度得分的条件密度给出。提出三头分位数网络，通过使用辅助分位数级别(1-α±δ)的有限差分来估计这些权重，然后通过优化加权损失来微调中心分位数。

Result: 提供了具有精确非渐近保证的理论分析，表征了由此产生的超额风险。在多样化的高维真实世界数据集上进行广泛实验，展示了条件覆盖性能的显著改进。

Conclusion: 该方法通过直接最小化条件覆盖的均方误差，显著提升了共形预测的条件覆盖性能，为改进分位数回归组件提供了一种有效的新方法。
...
### [184] [Paired Seed Evaluation: Statistical Reliability for Learning-Based Simulators](https://arxiv.org/abs/2512.24145)
*Udit Sharma*

Main category: cs.LG

TL;DR: 论文提出配对种子评估设计，通过让竞争系统在相同随机种子下运行来减少评估方差，提高统计效率和可靠性。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统虽然看似随机，但实际上是确定性随机（由种子伪随机数生成器控制）。基于学习的模拟器广泛用于算法比较，但由于随机初始化和学习随机性，评估结果方差很大。标准独立评估设计未能利用不同方案间共享的随机源。

Method: 提出配对种子评估设计：让竞争系统在相同的随机种子下进行评估，诱导随机组件的匹配实现。当结果在种子层面正相关时，这种方法能严格减少方差。该方法在存在相关性时提高统计可靠性，在无相关性时退化为独立评估而不损失有效性。

Result: 配对种子评估能产生更紧的置信区间、更高的统计功效，在固定计算预算下获得有效的样本量增益。实证表明，种子层面的相关性通常很大且为正，能产生数量级的效率提升。
...
### [185] [Micro-Macro Tensor Neural Surrogates for Uncertainty Quantification in Collisional Plasma](https://arxiv.org/abs/2512.24205)
...
<details>
  <summary>Details</summary>
Motivation: 等离子体动力学方程对微观扰动高度敏感，传统不确定性量化方法面临计算成本高、相空间维度高、多尺度刚度等挑战，特别是在包含碰撞项时，高维非局部碰撞积分和守恒性质约束更加严格

Method: 结合高保真渐近保持VPL求解器与基于VPFP和EP方程的廉价相关代理模型；提出基于各向异性微观-宏观分解的张量神经网络（SPINN的推广），降低速度矩成本；校准VPFP模型并设计渐近保持SPINN，在小和大Knudsen数极限下分别恢复EP和VP系统

Result: 相比标准蒙特卡洛方法实现显著方差缩减，用更少的高保真样本获得准确统计量，降低计算时间，同时保持对随机维度的鲁棒性

Conclusion: 该方差缩减蒙特卡洛框架为Vlasov-Poisson-Landau系统的不确定性量化提供了高效可靠的解决方案，通过神经网络代理和渐近保持设计克服了传统方法的计算挑战
...
TL;DR: 开发了四种基于心率数据的机器学习算法，通过遗传算法优化架构，用于可穿戴设备上的脓毒症早期预测，预测窗口为1-4小时。


<details>
  <summary>Details</summary>
Motivation: 脓毒症导致高死亡率、发病率和医疗成本，及时预测对早期干预至关重要。现有模型主要针对ICU患者，缺乏非病房环境下的早期检测方法。

Method: 开发了四种机器学习算法，通过遗传算法优化模型架构，平衡性能、计算复杂度和内存需求。使用心率数据进行预测，预测窗口为1小时，并通过迁移学习扩展到4小时。

Result: 获得了令人鼓舞的结果，表明这些模型在可穿戴设备上具有实施可行性，能够准确监测心率并预测脓毒症发作。

Conclusion: 可穿戴技术有潜力在ICU和病房之外的环境中进行早期脓毒症检测，为脓毒症管理提供了新的可能性。
...
Method: 提出SaM2B框架：1) 利用环境视觉、飞行姿态和地理空间数据等轻量级线索，通过可靠性感知动态权重更新，在不同时间点自适应分配各模态贡献；2) 通过跨模态对比学习，将与特定波束信息相关的"多源表示波束语义"对齐到共享语义空间，增强判别能力和模态噪声及分布偏移下的鲁棒性。

Result: 在真实低空无人机数据集上的实验表明，SaM2B相比基线方法取得了更满意的结果。

Conclusion: SaM2B通过动态权重分配和语义对齐，有效解决了多模态波束预测中的模态可靠性波动和泛化问题，为低空经济中的无人机通信提供了更可靠的连接方案。
...
### [188] [Tubular Riemannian Laplace Approximations for Bayesian Neural Networks](https://arxiv.org/abs/2512.24381)
*Rodrigo Pereira David*

Main category: cs.LG

TL;DR: TRL是一种新的贝叶斯近似方法，通过建模概率管状结构来适应深度神经网络的高度各向异性损失曲面，在保持单模型效率的同时达到集成模型的可靠性。
...
Method: 提出管状黎曼拉普拉斯近似，将后验建模为沿着由函数对称性诱导的低损失谷的概率管，使用Fisher/高斯-牛顿度量将先验主导的切向不确定性与数据主导的横向不确定性分离。

Result: 在ResNet-18（CIFAR-10和CIFAR-100）上的实验表明，TRL实现了优秀的校准性能，在ECE指标上匹配或超过了深度集成的可靠性，同时只需要1/5的训练成本。

Conclusion: TRL有效地弥合了单模型效率与集成级可靠性之间的差距，是一种可扩展的重新参数化高斯近似方法，利用隐式曲率估计在高维参数空间中运行。
...
### [189] [Lifting Vision: Ground to Aerial Localization with Reasoning Guided Planning](https://arxiv.org/abs/2512.24404)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态智能系统主要依赖文本信息进行推理，限制了在视觉导航、地理定位等空间任务中的有效性。文本推理难以理解空间依赖关系和几何关系，需要新的视觉推理范式。
...
Conclusion: 视觉推理是导航和定位任务的强有力补充方法，可以在无需实时GPS数据的情况下执行此类任务，提供更安全的导航解决方案，为空间智能系统开辟了新方向。
...
### [190] [Efficient Inference for Inverse Reinforcement Learning and Dynamic Discrete Choice Models](https://arxiv.org/abs/2512.24407)
*Lars van der Laan,Aurelien Bibaut,Nathan Kallus*

Main category: cs.LG

TL;DR: 提出了一种半参数去偏逆强化学习框架，用于在最大熵IRL和Gumbel-shock DDC模型中实现统计高效推断，支持灵活的非参数估计同时保持√n一致性和渐近正态性。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：灵活的IRL方法依赖机器学习但缺乏有效推断保证，而经典DDC方法需要限制性参数设定并经常需要重复动态规划。需要一种既能利用现代机器学习工具又能提供统计推断保证的统一框架。

Method: 开发半参数去偏逆强化学习框架，将行为策略的对数作为伪奖励来识别策略价值差异和奖励函数。将目标形式化为行为策略和转移核的光滑泛函，建立路径可微性并推导有效影响函数。构建自动去偏机器学习估计器，允许灵活非参数估计干扰项。

Result: 提出的估计器实现了√n一致性、渐近正态性和半参数效率，将经典DDC模型的推断扩展到非参数奖励和现代机器学习工具，提供了IRL中统计推断的统一且计算可行的途径。

Conclusion: 该框架统一了IRL和DDC模型的统计推断，既保持了经典方法的统计保证，又允许使用灵活的机器学习模型，为序列决策分析提供了计算可行且统计有效的解决方案。
...
### [191] [Sparse classification with positive-confidence data in high dimensions](https://arxiv.org/abs/2512.24443)
*The Tien Mai,Mai Anh Nguyen,Trung Nghia Nguyen*

Main category: cs.LG

TL;DR: 提出用于高维Pconf分类的稀疏惩罚框架，使用凸和非凸惩罚项解决收缩偏差并改进特征恢复，理论证明达到近极小极大最优稀疏恢复率，实验显示性能接近全监督方法。
...
Method: 提出稀疏惩罚框架，使用凸惩罚（Lasso）和非凸惩罚（SCAD、MCP）解决收缩偏差问题。开发高效近端梯度算法求解复合目标函数。

Result: 理论证明L1正则化Pconf估计器在受限强凸条件下达到近极小极大最优稀疏恢复率。实验显示所提方法在预测性能和变量选择准确性方面与全监督方法相当。

Conclusion: 成功弥合了弱监督与高维统计之间的差距，为高维Pconf分类提供了有效的稀疏正则化解决方案，性能接近全监督方法。
...
### [192] [Adaptive Learning Guided by Bias-Noise-Alignment Diagnostics](https://arxiv.org/abs/2512.24445)
*Akash Samanta,Sheldon Williamson*

Main category: cs.LG

TL;DR: 提出一个诊断驱动的自适应学习框架，将误差演化分解为偏差、噪声和对齐三个部分，用于稳定监督优化、强化学习和元学习。
...
Method: 提出偏差-噪声-对齐分解框架：偏差捕捉持续漂移，噪声捕捉随机变异，对齐捕捉重复方向激励导致的超调。通过轻量级统计在线计算这些诊断信号，独立于模型架构和任务领域。

Result: 该框架为监督优化、演员-评论家强化学习和学习优化器提供了统一控制骨干，在标准平滑性假设下建立了有界有效更新和稳定性保证。
...
### [193] [Generative forecasting with joint probability models](https://arxiv.org/abs/2512.24446)
*Patrick Wyrod,Ashesh Chattopadhyay,Daniele Venturi*

Main category: cs.LG

TL;DR: 将混沌系统预测重构为完全生成问题，通过学习滞后系统状态的联合概率分布进行预测，相比传统条件预测方法在短期预测、吸引子保持和长期统计行为方面表现更好。
...
Method: 将预测重构为完全生成问题，学习短时间窗口内滞后系统状态的联合概率分布，通过边缘化获得预测。提出模型无关的训练和推理框架，使用三种不确定性量化指标评估预测稳健性。

Result: 在Lorenz-63系统和Kuramoto-Sivashinsky方程上评估，联合生成模型相比传统条件下一步模型：1）短期预测技能提升；2）保持吸引子几何结构；3）长期统计行为更准确。

Conclusion: 将预测重构为联合生成问题能更好地捕捉非线性时间依赖，表示多步轨迹段，产生与学习分布一致的预测，为混沌系统预测提供了更可靠的方法。
...
### [194] [HOLOGRAPH: Active Causal Discovery via Sheaf-Theoretic Alignment of Large Language Model Priors](https://arxiv.org/abs/2512.24478)
*Hyunjun Kim*

Main category: cs.LG

TL;DR: HOLOGRAPH框架使用层理论将LLM引导的因果发现形式化，通过代数潜在投影处理隐藏混杂因子，在50-100变量任务上实现竞争性性能。
...
Method: 使用层理论将局部因果信念表示为变量子集上的预层截面，通过代数潜在投影处理隐藏混杂因子，在信念流形上使用自然梯度下降进行优化。

Result: 在合成和真实基准测试中，HOLOGRAPH在50-100变量的因果发现任务上实现竞争性性能，层理论分析显示恒等、传递和粘合公理满足数值精度，但局部性公理在大图上失效。

Conclusion: HOLOGRAPH为LLM引导的因果发现提供了严格的数学基础，揭示了潜在变量投影中存在根本性的非局部耦合，为因果发现开辟了新的理论方向。
...
### [195] [Can Small Training Runs Reliably Guide Data Curation? Rethinking Proxy-Model Practice](https://arxiv.org/abs/2512.24503)
*Jiachen T. Wang,Tong Wu,Kaifeng Lyu,James Zou,Dawn Song,Ruoxi Jia,Prateek Mittal*

Main category: cs.LG

TL;DR: 研究发现传统数据配方评估方法存在缺陷，提出使用降低学习率的代理模型训练方法，能更可靠地预测大规模LLM训练效果


<details>
  <summary>Details</summary>
Motivation: 当前AI公司使用小规模代理模型评估数据配方，但缺乏对小规模实验结果能否可靠迁移到大规模训练的理解。研究发现传统"公平比较"方法使用相同训练配置存在严重问题，因为最优训练配置是数据依赖的。

Method: 提出改进的评估协议：使用降低学习率的小规模代理模型训练。理论上证明对于随机特征模型，这种方法能保持数据集按最优可达损失的排序。实证验证了23种数据配方。

Result: 实验表明，传统固定配置方法得出的数据质量结论会因训练超参数的微小调整而翻转。而提出的降低学习率方法能显著提高小规模实验的可靠性，其相对性能与完全调优的大规模LLM预训练结果强相关。

Conclusion: 数据配方评估的目标应是识别在数据特定调优下表现最佳的配方。提出的降低学习率方法是一个简单有效的补丁，能大幅提高小规模实验的可靠性，为数据配方评估提供了更可靠的实验协议。
...
### [196] [Generalising E-prop to Deep Networks](https://arxiv.org/abs/2512.24506)
*Beren Millidge*
...
<details>
  <summary>Details</summary>
Motivation: 大脑中的学习涉及多层结构，包含深度和时间上的层次动态。现有RTRL和E-prop方法主要研究单层循环网络，无法处理深度网络中的信用分配问题。

Method: 扩展E-prop框架到深度网络，推导出跨深度的递归关系，将E-prop的资格迹扩展到更深层，实现同时跨时间和深度的在线信用分配。

Result: 提出了一种在线学习算法，能够同时在时间和深度维度上进行准确的信用分配，无需通过时间反向传播即可训练深度循环网络。
...
### [197] [More Than Bits: Multi-Envelope Double Binary Factorization for Extreme Quantization](https://arxiv.org/abs/2512.24545)
*Yuma Ichikawa,Yoshihiko Fujisawa,Yudai Fujimoto,Akira Sakai,Katsuki Fujisawa*

Main category: cs.LG

TL;DR: 提出MDBF方法改进LLM的极低比特量化，通过多包络设计提升表达能力，同时保持部署友好的二进制推理原语。


<details>
  <summary>Details</summary>
Motivation: 现有的Double Binary Factorization (DBF)方法在极低比特量化中虽然能实现高效推理，但其缩放参数过于受限，所有秩分量共享相同的幅度轮廓，导致性能饱和。

Method: 提出Multi-envelope DBF (MDBF)：保留共享的1比特符号基，但将单一包络替换为秩-l包络。通过在不同包络分量间共享符号矩阵，MDBF有效维持二进制载体，并将有限内存预算用于幅度表达能力。还引入了闭式初始化和交替优化方法。

Result: 在LLaMA和Qwen系列模型上，MDBF在相同权重比特数下，相比之前的二进制格式显著提升了困惑度和零样本准确率。

Conclusion: MDBF方法成功解决了DBF的局限性，在保持部署友好推理原语的同时，通过多包络设计显著提升了极低比特量化LLM的性能。
...
### [198] [From Perception to Punchline: Empowering VLM with the Art of In-the-wild Meme](https://arxiv.org/abs/2512.24555)
*Xueyan Li,Yingyi Xue,Mengjie Jiang,Qingzi Zhu,Yazhe Niu*

Main category: cs.LG

TL;DR: HUMOR是一个用于生成幽默梗图的多模态框架，通过分层推理和群体偏好对齐来提升VLMs的幽默生成能力。


<details>
  <summary>Details</summary>
Motivation: 生成幽默梗图是一个具有挑战性的多模态任务，需要超越直接的图像-字幕监督，进行视觉内容、上下文线索和主观幽默的细致推理。现有方法在视觉感知和幽默笑点创造之间存在差距。

Method: 提出HUMOR框架：1）采用分层多路径思维链：从模板级意图识别开始，在不同上下文中探索多样推理路径，最终锚定高质量上下文特定路径；2）训练基于相同模板梗图群体的成对奖励模型，通过群体强化学习优化实现单调改进的理论保证。

Result: 大量实验表明，HUMOR赋予各种VLMs卓越的推理多样性、更可靠的偏好对齐和更高的整体梗图质量。

Conclusion: 该工作提出了一个通用的开放式、人类对齐多模态生成训练范式，通过在连贯输出群体内进行对比判断来指导成功。
...
### [199] [CPR: Causal Physiological Representation Learning for Robust ECG Analysis under Distribution Shifts](https://arxiv.org/abs/2512.24564)
*Shunbo Jia,Caizhi Liao*

Main category: cs.LG

TL;DR: CPR方法通过因果解缠框架分离心电图的病理特征与伪影，在保持单次推理效率的同时，显著提升对抗攻击下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有心电图深度学习模型对对抗扰动（特别是模拟生物形态的SAP）脆弱，现有防御方法面临两难：对抗训练计算成本过高，而随机平滑等方法推理延迟大，不适用于实时临床监测。

Method: 提出因果生理表征学习（CPR），在因果解缠框架中融入生理结构先验，通过结构因果模型建模心电图生成，强制分离不变病理形态（P-QRS-T复合波）与非因果伪影。

Result: 在PTB-XL数据集上，CPR显著优于标准临床预处理方法。在SAP攻击下，CPR获得0.632的F1分数，比中值平滑（0.541）提升9.1%。CPR在保持单次推理效率的同时，达到了随机平滑的认证鲁棒性水平。

Conclusion: CPR在鲁棒性、效率和临床可解释性之间提供了优越的权衡，通过因果解缠解决心电图模型对虚假相关性的依赖问题，为实时临床监测提供了实用解决方案。
...
### [200] [Dynamic Large Concept Models: Latent Reasoning in an Adaptive Semantic Space](https://arxiv.org/abs/2512.24617)
...
TL;DR: DLCM提出了一种分层语言建模框架，通过将计算从token转移到压缩的概念空间来提高推理效率，实现了在相同FLOPs下性能提升2.69%


<details>
  <summary>Details</summary>
Motivation: 当前LLM对所有token采用统一计算，但语言信息密度高度不均匀，导致在可预测片段浪费计算能力，而在语义关键转换处计算不足

Method: 提出动态大概念模型(DLCM)：1)从潜在表示学习语义边界；2)将计算从token转移到压缩概念空间；3)端到端发现变长概念；4)引入压缩感知缩放定律；5)开发解耦μP参数化方法

Result: 在R=4（平均4个token对应1个概念）的设定下，DLCM将约三分之一的推理计算重新分配到更高容量的推理骨干网络，在匹配推理FLOPs下，在12个零样本基准测试中平均提升2.69%

Conclusion: DLCM通过分层压缩和计算重新分配，改变了语言模型的缩放行为，实现了更高效的推理，为语言建模提供了新的范式
...
### [201] [AutoFed: Manual-Free Federated Traffic Prediction via Personalized Prompt](https://arxiv.org/abs/2512.24625)
...
<details>
  <summary>Details</summary>
Motivation: 交通预测对智能交通系统至关重要，但存在数据隐私问题导致数据孤岛。现有联邦学习方法面临非独立同分布问题，而个性化联邦学习需要针对交通任务专门设计，且现有方法依赖超参数优化，这在现实场景中难以实现。

Method: 提出AutoFed框架，受提示学习启发，引入联邦表示器，使用客户端对齐适配器将本地数据提炼为紧凑的全局共享提示矩阵，该提示条件化个性化预测器，使每个客户端既能受益于跨客户端知识又能保持本地特异性。

Result: 在真实世界数据集上的大量实验表明，AutoFed在各种场景下始终实现优越性能。

Conclusion: AutoFed为交通预测提供了一种实用的个性化联邦学习解决方案，无需手动超参数调优，有效解决了数据隐私和知识共享的平衡问题。
...
### [202] [A Scalable Framework for logP Prediction: From Terabyte-Scale Data Integration to Interpretable Ensemble Modeling](https://arxiv.org/abs/2512.24643)
...
<details>
  <summary>Details</summary>
Motivation: 开发大规模logP预测框架，解决化学数据库整合挑战，探索脂溶性（logP）的多变量本质，为分子设计提供指导，并与最先进的图神经网络架构竞争。

Method: 从PubChem、ChEMBL和eMolecules三个权威数据库交叉整合42.7万生物活性化合物；开发基于字节偏移索引的高效计算架构；系统评估多种建模方法（线性模型、随机森林、XGBoost等）；采用分层建模策略（药物样分子91%和极端情况9%）。

Result: 计算架构将处理时间从预计100多天缩短到3.2小时（提升740倍）；分子量被识别为最重要的全局预测因子；树集成模型（R²=0.765，RMSE=0.731）优于存在异方差问题的线性模型；分层建模策略取得最佳性能：药物样分子RMSE=0.838，极端分子R²=0.767。

Conclusion: 精心策划的基于描述符的集成模型仍能与最先进的图神经网络架构竞争；为分子设计提供可操作的指导；建立了仅使用2D描述符的脂溶性预测稳健基线；分层建模策略是处理数据异质性的有效方法。
...
### [203] [HeteroHBA: A Generative Structure-Manipulating Backdoor Attack on Heterogeneous Graphs](https://arxiv.org/abs/2512.24665)
*Honglin Gao,Lan Zhao,Junhao Ren,Xiang Li,Gaoxi Xiao*

Main category: cs.LG

TL;DR: HeteroHBA：针对异构图神经网络的生成式后门攻击框架，通过显著性筛选选择有影响力的辅助邻居，合成多样化的触发特征和连接模式，使用AdaIN和MMD损失对齐特征分布以增强隐蔽性，在多个真实异构图数据集上实现高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 异构图神经网络（HGNNs）在实际应用中表现出色，但针对异构图的定向后门投毒攻击研究较少。现有攻击方法在异构图上的效果有限，需要开发更隐蔽、更有效的攻击框架来揭示异构图的潜在安全风险。

Method: 1. 通过显著性筛选选择有影响力的辅助邻居进行触发节点连接；2. 合成多样化的触发特征和连接模式以匹配局部异质上下文；3. 结合自适应实例归一化（AdaIN）和最大均值差异（MMD）损失对齐触发特征分布与良性统计特征；4. 采用双层优化目标联合促进攻击成功率和保持清洁准确率。

Result: 在多个真实异构图数据集和代表性HGNN架构上的实验表明，HeteroHBA相比现有后门基线方法，在保持相当或更小清洁准确率影响的同时，实现了更高的攻击成功率；攻击在异构感知的结构防御CSD下仍然有效。

Conclusion: HeteroHBA揭示了异构图学习中的实际后门风险，强调了开发更强防御机制的必要性。该框架为理解异构图神经网络的安全漏洞提供了重要见解，并推动了相关防御技术的发展。
...
### [204] [Mobility-Assisted Decentralized Federated Learning: Convergence Analysis and A Data-Driven Approach](https://arxiv.org/abs/2512.24694)
*Reza Jahani,Md Farhamdur Reza,Richeng Jin,Huaiyu Dai*

Main category: cs.LG

TL;DR: 本文系统研究了用户移动性对去中心化联邦学习性能的影响，提出利用诱导移动模式增强稀疏网络中的信息传播，显著提升DFL性能。


<details>
  <summary>Details</summary>
Motivation: 去中心化联邦学习在稀疏网络和数据异构情况下性能显著下降，而下一代无线网络中移动性日益普遍，但移动性对DFL的影响尚未得到充分研究，尽管其具有增强信息流动的潜力。

Method: 首先建立移动性下稀疏网络中DFL的收敛性理论，证明随机移动即可提升性能；然后提出利用诱导移动模式的DFL框架，使移动用户能够基于数据分布知识确定轨迹来增强网络信息传播。

Result: 理论证明即使部分用户的随机移动也能显著提升性能；实验验证了理论发现，证明所提方法优于基线，并全面分析了各种网络参数对移动网络中DFL性能的影响。

Conclusion: 移动性能够显著改善稀疏网络中去中心化联邦学习的性能，利用诱导移动模式可以更有效地传播信息，为下一代移动网络中的DFL应用提供了重要见解。
...
### [205] [Nested Learning: The Illusion of Deep Learning Architectures](https://arxiv.org/abs/2512.24695)
*Ali Behrouz,Meisam Razaviyayn,Peilin Zhong,Vahab Mirrokni*

Main category: cs.LG

TL;DR: 提出一种新的学习范式——嵌套学习(NL)，将机器学习模型表示为嵌套、多层次的优化问题，通过压缩上下文流来学习，自然产生上下文学习能力。


<details>
  <summary>Details</summary>
Motivation: 尽管语言模型取得进展，但在持续学习、自我改进和寻找有效解决方案方面仍存在根本性挑战。现有模型缺乏有效的持续学习机制和更强大的学习算法。

Method: 提出嵌套学习范式，将模型表示为多层次优化问题。开发了三种核心技术：1) 将梯度优化器视为关联记忆模块；2) 自修改学习模块，学习如何修改自身；3) 连续记忆系统，泛化长短时记忆概念。

Result: 提出了名为Hope的持续学习模块，在语言建模、知识整合、少样本泛化、持续学习和长上下文推理任务中显示出有希望的结果。

Conclusion: 嵌套学习为设计更强大的学习算法提供了新视角，通过增加层次实现高阶上下文学习，有望解锁有效的持续学习能力，为机器学习模型发展提供新方向。
...
### [206] [Causal Discovery with Mixed Latent Confounding via Precision Decomposition](https://arxiv.org/abs/2512.24696)
...
<details>
  <summary>Details</summary>
Motivation: 现实数据中常存在混合潜在混杂：一些未观测因子广泛影响多个变量（全局混杂），而另一些只影响少数变量（局部混杂）。现有方法存在局限：可微/基于分数的DAG学习器可能将全局潜在效应误认为因果边，而潜在变量图模型只能恢复无向结构。

Method: 提出DCL-DECOR模块化流程：1) 将观测精度矩阵分解为结构化分量（因果图+局部混杂）和低秩分量（全局混杂）；2) 对去混杂后的表示应用相关噪声DAG学习器恢复有向边，同时建模剩余的结构化误差相关性；3) 简单协调步骤确保无弓形结构。

Result: 提供了混合混杂下可恢复因果目标的可识别性结果，表明整体问题可分解为具有模块化保证的已研究子问题。合成实验显示，在不同强度和维度的全局混杂下，相比直接在混杂数据上应用相关噪声DAG学习，该方法在恢复有向边方面有持续改进。

Conclusion: DCL-DECOR方法能有效处理混合潜在混杂问题，通过精度矩阵分解分离全局混杂效应，再应用相关噪声DAG学习，在因果发现任务中表现优于现有方法。
...
### [207] [BandiK: Efficient Multi-Task Decomposition Using a Multi-Bandit Framework](https://arxiv.org/abs/2512.24708)
*András Millinghoffer,András Formanek,András Antos,Péter Antal*

Main category: cs.LG

TL;DR: BandiK：基于多臂老虎机的三阶段多任务辅助任务子集选择方法，通过估计任务间转移、构建线性候选集、多老虎机框架高效选择有益辅助任务，解决负迁移和计算成本问题。


<details>
  <summary>Details</summary>
Motivation: 多任务学习中知识迁移的有效性至关重要，但负迁移问题严重阻碍性能提升。现有方法面临三大挑战：评估候选辅助任务集的计算成本高、候选集数量庞大、不同目标任务的选择复杂度差异大。

Method: 三阶段方法：1) 估计任务间成对转移关系，识别可能受益于联合学习的任务；2) 基于初始估计为每个目标任务构建线性数量的候选辅助任务集；3) 为每个任务采用多臂老虎机框架，将候选集作为臂，通过训练测试神经网络评估性能，并利用多老虎机结构共享神经网络实现提高效率。

Result: BandiK显著减少了候选辅助任务集的数量（从指数级降至线性级），通过多老虎机框架高效评估候选集，利用半重叠臂特性优化成本/奖励结构，解决了计算成本高和负迁移问题。

Conclusion: BandiK提出了一种新颖的多任务辅助任务选择框架，通过三阶段方法有效解决负迁移和计算效率问题，为多任务学习中的知识迁移提供了实用解决方案。
...
### [208] [FPGA Co-Design for Efficient N:M Sparse and Quantized Model Inference](https://arxiv.org/abs/2512.24713)
*Fen-Yu Hsieh,Yun-Chang Teng,Ding-Yong Hong,Jan-Jan Wu*

Main category: cs.LG

TL;DR: 提出一个自动化框架，结合权重剪枝和低比特量化，通过软硬件协同设计在FPGA平台上生成加速器，实现高效LLM推理


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多种语言处理任务上表现出色，但计算和内存需求巨大，阻碍了在资源受限环境中的部署
...
Conclusion: 细粒度N:M稀疏性和量化的协同作用能够实现高效可部署的LLM推理，提出的FPGA加速器为支持更广泛稀疏模式提供了灵活的架构路径
...
### [209] [From Trial to Deployment: A SEM Analysis of Traveler Adoptions to Fully Operational Autonomous Taxis](https://arxiv.org/abs/2512.24767)
*Yutong Cai,Hua Wang*
...
Method: 在武汉收集了336份有效问卷，设计了包含实际服务属性的调查，使用结构方程模型识别六个潜在心理构念（信任与政策支持、成本敏感性、性能、行为意向、生活方式、教育），并分析它们对自动驾驶出租车选择频率的影响。

Result: 成本敏感性和行为意向是自动驾驶出租车采用的最强正向预测因素，其他潜在构念作用更为微妙。模型在多个拟合指数上表现出良好的拟合度。

Conclusion: 研究结果为自动驾驶出租车在实际城市环境中的规模化部署提供了支持政策制定、票价设计和公众宣传策略的实证证据。
...
### [210] [Gradient Descent as Implicit EM in Distance-Based Neural Models](https://arxiv.org/abs/2512.24780)
*Alan Oursland*
...
<details>
  <summary>Details</summary>
Motivation: 神经网络训练中出现的概率推理行为（软聚类、原型专业化、贝叶斯不确定性跟踪）通常通过混合模型类比或后验架构解释来理解，缺乏直接的理论推导

Method: 通过数学推导证明：对于任何具有log-sum-exp结构（基于距离或能量）的损失函数，每个距离的梯度恰好等于对应分量的负后验责任：∂L/∂d_j = -r_j

Result: 梯度下降在此类目标函数上隐式执行期望最大化算法，责任不是需要计算的辅助变量而是需要应用的梯度，无需显式推理算法

Conclusion: 该结果将无监督混合建模、注意力机制和交叉熵分类统一到单一机制下，表明transformer中观察到的贝叶斯结构不是涌现特性而是目标几何的必然结果，优化与推理是同一过程
...
### [211] [Self-Supervised Neural Architecture Search for Multimodal Deep Neural Networks](https://arxiv.org/abs/2512.24793)
...
<details>
  <summary>Details</summary>
Motivation: 多模态DNN需要特征融合，结构复杂，传统NAS方法需要大量标注数据，这限制了其应用。本文旨在解决多模态NAS对标注数据的依赖问题。

Method: 提出一种自监督学习方法，将SSL全面应用于多模态DNN的架构搜索和模型预训练两个过程，从而减少对标注数据的依赖。

Result: 实验结果表明，该方法能够成功地从无标注训练数据中设计出有效的DNN架构。

Conclusion: 自监督学习可以有效应用于多模态神经架构搜索，显著减少对标注数据的需求，为复杂多模态DNN的设计提供了更实用的解决方案。
...
### [212] [DTI-GP: Bayesian operations for drug-target interactions using deep kernel Gaussian processes](https://arxiv.org/abs/2512.24810)
...
<details>
  <summary>Details</summary>
Motivation: 药物-靶点相互作用预测需要精确的概率信息来理解模型局限性和提升预测性能，高斯过程提供可扩展框架来整合最先进的DTI表示和贝叶斯推断。

Method: 提出DTI-GP架构，包含化学化合物和蛋白质靶点的组合神经嵌入模块以及高斯过程模块，通过预测分布采样估计贝叶斯优先矩阵，用于快速准确的筛选和排序操作。

Result: DTI-GP优于现有最先进解决方案，能够构建贝叶斯准确度-置信度富集分数，实现改进富集的拒绝方案，以及估计和搜索具有高期望效用的top-K选择和排序。

Conclusion: DTI-GP为药物-靶点相互作用预测提供了概率框架，支持多种贝叶斯操作，在预测性能和功能扩展性方面表现出色。
...
### [213] [Unregularized Linear Convergence in Zero-Sum Game from Preference Feedback](https://arxiv.org/abs/2512.24818)
...
<details>
  <summary>Details</summary>
Motivation: 传统偏好建模使用Bradley-Terry模型假设传递性，忽略了人类群体偏好的复杂性。NLHF通过将非传递性偏好建模为两人零和博弈来解决此问题，但现有算法依赖正则化，在计算原始博弈对偶间隙时会产生不可避免的偏差。

Method: 采用乐观乘性权重更新(OMWU)算法，分析其在NLHF中的收敛特性。该方法不需要纳什均衡唯一性假设，当存在全支撑纳什均衡时，经过预热阶段后能实现最后迭代线性收敛。

Result: 首次证明了OMWU在NLHF中的收敛保证，识别了边际收敛行为，其中很少被选择的动作概率从指数级小值指数增长，相比先前结果在实例依赖常数上有指数级更好的依赖性。实验验证了OMWU在表格和神经策略类别中的理论优势。

Conclusion: OMWU在NLHF中具有强大的理论收敛保证，无需纳什均衡唯一性假设，能够收敛到原始纳什均衡，为LLM应用提供了有前景的方法。
...
### [214] [Discovering Coordinated Joint Options via Inter-Agent Relative Dynamics](https://arxiv.org/abs/2512.24827)
...
<details>
  <summary>Details</summary>
Motivation: 在多智能体环境中，联合状态空间随智能体数量呈指数增长，使得协调行为设计特别困难。现有多智能体选项发现方法往往牺牲协调性，产生松散耦合或完全独立的行为。

Method: 提出联合状态抽象方法压缩状态空间，同时保留发现强协调行为所需信息。基于智能体状态同步的归纳偏置，近似最大对齐的费马状态，定义团队级错位度量，然后使用神经图拉普拉斯估计器提取捕捉智能体间状态同步模式的选项。

Result: 在多个多智能体领域的多个场景中评估，结果显示该方法产生的选项相比其他选项发现方法能带来更强的下游协调能力。

Conclusion: 该方法通过联合状态抽象和费马状态近似，成功解决了多智能体选项发现中的协调挑战，能够发现强协调行为，在多智能体环境中表现出优越的协调能力。
...
### [215] [AODDiff: Probabilistic Reconstruction of Aerosol Optical Depth via Diffusion-based Bayesian Inference](https://arxiv.org/abs/2512.24847)
*Linhao Fan,Hongqiang Fang,Jingyang Dai,Yong Jiang,Qixing Zhang*

Main category: cs.LG

TL;DR: AODDiff：基于扩散贝叶斯推理的概率性AOD重建框架，无需完整训练数据，支持不确定性量化，适用于多种重建任务


<details>
  <summary>Details</summary>
Motivation: 当前AOD重建模型受限于完整训练数据稀缺和缺乏不确定性量化，需要能够处理不完整数据并提供置信度评估的灵活框架

Method: 提出AODDiff框架：1）使用感知损坏的训练策略从不完整数据学习时空AOD先验；2）采用解耦退火后验采样策略整合异构观测约束生成过程

Result: 在再分析数据上的降尺度和修复任务验证了AODDiff的有效性和鲁棒性，特别在保持高空间光谱保真度方面表现优异，并能通过多次采样进行不确定性量化

Conclusion: AODDiff为AOD重建提供了灵活的概率框架，能够处理不完整数据、整合异构观测、保持高保真度，并为下游应用提供关键置信度指标
...
### [216] [Characterization of Transfer Using Multi-task Learning Curves](https://arxiv.org/abs/2512.24866)
*András Millinghoffer,Bence Bolgár,Péter Antal*

Main category: cs.LG

TL;DR: 该研究提出使用多任务学习曲线来量化迁移效应，认为通过改变数据集规模而非梯度更新来扰动模型能更根本地刻画迁移现象，并开发了高效近似方法。


<details>
  <summary>Details</summary>
Motivation: 传统迁移效应研究多关注训练过程中的梯度更新，本文假设通过改变数据集规模（增加样本）而非扰动模型能提供更根本的迁移效应表征，需要新的量化方法。

Method: 使用多任务学习曲线近似不同样本规模下的归纳性能，开发了类似任务亲和性分组的高效近似方法，比较统计与计算两种迁移方法的成本与效果。

Result: 在药物-靶点相互作用基准数据集上的评估表明，学习曲线能更好地捕捉多任务学习效应，其多任务扩展能刻画基础模型中的成对和上下文迁移效应。

Conclusion: 学习曲线为量化迁移效应提供了更优框架，相比传统计算方法的计算成本更低，但统计方法具有更好的检测能力和更广泛的适用性。
...
### [217] [MSACL: Multi-Step Actor-Critic Learning with Lyapunov Certificates for Exponentially Stabilizing Control](https://arxiv.org/abs/2512.24955)
...
Method: MSACL框架整合了指数稳定性理论和最大熵强化学习，通过多步Lyapunov证书学习。使用指数稳定性标签(ESL)和λ加权聚合机制平衡多步学习中的偏差-方差权衡。策略优化由稳定性感知的优势函数指导，确保学习到的策略促进快速的Lyapunov下降。

Result: 在六个基准测试（包括稳定化和非线性跟踪任务）中，MSACL优于最先进的基于Lyapunov的强化学习算法。在简单奖励下实现指数稳定性和快速收敛，对不确定性具有显著鲁棒性，并能泛化到未见过的轨迹。敏感性分析确定多步视野n=20作为跨不同系统的稳健默认值。

Conclusion: 通过将Lyapunov理论与离策略actor-critic框架相结合，MSACL为可验证的安全学习控制提供了基础，实现了探索与严格安全保证之间的平衡。
...
### [218] [PRISM: A hierarchical multiscale approach for time series forecasting](https://arxiv.org/abs/2512.24898)
*Zihao Chen,Alexandre Andre,Wenrui Ma,Ian Knight,Sergey Shuvaev,Eva Dyer*

Main category: cs.LG

TL;DR: PRISM提出了一种基于树状分区的时间序列预测方法，通过分层学习全局趋势和局部细节，在多个基准数据集上优于现有方法。
...
Method: PRISM采用可学习的树状分区结构：根节点捕捉全局趋势，递归分割揭示局部视图；每层使用时间-频率基（如小波或指数移动平均）提取尺度特定特征，并在层次结构中聚合。
...
### [219] [Semi-overlapping Multi-bandit Best Arm Identification for Sequential Support Network Learning](https://arxiv.org/abs/2512.24959)
*András Antos,András Millinghoffer,Péter Antal*

Main category: cs.LG

TL;DR: 提出SSNL框架和SOMMAB模型，通过共享评估减少样本复杂度，用于多任务学习等场景中的支持网络识别


<details>
  <summary>Details</summary>
Motivation: 现代AI/ML问题需要评估合作伙伴贡献并选择最优候选集，但现有序列方法效率不高，需要统一框架来高效学习支持网络

Method: 提出Sequential Support Network Learning (SSNL)框架和semi-overlapping multi-(multi-armed) bandit (SOMMAB)模型，开发GapE算法并推导指数误差界

Result: SOMMAB模型能显著减少样本复杂度，误差界随重叠度线性缩放，改进了多臂赌博机最佳臂识别的最优常数

Conclusion: 为多任务学习、联邦学习等场景中的支持网络识别提供了理论基础和性能保证，共享评估能显著提升学习效率
...
### [220] [Spectral Graph Neural Networks for Cognitive Task Classification in fMRI Connectomes](https://arxiv.org/abs/2512.24901)
...
<details>
  <summary>Details</summary>
Motivation: 传统的机器学习方法在从神经影像数据解码大脑状态时，往往无法充分捕捉大脑网络中的复杂连接模式和拓扑依赖关系。需要更先进的模型来提取功能磁共振成像连接组中的多尺度交互信息。

Method: 提出SpectralBrainGNN模型，基于图傅里叶变换的谱卷积框架，通过归一化拉普拉斯矩阵特征分解计算GFT。将大脑区域建模为节点，功能连接建模为边，捕捉拓扑依赖和多尺度交互。

Result: 在Human Connectome Project-Task数据集上的实验表明，该方法有效，达到了96.25%的分类准确率。

Conclusion: SpectralBrainGNN模型能够有效解码认知任务，通过谱卷积框架捕捉大脑网络中的复杂连接模式，优于传统方法。代码已开源以支持可重复性和未来研究。
...
### [221] [Frequent subgraph-based persistent homology for graph classification](https://arxiv.org/abs/2512.24917)
*Xinyang Chen,Amaël Broustet,Guoting Chen*

Main category: cs.LG

TL;DR: 提出基于频繁子图的图过滤方法(FSF)，生成频率持久同调特征(FPH)，并开发FPH-ML和FPH-GNN两种图分类框架，在多个基准测试中优于传统方法。
...
Method: 提出频繁子图过滤(FSF)，从频繁子图中推导出稳定的频率持久同调(FPH)特征。开发两种图分类方法：FPH-ML（基于FPH的机器学习模型）和FPH-GNN（将FPH与图神经网络结合的混合框架）。

Result: FPH-ML在准确率上与基于核和基于度的过滤方法相当或更优。FPH与图神经网络集成后，在基准测试中相对性能提升0.4-21%，比GCN和GIN骨干网络提升高达8.2个百分点。
...
### [222] [Generative Classifiers Avoid Shortcut Solutions](https://arxiv.org/abs/2512.25034)
*Alexander C. Li,Ananya Kumar,Deepak Pathak*

Main category: cs.LG

TL;DR: 生成式分类器通过建模所有特征（包括核心和虚假特征）而非主要关注虚假特征，能有效避免分布偏移下的性能下降，在图像和文本分布偏移基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 判别式分类器容易学习虚假相关性特征，这些特征在分布内有效但在轻微分布偏移下就会失效。需要一种能避免过度依赖虚假特征的方法。

Method: 使用基于类条件生成模型（扩散模型和自回归模型）的生成式分类器，建模所有特征而非主要关注虚假特征。该方法简单易训练，无需特殊增强、强正则化、额外超参数或对特定虚假相关性的先验知识。

Result: 在五个标准图像和文本分布偏移基准上达到最先进性能，在医疗和卫星数据集等实际应用中减少虚假相关性的影响。通过高斯玩具设置分析生成式分类器的归纳偏置和数据特性。

Conclusion: 生成式分类器通过建模所有特征能有效避免虚假相关性依赖，在分布偏移场景下优于判别式分类器，且训练简单无需复杂技巧。
...
### [223] [Attribution-Guided Distillation of Matryoshka Sparse Autoencoders](https://arxiv.org/abs/2512.24975)
...
Method: 提出蒸馏套娃稀疏自编码器(DMSAEs)：通过迭代蒸馏循环训练Matryoshka SAE，使用梯度X激活度量特征贡献，保留最小特征子集解释固定比例归因，仅转移核心编码器权重向量

Result: 在Gemma-2-2B模型第12层残差流激活上，7个蒸馏循环(500M tokens, 65k宽度)得到197个核心特征，使用该核心训练改善了多个SAEBench指标

Conclusion: DMSAEs能够提取一致的特征集，并可在不同稀疏度水平间迁移，提高了稀疏自编码器特征的可重用性和解释稳定性
...
### [224] [Efficiently Estimating Data Efficiency for Language Model Fine-tuning](https://arxiv.org/abs/2512.24991)
*Gyung Hyun Je,Colin Raffel*

Main category: cs.LG

TL;DR: 提出基于梯度余弦相似度的任务数据效率预测方法，无需增量标注即可预估达到目标性能所需标注数据量


<details>
  <summary>Details</summary>
Motivation: 大语言模型在零样本任务上表现有限，通常需要微调来提升性能，但不同任务的数据效率（达到目标性能所需标注数据量）未知，导致昂贵的增量标注和重训练循环

Method: 提出使用低置信度样本的梯度余弦相似度来预测任务数据效率，基于少量标注样本即可预测，无需增量标注

Result: 在30个专门任务上验证，数据效率预测误差为8.6%，通常能为每个任务节省数百个不必要的标注

Conclusion: 提出的梯度余弦相似度方法能有效预测任务数据效率，显著减少标注成本，为高效微调策略提供指导
...
### [225] [Diffusion Language Models are Provably Optimal Parallel Samplers](https://arxiv.org/abs/2512.25014)
*Haozhe Jiang,Nika Haghtalab,Lijie Chen*

Main category: cs.LG

TL;DR: 扩散语言模型（DLMs）通过并行采样和链式思维（CoT）可以最优地模拟任何并行采样算法，但需要重掩码或修订功能来实现最优空间复杂度。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型作为自回归模型的替代方案，具有通过并行标记生成实现更快推理的潜力。本文旨在为这种优势提供严格的理论基础，并探索如何进一步优化DLMs的空间效率。

Method: 1. 形式化并行采样模型；2. 证明带多项式长度链式思维（CoT）的DLMs可以用最优顺序步骤模拟任何并行采样算法；3. 证明启用重掩码（将未掩码标记转换为掩码）或修订（将未掩码标记转换为其他未掩码标记）功能后，DLMs可以以最优空间复杂度模拟任何并行采样算法；4. 建立严格的表达能力差距分析。

Result: 1. 当目标分布可以用少量顺序步骤生成时，DLMs可以用相同的最优顺序步骤生成该分布；2. 带修订或重掩码功能的DLMs比不带这些功能的DLMs具有严格更强的表达能力；3. DLMs作为最高效的并行采样器具有理论依据。

Conclusion: 研究结果为DLMs作为最高效并行采样器的承诺提供了理论支持，并主张在DLMs中启用修订功能，以实现最优的空间复杂度和表达能力。
...
### [226] [ResponseRank: Data-Efficient Reward Modeling through Preference Strength Learning](https://arxiv.org/abs/2512.25023)
*Timo Kaufmann,Yannick Metz,Daniel Keim,Eyke Hüllermeier*

Main category: cs.LG

TL;DR: ResponseRank：一种利用局部相对强度信号（如响应时间、标注者一致性）来稳健学习偏好强度的方法，相比仅使用二元选择能提高样本效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统的二元选择（如RLHF）只能传达偏好方向，无法衡量偏好强度。强度信息对于不确定性决策和偏好模型泛化至关重要，但难以可靠测量。现有的元数据代理信号（响应时间、标注者一致性）通常存在噪声和混淆问题。

Method: 提出ResponseRank方法：1）利用代理信号的相对差异对成对比较中的响应进行偏好强度排序；2）通过精心构建的分层进行局部比较，控制系统性变异；3）在最小化强度信号假设下，学习与强度排序一致的效用差异。

Result: 在三个任务上验证：1）合成偏好学习（模拟响应时间）；2）语言建模（标注者一致性）；3）RL控制任务（模拟回合回报）。结果显示改进的样本效率和鲁棒性。同时提出Pearson Distance Correlation（PDC）新指标，从序数准确性中分离基数效用学习。

Conclusion: ResponseRank能够从噪声强度信号中稳健学习偏好强度，相比仅使用二元选择信息的方法，在多种任务上表现出更好的样本效率和鲁棒性。提出的PDC指标有助于评估基数效用学习质量。
...
### [227] [On the geometry and topology of representations: the manifolds of modular addition](https://arxiv.org/abs/2512.25060)
*Gabriela Moisescu-Pareja,Gavin McCracken,Harley Wiltzer,Vincent Létourneau,Colin Daniels,Doina Precup,Jonathan Love*

Main category: cs.LG

TL;DR: 研究发现，先前认为Clock和Pizza两种注意力架构会产生不同加法电路的观点是错误的，实际上它们通过拓扑和几何等价的表示实现了相同算法。


<details>
  <summary>Details</summary>
Motivation: 挑战先前关于不同注意力架构（均匀注意力与可训练注意力）会产生不同模块化加法电路的观点，揭示这些架构实际上可能实现相同的算法。

Method: 超越单个神经元和权重的解释，识别所有对应每个学习表示的神经元，将神经元群体作为一个整体实体研究。将学习表示视为流形，运用拓扑学工具分析，并对数百个电路的学习表示进行统计分析。

Result: 发现均匀注意力和可训练注意力架构通过拓扑和几何等价的表示实现了相同的算法，证明了从常见深度学习范式中自然产生的学习模块化加法电路之间的相似性。

Conclusion: 不同注意力架构（Clock和Pizza）实际上实现了相同的模块化加法算法，挑战了先前关于架构设计差异会导致不同电路的观点，表明这些架构在表示层面具有等价性。
...
### [228] [Many Minds from One Model: Bayesian Transformers for Population Intelligence](https://arxiv.org/abs/2512.25063)
*Diji Yang,Yi Zhang*

Main category: cs.LG

TL;DR: 提出Population Bayesian Transformers (B-Trans)，将标准大语言模型转换为贝叶斯Transformer，通过归一化层偏置的随机化实现从单一预训练权重采样多样模型实例，支持群体决策提升性能


<details>
  <summary>Details</summary>
Motivation: 现代Transformer通常训练为单一确定性系统，但智能可能源于多思维协作。受此启发，希望将标准LLM转换为贝叶斯Transformer，从单一预训练权重中采样多样但连贯的模型实例

Method: 将归一化层中的偏置偏移视为具有高斯变分近似的随机变量，建立贝叶斯后验代理；在序列级别冻结采样噪声以保证时间一致性；通过采样多个模型实例进行群体决策

Result: 在零样本生成、带可验证奖励的强化学习(RLVR)和无显式标签的RL任务中，B-Trans相比确定性基线在保持语义多样性的同时获得更好的任务性能

Conclusion: B-Trans成功将标准LLM转换为贝叶斯Transformer，通过群体智慧实现更好的探索和性能，为从单一预训练权重获得多样化模型行为提供了高效方案
...
### [229] [Scaling Open-Ended Reasoning to Predict the Future](https://arxiv.org/abs/2512.25070)
*Nikhil Chandak,Shashwat Goel,Ameya Prabhu,Moritz Hardt,Jonas Geiping*

Main category: cs.LG

TL;DR: 训练语言模型进行开放式预测任务，通过自动化新闻数据合成训练数据，开发OpenForecaster 8B模型，在预测准确性、校准和一致性方面表现优异，媲美更大规模的专有模型。


<details>
  <summary>Details</summary>
Motivation: 高风险决策需要在不确定性下对未来进行推理。本研究旨在训练语言模型进行开放式预测，解决传统预测方法的局限性，提升语言模型在实际决策中的应用能力。

Method: 1. 从每日新闻中自动合成预测问题；2. 使用离线新闻语料库防止信息泄露；3. 训练Qwen3思维模型；4. 采用检索机制和改进的强化学习奖励函数；5. 在2025年5-8月进行保留测试。

Result: OpenForecaster 8B模型在准确性、校准和一致性方面表现优异，能够匹配更大规模的专有模型。预测训练的校准改进能够泛化到其他基准测试中。

Conclusion: 通过自动化数据合成和专门训练，可以开发出高效的预测模型。研究开源了所有模型、代码和数据，促进了语言模型预测研究的可访问性。
