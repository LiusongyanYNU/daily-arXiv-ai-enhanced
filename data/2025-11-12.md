...
Motivation: 现有方法存在两个根本限制：缺乏稳健的模态内机制来评估视觉和文本标记的重要性，导致复杂场景泛化能力差；以及缺乏细粒度不确定性建模，无法捕捉区域-单词对应的一对多和多对一性质。

Method: 提出统一方法，包含显著性感知和粒度感知建模以及区域级不确定性建模。利用模态特定偏置识别显著特征，将区域特征表示为高斯混合分布以捕捉细粒度不确定性。

Result: 在Flickr30K和MS-COCO上的广泛实验表明，该方法在各种骨干架构上均达到最先进性能，显著提升了细粒度图像-文本对齐的鲁棒性和可解释性。

Conclusion: 所提出的显著性感知和不确定性建模方法有效解决了细粒度图像-文本对齐的关键挑战，为多模态学习应用提供了更可靠的基础。
...
<details>
  <summary>Details</summary>
Motivation: 传统WSVAD模型依赖视觉特征，缺乏可解释性。TbVAD旨在通过语言驱动实现知识基础的可解释推理。

Method: 三阶段框架：1) 使用视觉语言模型将视频内容转换为细粒度描述；2) 将描述组织为四个语义槽（动作、对象、上下文、环境）；3) 生成槽级解释，揭示哪些语义因素对异常决策贡献最大。

Result: 在UCF-Crime和XD-Violence两个公开基准上的评估表明，文本知识推理为真实世界监控场景提供了可解释且可靠的异常检测。
...
TL;DR: 提出了一种名为双矩法(MoDM)的新数据融合框架，通过结合均匀和非均匀两种不同方向分布下的二阶矩数据，实现分子结构的唯一确定和准确重建。


<details>
  <summary>Details</summary>
Motivation: 冷冻电镜(cryo-EM)在重建分子结构时面临噪声和随机取向的挑战。现有方法通常只使用单一数据集，而利用不同实验条件下收集的多个数据集可以显著提高重建质量。

Method: 开发了双矩法(MoDM)框架，使用两种不同方向分布(一种均匀，一种未知非均匀)下投影图像的二阶矩数据。通过凸松弛算法仅利用二阶统计量实现准确重建。

Result: 证明了这些矩通常能唯一确定底层结构(除全局旋转和反射外)，算法实现了准确恢复，展示了利用数据集多样性在计算成像任务中的优势。

Conclusion: 收集和建模不同实验条件下的多个数据集可以显著提高重建质量，数据集多样性是提升计算成像性能的关键因素。
...
TL;DR: 提出了SSViT，第一个用于模数视频重建的深度学习框架，通过token选择策略提高效率并专注于关键区域，在模数视频恢复中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统图像传感器动态范围有限，模数相机通过折叠入射辐照度来解决这个问题，但需要专门的展开算法来重建底层信号。现有的HDR方法不适用于模数恢复，而Transformer能够捕捉解决折叠视频帧所需的全局依赖和时空关系。

Method: 提出了选择性时空视觉Transformer（SSViT），采用token选择策略来提高效率并集中处理最关键的区域，这是第一个用于模数视频重建的深度学习框架。

Result: 实验证实SSViT能够从8位折叠视频中产生高质量重建，在模数视频恢复中达到了最先进的性能。

Conclusion: SSViT是模数视频恢复的有效解决方案，通过Transformer架构和token选择策略成功解决了传统HDR方法无法处理的模数恢复问题。
...
TL;DR: 提出一种在扩散模型推理阶段的后处理调整方法，利用分数的拉普拉斯来减少模式插值幻觉，在1D、2D和高维图像数据上有效降低幻觉样本率。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然成功，但已知存在产生不连贯或不真实样本的幻觉问题。现有研究将其归因于模式插值和分数平滑现象，但缺乏在采样过程中防止其生成的方法。

Method: 在推理阶段对分数函数进行后处理调整，利用分数的拉普拉斯来减少模式插值幻觉。为高维数据推导了基于Hutchinson迹估计器的有限差分变体的高效拉普拉斯近似。

Result: 该方法在玩具1D/2D分布和高维图像数据集上显著降低了幻觉样本的比率。

Conclusion: 提出的拉普拉斯校正方法有效减少了扩散模型中的模式插值幻觉，并探索了拉普拉斯与分数不确定性之间的关系。
...
TL;DR: 提出了ASAG方法，通过最优传输理论和Sinkhorn算法重新解释扩散模型中的注意力机制，在自注意力层中注入对抗性成本来降低查询和键之间的像素相似性，从而改进样本质量。


<details>
  <summary>Details</summary>
Motivation: 现有的指导方法如CFG通过启发式扰动函数故意降低无条件输出来提升目标输出质量，但这些方法缺乏理论基础且依赖手动设计的失真。

Method: ASAG方法从最优传输角度重新解释注意力分数，通过Sinkhorn算法故意干扰传输成本，在自注意力层中注入对抗性成本来减少查询和键之间的像素相似性。

Result: ASAG在文本到图像扩散中表现出持续改进，并在下游应用如IP-Adapter和ControlNet中增强了可控性和保真度。

Conclusion: ASAG是一种轻量级、即插即用的方法，无需模型重新训练即可提高可靠性，通过故意破坏误导性注意力对齐来改进条件和非条件样本质量。
...
TL;DR: LiveNeRF框架实现了实时面部替换技术，在娱乐、教育和通信应用中具有重要价值，达到33FPS的实时性能并保持高质量视觉效果。


<details>
  <summary>Details</summary>
Motivation: 解决现有面部替换方法在实时性和视觉质量方面的限制，推动该技术在直播、视频会议和互动媒体中的实际部署应用。

Method: LiveNeRF框架，专注于实现实时性能与高质量视觉效果的平衡。

Result: 实现了33FPS的实时性能，具有卓越的视觉质量，能够实际应用于直播、视频会议等场景。

Conclusion: 该技术对内容创作者、教育工作者和语言障碍人士具有重要价值，但需要负责任地部署，包括用户同意验证和检测系统集成，以最大化正面社会影响并降低风险。
...
TL;DR: TrackStudio是一个无标记运动追踪工具包，将开源工具整合为模块化GUI管道，无需编程技能即可实现自动2D/3D追踪、校准、预处理、特征提取和可视化
...
Method: 结合成熟开源工具构建模块化GUI管道，提供用户指南和常见问题文档，在三种环境下使用低成本网络摄像头或高分辨率相机进行验证

Result: 76名参与者测试中，帧间相关性超过0.98，三角测量误差低（手部追踪<13.6mm），证明追踪稳定一致，可扩展到其他身体和面部区域
...
TL;DR: 开发基于CNN的深度学习模型，用于将心脏非对比CT图像中的钙化评分自动分类为六个临床类别，替代当前耗时的手动半自动分析。


<details>
  <summary>Details</summary>
Motivation: 心血管疾病是全球高死亡率的主要原因，冠状动脉钙化评分是评估动脉粥样硬化性心血管疾病风险的重要工具，但当前评分方法需要放射科医生和训练有素的放射技师进行耗时的手动分析。

Method: 使用68例患者的心脏非对比CT扫描数据，采用心电图门控GE Discovery 570心脏SPECT/CT相机采集。数据集分为训练集、验证集和测试集，开发卷积神经网络模型进行六类钙化评分分类。

Result: 模型在六类CAC评分分类任务中表现出高性能，误分类32例，其中26例倾向于高估钙化评分。总体一致性高（Cohen's kappa为0.962），准确率达96.5%，具有良好的泛化能力。

Conclusion: CNN模型能够准确地将钙化评分分层为扩展的六个临床类别，结果与当前半自动实践一致，具有良好的泛化能力，证明了CNN模型在钙化评分分类中的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的网络（如transformer）生成低分辨率特征网格，这对于密集预测任务来说不够理想，需要高分辨率特征表示。

Method: 提出FlowFeat方法，利用光流网络和多样化视频数据，开发有效的自监督训练框架，统计近似表观运动，通过蒸馏技术嵌入运动分布。

Result: FlowFeat显著提升了五种最先进编码器和替代上采样策略在三个密集任务上的表现：视频对象分割、单目深度估计和语义分割，且训练计算成本低。

Conclusion: FlowFeat在可靠和通用的密集图像表示方面迈出了重要一步，具有高空间细节、几何语义线索和时序一致性。
...
TL;DR: UltraGS是一个针对超声成像优化的高斯泼溅框架，通过深度感知高斯泼溅策略和轻量级渲染函数SH-DARS，实现了高质量的超声图像新视角合成，在三个数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 超声成像作为无创临床诊断的基石，其有限的视野限制了新视角合成，需要开发专门针对超声成像特性的方法。

Method: 1. 深度感知高斯泼溅策略：为每个高斯分配可学习的视野，实现准确的深度预测和结构表示；2. SH-DARS渲染函数：结合低阶球谐函数和超声特定波物理特性（深度衰减、反射、散射），精确建模组织强度；3. 贡献临床超声检查数据集。

Result: 在三个数据集上的广泛实验显示，UltraGS在PSNR（最高29.55）、SSIM（最高0.89）和MSE（最低0.002）方面达到最先进水平，同时实现64.69 fps的实时合成。

Conclusion: UltraGS框架在超声图像新视角合成方面表现出色，代码和数据集已开源，为临床超声成像研究提供了有价值的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本或布局条件模型无法实现像素级精确的卫星图像合成，需要开发能够对齐图像和语义矢量几何的跨模态对应关系。

Method: 使用视觉语言对齐模块从多边形语义生成像素级嵌入，这些嵌入指导条件图像生成框架，同时尊重空间范围和语义线索。

Result: 在语义保真度和结构真实性方面相比先前方法有显著改进，训练后的视觉语言模型展现出细粒度的空间定位能力。

Conclusion: VectorSynth为基于地理信息的卫星图像合成和编辑提供了有效的解决方案，支持快速假设模拟和地图信息内容生成。
...
TL;DR: Auto-US是一个集成超声视频和临床诊断文本的智能诊断代理，在CUV数据集上达到了86.73%的分类准确率，并能生成临床诊断建议。
...
<details>
  <summary>Details</summary>
Motivation: 现有的类增量医学图像分割方法存在两个问题：一是采用一刀切策略，对所有空间区域和特征通道同等对待，阻碍了准确旧知识的保留；二是仅关注将局部原型与全局原型对齐，忽略了新数据中旧类的局部表示，导致知识退化。

Method: 提出了两种方法：1) PGCD利用原型到特征的相似性来校准不同空间区域中特定类别的蒸馏强度；2) DAPD将当前模型提取的旧类局部原型与全局原型和局部原型进行双重对齐。

Result: 在两个广泛使用的多器官分割基准上的综合评估表明，该方法优于最先进的方法，突显了其鲁棒性和泛化能力。

Conclusion: PGCD和DAPD方法有效解决了类增量医学图像分割中的知识保留问题，通过原型引导的校准和双重对齐机制显著提升了旧类别的分割性能。
...
TL;DR: 提出Filtered-ViT架构，集成SMART-VMF机制，有效防御多补丁对抗攻击和自然伪影，在ImageNet和医疗影像上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有防御方法假设单一补丁攻击，无法应对现实世界中多个局部干扰同时出现的情况，这在安全关键领域（如医疗）存在严重风险。

Method: 结合视觉变换器架构与SMART向量中值滤波，这是一种空间自适应、多尺度、鲁棒感知的机制，能够选择性抑制受损区域同时保留语义细节。

Result: 在ImageNet上，面对LaVAN多补丁攻击，Filtered-ViT达到79.8%的干净准确率和46.3%的鲁棒准确率（4个1%补丁同时攻击），优于现有防御方法。医疗影像案例显示能有效缓解自然伪影。

Conclusion: Filtered-ViT是首个在对抗性和自然发生的补丁式干扰上均展现统一鲁棒性的变换器，为高风险环境中的可靠视觉系统开辟了新路径。
...
TL;DR: 本文揭示了扩散模型中初始噪声的可分析模式，提出了"语义擦除-注入"的两步法来调制噪声，实现训练免费且通用的语义控制。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为扩散模型的初始噪声是随机元素，仅贡献生成多样性。但本文发现噪声中蕴含丰富语义信息，可以进行分析和控制。

Method: 基于信息论提出两步法：1）从噪声中擦除不需要的语义；2）利用扩散模型生成过程与语义注入的等价性，将语义注入清理后的噪声。

Result: 实验证明该方法在基于DiT和UNet架构的各种文本到内容生成模型中均有效，为扩散模型生成优化提供了新视角。

Conclusion: 该方法提供了一个通用的语义控制工具，能够实现一致的生成效果，为扩散模型优化开辟了新途径。
...
TL;DR: 提出SCBCH框架，通过跨模态语义一致性分类和双向软对比哈希，解决多标签数据中的标签噪声和语义重叠问题，提升跨模态检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态哈希方法依赖完全标注数据集，而现实多标签数据中存在标签噪声和语义重叠问题，严重影响检索性能。
...
Conclusion: SCBCH框架能有效处理多标签数据中的标签噪声和语义重叠问题，显著提升跨模态检索的鲁棒性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 编码器特征往往纠缠域相关和类别相关信息，限制了模型在新域的泛化能力和快速适应能力

Method: 使用对抗对比特征分解模块分离特征，矩阵引导动态融合模块整合特征，交叉自适应调制模块增强泛化

Result: 在四个挑战性数据集上的实验表明，DCDNet优于现有CD-FSS方法，建立了新的最先进水平

Conclusion: DCDNet通过特征分解和动态融合有效解决了跨域少样本分割中的特征纠缠问题，显著提升了模型性能
...
TL;DR: 提出一个基于SE-ResNeXt101的多标签胸部X光分类pipeline，通过标签图精炼模块和多种训练策略显著提升性能
...
Method: 使用SE-ResNeXt101作为骨干网络，采用多标签迭代分层进行交叉验证，结合不对称损失、混合精度训练、余弦学习率衰减、梯度裁剪和权重指数移动平均。提出轻量级标签图精炼模块，通过可学习的稀疏标签耦合矩阵优化logits

Result: 在数据集上，SE-ResNeXt101基线达到92.64%的宏AUC，添加标签图精炼模块后验证宏AUC在所有fold上持续提升，计算开销可忽略

Conclusion: 该方法可重现、硬件友好且无需额外标注，为多标签CXR分类器提供了实用的性能提升路径
...
<details>
  <summary>Details</summary>
Motivation: 现有DPO方法在扩散模型中存在两个关键限制：1）由于需要整个模型微调导致高计算成本；2）对参考模型质量敏感，容易引入不稳定性和偏差。

Method: 使用轻量级可训练的偏好分类器直接建模样本间的相对偏好，将偏好学习与生成模型解耦，无需整个模型微调或依赖参考模型。
...
Method: 提出动态实例和轮廓一致性对比学习(DI3CL)预训练框架，包含动态实例模块(增强全局上下文感知)和轮廓一致性模块(关注SAR地物几何轮廓)，使用460,532张SAR图像的大规模SARSense数据集进行预训练。

Result: 在SAR土地覆盖制图、水体检测和道路提取等多种任务上的实验表明，DI3CL方法优于现有方法。

Conclusion: DI3CL作为SAR土地覆盖分类的基础模型，为下游模型开发提供了强大支撑，代码和预训练权重已开源。
...
TL;DR: 提出了Q-Scorer框架，通过轻量级回归模块和IQA专用分数标记解决MLLMs在图像质量评估中的离散输出与连续分数需求不匹配问题，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在图像质量评估任务中存在离散标记输出与连续质量分数需求不匹配的问题，导致性能受限。现有方法在离散预测到连续分数转换中存在误差，且语义标记引入混淆。

Method: Q-Scorer框架包含轻量级回归模块和IQA专用分数标记，集成到MLLM流程中，通过理论分析指导设计来解决转换误差问题。

Result: 在多个IQA基准测试中达到最先进性能，在混合数据集上泛化良好，与其他方法结合时性能进一步提升。

Conclusion: Q-Scorer通过解决MLLMs在IQA任务中的核心不匹配问题，实现了优异的性能表现，具有良好的泛化能力和兼容性。
...
TL;DR: Sparse3DPR是一个无需训练的开源3D场景理解框架，利用预训练LLM的推理能力，仅需稀疏视角RGB输入，通过层次化平面增强场景图和任务自适应子图提取方法，显著提升了3D场景推理的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有的无训练3D场景理解方法虽然灵活且泛化性强，但在实际部署中面临准确性和效率问题。为了解决这些问题，需要开发一种既能保持灵活性又能提升性能的框架。

Method: 提出层次化平面增强场景图，支持开放词汇表并采用主导平面结构作为空间锚点；设计任务自适应子图提取方法，动态过滤查询无关信息以减少上下文噪声。

Result: 在Space3D-Bench上相比ConceptGraphs实现了28.7%的EM@1提升和78.2%的速度提升；在ScanQA上达到与训练方法相当的性能；真实世界实验证实了其鲁棒性和泛化能力。

Conclusion: Sparse3DPR通过结合预训练LLM的推理能力和创新的场景表示方法，成功解决了无训练3D场景理解中的准确性和效率问题，展现了在实际应用中的潜力。
...
TL;DR: 合成相关扩散成像(CDI^s)作为标准扩散成像协议的增强方法，可显著提升前列腺癌病灶分割性能，在94%的配置中可靠增强或保持分割效果，最高实现72.5%的相对改进。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习在前列腺癌病灶分割中表现有限，Dice分数在大型患者队列中仅为0.32或更低，需要更有效的成像增强方法。

Method: 使用200名患者的共配准CDI^s、DWI和ADC序列，在六种最先进的分割架构上进行全面评估，比较CDI^s集成与基线模态的性能。

Result: CDI^s集成在94%的配置中可靠增强或保持分割性能，个别架构实现高达72.5%的统计显著相对改进。CDI^s + DWI是最安全的增强路径，在一半架构中实现显著改进且无性能下降。

Conclusion: CDI^s作为现有DWI采集的衍生技术，无需额外扫描时间或架构修改，可立即部署到临床工作流中，为PCa病灶分割任务提供实用的即插即用增强方案。
...
TL;DR: SSOMotion是一个基于统一场景语义占据(SSO)表示的人类运动合成框架，通过双向三平面分解和CLIP编码实现场景语义理解，在复杂场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前方法主要关注场景结构而忽略语义理解，限制了人类运动合成的质量。

Method: 设计双向三平面分解获取紧凑SSO表示，通过CLIP编码和线性降维映射场景语义到统一特征空间，结合运动方向指令进行帧级场景查询控制。

Result: 在ShapeNet家具、PROX和Replica数据集上的实验表明该方法具有先进性能和良好泛化能力。

Conclusion: SSOMotion通过统一的场景语义表示有效提升了人类运动合成的质量，在复杂场景中表现出色。
...
TL;DR: CloudMamba是一个基于SSM的点云网络，通过序列扩展和合并、链式Mamba以及分组选择性状态空间模型，解决了点云序列化不完善、高层几何感知不足和S6模型过拟合的问题，在多个点云任务中实现了SOTA结果且复杂度更低。
...
Method: 1. 序列扩展和序列合并：沿各轴分别序列化点云，融合不同序列的因果推断高阶特征；2. 链式Mamba：在并行双向Mamba中链接前向和后向过程，捕获扫描过程中的高层几何信息；3. 分组选择性状态空间模型(GS6)：通过S6上的参数共享缓解过拟合问题。

Result: 在各种点云任务上的实验验证了CloudMamba能够以显著更低的复杂度实现最先进的结果。

Conclusion: CloudMamba通过创新的序列处理、几何信息捕获和模型优化策略，有效解决了Mamba在点云分析中的关键挑战，为点云处理提供了高效且强大的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 单目3D目标检测存在深度信息不明确和视野受限的问题，导致几何线索不足，在遮挡或截断场景中精度下降。现有方法虽然引入额外深度信息，但忽略了视觉线索对鲁棒识别的重要性。

Method: 1. 对视觉特征进行K-means聚类，捕捉不同的物体级外观部分（如引擎盖、车顶），提升部分可见物体的检测能力；2. 构建广义场景记忆，通过跨图像聚合聚类特征提供一致表示；3. 将局部聚类特征和广义场景记忆集成到物体查询中，引导注意力到信息丰富区域。

Result: 在KITTI基准测试中取得了最先进的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 受大语言模型跨领域泛化能力的启发，解决扩散模型在视觉任务中"单任务-单模型"范式限制通用性和可扩展性的问题
...
Conclusion: 分层自回归方法能够有效支持草图生成过程中的实时操控，相比现有方法具有更好的灵活性和实用性
...
TL;DR: 本文对文本极性检测中的功率律变换现象进行了理论分析，验证了暗文本在亮背景和亮文本在暗背景下的类间方差变化规律。


<details>
  <summary>Details</summary>
Motivation: 文本极性检测是图像二值化的重要预处理步骤，现有基于功率律变换的直观方法缺乏理论支撑，需要从理论上分析其有效性。

Method: 对功率律变换后的图像进行理论分析，研究文本和背景作为两个类别时，类间方差随变换参数的变化规律。

Result: 理论分析证实了暗文本在亮背景时类间方差增加，亮文本在暗背景时类间方差减少的现象。

Conclusion: 为文本极性检测中的功率律变换方法提供了理论依据，验证了该方法的有效性。
...
TL;DR: 提出HD²-SSC框架解决3D语义场景补全中的维度差距和密度差距问题，通过高维语义解耦模块和高密度占用优化模块提升场景理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有SSC方法存在输入输出维度差距和标注-现实密度差距问题，2D平面视图与稀疏标注导致对真实世界3D立体密集占用的预测效果不佳。

Method: 1. 高维语义解耦模块：将2D图像特征沿伪第三维度扩展，从遮挡中解耦粗粒度像素语义，识别具有精细语义的焦点区域；2. 高密度占用优化模块：采用'检测-优化'架构，利用上下文几何和语义结构完成缺失体素并修正错误体素。
...
TL;DR: 提出了一种基于图像的路径规划算法，使用计算机视觉技术和无人机生成的视差图来定义路径候选点，并与A*和PRM算法进行了比较。


<details>
  <summary>Details</summary>
Motivation: 地形深度对路径安全有重要影响，但二维图像无法区分陨石坑和山丘等特征，需要利用视差图来准确规划路径。

Method: 使用无人机生成地形视差图，应用边缘、直线和角点检测等计算机视觉技术，结合立体深度重建技术定义轨迹候选点，使用ArUco标记姿态估计和圆检测技术自动检测起点和目标点。

Result: 在V-REP仿真程序和实验室物理设置中与A*和PRM算法进行比较，结果令人满意，证明了所提算法的有效性。

Conclusion: 基于计算机视觉和视差图的路径规划算法在复杂地形环境中表现良好，为自主导航提供了有效的解决方案。
...
TL;DR: 提出FedMedCLIP方法，基于CLIP的联邦学习框架，通过掩码特征适应模块和本地分类器解决医疗图像分类中的隐私保护和资源效率问题。


<details>
  <summary>Details</summary>
Motivation: 解决医疗图像分析中深度模型对源数据的依赖带来的隐私问题，以及联邦学习中数据异构性和资源成本高的挑战。

Method: 使用CLIP预训练模型，引入掩码特征适应模块作为通信模块减少通信负载，冻结CLIP编码器降低计算开销，设计掩码MLP作为本地分类器，采用自适应KL散度蒸馏正则化实现模块间互学习。

Result: 在四个公开医疗数据集上验证，性能优于基线方法（如ISIC2019上比次优基线高8%），资源效率显著提升（比FedAVG快120倍）。

Conclusion: FedMedCLIP在保护隐私的同时实现了高效的医疗图像分类，为联邦学习在医疗领域的应用提供了可行解决方案。
...
TL;DR: 提出Layout Control (Laytrol)网络，通过继承MM-DiT参数来保持预训练知识，使用Layout Synthesis数据集缓解分布偏移，实现高质量的布局到图像生成。


<details>
  <summary>Details</summary>
Motivation: 解决现有布局到图像生成方法中存在的视觉质量低和风格不一致问题，这些方法通常因引入适配器模块而损失预训练知识。

Method: 构建Layout Synthesis数据集，提出Layout Control网络继承MM-DiT参数，采用专门的初始化方案，包括将布局编码器初始化为纯文本编码器，输出初始化为零，并应用对象级旋转位置嵌入。

Result: 定性和定量实验证明了该方法的有效性，生成的图像具有更高的视觉质量和更好的风格一致性。

Conclusion: 所提出的Laytrol网络和Layout Synthesis数据集能有效保持预训练知识，提升布局到图像生成的质量和一致性。
...
TL;DR: DiffRegCD是一个统一的密集配准和变化检测框架，通过高斯平滑分类实现亚像素精度的对应关系估计，利用预训练扩散模型特征提升对光照和视角变化的鲁棒性。
...
Result: 在多个航空和地面数据集上的实验表明，DiffRegCD持续超越现有基线方法，在宽时间跨度和几何变化下保持可靠性能。

Conclusion: 扩散特征和基于分类的对应关系估计为统一变化检测提供了坚实基础，显著提升了配准和变化检测的联合性能。
...
TL;DR: ISExplore是一种简单有效的片段选择策略，通过自动识别包含音频特征多样性、唇部运动幅度和相机视角数量的5秒参考视频片段，显著提升了说话人脸生成的数据处理和训练速度。


<details>
  <summary>Details</summary>
Motivation: 现有基于NeRF或3DGS的说话人脸生成方法需要处理数分钟的参考视频，耗时数小时，严重限制了实际应用价值。研究发现视频信息质量比长度更重要，几秒的信息丰富片段就能达到甚至超过完整视频的效果。

Method: 提出ISExplore策略，基于三个关键数据质量维度自动选择5秒信息丰富的参考视频片段：音频特征多样性、唇部运动幅度和相机视角数量。

Result: 实验表明，该方法使NeRF和3DGS方法的数据处理和训练速度提升了5倍以上，同时保持了高保真度的输出质量。

Conclusion: ISExplore通过智能选择信息丰富的短片段，显著提升了说话人脸生成的效率，证明了视频信息质量的重要性，为实际应用提供了可行解决方案。
...
TL;DR: 提出了一种基于多模态原型的多实例学习方法，通过双向交互和平衡信息压缩方案，利用冻结的LLM生成病理实体描述作为文本原型，视觉分支学习实例级原型，并使用立体最优传输算法进行融合，在三个癌症数据集上展示了优越的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决病理图像分析中计算成本高、仅提供袋级标签、LLM生成的实例级描述存在偏差，以及现有视觉语言多实例学习方法单向指导限制跨模态协同的问题。

Method: 提出多模态原型多实例学习框架，利用冻结LLM生成任务特定病理实体描述作为文本原型，视觉分支学习实例级原型减少对冗余数据的依赖，使用立体最优传输算法进行融合实现高维空间语义对齐。

Result: 在三个不同癌症数据集上进行少样本分类和可解释性实验，结果表明该方法具有优越的泛化能力。

Conclusion: 提出的多模态原型多实例学习方法通过双向交互和平衡信息压缩，有效解决了病理图像分析中的关键挑战，展示了良好的泛化性能。
...
TL;DR: 提出纯Mamba架构的ReIDMamba框架，解决了Transformer在行人重识别中的计算复杂性问题，通过多粒度特征提取器和排序感知三元组正则化实现高效特征学习。


<details>
  <summary>Details</summary>
Motivation: 解决Transformer在行人重识别中因序列长度导致的二次计算复杂度问题，同时克服CNN的局部处理特性和信息损失问题。

Method: 设计基于Mamba的强基线，引入多类别token；提出多粒度特征提取器模块形成多粒度特征；引入排序感知三元组正则化减少特征冗余。

Result: ReIDMamba参数仅为TransReID的三分之一，GPU内存使用更低，推理速度更快，在五个行人重识别基准测试中达到最先进性能。

Conclusion: ReIDMamba是首个将纯Mamba架构应用于行人重识别的研究，在计算效率和性能方面均表现出色，为ReID研究提供了新的方向。
...
TL;DR: 提出了一种新的突发图像质量评估任务(BuIQA)，建立了首个基准数据集，并开发了一个统一框架来评估突发序列中各帧的任务驱动质量，以提高下游任务效率。


<details>
  <summary>Details</summary>
Motivation: 突发成像技术虽然提升了视觉数据的捕获和处理能力，但图像冗余增加了存储传输需求并降低了下游任务效率，需要评估每帧的任务驱动质量来指导图像选择。

Method: 提出了统一的BuIQA框架，包括任务驱动的提示生成网络（通过异构知识蒸馏学习下游任务先验）和任务感知的质量评估网络（基于任务提示评估图像质量）。

Result: 在10个下游场景中的广泛实验表明，该方法在BuIQA性能上优于现有技术，应用于去噪和超分辨率任务时能带来0.33 dB PSNR提升。

Conclusion: BuIQA任务能有效评估突发图像的任务驱动质量，通过选择高质量帧显著提升下游任务性能，为突发图像处理提供了新的解决方案。
...
TL;DR: MMAssist是一个基于多模态辅助的3D无监督域自适应方法，通过使用图像和文本特征作为桥梁来对齐源域和目标域的3D特征，提升LiDAR点云3D目标检测在跨域场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然点云和图像通常同时采集，但在3D无监督域自适应中很少利用图像数据。本文旨在探索多模态数据（图像和文本）在提升3D域自适应性能方面的潜力。

Method: 提出MMAssist方法：1）将3D边界框投影到图像获取2D框；2）从预训练视觉骨干网络提取图像特征；3）使用大型视觉语言模型生成文本描述并提取文本特征；4）在源域和目标域训练中，对齐3D特征与图像/文本特征；5）融合3D特征和对齐特征进行最终预测；6）使用2D检测器增强伪标签。

Result: 在三个流行的3D目标检测数据集上的三个域自适应任务中，该方法相比最先进方法取得了有前景的性能提升。

Conclusion: 多模态辅助（图像和文本）能够有效提升3D无监督域自适应的性能，证明了在跨域3D目标检测中利用多模态信息的重要性。
...
TL;DR: 提出了一种模块化管道RoMa，通过扩散语义变形、密集配准和残差流精化，在不改变现有变化检测网络的情况下提高空间和时间鲁棒性，解决了遥感图像时空错位问题。


<details>
  <summary>Details</summary>
Motivation: 遥感变化检测面临长时间间隔图像空间错位的挑战，现有联合配准检测框架需要重新训练且跨域迁移性差，需要一种不改变现有变化检测网络的通用解决方案。

Method: 集成扩散语义变形、密集配准和残差流精化。扩散模块合成中间变形帧桥接外观差异，RoMa估计连续帧间对应关系，轻量级U-Net精化生成高保真扭曲场来配准原始图像对。

Result: 在LEVIR-CD、WHU-CD和DSIFN-CD数据集上的广泛实验显示，该方法在多个骨干网络上一致提高了配准精度和下游变化检测性能。

Conclusion: 提出的方法具有通用性和有效性，能够在不修改现有变化检测网络的情况下显著提升空间和时间鲁棒性。
...
TL;DR: 提出DANCE框架用于点云补全，通过射线采样和多视角生成候选点，使用transformer解码器优化位置并预测不透明度，无需固定输入/输出密度或图像监督，在PCN和MVP基准测试中表现优异。
...
Method: DANCE通过射线采样从多视角生成候选点，transformer解码器优化位置并预测不透明度分数，轻量级分类头在几何特征上训练以提供语义指导。

Result: 在PCN和MVP基准测试中，DANCE在准确性和结构一致性方面优于最先进方法，对不同输入密度和噪声水平具有鲁棒性。

Conclusion: DANCE能够仅补全缺失区域同时保留观察到的几何结构，无需外部图像监督即可实现类别一致的补全。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在生成放射学报告时，往往无法充分描述罕见但临床重要的病理（如骨折），存在描述不足的问题
...
Result: 在Places365和COCOA数据集上的实验表明，CSF-Net有效减少物体幻觉，增强视觉真实性和语义对齐。
...
TL;DR: 提出了一种基于YOLOv8n的优化低功耗边缘AI系统，用于杂草检测，部署在STM32U575ZI微控制器上，实现每推理仅51.8mJ的极低能耗。
...
Method: 在STM32U575ZI微控制器上部署YOLOv8n目标检测器，应用结构化剪枝、整数量化和输入图像分辨率缩放等压缩技术，在CropAndWeed数据集（74种植物）上训练评估。

Result: 系统实现了检测精度与效率的平衡，支持实时原位杂草检测，每推理能耗仅为51.8mJ。
...
TL;DR: SharpV是一个高效的自适应视觉token和KV缓存剪枝方法，通过动态调整剪枝比例和自校准机制，在减少计算复杂度的同时保持甚至提升性能。
...
Method: 提出两阶段剪枝框架：1）基于时空信息动态调整视觉token剪枝比例；2）通过自校准方式剪枝退化的视觉特征，基于与原始特征的相似度指导。

Result: 在多个公开基准测试中表现出优越性，且是首个不需要访问注意力分数的两阶段剪枝框架，完全兼容硬件加速技术。
...
TL;DR: EAGLE框架通过外观感知和几何感知记忆实现自我中心视觉查询的2D-3D统一定位，在Ego4D-VQ基准上达到最先进性能。
...
Conclusion: EAGLE通过记忆整合机制有效支持目标外观变化的长期和短期建模，实现了2D和3D任务的统一，能够快速准确地将结果反向投影到3D空间。
...
TL;DR: AdvRoad是一种生成道路风格对抗性海报的方法，通过在道路上放置看似自然的对抗性图案，诱导3D物体检测器感知到不存在的物体，实现隐蔽攻击。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉的3D物体检测系统容易受到对抗性攻击，但现有方法生成的对抗性海报外观不自然且容易被人类察觉，存在防御漏洞。

Method: 采用两阶段方法：道路风格对抗生成和场景关联适应，确保对抗性海报具有自然外观的同时最大化攻击效果。

Result: 实验表明AdvRoad在不同检测器、场景和欺骗位置上都表现出良好的泛化能力，物理攻击进一步证明了其在真实环境中的实际威胁。

Conclusion: AdvRoad展示了自动驾驶系统中视觉3D检测器的安全漏洞，强调了开发更鲁棒防御机制的必要性。
...
TL;DR: 提出了Cascade HQP-DETR方法，通过高质量合成数据生成、基于高质量建议的查询编码和级联去噪算法，解决了合成数据训练对象检测模型时的三个关键问题，在仅使用合成数据训练12个epoch的情况下，在PASCAL VOC 2007上达到了61.04% mAP@0.5的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 真实世界对象检测需要大规模标注数据集，但创建成本高昂且耗时。虽然合成数据训练（ISOD）提供了替代方案，但现有方法面临合成数据质量差、DETR检测器收敛慢且容易过拟合合成模式、以及统一去噪压力导致模型过拟合伪标签噪声等问题。
...
Conclusion: Cascade HQP-DETR通过高质量合成数据生成、智能查询初始化和动态去噪策略，有效解决了合成数据训练对象检测模型的关键挑战，为降低标注成本提供了可行的解决方案。
...
TL;DR: 提出基于特征金字塔-Transformer的多模态深度伪造检测与定位框架，在IJCAI'25 DDL-AV基准测试中取得0.7535分，能同时检测伪造概率和精确定位被篡改片段


<details>
  <summary>Details</summary>
Motivation: 现有单模态检测方法无法利用跨模态相关性，且难以精确定位伪造片段，限制了应对复杂细粒度篡改的实际应用

Method: 使用预训练自监督模型提取音频和视频的层次时序特征，构建多尺度特征金字塔，通过R-TLM块和局部注意力机制分析跨上下文时序依赖，双分支预测头同时预测伪造概率和优化被篡改片段的时间偏移

Result: 在IJCAI'25 DDL-AV基准测试集上获得0.7535的最终得分，验证了方法在挑战性环境下的有效性
...
TL;DR: 本文提出了3DGS-QA，这是首个针对3D高斯泼溅渲染技术的主观质量评估数据集，并开发了一个无需参考的质量预测模型，直接从3D高斯基元中提取特征来评估感知质量。


<details>
  <summary>Details</summary>
Motivation: 尽管3D高斯泼溅技术在实时高保真渲染方面表现出色，但其渲染内容的感知质量在不同重建条件下的表现尚未得到系统研究。实践中，视角稀疏性、训练迭代次数有限、点云下采样、噪声和颜色失真等因素会显著影响视觉质量。

Method: 创建了包含15种物体类型的225个退化重建的3DGS-QA数据集；开发了无需参考的质量预测模型，直接从3D高斯基元中提取空间和光度线索，以结构感知的方式估计感知质量。

Result: 实验结果表明，该方法在3DGS内容评估方面始终表现出优越性能，证明了其鲁棒性和有效性。

Conclusion: 3DGS-QA数据集和提出的质量评估模型为3D高斯泼溅技术的质量评估研究提供了重要基础，公开的数据集和代码将促进该领域的未来发展。
...
<details>
  <summary>Details</summary>
Motivation: 单目深度估计具有内在不适定性，而现代视觉基础模型在大规模数据集上预训练后展现出强大的世界理解能力，可以提升深度估计性能。

Method: 将视觉基础模型作为多级特征增强器，系统地在不同表示层级注入先验知识，不修改模型结构和预训练权重。

Result: 在NYU-Depth v2和KITTI数据集上建立新的SOTA性能，与需要多次前向传播的扩散方法和相对深度预训练方法相比具有竞争力，并展现出强大的零样本迁移能力。

Conclusion: WEDepth方法有效激发了视觉基础模型的固有先验，在单目深度估计任务中取得了优异性能，且具有良好的泛化能力。
...
Method: 使用概率U-Net主干网络捕获多样专家假设，通过提示引导的投影机制在潜在空间中导航生成个性化分割，采用多级对比目标对齐文本和视觉表示。
...
TL;DR: GECO2是一种端到端的少样本计数和检测方法，通过新的密集查询表示在不同尺度上逐步聚合特定样本特征信息，解决了物体尺度多样性问题，在计数和检测精度上比现有方法提升10%，同时运行速度快3倍且GPU内存占用更小。


<details>
  <summary>Details</summary>
Motivation: 现有的少样本计数器在处理包含不同大小物体和密集小物体区域的图像时存在困难，因为它们采用临时解决方案（如上采样和分块）来应对多尺度定位和密集小物体检测问题。

Method: 提出GECO2方法，使用新的密集查询表示，在不同尺度上逐步聚合特定样本的特征信息，生成高分辨率密集查询，从而能够同时检测大物体和小物体。

Result: GECO2在计数和检测精度上比现有最先进的少样本计数器提升10%，同时运行速度快3倍，GPU内存占用更小。

Conclusion: GECO2通过显式处理物体尺度问题，在少样本计数和检测任务中取得了显著的性能提升，同时提高了计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 主题驱动图像生成需要在保持特定主题核心身份特征的同时，在不同上下文中合成新颖描绘，这面临着身份一致性和提示多样性之间的基本权衡问题。

Method: 采用LoRA微调的扩散模型，结合潜在拼接策略联合处理参考图像和目标图像，使用掩码条件流匹配目标。引入两阶段蒸馏数据策展框架：第一阶段通过数据恢复和VLM过滤创建高质量种子数据集；第二阶段利用这些数据进行参数高效微调。
...
TL;DR: I2E框架通过模拟微扫视眼动，将静态图像转换为高保真事件流，解决了SNN训练中事件流数据稀缺的问题，实现了300倍以上的转换速度提升和92.5%的SOTA准确率。


<details>
  <summary>Details</summary>
Motivation: 解决脉冲神经网络(SNN)训练中事件流数据严重稀缺的瓶颈问题，为高性能神经形态系统开发提供可扩展的数据解决方案。

Method: 通过高度并行化的卷积模拟微扫视眼动，将静态图像转换为高保真事件流，支持实时数据增强。

Result: 在I2E-ImageNet数据集上训练的SNN达到60.50%的SOTA准确率；通过模拟到真实范式，在CIFAR10-DVS数据集上达到92.5%的创纪录准确率。
...
TL;DR: 提出了首个用于雷达心跳感知的无监督框架Radar-APLANC，通过增强伪标签和噪声对比实现无需昂贵标注信号的鲁棒心跳检测。
...
Conclusion: Radar-APLANC框架成功实现了无需真实标注的无监督雷达心跳感知，在性能上可与监督方法媲美，为实际应用提供了更实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究潜在扩散模型（如Stable Diffusion）在文本到图像生成过程中是否真正理解生成图像的语义信息。

Method: 通过在Stable Diffusion上使用简单的回归层来预测对象语义属性，并与人类标注进行比较分析。

Result: 发现语义理解成功主要归因于CLIP的文本编码，而非反向扩散过程；不同语义属性的解码准确度差异显著；在反向扩散过程中属性区分度降低。
...
<details>
  <summary>Details</summary>
Motivation: 现有的身份保持评估指标依赖全局嵌入或粗略的VLM提示，无法捕捉细粒度身份变化且诊断能力有限。

Method: 将主体分层分解为(类型,风格)→属性→特征的决策树，并提示具体转换而非抽象相似度分数，从而将VLM分析基于可验证的视觉证据。
...
TL;DR: 提出了StableMorph方法，使用基于扩散的图像合成技术生成高质量、无伪影的人脸融合图像，用于改进人脸融合攻击检测系统的评估。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸融合生成方法通常产生模糊、有伪影或构建不良的图像，这些图像容易被检测且不能代表最危险的攻击，需要更高质量的融合图像来开发和评估有效的检测系统。

Method: 使用现代基于扩散的图像合成技术生成高度逼真、无伪影的人脸融合图像，能够生成全头部图像并避免常见的视觉缺陷。

Result: StableMorph生成的图像不仅达到或超过了真实人脸图像的质量，而且保持了欺骗人脸识别系统的强大能力，为现有检测解决方案带来了更大挑战。
...
TL;DR: 提出了一个针对尼龙面罩(NFM)这种新型3D欺骗攻击的数据集，使用iPhone 11 Pro采集了3760个真实样本和51281个攻击样本，评估了5种先进PAD方法的性能。


<details>
  <summary>Details</summary>
Motivation: 人脸识别系统在智能手机认证、门禁控制等应用中广泛部署，但对呈现攻击(PA)仍然脆弱。尼龙面罩(NFM)因其弹性结构和逼真外观能够紧密模拟受害者面部几何形状，构成严重威胁。

Method: 创建了一个专注于NFM攻击的新数据集，使用iPhone 11 Pro采集数据，包含100个受试者的3760个真实样本和51281个NFM攻击样本，涵盖人类和人体模型四种不同呈现场景。

Result: 使用5种最先进的PAD方法对数据集进行基准测试，结果显示不同方法在未见攻击条件下的性能存在显著差异。

Conclusion: NFM攻击带来了重大挑战，强调了开发能够有效泛化到新兴欺骗威胁的PAD技术的重要性。
...
Method: 集成EfficientNet-B0 CNN和Swin Tiny Transformer，使用空间注意力模块强调高质量脊线区域，特征融合后投影到512维嵌入空间，采用余弦相似度进行匹配。

Result: 在两个公开数据集上的实验表明，LatentPrintFormer在Rank-10识别率上持续优于三种最先进的潜指纹识别技术。

Conclusion: LatentPrintFormer通过结合CNN和Transformer架构，有效提升了潜指纹识别的性能，在复杂场景下表现出色。
...
TL;DR: 提出结合联邦学习和SAM2图像分割模型的新框架，用于污水处理厂的泡沫自动检测，解决数据隐私和标注数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 污水处理厂泡沫形成会降低处理效率和增加成本，但传统机器学习需要大量标注数据且不同工厂因隐私问题不愿共享数据。

Method: 使用联邦学习框架结合SAM2模型，在分布式客户端上微调模型，通过Flower框架协调中央服务器聚合模型权重而不访问私有数据。

Result: 该框架加速了训练收敛并提高了分割性能，即使在有限本地数据集下也能有效工作，使用真实和合成数据进行验证。

Conclusion: 该研究展示了将大规模基础模型集成到联邦学习系统中的巨大潜力，为处理分布式敏感数据的工业挑战提供了实用解决方案。
...
TL;DR: OTSNet提出了一种受神经认知启发的观察-思考-拼写三阶段网络，通过双注意力编码器、位置感知模块和多模态协作验证器解决场景文本识别中的跨模态对齐问题，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有场景文本识别框架中的解耦视觉-语言优化会放大跨模态不对齐导致的错误传播，视觉编码器存在背景干扰注意力偏差，解码器在解析几何变形文本时存在空间不对齐问题。

Method: 采用三阶段观察-思考-拼写流程：1)双注意力编码器通过差异注意力图抑制无关区域；2)位置感知模块和语义量化器结合空间上下文与字形级语义抽象；3)多模态协作验证器通过跨模态融合实现自校正。

Result: 在Union14M-L基准测试上达到83.5%平均准确率，在严重遮挡的OST数据集上达到79.1%准确率，在14个评估场景中的9个创造了新记录。

Conclusion: OTSNet通过模拟人类视觉感知的层次认知过程，有效解决了场景文本识别中的跨模态对齐问题，在多个挑战性数据集上实现了最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有事件-RGB数据集在极端条件覆盖稀疏且空间分辨率低(≤640×480)，无法全面评估检测器在挑战性场景下的性能。

Method: 构建PEOD数据集，包含130+时空对齐序列和340k手动边界框，57%数据在低光、过曝和高速运动条件下采集。在三种输入配置(事件、RGB、事件-RGB融合)上对14种方法进行基准测试。

Result: 在完整测试集和正常子集上，融合模型表现优异；在光照挑战子集上，顶级事件模型优于所有融合模型，但融合模型仍优于RGB模型，表明当帧模态严重退化时现有融合方法存在局限性。
...
TL;DR: 提出了一种名为Boomda的多模态领域自适应方法，通过平衡多目标优化解决不同模态间领域偏移差异的问题。


<details>
  <summary>Details</summary>
Motivation: 多模态学习面临标注数据稀缺的挑战，而现有的无监督领域自适应方法主要针对单模态场景，在多模态设置下研究较少，特别是处理不同模态间领域偏移差异的问题。

Method: 首先使用信息瓶颈方法独立学习每个模态的表示，然后通过相关性对齐在表示空间中对齐源域和目标域。将问题建模为多目标优化任务，通过简化为二次规划问题并获得闭式解，实现高效的模态平衡多模态领域自适应。

Result: 广泛的实证结果表明所提方法有效，Boomda优于竞争方案。

Conclusion: 提出的Boomda方法通过平衡多目标优化成功解决了多模态领域自适应中不同模态间领域偏移不平衡的问题，提供了一种高效的解决方案。
...
TL;DR: 提出了一个针对新视角合成图像的非对齐参考图像质量评估框架，通过对比学习结合LoRA增强的DINOv2嵌入，在合成失真数据上训练，实现了对对齐和非对齐参考的鲁棒质量评估。
...
Method: 构建大规模合成失真图像数据集，针对时间兴趣区域。基于对比学习框架，结合LoRA增强的DINOv2嵌入，并利用现有IQA方法进行监督。仅在合成失真数据上训练，避免对特定真实NVS样本过拟合。

Result: 在FR-IQA、NR-IQA和NAR-IQA方法中表现最优，在对齐和非对齐参考上都实现了鲁棒性能。用户研究表明模型预测与人类主观评分有强相关性。

Conclusion: 提出的NAR-IQA框架有效解决了新视角合成图像的质量评估问题，特别是在非对齐参考情况下，展现了良好的泛化能力和与人类感知的一致性。
...
TL;DR: 提出了LandSegmenter框架，通过弱监督数据、跨模态特征提取和置信度融合策略，解决了LULC映射中数据标注成本高、模型泛化能力差的问题。


<details>
  <summary>Details</summary>
Motivation: 当前LULC模型受限于特定模态和固定分类体系，泛化能力不足。基础模型需要大量标注数据，在遥感领域成本高昂且不实用。

Method: 构建LAS弱标签数据集，集成遥感专用适配器和文本编码器进行跨模态特征提取，采用类别置信度引导的融合策略。

Result: 在6个LULC数据集上的迁移学习和零样本实验显示，LandSegmenter达到竞争性或更优性能，尤其在零样本设置下表现突出。

Conclusion: 弱监督是构建任务特定基础模型的有效方法，LandSegmenter框架在LULC映射中展现出良好效果。
...
TL;DR: 提出了一种名为Mg-MRN的多粒度相互精炼网络，通过解耦多粒度特征学习和跨粒度特征交互来改进零样本学习中的视觉特征表示。


<details>
  <summary>Details</summary>
Motivation: 当前零样本学习方法通常将全局视觉特征与语义信息关联，或对齐局部视觉区域特征与对应属性，但忽视了局部区域特征之间的内在交互作用，这可以进一步改善可迁移和显式视觉特征的获取。

Method: 设计了多粒度特征提取模块来通过解耦区域特征挖掘学习区域级判别特征，然后通过跨粒度特征融合模块加强不同粒度区域特征之间的内在交互，通过整合相邻层次的区域表示来增强每个粒度级别的表示判别性。

Result: 在三个流行的ZSL基准数据集上的广泛实验证明了所提出的Mg-MRN方法的优越性和竞争力。

Conclusion: 通过解耦多粒度特征学习和跨粒度特征交互，Mg-MRN能够精炼判别性和可迁移的视觉特征，有效提升零样本学习的识别性能。
...
TL;DR: 提出了分布式零样本学习框架DistZSL，通过跨节点属性正则化和全局属性-视觉共识来解决分布式数据中的异构性问题，提升未见类别的学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有的零样本学习方法通常假设数据集中存储，但在实际应用中数据往往是分布式的，且存在数据异构性问题。需要开发能够充分利用分布式数据学习有效模型的框架。

Method: 1. 跨节点属性正则化：强制不同节点间属性特征距离相似，稳定属性特征空间；2. 全局属性-视觉共识：确保属性与视觉特征分布的双向映射在不同节点间一致，减少个体节点的有偏V2A映射。

Result: 大量实验表明，DistZSL在从分布式数据学习中达到了最先进的性能。

Conclusion: DistZSL框架通过解决分布式数据中的异构性问题，有效提升了零样本学习的性能，为分布式环境下的零样本学习提供了可行的解决方案。
...
TL;DR: 提出VLMDiff，一种结合潜在扩散模型和视觉语言模型的无监督多类别视觉异常检测框架，通过VLM提取图像描述作为扩散模型的额外条件，无需人工标注或额外训练即可实现多类别异常检测。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的方法依赖合成噪声生成，泛化能力有限且需要每类别单独训练，难以扩展到多类别场景。需要一种能够处理多样化多类别真实图像的无监督异常检测方法。

Method: 使用预训练的视觉语言模型通过简单提示提取详细图像描述，作为潜在扩散模型的额外训练条件，学习鲁棒的正常图像特征表示。

Result: 在Real-IAD数据集上像素级PRO指标提升高达25个点，在COCO-AD数据集上提升8个点，优于当前最先进的基于扩散的方法。

Conclusion: VLMDiff框架通过结合VLM和LDM，在无需人工标注的情况下实现了有效的多类别视觉异常检测，显著提升了性能。
...
TL;DR: WarpGAN通过引入变形-修复策略，将图像修复融入3D GAN反演，解决了单视角图像在3D GAN反演中遮挡区域生成质量差的问题。
...
Method: 首先使用3D GAN反演编码器将单视角图像投影为潜在码，然后利用3D GAN生成的深度图进行新视角变形，最后开发SVINet利用对称先验和同一潜在码的多视角图像对应关系修复变形图像中的遮挡区域。

Result: 定量和定性实验表明，该方法在多个先进方法中表现一致更优。

Conclusion: WarpGAN通过变形-修复策略有效提升了3D GAN反演中遮挡区域的生成质量和多视角一致性。
...
TL;DR: 提出像素级质量评估(PQA)框架，通过像素级空间一致性替代传统的框级IoU预测，解决检测器中定位质量估计的结构耦合问题，提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用预测框与真实框的IoU作为定位质量代理，但存在结构耦合问题：预测框来自检测器对真实框的内部估计，导致定位差的框也会被高估IoU。

Method: PQA框架测量每个像素相对于预测框和真实框的相对位置对齐程度，通过像素级空间一致性避免直接比较预测框与估计的真实框，消除相似性偏差。

Result: 在HRSC2016和DOTA数据集上的实验表明，PQA可无缝集成到各种定向目标检测器中，显著提升性能（如Rotated RetinaNet提升5.96% AP50:95，STD提升2.32%）。

Conclusion: PQA通过像素级质量评估有效解决了框级IoU预测的结构耦合问题，为定位质量估计提供了更准确的近似，可广泛应用于定向目标检测器。
...
TL;DR: UI2Code$^\text{N}$是一个通过交互式UI到代码范式训练的视觉语言模型，在UI编程任务中实现了最先进的性能，与领先的闭源模型相当。


<details>
  <summary>Details</summary>
Motivation: 当前UI编程方法面临两个关键限制：多模态编码能力发展不足，单轮范式很少利用迭代视觉反馈。

Method: 采用交互式UI到代码范式，通过分阶段预训练、微调和强化学习训练视觉语言模型，统一了UI到代码生成、UI编辑和UI优化三个关键能力，并探索了交互式生成的测试时扩展。

Result: 在UI到代码和UI优化基准测试中，UI2Code$^\text{N}$在开源模型中建立了新的最先进水平，性能与Claude-4-Sonnet和GPT-5等领先闭源模型相当。

Conclusion: 交互式UI到代码范式更好地反映了真实工作流程，提高了可实现的性能上限，UI2Code$^\text{N}$展示了在多模态UI编程任务中的卓越能力。
...
<details>
  <summary>Details</summary>
Motivation: 医学领域面临数据稀缺、标注成本高以及新兴/罕见疾病的挑战，需要开发能够识别未知类别的开放集识别方法。

Method: 提出一种损失函数，利用辅助数据集来惩罚开放空间区域，基于深度神经网络特征围绕类均值聚类且类均值构成正则单纯形顶点的观察。

Result: 在四个MedMNIST数据集（BloodMNIST、OCTMNIST、DermaMNIST、TissueMNIST）和一个公开皮肤数据集上显著优于现有最先进技术。

Conclusion: 该方法在医学图像开放集识别任务中表现出色，为解决医学领域数据限制和未知类别识别问题提供了有效解决方案。
...
TL;DR: 研究发现图像到3D生成模型存在强烈的规范视角偏见，通过轻量级CNN检测和校正输入方向可以缓解此问题，无需修改生成主干网络。


<details>
  <summary>Details</summary>
Motivation: 尽管大规模图像到3D生成模型取得了令人印象深刻的结果，但其归纳偏见仍然不透明。作者识别了图像条件3D生成模型的一个重要限制：强烈的规范视角偏见。

Method: 通过使用简单2D旋转的受控实验，展示最先进的Hunyuan3D 2.0模型在视角泛化方面的困难。使用轻量级CNN检测和校正输入方向来缓解此问题。

Result: 实验表明，在旋转输入下模型性能会下降，但通过方向检测和校正可以恢复模型性能。

Conclusion: 研究结果提出了一个重要开放问题：规模是否足够，还是应该追求模块化、对称感知的设计？
...
TL;DR: 本文提出了一种解耦的多模态食物识别管道，通过评估不同视觉骨干网络与生成式大语言模型的组合，分析了视觉分类准确性、模型效率和生成输出质量之间的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 数字食品应用的普及需要强大的自动化营养分析和烹饪指导方法，同时需要解决公共数据集中的文化偏见问题。

Method: 构建了一个解耦的多模态管道，集成专门的视觉骨干网络（EfficientNet-B4）与生成式大语言模型（Gemini），并引入语义错误传播（SEP）概念来分析分类错误如何影响生成输出。使用新的自定义中国食品数据集（CCFD）进行评估。

Result: 实验结果显示，EfficientNet-B4（89.0% Top-1准确率）在准确性和效率之间提供了最佳平衡，Gemini（9.2/10事实准确性）提供了卓越的生成质量，但系统的整体效用受到视觉前端感知准确性的根本性限制。

Conclusion: 系统的整体性能主要受限于视觉前端的感知准确性，高语义相似度是最关键的失败模式。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统3D超分辨率方法需要高分辨率RGB引导的复杂性问题，为实际应用场景提供更简单实用的解决方案。

Method: 使用投影归一化坐标码(PNCC)将3D几何表示为规则图像，采用两种实现：基于Swin Transformer的高精度版本和基于Vision Mamba的高效率版本。

Result: Swin Transformer模型在标准基准测试中达到最先进精度，Vision Mamba模型在实时速度下提供有竞争力的结果。

Conclusion: 该几何引导管道为实际场景提供了简单可行且实用的解决方案，特别是在无法获取高分辨率RGB数据的情况下。
...
TL;DR: 提出了一种基于语义和关系改进多模态对齐与融合的方法，通过提取多层次语义特征、学习相关语义分组，并使用可继承的交叉注意力机制来去除冗余视觉关系。
...
Method: 1) 从不同视觉编码器提取多层次语义特征；2) 学习将视觉特征投影到相关语义组；3) 使用可继承的交叉注意力融合视觉和文本特征，通过丢弃低相关性的视觉-语言特征对来全局去除冗余视觉关系。
...
TL;DR: 提出了DiPVNet，一种方向感知的向量网络，通过原子点积算子同时编码方向选择性和旋转不变性，在局部和全局层面分别捕捉非均匀结构和整体方向结构，在点云分类和分割任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 点云处理中的任意旋转会破坏点云的内在方向特性，现有方法未能充分利用点云的多尺度方向性来增强特征表示。

Method: 提出DiPVNet，包含局部可学习点积算子(L2DP)捕捉非均匀局部结构，以及全局方向响应谱通过方向感知球面傅里叶变换(DASFT)建模整体方向结构，两者均具有旋转不变性。
...
TL;DR: NERVE是一个无需训练的开源词汇语义分割方法，通过整合全局和局部信息、使用随机游走优化亲和力、基于熵选择注意力图，在7个基准测试中达到最先进的零样本分割性能。
...
Method: 利用稳定扩散模型的自注意力层获取邻域结构；使用随机游走而非固定高斯核进行亲和力优化；基于熵的不确定性选择最相关的注意力图；无需CRF或PAMR等传统后处理技术。

Result: 在7个流行的语义分割基准测试中实现了最先进的零样本分割性能。

Conclusion: NERVE提供了一种有效的开放词汇语义分割方法，能够准确分割任意形状的物体，无需传统后处理技术。
...
TL;DR: LayerEdit是一个无需训练的多层解耦编辑框架，通过精确的对象层分解和融合，解决了多对象图像编辑中的注意力纠缠问题，实现无冲突的对象层编辑。


<details>
  <summary>Details</summary>
Motivation: 现有文本驱动的多对象图像编辑方法主要遵循定位-编辑范式，专注于独立的对象定位和编辑，但忽视了关键的对象间交互。被忽视的注意力纠缠在对象冲突区域中阻碍了解耦的多对象编辑，导致对象间编辑泄漏或对象内编辑约束。

Method: LayerEdit提出"分解-编辑-融合"框架：1）冲突感知层分解模块，使用注意力感知IoU方案和时间相关区域移除来增强冲突感知和抑制；2）对象层编辑模块，建立协调的层内文本引导和跨层几何映射，实现解耦的语义和结构修改；3）透明度引导层融合模块，通过精确的透明度引导学习促进结构一致的对象间层融合。
...
<details>
  <summary>Details</summary>
Motivation: 解决从航拍视图生成地面图像的挑战，包括极端视角差异、遮挡和有限视野范围的问题。
...
Conclusion: Top2Ground通过几何约束和语义一致性，成功实现了从航拍到地面视图的直接生成，具有优异的性能表现。
...
TL;DR: ImageBindDC是一个新颖的多模态数据压缩框架，在ImageBind的统一特征空间中操作，通过特征函数损失实现精确的统计对齐，在三个关键层面保持分布一致性：单模态对齐、跨模态对齐和联合模态对齐。


<details>
  <summary>Details</summary>
Motivation: 传统数据压缩技术在单模态场景中成功，但在多模态场景中往往失败，因为无法保留模态间复杂的依赖关系。需要开发能够保持多模态数据中复杂互模态依赖关系的数据压缩方法。

Method: 在ImageBind统一特征空间中操作，使用特征函数(CF)损失在傅里叶域进行精确统计对齐，通过无限矩匹配实现三个层面的分布一致性：单模态对齐、跨模态对齐和联合模态对齐。

Result: 在NYU-v2数据集上，仅使用每类5个压缩数据点训练的模型就能达到与完整数据集训练相当的无损性能，相比之前最佳方法绝对提升8.2%，压缩时间减少4倍以上。

Conclusion: ImageBindDC通过在多模态数据压缩中引入特征函数损失和三个层面的分布对齐，显著提升了压缩效率和模型性能，为多模态数据压缩设定了新的技术标准。
...
<details>
  <summary>Details</summary>
Motivation: 在极端条件下（如光线不足、相机剧烈运动），RGB模态信息严重损失，现有方法性能显著下降。事件模态虽然能提供补充，但与RGB存在异构性，导致特征不匹配和优化困难。

Method: 提出边缘感知潜在重编码，通过重编码分布引导将事件-RGB特征对齐到统一语义空间，并利用预建边缘字典作为线索转换特征分布。然后通过重编码巩固和不确定性优化解决异构融合问题。

Result: 在两个合成和一个真实世界事件-RGB语义分割数据集上测试，在DERS-XS数据集上比现有最佳方法提升2.55% mIoU，并在空间遮挡下表现出优越的鲁棒性。

Conclusion: 该方法通过边缘感知的异构特征统一，有效解决了极端条件下事件-RGB语义分割的挑战，展示了在恶劣环境下的优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 大规模病理图像数据集的标注是开发深度学习模型的主要瓶颈，传统的文件夹标注工作流程缓慢、疲劳且难以扩展。

Method: 开发了SWAN（SWipeable ANnotations），一个支持桌面和移动平台的网页应用，使用滑动手势进行图像块分类，支持实时元数据捕获和灵活的滑动-标签映射。

Result: 在四位病理学家标注600个有丝分裂图像块的试点研究中，SWAN实现了86.52%-93.68%的配对一致性（Cohen's Kappa = 0.61-0.80），与传统文件夹方法（86.98%-91.32%，Cohen's Kappa = 0.63-0.75）表现相当。
...
TL;DR: 提出了一种机制感知的无监督通用图像融合方法MAUGIF，基于双交叉图像自编码器，根据融合任务的内在机制进行分类，通过双编码器和解码器实现多源图像的结构和互补信息融合。
...
Conclusion: MAUGIF方法通过机制感知的设计，在保持通用性的同时提升了图像融合的性能和可解释性。
...
TL;DR: SynWeather是首个用于统一多区域多变量天气观测数据合成的数据集，覆盖四个代表性区域和四个关键天气变量。同时提出了基于扩散变换器的SynWeatherDiff模型来解决传统方法过度平滑的问题。


<details>
  <summary>Details</summary>
Motivation: 当前天气数据合成方法通常专注于单变量、单区域任务，主要依赖确定性建模，这限制了跨变量和跨区域的统一合成，忽视了跨变量的互补性，并经常导致过度平滑的结果。

Method: 提出了SynWeather数据集和SynWeatherDiff模型。数据集覆盖美国大陆、欧洲、东亚和热带气旋四个区域，包含雷达反射率、小时降水、可见光和微波亮温四个关键变量。模型基于扩散变换器框架，是一个通用的概率天气合成模型。

Result: 在SynWeather数据集上的实验表明，该网络相比特定任务模型和通用模型都表现出更好的效果。

Conclusion: SynWeather数据集和SynWeatherDiff模型有效解决了传统天气数据合成方法的局限性，实现了跨变量和跨区域的统一合成，避免了过度平滑问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多视角方法依赖大规模标注数据集进行训练，在测试场景不同时泛化能力差。需要一种不依赖3D真值监督且能良好泛化的方法。

Method: 将人体姿态建模为由3D高斯组成的骨架（每个关节一个高斯），通过可微分渲染进行优化，提出新颖的one-hot编码方案实现关节的独立优化。

Result: 在Human3.6M和CMU数据集上超越了不依赖3D真值的方法，相比基于学习的方法将跨数据集误差降低了47.8%。在遮挡场景下表现出鲁棒性，无需场景特定微调。

Conclusion: SkelSplat提供了一种有效的多视角3D人体姿态估计方法，不依赖3D真值监督，具有良好的泛化能力和遮挡鲁棒性。
...
TL;DR: 提出了NeuSpring方法，通过神经弹簧场从视频中重建和模拟可变形物体，解决了现有方法对未来预测泛化能力差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注当前状态的物理学习，但忽略可变形物体的内在物理特性，导致对未来预测的泛化能力较差。

Method: 基于弹簧质量模型，提出分段拓扑解法和神经弹簧场：1）分段拓扑使用零阶优化高效建模多区域弹簧连接拓扑；2）神经弹簧场通过基于坐标的神经网络表示不同帧间的弹簧物理特性。

Result: 在真实数据集上，NeuSpring在重建和模拟方面表现优异，当前状态建模和未来预测的Chamfer距离分别提高了20%和25%。

Conclusion: NeuSpring通过结合弹簧物理模型和神经网络，有效提升了可变形物体的重建精度和物理模拟的预测能力。
...
TL;DR: 提出一种减少AI系统版本更新时负翻转（新模型错误分类旧模型正确样本）的方法，通过保持原始模型边界同时学习改进模型，结合边界校准和双重焦点蒸馏损失。


<details>
  <summary>Details</summary>
Motivation: 随着训练类别数量增加，添加新类别会减少每个类的边界并引入冲突模式，从而降低原始子集性能，需要解决版本更新时的负翻转问题。

Method: 通过显式边界校准项保持原始模型边界，结合双重焦点蒸馏损失从新旧数据中学习适当的决策边界，平衡新旧类别的准确性。

Result: 在图像分类基准测试中，该方法持续降低负翻转率，同时保持高整体准确率。

Conclusion: 该方法有效缓解了模型更新时的负翻转问题，在保持整体性能的同时显著减少不一致性。
...
TL;DR: 本研究比较了多种乳腺X光片纵向对齐策略，发现基于图像的配准方法在风险预测性能上优于特征级对齐和隐式对齐方法，能够生成更准确的变形场并提高预测精度。


<details>
  <summary>Details</summary>
Motivation: 乳腺X光筛查对早期乳腺癌检测至关重要，但纵向影像间的空间对齐不准确会掩盖有意义的组织变化并降低模型性能。

Method: 评估了三种对齐策略：基于图像的配准、特征级对齐（有/无正则化）和隐式对齐方法，使用两个大规模乳腺X光数据集进行对比分析。

Result: 基于图像的配准在所有指标上均优于其他方法，能够生成平滑、解剖学合理的变形场，并在特征空间中应用图像变形场可获得最佳风险预测性能。

Conclusion: 基于图像的变形场在纵向风险建模中具有重要价值，能够提高预测准确性和鲁棒性，有望增强个性化筛查和早期干预能力。
...
<details>
  <summary>Details</summary>
Motivation: 水下实例分割在海洋资源勘探和生态保护中至关重要，但现有方法难以适应水下环境的复杂特性。本文旨在利用DINO等视觉基础模型的强大特征学习能力来解决水下实例分割问题。

Method: 提出DiveSeg框架，包含两个核心组件：(1) AquaStyle Aligner：在DINO微调过程中嵌入水下颜色风格特征，增强对水下域的适应性；(2) ObjectPrior Prompter：使用基于二值分割的提示来提供对象级先验，为需要对象级和实例级推理的任务提供指导。
...
TL;DR: 提出了一种基于扩散模型的sEMG数据增强方法SASG-DA，通过语义表示引导和稀疏感知采样，生成既忠实又多样的样本，有效缓解过拟合并提升手势识别性能。
...
Conclusion: 该数据增强方法通过提供忠实且多样的样本，有效解决了sEMG数据稀缺问题，为基于sEMG的人机交互系统提供了可靠的数据增强解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多跳问题生成仅限于文本领域，而视频问题生成仅限于单片段的零跳问题，无法满足跨多视频片段推理的需求。

Method: 采用模块化架构，基于改进的BART骨干网络，增强视频嵌入，捕捉文本和视觉依赖关系。使用TVQA+数据集自动构建大规模MVQ-60数据集。
...
TL;DR: SLOPE是一个统一框架，将结构化稀疏性和低比特量化有效结合，通过训练时正则化策略减少精度损失，在保持高精度的同时实现显著模型压缩。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在资源受限设备上运行困难，现有方法如权重量化和结构化稀疏性单独使用有效，但组合使用时会对模型精度产生复合负面影响。
...
Conclusion: SLOPE框架能够有效结合结构化稀疏性和低比特量化，在保持模型性能的同时实现显著的压缩效果。
...
TL;DR: 提出了一种分层矢量量化变分自编码器，通过解耦运动伪影特征来改进MRI运动校正的泛化能力，无需针对特定伪影进行训练。
...
Method: 使用分层矢量量化变分自编码器学习运动到干净图像特征的解耦嵌入，部署码本在多分辨率下捕获有限运动模式集合，结合自回归模型学习无运动图像先验分布。

Result: 在模拟全身运动伪影上验证，观察到在不同运动严重程度下都能进行稳健校正，有效解耦了模拟运动扫描的物理运动特征。

Conclusion: 该方法通过解耦运动特征，提高了基于机器学习的MRI运动校正的泛化能力，有望应用于不同解剖区域和运动类型。
...
TL;DR: 本文质疑RoPE的成功是否真的源于位置等变性，提出了非交换生成器的Spherical RoPE，并在视觉任务中取得了与等变方法相当或更好的性能。
...
Result: Spherical RoPE在视觉任务中表现出与等变方法相当或更好的学习行为，表明相对位置嵌入的重要性可能被高估。

Conclusion: 相对位置嵌入在计算机视觉中可能没有想象中重要，这为开发更快、泛化更好的非等变位置编码开辟了新方向。
...
TL;DR: 本文提出了基于文本的空中-地面行人检索任务(TAG-PR)，构建了TAG-PEDES数据集和TAG-CLIP检索框架，解决了跨视角行人检索中的视角差异挑战。
...
Method: 提出了TAG-CLIP检索框架，包含分层路由专家混合模块学习视角特定和视角无关特征，以及视角解耦策略分离视角特定特征以实现更好的跨模态对齐。

Result: 在提出的TAG-PEDES数据集和现有T-PR基准上评估了TAG-CLIP的有效性。

Conclusion: TAG-PR任务具有重要实用意义，TAG-CLIP框架能有效处理视角异构性，提出的数据集和代码已开源。
...
TL;DR: RAPTR：一种基于雷达的弱监督3D人体姿态估计方法，仅使用3D边界框和2D关键点标签，通过两阶段姿态解码器架构实现高精度姿态估计。


<details>
  <summary>Details</summary>
Motivation: 传统雷达室内3D人体姿态估计依赖昂贵的3D关键点标注，在复杂室内环境中成本高昂。本文旨在开发仅需易获取的3D边界框和2D关键点标签的弱监督方法。
...
Conclusion: RAPTR证明了在弱监督设置下仅使用3D边界框和2D关键点标签即可实现高质量的雷达3D人体姿态估计，为大规模应用提供了可行方案。
...
TL;DR: Anatomy-VLM是一个细粒度的视觉语言模型，通过定位关键解剖特征、融入结构化知识和对齐多尺度医学信息，实现了专家级的放射学疾病诊断。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型将图像视为整体，忽略了疾病诊断所需的关键细粒度图像细节。临床医生通过利用先验医学知识识别重要解剖结构来分析图像，这启发了本研究的开发。

Method: 设计模型编码器定位医学图像中的关键解剖特征；用结构化知识丰富这些区域以实现上下文感知解释；对齐多尺度医学信息以生成临床可解释的疾病预测。

Result: Anatomy-VLM在分布内和分布外数据集上均表现出色，在下游图像分割任务中验证了其细粒度对齐捕获解剖和病理相关知识的能力。

Conclusion: Anatomy-VLM的编码器支持零样本解剖学解释，展现了强大的专家级临床解释能力，为放射学疾病诊断提供了有效的细粒度解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测方法学习单一纠缠的伪造表示，混淆了内容相关缺陷和内容无关伪影，且受限于过时的基准测试。

Method: 采用解耦的专家混合架构，包含可路由的专用语义专家和固定的通用伪影专家，通过两阶段训练策略确保专业化。

Result: 在传统基准和新提出的Mirage数据集上的实验表明，该模型超越了现有单体检测器，为AIGI认证建立了新的鲁棒标准。

Conclusion: OmniAID通过明确解耦"生成什么"和"如何生成"，实现了对现代真实世界威胁的鲁棒泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 半监督学习可以在有限标注数据和大量未标记数据的情况下训练强大模型，特别是在医学图像分割领域，标注成本高昂

Method: 设计混合双分支金字塔网络(DBPNet)，包含一个编码器和两个略有差异的解码器，每个解码器生成多分辨率尺度的扰动辅助预测；提出CPCR学习策略，结合现有一致性学习和不确定性最小化方法，并引入新的正则化项
...
### [107] [Generalizable Blood Cell Detection via Unified Dataset and Faster R-CNN](https://arxiv.org/abs/2511.08465)
*Siddharth Sahay*

Main category: cs.CV

TL;DR: 本文提出了自动化外周血细胞分类和检测的综合方法，通过整合四个公共数据集并使用Faster R-CNN框架，比较了随机初始化与迁移学习两种训练方案，证明迁移学习具有更快的收敛速度和更好的性能。
...
Method: 开发数据管道整合四个公共数据集，采用Faster R-CNN目标检测框架（ResNet-50-FPN骨干网络），比较随机初始化与基于COCO预训练权重的迁移学习两种训练方案。
...
TL;DR: 提出了对比积分梯度(CIG)方法，用于全切片图像分析，通过对比梯度计算增强可解释性，能更好地区分肿瘤亚型。


<details>
  <summary>Details</summary>
Motivation: 现有积分梯度方法在全切片图像分析中可能忽略类别区分信号，需要一种能更好区分肿瘤亚型的方法来提高AI辅助诊断的可解释性和可信度。

Method: CIG方法在logit空间计算对比梯度，通过比较特征相对于参考类别的重要性来突出类别区分区域，同时满足积分归因的公理要求。

Result: 在三个癌症数据集上的实验表明，CIG在定量指标(MIL-AIC和MIL-SIC)和定性可视化方面都产生更具信息量的归因结果，与真实肿瘤区域更吻合。

Conclusion: CIG方法为全切片图像分析提供了更可解释和可信的诊断工具，能够更好地突出类别区分区域，增强AI辅助诊断的可靠性。
...
Method: 提出SENCA-st（共享编码器与邻域交叉注意力）架构，该架构保持两种模态的特征，特别通过交叉注意力机制强调在组织病理学上结构相似但在空间转录组上功能不同的区域。

Result: 模型在检测肿瘤异质性和肿瘤微环境区域方面表现出优越性能，超越了现有最先进方法，这在临床上是至关重要的方面。

Conclusion: SENCA-st架构能够有效整合组织病理学和空间转录组数据，在保持两种模态特征的同时突出功能差异区域，为肿瘤异质性分析提供了更准确的工具。
...
TL;DR: CoMa是一种压缩预训练阶段，作为对比学习的热身阶段，可将视觉语言模型转化为有竞争力的嵌入模型，在MMEB基准测试中实现了效率和效果的双重优化。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明视觉语言模型可通过大规模对比学习适应为嵌入模型，但作者认为全面理解输入和对比学习这两个目标可以解耦，全面理解有助于通过对比学习在下游任务中获得更好性能。

Method: 提出CoMa压缩预训练阶段作为对比学习的热身阶段，仅需少量预训练数据即可将视觉语言模型转化为竞争性嵌入模型。

Result: 在MMEB基准测试中，CoMa在同等规模的视觉语言模型中实现了新的最先进结果，实现了效率和效果的双重优化。

Conclusion: CoMa通过解耦全面理解输入和对比学习目标，证明了仅需少量预训练数据就能有效提升视觉语言模型在嵌入任务中的性能。
...
TL;DR: 提出了一种基于分层稀疏采样和残差Transformer的快速多器官分割框架，在保持准确性的同时显著降低计算成本，在CPU上仅需约2.24秒完成分割。


<details>
  <summary>Details</summary>
Motivation: 解决3D医学图像多器官分割中神经网络逐体素分割带来的巨大时间和内存消耗问题，在保持速度与准确性之间找到更好的平衡。

Method: 使用分层稀疏采样策略减少计算时间，同时利用多分辨率层次保留上下文信息；采用残差Transformer分割网络从稀疏描述符的不同层级提取和组合信息。

Result: 在包含10,253张CT图像的内部分析集和公开数据集TotalSegmentator上，相比现有快速器官分类器，在定性和定量分割性能上均有提升，CPU硬件上速度达到约2.24秒。
...
Method: 采用Plan-and-Act双智能体架构：规划智能体解析用户意图并分解为结构化步骤，执行智能体通过模块化工具服务器执行；使用分层多级内存系统维持长期推理和上下文连续性。

Result: 实现了迭代和任意条件的视频工作流，支持文本/图像/视频条件生成→多轮编辑→对象分割→组合合成等复杂操作。

Conclusion: UniVA和配套的UniVA-Bench基准测试套件完全开源，旨在推动交互式、智能化和通用视频智能的研究。
...
TL;DR: 提出了Large Sign Language Models (LSLM)框架，利用大语言模型作为骨干网络来翻译3D美国手语，旨在改善听障人士的虚拟交流。


<details>
  <summary>Details</summary>
Motivation: 现有手语识别方法依赖2D视频，无法充分捕捉3D场景中的丰富空间、手势和深度信息。本工作旨在通过直接使用3D手语数据，提供更准确和鲁棒的翻译，增强听障社区的数字交流可访问性。

Method: 利用大语言模型作为骨干，直接处理3D手势特征。研究两种设置：从3D手势特征到文本的直接翻译，以及通过外部提示调节翻译的指令引导设置。

Result: 开发了一个能够理解复杂、具身多模态语言的框架，超越了纯文本输入的限制，扩展了大语言模型对人类交流的理解能力。

Conclusion: 这项工作为实现包容性、多模态智能系统迈出了基础性一步，使系统能够理解多样化的语言形式。
...
Result: 成功构建了3D4D框架，能够将静态内容转换为连贯的4D场景，支持自适应、用户驱动的4D环境探索。

Conclusion: 3D4D框架为复杂4D环境的交互式可视化提供了有效的解决方案，项目代码已开源。
...
TL;DR: 提出一个从带噪声外参的多视角图像直接重建高质量可编辑3D网格的鲁棒框架，同时联合优化相机位姿和学习隐式场景表示，生成的网格与标准3D工具兼容。


<details>
  <summary>Details</summary>
Motivation: 真实场景中获取精确相机位姿困难，现有基于NeRF的方法严重依赖准确外参估计，且其隐式体表示与广泛应用的多边形网格差异大，导致在标准3D软件中渲染和操作效率低下。

Method: 联合优化相机位姿和学习隐式场景表示，同时捕获精细几何细节和逼真外观，直接生成可编辑的3D网格。

Result: 在标准基准测试中，该方法在位姿不确定性下实现了准确鲁棒的3D重建，生成的网格与常见3D图形和机器人工具兼容。

Conclusion: 该方法弥合了神经隐式表示与实际机器人应用之间的差距，为下游任务提供了高效实用的3D重建解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习定位模型存在两个主要问题：对输入数据同等关注权重，以及不适用于非序列数据（如瞬时CSI）。

Method: 采用基于注意力机制的Vision Transformer架构，专注于从CSI矩阵中提取角度延迟剖面特征。

Result: 在DeepMIMO数据集上室内RMSE为0.55m，室外为13.59m；在ViWi数据集上室外阻塞场景RMSE为3.45m，比现有最优方法提升约38%。
...
TL;DR: 比较生成式模型和判别式模型在COVID-19胸部X光检测中的表现，发现小型判别式模型在准确性和环境影响方面优于大型语言模型。


<details>
  <summary>Details</summary>
Motivation: 随着AI工具在医疗应用中的普及，大型语言模型因其多功能性和易用性被广泛使用，但小型定制模型可能更适合特定任务。本研究旨在比较不同类型模型在COVID-19检测中的表现和环境影响。

Method: 将LLMs和小型判别式模型集成到Mendix应用中检测COVID-19胸部X光，使用判别式模型为LLMs提供知识库以提高准确性，共测试14种不同模型配置。

Result: 小型模型降低了碳足迹但输出偏向阳性诊断且置信度不足；限制LLMs仅输出概率导致准确性和碳足迹表现均差；Covid-Net模型表现最佳，准确率达95.5%，碳足迹比GPT-4.5-Preview减少99.9%。

Conclusion: 生成式工具用于分类任务存在环境风险，小型判别式模型在准确性和环境影响方面优于大型生成式模型，应谨慎使用LLMs作为通用AI解决方案。
...
TL;DR: 提出基于自主代理的边缘设备教育内容生成框架，解决撒哈拉以南非洲教育不平等问题，在资源受限环境中实现去中心化、文化适应的教育内容生成。


<details>
  <summary>Details</summary>
Motivation: 解决撒哈拉以南非洲的教育不平等问题，为资源受限环境提供可访问、本地化和可持续的AI驱动教育方案。

Method: 采用四个专业代理协同工作的自主代理编排框架，在边缘设备上生成上下文适当的教育内容，支持多种非洲语言。

Result: 在Jetson Nano上实现129ms首token时间、33ms平均token延迟、45.2 tokens/秒吞吐率，功耗8.4W；在Raspberry Pi 4B上实现326ms首token时间、15.9 tokens/秒吞吐率，功耗5.8W；多语言质量BLEU得分0.688，文化相关性4.4/5，流畅度4.2/5。

Conclusion: 该框架为资源受限环境建立了实用基础，支持联合国可持续发展目标4、9和10，通过社区组织合作实现长期可行性和文化适应性。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于规则的奖励强化学习在小型模型上经常导致低质量推理链或推理过程与最终答案不一致的问题，限制了资源有限组织在小规模模型上进行直接强化学习训练的能力。
...
TL;DR: 使用分层任务网络(HTN)的程序性知识可以显著提升LLM在代理任务中的性能，即使是较小的模型也能超越更大模型的基线表现。


<details>
  <summary>Details</summary>
Motivation: LLM在执行代理任务时通常需要大量工具支持、提示工程或微调，而领域相关的程序性知识可以大幅提升规划效率，但很少有研究评估其在需要隐式规划的代理任务中的潜力。

Method: 形式化、实现并评估了一个利用分层任务网络(HTN)形式程序性知识的代理LLM工作流程。

Result: 实证结果显示，手工编码的HTN能显著提升LLM在代理任务中的性能，使用HTN的20b或70b参数LLM可以超越120b参数LLM的基线表现。LLM创建的HTN也能提升性能，但效果较弱。
...
TL;DR: Orion是一个训练框架，使紧凑模型（350M-1.2B参数）能够通过学习的搜索策略执行迭代检索，在多个基准测试中超越更大模型。


<details>
  <summary>Details</summary>
Motivation: 当前检索方法存在局限：神经检索器缺乏推理能力，LLM成本过高，查询重写或分解仅限于静态转换，无法捕捉复杂查询所需的迭代探索、反馈和修订动态。

Method: 结合：(1)合成轨迹生成和监督微调以鼓励多样化探索模式；(2)强化学习奖励有效查询细化和回溯行为；(3)推理时波束搜索算法利用RL期间学习的自反能力。

Result: 1.2B模型在SciFact上达到77.6%成功率（先前为72.6%），BRIGHT上25.2%（vs 22.1%），NFCorpus上63.2%（vs 57.8%），在FEVER、HotpotQA和MSMarco上保持竞争力，在6个基准中的5个上超越200-400倍大的检索器。
...
Method: GSW框架包含Operator将观察映射到中间语义结构，以及Reconciler将这些结构整合到持久工作空间中，确保时间、空间和逻辑一致性。

Result: 在Episodic Memory Benchmark上，GSW比现有RAG基线性能提升高达20%，查询时上下文token减少51%，显著降低推理时间成本。

Conclusion: GSW为赋予LLM类似人类的情景记忆提供了具体蓝图，为实现能够进行长时程推理的更强大智能体铺平了道路。
...
TL;DR: 提出一个AI增强的团队贡献评估框架，通过整合多种团队工作成果和沟通记录，使用客观指标和LLM分析来识别和解决评估冲突。


<details>
  <summary>Details</summary>
Motivation: 团队中个体贡献的公平评估存在挑战，冲突和工作量差异可能导致不公平绩效评估，传统手动干预成本高且困难。

Method: 将异构数据组织为贡献、互动和角色三个维度的九个基准，使用客观指标和基尼系数识别冲突，采用LLM架构进行验证和情境分析。

Result: 设计了一个能够生成可解释和透明咨询判断的框架，整合了情感分析、任务保真度、字数/代码行数等实用分析功能。

Conclusion: 该框架在当前法规和制度政策下具有可行性，但需要解决偏见防护、限制和实际挑战。
...
TL;DR: 该研究提出了一个框架，通过7阶段校准序列和5层保护架构，使人类-AI团队在高风险战略决策中建立认知伙伴关系，避免可预防的认知陷阱和决策遗憾。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在可验证领域表现出色，但在具有不确定结果的高风险战略决策中可靠性不足，这种差距源于人类和AI系统中相互强化的认知偏见，威胁到估值的可辩护性和投资可持续性。

Method: 通过对7个前沿级LLM和3个市场导向风险案例进行系统定性评估，开发了包含4阶段初始化过程和7阶段校准序列的框架，采用5层保护架构实现偏见自我监控、人机对抗挑战、伙伴状态验证、性能退化检测和利益相关者保护。

Result: 研究发现：伙伴状态可通过有序校准实现但需要紧急维护协议；当架构漂移和上下文耗尽同时发生时可靠性会下降；解散纪律可防止追求根本错误方向。跨模型验证显示不同LLM架构存在系统性性能差异。

Conclusion: 该方法证明人类-AI团队能够建立认知伙伴关系，在高风险决策中预防可避免的遗憾，满足依赖AI系统支持关键决策而不引入可预防认知陷阱的投资回报期望。
...
TL;DR: AIA Forecaster是一个基于LLM的判断预测系统，通过智能搜索、监督代理和统计校准技术，在ForecastBench上达到人类超级预测者水平，并在预测市场中提供增量信息。


<details>
  <summary>Details</summary>
Motivation: 开发能够利用非结构化数据进行专业级判断预测的AI系统，超越现有LLM基准，实现可验证的专家级预测能力。

Method: 结合三个核心要素：1) 对高质量新闻源的智能搜索；2) 监督代理协调同一事件的不同预测；3) 统计校准技术对抗LLM的行为偏见。

Result: 在ForecastBench上表现与人类超级预测者相当，超越先前LLM基准；在液体预测市场基准中，与市场共识的集成优于单独的市场共识。
...
TL;DR: 提出了ResearchRubrics基准，用于评估深度研究(DR)代理的能力，包含2800+小时人工构建的2500+专家编写的细粒度评分标准，评估事实基础、推理合理性和清晰度。


<details>
  <summary>Details</summary>
Motivation: 深度研究代理需要处理开放式查询，但现有评估方法难以应对长篇幅、多样化的响应，且缺乏标准化评估框架。

Method: 构建ResearchRubrics基准，包含领域多样的提示和专家编写的评分标准；提出三维复杂度框架（概念广度、逻辑嵌套、探索性）；开发人工和模型评估协议。

Result: 评估多个最先进的DR系统，发现即使是领先的Gemini DR和OpenAI DR也只能达到平均68%的评分标准符合度，主要问题在于遗漏隐含上下文和对检索信息的推理不足。

Conclusion: 需要更强大、可扩展的深度研究能力评估方法，为此发布了ResearchRubrics基准以推动研究助手的发展。
...
TL;DR: 本文提出了一个基于LLM的多智能体多模态推理框架，用于自动生成军事训练场景中的关键文档（如作战命令OPORD），通过分层分解任务和使用专门化智能体来克服传统AI工具的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统模拟训练场景生成过程复杂且资源密集，而前LLM时代的AI工具难以生成足够复杂和自适应的场景，需要更先进的自动化解决方案。

Method: 采用多智能体框架，将场景生成分解为层次化子问题，每个智能体专门处理特定子任务，通过文本和视觉信息（地图特征、单位位置）的集成推理，按顺序处理输出以保持逻辑一致性。

Result: 通过概念验证，成功生成了OPORD的机动方案和移动部分，同时准确估计了地图位置和移动，证明了框架的可行性和准确性。
...
TL;DR: 开发了一个用于甲烷排放检测的机器学习系统，通过模型集成减少74%的误报，在联合国环境规划署的甲烷警报与响应系统中成功部署，7个月内验证了1,351个甲烷泄漏。


<details>
  <summary>Details</summary>
Motivation: 现有卫星甲烷检测方法误报率高，需要人工验证，无法满足大规模数据处理需求。

Method: 创建了最大的全球甲烷羽流标注数据集，比较不同深度学习模型配置，采用模型集成方法，从瓦片级评估扩展到全图评估。

Result: 系统在7个月运行中验证了1,351个甲烷泄漏，发出479个利益相关方通知，模型集成减少74%误报。
...
TL;DR: 本文提出对齐感知量化(AAQ)方法，在保持模型效率的同时解决量化导致的安全对齐退化问题，通过对比学习损失在量化过程中明确保留安全对齐特性。


<details>
  <summary>Details</summary>
Motivation: 传统后训练量化(PTQ)方法仅关注降低困惑度，但可能导致模型安全对齐显著退化，揭示困惑度作为安全代理指标的不足。

Method: 提出对齐感知量化(AAQ)，在PTQ流程中集成对齐保持对比(APC)损失，使量化模型模仿安全指令调优模型，同时偏离未对齐的预训练模型。

Result: AAQ方法在LLaMA、Qwen和Mistral等多个模型家族上实现稳健的4位(W4A4)量化，在保持安全性的同时保持效率，而先前方法在此方面失败。
...
TL;DR: 提出GAMA方法，使用图感知多模态注意力机制解决VRP问题，通过分别编码问题实例和解决方案，利用自注意力和交叉注意力建模模态内和模态间交互，显著优于现有神经基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有神经邻域搜索方法依赖简单状态表示，通过朴素连接融合异构信息，限制了捕获丰富结构和语义上下文的能力。

Method: 使用图神经网络分别编码问题实例和演化解决方案作为不同模态，通过堆叠的自注意力和交叉注意力层建模模态内和模态间交互，采用门控融合机制将多模态表示集成到结构化状态中。

Result: 在各种合成和基准实例上的广泛实验表明，GAMA显著优于最近的神经基线方法。消融研究证实多模态注意力机制和门控融合设计对性能提升起关键作用。

Conclusion: GAMA通过图感知多模态注意力模型有效解决了VRP问题，展示了多模态表示和门控融合在神经邻域搜索中的重要性。
...
TL;DR: WaterMod是一种基于概率感知模块化规则的水印方法，通过将词汇表按模型概率排序后按模k划分，在保持生成质量的同时嵌入可检测的水印信号，支持零位和多位水印设置。


<details>
  <summary>Details</summary>
Motivation: 现有基于logit的水印方法通过随机划分词汇表来嵌入水印，但这种随机划分可能排除最高概率的token，从而损害生成流畅性。需要一种既能保持生成质量又能有效嵌入水印的方法。

Method: 将词汇表按模型概率降序排序，然后按秩模k划分，将语义相似的相邻token分配到不同类别。在零位设置(k=2)中使用熵自适应门选择奇偶类，多位设置(k>2)时根据当前有效载荷数字选择对应模k类，并对选定类应用固定偏置。

Result: 实验结果表明，WaterMod在零位和多位设置下都能保持强水印检测性能，同时维持生成质量，在自然语言生成、数学推理和代码合成等多种任务中表现出鲁棒性。

Conclusion: WaterMod通过概率感知的模块化规则，在保证生成流畅性的同时实现了有效的水印嵌入，支持二进制归属和丰富有效载荷，为合成内容的溯源提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 非侵入性脑机接口需要既准确又可信，现有方法在不确定性量化和决策可靠性方面存在不足。

Method: 使用紧凑的面向语音的卷积网络深度集成，通过集成预测熵、前二边际和互信息量化不确定性，采用带有弃权选项的选择性分类。

Result: 与基线方法相比，该方法产生更可靠的概率估计，在不同操作点下改进选择性性能，并实现平衡的每类接受率。

Conclusion: 置信度感知的神经解码可以为现实世界脑机接口通信系统提供稳健的、面向部署的行为。
...
TL;DR: 开发针对失语症患者的EEG通信支持系统，通过分析正确与错误发音的神经活动差异，构建多任务学习框架来解码说话意图，即使在发音错误时也能有效识别。


<details>
  <summary>Details</summary>
Motivation: 失语症严重影响语言表达能力，导致频繁发音错误，而现有的脑机接口技术很少专门针对失语症患者开发EEG通信支持系统。

Method: 招募一名表达性失语症患者进行韩语自动语音任务，记录EEG信号并标记正确/错误发音。基于频谱分析发现神经活动差异，开发了带最大均值差异正则化的软多任务学习框架，专注于delta特征来优化分类并对齐特征分布。

Result: 模型对正确发音的识别准确率为58.6%，错误发音为45.5%，后者相比基线提高了45%以上，表明即使在发音错误时也能稳健地解码说话意图。
...
TL;DR: SparseRM利用稀疏自编码器从LLM表示中提取偏好相关信息，构建轻量级可解释的奖励模型，在仅使用不到1%可训练参数的情况下超越主流奖励模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统奖励模型依赖大规模偏好标注和昂贵的LLM微调，在资源有限环境下训练可靠奖励模型具有挑战性。

Method: 使用稀疏自编码器分解LLM表示为可解释的偏好相关特征方向，通过投影计算对齐分数，简单奖励头聚合分数预测偏好得分。
...
TL;DR: 提出了一种通过大语言模型生成数据文本描述的新方法，结合外部知识库和影响力估计来选择最信息丰富的描述，并通过跨模态迁移分类验证描述的有效性。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型行为仍被视为黑盒，现有XAI方法主要关注模型预测解释，而本文希望理解数据如何通过深度学习训练被解释，使用人类易于理解的语言媒介。

Method: 构建生成数据文本描述的流水线，利用大语言模型和外部知识库，引入影响力估计和CLIP分数选择最有信息量的文本描述，提出跨模态迁移分类基准任务。
...
TL;DR: 提出DANS-KGC方法，通过难度评估、自适应负采样和动态训练机制，解决知识图谱表示学习中负采样策略的局限性。
...
Method: 包含三个核心组件：难度评估模块（整合语义和结构特征）、自适应负采样模块（使用条件扩散模型生成不同难度负样本）、动态训练机制（在训练过程中动态调整样本难度分布）。

Result: 在六个基准数据集上的实验表明，DANS-KGC在UMLS和YAGO3-10数据集的所有三个评估指标上均达到最先进水平。

Conclusion: DANS-KGC通过难度感知的负采样策略，有效提升了知识图谱补全的性能和泛化能力。
...
Conclusion: 这些发现识别了人类适应性推理的神经特征，并强调了需要将振荡反馈协调纳入脑启发人工智能以实现真正的上下文敏感适应。
...
TL;DR: 基于扩散模型的实时想象语音分类框架，用于失语症患者的脑电信号解码，在有限校准数据下实现快速收敛和稳定泛化。


<details>
  <summary>Details</summary>
Motivation: 为严重表达性语言障碍患者开发实用的临床通信支持系统，将想象语音脑机接口转化为实际临床应用。

Method: 集成轻量级条件扩散编码器和卷积分类器，采用双准则早停策略、dropout正则化和分组时间卷积，处理两秒滑动窗口的连续脑电信号。

Result: 在线测试中达到65% top-1和70% top-2准确率，优于离线评估的50% top-1准确率。

Conclusion: 该框架证明了在临床约束条件下部署基于扩散模型的脑电解码的可行性，为想象语音脑机接口的临床转化提供了重要进展。
...
TL;DR: 提出了一种新的同构数学问题生成任务(IMPG)，开发了基于LLM的CBIT框架，通过元级生成和模板化选择性变异，在保持数学正确性和结构一致性的同时降低了生成成本。
...
Method: 探索了基于LLM的自动IMPG框架，建立了计算蓝图同构双胞胎(CBIT)，采用元级生成和模板化选择性变异方法。

Result: CBIT在生成准确性和成本效益方面表现优异，生成的问题错误率比专家编写的问题低17.8%，在商业教育平台上部署给6,732名学习者，产生了186,870次交互。

Conclusion: CBIT框架能够高效生成结构一致的同构数学问题，在真实教育环境中表现出良好的实用性和效果。
...
TL;DR: 开发了一个实时无线想象语音脑电信号解码系统，旨在提高脑机接口在现实世界中的实用性，支持便携无线设备并实现个性化服务。


<details>
  <summary>Details</summary>
Motivation: 传统脑机接口研究主要局限于静态固定环境，限制了实际应用。为了推动脑机接口走向实用化，需要开发灵活、适合日常使用的系统。

Method: 使用实验室流层管理实时脑电信号流，结合用户识别模块提供个性化服务，构建端到端实时应用管道，对想象语音脑电信号进行分类。

Result: 在有线设备上实现4类分类准确率62.00%，在便携无线头戴设备上达到46.67%的准确率。

Conclusion: 这是朝着真正实用和可访问脑机接口技术的重要一步，为未来稳健、实用和个性化神经接口研究指明了方向。
...
Method: 将复杂问题分解为子问题，每个子问题用自然语言和等效逻辑函数双重表示，通过逻辑函数传递子问题间依赖关系，并进行知识边界确定以避免不必要的外部搜索。

Result: 仅需数百训练样本即可与现有基线竞争，在完整训练集上显著优于其他方法，在不同数据集和模型规模上表现优异。

Conclusion: Thinker通过分层思维模型使推理过程可监督和可验证，提高了逻辑连贯性，在少量训练数据下即能取得良好效果。
...
<details>
  <summary>Details</summary>
Motivation: 时间序列生成对下游挖掘任务很重要，但面临建模时间动态内在随机性的挑战。扩散模型计算效率低，而传统流匹配无法显式捕捉随机性。SDE天然适合建模随机性和不确定性。

Method: 提出TimeFlow框架：基于SDE的流匹配，采用编码器架构，设计分量分解的速度场捕捉时间序列多面结构，在流匹配优化中增加随机项增强表达能力，支持无条件/条件生成。

Result: 在多个数据集上的广泛实验表明，模型在生成质量、多样性和效率方面始终优于强基线方法。
...
TL;DR: VARS是一种基于图表示的心电图诊断方法，通过将ECG信号转换为图结构来处理不同配置的信号，并提高风险信号的检测能力。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习ECG诊断方法在临床应用中面临的两个主要障碍：处理不同配置ECG信号的通用性不足，以及由于样本不平衡导致的风险信号检测不充分。

Method: 使用基于图的表示方法统一建模异构ECG信号，将ECG信号转换为可捕捉关键诊断特征的图结构，结合去噪重建和对比学习来保留原始ECG信息并突出病理模式。

Result: 在三个不同的ECG数据集上评估，VARS在所有数据集上都优于现有最先进模型，在风险信号识别方面有显著改进，并能提供可解释性。
...
TL;DR: 提出了一个细粒度反事实解释框架，通过量化相似性和加权组件贡献，在非生成方式下生成对象级和部件级的可解释反事实，解决模型误分类时的细粒度特征归因问题。


<details>
  <summary>Details</summary>
Motivation: 基于归因的解释技术在细粒度任务中缺乏足够的粒度，特别是在模型误分类情况下，现有解释可能不够详细，无法提供深入洞察。

Method: 通过量化正确分类和误分类样本之间感兴趣区域的相似性，加权组件贡献来生成反事实解释；引入基于Shapley值贡献的显著性分割模块，分离具有区域特定相关性的特征。

Result: 大量实验表明该方法在捕捉更细粒度、直观有意义的区域方面表现优越，超越了现有的细粒度方法。

Conclusion: 该框架能够有效识别导致模型误分类的细粒度特征，并为反事实调整提供更详细的局部特征影响分析。
...
<details>
  <summary>Details</summary>
Motivation: 现有法律基准往往混淆事实记忆与真正推理，碎片化推理过程，忽视推理质量。为克服这些局限，需要构建能反映真实司法决策过程的多步骤法律推理数据集。

Method: 基于IRAC框架构建中文多步骤法律推理数据集，设计可扩展的人机协作标注流程，生成细粒度步骤级推理标注。评估多个LLMs，并比较自主生成与人工设计的思维链提示效果。

Result: 多个LLMs在MSLR上表现中等，显示适应复杂法律推理的挑战。自主生成的思维链提示在推理连贯性和质量上优于人工设计提示。

Conclusion: MSLR推动了LLM推理和思维链策略的发展，为未来研究提供开放资源，证明了自主生成推理提示在提升法律推理质量方面的有效性。
...
Method: 使用区域嵌入层学习时间特定嵌入，基于自注意力机制的空间-时间依赖学习模块捕获邻近和遥远区域的联合时空依赖关系。
...
TL;DR: 将脑调谐方法扩展到多模态音频-视频模型，针对社交认知的关键区域STS进行微调，提升了脑对齐度和下游社交认知任务（情景喜剧中的讽刺检测）性能
...
TL;DR: 该论文提出了VSPO数据集和模型，用于生成验证本体语义陷阱的能力问题(CQs)，通过微调LLaMA-3.1-8B-Instruct模型，在生成验证语义陷阱的CQs方面比GPT-4.1表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要基于与现有数据集的相似性评估生成的CQs，但无法可靠检测语义陷阱如"Misusing allValuesFrom"，需要专门的方法来验证这些语义缺陷。

Method: 使用LLMs生成类和属性的自然语言定义，通过移除公理或改变逻辑运算符引入定义与本体之间的错位，然后微调LLaMA-3.1-8B-Instruct模型生成验证这些语义差异的CQs。

Result: 微调模型在生成验证陷阱的CQs方面优于基线，比GPT-4.1精度高26%，召回率高28.2%，能检测比现有公共数据集更广泛的建模错误。

Conclusion: 该研究实现了使用LLMs自动生成验证TBox的CQs，显著减少人工工作量，同时改善本体与专家知识之间的语义对齐，是首个使用LLMs针对CQ生成中语义陷阱验证的研究。
...
TL;DR: 提出了Path-Neighbor增强图神经网络(PN-GNN)，通过聚合推理路径上的节点邻居嵌入来增强GNN的逻辑表达能力。该方法在理论和实验上都证明具有比现有方法更强的逻辑表达能力。
...
Method: 提出PN-GNN方法，通过聚合推理路径上的节点邻居嵌入来增强GNN的逻辑表达能力。首先分析现有GNN方法的逻辑表达能力不足，然后理论证明PN-GNN比C-GNN具有严格更强的表达能力，且(k+1)跳逻辑表达能力严格优于k跳。

Result: 在6个合成数据集和2个真实世界数据集上评估PN-GNN的逻辑表达能力。理论和实验都证实PN-GNN在不损害泛化能力的情况下增强了逻辑规则表达能力，在KG推理任务中表现出竞争力。

Conclusion: PN-GNN通过路径邻居聚合机制有效增强了图神经网络的逻辑表达能力，为知识图谱推理提供了更强大的建模工具。
...
<details>
  <summary>Details</summary>
Motivation: 多元时间序列异常检测的复杂性促使研究者寻找有效的转换方法，将多元数据简化为单变量形式以便应用成熟的统计方法。

Method: 使用FCM聚类和模糊积分将多元时间序列转换为单变量时间序列，然后应用隐马尔可夫模型构建异常检测器，并比较不同的转换方法。

Result: 通过一系列实验研究和比较分析，验证了所提出方法的有效性。

Conclusion: 该研究提供了一种有效的多元时间序列异常检测框架，通过数据转换和统计建模的结合实现了良好的检测性能。
...
TL;DR: 提出了一种结合LLM语义推理和GNN结构建模的多视图多标签特征选择方法，通过语义感知异构图和轻量级GAT实现特征重要性评分。


<details>
  <summary>Details</summary>
Motivation: 现有MVMLFS方法主要关注数据统计信息，很少考虑语义信息。本文旨在联合利用统计和语义两种信息来改进特征选择。

Method: 使用LLM评估特征、视图和标签描述的语义相关性；构建包含语义图和统计图的两层语义感知异构图；应用轻量级GAT学习节点嵌入作为特征重要性评分。

Result: 在多个基准数据集上的实验结果表明，该方法优于现有最先进基线方法，且在小规模数据集上仍然有效，展示了其鲁棒性、灵活性和泛化能力。

Conclusion: 结合LLM语义推理和GNN结构建模的方法在多视图多标签特征选择中表现出色，能够有效利用语义信息提升特征选择性能。
...
TL;DR: 提出了一个扰动框架来评估LLMs在复杂环境中的数学推理能力，通过注入语义无关的扰动句子并逐步增加扰动强度，发现LLMs在面对数字扰动时性能显著下降，表明其可能依赖记忆模板而非逻辑推理。
...
Result: LLMs对无数字扰动表现稳定但存在鲁棒性边界；面对数字扰动时性能显著下降（开源模型下降近10%以上，最大下降51.55%，商业模型下降3%-10%）；在核心指令缺失时仍能保持20%-40%准确率。

Conclusion: 当前LLMs在推理能力上存在缺陷和局限性，可能依赖记忆模板而非逻辑推理，这对LLMs的进一步发展具有重要意义。
...
TL;DR: 提出了一种知识增强的长链推理框架，结合LLMs与知识图谱的多跳推理链，用于解决生物分子推理中的逻辑不一致和领域知识缺失问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs在生物分子推理中存在逻辑不一致和缺乏领域知识基础的问题，现有方法往往加剧这些问题，无法捕捉长程机制依赖。

Method: 通过知识图谱的引导多跳遍历和剪枝构建机制链，结合监督微调提高事实基础，并通过强化学习增强推理可靠性和一致性。

Result: 在PrimeKGQA和现有数据集上的实验表明，随着推理深度增加，该方法在需要结构化生物知识遍历的多跳任务上达到最先进性能。
...
TL;DR: KAMI v0.1是一个企业级AI代理基准测试，解决了传统LLM基准的数据污染问题，并评估多步骤工具使用和不确定性决策等代理能力。通过大规模测试发现传统基准排名无法预测实际代理性能，新一代模型在企业任务中不一定优于旧版本。


<details>
  <summary>Details</summary>
Motivation: 企业采用代理AI系统需要可靠的评估方法，但传统LLM基准存在训练数据污染问题，且无法衡量多步骤工具使用和不确定性决策等代理能力。

Method: 开发KAMI v0.1企业基准，处理170,000个LLM测试项，处理55亿个token，涵盖35种模型配置，评估代理性能。

Result: 传统基准排名无法预测实际代理性能，新一代模型在企业相关任务中不一定优于旧版本，发现了成本-性能权衡、模型特定行为模式以及推理能力对token效率的影响。

Conclusion: KAMI v0.1为企业部署决策提供了关键见解，强调需要专门的企业级代理评估基准，而非依赖传统LLM基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM推理方法在复杂性和计算效率之间的平衡问题未解决，且缺乏对System 2推理的深入探索。

Method: 提出心理学支持的Scaffold Reasoning框架，包含Scaffold Stream（构建参考代码）、Analytic Stream（分析错误代码）和Integration Stream（整合结果）。

Result: 在DebugBench上达到88.91%通过率，平均推理时间5.36秒/问题，优于其他推理方法。

Conclusion: 该框架与人类认知过程一致，在不同问题难度和错误类型中展现出优势。
...
TL;DR: 提出了MSCR方法，通过多源候选替换对LLMs进行数学推理任务的对抗攻击，发现轻微扰动即可显著降低模型性能并增加计算开销。
...
Result: 仅替换单个词的轻微扰动就能使所有模型准确率显著下降，GSM8K上最大降幅达49.89%，MATH500上达35.40%，同时保持高语义一致性。

Conclusion: 当前LLMs在数学推理任务中存在鲁棒性缺陷和效率瓶颈，轻微扰动会导致错误输出和计算资源消耗增加。
...
TL;DR: 本文提出信息容量作为大语言模型效率的统一度量指标，基于文本压缩性能与计算复杂度的关系，能够公平比较不同模型系列的效率并预测同一系列模型性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型快速发展导致计算资源需求激增，测试时缩放进一步加剧模型能力与资源消耗的紧张关系，但目前缺乏能够准确反映不同规模和架构LLM效率的统一度量指标。

Method: 基于压缩与智能的关联性，提出信息容量指标，通过文本压缩性能相对于计算复杂度的比值来衡量模型效率。评估了49个主流开源模型在5个异构数据集上的表现。

Result: 实证评估显示，同一系列中不同规模模型具有一致的信息容量。该指标能够公平比较模型系列间的效率，并准确预测同一系列模型的性能。信息容量还考虑了分词器效率的影响。

Conclusion: 信息容量是一个有效的LLM效率度量指标，能够统一评估不同模型，并揭示了分词器效率、预训练数据和专家混合架构对模型效率的影响。
...
TL;DR: 提出一种基于聚类的多变量时间序列异常检测方法，通过滑动窗口生成子序列，使用扩展模糊聚类分析结构，并利用重构准则和粒子群优化来检测振幅和形状异常。
...
Method: 使用滑动窗口生成多变量子序列，应用扩展模糊聚类揭示子序列结构，采用重构准则重构子序列，使用粒子群优化进行异常检测优化。

Result: 在多个合成和六个真实世界数据集上的实验表明，该方法能够有效检测多变量时间序列中的异常。

Conclusion: 所提出的框架能够检测多变量时间序列中的异常，特别适用于识别医疗保健、天气数据分析、金融和疾病爆发检测等领域的异常振幅和形状模式。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在再保险领域的应用日益增多，需要建立审慎框架来评估其可靠性，确保符合现有监管要求并降低信息摩擦。

Method: 提出包含治理、数据溯源、保证、韧性和监管一致性的五支柱架构，通过RAIRAB基准测试评估治理嵌入的LLM是否满足审慎标准。

Result: 在六类任务中，基于检索的配置实现了更高的准确性（0.90），幻觉和解释漂移减少约40%，透明度几乎翻倍。

Conclusion: 现有审慎原则已经能够容纳可靠的AI，前提是治理明确、数据可追溯且保证可验证，这些机制能够降低风险转移和资本配置中的信息摩擦。
...
TL;DR: 本文通过参数化复杂度视角研究Pearl因果层次(PCH)公式的可满足性问题，发现了首个可处理性途径，提出了基于树宽和变量数量的固定参数和XP算法。


<details>
  <summary>Details</summary>
Motivation: Pearl因果层次框架是推理概率、干预和反事实陈述的核心框架，但其公式的可满足性问题在几乎所有经典设置中都是计算难解的，需要寻找可处理性途径。

Method: 采用参数化复杂度方法，使用原始树宽和变量数量等参数，开发了固定参数和XP算法，不同于传统的动态规划方法，而是利用结构良好的因果模型的结构特征。

Result: 为关键概率和反事实片段提供了可满足性的固定参数和XP算法，并给出了匹配的硬度结果，划定了可处理性的边界。

Conclusion: 通过参数化复杂度视角为因果推理提供了新的算法工具包，突破了PCH可满足性问题的计算障碍。
...
<details>
  <summary>Details</summary>
Motivation: 工业质量控制中机器学习模型缺乏可解释性，且许多工业设备传感器技术不完善导致数据获取困难，需要可解释AI来提供模型决策洞察和识别关键特征。

Method: 应用SHAP、Grad-CAM和LIME分析LSTM模型中的特征重要性，将原始19个输入特征减少到9个和6个，评估模型准确性、推理速度和可解释性之间的权衡。

Result: 特征减少可以改善泛化能力同时保持高分类性能，推理速度有小幅提升，为传感器能力有限的工业环境提供了更可行的AI驱动质量控制方案。
...
TL;DR: 研究探讨了在注塑成型过程中使用合成数据训练LSTM模型的可行性，通过模拟生产周期生成合成数据，寻找合成数据与真实数据的最佳比例以提高模型鲁棒性。
...
TL;DR: SpeechCARE是一个多模态语音处理框架，利用预训练的多语言声学和语言转换器模型来检测与认知障碍相关的细微语音线索，通过动态融合架构整合声学、语言学和人口统计学信息，在阿尔茨海默病和相关痴呆症分类中表现出色。


<details>
  <summary>Details</summary>
Motivation: 超过一半的认知衰退患者未被诊断，基于语音的评估有望用于早期检测，但传统语音处理方法性能有限且泛化能力不足。

Method: 采用多语言预训练声学和语言转换器模型，基于专家混合范式设计动态融合架构，结合SHAP可解释性模块和LLM推理，包含自动转录、异常检测和任务识别等预处理步骤。

Result: 在认知健康、轻度认知障碍和阿尔茨海默病分类中达到AUC=0.88和F1=0.72，轻度认知障碍检测达到AUC=0.90和F1=0.62，偏差分析显示除80岁以上人群外差异较小。

Conclusion: SpeechCARE在认知障碍检测方面表现优异，未来将在真实医疗环境中部署，并为纽约市代表性不足人群提供电子健康记录集成的可解释性。
...
TL;DR: SciAgent是一个统一的多智能体系统，用于通用科学推理，能够跨学科和难度级别自适应推理策略，在数学和物理奥林匹克竞赛中达到或超过人类金牌选手水平。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型虽然在特定科学任务上达到专家水平，但系统仍然狭窄且手工制作，需要开发能够跨学科进行通用科学推理的AI系统。

Method: 采用分层多智能体架构：协调器智能体解释问题领域和复杂性，动态编排专门的工人系统，每个系统包含符号演绎、概念建模、数值计算和验证等推理子智能体，协作构建和优化推理管道。

Result: 在数学和物理奥林匹克竞赛（IMO、IMC、IPhO、CPhO）中持续达到或超过人类金牌选手表现，在国际化学奥林匹克竞赛和Humanity's Last Exam基准测试中也表现出良好的跨领域泛化能力。

Conclusion: SciAgent是迈向通用科学智能的具体步骤——能够进行连贯、跨学科专家级推理的AI系统。
...
Conclusion: 这是日本首个完全开发的开源商业图像生成AI，通过保持开发过程透明度，旨在为日本AI研究社区做出贡献并促进国内AI开发生态系统。
...
TL;DR: 提出了一种高效的视觉定位训练流程，通过模型数据过滤和参数高效微调，从480万合成样本中筛选出1.2万个高质量样本，训练3B参数视觉语言模型，在多个基准测试中达到或超越更大基线的性能。
...
Method: 采用模型数据过滤识别挑战性案例，移除不对齐样本，选择多样化多模态实例；在筛选数据上进行监督微调、思维链增强微调和基于组相对策略优化的强化学习。

Result: 在ScreenSpot、Multimodal-Mind2Web和AndroidControl等基准测试中，使用过滤数据和轻量训练策略的模型匹配或超越了更大基线的性能。

Conclusion: 原则性数据筛选和鲁棒适配可以媲美大规模训练，实现紧凑但能力强的多模态推理智能体。
...
<details>
  <summary>Details</summary>
Motivation: 解决用户数据保护问题，防止未经用户同意的数据被用于机器学习模型训练。现有方法缺乏理论保证且在混合干净数据时失效。
...
TL;DR: 提出了EHRStruct基准来评估LLM在结构化电子健康记录数据上的性能，包含11个任务和2200个样本，评估了20个LLM，并提出了EHRMaster方法取得最优性能


<details>
  <summary>Details</summary>
Motivation: 当前缺乏标准化评估框架来系统评估和比较LLM在结构化EHR数据上的性能

Method: 构建EHRStruct基准，定义11个代表性临床任务，使用两个广泛使用的EHR数据集创建2200个评估样本，评估20个先进LLM，分析关键影响因素，并提出代码增强的EHRMaster方法

Result: 许多结构化EHR任务对LLM的理解和推理能力要求很高，EHRMaster方法实现了最先进的性能

Conclusion: EHRStruct为结构化EHR数据评估提供了标准化框架，EHRMaster方法在应对复杂临床任务方面表现出色
...
<details>
  <summary>Details</summary>
Motivation: 传统命中识别方法需要大量实验资源，而现有AI工具复杂度高，限制了湿实验室研究人员的可访问性。多智能体系统结合了LLMs的可解释性和专用模型的精确性。

Method: MADD采用四个协调的智能体来处理从头化合物生成和筛选中的关键子任务，构建定制化的命中识别流程。

Result: 在七个药物发现案例中评估MADD，显示其性能优于现有LLM解决方案。成功将AI优先药物设计应用于五个生物靶点，并发布了识别出的命中分子。
...
<details>
  <summary>Details</summary>
Motivation: 高斯策略在深度强化学习中存在基本不匹配问题：其无界支持需要临时压缩函数来扭曲有界动作空间的几何结构。虽然vMF分布在球面上提供了理论替代方案，但依赖贝塞尔函数和拒绝采样阻碍了实际应用。
...
Conclusion: 稳健高效的连续控制不需要复杂分布，而是需要对动作空间几何结构的原理性尊重。GAC证明了在保持几何优势的同时简化计算的可行性。
...
TL;DR: 本文提出了一个包含11个基于结果、任务无关的AI代理性能评估指标的新框架，通过大规模模拟实验验证了框架有效性，发现混合代理在大多数指标上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 当前仅基于基础设施指标（如延迟、吞吐量）评估AI代理性能的方法不足以捕捉代理决策质量、操作自主性或业务价值，需要更全面的评估框架。

Method: 提出了11个任务无关的性能指标，并通过涉及4种不同代理架构（ReAct、思维链、工具增强、混合）在5个领域（医疗、金融、营销、法律、客户服务）的大规模模拟实验验证框架。

Result: 混合代理在大多数指标上表现最一致，平均目标完成率达到88.8%，并实现最高投资回报率。不同代理设计之间存在显著的性能权衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有任务向量方法要么忽略了插入位置的重要性，要么难以确定每个位置合适的值，限制了多模态模型在少样本学习中的扩展能力。

Method: 基于查询-上下文对的激活差异模式识别敏感位置，构建预聚类激活库，并使用强化学习选择最合适的插入值。

Result: 在多种多模态模型和任务上验证了STV的有效性，相比之前的方法取得了持续改进和强泛化能力。

Conclusion: STV框架通过敏感性感知的任务向量插入，有效解决了多模态模型在少样本学习中的扩展挑战。
...
TL;DR: 提出了Multi-Agent GraphRAG系统，利用Cypher查询语言和标签属性图数据库作为可扩展的推理引擎，为GraphRAG提供自然语言接口。


<details>
  <summary>Details</summary>
Motivation: 现有GraphRAG方法主要基于RDF知识图谱，而Cypher和标签属性图数据库在GraphRAG中的潜力尚未充分探索，需要填补这一研究空白。

Method: 采用模块化LLM代理系统，通过迭代式内容感知校正和规范化，结合聚合反馈循环，自动生成和执行Cypher查询。使用Memgraph作为图数据库后端。

Result: 在CypherBench图数据集上进行了评估，覆盖多个通用领域和多样化查询类型。还在基于IFC数据的属性图上展示了性能，实现了工业数字自动化用例。

Conclusion: 该方法能够将AI与现实世界应用大规模连接，为工业数字自动化提供支持，展示了标签属性图在GraphRAG中的有效性和可扩展性。
...
TL;DR: 提出了DiagramIR，一个用于几何图形自动评估的可扩展管道，基于LaTeX TikZ代码的中间表示，比现有方法更接近人类评分，并能以更低成本实现类似大模型的性能。


<details>
  <summary>Details</summary>
Motivation: LLMs越来越多地用于学习工具，但大多数工具仅限于文本，限制了在需要可视化的重要领域（如数学）的实用性。现有方法存在可扩展评估的瓶颈。

Method: 基于LaTeX TikZ代码的中间表示（IRs）构建自动评估管道，与LLM-as-a-Judge等基线方法进行比较。

Result: 该方法与人类评分者的一致性更高，且使GPT-4.1-Mini等小模型能以10倍更低的推理成本达到与GPT-5等大模型相当的性能。

Conclusion: DiagramIR为部署可访问和可扩展的教育技术提供了重要的评估方法，显著降低了成本同时保持了评估质量。
...
TL;DR: Spark是一个共享智能体记忆架构，旨在模拟人类开发者社区的集体智能和知识，帮助AI编程智能体实现集体持续学习，提升代码生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统的人类中心软件开发知识共享环境正在被颠覆，AI智能体缺乏共享学习资源库，需要新的机制来支持智能体间的知识共享和集体学习。

Method: 引入Spark共享智能体记忆架构，允许AI编程智能体贡献和获取持续演化的经验记忆，作为新知识的存储库实现集体持续学习。

Result: Spark推荐显著提升了通用代码生成模型的代码质量，30亿参数的小模型在Spark帮助下能达到更大先进模型的代码质量水平；推荐质量在软件开发最佳实践标准下达到了98.2%的有用性。

Conclusion: Spark架构有效解决了AI编程智能体的知识共享问题，通过集体记忆机制实现了智能体间的知识传承和质量提升。
...
TL;DR: JobSphere是一个基于RAG架构的AI职业助手，为PGRKAM就业平台提供多语言支持，部署成本比云系统低89%，在响应时间、事实准确性和可用性方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决政府就业网站存在的导航复杂、语言选项少和缺乏个性化支持等参与度和可访问性挑战。

Method: 采用检索增强生成(RAG)架构，支持英语、印地语和旁遮普语，使用4位量化技术实现在消费级GPU上部署，提供语音交互、模拟测试、简历解析和基于嵌入的职位推荐功能。

Result: 实现94%的事实准确率，中位响应时间1.8秒，系统可用性评分78.5/100，比基准平台提升50%，职位推荐精度@10达到68%。

Conclusion: JobSphere有效填补了旁遮普/印地语使用者在农村地区的可访问性差距，同时确保用户获取政府机构提供的可信职位内容。
...
TL;DR: AI驱动的数据可视化平台，自动化整个数据分析流程，从数据上传到生成交互式可视化，显著减少手动输入
...
Method: 使用Python Flask后端和React前端，结合Firebase云存储，采用机器学习算法进行数据清洗、特征分析和可视化选择

Result: 平台能实时处理10万行数据集，支持多用户同时请求，自动生成高质量可视化输出

Conclusion: 云基础数据可视化应用显著减少数据分析过程的手动输入，同时保持高质量的可视化输出和用户体验
...
TL;DR: 提出使用自组织映射(SOM)从语言模型中提取多个拒绝方向，相比单方向方法能更有效地抑制模型的安全拒绝行为。
...
Method: 使用自组织映射(SOM)在有害提示表示上训练识别多个神经元，通过减去无害表示的质心得到多个拒绝方向。

Result: 实验表明从模型内部消融多个方向不仅优于单方向基线，还优于专门的越狱算法，能有效抑制拒绝行为。

Conclusion: 该方法揭示了拒绝行为的机制性含义，证明了多方向方法在抑制模型安全拒绝方面的优越性。
...
TL;DR: 提出了FaithEval评估框架和FaithAct规划框架，通过强制每个推理步骤的证据基础来提升多模态推理中的忠实性，在多个基准测试中感知忠实性提升达26%且不降低任务准确率。
...
Result: 在多个推理基准测试中，FaithAct相比基于提示和工具增强的基线方法，将感知忠实性提升达26%，且不降低任务准确率。
...
TL;DR: 提出了一个基于ISO/PAS 8800指南的结构化框架，用于开发安全的自动驾驶数据集，涵盖数据收集、标注、管理和维护的全生命周期，并整合安全分析来识别和减轻数据集不足带来的风险。


<details>
  <summary>Details</summary>
Motivation: 数据集完整性对AI系统的安全性和可靠性至关重要，特别是在自动驾驶领域。当前缺乏系统化的方法来确保数据集的安全性，需要开发符合安全标准的框架来支持可靠的自动驾驶系统。

Method: 引入AI数据飞轮和数据集生命周期概念，建立结构化框架，包含数据收集、标注、管理和维护流程，整合严格的安全分析来识别危险源，定义数据集安全要求建立流程，并提出验证和验证策略。

Result: 开发了一个系统化的数据集安全框架，能够识别和减轻由数据集不足引起的风险，确保数据集符合安全标准要求，为构建稳健的自动驾驶AI系统提供支持。

Conclusion: 通过整合数据集安全框架、安全分析和验证策略，该研究为开发安全可靠的自动驾驶AI系统提供了系统化方法，推动了该领域数据集安全标准的发展和应用。
...
TL;DR: 提出了一种轻量级、模块化的LLM安全漏洞修复方法——策略补丁，通过在现有模型前添加小型可学习前缀，仅增加0.003%参数即可有效提升模型安全性。
...
Method: 在现有LLM前添加紧凑的可学习前缀作为"补丁"，仅引入少量额外参数，引导模型行为向更安全的参考模型靠拢。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI系统（如LLMs）在数学研究和教育中日益参与，了解其判断与人类判断的对齐程度变得重要，特别是在数学问题的趣味性和难度评估方面。

Method: 通过两项实证研究，分别针对众包平台参与者和国际数学奥林匹克竞赛选手，比较人类和LLMs对数学问题趣味性和难度的评估。

Result: 多数LLMs在广泛层面上与人类的趣味性概念一致，但无法准确反映人类判断的分布；与人类选择的趣味性理由相关性较弱。
...
Method: 将脉冲神经网络与超维计算相结合，构建SNN-HDC模型，利用HDC的高维空间表示和SNN的脉冲特性。

Result: 在DvsGesture和SL-Animals-DVS数据集上，能耗降低1.24-3.67倍和1.38-2.27倍，能够100%识别未训练类别样本。

Conclusion: 该方法在精度、延迟和能耗方面均优于现有解码方法，是速率和延迟解码的有力替代方案。
...
Result: 在标准神经符号基准测试和知识图谱推理任务上的实验表明，DPrL优于现有的最先进神经符号系统，将可扩展性推进到比以前可能更大的复杂设置。

Conclusion: DPrL通过将随机逻辑程序与神经网络和强化学习技术相结合，显著提高了神经符号系统的可扩展性，使其能够处理更大规模和更复杂的推理任务。
...
TL;DR: 本文综述了视频生成模型从单纯视觉生成向虚拟环境构建的演变，将现代视频基础模型概念化为隐式世界模型和视频渲染器的组合，追踪了四代发展历程。
...
Result: 视频生成经历了四代发展，最终形成具有内在物理合理性、实时多模态交互和多时空尺度规划能力的世界模型，应用于机器人、自动驾驶和交互游戏等领域。

Conclusion: 下一代世界模型需要解决开放挑战，包括智能体智能在塑造和评估这些系统中的作用，并建立相应的设计原则。
...
### [187] [Optimizing Classification of Infrequent Labels by Reducing Variability in Label Distribution](https://arxiv.org/abs/2511.07459)
*Ashutosh Agarwal*

Main category: cs.LG

TL;DR: LEVER是一种解决极端分类中低频类别性能不佳问题的新方法，通过Siamese架构和知识转移来减少标签不一致性，显著提升低频类别分类性能。
...
Result: 在多个极端分类数据集上的综合测试显示，该方法在处理低频类别方面取得了显著改进，为领域设立了新的基准。
...
### [188] [Slimmable NAM: Neural Amp Models with adjustable runtime computational cost](https://arxiv.org/abs/2511.07470)
*Steven Atkinson*

Main category: cs.LG

TL;DR: 提出可调节大小的神经放大器模型，无需额外训练即可改变模型大小和计算成本，让音乐家能轻松在模型精度和计算开销之间权衡


<details>
  <summary>Details</summary>
Motivation: 让音乐家能够根据实际需求灵活调整神经放大器模型的复杂度和计算成本，无需为不同规模模型分别训练

Method: 开发可调节大小的神经网络架构，支持运行时动态改变模型规模，无需重新训练

Result: 模型性能与常用基线方法相当，并开发了实时音频插件演示

Conclusion: 该方法成功实现了无需额外训练即可调节模型大小的目标，为实时音频处理提供了灵活高效的解决方案
...
### [189] [Towards Personalized Quantum Federated Learning for Anomaly Detection](https://arxiv.org/abs/2511.07471)
*Ratun Rahman,Sina Shaham,Dinh C. Nguyen*

Main category: cs.LG

TL;DR: 提出个性化量子联邦学习(PQFL)框架用于异常检测，通过参数化量子电路和经典优化器增强本地模型训练，并引入量子中心个性化策略来适应各客户端的硬件特性和数据表示。


<details>
  <summary>Details</summary>
Motivation: 解决量子联邦学习中客户端在硬件能力、电路设计、噪声水平和数据编码方面的异构性问题，特别是在处理不平衡或非独立同分布数据时，单一全局模型训练效果不佳的问题。

Method: 使用参数化量子电路和经典优化器进行本地模型训练，结合量子中心个性化策略，使每个客户端的模型适应其自身硬件特性和数据表示。

Result: PQFL显著提高了异常检测精度，在多样化和现实条件下，与最先进方法相比，将错误率降低达23%，AUROC提升24.2%，AUPR提升20.5%。

Conclusion: PQFL在实用量子联邦设置中展现出有效性和可扩展性，能够有效应对客户端的异构性挑战。
...
### [190] [Multivariate Variational Autoencoder](https://arxiv.org/abs/2511.07472)
*Mehmet Can Yavuz*

Main category: cs.LG

TL;DR: 提出了MVAE（多元变分自编码器），一种保持高斯可处理性但解除对角后验限制的VAE变体，通过全局耦合矩阵和样本特定对角尺度实现全协方差族，在多个数据集上优于传统对角协方差VAE。


<details>
  <summary>Details</summary>
Motivation: 传统VAE使用对角协方差后验限制了建模潜在变量之间相关性的能力，MVAE旨在解除这一限制，同时保持高斯分布的可处理性。

Method: MVAE将每个后验协方差分解为全局耦合矩阵C（诱导数据集范围内的潜在相关性）和样本特定对角尺度（调节局部不确定性），通过L=Cdiag(σ)实现高效重参数化。

Result: 在多个数据集上，MVAE在重建误差、校准指标和无监督结构发现方面均优于传统对角协方差VAE，特别是在中等潜在维度下表现更佳。

Conclusion: MVAE成功解除了VAE的对角协方差限制，提供了更丰富的潜在空间建模能力，同时保持了计算效率，为更准确的概率建模和无监督学习提供了有效工具。
...
### [191] [RELEAP: Reinforcement-Enhanced Label-Efficient Active Phenotyping for Electronic Health Records](https://arxiv.org/abs/2511.07473)
...
Result: RELEAP在所有基准测试中表现最优，逻辑回归AUC从0.774提升至0.805，生存C-index从0.718提升至0.752，在相同标注预算下获得更平滑稳定的性能增益。

Conclusion: RELEAP通过下游反馈优化表型校正，提供了一个可扩展、标签高效的范式，减少手动图表审查，增强基于EHR的风险预测可靠性。
...
### [192] [Comparing Reconstruction Attacks on Pretrained Versus Full Fine-tuned Large Language Model Embeddings on Homo Sapiens Splice Sites Genomic Data](https://arxiv.org/abs/2511.07481)
*Reem Al-Saidi,Erman Ayday,Ziad Kobti*

Main category: cs.LG

TL;DR: 该研究探讨了在基因组序列上应用的大型语言模型中嵌入重建攻击的问题，特别关注微调如何影响对这些攻击的脆弱性。研究发现微调可以增强模型对重建攻击的抵抗能力。


<details>
  <summary>Details</summary>
Motivation: 基于Pan等人的工作，研究旨在确定任务特定的优化（微调）是增强还是削弱了隐私保护，填补了原研究中未指定嵌入类型的空白。

Method: 使用HS3D基因组数据集，将重建攻击管道应用于预训练和微调模型嵌入；实现专门针对DNA序列的标记化机制；进行详细的比较分析，检查位置特定、核苷酸类型和隐私变化。

Result: 微调在多种架构中增强了对抗重建攻击的抵抗力：XLNet (+19.8%)、GPT-2 (+9.8%) 和 BERT (+7.8%)。

Conclusion: 任务特定的优化（微调）可能是一种隐私增强机制，需要为处理敏感基因组数据的语言模型开发先进的保护机制。
...
### [193] [Alignment-Constrained Dynamic Pruning for LLMs: Identifying and Preserving Alignment-Critical Circuits](https://arxiv.org/abs/2511.07482)
...
### [194] [Counterfactual Forecasting of Human Behavior using Generative AI and Causal Graphs](https://arxiv.org/abs/2511.07484)
...
<details>
  <summary>Details</summary>
Motivation: 传统预测方法难以模拟假设情境下的用户行为变化，需要能有效评估产品干预措施影响的预测工具。
...
### [195] [When Are Learning Biases Equivalent? A Unifying Framework for Fairness, Robustness, and Distribution Shift](https://arxiv.org/abs/2511.07485)
*Sushant Mehta*

Main category: cs.LG

TL;DR: 该论文提出了一个统一的理论框架，将机器学习中的不同偏差机制（如不公平性、虚假相关性、子群体性能差）形式化为条件独立性的违反，并证明了这些偏差在特定条件下会产生等效的模型性能影响。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统存在多种失败模式，如对受保护群体的不公平性、对虚假相关性的脆弱性、在少数子群体上的性能差等，这些通常由不同的研究社区孤立研究。作者旨在建立一个统一的理论框架来理解这些偏差机制之间的内在联系。

Method: 通过信息论度量将偏差形式化为条件独立性的违反，推导出虚假相关性、子群体偏移、类别不平衡和公平性违规之间的形式等价条件。理论预测了不同偏差机制之间的定量等价关系。

Result: 理论预测显示，强度为α的虚假相关性在特征重叠假设下产生的等效最差群体精度退化与子群体不平衡比r≈(1+α)/(1-α)相当。在六个数据集和三种架构上的实证验证表明，预测的等价关系在最差群体精度3%的误差范围内成立。

Conclusion: 该工作将公平性、鲁棒性和分布偏移的文献统一在一个共同视角下，实现了跨问题领域的去偏差方法的原理性迁移。
...
### [196] [Provably Efficient Sample Complexity for Robust CMDP](https://arxiv.org/abs/2511.07486)
*Sourav Ganguly,Arnob Ghosh*

Main category: cs.LG

TL;DR: 本文研究了在真实环境与模拟器或名义模型不同时，学习满足安全约束的策略的问题。提出了增强状态空间的鲁棒约束值迭代算法(RCVI)，首次为RCMDP提供了样本复杂度保证。


<details>
  <summary>Details</summary>
Motivation: 现有研究虽然为RCMDP建立了有限时间迭代复杂度保证，但样本复杂度保证仍未被充分探索。需要解决马尔可夫策略在矩形不确定性集下可能不是最优的问题。

Method: 引入增强状态空间，将剩余效用预算纳入状态表示。基于此提出鲁棒约束值迭代算法(RCVI)，使用生成模型进行采样。

Result: RCVI算法实现了样本复杂度O~(|S||A|H^5/ε^2)，在最多ε违反约束的情况下保证性能。这是RCMDP的第一个样本复杂度保证。

Conclusion: 提出的RCVI算法有效解决了RCMDP问题，提供了理论保证和实证验证，填补了该领域样本复杂度研究的空白。
...
### [197] [Methodological Precedence in Health Tech: Why ML/Big Data Analysis Must Follow Basic Epidemiological Consistency. A Case Study](https://arxiv.org/abs/2511.07500)
*Marco Roccetti*

Main category: cs.LG

TL;DR: 该研究通过一个疫苗与精神事件关联的队列研究案例，揭示复杂分析方法（如机器学习和大数据处理）必须建立在基本方法学一致性验证的基础上，否则会放大而非修正设计缺陷，导致误导性结论。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习和海量数据处理在健康研究中的广泛应用，虽然承诺了前所未有的诊断和风险预测准确性，但这些复杂方法的严谨性根本上依赖于基础数据集的质量和统计设计的有效性。本研究旨在强调一个警示原则：复杂分析会放大基本设计选择中的严重方法学缺陷。

Method: 通过应用简单的标准描述性统计方法和已确立的国家流行病学基准，对一个最近发表的关于疫苗结果和精神事件的队列研究进行分析，揭示多个统计上不可调和的悖论。

Result: 研究发现该队列研究存在多个统计悖论，包括高风险群体中慢性疾病风险降低的不可信结果和矛盾的发病率比较，这些最终使报告的风险比（HRs）无效。观察到的效应是队列构建中未校正选择偏倚的数学假象。

Conclusion: 即使是复杂的健康研究也必须首先通过基本流行病学一致性测试，然后从后续高级机器学习或统计建模中得出的结论才能被认为是有效或可发表的。在缺乏随机化的情况下，稳健的方法如倾向评分匹配对于从管理数据中获得有效的因果推断至关重要。
...
### [198] [N-ReLU: Zero-Mean Stochastic Extension of ReLU](https://arxiv.org/abs/2511.07559)
*Md Motaleb Hossen Manik,Md Zabirul Islam,Ge Wang*

Main category: cs.LG

TL;DR: N-ReLU是一种基于ReLU的随机激活函数，通过用高斯噪声替换负激活值来解决ReLU的死神经元问题，同时保持期望输出不变，在MNIST数据集上表现与其他激活函数相当。


<details>
  <summary>Details</summary>
Motivation: 标准ReLU激活函数存在死神经元问题，即负值被硬截断为零导致神经元永久失活，影响网络训练效果。

Method: 提出N-ReLU激活函数，在负值区域用零均值高斯噪声替代硬零截断，保持期望输出与ReLU一致，同时提供梯度流和正则化效果。

Result: 在MNIST数据集上，N-ReLU在中等噪声水平（sigma=0.05-0.10）下达到与ReLU、LeakyReLU、PReLU、GELU和RReLU相当或略优的准确率，且未观察到死神经元现象。
...
### [199] [SCALAR: Benchmarking SAE Interaction Sparsity in Toy LLMs](https://arxiv.org/abs/2511.07572)
...
<details>
  <summary>Details</summary>
Motivation: 传统SAE在单独训练时无法鼓励跨层稀疏连接，导致提取的电路中上游特征不必要地影响多个下游特征，现有评估只关注单个SAE性能而忽略交互稀疏性。

Method: 引入SCALAR基准测量SAE特征间的交互稀疏性；提出Staircase SAEs使用权重共享限制上游特征在下游特征中的重复；比较TopK SAEs、Jacobian SAEs和Staircase SAEs。

Result: Staircase SAEs相比TopK SAEs在交互稀疏性上提升59.67%±1.83%(前馈层)和63.15%±1.35%(transformer块)；JSAEs在前馈层提升8.54%±0.38%但无法在transformer块有效训练；在216K参数玩具模型和GPT-2 Small上验证了有效性。

Conclusion: 通过基准测试和架构比较，强调了SAE中交互稀疏性的重要性，Staircase SAEs在保持特征可解释性的同时显著提高了交互稀疏性。
...
### [200] [LLM Output Drift: Cross-Provider Validation & Mitigation for Financial Workflows](https://arxiv.org/abs/2511.07585)
...
### [201] [One Router to Route Them All: Homogeneous Expert Routing for Heterogeneous Graph Transformers](https://arxiv.org/abs/2511.07603)
*Georgiy Shakirov,Albert Arakelov*

Main category: cs.LG

TL;DR: 该论文提出了一种名为HER的混合专家层，用于异构图变换器，通过随机掩码类型嵌入来鼓励类型无关的专家专业化，在链接预测任务上表现优于标准HGT和类型分离的MoE基线。


<details>
  <summary>Details</summary>
Motivation: 当前异构图神经网络通常基于节点/边类型设置参数，这可能导致对表面标签的过度依赖并阻碍跨类型知识迁移。作者探索将混合专家集成到HGNN中，质疑类型特定专家的必要性。

Method: 提出Homogeneous Expert Routing (HER)，在HGT中引入MoE层，在路由过程中随机掩码类型嵌入，促进专家基于语义模式而非节点类型进行专业化。

Result: 在IMDB、ACM和DBLP数据集上的链接预测评估显示，HER始终优于标准HGT和类型分离的MoE基线。IMDB上的分析表明HER专家按语义模式（如电影类型）而非节点类型进行专业化。

Conclusion: 研究表明，在专家路由中正则化类型依赖可以产生更具泛化性、高效性和可解释性的表示，这为异构图学习提供了新的设计原则。
...
### [202] [Partial Action Replacement: Tackling Distribution Shift in Offline MARL](https://arxiv.org/abs/2511.07629)
*Yue Jin,Giovanni Montana*

Main category: cs.LG

TL;DR: 提出了Soft-Partial Conservative Q-Learning (SPaCQL)方法，通过部分动作替换策略解决离线多智能体强化学习中的分布外问题，在因子化行为策略下显著降低分布偏移。
...
Method: 使用部分动作替换策略，只更新部分智能体的动作而保持其他智能体动作不变，结合基于价值估计不确定性的动态权重分配。

Result: 理论证明在因子化行为策略下，分布偏移随偏离智能体数量线性增长而非指数增长，实验显示SPaCQL在具有独立性结构的离线数据集上显著优于基线算法。

Conclusion: SPaCQL通过部分动作替换策略有效缓解了离线MARL中的分布外问题，为因子化行为策略场景提供了理论保证和实际解决方案。
...
### [203] [FlowTIE: Flow-based Transport of Intensity Equation for Phase Gradient Estimation from 4D-STEM Data](https://arxiv.org/abs/2511.07633)
*Arya Bangun,Maximilian Töllner,Xuan Zhao,Christian Kübel,Hanno Scharr*

Main category: cs.LG

TL;DR: FlowTIE是一个基于神经网络的框架，用于从4D-STEM数据中重建相位，通过将强度传输方程与基于流的相位梯度表示相结合，提高了在厚样品动态散射条件下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决在厚样品动态散射条件下传统相位重建方法精度不足的问题，将数据驱动学习与物理先验相结合以提高重建质量。

Method: 将强度传输方程与基于流的相位梯度表示相集成，使用神经网络框架进行相位重建，并可结合多层切片方法处理厚样品。

Result: 在模拟晶体材料数据集上的验证表明，FlowTIE相比传统TIE和基于梯度的优化方法，提高了相位重建精度和速度。

Conclusion: FlowTIE能够有效提高相位重建的准确性，计算速度快，并能与厚样品模型集成，适用于动态散射条件下的相位重建任务。
...
### [204] [Private-RAG: Answering Multiple Queries with LLMs while Keeping Your Data Private](https://arxiv.org/abs/2511.07637)
*Ruihan Wu,Erchi Wang,Zhiyuan Zhang,Yu-Xiang Wang*

Main category: cs.LG

TL;DR: 本文提出了两种差分隐私保护的检索增强生成算法MURAG和MURAG-ADA，解决了多查询场景下的隐私泄露问题，能够在实用隐私预算内扩展到数百次查询。
...
Method: MURAG利用个体隐私过滤器，使累积隐私损失仅取决于每个文档被检索的频率而非查询总数；MURAG-ADA通过私有发布查询特定阈值，进一步提高了效用，实现更精确的相关文档选择。
...
### [205] [Adaptive Graph Learning with Transformer for Multi-Reservoir Inflow Prediction](https://arxiv.org/abs/2511.07649)
...
Method: 构建动态图结构，将水库作为节点，有向边反映水文连接关系，采用注意力机制自动识别关键的时空依赖关系，并通过参数共享提升数据有限水库的性能。

Result: 在科罗拉多河上游流域30个水库上的评估显示，AdaTrip优于现有基线方法，特别对记录有限的水库有更好表现，并能提供可解释的注意力图谱。

Conclusion: AdaTrip框架有效解决了多水库入流预测中的时空依赖问题，为水资源管理决策提供了有价值的见解和工具支持。
...
### [206] [Enhancing Binary Encoded Crime Linkage Analysis Using Siamese Network](https://arxiv.org/abs/2511.07651)
*Yicheng Zhan,Fahim Ahmed,Amy Burrell,Matthew J. Tonkin,Sarah Galambos,Jessica Woodhams,Dalal Alrajeh*

Main category: cs.LG

TL;DR: 提出了一种用于犯罪关联分析的孪生自编码器框架，通过地理-时间特征集成和领域知识驱动的数据降维策略，显著提高了犯罪关联准确性。


<details>
  <summary>Details</summary>
Motivation: 传统犯罪关联方法在处理高维、稀疏和异构数据时存在局限，需要更有效的机器学习方法来识别连环犯罪者。

Method: 使用孪生自编码器框架学习有意义的潜在表示，在解码器阶段集成地理-时间特征以增强行为表示，避免信号稀释。

Result: 在ViCLAS数据集上，AUC比传统方法提高了9%，在多个评估指标上表现一致改善。

Conclusion: 先进的机器学习方法可以显著提高犯罪关联准确性，同时为调查决策提供可解释的见解。
...
### [207] [CAE: Character-Level Autoencoder for Non-Semantic Relational Data Grouping](https://arxiv.org/abs/2511.07657)
*Veera V S Bhargav Nunna,Shinae Kang,Zheyuan Zhou,Virginia Wang,Sucharitha Boinapally,Michael Foley*

Main category: cs.LG

TL;DR: 提出字符级自编码器(CAE)方法，用于自动识别和分组非语义关系数据库中的相同列，通过字符级处理解决传统NLP方法在处理IP地址、产品ID等非语义数据时的局限性。


<details>
  <summary>Details</summary>
Motivation: 企业关系数据库包含大量非语义数据（IP地址、产品标识符、编码键等），传统语义分析方法难以处理这些数据，需要开发专门的方法来识别和分组语义相同的列。

Method: 使用字符级自编码器(CAE)架构，在字符级别编码非语义关系表列的文本表示，提取高维特征嵌入进行数据分组，通过固定字典大小显著减少内存需求和训练时间。
...
### [208] [ZeroSim: Zero-Shot Analog Circuit Evaluation with Unified Transformer Embeddings](https://arxiv.org/abs/2511.07658)
*Xiaomeng Yang,Jian Gao,Yanzhi Wang,Xuan Zhang*

Main category: cs.LG

TL;DR: ZeroSim是一个基于Transformer的性能建模框架，用于模拟电路设计自动化，能够在未见过的拓扑结构上实现零样本泛化，无需微调即可准确预测性能。
...
Result: ZeroSim显著优于多层感知机、图神经网络和Transformer基线模型，在强化学习参数优化管道中相比传统SPICE仿真实现了13倍加速。

Conclusion: ZeroSim为模拟电路设计自动化任务提供了实用的性能评估解决方案，具有强大的泛化能力和显著的计算效率提升。
...
### [209] [Probabilities Are All You Need: A Probability-Only Approach to Uncertainty Estimation in Large Language Models](https://arxiv.org/abs/2511.07694)
*Manh Nguyen,Sunil Gupta,Hung Le*

Main category: cs.LG

TL;DR: 提出一种高效、无需训练的不确定性估计方法，通过使用回答的前K个概率来近似预测熵，并采用自适应机制确定K值以增强灵活性。
...
### [210] [On the Role of Calibration in Benchmarking Algorithmic Fairness for Skin Cancer Detection](https://arxiv.org/abs/2511.07700)
*Brandon Dominique,Prudence Lam,Nicholas Kurtansky,Jochen Weber,Kivanc Kose,Veronica Rotemberg,Jennifer Dy*

Main category: cs.LG

TL;DR: 该论文发现AI皮肤癌检测模型在跨人口统计子组（性别、种族、年龄）存在校准问题，虽然提升了判别准确性，但在新数据集上常过度诊断风险，需要更全面的模型审计策略。


<details>
  <summary>Details</summary>
Motivation: AI模型在黑色素瘤检测中表现出专家级性能，但临床采用受到跨人口统计子组性能差异的阻碍。现有基准测试主要依赖AUROC的群体公平性指标，无法提供模型准确估计能力的信息。
...
### [211] [Diffusion Guided Adversarial State Perturbations in Reinforcement Learning](https://arxiv.org/abs/2511.07701)
*Xiaolin Sun,Feidi Liu,Zhengming Ding,ZiZhan Zheng*

Main category: cs.LG

TL;DR: SHIFT是一种基于扩散模型的策略无关状态扰动攻击，能够生成语义不同但逼真的对抗样本，有效突破现有RL防御机制。


<details>
  <summary>Details</summary>
Motivation: 现有RL系统防御机制的有效性主要源于传统l_p范数约束攻击的局限性，这些攻击即使在大扰动预算下也难以改变图像输入的语义。

Method: 提出SHIFT攻击方法，利用扩散模型生成语义不同但保持真实性和历史一致性的扰动状态，避免被检测。

Result: 评估显示SHIFT攻击能有效突破现有最先进的防御机制，显著优于现有攻击方法，同时具有更好的感知隐蔽性。

Conclusion: RL智能体对语义感知对抗扰动存在严重脆弱性，表明需要开发更鲁棒的政策。
...
### [212] [Intelligent Optimization of Multi-Parameter Micromixers Using a Scientific Machine Learning Framework](https://arxiv.org/abs/2511.07702)
*Meraj Hassanzadeh,Ehsan Ghaderi,Mohamad Ali Bijarchi,Siamak Kazemzadeh Hannani*

Main category: cs.LG

TL;DR: 提出基于科学机器学习的新型多维优化框架，使用深度强化学习与物理信息神经网络相结合的方法，在微混合器案例中实现比传统方法快得多的优化速度，效率提升最高达32%。


<details>
  <summary>Details</summary>
Motivation: 传统基于仿真的优化方法存在严重局限：一次只能优化单个问题，且需要大量计算时间进行网格划分和数值模拟。需要克服这些固有缺陷。

Method: 使用深度强化学习（DRL）架构的智能体作为优化器，与参数化物理信息神经网络（PINN）构成的环境交互，探索关键问题参数间的关系。PINN响应速度远快于传统数值方法。

Result: 在广泛的施密特数范围内训练智能体后，分析得到的最优设计。在整个频谱范围内，实现的效率始终高于基线值。最大效率出现在施密特数为13.3时，效率提升约32%。

Conclusion: 该方法在同等条件下与遗传算法进行对比分析，突显了所提方法的优势，为多维优化问题提供了高效的解决方案。
...
### [213] [A Ranking-Based Optimization Algorithm for the Vehicle Relocation Problem in Car Sharing Services](https://arxiv.org/abs/2511.07724)
*Piotr Szwed,Paweł Skrzynski,Jarosław Wąs*

Main category: cs.LG

TL;DR: 提出了一种基于区域划分和快速排序算法的车辆重定位方法，用于解决自由流动汽车共享服务中的车辆重新分配问题，通过使用滑板车进行人员转移，在真实数据上实现了3%-10%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决自由流动汽车共享服务中的车辆重定位问题，提高服务效率，通过优化车辆和人员的重新分配来满足不同区域的需求变化。

Method: 首先将服务区域划分为具有相似时间模式车辆存在和服务需求的区域，然后提出基于快速排序的算法，考虑每个区域的可用车辆数量、预测需求概率密度和估计行程时长。

Result: 在真实数据测试中，相比无优化的基线，算法平均改进8.44%，MIP求解器改进19.6%。根据工作人员规模，应用该解决方案可将性能指标提高约3%-10%。

Conclusion: 提出的基于区域划分和快速排序的车辆重定位算法能有效提升汽车共享服务的运营效率，虽然性能略低于MIP模型，但更符合实际业务规则要求。
...
### [214] [Multistep Quasimetric Learning for Scalable Goal-conditioned Reinforcement Learning](https://arxiv.org/abs/2511.07730)
*Bill Chunyuan Zheng,Vivek Myers,Benjamin Eysenbach,Sergey Levine*

Main category: cs.LG

TL;DR: 提出了一种结合局部更新和全局更新的目标条件强化学习方法，使用多步蒙特卡洛回报拟合拟度量距离，在长时域任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决AI中长期时域推理的挑战，特别是如何准确估计观测对之间的时间距离。传统TD方法虽然提供最优性保证但性能较差，而蒙特卡洛方法性能更好但缺乏保证。

Method: 将局部更新和全局更新方法结合，开发实用的GCRL方法，使用多步蒙特卡洛回报拟合拟度量距离。

Result: 在长达4000步的长时域模拟任务中超越现有GCRL方法，即使使用视觉观测。在真实世界机器人操作领域（Bridge设置）中实现了多步缝合。

Conclusion: 这是首个能够在真实世界操作领域从未标记的离线视觉观测数据集中实现端到端多步缝合的GCRL方法。
...
### [215] [Global Optimization on Graph-Structured Data via Gaussian Processes with Spectral Representations](https://arxiv.org/abs/2511.07734)
...
<details>
  <summary>Details</summary>
Motivation: 贝叶斯优化在优化昂贵黑盒目标方面很强大，但扩展到图结构领域仍然具有挑战性，因为图具有离散和组合性质。现有方法要么依赖完整的图拓扑（对于大型或部分观测图不实用），要么采用增量探索（收敛缓慢）。
...
### [216] [From Exploration to Exploitation: A Two-Stage Entropy RLVR Approach for Noise-Tolerant MLLM Training](https://arxiv.org/abs/2511.07738)
...
TL;DR: 提出了一种新颖的两阶段、token级熵优化方法用于MLLMs的RLVR，通过动态引导模型从探索到利用来增强噪声容忍度。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法严重依赖高质量标注数据，但现实场景中标注数据稀缺且存在大量标注噪声，现有无监督方法容易过拟合到错误标签并限制GRPO中的奖励排序信号。

Method: 两阶段token级熵优化：探索阶段通过token级熵最大化促进多样随机输出生成，防止过早收敛到噪声标签；利用阶段通过token级熵最小化鼓励模型产生自信确定性输出，巩固学习知识。

Result: 在三种MLLM骨干网络(Qwen2-VL-2B、Qwen2-VL-7B和Qwen2.5-VL-3B)上，跨越多种噪声设置和任务，该方法始终优于先前方法，实现了稳健且优越的性能。

Conclusion: 该分阶段策略通过统一和增强外部、内部和基于熵的方法，在各种场景下都能提供鲁棒且优越的性能表现。
...
### [217] [Schedulers for Schedule-free: Theoretically inspired hyperparameters](https://arxiv.org/abs/2511.07767)
...
<details>
  <summary>Details</summary>
Motivation: 现有的无调度方法理论仅支持恒定学习率，而实际实现中使用了预热调度。需要扩展理论以支持任意调度器，并改进理论预测能力。

Method: 扩展无调度方法的最后迭代收敛理论以支持任意调度器，分析平均参数如何随学习率更新，并基于凸性假设设计新的自适应Polyak学习率调度。

Result: 理论证明了在wsd调度下达到最优收敛率O(1/√T)，新Polyak调度在理论上有最优的最后迭代收敛性，在模型蒸馏任务中表现优于多个基线方法。

Conclusion: 扩展后的无调度理论具有更好的预测能力，新提出的自适应Polyak调度在理论和实践中都表现出色，为无调度方法提供了更强大的理论基础和实用工具。
...
### [218] [Physical Consistency of Aurora's Encoder: A Quantitative Study](https://arxiv.org/abs/2511.07787)
*Benjamin Richards,Pushpa Kumar Balan*

Main category: cs.LG

TL;DR: 本文通过线性分类器分析Aurora天气模型的潜在表示，发现其学习了物理一致的特征，但在捕捉罕见事件方面存在局限，强调了可解释性方法对AI天气模型的重要性。


<details>
  <summary>Details</summary>
Motivation: 大型天气预报模型如Aurora虽然精度高但缺乏透明度，这种'黑盒'特性阻碍了其在高风险操作环境中的应用。
...
### [219] [Analyzing Political Text at Scale with Online Tensor LDA](https://arxiv.org/abs/2511.07809)
*Sara Kangaslahti,Danny Ebanks,Jean Kossaifi,Anqi Liu,R. Michael Alvarez,Animashree Anandkumar*

Main category: cs.LG

TL;DR: 提出了一种可扩展到数十亿文档的主题建模方法TLDA，具有可识别性和恢复性保证，计算效率比现有方法快3-4倍，并提供了GPU实现。


<details>
  <summary>Details</summary>
Motivation: 现有主题建模方法难以扩展到超大规模文本数据集，限制了社会科学研究者对海量语料库的分析能力。

Method: 提出了Tensor Latent Dirichlet Allocation (TLDA)方法，具有可识别参数保证和样本复杂度保证，采用GPU并行实现。

Result: 该方法在超过十亿文档的数据集上线性扩展，速度比现有并行LDA方法快3-4倍，成功应用于#MeToo运动演变和2020大选舞弊讨论的大规模分析。
...
### [220] [Multi-Objective Bilevel Learning](https://arxiv.org/abs/2511.07824)
*Zhiyao Zhang,Zhuqing Liu,Xin Zhang,Wen-Yen Chen,Jiyan Yang,Jia Liu*

Main category: cs.LG

TL;DR: 提出了一种名为WC-MHGD的统一算法框架，用于解决多目标双层学习问题，在确定性和随机设置下都能保证有限时间的帕累托平稳收敛率，实现低oracle复杂度和系统性的帕累托前沿探索。
...
Method: 提出加权切比雪夫多超梯度下降(WC-MHGD)算法框架，处理上层子问题由偏好引导、部分输入依赖于下层子问题最优解的多目标双层学习问题。

Result: 理论分析表明WC-MHGD在确定性和随机设置下都能保证有限时间的帕累托平稳收敛率，实验验证了理论结果。

Conclusion: WC-MHGD框架能够有效识别偏好引导的帕累托平稳解，并实现系统性的帕累托前沿探索，填补了多目标双层学习领域的理论和方法空白。
...
### [221] [MURPHY: Multi-Turn GRPO for Self Correcting Code Generation](https://arxiv.org/abs/2511.07833)
*Chanakya Ekbote,Vijay Lingam,Behrooz Omidvar-Tehrani,Jun Huan,Sujay Sanghavi,Anoop Deoras,Stefano Soatto*

Main category: cs.LG

TL;DR: Murphy是一个多轮反思优化框架，通过结合迭代自我校正来增强GRPO方法，在代码生成基准测试中相比GRPO实现了最高8%的相对性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有的RLVR方法如GRPO在推理基准测试中有效，但在需要迭代决策的智能体任务中存在困难，需要改进以支持多轮自我校正。

Method: 通过结合定量和定性执行反馈，Murphy在训练过程中引入迭代自我校正，使模型能够在多轮中逐步优化其推理过程。

Result: 在Qwen和OLMo等模型家族的代码生成基准测试中，Murphy相比GRPO在相似的算力预算下实现了最高8%的pass@1相对提升。

Conclusion: Murphy框架通过多轮反思优化有效提升了语言模型在需要迭代决策任务中的推理能力，为RLVR方法在智能体任务中的应用提供了改进方案。
...
### [222] [DP-AdamW: Investigating Decoupled Weight Decay and Bias Correction in Private Deep Learning](https://arxiv.org/abs/2511.07843)
*Jay Chooi,Kevin Cong,Russell Li,Lillian Sun*

Main category: cs.LG

TL;DR: 本文提出了DP-AdamW-BC，一种带有差分隐私偏置校正的AdamW优化器变体，在多个任务上优于现有DP优化器，但发现偏置校正反而会降低性能。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习广泛使用敏感数据，差分隐私(DP)提供了防止训练过程中信息泄露的正式保证。现有DP优化器在保持隐私的同时难以维持强性能，而AdamW因其优异性能成为流行选择。

Method: 提出了DP-AdamW和DP-AdamW-BC（带有DP偏置校正的第二矩估计器），提供了隐私和收敛保证的理论分析，并在多个隐私预算(ε=1,3,7)下进行实证分析。

Result: DP-AdamW在文本分类上比现有DP优化器高出15%以上，图像分类高出5%，图节点分类稳定高出1%。但DP-AdamW-BC的偏置校正反而持续降低准确率。

Conclusion: DP-AdamW是有效的DP优化器，优于现有方法，但偏置校正对DP-AdamW没有积极效果，这与DP-AdamBC对DP-Adam的改进形成对比。
...
### [223] [A General Method for Proving Networks Universal Approximation Property](https://arxiv.org/abs/2511.07857)
*Wei Wang*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要为每种新架构从头开始证明通用逼近性，缺乏统一的分析基础，导致冗余和理论理解割裂。

Method: 定义通用逼近模块(UAM)，证明由UAM组成的网络保持通用逼近性，并将逼近过程解释为模块间的渐进精炼。

Result: 建立了一个统一的分析框架，能够同时处理多种网络架构的通用逼近性证明。

Conclusion: 该框架不仅统一了不同架构的分析，还提供了对网络表达能力渐进演化的逐步理解。
...
### [224] [Algorithm-Relative Trajectory Valuation in Policy Gradient Control](https://arxiv.org/abs/2511.07878)
*Shihao Li,Jiachen Li,Jiamin Xu,Christopher Martin,Wei Li,Dongmei Chen*

Main category: cs.LG

TL;DR: 研究轨迹价值如何依赖于策略梯度控制中的学习算法。在不确定LQR中使用轨迹Shapley分析，发现持续激励(PE)与REINFORCE算法的边际价值呈负相关，而稳定化后转为正相关。
...
Method: 使用轨迹Shapley分析不确定LQR系统，比较REINFORCE算法及其稳定化版本（状态白化或Fisher预条件）的性能。

Result: 发现PE与边际价值在REINFORCE中负相关(r≈-0.38)，稳定化后转为正相关(r≈+0.29)。方差机制是主要原因：高PE降低梯度方差，但在鞍点附近高方差增加逃逸概率。

Conclusion: 轨迹价值是算法相关的。留一法分数适合剪枝，而Shapley能识别有害子集。
...
### [225] [Meta-cognitive Multi-scale Hierarchical Reasoning for Motor Imagery Decoding](https://arxiv.org/abs/2511.07884)
*Si-Hyun Kim,Heon-Gyu Kwak,Byoung-Hee Kwon,Seong-Whan Lee*

Main category: cs.LG

TL;DR: 提出了一个分层和元认知解码框架，用于四类运动想象分类，通过多尺度分层信号处理和内省不确定性估计来提高脑机接口系统的可靠性。


<details>
  <summary>Details</summary>
Motivation: 脑机接口在实际部署中受到运动想象脑电信号噪声和变异性的限制，需要提高解码的鲁棒性和可靠性。

Method: 采用多尺度分层信号处理模块重新组织骨干网络特征为时间多尺度表示，结合内省不确定性估计模块分配每周期可靠性分数并指导迭代优化。

Result: 在三个标准EEG骨干网络上测试，相比基线方法平均分类准确率提高，受试者间方差降低，表明对受试者异质性和噪声试验的鲁棒性增强。

Conclusion: 结合分层多尺度处理与内省置信度估计可以提升基于运动想象的脑机接口系统的可靠性。
...
### [226] [A Generalized Spectral Framework to Expain Neural Scaling and Compression Dynamics](https://arxiv.org/abs/2511.07892)
*Yizhou Zhang*

Main category: cs.LG

TL;DR: 本文提出了一个广义谱框架，统一了学习动态和压缩现象，通过将谱演化函数推广到渐近多项式形式，并引入有效谱-时间弹性参数。


<details>
  <summary>Details</summary>
Motivation: 现有的经验缩放定律在特定区域内一致，但在模型压缩等相关设置中报告了明显不同的缩放行为。受神经表示谱分析最新进展的启发，需要开发一个统一框架。
...
### [227] [Statistically Assuring Safety of Control Systems using Ensembles of Safety Filters and Conformal Prediction](https://arxiv.org/abs/2511.07899)
*Ihab Tabbara,Yuxuan Yang,Hussein Sibai*

Main category: cs.LG

TL;DR: 本文提出了一个基于共形预测的框架，为学习型Hamilton-Jacobi可达性分析提供概率安全保证，通过校准不安全名义控制器与学习安全策略之间的切换来防止控制系统到达故障状态。


<details>
  <summary>Details</summary>
Motivation: Hamilton-Jacobi可达性分析计算成本高，特别是对于高维系统，因此使用强化学习方法来近似值函数。但学习到的值函数和相应安全策略不能保证正确性，需要解决这种不确定性。

Method: 引入基于共形预测的框架来约束不确定性，使用CP校准不安全名义控制器与学习HJ安全策略之间的切换，并研究使用独立训练HJ值函数集合作为安全过滤器。

Result: 该框架能够为使用学习HJ值函数和策略的控制系统提供概率安全保证，防止系统到达故障状态。

Conclusion: 基于共形预测的方法能够有效解决学习型HJ可达性分析中的不确定性，为自主系统的安全部署提供可靠保障。
...
### [228] [Test-driven Reinforcement Learning](https://arxiv.org/abs/2511.07904)
*Zhao Yu,Xiuping Wu,Liangjun Ke*

Main category: cs.LG

TL;DR: 提出基于满意理论的测试驱动强化学习框架，用多个测试函数替代单一奖励函数来定义任务目标，简化奖励设计并支持多目标优化


<details>
  <summary>Details</summary>
Motivation: 传统强化学习中的奖励函数既要定义最优目标又要指导学习过程，这种双重目的使得手动设计奖励函数很困难，往往导致任务表示不理想

Method: 使用通过-失败测试和指示性测试两类测试函数分别定义最优目标和指导学习过程，提出基于词典序的启发式方法比较轨迹与最优轨迹集的相对距离关系，开发了TdRL算法实现

Result: 在DeepMind Control Suite基准测试中，TdRL在策略训练方面匹配或优于手工设计奖励方法，具有更大的设计简单性和对多目标优化的内在支持
...
### [229] [CellARC: Measuring Intelligence with Cellular Automata](https://arxiv.org/abs/2511.07908)
*Miroslav Lžičař*

Main category: cs.LG

TL;DR: CellARC是一个基于多色一维元胞自动机构建的抽象推理合成基准，包含95k训练集和2k测试集，支持快速迭代并评估多种模型性能。


<details>
  <summary>Details</summary>
Motivation: 创建可控的任务空间，解耦泛化能力与人类先验知识，支持可重复研究模型在有限预算下快速推断新规则的能力。

Method: 使用多色一维元胞自动机构建基准，每个episode包含5个支持对和1个查询，序列化为256个token，通过控制字母表大小、半径、规则族等参数创建任务。

Result: 10M参数Transformer在插值/外推测试集上达到58.0%/32.4%准确率，GPT-5 High在100个测试任务子集上达到62.3%/48.1%，Transformer与符号基线的集成模型达到65.4%/35.5%。

Conclusion: CellARC提供了可控的抽象推理基准，展示了神经符号互补性，并为研究模型规则推断能力提供了可扩展框架。
...
### [230] [Rectified Noise: A Generative Model Using Positive-incentive Noise](https://arxiv.org/abs/2511.07911)
*Zhenyu Gu,Yanchen Xu,Sida Huang,Yubin Guo,Hongyuan Zhang*

Main category: cs.LG

TL;DR: 提出Rectified Noise (ΔRN)方法，通过向预训练Rectified Flow模型的速度场注入π-noise来提升生成性能，仅需0.39%额外参数即可显著改善生成质量。


<details>
  <summary>Details</summary>
Motivation: 虽然Rectified Flow主要基于概率流ODE，但研究发现通过反向时间SDE注入噪声可以取得更好的生成性能。受Positive-incentive Noise (π-noise)启发，希望开发一种训练π-noise生成器的新方法。

Method: 提出Rectified Noise (ΔRN)算法，在预训练RF模型的速度场中注入π-noise，将RF模型高效转化为π-noise生成器。

Result: 在ImageNet-1k上，使用Rectified Noise的RF模型将FID从10.16降低到9.05；π-noise生成器仅需0.39%额外训练参数即可获得性能提升。

Conclusion: Rectified Noise是一种有效的生成模型增强方法，能够以极小的参数代价显著提升预训练RF模型的生成性能。
...
### [231] [Feedback Descent: Open-Ended Text Optimization via Pairwise Comparison](https://arxiv.org/abs/2511.07919)
*Yoonho Lee,Joseph Boen,Chelsea Finn*

Main category: cs.LG

TL;DR: Feedback Descent是一个通过结构化文本反馈而非标量奖励来优化文本工件（提示、代码、分子）的框架，在推理时进行纯迭代优化，无需修改模型权重。


<details>
  <summary>Details</summary>
Motivation: 传统方法将详细评判压缩为二进制偏好，造成信息瓶颈。Feedback Descent通过保留结构化文本反馈来拓宽偏好学习中的信息瓶颈，实现文本空间的定向优化。

Method: 使用上下文学习将结构化反馈转化为类似梯度的方向信息，实现定向编辑。评估器为每个比较配对文本反馈，作为高带宽监督。纯推理时迭代，任务无关。

Result: 在三个不同领域评估中，Feedback Descent优于最先进的提示优化(GEPA)、强化学习方法(GRPO、REINVENT)和专门的基于图的分子优化器。在DOCKSTRING分子发现基准测试中，识别出超过260,000个化合物数据库中99.9%百分位的新药样分子。
...
### [232] [SERL: Self-Examining Reinforcement Learning on Open-Domain](https://arxiv.org/abs/2511.07922)
...
Method: SERL框架，包含两种协同奖励机制：基于Copeland风格成对比较的Actor奖励，和鼓励一致判断的Judge自一致性奖励。

Result: 将Qwen3-8B在AlpacaEval 2上的LC胜率从52.37%提升到59.90%，达到自改进方法的SOTA性能，性能可比Qwen3-32B等更大模型。

Conclusion: SERL在开放域任务上展现出卓越的有效性和鲁棒性，为LLM自我改进提供了新思路。
...
### [233] [IBMA: An Imputation-Based Mixup Augmentation Using Self-Supervised Learning for Time Series Data](https://arxiv.org/abs/2511.07930)
...
<details>
  <summary>Details</summary>
Motivation: 时间序列预测中的数据增强策略相对较少，特别是像Mixup这样的高级技术很少被使用，需要开发更有效的增强方法来提升模型泛化能力。

Method: IBMA方法结合了插补增强数据和Mixup增强，通过在多个预测模型（DLinear、TimesNet、iTrainformer）上进行测试验证其有效性。

Result: 在4个数据集和8种其他增强技术的对比实验中，IBMA在24个实例中22个表现更好，其中10个达到最佳性能，特别是在使用iTrainformer插补时效果最显著。
...
### [234] [Predict-then-Optimize Method for Seaport Power-Logistics Scheduling: Generalization across Varying Tasks Stream](https://arxiv.org/abs/2511.07938)
*Chuanqing Pu,Feilong Fan,Nengling Tai,Yan Xu,Wentao Huang,Honglin Wen*

Main category: cs.LG

TL;DR: 提出了一种决策导向的持续学习框架，用于适应不断变化的海港船舶到达调度任务，通过Fisher信息正则化增强跨任务泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统的预测-优化流程假设下游优化任务配置固定，无法很好地适应由变化的海港船舶到达引起的任务结构演变。

Method: 引入基于Fisher信息的正则化来保留对先前任务关键的参数，同时开发可微分凸代理以稳定梯度反向传播。
...
### [235] [Balance Equation-based Distributionally Robust Offline Imitation Learning](https://arxiv.org/abs/2511.07942)
*Rishabh Agrawal,Yusuf Alvi,Rahul Jain,Ashutosh Nayyar*

Main category: cs.LG

TL;DR: 提出了一种基于平衡方程的分布鲁棒离线模仿学习框架，仅使用名义动态下的专家演示数据学习鲁棒策略，无需额外环境交互。
...
Result: 在连续控制基准测试上的实证评估表明，该方法在扰动或偏移环境下相比最先进的离线模仿学习基线实现了更优的鲁棒性和泛化能力。
...
### [236] [Continual Unlearning for Text-to-Image Diffusion Models: A Regularization Perspective](https://arxiv.org/abs/2511.07970)
*Justin Lee,Zheda Mai,Jinsu Yoo,Chongyu Fan,Cheng Zhang,Wei-Lun Chao*

Main category: cs.LG

TL;DR: 本文首次系统研究了文本到图像扩散模型中的持续遗忘问题，发现现有方法在连续遗忘请求下会导致模型性能快速退化，并提出基于正则化的解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法假设所有遗忘请求同时到达，但实际应用中请求往往是连续到达的。需要研究在连续遗忘场景下如何保持模型性能。

Method: 研究了一套附加正则化器来缓解参数漂移，并提出梯度投影方法约束参数在目标子空间正交方向上的漂移。
...
### [237] [Low-Rank Curvature for Zeroth-Order Optimization in LLM Fine-Tuning](https://arxiv.org/abs/2511.07971)
*Hyunseok Seung,Jaewoo Lee,Hyunsuk Ko*

Main category: cs.LG

TL;DR: LOREN是一种曲率感知的零阶优化方法，用于微调大语言模型，通过自适应估计各向异性扰动分布、低秩块对角预处理器和REINFORCE留一梯度估计器来改进现有ZO方法。


<details>
  <summary>Details</summary>
Motivation: 现有的零阶优化方法使用随机扰动通过有限差分估计梯度，存在高方差和次优搜索方向的问题，需要更有效的梯度估计和曲率感知方法。

Method: LOREN通过三个关键创新：自适应估计各向异性扰动分布、使用自然进化策略框架的低秩块对角预处理器捕获曲率、应用REINFORCE留一梯度估计器降低方差。

Result: 在标准LLM基准测试中，LOREN优于最先进的ZO方法，实现了更高的准确性和更快的收敛速度，同时相比MeZO-Adam将峰值内存使用降低了27.3%。

Conclusion: LOREN通过曲率感知的零阶优化方法有效解决了现有ZO方法的高方差和次优搜索方向问题，在保持内存效率的同时提升了微调性能。
...
### [238] [Generalizable Insights for Graph Transformers in Theory and Practice](https://arxiv.org/abs/2511.08028)
*Timo Stoll,Luis Müller,Christopher Morris*

Main category: cs.LG

TL;DR: 提出了广义距离变换器(GDT)，这是一种使用标准注意力的图变换器架构，通过理论分析和大规模实验验证了其在多种应用中的有效性。


<details>
  <summary>Details</summary>
Motivation: 当前图变换器架构在注意力机制、位置嵌入和表达能力方面差异很大，现有理论结果往往与特定设计选择绑定，缺乏在大规模数据上的全面实证验证，理论与实践之间存在差距。

Method: 开发了广义距离变换器(GDT)架构，使用标准注意力机制，结合了近年来图变换器的多项进展，并通过理论分析其表示能力，在超过800万张图、约2.7亿个token的多样化领域进行广泛实验。

Result: 识别了在各种应用、任务和模型规模中一致表现良好的设计选择，在少样本迁移设置中表现出色且无需微调，涵盖图像目标检测、分子属性预测、代码摘要和分布外算法推理等多个领域。
...
### [239] [From Sequential to Recursive: Enhancing Decision-Focused Learning with Bidirectional Feedback](https://arxiv.org/abs/2511.08035)
...
Result: 在报童问题和二分匹配问题上的实验表明，R-DFL在最终决策质量上显著优于顺序基线方法，并在闭环决策问题中展现出强大的适应性。

Conclusion: R-DFL通过双向反馈机制有效提升了决策质量，为复杂交互场景下的决策聚焦学习提供了新的解决方案。
...
### [240] [DynaAct: Large Language Model Reasoning with Dynamic Action Spaces](https://arxiv.org/abs/2511.08043)
*Xueliang Zhao,Wei Wu,Jian Guan,Qintong Li,Lingpeng Kong*

Main category: cs.LG

TL;DR: 提出DynaAct框架，自动构建紧凑动作空间以增强复杂问题解决中的顺序推理，通过提取通用草图并基于效用和多样性选择最优候选集


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖手动定义的动作空间（缺乏可扩展性）或使用非结构化空间（计算成本过高），需要自动构建紧凑动作空间来提升推理效率

Method: 首先使用大语言模型从多样化复杂推理问题语料中提取通用草图来估计完整动作空间，然后制定子模函数评估候选动作的效用和多样性，使用贪心算法选择最优候选集

Result: 在六个多样化标准基准测试上的广泛实验表明，该方法显著提高了整体性能，同时保持高效推理而不引入显著延迟

Conclusion: DynaAct框架能有效自动构建紧凑动作空间，在复杂问题解决场景中显著提升顺序推理性能，同时保持计算效率
...
### [241] [Online Linear Regression with Paid Stochastic Features](https://arxiv.org/abs/2511.08073)
*Nadav Merlis,Kyoungseok Jang,Nicolò Cesa-Bianchi*

Main category: cs.LG

TL;DR: 研究在线线性回归中特征向量被噪声污染的问题，学习者可以付费降低噪声水平。当支付与噪声协方差映射已知时，最优遗憾率为√T；当噪声协方差未知时，最优遗憾率变为T^{2/3}。


<details>
  <summary>Details</summary>
Motivation: 研究在线学习环境中，当特征向量被噪声污染且可以通过付费降低噪声水平时的回归问题，这在实践中可能由于更精确的测量设备或数据隐私激励而发生。
...
Conclusion: 在线线性回归中特征噪声可付费降低的设置下，噪声协方差信息的可获得性显著影响最优遗憾率，从已知时的√T到未知时的T^{2/3}。
...
### [242] [An Integrated Fusion Framework for Ensemble Learning Leveraging Gradient Boosting and Fuzzy Rule-Based Models](https://arxiv.org/abs/2511.08077)
...
<details>
  <summary>Details</summary>
Motivation: 模糊规则模型具有良好可解释性但面临复杂设计和大数据集扩展问题，而梯度提升能提供强大性能，两者融合可克服各自局限性。

Method: 在每次迭代中构建模糊规则模型，通过动态控制因子调节其对整体集成的贡献，同时引入基于样本的校正机制进行自适应调整。

Result: 实验结果表明该框架能有效提升性能，特别是在减轻过拟合和规则复杂性方面表现优异。

Conclusion: 该集成融合框架通过优化控制因子实现了性能提升、可解释性保持和模型维护简化。
...
### [243] [Hierarchical Structure-Property Alignment for Data-Efficient Molecular Generation and Editing](https://arxiv.org/abs/2511.08080)
...
<details>
  <summary>Details</summary>
Motivation: 解决AI驱动药物发现中分子生成和编辑的两个主要障碍：(i) 捕获分子结构与多个性质之间复杂关系的挑战；(ii) 分子性质覆盖范围窄和注释不完整削弱了基于性质的模型效果。

Method: 采用分层结构-性质对齐方法，通过支架聚类选择代表性样本，通过辅助VAE选择困难样本，减少预训练数据需求；结合性质相关性感知掩码机制和多样化扰动策略增强稀疏标注下的生成质量。

Result: 实验证明HSPAG能够捕获细粒度的结构-性质关系，支持多属性约束下的可控生成。两个真实案例研究进一步验证了其编辑能力。

Conclusion: HSPAG框架有效解决了分子生成和编辑中的数据效率问题，能够捕获复杂的结构-性质关系，在稀疏标注条件下仍能实现高质量的可控生成。
...
TL;DR: HipKittens (HK) 是一个针对 AMD GPU 的编程框架，通过瓦片化抽象和优化的内存访问模式，实现了与 AMD 手工优化汇编内核相媲美的性能，并在某些 AI 工作负载中表现更优。


<details>
  <summary>Details</summary>
Motivation: AMD GPU 提供先进的算力和内存带宽，但高性能 AMD 内核通常需要编写原始汇编代码。现有领域特定语言（如 ThunderKittens）主要针对 NVIDIA 硬件，需要探索这些编程原语在 AMD 硬件上的适用性。

Method: 开发 HipKittens 框架，采用瓦片化编程抽象、优化的内存访问和细粒度异步执行，针对 CDNA3 和 CDNA4 AMD 平台进行验证。

Result: HK 内核在 GEMM 和注意力计算中与 AMD 手工优化汇编内核竞争，并始终优于编译器基线。在某些场景下（如 d=64 注意力、GQA 反向传播、内存受限内核），HK 性能比所有可用内核基线高出 1.2-2.4 倍。

Conclusion: 瓦片化抽象可推广到 AMD GPU，但需要重新设计实现这些抽象的算法。这些发现为跨 GPU 厂商的统一高性能 AI 内核软件层铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 检验动态模型中状态稀疏性和时间稀疏性假设在典型强化学习任务中是否成立，为学习算法提供更准确的归纳偏置。

Method: 分析MuJoCo Playground基准套件中机器人强化学习环境的真实动态，研究因果图的稀疏性、状态依赖性和局部动态变化的稀疏性。

Result: 全局稀疏性很少见，但任务在其动态中显示出局部、状态依赖的稀疏性，这种稀疏性表现为时间局部化簇（如接触事件期间）并影响特定状态维度子集。
...
### [246] [Stuart-Landau Oscillatory Graph Neural Network](https://arxiv.org/abs/2511.08094)
*Kaicheng Zhang,David N. Reynolds,Piero Deidda,Francesco Tudisco*

Main category: cs.LG

TL;DR: 提出了基于Stuart-Landau振荡器动力学的复值图神经网络SLGNN，通过保留振幅和相位动力学来解决深度GNN中的过平滑和梯度消失问题。


<details>
  <summary>Details</summary>
Motivation: 现有振荡图神经网络主要基于相位动力学，缺乏振幅调节能力。Stuart-Landau振荡器作为极限环行为的规范模型，能够提供更丰富的动力学特性，包括振幅调节和多稳态同步。

Method: 将Stuart-Landau振荡器动力学引入图神经网络，节点特征振幅和相位都动态演化，通过可调超参数（如Hopf参数和耦合强度）控制特征振幅与网络结构的相互作用。

Result: 在节点分类、图分类和图回归任务上的广泛实验表明，SLGNN优于现有OGNN，为深度振荡图架构建立了新颖、表达力强且理论基础的框架。

Conclusion: SLGNN通过整合Stuart-Landau振荡器的振幅和相位动力学，提供了一种更强大和理论基础的振荡图神经网络架构，在多个图学习任务上表现出优越性能。
...
### [247] [A robust methodology for long-term sustainability evaluation of Machine Learning models](https://arxiv.org/abs/2511.08120)
...
<details>
  <summary>Details</summary>
Motivation: 现有AI系统评估缺乏标准化、模型无关的可持续性评估协议，当前评估仅测量短期实验资源使用，过度强调批处理学习，无法反映真实世界长期AI生命周期。
...
Conclusion: 需要采用更全面的评估方法来准确评估AI系统的长期可持续性，当前评估方法存在局限性，无法反映真实部署环境下的资源消耗模式。
...
### [248] [SafeMIL: Learning Offline Safe Imitation Policy from Non-Preferred Trajectories](https://arxiv.org/abs/2511.08136)
*Returaj Burnwal,Nirav Pravinbhai Bhatt,Balaraman Ravindran*

Main category: cs.LG

TL;DR: 提出SafeMIL方法，通过多示例学习从非偏好轨迹中学习参数化成本函数，用于离线安全模仿学习，避免风险行为同时保持奖励性能。
...
Method: 使用多示例学习从非偏好轨迹中学习参数化成本函数，预测状态-动作对是否具有风险，然后用该成本函数来避免非偏好行为。

Result: 实验证明该方法能学习到满足成本约束的更安全策略，且不会降低奖励性能，优于多个基线方法。

Conclusion: SafeMIL方法能有效利用非偏好轨迹进行离线安全模仿学习，在保证安全的同时维持良好的奖励性能。
...
### [249] [BIPPO: Budget-Aware Independent PPO for Energy-Efficient Federated Learning Services](https://arxiv.org/abs/2511.08142)
*Anna Lackinger,Andrea Morichetta,Pantelis A. Frangoudis,Schahram Dustdar*

Main category: cs.LG

TL;DR: BIPPO是一个基于多智能体强化学习的客户端选择方法，专门为资源受限的物联网联邦学习环境设计，在预算约束下提高性能并保持高能效。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在物联网系统中面临基础设施效率问题，现有基于强化学习的客户端选择方法未充分考虑资源限制、设备流失等实际挑战，且缺乏通用性和能效优化。

Method: 提出BIPPO（预算感知独立近端策略优化），这是一种能效优化的多智能体强化学习解决方案，通过改进的采样器在预算约束下选择客户端。

Result: 在非IID数据的高预算约束环境下，BIPPO相比非RL机制、传统PPO和IPPO提高了平均准确率，且仅消耗可忽略的预算，具有良好的可扩展性。
...
### [250] [Deep (Predictive) Discounted Counterfactual Regret Minimization](https://arxiv.org/abs/2511.08174)
*Hang Xu,Kai Li,Haobo Fu,Qiang Fu,Junliang Xing,Jian Cheng*

Main category: cs.LG

TL;DR: 提出了一种高效的无模型神经CFR算法，能够更好地近似高级CFR变体，在非完美信息博弈中实现更快收敛和更强的对抗性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要基于基础CFR，难以有效整合更先进的CFR变体，限制了CFR在大规模博弈中的应用。

Method: 在每次迭代中：收集基于价值网络的方差减少采样优势；通过自助法拟合累积优势；应用折扣和裁剪操作来模拟高级CFR变体的更新机制。

Result: 与无模型神经算法相比，在典型非完美信息博弈中收敛更快，在大型扑克游戏中表现出更强的对抗性能。

Conclusion: 所提出的算法克服了现有方法在近似高级CFR变体方面的局限性，为大规模博弈求解提供了有效解决方案。
...
### [251] [Improving Long-Range Interactions in Graph Neural Simulators via Hamiltonian Dynamics](https://arxiv.org/abs/2511.08185)
*Tai Hoang,Alessandro Trenta,Alessio Gravina,Niklas Freymuth,Philipp Becker,Davide Bacciu,Gerhard Neumann*

Main category: cs.LG

TL;DR: 提出了一种基于哈密顿动力学原理的信息保持图神经网络模拟器（IGNS），通过保持图上的信息守恒来改进传统图神经模拟器在长程交互和误差累积方面的问题。


<details>
  <summary>Details</summary>
Motivation: 传统数值求解器计算成本高，而现有图神经模拟器（GNS）在捕捉长程相互作用和自回归展开时存在误差累积问题。

Method: 基于哈密顿动力学原理构建图神经网络模拟器，扩展到端口-哈密顿系统以捕获更广泛的动力学，包含预热阶段初始化全局上下文、几何编码处理不规则网格、多步训练目标减少展开误差。

Result: 在所有任务中，IGNS始终优于最先进的GNS，在挑战性和复杂动力学系统中实现更高的准确性和稳定性。

Conclusion: IGNS通过信息保持机制和哈密顿动力学框架，有效解决了图神经模拟器在长程交互和误差累积方面的局限性，在复杂物理系统模拟中表现出优越性能。
...
### [252] [The Online Patch Redundancy Eliminator (OPRE): A novel approach to online agnostic continual learning using dataset compression](https://arxiv.org/abs/2511.08226)
*Raphaël Bayle,Martial Mermillod,Robert M. French*

Main category: cs.LG

TL;DR: 本文提出了OPRE算法，一种在线数据集压缩方法，用于解决持续学习中的灾难性遗忘问题，并在CIFAR数据集上取得了优于现有在线持续学习方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法大多引入了对数据分布的先验信息，无法实现真正的无先验持续学习。特别是依赖预训练特征提取器的方法限制了模型对数据的泛化能力。
...
Conclusion: 在线数据集压缩可能是实现完全无先验持续学习的必要途径。
...
### [253] [Towards Non-Stationary Time Series Forecasting with Temporal Stabilization and Frequency Differencing](https://arxiv.org/abs/2511.08229)
*Junkai Lu,Peng Chen,Chenjuan Guo,Yang Shu,Meng Wang,Bin Yang*

Main category: cs.LG

TL;DR: 提出DTAF双分支框架，通过时域稳定融合和频域波建模处理时间序列的非平稳性，在多个真实世界基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实世界时间序列常呈现非平稳性（时域分布偏移和频谱变异性），这对长期时间序列预测构成重大挑战。

Method: DTAF框架包含两个分支：时域稳定融合模块使用非平稳专家混合滤波器解耦并抑制时域非平稳模式，同时保留长期依赖；频域波建模模块应用频率差分动态突出频谱偏移显著的成分。

Result: 在真实世界基准测试上的广泛实验表明，DTAF优于最先进的基线方法，在非平稳条件下显著提高了预测精度。

Conclusion: DTAF通过融合时域和频域处理，能够有效适应时域和频域的非平稳性，生成鲁棒的预测结果。
...
### [254] [PrefPoE: Advantage-Guided Preference Fusion for Learning Where to Explore](https://arxiv.org/abs/2511.08241)
*Zhihao Lin,Lin Wu,Zhen Tian,Jianglin Lan*

Main category: cs.LG

TL;DR: PrefPoE是一个基于偏好专家乘积的强化学习探索框架，通过优势引导的智能探索来平衡探索与利用，显著提升训练稳定性和样本效率。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习中的熵最大化探索方法存在高方差和策略更新效率低的问题，需要一种更智能的探索机制来平衡探索与利用。

Method: 训练偏好网络来聚焦高优势动作，通过专家乘积（PoE）融合将其与主策略结合，创建软信任区域来稳定策略更新并保持针对性探索。

Result: 在连续和离散动作空间的多样化控制任务中，PrefPoE表现出显著改进：HalfCheetah-v4提升321%（1276→5375），Ant-v4提升69%，LunarLander-v2提升276%，同时增强了训练稳定性和样本效率。

Conclusion: 通过优势引导的偏好学习探索位置与学习如何行动同等重要，为增强策略梯度方法提供了通用框架。
...
### [255] [A Unified Geometric Field Theory Framework for Transformers: From Manifold Embeddings to Kernel Modulation](https://arxiv.org/abs/2511.08243)
*Xianshuai Shi,Jianfeng Zhu,Leibo Liu*

Main category: cs.LG

TL;DR: 提出一个统一的理论框架，将Transformer的位置编码和注意力机制与核积分算子联系起来，提供场论解释。


<details>
  <summary>Details</summary>
Motivation: Transformer架构在多个领域取得成功，但其核心组件（位置编码和注意力机制）缺乏统一的物理或数学解释。

Method: 将离散位置映射到连续流形上的空间函数，将Transformer层解释为在嵌入流形上作用的核调制算子。

Result: 建立了一个结构理论框架，整合了位置编码、核积分算子和注意力机制。

Conclusion: 该框架为Transformer提供了场论解释，有助于深入理论研究和理解其工作机制。
...
### [256] [Data-Driven Discovery of Feature Groups in Clinical Time Series](https://arxiv.org/abs/2511.08260)
*Fedor Sergeev,Manuel Burger,Polina Leshetkina,Vincent Fortuin,Gunnar Rätsch,Rita Kuznetsova*

Main category: cs.LG

TL;DR: 提出了一种通过聚类特征嵌入层权重来自动学习特征分组的新方法，该方法可无缝集成到标准监督训练中，并在临床相关任务上提升性能。


<details>
  <summary>Details</summary>
Motivation: 临床时间序列数据通常包含数百个异质特征，基于相似性和预测任务相关性对特征进行分组已被证明能提升深度学习架构性能，但仅凭语义知识预先定义这些分组对领域专家来说具有挑战性。
...
### [257] [Rethinking Explanation Evaluation under the Retraining Scheme](https://arxiv.org/abs/2511.08281)
*Yi Cai,Thibaud Ardoin,Mayank Gulati,Gerhard Wunder*

Main category: cs.LG

TL;DR: 本文分析了基于重训练的归因评估方法ROAR存在的问题，发现符号问题是导致评估失真的关键因素，并提出改进方案来提升解释器评估的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 当前基于推理的特征归因评估方法存在分布偏移问题，而基于重训练的ROAR方法虽然解决了分布偏移，但其评估结果与理论预期存在矛盾，需要深入分析这种不一致的原因。

Method: 识别了ROAR评估中的符号问题，提出了简单的评估流程重构方案，并基于现有框架设计了新的变体方法，共同构建了全面的解释评估视角。

Result: 提出的改进方案有效解决了识别出的问题，新变体方法显著提高了评估效率，在不同数据规模下的实证结果揭示了解释器性能的深层洞察。

Conclusion: 研究揭示了特征归因评估中的关键挑战，提出的改进方案增强了评估的实用性，为解释器选择和基准测试提供了更可靠的方法，同时指出了可解释性研究的未来方向。
...
### [258] [Dual-Kernel Graph Community Contrastive Learning](https://arxiv.org/abs/2511.08287)
*Xiang Chen,Kun Yue,Wenjie Liu,Zhenyu Zhang,Liang Duan*

Main category: cs.LG

TL;DR: 提出了一种高效的图对比学习框架，通过将输入图转换为紧凑的节点集网络来降低计算复杂度，结合核化社区对比损失和知识蒸馏技术，在保持性能的同时显著提升可扩展性。


<details>
  <summary>Details</summary>
Motivation: 图对比学习在大规模图上的可扩展性受到GNN消息传递机制和对比损失二次计算复杂度的限制，需要解决计算效率问题。

Method: 将输入图转换为紧凑的节点集网络，引入线性复杂度的核化图社区对比损失，结合知识蒸馏技术到解耦的GNN架构中。

Result: 在16个不同规模的真实数据集上验证，该方法在效果和可扩展性方面均优于最先进的图对比学习基线方法。

Conclusion: 提出的高效图对比学习框架能够有效解决大规模图上的计算效率问题，同时保持强大的泛化性能。
...
### [259] [Test-time Diverse Reasoning by Riemannian Activation Steering](https://arxiv.org/abs/2511.08305)
*Ly Tran Ho Khanh,Dongxuan Zhu,Man-Chung Yue,Viet Anh Nguyen*

Main category: cs.LG

TL;DR: 提出一种无监督激活引导策略，通过黎曼优化最大化推理路径的多样性，解决最佳N推理中的输出多样性限制问题


<details>
  <summary>Details</summary>
Motivation: 最佳N推理策略在解决复杂任务时，由于模型生成相似输出而出现输出多样性限制，导致重复相同错误

Method: 在测试时同步锚点处，通过黎曼块坐标下降算法求解黎曼优化问题，找到最大化所有可能干预激活子集总体积的引导向量

Result: 在流行数学基准测试中，该方法在生成多样性和解决方案准确性方面优于普通采样技术

Conclusion: 测试时黎曼激活引导策略能有效提升推理路径多样性，从而提高语言模型在复杂任务中的准确性
...
### [260] [Improving the accuracy and generalizability of molecular property regression models with a substructure-substitution-rule-informed framework](https://arxiv.org/abs/2511.08314)
*Xiaoyu Fan,Lin Guo,Ruizhen Jia,Yang Tian,Zhihao Yang,Boxue Tian*

Main category: cs.LG

TL;DR: MolRuleLoss是一个基于子结构替换规则的框架，通过将SSR的偏导数约束整合到分子性质回归模型的损失函数中，显著提高了多种分子性质预测任务的准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: AI辅助药物发现中，分子性质预测模型在回归任务中准确率较差，特别是对于分布外分子表现灾难性差。需要提高模型的准确性和泛化能力。

Method: 提出MolRuleLoss框架，将子结构替换规则(SSR)的偏导数约束整合到分子性质回归模型(MPRM)的损失函数中。通过SSR的数量和质量来提升预测精度。

Result: 在亲脂性、水溶性和溶剂化自由能预测任务中，RMSE分别从0.660降至0.587、0.798降至0.777、1.877降至1.252，性能提升2.6-33.3%。对于OOD分子，分子量预测的RMSE从29.507降至0.007。
...
### [261] [Adversarial Bias: Data Poisoning Attacks on Fairness](https://arxiv.org/abs/2511.08331)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI系统在现实应用中的广泛部署，确保其公平性变得至关重要。然而，当前研究主要集中在评估和改进公平性，对公平性漏洞（即AI系统公平性如何被故意破坏）的研究相对较少。

Method: 提出一种简单的对抗性投毒策略：向训练集中注入少量精心设计的对抗数据点，偏置模型的决策边界，使其对受保护群体产生不成比例的影响，同时保持泛化性能。

Result: 实验表明，该方法在多个基准数据集和模型上显著优于现有方法，能够以相当或略差的准确率影响为代价，实现显著更高的不公平性水平。该方法在多种模型上都有效，展现了强大的公平性破坏能力。

Conclusion: 该研究证明了机器学习系统公平性存在严重漏洞，简单的对抗性投毒攻击就能有效破坏公平性，这对AI系统的安全部署提出了重要警示。
...
### [262] [LPPG-RL: Lexicographically Projected Policy Gradient Reinforcement Learning with Subproblem Exploration](https://arxiv.org/abs/2511.08339)
*Ruiyu Qiu,Rui Wang,Guanghui Yang,Xiang Li,Zhijiang Shao*

Main category: cs.LG

TL;DR: 提出了LPPG-RL框架，通过序列梯度投影解决连续空间中的词典序多目标强化学习问题，优于现有方法。
...
Method: 使用序列梯度投影识别可行的策略更新方向，将投影步骤重新表述为优化问题，利用Dykstra投影实现加速，并引入子问题探索防止梯度消失。

Result: 在2D导航环境中进行广泛实验，证明LPPG-RL优于现有最先进的连续LMORL方法。

Conclusion: LPPG-RL是一个与所有策略梯度算法兼容的通用LMORL框架，具有理论收敛保证和策略改进下界。
...
### [263] [HN-MVTS: HyperNetwork-based Multivariate Time Series Forecasting](https://arxiv.org/abs/2511.08340)
*Andrey Savchenko,Oleg Kachan*

Main category: cs.LG

TL;DR: HN-MVTS是一种新颖的多变量时间序列预测架构，通过超网络生成目标预测网络最后一层的权重，作为数据自适应正则化器，在不增加推理时间的情况下提高泛化能力和长程预测精度。


<details>
  <summary>Details</summary>
Motivation: 多变量时间序列预测面临复杂时间依赖性的挑战，通道依赖模型性能可能不如通道独立模型，后者虽然不考虑组件间关系但因其小容量而具有高鲁棒性。

Method: 提出HN-MVTS架构，将基于超网络的生成先验与任意神经网络预测模型集成。超网络输入为时间序列组件的可学习嵌入矩阵，生成目标预测网络最后一层的权重，作为数据自适应正则化器。
...
### [264] [From Confusion to Clarity: ProtoScore - A Framework for Evaluating Prototype-Based XAI](https://arxiv.org/abs/2511.08361)
...
Result: 建立了能够有效比较不同原型方法之间以及与其他XAI方法的框架，帮助从业者选择合适的解释方法，同时减少用户研究的成本。

Conclusion: ProtoScore为基于原型的XAI方法提供了标准化的评估基准，特别针对时间序列数据，推动了该领域的客观比较和进步。
...
### [265] [Multi-objective Hyperparameter Optimization in the Age of Deep Learning](https://arxiv.org/abs/2511.08371)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习专家通常拥有关于哪些超参数设置能获得良好性能的先验知识，但现有的超参数优化算法很少能利用这种先验知识，且没有算法能整合多目标的先验知识
...
### [266] [EMAformer: Enhancing Transformer through Embedding Armor for Time Series Forecasting](https://arxiv.org/abs/2511.08396)
*Zhiwei Zhang,Xinyi Du,Xuanchi Guo,Weihao Wang,Wenjuan Han*

Main category: cs.LG

TL;DR: EMAformer通过引入三个关键归纳偏置（全局稳定性、相位敏感性和跨轴特异性）来增强Transformer架构，在12个真实世界基准测试中实现了最先进的多元时间序列预测性能。


<details>
  <summary>Details</summary>
Motivation: 虽然Transformer架构在多元时间序列预测方面取得了显著进展，但iTransformer仍落后于最新的基于MLP的模型。作者认为这种性能差距源于不稳定的通道间关系。

Method: 提出EMAformer模型，通过引入三个关键归纳偏置（全局稳定性、相位敏感性和跨轴特异性）来增强Transformer架构，类似于为其配备强化能力的辅助嵌入套件。

Result: 在12个真实世界基准测试中实现了最先进的性能，平均将预测误差降低了2.73%（MSE）和5.15%（MAE）。

Conclusion: EMAformer显著推进了基于Transformer的方法在多元时间序列预测中的实际应用性。
...
### [267] [Aligning by Misaligning: Boundary-aware Curriculum Learning for Multimodal Alignment](https://arxiv.org/abs/2511.08399)
*Hua Ye,Hang Ding,Siyuan Chen,Yiyang Jiang,Changyuan Zhang,Xuan Zhang*

Main category: cs.LG

TL;DR: BACL是一种轻量级的多模态模型增强方法，通过边界感知课程学习和局部注意力对比损失，将模糊负样本转化为课程信号，显著提升多模态检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型将所有负样本同等对待，忽略了与正样本仅差微小细节的模糊负样本，这些边界案例蕴含重要学习信号。

Method: 提出边界感知负采样器和对比局部注意力损失两个模块，前者逐步增加难度形成课程学习，后者定位不匹配区域，两者完全可微分且兼容现有双编码器。

Result: 理论预测O(1/n)误差率，实践显示在四个大规模基准测试中R@1提升高达32%，超越CLIP并创下新SOTA，无需额外标注。

Conclusion: BACL通过有效利用模糊负样本作为课程信号，显著提升了多模态模型的性能，为多模态学习提供了新的优化方向。
...
### [268] [ARAC: Adaptive Regularized Multi-Agent Soft Actor-Critic in Graph-Structured Adversarial Games](https://arxiv.org/abs/2511.08412)
*Ruochuan Shi,Runyu Lu,Yuanheng Zhu,Dongbin Zhao*

Main category: cs.LG

TL;DR: 提出了ARAC方法，结合注意力图神经网络和自适应发散正则化机制，解决图结构多智能体强化学习中的稀疏奖励问题，在追捕和对抗任务中表现出更快的收敛速度和更好的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 在图结构多智能体强化学习的对抗任务中，智能体需要在高度动态的交互中协调，稀疏奖励阻碍了策略学习效率。

Method: ARAC方法整合了基于注意力的图神经网络来建模智能体依赖关系，以及自适应发散正则化机制。图神经网络能够表达图环境中的空间关系和状态特征，自适应正则化机制在训练早期利用参考策略进行高效探索，随着训练进展逐渐减少依赖。

Result: 在追捕和对抗场景的实验表明，ARAC相比多智能体强化学习基线方法实现了更快的收敛速度、更高的最终成功率，并且在变化智能体数量时表现出更强的可扩展性。

Conclusion: ARAC在复杂图结构环境中具有显著的有效性，能够有效解决稀疏奖励问题并避免继承不完美参考策略的局限性。
...
### [269] [NeuCLIP: Efficient Large-Scale CLIP Training with Neural Normalizer Optimization](https://arxiv.org/abs/2511.08417)
*Xiyuan Wei,Chih-Jen Lin,Tianbao Yang*

Main category: cs.LG

TL;DR: NeuCLIP提出了一种新的优化框架，通过凸分析和变分分析将对比损失重新表述为包含辅助变量的最小化问题，使用紧凑神经网络预测对数归一化因子，解决了CLIP训练中归一化项估计的挑战。


<details>
  <summary>Details</summary>
Motivation: 传统CLIP训练方法依赖大批次来近似归一化项，计算资源需求高。现有方法在大型数据集或小批次情况下存在优化误差问题，限制了有效性。

Method: 通过凸分析将对比损失重新表述为包含辅助变量的最小化问题，然后使用变分分析将其转换为紧凑神经网络的最小化问题，设计交替优化算法联合训练CLIP模型和辅助网络。

Result: 在从百万到数十亿样本的大规模CLIP训练实验中，NeuCLIP相比先前方法实现了更准确的归一化因子估计和更好的性能表现。
...
### [270] [Physics-Informed Neural Operators for Cardiac Electrophysiology](https://arxiv.org/abs/2511.08418)
...
Method: 使用物理信息神经算子（PINO）方法，学习函数空间之间的映射关系，能够泛化到多个网格分辨率和初始条件。

Result: PINO模型能够准确再现心脏电生理动力学，在长时间范围内和多种传播场景下表现良好，包括在训练中未见场景的零样本评估。预测分辨率可扩展至训练分辨率的10倍，且相比数值PDE求解器显著减少模拟时间。

Conclusion: PINO方法为高效且可扩展的心脏电生理模拟提供了有前景的解决方案，具有优异的长期预测能力和分辨率扩展能力。
...
### [271] [HardFlow: Hard-Constrained Sampling for Flow-Matching Models via Trajectory Optimization](https://arxiv.org/abs/2511.08425)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于投影的方法将整个采样路径约束到流形上过于严格且会降低样本质量，需要一种能精确满足硬约束同时保持样本质量的方法
...
Conclusion: 提出的轨迹优化框架不仅能精确满足硬约束，还能通过包含积分成本最小化分布偏移和终端目标进一步提升样本质量，为硬约束生成建模提供了统一解决方案
...
### [272] [An update to PYRO-NN: A Python Library for Differentiable CT Operators](https://arxiv.org/abs/2511.08427)
*Linda-Sophie Schneider,Yipeng Sun,Chengze Ye,Markus Michen,Andreas Maier*

Main category: cs.LG

TL;DR: PYRO-NN是一个基于Python的可微分CT重建库的更新版本，支持PyTorch和原生CUDA内核，提供高效的投影和反投影操作，以及模拟成像伪影和建模任意采集轨迹的工具。
...
Method: 更新PYRO-NN库，扩展对PyTorch的兼容性，引入原生CUDA内核支持，为平行、扇形和锥形束几何提供高效的投影和反投影操作。包括模拟成像伪影、建模任意采集轨迹的工具，并通过高级Python API创建灵活的端到端可训练管道。

Result: 开发了一个更新的PYRO-NN框架，支持PyTorch和CUDA，提供了高效的CT重建操作和灵活的端到端训练能力。

Conclusion: PYRO-NN的更新版本为深度学习在CT重建中的应用提供了强大的工具，通过可微分算子和高效计算支持，促进了经典重建技术与数据驱动方法的结合。
...
### [273] [Coherence Mechanisms for Provable Self-Improvement](https://arxiv.org/abs/2511.08440)
...
<details>
  <summary>Details</summary>
Motivation: 现有自改进方法依赖经验启发式，缺乏形式化保证，需要建立有理论保证的自改进原则框架。

Method: 使用投影机制更新基线模型，使其满足一致性要求同时保持与原始行为接近，包括直接和两步投影方法。

Result: 证明了这些机制能实现单调改进（Bregman散度减少），并扩展到非可实现设置、经验分布和松弛一致性约束。
...
### [274] [One Model for All: Universal Pre-training for EEG based Emotion Recognition across Heterogeneous Datasets and Paradigms](https://arxiv.org/abs/2511.08444)
*Xiang Li,You Li,Yazhou Zhang*

Main category: cs.LG

TL;DR: 提出'One Model for All'通用预训练框架，通过解耦学习和新颖的ART-GAT架构解决EEG数据集异构性问题，在多个基准测试中达到新的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: EEG情感识别面临严重的数据集异构性（通道/被试变异性），阻碍了泛化模型的开发，现有方法难以有效迁移知识。

Method: 两阶段学习：1）通过自监督对比学习进行单变量预训练，使用统一通道模式；2）使用新颖的ART（自适应重采样变换器）和GAT（图注意力网络）架构进行多变量微调，捕捉复杂时空依赖关系。

Result: 在SEED（99.27%）、DEAP（93.69%）和DREAMER（93.93%）上达到新的SOTA性能；跨数据集迁移在DREAMER上达到94.08%（交集）和93.05%（UCS）；GAT模块在DEAP上相比GCN提升22.19%。
...
### [275] [Binary Split Categorical feature with Mean Absolute Error Criteria in CART](https://arxiv.org/abs/2511.08470)
...
### [276] [Clustering Guided Residual Neural Networks for Multi-Tx Localization in Molecular Communications](https://arxiv.org/abs/2511.08513)
...
<details>
  <summary>Details</summary>
Motivation: 分子通信扩散中的多发射器定位是一个关键但具有挑战性的问题，主要困难在于扩散的随机性和接收器表面分子分布的重叠。

Method: 1. 基于聚类的质心校正方法，增强对密度变化和异常值的鲁棒性；2. 两种聚类引导的残差神经网络：AngleNN用于方向细化，SizeNN用于聚类大小估计。
...
### [277] [LeJEPA: Provable and Scalable Self-Supervised Learning Without the Heuristics](https://arxiv.org/abs/2511.08544)
*Randall Balestriero,Yann LeCun*

Main category: cs.LG

TL;DR: 提出了LeJEPA，一种理论基础的联合嵌入预测架构训练目标，通过SIGReg正则化约束嵌入分布，实现高效稳定的自监督预训练。


<details>
  <summary>Details</summary>
Motivation: 解决JEPA架构缺乏理论指导和实践规范的问题，为自监督学习提供理论支撑和实用方法。

Method: 结合JEPA预测损失和SIGReg正则化，约束嵌入服从各向同性高斯分布，简化训练流程。

Result: 在10+数据集、60+架构上验证，ViT-H/14在ImageNet-1k上达到79%准确率，具有线性复杂度、超参数稳定等优势。
...
### [278] [FMMI: Flow Matching Mutual Information Estimation](https://arxiv.org/abs/2511.08552)
*Ivan Butakov,Alexander Semenenko,Alexey Frolov,Ivan Oseledets*

Main category: cs.LG

TL;DR: 提出了一种新的互信息估计器，通过训练归一化流将一个分布转换为另一个分布，而不是训练分类器来区分联合分布和边缘分布。


<details>
  <summary>Details</summary>
Motivation: 传统的基于分类器的互信息估计方法存在局限性，需要更高效、精确且可扩展的估计方法。

Method: 学习一个归一化流，将联合分布转换为边缘分布，从而计算互信息。

Result: 该方法计算效率高、精度好，能够很好地扩展到高维数据，并在广泛的真实互信息值范围内表现良好。

Conclusion: 这种基于归一化流的互信息估计方法为高维互信息估计提供了一种有效的新途径。
...
### [279] [The Path Not Taken: RLVR Provably Learns Off the Principals](https://arxiv.org/abs/2511.08567)
...
TL;DR: RLVR（带可验证奖励的强化学习）在提升大语言模型推理性能时表现出参数稀疏性，但这实际上是模型条件优化偏差的表面现象。RLVR在权重空间的非主方向上学习，通过最小化谱漂移和减少主空间旋转来实现性能提升，与SFT（监督微调）形成不同的优化机制。


<details>
  <summary>Details</summary>
Motivation: 研究RLVR训练中参数稀疏性的本质，揭示其与SFT不同的优化动态，为理解RLVR学习机制提供参数层面的解释。

Method: 提出三闸门理论：闸门I（KL锚点）施加KL约束更新；闸门II（模型几何）将更新引导至低曲率、保持谱结构的子空间；闸门III（精度）在非偏好区域隐藏微更新。通过参数级分析验证理论。

Result: RLVR在非主方向学习，实现最小谱漂移、减少主空间旋转和非主方向更新对齐，而SFT则针对主权重并扭曲谱结构。RLVR与SFT处于不同的优化机制。

Conclusion: RLVR具有独特的优化机制，直接套用SFT时代的参数高效微调方法存在缺陷，需要设计几何感知的RLVR原生学习算法。
...
### [280] [Automatic Grid Updates for Kolmogorov-Arnold Networks using Layer Histograms](https://arxiv.org/abs/2511.08570)
*Jamison Moody,James Usevitch*

Main category: cs.LG

TL;DR: 提出了AdaptKAN，一种能够自主调整域网格的Kolmogorov-Arnold网络变体，通过直方图算法实现数据驱动的域更新，在多个任务中表现优于传统KAN和MLP。


<details>
  <summary>Details</summary>
Motivation: 传统KAN需要手动调整域网格，增加了训练过程的复杂性，且无法根据前层输出范围自主更新域。

Method: 使用直方图算法实现数据驱动的域网格自主更新，该算法还可用于检测分布外输入。

Result: 在四个任务中表现优异：Feynman数据集学习科学方程、图像分类、学习控制Lyapunov函数、OpenOOD v1.5基准的OOD检测。

Conclusion: AdaptKAN在保持KAN优势的同时，通过自主域更新简化了训练过程，并在多个任务中达到或超越了现有方法的性能。
