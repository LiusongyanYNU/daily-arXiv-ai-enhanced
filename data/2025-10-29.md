...
Motivation: AI生成图像的逼真度不断提高，给视觉真实性验证带来挑战，需要开发可解释的检测系统。
...
Conclusion: 这项工作展示了结合视觉和语言推理在低分辨率图像中进行可解释真实性检测的可行性，并指出了在取证、工业检测和社交媒体审核等领域的跨领域应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有计数模型在处理复杂形状、内部对称或重叠组件时容易出错，无法像人类那样通过感知视觉重复和结构关系来计数。
...
Conclusion: 整合DINOv2等基础模型使计数系统能够接近人类的结构感知能力，朝着真正通用和无示例的计数范式迈进。
...
<details>
  <summary>Details</summary>
Motivation: 河流中漂浮的人造垃圾对生物多样性、水质及人类活动造成负面影响，需要有效的监测方法

Method: 使用固定原位摄像头采集数据，应用深度学习模型进行垃圾检测和量化，结合几何模型从2D图像估计物体实际尺寸，考虑数据泄露偏差和负样本集成

Result: 验证了在不同环境条件下深度学习模型的准确性和推理速度，证明了基于投影几何和回归校正的度量物体估计的可行性
...
<details>
  <summary>Details</summary>
Motivation: 传统遥感图像超分辨率方法在分布外条件下（如罕见地貌特征、多传感器数据）容易产生视觉合理但物理不准确的结果，需要开发能够保持物理一致性的鲁棒方法。

Method: 采用双条件架构：门控ControlNet保持低分辨率输入的几何保真度，文本提示提供语义引导；引入多面损失函数确保光谱和辐射一致性；使用随机前向传播量化预测不确定性。

Result: 在盲评估中，地球物理专家认为模型输出接近真实图像保真度，显著优于现有基线方法；定量指标显示FID降低近40%。
...
<details>
  <summary>Details</summary>
Motivation: 将CLIP模型扩展到语义分割具有挑战性，因为其图像级预训练目标与密集预测所需的像素级视觉理解存在不对齐。先前方法继承了前层的全局对齐偏差，导致分割性能不理想。

Method: 提出三种互补技术：语义-空间重加权、选择性头部增强和异常token替换，基于对CLIP视觉辨别能力的三个关键发现：最终层牺牲视觉辨别性、部分注意力头具有强视觉辨别性、异常token具有稀疏激活模式。

Result: 在8个常见语义分割基准测试中，LHT-CLIP实现了最先进的性能，证明了其有效性和实际部署的实用性。

Conclusion: LHT-CLIP通过系统利用CLIP的多层次视觉辨别能力，无需额外训练即可显著提升语义分割性能，为实际应用提供了高效解决方案。
...
Method: 使用YouCookII数据集，通过自适应帧采样和多模态窗口捕捉关键转换，采用多模态思维链生成多个动作-对象对，使用动态步长窗口选择算法进行精炼和融合。
...
Method: 采用前馈图像到化身生成管道获取初始3D表示和噪声渲染，然后使用基于输入图像的单步扩散模型进行多视角一致的精炼，并采用合成多视图数据预训练加高质量真实图像微调的训练策略。

Result: 该方法在质量和数量上都优于当前最先进的人像新视角合成方法，同时保持时间效率。

Conclusion: TurboPortrait3D成功证明了图像空间扩散模型可以显著提升现有图像到化身方法的生成质量，同时保持3D感知和低延迟运行。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS在低纹理区域（如室内环境）中，仅使用光度损失会导致几何模糊，无法恢复高保真3D表面。

Method: 设计语言提示平面先验（LP3）流程，使用预训练视觉语言分割模型，并通过交叉视图融合和几何先验检查优化区域提议。在3D高斯优化中加入平面先验监督项和几何先验监督项。

Result: 在标准室内基准测试中，PlanarGS重建出准确且详细的3D表面，大幅优于现有最先进方法。

Conclusion: PlanarGS通过结合平面和几何先验，有效解决了3DGS在室内场景重建中的几何模糊问题，显著提升了重建质量。
...
Method: 采用神经元剪枝机制避免冗余，通过识别贡献较小的神经元并应用目标权重衰减来转移信息，然后进行结构化剪枝；通过输入频率密集化在信号欠拟合的频谱区域添加频率来扩展表示基础。

Result: 在图像和SDF上的实验表明，AIRe在减少模型大小的同时保持甚至提高了重建质量。
...
Method: 受计算机图形学中USD标准启发，引入Neural USD框架，采用结构化层次化表示场景和对象，应用微调方法确保控制信号解耦。

Result: 评估了框架的多种设计考虑，证明Neural USD能够支持迭代和增量工作流程。

Conclusion: Neural USD框架为精确的对象级编辑控制提供了有效解决方案，推动了可控生成建模的发展。
...
Result: 在VisionHarm-T上比GPT-4o提升8.6%，在VisionHarm-C上提升15.5%，速度提升16倍以上，实现了最先进的性能。
...
Method: 采用两阶段训练方法：首先进行推理风格的监督微调，然后使用基于可验证奖励的强化学习，结合高保真视觉编码，输出符合放射科医生思维过程的推理。

Result: 在分布外评估中实现了有竞争力的多标签分类性能，同时提高了可解释性。在专家放射科医生的阅读研究中，完整推理轨迹增强了信心，支持错误审计，并减少了报告完成时间。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试通常孤立评估AI助手的能力，缺乏真实的流式场景，或仅支持短期任务，无法满足现实世界中AI助手需要处理多模态输入、实时响应并保持长期记忆的需求。

Method: 构建包含4个领域（工作与学习、生活方式与日常、社交活动、外出与文化）的长时数据集，所有数据在统一时间线上对齐，包含高质量视觉叙述和语音转录。定义12个子任务，使用3,291个人工验证的QA项目，在流式设置下严格评估。

Result: 提出了两个关键指标——实时准确率和记忆持久时间，用于联合评估正确性、时间响应性和长期记忆保持能力。

Conclusion: TeleEgo为开发实用的AI助手提供了现实且全面的评估框架，能够推动AI助手在实际应用中的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在糖尿病视网膜病变检测中面临分布变化导致的鲁棒性挑战，如采集设备差异、人口统计学差异和成像条件变化。
...
<details>
  <summary>Details</summary>
Motivation: 主动脉血管树的自动分析具有巨大临床潜力，但缺乏共享高质量数据阻碍了其发展，因此需要创建公开数据集来推动该领域进步。

Method: 通过SEG.A挑战赛引入大型公开多机构数据集，对隐藏测试集进行基准测试，随后进行可选的计算模拟表面网格生成任务。

Result: 深度学习方法是主要趋势，3D U-Net架构在顶级提交中占主导地位；集成排名最高的算法显著优于单个模型；性能与算法设计（特别是定制后处理步骤）和训练数据特征密切相关。
...
Method: 引入Mars-Bench基准，提供标准化的数据集和基线评估，使用在自然图像、地球卫星数据和最先进视觉语言模型上预训练的模型。

Result: 所有分析结果表明，火星特定的基础模型可能比通用领域模型具有优势。

Conclusion: Mars-Bench旨在为火星科学的机器学习模型开发和比较建立标准化基础，并鼓励进一步探索领域适应的预训练。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型的安全机制容易受到对抗性提示的攻击，现有的红队方法通常需要白盒访问，效率低下且生成语义无意义的提示容易被过滤器拦截。

Method: 采用交替优化微调流程，在对抗性后缀优化和LLM微调之间交替进行，并集成双重规避策略来绕过困惑度过滤器和黑名单过滤器。

Result: 实验证明该方法在红队测试中表现优异，生成的人类可读对抗性提示能够抵抗过滤器，并具有出色的零样本迁移能力，能够适应未见过的提示并暴露商业API的关键漏洞。

Conclusion: APT框架能够有效评估文本到图像模型的安全漏洞，生成人类可读且能绕过过滤器的对抗性提示，具有实际应用价值。
...
Result: 在CIFAR-10数据集上，ResNet-18达到89.9%准确率，相比传统深度CNN的84.1%有显著提升，同时收敛速度更快、训练更稳定。

Conclusion: ResNet通过跳跃连接有效解决了深度网络训练难题，实现了更深网络的稳定训练和更好性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的稀疏调优方法存在两个问题：1) 两阶段方法通过梯度信息定位任务相关权重，忽略了微调过程中的参数调整；2) 仅更新定位权重的稀疏掩码方法需要存储所有权重矩阵，内存使用高。

Method: 提出单阶段方法SNELLA：1) 通过两个低秩可学习矩阵合并成稀疏矩阵来选择性更新权重矩阵；2) 引入非线性核函数扩展低秩分解，防止权重更新间的相互依赖；3) 提出自适应双层稀疏分配机制，基于重要性分数在层间和层内进行端到端的权重竞争。
...
Conclusion: SNELLA通过单阶段设计和创新的稀疏优化技术，在参数高效微调中实现了性能提升和内存使用的显著优化。
...
Method: COLA框架包含两个步骤：(1)将对抗图像嵌入投影到类文本特征张成的子空间，过滤非语义失真；(2)将图像和文本建模为多个增强视图上的离散分布，通过最优传输优化对齐，并将子空间投影无缝集成到成本计算中。

Result: 在14个零样本分类基准测试中验证了COLA的有效性，特别是在ImageNet及其变体上，在PGD对抗攻击下平均提升6.7%，同时在干净样本上保持高准确率。

Conclusion: COLA是一种无需训练、与现有微调模型兼容的框架，能够有效解决对抗条件下的跨模态特征对齐问题，显著提升视觉语言模型的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: T2I模型用于合成数据集生成时，使用少量真实样本微调可提升数据质量，但可能导致过拟合和多样性降低，特别是在细粒度分类任务中。

Method: 提取类无关属性（如场景背景、物体姿态），在T2I模型微调时显式条件化这些属性，生成时边缘化它们，以缓解过拟合并保持生成先验。

Result: 在多个T2I模型、骨干网络和数据集上的实验表明，BOB在低样本细粒度分类中达到SOTA性能，在Aircraft数据集上比DataDream提升7.4%，在18/24个实验设置中优于先前方法。

Conclusion: BOB方法通过条件化和边缘化类无关属性，有效缓解了T2I模型在合成数据生成中的过拟合问题，显著提升了细粒度分类性能，特别是在低样本场景下。
...
Method: 提出OmniText框架，通过自注意力反转实现文本移除，通过重新分配交叉注意力减少文本幻觉。在潜在优化框架中引入交叉注意力内容损失和自注意力样式损失，以改善文本渲染精度和样式定制。

Result: OmniText在多个任务和指标上实现了最先进的性能，与其他文本修复方法相比表现优异，与专门方法相媲美。

Conclusion: OmniText是第一个能够执行多样化文本图像操作任务的通才方法，通过利用注意力机制的特性，成功解决了现有文本修复方法的关键限制。
...
Result: 发现可解释性和分类能力呈正相关，高分类能力的表示包含更多可解释语义；通过可解释性最大化微调可进一步提升分类能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法主要针对同构模型，在异构架构下中间特征存在语义差异，导致性能下降

Method: 使用傅里叶变换在频域捕获全局特征，通过特征转换模块和特征对齐模块实现跨架构特征匹配，结合中间特征MSE损失和logits KL散度联合训练

Result: 在CIFAR-100和ImageNet-1K上分别比最新方法提升5.59%和0.83%

Conclusion: UHKD能有效统一异构表示，实现视觉知识的高效利用
...
Method: 提出了三阶段实例特定优化流程：粗对齐、密集对应监督和时间正则化，将SMAL模型拟合到运动序列中。

Result: 建立了四个运动恢复基准设置，支持单目/多视角、RGB/RGB-D输入的系统评估。

Conclusion: 该数据集和方法为狗运动恢复研究提供了理论基础，并开启了计算机视觉、计算机图形学和动物行为建模交叉领域的新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有训练免费方法在加速扩散过程时忽略去噪趋势且缺乏误差控制，导致多步重用下的轨迹偏差和生成结果不一致。

Method: ETC框架包含：(1)一致性趋势预测器，利用扩散轨迹的平滑连续性预测稳定未来方向；(2)模型特定误差容限搜索机制，通过识别从语义规划到质量精炼的过渡点来推导校正阈值。
...
Method: 利用大语言模型从输入提示中合成显式布局，将这些布局注入图像生成过程，通过对象中心的视觉语言模型对多个候选结果进行重排序，迭代选择最符合提示的结果。

Result: 通过统一显式布局基础和基于自优化的推理时扩展，该框架在场景与提示对齐方面优于最近的文本到图像模型。

Conclusion: 该训练免费框架能有效提升文本到图像生成的布局忠实度，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在自动驾驶场景理解任务中的性能瓶颈，特别是在感知、预测、规划和异常检测等多样化任务上的表现优化需求。

Method: 采用四核心组件：混合提示路由分类问题并分发到专家提示；任务特定提示嵌入坐标系统、空间推理规则和思维链；视觉组装模块组合多视角图像和对象裁剪；按任务配置推理参数。

Result: 在Phase-1（干净数据）上达到70.87%平均准确率，在Phase-2（异常数据）上达到72.85%平均准确率，证明了结构化提示和空间基础的有效性。

Conclusion: 系统化的提示工程和空间基础显著提升了视觉语言模型在安全关键自动驾驶任务上的性能，为实际应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: SAM2作为图像分割基础模型SAM的继任者，在视频分割中表现出强大的泛化能力，但其对抗鲁棒性尚未被探索。现有针对SAM的攻击是否能直接迁移到SAM2存在疑问。

Method: 提出UAP-SAM2方法：1）目标扫描策略将每帧分为k个区域并随机分配提示以减少提示依赖性；2）双语义偏差框架通过扭曲当前帧语义和破坏连续帧间语义一致性来优化UAP。

Result: 在6个数据集上的两个分割任务实验表明，UAP-SAM2显著优于现有最先进攻击方法，表现出卓越的有效性。

Conclusion: UAP-SAM2成功解决了SAM2的对抗攻击挑战，为视频分割模型的鲁棒性评估提供了有效工具。
...
Method: 提出CLFSeg框架，集成模糊卷积模块来识别局部和全局特征，同时减少边界区域的不确定性和噪声；采用二元交叉熵与Dice损失结合来处理类别不平衡问题。

Result: 在四个公开数据集（CVC-ColonDB、CVC-ClinicDB、EtisLaribPolypDB、ACDC）上表现出色，超越了现有SOTA方法，并在解剖结构中聚焦相关感兴趣区域。
...
Method: 提出基于耦合的信息论方法MC-SJD，通过最大化连续迭代中采样相同草稿令牌的概率来加速标准SJD，仅需对现有算法进行单行修改。

Result: 在图像生成中实现约4.2倍加速，视频生成中实现约13.3倍加速，且输出质量无任何下降。

Conclusion: MC-SJD是一种高效的无损加速方法，显著提升了自回归视觉生成的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉token修剪方法主要基于注意力分数选择最显著的token，导致所选token语义不完整，存在冗余token导致的巨大计算开销问题。

Method: 提出SCOPE评分方法，结合token的显著性和覆盖度，通过计算集合覆盖度和token覆盖度增益，迭代选择具有最高SCOPE分数的token。

Result: 在多个视觉语言理解基准测试中使用LLaVA-1.5和LLaVA-Next模型进行实验，结果一致优于现有方法。

Conclusion: SCOPE方法通过联合考虑显著性和覆盖度，有效解决了视觉token修剪中的语义完整性问题，显著提升了多模态大语言模型的效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统微眼跳研究方法成本高、扩展性差、时间分辨率有限，而基于事件感知技术能高效捕捉精细时空变化，为认知计算中的小眼动动力学研究提供新途径。

Method: 使用Blender渲染高保真眼动场景，模拟0.5-2.0度角位移的微眼跳，分为7个类别，通过v2e转换为事件流，并开发Spiking-VGG16Flow等脉冲神经网络模型进行分类。

Result: 模型平均准确率达到约90%，成功根据角位移分类微眼跳，且分类结果与事件数量或持续时间无关。

Conclusion: 脉冲神经网络在精细运动识别方面具有潜力，该研究为基于事件的视觉研究建立了基准，数据集和代码将公开共享。
...
<details>
  <summary>Details</summary>
Motivation: 在恶劣条件下（如雾霾和低光照）提高检测鲁棒性时，图像恢复通常作为预处理步骤来增强图像质量。但恢复网络与检测网络之间的功能不匹配会引入不稳定性并阻碍有效集成，这一问题尚未得到充分探索。

Method: 通过Lipschitz连续性的视角分析恢复网络和检测网络在输入空间和参数空间的功能差异，提出LROD框架，将图像恢复直接集成到检测器的特征学习中，在训练过程中协调两个任务的Lipschitz连续性。实现为Lipschitz正则化YOLO(LR-YOLO)，可无缝扩展到现有YOLO检测器。

Result: 在雾霾和低光照基准测试上的大量实验表明，LR-YOLO持续提高了检测稳定性、优化平滑度和整体准确性。

Conclusion: LROD框架通过协调恢复和检测任务的Lipschitz连续性，有效解决了传统级联框架中的功能不匹配问题，显著提升了目标检测在恶劣条件下的性能和稳定性。
...
Conclusion: 通过将序列建模与结构完整性和色彩一致性要求相结合，该方法成功解决了阴影去除中的关键挑战。
...
Result: 在8个不同复杂度和粒度的基准数据集上进行广泛实验，UtilGen始终实现优越性能，平均准确率比之前SOTA提高3.87%。
...
<details>
  <summary>Details</summary>
Motivation: 合成图像来源归因在数据稀缺条件下具有挑战性，需要少样本或零样本分类能力。现有方法在训练样本有限时性能不足。

Method: 训练免费的one-shot归因方法：生成图像描述提示词，用所有候选源模型重合成图像，在特征空间中寻找与原图最接近的重合成结果对应的模型作为归因结果。

Result: 提出的重合成方法在训练样本有限时优于现有最先进的少样本方法和其他基线方法。新数据集被证明具有挑战性，是评估未来少样本和零样本方法的宝贵基准。

Conclusion: 基于重合成的方法在数据稀缺条件下优于现有技术，新数据集为开发更强大的归因模型提供了有价值的测试平台。
...
Method: 设计了两阶段任务，将视觉感知学习构建为从粗到细的渐进过程。开发了ViPER自举框架，通过自我批判和自我预测实现迭代进化，结合图像级和实例级重建与两阶段强化学习策略。

Result: 在Qwen2.5-VL系列上应用ViPER得到Qwen-Viper系列，在7个综合基准测试中平均提升1.7%，在细粒度感知任务上提升高达6.0%，同时保持通用性。

Conclusion: ViPER不仅实现了感知能力的自我提升，还为生成与理解之间的互惠关系提供了具体证据，是开发更自主、更强大VLMs的重要突破。
...
<details>
  <summary>Details</summary>
Motivation: 传统师生框架在医学图像分割中受到师生网络强相关性和不可靠知识传递的限制，需要改进协作机制和伪标签质量。

Method: 采用切换式双学生架构，迭代选择最可靠学生；引入损失感知指数移动平均策略，动态优化教师从学生吸收有意义信息。

Result: 在3D医学图像分割数据集上超越现有最先进的半监督方法，在有限监督下显著提升分割精度。

Conclusion: 该即插即用框架通过改进师生协作和伪标签质量，有效解决了半监督医学图像分割中的关键挑战。
...
TL;DR: W2-Net通过双查询机制解耦指代表达式计数问题，引入where-to-see查询从属性特定区域提取特征，结合子类可分匹配策略提升性能，在REC-8K数据集上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决指代表达式计数中标注点通常位于类别代表性位置（如头部）的问题，这导致模型关注类别级特征而忽略其他视觉区域（如腿部）的属性信息。

Method: 提出W2-Net框架，使用双查询机制：what-to-count查询定位对象，where-to-see查询从属性特定区域提取特征；引入子类可分匹配策略增强子类间可分性。

Result: 在REC-8K数据集上，计数误差降低22.5%（验证集）和18.0%（测试集），定位F1分别提升7%和8%，显著超越最先进方法。

Conclusion: W2-Net通过明确解耦"计数什么"和"从哪里看"的问题，有效解决了指代表达式计数中的关键挑战，为细粒度对象计数提供了新思路。
...
TL;DR: StrokeSeg是一个轻量级模块化框架，可将研究级脑卒中病灶分割模型转化为可部署的临床应用工具，通过解耦预处理、推理和后处理流程，使用ONNX Runtime和Float16量化，在保持性能的同时显著减小模型体积。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习框架如nnU-Net在脑卒中病灶分割中表现优异，但由于依赖繁重和整体化设计，难以在临床环境中部署使用。

Method: 采用模块化设计，预处理使用Anima工具箱生成BIDS兼容输出，推理使用ONNX Runtime和Float16量化（减少约50%模型体积），提供图形界面和命令行界面，支持Python脚本和独立Windows可执行文件。

Result: 在300名亚急性和慢性卒中患者的测试集上，分割性能与原始PyTorch流程相当（Dice差异<10^-3）。

Conclusion: 高性能研究流程可以成功转化为便携、临床可用的工具，证明了框架的有效性和实用性。
...
TL;DR: 提出了一种亮度感知多尺度网络（MLSN）用于偏振图像融合，通过动态亮度加权、全局-局部特征融合和亮度增强模块，解决了偏振图像在复杂光照环境下的对比度差异问题，并在多个数据集上取得了优于现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: 偏振图像融合结合S0和DOLP图像，通过互补的纹理特征揭示表面粗糙度和材料特性，在伪装识别、组织病理分析、表面缺陷检测等领域有重要应用。但现有方法在复杂光照环境下难以有效整合不同偏振图像的互补信息。

Method: 在编码器阶段提出多尺度空间权重矩阵，通过亮度分支动态加权注入亮度信息；在瓶颈层设计全局-局部特征融合机制，通过窗口自注意力计算平衡全局上下文和局部细节；在解码器阶段提出亮度增强模块，建立亮度分布与纹理特征的映射关系，实现融合结果的非线性亮度校正。

Result: 在MSP、PIF和GAND数据集上的实验验证表明，MLSN在主观和客观评估中均优于现有方法，MS-SSIM和SD指标分别比其他方法的平均值高出8.57%、60.64%、10.26%、63.53%、22.21%和54.31%。

Conclusion: 提出的MLSN网络有效解决了偏振图像融合中的亮度适应性问题，同时贡献了包含1000对偏振图像的MSP数据集，填补了该领域高质量数据集的空白。
...
TL;DR: 该论文系统研究了放射学报告在医学图像分类中的使用时机和方式，发现报告在预训练阶段对文本中明确表示的任务有益，但在弱相关任务中可能有害；在微调阶段使用报告能带来显著改进。


<details>
  <summary>Details</summary>
Motivation: 医学图像通常配有包含丰富专家注释的放射学报告，但依赖这些报告需要放射科医生的手动工作。研究旨在确定何时以及如何利用放射学报告来改进仅基于图像的分类。

Method: 系统研究了放射学报告在预训练和微调阶段的使用，涵盖诊断和预后任务，并在不同训练集规模下进行实验。

Result: 发现：(1) 当标签在文本中明确表示时，预训练阶段利用报告有益，但通过显式图像-文本对齐的预训练在弱相关设置中可能有害；(2) 微调阶段使用报告能带来显著改进，在某些情况下影响甚至大于预训练方法。
...
TL;DR: 评估REFLECT流生成模型在脑卒中后患者中无监督检测局灶性和非病灶性异常的能力，发现使用健康对照数据训练的模型性能优于使用卒中患者无病灶数据训练的模型。
...
Method: 使用REFLECT流生成模型进行无监督异常检测，在ATLAS数据上使用双专家中心切片标注，通过自由响应ROC分析评估异常图的对象级性能，比较在卒中患者无病灶切片和健康对照数据上训练的两种模型。
...
TL;DR: GenTrack是一种新颖的多目标跟踪方法，采用混合跟踪策略结合随机和确定性方式，利用粒子群优化处理非线性动态和目标数量变化，集成社交交互减少ID切换和轨迹丢失，在标准基准测试中表现优于现有最先进跟踪器。


<details>
  <summary>Details</summary>
Motivation: 解决多目标跟踪中未知和时变目标数量的挑战，特别是在保持目标身份一致性和处理非线性动态方面，以及应对弱噪声检测器和遮挡情况下的跟踪问题。

Method: 采用混合跟踪方法结合随机和确定性策略，利用粒子群优化引导随机粒子朝向目标分布模式，集成社交交互增强粒子引导，建立基于空间一致性、外观、检测置信度、轨迹惩罚和社交分数的综合状态观测模型。

Result: 在标准基准测试和实际场景中，GenTrack相比现有最先进跟踪器提供更优越的性能，减少了ID切换和轨迹丢失，特别是在遮挡情况下表现突出。

Conclusion: GenTrack通过创新的混合跟踪方法和PSO集成，有效解决了多目标跟踪中的关键挑战，为视觉MOT提供了新的基准，并提供了开源实现便于进一步研究和应用。
...
TL;DR: 提出了一种结合随机和确定性机制的多目标跟踪方法，使用粒子滤波和粒子群优化处理非线性动态，通过确定性关联确保标识符一致性，适用于预录制视频和实时摄像头流。
...
Method: 使用随机粒子滤波处理非线性动态和非高斯噪声，结合粒子群优化引导粒子；确定性关联通过成本矩阵确保标识符一致性；提出平滑更新目标状态的方案；使用速度回归提供趋势种子速度。

Result: 实验结果显示相比最先进的跟踪器具有优越性能。

Conclusion: 该方法在保持目标身份一致性方面表现优异，特别适用于目标交互和遮挡场景，且能灵活应用于预录制和实时视频流。
...
TL;DR: 该研究提出了一种基于卫星影像和机器学习的无传感器方法，用于实时监测黎巴嫩Qaraaoun水库的蓄水量，避免了传统传感器故障和维护问题。
...
TL;DR: 提出了一个专门针对语义分割任务的XAI评估框架，通过像素级评估策略和精心设计的指标来提供细粒度可解释性洞察。


<details>
  <summary>Details</summary>
Motivation: 随着AI模型在安全关键和高风险领域的应用增加，确保透明度和可信度变得至关重要。虽然分类任务的XAI评估已有显著进展，但针对语义分割的评估策略仍相对不足。

Method: 开发了一个全面系统的评估框架，专门用于评估语义分割中的XAI方法，明确考虑空间和上下文任务复杂性，采用像素级评估策略和精心设计的指标。

Result: 使用最近适应的基于类激活映射(CAM)的XAI方案的仿真结果表明，所提出方法具有高效性、鲁棒性和可靠性。
...
TL;DR: 提出DCIC-sgp框架，通过自生成先验进行深度条件化，有效分离图像的结构信息和纹理细节，在低码率下显著减少几何变形伪影。


<details>
  <summary>Details</summary>
Motivation: 现有学习型图像压缩方法难以建模自然图像中复杂的相关性结构，特别是全局结构和局部纹理的纠缠问题，导致低码率下出现严重几何变形。

Method: 采用功能分解框架，首先编码自生成先验来捕捉图像结构骨架，然后用该先验深度调节整个压缩流程，特别是分析变换，使其专注于残差的高熵细节。

Result: 视觉分析显示显著减轻了低码率下的几何变形伪影；在Kodak、CLIC和Tecnick数据集上分别实现了14.4%、15.7%和15.1%的BD-rate降低。

Conclusion: 提出的深度条件化方法通过层次化的依赖驱动实现了信息流的有效分离，在保持竞争力的同时显著改善了压缩性能。
...
TL;DR: 视频扩散模型(VDMs)在视觉任务中比大型语言模型(LLMs)表现出更高的数据效率，视频预训练提供了支持视觉基础模型发展的归纳偏差。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在语言领域取得了成功，但在视觉领域仍面临组合理解、样本效率和通用问题解决能力的挑战，需要探索更好的视觉基础模型方法。
...
Conclusion: 视频预训练提供了支持视觉基础模型发展的强归纳偏差，是推进视觉智能的有前景方向。
...
TL;DR: 该论文提出使用深度学习技术检测茶叶病害，包括红锈病、Helopeltis和红蜘蛛螨三种病害，并评估了SSD MobileNet V2和Faster R-CNN ResNet50 V1两种目标检测模型，以及使用Mask R-CNN进行实例分割来计算叶片受损区域。


<details>
  <summary>Details</summary>
Motivation: 开发能够自动识别茶叶病害并量化受损区域的深度学习系统，以帮助茶农及时监测和管理茶树健康。

Method: 使用SSD MobileNet V2和Faster R-CNN ResNet50 V1进行目标检测，并使用Mask R-CNN进行实例分割，开发了自定义方法来计算叶片受损区域。

Result: Faster R-CNN ResNet50 V1表现更好，mAP为25%，而SSD MobileNet V2的mAP为20.9%。Mask R-CNN成功实现了对病害区域的量化分析。

Conclusion: Faster R-CNN ResNet50 V1在茶叶病害检测中表现优于SSD MobileNet V2，结合Mask R-CNN的实例分割方法能够有效识别和量化叶片受损区域。
...
TL;DR: Kineo是一个完全自动化的无标定多视角运动捕捉系统，使用未同步、未标定的消费级RGB相机，通过2D关键点同时标定相机和重建3D关键点，显著提升了无标定方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有无标定多视角运动捕捉方法需要精确的相机标定，限制了非专家用户的使用和野外捕获的可行性，且现有无标定方法计算成本高、重建精度低。

Method: 利用现成的2D关键点检测器，通过置信度驱动的时空关键点采样策略和基于图的全局优化，同时标定相机（包括Brown-Conrady畸变系数）并重建3D关键点和密集场景点云，引入成对重投影一致性评分来量化3D重建可靠性。

Result: 在EgoHumans和Human3.6M数据集上的评估显示，相比先前最先进方法，相机平移误差减少约83-85%，相机角度误差减少86-92%，世界平均关节位置误差减少83-91%。

Conclusion: Kineo在真实场景中高效，能够比序列持续时间更快地处理多视角序列，为无标定运动捕捉提供了实用且高效的解决方案。
...
TL;DR: 提出Decoupled MeanFlow方法，将预训练的流模型转换为流映射模型，无需架构修改，实现1-4步高质量生成，在ImageNet上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有的去噪生成模型（如扩散模型和流模型）需要大量去噪步骤，存在离散化误差问题。流映射可以缓解此问题并加速采样，但通常需要架构修改，限制了与预训练模型的兼容性。

Method: 提出解耦均值流方法，通过条件化扩散变换器的最后模块于后续时间步，将预训练的流模型直接转换为流映射模型，结合增强训练技术。

Result: 在ImageNet 256x256和512x512上，1步FID分别达到2.16和2.12，4步FID分别达到1.51和1.68，比现有方法有显著提升，推理速度提升100倍以上。

Conclusion: 通过将流模型转换为流映射模型的方法比从头训练流映射更有效，实现了高质量快速生成，且与预训练模型完全兼容。
...
TL;DR: 提出DisK-NeuralRTI方法，通过知识蒸馏降低NeuralRTI的计算成本，解决传统RTI方法在复杂反射场建模和神经网络方法在有限硬件上实时渲染的挑战。


<details>
  <summary>Details</summary>
Motivation: 传统RTI方法（如PTM和HSH）使用固定基函数和少量系数难以准确捕捉复杂反射场，导致伪影；而NeuralRTI虽然质量更好但计算成本高，无法在有限硬件上实现全分辨率实时渲染。

Method: 采用知识蒸馏（Knowledge Distillation）技术，从大型教师网络向小型学生网络传递知识，在保持渲染质量的同时显著降低计算复杂度。

Result: DisK-NeuralRTI在保持与NeuralRTI相当渲染质量的前提下，大幅降低了计算成本，使高分辨率实时渲染在有限硬件上成为可能。

Conclusion: 基于知识蒸馏的DisK-NeuralRTI方法成功解决了神经网络RTI方法的计算效率问题，为高质量实时表面分析提供了可行解决方案。
...
TL;DR: Latent Sketchpad是一个为多模态大语言模型添加内部视觉草稿纸的框架，通过视觉潜在表示支持生成式视觉思维，同时保持推理能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在需要视觉规划和想象的复杂场景中表现不佳，而人类使用草图作为视觉思维来开发和交流想法，因此需要为模型提供类似的视觉思考能力。

Method: 在现有MLLMs基础上集成视觉生成到自回归推理过程中，包含两个组件：上下文感知视觉头自回归生成视觉表示，预训练草图解码器将其渲染为可解释图像。

Result: 在MazePlanning数据集上的实验表明，Latent Sketchpad在保持骨干模型推理性能的同时，甚至在某些情况下表现更优，并能泛化到不同的前沿MLLMs。
...
TL;DR: OSWorld-MCP是首个全面且公平的基准测试，用于评估计算机使用代理在真实环境中的工具调用、GUI操作和决策能力。通过自动化代码生成管道创建158个高质量工具，评估显示MCP工具能提高任务成功率，但即使最强模型也只有36.3%的工具调用率，表明仍有改进空间。


<details>
  <summary>Details</summary>
Motivation: 过去评估主要关注GUI交互技能，而工具调用能力（如通过模型上下文协议MCP启用）被忽视。将集成工具调用的代理与仅评估GUI交互的代理进行比较本质上是不公平的。

Method: 设计新颖的自动化代码生成管道来创建工具，并结合现有工具的精选集合。通过严格的人工验证获得158个高质量工具（涵盖7个常见应用），每个工具都验证了正确功能、实际适用性和多功能性。

Result: 在OSWorld-MCP上对最先进的多模态代理进行广泛评估显示，MCP工具通常能提高任务成功率（例如OpenAI o3在15步时从8.3%提高到20.4%，Claude 4 Sonnet在50步时从40.1%提高到43.3%）。但即使最强模型也只有相对较低的工具调用率36.3%。
...
Conclusion: 该方法通过结合物理先验知识和深度学习的优势，成功提升了CBCT散射伪影校正的效果，为医学影像诊断提供了更准确的图像质量。
...
TL;DR: 提出了一种新颖的双分支卷积神经网络用于人脸伪造检测，结合空间域和频域的互补线索，通过通道注意力模块自适应融合特征，并使用统一的FSC损失函数增强分类性能。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的快速发展使得伪造人脸图像变得高度逼真，这对AI安全、数字媒体完整性和公众信任构成严重威胁，迫切需要鲁棒且可泛化的人脸伪造检测方法。

Method: 采用双分支CNN架构：RGB分支捕获语义信息，频率分支关注高频伪影；引入通道注意力模块自适应融合异构特征；设计FSC损失函数结合焦点损失、监督对比损失和频率中心边界损失。

Result: 在DiFF基准测试中表现出色，涵盖文本到图像、图像到图像、人脸交换和人脸编辑四种伪造方法，性能优于平均人类准确率。

Conclusion: 该方法在人脸伪造检测中表现出高效性，有望为保护AI生态系统免受视觉伪造攻击做出重要贡献。
...
TL;DR: PathoGaze1.0是一个全面的病理学家行为数据集，记录了癌症诊断过程中视觉搜索和决策的动态过程，包含眼动追踪、鼠标交互等数据，用于分析诊断错误和一致性。


<details>
  <summary>Details</summary>
Motivation: 病理学家对千兆像素全切片图像的解释准确率仅约70%，且增加第二位病理学家并不能显著改善决策一致性，该领域缺乏足够的行为数据来解释诊断错误和不一致性。

Method: 开发了PathoGaze1.0数据集，通过PTAH应用基础测试平台收集了19位病理学家解读397张WSI的18.69小时数据，包括眼动追踪、鼠标交互、刺激追踪、视口导航和诊断决策数据。

Result: 总共记录了171,909次注视、263,320次扫视和1,867,362次鼠标交互事件，创建了一个全面的行为数据集。

Conclusion: 该数据集可用于改善病理学家和AI系统的训练，所有实验已在OSF预注册，完整数据集和分析代码已公开。
...
Method: 分析DiT模型中的MM-Attention机制，发现Query和Key共享仅与层相关的偏置向量。将该偏置解释为模型固有编辑行为，token与对应偏置的差值编码内容特定编辑信号。通过重新加权不同token的delta值来调节模型对输入图像相对于编辑指令的关注度。

Result: 实验表明GRAG仅需四行代码即可集成到现有编辑框架中，持续提升编辑质量。相比常用的Classifier-Free Guidance，GRAG能实现更平滑精确的编辑程度控制。

Conclusion: GRAG是一种简单有效的图像编辑控制方法，通过注意力机制重加权实现连续精细的编辑强度调节，显著改善编辑质量。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频过渡方法在处理大时间间隔或显著语义差异的视频片段时效果不佳，需要内容感知和视觉连贯的过渡解决方案

Method: 结合结构引导（线图和运动流）与生成合成，无需微调即可实现零样本视频过渡

Result: 在定量指标和用户研究中优于传统方法和生成基线，能够处理多样视频片段间的过渡

Conclusion: SAGE方法通过结构感知的生成视频过渡，有效解决了视频片段间大差异过渡的挑战
...
TL;DR: MIC-BEV是一个基于Transformer的鸟瞰图感知框架，用于基础设施多摄像头3D物体检测，支持可变数量的异构摄像头，在传感器退化条件下保持强鲁棒性。
...
Result: 在M2I合成数据集和真实世界RoScenes数据集上的广泛实验表明，MIC-BEV在3D物体检测方面达到了最先进的性能，在极端天气和传感器退化等挑战条件下保持鲁棒性。
...
TL;DR: 研究发现预训练的视觉变换器（ViTs）能够自然涌现出物体绑定能力，即识别哪些图像块属于同一物体的能力。这种能力在自监督学习模型中表现突出，但在ImageNet监督模型中较弱，表明这是通过特定预训练目标获得的能力。


<details>
  <summary>Details</summary>
Motivation: 探索视觉变换器是否自然具备物体绑定能力——将属于同一物体的图像块组合成整体表示的能力，这对于人类认知至关重要。

Method: 使用相似性探针从ViT各层的图像块嵌入中解码"IsSameObject"属性，分析不同预训练模型（DINO、MAE、CLIP、ImageNet监督）的物体绑定能力，并进行消融实验。

Result: 自监督ViTs能够以超过90%的准确率解码IsSameObject属性，而ImageNet监督模型表现明显较弱。IsSameObject编码在物体特征上的低维子空间中，并主动引导注意力机制。

Conclusion: ViTs自然涌现出物体绑定能力，挑战了认为ViTs缺乏物体绑定的观点，展示了符号知识如何在连接主义系统中自然出现。
...
<details>
  <summary>Details</summary>
Motivation: 现有将MoE应用于扩散变换器(DiTs)的尝试效果有限，主要原因是语言token和视觉token存在根本差异：语言token语义密集且变化显著，而视觉token具有空间冗余和功能异质性，阻碍了视觉MoE中的专家专业化。
...
Conclusion: 基于原型路由的相似性专家分配为融入显式语义指导提供了自然机制，这种指导对视觉MoE至关重要。ProMoE通过促进专家专业化，成功解决了视觉MoE中的挑战。
...
TL;DR: URSA是一个离散视频生成框架，通过全局迭代优化离散时空token，结合线性化度量路径和分辨率相关时间步偏移机制，实现了高效的高分辨率图像合成和长视频生成，性能媲美连续扩散方法。


<details>
  <summary>Details</summary>
Motivation: 连续空间视频生成发展迅速，而离散方法由于误差积累和长上下文不一致性问题而落后，需要重新审视离散生成建模来弥合这一差距。

Method: URSA将视频生成任务构建为离散时空token的迭代全局优化，核心设计包括线性化度量路径和分辨率相关时间步偏移机制，并采用异步时间微调策略统一多种任务。

Result: 在具有挑战性的视频和图像生成基准测试中，URSA持续优于现有离散方法，并达到与最先进连续扩散方法相当的性能。

Conclusion: URSA证明了离散生成模型在视频生成任务中的潜力，能够有效解决误差积累和长上下文一致性问题，为可扩展视频生成提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型虽然能够进行长序列生成，但无法利用未来信息来指导当前生成，在预定义相机轨迹的场景中容易导致与生成场景的碰撞，进而使自回归过程崩溃。

Method: GVS采样算法将机器人规划中的扩散缝合技术扩展到视频生成，利用现成的Diffusion Forcing训练的视频模型，结合全向引导技术同时考虑过去和未来信息，并提出了闭环机制保证长程一致性。

Result: GVS实现了稳定、无碰撞、帧间一致的相机引导视频生成，能够处理各种预定义相机路径，包括奥斯卡·路透斯瓦德的"不可能楼梯"等复杂场景。

Conclusion: 该方法成功解决了自回归视频扩散模型在相机引导生成中的局限性，为复杂相机轨迹下的视频生成提供了有效的解决方案。
...
### [72] [BLM$_1$: A Boundless Large Model for Cross-Space, Cross-Task, and Cross-Embodiment Learning](https://arxiv.org/abs/2510.24161)
...
TL;DR: BLM₁是一个统一的多模态空间基础模型，通过两阶段训练实现跨空间传输、跨任务学习和跨具身泛化，在数字和物理任务中均优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在数字-物理空间和具身代理间泛化能力差，VLAs缺乏高级推理能力，ELLMs局限于数字空间，需要统一模型实现跨空间和跨具身的无缝操作。

Method: 两阶段训练：第一阶段通过精选数字语料向MLLM注入具身知识；第二阶段通过意图桥接接口训练策略模块，提取MLLM高级语义指导控制，无需微调MLLM主干。

Result: 单个BLM₁实例在数字任务中提升约6%，在物理任务中提升约3%，优于MLLMs、ELLMs、VLAs和GMLMs四类模型。
...
TL;DR: Game-TARS是一个通用游戏智能体，使用统一可扩展的动作空间进行训练，基于人类对齐的键盘鼠标输入，在大规模异构领域进行持续预训练，在多个游戏基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有游戏智能体在异构领域（操作系统、网页、模拟游戏）中缺乏统一性和可扩展性的问题，开发一个能够跨领域持续学习并接近人类通用性的通用智能体。

Method: 采用基于人类键盘鼠标输入的统一动作空间，进行大规模持续预训练（超过500B tokens），使用衰减持续损失减少因果混淆，以及高效的稀疏思维策略平衡推理深度和推理成本。

Result: 在开放世界Minecraft任务中成功率比之前最优模型提高约2倍，在未见过的网页3D游戏中接近人类新手水平，在FPS基准测试中超越GPT-5、Gemini-2.5-Pro和Claude-4-Sonnet。

Conclusion: 简单可扩展的动作表示与大规模预训练相结合，为构建具有广泛计算机使用能力的通用智能体提供了一条有前景的路径。
...
TL;DR: 本文探讨人工智能在科学问题解决中的作用，重点关注其对学科创造力的影响。通过区分创造性方法和创造性产品，引入学科创造力的概念，并以数学领域两个案例说明AI可能取代而非扩展学科创造力。


<details>
  <summary>Details</summary>
Motivation: 研究AI在科学问题解决中的角色，特别关注其对学科创造力的潜在影响，因为AI可能改变科学追求的价值。

Method: 基于创造力哲学理论，区分创造性方法和创造性产品，提出学科创造力的概念，并通过数学领域的两个案例研究进行分析。

Result: 研究发现计算可以扩展学科创造力，但某些AI方法可能取代学科创造力，这种取代有可能改变（甚至削弱）科学追求的价值。

Conclusion: AI在科学问题解决中具有双重作用：既能扩展学科创造力，也可能通过某些方法取代它，这对科学追求的价值产生重要影响。
...
TL;DR: 本文提出了多环境POMDP（ME-POMDP）及其推广形式AB-POMDP，用于处理具有离散模型不确定性的POMDP问题，并开发了精确和近似算法来计算鲁棒策略。


<details>
  <summary>Details</summary>
Motivation: 当多个领域专家对问题建模存在分歧时，需要一种能够处理模型不确定性的框架。ME-POMDP扩展了标准POMDP，允许过渡、观察和奖励模型存在不确定性，目标是找到在所有可能POMDP中都能获得最坏情况奖励最大化的单一策略。

Method: 1）将ME-POMDP推广为具有初始信念集合的AB-POMDP；2）证明任意ME-POMDP可以简化为仅在过渡和奖励函数或仅在观察和奖励函数上变化的等价形式；3）开发了精确和近似（基于点）的算法来计算鲁棒策略。

Result: 成功将标准POMDP基准扩展到多环境设置，并能够计算相应的策略。算法能够处理ME-POMDP和AB-POMDP的鲁棒策略求解问题。

Conclusion: ME-POMDP和AB-POMDP为处理模型不确定性提供了有效框架，所提出的算法能够计算鲁棒策略，适用于多环境POMDP问题。
...
TL;DR: 提出了一种基于测试时调优的框架，直接从串联质谱和分子式进行端到端从头分子结构生成，无需数据库匹配或中间步骤，在两个基准测试中显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前串联质谱分析方法依赖数据库匹配或多步骤流程，难以识别数据库中未收录的化合物，需要一种能够直接从质谱数据生成分子结构的端到端方法。

Method: 利用测试时调优增强预训练transformer模型的学习能力，直接从串联质谱和分子式进行端到端分子结构生成，无需人工标注或中间步骤。

Result: 在NPLIB1和MassSpecGym两个基准测试中分别超越DiffMS方法100%和20%，测试时调优相比传统微调在MassSpecGym上带来62%的相对性能提升。

Conclusion: 该框架能够动态适应新质谱数据，即使预测偏离真实值，生成的分子候选结构仍保持结构准确性，为人工解释和可靠识别提供有价值指导。
...
TL;DR: 研究开发了一个AI系统来生成具有美学吸引力、新颖性和反直觉解决方案的国际象棋谜题，并邀请三位国际象棋专家评估其创造力。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的快速发展，需要探究其是否能够产生真正具有创造性和新颖性的输出，特别是在国际象棋谜题领域。

Method: 开发了一个专门设计用于生成具有美学吸引力、新颖性、反直觉和独特解决方案的国际象棋谜题AI系统，并通过专家评估来验证其创造力。

Result: 三位世界知名的国际象棋专家（国际象棋排局国际大师Amatzia Avni、特级大师Jonathan Levitt和Matthew Sadler）对AI生成的谜题进行了评估，并选择了他们最喜欢的谜题，解释了其吸引力所在。

Conclusion: 该研究展示了AI系统能够生成被专家认可为具有创造力、挑战性和美学设计的国际象棋谜题，证明了生成式AI在创造性任务中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 尽管基础模型在计算机视觉和语言处理领域取得了革命性突破，但在计算病理学中的快速应用并未带来预期的突破，反而暴露了根本性弱点。

Method: 本文通过系统性评估，分析了病理学基础模型的七个相互关联的根本问题：生物复杂性、无效的自监督、过度泛化、过度架构复杂性、缺乏领域特定创新、数据不足以及与组织块大小相关的基本设计缺陷。

Result: 研究发现当前病理学基础模型在诊断准确性、鲁棒性、几何稳定性等方面表现不佳，存在严重的安全漏洞，且计算需求巨大。

Conclusion: 当前病理学基础模型在概念上与组织形态学的本质存在错位，需要从根本上重新思考基础模型在病理学中的应用范式。
...
TL;DR: ReCAP是一个用于大语言模型的递归上下文感知推理规划框架，通过计划分解、结构化父计划重注入和内存高效执行机制，显著提升长时程任务的推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有序列提示方法存在上下文漂移、目标信息丢失和循环失败问题，而分层提示方法则削弱跨层连续性或产生高运行时开销。

Method: 结合三个关键机制：(i) 计划分解：生成完整子任务列表，执行第一项并优化剩余；(ii) 结构化父计划重注入：在递归返回时保持多级上下文一致性；(iii) 内存高效执行：限制活动提示规模，使成本随任务深度线性增长。

Result: 在多个长时程推理基准测试中显著提升子目标对齐和成功率，在同步Robotouille上获得32%增益，在异步Robotouille上获得29%改进。

Conclusion: ReCAP框架能有效对齐高层目标与低层动作，减少冗余提示，保持跨递归的连贯上下文更新，为长时程任务提供高效解决方案。
...
### [80] [Decentralized Multi-Agent Goal Assignment for Path Planning using Large Language Models](https://arxiv.org/abs/2510.23824)
*Murad Ismayilov,Edwin Meriaux,Shuo Wen,Gregory Dudek*

Main category: cs.AI

TL;DR: 该论文研究多智能体路径规划中的去中心化目标分配问题，比较了贪心启发式、最优分配和基于大语言模型（LLM）的智能体在完全可观测网格环境中的表现。
...
Method: 智能体基于环境的结构化表示（包括网格可视化和场景数据）独立生成目标排名偏好，然后交换目标排名，通过固定的确定性冲突解决规则（如智能体索引排序）确定分配，无需协商或迭代协调。

Result: 当提供精心设计的提示和相关定量信息时，基于LLM的智能体可以实现接近最优的完成时间，并始终优于传统启发式方法。
...
TL;DR: IBM开发了通用智能体CUGA，采用分层规划-执行架构，在学术基准和业务流程外包人才招聘领域试点中表现优异，展示了通用智能体在企业级应用中的潜力。


<details>
  <summary>Details</summary>
Motivation: 解决企业在部署智能体系统时面临的框架碎片化、开发缓慢和缺乏标准化评估等问题，推动通用智能体从研究原型向企业级应用转化。

Method: 采用分层规划-执行架构，具有强大的分析基础，在AppWorld和WebArena基准测试中达到最先进性能，并在业务流程外包人才招聘领域进行试点评估。

Result: CUGA在BPO-TA基准测试中接近专用智能体的准确率，同时显示出减少开发时间和成本的潜力，为通用智能体在企业级应用提供了早期证据。

Conclusion: 通用智能体在企业级应用中具有潜力，但需要满足可扩展性、可审计性、安全性和治理等企业要求，论文为研究级架构向企业级系统转化提供了技术路线图。
...
TL;DR: 使用强化学习框架生成更具创造性、反直觉的象棋谜题，通过基于象棋引擎搜索统计的新型奖励机制，显著提升了谜题的反直觉性和多样性。


<details>
  <summary>Details</summary>
Motivation: 生成式AI在生成真正创造性、美学性和反直觉输出方面仍面临挑战，特别是在象棋谜题领域需要突破现有方法的局限。

Method: 首先对生成式AI架构进行基准测试，然后引入基于象棋引擎搜索统计的新型奖励机制的强化学习框架，增强谜题的独特性、反直觉性、多样性和真实性。

Result: 强化学习方法将反直觉谜题生成率从0.22%提升至2.5%，超过现有数据集(2.1%)和最佳Lichess训练模型(0.4%)。生成的谜题在人类专家评估中被评为更具创造性、趣味性和反直觉性。

Conclusion: 该方法成功生成了高质量的AI象棋谜题，获得了三位世界知名专家的认可，证明强化学习在创造性内容生成方面的潜力。
...
TL;DR: 该研究比较了四种预测模型（线性、物理建模、LSTM、HAM）和三种控制策略（MPC、RL、LLM）在数字孪生系统中的应用，发现HAM模型在精度、泛化性和计算效率方面表现最均衡，MPC控制器性能稳健，RL适应性强，LLM控制器提供灵活的人机交互。
...
Method: 使用微型温室作为测试平台，开发四种预测模型（线性、PBM、LSTM、HAM）和三种控制策略（MPC、RL、LLM），在插值和外推场景下进行对比评估。

Result: HAM模型在精度、泛化性和计算效率方面表现最均衡；LSTM精度高但资源消耗大；MPC控制器性能稳健可预测；RL控制器适应性强；LLM控制器结合预测工具可实现灵活的人机交互。

Conclusion: HAM模型在数字孪生系统建模中提供最佳平衡性能，不同控制器各有优势：MPC稳健、RL适应性强、LLM交互灵活，为实际应用提供了多样化的选择方案。
...
### [84] [Agentic AI Security: Threats, Defenses, Evaluation, and Open Challenges](https://arxiv.org/abs/2510.23883)
*Shrestha Datta,Shahriar Kabir Nahin,Anshuman Chhabra,Prasant Mohapatra*

Main category: cs.AI

TL;DR: 该论文调查了基于大语言模型的智能AI系统带来的新型安全风险，提出了威胁分类法，回顾了评估方法和防御策略。


<details>
  <summary>Details</summary>
Motivation: 智能AI系统在自动化应用中展现出强大能力，但其自主执行任务的能力在网络、软件和物理环境中产生了新的放大安全风险，这些风险不同于传统AI安全和软件安全。

Method: 采用调查分析方法，构建了智能AI特有的威胁分类法，回顾了最近的基准测试和评估方法，并从技术和治理角度讨论了防御策略。

Result: 系统性地识别了智能AI系统的安全威胁类型，综合了当前研究成果，为安全设计提供了理论框架。

Conclusion: 智能AI系统需要专门的安全考虑，论文为开发安全设计的智能系统提供了重要参考，并指出了未来研究面临的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有训练算法（SFT、PPO、GRPO）在未见推理任务上泛化能力不足，且过度依赖有偏的奖励模型，需要更可靠的推理训练方法。

Method: 将LVLMs推理重新定义为后验推理，采用摊销变分推理和多样性强化学习，引入稀疏奖励函数促进多样化高似然潜在CoT，并用贝叶斯推理扩展策略替代昂贵搜索方法。

Result: 在七个推理基准测试中，该方法提升了最先进LVLMs的效果、泛化性和可解释性。

Conclusion: 提出的变分推理框架能够有效改进LVLMs的推理能力，克服传统训练方法的局限性，提供更可靠和可解释的思维链推理。
...
### [86] [Decentralized Causal Discovery using Judo Calculus](https://arxiv.org/abs/2510.23942)
*Sridhar Mahadevan*

Main category: cs.AI

TL;DR: 提出基于judo演算的直觉主义去中心化因果发现框架，使用j稳定因果推断和j-do演算在层拓扑中形式化处理因果关系的上下文依赖性。
...
Method: 结合judo演算与标准得分基、约束基和梯度基因果发现方法，使用Lawvere-Tierney模态算子j选择相关情境，在情境覆盖上证明因果主张。

Result: 实验结果显示层理论因果发现的去中心化特性带来计算效率提升，并在合成和真实数据集上表现优于经典因果发现方法。

Conclusion: judo演算为处理上下文依赖的因果发现提供了形式化框架，在计算效率和性能方面均有优势。
...
<details>
  <summary>Details</summary>
Motivation: 传统LLM对齐方法对异质性人类偏好很脆弱，使用朴素概率模型拟合成对比较数据会产生不一致的群体平均效用估计。

Method: 提出sign estimator方法，在聚合步骤中用二元分类损失替换交叉熵损失，在温和假设下实现一致的有序对齐。

Result: 在LLM对齐的数字孪生模拟中，sign estimator显著减少了偏好扭曲，将估计误差降低近35%，与真实群体偏好的分歧从12%降至8%。

Conclusion: 该方法优于显式建模用户异质性的面板数据启发式方法，同时保持了现有LLM对齐管道的实现简单性。
...
<details>
  <summary>Details</summary>
Motivation: 预测破坏性事件前个体移动模式的变化具有挑战性，因为缺乏衡量个体异质性社会基础设施韧性的方法，传统特征在规模上受限，且个体移动与空间上下文的复杂交互关系未被充分捕捉。

Method: 构建条件深度学习模型，整合个体的社会基础设施韧性(SIR)和局部空间上下文，利用大规模稀疏个体级数据来捕捉个体移动模式与空间环境的复杂关系。

Result: 实验表明，整合个体的SIR和空间上下文能够增强模型预测事件后个体移动模式的能力。条件模型能够捕捉到具有相似事件前移动模式但SIR不同的个体在移动模式上的差异变化。

Conclusion: 该研究证明了将个体社会基础设施韧性和空间上下文整合到深度学习模型中的有效性，能够更好地预测破坏性事件后个体移动模式的变化。
...
TL;DR: 利用大型语言模型发现了两种新的启发式算法EDDC和MDDC，用于解决单机总延迟问题，在大型实例中表现优于传统启发式方法。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在调度和组合优化问题中的潜力，特别是在传统精确方法计算不可行的大规模实例中，开发可扩展的高性能启发式算法。

Method: 开发了两种基于LLM发现的启发式算法：EDDC（基于最早截止日期规则）和MDDC（基于修正截止日期规则），并与混合整数规划和动态规划等精确方法进行基准测试。

Result: 对于超过100个作业的实例，EDDC改进了经典EDD规则和其他常用算法；MDDC在大型复杂实例中始终优于传统启发式方法，并与精确方法保持竞争力。
...
TL;DR: OneCast是一个结构化、模块化的跨域时间序列预测框架，通过分解时间序列为季节性和趋势组件，分别使用轻量级投影模块和离散扩散机制进行建模，在多个领域实验中表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决跨域时间序列预测中由于领域特定趋势变化和不一致周期性模式导致的泛化能力不足问题，现有方法将时间序列视为未分化的序列而忽略了其内在结构组件。

Method: 将时间序列分解为季节性和趋势组件：季节性组件通过轻量级投影模块使用可解释基函数重建周期模式；趋势组件通过语义感知分词器编码为离散标记，使用掩码离散扩散机制进行推断。

Result: 在八个领域的广泛实验中，OneCast大多优于最先进的基线方法。

Conclusion: 通过显式解耦时间序列的结构组件并采用专门建模方法，OneCast能够有效捕捉季节性模式并跟踪领域特定趋势，实现更好的跨域泛化性能。
...
Method: 采用模块化架构，包含路由器、日志识别器、日志解析器和搜索工具，结合聚类方法和LLM技术，解决LLM的上下文窗口限制和结构化文本处理能力不足的问题。

Result: 在四个不同领域日志和多种任务上的评估显示，相比ChatGPT、ChatPDF和NotebookLM等先进LLM聊天机器人，性能显著提升39%-68%，鲁棒性增强，使用ROUGE-1分数的四分位距减少93%。

Conclusion: 该框架通过模块化设计增强LLM在结构化文本分析方面的能力，提高准确性和鲁棒性，为网络安全专家和非技术用户提供了有价值的资源。
...
Result: 随机森林模型表现最佳，RMSE分别为0.0046（中等间距）、0.0016（长间距）和0.0025（超长间距）；经典模型中CACC表现最好，RMSE为2.67（长间距）。
...
TL;DR: HistoLens是一个透明的AI病理学助手，允许病理学家用自然语言提问，提供结构化报告和可视化证据，帮助医生更快更自信地做出诊断。


<details>
  <summary>Details</summary>
Motivation: 为了让医生真正信任人工智能，需要使其推理过程透明化，就像咨询同事一样，而不是黑盒系统。

Method: 创建HistoLens系统，将自然语言问题转化为AI引擎的精确查询，提供结构化报告和热图可视化证据，并训练AI专注于患者组织而忽略背景噪声。

Result: 开发了一个工作流程，使病理学家保持专家主导地位，使用可信赖的AI助手验证他们的见解，实现更快更自信的诊断。
...
TL;DR: OpsAgent是一个轻量级、自演化的多代理系统，用于云系统事故管理，通过免训练数据处理和多代理协作框架实现自动化诊断，并在OPENRCA基准测试中表现出最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 传统手动事故管理在处理大规模异构可观测数据时劳动密集且容易出错，现有自动化方法存在泛化能力差、可解释性有限和部署成本高等问题，阻碍了实际应用。

Method: 使用免训练数据处理器将异构可观测数据转换为结构化文本描述，采用多代理协作框架实现透明可审计的诊断推理，并引入双自演化机制整合内部模型更新和外部经验积累。

Result: 在OPENRCA基准测试中表现出最先进的性能，证明OpsAgent具有泛化性、可解释性、成本效益和自演化能力。
...
TL;DR: 提出了一个自动化框架，用于从半结构化知识源生成高难度的多跳问答训练数据集，解决人工标注成本高且难以规模化的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多跳问答评估数据集稀缺且不适合监督微调或强化学习训练，而人工标注非平凡可检索问题成本高昂且难以扩展，形成了训练高能力检索推理代理的关键数据瓶颈。

Method: 系统包含三个主要步骤：(1) 通过自然语言推理关系类型化和多样性感知扩展生成多样化逻辑标记的证据簇；(2) 应用逆向问题构建来组合间接线索；(3) 采用多模型共识过滤与结构化约束分解和基于证据匹配的两步评估流程确保质量。

Result: 该框架能够规模化生成复杂、检索抵抗但可验证的问题，既适合SFT/RL训练也适合挑战性评估，大幅减少人工标注工作同时保持强评估基准的难度特征。

Conclusion: 该自动化框架有效解决了多跳问答训练数据稀缺的问题，为训练高能力检索推理代理提供了可扩展的解决方案。
...
TL;DR: UniPlanner是一个用于自动驾驶决策的多数据集集成规划框架，通过三个创新组件实现跨数据集统一学习，提升轨迹规划的安全性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法局限于单数据集训练，限制了规划鲁棒性。研究发现不同数据集中车辆轨迹分布和历史-未来相关性具有一致性，这为多数据集集成提供了基础。

Method: 1. 历史-未来轨迹字典网络(HFTDN)：聚合多数据集轨迹对，基于历史轨迹相似性检索相关未来轨迹生成规划指导
2. 无梯度轨迹映射器(GFTM)：从多数据集学习鲁棒的历史-未来相关性，将历史轨迹转换为通用规划先验
3. 稀疏到稠密(S2D)范式：训练时自适应丢弃部分先验确保鲁棒性，推理时充分利用所有先验最大化性能

Result: UniPlanner实现了跨数据集统一学习，能够生成更安全、高效的轨迹规划，解决了单数据集训练的局限性。

Conclusion: 该框架通过多数据集集成显著提升了自动驾驶规划系统的鲁棒性和性能，为未来更复杂的真实世界应用奠定了基础。
...
TL;DR: 提出了MGA（Memory-Driven GUI Agent），通过“先观察后决策”原则重构GUI交互，解决了现有方法依赖历史轨迹和局部探索偏差的问题，在多个基准测试中表现出更强的鲁棒性、泛化性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有GUI代理方法存在两个主要问题：依赖历史轨迹导致错误传播放大，以及“先决策后观察”机制忽视关键界面线索。需要一种更鲁棒和泛化的GUI交互方法。
...
Conclusion: MGA通过重构GUI交互为“先观察后决策”范式，有效解决了现有方法的局限性，为GUI代理开发提供了更可靠和高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有MCP研究覆盖服务器数量有限，依赖昂贵的人工筛选，缺乏训练支持，阻碍了在现实世界中的部署。

Method: 通过自动化的网络代理驱动管道，从1166个服务器和11536个工具中收集和筛选数据，生成68733个高质量指令-函数调用对和6439个轨迹。
...
TL;DR: 本文提出了几种替代随机策略的抽象内策略，用于解决MCTS中当多个动作属于同一抽象节点时的平局问题，多数策略在环境和参数设置中表现优于随机策略。


<details>
  <summary>Details</summary>
Motivation: MCTS的样本效率问题可以通过状态/动作抽象来解决，但现有方法如pruned OGA未注意到同一抽象节点中多个动作具有相同UCB值的情况，只能使用随机平局规则。

Method: 提出并实证评估了多种抽象内策略，作为随机平局规则的替代方案。

Result: 多个提出的策略在大多数环境和参数设置中表现优于随机策略。

Conclusion: 抽象内策略的选择对MCTS性能有重要影响，提出的替代策略能有效提升性能。
...
TL;DR: 提出了一种基于LLM内部行为的自指示方法，通过计算输入问题与输出推理路径之间的相关矩阵秩来判断推理正确性，无需外部资源即可有效验证LLM输出。


<details>
  <summary>Details</summary>
Motivation: 现有验证方法依赖外部资源（如训练验证器或复杂提示），导致高计算开销且仅适用于特定领域。本文探索LLM内部行为是否已隐含其推理路径的可信度。

Method: 发现输入问题与输出推理路径之间的相关矩阵秩是推理正确性的稳健指标。基于此设计自指示方法，通过计算相关矩阵秩来重新加权候选推理路径。

Result: 在多个不同规模和家族的LLM上实验，自指示方法在区分正确与错误推理路径方面达到75%以上准确率，并在三个推理基准上将准确率提高超过8%。

Conclusion: LLM内部行为确实隐含推理路径的可信度信息，自指示方法提供了一种简单、即插即用的验证方案，显著优于其他投票和验证方法，且计算开销极低。
...
TL;DR: 本文提出了一个用于判断预测的多智能体框架，通过不同智能体对主张真实性产生分歧并收集正反证据，使用定量双极论证框架(QBAFs)表示，实验表明多智能体结合证据能提高预测准确性。


<details>
  <summary>Details</summary>
Motivation: 将判断预测视为主张验证任务，需要评估未来事件的可能性。传统方法缺乏多视角证据收集和解释性验证过程。

Method: 提出多智能体框架，使用三种LLM驱动的智能体：ArgLLM(生成和评估QBAFs)、RbAM(基于关系从外部源挖掘论证)、RAG-ArgLLM(结合检索增强生成的ArgLLM)，在标准判断预测数据集上进行实验。

Result: 实验表明，结合多个智能体的证据可以改善预测准确性，特别是在三个智能体的情况下，同时为主张验证提供可解释的证据组合。

Conclusion: 多智能体框架能够有效结合不同证据源，提高判断预测的准确性和可解释性，为未来事件的可能性评估提供了新方法。
...
TL;DR: 生成式大语言模型在传播研究内容分析中展现出超越人工编码的潜力，但面临七大挑战，本文提出最佳实践指南以提升分析质量。


<details>
  <summary>Details</summary>
Motivation: gLLMs在传播研究内容分析中具有时间成本低、能理解隐含含义等优势，但其方法学整合仍不成熟，需要解决影响结果质量的七大挑战。

Method: 综合新兴研究，提出包含代码本开发、提示工程、模型选择、参数调优、迭代优化、可靠性验证和性能提升的七步最佳实践指南。

Result: 建立了系统化的gLLM辅助定量内容分析方法框架，使该技术更易被传播研究者采用。

Conclusion: 通过解决七大挑战并提供实践指南，可以促进gLLM在传播研究中的规范应用，确保分析结果符合效度、信度、可复现性和研究伦理等质量标准。
...
Result: 在9个不同特征的数据集上评估，使用DeepSeek-V3和GPT-4o作为后端，VDSAgents持续优于AutoKaggle和DataInterpreter等最先进系统。
...
TL;DR: 本文提出了“类脑空间”概念，这是一个统一的几何空间，可以将任何AI模型的内在空间注意力拓扑组织映射到人类功能脑网络上进行比较，不受输入模态、任务或感官域的限制。
...
Method: 通过分析151个基于Transformer的模型，将模型的内在空间注意力拓扑组织映射到规范的人类功能脑网络上，构建统一的类脑空间。

Result: 发现了一个连续的弧形几何结构，反映了类脑程度的逐渐增加；不同模型在该几何中表现出与类脑程度相关的不同分布模式。
...
TL;DR: 提出多智能体生态系统用于N-of-1决策支持，通过协调多个专业智能体来为个体患者提供个性化医疗决策，解决传统AI医疗系统对罕见病例和边缘人群的不足。


<details>
  <summary>Details</summary>
Motivation: 传统AI医疗系统为平均患者设计，在罕见变异、多病共存和代表性不足人群上表现不佳，这种平均患者谬误损害了公平性和信任度。

Method: 构建多智能体生态系统，智能体按器官系统、患者群体和分析模式聚类，共享模型库和证据合成工具，通过协调层整合结果并评估可靠性、不确定性和数据密度。

Result: 系统为临床医生提供决策支持包，包括置信区间限定的风险估计、异常值标记和相关证据链接，验证重点从群体平均转向个体可靠性。
...
TL;DR: Policy Cards是一种机器可读的部署层标准，用于表达AI代理的操作、监管和伦理约束，使其能在运行时遵循要求。


<details>
  <summary>Details</summary>
Motivation: 为AI代理提供可验证的合规性机制，将高层治理与工程实践相结合，实现大规模的可问责自主性。

Method: 定义规范层，编码允许/拒绝规则、义务、证据要求和与NIST AI RMF、ISO/IEC 42001、欧盟AI法案等保证框架的映射关系。

Result: 每个Policy Card可自动验证、版本控制，并链接到运行时执行或持续审计管道，为多代理生态系统中的分布式保证奠定基础。

Conclusion: Policy Cards为自主代理提供了实用的合规机制，是实现可问责自主性的重要工具。
...
TL;DR: Orion是一个高效推理框架，通过依赖感知查询分解和逻辑并行内容扩展，解决了LLM在实时Web应用中的效率与质量平衡问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM推理存在计算效率低和推理策略僵化的问题，无法同时满足Web服务对高质量推理和低延迟高吞吐的要求。

Method: 将查询推理分解为关键点生成和内容并行扩展两个协同阶段，并引入流水线调度机制实现跨查询并行处理。

Result: 实验显示Orion相比基线方法，token生成速度提升4.33倍，答案延迟降低3.42倍，推理质量提升18.75%。

Conclusion: Orion通过显式建模关键点间依赖关系，成功实现了LLM推理效率与质量的双重提升。
...
Result: 相比现有通用基准，APTBench能更准确地预测模型在下游作为智能体的表现，同时比后训练的全规模端到端评估更轻量、成本效益更高。

Conclusion: APTBench填补了预训练阶段智能体能力评估的空白，为模型训练提供了更有效的指导工具。
...
TL;DR: 提出了MobileRisk-Live动态沙盒环境和OS-Sentinel混合安全检测框架，用于检测移动代理在数字环境中的安全风险，相比现有方法有10%-30%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 基于视觉语言模型的计算机代理在移动平台等数字环境中展现出类人能力，但这些代理存在系统破坏和隐私泄露等不安全操作风险，在复杂移动环境中的安全检测仍是一个重大挑战。

Method: 开发了MobileRisk-Live动态沙盒环境与安全检测基准，提出了OS-Sentinel混合安全检测框架，结合形式验证器检测系统级违规和基于VLM的上下文评估器评估上下文风险与代理行为。
...
### [110] [Human-Level Reasoning: A Comparative Study of Large Language Models on Logical and Abstract Reasoning](https://arxiv.org/abs/2510.24435)
*Benjamin Grando Moreira*

Main category: cs.AI

TL;DR: 本研究比较了多个大型语言模型在逻辑和抽象推理能力方面的表现，并与人类表现进行基准测试，揭示了模型在演绎推理方面的不足。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型的推理能力对人工智能发展至关重要，这超越了单纯的语言任务表现，涉及模型是否真正理解信息、进行推理并以逻辑有效的方式得出结论。
...
Conclusion: 大型语言模型在推理能力方面仍有待改进，特别是在演绎推理领域存在明显短板。
...
### [111] [Law in Silico: Simulating Legal Society with LLM-Based Agents](https://arxiv.org/abs/2510.24442)
*Yiding Wang,Yuxuan Chen,Fanxu Meng,Xifan Chen,Xiaolei Yang,Muhan Zhang*

Main category: cs.AI

TL;DR: 提出了Law in Silico框架，使用基于LLM的智能体模拟法律社会，验证了其能复现宏观犯罪趋势并揭示法律系统对弱势群体权利保护的重要性。


<details>
  <summary>Details</summary>
Motivation: 由于现实法律实验成本高且难以实施，需要利用AI系统模拟法律社会来验证和发展法律理论，支持法律管理。LLM具备世界知识和角色扮演能力，适合作为法律社会模拟的基础。

Method: 引入Law in Silico框架，基于LLM的智能体模拟法律场景，包含个体决策和立法、裁决、执法等制度机制。

Result: 实验显示LLM智能体能够复现宏观犯罪趋势，提供与现实观察一致的见解；微观模拟表明功能良好、透明且适应性强的法律系统能更好保护弱势个体权利。

Conclusion: LLM能够有效模拟法律系统，为法律理论验证和系统改进提供可行工具，同时强调了优化法律机制对保护弱势群体的重要性。
...
### [112] [Affordance Representation and Recognition for Autonomous Agents](https://arxiv.org/abs/2510.24459)
*Habtom Kahsay Gidey,Niklas Huber,Alexander Lenz,Alois Knoll*

Main category: cs.AI

TL;DR: 提出两种架构模式：DOM转换模式用于简化网页DOM结构，超媒体功能识别模式用于动态发现和集成Web服务能力，共同构建软件代理的世界模型。
...
### [113] [Adaptive Surrogate Gradients for Sequential Reinforcement Learning in Spiking Neural Networks](https://arxiv.org/abs/2510.24461)
*Korneel Van den Berghe,Stein Stroobants,Vijay Janapa Reddi,G. C. H. E. de Croon*

Main category: cs.AI

TL;DR: 该论文提出了一种改进SNN训练的方法，通过分析替代梯度斜率设置和引入引导策略，显著提升了强化学习中SNN的性能，在真实无人机控制任务中取得了400分的平均回报，远超现有方法的-200分。


<details>
  <summary>Details</summary>
Motivation: 解决SNN在复杂控制任务中面临的两个关键挑战：非可微脉冲神经元需要替代梯度但优化特性不明确，以及SNN的状态动态需要序列训练但在强化学习中早期训练序列长度有限阻碍网络度过预热期。

Method: 系统分析替代梯度斜率设置，发现较浅斜率增加深层梯度但降低与真实梯度对齐；提出利用特权引导策略引导学习过程的新训练方法，结合自适应斜率调度。

Result: 在强化学习设置中，较浅斜率或调度斜率使训练和最终部署性能提升2.1倍；在真实无人机位置控制任务中实现400分平均回报，显著优于行为克隆和TD3BC等现有方法。

Conclusion: 这项工作推进了对SNN中替代梯度学习的理论理解，并为神经形态控制器的实际训练方法提供了验证，在真实机器人系统中展示了优越性能。
...
Method: 两阶段管道：1) 利用跨任务示例提示LLM伪标记少量目标实例；2) 引入图标签传播方法将标签信息扩展到剩余目标示例，无需额外LLM查询。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI在医疗领域具有巨大潜力，但需要深入理解医疗任务和可实现的范围，因此需要建立可持续的数据生态系统来支持AI系统的设计和部署。

Method: 重新定位数据生命周期，以医疗数据生态系统为生成式医疗系统的基础，通过语义向量搜索和上下文查询等高效数据处理管道，支持上游模型组件和下游临床应用。
...
TL;DR: FunReason-MT是一个用于真实世界多轮工具使用的数据合成框架，通过环境-API图交互、高级工具查询合成和引导迭代链来解决多轮函数调用数据的复杂性挑战。


<details>
  <summary>Details</summary>
Motivation: 现有数据合成方法（如随机环境采样或多智能体角色扮演）不足以在真实世界环境中生成高质量数据，面临目标模型训练、工具架构隔离和多轮逻辑依赖三大挑战。

Method: 采用环境-API图交互收集多样化高质量轨迹，高级工具查询合成简化困难查询构建，引导迭代链生成复杂思维链。

Result: 在Berkeley函数调用排行榜上，基于FunReason-MT生成数据训练的4B模型在同类规模模型中达到最先进性能，优于大多数闭源模型。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习和深度学习的发展，作物病害管理从手工特征提取发展到大规模自动特征学习。基础模型能够整合视觉和文本数据，解释症状文本，推理症状与管理的关系，支持种植者和教育者的交互问答。

Method: 通过文献综述方法，筛选了约40篇相关论文，重点关注大型语言模型和视觉语言模型在精准病害管理中的应用，包括自适应学习、强化学习和数字孪生框架。

Result: 主要发现：(a)基础模型在2023-24年文献激增；(b)视觉语言模型发展快于语言模型，发表量增加5-10倍；(c)强化学习和自适应学习在智能喷洒中仍处于早期阶段；(d)数字孪生与强化学习可虚拟模拟精准喷洒；(e)解决模拟到现实的差距对实际部署至关重要；(f)人机协作仍然有限；(g)多模态基础模型与实时反馈将推动下一代精准病害管理。

Conclusion: 基础模型在作物精准病害管理中展现出巨大潜力，特别是视觉语言模型和多模态方法。未来需要解决模拟到现实的差距，加强人机协作，开发具有实时反馈的多模态基础模型来推动精准病害管理的发展。
...
TL;DR: OrchDAG是一个合成数据生成管道，将工具执行建模为具有可控复杂度的有向无环图(DAGs)，用于基准测试模型性能并提出基于图的奖励来增强RLVR训练。
...
Method: 引入OrchDAG合成数据生成管道，将工具执行建模为DAGs，并提出基于图的奖励结合GRPO风格算法进行训练。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法未能充分利用工具间依赖关系与领域知识的结合，限制了示例工件生成的质量和效果。

Method: 构建工具知识图谱和文档知识图谱，采用深度-稀疏集成策略将工具结构依赖与程序知识对齐，生成示例计划。
...
### [120] [MSRANetV2: An Explainable Deep Learning Architecture for Multi-class Classification of Colorectal Histopathological Images](https://arxiv.org/abs/2510.24136)
*Ovi Sarkar,Md Shafiuzzaman,Md. Faysal Ahamed,Golam Mahmud,Muhammad E. H. Chowdhury*

Main category: eess.IV

TL;DR: 提出了MSRANetV2卷积神经网络架构，专门用于结直肠组织图像分类，在公开数据集上取得了优异的分类性能。
...
Result: 在两个公开数据集上表现优异：CRC-VAL-HE-7K数据集上平均精度0.9884±0.0151，召回率0.9900±0.0151，F1分数0.9900±0.0145，AUC 0.9999±0.00006，测试准确率0.9905±0.0025；NCT-CRC-HE-100K数据集上各项指标也接近0.99。

Conclusion: MSRANetV2是一个可靠、可解释且高性能的结直肠组织分类架构模型，能够有效提升诊断精度和效率。
...
### [121] [An Enhanced Dual Transformer Contrastive Network for Multimodal Sentiment Analysis](https://arxiv.org/abs/2510.23617)
...
Method: BERT-ViT-EF结合BERT和ViT编码器进行早期融合；DTCN在此基础上增加Transformer层细化文本上下文，并使用对比学习对齐文本和图像表示
...
### [122] [Speeding Up MACE: Low-Precision Tricks for Equivarient Force Fields](https://arxiv.org/abs/2510.23621)
*Alexandre Benoit*
...
<details>
  <summary>Details</summary>
Motivation: 机器学习力场计算成本高，需要系统评估SO(3)-等变模型如MACE是否可以通过降低精度算术和GPU优化内核来降低成本而不损害物理保真度。

Method: 端到端和逐块分析MACE性能，比较e3nn和cuEquivariance后端，评估FP64/FP32/BF16/FP16设置，在推理、NVT/NPT水模拟和训练中测试可重现的稳态时序。

Result: cuEquivariance减少推理延迟约3倍，线性层使用BF16/FP16获得额外4倍加速，NVT/NPT MD中的能量和热力学观测值保持在运行间变异性内，但训练中使用半精度权重会降低力RMSE。

Conclusion: 融合等变内核和混合精度推理能显著加速最先进的力场，对下游MD影响可忽略。建议默认使用cuEquivariance和FP32，线性层启用BF16/FP16以获得最大吞吐量，训练保持FP32。
...
### [123] [Adversarially-Aware Architecture Design for Robust Medical AI Systems](https://arxiv.org/abs/2510.23622)
*Alyssa Gerhart,Balaji Iyangar*

Main category: cs.LG

TL;DR: 论文探讨了医疗AI系统中的对抗性攻击风险，通过在皮肤病数据集上的实验展示了攻击如何显著降低分类准确性，并评估了防御方法的有效性。
...
### [124] [DiNo and RanBu: Lightweight Predictions from Shallow Random Forests](https://arxiv.org/abs/2510.23624)
...
Method: DiNo通过观测对的最远共同祖先测量同源距离，RanBu对Breiman经典邻近度度量应用核平滑。两种方法都在森林训练后完全操作，无需额外生长树，仅需轻量级矩阵向量操作调整带宽参数h。

Result: 在3个合成基准和25个公共数据集上，RanBu匹配或超过全深度随机森林的准确性（尤其在高噪声设置中），同时将训练加推理时间减少高达95%。DiNo在低噪声机制中以适度的计算成本实现最佳偏差-方差权衡。两种方法可直接扩展到分位数回归。

Conclusion: DiNo和RanBu方法有效解决了随机森林的部署限制问题，在保持准确性的同时大幅提升效率，为资源受限环境提供了实用的解决方案。
...
### [125] [From Detection to Discovery: A Closed-Loop Approach for Simultaneous and Continuous Medical Knowledge Expansion and Depression Detection on Social Media](https://arxiv.org/abs/2510.23626)
*Shuang Geng,Wenli Zhang,Jiaheng Xie,Rui Wang,Sudha Ram*

Main category: cs.LG

TL;DR: 提出了一个闭环LLM-知识图谱框架，将抑郁症检测与医学知识扩展相结合，通过迭代学习循环实现预测准确性和医学知识的共同进化。


<details>
  <summary>Details</summary>
Motivation: 现有研究仅利用医学知识提升预测准确性，但忽略了通过预测过程扩展医学知识的机会。需要同时实现预测和知识扩展的闭环系统。

Method: 开发闭环LLM-知识图谱框架：1）知识感知抑郁症检测阶段，LLM联合执行抑郁症检测和实体提取；2）知识精炼与扩展阶段，在专家监督下将新实体、关系和类型整合到知识图谱中。

Result: 使用大规模用户生成内容，框架提升了预测准确性和医学理解。专家评估确认发现了与现有文献互补的临床有意义症状、共病和社会触发因素。

Conclusion: 概念化和操作化了"通过预测学习"和"通过学习预测"的相互强化过程，展示了计算模型和领域知识的共同进化，为其他动态风险监测环境提供了自适应、数据驱动的知识系统基础。
...
### [126] [Chain of Execution Supervision Promotes General Reasoning in Large Language Models](https://arxiv.org/abs/2510.23629)
*Nuo Chen,Zehua Li,Keqin Bao,Junyang Lin,Dayiheng Liu*

Main category: cs.LG

TL;DR: TracePile是一个包含260万样本的大规模语料库，将代码执行转化为显式的逐步推理链（Chain of Execution），通过在数学、算法等领域的训练，显著提升了LLaMA、Qwen等模型在数学、代码、逻辑和算法任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法直接在原始代码上训练存在不足，因为代码中的推理往往是隐式的，并与语法或实现噪声纠缠在一起，导致训练效果不理想。

Method: 构建TracePile语料库，将代码执行转化为显式的逐步推理链（CoE），包含变量追踪问题和代码重写，采用三种训练设置：继续预训练、预训练后指令调优和两阶段微调。

Result: 在四个基础模型（LLaMA 3、LLaMA 3.1、Qwen-2.5和Qwen-2.5 Coder）和20个基准测试上的实验显示了一致的改进，特别是LLaMA3.1-8B在九个数学数据集上平均提升了7.1%。

Conclusion: TracePile通过将代码执行转化为显式推理链，有效提升了语言模型在数学、代码、逻辑和算法任务上的推理能力。
...
### [127] [NUM2EVENT: Interpretable Event Reasoning from Numerical time-series](https://arxiv.org/abs/2510.23630)
*Ninghui Feng,Yiyan Qi*

Main category: cs.LG

TL;DR: 提出数字到事件推理任务，通过推理感知框架从数值时间序列中推断可解释的结构化事件，解决了现有方法仅关注预测而缺乏推理过程的问题。
...
Method: 提出推理感知框架，包含代理引导的事件提取器(AGE)、标记多元Hawkes合成生成器(EveDTS)，以及结合时间序列编码器和结构化解码器的两阶段微调管道。模型显式推理数值变化，生成中间解释，输出结构化事件假设。

Result: 在多领域数据集上的实验表明，该方法在事件级精度和召回率方面显著优于强大的LLM基线。

Conclusion: 该研究为连接定量推理和语义理解提供了新方向，使LLM能够直接从数值动态中解释和预测事件。
...
### [128] [Beyond Pairwise: Empowering LLM Alignment With Ranked Choice Modeling](https://arxiv.org/abs/2510.23631)
*Yuxuan Tang,Yifan Feng*

Main category: cs.LG

TL;DR: 提出了Ranked Choice Preference Optimization (RCPO)框架，通过最大似然估计将偏好优化与排名选择建模统一起来，支持基于效用和基于排名的选择模型，能够利用更丰富的人类反馈格式。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型对齐主要依赖成对偏好优化，这种方法忽略了从更丰富的人类反馈形式（如多比较和top-k排名）中学习的机会。

Method: 提出了RCPO统一框架，支持基于效用和基于排名的选择模型，包含多种现有的成对方法，并为更丰富的反馈格式提供原则性的训练目标。具体实例化了两个代表性的排名选择模型：多项Logit和Mallows-RMJ。

Result: 在Llama-3-8B-Instruct和Gemma-2-9B-it模型上的实证研究表明，RCPO在AlpacaEval 2和Arena-Hard基准测试中始终优于竞争基线。

Conclusion: RCPO展示了直接利用排名偏好数据结合正确的选择模型能够实现更有效的对齐，为将排名选择建模融入LLM训练提供了一个多功能和可扩展的基础。
...
### [129] [LLMComp: A Language Modeling Paradigm for Error-Bounded Scientific Data Compression](https://arxiv.org/abs/2510.23632)
*Guozhong Li,Muhannad Alhumaidi,Spiros Skiadopoulos,Panos Kalnis*

Main category: cs.LG

TL;DR: LLMCOMP是一种基于解码器大型语言模型的科学数据有损压缩方法，通过将3D场量化为离散token、Z-order曲线排列和覆盖引导采样，训练自回归变换器建模token转换，在严格误差约束下实现比现有方法高30%的压缩比。


<details>
  <summary>Details</summary>
Motivation: 高分辨率科学模拟和观测系统产生海量时空数据集，需要高效且误差有界的压缩方法。同时，解码器大型语言模型在建模复杂序列数据方面表现出卓越能力。

Method: 将3D场量化为离散token，通过Z-order曲线排列保持局部性，应用覆盖引导采样提高训练效率。训练带有时空嵌入的自回归变换器建模token转换，在压缩时执行top-k预测，仅存储排名索引和回退校正以确保严格误差界限。
...
### [130] [Noise is All You Need: Solving Linear Inverse Problems by Noise Combination Sampling with Diffusion Models](https://arxiv.org/abs/2510.23633)
...
<details>
  <summary>Details</summary>
Motivation: 预训练扩散模型在零样本逆问题求解中面临两难：过度集成观测信息会破坏生成过程，而集成不足则无法有效施加逆问题约束。

Method: 提出噪声组合采样方法，从噪声子空间合成最优噪声向量来近似测量分数，替代标准DDPM过程中的噪声项，实现条件信息的自然嵌入。

Result: 该方法适用于多种逆问题求解器，包括图像压缩，特别是在生成步骤较少时，以可忽略的计算开销实现优越性能。
...
### [131] [Monotone and Separable Set Functions: Characterizations and Neural Models](https://arxiv.org/abs/2510.23634)
*Soutrik Sarangi,Yonatan Sverdlov,Nadav Dym,Abir De*

Main category: cs.LG

TL;DR: 该论文研究了单调分离集合函数的设计，这类函数能保持集合包含关系的偏序结构，即S⊆T当且仅当F(S)≤F(T)。在无限基础集情况下，完全MAS函数不存在，但提出了弱MAS模型，并展示了其在集合包含任务中的优势。


<details>
  <summary>Details</summary>
Motivation: 受集合包含问题应用的启发，需要设计能保持集合偏序关系的函数，即集合包含关系S⊆T能通过向量函数F(S)≤F(T)来体现。

Method: 提出了单调分离集合函数的概念，在无限基础集情况下构建了弱MAS模型，该模型具有Holder连续性稳定性，并能构造通用模型来逼近所有单调集合函数。

Result: 建立了MAS函数所需向量维度的上下界，证明了无限基础集上完全MAS函数不存在，但弱MAS模型在实验中优于不包含集合包含归纳偏置的标准集合模型。

Conclusion: MAS函数为集合包含问题提供了理论基础，弱MAS模型在实际应用中表现良好，代码已开源供进一步研究使用。
...
### [132] [Help the machine to help you: an evaluation in the wild of egocentric data cleaning via skeptical learning](https://arxiv.org/abs/2510.23635)
*Andrea Bontempelli,Matteo Busso,Leonardo Javier Malcotti,Fausto Giunchiglia*

Main category: cs.LG

TL;DR: 本研究在真实用户环境中评估Skeptical Learning (SKEL)方法，通过iLog移动应用让大学生参与四周实验，验证SKEL在减少标注工作量和提高数据质量方面的效果。
...
Method: 让大学学生在四周时间内使用iLog移动应用，用户可以基于当前视角和需求细化输入标签，在真实环境中评估SKEL方法。

Result: 研究结果突显了在用户努力和数据质量之间找到适当平衡的挑战，同时展示了SKEL在减少标注工作量和提高收集数据质量方面的潜在益处。

Conclusion: SKEL方法在实际用户环境中具有应用价值，能够在减少用户标注负担的同时提升数据质量，但需要在用户参与度和数据准确性之间找到最佳平衡点。
...
### [133] [Flight Delay Prediction via Cross-Modality Adaptation of Large Language Models and Aircraft Trajectory Representation](https://arxiv.org/abs/2510.23636)
*Thaweerath Phisannupawong,Joshua Julian Damanik,Han-Lim Choi*

Main category: cs.LG

TL;DR: 提出基于轻量级大语言模型的多模态航班延误预测方法，通过将轨迹数据转化为语言模态，结合飞行信息、天气报告等文本信息，实现终端区航班延误的分钟级预测精度。


<details>
  <summary>Details</summary>
Motivation: 航班延误反映了空中交通管理中的效率问题，影响整体网络性能。从管制员监控终端区飞机延误的角度出发，需要更准确的预测方法来提升运行效率。

Method: 采用轻量级大语言模型，将轨迹表示与文本航空信息（飞行信息、天气报告、机场通告）相结合，通过跨模态适配将轨迹数据转化为语言模态，捕捉空域状况。

Result: 实验结果显示模型持续实现分钟级预测误差，有效利用与延误来源相关的上下文信息，预测精度高。

Conclusion: 该框架证明语言理解与轨迹信息的跨模态适配相结合能增强延误预测能力，方法具有实际应用性和可扩展性，支持实时更新以优化预测结果。
...
### [134] [Combining Textual and Structural Information for Premise Selection in Lean](https://arxiv.org/abs/2510.23637)
*Job Petrovčič,David Eliecer Narvaez Denis,Ljupčo Todorovski*

Main category: cs.LG

TL;DR: 提出了一种图增强方法，将Lean形式化的密集文本嵌入与图神经网络相结合，在依赖图上进行前提选择，性能比语言基线提升25%以上。


<details>
  <summary>Details</summary>
Motivation: 现有基于语言的方法通常孤立处理前提，忽略了连接前提的依赖关系网络，这是定理证明在大规模形式库中扩展的关键瓶颈。

Method: 结合Lean形式化的密集文本嵌入与图神经网络，在包含状态-前提和前提-前提关系的异构依赖图上进行学习。
...
Method: 利用All of Us研究项目的多模态数据，扩展生成式AI在EHR基础模型中的应用，整合PRS作为基础数据模态，并探索迁移学习用于定制分类任务。

Result: 在AoU数据上的评估显示模型对2型糖尿病等多种疾病发作具有预测价值，并揭示了PRS与EHR数据之间的相互作用关系。
...
Method: 设计结构化融合管道(SFP)结合2D拓扑和3D几何形成统一结构先验，采用渐进注入(PI)机制非对称集成到序列流中，基于状态空间骨干网络支持长程依赖建模。

Result: 在Therapeutics Data Commons和MoleculeNet的29个基准任务上，平均比最佳基线提升2.7%，在22个任务中排名第一，其中LD50任务提升27%。
...
TL;DR: 提出SAL-T（空间感知线性Transformer），通过物理启发的空间分区和卷积层，在保持线性注意力的同时提升性能，解决了Transformer在粒子碰撞分析中的计算效率问题。


<details>
  <summary>Details</summary>
Motivation: Transformer在高能粒子碰撞分析中有效但计算复杂度高，在CERN LHC等高数据吞吐环境中部署困难，需要降低资源消耗和推理延迟。

Method: 结合linformer线性注意力架构，基于运动学特征进行空间感知粒子分区，在物理意义区域间计算注意力，并使用卷积层捕获局部相关性。

Result: 在喷注分类任务中优于标准linformer，性能与全注意力Transformer相当，但资源消耗显著减少且推理延迟更低；在ModelNet10点云分类数据集上也验证了此趋势。

Conclusion: SAL-T通过物理启发的设计，在保持性能的同时大幅提升了计算效率，为高能物理实验中的实时分析提供了可行解决方案。
...
### [138] [Efficient Low Rank Attention for Long-Context Inference in Large Language Models](https://arxiv.org/abs/2510.23649)
*Tenghui Li,Guoxu Zhou,Xuyang Zhao,Yuning Qiu,Qibin Zhao*

Main category: cs.LG

TL;DR: LRQK是一个两阶段框架，通过低秩分解查询和键矩阵来减少KV缓存的内存占用，同时保持注意力输出的精确性，在长上下文设置中实现显著内存节省。
...
Method: LRQK在预填充阶段将全精度查询和键矩阵分解为紧凑的秩-r因子，然后在解码步骤中使用这些低维投影以O(lr)时间复杂度计算代理注意力分数。通过选择top-k令牌和少量固定最近令牌，采用混合GPU-CPU缓存机制，仅传输缺失的全精度KV对。

Result: 在RULER和LongBench基准测试中使用LLaMA-3-8B和Qwen2.5-7B的实验表明，LRQK在长上下文设置中匹配或超越领先的稀疏注意力方法，同时实现显著内存节省且精度损失最小。

Conclusion: LRQK通过低秩分解和混合缓存策略，有效解决了长上下文推理中的内存瓶颈问题，在保持精确注意力的同时大幅降低内存使用。
...
### [139] [Beyond Hidden-Layer Manipulation: Semantically-Aware Logit Interventions for Debiasing LLMs](https://arxiv.org/abs/2510.23650)
*Wei Xia*
...
Method: 使用静态和动态两种零样本logits层干预方法，通过语义感知的logits干预来实现去偏。

Result: Dynamic方法在减少偏见方面表现最佳，偏见减少达70%，同时保持了良好的流畅性；logits干预方法优于隐藏层方法。
...
### [140] [The Structural Scalpel: Automated Contiguous Layer Pruning for Large Language Models](https://arxiv.org/abs/2510.23652)
*Yao Lu,Yuqi Li,Wenbin Xie,Shanqing Yu,Qi Xuan,Zhaowei Zhu,Shiping Wen*

Main category: cs.LG

TL;DR: CLP是一个连续层剪枝框架，通过可微凹门算法自动识别最佳剪枝层段，并通过截止端点调优策略恢复模型性能，在多种大语言模型上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在资源受限的边缘设备上部署面临模型尺寸大和计算成本高的问题。现有层剪枝方法依赖手工指标且忽略层间依赖，会破坏信息流并严重降低性能。

Method: 提出CLP框架：1）可微凹门算法通过梯度优化自动识别最佳连续层段进行剪枝；2）截止端点调优策略通过微调剪枝段相邻层来有效恢复模型性能。

Result: 在多种模型架构（LLaMA2、LLaMA3、Qwen）和尺寸（7B到70B参数）上的实验表明，CLP显著优于现有方法。在20%剪枝率下，LLaMA3-70B平均性能保持率达95.34%，比基线高4.29%-30.52%。
...
### [141] [Error Adjustment Based on Spatiotemporal Correlation Fusion for Traffic Forecasting](https://arxiv.org/abs/2510.23656)
...
Method: SAEA框架将预测误差建模为时空VAR过程，使用系数矩阵捕获时空误差相关性，引入结构稀疏正则化融入路网结构先验，并设计测试时误差调整机制。

Result: 在不同交通数据集上的实验表明，该方法能显著提升各种交通预测模型的性能。

Conclusion: SAEA框架通过系统调整自相关预测误差，有效提升了交通预测模型的准确性，具有很好的通用性。
...
### [142] [A machine learning framework integrating seed traits and plasma parameters for predicting germination uplift in crops](https://arxiv.org/abs/2510.23657)
*Saklain Niam,Tashfiqur Rahman,Md. Amjad Patwary,Mukarram Hossain*

Main category: cs.LG

TL;DR: 本研究开发了首个机器学习框架来预测冷等离子体处理对种子萌发的提升效果，使用Extra Trees模型取得了最佳预测性能（R²=0.919），并发现种子萌发存在激素效应响应。


<details>
  <summary>Details</summary>
Motivation: 冷等离子体是一种环保的促进种子萌发方法，但由于复杂的种子-等离子体-环境相互作用，结果难以预测，需要开发预测框架来优化处理参数。

Method: 使用机器学习模型（GB、XGB、ET及其混合模型）预测大豆、大麦、向日葵、萝卜和番茄在介质阻挡放电等离子体处理下的萌发提升，并进行了特征工程分析。

Result: Extra Trees模型表现最佳（R²=0.919），特征缩减后提升至0.925。发现激素效应响应：7-15 kV、200-500 s处理效果最佳，放电功率≥100 W时萌发率最高。不同物种和品种预测精度存在差异。
...
### [143] [Aligning Diffusion Language Models via Unpaired Preference Optimization](https://arxiv.org/abs/2510.23658)
*Vaibhav Jindal,Hejian Sang,Chun-Mao Lai,Yanning Chen,Zhipeng Wang*

Main category: cs.LG

TL;DR: ELBO-KTO是一种结合ELBO替代扩散对数似然和KTO无配对偏好目标的扩散语言模型对齐方法，在多个基准测试中表现优于基础模型，证明了无配对偏好优化在扩散LLMs中的可行性。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型(dLLMs)作为自回归生成器的替代方案，其对齐人类偏好具有挑战性，因为序列对数似然难以计算且配对偏好数据收集成本高昂。
...
Conclusion: 无配对偏好优化是扩散LLMs中配对对齐的可行替代方案，为扩散语言模型的对齐提供了有效方法。
...
### [144] [Quantum Machine Learning for Image Classification: A Hybrid Model of Residual Network with Quantum Support Vector Machine](https://arxiv.org/abs/2510.23659)
*Md. Farhan Shahriyar,Gazi Tanbhir,Abdullah Md Raihan Chy*

Main category: cs.LG

TL;DR: 提出了一种结合ResNet-50特征提取和量子支持向量机(QSVM)的混合方法，用于马铃薯病害检测，其中基于Z特征映射的QSVM在准确率上超越了经典机器学习模型。


<details>
  <summary>Details</summary>
Motivation: 经典机器学习和深度学习模型在处理高维复杂数据集时存在困难，需要量子计算等先进技术来提高分类效率，特别是在图像分类任务中。

Method: 使用ResNet-50从马铃薯病害RGB图像中提取深度特征，通过PCA降维后，采用QSVM模型应用ZZ、Z和Pauli-X等量子特征映射将经典数据转换为量子态进行分类。

Result: 基于Z特征映射的QSVM模型表现最佳，准确率达到99.23%，超过了传统SVM和随机森林模型。

Conclusion: 该研究展示了将量子计算集成到图像分类中的优势，通过量子-经典混合建模为病害检测提供了潜在解决方案。
...
### [145] [Quanvolutional Neural Networks for Pneumonia Detection: An Efficient Quantum-Assisted Feature Extraction Paradigm](https://arxiv.org/abs/2510.23660)
...
Method: 使用量子卷积层处理2x2图像块，通过参数化量子电路进行数据编码和纠缠层生成非经典特征表示，然后将量子提取的特征输入经典神经网络进行分类。

Result: 提出的QNN模型在验证集上达到83.33%的准确率，相比传统CNN的73.33%有显著提升，表现出更好的收敛性和样本效率。
...
### [146] [AI-Driven Carbon Monitoring: Transformer-Based Reconstruction of Atmospheric CO2 in Canadian Poultry Regions](https://arxiv.org/abs/2510.23663)
*Padmanabhan Jagannathan Prajesh,Kaliaperumal Ragunath,Miriam Gordon,Bruce Rathgeber,Suresh Neethirajan*

Main category: cs.LG

TL;DR: 提出了ST-ViWT框架，将小波时频表示与transformer注意力机制结合，从OCO-2卫星数据重建连续、不确定性量化的XCO2场，在加拿大南部家禽密集区实现高精度CO2制图。


<details>
  <summary>Details</summary>
Motivation: 准确绘制农业景观上空的柱平均CO2浓度对于指导排放减缓策略至关重要，特别是在家禽养殖密集区域。

Method: 融合小波时频表示与transformer注意力机制，结合气象、植被指数、地形和土地覆盖数据，构建时空视觉transformer模型。

Result: 在2024年OCO-2数据上达到R2=0.984和RMSE=0.468 ppm；92.3%的填补预测在±1 ppm内；与TCCON独立验证显示良好泛化能力（偏差=-0.14 ppm，r=0.928）。

Conclusion: 该方法支持卫星约束与国家清单和精准畜牧平台的整合，实现可扩展、透明、空间明确的碳核算、热点优先排序和政策相关的减缓评估。
...
### [147] [Transformers from Compressed Representations](https://arxiv.org/abs/2510.23665)
...
<details>
  <summary>Details</summary>
Motivation: 压缩文件格式是高效数据存储和传输的基础，但其在表示学习方面的潜力尚未充分探索。现有方法需要原始字节处理或完整解码，计算复杂度和内存使用较高。

Method: 利用压缩文件的固有字节流结构设计标记化和编码策略，通过紧凑编码使标准transformer能够直接从压缩数据流学习语义表示。

Result: 在多样化数据集、编码方案和模态上的实验表明，TEMPEST在保持与最先进方法竞争性准确度的同时，显著减少了语义分类所需的标记数量，降低了计算复杂度和内存使用。

Conclusion: TEMPEST展示了直接从压缩表示中学习的可行性，为高效表示学习提供了新途径，在保持准确性的同时实现了内存和计算效率的提升。
...
### [148] [Optimize Any Topology: A Foundation Model for Shape- and Resolution-Free Structural Topology Optimization](https://arxiv.org/abs/2510.23667)
*Amin Heyrani Nobari,Lyle Regenwetter,Cyril Picard,Ligong Han,Faez Ahmed*

Main category: cs.LG

TL;DR: OAT是一个基于深度学习的拓扑优化框架，能够直接预测任意长宽比、分辨率、体积分数、载荷和固定条件下的最小柔度布局，相比现有方法在多个基准测试中柔度降低高达90%，推理时间不到1秒。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法局限于固定正方形网格、少量手动编码的边界条件和后处理优化，无法实现通用部署。需要开发能够处理任意边界条件和几何参数的拓扑优化方法。

Method: 结合分辨率无关和形状无关的自编码器、隐式神经场解码器以及条件潜在扩散模型，在包含220万个优化结构的OpenTO数据集上进行训练。

Result: 在四个公共基准和两个具有挑战性的未见测试中，OAT相比最佳现有模型将平均柔度降低高达90%，在单个GPU上实现从64×64到256×256分辨率、高达10:1长宽比的亚秒级推理。

Conclusion: OAT为物理感知拓扑优化建立了一个通用、快速且分辨率无关的框架，并提供了大规模数据集以促进逆向设计生成建模的进一步研究。
...
### [149] [Traffic flow forecasting, STL decomposition, Hybrid model, LSTM, ARIMA, XGBoost, Intelligent transportation systems](https://arxiv.org/abs/2510.23668)
...
Conclusion: 分解策略有效隔离了时间特征，使每个模型能够专业化处理特定分量，从而提高了预测精度、可解释性和鲁棒性。
...
### [150] [Sparsity and Superposition in Mixture of Experts](https://arxiv.org/abs/2510.23671)
*Marmik Chaudhari,Jeremi Nuer,Rome Thorstenson*

Main category: cs.LG

TL;DR: 该论文研究了MoE模型与密集网络在机制上的差异，发现网络稀疏性（激活专家比例）是MoE模型的关键特征，而非特征稀疏性或重要性。研究表明更高的网络稀疏性导致更大的单义性，专家会自然围绕连贯特征组合组织。


<details>
  <summary>Details</summary>
Motivation: 理解MoE模型与密集网络在机制上的根本差异，探索MoE模型的可解释性潜力，挑战可解释性与性能不可兼得的传统假设。

Method: 开发了新的度量方法来测量专家间的叠加现象，提出了基于单义特征表示而非负载平衡的专家专业化新定义，研究了不同网络稀疏性下的模型行为。

Result: 发现网络稀疏性越大，模型表现出越大的单义性；专家在适当初始化时会自然围绕连贯特征组合组织；MoE模型可能在不牺牲性能的情况下实现更好的可解释性。

Conclusion: MoE模型中的网络稀疏性可能实现更可解释的模型，同时保持性能，这挑战了可解释性与能力根本对立的常见假设。
...
### [151] [DBLoss: Decomposition-based Loss Function for Time Series Forecasting](https://arxiv.org/abs/2510.23672)
*Xiangfei Qiu,Xingjian Wu,Hanyin Cheng,Xvyuan Liu,Chenjuan Guo,Jilin Hu,Bin Yang*

Main category: cs.LG

TL;DR: 提出了DBLoss分解损失函数，通过指数移动平均将时间序列分解为季节性和趋势分量，分别计算损失并加权，显著提升深度学习预测模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有MSE损失函数有时无法准确捕捉预测范围内的季节性或趋势模式，即使在前向传播中使用分解模块分别建模趋势和季节性。
...
### [152] [Informed Initialization for Bayesian Optimization and Active Learning](https://arxiv.org/abs/2510.23681)
*Carl Hvarfner,David Eriksson,Eytan Bakshy,Max Balandat*

Main category: cs.LG

TL;DR: 提出了HIPE方法，一种基于信息论的新型采集策略，用于贝叶斯优化的初始化阶段，平衡预测不确定性减少和超参数学习。
...
Method: HIPE方法在高斯过程框架下推导出闭式表达式，通过信息论原理平衡预测不确定性减少和超参数学习。

Result: 在主动学习和少样本贝叶斯优化实验中，HIPE在预测精度、超参数识别和后续优化性能方面优于标准初始化策略。

Conclusion: HIPE特别适用于大规模批量、少样本场景，在现实贝叶斯优化应用中表现出色。
...
### [153] [Beyond Prompt Engineering: Neuro-Symbolic-Causal Architecture for Robust Multi-Objective AI Agents](https://arxiv.org/abs/2510.23682)
*Gokturk Aytug Akarlar*

Main category: cs.LG

TL;DR: Chimera是一个神经-符号-因果架构，通过整合LLM策略师、形式验证符号约束引擎和因果推理模块，解决了LLM代理在决策中的脆弱性问题，在电子商务环境中实现了稳定的高性能和品牌信任提升。
...
Method: 提出Chimera架构，包含三个互补组件：LLM策略师、形式验证符号约束引擎和用于反事实推理的因果推理模块。在包含价格弹性、信任动态和季节性需求的电子商务环境中进行52周模拟测试。

Result: Chimera始终提供最高回报（152万美元和196万美元，某些情况下+220万美元），同时提升品牌信任度（+1.8%和+10.8%，某些情况下+20.86%），TLA+形式验证证明所有场景中零约束违规。
...
### [154] [Parallel BiLSTM-Transformer networks for forecasting chaotic dynamics](https://arxiv.org/abs/2510.23685)
...
Conclusion: 提出的Transformer-BiLSTM混合框架能够有效提升混沌系统预测的准确性，通过并行架构同时捕获局部和全局特征。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习系统越来越多地影响重要决策，这些数据驱动系统往往缺乏明确的公平性考虑，存在歧视性影响的风险。

Method: 开发了更合适的公平性测量方法，系统分解ML系统以预测偏见动态，以及减少算法歧视同时保持系统效用的有效干预措施。

Result: 为衡量ML系统公平性、分析偏见动态和实施减少歧视的干预措施提供了基础框架。

Conclusion: 随着ML系统（包括生成式AI）日益融入社会，该工作为确保ML的社会影响与更广泛的社会价值观保持一致奠定了基础，同时讨论了持续挑战和未来研究方向。
...
TL;DR: MUStReason是一个诊断性基准测试，用于评估视频语言模型在讽刺检测任务中的表现，包含模态特定相关线索和推理步骤的标注。作者还提出了PragCoT框架来引导模型关注隐含意图而非字面意义。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型在复杂任务如讽刺检测上表现不佳，这类任务需要跨模态识别相关线索并进行语用推理。讽刺检测不仅依赖字面内容，还需要语调、面部表情和对话上下文等非语言线索。

Method: 引入MUStReason诊断基准，包含模态特定相关线索和推理步骤的标注。提出PragCoT框架，将问题分解为感知和推理，引导视频语言模型关注隐含意图而非字面意义。

Result: 通过MUStReason对视频语言模型的讽刺分类性能和生成推理进行定量和定性评估。PragCoT框架能够提升模型在讽刺检测任务中的表现。

Conclusion: MUStReason基准和PragCoT框架为评估和改进视频语言模型在复杂语用推理任务（如讽刺检测）中的能力提供了有效工具，强调了关注隐含意图对于讽刺检测的重要性。
...
TL;DR: 提出了一种减轻奖励模型中虚假相关性的原则性框架，通过识别非虚假潜在变量来训练更稳健的奖励模型。
...
Method: 首先构建数据生成过程的公式化描述，假设观测数据来自虚假和非虚假潜在变量；然后使用变分推断恢复非虚假潜在变量，并利用这些变量训练奖励模型。

Result: 在合成和真实数据集上的实验表明，该方法有效减轻了虚假相关性问题，产生了更稳健的奖励模型。

Conclusion: 提出的框架能够有效缓解奖励模型中的偏差问题，同时保留反映真实偏好的潜在因素。
...
TL;DR: 该研究探讨了Cobweb/4V模型在持续学习中抵抗灾难性遗忘的机制，通过对比神经基线模型验证了三个假设：自适应结构重组、稀疏选择性更新和信息理论学习的优势。


<details>
  <summary>Details</summary>
Motivation: 灾难性遗忘是持续学习中的核心挑战，先前开发的Cobweb/4V模型在视觉领域表现出对灾难性遗忘的鲁棒性，研究旨在揭示这种稳定性的关键因素。

Method: 通过比较Cobweb/4V与神经基线模型（包括新提出的CobwebNN），在MNIST、Fashion-MNIST、MedMNIST和CIFAR-10等不同复杂度数据集上进行实验，验证三个假设。

Result: 实验表明自适应结构重组增强学习可塑性，稀疏更新减少干扰，基于充分统计量的信息理论学习过程无需重访过去数据即可保留先验知识。
...
Method: RSS在连续logit空间中操作，结合梯度引导探索和蛋白质语言模型引导的跳跃，其能量函数耦合了AlphaFold2的结构目标和ESM2的序列先验。

Result: 在蛋白质结合物设计任务中，RSS相比RSO基线产生了5倍的可设计结构和2-3倍的结构多样性，计算成本相同。

Conclusion: RSS是一个原则性的方法，能有效探索蛋白质设计空间。
...
### [160] [Revealing the Potential of Learnable Perturbation Ensemble Forecast Model for Tropical Cyclone Prediction](https://arxiv.org/abs/2510.23794)
*Jun Liu,Tao Zhou,Jiarui Li,Xiaohui Zhong,Peng Zhang,Jie Feng,Lei Chen,Hao Li*

Main category: cs.LG

TL;DR: FuXi-ENS是一种基于AI的台风集合预报系统，相比传统ECMWF-ENS在台风路径预报和物理变量预测方面表现更优，但强度预报仍有低估。
...
Method: FuXi-ENS采用可学习扰动方案生成集合预报，并与ECMWF-ENS在2018年90个全球台风上进行系统对比分析。

Result: FuXi-ENS在台风相关物理变量预测和路径预报方面优势明显，集合离散度更小，但强度预报仍低于观测值。动力学分析显示其能更好捕捉大尺度环流特征。
...
### [161] [Learning Interpretable Features in Audio Latent Spaces via Sparse Autoencoders](https://arxiv.org/abs/2510.23802)
*Nathan Paek,Yongyi Zang,Qihui Yang,Randal Leistikow*

Main category: cs.LG

TL;DR: 提出了一个解释音频生成模型潜在表示的框架，通过将稀疏自编码器特征映射到可解释的声学概念（音高、振幅、音色），实现对AI音乐生成过程的可控操作和分析。
...
Result: 验证了在DiffRhythm-VAE、EnCodec和WavTokenizer上的有效性，分析了DiffRhythm文本到音乐模型中音高、音色和响度在生成过程中的演变。
...
### [162] [How do simple rotations affect the implicit bias of Adam?](https://arxiv.org/abs/2510.23804)
*Adela DePavia,Vasileios Charisopoulos,Rebecca Willett*

Main category: cs.LG

TL;DR: Adam等自适应梯度方法对数据旋转敏感，可能丧失其非线性决策边界优势。通过正交重参数化可以恢复Adam的丰富性偏置。
...
Result: 即使小的数据旋转也会使Adam丧失丰富性偏置，收敛到线性决策边界。正交重参数化能够恢复Adam对丰富决策边界的偏置。

Conclusion: 自适应梯度方法对数据变换敏感，但通过适当的重参数化可以保持其优势，这对实际应用中的优化算法选择有重要意义。
...
### [163] [A Physics-informed Multi-resolution Neural Operator](https://arxiv.org/abs/2510.23810)
*Sumanta Roy,Bahador Bahmani,Ioannis G. Kevrekidis,Michael D. Shields*

Main category: cs.LG

TL;DR: 提出了一种物理信息驱动的算子学习方法，将分辨率无关神经算子框架扩展到完全无数据设置，解决了训练数据稀缺和分辨率不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 传统算子学习方法需要大量高质量训练数据，但在实际工程应用中获取困难，且数据往往存在分辨率不一致的问题。

Method: 将任意离散化的输入函数投影到潜在嵌入空间，使用预训练基函数。通过多层感知机近似偏微分方程算子，结合时空坐标在物理空间中求解。
...
### [164] [Combining SHAP and Causal Analysis for Interpretable Fault Detection in Industrial Processes](https://arxiv.org/abs/2510.23817)
*Pedro Cortes dos Santos,Matheus Becali Rocha,Renato A Krohling*

Main category: cs.LG

TL;DR: 该研究提出了一种结合SHAP特征重要性和因果分析的工业故障检测框架，使用田纳西伊斯曼过程作为基准，通过简化复杂数据来提升检测性能和可解释性。


<details>
  <summary>Details</summary>
Motivation: 工业过程数据复杂，现有机器学习方法在故障检测中表现不佳且难以解释，需要开发更透明和有效的检测方法。

Method: 使用SHAP识别关键过程特征，然后应用有向无环图进行因果分析，揭示故障传播机制。

Result: 该方法显著提高了故障检测准确性，因果结构与SHAP发现高度一致，识别出冷却和分离系统等关键故障驱动因素。

Conclusion: 该双方法框架将预测能力与因果理解相结合，为复杂工业环境提供了强大且可解释的故障检测工具。
...
### [165] [ScaLoRA: Optimally Scaled Low-Rank Adaptation for Efficient High-Rank Fine-Tuning](https://arxiv.org/abs/2510.23818)
*Yilang Zhang,Xiaodong Yang,Yiwei Cai,Georgios B. Giannakis*

Main category: cs.LG

TL;DR: 提出了一种通过累积连续低秩增量来构建高秩权重更新的方法，解决了LoRA在效率和收敛速度方面的限制，在多个任务上实现了优于现有LoRA变体的性能。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模扩大，计算开销成为任务特定微调的主要瓶颈。虽然LoRA通过将权重更新限制在低维子空间来降低成本，但这种限制会降低效果并减慢收敛速度。

Method: 通过累积连续的低秩增量来逐步构建高秩权重更新，识别每个更新的最优低秩矩阵以最小化损失函数并接近全参数微调。通过适当缩放原始低秩矩阵的列来实现高效无缝优化。

Result: 在参数规模达120亿的流行LLM上进行广泛测试，在自然语言理解、常识推理和数学问题解决等多样化任务上，相比最先进的LoRA变体展现出持续的性能提升和快速收敛。

Conclusion: 该方法通过累积低秩增量构建高秩更新，有效解决了LoRA的限制，在保持高效的同时实现了更好的性能和更快的收敛速度。
...
### [166] [A PDE-Informed Latent Diffusion Model for 2-m Temperature Downscaling](https://arxiv.org/abs/2510.23866)
...
<details>
  <summary>Details</summary>
Motivation: 传统扩散模型训练虽然能产生低PDE残差，但需要进一步正则化模型以增强生成场的物理合理性。通过集成PDE损失项来强制物理一致性。

Method: 基于预训练扩散架构，采用残差公式，在训练目标中集成偏微分方程损失项。PDE损失在完整分辨率空间中计算，通过有限差分近似有效平流-扩散平衡来实施物理约束。

Result: 经验观察表明传统扩散训练已产生低PDE残差，通过额外损失微调进一步正则化模型，提高了生成场的物理合理性。

Conclusion: 物理条件化的潜在扩散模型能有效进行大气数据动力学降尺度，PDE损失项增强了物理一致性，代码已在Github开源供未来发展参考。
...
### [167] [GIFT: Group-relative Implicit Fine Tuning Integrates GRPO with DPO and UNA](https://arxiv.org/abs/2510.23868)
*Zhichao Wang*
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法如PPO、GRPO直接最大化累积奖励的问题，以及离线方法如DPO、UNA缺乏探索能力的问题，提出一种更稳定、高效的LLM对齐框架。
...
Conclusion: GIFT提供了一个稳定、高效且具有探索能力的LLM对齐框架，优于现有方法，在保持计算效率的同时实现了更好的性能。
...
### [168] [Artificial Intelligence Based Predictive Maintenance for Electric Buses](https://arxiv.org/abs/2510.23879)
...
Method: 开发混合图基特征选择工具，结合统计过滤（皮尔逊相关、Cramer's V、ANOVA F检验）和基于优化的社区检测算法（InfoMap、Leiden、Louvain、Fast Greedy）。使用SVM、随机森林和XGBoost等机器学习模型，通过网格和随机搜索优化，并采用SMOTEEN和数据平衡技术。

Result: 开发的系统能有效预测车辆警报，增强特征可解释性，支持符合工业4.0原则的主动维护策略。

Conclusion: 基于图的特征选择与机器学习相结合的方法在电动公交车预测性维护中表现出色，为复杂系统的故障预测提供了有效解决方案。
...
### [169] [RS-ORT: A Reduced-Space Branch-and-Bound Algorithm for Optimal Regression Trees](https://arxiv.org/abs/2510.23901)
*Cristobal Heredia,Pedro Chumpitaz-Flores,Kaixun Hua*

Main category: cs.LG

TL;DR: 提出RS-ORT算法，通过两阶段优化和专门的分支定界方法解决大规模连续特征回归树训练的计算难题，保证收敛性并实现并行计算。
...
Method: 将最优回归树训练重构为两阶段优化问题，提出RS-ORT分支定界算法，专门在树结构变量上分支；引入边界收紧技术（闭式叶预测、经验阈值离散化、精确深度1子树解析）和可分解上下界策略。

Result: 在包含二元和连续特征的回归基准测试中，RS-ORT比最先进方法具有更好的训练和测试性能；在200万样本数据集上，4小时内可获得保证的训练性能、更简单树结构和更好泛化能力。

Conclusion: RS-ORT有效解决了大规模连续特征回归树训练的计算难题，提供保证的最优解，并在实际应用中展现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 虽然二元偏好数据在基础模型微调中变得基础，但用户决策过程中的宝贵时间信息仍未得到充分利用。响应时间可以揭示偏好强度，但现有方法忽略了这一信息。

Method: 提出将响应时间信息与二元选择数据结合的新方法，利用证据积累漂移扩散模型，开发Neyman正交损失函数，实现奖励模型学习的oracle收敛率。

Result: 理论分析表明，对于线性奖励函数，传统偏好学习的错误率随奖励幅度呈指数级增长，而响应时间增强方法将其降低为多项式增长。实验验证了在图像偏好学习中的有效性。

Conclusion: 整合响应时间信息可以显著提高偏好学习的样本效率，为奖励模型学习提供了更有效的方法，特别是在处理复杂奖励函数时表现优异。
...
### [171] [Group Interventions on Deep Networks for Causal Discovery in Subsystems](https://arxiv.org/abs/2510.23906)
*Wasim Ahmad,Maha Shadaydeh,Joachim Denzler*

Main category: cs.LG

TL;DR: 提出gCDMI方法，通过组级干预和模型不变性测试来发现多组变量间的因果关系，在模拟和真实数据集上表现优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有因果发现方法主要关注成对因果关系，忽略了变量组（子系统）及其集体因果影响，需要开发能够处理组级因果关系的多组因果发现方法

Method: 1. 使用深度学习联合建模所有时间序列组间的结构关系；2. 对训练好的模型应用组级干预；3. 进行模型不变性测试以确定变量组间的因果链接

Result: 在模拟数据集上表现出优越性能，能够有效识别组级因果关系；在真实世界数据集（脑网络和气候生态系统）上验证了方法的有效性
...
### [172] [Key and Value Weights Are Probably All You Need: On the Necessity of the Query, Key, Value weight Triplet in Decoder-Only Transformers](https://arxiv.org/abs/2510.23912)
*Marko Karbevski,Antonij Mijoski*

Main category: cs.LG

TL;DR: 研究发现注意力机制中的Query权重是冗余的，可以将其移除，从而将非嵌入/LM头参数减少超过8%，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 探索当前LLM中注意力机制的基本构建块Query-Key-Value权重三元组是否可以简化，以减少模型参数而不损失性能。

Method: 在简化假设下进行理论分析证明Query权重冗余性，并在完整复杂度的GPT-3小型架构上进行验证实验，包含层归一化、跳跃连接和权重衰减等组件。

Result: 理论证明和实验验证表明移除Query权重后，简化模型在验证损失上与标准基线模型表现相当。

Conclusion: Query权重在注意力机制中是冗余的，这一发现为在大规模模型中进一步研究Query权重冗余性提供了动机。
...
### [173] [Geometry-Inspired Unified Framework for Discounted and Average Reward MDPs](https://arxiv.org/abs/2510.23914)
...
### [174] [Improving the Straight-Through Estimator with Zeroth-Order Information](https://arxiv.org/abs/2510.23926)
...
Method: FOGZO（一阶引导的零阶梯度下降），通过结合STE和ZO方法的优势来减少STE偏差并降低计算复杂度
...
### [175] [Differential Privacy: Gradient Leakage Attacks in Federated Learning Environments](https://arxiv.org/abs/2510.23931)
*Miguel Fernandez-de-Retana,Unai Zulaika,Rubén Sánchez-Corcuera,Aitor Almeida*

Main category: cs.LG

TL;DR: 评估差分隐私机制（DP-SGD和PDP-SGD）在联邦学习中防御梯度泄漏攻击的效果，发现DP-SGD能显著降低攻击风险但牺牲模型性能，而PDP-SGD保持性能但防御效果不佳。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然保护数据隐私，但仍面临梯度泄漏攻击的风险，需要评估现有差分隐私机制的实际防御效果。

Method: 在模拟联邦学习环境中，使用不同隐私级别训练计算机视觉模型，分析从截获梯度中重建私有数据的质量。

Result: DP-SGD显著减轻梯度泄漏攻击风险，但模型性能有所下降；PDP-SGD保持良好分类性能，但无法有效防御重建攻击。

Conclusion: 需要超越理论保证，在实际分布式学习场景中实证评估隐私机制，因为信息泄漏可能对数据安全构成严重威胁。
...
### [176] [A data free neural operator enabling fast inference of 2D and 3D Navier Stokes equations](https://arxiv.org/abs/2510.23936)
...
Method: 采用物理基础架构，以初始条件、边界条件和强迫函数作为输入，直接输出解。该方法结合了数值基础架构和机器学习可扩展性。

Result: 在2D基准测试和3D测试案例中，该方法在精度上超越了先前的神经算子，对于集合预报比传统数值求解器更高效。首次实现了三维Navier-Stokes方程的准确求解。

Conclusion: 该方法为端到端科学模拟和预测建立了实用的无数据高保真PDE替代模型路径，结合了数值基础架构和机器学习的可扩展性。
...
### [177] [Modeling Biological Multifunctionality with Echo State Networks](https://arxiv.org/abs/2510.23940)
*Anastasia-Maria Leventi-Peetz,Jörg-Volker Peetz,Kai Weber,Nikolaos Zacharis*

Main category: cs.LG

TL;DR: 开发了结合可激发系统动力学与扩散过程的三维多组分反应-扩散模型，使用回声状态网络成功重现系统动态行为，证明数据驱动方法模拟生物动力学的可行性。


<details>
  <summary>Details</summary>
Motivation: 捕捉生物系统（特别是电生理过程）的时空行为，探索使用数据驱动模型模拟复杂生物动力学的有效性。

Method: 构建三维多组分反应-扩散模型，结合可激发系统动力学和扩散过程，数值求解生成时间序列数据，使用回声状态网络进行训练和评估。

Result: 回声状态网络成功重现了系统的动态行为，验证了模型的有效性。
...
### [178] [ChessQA: Evaluating Large Language Models for Chess Understanding](https://arxiv.org/abs/2510.23948)
*Qianfeng Wen,Zhenwei Tang,Ashton Anderson*

Main category: cs.LG

TL;DR: ChessQA是一个综合性的基准测试，用于评估大型语言模型在象棋理解方面的能力，涵盖五个任务类别，从基础规则到高级概念。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM象棋能力评估缺乏系统性和广度，难以准确衡量模型在不同规模、训练方法和架构选择下的象棋理解能力。

Method: 构建了包含五个任务类别（结构、模式、短战术、位置判断和语义）的ChessQA基准测试，对应玩家掌握象棋知识的递进抽象层次。

Result: 评估当代LLMs发现，在所有五个类别中都存在持续的弱点，并提供了按类别的结果和错误分析。

Conclusion: ChessQA提供了一个更全面的象棋能力评估框架，超越了之前的简单走子质量评估，为诊断和比较提供了受控环境，并将发布代码、数据集和排行榜。
...
### [179] [A Pragmatic Way to Measure Chain-of-Thought Monitorability](https://arxiv.org/abs/2510.23966)
*Scott Emmons,Roland S. Zimmermann,David K. Elson,Rohin Shah*

Main category: cs.LG

TL;DR: 提出了一种测量思维链可监控性的方法，包括可读性和覆盖度两个指标，使用自动评分器提示来评估现有模型的思维链质量。


<details>
  <summary>Details</summary>
Motivation: 思维链监控为AI安全提供了独特机会，但训练实践或模型架构的变化可能削弱这种可监控性，需要量化方法来保持监控能力。

Method: 开发了自动评分器提示，让任何有能力的LLM能够计算现有思维链的可读性和覆盖度，并通过合成思维链退化进行验证。

Result: 在多个前沿模型的挑战性基准测试中发现它们表现出高度的可监控性。

Conclusion: 该方法有助于测量思维链的默认可监控性，应被视为对抗性压力测试的补充工具，供开发者跟踪设计决策对可监控性的影响。
...
### [180] [An efficient probabilistic hardware architecture for diffusion-like models](https://arxiv.org/abs/2510.23972)
*Andraž Jelinčič,Owen Lockwood,Akhil Garlapati,Guillaume Verdon,Trevor McCourt*

Main category: cs.LG

TL;DR: 提出一种全晶体管概率计算机，在硬件层面实现强大的去噪模型，相比GPU能效提升约10,000倍


<details>
  <summary>Details</summary>
Motivation: 现有的概率AI专用计算机依赖于有限建模技术和不可扩展的异质硬件，未能获得广泛应用

Method: 设计基于全晶体管的概率计算机架构，在硬件层面直接实现去噪模型
...
### [181] [Diffusion Adaptive Text Embedding for Text-to-Image Diffusion Models](https://arxiv.org/abs/2510.23974)
...
<details>
  <summary>Details</summary>
Motivation: 传统扩散模型在整个扩散过程中使用固定的文本嵌入，限制了文本条件对生成过程的适应性，导致文本-图像对齐效果不佳。

Method: 提出扩散自适应文本嵌入(DATE)，在每个扩散时间步基于中间扰动数据动态优化文本嵌入，通过优化问题推导更新规则。

Result: DATE在保持模型生成能力的同时，在多概念生成和文本引导图像编辑等任务中显著优于固定文本嵌入方法。

Conclusion: DATE通过动态调整文本嵌入有效提升了扩散模型的文本-图像对齐性能，且无需额外训练，具有实用价值。
...
### [182] [Synergistic Neural Forecasting of Air Pollution with Stochastic Sampling](https://arxiv.org/abs/2510.23977)
*Yohan Abeysinghe,Muhammad Akhtar Munir,Sanoojan Baliah,Ron Sarafian,Fahad Shahbaz Khan,Yinon Rudich,Salman Khan*

Main category: cs.LG

TL;DR: SynCast是一个高分辨率神经预测模型，通过整合气象和空气成分数据来改进平均和极端污染水平的预测，特别针对罕见但危险的污染事件。


<details>
  <summary>Details</summary>
Motivation: 空气污染是全球健康和环境的主要风险，现有模型经常低估罕见但危险的污染事件，需要更准确的颗粒物浓度预测来及时发出公共卫生警告和干预。
...
### [183] [HyperGraphX: Graph Transductive Learning with Hyperdimensional Computing and Message Passing](https://arxiv.org/abs/2510.23980)
*Guojing Cong,Tom Potok,Hamed Poursiami,Maryam Parsa*

Main category: cs.LG

TL;DR: HDGC算法将图卷积与超维计算中的绑定和捆绑操作相结合，在转导图学习任务中实现了高准确率和极快的推理速度。


<details>
  <summary>Details</summary>
Motivation: 结合图神经网络和超维计算的优势，开发一种既准确又高效的图学习方法，特别适合在神经形态和内存计算设备上运行。

Method: 将图卷积与超维计算中的绑定和捆绑操作相融合，主要使用二进制向量进行学习操作。

Result: 在多种同质性和异质性图数据集上，HDGC在准确率上超越了主流图神经网络和最先进的超维计算方法；在GPU平台上，比GCNII快9561倍，比HDGL快144.5倍。

Conclusion: HDGC算法在保持高准确率的同时实现了显著的性能提升，特别适合在能效要求高的硬件平台上部署。
...
### [184] [STNet: Spectral Transformation Network for Solving Operator Eigenvalue Problem](https://arxiv.org/abs/2510.23986)
*Hong Wang,Jiang Yixuan,Jie Wang,Xinyi Li,Jian Luo,Huanshuo Dong*

Main category: cs.LG

TL;DR: 提出了谱变换网络(STNet)，通过谱变换技术改进算子特征值问题的深度学习求解方法，利用近似特征值和特征函数对原始算子进行变换，提高求解精度和效率。


<details>
  <summary>Details</summary>
Motivation: 传统数值方法在求解算子特征值问题时面临维度灾难，现有深度学习方法性能受算子谱分布影响较大，较大的特征值间隙能提高精度，因此需要利用谱分布信息来提升性能。

Method: STNet在每次迭代中使用近似特征值和特征函数对原始算子进行谱变换，包括：1) 使用收缩投影排除已求解特征函数对应的子空间；2) 使用滤波器变换放大目标区域特征值并抑制外部特征值。

Result: 大量实验表明STNet在精度上持续优于现有基于学习的方法，达到了最先进的性能水平。

Conclusion: STNet通过谱变换技术有效解决了算子特征值问题的维度灾难，显著提升了深度学习方法的求解精度和效率。
...
### [185] [Optimal Arm Elimination Algorithms for Combinatorial Bandits](https://arxiv.org/abs/2510.23992)
*Yuxiao Wen,Yanjun Han,Zhengyuan Zhou*

Main category: cs.LG

TL;DR: 提出了一种新的组合多臂老虎机消除算法，通过将臂分为确认、活跃和消除三类，并加入显式探索来更新这些集合。在组合多臂老虎机和组合线性上下文老虎机中实现了近乎最优的遗憾界。


<details>
  <summary>Details</summary>
Motivation: 组合老虎机扩展了经典老虎机框架，但在组合设置中适应臂消除方法一直具有挑战性。UCB算法自然扩展但可能因显式探索不足而失败，需要新的消除方案。

Method: 引入新颖的消除方案，将臂分为确认、活跃和消除三类，并加入显式探索来更新这些集合。在组合多臂老虎机和组合线性上下文老虎机两种设置中应用该算法。

Result: 在两种设置中都实现了近乎最优的遗憾界，而基于UCB的方法由于显式探索不足可能失败。同时提供了匹配的下界。

Conclusion: 该消除算法在组合老虎机设置中有效解决了UCB方法的局限性，通过显式探索实现了近乎最优的性能，并提供了理论保证。
...
### [186] [Predicting Barge Tow Size on Inland Waterways Using Vessel Trajectory Derived Features: Proof of Concept](https://arxiv.org/abs/2510.23994)
...
<details>
  <summary>Details</summary>
Motivation: 由于驳船的非自航特性和现有监测系统的局限性，准确实时估计内河驳船数量仍是一个关键挑战

Method: 使用AIS船舶跟踪数据，通过机器学习预测拖带驳船数量。手动标注卫星图像中的驳船实例，与AIS轨迹进行时空匹配，创建30个AIS衍生特征，使用递归特征消除筛选特征，训练6种回归模型

Result: 泊松回归器模型表现最佳，使用12个特征实现平均绝对误差1.92艘驳船。特征重要性分析显示船舶机动性指标（航向熵、速度变异性和行程长度）最能预测驳船数量

Conclusion: 该方法为增强海事领域感知提供了可扩展、易于实施的方法，在船闸调度、港口管理和货运规划方面具有强大应用潜力。未来将扩展到其他内河环境
...
### [187] [Training-Free Safe Text Embedding Guidance for Text-to-Image Diffusion Models](https://arxiv.org/abs/2510.24012)
*Byeonghu Na,Mina Kang,Jiseok Kwak,Minsang Park,Jiwoo Shin,SeJoon Jun,Gayoung Lee,Jin-Hwa Kim,Il-Chul Moon*

Main category: cs.LG

TL;DR: 提出了STG方法，一种无需训练的文本嵌入引导技术，通过调整文本嵌入来提升扩散模型的安全性，在不影响生成质量的前提下有效过滤有害内容。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型虽然能生成逼真图像，但训练数据中的不当内容可能导致模型生成有害输出，需要提升模型安全性。

Method: STG在采样过程中基于安全函数评估最终去噪图像的预期安全性，动态调整文本嵌入，无需额外训练即可实现安全约束。

Result: 在多种安全场景（如裸露、暴力、艺术家风格移除）的实验中，STG在移除不安全内容的同时保持输入提示核心语义方面优于基于训练和无训练基线方法。

Conclusion: STG通过理论证明能够将基础模型分布与安全约束对齐，在保持生成质量的同时实现更安全的输出，为扩散模型安全提供有效解决方案。
...
### [188] [NeuroPathNet: Dynamic Path Trajectory Learning for Brain Functional Connectivity Analysis](https://arxiv.org/abs/2510.24025)
*Guo Tianqi Guo,Chen Liping,Peng Ciyuan,Guo Jingjing,Ren Jing*

Main category: cs.LG

TL;DR: 提出NeuroPathNet框架，通过路径级轨迹建模来表征大脑功能分区间连接通路的动态行为，在fMRI数据集上优于现有主流方法


<details>
  <summary>Details</summary>
Motivation: 现有方法难以捕捉特定功能社区间连接的时间演化特征，需要新方法来理解大脑功能网络的时间演化

Method: 基于医学支持的静态分区方案提取功能分区间连接强度时间序列，使用时序神经网络建模路径级轨迹

Result: 在三个公开fMRI数据集上验证，在多个指标上优于现有主流方法

Conclusion: 该研究可促进脑网络分析中动态图学习方法的发展，为神经系统疾病诊断提供可能的临床应用
...
### [189] [Efficient Global-Local Fusion Sampling for Physics-Informed Neural Networks](https://arxiv.org/abs/2510.24026)
*Jiaqi Luo,Shixin Xu,Zhouwang Yang*

Main category: cs.LG

TL;DR: 提出了一种全局-局部融合采样策略(GLFS)，通过结合全局和局部采样的优势，在保持稳定性的同时提高PINNs的效率和准确性。
...
### [190] [Spatio-temporal Multivariate Time Series Forecast with Chosen Variables](https://arxiv.org/abs/2510.24027)
*Zibo Liu,Zhe Jiang,Zelin Xu,Tingsong Xiao,Yupu Zhang,Zhengkun Xiao,Haibo Wang,Shigang Chen*

Main category: cs.LG

TL;DR: 本文研究时空多元时间序列预测中的变量选择问题，提出了一种联合进行变量选择和模型优化的统一框架，通过掩码变量参数剪枝、优先变量参数重放和动态外推机制来提高预测精度和模型效率。


<details>
  <summary>Details</summary>
Motivation: 现有研究假设模型输入中的m个变量是预先确定的，但如何从n个位置中选择m个变量作为模型输入以最大化预测精度的问题从未被研究过。本文填补了这一空白。

Method: 提出包含三个技术组件的统一框架：(1) 掩码变量参数剪枝：通过基于分位数的掩码逐步剪枝信息量较少的变量和注意力参数；(2) 优先变量参数重放：重放低损失的过去样本以保持学习知识的稳定性；(3) 动态外推机制：通过可学习的空间嵌入和邻接信息将信息从输入变量传播到所有其他变量。

Result: 在五个真实世界数据集上的实验表明，该工作在准确性和效率方面显著优于现有最先进的基线方法。
...
### [191] [GraphNet: A Large-Scale Computational Graph Dataset for Tensor Compiler Research](https://arxiv.org/abs/2510.24035)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习计算图数据集缺乏真实世界的多样性和丰富元数据，且缺乏可靠的张量编译器性能评估指标。

Method: 构建包含2.7K真实深度学习计算图的数据集，提出S(t)和ES(t)两个评估指标，在CV和NLP任务上对主流张量编译器进行基准测试。

Result: 创建了GraphNet数据集，开发了性能评估指标，并展示了在CV和NLP样本上的实际应用价值。

Conclusion: GraphNet为张量编译器性能评估提供了实用的数据集和可靠的基准指标，有助于识别性能瓶颈和推动编译器优化。
...
### [192] [Geometric Algorithms for Neural Combinatorial Optimization with Constraints](https://arxiv.org/abs/2510.24039)
*Nikolaos Karalias,Akbar Rafiey,Yifei Xu,Zhishang Luo,Behrooz Tahmasebi,Connie Jiang,Stefanie Jegelka*

Main category: cs.LG

TL;DR: 提出了一种用于组合优化的自监督学习框架，通过凸几何和Carathéodory定理将神经网络输出分解为可行集多面体顶点的凸组合，从而解决离散约束问题。
...
Method: 设计端到端可微分框架，利用凸几何和Carathéodory定理将神经网络输出分解为可行集多面体顶点的凸组合，实现自监督训练和高效保质量的舍入。

Result: 在基数约束优化实验中，该方法持续优于神经基线方法，并可扩展到图独立集和拟阵约束问题等多样化组合优化任务。

Conclusion: 该分解方法为组合优化的自监督学习提供了有效解决方案，能够处理多种离散约束问题并保证解的质量。
...
### [193] [Localized Kernel Projection Outlyingness: A Two-Stage Approach for Multi-Modal Outlier Detection](https://arxiv.org/abs/2510.24043)
*Akira Tamamori*

Main category: cs.LG

TL;DR: 提出了Two-Stage LKPLO多阶段异常检测框架，通过结合广义损失异常度量、全局核PCA线性化和局部聚类，克服传统投影方法的固定统计度量和单一数据结构假设限制。


<details>
  <summary>Details</summary>
Motivation: 解决传统投影异常检测方法的两个主要限制：依赖固定统计度量和假设单一数据结构，这些限制导致在复杂数据结构上性能不佳。

Method: 三阶段方法：1）广义损失异常度量（PLO），使用自适应损失函数如SVM-like损失；2）全局核PCA阶段线性化非线性数据结构；3）局部聚类阶段处理多模态分布。

Result: 在10个基准数据集上的5折交叉验证实验显示，Two-Stage LKPLO达到最先进性能，在多簇数据（Optdigits）和复杂高维数据（Arrhythmia）上显著优于现有方法。消融研究证实核化和局部化阶段的协同组合至关重要。
...
### [194] [Mitigating Negative Transfer via Reducing Environmental Disagreement](https://arxiv.org/abs/2510.24044)
*Hui Sun,Zheng Xie,Hao-Yuan He,Ming Li*

Main category: cs.LG

TL;DR: 本文提出RED方法，通过因果解耦学习减少环境分歧来缓解无监督领域自适应中的负迁移问题


<details>
  <summary>Details</summary>
Motivation: 解决领域自适应中由于领域偏移导致的负迁移问题，特别是非因果环境特征在不同领域间的判别性分歧

Method: 通过对抗训练领域特定的环境特征提取器，将样本解耦为领域不变的因果特征和领域特定的非因果环境特征，然后估计并减少环境分歧

Result: 实验结果表明RED有效缓解了负迁移，并取得了最先进的性能

Conclusion: 通过因果解耦学习减少环境分歧是解决负迁移问题的有效方法
...
### [195] [Causal-Aware Generative Adversarial Networks with Reinforcement Learning](https://arxiv.org/abs/2510.24046)
*Tu Anh Hoang Nguyen,Dang Nguyen,Tri-Nhan Vo,Thuc Duy Le,Sunil Gupta*

Main category: cs.LG

TL;DR: CA-GAN是一个专门为表格数据设计的因果感知生成框架，通过因果图提取和条件WGAN-GP结合强化学习目标，在保持数据效用和隐私的同时捕捉复杂因果关系。


<details>
  <summary>Details</summary>
Motivation: 现有表格数据生成方法（特别是基于GAN的方法）在捕捉复杂因果关系、保持数据效用和提供可证明的隐私保证方面存在不足，限制了在企业环境中的部署。
...
Conclusion: CA-GAN为数据工程师提供了一个实用、高性能的解决方案，可创建高质量、隐私合规的合成数据集，用于数据库系统基准测试、软件开发加速和安全数据驱动研究。
...
### [196] [Learning from History: A Retrieval-Augmented Framework for Spatiotemporal Prediction](https://arxiv.org/abs/2510.24049)
*Hao Jia,Penghao Zhao,Hao Wu,Yuan Gao,Yangyu Tao,Bin Cui*

Main category: cs.LG

TL;DR: 提出检索增强预测（RAP）框架，结合深度学习预测能力和历史数据真实演化，通过检索历史相似状态作为动态指导，提升长期时空预测的物理合理性。
...
Method: RAP框架：1）从大规模数据库中检索与当前状态最相似的历史类比；2）将类比样本的真实未来演化作为参考目标；3）使用双流架构将参考目标作为条件输入，提供动态指导。

Result: 在气象学、湍流和火灾模拟等基准测试中，RAP不仅超越最先进方法，还显著优于仅使用类比预测的基线，生成更物理真实的预测，有效抑制长期预测中的误差发散。

Conclusion: RAP框架通过结合参数化和非参数化方法，成功解决了长期时空预测中的物理合理性挑战，为复杂物理系统的准确预测提供了新范式。
...
### [197] [Low-N Protein Activity Optimization with FolDE](https://arxiv.org/abs/2510.24053)
...
### [198] [FALQON: Accelerating LoRA Fine-tuning with Low-Bit Floating-Point Arithmetic](https://arxiv.org/abs/2510.24061)
...
Method: 提出FALQON框架，直接合并LoRA适配器到FP8量化主干中；重新制定前向和反向计算以减少量化开销；引入行向代理更新机制高效集成大量更新。

Result: 实验评估显示FALQON比现有量化LoRA方法实现约3倍训练加速，同时保持相似的准确度水平。

Conclusion: FALQON为高效大规模模型微调提供了实用解决方案，其端到端FP8工作流无需训练后量化，便于高效部署。
...
### [199] [Information-Theoretic Discrete Diffusion](https://arxiv.org/abs/2510.24088)
*Moongyu Jeon,Sangwoo Shin,Dongjae Jeon,Albert No*

Main category: cs.LG

TL;DR: 提出了离散扩散模型的信息理论框架，通过分数匹配损失获得对数似然的原则性估计器。推导了离散版本的I-MMSE恒等式，建立了信息-最小去噪分数熵关系，将互信息与最小去噪分数熵损失联系起来。


<details>
  <summary>Details</summary>
Motivation: 为离散扩散模型建立信息理论基础，证明常用的损失函数不仅是变分下界，而是紧致且原则性的对数似然估计器。

Method: 引入信息-最小去噪分数熵关系和信息-最小去噪交叉熵关系，将互信息与分数匹配损失联系起来，提供对数似然的时间积分分解。

Result: 实验验证了估计器的准确性、方差稳定性和实用性，支持时间无关公式、条件似然估计和耦合蒙特卡洛似然比估计等扩展应用。

Conclusion: 该框架为离散扩散模型提供了理论支撑，证明常用损失函数是紧致的对数似然估计器，并支持多种实用扩展。
...
### [200] [What do vision-language models see in the context? Investigating multimodal in-context learning](https://arxiv.org/abs/2510.24331)
*Gabriel O. dos Santos,Esther Colombini,Sandra Avila*

Main category: cs.LG

TL;DR: 该研究系统评估了视觉语言模型中的上下文学习能力，发现在多模态整合方面存在局限性，模型主要依赖文本线索而未能有效利用视觉信息。


<details>
  <summary>Details</summary>
Motivation: 尽管上下文学习在大语言模型中已被广泛研究，但在视觉语言模型中的有效性仍未被充分探索，需要系统评估其多模态上下文学习能力。

Method: 评估了7个涵盖4种架构的视觉语言模型在3个图像描述基准上的表现，分析了提示设计、架构选择和训练策略的影响，并首次分析了注意力模式随演示示例数量的变化。

Result: 结果显示：1）图像-文本交错数据训练提升性能但未实现有效的多模态信息整合；2）指令调优改善指令跟随但减少对上下文演示的依赖；3）当前模型主要关注文本线索，未能充分利用视觉信息。

Conclusion: 当前视觉语言模型在多模态上下文学习方面存在关键局限性，需要增强其从多模态上下文示例中学习的能力。
...
### [201] [Learning Parameterized Skills from Demonstrations](https://arxiv.org/abs/2510.24095)
*Vedant Gupta,Haotian Fu,Calvin Luo,Yiding Jiang,George Konidaris*

Main category: cs.LG

TL;DR: DEPS是一种端到端算法，能从专家演示中发现参数化技能。该方法联合学习参数化技能策略和元策略，通过时间变分推理和信息论正则化解决潜在变量模型的退化问题，确保技能具有时间扩展性、语义意义和适应性。


<details>
  <summary>Details</summary>
Motivation: 从多任务专家演示中学习参数化技能能显著提高对未见任务的泛化能力，但现有方法在潜在变量模型中常面临退化问题。

Method: 使用时间变分推理和信息论正则化方法，联合学习参数化技能策略和元策略，元策略在每个时间步选择合适的离散技能和连续参数。

Result: 在LIBERO和MetaWorld基准测试中，DEPS优于多任务和技能学习基线方法，并能发现可解释的参数化技能，如抓取物体的技能，其连续参数定义了抓取位置。

Conclusion: DEPS能有效发现参数化技能，提高任务泛化能力，并产生可解释的技能表示。
...
TL;DR: 提出了Graph-Guided Concept Selection (G2ConS)方法，通过选择重要文档块和构建概念图来降低知识图谱构建成本，同时提高检索效果和回答质量。


<details>
  <summary>Details</summary>
Motivation: 基于图的RAG方法在构建知识图谱时需要大量LLM调用来提取实体和关系，导致规模化应用成本过高。研究发现某些概念词及其相关文档更为重要。

Method: G2ConS包含块选择方法和独立于LLM的概念图。前者选择重要文档块以降低KG构建成本，后者零成本填补块选择引入的知识空白。

Result: 在多个真实数据集上的评估显示，G2ConS在构建成本、检索效果和回答质量方面均优于所有基线方法。

Conclusion: G2ConS通过智能块选择和概念图构建，有效解决了基于图的RAG方法成本高的问题，同时保持了优异的性能表现。
...
### [203] [Local Performance vs. Out-of-Distribution Generalization: An Empirical Analysis of Personalized Federated Learning in Heterogeneous Data Environments](https://arxiv.org/abs/2510.24503)
*Mortesa Hussaini,Jan Theiß,Anthony Stein*

Main category: cs.LG

TL;DR: 该论文提出了一种名为FLIU的个性化联邦学习方法，通过自适应个性化因子来改进FedAvg，在保持泛化能力的同时提升本地性能，并在多种数据分布条件下进行了评估。


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中由于数据异构性导致的客户端漂移问题，传统个性化联邦学习方法过度关注本地性能而忽视了泛化能力，需要平衡本地性能和泛化能力。

Method: 提出了FLIU方法，在FedAvg基础上增加个性化步骤，使用自适应个性化因子；在MNIST和CIFAR-10数据集上，通过IID、病理非IID和Dirichlet分布等多种数据分布条件进行评估。

Result: FLIU方法在保持良好泛化能力的同时，显著提升了本地性能，在各种数据异构环境下都表现优异。

Conclusion: FLIU方法有效解决了联邦学习中的客户端漂移问题，平衡了本地性能和泛化能力，在复杂数据异构环境下具有鲁棒性。
...
### [204] [Causal Convolutional Neural Networks as Finite Impulse Response Filters](https://arxiv.org/abs/2510.24125)
*Kiran Bacsa,Wei Liu,Xudong Jian,Huangbin Liang,Eleni Chatzi*

Main category: cs.LG

TL;DR: 该研究展示了因果卷积神经网络在应用多模态频率时间序列数据时，其行为类似于有限脉冲响应滤波器，特别是当卷积核长度较长时。整个网络可简化为等效的单层FIR滤波器，为理解CNN的频谱学习行为提供了新视角。


<details>
  <summary>Details</summary>
Motivation: 研究因果CNN在处理多模态频率时间序列时的行为特性，探索其与FIR滤波器的相似性，以增强对动态系统建模的可解释性。

Method: 使用具有准线性激活函数的因果CNN，采用长卷积核处理多模态频率时间序列数据，利用卷积的结合性将整个网络简化为等效单层FIR滤波器。

Result: 因果CNN表现出类似FIR滤波器的特性，能够隐式和显式地捕捉频谱特征，在模拟束流动力学和真实桥梁振动数据集上验证了方法的有效性。

Conclusion: 因果CNN可视为优化的FIR滤波器，为理解CNN在稀疏频率内容信号上的频谱学习行为提供了新见解，对动态物理系统的建模和识别具有重要价值。
...
<details>
  <summary>Details</summary>
Motivation: 电动汽车快速发展需要准确高效的锂离子电池诊断，传统元启发式方法计算成本高、收敛慢，现有机器学习方法依赖恒流数据且不实用

Method: 结合神经代理模型NeuralSPMe（在真实EV负载下训练预测锂浓度动态）和参数更新网络PUNet（固定点迭代更新），大幅减少评估时间和迭代次数

Result: 参数识别速度提升2000倍以上，样本效率显著提高，精度比传统元启发式算法高10倍以上，在动态负载场景表现优异

Conclusion: 所提深度学习框架能有效解决电化学电池模型参数识别问题，在计算效率和精度方面均有显著优势，适用于实际应用中的动态负载条件
...
<details>
  <summary>Details</summary>
Motivation: 复杂耗散系统在科学和工程中广泛存在，但远离平衡态时能量耗散和时间不可逆性难以从数据中量化。学习准确且可解释的动力学模型面临挑战：模型需要足够表达能力描述各种过程，又需要足够约束保持物理意义和数学可识别性。

Method: I-OnsagerNet扩展Onsager原理，确保学习到的势能从稳态密度获得，漂移项清晰地分解为时间可逆和时间不可逆分量（如Helmholtz分解所要求）。该方法能够计算熵产生并量化不可逆性。

Result: 在聚合物拉伸流动和随机梯度Langevin动力学中的应用揭示了新见解：势垒高度随应变率呈超线性缩放，熵产生率呈亚线性缩放，以及随着批量大小增加不可逆性被抑制。
...
TL;DR: EddyFormer是一个基于Transformer的谱元架构，用于大规模湍流模拟，结合了谱方法的精度和注意力机制的可扩展性，在256^3分辨率下达到DNS级精度，速度提升30倍，并展示了领域泛化能力。
...
Method: 提出基于Transformer的谱元架构，引入SEM标记化将流动分解为网格尺度和亚网格尺度分量，捕捉局部和全局特征。创建新的三维各向同性湍流数据集进行训练。

Result: 在256^3分辨率下达到DNS级精度，速度提升30倍；在比训练域大4倍的未见域中保持物理不变指标的准确性；在The Well基准测试套件中准确再现复杂动力学。

Conclusion: EddyFormer成功结合了谱方法的精度和注意力机制的可扩展性，为大规模湍流模拟提供了高效准确的解决方案，展示了强大的领域泛化能力。
...
TL;DR: V-SAT是一个统一的视频字幕标注框架，结合LLM、VLM、图像处理和ASR技术，自动检测和纠正多种字幕质量问题，显著提升字幕质量。


<details>
  <summary>Details</summary>
Motivation: 现有字幕生成方法存在同步性差、文本错误、格式不一致、阅读速度不当等问题，且多为孤立解决方案，导致后期编辑工作繁重耗时。
...
Conclusion: V-SAT提供了首个全面的字幕标注解决方案，能够有效解决多种字幕质量问题，为流媒体平台和社交媒体提供高质量的字幕服务。
...
TL;DR: SPEAR++改进了SPEAR攻击，通过应用稀疏字典学习技术，使联邦学习中线性层ReLU激活的梯度反演攻击在更大批量下变得可行，同时保持对DP噪声和FedAvg聚合的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的梯度反演攻击缺乏形式化保证，且SPEAR攻击在批量大小上具有指数级时间复杂度，限制了其在真实场景中的实用性。

Method: 应用最先进的稀疏字典学习技术来处理线性层ReLU激活的梯度反演问题，使其在计算上可行。

Result: SPEAR++在保持SPEAR所有理想特性的同时，能够处理10倍更大的批量大小，且对DP噪声和FedAvg聚合具有鲁棒性。

Conclusion: SPEAR++显著提升了梯度反演攻击的实用性，使联邦学习的隐私保护特性面临更严峻的挑战。
...
Result: 在多样化基准测试中，SPARK显著优于最先进的基线方法，特别是在具有挑战性的分布外场景和数据稀缺情况下表现优异。
...
TL;DR: 本文比较了ICU生命体征数据缺失值的插补方法，旨在为临床预测模型选择最准确的插补技术提供指导。


<details>
  <summary>Details</summary>
Motivation: ICU数据中生命体征测量存在大量缺失段，这会影响机器学习预测性能，但目前缺乏对代表性插补方法的全面比较，且实际中仍在使用可能降低预测准确性的临时插补技术。

Method: 创建了一个可扩展和可重用的基准测试框架，包含15种插补方法和4种缺失方法，用于在主要ICU数据集上进行基准测试。

Result: 通过系统比较建立了不同插补技术的性能基准，为研究人员提供了选择最准确插补方法的依据。

Conclusion: 该工作为ICU数据插补提供了比较基础，有助于促进更多机器学习模型进入临床实践。
...
TL;DR: 提出了PRIVET方法，通过极值统计和最近邻距离来检测深度生成模型中的隐私泄露，能够为每个合成样本分配个体隐私泄露分数。


<details>
  <summary>Details</summary>
Motivation: 深度生成模型常在敏感数据上训练，存在隐私泄露风险，现有方法主要依赖全局标准，缺乏样本级别的可解释隐私评估方法。
...
TL;DR: 提出了稀疏乐观信息导向采样算法，能够在数据丰富和数据匮乏两种情况下同时达到最优最坏情况遗憾，无需贝叶斯假设。


<details>
  <summary>Details</summary>
Motivation: 现有算法只能在数据丰富或数据匮乏其中一种情况下达到最优遗憾，无法同时适应两种场景。稀疏IDS算法虽然能在贝叶斯设置下实现这一目标，但需要贝叶斯假设。

Method: 使用稀疏乐观信息导向采样算法，通过时间依赖的学习率来平衡信息和遗憾，扩展了IDS的理论保证。

Result: SOIDS算法在数据丰富和数据匮乏两种情况下都能达到最优最坏情况遗憾，实验验证了其良好性能。

Conclusion: SOIDS是首个能在数据丰富和数据匮乏两种情况下同时达到最优最坏情况遗憾的算法，无需贝叶斯假设。
...
TL;DR: 提出了PaTaRM框架，通过偏好感知奖励机制和动态标准适应，统一了成对和点式奖励建模方法，无需显式点式标签即可构建鲁棒的点式训练信号。
...
Method: 结合偏好感知奖励机制和任务自适应标准系统，利用成对数据中的相对偏好信息构建点式训练信号，同时动态生成评估标准。

Result: 在RewardBench和RMBench上平均相对提升4.7%，在IFEval和InFoBench基准测试中下游RLHF性能平均提升13.6%。

Conclusion: PaTaRM提供了高效、可泛化且可解释的奖励建模方法，显著提升了RLHF性能。
...
TL;DR: 扩展TLogic框架，引入实体类别作为规则的关键组成部分，通过LLM生成未知类别，提高时序知识图谱预测的准确性和可解释性。
...
Method: 扩展TLogic规则框架，将实体类别纳入规则格式；使用LLM方法生成未知实体类别；研究类别预测中实体分数聚合方法的选择。

Result: 该方法在保持高准确率的同时提供了可解释的预测，增强了透明度。

Conclusion: 结合实体类别的规则框架能够有效提高时序知识图谱预测的可解释性和准确性。
...
TL;DR: SALS框架通过将KV缓存投影到紧凑的潜在空间，并在该空间中进行稀疏token选择，有效解决了长上下文LLM推理中的KV缓存压缩问题，避免了RoPE机制导致的性能下降。


<details>
  <summary>Details</summary>
Motivation: 长上下文LLM的推理面临KV缓存大小和内存带宽的挑战，现有低秩压缩方法因RoPE机制导致严重精度损失或速度瓶颈。

Method: 提出SALS框架：将KV缓存通过低秩投影到潜在空间，在该空间中进行RoPE无关的查询-键交互来执行稀疏token选择，仅重构重要token子集。

Result: 在多个模型上验证，SALS实现6.4倍KV缓存压缩和5.7倍注意力算子加速，端到端吞吐量相比GPT-fast提升1.4-4.5倍。

Conclusion: SALS通过潜在空间稀疏注意力机制，在保持竞争性精度的同时实现了SOTA性能，有效解决了长上下文LLM推理的挑战。
...
TL;DR: 提出EDC框架，将方程发现技术应用于二元分类，通过发现决策边界的解析函数实现可解释分类模型


<details>
  <summary>Details</summary>
Motivation: 方程发现在回归任务中已证明有效，但缺乏在分类任务中的应用。本文旨在将方程发现扩展到二元分类，寻找可解释的决策边界函数

Method: 提出EDC框架，使用包含线性、二次、指数项和特征乘积的语法来构建决策边界函数，避免过拟合同时保持灵活性

Result: 在人工和真实数据集上，EDC能够发现目标方程结构和参数值，在二元分类中优于现有ED方法，达到与最先进分类方法相当的性能

Conclusion: EDC为二元分类提供了可解释的方程发现方法，语法复杂度可配置，特别适合包含领域特定表达式的场景
...
TL;DR: Cluster-PFN是一个基于Transformer的模型，通过在高斯混合模型先验上训练，能够高效进行贝叶斯聚类，准确估计聚类数量和分配，并在处理缺失数据时优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯聚类方法计算成本高，且现实数据常包含缺失值，简单的插补方法会忽略不确定性导致结果不理想。

Method: 使用Transformer架构扩展先验数据拟合网络，完全在合成数据集上训练，学习估计聚类数量和分配的后验分布。

Result: 比AIC、BIC和变分推理更准确估计聚类数量，聚类质量与变分推理相当但速度快几个数量级，在基因组数据上处理高缺失率时优于插补基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法将感知和决策联合优化，导致感知质量难以评估和保证。PeL旨在分离这两个过程，使感知接口能够独立于具体任务进行优化，确保感知质量。

Method: 定义感知接口f_φ和决策函数g_θ的分离，使用任务无关信号优化感知属性，包括对扰动的稳定性、信息性不塌缩和几何控制，通过表示不变性指标评估。

Result: 证明了PeL更新在保持足够不变性时与贝叶斯任务风险梯度正交，提供了一套任务无关的评估指标来认证感知质量。

Conclusion: PeL范式能够有效分离感知和决策学习，通过任务无关信号优化感知接口，确保感知质量的可认证性，为感知学习提供了新的理论框架。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习模型在医疗等高风险领域应用广泛，但现有模型往往忽视不确定性，即使置信度低也提供预测，这会影响可靠性

Method: 1. 训练阶段使用实例硬度(IH)过滤问题实例优化数据集；2. 推理阶段引入置信度拒绝机制，仅保留可靠预测

Result: 在三个真实医疗数据集上验证，该方法能有效提升模型可靠性，同时平衡预测性能和拒绝率，保留大部分实例

Conclusion: 结合IH过滤和置信度拒绝的方法为安全关键应用中部署ML系统提供了实用方案
...
TL;DR: 提出了一个代表性-隐私-效用（RPU）框架，用于系统评估公共交通合成出行数据，填补了现有评估方法的空白。


<details>
  <summary>Details</summary>
Motivation: 合成数据虽然能解决智能卡数据的隐私和可访问性问题，但现有评估方法碎片化，缺乏对群体层面变化和任务特定效用的考虑。

Method: 开发了RPU框架，在三个互补维度（代表性、隐私、效用）和三个层次（记录、群体、总体）上系统评估合成出行数据，整合了一致的度量标准。

Result: 评估了12种代表性生成方法，发现合成数据不能固有保证隐私，不存在"一刀切"模型，隐私与代表性/效用之间存在明显权衡。CTGAN提供了最平衡的权衡。
...
TL;DR: 提出了APEX方法，用于在计算预算有限的情况下对大型组合合成库进行虚拟筛选，通过神经网络代理实现近似但全面的搜索，能在消费级GPU上在一分钟内完成全库枚举。


<details>
  <summary>Details</summary>
Motivation: 现有的组合合成库规模庞大（数十亿化合物），但虚拟筛选通常只能评估不到0.1%的化合物，导致许多高分化合物未被发现。现有算法在约束条件变化时缺乏复用性。

Method: APEX利用神经网络代理，利用组合合成库的结构来预测目标和约束条件，实现消费级GPU上的全库快速枚举，能够精确检索近似的top-k化合物集合。

Result: 开发了包含1000多万化合物的基准CSL，并标注了5个医学相关靶点的对接分数和理化性质。APEX在检索准确性和运行时间方面均优于替代方法。

Conclusion: APEX方法能够在计算预算有限的情况下有效识别高分化合物，为大型组合合成库的虚拟筛选提供了高效解决方案。
...
TL;DR: 提出使用少量成功演示初始化RL智能体价值函数的简单有效方法，通过离线演示预计算价值估计作为早期学习目标，显著提升稀疏奖励环境中的样本效率
...
Method: 混合离线到在线范式：使用离线演示预计算价值估计作为早期学习目标，然后通过标准在线交互进行精炼

Result: 在基准任务上的实验表明，该方法加速收敛并优于标准基线，即使使用最少或次优演示数据也能取得良好效果

Conclusion: 通过利用少量演示数据初始化价值函数，可以有效降低探索负担，提高稀疏奖励环境中的强化学习性能
...
<details>
  <summary>Details</summary>
Motivation: 传统生存分析方法在处理复杂数据时存在局限性，需要评估现代机器学习模型在生存分析中的表现，以改进生存预测和决策支持。

Method: 使用巴西圣保罗医院癌症登记处近45,000名结直肠癌患者数据，评估了六种机器学习生存分析模型（RSF、GBSA、SSVM、XGB-Cox、XGB-AFT、LGBM），采用超参数优化和多种性能指标（C-Index、IPCW、AUC、IBS）进行评估。

Result: XGB-AFT模型表现最佳（C-Index = 0.7618；IPCW = 0.7532），其次是GBSA和RSF模型。

Conclusion: 机器学习生存分析方法在改善生存预测和决策支持方面具有重要潜力和应用价值。
...
### [225] [Sample-efficient and Scalable Exploration in Continuous-Time RL](https://arxiv.org/abs/2510.24482)
*Klemens Iten,Lenart Treven,Bhavya Sukhija,Florian Dörfler,Andreas Krause*

Main category: cs.LG

TL;DR: 提出COMBRL算法解决连续时间强化学习问题，使用概率模型学习非线性ODE系统，通过贪婪最大化奖励和模型不确定性实现可扩展的样本高效方法。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习算法通常设计用于离散时间动态，但现实控制系统往往是连续时间的，需要解决连续时间环境下的强化学习问题。

Method: 利用高斯过程和贝叶斯神经网络等概率模型学习ODE系统的不确定性感知模型，COMBRL算法贪婪地最大化外部奖励和模型认知不确定性的加权和。
...
### [226] [MIMIC-Sepsis: A Curated Benchmark for Modeling and Learning from Sepsis Trajectories in the ICU](https://arxiv.org/abs/2510.24500)
*Yong Huang,Zhongqi Yang,Amir Rahmani*

Main category: cs.LG

TL;DR: MIMIC-Sepsis是一个基于MIMIC-IV数据库的脓毒症研究框架，包含35,239名ICU患者数据，提供标准化预处理流程和基准任务，证明纳入治疗变量能显著提升模型性能。
...
Result: 实证结果表明，纳入治疗变量能显著提升模型性能，特别是基于Transformer的架构表现更佳。
...
### [227] [LoRA-DA: Data-Aware Initialization for Low-Rank Adaptation via Asymptotic Analysis](https://arxiv.org/abs/2510.24561)
*Qingyue Zhang,Chang Chu,Tianren Peng,Qi Li,Xiangyang Luo,Zhihao Jiang,Shao-Lun Huang*

Main category: cs.LG

TL;DR: 提出了一种基于渐近分析的数据感知LoRA初始化理论框架，开发了LoRA-DA算法，通过结合目标域数据显著提升微调性能。


<details>
  <summary>Details</summary>
Motivation: 现有LoRA初始化方法存在局限性：许多方法未利用目标域数据，而基于梯度的方法仅依赖一步梯度分解，理论基础薄弱且依赖限制性假设，性能不理想。

Method: 建立基于渐近分析的理论框架，从最小化微调模型与目标模型参数差异的优化目标出发，推导包含偏置项和方差项的优化问题，开发LoRA-DA算法从少量目标域样本估计最优初始化。

Result: 在多个基准测试中，LoRA-DA相比现有初始化方法持续提升最终准确率，表现出更快、更稳定的收敛性，对秩变化具有鲁棒性，且初始化开销小。

Conclusion: 提出的数据感知LoRA初始化框架和LoRA-DA算法有效解决了现有方法的局限性，显著提升了微调性能，具有实际应用价值。
...
### [228] [DistDF: Time-Series Forecasting Needs Joint-Distribution Wasserstein Alignment](https://arxiv.org/abs/2510.24574)
...
Method: 提出DistDF方法，引入联合分布Wasserstein差异来上界条件差异，该差异可从经验样本中可微估计，并与基于梯度的训练无缝集成。

Result: 实验表明DistDF提高了多种预测模型的性能，并达到了最先进的预测性能。
...
### [229] [Physics-Informed Extreme Learning Machine (PIELM): Opportunities and Challenges](https://arxiv.org/abs/2510.24577)
*He Yang,Fei Ren,Hai-Sui Yu,Xiaohui Chen,Pei-Zhi Zhuang*

Main category: cs.LG

TL;DR: 本文是一篇关于物理信息极端学习机(PIELM)的综述，总结了该领域的发展现状、成功应用和面临的挑战。
...
Result: PIELM在求解具有尖锐梯度、非线性、高频行为、硬约束、不确定性和多物理场耦合的PDE方面取得了显著成功。

Conclusion: 虽然PIELM发展迅速且成果显著，但仍面临许多紧迫挑战，这为开发更鲁棒、可解释和可推广的PIELM框架提供了机会。
...
### [230] [A Novel XAI-Enhanced Quantum Adversarial Networks for Velocity Dispersion Modeling in MaNGA Galaxies](https://arxiv.org/abs/2510.24598)
*Sathwik Narkedimilli,N V Saran Kumar,Aswath Babu H,Manjunath K Vanahalli,Manish M,Vinija Jain,Aman Chadha*

Main category: cs.LG

TL;DR: 提出了一种结合量子神经网络与经典深度学习层的新型量子对抗框架，通过LIME可解释性评估器指导模型优化，在保持预测精度的同时提升模型可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决当前量子机器学习方法在预测精度、鲁棒性和可解释性之间难以平衡的问题，推动量子机器学习超越现有局限性。

Method: 采用混合量子神经网络与经典深度学习层的对抗框架，集成LIME可解释性评估器，并通过量子GAN和自监督变体进行扩展，使用对抗评估器计算反馈损失来同时优化预测精度和模型可解释性。

Result: Vanilla模型在回归指标上表现最一致：RMSE=0.27，MSE=0.071，MAE=0.21，R^2=0.59，优于对抗变体。

Conclusion: 量子启发方法与经典架构的结合能够开发轻量级、高性能且可解释的预测模型，扩展了量子机器学习的应用潜力。
...
### [231] [Semi-supervised and unsupervised learning for health indicator extraction from guided waves in aerospace composite structures](https://arxiv.org/abs/2510.24614)
*James Josep Perry,Pablo Garcia-Conde Ortiz,George Konstantinou,Cornelie Vergouwen,Edlyn Santha Kumaran,Morteza Moradi*

Main category: cs.LG

TL;DR: 提出了一个数据驱动框架，通过半监督和无监督学习方法结合多域信号处理来学习复合材料结构的健康指标，解决了传统方法无法捕捉中间退化状态的问题。
...
Method: 提出两种方法：(1) 多样性深度半监督异常检测(Diversity-DeepSAD)，使用连续辅助标签作为假设损伤代理；(2) 退化趋势约束变分自编码器(DTC-VAE)，通过显式趋势约束嵌入单调性准则。使用多频导波监测复合材料结构疲劳加载。
...
### [232] [Symbolic Snapshot Ensembles](https://arxiv.org/abs/2510.24633)
*Mingyue Liu,Andrew Cropper*

Main category: cs.LG

TL;DR: 提出一种单次训练即可获得多个假设的ILP集成方法，通过保存中间假设并使用最小描述长度加权方案组合，在多个基准测试中提升预测准确率4%，计算开销不到1%。


<details>
  <summary>Details</summary>
Motivation: 传统的ILP集成方法需要多次训练来学习多个假设，计算成本高。本文旨在通过单次训练获得多个假设来降低计算开销。

Method: 在单次ILP训练过程中保存中间假设，然后使用最小描述长度加权方案来组合这些假设。

Result: 在游戏玩法和视觉推理等多个基准测试中，该方法将预测准确率提高了4%，而计算开销不到1%。

Conclusion: 该方法提供了一种高效的ILP集成学习方式，能够在显著降低计算成本的同时提高预测性能。
...
### [233] [Causal Ordering for Structure Learning From Time Series](https://arxiv.org/abs/2510.24639)
*Pedro P. Sanchez,Damian Machlanski,Steven McDonagh,Sotirios A. Tsaftaris*

Main category: cs.LG

TL;DR: DOTS提出了一种基于扩散因果发现的时间序列因果结构学习方法，通过整合多个有效因果排序而非单一排序，有效恢复底层有向无环图的传递闭包，在合成和真实数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 时间序列因果发现面临组合复杂性挑战，传统基于排序的方法限制了模型表示能力。为解决单一排序方法的局限性，需要利用多个有效因果排序来提升性能。

Method: 使用基于扩散的因果发现方法，在平稳性和加性噪声模型假设下，通过扩散过程的得分匹配实现高效Hessian估计，整合多个因果排序来恢复因果结构。

Result: 在合成数据集上，DOTS将平均窗口图F1从基线最佳0.63提升到0.81；在CausalTime真实基准上获得最高平均摘要图F1，同时将运行时间减半。

Conclusion: DOTS为时间序列因果发现提供了一个可扩展且准确的解决方案，通过多排序方法有效克服了传统方法的局限性。
...
### [234] [The Cost of Robustness: Tighter Bounds on Parameter Complexity for Robust Memorization in ReLU Nets](https://arxiv.org/abs/2510.24643)
*Yujun Kim,Chaewon Moon,Chulhee Yun*

Main category: cs.LG

TL;DR: 该论文研究了ReLU网络稳健记忆的参数复杂性，分析了在不同稳健性比率ρ=μ/ε下所需的参数数量上下界。


<details>
  <summary>Details</summary>
Motivation: 研究ReLU网络在保证训练样本周围μ球内预测一致性的前提下，插值任意数据集所需的参数复杂性，填补了现有工作在ρ∈(0,1)整个范围内精细分析的空白。

Method: 通过理论分析建立参数数量的上下界，考虑稳健性比率ρ=μ/ε的影响，并与非稳健记忆的参数复杂性进行比较。

Result: 发现当ρ较小时，稳健记忆的参数复杂性与非稳健记忆相同，但随着ρ增大，参数复杂性会增长。获得了比现有结果更紧密的上下界。

Conclusion: 稳健记忆的参数复杂性取决于稳健性比率ρ，在ρ较小时与非稳健记忆相当，但随着ρ增大而增加，这为理解神经网络稳健性提供了重要理论洞见。
...
### [235] [Pearl: A Foundation Model for Placing Every Atom in the Right Location](https://arxiv.org/abs/2510.24670)
...
Method: 采用三种关键创新：(1)包含大规模合成数据的训练方案；(2)SO(3)等变扩散模块的架构，尊重3D旋转对称性；(3)可控推理，包括支持蛋白质和非聚合组件的多链模板系统。

Result: 在Runs N' Poses和PoseBusters基准测试中，Pearl在生成准确(RMSD < 2Å)且物理有效构象方面超越了AlphaFold 3和其他开源基线，分别提高了14.5%和14.2%。在口袋条件共折叠机制下，在更严格的RMSD < 1Å阈值下，对专有挑战性药物靶点实现了3.6倍改进。
...
### [236] [Eigenfunction Extraction for Ordered Representation Learning](https://arxiv.org/abs/2510.24672)
*Burak Varıcı,Che-Ping Tsai,Ritabrata Ray,Nicholas M. Boffi,Pradeep Ravikumar*

Main category: cs.LG

TL;DR: 提出了一个提取有序可识别特征函数的通用框架，通过模块化构建块实现与上下文核的兼容性和可扩展性，验证了特征值作为特征选择重要性指标的有效性。
...
Result: 在合成核和真实图像数据集上验证，恢复的特征值可作为特征选择的有效重要性评分，实现自适应维度表示的效率-精度权衡。

Conclusion: 该框架能够提取有序可识别的特征函数，为特征重要性分析和自适应表示提供了理论基础和实践方法。
...
### [237] [Learning to Drive Safely with Hybrid Options](https://arxiv.org/abs/2510.24674)
*Bram De Cooman,Johan Suykens*

Main category: cs.LG

TL;DR: 该论文将选项框架应用于高速公路自动驾驶任务，通过定义纵向和横向操作的专用选项，结合安全性和舒适性约束，提出分层控制方法，并开发了基于最先进强化学习技术的实用算法。


<details>
  <summary>Details</summary>
Motivation: 尽管选项框架天然适合分层控制应用和自动驾驶任务，但在深度强化学习的自动驾驶方法中很少使用该框架。因此，本研究旨在将选项框架应用于高速公路自动驾驶，以融入领域知识并更轻松地约束学习到的驾驶行为。

Method: 定义了具有嵌入安全性和舒适性约束的纵向和横向操作专用选项；提出了几种分层控制设置；开发了基于最先进强化学习技术的实用算法；通过分别选择纵向和横向控制动作，引入了组合和混合选项的策略。

Result: 在所有研究的方法中，基于混合选项的灵活策略在不同交通条件下表现最佳，优于基于动作的基线策略。这些策略具有与人类驾驶员相同的表达能力和灵活性，同时比传统的连续动作策略更易于解释。
...
### [238] [Greedy Sampling Is Provably Efficient for RLHF](https://arxiv.org/abs/2510.24700)
*Di Wu,Chengshuai Shi,Jing Yang,Cong Shen*

Main category: cs.LG

TL;DR: 本文研究了RLHF的理论基础，针对一般偏好模型提出了性能保证，发现贪婪采样足以实现最优性能，这改进了现有结果。
...
Method: 使用经验估计（即贪婪采样）而非构建乐观或悲观估计，利用KL正则化目标下最优策略类的独特结构特性。

Result: 获得了比现有结果有数量级改进的性能保证，并专门针对BT模型验证了贪婪采样的充分性。

Conclusion: 揭示了在RLHF中贪婪采样的惊人充分性，这源于KL正则化目标下最优策略类的独特结构特性。
