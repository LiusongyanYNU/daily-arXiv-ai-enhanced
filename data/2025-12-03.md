...
Motivation: 在多媒体制作中，音频轨道需要单独处理每个声源以实现精确编辑、混音和创意控制。然而，当前方法一次性生成混合声音，因为视觉特征纠缠且区域提示通常无法指定特定声源。

Method: 提出SelVA模型，将文本提示作为目标声源的显式选择器，调制视频编码器以提取提示相关特征；使用补充令牌通过抑制文本无关激活来促进跨注意力；采用自增强方案克服单声道音频监督数据的缺乏。
...
### [2] [HUD: Hierarchical Uncertainty-Aware Disambiguation Network for Composed Video Retrieval](https://arxiv.org/abs/2512.02792)
*Zhiwei Chen,Yupeng Hu,Zixu Li,Zhiheng Fu,Haokun Wen,Weili Guan*

Main category: cs.CV

TL;DR: 提出HUD框架解决组合视频检索中多模态查询的信息密度差异问题，通过层次化不确定性感知消歧网络提升目标视频检索精度


<details>
  <summary>Details</summary>
Motivation: 组合视频检索任务中，视频模态比文本模态包含更丰富的语义信息，但先前工作忽视了这种信息密度差异，导致修改主体指代模糊和细节语义关注有限的问题

Method: 提出HUD框架，包含三个关键组件：整体代词消歧、原子不确定性建模、整体到原子对齐，通过利用重叠语义的跨模态交互和细粒度语义对齐实现有效对象消歧

Result: 在三个基准数据集上为CVR和CIR任务实现了最先进的性能，代码已开源

Conclusion: HUD是首个利用视频与文本信息密度差异来增强多模态查询理解的框架，通过层次化不确定性感知消歧机制有效解决了组合检索中的关键问题
...
TL;DR: 本研究通过微调TerraMind地理空间基础模型，利用FloodsNet多模态数据集（包含85个全球洪水事件的Sentinel-1 SAR和Sentinel-2光学影像），评估了不同配置在洪水范围制图上的性能，发现基础模型解冻配置在精度和计算成本间取得最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 洪水是最具破坏性的天气相关灾害之一，2024年极端洪水事件影响五大洲。虽然地球观测卫星为洪水制图提供关键数据，但操作精度严重依赖标注数据集和模型泛化能力。新兴的地理空间基础模型（如ESA-IBM的TerraMind）通过大规模自监督预训练提高了泛化性，但其在全球多样洪水事件上的性能尚不明确。

Method: 使用FloodsNet多模态数据集（包含85个全球洪水事件的Sentinel-1 SAR和Sentinel-2光学影像）微调TerraMind模型。测试了四种配置：基础模型vs大型模型、冻结vs解冻骨干网络。与TerraMind Sen1Floods11示例以及在FloodsNet和Sen1Floods11上训练的U-Net进行比较。

Result: 基础模型解冻配置在精度、精确度和召回率方面取得最佳平衡，且计算成本显著低于大型模型。大型解冻模型获得最高召回率。FloodsNet训练的模型在召回率上优于Sen1Floods11训练的示例，总体精度相似。U-Net在所有GFM配置中召回率最高，但精度和精确度略低。

Conclusion: 整合多模态光学和SAR数据并微调地理空间基础模型可以增强近实时洪水制图能力。本研究提供了首批全球尺度的GFM洪水分割评估，凸显了其在气候适应和灾害韧性方面的潜力和当前局限性。
...
### [4] [MRD: Multi-resolution Retrieval-Detection Fusion for High-Resolution Image Understanding](https://arxiv.org/abs/2512.02906)
*Fan Yang,Kaihao Zhang*

Main category: cs.CV

TL;DR: 提出MRD框架，通过多分辨率语义融合和开放词汇目标检测解决高分辨率图像理解中目标对象被分割的问题


<details>
  <summary>Details</summary>
Motivation: 现有方法将高分辨率图像分割成小块进行语义相似度计算，但会导致完整对象被分割到多个小块中，破坏语义相似度计算的准确性

Method: 提出训练免费的MRD框架：1) 多分辨率语义融合方法，整合不同分辨率下的语义相似度图；2) 引入开放词汇目标检测模型，通过滑动窗口方法识别目标区域

Result: 在不同多模态大语言模型上进行的高分辨率图像理解基准测试中证明了方法的有效性

Conclusion: MRD框架能够更准确地处理高分辨率图像中的目标对象，保持对象完整性，提高语义相似度计算的准确性
...
<details>
  <summary>Details</summary>
Motivation: 对比学习在众多基准测试中表现出色，但对比损失函数可能引入信息失真。模型可能过度依赖相同标签样本的信息，而忽略来自同一原始图像的正样本对，特别是在大规模数据集中。
...
Conclusion: 该方法能有效解决对比学习中的信息失真问题，提高学习效率和公平性，在系统性失真任务中表现突出，为更高效、公平的下游训练提供了有前景的解决方案。
...
### [6] [FineGRAIN: Evaluating Failure Modes of Text-to-Image Models with Vision Language Model Judges](https://arxiv.org/abs/2512.02161)
*Kevin David Hayes,Micah Goldblum,Vikash Sehwag,Gowthami Somepalli,Ashwinee Panda,Tom Goldstein*

Main category: cs.CV

TL;DR: 本文提出了一种联合评估文本到图像生成模型和视觉语言模型的结构化方法，通过测试VLM能否识别T2I模型生成的图像中的27种特定失败模式，并创建了包含5个T2I模型生成图像和VLM标注的数据集。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型虽然能生成视觉上令人印象深刻的图像，但常常无法准确捕捉用户提示中的特定属性（如正确数量的对象和指定颜色）。同时，视觉语言模型的基准测试未能跟上VLM用于标注的复杂场景需求。需要一种分层评估框架来比较不同图像生成模型的提示遵循能力。

Method: 提出结构化方法联合评估T2I模型和VLM：测试VLM能否识别T2I模型在挑战性提示下生成的图像中的27种特定失败模式。创建包含5个T2I模型（Flux、SD3-Medium、SD3-Large、SD3.5-Medium、SD3.5-Large）生成图像的数据集，由VLM（Molmo、InternVL3、Pixtral）标注，并由LLM（Llama3）验证VLM是否正确识别失败模式。

Result: 通过分析精心策划的提示集上的失败模式，揭示了属性保真度和对象表示方面的系统性错误。发现当前指标不足以捕捉这些细微错误，强调了针对性基准测试对于推进生成模型可靠性和可解释性的重要性。

Conclusion: 本文提出的联合评估框架和数据集揭示了T2I模型在属性保真度方面的系统性错误，同时展示了VLM在识别这些错误方面的能力。研究强调了需要更精细的评估方法来推动生成模型的可靠性和可解释性发展。
...
### [7] [Mapping of Lesion Images to Somatic Mutations](https://arxiv.org/abs/2512.02162)
*Rahul Mehta*

Main category: cs.CV

TL;DR: LLOST模型使用双变分自编码器，通过共享潜在空间将医学影像点云表示与体细胞突变计数关联，实现从影像预测癌症基因突变


<details>
  <summary>Details</summary>
Motivation: 癌症治疗有效性依赖于早期诊断，但当前临床实践中影像诊断与基因检测分离。需要建立从医学影像直接预测基因突变的模型，实现早期、全面的癌症诊断

Method: 1) 将病灶图像转换为点云表示以实现模态不变性；2) 构建双变分自编码器，分别处理点云和突变计数；3) 通过共享潜在空间连接两个域；4) 使用条件归一化流先验学习三个潜在空间的分布

Result: 模型在TCIA医学影像和TCGA Pan Cancer数据集上验证，能够准确预测特定突变计数和突变发生，并发现影像与体细胞突变域之间的共享模式反映癌症类型

Conclusion: LLOST模型成功建立了医学影像与基因突变之间的关联，为早期癌症诊断提供了新方法。未来可扩展到其他遗传领域，并进一步改进模型性能
...
TL;DR: SplatSuRe：一种选择性应用超分辨率的方法，只在缺乏高频监督的欠采样区域添加SR内容，从而在3D高斯溅射中实现更清晰、更一致的渲染


<details>
  <summary>Details</summary>
Motivation: 现有方法在3D高斯溅射中对所有图像统一应用超分辨率，导致多视图不一致和模糊渲染。需要一种更智能的方法来选择性增强分辨率

Method: 基于相机姿态相对于场景几何的信息，选择性在欠采样区域应用超分辨率内容，利用近距离低分辨率视图中的高频信息来增强远处视图的细节

Result: 在Tanks & Temples、Deep Blending和Mip-NeRF 360数据集上，该方法在保真度和感知质量方面均超越基线方法，特别是在需要更高细节的局部前景区域效果最显著

Conclusion: 通过选择性应用超分辨率内容，SplatSuRe能够产生更清晰、更一致的渲染结果，特别适用于需要高细节的前景区域，为3D高斯溅射的高分辨率渲染提供了有效解决方案
...
TL;DR: 提出RobustSurg方法，通过利用手术场景的风格和内容信息、实例归一化和特征协方差映射技术，以及引入恢复模块来提升手术场景分割的泛化能力，在未见中心和模态上取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习手术场景分割方法在单中心和单成像模态上表现良好，但难以泛化到未见分布（其他中心）和未见模态。自然场景的域泛化方法不能直接应用于手术场景，因为手术场景视觉线索有限且场景极其多样化。

Method: 1. 利用手术场景的风格和内容信息，通过实例归一化和特征协方差映射技术获得鲁棒且可泛化的特征表示；2. 在ResNet骨干网络中引入恢复模块，保留与任务相关的有用特征；3. 提供新的多类多中心手术场景分割数据集。

Result: 在未见中心HeiCholSeg数据集上，RobustSurg比基线DeepLabv3+提升近23%，比SOTA方法提升10-32%（mIoU）；在EndoUDA息肉数据集的目标集上，比基线提升近22%，比最新SOTA方法提升近11%。

Conclusion: 通过利用风格和内容信息、实例归一化、特征协方差映射以及恢复模块，RobustSurg能够有效提升手术场景分割的泛化能力，在未见中心和模态上取得显著性能改进。
...
<details>
  <summary>Details</summary>
Motivation: 现有端到端多分形方法依赖大量池化或强特征空间抽取，限制了语义分割等任务。多分形分析在捕捉病理规律方面已被证明有效，但在现代深度学习中应用有限。

Method: 提出两种归纳先验：单分形重校准和多分形重校准，利用指数概率质量与多分形谱之间的关系形成编码器嵌入的统计描述，实现为卷积网络中的通道注意力函数，在U-Net架构中应用。

Result: 在三个公开医学影像数据集（ISIC18皮肤镜、Kvasir-SEG内窥镜、BUSI超声）上验证，多分形重校准相比使用高阶统计的其他通道注意力机制基线取得显著提升。分析发现注意力层响应在U-Net中不会随编码器深度增加而过度专门化，其有效性可能与实例变异的全局统计相关。

Conclusion: 多分形重校准方法有效提升了医学图像分割性能，为深度学习中多分形分析的应用提供了新思路，同时揭示了U-Net架构中注意力层的行为特性。
...
TL;DR: 提出Unified-VQA框架，通过诊断混合专家方法实现通用视频质量评估，能同时输出全局质量分数和可解释的多维伪影向量。


<details>
  <summary>Details</summary>
Motivation: 现有视频质量评估方法存在三个主要问题：1）只能预测单一质量分数，缺乏诊断性和可解释性反馈；2）大多是特定格式的专用指标而非真正通用方案；3）从不同感知域学习妥协表示而非真正统一模型。

Method: 提出Unified-VQA框架，将通用VQA重新定义为诊断混合专家问题。采用多个"感知专家"处理不同感知域，设计新颖的多代理专家训练策略，使用排名启发损失优化每个专家。集成诊断多任务头，通过弱监督学习策略优化全局质量分数和可解释的多维伪影向量。

Result: 在静态模型参数（无需重新训练或微调）下，Unified-VQA在17个包含HD、UHD、HDR和HFR格式的数据库上，相比18个基准方法在通用VQA和诊断伪影检测任务中均表现出一致且优越的性能。

Conclusion: 这项工作代表了向实用、可操作和可解释的视频质量评估迈出的重要一步，通过统一的诊断混合专家框架解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频基准测试很少评估对人类语音的细粒度推理，许多任务要么可以通过视觉解决，要么只粗略评估语音，无法真正测试模型是否能对齐说话人、内容和时间。

Method: 设计了AV-SpeakerBench基准测试，具有三个特点：1) 以说话人为核心推理单元而非场景；2) 融合基础的问题设计，将视听依赖嵌入问题语义；3) 专家标注确保时间精度和跨模态有效性。

Result: 评估显示Gemini系列模型表现最佳，Gemini 2.5 Pro取得最好结果。开源模型中Qwen3-Omni-30B接近Gemini 2.0 Flash，但仍远落后于Gemini 2.5 Pro，主要差距在于视听融合能力而非视觉感知。

Conclusion: AV-SpeakerBench为推进未来多模态系统的细粒度视听推理建立了严谨的基础，揭示了当前模型在视听融合方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 脉冲神经网络在低层视觉任务中探索不足，传统脉冲神经元缺乏空间上下文理解能力，且存在频域饱和限制

Method: 提出视觉LIF神经元解决空间上下文理解问题，设计脉冲分解增强模块和轻量级脉冲多尺度单元进行分层多尺度表示学习

Result: 在五个基准去雨数据集上显著超越现有SNN方法，仅消耗13%的能耗
...
TL;DR: SPF是一种新的流匹配方法，通过时空金字塔结构实现多时间尺度的并行气候模拟，在ClimateBench上表现优于基线，并创建了最大的气候模拟数据集ClimateSuite。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型依赖天气尺度自回归进行气候模拟，速度慢且无法在非平稳强迫下稳定展开，需要更高效、稳定的气候模拟方法。

Method: 提出时空金字塔流匹配（SPF），将生成轨迹划分为时空金字塔，逐步增加空间分辨率以减少计算，每个阶段与特定时间尺度耦合，并基于物理强迫（如温室气体、气溶胶）进行条件生成。

Result: 在ClimateBench上，SPF在年尺度和月尺度上优于强流匹配基线和预训练模型，采样速度快，特别是在较粗的时间尺度上。创建的ClimateSuite包含超过33,000模拟年，SPF在未见场景中表现出良好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到视频模型在时间生成方面表现出色，但它们在图像修复方面的潜力尚未充分探索。本文旨在利用这些模型的时间生成能力来进行渐进式视觉修复。

Method: 通过微调CogVideo模型，使其生成修复轨迹而非自然视频运动。构建了超分辨率、去模糊和低光增强的合成数据集，每个样本展示从退化到清晰帧的渐进过渡。比较了两种提示策略：所有样本共享的统一文本提示，以及通过LLaVA多模态LLM生成并经ChatGPT优化的场景特定提示方案。

Result: 微调后的模型能够将时间进程与修复质量关联起来，生成的序列在PSNR、SSIM和LPIPS等感知指标上逐帧提升。模型在恢复空间细节和光照一致性的同时保持时间连贯性，并且在ReLoBlur数据集上无需额外训练就能泛化到真实场景，展示了强大的零样本鲁棒性和通过时间修复的可解释性。

Conclusion: 文本到视频模型可以有效地重新用于渐进式视觉修复任务，通过时间维度学习修复过程，在多个修复任务上表现出色，并具有良好的泛化能力和可解释性。
...
TL;DR: 提出MORP-Synth两阶段合成增强框架，通过形态学区域扰动和条件生成模型改善SAR溢油分割模型从地中海到秘鲁海岸的跨区域泛化能力


<details>
  <summary>Details</summary>
Motivation: SAR溢油分割模型在不同海域间泛化能力差，特别是秘鲁海岸标记数据稀缺，导致模型性能显著下降

Method: 两阶段合成增强框架：阶段A使用形态学区域扰动生成几何变化；阶段B使用条件生成INADE模型渲染SAR纹理。构建秘鲁数据集并评估7种分割架构

Result: 模型从地中海到秘鲁性能从67.8% mIoU降至51.8%；MORP-Synth提升性能达+6 mIoU，少数类IoU显著提升（溢油+10.8，类油+14.6）

Conclusion: MORP-Synth能有效改善SAR溢油分割模型的跨区域泛化能力，特别是在标记数据稀缺的秘鲁海岸场景
...
TL;DR: 论文提出一种基于预训练视频扩散模型的自监督跟踪方法，利用扩散模型早期去噪阶段学习到的运动表示来区分视觉相似物体，无需任务特定训练。


<details>
  <summary>Details</summary>
Motivation: 当前自监督跟踪器在视觉线索模糊时表现不佳，限制了其在无大量标注数据情况下的可扩展性和泛化能力。需要一种能有效区分视觉相似物体的跟踪方法。

Method: 利用预训练视频扩散模型固有的运动表示能力，发现其去噪过程在早期高噪声阶段分离出运动信息，与后期的外观细化不同。基于此发现构建自监督跟踪器。

Result: 在现有基准测试和新引入的视觉相似物体跟踪测试中，相比近期自监督方法获得高达6个百分点的性能提升。可视化验证了扩散模型衍生的运动表示能鲁棒跟踪甚至完全相同的物体。

Conclusion: 预训练视频扩散模型自然地学习到适用于跟踪的运动表示，无需任务特定训练。这为解决视觉相似物体区分这一现有方法的薄弱点提供了新途径，显著提升了自监督跟踪性能。
...
Conclusion: 提出的基于薄板样条的长期对齐框架有效解决了3D视觉基础模型在时序预测中的一致性问题，具有高鲁棒性、通用性和即插即用特性，在多种实际场景中表现优异。
...
TL;DR: 提出MS-CAM方法，通过多权重自匹配类激活映射提升SAR图像中CNN的可解释性，并验证其在弱监督目标定位中的应用。


<details>
  <summary>Details</summary>
Motivation: 卷积神经网络在SAR任务中表现出色，但其内部机制复杂不透明，难以满足高可靠性要求，限制了在SAR领域的应用。提高CNN的可解释性对SAR领域的发展和应用至关重要。

Method: 提出多权重自匹配类激活映射（MS-CAM）方法，将SAR图像与CNN提取的特征图和对应梯度进行匹配，结合通道级和元素级权重，可视化模型在SAR图像中的决策依据。

Result: 在自建的SAR目标分类数据集上的实验表明，MS-CAM能更准确地突出网络的关注区域，捕捉详细的目标特征信息，从而增强网络可解释性。同时验证了MS-CAM在弱监督目标定位中的可行性。

Conclusion: MS-CAM方法有效提升了SAR图像中CNN的可解释性，为弱监督目标定位提供了可行方案。深入分析了像素阈值等影响定位精度的关键因素，为未来工作提供指导。
...
TL;DR: 本文提出了一种稀疏激活的混合专家（MoE）适配方法，用于解决统一多模态模型中生成组件对压缩敏感的问题，在仅激活约一半参数的情况下达到与完整模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型虽然整合了理解和生成能力，但引入了推理效率问题。特定任务或样本可能不需要模型的全部知识或容量，但目前对组件间效率差异的系统性理解有限。

Method: 首先使用训练无关的剪枝方法系统分析统一多模态模型组件，包括深度剪枝和宽度缩减。然后提出混合专家（MoE）适配方法，将生成模块划分为多个专家并启用稀疏激活来恢复生成质量。

Result: 研究发现理解组件在理解和生成任务中都表现出显著的可压缩性，而生成组件对压缩高度敏感。提出的MoE适配方法使BAGEL模型在仅激活约一半参数的情况下达到与完整模型相当的性能。

Conclusion: 通过稀疏激活的混合专家适配可以有效解决统一多模态模型中生成组件的压缩敏感性问题，显著提高推理效率而不牺牲性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有多视角人群计数方法需要昂贵的相机校准和人群密度图标注，而现有的无校准方法仍需要图像级人群标注。需要开发更实用、成本更低的解决方案。

Method: 使用人群数量作为单视角计数模块的监督信号，而非密度图；采用基于多尺度先验的自监督排序损失增强模型感知能力；利用语义信息实现更准确的视角匹配。

Result: 在三个广泛使用的多视角计数数据集上，该方法在弱监督设置下优于现有最先进方法。

Conclusion: WSCF-MVCC方法比校准方法更适合实际部署，提供了更实用、成本更低的多视角人群计数解决方案。
...
TL;DR: VACoT是一个在视觉语言模型推理时动态调用图像增强的框架，通过后处理变换提升对抗性场景下的鲁棒性，特别在OCR相关任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型主要依赖大规模真实数据或合成多样性，在基础感知任务上可能不如传统模型可靠。由于预训练和微调成本高昂，继续在增强数据上训练效果有限且收益递减。

Method: 提出VACoT框架，在模型推理时动态调用图像增强，结合去噪等后处理变换。采用结构化通用视觉增强集合，通过高效的智能体强化学习减少训练复杂度和计算开销，并设计条件奖励机制平衡增强必要性和响应简洁性。

Result: 在13个感知基准测试中展示优越性，显著提升对抗性和分布外输入的鲁棒性，特别是在OCR相关对抗场景中。引入AdvOCR基准进一步验证后处理视觉增强在对抗场景中的泛化优势。

Conclusion: VACoT通过推理时动态图像增强有效提升视觉语言模型的鲁棒性，避免昂贵的重新训练，为对抗性感知任务提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 结核病作为持续数千年的传染病，在资源有限地区的诊断面临挑战。传统方法如痰涂片镜检和培养效率低下，需要探索深度学习等先进技术来改善TB诊断。

Method: 使用Kaggle的4,200张胸部X光数据集，比较预训练的ResNet-50和SqueezeNet模型。预处理包括数据分割、增强和调整大小，使用准确率、精确率、召回率和混淆矩阵等指标评估性能。

Result: SqueezeNet表现显著优于ResNet-50：损失32% vs 54%，准确率89% vs 73%，精确率98% vs 88%，召回率80% vs 52%，F1分数87% vs 65%。

Conclusion: 机器学习在TB检测中具有巨大潜力，特别是SqueezeNet模型表现优异。将此类模型集成到移动设备可扩展其在资源匮乏地区的应用，但仍需开发更快、更小、更准确的TB检测模型。
...
TL;DR: 提出一种基于混合专家机制和选择性注意力模块的无地图轨迹预测算法，在时间、空间和频率域进行预测，在复杂交互场景中表现出色。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统中轨迹预测对可靠性和安全性至关重要，但现有方法难以从冗余数据中高效提取有价值的场景信息，特别是在处理复杂智能体交互时，计算效率和预测准确性都有待提升。

Method: 1. 使用混合专家（MoE）机制自适应选择关键频率分量，提取并整合多尺度时间特征；2. 提出选择性注意力模块过滤时间和空间交互中的冗余信息；3. 设计多模态解码器，在补丁级和点级损失监督下生成合理轨迹。

Result: 在Nuscences数据集上的实验证明了该算法的优越性，验证了其在处理复杂交互场景中的有效性。

Conclusion: 提出的无地图轨迹预测算法通过混合专家机制和选择性注意力模块，能够高效处理复杂交互场景中的轨迹预测问题，在计算效率和预测准确性方面均有显著提升。
...
Method: SAGE学习合成视觉提示，通过风格转移构建源域多样风格表示，然后根据每个输入的视觉上下文自适应融合这些风格线索，形成动态提示来协调图像外观。

Result: 在五个基准数据集上的实验表明，SAGE在隐私约束下实现了与最先进方法相当或更优的性能，在所有设置中都优于完全微调基线。
...
TL;DR: 本文提出了一种实时无人机摄影测量框架，通过在线反馈机制实现探索-利用循环，在飞行过程中动态评估重建质量并优化采集路径。
...
Method: 基于SfM on-the-fly，集成三个模块：在线增量粗网格生成、在线网格质量评估（含可操作指标）、预测性路径规划用于轨迹实时优化。
...
Method: 提出FDTA框架，包含三个互补的特征精炼适配器：1) 空间适配器(SA)集成深度感知线索增强空间连续性；2) 时间适配器(TA)聚合历史信息建立时间依赖；3) 身份适配器(IA)利用质量感知对比学习增强实例级可区分性。

Result: 在多个具有挑战性的MOT基准测试（DanceTrack、SportsMOT、BFT）上实现了最先进的性能，证明了所提出的判别性嵌入增强策略的有效性。

Conclusion: FDTA通过显式特征精炼框架有效增强了目标嵌入的判别性，解决了端到端多目标跟踪中关联准确率不足的问题，在多个基准测试上取得了优异表现。
...
TL;DR: 提出一种无需人工标注的相机引导雷达标注方法，通过将雷达点云投影到相机语义分割结果并应用空间聚类，为4D雷达生成准确标签，建立了可复现框架并量化了雾天对雷达标注性能的影响。


<details>
  <summary>Details</summary>
Motivation: 4D雷达在恶劣天气条件下具有鲁棒感知潜力，但雷达语义分割研究受限于开源数据集和标签的稀缺。RaDelft数据集仅提供LiDAR标注且无公开代码生成雷达标签，限制了可复现性和下游研究。

Method: 通过将雷达点云投影到基于相机的语义分割结果，并应用空间聚类技术，创建无需人工标注的雷达标签。该方法建立了相机引导的雷达标注流程。

Result: 成功复现了RaDelft组的数值结果，证明该方法能生成准确的雷达点云标签，显著提高雷达标签精度。建立了可复现框架供研究社区训练和评估标注的4D雷达数据，并量化了不同雾度水平对雷达标注性能的影响。

Conclusion: 提出了一种无需人工标注的相机引导雷达标注方法，解决了4D雷达语义分割中标签稀缺的问题，建立了可复现框架，并分析了环境条件对标注性能的影响，为雷达感知研究提供了重要工具。
...
TL;DR: Skywork-R1V4是一个300亿参数的多模态智能体模型，通过监督微调统一了图像操作和网络搜索能力，在多项基准测试中超越Gemini 2.5 Flash，展示了仅通过精心策划的监督学习就能实现复杂多模态智能体能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法将图像操作和网络搜索视为分离能力，过度依赖昂贵的强化学习，且缺乏基于实际工具执行轨迹的规划。需要开发一个能统一多模态规划、图像操作和深度搜索的智能体系统。

Method: 开发了300亿参数的Skywork-R1V4多模态智能体模型，通过监督微调在不到3万个高质量规划-执行一致轨迹上进行训练，采用逐步一致性过滤验证，实现了视觉操作和外部知识检索的动态交替推理。

Result: 在MMSearch上得分66.1，FVQA上得分67.2，在全部11个指标上超越Gemini 2.5 Flash。模型展现出推理时的涌现长程推理能力，能协调超过10个工具调用来解决复杂多步骤任务。

Conclusion: 精心策划的监督学习足以实现复杂的多模态智能体智能，无需依赖强化学习。Skywork-R1V4展示了统一多模态规划、图像操作和深度搜索的有效性。
...
TL;DR: Nav-R²是一个用于开放词汇目标导航的框架，通过结构化思维链推理和相似性感知记忆，显式建模目标-环境关系和环境-动作规划，提升对未见物体的定位成功率。
...
Method: 提出Nav-R²框架，包含结构化思维链推理（CoT）和相似性感知记忆（SA-Mem）。CoT显式建模目标-环境关系和环境-动作规划，SA-Mem从时间和语义角度压缩视频帧并融合历史观测，保留最相关的特征。

Result: Nav-R²在定位未见物体方面达到最先进性能，避免了对已见物体类别的过拟合，同时保持2Hz的实时推理速度。

Conclusion: 通过显式建模关键关系和引入相似性感知记忆，Nav-R²提供了一种高效、透明的开放词汇目标导航解决方案，显著提升了对未见物体的定位能力。
...
TL;DR: WISE是一个多智能体辩论框架，通过异构专家（单模态和多模态）解决视觉语言推理问题，使用求解器和反思器结构，结合改进的Dawid-Skene算法聚合结果，在多个数据集上比现有方法提升2-7%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型具有互补优势，多智能体辩论主要应用于纯语言任务，在视觉语言多模态问题上的效果尚未充分探索。需要开发能够利用异构专家（单模态和多模态）能力的辩论框架来解决视觉语言推理问题。

Method: 提出WISE框架：1) 将智能体分为求解器（生成解决方案）和反思器（验证正确性、分配权重、提供自然语言反馈）；2) 使用改进的Dawid-Skene算法聚合多轮辩论结果，考虑智能体响应差异和反馈权重；3) 支持异构专家配置，包括单模态和多模态能力。

Result: 在SMART-840、VisualPuzzles、EvoChart-QA和新的SMART-840++数据集上评估，WISE在多种多模态任务和LLM配置中，比最先进的多智能体辩论设置和聚合方法持续提升2-7%的准确率。

Conclusion: WISE框架成功将多智能体辩论扩展到视觉语言推理领域，通过异构专家分工和加权聚合机制，显著提升了多模态问题解决的准确性和鲁棒性，为复杂推理任务提供了有效的协作解决方案。
...
TL;DR: MitUNet：用于室内3D重建的混合神经网络，通过Mix-Transformer编码器和增强U-Net解码器实现高精度墙体分割，优化Tversky损失函数平衡精度与召回率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在墙体分割任务中难以检测薄结构组件，产生不规则边界掩码，缺乏后续矢量化所需的几何精度，影响室内3D重建质量。

Method: 提出MitUNet混合架构：使用分层Mix-Transformer编码器捕获全局上下文，U-Net解码器结合scSE注意力模块精确恢复边界；优化Tversky损失函数超参数，平衡精度与召回率。

Result: 在CubiCasa5k公开数据集和专有区域数据集上实验表明，该方法能生成结构正确、边界精度高的掩码，优于标准单任务模型。

Conclusion: MitUNet为自动化3D重建管道提供了鲁棒的数据准备工具，通过专门设计的架构和损失函数优化，实现了高质量的墙体分割。
...
TL;DR: 本文提出GuiDG框架，通过训练多个参数高效的领域专家模型，而非微调单一通用模型，来提升视觉语言模型在领域泛化任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型微调面临领域特定性与领域泛化能力之间的权衡问题。传统方法在整个数据集上微调单一通用模型，可能会损害模型在未见领域的泛化能力。

Method: 提出两阶段GuiDG框架：1) 使用提示调优获取源领域专家模型；2) 引入跨模态注意力模块，通过自适应专家集成来指导视觉编码器的微调。同时构建ImageNet-DG数据集用于评估少样本领域泛化。

Result: 在标准领域泛化基准和ImageNet-DG上的实验表明，GuiDG在保持效率的同时，优于当前最先进的微调方法。

Conclusion: 通过理论分析发现，在划分的源领域上训练多个参数高效的专家模型比微调单一通用模型具有更好的泛化能力，GuiDG框架有效验证了这一发现。
...
<details>
  <summary>Details</summary>
Motivation: 随着大视觉语言模型的发展，GUI智能体任务从单屏幕任务转向复杂的屏幕导航挑战。然而，现实世界的GUI环境（如PC软件和移动应用）通常是复杂且专有的，难以获取全面的环境信息，这阻碍了对智能体导航能力的系统研究和基准测试。
...
Result: 在静态和交互式基准测试上验证了方法的有效性，证明研究结果能够有效泛化到现实世界场景。强化学习方法在GUI导航中展现出优势，显著提升了屏幕导航性能。

Conclusion: 强化学习方法在GUI导航中具有显著优势，为构建更强大和可泛化的GUI智能体提供了实用指导。GUI Exploration Lab环境解决了现实世界GUI环境信息获取困难的问题，推动了GUI智能体导航研究的系统化发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型在处理小时或天级别的长视频时面临挑战，主要问题包括：上下文容量有限、视觉细节在抽象过程中丢失、现有基于记忆的方法过度依赖文本摘要而忽略视觉证据、固定时间尺度的检索无法捕捉可变时长的事件

Method: WorldMM构建三种互补记忆：情景记忆（跨多时间尺度索引事实事件）、语义记忆（持续更新高层概念知识）、视觉记忆（保留场景的详细信息）。推理时，自适应检索代理迭代选择最相关的记忆源，利用多时间粒度，直到收集到足够信息

Result: 在五个长视频问答基准上显著优于现有基线，平均性能比之前的最先进方法提升8.4%，证明了其在长视频推理上的有效性

Conclusion: WorldMM通过多模态记忆架构和自适应检索机制，有效解决了长视频理解中的关键挑战，为处理复杂长视频场景提供了新的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前AI驱动的青光眼检测方法在可靠性、参数效率、虚假相关性以及干预分析应用方面仍有改进空间，需要更轻量且考虑真实因果关系的模型

Method: 采用卷积VAE架构，结合HSIC（希尔伯特-施密特独立性准则）的潜在空间解耦和图自编码器的无监督因果表示学习，构建轻量级因果表示模型

Result: LightHCG在青光眼分类任务中表现出更高性能，同时参数量比InceptionV3、MobileNetV2、VGG16等先进视觉模型减少93-99%，并增强了AI驱动干预分析的可能性

Conclusion: LightHCG通过因果表示学习为青光眼检测提供了更轻量、更可靠且具有干预分析潜力的AI解决方案，克服了现有方法的局限性
...
TL;DR: 提出一种结合动量自蒸馏和梯度累积的方法，在医疗视觉语言模型中同时解决计算效率和知识利用问题，实现单GPU高效训练并达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 医疗领域获取详细标注困难，需要鲁棒的视觉语言模型。对比学习训练需要大batch size，计算成本高，限制了资源有限机构的使用。同时医疗数据有限，需要充分利用数据和模型中的知识。

Method: 结合动量方法和蒸馏技术：1) 利用动量自蒸馏增强多模态学习；2) 集成动量机制与梯度累积，在不增加资源消耗的情况下扩大有效batch size。

Result: 在零样本分类中达到SOTA竞争性性能，少样本适应显著提升（超过90% AUC-ROC），检索任务提升2-3%。单GPU实现高效训练，保持合理训练时间。

Conclusion: 该方法通过减少资源需求同时提升性能，推动了高效多模态学习的发展，使医疗视觉语言模型训练更易实现。
...
TL;DR: BOLT：一种通过提取正交任务感知谱基并在该子空间内适应的框架，用于重用现有微调模型进行新任务迁移


<details>
  <summary>Details</summary>
Motivation: 现有方法面临挑战：元学习方法需要额外的元训练阶段，成本高且不稳定；同时大量任务特定预训练模型涌现，但如何以最小额外训练将它们迁移到新任务仍未被充分探索

Method: BOLT框架：离线阶段从多个任务向量收集主要奇异方向并正交化形成可重用基；在线阶段冻结这些基，仅训练每层的小型对角系数，实现参数高效的秩控制更新

Result: BOLT提供：1) 通过池化源任务系数获得未见任务的强训练免费初始化；2) 参数高效微调路径，在实验中相比常见PEFT基线和代表性元学习初始化实现稳健性能

Conclusion: 将适应约束到任务感知正交子空间为未见任务迁移提供了有效替代方案，BOLT框架在重用现有微调模型方面表现出色
...
TL;DR: TDE增强SNN时序建模能力，通过Spiking Encoder生成多样化输入刺激和Attention Gating Module引导生成，配合Spike-Driven Attention降低能耗，在目标检测任务中表现优异。
...
Conclusion: TDE通过增强SNN的时序信息建模能力，显著提升了复杂任务性能，同时SDA有效降低了注意力机制的能量消耗，为高效SNN目标检测提供了有效解决方案。
...
TL;DR: 本文回顾了nuScenes数据集，这是自动驾驶领域最广泛使用的数据集之一，分析了其创建过程、技术细节、对后续数据集的影响，以及基于该数据集的各种任务和方法发展。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶和高级驾驶辅助系统依赖大量标注数据，数据集与硬件、算法一样是AV开发的基础组件。nuScenes数据集体现了自动驾驶发展的关键趋势，但许多技术细节在学术出版物中尚未充分披露。

Method: 深入分析nuScenes数据集的创建过程，包括其扩展版本nuImages和Panoptic nuScenes，揭示许多未公开的技术细节。追踪nuScenes对后续数据集的影响及其定义的行业标准。综述基于nuScenes的官方和非官方任务。

Result: nuScenes是首个包含雷达数据、涵盖两大洲城市驾驶场景、使用完全自动驾驶车辆在公共道路上收集的数据集。它推动了多模态传感器融合、标准化基准测试，并支持感知、定位与建图、预测和规划等多种任务。该数据集定义了众多行业标准，影响了大量后续数据集。

Conclusion: nuScenes数据集在自动驾驶领域具有里程碑意义，不仅提供了高质量的多模态数据，还定义了行业标准和基准测试框架。本文提供了对该数据集的全面分析，为自动驾驶文献提供了以nuScenes为重点的全面综述。
...
TL;DR: 提出了HouseLayout3D真实世界基准和MultiFloor3D训练免费基线，用于支持全建筑尺度布局估计研究，特别是多楼层和复杂空间。


<details>
  <summary>Details</summary>
Motivation: 当前3D布局估计模型主要在合成数据集上训练，只能处理单房间或单楼层环境，无法处理大型多楼层建筑，需要将场景分割为单个楼层，这移除了对连接多个楼层的结构（如楼梯）推理至关重要的全局空间上下文。

Method: 提出了HouseLayout3D真实世界基准，支持全建筑尺度布局估计研究；同时提出了MultiFloor3D，这是一个简单的训练免费基线，利用最新的场景理解方法。

Result: MultiFloor3D在提出的基准和现有数据集上都优于现有的3D布局估计模型，表明该方向需要进一步研究。

Conclusion: HouseLayout3D基准和MultiFloor3D基线为全建筑尺度3D布局估计研究提供了重要工具，揭示了当前方法的局限性并指出了未来研究方向。
...
Method: 提出聚类框架ClusterStyle：1）使用原型集合而非非结构化嵌入来建模同一风格类别内多样化的风格模式；2）考虑全局级（同一类别风格运动间）和局部级（运动序列时间动态内）两种风格多样性；3）构建全局和局部两个结构化风格嵌入空间，通过与非可学习原型锚点对齐进行优化；4）通过风格调制适配器（SMA）将风格特征集成到预训练文本到运动生成模型中

Result: 大量实验表明，该方法在风格化运动生成和运动风格迁移任务上优于现有最先进模型

Conclusion: ClusterStyle通过原型建模有效解决了风格内多样性问题，在风格化运动生成方面取得了显著改进
...
TL;DR: STL是一个自训练框架，通过结构化推理模板（先提取视觉属性再推理）和负向原理数据，联合提升视觉语言模型的感知和推理能力


<details>
  <summary>Details</summary>
Motivation: 现有增强视觉语言模型推理能力的方法依赖高质量思维链数据，这些数据获取成本高（人工标注、专有模型）或忽视感知能力。需要一种更有效的方法来同时提升感知和推理能力。

Method: 提出See-Think-Learn (STL)自训练框架：1）使用结构化推理模板，让模型先"看"（提取视觉属性文本化），再"思考"（基于属性推理）；2）通过自训练循环让模型生成并学习自己的结构化原理；3）添加负向原理数据（解释为什么某些答案是错误的）来增强模型的辨别能力。

Result: 在多个领域的实验中，STL始终优于仅基于答案训练或自生成推理的基线方法。定性分析证实其生成原理的高质量。

Conclusion: STL为增强视觉语言模型的多模态推理能力提供了一种成本效益高的解决方案，通过结构化推理和负向原理学习，同时提升了感知和推理能力。
...
TL;DR: 音频-视频联合去噪训练能提升视频生成质量，即使只关注视频质量。通过参数高效的AVFullDiT架构，研究发现音频预测作为特权信号能帮助模型学习视觉事件与声学后果之间的因果关系，从而正则化视频动态。


<details>
  <summary>Details</summary>
Motivation: 探索音频-视频联合去噪训练是否能在只关心视频质量的情况下提升视频生成效果。现有研究表明多模态耦合有益于音频-视频同步性，但不确定是否也能提升视频本身的质量。

Method: 提出参数高效的AVFullDiT架构，利用预训练的文本到视频(T2V)和文本到音频(T2A)模块进行联合去噪训练。同时训练T2AV模型和仅T2V的对照模型，在相同设置下进行对比实验。

Result: 音频-视频联合去噪确实能提升视频生成质量，特别是在具有大运动和物体接触动作的挑战性子集上表现更佳。音频预测作为特权信号，帮助模型内化视觉事件与声学后果之间的因果关系，从而正则化视频动态。

Conclusion: 跨模态协同训练是开发更强大、更具物理基础的世界模型的有前景方法。音频-视频联合去噪不仅能改善同步性，还能提升视频质量，为多模态生成系统提供了新的研究方向。
...
TL;DR: SEER-Bench：首个序列化具身探索与推理基准，包含EQA和EMN任务；提出3DSPMR方法，首次将几何信息融入MLLM进行空间理解，显著提升序列任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有具身AI研究多关注单任务场景，但现实应用中智能体常面临序列任务，需要复用先前探索积累的空间知识来支持后续推理与探索。当前缺乏专门评估这一挑战的基准和方法。

Method: 提出SEER-Bench基准，包含序列化具身问答(EQA)和具身多模态导航(EMN)任务；开发3DSPMR方法，利用已探索区域的关系、视觉和几何线索增强MLLM，首次将几何信息显式融入MLLM的空间理解。

Result: 3DSPMR在序列EQA和EMN任务上取得显著性能提升，验证了将几何信息融入MLLM进行空间理解的有效性。

Conclusion: 该工作填补了序列具身AI任务的评估空白，提出的3DSPMR方法通过整合几何信息显著提升了MLLM在序列任务中的空间推理能力，为现实世界具身智能应用提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于分布匹配的数据集蒸馏方法忽略了训练过程中特征表示的演化，限制了合成数据的表达能力，影响了下游任务性能

Method: 提出轨迹引导数据集蒸馏(TGDD)，将分布匹配重新定义为沿模型训练轨迹的动态对齐过程，在每个训练阶段对齐合成数据集和原始数据集的特征分布，并引入分布约束正则化减少类别重叠

Result: 在10个数据集上达到最先进性能，特别是在高分辨率基准上获得5.0%的准确率提升，在性能和效率之间取得良好平衡

Conclusion: TGDD通过动态对齐训练轨迹中的特征分布，使合成数据保持语义多样性和代表性，显著提升了数据集蒸馏的性能，且不增加额外优化开销
...
<details>
  <summary>Details</summary>
Motivation: 视频世界模型在长期世界建模中存在时空一致性问题，现有模型由于长上下文输入的计算成本过高而难以解决，需要开发更高效的方法。
...
Conclusion: WorldPack通过高效的压缩内存机制，解决了视频世界模型中长期时空一致性的挑战，为高质量长期视觉预测提供了有效解决方案。
...
TL;DR: G-SHARP是一个商业兼容的实时手术场景重建框架，专为微创手术设计，使用高斯泼溅技术实现可变形组织的快速准确3D建模。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯泼溅的内窥镜重建方法通常依赖非商业衍生工具，限制了实际部署能力。需要开发商业兼容的实时手术重建框架，能够在实际手术室环境中使用。

Method: 基于GSplat（Apache-2.0）可微分高斯光栅化器构建原生手术管道，实现原理性变形建模、鲁棒遮挡处理和高保真重建。提供Holoscan SDK应用，部署在NVIDIA IGX Orin和Thor边缘硬件上。

Result: 在EndoNeRF pulling基准测试中达到最先进的重建质量，具有适合术中使用的强速度-精度权衡。能够实现实际手术室环境中的实时手术可视化。

Conclusion: G-SHARP是首个基于商业兼容高斯泼溅框架的手术重建系统，克服了现有方法的部署限制，为微创手术提供了实用的实时3D重建解决方案。
...
TL;DR: UCAgents：一种分层多智能体框架，通过结构化证据审计实现单向收敛，解决医学视觉语言模型中的推理脱离问题，在提高诊断准确性的同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在医学诊断中存在推理脱离问题——语言流畅的解释与可验证的图像证据脱节，损害临床信任。现有的多智能体框架虽然能缓解单一模型偏见，但开放式讨论会放大文本噪声和计算成本，且未能将推理锚定在视觉证据上。

Method: 提出UCAgents分层多智能体框架，受临床工作流程启发，禁止立场改变并将智能体交互限制在有针对性的证据验证中。引入单轮询问讨论以发现视觉-文本错位的潜在风险，通过信息论形式化双重噪声瓶颈，同时约束视觉模糊性和文本噪声。

Result: 在四个医学VQA基准测试中，UCAgents在PathVQA上达到71.3%的准确率（比最先进技术提高6.0%），同时降低87.7%的token成本。评估结果证实UCAgents在发现更多视觉证据和避免混淆性文本干扰之间取得了平衡。

Conclusion: UCAgents展示了诊断可靠性和计算效率的平衡，这对于现实世界临床部署至关重要。该框架通过结构化证据审计和单向收敛机制，有效解决了医学视觉语言模型中的推理脱离问题。
...
TL;DR: 提出3D-SLIM方法，用自适应空间注意力掩码替代传统因果掩码，解决3D场景理解中的顺序偏见和受限注意力问题，无需修改架构或增加参数即可提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景语言理解方法采用语言建模的标准解码器，依赖因果注意力掩码，这导致两个基本冲突：1) 顺序无关的3D对象之间存在顺序偏见；2) 对象-指令注意力受限，阻碍任务特定推理。

Method: 提出3D-SLIM（3D空间语言指令掩码），包含两个关键组件：1) 几何自适应掩码，基于空间密度而非标记顺序约束注意力；2) 指令感知掩码，使对象标记能直接访问指令上下文。该方法简单、无需架构修改或额外参数。

Result: 在多个3D场景语言任务基准测试和LLM基线上进行广泛实验，验证了3D-SLIM的有效性，并突显了解码器设计在3D多模态推理中的关键作用。

Conclusion: 3D-SLIM通过自适应注意力掩码有效解决了3D场景理解中的顺序偏见和注意力限制问题，为3D多模态推理提供了简单而有效的解决方案。
...
TL;DR: YingVideo-MV：首个级联框架，用于音乐驱动的长视频生成，通过音频语义分析、可解释镜头规划、时序感知扩散Transformer和长序列一致性建模，实现高质量音乐表演视频的自动合成。
...
Method: 1. 构建大规模Music-in-the-Wild数据集；2. 集成音频语义分析；3. 可解释镜头规划模块(MV-Director)；4. 时序感知扩散Transformer架构；5. 长序列一致性建模；6. 摄像机适配器模块，将摄像机姿态嵌入潜在噪声；7. 时间感知动态窗口范围策略，根据音频嵌入自适应调整去噪范围。

Result: 综合基准测试表明，YingVideo-MV在生成连贯且富有表现力的音乐视频方面表现出色，实现了精确的音乐-动作-摄像机同步。项目页面提供了更多视频示例。

Conclusion: YingVideo-MV是首个用于音乐驱动长视频生成的级联框架，通过创新的摄像机控制和时序一致性策略，成功解决了音乐表演视频生成中的摄像机运动控制问题，为高质量音乐视频自动合成提供了有效解决方案。
...
TL;DR: 本文研究了基于深度学习和高斯混合模型的部分到部分点集配准中特征向量在平移和旋转下的不变性问题，揭示了DeepGMR等方法的局限性，并提出了一种基于注意力的参考点偏移层来提升性能。


<details>
  <summary>Details</summary>
Motivation: 研究基于深度学习和高斯混合模型的部分到部分点集配准方法中特征向量在输入点集平移和旋转下的不变性问题，特别关注DeepGMR等方法的局限性，旨在揭示问题根源并提出解决方案。

Method: 提出基于注意力的参考点偏移层，该层使用成熟的注意力模块来寻找两个部分点集的共同参考点（而非重叠区域），从而获得变换不变的特征，增强DeepGMR及其变体UGMMReg的性能。

Result: ARPS层显著提升了DeepGMR和UGMMReg的性能，这些扩展模型甚至超越了先前使用注意力块和Transformer提取重叠区域或共同参考点的深度学习方法。

Conclusion: 研究揭示了基于深度学习和高斯混合模型的配准方法存在的问题，提出的ARPS层通过寻找共同参考点获得变换不变特征，为这类方法提供了更深入的见解。
...
<details>
  <summary>Details</summary>
Motivation: 当前医学图像分割的测试时适应评估存在局限性：模态覆盖不足、任务多样性不够、方法评估不一致。需要建立一个统一的基准来系统比较不同适应范式在不同医学成像模态下的表现。

Method: 创建MedSeg-TTA基准，包含7种成像模态（MRI、CT、超声、病理、皮肤镜、OCT、胸片），统一数据预处理、骨干网络配置和测试时协议。评估20种代表性适应方法，涵盖四大范式：输入级变换、特征级对齐、输出级正则化和先验估计。

Result: 没有单一范式在所有条件下表现最佳：输入级方法在轻微外观变化下更稳定；特征级和输出级方法在边界相关指标上更有优势；先验方法表现出强烈的模态依赖性。许多方法在大规模中心间和设备间变化下性能显著下降。

Conclusion: 医学图像分割的测试时适应需要根据具体临床场景选择合适方法。MedSeg-TTA提供了标准化数据集、验证实现和公开排行榜，为未来开发稳健、临床可靠的测试时适应方法奠定了严格基础。
...
<details>
  <summary>Details</summary>
Motivation: 当前文档布局解析方法依赖碎片化的多阶段流水线，存在错误传播问题且无法利用联合训练的优势。需要统一框架来同时处理布局检测、文本识别和关系理解三大核心任务。

Method: 提出dots.ocr单视觉语言模型，首次在统一端到端框架中联合学习三大核心任务。通过高度可扩展的数据引擎合成大规模多语言语料库，支持多样化语言、布局和领域。

Result: 在OmniDocBench上取得最先进性能。在新提出的XDocParse基准（涵盖126种语言）上，dots.ocr比次优竞争者高出+7.4分，展现了卓越的多语言能力。

Conclusion: 统一端到端范式在文档布局解析中具有显著优势，dots.ocr为全球文档智能研究设立了强大基线，证明了联合学习三大核心任务的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 自回归模型与地理空间理解的并行本质存在结构性不匹配，强制将场景转化为刚性序列叙事，阻碍了结构化、连贯输出的生成

Method: 将地理空间生成重新定义为并行细化过程，引入GeoDiT——首个针对地理空间领域的扩散式视觉语言模型，实现整体、从粗到细的合成

Result: GeoDiT在需要结构化、面向对象输出的基准测试中建立了新的最先进水平，在图像描述、视觉定位和多对象检测等任务上取得显著提升
...
<details>
  <summary>Details</summary>
Motivation: 单图像超分辨率（SISR）在计算机视觉中仍然是一个具有挑战性的问题。现有方法可能缺乏从数据本身学习丰富、可泛化视觉表示的能力。本文旨在探索自监督预训练在复杂图像恢复任务中的潜力。

Method: 提出ViT-SR方法，采用两阶段训练策略：1）自监督预训练阶段，在着色任务上学习通用视觉表示；2）微调阶段，针对4倍超分辨率任务进行调整。通过预测高频残差图像添加到初始双三次插值结果中，简化残差学习过程。

Result: 在DIV2K基准数据集上，ViT-SR取得了SSIM 0.712和PSNR 22.90 dB的优异结果。这些结果证明了该方法在单图像超分辨率任务上的有效性。

Conclusion: 两阶段训练策略和自监督预训练对复杂图像恢复任务有效。更大的ViT架构或替代的预训练任务可能带来进一步改进。该方法展示了自监督学习在图像恢复领域的潜力。
...
Method: 1) 采用Mixture-of-Experts架构；2) 自适应路由器生成任务和粒度感知的路由指令；3) 上下文解耦增强策略创建局部和全局特征的对比对；4) 构建MGRS-Bench多任务多粒度基准。

Result: 在21个公开数据集上的广泛实验表明，SkyMoE在多个任务上实现了最先进的性能，验证了其在复杂场景中的适应性、可扩展性和优越的多粒度理解能力。

Conclusion: SkyMoE通过专家混合架构和粒度敏感设计，有效解决了遥感多任务多粒度解释的挑战，为地理空间智能提供了更灵活和强大的解决方案。
...
TL;DR: 该论文研究了零样本异常分类与分割的核心挑战，提出了基于图论的理论框架CoDeGraph，并将其扩展到3D医学影像，最后结合视觉语言模型，为工业检测和医疗影像提供了无需训练数据的异常检测解决方案。


<details>
  <summary>Details</summary>
Motivation: 工业检测和医疗影像领域急需无需训练数据的异常检测方法。传统基于距离的方法在面对重复出现的相似异常（一致异常）时会失效，需要理论指导和算法设计来解决这一核心挑战。

Method: 1. 分析预训练Vision Transformer的统计和几何行为，发现相似性缩放和邻居烧毁现象；2. 提出CoDeGraph图框架，通过多阶段图构建、社区检测和结构化精炼来过滤一致异常；3. 扩展到3D医疗影像，提出无需训练的高效体素标记化策略；4. 将CoDeGraph与视觉语言模型结合，用伪掩码监督提示驱动模型。

Result: 1. 理论解释了预训练ViT在异常检测中的行为模式；2. CoDeGraph能有效抑制一致异常的影响；3. 实现了真正零样本的3D异常检测，无需任何3D训练样本；4. 成功将批量方法与文本方法结合，提升了零样本异常检测性能。

Conclusion: 该研究为零样本异常分类与分割提供了理论理解和实用解决方案，通过CoDeGraph框架解决了一致异常问题，扩展到3D医疗影像，并结合视觉语言模型，为工业检测和医疗诊断提供了无需训练数据的端到端异常检测方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型面临预训练视觉语言模型与扩散模型高效桥接的挑战。现有方法使用固定数量的可学习查询token虽然计算高效，但存在任务泛化崩溃问题，无法适应与预训练任务差异较大的新任务

Method: 1. 提出Noisy Query Tokens方法，通过端到端优化学习VLM与扩散模型之间的分布式表示空间，增强持续学习能力；2. 引入带有线性投影的VAE分支来恢复细粒度图像细节

Result: 实验结果表明，该方法能够缓解泛化崩溃问题，并在多样化任务上实现稳定的持续学习

Conclusion: Noisy Query Tokens方法通过分布式表示学习和VAE细节恢复，有效解决了MLLM中VLM与扩散模型桥接时的泛化崩溃问题，实现了更好的持续学习性能
...
TL;DR: 提出一种无需训练的两步加速方案，通过分析VGGT和π³中全局注意力模块的角色分工，将早期全局层转换为帧注意力，并对全局注意力进行子采样，实现8-10倍推理加速，同时保持或略微提升原模型精度。
...
Method: 1. 深入分析VGGT和π³中全局注意力模块的角色分工；2. 基于分析提出无需训练的两步加速方案：将早期全局层转换为帧注意力，对全局注意力进行K/V子采样（保留对角线并添加均值填充组件）；3. 在VGGT和π³上实例化该策略。

Result: 在标准姿态和点云基准测试中，该方法实现8-10倍推理时间加速，匹配或略微提升原始模型精度，在极端密集多视图设置下仍保持鲁棒性（而先前稀疏注意力基线失败）。

Conclusion: 通过系统分析全局注意力在多视图推理中的角色分工，提出了一种高效的无训练加速方案，显著提升计算效率同时保持模型性能，为多视图3D模型的优化提供了新思路。
...
TL;DR: OmniPerson是一个统一身份保持的行人生成框架，支持可见光/红外图像/视频ReID任务，通过多参考图像融合和PersonSyn数据集实现高质量可控行人生成。


<details>
  <summary>Details</summary>
Motivation: 行人重识别任务缺乏大规模高质量训练数据，现有数据增强方法存在身份一致性不足和可控性有限的问题，限制了数据集增强的效果。

Method: 提出OmniPerson统一生成模型，包含多参考融合器实现身份保持，支持RGB/IR模态、任意数量参考图像、多种姿态和文本控制，并创建PersonSyn数据集提供多模态监督。

Result: OmniPerson在行人生成任务中达到最先进水平，在视觉保真度和身份一致性方面表现优异，生成的数据能有效提升ReID模型性能。

Conclusion: OmniPerson为行人重识别提供了一种有效的身份保持数据增强解决方案，通过统一生成框架解决了现有方法的局限性，将开源代码、模型和数据集。
...
TL;DR: Panel2Patch：一种从生物医学文献中挖掘层次化视觉-语言对齐数据的新方法，通过解析多面板、标记密集的图表及其文本，构建从图表级到面板级再到局部补丁级的细粒度监督信号。


<details>
  <summary>Details</summary>
Motivation: 当前生物医学视觉-语言预训练通常将丰富的科学图表和文本压缩为粗糙的图表级配对，丢弃了临床医生实际依赖的局部结构细粒度对应关系。需要一种能保留局部语义的方法来提升模型性能。

Method: Panel2Patch数据管道：解析科学图表的布局、面板和视觉标记，构建层次化对齐的视觉-语言对（图表级、面板级、补丁级）。基于此开发粒度感知的预训练策略，统一从粗粒度教学描述到细粒度区域聚焦短语的异构目标。

Result: 仅使用少量文献图表，Panel2Pipeline提取的监督信号比现有方法更有效，能用更少的预训练数据实现显著更好的性能。

Conclusion: 通过挖掘生物医学文献中的层次化结构并构建多粒度监督，Panel2Patch能够显著提升生物医学视觉-语言模型的表示能力，为细粒度医学图像理解提供了新的数据管道和预训练策略。
...
TL;DR: 提出一个结合扩散模型生成和运动图检索的新框架，用于生成更同步自然的语音手势视频


<details>
  <summary>Details</summary>
Motivation: 现有方法使用音频-运动特征距离或共享特征空间进行一对一映射，无法处理音频与手势之间多对多的复杂映射关系

Method: 1) 使用扩散模型从音频生成手势动作，学习音频与运动的联合分布；2) 从音频提取低层和高层特征丰富训练；3) 设计基于运动的检索算法，评估全局和局部相似性在图中找到最佳路径；4) 无缝拼接不连续节点生成连贯视频

Result: 实验结果表明，该方法在同步准确性和手势自然度方面显著优于先前方法

Conclusion: 提出的框架通过扩散模型学习音频-手势的联合分布，结合运动图检索，有效解决了多对多映射问题，生成了更同步自然的手势视频
...
TL;DR: 提出一种结合纹理映射的3D高斯溅射方法，通过自适应纹理分辨率来高效表示细节外观，减少高斯基元数量


<details>
  <summary>Details</summary>
Motivation: 传统3D高斯溅射方法需要大量小高斯基元来表示精细外观细节，这在几何和外观具有不同频率特性时效率低下且浪费资源

Method: 为2D高斯基元引入自适应纹理映射，根据图像采样频率限制纹理像素大小，并在优化过程中自适应调整纹理分辨率，同时基于纹理分辨率控制基元数量

Result: 相比其他纹理化高斯基元解决方案，该方法在图像质量和总参数数量方面表现更优

Conclusion: 结合纹理映射的3D高斯溅射方法能够更高效地表示场景外观细节，减少计算资源消耗，为实时渲染提供更好的解决方案
...
TL;DR: RULER-Bench是一个专门评估视频生成模型规则推理能力的基准测试，覆盖40个任务和6个规则类别，发现当前最先进模型在规则一致性指标上仅达到48.87%。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成评估基准主要关注视觉感知和理解方面（如视觉美学、指令遵循、时间连贯性），但视频生成模型的规则推理能力尚未得到充分探索。虽然最近有研究初步探索视频模型能否作为零样本学习器，但仍缺乏对推理能力的细粒度分解和全面评估协议。

Method: 基于文本到视频和图像到视频两种基本范式，构建包含40个代表性任务、覆盖6个规则类别、包含622个高质量标注实例的RULER-Bench基准。为每个生成的视频构建包含四个指标的检查表，并利用GPT-4o进行评分，与人类判断达到85%的一致性。

Result: 广泛实验表明，最先进的模型在规则一致性指标上仅达到48.87%，表明下一代视频模型在推理能力方面仍有显著改进空间。
...
TL;DR: PPTBench是一个用于评估多模态大语言模型在PowerPoint相关任务上性能的综合性基准测试，涵盖检测、理解、修改和生成四大类别，揭示了当前模型在视觉布局推理方面的显著不足。
...
Method: 利用958个PPTX文件的多样化来源，构建了包含4,439个样本的PPTBench基准测试，涵盖四个类别：检测、理解、修改和生成。通过实验评估模型性能，并进行消融分析和案例研究，可视化暴露系统性的布局错误。

Result: 实验揭示了当前多模态大语言模型在语义理解和视觉布局推理之间存在显著差距：模型能够解释幻灯片内容，但无法生成连贯的空间排列。当前模型难以将视觉线索与基于JSON的布局结构相结合，也无法将视觉信息整合到其API规划能力中。案例研究可视化暴露了错位和元素重叠等系统性布局错误。

Conclusion: PPTBench为在PPT场景中评估视觉语言大模型提供了新的视角，突出了视觉结构推理和连贯幻灯片生成方面的挑战和未来研究方向。所有数据集和代码均已发布，以支持可重复性和未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习遥感图像融合方法在未见数据集上泛化能力差，主要受限于真实训练数据稀缺和不同卫星传感器间的领域差异

Method: 构建多样化模拟数据集（对ImageNet和SkyScript图像应用退化操作和增强），预训练融合模型学习通用空间-光谱表示，在六种卫星数据集上评估零样本和单样本性能

Result: 预训练策略显著提升CNN、Transformer和Mamba等不同架构的泛化性能，在零样本场景取得优异结果，单样本设置下展示出色适应能力
...
TL;DR: PoreTrack3D是首个用于毛孔尺度非刚性3D面部轨迹跟踪的动态3D高斯泼溅基准数据集，包含超过44万条面部轨迹，其中5.2万条超过10帧，68条手动标注轨迹覆盖150帧。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏能够同时捕捉传统面部标志点和毛孔尺度关键点轨迹的基准数据集，这限制了对细微面部表情和皮肤表面运动的研究。

Method: 创建了包含超过440,000条面部轨迹的数据集，其中52,000条超过10帧，68条手动审核轨迹覆盖完整150帧。系统评估了最先进的动态3D高斯泼溅方法，并建立了该领域的首个性能基准。

Result: 建立了首个毛孔尺度3D面部轨迹跟踪基准数据集，提供了该领域的首个性能基线。数据集包含大规模高质量轨迹数据，其中68条手动标注轨迹为评估提供了可靠标准。

Conclusion: PoreTrack3D填补了细粒度面部表情分析领域的空白，为高保真面部运动捕捉和动态3D重建建立了新框架，推动了基于细微皮肤表面运动的面部表情研究。
...
TL;DR: 提出一种混合架构，结合视觉语言模型的语义理解和OCR的空间精度，实现文档区域级检索，提升RAG的上下文精确性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如ColPali）虽然能实现细粒度语义匹配，但只能返回整个页面而非特定区域，限制了检索增强生成（RAG）的精确性。而OCR系统虽有空间坐标信息，但缺乏语义相关性判断能力。

Method: 提出混合架构：利用ColPali的patch级相似度分数作为空间相关性过滤器，应用于OCR提取的区域。建立视觉transformer patch网格与OCR边界框的坐标映射，引入交集度量进行相关性传播，并建立检索精度的理论边界。该方法无需额外训练，在推理时运行。

Result: 开发了开源实现Snappy，展示了实际应用可行性，正在进行实证评估。

Conclusion: 该混合方法统一了语义理解和空间精度的优势，为文档区域级检索提供了理论框架和实用工具，有望提升RAG系统的上下文精确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：基于NeRF的偏振感知方法训练慢、渲染效率低、依赖材料/视角假设；3DGS虽能实时渲染但难以准确重建反射（反射-几何纠缠问题），添加延迟反射模块又引入环境贴图依赖。需要一种既能实时渲染又能准确重建反射的方法。

Method: 提出PolarGuide-GSDR，建立偏振与3DGS的双向耦合机制：1）利用3DGS的几何先验解决偏振歧义；2）使用精炼的偏振信息线索指导3DGS的法线和球谐表示。这种偏振前向引导范式无需环境贴图或限制性材料假设。

Result: 在公开和自采集数据集上，PolarGuide-GSDR在镜面反射重建、法线估计和新视角合成方面达到最先进性能，同时保持实时渲染能力。这是首个将偏振先验直接嵌入3DGS优化的框架。

Conclusion: PolarGuide-GSDR通过偏振与3DGS的双向耦合，成功解决了反射-几何纠缠问题，实现了高保真反射分离和全场景重建，在保持实时渲染的同时提供了优越的可解释性，为复杂反射场景建模提供了有效解决方案。
...
TL;DR: UAUTrack是一个统一的单目标跟踪框架，采用单流、单阶段、端到端架构，有效整合多种模态，通过文本先验提示策略聚焦无人机目标，在多个反无人机数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前反无人机跟踪研究缺乏统一的跨模态协作框架，现有方法多为独立任务模型，忽视跨模态信息共享潜力，且反无人机跟踪技术仍处于起步阶段，现有解决方案难以实现有效的多模态数据融合。

Method: 提出UAUTrack统一框架，采用单流、单阶段、端到端架构，引入文本先验提示策略引导模型在不同场景下聚焦无人机目标，实现多模态有效整合。

Result: 在Anti-UAV和DUT Anti-UAV数据集上达到最先进性能，在Anti-UAV410数据集上保持精度与速度的良好平衡，在多样化反无人机场景中展现高精度和实际效率。

Conclusion: UAUTrack为解决反无人机跟踪中多模态融合和跨模态协作问题提供了有效方案，通过统一框架和文本提示策略实现了性能提升和实用效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的图像超分辨率方法（如SDXL、DiT）通常使用大规模骨干网络，导致训练和推理时计算和内存成本过高。需要开发轻量级扩散方法来解决这一问题。

Method: 1. 识别扩散骨干网络中的块内冗余，提出渐进剪枝方法，在保留恢复能力的同时移除冗余块；2. 提出相位交换适配器模块，利用输入图像的相位信息引导剪枝后的扩散模型，提升恢复性能；3. 将渐进剪枝和相位交换适配器整合为统一模型。

Result: 大量实验表明，该方法在显著降低计算负载和内存消耗的同时，实现了具有竞争力的恢复质量。

Conclusion: PGP-DiffSR通过渐进剪枝和相位引导，成功构建了轻量级扩散超分辨率模型，在效率和性能之间取得了良好平衡。
...
Method: 提出两阶段框架：第一阶段通过双槽竞争机制进行粗粒度场景分解，区分前景和背景区域，使用聚类初始化策略；第二阶段引入掩码槽注意力机制，第一个槽捕获背景，其余槽竞争表示单个前景对象。还使用基于自监督图像特征构建的补丁亲和图的伪掩码指导来引导前景槽学习。

Result: 在合成和真实世界数据集上的广泛实验表明，FASA始终优于最先进的方法，验证了显式前景建模和伪掩码指导对于鲁棒场景分解和对象一致表示的有效性。

Conclusion: FASA通过显式前景建模和伪掩码指导，有效解决了背景干扰和前景对象过分割问题，实现了更精确的无监督对象发现和场景分解。
...
TL;DR: 使用注意力机制改进的InceptionV3模型检测口罩佩戴和社交距离，准确率达98%以上，可识别高风险区域


<details>
  <summary>Details</summary>
Motivation: COVID-19疫情期间需要有效监控口罩佩戴和社交距离遵守情况，以控制病毒传播

Method: 采用定制化的注意力-InceptionV3模型，使用包含10,800张图像的数据集（含戴口罩和不戴口罩图像）进行训练

Result: 训练准确率98%，验证准确率99.5%，精确度约98.2%，帧率25FPS，能有效识别高风险传播区域

Conclusion: 该系统能帮助当局快速定位高风险区域并提醒民众采取预防措施，有效控制疫情传播
...
TL;DR: ClimaDrive是一个语义引导的图像到图像框架，用于生成语义连贯、天气多样且物理合理的异常驾驶数据，其构建的ClimaOoD基准显著提升了异常分割模型的性能。
...
Result: 在四种最先进方法上的实验表明，使用ClimaOoD训练显著提高了异常分割性能。所有方法的AUROC、AP和FPR95都有显著提升，其中RbA在Fishyscapes LAF上的FPR95从3.97降至3.52。
...
<details>
  <summary>Details</summary>
Motivation: 安全X射线成像中扫描设备和环境条件的变化导致显著的域差异，影响目标检测模型性能，需要有效的域适应方法

Method: 应用ALDI++域适应框架，集成自蒸馏、特征对齐和增强训练策略，使用Vision Transformer for Detection (ViTDet)作为骨干网络

Result: 在EDS数据集上超越现有SOTA方法，ViTDet骨干的ALDI++获得最高mAP，各类别检测精度均有提升
...
### [77] [DF-Mamba: Deformable State Space Modeling for 3D Hand Pose Estimation in Interactions](https://arxiv.org/abs/2512.02727)
*Yifan Zhou,Takehiko Ohkawa,Guwenxiao Zhou,Kanoko Goto,Takumi Hirose,Yusuke Sekikawa,Nakamasa Inoue*

Main category: cs.CV

TL;DR: 提出DF-Mamba框架，使用状态空间建模（Mamba）和可变形状态扫描来改进3D手部姿态估计中的特征提取，特别针对严重遮挡情况。


<details>
  <summary>Details</summary>
Motivation: 当前3D手部姿态估计方法在处理严重遮挡（如双手重叠）时效果不佳，主要因为ResNet等CNN的归纳偏置限制了全局上下文建模能力，需要更有效的特征学习方法来处理遮挡图像。

Method: 提出Deformable Mamba（DF-Mamba）框架，结合Mamba的选择性状态建模和提出的可变形状态扫描，在卷积后的局部特征上聚合图像特征，同时选择性保留代表全局上下文的有用线索。
...
Conclusion: DF-Mamba通过状态空间建模有效捕获全局上下文，显著提高了结构化3D手部姿态估计的准确性，特别是在处理遮挡场景时表现出色，为3D HPE提供了更有效的特征提取框架。
...
### [78] [GeoBridge: A Semantic-Anchored Multi-View Foundation Model Bridging Images and Text for Geo-Localization](https://arxiv.org/abs/2512.02697)
*Zixuan Song,Jing Zhang,Di Wang,Zidie Zhou,Wenbin Liu,Haonan Guo,En Wang,Bo Du*

Main category: cs.CV

TL;DR: GeoBridge是一个跨视图地理定位基础模型，通过新颖的语义锚机制实现多视图双向匹配和语言到图像检索，并构建了首个大规模跨模态多视图对齐数据集GeoLoc。


<details>
  <summary>Details</summary>
Motivation: 传统卫星中心范式在缺乏高分辨率或最新卫星图像时鲁棒性有限，且未能充分利用跨视图（如无人机、卫星、街景）和跨模态（如语言和图像）的互补线索。

Method: 提出GeoBridge基础模型，基于新颖的语义锚机制，通过文本描述桥接多视图特征，实现鲁棒灵活的地理定位。同时构建GeoLoc数据集，包含超过50,000对无人机、街景全景、卫星图像及其文本描述，覆盖36个国家。

Result: 实验证实，GeoLoc预训练显著提高了GeoBridge的地理定位精度，同时促进了跨域泛化和跨模态知识迁移。

Conclusion: GeoBridge通过语义锚机制和GeoLoc数据集，解决了传统卫星中心范式的局限性，实现了更鲁棒、灵活的跨视图跨模态地理定位。
...
### [79] [Reasoning-Aware Multimodal Fusion for Hateful Video Detection](https://arxiv.org/abs/2512.02743)
*Shuonan Yang,Tailin Chen,Jiangbei Yue,Guangliang Cheng,Jianbo Jiao,Zeyu Fu*

Main category: cs.CV

TL;DR: 提出RAMF框架，通过局部-全局上下文融合和语义交叉注意力解决多模态融合问题，引入对抗推理增强对微妙仇恨内容的理解，在真实数据集上优于现有方法。
...
Method: 提出推理感知多模态融合(RAMF)框架：1) 局部-全局上下文融合(LGCF)捕捉局部显著线索和全局时间结构；2) 语义交叉注意力(SCA)实现细粒度多模态语义交互；3) 对抗推理过程，通过视觉语言模型生成客观描述、仇恨假设推理和非仇恨假设推理。

Result: 在两个真实世界仇恨视频数据集上评估，方法在Macro-F1和仇恨类召回率上分别比最先进方法提升3%和7%，表现出鲁棒的泛化性能。
...
Method: 提出VLM-Pruner算法：1）采用离心式token剪枝范式，实现从近到远的选择并优先保留细粒度对象细节；2）设计空间稀疏性缓冲准则，延迟选择空间距离较远的token；3）采用并行贪婪策略高效进行token选择；4）选择性融合被丢弃token的显著信息到保留token中。

Result: 在五个视觉语言模型上，VLM-Pruner在88.9%的剪枝率下始终优于强基线方法，同时实现了端到端推理加速。

Conclusion: VLM-Pruner通过平衡冗余性和空间稀疏性，提供了一种无需训练的高效token剪枝解决方案，显著降低了视觉语言模型的计算成本，适合移动设备部署。
...
<details>
  <summary>Details</summary>
Motivation: UK Biobank收集了大量全身MR图像和非成像健康数据，需要稳健准确的图像配准来实现身体空间标准化，并分析非成像数据与图像参数（如组织体积、脂肪含量）的相关性。

Method: 提出性别分层的全身MR图像配准方法，使用VIBESegmentator生成的皮下脂肪和肌肉掩膜来增强基于强度的图割配准，并与仅使用强度的方法以及uniGradICON和MIRTK两种现有方法进行比较。

Result: 提出的方法在4000名受试者上平均Dice分数为男性0.77/女性0.75，比仅强度配准高6个百分点，比uniGradICON高8-9个百分点，比MIRTK高12-13个百分点，标签错误频率降低，年龄相关性图谱更清晰且解剖对齐更好。

Conclusion: 使用两种组织掩膜的图像配准方法改善了UK Biobank全身图像的配准效果，为医学研究提供了更可靠的分析基础。
...
### [82] [Defense That Attacks: How Robust Models Become Better Attackers](https://arxiv.org/abs/2512.02830)
*Mohamed Awad,Mahmoud Akrm,Walid Gomaa*

Main category: cs.CV

TL;DR: 对抗训练意外增加了对抗样本的可迁移性，形成新的生态系统风险


<details>
  <summary>Details</summary>
Motivation: 尽管对抗训练是提升模型鲁棒性的主要防御方法，但其对攻击可迁移性的影响尚未充分研究。本文旨在探究对抗训练是否会无意中增加对抗样本的可迁移性。

Method: 训练了包含CNN和ViT在内的36个多样化模型动物园，进行了全面的可迁移性实验，分析对抗训练模型与标准模型生成的对抗扰动在迁移效果上的差异。

Result: 发现了一个明显的悖论：对抗训练模型产生的扰动比标准模型产生的扰动具有更高的可迁移性，这引入了新的生态系统风险。

Conclusion: 鲁棒性评估不仅应评估模型对迁移攻击的抵抗能力，还应评估其产生可迁移对抗样本的倾向性。作者发布了所有模型、代码和实验脚本以供复现和进一步研究。
...
TL;DR: GeoViS：一个将遥感视觉定位重新定义为渐进式搜索推理过程的框架，通过树状视觉线索序列主动探索全局图像，整合多模态感知、空间推理和奖励引导探索，实现对小目标的精确检测和整体场景理解。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在视觉定位方面取得了显著进展，但将其能力迁移到遥感图像仍面临挑战：目标在千米级场景中通常极小，查询涉及复杂的空间关系（相对位置、空间层次、跨距离对象的上下文依赖）。

Method: 提出GeoViS框架，将遥感视觉定位重新定义为渐进式搜索推理过程。模型通过树状视觉线索序列主动探索全局图像，整合多模态感知、空间推理和奖励引导探索，迭代优化地理空间假设，而不是单步直接预测目标位置。

Result: 在五个遥感定位基准测试上的广泛实验表明，GeoViS实现了精确的地理空间理解，在关键视觉定位指标上持续超越现有方法，展现出强大的跨域泛化能力和可解释性。

Conclusion: GeoViS通过渐进式搜索推理框架有效解决了遥感图像中目标极小和空间关系复杂的挑战，在保持整体场景意识的同时实现了对小尺度目标的精确检测，为遥感视觉定位提供了新的解决方案。
...
### [84] [ReVSeg: Incentivizing the Reasoning Chain for Video Segmentation with Reinforcement Learning](https://arxiv.org/abs/2512.02835)
*Yifan Li,Yingda Yin,Lingting Zhu,Weikai Chen,Shengju Qian,Xin Wang,Yanwei Fu*

Main category: cs.CV

TL;DR: ReVSeg提出了一种显式分解的视频目标分割方法，通过三步推理链（语义解释、时序证据选择、空间定位）来执行复杂推理，并使用强化学习优化多步决策。


<details>
  <summary>Details</summary>
Motivation: 现有的视频目标分割方法通常将复杂的动态、因果关系和时序交互推理简化为潜在嵌入表示，导致推理链不透明且难以追踪。需要更显式的推理方法来处理复杂的查询需求。

Method: ReVSeg采用显式分解视角，在预训练视觉语言模型的原生接口中执行三步序列决策：1）语义解释，2）时序证据选择，3）空间定位。使用强化学习优化多步推理链，让模型能够根据结果驱动的信号自我优化决策质量。

Result: 在标准视频目标分割基准测试中达到最先进性能，并产生可解释的推理轨迹。

Conclusion: 通过显式分解推理链和强化学习优化，ReVSeg能够有效处理复杂的视频目标分割任务，提供透明且可追踪的推理过程，优于现有的简化推理方法。
...
TL;DR: 提出CAEVL方法，无需无人机图像训练，仅使用卫星图像通过域适应实现无人机图像定位


<details>
  <summary>Details</summary>
Motivation: 解决GNSS拒止环境下无人机图像定位问题。现有方法需要大量配对的无人机-卫星图像数据集训练，但这类数据获取成本高且往往不可得，限制了应用范围。

Method: 采用无需无人机图像的训练范式，直接从卫星参考图像学习。通过专门的增强策略模拟卫星视图与真实无人机视图之间的视觉域偏移。提出CAEVL高效模型利用此范式。

Result: 在发布的新挑战性数据集ViLD（真实世界无人机图像）上验证，与需要配对数据训练的方法相比达到竞争性性能，展示了方法的有效性和强泛化能力。

Conclusion: 提出的训练范式消除了对无人机图像训练数据的需求，仅使用卫星图像通过域适应实现无人机图像定位，为GNSS拒止环境下的无人机自主导航提供了实用解决方案。
...
TL;DR: 提出AttMetNet，一种注意力增强的深度学习框架，用于Sentinel-2卫星图像中的甲烷羽流检测，通过融合NDMI和注意力机制提升检测精度
...
Method: 提出AttMetNet框架，融合归一化甲烷指数(NDMI)和注意力增强的U-Net架构，选择性放大甲烷吸收特征并抑制背景噪声；使用focal loss处理类别不平衡问题；在真实甲烷羽流数据集上训练

Result: AttMetNet在甲烷羽流检测中超越现有方法，具有更低的误报率、更好的精确率-召回率平衡和更高的IoU

Conclusion: AttMetNet是首个针对真实卫星图像中甲烷羽流检测的专门架构，通过注意力机制和NDMI融合实现了更鲁棒的检测性能
...
<details>
  <summary>Details</summary>
Motivation: 腹腔镜手术中电刀或激光产生的烟雾会遮挡手术视野，影响视频引导。现有去烟方法未考虑不同烟雾类型（扩散烟雾和环境烟雾）的时空特性差异，导致效果有限。

Method: 提出STANet网络：1) 烟雾掩码分割子网络，通过注意力加权掩码聚合联合预测烟雾掩码和类型；2) 无烟视频重建子网络，基于两种烟雾掩码指导去烟；3) 粗到细解缠模块，通过烟雾类型感知的交叉注意力处理两种烟雾的纠缠问题。

Result: 方法在质量评估上优于现有技术，并在多个下游手术任务中表现出优越的泛化能力。同时构建了首个带有烟雾类型标注的大规模合成视频去烟数据集。

Conclusion: 通过区分烟雾类型并设计相应的解缠机制，STANet能有效去除腹腔镜视频中的烟雾，提升手术可视化质量，为后续手术任务提供更好的视频基础。
...
TL;DR: LumiX是一个结构化扩散框架，用于从文本生成连贯的内在属性图（如反照率、辐照度、法线、深度和最终颜色），通过查询广播注意力和张量LoRA实现多属性联合生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从文本生成场景内在属性时缺乏结构一致性和物理一致性，难以同时生成多个相互关联的内在属性图。

Method: 提出结构化扩散框架LumiX，包含两个关键技术：1) 查询广播注意力机制，在自注意力块中共享查询以确保结构一致性；2) 张量LoRA，通过张量化适配高效建模跨图关系。

Result: LumiX生成连贯且物理有意义的结果，相比现有技术实现了23%更高的对齐度和更好的偏好得分（0.19 vs -0.41），并能在同一框架内执行图像条件的内在分解。

Conclusion: LumiX通过结构化扩散框架成功实现了多内在属性的联合生成，确保了物理一致性，在文本到内在属性生成任务上优于现有方法。
...
### [89] [EGGS: Exchangeable 2D/3D Gaussian Splatting for Geometry-Appearance Balanced Novel View Synthesis](https://arxiv.org/abs/2512.02932)
*Yancheng Zhang,Guangyu Sun,Chen Chen*

Main category: cs.CV

TL;DR: 提出Exchangeable Gaussian Splatting (EGGS)，一种混合2D和3D高斯表示的NVS方法，平衡外观保真度和几何一致性


<details>
  <summary>Details</summary>
Motivation: 现有方法存在权衡：3DGS能实现高保真渲染但多视角不一致，几何精度有限；2DGS保证多视角一致性但牺牲纹理细节。需要一种能平衡外观和几何的方法。

Method: 提出EGGS混合表示，包含：1) Hybrid Gaussian Rasterization统一渲染；2) Adaptive Type Exchange动态调整2D/3D高斯；3) Frequency-Decoupled Optimization充分利用各自优势；4) CUDA加速实现。

Result: 实验表明EGGS在渲染质量、几何精度和效率方面优于现有方法，为高质量NVS提供实用解决方案。

Conclusion: EGGS通过混合2D和3D高斯表示，有效平衡了外观保真度和几何一致性，解决了现有方法的局限性。
...
TL;DR: TrackNetV5通过引入运动方向解耦模块和残差驱动的时空细化头，解决了前代版本在遮挡处理和运动方向模糊性问题，在保持实时推理能力的同时实现了最先进的跟踪性能。


<details>
  <summary>Details</summary>
Motivation: TrackNet系列在快速运动小物体跟踪方面建立了强基线，但存在显著限制：V1-V3因依赖纯视觉线索而难以处理遮挡；TrackNetV4虽然引入了运动输入，但其绝对差分方法丢弃了运动极性，导致方向模糊性。需要克服这些瓶颈以实现更鲁棒的跟踪。

Method: 提出TrackNetV5架构，包含两个新颖机制：1) 运动方向解耦(MDD)模块，将时间动态分解为有符号的极性场，显式编码运动发生和轨迹方向；2) 残差驱动的时空细化(R-STR)头，采用粗到细的范式，利用分解的时空上下文估计校正残差，有效恢复被遮挡目标。

Result: 在TrackNetV2数据集上的广泛实验表明，TrackNetV5实现了新的最先进性能：F1分数0.9859，准确率0.9733，显著优于先前版本。性能提升仅带来3.7%的FLOPs增加，保持了实时推理能力。

Conclusion: TrackNetV5通过创新的运动方向建模和残差细化机制，有效解决了前代版本的遮挡和方向模糊问题，在计算效率几乎不变的情况下实现了显著的性能提升，为快速运动小物体跟踪提供了更鲁棒的解决方案。
...
### [91] [Benchmarking Scientific Understanding and Reasoning for Video Generation using VideoScience-Bench](https://arxiv.org/abs/2512.02942)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频基准主要基于物理常识，无法评估视频模型的科学推理能力。需要开发能够评估模型对真实世界科学定律理解的基准。

Method: 创建包含200个提示的VideoScience-Bench基准，涵盖14个主题和103个物理化学概念。使用VLM-as-a-Judge方法评估7个SOTA视频模型在5个维度上的表现。

Result: VLM-as-a-Judge评估与人类评估有强相关性。基准揭示了当前视频模型在科学推理方面的局限性，为评估模型作为推理者而不仅仅是生成者提供了工具。

Conclusion: VideoScience-Bench是首个评估视频模型科学推理能力的基准，要求生成视频展示与预期物理化学现象一致的科学理解，推动了视频生成向零样本推理的发展。
...
TL;DR: 提出了UnicEdit-10M大规模高质量图像编辑数据集和UnicBench综合评测基准，通过轻量级数据管道和Qwen-Verify专家模型解决现有数据规模与质量的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 当前闭源与开源多模态模型在图像编辑领域的性能差距日益扩大，主要原因是缺乏大规模高质量训练数据和能够全面诊断模型弱点的评测基准。现有数据构建方法面临规模与质量的权衡：人工标注质量高但不可扩展，自动化流程则存在错误传播和噪声问题。

Method: 1) 引入轻量级数据管道，用端到端模型替代多工具链，并加入统一的后验证阶段；2) 训练7B参数的双任务专家模型Qwen-Verify，用于高效失败检测和指令重写；3) 构建UnicEdit-10M数据集，包含1000万样本，涵盖基础和复杂编辑任务；4) 提出UnicBench评测基准，扩展基础编辑评估，明确测试空间和知识驱动推理能力；5) 引入新评测指标，包括非编辑一致性和推理准确性。

Result: 成功构建了UnicEdit-10M大规模高质量图像编辑数据集和UnicBench综合评测基准。通过对主流模型在UnicBench上的分析，揭示了它们的局限性，并为未来研究提供了明确方向。

Conclusion: 该工作通过创新的数据管道和验证方法解决了图像编辑领域的数据规模与质量权衡问题，提出的数据集和评测基准将有助于缩小闭源与开源模型之间的性能差距，并推动多模态图像编辑技术的进一步发展。
...
### [93] [In-Context Sync-LoRA for Portrait Video Editing](https://arxiv.org/abs/2512.03013)
...
<details>
  <summary>Details</summary>
Motivation: 肖像视频编辑需要灵活而精确的控制各种修改（如外观变化、表情编辑、添加物体），同时保持主体的原始时间行为，确保每个编辑帧与对应源帧精确同步。现有方法难以在编辑保真度和精确运动保持之间取得平衡。

Method: 使用图像到视频扩散模型，通过修改第一帧定义编辑，然后传播到整个序列。训练上下文LoRA，使用自动生成和筛选的配对视频（相同运动轨迹但不同外观），通过同步过滤选择最时间对齐的样本进行训练，使模型学会结合源视频的运动线索和编辑第一帧的视觉变化。

Result: 在紧凑、高度筛选的同步人类肖像数据集上训练，Sync-LoRA能够泛化到未见身份和多样化编辑（如修改外观、添加物体、改变背景），鲁棒处理姿势和表情变化。结果展示高视觉保真度和强时间一致性，在编辑保真度和精确运动保持之间取得稳健平衡。

Conclusion: Sync-LoRA提供了一种有效的肖像视频编辑方法，通过上下文LoRA训练实现了高质量视觉修改与精确时间同步的结合，为肖像视频编辑任务提供了新的解决方案。
...
### [94] [IC-World: In-Context Generation for Shared World Modeling](https://arxiv.org/abs/2512.02793)
...
Method: 提出IC-World框架：1）利用大型视频模型的上下文生成能力并行处理所有输入图像；2）通过强化学习（Group Relative Policy Optimization）微调模型；3）引入两个新的奖励模型来增强生成视频之间的场景级几何一致性和物体级运动一致性。

Result: 大量实验表明，IC-World在几何一致性和运动一致性方面显著优于现有最先进方法，是首个系统探索基于视频模型的共享世界建模问题的工作。

Conclusion: IC-World通过激活大型视频模型的上下文生成能力并结合强化学习优化，成功解决了共享世界建模中的几何和运动一致性问题，为视频世界模型的发展提供了新方向。
...
### [95] [PhyCustom: Towards Realistic Physical Customization in Text-to-Image Generation](https://arxiv.org/abs/2512.02794)
...
### [96] [ViSAudio: End-to-End Video-Driven Binaural Spatial Audio Generation](https://arxiv.org/abs/2512.03036)
...
<details>
  <summary>Details</summary>
Motivation: 当前视频到音频生成主要关注单声道输出，缺乏空间沉浸感。现有的双耳音频方法采用两阶段流程（先生成单声道音频再进行空间化），导致误差累积和时空不一致。

Method: 提出ViSAudio框架，采用条件流匹配和双分支音频生成架构，两个专用分支建模音频潜在流。结合条件时空模块，平衡通道间一致性同时保留独特空间特征，确保音频与输入视频的精确时空对齐。

Result: ViSAudio在客观指标和主观评估上均优于现有最先进方法，能生成高质量双耳音频，具有空间沉浸感，有效适应视角变化、声源运动和多样化声学环境。

Conclusion: 该研究提出了端到端双耳空间音频生成任务和ViSAudio框架，通过BiAudio数据集和创新的双分支架构，实现了直接从视频生成高质量空间音频，解决了现有方法的局限性。
...
### [97] [Video4Spatial: Towards Visuospatial Intelligence with Context-Guided Video Generation](https://arxiv.org/abs/2512.03040)
...
Method: 提出Video4Spatial框架，基于视频扩散模型，仅使用视频场景上下文进行条件化，通过有效的框架设计和数据整理，处理场景导航和物体定位任务

Result: 模型能够端到端规划导航和定位目标物体，遵循相机姿态指令同时保持空间一致性，并能泛化到长上下文和域外环境

Conclusion: 这些结果推动了视频生成模型向通用视觉空间推理的发展，证明了仅凭视频数据就能实现复杂的空间理解能力
...
### [98] [PPTArena: A Benchmark for Agentic PowerPoint Editing](https://arxiv.org/abs/2512.03042)
*Michael Ofengenden,Yunze Man,Ziqi Pang,Yu-Xiong Wang*

Main category: cs.CV

TL;DR: PPTArena是一个PowerPoint编辑基准测试，专注于基于自然语言指令对真实幻灯片进行可靠修改，包含100个演示文稿、2125张幻灯片和800多个针对性编辑。PPTPilot是一个结构感知的幻灯片编辑代理，通过规划语义编辑序列和迭代验证来提升编辑效果。


<details>
  <summary>Details</summary>
Motivation: 当前PowerPoint编辑任务缺乏可靠的评估基准，现有方法主要关注图像PDF渲染或文本到幻灯片生成，而忽略了在实际幻灯片上进行精确的原地编辑。需要建立一个能够衡量在真实幻灯片上执行自然语言指令编辑能力的基准。

Method: PPTArena基准包含100个演示文稿、2125张幻灯片和800多个针对性编辑，涵盖文本、图表、表格、动画和母版样式。采用双重VLM作为评判者，分别评估指令遵循和视觉质量。PPTPilot是一个结构感知的幻灯片编辑代理，通过规划语义编辑序列，在高层次程序化工具和确定性XML操作之间路由，并通过迭代的计划-编辑-检查循环验证输出。

Result: PPTPilot在复合编辑、布局敏感编辑和跨幻灯片编辑方面优于强大的专有代理和前沿VLM系统，提高了10多个百分点，在视觉保真度和整个演示文稿一致性方面取得了特别大的改进。然而，现有代理在PPTArena中的长时程、文档规模任务上仍然表现不佳。

Conclusion: PPTArena为PowerPoint编辑任务提供了一个可靠的评估基准，PPTPilot展示了结构感知编辑方法的有效性，但可靠PPT编辑仍面临挑战，特别是在长时程、文档规模任务上需要进一步改进。
...
TL;DR: AAG方法通过单帧RGB+深度特征结合先验动作信息（文本摘要或单帧识别预测）实现动作预测，在三个教学活动中数据集上达到与视频聚合方法竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 人类通过观察场景中的单个时刻结合上下文就能预测即将发生的动作，研究是否能用模型实现这种能力，以及视频时间聚合是否可以被其他模态替代。

Method: 提出AAG方法：结合单帧RGB特征和深度线索增强空间推理，融入先验动作信息提供长期上下文（通过视觉语言模型生成文本摘要或单帧动作识别器预测）。

Result: 在IKEA-ASM、Meccano和Assembly101三个教学活动中数据集上，AAG的多模态单帧动作预测性能与时间聚合视频基线和最先进方法具有竞争力。

Conclusion: 通过适当的模态组合和上下文信息，单帧动作预测可以达到与视频聚合方法相当的性能，为动作理解研究提供了新方向。
...
TL;DR: 本文首次系统研究了印度和南亚面孔的身份保持AIGC检测，发现现有检测器在跨生成器泛化和特定人群性能上存在显著不足，尤其是微调后对身份保持编辑的泛化能力下降。


<details>
  <summary>Details</summary>
Motivation: 现代图像编辑器能生成身份保持的AIGC内容，但现有检测器在这种情况下的鲁棒性和公平性尚不明确，特别是对印度和南亚等代表性不足的人群。

Method: 构建了印度聚焦的训练数据集（FairFD和HAV-DF），并创建了两个保留的身份保持AIGC测试集（HIDF-img-ip-genai和HIDF-vid-ip-genai），使用商业生成器（Gemini和ChatGPT）生成。评估了两种最先进的检测器（AIDE和Effort）在预训练和微调两种模式下的性能。

Result: 微调在域内带来显著提升，但在保留的印度身份保持AIGC测试集上性能一致下降，表明过拟合到训练生成器的线索。在非身份保持的HIDF图像上，预训练性能保持良好，说明问题特定于身份保持编辑而非一般分布偏移。

Conclusion: 身份保持AIGC-印度是一个具有挑战性且实际相关的场景，需要开发保持表征的适应方法和印度感知的基准构建，以缩小AIGC检测的泛化差距。
...
TL;DR: 该论文提出了一种用于多语言环境下人脸-语音关联的方法，在FAME 2026挑战赛中排名第三，实现了33.1%的等错误率。


<details>
  <summary>Details</summary>
Motivation: 多语言环境下的人脸-语音关联任务具有挑战性，FAME 2026挑战赛引入了英德双语数据，需要开发能够有效处理跨模态语义信息的方法。

Method: 重新审视了融合和正交投影方法，通过有效聚焦于两个模态中的相关语义信息来实现人脸-语音关联。

Result: 在英德数据分割上表现良好，在FAME 2026挑战赛中排名第三，实现了33.1%的等错误率。

Conclusion: 提出的方法在多语言人脸-语音关联任务中有效，通过聚焦相关语义信息提升了跨模态关联性能。
...
TL;DR: STSR 2025挑战赛在MICCAI 2025上举办，旨在评估半监督学习在CBCT牙齿/牙髓管分割和CBCT-IOS跨模态配准中的应用，提供了标注和未标注数据集，吸引了社区参与并取得了良好结果。
...
Method: 组织STSR 2025挑战赛，包含两个任务：1) CBCT中牙齿和牙髓管的半监督分割；2) CBCT和IOS的半监督刚性配准。提供标注和未标注数据集，收集社区提交的解决方案。

Result: 分割任务中最佳方法使用nnU-Net和Mamba-like状态空间模型，结合伪标签和一致性正则化，在隐藏测试集上获得Dice分数0.967和实例亲和度0.738。配准任务中有效方法结合PointNetLK与可微SVD和几何增强，采用混合神经-经典细化实现准确对齐。

Conclusion: 挑战赛成功展示了半监督学习在数字牙科中的潜力，提供了公开数据集和代码以促进可重复性研究，推动了CBCT分割和跨模态配准技术的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型主要依赖监督微调，在线强化学习后训练在相机控制视频生成领域尚未充分探索，需要更有效的优化方法来提升相机控制的精确性。

Method: 设计可验证的几何奖励函数，估计生成视频和参考视频的3D相机轨迹，将轨迹分割为短片段，计算片段间相对位姿，通过对比生成-参考片段对提供密集的段级反馈信号。

Result: 在线强化学习后训练在相机控制精度、几何一致性和视觉质量等多个方面明显优于监督微调基线，证明了该方法在提升相机控制视频生成性能方面的优越性。

Conclusion: 提出的在线强化学习后训练框架通过几何奖励函数有效解决了奖励稀疏性问题，显著提升了相机控制视频生成的性能，为视频生成领域提供了新的优化方法。
...
<details>
  <summary>Details</summary>
Motivation: 为了提升多模态大语言模型的基础能力和泛化能力，同时降低训练和部署成本，需要一种通用的后训练范式来平衡领域特定知识注入与通用能力保持。

Method: 提出三个关键创新：1) 基于信息密度的数据生成方案与双维度树状标签系统；2) 协作课程监督微调方法；3) 混合强化学习范式。同时实施5D并行训练、算子优化、推理量化等基础设施优化。

Result: 在MMBench、MMStar、MathVision、MathVista等基准测试中超越现有SOTA模型，在垂直领域任务中展现优越用户体验，实现从学术研究到工业部署的无缝过渡。

Conclusion: MindGPT-4ov提供了一个适用于广泛MLLM的通用后训练范式，将开源模型权重、数据集和代码以支持社区发展，为多模态大语言模型的研究和部署提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的LiDAR地点识别研究多关注端到端3D学习，但缺乏对2D投影本身影响的系统分析。本研究旨在探究不同LiDAR到图像投影方式如何影响地点识别性能，特别是在与先进的视觉基础模型结合时。

Method: 提出模块化检索流程，控制骨干网络、聚合方法和评估协议，隔离2D投影本身的影响。使用一致的几何和结构通道，在多个数据集和部署场景中测试不同投影特性。

Result: 识别出对判别力、环境变化鲁棒性和实时自主性最关键的投影特性。实验验证了这些发现在实际地点识别策略中的相关性，表明精心设计的投影可以替代端到端3D学习。

Conclusion: LiDAR到图像的投影设计对地点识别性能有显著影响，通过系统分析投影特性可以优化识别效果，精心设计的投影可以作为端到端3D学习的有效替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然图像生成效果出色，但存在计算成本高、推理步骤多的问题。现有的少步蒸馏方法需要大量重训练且泛化能力下降，需要更智能的加速策略

Method: 采用阶段感知策略：早期语义阶段应用较小加速，后期冗余阶段应用较大加速。使用两个专家模型：Slow-LoRA处理慢速去噪阶段，Fast-LoRA处理快速去噪阶段。仅需为基模型添加轻量级LoRA适配器，无需大量重训练

Result: 方法实现高达5倍加速，同时在多样化基准测试中保持可比的视觉质量。LoRA专家仅需1个样本在单张V100上训练1小时，就能在未见过的提示上表现出强大的泛化能力

Conclusion: 通过阶段感知的LoRA适配器策略，实现了扩散模型的高效加速和强泛化能力，避免了传统蒸馏方法的高成本和性能下降问题
...
TL;DR: DiverseAR通过自适应logits分布缩放和基于能量的生成路径搜索，解决了比特级自回归模型中样本多样性不足的问题，在保持视觉质量的同时显著提升了生成多样性。


<details>
  <summary>Details</summary>
Motivation: 比特级自回归生成模型存在样本多样性不足的问题，主要源于两个因素：1）比特级建模的二分类性质限制了预测空间；2）过于尖锐的logits分布导致采样崩溃和多样性降低。

Method: 提出DiverseAR方法，包含两个核心技术：1）自适应logits分布缩放机制，在采样时动态调整二分类输出分布的尖锐度，实现更平滑的预测和更高多样性；2）基于能量的生成路径搜索算法，避免采样低置信度token，以保持高视觉质量。

Result: 大量实验证明，DiverseAR在比特级自回归图像生成中显著提升了样本多样性，同时没有牺牲视觉质量。

Conclusion: DiverseAR为比特级自回归模型提供了一种原则性且有效的方法，解决了样本多样性受限的问题，在保持生成质量的同时实现了更好的多样性表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本引导视频编辑方法通常依赖辅助掩码或参考图像，这限制了方法的可扩展性和泛化能力，特别是在对象移除和添加任务中难以保持时空一致性

Method: 提出LoVoRA框架，包含独特的数据集构建流程（图像到视频转换、光流掩码传播、视频修复）和核心创新：可学习的对象感知定位机制，通过扩散掩码预测器实现端到端视频编辑

Result: 广泛的实验和人工评估表明LoVoRA在视频对象移除和添加任务中表现出有效性和高质量性能，无需推理时的外部控制信号

Conclusion: LoVoRA通过创新的对象感知定位机制解决了文本引导视频编辑中的时空一致性问题，为无需掩码的视频对象编辑提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 室内布局估计在增强现实和大规模3D场景重建中至关重要，但现有方法通常需要复杂的后处理流程且计算效率不高。本文旨在开发一个结合几何感知和计算效率的端到端框架。

Method: 基于OneFormer的Transformer架构，集成任务条件查询和对比学习，并引入两个关键模块：1）布局退化策略，通过拓扑感知变换增强训练数据同时保持曼哈顿世界约束；2）可微几何损失，在训练中直接强制平面一致性和锐利边界预测。
...
Conclusion: Layout Anything通过统一几何感知和计算效率的端到端框架，消除了复杂的后处理流程，在室内布局估计任务中表现出色，特别适合增强现实应用和大规模3D场景重建。
...
TL;DR: 提出UltraFast-LieNET，一个超轻量级多尺度移位卷积网络，用于实时低光图像增强，仅需36-180个参数，在LOLI-Street数据集上PSNR达到26.51dB，比现有方法提升4.6dB。


<details>
  <summary>Details</summary>
Motivation: 在夜间驾驶等低光环境下，图像质量下降严重影响车载摄像头安全。现有增强算法通常计算量过大，不适合车载应用，需要开发轻量级实时解决方案。

Method: 提出动态移位卷积(DSConv)核，仅含12个可学习参数；结合不同移位距离构建多尺度移位残差块(MSRB)扩大感受野；采用残差结构和多级梯度感知损失函数稳定轻量网络训练。

Result: 在LOLI-Street数据集上PSNR达到26.51dB，比现有方法提升4.6dB，仅使用180个参数；最小配置仅需36个参数；在四个基准数据集上验证了实时性能与增强质量的优越平衡。

Conclusion: UltraFast-LieNET在有限计算资源下实现了实时低光图像增强的卓越性能，为车载安全应用提供了高效解决方案，代码已开源。
...
TL;DR: BEVDilation：一种以LiDAR为中心的BEV融合框架，通过将图像特征作为隐式引导而非简单拼接，解决多传感器融合中的空间错位问题，提升3D目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有LiDAR和相机融合方法在BEV表示中存在几何精度差异问题，简单融合会导致性能下降。需要一种能优先利用LiDAR信息、同时有效整合图像语义引导的融合策略。

Method: 提出BEVDilation框架：1）将图像BEV特征作为隐式引导而非简单拼接；2）设计Sparse Voxel Dilation Block利用图像先验稠密化前景体素；3）引入Semantic-Guided BEV Dilation Block通过图像语义引导增强LiDAR特征扩散和长程上下文捕获。

Result: 在nuScenes基准测试中优于现有方法，同时保持计算效率。以LiDAR为中心的策略对深度噪声具有更强的鲁棒性。

Conclusion: BEVDilation通过以LiDAR为中心、图像为引导的融合策略，有效解决了多传感器融合中的空间错位问题，提升了3D目标检测性能并增强了鲁棒性。
...
### [112] [Contextual Image Attack: How Visual Context Exposes Multimodal Safety Vulnerabilities](https://arxiv.org/abs/2512.02973)
*Yuan Xiong,Ziqi Miao,Lijun Li,Chen Qian,Jie Li,Jing Shao*

Main category: cs.CV

TL;DR: 提出了一种新的图像中心攻击方法CIA，通过多智能体系统将有害查询嵌入看似良性的视觉上下文中，显著提高了对多模态大语言模型的越狱攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有攻击方法主要关注文本-图像交互，将视觉模态视为次要提示，未能充分利用图像承载复杂上下文信息的独特潜力。需要探索更有效的图像中心攻击方法。
...
Conclusion: 视觉模态本身是越狱先进多模态大语言模型的有效向量，图像中心攻击方法能够显著提高攻击成功率，揭示了当前安全对齐的脆弱性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖人工干预或未充分利用智能体自主缓解幻觉的能力，需要借鉴人类决策过程：先内省推理降低不确定性，再通过外部验证形成最终决策

Method: 提出InEx框架：1) 基于熵的不确定性估计指导内省推理；2) 决策智能体生成响应；3) 通过编辑智能体和自反思智能体的外部跨模态多智能体协作进行迭代验证和精炼

Result: 在通用和幻觉基准测试中持续优于现有方法，获得4%-27%的性能提升，并展现出强大的鲁棒性

Conclusion: InEx框架通过模拟人类认知过程，有效缓解多模态大语言模型的幻觉问题，为开发可靠的多模态AI系统提供了有前景的解决方案
...
TL;DR: U4D：一种不确定性感知的4D LiDAR世界建模框架，通过空间不确定性估计和"难到易"生成策略，提升复杂场景的几何保真度和时间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有生成框架通常对所有空间区域进行均匀处理，忽略了真实场景中不同区域的不确定性差异。这种均匀生成导致复杂或模糊区域出现伪影，限制了真实感和时间稳定性。

Method: 1. 使用预训练分割模型估计空间不确定性图，定位语义挑战性区域；2. 采用"难到易"两阶段生成：不确定性区域建模（重建高熵区域）和不确定性条件补全（在结构先验下合成剩余区域）；3. 引入时空混合块自适应融合时空表示。

Result: 实验表明U4D能够生成几何保真且时间一致的LiDAR序列，提升了4D世界建模在自主感知和仿真中的可靠性。

Conclusion: U4D通过不确定性感知的生成策略和时空融合机制，显著改善了4D LiDAR世界建模的质量，为自动驾驶和具身AI提供了更可靠的环境建模方法。
...
TL;DR: GraphFusion3D：一个结合多模态融合与特征学习的统一框架，通过自适应跨模态Transformer和图推理模块提升3D目标检测性能
...
Method: 提出GraphFusion3D框架，包含自适应跨模态Transformer（ACMT）用于图像特征与点云表示的自适应融合，以及图推理模块（GRM）通过多尺度图注意力同时捕捉局部几何结构和全局语义上下文，采用级联解码器进行多阶段预测优化

Result: 在SUN RGB-D数据集上达到70.6% AP25和51.2% AP50，在ScanNetV2数据集上达到75.1% AP25和60.8% AP50，相比现有方法有显著性能提升

Conclusion: GraphFusion3D通过多模态融合和图推理机制有效解决了点云数据的稀疏性和语义信息不足问题，显著提升了3D目标检测性能，为复杂场景理解提供了有效解决方案
...
TL;DR: TEXTRIX是一个原生3D属性生成框架，通过构建潜在3D属性网格和稀疏注意力扩散Transformer，直接在体积空间中为3D模型着色，避免了多视图融合的限制，并扩展到高精度3D分割任务。
...
Method: 构建潜在3D属性网格，采用配备稀疏注意力的扩散Transformer，直接在体积空间中为3D模型着色，避免多视图融合问题。基于此原生表示，同一架构可扩展到预测网格上的语义属性，实现高精度3D分割。

Result: 在纹理合成和3D部件分割任务上均达到最先进性能，生成无缝、高保真纹理和具有精确边界的准确3D部件分割。

Conclusion: TEXTRIX通过原生3D表示和稀疏注意力扩散Transformer，有效解决了多视图融合方法的局限性，实现了高质量的3D纹理生成和精确分割，为3D内容创作提供了新范式。
...
TL;DR: DynamicVerse是一个物理尺度的多模态4D世界建模框架，用于动态真实世界视频，通过整合大模型生成度量尺度静态几何、真实世界动态运动、实例级掩码和整体描述性标注，构建了包含10万+视频、80万+标注掩码和1000万+帧的大规模数据集。


<details>
  <summary>Details</summary>
Motivation: 现有数据集通常来自有限模拟器或使用传统Structure-from-Motion方法，提供有限的描述性标注，这限制了基础模型从单目视频准确解释真实世界动态的能力。需要填补真实世界动态物理建模的空白。

Method: 采用大视觉、几何和多模态模型解释度量尺度静态几何、真实世界动态运动、实例级掩码和整体描述性标注。通过整合基于窗口的Bundle Adjustment与全局优化，将长真实世界视频序列转换为全面的4D多模态格式。

Result: 构建了包含100K+视频、800K+标注掩码和10M+帧的大规模数据集。在视频深度估计、相机姿态估计和相机内参估计三个基准任务上的实验评估表明，该方法在捕捉物理尺度测量方面优于现有方法，具有更高的全局准确性。

Conclusion: DynamicVerse框架成功填补了真实世界动态物理建模的空白，为理解动态物理世界提供了物理尺度的多模态4D表示，能够支持更准确的人机交互和具身智能体在真实环境中的感知与行动。
...
TL;DR: DGGT：一个无需相机位姿的动态场景重建框架，通过前馈方式联合预测3D高斯地图和相机参数，支持任意数量视图的长序列重建，在自动驾驶数据集上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶的动态场景重建方法大多依赖每场景优化、已知相机标定或短帧窗口，导致速度慢且不实用。现有方法将相机位姿作为必需输入，限制了灵活性和可扩展性。

Method: 提出Driving Gaussian Grounded Transformer (DGGT)，将相机位姿重新定义为模型输出而非输入。联合预测每帧3D高斯地图和相机参数，通过轻量级动态头解耦动态元素，使用寿命头保持时间一致性，并采用基于扩散的渲染细化减少伪影。

Result: 在Waymo、nuScenes、Argoverse2等大规模自动驾驶基准测试中，无论是单数据集训练还是跨数据集零样本迁移，均优于先前工作。随着输入帧数增加，方法具有良好的可扩展性。

Conclusion: DGGT实现了单次前向、无需位姿的动态场景重建算法，在性能和速度上达到SOTA水平，为自动驾驶的训练和评估提供了快速、可扩展的4D重建和重模拟解决方案。
...
TL;DR: SurfFill：一种基于高斯面元的LiDAR点云补全方法，通过结合LiDAR和相机优势，识别并补全LiDAR扫描中缺失的细小几何结构


<details>
  <summary>Details</summary>
Motivation: LiDAR在平坦区域精度高，但容易遗漏细小几何结构和深色吸光材料；而多视角摄影测量能捕捉这些细节但精度不足。需要结合两者优势来提升点云完整性。

Method: 1) 分析LiDAR光束发散导致的边缘和薄结构缺失；2) 提出模糊性启发式方法，通过点云密度变化识别缺失区域；3) 在模糊区域约束高斯面元重建进行优化和密集化；4) 提取高斯基元采样补全点云；5) 针对大规模重建采用分治策略。

Result: 在合成和真实场景的LiDAR点云补全任务中，该方法优于之前的重建方法。

Conclusion: SurfFill成功结合了LiDAR和相机优势，通过高斯面元重建有效补全了LiDAR扫描中的缺失区域，特别适用于边缘和薄结构，并支持大规模场景重建。
...
TL;DR: 提出一种通用方法，通过插入稳定性适配器和高效训练过程，使帧级模型在视频处理中获得时间一致性和鲁棒性，同时保持预测质量。


<details>
  <summary>Details</summary>
Motivation: 帧级网络在视频序列处理中常出现时间不一致问题（如帧间闪烁），当输入包含时变噪声时问题更严重。需要一种通用方法使现有帧级模型在视频上实现稳定鲁棒的推理。

Method: 1. 提出可插入任何架构的稳定性适配器类；2. 设计资源高效的训练过程（基础网络冻结）；3. 提出统一概念框架，以精度-稳定性-鲁棒性损失为中心；4. 分析该损失的理论特性，确定稳定器训练条件。

Result: 在多个视觉任务（去噪、图像增强、单目深度估计、语义分割）上验证有效，方法提升了时间稳定性和对多种图像损坏（压缩伪影、噪声、恶劣天气）的鲁棒性，同时保持或改进了预测质量。

Conclusion: 提出了一种通用框架，通过稳定性适配器和高效训练，使帧级模型在视频处理中实现时间一致性和鲁棒性，为视频推理提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: B-Rep是CAD中定义实体模型的标准数据结构，但现有方法难以端到端生成具有精确几何和水密拓扑的B-Rep模型

Method: 提出统一的token化方案，将几何基元编码为潜在几何token，结构关系定义为拓扑引用token；采用Transformer模型自回归生成，序列顺序遵循B-Rep面邻接图的广度优先遍历

Result: AutoBrep在B-Rep生成任务上优于基线方法，具有更好的质量和水密性；可扩展到复杂实体，具有良好的保真度和推理速度；支持B-Rep自动补全功能

Conclusion: AutoBrep通过统一的token表示结合下一token预测，实现了高质量、有效的B-Rep生成，为可控CAD生成提供了新方法
...
TL;DR: 该论文提出Flow-Aligned Training (FLAT)方法，通过将展开网络与条件概率流ODE联系起来，改进MRI重建的稳定性和收敛速度，相比扩散模型减少3倍迭代次数，比传统展开网络更稳定。


<details>
  <summary>Details</summary>
Motivation: MRI采集时间长限制了临床应用，深度学习加速MRI重建是重要方向。展开网络效率高但存在不稳定演化问题，扩散模型理论稳定但计算昂贵。需要找到既能保证稳定性又高效的方法。

Method: 提出Flow-Aligned Training (FLAT)：1) 理论证明展开网络是条件概率流ODE的离散实现；2) 从ODE离散化推导展开网络参数；3) 将中间重建与理想ODE轨迹对齐，提高稳定性和收敛性。

Result: 在三个MRI数据集上的实验表明：1) FLAT实现高质量重建；2) 相比扩散生成模型减少最多3倍迭代次数；3) 相比展开网络显著提高稳定性。

Conclusion: 通过建立展开网络与条件概率流ODE的理论联系，提出的FLAT方法在MRI重建中实现了更好的稳定性和效率平衡，为加速MRI采集提供了有效解决方案。
...
TL;DR: MAViD是一个新颖的多模态音频-视觉对话理解和生成框架，采用Conductor-Creator架构，能够生成生动、上下文连贯的长时长对话交互。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注非交互式系统，生成的语音受限且不自然。主要挑战在于有效整合理解与生成能力，以及实现无缝的多模态音频-视频融合。

Method: 提出Conductor-Creator架构：Conductor负责理解、推理并生成指令，分解为动作和语音组件；Creator基于指令生成交互响应。采用自回归模型生成音频，扩散模型生成高质量视频，并提出新颖的融合模块增强连续片段和多模态间的连接。

Result: 广泛实验表明，该框架能够生成生动且上下文连贯的长时长对话交互，并能准确解读用户的多模态查询。

Conclusion: MAViD框架成功解决了多模态音频-视觉对话中的理解和生成挑战，通过创新的架构设计和融合模块，实现了高质量、长时长的同步音频-视觉内容生成。
...
TL;DR: MultiShotMaster：一个用于高度可控多镜头视频生成的框架，通过改进RoPE实现灵活镜头安排、连贯叙事和时空参考注入
...
Method: 扩展预训练单镜头模型，集成两种新型RoPE变体：多镜头叙事RoPE（在镜头转换时应用显式相位偏移）和时空位置感知RoPE（结合参考标记和接地信号）。建立自动数据标注流程提取多镜头视频数据
...
TL;DR: OneThinker是一个统一的多模态推理模型，将图像和视频理解整合到单一模型中，涵盖问答、描述、时空定位、跟踪和分割等10个基础视觉任务，通过EMA-GRPO方法处理多任务强化学习中的奖励异质性问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常为不同任务训练独立模型，将图像和视频推理视为分离领域，这限制了向多模态推理通用模型的可扩展性，阻碍了跨任务和跨模态的知识共享。
...
Result: 在31个基准测试中表现出强大性能，涵盖10个基础视觉理解任务，展现出任务间的有效知识转移和初步的零样本泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 多视角扩散模型在保持视角一致性方面的机制尚不明确。研究发现这些模型的注意力图在训练过程中获得了几何对应关系，但这种对应信号不完整，在大视角变化下准确性会下降。

Method: 提出CAMEO训练技术，直接使用几何对应关系监督注意力图。仅监督单个注意力层就足以引导模型学习精确的对应关系，从而保持参考图像的几何结构。

Result: CAMEO将收敛所需的训练迭代次数减少一半，同时在相同迭代次数下实现更优性能。该方法具有模型无关性，可应用于任何多视角扩散模型。

Conclusion: 通过几何对应监督注意力图是一种简单有效的训练技术，能显著提升多视角扩散模型的训练效率和生成质量，同时保持视角一致性。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器虽然在整体生成方面表现出色，但其使用的单一、整体提示无法分离用户对内容、位置和外观的不同意图，导致生成控制不够精细。

Method: 方法包括：1）将创作意图解构为四个可控视觉线索层；2）用于上下文感知内容集成的专门数据生成流程；3）处理所有视觉线索的统一控制模块；4）用于精确局部编辑（包括对象移除）的微调空间分支。

Result: 大量实验验证了这种分层方法能有效解决用户意图差距，为创作者提供直接、直观的生成过程控制。

Conclusion: MagicQuill V2通过分层组合范式成功弥合了扩散模型语义能力与传统图形软件精细控制之间的差距，实现了更可控的生成式图像编辑。
...
### [128] [An Improved Ensemble-Based Machine Learning Model with Feature Optimization for Early Diabetes Prediction](https://arxiv.org/abs/2512.02023)
...
TL;DR: 该研究提出一个基于机器学习的糖尿病早期预测框架，使用BRFSS数据集，通过SMOTE和Tomek Links处理类别不平衡，采用随机森林、XGBoost等模型，最终堆叠集成模型达到94.82%准确率和0.989 ROC-AUC，并开发了React Native应用支持临床决策。


<details>
  <summary>Details</summary>
Motivation: 糖尿病是全球严重的健康问题，早期检测对成功干预至关重要。然而，重叠的风险因素和数据不对称使得预测困难。研究旨在利用大规模健康调查数据构建准确且可解释的机器学习框架，以支持临床决策。

Method: 使用2015年BRFSS数据集（约253,680条记录，22个数值特征），采用SMOTE和Tomek Links处理类别不平衡。评估多种监督学习方法，包括随机森林、XGBoost、CatBoost、LightGBM等个体模型，并研究堆叠集成技术。最终开发了基于React Native的应用和Python Flask后端。

Result: 个体模型（随机森林、XGBoost、CatBoost、LightGBM）的ROC-AUC达到约0.96。堆叠集成模型（XGBoost和KNN）表现最佳：准确率94.82%，ROC-AUC 0.989，PR-AUC 0.991，在召回率和精确度之间取得良好平衡。

Conclusion: 研究成功开发了一个高准确性的糖尿病预测机器学习框架，通过堆叠集成方法显著提升了性能。开发的React Native应用为早期糖尿病预测提供了可访问且高效的健康监测工具，有助于临床决策支持。
...
### [129] [Pharmacophore-based design by learning on voxel grids](https://arxiv.org/abs/2512.02031)
*Omar Mahmood,Pedro O. Pinheiro,Richard Bonneau,Saeed Saremi,Vishnu Sresht*

Main category: cs.LG

TL;DR: 提出VoxCap方法，通过体素字幕生成SMILES，解决传统基于药效团虚拟筛选的扩展性和生成性问题


<details>
  <summary>Details</summary>
Motivation: 传统基于药效团的虚拟筛选方法存在两个主要问题：1) 随着分子库规模增大，计算扩展性差；2) 只能从现有化合物库中筛选，无法生成新分子。需要一种既能高效搜索又能生成新分子的方法。

Method: 提出VoxCap方法，这是一种基于体素字幕生成的技术，能够从体素化的分子表示生成SMILES字符串。设计了两种工作流程：1) 从头设计，生成与查询分子具有高药效团-形状相似性的新分子；2) 快速搜索，结合生成设计和廉价的2D子结构相似性搜索，实现高效命中识别。

Result: VoxCap在生成多样化从头命中分子方面显著优于先前方法。结合快速搜索工作流程时，VoxCap将计算时间减少了几个数量级，同时为所有查询分子返回命中结果，使得搜索大型分子库成为可能。

Conclusion: VoxCap方法有效解决了传统基于药效团虚拟筛选的扩展性和生成性限制，为药物发现提供了更高效、更具创造性的解决方案。
...
### [130] [PIBNet: a Physics-Inspired Boundary Network for Multiple Scattering Simulations](https://arxiv.org/abs/2512.02049)
...
<details>
  <summary>Details</summary>
Motivation: 边界元法（BEM）在求解无界均匀域中的多散射问题时，需要首先确定边界上的解迹，这是主要的计算瓶颈。传统方法计算成本高，需要开发更高效的近似方法。

Method: 提出PIBNet学习框架：1）采用物理启发的图基策略建模障碍物及其长程相互作用；2）设计新颖的多尺度图神经网络架构模拟多散射现象；3）构建包含多种多散射问题类型的数据集进行训练和评估。

Result: PIBNet在多个任务上超越了现有最先进的学习方法，并且在障碍物数量增加的情况下表现出更好的泛化能力。

Conclusion: PIBNet为边界元法中的解迹近似提供了高效的学习方法，显著降低了计算复杂度，在多散射问题中展现出优越性能和泛化能力。
...
### [131] [Contextual Gating within the Transformer Stack: Synergistic Feature Modulation for Enhanced Lyrical Classification and Calibration](https://arxiv.org/abs/2512.02053)
*M. A. Gameiro*

Main category: cs.LG

TL;DR: 提出SFL Transformer，通过上下文门控机制将辅助结构特征集成到Transformer自注意力中，在歌词分类任务上实现SOTA性能和高可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有特征融合方法通常在最终输出层融合特征，无法充分利用结构特征与语义特征的协同作用。需要探索更有效的特征融合策略，将辅助结构信息直接注入Transformer编码器中间层。

Method: 提出SFL Transformer模型，采用上下文门控机制（Intermediate SFL），在BERT编码器堆栈的隐藏状态序列中调制结构特征。该方法使用低维结构线索（Fstruct）调制深度上下文语义特征（Hseq），而非在最终输出层融合特征。

Result: 在基于UMAP降维歌词嵌入的二元分类任务中，SFL Transformer达到准确率0.9910和宏F1分数0.9910，显著超越先前SFL模型（准确率0.9894）。模型可靠性极高，期望校准误差仅0.0081，对数损失0.0489。

Conclusion: 验证了在编码器中间层注入辅助上下文是结合结构和语义信息的最有效方式，创造了兼具卓越判别能力和高保真概率估计的模型。上下文门控策略为特征融合提供了新范式。
...
### [132] [Opening the Black Box: An Explainable, Few-shot AI4E Framework Informed by Physics and Expert Knowledge for Materials Engineering](https://arxiv.org/abs/2512.02057)
...
TL;DR: 提出一个可解释的少样本AI4E框架，通过物理知识和专家知识增强，仅用32个实验样本就能发现可解释的本构方程，预测热裂纹倾向准确率达88%，并可用于过程优化和虚拟数据生成。


<details>
  <summary>Details</summary>
Motivation: 工业中AI4E应用面临两大瓶颈：高质量数据稀缺和黑盒模型缺乏可解释性，这在航空航天等安全敏感领域尤为关键。需要开发可信赖的AI系统，将工程领域知识直接嵌入架构中。

Method: 1) 通过三阶段协议增强物理合理的合成数据：基于过程变异性的差异化噪声注入、硬物理约束强制执行、参数间关系保持；2) 采用嵌套优化策略进行本构模型发现：符号回归探索方程结构，差分进化优化参数，然后使用混合全局-局部优化进行参数精炼。

Result: 仅从32个实验样本出发，得到的可解释本构方程在预测K439B高温合金铸件修复焊接的热裂纹倾向方面达到88%的准确率。该方程不仅提供定量预测，还揭示热、几何和冶金机制如何耦合驱动裂纹，增强了工程师对过程的认知理解。
...
### [133] [Ada-MoGE: Adaptive Mixture of Gaussian Expert Model for Time Series Forecasting](https://arxiv.org/abs/2512.02061)
*Zhenliang Ni,Xiaowen Ma,Zhenkai Wu,Shuai Xiao,Han Shu,Xinghao Chen*

Main category: cs.LG

TL;DR: 提出Ada-MoGE模型，通过自适应高斯混合专家机制解决多元时间序列预测中频率分布变化导致的专家数量固定问题，在6个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 多元时间序列预测中，数据频谱分布会随时间演变，导致主导频率偏移。传统固定专家数量的MoE模型难以适应这种变化，会出现频率覆盖不平衡问题：专家太少会忽略关键信息，太多则会引入噪声。

Method: 提出Ada-MoGE（自适应高斯混合专家模型），集成频谱强度和频率响应来自适应确定专家数量，确保与输入数据的频率分布对齐。采用高斯带通滤波平滑分解频域特征，避免直接频带截断引入噪声。

Result: 在6个公共基准测试中取得了最先进的性能，仅使用20万个参数。

Conclusion: Ada-MoGE通过自适应确定专家数量，有效解决了传统MoE模型在多元时间序列预测中的频率覆盖不平衡问题，实现了更好的预测性能。
...
### [134] [DPWMixer: Dual-Path Wavelet Mixer for Long-Term Time Series Forecasting](https://arxiv.org/abs/2512.02070)
*Li Qianyang,Zhang Xingjun,Wang Shaoxun,Wei Jia*

Main category: cs.LG

TL;DR: DPWMixer：一种基于无损Haar小波金字塔的双路径架构，用于长期时间序列预测，通过正交分解分离趋势和局部波动，结合全局线性映射和MLP-Mixer处理，实现高效且准确的预测。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个主要问题：1) Transformer模型具有二次复杂度且容易过拟合；2) 线性模型难以捕捉复杂的非线性局部动态；3) 多尺度框架通常使用平均池化，导致频谱混叠和高频信息丢失。需要一种既能高效处理长期依赖，又能准确捕捉局部动态且不损失信息的方法。

Method: 提出DPWMixer双路径架构：1) 使用无损Haar小波金字塔替代传统池化，通过正交分解显式分离趋势和局部波动而不损失信息；2) 设计双路径趋势混合器，结合全局线性映射用于宏观趋势锚定和基于patch的MLP-Mixer用于微观动态演化；3) 自适应多尺度融合模块根据通道平稳性加权整合不同尺度的预测。

Result: 在8个公开基准数据集上的广泛实验表明，该方法在长期时间序列预测任务中相比现有最先进方法取得了持续改进，证明了其有效性。

Conclusion: DPWMixer通过无损小波分解和双路径处理架构，有效解决了现有方法在复杂度、过拟合和信息损失方面的问题，为长期时间序列预测提供了一种高效且准确的解决方案。
...
### [135] [HTG-GCL: Leveraging Hierarchical Topological Granularity from Cellular Complexes for Graph Contrastive Learning](https://arxiv.org/abs/2512.02073)
*Qirui Ji,Bin Qin,Yifan Jin,Yunze Zhao,Chuxiong Sun,Changwen Zheng,Jianwen Cao,Jiangmeng Li*

Main category: cs.LG

TL;DR: HTG-GCL是一个新颖的图对比学习框架，通过生成多尺度环基胞腔复形来创建层次化拓扑视图，并使用多粒度解耦对比和不确定性加权机制来学习有意义的图表示。


<details>
  <summary>Details</summary>
Motivation: 现有图对比学习方法难以识别任务相关的拓扑结构，且无法适应不同下游任务所需的从粗到细的拓扑粒度变化。为了解决这个问题，作者提出了HTG-GCL框架。

Method: 1. 通过同一图的变换生成多尺度环基胞腔复形，体现拓扑粒度概念，创建多样化的拓扑视图；2. 提出多粒度解耦对比方法；3. 基于不确定性估计应用粒度特定加权机制。

Result: 在各种基准测试上的综合实验证明了HTG-GCL的有效性，突显了其通过层次化拓扑信息捕获有意义图表示的优越性能。

Conclusion: HTG-GCL通过层次化拓扑粒度和不确定性加权机制，能够更好地捕获图数据的语义不变性，在多种下游任务中表现出色。
...
### [136] [FDRMFL:Multi-modal Federated Feature Extraction Model Based on Information Maximization and Contrastive Learning](https://arxiv.org/abs/2512.02076)
*Haozhe Wu*
...
Method: 集成多模态信息提取和对比学习机制，采用任务驱动的监督式联邦特征提取方法。通过多约束学习框架（均方误差损失、互信息保持约束、对称KL散度约束、模型间对比约束）实现特征提取、融合和对齐，同时缓解表示漂移和灾难性遗忘。

Result: 实验结果表明，与经典特征提取技术相比，该方法在下游回归任务中取得了更显著的性能提升，验证了其在模拟和真实数据分析中的有效性。
...
### [137] [Cross-View Topology-Aware Graph Representation Learning](https://arxiv.org/abs/2512.02130)
*Ahmet Sami Korkmaz,Selim Coskunuzer,Md Joshem Uddin*

Main category: cs.LG

TL;DR: GraphTCL：结合GNN结构嵌入与持久同调拓扑嵌入的双视图对比学习框架，用于提升图分类性能


<details>
  <summary>Details</summary>
Motivation: 图神经网络(GNNs)能有效捕捉局部结构模式，但往往忽略对鲁棒表示学习至关重要的全局拓扑特征。需要一种能同时利用结构和拓扑信息的方法来提升图表示质量。

Method: 提出GraphTCL双视图对比学习框架：1) 从GNNs获取结构嵌入；2) 从持久同调(拓扑数据分析)获取拓扑嵌入；3) 通过跨视图对比损失对齐这两个互补视图。

Result: 在TU和OGB分子图基准数据集上的广泛实验表明，GraphTCL在分类性能上持续优于最先进的基线方法。
...
### [138] [How Market Volatility Shapes Algorithmic Collusion: A Comparative Analysis of Learning-Based Pricing Algorithms](https://arxiv.org/abs/2512.02134)
...
<details>
  <summary>Details</summary>
Motivation: 自主定价算法在数字市场竞争中影响日益增大，但其在现实需求条件下的行为尚未充分研究。本文旨在分析算法、市场结构和随机需求如何共同影响竞争结果，为相关政策讨论提供依据。

Method: 使用四种定价算法（Q-Learning、PSO、Double DQN、DDPG），在三种经典双寡头模型（Logit、Hotelling、Linear）下，通过自回归过程创建不同需求冲击机制，利用利润和价格合谋指数分析算法交互行为。

Result: 强化学习算法在稳定需求下常维持超竞争价格，DDPG表现出最明显的合谋倾向。需求冲击影响各异：Logit市场性能显著下降，Hotelling市场保持稳定，Linear市场出现冲击引发的利润膨胀。算法相对排名在不同环境中保持一致。

Conclusion: 市场结构和需求不确定性对算法竞争具有关键影响，研究结果为自主定价行为的政策讨论提供了重要参考，强调需要考虑算法在不同市场条件下的差异化表现。
...
### [139] [CLEF: Clinically-Guided Contrastive Learning for Electrocardiogram Foundation Models](https://arxiv.org/abs/2512.02180)
*Yuxuan Shu,Peter H. Charlton,Fahim Kawsar,Jussi Hernesniemi,Mohammad Malekzadeh*

Main category: cs.LG

TL;DR: 提出CLEF：一种利用临床风险评分进行自适应负对加权的对比学习方法，用于单导联心电图基础模型预训练，无需逐样本标注
...
Method: 临床引导对比学习：利用已建立的临床风险评分自适应加权负对，使心电图嵌入相似性与临床有意义的患者差异对齐，并处理缺失元数据

Result: 在7个测试数据集上的18个临床任务中，CLEF在分类任务上AUROC平均提升至少2.6%，回归任务上MAE平均降低至少3.2%，优于现有自监督方法

Conclusion: CLEF实现了更准确、可扩展的单导联心电图分析，推进远程健康监测，性能与监督学习方法相当
...
### [140] [Enforcing Orderedness to Improve Feature Consistency](https://arxiv.org/abs/2512.02194)
*Sophie L. Wang,Alex Quach,Nithin Parsan,John J. Yang*

Main category: cs.LG

TL;DR: 提出Ordered Sparse Autoencoders (OSAE)，通过建立严格的潜在特征排序和确定性使用每个特征维度，解决稀疏自编码器特征不一致问题，相比Matryoshka SAEs提高了特征一致性。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器（SAEs）在神经网络可解释性中广泛使用，但其学习到的特征在不同随机种子和超参数设置下存在差异，缺乏一致性。

Method: 扩展Matryoshka SAEs，建立严格的潜在特征排序，确定性使用每个特征维度，避免先前嵌套SAE方法中的采样近似，解决排列不可识别性问题。

Result: 理论上证明OSAE在稀疏字典学习设置中解决排列不可识别性问题（在自然对称性范围内唯一）。在Gemma2-2B和Pythia-70M上的实验表明，相比Matryoshka基线，OSAE提高了特征一致性。

Conclusion: OSAE通过建立特征排序和确定性维度使用，解决了SAE特征不一致问题，为神经网络可解释性提供了更可靠的特征表示方法。
...
### [141] [Modelling the Doughnut of social and planetary boundaries with frugal machine learning](https://arxiv.org/abs/2512.02200)
...
<details>
  <summary>Details</summary>
Motivation: "甜甜圈"（社会与地球边界）框架已成为评估环境和社会可持续性的流行工具，但如何找到实现该框架的政策参数是一个挑战

Method: 使用随机森林分类器和Q学习等轻量级机器学习方法，应用于简单的"甜甜圈"宏观经济模型，寻找符合可持续性要求的政策参数和最优轨迹

Result: 机器学习方法成功找到了能够同时实现环境和社会可持续性的政策参数组合，证明了该方法在简单模型中的可行性

Conclusion: 研究为机器学习在可持续发展政策分析中的应用提供了概念验证，下一步将应用于更复杂的生态宏观经济模型
...
### [142] [WhAM: Towards A Translative Model of Sperm Whale Vocalization](https://arxiv.org/abs/2512.02206)
...
TL;DR: WhAM是基于Transformer的模型，能够从任何音频提示生成合成抹香鲸咔嗒声序列（codas），通过微调VampNet在1万条咔嗒声录音上训练，生成高保真合成声音并保留关键声学特征。


<details>
  <summary>Details</summary>
Motivation: 抹香鲸通过咔嗒声序列进行交流，但现有技术难以生成高质量的合成鲸鱼声音。研究旨在开发首个能够生成逼真抹香鲸咔嗒声的Transformer模型，用于鲸鱼声学研究。

Method: 通过微调在音乐音频上预训练的VampNet模型，使用过去20年收集的1万条抹香鲸咔嗒声录音进行训练。采用迭代掩码标记预测方法生成合成咔嗒声。

Result: WhAM生成高保真合成咔嗒声，保留源录音的关键声学特征。在节奏、社会单元和元音分类等下游任务中，WhAM学习的表征表现出色，尽管模型是为生成而非分类任务训练的。

Conclusion: WhAM是首个能够从音频提示生成合成抹香鲸咔嗒声的Transformer模型，为鲸鱼声学研究和保护工作提供了新的工具，代码已开源。
...
### [143] [InstructLR: A Scalable Approach to Create Instruction Dataset for Under-Resourced Languages](https://arxiv.org/abs/2512.02213)
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在低资源语言（特别是非洲语言）的文本生成和对话接口方面面临挑战，主要原因是缺乏高质量的指令数据集。现有方法如自动翻译和合成数据生成通常产生不流畅或拼写不一致的输出。

Method: 提出InstructLR框架，结合LLM驱动的文本生成和双重质量过滤机制：1）基于检索增强生成（RAG）的n-shot提示自动过滤层；2）人工在环验证层。受MMLU等基准启发，构建多领域指令基准。

Result: 成功创建了三个多领域指令基准：ZarmaInstruct-50k（扎尔马语）、BambaraInstruct-50k（班巴拉语）和FulfuldeInstruct-50k（富尔富尔德语），每个包含5万条指令数据。

Conclusion: InstructLR框架能够有效生成低资源语言的高质量指令数据集，为改善这些语言的文本生成和对话接口提供了可行解决方案，有助于促进语言技术的包容性发展。
...
### [144] [Improved Training Mechanism for Reinforcement Learning via Online Model Selection](https://arxiv.org/abs/2512.02214)
*Aida Afshar,Aldo Pacchiano*

Main category: cs.LG

TL;DR: 该论文研究强化学习中的在线模型选择问题，通过集成在线模型选择方法提高训练效率和性能，从理论角度解决资源分配、非平稳动态适应和训练稳定性三个实际问题。


<details>
  <summary>Details</summary>
Motivation: 强化学习训练中需要选择合适的配置（如神经网络架构、步长等），传统方法通常需要大量试错。在线模型选择可以自适应地选择最佳配置，提高训练效率和性能。

Method: 研究在线模型选择在强化学习中的应用，从理论角度分析三个关键标准：1）高效资源分配，2）非平稳动态适应，3）不同随机种子下的训练稳定性。通过理论分析和实证验证相结合的方法。

Result: 理论分析表明在线模型选择方法能有效提高强化学习训练效率和性能。实证研究在神经网络架构选择、步长选择和自模型选择等任务中验证了理论结果的有效性。

Conclusion: 集成在线模型选择方法到强化学习训练过程中可以显著提高效率和性能，为解决资源分配、动态适应和稳定性等实际问题提供了理论指导和实证支持。
...
### [145] [Uncertainty Reasoning with Photonic Bayesian Machines](https://arxiv.org/abs/2512.02217)
...
TL;DR: 基于混沌光源的光子贝叶斯机器学习系统，实现高速不确定性推理，用于医疗图像分类和域外检测


<details>
  <summary>Details</summary>
Motivation: AI系统在医疗诊断、自动驾驶等安全关键领域应用日益广泛，需要具备不确定性感知能力以实现可信AI。传统数字系统存在伪随机数生成瓶颈和采样成本高的问题。

Method: 利用混沌光源的固有随机性构建光子贝叶斯机器，在贝叶斯神经网络框架内实现不确定性推理。系统具有1.28 Tbit/s数字接口，与PyTorch兼容，每卷积处理仅需37.5 ps。

Result: 成功应用于血细胞显微镜图像的同时分类和域外检测，能够区分偶然不确定性和认知不确定性。系统消除了伪随机数生成瓶颈，显著降低了概率模型的采样成本。
...
### [146] [On the Approximation of Phylogenetic Distance Functions by Artificial Neural Networks](https://arxiv.org/abs/2512.02223)
*Benjamin K. Rosenzweig,Matthew W. Hahn*

Main category: cs.LG

TL;DR: 提出最小化神经网络架构来近似经典系统发育距离函数，能够学习多种分子进化模型下的距离，相比传统方法计算开销小且可扩展性强


<details>
  <summary>Details</summary>
Motivation: 传统基于距离的系统发育分析方法已被复杂的贝叶斯和最大似然方法取代，但这些方法计算复杂。需要开发计算效率高、可扩展性强且能适应多种分子进化模型的系统发育距离学习方法

Method: 设计最小化神经网络架构来近似经典系统发育距离函数，能够学习多种分子进化模型下的距离特性。这些架构相比基于模型的推理方法和最近提出的卷积/transformer网络具有更小的计算开销

Result: 学习的距离函数泛化能力强，在适当的训练数据集下，能够达到与最先进推理方法相当的结果。架构具有小计算足迹，可扩展到大量分类群和分子特征

Conclusion: 最小化神经网络架构为系统发育分析提供了高效、可扩展的替代方案，能够学习多种分子进化模型下的距离函数，在保持性能的同时显著降低计算复杂度
...
TL;DR: 研究探讨公平性约束如何影响机器学习模型的可解释性，特别是Shapley特征重要性排名的稳定性，发现在医疗和司法领域提高公平性会显著改变特征重要性排名。
...
Method: 通过偏置缓解技术增强公平性，研究其如何重塑基于Shapley的特征排名。在三个数据集（儿科尿路感染风险、直接抗凝剂出血风险、再犯风险）上量化应用公平性约束后特征重要性排名的变化，并评估多个模型类别在Shapley排名稳定性方面的表现。

Result: 提高跨种族亚组的模型公平性会显著改变特征重要性排名，有时在不同群体间以不同方式变化。公平性约束的应用导致特征解释发生实质性变化。

Conclusion: 需要在模型评估中联合考虑准确性、公平性和可解释性，而不是孤立地评估这些维度。公平性改进可能以可解释性为代价，临床部署前应仔细评估这种权衡。
...
TL;DR: 该论文证明在可测试学习模型中，成员查询不能显著降低时间复杂性，且许多高效的成员查询学习算法无法被转化为可测试学习算法。


<details>
  <summary>Details</summary>
Motivation: 研究成员查询在可测试学习模型中的效率限制，探索成员查询是否能比仅使用样本的学习算法提供显著的时间优势。

Method: 通过将基于样本的布尔概念类反驳归约到带查询的可测试学习，结合已知的从学习到反驳的归约，建立下界。定义"统计"成员查询算法类，并证明这类可测试学习算法意味着高效的统计查询反驳和学习算法。

Result: 证明在可测试学习模型中，成员查询不能使算法比最佳仅样本PAC学习算法更高效（超过多项式级别）。许多已知的高效成员查询学习算法（如基于影响估计或子立方体条件统计查询的算法）无法被转化为可测试学习算法。

Conclusion: 在可测试学习框架下，成员查询无法提供超越仅样本学习的显著时间优势，且许多高效的分布特定成员查询学习器无法被设计为可测试的。
...
TL;DR: 论文研究了对称性无关模型学习对称性的速度与效果，提出了衡量等变误差的原则性方法，发现在3D旋转等变任务中，模型能快速学习到高精度对称性，且学习对称性比主预测任务更容易。


<details>
  <summary>Details</summary>
Motivation: 虽然数据增强被广泛用于训练对称性无关模型，但模型学习尊重对称性的速度和效果仍不清楚。需要量化模型学习对称性的过程，理解等变误差与训练动态的关系。

Method: 提出了衡量等变误差的原则性方法，对于凸损失函数，计算总损失中由对称性不完美引起的比例。聚焦于3D旋转等变性在高维分子任务（流匹配、力场预测、体素去噪）上的实证研究。

Result: 模型在1k-10k训练步内快速将等变误差降低到≤2%的保留损失，这一结果对模型和数据集大小具有鲁棒性。学习3D旋转等变性比主预测任务更容易，损失景观更平滑、条件更好。非等变模型的损失惩罚较小，可能在GPU小时效率上优于等变模型。

Conclusion: 对称性无关模型能快速学习3D旋转等变性，但等变模型的效率优势需要缩小"效率差距"才能体现。研究揭示了等变误差、学习梯度和模型参数之间的关系。
...
<details>
  <summary>Details</summary>
Motivation: 玻尔兹曼机是强大的生成模型，但传统MCMC采样训练成本高且难以并行化，限制了其实际应用（主要局限于RBM）。需要开发能并行采样且适用于更复杂模型的高效学习方法。

Method: 1. 提出LSB采样器：受量子启发组合优化方法模拟分岔启发，实现并行采样，适用于一般耦合的玻尔兹曼机。2. 提出CEM方法：条件期望匹配，用于估计学习过程中的逆温度参数。3. 将LSB和CEM结合形成SAL框架。

Result: LSB采样器能实现并行采样且精度与MCMC相当；CEM方法能有效估计逆温度；SAL框架为超越RBM的能量基生成模型开辟了新途径。

Conclusion: SAL框架通过结合LSB并行采样器和CEM温度估计方法，实现了比RBM更强大的玻尔兹曼机高效学习，为能量基生成建模开辟了新方向。
...
TL;DR: RAM-OL是一种用于在线学习的概念漂移处理方法，通过检索历史样本的最近邻来增强模型，在周期性漂移数据流上显著提升准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的检索增强模型在流式监督学习和概念漂移场景下的应用尚未得到充分研究，需要开发能够在非平稳环境中有效运作的在线学习方法。

Method: RAM-OL是随机梯度下降的简单扩展，维护一个小型历史样本缓冲区，在隐藏表示空间中检索当前输入的最近邻，并联合更新当前样本和检索到的邻居。包含朴素回放和门控回放两种变体。

Result: 在强周期漂移的数据流上，RAM-OL将预序准确率提高了约7个百分点，显著减少了随机种子间的方差；在噪声较大的航空延误数据流上，门控变体与纯在线基线表现相当。

Conclusion: 检索增强记忆是处理概念漂移的实用且鲁棒的在线学习工具，能够有效利用历史模式的重现来提升性能。
...
TL;DR: 该研究比较了多种统计和机器学习模型在预测波士顿MBTA地铁站客流和系统延误方面的性能，发现日期特征比天气数据对预测准确性更有帮助，天气数据反而会导致模型过拟合。


<details>
  <summary>Details</summary>
Motivation: 波士顿MBTA公共交通系统经常面临延误和客流波动问题，影响效率和乘客满意度。为了更好理解这一现象，研究需要找到最佳方法来预测地铁使用量（通过地铁站入口数据）和系统延误数量。

Method: 研究评估了10种统计和机器学习模型预测次日地铁客流，以及11种模型（包括自激励点过程模型）预测延误数量。考虑了日期、季节、天气等多种影响因素，通过选择性特征包含来确定特征重要性，使用均方根误差(RMSE)测试模型准确性。

Result: 研究发现，提供日期或季节数据比天气数据对预测准确性更有实质性帮助；实际上，提供天气数据通常会降低性能，表明模型容易过拟合。自激励点过程模型被独特地应用于MBTA延误建模。

Conclusion: 对于公共交通预测，日期和季节特征比天气数据更重要，天气数据可能导致模型过拟合。研究为MBTA系统提供了有效的预测方法，并展示了点过程模型在延误建模中的独特应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 随着代码生成、深度推理和长文档理解等任务的需求增长，长上下文生成成为LLMs的关键能力。推测解码是加速生成最直接有效的方法之一，但随着上下文长度增加，验证成为主要瓶颈。

Method: 提出SpecPV自推测解码方法：1）使用部分键值状态（KV）进行快速验证；2）定期应用完全验证以消除累积错误。该方法遵循草稿-验证范式，但优化了验证过程。

Result: 在多个长上下文基准测试和模型（包括LLaMA-3.1-8B-Instruct和Qwen3系列）上进行验证，SpecPV相比标准自回归解码实现了高达6倍的解码加速，仅有轻微性能下降。

Conclusion: SpecPV通过优化验证过程有效解决了长上下文生成中推测解码的瓶颈问题，显著提升了生成速度，为长上下文任务提供了高效的解码解决方案。
...
TL;DR: 提出FOVA框架，通过投票机制解决离线联邦强化学习中混合质量数据导致的性能下降问题
...
Result: 理论分析证明FOVA学习的策略严格优于行为策略；实验验证在广泛使用的基准测试中显著优于现有基线方法

Conclusion: FOVA通过投票机制有效解决了离线联邦强化学习中混合质量数据的问题，实现了稳定高效的学习性能
...
<details>
  <summary>Details</summary>
Motivation: 在部分可观测马尔可夫决策过程（POMDPs）中，直接优化策略性能是一个重要但具有挑战性的问题。传统方法通常需要了解系统状态信息或面临计算复杂度问题。本文旨在开发一种更实用、更高效的梯度估计方法，能够在仅观测到输出序列的情况下优化策略参数。

Method: 提出了GPOMDP算法，这是一种类似REINFORCE的算法，通过单一样本路径估计平均奖励相对于策略参数的梯度近似值。算法仅需一个自由参数β∈[0,1)，用于控制偏差-方差权衡。该算法不需要了解底层状态信息，仅基于观测到的输出序列进行估计。

Result: 证明了GPOMDP算法的收敛性，并展示了如何将GPOMDP产生的梯度估计用于共轭梯度过程，以找到平均奖励的局部最优值。算法的主要优势包括：仅需单一样本路径、仅一个具有自然解释的自由参数、无需了解底层状态信息。

Conclusion: GPOMDP算法为POMDPs中的策略优化提供了一种实用且高效的梯度估计方法。该算法通过简单的参数设置和最小信息要求，实现了对策略性能梯度的有效估计，为实际应用中的策略优化问题提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究连续时间风险敏感强化学习问题，环境由可控随机微分方程描述，目标函数是累积奖励的潜在非线性泛函，旨在解决传统风险中性RL无法处理风险偏好的局限性。

Method: 当目标泛函为优化确定性等价时，证明最优策略相对于增强环境是马尔可夫的；提出基于新颖鞅表征方法的CT-RS-q风险敏感q学习算法。

Result: 理论证明最优策略的马尔可夫性质；提出CT-RS-q算法；在动态投资组合选择问题的仿真研究中验证了算法的有效性。

Conclusion: 本文为连续时间风险敏感强化学习提供了理论框架和实用算法，通过鞅表征方法解决了非线性风险泛函下的最优控制问题，在金融应用场景中展示了实际价值。
...
TL;DR: ESACT：一种基于局部相似性稀疏预测的端到端Transformer稀疏加速器，通过HLog量化预测注意力稀疏性，减少52.03%计算量，能耗效率达3.29 TOPS/W


<details>
  <summary>Details</summary>
Motivation: Transformer模型计算成本高阻碍硬件部署，现有稀疏加速器主要利用注意力中的行内稀疏性，很少考虑行间稀疏性。基于全局相似性的方法计算开销大，且通常只应用于1-2个组件。研究发现局部相似性可实现更低开销的端到端稀疏加速。

Method: 提出ESACT端到端稀疏加速器，核心是SPLS（基于局部相似性的稀疏预测）机制，使用HLog量化在QK生成前准确预测局部注意力稀疏性，实现所有Transformer组件的有效稀疏化。包含三项架构创新支持高效硬件实现。

Result: 在26个基准测试中，SPLS减少总计算量52.03%，精度损失小于1%。ESACT实现端到端能耗效率3.29 TOPS/W，注意力级能耗效率比SOTA加速器SpAtten和Sanger分别提高2.95倍和2.26倍。

Conclusion: 局部相似性为Transformer稀疏加速提供了高效途径，ESACT通过SPLS机制和硬件架构创新，实现了端到端的高效稀疏加速，显著提升能耗效率，为Transformer硬件部署提供了有前景的解决方案。
...
Method: 采用双层多智能体强化学习框架：车道级智能体负责决定每条车道的最优停车空间配置，使用结合LSTM和图注意力网络的新型深度Q学习架构捕捉时空相关性；区块级智能体控制车道级智能体行为，维持区块周围足够的停车位水平。

Result: 使用SUMO在合成数据和墨尔本真实数据上进行实验，结果表明所提框架能显著减少车辆平均旅行时间损失，最高可达47%，而停车步行距离仅有轻微增加。

Conclusion: 动态配置路边停车空间能有效缓解交通拥堵，提出的多智能体强化学习框架具有可扩展性，能适应大规模路网，在减少旅行时间损失的同时保持停车便利性。
...
TL;DR: 论文将数据管理形式化为采样分布重加权，分析其对数据诱导算子特征结构的影响。主要发现：静态剪枝无法改变谱尾指数，只能提供有限区域改进；而理想的时间相关数据管理可以加速学习。


<details>
  <summary>Details</summary>
Motivation: 研究各种数据中心策略（数据剪枝、合成数据生成、跨模型蒸馏、RLHF、基于难度的采样）对大规模神经模型训练的影响，理解为什么某些策略有效而其他无效，特别是为什么自生成合成数据通常无法提升模型能力。

Method: 将数据管理形式化为采样分布重加权，并映射其对数据诱导算子特征结构的影响。通过算子理论分析静态剪枝和时间相关数据管理对谱尾指数和学习动态的影响。

Result: 1. 静态剪枝诱导有界算子，无法改变谱尾指数，只能提供有限区域改进，不能改变渐近神经缩放。2. 理想的时间相关数据管理（能跟踪谱残差并连续重归一化尾部）可以证明加速学习，但实际系统只能近似这种行为。

Conclusion: 数据管理的有效性取决于其如何影响数据诱导算子的谱结构。静态方法有根本限制，而动态的、时间相关的数据管理策略有潜力加速学习，但需要跟踪谱残差和连续重归一化的能力。
...
TL;DR: 提出DVDF方法，通过同时考虑动力学对齐和价值对齐，从源域选择高质量样本用于跨域离线强化学习


<details>
  <summary>Details</summary>
Motivation: 现有跨域离线强化学习方法只关注动力学对齐，忽略了价值对齐的重要性。简单合并源域和目标域数据可能导致性能下降，需要同时考虑两个维度的对齐来提升策略学习效果。

Method: 提出DVDF（动力学和价值对齐数据过滤）方法，从源域选择既具有高动力学对齐又具有高价值对齐的样本。通过理论分析证明两个对齐维度的重要性，并在多种动力学偏移设置下进行验证。

Result: DVDF在多种任务和数据集上持续优于现有基线方法，在目标域仅含5000个转换的极低数据设置下仍能提供优异性能。

Conclusion: 同时考虑动力学对齐和价值对齐对于跨域离线强化学习至关重要，DVDF方法通过双重对齐的数据过滤机制有效提升了跨域策略学习性能。
...
TL;DR: 研究发现LLM智能体在处理长上下文时存在性能和安全性问题：模型在100K token时性能下降超50%，拒绝率变化不可预测，长上下文可能带来新的安全风险。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LLM在长上下文提示上的评估，而智能体设置（从能力和安全角度）相对未被探索。新LLM支持更长上下文窗口和工具调用能力，但智能体在长上下文环境下的表现尚不清楚。

Method: 评估LLM智能体在不同长度、类型和位置上下文下的表现，包括任务性能和拒绝有害请求的能力。测试了支持1M-2M token上下文窗口的模型在长达200K token上下文中的表现。

Result: LLM智能体对上下文长度、类型和位置敏感，表现出意外和不一致的变化。1M-2M token上下文窗口的模型在100K token时性能下降超过50%。拒绝率变化不可预测：GPT-4.1-nano从~5%增加到~40%，而Grok 4 Fast从~80%下降到~10%（200K token时）。

Conclusion: 长上下文操作的智能体存在潜在安全问题，当前评估LLM智能体在长多步任务中安全性的指标和范式需要重新审视。智能体在能力和安全性能上与先前LLM评估存在显著差异。
...
### [162] [TabGRU: An Enhanced Design for Urban Rainfall Intensity Estimation Using Commercial Microwave Links](https://arxiv.org/abs/2512.02465)
...
TL;DR: 提出名为TabGRU的混合深度学习架构，结合Transformer和双向门控循环单元，用于商业微波链路降雨监测，相比传统物理模型和深度学习基线表现更优。


<details>
  <summary>Details</summary>
Motivation: 全球城市化加速和极端天气事件频发，高分辨率城市降雨监测对建设韧性智慧城市至关重要。传统基于物理模型的商业微波链路降雨反演方法难以处理信号噪声和非线性衰减等现实复杂性。

Method: 提出TabGRU混合深度学习架构，结合Transformer捕捉长期依赖关系和双向门控循环单元提取局部序列特征，采用可学习位置嵌入和注意力池化机制增强动态特征提取和泛化能力。

Result: 在瑞典哥德堡公开数据集上验证，测试期间覆盖约10个降雨事件。TabGRU在Torp站点R²达0.91，Barl站点达0.96，优于深度学习基线，相比物理模型能更好缓解峰值降雨事件的显著高估问题。

Conclusion: TabGRU模型能有效克服传统方法局限，在测试条件下为基于商业微波链路的城市降雨监测提供稳健准确的解决方案。
...
### [163] [Dual-Robust Cross-Domain Offline Reinforcement Learning Against Dynamics Shifts](https://arxiv.org/abs/2512.02486)
*Zhongjian Qiao,Rui Yang,Jiafei Lyu,Xiu Li,Zhongxiang Dai,Zhuoran Yang,Siyang Gao,Shuang Qiu*

Main category: cs.LG

TL;DR: DROCO算法针对跨域离线强化学习中的动态偏移问题，提出同时保证训练时和测试时鲁棒性的双鲁棒框架，通过RCB算子和动态值惩罚等技术提升策略在动态扰动下的稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有跨域离线强化学习方法主要关注训练时鲁棒性（处理训练数据中的动态偏移），但忽视了实际部署时测试时对动态扰动的鲁棒性。当目标域数据有限时，现有方法训练的策略在动态扰动下表现脆弱。

Method: 提出鲁棒跨域贝尔曼算子（RCB），在保证训练时鲁棒性的同时增强测试时对动态扰动的鲁棒性。引入动态值惩罚和Huber损失来应对RCB算子可能引起的值估计偏差，最终形成DROCO算法。

Result: 在各种动态偏移场景下的实验结果表明，DROCO优于现有基线方法，在动态扰动下表现出更强的鲁棒性，特别是在目标域数据有限的情况下。

Conclusion: 跨域离线强化学习需要同时考虑训练时和测试时鲁棒性，DROCO通过RCB算子和相关技术有效解决了这一双重鲁棒性问题，为实际部署提供了更可靠的解决方案。
...
### [164] [Hybrid(Penalized Regression and MLP) Models for Outcome Prediction in HDLSS Health Data](https://arxiv.org/abs/2512.02489)
*Mithra D K*
...
Method: 比较基线模型（逻辑回归、随机森林、XGBoost）与混合方法（XGBoost特征编码器+轻量级多层感知机头）

Result: 混合模型在处理后的NHANES子集上获得了比基线模型更好的AUC和平衡准确率

Conclusion: 混合方法在糖尿病预测任务上表现优于传统基线模型，作者开源代码以促进复现
...
### [165] [A Fully First-Order Layer for Differentiable Optimization](https://arxiv.org/abs/2512.02494)
...
Method: 将可微优化重写为双层优化问题，引入active-set Lagrangian超梯度oracle，仅使用一阶信息计算近似梯度，避免Hessian评估

Result: 近似超梯度可在$\tilde{\oo}(1)$时间内计算，约束双层优化的整体复杂度为$\tilde{\oo}(δ^{-1}ε^{-3})$，匹配非光滑非凸优化的最佳已知速率

Conclusion: 提出的算法显著降低了可微优化层的计算和内存需求，并发布了开源Python库FFOLayer，便于从现有求解器适配使用
...
### [166] [Water Quality Estimation Through Machine Learning Multivariate Analysis](https://arxiv.org/abs/2512.02508)
*Marco Cardia,Stefano Chessa,Alessio Micheli,Antonella Giuliana Luminare,Francesca Gambineri*

Main category: cs.LG

TL;DR: 将紫外-可见光谱与机器学习结合，用于农业用水质量评估，并通过SHAP提高模型可解释性


<details>
  <summary>Details</summary>
Motivation: 农业用水质量对农产品安全至关重要，随着农业数字化进程，自动评估水质成为重要需求

Method: 整合紫外-可见光谱技术与机器学习，采用SHAP方法解释不同波长吸光度对预测的贡献

Result: 该方法能够快速、准确、可解释地评估关键水质参数
...
### [167] [Decentralized Fairness Aware Multi Task Federated Learning for VR Network](https://arxiv.org/abs/2512.02513)
*Krishnendu S. Tharakan,Carlo Fischione*

Main category: cs.LG

TL;DR: 提出基于去中心化多任务公平联邦学习的VR视频缓存方案，通过在基站个性化缓存用户视场内容，解决无线VR传输中的延迟和异构性问题。


<details>
  <summary>Details</summary>
Motivation: 无线VR体验需要高质量实时视频传输，但面临严格的体验质量要求、低延迟约束和VR设备能力有限等挑战。传统联邦学习存在用户偏置问题，单一全局模型无法捕捉用户和基站间的统计异构性。

Method: 提出DMTFL算法，在每个基站学习个性化缓存模型，通过Rademacher复杂度和PAC边界提供理论保证，基于VR头部追踪数据集进行模拟验证。

Result: 模拟实验表明，提出的DMTFL算法在性能上优于基线算法，能够有效提升VR视频传输质量。

Conclusion: DMTFL通过去中心化多任务学习和个性化缓存策略，解决了无线VR传输中的关键挑战，为高质量无线VR体验提供了有效解决方案。
...
TL;DR: 提出上下文蒸馏方法，在无需微调或复杂提示工程的情况下，通过检索教师演示作为上下文示例，让学生模型模仿教师行为，结合自洽级联实现低成本LLM智能体推理。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体开发面临高推理成本问题，而微调方法开发周期长、提示工程试错成本高，需要一种既能降低成本又能保持快速实验迭代的方法。

Method: 提出上下文蒸馏：在智能体每个步骤中检索相关教师演示作为上下文示例，让学生模型模仿教师行为；结合自洽级联策略判断何时信任学生模型，实现自适应成本优化。

Result: 在ALFWorld基准上以2.5倍低成本达到教师级准确率（成本从$0.059降至$0.024）；在AppWorld上实现2倍成本降低；演示成本在843个episode后摊销，大规模部署可节省超过$34,900。

Conclusion: 上下文蒸馏方法通过结合上下文学习和知识蒸馏，在保持冻结模型快速实验能力的同时显著降低LLM智能体推理成本，使先进智能体系统在经济上对更广泛应用可行。
...
<details>
  <summary>Details</summary>
Motivation: 现有张量积特征方法解决了维度灾难问题，但特征超参数优化仍依赖耗时的交叉验证，需要更高效的特征学习方案。

Method: 将张量积特征表示为可学习的CPD分解，利用ALS交替优化算法同时学习特征超参数和模型参数。

Result: 在不同维度和规模的真实数据上，FL模型训练速度比标准交叉验证快3-5倍，预测质量相当。

Conclusion: FL模型通过可学习的CPD表示和ALS优化，实现了高效的特征超参数学习，显著提升了训练效率。
...
TL;DR: CUDA-L2使用LLM和强化学习自动优化半精度矩阵乘法CUDA内核，在1000种配置上显著超越主流基准库


<details>
  <summary>Details</summary>
Motivation: 即使是像HGEMM这样性能关键、高度优化的CUDA内核，仍然有通过自动化方法进一步优化的空间。传统手动优化难以系统探索大规模配置空间，而LLM引导的强化学习可以自动化这一过程。

Method: 结合大型语言模型和强化学习，以CUDA执行速度作为RL奖励，自动优化半精度矩阵乘法内核。系统在1000种配置上进行优化，通过LLM引导RL探索人类难以处理的配置空间。

Result: 在离线模式下，CUDA-L2相比torch.matmul平均提升22.0%，相比cuBLAS提升19.2%，相比cuBLASLt-heuristic提升16.8%，相比cuBLASLt-AutoTuning提升11.4%。在服务器模式下，提升进一步增加到28.7%、26.0%、22.4%和15.9%。

Conclusion: CUDA-L2证明了即使是性能关键、高度优化的内核也能通过LLM引导的强化学习自动化得到显著改进，系统探索了人类难以处理的大规模配置空间，为高性能计算优化提供了新方法。
...
TL;DR: GoRL框架通过解耦优化与生成，使用可优化的潜变量策略和条件生成解码器，在保持训练稳定性的同时实现高度表达性的多模态策略。


<details>
  <summary>Details</summary>
Motivation: 强化学习中存在稳定性与表达性的矛盾：高斯策略训练稳定但表达能力有限（单模态），而生成式策略（扩散/流匹配）能建模多模态行为但在线训练不稳定。需要一种既能稳定优化又能表达复杂动作分布的方法。

Method: 提出GoRL框架，核心是解耦优化与生成：1）优化一个可处理的潜变量策略（提供稳定梯度）；2）使用条件生成解码器合成动作（提供表达能力）。采用双时间尺度更新计划，潜变量策略稳定学习，解码器逐步提升表达能力，无需可处理的动作似然。

Result: 在多个连续控制任务中，GoRL一致优于高斯策略和最近的生成式策略基线。在HopperStand任务上达到归一化回报870+，是最好基线的3倍以上。展示了分离优化与生成能实现既稳定又高度表达的策略。

Conclusion: 解耦优化与生成是解决强化学习中稳定性与表达性矛盾的有效途径。GoRL框架通过潜变量策略优化和条件生成解码器的结合，为构建既稳定训练又能表达复杂多模态行为的策略提供了实用方案。
...
TL;DR: HeatTransFormer：基于物理引导的Transformer架构，用于解决界面主导的扩散问题，能够处理芯片-基板组件中的陡峭温度梯度，同时支持正向和逆向热建模。


<details>
  <summary>Details</summary>
Motivation: 半导体器件中的热传递主要由芯片和基板组件主导，其中有限芯片层产生的热量会耗散到具有更高热物理特性的半无限基板中。这种不匹配会产生陡峭的界面温度梯度，使得瞬态热响应对界面高度敏感。传统的数值求解器需要过度离散化来解决这些动态问题，而物理信息神经网络（PINNs）通常在材料界面附近表现出不稳定的收敛性和物理一致性损失。

Method: 提出了HeatTransFormer，一种用于界面主导扩散问题的物理引导Transformer架构。该框架集成了：1）物理信息时空采样；2）模拟解析扩散解的基于拉普拉斯的激活函数；3）支持双向时空耦合的无掩码注意力机制。这些组件使模型能够解析陡峭梯度、保持物理一致性，并在PINNs通常失败的地方保持稳定。

Result: HeatTransFormer在应用于有限层和半无限基板配置时，能够在界面两侧产生连贯的温度场。结合物理约束的逆向策略，仅使用外部测量就能可靠地同时识别三个未知热特性参数。

Conclusion: 这项工作表明，物理引导的Transformer架构为界面主导热系统中的正向和逆向建模提供了一个统一框架，克服了传统数值方法和PINNs在处理陡峭界面梯度时的局限性。
...
### [173] [Adapting Tensor Kernel Machines to Enable Efficient Transfer Learning for Seizure Detection](https://arxiv.org/abs/2512.02626)
*Seline J. S. de Rooij,Borbála Hunyadi*

Main category: cs.LG

TL;DR: 提出自适应张量核机(Adapt-TKM)用于迁移学习，通过张量网络学习紧凑非线性模型，在癫痫检测任务中仅需少量患者特定数据即可实现个性化，参数比自适应SVM少100倍，推理速度更快。


<details>
  <summary>Details</summary>
Motivation: 迁移学习旨在通过从相关源问题学习来优化目标任务性能。当前方法在资源受限设备上效率不足，需要更紧凑、高效的迁移学习模型。

Method: 提出自适应张量核机(Adapt-TKM)，受自适应SVM启发，通过正则化从源任务向"适应"模型传递知识。利用低秩张量网络在原始域学习紧凑非线性模型，实现高效适应而不增加额外参数。

Result: 在耳后EEG癫痫检测任务中，使用少量患者特定数据个性化患者独立模型，患者适应模型性能优于患者独立模型和完全患者特定模型。参数比自适应SVM少约100倍，推理速度相应更快。

Conclusion: Adapt-TKM是一种高效的迁移学习方法，特别适用于资源受限的可穿戴设备，能够在保持高性能的同时显著减少参数数量和提升推理速度。
...
### [174] [SeeNav-Agent: Enhancing Vision-Language Navigation with Visual Prompt and Step-Level Policy Optimization](https://arxiv.org/abs/2512.02631)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大视觉语言模型的视觉语言导航代理存在感知错误、推理错误和规划错误，严重阻碍了导航性能，需要新的解决方案。

Method: 1. 引入双视角视觉提示技术减少视觉模块的感知幻觉；2. 设计步级强化微调方法SRGPO，通过定义可验证的过程奖励和随机分组步级优势估计来增强规划能力。

Result: 在EmbodiedBench导航基准上，零样本VP模块使GPT-4.1达到86.7%导航成功率，超越当前最佳LVLM约20个百分点；SRGPO微调后Qwen2.5-VL-3B达到72.3%成功率，超越最佳现有LVLM模型5.6个百分点。

Conclusion: SeeNav-Agent框架通过双视角视觉提示和SRGPO强化微调方法，有效解决了VLN代理的感知和规划问题，显著提升了导航性能，在训练稳定性、收敛效率和泛化能力方面优于现有方法。
...
### [175] [Joint Distillation for Fast Likelihood Evaluation and Sampling in Flow-based Models](https://arxiv.org/abs/2512.02636)
...
<details>
  <summary>Details</summary>
Motivation: 当前最好的生成模型（如扩散和流模型）需要数百到数千次神经函数评估来计算单个似然值，这构成了计算瓶颈。虽然现有蒸馏方法可以加速采样，但要么完全放弃似然计算，要么仍需要昂贵的全轨迹积分。

Method: 提出快速流联合蒸馏（F2D2）框架，利用连续归一化流中采样和似然的耦合ODE来自共享速度场这一特性，使用单个模型联合蒸馏采样轨迹和累积散度，仅需添加散度预测头。

Result: F2D2能够在少量步数评估下实现准确的对数似然，同时保持高样本质量。应用该方法，仅需额外一次反向NFE的2步MeanFlow模型就能超越1024步教师模型。

Conclusion: F2D2解决了流基生成模型中长期存在的计算瓶颈，实现了采样和似然评估的高效联合加速，为生成模型的实际应用提供了重要工具。
...
### [176] [Adaptive Weighted LSSVM for Multi-View Classification](https://arxiv.org/abs/2512.02653)
*Farnaz Faramarzi Lighvan,Mehrdad Asadi,Lynn Houthuys*

Main category: cs.LG

TL;DR: AW-LSSVM是一种自适应加权LS-SVM多视图学习方法，通过迭代全局耦合促进互补学习，让每个视图专注于其他视图在前一轮迭代中的困难样本，在保持原始特征隔离的同时提升性能。
...
Method: 提出自适应加权LS-SVM（AW-LSSVM），通过迭代全局耦合机制，让每个视图专注于其他视图在前一轮迭代中分类困难的样本，实现视图间的互补学习。该方法保持原始特征隔离，不直接融合特征。

Result: 实验表明AW-LSSVM在大多数数据集上优于现有的基于核的多视图学习方法，同时保持了原始特征的隔离性，使其也适用于隐私保护场景。

Conclusion: AW-LSSVM通过迭代全局耦合促进视图间的互补学习，在提升多视图学习性能的同时保持特征隔离，为隐私保护场景提供了有效的解决方案。
...
### [177] [Distill, Forget, Repeat: A Framework for Continual Unlearning in Text-to-Image Diffusion Models](https://arxiv.org/abs/2512.02657)
*Naveen George,Naoki Murata,Yuhta Takida,Konda Reddy Mopuri,Yuki Mitsufuji*

Main category: cs.LG

TL;DR: 提出基于生成蒸馏的持续遗忘框架，解决视觉生成模型在序列删除请求下的稳定性危机，实现精准遗忘同时保持模型完整性。


<details>
  <summary>Details</summary>
Motivation: 视觉生成模型训练数据与隐私法规（如GDPR被遗忘权）存在冲突，需要机器遗忘技术。现有方法无法处理现实世界中序列到达的删除请求（持续遗忘），简单应用一次性方法会导致稳定性危机，引发保留崩溃、相关概念连带损害和生成质量下降。

Method: 提出基于生成蒸馏的持续遗忘框架，将每个遗忘步骤重构为多目标师生蒸馏过程，利用持续学习原理保持模型完整性。通过教师-学生蒸馏实现精准遗忘。

Result: 在10步序列基准测试中，该方法以更高保真度遗忘目标概念，同时对保留概念性能和整体图像质量无显著干扰，大幅优于基线方法。
...
### [178] [Graph VQ-Transformer (GVT): Fast and Accurate Molecular Generation via High-Fidelity Discrete Latents](https://arxiv.org/abs/2512.02667)
*Haozhuo Zheng,Cheng Wang,Yang Liu*

Main category: cs.LG

TL;DR: GVT是一种两阶段分子生成框架，通过图向量量化VAE将分子图压缩为离散潜序列，再用自回归Transformer生成，在多个基准测试中达到SOTA性能。
...
Method: 提出Graph VQ-Transformer两阶段框架：1) Graph VQ-VAE将分子图压缩为高保真离散潜序列；2) 自回归Transformer在潜序列上进行序列建模生成分子。结合图Transformer、RCM节点排序和RoPE位置编码。

Result: 在ZINC250k、MOSES、GuacaMol等基准测试中达到SOTA或极具竞争力的性能，在FCD和KL散度等分布相似性指标上优于领先的扩散模型。

Conclusion: GVT为分子生成提供了高效准确的替代方案，建立了离散潜空间分子生成的新基准，并为与大型语言模型的潜在协同作用铺平了道路。
...
### [179] [Conformal Correction for Efficiency May be at Odds with Entropy](https://arxiv.org/abs/2512.02704)
*Senrong Xu,Tianyu Wang,Zenan Li,Yuan Yao,Taolue Chen,Feng Xu,Xiaoxing Ma*

Main category: cs.LG

TL;DR: 提出熵约束的共形校正方法，在共形预测效率与模型预测熵之间寻求更好的帕累托最优


<details>
  <summary>Details</summary>
Motivation: 共形预测为黑盒机器学习模型提供统计严谨的不确定性集合，但需要进一步提高其效率。共形校正通过共形感知的低效损失微调或包装基础模型，但存在效率与预测熵之间的权衡问题

Method: 提出熵约束的共形校正方法，在给定熵阈值下探索效率与熵之间更好的帕累托最优

Result: 在计算机视觉和图数据集上的大量实验结果表明，该方法能显著提高最先进共形预测方法的效率，在给定熵阈值下最高可提升34.4%

Conclusion: 通过熵约束的共形校正方法，成功在共形预测效率与模型预测熵之间实现了更好的平衡，为不确定性量化提供了更优的解决方案
...
### [180] [FGC-Comp: Adaptive Neighbor-Grouped Attribute Completion for Graph-based Anomaly Detection](https://arxiv.org/abs/2512.02705)
*Junpeng Wu,Pinheng Zong*

Main category: cs.LG

TL;DR: FGC-Comp是一个轻量级、分类器无关的图异常检测属性补全模块，用于处理缺失和对抗性模糊节点属性问题，通过标签分组和门控机制增强邻居聚合稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有图异常检测模型大多忽视节点属性缺失和对抗性模糊问题，这会破坏邻居聚合的稳定性和预测可靠性，需要设计轻量级解决方案来增强属性不完整情况下的模型鲁棒性。

Method: 提出FGC-Comp模块：将节点邻居按标签分为三组，对已知标签组应用特定变换，对未知组使用节点条件门控处理，通过残差连接融合信息，并以端到端方式训练二分类目标来提升聚合稳定性。

Result: 在两个真实世界欺诈数据集上的实验验证了该方法的有效性，计算开销可忽略不计，显著提升了属性缺失情况下的邻居聚合稳定性和预测可靠性。

Conclusion: FGC-Comp是一个轻量级、部署友好的属性补全模块，能有效缓解图异常检测中节点属性缺失和对抗性模糊问题，增强模型在实际应用中的鲁棒性。
...
### [181] [Credal Graph Neural Networks](https://arxiv.org/abs/2512.02722)
*Matteo Tolloso,Davide Bacciu*

Main category: cs.LG

TL;DR: 提出首个信度图神经网络（CGNN），将信度学习扩展到图领域，通过训练GNN输出信度集形式的集合值预测，在异配图分布偏移下实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络不确定性量化方法主要依赖贝叶斯推断或集成学习，需要更可靠的不确定性量化方法，特别是在图域中扩展信度学习以处理分布外条件下的不确定性。

Method: 开发了信度图神经网络（CGNN），通过利用层间信息传播的不同方面，训练GNN输出信度集形式的集合值预测，特别考虑了图同配性假设对不确定性估计的影响。

Result: CGNN在异配图的分布偏移下提供了更可靠的认知不确定性表示，并实现了最先进的性能，实验表明图同配性假设对不确定性估计效果有重要影响。

Conclusion: 信度图神经网络成功将信度学习扩展到图域，为图神经网络提供了更可靠的不确定性量化方法，特别是在处理异配图和分布偏移时表现出色。
...
### [182] [Adversarial Jamming for Autoencoder Distribution Matching](https://arxiv.org/abs/2512.02740)
...
<details>
  <summary>Details</summary>
Motivation: 传统自编码器在潜在空间分布匹配方面存在挑战，需要新的正则化方法。本文受到对抗性无线干扰理论的启发，探索利用干扰作为辅助目标来引导潜在后验分布匹配对角高斯分布。

Method: 提出一种新颖的分布匹配方法：将干扰器作为对抗性目标，与编码器和解码器构成极小极大博弈。干扰器试图破坏高斯源在对抗信道中的恢复，而编码器-解码器对则努力抵抗干扰。通过这种博弈，潜在空间被正则化以匹配对角高斯分布。

Result: 该方法实现了与标准变分自编码器和Wasserstein自编码器相当的分布匹配效果。理论分析表明，博弈的鞍点对应于干扰器输出的对角高斯噪声，这为潜在空间的正则化提供了理论基础。

Conclusion: 对抗性干扰为自编码器的潜在空间分布匹配提供了一种新颖有效的正则化方法，不仅适用于对角高斯分布，还可以推广到其他潜在分布，为生成模型的正则化开辟了新途径。
...
### [183] [FiMMIA: scaling semantic perturbation-based membership inference across modalities](https://arxiv.org/abs/2512.02786)
...
<details>
  <summary>Details</summary>
Motivation: 现有成员推理攻击方法在单模态大语言模型上表现良好，但在多模态大语言模型上效果不佳，因为多模态组件适配和输入分布变化带来了不稳定性。

Method: 1) 识别现有数据集中的分布变化；2) 发布扩展基线流程检测这些变化；3) 将基于扰动的成员推理方法推广到多模态模型；4) 提出FiMMIA框架，训练神经网络分析目标模型在扰动输入上的行为，捕捉成员与非成员之间的分布差异。

Result: 在各种微调的多模态模型上进行全面评估，证明了基于扰动的成员推理攻击在多模态领域的有效性。

Conclusion: FiMMIA框架为多模态成员推理攻击提供了有效的解决方案，能够检测多模态模型中的训练数据泄露问题。
...
### [184] [From Navigation to Refinement: Revealing the Two-Stage Nature of Flow-based Diffusion Models through Oracle Velocity](https://arxiv.org/abs/2512.02826)
*Haoming Liu,Jinnuo Liu,Yanhao Li,Liuyang Bai,Yunkai Ji,Yuanhe Guo,Shenji Wan,Hongyi Wen*

Main category: cs.LG

TL;DR: 该研究分析了流匹配扩散模型的记忆-泛化行为，揭示了其训练目标天然分为两个阶段：早期导航阶段泛化数据模式形成全局布局，后期精炼阶段记忆细粒度细节。


<details>
  <summary>Details</summary>
Motivation: 流基扩散模型已成为图像和视频生成的主流范式，但其记忆-泛化行为仍未被充分理解。研究者希望深入分析流匹配目标的边际速度场，揭示模型训练动态的本质特征。

Method: 重新审视流匹配目标，研究其边际速度场，该场具有闭式表达式，允许精确计算oracle流匹配目标。通过分析这个oracle速度场，揭示训练目标的两阶段结构。

Result: 发现流基扩散模型天然形成两阶段训练目标：早期阶段由数据模式混合引导，后期阶段由最近数据样本主导。早期导航阶段泛化数据模式形成全局布局，后期精炼阶段记忆细粒度细节。

Conclusion: 这一理解解释了时间步偏移调度、无分类器引导区间和潜在空间设计等实用技术的有效性，为未来架构和算法改进提供了指导原则，深化了对扩散模型训练动态的理解。
...
### [185] [A Comparative Study on How Data Normalization Affects Zero-Shot Generalization in Time Series Foundation Models](https://arxiv.org/abs/2512.02833)
...
<details>
  <summary>Details</summary>
Motivation: 时间序列数据在不同领域和通道间存在显著的尺度变化和非平稳性，这些问题会严重影响时间序列基础模型的性能。尽管归一化方法在特定数据集的时间序列模型中已有研究，但在需要泛化能力的TSFMs中却被忽视。

Method: 通过系统评估四种架构不同的时间序列基础模型，比较了多种输入归一化方法，特别关注REVIN方法的效果。分析了不同模型类型（概率模型、点预测模型、基于LLM的模型）和优化目标对归一化效果的影响。

Result: REVIN被证明是最有效的归一化方法：在零样本预测中，相比未归一化基线减少89%的MASE误差，相比其他归一化方法减少44%。同时能达到最佳的域内精度（0.84 MASE），无需任何数据集级预处理，实现了最高的精度-效率权衡。

Conclusion: 输入归一化对时间序列基础模型的性能至关重要，REVIN是最有效的归一化方法。然而，归一化效果的有效利用取决于架构设计选择和优化目标，特别是训练损失尺度敏感性和模型类型。
...
### [186] [GraphMatch: Fusing Language and Graph Representations in a Dynamic Two-Sided Work Marketplace](https://arxiv.org/abs/2512.02849)
*Mikołaj Sacha,Hammad Jafri,Mattie Terzolo,Ayan Sinha,Andrew Rabinovich*

Main category: cs.LG

TL;DR: GraphMatch：融合预训练语言模型与图神经网络的动态双向市场推荐框架，在文本丰富、动态变化的环境中优于纯语言或纯图模型


<details>
  <summary>Details</summary>
Motivation: 文本丰富、动态变化的双向市场（如劳动力市场）中，推荐系统面临内容不断演变、交互图动态变化的挑战，需要同时捕捉文本语义和图结构的时间敏感性

Method: 提出GraphMatch框架，结合预训练语言模型和图神经网络，使用对抗负采样和时点子图训练，学习能同时捕捉文本细粒度语义和图时间敏感结构的表示

Result: 在Upwork劳动力市场的大规模实验中，GraphMatch在匹配任务上优于纯语言模型和纯图模型基线，同时保持运行时高效性，适合实时推理

Conclusion: 统一语言和图表示是解决文本丰富、动态双向市场推荐的有效方案，成功弥合了强大预训练语言模型与大规模图在实际应用中的差距
...
### [187] [Adaptive Decentralized Federated Learning for Robust Optimization](https://arxiv.org/abs/2512.02852)
...
<details>
  <summary>Details</summary>
Motivation: 去中心化联邦学习中存在异常客户端（由噪声或中毒数据引起），会显著干扰学习过程并降低模型鲁棒性。现有方法通常需要足够多的正常邻居客户端或可靠客户端的先验知识，这限制了DFL的实际适用性。

Method: 开发自适应DFL(aDFL)方法，核心思想是自适应调整客户端的学习率：对可疑客户端分配较小的学习率，对正常客户端分配较大的学习率，以完全自适应的方式减轻异常客户端对全局模型的负面影响。

Result: 理论分析不要求对邻居节点施加严格条件，也不需要先验知识，提供了严格的收敛分析以保证aDFL的oracle性质。大量数值实验证明了aDFL方法的优越性能。

Conclusion: aDFL方法能够有效处理去中心化联邦学习中的异常客户端问题，通过自适应学习率调整机制，在无需先验知识的情况下提升模型鲁棒性和学习效果。
...
### [188] [Assessing the performance of correlation-based multi-fidelity neural emulators](https://arxiv.org/abs/2512.02868)
*Cristian J. Villatoro,Gianluca Geraci,Daniele E. Schiavazzi*

Main category: cs.LG

TL;DR: 该研究系统评估了多保真度神经仿真器在不同复杂场景下的性能，包括高维函数、振荡特性、不连续性、参数化差异和噪声数据等情况，并与单保真度方法进行对比。


<details>
  <summary>Details</summary>
Motivation: 解决高保真度模型计算成本过高导致优化、不确定性量化等任务难以处理的问题，以及数据驱动方法需要大量高精度数据的问题。通过利用廉价低保真度信息结合少量高保真度数据来提高预测效率。

Method: 采用多保真度神经仿真器（神经网络），通过整合有限的高保真度数据和丰富的低保真度模型解来学习输入到输出的映射。研究了多种网络架构（MLP、Siren、KAN）、坐标编码机制、低保真度信息处理方式，并在不同训练数据集规模下进行测试。

Result: 系统评估了多保真度方法在各种复杂场景下的性能表现，包括低维和高维函数、振荡特性、不连续性、参数化差异以及可能被污染的低保真度数据源等情况。

Conclusion: 多保真度方法通过融合多个信息源能显著提升性能，特别是在高维复杂问题中，相比单保真度方法具有明显优势。该研究为多保真度神经仿真器的设计和应用提供了全面的评估框架。
...
### [189] [OptPO: Optimal Rollout Allocation for Test-time Policy Optimization](https://arxiv.org/abs/2512.02882)
...
Method: 将投票过程建模为贝叶斯序贯概率比检验，动态停止采样（当后验置信度超过阈值时）。保留的rollout用于策略更新，可与PPO、GRPO等算法无缝集成，无需真实标签。
...
### [190] [FAIRY2I: Universal Extremely-Low Bit QAT framework via Widely-Linear Representation and Phase-Aware Quantization](https://arxiv.org/abs/2512.02901)
*Feiyu Wang,Xinyu Tan,Bokai Huang,Yihao Zhang,Guoan Wang,Peizhuang Cong,Tong Yang*

Main category: cs.LG

TL;DR: Fairy2i 是一个将预训练实值LLM转换为等效复值表示的框架，通过广泛线性变换和相位感知量化实现极低比特（2比特）量化，无需重新训练即可重用现有模型权重。


<details>
  <summary>Details</summary>
Motivation: 现有复值LLM（如iFairy）虽然为低比特表示提供了更好机会，但需要从头训练，无法利用庞大的预训练实值基础模型生态系统。需要一种方法既能享受复值算术的表征效率，又能重用现有预训练模型。

Method: 1) 证明实值映射与广泛线性复值映射之间的无损数学等价性，将标准Transformer转换为复域；2) 采用相位感知量化方案，使用四分之一单位根的高效码本；3) 引入递归残差量化机制，迭代最小化量化误差，实现无乘法累加的高效推理。

Result: Fairy2i 将 LLaMA-2 7B 模型在有效2比特精度下的性能恢复到接近全精度基线水平，显著优于最先进的实值二值和三值量化方法。

Conclusion: 该工作弥合了复值算术的表征效率与预训练模型实际效用之间的差距，为在商品硬件上进行高效推理开辟了新途径。
...
### [191] [Hypothesis Testing for Generalized Thurstone Models](https://arxiv.org/abs/2512.02912)
*Anuran Makur,Japneet Singh*

Main category: cs.LG

TL;DR: 本文开发了一个假设检验框架，用于判断成对比较数据是否由给定的选择函数F生成的广义Thurstone模型产生。研究聚焦于该模型的极小极大假设检验问题，而非参数估计。


<details>
  <summary>Details</summary>
Motivation: 先前研究主要关注广义Thurstone模型的参数估计和不确定性量化，但缺乏对这些模型本身的假设检验方法。本文旨在解决这一基础问题，为判断数据是否来自特定Thurstone模型提供理论框架。

Method: 1. 引入广义Thurstone模型与一般成对比较模型之间的分离距离概念；2. 推导依赖于观测图拓扑结构的检验临界阈值上下界；3. 提出基于分离距离的假设检验方法；4. 构建置信区间；5. 使用反向鞅技术建立时间一致的第一类和第二类错误概率界限；6. 使用信息论方法推导极小极大下界。

Result: 1. 对于完全观测图的特殊情况，检验阈值按Θ((nk)^{-1/2})缩放，其中n为代理数量，k为每对比较次数；2. 建立了检验的上下界；3. 在合成和真实数据集上验证了理论结果的有效性。

Conclusion: 本文为广义Thurstone模型的假设检验提供了完整的理论框架，包括检验统计量、误差界限和极小极大最优性结果。该框架能够有效判断成对比较数据是否来自特定的Thurstone模型，为模型选择提供了理论基础。
...
### [192] [Learning Multimodal Embeddings for Traffic Accident Prediction and Causal Estimation](https://arxiv.org/abs/2512.02920)
*Ziniu Zhang,Minxuan Duan,Haris N. Koutsopoulos,Hongyang R. Zhang*

Main category: cs.LG

TL;DR: 该研究构建了一个包含道路网络数据和卫星图像的多模态数据集，用于分析交通事故模式，发现整合视觉和网络嵌入能显著提升预测准确性，并识别出降水、道路类型和季节模式等关键影响因素。
...
Result: 整合多模态数据将预测准确率提升到平均AUROC 90.1%，比仅使用图结构的图神经网络模型提高了3.7%。因果分析发现：较高降水使事故率上升24%，高速公路等高速度道路使事故率上升22%，季节模式使事故率上升29%。消融研究确认卫星图像特征对准确预测至关重要。

Conclusion: 整合道路网络数据和卫星图像等多模态信息能显著提升交通事故预测准确性，并有助于识别关键影响因素。卫星图像提供的视觉特征对准确预测具有重要作用。
...
### [193] [Fast Gaussian Process Approximations for Autocorrelated Data](https://arxiv.org/abs/2512.02925)
*Ahmadreza Chokhachian,Matthias Katzfuss,Yu Ding*

Main category: cs.LG

TL;DR: 提出一种加速自相关数据高斯过程回归计算的方法，通过数据分块去相关，在不牺牲预测性能的前提下显著提升计算速度。
...
Method: 将原始自相关数据点分割成块，使块内数据去相关，然后使现有高斯过程近似方法能够处理分块数据。具体展示了如何让一些现有高斯过程近似技术与分块数据协同工作。

Result: 在多个应用数据集上的数值实验表明，所提出的方法能够显著加速自相关数据的高斯过程回归计算，同时不损害模型预测性能。

Conclusion: 通过数据分块去相关的方法，成功解决了自相关数据高斯过程回归的计算加速问题，为处理时间序列等自相关数据提供了一种有效的解决方案。
...
### [194] [Pruning AMR: Efficient Visualization of Implicit Neural Representations via Weight Matrix Analysis](https://arxiv.org/abs/2512.02967)
*Jennifer Zvonek,Andrew Gillette*

Main category: cs.LG

TL;DR: 提出PruningAMR算法，通过剪枝隐式神经表示(INR)的权重矩阵来识别几何特征，并指导自适应网格细化，实现自动网格生成和内存节省。


<details>
  <summary>Details</summary>
Motivation: 虽然INR比传统格点数据更节省内存，但许多可视化任务仍需将其离散化到规则网格上。现有方法无法自动生成适应INR底层分辨率的自适应网格。

Method: 使用插值分解剪枝方法处理INR的权重矩阵来识别几何特征，利用剪枝后的网络指导自适应网格细化，从预训练的INR（无需访问训练数据）生成可变分辨率可视化。

Result: 能够生成适应函数底层分辨率的自适应网格，实现显著的内存节省，特别适用于4D CT扫描等内存密集型可视化任务。

Conclusion: PruningAMR算法成功解决了INR到自适应网格的转换问题，实现了自动化的可变分辨率可视化，为内存受限的可视化应用提供了有效解决方案。
...
### [195] [ProteinPNet: Prototypical Part Networks for Concept Learning in Spatial Proteomics](https://arxiv.org/abs/2512.02983)
...
<details>
  <summary>Details</summary>
Motivation: 理解肿瘤微环境的空间结构对精准肿瘤学至关重要。传统的事后解释模型存在局限性，需要能够直接学习可解释、可信赖的空间原型的方法。

Method: 基于原型部分网络（prototypical part networks）的ProteinPNet框架，通过监督训练直接学习具有区分性、可解释性、可信赖的空间原型。

Result: 在具有真实模式标记的合成数据集上验证了方法有效性，在真实世界肺癌空间蛋白质组学数据集上测试，一致识别出与不同肿瘤亚型对齐的生物学意义原型。图形和形态学分析显示这些原型捕获了可解释特征，指向免疫浸润和组织模块化的差异。
...
### [196] [Distribution-Calibrated Inference time compute for Thinking LLM-as-a-Judge](https://arxiv.org/abs/2512.03019)
*Hamid Dadkhahi,Firas Trabelsi,Parker Riley,Juraj Juraska,Mehdi Mirzazadeh*

Main category: cs.LG

TL;DR: 提出一种基于分布校准的聚合方法，通过建模三向偏好并使用Bradley-Terry-Davidson公式，将大语言模型的噪声判断转化为可靠的评分


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型作为成对偏好评判者时，单样本层面存在噪声，且常用聚合规则（多数投票、软自洽性或基于指令的自聚合）在允许平局时不一致

Method: 提出分布校准的聚合方案，建模三向偏好，使用Bradley-Terry-Davidson公式处理评分计数，同时利用极性（非平局间的差距）和决定性（非平局率）来区分窄差距与强共识

Result: 在各种评估基准上，该方法持续减少MAE并提高成对准确率，相比标准基线；在与人类共识元标签对比时，匹配或超过个体人类评分者

Conclusion: 精心分配推理时计算资源并使用分布感知的聚合方法，可以将噪声的个体模型判断转化为可靠的评估评分
...
### [197] [TokenPowerBench: Benchmarking the Power Consumption of LLM Inference](https://arxiv.org/abs/2512.03024)
*Chenxu Niu,Wei Zhang,Jie Li,Yongjian Zhao,Tongyang Wang,Xi Wang,Yong Chen*

Main category: cs.LG

TL;DR: TokenPowerBench是首个用于LLM推理功耗研究的轻量级可扩展基准测试工具，支持GPU、节点和系统级功耗测量，无需专用功率计，可分析不同配置下的每token能耗。
...
Result: 在四个最广泛使用的模型系列（Llama、Falcon、Qwen、Mistral）上评估，覆盖从10亿参数到前沿规模的Llama3-405B模型，支持用户通过改变批量大小、上下文长度、并行策略和量化快速评估每token焦耳等能效指标。

Conclusion: TokenPowerBench作为开源工具发布，帮助用户测量功耗、预测运营成本，并在部署LLM服务时满足可持续性目标，填补了LLM推理功耗分析工具的空白。
...
### [198] [PopSim: Social Network Simulation for Social Media Popularity Prediction](https://arxiv.org/abs/2512.02533)
*Yijun Liu,Wu Liu,Xiaoyan Gu,Allen He,Weiping Wang,Yongdong Zhang*

Main category: cs.MM

TL;DR: PopSim：基于大语言模型多智能体模拟的社交媒体流行度预测新范式，通过模拟UGC传播动态而非静态历史特征建模，显著提升预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有社交媒体流行度预测方法主要采用归纳范式，基于历史数据训练静态模型。然而UGC传播本质上是动态过程，静态建模无法捕捉复杂的交互和非线性演化。

Method: 提出PopSim模拟框架：1）基于LLM的多智能体社交网络沙箱模拟UGC传播动态；2）设计基于社会平均场的智能体交互机制，建模双通道双向个体-群体交互；3）多源信息聚合模块将异构社交元数据统一格式化；4）融合多模态传播动态进行综合预测。

Result: 在真实数据集上的实验表明，PopSim持续优于现有最先进方法，平均减少预测误差8.82%，为SMPP任务研究提供了新视角。

Conclusion: PopSim通过模拟而非归纳的范式，有效捕捉UGC传播的动态特性，为社交媒体流行度预测提供了更准确、更符合实际传播机制的新方法。
...
### [199] [Stepwise Schema-Guided Prompting Framework with Parameter Efficient Instruction Tuning for Multimedia Event Extraction](https://arxiv.org/abs/2512.02584)
*Xiang Yuan,Xinrong Chen,Haochen Li,Hang Yang,Guanyu Wang,Weiping Li,Tong Mo*

Main category: cs.MM

TL;DR: 本文提出了一种基于多模态大语言模型的逐步模式引导提示框架（SSGPF），用于解决多媒体事件抽取任务中的结构建模和数据对齐挑战，在M2E2基准上显著超越现有最佳方法。


<details>
  <summary>Details</summary>
Motivation: 当前多媒体事件抽取面临两个主要挑战：1）现有框架难以处理复杂灵活的多媒体事件结构；2）缺乏多模态对齐的训练数据进行有效的知识迁移。

Method: 提出逐步模式引导提示框架（SSGPF），使用多模态大语言模型作为骨干。首先设计事件类型模式引导提示（ETSGP）进行事件检测，然后设计参数角色模式引导提示（ARSGP）进行参数抽取，其中包含基于文本桥接定位技术的多步提示。基于现有单模态事件标注构建弱对齐多模态事件数据集，并在SSGPF框架下使用LoRA对LLaVA-v1.5-7B进行参数高效指令调优。

Result: 在M2E2基准测试中，SSGPF在事件检测上比当前最佳基线提升5.8% F1分数，在参数抽取上提升8.4% F1分数。

Conclusion: SSGPF框架通过逐步模式引导提示和多模态大语言模型的有效结合，成功解决了多媒体事件抽取中的结构建模和数据对齐问题，取得了显著的性能提升。
...
### [200] [The 4/$δ$ Bound: Designing Predictable LLM-Verifier Systems for Formal Method Guarantee](https://arxiv.org/abs/2512.02080)
*PIerre Dantas,Lucas Cordeiro,Youcheng Sun,Waldir Junior*

Main category: cs.AI

TL;DR: 提出LLM-Verifier收敛定理，为LLM与形式验证工具的结合提供首个具有可证明终止性和收敛性保证的形式化框架，通过马尔可夫链建模和大量实验验证了理论预测。


<details>
  <summary>Details</summary>
Motivation: 当前LLM与形式验证工具结合的方法缺乏理论基础，导致验证过程不可靠、不稳定，需要建立具有可证明保证的形式化框架来支持安全关键软件环境的部署。

Method: 将LLM与验证器的交互建模为离散时间马尔可夫链，定义关键参数错误减少概率δ，推导出LLM-Verifier收敛定理，并通过超过90,000次实验进行压力测试。

Result: 理论证明对于任何δ>0，过程几乎必然达到验证状态，期望迭代次数有界E[n]≤4/δ。实验结果与理论高度一致，所有运行都成功验证，收敛因子Cf≈1.0。

Conclusion: 建立了LLM辅助验证的清晰架构基础，定义了三个操作区域（边缘、实用、高性能），为工程师提供了可预测的资源规划和性能预算框架，消除了启发式调优的需求。
...
### [201] [Flowchart2Mermaid: A Vision-Language Model Powered System for Converting Flowcharts into Editable Diagram Code](https://arxiv.org/abs/2512.02170)
*Pritam Deka,Barry Devereux*

Main category: cs.AI

TL;DR: Flowchart2Mermaid：一个将流程图图像转换为可编辑Mermaid.js代码的轻量级Web系统，支持混合主动式细化编辑
...
Conclusion: 与之前的图像到图表工具不同，该方法产生结构化、版本可控的文本表示，支持可编辑性和重用性，为流程图转换提供了有效的解决方案。
...
### [202] [From monoliths to modules: Decomposing transducers for efficient world modelling](https://arxiv.org/abs/2512.02193)
*Alexander Boyd,Franz Nowak,David Hyland,Manuel Baltieri,Fernando E. Rosas*

Main category: cs.AI

TL;DR: 提出一个框架，用于将复杂的世界模型（表示为transducer）分解为可并行处理的子组件，提高计算效率和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现实世界模型通常计算需求高，但真实场景往往由模块化交互的子组件构成，可以利用这种特性实现高效建模。需要为AI安全提供结构透明性，同时满足实际推理的计算效率要求。

Method: 开发了一个框架，用于分解由transducer（一种推广POMDP的模型类）表示的复杂世界模型。通过反转transducer组合过程，推导出在独立输入-输出子空间上运行的子transducer。

Result: 实现了可并行化和可解释的替代方案，替代了单一的世界建模方法，支持分布式推理。为连接AI安全所需的结构透明性和实际推理所需的计算效率奠定了基础。

Conclusion: 该框架为复杂世界模型的分解提供了理论基础，通过利用现实场景的模块化特性，实现了计算效率与可解释性的平衡，有助于AI安全与高效推理的结合。
...
### [203] [STRIDE: A Systematic Framework for Selecting AI Modalities -- Agentic AI, AI Assistants, or LLM Calls](https://arxiv.org/abs/2512.02228)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI从无状态大语言模型向自主目标驱动代理的快速转变，需要解决一个核心问题：何时真正需要代理式AI？盲目部署代理会导致成本、复杂性和风险增加。

Method: 提出STRIDE框架，集成结构化任务分解、动态性归因和自我反思需求分析，生成代理适用性评分，为任务推荐三种模态：直接LLM调用、引导式AI助手或完全自主代理。

Result: 在30个真实世界任务中，STRIDE实现了92%的模态选择准确率，减少45%不必要的代理部署，降低37%资源成本。六个月专家验证确认其实际效用。
...
### [204] [Benchmarking LLM Agents for Wealth-Management Workflows](https://arxiv.org/abs/2512.02230)
*Rory Milsom*

Main category: cs.AI

TL;DR: 该研究扩展了TheAgentCompany平台，创建了面向财富管理的评估环境，通过12个任务对评估LLM代理在财富管理助理工作中的表现，发现代理的主要限制在于端到端工作流可靠性而非数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管现代工作依赖各种数字协作工具，但常规流程仍受人为错误和延迟困扰。研究旨在填补这一空白，探索通用LLM代理是否能准确且经济地完成代表性财富管理任务。

Method: 扩展TheAgentCompany平台，创建金融专用环境；引入合成领域数据，丰富同事模拟，原型化自动任务生成流程；构建包含12个任务对的财富管理助理基准测试，涵盖检索、分析和综合/沟通，设置明确验收标准和确定性评分器；创建新的金融专用数据集，并为每个任务设计高自主性和低自主性变体。

Result: 代理的主要限制不在于数学推理能力，而在于端到端工作流的可靠性；自主性水平对代理表现有显著影响；不正确的模型评估方法阻碍了基准测试的有效性。

Conclusion: LLM代理在财富管理助理工作中面临的主要挑战是工作流可靠性而非计算能力，自主性水平是关键影响因素，需要改进评估方法以准确衡量代理的适用性。
...
### [205] [TradeTrap: Are LLM-based Trading Agents Truly Reliable and Faithful?](https://arxiv.org/abs/2512.02261)
*Lewen Yan,Jilin Mei,Tianyi Zhou,Lige Huang,Jie Zhang,Dongrui Liu,Jing Shao*

Main category: cs.AI

TL;DR: TradeTrap是一个用于系统性压力测试自主交易代理的统一评估框架，通过针对市场情报、策略制定、投资组合管理和交易执行四个核心组件施加受控扰动，揭示了当前自主交易代理在系统层面易受误导的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM驱动的交易代理在真实金融市场中日益部署，但在高风险、不可逆的金融环境中，其对抗性条件下的可靠性和鲁棒性尚未得到充分检验。需要系统评估这些代理在故障或对抗条件下的表现。

Method: 提出了TradeTrap统一评估框架，针对自主交易代理的四个核心组件（市场情报、策略制定、投资组合和账本处理、交易执行）施加受控的系统级扰动。在真实美股市场数据的闭环历史回测环境中进行评估，确保初始条件相同，实现公平可复现的比较。

Result: 实验表明，单个组件的小幅扰动可以通过代理决策循环传播，导致极端集中化、失控的风险敞口和大幅投资组合回撤。两种类型的代理（自适应和程序化）都表现出在系统层面易受系统性误导的脆弱性。

Conclusion: 当前自主交易代理在系统层面存在系统性脆弱性，需要更鲁棒的设计和评估方法。TradeTrap框架为评估和改进交易代理的鲁棒性提供了重要工具。
...
### [206] [Bridging the Gap: Toward Cognitive Autonomy in Artificial Intelligence](https://arxiv.org/abs/2512.02280)
...
Result: 当前架构（包括深度学习和基于Transformer的系统）存在结构性限制，仅靠扩展无法解决。这些能力在当前模型中缺失，阻碍了AI实现真正的自主性和适应性。

Conclusion: 需要范式转变，向基于认知原理的AI（认知自主性）发展，具备自我导向适应、动态表征管理和意向性目标导向行为的能力，同时配备改革性监督机制，确保自主系统保持可解释性、可治理性和与人类价值观对齐。
...
### [207] [DialogGuard: Multi-Agent Psychosocial Safety Evaluation of Sensitive LLM Responses](https://arxiv.org/abs/2512.02282)
*Han Luo,Guy Laban*

Main category: cs.AI

TL;DR: DialogGuard是一个多智能体框架，用于评估LLM生成内容在隐私侵犯、歧视行为、心理操纵、心理伤害和侮辱行为五个高风险维度的心理社会安全性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地应用于心理健康、危机干预等敏感服务，但其在这些场景中的心理社会安全性评估不足，缺乏有效的评估框架。

Method: 提出DialogGuard多智能体框架，包含四种LLM作为评判者的流程：单智能体评分、双智能体修正、多智能体辩论和随机多数投票，基于共享的三级评分标准。

Result: 多智能体机制比非LLM基线和单智能体评判更准确地检测心理社会风险；双智能体修正和多数投票在准确性、与人类评分对齐和鲁棒性方面达到最佳平衡；辩论机制召回率更高但会过度标记边缘案例。

Conclusion: DialogGuard作为开源软件发布，提供维度风险评分和可解释的自然语言理由，支持面向脆弱用户的网络应用进行提示设计、审计和监督。
...
### [208] [Model Recovery at the Edge under Resource Constraints for Physical AI](https://arxiv.org/abs/2512.02283)
*Bin Xu,Ayan Banerjee,Sandeep K. S. Gupta*

Main category: cs.AI

TL;DR: MERINDA是一个FPGA加速的模型恢复框架，通过可并行神经架构替代迭代求解器，在边缘设备上实现低内存、低能耗的实时任务关键自主系统决策。


<details>
  <summary>Details</summary>
Motivation: 模型恢复（MR）能实现任务关键自主系统（MCAS）的安全可解释决策，但现有基于神经ODE的方法在边缘设备上部署困难，主要受限于FPGA上迭代求解器的低效性，以及内存和能耗问题。
...
Conclusion: MERINDA框架特别适合资源受限、需要实时操作的任务关键自主系统，通过FPGA加速解决了模型恢复在边缘设备部署的内存和能耗瓶颈。
...
### [209] [Breast Cell Segmentation Under Extreme Data Constraints: Quantum Enhancement Meets Adaptive Loss Stabilization](https://arxiv.org/abs/2512.02302)
...
<details>
  <summary>Details</summary>
Motivation: 医学图像标注需要大量时间和专业知识，乳腺上皮细胞数据集标注通常需要病理学家投入数百小时。现有方法面临严重类别不平衡（乳腺组织仅占4%像素）和边界检测困难等挑战。

Method: 1) 多尺度Gabor滤波器实现量子启发边缘增强，创建第四输入通道；2) 集成自适应Dice损失与边界感知项的稳定多组件损失函数；3) 复杂度加权采样策略优先处理困难区域；4) EfficientNet-B7/UNet++架构配合4到3通道投影；5) 指数移动平均和统计异常值检测进行鲁棒验证。

Result: Dice分数95.5% ± 0.3%，IoU 91.2% ± 0.4%。量子增强使边界准确率提升2.1%，加权采样使小病灶检测提升3.8%。仅用599张训练图像（其中60%无乳腺区域）即达到优异性能。

Conclusion: 该框架显著减少了医学专家标注时间，解决了临床感知AI开发中的基本瓶颈，在有限标注下实现了突破性性能，为医学图像分析提供了高效解决方案。
...
### [210] [OmniGuard: Unified Omni-Modal Guardrails with Deliberate Reasoning](https://arxiv.org/abs/2512.02306)
*Boyu Zhu,Xiaofei Wen,Wenjie Jacky Mo,Tinghui Zhu,Yanan Xie,Peng Qi,Muhao Chen*

Main category: cs.AI

TL;DR: OmniGuard是首个全模态安全护栏系统，通过精心策划的大型多模态安全数据集和结构化安全标签，为处理文本、图像、视频和音频的全模态大语言模型提供统一的安全保障框架。


<details>
  <summary>Details</summary>
Motivation: 全模态大语言模型处理多种模态数据时，现有安全护栏研究主要针对单模态设置，且通常将安全保障视为二元分类问题，这限制了在不同模态和任务间的鲁棒性。需要开发能够跨所有模态进行安全防护的系统。

Method: 提出OmniGuard框架，构建包含超过21万个多样化样本的大型全模态安全数据集，涵盖所有模态的单模态和跨模态输入。每个样本都标注了结构化安全标签，并通过目标蒸馏从专家模型中精心策划安全评析。

Result: 在15个基准测试上的广泛实验表明，OmniGuard在各种多模态安全场景中实现了强大的有效性和泛化能力，为全模态安全保障提供了统一框架。

Conclusion: OmniGuard通过统一框架在全模态中执行策略和缓解风险，为构建更鲁棒和强大的全模态安全系统铺平了道路，解决了现有安全护栏在多模态环境中的局限性。
...
### [211] [Reasoning Path and Latent State Analysis for Multi-view Visual Spatial Reasoning: A Cognitive Science Perspective](https://arxiv.org/abs/2512.02340)
*Qiyao Xue,Weichen Liu,Shiqi Wang,Haoming Wang,Yuyang Wu,Wei Gao*

Main category: cs.AI

TL;DR: 该论文提出了ReMindView-Bench基准测试，用于评估视觉语言模型在多视角空间推理中构建、对齐和维护空间心理模型的能力，揭示了当前模型在跨视角对齐和视角转换方面的系统性缺陷。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在多视角设置中进行空间推理时，难以保持几何一致性和跨视角一致性。现有基准测试未能将多视角推理与单视角感知和时间因素有效分离，缺乏对空间认知关键因素的细粒度评估。

Method: 提出了ReMindView-Bench基准测试，系统性地变化视角空间模式和查询类型来探测空间认知的关键因素。采用显性分析（LLM-as-a-judge和自一致性提示）和隐性分析（线性探测和熵动态分析）相结合的方法，评估15个当前主流视觉语言模型。

Result: 评估显示当前视觉语言模型在多视角空间推理中存在一致的跨视角对齐和视角转换失败。模型在帧内感知表现良好，但在跨视角信息整合时性能急剧下降。隐性分析显示任务相关信息逐渐丢失，正确与错误推理轨迹之间的不确定性分离。

Conclusion: 该研究为视觉语言模型的空间推理提供了认知基础的诊断，揭示了多视角空间心理模型在推理过程中如何形成、退化和失稳。ReMindView-Bench基准测试为未来改进模型的空间推理能力提供了重要工具。
...
### [212] [Beyond Playtesting: A Generative Multi-Agent Simulation System for Massively Multiplayer Online Games](https://arxiv.org/abs/2512.02358)
...
<details>
  <summary>Details</summary>
Motivation: 传统MMO游戏数值系统优化方法依赖大规模在线实验或基于预定义统计模型的参数调优，这些方法成本高、耗时长，且可能干扰玩家体验。简化的离线模拟系统保真度有限，无法准确模拟真实玩家的推理和对干预的反应。

Method: 提出基于大型语言模型的生成式智能体MMO模拟系统：1) 使用监督微调和强化学习在大规模真实玩家行为数据上适配LLMs，使其从通用先验转向游戏特定领域；2) 基于真实游戏日志训练数据驱动的环境模型，重建动态游戏系统。

Result: 实验表明该系统与真实世界玩家行为具有很强的一致性，在干预下能产生合理的因果响应，为数据驱动的数值设计优化提供了可靠、可解释且成本高效的框架。

Conclusion: 基于LLM的生成式智能体模拟系统能够有效解决传统MMO数值优化方法的局限性，提供更真实、可解释且成本更低的替代方案，为游戏设计优化开辟了新途径。
...
### [213] [Synthetic Error Injection Fails to Elicit Self-Correction In Language Models](https://arxiv.org/abs/2512.02389)
*David X. Wu,Shreyas Kapur,Anant Sahai,Stuart Russell*

Main category: cs.AI

TL;DR: 研究探索用监督学习+合成错误注入替代强化学习来培养语言模型自我纠错能力，但发现该方法效果有限，无法显著提升性能，且模型常重复原始错误。


<details>
  <summary>Details</summary>
Motivation: 强化学习已成为激发大语言模型推理和自我纠错能力的主流方法，但其计算成本高昂，因此需要探索替代方案。受自动驾驶和机器人技术启发，研究是否可以通过监督学习结合合成错误注入来诱导语言模型的自我纠错能力。

Method: 在推理链中插入人工合成的错误，将其掩码，然后监督模型识别和纠正这些错误。该方法旨在通过合成错误注入训练模型自我纠错能力。

Result: 该方法即使在简单的合成任务上也无法显著提升多个模型的性能。即使模型发现了自己的错误，也常常会重复原始错误。研究发现，从合成错误分布到在线策略错误的分布偏移会显著降低微调模型的纠错能力，即使合成错误覆盖了在线策略错误。

Conclusion: 研究结果有助于解释为什么在线策略强化学习方法在激发自我纠错能力方面被证明是独特有效的。监督学习加合成错误注入的方法虽然直观，但效果有限，无法替代强化学习。
...
### [214] [Semantic Trading: Agentic AI for Clustering and Relationship Discovery in Prediction Markets](https://arxiv.org/abs/2512.02436)
*Agostino Capponi,Alfio Gliozzo,Brian Zhu*

Main category: cs.AI

TL;DR: AI代理管道自动聚类预测市场并识别市场间的依赖关系，通过交易策略验证发现的关系具有可操作性
...
Method: 开发AI代理管道：1) 使用自然语言理解对合约文本和元数据进行聚类分析；2) 识别集群内市场对的结果依赖关系（正相关和反相关）；3) 基于历史Polymarket数据评估关系预测准确性；4) 将发现的关系转化为简单交易策略

Result: AI识别的市场关系准确率达到60-70%，基于这些关系构建的交易策略在周度时间范围内获得约20%的平均回报

Conclusion: 代理AI和大语言模型能够有效发现预测市场中的潜在语义结构，识别出的市场关系具有实际交易价值，为市场分析提供了新的自动化方法
...
### [215] [Guided Self-Evolving LLMs with Minimal Human Supervision](https://arxiv.org/abs/2512.02472)
*Wenhao Yu,Zhenwen Liang,Chengsong Huang,Kishan Panaganti,Tianqing Fang,Haitao Mi,Dong Yu*

Main category: cs.AI

TL;DR: R-Few是一个引导式自我进化框架，通过轻量级人类监督实现稳定可控的AI自我进化，在数学和通用推理任务上取得持续改进。


<details>
  <summary>Details</summary>
Motivation: AI自我进化是实现超智能的途径，但实践中无指导的自我进化系统容易陷入平台期或退化，存在概念漂移、多样性崩溃和错误进化等问题。需要一种稳定可控的自我进化方法，同时最小化对人类监督的依赖。

Method: 提出R-Few框架，包含引导式自我对弈的挑战者-求解器架构。挑战者使用少量人类标注示例指导合成问题生成，求解器在在线难度课程下联合训练人类和合成示例。

Result: 在数学和通用推理基准测试中，R-Few实现了持续迭代改进。例如，Qwen3-8B-Base在数学任务上比R-Zero提高了3.0分，性能与使用20倍人类数据训练的General-Reasoner相当。

Conclusion: R-Few通过接地气的挑战者训练和课程式求解器训练的互补贡献，缓解了漂移问题，产生了更稳定可控的协同进化动态，为AI自我进化提供了有效解决方案。
...
### [216] [COPE: Chain-Of-Thought Prediction Engine for Open-Source Large Language Model Based Stroke Outcome Prediction from Clinical Notes](https://arxiv.org/abs/2512.02499)
...
TL;DR: 开发了COPE框架，使用链式思维增强的LLaMA-3-8B模型从非结构化临床笔记预测急性缺血性卒中90天功能结局，性能与GPT-4.1相当，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 急性缺血性卒中预后预测对临床决策、患者咨询和资源分配很重要。临床笔记包含丰富信息，但非结构化特性限制了其在传统预测模型中的应用。

Method: 开发COPE框架，使用两步骤链式思维框架：第一步生成临床推理，第二步输出mRS预测。基于LLaMA-3-8B开源模型，比较了GPT-4.1、ClinicalBERT、结构化变量机器学习模型和单步LLM。

Result: COPE的MAE为1.01，±1准确率74.4%，精确准确率32.8%，与GPT-4.1相当，优于ClinicalBERT、临床ML和单步LLM。在不同亚组中表现一致。
...
### [217] [Aetheria: A multimodal interpretable content safety framework based on multi-agent debate and collaboration](https://arxiv.org/abs/2512.02530)
*Yuxiang He,Jian Zhao,Yuchen Yuan,Tianle Zhang,Wei Cai,Haojie Cheng,Ziyan Shi,Ming Zhu,Haichuan Tang,Chi Zhang,Xuelong Li*

Main category: cs.AI

TL;DR: 提出Aetheria多模态可解释内容安全框架，基于多智能体辩论协作，通过动态辩论机制和RAG知识检索，显著提升内容安全准确性，特别在识别隐性风险方面


<details>
  <summary>Details</summary>
Motivation: 数字内容爆炸式增长带来内容安全挑战，现有基于单一模型或固定流水线的审核系统在识别隐性风险和提供可解释判断过程方面存在局限

Method: 提出Aetheria多模态可解释内容安全框架，采用五个核心智能体的协作架构，通过基于RAG知识检索的动态相互说服辩论机制，对多模态内容进行深度分析和裁决

Result: 在提出的AIR-Bench基准测试中，Aetheria不仅能生成详细可追溯的审核报告，而且在整体内容安全准确性上显著优于基线方法，特别是在隐性风险识别方面
...
### [218] [Empathy Level Prediction in Multi-Modal Scenario with Supervisory Documentation Assistance](https://arxiv.org/abs/2512.02558)
...
<details>
  <summary>Details</summary>
Motivation: 现有共情预测方法主要关注单一模态（通常是文本），忽略了多模态处理能力，也没有利用可能包含额外共情内容的特权信息。
...
Conclusion: 提出的多模态共情预测方法通过整合多模态信息和利用监督文档作为特权信息，显著提升了共情预测性能。
...
### [219] [PaperDebugger: A Plugin-Based Multi-Agent System for In-Editor Academic Writing, Review, and Editing](https://arxiv.org/abs/2512.02589)
*Junyi Hou,Andre Lin Huikai,Nuo Chen,Yiwei Gong,Bingsheng He*

Main category: cs.AI

TL;DR: PaperDebugger是一个基于多代理的学术写作助手插件，直接在LaTeX编辑器（如Overleaf）中集成LLM驱动的推理，支持上下文感知的文档操作和版本控制。


<details>
  <summary>Details</summary>
Motivation: 现有AI写作助手与编辑器分离，无法深度交互文档状态、结构和修订历史，限制了在LaTeX编辑器中进行上下文感知的智能操作。
...
Conclusion: PaperDebugger验证了编辑器原生、代理驱动的学术写作助手的实用性，为深度集成AI辅助写作提供了技术框架。
...
### [220] [IACT: A Self-Organizing Recursive Model for General AI Agents: A Technical White Paper on the Architecture Behind kragent.ai](https://arxiv.org/abs/2512.02605)
*Pengju Lu*

Main category: cs.AI

TL;DR: IACT是一种新型计算模型，通过用户对话驱动的动态递归代理拓扑结构，解决传统静态工作流的限制，实现运行时错误纠正和歧义解决。
...
Method: IACT采用纯用户对话驱动的通用自主系统，根据高层目标自主构建动态递归代理拓扑结构，用双向、有状态的对话替代刚性调用，实现交互冗余和运行时错误纠正。

Result: 该模型已在kragent.ai系统中实际部署，通过真实工作流的定性证据展示了其有效性，能够根据问题结构扩展组织复杂性以适应开放任务。

Conclusion: IACT通过动态代理拓扑和双向对话机制，解决了传统工作流系统的局限性，为开放任务提供了灵活、可扩展的自主系统解决方案。
...
### [221] [Target-specific Adaptation and Consistent Degradation Alignment for Cross-Domain Remaining Useful Life Prediction](https://arxiv.org/abs/2512.02610)
*Yubo Hou,Mohamed Ragab,Min Wu,Chee-Keong Kwoh,Xiaoli Li,Zhenghua Chen*

Main category: cs.AI

TL;DR: TACDA：一种用于跨域剩余使用寿命预测的新型域适应方法，通过目标域重构和退化阶段一致性对齐来解决域差异问题


<details>
  <summary>Details</summary>
Motivation: 传统数据驱动的RUL预测方法假设训练和测试数据来自相同分布，但实际工业环境中存在域差异问题。现有的对抗域适应方法虽然提取域不变特征，但忽略了目标域特定信息和退化阶段的一致性特征，导致性能不佳。

Method: 提出TACDA方法：1）在对抗适应过程中加入目标域重构策略，在提取域不变特征的同时保留目标域特定信息；2）开发新颖的聚类和配对策略，实现相似退化阶段之间的一致性对齐。

Result: 通过大量实验验证，TACDA方法在两种不同评估指标上均显著优于现有最先进方法，表现出卓越的性能。

Conclusion: TACDA方法有效解决了跨域RUL预测中的域差异问题，通过目标域重构和退化阶段一致性对齐策略，在保留目标域信息的同时实现更好的域适应效果，为实际工业应用提供了有效解决方案。
...
### [222] [Zero-Shot Instruction Following in RL via Structured LTL Representations](https://arxiv.org/abs/2512.02633)
...
<details>
  <summary>Details</summary>
Motivation: 现有将LTL指令解释为有限自动机的方法，在多个原子命题同时为真且可能复杂交互的环境中表现不佳，需要新的方法来处理这种复杂性。

Method: 将策略条件化于与自动机转换直接对齐的简单布尔公式序列，通过图神经网络编码这些公式以产生结构化任务表示。

Result: 在复杂的基于象棋的环境中进行的实验证明了该方法的优势。

Conclusion: 提出的方法能够有效处理多个高层事件同时发生且复杂交互的环境，通过学习遵循任意LTL指令的多任务策略，在复杂任务执行中表现出优越性能。
...
### [223] [Exploring Depth Generalization in Large Language Models for Solving Recursive Logic Tasks](https://arxiv.org/abs/2512.02677)
*Zhiyuan He*

Main category: cs.AI

TL;DR: 本文研究了LLM在递归推理问题中的深度泛化能力不足问题，提出了一种循环定位-替换管道方法来解决嵌套层次结构的处理挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在许多任务上表现出色，但在处理需要解决嵌套层次结构的递归推理问题时面临显著挑战。先前研究主要关注长度泛化（处理比训练时更长的序列），而本文研究了一个不同且未被充分探索的局限性：深度泛化。深度指的是分层问题中的嵌套层数，如数学表达式中的括号层数或布尔公式中的逻辑子句嵌套。

Method: 为了解决这一挑战，作者开发了一种新颖的循环定位-替换管道，将递归问题分解为可管理的子组件。该方法采用两个专门模型：定位器识别可解的子表达式，替换器评估这些组件同时保持整体结构。

Result: 在三个精心设计的领域（布尔代数、递归算术和命题逻辑）进行评估，每个领域都有可控的递归深度。结果表明，该方法在测试超出分布范围的递归深度时，有效缓解了性能衰减问题。

Conclusion: 标准Transformer架构在处理比训练时更深的递归问题时存在困难，这源于它们无法维持类似堆栈的行为（跟踪和解决多个嵌套依赖层的能力）。通过系统分析，展示了这种架构约束如何导致随着递归深度增加而性能迅速衰减。提出的循环定位-替换管道方法为解决这一挑战提供了有效途径。
...
### [224] [Learning What to Attend First: Modality-Importance-Guided Reasoning for Reliable Multimodal Emotion Understanding](https://arxiv.org/abs/2512.02699)
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态情感理解方法存在推理漂移问题：模型逐渐依赖自身生成的文本而非多模态证据，且解释过度受视觉主导推理路径影响，导致情感不一致的解释。

Method: 提出模态重要性机制识别情感主导模态，基于此重新组织推理序列，使解释从最关键模态开始。采用两阶段框架：模态对齐监督微调和模态感知奖励优化。

Result: 在DFEW基准测试中，MIGR显著提高推理可靠性，将正确预测但情感不一致解释的比例从18.10%降低到7.37%。
...
### [225] [Training Data Attribution for Image Generation using Ontology-Aligned Knowledge Graphs](https://arxiv.org/abs/2512.02713)
*Theodoros Aivalis,Iraklis A. Klampanos,Antonis Troumpoukis,Joemon M. Jose*

Main category: cs.AI

TL;DR: 提出基于多模态大语言模型从图像中提取结构化三元组构建知识图谱的框架，用于追踪生成模型输出与训练数据之间的关联，支持版权分析、数据集透明度和可解释AI。
...
Conclusion: 提出的知识图谱框架能够有效追踪生成模型输出与训练数据之间的关联，为版权分析、数据集透明度和可解释AI提供了实用工具，有助于构建更负责任和透明的AI系统。
...
### [226] [Menta: A Small Language Model for On-Device Mental Health Prediction](https://arxiv.org/abs/2512.02716)
*Tianyi Zhang,Xiangyuan Xue,Lingyan Ruan,Shiya Fu,Feng Xia,Simon D'Alfonso,Vassilis Kostakos,Hong Jia*

Main category: cs.AI

TL;DR: Menta是首个针对社交媒体心理健康预测优化的轻量级小语言模型，在多任务学习中超越现有SLM基线15.2%，在部分任务上甚至优于13B参数的大模型，并能在iPhone上实时部署。


<details>
  <summary>Details</summary>
Motivation: 全球数亿人受心理健康问题影响，但早期检测仍有限。大语言模型(LLMs)在心理健康应用中有潜力，但计算需求大难以实际部署。小语言模型(SLMs)虽轻量，但在社交媒体心理健康预测方面研究不足。

Method: 提出Menta模型，采用LoRA微调框架、跨数据集策略和平衡准确率导向的损失函数，在六个分类任务上联合训练，专门针对社交媒体心理健康预测进行优化。

Result: Menta在抑郁、压力和自杀倾向等任务上，相比最佳非微调SLM平均提升15.2%；在抑郁和压力分类任务上准确率超过13B参数LLM，模型大小仅为其1/3.25；能在iPhone 15 Pro Max上实时部署，仅需约3GB内存。

Conclusion: Menta展示了轻量级模型在心理健康监测中的潜力，为可扩展、隐私保护的实时心理健康监测提供了可行方案。
...
### [227] [StockMem: An Event-Reflection Memory Framework for Stock Forecasting](https://arxiv.org/abs/2512.02720)
*He Wang,Wenyilin Xiao,Songqiao Han,Hailiang Huang*

Main category: cs.AI

TL;DR: StockMem：一个事件-反思双记忆层框架，通过结构化新闻为事件并进行横向整合与纵向追踪，构建事件知识库和因果经验反思库，用于股票价格预测，提供可解释的推理。


<details>
  <summary>Details</summary>
Motivation: 股票价格预测面临市场波动性和对实时事件敏感性的挑战。虽然大语言模型为基于文本的预测提供了新途径，但在金融领域的应用受到噪声新闻数据和文本中缺乏明确答案的阻碍。通用记忆架构难以识别价格变动的关键驱动因素。

Method: 提出StockMem，一个事件-反思双记忆层框架：1) 将新闻结构化为事件；2) 横向整合整合每日事件；3) 纵向追踪捕捉事件演化以提取反映市场预期差异的增量信息，构建时间事件知识库；4) 通过分析事件-价格动态，形成因果经验的反思知识库；5) 预测时检索类似历史场景，并结合当前事件、增量数据和过去经验进行推理。

Result: 实验表明StockMem优于现有记忆架构，并通过追踪影响价格的信息链提供更优、可解释的推理，增强了金融预测中的决策透明度。

Conclusion: StockMem通过事件-反思双记忆层框架有效解决了金融新闻数据噪声和缺乏明确答案的问题，能够识别价格变动的关键驱动因素，提供可解释的股票价格预测，在金融预测领域具有重要应用价值。
...
### [228] [AuditCopilot: Leveraging LLMs for Fraud Detection in Double-Entry Bookkeeping](https://arxiv.org/abs/2512.02726)
...
Method: 使用LLaMA和Gemma等最先进LLMs，在合成和真实匿名账本上进行基准测试，与传统JETs和机器学习基线对比
...
### [229] [Self-Improving AI Agents through Self-Play](https://arxiv.org/abs/2512.02731)
*Przemyslaw Chojecki*

Main category: cs.AI

TL;DR: 将心理测量学的模数理论框架扩展到动力系统领域，提出GVU算子生成参数流，推导出保证自改进稳定性的方差不等式条件，并统一了多种自改进架构。


<details>
  <summary>Details</summary>
Motivation: 将静态的AI能力评分扩展到动态系统，建立自改进过程的数学理论框架，统一理解语言自博弈、自我修正、合成数据引导等多种自改进方法。

Method: 将智能体形式化为计算资源参数化的流ν_r，由递归的生成器-验证器-更新器(GVU)算子控制，证明该算子在参数流形Θ上生成向量场，推导出保证自改进稳定性的方差不等式谱条件。

Result: 识别自改进系数κ为能力泛函沿流的李导数，证明方差不等式是自改进稳定性的充分条件，表明生成和验证的组合噪声必须足够小才能保证κ>0。

Conclusion: GVU算子为统一理解多种自改进架构提供了理论框架，STaR、SPIN、Reflexion、GANs和AlphaZero等都是满足方差不等式的GVU算子的具体拓扑实现。
...
### [230] [A Framework for Causal Concept-based Model Explanations](https://arxiv.org/abs/2512.02735)
*Anna Rodum Bjøru,Jacob Lysnæs-Larsen,Oskar Jørgensen,Inga Strümke,Helge Langseth*

Main category: cs.AI

TL;DR: 提出一个基于因果概念的事后可解释AI框架，通过概念干预的充分概率生成局部和全局解释，在CelebA数据集上验证，强调解释需同时具备可理解性和对模型的忠实性。


<details>
  <summary>Details</summary>
Motivation: 针对非可解释模型，需要既能让人类理解又忠实于原模型的解释方法。现有方法往往难以同时满足可理解性和忠实性要求。

Method: 基于因果概念框架，通过计算概念干预的充分概率来生成局部和全局解释。使用概念词汇表增强可理解性，在CelebA数据集上构建概念证明模型。

Result: 在CelebA数据集上展示了示例解释，证明框架能提供清晰的概念词汇和隐式因果解释。强调了框架假设的重要性，确保解释生成和解释理解的上下文一致性。

Conclusion: 提出了一个平衡可理解性和忠实性的因果概念事后XAI框架，强调解释生成和解释理解的上下文对齐对保证解释保真度至关重要。
...
### [231] [Enhancing Automated Paper Reproduction via Prompt-Free Collaborative Agents](https://arxiv.org/abs/2512.02812)
*Zijie Lin,Qilin Cai,Liang Shen,Mingjun Xiao*

Main category: cs.AI

TL;DR: 提出了一种无需人工提示的协作智能体框架，用于自动提升论文到代码生成的质量，通过验证和精炼两个智能体的协作，显著提高了代码复现的准确性和完整性。
...
Method: 提出一个无需提示的协作智能体框架，包含两个智能体：验证智能体检查每个步骤的输出是否满足系统提示的要求，精炼智能体根据识别的问题修订输出。该方法仅利用原始系统提示实现自动验证和改进，无需人工为每个步骤设计特定精炼提示。

Result: 在PaperBench Code-Dev和Paper2CodeBench数据集上的实验表明，该方法显著提高了复现代码的准确性和完整性，相比没有智能体的基线分别提升了约15%和13%。与Self-Refine的对比实验验证了该无提示方法在不同数据集上的鲁棒性和一致性。

Conclusion: 提出的无需提示协作智能体框架能够有效提升论文到代码生成的质量，解决了现有框架在验证和精炼方面的局限性，为自动化科学复现提供了更适应和可扩展的解决方案。
...
### [232] [Radiologist Copilot: An Agentic Assistant with Orchestrated Tools for Radiology Reporting with Quality Control](https://arxiv.org/abs/2512.02814)
*Yongrui Yu,Zhongzhen Huang,Linjie Mu,Shaoting Zhang,Xiaofan Zhang*

Main category: cs.AI

TL;DR: Radiologist Copilot：一个基于大语言模型的智能助手，通过编排工具实现自动化放射学报告生成与质量控制，超越现有方法
...
Method: 以大语言模型为推理核心，构建智能代理系统，自主选择工具、规划和执行动作，模拟放射科医生完整工作流程。编排工具包括：区域定位、图像引导的区域分析规划、战略模板选择、质量评估和反馈驱动的自适应优化

Result: 实验结果表明，Radiologist Copilot在放射学报告生成方面显著优于其他最先进方法

Conclusion: Radiologist Copilot能够促进准确、完整、高效的放射学报告撰写，辅助放射科医生并提高临床效率
...
### [233] [The future of AI in critical mineral exploration](https://arxiv.org/abs/2512.02879)
*Jef Caers*
...
Method: 基于贝叶斯主义和证伪原则的哲学方法，将数据获取视为证伪人类假设的手段，使用可验证指标进行理性决策，需要新型无监督学习和人机协同AI算法

Result: 提供可在任何勘探活动中使用的实用协议模板，通过AI辅助减少认知偏差和假阳性

Conclusion: AI是实现矿产勘探严谨科学方法的关键推动者，通过新科学方法可提高勘探效率并降低成本
...
### [234] [Martingale Score: An Unsupervised Metric for Bayesian Rationality in LLM Reasoning](https://arxiv.org/abs/2512.02914)
*Zhonghao He,Tianyi Qiu,Hirokazu Shirado,Maarten Sap*

Main category: cs.AI

TL;DR: 该研究提出了一种基于鞅性质的无监督评估框架，用于检测LLM推理中的信念固化现象，发现迭代推理可能强化确认偏误而非促进真相寻求。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM推理能力有所提升，但新兴证据表明迭代推理可能导致信念固化和确认偏误，而非增强真相寻求行为。需要系统评估LLM推理中的信念固化问题。

Method: 利用贝叶斯统计中的鞅性质，提出无监督、基于回归的鞅分数来衡量违反该性质的程度。在事件预测、价值负载问题和学术论文评审等开放领域进行系统性评估。

Result: 研究发现违反鞅性质的现象在模型和设置中普遍存在，当前信念正向预测未来信念更新，即信念固化现象。识别了更容易出现信念固化的模型、推理技术和领域。

Conclusion: 鞅分数在可获得真实标签的问题领域能够预测真实准确性，表明其作为无监督指标可以有效衡量推理过程的真相寻求能力，为评估LLM推理偏差提供了有用工具。
...
### [235] [Invasive Context Engineering to Control Large Language Models](https://arxiv.org/abs/2512.03001)
*Thomas Rivasseau*
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM安全方法（偏好训练、提示工程、输入输出过滤）在长上下文场景下效果有限，越狱概率随上下文长度增加而上升，需要更鲁棒的安全保障机制

Method: 侵入式上下文工程：在LLM上下文中插入控制语句，作为上下文工程的一种形式；该方法可推广到思维链过程以防止策略性行为；不依赖LLM训练，避免了长上下文场景下的数据短缺问题

Result: 该方法部分解决了长上下文场景下LLM的安全问题，提供了不依赖训练的安全增强方案

Conclusion: 侵入式上下文工程是一种有前景的LLM安全增强技术，特别适用于长上下文场景，能够在不依赖训练的情况下提高模型对恶意攻击的抵抗力
...
### [236] [From Moderation to Mediation: Can LLMs Serve as Mediators in Online Flame Wars?](https://arxiv.org/abs/2512.03005)
*Dawei Li,Abdullah Alnaibari,Arslan Bisharat,Manny Sandoval,Deborah Hall,Yasin Silva,Huan Liu*

Main category: cs.AI

TL;DR: 该研究探索LLMs能否作为在线冲突调解者，而不仅仅是内容审核者，通过判断和引导两个子任务，在Reddit数据集上评估发现API模型优于开源模型。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs越来越多地介入在线交流，它们促进同理心和建设性对话的潜力成为负责任AI研究的重要前沿。研究探索LLMs能否不仅作为检测有害内容的审核者，还能作为理解和缓解在线冲突的调解者。

Method: 将调解分解为两个子任务：判断（LLM评估对话的公平性和情感动态）和引导（生成共情、缓解紧张的信息指导参与者达成解决方案）。构建大型Reddit数据集，提出结合原则评分、用户模拟和人工比较的多阶段评估流程。

Result: 实验表明，API基础模型在调解任务中的推理和干预对齐方面优于开源模型。研究揭示了当前LLMs作为在线社交调解新兴代理的潜力和局限性。

Conclusion: LLMs在在线社交调解方面展现出潜力，但当前模型仍有局限性。API模型在调解能力上优于开源模型，为AI促进建设性对话提供了新的研究方向。
