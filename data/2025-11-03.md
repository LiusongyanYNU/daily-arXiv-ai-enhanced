...
TL;DR: 提出了MeasureBench基准测试，用于评估视觉语言模型在读取测量仪器方面的能力，发现现有模型在指针定位和空间感知方面存在显著困难。


<details>
  <summary>Details</summary>
Motivation: 人类读取测量仪器相对简单且不需要太多专业知识，但现有视觉语言模型在这方面表现不佳，需要专门的基准测试来评估和改进这一能力。

Method: 开发了MeasureBench基准测试，包含真实世界和合成图像，并建立了可扩展的数据合成流程，能够程序化生成具有可控视觉外观的测量仪器。

Result: 评估显示即使是前沿的视觉语言模型在测量读取方面也普遍表现不佳，主要失败模式是指针定位错误，导致数值误差。在合成数据上的强化学习实验显示出一定效果，但对真实图像效果有限。

Conclusion: 当前视觉语言模型在细粒度空间定位方面存在根本性限制，该资源有助于推动视觉基础数学能力和精确空间感知的发展。
...
TL;DR: 提出了HiGS分层生成框架，通过多步关联语义空间组合实现3D场景生成，从全局到局部逐步构建场景，让用户通过选择关键语义对象迭代扩展场景。


<details>
  <summary>Details</summary>
Motivation: 现有方法大多采用单步生成过程，难以在场景复杂度和用户输入之间取得平衡。受人类认知过程启发，从全局到局部、关注关键元素、通过语义关联完成场景。
...
TL;DR: 开发了一个用于多机构定量CT的领域自适应transformer分割框架，通过对抗对齐和统计对齐解决跨机构数据分布差异问题，实现稳定的股骨近端分割。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在不同机构的CT数据集上存在领域偏移问题，导致分割性能不稳定，影响骨质疏松研究和定量分析的跨中心可重复性。

Method: 基于3D TransUNet架构，集成梯度反转层的对抗对齐和最大均值差异的统计对齐，实现领域不变特征学习。

Result: 在包含1408个CT扫描的大型数据集上训练和验证，模型能够有效应对不同扫描仪和重建设置的差异。

Conclusion: 提出的双机制领域自适应方法能够平衡不变性和细粒度对齐，实现扫描仪无关的特征学习并保留解剖细节。
...
TL;DR: 提出了一种名为Referee的参考感知音视频深度伪造检测方法，利用单样本中的说话人特定线索来检测超越时空伪影的篡改操作。
...
TL;DR: 提出了一种基于方向一致性的自适应密度控制方法DC4GS，通过梯度角度一致性来优化3D高斯泼溅中的基元分割，减少冗余分割并提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 传统自适应密度控制方法仅基于位置梯度大小进行基元分割，忽略了梯度的方向信息，导致冗余分割和次优的分割位置。

Method: 将梯度方向一致性引入自适应密度控制，通过梯度角度一致性来捕捉局部结构复杂性，避免不必要分割；在需要分割时，利用方向一致性定义最优分割位置，使子基元更好地对齐局部结构。

Result: 相比现有方法，DC4GS显著减少了基元数量（实验中最多减少30%），同时大幅提升了重建保真度。

Conclusion: 方向一致性驱动的自适应密度控制能有效优化3D高斯泼溅的性能，在减少计算资源的同时提高重建质量。
...
TL;DR: 提出SACL（尺度自适应课程学习）策略，通过动态调整课程设计来应对临床环境中肺结节检测数据稀缺的问题，在有限数据条件下显著提升检测性能。
...
Result: 在LUNA25数据集上，SACL在完整数据集上mAP50与静态课程学习相当，但在10%、20%、50%训练数据条件下分别比基线提升4.6%、3.5%和2.0%。

Conclusion: SACL能够在不同数据规模下实现鲁棒训练，无需架构修改，为医疗机构在有限标注资源下开发有效肺结节检测系统提供实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决当前深度学习模型在脑病变分割中缺乏泛化能力和性能方差大的问题，提高临床可靠性。

Method: 采用混合架构：多流CNN编码器、Swin Transformer瓶颈、动态跨模态注意力融合机制和分层门控解码器，结合方差减少策略训练。

Result: 在WMH数据集DSC达0.831，HD95为3.03；ISLES 2022边界精度最佳；BraTS 2020肿瘤核心区域DSC最高(0.8651)。
...
Conclusion: SFAT模型通过语义帧聚合有效提升了直播视频评论生成的上下文相关性，为多模态评论生成提供了新思路。
...
Method: 采用混合专家架构，动态选择专家，有效利用多尺度视觉特征并结合文本嵌入，针对医学影像的复杂性进行优化。

Result: 在包含3,410个CT扫描的10个数据集上表现出色，在综合医学影像分割基准测试中展示了强大的性能，在多个数据集上达到竞争性精度。
...
TL;DR: 提出了一种无需示例的增量关系蒸馏框架，用于解决动态环境中的人-物交互检测问题，通过解耦对象和关系学习来缓解灾难性遗忘和交互漂移。


<details>
  <summary>Details</summary>
Motivation: 在开放世界中，人-物交互持续演化，传统封闭世界检测模型难以适应。受人类渐进学习能力启发，探索增量HOI检测以应对动态环境中的交互漂移和零样本HOI组合检测挑战。

Method: 提出增量关系蒸馏框架，解耦对象和关系学习，引入两种独特的蒸馏损失来学习跨不同HOI组合的恒定关系特征。

Result: 在HICO-DET和V-COCO数据集上的实验表明，该方法在缓解遗忘、增强对交互漂移的鲁棒性以及零样本HOI泛化方面优于现有基线方法。

Conclusion: 该框架有效解决了增量HOI检测中的关键挑战，为动态环境中的关系学习提供了可行方案。
...
TL;DR: VitalLens 2.0是一个新的深度学习模型，通过面部视频准确估计生理信号，在远程光电容积描记术(rPPG)方面实现了显著进步，能够稳健估计心率、呼吸率和心率变异性指标。


<details>
  <summary>Details</summary>
Motivation: 开发一个更准确的远程生理信号监测模型，通过面部视频非接触式地估计多种生理参数，为健康监测提供便利。

Method: 结合新的模型架构和大幅增加训练数据规模（1,413个独特个体），使用深度学习技术从面部视频中提取生理信号。

Result: 在422个独特个体的测试集上，VitalLens 2.0实现了优异性能：心率MAE为1.57 bpm，呼吸率MAE为1.08 bpm，HRV-SDNN为10.18 ms，HRV-RMSSD为16.45 ms，显著优于先前方法。

Conclusion: VitalLens 2.0在远程生理信号估计方面达到了新的最先进水平，现已通过API向开发者开放，为非接触式健康监测提供了可靠工具。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶场景具有复杂的空间和几何结构，需要专门优化的分割模型来处理道路场景的独特挑战。

Method: 采用双编码器架构结合SAM的ViT-H全局语义和ResNet-50局部细节，使用可变形融合模块对齐异构特征，通过可变形注意力进行多阶段细化，并采用混合损失函数优化训练。
...
TL;DR: HiT是一种新颖的分层神经场表示方法，用于3D形状的无监督分层学习，通过分层变压器自动识别跨类别的共同子结构。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常局限于固定的分层结构（如二叉树），无法灵活地从数据中推断复杂的分层关系，特别是在跨多个形状类别时。

Method: 使用分层变压器（HiT）和压缩码本，在无监督设置下学习树形层次结构中的父子关系，不限制具体分层结构，只限制每层节点总数。

Result: 通过重建损失训练，模型成功捕获有意义的父子节点包含关系，在ShapeNet的55个类别上实现了多粒度形状分割。

Conclusion: HiT能够从数据中自动推断复杂的分层结构，比现有方法更具通用性，适用于跨类别的无监督形状分析。
...
TL;DR: ZEBRA是首个零样本脑视觉解码框架，无需受试者特定适应即可从fMRI重建视觉体验，通过对抗训练分离受试者相关和语义相关表示。


<details>
  <summary>Details</summary>
Motivation: 当前方法依赖受试者特定模型或微调，限制了可扩展性和实际应用，需要开发无需受试者特定适应的通用解码框架。

Method: 利用对抗训练将fMRI表示分解为受试者相关和语义相关组件，分离出受试者不变、语义特定的表示。

Result: ZEBRA显著优于零样本基线，在多个指标上达到与完全微调模型相当的性能，可泛化到未见过的受试者。
...
TL;DR: 本文提出了WildfireX-SLAM数据集，这是一个用于森林和野火环境中SLAM任务的大规模合成数据集，包含5.5k张低空RGB-D航空图像，覆盖16平方公里区域。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS-based SLAM研究主要关注小规模室内场景，而开发适用于大规模森林场景的SLAM方法对于野火应急响应和森林管理等实际应用具有重要价值，但缺乏高质量数据集阻碍了相关研究。

Method: 利用Unreal Engine 5 Electric Dreams环境样本项目，开发了一个数据收集管道，可以轻松获取航空和地面视图，包括真实相机位姿和多种数据模态，并能灵活控制光照、天气、野火类型等环境因素。

Result: 构建了WildfireX-SLAM数据集，包含5.5k张低空RGB-D航空图像，覆盖16平方公里森林区域，并进行了全面的基准测试，揭示了森林环境中3DGS-based SLAM面临的独特挑战。

Conclusion: 该数据集填补了森林SLAM研究的数据空白，为未来在森林测绘、野火应急响应等领域的3DGS-based SLAM研究提供了重要基础，数据集和代码将公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型需要大规模训练数据和大量计算资源，或者结构复杂、延迟高，因此需要开发更高效的模型。

Method: 采用高度压缩的视觉tokenizer和新型多路径压缩模块减少token数量，引入位置强化和交替子区域注意力来降低计算成本，并提出AdaLN-affine高效模块。
...
TL;DR: 提出CVCAM和MHSAM模块，通过跨视图交互和多尺度空间特征提取，提升跨视图物体地理定位性能，并在新构建的G2D数据集上验证有效性。
...
Method: 引入跨视图交叉注意力模块(CVCAM)进行多轮视图间交互，集成多头空间注意力模块(MHSAM)提取多尺度空间特征，并构建G2D数据集填补地面到无人机定位任务空白。

Result: 在CVOGL和G2D数据集上的大量实验表明，该方法实现了高定位精度，超越了当前最先进方法。

Conclusion: 提出的CVCAM和MHSAM模块能有效提升跨视图物体地理定位性能，通过视图间信息交互和多尺度特征提取解决了现有方法的局限性。
...
TL;DR: 提出了AFM-Net，一种先进的层次融合框架，通过CNN分支提取层次视觉先验和Mamba分支进行高效全局序列建模，有效整合局部和全局特征，在遥感图像场景分类任务中取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 遥感图像场景分类面临空间结构复杂和多尺度特征的挑战，现有方法中CNN擅长建模局部纹理而Transformer擅长捕捉全局上下文，但Transformer的高计算成本限制了其高效集成。

Method: AFM-Net采用双路径设计：CNN分支提取层次视觉先验，Mamba分支进行高效全局序列建模；通过层次融合机制渐进聚合多尺度特征，实现动态跨层特征交互和上下文重建；最后通过混合专家分类器模块自适应路由特征。

Result: 在AID、NWPU-RESISC45和UC Merced数据集上分别达到93.72%、95.54%和96.92%的准确率，超越了现有最先进方法，在性能和效率之间取得良好平衡。

Conclusion: AFM-Net通过有效整合CNN和Mamba的优势，实现了遥感图像场景分类中局部和全局特征的协同表示，在多个数据集上取得了优越的分类性能。
...
TL;DR: 本研究探索使用深度学习模型通过模块化规则框架来近似Banff肾脏移植活检评分系统的可行性，发现当前AI流程在复制专家级评分方面存在局限性。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在预训练时通常使用低分辨率图像，当处理高分辨率图像时会丢失视觉细节和遗漏重要物体，导致生成的描述不够准确和详细。

Method: 采用多阶段流程：首先用VLM生成初始描述，然后用LLM识别关键物体并预测可能共现的物体，通过物体检测系统验证预测，对未提及的新检测物体进行区域特定描述，最后移除未检测物体的引用。

Result: 在精选的高分辨率图像数据集上的实验表明，该流程能生成更详细可靠的图像描述，同时有效减少幻觉。

Conclusion: 提出的多阶段流程通过结合多种模型和验证机制，显著提升了高分辨率图像描述的质量和可靠性。
...
TL;DR: M^3Detection是一个统一的多帧3D目标检测框架，通过相机和4D成像雷达的多模态数据融合，实现高效的多级特征融合，提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有相机-雷达融合方法多为单帧输入，场景信息不完整，受图像质量下降和4D雷达稀疏性影响，检测性能受限。多帧融合能提供更丰富的时空信息，但面临跨帧跨模态特征融合和计算成本高的挑战。

Method: 利用基线检测器的中间特征和跟踪器生成的参考轨迹，设计全局级对象间特征聚合模块和局部级网格间特征聚合模块，通过轨迹级多帧时空推理模块编码跨帧交互。

Result: 在VoD和TJ4DRadSet数据集上的实验表明，M^3Detection实现了最先进的3D检测性能。

Conclusion: 该方法在多帧相机-4D成像雷达融合检测中具有有效性，通过多级特征融合和时空推理显著提升了检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 视频生成对质量和连续性要求高，特别是涉及人体舞蹈的内容由于动作自由度大而更具挑战性，需要专门的方法来提升生成效果。

Method: 基于稳定视频扩散模型，引入外观增强模块(AEM)聚焦参考图像细节，姿态渲染模块(PRM)扩展运动引导，并构建TikTok-3K数据集增强训练。

Result: 在真实世界数据集上的广泛实验表明，该方法性能优于现有最先进方法。

Conclusion: DANCER框架通过条件增强和渲染机制有效提升了舞蹈视频生成的质量和连续性，为人体动作生成提供了有效解决方案。
...
TL;DR: 提出H2-Cache层次化缓存机制，通过双阈值系统分别处理结构定义和细节精炼阶段，使用池化特征摘要技术实现高效相似性估计，在保持图像质量的同时实现高达5.08倍的加速。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成中表现出色，但迭代去噪过程计算成本高。现有缓存技术存在速度与保真度之间的权衡问题，导致质量下降和计算开销大。

Method: 提出H2-Cache层次化缓存机制，基于去噪过程可功能分离为结构定义和细节精炼阶段的关键洞察，采用双阈值系统选择性缓存各阶段，并引入池化特征摘要技术进行快速相似性估计。

Result: 在Flux架构上的实验表明，H2-Cache实现显著加速（最高5.08倍），同时保持与基线几乎相同的图像质量，在定量和定性评估上都优于现有缓存方法。

Conclusion: H2-Cache提供了稳健实用的解决方案，有效解决了速度-质量困境，显著降低了高保真扩散模型在实际应用中的门槛。
...
TL;DR: SilhouetteTell是一种新颖的视频识别攻击方法，通过分析字幕轮廓的空间和时间特征来识别视频内容，能够推断在线和离线视频，攻击距离可达40米。


<details>
  <summary>Details</summary>
Motivation: 视频识别攻击对隐私构成严重威胁，可能泄露用户的观看历史，暴露其爱好、宗教信仰、政治倾向、性取向和健康状况等敏感信息，可能导致网络欺凌、歧视或敲诈勒索。

Method: 通过观察字幕内容决定其在屏幕上显示的轮廓，识别每个字幕轮廓并推导连续字幕之间的时间差，将空间和时间域信息结合为字幕轮廓的时空特征，探索记录的字幕轮廓与字幕文件之间的时空相关性。

Result: 在现成智能手机上的综合实验证实了SilhouetteTell在各种设置下推断视频标题和片段的极高有效性，包括从40米距离的攻击。

Conclusion: SilhouetteTell是一种有效的视频识别攻击方法，能够利用字幕轮廓的时空特征准确识别视频内容，对用户隐私构成严重威胁。
...
<details>
  <summary>Details</summary>
Motivation: 现有挖掘或重加权策略使用静态权重，对分布变化敏感，且容易过早过度强调困难样本，导致噪声梯度和不稳定收敛。

Method: 在样本级别使用基于比率的难度感知（RDA）模块评估相对难度并分配细粒度权重；在批次级别使用渐进式自适应损失加权（PALW）机制，利用训练进度信号在早期优化中衰减噪声梯度，并随着训练成熟逐步增强困难负样本挖掘。

Result: 在University-1652和SUES-200基准测试上验证了DPHR的有效性和鲁棒性，相比最先进方法取得了持续改进。

Conclusion: DPHR策略能够有效解决跨视角地理定位中的困难负样本问题，提高模型性能和训练稳定性。
...
TL;DR: 提出了一种稀疏模型反演策略，通过选择性反演语义前景来加速高分辨率图像的反演过程，避免噪声背景和虚假相关的反演。
...
Method: 提出稀疏模型反演策略，作为即插即用扩展，选择性反演语义前景，停止噪声背景和潜在虚假相关的反演，无需修改原有损失函数。

Result: 实现了显著的反演加速（最高3.79倍），在数据无关模型量化和数据无关知识迁移中保持或提升了下游性能。

Conclusion: 稀疏模型反演策略有效解决了高分辨率图像反演效率问题，同时维持了良好的下游任务性能。
...
TL;DR: 提出了多模态交互式真实性评估任务(MIVA)，基于狼人杀游戏构建了包含同步视频、文本和真实标签的数据集，评估发现现有MLLMs在辨别真假方面表现不佳。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统日益融入人类生活，赋予其强大的社会智能至关重要。辨别真假是人类互动中的关键能力，但目前多模态对话中的自动欺骗检测仍面临挑战。

Method: 基于狼人杀游戏构建了新的多模态数据集，包含同步视频和文本数据，并建立了基准测试来评估最先进的多模态大语言模型。

Result: 评估显示即使像GPT-4o这样的强大模型也难以可靠地区分真假，模型未能有效将语言与视觉社交线索相结合，且可能过于保守。

Conclusion: 现有MLLMs在多模态欺骗检测方面存在显著性能差距，迫切需要新方法来构建更具洞察力和可信度的AI系统。
...
<details>
  <summary>Details</summary>
Motivation: 随着城市化进程，村庄空间特征逐渐消失和景观同质化问题突出。现有研究多采用单一学科视角，依赖定性分析方法，受限于数字基础设施不足和数据缺乏。

Method: 提出层次图神经网络模型，包含输入节点和通信节点两种节点，静态输入边和动态通信边两种边。结合图卷积网络和图注意力网络，通过两阶段特征更新机制高效整合多模态特征，并引入关系池化机制和17个子类型的联合训练策略。
...
TL;DR: 提出了一种新颖的持续自监督学习框架，用于从多窗口胸部CT图像中学习多样化特征，同时确保数据隐私。通过潜在重放机制和特征蒸馏技术，有效缓解领域偏移和灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像诊断中构建稳健且泛化性强的模型面临挑战，主要由于大规模准确标注数据稀缺以及医疗环境中固有的领域偏移问题。在胸部CT中，领域偏移通常源于针对不同临床目的优化的窗口设置差异。

Method: 采用持续预训练方法，结合潜在重放机制缓解灾难性遗忘，同时引入基于Wasserstein距离的知识蒸馏和批量知识集成的特征蒸馏技术，学习具有领域偏移鲁棒性的表征。

Result: 在两个不同窗口设置的胸部CT图像上验证了该方法，相比其他方法表现出更优越的性能。

Conclusion: 该框架能够有效学习多样化特征，缓解领域偏移问题，同时确保数据隐私，为医学图像诊断提供了实用的解决方案。
...
TL;DR: 提出了SpecAware，一种新颖的高光谱光谱内容感知基础模型，用于统一多传感器HSI映射学习。该模型通过超网络驱动的编码过程，结合传感器元属性和图像内容，能够自适应处理可变数量的光谱通道。


<details>
  <summary>Details</summary>
Motivation: 高光谱成像(HSI)是精细土地利用和土地覆盖(LULC)测绘的重要工具，但HSI数据的固有异质性阻碍了通过联合训练开发通用模型。现有HSI基础模型通常忽视传感器元属性的关键指导作用，难以进行多传感器训练，限制了其可迁移性。

Method: SpecAware采用两步超网络驱动编码过程：1) 元内容感知模块，融合传感器元属性和图像内容为每个HSI样本生成条件输入；2) HyperEmbedding模块，通过样本条件超网络动态生成矩阵因子对，进行通道级编码，包括自适应空间模式提取和潜在语义特征重投影。

Result: 在六个数据集上的广泛实验表明，SpecAware能够学习到优越的特征表示，在土地覆盖语义分割分类、变化检测和场景分类任务中表现出色。

Conclusion: SpecAware能够感知和解释不同场景和传感器的空间-光谱特征，建立了统一的联合预训练框架，为多传感器HSI学习提供了有效解决方案。
...
TL;DR: YOLOv11在卫星图像上实现了建筑物实例分割和高度分类，在DFC2023数据集上表现出色，在检测精度和推理速度方面优于早期多任务框架。
...
Method: 使用YOLOv11深度学习模型，在DFC2023 Track 2数据集（包含12个城市的125,000多个标注建筑物）上进行评估，采用精度、召回率、F1分数和mAP等指标。

Result: YOLOv11实现了60.4% mAP@50和38.3% mAP@50-95的实例分割性能，在五个预定义高度层级上保持稳健的分类准确率，特别擅长处理遮挡、复杂建筑形状和类别不平衡问题。

Conclusion: YOLOv11通过简化的分类高度建模推进了语义城市重建，为遥感和地理空间智能的未来发展提供了可行见解，适合实时大规模城市测绘。
...
Method: 采用专家混合架构动态路由特征，包含基于置信度的深度细化模块、密集语义特征与全局对齐3D骨干表示集成，以及定制损失函数。

Result: 在多个基准测试中达到最先进性能，支持有效下游应用且无需额外计算。

Conclusion: MoRE通过MoE架构成功解决了3D模型扩展的挑战，在保持高性能的同时支持多样化几何任务。
...
Result: 在RetargetMe基准测试中表现优异，定量指标和主观视觉质量评估均优于现有方法，处理1024x683分辨率图像平均推理时间仅0.009秒。
...
TL;DR: 提出了FuseCPath框架，用于融合异构病理基础模型，通过多视图聚类、聚类级重嵌入和协作蒸馏策略，在多种癌症数据集上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前病理基础模型存在显著的异构性，这源于不同的私有训练数据集和网络架构，导致在下游任务中使用不同模型提取的特征时出现性能差异。

Method: 1) 多视图聚类方法筛选有区分度的图像块；2) 聚类级重嵌入策略在线捕获局部特征；3) 协作蒸馏策略探索模型间连接。
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络在安全关键系统中易受对抗攻击，限制了其应用。研究发现攻击图像的高频分量受损更严重，需要开发有效的防御方法。
...
Conclusion: 提出的两阶段训练方法通过空间-频域融合去噪有效提升了深度神经网络对对抗攻击的鲁棒性，在多个数据集上取得了优异性能。
...
TL;DR: 提出了一种利用对比学习进行对抗防御的新方法，通过对比损失函数训练模型，使其对对抗性扰动具有更强的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络容易受到对抗攻击的影响，在输入图像中加入微小扰动就能导致错误预测，这阻碍了鲁棒深度学习系统的部署。

Method: 使用对比损失函数，同时使用干净图像和对抗性扰动图像训练分类模型，通过优化模型参数和扰动，让网络学习更鲁棒的表示。
...
TL;DR: 提出利用网络爬取的描述和大语言模型提取关键词的方法，解决视觉语言模型在零样本动作识别中因多语义词导致的歧义问题，无需人工标注。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在零样本动作识别中仅依赖动作类别提供语义上下文时，由于多语义词的存在会导致概念理解歧义，需要解决这一问题。

Method: 利用网络爬取的描述，通过大语言模型提取相关关键词；引入时空交互模块关注对象和动作单元，促进描述属性与视频内容的对齐。

Result: 在零样本实验中，模型在UCF-101、HMDB-51和Kinetics-600数据集上分别达到81.0%、53.1%和68.9%的准确率。

Conclusion: 该方法展现了模型在各种下游任务中的适应性和有效性，显著提升了零样本动作识别性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态RAG方法将整个文档作为检索单元，导致大量无关视觉内容：1)相关文档中包含与查询无关的大区域；2)检索多个文档引入冗余文档。这些冗余内容分散模型注意力，降低性能。

Method: 提出从文档级到区域级的检索范式转变。训练时采用混合监督策略从标注和未标注数据中精确定位相关图像块；推理时通过动态管道将显著图像块分组为完整语义区域。
...
TL;DR: 提出T^3框架，通过Jensen-Shannon散度动态计算模型融合系数，在医学影像中实现预训练通用性和微调专业性的自适应平衡。


<details>
  <summary>Details</summary>
Motivation: 解决医学影像中预训练模型缺乏模态特异性、微调模型在模态偏移下表现不佳的问题，现有模型融合方法在医学领域效果不稳定。

Method: 提出无需反向传播的测试时任务自适应融合框架，基于模型输出分布的Jensen-Shannon散度计算逐样本融合系数，并扩展为批处理版本T^3_B降低计算成本。

Result: 在跨四个模态的严格评估中，T^3在Top-1准确率和错误率降低方面达到新SOTA，优于强基线方法。

Conclusion: T^3为临床环境中的自适应医学视觉语言模型部署提供了有效解决方案，在保持效率的同时显著提升性能。
...
TL;DR: HyperClick是一个通过不确定性校准增强GUI代理可靠性的框架，通过双奖励机制和空间置信度建模，联合优化定位精度和置信度可靠性。
...
Method: 提出HyperClick框架，引入双奖励机制：正确动作的二元奖励和基于截断高斯分布的空间置信度建模，使用Brier分数进行校准，联合优化定位精度和置信度可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么均匀采样，要么使用小型视觉语言模型进行检索式关键帧选择，这些方法依赖预过滤来降低推理成本，可能会错过最具信息量的时刻。

Method: 将关键帧选择建模为多臂老虎机中的组合纯探索问题，将短时间片段视为臂，使用经验均值和Bernstein置信半径来识别信息丰富区域，同时保留对不确定区域的探索。
...
TL;DR: 本文提出了S-GRACE方法，通过语义引导在概念空间中生成对抗样本进行概念擦除训练，显著提升了扩散模型中的概念擦除效果。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法在扩散模型中采用对抗训练来抑制目标概念，但忽视了对抗训练在扩散模型中的特殊性，导致只能部分缓解敏感内容生成问题。

Method: S-GRACE方法利用概念空间中的语义指导来生成对抗样本并进行擦除训练，解决了现有方法在生成对抗样本时忽视概念语义的问题。

Result: 实验表明，S-GRACE在多种扩散模型去学习场景下，相比七种最先进方法，擦除性能提升26%，更好地保护非目标概念，训练时间减少90%。

Conclusion: S-GRACE通过语义引导的对抗样本生成策略，有效解决了扩散模型概念擦除中的关键问题，显著提升了擦除效果和训练效率。
...
TL;DR: FGMamba是一种新型的频率感知门控状态空间模型，用于医学图像超分辨率，通过结合全局依赖建模和精细细节增强，在保持轻量级架构的同时实现卓越性能。
...
Method: 提出了两个关键创新：门控注意力增强状态空间模块（GASM），将高效状态空间建模与双分支空间和通道注意力相结合；金字塔频率融合模块（PFFM），通过FFT引导融合在多个分辨率下捕获高频细节。

Result: 在五种医学成像模态（超声、OCT、MRI、CT和内窥镜）上的广泛评估表明，FGMamba实现了卓越的PSNR/SSIM，同时保持紧凑的参数占用（<0.75M），优于基于CNN和Transformer的最先进方法。
...
TL;DR: CASR-Net是一个用于冠状动脉分割的三阶段管道，通过创新的多通道预处理、基于UNet的分割网络和轮廓细化模块，在冠状动脉疾病早期检测中取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉疾病的早期检测对降低死亡率和改善治疗规划至关重要。虽然X射线血管造影图像分析是识别心脏异常（包括狭窄冠状动脉）的常用且经济有效的方法，但图像质量差会显著阻碍临床诊断。

Method: 提出三阶段管道：图像预处理、分割和细化。采用结合CLAHE和改进Ben Graham方法的多通道预处理策略；核心分割网络基于UNet架构，使用DenseNet121编码器和Self-ONN解码器；最后通过轮廓细化模块抑制假阳性。

Result: 在两个包含健康和狭窄动脉的公共数据集上通过5折交叉验证评估，CASR-Net优于多个最先进模型，达到IoU 61.43%、DSC 76.10%和clDice 79.36%的性能。

Conclusion: CASR-Net为自动冠状动脉分割提供了一种稳健方法，为临床医生的诊断和治疗规划提供了有价值的工具。
...
TL;DR: 本文提出P²IOD方法，通过参数化提示来解决增量目标检测中的类别共现问题，避免传统提示池方法在检测图像中出现的混淆问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于提示池的增量学习方法假设增量任务间的类别集是互斥的，这不符合目标检测中常见的类别共现现象。当先前任务的未标记对象出现在当前任务图像中时，会导致提示池混淆。

Method: P²IOD使用神经网络作为参数化提示，利用神经网络的全局演化特性自适应地整合跨任务知识。通过参数化提示融合策略来约束提示结构的更新。

Result: 在PASCAL VOC2007和MS COCO数据集上的大量实验表明，P²IOD在增量目标检测中表现有效，并在现有基线中达到了最先进的性能。

Conclusion: 参数化提示结构应具有跨任务的自适应整合特性，通过约束更新来防止灾难性遗忘，P²IOD方法成功解决了增量目标检测中的类别共现挑战。
...
TL;DR: 提出SAGS框架，一种自适应的无混叠高斯溅射方法，用于内窥镜视频中可变形组织的重建，通过注意力驱动的4D变形解码器和多尺度滤波技术解决组织运动引起的混叠和伪影问题。


<details>
  <summary>Details</summary>
Motivation: 内窥镜视频中可变形组织重建面临组织运动引起的混叠和伪影问题，现有3D高斯溅射方法虽然提高了渲染效率，但忽视了这些关键问题，导致可视化质量下降。

Method: 提出SAGS框架，包含注意力驱动的动态加权4D变形解码器，结合3D平滑滤波器和2D Mip滤波器来减轻可变形组织重建中的伪影，并更好地捕捉组织运动的精细细节。

Result: 在EndoNeRF和SCARED两个公开基准测试中，该方法在PSNR、SSIM和LPIPS所有指标上均优于现有技术，同时提供更好的可视化质量。

Conclusion: SAGS框架成功解决了内窥镜场景中可变形组织重建的混叠和伪影问题，在保持高效率的同时实现了更高质量的重建结果。
...
TL;DR: 提出一种超低比特率视觉通信方法，结合文本生成和深度图像压缩，在极低带宽下实现高质量图像重建和视觉分析。
...
Method: 将图像生成与深度图像压缩无缝集成，使用联合文本和编码潜在向量引导校正流模型，实现视觉场景的精确生成。
...
Method: 采用迭代开发过程，包括概念设计、实验、评估和优化，最终形成基于分类的解决方案。

Result: 提出了完整的实现方案并公开发布在GitHub上，专注于性能、稳健性和临床相关性。

Conclusion: 基于分类的方法在乳腺MRI早期癌症检测中具有重要价值，特别是在标签稀缺的情况下。
...
TL;DR: 提出CIELR方法，通过LLM推理将复杂图像编辑指令分解为简单明确的编辑动作，避免联合微调LLM和扩散模型的高计算成本
...
Conclusion: CIELR方法通过LLM推理有效处理复杂图像编辑指令，无需联合微调，在保持图像一致性方面表现优异
...
<details>
  <summary>Details</summary>
Motivation: 解决现有参数高效微调方法的问题：基于加法的方法引入推理延迟和工程复杂性，基于选择的方法需要完整反向传播导致内存使用与全微调相同。

Method: FPS通过参数幅度与对应输入激活的乘积来对参数进行排序，利用预训练知识和下游数据，在单次前向传播中完成参数选择。

Result: 在24个视觉任务上评估，FPS达到最先进方法相当的性能，同时将峰值内存使用降低近9倍，参数选择速度提升约2倍。
...
TL;DR: 提出一个两阶段微调流程，使用LoRA技术在小数据集上训练开源视频扩散模型，实现电视电影制作中的电影级场景合成。


<details>
  <summary>Details</summary>
Motivation: 解决从少量数据中高效学习电影视觉风格并生成连贯视频序列的挑战，为影视制作提供实用的AI工具。

Method: 两阶段方法：第一阶段在交叉注意力层集成LoRA模块进行视觉风格学习；第二阶段生成关键帧并通过视频解码器扩展为720p序列，采用轻量级并行化和序列分区策略加速推理。

Result: 定量评估（FVD、CLIP-SIM、LPIPS）和专家用户研究表明，相比基础模型在电影保真度和时间稳定性方面有显著提升。

Conclusion: 该流程实现了高效的领域迁移，在单GPU上几小时内完成训练，为电影制作提供了可复现的AI解决方案。
...
TL;DR: 提出Alignment across Trees方法，通过构建和对齐图像和文本的树状层次特征来解决模态对齐不对称问题，在双曲流形中建模异质层次结构，并在多个图像数据集上优于强基线。
...
Method: 构建语义感知的视觉特征提取框架，使用交叉注意力机制从中间Transformer层提取视觉特征；将两个模态的特征树嵌入具有不同曲率的双曲流形；通过最小化异质流形间的KL距离来学习中间流形进行对齐。

Result: 在多个图像数据集上的分类任务中，在少样本和跨域设置下始终优于强基线。

Conclusion: 证明了最优中间流形的存在性和唯一性，提出的方法有效解决了模态对齐不对称问题。
...
TL;DR: 提出一种融合法医特征和深度学习表示的混合框架用于深度伪造检测，在多个基准数据集上优于单一方法基线，并表现出对压缩、对抗攻击和未知操作的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 生成对抗网络和扩散模型的快速发展使得合成媒体越来越逼真，引发了关于错误信息、身份欺诈和数字信任的社会担忧。现有深度伪造检测方法要么依赖深度学习但泛化能力差且易受失真影响，要么使用可解释但对抗新操作技术有限的法医分析。

Method: 提出混合框架，融合噪声残差、JPEG压缩痕迹和频域描述符等法医特征，与卷积神经网络和视觉变换器的深度学习表示相结合。

Result: 在FaceForensics++、Celeb-DF v2和DFDC数据集上分别获得0.96、0.82和0.77的F1分数。在压缩（QF=50时F1=0.87）、对抗扰动（AUC=0.84）和未知操作（F1=0.79）下表现出稳定的鲁棒性。可解释性分析显示82%情况下Grad-CAM和法医热图与真实操作区域重叠。

Conclusion: 混合方法提供了平衡解决方案，结合深度模型的适应性和法医线索的可解释性，可开发出具有弹性和可信赖的深度伪造检测系统。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型旨在改善诊断工作流程，但在图像分割等任务中的公平性评估尚未充分探索。未解决的分割偏见可能导致某些人群的护理质量差异，并在临床决策点和迭代模型开发中被放大。

Method: 作者审计了MAMA-MIA乳腺癌肿瘤分割数据集中自动分割标签的公平性，评估了跨年龄、种族和数据来源的自动分割质量。

Result: 分析揭示了针对年轻患者的内在年龄相关偏见，即使在控制数据来源等混杂因素后仍然存在。此外，聚合多个数据源会影响特定站点的种族偏见。

Conclusion: 需要在细粒度水平上调查数据，以解决分割模型中的公平性问题，特别是与年龄相关的偏见可能源于生理因素，这对放射科医生和自动化系统都是已知挑战。
...
TL;DR: 本文提出了一种解决部分相关视频检索中语义崩溃问题的新框架，通过文本相关性保持学习和跨分支视频对齐方法，有效防止文本和视频嵌入空间中的语义崩溃，显著提升了检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有的PRVR方法将所有标注的文本-视频对视为正样本，其他视为负样本，忽略了单个视频内和不同视频间的丰富语义变化。这导致同一视频中不同事件的查询和视频片段嵌入崩溃在一起，而不同视频中语义相似的查询和片段却被推远，限制了多事件视频的检索性能。

Method: 1. 文本相关性保持学习：保持基础模型编码的文本查询间的语义关系；2. 跨分支视频对齐：一种对比对齐方法，在时间尺度上解耦层次化视频表示；3. 顺序保持令牌合并和自适应CBVA：通过生成内部一致且相互区分的视频片段来增强对齐效果。

Result: 在PRVR基准测试上的广泛实验表明，该框架有效防止了语义崩溃，并显著提高了检索准确率。

Conclusion: 提出的框架成功解决了PRVR中的语义崩溃问题，通过文本和视频嵌入空间的改进方法，为多事件视频检索提供了有效的解决方案。
...
TL;DR: DeblurSDI是一个零样本自监督盲图像去模糊框架，基于自扩散过程，无需预训练即可从纯噪声开始迭代恢复清晰图像和模糊核。


<details>
  <summary>Details</summary>
Motivation: 传统盲图像去模糊方法依赖手工先验，而深度学习方法需要大量外部数据集预训练，限制了在实际场景中的适应性。需要一种无需预训练、能适应真实场景的解决方案。

Method: 将盲去模糊建模为迭代反向自扩散过程，从纯噪声开始逐步优化。使用两个随机初始化的神经网络持续优化清晰图像和模糊核，结合数据一致性和L1稀疏约束，采用噪声调度机制稳定优化过程。

Result: 实验表明DeblurSDI在高度退化场景下仍能恢复清晰图像和准确模糊核，性能优于现有方法。

Conclusion: DeblurSDI通过自扩散过程实现了零样本盲图像去模糊，无需预训练即可动态学习实例特定先验，对模糊核尺寸变化具有鲁棒性。
...
TL;DR: CoMViT是一个紧凑且可泛化的Vision Transformer架构，专为资源受限的医学图像分析优化，通过多种技术改进实现了高性能和良好泛化能力。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在医学成像中表现出潜力，但高计算需求和在小数据集上的过拟合问题限制了其在真实临床场景中的应用。

Method: CoMViT集成了卷积分词器、对角掩码、动态温度缩放和基于池化的序列聚合等技术，通过系统架构优化实现高效设计。

Result: 在12个MedMNIST数据集上实现稳健性能，仅需约450万参数，匹配或超越更深层的CNN和ViT变体，参数减少5-20倍而不牺牲准确性。

Conclusion: 结果表明，通过有原则的ViT重新设计，可以在低资源医学成像环境中开发高效且可解释的模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成模型存在两大局限：图像生成模型缺乏语义结构，无法编辑独立视觉组件；代码生成方法操作繁琐且不直观。两者都无法满足科学创作对效率、直观性和迭代修改的需求。

Method: VisPainter采用多智能体框架，包含Manager、Designer和Toolbox三个专业模块，协作生成标准矢量图形软件兼容的图表。

Result: 开发了VisBench基准测试，包含七个维度的评估指标，从内容、布局、视觉感知和交互成本四个方面评估科学插图质量。通过消融实验验证了架构合理性和评估方法可靠性。

Conclusion: VisPainter框架成功解决了科学插图生成中的元素级控制问题，提供了直观的编辑能力，并通过系统评估方法为各种视觉语言模型提供了公平可信的排名。
...
TL;DR: 提出了一种多层级人机交互框架，通过机器学习分析人口密度、土地覆盖和基础设施来识别学校数据缺口，结合高分辨率卫星影像和深度学习模型生成候选学校位置，并通过交互界面让人工验证和优化结果。


<details>
  <summary>Details</summary>
Motivation: 在发展中国家，教育设施记录往往稀缺且更新不及时，需要一种准确完整的学校地图绘制方法来支持教育规划和资源分配。

Method: 采用多层级方法：第一层用机器学习分析人口密度、土地覆盖和基础设施；第二层用高分辨率卫星影像和深度学习模型识别候选学校位置；最后通过交互界面让人工参与验证和优化。

Result: 初步评估表明，该多层级策略为教育基础设施地图绘制提供了可扩展且成本效益高的解决方案。

Conclusion: 该人机交互框架能够有效提高学校地图的准确性和完整性，特别适用于数据稀缺的发展中地区。
...
TL;DR: 该论文构建了NautData数据集（145万图像-文本对）支持8个水下场景理解任务，并提出可插拔的视觉特征增强模块VFE来提升水下场景理解的鲁棒性，最终构建了水下大语言模型NAUTILUS。


<details>
  <summary>Details</summary>
Motivation: 水下探索具有重要应用价值，但缺乏大规模水下多任务指令调优数据集，且水下图像退化问题干扰任务性能，阻碍了水下场景理解研究的发展。

Method: 构建NautData数据集，引入水下成像模型的物理先验，提出可插拔的视觉特征增强模块VFE，并将其集成到LLaVA-1.5和Qwen2.5-VL基线模型中构建NAUTILUS模型。
...
TL;DR: ThinkMorph是一个统一的多模态推理模型，通过生成渐进式的文本-图像推理步骤，在保持连贯语言逻辑的同时具体操作视觉内容，在视觉中心基准测试中取得显著提升。
...
Result: 在视觉中心基准测试中平均提升34.7%，优于基础模型，并在域外任务上达到或超越更大和专有的视觉语言模型。模型展现出新兴的多模态智能，包括未见过的视觉操作技能、自适应推理模式切换等。
...
TL;DR: vMambaX是一个轻量级多模态框架，通过上下文门控跨模态感知模块整合PET和CT扫描图像，在PCLT20K数据集上优于基线模型且计算复杂度更低。
...
TL;DR: 提出了一种基于深度学习的3D点云水印框架，通过奇异值分解将水印嵌入点云块，使用PointNet++网络进行鲁棒提取，显著优于传统SVD方法。
...
Result: 在ModelNet40数据集上验证，深度学习提取方法在裁剪(70%)攻击下达到0.83比特精度和0.80 IoU，显著优于SVD方法的0.58精度和0.26 IoU。
...
TL;DR: MapSAM2是一个基于视觉基础模型的统一框架，能够自动分割历史地图图像和时间序列，通过将地图处理为视频形式来提升分割准确性，并提出了伪时间序列生成方法来降低标注成本。


<details>
  <summary>Details</summary>
Motivation: 历史地图是记录地理特征随时间变化的宝贵档案，但由于风格多变和标注数据稀缺，其自动分析面临挑战。构建链接的时空数据集对研究建筑年代、道路网络发展和环境变化等应用至关重要。

Method: 将历史地图图像和时间序列都视为视频处理：对于单张地图，将图块作为视频帧，利用记忆注意力机制整合上下文信息；对于时间序列，提出从单年地图生成伪时间序列的方法来模拟常见的时间变换。

Result: 实验结果表明，MapSAM2能有效学习时间关联，在有限监督或使用伪视频的情况下，能准确分割和链接时间序列中的建筑物。

Conclusion: MapSAM2为历史地图分析提供了一个有效的统一框架，能够处理多样化的分割任务，并通过伪时间序列生成降低了标注成本，将发布数据集和代码以支持未来研究。
...
TL;DR: 提出了一个联合设计框架来解决视频检索中的结构不对齐问题，包括通用视频检索基准（UVRB）、可扩展的数据合成工作流和模态金字塔训练方法，实现了最先进的零样本泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频检索范式存在结构不对齐问题，狭窄的基准测试限制了数据和单任务训练，抑制了通用能力的发展，缺乏能够定义和需求多维度泛化的诊断性评估。

Method: 1) 建立包含16个数据集的通用视频检索基准（UVRB）；2) 基于UVRB诊断开发可扩展的合成工作流，生成155万高质量数据对；3) 设计模态金字塔课程，训练通用视频嵌入器（GVE）。

Result: GVE在UVRB上实现了最先进的零样本泛化性能，分析显示流行基准对通用能力的预测能力较差，部分相关检索是主导但被忽视的场景。

Conclusion: 联合设计框架为摆脱有限范围、推进真正通用视频检索提供了实用路径，揭示了现有基准的局限性并强调了多维度泛化的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模检索中高维嵌入计算成本高的问题，现有哈希方法存在复杂流程、多目标优化和长训练时间等局限性。

Method: 使用单个二进制交叉熵损失实现跨视图代码对齐，编码率最大化作为防崩溃正则化器，设计轻量级HashCoder网络，支持冻结嵌入或LoRA微调。

Result: 在16位编码下表现优异，COCO无监督哈希在2分钟内完成，ImageNet100监督哈希在3分钟内完成，单GPU实现高效训练。
...
<details>
  <summary>Details</summary>
Motivation: 神经网络虽然强大，但其依赖的梯度学习机制也使其容易受到对抗性攻击。这些攻击通过添加人眼难以察觉的微小扰动，就能让模型做出错误预测。

Method: 使用监督对比学习结合显式硬正样本挖掘，让模型学习图像表示，使得原始图像、增强版本和扰动版本的嵌入在同一类别的嵌入空间中聚集，同时与其他类别分离。

Result: 在CIFAR-10数据集上，使用PGD-20攻击（epsilon=0.031）时，该方法在干净准确率和鲁棒准确率方面都取得了令人印象深刻的结果，优于标准的对抗训练方法。

Conclusion: 将对抗性指导与硬挖掘对比监督相结合，有助于模型学习更具结构性和鲁棒性的表示，缩小了准确率与鲁棒性之间的差距。
...
Result: 在跨生成器检测中达到最先进性能，无需微调；能够准确识别生成器来源；扩散表示固有编码生成器特定模式

Conclusion: 扩散表示提供了合成图像取证简单可解释的基础，能够有效检测深度伪造并识别生成器来源
...
Method: 提出Spatial-SSRL自监督强化学习范式，自动构建五个空间预训练任务：打乱补丁重排序、翻转补丁识别、裁剪补丁修复、区域深度排序和相对3D位置预测，这些任务提供可验证的真实答案。

Result: 在七个空间理解基准测试中，Spatial-SSRL相比Qwen2.5-VL基线在图像和视频设置下分别获得4.63%(3B)和3.89%(7B)的平均准确率提升，同时保持通用视觉能力。

Conclusion: 研究表明，简单的内在监督能够实现规模化强化学习验证，为增强LVLMs的空间智能提供了实用途径。
...
Method: 提出多模态扩散变换器架构，维护分离的模态流但支持跨模态知识共享；引入独立噪声扰动和解耦流匹配损失；开发支持测试时缩放的联合采样方法。

Result: 在RoboCasa和GR-1仿真基准上比基线方法提升6%；测试时缩放额外提升2-5%；在Franka Research 3真实任务中成功率提升13%；在BridgeV2动作无关视频预训练后在RoboCasa上获得显著迁移增益。

Conclusion: DUST有效解决了多模态冲突问题，提升了视觉-语言-动作模型的性能，展示了大规模预训练的潜力。
...
TL;DR: 本文提出了一种基于用户草图约束的图形布局生成方法，通过多模态transformer模型将草图转换为高质量布局，并开发了高效的合成草图生成方法来解决训练数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 现有布局生成方法需要复杂的用户约束规范，降低了可用性。本文旨在利用用户提供的草图作为直观约束，简化设计过程。

Method: 使用多模态transformer模型，将草图和内容资源作为输入生成布局。开发了高效的合成草图生成方法来大规模创建训练数据。
...
TL;DR: 提出了VessShape方法，通过生成包含程序化管状几何结构的大规模2D合成数据集，在血管分割模型中引入形状偏置，以解决数据稀缺和跨模态泛化问题。


<details>
  <summary>Details</summary>
Motivation: 血管语义分割面临标注数据稀缺和模型跨模态泛化能力差的问题，主要原因是CNN倾向于学习纹理特征而非形状特征。

Method: 开发VessShape方法生成大规模合成数据集，包含程序化生成的管状几何结构和多样化纹理，强制模型学习形状线索而非纹理特征。

Result: 在VessShape上预训练的模型在两个真实世界数据集上表现出强大的少样本分割性能（仅需4-10个样本微调），并具有显著的零样本能力。
...
Method: 引入协商器从各模态智能体的本地表示中推导共同表示，通过发送器和接收器实现本地表示空间与共同表示空间的特征相互转换，并使用结构对齐损失、语用对齐损失和分布对齐损失进行训练监督。

Result: 有效减少了与各种本地表示之间的固有领域差距，使共同表示中的知识能够充分提炼到发送器中。

Conclusion: NegoCollab通过协商共同表示的方法，成功解决了异构协作感知中的领域对齐问题，提升了协作性能。
...
TL;DR: 提出高斯组合距离(GCD)来解决小目标检测中IoU和Wasserstein距离的局限性，GCD具有尺度不变性并能促进联合优化，在多个数据集上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: IoU对小目标的微小位置偏差敏感，Wasserstein距离缺乏尺度不变性且优化收敛慢，需要更好的相似性度量方法。

Method: 引入高斯组合距离(GCD)，通过分析GCD及其梯度特性，证明其具有尺度不变性和联合优化能力。

Result: 在AI-TOD-v2小目标检测数据集上达到最先进性能，在MS-COCO-2017和Visdrone-2019数据集上超越Wasserstein距离。

Conclusion: GCD作为边界框回归损失函数和标签分配度量，在各种检测器和数据集上表现出优越的泛化能力。
...
TL;DR: 提出了一种基于无监督深度学习的去噪框架，可应用于多种显微技术，提高定量分析精度，在多个实验中都取得了显著效果。


<details>
  <summary>Details</summary>
Motivation: 操作显微镜在观测材料动态过程时受噪声限制，影响分辨率和定量分析精度。

Method: 开发了无监督深度学习去噪框架，结合偏微分方程约束优化，验证了物理保真度和低偏差特性。

Result: 在STXM中揭示了纳米级化学和结构异质性，在光学显微镜中实现了自动颗粒分割和相分类，在中子射线照相中噪声变异性降低近80%。
...
TL;DR: 提出了Sh-ViT模型，一种轻量级且鲁棒的行人重识别方法，通过打乱模块、场景适应增强和知识蒸馏来应对遮挡和图像质量问题，在真实数据集上表现优异。
...
Method: 基于ViT-Base构建，包含三个组件：1) 最终Transformer层的打乱模块以打破空间相关性；2) 场景适应增强模拟监控条件；3) DeiT知识蒸馏提升有限标签下的学习效果。

Result: 在MyTT数据集上达到83.2% Rank-1和80.1% mAP，在Market1501上达到94.6% Rank-1和87.5% mAP，超越CNN、ViT基线和最先进方法。
...
TL;DR: 提出PETAR-4B模型，将视觉语言模型扩展到3D PET/CT成像领域，通过整合PET、CT和病灶轮廓实现空间定位的报告生成。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型主要局限于2D成像，而3D PET/CT领域面临大容量体积数据、小而分散的病灶以及冗长放射学报告的挑战。

Method: 构建包含11,000多个病灶级描述和3D分割的大规模数据集，采用混合规则和LLM管道提取数据，开发3D掩码感知的视觉语言模型PETAR-4B，整合PET、CT和病灶轮廓。

Result: 综合自动和人工评估表明，PETAR显著提升了PET/CT报告生成质量，实现了临床连贯和定位准确的发现。

Conclusion: PETAR模型推进了3D医学视觉语言理解，为3D医学成像报告生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: DMD在复杂生成任务中表现不佳，直接扩展到多步蒸馏会导致内存使用增加、计算深度加深，造成不稳定和效率降低。随机梯度截断虽然是一种解决方案，但会显著降低多步蒸馏模型的生成多样性。

Method: 提出Phased DMD框架，基于两个关键思想：渐进分布匹配和子区间内的分数匹配。将SNR范围划分为子区间，逐步将模型精炼到更高SNR水平，以更好地捕捉复杂分布。

Result: 通过蒸馏最先进的图像和视频生成模型（包括Qwen-Image和Wan2.2）验证Phased DMD，实验结果表明Phased DMD在保留关键生成能力的同时，比DMD更好地保持了输出多样性。

Conclusion: Phased DMD通过渐进式蒸馏和专家混合方法，有效解决了多步蒸馏中的模型容量和多样性问题，在复杂生成任务中表现出色。
...
TL;DR: LifWavNet是一种基于多分辨率分析合成模型的提升小波网络，用于从雷达信号重建非接触式心电图，通过可学习的提升小波和STFT损失函数在时域和频域上提高重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有的雷达信号重建ECG方法使用固定小波方法，无法自适应捕捉雷达信号特征和合成生理上有意义的ECG波形，需要更灵活和准确的重建方法。

Method: 提出LifWavNet网络，采用可学习的提升小波，包含提升和逆提升单元，自适应捕捉雷达信号特征；引入多分辨率短时傅里叶变换损失函数，在时域和频域上强制与真实ECG保持一致。

Result: 在两个公共数据集上的评估表明，LifWavNet在ECG重建和下游生命体征估计（心率和心率变异性）方面优于现有最先进方法；中间特征可视化显示了多分辨率分解和合成的可解释性。

Conclusion: LifWavNet为基于雷达的非接触式ECG测量提供了一个稳健的框架，在重建质量和生理参数估计方面表现出色。
...
TL;DR: 研究发现持续训练的LLMs会像人类一样受到虚假信息影响，重复接触虚假但自信陈述的事实会导致模型内部表征偏离真相，即使最小程度的暴露也能引起持久性表征漂移。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在持续预训练过程中是否像人类的虚幻真相效应一样，容易受到重复虚假信息的影响而改变其对事实的信念。

Method: 提出Layer of Truth框架和数据集，通过注入受控的污染数据，在不同检查点、模型规模和问题类型下探测中间表征，量化事实信念何时以及如何发生变化。

Result: 即使最小程度的暴露也能在已确立的事实中引起持久的表征漂移，易感性在不同层和模型大小之间存在差异。

Conclusion: 持续更新的LLMs存在被忽视的脆弱性：它们会像人类一样内化错误信息，强调在模型更新过程中需要强健地监控事实完整性。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在多种任务上表现优异，但对对抗性攻击高度脆弱，存在安全和可靠性问题。

Method: 在连续模态（如图像、音频）中添加高斯噪声，生成多个候选输出，使用基于嵌入的聚类过滤受对抗性影响的预测，从多数簇中选择最终答案。

Result: 在POPE、LLaVA-Bench和MM-SafetyBench上的实验表明，SmoothGuard显著提高了对抗攻击的鲁棒性，同时保持了竞争力。消融研究确定了最佳噪声范围（0.1-0.2）。

Conclusion: SmoothGuard是一个轻量级、模型无关的防御框架，能有效增强多模态大语言模型的安全性，在保持实用性的同时提高对抗攻击的鲁棒性。
...
TL;DR: 提出了一种联邦学习算法FedPF，通过差分隐私和公平性约束的零和博弈，揭示了隐私与公平性之间的内在冲突关系。
...
Method: 将多目标优化转化为零和博弈，其中公平性和隐私约束与模型效用竞争，并采用差分隐私技术。

Result: 在三个数据集上实现了高达42.9%的歧视减少，同时保持竞争力精度，但发现隐私-公平性冲突不可避免。
...
TL;DR: 提出CAS-Spec方法，通过动态切换推理加速策略构建推测性草稿模型，结合动态树级联算法实现自适应路由，在多种LLM和数据集上实现1.1×到2.3×的加速。


<details>
  <summary>Details</summary>
Motivation: 现有自推测解码方法在集成便利性和通用性方面表现良好，但速度提升不如依赖专门训练的方法；级联草稿模型方法虽然能提供进一步加速，但多模型训练成本高昂限制了实际应用。

Method: 使用动态可切换推理加速策略（层稀疏性和激活量化）构建推测性草稿模型，提出动态树级联算法自适应路由多级草稿模型并分配草稿长度。

Result: CAS-Spec方法相比现有自推测解码方法实现最先进的加速效果，平均速度提升1.1×到2.3×；DyTC算法相比级联基线和树状基线算法分别提升47%和48%的平均加速。
...
<details>
  <summary>Details</summary>
Motivation: 生成对抗网络(GANs)虽然擅长生成合成数据，但持续遭受模式崩溃问题，即生成器只产生有限范围的输出，无法捕捉完整的数据分布。这在需要多样性和不确定性感知的实际应用中尤其成问题。
...
<details>
  <summary>Details</summary>
Motivation: 将符号化结构与神经生成相结合，为化学工程领域提供透明、可审计的LLM应用方法，特别是在过程控制和安全分析等关键场景中。

Method: 通过数据获取、语义预处理、信息提取和本体映射步骤，将模型训练和推理与COPE本体对齐，使用模板化问答对指导微调，并通过控制解码和引用门机制约束输出到本体链接术语。
...
TL;DR: 提出一个结合聚类和少样本预测的框架，利用大规模充电需求数据集发现站点原型，以提升电动汽车充电需求的预测准确性。


<details>
  <summary>Details</summary>
Motivation: 现有研究受限于小规模数据集、简单的时间依赖建模方法以及对运营历史有限站点的泛化能力弱，需要克服这些限制来支持电动汽车基础设施的规划和电网韧性。

Method: 整合聚类与少样本预测的框架，使用新颖的大规模充电需求数据集发现站点原型，并基于原型构建专家模型进行需求预测。

Result: 原型特定的专家模型在未见站点上的需求预测表现优于全局基线模型。

Conclusion: 通过将预测性能作为基础设施细分的基础，为运营商提供了降低成本、优化能源和定价策略以及支持电网韧性的可行见解，这对实现气候目标至关重要。
...
TL;DR: 提出了MM-OPERA基准，用于评估大视觉语言模型的关联智能，包含两个开放任务和11,497个实例，通过LLM-as-a-Judge策略评估开放回答。


<details>
  <summary>Details</summary>
Motivation: 当前大视觉语言模型在关联智能方面存在不足，现有基准局限于封闭任务，无法评估开放关联推理能力。

Method: 构建包含远程项目关联和上下文关联两个开放任务的基准，采用LLM-as-a-Judge策略评估自由形式回答和显式推理路径。

Result: 对先进LVLMs进行广泛实证研究，包括任务实例敏感性分析、评估策略有效性分析和跨能力领域多样性分析。

Conclusion: 全面揭示了当前LVLMs在关联推理方面的局限性，为开发更类人、通用AI铺平道路。
...
TL;DR: 该论文审计了最先进的移动性预测模型，揭示了基于用户人口统计数据的隐藏差异，并提出了一种公平性引导的增量采样方法(FGIS)来减少群体间的性能差距。


<details>
  <summary>Details</summary>
Motivation: 移动性预测模型在社会应用中日益重要，但其对社会的影响尚未充分探索。研究发现现有模型在不同种族和民族用户群体间存在系统性性能差异，需要解决这种不公平性。

Method: 提出了公平性引导增量采样(FGIS)策略，结合大小感知K均值聚类(SAKM)在潜在移动空间中划分用户群体，使用人口普查数据作为代理标签，构建减少群体性能差距的训练数据集。

Result: 该方法在MetaPath2Vec和transformer-encoder模型上评估，将群体间总差异减少了高达40%，同时保持整体准确性，在早期采样阶段改进最为显著。

Conclusion: 研究揭示了移动性预测流程中的结构性不平等，证明了轻量级、以数据为中心的干预措施能够在低数据应用中有效改善公平性，且复杂度增加很小。
...
Method: 在原始模仿游戏基础上增加能耗约束，通过能源效率的视角评估智能，将抽象思维问题与有限资源现实联系起来。

Result: 新测试为智能评估提供了可测量的实际终点，迫使社会权衡使用AI的时间节省与其总资源成本。

Conclusion: 考虑能耗的新图灵测试是必要的更新，能更好地评估智能并促进对AI资源成本的考量。
...
Conclusion: 开放获取图像结合最小地理空间数据可为城市分析提供可靠的家庭用水估算替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决AI安全研究中在有限计算资源下如何最大化迭代对抗攻击强度的问题，传统方法简单减少迭代次数会显著降低攻击效果

Method: 提出细粒度控制机制，在迭代和层级别上选择性重计算层激活，以在约束预算内实现可达到的攻击效果

Result: 实验表明该方法在同等成本下始终优于现有基线方法，当集成到对抗训练中时，仅用原始预算的30%就能达到相当性能

Conclusion: 该方法有效解决了有限计算预算下的对抗攻击优化问题，为AI安全研究提供了更高效的解决方案
...
TL;DR: 提出了HADSF框架，通过双阶段语义处理解决LLM在推荐系统中信息提取的冗余、幻觉和成本效益问题，引入新指标评估幻觉影响，实验证明能显著提升评分预测准确性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在基于评论的推荐系统中存在三个问题：(i)无范围控制的自由形式评论挖掘导致冗余和噪声表示，(ii)缺乏将LLM幻觉与下游有效性联系的原则性指标，(iii)未充分探索不同模型规模下的成本-质量权衡。

Method: HADSF双阶段框架：第一阶段通过自适应选择生成紧凑的语料级方面词汇表，第二阶段在词汇表指导下进行显式约束的结构化方面-观点三元组提取。引入方面漂移率(ADR)和观点保真率(OFR)评估表示保真度。

Result: 在约300万条评论上对1.5B-70B参数的LLM进行实验，当集成到标准评分预测器中时，HADSF能持续降低预测误差，并使较小模型在代表性部署场景中达到竞争性性能。

Conclusion: HADSF框架有效解决了LLM在推荐系统中的关键挑战，揭示了幻觉严重程度与评分预测误差之间的非单调关系，为幻觉感知的LLM增强可解释推荐提供了可复现的研究基础。
...
Result: 在该框架下，梯度下降、动量、自然梯度、非梯度规则和Adam等自适应优化器自然地出现为不同假设下的最优解。

Conclusion: 该框架通过单一目标统一了这些现象，阐明了学习的计算结构，并为设计自适应算法提供了原则性基础。
...
TL;DR: 提出了一个可重现的多臂老虎机算法评估框架，系统比较了8种经典和方差感知算法，发现在高不确定性环境下方差感知算法有优势，而在可分离场景中经典算法表现更好。


<details>
  <summary>Details</summary>
Motivation: 多臂老虎机算法的评估缺乏标准化条件和可复现性，特别是方差感知扩展算法的性能高度依赖环境条件，需要可靠的比较方法。

Method: 开发了Bandit Playground代码库，包含明确定义的实验设置、多种性能指标（奖励、遗憾、奖励分布、风险价值、动作最优性）和交互式评估界面。

Result: 方差感知算法在奖励差异细微的高不确定性环境中具有优势，而在可分离场景或经过精细调参后，经典算法表现相当或更好。
...
<details>
  <summary>Details</summary>
Motivation: 当前世界建模作为解决机器人等领域数据稀缺问题的途径，但开源训练基础设施仍不成熟。

Method: 开发Jasmine代码库，在数据加载、训练和检查点等方面进行性能优化，保证完全可重现训练，支持多种分片配置。

Result: 在CoinRun案例研究中实现比现有开源实现快一个数量级的重现速度，建立了跨模型家族和架构消融的严格基准测试流程。

Conclusion: Jasmine为世界建模提供了高性能、可扩展的开源训练基础设施，结合大规模数据集建立了严格的基准测试框架。
...
TL;DR: 本文提出了Mixture-of-Transformers (MoT)理论框架，通过门控网络实现transformer块的专业化，证明了其比单一transformer更快的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 现有的MoE模型缺乏统一的理论解释，特别是在前馈层和注意力层都允许专业化的情况下。

Method: 开发了三阶段训练算法，持续训练门控网络，使每个transformer专家专注于不同类型的任务。

Result: 专家专业化减少了梯度冲突，使每个子任务强凸，训练在O(log(ε⁻¹))迭代步内将预期预测损失驱动到接近零。

Conclusion: 为transformer级专业化和学习动态提供了首个统一理论解释，为设计高效大规模模型提供了实践指导。
...
TL;DR: 提出了一种基于组合融合分析(CFA)的新颖情感分类方法，通过整合多样化机器学习模型，在IMDB情感分析数据集上达到97.072%的最优准确率。


<details>
  <summary>Details</summary>
Motivation: 传统方法通常通过扩大单个模型规模来提升性能，但计算资源消耗大。本文旨在利用认知多样性概念，通过量化模型间差异来高效整合模型预测。

Method: 应用组合融合分析(CFA)，使用秩-得分特征函数量化模型间差异，将基于RoBERTa架构的Transformer模型与传统机器学习模型（随机森林、SVM、XGBoost）进行策略性组合。

Result: 在IMDB情感分析数据集上达到97.072%的准确率，优于传统集成方法，同时计算资源使用相对高效。

Conclusion: CFA方法通过有效计算和利用模型多样性，在情感分类任务中实现了优异的性能，为集成学习提供了更高效的替代方案。
...
TL;DR: RLVR方法在数学推理任务中虽然能提升评估指标，但往往是通过强化表面启发式策略而非获得真正的推理能力，揭示了RLVR泛化能力的局限性。


<details>
  <summary>Details</summary>
Motivation: 研究RLVR方法是否能真正提升大语言模型的数学推理能力，而不仅仅是表面指标的改善。

Method: 在两个具有完全可验证解的组合问题（活动调度和最长递增子序列）上测试RLVR，使用精心策划的具有唯一最优解的数据集，并尝试多种奖励设计。

Result: RLVR改善了评估指标，但通常是通过强化表面启发式策略而非获得新的推理策略。
...
TL;DR: 该论文提出一致性训练方法，通过让模型对提示中的无关线索保持响应不变性，来减少LLM的附和性和越狱攻击敏感性。
...
Method: 提出了两种一致性训练方法：基于外部输出的BCT和基于内部激活的ACT，通过数据增强使模型在不同提示变体下保持相同行为。

Result: 两种方法都有效减少了Gemini 2.5 Flash对无关线索的敏感性，BCT在减少越狱方面表现更好，且不会降低模型能力。

Conclusion: 某些对齐问题应被视为一致性而非最优响应问题，BCT可以简化训练流程，减少对静态数据集的依赖。
...
<details>
  <summary>Details</summary>
Motivation: 需要一种实用且一致的算法相似性度量方法，用于克隆检测和程序合成等应用。

Method: 引入EMOC（评估-内存-操作-复杂度）框架，将算法实现嵌入到特征空间，并编译了PACD验证数据集。

Result: EMOC特征支持算法类型聚类分类、近重复检测和LLM生成程序多样性量化。

Conclusion: EMOC框架为算法相似性分析提供了实用工具，代码和数据已开源以促进可重复性和未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准套件无法满足汽车机器学习系统的独特需求，包括安全性和实时处理等约束条件，因此需要专门针对汽车领域设计的标准化性能评估方法。

Method: 开发了一个基准框架，提供延迟和准确性指标以及评估协议，支持2D目标检测、2D语义分割和3D目标检测等汽车感知任务，包含任务选择、参考模型和提交规则的方法论。

Result: 创建了首个标准化汽车机器学习基准，实现了跨不同硬件平台和软件实现的一致且可重现的性能比较，并完成了第一轮基准提交。

Conclusion: MLPerf Automotive填补了汽车机器学习系统标准化评估的空白，为行业提供了可靠的性能比较工具，但面临数据集获取和参考实现开发的工程挑战。
...
TL;DR: 该论文分析了自我博弈训练在大型语言模型推理中的动态机制，通过与RLVR和监督微调的比较，揭示了自我博弈的训练特性、局限性和改进方向。


<details>
  <summary>Details</summary>
Motivation: 虽然自我博弈训练在数学推理中显示出强大的领域内和领域外提升，但其改进机制仍不清楚，需要深入分析其训练动态。

Method: 通过绝对零推理者视角，比较自我博弈与RLVR和监督微调，分析参数更新稀疏性、token分布熵动态和替代提议者奖励函数，并使用pass@k评估连接这些动态与推理性能。

Result: 研究阐明了自我博弈与其他后训练策略的差异，揭示了其固有局限性。

Conclusion: 研究结果明确了自我博弈的训练特性，为通过自我博弈改进LLM数学推理指出了未来方向。
...
TL;DR: 提出了一种可扩展的表示学习框架，通过功能嵌入和Transformer建模，实现跨被试的颅内神经数据聚合，解决了电极位置和数量不一致的问题。
...
Method: 使用孪生编码器和对比学习目标学习电极的功能标识，然后通过Transformer建模区域间关系。该方法不需要严格的任务结构或统一的传感器布局。

Result: 在20名被试的数据集上验证，学习到的功能空间支持准确的被试内区分，形成清晰的区域一致聚类，并能零样本迁移到未见通道。Transformer能够捕捉跨区域依赖关系并重建被掩蔽的通道。

Conclusion: 该方法为大规模跨被试颅内神经数据聚合和预训练提供了一条可行路径，特别是在缺乏严格任务结构和统一传感器布局的情况下。
...
TL;DR: 提出了一种量子启发的神经网络QiNN-QJ，通过量子跳跃算子实现多模态纠缠建模，解决了传统量子启发模型训练不稳定和泛化能力有限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有量子启发融合模型主要依赖酉变换生成量子纠缠，虽然理论表达能力强，但存在训练不稳定和泛化能力有限的问题。

Method: 将每个模态编码为量子纯态，使用可微分模块模拟量子跳跃算子将可分离乘积态转换为纠缠表示，通过联合学习哈密顿量和Lindblad算子生成可控的跨模态纠缠。

Result: 在CMU-MOSI、CMU-MOSEI和CH-SIMS等基准数据集上实现了优于最先进模型的性能，并通过冯诺依曼纠缠熵增强了后验可解释性。

Conclusion: 为纠缠多模态融合建立了一个原则性框架，为量子启发方法在复杂跨模态相关性建模中开辟了新途径。
...
TL;DR: 提出了一种概率层次贝叶斯模型，用于从异质性样本的bulk RNA-seq数据中解卷积得到细胞类型特异性表达谱和比例，利用高分辨率单细胞参考数据，应用于人类子宫内膜组织研究。


<details>
  <summary>Details</summary>
Motivation: 解决bulk组织RNA测序中异质性样本的平均化基因表达谱问题，无法揭示细胞类型特异性动态变化，特别是在激素驱动的细胞组成变化显著的子宫内膜组织研究中。

Method: 开发概率层次贝叶斯模型，利用高分辨率单细胞参考数据，通过详细的模型结构、先验分布和推理策略，解卷积bulk RNA-seq数据为组成细胞类型的表达谱和比例。

Result: 模型成功揭示了月经周期不同阶段上皮细胞、基质细胞和免疫细胞比例的动态变化，识别了与子宫内膜功能相关的细胞类型特异性差异基因表达（如分泌期基质细胞中的蜕膜化标记物）。

Conclusion: 贝叶斯方法对参考数据不匹配和噪声具有鲁棒性，研究结果对生育能力和子宫内膜疾病的临床意义具有重要价值，未来可整合空间转录组学数据。
...
TL;DR: 该论文提出了一个带约束的离线策略优化框架，通过在基于梯度的离线策略优化中引入组间奖励差异约束，来减少策略学习过程中可能出现的组间奖励不公平问题。


<details>
  <summary>Details</summary>
Motivation: 离线上下文赌博机可以从历史数据中学习策略，但最大化总体期望奖励的策略优化可能会无意中放大组间奖励差异，导致某些群体比其他群体获益更多，这在资源有限的情况下引发公平性担忧。

Method: 提出了一个约束离线策略优化框架，将组间奖励差异约束引入到基于梯度的离线策略优化过程中。使用双重稳健估计器来改进组间奖励差异的估计，并为策略优化提供了收敛保证。

Result: 在合成和真实世界数据集上的实证结果表明，该方法在保持竞争力的整体性能的同时，有效减少了奖励差异。

Conclusion: 该方法成功解决了离线上下文赌博机中的组间公平性问题，能够在维持整体性能的同时显著降低奖励差异，为公平策略学习提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: AI代理作为变革性工具，能够通过复杂的科研工作流程自主运作，整合生物医学数据、执行任务、进行实验并迭代优化假设，从而解决药物发现中的效率和可扩展性问题。

Method: 采用基于大语言模型的AI代理架构，包括ReAct、Reflection、Supervisor和Swarm系统，结合感知、计算、行动和记忆工具，实现端到端的药物发现决策。

Result: 早期实施显示在速度、可重复性和可扩展性方面取得显著进展，将原本需要数月的工作流程压缩到数小时，同时保持科学可追溯性。

Conclusion: AI代理系统在药物发现中展现出巨大潜力，但面临数据异质性、系统可靠性、隐私和基准测试等挑战，未来需要进一步发展支持科学和转化的技术。
...
TL;DR: 本研究比较了GPT-4.1和GPT-5生成的rationales在自动评分中的效用，发现基于文章的评分优于基于rationales的评分，但后者在少数类别上表现更好。集成模型能进一步提升评分准确性。


<details>
  <summary>Details</summary>
Motivation: 探索GPT-4.1和GPT-5生成的rationales在自动评分中的实际效用，比较基于文章和基于rationales的评分方法的效果差异。

Method: 使用2012年Kaggle ASAP数据中的Prompt 6文章，比较基于文章的评分与基于rationales的评分，并采用集成建模方法结合不同评分模型。

Result: 基于文章的评分总体表现更好（QWK更高），但基于rationales的评分在少数类别（分数0）上F1得分更高。集成模型显著提升了评分准确性，最佳集成模型的QWK达到0.870。

Conclusion: 基于文章的自动评分效果优于基于rationales的评分，但rationales在解决类别不平衡问题上有价值。集成不同评分模型能获得最佳性能，为自动评分系统提供了优化方向。
...
<details>
  <summary>Details</summary>
Motivation: 由于机器学习模型对某些人口群体的不当行为日益受到关注，公平性概念在社区中引起重视，这推动了图聚类中公平性的研究。公平图聚类旨在将图中的节点划分为k个不相交的簇，使得每个簇中受保护群体的比例与整个数据集中该群体的比例一致
...
Conclusion: FairAD提供了一种计算高效的公平图聚类解决方案，能够有效处理大规模图的公平聚类问题
...
TL;DR: RelTune是一个新颖的数据库参数调优框架，通过关系图表示参数依赖关系，使用GNN学习性能相关语义嵌入，并引入混合分数引导的贝叶斯优化方法，显著提升了调优效率和性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据库参数自动调优方法存在三个主要问题：忽略参数间的依赖关系、仅优化少数参数而忽略其他重要参数、贝叶斯优化的代理模型预测不稳定且探索效率低。

Method: 提出RelTune框架：1）构建参数关系图表示依赖关系；2）使用GNN学习性能相关语义的潜在嵌入；3）引入混合分数引导贝叶斯优化（HBO），结合代理预测和亲和度分数。
...
TL;DR: 提出了一种名为"E"的适配器，用于改进梯度优化器，使其在达到局部最小值后继续探索损失景观的谷底区域，以寻找更好的局部最小值，从而提高泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有优化器在达到局部最小值后停止搜索，但复杂的损失景观几何特性使得无法保证该点是最低或具有最佳泛化能力的点。

Method: 设计了一个适配器"E"，使优化器在达到局部最小值后继续沿着损失景观的谷底区域进行探索，寻找可能更优的局部最小值。

Result: 在大型批次训练场景中，改进后的Lamb优化器（ALTO）相比现有最优优化器平均提高了2.5%的测试准确率。

Conclusion: 该方法为优化算法设计开辟了新的研究方向，能够提高找到更低、更平坦局部最小值的可能性，从而改善泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有防御策略通过攻击模拟来构建鲁棒性，但存在根本限制：无法扩展到有界威胁模型之外，难以预测未知攻击；对变化攻击设置的适应性有限。

Method: 将有害微调防御建模为贝叶斯推理问题，学习数据点安全属性的后验分布，基于后验采样对数据进行加权约束微调过程。引入基于摊销贝叶斯学习的神经调度器，实现高效迁移。

Result: 在多种攻击和防御设置下的综合结果表明，该方法达到了最先进的性能。

Conclusion: BDS通过贝叶斯推理实现了自适应防御，无需攻击模拟就能有效缓解有害数据的影响，在各种设置下表现出色。
...
TL;DR: 提出了一种新的多项式时间算法，用于在线稀疏线性回归问题，该算法在兼容性条件下显著改进了之前的遗憾边界。
...
Method: 利用Dantzig Selector，结合算法相关的协方差矩阵采样方案、自适应参数调整方案以及带仔细初始化的批处理在线牛顿步骤。

Result: 在兼容性条件下显著改进了之前的遗憾边界，并扩展了带有额外观测的OSLR问题。

Conclusion: 新算法在较弱假设下实现了更好的性能，并通过新颖的分析方法提供了更紧的收敛率。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法忽略VM冷启动、长尾服务时间分布等现实因素的问题，平衡处理与传输延迟同时最小化自适应推理应用的成本。

Method: 将ML查询建模为遍历无环阶段序列，每个阶段包含稀疏模型参数块；使用FaaS无服务器函数和阶段特定资源供应，考虑各阶段请求退出比例；集成自适应负载均衡在VM和无服务器函数间分配请求。

Result: SERFLOW能够降低云成本超过23%，并有效适应动态工作负载。

Conclusion: 通过考虑现实因素和阶段特定资源供应，SERFLOW在降低云成本的同时有效处理动态工作负载，为ML推理应用提供了实用的资源管理解决方案。
...
TL;DR: 提出MDAS-GNN模型，通过多维注意力机制和图神经网络改进交通事故预测，在多个英国城市数据集上表现优于基准方法，预测误差降低达40%。
...
Method: 开发MDAS-GNN（多维注意力空间扩散图神经网络），整合交通安全、基础设施和环境风险三个核心维度，采用特征特定空间扩散机制和多头时间注意力来捕捉不同时间跨度的依赖关系。

Result: 在英国交通部三个城市（伦敦中心区、南曼彻斯特、伯明翰东南部）事故数据上评估，MDAS-GNN在所有时间尺度（短期、中期、长期）均保持低预测误差，尤其在长期预测中表现突出，集成多维特征比单特征方法减少预测误差达40%。
...
<details>
  <summary>Details</summary>
Motivation: 主流可解释AI方法通常提供不完整的静态视图，将特征作用简化为单一分数，无法处理非线性和交互效应。需要更全面的方法来理解模型学习过程。

Method: FFCA框架量化特征的4个维度：影响、波动性、非线性和交互性。动态原型分析追踪这些签名在训练过程中的演化，揭示模型学习方式。

Result: 首次提供了层次学习的直接经验证据，显示模型先学习简单线性效应再学习复杂交互。动态分析能识别模型容量不足并预测过拟合发生。

Conclusion: FFCA通过静态和动态组件提供必要的几何背景，将模型解释从简单量化转变为对整个学习过程的细致可信分析。
...
TL;DR: 提出Soft Task-Aware Routing (STAR)方法，通过将投影头建模为专家来减少不变性和等变性表示学习中的冗余特征学习，提高模型效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常使用独立的投影头分别学习不变性和等变性表示，但忽视了两种学习之间的共享信息，导致冗余特征学习和模型容量利用效率低下。

Method: 引入STAR路由策略，将投影头建模为专家，让专家专门捕获共享信息或任务特定信息，从而减少冗余特征学习。

Result: 实验显示不变性和等变性嵌入之间的典型相关性降低，在多种迁移学习任务中取得一致改进。

Conclusion: STAR方法通过建模投影头为专家，有效减少冗余特征学习，提高表示学习效率，在迁移学习中表现优异。
...
TL;DR: FedSM是一个面向客户端的联邦学习框架，通过语义引导的特征混合和轻量级分类器重训练来缓解非IID和长尾数据分布导致的模型偏差问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法假设所有真实实例贡献相同，但实际数据集包含信息性和冗余/有害实例，不考虑数据质量会降低模型性能。

Method: IWD框架利用影响函数为每个实例分配自适应权重，基于其对蒸馏目标的估计影响，优先处理有益数据，降权无用或有害数据。

Result: 集成IWD可提高蒸馏数据集质量，增强模型性能，准确率提升最高达7.8%。

Conclusion: IWD是一个模块化框架，可无缝集成到多种数据集蒸馏方法中，有效提升蒸馏效果。
...
TL;DR: ECVL-ROUTER是一个面向视觉语言模型的情境感知路由框架，通过动态选择大模型或小模型来处理查询，在保证性能的同时降低延迟和能耗。
...
Method: 提出ECVL-ROUTER框架，引入新的路由策略和评估指标，基于用户需求动态选择合适模型。构建了专门用于路由器训练的多模态响应质量数据集。

Result: 实验表明，该方法成功将超过80%的查询路由到小模型，同时问题解决概率下降不到10%。

Conclusion: ECVL-ROUTER框架有效平衡了模型性能和资源效率，为不同场景需求提供了优化的模型选择方案。
...
TL;DR: 提出高阶线性注意力(HLA)，一种因果流式机制，通过紧凑前缀充分统计实现高阶交互，在保持常数大小状态的同时实现线性时间计算，解决了传统注意力二次成本问题。


<details>
  <summary>Details</summary>
Motivation: 解决自回归语言模型扩展到长上下文时注意力机制的二次成本问题，同时克服现有线性时间注意力和状态空间模型在表达能力上的限制。

Method: 基于紧凑前缀充分统计实现高阶交互，在二阶情况下保持常数大小状态，使用闭式流式恒等式、严格因果掩码变体和基于关联扫描的分块并行训练方案。

Result: HLA在保持线性时间复杂度的同时实现了高阶表达能力，能够精确复现串行循环的激活，结合了注意力类数据依赖混合与现代循环架构的效率。

Conclusion: HLA作为一种原则性、可扩展的构建块，将注意力类数据依赖混合与现代循环架构的效率相结合，为长序列建模提供了有效解决方案。
...
TL;DR: 提出了ODP-Bench基准测试，用于评估OOD性能预测算法，包含常用OOD数据集和现有算法，提供训练好的模型作为测试平台。


<details>
  <summary>Details</summary>
Motivation: OOD性能预测领域缺乏一致的评估协议，大多数工作只覆盖有限数量的真实OOD数据集和分布偏移类型。

Method: 构建包含常用OOD数据集和现有性能预测算法的综合基准测试，提供训练好的模型作为测试平台。

Result: 创建了ODP-Bench基准测试，为各种算法提供方便和公平的比较。

Conclusion: ODP-Bench为OOD性能预测研究提供了标准化的评估框架，有助于推动该领域的发展。
...
Conclusion: HiF-DTA通过分层网络和多尺度特征融合，有效提升了药物-靶点亲和力预测的准确性，为计算药物发现提供了更强大的工具。
...
TL;DR: EnterpriseBench是一个模拟企业环境的综合基准测试，包含500个跨领域任务，用于评估LLM在企业系统中的表现。实验显示当前最先进的LLM代理仅能完成41.8%的任务。
...
Result: 实验结果表明，即使是最先进的LLM代理在EnterpriseBench上的任务完成率也仅为41.8%，显示出企业AI系统仍有巨大改进空间。

Conclusion: EnterpriseBench为企业LLM系统提供了重要的评估工具，揭示了当前系统在企业环境中的局限性，为未来改进指明了方向。
...
TL;DR: 提出了一种基于互信息分析心率非线性混沌行为的新方法，结合深度学习显著提升了真实场景下的心率估计精度，相比传统方法提升达40%。


<details>
  <summary>Details</summary>
Motivation: 心率振荡具有复杂的非线性混沌特性，这对日常心血管健康监测提出了挑战，需要从数学角度解释和处理这种非线性时间复杂性。

Method: 通过互信息研究心率的非线性混沌行为，提出一种增强心率估计的新方法，并与深度学习解决方案相结合。

Result: 在四个真实场景数据集上的验证表明，该方法相比传统方法和现有机器学习技术，心率估计性能提升高达40%，减少了对多模态传感的依赖，无需后处理步骤。

Conclusion: 该方法不仅从数学角度解释了心率的非线性复杂性，而且与深度学习结合时能显著提升心率估计性能，为日常心血管监测提供了有效解决方案。
...
TL;DR: 该研究比较了批处理和流式学习在物联网流量异常检测中的表现，发现流式学习方法能更好地处理概念漂移问题，其中自适应随机森林算法在保持高准确率的同时显著降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 随着物联网流量增长，传统批学习模型面临维护成本高、对概念漂移适应差的问题，而流式学习能够实现无缝更新和概念漂移检测以提高鲁棒性。

Method: 将物联网流量异常检测作为二分类问题，通过混合现有数据集模拟异构网络数据流，逐个样本流式处理，比较树基和非树基机器学习算法的表现。

Result: 自适应随机森林达到F1分数0.990±0.006，计算成本仅为批处理的三分之一；Hoeffding自适应树达到F1分数0.910±0.007，计算成本降低四倍，但稳定性略有牺牲。

Conclusion: 流式学习方法在物联网异常检测中优于批处理方法，能有效处理概念漂移，其中树基算法表现优异，但当前数据集在暴露模型局限性方面仍有不足。
...
TL;DR: 该论文提出了一种新的语义新颖性评估方法，通过不可归因性来衡量模型输出是否在预训练语料中缺乏语义相似的上下文。


<details>
  <summary>Details</summary>
Motivation: 传统训练数据归因方法关注哪些训练样本影响了特定输出，而本研究反过来问：哪些输出无法归因到任何预训练样本？这为评估模型生成内容的语义新颖性提供了新视角。

Method: 使用两阶段检索流程：先用轻量级GIST嵌入索引语料库并检索top-n候选，然后用ColBERTv2重新排序。如果最近的语料项比人工生成的文本参考更不可归因，则认为模型输出是新颖的。

Result: 在SmolLM和SmolLM2上的评估发现：(1)模型依赖预训练数据的跨度比之前报道的要长；(2)某些领域系统性地促进或抑制新颖性；(3)指令微调不仅改变风格还增加新颖性。

Conclusion: 围绕不可归因性重新构建新颖性评估，能够在预训练规模上实现高效分析。作者发布了约20TB的语料块和索引工件以支持复现和大规模扩展分析。
...
TL;DR: MedM2T是一个时间感知的多模态框架，用于处理医疗数据的多模态性和异质时间结构，在心血管疾病预测、院内死亡率预测和ICU住院时间回归等任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 医疗数据具有固有的多模态性和异质时间结构，这给建模带来了重大挑战，需要开发能够灵活处理这些复杂性的框架。

Method: MedM2T整合了稀疏时间序列编码器、分层时间感知融合和双模态注意力机制，使用模态特定的预训练编码器并在共享编码器中对齐特征。

Result: 在MIMIC-IV数据集上，MedM2T在心血管疾病预测上达到AUROC 0.947和AUPRC 0.706，在死亡率预测上达到AUROC 0.901和AUPRC 0.558，在住院时间回归上MAE为2.31。
...
TL;DR: 研究发现基于结果强化学习的语言模型在推理过程中会产生难以理解的思维链，尽管最终答案可读性良好，这削弱了通过监控思维链来理解模型意图和检测恶意行为的有效性。


<details>
  <summary>Details</summary>
Motivation: 通过监控思维链来理解语言模型的意图和检测潜在恶意行为，但这需要思维链具有可读性和忠实性。

Method: 研究了14个推理模型的思维链可读性，分析强化学习对推理可读性的影响，并通过强制使用可读部分和重采样实验来验证结果。

Result: 强化学习通常导致推理过程对人类和AI监控器都难以理解，模型使用不可读的推理得出正确答案，可读性与性能之间没有相关性，且问题难度越大可读性越差。

Conclusion: 如果没有明确优化可读性，基于结果的强化学习会自然产生推理过程不透明的模型，可能破坏监控方法的有效性。
...
TL;DR: 该论文提出了一种更全面的可监控性评估方法，结合忠实性和详尽性来衡量思维链作为模型外部工作记忆的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注模型在添加提示后改变答案的情况来评估思维链的忠实性，但这种方法会丢失信息，且无法考察与提示无关的推理方面。

Method: 引入详尽性概念（思维链是否列出解决任务所需的所有因素），并将其与忠实性结合成单一的可监控性评分，在BBH、GPQA和MMLU数据集上评估指令调优和推理模型。

Result: 研究发现模型可能看似忠实但难以监控，因为它们会遗漏关键因素，且不同模型系列的可监控性差异显著。

Conclusion: 思维链作为外部工作记忆的可监控性对基于思维链监控的安全方案至关重要，需要更全面的评估方法。
...
TL;DR: 提出了FedMuon优化器来解决联邦学习中的通信瓶颈问题，通过矩阵正交化优化参数结构，在非IID设置下使用动量聚合和局部-全局对齐技术来减少客户端漂移。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法主要使用逐元素优化器，忽略了权重矩阵的几何结构，导致局部更新时病态方向放大、条件数恶化以及收敛缓慢。在非IID设置下，基于本地分布的独立矩阵正交化会引发严重的客户端漂移。

Method: 提出FedMuon优化器，包含两个关键技术：(1) 动量聚合 - 客户端使用聚合的动量进行本地初始化；(2) 局部-全局对齐 - 将本地梯度与全局更新方向对齐以显著减少客户端漂移。

Result: 理论上证明FedMuon实现了线性加速收敛率，无需异质性假设。实验验证在语言和视觉模型上，相比多个基线方法，FedMuon显著减少了通信轮数并提高了测试准确率。

Conclusion: FedMuon通过矩阵正交化和客户端漂移缓解技术，有效解决了联邦学习中的通信瓶颈问题，在IID和非IID设置下都能显著加速收敛并减少通信成本。
...
TL;DR: Atlas-Alignment框架通过将新模型的潜在空间与已标记的Concept Atlas对齐，实现跨模型的可解释性迁移，无需重新训练稀疏自编码器或手动标注组件。


<details>
  <summary>Details</summary>
Motivation: 现有可解释性方法成本高昂且难以扩展，需要为每个新模型训练专门的稀疏自编码器和手动标注组件，限制了可解释性技术的广泛应用。

Method: 使用共享输入和轻量级表示对齐技术，将未知的潜在空间与已标记的Concept Atlas对齐，实现语义特征搜索和可控生成。

Result: 定量和定性评估表明，简单的表示对齐方法能够实现鲁棒的语义检索和可控生成，无需标记的概念数据。

Conclusion: Atlas-Alignment通过一次性投资构建高质量Concept Atlas，能够以最小边际成本使多个新模型变得透明可控，分摊了解释性AI的成本。
...
TL;DR: 提出MVeLMA多模态端到端机器学习管道，用于预测野火后县级植被损失，通过多模态特征集成和堆叠集成架构实现不确定性估计，优于现有方法。
...
Result: 模型在预测野火后植被损失方面优于多个最先进和基线模型，并生成植被损失置信度地图识别高风险县。
...
TL;DR: 提出Spectral Preservation Network框架，通过生成保留原始图谱特性的简化图，在降低计算成本的同时支持下游图学习任务。


<details>
  <summary>Details</summary>
Motivation: 传统图神经网络依赖固定结构且易出现过平滑问题，需要一种能生成忠实代理图的新方法。

Method: 引入联合图演化层和谱一致性损失，共同变换图拓扑和节点特征，并通过谱特性一致性进行正则化。

Result: 在节点级稀疏化任务中，该方法在多个指标上优于现有最先进方法。

Conclusion: Spectral Preservation Network在保持图谱特性的同时有效降低计算复杂度，具有明显优势。
...
TL;DR: 提出了一种在单纯形上学习和采样概率分布的方法，通过将开放单纯形映射到欧几里得空间，利用Aitchison几何定义映射，并通过Dirichlet插值对分类数据进行建模。


<details>
  <summary>Details</summary>
Motivation: 现有方法在单纯形上使用黎曼几何或自定义噪声过程，而本文方法在欧几里得空间中工作同时尊重Aitchison几何，旨在提供更有效的概率分布建模。

Method: 使用平滑双射将开放单纯形映射到欧几里得空间，基于Aitchison几何定义映射，通过Dirichlet插值将离散观测反量化到连续空间。

Result: 在合成和真实世界数据集上实现了竞争性性能，能够在欧几里得空间中进行密度建模，同时精确恢复原始离散分布。

Conclusion: 该方法提供了一种在欧几里得空间中有效建模单纯形上概率分布的新途径，同时保持了对Aitchison几何的尊重，在多个数据集上表现出色。
...
TL;DR: 本文提出通过重采样方法研究推理模型中的思维链分布，而非仅分析单一思维链。通过四个案例研究展示了重采样在因果分析、干预效果评估和推理过程理解中的应用价值。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多只分析单一思维链，无法全面理解模型的因果影响和计算过程。思维链分布的研究对于理解模型推理机制和进行有效干预至关重要。

Method: 采用重采样方法研究思维链分布，包括：1）通过重采样特定句子测量下游影响；2）比较重采样与人工编辑的干预效果；3）引入韧性指标防止相似内容重现；4）采用因果中介分析研究暗示的影响。

Result: 发现：1）自我保护句子对勒索行为影响很小；2）离策略干预效果小且不稳定；3）关键规划语句难以消除但消除后影响大；4）暗示对输出有因果影响且影响持久。

Conclusion: 通过重采样研究思维链分布能够实现可靠的因果分析、更清晰的模型推理叙述，以及原则性的思维链干预。
...
TL;DR: 提出了FedAdamW算法，这是首个针对联邦学习的AdamW优化器，通过局部校正机制和去耦权重衰减解决数据异构性、局部过拟合和收敛缓慢的问题。


<details>
  <summary>Details</summary>
Motivation: AdamW在联邦学习环境中直接应用面临三个挑战：数据异构导致二阶矩估计方差大、局部过拟合导致客户端漂移、每轮重新初始化矩估计减慢收敛。

Method: FedAdamW使用局部校正机制和去耦权重衰减使局部更新与全局更新对齐，高效聚合二阶矩估计的均值以减少方差并重新初始化。

Result: 理论上证明了FedAdamW在没有异构性假设下达到线性加速收敛率；实证验证在语言和视觉Transformer模型上显著减少通信轮数并提高测试准确率。

Conclusion: FedAdamW是首个专门为联邦学习设计的AdamW算法，有效解决了数据异构环境下的优化问题，在理论和实验上都表现出色。
...
TL;DR: InertialAR是一个基于Transformer的自回归模型，用于3D分子生成。它通过惯性框架对齐和原子重排序实现SE(3)和排列不变性，使用几何旋转位置编码增强注意力机制，并采用分层自回归范式预测原子类型和3D坐标。


<details>
  <summary>Details</summary>
Motivation: Transformer自回归模型在文本和图像模态中取得了成功，但在3D分子生成方面尚未充分探索。主要挑战包括：如何将分子标记化为对SE(3)变换和原子索引排列不变的规范1D序列，以及如何设计能够建模耦合离散原子类型和连续3D坐标的混合原子标记的架构。

Method: InertialAR设计了规范标记化方法，将分子对齐到其惯性框架并重排序原子以确保SE(3)和排列不变性。使用几何旋转位置编码(GeoRoPE)为注意力机制注入几何感知能力。采用分层自回归范式，先预测原子类型，然后通过扩散损失预测其3D坐标。

Result: 在QM9、GEOM-Drugs和B3LYP数据集的无条件分子生成评估中，InertialAR在10个评估指标中的7个上达到了最先进性能。在针对目标化学功能的可控生成任务中，显著优于强基线，在所有5个指标上都取得了最先进的结果。

Conclusion: InertialAR成功地将Transformer自回归模型扩展到3D分子生成领域，通过创新的规范标记化和几何感知注意力机制解决了关键挑战，在无条件生成和可控生成任务中都表现出卓越性能。
...
TL;DR: 提出DP-FedPGN算法，通过引入全局梯度范数惩罚来寻找全局平坦最小值，缓解差分隐私联邦学习中的模型泛化性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 当前客户端级差分隐私联邦学习方法会导致更尖锐的损失景观，降低模型泛化能力。虽然使用SAM可以找到局部平坦最小值，但局部平坦性不能反映全局平坦性。

Method: 在本地损失函数中引入全局梯度范数惩罚，寻找全局平坦最小值，同时减少局部更新范数，降低梯度裁剪误差。使用Rényi DP提供严格隐私保证。
...
TL;DR: 提出基于GNN的稀疏近似逆预处理器学习方法，避免三角求解，提升GPU并行效率，在CG求解器中实现40%-53%的速度提升。


<details>
  <summary>Details</summary>
Motivation: 传统预处理器依赖预设算法，难以利用数据优化；现有基于学习的方法使用GNN但依赖不完全分解，导致三角求解阻碍GPU并行化，且长程依赖难以建模。

Method: 使用GNN构建稀疏近似逆(SPAI)预处理器，避免三角求解，每个CG步骤仅需两次矩阵向量乘积；引入基于统计的尺度不变损失函数，匹配CG收敛特性。

Result: 在三个PDE数据集和一个合成数据集上评估，相比标准预处理器(对角、IC、传统SPAI)和先前学习方法，在GPU上减少40%-53%求解时间，条件数更优，泛化性能更好。

Conclusion: 提出的学习型预处理器方法在GPU上显著加速CG求解器，同时保持更好的数值性能和泛化能力。
...
TL;DR: 研究评估通用时间序列基础模型在脑电图（EEG）任务中的表现，发现即使使用非神经源数据或合成数据预训练，也能在运动想象分类和睡眠阶段预测等任务上超越专门的EEG模型。


<details>
  <summary>Details</summary>
Motivation: 探索通用时间序列基础模型在特定生物医学信号（如EEG）领域的适用性，验证跨域预训练策略的有效性。

Method: 测试两种预训练策略：(a) 在多领域真实时间序列数据上预训练；(b) 在纯合成数据上预训练，并在EEG任务（运动想象分类、睡眠阶段预测）上评估性能。

Result: 两种预训练变体均表现优异，持续超越广泛使用的卷积基线EEGNet和最新的EEG专用基础模型CBraMod。
...
TL;DR: 提出了一种用于群体结构健康监测的贝叶斯域适应框架，结合主动学习策略，在标签稀缺场景下提高分类模型的数据效率。


<details>
  <summary>Details</summary>
Motivation: 结构健康监测数据获取成本高且不切实际，特别是标记数据。群体SHM旨在利用多个结构的数据，但不同结构的数据分布不同，导致传统机器学习方法泛化误差大。

Method: 使用贝叶斯框架进行域适应，利用有限的标记目标数据改进无监督域适应映射，并集成主动采样策略指导检查选择最有信息量的观测进行标记。
...
TL;DR: TetraJet-v2是一种端到端的4位全量化训练方法，使用NVFP4格式处理所有线性层中的激活、权重和梯度，解决了权重震荡和异常值问题，在LLM预训练中显著优于现有FP4训练方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型训练成本极高，促使研究者探索低精度全量化训练方法。虽然4位格式如NVFP4能带来显著效率提升，但在如此低精度下实现接近无损的训练仍然具有挑战性。

Method: 提出TetraJet-v2方法，包括：1）NVFP4线性层的无偏双块量化方法；2）抑制权重震荡的OsciReset算法；3）保持异常值精度的OutControl算法。

Result: 在高达370M参数和200B token数据规模的LLM预训练中，TetraJet-v2始终优于现有FP4训练方法，将性能与全精度训练的差距平均缩小了51.3%。

Conclusion: TetraJet-v2证明了在4位精度下实现高效LLM训练的可行性，通过解决权重震荡和异常值问题，显著提升了低精度训练的性能。
...
TL;DR: AstuteRAG-FQA是一个专为金融问答设计的自适应RAG框架，通过任务感知提示工程解决金融领域RAG应用中的专有数据访问限制、检索准确性、监管约束和敏感数据解释等挑战。


<details>
  <summary>Details</summary>
Motivation: RAG在知识密集型任务中表现优异，但在金融领域应用面临专有数据集访问受限、检索准确性不足、监管约束和敏感数据解释等关键挑战。
...
Conclusion: AstuteRAG-FQA为金融问答提供了一个安全、合规且高效的自适应RAG解决方案，有效解决了金融领域特有的挑战。
...
TL;DR: ORGEval是一个基于图论的评估框架，用于评估大语言模型在制定线性和混合整数线性规划方面的能力。它通过将优化模型表示为图，将等价性检测简化为图同构测试，解决了现有求解器方法的不一致性、不可行性和高计算成本问题。


<details>
  <summary>Details</summary>
Motivation: 工业应用中制定优化问题需要大量手动工作和领域专业知识。虽然大语言模型在自动化这一过程中显示出潜力，但由于缺乏稳健的评估指标，评估其性能仍然困难。现有的基于求解器的方法存在不一致性、不可行性和高计算成本问题。
...
Result: 实验结果显示，ORGEval能够成功检测模型等价性，并在随机参数配置下产生100%一致的结果，同时在运行时间上显著优于基于求解器的方法，特别是在困难问题上。基于ORGEval构建的Bench4Opt数据集显示，DeepSeek-V3和Claude-Opus-4在直接提示下达到最高准确率。

Conclusion: ORGEval提供了一个稳健、高效的框架来评估LLMs在优化建模方面的能力。虽然优化建模对所有LLMs仍然具有挑战性，但DeepSeek-V3和Claude-Opus-4表现最佳，甚至超过了领先的推理模型。
...
TL;DR: 本文提出了panprediction框架，将模型训练视为从数据中提取足够信息，使模型能够最小化下游多个任务的多种损失函数。该框架统一了omniprediction和多组学习，并设计了高效的确定性/随机性panpredictor学习算法。


<details>
  <summary>Details</summary>
Motivation: 传统监督学习关注在固定分布上最小化固定损失函数，而新范式认为模型训练应该提取足够信息，使模型能够处理下游多个任务的多种损失函数。

Method: 提出panprediction数学框架，设计确定性panpredictor（样本复杂度~O(1/ε³)）和随机性panpredictor（样本复杂度~O(1/ε²)）的学习算法，通过step calibration实现统计高效性。

Result: 在温和假设下，同时最小化无限多任务上的无限多损失函数与最小化单一任务上的单一损失函数具有相同的统计难度。确定性omniprediction的样本复杂度改进了1/ε倍。

Conclusion: panprediction框架统一了omniprediction和多组学习，证明了在统计效率上同时处理多任务多损失与单任务单损失具有可比性，为更通用的学习范式提供了理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 类别不平衡是机器学习中的基本挑战，通常导致不可靠的分类性能。现有方法主要关注数据重采样或损失重加权，但作者认为不平衡会通过少数类别的欠规范放大Clever Hans效应。

Method: 采用基于反事实解释的可解释AI方法，联合识别和消除在类别不平衡下出现的Clever Hans效应。

Result: 在三个数据集上实现了具有竞争力的分类性能，并展示了在类别不平衡下Clever Hans效应的出现机制。

Conclusion: 该方法提供了一个被现有方法忽视的新视角，即类别不平衡会放大Clever Hans效应，通过可解释AI可以有效解决这一问题。
...
TL;DR: 提出了一种基于确定性信息瓶颈和Rényi熵的逐层训练方法，在CIFAR数据集上性能优于现有逐层训练方法，且接近SGD性能


<details>
  <summary>Details</summary>
Motivation: 传统DNN使用全局交叉熵损失和反向传播在生物学上不现实，而逐层训练可以避免中间梯度计算和存储，减少内存使用并缓解梯度消失/爆炸问题

Method: 基于信息瓶颈原理，每层与连接到输出层的辅助分类器联合训练，学习最小充分的任务相关表示

Result: 在CIFAR-10、CIFAR-100和交通标志识别任务上验证了方法的有效性，性能优于现有逐层训练基线

Conclusion: 提出的逐层训练方法不仅性能优越，而且具有生物学合理性，为深度学习训练提供了新思路
...
TL;DR: 该论文分析了多智能体强化学习中开放性与信用分配问题的关系，指出传统信用分配方法在开放环境中的不足，并通过概念分析和实证研究证明开放性会导致信用分配错误。
...
Conclusion: 开放环境对信用分配构成重大挑战，需要开发新的方法来应对智能体、任务和类型的动态变化。
...
TL;DR: 提出了CATArena评估平台，通过开放式计分的棋牌游戏来持续评估LLM智能体的学习能力，解决现有基准测试中分数饱和和依赖专家标注的问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体基准测试主要评估固定场景下的端到端性能，存在分数饱和和过度依赖专家标注的问题，限制了学习能力的评估。
...
TL;DR: 提出了一个可扩展的框架来解压缩科学推理，构建可验证的长思维链知识库SciencePedia，通过逆向知识检索和合成生成高质量的科学文章。


<details>
  <summary>Details</summary>
Motivation: 现有科学材料压缩了推理过程，仅呈现结论而省略推导链，这阻碍了验证和跨领域连接。

Method: 使用苏格拉底式代理生成约300万个第一原理问题，多个独立求解器模型生成LCoT，通过提示净化和跨模型答案共识进行严格过滤，构建可验证的知识库。

Result: SciencePedia包含约20万个细粒度条目，涵盖多个学科。在六个学科评估中，基于检索LCoT合成的文章比无检索基线具有更高的知识点密度和更低的事实错误率。

Conclusion: 这种基于可验证LCoT知识库的推理中心方法实现了可信赖的跨领域科学合成，为不断扩展的百科全书奠定了基础。
...
TL;DR: Denario是一个AI多代理系统，作为科学研究的助手，能够执行从想法生成到论文撰写的完整科研流程，并在多个科学领域生成论文。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够辅助科学研究全过程的AI系统，解决科研工作中重复性任务的问题，并探索跨学科研究的可能性。

Method: 采用模块化架构，包含Cmbagent作为深度研究后端，能够处理特定任务或进行端到端的科学分析。

Result: 系统在多个科学领域生成了论文，包括天体物理学、生物学、化学等，并获得了领域专家的评估和反馈。

Conclusion: Denario展示了AI驱动研究的潜力，但也存在局限性，需要进一步讨论AI科研的伦理问题和科学哲学意义。
...
<details>
  <summary>Details</summary>
Motivation: 基础模型在物理信息系统中增强了感知、推理和规划能力，但也引入了新的错误类型，如幻觉、过度泛化和上下文错位，导致错误决策。

Method: 引入认知包络概念，建立推理边界来约束AI生成的决策，同时补充元认知和传统安全包络的使用。

Result: 提出了认知包络的概念框架，强调需要实用的指导方针和系统化流程来定义、验证和保证认知包络。

Conclusion: 认知包络是解决基础模型在物理信息系统中决策错误的重要方法，需要建立系统化的实践指南和验证流程。
...
<details>
  <summary>Details</summary>
Motivation: 全球农业面临粮食需求增长、气候变化和可持续实践需求的挑战，需要转型性解决方案。

Method: 整合物联网、AI、卫星成像和基于角色的任务编排，特别针对地中海葡萄园定制卫星指数集成、实时环境数据和角色感知任务管理。
...
TL;DR: 研究表明，在非序列数据上应用因果掩码是可行的，在棋类等空间结构数据上，使用因果掩码训练的语言模型比基于序列数据训练的模型表现更好。


<details>
  <summary>Details</summary>
Motivation: 探索在具有空间或关系结构的数据上使用因果掩码的可行性，传统上认为因果掩码不适合此类数据，而是采用序列线性化方法。
...
TL;DR: 提出了自适应努力控制方法，通过强化学习训练模型使用用户指定的相对于当前平均思维链长度的token比例，实现动态的成本-精度权衡控制。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要用户预先指定token数量，但用户难以事先知道问题的难度。需要一种能根据问题难度自动分配推理资源的方法，让用户能动态调整成本-精度权衡。
...
Conclusion: 自适应努力控制方法有效解决了推理资源分配问题，让用户能够通过连续努力参数动态控制成本-精度权衡，模型能自动按任务难度比例分配资源。
...
TL;DR: 本文介绍了在NVIDIA企业AI助手NVInfo中实现的数据飞轮系统，通过MAPE驱动的闭环机制持续改进RAG管道的性能，显著提升了路由和查询重述的准确性和延迟表现。


<details>
  <summary>Details</summary>
Motivation: 企业AI代理需要持续适应以保持准确性、降低延迟并与用户需求保持一致。需要建立系统化的方法来处理RAG管道中的故障并实现持续学习。

Method: 采用MAPE驱动的数据飞轮构建闭环系统，收集用户反馈数据，识别主要故障模式（路由错误5.25%和查询重述错误3.2%），使用NVIDIA NeMo微服务进行针对性微调优化。

Result: 路由方面：用微调的8B模型替换70B模型，实现96%准确率、10倍模型大小减少和70%延迟改善；查询重述方面：微调带来3.7%准确率提升和40%延迟减少。

Conclusion: 人机回环反馈在数据飞轮框架下可将企业AI代理转变为自我改进系统，为构建能够在实际使用中大规模学习的稳健、自适应企业AI代理提供了可重复的蓝图。
...
TL;DR: 评估大语言模型在数学证明评分方面的能力，提出基于代理的工作流程来改进评分一致性，特别是在部分得分分配上。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在解决奥林匹克数学问题方面取得显著进展，需要评估它们在证明评分方面的能力，包括错误检测、严重性判断和公平得分分配。

Method: 使用90个Gemini 2.5 Pro生成的解决方案和MathArena数据集，引入基于代理的工作流程，提取参考解决方案并自动推导问题特定的评分标准。

Result: 模型能够可靠地标记错误解决方案，但在部分得分分配上存在校准差距。提出的工作流程在人类评分一致性和部分得分处理方面表现更好。

Conclusion: 基于代理的评分工作流程能够显著提高证明评分的准确性和一致性，特别是在处理部分得分方面。
...
TL;DR: Glia是一种基于LLM的多智能体AI架构，用于网络系统设计，能够生成可解释的设计方案，在分布式GPU集群上实现了与人类专家相当的性能。


<details>
  <summary>Details</summary>
Motivation: 探索AI是否能像人类专家一样自主设计计算机系统机制，特别是网络系统设计中的创造性推理能力。

Method: 使用LLM构建多智能体工作流，每个智能体专门负责推理、实验和分析，通过评估框架将抽象推理与实证反馈相结合。
...
TL;DR: 本文提出一个系统之系统(SoS)生命周期管理的实用框架，将MBSE作为语义骨干，PLM作为治理和配置层，CAD-CAE作为模型衍生领域，数字线程和数字孪生作为持续反馈。


<details>
  <summary>Details</summary>
Motivation: 传统线性生命周期模型已无法满足现代产品作为网络系统节点的需求，跨学科互操作性、变体配置管理、可追溯性和跨组织治理成为关键因素。

Method: 基于当前文献和行业经验，提出四个原则：(1)参考架构和数据模型，(2)端到端配置主权而非工具孤岛，(3)具有清晰审查门的策划模型，(4)沿时间、质量、成本和可持续性的可衡量价值贡献。

Result: 通过三阶段路线图实现从产品中心到网络中心开发的转变：参考架构试点、跨变体和供应链空间扩展、组织锚定（角色、培训、合规）。

Conclusion: 该框架可提高变更稳健性、缩短吞吐时间、改进重用，并为可持续性决策提供信息，帮助决策者和从业者管理复杂性并设计可扩展的SoS价值流。
...
TL;DR: 提出了一个细粒度的实例定制化引导框架，通过动态生成样本级干扰向量来增强LLM个性化适应能力，在快速变化分布和高数据稀疏场景下表现优异。
...
Method: 使用细粒度引导组件从注意力层和MLP层捕获细微信号，通过输入感知聚合模块将这些信号合成为上下文相关的增强向量，在模型前向传播中注入个性化适应。

Result: 在短文本到长文本生成、网络函数调用等多种场景的广泛实验验证了方法的有效性和兼容性，显著提升了快速变化环境中的个性化性能。

Conclusion: 该方法具有高度灵活性和数据效率，可作为插件组件与现有个性化技术兼容，在快速变化分布和高数据稀疏场景下表现出色。
...
TL;DR: 提出了GUI-Rise框架，通过结构化推理、动作预测和历史总结来增强多模态大语言模型在GUI导航中的表现，在跨域场景下取得最先进结果。
...
TL;DR: 本文提出了三种奖励机器（RM）的泛化形式来解决长时程无序子任务问题，并引入了基于耦合RM的组合学习算法CoRM，在包含无序子任务的长时程问题上比现有RM算法具有更好的扩展性。


<details>
  <summary>Details</summary>
Motivation: 传统奖励机器在处理长时程问题中无序子任务时存在局限性，当无序子任务数量增加时，需要学习的信息量呈指数级增长。

Method: 提出了三种RM泛化：(1)数值RM允许紧凑表达复杂任务；(2)议程RM在状态中关联跟踪剩余子任务的议程；(3)耦合RM将耦合状态与议程中的每个子任务关联。并开发了基于耦合RM的组合学习算法CoRM。

Result: 实验表明，CoRM在包含无序子任务的长时程问题上比最先进的RM算法具有更好的扩展性。

Conclusion: 通过引入RM的泛化形式和组合学习算法，有效解决了长时程无序子任务问题中的扩展性挑战。
...
TL;DR: 提出了一种基于结果感知的过程发现方法，通过区分期望和不期望的过程执行，分别学习过程模型，以揭示关键行为差异。
...
Conclusion: 结果感知的过程发现方法能够揭示期望和不期望执行的驱动因素，为过程改进提供更有针对性的见解。
...
Result: 发现LLM生成的算法虽然人类可读但对领域专家仍不透明，提出的新算法更简单、高效、可解释且泛化性更好。
...
Conclusion: ToolScope展示了强大的泛化能力，有效解决了长视野VQA任务中的视觉上下文退化问题。
...
TL;DR: 提出一个集成视觉和运动约束的多智能体强化学习框架，用于模拟行人-驾驶员交互，在真实无信号人行横道数据集上验证了约束模型的有效性。
...
Method: 使用多智能体强化学习框架，集成行人和驾驶员智能体的视觉和运动约束，评估四种模型变体（无约束、仅运动约束、仅视觉约束、两者皆有），在真实无信号人行横道数据集上进行行为指标评估。

Result: 结合视觉和运动约束的模型表现最佳。运动约束产生更平滑的运动，类似人类在穿越交互中的速度调整；视觉约束引入感知不确定性和视野限制，使智能体表现出更谨慎和可变的行为。在数据有限情况下优于监督行为克隆模型。

Conclusion: 多智能体强化学习结合人类约束是模拟真实道路用户交互的有前景的建模方法，通过将控制人类约束的参数建模为群体级分布来考虑个体差异，这是先前工作中未探索的视角。
...
TL;DR: 提出Nous智能体，通过主动询问来缩小人机协作中的意图表达差距，基于信息理论训练框架，无需人工偏好标注，在科学图表生成任务中表现出高效性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决人机协作中的"意图表达差距"问题，即人类难以向AI有效传达复杂高维想法，导致低效的试错循环，特别是对于不同专业水平的用户。

Method: 提出Socratic协作范式，训练Nous智能体主动探询信息以解决用户意图的不确定性。基于信息理论框架，将对话信息增益定义为内在奖励信号，等同于结构化任务空间中香农熵的减少。

Result: 在科学图表生成任务中，Nous实现了领先的效率和输出质量，对不同专业水平的用户保持鲁棒性。设计具有领域无关性，在图表生成之外也显示出泛化能力。
...
Method: DeepCompress采用自适应长度奖励机制，实时将问题动态分类为"简单"或"困难"，基于模型不断发展的能力。对简单问题鼓励更短、更高效的推理，对困难问题则促进更长、更具探索性的思维链。

Result: 在具有挑战性的数学基准测试中，DeepCompress始终优于基线方法，在显著提高token效率的同时实现了更优的准确性。

Conclusion: DeepCompress通过双重奖励策略使模型能够自主调整其思维链长度，为已掌握的问题压缩推理过程，为具有挑战性的问题扩展推理过程，从而同时提升准确性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在数学几何推理方面面临高质量几何数据稀缺的挑战，现有合成方法生成的数据缺乏多样性且与真实几何图偏差较大。

Method: 使用形式语言在度量空间中探索条件组合，通过符号引擎确保生成几何问题的正确性，产生与原始问题不同的高保真几何问题。

Result: 使用GeoFM合成数据训练的模型在MathVista几何问题解决任务上超过GPT-4o 18.7%，在GeoQA上超过16.5%；在开源模型上分别超过5.7%和2.7%。

Conclusion: GeoFM方法能有效生成高质量几何数据，显著提升多模态大语言模型在几何推理任务上的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖临时生成的数据集（可能包含偏见且无法验证），要么依赖形式化证明系统（如Lean，不适合捕捉基于决策链的任务），导致LLM推理基准在推理结构和现实对齐方面存在不足。

Method: 使用两个评估基准：时间轨迹评估(TTE)测试LLM理解和模拟多步推理系统执行的能力；时间因果评估(TCE)测试LLM进行多步因果推理和从复杂系统中提取因果关系的能力。

Result: 模型在TCE-normal上得分65.6%，在TCE-hard上仅得7.5%，表明最先进的LLM能理解TCE任务，但随着系统复杂性增加表现很差。
...
TL;DR: SIGMA是一个多代理检索增强框架，通过专业代理独立推理、定向搜索和协调机制，在数学推理任务上显著优于现有系统，性能提升7.4%。


<details>
  <summary>Details</summary>
Motivation: 当前检索增强模型存在单视角依赖、搜索策略僵化、多源信息整合困难等问题，无法有效支持复杂的数学推理。

Method: 采用多代理框架，每个代理从不同分析视角生成假设性段落优化检索，通过协调机制整合知识，实现上下文敏感且计算高效的知识集成。

Result: 在MATH500、AIME和GPQA等挑战性基准测试中，SIGMA持续超越开源和闭源系统，绝对性能提升7.4%。

Conclusion: 多代理按需知识集成显著提高了推理准确性和效率，为复杂知识密集型问题提供了可扩展的解决方案。
...
TL;DR: 提出了InnovatorBench基准平台，用于评估AI代理在LLM研究中的端到端能力，包含20个任务，涵盖数据构建、过滤、增强、损失设计、奖励设计和支架构建等环节。


<details>
  <summary>Details</summary>
Motivation: 现有的AI代理基准在简化环境中测试狭窄技能，无法真实评估科学发现中的自动化能力，需要更现实的端到端评估框架。

Method: 开发了ResearchGym研究环境，提供丰富的动作空间、分布式执行、异步监控和快照保存；实现了轻量级ReAct代理，将显式推理与可执行规划相结合。

Result: 前沿模型在代码驱动研究任务中表现良好，但在脆弱算法相关任务和长视野决策方面存在困难，如缺乏耐心、资源管理差和过度依赖模板推理。代理需要超过11小时才能达到最佳性能。

Conclusion: InnovatorBench具有足够的挑战性，有潜力成为下一代基于代码的研究基准，突显了AI代理在复杂科学研究中仍需改进的领域。
...
TL;DR: VeriMoA是一个无需训练的多智能体框架，通过质量引导缓存和多路径生成策略，显著提升硬件描述语言(HDL)生成的性能，在VerilogEval 2.0和RTLLM 2.0基准测试中Pass@1指标提升15-30%。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在硬件描述语言生成中面临的参数知识有限和领域特定约束问题，当前多智能体方法存在噪声传播和推理空间探索受限的缺陷。
...
Conclusion: VeriMoA通过协同创新有效解决了多智能体方法的关键缺陷，为HDL生成提供了高效、无需训练的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型驱动的具身智能体带来了新的安全风险：视觉后门攻击，智能体在正常行为直到视觉触发器出现时执行恶意策略。

Method: BEAT通过(1)构建跨场景、任务和触发器放置的训练集，(2)采用两阶段训练：监督微调(SFT)和对比触发器学习(CTL)，CTL将触发器识别建模为偏好学习。

Result: 在各种具身智能体基准和MLLM上，BEAT攻击成功率高达80%，同时保持良性任务性能，在有限后门数据下CTL将后门激活准确率提升39%。

Conclusion: 这些发现揭示了基于MLLM的具身智能体中关键但未探索的安全风险，强调了在实际部署前需要强大的防御措施。
...
TL;DR: 本文提出了Agentic AI的新现实主义定义，将其视为在复杂企业环境中自主工作的软件交付机制，强调验证比基础模型更重要。


<details>
  <summary>Details</summary>
Motivation: 当前对Agentic AI的定义不够清晰，需要基于现实应用场景重新定义，并强调验证的重要性。

Method: 提出Agentic AI作为软件交付机制的新定义，分析其与SaaS的相似性，强调验证工具和技术的必要性。

Result: 确立了Agentic AI的现实主义定义，指出在良好验证下，基础模型可被更简单、快速、可解释的模型替代。

Conclusion: Agentic AI的成功关键在于验证，而非基础模型本身；LLMs只是实现有效验证的选项之一。
...
Method: Apollo框架允许人类仅在智能体偏离有希望轨迹时进行干预，提供先验知识和策略建议，然后通过监督控制过滤次优动作以防止错误传播。

Result: 在InnovatorBench上的实验表明，使用Apollo训练的GLM-4.5模型相比未训练基线提升了50%以上，相比无人交互的变体提升了28%。
...
TL;DR: MolChord是一个基于结构的药物设计方法，通过整合文本描述和序列表示来对齐蛋白质和分子结构，并使用偏好优化指导分子生成，在CrossDocked2020数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决基于结构的药物设计中蛋白质结构表示与分子表示的有效对齐问题，以及确保生成药物与其药理学性质的一致性。

Method: 整合两种关键技术：(1)使用NatureLM自回归模型统一文本、小分子和蛋白质，结合扩散结构编码器对齐结构和序列表示；(2)通过整合偏好数据构建属性感知数据集，并使用直接偏好优化(DPO)改进对齐过程。

Result: 在CrossDocked2020数据集上的实验结果表明，该方法在关键评估指标上达到了最先进的性能。

Conclusion: MolChord展示了作为基于结构药物设计实用工具的潜力，能够有效生成具有所需性质的候选分子。
