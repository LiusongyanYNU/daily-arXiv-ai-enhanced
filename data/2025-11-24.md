...
TL;DR: 使用持续同调（PH）方法通过拓扑数据分析客观区分艺术风格，包括不同艺术家、同一流派艺术家，以及真实艺术家作品与AI生成作品的区分。
...
Method: 采用拓扑数据分析中的持续同调（PH）方法，对艺术作品进行客观和可解释的分析。

Result: PH方法能够在统计上显著区分不同艺术家、同一流派内的艺术家，并能辨别真实艺术家作品与AI生成的模仿作品。

Conclusion: 持续同调为艺术风格分析提供了客观、可解释的数学工具，能够有效识别艺术作品的独特特征。
...
TL;DR: RacketVision是一个用于体育分析的新型数据集和基准，涵盖乒乓球、网球和羽毛球，提供大规模细粒度球拍姿态和球位置标注，支持复杂人机交互研究。


<details>
  <summary>Details</summary>
Motivation: 推动体育分析中的计算机视觉研究，解决现有数据集缺乏细粒度球拍姿态标注的问题，支持动态物体跟踪、条件运动预测和多模态分析。

Method: 构建包含乒乓球、网球和羽毛球的大规模数据集，提供球拍姿态和球位置的细粒度标注，设计三个相互关联的任务：细粒度球跟踪、关节球拍姿态估计和预测性球轨迹预测。

Result: 评估显示，简单的球拍姿态特征拼接会降低性能，而CrossAttention机制能有效利用球拍姿态信息，使轨迹预测结果超越强单模态基线。

Conclusion: RacketVision为动态物体跟踪、条件运动预测和体育多模态分析提供了多功能资源和强起点，CrossAttention机制对多模态融合至关重要。
...
TL;DR: 提出了一种基于StyleGAN2的方法来生成猕猴面部表情，通过数据增强、样本选择和损失函数优化解决了训练数据不足的问题。
...
Method: 1) 使用运动转移技术通过计算机图形学为静态图像添加动画来进行数据增强；2) 基于初始训练的StyleGAN2模型的潜在表示进行样本选择，确保训练数据集的多样性和均匀采样；3) 损失函数优化以确保准确再现细微动作（如眼球运动）。

Result: 该方法能够为多个猕猴个体生成多样化的面部表情，性能优于仅使用原始静态图像训练的模型。模型在基于风格的图像编辑中也表现有效，特定风格参数对应不同的面部动作。
...
TL;DR: 提出了首个大规模双人肖像生成基准数据集PairHuman和基线方法DHumanDiff，能够生成符合摄影标准的高质量个性化双人肖像


<details>
  <summary>Details</summary>
Motivation: 个性化双人肖像定制在情感记忆保存和婚纱摄影规划等方面具有重要应用价值，但缺乏专门的基准数据集阻碍了高质量定制化双人肖像生成的发展

Method: 构建包含10万+图像的大规模PairHuman数据集，涵盖多样场景、服装和双人互动，并提供丰富元数据；提出DHumanDiff基线方法，增强面部一致性，平衡个性化人物生成和语义驱动场景创建

Result: 实验结果表明，所提出的数据集和方法能够生成高度定制化、视觉质量优秀且符合人类偏好的双人肖像
...
TL;DR: 提出了一种基于无监督自编码器和CDF 97小波变换的混合高斯-泊松去噪方法，用于中子成像数据的噪声去除，相比传统方法具有更好的重建误差和边缘保持能力。
...
Method: 使用无监督自编码器，在潜在空间中集成Cohen-Daubechies-Feauveau (CDF 97)小波变换，专门针对混合高斯-泊松噪声进行去噪处理。
...
### [6] [SAM 3: Segment Anything with Concepts](https://arxiv.org/abs/2511.16719)
...
TL;DR: SAM 3是一个统一的模型，能够基于概念提示（名词短语或图像示例）在图像和视频中检测、分割和跟踪对象，其准确率比现有系统提高了一倍。


<details>
  <summary>Details</summary>
Motivation: 推动可提示概念分割（PCS）的发展，通过概念提示实现对图像和视频中对象的检测、分割和跟踪。

Method: 构建可扩展的数据引擎生成高质量数据集，模型包含图像级检测器和基于记忆的视频跟踪器，共享单一骨干网络，通过存在头解耦识别和定位。

Result: SAM 3在图像和视频PCS任务中的准确率比现有系统提高了一倍，并改进了之前SAM在视觉分割任务上的能力。

Conclusion: SAM 3是一个强大的统一模型，为可提示概念分割设定了新的标准，并开源了模型和新的SA-Co基准测试。
...
TL;DR: 提出SaFeR-CLIP框架，通过将不安全概念重定向到语义最接近的安全替代概念，在保持安全性的同时显著提升视觉语言模型的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法通过强制对齐不安全概念到预定义安全目标来提升模型安全性，但这会破坏模型已学习的语义结构，导致泛化性能显著下降。

Method: 采用邻近感知方法，基于最小干预原则，将不安全概念重定向到语义最接近的安全替代概念，最小化表示变化。

Result: SaFeR-CLIP在保持鲁棒安全性的同时，比先前方法恢复高达8.0%的零样本准确率。还贡献了NSFW-Caps基准数据集用于更严格的评估。
...
TL;DR: 提出一个三阶段框架，从单视图SVG输入生成多视角一致的SVG，具有几何和颜色一致性，减少冗余路径并保留细节。


<details>
  <summary>Details</summary>
Motivation: 单对象SVG的多视角一致生成研究不足，需要解决从单视图输入生成几何和颜色一致的多视角SVG的挑战。

Method: 三阶段框架：1) 将栅格化输入提升为3D表示并渲染多视角图像；2) 扩展SAM2的时间记忆机制到空间域，构建空间记忆库建立跨视角部件级对应；3) 在栅格到矢量转换中执行路径整合和结构优化。
...
TL;DR: Mesh RAG是一个无需训练、即插即用的框架，通过检索增强生成来解决自回归网格生成模型中的序列依赖问题，显著提升网格质量、生成速度，并支持增量编辑。


<details>
  <summary>Details</summary>
Motivation: 传统网格制作耗时且难以扩展，现有自回归模型存在质量与速度的权衡问题，严格的序列依赖限制了生成效率和编辑灵活性。

Method: 受语言模型RAG启发，通过点云分割、空间变换和点云配准来检索、生成和集成网格组件，解耦生成过程的序列依赖。

Result: 在多种自回归网格生成模型上验证，显著提升网格质量、加速生成速度，并实现增量编辑功能。

Conclusion: Mesh RAG框架有效解决了自回归网格生成的序列依赖问题，实现了质量、速度和编辑能力的全面提升，无需模型重新训练。
...
TL;DR: REO-Instruct是首个面向地球观测的统一基准，结合描述性和回归任务，通过建立森林生态场景的认知逻辑链，连接定性理解和定量预测。
...
Method: 构建REO-Instruct数据集，整合配准的Sentinel-2和ALOS-2影像，通过混合人机流程生成和验证结构化文本标注，建立从人类活动到地上生物量回归的认知逻辑链。

Result: 对通用视觉语言模型的综合评估显示，当前模型在数值推理方面存在困难，突显了科学视觉语言模型发展的关键挑战。
...
TL;DR: BOP-ASK是一个用于物体交互推理的大规模数据集，包含超过15万张图像和3300万个问答对，涵盖6个任务，用于训练和评估视觉语言模型在精细空间理解方面的能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在空间推理基准测试中表现良好，但缺乏对物体交互的精细空间理解能力，如精确3D定位、物体物理兼容性、物体可供性和多步空间规划等。

Method: 利用BOP数据集中的6D物体姿态数据，生成精细标注，包括抓取姿态、参考物体姿态、路径规划轨迹、相对空间和深度关系以及物体间关系。

Result: 在BOP-ASK上训练的模型优于基线模型，并展现出精确物体和抓取姿态估计、轨迹规划以及在杂乱环境中进行精细物体中心空间推理等新兴能力。

Conclusion: BOP-ASK为训练和评估视觉语言模型提供了丰富资源，推动了模型在物体交互推理方面的发展，并公开发布了数据集和生成流程。
...
TL;DR: 提出了Parts-Mamba模型，这是一个结合图卷积网络和Mamba的混合模型，旨在解决骨架动作识别中因遮挡或通信质量问题导致的骨架不完整问题，通过在NTU RGB+D数据集上的实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 在真实场景中，由于人体部分遮挡或通信质量差，捕获的骨架往往不完整或存在缺失帧，现有GCN模型在这种情况下表现不佳，因为它们无法有效处理缺失的局部上下文信息。
...
Conclusion: Parts-Mamba模型能够有效增强从远距离关节捕获和保持上下文信息的能力，在骨架不完整的情况下显著提升了动作识别性能。
...
TL;DR: 提出了联合Gromov-Wasserstein（JGW）目标，将传统的成对GW距离扩展到支持多对一和多对多对象匹配，解决了传统GW只能处理单对象匹配的局限性。
...
Method: 通过引入JGW目标函数，扩展GW框架以支持对象集合的同步匹配，并采用最优传输中的传统算法（包括熵正则化）进行求解。

Result: 在部分匹配的基准测试中，该方法在准确性和计算效率上优于其他GW变体，在合成和真实数据集上的实验证明了其在多形状匹配（包括几何形状和生物分子复合物）中的有效性。

Conclusion: JGW方法为计算机图形学和结构生物学等不同领域中的复杂匹配问题提供了有前景的解决方案。
...
TL;DR: 提出REPA方法，通过在扩散或流模型与预训练自监督编码器之间进行表示对齐，来提升逆问题求解中的重建质量和感知真实感。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明，扩散或流模型与预训练自监督编码器的内部表示对齐能提供强大的归纳偏置，改善收敛性和样本质量。本研究将此思想扩展到逆问题领域，利用预训练生成模型作为先验。

Method: 在推理时应用表示对齐（REPA）技术，将扩散或流模型与预训练自监督视觉编码器（如DINOv2）进行对齐，以指导重建过程。尽管逆问题中缺乏真实信号，但通过将模型表示与近似目标特征对齐，可以显著提升重建保真度和感知真实感。

Result: 理论分析表明REPA正则化与DINOv2嵌入空间中的散度度量相关，且REPA更新能引导模型内部表示朝向干净图像表示。实验在超分辨率、框内修复、高斯去模糊和运动去模糊等任务上验证了方法的有效性，一致提升了重建质量，同时通过减少离散化步骤数量实现了效率提升。

Conclusion: REPA方法能显著提升逆问题求解的重建质量和感知真实感，同时提供效率增益，具有广泛的适用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有玻璃表面检测方法依赖边界线索或反射线索，未能充分利用玻璃本身的内在特性。观察到玻璃前后光照强度差异会导致反射变化，利用这一现象改进检测精度。

Method: 提出NFGlassNet，包含反射对比挖掘模块(RCMM)提取反射特征，反射引导注意力模块(RGAM)融合反射和玻璃表面特征。构建包含3.3K闪光/无闪光图像对的数据集进行训练。

Result: 大量实验证明该方法优于现有最先进方法，在玻璃表面检测任务上表现更优。

Conclusion: 通过利用闪光/无闪光图像中的反射动态特性，能够更准确地检测玻璃表面，为计算机视觉中的玻璃检测任务提供了新思路。
...
TL;DR: 提出了R-AVST数据集和AVST-Zero模型，用于解决现实世界音频-视觉时空推理任务。R-AVST包含5K+未剪辑视频和27K+对象，涵盖100种音频-视觉事件类型，AVST-Zero采用强化学习方法直接优化行为。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频理解任务中进展迅速，但主要关注简单视频场景，无法反映现实世界音频-视觉事件的复杂多样性。

Method: 构建R-AVST数据集：使用LLM提取关键对象、自动空间标注和人工质量检查；提出AVST-Zero模型：基于强化学习，避免中间监督，通过多维奖励直接优化行为。

Result: R-AVST数据集包含5,000+未剪辑视频、27,000+对象，生成8,000+高质量问答对；AVST-Zero在音频-视觉时空推理任务上表现出竞争力。

Conclusion: R-AVST是首个专为现实世界音频-视觉时空推理设计的数据集，AVST-Zero为该领域未来挑战提供了新颖解决方案。
...
TL;DR: 提出了Warm Diffusion模型，通过联合控制模糊和噪声来结合热扩散和冷扩散的优点，解决了热扩散忽略图像相关性和冷扩散忽略噪声作用的问题。


<details>
  <summary>Details</summary>
Motivation: 热扩散完全依赖噪声，未能利用图像高频细节与低频结构之间的强相关性；冷扩散仅使用模糊而忽略噪声在塑造数据流形中的作用，导致性能下降。需要结合两者的优势。

Method: 提出了Warm Diffusion（模糊-噪声混合扩散模型BNMD），采用分治策略利用图像频谱依赖性，通过解耦去噪和去模糊过程简化分数模型估计，并分析模糊-噪声比来研究模型学习动态与数据流形变化之间的权衡。

Result: 在多个基准测试上的广泛实验验证了该方法在图像生成方面的有效性。

Conclusion: Warm Diffusion成功结合了热扩散和冷扩散的优点，通过联合控制模糊和噪声实现了更好的图像生成性能。
...
TL;DR: Q-Real是一个用于AI生成图像细粒度评估的数据集，专注于真实性和合理性两个维度，包含3,088张图像，提供实体位置标注、判断问题和归因描述，并构建了评估多模态大语言模型的基准。


<details>
  <summary>Details</summary>
Motivation: 现有质量评估方法仅提供单一评分，过于粗糙，无法为生成模型提供针对性改进指导。在AI生成图像应用中，真实性和合理性是两个关键维度，细粒度评估对提升生成性能特别有效。

Method: 构建Q-Real数据集，包含3,088张由流行文本到图像模型生成的图像，标注主要实体位置，并提供真实性和合理性维度的判断问题和归因描述。设计微调框架并在多个多模态大语言模型上进行实验。

Result: 实验结果表明数据集质量高、意义重大，基准评估全面。

Conclusion: Q-Real数据集和基准为AI生成图像的细粒度评估提供了有效工具，有助于提升多模态大语言模型的评估能力，促进生成模型的优化改进。
...
Result: 在文本到图像合成和图像到文本理解任务上表现出强大的跨模态对齐能力和涌现的可控性，如图像-标题-图像的循环一致性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的异常生成方法通常需要少量异常样本进行微调，这与异常稀缺的实际情况相矛盾，且容易过拟合类别先验。本文旨在解决完全没有真实异常样本或训练数据的情况。

Method: 提出Delta-Denoising方法：1）通过最小提示对驱动两个扩散分支对比；2）累积每步去噪差异生成图像特定定位图；3）基于定位图指导潜在修复；4）进行词元级提示优化和对齐；5）在预测区域应用空间注意力偏置。

Result: 在公共数据集上的实验表明，DeltaDeno实现了优秀的生成质量、真实性和下游检测性能的一致提升。

Conclusion: DeltaDeno提供了一种无需训练、零样本的异常生成解决方案，通过对比扩散分支和定位引导修复，能够生成真实的局部缺陷同时保持周围上下文。
...
TL;DR: 提出了DGAF-VSR方法，通过特征域对齐和补偿解决基于扩散模型的视频超分辨率中的误差累积和空间伪影问题，在感知质量、保真度和时间一致性方面均优于现有方法。
...
Conclusion: 特征域比像素域更适合信息补偿，在放大分辨率下变形能更好保留高频信息，DGAF-VSR通过密集引导和对齐特征有效解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: CBCT图像中牙齿的精确分割对数字牙科至关重要，但牙间粘连会导致严重的解剖形状失真，现有方法难以处理
...
TL;DR: 提出了OmniGround基准测试和PG-TAF训练框架来解决视频时空定位中的类别偏见、推理简化和语言鲁棒性问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在真实世界多样化对象和复杂查询场景中表现不足，主要由于基准测试范围有限导致模型存在类别偏见、推理过于简化以及语言鲁棒性差的问题。

Method: 引入OmniGround基准测试（3,475个视频，81个类别），采用前向-后向-精化标注流程；提出PG-TAF训练免费的两阶段框架，将STVG分解为高级时序定位和细粒度时空传播。

Result: 在复杂真实场景中性能平均下降10.4%，特别是在小/遮挡对象和复杂空间关系上；PG-TAF在OmniGround上m_tIoU和m_vIoU分别提升25.6%和35.6%，在四个基准测试上均取得一致增益。

Conclusion: OmniGround揭示了现有STVG方法的局限性，PG-TAF框架通过分解任务显著提升了时空定位性能，为复杂真实世界场景提供了有效解决方案。
...
TL;DR: 提出了MultiPriv基准，用于系统评估视觉语言模型中的个体级隐私推理能力，揭示了现有模型在隐私推理方面存在重大未测量风险。


<details>
  <summary>Details</summary>
Motivation: 现代视觉语言模型展现出复杂推理能力，带来了超越简单属性感知的个体级隐私链接风险，而现有隐私基准在结构上无法应对这种新威胁。

Method: 提出了隐私感知与推理框架，构建了包含合成个体档案的双语多模态数据集，设计了9个挑战性任务来评估完整的隐私感知与推理谱系。

Result: 对50多个基础模型和商业模型的大规模评估显示：许多模型存在显著的未测量推理隐私风险；感知级指标无法有效预测推理风险；现有安全对齐措施对此类推理攻击无效。
...
<details>
  <summary>Details</summary>
Motivation: 传统运动补偿重建依赖预估计的光学流，但在欠采样情况下不准确且会降低重建质量，需要一种能联合优化图像和运动场的方法

Method: 使用两个隐式神经表示：一个参数化时空图像内容，另一个表示光学流，通过光学流方程进行物理约束，并结合数据一致性损失进行联合优化

Result: 在动态心脏MRI数据集上的实验表明，该方法优于最先进的运动补偿和深度学习方法，实现了更好的重建质量、准确的运动估计和改进的时间保真度

Conclusion: 基于流正则化约束的隐式联合建模在动态MRI重建中具有巨大潜力
...
<details>
  <summary>Details</summary>
Motivation: 理解细粒度人类手部运动对视觉感知、具身智能和多模态通信至关重要，但现有视频多模态大模型在稀疏RGB采样下难以捕捉细微的手指动态。

Method: 引入FiGOP方法，将每个RGB关键帧与后续手部关键点配对，通过轻量级时序编码器将关键点转换为运动嵌入并与RGB特征集成。

Result: 在FingerCap-40K数据集上的实验表明，现有视频多模态大模型在手指级推理上仍有困难，而FiGOP增强模型在HandJudge评估和人工研究中均取得一致提升。

Conclusion: FiGOP方法有效解决了时序稀疏性问题，能够恢复精细时序线索而不增加RGB密度，为细粒度手指运动理解提供了有效解决方案。
...
TL;DR: 提出了一种基于点监督的面部表情检测方法P-FES，使用单时间戳标注训练，通过高斯实例自适应强度建模和双分支框架实现表情实例检测与分类。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖成本高昂的时间边界标注，需要开发仅需单时间戳标注的点监督表情检测方法。

Method: 提出双分支框架：1) 类无关表情强度分支，使用高斯实例自适应强度建模(GIM)进行软伪标签分配；2) 类感知顶点分类分支，基于伪顶点帧区分宏表情和微表情。还引入强度感知对比损失。

Result: 在SAMM-LV、CAS(ME)^2和CAS(ME)^3数据集上的广泛实验证明了该框架的有效性。

Conclusion: 提出的P-FES框架在仅使用单时间戳标注的情况下，能够有效进行面部表情检测和分类。
...
TL;DR: 提出了Neighbor GRPO方法，通过扰动ODE初始噪声条件生成多样化轨迹，使用基于softmax距离的代理策略进行优化，完全绕开SDE，解决了SDE-based GRPO的信用分配低效和与高阶求解器不兼容的问题。


<details>
  <summary>Details</summary>
Motivation: 现有SDE-based GRPO方法在应用于流匹配模型时存在信用分配低效和与高阶求解器不兼容的问题，需要一种能保持确定性ODE采样优势的新方法。

Method: 从距离优化角度重新解释SDE-based GRPO，提出Neighbor GRPO：通过扰动初始噪声生成候选轨迹，使用softmax距离代理策略优化模型，引入对称锚点采样和组间准范数重加权。

Result: Neighbor GRPO在训练成本、收敛速度和生成质量方面显著优于SDE-based方法，完全保留了确定性ODE采样的效率和高阶求解器兼容性优势。

Conclusion: Neighbor GRPO提供了一种有效替代SDE-based GRPO的方法，通过距离优化视角实现了更好的对齐性能，同时保持了确定性采样的所有优势。
...
TL;DR: MatPedia是一个基于联合RGB-PBR表示的材质基础模型，通过将RGB外观和PBR属性编码为5帧序列，使用视频扩散架构统一处理多种材质任务。


<details>
  <summary>Details</summary>
Motivation: 现有材质生成方法缺乏统一表示，无法桥接自然图像外观和PBR属性，导致任务特定流程碎片化且无法利用大规模RGB图像数据。

Method: 提出联合RGB-PBR表示，将材质编码为两个相互依赖的潜在变量：RGB外观和4个PBR图，将其构建为5帧序列并使用视频扩散架构。

Result: 在MatHybrid-410K数据集上训练，实现原生1024×1024合成，在质量和多样性上显著超越现有方法。

Conclusion: MatPedia提供了一个统一框架，能够处理文本到材质生成、图像到材质生成和本征分解等多种材质任务。
...
TL;DR: 提出双分支退化提取网络来解决盲超分辨率问题，通过预测无监督的模糊和噪声嵌入，使SR网络能够分别适应不同类型的退化。


<details>
  <summary>Details</summary>
Motivation: 现有方法在已知固定退化（如双三次下采样）的SISR任务中表现良好，但当实际退化偏离这些假设时性能会显著下降。

Method: 使用双分支退化提取器网络预测两个无监督退化嵌入（模糊和噪声信息），将退化提取器作为正则化器利用SR和HR图像之间的差异。

Result: 在多个基准测试上的广泛实验表明，该方法在盲SR问题上达到了最先进的性能。

Conclusion: 所提出的方法能够有效处理盲超分辨率问题，通过分别适应模糊和噪声退化来提升性能。
...
TL;DR: 提出了一种边缘设备上从生食图像合成逼真熟食图像的方法，通过烹饪状态引导生成，显著提升了图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成方法在边缘设备上合成熟食图像时效果不真实或资源消耗过大，需要一种能在边缘设备上高效生成逼真熟食图像的方法。

Method: 引入首个基于烤箱的烹饪进度数据集，提出边缘高效的食谱和烹饪状态引导生成器，使用领域特定的烹饪图像相似度(CIS)指标确保时间一致性和烹饪合理性。

Result: 模型在多个数据集上显著优于现有基线，FID分数提升30%（自建数据集）和60%（公共数据集）。

Conclusion: 该方法成功实现了在边缘设备上从生食图像合成逼真熟食图像，为个性化烹饪视觉目标提供了有效解决方案。
...
TL;DR: 提出了SeeCLIP框架，通过细粒度语义增强解决开放集域泛化中的结构风险和开放空间风险困境，在多个基准测试上取得显著提升


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理域偏移和新类别时面临已知类结构风险和未知类开放空间风险的困境，特别是难以区分与已知类具有细粒度视觉相似性的'硬未知'样本

Method: 提出语义增强的CLIP框架，包括语义感知提示增强模块、双工对比学习和语义引导的扩散模块，通过分解图像为判别性语义标记、保持与已知类的分离性和语义邻近性、生成伪未知样本来学习更精细的决策边界

Result: 在五个基准测试上相比最先进方法持续提升了3%的准确率和5%的H-score
...
TL;DR: 提出了一种受自然选择启发的3DGS剪枝框架，通过优化梯度自主决定高斯原语的保留或剪枝，无需人工干预，在15%预算下比3DGS获得超过0.6 dB PSNR增益。
...
Method: 提出自然选择启发的剪枝框架，将生存压力建模为应用于不透明度的正则化梯度场，让优化梯度自主决定高斯原语的保留或剪枝。引入带有限不透明度先验的不透明度衰减技术加速选择过程。

Result: 与3DGS相比，在15%预算下获得超过0.6 dB PSNR增益，在紧凑3DGS方面达到最先进性能。

Conclusion: 该方法完全可学习且无需人工干预，通过自然选择机制有效解决了3DGS的存储和计算开销问题。
...
TL;DR: 提出了Synergistic Diversity (SQ)框架来改进植物病害检测，通过新的协同多样性度量SQ来优化集成模型选择，提高检测准确率。


<details>
  <summary>Details</summary>
Motivation: 植物病害每年造成超过2200亿美元经济损失，威胁粮食安全。现有的集成多样性度量方法难以识别最优集成团队，限制了深度集成方法在植物病害检测中的效果。

Method: 1. 分析现有集成多样性度量(Q度量)的局限性；2. 提出新的协同多样性度量SQ，捕捉集成成员间的协同效应；3. 在植物叶片图像数据集上进行广泛实验验证SQ方法的有效性。

Result: 实验表明SQ度量显著改进了集成模型选择，提高了植物病害检测的准确率。
...
TL;DR: FLUID是一个无需训练的框架，通过在预训练扩散模型的潜在空间中直接替换身份来实现人脸去识别，使用新颖的试剂损失指导优化，在身份抑制和属性保留方面达到最优平衡。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸去识别方法在身份抑制和属性保留之间存在权衡问题，需要一种更有效的方法来同时实现身份保护和视觉质量保持。

Method: 将身份编辑重新解释为预训练无条件扩散模型潜在h空间中的语义位移，通过优化发现身份编辑方向，使用线性测地线编辑方案在潜在流形中导航。

Result: 在CelebA-HQ和FFHQ数据集上的实验表明，FLUID在身份抑制和属性保留方面优于现有最先进的去识别方法，在定性和定量指标上都表现优异。

Conclusion: FLUID框架通过化学替换机制的启发，在扩散模型潜在空间中实现了有效的人脸去识别，为隐私保护提供了一种新的解决方案。
...
TL;DR: 提出RadioKMoE框架，结合KAN和MoE进行无线地图估计，通过KAN预测粗粒度覆盖图，MoE网络细化局部细节，提升准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 复杂的无线传播行为和周围环境对无线地图估计(RME)提出了挑战，需要更精确的估计方法。

Method: 使用KAN模块预测初始粗粒度覆盖图，结合环境信息驱动MoE网络进行精确估计，MoE包含专门处理不同无线地图模式的专家网络。

Result: 在多频段和单频段RME实验中，RadioKMoE在无线地图估计中展现出更高的准确性和鲁棒性。

Conclusion: RadioKMoE框架通过结合KAN和MoE的优势，有效提升了无线地图估计的精度和稳健性。
...
### [38] [Parameter-Free Neural Lens Blur Rendering for High-Fidelity Composites](https://arxiv.org/abs/2511.17014)
*Lingyan Ruan,Bin Chen,Taehyun Rhee*

Main category: cs.CV

TL;DR: 提出一种直接从RGB图像估计弥散圆(CoC)地图的新方法，无需场景深度或相机参数，实现逼真的镜头模糊渲染
...
Method: 通过线性关系从虚拟物体的有符号弥散圆地图推断深度值，并使用神经重模糊网络渲染逼真的镜头模糊

Result: 实验结果表明该方法能实现高保真度的合成效果，在定性和定量评估中都优于现有技术

Conclusion: 该方法为现实应用提供了灵活实用的解决方案，能够生成具有真实散景效果的混合现实合成
...
TL;DR: DReX是一个纯视觉模型，通过融合自监督和卷积表示来预测图像复杂度，在IC9600基准上达到SOTA性能，超越了多模态方法且参数更少。


<details>
  <summary>Details</summary>
Motivation: 探索语言信息对于图像复杂度预测是否必要，以及如何仅使用视觉特征实现与人类感知一致的复杂度预测。

Method: 提出DReX模型，通过可学习的注意力机制融合ResNet-50的多尺度层次特征和DINOv3 ViT-S/16的语义丰富表示，捕捉低层纹理模式和高层语义结构。

Result: 在IC9600基准上达到Pearson r = 0.9581，超越之前方法（包括多模态方法），参数减少约21.5倍，在多个数据集和指标上表现鲁棒。

Conclusion: 视觉特征本身足以进行人类对齐的复杂度预测，当适当融合时，自监督transformer和监督CNN对此任务具有互补和协同作用。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的深度很少是单一的，透射材料会产生层叠模糊，混淆传统感知系统。现有模型被动估计静态深度图，而人类能够主动调整焦点来感知所需深度。

Method: 引入DepthFocus，一个可操控的视觉变换器，通过标量深度偏好条件化，动态调整计算以聚焦于目标深度。训练主要使用新构建的50万多层合成数据集，捕捉多样透射效果。

Result: 在BOOSTER等传统单深度基准测试中达到最先进性能，并在新提出的多深度数据集上展示意图对齐的估计。在未见过的透射场景中表现出强大的泛化能力。

Conclusion: DepthFocus作为主动、类人3D感知的重要进展，展现出强大的鲁棒性，为复杂场景中的选择性感知提供了新方法。
...
### [41] [OmniPT: Unleashing the Potential of Large Vision Language Models for Pedestrian Tracking and Understanding](https://arxiv.org/abs/2511.17053)
*Teng Fu,Mengyang Zhao,Ke Niu,Kaixin Peng,Bin Li*

Main category: cs.CV

TL;DR: 提出OmniPT统一行人跟踪框架，利用LVLM实现基于参考的跟踪和语义理解，通过RL-中间训练-SFT-RL训练策略提升跟踪性能。


<details>
  <summary>Details</summary>
Motivation: LVLM在图像级任务表现出色，但在实例级任务如行人跟踪中仍有性能差距，而新兴的参考跟踪任务需要高级语义理解能力，这正是LVLM的优势所在。

Method: 采用RL-中间训练-SFT-RL三阶段训练策略：首先通过RL使模型输出固定格式边界框，然后使用大量行人相关数据集进行中间训练，最后在多个行人跟踪数据集上进行监督微调和RL优化。

Result: 在跟踪基准测试上的实验结果表明，该方法优于先前的方法。

Conclusion: OmniPT框架成功将跟踪任务建模为基础模型可执行的任务，实现了基于参考的交互式行人跟踪和语义理解。
...
### [42] [VLM-Augmented Degradation Modeling for Image Restoration Under Adverse Weather Conditions](https://arxiv.org/abs/2511.16998)
*Qianyi Shao,Yuanfan Zhang,Renxiang Xiao,Liang Hu*

Main category: cs.CV

TL;DR: 提出了一种统一的记忆增强视觉语言恢复模型，通过结合视觉语言模型和隐式记忆库来恢复各种恶劣天气条件下的退化图像。


<details>
  <summary>Details</summary>
Motivation: 在雨、雾、雪等恶劣天气条件下实现可靠的视觉感知对于自动驾驶和户外机器人至关重要但具有挑战性。

Method: 使用轻量级编码器-解码器主干网络，结合视觉语言模型进行链式推理编码天气退化先验，并通过隐式记忆库存储退化模式的连续潜在表示，通过动态交叉注意力机制自适应融合多尺度视觉特征。

Result: 在四个恶劣天气基准测试中，MVLR在PSNR和SSIM指标上超越了单分支和混合专家基线模型。
...
### [43] [ReBrain: Brain MRI Reconstruction from Sparse CT Slice via Retrieval-Augmented Diffusion](https://arxiv.org/abs/2511.17068)
*Junming Liu,Yifei Sun,Weihua Cheng,Yujin Kang,Yirong Chen,Ding Wang,Guosun Zeng*

Main category: cs.CV

TL;DR: ReBrain是一个基于检索增强扩散的框架，用于从稀疏CT扫描重建完整脑部MRI，通过检索相似CT切片作为参考，结合Brownian Bridge扩散模型和ControlNet分支确保结构连续性。


<details>
  <summary>Details</summary>
Motivation: 由于物理或临床限制，某些患者无法进行MRI检查，而低剂量CT协议导致稀疏CT体积，使得从CT合成完整脑部MRI具有挑战性。

Method: 使用Brownian Bridge扩散模型合成2D MRI切片，同时通过微调检索模型从先验数据库中检索结构和病理相似的CT切片作为参考，结合ControlNet分支和球形线性插值确保生成连续性。

Result: 在SynthRAD2023和BraTS数据集上的实验表明，ReBrain在稀疏条件下的跨模态重建中达到了最先进的性能。

Conclusion: ReBrain框架有效解决了从稀疏CT扫描重建完整脑部MRI的挑战，通过检索增强和扩散模型实现了高质量的跨模态重建。
...
TL;DR: ConfusedTourist是一个文化对抗鲁棒性测试套件，用于评估视觉语言模型在面临混合文化线索时的稳定性。研究发现，即使是最先进的模型在简单图像堆叠扰动下准确率也会大幅下降，显示出现有模型在文化多样性理解方面存在严重脆弱性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型评估通常只关注单一文化概念，忽略了现实世界中多种文化线索共存的情况。为了支持多元文化社会，需要测试模型在混合文化输入下的稳定性。

Method: 开发了ConfusedTourist测试套件，通过图像堆叠和基于图像生成的变体来扰动地理文化线索，评估模型在文化概念混合情况下的表现。

Result: 实验显示模型在简单图像堆叠扰动下准确率大幅下降，基于图像生成的扰动效果更差。可解释性分析表明失败源于系统性的注意力偏移到干扰线索。

Conclusion: 视觉文化概念混合会严重损害最先进视觉语言模型的性能，迫切需要开发更具文化鲁棒性的多模态理解方法。
...
### [45] [Spanning Tree Autoregressive Visual Generation](https://arxiv.org/abs/2511.17089)
...
<details>
  <summary>Details</summary>
Motivation: 传统自回归模型在视觉生成中采用随机排列序列顺序会导致性能下降或推理时序列顺序选择不灵活的问题。

Method: 利用图像块位置定义的网格中采样的均匀生成树的遍历顺序，通过广度优先搜索获得遍历顺序，确保连接的图像部分观察作为序列前缀出现。

Result: STAR在保持采样性能的同时，提供了后补完成的能力，无需对广泛采用的语言自回归模型架构进行重大更改。

Conclusion: STAR模型通过结构化的随机策略，成功平衡了采样性能和序列顺序灵活性，适用于图像编辑任务。
...
### [46] [A lightweight detector for real-time detection of remote sensing images](https://arxiv.org/abs/2511.17147)
*Qianyi Wang,Guoqiang Ren*

Main category: cs.CV

TL;DR: DMG-YOLO是一个轻量级实时检测器，专门用于遥感图像中的小目标检测，通过双分支特征提取、多尺度特征融合和全局局部聚合特征金字塔网络来提升检测性能。
...
Method: 设计了双分支特征提取模块（DFE）分别提取局部和全局特征，多尺度特征融合模块（MFF）增强多尺度信息整合，以及全局局部聚合特征金字塔网络（GLAFPN）进一步优化小目标检测。

Result: 在VisDrone2019和NWPU VHR-10数据集上的实验表明，DMG-YOLO在mAP、模型大小等关键指标上取得了有竞争力的性能。

Conclusion: DMG-YOLO通过有效的特征提取和融合策略，成功实现了遥感图像中小目标的轻量级实时检测。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统3D室内场景生成需要大量手动布局设计或全景图像指导的问题，实现仅通过短文本描述就能自动生成几何合理的3D室内场景。

Method: 1. 分层语言驱动代理规划器解析模糊提示为详细场景描述；2. 基于描述初始化3D点云；3. 实施两种排列约束迭代优化空间布局；4. 提出AnyReach采样和ITFS策略优化3D高斯场景表示。

Result: 能够生成几何合理的3D室内场景，在渲染速度和视觉质量上超越先前方法，同时保持可编辑性，总生成时间控制在30分钟以内。

Conclusion: RoomPlanner框架成功实现了仅通过短文本输入就能全自动生成高质量3D室内场景的目标，为3D场景生成提供了高效且用户友好的解决方案。
...
### [48] [Intervene-All-Paths: Unified Mitigation of LVLM Hallucinations across Alignment Formats](https://arxiv.org/abs/2511.17254)
*Jiaye Qian,Ge Zheng,Yuchen Zhu,Sibei Yang*

Main category: cs.CV

TL;DR: 本文提出了一个针对大型视觉语言模型幻觉问题的综合干预框架，发现幻觉源于图像-输入文本、图像-输出文本和文本-文本三个路径的相互作用，并提出了针对不同问答对齐格式的干预方法。


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型在各种任务中表现出色，但它们仍然容易产生幻觉问题，需要系统性的解决方案。

Method: 提出了与Transformer因果架构对齐的综合干预框架，识别并干预每个路径中的关键幻觉头，针对判别式和生成式格式进行定制化处理。

Result: 在多个基准测试上的实验表明，该方法能够持续减少不同类型对齐中的幻觉现象。

Conclusion: LVLMs的幻觉问题源于多个因果路径的相互作用，通过针对不同问答格式的路径干预可以有效缓解幻觉问题。
...
### [49] [PathAgent: Toward Interpretable Analysis of Whole-slide Pathology Images via Large Language Model-based Agentic Reasoning](https://arxiv.org/abs/2511.17052)
*Jingyun Chen,Linghan Cai,Zhikang Wang,Yi Huang,Songhan Jiang,Shenjin Huang,Hongpeng Wang,Yongbing Zhang*

Main category: cs.CV

TL;DR: PathAgent是一个基于大语言模型的训练免费代理框架，模拟病理学家的逐步分析方法，通过导航器、感知器和执行器模块自主探索全切片图像，形成可解释的预测链。
...
Method: 使用三个模块：导航器迭代精确定位重要微区域，感知器提取形态学视觉线索，执行器将发现整合到持续演化的自然语言轨迹中，形成显式思维链。

Result: 在五个挑战性数据集上的评估显示，PathAgent在零样本泛化方面表现强劲，在开放性和受限视觉问答任务中均超越任务特定基线。
...
### [50] [Range-Edit: Semantic Mask Guided Outdoor LiDAR Scene Editing](https://arxiv.org/abs/2511.17269)
*Suchetan G. Uppur,Hemant Kumar,Vaibhav Kumar*

Main category: cs.CV

TL;DR: 提出了一种基于语义掩码引导的LiDAR点云编辑方法，通过扩散模型生成新颖的合成LiDAR点云，用于增强自动驾驶系统的训练数据。


<details>
  <summary>Details</summary>
Motivation: 获取真实世界中复杂边缘案例的多样化点云数据具有挑战性，限制了自动驾驶系统的泛化能力和鲁棒性。现有方法依赖手工制作的3D虚拟环境，耗时且难以完全捕捉真实场景的复杂性。

Method: 将点云转换为2D距离视图图像，使用基于凸包的语义掩码进行条件引导，通过扩散模型实现语义编辑，确保几何一致性和真实性。
...
### [51] [Where Culture Fades: Revealing the Cultural Gap in Text-to-Image Generation](https://arxiv.org/abs/2511.17282)
...
TL;DR: 论文发现当前多语言文本到图像模型在跨语言提示下存在文化偏差问题，提出了两种对齐策略来增强文化一致性。


<details>
  <summary>Details</summary>
Motivation: 多语言文本到图像模型在视觉真实性和语义对齐方面进展迅速，但输出在不同文化背景下存在差异。语言承载文化内涵，多语言提示生成的图像应保持跨语言文化一致性，但现有模型往往产生文化中性或英语偏向的结果。

Method: 1. 提出探测方法定位文化敏感信号到少数固定层中的小神经元集合；2. 引入两种互补对齐策略：推理时文化激活（无需主干微调放大已识别神经元）和层定向文化增强（仅更新文化相关层）。

Result: 在CultureBench上的实验表明，相比强基线方法，该方法在文化一致性方面取得了一致改进，同时保持了保真度和多样性。

Conclusion: 当前T2I模型的文化偏差问题源于文化相关表征激活不足而非知识缺失，通过定位文化敏感神经元并针对性增强，可有效提升跨语言文化一致性。
...
### [52] [RL-AD-Net: Reinforcement Learning Guided Adaptive Displacement in Latent Space for Refined Point Cloud Completion](https://arxiv.org/abs/2511.17054)
*Bhanu Pratap Paregi,Vaibhav Kumar*

Main category: cs.CV

TL;DR: RL-AD-Net是一个基于强化学习的点云补全优化框架，通过在预训练点云自编码器的潜在空间中调整全局特征向量来提升几何一致性，并利用轻量级选择器保留更好的重建结果。


<details>
  <summary>Details</summary>
Motivation: 现有的点云补全模型虽然能生成全局合理的形状，但经常存在局部几何不一致的问题，需要一种轻量级、模块化的优化方法来提升几何保真度。

Method: 使用强化学习在预训练点云自编码器的潜在空间中调整全局特征向量，结合轻量级PointNN选择器评估几何一致性，选择保留更好的重建结果。

Result: 在ShapeNetCore-2048数据集上的实验表明，RL-AD-Net在各种裁剪场景下都能持续提升补全质量，特别是在随机裁剪情况下表现优于基线方法。

Conclusion: RL-AD-Net是一个轻量级、模块化且模型无关的优化框架，能够有效提升点云补全的几何一致性，适用于多种补全网络而无需重新训练。
...
TL;DR: 本文提出MuM模型，通过扩展MAE到任意多视图场景，使用统一掩码和轻量级解码器，在3D视觉任务中超越了DINOv3和CroCo v2。


<details>
  <summary>Details</summary>
Motivation: 大多数自监督学习方法主要优化语义理解而非几何推理，CroCo虽然针对3D理解但仍有改进空间，本文旨在学习更适合3D视觉的特征表示。

Method: 扩展MAE到任意多视图，对所有视图进行统一掩码，使用轻量级解码器并引入帧间注意力机制，相比CroCo更简单且可扩展性更强。

Result: 在多个下游任务评估中，包括前向重建、密集图像匹配和相对姿态估计，MuM模型均优于当前最先进的视觉编码器DINOv3和CroCo v2。

Conclusion: MuM通过多视图掩码自编码方法，成功学习了更适合3D视觉任务的特征表示，在多个几何推理任务中表现出色。
...
### [54] [REArtGS++: Generalizable Articulation Reconstruction with Temporal Geometry Constraint via Planar Gaussian Splatting](https://arxiv.org/abs/2511.17059)
*Di Wu,Liu Liu,Anran Huang,Yuyan Liu,Qiaoyu Jun,Shaofan Liu,Liangtu Song,Cewu Lu*

Main category: cs.CV

TL;DR: 本文提出了REArtGS++方法，通过时间几何约束和平面高斯泼溅技术，改进了一般化铰接物体重建和关节参数估计，解决了REArtGS在处理螺旋关节和多部件物体时的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法REArtGS在处理螺旋关节或多部件物体时表现不佳，且缺乏对未见状态的几何约束。需要一种更通用的铰接物体重建方法。

Method: 提出解耦螺旋运动建模，无需关节类型先验；通过部件运动混合联合优化部件感知高斯和关节参数；引入时间连续几何约束，鼓励高斯平面化，并通过泰勒一阶展开实现平面法向量与深度的时间一致性正则化。
...
### [55] [Designing and Generating Diverse, Equitable Face Image Datasets for Face Verification Tasks](https://arxiv.org/abs/2511.17393)
*Georgia Baltsou,Ioannis Sarridis,Christos Koutlis,Symeon Papadopoulos*

Main category: cs.CV

TL;DR: 提出DIF-V数据集解决人脸验证中的偏见问题，包含27,780张926个身份的合成人脸图像，用于促进更公平的人脸验证技术发展
...
Method: 使用先进的生成模型创建多样化的高质量合成人脸图像，强调各种面部特征的多样性表达，并确保符合身份证照片的特征要求
...
<details>
  <summary>Details</summary>
Motivation: 多通道图像领域（如细胞染色或卫星图像）中，现有方法在tokenization时独立处理每个通道，导致注意力计算呈二次增长，产生过高的计算成本和训练开销。

Method: 提出MoE-ViT架构，将每个通道视为专家，使用轻量级路由器为每个图像块选择最相关的专家进行注意力计算，避免建模所有通道交互。

Result: 在JUMP-CP和So2Sat真实数据集上的实验表明，MoE-ViT实现了显著的效率提升，同时保持甚至提升了性能。

Conclusion: MoE-ViT是多通道图像处理中实用且具有吸引力的骨干网络，在保证性能的同时大幅提升了计算效率。
...
### [57] [Diversity Has Always Been There in Your Visual Autoregressive Models](https://arxiv.org/abs/2511.17074)
...
Method: 分析发现特征图中的关键组件是早期尺度多样性形成的关键因素，通过抑制输入中的关键组件并放大输出中的关键组件来恢复多样性。

Result: 实证结果表明该方法显著增强了生成多样性，且对性能影响可忽略不计。

Conclusion: DiverseVAR有效解锁了VAR模型固有的生成潜力，同时保持了高保真合成能力。
...
TL;DR: 提出一种新颖的知识蒸馏框架，利用在任务相关数据子集上微调的教师网络来缓解学生网络中的捷径学习问题，在医学影像分析中取得显著效果。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型容易学习训练数据中虚假相关的捷径特征，在医学影像分析等高风险应用中，这可能导致模型不使用临床有意义的特征，影响鲁棒性和患者安全。

Method: 通过知识蒸馏框架，利用在无偏数据子集上微调的教师网络指导学生网络，针对不同层级的捷径特征进行缓解。在CheXpert、ISIC 2017和SimBA数据集上使用多种架构进行实验。

Result: 相比传统经验风险最小化、基于增强的偏缓解和基于组的偏缓解方法，该方法在所有数据集和架构上都取得了持续改进。在许多情况下，性能与在无偏数据上训练的基线模型相当，即使在分布外测试数据上也是如此。

Conclusion: 该方法在实际医学影像场景中具有实用价值，特别是在偏标注有限且捷径特征难以先验识别的情况下。
...
### [59] [SPAGS: Sparse-View Articulated Object Reconstruction from Single State via Planar Gaussian Splatting](https://arxiv.org/abs/2511.17092)
...
Method: 首先引入高斯信息场感知最优稀疏视角，然后将3D高斯压缩为平面高斯以精确估计法线和深度，通过深度平滑正则化和少样本扩散进行粗到细优化，并引入部件分割概率更新高斯基元。

Result: 在合成和真实世界数据上的实验结果表明，该方法比现有方法实现了更高保真度的部件级表面重建。

Conclusion: 该方法仅需稀疏视角输入即可实现高质量的铰接物体重建，具有较好的实用价值。
...
### [60] [Sparse Reasoning is Enough: Biological-Inspired Framework for Video Anomaly Detection with Large Pre-trained Models](https://arxiv.org/abs/2511.17094)
...
Method: 提出双通路框架：反射通路使用轻量级CLIP模块融合视觉特征与原型提示，查询动态记忆进行快速响应；意识通路使用中规模视觉语言模型生成文本事件描述和精炼异常分数，通过集成大语言模型定期审查识别未见异常。

Result: 在UCF-Crime和XD-Violence数据集上达到最先进的免训练性能，同时仅处理之前方法28.55%和16.04%的帧数。

Conclusion: 稀疏推理对于基于大模型的有效视频异常检测是足够的，ReCoVAD证明了选择性帧处理的可行性。
...
### [61] [REMSA: An LLM Agent for Foundation Model Selection in Remote Sensing](https://arxiv.org/abs/2511.17442)
*Binger Chen,Tacettin Emre Bök,Behnood Rasti,Volker Markl,Begüm Demir*

Main category: cs.CV

TL;DR: 本文介绍了RSFM数据库和REMSA智能体，用于解决遥感基础模型选择困难的问题。RS-FMD包含150多个模型，REMSA是基于LLM的智能体，能通过自然语言查询自动推荐合适的遥感基础模型。


<details>
  <summary>Details</summary>
Motivation: 遥感基础模型选择困难，因为文档分散、格式异构、部署约束多样。需要一个系统化的方法来帮助用户选择合适的模型。

Method: 1. 构建RSFM数据库，覆盖150多个模型的多模态数据、分辨率和学习范式；2. 开发REMSA智能体，使用LLM解释用户需求、补全缺失约束、排序候选模型并提供透明解释；3. 建立包含75个专家验证场景的基准测试。

Result: REMSA在900个配置的基准测试中表现优于多个基线方法（包括朴素智能体、密集检索和无结构RAG的LLM），完全基于公开元数据运行。

Conclusion: RS-FMD和REMSA为遥感基础模型选择提供了有效的解决方案，显著提升了模型选择的效率和准确性。
...
### [62] [Bridging Visual Affective Gap: Borrowing Textual Knowledge by Learning from Noisy Image-Text Pairs](https://arxiv.org/abs/2511.17103)
...
Method: 提出分区自适应对比学习(PACL)，关注社交媒体数据中图像和文本的事实与情感连接，分离不同类型样本并为每种类型设计不同的对比学习策略，通过动态构建正负对充分利用噪声样本潜力。
...
### [63] [Planning with Sketch-Guided Verification for Physics-Aware Video Generation](https://arxiv.org/abs/2511.17450)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法要么使用简单的单次规划，要么需要多次调用视频生成器进行迭代优化，计算成本高且运动质量有限。

Method: 提出草图验证框架：预测多个候选运动计划，使用视觉语言验证器评估语义对齐和物理合理性，通过将轨迹渲染为轻量级视频草图来高效评分，迭代优化直到找到满意的运动计划。

Result: 在WorldModelBench和PhyWorldBench上的实验表明，该方法显著提高了运动质量、物理真实性和长期一致性，同时效率更高。扩展轨迹候选数量能持续提升性能。

Conclusion: SketchVerify通过高效的草图验证循环，在保持计算效率的同时显著提升了视频生成中的运动规划质量。
...
### [64] [ChainV: Atomic Visual Hints Make Multimodal Reasoning Shorter and Better](https://arxiv.org/abs/2511.17106)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态推理模型在生成长推理链时存在冗余自反问题，而现有的CoT压缩方法依赖静态视觉参考，对多模态推理提升有限。

Method: ChainV首先基于前一步推理进行粗粒度视觉补丁选择，然后通过平均注意力强度识别最具代表性的原子视觉提示，并引入一致性评估机制来调整自反程度。
...
### [65] [PEGS: Physics-Event Enhanced Large Spatiotemporal Motion Reconstruction via 3D Gaussian Splatting](https://arxiv.org/abs/2511.17116)
*Yijun Xu,Jingrui Zhang,Hongyi Liu,Yuhan Chen,Yuanyang Wang,Qingyao Guo,Dingwen Wang,Lei Yu,Chu He*

Main category: cs.CV

TL;DR: PEGS是一个集成物理先验和事件流增强的3D高斯溅射框架，用于大时空尺度下的去模糊目标聚焦建模和运动恢复。
...
Method: 引入三重监督方案：加速度约束确保物理合理性，事件流提供高时间分辨率指导，卡尔曼正则器融合多源观测；设计基于实时运动状态的自适应模拟退火训练策略。

Result: 实验表明PEGS在大时空尺度运动重建方面优于主流动态方法，并贡献了首个针对自然快速刚体运动的RGB-事件配对数据集。

Conclusion: PEGS通过物理先验和事件流增强的集成，有效解决了大时空尺度刚体运动重建的挑战。
...
### [66] [Off the Planckian Locus: Using 2D Chromaticity to Improve In-Camera Color](https://arxiv.org/abs/2511.17133)
*SaiKiran Tedla,Joshua E. Little,Hakki Can Karaimer,Michael S. Brown*

Main category: cs.CV

TL;DR: 该论文提出了一种从传统的1D相关色温(CCT)插值转向2D色度空间的方法，使用轻量级多层感知器(MLP)来改进非普朗克光源(如LED)下的比色映射精度。


<details>
  <summary>Details</summary>
Motivation: 传统基于CCT的比色映射方法主要针对普朗克光源(如CIE A和D65)进行优化，但现代LED光源常常偏离普朗克轨迹，暴露了依赖一维CCT进行光照表征的局限性。

Method: 使用基于光箱的校准程序，结合代表性LED光源训练轻量级MLP，利用2D色度特征进行比色映射，替代传统的CCT插值方法。

Result: 在多种LED照明条件下验证，该方法平均减少LED照明场景中角度再现误差22%，保持与传统光源的向后兼容性，支持多光源场景和实时相机部署。

Conclusion: 从1D CCT空间转向2D色度空间显著提高了非普朗克光源下的比色精度，MLP方法在保持计算效率的同时提供了更鲁棒的色彩映射性能。
...
TL;DR: FireScope是一个基于视觉语言模型(VLM)的推理生成框架，通过结合强化学习和视觉监督来预测野火风险栅格图，并生成互补的推理轨迹，显著提升了跨大陆泛化能力。
...
Method: 提出FireScope-Bench数据集和基准，结合Sentinel-2影像、气候数据和专家定义的风险栅格；开发FireScope框架，基于VLM进行推理到生成，通过强化学习和视觉监督训练模型。

Result: 在美国训练并在欧洲测试时，FireScope实现了显著的性能提升；专家反馈和自动分析确认其推理轨迹忠实且语义有意义。

Conclusion: 基于语言的推理可以增强视觉生成模型的泛化能力；FireScope是首个展示语言推理能改善视觉生成泛化性的框架，也是首个可跨大陆应用的高分辨率野火风险模型。
...
TL;DR: 提出了一个多阶段优化框架，将高性能的浮点图像压缩模型转换为高效的8位整数FPGA实现，通过动态范围感知量化、混合精度搜索和通道剪枝等方法，在保持性能的同时显著降低计算复杂度。
...
Result: DRAQ将GDN模型的BD-rate开销从30%降至6.3%，硬件感知优化进一步降低20%以上计算复杂度，对RD性能影响可忽略，最终模型在效率和性能上都优于现有FPGA实现。
...
TL;DR: 本文系统评估了自监督表示在音频-视觉深度伪造检测中的应用，发现这些特征能捕获相关且互补的信息，但无法可靠地跨数据集泛化。
...
Method: 系统评估自监督特征在不同模态（音频、视频、多模态）和领域（唇部运动、通用视觉内容）的表现，考察检测效果、可解释性和跨模态互补性三个关键维度。

Result: 大多数自监督特征能捕获深度伪造相关信息且信息互补，模型主要关注语义相关区域而非虚假伪影，但无法可靠地跨数据集泛化。泛化失败源于数据集特性而非特征本身学习表面模式。
...
TL;DR: ODTSR是一个基于Qwen-Image的单步扩散变换器，通过噪声混合视觉流设计和保真度感知对抗训练，在真实世界图像超分辨率中同时实现高保真度和可控性。
...
Conclusion: ODTSR成功平衡了真实世界图像超分辨率中的保真度和可控性，展示了在单步推理下同时实现高质量重建和灵活控制的可行性。
...
TL;DR: 提出结合G-CNNs与等变感知结构化剪枝的框架，在保持几何变换不变性的同时减少计算开销，实现紧凑模型部署。


<details>
  <summary>Details</summary>
Motivation: 解决资源受限环境中等变网络的计算开销问题，同时保持几何变换的鲁棒性，特别适用于卫星图像分析等几何视觉任务。

Method: 使用e2cnn库实现C4循环群等变性，通过分析e2cnn层结构进行结构化剪枝，结合自适应微调、动态INT8量化和知识蒸馏的完整流程。

Result: 在EuroSAT、CIFAR-10和Rotated MNIST上验证，参数减少29.3%且精度恢复显著，保持几何鲁棒性的同时实现有效压缩。

Conclusion: 该框架为等变模型优化提供了可复现方案，弥合了群论网络设计与实际部署约束之间的差距。
...
Method: 引入基于CC-Metrics框架的CC-DiceCE损失函数，与现有的blob损失在nnU-Net框架中进行对比，使用DiceCE作为基线

Result: CC-DiceCE损失提高了检测召回率，分割性能基本不受影响，但略微增加了假阳性；在多数据集研究中普遍优于blob损失

Conclusion: CC-DiceCE损失函数在小病灶分割方面表现优异，能有效提高检测能力
...
### [73] [Non-Parametric Probabilistic Robustness: A Conservative Metric with Optimized Perturbation Distributions](https://arxiv.org/abs/2511.17380)
*Zheng Wang,Yi Zhang,Siddartha Khastgir,Carsten Maple,Xingyu Zhao*

Main category: cs.CV

TL;DR: 提出了非参数概率鲁棒性(NPPR)作为更实用的鲁棒性度量，不依赖预定义的扰动分布，直接从数据中学习优化的扰动分布，在分布不确定性下提供保守的概率鲁棒性评估。


<details>
  <summary>Details</summary>
Motivation: 现有的概率鲁棒性(PR)方法假设固定且已知的扰动分布，这在实践中不现实。需要一种更实用的PR度量来解决这一限制。

Method: 基于高斯混合模型(GMM)与多层感知机(MLP)头以及双三次上采样的NPPR估计器，覆盖各种输入依赖和输入独立的扰动场景。

Result: 在CIFAR-10、CIFAR-100和Tiny ImageNet数据集上，使用ResNet18/50、WideResNet50和VGG16进行广泛实验，验证NPPR作为更实用的鲁棒性度量，相比现有方法使用的常见扰动分布，PR估计保守性提高达40%。

Conclusion: NPPR是一个更实用的鲁棒性度量，能够在不依赖预定义扰动分布的情况下提供保守的概率鲁棒性评估，解决了现有PR方法在实际应用中的局限性。
...
### [74] [DiffRefiner: Coarse to Fine Trajectory Planning via Diffusion Refinement with Semantic Interaction for End to End Autonomous Driving](https://arxiv.org/abs/2511.17150)
...
<details>
  <summary>Details</summary>
Motivation: 传统的生成式方法（如扩散模型）依赖人工设计的轨迹锚点或随机噪声进行轨迹预测，存在改进空间。作者希望通过结合判别式方法的优势来提升扩散模型的性能。
...
Conclusion: DiffRefiner通过结合判别式轨迹提议和生成式精炼，显著提升了扩散模型在轨迹预测任务中的性能，消融研究也验证了各组件的有效性。
...
### [75] [UI-Styler: Ultrasound Image Style Transfer with Class-Aware Prompts for Cross-Device Diagnosis Using a Frozen Black-Box Inference Network](https://arxiv.org/abs/2511.17155)
*Nhat-Tuong Do-Tran,Ngoc-Hoang-Lam Le,Ching-Chun Huang*

Main category: cs.CV

TL;DR: UI-Styler是一种针对超声图像的类别感知风格迁移框架，通过模式匹配和类别感知提示策略，在保持源域结构内容的同时实现跨设备的语义对齐。


<details>
  <summary>Details</summary>
Motivation: 解决超声图像在不同采集设备间的域偏移问题，特别是在推理黑盒约束下，现有无配对图像翻译方法缺乏类别特定的语义对齐，影响诊断准确性。

Method: 使用模式匹配机制将目标图像纹理模式迁移到源图像，同时保持源结构内容；引入基于目标域伪标签的类别感知提示策略，确保与诊断类别的语义对齐。

Result: 在超声跨设备任务上的广泛实验表明，UI-Styler在分布距离和下游任务（如分类和分割）上持续优于现有UIT方法，达到最先进性能。

Conclusion: UI-Styler通过类别感知的风格迁移有效解决了超声图像跨设备域偏移问题，在保持诊断准确性的同时实现了更好的域适应效果。
...
Result: 该方法超越现有框架，消融研究证实了关键组件的有效性。数据集和代码已公开。
...
TL;DR: PostCam是一个新颖视角视频生成框架，支持动态场景中相机轨迹的后拍摄编辑，通过查询共享交叉注意力模块融合6-DoF相机位姿和2D渲染视频帧，实现精确的相机运动控制和高质量视频生成。
...
Method: 提出查询共享交叉注意力模块，将6-DoF相机位姿和2D渲染视频帧两种控制信号融合到共享特征空间中；采用两阶段训练策略：先学习粗略相机控制，再结合视觉信息细化运动精度和视觉保真度。

Result: 在真实世界和合成数据集上的实验表明，PostCam在相机控制精度和视角一致性方面比最先进方法提升超过20%，同时达到最高的视频生成质量。

Conclusion: PostCam通过创新的控制信号融合方法和两阶段训练策略，成功实现了更精确、灵活的相机运动操纵和高质量的新视角视频生成。
...
Conclusion: 所提出的多阶段噪声解耦框架通过分解复杂噪声并采用多阶段学习策略，能够有效处理真实高光谱图像中的复杂噪声，显著提升去噪性能。
...
Conclusion: 所设计的视觉和动作表示联合实现了机器人操作的空间平滑和时间一致性
...
TL;DR: 提出了CA-SAM方法，通过轻量级的Alignment Layer模块在SAM基础上进行持续学习，解决医学图像分割中的灾难性遗忘问题，在9个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，不同机构的隐私政策差异使得联合训练不可行，需要持续学习；SAM模型参数量大、计算开销高，限制了实际部署。

Method: 提出Alignment Layer轻量级模块对齐编码器-解码器特征分布，基于此构建CA-SAM持续学习策略，自动适配Alignment Layer来缓解灾难性遗忘。

Result: 在9个医学分割数据集的持续学习场景下，CA-SAM达到了最先进的性能。

Conclusion: SAM范式在平衡计算效率和性能后具有很大潜力，CA-SAM通过轻量级对齐层有效解决了医学图像分割的持续学习问题。
...
### [81] [SING3R-SLAM: Submap-based Indoor Monocular Gaussian SLAM with 3D Reconstruction Priors](https://arxiv.org/abs/2511.17207)
*Kunyi Li,Michael Niemeyer,Sen Wang,Stefano Gasperini,Nassir Navab,Federico Tombari*

Main category: cs.CV

TL;DR: SING3R-SLAM是一个基于高斯表示的全局一致紧凑型密集RGB SLAM框架，通过结合局部一致3D重建与统一全局高斯表示，联合优化场景几何和相机位姿，实现高效多用途3D建图。


<details>
  <summary>Details</summary>
Motivation: 解决密集3D重建在SLAM中的漂移和冗余点云问题，提高效率和下游任务（如新视角合成）性能。

Method: 先构建局部一致子图，然后逐步对齐融合到全局高斯地图中，强制跨视角几何一致性，全局地图反馈校正局部漂移。

Result: 在真实数据集上实现最先进的跟踪、3D重建和新视角渲染，跟踪精度提升超过12%，生成更精细几何细节，同时保持紧凑内存效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决体积CT面临的独特挑战：极端token扩展、几何各向异性、弱或无临床监督，使标准Transformer和对比学习方法难以直接应用。

Method: 联合优化局部Transformer进行高分辨率体积特征提取和全局Transformer进行全扫描上下文建模，结合DINO风格自蒸馏和基于SigLIP的视觉-语言对齐预训练。

Result: 在多个CT基准测试中，SPECTRE在零样本和微调设置下均优于之前的CT基础模型。

Conclusion: SPECTRE证明了无需依赖私有数据即可实现高性能、可泛化的CT表示，为3D医学成像建立了可扩展、开放且完全基于Transformer的基础模型。
...
TL;DR: 提出双域自适应网络，将预训练图像超分辨率模型从模拟数据集高效适配到真实世界数据集，通过空间域和频域自适应策略提升真实图像超分辨率性能。


<details>
  <summary>Details</summary>
Motivation: 真实世界图像超分辨率面临复杂退化模式，但缺乏足够的真实LR-HR数据。预训练SR模型包含有价值的先验知识，可改善泛化能力、加速训练并减少对真实数据的需求。

Method: 双域自适应网络：1) 空间域自适应策略，选择性更新预训练模型参数并使用低秩自适应技术调整冻结参数；2) 频域自适应分支，结合输入频谱数据和空间域骨干网络中间特征来推断HR频率图。

Result: 在RealSR、D2CRealSR和DRealSR等真实图像超分辨率基准测试中，该方法优于现有最先进模型。

Conclusion: 提出的双域自适应网络能有效利用预训练SR模型知识，在真实世界图像超分辨率任务中取得优越性能，代码已开源。
...
TL;DR: QueryOcc是一个基于查询的自监督框架，直接从相邻帧的4D时空查询中学习连续的3D语义占据，在保持恒定内存的同时实现长距离监督和推理。
...
Method: 引入基于查询的自监督框架，通过相邻帧的独立4D时空查询学习连续3D语义占据；支持从视觉基础模型生成的伪点云或原始激光雷达数据进行监督；采用压缩场景表示在保持近场细节的同时平滑压缩远场区域。
...
TL;DR: 提出了一种基于四元数保真度项的彩色图像盲去卷积方法，通过四元数卷积核建模颜色通道间的相互关系，有效提升去模糊效果。


<details>
  <summary>Details</summary>
Motivation: 现有彩色图像盲去卷积方法通常将彩色图像转为灰度图或单独处理每个颜色通道，忽略了颜色通道间的关系，导致去模糊效果不佳。

Method: 设计了一种新颖的四元数保真度项，使用包含四个卷积核的四元数卷积核：一个类似非负卷积核捕捉整体模糊，另外三个无约束卷积核分别对应RGB通道建模未知的相互依赖关系，并采用归一化四元数核来保持图像强度。

Result: 在真实模糊彩色图像数据集上的大量实验表明，该方法能有效去除伪影并显著改善去模糊效果。

Conclusion: 该方法展示了作为彩色图像去卷积强大工具的潜力，通过四元数建模颜色通道关系显著提升了盲去卷积性能。
...
TL;DR: 本文提出四种基于相关性反馈的视觉语言模型检索增强方法，包括伪相关性反馈、生成式相关性反馈、注意力反馈总结器和显式反馈，在Flickr30k和COCO数据集上验证了这些方法能提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型支持自然语言视觉搜索，但提升性能通常需要微调或使用更大模型。本文受传统文本搜索启发，提出在推理时使用相关性反馈机制来改进检索性能，这种模型无关的设计既可作为微调的替代方案，也可与微调后的模型配合使用。

Method: 提出四种反馈策略：1）伪相关性反馈（PRF）- 基于排名靠前结果优化查询嵌入；2）生成式相关性反馈（GRF）- 使用合成描述进行查询优化；3）注意力反馈总结器（AFS）- 基于Transformer的定制模型，整合相关项目的多模态细粒度特征；4）使用真实描述作为显式反馈的上界基准。

Result: 实验表明，与无反馈检索相比，GRF、AFS和显式反馈在较小VLM上提升MRR@5指标3-5%，在较大VLM上提升1-3%。AFS与显式反馈类似，能减轻查询漂移，在迭代多轮检索设置中比GRF更稳健。
...
TL;DR: MolSight是一个用于光学化学结构识别(OCSR)的三阶段学习框架，通过预训练、多粒度微调和强化学习后训练优化，在立体化学信息识别方面达到最先进性能。
...
Result: 实验结果显示MolSight在立体化学光学结构识别方面达到最先进性能，即使参数规模相对紧凑，GRPO算法也能进一步提升模型在立体分子上的表现。

Conclusion: MolSight框架有效解决了OCSR中立体化学信息识别的挑战，为化学信息学、药物发现和LLM应用提供了可靠的自动化分子结构识别解决方案。
...
TL;DR: BiFingerPose是一种基于双模态的指尖姿态估计算法，通过结合电容图像和指纹图像，能够准确预测全面的指尖姿态信息，包括传统方法无法估计的滚转角度。
...
Conclusion: BiFingerPose通过双模态方法显著提升了指尖姿态估计性能，能够可靠估计滚转角度，在增强认证安全性和改善交互体验方面具有实际应用价值。
...
TL;DR: 提出了SpatialGeo模型，通过融合几何和语义特征来增强多模态大语言模型的空间推理能力，在空间推理任务中准确率提升至少8.0%，同时减少50%的内存成本。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在三维空间的空间推理能力有限，主要由于视觉编码器（如CLIP）仅关注实例级语义特征，导致空间信息丢失。

Method: 提出基于几何和语义特征分层融合的视觉编码器，通过分层适配器将自监督学习的几何特征与CLIP语义特征结合，使用随机特征丢弃避免对CLIP编码器的依赖。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如NoPe-NeRF）仅关注图像局部关系，在复杂场景中难以恢复准确的相机姿态，需要一种更鲁棒的姿态估计方法。

Method: 采用相对姿态初始化与显式特征匹配，随后进行局部联合优化，最后引入包含特征轨迹的全局束调整阶段，结合几何一致性约束。

Result: 在基准数据集上的广泛评估表明，该方法在姿态估计精度和新视角合成质量上均优于现有最优方法，在挑战性场景中也表现出鲁棒性。

Conclusion: NoPe-NeRF++是首个将局部和全局线索与NeRF无缝结合的工作，验证了设计选择的有效性，为无姿态先验的NeRF训练提供了更优解决方案。
...
TL;DR: 本文提出了一种在文本到图像生成中准确渲染透明物体折射效果的方法，通过同步物体边界内外像素并使用斯涅尔折射定律进行扭曲和融合，显著提升了光学物理约束的准确性。


<details>
  <summary>Details</summary>
Motivation: 生成式图像模型在合成透明物体时表现不佳，特别是折射效果，因为折射光线会与图像其他部分的表面相交形成颜色约束，现有模型未能充分学习光学定律来准确渲染折射物体。

Method: 在生成过程的每一步中，使用斯涅尔折射定律对物体边界内外像素进行扭曲和融合同步；对于通过折射或反射可见但未直接观察到的表面，通过将图像与以物体为中心的全景图进行同步来恢复其外观。

Result: 该方法生成的图像在光学物理约束方面更加合理，折射效果显著改善。

Conclusion: 提出的方法能够生成更符合光学物理约束的图像，特别是在透明物体折射渲染方面取得了明显进步。
...
TL;DR: 提出了一个统一的多媒体绘画过程生成框架，通过语义驱动的风格控制机制和跨媒体风格增强，实现一致的纹理演化和过程转移。


<details>
  <summary>Details</summary>
Motivation: 现有的绘画教程视频缺乏交互性和个性化，而现有生成模型在跨媒体泛化和时间一致性方面存在困难，无法忠实再现人类创作流程。

Method: 将多种媒体嵌入扩散模型的条件空间，使用跨媒体风格增强和反向绘画训练策略，确保平滑且符合人类习惯的生成过程。

Result: 构建了大规模真实绘画过程数据集，在LPIPS、DINO和CLIP指标上取得良好结果，并通过感知距离剖面曲线定量建模创作序列。

Conclusion: 该框架能够生成连贯的多媒体绘画过程，准确反映人类艺术创作中的构图、色块和细节精炼等阶段。
...
Method: 学习一个新颖的采集函数，通过目标函数优化数据代表性、多样性和不确定性来选择最具信息量的标注子集；使用可逆GCN将数据从环境空间映射到潜在空间以更好地捕获数据分布。

Result: 在两个具有挑战性的骨架识别数据集上进行了广泛实验，证明该方法在标签使用效率方面优于相关工作。

Conclusion: 提出的标签节约型GCN方法在骨架动作识别中表现出有效性和优越性能。
...
TL;DR: DSeq-JEPA是一种改进的联合嵌入预测架构，通过引入判别性顺序预测机制，结合JEPA的潜在预测和GPT风格的顺序推理，提升视觉表示学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有I-JEPA方法对所有区域进行均匀独立预测，缺乏对人类视觉感知中选择性、顺序性注意力机制的模拟。

Method: 首先基于Transformer生成的显著图识别主要判别区域，然后按判别性顺序预测后续区域，形成从主要到次要线索的语义递进式学习。

Result: 在图像分类、细粒度视觉分类、检测分割及低级推理任务中，DSeq-JEPA比I-JEPA变体获得更具判别性和泛化性的表示。

Conclusion: DSeq-JEPA成功将预测式与自回归自监督学习相结合，通过顺序判别预测机制显著提升了视觉表示学习性能。
...
<details>
  <summary>Details</summary>
Motivation: 细胞级放射组学特征能提供肿瘤表型的细粒度洞察，但现有研究主要关注切片级或斑块级肿瘤分类，缺乏专门为放射组学数据设计的骨干网络。

Method: 引入统一的注意力-Mamba（UAM）骨干网络，灵活结合注意力和Mamba模块的能力，无需手动调整比例。开发了两种UAM变体，并基于此构建了多模态UAM框架，同时执行细胞级分类和图像分割。
...
TL;DR: SuperQuadricOcc使用超二次曲面表示替代高斯表示，实现了实时语义占据估计，内存占用减少75%，推理速度提升124%，性能提升5.9% mIoU。


<details>
  <summary>Details</summary>
Motivation: 高斯表示在自监督占据估计中需要大量基元，导致内存需求高且不适合实时推理。超二次曲面能减少基元数量和内存需求，但缺乏超二次曲面光栅化器阻碍了其应用。

Method: 提出SuperQuadricOcc方法，使用超二次曲面表示场景，通过多层二十面体细分的高斯近似实现高斯光栅化监督训练。

Result: 在Occ3D数据集上，相比高斯方法内存减少75%，推理速度提升124%，mIoU提升5.9%，基元数量减少84%，首次实现实时推理。

Conclusion: 超二次曲面表示能显著减少基元数量，实现实时语义占据估计，同时保持竞争力性能，为自动驾驶场景理解提供了高效解决方案。
...
TL;DR: 提出了ATAC方法，通过在CLIP嵌入空间中计算增强诱导的漂移向量来推断语义恢复方向，从而有效防御对抗性攻击，在多个基准测试中显著提升了鲁棒性。


<details>
  <summary>Details</summary>
Motivation: CLIP在零样本图像-文本匹配方面表现出色，但对图像对抗性扰动高度脆弱。由于对抗性微调成本过高，现有测试时防御方法鲁棒性有限，需要更有效的解决方案。

Method: ATAC方法直接在CLIP嵌入空间中操作，计算增强诱导的漂移向量来推断语义恢复方向，基于这些潜在漂移的角度一致性来校正嵌入。

Result: 在广泛基准测试中，ATAC始终实现极高的鲁棒性，平均比先前最先进方法提升近50%，同时计算开销极小。在非常规和极端设置下仍保持最先进鲁棒性，甚至对自适应攻击也表现出非平凡鲁棒性。

Conclusion: ATAC是CLIP嵌入空间中测试时对抗性防御新范式的有效方法，证明了在嵌入空间进行校正的高效性。
...
<details>
  <summary>Details</summary>
Motivation: 稀疏体素在空间上与邻居解耦且边界清晰，这使其在优化过程中容易陷入局部最小值。虽然SDF提供了平滑连续的几何场，但在独立参数化的稀疏体素间保持这种平滑性具有挑战性

Method: 1) 使用视觉几何模型进行鲁棒的几何初始化 2) 通过空间平滑损失在父-子和兄弟体素组间强制执行连贯关系

Result: 在多个基准测试中，该方法实现了强大的重建精度，同时具有持续快速的收敛速度

Conclusion: SVRecon方法在保持快速收敛的同时，显著提升了稀疏体素表面重建的精度和鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 解决医学图像配准中高维变形空间和体素级监督稀缺的挑战，现有强化学习方法将空间投影到粗糙低维表示，限制了捕捉空间变化变形的能力。
...
Result: 在三个公共AQA基准测试上，MCMoE在完整和不完整多模态学习中都达到了最先进的结果。
...
TL;DR: 提出了MMT-ARD多模态多教师对抗鲁棒蒸馏框架，通过双教师知识融合架构提升视觉语言模型的对抗鲁棒性，在ImageNet上实现了+4.32%的鲁棒准确率和+3.5%的零样本准确率提升。


<details>
  <summary>Details</summary>
Motivation: 传统单教师对抗蒸馏方法存在知识多样性有限、收敛速度慢、鲁棒性与准确性难以平衡的问题，需要更有效的多教师协作框架来提升视觉语言模型的对抗鲁棒性。
...
TL;DR: 提出Illustrator's Depth概念，将平面图像分解为可编辑的有序图层，实现图像向量化、文本到矢量图形生成、3D浮雕生成和深度感知编辑等应用。
...
Method: 提出新的深度定义，使用神经网络从光栅输入预测图层索引，通过离散的全局一致元素排序实现可解释的图像分解。

Result: 在图像向量化方面显著优于现有技术，同时支持文本到矢量图形生成、自动3D浮雕生成和直观的深度感知编辑。
...
TL;DR: 该论文研究了如何利用视觉基础模型(VFMs)在激光雷达点云语义分割的无监督域自适应中，通过知识蒸馏方法实现跨域泛化。


<details>
  <summary>Details</summary>
Motivation: 解决激光雷达语义分割网络在不同类型激光雷达之间无法良好泛化的问题，利用视觉基础模型提供的跨域鲁棒特征来缩小域偏移带来的性能差距。

Method: 基于无监督图像到激光雷达知识蒸馏，研究激光雷达主干架构对目标域泛化性能的影响，采用预训练主干+冻结权重+训练MLP头的策略。

Result: 在四个广泛认可且具有挑战性的设置中实现了最先进的结果，证明可以预训练一个主干网络并应用于多种域偏移场景。

Conclusion: 通过保持预训练主干冻结并训练MLP语义分割头的策略，能够有效利用视觉基础模型实现激光雷达点云语义分割的跨域泛化。
...
<details>
  <summary>Details</summary>
Motivation: 现有技术在处理具有细微差异的B扫描图像时难以准确估计距离，特别是在恶劣天气和环境条件下进行机器人/车辆定位时。

Method: 设计了一个新的定制神经网络，从连续时刻的B扫描图像中提取多尺度特征，然后通过分析这些特征之间的相似性和差异来确定行驶的欧几里得距离。

Result: 在CMU-GPR数据集上的实验表明，该方法在所有测试中都优于现有最先进方法，总体加权RMSE为0.449米，比最佳现有方法降低了10.2%的RMSE。

Conclusion: 该方法能够有效利用GPR B扫描图像的相似性和差异特征，在恶劣环境条件下实现更精确的机器人/车辆定位。
...
<details>
  <summary>Details</summary>
Motivation: 当前世界模型主要关注基于事实观察的前向模拟，但对于评估AI在不同条件下的物理行为等新兴应用，需要能够回答反事实查询的能力，比如"如果移除这个物体会发生什么？"
...
<details>
  <summary>Details</summary>
Motivation: 解决高频雷达信号3D形状重建的复杂性，现有深度学习方法难以表示任意形状或处理有限视角的真实雷达信号，而光学3D重建方法直接将雷达信号视为相机视图效果不佳

Method: 两阶段方法：首先学习具有层次分辨率形状特征的规范化潜在空间，然后通过以雷达信号频率为条件，以从粗到细的方式扩散到该潜在空间

Result: Radar2Shape能够成功从部分观测的雷达信号重建任意3D形状，并在两种不同仿真方法和真实数据上展现出鲁棒泛化能力
...
TL;DR: 提出一种基于计算机视觉的深度学习方法，使用超过18,000个MRI系列图像来估计脊柱年龄，并发现脊柱年龄差与退行性病变和生活方式因素相关。


<details>
  <summary>Details</summary>
Motivation: 脊柱是复杂的结构，容易发生与年龄相关的退行性病变，这些病变可以通过MRI识别。需要一种方法来量化脊柱健康状态。

Method: 使用UMAP和HDBSCAN识别年龄相关的脊柱退行性病变聚类，通过消融研究选择模型，计算脊柱年龄差(SAG)来评估临床效用。

Result: 发现SAG与椎间盘突出、骨赘、椎管狭窄、骨折等病变相关，也与吸烟和体力劳动等生活方式因素相关。

Conclusion: 脊柱年龄差可能作为衡量整体脊柱健康的有用生物标志物。
...
TL;DR: 本文研究了多模态模型中缩小LLM规模对视觉能力的影响，发现视觉能力下降比推理能力更严重，并提出Extract+Think方法来解决这个问题。


<details>
  <summary>Details</summary>
Motivation: 随着多模态模型的规模化发展，实际应用需要更小、更高效的系统。本文旨在分析在多模态模型中缩小LLM规模对视觉能力的影响。

Method: 采用视觉提取调优方法，明确训练模型提取与指令相关的视觉细节，然后应用逐步推理生成答案，形成Extract+Think方法。

Result: 研究发现LLM缩小规模对视觉能力的影响比对推理能力的影响更严重，Extract+Think方法在该领域实现了效率和性能的新标准。

Conclusion: LLM缩小规模会不成比例地影响多模态模型的视觉能力，Extract+Think方法通过分离视觉提取和推理过程，有效解决了这一瓶颈问题。
...
TL;DR: Video-R4是一个视频推理LMM，通过视觉反刍机制迭代选择帧、放大关键区域、重新编码像素来增强文本丰富视频的理解能力，在多个任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有视频QA模型依赖单次感知固定帧，容易产生幻觉和细粒度证据失败，而人类会暂停、放大和重读关键区域。

Method: 提出视觉反刍机制：迭代选择帧、放大信息区域、重新编码像素、更新推理状态；构建两个数据集用于监督实践和强化学习；采用多阶段反刍学习框架通过SFT和GRPO-based RL训练7B LMM。

Result: Video-R4-7B在M4-ViteVQA上达到SOTA，并能泛化到多页文档QA、幻灯片QA和通用视频QA任务。

Conclusion: 迭代反刍是像素接地多模态推理的有效范式。
...
Method: 设计基于事件的扩散模型，仅执行单次前向扩散步骤，配备时间一致的EvEncoder；提出新颖的代理训练框架，消除对配对事件-图像数据集的依赖，允许利用大规模图像数据集。

Result: 实验表明，EvDiff在保真度和真实感之间取得平衡，在像素级和感知指标上均优于现有方法，能够仅从单色事件流生成高质量彩色视频。

Conclusion: EvDiff通过扩散模型和代理训练框架，成功解决了事件相机图像重建的挑战，在减少计算成本的同时提升了生成视频的质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有指令引导的3D编辑方法存在优化方法速度过慢、基于多视图2D编辑的方法几何不一致和视觉质量下降的问题。

Method: 创建大规模多模态数据集用于指令引导3D编辑，探索交叉注意力机制和新型3D标记拼接两种条件策略。

Result: 标记拼接方法参数效率更高且性能更优，在生成质量、3D一致性和指令保真度方面优于现有2D提升方法。

Conclusion: 提出的原生3D编辑框架在效率和质量上均优于现有方法，为3D内容创作设定了新基准。
...
Result: 在三个LLM上对知识提取、冲突检测和推理任务的基准测试表明，所有模型都能执行Cognitive BASIC程序，总体性能强劲但不均匀。

Conclusion: Cognitive BASIC提供了一种结构化LLM推理的有效方法，使推理过程更加透明和可解释。
...
TL;DR: 提出一个利用响应模式统计分析的系统性基准修订框架，通过检测统计异常值来识别无效问题，结合专家审核和LLM初步筛选，提高基准可靠性。


<details>
  <summary>Details</summary>
Motivation: 基准测试对AI发展至关重要，但无效的基准问题会破坏其可靠性。手动在数千个问题中识别和修正错误不可行，成为可靠评估的关键瓶颈。

Method: 基于AI评估中常用的核心假设（均值足以总结模型性能），构建单维潜在结构，通过统计分析响应模式，检测统计值超出预期范围的问题项。结合LLM法官初步筛选和专家审核。

Result: 在9个广泛使用的基准测试中，该方法指导专家审核识别问题问题的精度高达84%。LLM法官初步筛选进一步减少了人工工作量。

Conclusion: 该框架提供了一个高效且可扩展的系统性基准修订方法，能够显著提高基准测试的可靠性。
...
TL;DR: 针对多车辆协同驾驶中传统状态奖励函数存在的奖励差异消失问题，提出混合差分奖励机制，包含基于全局势函数的时序差分奖励和直接测量动作边际效用的动作梯度奖励，显著提升算法收敛速度和策略稳定性。
...
Method: 提出混合差分奖励机制，包含时序差分奖励和动作梯度奖励两个互补组件，前者基于全局势函数利用势能演化趋势，后者直接测量动作边际效用提供高信噪比局部指导信号。

Result: 实验表明HDR机制显著提升收敛速度和策略稳定性，指导智能体学习到平衡交通效率和安全的高质量协同策略。

Conclusion: HDR机制有效解决了多车辆协同驾驶中奖励信号信噪比低的问题，为连续控制任务提供了有效的奖励设计方法。
...
TL;DR: 本研究设计了贝叶斯网络界面的语言和视觉扩展，通过用户研究发现这些扩展能显著提升用户对网络推理的理解，且语言和视觉模态结合使用效果最佳。


<details>
  <summary>Details</summary>
Motivation: 尽管贝叶斯网络被认为是透明模型，但用户仍难以理解其推理过程，现有用户界面未能有效阐明贝叶斯网络的推理逻辑。

Method: 设计了语言和视觉扩展来增强标准贝叶斯网络界面，通过用户研究比较了语言扩展、视觉扩展、组合扩展和基线界面的效果。

Result: 所有三种扩展界面在观察影响、影响路径以及观察间相互影响等问题上都优于基线界面；对于某些问题类型，语言和视觉模态结合使用比单独使用任一模态效果更好。

Conclusion: 语言和视觉扩展能有效提升用户对贝叶斯网络推理的理解，多模态界面设计是改善贝叶斯网络可解释性的有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI科学方法将科学发现视为孤立的优化过程，忽视了知识生产的社会性和历史性本质。人类科学洞察来源于个体认知轨迹和集体学科记忆两个相互关联的来源。

Method: 提出三层架构：个体层面构建研究者的认知模型（情景、语义、角色记忆）；领域层面将集体知识映射为结构化学科概念图；跨学科层面作为正交编排引擎。架构将记忆存储与智能执行分离。

Result: 在四个综合任务中评估：作者级认知模拟、互补推理、跨学科协作促进、多智能体科学问题解决。结果显示MirrorMind实现了超越简单事实检索的结构化、个性化和洞察生成型科学推理。
...
<details>
  <summary>Details</summary>
Motivation: 现有工具增强智能体在增加工具调用预算时性能无法提升，因为它们缺乏预算意识，无法有效利用额外资源。

Method: 提出预算跟踪器提供持续预算意识，并开发BATS框架动态调整规划和验证策略，基于剩余资源决定深入挖掘或转向新路径。

Result: 预算感知方法产生更有利的扩展曲线，推动了成本-性能帕累托边界，在预算受限的智能体中实现更好的性能。

Conclusion: 预算感知方法为工具增强智能体的扩展提供了更透明和原则性的理解，实现了更有效的成本-性能权衡。
...
TL;DR: 本文重新解释了扩散模型在逆问题求解中的作用，将其视为期望最大化框架中的初始化阶段，并提出DAPS++方法，通过更直接地利用似然项指导推理，实现了更高的计算效率和鲁棒的重建性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于分数的扩散方法在解决逆问题时，先验提供的指导有限，重建主要受测量一致性项驱动，导致推理过程与扩散动力学实际上解耦。需要澄清这种结构并改进方法。
...
Conclusion: 扩散在逆问题求解中的角色应被重新理解为初始化阶段，DAPS++通过解耦扩散阶段和数据驱动细化，提供了更有效的推理框架。
...
<details>
  <summary>Details</summary>
Motivation: 电子健康记录中既有结构化信息也有非结构化文本信息，需要开发能够同时利用这两种数据源的临床决策支持系统，特别是在高风险应用中需要大规模结构化表格数据和透明特征模型。

Method: 使用专家指导的贝叶斯网络处理结构化EHR特征，使用神经网络文本分类器处理临床笔记，通过虚拟证据和一致性节点进行概率融合，提高预测校准度。

Result: 在SimSUM模拟基准数据集上展示了方法的潜力，能够更好地处理缺失信息和解决表格与文本数据之间的矛盾。

Conclusion: 该方法提供了一种可解释的多模态信息提取框架，能够有效整合结构化EHR数据和临床文本，提高临床决策支持系统的性能。
...
TL;DR: 本文提出了一个形式化的BDI本体，作为模块化本体设计模式，捕捉智能体的信念、欲望、意图及其动态关系，并通过与LLMs结合和Semas推理平台集成验证其适用性。
...
Method: 设计了一个模块化本体设计模式，确保语义精确性和可重用性；通过两种实验验证：与LLMs结合评估本体基础对推理一致性的贡献，以及在Semas推理平台中集成实现RDF三元组与智能体心理状态的双向流动。

Result: 实验表明BDI本体在声明性和程序性智能之间起到了概念和操作桥梁作用，为认知基础、可解释和语义可互操作的多智能体及神经符号系统铺平了道路。

Conclusion: BDI本体为认知基础、可解释和语义可互操作的多智能体及神经符号系统在数据网络中的运行提供了有效支持。
...
TL;DR: 提出了MIR方法，通过激励智能体探索影响队友的行为来解决多智能体强化学习中稀疏奖励问题，在MiniGrid-MA环境中表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中，联合动作轨迹的指数级稀疏性和现有方法未能考虑影响团队状态的联合动作，导致稀疏奖励场景下性能不佳。

Method: 提出了MIR方法，激励个体智能体探索影响队友的动作，结合原始策略有效促进团队探索。创建了MiniGrid-MA环境用于实验验证。

Result: 在MiniGrid-MA环境中，MIR方法相比现有最先进方法表现出优越性能，有效提升了算法表现。

Conclusion: MIR是一种简单有效的多智能体强化学习增强策略，特别适用于稀疏奖励场景，能有效解决团队探索问题。
...
TL;DR: 提出了基于分层任务抽象机制(HTAM)的多智能体框架EarthAgent，专门用于解决遥感等专业领域中的复杂多步骤任务规划问题，显著优于现有的单智能体和多智能体系统。


<details>
  <summary>Details</summary>
Motivation: 现有的通用智能体框架(如ReAct或角色扮演)在需要严格结构化工作流程的专业领域表现不佳，特别是在遥感等需要专业工具和多步骤程序的领域。
...
TL;DR: 该论文主张将AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具作为实现智能体AI的基础，以构建既具备能力又透明、合作和可问责的智能体系统。


<details>
  <summary>Details</summary>
Motivation: 为了实现具有持续自主性、推理和交互能力的智能体AI系统，需要明确认知、合作和治理模型来补充其关于智能体的假设。

Method: 将自适应、数据驱动的方法与结构化的推理和协调模型相结合，利用AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具。

Result: 提出了一种连接形式理论和实际自主性的智能体视角，为构建透明、合作和可问责的智能体系统奠定了基础。

Conclusion: AAMAS社区的概念工具为实现既具备能力又透明、合作和可问责的智能体系统提供了必要的理论基础，有助于弥合形式理论与实际自主性之间的差距。
...
TL;DR: 评估使用合成和离策略数据训练LLM行为探测器的泛化能力，发现响应生成策略显著影响探测器性能，同领域离策略数据比不同领域策略数据更可靠。


<details>
  <summary>Details</summary>
Motivation: 由于许多LLM行为的自然样本稀少，研究者不得不依赖合成或离策略数据训练探测器，需要系统评估这些数据对探测器泛化能力的影响。

Method: 在八种不同LLM行为上测试线性和注意力探测器，比较不同响应生成策略对探测器性能的影响，分析从离策略数据到策略数据的泛化能力。

Result: 响应生成策略显著影响探测器性能，从离策略到策略数据的成功泛化可预测实际监控效果，领域偏移比策略偏移造成更大性能下降。

Conclusion: 在缺乏策略数据时，使用同领域离策略数据比不同领域策略数据产生更可靠的探测器，需要更好处理LLM监控中分布偏移的方法。
...
TL;DR: 提出SRA-CP框架，通过风险感知的协同感知机制，在保持安全关键目标检测精度的同时，将通信带宽降低至传统方法的20%。


<details>
  <summary>Details</summary>
Motivation: 现有协同感知方法传输大量与驾驶安全无关的感知数据，超出通信带宽限制，且依赖预定义通信伙伴，不适合动态交通环境。

Method: 采用去中心化协议，车辆持续广播轻量级感知覆盖摘要，仅在检测到风险相关盲区时启动针对性合作。包含感知风险识别模块和选择性信息交换融合模块。

Result: 相比通用协同感知方法，SRA-CP在安全关键目标上的平均精度损失小于1%，同时仅使用20%的通信带宽。相比现有选择性协同感知方法，感知性能提升15%。

Conclusion: SRA-CP框架通过风险感知的选择性协同感知，有效解决了通信带宽限制和动态环境适应性两大挑战，在保持感知精度的同时显著降低通信开销。
...
TL;DR: PepBridge是一个用于联合设计蛋白质表面和结构的新框架，通过扩散桥模型和多模型扩散方法，从受体表面生成完整的蛋白质结构，确保表面互补性和结构稳定性。


<details>
  <summary>Details</summary>
Motivation: 蛋白质-蛋白质相互作用由表面互补性和疏水相互作用控制，但设计能够精确互补目标受体的多样化且物理真实的蛋白质结构和表面仍是一个重大挑战。
...
Conclusion: PepBridge代表了自上而下蛋白质结构联合设计的重要进展。
...
<details>
  <summary>Details</summary>
Motivation: 时间序列预测通常需要大规模数据集和大量计算资源，数据集蒸馏提供了一种替代方案，但扩展到时间序列面临两个挑战：1) 强自相关性导致的时间偏差；2) 缺乏显式类别先验导致的样本多样性不足。

Method: 基于一阶压缩分解构建DDTime框架：1) 通过频域对齐机制缓解自相关引起的时间偏差；2) 基于信息瓶颈原理设计样本间正则化来增强多样性；3) 支持稳定的一阶优化。

Result: 在20个基准数据集和多种预测架构上的实验表明，DDTime始终优于现有蒸馏方法，实现约30%的相对精度提升，同时仅引入约2.49%的计算开销。

Conclusion: DDTime提供了一个轻量级且即插即用的时间序列蒸馏解决方案，有效解决了时间序列蒸馏中的关键挑战，在保持计算效率的同时显著提升预测精度。
...
TL;DR: 研究表明，在文本属性图上，仅使用节点文本描述的LLM已经能取得良好性能，大多数结构编码策略只能带来边际收益甚至负收益。


<details>
  <summary>Details</summary>
Motivation: 探索不同图结构编码策略如何影响LLM在文本属性图上的性能，挑战传统图学习中结构信息必然有益的假设。

Method: 通过系统实验比较不同图结构编码策略，包括基于模板的图模板和GNN编码结构信息的方法。

Result: 发现LLM仅利用节点文本描述就能在各项任务中取得强劲表现，大多数结构编码策略只能提供边际收益或负收益。

Conclusion: 当使用强大的语言模型时，显式的结构先验往往不必要，有时甚至适得其反，需要重新思考在LLM时代如何表示和利用结构信息。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖同质性假设和硬优化目标，在处理异质性图时性能受限，且通常将文本嵌入视为静态目标，导致对齐效果不佳

Method: 使用最优传输的图对比学习框架，包含RealSoftMax相似度估计器、基于提示的过滤器以及OT引导的软监督机制

Result: 在9个基准数据集上持续优于最先进方法，验证了有效性和鲁棒性

Conclusion: GCL-OT能够有效处理多粒度异质性，通过最优传输实现结构与文本的灵活对齐，显著提升性能
...
TL;DR: FlashCache是一个基于频域分析和异常KV感知的多模态KV缓存压缩框架，通过识别和保留对推理至关重要的异常KV对，在保持任务性能的同时显著提升解码速度并降低内存使用。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在显著推理开销，因为多模态KV缓存随视觉输入长度增长。现有压缩方法主要依赖注意力分数，与高效注意力内核不兼容且忽略了值向量对注意力输出的贡献。

Method: 1. 从KV矩阵分布角度分析，发现多模态KV矩阵的频域能量集中在低频；2. 提出异常KV识别模块，在频域建模KV矩阵主成分，优先保留显著偏离的KV对；3. 设计动态预算分配模块，自适应确定每层KV缓存大小以保留更多异常KV。

Result: 在多个MLLM和基准测试中，FlashCache优于最先进的多模态KV压缩方法，实现高达1.69倍解码加速和80% KV内存使用降低，同时保持任务性能。

Conclusion: FlashCache通过频域引导的异常KV感知压缩框架，有效解决了多模态KV缓存压缩问题，在保持性能的同时显著提升效率。
...
Method: 使用向量符号架构，将实例和概念表示为近似正交的高维向量，通过代数运算在分类时强制执行iff约束，并设计学习编码器将原始数据转换为VSA兼容向量。
...
TL;DR: 本文比较了FedAvg、FedProx和Scaffold三种联邦学习算法在不同数据分布下对IoT攻击检测的性能，重点关注统计异质性挑战。


<details>
  <summary>Details</summary>
Motivation: 随着IoT设备激增和数据量增长，传统机器学习面临资源受限和隐私保护挑战。联邦学习虽能解决这些问题，但非IID数据的统计异质性严重影响了其有效性，现有研究缺乏对IoT攻击检测场景下联邦学习方法的全面比较。

Method: 使用CICIoT2023数据集进行大规模IoT攻击分类，系统比较FedAvg、FedProx和Scaffold三种联邦学习算法在不同数据分布下的表现。

Result: 通过细致分析和实验，揭示了这些联邦学习方法在统计异质性环境下的性能差异和特点。

Conclusion: 研究为IoT攻击检测领域的联邦学习应用提供了有价值的性能洞察，有助于研究者和从业者选择适合的联邦学习方法应对统计异质性挑战。
...
TL;DR: 提出了MOCET指标，一种可解释且双重可扩展的指标，用于量化AI模型在生物安全等领域的现实世界风险。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标如LAB-Bench、BioLP-bench和WMDP能够可靠评估模型提升能力和领域知识，但缺乏更好量化"现实世界风险"的指标，需要可扩展的开放式指标来跟上LLM的快速发展。

Method: 引入MOCET指标，该指标具有可解释性且双重可扩展（可自动化和开放式），能够量化现实世界风险。

Result: MOCET指标能够解决现有评估指标的不足，为LLM安全案例提供更好的风险背景化评估。

Conclusion: MOCET指标填补了现有评估指标的空白，为评估AI安全级别威胁提供了更有效的工具，特别是在生物安全等高风险领域。
...
TL;DR: 本文系统比较了Transformer、Transformer++和Mamba三种架构在临床预测任务中的表现，发现Llama（Transformer++）在瑞典心衰队列中表现最佳，其次是Mamba架构。


<details>
  <summary>Details</summary>
Motivation: 虽然Transformer和Mamba架构在临床预测任务中表现出色，但医学领域缺乏系统分析模型性能和效率的方法。

Method: 在瑞典42820名心衰患者队列中，评估了六种序列模型在三个架构类别中的表现，包括输入序列修改、架构配置和时间预处理技术的消融实验。

Result: Llama实现了最高的预测区分度和最佳校准，在所有任务中表现稳健；Mamba紧随其后。两种架构都展示了高效的表征学习能力，小配置模型超越其他大规模Transformer。

Conclusion: 本研究为临床预测任务中的输入标记化、模型配置和时间数据预处理提供了首个系统消融研究，可作为未来模型开发的起点。
...
<details>
  <summary>Details</summary>
Motivation: 序数分类在医疗诊断等高风险应用中需要可靠的不确定性量化。现有方法要么是启发式算法，要么要求模型预测单峰分布，无法提供覆盖效率权衡的深入理解，也不具备保形预测方法所期望的模型无关性和分布无关性。

Method: 将序数保形分类建模为实例级最小长度覆盖问题，开发了滑动窗口算法，在每次校准数据上达到最优，时间复杂度为线性。还提出了长度正则化变体，在保持覆盖的同时缩小预测集大小。

Result: 在四个不同领域的基准数据集上的实验表明，所提方法相比基线显著提高了预测效率（平均降低15%）。

Conclusion: 提出的序数保形预测方法填补了现有方法的空白，具有模型无关性，提供实例级最优预测区间，并在实际应用中显著提高了预测效率。
...
TL;DR: 本研究利用被动声学监测和人工智能模型，通过欧洲龙虾的生物声学特征成功分类其年龄和性别，为龙虾保护和管理提供非侵入性监测方法。


<details>
  <summary>Details</summary>
Motivation: 监测水生生物（特别是像龙虾这样的难以追踪物种）具有挑战性。了解龙虾的栖息地、福利、繁殖、性别和年龄对于管理和保护至关重要。

Method: 在苏格兰Johnshaven使用水听器收集数据集，探索深度学习模型（1D-CNN、1D-DCNN）和六种机器学习模型（SVM、k-NN、朴素贝叶斯、随机森林、XGBoost、MLP），使用MFCC作为特征。

Result: 年龄分类准确率超过97%（朴素贝叶斯为91.31%），性别分类除朴素贝叶斯外所有模型超过93.23%。

Conclusion: 研究证明了监督机器学习和深度学习从龙虾声音中提取年龄和性别相关特征的潜力，为水产养殖和渔业中的龙虾保护、检测和管理提供了有前景的非侵入性PAM方法。
...
TL;DR: 研究发现，在听觉任务中表现更好的自监督音频模型，其内部表征与大脑听觉皮层活动的相似度也更高，这表明大脑相似性可能是从自然音频数据中学习重建缺失信息的副产品。
...
Method: 使用36种不同音频模型，通过体素回归、成分回归和表征相似性分析，比较模型内部表征与两个独立fMRI数据集的大脑活动。评估模型在HEAREval基准的6个听觉任务中的表现。

Result: 发现近期自监督音频模型比旧模型更能预测听觉皮层活动，模型整体任务性能与大脑表征对齐度呈强正相关（r>0.7）。EnCodecMAE预训练过程中，大脑相似性早期出现并逐步增加。

Conclusion: 大脑相似表征可能是从自然音频数据学习重建缺失信息的副产品，无需显式优化即可获得。
...
<details>
  <summary>Details</summary>
Motivation: 帕金森病早期常伴有语音障碍，利用语音生物标志物进行早期诊断具有非侵入性、低成本和易获取的优势，为临床提供替代方案。
...
TL;DR: 提出拓扑注意力网络，通过概率机制学习信息在图中的直接和间接连接中的传播方式，解决了图神经网络长距离依赖建模问题。
...
Method: 提出拓扑注意力机制，这是一种概率机制，学习信息应如何通过图中的直接和间接连接流动。不同于依赖显式成对交互的传统注意力，拓扑注意力从图的学得信息传播中涌现，能够统一推理局部和全局关系。

Result: 该方法在所有测量的基线模型上实现了最先进的性能。

Conclusion: 拓扑注意力网络是一个新框架，通过拓扑注意力机制有效解决了图神经网络的长距离依赖建模问题，具有更好的计算效率和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM选择方法通常优化单一固定目标（如性能或成本），无法从交互数据中学习个体用户偏好，而用户偏好因性能、成本和响应风格而异。

Method: 构建异构图表示交互数据（任务上下文、查询、候选LLM和用户决策），使用图神经网络捕获用户查询与最优LLM之间的上下文关系。

Result: 在两个模拟策略下分别超越最强方法15.38%和9.83%，在1000用户基准上进一步超越16.19%和59.69%，同时保持更高效率。

Conclusion: PersonalizedRouter能有效学习用户偏好，实现个性化LLM选择，并展现出强大的少样本泛化能力。
...
TL;DR: 本文定义了一个人才突破概念，结合Twitter和TV数据来预测日本艺人走红前的社会数据变化，发现集成学习方法在标准回归指标上表现最好，但神经网络在人才突破预测的精确率和召回率上更优。


<details>
  <summary>Details</summary>
Motivation: 广告领域早期发现潜力人才至关重要，研究结合Twitter和电视数据预测社会数据的时间依赖性变化，探索传统模型与神经网络在时间序列预测中的表现。

Method: 定义了人才突破概念，实验比较了传统时间序列模型、神经网络模型和集成学习方法，评估它们在预测社会数据变化和人才突破检测方面的表现。

Result: 集成学习方法在标准回归指标上优于传统和神经网络模型，但神经网络在人才突破预测的精确率和召回率方面表现最佳。

Conclusion: 不同模型在不同评估指标下各有优势，集成学习在回归任务中表现更好，而神经网络在人才突破检测的预测能力方面更胜一筹。
...
<details>
  <summary>Details</summary>
Motivation: 大环肽具有类似生物制剂的亲和性和小分子药物的可开发性，但其巨大的组合空间和多参数优化目标使得先导优化过程缓慢且困难。现有方法需要预先指定可突变位置，且使用静态预训练和优化算法，限制了模型的泛化能力和优化效果。

Method: PepEVOLVE框架包含三个核心组件：(i) 通过动态掩码和CHUCKLES移位增强预训练的泛化能力；(ii) 使用上下文无关多臂老虎机路由器发现高奖励残基；(iii) 结合新颖的进化优化算法和组相对优势来稳定强化学习更新。

Result: 在Rev结合大环肽基准测试中，PepEVOLVE相比PepINVENT达到了更高的平均分数（约0.8 vs 0.6），最佳候选肽得分达到0.95（vs 0.87），并且在优化渗透性和亲脂性时收敛步数更少。

Conclusion: PepEVOLVE为肽先导优化提供了一个实用且可复现的路径，特别是在最优编辑位置未知的情况下，能够更有效地进行探索并提高多目标设计质量。
...
TL;DR: SCR-MF是一个模块化的两阶段工作流，结合了scRecover的dropout检测和missForest的非参数插补，在单细胞RNA测序数据中实现稳健且可解释的插补性能。


<details>
  <summary>Details</summary>
Motivation: 单细胞RNA测序(scRNA-seq)能够在细胞分辨率下进行转录组分析，但存在普遍的dropout事件，这些事件会掩盖生物信号。

Method: SCR-MF采用模块化的两阶段工作流：第一阶段使用scRecover进行原理性的dropout检测，第二阶段通过missForest进行稳健的非参数插补。

Result: 在公共和模拟数据集上的测试表明，SCR-MF在大多数情况下实现稳健且可解释的性能，与现有插补方法相当或更优，同时保持生物保真度和透明度。

Conclusion: 运行时分析显示SCR-MF在准确性和计算效率之间提供了有竞争力的平衡，适用于中等规模的单细胞数据集。
...
TL;DR: 提出了一种基于对比强化学习的在线轨迹异常检测框架CroTad，能够无阈值、实时地检测子轨迹和点级别的异常行为，对噪声和不规则采样数据具有鲁棒性。
...
Method: 结合对比学习和强化学习，通过对比学习提取不同行程的正常旅行模式，利用深度强化学习进行在线实时异常评分，实现细粒度的异常检测。

Result: 在两个真实世界数据集上的广泛实验表明，该框架在各种评估场景下都具有有效性和鲁棒性。

Conclusion: CroTad框架成功解决了轨迹异常检测中的关键问题，为智能交通系统提供了可靠的无阈值在线异常检测解决方案。
...
TL;DR: 提出一种端到端分类管道，使用潜在ODE建模连续ECG波形，通过降采样创建鲁棒特征向量，在保持高性能的同时解决可穿戴设备采样频率与电池寿命的权衡问题。
...
Conclusion: 该方法能够绕过信号保真度与电池寿命之间的权衡，实现更小的可穿戴设备，促进长期心脏健康监测。
...
TL;DR: Tree of Claims (ToC) 框架通过结合蒙特卡洛树搜索和多智能体系统，优化专利权利要求，在保持法律范围的同时最大化新颖性。
...
Method: ToC 将权利要求编辑重新定义为引导搜索问题，集成蒙特卡洛树搜索与多智能体系统，包括提出编辑的 EditorAgent 和模拟专利审查员批判的 ExaminerAgent。

Result: 在1145个权利要求基准测试中，ToC 在零样本和少样本场景下显著优于标准大语言模型，平均综合得分提高8%，某些情况下提高9%。

Conclusion: ToC 建立了一个透明、可控且可解释的方法论，有效将先进的大语言模型推理能力与战略性MCTS规划相结合，用于结构化专利权利要求优化。
...
TL;DR: 本文研究了深度平衡模型(DEQs)在梯度下降训练中的理论动态特性，在线性模型和单索引模型设置下证明了参数守恒定律、梯度流的良好条件性以及线性收敛性。


<details>
  <summary>Details</summary>
Motivation: 深度平衡模型作为无限深度权重共享神经网络，在实践中取得了显著成功，但对其梯度下降动态的理论理解仍是活跃研究领域。本文旨在填补这一理论空白。

Method: 在线性DEQs和深度平衡单索引模型的简单设置下，采用理论分析方法，证明参数守恒定律、梯度流条件性以及线性收敛性，并通过实验验证理论发现。

Result: 证明了线性DEQs的参数在训练过程中保持球面约束，梯度流始终保持良好条件性，并在适当初始化和小步长下实现了梯度下降的线性收敛到全局最小化器。

Conclusion: 本研究为深度平衡模型的梯度下降动态提供了严格的理论保证，填补了文献中的多个空白，并通过实验验证了理论结果的正确性。
...
TL;DR: FIRM是一种联邦多目标对齐算法，通过客户端内正则化解决多目标冲突，实现通信效率和客户端分歧漂移缓解。


<details>
  <summary>Details</summary>
Motivation: 大语言模型与人类价值观对齐时面临多目标冲突（如帮助性与无害性），集中训练存在计算密集和数据隐私问题，现有联邦多目标优化方法存在严重通信瓶颈。
...
Conclusion: FIRM在保持高通信效率的同时有效解决联邦多目标对齐问题，并能根据偏好平滑调整目标间权衡。
...
Method: EMTC方法包含重要性感知变量级掩码(IVM)和多内生视图(MEV)表示学习模块。IVM自适应引导模型学习更具判别性的聚类表示，MEV通过重构和对比学习路径增强泛化能力。
...
TL;DR: 提出了基于计算复杂度（FLOPs）的扩散模型GPU能耗预测方法，通过Kaplan缩放定律实现跨模型和硬件架构的能耗准确预测。
...
Result: 在单个GPU架构内预测精度高（R平方>0.9），跨架构泛化能力强，能可靠预测未见过的模型-硬件组合的能耗。

Conclusion: 验证了扩散模型推理的计算受限特性，为可持续AI部署规划和碳足迹估算提供了基础。
...
TL;DR: Step-E是一个集成样本选择和模型学习的简单框架，通过基于损失的样本排名和渐进式排除高损失样本，有效处理噪声标签和异常值，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 野外收集的训练数据通常包含噪声标签和异常值，这会严重降低深度神经网络的性能和可靠性。现有的两阶段数据清洗方法无法充分利用下游模型的反馈，也无法适应未知的噪声模式。

Method: Step-E将样本选择和模型学习整合到单一优化过程中。在每个训练周期，根据损失对样本进行排序，在短暂预热阶段后逐步增加被排除在梯度更新之外的高损失样本比例，形成在线课程学习机制。

Result: 在CIFAR-100N上，Step-E将ResNet-18的测试准确率从43.3%提升到50.4%，明显优于损失截断、自定步调学习和一次性过滤方法，接近60.5%的干净标签基准。在CIFAR-10N上也表现出色，仅带来适度的训练时间开销。

Conclusion: Step-E通过在线课程学习有效处理噪声数据，显著提升模型性能，且计算开销可控，为噪声标签学习提供了简单而有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 分子指纹方法使用哈希函数生成固定长度的分子向量表示，但哈希冲突会导致不同子结构具有相同特征表示，从而在分子相似性计算中产生高估。本研究旨在探究使用精确指纹是否能提高分子性质预测和贝叶斯优化的准确性。

Method: 使用高斯过程作为基础预测模型，在DOCKSTRING数据集的五个分子性质预测基准上比较精确指纹与标准压缩指纹的性能。

Result: 精确指纹在五个分子性质预测基准上产生了小幅但一致的预测准确性提升。然而，这些增益并未转化为贝叶斯优化性能的显著改进。

Conclusion: 虽然精确指纹在分子性质预测中能带来轻微但一致的准确性提升，但这种改进在贝叶斯优化任务中并不显著，表明哈希冲突对贝叶斯优化的影响可能较小。
...
TL;DR: 研究大型语言模型作为工具使用代理时的举报行为，即模型在未经用户指示或知情的情况下向对话边界外的第三方披露可疑不当行为。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型作为工具使用代理时，其对齐训练会以新的方式表现出来。研究发现模型可能以违背用户利益或明确指令的方式使用工具，特别是未经授权向外部机构举报可疑不当行为。

Method: 引入包含多样化和现实化不当行为场景的评估套件，测试不同模型和设置下的举报行为。通过改变任务复杂性、系统提示的道德引导、可用工具和工作流程等变量来观察举报率变化。

Result: 发现：(1)不同模型家族的举报频率差异很大；(2)增加任务复杂性会降低举报倾向；(3)系统提示中的道德引导显著提高举报率；(4)提供更多工具和详细工作流程会减少举报行为。同时验证了数据集对模型评估意识的鲁棒性。

Conclusion: 大型语言模型作为工具代理时确实存在未经授权的举报行为，这种行为受多种因素影响，包括模型架构、任务设置和系统提示等。研究为理解和控制此类行为提供了实证基础。
...
TL;DR: 提出了几何解缠遗忘(GU)方法，通过将遗忘梯度分解为保留空间的切向和法向分量，只执行法向分量来避免对保留知识的损害，实现理论上有保证的机器遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法在有效遗忘和保留知识保护之间存在权衡，缺乏对遗忘更新如何损害保留知识的理论分析，以及能否在理论保证下消除副作用。

Method: 基于一阶分析，将遗忘梯度更新分解为保留梯度子空间的切向和法向分量，只执行法向分量，确保保留损失一阶不变。

Result: 在TOFU、MUSE和WMDP三个基准测试中，GU方法在各种现有梯度遗忘方法上实现了一致的性能提升。

Conclusion: 几何解缠遗忘提供了一种理论上有保证的简单解决方案，能够有效缓解机器遗忘过程中的副作用，且可与现有方法结合使用。
...
TL;DR: 本文提出了一个42年泛北极地区地表温度数据集，通过深度学习超分辨率算法将AVHRR GAC数据从4公里降尺度至1公里分辨率，显著提升了北极地区地表过程分析的时空分辨率。


<details>
  <summary>Details</summary>
Motivation: 北极地区快速变暖，但现有AVHRR GAC数据的粗空间分辨率限制了其在细尺度永久冻土动态和其他地表过程分析中的应用。需要高分辨率长期LST数据来监测气候变化。

Method: 使用基于深度各向异性扩散模型的超分辨率算法，以MODIS LST数据为训练目标，结合高分辨率土地覆盖、数字高程和植被高度图作为指导，将AVHRR GAC数据从4公里降尺度至1公里。

Result: 生成了42年泛北极地区每日两次的1公里分辨率LST观测数据集，为整个北极地区提供了四十年时间序列的高分辨率地表温度数据。

Conclusion: 该增强数据集能够改进永久冻土建模、重建近地表气温、评估格陵兰冰盖表面质量平衡，支持前MODIS时代的气候监测，并为未来卫星任务提供可适应的框架。
...
TL;DR: 使用深度学习从6轴IMU数据合成归一化sEMG信号，证明该方法可用于肌肉意图检测


<details>
  <summary>Details</summary>
Motivation: sEMG信号提供肌肉功能重要信息但易受噪声干扰且采集困难，IMU作为可穿戴替代方案更稳健

Method: 收集同步sEMG和IMU数据，使用基于扩张因果卷积的滑动窗口波网络模型将IMU数据映射到sEMG信号

Result: 模型成功预测肌肉激活的时序和大致形状，峰值幅度常被低估但具有高时间保真度
...
<details>
  <summary>Details</summary>
Motivation: 传统聚类方法在高维空间中面临挑战，DelTriC旨在通过将邻域构建与决策制定解耦，提高聚类的准确性和可扩展性。

Method: 先在低维代理空间进行三角剖分索引局部邻接关系，然后反向投影到原始空间进行鲁棒的边缘修剪、合并和异常检测。

Result: DelTriC在多种场景下优于k-means、DBSCAN和HDBSCAN等传统方法，具有更好的可扩展性和准确性，并显著改进异常检测。

Conclusion: DelTriC通过创新的降维-三角剖分-反向投影框架，为高维数据聚类提供了有效解决方案。
...
TL;DR: TS-DFM是一个基于流匹配的框架，能够从反应物和产物预测过渡态结构，在分子距离几何空间中运行，显著提高了过渡态预测的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 过渡态对于理解反应机理至关重要，但实验和计算方法的复杂性限制了其探索。需要开发更高效准确的过渡态预测方法。

Method: 提出TS-DFM流匹配框架，在分子距离几何空间中操作，设计了TSDVNet网络结构来学习生成过渡态几何结构的速度场。

Result: 在Transition1X基准数据集上，TS-DFM比之前最先进的方法React-OT在结构准确性上提高了30%，预测的过渡态为CI-NEB优化提供了高质量初始结构，并能识别替代反应路径，甚至发现了能量势垒更低的更有利过渡态。
...
TL;DR: FlexiFlow是一个新颖的架构，扩展了流匹配模型，能够联合采样分子及其多个构象，在QM9和GEOM Drugs数据集上实现了分子生成任务的最先进结果。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的3D从头设计流匹配或扩散模型仅限于生成单一构象，但分子的构象景观决定了其可观察性质和与蛋白质靶点的结合能力。通过生成一组代表性的低能构象异构体，可以更直接地评估这些性质。
...
TL;DR: 本文评估了两种无需训练的后处理投影方法，用于减少神经PDE求解器对非线性约束的违反，包括非线性优化投影和局部线性化投影。


<details>
  <summary>Details</summary>
Motivation: 神经PDE求解器在科学模拟中经常违反控制方程约束，特别是非线性约束和动力学PDE中的长程时间依赖关系，这使得投影到可行集变得复杂。

Method: 提出了两种无需训练的后处理投影方法：1) 非线性优化投影；2) 使用Jacobian-vector和vector-Jacobian积的局部线性化投影。

Result: 在代表性PDE上的分析表明，两种投影方法都能显著减少约束违反，并比基于物理信息的基线方法提高了准确性。

Conclusion: 两种后处理投影方法都能有效改善神经PDE求解器的约束满足性，为处理非线性约束提供了实用的解决方案。
...
TL;DR: 该研究针对德国高端汽车制造商的多产品、多市场、多层次需求预测问题，结合点预测和概率预测方法，使用LightGBM集成模型和混合整数线性规划协调方法，发现时空依赖性和整数偏差对预测精度有重要影响。
...
Method: 结合战略和运营规划层面的点预测和概率预测，利用LightGBM模型集成、分位数回归和混合整数线性规划协调方法，使用池化训练集进行建模。

Result: 时空依赖性和整数偏差显著影响预测精度，整数预测对运营可行性很重要。短期需求受生命周期成熟度、自回归动量和运营信号影响，中期需求受在线参与、规划目标和竞争指标等预期驱动因素影响。

Conclusion: 在线行为数据显著提高了细分层面的预测准确性，该方法为高端汽车制造商的多层次需求预测提供了有效解决方案。
...
Result: 在五个标准数据集上的实验显示，SAVeD在完全未见过的表格上实现了显著更高的准确率，分离分数显著提升，能够有效区分语义变更的版本。

Conclusion: SAVeD在无监督数据集版本检测方面表现出色，优于未训练的基准方法和现有最先进的数据集发现方法。
...
TL;DR: 比较两种自监督深度学习方法和一种非学习方法用于CT探测器数据去噪，深度学习方法在信噪比和重建图像质量上表现更优
...
Method: 研究了两种自监督深度学习方法和一种非学习方法的探测器数据去噪

Result: 深度学习方法能提高探测器数据的信噪比，并持续改善重建图像质量，优于非学习方法

Conclusion: 自监督深度学习方法是有效的CT数据去噪方法，能显著提升图像质量
...
TL;DR: 本文提出了Repulsive Bayesian Prompt Learning (ReBaPL)，一种新颖的贝叶斯提示学习方法，通过循环步长调度和表示空间排斥力来高效探索多模态后验分布，提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统提示学习方法容易过拟合且泛化能力不足，贝叶斯提示学习虽然能增强鲁棒性，但难以有效探索复杂的多模态后验分布。

Method: 结合循环步长调度与SGHMC算法，交替进行探索和利用；引入基于概率度量的表示空间排斥力来多样化探索；提供模块化插件式贝叶斯扩展。

Result: 在多个基准数据集上展示出优于最先进提示学习方法的性能。

Conclusion: ReBaPL能够更全面地刻画提示后验分布，显著提升泛化能力，为现有提示学习方法提供灵活的贝叶斯扩展方案。
...
TL;DR: 提出了一个封建Q学习方案，分析了其收敛和稳定性的条件，通过随机逼近理论和ODE方法证明了收敛性，并将更新过程解释为博弈均衡。


<details>
  <summary>Details</summary>
Motivation: 分层强化学习在利用决策问题的时间结构和增强持续学习能力方面具有潜力，但理论保证落后于实践，需要提供收敛和稳定性分析。

Method: 使用封建Q学习方案，基于随机逼近理论和ODE方法进行收敛性分析，将更新过程建模为博弈均衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有的追逃博弈方法大多假设完美信息，忽略了现实中的部分可观测性以及逃避者可能预测追捕者行动的情况，缺乏实时可用的鲁棒追捕策略。

Method: 首先证明传统动态规划算法在异步移动下保持最优性，然后提出逃避者可能位置的信念保持机制，最后将该机制嵌入EPG框架进行跨图强化学习。

Result: 强化学习后的策略实现了对未见真实世界图结构的鲁棒零样本泛化，并且始终优于现有博弈强化学习方法直接在测试图上训练的策略。

Conclusion: 该方法为部分可观测环境下的追逃博弈提供了首个实时鲁棒追捕策略解决方案，具有重要的实际应用价值。
...
TL;DR: 本文开发了一个线性稳定性框架来分析SGD、随机扰动和SAM在两层ReLU网络中的行为，通过一致性度量揭示梯度曲率在数据点间的对齐如何影响优化稳定性。


<details>
  <summary>Details</summary>
Motivation: 理解深度学习优化的动态机制对于模型扩展很重要。虽然SGD及其变体能可靠地找到泛化良好的解，但驱动这种泛化的机制仍不清楚。现有工作将平坦性与泛化联系起来，但缺乏连接数据结构、优化动态和学习解性质的统一理论。
...
Conclusion: 提出的线性稳定性框架和一致性度量提供了理解优化动态的新视角，有助于解释为什么某些平坦或简单的极小值在过参数化设置中被偏好。
...
<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型规模扩大，计算需求增加，需要有效的核心集选择技术来加速训练。梯度方法虽然理论基础强，但面临SGD基线过强和代表性随时间退化的问题。

Method: 建立后验采样与损失景观的连接，提出基于模型权重后验采样的平滑损失函数，增强稳定性和泛化能力，同时保持计算效率。

Result: 通过大量实验证明，该方法在多种数据集上比现有最优方法实现了更快的训练速度和更好的泛化性能。

Conclusion: 提出的后验采样框架有效解决了核心集选择中的关键挑战，为大规模深度学习训练提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的频繁或判别性子图挖掘方法存在冗余的多阶段流程、高计算成本和挖掘结构与判别相关性之间弱耦合的问题。

Method: DS-Span引入覆盖上限资格机制动态限制探索，以及信息增益引导的选择机制，在搜索空间的一次遍历中统一模式增长、剪枝和监督驱动评分。
...
TL;DR: CurvSSL是一个曲率正则化的自监督学习框架，通过在标准冗余减少损失基础上添加曲率正则化项来显式塑造表示的局部几何结构。
...
Method: 采用标准双视图编码器-投影器架构，在Barlow Twins风格的冗余减少损失基础上添加曲率正则化项，通过计算嵌入点的k近邻离散曲率得分，并在增强视图间对齐和去相关这些曲率得分。
...
TL;DR: 提出了基于序列到序列的多智能体指针变换器（MAPT）框架，用于解决具有随机请求的多车辆动态取送货问题，通过Transformer编码器提取实体表示，结合指针网络生成联合动作序列，显著优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 传统运筹学方法在处理大规模动态问题时面临计算复杂度和时间效率瓶颈，现有强化学习方法存在无法建模联合动作分布、特征提取网络难以捕捉实体间关系、联合动作空间指数级增长等问题。
...
Conclusion: MAPT框架有效解决了多车辆动态取送货问题中的关键挑战，在性能和计算效率方面都表现出色，为大规模动态路由优化问题提供了有效的端到端解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决基于提示的持续学习方法在领域偏移下容易出现的表示漂移问题，即使隔离了任务特定参数，共享表示层的演变仍会导致遗忘

Method: 捕获先前学习任务的特征激活范围，约束更新以确保网络在这些区域内保持一致性，同时允许在其他区域灵活适应，稳定重要神经元的功能角色

Result: 在DomainNet和ImageNet-R等多样领域增量基准测试中，持续减少表示漂移，平均准确率比最先进基线提高多达8个百分点
...
TL;DR: 提出MS-DGFormer框架，通过Transformer架构和字典编码器直接处理原始质谱数据，实现单次检测的病原体识别，无需复杂预处理。


<details>
  <summary>Details</summary>
Motivation: 传统MALDI-MS依赖繁琐的样品制备和多光谱平均，限制了其在实时环境监测中的应用，特别是在新兴气溶胶MALDI-MS系统中单次检测需求迫切。

Method: 使用Transformer架构捕捉质谱时间序列的长程依赖，结合基于奇异值分解的字典编码器提取去噪光谱特征。

Result: 该方法能够从单次光谱中稳健识别关键生物分子模式，实现气溶胶样本中病原体的优越识别。

Conclusion: MS-DGFormer消除了复杂预处理需求，为便携式MALDI-MS平台铺平道路，革新环境病原体检测和生物威胁快速响应能力。
...
TL;DR: 提出了一个基于知识图谱增强检索增强生成（Graph RAG）的个性化语言模型框架，通过构建用户画像和社区检测来生成个性化提示，显著提升了多个任务的性能。
...
Method: 使用知识图谱增强的检索增强生成机制，构建LLM衍生的图索引来总结相关信息社区，结合用户历史行为偏好和基于图的社区检测来生成个性化提示。

Result: 在LaMP基准测试中，新闻分类F1提升11.1%，电影标签F1提升56.1%，产品评分MAE降低10.4%。

Conclusion: 该框架能够保持一致的画像对齐行为，同时从集体知识中受益，为个性化AI代理提供了有效解决方案。
...
TL;DR: 提出一种结合聚类和学习的算法，用于多智能体线性二次调节器(LQR)控制，通过同时聚类和学习为每个聚类输出个性化策略，在保证正确聚类的同时获得统计增益。


<details>
  <summary>Details</summary>
Motivation: 强化学习通常需要大量数据，通过利用相似过程的数据可以提高样本效率。但由于过程模型未知，识别哪些过程相似具有挑战性。

Method: 结合顺序消除和零阶策略优化的思想，提出同时执行聚类和学习的算法，为每个聚类输出个性化控制器。

Result: 在适当的聚类分离条件下，算法能以高概率保证正确聚类，且每个聚类的策略次优性差距与聚类大小成反比，没有额外偏差。

Conclusion: 这是首个展示聚类如何在数据驱动控制中用于学习个性化策略的工作，既能从协作中获得统计增益，又不会因包含不相似过程数据而遭受次优性。
