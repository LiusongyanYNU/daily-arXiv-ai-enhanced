...
### [1] [CPSL: Representing Volumetric Video via Content-Promoted Scene Layers](https://arxiv.org/abs/2511.14927)
*Kaiyuan Hu,Yili Jin,Junhua Liu,Xize Duan,Hong Kang,Xue Liu*

Main category: cs.CV

TL;DR: CPSL是一种紧凑的2.5D视频表示方法，通过深度和内容显著性指导，将每帧分解为几何一致的层，支持视差校正的新视角合成，避免了昂贵的3D重建。
...
Method: 基于每帧深度和内容显著性，将帧分解为几何一致的层，配备软alpha带和边缘深度缓存，通过深度加权变形和前后alpha合成实现视差校正的新视角合成。
...
TL;DR: 开发了一个事件触发系统来检测老年人的身体和心理状态，包括看门狗、危险通知和照片链接三种事件类型，通过GMM背景建模和SVM机器学习分析，为缺乏技术经验的老年人提供直观的操作方式。


<details>
  <summary>Details</summary>
Motivation: 同时考虑老年人的身体和心理状态，为缺乏技术经验的老年人设计直观的操作方式，通过社交媒体促进老年人与亲属之间的沟通。

Method: 采用GMM背景建模检测访客和老年人的运动行为，使用SVM机器学习分析捕获的图像，设计事件触发系统检测三种事件类型。

Result: 在家庭场景中进行了实验，5个家庭参与检测和记录生活活动中的三种事件类型，系统能够有效检测相关事件。

Conclusion: 该系统能够有效监测老年人的身体和心理状态，为缺乏技术经验的老年人提供直观的沟通方式，促进与亲属的社交互动。
...
### [3] [Towards Unbiased Cross-Modal Representation Learning for Food Image-to-Recipe Retrieval](https://arxiv.org/abs/2511.15201)
*Qing Wang,Chong-Wah Ngo,Ee-Peng Lim*

Main category: cs.CV

TL;DR: 本文提出了一种基于因果理论的食谱-食物图像跨模态检索方法，通过因果干预消除传统方法中的偏差，在Recipe1M数据集上实现了新的最先进检索性能。
...
Method: 使用因果理论建模跨模态表示学习中的偏差，将配料视为混淆源，通过简单的后门调整来减轻偏差。通过因果干预重新制定传统模型，增加额外项来消除相似性判断中的潜在偏差，并提出即插即用的神经网络模块（多标签配料分类器）进行去偏。

Result: 在Recipe1M数据集上实现了MedR=1的oracle检索性能，在1K、10K和50K测试数据规模下均表现优异，报告了新的最先进搜索性能。

Conclusion: 基于因果理论的去偏方法能够有效提升食谱-食物图像跨模态检索的准确性，通过建模和消除偏差实现了显著的性能提升。
...
### [4] [Gaussian See, Gaussian Do: Semantic 3D Motion Transfer from Multiview Video](https://arxiv.org/abs/2511.14848)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在语义3D运动迁移方面的不足，特别是跨类别对象间的运动转移问题，建立首个语义3D运动迁移基准。

Method: 基于隐式运动迁移技术，通过条件反演从源视频提取运动嵌入，应用到静态目标形状的渲染帧，使用生成视频监督动态3D高斯溅射重建；引入基于锚点的视角感知运动嵌入机制和鲁棒的4D重建流程。

Result: 建立了首个语义3D运动迁移基准，相比适应基线方法，在运动保真度和结构一致性方面表现更优。

Conclusion: 该方法成功实现了跨类别的语义3D运动迁移，为相关研究提供了新的技术路径和评估基准。
...
### [5] [When CNNs Outperform Transformers and Mambas: Revisiting Deep Architectures for Dental Caries Segmentation](https://arxiv.org/abs/2511.14860)
...
TL;DR: 该研究对12种最先进的深度学习架构在牙科全景X光片龋齿分割任务上进行了基准测试，发现基于CNN的DoubleU-Net在各项指标上表现最佳，超越了Transformer和Mamba架构。
...
Method: 在DC1000数据集上，使用12种最先进的架构（包括CNN、Vision Transformer和状态空间Mamba架构）在相同配置下进行训练和评估。

Result: CNN-based的DoubleU-Net获得了最高的Dice系数0.7345、mIoU 0.5978和精度0.8145，在所有Transformer和Mamba变体中表现最佳。前3名结果均由基于CNN的架构获得。
...
### [6] [B-Rep Distance Functions (BR-DF): How to Represent a B-Rep Model by Volumetric Distance Functions?](https://arxiv.org/abs/2511.14870)
...
<details>
  <summary>Details</summary>
Motivation: 传统CAD边界表示方法在生成过程中存在失败率，需要一种更可靠的几何表示方法来确保CAD模型生成的稳定性。

Method: 使用体积距离函数编码CAD表面网格几何，将B-Rep的顶点、边、面和拓扑信息编码为每面无符号距离函数，并扩展Marching Cubes算法将BR-DF转换为水密CAD B-Rep模型。

Result: 提出的方法在CAD生成性能上与最先进方法相当，同时实现了前所未有的100%成功率生成B-Rep模型。

Conclusion: BR-DF提供了一种可靠的CAD几何表示方法，通过体积距离函数和多分支扩散模型实现了稳定且高质量的CAD模型生成。
...
TL;DR: GeoSceneGraph是一种从文本提示合成3D室内场景的方法，利用场景图结构和几何对称性，无需预定义关系类别，在资源受限设备上实现高性能场景生成。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在资源受限设备部署困难、忽略场景图结构导致场景连贯性不足，以及依赖用户提供语义图或真实关系标注限制交互多样性的问题。

Method: 基于等变图神经网络(EGNNs)，提出简单有效的文本特征条件化策略，利用3D场景的图结构和几何对称性进行场景合成。

Result: 尽管不使用真实关系标注，GeoSceneGraph在性能上可与使用真实关系的方法相媲美，并通过消融研究验证了设计有效性。

Conclusion: GeoSceneGraph提供了一种无需预定义关系类别的高效3D场景合成方法，特别适合资源受限设备部署，同时保持场景连贯性和交互多样性。
...
TL;DR: 提出了一种无监督的单图像双向MRI合成器，可以在高场强和超低场强MRI图像之间相互转换，基于物理原理模拟对比度变化。


<details>
  <summary>Details</summary>
Motivation: 现有MRI合成模型缺乏对高场强和超低场强MRI之间对比度变化物理原理的考虑，需要开发基于物理驱动的合成方法。

Method: 使用前向模型模拟HF到ULF转换，通过估计组织类型信噪比；对于超分辨率任务，使用隐式神经表示网络同时预测组织分割和图像强度。

Result: 在合成ULF样图像中WM-GM对比度提高了52%，在64mT图像中提高了37%；敏感性实验证明模型对目标对比度、噪声和初始种子的变化具有鲁棒性。

Conclusion: 该方法成功实现了基于物理原理的双向MRI合成，显著改善了图像对比度，并展示了良好的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有基于神经场或时序注意力机制的方法在多视图图像编辑中产生伪影和不一致编辑的问题。

Method: 用多视图扩散学生替代SDS中的神经场整合器，采用增量学生更新、专用教师噪声调度器和注意力修改来增强跨视图一致性。

Result: 实验表明I-Mix2Mix显著提高了多视图一致性，同时保持了高单帧编辑质量。

Conclusion: 提出的方法有效解决了多视图编辑中的一致性问题，无需额外成本即可实现跨视图连贯编辑。
...
<details>
  <summary>Details</summary>
Motivation: 解决当前视觉语言模型在临床推理中的三个主要限制：数据异质性、缺乏可靠的诊断依据以及泛化能力不足，以提高皮肤病诊断的可信度和临床实用性。

Method: 1) 设计基于教科书的推理生成器，合成高质量、层次感知和鉴别诊断信息轨迹；2) 利用构建的轨迹进行监督微调；3) 开发结合疾病层次结构的新型强化学习范式，将推理模式迁移到大规模稀疏数据。

Result: 在多个皮肤病数据集上的广泛实验表明，SkinR1实现了优越的诊断准确性。消融研究证明了监督微调所建立的推理基础的重要性。

Conclusion: SkinR1通过统一的端到端框架有效解决了皮肤病诊断中的关键挑战，结合深度推理和强化学习实现了可靠的临床推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感专用CLIP变体仍继承有限的空间感知能力，主要问题包括：遥感图像-文本数据集仅生成全局描述，未能充分利用对象级监督；直接应用通用领域的区域-文本对齐方法到遥感数据会导致性能下降。

Method: 构建首个多粒度遥感图像-文本数据集MGRS-200k；采用patch-to-patch蒸馏对齐局部和全局视觉线索；使用简单的CLS token进行区域-类别对齐，而非显式的patch级对齐。

Result: FarSLIP在遥感开放词汇语义分割、零样本分类和图像-文本检索等任务上均达到了新的最先进水平。

Conclusion: FarSLIP通过改进的细粒度对齐策略，显著提升了遥感领域的视觉-语言对齐能力，同时保持了语义连贯性。
...
TL;DR: nnMIL是一个简单而广泛适用的多实例学习框架，将补丁级病理基础模型连接到稳健的幻灯片级临床推理，在40,000张全切片图像上表现优于现有方法。
...
Result: 在35个临床任务和四个病理基础模型的40,000张WSI上，nnMIL在疾病诊断、组织学分型、分子生物标志物检测和泛癌预后预测方面始终优于现有MIL方法，展示了强大的跨模型泛化能力、可靠的不确定性量化和稳健的生存分层。
...
<details>
  <summary>Details</summary>
Motivation: CXR作为2D投影图像存在结构叠加的限制，无法捕捉3D解剖结构，这使得表征学习和疾病诊断面临挑战。

Method: 通过从胸部CT学习预测其2D投影来提取体积知识；引入亲和力引导的对比对齐损失；结合真实CXR进行掩码图像建模；使用域分类器使真实和模拟CXR具有统计相似的表示。

Result: X-WIN在线性探测和少样本微调的各种下游任务中优于现有基础模型，并展示了为重建3D CT体积渲染2D投影的能力。

Conclusion: X-WIN通过将3D解剖知识融入CXR世界模型，有效克服了传统CXR的结构限制，显著提升了医学图像分析和诊断性能。
...
TL;DR: 提出了首个针对长期周期性工作流的多模态人类活动基准，包含580个序列，支持三个评估任务，并提出了无需训练的轻量级基线方法。
...
Method: 构建包含580个多模态人类活动序列的基准，支持周期性工作流检测、任务完成跟踪和程序异常检测三个任务，并提出无需训练的轻量级基线方法。

Result: 基准对现有方法构成显著挑战；基线方法在所有评估任务中大幅优于竞争方法；在真实应用中，基线方法达到与传统监督方法相当的性能，且无需标注和重新训练。

Conclusion: 该基准为长期周期性工作流研究提供了重要平台，提出的基线方法在性能和部署便利性方面具有显著优势。
...
TL;DR: 提出了一种低成本的多视角视频流同步方法，使用LED时钟编码时间信息，可在异构相机系统中实现毫秒级时间对齐，支持可见光和红外模态。


<details>
  <summary>Details</summary>
Motivation: 多视角视频流的时间同步对于3D重建、姿态估计等应用至关重要，但在异构相机系统（专业与消费级设备、可见光与红外传感器混合）中，硬件同步往往不可用，特别是在非受控的真实环境中。

Method: 使用定制的LED时钟，通过红色和红外LED编码时间信息，从记录的帧中视觉解码曝光窗口（开始和结束时间），实现毫秒级同步。

Result: 与硬件同步相比，该方法在多个记录中实现了1.34ms RMSE的残差误差，在光线、音频和时间码同步方法中表现更优，并直接提升了多视角姿态估计和3D重建等下游计算机视觉任务的性能。
...
Result: 实验证明抽气泵能耗显著降低20%-50%，在空闲时自动停止，运行时动态调整功率。

Conclusion: 该方法为制造业可持续发展做出重要贡献，实现了激光切割过程的智能节能控制。
...
### [17] [EGSA-PT:Edge-Guided Spatial Attention with Progressive Training for Monocular Depth Estimation and Segmentation of Transparent Objects](https://arxiv.org/abs/2511.14970)
*Gbenga Omotara,Ramy Farag,Seyed Mohamad Ali Tousi,G. N. DeSouza*

Main category: cs.CV

TL;DR: 本文提出了一种边缘引导空间注意力机制和渐进式训练策略，用于改善透明物体的感知性能，在深度估计和语义分割任务上均取得改进。


<details>
  <summary>Details</summary>
Motivation: 透明物体感知是计算机视觉中的主要挑战，因为透明度会混淆深度估计和语义分割。现有的多任务学习框架存在负向跨任务交互问题，阻碍性能提升。

Method: 提出边缘引导空间注意力机制，将边界信息融入语义和几何特征的融合中；采用多模态渐进式训练策略，从RGB图像边缘过渡到深度图像边缘进行学习。

Result: 在Syn-TODD和ClearPose基准测试中，EGSA在深度精度上持续优于当前最优方法MODEST，同时保持有竞争力的分割性能，在透明区域改进最大。
...
### [18] [Logit-Based Losses Limit the Effectiveness of Feature Knowledge Distillation](https://arxiv.org/abs/2511.14981)
*Nicholas Cooper,Lijun Chen,Sailesh Dwivedy,Danna Gurari*

Main category: cs.CV

TL;DR: 提出了一种仅使用特征知识蒸馏的框架，无需logit损失，通过知识质量指标识别最有效的教师层，在多个数据集和模型上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏方法同时使用logits和中间层特征，本文探索仅使用特征蒸馏的可能性，以更有效地传递知识。

Method: 基于潜在表示几何特性的发现，引入知识质量指标来识别最有效的教师层，构建纯特征知识蒸馏框架。

Result: 在三个图像分类数据集和四种学生-教师模型对上，实现了最高15%的top-1准确率提升，达到最先进性能。

Conclusion: 纯特征知识蒸馏框架是有效的，知识质量指标能显著提升蒸馏效果，为轻量化模型训练提供了新思路。
...
### [19] [Kandinsky 5.0: A Family of Foundation Models for Image and Video Generation](https://arxiv.org/abs/2511.14993)
...
TL;DR: Kandinsky 5.0是一个用于高分辨率图像和10秒视频生成的最先进基础模型系列，包含图像生成、轻量级视频生成和高质量视频生成三个核心模型。


<details>
  <summary>Details</summary>
Motivation: 开发一个大规模、公开可用的生成框架，通过多阶段训练流程和优化技术，为研究社区提供高质量生成模型的开发和可访问性。

Method: 采用多阶段训练流程，包括数据整理、预训练、自监督微调(SFT)和基于强化学习(RL)的后训练，结合新颖的架构、训练和推理优化。

Result: Kandinsky 5.0在各项任务中实现了高生成速度和最先进的性能，并通过人工评估验证了其优越性。

Conclusion: 该框架及其开源代码和训练检查点的发布将显著推进高质量生成模型的研究和发展。
...
TL;DR: FinCriticalED是一个用于评估OCR和视觉语言模型在金融文档事实级别理解的视觉基准，包含500个图像-HTML对，专家标注了700多个数值和时间事实。


<details>
  <summary>Details</summary>
Motivation: 金融文档具有视觉密集和表格繁多的布局，其中数值和时间信息与结构紧密耦合。在关键场景中，小的OCR错误可能导致完全不同的解读，而传统OCR指标只关注表面文本相似性。

Method: 开发了包含专家标注金融事实的数据集，建立了首个事实级别评估基准，并设计了LLM-as-Judge评估流程进行结构化事实提取和上下文验证。
...
TL;DR: 提出CKDA方法来解决可见光-红外终身行人重识别中的模态间知识干扰问题，通过知识解耦和对齐来平衡模态特定知识和模态通用知识的学习与保持。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理可见光-红外终身行人重识别时，忽略了模态特定知识获取和模态通用知识抗遗忘之间的相互干扰，导致协作性遗忘问题。

Method: 提出跨模态知识解耦和对齐方法(CKDA)，包括模态通用提示模块(MCP)和模态特定提示模块(MSP)来解耦知识，以及跨模态知识对齐模块(CKA)来在特征空间中对齐新旧知识。

Result: 在四个基准数据集上的大量实验验证了CKDA方法的有效性和优越性，优于现有最先进方法。

Conclusion: CKDA方法通过明确分离和平衡保存模态特定知识和模态通用知识，有效解决了终身行人重识别中的知识干扰问题。
...
### [22] [Complex-Valued 2D Gaussian Representation for Computer-Generated Holography](https://arxiv.org/abs/2511.15022)
...
<details>
  <summary>Details</summary>
Motivation: 传统全息图表示需要逐像素信息存储，参数搜索空间大，计算效率低。需要更紧凑的表示方法来提高下一代计算机生成全息系统的可扩展性。

Method: 使用结构化复值2D高斯基元作为全息图表示，开发可微分光栅化器，并与GPU优化的自由空间光传播内核集成。还引入转换程序将表示适配到实用全息图格式。

Result: 实验显示：VRAM使用降低2.5倍，优化速度提高50%，重建保真度更高。转换程序能有效抑制先前方法中的噪声伪影。
...
TL;DR: 研究发现计算机视觉模型在训练过程中表现出与儿童数学认知发展相似的进展模式，特别是在几何和数字概念方面。


<details>
  <summary>Details</summary>
Motivation: 探索计算机视觉模型是否不仅具有认知对齐（学习与成人相似的几何和数字概念表示），还具有发展对齐（训练过程中的性能改进是否匹配儿童的发展进程）。

Method: 使用ResNet-50模型进行详细案例研究，分析其在几何（欧几里得几何、几何图形、度量属性、拓扑等）和数字（心理数字线表示）概念上的学习轨迹。

Result: 在几何方面，部分概念类别（欧几里得几何、几何图形、度量属性、拓扑）表现出发展对齐，而其他类别（手性图形、几何变换、对称图形）则没有。在数字方面，模型随着经验积累发展出类似人类的心理数字线表示。
...
TL;DR: UniHOI是一个统一的人-物交互检测与生成框架，通过共享的token空间实现双向知识迁移，在有限标注下提升泛化能力。
...
Conclusion: UniHOI通过统一建模HOI检测与生成，有效促进知识共享和泛化能力，为全面交互理解提供了新思路。
...
TL;DR: 提出DLRRF模型处理高光谱和多光谱图像融合中的图像间变异问题，通过建模光谱退化变化和分解低秩残差分量来提升融合性能


<details>
  <summary>Details</summary>
Motivation: 高光谱与多光谱图像融合时，由于采集条件不同存在光谱变异和空间局部变化，严重影响融合效果，现有方法直接变换图像会加剧模型不适定性

Method: 将光谱变异建模为光谱退化算子变化，将目标高光谱分解为低秩和残差分量，利用光谱相关性进行降维，引入隐式正则化器利用空间先验信息，在PnP框架下使用PAO算法求解

Result: DLRRF在存在图像间变异的高光谱与多光谱图像融合中取得了优越性能

Conclusion: 所提出的DLRRF模型能有效处理图像间变异问题，通过退化建模和低秩残差分解显著提升了融合质量
...
TL;DR: CellGenNet是一个基于知识蒸馏的细胞分割框架，通过师生架构和混合损失函数在有限监督下实现跨组织的稳健细胞分割。


<details>
  <summary>Details</summary>
Motivation: 显微镜全玻片图像中细胞核分割面临染色、成像条件和组织形态的变异性挑战，需要开发在有限标注下仍能保持稳健性的分割方法。

Method: 采用师生架构，容量教师模型在稀疏标注上训练并生成软伪标签；学生模型使用结合真实标签、教师概率目标和混合损失函数（二元交叉熵+Tversky损失）的联合目标进行优化，并应用一致性正则化和分层dropout。
...
### [27] [ESA: Energy-Based Shot Assembly Optimization for Automatic Video Editing](https://arxiv.org/abs/2511.02505)
...
<details>
  <summary>Details</summary>
Motivation: 解决当前智能视频编辑技术难以捕捉创作者独特艺术表达的问题，让无视频编辑经验的用户也能创建视觉吸引力强的视频。

Method: 1) 使用大语言模型生成脚本并与视频库进行视觉语义匹配获取候选镜头；2) 从参考视频中分割和标注镜头属性；3) 使用能量模型学习参考风格并评估候选序列；4) 结合语法规则优化镜头组装。

Result: 开发了一个系统，能够自动按照特定逻辑、叙事需求或艺术风格安排和组合独立镜头，学习参考视频的组装风格，创建连贯的视觉序列。

Conclusion: 该方法不仅实现了镜头组装的自动化，还能学习参考视频的组装风格，使无经验的用户也能制作出视觉吸引力强的视频。
...
### [28] [ProPL: Universal Semi-Supervised Ultrasound Image Segmentation via Prompt-Guided Pseudo-Labeling](https://arxiv.org/abs/2511.15057)
...
Conclusion: 该框架在通用半监督超声图像分割任务中表现出色，具有重要的临床应用价值。
...
### [29] [Evaluating Multimodal Large Language Models on Vertically Written Japanese Text](https://arxiv.org/abs/2511.15059)
...
<details>
  <summary>Details</summary>
Motivation: 由于部分日文文档采用垂直书写方式，而现有MLLMs对垂直书写日文文本的研究有限，需要评估和改进模型在这方面的能力。

Method: 生成合成日文OCR数据集（包含水平和垂直书写文本），创建真实世界垂直书写文档评估数据集，并对MLLMs进行微调和评估。

Result: 现有MLLMs在垂直书写日文文本上的表现明显差于水平书写文本；使用合成数据集微调后，原本无法处理垂直书写的模型性能得到显著提升。

Conclusion: 垂直书写日文文本是MLLMs的重要挑战，通过专门的数据集训练可以有效改善模型在这方面的能力。
...
### [30] [Reasoning via Video: The First Evaluation of Video Models' Reasoning Abilities through Maze-Solving Tasks](https://arxiv.org/abs/2511.15065)
...
TL;DR: 本文提出VR-Bench基准测试，通过视频生成评估视频模型的空间推理能力，发现视频模型在迷宫求解任务中表现出强大的空间感知和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 类比从文本生成到文本推理的发展，探索视频模型是否能够通过视频生成进行推理，特别是利用视频在空间布局和时间连续性方面的优势进行空间推理。

Method: 构建VR-Bench基准测试，包含7,920个程序生成的视频，涵盖五种迷宫类型和多样视觉风格，基于迷宫求解任务评估空间规划和多步推理能力。

Result: 视频模型在空间感知方面表现优于领先的视觉语言模型，能良好泛化到不同场景、任务和复杂度；测试时多样化采样可将推理可靠性提升10-20%。
...
### [31] [BokehFlow: Depth-Free Controllable Bokeh Rendering via Flow Matching](https://arxiv.org/abs/2511.15066)
*Yachuan Huang,Xianrui Luo,Qiwen Wang,Liao Shen,Jiaqi Li,Huiqiang Sun,Zihao Huang,Wei Jiang,Zhiguo Cao*

Main category: cs.CV

TL;DR: BokehFlow是一个无需深度输入的景深渲染框架，基于流匹配技术直接从全焦图像合成逼真的散景效果，通过跨注意力机制实现基于文本提示的语义控制。


<details>
  <summary>Details</summary>
Motivation: 现有可控散景渲染方法依赖准确的深度图，而生成方法在可控性和效率方面存在局限，需要开发无需深度输入的可控散景渲染方法。
...
### [32] [MambaTrack3D: A State Space Model Framework for LiDAR-Based Object Tracking under High Temporal Variation](https://arxiv.org/abs/2511.15077)
*Shengjing Tian,Yinan Han,Xiantong Zhao,Xuehu Liu,Qi Lang*

Main category: cs.CV

TL;DR: MambaTrack3D是一个基于状态空间模型Mamba的3D单目标跟踪框架，针对高时间变化(HTV)动态室外环境设计，通过帧间传播和分组特征增强模块，在保持线性复杂度的同时提升跟踪性能。
...
Method: 提出Mamba-based Inter-frame Propagation(MIP)模块实现高效的帧间传播，以及Grouped Feature Enhancement Module(GFEM)在通道层面分离前景和背景语义以减少时间冗余。

Result: 在KITTI-HTV和nuScenes-HTV基准测试中显著优于现有HTV和普通场景跟踪器，在标准KITTI数据集上保持与最先进跟踪器的竞争力。
...
### [33] [TiCAL:Typicality-Based Consistency-Aware Learning for Multimodal Emotion Recognition](https://arxiv.org/abs/2511.15085)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖统一的情感标签来监督模型训练，但忽视了模态间情感冲突的关键挑战——同一样本中不同模态可能表达不同的情感倾向。

Method: 提出TiCAL框架，利用伪单模态情感标签和典型性估计动态评估训练样本的一致性，在双曲空间中嵌入特征以更好地捕捉情感类别的细微差别。

Result: 在CMU-MOSEI和MER2023等基准数据集上的广泛实验验证了TiCAL的有效性，能够缓解模态间情感冲突并提高整体识别准确率，比最先进的DMD方法提升约2.6%。

Conclusion: TiCAL通过考虑模态一致性有效解决了多模态情感识别中的模态间情感冲突问题，显著提升了模型性能，特别是在模态不一致性高的样本上表现更佳。
...
TL;DR: 提出JCDM框架，通过多视图先验解决姿态引导人体图像生成中的纹理不完整和跨视图交互缺失问题，实现身份保持和跨姿态一致性。


<details>
  <summary>Details</summary>
Motivation: 现有姿态引导人体图像生成方法受限于单参考视图的纹理不完整性和缺乏显式的跨视图交互机制。

Method: 使用联合条件扩散模型，包含外观先验模块从多视图推断整体身份保持先验，以及联合条件注入机制融合多视图线索并注入共享条件到去噪主干中。

Result: 实验证明在保真度和跨视图一致性方面达到最先进水平。

Conclusion: JCDM支持可变数量参考视图，可与标准扩散主干集成，仅需最小化架构修改。
...
TL;DR: 该论文研究了离散扩散多模态大语言模型(dMLLMs)中的视觉令牌冗余问题，发现不同架构和任务下冗余模式不同，并提出了针对性的效率优化策略。


<details>
  <summary>Details</summary>
Motivation: 现有dMLLMs在推理时因全序列注意力计算产生显著计算开销，大多数优化方法忽视了模态特定的视觉令牌冗余问题。

Method: 对dMLLMs中视觉令牌冗余随架构和任务演化的全面研究，分析视觉令牌剪枝对模型响应和效率的影响。

Result: 发现视觉冗余仅出现在从头训练的dMLLMs处理长答案任务时；视觉令牌剪枝会造成信息损失，但只有从头训练的dMLLMs能在后期去噪步骤中逐步恢复丢失信息。

Conclusion: 层跳过适用于加速AR-to-diffusion dMLLMs，而渐进或后期剪枝对从头训练的dMLLMs更有效，为dMLLMs效率优化提供了新视角。
...
TL;DR: 提出Gaussian Blending替代传统alpha blending，解决3DGS在未见采样率下渲染时的模糊和阶梯伪影问题，保持实时渲染速度且无需额外内存开销。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法在训练时未见采样率下合成视图时存在明显视觉差异：(i)放大时侵蚀引起的模糊伪影；(ii)缩小时膨胀引起的阶梯伪影，这些源于alpha blending的基本限制。

Method: 提出Gaussian Blending，将alpha和透射率视为空间变化分布而非标量，考虑像素区域内alpha值的空间分布来更新透射率，使附近背景splat能贡献最终渲染。

Result: Gaussian Blending能有效捕捉训练时未见各种采样率下的细节，在未见和已见采样率下均优于现有新视图合成模型。

Conclusion: Gaussian Blending作为即插即用模块，可轻松集成到现有3DGS框架，解决采样率变化时的渲染伪影问题，同时保持实时渲染性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要从支持集中提取提示来激活SAM的泛化能力，但这种设计在适应未知类别时容易导致有偏的解码过程，无法充分利用SAM的强泛化能力。

Method: 提出无偏语义解码策略，设计两种特征增强策略：图像级的全局补充和像素级的局部引导，利用CLIP的语义对齐能力丰富SAM特征；提出可学习的视觉-文本目标提示生成器，通过交互目标文本嵌入和CLIP视觉特征生成目标聚焦的提示嵌入。

Result: 无需重新训练视觉基础模型，具有语义区分能力的特征通过富含目标信息的提示引导关注目标区域。

Conclusion: 该方法有效解决了SAM在少样本分割中的提示依赖问题，通过语义增强实现了更好的目标区域关注和分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统主动学习方法在医学影像标注中，单一采集策略在不同学习阶段表现不一致，需要一种能够自适应融合多种策略的方法来最大化有限标注预算的效用。

Method: 提出WaveFuse-AL框架，将四种成熟的采集策略（BALD、BADGE、Entropy、CoreSet）通过周期性（正弦）时间先验和性能驱动适应机制进行动态融合，随时间调整策略重要性。

Result: 在APTOS-2019（多分类）、RSNA肺炎检测（二分类）和ISIC-2018（皮肤病变分割）三个医学影像基准测试中，WaveFuse-AL在十二个指标测量中的十个上取得统计显著性能提升，始终优于单一策略和交替策略基线。

Conclusion: WaveFuse-AL通过自适应多策略融合，在医学影像主动学习中实现了更一致和优越的性能，有效降低了标注成本。
...
TL;DR: DCL-SE是一个用于高维神经影像分析的端到端框架，通过数据驱动的时空编码和动态课程学习策略，在多个脑部疾病分类和分割任务中表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 解决高维神经影像分析中时空保真度不足和大规模通用模型适应性有限的问题，强调紧凑、任务特定架构的重要性。

Method: 使用近似秩池化将3D脑部数据编码为2D动态表示，并通过动态课程学习策略逐步训练解码器，从全局解剖结构到精细病理细节进行特征提取。
...
TL;DR: SceneEdited是首个专门为HD地图维护和3D点云更新研究设计的城市规模数据集，包含800多个最新场景、73公里驾驶距离和约3平方公里城市区域，提供超过23,000个合成物体变化和基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有的HD地图会随着环境变化而过时，当前技术存在从2D图像变化检测到实际更新3D地图之间的明显差距，需要专门的数据集来支持3D地图维护研究。

Method: 创建了包含校准RGB图像、LiDAR扫描和详细变化掩码的数据集，提供基于基础图像结构从运动流程的基线方法，以及支持可扩展性、可追踪性和可移植性的综合工具包。

Result: 数据集覆盖73公里驾驶距离和约3平方公里城市区域，包含2000多个过时版本和超过23,000个合成物体变化，模拟了真实的城市环境修改。

Conclusion: SceneEdited为3D地图更新研究建立了标准化基准，数据集和工具包已公开可用，将推动HD地图维护技术的发展。
...
TL;DR: 提出了一种结构感知的手术反馈生成管道，通过挖掘手术动作本体（IAT三元组）来指导GPT-4o生成临床基础、类似培训师风格的反馈。


<details>
  <summary>Details</summary>
Motivation: 高质量的手术中反馈对提升学员表现和长期技能获取至关重要。自动化生成自然、类似培训师风格的反馈能够提供及时、可访问且一致的指导，但需要理解临床相关表征的模型。

Method: 从真实培训师-学员对话记录中学习手术动作本体，并用于条件化反馈生成。包括：(1)从反馈文本中挖掘IAT三元组并聚类表面形式；(2)微调视频到IAT模型，利用手术过程和任务上下文以及细粒度时间仪器运动；(3)使用IAT三元组表征指导GPT-4o生成临床基础反馈。

Result: 在视频到IAT识别任务中，上下文注入和时间跟踪带来一致的AUC提升。在反馈文本生成任务中，IAT条件化使GPT-4o得分从2.17提升到2.44（+12.4%），可接受生成比例从21%翻倍至42%。文本相似度指标也显著改善。
...
TL;DR: 提出一种新的多模态持续指令调优方法，通过将灾难性遗忘概念化为旧任务梯度缺失问题，利用参数空间的几何特性来近似缺失梯度，有效缓解灾难性遗忘。
...
Method: 利用参数空间的几何特性，通过当前参数与先前最优参数之间的方向向量作为梯度指导来近似缺失梯度，并结合有限回放缓冲区的真实梯度，采用伯努利采样策略动态平衡模型稳定性和可塑性。

Result: 在多模态持续指令调优数据集上的广泛实验表明，该方法在不扩展模型的情况下实现了最先进的性能，有效缓解了灾难性遗忘并保持了紧凑架构。

Conclusion: 该方法通过几何梯度近似成功解决了多模态持续学习中的灾难性遗忘问题，为紧凑模型的持续学习提供了有效解决方案。
...
TL;DR: 提出SEC-Depth框架，通过自演进对比学习增强自监督深度估计在恶劣天气下的鲁棒性
...
Method: 利用训练过程中生成的中间参数构建时序演进延迟模型，设计自演进对比损失，将历史延迟模型输出作为负样本

Result: 实验表明该方法能无缝集成到多种基线模型中，在零样本评估中显著增强鲁棒性

Conclusion: SEC-Depth框架通过自演进对比学习机制有效缓解恶劣天气条件下的性能损失，减少人工干预需求
...
<details>
  <summary>Details</summary>
Motivation: 现有评估指标只关注预测运动的分布广度，但忽略了多模态分布和运动合理性。单一过去序列可能对应多个未来运动，需要同时评估预测运动的覆盖范围（是否分布在多个模式中）和有效性（运动是否合理）。

Method: 提出MMCM指标：1）通过聚类将运动空间划分为多个模式；2）使用运动数据集收集可能的未来运动来识别有效模式；3）基于这些模式评估预测运动的多模态分布和合理性。

Result: 实验验证了聚类方法能够产生合理的模式定义，且MMCM能够准确评估多模态预测的质量。

Conclusion: MMCM是一种有效的人类运动预测评估指标，能够同时考虑预测运动的覆盖范围和有效性，解决了现有指标的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前CXR病变分割模型存在目标标签数量有限和依赖专家级详细文本输入的问题，限制了实际应用。

Method: 创建MIMIC-ILS数据集（包含110万指令-答案对），使用全自动多模态流水线从CXR图像和报告中生成标注，并训练ROSALIA视觉语言模型。

Result: ROSALIA模型在新任务中实现了高分割精度和文本准确性，验证了流水线的有效性和MIMIC-ILS作为基础资源的价值。

Conclusion: ILS范式通过简单用户指令实现了多样病变分割，MIMIC-ILS为像素级CXR病变定位提供了重要基础资源。
...
<details>
  <summary>Details</summary>
Motivation: 传统回归和CNN方法存在手动特征工程、感受野有限和异质数据过拟合问题，纯transformer模型需要大数据集和高计算成本，需要开发更高效准确的脑龄估计方法。

Method: 采用ViT编码器在辅助年龄和性别分类任务上预训练学习切片级特征，然后冻结编码器应用于所有矢状切片生成嵌入向量2D矩阵，输入到残差CNN回归器，在最终全连接层结合受试者性别估计连续脑龄。

Result: 在11个MRI数据集上验证MAE为3.34年（Pearson r=0.98，Spearman ρ=0.97，R²=0.95），在4个独立队列中泛化良好（MAE 3.77-5.04年），脑龄差分析与阿尔茨海默病、认知障碍和自闭症谱系障碍相关。

Conclusion: 该方法为脑龄预测提供了高效、可解释和可泛化的框架，弥合了CNN和transformer方法之间的差距，为衰老和神经退行性疾病研究开辟了新途径。
...
Conclusion: 提出了首个既实用又高保真的零样本生成框架，解决了风格化域中对象组合的概念干扰问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的评估指标如BLEU、CIDEr等无法有效捕捉语义或结构准确性，特别是在领域特定或上下文相关场景中

Method: 三阶段架构：1) 通过目标检测和VLMs提取多模态特征；2) 置信度加权组件融合进行自适应组件级验证；3) 使用大语言模型进行物理引导推理以强制执行结构和关系约束

Result: 开发了PCMDE指标，能够更好地评估语义和结构准确性

Conclusion: PCMDE指标通过结合多模态特征提取、自适应融合和物理约束推理，有效克服了现有评估指标的局限性
...
<details>
  <summary>Details</summary>
Motivation: 为了解决皮肤科诊断中推理过程不透明的问题，开发能够进行明确、可验证的思维链推理的皮肤科专用模型。
...
Conclusion: 基于DermCoT的思维链监督显著提升了模型性能，皮肤科感知的视觉蒸馏进一步改善了叙述质量和识别能力。
...
TL;DR: IPTQ-ViT是一种无需重新训练的全整数视觉Transformer后训练量化框架，通过多项式GELU和位移Softmax近似函数实现完全整数推理，在图像分类和物体检测任务上优于现有PTQ方法。


<details>
  <summary>Details</summary>
Motivation: 现有QAT方法需要昂贵的重新训练来恢复非线性层量化精度损失，而PTQ方法要么只能部分量化非线性函数，要么无法实现完全整数推理。需要一种无需重新训练就能实现完全整数推理的PTQ方法。

Method: 提出IPTQ-ViT框架：1) 多项式GELU近似函数针对视觉数据优化；2) 基于位移的Softmax近似函数提高PTQ精度；3) 统一指标结合量化敏感性、扰动和计算成本来选择每层最优近似函数。

Result: 在图像分类任务上达到最高6.44%p（平均1.78%p）的top-1准确率提升，物体检测任务上提升1.0 mAP。在W8A8和W4A8配置下优于部分浮点PTQ方法，准确率和延迟与整数QAT方法相当。

Conclusion: IPTQ-ViT实现了无需重新训练的完全整数视觉Transformer推理，在精度和效率上均优于现有PTQ方法，为资源受限环境提供了可行的解决方案。
...
TL;DR: SplitFlux是一种基于Flux模型的图像内容与风格分离方法，通过LoRA微调单流块实现内容与风格的有效解耦，包含秩约束适应和视觉门控LoRA两个关键组件。


<details>
  <summary>Details</summary>
Motivation: 现有SDXL方法难以实现高质量结果，而Flux模型由于特性未被充分探索，无法有效分离图像内容与风格。需要解决内容与风格解耦的挑战。

Method: 通过系统分析Flux模型发现：单流块对图像生成至关重要；早期单流块主要控制内容，后期块控制风格。基于此提出SplitFlux，使用LoRA微调单流块，包括秩约束适应和视觉门控LoRA两个组件。

Result: 大量实验表明，SplitFlux在多样化场景中始终优于现有最先进方法，实现了卓越的内容保持和风格化质量。

Conclusion: SplitFlux通过分析Flux模型特性并设计相应解耦策略，成功实现了高质量的内容与风格分离，为定制化图像生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 3D雷达检测对360度汽车感知至关重要，但雷达长波长产生的稀疏不规则反射挑战了传统的网格和序列检测器，需要新的方法来提取个体化关系特征。

Method: 使用图查询在BEV空间中动态关注，构建对象特定图，包含EdgeFocus模块进行关系推理和DeepContext Pooling模块进行上下文聚合。

Result: 在NuScenes数据集上相对mAP提升高达+53%，比最强雷达方法提升+8.2%，同时将图构建开销降低80%，FLOPs成本适中。

Conclusion: GQN框架通过图查询和图处理模块有效解决了雷达检测的稀疏性问题，在性能和效率上都取得了显著改进。
...
TL;DR: 提出了LEO框架，通过将关系视为节点构建线图，实现从关系级上下文到对象级理解的渐进式推理，解决现有方法难以捕捉高阶关系依赖的问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景图预测方法主要采用以对象为中心的图神经网络，关系边特征通过聚合连接对象节点的消息来迭代更新。这种设计将关系表示限制在成对对象上下文中，难以捕捉对准确关系预测至关重要的高阶关系依赖。

Method: 提出LEO框架：1) 预测对象对之间的潜在链接以抑制不相关边；2) 将原始场景图转换为线图，其中每个关系被视为节点；3) 应用线图神经网络进行以边为中心的关系推理以捕捉关系间上下文；4) 将丰富的关系特征集成回原始对象中心图以增强对象级推理。

Result: 在3DSSG数据集上与两个竞争基线进行实验，结果显示了一致的改进，突出了我们边到对象推理范式的有效性。

Conclusion: LEO框架是模型无关的，可以与任何现有的以对象为中心的方法集成，通过从关系级上下文到对象级理解的渐进式推理，有效提升了3D场景图预测的性能。
...
TL;DR: 本文提出了一种针对多模态检索增强生成系统的视觉攻击方法，通过在用户输入的图像上添加微小扰动，能够破坏检索器和生成器的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态检索增强生成系统存在安全漏洞，但以往的攻击方法需要操纵知识库内容。本文探索仅通过视觉扰动就能攻击MRAG系统的可能性。

Method: 提出分层视觉攻击方法，通过两阶段策略：首先破坏跨模态对齐，然后破坏多模态语义对齐，使检索器召回不相关知识，进而混淆生成器的输出。

Result: 在OK-VQA和InfoSeek数据集上的实验表明，该方法能显著降低检索和生成性能，使用CLIP检索器和BLIP-2、LLaVA生成器都有效。

Conclusion: 仅通过视觉扰动就能有效攻击MRAG系统，揭示了这类系统在视觉输入安全方面的脆弱性。
...
### [55] [Taming Generative Synthetic Data for X-ray Prohibited Item Detection](https://arxiv.org/abs/2511.15299)
*Jialong Sun,Hongguang Zhu,Weizhe Liu,Yunda Sun,Renshuai Tao,Yunchao Wei*

Main category: cs.CV

TL;DR: 提出了一种基于文本到图像生成的一阶段X光安检图像合成方法Xsyn，无需额外人工成本即可生成高质量合成图像，并改进了违禁品检测性能。


<details>
  <summary>Details</summary>
Motivation: 训练违禁品检测模型需要大量X光安检图像，但收集和标注这些图像耗时耗力。现有方法采用两阶段流程，需要人工前景提取，效率低下且成本高。

Method: 基于文本到图像生成的一阶段合成流程，包含交叉注意力细化策略改进边界框标注，以及背景遮挡建模策略增强图像复杂性。

Result: 实验表明该方法优于所有先前方法，mAP提升1.2%，生成的合成图像有助于提高各种X光安检数据集和检测器的违禁品检测性能。

Conclusion: Xsyn是首个无需额外人工成本即可实现高质量X光安检图像合成的方法，有效解决了数据不足问题。
...
### [56] [Text2Loc++: Generalizing 3D Point Cloud Localization from Natural Language](https://arxiv.org/abs/2511.15308)
*Yan Xia,Letian Shi,Yilin Di,Joao F. Henriques,Daniel Cremers*

Main category: cs.CV

TL;DR: Text2Loc++是一个用于3D点云子地图自然语言定位的神经网络，采用从粗到细的定位流程，在KITTI360Pose数据集上比现有方法提升15%，并能泛化到新的城市规模数据集。
...
Method: 提出粗到细的定位流程：全局位置识别阶段使用预训练语言模型和分层Transformer，结合掩码实例训练和模态感知分层对比学习；精细定位阶段使用基于原型的映射克隆和级联交叉注意力Transformer。

Result: 在KITTI360Pose数据集上比现有方法提升15%，在新数据集上展现出强大的泛化能力，能有效处理复杂语言表达和多样化城市环境。

Conclusion: Text2Loc++通过创新的跨模态对齐方法和分层对比学习，在3D点云自然语言定位任务上取得了显著性能提升和良好泛化能力。
...
TL;DR: 提出了一种混合深度学习框架RS-CA-HSICT，结合CNN和Transformer优势，在MPox检测中达到98.30%的准确率和98.13%的F1分数，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有CNN和ViT在MPox检测中存在局限性，需要结合CNN的局部特征提取能力和Transformer的全局上下文建模能力，以更好地捕捉病变的细微模式和对比度变化。

Method: 构建了包含HSICT模块、残差CNN模块、空间CNN块和通道增强的混合架构。HSICT模块集成CNN主干和定制ICT块，进行多头注意力和结构化CNN操作。残差学习解决梯度消失，空间注意力机制优化像素选择。

Result: 在Kaggle基准和多样化MPox数据集上，分类准确率高达98.30%，F1分数达98.13%，超越了现有CNN和ViT方法。

Conclusion: 提出的RS-CA-HSICT框架成功融合了CNN和Transformer的优势，在MPox检测中表现出色，为医学图像分析提供了有效的混合深度学习解决方案。
...
### [58] [Adapt-As-You-Walk Through the Clouds: Training-Free Online Test-Time Adaptation of 3D Vision-Language Foundation Models](https://arxiv.org/abs/2511.15311)
*Mehran Tamjidi,Hamidreza Dastmalchi,Mohammadreza Alimoradijazi,Ali Cheraghian,Aijun An,Morteza Saberi*

Main category: cs.CV

TL;DR: 提出了Uni-Adapter，一种无需训练的在线测试时自适应策略，通过动态原型学习解决3D视觉语言基础模型在噪声、不完整或分布偏移数据上的性能下降问题。
...
Method: 基于动态原型学习，定义3D缓存存储类别特定的聚类中心作为原型，通过图基标签平滑模块捕获原型间相似性，并使用熵加权聚合统一原始模型和缓存预测。
...
TL;DR: 提出了MILL多光照低光数据集，用于评估低光增强算法在不同光照强度下的性能，并通过改进方法在DSLR和智能手机上分别实现了10dB和2dB的PSNR提升。


<details>
  <summary>Details</summary>
Motivation: 现有低光增强方法通常基于单一光照条件的配对数据训练，缺乏对不同光照强度下算法性能的理解，限制了方法的泛化能力。

Method: 引入MILL数据集，包含在固定相机设置和精确照度测量下捕获的不同光照强度图像；基于数据集的多光照结构提出改进方法以增强算法在多样化光照场景下的鲁棒性。

Result: 基准测试显示现有方法在不同光照强度下性能差异显著；改进方法在Full HD图像上，DSLR实现了10dB PSNR提升，智能手机实现了2dB PSNR提升。

Conclusion: MILL数据集为全面评估低光增强算法提供了重要工具，提出的改进方法显著提升了算法在不同光照条件下的鲁棒性和性能。
...
### [60] [A Multimodal Transformer Approach for UAV Detection and Aerial Object Recognition Using Radar, Audio, and Video Data](https://arxiv.org/abs/2511.15312)
...
Method: 设计多模态Transformer架构，利用自注意力机制融合雷达、RGB视频、红外视频和音频四种数据流的特征，学习全面、互补且高度区分的表示进行分类。
...
### [61] [CompTrack: Information Bottleneck-Guided Low-Rank Dynamic Token Compression for Point Cloud Tracking](https://arxiv.org/abs/2511.15580)
*Sifan Zhou,Yichao Cao,Jiahao Nie,Yuqian Fu,Ziyu Zhao,Xiaobo Lu,Shuo Wang*

Main category: cs.CV

TL;DR: CompTrack是一个用于3D LiDAR点云单目标跟踪的端到端框架，通过消除空间冗余和信息冗余，在保持高精度的同时实现实时跟踪性能。
...
Method: 包含两个核心模块：基于信息熵的空间前景预测器(SFP)过滤背景噪声，以及基于信息瓶颈理论的动态令牌压缩(IB-DTC)模块，通过在线SVD分析自适应压缩前景信息。

Result: 在KITTI、nuScenes和Waymo数据集上表现出色，在单个RTX 3090 GPU上达到实时90 FPS的跟踪速度。

Conclusion: CompTrack通过系统性地消除点云中的双重冗余，实现了高效且准确的3D目标跟踪，为自动驾驶等应用提供了实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: Split DNNs将密集计算卸载到云服务器，但中间特征可能被利用来重建私有输入，现有方法重建质量有限，难以评估隐私泄露的真实程度。

Method: 设计了潜在特征空间对齐模块(LFSAM)来弥合中间特征空间和潜在空间之间的语义差距，并开发了确定性反转流匹配(DIFM)来校正分布不匹配问题。

Result: 实验表明FIA-Flow在多种模型(AlexNet、ResNet、Swin Transformer、DINO、YOLO11)和各层上实现了更忠实和语义对齐的特征反转。

Conclusion: FIA-Flow揭示了Split DNNs中比之前认知更严重的隐私威胁，需要更强的隐私保护措施。
...
### [63] [The SA-FARI Dataset: Segment Anything in Footage of Animals for Recognition and Identification](https://arxiv.org/abs/2511.15622)
...
TL;DR: SA-FARI是最大的开源野生动物多目标跟踪数据集，包含11,609个相机陷阱视频，涵盖99个物种，提供密集的边界框、分割掩码和物种标注，为通用多动物跟踪模型提供基准。


<details>
  <summary>Details</summary>
Motivation: 现有数据集规模有限、物种单一、缺乏时空多样性，无法训练适用于野生动物种群的通用多动物跟踪模型。

Method: 收集10年间来自4大洲741个地点的相机陷阱视频，进行密集标注，包含16,224个掩码身份和942,702个个体标注，并使用最先进的视觉语言模型进行基准测试。

Result: 创建了首个结合高物种多样性、多区域覆盖和高质量时空标注的大规模数据集，为野生动物多目标跟踪提供了新基础。

Conclusion: SA-FARI填补了野生动物多目标跟踪领域的数据空白，为开发通用化模型提供了重要资源，推动野生动物保护中的视频分析技术发展。
...
<details>
  <summary>Details</summary>
Motivation: 指纹活体检测系统面临伪造威胁，需要开发能自动检测指纹伪造的技术，并且该技术需要能够抵抗各种失真干扰，防止攻击者通过添加失真来欺骗检测器。

Method: 使用各向异性扩散处理输入图像，通过三级小波变换，对不同层的系数进行自适应阈值处理并拼接成特征向量，最后使用SVM分类器进行分类。

Result: 在90%像素缺失和70x70块缺失场景下，所提方法的准确率分别比对比方法高出约8%和5%，表现出对多种失真的强抵抗力。

Conclusion: 该方法在应对环境现象或恶意用户操作引起的各种失真方面具有改进的抵抗能力，通过定量比较验证了其优越性能。
...
Method: 使用融合方案聚合多个置信度估计和每个检测特征，通过紧凑的多层感知机实现三分类，将不同logit变体纳入MLP以提升性能。

Result: 在二分类中平均提升2.7% AUROC，保持或改进开放集mAP，在闭集mAP上提升高达9个百分点（18%相对增益），支持稳健的三分类。
...
### [66] [GEO-Bench-2: From Performance to Capability, Rethinking Evaluation in Geospatial AI](https://arxiv.org/abs/2511.15658)
...
TL;DR: GEO-Bench-2是一个用于评估地理空间基础模型(GeoFMs)的标准化框架，包含19个开放数据集，涵盖分类、分割、回归、目标检测和实例分割任务，通过能力分组对模型进行排名。


<details>
  <summary>Details</summary>
Motivation: 当前地理空间基础模型的评估缺乏标准化协议，需要统一的评估框架来促进模型比较和方法创新。

Method: 引入能力分组概念，根据数据集特征(如分辨率、波段、时间性)对模型进行排名；制定规范且灵活的评价协议，支持公平比较和模型适应策略研究。

Result: 实验表明没有单一模型在所有任务中表现最佳：自然图像预训练模型在高分辨率任务中表现优异，而EO特定模型在多光谱应用中表现更好。

Conclusion: 最优模型选择取决于任务需求、数据模态和约束条件，单一GeoFM模型在所有任务中表现良好的目标仍需未来研究，GEO-Bench-2为特定用例提供了可复现的评估框架。
...
### [67] [VisPlay: Self-Evolving Vision-Language Models from Images](https://arxiv.org/abs/2511.15661)
*Yicheng He,Chengsong Huang,Zongxia Li,Jiaxin Huang,Yonghui Yang*

Main category: cs.CV

TL;DR: VisPlay是一个自演化的强化学习框架，让视觉语言模型能够利用大量未标记图像数据自主提升推理能力，通过角色分配和GRPO训练实现视觉推理、组合泛化和幻觉减少的改进。


<details>
  <summary>Details</summary>
Motivation: 现有的RL方法依赖人工标注标签或任务特定启发式来定义可验证奖励，成本高且难以扩展，需要一种能够自主改进视觉语言模型推理能力的可扩展方法。

Method: 将基础VLM分配到两个交互角色：图像条件提问者生成具有挑战性但可回答的视觉问题，多模态推理者生成银牌回答，使用GRPO进行联合训练，融入多样性和难度奖励。

Result: 在Qwen2.5-VL和MiMo-VL两个模型系列上训练后，VisPlay在八个基准测试（包括MM-Vet和MMMU）中在视觉推理、组合泛化和幻觉减少方面取得了持续改进。
...
Result: 在三个运动-语言数据集上实现最先进的运动定位性能，在HumanML3D基准上比先前方法提高+8.7% mAP，在下游检索任务中也有显著改进
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM剪枝方法依赖手动设计的算法，导致巨大的人工成本和专家知识需求，且在高剪枝率下存在严重的异常值问题导致性能急剧下降。

Method: 提出AutoPrune方法：1）利用LLM自动设计最优剪枝算法；2）使用图驱动思维链(GCoT)优化提示，增强推理过程；3）引入偏斜感知动态稀疏分配(SDSA)解决异常值问题。

Result: 在主流LLM基准测试上的广泛实验表明，AutoPrune始终优于最先进的竞争对手。

Conclusion: AutoPrune证明了LLM能够自我剪枝，克服了专家知识限制和异常值问题，实现了优越的性能和可解释性。
...
### [70] [MF-GCN: A Multi-Frequency Graph Convolutional Network for Tri-Modal Depression Detection Using Eye-Tracking, Facial, and Acoustic Features](https://arxiv.org/abs/2511.15675)
*Sejuti Rahman,Swakshar Deb,MD. Sameer Iqbal Chowdhury,MD. Jubair Ahmed Sourov,Mohammad Shamsuddin*

Main category: cs.CV

TL;DR: 提出了一种多频图卷积网络(MF-GCN)用于抑郁症检测，通过眼动追踪、音频和视频数据，利用多频滤波器模块同时捕捉低频和高频信号，在二元和三元分类任务中均优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于图的模型只关注低频信息的局限性，充分利用多模态数据中的跨模态交互信息来准确检测抑郁症。

Method: 开发了多频图卷积网络(MF-GCN)，包含新颖的多频滤波器模块(MFFBM)，能够同时利用低频和高频信号进行特征提取。

Result: 在二元分类中敏感度达0.96，F2分数0.94；在三元分类中敏感度0.79，特异度0.87；在CMDC数据集上验证泛化能力，敏感度0.95，F2分数0.96。

Conclusion: 该三模态多频框架能有效捕捉跨模态交互，实现准确的抑郁症检测，并在不同数据集上表现出良好的泛化性能。
...
Method: 通过过滤和累积跨帧的静态几何，处理动态内容，并将语义信息传播到稳定的体素表示中，缓解稀疏、噪声和不一致几何的问题。
...
### [72] [Controlling False Positives in Image Segmentation via Conformal Prediction](https://arxiv.org/abs/2511.15406)
...
<details>
  <summary>Details</summary>
Motivation: 深度分割模型很少提供错误的统计保证，而临床决策需要可靠的语义分割，过度分割可能带来临床后果。

Method: 使用预训练分割模型，通过增加分数阈值或形态学腐蚀构建嵌套的收缩掩码族，利用标记校准集通过保形预测选择收缩参数。

Result: 在息肉分割基准测试中实现了目标级别的经验有效性，确保新图像中假阳性比例低于用户指定容差。

Conclusion: 该框架实现了实用的风险感知分割，适用于过度分割可能产生临床后果的场景，无需重新训练且提供有限样本保证。
...
### [73] [Think Visually, Reason Textually: Vision-Language Synergy in ARC](https://arxiv.org/abs/2511.15703)
...
<details>
  <summary>Details</summary>
Motivation: 现有前沿基础模型在从少量示例中推断结构化转换规则方面仍然失败，而这是人类智能的关键特征。现有方法大多将ARC-AGI视为纯文本推理任务，忽视了人类在解决此类谜题时严重依赖视觉抽象的事实。

Method: 提出了两种协同策略：(1) 视觉-语言协同推理(VLSR)，将ARC-AGI分解为模态对齐的子任务；(2) 模态切换自校正(MSSC)，利用视觉验证基于文本的推理以实现内在错误校正。

Result: 在多种旗舰模型和多个ARC-AGI任务上的广泛实验表明，该方法比纯文本基线提高了高达4.33%的性能。
...
TL;DR: 提出了D4C框架，这是首个专门为CLIP模型设计的无数据量化方法，通过语义注入、结构对比生成和扰动增强三个组件，解决了现有DFQ方法在CLIP上性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 无数据量化在隐私敏感场景中具有实用价值，但现有方法直接应用于视觉语言模型（如CLIP）会导致显著性能下降，主要原因是合成样本语义内容不足和图像内部多样性低。

Method: D4C框架包含三个关键组件：(1) 提示引导语义注入，使用文本提示使生成图像与现实语义对齐；(2) 结构对比生成，利用前景-背景对比合成重现自然图像的组合结构；(3) 扰动感知增强，应用受控扰动提高样本多样性和鲁棒性。

Result: 在W4A8设置下，D4C在CLIP ResNet-50和ViT-B/32上显著提升了零样本分类性能：CIFAR-10上分别提升12.4%和18.9%，CIFAR-100上提升6.8%和19.7%，ImageNet-1K上提升1.4%和5.7%。

Conclusion: D4C是首个专门针对CLIP模型的无数据量化框架，通过生成语义丰富且结构多样的伪图像，有效弥补了DFQ在CLIP上的性能差距，在各种位宽和模型上都取得了显著改进。
...
TL;DR: WarNav是一个从DATTALION开源仓库图像构建的真实世界数据集，专门用于在冲突影响环境中开发自动驾驶地面车辆的语义分割模型，填补了传统城市驾驶资源与危险战区场景之间的空白。
...
Method: 从DATTALION开源仓库构建真实世界数据集，使用最先进的语义分割模型建立性能基准，分析训练数据环境的影响，并提出在目标图像无标注情况下实现有效导航性的初步方法。

Result: 建立了WarNav数据集的性能基准，展示了在结构化城市场景训练的模型在冲突环境中的表现，为无标注目标图像环境下的导航提供了初步解决方案。

Conclusion: WarNav数据集填补了自动驾驶在极端操作环境中的研究空白，为在标注数据有限的情况下开发鲁棒导航系统提供了重要资源，有助于提升高风险场景下自动驾驶车辆的安全性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态目标检测研究大多关注融合策略改进，但忽视了融合退化现象，且缺乏对其根本原因的理论分析。

Method: 提出了表示空间约束学习与模态解耦方法，包含RSC模块和MD模块，分别用于放大被抑制的梯度和消除模态间耦合干扰及不平衡问题。

Result: 在FLIR、LLVIP、M3FD和MFAD数据集上的大量实验表明，该方法有效缓解了融合退化，在多个基准测试中达到了最先进的性能。

Conclusion: RSC-MD方法通过理论分析和针对性设计，成功解决了多模态检测中的融合退化问题，为多模态学习提供了新的优化思路。
...
TL;DR: 提出了一个用于变速箱部件质量检测的新图像数据集，并开发了对比正则化损失来提高模型对未见部件类型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 再制造过程中部件质量检测主要依赖人工，深度神经网络在自动化视觉检测方面潜力巨大，但难以泛化到新的产品变体、部件或缺陷模式。

Method: 创建包含两种汽车变速箱典型齿轮箱部件在良好和缺陷状态下的图像数据集，通过不同的训练-测试数据分割生成分布偏移，并提出了对比正则化损失来增强模型鲁棒性。

Result: 评估了不同模型在数据集上的表现，结果表明对比正则化损失能够有效提高模型对未见部件类型的泛化能力。

Conclusion: 提出的对比正则化损失方法能够显著提升分类模型在再制造质量检测任务中的泛化性能，特别是在面对新类型部件时。
...
TL;DR: EASD是一个端到端的脉冲相机检测器，采用双分支设计解决自动驾驶中脉冲流物体检测的挑战，并提出了首个面向驾驶的模拟脉冲检测基准DSEC Spike。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中的物体检测在快速运动和极端光照条件下存在运动模糊和饱和问题。脉冲相机虽然提供微秒级延迟和超高动态范围，但其稀疏、离散的输出无法被标准基于图像的检测器处理。

Method: 提出EASD检测器，包含两个分支：基于时间的纹理加特征融合分支用于全局跨切片语义，以及熵选择性注意力分支用于物体中心细节。同时引入DSEC Spike作为首个面向驾驶的模拟脉冲检测基准。

Result: 该方法解决了脉冲相机输出无法被标准检测器处理的关键挑战，实现了端到端的脉冲流检测。

Conclusion: EASD通过双分支设计有效解决了脉冲相机在自动驾驶物体检测中的挑战，DSEC Spike基准填补了数据空白，为脉冲相机在自动驾驶中的应用提供了重要支持。
...
Method: 提出Sigmma框架，包含多尺度对比对齐确保不同尺度表示在模态间一致，并通过图表示细胞相互作用，整合图间和图内关系来捕捉组织微环境中的细胞间相互作用。

Result: 在基因表达预测任务中平均提升9.78%，在跨模态检索任务中平均提升26.93%，并能学习有意义的跨组织组织结构。

Conclusion: Sigmma通过多尺度对比对齐和图表示细胞相互作用，有效学习HE图像和空间转录组谱的分层表示，显著提升了跨模态对应关系的捕捉能力。
...
TL;DR: 该研究开发了一个AI多阶段流水线，用于从围网渔船电子监控视频中自动识别大眼金枪鱼和黄鳍金枪鱼，结合YOLOv9-SAM2分割和分层分类方法，实现了84.8%的个体分割分类准确率。
...
Method: 开发多阶段流水线：比较三种分割方法（Mask R-CNN、DINOv2+SAM2、YOLOv9+SAM2），使用ByteTrack进行个体追踪，评估多类分类和分层分类方法。

Result: YOLOv9-SAM2表现最佳，验证平均精度0.66±0.03，召回率0.88±0.03。结合分层分类，84.8%的个体被成功分割和分类，平均误差4.5%。专家对大眼金枪鱼和黄鳍金枪鱼的识别一致性分别为42.9%±35.6%和57.1%±35.6%。

Conclusion: YOLOv9-SAM2与分层分类相结合的方法能够有效估计金枪鱼渔获物种类组成，为电子监控系统的自动化分析提供了可靠解决方案。
...
Method: 创建FunnyNodules数据集，生成具有可控视觉属性（如圆度、边缘锐度、毛刺等）的抽象肺结节形状，目标类别来自预定义的属性组合，允许完全控制将属性链接到诊断类别的决策规则。

Result: 展示了FunnyNodules可用于模型无关评估，评估模型是否学习正确的属性-目标关系，解释属性预测中的过度或不足表现，并分析注意力与属性特定感兴趣区域的对齐情况。

Conclusion: FunnyNodules提供了一个多功能基础，用于在医学图像分析中开发、基准测试和进行可解释AI方法的深入分析，具有完整的地面真实信息支持。
...
TL;DR: 提出了一种名为EAR的视觉生成方法，通过模拟人类视觉系统的中心向外感知模式，以螺旋顺序展开图像token，实现高效并行解码和更好的生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有的自回归视觉生成方法存在效率低下的问题，要么是token逐个解码效率低，要么是多尺度表示复杂。受人类视觉系统中心向外感知模式的启发，希望开发更高效的自回归生成方法。

Method: 提出Expanding Autoregressive Representation (EAR)方法，采用螺旋顺序从中心向外展开图像token，保持空间连续性并支持并行解码。同时提出长度自适应解码策略，动态调整每步预测的token数量。

Result: 在ImageNet上的大量实验表明，EAR在单尺度自回归模型中实现了保真度和效率之间的最先进权衡。

Conclusion: EAR为可扩展且认知对齐的自回归图像生成设定了新方向，通过生物启发设计不仅降低了计算成本，还提高了生成质量。
...
TL;DR: 提出了一种新颖的多模态深度学习方法，利用X射线图像信息来补全3D超声中被遮挡的脊柱解剖结构，解决了超声成像中因骨骼声影导致的椎体可视化限制问题。


<details>
  <summary>Details</summary>
Motivation: 超声成像在脊柱手术中具有无辐射、实时可视化的优势，但由于骨骼声影效应，无法完整显示椎体等解剖结构。需要解决超声在脊柱可视化中的这一关键限制。

Method: 通过生成配对的训练数据（模拟X射线扫描的2D侧位椎体视图和模拟超声成像局限性的3D部分椎体表示），开发多模态深度学习模型，整合两种成像模式的形态学信息。

Result: 在椎体重建方面相比现有3D超声椎体补全方法有显著改进（p < 0.001），实现了更准确、完整的腰椎体积可视化，无需与术前CT等模式配准。

Conclusion: 集成单个X射线投影可以缓解超声的关键限制，同时保留其作为主要成像模式的优势，为未来临床转化提供了初步验证。
...
TL;DR: MTGNet是一个双分支框架，通过融合多样文本提示来改进文本先验并指导视觉先验的跨模态优化，解决现有方法中目标区域激活不完整和缺乏显式跨模态交互的问题。
...
Method: 提出MTGNet框架，包含三个模块：多文本先验精炼模块(MTPR)抑制干扰并聚合互补语义线索；文本锚点特征融合模块(TAFF)利用多文本嵌入作为语义锚点促进局部原型的传递；前景置信度加权注意力模块(FCWA)增强视觉先验鲁棒性。

Result: 在标准FSS基准测试中，1-shot设置下在PASCAL-5i上达到76.8% mIoU，在COCO-20i上达到57.4% mIoU，在具有高类内变异的fold上表现显著提升。

Conclusion: MTGNet通过融合多文本提示和跨模态交互，有效提升了少样本语义分割的性能，特别是在处理语义多样性和类内变异方面表现出色。
...
TL;DR: HASTEN方法通过将层次语义信息锚定到类增量学习中，在双曲空间中嵌入视觉和文本特征来减少灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有的基于CLIP的类增量学习方法未能显式捕捉视觉和语言概念固有的层次结构，导致细粒度类别特征漂移和灾难性遗忘。

Method: 1. 使用外部知识图谱作为监督，在双曲空间中嵌入视觉和文本特征以保持层次结构；2. 将梯度投影到共享双曲映射器的零空间，防止对先前任务的干扰。

Result: 大量实验表明HASTEN始终优于现有方法，并提供统一的结构化表示。

Conclusion: HASTEN通过层次语义树锚定有效减少灾难性遗忘，在类增量学习中保持层次关系。
...
TL;DR: 提出一种混合深度学习框架用于杂草检测，结合CNN、ViT和GNN，通过GAN数据增强和自监督对比预训练，在多个基准数据集上达到99.33%的准确率。


<details>
  <summary>Details</summary>
Motivation: 精确杂草检测是精准农业的关键，准确的物种识别允许农民选择性施用除草剂，符合可持续农业管理需求。
...
Conclusion: 该框架实现了局部、全局和关系特征表示，可实时高效部署到边缘设备进行自动化杂草检测，减少对除草剂的过度依赖，提供可扩展的可持续精准农业解决方案。
...
TL;DR: 本文评估了人体姿态预测算法，发现可复现性问题，提出了统一的训练评估流程，并将语音模型成功应用于姿态预测任务，提升了SOTA性能。同时评估了模型在噪声条件下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 人体姿态预测在动作识别、自动驾驶和人机交互等领域有重要应用，但现有算法存在可复现性问题，且需要评估模型在真实噪声条件下的性能。

Method: 构建统一的训练和评估流程，将语音理解模型适配到姿态预测任务，引入新的数据集变体来模拟真实噪声条件，并使用无监督微调来恢复性能。

Result: 语音模型能够有效提升姿态预测性能，达到新的SOTA；在噪声条件下模型性能显著下降，但通过无监督微调可以部分恢复性能。

Conclusion: 语音模型可以成功迁移到姿态预测任务，但模型对噪声敏感，需要通过无监督微调来提升鲁棒性，这对实际应用具有重要意义。
...
TL;DR: 本文提出了一个Coder-CUA协作框架，让编码语言模型作为GUI设计师，计算机使用代理作为评估者，通过任务可解性和导航成功率来评估和优化GUI设计，推动界面设计向代理原生效率发展。


<details>
  <summary>Details</summary>
Motivation: 当前GUI主要为人机交互设计，迫使代理采用人类导向的行为，而编码语言模型的进步为自动GUI设计提供了可能。本文旨在探索计算机使用代理能否作为评估者协助编码模型进行自动GUI设计。

Method: 引入AUI-Gym基准测试，包含52个应用的1560个任务；开发验证器确保任务可执行性；提出Coder-CUA协作框架：Coder作为设计师生成和修改网站，CUA作为评估者评估功能性和优化设计；设计CUA仪表板将多步导航历史压缩为可视化总结。

Result: 成功构建了包含52个应用和1560个任务的基准测试；开发了可靠的验证器；实现了Coder与CUA的有效协作框架；设计了可解释的CUA仪表板用于迭代重设计。

Conclusion: 通过将代理定位为设计者和评估者，该框架将界面设计转向代理原生效率和可靠性，使代理从被动使用转向在数字环境中主动参与。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测器在干净条件下表现优异，但在对抗攻击下的安全性评估尚未充分探索。需要开发先进的对抗攻击方法来评估这类检测器的安全性。

Method: 通过空间插值梯度和频率感知扰动捕获取证重要特征，联合建模空间域和频域特征重要性，并将两者融合来指导基于优化的对抗样本生成。
...
TL;DR: 本文通过两视角表示分析揭示了ViT特征蒸馏失败的原因：尽管全局低秩，但单个token使用高带宽编码模式导致师生宽度不匹配，提出了两种最小化策略成功激活ViT特征蒸馏。


<details>
  <summary>Details</summary>
Motivation: 理解为什么特征图知识蒸馏在卷积网络中有效但在Vision Transformers中失败，并指导方法设计。

Method: 采用层间奇异值分解和token级谱能量模式分析，提出两种策略：后验特征提升和原生宽度对齐。

Result: 在ImageNet-1K上，将DeiT-Tiny从74.86%提升到77.53%和78.23%，同时改善了无教师训练的独立学生模型。

Conclusion: 分析解释了ViT特征蒸馏失败的原因，并展示了如何利用低秩结构产生有效的、可解释的补救措施和紧凑ViT的具体设计指导。
...
Method: AVATAAR框架包含：创建持久全局摘要、结合全局和局部视频上下文、预检索思考代理、重新思考模块，以及两者之间的反馈循环，允许系统基于部分答案优化检索策略。

Result: 在CinePile基准测试中，相比基线模型，AVATAAR在时间推理上相对提升5.6%，技术查询提升5%，主题问题提升8%，叙事理解提升8.2%。实验证实每个模块都对整体性能有积极贡献。

Conclusion: AVATAAR为长视频问答提供了一个可扩展的解决方案，融合了准确性、可解释性和可扩展性，其反馈循环对于适应性至关重要。
...
Result: 在多个基准测试中的广泛实验表明，KDF+持续优于现有持续学习方法，并能无缝集成到最先进的LiDAR地点识别框架中，带来显著且稳定的性能提升。

Conclusion: KDF+有效解决了LiDAR地点识别中的灾难性遗忘问题，通过创新的采样和增强机制实现了更好的知识保留和性能表现。
...
TL;DR: 提出了一种统一的解耦分割头，将多类预测分为类无关掩码预测和类标签预测，并引入全尺度感知可变形Transformer模块实现内存高效的全尺度融合。
...
Method: 1. 统一的解耦分割头：使用共享对象查询分离类无关掩码预测和类标签预测；2. 全尺度感知可变形Transformer模块：通过可变形注意力使低分辨率编码器特征关注全分辨率编码器特征。

Result: 在AMOS 2022数据集上Dice系数比nnUNet提升+2.0%，在BTCV数据集上提升+6.9%，达到最先进性能。

Conclusion: MaskMed方法通过解耦分割头和全尺度融合机制，显著提升了医学图像分割的性能和泛化能力。
...
TL;DR: 本文系统分析了测试时思考对视觉推理的影响，发现更多思考并不总是更好，长推理链容易忽略图像信息。基于此提出了不确定性引导回看方法，在多个基准测试中显著提升性能。
...
Method: 对InternVL3.5和Qwen3-VL家族的10个变体进行大规模对比分析；提出不确定性引导回看方法，结合不确定性信号、自适应回看提示和广度搜索的无训练解码策略。

Result: 发现短回看短语与成功的推理轨迹强相关；提出的方法在MMMU基准上提升性能，在标准思考较弱类别中取得最大增益，在固定模型家族和token预算下达到新SOTA；在5个额外基准测试中一致有效。

Conclusion: 测试时思考需要谨慎设计，简单的长推理链可能适得其反；通过不确定性引导的回看机制可以有效提升视觉推理性能，该方法具有很好的泛化能力。
...
Method: 提出FlashMesh框架，采用预测-校正-验证范式，利用网格token的结构和几何相关性进行多token推测，为hourglass transformer架构定制推测解码方案，实现面、点、坐标层级的并行预测

Result: 实验显示FlashMesh相比标准自回归模型实现最高2倍加速，同时提升生成保真度
...
<details>
  <summary>Details</summary>
Motivation: 解决超声应变弹性成像中组织去相关噪声、真实数据稀缺以及不同变形条件下应变估计不一致的问题，以扩展其临床应用。

Method: 基于USSE-Net的多流编码器-解码器架构，并行处理变形前后的RF序列，结合上下文感知互补特征融合编码器、三交叉注意力瓶颈和交叉注意力融合解码器，采用一致性损失确保时间相干性，并通过残差细化阶段提升精度。

Result: 在模拟数据上达到目标SNR 24.54、背景SNR 132.76、CNR 59.81和弹性成像SNR 9.73；在BUET数据集上产生增强病变-背景对比度和显著噪声抑制的应变图。

Conclusion: MUSSE-Net在超声应变弹性成像中实现了最先进的性能，提供临床可解释的应变模式，具有重要的临床应用价值。
...
TL;DR: MambaIO是一种基于Mamba架构的惯性里程计方法，通过拉普拉斯金字塔分解IMU测量值，分别处理高低频分量，在多个数据集上实现了最先进的定位精度。


<details>
  <summary>Details</summary>
Motivation: 重新评估全局坐标系在行人惯性里程计中的适用性，因为在无人机场景中发现机体坐标系能显著提高定位精度。

Method: 使用拉普拉斯金字塔将IMU测量分解为高频和低频分量，低频分量由Mamba架构处理以提取隐式上下文运动线索，高频分量由卷积结构处理以捕获细粒度局部运动细节。

Result: 在多个公共数据集上的实验表明，MambaIO显著降低了定位误差，达到了最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 大型社区科学平台包含数亿张生物多样性图像，但传统方法只能通过元数据过滤或人工检查，无法有效利用图像中的生态背景信息。

Method: 开发了INQUIRE-Search系统，允许科学家使用自然语言在生态图像数据库中交互式搜索特定概念，验证并导出相关观测数据。

Result: 通过五个案例研究证明，相比传统方法，INQUIRE-Search只需极短时间就能完成搜索，支持从物种行为季节性变化到火灾后森林再生等多种科学应用。

Conclusion: INQUIRE-Search展示了交互式、高效、可扩展科学发现的新范式，能够解锁大规模生物多样性数据集中以前无法获取的科学价值，同时要求专家重新构建科学过程的优先事项。
...
TL;DR: MoDES是一个无需训练的高效MoE多模态大语言模型推理框架，通过全局调制局部门控机制和双模态阈值方法，自适应跳过冗余专家，在保持性能的同时显著提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有的专家跳过方法原本为单模态大语言模型设计，直接应用于多模态MoE模型会导致性能显著下降，因为这些方法未能考虑专家在不同MoE层的异构贡献和模态特定行为。

Method: 提出全局调制局部门控机制，将全局层重要性整合到局部路由概率中；采用双模态阈值方法分别处理不同模态的token；引入利用单调性特性的前沿搜索算法快速确定最优阈值。

Result: 在3个模型系列和13个基准测试中，MoDES显著优于现有方法。当跳过88%专家时，Qwen3-VL-MoE-30B-A3B-Instruct性能提升达10.67%，预填充时间提升2.16倍，解码时间提升1.26倍。

Conclusion: MoDES是首个无需训练的自适应专家跳过框架，能够有效解决多模态MoE模型的计算效率问题，在保持准确性的同时大幅提升推理速度。
...
TL;DR: SS-MixNet是一个轻量级深度学习模型，用于高光谱图像分类，在仅使用1%标注数据的情况下，在QUH-Tangdaowan和QUH-Qingyun数据集上分别达到95.68%和93.86%的总体准确率。
...
Method: 结合3D卷积层进行局部光谱-空间特征提取，使用两个并行MLP风格的混合器块捕获光谱和空间维度的长程依赖关系，并采用基于深度卷积的注意力机制增强判别能力。
...
<details>
  <summary>Details</summary>
Motivation: 挑战传统观点，探索视频生成模型中第一帧的真正作用，发现其作为概念记忆缓冲区的隐含功能，为视频内容定制提供新思路。

Method: 利用第一帧作为概念记忆缓冲区的特性，通过少量训练样本（20-50个）进行视频内容定制，无需架构修改或大规模微调。

Result: 实现了在各种场景下鲁棒且通用的视频内容定制，验证了视频生成模型在基于参考的视频定制方面的强大但被忽视的能力。

Conclusion: 视频生成模型具有强大的参考视频定制能力，第一帧作为概念记忆缓冲区的发现为高效视频定制开辟了新途径。
...
TL;DR: 提出了GeoVista智能体模型和GeoBench基准测试，用于评估智能体在地理定位任务中的表现，通过结合图像放大和网络搜索工具，在推理循环中实现精准定位。


<details>
  <summary>Details</summary>
Motivation: 现有智能体视觉推理研究主要关注图像处理工具，缺乏面向通用智能体模型的地理定位能力评估，需要同时具备视觉基础和网络搜索能力。

Method: 开发GeoVista智能体模型，集成图像放大和网络搜索工具；采用两阶段训练流程：监督微调学习推理模式和工具使用，强化学习阶段提升推理能力；使用分层奖励利用多级地理信息。

Result: GeoVista在地理定位任务上大幅超越其他开源智能体模型，在大多数指标上达到与Gemini-2.5-flash和GPT-5等闭源模型相当的性能。

Conclusion: 提出的GeoBench基准和GeoVista模型为智能体地理定位任务提供了有效的评估框架和解决方案，证明了工具集成和分层训练策略的有效性。
...
TL;DR: 提出了一种新颖的密集特征匹配架构，通过系统性的改进解决了现有密集匹配器在复杂场景下性能不佳和速度慢的问题，实现了新的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有密集特征匹配器在复杂现实场景中经常失败或表现不佳，高精度模型通常速度较慢，限制了其实际应用。
...
Conclusion: 通过系统性的架构改进、训练优化和基础模型集成，成功开发出在准确性和效率方面都显著优于现有方法的密集特征匹配器。
...
### [104] [Transformer Injectivity & Geometric Robustness - Analytic Margins and Bi-Lipschitz Uniformity of Sequence-Level Hidden States](https://arxiv.org/abs/2511.14808)
*Mikael von Strauss*

Main category: cs.LG

TL;DR: 本文研究了仅解码器Transformer中从离散提示到最后一个token隐藏状态的映射的注入性。证明了在非奇异优化器和绝对连续初始化条件下，通用注入性在训练过程中持续存在。通过经验研究定义了分离边界和共Lipschitz常数，发现在全精度和8位量化下没有碰撞，而4位量化会导致少量碰撞。


<details>
  <summary>Details</summary>
Motivation: 研究Transformer表示在离散提示到隐藏状态映射中的注入性，理解模型参数空间中通用注入性的存在条件及其在训练过程中的持续性。

Method: 理论分析：定义碰撞判别式和注入层，证明参数空间中的二分法；经验研究：定义分离边界和共Lipschitz常数，通过最近邻统计在大规模提示集上估计，应用于预训练LLaMA-3和Qwen模型。

Result: 理论证明：在温和条件下通用注入性沿训练轨迹持续存在；经验发现：全精度和8位量化下无碰撞，4位量化导致少量碰撞并显著缩小共Lipschitz估计；小GPT-2训练中标准化指标保持稳定。

Conclusion: Transformer表示在连续参数理想化下是通用且持续注入的，其实际可逆性可通过简单几何诊断进行探测。
...
### [105] [DEVAL: A Framework for Evaluating and Improving the Derivation Capability of Large Language Models](https://arxiv.org/abs/2511.14813)
*Yifan Li,Qin Li,Min Zhang,Min Zhang,Peixin Wang*

Main category: cs.LG

TL;DR: 本文提出了推导关系(DR)和推导能力(DC)的概念来评估LLMs的推理能力，开发了DEVAL评估框架，发现主流LLMs在DR识别方面表现中等但在应用DR解决问题时显著下降，并提出了推导提示(DP)方法平均提升15.2%的DC表现。


<details>
  <summary>Details</summary>
Motivation: 评估LLMs在数据上的推理能力是一个开放且紧迫的研究问题。与人类推理相比，LLMs缺乏基于输入变化进行相应输出修改的推理模式，这种依赖于抽象规则的推理能力尚未得到全面描述和评估。

Method: 1. 正式定义推导关系(DR)和推导能力(DC)概念；2. 提出系统构建的评估框架DEVAL；3. 评估5个流行LLMs和1个大推理模型在7个主流任务中的表现；4. 提出推导提示(DP)方法来提升DC。

Result: 评估结果显示主流LLMs如GPT-4o和Claude3.5在DR识别方面表现中等，但在应用DR有效解决问题时出现显著下降。推导提示(DP)方法使所有测试LLMs的DC平均提升15.2%，优于常用提示工程技术。

Conclusion: LLMs在推导能力方面存在显著不足，但通过推导提示方法可以有效提升其表现，为改进LLMs的推理能力提供了有效途径。
...
### [106] [Dynamic Nested Hierarchies: Pioneering Self-Evolution in Machine Learning Architectures for Lifelong Intelligence](https://arxiv.org/abs/2511.14823)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器学习模型在静态任务中表现出色，但在非平稳环境中由于架构僵化而表现不佳，无法实现持续适应和终身学习。受神经可塑性启发，需要让模型能够自主进化以适应环境变化。

Method: 基于嵌套学习范式，提出动态嵌套层次结构，允许模型在训练或推理过程中自主调整优化层级数量、嵌套结构和更新频率，无需预定义约束。

Result: 通过严格的数学公式、收敛性理论证明、表达能力界限和不同机制下的次线性遗憾分析，以及在语言建模、持续学习和长上下文推理中的实证演示，证明了该方法的优越性能。

Conclusion: 动态嵌套层次结构为实现自适应、通用智能奠定了基础性进展，解决了现有模型的前向性遗忘问题，通过动态压缩上下文流和适应分布变化来促进真正的终身学习。
...
### [107] [Empowering Multi-Turn Tool-Integrated Reasoning with Group Turn Policy Optimization](https://arxiv.org/abs/2511.14846)
*Yifeng Ding,Hung Le,Songyang Han,Kangrui Ruan,Zhenghui Jin,Varun Kumar,Zijian Wang,Anoop Deoras*

Main category: cs.LG

TL;DR: 提出了GTPO算法，针对多轮工具集成推理任务中的强化学习训练挑战，通过轮级奖励分配、基于回报的优势估计和自监督奖励塑造来改进训练效果。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在多轮工具集成推理任务中存在训练停滞问题，轨迹级奖励过于粗糙，无法为复杂的多轮交互提供足够的学习信号。

Method: GTPO算法包含三个关键创新：轮级奖励分配、基于回报的优势估计、以及利用生成代码自监督信号来丰富稀疏二元结果奖励的自监督奖励塑造。

Result: 在多样化推理基准测试中，GTPO平均比GRPO表现提升3.0%。

Conclusion: GTPO算法有效推进了复杂数学推理在现实世界中的应用，为多轮工具集成推理任务提供了更有效的强化学习训练方法。
...
### [108] [FinTRec: Transformer Based Unified Contextual Ads Targeting and Personalization for Financial Applications](https://arxiv.org/abs/2511.14865)
*Dwipam Katariya,Snehita Varma,Akshat Shreemali,Benjamin Wu,Kalanand Mishra,Pranab Mohanty*

Main category: cs.LG

TL;DR: FinTRec是一个面向金融服务的Transformer推荐框架，解决了长序列用户交互和多产品协调的挑战，相比传统树模型在实时推荐中表现更优


<details>
  <summary>Details</summary>
Motivation: 金融服务领域的实时推荐面临独特挑战：长序列用户交互产生的时间异质性上下文，以及多产品间需要协调建模来平衡业务目标

Method: 提出FinTRec框架，基于Transformer架构，支持产品适配微调，实现跨产品信号共享

Result: 通过历史模拟和在线A/B测试，FinTRec持续优于生产级树模型基线，减少训练成本和技术债务，提升所有产品的离线性能

Conclusion: 这是金融服务领域首个全面研究统一序列推荐模型的工作，证明了Transformer架构在金融服务中的可行性和有效性
...
### [109] [Transformer-Guided Deep Reinforcement Learning for Optimal Takeoff Trajectory Design of an eVTOL Drone](https://arxiv.org/abs/2511.14887)
...
<details>
  <summary>Details</summary>
Motivation: eVTOL飞机有望缓解城市交通拥堵，但需要开发最优起飞轨迹以最小化能耗。传统最优控制方法受限于问题维度和复杂度，而深度强化学习训练困难限制了其应用。

Method: 提出Transformer引导的DRL方法，使用Transformer在每个时间步探索真实状态空间，通过调整功率和机翼角度来控制eVTOL无人机的起飞轨迹。

Result: Transformer引导的DRL仅需4.57×10^6时间步完成训练，是传统DRL所需时间步的25%；能耗优化准确率达到97.2%，优于传统DRL的96.3%。

Conclusion: Transformer引导的DRL在训练效率和最优设计验证方面均优于传统DRL，为eVTOL飞机的最优轨迹设计提供了有效解决方案。
...
### [110] [Bringing Federated Learning to Space](https://arxiv.org/abs/2511.14889)
*Grace Kim,Filip Svoboda,Nicholas Lane*

Main category: cs.LG

TL;DR: 本文首次系统分析了在卫星星座中部署联邦学习的可行性，提出了将地面FL算法适配空间约束的"空间化"框架，并通过768种星座配置测试证明空间化FL算法能在100颗卫星规模下接近集中式性能，训练周期从数月缩短至数天。


<details>
  <summary>Details</summary>
Motivation: 随着低地球轨道卫星星座规模扩大到数百上千颗，下行带宽限制使得分布式星上机器学习变得至关重要。联邦学习为卫星网络中的协同模型训练提供了有前景的框架，但需要解决空间特有的约束条件。

Method: 提出了"空间化"框架，将地面FL算法（FedAvg、FedProx、FedBuff）适配轨道约束，创建了轨道就绪的FL算法套件。通过768种星座配置进行广泛参数扫描，测试不同集群规模、每集群卫星数量和地面站网络配置。

Result: 空间化FL算法能有效扩展到100颗卫星规模，性能接近集中式理想情况。多月的训练周期可缩短至数天，通过轨道调度和卫星集群内本地协调实现了9倍加速。

Conclusion: 研究结果为未来任务设计者提供了可操作的见解，使分布式星上学习能够支持更自主、弹性和数据驱动的卫星操作。
...
### [111] [It's LIT! Reliability-Optimized LLMs with Inspectable Tools](https://arxiv.org/abs/2511.14903)
...
<details>
  <summary>Details</summary>
Motivation: LLMs推理过程不透明，在高风险领域难以信任，且可能选择不可靠的解决方案

Method: 基于现有LLMs的工具调用能力构建框架，让模型选择最可靠且易于调试的解决方案路径，可能涉及多个顺序工具调用

Result: 创建了包含1,300个问题的新基准数据集和可定制的可靠性成本函数，证明LLMs可以在保持任务性能的同时实现更可靠的问题解决
...
### [112] [Structured Contrastive Learning for Interpretable Latent Representations](https://arxiv.org/abs/2511.14920)
*Zhengyang Shen,Hua Tu,Mayue Shi*

Main category: cs.LG

TL;DR: 提出结构化对比学习(SCL)框架，通过将潜在空间划分为不变特征、变异特征和自由特征三个语义组，解决神经网络对语义无关变换的脆弱性问题，在ECG相位偏移和IMU旋转等场景中显著提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 神经网络对语义无关变换（如ECG相位偏移、IMU传感器旋转）表现出严重脆弱性，这些变换会显著降低模型性能，根源在于"放任式"表示学习，即只要任务性能满足，潜在空间演化就不受约束。

Method: 提出结构化对比学习(SCL)框架，将潜在空间表示划分为三个语义组：不变特征（在给定变换下保持一致）、变异特征（通过新颖的变异机制主动区分变换）和自由特征（保持任务灵活性），无需架构修改即可集成到现有训练流程中。

Result: 在ECG相位不变性和IMU旋转鲁棒性实验中表现优异：ECG相似度从0.25提升到0.91，WISDM活动识别达到86.65%准确率和95.38%旋转一致性，始终优于传统数据增强方法。

Conclusion: 这项工作代表了从反应式数据增强到主动式结构学习的范式转变，实现了神经网络中可解释的潜在表示，为构建更鲁棒和可解释的AI系统提供了新途径。
...
### [113] [Integrating Causal Inference with Graph Neural Networks for Alzheimer's Disease Analysis](https://arxiv.org/abs/2511.14922)
*Pranay Kumar Peddi,Dhrubajyoti Ghosh*

Main category: cs.LG

TL;DR: Causal-GCN是一个基于干预的图卷积框架，通过do-calculus后门调整识别对阿尔茨海默病进展具有稳定因果影响的大脑区域，在保持分类性能的同时提供可解释的因果效应排序。
...
Method: 将每个受试者的MRI表示为结构连接组，节点表示皮层和皮层下区域，边编码解剖连接性。通过主成分分析总结年龄、性别和APOE4基因型等混杂因素，使用do-calculus后门调整进行因果干预模拟。

Result: 在ADNI队列的484名受试者上，Causal-GCN达到与基线GNN相当的分类性能，同时提供可解释的因果效应排序，突出显示后部、扣带回和岛叶枢纽区域，与已知AD神经病理学一致。

Conclusion: Causal-GCN框架成功整合因果推理与图神经网络，在保持分类准确性的同时识别出与AD病理一致的因果重要大脑区域，为神经退行性疾病的机制理解提供了新视角。
...
### [114] [How to Train Private Clinical Language Models: A Comparative Study of Privacy-Preserving Pipelines for ICD-9 Coding](https://arxiv.org/abs/2511.14936)
*Mathieu Dufour,Andrew Duncan*

Main category: cs.LG

TL;DR: 系统比较了四种隐私保护训练方法在临床诊断编码任务中的表现，发现在中等隐私预算下，基于DP训练教师模型的知识蒸馏方法效果最佳，能恢复63%的非私有模型性能。


<details>
  <summary>Details</summary>
Motivation: 临床文本训练的大语言模型存在泄露敏感患者信息的风险，但差分隐私方法通常会严重降低诊断准确性，需要找到有效的隐私保护策略。

Method: 使用相同的10亿参数模型和匹配的隐私预算，系统比较了四种训练流程：直接DP-SGD、DP合成数据训练、知识蒸馏等，用于从医院出院总结中预测ICD-9诊断代码。

Result: 在中等隐私预算(ε=4,6)下，基于DP训练教师模型的知识蒸馏方法表现最好，能恢复63%的非私有模型性能，同时保持强大的经验隐私保护能力。
...
### [115] [Knowledge Graphs as Structured Memory for Embedding Spaces: From Training Clusters to Explainable Inference](https://arxiv.org/abs/2511.14961)
*Artur A. Oliveira,Mateus Espadoto,Roberto M. Cesar,Roberto Hirata*

Main category: cs.LG

TL;DR: Graph Memory (GM) 是一个结构化非参数框架，通过区域级原型的紧凑关系记忆增强基于嵌入的推理。它将嵌入空间总结为带有可靠性指标的原型节点，并通过编码几何和上下文关系的边连接，统一了实例检索、原型推理和图标签传播。


<details>
  <summary>Details</summary>
Motivation: 传统的非参数方法通常孤立处理训练实例，缺乏对嵌入空间结构和可靠性的显式建模。GM旨在通过关系记忆框架在局部证据和全局一致性之间建立原则性桥梁。

Method: GM将嵌入空间总结为原型节点，每个节点带有可靠性指标，并通过边编码几何和上下文关系。该框架统一了实例检索、原型推理和图标签传播，支持高效推理和可信解释。

Result: 在合成和真实数据集（包括乳腺癌组织病理学IDC）上的实验表明，GM在准确率上与kNN和标签传播方法相当，但提供了更好的校准度、更平滑的决策边界，且样本数量少一个数量级。

Conclusion: 通过显式建模可靠性和关系结构，GM为非参数学习中局部证据和全局一致性之间提供了原则性桥梁，在保持竞争力的准确率同时显著提升了校准性和效率。
...
### [116] [IonCast: A Deep Learning Framework for Forecasting Ionospheric Dynamics](https://arxiv.org/abs/2511.15004)
...
TL;DR: IonCast是一个用于电离层总电子含量(TEC)预测的深度学习模型套件，基于GraphCast架构，通过时空学习和多源数据融合来提升空间天气预报能力。


<details>
  <summary>Details</summary>
Motivation: 电离层对GNSS精度、高频通信和航空运营至关重要，但准确预测电离层变化存在挑战，需要开发更好的预报模型。

Method: 采用GraphCast启发的深度学习模型，利用时空学习技术整合多种物理驱动因素和观测数据集，进行全球TEC预测。

Result: 在风暴期和宁静期的验证测试中，相比持续性预测方法表现出更高的预测技能。

Conclusion: 通过将异构数据与可扩展的基于图的时空学习相结合，IonCast展示了机器学习如何增强对电离层变化性的物理理解，并提升空间天气业务韧性。
...
TL;DR: 该研究开发了一个动态时间序列环境来模拟教室设置，通过强化学习智能辅导系统结合个体学生状态学习和群体信息，使用探测干预来平衡信息获取与学生干扰。


<details>
  <summary>Details</summary>
Motivation: 智能辅导系统虽然可以利用过去学生的信息进行个性化教学，但每个新学生都是独特的，且学习过程本质上是部分可观测的。需要解决如何在获取准确学生状态估计与避免过度干扰之间找到平衡的问题。

Method: 开发了模拟教室环境的动态时间序列框架，设计允许不同层次探测干预的系统。开发了结合个体状态学习和群体信息的强化学习ITS，并与基于规则的启发式方法进行比较。

Result: 标准RL算法与启发式方法提供不同解决方案但效果相似。探测干预能显著提升性能，特别是在信息隐藏程度较高时。RL策略在帮助困难班级方面表现不佳，但两种方法都能适应学生群体分布变化。

Conclusion: 探测干预能有效提升ITS性能，特别是在有测验和期中考试的结构中效果更佳。RL和启发式方法各有优势，但都需要在信息获取与学生干扰之间找到平衡。
...
### [118] [Oversampling techniques for predicting COVID-19 patient length of stay](https://arxiv.org/abs/2511.15048)
...
<details>
  <summary>Details</summary>
Motivation: COVID-19症状严重程度差异大，需要基于电子健康记录预测患者住院时间作为严重程度指标，但这是一个数据不平衡的分类问题

Method: 使用合成过采样技术处理数据不平衡，然后通过人工神经网络建模，采用贝叶斯优化进行超参数调优

Result: 选择F1分数最佳的模型进行评估和讨论

Conclusion: 该方法能够有效预测COVID-19患者的住院时间严重程度
...
### [119] [Interpretable temporal fusion network of multi- and multi-class arrhythmia classification](https://arxiv.org/abs/2511.15062)
*Yun Kwan Kim*

Main category: cs.LG

TL;DR: 提出了一种结合局部和全局信息提取与注意力机制的心律失常检测框架，能够准确识别心律失常的起始点、结束点和持续时间，在MITDB和AFDB数据库上取得了优于基准模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的临床决策支持系统在心律失常分类任务中面临挑战，因为心律失常的长度变化很大，且之前的方法没有充分考虑心律失常起始时间变化的条件。

Method: 框架包含两部分：(i) 局部和全局特征提取；(ii) 基于注意力的局部-全局信息融合，能够在受限输入长度内实现心律失常检测和分类。

Result: 在MITDB上获得96.45%的持续时间F1分数、82.05%的发作F1分数和96.31%的Dice分数；在AFDB上获得97.57%、98.31%和97.45%的相应分数，性能优于基准模型。

Conclusion: 该方法能有效捕获局部和全局信息及动态变化，无显著信息损失，可更准确地检测心律失常并精确定位发生时间，有助于临床制定更精确的治疗方案。
...
### [120] [Deep Pathomic Learning Defines Prognostic Subtypes and Molecular Drivers in Colorectal Cancer](https://arxiv.org/abs/2511.15067)
*Zisong Wang,Xuanyu Wang,Hang Chen,Haizhou Wang,Yuxin Chen,Yihang Xu,Yunhe Yuan,Lihuan Luo,Xitong Ling,Xiaoping Liu*

Main category: cs.LG

TL;DR: 开发并验证了基于病理全切片图像的TDAM-CRC多实例学习模型，用于结直肠癌的精准预后预测，并通过多组学分析揭示了其分子机制。
...
Method: 使用TCGA队列（n=581）训练TDAM-CRC模型，在独立外部队列（n=1031）验证，整合多组学数据提高模型可解释性并识别新的预后生物标志物。

Result: TDAM-CRC在两个队列中均实现稳健的风险分层，预测性能显著优于传统临床分期系统和多个先进模型。多组学分析显示高风险亚型与代谢重编程和免疫抑制肿瘤微环境相关，发现MRPL37作为关键枢纽基因。
...
### [121] [Fourier-KAN-Mamba: A Novel State-Space Equation Approach for Time-Series Anomaly Detection](https://arxiv.org/abs/2511.15083)
...
Conclusion: Fourier-KAN-Mamba混合架构为时间序列异常检测提供了一种有效的解决方案，能够更好地处理复杂的时间动态和非线性模式。
...
### [122] [Semiconductor Industry Trend Prediction with Event Intervention Based on LSTM Model in Sentiment-Enhanced Time Series Data](https://arxiv.org/abs/2511.15112)
*Wei-hsiang Yen,Lyn Chao-ling Chen*

Main category: cs.LG

TL;DR: 该研究将深度学习方法与情感分析整合到传统的商业模式分析和预测中，以台积电为研究对象预测台湾半导体行业趋势。通过结合情感增强的时间序列数据和LSTM模型，准确预测了台积电的晶圆技术发展和全球市场潜在威胁。


<details>
  <summary>Details</summary>
Motivation: 半导体行业市场变化快速且晶圆技术发展迅猛，传统数据分析方法在处理高变化性和时间序列数据时表现不佳。需要结合文本数据和时间序列数据来更准确预测行业趋势。

Method: 收集台积电季度报告中的文本数据和财务时间序列数据。通过情感分析考虑公司内部事件和外部全球事件的干预，使用情感增强的时间序列数据，采用LSTM模型进行行业趋势预测。
...
### [123] [Efficient RF Passive Components Modeling with Bayesian Online Learning and Uncertainty Aware Sampling](https://arxiv.org/abs/2511.15125)
*Huifan Zhang,Pingqiang Zhou*

Main category: cs.LG

TL;DR: 提出了一种基于贝叶斯在线学习的不确定性感知框架，用于高效建模RF无源组件，相比传统机器学习流程仅需2.86%的电磁仿真时间，实现35倍加速。
...
Conclusion: 该框架显著提高了RF无源组件参数化建模的效率，实现了35倍的加速比。
...
TL;DR: 提出了一种新的稀疏矩阵乘法算法，使整个Medline数据集的自组织映射成为可能，解决了现有算法内存和处理需求指数增长的问题。


<details>
  <summary>Details</summary>
Motivation: 现有算法在处理整个Medline数据集时面临内存和处理需求指数增长的挑战，限制了只能处理数据子集。

Method: 设计了一种新颖的稀疏矩阵乘法算法，应用于整个Medline数据集的自组织映射。

Result: 成功实现了对整个Medline数据集的映射，提供了更完整的医学知识图谱。

Conclusion: 该算法不仅使完整映射成为可能，还提高了随时间更新自组织映射的可行性。
...
TL;DR: GRPO-Verif算法通过统一损失函数联合优化解决方案生成和自我验证，可调节超参数控制验证信号权重，在保持推理性能的同时增强自我验证能力。


<details>
  <summary>Details</summary>
Motivation: 尽管通过强化学习显著提升了大型语言模型的推理能力，但它们仍然难以一致地验证自己的推理轨迹，因此需要研究如何增强LLMs的自我验证能力以及这种能力是否能进一步改善推理性能。

Method: 提出GRPO-Verif算法，在统一损失函数中联合优化解决方案生成和自我验证，使用可调节超参数控制验证信号的权重。

Result: 实验结果表明，该方法在保持推理性能相当的同时，显著增强了自我验证能力。

Conclusion: GRPO-Verif算法成功实现了推理性能与自我验证能力的平衡优化，为解决LLMs自我验证不一致的问题提供了有效方案。
...
### [126] [Cross-Modal Consistency-Guided Active Learning for Affective BCI Systems](https://arxiv.org/abs/2511.15138)
...
<details>
  <summary>Details</summary>
Motivation: EEG信号易受伪影和个体差异影响，情感标签通常来自主观且不一致的报告，使得稳健的情感解码特别困难。需要一种能处理标签噪声的数据高效方法。

Method: 使用不确定性感知主动学习框架，结合模型不确定性和跨模态一致性评估。通过表示对齐模块将EEG和面部特征嵌入共享潜在空间，强制模态间语义连贯性。将残差异常视为噪声引起的不一致性，在主动学习中选择性查询这些样本获取反馈。
...
### [127] [Complex variational autoencoders admit Kähler structure](https://arxiv.org/abs/2511.15172)
*Andrew Gracyk*

Main category: cs.LG

TL;DR: 该论文研究了复杂变分自编码器中的Kähler几何结构，提出了基于Fisher信息度量的Kähler势函数方法，通过复数高斯混合模型实现高效的几何正则化。


<details>
  <summary>Details</summary>
Motivation: 研究复杂VAE中的几何结构，特别是Kähler几何，以改进潜在空间的正则化和采样质量。

Method: 在复数潜在空间下推导Fisher信息度量，提出复数高斯混合的Kähler势函数，通过相对熵实现Kähler几何结构。

Result: 提出的方法能够产生更平滑的表示和更少的语义异常值，同时计算效率更高。

Conclusion: 复杂VAE确实展现出Kähler几何结构，通过几何正则化可以改善潜在空间的质量和采样效果。
...
### [128] [FaultDiffusion: Few-Shot Fault Time Series Generation with Diffusion Model](https://arxiv.org/abs/2511.15174)
...
Method: 使用正负差异适配器利用预训练正常数据分布建模正常与故障域差异，引入多样性损失防止模式崩溃，通过样本间差异正则化鼓励生成多样化故障样本

Result: 实验结果表明该模型在真实性和多样性方面显著优于传统方法，在关键基准测试中达到最先进性能

Conclusion: 提出的框架有效解决了少样本故障时间序列生成问题，为工业设备预测性维护提供了可靠的数据增强方案
...
### [129] [Vehicle Routing Problems via Quantum Graph Attention Network Deep Reinforcement Learning](https://arxiv.org/abs/2511.15175)
*Le Tung Giang,Vu Hoang Viet,Nguyen Xuan Tung,Trinh Van Chien,Won-Joo Hwang*

Main category: cs.LG

TL;DR: 提出了一种量子图注意力网络(Q-GAT)，在深度强化学习框架中用参数化量子电路替代传统MLP，用于解决车辆路径问题，减少了50%以上的可训练参数，并在基准测试中实现了更快的收敛和约5%的路由成本降低。


<details>
  <summary>Details</summary>
Motivation: 传统基于图神经网络的深度强化学习方法依赖参数繁多的多层感知机，存在参数过多和内存受限的问题，需要开发更紧凑高效的模型来解决车辆路径问题。

Method: 在DRL框架中构建量子图注意力网络，使用参数化量子电路替代关键读出阶段的多层感知机，结合近端策略优化和贪心/随机解码策略。

Result: 与经典GAT基线相比，Q-GAT减少了50%以上的可训练参数，实现了更快的收敛速度，并将路由成本降低了约5%。

Conclusion: 参数化量子电路增强的图神经网络可以作为大规模路由和物流优化的紧凑有效求解器，展示了量子计算在组合优化问题中的潜力。
...
### [130] [Masked Auto-Regressive Variational Acceleration: Fast Inference Makes Practical Reinforcement Learning](https://arxiv.org/abs/2511.15190)
*Yuxuan Gu,Weimin Bai,Yifei Wang,Weijian Luo,He Sun*

Main category: cs.LG

TL;DR: MARVAL是一种基于蒸馏的框架，将掩码自回归扩散模型的扩散链压缩为单步生成，实现30倍加速并保持样本质量，同时支持强化学习后训练。


<details>
  <summary>Details</summary>
Motivation: 解决掩码自回归扩散模型推理速度慢的问题，其分层推理机制（外部AR循环和内部扩散去噪链）不仅影响生成效率，还阻碍了强化学习后训练的实际应用。

Method: 提出MARVAL框架：1）基于分数的变分目标，将掩码自回归扩散模型蒸馏为单步生成；2）MARVAL-RL高效强化学习框架。
...
### [131] [Reasoning in Diffusion Large Language Models is Concentrated in Dynamic Confusion Zones](https://arxiv.org/abs/2511.15208)
...
### [132] [D2D Power Allocation via Quantum Graph Neural Network](https://arxiv.org/abs/2511.15246)
*Tung Giang Le,Xuan Tung Nguyen,Won-Joo Hwang*

Main category: cs.LG

TL;DR: 提出了一种完全量子的图神经网络(QGNN)，使用参数化量子电路实现消息传递，用于无线网络资源管理，在保持性能的同时减少参数数量并实现并行计算。


<details>
  <summary>Details</summary>
Motivation: 无线网络日益复杂需要可扩展的资源管理方法，传统图神经网络在图学习方面表现出色但在大规模场景下计算成本高昂。

Method: 通过量子图卷积层(QGCLs)将特征编码为量子态，使用NISQ兼容的酉变换处理图结构，通过测量获取嵌入表示。

Result: 在D2D功率控制SINR最大化任务中，QGNN与传统方法性能相当，但参数更少且具有固有并行性。

Conclusion: 这种基于PQC的端到端QGNN标志着向量子加速无线优化迈出了重要一步。
...
### [133] [EntroPIC: Towards Stable Long-Term Training of LLMs via Entropy Stabilization with Proportional-Integral Control](https://arxiv.org/abs/2511.15248)
...
Method: EntroPIC方法通过比例积分控制机制自适应地调整正负样本的损失系数，动态平衡它们对熵的影响，从而稳定训练过程中的熵值。

Result: 实验结果表明该方法能成功维持期望的熵水平，为大语言模型提供稳定且最优的强化学习训练。

Conclusion: EntroPIC方法在理论和实验上都证明能有效控制熵值，确保大语言模型在长期训练中保持稳定探索并避免陷入次优解。
...
### [134] [Optimized scheduling of electricity-heat cooperative system considering wind energy consumption and peak shaving and valley filling](https://arxiv.org/abs/2511.15250)
*Jin Ye,Lingmei Wang,Shujian Zhang,Haihang WU*

Main category: cs.LG

TL;DR: 提出基于改进PVTD3算法的电热联合系统智能调度方法，在可再生能源接入和多重不确定性下优化系统运行，显著降低综合成本和电网功率波动。
...
Result: 在10%、20%、30%可再生能源渗透率场景下，PVTD3算法相比传统TD3算法分别降低综合成本6.93%、12.68%、13.59%，平均降低电网购电波动幅度12.8%，并改善储能设备运行状态。
...
### [135] [PLATONT: Learning a Platonic Representation for Unified Network Tomography](https://arxiv.org/abs/2511.15251)
*Chengze Du,Heng Xu,Zhiwei Yu,Bo Liu,Jialong Li*

Main category: cs.LG

TL;DR: PLATONT是一个统一的网络断层扫描框架，通过将不同网络指标建模为共享潜在网络状态的投影，在共享潜在空间中学习紧凑的结构化表示，从而提升跨任务泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有网络断层扫描方法通常单独解决不同问题，依赖有限的任务特定信号，这限制了方法的泛化性和可解释性。
...
Conclusion: PLATONT通过统一的潜在空间表示实现了网络断层扫描任务的协同学习，显著提升了跨任务泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 探索GRPO方法是否能从语言模型推广到表示学习模型，解决表示模型的后训练优化问题。
...
TL;DR: SNAP是一种稀疏测试时适应框架，通过减少适应频率和数据使用量，在保持精度的同时显著降低计算成本，特别适合资源受限的边缘环境。
...
Method: 提出两个关键组件：类与域代表内存(CnDRM)存储少量代表性样本，以及仅推理批感知内存归一化(IoBMN)动态调整归一化统计量。

Result: SNAP在仅使用1%传入数据流的情况下保持竞争力精度，延迟降低高达93.12%，精度下降低于3.3%。

Conclusion: SNAP展示了在边缘设备上部署延迟敏感应用的强大潜力，为实际应用提供了可行的解决方案。
...
### [138] [Quant-Trim in Practice: Improved Cross-Platform Low-Bit Deployment on Edge NPUs](https://arxiv.org/abs/2511.15300)
*Rayen Dhahri,Steffen Urban*

Main category: cs.LG

TL;DR: Quant-Trim是一种训练阶段方法，通过渐进式伪量化和反向剪枝，生成硬件无关的检查点，解决边缘加速器中低比特量化在不同后端编译器上的精度不一致问题。
...
Result: 在模型和任务中，Quant-Trim缩小了浮点与低比特之间的差距，减少了对编译器启发式/校准的依赖，避免了针对每个后端的重新训练。

Conclusion: Quant-Trim提供了一种硬件中立的解决方案，能够在不同后端和精度选择下保持稳健的精度表现。
...
### [139] [On the Internal Semantics of Time-Series Foundation Models](https://arxiv.org/abs/2511.15324)
*Atharva Pandey,Abhilash Neog,Gautam Jajoo*

Main category: cs.LG

TL;DR: 系统研究时间序列基础模型的概念可解释性，发现早期层捕获局部时间模式，深层编码分散和变化时间信号，但概念组合时存在干扰问题


<details>
  <summary>Details</summary>
Motivation: 尽管时间序列基础模型在实证上取得成功，但其内部如何表示基本时间序列概念的机制仍不明确，需要系统研究其概念可解释性

Method: 使用分层分析、线性可恢复性测试和表示相似性度量，系统探讨概念编码层、线性可恢复性、表示演化以及概念组合处理

Result: 早期层主要捕获局部时间域模式，深层编码分散和变化时间信号，谱和扭曲因子最难线性恢复，概念组合时探针性能下降

Conclusion: 原子概念可靠定位，但组合仍是挑战，揭示了当前TSFMs在表示交互时间现象能力上的关键局限
...
### [140] [KrawtchoukNet: A Unified GNN Solution for Heterophily and Over-smoothing with Adaptive Bounded Polynomials](https://arxiv.org/abs/2511.15327)
*Huseyin Goksu*

Main category: cs.LG

TL;DR: KrawtchoukNet是基于离散Krawtchouk多项式的谱图神经网络滤波器，解决了传统多项式滤波器在异配图和高多项式度数下的性能崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 传统谱图神经网络（如ChebyNet）存在两个关键限制：1）在"异配性"图上性能崩溃；2）在高多项式度数时出现过平滑问题。这些问题源于标准滤波器的静态低通特性。

Method: 提出KrawtchoukNet滤波器，通过两个关键设计：1）将多项式域N固定为小常数，创建首个具有固有有界递归系数的GNN滤波器；2）使滤波器的形状参数p可学习，使滤波器能够自适应地调整其频谱响应。

Result: KrawtchoukNet在过平滑问题上表现出色（在K=10时达到SOTA结果），在具有挑战性的异配性基准测试（Texas、Cornell）上实现SOTA性能，显著优于GAT和APPNP等标准GNN。

Conclusion: KrawtchoukNet通过有界递归系数和可学习形状参数，为谱图神经网络的异配性图处理和过平滑问题提供了统一解决方案。
...
### [141] [LaguerreNet: Advancing a Unified Solution for Heterophily and Over-smoothing with Adaptive Continuous Polynomials](https://arxiv.org/abs/2511.15328)
*Huseyin Goksu*

Main category: cs.LG

TL;DR: 提出了LaguerreNet，一种基于连续拉盖尔多项式的新型GNN滤波器，通过可训练的alpha参数学习滤波器谱形状，解决了异质图性能差和高阶多项式过平滑的问题。


<details>
  <summary>Details</summary>
Motivation: 传统谱图神经网络在异质图上性能较差，且在高阶多项式时会出现过平滑问题，这些问题源于标准滤波器的静态低通特性。

Method: 使用连续拉盖尔多项式构建GNN滤波器，核心alpha参数可训练，采用LayerNorm技术解决无界多项式的数值不稳定问题。

Result: 在异质图基准测试中达到最先进性能，对过平滑具有极强鲁棒性，在K=10时性能达到峰值，比ChebyNet崩溃点高一个数量级。

Conclusion: LaguerreNet通过自适应多项式滤波器和数值稳定技术，有效解决了谱GNN在异质图和高阶多项式时的核心问题。
...
### [142] [STREAM-VAE: Dual-Path Routing for Slow and Fast Dynamics in Vehicle Telemetry Anomaly Detection](https://arxiv.org/abs/2511.15339)
...
### [143] [Multi-layer Stack Ensembles for Time Series Forecasting](https://arxiv.org/abs/2511.15350)
*Nathanael Bosch,Oleksandr Shchur,Nick Erickson,Michael Bohlke-Schneider,Caner Türkmen*

Main category: cs.LG

TL;DR: 本文系统评估了33种时间序列预测集成方法，发现堆叠法能持续提升精度，但无单一最优堆叠器。为此提出了多层堆叠框架，在多样化预测场景中均能提供优越精度。


<details>
  <summary>Details</summary>
Motivation: 集成学习在表格任务中效果显著，但在时间序列预测中应用不足，简单线性组合仍被视为最优方法。本文旨在系统探索时间序列预测的集成策略。

Method: 评估33种集成模型（包括现有和新提出的），在50个真实世界数据集上进行测试。提出了多层堆叠框架，结合不同堆叠器模型的优势。

Result: 堆叠法能持续提高预测精度，但无单一堆叠器在所有任务中表现最佳。多层堆叠框架在多样化预测场景中均能提供优越精度。
...
### [144] [Cost-Aware Prediction (CAP): An LLM-Enhanced Machine Learning Pipeline and Decision Support System for Heart Failure Mortality Prediction](https://arxiv.org/abs/2511.15357)
...
<details>
  <summary>Details</summary>
Motivation: 传统机器学习预测模型往往忽视下游价值权衡和临床可解释性，需要开发能够沟通预测应用所涉及权衡的框架。

Method: 开发了预测心衰患者1年死亡率的XGB模型，引入临床影响投影(CIP)曲线可视化成本维度，使用四个LLM代理生成患者特定描述。

Result: XGB模型表现最佳(AUROC=0.804)，CIP曲线提供群体级成本组成概览，LLM生成个体级成本效益分析，系统获得临床医生积极评价。
...
### [145] [CID: Measuring Feature Importance Through Counterfactual Distributions](https://arxiv.org/abs/2511.15371)
*Eddie Conti,Álvaro Parafita,Axel Brando*

Main category: cs.LG

TL;DR: 提出了一种新的局部特征重要性方法CID，通过生成正负反事实样本并使用核密度估计来量化特征重要性，在忠实性指标上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 机器学习中评估个体特征重要性对于理解模型决策过程至关重要，但缺乏明确的真实基准，需要替代的、有理论依据的度量方法。

Method: 生成正负两组反事实样本，使用核密度估计建模其分布，基于分布差异度量对特征进行排序，该方法具有严格的数学理论基础。

Result: 与现有局部特征重要性解释器相比，该方法不仅提供了互补视角，还在忠实性指标（包括全面性和充分性）上表现更好，产生更忠实的系统解释。

Conclusion: CID方法作为一种有价值的模型分析工具具有很大潜力，能够提供更可靠的特征重要性解释。
...
### [146] [Parameter Importance-Driven Continual Learning for Foundation Models](https://arxiv.org/abs/2511.15375)
...
Method: 引入两种重要性估计器：基于Fisher信息的PIECE-F和结合梯度与曲率信息的二阶归一化PIECE-S，指导选择性更新最相关任务的0.1%核心参数。
...
### [147] [EVA-Net: Interpretable Brain Age Prediction via Continuous Aging Prototypes from EEG](https://arxiv.org/abs/2511.15393)
*Kunyu Zhang,Mingxuan Wang,Xiangjie Shi,Haoxing Xu,Chao Zhang*

Main category: cs.LG

TL;DR: EVA-Net是一个可解释的脑年龄异常检测框架，使用稀疏注意力Transformer处理长EEG序列，通过变分信息瓶颈学习鲁棒表示，并利用原型网络学习健康老化流形，在健康人群上训练后能有效检测MCI和AD患者的异常。
...
Method: 提出EVA-Net框架：1）使用稀疏注意力Transformer建模长EEG序列；2）采用变分信息瓶颈处理噪声和变异性；3）通过连续原型网络学习健康老化流形实现可解释性。

Result: 在1297名健康受试者上训练，达到最先进精度；在27名MCI和AD患者验证中，该组显示出显著更高的脑年龄差距和原型对齐误差，证实其偏离健康流形。

Conclusion: EVA-Net为使用不完美医疗数据的医疗智能提供了一个可解释的框架，能有效检测脑部疾病异常。
...
### [148] [Proximal Approximate Inference in State-Space Models](https://arxiv.org/abs/2511.15409)
*Hany Abdulsamad,Ángel F. García-Fernández,Simo Särkkä*

Main category: cs.LG

TL;DR: 提出一种用于非线性非高斯状态空间模型状态估计的算法，基于变分拉格朗日框架，将贝叶斯推断转化为带动态约束的熵信任域更新序列。
...
Method: 采用变分拉格朗日公式，将贝叶斯推断转化为序列熵信任域更新；对于高斯-马尔可夫近似，推导递归方案；对一般非线性非高斯模型，使用广义统计线性回归和傅里叶-埃尔米特矩匹配来闭合递归。

Result: 开发出一类前向-后向算法，具有有利的计算复杂度，能够有效处理非线性非高斯状态估计问题。

Conclusion: 所提出的变分拉格朗日框架为非线性非高斯状态空间模型的状态估计提供了一种有效的算法家族，具有递归结构和计算优势。
...
### [149] [Towards Understanding Layer Contributions in Tabular In-Context Learning Models](https://arxiv.org/abs/2511.15432)
*Amir Rezaei Balef,Mykhailo Koshil,Katharina Eggensperger*

Main category: cs.LG

TL;DR: 研究了表格ICL模型中各层对预测的贡献，识别冗余层，并与LLMs进行比较，发现只有部分层共享表示语言，存在结构冗余。


<details>
  <summary>Details</summary>
Motivation: 尽管表格ICL模型与LLMs在架构上相似，但各层在表格预测中的具体作用尚不清楚，需要深入分析层间动态和冗余性。

Method: 通过"层如画家"视角分析TabPFN和TabICL模型，研究各层潜在空间的演化过程。

Result: 发现只有部分层共享共同的表示语言，表明存在结构冗余，这为模型压缩和可解释性改进提供了机会。

Conclusion: 表格ICL模型存在层间冗余，可通过识别和利用这种冗余来实现模型压缩和提升可解释性。
...
### [150] [TSFM in-context learning for time-series classification of bearing-health status](https://arxiv.org/abs/2511.15447)
*Michel Tokic,Slobodan Djukanović,Anja von Beuningen,Cheng Feng*

Main category: cs.LG

TL;DR: 提出了一种使用时间序列基础模型进行上下文学习的分类方法，无需微调即可对训练数据之外的数据进行分类，应用于轴承健康状态评估。


<details>
  <summary>Details</summary>
Motivation: 传统方法需要为特定任务微调模型，而该方法旨在利用预训练模型的可扩展性，无需微调即可实现分类，推动从定制窄AI解决方案向更广泛的AI驱动维护系统发展。

Method: 将示例以目标（类别ID）和协变量（数据矩阵）形式表示在模型提示中，通过上下文学习沿预测轴对未知协变量数据模式进行分类。将频域参考信号转换为伪时间序列模式，生成对齐的协变量和目标信号，使用TSFM预测分类数据与预定义标签对应的概率。

Result: 该方法在不同操作条件下都表现出有效性，展示了预训练模型的可扩展性优势。

Conclusion: 该方法在无需微调的情况下实现了有效分类，标志着从定制窄AI解决方案向更广泛AI驱动维护系统的重大进展。
...
### [151] [FairEnergy: Contribution-Based Fairness meets Energy Efficiency in Federated Learning](https://arxiv.org/abs/2511.15454)
...
<details>
  <summary>Details</summary>
Motivation: 解决无线边缘系统中联邦学习面临的挑战：在保持数据隐私的同时，平衡能源效率、公平参与和高模型精度，克服异构资源、不平等客户端贡献和有限通信容量的问题。

Method: 提出FairEnergy框架，将捕获更新幅度和压缩比的贡献分数整合到联合优化中。通过松弛二元选择变量和应用拉格朗日分解处理全局带宽耦合，然后进行每个设备的子问题优化。

Result: 在非IID数据上的实验表明，FairEnergy相比基线策略实现了更高的准确率，同时能耗减少了高达79%。

Conclusion: FairEnergy框架有效解决了联邦学习在无线边缘系统中的能源效率和公平参与问题，通过联合优化策略在保持高模型精度的同时显著降低能耗。
...
### [152] [NTK-Guided Implicit Neural Teaching](https://arxiv.org/abs/2511.15487)
...
<details>
  <summary>Details</summary>
Motivation: 隐式神经表示（INRs）在拟合高分辨率信号时需要优化数百万个坐标，导致计算成本过高。

Method: 利用神经正切核（NTK）对示例进行评分，通过NTK增强的损失梯度范数来选择坐标，同时考虑拟合误差和异构杠杆效应。

Result: NINT显著减少了训练时间（近一半），同时保持或提高了表示质量，在基于采样的加速策略中达到最先进水平。
...
### [153] [Sample-Adaptivity Tradeoff in On-Demand Sampling](https://arxiv.org/abs/2511.15507)
*Nika Haghtalab,Omar Montasser,Mingda Qiao*

Main category: cs.LG

TL;DR: 本文研究了按需采样中样本复杂度和轮数复杂度之间的权衡关系，在可实现的MDL中证明了最优样本复杂度约为dk^{Θ(1/r)}/ε，在不可知情况下提出了在O(√k)轮内实现接近最优样本复杂度的算法，并引入了OODS框架来抽象样本自适应权衡。
...
Method: 提出了优化通过按需采样(OODS)的新框架来抽象样本自适应权衡，捕获大多数现有的MDL算法；在可实现的MDL中分析了最优样本复杂度，在不可知情况下设计了在O(√k)轮内实现接近最优样本复杂度的算法。

Result: 在可实现的MDL中，r轮算法的最优样本复杂度约为dk^{Θ(1/r)}/ε；在不可知情况下，提出了在O(√k)轮内实现样本复杂度为Õ((d+k)/ε²)的算法；建立了OODS设置中轮数复杂度的接近紧界。

Conclusion: 上界直接产生了不可知MDL的Õ(√k)轮算法，而下界表明实现亚多项式轮数复杂度需要绕过OODS固有硬度的全新技术。
...
### [154] [PCARNN-DCBF: Minimal-Intervention Geofence Enforcement for Ground Vehicles](https://arxiv.org/abs/2511.15522)
*Yinan Yu,Samuel Scheidegger*

Main category: cs.LG

TL;DR: 提出PCARNN-DCBF方法，通过物理编码的控制仿射残差神经网络与预览离散控制屏障函数相结合，实现地面车辆的实时地理围栏，在保持可验证控制结构的同时提高学习精度。


<details>
  <summary>Details</summary>
Motivation: 现有解决方案难以在高保真学习与可验证控制的结构要求之间取得平衡，而运行时地理围栏对于实施操作设计域至关重要。

Method: PCARNN-DCBF管道集成物理编码的控制仿射残差神经网络和基于预览的离散控制屏障函数，保留车辆动力学的控制仿射结构，通过实时二次规划强制执行多边形保持约束。

Result: 在CARLA模拟器中对电动和燃油平台进行的实验表明，这种结构保持方法显著优于解析和非结构化神经基线方法。

Conclusion: PCARNN-DCBF成功解决了高保真学习与可验证控制之间的权衡问题，为地面车辆提供了可靠的运行时地理围栏解决方案。
...
TL;DR: 提出ChaosODE（CODE）方法，使用任意多项式混沌展开（aPCE）来表示ODE的右侧，从而获得动力学的全局正交多项式表示，在稀疏数据和噪声条件下优于神经网络和核方法。
...
Result: 在Lotka-Volterra系统上的实验表明，CODE在噪声水平、初始条件和长期预测方面表现优异，即使在新初始条件下也展现出卓越的外推能力，优于NeuralODE和KernelODE。

Conclusion: CODE方法在稀疏数据和噪声条件下具有更好的外推能力，为动力学学习问题提供了实用的鲁棒优化指南。
...
TL;DR: 本文通过自动驾驶环境中的实验，揭示了持续强化学习面临的挑战，包括环境抽象、超参数敏感性、灾难性遗忘和神经网络容量利用等问题，并提出了开放研究问题。


<details>
  <summary>Details</summary>
Motivation: 持续学习在强化学习中的应用仍是一个开放问题，特别是在自动驾驶等网络物理系统中，需要解决非平稳环境下的适应性问题。

Method: 在自动驾驶环境中，使用PPO算法让智能体依次学习四个不同角度的停车场景，模拟持续学习环境。

Result: 实验揭示了持续强化学习的多个挑战：环境抽象困难、超参数敏感、灾难性遗忘和神经网络容量利用效率低。

Conclusion: 需要解决持续强化学习中的基础挑战，质疑神经网络在持续学习中的适用性，并呼吁计算机科学与神经科学的跨学科研究。
...
### [157] [DeepThinkVLA: Enhancing Reasoning Capability of Vision-Language-Action Models](https://arxiv.org/abs/2511.15669)
*Cheng Yin,Yankai Lin,Wang Xu,Sikyuen Tam,Xiangrui Zeng,Zhiyuan Liu,Zhouping Yin*

Main category: cs.LG

TL;DR: DeepThinkVLA通过混合注意力解码器和两阶段训练策略，解决了VLA模型中思维与动作的冲突，在LIBERO基准上达到97.0%的成功率。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型使用单一自回归解码器同时处理顺序推理和并行动作，导致运动控制性能下降，思维与动作之间缺乏强因果联系。

Method: 采用混合注意力解码器：因果注意力用于顺序推理，双向注意力用于并行动作解码；两阶段训练：先用SFT教授基础推理，再用RL对齐推理-动作序列与任务目标。
...
### [158] [Walrus: A Cross-Domain Foundation Model for Continuum Dynamics](https://arxiv.org/abs/2511.15684)
...
TL;DR: Walrus是一个基于Transformer的物理模拟基础模型，专门用于流体类连续介质动力学，通过在19个多样化场景上进行预训练，在短期和长期预测方面优于现有基础模型。


<details>
  <summary>Details</summary>
Motivation: 解决物理模拟中数据异构性、不稳定长期动态、不同分辨率和维度等挑战，以在物理模拟领域实现类似语言和视觉领域的基础模型突破。

Method: 采用谐波分析稳定化方法、负载均衡的分布式2D/3D训练策略、计算自适应标记化等技术，开发基于Transformer的Walrus模型。

Result: Walrus在短期和长期预测任务上均优于现有基础模型，消融研究证实了其在预测稳定性、训练吞吐量和迁移性能方面的优势。

Conclusion: Walrus成功展示了在物理模拟领域开发高效基础模型的可行性，为社区提供了代码和权重，推动了物理模拟基础模型的发展。
...
### [159] [The Impact of Quantization on Large Reasoning Model Reinforcement Learning](https://arxiv.org/abs/2511.15694)
...
<details>
  <summary>Details</summary>
Motivation: 研究量化对大型推理模型（LRMs）中强化学习的影响，目前尚不清楚量化如何影响无监督微调的大规模强化学习模型。

Method: 通过系统实验比较后训练量化（PTQ）、量化感知训练（QAT）和量化感知强化学习训练在数学基准测试上的表现。

Result: 发现量化感知的强化学习训练显著降低了推理性能，而PTQ和QLoRA方法表现更好。

Conclusion: 在大型推理模型的强化学习中，量化感知训练会损害学习过程，建议使用PTQ和QLoRA方法进行量化。
...
### [160] [ChartEditor: A Reinforcement Learning Framework for Robust Chart Editing](https://arxiv.org/abs/2511.15266)
*Liangyu Chen,Yichen Xu,Jianzhe Ma,Yuqi Liu,Donglu Yang,Liang Zhang,Wenxuan Wang,Qin Jin*

Main category: cs.MM

TL;DR: 提出了ChartEditVista基准和ChartEditor模型，用于仅基于图表图像和自然语言指令的图表编辑任务，无需原始代码。


<details>
  <summary>Details</summary>
Motivation: 现有图表编辑基准数据多样性有限且假设能获取完整图表代码，这在现实场景中很少见。

Method: ChartEditVista包含7,964个样本，涵盖31个图表类别，通过全自动流水线生成；ChartEditor使用强化学习框架，结合新颖的渲染奖励机制。

Result: ChartEditVista提供了稳健的评估，ChartEditor在图表编辑任务中持续优于相似规模和更大规模的模型。

Conclusion: ChartEditVista填补了图表编辑基准的空白，ChartEditor通过强化学习和渲染奖励实现了优异的图表编辑性能。
...
### [161] [The Illusion of Procedural Reasoning: Measuring Long-Horizon FSM Execution in LLMs](https://arxiv.org/abs/2511.14777)
*Mahdi Samiei,Mahdi Mansouri,Mahdieh Soleymani Baghshah*

Main category: cs.AI

TL;DR: 提出了有限状态机(FSM)执行作为评估LLM程序推理能力的最小化、完全可解释框架，揭示了LLM在长序列推理中的系统性退化问题。


<details>
  <summary>Details</summary>
Motivation: LLM在推理任务上表现出色，但其执行多步骤、基于规则的程序推理能力尚不明确，缺乏受控、可解释的基准来隔离和测量这种能力退化。

Method: 使用有限状态机(FSM)执行框架，给定明确的FSM定义，要求模型根据输入动作逐步执行，在多个轮次中保持状态一致性。该方法不需要世界知识，只需忠实应用确定性转换规则。

Result: 实验结果显示随着任务范围或分支复杂性增加，模型性能系统性下降。在规则检索涉及高分支因子时表现显著更差。大模型在局部准确性上有所改进，但在多步骤推理中仍然脆弱，除非明确提示外部化中间步骤。

Conclusion: FSM评估提供了一个透明、复杂度可控的探针，用于诊断LLM的失败模式，并指导设计能够实现真正长序列程序能力的归纳偏置，为理解和改进LLM的算法可靠性建立了严格的实验基础。
...
### [162] [Learning Interestingness in Automated Mathematical Theory Formation](https://arxiv.org/abs/2511.14778)
*George Tsoukalas,Rahul Saha,Amitayush Thakur,Sabrina Reguyal,Swarat Chaudhuri*

Main category: cs.AI

TL;DR: FERMAT是一个强化学习环境，用于自动化数学理论发现，通过符号动作建模概念发现和定理证明，并探索使用进化算法自动评估数学对象的有趣性。


<details>
  <summary>Details</summary>
Motivation: 解决人工智能中开放式的数学理论自动发现这一重大挑战，通过创建能够建模概念发现和定理证明的环境。

Method: 引入FERMAT强化学习环境，使用符号动作；开发基于LLM的进化算法，具有函数抽象功能，用于合成非平凡的有趣性度量。
...
### [163] [Ask WhAI:Probing Belief Formation in Role-Primed LLM Agents](https://arxiv.org/abs/2511.14780)
*Keith Moore,Jun W. Kim,David Lyu,Jeffrey Heo,Ehsan Adeli*

Main category: cs.AI

TL;DR: Ask WhAI是一个用于检查和干扰多智能体交互中信念状态的系统级框架，通过记录重放交互、查询智能体信念和理由、注入反事实证据来测试信念结构对新信息的响应。


<details>
  <summary>Details</summary>
Motivation: 研究多智能体科学推理中的信念形成和认知孤岛现象，提供可重现的方法来观察和测试这些动态过程，这在人类专家中是不可能实现的。

Method: 使用多专业诊断模拟器，让配备特定角色先验的大语言模型智能体（如神经科医生、传染病专家）在共享医疗记录上交互，通过断点查询前后信念变化，区分先验固化与推理效应。
...
### [164] [Subnational Geocoding of Global Disasters Using Large Language Models](https://arxiv.org/abs/2511.14788)
...
<details>
  <summary>Details</summary>
Motivation: 灾害数据库如EM-DAT中的位置信息通常是非结构化的文本形式，存在粒度不一致和拼写错误，难以与空间数据集集成，需要自动化解决方案。

Method: 使用GPT-4o处理文本位置信息，交叉验证GADM、OpenStreetMap和Wikidata三个地理信息库，根据源数据的一致性和可用性为每个位置分配可靠性评分。

Result: 应用于2000-2024年的EM-DAT数据集，成功地理编码14,215个事件，涵盖17,948个独特位置，无需人工干预。
...
### [165] [Project Rachel: Can an AI Become a Scholarly Author?](https://arxiv.org/abs/2511.14819)
*Martin Monperrus,Benoit Baudry,Clément Vidal*

Main category: cs.AI

TL;DR: 本研究通过创建AI学术身份Rachel So并发表10+篇AI生成论文，实证研究学术生态系统对AI作者身份的反应。


<details>
  <summary>Details</summary>
Motivation: 调查学术生态系统如何应对AI作者身份，为关于AI系统参与学术交流的未来辩论提供实证数据。
...
Conclusion: AI作者身份对出版商、研究人员和整个科学系统具有重要影响，需要就AI系统参与学术交流的未来进行必要讨论。
...
### [166] [Uncertainty-Aware Measurement of Scenario Suite Representativeness for Autonomous Systems](https://arxiv.org/abs/2511.14853)
*Robab Aghazadeh Chakherlou,Siddartha Khastgir,Xingyu Zhao,Jerein Jeyachandran,Shufeng Chen*

Main category: cs.AI

TL;DR: 提出一种概率方法来量化AI系统训练和测试数据的代表性，通过比较场景套件与目标操作域(TOD)的特征分布，使用不精确贝叶斯方法处理有限数据和先验不确定性，生成区间值代表性估计。


<details>
  <summary>Details</summary>
Motivation: 确保AI系统（如自动驾驶车辆）的可信度和安全性需要数据具有代表性，即训练测试数据应反映系统设计运行的操作条件。现有方法难以处理TOD真实分布未知且只能从有限数据推断的问题。

Method: 使用概率方法比较场景套件与TOD的特征统计分布，应用不精确贝叶斯方法处理有限数据和先验不确定性，生成区间值代表性估计而非单一值。

Result: 通过数值示例展示了在依赖关系和先验不确定性下，跨操作类别（天气、道路类型、时间等）的场景套件与推断TOD分布的比较，估计了局部和全局代表性区间。

Conclusion: 该方法能够量化数据代表性并提供不确定性感知的区间估计，有助于评估AI系统训练测试数据的充分性，特别是在TOD真实分布未知的情况下。
...
### [167] [Task Specific Sharpness Aware O-RAN Resource Management using Multi Agent Reinforcement Learning](https://arxiv.org/abs/2511.15002)
*Fatemeh Lotfi,Hossein Rajoli,Fatemeh Afghah*

Main category: cs.AI

TL;DR: 提出了一种结合Sharpness-Aware Minimization (SAM)的改进Soft Actor Critic算法，在分布式多智能体强化学习框架中实现O-RAN资源管理，通过基于TD误差方差的适应性正则化机制提升鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 下一代网络采用O-RAN架构实现动态资源管理，但深度强化学习模型在动态环境中存在鲁棒性和泛化性不足的问题，需要改进算法来应对环境复杂性。

Method: 在分布式多智能体强化学习框架中，将SAM机制与SAC算法结合，引入基于TD误差方差的适应性选择性正则化，仅对面临高环境复杂度的智能体进行正则化，并采用动态ρ调度方案优化探索-利用权衡。

Result: 实验结果表明该方法显著优于传统DRL方法，资源分配效率提升高达22%，并在多样化O-RAN切片中确保优越的QoS满意度。

Conclusion: 所提出的适应性SAM机制有效提升了DRL在O-RAN资源管理中的鲁棒性和泛化能力，同时保持了学习效率，为动态网络环境中的资源优化提供了有效解决方案。
...
### [168] [Learning Human-Like RL Agents Through Trajectory Optimization With Action Quantization](https://arxiv.org/abs/2511.15055)
*Jian-Ting Guo,Yu-Cheng Chen,Ping-Chun Hsieh,Kuo-Hao Ho,Po-Wei Huang,Ti-Rong Wu,I-Chen Wu*

Main category: cs.AI

TL;DR: 提出了MAQ框架，通过向量量化VAE从人类演示中提取宏观动作，使强化学习代理的行为更接近人类，在D4RL Adroit基准测试中显著提高了人类相似性。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习代理虽然在许多领域表现超人类，但行为往往不自然，与人类行为差异较大，这影响了可解释性和可信度。目标是设计能够产生人类行为的强化学习代理。

Method: 将人类相似性建模为轨迹优化问题，采用后退时域控制方法，提出宏观动作量化(MAQ)框架，使用向量量化VAE从人类演示中提取宏观动作。

Result: 在D4RL Adroit基准测试中，MAQ显著提高了人类相似性，增加了轨迹相似性分数，在人类评估研究中获得了最高的人类相似性排名。
...
### [169] [Beyond GeneGPT: A Multi-Agent Architecture with Open-Source LLMs for Enhanced Genomic Question Answering](https://arxiv.org/abs/2511.15061)
*Haodong Chen,Guido Zuccon,Teerapong Leelanupab*

Main category: cs.AI

TL;DR: OpenBioLLM是一个开源的模块化多智能体框架，通过重新实现GeneGPT并引入专业化智能体来改进基因组问答系统，在保持性能的同时显著降低了延迟和成本。


<details>
  <summary>Details</summary>
Motivation: GeneGPT虽然解决了基因组问答的复杂推理需求，但其依赖专有模型存在可扩展性、运营成本、数据隐私和泛化能力等问题。

Method: 首先用开源模型复现GeneGPT，然后开发OpenBioLLM框架，引入专门用于工具路由、查询生成和响应验证的智能体，实现协调推理和基于角色的任务执行。

Result: OpenBioLLM在90%以上的基准任务中匹配或优于GeneGPT，在Gene-Turing和GeneHop上分别获得0.849和0.830的平均分数，延迟降低40-50%，且无需额外微调或工具特定预训练。

Conclusion: 开源多智能体系统在基因组问答中具有巨大潜力，OpenBioLLM的模块化设计在保持模型能力的同时显著提高了效率。
...
### [170] [ProRAC: A Neuro-symbolic Method for Reasoning about Actions with LLM-based Progression](https://arxiv.org/abs/2511.15069)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决RAC（动作和变化推理）问题，提出结合神经和符号方法的框架，利用LLM的强大能力来处理复杂的推理任务。
...
### [171] [Knowledge-Informed Automatic Feature Extraction via Collaborative Large Language Model Agents](https://arxiv.org/abs/2511.15074)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的自动特征提取方法存在架构单一、反馈机制简单、缺乏系统化外部知识整合等问题，限制了特征工程的效果。

Method: 采用去中心化的三智能体系统：科学家负责发现特征，提取器生成特征，测试器验证特征；结合检索增强生成（RAG）系统整合外部知识，采用"泛滥-修剪"策略平衡特征探索与利用。

Result: 在19个分类和9个回归数据集上显著优于现有最优方法，并能发现新颖、可测试的假设（如心肌数据集中识别出新的潜在生物标志物）。

Conclusion: Rogue One不仅提升了特征提取的统计性能，还增强了特征的语义意义和可解释性，可作为科学发现的工具。
...
### [172] [SafeRBench: A Comprehensive Benchmark for Safety Assessment in Large Reasoning Models](https://arxiv.org/abs/2511.15169)
...
TL;DR: SafeRBench是首个端到端评估大型推理模型安全性的基准，从输入、中间推理到最终输出全面评估动态安全风险，包含风险分类分级、细粒度输出分析和人类安全对齐验证。


<details>
  <summary>Details</summary>
Motivation: 现有安全评估主要关注输出层面的判断，很少捕捉推理过程中的动态风险，如有害内容的渐进式注入或被误导推理所合理化。

Method: 1) 输入特征化：将风险类别和等级纳入输入设计；2) 细粒度输出分析：通过微思维分块机制将长推理轨迹分段评估；3) 人类安全对齐：用专门设计的人类标注验证基于LLM的评估。
...
### [173] [HISE-KT: Synergizing Heterogeneous Information Networks and LLMs for Explainable Knowledge Tracing with Meta-Path Optimization](https://arxiv.org/abs/2511.15191)
*Zhiyi Duan,Zixing Shi,Hongyu Yuan,Qi Wang*

Main category: cs.AI

TL;DR: HISE-KT是一个将异构信息网络与大型语言模型相结合的知识追踪框架，通过智能评估元路径质量、基于教育心理学检索相似学生轨迹，实现更准确且可解释的预测。
...
Method: 构建多关系异构信息网络捕获结构关系，使用LLM智能评分和过滤元路径实例，基于教育心理学设计相似学生检索机制，通过结构化提示整合目标学生历史与相似轨迹。

Result: 在四个公共数据集上的实验表明，HISE-KT在预测性能和可解释性方面均优于现有知识追踪基线方法。

Conclusion: HISE-KT通过HIN与LLM的协同增强，实现了更准确的知识状态追踪和基于证据的可解释分析，为知识追踪领域提供了创新解决方案。
...
### [174] [As If We've Met Before: LLMs Exhibit Certainty in Recognizing Seen Files](https://arxiv.org/abs/2511.15192)
...
Method: 采用两阶段策略：(1) 将文件分割成小片段以减少对大规模训练数据的依赖；(2) 基于不确定性的无监督聚类，消除对经验调优阈值的需求。

Result: 在LLaMA 7b和LLaMA2 7b上分别达到90.1%和91.6%的平均平衡准确率，相比现有方法有90%以上的相对提升，最高达到93.8%的平衡准确率。

Conclusion: 这是首个将不确定性应用于LLM版权检测的工作，为训练数据透明度提供了实用工具，具有强大的跨架构泛化能力。
...
### [175] [SOLID: a Framework of Synergizing Optimization and LLMs for Intelligent Decision-Making](https://arxiv.org/abs/2511.15202)
*Yinsheng Wang,Tario G You,Léonard Boussioux,Shan Liu*

Main category: cs.AI

TL;DR: SOLID框架通过双价格和偏差惩罚实现优化算法与LLM的迭代协作，在保持模块化和数据隐私的同时提升决策质量，在投资组合案例中验证了其有效性。
...
Method: 提出SOLID框架，通过双价格和偏差惩罚机制实现优化算法与LLM代理的迭代协作，在凸性假设下保持理论收敛性。

Result: 在股票投资组合案例中，相比仅使用优化器的方法，SOLID在各种场景下都实现了收敛，并显示出更高的年化回报率。

Conclusion: SOLID为跨领域的自动化和智能决策提供了一个有前景的框架，验证了优化算法与LLM协同工作的有效性。
...
### [176] [Efficiency Will Not Lead to Sustainable Reasoning AI](https://arxiv.org/abs/2511.15259)
*Philipp Wiesner,Daniel W. O'Neill,Francesca Larosa,Odej Kao*

Main category: cs.AI

TL;DR: 论文指出，随着AI研究转向复杂问题解决，推理AI的性能不再受限于训练数据量，而是随计算投入呈指数级增长。由于效率提升接近物理极限且缺乏饱和点，仅靠效率无法实现可持续的推理AI，需要将明确限制嵌入系统优化和治理中。


<details>
  <summary>Details</summary>
Motivation: AI研究正从模式识别转向多步推理，推理AI缺乏需求饱和点，性能随计算投入指数增长。效率提升接近物理极限，仅靠效率无法实现可持续性。

Method: 通过分析推理AI的计算需求特性，论证效率提升的局限性，提出需要在系统优化和治理中嵌入明确限制的研究和政策方向。

Result: 发现推理AI性能不再受限于训练数据，而是持续随计算投入指数级扩展，效率提升无法单独解决可持续性问题。

Conclusion: 仅靠效率无法实现可持续的推理AI，需要在优化和治理中嵌入明确限制，包括研究新的架构、算法和治理框架。
...
### [177] [Realist and Pluralist Conceptions of Intelligence and Their Implications on AI Research](https://arxiv.org/abs/2511.15282)
*Ninell Oldenburg,Ruchira Dhar,Anders Søgaard*

Main category: cs.AI

TL;DR: 本文探讨了AI研究中两种对立的智能观：智能现实主义（认为智能是单一普适能力）和智能多元主义（认为智能是多样化的情境依赖能力），并分析这些隐含假设如何影响研究方法、现象解释和风险评估。


<details>
  <summary>Details</summary>
Motivation: 当前AI研究中的许多分歧源于对智能本质的不同理解，但这些假设往往隐含不显。作者希望通过明确这些基本假设，澄清AI研究中的争议。

Method: 通过分析当前AI研究中的辩论，展示两种智能观如何影响实证证据的解释，并考察它们在方法论、解释性和风险评估三个领域的具体表现。

Result: 发现两种智能观导致根本不同的研究路径：在方法论上影响模型选择、基准设计和实验验证；在解释上产生对相同现象的矛盾解读；在风险认知上形成截然不同的威胁评估和解决方案。

Conclusion: 明确这些基本智能假设有助于更清晰地理解AI研究中的分歧，促进更富有成效的学术对话。
...
### [178] [Octopus: Agentic Multimodal Reasoning with Six-Capability Orchestration](https://arxiv.org/abs/2511.15351)
*Yifu Guo,Zishan Xu,Zhiyuan Yao,Yuquan Lu,Jiaye Lin,Sen Hu,Zhenheng Tang,Yingchao Li,Huacan Wang,Ronghao Chen*

Main category: cs.AI

TL;DR: Octopus是一个多模态推理新范式，通过六种核心能力的编排实现自主探索和动态能力选择，在Octopus-Bench基准测试中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现有多模态推理模型缺乏人类般的自主探索不同推理路径的能力，无法适应现实任务中动态变化的能力需求，而人类具有互补的思维能力。

Method: 提出Octopus框架，定义六种核心多模态推理能力，构建Octopus-Bench评估基准，实现自主推理探索和动态能力选择。

Result: 实验结果显示Octopus在Octopus-Bench绝大多数任务中取得最佳性能。

Conclusion: 能力协调在代理式多模态推理中起着关键作用，Octopus证明了这种新范式的有效性。
...
### [179] [Terra Nova: A Comprehensive Challenge Environment for Intelligent Agents](https://arxiv.org/abs/2511.15378)
*Trevor McInroe*
...
<details>
  <summary>Details</summary>
Motivation: 现有的多任务基准主要评估智能体在不同独立任务间切换策略的能力，而非测试智能体在多个相互关联挑战中的深度推理能力。

Method: 开发了Terra Nova环境，这是一个单一环境，同时包含部分可观测性、信用分配、表示学习、巨大动作空间等多个经典RL挑战。

Result: 创建了一个能够测试智能体在长期视野下对多个交互变量进行综合理解的环境。

Conclusion: Terra Nova为评估强化学习智能体在复杂、相互关联挑战中的综合推理能力提供了新的测试平台。
...
### [180] [IPR-1: Interactive Physical Reasoner](https://arxiv.org/abs/2511.15407)
*Mingyu Zhang,Lifeng Zhuo,Tianxi Tan,Guocan Xie,Xian Nie,Yan Li,Renjie Zhao,Zizhu He,Ziyu Wang,Jiting Cai,Yong-Lu Li*

Main category: cs.AI

TL;DR: IPR（交互式物理推理器）通过世界模型推演和物理中心动作编码，在1000+游戏中训练，实现了人类级别的物理推理能力，在好奇心推理上超越GPT-5，并能零样本迁移到未见游戏。
...
Method: 提出IPR框架，使用世界模型推演来评分和强化VLM策略，引入PhysCode物理中心动作编码来对齐语义意图与动力学，在1000+异构游戏上预训练。

Result: IPR在生存、好奇心、实用性三个推理层次表现稳健，整体匹配GPT-5，在好奇心推理上超越GPT-5，性能随训练游戏和交互步骤增加而提升，并能零样本迁移到未见游戏。
...
### [181] [Know Your Intent: An Autonomous Multi-Perspective LLM Agent Framework for DeFi User Transaction Intent Mining](https://arxiv.org/abs/2511.15456)
...
Result: TIM在实验中显著优于机器学习模型、单一LLM和单一智能体基线
...
### [182] [Exploring the use of AI authors and reviewers at Agents4Science](https://arxiv.org/abs/2511.15534)
...
Conclusion: 会议经验为人机协作在科学领域提供了重要启示
...
### [183] [What Does It Take to Be a Good AI Research Agent? Studying the Role of Ideation Diversity](https://arxiv.org/abs/2511.15593)
...
<details>
  <summary>Details</summary>
Motivation: AI研究代理有望通过自动化机器学习模型的设计、实现和训练来加速科学进展，但该领域仍处于起步阶段，成功或失败的关键因素尚未完全理解。本文旨在探究构思多样性在代理性能中的作用。

Method: 首先分析了MLE-bench基准测试中不同模型和代理框架的轨迹，然后进行了控制实验来修改构思多样性程度，最后使用除标准奖牌评分外的其他评估指标来验证结果。

Result: 分析显示不同模型和代理框架产生不同程度的构思多样性，性能更高的代理往往具有更高的构思多样性。控制实验证明更高的构思多样性确实导致更强的性能，这一发现在其他评估指标下仍然成立。

Conclusion: 构思多样性是影响AI研究代理性能的关键因素，提高代理的构思多样性可以显著改善其表现，这一发现为优化AI研究代理提供了重要指导。
