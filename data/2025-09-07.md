...
Motivation: 利用重力场景可视化中自然引入的鲁哉计量，从几何角度提出更有效的优化算法

Method: 通过重力场景在高维空间中的嵌入构建鲁哉计量，开发新型优化器，与SGD、Adam、AdamW、Muon等方法进行对比评估

Result: 在低维示例中效果显著，在神经网络训练中比现有最优方法略有改进，计算复杂度与Adam相似

Conclusion: 新优化器具有理论优势，能在高曲率区域自动降低学习率（类似梯度剪切），支持解耦权重衰减，可以修改现有的预处理方法
...
<details>
  <summary>Details</summary>
Motivation: 现有AI模型大多针对单一任务设计，限制了在真实医疗场景中的通用性和实用性，需要开发能够统一处理多种临床任务的通用模型。

Method: 提出了CEHR-GPT模型，采用新颖的时间令牌学习框架，将患者的动态时间线显式编码到模型结构中，支持词汇扩展和微调以适应外部数据集。

Result: 模型在三个核心任务上均表现出色，能够有效泛化到外部数据集，支持快速模型开发、队列发现和患者预后预测，无需针对特定任务重新训练。

Conclusion: CEHR-GPT作为一个多功能基础模型，为电子健康记录分析提供了统一的解决方案，显著提升了临床AI应用的通用性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 探索非负矩阵分解(NMF)与概率因果性中共同原因原则(PCC)之间的内在联系，利用两者的互补性解决NMF的秩估计不稳定性和非可识别性问题

Method: 通过将灰度图像数据映射为概率模型，建立NMF与PCC的对应关系。使用PCC提供的预测性工具来稳健估计NMF的有效秩，并利用NMF实现PCC的近似计算方法

Result: 提出的秩估计方法对弱噪声具有稳定性，解决了NMF的非可识别性问题。基于此开发了聚类方法和数据去噪技术，能够将具有相同共同原因的数据点分组

Conclusion: NMF和PCC之间存在深刻的相互联系，这种双向关系为数据降维、特征提取、聚类分析和数据去噪提供了新的理论基础和实践方法
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习在物联网场景中面临数据异构性、客户端能量和可用性限制等挑战，特别是时间序列数据的客户端可用性对训练性能有重要影响，需要有效的客户端选择机制。
...
Conclusion: FedDeCAB通过半去中心化的客户端选择和邻居参数共享机制，有效解决了联邦学习中客户端可用性问题，提升了离线模型性能并降低了通信开销。
...
<details>
  <summary>Details</summary>
Motivation: 为遥远社区提供自主微网格管理方案，最大化可再生能源利用并最小化成本，推进零碳排放能源系统

Method: 结合深度强化学习和时间序列预测模型，使用Transformer架构预测可再生能源生产，采用近端策略优化(PPO)算法在模拟环境中做决策

Result: 实验结果显示在能源效率和运营强性方面较传统规则基础方法有显著提升
...
Conclusion: SharedRep-RLHF通过利用群体间的共享特征，有效解决了现有方法在少数群体上的局限性，为多群体RLHF提供了更优的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决供应链管理中的效率损失、融资约束和风险传播问题，推动高质量供应链发展。

Method: 结合交易成本和信息不对称理论，使用随机森林、LSTM网络、聚类/回归算法、游戏论、强化学习和XGBoost等机器学习算法，构建了数据驱动的三维分析框架，实施核心企业信用赋能加动态抵押融资模型。
...
<details>
  <summary>Details</summary>
Motivation: 梯度动态对深度神经网络的稳定性和泛化能力至关重要。研究发现梯度方差和标准差在训练过程中存在一致的演化模式，需要防止意外的梯度放大并稳定优化过程。

Method: 基于梯度方差和标准差演化规律的无超参数梯度归一化方法，使梯度缩放与其自然演化对齐。

Result: 在CIFAR-100基准测试中，使用ResNet-20、ResNet-56和VGG-16-BN等模型，该方法保持或提高了测试准确率，即使在强泛化条件下也表现良好。
...
<details>
  <summary>Details</summary>
Motivation: 随着MARL在玩家游戏中的应用日益扩大，从AlphaStar和OpenAI Five等重大成果中可以看出MARL的巨大潜力，需要一份系统的综述来总结当前研究进展、分析挑战并指明未来发展方向。

Method: 论文采用综述性方法，从轮流式双自然游戏到实时多自然游戏(包括体育游戏、FPS、RTS、MOBA等)进行全面考察，分析了非稳态性、部分可观测性、稀疏奖励、团队协作、可扩展性等关键挑战，并提出了一种新的游戏复杂性估计方法。

Result: 论文系统总结了MARL在多种游戏类型中的成功应用，包括Rocket League、Minecraft、Quake III Arena、StarCraft II、Dota 2、Honor of Kings等，展示了MARL在超人类表现方面的能力，为游戏AI系统的设计提供了有价值的见解。

Conclusion: 论文成功构建了MARL在游戏领域的完整研究框架，不仅总结了现有成果和挑战，还提出了新的游戏复杂性量化方法。这为MARL的未来研究和在游戏开发中的应用指明了方向，将促进这一迅速发展领域的进一步创新。
...
<details>
  <summary>Details</summary>
Motivation: 解决在离散输入空间上扩展高斯过程时遇到的计算复杂度问题，特别是对于大规模图节点的情况

Method: 使用图随机特征(GRFs)作为图节点内核的随机估计器，进行贝叶斯推断

Result: 在轻微假设下，实现了O(N^{3/2})的时间复杂度，在单片上处理超过10^6节点的图谱优化，同时保持竞争性能

Conclusion: GRFs技术为大规模图数据上的贝叶斯推断提供了可扩展的解决方案，显著提高了计算效率
...
<details>
  <summary>Details</summary>
Motivation: 随着基础模型向多模态多任务方向发展，如何利用无线设备的分布式地理数据成为关键挑战。现有的联邦基础模型未能充分考虑雾/边缘网络中模态和任务的异构性问题。

Method: 提出HF-FMs框架，将M3T基础模型的模块化结构（模态编码器、提示、MoE专家混合、适配器、任务头）与雾/边缘基础设施的分层特性对齐，支持D2D通信和局部协同训练。

Result: 开发了HF-FMs的原型系统并在无线网络环境中验证，发布了开源代码以促进该领域的研究发展。

Conclusion: HF-FMs为解决雾/边缘网络中多模态多任务联邦学习的异构性问题提供了创新解决方案，为未来研究开辟了新的方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有SNE算法如UMAP和tSNE在处理噪声高维数据时，往往无法保持数据的几何结构，会错误地分离连通分量，在良好可分簇的数据中无法正确识别簇结构

Method: 提出EmbedOR算法，采用曲率增强的距离度量方法，强调底层簇结构，通过离散图曲率来改进嵌入过程

Result: 理论证明EmbedOR距离度量扩展了tSNE的一致性结果到更广泛的数据集类别，实验显示相比其他SNE算法和UMAP，EmbedOR能更好地保持连续高密度区域的完整性

Conclusion: EmbedOR不仅能产生更好的可视化效果，其距离度量还可作为工具来标注现有可视化结果，识别碎片化问题并提供对数据底层几何结构的深入洞察
...
<details>
  <summary>Details</summary>
Motivation: 在实际应用中，对一系列受试者进行序列测试时，由于测试相关性和成本考虑，希望选择部分测试而非全部测试。但测试结果的联合分布通常未知，需要在线学习，而数据缺失会导致估计偏差，这使问题变得更加复杂。

Method: 论文提出了两种算法：1) 对于OTP问题，设计了一种Explore-Then-Commit算法，达到了集中性的Õ(T^{2/3})遗憾界；2) 对于奖励与缺失数据无关的变体问题，提出了迭代消除算法，实现了Õ(√T)的更优遗憾界。

Result: 证明了OTP问题的最小最大遗憾下界为Ω(T^{2/3})，显著高于标准弹性MDP的Ω(√T)。提出的算法能够达到这个下界，数值实验结果验证了理论分析。对于奖励与缺失数据无关的情况，证明了可以获得更好的Ω(√T)遗憾界。

Conclusion: 本文深化了对缺失数据下探索-利用交换的理解，并为设计高效的序列测试策略提供了指导。工作显示了数据缺失如何根本性地增加在线学习的难度，以及如何通过问题结构的设计来缓解这些难题。
...
<details>
  <summary>Details</summary>
Motivation: 推广和扩展去中心化概念，提出新的分布式学习架构XL，探索其设计考虑因素和自由度

Method: 通过图论和马尔可夫链的理论框架来分析XL架构，建立直观但非平凡的联系

Result: 提出了XL架构的概念框架，揭示了其与图论和马尔可夫链的理论联系

Conclusion: XL是一个有前景的分布式学习架构，需要进一步研究其设计空间和实际应用
...
<details>
  <summary>Details</summary>
Motivation: 范围分区熵在算法设计中提供了强大保证，但尚未在深度学习中得到应用。研究者希望通过可微分估计器，使得熵概念能够作为可训练的损失函数或正则化器。

Method: 设计了可微分的范围分区熵估计器，构建了EntropyNet神经模块来重新组织数据以降低熵值，并将熵正则化扩展到Transformer注意力机制中。

Result: 在计算几何任务中实现了至4.1倍的运行时间加速，错误率<0.2%；在深度学习中，在80%稀疏度下比L1基线提高了6%的准确性。

Conclusion: 熵有界计算不仅在理论上优雅，也是一种实用的自适应学习机制，能够同时提高效率、性能和结构化表征能力。
...
<details>
  <summary>Details</summary>
Motivation: 研究神经算子表示特性的统一框架，尽管柏拉图表示假说认为神经网络会收敛到相似的表示，但神经算子的表示特性在科学计算中日益重要却仍未得到充分探索。
...
<details>
  <summary>Details</summary>
Motivation: 卫星图像覆盖全球但标注训练数据稀疏、空间聚集且收集成本高，现有研究主要关注模型架构而忽略了数据条件建模，导致大规模监测应用存在不确定性。

Method: 提出了考虑异构数据收集成本和实际预算约束的空间训练数据优化问题框架，开发了新的优化方法来选择最优采样位置。

Result: 在三大洲四个任务的实验中，优化采样策略显示出显著性能提升，并明确了优化采样特别有效的应用场景。

Conclusion: 该问题框架和方法可推广到卫星图像机器学习的各个应用领域，特别是在农业调查数据增强方面具有直接应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 解决在高维平滑流形上近似函数时的计算挑战，提高计算效率和可扩展性

Method: 利用散射地图框架、热方程和Laplace-Beltrami运算符的联系，通过积分矩阵分解(SVD)实现降维，并开发在线更新机制以处理新数据

Result: 数值实验表明，该方法在稀疏CT重建等应用中在准确性和效率方面都超过传统前向神经网络和插值方法

Conclusion: 提出的方法为高维流形上的函数近似提供了一种高效、可扩展的解决方案，具有良好的应用前景
...
Result: 在100个系统采样领域中发现超过一半存在可利用结构，PAMC相比强探索、结构化和表示学习基线样本效率提升1.6-2.1倍，仅增加约20%计算开销。
...
<details>
  <summary>Details</summary>
Motivation: 时间序列预测面临分布漂移的挑战，特别是在在线部署场景中需要持续适应变化模式。传统方法主要关注参数选择和更新策略，但作者认为分布漂移源于潜在因素的变化，因此需要更新这些因素的表示。

Method: 提出了ADAPT-Z方法，使用适配器模块结合当前特征表示和历史梯度信息，在延迟反馈的多步预测场景中实现稳健的参数更新，专注于更新潜在因素的表示而非传统参数。

Result: 大量实验表明，该方法在多个数据集上持续优于无适应的基准模型，并且超越了最先进的在线学习方法。

Conclusion: 通过关注潜在因素表示更新而非传统参数选择，ADAPT-Z有效解决了时间序列在线学习中的分布漂移和延迟反馈问题，为在线时间序列预测提供了新的有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 高频(mmWave和sub-THz)信号容易被阻塞，面的散射特急影响连接可靠性，表面粗糕度决定了能量是否保持在镜面方向还是激光散射。

Method: 收集15种代表性室内材料的超78,000个点的数据集，分割成3cm×3cm的补丁，提取包含高度角、自然对数度强度、最大强度比等几何和强度特征，训练Random Forest、XGBoost和神经网络分类器。

Result: 集成树基模型在准确性和稳健性方面表现最佳，证明LiDAR特征能够抓取由粗糕度引起的散射效应。

Conclusion: 该框架能够生成具有散射意识的环境地图和数字双生体，支持适应性材料管理、阻塞恢复和环境意识连接等下一代网络功能。
...
TL;DR: 使用深度神经网络分析交通事故数据，通过自编码器降维和密集网络实现事故严重程度分类，准确率达到92%


<details>
  <summary>Details</summary>
Motivation: 利用机器学习方法研究交通事故数据，通过分析特征相关性和降维技术来提高事故严重程度的预测准确性，从而帮助减少交通事故风险

Method: 首先分析特征共线性，使用自编码器进行无监督降维，然后构建密集神经网络对交通事故严重程度进行分类

Result: 实验结果显示，提出的深度神经网络在交叉验证中达到了92%的准确率，能够有效处理不平衡数据并具有良好的泛化能力
...
Method: 将CPM重新解释为潜在享乐博弈，将全局哈密顿量分解为局部效用函数；提出两种稳定性标准：基于鲁棒性的严格标准和基于加权和的松弛标准；在社区追踪场景中使用Leiden算法进行实验验证。

Result: 证明局部优化通过更好响应动态可在伪多项式时间内收敛到均衡分区；实验表明鲁棒分区在恢复真实社区方面具有更高的准确性。

Conclusion: 博弈论视角为CPM提供了新的理论框架，证明了其效率优势，提出的鲁棒性标准在实际社区检测中表现出更好的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现代车辆传感器数据丰富，但多模态大语言模型缺乏三维空间理解能力，需要提升V2I通信链路质量预测的准确性

Method: 开发轻量级即插即用BEV注入连接器，构建环境鸟瞰图并与自车输入融合，为LLM提供空间上下文；建立CARLA和MATLAB联合仿真环境生成多模态数据

Result: 在LoS/NLoS分类、链路可用性和阻塞预测三个任务中，相比仅使用自车数据的基线，准确率宏观平均提升最高达13.9%，在雨夜恶劣条件下性能增益达32.7%

Conclusion: BEV注入框架能有效提升MLLM在V2I链路预测任务中的性能，特别是在恶劣环境下表现出强鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 现实应用中为多个交互智能体设计合适的奖励函数具有挑战性，现有MFG中的IRL方法假设智能体同质性，无法处理具有异构和未知目标的专家演示

Method: 开发了深度隐变量平均场博弈模型和相应的逆强化学习方法，能够推断不同但结构相似任务的奖励函数

Result: 在模拟场景和真实世界出租车定价问题上的实验表明，该方法在MFG中的IRL性能优于现有最先进方法

Conclusion: 该方法成功解决了异构目标下的奖励函数推断问题，为实际应用提供了更灵活的IRL框架
...
<details>
  <summary>Details</summary>
Motivation: 现有的量化感知训练和知识蒸馏研究主要关注网络输出层面的改进，但忽视了输入变换（如数据增强）对量化模型的影响。量化感知知识蒸馏与数据增强之间的关系尚未被探索，特别是在低精度模型中如何选择合适的数据增强策略。

Method: 提出了一种新颖的度量标准，根据数据增强策略最大化上下文互信息（与图像标签不直接相关的信息）的能力来评估它们，同时确保每个类别的预测接近真实标签的平均值。该方法自动对数据增强策略进行排序和选择，训练开销极小，且与任何知识蒸馏或量化感知训练算法兼容。

Result: 广泛的评估表明，使用该度量标准选择数据增强策略能够显著改进各种模型架构和数据集上的最先进量化感知训练和知识蒸馏工作。

Conclusion: 该方法为量化感知知识蒸馏中的数据增强选择提供了有效的解决方案，通过最大化上下文互信息来优化低精度模型的性能，具有很好的兼容性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法虽然能够捕获变量内和变量间的依赖关系，但往往忽略了多分组尺度上的领先-滞后依赖关系，无法在复杂系统中捕获分层的领先-滞后效应。
...
Conclusion: MillGNN能够全面考虑变量级和组级动态与衰减的领先-滞后效应，在统计可解释性和数据驱动灵活性之间取得平衡，同时保持全面性和效率的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 决策心脏病是全球主要死因之一，需要开发无创伤的诊断方法来降低医疗保健支出。

Method: 使用遗传算法选择50个关键尿液肽肠标记物，采用SMOTE技术平衡备测集，构建三隐藏层MLP神经网络模型进行分层验证训练。

Result: 模型准确度、敏感性和特异性均为95.67%，F1分数0.9565，ROC曲线面积(AUC)达到0.9748，Matthews相关系数和Cohen's kappa系数分别为0.9134和0.9131。

Conclusion: 该模型为决策心脏病提供了一种高准确、可靠的无创伤诊断工具，具有重要的临床应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于序列和图的方法无法充分捕捉蛋白质结构的层次组织特性，需要新的方法来理解蛋白质的结构-功能关系。

Method: 提出蛋白质组合复合体(PCC)在多个层次（残基、二级结构、完整蛋白质）表示蛋白质结构，并开发SE(3)-等变消息传递的TCPNet网络进行多尺度结构模式学习。

Result: 在四个蛋白质表示学习任务中，TCPNet始终优于最先进的几何图神经网络，特别是在需要理解二级结构排列的折叠分类任务中表现突出。

Conclusion: 层次拓扑特征对蛋白质分析具有重要意义，Topotein框架验证了拓扑深度学习在蛋白质表示学习中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 在在线学习中，算术操作的计算复杂度是重要考虑因素。本文动机在于探索在每轮算术操作次数有限的前提下，学习任意函数族所需的最小操作次数限制

Method: 通过理论分析和数学证明，提出在算术操作次数限制下的一般性界限，解决了(Filmus et al, 2024)和(Geneson & Tang, 2024)中提出的带有户反馈的无盱错误边界在线学习问题

Result: 证明了在算术操作次数限制下学习任意函数族所需的最小操作次数限制，并将这一结果扩展到操作限制设置中

Conclusion: 本文为在线学习中算术操作复杂度的理论研究提供了重要进展，解决了现有文献中的问题，并为复杂度受限情况下的在线学习算法设计基础
...
<details>
  <summary>Details</summary>
Motivation: 卫星组件故障修复成本高、挑战大，需要在卫星上嵌入高可靠性的AI基于故障检测系统以实现更早的故障发现

Method: 采用形式验证工具Marabou来验证神经网络模型的本地稳健性，量化模型输入在使输出行为变得不稳定前可承受的扰动程度

Result: 通过形式验证可以提高AI算法在不确定性条件下的可信过性能，确保系统运行的极高可靠性

Conclusion: 形式验证技术能够有效提升卫星故障检测系统的可靠性，为实际应用提供了重要的安全保障
...
<details>
  <summary>Details</summary>
Motivation: 现代机器学习模型虽然性能高，但有时无法捕捉到临床专家期望的模式（如癌症分期与生存率之间的单调递减关系），存在模型行为与临床经验知识不一致的问题。

Method: 通过调查收集临床知识，将其作为约束条件整合到ML模型中；在前列腺癌预后预测案例中分析不同欠规范程度下的模型表现；采用基于生成式AI的反馈驱动对齐方法进行随机实验。

Result: 研究表明，将ML模型与临床经验知识对齐是可行的，且不会影响模型性能；约束模型与非约束模型预测差异越大，临床解释差异越明显。

Conclusion: 该框架能够有效弥合机器学习模型与临床经验知识之间的差距，通过约束整合临床知识可以改善模型的可解释性和临床实用性。
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习中数据异构性导致全局模型泛化能力下降，特别是在数据集小且类别不平衡的情况下，需要解决客户端间表示学习的问题

Method: 提出FedQuad方法，显式优化类内方差最小化和类间方差最大化，通过减小相似对距离、增大负对距离来解耦客户端数据在共享特征空间中的表示
...
Result: 理论证明SP-CCI在保持边际覆盖的同时获得更紧密的预测区间，实证结果显示SP-CCI在不同数据集上相比标准CCI方法都能一致地减少区间宽度。

Conclusion: SP-CCI通过合成数据增强有效解决了反事实推理中的保守性问题，在理论和实证上都表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习在敏感决策中的应用增加，算法追索（提供可操作的步骤来逆转负面决策）的需求日益重要。但现有研究对追索过程中的公平性保障存在担忧，需要建立理论框架来确保追索的公平性。

Method: 提出了基于社会负担的公平性框架，开发了MISOB算法，该算法在现实条件下广泛适用，通过理论分析将追索和分类中的公平性保障正式联系起来。

Result: 在真实世界数据集上的实验结果表明，MISOB算法能够在不影响整体分类器准确性的情况下，降低所有群体的社会负担。

Conclusion: 该研究提供了一个全面的理论框架来解决算法追索中的不公平问题，提出的MISOB算法在实践中有效，为算法追索的公平性提供了新的解决方案。
...
Method: 使用基于大型语言模型(LLM)的智能体工作流，通过自动迭代过程和反馈机制来生成和改进合成表格数据，可以融入外部知识。
...
Method: 将输入序列建模为线性随机微分方程的离散观测，利用微分Lyapunov方程的闭式解高效传播成对不确定性，注意力权重作为线性SDE的最大似然解

Result: 提出了具有与标准注意力相同计算和内存复杂度的简化变体，在动力学和过程噪声消失极限下可恢复普通点积注意力

Conclusion: AFA提供了一种将动力学建模融入注意力的新框架，在保持计算效率的同时增强了序列建模能力，为注意力机制提供了新的理论解释
...
<details>
  <summary>Details</summary>
Motivation: 研究语音自盛盛学习模型在非语音数据（特别是生物声音）上的转移学习能力，探索其在生物声音检测和分类任务中的效果

Method: 使用HuBERT、WavLM、XEUS等语音模型生成动物声音的丰富潜在表征，通过线性探测分析时间平均表征，并扩展方法考虑时间信息和下游架构

Result: 模型能够在各种动物类群中生成丰富的声音表征，性能与精细调整的生物声音领先模型相当，并显示了噪声精灵领先训练设置的积极影响

Conclusion: 语音基础的自盛盛学习为生物声音研究提供了高效框架，具有推进该领域研究的潜力
...
<details>
  <summary>Details</summary>
Motivation: 虽然成员推理攻击在分类模型中得到广泛研究，但在时间序列预测领域的应用和影响仍未被充分探索，需要填补这一研究空白。

Method: 提出了两种新攻击方法：(1)将多变量LiRA攻击从分类模型适配到时间序列预测；(2)新颖的端到端学习方法Deep Time Series (DTS)攻击。在TUH-EEG和ELD数据集上针对LSTM和N-HiTS模型进行评估。

Result: 预测模型存在漏洞，用户级攻击通常能实现完美检测。提出的方法在多个设置中表现最强，建立了时间序列预测隐私风险评估的新基准。漏洞随着预测周期增长和训练群体变小而增加。

Conclusion: 时间序列预测模型对成员推理攻击存在显著脆弱性，需要建立相应的隐私保护机制，特别是在用户级隐私保护和长预测周期场景下。
...
<details>
  <summary>Details</summary>
Motivation: 对现有GNN过平滑分析工作进行深入探讨和扩展，特别是在Dirichlet能量框架下，为理解过平滑现象提供更全面的理论分析。

Method: 通过理论分析证明在特定光谱条件下Dirichlet能量的指数衰减特性，扩展到光谱多项式滤波器，并通过实验验证（边删除和权重放大）来观察Dirichlet能量的变化。

Result: 证明了在温和光谱条件下Dirichlet能量随深度指数衰减的规律，实验显示了Dirichlet能量增加的条件，为缓解过平滑提供了实践指导。

Conclusion: 该研究深化了对GNN过平滑现象的理论理解，通过Dirichlet能量分析揭示了过平滑的机制，并提出了实际缓解过平滑的方法途径。
...
Method: 提出Set Block Decoding (SBD)范式，在单一架构中整合标准next token prediction和masked token prediction，允许并行采样多个非连续的未来token，无需架构修改或额外训练超参数，可通过微调现有模型实现。

Result: 通过对Llama-3.1 8B和Qwen-3 8B进行微调，SBD实现了3-5倍的前向传播次数减少，同时保持与等效NTP训练相同的性能。

Conclusion: SBD提供了一种简单灵活的生成加速方法，利用离散扩散求解器实现显著加速，且不牺牲准确性，与现有KV缓存技术兼容。
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列基础模型各有优势但无法在所有场景表现最佳，需要利用不同模型的互补能力来提升零样本预测性能。

Method: 提出One-Embedding-Fits-All范式，构建统一表示空间，每个模型用单个嵌入表示，通过相似度匹配为不同任务动态选择最优模型。

Result: 在GIFT-Eval零样本预测基准测试中表现优异，保持单模型效率的同时实现性能提升，支持新模型无缝添加。

Conclusion: ZooCast框架有效整合多个TSFMs的互补优势，为时间序列零样本预测提供了高效灵活的解决方案。
...
Result: SSM的长程依赖随序列长度指数衰减，类似RNN；而注意力机制不受指数衰减约束，理论上在充足资源下能更好建模长程依赖。新提出的SSM公式在标准高斯输入下保持稳定。

Conclusion: 注意力机制在长程依赖建模方面更具灵活性，新提出的SSM公式成功结合了注意力机制的灵活性和SSM的计算效率优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有的层间高斯机制(LGM)方法依赖启发式噪声分配策略，缺乏理论依据，无法有效连接噪声分配与隐私-效用权衡的正式关系，且存在层间信噪比不一致和隐私预算利用效率低的问题。

Method: 提出了一个统一的分析框架，系统连接层间噪声注入策略与其隐含优化目标和隐私预算分配。基于此提出了SNR一致性噪声分配策略，确保层间信噪比一致性和隐私预算高效利用。

Result: 在集中式和联邦学习设置中的大量实验表明，该方法始终优于现有分配策略，实现了更好的隐私-效用权衡。

Conclusion: 该框架不仅为现有方法提供了诊断性见解，还为设计深度模型中自适应有效的噪声注入方案提供了理论指导。
...
TL;DR: 深度学习生成的心力衰竭合成数据能够保持高保真度和隐私保护，解决医学研究数据共享障碍
...
Conclusion: 深度学习基于的合成数据生成技术能够产生适合研究应用的高保真度、隐私保护的心力衰竭数据集，为解决数据共享障碍和推进心力衰竭研究提供了价值资源
...
TL;DR: 对比RL和SFT精调模型，RL在保持原有知识方面更优，其遗忘程度取决于分布偏移（KL散度），RL隐式偏向KL最小解。


<details>
  <summary>Details</summary>
Motivation: 探索不同精调方法（RL与SFT）在完成新任务时对模型原有知识和能力保持的影响，理解遗忘机制。

Method: 通过异涉积（KL-divergence）测量分布偏移，对比在策略上RL与SFT的收敛特性，并进行理论分析和实验验证（包括大语言模型和机器人基础模型）。

Result: 证实RL在保持原有知识方面更好，遗忘程度与KL散度相关，且在策略上RL更容易收敛到KL最小的解。

Conclusion: 提出"RL's Razor"原理：在所有解决新任务的方式中，RL优先选择与原始模型KL距离最近的解。
...
TL;DR: 本文提出了一种针对差分隐私(DP)中隐私-准确性权衡优化的高效方法，通过直接建模帕累托前沿和更有效的用户交互来减少计算成本和用户交互需求。


<details>
  <summary>Details</summary>
Motivation: 差分隐私在隐私保护和模型性能之间存在基本权衡，传统多目标优化方法效率低下，未能充分利用DP问题的独特结构——可以通过固定隐私级别直接最大化准确性来生成帕累托前沿上的点。

Method: 1) 理论推导权衡曲线的形状，直接高效建模帕累托前沿；2) 用更信息丰富的交互方式替代简单的成对比较，向用户展示假设的权衡曲线并让其选择偏好权衡。

Result: 在差分隐私逻辑回归和深度迁移学习的实验中，该方法以显著更少的计算成本和用户交互收敛到最优隐私-准确性权衡。

Conclusion: 通过利用DP问题的结构特性和改进的用户交互机制，本文提出的方法能够更高效地找到最优的隐私-准确性权衡平衡点。
...
TL;DR: 本文分析了机器学习在图像分析中的失败原因，提出了"无公平午餐"和"子群可分性"两个被忽视的问题，并指出了当前公平表示学习方法的不足及改进方向。


<details>
  <summary>Details</summary>
Motivation: 机器学习方法在现实世界中经常失败，特别是在高风险场景和社会敏感领域，这阻碍了其在医疗诊断等关键领域的应用部署。

Method: 通过引入因果和统计结构来分析机器学习在图像分析中的失败机制，识别出两个关键问题，并评估现有公平表示学习方法的局限性。

Result: 发现了"无公平午餐"和"子群可分性"两个被忽视的问题，当前公平表示学习方法无法有效解决这些问题。

Conclusion: 需要为图像分析领域的公平机器学习开发新的方法和路径，以解决这些根本性问题，确保模型在现实世界中的安全部署。
...
TL;DR: 提出了AT-UCB算法，利用因果抽象理论在不同抽象层次的因果多臂老虎机问题之间共享信息，通过粗粒度模拟探索后，在目标问题上使用UCB算法，显著减少累积遗憾。
...
Method: AT-UCB算法结合因果抽象理论，先在计算成本低的粗粒度CMAB实例中探索，然后在目标CMAB中对潜在最优动作子集应用传统UCB算法。

Result: 理论分析提出了新的累积遗憾上界，实证研究表明在具有不同分辨率和计算成本的流行病学模拟器中，相比经典UCB算法能显著减少累积遗憾。

Conclusion: AT-UCB算法有效利用了不同抽象层次CMAB实例之间的共享信息，实现了更好的遗憾性能，为多抽象层次决策问题提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 气候变化和极端天气威胁美军基地基础设施安全，需要建立气候韧性措施保护关键任务设施，特别是对商业能源和水源的依赖使其面临独立能源资源的脆弱性

Method: 使用多模态数据构建个体建筑行为模型，能够准确分析、预测和聚类非住宅建筑的能源使用数据，采用类似结构的开放获取数据进行方法验证

Result: 建立能源使用行为特征档案，为能源系统意外中断的影响提供基线评估，并为未来韧性措施建立基准

Conclusion: 该方法为美军基地能源韧性管理提供了数据驱动的分析框架，有助于提升关键基础设施的气候适应能力
...
TL;DR: 提出SST-iTransformer模型，通过自监督时空表示学习和双分支注意力机制，整合多源交通数据来预测停车位可用性，在成都真实数据上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 私人汽车快速增长加剧了城市停车难题，需要准确有效的停车可用性预测来支持城市规划管理，现有方法在建模时空依赖性和利用多源数据方面存在局限。

Method: 使用K-means聚类建立停车集群区域，提取整合地铁、公交、网约车、出租车等多源交通需求特征；基于iTransformer改进，集成掩码重构自监督学习，采用序列注意力和通道注意力的双分支机制分别捕获长期时间依赖和跨变量交互。

Result: 在成都真实数据上的实验表明，SST-iTransformer优于包括Informer、Autoformer等基准模型，达到最低MSE和竞争性MAE；消融研究显示网约车数据贡献最大，出租车次之，固定路线交通特征贡献较小；空间相关性分析证实排除相关停车位历史数据会导致性能显著下降。

Conclusion: SST-iTransformer通过有效整合多源交通数据和建模时空依赖性，为停车可用性预测提供了先进解决方案，强调了网约车数据和空间依赖建模的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 实际实验场景中存在异方差的偶然性不确定性，且在批处设置中这种不确定性可能相关。需要新的主动学习策略来直接减少实验轮次间的偏差

Method: 利用偏差-方差权衡关系提出新的主动学习策略，考虑带噪声和不带噪声的模型系统。通过偏差-协方差关系以二次方式利用历史数据，采用特征分解策略实现批处处理

Result: 在批处设置中使用基于偏差-协方差关系的差分方法（配合二次估计器）时，性能超过了BALD、最小信心等多个标准方法

Conclusion: 该方法通过偏差-协方差关系有效地减少实验轮次间的偏差，在带有异方差偶然性不确定性的实际实验场景中表现优异，为批处主动学习提供了新的解决方案
...
TL;DR: PagedEviction是一种针对vLLM PagedAttention的细粒度KV缓存剪枝策略，通过块级逐出算法提高内存效率，在长上下文任务中实现更好的内存使用和准确率
...
Method: 提出PagedEviction方法，采用块级逐出算法，专门为分页内存布局设计，无需修改CUDA attention内核

Result: 在Llama-3.1-8B-Instruct等模型和LongBench基准测试中显示内存使用改善且准确率优于基线

Conclusion: PagedEviction有效解决了KV缓存的内存瓶颈问题，与PagedAttention无缝集成，在长上下文任务中表现优异
...
TL;DR: TiM模型通过精确的连续时间动力学方程解决了生成模型中迭代扩散模型计算成本高与少步生成模型质量上限的矛盾，实现了任意步数的状态转换，在865M参数下超越了大参数模型SD3.5和FLUX.1的性能
...
Method: 引入精确的连续时间动力学方程，分析定义任意有限时间间隔内的状态转换，提出Transition Models (TiM)生成范式，适应任意步数转换

Result: TiM在865M参数下实现最先进性能，超越SD3.5(8B参数)和FLUX.1(12B参数)，在所有评估步数上表现优异，且随着采样预算增加呈现单调质量改进，在4096x4096分辨率下提供卓越保真度

Conclusion: TiM模型成功解决了生成步骤与质量之间的权衡问题，提供了一个灵活高效的生成框架，能够在不同计算预算下实现最优性能
...
Result: 在语言和视觉基准测试中，IPA持续优于LoRA和DoRA，在常识推理上平均提高1.5个准确率点，在VTAB-1k上提高2.3个点，当投影冻结时用约一半可训练参数匹配完整LoRA性能
...
Method: HCL通过引入编码结构异质性和混杂因素的等价表示，放宽了同质性和充分性假设。采用双向迭代策略交替优化因果聚类和结构学习，并利用自监督正则化平衡跨聚类普遍性和特异性。

Result: 理论上证明在温和条件下可识别异质因果结构。实证显示HCL在聚类和结构学习任务中表现优异，在真实单细胞扰动数据中恢复了具有生物学意义的机制。
...
Method: 推导了统一策略梯度估计器，将各种后训练方法表示为不同数据分布假设下的共同目标梯度。提出了HPT算法，动态选择不同训练信号，平衡演示利用和稳定探索

Result: 在6个数学推理基准和2个分布外测试集上，HPT在不同规模和类型的模型上都 consistently 超越了强基线方法

Conclusion: 本文的理论框架统一了RL和SFT方法，HPT算法有效结合了演示利用和探索稳定性，在数学推理任务上表现出色
...
TL;DR: 将回声状态网络重新表述为状态空间模型，提供系统理论框架，分析稳定性条件，开发线性化和特征扩展方法，并提出基于卡尔曼滤波的改进学习算法


<details>
  <summary>Details</summary>
Motivation: 传统ESN的设计和动态分析主要基于启发式方法，缺乏理论基础。本文旨在从状态空间模型的角度为ESN提供统一的理论框架和系统化分析方法

Method: 1) 将ESN重新表述为状态空间模型；2) 推导输入到状态稳定性的可验证条件；3) 开发小信号线性化和Koopman特征扩展两种映射方法；4) 提出基于卡尔曼滤波/EKF的读出学习方法和EM超参数优化

Result: 建立了ESN与经典系统辨识和现代核化状态空间模型的联系，提供了频域内存谱表征，阐明了ESN模拟结构化SSM核的条件，改进了学习算法性能

Conclusion: 状态空间模型视角为ESN提供了统一的理论基础和分析工具，使得网络设计更加系统化，性能得到显著提升，为后续研究提供了新的方向
...
<details>
  <summary>Details</summary>
Motivation: 尽管表格深度学习在实践中表现出色，但缺乏对其成功原因的理论理解。本文旨在填补这一空白，通过数据不确定性的概念来解释现代表格深度学习方法为何有效。

Method: 通过分析现有表格深度学习方法（如数值特征嵌入、检索增强模型和高级集成策略）的隐含机制，揭示它们如何管理高数据不确定性，并基于这些洞察开发改进的数值特征嵌入技术。

Result: 研究发现许多有益的设计选择之所以成功，主要是因为它们隐含地处理了数据不确定性。基于这一理解，开发出了更有效的数值特征嵌入方法。

Conclusion: 本文为理解现代表格方法的优势提供了理论基础，推动了现有技术的具体改进，并为表格深度学习的未来研究方向指明了道路。
...
TL;DR: Delta Activations方法通过测量微调模型相对于基础模型内部激活的变化，将微调模型表示为向量嵌入，用于模型聚类和分析。


<details>
  <summary>Details</summary>
Motivation: 开源大语言模型社区创建了大量针对特定任务和领域微调的模型，但由于元数据不一致和存储库非结构化，导航和理解这些模型变得困难。

Method: 提出Delta Activations方法，通过比较微调模型与基础模型在内部激活上的差异，将模型表示为向量嵌入，支持基于领域和任务的聚类分析。

Result: 该方法在不同微调设置下表现稳健，当微调数据集混合时表现出可加性，还可用于少样本微调任务嵌入、模型选择和模型合并。

Conclusion: Delta Activations有助于促进公开可用模型的重用实践，为模型生态系统提供结构化的理解和导航方式。
...
TL;DR: 这篇论文提出了一种基于公理方法的模型，从成对比较中学习符合认知过程的人类决策模型，解决传统偏好式活化方法无法抓取真实认知过程的问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI模型对齐人类偏好时，传统偏好式活化方法常常无法抓取人类真实的认知决策过程（如使用直见简化信息），导致学习到的模型与实际认知过程不一致，并限制了模型在其他决策任务上的普遍化能力。

Method: 采用公理方法，建立了一类模型，先对各个特征进行质量处理和跨选项比较，然后通过固定规则（如Bradley-Terry规则）聚合处理后的特征。这种结构化信息处理确保模型更符合人类决策过程的真实性。

Result: 在肾臀分配任务中，证明了该模型方法的有效性，学到了可解释的人类决策模型，并且准确性达到或超过了之前的成对比较决策模型。

Conclusion: 通过公理方法建立结构化信息处理模型，能够更好地学习符合人类认知过程的决策模型，提高了模型的真实性和可解释性，为AI模型对齐人类偏好提供了更有效的方法。
...
TL;DR: ChronoGraph是一个基于真实微服务系统的图结构多变量时间序列预测数据集，包含服务性能指标和依赖关系，提供异常标签用于评估预测鲁棒性和异常检测方法。


<details>
  <summary>Details</summary>
Motivation: 现有基准数据集主要来自工业控制系统或交通空气质量领域，缺乏结合多变量时间序列、明确依赖图和真实异常标签的微服务系统数据集，需要构建更真实的评估基准。

Method: 从真实生产微服务构建图结构数据集：节点为服务（包含CPU、内存、网络等多变量性能指标），有向边表示服务间依赖关系，并提供专家标注的异常事件窗口作为标签。

Result: 提供了包含预测模型、预训练时间序列基础模型和标准异常检测器的基线结果，展示了数据集在结构感知预测和事件感知评估方面的实用性。

Conclusion: ChronoGraph为研究微服务系统中的结构感知预测和事件感知评估提供了一个真实可靠的基准数据集，填补了现有数据集的空白。
