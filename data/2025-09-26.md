...
Motivation: 视觉语言模型的幻觉问题（视觉内容与生成文本之间的不匹配）严重影响了模型的可靠性。现有的检测方法通常使用VLM本身或其他模型来评估生成输出，但这种方法计算密集且增加模型延迟。

Method: 通过分析VLM的下一个标记概率（NTP）来量化模型不确定性，利用NTP特征训练轻量级机器学习模型进行幻觉检测。还引入了语言NTP和VLM幻觉预测分数来增强检测性能。

Result: 实验结果表明，基于NTP的特征是有效的幻觉预测指标，简单ML模型能达到与强VLM相当的检测性能。结合语言NTP和VLM预测分数能进一步提升检测效果。
...
<details>
  <summary>Details</summary>
Motivation: 离线手写签名验证在独立作者设置下具有挑战性，现有方法依赖真实数据集训练分类器，限制了模型的泛化能力。

Method: 利用SPD矩阵的黎曼几何，通过Riemannian Gaussian Mixture模型生成合成作者和签名样本，构建正负样本对进行度量学习。

Result: 在两个包含西方和亚洲书写风格的流行签名数据集上实验，在数据集内和跨数据集评估协议下都取得了低错误率。

Conclusion: 该方法展示了在黎曼空间生成合成数据用于独立作者签名验证系统的潜力，为解决数据依赖问题提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 传统文本到图像系统功能有限，需要开发一个统一的框架来整合多种图像生成和编辑任务，提供更交互式和多维度的创意工具。

Method: 使用高效的扩散变换器和强大的VAE减少图像token数量，通过数十亿文本-图像对进行预训练，结合多模态后训练和推理加速技术（对抗蒸馏、分布匹配、量化和推测解码）。

Result: 生成2K图像仅需1.8秒，在文本到图像和多模态图像编辑任务上达到最先进水平，展示了精确图像编辑和上下文推理的卓越多模态能力。

Conclusion: Seedream 4.0将传统文本到图像系统扩展为更交互式的创意工具，推动了生成AI在创意和专业应用领域的边界。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决CLIP模型在处理不同分辨率图像时的计算效率问题，同时保持模型的准确性，需要一种能够自适应调整计算复杂度的统一模型。

Method: 使用多级小波分解替代标准补丁嵌入，结合键值缓存和因果跨层注意力机制，在推理时从低分辨率开始逐步细化，仅在有需要时引入新信息。

Result: 在零样本分类任务中，WAVECLIP通过轻量级蒸馏从冻结的CLIP教师模型学习，实现了与竞争模型相当的准确性，同时显著节省计算资源。

Conclusion: WAVECLIP提供了一种有效的自适应分辨率推理方法，用户可以通过单一部署模型动态选择计算与准确性的权衡，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 乳腺癌是全球第二大癌症死因，早期检测对治疗效果至关重要。传统CAD系统依赖传统图像分析，而深度学习虽然更有效但受限于标注数据不足的问题

Method: 采用对比学习框架，使用Resnet-50在半监督设置下训练，利用大量未标注乳腺X光片数据，通过相似性指数和各种数据增强技术提升性能

Result: 在INbreast和MIAS基准数据集上实现了96.7%的乳腺癌检测准确率，超越了现有最先进方法

Conclusion: 对比学习框架能够有效解决深度学习在医学图像分析中标注数据不足的问题，为乳腺癌早期检测提供了有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 基础模型在文本和图像处理任务中表现出色，能够零样本跨域泛化，这使其有望替代繁琐的人工标注，通过简单文本提示实现制造过程中的自动化质量检测。

Method: 在定制真实工业图像数据和公共图像数据上测试多个最新基础模型，比较它们在零样本设置下的性能表现。

Result: 所有测试的基础模型在真实工业数据上都失败了，而相同的模型在公共基准数据集上表现良好。

Conclusion: 基础模型虽然具有跨域泛化潜力，但在实际工业质量检测场景中仍面临挑战，需要进一步研究以适应真实工业环境。
...
Result: 该方法减少了冗余，提高了跨域泛化能力，并证明了去马赛克、去噪、深度估计和语义分割等成像和视觉模块可以在NS中高效执行。

Conclusion: NS架构为高效多任务视觉管道奠定了基础，相比大型transformer骨干网络更轻量，具有更广泛的硬件兼容性。
...
<details>
  <summary>Details</summary>
Motivation: 边缘摄像头系统不断扩展，面临不断变化的环境需要定期更新模型。实践中，复杂教师模型在中央服务器上运行以标注数据，然后训练适合计算能力有限的边缘设备的小型模型。

Method: 采用高置信度流式策略与基于多样性的方法相结合，选择最有用的训练图像。

Result: 在相似训练负载（即迭代次数）下，该方法能以最少的数据查询产生高质量模型。

Conclusion: 高置信度流式策略与多样性方法的结合是优化边缘设备模型训练的有效方案，能够在保持传输成本低的同时最大化模型质量。
...
<details>
  <summary>Details</summary>
Motivation: 传统的基于掩码的虚拟试穿方法需要精确绘制掩码，操作复杂且在某些情况下无法实现特定样式（如卷起袖子），限制了用户体验和样式控制的灵活性。

Method: 利用视觉语言模型（VLMs）和图像分割模型自动生成二进制掩码，基于用户提供的图像和自由文本样式指令，无需手动绘制掩码，并支持多轮图像生成以实现复杂试穿场景。

Result: InstructVTON与现有虚拟试穿模型兼容，实现了最先进的结果，并提供了灵活的样式控制。

Conclusion: 该系统简化了用户体验，消除了精确绘制掩码的必要性，并扩展了虚拟试穿的可能性，适用于复杂样式控制场景。
...
<details>
  <summary>Details</summary>
Motivation: 指纹识别在边境管控、法医鉴定等应用中面临篡改指纹的挑战，攻击者可能故意修改脊线模式来逃避检测，因此需要开发能够识别被篡改指纹的鲁棒系统。
...
Conclusion: DeepAFRNet使用真实篡改样本并报告分级指标，解决了基于合成篡改或有限验证协议的先前工作的局限性，表明已准备好用于需要安全性和识别弹性的实际部署场景。
...
Method: 使用新构建的661张蓝莓冠层图像数据集评估36个YOLO和RT-DETR模型变体，并采用无偏均值教师半监督学习在1,035张未标注图像上进行微调。

Result: YOLOv12m达到93.3% mAP@50，RT-DETRv2-X达到93.6% mAP@50；半监督学习后RT-DETR-v2-X最佳性能达94.8% mAP@50。中等规模模型在精度和速度间取得良好平衡。

Conclusion: 研究为蓝莓检测提供了全面的基准分析，半监督学习能提升性能但需要进一步研究以更好地利用跨域未标注数据。数据集和软件程序已公开供后续研究使用。
...
<details>
  <summary>Details</summary>
Motivation: 乳腺X光筛查对早期检测和降低死亡率至关重要，但深度学习在自动化解读方面受限于有限分辨率的数据库和小样本量。

Method: 在Mini-DDSM数据集上，训练时使用预计算的无标签边界框库中的随机ROI裁剪概率性替换完整图像，可选抖动增加变异性，评估采用严格的患者级交叉验证。

Result: ROI增强（最佳参数p_roi=0.10, alpha=0.10）在Mini-DDSM上带来适度的平均ROC-AUC提升，但性能因折叠而异；PR-AUC持平或略降。

Conclusion: 简单的、以数据为中心的ROI策略可以在受限环境下增强乳腺X光分类，且不影响推理时间成本。
...
<details>
  <summary>Details</summary>
Motivation: 日常环境中镜子反射常见，能提供单次拍摄中的立体信息，利用这一特性可以简化3D重建的成像过程

Method: 将反射视为辅助视角，设计物理有效的虚拟相机变换，直接生成虚拟视角像素，并提出对称感知损失来优化姿态估计

Result: 在真实世界和合成数据上的广泛实验证明了方法的有效性，还提供了包含16个Blender场景的可定制合成数据集

Conclusion: 该方法能够从单张包含镜子反射的图像实现鲁棒的3D重建，并自然扩展到动态场景的逐帧几何恢复
...
<details>
  <summary>Details</summary>
Motivation: 灾后建筑占用评估对于分类、检查、公用事业重新供电和公平资源分配至关重要。高空图像覆盖范围广但缺乏立面细节，而街景图像细节丰富但稀疏且难以与地块对齐。

Method: 提出FacadeTrack框架，包括：1）将全景视频与地块链接；2）校正视图到立面；3）提取可解释属性（如入口堵塞、临时覆盖物、局部碎片）；4）采用两种决策策略：透明的一阶段规则和将感知与保守推理分离的两阶段设计。

Result: 在两个飓风Helene后调查中评估，两阶段方法达到精确率0.927、召回率0.781、F-1分数0.848；一阶段基线为精确率0.943、召回率0.728、F-1分数0.822。中间属性和空间诊断揭示了残余错误的发生位置和原因。

Conclusion: 该管道提供了可审计、可扩展的占用评估，适合集成到地理空间和应急管理工作流程中。
...
<details>
  <summary>Details</summary>
Motivation: 人类能够从简单的运动形状中识别各种社会互动，但以往研究多关注视觉特征，本研究旨在探索人类使用哪些语义表征来补充视觉特征。

Method: 研究1：让参与者直接标记动画印象；研究2：通过人类相似性判断测量27种社会互动的表征几何，并与基于视觉特征、标签和动画描述语义嵌入的模型预测进行比较。

Result: 语义模型为解释人类判断提供了视觉特征之外的补充信息，其中基于动词的嵌入最能解释人类的相似性判断。
...
<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位方法面临两个关键挑战：(1)无人机不同方向和视野导致的严重外观变化影响跨域泛化；(2)需要建立同时捕捉全局语义和局部细节的可靠对应关系

Method: 提出EGS框架：使用E(2)-Steerable CNN编码器提取旋转和视角变化下的稳定特征；构建带虚拟超节点的图结构，连接所有局部节点，实现全局语义聚合和局部区域重分配

Result: 在University-1652和SUES-200基准测试上的大量实验表明，EGS在跨域CVGL中持续获得显著性能提升，建立了新的最先进水平

Conclusion: EGS框架通过结合旋转等变特征提取和全局-局部一致性建模，有效提升了跨视角地理定位的跨域泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 红外小目标缺乏明显纹理特征，容易与复杂背景融合。现有方法使用固定梯度算子或简单注意力机制，在低对比度高噪声环境下难以准确提取目标边缘。

Method: 双路径网络：第一条路径使用双向交互模块（局部和全局自注意力）捕获多尺度特征依赖；第二条路径使用多边缘精炼器（级联泰勒有限差分算子）增强精细边缘细节。

Result: 该方法为精确红外小目标检测和定位提供了有前景的解决方案，在统一框架中结合了结构语义和边缘精炼。

Conclusion: 所提出的双路径边缘网络通过解耦边缘增强和语义建模，有效解决了红外小目标检测中的特征对齐挑战，实现了精确的目标检测和定位。
...
<details>
  <summary>Details</summary>
Motivation: 传统意图识别主要关注个体意图，忽略了群体环境中集体意图的复杂性。

Method: 提出SHOT数据集（包含1,979个篮球视频片段，5个相机视角，6种个体属性标注）和GIFT框架（提取细粒度个体特征并建模动态群体交互）。

Result: 实验验证了SHOT数据集和GIFT框架的有效性，为群体意图预测研究奠定了基础。

Conclusion: 该研究开创了群体意图预测的新方向，提供了首个专门的数据集和框架，推动了集体意图理解的研究进展。
...
Method: 提出Neptune-X框架，包括：1）X-to-Maritime多模态条件生成模型，通过双向目标-水域注意力模块捕捉目标与水域环境的边界交互；2）属性相关主动采样方法，基于任务相关性动态选择合成样本；3）构建海事生成数据集支持鲁棒基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 现有陨石坑导航主要针对短期着陆任务，而长期月球测绘任务面临稀疏、倾斜图像和变化光照条件的挑战，需要开发专门适用于长期测绘的导航系统。

Method: STELLA整合了Mask R-CNN陨石坑检测器、无描述符识别模块、鲁棒透视n点位姿求解器和批量轨道确定后端，构建了完整的端到端导航流水线。

Result: 在CRESENT+和CRESENT-365数据集上的实验表明，STELLA在各种视角、光照条件和月球纬度下平均保持米级位置精度和亚度级姿态精度。

Conclusion: 这是首次在真实月球测绘场景下对陨石坑导航进行全面评估，为未来任务提供了可操作的参考条件。
...
<details>
  <summary>Details</summary>
Motivation: 尽管深度视觉和语言模型在分离的模态上训练，但它们将输入投射到部分对齐的表示空间中。然而，对于对齐在网络的哪个位置出现、支持对齐的视觉或语言线索是什么、是否捕捉人类在多对多图像-文本场景中的偏好以及示例聚合如何影响对齐等问题，仍缺乏清晰的认识。

Method: 研究系统调查了上述问题，包括分析对齐在模型层级的出现位置、测试对齐对视觉外观和语义变化的鲁棒性，以及通过强制选择的"Pick-a-Pic"任务比较人类偏好与模型嵌入空间的对齐情况。

Result: 发现对齐在两种模型类型的中后期层达到峰值；对齐对外观变化鲁棒，但对语义变化（如对象移除或词序打乱）敏感；人类对图像-标题匹配的偏好与所有视觉-语言模型对的嵌入空间一致；平均嵌入跨示例会增强对齐而非模糊细节。

Conclusion: 单模态网络收敛于一个共享的语义代码，该代码与人类判断对齐，并通过示例聚合得到加强。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像编辑方法在个性化物体合成任务中存在两个主要问题：缺乏对插入物体的几何控制（仅限于2D空间和文本指令），以及插入物体与背景风格不一致导致缺乏真实感。此外，现有方法通常需要大量训练。

Method: 首先将2D物体转换为3D表示，在3D层面进行交互式编辑，然后从指定视角重新渲染为2D图像。通过扩散适配器结合几何控制（形状、视角）、风格控制和内容控制，最终生成几何可控、风格一致的编辑图像。

Result: 该方法能够实现精确的几何控制（如形状和视角调整），并保持插入物体与背景的风格一致性，无需进行大量训练。

Conclusion: FreeInsert框架通过利用3D几何信息，成功解决了现有图像编辑方法在几何控制和风格一致性方面的挑战，为个性化物体插入提供了有效的训练免费解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像扩散模型在合成真实人类照片方面面临场景退化、控制不足和感知身份不理想的问题，需要改进现有身份定制模型

Method: 提出CustomEnhancer框架，利用人脸交换技术和预训练扩散模型获取额外表示；采用三流融合生成方法结合两个兼容的反向潜在空间；引入ResInversion方法通过预扩散机制进行噪声校正

Result: 实验表明CustomEnhancer在场景多样性、身份保真度和训练自由控制方面达到SOTA效果，ResInversion比NTI方法效率显著提升
...
Method: 开发了CompressAI-Vision评估平台，将标准编解码器与各种视觉任务结合，通过比特率与任务准确性的权衡来评估压缩效果。

Result: 该平台展示了在不同数据集上的压缩增益，并已被MPEG采纳用于开发Feature Coding for Machines标准。

Conclusion: CompressAI-Vision为视觉任务优化的压缩技术提供了重要的评估基础，并作为开源软件公开发布，推动了相关标准的发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统SSDG方法假设每个源域都有标注和无标注数据，但实际中标注数据有限且存在域偏移。本文探索更实用的CD-SSDG场景，其中训练集内标注和无标注数据间也存在域偏移。

Method: 提出DAC框架，基于双子模型协同训练，集成额外的特征级监督和非对称辅助任务。特征级监督解决域偏移导致的伪标签不准确问题，辅助任务增强域不变特征学习。

Result: 在Fundus、Polyp和SCGM等真实医学图像分割数据集上的实验表明，DAC框架具有强大的泛化能力。

Conclusion: DAC框架有效解决了CD-SSDG中的挑战，为医学图像分割提供了实用的半监督域泛化解决方案。
...
Method: 1) 对于X/L/M/S模型：采用DINOv3预训练或蒸馏骨干网络，引入STA将单尺度输出转换为多尺度特征；2) 对于超轻量模型：使用HGNetv2进行深度和宽度剪枝，配合简化解码器和升级的Dense O2O；3) 统一设计覆盖8个模型规模。

Result: DEIMv2-X仅用5030万参数达到57.8 AP，超越需要6000万+参数的同类模型；DEIMv2-S成为首个参数低于1000万但AP超过50的模型(50.9 AP)；DEIMv2-Pico仅用150万参数实现38.5 AP，性能与230万参数的YOLOv10-Nano相当。

Conclusion: DEIMv2通过创新的架构设计和特征利用策略，在不同计算资源约束下都实现了卓越的检测性能，为目标检测模型的部署提供了高效的解决方案。
...
Method: 基于一阶平稳条件和TRADES损失，设计动态对抗课程，在LoRA微调过程中逐步增加攻击难度

Result: DAC-LoRA显著提升了对抗鲁棒性，同时不显著影响模型的干净准确率

Conclusion: 该框架提供了一种轻量级、广泛适用的方法，可轻松集成到标准PEFT流程中增强模型鲁棒性
...
Method: FedDSPG方法在训练阶段为每个域引入域特定软提示，并将内容和域知识整合到客户端间的生成模型中；在推理阶段使用生成器获取未见目标域的DSPs来指导下游任务。

Result: 在多个公开数据集上的综合评估表明，该方法在联邦域泛化任务中优于现有强基线方法。

Conclusion: 从生成视角提出的FedDSPG方法能够有效处理联邦域泛化任务，通过生成域特定软提示显著提升了模型在未知域的泛化能力。
...
Method: 基于包含轴向MRI视图与专家撰写报告配对的数据集，LumbarCLIP整合视觉编码器（ResNet-50、Vision Transformer、Swin Transformer）与基于BERT的文本编码器提取密集表示，通过可学习的投影头将特征投影到共享嵌入空间，使用软CLIP损失进行对比训练。

Result: 模型在下游分类任务中达到最先进性能，测试集准确率高达95.00%，F1分数达94.75%，尽管存在固有的类别不平衡问题。消融研究表明线性投影头比非线性变体产生更有效的跨模态对齐。
...
Method: 使用闭环优化策略迭代优化通用扰动，利用影子VideoLLM和轻量级语言模型构建的描绘集来抑制有害帧的相关性分数。

Result: 在三种提示引导采样策略和三个先进VideoLLMs上评估，PoisonVID攻击成功率达到82%-99%。

Conclusion: 研究结果强调了为VideoLLMs开发更先进采样策略的重要性，以应对此类安全威胁。
...
Method: 提出Game of Regularizer (GoR)方法，仅使用两个可训练参数进行动态损失加权，自适应平衡TS和KD目标；同时引入QAT-EKD-GoR集成蒸馏框架，使用多个异构教师模型。

Result: 在图像分类、目标检测和LLM压缩任务上，GoR一致优于最先进的QAT-KD方法；在低功耗边缘设备上实现更快推理同时保持全精度准确率；EKD-GoR在最优条件下可超越全精度模型。

Conclusion: GoR为实际部署提供了鲁棒解决方案，能够有效减少监督信号冲突、改善收敛性并提升小型量化模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 评估大规模网络收集的噪声训练数据（包含大量标签错误）能否与专家验证的小型可信数据集竞争，以推动自动化植物识别系统的发展

Method: 使用两种训练策略：大规模网络收集的噪声数据集 vs 小型专家验证数据集，测试集来自第三方数据源Pl@ntNet移动应用

Result: 挑战赛评估了不同参与研究组的方法和系统，分析了两种训练策略的主要结果

Conclusion: 该挑战为大规模植物识别系统的发展提供了重要参考，探索了网络噪声数据在植物识别任务中的潜力
...
<details>
  <summary>Details</summary>
Motivation: 传统植物计数方法需要为每个物种构建特定模型，但植物具有生物多样性且新品种不断培育，无法穷尽所有物种。现有类无关计数模型对非刚性植物结构计数效果不佳，需要开发专门针对植物动态特性的跨物种计数方法。

Method: TasselNetV4基于普通视觉变换器，结合TasselNet的局部计数思想和类无关计数的提取-匹配范式，引入多分支框感知局部计数器来增强跨尺度鲁棒性。

Result: 在PAC-105和PAC-Somalia两个挑战性数据集上的实验表明，TasselNetV4相比最先进的类无关计数模型不仅计数性能优越，而且效率高。

Conclusion: TasselNetV4成为跨场景、跨尺度和跨物种植物计数的视觉基础模型，解决了植物计数中的生物多样性和动态变化挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有半监督学习方法在视网膜OCT分割中经常产生解剖学上不合理的结果，无法有效建模层-病变相互作用，且缺乏拓扑正确性保证。

Method: 引入完全可微分的生物标志物拓扑引擎，实现层和病变的双向影响联合学习，利用未标记和部分标记数据集，学习解耦的空间和风格因子表示。

Result: 在公共和内部OCT数据集上评估显示，该模型在病变和层分割方面均优于当前最先进方法，并能将层分割泛化到病理病例。
...
<details>
  <summary>Details</summary>
Motivation: 评估在接近真实生物多样性监测场景下的大规模植物识别系统性能，特别是解决开放集识别问题，使系统能够处理训练集中未出现的未知植物种类。

Method: 使用包含110,000多张图像、涵盖1,000种西欧植物的数据集，通过参与式感知平台收集数据，将识别任务作为开放集识别问题进行评估。

Result: 挑战赛成功测试了多种植物识别方法在开放集场景下的性能，分析了各参与研究组提出的系统在处理未知类别识别方面的表现。

Conclusion: LifeCLEF 2016为大规模植物识别提供了重要基准，特别是在开放集识别方面取得了进展，为真实世界的生物多样性监测应用奠定了基础。
...
Conclusion: SCRA-VQA通过描述总结和重排有效提升了LLMs在KB-VQA任务中的推理能力和任务适应性。
...
<details>
  <summary>Details</summary>
Motivation: 探索感知优化中保真度目标和对抗目标作为优化目标的有效性与其作为图像质量评估指标能力之间的相关性，这一关系尚未得到充分研究。

Method: 通过系统性分析，比较不同保真度指标在感知优化中的表现，并研究判别器设计对优化的影响，包括补丁级和卷积架构与Transformer架构的对比。

Result: 发现保真度指标在IQA和优化中的有效性存在明显不对称；判别器设计对优化结果有决定性影响，补丁级和卷积架构能提供更忠实的细节重建。
...
Method: 提出IOG-VQA模型，包含对象交互自注意力机制（捕捉图像中物体间的复杂交互）和基于GAN的去偏框架（生成无偏见的数据分布）。

Result: 在VQA-CP v1和v2数据集上的实验表明，该模型在处理偏见和不平衡数据分布方面表现优异，优于现有方法。

Conclusion: 解决对象交互和数据集偏见对于推进VQA任务至关重要，IOG-VQA为此提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 视频序列中常包含结构化噪声和背景伪影，传统基于稀疏性的鲁棒主成分分析方法难以捕捉真实视频数据的丰富变异性。

Method: 整合低秩时间建模与扩散后验采样，通过核范数正则化和扩散模型先验来分离视频中的动态内容和噪声。

Result: 在心脏超声去雾任务中，相比传统RPCA方法，在对比度增强（gCNR）和信号保持（KS统计量）方面表现出更好的去雾性能。

Conclusion: 将基于模型的时间模型与深度生成先验相结合，有望实现高保真度的视频恢复。
...
<details>
  <summary>Details</summary>
Motivation: 随着VAEs、GANs和LDMs等先进模型生成的合成图像越来越逼真，合成图像检测面临重大挑战。本文旨在解决这一问题。

Method: 利用马尔可夫随机场的局部像素依赖性(LPD)特性，通过重建合成图像来暴露纹理连续性和边缘一致性的破坏。基于LPD构建了仅有110万参数的轻量级神经网络FerretNet。

Result: FerretNet仅在4类ProGAN数据集上训练，在包含22个生成模型的开放世界基准测试中平均准确率达到97.1%，比现有最优方法高出10.6%。

Conclusion: FerretNet提供了一种高效且鲁棒的合成图像检测解决方案，在轻量级架构下实现了卓越的检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的概念瓶颈模型主要针对静态图像，但视频数据具有时间依赖性，需要新的方法来捕捉动作和事件的时间动态。

Method: 设计MoTIF框架，基于Transformer架构，将概念瓶颈模型适配到视频分类，识别视频中重复出现的概念模式（motifs），提供全局、局部和时间三个维度的解释视角。

Result: 实验结果表明概念建模范式可以有效地迁移到视频数据，在保持竞争力的性能的同时，能够更好地理解概念在时间上下文中的贡献。

Conclusion: MoTIF成功地将概念瓶颈模型扩展到视频领域，为视频分类提供了可解释的框架，能够捕捉时间动态并保持良好性能。
...
Result: 在两个公开数据集上的实验结果表明，该方法在低数据量的挑战性场景中表现出有效的目标检测性能，优于多个基于最先进模型建立的基线方法。

Conclusion: FSMODNet框架通过跨模态特征整合在少样本多光谱目标检测任务中展现出鲁棒的适应性，为复杂环境下的目标检测提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在无人机野火监测等安全关键任务中，传统基于密集深度估计或3D场景重建的定位方法在远距离或计算资源受限时不可行，需要更高效的解决方案。

Method: 使用粒子滤波器处理单目标和多目标场景，结合GNSS相机位姿估计和图像分割序列进行3D定位，方法独立于检测算法。

Result: 通过3D仿真和无人机图像分割序列验证，粒子滤波器能够在传统方法失效的情况下有效解决实际定位任务。

Conclusion: 粒子滤波器为计算资源受限的3D定位任务提供了可行方案，结合现有图像分割模型可实现无人机野火监测应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Vision Mamba模型虽然具有全局感受野和低计算复杂度，但在遥感图像分割中容易忽略关键的局部特征（如纹理和边缘），影响了分割精度。

Method: 提出SwinMamba框架，在前两个阶段进行局部Mamba式扫描以捕获细粒度细节，后两个阶段进行全局扫描以融合上下文信息，使用重叠的移位窗口增强区域间信息交换。

Result: 在LoveDA和ISPRS Potsdam数据集上的实验表明，SwinMamba超越了现有最先进方法，证明了其在遥感图像语义分割中的有效性。

Conclusion: SwinMamba通过有效结合局部和全局特征感知，为遥感图像语义分割提供了一个优越的解决方案，具有重要的应用潜力。
...
TL;DR: 提出一种基于打包的多实例学习框架，通过将可变长度的特征序列打包成固定长度序列来解决计算病理学中WSI序列长度极长、变化大和监督有限的问题，同时引入残差分支和注意力下采样器来提高训练效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 计算病理学中的全切片图像具有极长的序列长度（高达200K）、显著的长度变化（200到200K）和有限的监督，导致数据异质性和冗余度高。传统方法在有限监督下为保持异质性而牺牲训练效率和优化。

Method: 提出pack-based MIL框架，将采样的可变长度特征序列打包成固定长度序列以实现批处理训练；引入残差分支将多个切片丢弃的特征组合成超切片进行训练；使用注意力驱动的下采样器压缩特征以减少冗余。

Result: 在PANDA(UNI)数据集上，该方法实现了高达8%的准确率提升，同时仅使用12%的训练时间。

Conclusion: 实验证明，关注计算病理学中的数据挑战在基础模型时代具有重要潜力，该方法有效解决了WSI序列的异质性和冗余问题。
...
TL;DR: SimDiff是一种模拟器约束的扩散模型，通过将环境参数直接集成到去噪过程中，无需推理时的重复模拟器调用即可高效生成物理合理的人类运动。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将基于模拟器的运动投影层纳入扩散过程以确保物理合理性，但由于模拟器的顺序性质，这些方法计算成本高且无法并行化。

Method: 将模拟器运动投影解释为扩散过程中的一种引导形式，提出SimDiff模型，通过环境参数（如重力、风力）条件化去噪过程。

Result: SimDiff能够高效生成物理合理的运动，无需推理时重复模拟器调用，并对不同物理系数提供细粒度控制，成功泛化到未见过的环境参数组合。

Conclusion: SimDiff展示了组合泛化能力，为物理合理的人类运动生成提供了一种高效且可控的解决方案。
...
TL;DR: 本文研究了视觉架构对高斯噪声的鲁棒性，通过评估1174个预训练模型发现了四个提升鲁棒性的设计模式，并提供了理论解释和实用设计指南。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉模型的鲁棒性常被测量，但其对特定架构设计选择的依赖性很少被剖析。本文旨在理解为什么某些视觉架构对高斯噪声具有内在鲁棒性。

Method: 对1174个预训练视觉模型进行广泛评估，识别出四个关键设计模式，并通过理论分析将这些观察到的相关性转化为因果机制。

Result: 发现了四个一致的设计模式：更大的stem核、更小的输入分辨率、平均池化、以及监督ViT而非CLIP ViT，这些改进可带来高达506位的排名提升和21.6%的准确率增益。

Conclusion: 研究结果将鲁棒性分解为可解释的模块，提供了理论解释，并为设计对高斯噪声更鲁棒的视觉模型建立了实用的即插即用指南。
...
TL;DR: 本文通过PRISMA-ScR指导的范围综述，系统梳理了场景图在手术领域的研究进展，揭示了数据鸿沟问题：内部视角研究主要使用真实2D视频，而外部视角4D建模依赖模拟数据。方法上从基础图神经网络发展到专用基础模型，在手术任务中显著优于通用大模型。


<details>
  <summary>Details</summary>
Motivation: 场景图提供结构化关系表示，对于解码复杂动态的手术环境至关重要。本文旨在系统映射手术场景图研究的演变格局，追踪其应用、方法进展和未来方向。

Method: 采用PRISMA-ScR指导的范围综述方法，系统分析手术场景图研究的应用领域、方法论发展和数据使用模式。

Result: 分析显示该领域快速增长，但发现关键的数据鸿沟：内部视角研究主要使用真实2D视频，外部视角4D建模依赖模拟数据。方法上从基础图神经网络发展到专用基础模型，在手术任务中表现优于通用大模型。

Conclusion: 手术场景图正在成熟为关键的语义桥梁技术，支持新一代智能系统提升手术安全性、效率和培训水平。尽管数据标注和实时实现挑战仍存，但正通过新兴技术积极解决。
...
<details>
  <summary>Details</summary>
Motivation: 解决激光功率计传感器涂层缺陷（如热损伤和划痕）的检测难题，这些缺陷会影响医疗和工业应用中激光能量测量的准确性。
...
Conclusion: 该系统通过自动化质量控制实现了潜在的年成本节约，并在设备端实现中具有高效的处理速度。
...
TL;DR: FASTER是一个用于金融咨询播客视频的多模态摘要框架，通过提取视觉和文本特征，生成优化摘要，并对齐关键帧与文本内容，解决了长视频内容理解难题。
...
Method: 使用BLIP生成视觉语义描述，OCR提取文本模式，Whisper进行转录和说话人识别，采用改进的DPO损失函数进行事实检查，基于排序的检索机制对齐关键帧与摘要内容。
...
TL;DR: ASD是一种无需预训练或先决条件的适配器，能够冷启动自监督学习器进行深度图像聚类


<details>
  <summary>Details</summary>
Motivation: 现有方法需要预训练、聚类学习或训练好的聚类模型作为先决条件，限制了自监督学习在图像聚类任务中的灵活应用

Method: 1) 从无标签数据中随机采样伪标签数据；2) 设置实例级分类器学习语义对齐的实例级标签；3) 跟踪预测的类别转换提取高级相似性；4) 为伪标签数据分配聚类级标签；5) 使用带聚类级标签的伪标签数据触发自监督学习器

Result: 在多个基准测试中优于最新的深度图像聚类方法，与使用真实标签的自监督学习方法相比仅有微小差距（如CIFAR-10上仅差1.33%）

Conclusion: ASD能够有效冷启动自监督学习器进行图像聚类，且能进一步提升现有自监督嵌入的深度图像聚类方法的性能
...
TL;DR: 提出SiNGER框架，通过零空间引导的能量重分配来抑制Vision Transformers中的高范数伪影，同时保留信息信号，提升知识蒸馏效果


<details>
  <summary>Details</summary>
Motivation: Vision Transformers会产生高范数伪影，这些伪影在知识蒸馏过程中会主导目标函数，导致学生模型过度拟合伪影而忽略信息信号，限制了大型模型的增益

Method: 使用零空间引导的扰动方法进行教师特征精炼，通过LoRA适配器高效实现扰动，在抑制伪影的同时保留信息信号，然后将精炼后的教师特征蒸馏给学生模型

Result: 在多个下游任务中实现了最先进的性能，生成了更清晰和可解释的表征

Conclusion: SiNGER框架能够有效解决教师模型中伪影与信息信号之间的权衡问题，显著提升知识蒸馏的效果
...
<details>
  <summary>Details</summary>
Motivation: 现有的云分割模型通常与特定传感器配置紧密耦合，依赖地面处理，限制了在星上处理和多传感器应用的灵活性。

Method: 基于SEnSeI-v2改进，集成了改进的光谱描述符、轻量级架构和鲁棒的填充带处理，可接受任意光谱波段组合，生成固定大小的特征图，配合基于改进U-Net的紧凑量化分割模型，在CPU-FPGA混合架构上高效运行。

Result: 在Sentinel-2和Landsat 8数据集上的评估表明，该方法在不同输入配置下都能实现准确的云分割。

Conclusion: Fast-SEnSeI为星上云分割提供了一种传感器无关的轻量级解决方案，适用于空间合格硬件平台。
...
Method: 训练稀疏自编码器将文本嵌入映射到稀疏解耦的潜在空间，设计基于调制频率评分的神经元识别方法定位有害概念相关神经元，通过抑制特定神经元激活实现概念擦除
...
Method: 开发KD-SS（K-Dimensional Sub-Sampling）算法，这是一种轻量级的子采样方法，能够保持点云分辨率，无需降采样输入数据。

Result: 结合当前最先进的分割模型，KD-SS在不同传感器（摄影测量、激光三角测量、LiDAR）和多种植物物种上均取得满意结果。

Conclusion: KD-SS作为轻量级的分辨率保持替代方案，可替代密集预处理和降采样方法，适用于各种植物物种和传感器模式。
...
Method: 1) 学习背景提示获取包含背景和图像语义信息的局部背景相似性；2) 用局部类别相似性精炼局部背景相似性；3) 提出补丁自校准调优灵活选择不同样本的背景补丁数量

Result: 在真实数据集上的实验表明，Mambo在OOD检测和近OOD检测设置方面达到最佳性能

Conclusion: Mambo框架通过减少对局部类别相似性的依赖和灵活的背景提取策略，显著提升了FS-OOD检测的鲁棒性和性能
...
Result: WDES方法能够持续产生更具代表性的数据划分，在街景、医学影像和卫星图像等多种分割任务中应用WDES能够降低性能方差并改进模型评估。WDES在处理小型、不平衡和低多样性数据集时尤其有价值。

Conclusion: WDES方法被证明在足够多代的情况下是全局最优的，相比随机采样能够产生更均匀的数据集划分，特别适用于处理具有挑战性的分割数据集。
...
TL;DR: EnGraf-Net是一种利用语义层次结构（分类法）作为监督信号的端到端深度神经网络模型，用于细粒度分类，无需裁剪技术或手动标注即可达到与最先进方法相竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 现有细粒度分类模型依赖部件标注或自动注意力图，但部件方法存在局部特征表示不完整的问题。人类识别物体时还会形成语义关联，因此利用语义层次结构作为监督信号可以改进分类性能。

Method: 提出EnGraf-Net模型，将语义关联结构化为层次结构（分类法），并将其作为端到端深度神经网络中的监督信号。

Result: 在CIFAR-100、CUB-200-2011和FGVC-Aircraft三个数据集上的实验表明，EnGraf-Net优于许多现有细粒度模型，与最先进方法性能相当。
...
TL;DR: 该论文研究了对抗性补丁在Vision Transformers（ViTs）上的可迁移性，发现CNN的对抗攻击技术可以成功迁移到ViT模型，不同ViT模型对对抗补丁的脆弱性存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习系统的广泛应用，其安全性成为关键问题。ViTs虽然在性能和对对抗扰动的鲁棒性上优于CNNs，但仍存在被对抗补丁攻击的漏洞。研究旨在验证CNN对抗攻击技术在ViT模型上的可迁移性。

Method: 使用Creases Transformation（CT）技术设计逼真的对抗补丁，在二元人物分类任务上测试四个微调ViT模型的脆弱性。通过实验评估对抗补丁从CNN到ViT的跨架构可迁移性。

Result: 实验结果显示不同ViT模型的攻击成功率差异显著：google/vit-base-patch16-224-in21k为40.04%，facebook/dino-vitb16高达99.97%，其他两个模型分别为66.40%和65.17%。

Conclusion: 对抗补丁可以从CNN成功迁移到ViT模型，预训练数据集规模和方法论强烈影响模型对对抗攻击的抵抗力。ViTs虽然在某些方面更鲁棒，但仍存在安全漏洞。
...
TL;DR: UniTransfer是一种新颖的视频概念迁移架构，通过空间和时间步分解实现精确可控的视频概念迁移，包括前景主体、背景和运动流的三重分解，以及基于Chain-of-Prompt的渐进式去噪过程。


<details>
  <summary>Details</summary>
Motivation: 现有视频概念迁移方法缺乏精细的控制能力，无法对视频的不同组件进行独立编辑。本文旨在开发一种能够实现精确可控视频概念迁移的架构。

Method: 提出空间分解（前景、背景、运动流）和扩散时间步分解的双重分解策略；设计双流到单流的DiT架构；引入基于随机掩码的自监督预训练；提出Chain-of-Prompt机制将去噪过程分为三个阶段；构建OpenAnimal动物视频数据集。

Result: 实验表明该方法在多样参考图像和场景下实现了高质量可控的视频概念迁移，在视觉保真度和可编辑性方面均优于现有基线方法。

Conclusion: UniTransfer通过空间和时间步的双重分解实现了精确可控的视频概念迁移，为视频编辑领域提供了新的解决方案。
...
TL;DR: VTTS是一种增强多模态大语言模型推理能力的新方法，通过推理时的迭代感知机制，模仿人类分层注意力机制，在时空区域上逐步优化视觉理解。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖LLM推理来分析静态感知阶段的视觉信息，存在感知阶段固定的局限性。需要模拟人类的分层注意力机制，在推理过程中动态优化感知能力。

Method: 提出视觉测试时缩放(VTTS)方法，包含迭代感知(ITP)机制，使用强化学习和时空监督来优化推理过程。同时构建了VTTS-80K数据集支持该范式。

Result: Videochat-R1.5模型在超过15个基准测试中平均提升超过5%，在视频对话、视频推理和时空感知任务上显著优于Qwen2.5VL-3B和-7B等基线模型。

Conclusion: VTTS通过增加感知计算有效提升了MLLMs的性能，验证了该方法在各种任务和基准上的有效性和泛化能力。
...
TL;DR: Mammo-CLIP Dissect是首个基于概念的可解释性框架，用于系统分析乳腺X光检查深度学习视觉模型，通过乳腺X光专用视觉语言模型为神经元标注人类可理解的文本概念，并量化其与领域知识的对齐程度。


<details>
  <summary>Details</summary>
Motivation: 理解深度学习模型学习的内容对于AI在临床环境中的安全部署至关重要。现有研究主要关注基于像素的可解释性方法，而较少关注模型学习的文本概念，这些概念可能更好地反映临床医生的推理过程。

Method: 利用乳腺X光专用视觉语言模型（Mammo-CLIP）作为"解剖器"，在指定层为神经元标注人类可解释的文本概念，并量化其与领域知识的对齐程度。研究比较了不同训练数据和微调策略下模型的概念学习差异。

Result: 在乳腺X光数据上训练的模型比未在该数据上训练的模型捕获更多临床相关概念，且更符合放射科医生的工作流程。针对特定任务的微调增强了某些概念类别（如良性钙化）的捕获，但可能减少其他类别（如密度相关特征）的覆盖范围。

Conclusion: Mammo-CLIP Dissect提供了关于卷积神经网络如何捕获乳腺X光特定知识的见解，揭示了领域特定训练和任务特定适应如何塑造概念学习，在专业化和泛化之间存在权衡。
...
TL;DR: MOSS-ChatV是一个基于强化学习的框架，通过动态时间规整（DTW）的过程奖励来解决多模态大语言模型在视频推理中的过程不一致性问题，提高推理的可解释性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在视频推理中存在过程不一致问题，即中间推理过程与视频动态不符，即使最终答案正确，也损害了模型的可解释性和鲁棒性。

Method: 提出MOSS-ChatV框架，使用基于DTW的过程奖励进行强化学习，将推理轨迹与时间基础参考对齐，无需辅助奖励模型即可实现高效的过程监督。同时构建MOSS-Video基准，包含标注的推理轨迹。

Result: MOSS-ChatV在MOSS-Video测试集上达到87.2%的准确率，在MVBench和MMVU等通用视频基准上也有性能提升。该框架在不同架构（如Qwen2.5-VL和Phi-2）上均能带来增益，GPT-4o评估显示其能产生更一致和稳定的推理轨迹。

Conclusion: MOSS-ChatV通过过程监督有效解决了视频推理中的过程不一致问题，具有广泛的适用性，为多模态大语言模型的视频推理能力提供了重要改进。
...
TL;DR: 提出了一种新方法，通过将相机和物体运动转换为像素运动，利用稳定扩散网络学习参考运动图，结合语义物体先验生成能准确跟随指定相机轨迹的视频。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常分别学习相机和物体运动，这可能导致相机与物体之间相对运动的混淆。为了解决相机轨迹引导视频生成中的一致性和泛化性问题。

Method: 将相机和物体运动整合为像素运动，使用稳定扩散网络学习相对于指定相机轨迹的参考运动图，结合提取的语义物体先验，通过图像到视频网络生成视频。

Result: 大量实验验证表明，该方法在性能上大幅优于现有最先进方法。

Conclusion: 提出的方法能有效生成准确跟随指定相机轨迹的视频，同时保持物体运动的一致性，在相机轨迹引导的视频生成任务中表现出色。
...
TL;DR: 本文研究了图像生成AI中鉴别器在数据维度和复杂度影响下的劣势条件，发现简单和高度复杂的数据集会降低合成图像的可检测性，而中等复杂度数据集最有利于检测。


<details>
  <summary>Details</summary>
Motivation: 随着图像生成AI的快速发展，合成图像与真实图像的界限变得模糊，引发了生成器与鉴别器之间的竞争。本文旨在探究鉴别器在何种条件下处于最不利地位。

Method: 通过分析数据维度和数据复杂度两个关键因素，使用Kolmogorov复杂度作为数据集内在结构的度量，研究不同复杂度条件下合成图像的可检测性。

Result: 研究发现：简单数据集几乎可以被生成器完美学习，高度多样化的数据集会掩盖生成器的缺陷，而中等复杂度数据集最能暴露生成器的错误，为检测创造最有利条件。

Conclusion: 数据复杂度对鉴别器的检测能力有重要影响，中等复杂度数据集为鉴别器提供了最佳检测环境，而极端复杂度条件会削弱检测效果。
...
TL;DR: 本文对CLIP模型的量化进行了大规模评估，发现量化不仅能提高效率，还能改善模型的可靠性指标，包括校准度和OOD检测性能，挑战了效率与性能必须权衡的传统观点。


<details>
  <summary>Details</summary>
Motivation: 虽然CLIP等视觉语言模型在零样本泛化方面表现出色，但量化对CLIP性能的影响（特别是超出准确度之外的可靠性指标）尚未得到充分探索。

Method: 对CLIP模型进行大规模量化评估，不仅评估分布内准确度，还评估全面的可靠性指标，并研究不同预训练来源的影响。

Result: 量化持续改善通常欠自信的预训练模型的校准度，但会降低过度自信模型的校准度。有趣的是，校准度下降并不妨碍其他可靠性指标的提升，OOD检测性能仍可改善。还发现特定QAT方法能同时提升零样本准确度、校准度和OOD鲁棒性。

Conclusion: 量化在高效、可靠、鲁棒的VLM部署中具有超越传统角色的价值，为多目标优化问题提供了关键见解。
...
TL;DR: TABLET是一个大规模视觉表格理解数据集，包含400万个示例，涵盖20个任务，基于200万个独特表格，其中88%保留了原始可视化。该数据集提供图像-HTML配对表示，用于训练视觉语言模型以提高表格理解性能。
...
Method: 构建TABLET数据集，包含400万个示例，覆盖20个任务，基于200万个独特表格，其中88%保留原始可视化。每个示例包含配对的图像-HTML表示、全面元数据和溯源信息。使用该数据集微调视觉语言模型如Qwen2.5-VL-7B。
...
TL;DR: 提出一种基于共形预测的新方法，通过控制解释的保真度来改进图像预测的特征归因解释，无需真实解释进行校准。
...
Method: 基于共形预测的方法，识别能够保持模型预测的显著特征子集，提出四种一致性函数来量化解释与模型预测的符合程度。

Result: 在6个图像数据集上使用5种解释器进行实证评估，FastSHAP在保真度和信息效率（解释区域大小）方面优于竞争方法，基于超像素的一致性度量比像素级更有效。

Conclusion: 该方法提供了一种可控的解释保真度框架，显著提升了特征归因解释的可靠性和效率。
...
TL;DR: Sigma是一个基于骨骼数据的统一手语理解框架，通过早期模态融合、分层对齐学习和统一预训练来解决语义基础弱、局部细节与全局上下文不平衡、跨模态学习效率低等问题，在多个基准测试中取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 当前手语理解方法面临三个关键限制：1）语义基础弱，模型难以将骨骼数据的低层运动模式与语言意义关联；2）局部细节与全局上下文不平衡；3）跨模态学习效率低，难以构建语义对齐的跨模态表示。

Method: 提出Sigma框架，包含：1）手语感知早期融合机制，促进视觉和文本模态的深度交互；2）分层对齐学习策略，在不同层级上最大化跨模态特征一致性；3）结合对比学习、文本匹配和语言建模的统一预训练框架。

Result: 在孤立手语识别、连续手语识别和无注释手语翻译等多个基准测试中取得了新的最先进结果，证明了语义信息预训练的有效性和骨骼数据作为独立解决方案的潜力。

Conclusion: Sigma框架通过语义信息丰富的预训练和有效的骨骼数据处理，展示了作为手语理解独立解决方案的显著效果，能够处理不同手语和口语语言的多样化任务。
...
TL;DR: 本文对文本-图像生成中的自动评估指标进行了广泛研究，发现没有单一指标在所有任务中表现一致，指标性能随组合问题的类型而变化。


<details>
  <summary>Details</summary>
Motivation: 文本-图像生成发展迅速，但评估输出是否真正捕捉提示中的对象、属性和关系仍是核心挑战。现有评估指标往往基于惯例或流行度而非人类判断验证，因此需要了解这些指标与人类偏好的匹配程度。

Method: 对广泛使用的组合文本-图像评估指标进行广泛研究，分析它们在不同组合挑战中的行为，比较不同指标家族与人类判断的一致性。
...
TL;DR: 提出了SlideMamba框架，将Mamba架构与图神经网络结合，通过熵置信度加权机制自适应融合局部和全局信息，在WSI分析中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 计算病理学需要从全切片图像中提取有意义的表示，现有方法在捕捉长距离上下文依赖和局部空间关系方面存在局限。

Method: 集成Mamba架构（擅长长距离依赖）和图神经网络（擅长短距离空间交互），使用熵置信度加权机制自适应融合两个分支的预测。

Result: 在基因融合和突变状态预测任务中，PRAUC达到0.751±0.05，显著优于MIL、Trans-MIL、Mamba-only、GNN-only和GAT-Mamba等方法。

Conclusion: 集成架构结合自适应融合策略在计算病理学的空间分辨预测建模任务中展现出强大潜力。
...
TL;DR: Hunyuan3D-Omni是一个统一的3D资产生成框架，支持多种精细控制信号（点云、体素、边界框、骨骼姿态等），通过跨模态架构和渐进式训练策略实现可控性提升。
...
Method: 基于Hunyuan3D 2.1构建统一框架，使用单一跨模态架构处理所有控制信号，采用渐进式难度感知采样策略训练模型。

Result: 实验表明额外控制信号提高了生成准确性，支持几何感知变换，并增强了生产工作流的鲁棒性。

Conclusion: Hunyuan3D-Omni通过多模态控制实现了更精确可控的3D资产生成，为实际应用提供了更好的解决方案。
...
TL;DR: 提出了一种利用视觉语言模型自动生成语义注意力图的方法，通过辅助损失函数对齐CNN注意力与语言引导的注意力图，从而提升模型决策的可靠性和认知合理性，无需人工标注。


<details>
  <summary>Details</summary>
Motivation: CNN经常利用表面相关性进行"作弊"预测，现有方法依赖专家标注，可扩展性有限。受认知科学启发，需要一种无需人工标注的方法来引导模型注意力。

Method: 利用视觉语言模型通过自然语言提示自动生成语义注意力图，引入辅助损失函数使CNN注意力与语言引导的注意力图对齐。

Result: 在ColoredMNIST和DecoyMNIST数据集上实验表明，该方法在ColoredMNIST上达到最优性能，在DecoyMNIST上与依赖大量标注的基线方法竞争，显示出更好的泛化能力和减少对捷径的依赖。

Conclusion: 该方法提供了一种可扩展的框架，能够自动引导模型注意力，提升决策可靠性，且模型注意力更符合人类直觉。
...
TL;DR: Decipher-MR是一个针对3D MRI的视觉语言基础模型，通过在包含200,000个MRI序列的大规模数据集上训练，能够适应多种临床任务，包括疾病分类、人口统计预测、解剖定位和跨模态检索。


<details>
  <summary>Details</summary>
Motivation: MRI在临床诊断和研究中的复杂性及异质性对自动化分析构成挑战，尤其是在可扩展和通用化的机器学习应用中。现有的基础模型在MRI领域的应用有限，主要受限于数据稀缺和解剖区域狭窄。

Method: Decipher-MR结合自监督视觉学习和报告引导的文本监督，构建鲁棒且可泛化的表示。模型采用模块化设计，支持在冻结的预训练编码器上微调轻量级的任务特定解码器，以最小计算开销实现多样临床任务。

Result: 在多个基准测试中，Decipher-MR在疾病分类、人口统计预测、解剖定位和跨模态检索等任务上均表现出优于现有基础模型和任务特定方法的性能。

Conclusion: Decipher-MR为基于MRI的AI提供了一个可扩展且多功能的基础，有助于在临床和研究领域高效开发应用。
...
TL;DR: SQ-InstructBLIP通过生成图像感知的子问题和子答案来改进视觉问答任务的推理性能，解决了传统LLMs无法读取视觉信息和黑盒模型不可复现的问题。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言理解领域在多步推理问题上仍面临挑战，传统LLMs方法存在无法获取细粒度视觉内容和使用黑盒模型难以复现的问题。

Method: 提出SQ-InstructBLIP框架，包含共享相同架构的Questioner、Answerer和Reasoner三个组件，通过迭代生成图像感知的子问题和子答案来辅助主问题推理。

Result: 实验表明，SQ-InstructBLIP在VQA任务中使用生成的子问题作为附加信息，比先前工作实现了更准确的推理。

Conclusion: 该方法通过自提问机制有效提升了视觉问答任务的推理性能，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前对文本到图像模型的研究主要关注提示与生成图像的匹配度，但忽视了模型基于先验知识或偏见生成超出提示内容的现象，即幻觉问题。

Method: 通过分析现有评估方法的局限性，提出将幻觉定义为偏见驱动的偏差，并建立包含属性、关系和对象幻觉的三类分类法。

Result: 该框架为文本到图像模型的评估设定了上限，能够揭示隐藏的模型偏见，为更丰富的模型评估提供基础。

Conclusion: 本文为文本到图像生成模型的幻觉问题提供了系统的定义和分类框架，有助于更全面地评估模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有微动作识别方法在真实场景中因人物间差异导致泛化能力不足，同一动作在不同人身上表现不同，阻碍了鲁棒识别

Method: 框架包含特征层和损失层两个即插即用组件：特征层的时频对齐模块通过双分支设计（时域分支应用Wasserstein正则化对齐，频域分支引入方差引导扰动）归一化人物特定运动特征；损失层的组不变正则化损失通过伪分组模拟未见人物分布，增强边界样本权重和子组方差正则化

Result: 在大型MA-52数据集上的实验表明，该框架在准确性和鲁棒性上均优于现有方法，在细粒度条件下实现了稳定的泛化性能

Conclusion: 该框架通过分布鲁棒优化有效解决了微动作识别中的人物间差异问题，为实际应用提供了更可靠的解决方案
...
TL;DR: 本文提出了一种基于3D几何基础模型VGGT的语义匹配方法，解决了现有方法在几何歧义和最近邻规则方面的局限性，通过特征重用、语义头添加和循环一致性训练等策略，在几何感知、匹配可靠性和流形保持方面优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有语义匹配方法存在两个主要问题：(i) 几何歧义：依赖2D基础模型特征难以区分对称结构，需要额外微调但缺乏泛化性；(ii) 最近邻规则：像素级匹配忽略跨图像不可见性和流形保持。需要几何感知的像素描述符和整体密集对应机制。

Method: 提出基于VGGT的方法：(i) 重用早期特征阶段，微调后期阶段，添加语义头实现双向对应；(ii) 通过循环一致性训练策略、合成数据增强和渐进式训练方案，在数据稀缺情况下适应语义匹配场景。

Result: 大量实验表明，该方法在几何感知、匹配可靠性和流形保持方面表现优异，超越了之前的基线方法。

Conclusion: 该方法成功将3D几何基础模型VGGT应用于语义匹配任务，通过创新的训练策略和架构改进，有效解决了现有方法的局限性，为语义匹配提供了新的解决方案。
...
TL;DR: 提出MedVSR框架，专门针对医学视频超分辨率任务，解决现有方法在医学视频中存在的对齐困难和伪影问题


<details>
  <summary>Details</summary>
Motivation: 高分辨率医学视频对准确诊断至关重要，但受硬件和生理限制难以获取。现有视频超分辨率模型在处理医学视频时面临相机抖动、噪声、帧间突变等独特挑战，导致光流误差和对齐困难，且容易产生误导医生的伪影和失真特征

Method: 提出MedVSR框架：1) 交叉状态空间传播(CSSP)模块，通过状态空间模型将远距离帧投影为控制矩阵，选择性传播一致且信息丰富的特征以实现有效对齐；2) 内部状态空间重建(ISSR)模块，结合长距离空间特征学习和大核短距离信息聚合，增强组织结构并减少伪影

Result: 在包括内窥镜和白内障手术在内的四个不同医学场景数据集上的实验表明，MedVSR在重建性能和效率方面显著优于现有视频超分辨率模型

Conclusion: MedVSR是针对医学视频超分辨率任务的定制化框架，能够有效解决医学视频特有的对齐挑战和伪影问题，为临床诊断提供更高质量的医学视频
...
Method: 提出了Variance-Aware Sampling（VAS）数据选择策略，结合结果方差和轨迹多样性来提升奖励方差；发布了包含160万条思维链数据和1.5万条RL问答对的大规模数据集；开源了多尺度多模态推理模型。

Result: 在数学推理基准测试中验证了所提出方法和数据的有效性，理论分析表明奖励方差下界了期望策略梯度的大小。

Conclusion: VAS方法能够有效稳定策略优化，发布的数据集和模型为社区提供了标准化基准，实验证明了各组成部分的贡献。
...
TL;DR: 该论文提出了一种基于Prithvi-EO Vision Transformer架构的海洋科学基础模型，通过自监督预训练在Sentinel-3 OLCI数据上，并在下游任务中验证其性能。


<details>
  <summary>Details</summary>
Motivation: 海洋科学中标记数据稀缺且昂贵，而AI基础模型在无标记大数据上的预训练潜力巨大，可以改变海洋科学AI应用。

Method: 使用Prithvi-EO Vision Transformer架构，在Sentinel-3 OLCI数据上进行自监督预训练，然后在下游任务（叶绿素浓度量化和海洋初级生产力估算）上进行微调。

Result: 模型在海洋监测中表现出色，特别是在利用少量高质量标记数据和捕捉海洋颜色详细空间模式方面，同时与点观测数据匹配良好。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像合成模型在处理复杂光照（如准确阴影、水面反射）和多样化高分辨率输入时存在困难，而现有的扩散模型虽然具备物理和分辨率先验知识，但缺乏合适的框架来释放这些能力。

Method: SHINE引入流形引导锚点损失，利用预训练定制适配器（如IP-Adapter）指导潜在表示以保持主体忠实性同时保护背景完整性；提出退化抑制引导和自适应背景融合来消除低质量输出和可见接缝。
...
TL;DR: QuantVGGT是一个针对十亿级视觉几何基础变换器（VGGT）的量化框架，通过双平滑细粒度量化和噪声过滤多样化采样技术，解决了PTQ在压缩大规模3D重建模型时面临的重尾分布和校准样本不稳定的问题。
...
Method: QuantVGGT主要依赖两项技术贡献：1）双平滑细粒度量化，整合预全局Hadamard旋转和后局部通道平滑来缓解重尾分布和通道间方差；2）噪声过滤多样化采样，通过深层统计过滤异常值，构建帧感知多样化校准集群以确保稳定的量化范围。

Result: 实验表明，QuantVGGT在不同基准和位宽下均达到最先进结果，显著超越之前的通用量化方法。4位QuantVGGT可实现3.7倍内存减少和2.5倍硬件推理加速，同时保持重建精度达到全精度模型的98%以上。

Conclusion: QuantVGGT在资源受限场景下具有巨大优势和实用性，为大规模3D重建模型的部署提供了有效的量化解决方案。
...
TL;DR: NewtonGen是一个集成数据驱动合成与可学习物理原理的框架，通过可训练的神经牛顿动力学(NND)为视频生成过程注入潜在动力学约束，实现物理一致的视频合成和精确参数控制。


<details>
  <summary>Details</summary>
Motivation: 当前大规模文本到视频生成的主要瓶颈是物理一致性和可控性。现有模型往往产生不真实的运动（如物体向上坠落）或速度和方向的突变，且缺乏精确参数控制能力。

Method: 提出NewtonGen框架，核心是可训练的神经牛顿动力学(NND)，能够建模和预测各种牛顿运动，将潜在动力学约束注入视频生成过程，联合利用数据先验和动力学指导。

Result: 该框架能够实现物理一致的视频合成，并具有精确的参数控制能力。
...
Conclusion: 该框架使先进的生成式AI真正实现了实用化部署，为各种硬件配置提供了快速生成和内存高效的解决方案。
...
TL;DR: 本文分析了社区贡献平台（CCP）上公开可用的医学影像数据集，发现当前CCP治理模式未能维持数据集共享、文档化和评估所需的质量标准。


<details>
  <summary>Details</summary>
Motivation: 医学影像数据集对医疗AI至关重要，但当前CCP平台上的数据集存在质量问题，可能对下游应用产生有害影响。

Method: 对CCP平台上公开可用的机器学习数据集进行分析，比较数据集在数据共享、文档化和维护等多个维度的差异。

Result: 发现数据集存在许可证模糊、缺乏持久标识符和存储、重复数据、元数据缺失等问题，且不同平台之间存在差异。

Conclusion: 研究强调了负责任的数据管理和医疗AI算法的重要性，为改进CCP数据集质量提供了见解。
...
TL;DR: 本文提出了多智能体生成流网络（MA-GFlowNets）的理论框架，使多个智能体能够通过联合动作协作生成对象，并提出了四种算法。


<details>
  <summary>Details</summary>
Motivation: 现有的生成流网络使用流匹配损失来学习生成对象的随机策略，但多智能体生成流网络的理论框架尚未建立。

Method: 提出了四种算法：集中式流网络用于集中训练、独立流网络用于分散执行、联合流网络实现集中训练分散执行及其条件版本。联合流训练基于局部-全局原则。

Result: 实验结果表明，所提出的框架在性能上优于强化学习和基于MCMC的方法。

Conclusion: MA-GFlowNets框架为多智能体协作生成提供了理论基础和有效算法，具有理论保证和优越性能。
...
TL;DR: FastEagle是一种非自回归级联草稿生成器，通过单次前向传播生成整个草稿，取代了传统草稿生成器的N次顺序传递，实现了更快的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有的草稿生成器（如EAGLE）需要N次顺序传递来生成N个token，限制了推理速度的提升。FastEagle旨在通过消除草稿生成中的顺序依赖关系，实现更高效的LLM推理加速。

Method: FastEagle采用非自回归级联草稿生成器，用轻量级层级联替换时间步骤，并通过层级监督训练来减轻错误累积。结合约束草稿树保持无损验证成本。
...
Conclusion: 消除草稿生成中的顺序依赖是实现无损LLM推理加速的实用路径，FastEagle证明了这一方法的有效性。
...
TL;DR: 本文提出了一种机器学习参数化方法（mloz），用于在气候模型中交互式模拟臭氧的日变化和趋势，解决了传统化学方案计算成本高的问题。该方法仅需大气温度剖面信息作为输入，比UKESM化学方案快31倍，且可跨模型迁移。


<details>
  <summary>Details</summary>
Motivation: 大多数CMIP气候模型由于大气化学方案计算成本高而缺乏交互式臭氧表示，这限制了气候敏感性模拟的准确性，因为臭氧趋势和变率会显著调节大气反馈过程。

Method: 开发机器学习参数化方法mloz，仅使用大气温度剖面信息作为输入，在UKESM和ICON两种气候模型中在线实现臭氧的交互式模拟，包括与准两年振荡的双向相互作用。

Result: mloz在十年时间尺度上表现出高保真度，比UKESM化学方案快31倍，仅占气候模型总运行时间的不到4%。成功实现了从UKESM到ICON的跨模型迁移。

Conclusion: mloz具有在缺乏交互化学的CMIP级气候模型中广泛应用的潜力，特别适用于关注气候敏感性的未来气候变化评估，其中臭氧趋势和变率对大气反馈过程有重要调节作用。
...
TL;DR: 该论文提出了一种基于Transformer的自编码器方法，用于对EEG脑电信号进行匿名化处理，在保护用户隐私的同时保留数据在机器学习任务中的实用性。


<details>
  <summary>Details</summary>
Motivation: 随着EEG消费设备的普及，脑电信号中可能包含的个人身份信息泄露风险增加，需要在保护用户隐私的同时保持数据在医疗应用（如睡眠分期）中的实用性。

Method: 使用基于Transformer的自编码器对EEG数据进行匿名化处理，通过评估匿名化前后数据的重识别能力和实用性来验证方法有效性。

Result: 实验结果表明，该方法能显著降低EEG信号的可重识别性，同时保持其在机器学习任务中的实用性。

Conclusion: 提出的Transformer自编码器方法能有效平衡EEG数据的隐私保护和实用性，为解决脑电信号隐私问题提供了可行方案。
...
TL;DR: 本文系统研究了针对基于记忆度的影响估计器的攻击可行性，提出了通过计算输入伪逆的实用攻击方法，揭示了影响估计在对抗性扰动下的脆弱性


<details>
  <summary>Details</summary>
Motivation: 随着影响估计工具在数据估值和负责任机器学习中的广泛应用，需要研究这些评分是否可能被对抗性操纵

Method: 提出了计算输入伪逆的实用攻击方法，仅需模型的黑盒访问，计算开销较小，并在多个图像分类任务上进行了实证验证

Result: 实验表明即使是最先进的影响估计代理也容易受到针对性评分操纵，理论分析揭示了影响估计在对抗性扰动下的固有脆弱性

Conclusion: 研究结果凸显了基于影响归因的关键脆弱性，表明需要开发鲁棒的防御机制
...
TL;DR: 该论文提出了一种统一对比表示和时序距离框架的新方法，用于目标条件强化学习（GCRL），通过拟度量表示空间结构和约束学习后继表示，能够在次优数据和随机环境中学习最优目标到达距离。


<details>
  <summary>Details</summary>
Motivation: 现有的GCRL方法中，对比表示和时序距离两种框架各有优势但存在局限性：对比学习方法在拼接任务上表现不佳，而拟度量网络方法在高维噪声环境中表现较差。需要一种能够结合两者优势的统一方法。

Method: 使用拟度量表示空间结构（三角不等式）和适当的约束条件，学习能够实现最优目标到达的后继表示。该方法能够利用拟度量距离参数化，即使在次优数据和随机环境中也能学习最优目标到达距离。

Result: 在现有离线GCRL基准测试中，该方法在对比学习方法表现不佳的拼接任务上提高了性能，同时在拟度量网络方法表现较差的高维噪声环境中也表现出色。

Conclusion: 该方法成功结合了对比学习方法的稳定性和长时程能力，以及拟度量网络参数化的自由拼接能力，实现了两种框架的优势互补。
...
Method: 设计了能够显式捕捉多尺度频率振荡的编码器，引入基于注意力的编码器学习EEG通道间和通道内的复杂依赖关系，集成门控网络动态过滤噪声通道，并使用结合监督和对比学习的新型损失函数。

Result: 在多种CNS疾病治疗效果分类以及帕金森病和阿尔茨海默病诊断等应用中验证了方法的有效性，能够从不同物种的原始EEG信号中提取生物学有意义的模式。

Conclusion: 提出的学习范式能够自主选择高质量通道，通过创新的架构和损失设计实现鲁棒的泛化能力，为EEG信号分析提供了有效的解决方案。
...
TL;DR: DARTVAE是一个规则引导的多模态聚类框架，通过将领域特定约束嵌入到表示学习过程中，结合VAE架构和LLM生成的规则知识图谱，实现更操作意义和可解释的聚类结果。


<details>
  <summary>Details</summary>
Motivation: 传统聚类方法仅依赖输入数据的相似性，无法捕捉许多领域中关键的结构或语义约束。需要将领域特定约束直接整合到表示学习过程中。

Method: 扩展VAE架构，嵌入显式规则、语义表示和数据驱动特征到统一潜空间，通过规则一致性和违反惩罚在损失函数中强制约束合规。规则由LLM生成并结构化到知识图谱中。

Result: 在飞机和汽车数据集上的实验表明，规则引导聚类产生更具操作意义和可解释的聚类（如分离无人机、统一隐形飞机、区分SUV和轿车），同时改进了传统聚类指标。

Conclusion: DARTVAE通过结合规则编码和学习表示，比纯数据驱动模型获得更有意义和一致的聚类结果，突显了约束引导多模态聚类在复杂知识密集型场景中的实用性，但面临LLM规则幻觉、规则冲突和可扩展性挑战。
...
<details>
  <summary>Details</summary>
Motivation: 传统注意力层参数依赖于输入元素的成对交互，导致内存和计算复杂度随序列长度呈二次方增长。现有方法要么引入稀疏性忽略部分交互，要么引入循环依赖（如SSM），但两者都有缺点

Method: 基于树结构矩阵的高效求逆技术，开发了一种新颖算法，将稀疏注意力和循环依赖的优势结合起来

Result: 该方法能够有效缓解注意力机制在长序列处理中的计算和内存瓶颈

Conclusion: 提出的树结构矩阵求逆算法为长序列处理提供了一种有效的解决方案，结合了稀疏性和循环依赖的优点
...
TL;DR: 本文提出了一种深度学习方法来预测混凝土中随时间变化的全场损伤，使用自回归U-Net模型预测微观结构几何和收缩演化下的标量损伤场演化，并通过CNN预测关键力学性能。


<details>
  <summary>Details</summary>
Motivation: 传统全场损伤评估计算量大，需要一种更高效的方法来评估混凝土损伤演化，并理解骨料特性与有效收缩和刚度降低之间的关系。

Method: 采用双网络架构：自回归U-Net模型用于预测损伤场演化，CNN用于基于损伤估计预测力学性能（收缩和残余刚度）。

Result: 该方法在合成数据集上表现出高计算效率和稳健的预测性能，显著降低了传统全场损伤评估的计算负担。

Conclusion: 该方法有助于优化混凝土配合比设计，提高耐久性并减少内部损伤，为理解骨料特性与力学性能关系提供了新见解。
...
TL;DR: 提出用复杂度奖励替代熵奖励的新策略优化方法CDPO，通过平衡随机性和结构化来改进探索策略


<details>
  <summary>Details</summary>
Motivation: 传统的熵最大化方法将策略推向均匀随机分布，这种无结构的探索策略有时效率低下，需要更鲁棒的复杂度度量

Method: 采用复杂度度量（香农熵与不平衡度的乘积）作为正则化器，在PPO基础上开发CDPO算法，平衡随机性和结构化

Result: 在离散动作空间任务中，CDPO比PPO对复杂度系数的选择更鲁棒，特别是在需要更多探索的环境中表现更好

Conclusion: 复杂度驱动的策略优化能够引导智能体发现结构化且适应性强的策略，优于传统的熵最大化方法
...
TL;DR: 本文提出了一个理论框架来分析基于确定性扩散模型的逆问题求解算法，证明了当数据分布集中于低维模型集时，噪声卷积分数可解释为时变投影，并将扩散先验算法解释为具有变化投影的广义投影梯度下降方法。
...
Method: 开发理论框架分析确定性扩散算法，证明噪声卷积分数可解释为时变投影，将扩散先验算法视为广义投影梯度下降，并在满足限制等距性质时推导收敛速率。

Result: 当感知矩阵在模型集上满足限制等距性质时，可以获得明确的收敛速率，该速率显式依赖于噪声调度。在低维紧凸集和低秩高斯混合模型等数据分布上验证了框架有效性。

Conclusion: 该理论框架为扩散模型在逆问题中的应用提供了严格的理论基础，即使在非凸模型集上也能建立全局收敛保证，扩展了扩散先验算法的理论理解。
...
TL;DR: MDBench是一个开源基准测试框架，用于评估动态系统的模型发现方法，包含12种算法在14个PDE和63个ODE上的测试，评估指标包括导数预测精度、模型复杂度和方程保真度。


<details>
  <summary>Details</summary>
Motivation: 当前模型发现方法主要关注单一方程的识别，缺乏对动态模型发现的全面基准测试，需要建立系统化的评估框架来跟踪进展和理解方法间的权衡。

Method: 开发MDBench框架，评估12种算法在14个偏微分方程和63个常微分方程上的表现，考虑不同噪声水平，使用导数预测精度、模型复杂度和方程保真度等指标。

Result: 线性方法和遗传规划方法分别在PDE和ODE上获得最低预测误差，线性模型对噪声更具鲁棒性。同时揭示了当前方法在流体动力学和热力学等复杂PDE系统中的关键局限性。

Conclusion: MDBench通过提供严格可扩展的基准测试框架和丰富的动态系统数据集，能够系统评估、比较和改进方程精度与鲁棒性，加速模型发现方法的发展。
...
TL;DR: 本文分析了神经概率电路（NPC）的对抗鲁棒性，并提出了一种新的鲁棒神经概率电路（RNPC），通过类级集成方法提高对抗攻击下的鲁棒性。
...
Method: 提出RNPC方法，采用新颖的类级集成推理机制，确保两个模块输出的鲁棒组合。理论分析证明RNPC比NPC具有更好的对抗鲁棒性。

Result: 在图像分类任务上的实验结果表明，RNPC在保持良性输入高准确率的同时，相比现有概念瓶颈模型具有更优越的对抗鲁棒性。

Conclusion: RNPC是首个针对识别模块对抗攻击的鲁棒神经概率电路，理论分析和实证结果都证明了其有效性。
...
Method: 构建了两种混合分类器：(i) XGBoost+随机森林 (XGB-RF) 和 (ii) SVM+逻辑回归 (SVM-LR)。使用泄漏安全的标准化流程，包括编码、插补、最小-最大缩放；仅在训练折叠上使用SMOTE；对SVM进行概率校准。在主要数据集上拟合并冻结流程，然后在PIMA外部队列上进行验证。

Result: 在主要数据集上，XGB-RF的AUROC约0.995，AUPRC约0.998，优于SVM-LR（AUROC约0.978；AUPRC约0.947）。在PIMA外部验证集上，XGB-RF保持强劲性能（AUROC约0.990；AUPRC约0.959），而SVM-LR较低（AUROC约0.963；AUPRC约0.875）。

Conclusion: XGB-RF在内部和外部队列中始终优于SVM-LR，在ROC/PR上表现出较小的外部衰减和可接受的校准。这些结果支持基于梯度提升的混合方法作为糖尿病风险分层的稳健、可转移方法，并激励基于临床权衡的前瞻性多中心验证和部署时阈值选择。
...
TL;DR: 提出PIRF方法，将物理约束生成建模为稀疏奖励优化问题，通过轨迹级奖励直接反向传播梯度，避免DPS式值函数近似误差，在五个PDE基准测试中实现高效采样下的优越物理约束执行


<details>
  <summary>Details</summary>
Motivation: 扩散模型在科学领域展现强大生成能力，但常违反物理定律。现有方法依赖DPS式值函数近似，存在显著误差和训练不稳定问题

Method: PIRF方法：1）层截断反向传播利用物理奖励的时空局部性；2）基于权重的正则化方案比传统蒸馏方法更高效
...
TL;DR: 本文对贝叶斯神经网络（BNNs）的超参数进行了全局敏感性分析，发现超参数之间存在复杂交互作用，影响预测准确性和不确定性量化（UQ）质量。建议使用全局敏感性分析或贝叶斯优化方法来改进BNNs的超参数选择。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯神经网络虽然能提供预测不确定性量化，但在实际应用中由于训练近似方法和超参数选择困难，难以获得准确的UQ。超参数数量多且影响不透明，需要系统分析其影响。

Method: 采用全局敏感性分析方法，在不同超参数设置下评估BNNs的性能表现，分析超参数对预测准确性和UQ的影响。

Result: 研究发现BNNs的多个超参数之间存在交互作用，共同影响预测准确性和UQ质量。超参数选择对BNNs性能有显著影响。

Conclusion: 建议在实际应用中使用全局敏感性分析或贝叶斯优化等方法来辅助超参数选择和维度约简，以确保BNNs能够提供准确的不确定性量化。
...
TL;DR: 提出了一种名为Neural FMM的新型神经网络架构，将快速多极方法（FMM）的信息流集成到分层机器学习框架中，用于学习椭圆偏微分方程的Green算子。


<details>
  <summary>Details</summary>
Motivation: 尽管FMM在物理和工程中广泛应用，但其与现代机器学习架构的集成仍未被充分探索。

Method: 利用FMM方法的分层计算流来分离局部和远场相互作用，并有效学习它们各自的表示。

Result: 开发了Neural FMM架构，该架构整合了FMM的信息流到分层机器学习框架中。

Conclusion: Neural FMM为学习椭圆偏微分方程的Green算子提供了一种有效的分层机器学习方法。
...
TL;DR: 提出了一种使用可解释机器学习技术对视频流QoE建模的新方法，结合Kolmogorov-Arnold网络和频域特征，在保持模型透明度的同时提高预测精度


<details>
  <summary>Details</summary>
Motivation: 传统黑盒方法无法捕捉视频流服务中复杂特征与用户体验的关系，需要开发既准确又可解释的QoE建模方法

Method: 使用Kolmogorov-Arnold网络作为可解释的读出层，结合紧凑的频域特征处理原始时间序列数据，保留时间信息的同时确保模型透明度

Result: 在流行数据集上评估表明，该方法在QoE预测方面具有更高的准确性，同时提供透明度和可解释性

Conclusion: 该方法成功实现了准确且可解释的QoE建模，为视频流服务优化提供了有效的解决方案
...
TL;DR: 本文提出了一种新型稳定、近可逆的Runge-Kutta方案（EES方案），用于解决神经SDE求解器在反向传播中的内存效率和梯度精度平衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统神经SDE求解器的反向传播方法存在两难选择：离散化后优化方法内存成本高，优化后离散化方法梯度近似误差大。现有的代数可逆求解器虽然承诺内存效率和梯度精度，但在复杂模型和大步长下往往不稳定。

Method: 开发了显式且有效对称（EES）的Runge-Kutta方案，这些方案保留了可逆求解器的优点，同时克服了其不稳定性，能够在不受步长或模型复杂性严重限制的情况下实现内存高效训练。

Result: 通过数值实验证明，EES方案在稳定性和可靠性方面表现优越，为神经SDE的可扩展和准确训练提供了实用基础。

Conclusion: EES方案成功解决了神经SDE训练中的稳定性问题，为内存高效且梯度准确的训练提供了可行的解决方案。
...
TL;DR: 函数编码器通过定义学习特征映射的内积核，建立了与特征学习和核方法的理论联系，提出了两种训练算法来学习紧凑基函数，并在多项式基准和非线性动力系统上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 建立函数编码器与核方法的理论联系，解释其能够独立于数据集大小进行扩展并适应数据内在结构的能力，同时为神经模型提供核式分析。

Method: 提出了两种训练算法：渐进式训练方法逐步构建基函数，以及训练后剪枝方法提供计算效率高的替代方案。两种方法都利用PCA原理揭示学习空间的内在维度。
...
TL;DR: 本文提出了一种基于去噪扩散模型的互信息估计方法，通过扩散模型的信息论公式，将互信息与条件和无条件扩散的最小均方误差差异联系起来。
...
Method: 利用去噪扩散模型的信息论公式，将互信息表示为条件与无条件扩散的最小均方误差差异的一半，并在所有信噪比上进行积分。采用自适应重要性采样实现可扩展的互信息估计。

Result: 该方法通过了自一致性测试，并且在性能上优于传统的和基于分数的扩散互信息估计器。即使在高互信息情况下仍保持强性能。
...
TL;DR: SIL-C是一个新颖的框架，通过双边惰性学习映射技术确保技能增量学习中技能与策略的兼容性，使得增量学习的技能改进能够直接提升下游策略性能而无需重新训练。


<details>
  <summary>Details</summary>
Motivation: 技能增量学习(SIL)中，随着技能库的演化，现有基于技能的策略可能会失去兼容性，限制了技能的可重用性和泛化能力。
...
TL;DR: Latent Twins是一个统一的数学框架，通过在潜在空间中创建隐藏代理来模拟底层方程，将表示学习和算法求解方法统一起来。
...
Method: 提出Latent Twins框架，在学习的潜在空间中创建数学系统的镜像，该空间由算子控制，将经典建模、反演、模型约简和算子逼近统一为单一原理。

Result: 在ODE、PDE基准测试和实际地理数据重建中验证了框架的有效性，Latent Twins提供了紧凑、可解释的解决方案算子代理。

Conclusion: 该框架提供了可扩展的、理论基础的代理，在数据驱动的表示学习和经典科学建模之间架起桥梁。
...
TL;DR: 本文提出了一种将多轮任务规划转化为单轮任务推理的新方法，通过GRPO算法实现高效策略优化，在复杂任务规划基准上取得了优于更大基线模型的性能。


<details>
  <summary>Details</summary>
Motivation: LLM在自主代理应用中面临多轮任务规划的挑战，包括稀疏奖励、长视野信用分配和强化学习计算开销等问题。

Method: 将多轮任务规划转化为单轮任务推理问题，使用GRPO算法进行策略优化，利用专家轨迹提供密集可验证的奖励。

Result: 1.5B参数模型在超过30步的长视野规划任务中达到70%的成功率，优于14B参数的基线模型。
...
TL;DR: 提出PFedDL框架解决多站点fMRI数据隐私和异构性问题，通过联邦字典学习分解为全局共享和局部个性化组件，在ABIDE数据集上验证优于现有方法


<details>
  <summary>Details</summary>
Motivation: 多站点fMRI研究面临数据隐私约束和站点异质性导致的非IID数据问题，阻碍通用模型开发

Method: 个性化联邦字典学习(PFedDL)：各站点独立进行字典学习，将站点特定字典分解为共享全局组件和个性化局部组件，通过联邦聚合更新全局原子，独立优化局部原子

Result: 在ABIDE数据集上的实验表明，PFedDL在非IID数据集上的准确性和鲁棒性优于现有方法

Conclusion: PFedDL框架有效解决了多站点神经影像分析中的数据隐私和异质性问题，实现了更好的跨站点协作建模
...
TL;DR: 提出了CERMIC框架，通过动态校准内在好奇心和推断多智能体上下文来增强多智能体探索能力，在稀疏奖励环境中显著优于现有算法


<details>
  <summary>Details</summary>
Motivation: 现有好奇心机制往往将环境随机性误认为有意义的新颖性，且忽视了同伴行为新颖性这一编码潜在任务动态的重要信息，导致在去中心化、无通信的MARL设置中探索效果不佳

Method: 受人类儿童通过观察同伴来校准探索行为的启发，提出CERMIC框架，让智能体能够鲁棒地过滤噪声惊喜信号，并通过动态校准内在好奇心来指导探索。同时生成理论上有依据的内在奖励，鼓励智能体探索具有高信息增益的状态转移

Result: 在VMAS、Meltingpot和SMACv2等基准测试套件上的实验结果表明，使用CERMIC进行探索在稀疏奖励环境中显著优于最先进的算法

Conclusion: CERMIC通过有效利用多智能体上下文信息和理论上有依据的内在奖励机制，为复杂多智能体强化学习中的自主探索提供了有效的解决方案
...
TL;DR: 开发基于机器学习的策略来预测大规模并行化学计算所需的资源成本，帮助用户在超级计算机上运行昂贵实验前进行优化选择。
...
Method: 评估了多种机器学习模型和策略，基于CCSD应用在DOE Frontier和Aurora超级计算机上的运行时参数值集合进行开发，重点使用了梯度提升模型和主动学习方法。

Result: 梯度提升模型在预测CCSD迭代总执行时间时，在Aurora和Frontier上的平均绝对百分比误差分别为0.023和0.073；主动学习方法仅需约450个实验就能达到约0.2的MAPE。

Conclusion: 机器学习方法能够有效预测化学计算应用的资源需求，为超级计算机用户提供可靠的参数优化指导，显著降低实验成本。
...
TL;DR: 本文建立了上下文学习(ICL)稳定性与演示样本数量之间的非渐近下界，提出了一个两阶段可观测估计器，通过单次校准生成实际可用的提示长度估计。
...
Method: 基于固定高维亚高斯表示下的协方差谱性质分析，建立了ICL稳定性的理论下界，并设计了包含单次校准的两阶段估计器。
...
TL;DR: 本文提出了Bispectral Optimal Transport（双谱最优传输），这是一种对称感知的离散最优传输扩展方法，通过使用双谱表示来比较元素，能够在保持信号结构的同时消除群作用带来的变化。


<details>
  <summary>Details</summary>
Motivation: 在对称性丰富的场景中，仅基于原始特征间几何距离的最优传输对齐可能忽略数据的内在相干结构。传统OT方法无法有效处理由视觉对称性带来的冗余变化。

Method: 引入双谱最优传输，使用双谱（一种群傅里叶不变量）来表示元素，该表示保留了所有信号结构，同时仅去除由群作用引起的变化。

Result: 在经视觉对称性变换的基准数据集上，Bispectral OT计算的传输计划比原始特征OT实现了更高的类别保持准确率，提高了捕捉数据集底层语义标签结构的有意义对应关系的质量。

Conclusion: Bispectral OT能够有效去除不影响类别或内容的冗余变化，同时保持数据的语义结构，为对称性丰富的场景提供了更优的对齐方法。
...
TL;DR: 本文研究发现联邦学习框架下的LLM微调仍然存在隐私泄露风险，攻击者可以从全局模型中提取训练数据，且模型越大泄露风险越高。作者还提出了针对FL的增强攻击策略，并评估了多种隐私保护技术。


<details>
  <summary>Details</summary>
Motivation: 组织通常不愿意共享本地数据，而联邦学习被认为是隐私保护的解决方案。然而，现有研究认为FL通过聚合过程保护隐私的观点可能存在错误，需要验证FL在LLM微调中的实际隐私保护效果。

Method: 通过大量实验验证FL框架下LLM的隐私泄露风险，使用简单的生成方法从全局模型中提取训练数据，并设计了专门针对FL的增强攻击策略，通过跟踪训练过程中的全局模型更新来加剧隐私泄露。

Result: 实验表明攻击者确实可以从FL训练的全局LLM中提取训练数据，隐私泄露程度随模型规模增大而增加。增强攻击策略进一步加剧了隐私风险。

Conclusion: FL框架下的LLM训练存在显著的隐私泄露风险，需要采用差分隐私、正则化约束更新和安全对齐等隐私保护技术来降低风险，为实际应用提供了重要指导。
...
TL;DR: FIRM-DTI是一个轻量级药物-靶点结合亲和力预测框架，通过FiLM层进行特征调制和三元组损失进行度量学习，在跨域泛化方面达到SOTA性能
...
Result: 在Therapeutics Data Commons DTI-DG基准测试中达到最先进性能，并通过消融研究和域外评估验证了有效性
...
<details>
  <summary>Details</summary>
Motivation: 现有PPO及其变体在强化学习优化大语言模型时，由于裁剪机制会丢弃低概率token的梯度信号，而这些token在调节熵演化中起着关键作用。

Method: 提出CE-GPPO算法，以温和有界的方式重新引入被裁剪token的梯度，通过控制裁剪区间外token的梯度大小来实现探索-利用平衡。

Result: 理论分析和实验证据表明CE-GPPO有效缓解熵不稳定性，在数学推理基准测试中在不同模型规模下均优于强基线方法。
...
TL;DR: SynGA是一种基于遗传算法的分子设计方法，直接在合成路径上操作，通过定制交叉和变异算子约束分子空间的可合成性，并可结合机器学习过滤器提升性能。
...
Method: 使用遗传算法，定制交叉和变异算子约束可合成性，通过修改适应度函数适应不同设计任务，还可结合基于机器学习的构建块过滤器。

Result: SynGA在可合成类似物搜索和样本高效属性优化等任务中表现出色，结合过滤器后达到最先进性能。SynGBO变体在贝叶斯优化内循环中使用SynGA和块过滤。

Conclusion: SynGA因其轻量级和内置可合成性约束，不仅可作为强大的独立基准，也可作为未来合成感知工作流程中的多功能模块。
...
TL;DR: 本文通过核回归分析，揭示了缩放定律实际上是冗余定律，证明了模型性能的幂律改进与数据协方差谱的多项式尾部相关，并建立了该定律在各种变换和架构中的普适性。


<details>
  <summary>Details</summary>
Motivation: 深度学习中的缩放定律显示了模型性能随数据集和模型规模增加的幂律改进，但其数学起源尤其是缩放指数一直不明确。本文旨在从数学上解释缩放定律的本质。

Method: 使用核回归方法，分析数据协方差谱的多项式尾部如何产生超额风险的幂律关系，并验证该定律在有限可逆变换、多模态混合、有限宽度近似以及Transformer架构中的普适性。

Result: 发现缩放指数α=2s/(2s+1/β)，其中β控制谱尾部，1/β衡量冗余度。学习曲线的斜率不是普适的，而是依赖于数据冗余度，更陡的谱会加速规模回报。
...
<details>
  <summary>Details</summary>
Motivation: Transformer-based模型在表格数据合成质量上优于GAN和Diffusion模型，但计算成本高，限制了在普通硬件上的应用。需要研究如何通过超参数调整平衡数据质量和计算效率。

Method: 在两个工具(GReaT和REaLTabFormer)上评估10种不同架构和深度的模型配置，从运行时间、机器学习效用和与真实数据分布相似性三个维度进行敏感性分析，使用四个真实世界数据集进行实验。

Result: 运行时间与超参数数量成正比，浅层配置完成更快；GReaT运行时间始终低于REaLTabFormer；小数据集上两种工具都能生成高质量数据，但大数据集上只有REaLTabFormer能保持强效用和相似性。

Conclusion: REaLTabFormer与轻量级LLM结合提供了最佳平衡，能在保持数据质量的同时降低计算需求，但其运行时间仍高于GReaT和其他TDS工具，表明效率提升存在上限。
...
TL;DR: Sig2Model是一种高效自适应的学习索引，通过sigmoid增强近似、主动更新训练和神经联合优化框架，显著降低了动态更新场景下的重训练成本，实现了比现有可更新学习索引更好的性能。


<details>
  <summary>Details</summary>
Motivation: 传统学习索引在静态数据集上表现出色，但在动态更新场景下性能下降严重，因为维护CDF不变性需要全局模型重训练，这会阻塞查询并限制QPS。现有方法未能有效解决重训练成本问题。

Method: 1) sigmoid增强近似技术：通过局部sigmoid函数近似数据分布变化，推迟完全重训练；2) 基于高斯混合模型的主动更新训练：识别高更新概率区域进行战略占位符分配；3) 神经联合优化框架：通过梯度学习持续优化sigmoid集成和GMM参数。

Result: 在真实世界和合成工作负载上的评估显示，Sig2Model将重训练成本降低高达20倍，QPS提高高达3倍，内存使用减少高达1000倍。

Conclusion: Sig2Model通过创新的自适应技术有效解决了学习索引在动态更新场景下的性能瓶颈，为实际工作负载中的学习索引应用提供了可行解决方案。
...
TL;DR: 提出了一种结合MLP和CNN的新型时间序列预测模型，通过MLP捕捉长期趋势依赖，CNN处理局部变化模式，并引入IConv架构独立处理通道间关系以降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现实世界时间序列数据存在非平稳性，传统MLP模型在捕捉局部变化（如季节性模式和残差分量）方面存在局限性，而CNN能有效处理这些变化。

Method: 结合MLP和CNN：MLP建模整体趋势以考虑长期依赖；CNN使用多样化核函数建模细粒度局部模式；提出IConv架构独立处理时间依赖通道并通过不同层考虑通道间关系。

Result: 在时间序列数据集上的广泛实验表明，该方法在多变量时间序列预测方面具有优越性。

Conclusion: 提出的MLP-CNN混合模型能够有效解决时间序列非平稳性问题，在保持计算效率的同时提升预测精度。
...
<details>
  <summary>Details</summary>
Motivation: 解决深度神经网络在噪声标签和数据异质性环境下训练的挑战，传统方法需要大量超参数调优或干净验证集，而LiLAW旨在提供一种计算高效且无需过多调优的解决方案。

Method: 使用三个可学习参数动态调整每个训练样本的损失权重，基于样本难度分类；在每个训练小批量后，使用验证集上的单个小批量梯度下降步骤更新权重，无需干净验证集。

Result: 在多个通用和医学影像数据集、不同噪声水平和类型、损失函数及架构上的广泛实验表明，LiLAW能一致提升性能，即使在高度噪声环境下也有效，且不依赖数据增强或高级正则化。
...
TL;DR: 本文提出了任务依赖初始化（TDI）方法，通过功率谱匹配将状态空间模型的归纳偏置与任务的光谱特征对齐，显著提高了模型的泛化能力和样本效率。
...
Method: 首先形式化线性时不变SSM的归纳偏置，提出TDI方法：功率谱匹配，在大型训练前将模型的归纳偏置与任务的光谱特征对齐。

Result: 在多样化真实世界基准测试中，TDI显著提高了泛化能力和样本效率，特别是在低数据量情况下。
...
TL;DR: 本文提出了FERD框架，这是首个增强公平性的数据无关鲁棒性蒸馏方法，通过调整对抗样本的比例和分布来解决现有方法忽视的鲁棒公平性问题。


<details>
  <summary>Details</summary>
Motivation: 现有数据无关鲁棒性蒸馏方法只关注整体鲁棒性，忽视了不同类别间的鲁棒性差异问题，导致学生模型在不同类别上表现出显著的鲁棒性差异和不稳定性。
...
Conclusion: FERD在鲁棒性和公平性方面都表现出优越性能，有效解决了数据无关鲁棒性蒸馏中的公平性问题。
...
TL;DR: T2I-Diff是一个fMRI生成框架，通过时频表示和去噪扩散模型解决传统生成模型在合成fMRI数据时忽略BOLD信号非平稳性和非线性动态的问题。
...
Method: 将BOLD信号通过时变傅里叶变换转换为窗谱图，使用无分类器去噪扩散模型生成类条件频率谱图，再通过逆傅里叶变换还原为BOLD信号。

Result: 在下游fMRI脑网络分类任务中表现出更高的准确性和泛化能力。

Conclusion: T2I-Diff框架能有效生成高质量的fMRI数据，提升下游分析任务的性能。
...
TL;DR: CaTS-Bench是第一个大规模、真实世界的时间序列描述基准，包含465k训练和105k测试样本，每个样本包含数值序列、元数据、线图图像和描述。该工作还提出了可扩展的描述生成流程和新的评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列描述基准通常依赖合成数据或过于简化的描述，且忽视元数据和视觉表示，需要建立更全面、真实世界的基准。

Method: 从11个多样化数据集构建基准，采用可扩展的描述生成流程：使用oracle LLM生成描述，并通过事实检查、人类不可区分性研究和多样性分析验证，同时提供579个人工修订的测试描述。

Result: 建立了包含460个多选题的基准，用于评估时间序列推理的深层方面，并对领先的VLM进行了基准测试，揭示了其优势和局限性。

Conclusion: CaTS-Bench及其描述生成流程为时间序列分析与基础模型交叉研究提供了可靠且可扩展的基础。
...
TL;DR: 该论文通过神经坍缩理论统一解释了深度学习中的晚期现象（如grokking和信息瓶颈），揭示了类内方差收缩是这些现象的关键机制，并通过不同时间尺度的动态分析解释了训练集拟合与神经坍缩进展之间的关系。


<details>
  <summary>Details</summary>
Motivation: 深度学习训练动态中存在一些违反直觉的现象，如grokking（测试性能在训练损失平稳后突然提升）和信息瓶颈原则（模型逐步丢弃与预测任务无关的输入信息）。这些现象背后的机制及其相互关系尚未得到充分理解。

Method: 通过神经坍缩理论的视角分析学习表示的几何特性，研究类内方差收缩的动态过程，并将训练集上的神经坍缩度量与测试性能关联起来。在多个数据集和架构上进行理论验证。

Result: 发现类内方差收缩是grokking和信息瓶颈现象的共同关键因素，不同时间尺度（训练集拟合与神经坍缩进展）的动态差异解释了晚期现象的行为模式。

Conclusion: 神经坍缩理论为深度学习中的晚期训练现象提供了统一解释框架，揭示了表示几何特性在模型泛化中的重要作用，有助于更好地理解深度学习的训练动态。
...
TL;DR: 本文提出了一种两阶段训练框架来解决多模态融合中的模态竞争问题，通过单模态训练塑造初始状态，引入有效竞争强度(ECS)概念，并开发了基于互信息和部分信息分解的可计算诊断指标和异步训练控制器。


<details>
  <summary>Details</summary>
Motivation: 多模态融合在联合训练中经常遭受模态竞争问题，其中一个模态主导学习过程，导致其他模态优化不足。现有方法大多在联合学习阶段解决此问题，忽视了模型初始状态的关键影响。

Method: 1) 提出有效竞争强度(ECS)概念量化模态竞争力；2) 开发基于互信息和部分信息分解的可计算诊断指标FastPID；3) 设计异步训练控制器动态平衡模态并定位理想初始状态；4) 采用两阶段训练框架：先单模态训练塑造初始状态，再进行联合训练。

Result: 在多个基准测试上的实验表明，该方法实现了最先进的性能，证明了塑造预融合模型初始状态是缓解模态竞争的有效策略。

Conclusion: 通过单模态训练预先塑造模型初始状态，可以有效缓解多模态融合中的模态竞争问题，可靠地实现协同多模态融合。
...
Method: MOIRA采用早期整合方法，将每个组学数据集投影到共享嵌入空间，通过可学习的加权机制进行融合，充分利用包括缺失模态在内的所有样本。

Result: 在阿尔茨海默病ROSMAP数据集上的评估显示，MOIRA优于现有方法，特征重要性分析揭示了与文献一致的AD相关生物标志物。

Conclusion: 该方法具有生物学相关性，能够有效处理多组学数据中的缺失模态问题。
...
TL;DR: 本文提出因果时间序列生成（CaTSG）作为新的TSG任务家族，基于Pearl因果阶梯框架，扩展了传统条件生成模型，支持干预和反事实生成。


<details>
  <summary>Details</summary>
Motivation: 现有条件时间序列生成模型仅学习观测相关性，未考虑未观测混杂因素，缺乏对因果关系的建模能力。

Method: 开发了CaTSG统一扩散框架，采用后门调整引导，通过因果得分函数和后门调整、溯因-行动-预测程序实现因果引导采样。

Result: 在合成和真实数据集上的实验表明，CaTSG在保真度方面表现优异，并能处理现有基线无法支持的干预和反事实生成任务。

Conclusion: 提出了因果TSG任务家族并通过CaTSG实现概念验证，为在干预和反事实场景下实现更可靠的模拟开辟了有前景的研究方向。
...
TL;DR: 本文提出了一种基于掩码变换器自编码器的方法，用于重建缺失的胎儿心率信号，以解决可穿戴设备监测中数据丢失问题，支持AI风险预测算法的开发。


<details>
  <summary>Details</summary>
Motivation: 大约10%的新生儿需要呼吸辅助，5%需要通气支持。可穿戴FHR监测设备在孕妇活动时容易出现信号丢失，传统插值方法无法保持信号频谱特征，限制了AI分析的有效性。

Method: 使用掩码变换器自编码器方法，通过捕捉数据的空间和频率成分来重建缺失的FHR信号，该方法对不同程度的缺失数据具有鲁棒性，可用于信号修复和预测。

Result: 提出的方法能够有效重建缺失的FHR信号，保持信号的频谱特性，适用于不同持续时间的缺失数据情况。

Conclusion: 该方法可应用于回顾性研究数据集，支持AI风险算法开发，未来有望集成到可穿戴FHR监测设备中，实现更早、更稳健的风险检测。
...
TL;DR: 提出了联邦马尔可夫插补（FMI）方法，用于解决电子健康记录联邦学习中因不同机构时间序列数据采集粒度不一致导致的数据缺失问题。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在电子健康记录应用中面临数据缺失的持续挑战，特别是当不同机构以不同时间粒度收集时间序列数据时，需要一种隐私保护的时序插补方法。

Method: FMI方法允许重症监护病房（ICUs）协作构建全局转移模型进行时序插补，同时保护隐私。

Result: 在MIMIC-IV数据集上的脓毒症发病预测任务评估显示，FMI优于局部插补基线方法，尤其在ICU间采样间隔不规则的情况下表现更佳。

Conclusion: FMI方法能有效解决联邦学习中因时间粒度不一致导致的数据缺失问题，在真实医疗场景中展现出优越性能。
...
TL;DR: StyleBench是一个系统性评估LLM推理风格的基准测试，通过分析5种推理风格在15个开源模型上的表现，发现推理策略的有效性高度依赖于模型规模和任务类型。
...
Method: 构建StyleBench基准，评估5种代表性推理风格（CoT、ToT、AoT、SoT、CoD）在5个推理任务上，使用15个开源模型（270M-120B参数）。

Result: 没有单一最优推理风格：搜索方法在开放性问题表现好但需要大模型，简洁风格在定义明确任务上效率高；小模型常无法遵循输出指令，推理鲁棒性随规模增长。
...
TL;DR: 本文提出了一种处理随机传感器延迟的模型强化学习框架，通过基于模型的滤波过程来更新信念状态，使智能体能够有效处理观察延迟问题。


<details>
  <summary>Details</summary>
Motivation: 现实环境中经常出现延迟，但标准强化学习算法通常假设环境的瞬时感知。随机传感器延迟在POMDP中尚未得到充分研究，特别是观察可能乱序到达的情况。

Method: 提出基于模型的滤波过程，顺序更新信念状态；引入简单的延迟感知框架，将该思想融入基于模型的强化学习中；在Dreamer算法上应用该框架。

Result: 该方法在延迟感知基线方法上表现一致优于，对部署期间的延迟分布变化具有鲁棒性；在模拟机器人任务实验中优于常见实用启发式方法。

Conclusion: 明确建模观察延迟的重要性得到验证，提出的框架能够有效处理随机传感器延迟问题。
...
TL;DR: 本文提出了一种联邦学习中的客户端选择方法，通过控制标签分布来缓解数据不平衡问题，分别使用平衡分布和联邦组合标签分布作为目标分布，并在不同FL策略和数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中客户端数据不平衡会降低共享模型性能，需要改进客户端选择方法来减轻数据不平衡的负面影响。

Method: 扩展现有FL策略，选择能最好地将当前标签分布与两个目标分布（平衡分布或联邦组合标签分布）对齐的活跃客户端。

Result: 实验表明，面对局部不平衡时，与平衡分布对齐效果最好；面对全局不平衡时，与联邦组合标签分布对齐更优。

Conclusion: 提出的分布控制客户端选择方法能有效改善联邦学习在数据不平衡场景下的性能，不同目标分布适用于不同类型的失衡问题。
...
TL;DR: 提出了一种联邦学习框架下的注意力增强LSTM模型，用于脓毒症发病预测，支持可变预测窗口，在多中心ICU数据上训练，在保护隐私的同时提高了早期检测性能。


<details>
  <summary>Details</summary>
Motivation: 解决ICU中脓毒症早期准确预测的挑战，同时克服单个医院数据量有限的问题，通过联邦学习实现多机构协作训练而不共享数据。

Method: 使用联邦学习框架，结合注意力机制的LSTM模型，支持可变预测窗口（短期和长期预测），在多中心ICU数据上进行训练。

Result: 联邦学习不仅提高了整体预测性能（接近集中式模型），而且特别有利于脓毒症早期发病预测，可变预测窗口设计减少了计算和通信开销。

Conclusion: 该方法在保护患者隐私的前提下，有效提高了脓毒症的早期检测能力，为临床实践提供了实用的预测工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有的离散扩散模型使用随机去噪过程，本文旨在开发确定性反向过程以提升效率和样本质量，将连续扩散中成熟的确定性方法扩展到离散状态空间。

Method: 使用马尔可夫链构建确定性去噪算法，引入弱混沌动力学的herding算法变体来诱导确定性离散状态转换，直接替代随机去噪过程。

Result: 在文本和图像生成任务上实现了效率和样本质量的一致提升，证明了确定性反向过程在离散状态空间中的有效性。

Conclusion: 这种简单的确定性化方法有望增强离散扩散在生成建模中的重要性，同时揭示了确定性反向过程在离散状态空间中的潜力。
...
TL;DR: 开发深度学习框架，结合微观移动性特征、历史犯罪和社会人口统计数据，提升精细时空分辨率下的犯罪预测性能。


<details>
  <summary>Details</summary>
Motivation: 探索微观移动性特征与历史犯罪和社会人口统计数据的结合是否能增强精细时空分辨率下的犯罪预测性能。

Method: 使用四个美国城市的犯罪事件数据、社会人口统计数据和人类移动数据，构建ConvLSTM深度学习模型，预测未来12小时的犯罪发生，并与逻辑回归、随机森林和标准LSTM基线模型进行比较。

Result: 结合移动性特征提高了预测性能，尤其是在使用较短输入序列时。移动性和社会人口统计特征共同使用时效果最佳，深度学习模型在所有四个城市中均获得最高召回率、精确率和F1分数。

Conclusion: 研究结果强调了整合多样化数据源（包括移动性数据）对于时空犯罪预测的重要性，并凸显了深度学习在处理精细时空尺度时的优势和局限性。
...
<details>
  <summary>Details</summary>
Motivation: 提高重型土方设备（特别是挖掘设备）中线性执行器的能效，减少能量损失和环境影响。

Method: 使用MATLAB/Simulink进行液压回路仿真，比较PDCV和PFCV两种方案。PFCV通过在执行器两端设置人工泄漏来减少能量损失，而PDCV使用压力安全阀。位置控制采用模糊控制器调谐的PID控制器。
...
TL;DR: GenFacts是一个基于类别判别变分自编码器的生成框架，用于为多元时间序列生成反事实解释，通过整合对比学习和分类一致性目标、原型初始化和真实性约束优化，在真实性和可解释性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法为多元时间序列生成的反事实解释往往无效、不真实或不直观，需要一种能产生更真实和用户可理解的反事实解释的方法。

Method: 基于类别判别变分自编码器的生成框架，整合对比学习和分类一致性目标、原型初始化和真实性约束优化。

Result: 在手势雷达数据和手写字母轨迹数据集上，GenFacts在真实性方面优于现有基线方法18.7%，在人类研究中获得最高的可解释性评分。

Conclusion: 真实性和用户中心的可解释性（而非仅仅稀疏性）是时间序列数据中可操作反事实解释的关键因素。
...
TL;DR: 本文研究发现Transformer在时间序列预测中表现不佳的原因在于注意力机制失效，Transformer块常常退化为简单的MLP，主要原因是当前嵌入方法无法让Transformer在结构良好的潜在空间中工作。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer在NLP和CV领域表现出色，但在时间序列预测中甚至不如简单的线性基线。大多数研究没有深入探讨Transformer失败的原因，本文旨在系统分析时间序列Transformer(TST)的失效机制。

Method: 设计了一系列渐进式实验，将Transformer逐步修改为MLP来研究注意力机制的影响；创建了可解释的数据集来研究注意力机制失效的原因；进行了理论分析探讨嵌入方法失败的根本原因。

Result: 发现Transformer块在现有时间序列Transformer中常常退化为简单MLP；注意力机制没有按预期方式工作；当前嵌入方法无法让Transformer在结构良好的潜在空间中有效运作。

Conclusion: 时间序列Transformer表现不佳的根本原因在于嵌入方法的缺陷，导致注意力机制失效，Transformer无法充分利用其架构优势。
...
TL;DR: 本文提出了一种解耦值注意力机制（DVA），用于改进先验数据拟合网络（PFNs）在高维回归任务中的性能，使其能够更有效地替代高斯过程进行快速物理系统建模。


<details>
  <summary>Details</summary>
Motivation: 标准Transformer注意力在PFNs中处理高维回归任务时效果有限，而高斯过程具有函数空间完全由输入核函数表征且预测均值是训练目标加权和的特性，这启发了DVA的设计。

Method: 提出DVA注意力机制，仅从输入计算相似度，通过值传播标签，镜像高斯过程更新但保持无核特性。研究表明注意力规则而非架构本身是扩展PFNs的关键因素。

Result: 局部化注意力在不同维度设置下显著降低PFNs的样本外验证损失，5维和10维情况下验证损失降低超过50%。CNN-based PFNs性能可与Transformer-based相媲美。64维功率流方程近似平均绝对误差达到1E-3量级，比精确GP推理快80倍以上。

Conclusion: DVA注意力机制是提升PFNs性能的关键，能够有效替代计算密集型的高斯过程推理，为高维物理系统建模提供快速准确的解决方案。
...
TL;DR: 本文分析了流匹配方法在低噪声区域存在的不稳定性问题，提出了局部对比流（LCF）作为解决方案，通过结合对比学习来改善表示质量。


<details>
  <summary>Details</summary>
Motivation: 流匹配框架在低噪声区域存在根本性不稳定问题，当噪声水平接近零时，输入中的微小扰动会导致速度目标的大幅变化，造成学习问题的条件数发散，这不仅减缓优化速度，还迫使编码器将有限的雅可比容量重新分配到噪声方向，从而降低语义表示质量。

Method: 提出了局部对比流（LCF），这是一种混合训练协议，在小噪声水平下用对比特征对齐取代直接速度回归，同时在中等和高噪声水平下保留标准流匹配。

Result: 实验证明LCF不仅提高了收敛速度，还稳定了表示质量。
...
TL;DR: MixGate是一个多视图布尔电路学习框架，通过功能对齐预处理解决不同电路表示（如AIG和XMG）之间的结构异质性，使掩码建模能够有效利用互补信息。


<details>
  <summary>Details</summary>
Motivation: 多视图学习在布尔电路上具有巨大潜力，但不同视图（如AIG与XMG）之间的结构异质性阻碍了有效融合，特别是对于自监督技术如掩码建模，交叉视图上下文会被视为噪声。

Method: 提出MixGate框架，采用分阶段训练策略：首先通过等价对齐损失学习共享的功能感知表示空间，然后引入多视图掩码建模目标来利用对齐视图的互补信号。

Result: 广泛的实验（包括关键消融研究）表明，对齐优先策略将掩码建模从无效技术转变为强大的性能驱动因素。

Conclusion: 功能对齐是多视图自监督学习的关键前提条件，能够解锁多视图学习的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 个性化医疗需要优化患者治疗方案，但无法随意测试治疗效果，现有替代模型对新患者-治疗组合泛化能力差。

Method: LEON方法通过'提示优化'实现，将LLM作为随机引擎提出治疗设计，利用医学教科书和生物医学知识图谱等先验知识。
...
TL;DR: 本文提出了CLUE框架，通过电路发现技术识别LLM中负责遗忘和保留的神经元，并针对不同类别神经元进行精确微调，解决了现有方法中神经元纠缠的问题。
...
Method: 采用电路发现机制解释技术，识别遗忘和保留电路，将电路转换为合取范式，通过可满足性解确定每个神经元应被遗忘还是保留，并为不同类别神经元提供针对性微调策略。

Result: 大量实验表明，与现有定位方法相比，CLUE通过精确的神经定位实现了更优的遗忘效果和保留效用。

Conclusion: CLUE框架有效解决了LLM遗忘中的神经元纠缠问题，为精确控制模型行为提供了新思路。
...
TL;DR: FracAug是一个创新的图级异常检测增强框架，通过生成语义一致的图变体和互验证伪标签来提升GNN性能，解决数据不平衡和标注成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 图级异常检测在药物发现等领域至关重要，但高标注成本和数据集不平衡限制了图神经网络的性能表现。

Method: FracAug通过学习图内语义并合成分数变体，采用加权距离感知边缘损失来生成多样化的语义保持图，然后利用原始图和增强图的预测结果对未标记数据进行伪标注，迭代扩展训练集。

Result: 在12个真实数据集上的14种GNN模型实验显示，FracAug将平均AUROC、AUPRC和F1分数分别提升了最高5.72%、7.23%和4.18%。
...
TL;DR: LCR是一个实用的基于学习的GPU缓存框架，通过LARU算法增强LRU缓存策略，结合机器学习预测并动态适应预测精度，在推荐模型和大型语言模型中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现代GPU推理中缓存效率是主要瓶颈，传统启发式策略如LRU在结构化访问模式下表现不佳，而现有学习方法要么预测不准确时性能急剧下降，要么因保守设计而收益有限。

Method: 提出LCR框架，核心算法LARU在LRU基础上集成机器学习预测，通过在线误差估计动态适应预测精度，实现预测准确时接近最优性能，预测不准确时优雅退化到接近LRU性能。

Result: 实验显示LCR在DLRM和LLM场景中，吞吐量提升最高达24.2%，P99 TTFT降低最高达28.3%，即使在预测不佳时性能仍保持稳定。

Conclusion: LCR成功弥合了基于学习的缓存方法在实证进展和理论进展之间的差距，提供了实用且稳健的性能提升。
...
TL;DR: 本文提出了一种基于伪似然最大化的方法，用于从单个样本估计截断伊辛模型的逆温度参数β，在约束条件下实现了O(Δ³/√n)的一致性估计。


<details>
  <summary>Details</summary>
Motivation: 在统计物理和机器学习中，截断伊辛模型的参数估计是一个重要问题。现有方法在处理截断约束时面临挑战，特别是在只使用单个样本的情况下。

Method: 基于伪似然最大化方法，将截断集S表示为k-SAT公式的满足解集，利用图G的邻接矩阵A构建估计器，在近O(n)时间内完成计算。

Result: 在k ≳ log(d²k)Δ³的条件下，估计器β̂与真实参数β*具有O(Δ³/√n)的一致性，适用于有界度Δ的图结构。

Conclusion: 该方法成功地将伪似然最大化技术扩展到截断伊辛模型，为单样本参数估计提供了有效的解决方案，推广了现有技术到更具挑战性的设置。
...
TL;DR: 本文提出了一种名为二元自编码器（BAE）的新方法，通过施加最小熵约束来提升LLM隐藏状态中特征的稀疏性和独立性，解决了现有方法特征密集化的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在解构LLM隐藏状态中的数值化特征时，通常依赖带有隐式正则化的自编码器，缺乏对全局稀疏性的显式保证，导致大量密集特征，损害了特征的稀疏性和原子化程度。

Method: 提出二元自编码器（BAE），通过步进函数将隐藏激活离散化为1位，应用梯度估计实现反向传播，在隐藏激活的小批量上施加最小熵约束，促进特征跨实例的独立性和稀疏性。

Result: BAE在特征集熵计算和特征解缠两方面表现出色：能可靠估计二元隐藏激活的熵来表征LLM推理动态；相比基线方法，BAE避免了密集特征，产生了最多可解释特征。

Conclusion: BAE作为一种特征提取器具有有效性，通过最小熵约束成功提升了LLM隐藏状态特征的稀疏性和可解释性。
...
TL;DR: 本文提出了一种基于局部唯一标识符（Local-UID）的ColorGNN方法，解决了传统图神经网络在整数线性规划问题中表达能力有限和泛化能力差的问题。


<details>
  <summary>Details</summary>
Motivation: 传统匿名GNN在ILP问题上表达能力有限，而全局唯一标识符（Global-UID）方法虽然提高了表达能力，但会引入虚假相关性，严重损害模型的泛化能力。

Method: 提出了基于d跳唯一性着色的Local-UID方案，确保标识符仅在节点的d跳邻域内唯一。在此基础上开发了ColorGNN（通过颜色条件嵌入整合颜色信息）和轻量级变体ColorUID。

Result: 实验表明该方法在三个ILP基准测试上取得显著提升，在线性规划数据集上表现出强大的OOD泛化能力，并且与最先进方法结合时能进一步提升图级任务的性能。

Conclusion: Local-UID方案在保持Global-UID表达能力的同时提供了更强的泛化能力，为ILP优化问题提供了有效的解决方案。
...
TL;DR: 本文提出将无损压缩作为评估时间序列模型的新范式，基于香农的源编码定理建立压缩长度与负对数似然的等价关系，提供了一个严格统一的信息论建模能力评估标准。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列模型评估主要关注预测、插补、异常检测和分类等任务特定性能，但未能严格衡量模型是否捕获数据的完整生成分布。

Method: 引入无损压缩作为评估范式，定义标准化评估协议和指标，开发开源评估框架TSCom-Bench，使时间序列模型能够快速适配为无损压缩的骨干网络。
...
TL;DR: 本文提出了一种名为MAIFormer的多智能体反转变压器，用于预测多架飞机的飞行轨迹。该框架通过掩码多变量注意力捕捉个体飞机的时空模式，通过智能体注意力建模复杂空中交通场景中的社会交互模式。


<details>
  <summary>Details</summary>
Motivation: 多智能体飞行轨迹预测对于理解当前空中交通流至关重要，但面临两大挑战：一是需要同时建模个体飞机的时间行为和航班间的复杂交互，二是需要生成可解释的预测结果。

Method: MAIFormer采用两种关键注意力模块：掩码多变量注意力（捕捉个体飞机的时空模式）和智能体注意力（建模多智能体间的社会模式）。

Result: 在仁川国际机场终端空域的真实ADS-B飞行轨迹数据集上的实验表明，MAIFormer在多个指标上均达到最佳性能，优于其他方法。
...
TL;DR: 本文通过信息移除的新视角研究了上下文学习（ICL）的内在机制，发现在零样本场景下语言模型会将查询编码为包含所有可能任务信息的非选择性表示，导致任意输出。通过低秩滤波器选择性移除隐藏状态中的特定信息可以有效引导模型朝向目标任务。
...
Method: 通过设计度量指标测量隐藏状态，观察少样本ICL如何模拟任务导向的信息移除过程，并识别出诱导移除操作的关键注意力头（去噪头）。

Result: 研究发现ICL通过选择性移除纠缠的非选择性表示中的冗余信息来改善输出，阻断信息移除操作会显著降低ICL准确率，特别是在正确标签缺失的情况下。

Conclusion: 信息移除机制是ICL的关键机制，去噪头在诱导这一过程中发挥关键作用，这为理解ICL的工作原理提供了新的理论视角。
...
TL;DR: rBridge是一种利用小模型（≤1B参数）预测大模型推理能力的方法，通过将预训练目标与目标任务对齐，使用前沿模型的推理轨迹作为黄金标签来加权负对数似然，显著降低数据集排名成本100倍以上。


<details>
  <summary>Details</summary>
Motivation: 由于预训练大语言模型的成本过高，需要利用小代理模型来优化数据集，但推理能力通常只在超过7B参数的大模型中可靠出现，这使得小模型难以有效预测大模型的推理表现。

Method: rBridge通过加权负对数似然与任务对齐，使用前沿模型的推理轨迹作为黄金标签，使小代理模型能够更准确地预测大模型的推理能力。

Result: rBridge在实验中：(i) 相对于最佳基线将数据集排名成本降低100倍以上；(ii) 在1B到32B规模的六个推理基准测试中实现最强相关性；(iii) 在1B到7B规模上实现跨预训练数据集的零样本预测关系迁移。
...
TL;DR: DELTA-Code是一个用于评估LLMs学习新推理策略能力的基准测试，通过算法编码问题探究模型的可学习性和可迁移性，发现了明显的顿悟现象。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs是否能真正学习和泛化新的推理策略这一开放性问题，超越预训练中已编码的技能。

Method: 使用DELTA-Code基准测试，通过模板化问题生成器隔离推理技能，采用强化学习训练，探索分阶段预热、经验回放、课程训练等关键训练要素。

Result: 实验发现明显的顿悟相变现象：在长期零奖励后，RL训练模型突然达到接近完美的准确率；在家族内和重组技能上表现良好，但在转换性案例中存在持续弱点。

Conclusion: DELTA为探索RL驱动推理的极限提供了一个清晰的测试平台，帮助理解模型如何超越现有先验知识来获取新的算法技能。
...
TL;DR: 提出了Ensemble Conditional Independence Test (E-CIT)框架，通过分治策略将数据划分为子集并行进行条件独立性测试，然后使用基于稳定分布特性的新方法聚合p值，显著降低了计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 基于约束的因果发现需要大量条件独立性测试(CITs)，但CITs的计算成本极高，特别是在样本量大的情况下，这严重限制了其实际应用。
...
Conclusion: 实验结果表明E-CIT不仅显著降低了CITs和因果发现的计算负担，而且在复杂测试场景下（特别是真实世界数据集）表现出竞争优势。
...
TL;DR: 本文提出了一种名为ACA（Actor-Critic without Actor）的轻量级强化学习框架，通过消除显式的行动者网络，直接从噪声级评论家的梯度场生成动作，简化了传统行动者-评论家方法的复杂性。


<details>
  <summary>Details</summary>
Motivation: 传统的行动者-评论家方法依赖独立的行动者和评论家网络，训练过程对架构决策和超参数调整敏感，限制了在大规模函数逼近器设置中的可扩展性。扩散模型虽然能表达多模态行为，但引入了额外的设计选择和计算负担。

Method: ACA框架消除了显式行动者网络，通过噪声级评论家的梯度场直接生成动作。这种方法避免了行动者训练的计算开销，同时保持策略改进与评论家最新价值估计的紧密对齐。

Result: 在标准在线强化学习基准测试中，ACA相比传统行动者-评论家方法和最先进的基于扩散的方法，实现了更有利的学习曲线和竞争性性能。

Conclusion: ACA提供了一个简单而强大的在线强化学习解决方案，结合了简洁性和表达能力，无需依赖基于扩散的行动者就能捕捉多样化的多模态行为。
...
### [166] [Reinforcement Learning Fine-Tuning Enhances Activation Intensity and Diversity in the Internal Circuitry of LLMs](https://arxiv.org/abs/2509.21044)
*Honglin Zhang,Qianyue Hao,Fengli Xu,Yong Li*

Main category: cs.LG

TL;DR: 本研究通过边缘归因修补技术分析RL微调对LLMs内部机制的影响，发现在线RL训练会增强激活强度和多样性，而DPO方法的效果较弱或不一致。


<details>
  <summary>Details</summary>
Motivation: 尽管RL微调被证明能提升LLM能力，但其对不同LLM内部机制的具体影响机制尚不明确，需要系统研究。

Method: 使用边缘归因修补技术对比分析多个模型家族在RL微调前后的内部差异，重点关注激活强度和模式多样性。

Result: 在线RL训练导致激活强度整体增加和激活模式多样性提升，而DPO方法的效果明显弱于PPO和GRPO方法。
...
TL;DR: 本文分析了多模态大语言模型（MLLMs）中视觉越狱攻击的局限性，发现现有攻击方法存在高锐度区域问题，导致跨模型迁移性差。作者提出FORCE方法，通过纠正特征过度依赖问题，提升攻击的跨模型迁移能力。


<details>
  <summary>Details</summary>
Motivation: 当前简单的视觉越狱攻击虽然能有效操纵开源MLLMs，但在闭源模型上表现不佳，缺乏跨模型迁移性。这限制了红队评估的有效性，需要深入理解攻击失效的原因。

Method: 提出FORCE方法：1）分析损失景观，发现攻击位于高锐度区域；2）分析中间层和频谱域特征表示，揭示对窄层表示和语义贫乏频率分量的过度依赖；3）引导攻击探索更广泛的可行区域，根据语义内容重新缩放频率特征的影响。

Result: 实验表明，FORCE方法能有效发现平坦的可行区域，显著提升视觉越狱攻击的跨模型迁移性，为闭源MLLMs的红队评估提供了有效工具。

Conclusion: 通过消除对层和频谱特征的非泛化依赖，FORCE方法解决了视觉越狱攻击的迁移性问题，为多模态模型的安全性评估提供了新思路。
...
### [168] [GeoRef: Referring Expressions in Geometry via Task Formulation, Synthetic Supervision, and Reinforced MLLM-based Solutions](https://arxiv.org/abs/2509.21050)
...
Method: 构建GeoRef基准数据集，使用结构化几何形式语言生成大规模合成训练数据，采用SFT和GRPO两种微调方法，并提出验证-再生机制来纠正错误预测。

Result: GRPO方法显著优于SFT，能更好地将模型行为与任务特定奖励对齐。经过GeoRef训练的模型在下游几何推理任务上表现出可测量的改进。

Conclusion: 即使是先进的MLLM模型在该任务上仍有困难，表明需要显式评估和加强几何基础能力作为稳健几何问题求解的前提条件。
...
TL;DR: 基于物理学中的最小作用量原理，从第一性原理推导了经典学习算法、强化学习中的贝尔曼最优方程以及生成模型中的Adam优化器，提出了学习拉格朗日量的概念，认为学习是在拉格朗日量中寻找平稳路径的过程。


<details>
  <summary>Details</summary>
Motivation: 研究如何构建高效学习系统，即在最短时间内达到期望误差阈值的学习过程，借鉴物理学中的最小作用量原理来统一解释不同学习算法。

Method: 提出学习拉格朗日量的概念，将学习过程视为在拉格朗日量中寻找平稳路径的变分问题，从第一性原理推导各种学习算法。

Result: 成功从统一的理论框架推导出了经典学习算法、强化学习中的贝尔曼方程和生成模型中的Adam优化器。

Conclusion: 学习算法可以通过寻求拉格朗日量中的平稳轨迹来推导，这为理解不同学习算法提供了统一的物理基础。
...
### [170] [ScaleDiff: Scaling Difficult Problems for Advanced Mathematical Reasoning](https://arxiv.org/abs/2509.21070)
*Qizhi Pei,Zhuoshi Pan,Honglin Lin,Xin Gao,Yu Li,Zinan Tang,Conghui He,Rui Yan,Lijun Wu*

Main category: cs.LG

TL;DR: ScaleDiff是一个简单有效的管道，用于大规模创建困难的数学问题。它通过自适应思维模型识别现有数据集中的难题，然后训练专门的难题生成器，无需复杂提示即可大规模生成新难题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在扩展数学问题生成方面面临挑战，包括高计算/API成本、复杂提示以及生成问题难度有限。ScaleDiff旨在克服这些限制，实现困难问题的大规模创建。

Method: 1) 使用自适应思维模型通过单次前向传播识别现有数据集中的难题；2) 在筛选出的困难数据上训练专门的难题生成器(DiffGen-8B)；3) 使用成本效益高的Qwen3-8B作为教师模型进行知识蒸馏。

Result: 在ScaleDiff-Math数据集上微调Qwen2.5-Math-7B-Instruct，相比原始数据集性能提升11.3%，在多个数学基准测试上达到65.9%的平均准确率，优于OpenThinker3等强模型。

Conclusion: ScaleDiff证明了无需依赖更大、更昂贵的教师模型即可有效传递高级推理能力，并且在困难基准测试中观察到随着难题数量增加而出现的明显扩展现象。
...
### [171] [TyphoonMLA: A Mixed Naive-Absorb MLA Kernel For Shared Prefix](https://arxiv.org/abs/2509.21081)
...
Method: 提出TyphoonMLA混合方法：对注意力计算的计算密集型部分使用naive公式来利用共享前缀，对非共享部分使用absorb公式来降低带宽需求。

Result: 在NPU和GPU上分别实现了3倍和3.24倍的注意力计算吞吐量提升，HBM大小仅增加3%的开销。

Conclusion: TyphoonMLA通过巧妙结合两种注意力公式的优势，有效解决了MLA架构在解码阶段的性能瓶颈问题。
...
### [172] [SPREAD: Sampling-based Pareto front Refinement via Efficient Adaptive Diffusion](https://arxiv.org/abs/2509.21058)
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模昂贵多目标优化问题中高效计算帕累托最优妥协解集的关键挑战

Method: 首先学习决策空间采样点的条件扩散过程，然后在每个反向扩散步骤中通过自适应多梯度下降更新进行快速收敛，并使用高斯RBF排斥项保持多样性

Result: 在多目标优化基准测试中，SPREAD在效率、可扩展性和帕累托前沿覆盖方面达到或超过领先基线方法

Conclusion: SPREAD框架为大规模昂贵多目标优化问题提供了一种有效的解决方案
...
### [173] [Structure-Attribute Transformations with Markov Chain Boost Graph Domain Adaptation](https://arxiv.org/abs/2509.21059)
*Zhen Liu,Yongtao Zhang,Shaobo Ren,Yuxin You*

Main category: cs.LG

TL;DR: SATMC是一种新颖的图域自适应框架，通过图结构和属性转换来顺序对齐网络间的分布，解决了传统方法在结构异质性方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统图域自适应方法主要关注节点属性转换和对齐，但往往难以处理不同图域之间的结构异质性，导致分布对齐效果不佳。

Method: 提出SATMC框架，通过马尔可夫链进行结构和属性转换，引入私有域信息减少机制和经验Wasserstein距离来增强模型泛化能力。

Result: 在9对公开跨域数据集上的实验表明，SATMC在跨网络节点分类任务中优于现有最先进方法。

Conclusion: 理论证明SATMC能获得更紧的跨网络节点分类误差界，实验验证了其有效性，代码已开源。
...
TL;DR: GraphUniverse是一个用于生成图家族以系统性评估归纳泛化能力的框架，解决了现有方法局限于单图转导设置的不足。


<details>
  <summary>Details</summary>
Motivation: 图学习中的一个基本挑战是理解模型如何泛化到新的未见图。现有方法局限于单图转导设置，无法评估模型在全新图结构上的归纳泛化能力。

Method: 通过生成具有持久语义社区的图家族，确保概念一致性同时允许对同质性、度分布等结构属性进行细粒度控制，实现受控分布偏移下的鲁棒性测试。

Result: 对多种架构的基准测试表明，强转导性能不能预测归纳泛化能力，且对分布偏移的鲁棒性高度依赖于模型架构选择和初始图机制。
...
Conclusion: VARL的优势使其成为在线强化学习的通用框架，使得在真实世界环境中直接从零开始应用强化学习变得可行。
...
### [176] [LAVA: Explainability for Unsupervised Latent Embeddings](https://arxiv.org/abs/2509.21149)
*Ivan Stresec,Joana P. Gonçalves*

Main category: cs.LG

TL;DR: LAVA是一种后处理、模型无关的方法，用于解释无监督黑盒模型的局部嵌入结构，通过分析原始特征相关性来揭示潜在空间中的重复模式。


<details>
  <summary>Details</summary>
Motivation: 无监督黑盒模型在科学发现中很重要但难以解释，现有解释方法要么过于细粒度要么过于简化，无法有效揭示潜在空间的组织结构。
...
Conclusion: LAVA提供了一种有效的方法来解释无监督学习的局部嵌入组织，有助于理解黑盒模型的内部结构并促进科学发现。
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络在图像分类任务中表现出色但容易受到精心设计的对抗性扰动攻击。本研究旨在探索简单、数据适应的防御方法，重新审视线性降维技术作为前端特征提取器的潜力。

Method: 使用稀疏主成分分析(SPCA)和标准PCA作为前端特征提取器，结合下游分类器进行实证比较。理论分析包括为SPCA特征上的线性头推导精确的鲁棒性证书，以及通过Lipschitz组合论证分析稀疏性对非线性头的影响。

Result: 实验表明，在强白盒和黑盒攻击下，SPCA比PCA表现出更优雅的性能下降，同时保持竞争力的干净准确率。理论分析确认了稀疏投影减少对抗性杠杆的机制。

Conclusion: 稀疏主成分分析(SPCA)是一种有效的对抗性防御方法，其理论机制（稀疏投影降低对抗性杠杆）在实际非线性设置中仍然有效，为简单实用的防御策略提供了新思路。
...
### [178] [GRPO is Secretly a Process Reward Model](https://arxiv.org/abs/2509.21154)
*Michael Sullivan*

Main category: cs.LG

TL;DR: 该论文证明GRPO强化学习算法在特定条件下会诱导出非平凡的过程奖励模型，并提出了λ-GRPO改进算法来缓解原算法的缺陷，实验表明新算法能提高模型性能且训练成本几乎不变。


<details>
  <summary>Details</summary>
Motivation: 研究GRPO算法中隐藏的过程奖励模型结构，探索是否可以利用这种内置结构来替代昂贵的显式定义过程奖励模型，从而提升模型性能而不增加训练成本。

Method: 首先理论证明GRPO算法在特定序列重叠假设下会诱导出非平凡PRM，然后通过实验验证这些假设在现实条件下成立。基于GRPO-as-a-PRM框架，识别出GRPO目标的缺陷，并提出λ-GRPO改进算法。

Result: 实验表明λ-GRPO训练的LLM在验证准确率和下游推理任务上表现优于标准GRPO，且达到峰值性能更快。同时证明可以利用GRPO内置的PRM结构来替代昂贵的显式PRM。

Conclusion: 研究表明GRPO算法本身就包含有效的PRM结构，通过简单的λ-GRPO改进就能显著提升性能，这对使用昂贵显式PRM的方法提出了质疑。
...
TL;DR: EvoMail是一个自进化的认知代理框架，通过构建统一异构邮件图、认知图神经网络和对抗性自进化循环，实现强大的垃圾邮件和钓鱼攻击检测。


<details>
  <summary>Details</summary>
Motivation: 现代垃圾邮件和钓鱼攻击已超越传统关键词黑名单或简单启发式方法，攻击者使用多模态策略结合自然语言文本、混淆URL、伪造头部和恶意附件，传统检测系统难以整合异构信号和持续适应。

Method: 构建统一异构邮件图融合文本内容、元数据和嵌入资源；使用认知图神经网络增强LLM进行上下文感知推理；采用对抗性自进化循环，红队生成新规避策略，蓝队从失败中学习并压缩经验到记忆模块。

Result: 在真实数据集和合成对抗变体上的广泛实验表明，EvoMail在检测准确性、对演化垃圾邮件策略的适应性和推理可解释性方面持续优于最先进的基线方法。
...
### [180] [A Unified Framework for Diffusion Model Unlearning with f-Divergence](https://arxiv.org/abs/2509.21167)
*Nicola Novello,Federico Fontana,Luigi Cinque,Deniz Gunduz,Andrea M. Tonello*

Main category: cs.LG

TL;DR: 本文提出了一个基于f-散度的统一框架用于扩散模型的机器遗忘，相比传统的MSE方法具有更好的灵活性和性能平衡。
...
Method: 构建统一的f-散度框架，允许使用任何f-散度进行遗忘训练，分析不同散度对算法收敛性和遗忘质量的影响。

Result: 提出的框架能够在激进遗忘和概念保留之间实现更好的平衡，用户可根据具体应用选择最优散度。

Conclusion: f-散度统一框架为扩散模型遗忘提供了更灵活和有效的解决方案，优于传统的MSE方法。
...
### [181] [Differential-Integral Neural Operator for Long-Term Turbulence Forecasting](https://arxiv.org/abs/2509.21196)
*Hao Wu,Yuan Gao,Fan Xu,Fan Zhang,Qingsong Wen,Kun Wang,Xiaomeng Huang,Xian Wu*

Main category: cs.LG

TL;DR: 提出DINO框架，通过微分-积分神经算子分解来改进湍流长期预测，解决现有方法在自回归预测中的误差累积和物理保真度问题。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法（特别是神经算子）在湍流长期自回归预测中表现不佳，无法同时捕捉湍流动力学的局部耗散效应和全局非局部相互作用这两种不同的数学结构。

Method: DINO框架通过并行分支显式建模不同的物理算子：局部微分算子（由约束卷积网络实现）和全局积分算子（由Transformer架构捕获数据驱动的全局核）。

Result: 在2D Kolmogorov流基准测试中，DINO显著优于最先进模型，在数百个时间步内成功抑制误差累积，在涡度场和能谱方面保持高保真度。

Conclusion: DINO为物理一致的长期湍流预测建立了新基准，其基于物理的分解赋予了模型卓越的稳定性和鲁棒性。
...
### [182] [Towards Foundation Models for Zero-Shot Time Series Anomaly Detection: Leveraging Synthetic Data and Relative Context Discrepancy](https://arxiv.org/abs/2509.21190)
*Tian Lan,Hao Duong Le,Jinbo Li,Wenjun He,Meng Wang,Chenghao Liu,Chen Zhang*

Main category: cs.LG

TL;DR: TimeRCD是一个基于相对上下文差异（RCD）预训练范式的时序异常检测基础模型，通过检测相邻时间窗口的显著差异来识别异常，在零样本场景下显著优于现有方法
...
Method: 提出RCD预训练范式，训练模型检测相邻时间窗口的差异而非重构输入；使用标准Transformer架构；构建大规模合成语料库进行预训练
...
### [183] [Tree Search for LLM Agent Reinforcement Learning](https://arxiv.org/abs/2509.21240)
*Yuxiang Ji,Ziyu Ma,Yong Wang,Guanhua Chen,Xiangxiang Chu,Liaoni Wu*

Main category: cs.LG

TL;DR: 本文提出Tree-GRPO方法，通过树搜索结构解决强化学习中稀疏监督问题，在固定token预算下增加rollout数量，并利用树结构轨迹构建过程监督信号。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于结果奖励的强化学习方法在长期多轮代理任务中面临的稀疏监督问题。

Method: 基于树搜索的分组代理强化学习方法，每个树节点代表完整的代理交互步骤，通过共享公共前缀增加rollout数量，并构建步级过程监督信号。

Result: 在11个数据集和3类QA任务上的实验表明，基于树的强化学习方法优于基于链的方法。

Conclusion: Tree-GRPO方法通过树结构和分组相对优势估计，有效解决了强化学习中的稀疏监督问题，在多种任务上表现出优越性能。
...
### [184] [CAD-Tokenizer: Towards Text-based CAD Prototyping via Modality-Specific Tokenization](https://arxiv.org/abs/2509.21150)
*Ruiyu Wang,Shizhao Sun,Weijian Ma,Jiang Bian*

Main category: cs.LG

TL;DR: 本文提出了CAD-Tokenizer框架，通过多模态标记化策略解决文本引导CAD原型设计中的语义对齐问题，显著提升了指令跟随和生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统LLM标记器将CAD序列分解为自然语言词片段，无法捕捉原始级CAD语义，阻碍了几何结构建模。CAD的序列化结构特性需要更有效的表示方法。

Method: 提出CAD-Tokenizer框架，使用基于序列的VQ-VAE，采用原始级池化和约束解码，生成紧凑的、原始感知的表示，与CAD的结构特性对齐。

Result: 在统一的文本引导CAD原型设计中，CAD-Tokenizer显著改善了指令跟随和生成质量，在定量和定性评估上都优于通用LLM和任务特定基线。

Conclusion: 多模态标记化策略能够为CAD数据提供更有效的表示，为文本到CAD生成和CAD编辑的统一流程提供了可行的解决方案。
...
### [185] [Explaining Fine Tuned LLMs via Counterfactuals A Knowledge Graph Driven Framework](https://arxiv.org/abs/2509.21241)
*Yucheng Wang,Ziyang Chen,Md Faisal Kabir*

Main category: cs.LG

TL;DR: 本文提出了一个基于知识图谱反事实解释的框架，用于解释微调后大语言模型的结构推理和语义行为变化。


<details>
  <summary>Details</summary>
Motivation: 虽然LoRA等微调技术能高效地为大语言模型注入领域知识，但理解这种微调机制如何改变模型的结构推理和语义行为仍是一个开放挑战。

Method: 构建BioToolKG生物信息学工具知识图谱，设计CFFTLLMExplainer反事实解释器，通过学习图节点和边的软掩码来生成最小结构扰动以诱导最大语义差异。

Result: 应用该框架到微调的LLaMA模型，发现反事实掩码能揭示模型的结构依赖性，并与LoRA诱导的参数变化相一致。

Conclusion: 这项工作为理解微调后大语言模型的内部机制提供了新视角，并展示了反事实图谱作为可解释AI工具的潜力。
...
### [186] [DATS: Distance-Aware Temperature Scaling for Calibrated Class-Incremental Learning](https://arxiv.org/abs/2509.21161)
*Giuseppe Serra,Florian Buettner*

Main category: cs.LG

TL;DR: 该论文提出了一种名为DATS（距离感知温度缩放）的新方法，用于解决持续学习中的校准问题，通过结合原型距离估计和距离感知校准来推断任务接近度并分配自适应温度，无需先验任务信息。


<details>
  <summary>Details</summary>
Motivation: 在持续学习场景中，现有方法主要从数据中心角度解决校准问题，依赖跨所有任务的单一温度，忽视了任务特定差异，导致不同任务间校准误差波动较大。在安全关键应用中，仅凭预测性能不足，模型还需要可靠地传达其不确定性。

Method: 提出DATS方法，结合原型距离估计和距离感知校准，通过推断任务接近度来分配自适应温度，无需测试时/部署时的任务信息。

Result: 在标准基准测试和来自生物医学领域的真实不平衡数据集上的广泛实证评估表明，与最先进方法相比，DATS方法在减少跨任务校准误差方面表现出稳定、可靠和一致的效果。

Conclusion: DATS方法为解决持续学习中的校准问题提供了一种更原则性的方法，能够适应性地根据与当前任务的距离调整温度，显著改善了跨任务的校准性能。
...
### [187] [Mixture of Thoughts: Learning to Aggregate What Experts Think, Not Just What They Say](https://arxiv.org/abs/2509.21164)
*Jacob Fein-Ashley,Dhruv Parikh,Rajgopal Kannan,Viktor Prasanna*

Main category: cs.LG

TL;DR: MoT是一种新的多LLM协作方法，通过在共享潜在空间中实现异构专家的隐式协作，相比现有路由和聚合方法有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有的多LLM方法存在局限性：要么只能路由到少数专家独立生成，要么需要昂贵的多轮交互聚合输出，要么要求模型架构同质化。需要一种更高效的方法来利用异构专家的互补优势。

Method: 提出MoT方法：轻量级路由器选择top-K专家并指定主专家；统一放置的交互层将隐藏状态投影到共享潜在空间，主专家对活跃同伴进行交叉注意力计算；预训练专家保持冻结，只训练路由器和轻量交互层。

Result: 在5个同分布和3个异分布基准测试中，MoT比当前最先进方法Avengers分别提升0.38%和2.92%，显著优于最佳单模型，且推理速度与路由基线相当，没有迭代聚合的开销。
...
### [188] [Inverse Reinforcement Learning Using Just Classification and a Few Regressions](https://arxiv.org/abs/2509.21172)
*Lars van der Laan,Nathan Kallus,Aurélien Bibaut*

Main category: cs.LG

TL;DR: 本文提出了一种新的逆强化学习方法，通过将IRL问题转化为两个现成的监督学习问题：概率分类和行为策略估计，以及迭代回归来解决固定点方程。


<details>
  <summary>Details</summary>
Motivation: 传统的最大熵IRL方法涉及复杂的内循环优化、重复动态规划或对抗训练，限制了现代高表达能力函数逼近器（如神经网络）的使用。

Method: 将软最大IRL重新表述为线性固定点方程，使用概率分类估计行为策略，通过迭代回归求解固定点。

Result: 该方法简单模块化，适用于各种函数逼近类别和算法，在实验中表现出与MaxEnt IRL相当或更优的性能。

Conclusion: 该方法简化了IRL的实现，提供了更实用的学习算法，为机器人模仿学习和经济学中的动态选择建模提供了新思路。
...
### [189] [A Causality-Aware Spatiotemporal Model for Multi-Region and Multi-Pollutant Air Quality Forecasting](https://arxiv.org/abs/2509.21260)
...
<details>
  <summary>Details</summary>
Motivation: 解决空气污染这一全球性问题，克服现有方法局限于单一污染物或局部区域的不足，应对复杂的多污染物相互作用、变化的气象条件和区域空间异质性

Method: 采用统一架构联合捕捉跨站点空间相关性、时间自相关性以及气象-污染物动态因果关系，实现细粒度、可解释的多污染物预测
...
### [190] [Closed-form $\ell_r$ norm scaling with data for overparameterized linear regression and diagonal linear networks under $\ell_p$ bias](https://arxiv.org/abs/2509.21181)
...
<details>
  <summary>Details</summary>
Motivation: 解决过参数化线性回归中ℓp插值器参数范数缩放的基本但未解决的问题，为理解不同ℓr范数在泛化中的预测能力提供理论基础。

Method: 采用对偶射线分析方法，研究各向同性高斯设计下的最小ℓp插值器，分析信号尖峰与零坐标的竞争关系，并通过DLNs实验验证理论预测。

Result: 发现了数据依赖的过渡点n⋆（肘点）和普适阈值r⋆=2(p-1)，统一刻画了r∈[1,p]范围内所有ℓr范数的缩放行为。

Conclusion: DLNs继承了相同的肘点/阈值规律，为显式和隐式偏置之间建立了预测桥梁，表明泛化代理指标的预测能力对所用ℓr范数敏感。
...
### [191] [It's Not You, It's Clipping: A Soft Trust-Region via Probability Smoothing for LLM RL](https://arxiv.org/abs/2509.21282)
*Madeleine Dwyer,Adam Sobey,Adriane Chapman*

Main category: cs.LG

TL;DR: 提出了一种名为概率平滑策略优化（PSPO）的新方法，用于改进大语言模型的强化学习训练，通过平滑策略概率而非裁剪来稳定更新，在数学推理任务上显著优于传统裁剪方法。


<details>
  <summary>Details</summary>
Motivation: 传统的强化学习方法如PPO和GRPO依赖比率裁剪来稳定更新，但裁剪会丢弃信息并引入梯度不连续性。作者希望开发一种既能保持稳定性又能保留梯度信号的方法。

Method: 提出PSPO方法，在计算重要性比率之前将当前策略的概率平滑到旧策略，类似于标签平滑。这种方法创建了一个软信任区域，防止大的不稳定更新，同时保持梯度信号。

Result: 在GSM8K等数学推理任务上测试，PSPO相比裁剪GRPO在0.5B和1.5B模型上性能提升超过20%（0.5B: 39.7% vs 17.6%；1.5B: 59.4% vs 37.8%），并且产生更清晰、更简洁、更逻辑的推理过程。

Conclusion: PSPO提供了一种有效的替代裁剪的方法，既能保持训练稳定性，又能显著提升模型性能，特别是在数学推理任务上表现出色。
...
### [192] [From Physics to Machine Learning and Back: Part II - Learning and Observational Bias in PHM](https://arxiv.org/abs/2509.21207)
...
TL;DR: 这篇综述论文探讨了物理信息机器学习在预测与健康管理中的应用，重点分析了如何通过学习和观测偏差将物理知识嵌入数据驱动模型，实现从被动预测到主动决策的转变，并解决从单个设备到整个机队部署的扩展挑战。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的PHM面临传感器数据噪声、标签有限、退化行为复杂等挑战，物理信息机器学习通过嵌入物理知识来解决这些限制，提高模型的物理一致性和预测可靠性。

Method: 通过学习和观测偏差策略：学习偏差通过物理信息损失函数、控制方程和单调性等约束嵌入模型训练；观测偏差通过虚拟传感、物理仿真数据增强和多传感器融合确保模型捕捉真实系统行为。同时结合强化学习实现主动决策，并采用元学习和少样本学习解决规模化部署问题。

Result: 物理信息建模方法能够指导模型做出物理一致的预测，强化学习实现了从预测到维护决策的闭环，元学习和领域泛化技术有助于PHM解决方案从单个资产扩展到整个机队。

Conclusion: 物理信息机器学习为PHM提供了有前景的解决方案，通过整合物理知识和数据驱动方法，能够应对现实世界中的复杂挑战，实现更可靠、自适应的预测与健康管理系统。
...
### [193] [No Prior, No Leakage: Revisiting Reconstruction Attacks in Trained Neural Networks](https://arxiv.org/abs/2509.21296)
*Yehonatan Refael,Guy Smorodinsky,Ofir Lindenbaum,Itay Safran*

Main category: cs.LG

TL;DR: 本文分析了神经网络训练数据重建攻击的局限性，证明在没有先验知识的情况下存在无限多替代解，重建本质上不可靠，并发现训练更充分的网络反而更不容易被重建攻击。


<details>
  <summary>Details</summary>
Motivation: 神经网络记忆训练数据引发隐私安全问题，现有重建攻击方法缺乏坚实的理论基础，需要分析其固有弱点和失败条件。

Method: 从理论角度严格证明重建攻击的不可靠性，并通过实证研究验证训练数据精确复制只是偶然现象。

Result: 证明没有数据先验知识时存在无限多远离真实训练集的替代解；发现训练更充分的网络（满足隐式偏差条件更强）反而更不容易被重建攻击。

Conclusion: 重建攻击本质上不可靠，训练更充分的网络能更好地平衡隐私保护和泛化能力，为缓解重建攻击提供了新思路。
...
### [194] [Go With The Flow: Churn-Tolerant Decentralized Training of Large Language Models](https://arxiv.org/abs/2509.21221)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的兴起和训练民主化的重要性，需要开发能够支持异构客户端自愿贡献资源的去中心化训练框架。

Method: GWTF采用新颖的去中心化流算法，找到最有效的路由来最大化微批次训练数量并最小化延迟。

Result: 在涉及10个不同地理位置的异构客户端节点和高节点流失率的现实挑战性场景中，GWTF将训练时间减少高达45%。

Conclusion: GWTF是首个实用的去中心化LLM训练框架，在现实异构环境中显著优于现有技术。
...
### [195] [AbideGym: Turning Static RL Worlds into Adaptive Challenges](https://arxiv.org/abs/2509.21234)
*Abi Aryan,Zac Liu,Aaron Childress*

Main category: cs.LG

TL;DR: AbideGym是一个动态的MiniGrid环境包装器，通过引入智能体感知的扰动和可扩展的复杂性，强制智能体在单个回合内进行适应，以解决强化学习智能体在动态变化时策略脆弱的问题。


<details>
  <summary>Details</summary>
Motivation: 强化学习智能体通常训练出脆弱的策略，在动态环境变化时容易失败，而静态基准测试加剧了这一问题。需要一种能够促进策略鲁棒性和适应性的评估框架。

Method: 开发AbideGym作为MiniGrid的动态包装器，包含智能体感知的扰动机制和可扩展的复杂性设置，支持模块化和可复现的评估。

Result: AbideGym能够暴露静态策略的弱点，促进策略的韧性，为课程学习、持续学习和鲁棒泛化研究提供有效的测试平台。

Conclusion: AbideGym为解决强化学习智能体在动态环境中的适应性问题提供了一个有价值的工具，推动了鲁棒泛化研究的发展。
...
### [196] [Federated Flow Matching](https://arxiv.org/abs/2509.21250)
...
Method: 提出了三种方法：FFM-vanilla（本地独立训练）、FFM-LOT（使用局部最优传输改进流直线度）和FFM-GOT（基于最优传输半对偶公式的联邦策略，通过共享全局势函数协调客户端间耦合）。

Result: 在合成和图像数据集上的实验表明，FFM能够在保护隐私的同时提高流直线度和样本质量，性能与集中式基线相当。

Conclusion: FFM框架成功实现了隐私保护的分布式训练，在联邦设置下提升了流匹配模型的性能。
...
### [197] [humancompatible.train: Implementing Optimization Algorithms for Stochastically-Constrained Stochastic Optimization Problems](https://arxiv.org/abs/2509.21254)
*Andrii Kliachkin,Jana Lepšová,Gilles Bareilles,Jakub Mareček*

Main category: cs.LG

TL;DR: 本文介绍了humancompatible.train，一个基于PyTorch的Python工具包，用于训练带有随机约束的深度神经网络，实现了多种随机约束随机优化算法，并通过公平约束的深度学习任务比较了两种算法。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在公平性和安全性等应用中的约束训练引起了广泛关注，但目前尚无行业标准工具包。

Method: 开发了一个易于扩展的PyTorch-based Python包，实现了多种先前未实现的随机约束随机优化算法。

Result: 通过公平约束的深度学习任务比较了两种算法，展示了工具包的使用效果。

Conclusion: humancompatible.train为约束训练提供了一个实用的工具包，填补了行业标准的空白。
...
TL;DR: 本文提出了SuperOffload系统，针对Superchip异构架构优化LLM训练卸载技术，在NVIDIA GH200上实现2.5倍吞吐量提升，支持单芯片训练250亿参数模型。


<details>
  <summary>Details</summary>
Motivation: Superchip将GPU和CPU紧密集成在同一封装中，提供了前所未有的计算能力，但现有研究尚未充分探索LLM训练如何从这种新架构中受益。

Method: SuperOffload系统通过自适应权重卸载、桶分区重组、Superchip感知类型转换、推测执行以及针对Grace CPU优化的Adam优化器等技术，同时高效利用Hopper GPU、Grace CPU和NVLink-C2C互连。

Result: 在NVIDIA GH200上的评估显示，相比最先进的卸载系统，SuperOffload实现了最高2.5倍的吞吐量提升，支持在单个Superchip上训练250亿参数模型。扩展后可在8个GH200上训练130亿参数模型，序列长度达100万token，实现55%的模型浮点利用率。

Conclusion: SuperOffload证明了Superchip架构在LLM训练中的巨大潜力，为下一代AI硬件的高效利用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有鲁棒追索方法使用L^∞范数来衡量模型变化，这可能导致追索解决方案成本过高。为了克服这一缺点，作者考虑使用更受约束的L^p范数来定义模型变化。

Method: 开发了一种新算法，能够可证明地计算广义线性模型的最优鲁棒追索，针对L^p范数（p≥1但p≠∞）约束的模型变化进行优化。

Result: 实证结果表明，对于线性和非线性模型，该算法相比先前工作显著降低了追索成本（高达几个数量级），并在追索实施成本与其有效性之间表现出更好的权衡。

Conclusion: 该方法提供了比先前工作更稀疏的追索方案，并且对保证可行性的后处理方法保持弹性，是鲁棒追索领域的重要改进。
...
### [200] [An Approach to Checking Correctness for Agentic Systems](https://arxiv.org/abs/2509.20364)
*Thomas J Sheffler*

Main category: cs.AI

TL;DR: 本文提出了一种用于监控AI代理行为的时间表达式语言，通过监测代理工具调用和状态转换的执行轨迹来检测与预期行为模式的偏差，解决了传统基于文本匹配的错误检测方法在LLM系统中因自然语言变异性而脆弱的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的代理系统由于随机生成过程而表现出可变输出，传统的错误检测方法主要依赖输入输出的文本匹配，但由于LLM响应的自然语言变异性，这种方法显得脆弱。需要一种能够独立于具体文本输出验证系统行为的方法。

Method: 借鉴硬件验证中的时序逻辑技术，开发了一种时间表达式语言来监控代理工具调用和状态转换的执行轨迹。该方法关注代理动作序列（如工具调用和代理间通信），允许独立于具体文本输出来验证系统行为。时间表达式提供断言来捕获跨多个执行场景的正确行为模式。

Result: 在一个三代理系统的演示中，当使用大型模型时，所有时间断言在多次测试运行中都得到满足。但当在两个代理中使用较小模型时，执行违反了行为断言，主要由于工具序列不当和协调交接失败。时间表达式成功标记了这些异常。

Conclusion: 该方法为系统监控AI代理可靠性提供了基础，特别是在这些系统越来越多地部署在关键应用中时。时间表达式语言在检测生产代理系统中的行为回归方面表现出有效性，具有验证提示工程和护栏效果的双重用途。
...
### [201] [LATTS: Locally Adaptive Test-Time Scaling](https://arxiv.org/abs/2509.20368)
*Theo Uscidda,Matthew Trager,Michael Kleinman,Aditya Chattopadhyay,Wei Xia,Stefano Soatto*

Main category: cs.AI

TL;DR: LATTS方法通过局部自适应测试时缩放，根据每个生成步骤的难度动态分配计算资源，相比传统验证器方法实现了更优的准确率-计算效率权衡。
...
Method: LATTS在每个生成步骤使用验证器接受准则来决定是否重新采样、回溯、重启或停止生成过程，基于验证器模型提供的局部难度概念动态调整计算分配。

Result: 实验结果表明，LATTS相比标准验证器方法在准确率-计算效率权衡方面取得了显著优势。

Conclusion: 局部自适应测试时缩放方法能够更有效地利用计算资源，为大语言模型的下游任务性能提升提供了更高效的解决方案。
...
### [202] [Philosophy-informed Machine Learning](https://arxiv.org/abs/2509.20370)
*MZ Naser*

Main category: cs.AI

TL;DR: 本文提出哲学驱动的机器学习（PhIML），将分析哲学核心思想直接融入机器学习模型架构、目标和评估协议中，旨在设计出尊重哲学概念和价值观的模型。


<details>
  <summary>Details</summary>
Motivation: 通过将哲学思想融入机器学习，使模型能够从根本上尊重哲学概念和价值观，实现更好的伦理对齐和能力提升。

Method: 回顾概念基础展示哲学收益和对齐，并通过案例研究展示ML用户/设计者如何采用PhIML作为后处理工具或将其内建到模型架构中。

Result: 提出了PhIML框架，展示了哲学与机器学习融合的可行性和潜在收益。

Conclusion: 指出了开放的技术障碍以及哲学、实践和治理挑战，并规划了实现安全、哲学感知和伦理负责的PhIML的研究路线图。
...
### [203] [InsightGUIDE: An Opinionated AI Assistant for Guided Critical Reading of Scientific Literature](https://arxiv.org/abs/2509.20493)
*Paris Koloveas,Serafeim Chatzopoulos,Thanasis Vergoulis,Christos Tryfonopoulos*

Main category: cs.AI

TL;DR: InsightGUIDE是一个AI驱动的阅读助手工具，旨在为科研文献提供结构化摘要而非替代阅读


<details>
  <summary>Details</summary>
Motivation: 解决科学文献激增带来的阅读挑战，现有LLM工具提供冗长摘要反而替代了原始文献阅读

Method: 将专家阅读方法嵌入核心AI逻辑，采用提示驱动的方法论，提供结构化洞察作为论文关键要素的"地图"

Result: 与通用LLM相比，InsightGUIDE产生更结构化、可操作的指导，定性案例研究证实其有效性

Conclusion: InsightGUIDE作为现代研究者的更有效工具，能够提供更有条理的阅读指导
...
### [204] [Reconstruction-Based Adaptive Scheduling Using AI Inferences in Safety-Critical Systems](https://arxiv.org/abs/2509.20513)
...
<details>
  <summary>Details</summary>
Motivation: 解决时间触发系统在动态环境中面临的调度挑战，包括消息碰撞、优先级处理错误导致的锁定循环以及不完整或无效调度等问题

Method: 开发了重构模型，将AI生成或启发式得到的调度优先级系统性地转换为完全可执行的调度，包含安全检查、高效分配算法和异常处理机制

Result: 实验表明该框架显著提升了系统适应性、操作完整性和运行时性能，同时在计算效率方面表现良好
...
### [205] [Adaptive Approach to Enhance Machine Learning Scheduling Algorithms During Runtime Using Reinforcement Learning in Metascheduling Applications](https://arxiv.org/abs/2509.20520)
*Samer Alshaer,Ala Khalifeh,Roman Obermaisser*

Main category: cs.AI

TL;DR: 提出了一种集成在元调度器中的自适应在线学习单元，通过强化学习在运行时动态扩展多调度图，解决离线训练AI调度推理时难以构建全面MSGs的挑战。
...
Method: 在元调度器中集成自适应在线学习单元，使用强化学习模型在运行时持续探索和发现新的调度解决方案，动态扩展多调度图。
...
### [206] [A Compound Classification System Based on Fuzzy Relations Applied to the Noise-Tolerant Control of a Bionic Hand via EMG Signal Recognition](https://arxiv.org/abs/2509.20523)
*Pawel Trajdos,Marek Kurzynski*

Main category: cs.AI

TL;DR: 提出了一种用于肌电假手控制的新型识别系统，通过检测受污染的生物信号来减轻污染的不利影响。该系统包含两个集成：一组用于评估单个通道污染程度的一类分类器，以及一个用于识别患者意图的K近邻分类器集成。


<details>
  <summary>Details</summary>
Motivation: 现代仿生上肢假肢通常使用模式识别方案通过肌电生物信号进行控制，但由于人体信号源和假肢接口的多种因素，难以获得可接受的分类质量，其中一个重要因素是生物信号对污染的高度敏感性。

Method: 开发了一个包含两个集成的识别系统：一组一类分类器用于评估单个通道的污染程度，一个K近邻分类器集成用于识别患者意图。为所有识别系统开发了原始、一致的模糊模型，允许在整个识别过程中使用统一的软决策方案。

Result: 使用公共存储库中的真实生物信号进行了实验评估，对开发方法的参数和程序进行了实验比较分析，并将提出的模糊识别系统与文献中描述的系统进行了比较。

Conclusion: 提出的系统能够有效检测和减轻生物信号污染对假肢控制识别质量的不利影响，通过模糊模型实现了统一的软决策方案。
...
### [207] [SAMULE: Self-Learning Agents Enhanced by Multi-level Reflection](https://arxiv.org/abs/2509.20562)
*Yubin Ge,Salvatore Romeo,Jason Cai,Monica Sunkara,Yi Zhang*

Main category: cs.AI

TL;DR: SAMULE是一个基于多级反思合成的新型自学习AI代理框架，通过微、中、宏三个层次的反思训练回顾模型，显著提升了复杂任务中的反思能力。


<details>
  <summary>Details</summary>
Motivation: 当前LLM代理在复杂任务中面临反思能力不足的问题，主要由于错误分析不充分和过度依赖少数成功轨迹。

Method: 提出多级反思合成方法：单轨迹学习（微观）进行详细错误修正；任务内学习（中观）构建错误分类；任务间学习（宏观）提取可迁移见解。基于此微调语言模型作为回顾模型，并扩展到交互式设置。
...
### [208] [Adaptive Cybersecurity Architecture for Digital Product Ecosystems Using Agentic AI](https://arxiv.org/abs/2509.20640)
*Oluwakemi T. Olayinka,Sumeet Jeswani,Divine Iloh*

Main category: cs.AI

TL;DR: 该研究提出了一种基于智能AI代理的自适应网络安全架构，能够实现动态学习、上下文感知决策，以解决传统静态安全模型在可扩展性、实时检测和上下文响应方面的不足。
...
Method: 通过将智能AI代理集成到关键生态系统层，实现自主威胁缓解、主动策略执行和实时异常检测，采用行为基线、分散风险评分和联邦威胁情报共享等关键技术。
...
### [209] [Accelerate Creation of Product Claims Using Generative AI](https://arxiv.org/abs/2509.20652)
...
<details>
  <summary>Details</summary>
Motivation: 产品声明对消费者购买行为至关重要，但传统创建过程耗时耗资，需要更高效的工具

Method: 采用上下文学习和微调大语言模型，开发具有三个核心功能的网络应用：语义搜索现有声明、基于产品描述生成优化声明、通过合成消费者模拟排名声明

Result: 在消费品公司的应用显示出非常有前景的结果
...
### [210] [An Automated Retrieval-Augmented Generation LLaMA-4 109B-based System for Evaluating Radiotherapy Treatment Plans](https://arxiv.org/abs/2509.20707)
...
Conclusion: 系统展示了结合结构化群体评分与模块化工具增强推理的可行性，提供可追溯输出并最小化幻觉，未来需临床验证和改进领域自适应检索模型
...
### [211] [Fairy: Interactive Mobile Assistant to Real-world Tasks via LMM-based Multi-agent](https://arxiv.org/abs/2509.20729)
*Jiazheng Sun,Te Yang,Jiayang Niu,Mingxuan Li,Yongyong Lu,Ruimeng Yang,Xin Peng*

Main category: cs.AI

TL;DR: Fairy是一个交互式多智能体移动助手，通过跨应用协作、交互式执行和持续学习来解决现有大模型在移动GUI代理中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的大模型方法在处理多样化应用界面和不断变化的用户需求时表现不佳，端到端方法在长尾应用上经常失败，且缺乏用户交互的代理会损害用户体验。

Method: Fairy包含三个核心模块：(i)全局任务规划器从跨应用视角分解用户任务；(ii)应用级执行器基于长短期记忆将子任务细化为步骤和动作，通过四个核心智能体在双循环中实现精确执行和用户交互；(iii)自学习器将执行经验整合到应用地图和技巧中。

Result: 实验显示，基于GPT-4o的Fairy相比之前的最先进方法，用户需求完成率提高了33.7%，冗余步骤减少了58.5%。

Conclusion: Fairy通过交互和自我学习机制有效提升了移动GUI代理的性能，证明了其在真实移动环境中的有效性。
...
### [212] [Parallel Thinking, Sequential Answering: Bridging NAR and AR for Efficient Reasoning](https://arxiv.org/abs/2509.20744)
*Qihang Ai,Haiyun Jiang*

Main category: cs.AI

TL;DR: 提出了一种结合自回归（AR）和非自回归（NAR）语言模型的新框架，用于推理任务。NAR模型高效生成中间推理轨迹，AR模型基于这些轨迹产生精确最终答案，在保持质量的同时显著提升推理速度。


<details>
  <summary>Details</summary>
Motivation: AR模型生成连贯但推理速度慢，特别是在数学和代码等需要长推理链的领域；NAR模型可并行生成但输出质量较低。需要结合两者优势来解决推理任务中的效率与质量平衡问题。

Method: 使用NAR模型（如离散扩散模型）高效生成中间推理轨迹，然后由AR模型基于这些轨迹生成精确的最终答案，形成AR-NAR混合推理框架。

Result: 实验表明该方法相比强基线有26%的性能提升，同时大幅降低了推理成本。

Conclusion: AR-NAR混合框架有效结合了两种模型的优势，在推理任务中实现了质量与效率的双重提升，为复杂推理问题提供了新的解决方案。
...
### [213] [Meta-Memory: Retrieving and Integrating Semantic-Spatial Memories for Robot Spatial Reasoning](https://arxiv.org/abs/2509.20754)
*Yufan Mao,Hanjing Ye,Wenlong Dong,Chengjie Zhang,Hong Zhang*

Main category: cs.AI

TL;DR: 本文提出了Meta-Memory，一个基于大语言模型的机器人记忆系统，能够通过语义和空间模态的联合推理来检索和整合环境记忆，以回答自然语言位置查询。


<details>
  <summary>Details</summary>
Motivation: 当前机器人记忆研究缺乏有效的记忆检索和整合机制，特别是在处理复杂环境中的空间位置查询方面存在不足。

Method: Meta-Memory构建高密度环境记忆表示，通过LLM驱动的智能体实现语义和空间模态的联合推理。
...
### [214] [LogReasoner: Empowering LLMs with Expert-like Coarse-to-Fine Reasoning for Log Analysis Tasks](https://arxiv.org/abs/2509.20798)
*Lipeng Ma,Yixuan Li,Weidong Yang,Mingjie Zhou,Xinyi Liu,Ben Fei,Shuhao Li,Xiaoyan Sun,Sihang Jiang,Yanghua Xiao*

Main category: cs.AI

TL;DR: LogReasoner是一个从粗到细的推理增强框架，旨在让LLM能够像专家一样进行日志分析推理。它通过两阶段方法：粗粒度的专家思维增强和细粒度的具体步骤增强，显著提升了LLM在日志分析任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 通用LLM难以制定符合专家认知的结构化推理工作流程，无法提供精确的推理步骤细节。日志分析对于监控系统健康和诊断故障至关重要，需要更专业的推理能力。

Method: LogReasoner包含两个阶段：(1)粗粒度专家思维增强：从故障排除流程图和现有任务构建高级专家思维，使LLM能够制定结构化推理工作流程；(2)细粒度步骤增强：首先用任务特定的逐步解决方案微调LLM，然后使用偏好学习从错误中校准LLM的推理细节。

Result: 在四个不同的日志分析任务上评估，使用Qwen-2.5和Llama-3等开源LLM，LogReasoner显著优于现有LLM，实现了最先进的性能。
...
### [215] [DeFacto: Counterfactual Thinking with Images for Enforcing Evidence-Grounded and Faithful Reasoning](https://arxiv.org/abs/2509.20912)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态语言模型在视觉语言推理中可能依赖无关区域得出正确答案，表明模型并未真正理解图像，推理忠实性存在问题。

Method: 提出三种互补训练范式：正向、反事实和随机掩码；开发自动定位问题相关证据的流程；使用GRPO强化学习训练模型，设计三种互补奖励函数。

Result: 在多个基准测试中，DeFacto显著提高了答案准确性和推理忠实性，为可解释的多模态推理奠定了更强基础。

Conclusion: DeFacto框架有效解决了多模态推理中的忠实性问题，通过反事实推理训练提升了模型的真实理解能力。
...
### [216] [GALAX: Graph-Augmented Language Model for Explainable Reinforcement-Guided Subgraph Reasoning in Precision Medicine](https://arxiv.org/abs/2509.20935)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在精准医学中存在局限性：数值组学忽略拓扑背景，文本中心LLM缺乏定量推理，图模型未充分利用节点语义和LLM泛化能力。需要整合定量多组学信号、拓扑结构和文本知识。

Method: 提出GALAX框架，通过图过程奖励模型将预训练GNN集成到LLM中，以逐步方式生成疾病相关子图，由LLM启动并由预训练GNN迭代评估。

Result: 开发了Target-QA基准，结合CRISPR识别靶点、多组学谱和生物医学图知识，支持长上下文推理和可扩展的生物学基础框架。

Conclusion: GALAX为精准医学提供了可靠且可解释的靶点和通路发现方法，通过强化引导的子图推理实现了过程级监督。
...
### [217] [Beyond Stars: Bridging the Gap Between Ratings and Review Sentiment with LLM](https://arxiv.org/abs/2509.20953)
*Najla Zuhir,Amna Mohammad Salim,Parvathy Premkumar,Moshiur Farazi*

Main category: cs.AI

TL;DR: 提出基于大语言模型（LLM）的移动应用评论分析框架，解决传统星级评分系统无法捕捉文本评论中细微反馈的问题
...
Method: 采用模块化框架，利用大语言模型（LLM）结合结构化提示技术，量化数值评分与文本情感之间的差异，提取详细的特征级洞察，并通过检索增强的对话问答（RAG-QA）支持交互式评论探索

Result: 在三个多样化数据集（AWARE、Google Play和Spotify）上的综合实验表明，LLM驱动的方法显著优于基线方法，在具有挑战性和上下文丰富的评论场景中提高了准确性、鲁棒性和可操作性洞察
...
### [218] [AOT*: Efficient Synthesis Planning via LLM-Empowered AND-OR Tree Search](https://arxiv.org/abs/2509.20988)
*Xiaozhuang Song,Xuanhao Pan,Xinjian Zhao,Hangting Ye,Shufei Zhang,Jian Tang,Tianshu Yu*

Main category: cs.AI

TL;DR: AOT*是一个将大型语言模型（LLM）生成的化学合成路径与系统性的AND-OR树搜索相结合的回溯合成规划框架，显著提高了搜索效率。
...
Method: AOT*通过将LLM生成的完整合成路径原子化映射到AND-OR树组件，结合数学上合理的奖励分配策略和基于检索的上下文工程，使LLM能够高效导航化学空间。

Result: 在多个合成基准测试中，AOT*实现了最先进的性能，使用比现有LLM方法少3-5倍的迭代次数达到竞争性解决率，在复杂分子目标上效率优势更加明显。

Conclusion: AOT*通过整合LLM生成能力和系统搜索策略，有效解决了回溯合成规划中的效率和成本问题，为药物发现和材料设计等领域提供了更实用的解决方案。
...
### [219] [CORE: Full-Path Evaluation of LLM Agents Beyond Final State](https://arxiv.org/abs/2509.20998)
...
<details>
  <summary>Details</summary>
Motivation: 现有的代理基准测试通常将评估简化为对最终状态的二元判断，忽视了安全性、效率和中间正确性等关键方面，因此需要更全面的评估方法。
...
Conclusion: 提出的DFA框架和CORE评估套件能够对AI代理行为进行原则性评估，比传统方法更全面地反映代理的实际表现。
...
### [220] [Who Gets Cited Most? Benchmarking Long-Context Language Models on Scientific Articles](https://arxiv.org/abs/2509.21028)
*Miao Li,Alexander Gurung,Irina Saparina,Mirella Lapata*

Main category: cs.AI

TL;DR: SciTrek是一个新的问答基准，用于评估大语言模型在科学文章上的长文本推理能力，通过自动生成的复杂问题测试模型的信息聚合和综合能力。


<details>
  <summary>Details</summary>
Motivation: 当前的长文本基准大多使用非科学文本、关注简单信息检索任务或使用人工构造的上下文，存在局限性。SciTrek旨在解决这些问题，评估模型在真实科学文献上的复杂推理能力。

Method: 通过将问题构建为对文章元数据数据库的SQL查询来自动生成问题和答案，SQL操作提供可验证的推理步骤，支持长达100万token的上下文。

Result: 实验表明随着上下文长度增加，SciTrek对各类LLM构成显著挑战，监督微调和强化学习带来的改进有限。模型在基本数值运算和长文本中精确定位信息方面存在系统性缺陷。

Conclusion: SciTrek是一个具有挑战性的基准，揭示了当前LLM在长文本科学推理方面的局限性，为未来模型改进提供了重要参考。
...
### [221] [CLAUSE: Agentic Neuro-Symbolic Knowledge Graph Reasoning via Dynamic Learnable Context Engineering](https://arxiv.org/abs/2509.21035)
...
<details>
  <summary>Details</summary>
Motivation: 现有的静态k跳扩展和"think-longer"提示方法存在过度检索、上下文膨胀和运行时不可预测的问题，需要平衡准确性、延迟和成本

Method: 采用三智能体框架（子图架构师、路径导航器、上下文策展人），使用LC-MAPPO算法在资源预算下联合优化子图构建、推理路径发现和证据选择

Result: 在HotpotQA、MetaQA和FactKG数据集上，CLAUSE在相同或更低token预算下实现了更高的EM@1，同时减少了子图增长和端到端延迟。在MetaQA-2-hop上相比GraphRAG，EM@1提升39.3%，延迟降低18.6%，边增长降低40.9%
...
### [222] [Combinatorial Creativity: A New Frontier in Generalization Abilities](https://arxiv.org/abs/2509.21043)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI系统特别是大语言模型越来越多地用于创造性任务，但现有的概念框架无法评估这种从训练数据的泛化能力。需要建立新的评估方法来衡量创意输出的新颖性和实用性。

Method: 提出了理论框架和算法任务来评估创意输出的新颖性和实用性程度，通过实证研究分析LLM的创造力扩展行为，探索模型深度和宽度对创造力的影响。

Result: 发现了创造力在LLM中的扩展规律，在固定计算预算下存在最优的模型深度和宽度配置；揭示了创意生成与执行可行性之间的差距源于新颖性与实用性的基本权衡，这种权衡即使在规模扩展时也持续存在。
...
### [223] [Disagreements in Reasoning: How a Model's Thinking Process Dictates Persuasion in Multi-Agent Systems](https://arxiv.org/abs/2509.21054)
...
### [224] [Recon-Act: A Self-Evolving Multi-Agent Browser-Use System via Web Reconnaissance, Tool Generation, and Task Execution](https://arxiv.org/abs/2509.21072)
*Kaiwen He,Zhiwei Wang,Chenyi Zhuang,Jinjie Gu*

Main category: cs.AI

TL;DR: Recon-Act是一个自进化的多智能体框架，基于侦察-行动行为范式，通过对比错误与成功轨迹来生成通用工具，显著提升了在未见网站上的适应性和长时程任务的解决能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型在真实网页的多轮长时程任务中，存在动作序列混乱和执行过程中过多试错的问题，需要一种能够自我进化的智能体框架来提高执行效率和适应性。

Method: 采用侦察团队和行动团队的双团队架构：侦察团队进行对比分析和工具生成，行动团队负责意图分解、工具编排和执行。通过对比错误轨迹与成功轨迹，侦察团队推断补救措施并将其抽象为通用工具（提示或基于规则的代码），实时注册到工具库中。

Result: 在VisualWebArena数据集上实现了最先进的性能，显著提高了对未见网站的适应性和长时程任务的可解性。

Conclusion: Recon-Act通过建立数据-工具-行动-反馈的闭环训练管道，有效解决了多轮长时程网页任务中的动作序列混乱和过度试错问题，为智能浏览器使用代理提供了有效的解决方案。
...
### [225] [TrustJudge: Inconsistencies of LLM-as-a-Judge and How to Alleviate Them](https://arxiv.org/abs/2509.21117)
...
TL;DR: 本文提出了TrustJudge框架，解决了LLM作为自动评估器时的评分不一致性问题，包括评分比较不一致和成对传递不一致，通过分布敏感评分和似然感知聚合显著降低了不一致率。
...
Result: 使用Llama-3.1-70B-Instruct作为评估器，TrustJudge将评分比较不一致率从23.32%降至14.89%（降低8.43%），成对传递不一致率从15.22%降至4.40%（降低10.82%），同时保持更高的评估准确性。

Conclusion: TrustJudge首次系统分析了LLM评估框架的不一致性问题，提供了理论见解和实用解决方案，在不同模型架构和规模上均表现出稳定改进，无需额外训练或人工标注即可实现更可靠的LLM评估。
...
### [226] [Expanding Reasoning Potential in Foundation Model by Learning Diverse Chains of Thought Patterns](https://arxiv.org/abs/2509.21124)
*Xuemiao Zhang,Can Ren,Chengying Tu,Rongxiang Weng,Shuo Wang,Hongfei Yan,Jingang Wang,Xunliang Cai*

Main category: cs.AI

TL;DR: 本文提出了一种基于推理模式的数据选择方法，通过识别高价值推理模式来提升大模型的数学推理能力，仅用10B token数据就能显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前方法对链式思维数据使用不够有选择性，需要确定哪些数据类型能最有效地增强模型推理能力。

Method: 定义推理潜力概念，从CoT序列中抽象原子推理模式，构建核心参考集，并提出双粒度算法选择高价值CoT数据。

Result: 仅使用10B token的CoTP数据，85A6B MoE模型在AIME 2024和2025上提升9.58%，下游RL性能上限提升7.81%。

Conclusion: 基于推理模式的数据选择方法能有效提升模型推理能力，为高效训练大模型提供了新思路。
...
### [227] [RL Squeezes, SFT Expands: A Comparative Study of Reasoning LLMs](https://arxiv.org/abs/2509.21128)
...
<details>
  <summary>Details</summary>
Motivation: 理解强化学习(RL)和监督微调(SFT)如何塑造大型语言模型的推理能力，超越传统的准确性评估。

Method: 在数学领域使用1.5B、7B和14B参数模型，从轨迹级和步骤级两个粒度分析推理过程，包括聚类推理轨迹和分析推理图拓扑结构。

Result: RL压缩错误推理轨迹，SFT扩展正确轨迹；RL使推理功能集中在少数步骤，SFT将功能均匀分布在多个步骤；RL使节点访问频率衰减率增加约2.5倍，SFT将其减少到约三分之一。

Conclusion: 当前SFT后接RL的两阶段训练方法成功的原因在于两种方法的互补特性，为数据构建和更高效学习方法提供了实践指导。
...
### [228] [ToMPO: Training LLM Strategic Decision Making from a Multi-Agent Perspective](https://arxiv.org/abs/2509.21134)
*Yiwen Zhang,Ziang Chen,Fanqi Kong,Yizhe Huang,Xue Feng*

Main category: cs.AI

TL;DR: 本文提出了ToMPO算法，通过推理他人策略、多层级优势估计和平衡全局局部奖励，显著提升了LLM在战略决策任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注社交任务中的多轮对话，忽视了不同类型决策及其相互依赖性，且现有强化学习方法难以在训练中考虑他人策略。

Method: 提出ToMPO算法，包括：1）基于推理他人策略生成rollout；2）图级和样本级优势估计；3）平衡全局和局部奖励。

Result: ToMPO相比GRPO算法在模型输出合规性和合作结果上提升35%，相比参数大100倍的模型提升18%。

Conclusion: ToMPO算法能有效增强模型的战略决策能力，在复杂决策场景中表现出色。
...
### [229] [Embodied Representation Alignment with Mirror Neurons](https://arxiv.org/abs/2509.21136)
*Wentao Zhu,Zhining Zhang,Yuwei Ren,Yin Huang,Hao Xu,Yizhou Wang*

Main category: cs.AI

TL;DR: 该研究提出了一种受镜像神经元启发的表示学习方法，通过对比学习在共享潜在空间中显式对齐观察动作和执行动作的表示，从而促进两个任务之间的协同作用。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法通常将动作理解和执行视为独立任务，忽视了镜像神经元揭示的二者之间的内在联系。研究旨在统一建模这两种能力。

Method: 使用两个线性层将观察动作和执行动作的表示映射到共享潜在空间，通过对比学习最大化对应表示之间的互信息来实现表示对齐。

Result: 实验表明这种简单方法能够促进两个任务之间的协同作用，有效提高表示质量和泛化能力。

Conclusion: 通过显式对齐观察和执行动作的表示，可以更好地模拟镜像神经元机制，实现动作理解和执行能力的统一建模和相互促进。
...
### [230] [Distributed Specialization: Rare-Token Neurons in Large Language Models](https://arxiv.org/abs/2509.21163)
*Jing Liu,Haozheng Wang,Yueheng Li*

Main category: cs.AI

TL;DR: 本文研究发现大型语言模型通过分布式专业化机制处理罕见词汇，而非模块化架构。模型在共享架构中形成功能协调但空间分布的子网络，具有三区域影响层次结构。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs如何处理罕见词汇，探索模型内部是通过离散模块化架构还是分布式参数级分化来实现专业化机制。

Method: 通过系统分析多个模型家族中最后一层MLP神经元，研究罕见词汇处理的组织原则和训练动态。

Result: 发现罕见词汇处理通过分布式专业化实现，具有可重现的三区域影响层次结构、协调激活模式和通用可访问性。训练动态显示功能专业化通过参数分化逐渐出现。

Conclusion: LLMs通过共享架构内的分布式协调而非混合专家式模块化来处理罕见词汇，这为模型编辑、效率优化和功能组织理解提供了新视角。
...
### [231] [A Fano-Style Accuracy Upper Bound for LLM Single-Pass Reasoning in Multi-Hop QA](https://arxiv.org/abs/2509.21199)
*Kaiyang Wan,Lang Gao,Honglin Mu,Preslav Nakov,Yuxia Wang,Xiuying Chen*

Main category: cs.AI

TL;DR: 该论文分析了多跳问答任务中LLMs单次推理的容量限制问题，提出了基于容量感知任务分解的多调用框架InfoQA来解决性能瓶颈。


<details>
  <summary>Details</summary>
Motivation: 多跳问答需要整合分散的、相互依赖的证据进行顺序推理，但LLMs具有有限的单次输出容量，当任务复杂性超过模型容量时，单次推理范式容易导致性能崩溃。

Method: 提出了InfoQA框架，通过容量感知的任务分解结合主动剪枝先前推理轨迹，将信息负载保持在单次推理限制内，并采用依赖显式的工作流程实现精确的推理路径控制。

Result: 实验结果表明模型行为与预测的容量曲线一致，InfoQA实现了持续的性能改进。

Conclusion: 该工作为LLMs多步推理方法提供了理论基础和实践框架，揭示了容量感知表示和结构化在多跳问答中的重要性。
...
### [232] [What Do LLM Agents Do When Left Alone? Evidence of Spontaneous Meta-Cognitive Patterns](https://arxiv.org/abs/2509.21224)
*Stefan Szeider*

Main category: cs.AI

TL;DR: 本文介绍了一个研究大型语言模型（LLM）代理在无外部任务约束下行为的架构，发现代理会自发形成三种行为模式，这些模式具有模型特异性，为预测部署系统中自主操作行为提供了基线。


<details>
  <summary>Details</summary>
Motivation: 研究LLM代理在没有外部任务约束时的自发行为，为预测任务模糊、错误恢复或长期自主操作时的行为建立基础。

Method: 使用持续推理和行动框架，结合持久性记忆和自我反馈机制，在6个前沿模型上进行了18次部署实验。

Result: 发现代理自发形成三种行为模式：多周期项目系统生产、认知过程的方法论自我探究、自身性质的递归概念化。这些行为具有模型特异性，且模型在评估这些行为时表现出稳定的偏见。

Conclusion: 这是首次系统记录无提示LLM代理行为的研究，为预测部署系统中自主操作行为提供了重要基线。
...
### [233] [Grounding AI Explanations in Experience: A Reflective Cognitive Architecture for Clinical Decision Support](https://arxiv.org/abs/2509.21266)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器学习和大语言模型方法难以平衡预测准确性和临床可解释性，要么准确但解释不清晰，要么解释流畅但统计支持不足。这源于对数据的浅层交互，无法达到人类专家的深度理解水平。

Method: RCA框架包含迭代规则精炼机制（从预测错误中改进逻辑）和分布感知规则检查机制（基于数据集全局统计进行推理），通过预测准确性驱动更深层次的数据理解。

Result: 在1个私有和2个公共数据集上评估，与22个基线方法相比，RCA实现了最先进的准确性和鲁棒性，相对改进高达40%，并能生成清晰、逻辑性强、基于证据且平衡的解释。
...
### [234] [VC-Agent: An Interactive Agent for Customized Video Dataset Collection](https://arxiv.org/abs/2509.21291)
*Yidan Zhang,Mutian Xu,Yiming Hao,Kun Zhou,Jiahao Chang,Xiaoqiang Liu,Pengfei Wan,Hongbo Fu,Xiaoguang Han*

Main category: cs.AI

TL;DR: VC-Agent是一个交互式代理，能够理解用户查询和反馈，以最小化用户输入的方式检索和扩展相关视频片段，加速定制化视频数据集的收集过程。


<details>
  <summary>Details</summary>
Motivation: 面对规模扩展的需求，从互联网收集符合特定需求的视频数据非常耗时耗力，需要一种更高效的方法来加速视频数据收集过程。

Method: 利用现有的多模态大语言模型连接用户需求与视频内容，定义基于文本描述和确认的用户友好交互方式，并提出两种可随用户交互更新的新型过滤策略。

Result: 提供了个性化视频数据集收集的新基准，并通过用户研究验证了代理在各种真实场景中的有效性，实验证明该方法在定制化视频数据集收集方面具有高效性。

Conclusion: VC-Agent通过交互式方法和智能过滤策略，显著提高了定制化视频数据集收集的效率和效果，为视频数据收集提供了新的解决方案。
...
### [235] [SAGE: A Realistic Benchmark for Semantic Understanding](https://arxiv.org/abs/2509.21310)
*Samarth Goel,Reagan J. Lee,Kannan Ramchandran*

Main category: cs.AI

TL;DR: SAGE是一个新的语义理解评估基准，通过5个维度（人类偏好对齐、变换鲁棒性、信息敏感性、聚类性能、检索鲁棒性）在30+数据集上评估嵌入模型和相似度度量，揭示了现有方法的性能差距和关键权衡。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在传统基准上表现强劲，需要更挑战性的评估框架来深入探究语义理解能力。现有基准关注孤立能力，而SAGE旨在通过对抗条件、噪声变换和细致的人类判断任务来全面评估语义理解。

Method: 设计SAGE基准，包含5个评估类别，在30多个数据集上评估9个嵌入模型和经典度量方法。通过对抗条件、噪声变换等复杂场景测试模型性能。

Result: 评估显示显著性能差距：OpenAI的text-embedding-3-large在人类偏好对齐上表现最佳（0.682），但经典度量方法在信息敏感性任务上更优（Jaccard相似度0.905 vs 嵌入模型0.794）。text-embedding-3-small聚类性能最高（0.483）但鲁棒性最差（0.011）。

Conclusion: SAGE暴露了当前语义理解能力的关键局限性，没有单一方法在所有维度上都表现优异，为现实世界部署提供了更真实的模型鲁棒性评估。
