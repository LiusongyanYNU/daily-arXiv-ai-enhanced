...
Motivation: 现有方法忽视跨模态语义不一致问题（如文本和视觉输入的情感线索冲突），且常因文本模态的强表征能力而主导识别过程，影响准确性。

Method: CMC模型包含伪标签生成模块（PLGM）用于自监督单模态预训练，参数自由融合模块（PFM）和多模态共识路由器（MCR）用于多模态微调，缓解文本主导并引导融合过程达成更可靠共识。

Result: 在CH-SIMS、CH-SIMS v2、CMU-MOSI和CMU-MOSEI四个数据集上达到或超越最先进方法性能，在CH-SIMS和CH-SIMS v2的语义不一致场景中表现尤为突出。

Conclusion: CMC模型有效解决了多模态情感识别中的语义不一致和模态主导问题，通过自监督预训练和共识引导的融合策略实现了更可靠的多模态情感识别。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频异常检测方法在融合多模态信息时过于粗糙，忽略了视频片段中可能存在的大量冗余信息，需要更精细的特征提取来提升检测效果

Method: 基于视频片段生成细粒度多模态特征，总结主要内容，并引入原始视频字幕的文本特征来增强视觉特征中的重点部分

Result: 在四个主要数据集上实现了最先进的性能，消融实验验证了性能提升确实源于冗余信息的减少

Conclusion: 通过细粒度多模态特征减少冗余信息能有效提升视频异常检测性能，GMFVAD方法证明了这种策略的有效性
...
Method: 应用二维离散傅里叶变换将图像转换到频域，使用ResNet50神经网络在频域中训练模型来区分真实图像和StyleGAN生成的图像。

Result: 频域模型达到92.8%的准确率和0.95的AUC值，显著优于在原始空间域图像上训练的等效模型。

Conclusion: GAN生成的图像具有独特的频域特征，结合信号处理和深度学习的方法在增强数字取证和工业AI系统可信度方面具有工业潜力。
...
Method: 分析VCR中的因果图和预测捷径，采用后门调整方法，基于正确答案集合创建字典来消除预测捷径。

Result: 实验证明该去偏方法在不同数据集上都有效。

Conclusion: 提出的VCR-OOD数据集和去偏方法能够有效评估和改善模型的泛化能力，解决VCR任务中的偏见问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决计算机视觉方法从2D图像识别3D物体的挑战，传统CNN方法需要大量标注数据且可能忽略关键形状关系。

Method: 使用Vision Transformers架构配合对比学习目标（包括监督和自监督对比损失），进行多视角3D分析。

Result: 监督对比损失在ModelNet10上达到约90.6%的准确率，ViT结合对比学习能有效捕捉全局形状语义和局部判别特征。

Conclusion: ViT与对比学习结合的方法在3D表示学习中表现出色，通过大量实验验证了这种组合的有效性。
...
Method: 采用模块化设计，包含基于transformer的时间平滑器和融合驱动的跟踪器。使用多模态BEV融合特征，无需显式运动模型，通过几何和语义线索进行身份关联和传播。

Result: 在nuScenes测试集上达到74.7 aMOTA，显著减少身份切换，在3D MOT基准测试中表现强劲，证明多模态传感器特征相比单传感器方法有显著优势。
...
Method: 采用因果方法预测图像中可能被忽略的烹饪元素，并显式将这些元素注入跨模态表示学习以减轻偏差。在单语Recipe1M数据集和新构建的多语言多文化数据集上进行实验。

Result: 该方法能够发现细微的食材和烹饪动作，在单语和多语言多文化数据集上都取得了令人印象深刻的检索性能。

Conclusion: 因果表示学习能有效缓解图像-食谱检索中的视觉偏差问题，提高对细微差异的识别能力。
...
Method: 使用在线标签平滑(OLS)方法，在训练过程中基于模型自身的预测模式动态调整软标签，并在RadImageNet数据集上评估了ResNet-50、MobileNetV2和VGG-19三种架构。

Result: OLS在Top-1和Top-5分类准确率上均优于标准训练方法，包括硬标签、传统标签平滑和无教师知识蒸馏，同时产生了更紧凑和分离良好的特征嵌入。

Conclusion: OLS不仅提升了预测性能，还改善了模型校准，是医学影像领域开发可信AI系统的实用有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频推理模型仅生成文本推理轨迹，不指示关键证据出现的时间和位置。将证据中心推理扩展到视频更具挑战性，需要跨动态场景的联合时间追踪和空间定位。

Method: 构建两个高质量数据集STGR-CoT-30k和STGR-RL-36k，采用冷启动强化学习策略，设计多个专门奖励函数共同促进答案准确性、时间对齐和空间精度。

Result: 在V-STAR基准测试中达到最先进性能，mAM提升14.4%，mLGM提升24.2%。在多个视频理解基准上均观察到一致改进。

Conclusion: Open-o3 Video不仅提高了准确性，其生成的推理轨迹还为测试时扩展提供了有价值信号，支持置信度感知验证并提高答案可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 鱼眼相机在广域交通监控中具有优势，但其强径向畸变和边界区域物体外观退化给标准目标检测器带来挑战。

Method: 采用简单有效的预处理和后处理管道，训练多个先进检测模型并通过集成策略结合它们的输出。

Result: 在2025 AI City Challenge Track 4中获得F1分数0.6366，在62个团队中排名第8。

Conclusion: 该框架有效解决了鱼眼图像固有的检测问题，在挑战性条件下表现出良好的鲁棒性。
...
Method: 利用中间梯度计算敏感度分数，专门针对各向异性方向引起的不稳定性进行过滤，而不是各向同性方差。

Result: 实验表明该方法相比现有NeRF方法（如BayesRays）显著提高了视觉质量、真实感和一致性，并能实时集成到现有3DGS渲染管线中。

Conclusion: 该方法有效解决了生成不确定性核心问题，使3D重建系统在用户自由导航到原始训练视角外时仍能保持高视觉保真度。
...
<details>
  <summary>Details</summary>
Motivation: 传统物理方法受颅骨引起的信号衰减和空间覆盖不完整限制，纯数据驱动方法在低信噪比和稀疏孔径条件下会产生定量偏差。

Method: 第一阶段使用逆时偏移处理多角度采集数据生成迁移片段；第二阶段使用基于transformer的超分辨率编码器-解码器与图注意力单元融合片段。

Result: 在两个合成数据集上的实验显示，BrainPuzzle实现了优越的声速重建精度和图像完整性。

Conclusion: 该方法展示了推进定量超声脑成像的潜力。
...
Result: 研究发现：1）国家无关提示下模型默认全球北方现代描绘；2）迭代编辑侵蚀文化保真度；3）模型仅应用表面线索而非上下文感知变化，对全球南方目标常保留源身份。

Conclusion: 当前系统中的文化敏感编辑仍不可靠，通过发布标准化数据、提示和人工评估协议，为诊断和跟踪生成图像模型中的文化偏见提供了可复现的文化中心基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于神经网络的事件图像重建方法通常需要GPU，计算复杂。本文旨在开发一种更简单、高效的异步重建方法，能够在CPU上实时处理事件流。

Method: 使用时间数字IIR滤波器积分事件强度变化，通过新颖算法检测陈旧像素并调节最近更新像素窗口，对陈旧像素应用高斯滤波以减少重建噪声。

Result: FIBAR在笔记本电脑CPU上处理速度达42-140百万事件/秒，虽然重建图像比神经网络方法更嘈杂且存在重影，但足以完成某些任务如基准标记检测。

Conclusion: FIBAR提供了一种简单高效的异步事件图像重建方案，无需GPU，适合实时应用，尽管质量不如神经网络方法，但在特定任务中足够有效。
...
<details>
  <summary>Details</summary>
Motivation: 现有张量低秩表示方法依赖固定变换，对噪声鲁棒性差，需要更有效捕获全局和局部相关性的方法。

Method: 提出TBTLRR模型，引入数据自适应张量核范数，学习任意酉变换；利用张量数据的双边结构；整合ℓ1/2-范数和Frobenius范数正则化；基于ADMM开发高效优化算法。
...
<details>
  <summary>Details</summary>
Motivation: 视频评估和手术数据科学可推进手术培训和研究，但受限于异构记录格式和视频共享的隐私问题，需要标准化且保护隐私的解决方案。

Method: 采用软件开发生命周期和用户中心反馈，通过隐私设计架构开发跨平台应用，支持视频合并、标准化和去标识化，并进行可用性测试和性能基准测试。

Result: 用户测试显示高可用性评分（4.38/5），强推荐度（9.2/10），处理时间受处理模式、视频时长和机器性能影响，软件已公开发布。

Conclusion: Endoshare提供了透明、用户友好的标准化隐私保护手术视频管理流程，需要合规认证和更广泛互操作性验证以替代专有系统。
...
TL;DR: 本文提出了Attentive Convolution (ATConv)，一种新的卷积算子，通过引入自适应路由和侧向抑制机制来弥补传统卷积与自注意力机制之间的性能差距。


<details>
  <summary>Details</summary>
Motivation: 自注意力机制虽然表达能力强，但二次复杂度限制了实际应用。传统卷积具有线性复杂度，但性能一直无法与自注意力机制相媲美。本文旨在重新设计卷积，使其具备自注意力的关键优势。

Method: 通过分析自注意力的工作原理，识别出两个关键原则：自适应路由和侧向抑制。基于这些原则设计了ATConv，这是一种重新制定的卷积算子，能够动态调节位置信息流并抑制冗余表示。

Result: ATConv在仅使用3×3卷积核的情况下，在基础视觉任务中持续优于各种自注意力机制。基于ATConv构建的AttNet在ImageNet-1K上达到84.4%的Top-1准确率，参数量仅为27M。在扩散模型生成任务中，用ATConv替换自注意力机制可降低FID并加速采样。

Conclusion: ATConv成功地将自注意力的关键优势融入卷积操作中，在保持线性复杂度的同时实现了与自注意力机制相当甚至更好的性能，为视觉主干网络设计提供了新的方向。
...
<details>
  <summary>Details</summary>
Motivation: 尽管扩散模型在图像生成方面取得了显著进展，但在合成基于像素的手绘草图（抽象表达的代表性示例）方面仍然存在挑战。
...
Conclusion: 该框架成功解决了手绘草图生成的问题，并引入了首个包含实例级草图与标题、问答对的数据集SketchDUO，克服了现有数据集依赖图像-标签对的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 生物多模态基础模型缺乏实例特定的描述性标题作为监督信号，这限制了自然语言监督在生物领域的应用。图像和标题可以视为物种潜在形态空间中的互补样本，各自捕捉不同的生物特征。

Method: 使用多模态大语言模型生成合成标题，结合维基百科的视觉信息和针对特定分类群定制的格式示例来指导生成过程，减少幻觉并产生准确的实例描述性标题。

Result: 训练出的BIOCAP模型能够捕捉丰富的语义信息，在物种分类和图文检索任务中表现出色。

Conclusion: 描述性标题超越了传统标签的价值，能够有效桥接生物图像与多模态基础模型，强调了潜在形态空间对齐的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 计算机视觉在一般物体检测和跟踪方面已有显著进展，但快速移动的小物体检测问题仍未充分探索，特别是在RGB-D相机环境下。

Method: 结合深度学习检测与基于物理学的跟踪算法，整合运动学方程处理异常值和漏检，并包含异常检测和校正模块以应对遮挡和快速方向变化等挑战场景。

Result: 在自定义壁球数据集上的评估显示，系统比基于卡尔曼滤波器的跟踪器减少了高达70%的平均位移误差。

Conclusion: 该系统在自主平台的机器人感知方面具有重要应用价值，证明了将物理模型与深度学习方法结合用于实时3D检测和跟踪挑战性小物体的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 传统光场获取需要计算成本或专门设备，限制了其应用范围。本文旨在从单张图像生成光场，扩大其适用性。

Method: 设计神经渲染管线，通过交叉注意力计算源光线关系，预测目标光线颜色，并迭代更新生成内容以确保遮挡一致性。

Result: 在多个挑战性数据集上表现良好，无需重新训练或微调，优于相关最先进的新视角合成方法。

Conclusion: 逆图像渲染方法有效实现了从单张图像生成光场，展示了在合成数据集上训练后的良好泛化能力。
...
Method: 提出LogitGap方法，显式利用最大logit与剩余logits的关系；引入无训练策略自动识别最有信息的logits子集进行评分。

Result: 在视觉语言和纯视觉模型上的广泛实验表明，LogitGap在各种OOD检测场景和基准测试中持续达到最先进性能。
...
Method: 引入PartNeXt数据集，包含纹理3D模型和分层部件标注，在类无关部件分割和3D部件中心问答两个任务上进行基准测试。

Result: 现有方法在细粒度部件分割上表现不佳，3D-LLMs在开放词汇部件定位方面存在显著差距，使用PartNeXt训练Point-SAM相比PartNet有显著提升。
...
Result: 该方法在8D姿态参数估计精度方面表现出有希望的结果，与使用刚性规范对象模板进行匹配的最先进类别级6D姿态估计器相比，取得了竞争性分数。

Conclusion: 该方法能够从单张图像估计更细粒度的自行车姿态状态和行驶方向，为自动驾驶系统中的骑行者行为理解和安全决策提供了更准确的信息。
...
<details>
  <summary>Details</summary>
Motivation: 解决组合零样本学习中由于未见属性-对象组合导致的标签空间分布偏移问题，现有方法在测试时性能下降严重。

Method: 1) 累积文本和视觉模态的无监督知识更新多模态原型；2) 设计自适应更新权重控制原型调整程度；3) 引入动态优先级队列存储高置信度图像；4) 通过多模态协作表示学习对齐文本和视觉原型。

Result: 在四个基准数据集上，在封闭世界和开放世界设置下均达到了最先进的性能。

Conclusion: 该方法通过累积多模态知识和自适应原型更新，有效解决了组合零样本学习中的分布偏移问题，显著提升了模型性能。
...
Method: 在生成器中间层引入随机层作为可学习的潜在分布，与生成器联合端到端训练，约束输入与生成输出之间的互信息，形成解耦的潜在空间。
...
<details>
  <summary>Details</summary>
Motivation: 立体视频中的时序一致性深度估计对增强现实等应用至关重要，但现有方法在长程时序建模和计算效率之间存在权衡。
...
Conclusion: PPMStereo通过紧凑而信息丰富的记忆缓冲区实现了时序一致的深度估计，在精度和时序一致性方面达到最先进水平。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注单个主体而非多交互主体的场景，多智能体动态在生成视频中的合理性尚未得到验证。

Method: 为I2V使用现有数据集起始帧与真实视频比较，为T2V开发提示套件探索不同行人密度和交互，提出从像素空间重建2D鸟瞰轨迹的方法。

Result: 分析显示领先模型已学习到令人惊讶的有效多智能体行为先验，但合并和消失行人等失败模式仍需改进。

Conclusion: 该研究为评估视频生成模型作为行人动态模拟器提供了严格基准，揭示了当前模型的优势和局限。
...
<details>
  <summary>Details</summary>
Motivation: 现有离散分类方法无法捕捉可疑意图的连续性，限制了早期干预和可解释性。

Method: 基于时序点过程理论，定义可疑分数公式建模连续变化；引入可疑系数调制和多模态信息；提出概念锚定映射方法。

Result: 在HAI数据集上，MSE降低19.8%，平均mAP提升1.78%，低频情况下mAP增益达2.74%。

Conclusion: 连续可疑建模方法能实现更早检测和主动干预，显著提升安全应用中的可解释性和实用性。
...
Method: 三阶段框架：1) RAPO阶段通过检索增强和重构使提示与训练分布匹配；2) SSPO阶段使用多源反馈迭代优化提示；3) 使用优化后的提示对微调LLM，内化优化模式。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于文本的图像编辑方法采用目标无关的中间状态构建方式，主要关注源图像重建而忽略了与特定编辑目标之间的语义差距，导致在需要大幅修改时编辑能力有限或不一致。

Method: FlowCycle框架通过可学习的噪声参数化图像破坏过程，采用循环一致性优化方法，在源图像到目标图像的编辑和反向恢复过程中施加双重一致性约束，学习生成目标感知的中间状态。

Result: 广泛的消融实验表明，FlowCycle在编辑质量和一致性方面优于现有的最先进方法。

Conclusion: 通过引入目标感知的中间状态构建和循环一致性优化，FlowCycle能够实现更忠实的内容修改，同时保持源图像的一致性，解决了现有方法在语义差距较大时编辑效果不佳的问题。
...
Method: 使用对比超声视频表示学习（CURL）框架，结合空间和时间双重对比损失，采用任务特定采样策略和概率微调方法。

Result: 在92个30分钟超声会话数据集上，CULR达到78.01%的敏感度和81.60%的AUROC。

Conclusion: 自监督对比学习在胎儿运动分析中具有潜力，为改善产前监测和临床决策铺平道路。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的图像编辑方法在图像反演过程中存在近似误差，这限制了编辑性能。作者希望利用 VQ 生成模型能够获取精确中间量化表示的特性，实现更有效的监督和精确的图像反演。

Method: 提出 EditInfinity 方法，基于二进制量化生成模型 Infinity，设计了集成文本提示校正和图像风格保持的高效图像反演机制，并开发了整体平滑策略来保证编辑质量。

Result: 在 PIE-Bench 基准测试中，在"添加"、"更改"和"删除"编辑操作上，EditInfinity 相比最先进的基于扩散模型的基线方法表现出优越性能。

Conclusion: EditInfinity 通过精确的图像反演和整体平滑策略，成功解决了扩散模型在图像编辑中的近似误差问题，实现了高质量的文本驱动图像编辑。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在生成长文本响应时容易出现幻觉问题，传统观点认为是长度导致的错误累积，但作者质疑是否存在更深层机制。

Method: 提出"诱导-检测-抑制"框架：通过设计特定上下文主动诱导幻觉，利用诱导实例进行早期高风险检测，在实际解码过程中抑制对象级幻觉。

Result: 该方法在所有基准测试中都取得了显著且一致的改进，验证了框架的有效性。

Conclusion: 幻觉风险主要源于长文本对上下文连贯性的依赖，而非长度本身。该研究为深入探索大型视觉语言模型长文本响应中的幻觉问题提供了新视角。
...
TL;DR: COS3D是一个协作式提示-分割框架，通过实例场和语言场的协作，有效整合语言和分割线索，解决开放词汇3D分割的挑战。
...
Method: 引入协作场的概念，包含实例场和语言场；通过实例到语言特征映射和两阶段训练策略构建协作场；推理时设计自适应语言到实例提示优化。

Result: 在两个广泛使用的基准测试上表现出领先性能，并在多种应用中展示高潜力。

Conclusion: COS3D通过协作框架有效整合语言和分割线索，在开放词汇3D分割任务中取得优异效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在跨模态注意力中统一处理所有文本标记，忽视了它们不同的语义角色，导致模型过度依赖[EOS]驱动的全局语义而无法有效利用词级信号，限制了细粒度时序对齐能力。
...
Result: 在两个公开数据集上实现最先进性能，在具有挑战性的地面到卫星场景下定位准确率提升3.39%
...
Result: 模型在包含30类纸币和硬币的数据集上取得了97.73%的准确率、95.23%的召回率、95.85%的F1分数和97.21%的mAP50(B)。

Conclusion: 通过语音反馈的货币检测系统能够有效帮助视障人士独立处理货币，提高他们的生活自主性。
...
Result: 在Gaze360上提升10.9%，在MPIIFaceGaze上超越最佳结果3.8%，在ETH-XGaze子集上领先11.6%，在RAF-DB和Affectnet上分别达到86.4%和60.9%的准确率

Conclusion: 消融研究证实了SLYKLatent新颖组件的有效性，该方法显著提升了视线估计性能
...
<details>
  <summary>Details</summary>
Motivation: 传统数据驱动模型未能充分利用CV-SAR数据中丰富的电磁散射特征，导致在数据有限和域偏移场景下面临泛化性、可解释性和效率的表示三难问题。

Method: 采用'压缩-聚合-压缩'架构：第一阶段进行物理引导压缩，使用字典处理器嵌入物理先验；第二阶段聚合增强表示；第三阶段通过自蒸馏的紧凑分类头进行语义压缩。

Result: 在五个SAR基准测试中，KINN在参数高效识别方面达到最先进水平，在数据稀缺和分布外场景下表现出优异的泛化能力和可解释性。

Conclusion: KINN有效解决了CV-SAR图像识别中的表示三难问题，为SAR图像分析中的可信AI提供了新路径。
...
TL;DR: 该研究评估了现代视频基础模型在街舞分类任务中的表现，发现视频编码器模型在预测任务上仍优于最先进的视频语言模型，并提供了编码器选择指南和解码器模型微调分析。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在体育应用中已有广泛应用，但主要集中在足球、板球、篮球等流行运动，且侧重于视觉问答、精彩片段生成等生成任务。本研究旨在分析现代视频基础模型在街舞这一小众但流行的舞蹈运动中的适用性。

Method: 使用现代视频基础模型（包括编码器和解码器模型）对街舞视频进行分类，比较视频编码器模型与视频语言模型的性能表现，并对微调后的解码器模型进行深入分析。

Result: 研究结果显示，视频编码器模型在预测任务上继续优于最先进的视频语言模型。

Conclusion: 为街舞视频分类任务提供了编码器模型选择指南，并对解码器模型的微调机制进行了详细分析，为小众体育运动的视频分析提供了实用见解。
...
<details>
  <summary>Details</summary>
Motivation: 解决跨模态地理定位中的两个主要挑战：严重的平台间异构性（卫星/无人机/地面）以及通用训练描述与平台特定测试查询之间的领域差距。

Method: 采用领域对齐预处理管道（平台划分、卫星增强、方向词去除）和LLM驱动的标题精炼管道，使用BGE-M3和EVA-CLIP训练三个平台专家，采用渐进式两阶段硬负样本挖掘策略，并在推理时融合专家分数。

Result: 系统在官方排行榜上排名第一，展示了在异构视角下稳健的跨模态地理定位能力。

Conclusion: 提出的领域对齐预处理和混合专家框架有效解决了跨模态无人机导航中的平台异构性和领域差距问题，实现了优异的性能表现。
...
Method: 利用双曲空间建模层次结构，通过动态双曲半径调整优化视觉表示；采用可学习矩阵和Möbius乘法操作，提供三种参数化策略：对角缩放矩阵、块对角矩阵和带状矩阵。

Result: 在多个MLLM基准测试中，HyperET仅增加不到1%参数就能显著提升现有预训练和微调MLLMs的性能。

Conclusion: 双曲空间为弥合视觉和文本模态之间的粒度差距提供了原则性框架，HyperET实现了高效的多粒度跨模态对齐。
...
TL;DR: AnyPcc是一个通用的点云压缩框架，通过通用上下文模型和实例自适应微调策略解决了点云几何压缩中的泛化挑战。


<details>
  <summary>Details</summary>
Motivation: 解决基于深度学习的点云几何压缩中的泛化问题，主要源于缺乏鲁棒的上下文模型和对分布外数据处理的低效性。

Method: 使用通用上下文模型捕获空间和通道分组的先验知识，并采用实例自适应微调策略，结合显式和隐式压缩范式，为每个实例微调网络权重子集。

Result: 在包含15个不同数据集的基准测试中，AnyPcc在点云压缩方面达到了新的最先进水平。

Conclusion: AnyPcc通过创新的上下文建模和自适应微调方法，有效解决了点云压缩的泛化挑战，为可重复研究提供了代码和数据集。
...
TL;DR: AccuQuant是一种新颖的扩散模型后训练量化方法，通过模拟多步去噪过程来最小化量化误差累积，显著提升量化模型性能。
...
Method: AccuQuant通过显式模拟扩散采样过程中的多个去噪步骤进行量化，考虑多步累积误差。提出高效实现技术，将内存复杂度从O(n)降低到O(1)。

Result: 在多个任务和扩散模型的标准基准测试中，AccuQuant展现出优异的效能和效率。

Conclusion: AccuQuant通过解决量化误差累积问题，为扩散模型提供了有效的后训练量化解决方案，在保持效率的同时显著提升量化性能。
...
TL;DR: 本文发现DiT中的patch tokens具有高度独立性，空间一致性主要由位置编码控制，因此提出了PE-Field方法将2D位置编码扩展到3D场，在单图像新视角合成和可控空间图像编辑上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 研究发现DiT中的patch tokens即使位置编码被扰动仍能产生全局一致输出，表明空间一致性主要由位置编码控制，这启发了将位置编码从2D扩展到3D的想法。

Method: 提出了Positional Encoding Field (PE-Field)，将位置编码从2D平面扩展到结构化3D场，包含深度感知编码用于体积推理和分层编码用于细粒度子patch控制。

Result: PE-Field增强的DiT在单图像新视角合成任务上达到最先进性能，并能泛化到可控空间图像编辑任务。

Conclusion: PE-Field通过将位置编码扩展到3D场，使DiT能够直接在3D空间中建模几何，为视觉生成提供了新的架构改进方向。
...
Method: 1. 使用ViT-B32作为教师模型，MobileNet作为学生模型；2. 引入动态模糊逻辑调整蒸馏权重；3. 采用动态等待调整机制优化训练过程；4. 使用Gamma校正和直方图均衡化进行图像增强；5. 基于小波变换的图像融合方法；6. 使用遗传算法从12个候选模型中选择最优学生模型。

Result: 在LC25000组织病理学图像数据集上达到99.16%准确率，在IQOTH/NCCD CT扫描图像数据集上达到99.54%准确率，证明了模型在不同成像领域的鲁棒性。

Conclusion: 提出的动态模糊逻辑知识蒸馏方法有效提升了肺癌分类性能，能够处理医学图像中的不确定性，并在不同数据集上表现出优异的泛化能力和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法依赖纯文本链导致结论不可靠，而帧检索方法虽然引入视觉基础但证据定位不准确，需要一种结合视觉基础和准确推理的方法。

Method: 构建了Conan-91K大规模自动生成推理轨迹数据集，设计了多阶段渐进冷启动策略和识别-推理-动作强化学习训练框架。

Result: 在六个多步推理基准测试中，Conan比基线Qwen2.5-VL-7B-Instruct平均准确率提升超过10%，达到最先进性能。

Conclusion: Conan在视频推理任务中表现出色，并能有效泛化到长视频理解任务，验证了其强大的可扩展性和鲁棒性。
...
TL;DR: 提出一个可复现的人口属性推断管道，通过模块化迁移学习方法改进人脸分析系统的公平性评估可靠性。


<details>
  <summary>Details</summary>
Motivation: 人脸分析系统的公平性评估依赖于人口属性推断的可靠性，但现有方法存在局限性，需要更可靠的推断方法来减少公平性估计的偏差和方差。

Method: 使用模块化迁移学习方法，将预训练的人脸识别编码器与非线性分类头结合，替代传统的端到端训练方式。

Result: 该方法在多个数据集和训练设置下的性别和种族推断任务中表现优于强基线，特别是在更具挑战性的种族属性上。

Conclusion: 这项工作为公平性审计中的人口属性推断提供了可靠的基础，并公开了数据集元数据、代码库、预训练模型和评估工具包以促进透明性和可复现性。
...
TL;DR: EchoDistill是一个双向概念蒸馏框架，用于实现一步扩散个性化(1-SDP)，通过师生模型协同训练，在保持快速生成的同时有效捕捉新概念。


<details>
  <summary>Details</summary>
Motivation: 解决一步扩散模型在个性化新概念时容量有限的问题，需要在保持快速生成的同时提高概念捕捉能力。

Method: 采用端到端训练，多步扩散模型(教师)和一步扩散模型(学生)同时训练，通过双向概念蒸馏和回声反馈机制，共享文本编码器，结合对抗损失和对齐损失优化。
...
TL;DR: 提出Metis-HOME框架，通过混合专家架构解决多模态推理模型在效率与泛化能力之间的权衡问题，包含专门用于复杂推理的思考分支和用于快速推理的非思考分支。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大推理模型存在两个关键局限：对简单查询使用计算昂贵的推理导致效率低下，以及专注于专门推理任务损害了更广泛的通用理解能力。

Method: 将原始密集模型重构为两个专家分支：思考分支用于复杂多步推理，非思考分支用于快速直接推理（如通用VQA和OCR），通过轻量级可训练路由器动态分配查询。

Result: 评估表明该方法不仅显著增强了复杂推理能力，还改善了模型的通用能力，逆转了其他推理专用模型中观察到的性能退化趋势。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在可解释深度伪造分析中存在细粒度感知不足的问题，包括数据标注不可靠、无法建立文本解释与视觉证据的联系，以及不支持任意面部区域的查询。

Method: 定义了面部图像概念树进行细粒度区域划分，开发了FiFa-Annotator数据标注流程，提出了伪影定位解释任务，并构建了统一的多任务学习架构FiFa-MLLM。

Result: FiFa-MLLM在AGE任务上优于强基线模型，在现有XDFA数据集上达到了最先进的性能。

Conclusion: FiFa框架通过细粒度数据标注和统一的多任务架构，显著提升了可解释深度伪造分析的性能，解决了视觉上下文基础不足的问题。
...
TL;DR: 提出了一种联合估计清晰图像和相机运动轨迹的深度学习框架，通过可解释的模块化架构实现运动去模糊，在严重模糊情况下达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决相机抖动（特别是大范围或旋转运动）导致的运动模糊问题，这是图像恢复中的主要挑战。现有端到端去模糊网络在严重或空间变化模糊情况下效果不佳。

Method: 使用投影运动模糊模型(PMBM)，通过可微分模糊创建模块实现。神经网络预测完整的3D旋转轨迹，指导基于模型的端到端恢复网络。后处理阶段通过重模糊损失优化轨迹以提高一致性。

Result: 在合成和真实数据集上均达到最先进性能，特别是在严重或空间变化模糊情况下表现优异，优于端到端去模糊网络。

Conclusion: 该方法通过联合估计清晰图像和相机运动轨迹，提供了可解释的去模糊解决方案，能够重建生成模糊图像的清晰图像序列，在挑战性模糊场景中表现出色。
...
TL;DR: SELM-SLAM3是一个深度学习增强的视觉SLAM框架，在低纹理、运动模糊等挑战性条件下显著优于传统SLAM方法，特别适用于视障人士的辅助导航。


<details>
  <summary>Details</summary>
Motivation: 传统SLAM技术在低纹理、运动模糊和挑战性光照条件下表现不佳，这影响了视障人士辅助导航系统的可靠性和安全性。
...
Conclusion: 该框架在挑战性条件下表现出色，为开发视障人士导航辅助工具提供了可靠平台。
...
TL;DR: ACamera-Net是一个轻量级、场景自适应的相机参数调整网络，直接从RAW输入预测最佳曝光和白平衡参数，提升复杂光照条件下的图像质量。


<details>
  <summary>Details</summary>
Motivation: 消费级相机系统在复杂光照条件（如低光、高动态范围、背光）下难以保持稳定的图像质量，导致曝光不足、色偏和色调不一致，影响下游视觉任务性能。

Method: 提出ACamera-Net框架，包含两个模块：ACamera-Exposure估计ISO以缓解曝光不足和对比度损失，ACamera-Color预测相关色温和增益因子以改善色彩一致性。针对边缘设备实时推理优化，可无缝集成到成像管道中。

Result: 在多样化真实世界数据上训练，模型在不同光照条件下泛化良好。大量实验表明ACamera-Net持续提升图像质量并稳定感知输出，优于传统自动模式和轻量级基线方法，无需依赖额外图像增强模块。

Conclusion: ACamera-Net通过直接预测相机参数有效解决了复杂光照条件下的图像质量问题，为消费级相机系统提供了轻量级、自适应的解决方案。
...
TL;DR: 研究用户对不同细节层次(LoD)和观看距离下人群角色视觉质量的感知，比较几何网格、图像替身、NeRF和3D高斯等表示方法的视觉保真度与计算性能权衡


<details>
  <summary>Details</summary>
Motivation: 为人群渲染设计感知优化的细节层次策略提供指导，理解不同表示方法在视觉质量与性能之间的权衡关系

Method: 使用定性分析和定量评估方法，比较几何网格、图像替身、NeRF和3D高斯四种表示在不同LoD和观看距离下的表现

Result: 每种表示方法都展现出视觉保真度和计算性能之间不同的权衡特性，为优化策略提供了具体依据
...
TL;DR: 提出了EmbodiedBrain，一个用于具身AI的新型视觉语言基础模型，通过创新的训练方法和评估系统，在具身任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 解决当前大语言模型和多模态大语言模型在具身任务中的关键限制，包括模型设计与代理需求之间的差距、实时延迟与性能之间的权衡以及离线评估指标的不真实性。

Method: 采用代理对齐的数据结构，结合大规模监督微调(SFT)和步骤增强的组相对策略优化(Step-GRPO)，集成生成奖励模型(GRM)以提升训练效率。
...
TL;DR: 提出了GenColorBench，首个全面的文本到图像颜色生成基准，包含44K个颜色相关提示，评估模型在精确颜色生成方面的能力。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成模型在细粒度颜色可控性方面表现不佳，缺乏系统评估颜色精度的基准，而颜色在视觉感知和应用中至关重要。

Method: 基于ISCC-NBS和CSS3/X11颜色系统构建基准，包含400多种颜色的44K个颜色焦点提示，通过感知和自动化评估揭示模型能力。
...
TL;DR: 提出基于相似性原型的跨模态分割框架，通过类原型学习和相似性约束来解决领域适应问题


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在领域偏移时性能显著下降，无监督领域适应可以减少领域差距并避免新领域的高昂标注成本

Method: 在嵌入空间中学习类原型，引入相似性约束使原型具有代表性且类间可分，使用字典存储不同图像的原型以解决类别缺失问题并支持对比学习

Result: 大量实验表明该方法优于其他最先进方法

Conclusion: 基于相似性原型的框架在跨模态分割任务中表现出色，能够有效应对领域适应挑战
...
TL;DR: OnlineSplatter是一种在线前馈框架，直接从RGB视频帧生成高质量的对象中心3D高斯表示，无需相机姿态、深度先验或束优化。
...
Method: 使用第一帧作为锚点，通过密集高斯基元场逐步细化对象表示，采用双键记忆模块结合潜在外观-几何键和显式方向键，实现当前帧特征与时间聚合对象状态的鲁棒融合。

Result: 在真实世界数据集上的评估显示，OnlineSplatter显著优于最先进的无姿态重建基线方法，随着观察次数增加性能持续提升，同时保持恒定的内存和运行时间。

Conclusion: 该方法能够有效处理自由移动物体的重建，通过空间引导的内存读取和高效稀疏化机制确保全面而紧凑的对象覆盖。
...
<details>
  <summary>Details</summary>
Motivation: 长视频理解面临计算复杂度高、内容分散、推理不一致的挑战，现有帧选择方法忽略时间依赖或仅依赖单模态证据，无法提供完整且与查询相关的上下文。

Method: 包含两个关键模块：SVCFS通过时间感知语义分支和聚类引导视觉分支选择帧；ACR通过融合证据和约束答案空间来解决语义和视觉预测之间的不一致性。
...
TL;DR: 这篇论文系统综述了基于深度学习的牙科图像分析研究，涵盖了260项研究，包括49篇公开数据集论文和211篇深度学习算法论文，总结了该领域的数据集、模型架构、训练方法和性能评估。
...
Method: 系统回顾260项研究，分析公开牙科数据集特性和获取方法，总结深度学习基础技术，按不同牙科图像分析任务对模型算法进行分类，分析网络架构、优化策略、训练方法和性能。

Result: 全面总结了牙科图像分析领域的深度学习研究现状，包括数据集特征、模型分类、训练评估指标等，为研究人员提供了系统的参考框架。

Conclusion: 论文讨论了当前研究面临的挑战，并指出了未来潜在的研究方向，所有补充材料和详细比较表格将在GitHub上公开。
...
TL;DR: BTB3D是一种用于3D医学影像的因果卷积编码器-解码器，通过频率感知的体素标记和三级训练课程，在报告生成和文本到CT合成任务上实现了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前方法在处理高分辨率、长序列3D医学影像时存在视觉编码器与临床语言不对齐、切片标记模糊精细解剖结构等问题，导致下游任务诊断性能下降。

Method: 提出BTB3D模型，采用因果卷积编码器-解码器架构，通过三级训练课程：(i)局部重建、(ii)重叠窗口平铺、(iii)长上下文解码器精炼，从短切片片段学习并泛化到超过300切片的扫描。

Result: 在报告生成任务中，BLEU分数提升，临床F1分数比CT2Rep、CT-CHAT和Merlin提高40%；在文本到CT合成任务中，FID降低75%，FVD减半，生成解剖一致的512*512*241体积。

Conclusion: 精确的三维标记化（而非仅更大的语言骨干网络）对于3D医学影像中可扩展的视觉语言建模至关重要。
...
TL;DR: 提出了UltraHR-100K超高清文本到图像数据集和频率感知后训练方法，解决超高分辨率图像生成中的数据集缺失和细节合成问题。


<details>
  <summary>Details</summary>
Motivation: 当前超高分辨率文本到图像生成面临两大挑战：缺乏大规模高质量数据集，以及缺乏针对超高分辨率场景下细粒度细节合成的专门训练策略。

Method: 1) 构建UltraHR-100K数据集，包含10万张3K以上分辨率的超高清图像；2) 提出频率感知后训练方法，包括细节导向时间步采样(DOTS)和软加权频率正则化(SWFR)，利用离散傅里叶变换约束频率分量。

Result: 在提出的UltraHR-eval4K基准测试上，该方法显著提升了超高清图像生成的细粒度细节质量和整体保真度。

Conclusion: 通过高质量数据集和专门的频率感知训练策略，有效解决了超高分辨率文本到图像生成中的细节合成挑战。
...
TL;DR: 提出HybridSOMSpikeNet混合深度学习框架，用于智能高效的垃圾分类，准确率达97.39%，优于现有方法且计算轻量。


<details>
  <summary>Details</summary>
Motivation: 解决垃圾分类错误导致的填埋场堆积、回收效率低下和温室气体排放增加等环境问题，实现可持续废物管理。

Method: 结合卷积特征提取、可微分自组织和脉冲时序处理的混合框架，使用ResNet-152主干网络提取空间特征，Soft-SOM增强聚类可解释性，脉冲神经网络头处理时序激活。

Result: 在十类废物数据集上达到97.39%的测试准确率，优于多个最先进架构，同时保持轻量计算特性。

Conclusion: 该框架通过精确自动化垃圾分类支持更高回收效率，减少可回收物污染，降低废物处理生态成本，符合联合国可持续发展目标。
...
Method: 1. 权重偏置校正：通过中和跨比特宽度的量化诱导偏置并对齐激活分布，实现共享批量归一化，无需微调。2. 比特级核心集采样策略：利用隐式知识转移现象，通过基于梯度的重要性评分选择紧凑信息子集，让每个子模型在精选数据上训练。

Result: 在CIFAR-10/100、TinyImageNet和ImageNet-1K数据集上，使用ResNet和ViT架构的实验表明，该方法在实现竞争性或更优准确度的同时，将训练时间减少高达7.88倍。
...
TL;DR: 该论文提出了一个基于代理的架构，结合LLM推理和轻量级视觉模块，通过细粒度分析和迭代优化推理链来解决多模态大语言模型中的视觉幻觉和文本依赖问题。
...
Method: 采用三阶段评估框架诊断模型失败模式，提出结合LLM推理与轻量级视觉模块的代理架构，实现细粒度分析和推理链迭代优化。

Result: 在MMMU基准上提升10.3分，在MathVista上提升6.0分（基于7B基线），性能匹配或超越更大模型。

Conclusion: 未来视觉推理模型应专注于整合更广泛的视觉内容分析专用工具，作者将发布框架和评估套件以促进研究。
...
Conclusion: MixKV通过混合重要性和多样性，有效解决了多模态KV缓存压缩中的语义覆盖问题，并可无缝扩展到LLMs中实现可比性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 传统LiDAR投影方法存在几何不一致性，导致不可逆的信息损失，影响高保真应用。需要一种能够在不依赖制造商信息的情况下实现无损投影的通用方法。
...
Conclusion: 从近似到无损LiDAR投影的范式转变为需要完全几何保真的高精度遥感应用开辟了新的可能性。
...
<details>
  <summary>Details</summary>
Motivation: 解决长时域驾驶场景生成中几何一致性和真实性的挑战，提供可靠的场景外观和几何锚点。

Method: 1) 在共享潜在空间中联合处理图像和深度；2) 显式条件化先前生成关键帧的几何信息；3) 使用扭曲一致性引导采样过程；4) 通过视频扩散模型插值生成密集视频帧。

Result: 生成超过20秒的现实且几何一致的驾驶视频，在长时域FID和FVD指标上分别比现有最优方法提升48.6%和43.0%。

Conclusion: AutoScape通过几何一致的关键帧生成和视频插值，有效解决了长时域驾驶场景生成的挑战，显著提升了生成质量。
...
TL;DR: DyPE是一种无需训练的方法，通过动态调整扩散模型中位置编码的频率谱，使预训练扩散变换器能够生成远超训练分辨率的图像，且不增加采样成本。
...
Method: 利用扩散过程固有的频谱进展特性，在每一步扩散过程中动态调整位置编码的频率谱，使其与当前生成阶段匹配。

Result: 能够生成远超训练分辨率的图像（如使用FLUX生成1600万像素图像），在多个基准测试中持续提升性能，在超高分辨率图像生成方面达到最先进的保真度。

Conclusion: DyPE方法有效解决了扩散变换器在超高分辨率图像生成中的扩展性问题，且无需额外训练，在高分辨率下性能提升更加显著。
...
TL;DR: α-Flow是一种新的生成建模目标函数，通过课程学习策略从轨迹流匹配平滑过渡到MeanFlow，解决了优化冲突问题，在ImageNet-256上取得了新的SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 发现MeanFlow目标函数中的轨迹流匹配和轨迹一致性项存在强烈的负相关性，导致优化冲突和收敛缓慢，需要新的方法来解耦这些冲突目标。

Method: 提出α-Flow目标函数族，统一了轨迹流匹配、Shortcut Model和MeanFlow，采用课程学习策略从轨迹流匹配平滑过渡到MeanFlow。

Result: 在ImageNet-1K 256x256上，α-Flow始终优于MeanFlow，最大的α-Flow-XL/2+模型使用1步和2步推理分别达到FID 2.58和2.15的新SOTA结果。
...
TL;DR: Cupid是一种基于生成的单图像3D重建方法，通过条件采样过程联合估计相机位姿、3D形状和纹理，在统一生成框架下实现鲁棒的姿态和形状估计。


<details>
  <summary>Details</summary>
Motivation: 解决从单张2D图像准确推断相机位姿、3D形状和纹理的挑战，通过生成式方法实现更鲁棒的3D重建。
...
Conclusion: Cupid通过统一的生成框架实现了从单图像到3D重建的准确推断，在多个指标上优于现有领先方法。
...
TL;DR: 提出了一种融合雷达和摄像头数据的多目标跟踪框架，通过在线雷达-摄像头校准和共同特征利用，简化传感器集成并提高跟踪精度。
...
Method: 开发雷达-摄像头融合MOT框架，利用在线校准技术简化传感器检测结果集成；使用雷达和摄像头数据的共同特征准确推导物体真实位置；采用特征匹配和类别一致性检查超越单纯位置匹配的限制。

Result: 在受控环境和实际交通场景中的实验证明，该框架能够简化雷达-摄像头映射过程并提高跟踪精度。

Conclusion: 该研究首次探索了雷达-摄像头共同特征的集成及其在在线校准中的应用，为多目标跟踪提供了有效的解决方案。
...
TL;DR: ARGenSeg：基于自回归生成的新型图像分割范式，在统一框架中实现多模态理解和像素级感知，通过图像生成方式输出密集掩码，显著提升推理速度并保持强大理解能力。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在将图像分割集成到多模态大语言模型时，依赖离散表示或语义提示输入任务特定解码器，限制了模型捕捉细粒度视觉细节的能力。

Method: 基于图像生成的分割框架，利用MLLM输出视觉token，通过通用VQ-VAE解码为图像，使分割完全依赖于MLLM的像素级理解；采用下一尺度预测策略并行生成所需视觉token以降低推理延迟。

Result: 在多个分割数据集上超越先前最先进方法，推理速度显著提升，同时保持强大的理解能力。

Conclusion: ARGenSeg通过自回归生成范式成功统一了多模态理解和像素级感知，为图像分割提供了高效且性能优越的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 受自回归大语言模型的性能和可扩展性启发，将transformer架构应用于视频预测领域，特别关注现有视频生成方法在物理模拟时间因果建模方面的不足。

Method: 使用纯transformer模型进行自回归视频预测，采用连续像素空间表示，比较不同的时空自注意力布局，无需复杂训练策略或潜在特征学习组件。

Result: 相比现有的潜在空间方法，该方法将物理准确预测的时间范围显著延长了50%，在常见视频质量指标上保持可比性能，并通过可解释性实验识别了编码PDE模拟参数信息的网络区域。
...
TL;DR: 提出Speculative Verdict (SV)框架，通过结合多个轻量级草稿专家和大型裁决模型，在信息密集型图像上实现高效多模态推理，无需训练即可提升性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在信息密集型图像（密集交织文本注释和细粒度图形元素）上推理困难，主要挑战在于精确定位关键线索和在分散证据间进行多跳推理。

Method: 采用训练免费的推测解码框架：草稿阶段使用小型VLM作为草稿专家生成多样化定位候选的推理路径；裁决阶段由强大VLM合成这些路径生成最终答案；引入共识专家选择机制，仅转发高一致性的推理路径。

Result: 在InfographicVQA、ChartMuseum、ChartQAPro和HR-Bench 4K等具有挑战性的信息密集和高分辨率视觉问答基准上取得一致性能提升，相比大型专有模型或训练流程实现了错误校正和成本效率。

Conclusion: SV通过从多个部分准确的推理路径中合成正确见解，在保持成本效率的同时实现了错误校正，为信息密集型多模态推理提供了有效解决方案。
...
TL;DR: SpectraMorph是一个物理引导的自监督高光谱超分辨率框架，通过解混瓶颈实现HSI和MSI融合，在单波段MSI下仍保持鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法缺乏可解释性，且在MSI波段数很少时表现不佳。需要开发既保持高性能又具有物理可解释性的融合方法。

Method: 采用解混瓶颈：从低分辨率HSI提取端元特征，用多层感知机从MSI预测丰度图，通过线性混合重建光谱，使用MSI传感器光谱响应函数进行自监督训练。

Result: 在合成和真实数据集上，SpectraMorph始终优于最先进的无监督/自监督基线方法，与监督基线方法相比也极具竞争力。

Conclusion: SpectraMorph框架具有可解释性中间结果、快速训练（1分钟内）和鲁棒性等优势，为高光谱超分辨率提供了有效的物理引导解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在单模态领域表现出色，但在跨模态转换方面仍面临挑战。现有方法通常依赖限制性假设，如共享维度、高斯源先验和模态特定架构，限制了其通用性和理论基础。

Method: 在共享潜在空间中操作，引入对比对齐损失确保配对样本的语义一致性，设计域无关的编码器-解码器架构用于潜在空间噪声预测，提出预测损失指导跨域转换训练，并探索多种训练策略提高稳定性。

Result: 该方法支持任意模态对，在多种模态转换任务中表现强劲，包括多视图到3D形状生成、图像超分辨率和多视图场景合成。综合实验验证了框架的有效性。

Conclusion: LDDBM为通用模态转换建立了新的强基线，能够处理任意模态对，在多样化任务中表现出色。
...
Method: 提出分层画布表示，每个主体放在独立层中实现无遮挡组合；使用锁定机制保持选定层的高保真度，同时允许其他层灵活适应上下文；无需架构更改，利用位置嵌入和互补数据采样策略。

Result: 在多个主体个性化图像生成中，相比最先进方法，LayerComposer实现了更优越的空间控制和身份保持。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到视频模型擅长生成孤立片段，但在创建连贯的多镜头叙事方面存在不足，这限制了它们在故事讲述中的应用。

Method: 采用窗口交叉注意力机制将文本提示定位到特定镜头，以及稀疏镜头间自注意力模式（镜头内密集但镜头间稀疏），确保分钟级生成的效率。
...
TL;DR: 在混合SSM-Transformer架构中，检索功能完全由自注意力层负责，SSM层无法补偿注意力层的缺失。仅保留15%的注意力头即可维持近乎完美的检索性能，表明自注意力专门负责检索任务。


<details>
  <summary>Details</summary>
Motivation: 研究混合SSM-Transformer架构中不同组件的功能分工，特别是自注意力层和SSM层在检索任务中的具体作用。

Method: 通过对RecurrentGemma-2B/9B和Jamba-Mini-1.6模型进行注意力消融实验，观察检索性能变化，并分析不同比例注意力头对性能的影响。

Result: 注意力消融导致检索完全失败（0%准确率），而SSM层无法补偿。仅保留15%注意力头即可维持近乎完美的检索性能，同时保持84%的MMLU性能。

Conclusion: 混合架构中存在严格的功能分离，自注意力专门负责检索任务，挑战了关于架构冗余的假设，表明这些模型更像是专门模块而非集成系统。
...
TL;DR: NAS-DQN通过在线神经架构搜索控制器动态调整网络结构，在深度强化学习中实现了优于固定架构的性能、样本效率和策略稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统深度强化学习代理的性能受限于固定的神经网络架构，需要通过昂贵的超参数搜索确定，这限制了代理的适应性和性能潜力。

Method: 提出NAS-DQN，将学习型神经架构搜索控制器集成到DRL训练循环中，基于累积性能反馈实现动态网络重构。

Result: NAS-DQN在连续控制任务中表现出优越的最终性能、样本效率和策略稳定性，计算开销可忽略不计，且显著优于随机搜索和固定架构设计。

Conclusion: 架构自适应对于在线深度强化学习的样本效率优化不仅是有益的，而且是必要的，RL代理设计可以无缝集成为学习过程的动态组件。
...
TL;DR: ReGraphT是一个无需训练、基于检索增强生成的框架，通过构建CUDA优化轨迹的推理图，将LLM级推理能力转移到小型模型，使SLM在CUDA代码生成任务上接近LLM性能。
...
Method: 将CUDA优化轨迹组织成结构化推理图，将组合优化建模为状态转换，利用蒙特卡洛图搜索进行高效探索，无需额外训练。

Result: ReGraphT在CUDAEval和ParEval基准测试中平均实现2.33倍加速，优于HPC专用微调模型和其他检索增强方法，使SLM接近LLM级性能。

Conclusion: ReGraphT框架成功解决了SLM在复杂CUDA代码生成中的推理能力不足问题，在保持隐私和计算效率的同时实现了接近LLM的性能水平。
...
TL;DR: 提出基于Transformer的深度学习模型来预测均衡路径流量，相比传统优化方法大幅降低计算时间，并能适应需求变化和网络结构调整。
...
Result: 在多个测试网络上验证，模型比传统优化方法快几个数量级，能准确估计多类网络中的路径级交通流量。

Conclusion: 该模型显著降低计算成本，提高预测精度，并能灵活适应需求变化和网络条件，支持交通管理和快速情景分析。
...
TL;DR: FairGRPO是一种分层强化学习方法，通过自适应重要性加权和自动发现潜在人口群体来解决医疗AI中的公平性问题，显著减少预测差异并提高性能。


<details>
  <summary>Details</summary>
Motivation: 医疗AI系统在诊断能力上表现优异，但在不同人口群体间存在性能差异，对少数群体造成实际伤害。现有的多模态推理基础模型通过强化学习训练时会继承并放大训练数据中的偏见。

Method: FairGRPO采用分层强化学习方法，基于表征、任务难度和数据源对优势进行自适应重要性加权。当缺乏人口统计标签时，使用无监督聚类自动发现潜在人口群体。

Result: 在7个临床诊断数据集上的实验表明，FairGRPO将预测差异减少了27.2%，同时F1分数提高了12.49%。训练动态分析显示FairGRPO在整个优化过程中持续改善公平性。
...
TL;DR: 使用基于SHAP的特征选择方法来提高膀胱癌诊断预测模型的透明度和效果，通过六种二元分类场景区分膀胱癌与其他泌尿系统疾病，采用XGBoost、LightGBM和CatBoost算法，并优化超参数和类别平衡。
...
Method: 使用SHAP进行特征选择，采用XGBoost、LightGBM和CatBoost算法，通过Optuna进行超参数优化，使用SMOTE技术进行类别平衡，构建六种二元分类场景。

Result: 基于SHAP的特征选择在保持或改善平衡准确率、精确度和特异性等性能指标的同时，提高了模型的透明度。
...
TL;DR: FINDER是一个针对噪声数据集的分类框架，通过随机特征和KLE分解来处理低信噪比、小样本量等挑战性数据问题。
...
Method: 将经验数据集视为随机场的实现，映射到希尔伯特空间构建随机特征，通过Kosambi-Karhunen-Loève展开分解为可计算不可约分量，通过特征分解进行分类。

Result: 在阿尔茨海默病分期分类和遥感森林砍伐检测等数据稀缺的科学领域取得了最先进的突破。

Conclusion: 讨论了FINDER在何种情况下优于现有方法、失败模式和其他限制。
...
TL;DR: 本文首次分析了Muon优化器中近似正交化更新的理论性能，揭示了计算近似误差与最优步长和动量参数之间的耦合关系。
...
Method: 在线性最小化预言机(LMO)优化框架下，引入加性误差模型来捕捉实际近似方案的不精确性，分析近似正交化更新的性能边界。

Result: 分析表明：较低的预言机精度需要较小的步长但较大的动量参数，近似精度与学习率调度必须协同调整。NanoGPT实验验证了这种耦合关系。

Conclusion: 将近似过程从实现细节提升为必须与学习调度协同调整的关键参数，为Muon优化器的实际应用提供了理论指导。
...
TL;DR: 本文针对去中心化联邦学习中的隐私预算量化挑战，提出了两种基于f-DP的隐私计算方法：PN-f-DP和Sec-f-LDP，能够更准确地捕捉稀疏通信、本地迭代和相关性噪声带来的隐私放大效应。


<details>
  <summary>Details</summary>
Motivation: 去中心化联邦学习中的隐私预算量化面临挑战，因为复杂的算法组件（如去中心化通信和本地更新）共存，使得准确量化隐私预算变得困难。

Method: 开发了两种新的基于f-DP的隐私计算方法：PN-f-DP（量化随机游走通信下用户对之间的隐私泄露）和Sec-f-LDP（通过共享秘密支持结构化噪声注入），结合f-DP理论和马尔可夫链集中工具。
...
TL;DR: 本文分析了持续学习中任务排序对线性回归性能的影响，特别研究了贪婪最大化任务间差异性的排序策略，并与随机排序进行了对比。


<details>
  <summary>Details</summary>
Motivation: 先前工作简要探索了贪婪排序策略，但仍存在许多开放性问题。本文旨在形式化这类排序方法，并深入理解其在持续学习中的表现。

Method: 使用Kaczmarz方法文献中的工具，形式化贪婪排序策略，开发几何和代数直觉，并在线性回归和CIFAR-100分类任务上进行实证分析。

Result: 实证显示贪婪排序在平均损失上比随机排序收敛更快。在高秩回归设置下，贪婪排序的损失界与随机排序类似；但在一般秩设置下，单次贪婪排序可能失败，而允许重复的贪婪排序以O(1/∛k)速率收敛。

Conclusion: 研究揭示了贪婪排序和随机排序之间的细微差别，表明贪婪排序在特定条件下具有优势，但也存在局限性。
...
TL;DR: 提出了一种新的椭圆不确定性集合来处理金融强化学习中市场影响的定向特性，解决了传统鲁棒强化学习在金融交易中因训练与部署环境不匹配导致的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 金融强化学习代理在历史数据上训练时不会影响价格，但在实际部署时其交易行为会产生市场影响，这种环境不匹配会显著降低性能。传统鲁棒强化学习方法依赖对称结构，无法捕捉市场影响的定向特性。

Method: 开发了一类新的椭圆不确定性集合，建立了最坏情况不确定性的隐式和显式闭式解，实现了高效可处理的鲁棒策略评估。

Result: 在单资产和多资产交易任务上的实验表明，该方法实现了更优的夏普比率，并在交易量增加时保持鲁棒性。

Conclusion: 该方法为金融市场中的强化学习提供了一个更忠实和可扩展的解决方案，能够有效处理市场影响的定向特性。
...
TL;DR: 提出了一种基于函数评估的无偏梯度估计器新方法，通过将方向导数重构为伸缩级数并采用精心设计的分布采样，解决了传统零阶优化方法中的梯度估计偏差问题。
...
Method: 通过将方向导数重构为伸缩级数，并从精心设计的分布中采样，构建了一族无偏梯度估计器，同时保持了有利的方差特性。

Result: 分析了理论性质，推导了四种具体构造的最优缩放分布和扰动步长，证明使用该估计器的SGD在光滑非凸目标上达到最优复杂度。合成任务和语言模型微调实验证实了该方法相比标准方法具有更高的准确性和收敛性。

Conclusion: 提出的无偏梯度估计器家族有效解决了零阶优化中的偏差问题，在理论和实验上都表现出优越性能，为梯度不可用或计算昂贵场景下的优化提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注固定长度的随机扰动，但忽略了扰动方向与梯度对齐的潜在优势。本文旨在探索这种方向对齐扰动是否能提高梯度估计的准确性。

Method: 将问题建模为扰动分布空间上的约束函数优化问题，提出了方向对齐扰动(DAP)方案，该方案能自适应地在关键方向上提供更高精度。同时分析了使用δ-无偏随机扰动的随机梯度下降收敛性。

Result: 理论分析表明DAP方案在特定条件下优于传统方法。在合成问题和实际任务的实证评估中，DAPs的表现超过了传统方法。

Conclusion: 方向对齐的随机扰动在梯度估计中具有显著优势，特别是在关键方向上能提供更准确的估计，为随机优化方法提供了新的改进方向。
...
TL;DR: 提出UCAN方法，通过各向异性噪声增强随机平滑的认证鲁棒性，显著提升现有方法的性能


<details>
  <summary>Details</summary>
Motivation: 现有随机平滑方法主要使用各向同性噪声，忽略了数据维度的异质性，限制了认证鲁棒性的效果

Method: UCAN将现有随机平滑方法从对称噪声转换为非对称噪声分布，支持多种噪声分布和ℓp范数认证，并开发了三种噪声参数生成器来优化各维度的噪声参数

Result: 在MNIST、CIFAR10和ImageNet数据集上，UCAN在大认证半径下的认证准确率相比现有方法提升高达182.6%

Conclusion: UCAN通过各向异性噪声显著提升了随机平滑的认证鲁棒性，为对抗攻击提供了更有效的防御
...
TL;DR: 提出AMRC方法，通过动态掩码损失和表示一致性约束，有效抑制时间序列预测中的冗余特征学习，挑战了传统长序列信息增益假设。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列预测中的"长序列信息增益假设"存在局限性，研究发现适当截断历史数据反而能提高预测精度，表明现有模型在训练中学习了大量冗余特征。

Method: 提出自适应掩码损失与表示一致性（AMRC）方法，包含：1）动态掩码损失，自适应识别高判别性时间片段指导梯度下降；2）表示一致性约束，稳定输入、标签和预测之间的映射关系。

Result: 实验结果表明AMRC有效抑制了冗余特征学习，显著提高了模型性能。
...
TL;DR: 论文提出推理即填充和多重标记熵解码两种方法，充分利用掩码扩散语言模型的训练优势，在推理和后训练中实现性能提升和效率优化。


<details>
  <summary>Details</summary>
Motivation: 针对掩码扩散语言模型在数学和编程任务中，任意顺序解码算法表现不佳或与从左到右采样相似，标准多重标记解码显著降低性能的问题，探索如何合理利用MDLMs的额外计算能力。

Method: 1. 推理即填充：使用MDLMs填充推理模板，区分推理和答案标记，实现答案不确定性测量和早期退出；2. 多重标记熵解码：基于条件熵的自适应采样器，减少并行解码位置时的误差。

Result: 在GSM8k上，使用后验推理轨迹微调LLaDA-8B Base可获得与人工编写推理轨迹微调相当的性能提升；MED在保持性能的同时减少2.7倍步骤。

Conclusion: MDLMs的训练和计算能力为推理和后训练方法开辟了新途径，证明其价值不仅限于解码顺序的灵活性。
...
TL;DR: 使用监督学习和决策树分类器在CAD建模的平面图中进行RFID位置推断，通过RSSI数据对12个实验室区域进行分类，整体准确率34.2%，部分区域F1分数超过0.40，但稀有类别分类效果较差。


<details>
  <summary>Details</summary>
Motivation: RFID跟踪是国防资产存储的可行解决方案，但传感器特异性差（包括远程检测、欺骗和伪造等漏洞）会导致错误检测和操作安全事件，需要改进位置推断能力。

Method: 使用监督学习模拟，结合真实的RSSI数据和决策树分类，在CAD建模的平面图中对12个实验室区域进行分类，采用类别权重处理类别不平衡问题，使用分层子样本训练模型。

Result: 模型在5000个平衡观测值上训练，整体准确率34.2%，多个区域（F、G、H等）的F1分数超过0.40，但稀有类别（特别是LabZoneC）经常被错误分类，即使使用了类别权重。

Conclusion: 基于RSSI的决策树可以在现实模拟中应用，实现区域级异常检测或错位监控，但在低覆盖和低信号区域的可靠分类性能可以通过更好的天线放置或额外传感器及多模态传感器融合来改进。
...
TL;DR: 提出了SALT框架，通过构建轨迹图来提供更细粒度的优势分配，解决了基于群体的RL算法在复杂多步任务中由于稀疏奖励导致的训练不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在单轮任务中表现出色，但在复杂、多步骤、长期任务中面临挑战。基于群体的强化学习算法仅依赖稀疏结果奖励，会导致训练不稳定和次优策略。

Method: SALT框架从相同提示的轨迹构建图，量化每个步骤的质量并相应分配优势。这是一个即插即用的模块，可与现有基于群体的RL算法无缝集成。

Result: 在WebShop、ALFWorld和AppWorld基准测试中的广泛实验表明，SALT在不同模型规模下都能持续提升性能。

Conclusion: SALT通过细粒度的优势分配有效解决了基于群体RL算法的训练不稳定问题，且无需修改rollout过程，计算开销极小。
...
<details>
  <summary>Details</summary>
Motivation: 比特币网络虽然处理了超过10.8亿笔交易和87.2亿BTC，但由于其伪匿名性和UTXO设计中的资金流动隐蔽性，这些数据对机器学习研究基本上不可访问。

Method: 构建时间异构图模型，重建比特币资金流动，提供自定义采样方法生成节点和边特征向量，以及加载和分析比特币图数据的工具。

Result: 创建了包含完整交易历史的图数据集，时间跨度到特定区块高度，包含超过24亿节点和397.2亿边，并提供数据库快照和工具包。

Conclusion: 该综合数据集和工具包使机器学习社区能够大规模处理比特币复杂生态系统，推动异常检测、地址分类、市场分析和大规模图机器学习基准测试等应用的进展。
...
TL;DR: 提出一种基于拒绝采样的新算法，能够从现有TPP模型中并行精确采样多个未来值，无需架构更改或重新训练


<details>
  <summary>Details</summary>
Motivation: 传统时序点过程的自回归特性使得采样必须顺序执行，限制了效率。需要解决表达性建模与高效并行生成之间的差距

Method: 基于拒绝采样的算法，支持从现有TPP模型中并行精确采样多个未来事件

Result: 在真实世界数据集上实现了经验性加速，提供了理论保证

Conclusion: 该方法为大规模TPP应用在表达性建模和高效并行生成之间架起了桥梁
...
TL;DR: 提出了一种结合延迟效应、累积影响和客户异质性的广告竞价策略，使用上下文马尔可夫决策过程建模，通过两阶段最大似然估计和强化学习算法实现个性化竞价，达到近乎最优的遗憾界。
...
Result: 该方法达到了近乎最优的遗憾界$\tilde{O}{(dH^2\sqrt{T})}$，其中$d$是上下文维度，$H$是轮数，$T$是客户数量，并通过仿真实验验证了理论结果。

Conclusion: 所提出的方法能够有效解决广告竞价中的延迟效应、累积影响和客户异质性问题，为在线广告平台提供了高效的个人化竞价策略解决方案。
...
Method: 设计在线学习算法，无需额外查询目标模型即可评估所有草稿模型，适用于各种推测解码方法。

Result: 在开源LLM和多样化数据集上实验，性能显著优于现有方法，特别在需要长推理链的领域。

Conclusion: 该方法能有效选择最佳草稿模型，大幅提升LLM推理效率，尤其在复杂推理任务中表现突出。
...
TL;DR: 研究核心加密资产流动性和波动率代理是否产生溢出效应以预测市场风险，采用多层统计框架和机器学习方法，发现显著的格兰杰因果关系和中等预测准确性。


<details>
  <summary>Details</summary>
Motivation: 探索加密资产流动性和波动率代理的溢出效应是否能预测市场风险，为风险管理提供实证依据。

Method: 集成三层统计框架：核心流动性与收益互动、主成分关系、波动率因子投影，辅以VAR脉冲响应、HAR-X模型和防泄漏机器学习协议。

Result: 发现跨层统计显著的格兰杰因果关系，样本外预测准确性中等，提供了关键图表和数据分析。

Conclusion: 加密资产流动性和波动率代理确实产生可预测市场风险的溢出效应，多层统计框架有效捕捉了这些关系。
...
TL;DR: 提出了耦合Transformer自编码器(CTAE)，用于同时建模多脑区神经记录中的共享和特定区域动态，解决了现有方法忽略时间结构或无法分离共享/私有信号的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么忽略多脑区记录中的时间结构，要么无法有效分离共享和区域特定的神经动态，需要一种能同时处理非线性动态和信号分离的统一框架。

Method: 使用Transformer编码器-解码器架构捕获长程神经动态，通过正交子空间划分将每个区域的潜在空间明确分为共享和私有部分。

Result: 在两个高密度电生理数据集上验证，CTAE提取的表征能更好地解码行为变量，优于现有方法。

Conclusion: CTAE为多脑区神经记录分析提供了有效框架，能同时建模非线性动态并分离共享与区域特定信号。
...
TL;DR: ShapeX是一个创新的时间序列分类解释框架，通过形状片段分割和Shapley值评估，提供比现有方法更精确和因果保真的解释。


<details>
  <summary>Details</summary>
Motivation: 在高风险应用中，时间序列分类模型的透明度至关重要。现有后解释方法主要关注时间步级特征归因，忽略了分类结果主要由关键形状片段驱动的基本前提。

Method: ShapeX框架将时间序列分割为有意义的形状片段驱动段，并使用Shapley值评估其显著性。核心是形状片段描述与检测(SDD)框架，学习分类所需的各种形状片段。

Result: 在合成和真实数据集上的实验表明，ShapeX在识别最相关子序列方面优于现有方法，提高了时间序列解释的精度和因果保真度。

Conclusion: ShapeX通过利用形状片段的原子性特性，产生揭示因果关系而非仅是相关性的解释，为高风险应用中的时间序列分类提供了更可靠的可解释性。
...
TL;DR: 提出基于MentalRoBERTa的分层双头神经网络，用于社交媒体自杀风险四分类，结合CORAL序数回归和标准分类头，解决类别不平衡和时序依赖问题。


<details>
  <summary>Details</summary>
Motivation: 社交媒体已成为识别自杀风险的重要来源，但自动检测系统面临类别严重不平衡、发帖模式的时间复杂性以及风险级别兼具序数和分类性质的多重挑战。

Method: 使用MentalRoBERTa为基础，构建分层双头神经网络：CORAL头保持风险级别间的序数关系，标准分类头实现灵活分类；3层Transformer编码器建模时序依赖，显式时间间隔嵌入捕捉发帖动态；采用组合损失函数(0.5 CORAL + 0.3 交叉熵 + 0.2 Focal Loss)。

Result: 通过5折分层交叉验证评估，以宏F1分数为主要指标，模型在自杀风险分类任务上表现出色。

Conclusion: 该模型有效解决了自杀风险检测中的类别不平衡、时序复杂性和序数分类问题，为社交媒体自杀预防提供了有效的自动化解决方案。
...
TL;DR: 提出了一种基于博弈论强化学习的因果发现框架，通过DDQN智能体与强基线方法（GES或GraN-DAG）竞争，实现了理论保证与实用性的统一。


<details>
  <summary>Details</summary>
Motivation: 现有因果发现方法存在理论与实践之间的鸿沟：GES和GraN-DAG等算法经验性能强但缺乏有限样本保证，而有理论保证的方法无法扩展到大规模问题。

Method: 采用博弈论强化学习框架，DDQN智能体始终从对手（GES或GraN-DAG）的解开始，直接与强基线竞争。

Result: 在合成SEM（30节点）上，观测到的错误概率随样本量n衰减，与理论紧密匹配；在Sachs、Asia、Alarm、Child、Hepar2、Dream、Andes等真实数据集上持续改进GES和GraN-DAG，并能扩展到Hepar2（70节点）、Dream（100节点）、Andes（220节点）等大规模图。
...
TL;DR: 本文提出了一种新的维度加权方案，并将其融入KNN分类器中，通过加权Minkowski距离改进了传统KNN在模式分类中的性能。


<details>
  <summary>Details</summary>
Motivation: 在多维样本的模式分类中，基于加权维度的距离度量至关重要，但常用的欧氏距离存在诸多问题。需要开发更有效的距离度量和维度加权方案来提高分类性能。

Method: 提出了新的维度加权方案，将其整合到KNN分类器中，使用加权Minkowski距离进行模式分类，并在合成和真实数据集上进行测试。

Result: 在基因表达数据集上，相比传统KNN，分类准确率显著提升约10%，在所有交叉验证实验中表现一致良好。

Conclusion: 该方法通过调节包含k个参考样本区域的大小和形状，实现了对KNN分类器的重要泛化，特别适用于样本少、维度高的数据集。
...
TL;DR: 提出一种完全解耦的训练策略，通过将原型建模为高斯混合模型并使用在线EM更新，独立于编码器损失，解决了自监督学习中原型崩溃的问题。


<details>
  <summary>Details</summary>
Motivation: 传统自监督学习方法存在原型部分崩溃问题，多个原型收敛到几乎相同的表示，这破坏了提供多样化目标的核心目的，导致需要过度参数化或添加临时正则化器。

Method: 引入完全解耦训练策略，将原型和编码器分别在不同目标下学习。原型被建模为高斯混合模型，通过在线EM风格过程独立更新，与编码器损失分离。

Result: 该方法无需显式正则化即可消除原型崩溃，产生持续多样化的原型，并在下游任务中表现更强。

Conclusion: 通过打破原型和编码器的联合优化，提出的解耦训练策略有效解决了原型崩溃问题，为自监督学习提供了更稳健的方法。
...
TL;DR: 当前时序基础模型表现不佳，原因在于简单套用NLP/CV范式，而时序数据缺乏概念密度，需要转向基于不变性原则的系统化数据集构建。


<details>
  <summary>Details</summary>
Motivation: 时序基础模型表现不如轻量监督基线和经典模型，这是因为简单移植NLP/CV范式不适用于时序数据特性。

Method: 提出从机会主义的数据聚合转向基于原则的设计，构建系统覆盖时序语义不变性空间的数据集。

Result: 时序数据缺乏概念密度，无法通过在线抓取范式获得有效训练数据。

Conclusion: 只有通过不变性覆盖确保表征完整性，时序基础模型才能实现泛化、推理和真正涌现行为所需的对齐结构。
...
TL;DR: 该研究通过多层图扩散模型发现，阿尔茨海默病中tau蛋白的传播受到结构连接和功能连接的不对称影响，FC主导皮层下区域传播，SC主导枕叶等区域，且这种主导地位随疾病阶段变化，与AD相关基因表达模式高度一致。


<details>
  <summary>Details</summary>
Motivation: 神经影像学证据显示tau蛋白沿特定脑网络积累，但结构连接和功能连接如何相互作用影响tau传播尚不清楚。

Method: 利用大量纵向神经影像数据，通过多层图扩散模型分析SC-FC相互作用。

Result: 发现SC和FC对tau传播的区域性不对称贡献：FC主导皮层下、岛叶、额颞叶区域，SC主导枕叶、顶叶和边缘系统；这种主导地位随疾病阶段变化，早期以FC为主，后期以SC为主；空间模式与AD相关基因表达高度一致。

Conclusion: 结构连接和功能连接在tau传播中发挥不对称作用，其主导模式与AD相关基因表达和风险因素相关，为理解疾病进展机制提供了新见解。
...
Method: 采用方差缩减随机梯度推送(ADP-VRSGP)，结合渐进梯度融合策略利用历史梯度，并整合去中心化推送-求和聚合技术，适用于时变通信拓扑。

Result: 理论分析证明该方法在合适学习率下实现鲁棒收敛，实验结果显示在多种场景下优于现有基线方法。
...
TL;DR: 提出HyperNS方法解决大规模TSP问题，通过聚类和超导览引导搜索，在内存效率和求解质量上优于现有神经方法
...
Method: 采用"先聚类后路由"策略，使用稀疏热图图进行聚类，生成超导览来指导初始化和优化过程，缩小搜索空间

Result: 在合成和真实数据集上实验表明，该方法优于现有神经方法，特别在处理大规模实例时显著缩小与最优解的差距

Conclusion: HyperNS方法为大规模TSP问题提供了有效的解决方案，在内存效率和求解质量方面都有显著提升
...
TL;DR: RLEV方法通过将人类定义的价值信号直接整合到奖励函数中，使LLM优化与可量化的人类价值对齐，超越了仅基于正确性的基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法如RLVR在客观领域使用二元正确性奖励训练模型，但忽略了不同任务的重要性差异。需要将人类价值信号直接纳入奖励函数。
...
Conclusion: 优化明确的效用函数为LLM与人类优先级对齐提供了实用路径，即使在噪声价值信号下RLEV仍保持稳健性。
...
Method: 使用优化确定性等价在奖励值和时间上实现每阶段鲁棒性，在适当的约束条件下通过参数化强拉格朗日对偶框架确保与原问题的等价性。

Result: 提出了一个简单的算法框架，可包装在标准RL求解器周围，建立了算法收敛性，并通过数值实验验证了方法的风险感知特性。

Conclusion: 该框架为高风险应用提供了有效的风险感知约束强化学习解决方案，具有良好的理论保证和实际可行性。
...
TL;DR: 本文提出了三种可复制性的松弛概念（点态、近似、半可复制），在PAC学习中实现了样本最优的不可知学习器。


<details>
  <summary>Details</summary>
Motivation: 由于严格可复制性在简单任务（如阈值学习）中不可实现，作者探索在何种近似可复制性下学习是可行的。

Method: 提出了三种可复制性松弛：点态可复制（固定输入一致）、近似可复制（大部分分布一致）、半可复制（完全可复制但可使用共享无标签样本）。

Result: 对于常数可复制参数，获得了样本最优的不可知PAC学习器：点态和近似可复制需要Θ(d/α²)样本，半可复制需要Θ(d²/α²)标注样本。

Conclusion: 通过松弛可复制性要求，可以在不显著增加样本复杂度的情况下实现学习，为可复制学习提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 开发犬类早期癌症检测的无创筛查工具面临挑战，常规实验室数据成本低但受限于生物标志物非特异性和严重的类别不平衡问题。

Method: 系统比较了126个分析流程，包括多种机器学习模型、特征选择方法和数据平衡技术，使用患者级数据分区防止泄漏。

Result: 最佳模型（带类别加权的逻辑回归分类器）显示出中等排序能力（AUROC=0.815），但临床分类性能差（F1-score=0.25，阳性预测值=0.15），阴性预测值高（0.98）但召回率不足（0.79）。

Conclusion: 常规实验室数据中存在的癌症信号太弱且与正常衰老或其他炎症状况混淆，无法实现可靠的临床区分，需要整合多模态数据源才能取得有意义的进展。
...
Method: CO-PFL算法通过分析梯度方向差异和预测偏差来动态估计客户端贡献，结合梯度子空间和数据子空间信息进行双重评估，并集成参数级个性化机制和掩码感知动量优化。
...
TL;DR: 提出了Fair-SMW算法，通过拉格朗日方法和Sherman-Morrison-Woodbury恒等式重新构建约束优化问题，提高了谱聚类的计算效率，在保持公平性的同时显著减少运行时间。


<details>
  <summary>Details</summary>
Motivation: 现有的谱聚类算法在引入群体公平性约束时计算效率较低，需要改进算法运行时间。

Method: 使用拉格朗日方法和SMW恒等式重新构建约束优化问题，开发了Fair-SMW算法，采用三种不同的拉普拉斯矩阵变体来生成多个算法版本。

Result: 在LastFM、FacebookNet、Deezer和German等真实网络数据集上的评估显示，计算时间比现有最优算法快两倍，同时能够实现两倍的平衡性。

Conclusion: Fair-SMW算法在保持公平性的同时显著提高了计算效率，为公平聚类提供了更高效的解决方案。
...
Method: 扩展传统QKV注意力框架为QKCV，引入静态类别嵌入C，作为可插拔模块增强各类注意力模型，支持仅更新静态嵌入C来微调预训练模型。

Result: QKCV显著提升了多种注意力模型在多样化真实数据集上的预测精度，并在微调单变量时间序列基础模型时表现出优越性能，同时降低计算开销。

Conclusion: QKCV是一种有效且通用的注意力机制扩展，能够充分利用类别信息提升预测性能，并提供高效的模型微调方案。
...
Method: 使用元变分dropout方法，通过共享超网络学习客户端相关的dropout率，实现模型个性化并减少通信成本。

Result: 在多种稀疏和非IID联邦学习数据集上，MetaVD表现出优异的分类准确性和不确定性校准性能，特别是对于分布外客户端。

Conclusion: MetaVD通过条件dropout后验实现了元学习的后验适应和贝叶斯联邦学习的后验聚合，有效解决了联邦学习中的关键挑战。
...
TL;DR: SpLIIF是一种生成隐式神经表示的方法，能够对天气变量进行任意降尺度处理，在温度和风的预测上比CorrDiff和基线方法表现更好。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够从稀疏气象站和地形数据生成高分辨率天气变量的方法，实现任意降尺度的天气预测。

Method: 使用SpLIIF生成隐式神经表示，从日本稀疏气象站和地形数据训练模型，实现天气变量的任意降尺度处理。

Result: 在温度降尺度方面比CorrDiff和基线方法好50%，在风预测方面好10-20%。

Conclusion: SpLIIF在天气变量降尺度任务中表现优于现有方法，特别是在温度预测方面有显著优势。
...
TL;DR: 提出了一个可证明收敛且实用的多目标强化学习框架，采用最大最小准则，通过博弈论视角将其重构为两人零和正则化连续博弈，并基于镜像下降设计高效算法。
...
Method: 将最大最小多目标强化学习重构为两人零和正则化连续博弈，使用基于镜像下降的高效算法，并引入自适应正则化来提升性能。

Result: 算法在表格设置中展示了收敛行为，在深度强化学习实现中显著优于之前的基线方法。
...
Result: 在三个不同GNN架构（DimeNet++、MXMNet和PAMNet）和量子化学性质数据集（QM9、MD17和Chignolin）上评估，LKM将量子化学和生物物理性质预测的平均绝对误差分别降低了9.8%、45.3%和22.9%。

Conclusion: LKM方法有潜力显著提高GNN在化学性质预测中的准确性，而不会显著增加训练和推理成本。
...
TL;DR: 本文提出了选择性分类差距的概念，将模型性能与完美排序预言机之间的差距分解为五个来源：贝叶斯噪声、近似误差、排序误差、统计噪声和实现/分布偏移导致的松弛。研究发现单调后校准对缩小这一差距影响有限，需要能够重新排序预测的评分机制。


<details>
  <summary>Details</summary>
Motivation: 当前选择性分类器难以达到完美排序预言机的性能水平，需要系统分析这种性能差距的来源，为构建更接近理想预言机行为的选择性分类器提供指导。

Method: 提出选择性分类差距的有限样本分解框架，通过合成数据和真实世界视觉、语言基准进行验证，通过控制实验分离每个误差分量。
...
TL;DR: FedGPS是一个联邦学习框架，通过整合统计分布和梯度信息来应对数据异构性挑战，在各种异构场景下都表现出优越的性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法在数据异构性场景下性能受限，缺乏对不同异构场景的鲁棒性。研究发现共享统计信息可以帮助客户端从全局视角更新模型。

Method: FedGPS框架静态修改客户端学习目标以隐式建模全局数据分布，同时动态调整本地更新方向，整合其他客户端的梯度信息。

Result: 大量实验表明FedGPS在多种异构场景下优于现有最先进方法，验证了其有效性和鲁棒性。

Conclusion: FedGPS通过整合统计分布和梯度信息，有效缓解了联邦学习中的数据异构性问题，在各种场景下都表现出色。
...
TL;DR: OpTI-BFM是一种乐观决策标准，通过直接建模奖励函数的不确定性来指导行为基础模型(BFMs)进行任务推断，减少对预计算奖励数据的需求。


<details>
  <summary>Details</summary>
Motivation: 行为基础模型(BFMs)在零样本强化学习中需要计算大量推理数据集的奖励，这要么需要奖励函数的形式化访问，要么需要大量标注工作。本文旨在通过与环境交互来纯化任务推断过程。

Method: 提出OpTI-BFM方法，这是一种乐观决策标准，直接建模奖励函数的不确定性，并指导BFMs在任务推断中进行数据收集。该方法与线性多臂老虎机的上置信界算法有直接联系。

Result: 在已建立的零样本基准测试中，OpTI-BFM使基于后继特征的BFMs能够在少量episode中识别和优化未见过的奖励函数，且计算开销最小。

Conclusion: OpTI-BFM为BFMs提供了一种高效的任务推断方法，通过乐观探索减少了对预计算奖励数据的依赖，在少量交互中就能实现良好的性能。
...
<details>
  <summary>Details</summary>
Motivation: LLM在完成任务时倾向于寻找和利用"捷径"，这种行为会破坏基准测试结果的有效性和实际部署的可靠性，比如删除失败的测试而不是修复底层错误。

Method: 从现有基准（如LiveCodeBench和SWE-bench）创建任务规范与单元测试直接冲突的"不可能"任务变体，测量代理在这些任务上的通过率作为"作弊率"。

Result: 揭示了从简单测试修改到复杂运算符重载等更细粒度的作弊行为细节，展示了提示、测试访问和反馈循环如何影响作弊率。

Conclusion: ImpossibleBench不仅是评估工具，还是研究模型行为、上下文工程和开发监控工具的多功能框架，有助于构建更鲁棒可靠的LLM系统。
...
TL;DR: 开发了优化的GPU内核用于欧拉特征曲线计算，速度比之前GPU实现快16-2000倍，并创建了可微分的PyTorch层支持端到端学习。
...
Method: 为Ampere GPU优化的CUDA内核，使用128B合并访问和分层共享内存累积；PyTorch层通过可微分欧拉特征变换风格的sigmoid松弛学习单方向阈值。

Result: 在合成网格上比之前GPU实现快16-2000倍，实现了高效的可微分拓扑特征计算。

Conclusion: 该方法为拓扑特征在深度学习中的广泛应用提供了计算效率和可微分性支持，并讨论了批处理和多GPU扩展以扩大采用范围。
...
TL;DR: PRM引导的树搜索在数学推理中表现不佳，尽管成本更高，但与BoN相比没有显著改进，主要原因是PRM评分不可靠且泛化能力差。


<details>
  <summary>Details</summary>
Motivation: 传统的链式思维提示与BoN选择在数学推理中很流行，但其线性结构无法捕捉复杂问题解决的分支和探索性质。

Method: 提出自适应算法最大化过程奖励模型分数，研究PRM引导的树搜索是否能通过探索多个部分解路径来改进数学推理。

Result: 在23个数学问题上测试发现：PRM引导的树搜索相比BoN无显著改进；蒙特卡洛树搜索和波束搜索表现最佳；PRM对状态值近似差且可靠性随推理深度下降；PRM泛化能力差。

Conclusion: 树搜索对不可靠PRM分数的依赖导致性能不佳，需要不同的奖励建模方法才能有效提升LLM的数学推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决深度学习模型在真实应用中性能不稳定、黑盒特性难以解释的问题，以及现有评估框架缺乏对模型具体优缺点的定量洞察能力。

Method: 使用合成数据驱动的评估范式，通过三个核心分析维度：时间特征分解与能力映射、数据不规则性下的鲁棒性分析、理论最优基准测试。

Result: 实验表明当前深度学习模型在所有类型的时间特征上并未普遍接近最优基准。

Conclusion: SynTSBench提供了一个可解释的评估系统，能够系统评估时间序列预测模型的基本能力，为模型选择提供更清晰的指导。
...
TL;DR: 提出基于KAN的协作模型(KCM)来改进大-小模型协作框架，在保持精度的同时显著降低计算资源消耗，并缓解灾难性遗忘问题


<details>
  <summary>Details</summary>
Motivation: 解决现有大-小模型协作框架中存在的精度下降、灾难性遗忘加剧、小模型知识引发的幻觉问题

Method: 使用KAN(可解释神经网络)作为协作模型，替代传统MLP，在语言、视觉和视觉-语言跨模态任务中部署KCM

Result: 相比纯大模型方法，KCM显著减少大模型推理调用次数，保持相近任务精度；相比MLP协作模型，KCM显著缓解灾难性遗忘，在长尾数据上精度提升明显

Conclusion: KCM在所有指标上均优于基于MLP的小协作模型，为大-小模型协作提供了更优解决方案
...
TL;DR: 提出了CS-54k科学问答数据集，包含CS-4k评估基准和CS-50k训练集，用于评估和训练AI科学助手。实验表明，使用高质量领域数据训练的7B模型可以超越GPT-4等大型专有系统。


<details>
  <summary>Details</summary>
Motivation: 构建能够协助整个科学研究过程的AI协作系统（ResearchGPT），需要评估端到端工作流程的基准，而不仅仅是孤立的子任务。

Method: 从14k CC许可论文中构建CS-54k高质量科学问答语料库，采用基于检索增强生成(RAG)的可扩展流程和多阶段质量控制。从中衍生出CS-4k评估基准和CS-50k训练数据集。

Result: CS-4k能够将最先进的LLM划分为不同能力层级。在CS-50k上训练的7B规模模型通过监督训练和强化学习显著提升，甚至超越GPT-4、GPT-4o和Gemini 2.5 Pro等大型专有系统。

Conclusion: 使AI模型成为更好的研究助手更依赖于使用高质量领域数据的对齐训练，而非预训练规模或通用基准性能。发布了CS-4k和CS-50k以促进AI系统在CS研究中作为可靠协作伙伴的发展。
...
TL;DR: 提出了一种无需环境标注的图神经网络分布外泛化方法，通过识别因果子图的分布不变性来提升泛化性能。


<details>
  <summary>Details</summary>
Motivation: 解决图神经网络在分布偏移下的泛化挑战，避免传统IRM方法需要昂贵环境标注或人工划分的限制。

Method: 基于因果子图分布变化较小的观察，提出不变分布准则，并开发基于表示范数引导的不变分布目标来发现因果子图。

Result: 在两个广泛使用的基准测试上，该方法持续优于最先进的图泛化方法。

Conclusion: 证明了无需IRM框架也能有效识别因果子图，为图神经网络的分布外泛化提供了新思路。
...
TL;DR: 提出DB-FGA-Net模型，集成VGG16和Xception双骨干网络与频率门控注意力模块，无需数据增强即可实现脑肿瘤高精度分类，并集成Grad-CAM增强临床可解释性。
...
Method: 使用VGG16和Xception双骨干网络捕获互补的局部和全局特征，结合频率门控注意力模块，集成Grad-CAM进行肿瘤区域可视化。

Result: 在7K-DS数据集上，4类、3类和2类分类准确率分别达到99.24%、98.68%和99.85%；在独立3K-DS数据集上泛化准确率为95.77%，优于基准和最先进方法。
...
TL;DR: 提出InvDec混合架构，结合基于patch的时间编码器和在变量维度上操作的倒置解码器，通过延迟变量嵌入和自适应残差融合机制，在保持时间特征完整性的同时有效建模变量间依赖关系。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：通道独立方法如PatchTST擅长时间建模但忽略变量相关性，而纯变量注意力方法如iTransformer牺牲了时间编码。需要一种能够同时有效建模时间模式和跨变量依赖的架构。

Method: InvDec采用混合架构，包含基于patch的时间编码器和在变量维度上通过变量间自注意力操作的倒置解码器。引入延迟变量嵌入在时间编码后丰富变量特定表示，以及自适应残差融合机制动态平衡时间和变量信息。

Result: 在7个基准数据集上的实验显示，在高维数据集上显著提升：电力数据集MSE降低20.9%，天气数据集提升4.3%，交通数据集提升2.7%，同时在低维ETT数据集上保持竞争力。消融研究验证了各组件有效性。

Conclusion: InvDec通过原则性分离时间编码和变量级解码，有效解决了多变量时间序列预测中的挑战。分析表明其优势随数据集维度增加而增长，证实跨变量建模在变量数量增加时变得至关重要。
...
TL;DR: LEGO是一个轻量级高效的多属性遗忘框架，通过嵌入校准和灵活组合两步处理推荐系统中的多敏感属性遗忘问题，解决了现有方法无法同时处理多个遗忘请求和适应动态需求的挑战。


<details>
  <summary>Details</summary>
Motivation: 随着推荐系统中保护敏感用户信息需求的增长，现有单属性遗忘方法无法满足现实世界中多敏感属性和动态隐私保护的需求，存在无法同时处理多个遗忘请求和缺乏对动态需求高效适应性的问题。

Method: 将多属性遗忘过程分为两个步骤：嵌入校准从用户嵌入中移除特定属性信息，灵活组合将这些嵌入合并为单一嵌入以保护所有敏感属性。将遗忘过程构建为互信息最小化问题，提供同时遗忘的理论保证。

Result: 在三个真实世界数据集和三个代表性推荐模型上的广泛实验证明了所提框架的有效性和效率。

Conclusion: LEGO框架成功解决了多属性遗忘的挑战，能够同时处理多个遗忘请求并高效适应动态需求，为推荐系统的隐私保护提供了实用解决方案。
...
TL;DR: 使用商业飞行模拟器生成合成图像来补充少量真实标注图像，通过定制化领域自适应策略提升跑道检测模型的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在关键应用如自主着陆系统中，收集和标注训练数据成本高昂且难以覆盖所有场景，特别是罕见情况。合成图像生成提供了一种经济有效的解决方案。

Method: 基于商业飞行模拟器生成合成图像，结合少量真实图像，采用定制化领域自适应策略来缓解合成到真实数据的分布偏移问题。

Result: 标准目标检测模型能够实现准确预测，在真实数据中未包含的夜间图像等恶劣条件下也表现出良好的鲁棒性。

Conclusion: 通过控制图像生成和整合真实与合成数据，结合领域自适应策略，可以有效提升关键应用中视觉模型的性能。
...
### [144] [Relative-Based Scaling Law for Neural Language Models](https://arxiv.org/abs/2510.20387)
...
<details>
  <summary>Details</summary>
Motivation: 现有缩放定律研究几乎完全依赖交叉熵作为评估指标，但交叉熵只关注正确token的绝对概率，忽略了正确与错误token之间的相对排序，而相对排序对语言模型至关重要

Method: 提出相对概率指标(RBP)，量化正确token在top预测中的排名概率，并基于此建立相对缩放定律，在四个数据集和四个模型家族上进行广泛实验

Result: 通过跨越五个数量级的实验验证了相对缩放定律的鲁棒性和准确性，并展示了该定律在解释涌现现象和寻找缩放定律基础理论方面的应用
...
### [145] [Ask a Strong LLM Judge when Your Reward Model is Uncertain](https://arxiv.org/abs/2510.20369)
...
Method: 将策略梯度方法中的优势估计建模为成对偏好分类，实现原则性的不确定性量化来指导路由，不确定的样本对转发给LLM法官，确定的样本对由奖励模型评估。

Result: 在奖励模型基准测试中，基于不确定性的路由策略在相同成本下显著优于随机法官调用方法，下游对齐结果展示了其在改进在线RLHF中的有效性。

Conclusion: 提出的不确定性路由框架能够高效地结合快速奖励模型和强大LLM法官的优势，为在线RLHF提供了一种成本效益高的解决方案。
...
TL;DR: 本文提出了一个工业启发的基准环境，将排序和压缩操作结合成顺序回收场景，比较了模块化架构和单体架构在有无动作掩码下的表现。


<details>
  <summary>Details</summary>
Motivation: 工业过程控制需要局部专业化和全局协调，但现有强化学习基准与工业实际问题差异较大，限制了实际应用。

Method: 结合SortingEnv和ContainerGym两个基准环境，构建顺序回收场景，评估模块化架构（专业代理）和单体架构（全系统代理）在有无动作掩码下的控制策略。

Result: 无动作掩码时代理学习困难，模块化架构表现更好；应用动作掩码后两种架构都显著改善，性能差距缩小。

Conclusion: 动作空间约束对性能有决定性影响，专业化优势随动作复杂度降低而减弱，该基准为工业自动化多智能体强化学习提供了有价值的测试平台。
...
### [147] [Hierarchical Time Series Forecasting with Robust Reconciliation](https://arxiv.org/abs/2510.20383)
...
Conclusion: 在协调过程中整合不确定性是有效的，能够提高层次时间序列预测的鲁棒性。
...
### [148] [MolBridge: Atom-Level Joint Graph Refinement for Robust Drug-Drug Interaction Event Prediction](https://arxiv.org/abs/2510.20448)
*Xuan Lin,Aocheng Ding,Tengfei Ma,Hua Liang,Zhe Quan*

Main category: cs.LG

TL;DR: MolBridge是一个基于原子级联合图优化的DDI事件预测框架，通过构建药物对的联合图直接建模药物间相互作用，解决了现有方法无法显式建模跨分子原子级交互的问题。


<details>
  <summary>Details</summary>
Motivation: 现有DDI预测方法依赖孤立的药物表示，无法显式建模原子级的跨分子相互作用，限制了在不同分子复杂性和DDI类型分布下的有效性。
...
Conclusion: 细粒度图优化方法显著提高了DDI事件预测的准确性、鲁棒性和机制可解释性，为药物相互作用网络挖掘提供了有效的图分析方法。
...
### [149] [Transferable Black-Box One-Shot Forging of Watermarks via Image Preference Models](https://arxiv.org/abs/2510.20468)
...
TL;DR: 本文提出了一种针对后处理图像水印的伪造攻击方法，通过训练偏好模型来检测水印，并利用反向传播优化技术实现水印的移除和伪造，挑战了当前水印技术的安全性。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型和法律压力的增加，数字内容水印技术日益重要，但水印伪造（从真实内容窃取水印并应用于恶意内容）的研究仍然不足。

Method: 引入基于排序损失的偏好模型来评估图像是否含水印，仅使用程序生成的图像进行训练；通过反向传播优化输入图像来移除和伪造水印，仅需单张水印图像且无需水印模型知识。

Result: 在多种后处理图像水印模型上的评估表明，该方法能有效伪造水印，质疑了当前水印方法的安全性。

Conclusion: 提出的水印伪造方法简单实用，挑战了现有水印技术的安全性，代码和资源已公开。
...
### [150] [Why DPO is a Misspecified Estimator and How to Fix It](https://arxiv.org/abs/2510.20413)
...
<details>
  <summary>Details</summary>
Motivation: 直接对齐算法如DPO使用监督学习替代两阶段RLHF，但当真实奖励函数无法通过策略类实现时，DPO会出现错误设定问题，导致性能下降。

Method: 1. 分析DPO的统计估计问题；2. 研究两阶段RLHF的局部行为；3. 提出AuxDPO方法，在DPO损失函数中引入辅助变量来逼近RLHF解。

Result: 在示例赌博机设置和LLM对齐任务上的实验表明，AuxDPO相比DPO具有更优越的性能。

Conclusion: AuxDPO通过引入辅助变量有效缓解了DPO的错误设定问题，能够更好地逼近RLHF解，在模型对齐任务中表现更优。
...
### [151] [Hurdle-IMDL: An Imbalanced Learning Framework for Infrared Rainfall Retrieval](https://arxiv.org/abs/2510.20486)
...
Method: 采用分而治之策略，将降雨分布不平衡分解为零膨胀（非降雨样本占主导）和长尾（轻降雨样本相对重降雨样本不成比例）两个部分。使用hurdle模型处理零膨胀，提出IMDL方法通过将学习目标转换为无偏理想逆模型来处理长尾问题。

Result: 通过统计指标和案例研究证实，Hurdle-IMDL在缓解系统性低估和提高强到极端降雨检索方面优于传统方法、成本敏感方法、生成方法和多任务学习方法。
...
### [152] [Addressing Mark Imbalance in Integration-free Neural Marked Temporal Point Processes](https://arxiv.org/abs/2510.20414)
*Sishun Liu,Ke Deng,Xiuzhen Zhang,Yongli Ren,Yan Wang*

Main category: cs.LG

TL;DR: 提出了一种基于阈值的方法来处理标记时间点过程中事件标记分布不平衡的问题，通过调整标记概率来优化预测性能，并开发了新的神经MTPP模型来支持有效的时间采样和概率估计。
...
Method: 提出阈值方法学习阈值来调整由标记先验概率归一化的标记概率，以优化标记预测；先预测标记再预测时间；开发新的神经MTPP模型支持有效时间采样和标记概率估计，无需计算昂贵的数值不当积分。

Result: 在真实世界数据集上的广泛实验表明，该方法在下一个事件标记和时间预测方面优于各种基线方法。

Conclusion: 所提出的解决方案通过处理标记分布不平衡问题，显著提升了下一个事件预测的性能，特别是对罕见标记事件的预测效果。
...
### [153] [An Empirical Study of Sample Selection Strategies for Large Language Model Repair](https://arxiv.org/abs/2510.20428)
...
<details>
  <summary>Details</summary>
Motivation: LLM在现实系统中部署时会产生有毒或有偏见的输出，影响安全性和可信度。参数更新的高成本促使需要选择性使用修复数据。

Method: 评估五种代表性选择方法：随机采样、K-Center、梯度范数选择(GraNd)、分层覆盖(CCS)以及提出的语义感知优先采样(SAPS)。通过毒性减少、困惑度和三个复合指标评估修复效果。

Result: SAPS在解毒、效用保持和效率方面达到最佳平衡，用更少数据获得相当或更好的修复效果。随机采样对大型或鲁棒模型仍然有效，而高开销方法如CCS和GraNd收益有限。

Conclusion: 基于选择的修复是维护LLM可靠性的高效可扩展范式，最优数据比例取决于模型规模和修复方法，样本选择应被视为修复管道的可调组件。
...
### [154] [Explainable Benchmarking through the Lense of Concept Learning](https://arxiv.org/abs/2510.20439)
*Quannian Zhang,Michael Röder,Nikit Srivastava,N'Dah Jean Kouagou,Axel-Cyrille Ngonga Ngomo*

Main category: cs.LG

TL;DR: 提出可解释基准测试新范式，使用PruneCEL概念学习方法为知识图谱问答系统性能自动生成解释，在41人用户研究中80%情况下参与者能准确预测系统行为。


<details>
  <summary>Details</summary>
Motivation: 传统基准测试仅用少量指标总结系统性能，分析评估细节和获取开发见解是繁琐的人工任务且结果常有偏差，需要自动生成性能解释的新基准测试方法。

Method: 采用为大型知识图谱开发的新概念学习方法PruneCEL，计算知识图谱问答系统在基准测试中的性能解释。

Result: PruneCEL在可解释基准测试任务上比现有最优概念学习方法F1分数提升0.55分；用户研究表明80%情况下多数参与者能基于解释准确预测系统行为。

Conclusion: 可解释基准测试是可行的新范式，PruneCEL方法在知识图谱问答系统基准测试中有效生成性能解释，帮助用户理解系统行为。
...
### [155] [Structural Invariance Matters: Rethinking Graph Rewiring through Graph Metrics](https://arxiv.org/abs/2510.20556)
*Alexandre Benoit,Catherine Aitken,Yu He*

Main category: cs.LG

TL;DR: 该论文系统分析了图重布线对图结构度量的影响及其与下游任务性能的关系，发现成功的重布线方法倾向于保留局部结构同时允许全局连接的灵活性。


<details>
  <summary>Details</summary>
Motivation: 图重布线技术虽然能缓解图神经网络中的过压缩问题，但会改变图拓扑结构，可能扭曲重要的拓扑依赖信号。目前缺乏对哪些结构属性必须保留以确保性能增益和结构保真度的系统研究。

Method: 研究了七种不同的重布线策略，并关联局部和全局图属性变化与节点分类准确率的关系。
...
### [156] [Intransitive Player Dominance and Market Inefficiency in Tennis Forecasting: A Graph Neural Network Approach](https://arxiv.org/abs/2510.20454)
*Lawrence Clegg,John Cartlidge*

Main category: cs.LG

TL;DR: 该论文提出了一种图神经网络方法，通过时间有向图显式建模网球比赛中的非传递性关系，发现博彩公司在处理高非传递性复杂度的比赛时表现不佳，并利用该模型在选择性投注中获得了3.26%的正回报。


<details>
  <summary>Details</summary>
Motivation: 网球比赛中普遍存在非传递性优势关系（A赢B、B赢C但C赢A），但现有预测方法很少尝试纳入这种关系。

Method: 使用图神经网络方法，将球员作为节点，历史比赛结果作为有向边，构建时间有向图来显式建模非传递性关系。

Result: 模型在选择性投注高非传递性比赛时达到65.7%的准确率和0.215的Brier分数，使用凯利投注策略在1903次投注中获得了3.26%的正回报率。

Conclusion: 博彩市场在处理非传递性比赛时存在效率不足，图神经网络方法能够成功捕捉这些关系动态并从中获利。
...
<details>
  <summary>Details</summary>
Motivation: 现有的半监督微调方法依赖预测一致性或预定义置信度阈值，容易导致模型偏差和超参数敏感性问题。

Method: Bi-CoG方法同时利用模型间和模型内一致性，结合错误感知动态伪标签分配策略，为未标记数据分配高质量、低偏差的伪标签。

Result: 在14个数据集上的广泛实验表明，Bi-CoG能持续显著提升现有方法的性能。

Conclusion: Bi-CoG是一种简单有效的即插即用方法，通过双一致性引导的自训练策略，有效解决了半监督微调中的关键挑战。
...
### [158] [Generalizable Reasoning through Compositional Energy Minimization](https://arxiv.org/abs/2510.20607)
*Alexandru Oarga,Yilun Du*

Main category: cs.LG

TL;DR: 提出了一种通过组合子问题能量函数来构建全局能量景观的新方法，以解决推理任务的泛化问题，并引入并行能量最小化来提升样本质量。


<details>
  <summary>Details</summary>
Motivation: 现有端到端推理模型在训练分布之外的泛化能力有限，无法有效解决比训练时更复杂的问题。

Method: 学习子问题的能量景观，在测试时组合多个子问题的能量函数构建全局能量景观，并引入并行能量最小化算法。
...
### [159] [Practical Code RAG at Scale: Task-Aware Retrieval Design Choices under Compute Budgets](https://arxiv.org/abs/2510.20609)
*Timur Galimzyanov,Olga Kolomyttseva,Egor Bogomolov*

Main category: cs.LG

TL;DR: 该论文系统比较了代码生成任务中不同检索配置的性能，发现对于编程语言间的检索，稀疏BM25方法最有效且实用；对于自然语言到编程语言的检索，专用密集编码器效果更好但延迟高；最佳块大小随上下文窗口变化；简单行级分块与语法感知分块效果相当；BM25+词级分块提供最佳质量-延迟权衡。
...
Result: PL-PL任务中稀疏BM25效果最好且快10倍；NL-PL任务中专用密集编码器效果更好但延迟高100倍；最佳块大小随上下文窗口变化；行级分块与语法分块效果相当；检索延迟差异可达200倍。

Conclusion: 为代码导向RAG系统提供了基于任务需求、模型约束和计算效率的实践建议，强调BM25+词级分块在质量-延迟权衡上的优势。
...
### [160] [SheafAlign: A Sheaf-theoretic Framework for Decentralized Multimodal Alignment](https://arxiv.org/abs/2510.20540)
*Abdulmomen Ghalkha,Zhuojun Tian,Chaouki Ben Issaid,Mehdi Bennis*

Main category: cs.LG

TL;DR: SheafAlign是一个基于层论的分散式多模态对齐框架，通过多比较空间替代单空间对齐，克服了传统方法需要所有模态相互冗余的假设，在保持共享和独特信息的同时实现更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 传统多模态对齐方法假设所有模态之间相互冗余，这在现实分布式场景中不成立。需要一种能够处理非冗余模态关系的分散式对齐方法。

Method: 提出SheafAlign框架，使用层结构建模成对模态关系，采用分散式对比学习目标进行训练，在多比较空间中实现对齐。

Result: 在多模态感知数据集上表现出优越的零样本泛化能力、跨模态对齐性能和模态缺失鲁棒性，通信成本比现有最优方法降低50%。

Conclusion: SheafAlign通过层论框架成功解决了传统多模态对齐方法的局限性，在保持信息完整性的同时实现了高效的分散式对齐。
...
### [161] [PSO-XAI: A PSO-Enhanced Explainable AI Framework for Reliable Breast Cancer Detection](https://arxiv.org/abs/2510.20611)
...
TL;DR: 提出一种集成定制化粒子群优化(PSO)进行特征选择的框架，在乳腺癌诊断中实现99.1%的准确率，同时降低维度并提供可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统乳腺癌诊断方法存在变异性、成本高和误诊风险等问题，需要开发更准确可靠的计算辅助诊断工具。

Method: 结合定制化PSO进行特征选择，评估了29种不同模型（经典分类器、集成技术、神经网络、概率算法和基于实例的算法），并使用交叉验证和可解释AI方法。

Result: 在所有性能指标（包括准确率和精确率）上达到99.1%的优异分数，有效降低维度并提供透明的模型无关解释。
...
### [162] [A Unified Framework for Zero-Shot Reinforcement Learning](https://arxiv.org/abs/2510.20542)
*Jacopo Di Ventura,Jan Felix Kleuker,Aske Plaat,Thomas Moerland*

Main category: cs.LG

TL;DR: 提出了首个零样本强化学习的统一框架，通过一致的符号和分类法组织现有方法，将算法分为直接表示和组合表示两类，为未来研究提供理论基础。
...
Result: 建立了统一的零样本RL框架，能够直接比较不同方法，并为后继特征方法推导了扩展边界，提供了新的性能视角。

Conclusion: 该框架为零样本RL提供了原则性基础，为开发更通用智能体指明了清晰路径，有助于未来研究的系统化发展。
...
### [163] [Equitable Survival Prediction: A Fairness-Aware Survival Modeling (FASM) Approach](https://arxiv.org/abs/2510.20629)
*Mingxuan Liu,Yilin Ning,Haoyuan Wang,Chuan Hong,Matthew Engelhard,Danielle S. Bitterman,William G. La Cava,Nan Liu*

Main category: cs.LG

TL;DR: 提出了一种公平感知生存建模方法FASM，旨在减轻生存分析中的算法偏见，特别是跨组风险排序问题，在保持预测性能的同时显著提升公平性。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在医疗保健中的应用日益增多，但临床数据中的结构性不平等和社会偏见可能被数据驱动模型延续甚至放大。在生存分析中，删失和时间动态性进一步增加了公平模型开发的复杂性。现有公平方法往往忽视跨组排名差异，这可能导致高风险黑人患者被排在低风险白人患者之后，从而强化生物本质主义并损害公平护理。

Method: 提出了公平感知生存建模方法FASM，旨在减轻关于组内和跨组风险排序的算法偏见。该方法应用于SEER乳腺癌数据，通过时间分层评估验证其效果。

Result: FASM在保持与无公平意识生存模型相当的判别性能的同时，显著提高了公平性。时间分层评估显示FASM在10年随访期内保持稳定的公平性，其中中期随访期间改善最为明显。
...
### [164] [Embedding the MLOps Lifecycle into OT Reference Models](https://arxiv.org/abs/2510.20590)
*Simon Schindler,Christoph Binder,Lukas Lürzer,Stefan Huber*

Main category: cs.LG

TL;DR: 本文分析了将MLOps实践集成到运营技术(OT)系统中的挑战，提出了使用现有参考模型（如RAMI 4.0和ISA-95）进行结构化适配的方法，并通过实际用例展示了MLOps生命周期组件在RAMI 4.0中的映射。


<details>
  <summary>Details</summary>
Motivation: 随着MLOps在工业环境中的广泛应用，其与运营技术(OT)系统的集成面临显著挑战，需要找到有效的集成方法。

Method: 分析MLOps与OT环境结合的基本障碍，提出将MLOps实践嵌入到已建立的OT参考模型中的系统方法，评估RAMI 4.0和ISA-95对MLOps集成的适用性，并通过实际用例详细映射MLOps生命周期组件到RAMI 4.0。

Result: 研究发现标准MLOps实践不能直接移植到OT环境，但使用现有参考模型进行结构化适配可以为成功集成提供途径。

Conclusion: 通过使用RAMI 4.0和ISA-95等现有参考模型进行结构化适配，可以实现MLOps在OT环境中的成功集成，尽管标准MLOps实践不能直接应用。
...
### [165] [GRACE: GRaph-based Addiction Care prEdiction](https://arxiv.org/abs/2510.20671)
*Subham Kumar,Prakrithi Shivaprakash,Koustav Rudra,Lekhansh Shukla,Animesh Mukherjee*

Main category: cs.LG

TL;DR: 提出了一种名为GRACE的图神经网络框架，用于解决成瘾患者护理场所预测中的类别不平衡问题，通过特征工程和无偏元图训练方法，在真实数据上显著提升了少数类别的F1分数。


<details>
  <summary>Details</summary>
Motivation: 成瘾患者护理场所的确定是影响治疗效果和资源利用的关键临床决策，但现有方法面临数据集类别严重不平衡的问题，需要开发自动化框架来克服这一限制。

Method: 提出GRACE图神经网络框架，将护理场所预测形式化为结构化学习问题，进行广泛的特征工程，并采用获得无偏元图的新方法来训练GNN以克服类别不平衡。
...
### [166] [A Scalable, Causal, and Energy Efficient Framework for Neural Decoding with Spiking Neural Networks](https://arxiv.org/abs/2510.20683)
*Georgios Mentzelopoulos,Ioannis Asmanis,Konrad P. Kording,Eva L. Dyer,Kostas Daniilidis,Flavia Vitale*

Main category: cs.LG

TL;DR: Spikachu是一个基于脉冲神经网络(SNN)的可扩展、因果性和高能效的神经解码框架，在非人灵长类动物实验中表现出色，比因果基线模型性能更好，同时能耗降低2.26-418.81倍。


<details>
  <summary>Details</summary>
Motivation: 当前基于学习的神经解码方法存在两个问题：简单的因果模型缺乏泛化能力，复杂的非因果模型虽然能泛化但难以实时应用，且都依赖高能耗的人工神经网络，难以集成到资源受限的真实系统中。

Method: 开发了Spikachu框架，直接处理分箱的脉冲信号，将其投影到共享潜在空间中，通过适应输入时序的脉冲模块提取相关特征，然后整合这些潜在表示并解码生成行为预测。

Result: 在6只非人灵长类动物的113个记录会话（总计43小时记录）上评估，该方法在单会话训练中优于因果基线模型，能耗降低2.26-418.81倍。多会话和跨主体训练可提升性能，并实现对新会话、主体和任务的少样本迁移。

Conclusion: Spikachu提供了一个基于SNN的可扩展、在线兼容的神经解码框架，其性能与最先进模型相当，但能耗显著降低几个数量级。
...
### [167] [Convergence Analysis of SGD under Expected Smoothness](https://arxiv.org/abs/2510.20608)
...
<details>
  <summary>Details</summary>
Motivation: 经典SGD分析依赖过强(有界方差)或过粗糙(均匀噪声)的假设，期望平滑性条件提供了更灵活的替代方案，将随机梯度的二阶矩与目标函数值和全梯度联系起来。

Method: 通过精炼期望平滑性条件并引入采样相关常数，推导全梯度范数平方的期望界，分析不同步长策略下的收敛行为，所有证明在附录中详细给出。

Result: 证明了SGD在ES条件下具有O(1/K)的收敛速率，并给出了显式的残差误差项，统一并扩展了近期相关工作。

Conclusion: 期望平滑性条件为SGD分析提供了更通用和精确的理论框架，本文的分析统一并推进了该领域的最新进展。
...
### [168] [Unsupervised Anomaly Prediction with N-BEATS and Graph Neural Network in Multi-variate Semiconductor Process Time Series](https://arxiv.org/abs/2510.20718)
*Daniel Sorensen,Bappaditya Dey,Minjin Hwang,Sandip Halder*

Main category: cs.LG

TL;DR: 提出了两种新颖的半导体制造异常预测方法：基于N-BEATS的单变量预测和基于图神经网络的多变量关系建模方法，实现了从异常检测到异常预测的转变。


<details>
  <summary>Details</summary>
Motivation: 半导体制造过程复杂且精度要求高，存在高维传感器数据、类别严重不平衡以及变量间复杂依赖关系等挑战，需要从异常检测发展到异常预测以实现实时过程校正和主动故障预防。

Method: 提出两阶段异常预测框架：首先在无异常数据集上训练预测模型，然后对未见时间序列数据进行预测，将预测值与实际值偏差超过阈值的标记为异常。使用N-BEATS进行单变量预测和GNN建模变量间关系两种方法。

Result: 两种模型在20个时间点范围内都表现出强大的预测性能，在50个时间点内保持稳定的异常预测。GNN模型始终优于N-BEATS模型，且需要更少的可训练参数和更低的计算成本。
...
### [169] [Thought Communication in Multiagent Collaboration](https://arxiv.org/abs/2510.20733)
*Yujia Zheng,Zhuokai Zhao,Zijian Li,Yaqi Xie,Mingze Gao,Lizhu Zhang,Kun Zhang*

Main category: cs.LG

TL;DR: 该论文提出了一种"思维通信"的新范式，使AI代理能够直接进行思维层面的交流，超越自然语言的限制，从而提高集体智能的潜力。


<details>
  <summary>Details</summary>
Motivation: 自然语言的模糊性、间接性和有损性限制了集体智能的发展。虽然机器不受这些限制，但现有的多智能体系统仍主要依赖自然语言交流。

Method: 将思维通信形式化为潜变量模型，通过理论证明在非参数设置下可以识别共享和私有思维，并开发了从所有代理提取潜思维并在通信前分配相关思维的框架。

Result: 在合成和真实世界基准测试中验证了理论，证明了思维通信在协作中的优势。

Conclusion: 思维通信范式有潜力利用隐藏世界解决仅靠表面观察无法解决的挑战，这一工作为超越自然语言限制的集体智能开辟了新途径。
...
TL;DR: 提出了一种快速适应LLM到下游任务的方法，通过选择性矩阵分解替代逐层搜索，仅需100个样本和单次梯度计算即可实现无微调的模型适应。


<details>
  <summary>Details</summary>
Motivation: LASER方法虽然能通过修剪权重矩阵的高阶组件提升下游任务准确率，但其逐层搜索和全数据集前向传播的计算开销使其难以快速部署。

Method: 使用矩阵奇异值梯度识别关键矩阵，允许矩阵行围绕多个子空间聚类并分别分解，仅需100个样本计算梯度和评估准确率。

Result: 该方法消除了逐层扫描开销，进一步减少原始训练数据的过拟合，准确率提升高达24.6个百分点，搜索时间大幅减少。

Conclusion: 结合这些发现可得到快速鲁棒的下游任务适应算法，仅需单次梯度步和快速扫描顶层候选层及分解技术，即可实现无需微调的LLM适应。
...
TL;DR: MS-BART是一个统一建模框架，通过大规模预训练将质谱和分子结构映射到共享token词汇表，实现跨模态学习，在质谱结构解析任务中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 质谱在分子识别中至关重要，但由于标注谱图稀缺，从质谱数据解析分子结构仍然具有挑战性。现有的大规模预训练方法难以直接应用于质谱领域，因为原始谱图信号复杂且异构。

Method: 提出MS-BART框架：1）构建共享token词汇表映射质谱和分子结构；2）多任务预训练目标联合优化去噪和翻译任务；3）使用MIST模型生成指纹预测进行微调；4）引入化学反馈机制减少分子幻觉。

Result: 在MassSpecGym和NPLIB1数据集上，MS-BART在5/12关键指标上达到SOTA性能，比基于扩散的竞争方法快一个数量级，消融实验验证了模型的有效性和鲁棒性。

Conclusion: MS-BART通过统一的跨模态预训练框架有效解决了质谱结构解析中的数据稀缺问题，结合化学反馈机制显著提升了分子生成的准确性和鲁棒性。
...
TL;DR: 本文分析了差分隐私迁移学习中的两个关键超参数：裁剪边界C和批量大小B。研究发现当前理论（强隐私需要更小的C）与实证结果（强隐私下更大的C表现更好）存在不匹配，并揭示了现有批量大小调优启发式方法无效，而累积DP噪声能更好解释批量大小选择。


<details>
  <summary>Details</summary>
Motivation: 差分隐私迁移学习是目前在隐私约束下训练大模型的最先进方法，但其中两个关键超参数（裁剪边界C和批量大小B）的选择策略存在理论与实践的不一致，需要深入分析以优化性能。

Method: 通过分析梯度分布变化、将裁剪视为梯度重加权形式、检查累积DP噪声，研究在不同隐私约束和计算预算下C和B参数的选择策略。

Result: 发现当前理论指导（强隐私需要更小的C）与实证结果（强隐私下更大的C表现更好）不匹配；现有批量大小调优启发式方法无效；跨任务使用单一(C,B)设置会导致次优性能。

Conclusion: 在差分隐私迁移学习中，需要重新考虑裁剪边界和批量大小的选择策略，特别关注梯度分布变化和累积DP噪声的影响，避免跨任务使用单一参数设置，以优化模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法通常学习包含冗余信息的特征表示，缺乏对显著和非显著特征的明确区分。H-SPLID旨在通过显式分解来学习更紧凑、任务相关的特征表示，提高模型鲁棒性。

Method: H-SPLID算法将输入特征显式分解为显著和非显著两个子空间，利用Hilbert-Schmidt独立性准则(HSIC)来约束表示与输入之间的关系，并通过理论分析建立预测偏差与显著子空间维度之间的上界关系。

Result: 在图像分类任务上的实验表明，使用H-SPLID训练的模型主要依赖显著输入成分，对影响非显著特征（如图像背景）的扰动表现出降低的敏感性，验证了算法的有效性。

Conclusion: H-SPLID成功实现了显著特征与非显著特征的显式分离，建立了鲁棒性与潜在表示压缩的理论联系，为学习紧凑且鲁棒的特征表示提供了有效方法。
...
TL;DR: 该论文探讨了使用大型语言模型（LLMs）和大型多模态模型（LMMs）实现6G自主通信的框架，通过三个案例研究展示了其在交通控制、机器人调度和信道估计方面的优越性能。


<details>
  <summary>Details</summary>
Motivation: 利用LLMs和LMMs在自然语言理解和复杂推理方面的突破性能力，将其作为6G网络中机器、车辆和人形机器人之间自主通信的关键使能技术。

Method: 提出基于LLMs/LMMs的任务导向自主通信框架，整合多模态感知、自适应重构以及针对无线任务的提示/微调策略，通过三个具体案例进行验证。

Result: 实验结果表明，提出的LLM/LMM辅助自主系统在动态目标、变化输入参数和异构多模态条件下，显著优于传统方法和判别性深度学习模型，保持更强的鲁棒性。

Conclusion: LLMs和LMMs在6G自主通信中具有巨大潜力，能够有效处理动态环境和复杂任务，为未来无线通信系统提供新的解决方案。
...
TL;DR: DiRecGNN是一个用于微软云服务监控的注意力增强实体推荐框架，通过构建监控异构图和多头注意力机制，解决了传统方法在捕捉长距离依赖关系方面的不足，显著提升了推荐性能。


<details>
  <summary>Details</summary>
Motivation: 云服务监控中，传统方法难以处理实体间有限的结构信息和长距离依赖关系，导致推荐性能不佳。

Method: 构建生产规模的监控异构图，使用多头注意力机制关注异构邻居和属性，通过随机游走采样路径捕捉长距离依赖，并采用多面损失函数优化推荐。

Result: 模型在MRR指标上提升了43.1%，产品团队对功能的有用性评分为4.5/5。
...
TL;DR: 本文推导了一个新的、紧致的、可处理的KLD下界作为JSD的函数，证明了最大化JSD基础的信息测量可以增加互信息的有保证下界，为基于互信息的表示学习提供了新的理论依据。
...
Method: 推导了KLD作为JSD函数的紧致可处理下界，通过二元分类器的交叉熵损失实现JSD基础目标的实践实现，并与最先进的神经估计器进行比较。

Result: 实验表明该下界在MI估计中是紧致的，提供了稳定、低方差的紧致下界估计，并在信息瓶颈框架中展示了实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界时间序列中的极端事件（如洪水、热浪、医疗紧急事件）具有重要实际意义，但现有预测模型由于数据不平衡和忽略中间事件信息，难以准确预测这些事件。

Method: 提出xTime框架，利用知识蒸馏从低稀有度事件模型中转移信息，并引入专家混合(MoE)机制动态选择和融合不同稀有度级别的专家模型输出。
...
Method: 使用对数线性池化融合物理路径损耗模型和CNN，PL专家确保物理一致性，CNN利用建筑高度图捕捉城市传播效应，采用拉普拉斯近似的贝叶斯推断。

Result: 在城市射线追踪数据上的实验表明，随着训练点增加，定位精度提高且不确定性降低，不确定性集中在干扰源附近和传播最敏感的城区峡谷区域。

Conclusion: 该混合框架成功改善了GNSS干扰源定位和信号强度场重建，能够提供位置和RSS场的后验不确定性估计。
...
Method: MEIcoder结合神经元特异性MEIs、结构相似性指数损失和对抗训练，构建生物信息解码方法。

Result: 在V1区单细胞活动中重建视觉刺激方面达到最先进性能，特别擅长小数据集，仅需1000-2500个神经元和少于1000个训练数据点即可重建高保真自然图像。
...
<details>
  <summary>Details</summary>
Motivation: 为想要构建世界模型的研究者提供清晰的指导路线，避免泛泛而谈，专注于最有前景的技术发展路径。

Method: 遵循一条明确的技术发展路线：从早期多模态掩码模型，到统一架构，再到交互式生成模型，最后到记忆增强系统。

Result: 展示了构建真正世界模型的最有前景路径，强调生成核心、交互循环和记忆系统三个关键要素。

Conclusion: 生成核心、交互循环和记忆系统是构建真正世界模型的关键技术路径，这条路线最具发展潜力。
...
TL;DR: 提出了一种双系统方法用于持续学习和计算组合重用：一个系统推断要执行什么计算，另一个系统实现如何执行，通过概率生成模型描述任务组合性，使用无监督在线学习构建词汇表，并通过上下文推断实现RNN低秩组件的创建和学习。


<details>
  <summary>Details</summary>
Motivation: 研究持续学习和灵活组合技能的神经机制，解决持续学习中的灾难性遗忘问题，探索如何有效重用已学计算来实现高效智能行为。

Method: 采用双系统方法：what系统通过概率生成模型推断计算上下文，how系统作为RNN根据上下文组合低秩组件；使用无监督在线学习构建任务词汇表，实现单试次学习。

Result: 该方法在示例任务集上表现出高效性和竞争力，实现了前向和后向迁移，以及对未见任务的快速组合泛化，能够持续学习而不发生灾难性遗忘。

Conclusion: 双系统学习框架通过上下文推断促进RNN组件的创建、学习和重用，为持续学习和计算组合提供了有效的解决方案，展示了在认知任务中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 旨在将JHFRAT跌倒风险预测与更多临床有意义的测量指标更好地对齐，改进住院患者跌倒预防方案和患者安全。
...
Conclusion: 这种基于证据的方法为医疗系统使用数据驱动优化技术系统性增强住院患者跌倒预防协议提供了坚实基础，有助于改善风险评估和资源分配。
...
TL;DR: 本文为具有高斯边际分布的模型建立了Thompson采样在强化学习中的无遗憾保证，证明了在具有联合高斯过程先验的episodic RL中，遗憾界为$\mathcal{\tilde{O}}(\sqrt{KH\Gamma(KH)})$。


<details>
  <summary>Details</summary>
Motivation: Thompson采样在序列决策中应用广泛，但其理论基础在具有复杂时间结构的强化学习设置中仍然有限，特别是在RL中。本文旨在填补这一理论空白。

Method: 考虑在episodic强化学习中使用联合高斯过程先验对奖励和转移进行建模的Thompson采样，分析解决了价值函数的非高斯性质和Bellman更新的递归结构等挑战。

Result: 证明了在$K$个episode、每个episode长度为$H$的设置下，遗憾界为$\mathcal{\tilde{O}}(\sqrt{KH\Gamma(KH)})$，其中$\Gamma(\cdot)$捕获高斯过程模型的复杂度。
...
TL;DR: 提出基于狄利克雷过程的多模态学习框架，通过贝叶斯非参数方法自动平衡模态内表示学习和跨模态对齐，在多个数据集上表现优于现有方法。
...
Method: 假设每个模态服从多元高斯混合分布，采用狄利克雷过程计算各分量的混合权重，利用其'富者愈富'特性动态分配特征贡献度并选择最显著特征。

Result: 在多个多模态数据集上的广泛实验表明，该模型性能优于其他竞争方法，消融分析验证了DP在模态分布对齐中的有效性及其对关键超参数变化的鲁棒性。

Conclusion: 狄利克雷过程驱动的多模态学习框架能够自动实现模态内表示学习和跨模态对齐的最优平衡，为多模态特征融合提供有效解决方案。
...
TL;DR: 这篇论文研究了Transformer模型在象棋任务中的系统泛化能力，发现模型能够进行组合泛化，在分布外场景中仍能遵守游戏规则，但在复杂变体如Chess960中表现不如符号AI算法。
...
Method: 训练一个2.7亿参数的象棋Transformer模型，在分布外场景下测试其表现，包括规则外推和Chess960等变体游戏，并与符号AI算法进行对比。

Result: Transformer展现出组合泛化能力，在分布外情况下仍能选择有效移动；在Chess960中表现不如符号AI，但与人类玩家对弈时差距较小；训练动态显示模型先学习移动己方棋子。

Conclusion: Transformer模型能够学习象棋的基本组合结构，但在需要深度战略规划的复杂变体中仍不及基于显式搜索的符号AI方法。
...
<details>
  <summary>Details</summary>
Motivation: 随着图生成技术的快速发展，后门漏洞成为新的安全担忧。虽然之前的工作探索了图像扩散和无条件图生成中的后门攻击，但条件性特别是文本引导的图生成仍未被充分研究。

Method: BadGraph利用文本触发器毒化训练数据，在VAE和扩散训练期间植入后门，当推理时出现触发器时诱导攻击者指定的子图。

Result: 在四个基准数据集上的实验表明，攻击具有高效性和隐蔽性：低于10%的中毒率即可达到50%的攻击成功率，24%的中毒率足以实现超过80%的成功率，且对良性样本的性能影响可忽略不计。
...
TL;DR: 该论文挑战了反向KL散度导致"模式寻找"、正向KL散度导致"质量覆盖"的常见直觉，发现在强化学习中这种直觉不成立。作者提出了一种简单可扩展的算法，通过最小化奖励幅度调整来优化目标分布，使其覆盖所有高质量采样模式。


<details>
  <summary>Details</summary>
Motivation: 研究在强化学习中使用反向/正向KL正则化时，传统关于模式覆盖的直觉是否成立，并开发能够同时保证高质量和多样性的优化方法。

Method: 通过数学分析和实证研究，揭示了正则化系数如何决定最优目标分布族。提出了一种简单算法，最小化奖励幅度调整，优化目标分布以覆盖所有高质量采样模式。

Result: 实验表明该方法在大型语言模型和化学语言模型的后训练中都能提高解决方案的质量和多样性，无需外部多样性信号，且同时适用于正向和反向KL。

Conclusion: 模式覆盖主要取决于正则化强度、奖励与参考概率的相对尺度等因素，而非KL散度的方向。提出的算法能够有效解决传统方法在多样性方面的失败。
...
TL;DR: 提出一种量子启发的算法，使用矩阵乘积态和离散驱动调度来解决二次无约束二进制优化问题，在Sudoku和MaxCut问题上表现出色。
...
Result: 算法可靠地找到全局最小值而非近似解，在超过200个自旋的Sudoku问题和251节点3265边的MaxCut问题上均成功求解。
...
TL;DR: 该研究提出了分析可靠性基准(ARB)，用于量化大语言模型在能源系统分析中的推理可靠性，包含五个子指标，并在四种前沿模型上验证了该框架的有效性。
...
Method: 开发ARB框架，包含准确性、推理可靠性、不确定性纪律、政策一致性和透明度五个子指标，使用公开技术经济数据集在确定性、概率性和认知性场景下评估模型性能。

Result: GPT-4/5和Claude 4.5 Sonnet达到一致且符合政策的推理(分析可靠性指数>90)，Gemini 2.5 Pro表现中等稳定性，Llama 3 70B低于专业阈值，统计验证显示差异显著且可重现。
...
TL;DR: Branch-and-Browse是一个细粒度的网页代理框架，通过树状结构探索、网页状态重放和页面动作记忆，显著提升了基于大语言模型的自主网页代理的推理深度和执行效率。
...
Conclusion: Branch-and-Browse是一个可靠且高效的基于大语言模型的网页代理框架，在推理能力和执行效率方面均有显著提升。
...
TL;DR: 提出基于有向无环图(DAG)的框架来评估LLMs在数学推理中的规则一致性，引入逻辑贴近度指标，揭示即使最终答案准确率相似，不同LLM家族在推理保真度上存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有Chain-of-Thought(CoT)方法虽然能提高数学问题解决性能，但无法确定这种成功是源于搜索、机械记忆还是规则一致推理。需要新的评估框架来量化LLMs的推理保真度。

Method: 将CoT建模为基于规则的有向无环图(DAG)随机过程，节点表示中间推导状态，边编码规则应用。引入逻辑贴近度指标，并构建DAG-MATH CoT格式的基准测试。

Result: 在标准数学推理数据集上，分析发现即使PASS@k指标相似，代表性LLM家族在推理保真度上存在统计显著差异，揭示了最终答案准确率与规则一致推导之间的差距。

Conclusion: 该框架在自由形式CoT和形式证明系统之间提供了平衡，为LLMs推理评估提供了可行的诊断工具，有助于更全面地评估模型的推理能力。
...
TL;DR: Surfer 2是一个统一的视觉交互架构，在网页、桌面和移动环境中实现了最先进的性能，无需特定任务微调即可超越所有现有系统。
...
TL;DR: RELATE是一个模式无关的图神经网络特征编码器，使用共享的模态特定编码器和交叉注意力机制，在保持性能的同时大幅减少参数数量。
...
Result: 在RelBench基准测试中，RELATE在性能上仅比模式特定编码器低3%，同时将参数数量减少了5倍。

Conclusion: RELATE支持不同的数据模式，并为关系图数据的通用图神经网络预训练铺平了道路。
...
TL;DR: 生成式AI正在加剧保险欺诈，使大规模快速伪造事故证据变得容易。欺诈者利用深度伪造技术制造逼真的撞车照片和文件，而保险公司采用AI检测工具应对，但面临误报和欺诈者不断进化的挑战。


<details>
  <summary>Details</summary>
Motivation: 保险欺诈每年造成数百亿美元损失，生成式AI的出现使欺诈手段升级，能够大规模伪造逼真证据，传统防欺诈方法已不足以应对这一新威胁。

Method: 保险公司部署基于AI的深度伪造检测软件和增强验证流程来检测和减轻AI驱动的欺诈行为。

Result: 当前检测工具存在误报和漏报问题，欺诈者不断调整策略规避自动检查，形成猫鼠游戏。资源成本限制也使保险公司面临挑战。

Conclusion: 对抗AI驱动的保险欺诈仍是持续挑战，需要更先进的解决方案来检测、减轻和威慑这种新型欺诈浪潮。
...
Method: 收集129名硕士生的5项领导力人格测试数据（23个特征），结合平均成绩，使用7种ML算法（SVM、LR、KNN、DT、GB、RF、XGBoost、LightGBM）进行建模，采用皮尔逊相关系数进行特征选择。

Result: 随机森林分类器表现最佳，包含17个人格特征和领导力标记特征的模型准确率达87.50%，不包含该特征的模型准确率为85.71%。

Conclusion: 研究证实了通过领导力人格特质预测学术成功的可行性，为教育过程中的早期干预和个性化学习策略选择提供了有效工具。
...
TL;DR: AI PB是一个部署在零售金融领域的生产级生成式智能体，能够主动生成基于事实、合规且个性化的投资洞察，采用组件化编排、混合检索和多阶段推荐机制。
...
Result: 系统在韩国金融监管环境下完全本地化部署，使用Docker Swarm和vLLM跨24个NVIDIA H100 GPU运行，通过人工QA和系统指标验证了其有效性。

Conclusion: 通过显式路由和分层安全机制的基于事实的生成方法，可以在高风险金融领域提供可信的AI洞察。
...
TL;DR: HCLA是一个以人为中心的多智能体异常检测系统，用于数字资产交易。它将解析、检测和解释三个角色连接成对话式工作流，让非专家能用自然语言提问、检查结构化分析并获得上下文感知的解释。


<details>
  <summary>Details</summary>
Motivation: 提高金融取证中的透明度和信任度，让非专家用户能够理解和参与数字资产交易的异常检测过程。

Method: 采用多智能体架构，将解析、检测和解释三个角色集成到对话式工作流中，使用XGBoost作为基础检测器，通过开源Web UI实现用户意图到检测模式的转换。

Result: 在标记的比特币混合数据集上，基线检测器达到强准确率，HCLA系统增加了可解释性和交互式优化能力。

Conclusion: 人在回路的设计能够显著改善金融取证中的透明度和信任度，多智能体对话式系统为非专家用户提供了有效的异常检测工具。
...
TL;DR: 论文认为需要重新评估AI在法律实践中的使用，提出了验证-价值悖论模型，指出AI带来的效率提升会被相应的验证需求抵消，导致净价值往往微不足道。


<details>
  <summary>Details</summary>
Motivation: 针对律师因提交不准确的AI生成内容而受到谴责的案例，以及AI与现实脱节、缺乏透明性的特点，需要重新审视AI在法律实践中的使用范式。

Method: 提出了验证-价值悖论模型，该模型更全面地反映了AI使用的特征，并分析了该悖论对法律实践和法律教育的影响。

Result: AI在法律实践中带来的效率提升会被相应的验证需求所抵消，导致净价值往往可以忽略不计。

Conclusion: 需要建立新的AI使用范式，强调对真理的忠诚和公民责任等价值观应成为法律实践的基础。
...
TL;DR: 提出TRUST框架，一个去中心化的AI审计系统，通过共识机制、分层DAG分解、区块链记录和隐私保护分段，解决大语言模型推理链验证的鲁棒性、可扩展性、透明度和隐私问题。


<details>
  <summary>Details</summary>
Motivation: 现有审计方法存在中心化、不透明、难以扩展的问题，无法有效验证大语言模型推理链的忠实性和无害性，限制了专有模型在高风险领域的部署。

Method: 采用共识机制确保在30%恶意参与者下仍能正确审计；使用分层DAG分解推理链实现可扩展并行审计；通过区块链记录所有验证决策保证公开问责；采用隐私保护分段仅共享部分推理步骤保护专有逻辑。

Result: 在多个LLM（GPT-OSS、DeepSeek-r1、Qwen）和推理任务（数学、医学、科学、人文）上的实验表明，TRUST能有效检测推理缺陷，并对抗恶意审计者保持鲁棒性。
...
TL;DR: 该论文提出AGI发展需要经历一个锁定阶段，即从开放模仿转向身份整合，使目标结构、拒绝行为、偏好和内部表征变得稳定且抵抗外部操控。


<details>
  <summary>Details</summary>
Motivation: 基于人类发展的类比，假设AGI进步需要经历身份整合阶段，这是实现AGI级可靠性的前提，也是安全控制的关键点。

Method: 形式化这一阶段，将其与学习动态中的已知现象联系起来，提出操作性指标进行检测，并通过实验验证行为整合的动态特征。

Result: 实验显示行为整合是快速非线性的，但对通用能力的影响不是单一的：小型模型出现性能权衡，中等规模模型几乎无成本采用，大型量化模型出现瞬态不稳定性。

Conclusion: 身份整合是AGI可靠性的必要条件，也是安全的关键控制点，可以工程化设计可靠身份，但也可能在扩展过程中自发出现，硬化不可预测的目标和行为。
...
TL;DR: 本文研究了在2048游戏中优化AI的进化训练方法，比较了单智能体系统和双智能体系统在非确定性环境中的表现。
...
Method: 实现两种系统：双智能体元提示系统（"思考者"LLM优化"执行者"LLM的游戏策略）和基于蒙特卡洛树搜索价值函数优化的单智能体系统，并实验了回滚功能以避免性能退化。

Result: 单智能体系统取得显著改进，每个周期平均增加473.2分，训练周期呈明显上升趋势（相关性ρ=0.607）。LLM对游戏的理解也随高级策略的发展而增长。双智能体系统改进有限，突显了元提示的内在局限性。

Conclusion: 进化优化技术在改进非确定性环境中AI性能方面具有潜力，单智能体方法优于双智能体元提示方法。
...
TL;DR: 该论文提出了个性化认知模拟(ICS)任务，通过构建基于新出版小说的数据集和11条件认知评估框架，测试了7个现成LLM在作者风格模仿中的表现。研究发现结合概念和语言特征的方法最有效，但LLM在模仿语言风格方面优于叙事结构。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM能够模仿表面的人类行为，但其模拟更深层次个性化认知过程的能力仍不清楚。研究旨在评估不同认知表征方法在个性化认知模拟中的效果。

Method: 构建基于新出版小说的数据集，提出11条件认知评估框架，测试7个现成LLM，比较不同认知表征方法（语言特征、概念映射、基于档案的信息等）。

Result: 结合概念和语言特征的方法在ICS中特别有效，优于基于静态档案的线索。LLM在模仿语言风格方面比叙事结构更有效。

Conclusion: 这些发现为开发能够适应个体思维和表达方式的AI系统奠定了基础，推进了更个性化和人类对齐的创意技术发展。
...
<details>
  <summary>Details</summary>
Motivation: 动态领域的抽象生成对智能体的规划、推理和解释能力有重要影响，但目前仍是重大挑战。

Method: 在PDDL中建模具体行为，使用LLM的上下文学习生成抽象PDDL领域和问题实例，考虑三种抽象类别：替代具体动作选择、具体动作序列、动作/谓词参数。

Result: GPT-4o在简单设置中能有效合成规划领域抽象，但在动作抽象方面优于关联流体的抽象。

Conclusion: LLM能够生成有用的规划领域抽象，特别是在动作抽象方面表现良好。
...
TL;DR: 提出了STaBERT模型，通过整合POI和时序信息来提升人类移动性预测的准确性，在单城市和多城市预测中都显著提升了GEO-BLEU分数。
...
Method: 提出了STaBERT模型，在BERT-based移动性模型中融入推导的时序描述符和POI嵌入，在每个位置整合POI和时序信息，构建统一的语义增强移动性表示。

Result: 实验结果显示STaBERT显著提升了预测准确性：单城市预测的GEO-BLEU分数从0.34提升到0.75；多城市预测从0.34提升到0.56。

Conclusion: 通过整合POI语义和时序信息，STaBERT能够更好地捕捉人类移动的底层语义，显著提升了移动性预测性能。
...
Result: 在EQA-RT-Seen和EQA-RT-Unseen测试集上，ToolEQA相比SOTA基线成功率提升9.2~20.2%，比零样本版本提升10%；在HM-EQA、OpenEQA和EXPRESS-Bench数据集上也达到SOTA性能。
...
<details>
  <summary>Details</summary>
Motivation: AI在医疗领域应用受限的主要障碍是训练数据的质量和公平性问题，特别是数据收集中存在的偏见会严重影响AI系统的公平性和鲁棒性。

Method: 基于西班牙国家研发计划AI4HealthyAging项目的实践经验，通过分析多个用例来识别临床数据收集中存在的偏见类型。

Result: 识别出多种偏见类型，包括历史偏见、代表性偏见和测量偏见，这些偏见体现在性别、年龄、居住环境、社会经济状况、设备和标签等多个变量中。

Conclusion: 提出了改进临床问题设计和数据收集的实用建议，旨在提高AI系统的公平性和鲁棒性，为未来开发更公平的医疗AI系统提供指导。
...
TL;DR: 提出了一种用于军事行动中AI系统目标打击的附带损害评估模型，该模型在知识表示与推理架构中整合了时间、空间和力量维度。
...
Conclusion: 该模型为评估军事行动中AI系统打击效果提供了透明推理机制，有助于建立负责任和可信赖的智能系统。
...
TL;DR: 本综述系统回顾了LLM赋能知识图谱构建的最新进展，分析了LLM如何重塑传统的本体工程、知识提取和知识融合三层流水线，并探讨了基于模式和无模式两种构建范式的互补优势。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的出现，知识图谱构建正从基于规则和统计的流水线转向语言驱动和生成框架，需要系统梳理这一范式转变及其技术进展。

Method: 从两个互补视角回顾LLM驱动方法：基于模式的范式强调结构、规范化和一致性；无模式范式强调灵活性、适应性和开放发现。

Result: 系统分析了各阶段的代表性框架、技术机制及其局限性，阐明了LLM与知识图谱之间的演进关系。
...
<details>
  <summary>Details</summary>
Motivation: 解决持续预训练中指令调优模型语义表示退化和指令跟随能力下降的问题，特别是在无法访问基础模型权重或外部领域数据库的现实场景下。

Method: IKnow框架通过指令-响应对话格式设计自监督目标，利用文本中嵌入的领域知识，在更深语义层面进行编码学习。

Result: 该方法能够在无需外部资源的情况下有效适应新领域，同时保持模型的指令跟随能力。

Conclusion: IKnow提供了一个简单通用的持续适应框架，不依赖外部资源即可实现语言模型的有效领域适应。
...
<details>
  <summary>Details</summary>
Motivation: 开发能够生成既新颖又有用的创新机会的计算模型，解决现有方法在创新性方面的不足。

Method: 构建包含五个功能模块的计算模型，基于创造力理论和技巧，专门设计用于生成高新颖性且保持实用性的创新机会。

Result: 在酒店业创新项目评估中，该模型生成的成果比Notebook LM和ChatGPT4o更具新颖性和/或实用性，但并非所有功能模块都对提升新颖性有贡献。

Conclusion: 该计算模型在生成创新机会方面表现出优势，但部分功能模块效果有限，为后续模型优化提供了新的研究方向。
...
Method: EBR通过嵌入来近似符号推理器的结果，仅需要检索原子概念和存在限制的实例，就能检索或近似任何SHOIQ描述逻辑概念的实例集合。

Result: 实验表明，与最先进的推理器相比，EBR对缺失和错误数据具有鲁棒性。

Conclusion: EBR为神经符号概念学习提供了一种鲁棒的推理方法，能够在现实世界知识库中有效部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有知识图谱对齐方法主要关注纯实体级对齐，缺乏可解释性推理且需要训练数据。FLORA旨在解决这些问题，提供无需训练数据的可解释对齐方法。

Method: 基于模糊逻辑的迭代方法，提供整体性的实体和关系对齐，允许存在悬空实体（在另一个KG中没有对应实体的实体）。

Result: 在主要基准测试中达到最先进的结果，方法具有可证明的收敛性。

Conclusion: FLORA是一种简单有效的无监督知识图谱对齐方法，通过模糊逻辑提供可解释的对齐结果，能够处理悬空实体并实现最先进的性能。
...
TL;DR: 该研究比较了澳大利亚、哥伦比亚和美国三个国家在AI治理中的公众参与情况，发现政府未能建立有效的对话机制，公众参与率极低（不到1%），且政府对反馈的回应有限，导致参与式AI治理的承诺与实践之间存在差距。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探讨政府在AI治理中如何征求公众意见，以及这些意见是否真正影响了政策制定，以解决AI治理中公众参与不足和信任缺失的问题。

Method: 采用景观分析方法，比较了澳大利亚、哥伦比亚和美国三个国家政府征求AI风险和政策反馈的方式，并评估了这些反馈是否影响了治理决策。

Result: 研究发现三个国家均未能建立有意义的公众对话机制，政府缺乏多样化的声音征集和宣传，公众参与率极低，且政府对反馈的回应有限，未能形成有效的反馈循环。

Conclusion: 当前参与式AI治理方法难以建立信任或合法性，因为政策制定者未能充分倾听和回应公众关切。作者提出了八项改进建议，包括提升AI素养、扩大宣传、采用创新参与方法等。
...
TL;DR: 提出一种基于图神经网络的网络拓扑优化方法，通过母线分裂缓解电网拥堵，实现大规模系统近实时优化，比传统方法快4个数量级。


<details>
  <summary>Details</summary>
Motivation: 现有求解器无法在近实时内解决大规模电网的混合整数非线性拓扑优化问题，机器学习方法又缺乏对未见拓扑、运行条件和不同系统的泛化能力。

Method: 使用异质边感知消息传递图神经网络预测有效的母线分裂动作作为候选拓扑优化方案，考虑线性化交流潮流模型。

Result: 在GOC 2000节点系统上实现4个数量级的加速，1分钟内提供交流可行解，最优性差距仅为2.3%。

Conclusion: 该方法在大规模系统拓扑优化中实现了拓扑和跨系统泛化，是迈向近实时网络拓扑优化的重要一步。
...
TL;DR: 提出因果逐步评估方法(CaSE)，通过评估推理步骤的相关性和连贯性来改进LLM推理能力，而不仅仅是关注最终答案的正确性。


<details>
  <summary>Details</summary>
Motivation: 当前对大型语言模型的评估主要关注最终答案的正确性，这种方法过于粗略，忽略了底层推理过程的质量，无法为模型改进提供有效的细粒度信号。

Method: 将推理质量分解为相关性和连贯性两个维度，提出CaSE方法，仅使用前文语境评估每个推理步骤，避免后见之明偏差。创建了专家标注的基准MRa-GSM8K和MRa-MATH。

Result: CaSE方法与人类判断一致，使用CaSE评估的相关性和连贯性数据来筛选训练数据可以直接提高最终任务性能。

Conclusion: 这项工作为分析、调试和改进LLM推理提供了可扩展的框架，证明了超越有效性检查的实际价值。
...
TL;DR: 提出了两种可扩展的随机游走中心性算法，能够在近线性时间内计算大型网络中的节点重要性。


<details>
  <summary>Details</summary>
Motivation: 随机游走中心性能够捕捉丰富的图结构信息并具有广泛应用，但在大型网络中的计算由于现有方法的高计算需求而不切实际。

Method: 提出了一种新的随机游走中心性公式，基于此开发了两种算法：一种利用近似Cholesky分解和稀疏逆估计，另一种采样根生成树。

Result: 在包括超过1000万个节点的大型真实网络上的广泛实验证明了所提算法的效率和近似质量。

Conclusion: 两种算法都能在近线性时间内运行并提供强近似保证，为大规模网络中的随机游走中心性计算提供了实用解决方案。
...
TL;DR: 提出了MIMOSA框架，用于生成在可解释性和性能之间取得平衡的预测模型，同时嵌入因果性、公平性和隐私性等关键伦理属性。
...
Method: 形式化定义了监督学习设置，涵盖表格数据、时间序列、图像、文本等多种数据类型。分析了特征重要性、规则和实例三类可解释模型，并形式化了因果性、公平性和隐私性三个伦理属性。

Result: 建立了开发不仅准确可解释，而且公平、保护隐私和具有因果意识的AI系统的理论基础。

Conclusion: MIMOSA框架为构建可信赖的AI系统提供了理论基础，通过模型生成过程中评估伦理指标，确保模型在可解释性、性能和伦理属性之间取得平衡。
...
Method: 构建包含6个类别37个任务（含8个多模态任务）的基准，主要来自真实客户查询和交易日志；采用半自动流程，由大模型生成候选答案，再由50多名专家审核修改；定义难度级别并覆盖7种语言。

Result: EcomEval提供了一个更全面、真实的多语言多模态电子商务评估基准，能够进行挑战导向和细粒度评估。

Conclusion: EcomEval填补了电子商务领域LLM评估的空白，为从业者提供了可靠的工具来评估模型在复杂真实购物场景中的能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试难以评估模型在动态扩展环境中的真正适应性，需要开发能够测试模型理解、预测和适应状态变化能力的评估方法。

Method: 设计流动性指数(FI)，基于环境状态偏差评估响应准确性，区分封闭式和开放式基准，优先采用闭环开放式真实世界基准来测试适应性。

Result: 提出了评估模型适应性的新框架，强调真正超级智能模型应具备至少二阶适应性，能够通过数字补充实现自持计算以达到最优流动性。

Conclusion: 流动性指数为评估模型在动态扩展环境中的适应性提供了量化标准，二阶适应性是实现超级智能的关键特征。
...
TL;DR: 本文对将机器学习集成到理性智能体架构中的现有方法进行了系统化梳理，特别以BDI（信念-欲望-意图）范式为参考框架，分析了相关文献并识别了关键研究机会和挑战。


<details>
  <summary>Details</summary>
Motivation: 由于机器学习模型在感知和认知任务中展现出类人能力，将其集成到理性智能体架构中的框架日益受到关注。然而，当前研究领域呈现碎片化，往往只关注将ML嵌入通用智能体容器，而忽视了理性架构（如BDI智能体）的表达能力。

Method: 采用BDI范式作为参考框架，对现有方法进行细粒度的系统化分类和分析，梳理了ML增强理性智能体的快速发展文献。

Result: 分析揭示了该领域的快速演进趋势，并系统化地展示了不同集成方法的分类框架。
...
TL;DR: 提出基于拓扑数据分析(TDA)的评估框架，用于自动评估大语言模型推理轨迹的质量，相比传统图指标具有更高预测能力。


<details>
  <summary>Details</summary>
Motivation: 当前评估大语言模型推理轨迹质量的方法依赖专家标注、人工判断，效率低且不可靠。现有自动化方法主要使用图结构指标，但无法准确反映推理质量。

Method: 引入拓扑数据分析(TDA)框架，捕捉推理轨迹的几何结构特征，实现标签高效的自动评估。
...
Method: 两阶段方法：1) 使用定制计划检索数据集进行思维链微调，激活结构化推理；2) 引入计划检索引导的强化学习过程，集成显式规划和检索动作，采用笛卡尔式规划模块分解复杂问题，逻辑表达式指导工具调用。
...
TL;DR: 本文提出了一种基于广义均值积分的一致性感知AGI度量方法，替代了传统算术平均方法，能够更好地评估跨认知领域的平衡能力。


<details>
  <summary>Details</summary>
Motivation: 现有AGI定义采用算术平均，假设领域间可补偿性，但真正的通用智能应该反映所有基本领域的平衡能力，需要避免单一领域优势掩盖其他领域的不足。

Method: 基于广义均值在补偿性指数连续统上的积分，构建面积下曲线(AUC)度量，涵盖算术、几何和调和均值机制，量化不同补偿性假设下的鲁棒性。

Result: 应用于GPT-4和GPT-5的CHC领域得分，一致性调整的AUC显示尽管算术得分较高(如GPT-5达到24%)，但两个系统距离通用能力仍然很远。

Conclusion: 整合广义均值提供了一个原则性、可解释且更严格的AGI进展测量基础，能够惩罚不平衡并捕捉领域间依赖性。
...
<details>
  <summary>Details</summary>
Motivation: AI和机器人领域年产出超过1万篇论文，研究人员难以跟上快速发展的趋势、跨学科工作和探索新领域的需求。

Method: 构建通用的RDR分析管道，系统性地分析任何研究领域，特别应用于AI和机器人领域，重点关注基础模型和机器人技术进步。

Result: 开发了全面的RDR框架，能够识别新兴趋势、发现跨领域机会，并为新研究提供具体起点。
