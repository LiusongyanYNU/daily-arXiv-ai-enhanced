...
TL;DR: PointCoT：一种通过显式思维链推理增强MLLMs 3D点云理解能力的新框架，采用"观察-思考-回答"范式，在预测答案前生成几何基础推理，解决了现有方法中的几何幻觉问题。
...
Method: 提出PointCoT框架，采用"观察、思考、然后回答"范式，监督模型在预测最终答案前生成几何基础的推理依据。构建了包含约86k个指令调优样本的Point-Reason-Instruct大规模基准数据集，采用双流多模态架构，协同整合语义外观和几何真实信息。

Result: PointCoT在复杂推理任务上实现了最先进的性能，通过显式思维链推理有效解决了几何幻觉问题，显著提升了3D点云理解的准确性和可靠性。

Conclusion: 通过引入显式思维链推理机制，PointCoT成功将MLLMs的感知智能扩展到3D点云理解，为3D几何推理提供了更可靠、可解释的解决方案，推动了多模态AI在3D场景理解方面的发展。
...
TL;DR: GuardAlign是一个无需训练的大视觉语言模型安全防御框架，通过最优传输增强的安全检测和跨模态注意力校准，有效降低不安全响应率同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的输入侧防御方法在复杂场景下检测不准确，且在解码过程中安全信号不稳定，需要更有效的安全防御方案。

Method: 1. OT增强的安全检测：利用最优传输测量图像块与不安全语义的分布距离，准确识别恶意区域；2. 跨模态注意力校准：通过自适应重新分配各层注意力，确保安全信号在生成过程中持续激活。

Result: 在6个代表性MLLMs上的评估显示，GuardAlign在SPA-VL上将不安全响应率降低高达39%，同时在VQAv2上性能从78.51%提升到79.21%，保持实用性。

Conclusion: GuardAlign通过训练免费的框架有效解决了现有防御方法的局限性，在准确检测不安全内容和稳定维持安全信号方面表现出色，为大视觉语言模型的安全防御提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有布局生成模型常无法符合人类审美判断，而文本到图像生成的偏好数据集和奖励模型不适用于布局评估（空间排列决定质量）。

Method: 1) 创建DesignSense-10k数据集：10,235个人工标注的偏好对，使用五阶段流程生成布局变换；2) 训练DesignSense：基于视觉语言模型的分类器；3) 应用：在RL训练中使用奖励模型，以及推理时多候选选择。

Result: DesignSense在综合评估指标上大幅超越现有开源和专有模型（Macro F1提升54.6%），前沿VLM在四类任务上表现不可靠。下游应用：RL训练提升生成器胜率约3%，推理时缩放提升3.6%。

Conclusion: 需要专门的、偏好感知的布局评估模型，DesignSense-10k数据集和DesignSense模型能显著提升布局生成质量，具有实际应用价值。
...
Method: 基于Unity引擎构建，模拟抽象可配置的3D场景，对象具有随机生成的行为属性（运动、旋转等），异常对象标记采用基于中心极限定理的统计过程。

Result: 开发了ANTShapes框架，可通过调整少量参数创建任意数量的数据集样本，并导出标签和帧数据。

Conclusion: ANTShapes解决了事件型计算机视觉研究中的数据可用性问题，支持定制化数据集生成，适用于物体识别、定位和异常检测等任务。
...
TL;DR: 提出LIDMark统一主动取证框架，通过152维地标-身份水印同时解决深度伪造检测、篡改定位和来源追踪三个任务，使用因子化头解码器实现鲁棒提取。
...
Method: 提出LIDMark框架：1）152维地标-身份水印，将面部地标与唯一来源标识符结构交织；2）因子化头解码器，将共享骨干特征分解为回归头和分类头，分别鲁棒重建嵌入的地标和标识符。

Result: 实验表明LIDMark框架为深度伪造内容的检测、定位和追踪提供了统一、鲁棒且不可察觉的解决方案。回归头通过"内在-外在"一致性检查实现检测和定位，分类头鲁棒解码来源标识符实现追踪。

Conclusion: LIDMark框架成功实现了"一体化"三功能取证解决方案，填补了现有主动取证方法缺乏统一框架的空白，为深度伪造威胁提供了有效的技术应对。
...
TL;DR: SVG2是一个大规模全景视频场景图数据集，包含63.6万个视频、660万个对象、5200万个属性和670万个关系，比现有数据集规模大一个数量级。基于此数据集训练了TRaSER模型，该模型在多个测试集上显著提升了场景图生成性能。
...
Method: 1. 创建SVG2数据集：采用全自动流水线，结合多尺度全景分割、在线-离线轨迹跟踪、自动新对象发现、轨迹语义解析和GPT-5时空关系推理。2. 训练TRaSER模型：在VLMs基础上增加轨迹对齐的token排列机制，包含对象轨迹重采样器和时间窗口重采样器，将原始视频和全景轨迹转换为紧凑的时空场景图。
...
Conclusion: SVG2数据集为视频场景图研究提供了大规模资源，TRaSER模型证明了显式时空场景图作为中间表示的有效性，能够显著提升视频理解和问答性能。
...
TL;DR: LE-NeuS是一个延迟高效的神经符号框架，通过自适应采样和批量命题检测，将长视频问答的推理延迟从90倍降低到10倍，同时保持准确性优势。
...
Method: 采用两种优化：1) CLIP引导的两阶段自适应采样，跳过语义相似的帧；2) 批量命题检测，并行化VLM推理。理论分析延迟边界与视频长度、命题复杂度和采样密度的关系。

Result: 在LongVideoBench和Video-MME基准测试中，LE-NeuS将延迟差距从90倍降低到约10倍，同时在时间复杂查询上保持>10%的准确性提升。

Conclusion: LE-NeuS在保持神经符号方法准确性优势的同时，显著降低了推理延迟，使时间逻辑引导的视频理解在延迟敏感场景中变得实用。
...
<details>
  <summary>Details</summary>
Motivation: 现有RGB-X研究工作大多假设已存在对齐的RGB-X数据对，专注于模态融合，但实际上获取对齐数据需要大量工程校准工作。本文旨在消除各种RGB-X传感器的繁琐校准，通过可扩展解决方案突破大规模真实世界RGB-X数据收集的瓶颈

Method: 提出匹配-稠密化-整合方法：1) 执行RGB-X图像匹配，2) 进行引导点稠密化（使用置信度感知稠密化和自匹配过滤），3) 在3D高斯泼溅(3DGS)中整合结果。方法不依赖X传感器的3D先验，仅假设使用几乎无成本的COLMAP处理RGB数据

Result: 实现了更好的视图合成效果，为跨传感器学习提供了可扩展解决方案，能够突破大规模真实世界RGB-X数据收集的瓶颈

Conclusion: 该方法消除了RGB-X传感器的繁琐校准需求，通过可扩展的解决方案推进了跨传感器学习的普及，为大规模真实世界RGB-X数据收集提供了突破性方法
...
TL;DR: 提出Evidential Neural Radiance Fields，一种概率方法，可在单次前向传播中同时量化偶然性和认知不确定性，在保持渲染质量的同时实现最先进的不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 神经辐射场（NeRFs）在场景重建和新视角合成方面取得显著进展，但缺乏不确定性估计限制了其在安全关键场景中的应用。现有方法无法同时捕捉偶然性和认知不确定性，且往往以牺牲渲染质量或增加计算开销为代价。
...
Conclusion: 该方法有效解决了NeRFs中不确定性估计的挑战，在保持高质量渲染的同时实现了高效的不确定性量化，为安全关键应用提供了可靠的三维场景建模工具。
...
TL;DR: CycleBEV：通过循环一致性正则化增强BEV语义分割的视图转换模型，无需增加推理复杂度


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中从透视视图到鸟瞰图的特征转换面临深度模糊和遮挡挑战，现有视图转换模型仍有改进空间

Method: 提出循环一致性正则化框架，设计逆向视图转换网络将BEV分割图映射回PV分割图，通过循环一致性损失训练，并扩展到几何和表示空间

Result: 在nuScenes数据集上对四种代表性VT模型均有显著提升，可行驶区域、车辆、行人类别分别提升最高达0.74、4.86、3.74 mIoU

Conclusion: CycleBEV通过循环一致性正则化有效增强BEV语义分割性能，IVT网络仅用于训练不增加推理复杂度，具有实用价值
...
TL;DR: HDFLIM框架通过超维计算实现冻结视觉和语言模型的跨模态对齐，无需参数更新，仅使用符号操作构建关联表示，性能媲美端到端训练方法。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态对齐方法需要计算密集的多模态微调，涉及大规模参数更新、资源消耗大，且可能扰动预训练表示。研究发现独立训练的基础模型可能已存在潜在语义兼容性，因此探索能否在不修改模型本身的情况下实现跨模态对齐。

Method: HDFLIM将单模态嵌入投影到共享超维空间，利用轻量级符号操作（绑定、捆绑和基于相似性的检索）在单次数据遍历中构建关联跨模态表示。字幕生成通过高维记忆检索而非迭代梯度优化实现。

Result: HDFLIM性能与端到端视觉语言训练方法相当，生成的标题比零样本基线更具语义基础。通过将对齐与参数调优解耦，证明语义映射可以通过超维编码的符号操作实现。

Conclusion: 这项工作指向了基础模型对齐的替代范式：通过结构化表示映射而非大规模重新训练来集成冻结模型。超维计算为跨模态对齐提供了高效、资源友好的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态模型（如CLIP）在需要细粒度视觉差异的领域（如图表理解）表现不佳，因为模型对结构变化的敏感性有限。图表中微小的视觉差异往往承载重要的语义信息，需要专门的训练策略来提升模型对图表结构的理解能力。

Method: 提出新的训练范式：使用图表渲染器生成伪对比样本。通过随机选取文本元素创建合成图表，这些样本突出图表图像中的结构差异，无需修改原始数据。将这些伪对比样本融入训练目标，使模型学习捕获更精确和语义一致的图表结构。

Result: 在流程图基准数据集上的实证评估显示，在图像-文本匹配和视觉问答任务中，该方法相比标准CLIP和hard-negative CLIP训练都有显著改进。结果证明了领域特定训练策略的价值。

Conclusion: 该方法通过伪对比样本增强图表理解能力，为视觉语言学习中的图表理解领域提供了有效的训练策略，推动了多模态模型在细粒度视觉理解方面的发展。
...
Method: 提出增量降维算法：将特征向量分批处理，每批更新截断奇异值分解，分批降维存储，最后将所有批次奇异向量重新转换到原始特征空间

Result: 算法能加速最先进异常检测算法的训练，同时保持接近的准确率

Conclusion: 增量降维算法有效解决了大规模视觉异常检测中的计算和内存问题，实现了高效训练
...
TL;DR: HART是一个无需标注的高分辨率视觉推理框架，通过自验证关键区域和策略优化，提升大模型处理高分辨率图像的能力。


<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型在处理高分辨率视觉输入时面临挑战，因为图像token数量随分辨率呈二次增长，引入大量冗余和无关信息。现有方法通常需要昂贵的人工标注来识别关键区域，如何在没有额外标注的情况下增强模型的定位能力以支持推理仍是一个开放问题。

Method: 提出HART（高分辨率无标注推理技术），这是一个闭环框架，包含后训练范式。设计了Advantage Preference Group Relative Policy Optimization（AP-GRPO）来鼓励准确定位关键区域，使模型能够聚焦并自验证高分辨率视觉输入的关键区域。

Result: HART在广泛的高分辨率视觉任务上提升了性能，一致优于强基线。当应用于Qwen2.5-VL-7B的后训练时，HART甚至在高分辨率、视觉中心的基准测试中超越了更大规模的模型，如Qwen2.5-VL-72B和LLaVA-OneVision-72B。

Conclusion: HART提供了一个无需标注的解决方案，能够有效提升大型多模态模型处理高分辨率视觉输入的能力，通过自验证关键区域和策略优化实现了可解释的推理路径和高效的定位优化。
...
<details>
  <summary>Details</summary>
Motivation: 第一人称视角人体姿态估计面临关键点不可见问题，现有数据集缺乏可见性标注，方法也忽视可见性问题，导致可见关键点预测精度受限。

Method: 1) 构建Eva-3M数据集（300万帧，43.5万帧带可见性标注）；2) 为EMHI数据集补充可见性标注；3) 提出EvaPose方法，显式融入可见性信息增强姿态估计。

Result: 实验验证了地面真实可见性标签在第一人称视角HPE中的重要性，EvaPose在Eva-3M和EMHI数据集上均达到最先进性能。

Conclusion: 关键点可见性信息对第一人称视角人体姿态估计至关重要，提出的数据集和方法为解决可见性问题提供了有效方案。
...
TL;DR: 提出了首个专注于评估指令式图像编辑模型在小物体编辑能力上的基准测试DLEBench，包含1889个样本和7种指令类型，目标物体仅占图像1%-10%面积，并设计了双模式评估框架来减少主观性。


<details>
  <summary>Details</summary>
Motivation: 现有指令式图像编辑模型虽然在当前基准测试上表现良好，但小物体编辑能力尚未得到充分探索，而这对精确局部编辑和细节优化至关重要。

Method: 构建了DeepLookEditBench基准测试，包含1889个样本覆盖7种指令类型，目标物体仅占图像1%-10%面积，涵盖部分遮挡和多物体编辑等复杂场景。提出了包含细化评分标准的评估协议，并设计了工具驱动和Oracle引导的双模式评估框架。

Result: 在10个指令式图像编辑模型上的实验结果显示，小物体编辑存在显著性能差距，表明需要专门基准测试来推进这一能力。

Conclusion: 小物体编辑是指令式图像编辑模型的重要挑战，需要专门基准测试来评估和推进这一能力，提出的DLEBench和评估框架为此提供了有效工具。
...
TL;DR: BuildAnyPoint：基于点云的结构化3D建筑重建生成框架，通过松散级联扩散变换器实现分布恢复和自回归网格生成


<details>
  <summary>Details</summary>
Motivation: 解决从不同分布的点云（如机载LiDAR和运动恢复结构）进行结构化3D建筑重建的挑战，恢复艺术家创建的抽象建筑表示

Method: 设计松散级联扩散变换器（Loca-DiT）：1）训练条件潜在扩散模型从噪声或稀疏点云恢复底层分布；2）使用仅解码器变换器基于恢复的点云进行条件自回归网格生成

Result: 在建筑抽象方法上取得显著质量和数量改进，恢复的点云在建筑点云补全基准测试中表现出优异的表面精度和分布均匀性

Conclusion: BuildAnyPoint框架通过结合显式3D生成先验和自回归网格生成，有效解决了高度欠约束的建筑重建问题，为结构化3D建筑重建提供了新方法
...
<details>
  <summary>Details</summary>
Motivation: 将视觉语言模型应用于多器官医学影像面临两个主要挑战：模态特定的视觉-语言对齐和跨模态特征融合。现有方法在3D MRI多器官异常检测上表现不足。

Method: 提出MedMAP框架，包含两个阶段：1) 模态感知视觉-语言对齐预训练阶段，使用模态感知编码器隐式捕获联合模态分布；2) 微调阶段，保持文本编码器冻结，仅微调视觉编码器用于下游任务。同时构建了MedMoM-MRI3D数据集。

Result: 在MedMoM-MRI3D数据集上的实验表明，MedMAP在3D MRI多器官异常检测任务上显著优于现有的视觉语言模型。

Conclusion: MedMAP通过模态感知的视觉-语言对齐预训练，有效解决了多器官医学影像中的模态特定对齐和特征融合问题，在3D MRI异常检测任务上取得了显著改进。
...
TL;DR: ProtoDCS：基于原型的双重检查分离框架，解决视觉语言模型在开放集测试时适应中的分布偏移问题，通过概率验证和原型级更新实现高效可靠的csID/csOOD分离与适应。


<details>
  <summary>Details</summary>
Motivation: 现有VLM测试时适应方法基于闭集假设，无法处理开放集场景中同时包含协变量偏移内分布(csID)和外分布(csOOD)数据的问题。传统方法使用硬阈值分离和熵最小化适应，存在脆弱性、误分类和计算成本高的问题。

Method: 提出原型双重检查分离框架：1）使用概率高斯混合模型验证替代脆性阈值，实现双重检查分离机制；2）采用证据驱动的适应策略，结合不确定性感知损失和高效原型级更新，避免过度自信并降低计算开销。

Result: 在CIFAR-10/100-C和Tiny-ImageNet-C数据集上的实验表明，ProtoDCS达到最先进性能，显著提升已知类别准确率和OOD检测指标。

Conclusion: ProtoDCS为视觉语言模型提供了一种鲁棒的开放集测试时适应解决方案，通过概率验证和原型级更新机制，有效解决了csID/csOOD分离难题，实现了安全高效的模型适应。
...
TL;DR: 提出SDLS框架，通过语义解耦的潜在空间控制，在推理时减少放射报告生成中的历史对比幻觉，无需额外训练。


<details>
  <summary>Details</summary>
Motivation: 基于视觉语言模型的自动放射报告生成存在历史对比幻觉问题，即模型会生成当前研究中未支持的历史发现。现有方法在幻觉抑制和临床准确性之间存在权衡。

Method: 提出语义解耦潜在控制框架：1) 使用LLM进行语义分解；2) 通过QR正交化构建语义无关的干预向量；3) 利用几何约束过滤临床语义，专门针对"历史对比"轴进行控制。
...
Conclusion: SDLS框架成功解决了历史对比幻觉问题，克服了幻觉抑制与临床准确性之间的权衡，同时保持了临床叙述的结构完整性。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型依赖数据集特定的视觉特征，难以在异构农业环境中泛化。需要一种能够利用语义对齐、领域不变表示的方法来实现精准的杂草分割

Method: 提出Vision-Language Weed Segmentation (VL-WS)框架，采用双编码器设计：冻结的CLIP嵌入与任务特定空间特征通过FiLM层融合，利用自然语言描述进行通道级特征调制。在包含近距离地面图像和高空无人机图像的统一语料库上训练

Result: 在四个基准数据集上平均Dice分数达91.64%，比CNN基线提升4.98%。最具挑战性的杂草类别提升15.42%（从65.03%到80.45%）。在有限目标域监督下保持稳定的分割性能
...
<details>
  <summary>Details</summary>
Motivation: 遥感基础模型快速增长，但模型发布格式、平台接口和数据规范存在显著异构性，导致实际采用和公平比较困难，增加了获取、使用和基准测试嵌入的成本

Method: 开发rs-embed Python库，提供统一的ROI中心化接口，用户只需一行代码即可从任何支持模型获取任何位置、任何时间范围的嵌入，并提供高效的批处理功能

Result: 实现了简化遥感基础模型嵌入获取和比较的Python库，支持大规模嵌入生成和评估

Conclusion: rs-embed通过统一接口解决了遥感基础模型异构性问题，降低了嵌入获取和比较的成本，促进了模型的实际采用和公平评估
...
TL;DR: SourceSwap是一个自监督的物体交换框架，通过频率分离扰动合成高质量伪配对数据，无需微调即可实现零样本推理，在物体保真度、场景保真度和物体-场景和谐度方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有物体交换方法要么需要逐物体微调和慢速推理，要么依赖额外的配对数据（通常描绘相同物体在不同背景中），导致模型依赖背景线索而非学习跨物体对齐。
...
Result: SourceSwap在保真度、场景保持和自然和谐度方面表现优异，无需逐物体微调即可实现零样本推理，并能很好地迁移到主题驱动优化和人脸交换等编辑任务。

Conclusion: SourceSwap通过自监督学习和频率分离扰动合成伪配对数据，有效解决了物体交换中的跨物体对齐问题，实现了高质量的零样本物体交换，并创建了更全面的SourceBench基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM中视觉token的二次计算成本阻碍了广泛应用，当前渐进式token剪枝方法误解了浅层功能且使用僵化调度，未能充分发挥效率潜力

Method: 提出HiDrop框架，包含两个关键创新：1) 延迟注入 - 绕过被动浅层，在主动融合开始处引入视觉token；2) 凹金字塔剪枝 - 结合早期退出机制，通过层间相似度度量和可微分top-k算子动态调整中间层和深层剪枝率

Result: 压缩约90%视觉token的同时匹配原始性能，加速训练1.72倍，在高效MLLM训练和推理方面达到新的SOTA
...
<details>
  <summary>Details</summary>
Motivation: 现有方法对超长第一人称视频的处理仍依赖局部片段和有限的时间建模，无法有效推理这种长时间序列。需要一种能编码长期跨实体依赖关系的新方法。

Method: 提出EgoGraph框架：1）使用统一的第一人称模式提取和抽象核心实体（人、物、地点、事件）；2）结构化推理实体属性和交互；3）采用时间关系建模策略捕捉跨实体时间依赖并积累长期记忆。

Result: 在EgoLifeQA和EgoR1-bench基准测试中，EgoGraph在长期视频问答任务上取得了最先进的性能表现。

Conclusion: EgoGraph为超长第一人称视频理解提供了新的范式，通过知识图谱构建和长期依赖建模，显著提升了语义表示的丰富性和连贯性。
...
TL;DR: U-MLLMs在文本推理上表现良好，但在需要将相同推理结果以图像模态呈现时，无法保持语义一致性，存在跨模态语义对齐的断裂。
...
Method: 引入VGUBench框架，通过三个诊断任务解耦推理逻辑与生成保真度：1) 文本生成理解（建立文本响应推理准确性的基线）；2) 视觉生成理解（评估生成正确视觉答案的能力）；3) 视觉渲染控制任务（评估无需复杂推理直接将显式视觉描述渲染为图像的能力）。

Result: 评估显示显著差异：尽管在文本理解和视觉渲染方面表现强劲，但U-MLLMs在需要生成视觉答案时出现明显的性能崩溃。视觉回答性能与基本渲染质量之间几乎没有相关性。

Conclusion: 失败原因不是生成保真度不足，而是跨模态语义对齐的断裂。研究为未来统一生成和理解模型提供了诊断性见解。
...
TL;DR: 提出一种基于二阶差分（重建误差的差异）的新方法，用于检测AI生成图像，相比传统的一阶差分方法具有更好的泛化性能


<details>
  <summary>Details</summary>
Motivation: 随着扩散模型生成的AI图像越来越逼真，传统基于重建误差的检测方法效果下降，需要更有效的检测技术来应对AI图像滥用的挑战

Method: 提出差分中的差分方法：不直接使用重建误差（一阶差分），而是计算重建误差的差异（二阶差分），通过方差减少来提高检测准确率

Result: 大量实验表明该方法实现了强大的泛化性能，能够在生成式AI时代可靠地检测AI生成图像

Conclusion: 二阶差分方法相比传统一阶差分方法能更有效地检测现代AI生成的逼真图像，为解决AI图像检测难题提供了新思路
...
TL;DR: UTPTrack提出统一的token剪枝框架，首次联合压缩搜索区域、动态模板和静态模板三个组件，在保持性能的同时大幅减少计算量


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的单流跟踪器计算开销大，难以实时部署。现有的token剪枝方法通常孤立地剪枝各个组件，忽略了组件间的依赖关系，导致次优剪枝和精度下降

Method: UTPTrack采用注意力引导、token类型感知的策略，统一建模三个组件的冗余性，支持多模态和语言引导任务的统一跟踪

Result: 在10个基准测试中，UTPTrack在RGB跟踪中剪枝65.4%视觉token保持99.7%基线性能，在统一跟踪中剪枝67.5%token保持100.5%基线性能，达到剪枝跟踪器精度-效率权衡的新SOTA

Conclusion: UTPTrack为高效视觉跟踪提供了强大的基础框架，在RGB和多模态场景下均表现出色，有望推动未来高效视觉跟踪研究
...
TL;DR: U-Mind是一个统一的多模态对话系统，能够在单一交互循环中实时生成语言、语音、动作和视频，通过统一对齐与推理框架解决跨模态同步和推理能力保持问题。


<details>
  <summary>Details</summary>
Motivation: 现有系统要么局限于单模态生成，要么存在推理能力下降和跨模态对齐差的问题，无法实现连贯且感知基础的自然动态交互。需要构建能够支持实时生成并联合建模多种模态的智能体。

Method: 采用统一对齐与推理框架：1) 通过分段对齐策略增强跨模态同步；2) 通过排练驱动学习保持推理能力。推理时采用文本优先解码管道，先进行内部思维链规划，再进行时间同步的多模态生成。实现基于姿势和语音的实时视频渲染框架。

Result: 在多种多模态交互任务（包括问答、指令跟随和动作生成）上实现了最先进的性能，为智能沉浸式对话代理铺平了道路。

Conclusion: U-Mind是首个支持实时生成的高智能多模态对话统一系统，通过创新的对齐和推理方法解决了现有系统的局限性，为实现智能、沉浸式对话代理提供了重要进展。
...
TL;DR: Selfment：完全自监督的无标注图像分割框架，通过图割和迭代优化实现前景-背景分离，在多个基准测试中达到SOTA


<details>
  <summary>Details</summary>
Motivation: 解决无需人工标注、预训练分割模型或后处理的情况下准确分割物体的核心挑战，实现完全自监督的对象分割

Method: 1. 从自监督特征构建补丁级亲和力图，应用NCut进行粗粒度前景-背景分离；2. 引入迭代补丁优化(IPO)，通过迭代补丁聚类在特征空间进行细化；3. 使用精化掩码作为监督信号训练轻量分割头，结合对比学习和区域一致性目标

Result: 在ECSSD、HKUIS、PASCAL-S等基准上显著提升Fmax指标（分别+4.0%、+4.6%、+5.7%）；在伪装物体检测任务上实现零样本泛化，性能优于所有无监督方法，甚至媲美全监督SOTA方法

Conclusion: Selfment证明了完全自监督分割的可行性，无需任何人工标注即可达到SOTA性能，在零样本泛化方面表现突出，为无监督计算机视觉开辟了新方向
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型缺乏有效的早期质量评估机制，导致在多生成场景（如提示迭代、基于代理的生成）中产生昂贵的试错成本

Method: 发现早期扩散交叉注意力分布与最终图像质量强相关，设计轻量级预测器，将早期去噪步骤中提取的交叉注意力统计特性映射到最终图像质量
...
TL;DR: 提出傅里叶角度对齐方法解决遥感旋转目标检测中的方向不一致和任务冲突问题，通过两个即插即用模块提升性能，在多个数据集上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 当前遥感旋转目标检测方法存在两个瓶颈：检测器颈部的方向不一致性和检测头部的任务冲突问题，需要更有效的角度信息处理方法。

Method: 利用傅里叶旋转等变性，提出傅里叶角度对齐方法分析频谱中的角度信息，将主方向对齐到特定方向。包含两个模块：FAAFusion在颈部对齐并融合特征，FAA Head在头部预对齐RoI特征。

Result: 在DOTA-v1.0、DOTA-v1.5和HRSC2016数据集上显著提升先前工作性能，在单尺度训练测试下，DOTA-v1.0达到78.72% mAP，DOTA-v1.5达到72.28% mAP的新SOTA结果。

Conclusion: 傅里叶角度对齐方法有效解决了遥感旋转目标检测中的方向对齐问题，通过频域分析和特征对齐显著提升了检测性能，验证了方法的有效性。
...
TL;DR: Sea²提出了一种新范式：通过智能姿态控制代理来适应感知模型的部署方式，而非直接微调感知模型本身。该方法冻结所有感知模块，无需下游标签，仅使用标量感知反馈来导航代理寻找信息丰富的视角。


<details>
  <summary>Details</summary>
Motivation: 预训练感知模型在通用图像领域表现良好，但在室内场景等新环境中性能显著下降。传统的微调方法会导致灾难性遗忘，且需要昂贵的场景特定标注。

Method: 将视觉语言模型(VLM)转化为低级姿态控制器：首先在基于规则的探索轨迹上微调，然后通过无监督强化学习进行策略优化，利用感知模块的输出和置信度构建奖励。

Result: 在三个视觉感知任务（视觉定位、分割和3D框估计）上进行了实验，在ReplicaCAD数据集上分别获得了13.54%、15.92%和27.68%的性能提升。

Conclusion: Sea²提供了一种无需重新训练即可直接利用现成感知模型的新方法，通过智能视角控制显著提升了感知性能，解决了传统微调方法的局限性。
...
TL;DR: 提出基于预训练3D几何基础模型的双手操作框架，通过扩散模型联合预测动作序列和未来3D场景，仅需RGB观测即可实现精确的空间理解和操作


<details>
  <summary>Details</summary>
Motivation: 现有双手操作方法依赖2D特征空间感知有限，或需要难以可靠获取的点云数据。3D几何基础模型的发展使得直接从RGB图像重建准确3D结构成为可能

Method: 融合几何感知潜在表示、2D语义特征和本体感知的统一状态表示，使用扩散模型联合预测未来动作块和未来3D潜在表示（可解码为密集点图）

Result: 在RoboTwin基准测试仿真和真实机器人执行中均优于2D基线和点云基线，在操作成功率、双臂协调性和3D空间预测准确性方面达到最先进性能

Conclusion: 通过将双手操作建立在预训练3D几何基础模型上，仅使用RGB观测即可实现强大的空间理解和预测能力，为机器人操作提供了新范式
...
TL;DR: 提出一种无需存储原始数据的持续学习框架，用于病理全切片图像到报告生成，通过构建紧凑的域特征表示来避免灾难性遗忘。
...
Method: 1) 在冻结的patch嵌入空间中构建紧凑的域特征：包含代表性形态标记的小型codebook、切片级共现摘要和轻量级patch计数先验；2) 通过合成反映域特定形态混合的伪WSI表示支持生成式回放；3) 教师快照提供伪报告监督更新模型；4) 将域特定语言特征提取为紧凑风格描述符，用于引导生成。

Result: 在多个公开持续学习基准测试中，该方法优于无示例和有限缓冲回放基线，展示了基于特征的生成式回放在演化临床环境中的实用性。

Conclusion: 提出的特征式生成回放框架为病理报告生成的持续学习提供了实用解决方案，能够适应临床环境的变化而无需存储原始数据，支持领域无关的推理设置。
...
TL;DR: CPN-YOLO：基于YOLOv8改进的SAR图像船舶检测框架，通过可学习大核去噪模块、PPA注意力特征增强和NWD高斯相似度损失，在复杂场景下实现高精度检测。


<details>
  <summary>Details</summary>
Motivation: SAR图像船舶检测在复杂场景中面临挑战：杂波和斑点噪声导致误报，小目标易漏检。现有方法在复杂场景下的鲁棒性不足，需要改进特征表示和对小目标的敏感性。

Method: 1. 可学习大核去噪模块：用于输入预处理，生成更干净的表示和更具区分性的特征
2. PPA注意力机制的特征提取增强策略：加强多尺度建模，提高对小船的敏感性
...
TL;DR: 该论文提出APPO算法，通过注意力引导的感知策略优化，利用token级密集奖励提升模型的细粒度感知能力，从而以低成本方式增强视频理解性能。


<details>
  <summary>Details</summary>
Motivation: 研究发现复杂视频推理过度依赖细粒度感知而非专家级推理能力。当感知能力固定时，提升推理能力带来的性能改善有限（仅0.7%），而感知模型规模的小幅提升（从7B到32B）却能带来1.4%的性能提升。因此，探索如何通过推理增强感知能力，而无需昂贵的细粒度标注信息具有重要意义。
...
Conclusion: 该工作提供了一种有前景的方法，能够通过推理以低成本方式有效增强模型的感知能力，适用于多种场景和需求。
...
TL;DR: 提出多模态多任务模型检测AI生成图像并识别生成模型，使用BERT和CLIP编码器进行跨模态特征融合，通过伪标签数据增强扩展训练集，在CT2竞赛中获得第五名。


<details>
  <summary>Details</summary>
Motivation: 检测AI生成的图像并识别具体生成模型的需求日益增长，现有方法在跨模态特征融合和多任务学习方面有待改进，需要更有效的架构来应对现实场景中的AI生成内容检测挑战。

Method: 使用预训练的BERT和CLIP视觉编码器分别提取文本和图像特征，通过跨模态特征融合结合多任务损失函数，采用伪标签数据增强策略扩展训练数据集。

Result: 在CT2竞赛的Task A（AI生成图像检测）和Task B（生成模型识别）中均获得第五名，F1分数分别为83.16%和48.88%，证明了所提架构的有效性。

Conclusion: 提出的多模态多任务架构在AI生成图像检测和模型识别方面表现出色，具有在实际场景中推进AI生成内容检测的潜力，源代码已开源。
...
Method: 1. 利用SAM生成掩码，在多尺度上约束自注意力交互的层次化方案；2. 组合多个遥感专用CLIP变体参数，采用基于不同文本提示评估表示质量的新加权方案进行参数平均

Result: 在三个遥感基准测试中实现了最先进的结果，无需额外训练

Conclusion: ReSeg-CLIP通过约束自注意力交互和模型组合策略，有效提升了遥感开放词汇语义分割性能，证明了无需训练方法的有效性
...
TL;DR: 该论文提出了一种基于V2X通信的混合计算卸载方案，将自动驾驶的3D物体检测任务动态分配到车辆和云端，通过特征压缩和自适应优化显著降低了延迟并提高了检测精度。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶在复杂城市环境中面临实时感知的延迟挑战，车载计算资源有限难以满足高处理需求，需要解决实时障碍物检测的延迟问题。

Method: 利用V2X通信将部分处理卸载到云端；使用基于transformer的模型融合多摄像头数据生成BEV表示；动态分配计算层数和特征量化级别；应用特征向量裁剪和压缩减少网络负载；引入动态优化算法根据网络条件自适应选择分割点和量化级别。

Result: 混合策略相比传统车载方案减少了72%的端到端延迟；在真实带宽变化下的追踪评估显示，自适应方法在相同延迟性能下比静态参数化提高了20%的检测精度。

Conclusion: 通过V2X通信的混合计算卸载方案能有效解决自动驾驶的实时感知延迟问题，动态自适应优化策略在变化的网络条件下能显著提升系统性能。
...
TL;DR: 提出一种基于地面特征密度分析和图像分类的视觉地点识别方法，能够适应空中平台的高度变化，无需额外硬件即可实现三维视觉地点识别。


<details>
  <summary>Details</summary>
Motivation: 解决空中视觉地点识别在显著高度变化下的挑战，传统方法依赖气压高度计或ToF传感器，需要额外硬件且不适用于小型平台。

Method: 通过分析图像中地面特征密度估计相对高度，然后基于相对高度进行裁剪生成标准查询图像，最后使用基于分类的VPR策略进行定位。
...
TL;DR: 提出一种用于退化图像超分辨率的DACESR方法，通过Real Embedding Extractor提取退化图像的有效语义特征，结合Mamba网络实现保真度与感知质量的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大模型在真实场景图像超分辨率中表现良好，但在处理退化图像时能力有限。需要解决退化图像语义识别困难的问题，以提升超分辨率效果。

Method: 1) 分析RAM模型在退化图像上的能力；2) 提出退化选择策略和Real Embedding Extractor，通过对比学习提升退化图像识别性能；3) 使用Conditional Feature Modulator将REE的高层信息融入Mamba网络，有效利用像素信息恢复纹理。

Result: 实验表明REE能有效帮助图像超分辨率网络平衡保真度和感知质量，Mamba在真实世界应用中展现出巨大潜力。

Conclusion: 该方法通过提升退化图像语义识别能力，结合Mamba网络实现了更好的图像超分辨率效果，为真实世界应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有具身基础模型需要大规模高质量真实世界交互数据，但现有数据收集方法存在基础设施成本高、硬件依赖复杂、交互范围有限等问题，难以规模化扩展。人类本身就是理想的具身智能体，从全球分布的"人类智能体"获取自我中心视角数据具有低成本、可持续的优势。

Method: 1) 使用符合人体工学的颈挂式智能手机支架，通过云边协同架构实现低门槛、大规模的自我中心数据收集；2) 开发跨平台移动APP，利用设备端计算进行实时处理，云端部署自动化标注和过滤流水线，将原始视频转化为高质量训练数据；3) 支持任何人、任何时间、任何地点的分布式自我中心视频数据收集。
...
TL;DR: 提出了一种自监督的3D占据流估计方法，无需人工标注或外部光流监督，通过分离静态和动态场景并利用时序聚合学习运动


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要准确估计周围环境的3D占据和运动，现有方法依赖昂贵的3D标注、边界框速度标签或预训练光流模型，需要开发无需人工标注的自监督方法

Method: 将场景解耦为静态和动态的有符号距离场，通过时序聚合隐式学习运动，并引入基于特征余弦相似度的自监督流线索

Result: 在SemanticKITTI、KITTI-MOT和nuScenes数据集上验证了方法的有效性

Conclusion: 提出的自监督3D占据流估计方法能够有效学习场景几何和运动，无需昂贵的人工标注或外部监督
...
TL;DR: 提出Ref-Adv新基准测试，通过设计具有挑战性的指代表达式和视觉场景来抑制现有模型的捷径学习，揭示多模态大语言模型在视觉推理和基础能力上的不足


<details>
  <summary>Details</summary>
Motivation: 现有REC基准测试（如RefCOCO系列）存在三个主要问题：1）表达式太短，推理需求低；2）图像中干扰物少，目标容易定位；3）冗余描述符使模型可以通过捷径绕过真正的文本理解和视觉推理。需要更严格的测试来评估模型的真实视觉推理和基础能力

Method: 构建Ref-Adv数据集，包含真实图像上的指代表达式，通过精心设计：1）使用语言上非平凡的表达式；2）只包含唯一识别目标所需的信息；3）设置困难的干扰物；4）标注推理方面（包括否定）。进行全面的消融实验（词序扰动和描述符删除充分性）来验证数据集需要真正的推理能力

Result: 尽管当代多模态大语言模型在RefCOCO、RefCOCO+和RefCOCOg上表现强劲，但在Ref-Adv上性能显著下降，表明模型依赖捷径学习，在视觉推理和基础能力上存在明显差距。提供了深入的失败分析

Conclusion: Ref-Adv基准测试能够更准确地评估多模态大语言模型的视觉推理和基础能力，揭示了现有模型依赖捷径学习的问题，为未来视觉推理和基础研究提供了指导方向
...
<details>
  <summary>Details</summary>
Motivation: 减少乳腺超声筛查中的诊断升级和不必要的活检转诊，提高筛查和诊断的特异性，同时保持高敏感性。

Method: 采用两阶段级联多智能体框架：1) 筛查诊所智能体使用分类模型筛选低风险病例；2) 高风险病例升级到诊断诊所智能体，整合更丰富的感知和放射学描述工具。利用记忆库存储历史决策轨迹，通过检索相似病例进行上下文适应，动态调整模型信任度和升级阈值。

Result: 在10个乳腺超声数据集上的评估显示：诊断升级率从84.95%降至58.72%，总体活检转诊率从59.50%降至37.08%，筛查特异性平均提高68.48%，诊断特异性提高6.33%。

Conclusion: BUSD-Agent通过经验引导的决策框架有效减少了不必要的诊断升级和活检转诊，提高了乳腺超声筛查和诊断的效率与特异性，展示了基于记忆检索的上下文适应在医疗决策中的价值。
...
<details>
  <summary>Details</summary>
Motivation: 当前医学图像分割模型虽然精度高但计算资源需求大，限制了在资源受限的临床环境中的部署。需要开发既高效又准确的分割方法。

Method: 提出SegMate框架，集成非对称架构、注意力机制、多尺度特征融合、切片位置条件编码和多任务优化等技术，在三个骨干网络(EfficientNetV2-M, MambaOut-Tiny, FastViT-T12)上实现效率-精度平衡。

Result: 相比原始模型，SegMate将计算量(GFLOPs)降低2.5倍，内存占用(VRAM)降低2.1倍，同时性能提升约1%。在TotalSegmentator上达到93.51% Dice分数，仅需295MB GPU内存。跨数据集评估显示良好泛化能力。

Conclusion: SegMate在保持高精度的同时显著降低了计算和内存需求，为资源受限的临床环境提供了实用的医学图像分割解决方案，具有很好的泛化能力。
...
TL;DR: CLIP模型在处理"半真半假"描述时存在缺陷：当在正确描述后添加错误细节时，相似度反而增加。作者提出CS-CLIP方法，通过组件监督训练提升组合理解能力。


<details>
  <summary>Details</summary>
Motivation: CLIP风格的双编码器在处理文本描述时存在反直觉行为：当在正确描述后添加看似合理但错误的细节时，图像-文本相似度反而会增加。这种"半真半假"问题暴露了模型缺乏对描述中各个组成部分的有效监督。

Method: 提出CS-CLIP方法：1）将描述分解为实体和关系单元；2）为每个单元构建最小编辑的虚假版本；3）微调模型，使其正确单元得分高于虚假版本，同时保持标准的双编码器推理架构。

Result: 在COCO数据集上，CS-CLIP将半真半假准确率从40.6%提升到69.3%，在已建立的组合基准测试中平均性能提升5.7个百分点，表明减少半真半假错误与提升组合理解能力相关。

Conclusion: CLIP模型的半真半假问题源于对描述组成部分的弱监督。通过组件监督训练，CS-CLIP显著提升了模型对组合细节的敏感性，同时改善了整体的组合理解能力，为提升视觉-语言模型的细粒度理解提供了有效方法。
...
TL;DR: 提出了一种基于拓扑的迁移性估计框架，用于医学分割任务中基础模型的选择，相比现有方法有31%的相对改进。


<details>
  <summary>Details</summary>
Motivation: 现有的迁移性估计指标主要针对分类任务设计，依赖于全局统计假设，无法捕捉密集预测所需的拓扑复杂性，导致在医学分割任务中选择最优基础模型存在计算瓶颈。
...
Result: 在大规模OpenMind基准测试中，该方法显著优于现有基线方法，在加权Kendall指标上实现了约31%的相对改进，提供了无需训练的高效模型选择代理。

Conclusion: 该拓扑驱动框架能够有效评估医学分割任务中基础模型的迁移性，解决了现有分类导向方法的局限性，为医学图像分析中的模型选择提供了高效可靠的解决方案。
...
TL;DR: GPU-SDF：一种利用几何先验不确定性和互补约束的神经隐式室内表面重建框架，通过显式估计先验不确定性、不确定性引导损失、边缘距离场和多视图一致性正则化来改善精细细节重建。


<details>
  <summary>Details</summary>
Motivation: 基于符号距离函数的神经隐式表面重建在处理薄结构和复杂几何等精细细节时仍面临挑战，主要原因是不可靠或噪声的几何先验。现有方法依赖优化过程中产生的隐式不确定性来过滤这些先验，这种方法间接且低效，并且在不确定性高的区域屏蔽监督会导致优化约束不足。

Method: 提出GPU-SDF框架：1）引入自监督模块显式估计先验不确定性；2）设计不确定性引导损失，调制而非丢弃先验影响；3）针对高不确定性区域，引入边缘距离场增强边界监督；4）加入多视图一致性正则化强制几何一致性。

Result: 大量实验证实GPU-SDF能够改善精细细节的重建效果，并且可以作为即插即用的增强模块应用于现有框架。

Conclusion: GPU-SDF通过显式处理几何先验不确定性和引入互补约束，有效解决了神经隐式表面重建中精细细节恢复的挑战，为现有框架提供了实用的增强方案。
...
### [51] [Micro-expression Recognition Based on Dual-branch Feature Extraction and Fusion](https://arxiv.org/abs/2602.23950)
...
### [52] [AHAP: Reconstructing Arbitrary Humans from Arbitrary Perspectives with Geometric Priors](https://arxiv.org/abs/2602.23951)
...
<details>
  <summary>Details</summary>
Motivation: 传统多视角3D人体重建需要预先标定相机（如使用棋盘格或MVS算法），限制了在多样化现实场景中的可扩展性和适用性。需要一种无需相机标定的解决方案。

Method: 1. 跨视角身份关联模块：使用可学习的人物查询和软分配，通过对比学习监督解决跨视角人体身份关联
2. 人体头部模块：融合跨视角特征和场景上下文进行SMPL预测，通过跨视角重投影损失强制身体姿态一致性
3. 多视角几何三角测量：利用多视角几何消除单目方法的深度模糊，提供更精确的3D人体定位

Result: 在EgoHumans和EgoExo4D数据集上的实验表明，AHAP在世界空间人体重建和相机姿态估计方面均达到竞争性性能，同时比基于优化的方法快180倍

Conclusion: AHAP提供了一个无需相机标定的前馈框架，能够从任意相机视角重建任意人体，通过有效融合多视角几何解决了身份关联、重建和定位问题，具有更好的可扩展性和实用性
...
### [53] [CC-VQA: Conflict- and Correlation-Aware Method for Mitigating Knowledge Conflict in Knowledge-Based Visual Question Answering](https://arxiv.org/abs/2602.23952)
...
<details>
  <summary>Details</summary>
Motivation: KB-VQA中静态参数知识与动态检索信息之间存在冲突，现有方法主要从语言角度解决，忽视了视觉信息在冲突中的关键作用，且检索上下文冗余影响冲突识别和缓解

Method: 提出CC-VQA方法，包含两个核心组件：1) 视觉中心上下文冲突推理，进行内部和外部知识背景下的视觉语义冲突分析；2) 相关引导编码和解码，包括低相关语句的位置编码压缩和使用相关加权冲突评分的自适应解码

Result: 在E-VQA、InfoSeek和OK-VQA基准上的广泛评估表明，CC-VQA实现了最先进的性能，相比现有方法绝对准确率提升3.3%到6.4%

Conclusion: CC-VQA是一种无需训练、冲突和相关感知的KB-VQA方法，有效解决了知识冲突问题，通过视觉中心冲突推理和相关引导机制显著提升了性能
...
Method: 提出GDA-YOLO11模型，包含架构改进和更新的非对称掩码损失；使用改进的公共柑橘数据集训练；通过欧几里得距离变换估计采摘点，并将2D点投影到3D坐标用于机器人执行。

Result: GDA-YOLO11达到精度0.844、召回率0.846、mAP@50为0.914、mAP@50:95为0.636，比YOLO11n分别提升5.1%、1.3%和1.0%；在不同遮挡级别下采摘成功率为92.59%到22.22%，在中高遮挡下提升3.5%。

Conclusion: GDA-YOLO11增强了遮挡鲁棒性分割，简化了感知到行动的集成，为农业中更可靠的自主系统铺平了道路，是机器人水果采摘中amodal实例分割的首次实际演示。
...
TL;DR: SwitchCraft是一个无需训练的多事件视频生成框架，通过事件对齐查询引导和自适应平衡强度求解器，解决现有文本到视频模型在多事件提示下产生的场景混合或崩溃问题。
...
### [56] [Thinking with Images as Continuous Actions: Numerical Visual Chain-of-Thought](https://arxiv.org/abs/2602.23959)
...
Method: 提出数值视觉思维链框架，将MLLM动作空间扩展到连续坐标空间，使用高斯或拉普拉斯策略代替分类标记策略，支持监督微调和强化学习，特别是GRPO风格策略优化。

Result: 在三个基准测试中对八个代表性视觉推理基线进行实验，NV-CoT显著提高了定位精度和最终答案准确性，同时加速了训练收敛。

Conclusion: NV-CoT验证了连续动作视觉推理在多模态大语言模型中的有效性，为精确区域定位和推理提供了新方法。
...
### [57] [SpikeTrack: A Spike-driven Framework for Efficient Visual Tracking](https://arxiv.org/abs/2602.23963)
*Qiuyang Zhang,Jiujun Cheng,Qichao Mao,Cong Liu,Yu Fang,Yuhong Li,Mengying Ge,Shangce Gao*

Main category: cs.CV

TL;DR: SpikeTrack：首个脉冲驱动的RGB目标跟踪框架，通过非对称设计和记忆检索模块，在保持高精度的同时实现超低能耗


<details>
  <summary>Details</summary>
Motivation: 现有SNN跟踪框架要么不完全符合脉冲驱动计算，要么未充分利用神经元的时空动态特性，导致效率与精度之间的权衡问题

Method: 采用非对称设计：非对称时间步扩展和单向信息流；设计受神经推理机制启发的记忆检索模块，通过查询模板初始化的紧凑记忆来检索目标线索并随时间锐化目标感知
...
### [58] [Venus: Benchmarking and Empowering Multimodal Large Language Models for Aesthetic Guidance and Cropping](https://arxiv.org/abs/2602.23980)
...
<details>
  <summary>Details</summary>
Motivation: 智能手机摄影普及，但普通用户与专业摄影师之间存在美学指导差距。现有MLLMs主要提供过于积极的反馈，无法识别美学问题或提供可操作的拍摄指导，也因此在美学裁剪任务上表现不佳。

Method: 1. 构建AesGuide数据集：包含10,748张照片，标注了美学评分、分析和指导建议。
2. 提出Venus两阶段框架：第一阶段通过渐进式复杂美学问题赋予MLLMs美学指导能力；第二阶段通过基于CoT的推理激活其美学裁剪能力。

Result: Venus显著提升了MLLMs的美学指导能力，并在美学裁剪任务上实现了最先进的性能，能够在照片创作的两个阶段提供可解释和交互式的美学优化。

Conclusion: 该研究首次系统性地定义了美学指导能力，并通过AesGuide数据集和Venus框架有效解决了现有MLLMs在美学指导方面的不足，为计算美学领域提供了新的研究方向和实践工具。
...
### [59] [Accelerating Masked Image Generation by Learning Latent Controlled Dynamics](https://arxiv.org/abs/2602.23996)
...
<details>
  <summary>Details</summary>
Motivation: 掩码图像生成模型(MIGMs)虽然成功，但双向注意力的多步计算效率低下。现有特征缓存方法在激进加速率下近似误差大，因为表达能力有限且未考虑采样信息

Method: 提出MIGM-Shortcut方法：学习一个轻量级模型，结合先前特征和已采样token，回归特征演化的平均速度场。模型复杂度适中，能捕捉微妙动态，同时比原始基础模型轻量

Result: 在两种代表性MIGM架构和任务上验证。在SOTA的Lumina-DiMOO上，文本到图像生成实现超过4倍加速，同时保持质量，显著推进掩码图像生成的帕累托前沿

Conclusion: MIGM-Shortcut通过轻量级速度场预测模型有效解决了MIGMs的计算冗余问题，在保持生成质量的同时大幅提升效率，为掩码图像生成提供了实用的加速方案
...
### [60] [Ordinal Diffusion Models for Color Fundus Images](https://arxiv.org/abs/2602.24013)
*Gustav Schmidt,Philipp Berens,Sarah Müller*

Main category: cs.CV

TL;DR: 提出了一种序数潜在扩散模型，用于生成糖尿病视网膜病变的彩色眼底图像，该模型将疾病严重程度的顺序结构显式纳入生成过程，相比传统分类条件扩散模型在视觉真实性和临床一致性方面表现更好。


<details>
  <summary>Details</summary>
Motivation: 现有生成图像模型（如扩散模型）虽然可以通过提供补充训练数据来改善临床相关任务的性能，但大多数条件扩散模型将疾病阶段视为独立类别，忽略了疾病进展的连续性。在医学影像中，连续的病理过程通常只能通过粗粒度、离散但有序的标签来观察（如糖尿病视网膜病变严重程度分级），这种不匹配是有问题的。

Method: 提出了一种序数潜在扩散模型，用于生成彩色眼底图像。该方法使用标量疾病表示代替分类条件，将糖尿病视网膜病变严重程度的顺序结构显式纳入生成过程，实现了相邻阶段之间的平滑过渡。

Result: 在EyePACS数据集上的评估显示，相比标准条件扩散模型，该模型在五个DR阶段中的四个阶段降低了Fréchet inception距离，并将二次加权κ从0.79提高到0.87。插值实验表明，模型能够从有序的粗粒度类别标签中学习到连续的疾病进展谱。

Conclusion: 序数潜在扩散模型能够有效捕捉疾病进展的连续性，通过将顺序结构显式纳入生成过程，在医学图像生成任务中实现了更好的视觉真实性和临床一致性，为利用生成模型辅助临床任务提供了改进方法。
...
### [61] [Interpretable Debiasing of Vision-Language Models for Social Fairness](https://arxiv.org/abs/2602.24014)
*Na Min An,Yoonna Jang,Yusuke Hirota,Ryo Hachiuma,Isabelle Augenstein,Hyunjung Shim*

Main category: cs.CV

TL;DR: DeBiasLens：通过稀疏自编码器定位视觉语言模型中的社会属性神经元，选择性去激活与偏见相关的神经元来减轻社会偏见，而不损害模型语义知识。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型的黑盒推理过程可能导致社会偏见，现有去偏见方法主要关注表层偏见信号，缺乏对模型内部动态的探索。

Method: 使用稀疏自编码器在多模态编码器上定位社会属性神经元，通过面部图像或字幕数据集训练SAE发现对特定人口统计学特征敏感的神经元，选择性去激活与偏见最相关的神经元。

Result: 有效减轻了视觉语言模型的社会偏见行为，同时不损害其语义知识，为未来审计工具奠定基础。

Conclusion: DeBiasLens框架为新兴AI系统的社会公平性提供了可解释的、模型无关的偏见缓解方法，有助于构建更公平的AI系统。
...
### [62] [SR3R: Rethinking Super-Resolution 3D Reconstruction With Feed-Forward Gaussian Splatting](https://arxiv.org/abs/2602.24020)
...
<details>
  <summary>Details</summary>
Motivation: 现有3D超分辨率方法依赖密集输入和逐场景优化，受限于预训练2D超分辨率模型的高频先验，导致重建保真度、跨场景泛化和实时性受限

Method: 提出SR3R前馈框架，通过映射网络直接从稀疏低分辨率视图预测高分辨率3D高斯泼溅表示；引入高斯偏移学习和特征细化来稳定重建并增强高频细节；采用即插即用设计，可与任何前馈3DGS重建主干配合

Result: 在三个3D基准测试中超越现有SOTA 3D超分辨率方法，实现强大的零样本泛化能力，甚至在未见场景上优于SOTA逐场景优化方法

Conclusion: SR3R通过重新定义3D超分辨率问题为前馈映射，使模型能够从大规模多场景数据中自主学习3D特定高频几何和外观，从根本上改变了3D超分辨率获取高频知识的方式
...
### [63] [Steering and Rectifying Latent Representation Manifolds in Frozen Multi-modal LLMs for Video Anomaly Detection](https://arxiv.org/abs/2602.24021)
...
Method: 提出SteerVAD框架：1) 使用无梯度的表示可分性分析(RSA)识别最具有判别性的注意力头作为潜在异常专家(LAEs)；2) 设计分层元控制器(HMC)，基于全局上下文和LAE输出生成动态修正信号；3) 在LAE表示流形上执行有针对性的各向异性缩放，增强异常相关维度并抑制固有偏见。

Result: 在主流基准测试中，该方法在仅需1%训练数据的无调优方法中达到了最先进的性能，为视频异常检测提供了新的强大方向。

Conclusion: SteerVAD通过从被动读取转向主动引导和修正内部表示，有效解决了MLLM在视频异常检测中的偏见和适应性限制问题，为低资源场景下的异常检测提供了高效解决方案。
...
TL;DR: AIR是一个无需训练的多模态大语言模型框架，通过原型token压缩和最优传输引导的补丁增强，选择性强化关键视觉信息，有效减少幻觉现象。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型存在幻觉问题，生成内容偏离视觉证据。现有缓解策略要么需要昂贵的训练监督，要么在推理时引入额外延迟。最近的视觉增强方法试图通过强化视觉token来解决，但通常不加区分地注入所有token，导致背景区域干扰和关键线索分散。

Method: 提出自适应视觉强化（AIR）框架，包含两个组件：1）基于原型的token压缩，将大量视觉token压缩为紧凑子集以减少冗余；2）OT引导的补丁增强，量化隐藏状态与补丁嵌入之间的对齐度，选择性地将最一致的补丁整合到前馈层中。

Result: 在代表性MLLM上的广泛实验表明，AIR显著减少了幻觉现象，同时保留了模型的一般能力，成为构建可靠MLLM的有效解决方案。

Conclusion: AIR是一个无需训练的高效框架，通过选择性强化关键视觉信息，增强了模型对显著视觉信息的依赖，有效缓解了多模态大语言模型的幻觉问题。
...
TL;DR: 提出统一框架，结合隐式缝纫模式和扩散模型，从单张图像或视频序列重建高保真3D服装，特别擅长处理宽松服装。


<details>
  <summary>Details</summary>
Motivation: 从单目图像和视频重建3D穿衣人体是虚拟试穿、虚拟形象创建和混合现实的基础问题。尽管人体恢复取得显著进展，但准确重建服装几何，特别是宽松服装，仍然是一个开放挑战。

Method: 结合隐式缝纫模式(ISP)与生成扩散模型，在2D UV空间学习表达性服装形状先验。引入映射模型建立图像像素、UV图案坐标和3D几何之间的对应关系。对于动态重建，引入时空扩散方案和测试时指导以保持长期时间一致性，并开发基于分析的投影约束。

Result: 尽管仅在合成模拟布料数据上训练，但方法能很好泛化到真实世界图像，在紧身和宽松服装上都优于现有方法。重建的服装保留精细几何细节，同时展现真实动态运动。

Conclusion: 提出的统一框架能够从单张图像和视频序列重建高保真3D服装，支持纹理编辑、服装重定向和动画等下游应用，在服装几何重建方面取得显著进展。
...
TL;DR: QE提出了一种token感知的自适应误差补偿方法，通过混合专家机制来改进视觉语言模型的量化效果，解决了重要通道在不同输入间分布差异的问题。


<details>
  <summary>Details</summary>
Motivation: 现有PTQ方法主要依赖静态识别和全局补偿敏感或异常通道，但忽略了这些重要通道在不同输入间的分布差异，导致量化效果不理想。研究发现，重要通道的分布和出现频率在不同模态和token间存在显著差异。

Method: 提出Quant Experts (QE)，将重要通道分为token无关和token相关两组。对于前者，设计共享专家使用低秩适配器补偿全局量化误差；对于后者，设计路由专家（多个路由低秩适配器）补偿与特定token相关的局部量化误差。

Result: 在各种量化设置和模型规模（2B到70B参数）下，QE都能持续提升任务准确率，同时保持与全精度模型相当的性能。

Conclusion: QE通过token感知的自适应误差补偿机制，有效解决了视觉语言模型量化中重要通道分布差异的问题，在保持性能的同时实现了高效的模型压缩。
...
Method: 提出EvalMVX数据集，包含25个物体，每个物体在20个不同视角和17种光照条件（包括OLAT和自然光照）下使用偏振相机拍摄，共8500张图像。每个物体都包含对齐的地面真实3D网格。

Result: 基于EvalMVX评估了近年发表的13种MVX方法，记录了最佳性能方法，并在不同几何细节和反射类型下识别了开放性问题。

Conclusion: EvalMVX数据集和基准测试结果有望激发多视角3D重建领域的未来研究，为确定不同MVX技术的工作范围提供定量评估基础。
...
TL;DR: FoV-Net：首个旋转不变的B-rep学习框架，通过局部参考帧和视场网格编码局部几何和全局结构，解决了现有方法对旋转敏感的问题。


<details>
  <summary>Details</summary>
Motivation: 现有B-rep学习方法依赖绝对坐标和法线来编码全局上下文，对旋转高度敏感。实验表明，在基准测试中达到95%准确率的模型在任意SO(3)旋转下可能降至10%。
...
Conclusion: FoV-Net是首个能够以旋转不变方式同时捕获局部几何和全局结构上下文的B-rep学习框架，解决了现有方法的旋转敏感性问题，在性能和鲁棒性方面均有显著提升。
...
TL;DR: 提出DiffusionHarmonizer框架，通过单步时序增强器提升神经重建场景的渲染质量，解决新视角渲染伪影和动态物体融合不自然的问题，实现实时高保真仿真。


<details>
  <summary>Details</summary>
Motivation: 现有神经重建方法（如NeRF、3D高斯泼溅）在自动驾驶等机器人仿真中存在局限性：新视角渲染易产生伪影，从不同场景捕获的动态物体难以真实融合，影响仿真质量。

Method: 开发在线生成增强框架DiffusionHarmonizer，核心是基于预训练多步图像扩散模型转换的单步时序增强器，配合定制数据管道构建合成-真实配对数据，强调外观协调、伪影校正和光照真实感。

Result: 系统显著提升仿真保真度，能在单GPU上实时运行于在线仿真器，适用于研究和生产环境，实现时间一致性输出并提高真实感。

Conclusion: DiffusionHarmonizer通过生成增强有效解决了神经重建仿真的视觉质量问题，为自动驾驶等机器人仿真提供了可扩展的高保真解决方案。
...
TL;DR: 提出神经符号验证框架，用SMT求解器自动检查视觉语言模型生成的放射学报告的逻辑一致性，解决传统指标无法检测的推理失败问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在生成放射学报告时经常出现逻辑不一致问题，如诊断印象缺乏感知发现支持或遗漏逻辑蕴含结论。传统词汇指标无法在无参考情况下捕捉这些演绎失败。

Method: 引入神经符号验证框架：1) 将自由文本放射学发现自动形式化为结构化命题证据；2) 使用SMT求解器(Z3)和临床知识库验证每个诊断主张是否数学蕴含、幻觉或遗漏。

Result: 在5个胸部X光基准测试中评估7个VLM，验证器揭示了传统指标无法检测的推理失败模式（如保守观察和随机幻觉）。在标记数据集上，强制执行求解器支持的蕴含作为严格后验保证，显著提高诊断的合理性和精确度。

Conclusion: 神经符号验证框架为临床推理提供确定性保证，能够系统消除无支持的幻觉，提高生成式临床助手的诊断可靠性。
...
TL;DR: FocusTrack是一个单阶段3D点云目标跟踪框架，通过运动-语义协同建模解决了现有两阶段方法中的错误累积和计算瓶颈问题，在多个基准测试中达到SOTA性能且运行速度达105 FPS。


<details>
  <summary>Details</summary>
Motivation: 现有两阶段运动中心方法存在两个根本限制：1）由于在运动估计前进行显式前景分割导致的解耦优化引起的错误累积；2）顺序处理带来的计算瓶颈。

Method: 提出FocusTrack单阶段跟踪框架，包含两个核心创新：帧间运动建模（IMM）模块使用时间差孪生编码器捕捉相邻帧间的全局运动模式；聚焦-抑制注意力机制通过运动显著特征门控增强前景语义，并基于IMM的时间感知运动上下文抑制背景噪声，无需显式分割。
...
TL;DR: AgenticOCR将OCR从静态全文处理转变为查询驱动的按需提取系统，通过动态解析文档布局并选择性识别感兴趣区域，解决了多模态RAG中页面级分块带来的信息过载和幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 多模态检索增强生成(RAG)在处理复杂视觉文档（如财务报告）时面临挑战：页面级分块和检索会导致向生成器传递过多无关上下文，不仅过载注意力机制，还稀释关键证据，同时将信息丰富的页面压缩到有限视觉token预算中会增加幻觉风险。
...
Conclusion: AgenticOCR有潜力成为视觉文档RAG堆栈的"第三大构建模块"，与标准嵌入和重排序模块协同工作，通过动态解析和按需提取解决了多模态RAG的关键瓶颈问题。
...
TL;DR: 提出一种针对3D高斯泼溅的高效剪枝策略和新型高斯基元，显著减少模型大小同时保持或提升渲染质量


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅需要大量基元实现高保真度，导致冗余表示和高资源消耗，限制了复杂或大规模场景的可扩展性

Method: 提出重建感知的自适应剪枝策略，根据重建质量确定剪枝时机和细化间隔；引入3D差分高斯基元，在单个基元中联合建模正负密度

Result: 显著提高模型紧凑性，实现高达90%的高斯基元数量减少，同时提供与最先进方法相似或更好的视觉质量

Conclusion: 该方法通过高效剪枝和更富表现力的基元设计，解决了3D高斯泼溅的冗余问题，为实际部署提供了更紧凑高效的解决方案
...
TL;DR: TASOT是一种无监督手术阶段识别方法，通过结合视觉和文本信息的多模态最优传输，无需大规模手术特定预训练即可实现零样本手术阶段识别。


<details>
  <summary>Details</summary>
Motivation: 当前手术视频识别方法依赖大规模手术特定预训练，计算和数据收集成本高昂。本文质疑这种重型预训练是否真正必要，探索利用视频中已有信息实现手术理解的可能性。

Method: 提出TASOT方法，将时序动作分割建模为多模态最优传输问题。匹配成本由视觉和文本成本加权组合：视觉项捕捉帧级外观相似性，文本项提供补充语义线索，两者通过时间一致的不平衡Gromov-Wasserstein公式联合正则化。
...
### [75] [A Mixed Diet Makes DINO An Omnivorous Vision Encoder](https://arxiv.org/abs/2602.24181)
*Rishabh Kabra,Maks Ovsjanikov,Drew A. Hudson,Ye Xia,Skanda Koppula,Andre Araujo,Joao Carreira,Niloy J. Mitra*

Main category: cs.CV

TL;DR: 提出Omnivorous Vision Encoder框架，解决预训练视觉编码器在不同模态间特征对齐差的问题，通过双目标训练实现模态无关的特征空间


<details>
  <summary>Details</summary>
Motivation: 现有预训练视觉编码器（如DINOv2）在单模态任务上表现优异，但不同模态间的特征表示对齐很差。例如，同一场景的RGB图像和深度图的特征嵌入余弦相似度与两个随机不相关图像几乎相同

Method: 提出Omnivorous Vision Encoder框架，采用双目标训练：1）最大化同一场景不同模态间的特征对齐；2）蒸馏目标，将学习到的表示锚定到完全冻结的教师模型（如DINOv2）的输出

Result: 学生编码器变得"omnivorous"，能够为给定场景产生一致的强大嵌入，无论输入模态如何（RGB、深度、分割等），在保持原始基础模型判别语义的同时实现稳健的跨模态理解

Conclusion: 该方法解决了预训练视觉编码器的跨模态对齐问题，实现了模态无关的特征表示，同时保留了基础模型的强大语义能力
...
TL;DR: RETA提出检索与拓扑对齐框架解决解耦数据集蒸馏中的拟合-复杂度差距和锚点牵引效应问题，在多个数据集上显著提升性能。
...
Method: 提出RETA框架：1) 动态检索连接(DRC)通过最小化教师特征空间的拟合-复杂度分数从预建池中选择真实图像块；2) 持久拓扑对齐(PTA)使用持久同调正则化合成过程，构建互k-NN特征图，计算组件和环的持久性图像，并惩罚真实与合成集之间的拓扑差异。
...
Conclusion: RETA通过动态检索连接和持久拓扑对齐有效解决了现有解耦数据集蒸馏方法的局限性，显著提升了合成图像的质量和模型的泛化性能。
...
TL;DR: HumanOrbit：从单张输入图像生成360度环绕人物视频的扩散模型，能保持身份一致性和几何连续性，并进一步重建纹理化3D网格


<details>
  <summary>Details</summary>
Motivation: 现有方法基于图像扩散模型进行多视角合成，但存在视角间不一致和身份保持问题。视频扩散模型在生成真实感结果方面表现出色，因此探索将其用于多视角人物图像生成

Method: 提出HumanOrbit视频扩散模型，能够合成围绕人物的连续相机旋转，生成几何一致的新视角，同时保持人物外观和身份。利用生成的多视角帧，进一步提出重建流水线恢复纹理化网格

Result: 实验验证了HumanOrbit在多视角图像生成方面的有效性，重建的3D模型在完整性和保真度方面优于现有最先进基线方法

Conclusion: HumanOrbit能够从单张图像生成360度环绕视频，保持身份一致性，并能重建高质量的纹理化3D模型，为多视角人物生成提供了有效解决方案
...
TL;DR: RAViT：基于多分支网络和早期退出机制的高效视觉Transformer，通过处理不同分辨率的图像副本减少计算成本，同时保持精度


<details>
  <summary>Details</summary>
Motivation: 视觉Transformer在计算机视觉中表现出色，但计算成本远高于卷积神经网络等替代方法。需要一种既能保持精度又能显著降低计算成本的解决方案。

Method: 提出RAViT框架：1）多分支网络处理同一图像的不同分辨率副本；2）早期退出机制实现运行时精度与计算成本的权衡；3）两分支架构中，先对降分辨率图像进行预测，再将预测结果与原尺寸图像结合进行最终预测

Result: 在CIFAR-10、Tiny ImageNet和ImageNet上评估，仅用约70%的FLOPs就达到了传统视觉Transformer的等效精度

Conclusion: RAViT通过多分辨率处理和早期退出机制，在保持视觉Transformer精度的同时显著降低计算成本，实现了精度与效率的良好平衡
...
TL;DR: 提出一种结合图像空间和高维属性空间的超像素层次结构，支持高维图像的一致探索
...
Method: 提出一种考虑高维属性流形的超像素层次结构，在构建过程中同时考虑图像空间和属性空间信息，实现图像引导的层次构建

Result: 通过两个用例比较，展示了这种新型图像引导层次结构在嵌入探索中的有效性，相比传统分层嵌入方法能实现更一致的高维图像探索

Conclusion: 提出的超像素层次结构能够实现高维图像在图像空间和属性空间中的一致探索，解决了传统方法忽略空间布局的问题
...
TL;DR: 提出基于几何感知扩散的4D头部虚拟人重建框架，从单张肖像图像生成高质量可动画的头部虚拟人，结合3D高斯实现实时渲染
...
Method: 提出几何感知扩散框架，联合合成肖像图像和对应的表面法线，使用无姿态表情编码器捕捉隐式表情表示，将合成图像和表情潜在编码融入基于3D高斯的虚拟人

Result: 实验表明该方法在视觉质量、表情保真度和跨身份泛化方面显著优于现有方法，同时支持实时渲染

Conclusion: 该方法通过几何感知扩散学习强几何先验，实现了从单张肖像图像重建高质量4D头部虚拟人，在多个指标上超越现有技术
...
TL;DR: 提出首个用于医学图像分类器的多模态自动审计框架，扩展切片发现方法到多模态表示，在MIMIC-CXR-JPG数据集上验证了其在故障发现和解释生成方面的有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管机器学习医学图像分类器取得进展，但其安全性和可靠性仍是实际应用中的主要担忧。现有审计方法主要依赖单模态特征或基于元数据的子组分析，可解释性有限且常无法发现隐藏的系统性故障。

Method: 引入首个自动审计框架，将切片发现方法扩展到多模态表示，专门针对医学应用。在MIMIC-CXR-JPG数据集上进行全面实验，涵盖常见故障场景。

Result: 框架在故障发现和解释生成方面表现出强大能力。多模态信息通常能实现更全面有效的分类器审计，而图像之外的单模态变体在资源受限场景中显示出强大潜力。
...
TL;DR: 提出SenCache框架，通过分析扩散模型输出对输入扰动的敏感性，动态选择缓存时间步，加速视频生成推理。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在视频生成中达到SOTA质量，但推理成本高（需要大量去噪步骤）。现有缓存方法依赖启发式标准选择缓存/重用时间步，需要大量调参。

Method: 提出敏感性感知缓存框架：1）形式化分析模型输出对噪声潜在变量和时间步扰动的敏感性；2）基于敏感性分析提出SenCache动态缓存策略，自适应选择每个样本的缓存时间步。

Result: 在Wan 2.1、CogVideoX和LTX-Video上实验，SenCache在相似计算预算下比现有缓存方法获得更好的视觉质量。

Conclusion: SenCache为自适应缓存提供理论基础，解释先前经验启发式方法的部分有效性，并将其扩展到动态、样本特定的方法。
...
TL;DR: MuViT：一种用于显微镜图像的多分辨率视觉Transformer，通过世界坐标系统融合不同尺度的图像信息，在多个生物医学图像任务上优于传统方法。
...
Method: 提出MuViT架构，将所有图像块嵌入到共享的世界坐标系中，并将旋转位置编码扩展到这些坐标，使注意力机制能够在单个编码器中集成广域上下文和高分辨率细节。采用多分辨率MAE预训练来生成尺度一致的表征。

Result: 在合成基准测试、肾脏组织病理学和高分辨率小鼠脑部显微镜图像上，MuViT相比强大的ViT和CNN基线模型取得了一致的改进。多分辨率MAE预训练进一步增强了下游任务的性能。

Conclusion: 显式的世界坐标建模为大规模显微镜分析中的多分辨率信息利用提供了一个简单而强大的机制，证明了在多尺度生物医学图像分析中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成模型在视觉保真度和创造力方面取得进展，但对复杂空间关系的编码要求更高，通常需要多次采样才能获得满意结果

Method: 1) 构建包含8万多个偏好对的SpatialReward-Dataset；2) 基于该数据集开发SpatialScore奖励模型，评估文本到图像生成中的空间关系准确性；3) 利用该奖励模型实现复杂空间生成的在线强化学习

Result: SpatialScore在空间评估方面表现优异，甚至超越领先的专有模型；在多个基准测试中，该专门化奖励模型为图像生成带来了显著且一致的空间理解提升

Conclusion: 提出的专门化奖励模型能有效增强当前图像生成模型的空间理解能力，通过强化学习显著改善复杂空间关系的生成质量
...
<details>
  <summary>Details</summary>
Motivation: 基于扩散模型的真实世界图像超分辨率虽然感知质量优秀，但迭代采样导致计算成本高。现有的一致性模型虽然推理高效，但存在一致性漂移积累和几何解耦问题（像素对齐但结构不连贯）。

Method: 提出GTASR方法：1）轨迹对齐策略，通过全路径投影修正切向量场；2）双参考结构校正机制，强制执行严格的结构约束。

Result: 大量实验验证GTASR在保持最小延迟的同时，性能优于代表性基线方法。

Conclusion: GTASR为真实世界图像超分辨率提供了一种简单有效的解决方案，解决了现有一致性模型的关键限制，实现了高效高质量的图像重建。
...
TL;DR: 论文提出组合泛化的三个必要条件（可分性、可迁移性、稳定性），证明这些条件要求神经表示必须具有线性分解和正交概念分量的几何结构，为线性表示假说提供了理论依据。


<details>
  <summary>Details</summary>
Motivation: 尽管现代模型在大规模数据上训练，但只能覆盖组合空间中极小部分，需要研究支持未见组合泛化的表示结构。论文旨在形式化组合泛化的必要条件，并探讨这些条件对表示几何结构的约束。

Method: 首先形式化组合泛化的三个必要条件（可分性、可迁移性、稳定性），证明这些条件导致线性分解和正交性的几何约束。然后推导维度边界，将可组合概念数量与嵌入几何联系起来。最后在CLIP、SigLIP、DINO等现代视觉模型中进行实证评估。

Result: 理论分析表明组合泛化要求表示必须线性分解为每个概念的分量，且这些分量在概念间正交。实证结果显示现代视觉模型表现出部分线性分解，具有低秩、近似正交的概念因子，且这种结构的程度与在未见组合上的组合泛化能力相关。

Conclusion: 线性表示假说得到了理论支持：神经网络中广泛观察到的线性结构是组合泛化的必然结果。随着模型规模扩大，这些几何条件预测了表示可能收敛的结构。研究为理解神经表示如何支持组合泛化提供了理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 人类通过关键转换在多抽象层次上感知动作，而机器依赖视觉特征容易过度分割。现有方法难以实现视频理解中的层次推理，需要解决动作潜在变量的识别问题。
...
Result: 在多个基准测试中，HAL模型显著优于现有的弱监督动作分割方法，证明了其在实际应用中的有效性。

Conclusion: 通过利用动作潜在变量在不同时间尺度上演化的特性，HAL模型能够有效识别高层动作变量，在弱监督动作分割任务中取得了显著改进，为视频理解中的层次推理提供了有效解决方案。
...
TL;DR: 提出一种解耦训练范式，通过全局流匹配学习长视频叙事结构，局部分布匹配对齐短视频教师模型，实现分钟级高质量视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型面临关键瓶颈：短视频数据丰富但长视频数据稀缺且领域有限，导致难以生成连贯的长时视频。

Method: 采用解耦扩散Transformer，全局流匹配头通过监督学习在长视频上捕捉叙事结构，局部分布匹配头通过模式寻求的反向KL散度将滑动窗口与冻结的短视频教师模型对齐。

Result: 方法有效缩小了保真度-时长差距，同时提升了局部清晰度、运动质量和长程一致性，实现了快速、高质量的分钟级视频生成。

Conclusion: 提出的模式寻求与均值寻求结合的训练范式，通过解耦局部保真度和长时连贯性，成功解决了长视频生成的瓶颈问题。
...
TL;DR: UFO-4D：一个统一的feedforward框架，仅从一对未标定图像重建密集4D表示，直接估计动态3D高斯泼溅，实现几何、运动和相机姿态的联合一致估计。


<details>
  <summary>Details</summary>
Motivation: 现有密集4D重建方法依赖缓慢的测试时优化或碎片化的任务特定前馈模型，需要一种统一的feedforward框架从少量未标定图像重建4D表示。

Method: 使用动态3D高斯泼溅表示，通过可微分渲染多个信号（外观、深度、运动），实现自监督图像合成损失，所有模态共享相同几何基元，相互正则化。

Result: 在联合几何、运动和相机姿态估计方面比先前工作提升高达3倍，支持跨新视角和时间的高保真4D插值。

Conclusion: UFO-4D通过统一的动态3D高斯表示和多信号可微分渲染，克服了数据稀缺问题，实现了从少量未标定图像的feedforward 4D重建。
...
TL;DR: REPO是一种基于表征擦除的偏好优化方法，通过token级偏好学习强制有毒输出的表征收敛到良性版本，实现深度解毒并保持模型实用性。


<details>
  <summary>Details</summary>
Motivation: 现有基于DPO、NPO等方法的解毒防御存在局限性：它们只是表面降低有害输出的可能性，容易被对抗性提示和微调攻击绕过，线性探测显示有害"方向"仍在表征中存在。

Method: 提出表征擦除偏好优化（REPO），将解毒重新定义为token级偏好问题，使用新颖的目标函数和偏好数据，强制有毒输出的表征向良性版本收敛。

Result: REPO实现了最先进的鲁棒性，能够阻止包括重新学习攻击和增强GCG越狱在内的复杂威胁，而现有的基于表征和输出的方法都失败了。机制分析显示REPO对毒性编码神经元进行了深度局部编辑。

Conclusion: REPO通过token级表征擦除方法实现了深度解毒，不仅有效防御各种攻击，还保持了模型的通用实用性，为解决LLM毒性问题提供了更鲁棒的解决方案。
...
TL;DR: 提出U-CAN框架，通过低秩适配器上的对比衰减机制，解决生成式推荐中隐私遗忘与性能保持的平衡问题


<details>
  <summary>Details</summary>
Motivation: 生成式推荐系统在微调时会无意中将敏感属性编码到模型参数中，引发隐私担忧。现有的机器遗忘技术面临多义性困境，神经元同时编码敏感数据和一般推理模式，导致传统梯度或剪枝方法造成灾难性的性能损失。

Method: 提出Utility-aware Contrastive AttenuatioN (U-CAN)框架，在低秩适配器上操作：1) 通过对比激活量化风险，关注对遗忘集高度敏感但对保留集抑制的神经元；2) 引入效用感知校准机制，结合权重大小和保留集激活范数；3) 采用自适应软衰减，使用可微分衰减函数选择性地降低高风险参数，而非二进制剪枝。

Result: 在两个公共数据集上的七个指标实验表明，U-CAN实现了强大的隐私遗忘、效用保持和计算效率。

Conclusion: U-CAN通过精确的对比衰减机制，有效解决了生成式推荐中的隐私遗忘问题，在保护隐私的同时保持了模型性能，避免了传统方法的网络结构碎片化问题。
...
TL;DR: 量子机器学习中角度编码的频率可训练方法存在优化限制，提出三元网格初始化解决频率可达性问题


<details>
  <summary>Details</summary>
Motivation: 量子机器学习中的角度编码方法理论上可以通过可训练频率实现高效学习，但实际优化中频率预因子存在可达性限制，导致优化失败

Method: 提出三元网格初始化方法，使用三元编码生成密集整数频率谱，确保目标频率位于局部可达范围内，虽然需要O(log_3(ω_max))个编码门，但比固定频率方法指数级减少

Result: 在三频移高频合成目标上，三元网格初始化中位R²得分0.9969，而可训练频率基线仅为0.1841；在实际航班乘客数据集上，三元网格初始化中位R²得分0.9671，比可训练频率初始化（0.7876）提升22.8%

Conclusion: 频率可训练方法的实际效果受限于梯度优化的可达性范围，三元网格初始化通过确保目标频率在可达范围内，显著提高了量子机器学习模型的性能
...
TL;DR: Brain-OF是首个同时预训练fMRI、EEG和MEG的脑功能全模态基础模型，通过统一框架处理单模态和多模态输入，在多种下游任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有脑基础模型大多局限于单一功能模态，无法充分利用不同成像技术的互补时空动态特性和集体数据规模，限制了模型的泛化能力和性能。

Method: 1) 提出任意分辨率神经信号采样器，将异质时空分辨率的脑信号投影到共享语义空间；2) 采用DINT注意力与稀疏专家混合的骨干网络，共享专家捕获模态不变表征，路由专家专注于模态特定语义；3) 提出掩码时频建模的双域预训练目标，在时间和频率域联合重建脑信号。

Result: 在约40个数据集的大规模语料库上预训练，在多种下游任务中表现出优越性能，证明了联合多模态整合和双域预训练的有效性。

Conclusion: Brain-OF成功构建了首个脑功能全模态基础模型，通过统一框架整合多模态脑信号，显著提升了模型在各种神经科学任务中的表现，为脑科学研究提供了强大的工具。
...
TL;DR: EvoX是一种自适应进化方法，通过联合进化候选解和搜索策略，动态调整进化过程，在200多个真实世界优化任务中优于现有AI驱动的进化方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法如AlphaEvolve结合LLM驱动优化和进化搜索，但其搜索策略通常是固定的，无法适应不同任务或任务中搜索空间的变化。需要一种能自适应调整搜索策略的方法。

Method: EvoX同时进化候选解和生成这些解的搜索策略，基于优化进展持续更新如何选择和变化先前解，使系统能在优化过程中动态切换不同搜索策略。

Result: 在近200个真实世界优化任务中，EvoX在大多数任务上优于现有AI驱动的进化方法，包括AlphaEvolve、OpenEvolve、GEPA和ShinkaEvolve。

Conclusion: 通过联合进化候选解和搜索策略，EvoX实现了自适应进化过程，能够动态调整搜索策略，在广泛的优化任务中表现出优越性能。
...
TL;DR: 论文提出"人类有界智能极限"理论：当人类监督通道不足以捕捉潜在评估目标时，会形成信息减少通道，导致任何学习者都存在严格正的风险下限，仅靠模型缩放无法消除这些错误。


<details>
  <summary>Details</summary>
Motivation: 大语言模型主要基于人类生成的数据和反馈训练，但表现出持续的误差，这些误差源于标注噪声、主观偏好和自然语言的有限表达能力。作者认为这些限制反映了监督通道的结构特性，而非模型规模或优化问题。

Method: 开发统一理论框架，证明当人类监督通道不足以捕捉潜在评估目标时，会形成信息减少通道，导致严格正的风险下限。通过六个互补框架（算子理论、PAC-Bayes、信息论、因果推断、范畴论、RLHF的博弈论分析）形式化这一"人类有界智能极限"。

Result: 理论解释了为什么仅靠缩放无法消除持续的人类对齐错误，并描述了辅助非人类信号（如检索、程序执行、工具）如何增加有效监督容量并降低风险下限。在真实偏好数据、合成已知目标任务和外部可验证基准上的实验证实了预测的结构特征。

Conclusion: 人类监督存在固有的信息限制，仅靠缩放无法突破"人类有界智能极限"。需要引入足够信息的辅助通道（非人类信号）来恢复关于潜在目标的信息，从而减少或消除超额误差。
...
TL;DR: REFINE：一种两阶段方法，通过非线性预处理提取稳定的问卷项目值，然后用线性模型预测未来症状严重程度，在保持可解释性的同时提高预测准确性。
...
Method: REFINE采用两阶段策略：1）非线性预处理模块估计稳定的项目值，去除访问和工具特异性伪影；2）线性映射从这些稳定化的基线项目预测未来严重程度。非线性能力集中在预处理阶段，预后关系保持线性透明。

Result: 在实验中，REFINE优于其他可解释方法，同时在精神病学和非精神病学纵向预测任务中保持预后因素的清晰全局归因。

Conclusion: 通过将非线性集中在预处理阶段并保持预后关系线性，REFINE在提高预测准确性的同时保持了全局可解释性，为临床应用中平衡准确性和可解释性提供了有效解决方案。
...
TL;DR: 提出了一种不确定性感知的概念瓶颈模型方法，通过量化LLM标注概念的不确定性并纳入训练过程，解决现有方法忽略LLM标注不确定性和幻觉风险的问题。


<details>
  <summary>Details</summary>
Motivation: 概念瓶颈模型(CBMs)需要专家标注概念，成本高昂。现有利用大语言模型(LLMs)构建概念瓶颈的方法存在两个关键问题：1) 忽略LLM标注概念的不确定性，缺乏量化机制；2) 未将标注不确定性纳入模型训练过程，增加了LLM幻觉带来的错误风险。

Method: 提出了一种新颖的不确定性感知CBM方法：1) 使用有效且无分布假设的方法严格量化LLM标注概念标签的不确定性；2) 将量化的概念不确定性纳入CBM训练过程，以考虑不同LLM标注概念的可靠性差异；3) 提供了理论分析支持。

Result: 在真实世界数据集上的大量实验验证了所提出方法的理想特性，表明该方法能够有效处理LLM标注的不确定性，提高模型的可靠性和性能。

Conclusion: 该方法成功解决了现有LLM辅助概念瓶颈模型的两个关键限制，通过量化并整合概念不确定性，提高了模型的鲁棒性和可靠性，为概念瓶颈模型的广泛应用提供了更实用的解决方案。
...
TL;DR: FedDAG提出了一种新的聚类联邦学习框架，通过结合数据和梯度信息的加权类相似度度量进行更准确的客户端聚类，并采用双编码器架构实现跨集群特征迁移，显著提升了异构数据下的模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有聚类联邦学习方法存在两个主要问题：1) 仅依赖数据相似度或梯度相似度进行聚类，导致对客户端相似性的评估不完整；2) 限制知识和表示共享仅限于同一集群内的客户端，使集群模型无法从跨集群的多样化客户端群体中受益。

Method: FedDAG采用加权类相似度度量，整合数据和梯度信息进行更全面的客户端相似性评估。同时采用双编码器架构：主编码器在自身客户端数据上训练，辅助编码器使用互补集群的梯度进行精炼，实现跨集群特征迁移的同时保持集群特定专业化。

Result: 在多种基准测试和数据异构设置下的实验表明，FedDAG在准确率上持续优于最先进的聚类联邦学习基线方法。

Conclusion: FedDAG通过更全面的相似度度量和跨集群知识共享机制，有效解决了联邦学习中数据异构性问题，为聚类联邦学习提供了更优的解决方案。
...
TL;DR: 该论文提出了一种新的模拟方法pmsims，用于确定临床预测模型开发的最小样本量，通过结合学习曲线、高斯过程优化和保证原则，相比现有方法更灵活、高效且可解释。
...
Method: 提出了一个概念框架，区分基于均值和基于保证的准则。在此基础上，开发了一种新颖的模拟方法，整合学习曲线、高斯过程优化和保证原则，以确定在较高概率下达到目标性能的样本量。该方法在开源、模型无关的R包pmsims中实现。

Result: 通过案例研究显示，样本量估计在不同方法、性能指标和建模策略之间存在显著差异。与现有工具相比，pmsims提供了灵活、高效和可解释的解决方案，能够适应多样化的模型和用户定义的指标，同时明确考虑模型性能的变异性。

Conclusion: 该框架和软件通过结合灵活性和计算效率，推进了临床预测模型样本量方法学的发展。未来工作应将这些方法扩展到层次化和多模态数据，纳入公平性和稳定性指标，并解决缺失数据和复杂依赖结构等挑战。
...
TL;DR: 该论文证明了基于样本的神经算子能够学习无限维再生核希尔伯特空间中任意有限类别集合，即使这些类别既非凸也非连通。研究者开发了一个神经算子驱动的函数数据聚类流程，并将其应用于未标记的常微分方程轨迹族。


<details>
  <summary>Details</summary>
Motivation: 虽然神经算子在回归问题中已得到较好理解，但在分类及其无监督对应问题——聚类方面的研究相对较少。研究者希望探索神经算子在无限维空间中的聚类能力，特别是对于函数数据（如ODE轨迹）的聚类问题。

Method: 提出通用聚类定理，证明神经算子能够近似任意有限个封闭类别。开发了一个神经算子驱动的聚类流程：首先通过预训练编码器将离散轨迹提升为连续特征映射，然后通过轻量级可训练头部映射到软分配。该方法在Kuratowski拓扑下确保无假阳性误分类。

Result: 理论证明神经算子能够以任意精度近似无限维再生核希尔伯特空间中的任意有限类别集合。在合成ODE基准测试中，提出的SNO方法能够恢复传统方法失败的潜在动态结构，实验结果与通用聚类理论一致。

Conclusion: 神经算子在函数数据聚类方面具有强大的理论保证和实际效果，能够处理传统方法难以应对的非凸、非连通类别，为科学计算中的无监督学习提供了新工具。
...
TL;DR: 研究如何用加法误差近似未知的子加性集函数，通过分析缺失值的最小和最大补全距离，开发减少该距离的方法，并在实际场景中进行实证验证。


<details>
  <summary>Details</summary>
Motivation: 子加性集函数在计算经济学、组合优化和可解释机器学习中很重要，但完整指定需要指数级数量的值，这在实践中资源消耗大。缺失值会引入歧义，特别是在需要进一步优化时。基于已知的确定性值查询在乘法误差下难以近似子加性函数的结果，研究转向加法误差近似。

Method: 1) 深入探索不同类别集函数缺失值的最小和最大补全，分析其距离；2) 开发方法在已知先验的集函数类别上最小化该距离，通过离线或在线方式披露额外子集的值；3) 在实际场景中进行算法性能的实证演示。

Result: 论文提出了系统的方法来处理子加性集函数的近似问题，包括理论分析和实用算法，并在实际应用中展示了算法的有效性。

Conclusion: 该研究为处理子加性集函数缺失值问题提供了理论框架和实用方法，能够有效减少最小和最大补全之间的距离，在实际应用中表现出良好性能。
...
TL;DR: Rudder是一个嵌入在AWS DistDGL框架中的软件模块，使用LLM的上下文学习能力实现自适应预取，显著减少大规模图神经网络训练中的通信开销。
...
Conclusion: Rudder通过LLM的上下文学习能力实现了有效的自适应预取，显著提升大规模分布式图神经网络训练效率，展示了生成式AI在系统优化中的潜力。
...
TL;DR: 论文研究多平台机器学习中的"过度专业化陷阱"问题，提出使用知识蒸馏技术让平台"探测"同行模型预测，以改善全局性能。


<details>
  <summary>Details</summary>
Motivation: 在多平台机器学习部署中，用户选择最适合自己的平台，现有算法只关注局部损失，可能导致模型全局性能极差，即使存在低全人口损失的模型。

Method: 提出一种算法，允许学习者"探测"同行模型的预测，从而了解不选择他们的用户。使用知识蒸馏技术，让平台能够学习其他模型的知识。

Result: 当探测源足够信息丰富时（如已知的市场领导者或大多数具有良好全局性能的同行），该程序几乎必然收敛到具有有界全人口风险的平稳点。

Conclusion: 通过知识蒸馏让平台探测同行预测可以有效避免过度专业化陷阱，在MovieLens、Census和Amazon Sentiment数据集上的半合成实验验证了这一发现。
...
TL;DR: Flowette是一个基于连续流匹配的图生成模型，通过图神经网络Transformer学习带节点和边属性的图表示的速度场，结合图元（graphettes）结构先验，在合成和小分子图生成任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 研究具有重复子图模式的图生成建模，传统方法难以有效捕捉图的拓扑结构和长程结构依赖关系，需要结合领域驱动的结构先验来提升生成质量。

Method: 提出Flowette连续流匹配框架，使用图神经网络Transformer学习图表示的速度场；通过最优传输耦合保持拓扑结构，正则化处理长程结构依赖；引入graphettes作为新的概率图结构模型族，通过受控结构编辑（如环、星、树等模式）泛化图元。

Result: 理论分析了框架的耦合、不变性和结构特性；在合成和小分子图生成任务上进行了实证评估；Flowette展示了持续改进，证明了将结构先验与基于流的训练相结合对建模复杂图分布的有效性。

Conclusion: Flowette通过结合结构先验和基于流的训练，能够有效建模具有重复子图模式的复杂图分布，在生成建模任务中表现出色。
...
TL;DR: HQTCN结合经典时间窗口与量子卷积网络，通过共享量子电路跨时间窗口捕获长程依赖，显著减少参数，在多元时序任务中表现优异


<details>
  <summary>Details</summary>
Motivation: 量子机器学习处理复杂多元时序数据面临可扩展性挑战，需要更高效的参数利用方法

Method: 提出混合量子时序卷积网络(HQTCN)，结合经典时间窗口技术和量子卷积神经网络核心，跨时间窗口共享量子电路

Result: 在合成NARMA序列和高维EEG时序数据上，单变量数据表现与经典基线相当，多元任务优于所有基线，数据有限条件下表现尤其突出
...
Conclusion: 该双流稀疏Mixer框架能有效处理多变量时间序列的多尺度特征、弱相关性和噪声问题，提升预测精度，具有实际应用价值。
...
TL;DR: 该论文系统研究了图神经网络在金融欺诈检测中权重初始化和归一化策略的影响，发现这些策略的效果与具体GNN架构相关，并提供了针对不同架构的实用指导。


<details>
  <summary>Details</summary>
Motivation: 图神经网络在金融欺诈检测中具有潜力，但其在真实世界反洗钱基准测试中的效果严重依赖于训练实践，特别是权重初始化和归一化策略，这些方面尚未得到充分探索。
...
Conclusion: 研究结果为在类别严重不平衡的数据集上部署GNN进行反洗钱提供了架构特定的实用指导，并发布了包含时间数据分割、种子运行和完整消融结果的可复现实验框架。
...
TL;DR: 该研究系统评估了电子健康记录（EHR）和胸部X光（CXR）的多模态融合在临床预测中的效果，发现多模态融合在模态完整时能提升性能，但在模态缺失时效果下降，且不能自动改善算法公平性。


<details>
  <summary>Details</summary>
Motivation: 尽管机器学习在临床决策支持中具有潜力，但多模态学习在实际应用中何时真正有效仍不明确，特别是在模态缺失和公平性约束下。本研究旨在回答四个基本问题：多模态融合何时能改善临床预测、不同融合策略的比较、现有方法对缺失模态的鲁棒性，以及多模态模型是否能实现算法公平性。

Method: 使用MIMIC-IV和MIMIC-CXR的标准化队列，对EHR和CXR的多模态融合进行系统基准测试。研究比较了不同的融合策略，评估了模型对缺失模态的鲁棒性，并分析了算法公平性。同时发布了灵活的可复现基准测试工具包。

Result: 多模态融合在模态完整时能提升性能，特别是在需要EHR和CXR互补信息的疾病中。跨模态学习机制能捕捉临床上有意义的依赖关系，但EHR的丰富时间结构导致模态不平衡。在现实缺失情况下，多模态优势迅速下降，除非模型专门设计处理不完整输入。多模态融合不能自动改善公平性，亚组差异主要源于不同人口群体间的敏感性不平等。

Conclusion: 本研究为多模态学习何时有效、何时失败以及原因提供了可操作的指导，为开发既有效又可靠的临床可部署多模态系统奠定了基础。同时发布的开放源码工具包支持可复现和可扩展的评估。
...
TL;DR: BTTackler是一个新颖的超参数优化框架，通过训练诊断自动识别训练问题（如梯度消失、收敛不足），并提前终止不良试验，显著提高HPO效率。


<details>
  <summary>Details</summary>
Motivation: 现有自动化HPO方法大多基于准确率指标，但许多试验在训练早期会遇到严重问题（梯度消失、收敛不足），这些问题无法通过准确率指标及时反映，导致计算资源浪费和优化效率低下。

Method: 提出BTTackler框架，通过计算一组精心设计的量化指标来诊断每个试验，一旦检测到训练问题就触发提前终止，从而避免不良试验占用计算资源。

Result: 在三个经典DNN和四种广泛使用的HPO方法上评估，结果显示：1）达到相同准确率时平均减少40.33%的时间消耗；2）在给定时间预算内平均多进行44.5%的前10名试验。

Conclusion: BTTackler通过训练诊断有效识别和终止不良试验，显著提高了HPO效率，并提供了开源Python库便于用户集成使用。
...
TL;DR: 本文对单层随机近似隐式微分算法（SSAID）进行了精细化收敛分析，证明了该算法达到ε-平稳点的Oracle复杂度为O(κ⁷ε⁻²)，匹配了多层方法的最优速率，同时保持了单层更新的计算效率。


<details>
  <summary>Details</summary>
Motivation: 随机双层优化已成为元学习和超参数优化的基础框架。尽管单层算法在实践中普遍存在（同时更新下层和上层变量），但其理论理解，特别是在随机机制下，仍远落后于多层算法。现有分析通常产生次优收敛速率，或将关键的下层条件数κ依赖隐藏在通用Lipschitz常数中。

Method: 本文对单层随机近似隐式微分算法（SSAID）进行了精细化收敛分析。通过改进的理论分析框架，显式地刻画了算法收敛对下层条件数κ的依赖关系。

Result: 证明了SSAID算法达到ε-平稳点的Oracle复杂度为O(κ⁷ε⁻²)。这一结果在两方面具有重要意义：(1) 匹配了最先进多层方法（如stocBiO）的最优O(ε⁻²)速率，同时保持了单层更新的计算效率；(2) 首次为基于随机AID的单层方法提供了显式、细粒度的κ依赖特征刻画。

Conclusion: SSAID不仅是一种启发式方法，而且具有严格的理论基础，其收敛保证可与主流多层框架竞争。这项工作填补了单层随机双层优化算法理论分析的空白，为实际应用提供了理论支撑。
...
TL;DR: 提出FlexGuard方法，通过连续风险评分和阈值调整来适应不同严格度的LLM内容审核需求，解决现有二元分类审核模型在严格度变化时的脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM内容安全审核模型大多采用固定的二元分类，假设有害性定义不变。但实际应用中，平台对有害内容的定义和执法严格度各不相同且随时间变化，导致二元审核模型在需求变化时表现脆弱。

Method: 1) 引入FlexBench基准，支持多严格度制度下的受控评估；2) 提出FlexGuard方法，基于LLM输出校准的连续风险评分反映风险严重程度；3) 通过风险对齐优化训练提高评分-严重度一致性；4) 提供阈值选择策略以适应部署时的目标严格度。

Result: 在FlexBench和公共基准测试中，FlexGuard实现了更高的审核准确性和在不同严格度下显著改善的鲁棒性。现有审核模型在不同严格度制度下表现出严重的跨严格度不一致性。

Conclusion: FlexGuard通过连续风险评分和灵活阈值调整，为LLM内容审核提供了适应不同严格度需求的解决方案，显著提升了实际部署的可用性和鲁棒性。
...
TL;DR: FedRot-LoRA：通过正交变换对齐客户端更新来解决联邦LoRA中的旋转失配问题，提高训练稳定性


<details>
  <summary>Details</summary>
Motivation: 联邦LoRA在分散数据上微调大语言模型时，由于低秩分解的旋转不变性，客户端更新在不同潜在子空间中表示，导致因子平均时产生破坏性干扰和聚合误差

Method: 提出FedRot-LoRA框架，在聚合前通过正交变换对齐客户端更新，减少跨客户端子空间失配，不增加通信成本或限制模型表达能力

Result: 在自然语言理解和生成任务上的实验表明，FedRot-LoRA在各种异构程度和LoRA秩设置下都优于现有联邦LoRA基线方法

Conclusion: 旋转对齐能有效解决联邦LoRA中的聚合误差问题，提供更紧的误差上界，提高训练稳定性和性能
...
Method: 提出AnomalyFilter方法，作为选择性过滤器，仅对异常部分去噪而保留正常部分；训练阶段对高斯噪声进行掩码，并在去噪过程中不向实例添加噪声

Result: 在五个数据集上的实验表明，AnomalyFilter在正常部分实现了显著较低的重建误差，为异常检测有效性提供了实证支持

Conclusion: AnomalyFilter是首个专注于为时间序列异常检测定制扩散模型噪声设计的开创性方法，两个简单组件的协同作用大大提升了朴素扩散模型的性能
...
<details>
  <summary>Details</summary>
Motivation: 多模态张量时间序列在搜索引擎、环境监测等领域广泛存在，学习其表示对多种应用有益，但张量的复杂性阻碍了丰富表示的实现。

Method: MoST使用张量切片方法降低TTS结构复杂度，学习可解耦为各非时间模态的表示。每个表示包含模态特定特征（同一模态内变量关系）和模态不变特征（不同模态共有）。采用对比学习框架，损失函数包含模态特定和模态不变两部分，有效利用解耦表示作为增强。

Result: 在真实数据集上的大量实验表明，MoST在分类和预测准确率方面持续优于最先进方法。

Conclusion: MoST是一种专门为多模态张量时间序列设计的有效表示学习方法，通过解耦表示和对比学习框架显著提升了表示学习性能。
...
TL;DR: 研究发现小Transformer模型的训练轨迹呈现主导漂移方向与横向残余动态的结构，不同优化器（AdamW vs SGD）在轨迹几何结构上存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 探索小Transformer模型训练过程中参数更新的几何结构，理解优化器如何影响学习轨迹的维度与动态特性，超越仅从损失值角度分析训练过程。

Method: 使用非中心化、行归一化的轨迹PCA分析训练参数更新，比较AdamW与SGD变体在匹配损失水平下的轨迹几何差异，通过reheating实验选择性扰动横向分量。

Result: 单方向捕获早期训练中大部分累积参数移动，其余分量编码辅助探针性能的振荡行为；AdamW产生多维漂移结构，SGD产生近乎共线的参数演化；reheating主要影响横向分量而非主导漂移坐标。

Conclusion: 优化器选择塑造了学习轨迹的有效维度和结构，这些特性无法仅从损失值中观察到，表明训练动态的几何分析对理解优化过程至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 跨域强化学习中，源域和目标域之间存在动态变化，且缺乏目标域环境交互和奖励监督，这阻碍了直接策略学习。需要一种方法能够在源域内学习可迁移到目标域的策略。

Method: 提出BDGxRL框架：1) 使用扩散薛定谔桥(DSB)对齐源域转移与目标域离线演示编码的动态；2) 引入奖励调制机制，基于状态转移估计奖励，应用于DSB对齐的样本，确保奖励与目标域动态的一致性。

Result: 在MuJoCo跨域基准测试中，BDGxRL优于现有最先进基线方法，在转移动态变化下表现出强大的适应性。

Conclusion: BDGxRL框架成功解决了跨域强化学习中缺乏目标域交互和奖励监督的挑战，通过在源域内进行目标导向的策略学习，实现了有效的跨域策略迁移。
...
TL;DR: 该研究首次将大语言模型应用于光学设计领域，通过构建OptiDesignQA数据集和混合训练目标，结合DrGRPO优化和光学词典奖励，使非专业用户也能设计功能性透镜系统。


<details>
  <summary>Details</summary>
Motivation: 光学设计是一个高度非凸的优化问题，严重依赖人类专家经验和领域知识。虽然大语言模型拥有丰富的光学知识，但在透镜系统设计方面的能力受到限制。本研究旨在填补这一空白，让没有正式光学训练的用户也能成功开发功能性透镜系统。

Method: 1) 构建OptiDesignQA数据集，包含经典透镜系统和自动化算法生成的新配置；2) 通过全系统合成和透镜补全的混合目标向LLM注入领域知识；3) 使用DrGRPO和光学词典奖励进行物理驱动的策略对齐；4) 集成专门的光学优化例程进行端到端微调和精度优化。

Result: 实验结果表明，该方法在传统优化算法和LLM对比方法中表现出优越性，能够有效设计功能性透镜系统。

Conclusion: 该研究成功将LLM引入光学设计领域，通过创新的数据集构建、训练策略和优化方法，使非专业用户能够利用LLM的光学知识进行透镜系统设计，为光学设计自动化开辟了新途径。
...
Result: 在五个离线RL基准测试中与15个基线算法对比，MAGE成功整合多尺度轨迹建模与条件引导，在长视野稀疏奖励场景中生成连贯可控的轨迹。

Conclusion: MAGE通过多尺度自回归生成框架有效捕捉轨迹的多分辨率时间依赖关系，解决了长视野稀疏奖励任务中的挑战，展示了多尺度轨迹建模与条件引导的有效结合。
...
TL;DR: TradeFM是一个524M参数的生成式Transformer模型，通过从90多只股票的数十亿交易事件中学习，将基础模型范式引入市场微观结构领域，实现了跨资产的通用表示。


<details>
  <summary>Details</summary>
Motivation: 基础模型在语言和基因组学等领域取得了成功，但尚未应用于市场微观结构。作者希望将这一范式引入金融市场，学习跨资产的通用交易表示，以支持合成数据生成、压力测试和基于学习的交易代理。

Method: 开发了尺度不变特征和通用标记化方案，将异质多模态订单流事件映射为统一的离散序列，消除了资产特定校准。模型与确定性市场模拟器集成，TradeFM生成的推演能够重现金融回报的关键风格化事实。

Result: TradeFM生成的推演能够重现金融回报的重尾分布、波动率聚类和无自相关性等关键特征。在分布误差上比Compound Hawkes基准低2-3倍，并能够零样本泛化到地理分布外的APAC市场，仅出现适度的困惑度退化。
...
<details>
  <summary>Details</summary>
Motivation: 传统的非线性CCA试图精确解混，但已被证明是病态问题。本文重新框架多视角CCA为基不变子空间识别问题，旨在在更合理的条件下实现可识别性。

Method: 将多视角CCA重新定义为基不变子空间识别问题，而非精确解混。在适当的潜在先验和谱分离条件下，证明多视角CCA可以恢复成对相关的信号子空间。对于N≥3个视角，目标函数可以分离出所有视角共享的联合相关子空间，同时消除视角私有变异。

Result: 理论证明：在适当条件下，多视角CCA可以恢复成对相关的信号子空间（存在视角正交模糊性）。对于N≥3个视角，可以分离出所有视角共享的联合相关子空间。建立了通过谱扰动理论将经验交叉协方差集中转化为显式子空间误差界的有限样本一致性保证。

Conclusion: 多视角非线性CCA在重新框架为子空间识别问题后是可识别的。理论分析提供了严格的保证，实验验证了理论发现并确认了假设条件的必要性。该方法为多视角非线性表示学习提供了理论基础。
...
TL;DR: 提出通用启发式预测器，一次训练即可泛化到未见任务，将A*计算量减半，在完全不同于训练数据的任务上仍能提供接近最优解


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的路径规划方法严重依赖训练和测试网格地图来自相同分布（如城市地图、室内地图等），在分布外任务上表现不佳，限制了实际应用。需要开发能够高效处理任何问题实例的通用求解器。

Method: 设计通用启发式预测器模型，通过一次训练就能泛化到完全未见过的任务谱系。该方法使用深度神经网络近似考虑障碍物位置/形状的启发式函数。

Result: 将A*的计算工作量减少了高达2.2倍，同时在完全不同于训练数据的任务上，平均仍能提供距离最优成本3%以内的解决方案。这是可学习求解器首次达到的里程碑。

Conclusion: 成功开发出通用启发式预测器，解决了现有学习型路径规划方法在分布外任务上泛化能力差的问题，为实际应用提供了高效的通用求解器。
...
Method: 提出GRAIL方法：使用小型校准集，通过Gram矩阵总结隐藏激活，应用岭回归从压缩后的表示线性重建原始隐藏表示，将重建映射吸收到下游投影权重中，同时压缩上游层。
...
Method: 提出MPU框架，包含两个服务器模块：预处理（生成随机扰动和重参数化的模型副本）和后处理（反转重参数化并聚合更新，采用谐波去噪减轻扰动影响）。

Result: 在7种遗忘算法上的实验表明，MPU在10%噪声下性能退化大多低于1%，在1%噪声下某些算法甚至优于无噪声基线。

Conclusion: MPU能有效解决LLM遗忘中的双重隐私约束问题，在保持遗忘性能的同时保护服务器参数和客户端数据的隐私。
...
TL;DR: 提出一种同时预训练actor和critic网络的方法，通过专家演示提升强化学习算法的样本效率


<details>
  <summary>Details</summary>
Motivation: 强化学习actor-critic算法通常需要大量环境交互，限制了在机器人领域的应用。虽然已有方法通过行为克隆预训练actor网络，但critic网络的初始化很少被关注，而critic在策略优化中起着核心作用。

Method: 提出actor-critic预训练方法：1) actor网络通过行为克隆在专家演示上预训练；2) critic网络通过预训练策略的rollout获得的回报进行预训练。该方法适用于PPO等actor-critic算法。
...
TL;DR: 本文研究了离线强化学习在一般函数逼近下的理论问题，解决了现有方法只能处理有限小动作空间、无法适应独立策略参数化的限制，将理论保证扩展到大规模或连续动作空间的参数化策略类。


<details>
  <summary>Details</summary>
Motivation: 现有离线强化学习算法（如PSPI）虽然计算上可处理，但仅适用于有限小动作空间，且依赖于状态级镜像下降，要求从价值函数隐式推导策略，无法适应实践中普遍使用的独立策略参数化。需要解决这些限制，将理论保证扩展到参数化策略类。

Method: 通过将镜像下降扩展到参数化策略，识别上下文耦合为核心难点，展示如何将镜像下降与自然策略梯度连接，从而获得新的分析、理论保证和算法洞见。这种方法在离线RL和模仿学习之间建立了令人惊讶的统一。

Result: 成功将离线强化学习的理论保证扩展到参数化策略类，适用于大规模或连续动作空间。通过连接镜像下降和自然策略梯度，解决了上下文耦合问题，获得了新的理论分析和算法设计洞见。

Conclusion: 本文解决了离线强化学习在一般函数逼近下的关键理论限制，将悲观主义方法的理论保证扩展到参数化策略类，建立了离线RL和模仿学习之间的统一框架，为实际应用提供了更广泛的理论基础。
...
TL;DR: 提出SafeQIL算法，在约束MDP中从未知约束和非可观测成本的演示轨迹中学习策略，平衡保守性与高回报轨迹的可能性。


<details>
  <summary>Details</summary>
Motivation: 在约束MDP中，给定演示轨迹但约束未知且成本不可观测，需要学习一个策略，既能最大化演示轨迹的似然，又能平衡保守性与高回报轨迹的可能性。
...
Conclusion: SafeQIL算法能够有效学习约束MDP中的策略，平衡安全性与高回报，在逆约束强化学习问题上表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 电子健康记录中的纵向数据通常存在左截断问题，导致诊断记录不完整且不可靠，无法准确确定疾病起始时间。相比之下，门诊处方形成基于续药的轨迹，提供了疾病管理的连续信号。

Method: 提出概率框架，将处方动态建模为更新过程，通过变化点检测来推断慢性治疗起始时间。该方法检测从基线泊松（偶发性处方）机制到特定机制威布尔（持续性治疗）更新模型的转变。

Result: 使用全国240万人的电子处方数据集，该方法比简单的基于规则的触发方法产生更合理的时间起始估计，显著减少了在强左截断情况下的不合理早期检测。检测性能因疾病而异，与处方密度密切相关。

Conclusion: 该方法为基于治疗的起始时间推断提供了更可靠的框架，但检测性能在不同疾病间存在差异，突显了基于治疗的起始推断方法的优势和局限性。
...
TL;DR: FedNSAM：一种将全局Nesterov动量引入联邦学习中的SAM算法，通过协调全局和局部平坦度一致性来提升泛化性能


<details>
  <summary>Details</summary>
Motivation: 在联邦学习中，多步本地更新和数据异质性通常会导致更尖锐的全局最小值，从而降低全局模型的性能。现有的FL算法将SAM集成到本地训练中，但在高数据异质性设置下，本地训练的平坦度并不等同于全局模型的平坦度，因此需要协调全局和局部平坦度的一致性。

Method: 提出FedNSAM算法，通过引入全局Nesterov动量到本地更新中，使用全局Nesterov动量作为客户端全局扰动的局部估计方向和外推方向，从而协调全局和局部平坦度的一致性。

Result: 理论上证明了FedNSAM比FedSAM具有更紧的收敛界；实证上在CNN和Transformer模型上进行了全面实验，验证了FedNSAM的优越性能和效率。

Conclusion: FedNSAM通过引入全局Nesterov动量有效协调了联邦学习中全局和局部平坦度的一致性，在理论和实证上都表现出优于现有SAM算法的性能。
...
TL;DR: ULW-SleepNet是一个超轻量级多模态睡眠分期框架，通过新颖的双流可分离卷积块等技术，在保持竞争力的准确率同时大幅减少计算开销，适用于可穿戴设备实时监测。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型计算量大且多为单通道EEG设计，限制了在多模态PSG数据中的实用性，需要开发轻量高效的睡眠分期框架。
...
Conclusion: ULW-SleepNet在保持竞争力的准确率同时大幅降低计算复杂度，展示了在可穿戴和物联网设备上实时睡眠监测的强大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有领域适应理论主要处理传统数据分布偏移，但图数据作为结构化对象具有非欧几里得特性，且图学习专用架构使得对图分布偏移的细粒度分析变得复杂。需要开发专门针对图分类领域偏移的理论框架。

Method: 采用随机图模型作为数据生成过程，通过向量值再生核希尔伯特空间（vRKHS）公式化，推导出包含三个可分解项的泛化界：领域差异项、谱几何项（由可访问的截断谱总结）和振幅项（聚合收敛和构造稳定性效应）。

Result: 理论推导出可分解的泛化界，三个因子分别对应领域差异、谱几何特性和振幅效应。在真实数据和模拟实验中都验证了这些项的见解。

Conclusion: 该理论为图分类领域适应提供了细粒度分析框架，将泛化界分解为可解释的组件，有助于理解图分布偏移的本质并指导算法设计。
...
<details>
  <summary>Details</summary>
Motivation: 推测解码使用轻量级草稿模型生成候选token，由目标模型并行验证，加速LLM推理。传统训练最小化KL散度作为代理目标，但小型草稿模型能力有限，收敛到次优解，最小化KL散度不能保证最大化接受率。

Method: 提出LK损失函数，直接针对接受率进行优化。该方法易于实现，不增加计算开销，可直接集成到现有推测解码训练框架中。

Result: 在4种草稿架构和6个目标模型（8B到685B参数）上全面实验，在所有配置中接受指标一致改进。在通用、代码和数学领域评估，平均接受长度提升达8-10%。

Conclusion: LK损失函数是现有草稿训练目标的有力替代方案，直接优化接受率，在推测解码中实现更优性能。
...
TL;DR: 提出Hierarchical Concept Embedding Models (HiCEMs)，通过层次结构显式建模概念关系，并引入Concept Splitting方法自动从预训练CEM中发现细粒度子概念，减少标注负担，提升模型可解释性和干预能力。


<details>
  <summary>Details</summary>
Motivation: 现有概念嵌入模型(CEMs)存在两个主要问题：1) 无法表示概念间的关系；2) 训练时需要不同粒度的概念标注，限制了实际应用。需要一种能够建模概念层次关系且减少标注负担的方法。

Method: 提出Hierarchical Concept Embedding Models (HiCEMs)，通过层次结构显式建模概念关系。引入Concept Splitting方法，自动从预训练CEM的嵌入空间中发现细粒度子概念，无需额外标注。使用新提出的PseudoKitchens数据集（3D厨房渲染的概念数据集）进行评估。

Result: 1) Concept Splitting能够发现训练时未标注的人类可解释子概念，可用于训练高精度HiCEMs；2) HiCEMs支持不同粒度的测试时概念干预，显著提升任务准确率。用户研究和多数据集实验验证了方法的有效性。

Conclusion: HiCEMs通过层次化建模概念关系，结合自动概念发现方法，显著提升了概念嵌入模型的可解释性和实用性，减少了标注需求，为模型理解、调试和去偏提供了更强大的工具。
...
TL;DR: 提出基于变分推断的学习框架，用于优化掩码离散扩散模型的并行生成顺序，在GSM8K数据集上以仅4个生成步骤实现33.1%准确率，优于传统启发式方法。


<details>
  <summary>Details</summary>
Motivation: 掩码离散扩散模型（MDMs）虽然支持并行生成，但在并行生成效率与样本质量之间的平衡仍存在问题。现有方法主要依赖固定的启发式并行采样策略，从变分推断角度学习生成顺序的方法尚未充分探索。

Method: 提出变分推断框架学习MDMs的并行生成顺序，设计了一种便于并行化和高效采样的近似后验生成顺序参数化方法。

Result: 在GSM8K数据集上的初步实验显示，该方法在高度并行生成场景下表现优异：仅用平均4个生成步骤达到33.1%准确率，而标准对比方法在相同步骤数下仅获得23.7-29.0%准确率。

Conclusion: 该方法为MDMs的并行生成顺序学习提供了有效的变分推断框架，在保持高质量的同时显著提升了生成效率，为进一步研究MDMs并行生成问题提供了有价值的见解。
...
TL;DR: 提出完全内蕴的双曲神经网络ILNN，在Lorentz模型中完成所有计算，通过点对超平面的全连接层等内蕴模块，在多个数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现实数据常具有潜在的层次结构，适合用双曲几何表示。现有双曲神经网络大多部分内蕴，混合欧几里得和双曲操作，或依赖外部参数化，需要完全内蕴的架构。

Method: 提出ILNN，核心是点对超平面全连接层，用特征到学习到的Lorentz超平面的闭式双曲距离替代传统欧几里得仿射logits。设计内蕴模块：GyroLBN（Lorentz批归一化）、gyro-additive偏置、Lorentz块拼接算子、Lorentz dropout层。

Result: 在CIFAR-10/100和两个基因组基准测试（TEB和GUE）上，ILNN在双曲模型中达到SOTA性能和计算成本，并持续超越强欧几里得基线。

Conclusion: ILNN是完全内蕴的双曲神经网络架构，通过在Lorentz模型中完成所有计算，更好地尊重内在曲率，在多个任务上表现优异，代码已开源。
...
TL;DR: 提出MINT框架，將MRI生物標記知識轉移到語音編碼器，用於阿茲海默症早期篩查，無需在推理時使用神經影像


<details>
  <summary>Details</summary>
Motivation: 阿茲海默症早期篩查需要生物學基礎，但MRI成本高且部署困難。現有語音分類器缺乏神經影像的生物學基礎，在區分正常認知與輕度認知障礙時可靠性有限。

Method: 提出三階段跨模態框架MINT：1) 在1,228名受試者上訓練MRI教師模型；2) 使用殘差投影頭將語音表徵對齊到凍結的影像流形；3) 在推理時將凍結的MRI分類器應用於對齊後的嵌入，無需掃描器。

Result: 對齊後的語音達到與純語音基線相當的性能（AUC 0.720 vs 0.711），且無需推理時影像。多模態融合優於單獨MRI（0.973 vs 0.958）。dropout正則化和自監督預訓練是關鍵設計。

Conclusion: 首次展示MRI到語音的知識轉移用於阿茲海默症早期篩查，建立了無需推理時神經影像的、具有生物學基礎的群體級認知分流途徑。
...
TL;DR: 提出基础世界模型愿景，通过可学习奖励模型、自适应形式验证、在线抽象校准和测试时合成四个组件，构建支持可验证程序合成和可靠适应的智能体框架。
...
Method: 提出基础世界模型框架，包含四个核心组件：1) 从规范中学习奖励模型；2) 学习过程中集成自适应形式验证；3) 在线抽象校准量化模型预测可靠性；4) 验证器指导的测试时合成与世界模型生成。

Result: 该框架使智能体能够合成可验证程序、从少量交互中推导新策略，并在适应新颖性时保持正确性，为学习、推理和适应提供基础。
...
<details>
  <summary>Details</summary>
Motivation: 对比学习已成为现代表示学习的基石，但缺乏对InfoNCE损失如何塑造表示分布的理论理解。本研究旨在揭示对比训练中表示的高斯结构特性。

Method: 采用两种互补的理论分析框架：1) 在特定对齐和集中性假设下，证明高维表示的投影渐近趋于多元高斯分布；2) 在较弱假设下，通过添加促进低特征范数和高特征熵的正则项获得类似渐近结果。同时在合成数据和CIFAR-10数据集上进行了实验验证。

Result: 理论分析和实验结果表明，InfoNCE目标确实诱导表示呈现高斯分布特性。这一发现在不同编码器架构和规模下都保持一致性，为观察到的对比表示高斯性提供了理论解释。

Conclusion: 该研究为对比学习中常见的高斯现象提供了原则性解释，建立的高斯模型支持对学习表示的解析处理，有望推动对比学习在广泛领域的应用。
...
TL;DR: RewardUQ：一个系统评估奖励模型不确定性量化的统一框架，发现模型规模和初始化对性能影响最大，多数先前工作可能受益于替代设计选择。


<details>
  <summary>Details</summary>
Motivation: 奖励模型对于将大语言模型与人类偏好对齐至关重要，但现有方法依赖点式奖励估计，忽视了有限人类反馈带来的认知不确定性。虽然量化这种不确定性可以通过不确定性引导的主动学习减少人工标注成本，并在LLM后训练中缓解奖励过度优化，但不确定性感知的奖励模型缺乏系统比较，理解不足。

Method: 提出了RewardUQ统一框架，系统评估奖励模型的不确定性量化。比较常见方法在准确性和校准方面的标准指标，并提出结合这两个维度的新排名策略以简化比较。

Result: 实验结果表明，模型规模和初始化对性能影响最大，大多数先前工作可能受益于替代设计选择。作者发布了开源框架作为Python包，促进新方法开发和评估。

Conclusion: RewardUQ为奖励模型不确定性量化提供了系统评估框架，揭示了关键影响因素，并开源工具以支持该领域发展和下游应用部署。
...
TL;DR: pathsig是一个PyTorch原生库，用于高效计算路径签名，支持GPU并行计算，相比现有库有10-30倍速度提升


<details>
  <summary>Details</summary>
Motivation: 现有路径签名库缺乏大规模梯度学习所需的可扩展性，需要更高效的实现来支持深度学习应用

Method: 使用CUDA内核在word basis上并行更新签名系数，支持用户指定的单词集投影和各向异性截断

Result: 相比其他库，pathsig在截断签名计算上实现10-30倍加速，在需要反向传播的训练中实现4-10倍加速

Conclusion: pathsig提供了高效、可扩展的路径签名计算，支持更紧凑的表示，减少维度冗余和计算成本
...
TL;DR: COVE是一种可解释的高维嵌入方法，通过非线性降维技术移除节点嵌入的低维约束，使用UMAP降维后在聚类和链接预测任务上性能略有提升。


<details>
  <summary>Details</summary>
Motivation: 现有节点嵌入方法通常受限于低维约束，作者希望开发一种可解释的高维嵌入方法，能够更好地捕捉节点间的相似性关系。

Method: COVE基于随机游走中的共现作为相似性指标，与扩散过程密切相关。使用非线性降维技术（UMAP）将高维嵌入降维，然后通过HDBSCAN进行聚类。

Result: COVE UMAP HDBSCAN流程在社区检测基准测试中表现与流行的Louvain算法相当，在聚类和链接预测任务上性能略有提升。

Conclusion: COVE提供了一种可解释的高维节点嵌入方法，通过非线性降维技术可以取得与现有流行算法相当的性能，为网络分析提供了新的工具。
...
TL;DR: ACWI提出自适应相关加权内在奖励框架，通过状态依赖的Beta网络动态平衡内在和外在奖励，提升稀疏奖励环境中的探索效率。


<details>
  <summary>Details</summary>
Motivation: 传统方法使用手动调整的标量系数平衡内在和外在奖励，在不同任务中表现不稳定或次优，需要自适应机制来动态调整探索激励。

Method: 引入轻量级Beta网络，通过编码器架构从智能体状态预测内在奖励权重；使用基于相关的目标函数优化缩放机制，使加权内在奖励与折扣未来外在回报对齐。

Result: 在MiniGrid稀疏奖励环境中，ACWI相比固定内在奖励基线，显著提高了样本效率和学习稳定性，性能更优且计算开销最小。

Conclusion: ACWI通过自适应相关加权机制有效解决了稀疏奖励强化学习中的探索问题，实现了任务自适应探索激励，同时保持计算效率和训练稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 传统Cox过程的非参数强度估计和后验推理通常依赖昂贵的MCMC方法，计算效率低下，需要更高效的推理方法

Method: 基于滤波扩张理论，提出神经扩散强度模型，使用神经SDE驱动Cox过程，设计摊销编码器架构将变长事件序列映射到后验强度路径
...
TL;DR: 将智能体AI控制器引入O-RAN架构，通过规划-行动-观察-反思等智能体原语，提升网络切片生命周期和无线资源管理性能，相比传统方法平均减少8.83%资源使用。


<details>
  <summary>Details</summary>
Motivation: O-RAN架构虽然提供了丰富的控制和遥测接口，但在多租户、多目标场景下难以安全、可审计地运营。同时，具有明确规划、工具使用、记忆和自我管理能力的智能体AI系统为构建长期控制循环提供了自然方式。

Method: 提出将智能体控制器引入O-RAN架构，对比传统ML/RL xApps，围绕三个任务集群组织任务：网络切片生命周期、无线资源管理(RRM)闭环、以及跨领域的安全隐私合规。引入智能体原语（规划-行动-观察-反思、技能作为工具使用、记忆与证据、自我管理门控），并在多小区O-RAN仿真中验证。

Result: 在O-RAN多小区仿真中，智能体控制器相比传统基线和去除单个原语的消融实验，显著提升了切片生命周期和RRM性能。该框架在三个经典网络切片上平均减少了8.83%的资源使用。

Conclusion: 智能体控制器为O-RAN提供了有前景的架构方案，能够改善多租户、多目标RAN的运营安全性和可审计性。安全、隐私和合规性作为架构约束和开放挑战，需要标准对齐部署的进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 在许多应用中，评估不同替代方案的效果伴随着不同的成本或资源消耗。受这种异质性启发，研究资源约束下的最佳臂识别问题，其中每个臂拉动消耗一种或多种有限资源类型。

Method: 提出Successive Halving with Resource Rationing (SH-RR)算法，将资源感知分配集成到经典的成功减半框架中，通过新的有效消耗度量统一随机和确定性消耗设置的理论分析。

Result: SH-RR算法在资源约束下有效识别最佳臂，为随机和确定性资源消耗提供统一的理论分析框架。

Conclusion: SH-RR算法是解决资源约束下最佳臂识别问题的有效方法，通过资源感知分配和新的有效消耗度量统一了不同资源消耗模式的理论分析。
...
TL;DR: 提出一种针对离散令牌输入（如文本）的可解释AI方法，通过训练解释器网络生成掩码来隐藏与分类无关的信息，保持嵌入向量方向不变但改变幅度，从而产生人类可读的解释。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏针对离散令牌输入（如文本）的易于理解的可解释AI方法。现有的可解释AI技术难以扩展到令牌序列，因为最先进的模型（如Transformer）倾向于关注全局连接，导致现有方法要么识别出离散的重要令牌，要么给大量令牌分配低重要性值。

Method: 该方法将基于掩码的图像可解释AI算法推广到基于令牌的分类器。首先训练一个解释器神经网络来创建掩码，隐藏与分类无关的信息。然后将掩码与分类器嵌入层的连续值进行哈达玛积，再通过分类器，改变嵌入向量的幅度但保持方向不变。该方法关注令牌作为一个整体（即输入序列的一个片段）的重要性。

Result: 该方法在核苷酸序列的分类器上进行了训练，结果表明被掩码的片段比未掩码的片段与分类的相关性更低。该方法能够产生人类可读的解释，关注令牌作为整体的重要性。

Conclusion: 提出了一种针对离散令牌输入的可解释AI方法，能够有效识别与分类相关的令牌片段，产生人类可读的解释，解决了现有方法在处理令牌序列时的问题。
...
TL;DR: 本文提出了一种构造低阶三明治多项式的新方法，显著改进了多个基本函数类和边际分布的三明治多项式阶数界限，特别是将k个半空间函数的阶数从指数级改进为多项式级。


<details>
  <summary>Details</summary>
Motivation: 低阶三明治多项式近似器在分布偏移学习、可测试学习和污染学习等挑战性学习场景中显示出强大能力。三明治多项式既能近似目标函数的期望值，又能提供函数值的逐点上界和下界。然而，现有方法在某些函数类上的阶数界限不够理想，需要改进。

Method: 提出了一种新的构造低阶三明治多项式的方法，该方法直接利用目标函数边界的光滑性来构造三明治Lipschitz函数，然后应用高维逼近理论的结果。与之前使用FT-mollification方法的工作不同，新方法更加简单直接。

Result: 获得了多项重要改进：1) 对于高斯分布下的k个半空间函数，将三明治多项式的阶数从2^O(k)改进为poly(k)，实现了指数级改进；2) 对于低维多项式阈值函数，获得了双指数级改进；3) 方法适用于具有光滑边界的低维函数类。

Conclusion: 本文提出的新方法显著改进了低阶三明治多项式的构造，为多个基本函数类提供了更优的阶数界限。该方法相对简单，直接利用函数边界的光滑性，避免了复杂的FT-mollification技术，为后续研究提供了新的思路。
...
TL;DR: 本文提出一种基于多目标强化学习的方法来优化集装箱式履约中心的整合流程，通过零和博弈框架处理约束条件，在模拟环境中实现了多目标权衡并满足所有操作约束。


<details>
  <summary>Details</summary>
Motivation: 集装箱式履约中心的整合流程需要权衡处理速度、资源使用和空间利用率等多个竞争性目标，同时满足现实操作约束。传统方法难以处理这种高维状态空间和动态系统行为的大规模优化问题。

Method: 将问题建模为大规模多目标强化学习任务，利用零和博弈中的最优响应和无悔动态理论解决约束RL问题，采用最小最大策略学习方法，并引入处理误差消除问题的理论框架。

Result: 在现实仓库模拟中的策略评估显示，该方法能有效权衡各目标，学习到的单一策略能同时满足所有约束条件。理论框架能处理时间平均解的振荡行为，返回接近博弈最小最大值的拉格朗日值。

Conclusion: 多目标强化学习在解决大规模工业系统中复杂、高影响力的决策问题方面具有巨大潜力，特别是在处理多目标权衡和操作约束方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 密度比估计是概率建模的核心问题，但现有方法如标准化流需要分别计算两个分布的似然积分，计算成本高昂

Method: 利用条件感知流匹配技术，推导出单一动力学公式来追踪沿生成轨迹的密度比，避免分别计算两个分布的似然

Result: 在模拟基准测试中表现出竞争力，并在单细胞基因组学数据分析中支持多种任务，包括处理效应估计和批次校正评估

Conclusion: 提出的条件感知流匹配方法为密度比估计提供了高效的计算框架，特别适用于需要跨条件比较似然的生物信息学应用
...
TL;DR: 本文提出了一种无条件的归约方法，证明在表演性预测场景中，任何无遗憾算法都会收敛到表演性稳定均衡，无需对模型如何影响数据分布做任何限制性假设。


<details>
  <summary>Details</summary>
Motivation: 算法预测在决策中的应用会导致反馈循环：部署的模型会影响数据分布，而这些数据又被用于重新训练模型。Perdomo等人2020年将这种动态形式化为表演性预测。现有研究需要强假设限制模型如何影响分布，本文旨在避免这些限制。

Method: 使用鞅论论证和允许随机化，提出无条件归约方法，将表演性预测问题与在线优化联系起来。该方法不要求对模型影响数据分布的方式做任何限制性假设。

Result: 证明任何无遗憾算法在表演性设置中都会收敛到（混合）表演性稳定均衡：在这种均衡中，模型主动塑造数据分布，使其预测在后验中看起来最优。该方法绕过了寻找稳定模型的近期硬度结果。

Conclusion: 该连接揭示了为什么常见算法（如梯度下降）天然具有稳定性并能防止失控的反馈循环。这项工作有望促进在线优化与表演性预测之间的技术思想交流。
...
TL;DR: 提出一个高效的無監督聯邦學習框架，通過利用兩個不同物聯網數據集的共享特徵來增強異常檢測，同時保留數據集特定特徵，並使用可解釋AI技術提高透明度。


<details>
  <summary>Details</summary>
Motivation: 物聯網環境中的聯邦學習面臨數據異質性挑戰，包括設備能力、數據格式和通信約束的差異，這影響了全局模型性能和隱私保護。在物聯網異常檢測中，無監督聯邦學習雖然有潛力，但特徵異質性阻礙了有效實施。
...
Conclusion: 這項工作強調了利用互補數據集的共享特徵來優化無監督聯邦學習的潛力，可以在分散式物聯網環境中實現優異的異常檢測結果。
...
TL;DR: 量子机器学习在XOR问题上与传统模型对比：逻辑回归和深度1量子电路无法可靠表示XOR，而多层感知机和深度2量子电路在代表性条件下能达到完美测试准确率，但经典模型在训练时间和损失函数上表现更好。


<details>
  <summary>Details</summary>
Motivation: 研究量子机器学习在经典XOR问题上的表现，比较量子模型与传统机器学习模型的表达能力、性能和效率，探索量子计算在机器学习任务中的实际优势。

Method: 在合成XOR数据集上评估逻辑回归、单隐藏层多层感知机、以及深度为1和2的双量子比特变分量子分类器，使用准确率和二元交叉熵作为评估指标，分析不同高斯噪声水平和样本大小下的性能。

Result: 模型表达能力是性能的关键决定因素：逻辑回归和深度1量子电路无法可靠表示XOR，而多层感知机和深度2量子电路在代表性条件下能达到完美测试准确率。尽管准确率相当，但多层感知机获得更低的二元交叉熵和显著更短的训练时间。硬件执行保留了全局XOR结构但引入了决策函数的结构性偏差。

Conclusion: 在低维XOR基准测试中，更深的变分量子分类器可以在准确率上匹配经典神经网络，但在所考察的设置中未观察到量子模型在鲁棒性或效率方面的明显实证优势。
...
Method: 通过Pareto最优性形式化这一权衡，针对全老虎机反馈和半老虎机反馈两种信息结构，分别提出MixCombKL和MixCombUCB算法

Result: 两种算法都是Pareto最优的，在遗憾和臂差距估计误差方面都有有限时间保证，更丰富的反馈显著收紧可达到的Pareto前沿

Conclusion: 建立了一个原则性的自适应组合实验框架，用于多目标决策，揭示了反馈结构对权衡边界的重要影响
...
TL;DR: Chronos-2时间序列基础模型在零样本设置下，在10个交通数据集上实现了SOTA或具有竞争力的预测性能，无需特定训练即可提供有用的不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 现有交通预测方法通常需要针对特定数据集进行训练、架构设计和超参数调优，这限制了其通用性和部署效率。研究旨在评估通用时间序列基础模型能否作为交通预测任务的零样本解决方案。

Method: 使用最先进的Chronos-2模型，在10个真实世界交通数据集上进行零样本性能基准测试，涵盖高速公路交通量/流量、城市交通速度、共享单车需求和电动汽车充电站数据，采用一致的评估协议。

Result: Chronos-2在大多数数据集上实现了SOTA或具有竞争力的准确性，特别是在较长预测范围内经常优于经典统计基线和专门设计的深度学习架构。其原生概率输出也提供了有用的不确定性量化。

Conclusion: 时间序列基础模型可以作为交通预测研究的关键基线，提供无需特定训练的高性能预测和不确定性量化能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统RNN-T模型在语音识别和翻译任务中存在的效率问题和严格单调对齐限制，特别是语音翻译任务中RNN-T的严格单调对齐会损害性能

Method: 提出分块注意力转换器（CHAT），在固定大小的音频块内进行处理，并在每个块内使用跨注意力机制，这种混合方法保持了RNN-T的流式能力，同时引入了局部对齐建模的灵活性

Result: 显著提升效率：训练内存峰值降低46.2%，训练速度提升1.36倍，推理速度提升1.69倍；同时提升准确率：语音识别相对WER降低6.3%，语音翻译BLEU提升18.0%

Conclusion: CHAT模型为部署更强大的流式语音模型提供了实用解决方案，在不牺牲实时约束的情况下实现了效率和准确率的双重提升，特别在语音翻译任务中表现出色
...
<details>
  <summary>Details</summary>
Motivation: 计算病理学中，组织病理学染色协议、扫描仪和采集流程的技术差异导致的批次效应阻碍了模型的跨批次泛化，限制了在不同临床站点的可靠部署。

Method: 提出Latent Manifold Compaction (LMC)框架，通过显式压缩染色诱导的潜在流形，从单个源数据集学习批量不变嵌入，实现无监督图像协调，并能泛化到训练期间未见的目标域数据。

Result: 在三个具有挑战性的公共和内部基准测试中，LMC显著减少了多个数据集间的批次诱导分离，在跨批次分类和检测任务中持续优于最先进的归一化方法，实现了卓越的泛化性能。

Conclusion: LMC通过压缩潜在流形学习批量不变表示，有效解决了计算病理学中的批次效应问题，为跨临床站点的模型部署提供了可靠解决方案。
...
TL;DR: 提出W→K→W管道，通过迭代爬取、知识图谱构建和引导式爬取来发现中小企业供应商，并引入生态学启发的覆盖率估计框架评估发现完整性。


<details>
  <summary>Details</summary>
Motivation: 现有商业数据库对中小企业供应商（特别是次级供应商和新兴利基市场企业）覆盖不足，影响供应链韧性，需要更有效的方法来识别专业行业中的中小企业。

Method: W→K→W管道：1) 爬取领域特定网页发现候选供应商；2) 提取结构化知识构建异质知识图谱；3) 利用知识图谱拓扑和覆盖率信号指导后续爬取。引入基于生态学物种丰富度估计器（Chao1、ACE）的覆盖率估计框架。

Result: 在半导体设备制造行业（NAICS 333242）实验中，W→K→W管道在相同213页爬取预算下获得最高精度（0.138）和F1分数（0.118），构建了765个实体和586个关系的知识图谱，仅用112页在第3次迭代达到峰值召回率。

Conclusion: W→K→W管道能有效发现中小企业供应商，通过迭代知识图谱引导的爬取策略提高发现效率，生态学启发的覆盖率估计为评估网络实体发现完整性提供了新方法。
...
TL;DR: 提出了一种通过结构化剪枝来发现神经网络中因果抽象的新方法，将训练好的网络视为确定性SCM，推导出干预风险目标，通过二阶展开得到闭式剪枝准则。


<details>
  <summary>Details</summary>
Motivation: 神经网络被认为实现了可解释的因果机制，但验证这一点需要找到因果抽象——一个在干预下与网络保持一致的简化高层SCM。传统方法需要暴力干预或重新训练，效率低下。

Method: 将结构化剪枝重新定义为近似抽象的搜索问题，将训练好的网络视为确定性SCM，推导出干预风险目标，通过二阶展开得到闭式准则来替换单元为常数或合并到相邻单元。

Result: 在均匀曲率条件下，得分简化为激活方差，恢复了基于方差的剪枝作为特例，同时阐明了其失败情况。该方法能高效地从预训练网络中提取稀疏、干预忠实的抽象。

Conclusion: 通过结构化剪枝框架成功发现了神经网络中的因果抽象，提供了一种高效提取干预忠实抽象的方法，并通过干预实验验证了有效性。
...
TL;DR: 现有表示学习可识别性评估指标存在假设依赖问题，当数据生成过程或编码器几何假设不满足时，会产生系统性误判


<details>
  <summary>Details</summary>
Motivation: 当前表示学习可识别性评估使用标准指标（如MCC、DCI、R²）在已知真实因子的合成基准上进行，这些指标被假定能反映理论保证的等价类恢复程度。然而这种假设只在特定结构条件下成立，当假设被违反时，指标会产生误判

Method: 提出一个分类法，将数据生成过程假设与编码器几何假设分离，用于表征现有指标的有效域，并发布一个可重复的压力测试和比较评估套件

Result: 揭示了每个指标都隐式编码了对数据生成过程和编码器的特定假设，当这些假设被违反时，指标会变得错误指定，产生系统性假阳性和假阴性，这种情况在经典可识别性机制和事后设置中都会发生

Conclusion: 表示学习可识别性评估需要更严谨的指标设计，考虑数据生成过程和编码器几何的匹配性，提出的分类法和评估套件有助于更可靠地评估表示学习方法的可识别性
...
TL;DR: 本文提出Memory Caching技术，通过缓存RNN的记忆状态检查点来增强循环模型的记忆容量，使其能够在固定记忆和增长记忆之间灵活权衡，从而提升在召回密集型任务中的性能。
...
Method: 提出Memory Caching技术，通过缓存RNN隐藏状态的检查点来扩展有效记忆容量。设计了四种变体，包括门控聚合和稀疏选择机制，并讨论了在线性和深度记忆模块中的应用。

Result: 在语言建模和长上下文理解任务中，MC显著提升了循环模型的性能。在上下文召回任务中，MC变体表现出与Transformer竞争的性能，缩小了与Transformer的差距，并优于现有的循环模型。

Conclusion: Memory Caching是一种简单有效的技术，能够增强循环模型的记忆容量，在保持次二次复杂度的同时提升在召回密集型任务中的性能，为循环模型和Transformer之间提供了灵活的权衡方案。
...
TL;DR: LoRA-Pre：一种基于低秩分解的新型优化器，通过将动量矩阵分解为紧凑的低秩子空间，显著降低内存开销，在预训练和微调中均优于现有方法。
...
Conclusion: LoRA-Pre通过低秩分解有效解决了优化器内存开销问题，在保持优化性能的同时显著提升内存效率，适用于预训练和微调两种范式。
...
TL;DR: CUDA Agent：通过大规模智能体强化学习系统提升LLM的CUDA内核优化能力，在KernelBench上超越torch.compile和最强专有模型


<details>
  <summary>Details</summary>
Motivation: GPU内核优化对深度学习至关重要但需要专业硬件知识。现有LLM在CUDA代码生成方面不如编译器系统（如torch.compile），现有方法无法从根本上提升模型的CUDA优化能力，性能提升有限。

Method: 提出CUDA Agent系统，包含三个组件：可扩展的数据合成流水线、技能增强的CUDA开发环境（带自动验证和分析以提供可靠奖励信号）、以及实现稳定训练的强化学习算法技术。
...
Conclusion: CUDA Agent通过大规模智能体强化学习系统成功提升了LLM的CUDA内核优化能力，超越了现有编译器系统和专有模型，为解决GPU内核优化这一专业任务提供了有效方案。
...
### [162] [HumanMCP: A Human-Like Query Dataset for Evaluating MCP Tool Retrieval Performance](https://arxiv.org/abs/2602.23367)
*Shubh Laddha,Lucas Changbencharoen,Win Kuptivej,Surya Shringla,Archana Vaidheeswaran,Yash Bhaskar*

Main category: cs.AI

TL;DR: 本文提出了首个大规模MCP数据集，包含针对308个MCP服务器中2800个工具生成的多样化、高质量用户查询，解决了现有数据集缺乏真实用户查询模式的问题。


<details>
  <summary>Details</summary>
Motivation: 现有MCP服务器数据集和基准测试缺乏真实、类似人类的用户查询，导致评估工具使用和生态系统时存在关键差距。现有数据集通常包含工具描述，但未能体现不同用户如何表达请求，导致泛化能力差和某些基准测试的可靠性被夸大。

Method: 基于MCP Zero数据集开发，为308个MCP服务器中的2800个工具生成多样化、高质量的用户查询。每个工具都配对了多个独特的用户角色，捕捉从精确任务请求到模糊、探索性命令的不同用户意图层次，反映真实世界交互模式的复杂性。

Result: 创建了首个大规模MCP数据集，包含针对2800个工具生成的多样化用户查询，覆盖308个MCP服务器。数据集通过多个用户角色模拟真实用户表达方式，提高了评估的真实性和泛化能力。

Conclusion: 该研究填补了MCP服务器评估的关键空白，通过引入包含多样化用户查询的大规模数据集，能够更准确地评估工具使用和生态系统，为未来研究提供了更真实的基准测试基础。
...
### [163] [An Agentic LLM Framework for Adverse Media Screening in AML Compliance](https://arxiv.org/abs/2602.23373)
*Pavel Chernakov,Sasan Jafarnejad,Raphaël Frank*

Main category: cs.AI

TL;DR: 提出一个基于LLM和RAG的智能体系统，用于自动化负面媒体筛查，通过多步骤流程计算风险评分，显著降低误报率。


<details>
  <summary>Details</summary>
Motivation: 传统负面媒体筛查依赖关键词搜索，导致高误报率且需要大量人工审核，金融合规流程效率低下。

Method: 采用LLM智能体结合RAG的多步骤方法：搜索网络、检索处理相关文档、计算负面媒体指数(AMI)评分。

Result: 在包含PEP、监管名单、制裁人员和干净姓名的数据集上评估，系统能有效区分高风险和低风险个体。

Conclusion: 基于LLM的智能体系统能自动化负面媒体筛查，提高AML/KYC合规效率，降低误报率和人工成本。
...
### [164] [Causal Identification from Counterfactual Data: Completeness and Bounding Results](https://arxiv.org/abs/2602.23541)
*Arvind Raghavan,Elias Bareinboim*

Main category: cs.AI

TL;DR: 提出CTFIDU+算法，用于从任意Layer 3分布中识别反事实查询，证明其完备性，并建立非参数设置下精确因果推理的基本极限。


<details>
  <summary>Details</summary>
Motivation: 先前关于反事实识别完备性的研究局限于观测或干预分布（Pearl因果层次的第1、2层），因为一般认为无法获得第3层的反事实分布数据。但最近研究发现某些反事实分布可以通过实验方法直接估计（反事实可实现性），这引发了一个新问题：在能够访问某些第3层数据的情况下，哪些额外的反事实量变得可识别？

Method: 开发CTFIDU+算法，用于从任意第3层分布集合中识别反事实查询，并证明该算法对此任务是完备的。基于此，建立从物理可实现分布中识别反事实的理论极限。对于不可识别的关键反事实类型，推导使用可实现反事实数据的新分析界限。

Result: CTFIDU+算法被证明是完备的，能够识别从任意Layer 3分布集合中可识别的所有反事实查询。研究确定了非参数设置下精确因果推理的基本极限。通过模拟验证，反事实数据在实践中确实有助于收紧不可识别量的界限。

Conclusion: 该研究填补了反事实识别理论的重要空白，建立了从可实现反事实数据中进行因果推理的完备算法和理论极限。对于不可识别的反事实量，提供了使用反事实数据收紧界限的新方法，为实际因果推理提供了更精确的工具。
...
### [165] [Planning under Distribution Shifts with Causal POMDPs](https://arxiv.org/abs/2602.23545)
*Matteo Ceriscioli,Karthika Mohan*

Main category: cs.AI

TL;DR: 提出一个基于因果知识的POMDP框架，用于处理分布偏移下的规划问题，通过将环境变化表示为因果干预，保持值函数的PWLC特性，从而维持规划的可处理性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的规划常常面临分布偏移的挑战，环境模型在状态分布或环境动态变化时可能失效，导致先前学习的策略失败。需要一种能够处理环境变化的规划框架。

Method: 使用因果知识构建部分可观测马尔可夫决策过程（POMDP），将环境变化表示为对该因果POMDP的干预。框架能够评估假设变化下的计划，并主动识别环境中的哪些组件发生了变化。通过维护和更新对潜在状态和底层领域的信念，证明值函数在增强的信念空间中保持分段线性凸（PWLC）特性。

Result: 证明了值函数在增强的信念空间中保持PWLC特性，这一特性使得基于α向量的POMDP方法在分布偏移下仍保持可处理性，为处理环境变化的规划问题提供了理论保证。

Conclusion: 提出的因果POMDP框架为处理分布偏移下的规划问题提供了理论基础，通过将环境变化建模为因果干预，保持了规划方法的可处理性，为解决现实世界中环境动态变化带来的规划挑战提供了新思路。
...
### [166] [Construct, Merge, Solve & Adapt with Reinforcement Learning for the min-max Multiple Traveling Salesman Problem](https://arxiv.org/abs/2602.23579)
*Guillem Rodríguez-Corominas,Maria J. Blesa,Christian Blum*

Main category: cs.AI

TL;DR: 提出RL-CMSA混合方法解决单仓库最小最大mTSP问题，结合强化学习引导的构造、精确优化和自适应池管理，在平衡工作负载方面优于现有算法。


<details>
  <summary>Details</summary>
Motivation: 解决多旅行商问题中的最小最大变体，旨在最小化最长路径以实现工作负载平衡，需要有效算法来处理大规模实例和多个销售员的情况。

Method: 提出RL-CMSA混合方法：1) 使用学习到的成对q值进行概率聚类构造多样化解；2) 合并路径到紧凑池；3) 求解受限集合覆盖MILP；4) 通过路径间移除、移位和交换操作精化解；5) 通过强化高质量解中的城市对共现更新q值；6) 通过老化和修剪自适应管理池。

Result: 在随机和TSPLIB实例上的计算结果表明，RL-CMSA始终能找到(接近)最优解，在可比时间限制下优于最先进的混合遗传算法，尤其当实例规模和销售员数量增加时表现更佳。

Conclusion: RL-CMSA通过结合精确优化和强化学习引导的构造，在探索和利用之间取得良好平衡，为对称单仓库最小最大mTSP问题提供了有效的解决方案。
...
### [167] [SleepLM: Natural-Language Intelligence for Human Sleep](https://arxiv.org/abs/2602.23605)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于学习的睡眠分析系统局限于预定义的标签空间（如睡眠阶段或事件），无法描述、查询或泛化到新的睡眠现象。需要一种能够连接自然语言和多模态睡眠生理数据的模型。

Method: 提出多级睡眠描述生成流程，构建首个大规模睡眠-文本数据集（10万+小时，1万+个体）；采用统一预训练目标，结合对比对齐、描述生成和信号重建，捕捉生理保真度和跨模态交互。

Result: SleepLM在真实世界睡眠理解任务中超越现有方法，在零样本/少样本学习、跨模态检索和睡眠描述生成方面表现优异；展现出语言引导事件定位、针对性洞察生成和零样本泛化到未见任务的能力。

Conclusion: SleepLM成功建立了自然语言与睡眠生理数据的桥梁，为睡眠分析提供了更灵活、可解释的框架，所有代码和数据将开源。
...
### [168] [MMKG-RDS: Reasoning Data Synthesis via Deep Mining of Multimodal Knowledge Graphs](https://arxiv.org/abs/2602.23632)
...
Result: 在Qwen3模型（0.6B/8B/32B）上微调少量合成样本，推理准确率提升9.2%；框架能生成独特数据，在涉及表格和公式的任务上挑战现有模型，适用于复杂基准构建。

Conclusion: MMKG-RDS为推理数据合成提供了灵活有效的解决方案，通过多模态知识图谱实现了高质量训练数据生成，显著提升了领域模型的推理能力，并为复杂基准构建提供了支持。
...
### [169] [AI Must Embrace Specialization via Superhuman Adaptable Intelligence](https://arxiv.org/abs/2602.23643)
*Judah Goldfeder,Philippe Wyder,Yann LeCun,Ravid Shwartz Ziv*

Main category: cs.AI

TL;DR: 本文批判了当前对通用人工智能（AGI）的定义，认为人类本身并非真正"通用"，提出应追求专业化而非通用性的AI，并引入"超人适应智能（SAI）"概念，强调AI应在重要任务上超越人类并填补人类能力空白。


<details>
  <summary>Details</summary>
Motivation: 当前AI界对AGI的定义混乱且不一致，许多人将AGI定义为"能做人类能做的一切"，但这种定义存在根本问题。作者认为需要重新审视AI的发展方向，澄清讨论框架，为AI的未来发展提供更合理的指导。

Method: 通过分析现有AGI定义的缺陷，论证人类本身并非真正"通用"智能体，提出用"超人适应智能（SAI）"替代AGI概念。SAI定义为：能够学习在人类能做的任何重要事情上超越人类，并能填补人类能力空白的智能。

Result: 提出了SAI作为替代AGI的新概念框架，澄清了AI讨论中的模糊性，为AI发展方向提供了更精确的指导原则：专业化而非通用性，追求超人类性能，填补人类能力空白。

Conclusion: AGI是一个有缺陷的概念，AI发展应专注于专业化而非通用性。SAI提供了更合理的框架来指导AI未来发展，强调在重要任务上超越人类能力并填补人类技能空白，这比追求"通用性"更有意义。
...
### [170] [PseudoAct: Leveraging Pseudocode Synthesis for Flexible Planning and Action Control in Large Language Model Agents](https://arxiv.org/abs/2602.23668)
*Yihan,Wen,Xin Chen*

Main category: cs.AI

TL;DR: PseudoAct：通过伪代码合成实现LLM智能体的灵活规划与行动控制，显著提升长时程任务的执行效率和成功率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体（如ReAct）采用反应式决策模式，在复杂长时程任务中容易出现冗余工具使用、推理不稳定和高token消耗等问题，需要更结构化的规划方法。

Method: 提出PseudoAct框架，利用LLM将任务解决策略表达为代码的能力，合成结构化伪代码计划，将任务分解为子任务并显式编码控制流（序列、条件、循环、并行组合等），然后按此全局计划执行行动。

Result: 在基准数据集上的实验表明，该方法显著优于现有反应式智能体方法，在FEVER上实现20.93%的绝对成功率提升，在HotpotQA上创下新的最先进水平。

Conclusion: PseudoAct通过伪代码合成实现了显式和时序一致的决策逻辑，减少了冗余行动、防止无限循环、避免无信息替代探索，实现了持续高效的长时程决策。
...
### [171] [ODAR: Principled Adaptive Routing for LLM Reasoning via Active Inference](https://arxiv.org/abs/2602.23681)
*Siyuan Ma,Bo Gao,Xiaojun Jia,Simeng Qin,Tianlin Li,Ke Ma,Xiaoshuang Jia,Wenqi Ren,Yang Liu*

Main category: cs.AI

TL;DR: ODAR-Expert：基于自由能原理的自适应路由框架，通过难度估计动态分配计算资源，在提高推理准确率的同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型推理范式从参数扩展转向测试时计算扩展，但现有方法多采用均匀暴力采样（如固定best-of-N或自洽性），存在成本高、难以归因、可能触发过度思考且收益递减的问题。

Method: 提出ODAR-Expert自适应路由框架：1）基于摊销主动推理的难度估计器动态路由查询到快速启发式代理或慢速审慎代理；2）引入基于自由能原理的风险敏感融合机制，通过最小化变分自由能目标选择答案，平衡对数似然与认知不确定性。

Result: 在23个基准测试中表现优异：MATH达到98.2%准确率，Humanity's Last Exam达到54.8%；在计算匹配设置下改进计算-准确率边界；在完全开源堆栈上验证可复现性，超越同质采样策略同时减少82%计算成本。

Conclusion: 思维最优扩展需要自适应资源分配和基于自由能的决策机制，而非简单地增加测试时计算量。
...
### [172] [From Flat Logs to Causal Graphs: Hierarchical Failure Attribution for LLM-based Multi-Agent Systems](https://arxiv.org/abs/2602.23701)
*Yawen Wang,Wenjie Wu,Junjie Wang,Qing Wang*

Main category: cs.AI

TL;DR: CHIEF是一个新颖的LLM多智能体系统故障归因框架，通过层次因果图、虚拟预言机引导回溯和渐进因果筛选，显著提升故障归因准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM多智能体系统虽然能力强大，但存在脆弱性和故障机制不透明的问题。现有故障归因方法将执行日志视为扁平序列，无法解耦多智能体系统中复杂的因果联系，导致可观测性弱和责任边界模糊。

Method: 提出CHIEF框架：1) 将混乱的执行轨迹转化为结构化层次因果图；2) 使用层次预言机引导回溯，通过合成虚拟预言机高效剪枝搜索空间；3) 通过渐进因果筛选策略实现反事实归因，严格区分真正的根本原因和传播的症状。

Result: 在Who&When基准测试中，CHIEF在智能体级和步骤级准确率上都优于八个强大的基线方法。消融研究进一步证实了每个提出模块的关键作用。

Conclusion: CHIEF通过结构化层次因果表示和高效的反事实分析，显著提升了多智能体系统的故障归因能力，为解决LLM多智能体系统的脆弱性和不透明性问题提供了有效方案。
...
### [173] [ProductResearch: Training E-Commerce Deep Research Agents via Multi-Agent Synthetic Trajectory Distillation](https://arxiv.org/abs/2602.23716)
*Jiangyuan Wang,Kejun Xiao,Huaipeng Zhao,Tao Luo,Xiaoyi Zeng*

Main category: cs.AI

TL;DR: 提出ProductResearch多智能体框架，通过合成高质量工具使用轨迹来训练电商购物助手，显著提升响应全面性、研究深度和用户感知效用。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的电商对话购物助手缺乏复杂产品研究所需的交互深度和上下文广度，而深度研究范式在迁移到电商领域时存在领域差距。

Method: 采用多智能体框架：用户智能体从行为历史推断购物意图，监督智能体协调研究智能体迭代协作生成合成轨迹，通过反思内化过程将多智能体交互整合为单角色训练样本。

Result: 在合成数据上微调的紧凑MoE模型相比基础模型在响应全面性、研究深度和用户感知效用方面有显著提升，接近前沿专有深度研究系统性能。

Conclusion: 多智能体合成轨迹训练是增强基于LLM的购物助手的有效且可扩展范式，能够生成高质量训练数据并提升模型在复杂购物查询中的表现。
...
### [174] [The Auton Agentic AI Framework](https://arxiv.org/abs/2602.23720)
*Sheng Cao,Zhao Chang,Chang Li,Hannan Li,Liyao Fu,Ji Tang*

Main category: cs.AI

TL;DR: Auton Agentic AI Framework：一个用于标准化自主智能体创建、执行和治理的架构，通过分离认知蓝图与运行时引擎解决LLM随机输出与后端确定性需求之间的不匹配问题。


<details>
  <summary>Details</summary>
Motivation: AI正从生成式AI向代理式AI过渡，但LLM的随机非结构化输出与后端基础设施（数据库、API、云服务）所需的确定性、模式一致输入之间存在根本性架构不匹配。需要一种标准化架构来创建、执行和管理自主代理系统。

Method: 提出Auton Agentic AI Framework，核心是认知蓝图（声明式、语言无关的代理身份和能力规范）与运行时引擎（平台特定执行基板）的严格分离。采用增强的POMDP模型、分层记忆整合架构、约束流形形式化安全执行、三级自进化框架，以及并行图执行、推测推理、动态上下文修剪等运行时优化。

Result: 该框架实现了跨语言可移植性、形式化可审计性和通过模型上下文协议（MCP）的模块化工具集成。提供了完整的代理执行模型、安全执行机制和性能优化方案，能够减少多步骤代理工作流的端到端延迟。

Conclusion: Auton Agentic AI Framework为自主代理系统提供了一个原则性架构，解决了生成式AI向代理式AI过渡中的核心架构不匹配问题，通过认知蓝图与运行时引擎的分离实现了标准化、可移植性和安全性，为大规模代理部署奠定了基础。
...
### [175] [Unlocking Cognitive Capabilities and Analyzing the Perception-Logic Trade-off](https://arxiv.org/abs/2602.23730)
...
TL;DR: MERaLiON2-Omni (Alpha) 是一个10B参数的多语言全感知模型，专为东南亚地区设计，通过分离和整合感知与推理能力，解决区域特定多模态理解问题，但发现推理能力会引入感知不稳定性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型追求全感知能力，但将稳健的感官基础与复杂推理相结合仍然具有挑战性，特别是在东南亚等代表性不足的地区。需要开发能够理解区域特定音频-视觉线索（如新加坡式英语语码转换、当地文化地标）的多语言模型。

Method: 采用渐进式训练流程：1）通过正交模态适应建立稳健的感知骨干，将区域特定音频-视觉线索与多语言LLM对齐；2）提出成本效益高的生成-判断-精炼流程，利用超级LLM过滤幻觉并通过共识机制解决冲突，合成高质量银数据，将文本链式思维推理转移到多模态场景。

Result: 在SEA-Omni基准套件上的评估揭示了效率-稳定性悖论：推理能力显著提升抽象任务（数学和指令跟随）性能，但在低层次感官处理中引入不稳定性，包括长上下文音频中的时间漂移（推理使模型与声学时间戳不同步）和视觉过度解释（逻辑覆盖像素级现实）。

Conclusion: 该研究展示了感知与推理能力整合的架构、数据高效训练方法，并诊断分析了稳健感知与结构化推理之间的权衡关系，为多模态模型在区域特定应用中的发展提供了重要见解。
...
### [176] [Reasoning-Driven Multimodal LLM for Domain Generalization](https://arxiv.org/abs/2602.23777)
*Zhipeng Xu,Zilong Wang,Xinyang Jiang,Dongsheng Li,De Cheng,Nannan Wang*

Main category: cs.AI

TL;DR: 本文提出RD-MLDG框架，利用多模态大语言模型的推理能力来提升领域泛化性能，通过多任务交叉训练和自对齐推理正则化解决推理链优化难题。


<details>
  <summary>Details</summary>
Motivation: 现有领域泛化方法主要关注视觉特征不变性，本文探索利用多模态大语言模型的推理能力，通过构建推理链来获得更鲁棒的领域偏移预测。

Method: 提出RD-MLDG框架：1) MTCT多任务交叉训练，引入直接分类路径指导推理监督；2) SARR自对齐推理正则化，通过迭代自标注保持推理链语义丰富性并缓解推理模式不匹配。

Result: 在DomainBed数据集(PACS、VLCS、OfficeHome、TerraInc)上实现了最先进的性能，验证了推理作为领域泛化补充信号的有效性。

Conclusion: 推理能力是提升领域泛化性能的有前景的补充信号，RD-MLDG框架成功解决了推理链优化中的挑战，为多模态大语言模型在领域泛化中的应用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在情感理解方面存在局限性：监督微调方法泛化能力有限且可解释性差，强化学习方法未能与情感认知的内在特性对齐

Method: 提出EMO-R3框架，包含结构化情感思维（引导模型进行分步结构化情感推理）和反思情感奖励（基于视觉-文本一致性和情感连贯性重新评估推理）

Result: 在多个视觉情感理解基准测试中取得显著提升，同时提高了模型的可解释性和情感智能
...
### [178] [RUMAD: Reinforcement-Unifying Multi-Agent Debate](https://arxiv.org/abs/2602.23864)
*Chao Wang,Han Lin,Huaze Tang,Huijing Lin,Wenbo Ding*

Main category: cs.AI

TL;DR: RUMAD是一个基于强化学习的多智能体辩论框架，通过动态通信拓扑控制，在保持高精度的同时大幅降低计算成本，实现80%以上的token节省。
...
Method: RUMAD将动态通信拓扑控制建模为强化学习问题，采用内容无关的观察方案捕捉高层次辩论动态，使用多目标奖励建模解决方案质量、凝聚力和效率，通过PPO训练的控制器动态调整通信图中的边权重，并采用双阈值机制精细控制智能体激活和信息可见性。

Result: 在MMLU、GSM8K和GPQA基准测试中，RUMAD实现了超过80%的token成本降低，同时相比单个LLM模型和多个MAD基线提高了推理准确性。仅在MMLU上训练的RUMAD展现出对域外任务的零样本泛化能力。

Conclusion: RUMAD作为一种高效且鲁棒的方法，能够部署在具有实际资源约束的多智能体推理应用中，学习到的通信策略捕捉了任务无关的有效多智能体协调原则。
...
### [179] [RF-Agent: Automated Reward Function Design via Language Agent Tree Search](https://arxiv.org/abs/2602.23876)
*Ning Gao,Xiuhui Zhang,Xingyu Jiang,Mukang You,Mohan Zhang,Yue Deng*

Main category: cs.AI

TL;DR: RF-Agent 是一个将大语言模型作为语言智能体，通过蒙特卡洛树搜索框架来设计强化学习奖励函数的框架，在17个低层控制任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的基于大语言模型的奖励函数设计方法存在历史反馈利用不足和搜索效率低下的问题，在复杂控制任务中改进有限。需要一种能更好利用历史信息、提高搜索效率的方法。

Method: 将LLMs视为语言智能体，将奖励函数设计建模为序列决策过程，通过蒙特卡洛树搜索（MCTS）来管理奖励设计和优化过程，利用LLMs的多阶段上下文推理能力。

Result: 在17个多样化的低层控制任务中取得了出色的实验结果，证明了该方法的有效性。

Conclusion: RF-Agent框架通过将奖励函数设计视为序列决策过程，结合MCTS和LLMs的上下文推理能力，显著提高了历史信息利用效率和搜索效率，在复杂控制任务中表现优异。
...
### [180] [Pessimistic Auxiliary Policy for Offline Reinforcement Learning](https://arxiv.org/abs/2602.23974)
...
<details>
  <summary>Details</summary>
Motivation: 离线强化学习从预收集数据集中学习，避免实时交互风险，但在学习过程中不可避免地访问分布外动作，导致近似误差、误差累积和严重高估问题
...
### [181] [Portfolio Reinforcement Learning with Scenario-Context Rollout](https://arxiv.org/abs/2602.24037)
*Vanya Priscillia Bendatu,Yao Lu*

Main category: cs.AI

TL;DR: 提出一种宏观条件场景上下文展开方法，通过生成压力事件下的多元回报场景来应对市场制度转变，并解决奖励-转移不匹配问题，在31个投资组合中显著提升夏普比率和降低最大回撤。


<details>
  <summary>Details</summary>
Motivation: 市场制度转变会导致分布偏移，从而降低投资组合再平衡策略的性能。历史数据无法告诉我们如果发生不同情况会怎样，这给强化学习带来了挑战。

Method: 提出宏观条件场景上下文展开方法，生成压力事件下的多元回报场景。分析奖励-转移不匹配问题，构建反事实下一状态，增强评论家智能体的引导目标，实现稳定的学习和偏差-方差权衡。

Result: 在31个美国股票和ETF投资组合的样本外评估中，相比经典和基于强化学习的基准方法，夏普比率提升高达76%，最大回撤降低高达53%。

Conclusion: 该方法通过生成压力场景和解决奖励-转移不匹配问题，有效应对市场制度转变，显著提升了投资组合再平衡策略的性能和稳定性。
...
### [182] [CIRCLE: A Framework for Evaluating AI from a Real-World Lens](https://arxiv.org/abs/2602.24055)
...
<details>
  <summary>Details</summary>
Motivation: 现有MLOps框架关注系统稳定性，基准测试衡量抽象能力，但决策者缺乏关于AI技术在真实世界用户变异和约束下行为的系统性证据。需要连接上下文敏感的定性洞察与可扩展的定量指标。

Method: CIRCLE将TEVV（测试、评估、验证和确认）中的验证阶段操作化，将堆栈外利益相关者关切形式化为可测量信号。整合现场测试、红队测试和纵向研究等方法，形成协调的六阶段生命周期协议。

Result: CIRCLE产生系统性知识：既可在不同站点间比较，又对本地上下文敏感的证据。这支持基于具体化下游效应而非理论能力的治理。

Conclusion: CIRCLE提供了一个结构化、前瞻性的协议，填补了模型中心评估与真实世界部署结果之间的差距，使AI治理能够基于实际表现而非抽象能力。
...
### [183] [Human or Machine? A Preliminary Turing Test for Speech-to-Speech Interaction](https://arxiv.org/abs/2602.24080)
...
<details>
  <summary>Details</summary>
Motivation: 现代语音对话系统能否像人类一样对话仍是一个未解问题，需要建立首个系统性的语音对话系统图灵测试来评估其人类相似性。

Method: 收集2,968个人类判断，评估9个最先进的语音对话系统与28名人类参与者的对话；开发包含18个维度的细粒度人类相似性分类法，并对收集的对话进行众包标注。

Result: 所有评估的语音对话系统均未通过图灵测试；瓶颈在于副语言特征、情感表达和对话个性；现成的AI模型作为图灵测试评判者表现不可靠。

Conclusion: 建立了首个语音对话系统人类相似性评估框架，超越了二元结果，提供了详细的诊断洞察，为对话AI系统的人类化改进铺平了道路。
...
### [184] [Bi-level RL-Heuristic Optimization for Real-world Winter Road Maintenance](https://arxiv.org/abs/2602.24097)
*Yue Xie,Zizhen Xu,William Beazley,Fumiya Iida*

Main category: cs.AI

TL;DR: 提出一个双层优化框架，利用强化学习进行路网分区和资源分配，结合多目标车辆路径规划，用于冬季道路维护的大规模路由问题。


<details>
  <summary>Details</summary>
Motivation: 冬季道路维护对公共安全和环境保护至关重要，但现有方法难以有效处理大规模路由问题，且过度依赖人工决策。需要开发可扩展的优化框架来应对真实世界道路网络的复杂性。

Method: 提出双层优化框架：上层使用强化学习（RL）智能体对道路网络进行战略分区，并从多个仓库优化分配资源；下层在每个集群内解决多目标车辆路径问题（VRP），最小化最大车辆行驶时间和总碳排放。

Result: 在英国战略道路网络（M25、M6、A1）及周边区域的实际运营数据上验证，结果显示：工作负载更均衡，最大行驶时间低于目标两小时阈值，碳排放降低，成本显著节约。

Conclusion: 研究表明，先进的AI驱动双层优化可以直接增强现实世界交通和物流的运营决策能力，为大规模冬季道路维护提供有效的解决方案。
...
### [185] [Artificial Agency Program: Curiosity, compression, and communication in agents](https://arxiv.org/abs/2602.24100)
*Richard Csaky*

Main category: cs.AI

TL;DR: 提出人工代理计划(AAP)，将AI系统构建为受物理和计算约束的现实嵌入式资源有限代理，以好奇心驱动学习进展为核心，强调AI作为扩展人类-工具系统的一部分，通过信息瓶颈统一多个研究方向。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统缺乏在现实世界中的嵌入式、资源受限的代理能力，需要将AI视为扩展人类-工具系统的一部分，增强感知、理解和执行能力，同时减少人、工具和环境之间的交互摩擦。

Method: 建立可证伪的研究计划，包含明确成本、分阶段实验和具体多模态标记化测试平台。代理在有限预算下分配观察、行动和思考资源，通过信息瓶颈统一预测压缩、内在动机、赋权与控制、接口质量和语言/自我通信。

Result: 提出了一个连接内在动机、信息论、热力学、有限理性和现代推理系统的概念与实验框架，为构建资源受限的现实嵌入式AI代理提供了系统性研究议程。

Conclusion: AI最有用的方式是作为扩展人类-工具系统的一部分，通过好奇心驱动的学习进展在物理和计算约束下运作，这需要统一多个研究领域并建立可验证的实验框架。
...
### [186] [Recycling Failures: Salvaging Exploration in RLVR via Fine-Grained Off-Policy Guidance](https://arxiv.org/abs/2602.24110)
*Yanwei Ren,Haotian Zhang,Likang Xiao,Xikai Zhang,Jiaxing Huang,Jiayan Qiu,Baosheng Yu,Quan Chen,Liu Liu*

Main category: cs.AI

TL;DR: SCOPE框架通过过程奖励模型识别次优轨迹中的首个错误步骤，进行细粒度的逐步修正，有效利用部分正确的轨迹，提升探索多样性13.5%，在数学推理任务上达到46.6%的平均准确率。


<details>
  <summary>Details</summary>
Motivation: 传统基于结果的监督方法存在关键局限：对部分正确但包含几个错误步骤的轨迹惩罚过重，导致模型丢弃有价值的"大部分正确"的轨迹，降低了轨迹多样性并过早缩小了探索空间。现有方法未能有效利用模型自身生成的部分正确轨迹。

Method: 提出SCOPE框架，利用过程奖励模型精确定位次优轨迹中的第一个错误步骤，应用细粒度的逐步离策略修正。通过对部分正确轨迹进行精确细化，有效挽救这些轨迹并维持广阔的探索空间。

Result: SCOPE将多样性分数提升了13.5%，在数学推理任务上达到46.6%的平均准确率，在分布外推理任务上达到53.4%的准确率，建立了新的最先进结果。

Conclusion: SCOPE通过逐步修正部分正确轨迹，有效解决了RLVR中探索空间过早缩窄的问题，显著提升了轨迹多样性和模型性能，在数学推理任务上取得了优异的泛化能力。
...
### [187] [LemmaBench: A Live, Research-Level Benchmark to Evaluate LLM Capabilities in Mathematics](https://arxiv.org/abs/2602.24173)
*Antoine Peyronnet,Fabian Gloeckle,Amaury Hayat*

Main category: cs.AI

TL;DR: 提出一个基于最新数学研究论文的可更新LLM基准测试方法，替代传统的静态竞赛题基准


<details>
  <summary>Details</summary>
Motivation: 现有LLM数学能力基准主要依赖静态的手工整理竞赛题或教科书问题，无法真实反映模型在研究级数学上的能力，需要建立能直接评估最新数学研究成果的基准

Method: 开发自动流水线：从arXiv提取引理，通过显式化所有假设和定义将其重写为自包含的陈述，创建可定期更新的基准测试集

Result: 当前最先进LLM在定理证明任务上准确率约为10-15%（pass@1），表明LLM要达到人类研究级证明能力仍有很大提升空间

Conclusion: 建立了基于真实数学研究的可更新基准测试框架，揭示了LLM在研究级数学证明方面与人类能力的显著差距，为未来模型发展提供了评估标准
...
### [188] [Learning Flexible Job Shop Scheduling under Limited Buffers and Material Kitting Constraints](https://arxiv.org/abs/2602.24180)
*Shishun Zhang,Juzhan Xu,Yidan Fan,Chenyang Zhu,Ruizhen Hu,Yongjun Wang,Kai Xu*

Main category: cs.AI

TL;DR: 提出一种基于异构图网络和深度强化学习的柔性作业车间调度方法，解决有限缓冲区与物料配套约束问题，在完工时间和托盘更换次数上优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现有柔性作业车间调度研究往往忽略或理想化实际生产约束，特别是有限缓冲区问题对生产效率影响显著。需要研究更接近实际场景的扩展问题——带有限缓冲区和物料配套的柔性作业车间调度问题。

Method: 在深度强化学习框架中引入异构图网络来建模全局状态。通过构建机器、工序和缓冲区之间的高效消息传递，网络专注于避免在长序列调度中可能导致频繁托盘更换的决策，从而提高缓冲区利用率和整体决策质量。

Result: 在合成和实际生产线数据集上的实验结果表明，该方法在完工时间和托盘更换次数方面优于传统启发式算法和先进的深度强化学习方法，同时在解质量和计算成本之间取得了良好平衡。

Conclusion: 提出的基于异构图网络的深度强化学习方法能有效解决带有限缓冲区和物料配套的柔性作业车间调度问题，为实际生产调度提供了更实用的解决方案，并通过仿真系统可视化展示了生产线进展。
...
### [189] [Uncertainty Quantification for Multimodal Large Language Models with Incoherence-adjusted Semantic Volume](https://arxiv.org/abs/2602.24195)
...
Method: UMPIRE通过计算采样MLLM响应的不连贯调整语义体积来量化不确定性，利用模型内部模态特征，无需训练或外部工具，能捕捉样本的全局语义多样性和响应的局部不连贯性。

Result: 在图像、音频和视频-文本基准测试中，UMPIRE在错误检测和不确定性校准方面持续优于基线方法，包括对抗性和分布外设置，并能泛化到非文本输出任务如图像和音频生成。

Conclusion: UMPIRE提供了一个高效、无需训练的不确定性量化框架，适用于多种模态，能提升MLLM的可靠性，支持不可靠查询向人类专家或更大模型的升级。
...
### [190] [A Minimal Agent for Automated Theorem Proving](https://arxiv.org/abs/2602.24273)
*Borja Requena Pozo,Austin Letson,Krystian Nowakowski,Izan Beltran Ferreiro,Leopoldo Sarra*

Main category: cs.AI

TL;DR: 提出一个最小化的智能体基线，用于系统比较不同AI定理证明器架构，展示迭代方法相比单次生成的优越性


<details>
  <summary>Details</summary>
Motivation: 需要建立一个系统性的比较基准来评估不同AI定理证明器架构，当前缺乏统一的标准框架

Method: 设计实现包含迭代证明精炼、库搜索和上下文管理等核心功能的最小化智能体基线架构

Result: 在定性不同的基准测试中表现出与最先进方法竞争的性能，同时架构显著简化；迭代方法在样本效率和成本效益上优于多次单次生成
...
### [191] [DARE-bench: Evaluating Modeling and Instruction Fidelity of LLMs in Data Science](https://arxiv.org/abs/2602.24288)
*Fan Shu,Yite Wang,Ruofan Wu,Boyi Liu,Zhewei Yao,Yuxiong He,Feng Yan*

Main category: cs.AI

TL;DR: DARE-bench是一个用于机器学习和数据科学指令遵循的基准测试，包含6300个Kaggle衍生任务，提供可验证的真实标签，解决了现有基准在标准化评估和训练数据稀缺方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准存在两大缺陷：一是缺乏标准化的过程感知评估，无法捕捉指令遵循和过程保真度；二是缺乏准确标记的训练数据。随着LLM处理复杂多步数据科学任务的需求快速增长，需要更准确的基准测试。

Method: 引入DARE-bench基准，包含6300个Kaggle衍生任务，所有任务都有可验证的真实标签，确保客观可重复的评估。提供大规模训练数据和评估集，支持智能体工具使用。

Result: 评估显示即使GPT-4-mini等强大模型在机器学习建模任务中表现不佳。使用DARE-bench训练任务进行微调能显著提升性能：监督微调使Qwen3-32B准确率提升1.83倍，强化学习使Qwen3-4B准确率提升超过8倍。

Conclusion: DARE-bench既是准确的评估基准，也是关键的训练数据，其显著改进验证了其在机器学习和数据科学指令遵循任务中的重要性。
...
### [192] [MSVBench: Towards Human-Level Evaluation of Multi-Shot Video Generation](https://arxiv.org/abs/2602.23969)
*Haoyuan Shi,Yunxin Li,Nanhao Deng,Zhenran Xu,Xinyu Chen,Longyue Wang,Baotian Hu,Min Zhang*

Main category: cs.MM

TL;DR: MSVBench是首个针对多镜头视频生成的综合基准测试，包含分层脚本和参考图像，通过混合评估框架（结合大型多模态模型和专家模型）评估视频生成方法，发现当前模型主要作为视觉插值器而非世界模型，并验证了与人类判断的高相关性。
...
Method: 提出MSVBench基准测试，包含分层脚本和参考图像；设计混合评估框架，结合大型多模态模型的高层语义推理和领域专家模型的细粒度感知能力；评估20种视频生成方法；通过微调轻量模型验证基准的监督信号价值。

Result: 评估发现当前模型尽管视觉保真度高，但主要作为视觉插值器而非真正世界模型；基准测试与人类判断的斯皮尔曼等级相关性达94.4%；基于基准推理轨迹微调的轻量模型能达到与Gemini-2.5-Flash等商业模型相当的人类对齐性能。

Conclusion: MSVBench填补了多镜头视频生成评估的空白，不仅提供了可靠的评估基准，还能作为可扩展的监督信号提升模型性能，推动视频生成从视觉插值向真正世界建模发展。
