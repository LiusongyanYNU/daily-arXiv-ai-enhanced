...
TL;DR: 基于自由能原理，仅从广告视频场景级表达特征量化"愉悦度"、"惊喜"和"习惯化"情绪，无需生理信号或主观评分


<details>
  <summary>Details</summary>
Motivation: 广告观看中的情绪反应对理解媒体效果至关重要，但现有方法依赖外部信息（生理信号或主观评分），需要建立无需外部信息的可解释情绪估计方法基础

Method: 基于自由能原理，使用Kullback-Leibler散度(KLD)捕捉预测误差，贝叶斯惊喜(BS)捕捉信念更新，不确定性(UN)反映先验模糊性，仅从1059个15秒食品广告视频的场景级表达特征量化情绪

Result: KLD反映与品牌呈现相关的"愉悦度"，BS捕捉信息复杂性引起的"惊喜"，UN反映元素类型和空间安排不确定性驱动的"惊喜"，识别出三种特征情绪模式：不确定刺激、持续高情绪、瞬时峰值衰减

Conclusion: 该方法在9种超参数设置下保持稳健，在6种日本广告视频类型上具有良好泛化性，为无需外部信息的可解释情绪估计提供了方法论基础，可指导开发更吸引人的广告视频创作技术
...
Result: 当前生成模型能够模拟证件表面美学特征，但无法复制结构和法证真实性。模型在生成逼真身份证件伪造品方面存在显著局限性。

Conclusion: 生成式身份证件深度伪造达到法证级真实性的风险可能被高估，机器学习从业者与证件法证专家之间的合作对于现实风险评估具有重要价值。
...
TL;DR: 该研究比较了从头训练的CNN与迁移学习方法在儿童肺炎检测中的表现，发现微调的ResNet50达到近完美准确率（99.43%），显著优于从头训练模型。


<details>
  <summary>Details</summary>
Motivation: 肺炎是五岁以下儿童主要死因，每年导致超过70万例死亡。胸部X光诊断受限于放射科医生可用性和诊断一致性，需要自动化解决方案来改善资源有限地区的诊断能力。

Method: 使用5,216张儿童胸部X光数据集，按80/10/10比例划分训练、验证和测试集。比较了从头训练的CNN与三种迁移学习模型（ResNet50、DenseNet121、EfficientNet-B0），评估冻结主干和微调两种策略。采用准确率、F1分数和AUC进行评估，并使用Grad-CAM提供可解释性可视化。

Result: 微调的ResNet50表现最佳：99.43%准确率、99.61% F1分数和99.93% AUC，仅3例误分类。微调模型平均比冻结主干模型高5.5个百分点。Grad-CAM确认模型关注临床相关的肺部区域进行预测。

Conclusion: 迁移学习结合微调在儿童肺炎检测中显著优于从头训练的CNN，达到近乎完美的准确率。该系统在资源有限环境中具有强大筛查潜力。未来工作应在多中心和成人数据集上验证这些发现。
...
TL;DR: 对心脏超声分割文献进行综述，并在CAMUS数据集上对U-Net、Attention U-Net和TransUNet三种架构进行统一基准测试，比较不同预处理方法的效果。


<details>
  <summary>Details</summary>
Motivation: 现有综述大多只总结心脏成像和深度学习进展，缺乏统一且可复现的实验基准。本研究旨在填补这一空白，通过标准化比较为心脏超声分割提供实用指导。
...
Result: 原生NIfTI训练的U-Net达到94%平均Dice分数，PNG-16位流程为91%。Attention U-Net在小区域和低对比度区域表现更好，减少边界泄漏；TransUNet在挑战性帧上泛化能力最强，特别是自监督预训练后。伪标签扩展训练集并提高鲁棒性。

Conclusion: 提供了三种架构在标准化CAMUS预处理下的统一基准测试；给出了保持超声数据强度保真度、分辨率一致性和对齐的实用指导；展望了可扩展自监督和新兴多模态GPT标注流程在快速标注、质量保证和针对性数据集构建中的应用。
...
### [5] [Deepfake Detection with Multi-Artifact Subspace Fine-Tuning and Selective Layer Masking](https://arxiv.org/abs/2601.01041)
...
Method: 基于多伪影子空间和选择性层掩码的MASM方法：1）使用奇异值分解将预训练权重划分为稳定的语义主空间和多个可学习的伪影子空间；2）引入选择性层掩码策略，根据各伪影子空间学习状态自适应调节对应网络层更新；3）施加正交性和谱一致性约束，使多个伪影子空间学习互补多样的伪影表示

Result: 该方法能够解耦语义表示与伪影表示，约束伪影子空间的拟合强度，从而在跨数据集场景中提高泛化鲁棒性

Conclusion: MASM方法通过显式解耦语义与伪影表示，并约束伪影子空间拟合，有效解决了深度伪造检测中的跨数据集泛化问题，同时保持了语义稳定性
...
TL;DR: MCLSC：为资源受限的边缘设备设计的视觉态势感知系统，通过运动补偿的潜在语义画布和运动门控分割，显著减少计算开销


<details>
  <summary>Details</summary>
Motivation: 在资源受限的边缘设备上实现视觉态势感知面临计算资源有限的问题，需要减少昂贵的语义分割计算频率，同时保持语义信息的持续性和一致性

Method: 使用两个潜在语义画布（静态层和动态层），在稳定化的基准坐标系中维护语义元数据；通过运动门控机制，仅在检测到运动时才触发昂贵的全景分割（Mask2Former）；运动补偿技术保持坐标系一致性

Result: 在480p视频上，分割调用减少30倍以上，端到端平均处理时间降低20倍以上，同时保持连贯的静态/动态语义覆盖

Conclusion: MCLSC系统通过运动补偿和运动门控机制，在边缘设备上实现了高效的视觉态势感知，大幅降低了计算开销，同时保持了语义信息的质量和一致性
...
### [7] [LinMU: Multimodal Understanding Made Linear](https://arxiv.org/abs/2601.01322)
*Hongjie Wang,Niraj K. Jha*

Main category: cs.CV

TL;DR: LinMU是一种线性复杂度的视觉语言模型，通过双分支M-MATE模块替换自注意力层，在保持性能的同时显著降低计算复杂度，适用于高分辨率图像和长视频理解。
...
Method: 提出LinMU架构，用M-MATE双分支模块（Flex-MA分支用于全局上下文，Local-Swin分支用于局部相关性）替换所有自注意力层，并采用三阶段蒸馏框架从预训练VLM转换。

Result: 在MMMU、TextVQA、LongVideoBench等多个基准测试中，LinMU匹配教师模型性能，同时将首次令牌生成时间降低2.7倍，令牌吞吐量提高9.0倍。
...
### [8] [VL-OrdinalFormer: Vision Language Guided Ordinal Transformers for Interpretable Knee Osteoarthritis Grading](https://arxiv.org/abs/2601.00879)
*Zahid Ullah,Jihie Kim*

Main category: cs.CV

TL;DR: VLOrdinalFormer：结合视觉语言引导的序数学习框架，用于膝关节骨关节炎的自动分级，在KL1和KL2早期阶段表现优异，超越现有CNN和ViT基线模型。


<details>
  <summary>Details</summary>
Motivation: 膝关节骨关节炎是全球致残的主要原因，使用Kellgren Lawrence分级系统进行准确评估对临床决策至关重要。然而，早期阶段（特别是KL1和KL2）的放射学差异细微，导致放射科医生之间存在观察者间变异性。

Method: 提出VLOrdinalFormer框架，结合ViT L16骨干网络、基于CORAL的序数回归和CLIP驱动的语义对齐模块，融入关节间隙狭窄、骨赘形成和软骨下硬化等临床相关文本概念。采用分层五折交叉验证、类别感知重加权和测试时增强等技术提升鲁棒性。

Result: 在OAI kneeKL224数据集上实现最先进性能，在宏观F1分数和总体准确率上超越CNN和ViT基线。特别在KL1和KL2分级上获得显著性能提升，同时不损害轻度或重度病例的分类准确性。

Conclusion: 视觉语言对齐的序数变换器作为可靠且可解释的工具，在常规放射学实践中具有KOA分级和疾病进展评估的潜力。Grad-CAM和CLIP相似性图分析证实模型持续关注临床相关的解剖区域。
...
### [9] [Beyond Patches: Global-aware Autoregressive Model for Multimodal Few-Shot Font Generation](https://arxiv.org/abs/2601.01593)
*Haonan Cai,Yuxuan Luo,Zhouhui Lian*

Main category: cs.CV

TL;DR: GAR-Font是一个用于少样本字体生成的新型自回归框架，通过全局感知分词器、多模态风格编码器和后处理细化流程，解决了现有方法在结构完整性和风格保真度方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有少样本字体生成方法存在两个主要问题：1）传统自回归模型使用补丁级分词，忽略了字体合成所需的全局依赖关系；2）现有方法局限于图像到图像范式，忽略了语言在传达字体设计风格意图中的作用。

Method: 提出GAR-Font框架，包含三个关键组件：1）全局感知分词器，同时捕捉局部结构和全局风格模式；2）多模态风格编码器，通过轻量级语言风格适配器实现灵活的风格控制；3）后处理细化流程，增强结构保真度和风格连贯性。

Result: 大量实验表明，GAR-Font在少样本字体生成任务上优于现有方法，在保持全局风格忠实度和通过文本风格指导获得更高质量结果方面表现突出。

Conclusion: GAR-Font通过整合全局感知分词、多模态风格控制和后处理细化，成功解决了少样本字体生成中的结构完整性和风格保真度问题，为字体设计提供了更有效的自动化解决方案。
...
### [10] [VideoCuRL: Video Curriculum Reinforcement Learning with Orthogonal Difficulty Decomposition](https://arxiv.org/abs/2601.00887)
*Hongbo Jin,Kuanwei Lin,Wenhao Zhang,Yichen Jin,Ge Li*

Main category: cs.CV

TL;DR: VideoCuRL：一种将视频理解难度分解为视觉时间感知负载和认知推理深度两个维度的强化学习课程框架，通过2D课程网格和动态训练策略提升视频大语言模型的性能。
...
Method: 提出VideoCuRL框架，将难度分解为两个维度：使用光流和关键帧熵评估视觉复杂度，使用校准惊奇度评估认知复杂度，将数据映射到2D课程网格；采用能力感知对角线波前策略从基础对齐到复杂推理进行训练调度；引入动态稀疏KL和结构化重访来稳定训练。

Result: 在推理任务上比强RL基线提升2.5分（VSI-Bench），在感知任务上提升2.9分（VideoMME）；消除了基于生成的课程方法的高昂推理开销，为稳健的视频后训练提供了可扩展解决方案。

Conclusion: VideoCuRL通过将难度分解为视觉和认知两个维度，提供了一种高效、可扩展的强化学习课程框架，显著提升了视频大语言模型在复杂时空推理任务上的性能。
...
### [11] [DDNet: A Dual-Stream Graph Learning and Disentanglement Framework for Temporal Forgery Localization](https://arxiv.org/abs/2601.01784)
*Boyang Zhao,Xin Liao,Jiaxin Chen,Xiaoshuai Wu,Yufeng Wu*

Main category: cs.CV

TL;DR: DDNet：双流图学习与解缠框架，用于视频时序伪造定位，通过协调局部伪影和语义内容流，结合特征解缠与跨层嵌入，显著提升定位精度和跨域鲁棒性。


<details>
  <summary>Details</summary>
Motivation: AIGC技术快速发展使得仅篡改视频小片段即可误导观众，而视频级检测不准确且缺乏说服力。现有方法受限于局部视角，难以捕捉全局异常，需要更精确的时序伪造定位方法。

Method: 提出DDNet双流图学习与解缠框架：1) 时间距离流捕捉局部伪影；2) 语义内容流建立长程连接；3) 痕迹解缠与适配(TDA)分离通用伪造指纹；4) 跨层特征嵌入(CLFE)通过层次特征深度融合构建鲁棒特征基础。

Result: 在ForgeryNet和TVIL基准测试中，AP@0.95指标比现有最优方法提升约9%，在跨域鲁棒性方面有显著改进。

Conclusion: DDNet通过协调局部与全局信息流，结合特征解缠和跨层融合，有效解决了时序伪造定位中全局线索被局部平滑淹没的问题，在精度和鲁棒性上均优于现有方法。
...
### [12] [Comparative Evaluation of CNN Architectures for Neural Style Transfer in Indonesian Batik Motif Generation: A Comprehensive Study](https://arxiv.org/abs/2601.00888)
*Happy Gery Pangestu,Andi Prademon Yunus,Siti Khomsah*

Main category: cs.CV

TL;DR: 该研究系统比较了五种CNN骨干网络在印尼蜡染风格迁移中的表现，发现ResNet架构在保持结构相似性的同时，计算效率比VGG高16倍，收敛速度快5-6倍，更适合实际部署。


<details>
  <summary>Details</summary>
Motivation: 现有神经风格迁移方法主要基于VGG架构，虽然风格表达能力强但计算和内存需求高，限制了在资源有限环境中的实际应用。需要寻找更高效的架构来平衡结构保持、风格质量和计算效率。

Method: 通过245个对照实验，系统比较了VGG16、VGG19、Inception V3、ResNet50和ResNet101五种CNN骨干网络。结合定量指标（SSIM、LPIPS、FLOPs）、定性评估和统计分析（ANOVA），分析结构保持、风格行为和计算效率之间的权衡。

Result: 骨干网络选择对结构相似性无显著统计差异（SSIM的p=0.83）。ResNet架构比VGG模型收敛速度快5-6倍，FLOPs减少16倍以上（0.63 vs 10.12 GFLOPs），同时保持相似的感知相似性（LPIPS=0.53）。VGG产生更密集的绘画纹理，ResNet更注重几何稳定性和笔触保持，Inception V3表现中等但噪声较多。

Conclusion: 研究将NST中的架构选择从最大化风格强度转向效率感知和结构保持的部署，强调ResNet骨干网络作为可扩展、面向行业的蜡染生成的实用基础。ResNet在保持结构相似性的同时显著提高计算效率，更适合实际应用。
...
### [13] [CornViT: A Multi-Stage Convolutional Vision Transformer Framework for Hierarchical Corn Kernel Analysis](https://arxiv.org/abs/2601.00897)
*Sai Teja Erukude,Jane Mascarenhas,Lior Shamir*

Main category: cs.CV

TL;DR: CornViT：基于卷积视觉Transformer的三阶段玉米籽粒分级框架，通过纯度、形态和胚芽朝向的层次化分类实现自动化评估，性能优于传统CNN模型。


<details>
  <summary>Details</summary>
Motivation: 玉米籽粒分级对种子认证、定向播种和育种至关重要，但目前主要依赖人工检查，效率低且主观性强。需要开发自动化、准确的评估系统来替代人工。

Method: 提出CornViT三阶段CvT框架：第一阶段区分纯籽粒与杂质；第二阶段将纯籽粒分为扁平与圆形形态；第三阶段确定扁平纯籽粒的胚芽朝向（上/下）。使用ImageNet-22k预训练的CvT-13骨干网络进行头部微调。

Result: 在三个任务上分别达到93.76%（纯度）、94.11%（形态）和91.12%（胚芽朝向）的测试准确率，显著优于ResNet-50（76.56-81.02%）和DenseNet-121（86.56-89.38%）。

Conclusion: CornViT框架结合卷积增强的自注意力机制在玉米籽粒分析中表现出优越性能，提供了可部署的自动化解决方案，包括数据集、代码和Flask网络应用，有望改进种子质量工作流程。
...
### [14] [Evaluating Contextual Intelligence in Recyclability: A Comprehensive Study of Image-Based Reasoning Systems](https://arxiv.org/abs/2601.00905)
...
<details>
  <summary>Details</summary>
Motivation: 虽然高效回收的重要性被广泛认可，但公众准确判断物品的可回收性和正确处理方式仍然很困难。需要探索先进技术来帮助解决这一复杂任务。

Method: 使用精心策划的图像数据集，评估GPT-4o、GPT-4o-mini和Claude 3.5等视觉语言模型的能力，包括：1) 将物品匹配到合适的回收箱并考虑物理尺寸；2) 适应地区特定的回收指南；3) 考虑污染或结构损坏；4) 处理多材料物品。

Result: 研究结果显示这些模型在上下文理解方面相比之前版本有显著进步，但也识别出它们在某些方面仍存在不足。模型能够处理复杂的回收场景，但仍有改进空间。

Conclusion: 持续改进上下文感知模型对于提升公众回收实践和推进环境可持续发展至关重要。视觉语言模型在回收指导方面展现出潜力，但需要进一步优化以处理更复杂的现实场景。
...
### [15] [Clean-GS: Semantic Mask-Guided Pruning for 3D Gaussian Splatting](https://arxiv.org/abs/2601.00913)
*Subhankar Mishra*
...
Conclusion: Clean-GS使用仅3个分割掩码（1%的视图）的语义信息，有效去除背景杂波和漂浮物，实现显著模型压缩，解决了3DGS部署的实际限制
...
### [16] [Four-Stage Alzheimer's Disease Classification from MRI Using Topological Feature Extraction, Feature Selection, and Ensemble Learning](https://arxiv.org/abs/2601.00918)
*Faisal Ahmed*

Main category: cs.CV

TL;DR: TDA-Alz：使用拓扑数据分析和集成学习进行阿尔茨海默病严重程度四阶段分类的新框架，在OASIS-1数据集上达到98.19%准确率和99.75% AUC，优于深度学习方法。


<details>
  <summary>Details</summary>
Motivation: 从脑MRI准确高效地分类阿尔茨海默病严重程度是一个关键挑战，特别是在数据有限和模型可解释性方面存在问题。现有深度学习方法需要大量数据和计算资源，且缺乏可解释性。

Method: 提出TDA-Alz框架：1）使用拓扑数据分析提取脑MRI的内在结构模式描述符；2）通过特征选择保留最具区分性的拓扑特征；3）采用集成学习策略进行稳健的多类别分类。

Result: 在OASIS-1 MRI数据集上，该方法达到98.19%的准确率和99.75%的AUC，优于或匹配基于深度学习的现有方法。不需要数据增强、预训练网络或大规模计算资源。

Conclusion: TDA-Alz为MRI-based阿尔茨海默病严重程度分类提供了一个强大、轻量级且可解释的替代方案，具有在真实世界临床决策支持系统中应用的潜力。
...
### [17] [Application of deep learning techniques in non-contrast computed tomography pulmonary angiogram for pulmonary embolism diagnosis](https://arxiv.org/abs/2601.00925)
*I-Hsien Ting,Yi-Jun Tseng,Yu-Sheng Lin*

Main category: cs.CV

TL;DR: 使用3D卷积神经网络在无对比剂CT图像中自动分类肺栓塞，准确率达85%，AUC为0.84，证明该模型在肺栓塞诊断中的可行性。


<details>
  <summary>Details</summary>
Motivation: 肺栓塞是危及生命的疾病，早期检测和治疗可显著降低死亡率。传统使用对比剂CT血管造影诊断肺栓塞的方法存在两个问题：1) 对比剂可能导致慢性肾病患者的急性肾损伤；2) 对比剂需要时间发挥作用，急性肺栓塞患者可能错过黄金治疗时间。

Method: 本研究采用深度学习技术，使用3D卷积神经网络模型，在无对比剂的CT图像中自动分类肺栓塞。

Result: 深度学习模型在无对比剂CT图像的肺栓塞分类中表现显著，准确率达到85%，AUC为0.84。

Conclusion: 该研究证实了深度学习模型在无对比剂CT图像中诊断肺栓塞的可行性，为减少对比剂相关风险和缩短诊断时间提供了新途径。
...
### [18] [Analyzing the Shopping Journey: Computing Shelf Browsing Visits in a Physical Retail Store](https://arxiv.org/abs/2601.00928)
*Luis Yoichi Morales,Francesco Zanlungo,David M. Woollard*

Main category: cs.CV

TL;DR: 提出一种从3D轨迹中提取顾客"货架访问"行为的算法，用于分析零售店中的顾客浏览意图，并在不同商店环境中验证其泛化能力。


<details>
  <summary>Details</summary>
Motivation: 随着机器人在零售业客户服务角色中的部署挑战，需要自主理解顾客购物意图，特别是分析实体店中的顾客活动行为。

Method: 开发算法从基于机器视觉3D跟踪和头顶摄像头获取的轨迹中提取"货架访问"行为，使用两组不同商店收集的轨迹数据（8138和15129条）进行独立校准，由人工标注验证。

Result: 算法能够在与校准环境不同的商店中识别顾客浏览活动，并成功分析大规模轨迹中的顾客"浏览模式"及其与实际购买行为的关系。
...
### [19] [ShadowGS: Shadow-Aware 3D Gaussian Splatting for Satellite Imagery](https://arxiv.org/abs/2601.00939)
*Feng Luo,Hongbo Pan,Xiang Yang,Baoyu Jiang,Fengqing Liu,Tao Huang*

Main category: cs.CV

TL;DR: ShadowGS：基于3D高斯泼溅的卫星图像阴影建模框架，通过物理渲染方程和光线步进技术解决多时相卫星图像中的阴影不一致问题，提高3D重建精度。


<details>
  <summary>Details</summary>
Motivation: 多时相卫星图像中，由于光照条件变化，阴影存在显著不一致性，这影响了3D重建的几何精度和阴影解耦效果。

Method: 基于3D高斯泼溅框架，结合遥感物理渲染方程和高效光线步进技术，引入阴影一致性约束和阴影图先验，有效解耦不同光照分量和表观属性。

Result: 在阴影解耦精度、3D重建精度和新视角合成质量方面优于现有方法，仅需几分钟训练，在RGB、全色融合和稀疏视图卫星输入等多种设置下表现稳健。

Conclusion: ShadowGS能精确建模几何一致的阴影，有效解耦光照分量，显著提升3D重建的几何精度，适用于多种卫星图像输入场景。
...
### [20] [Learning to Segment Liquids in Real-world Images](https://arxiv.org/abs/2601.00940)
*Jonas Li,Michelle Li,Luke Liu,Heng Fan*

Main category: cs.CV

TL;DR: 提出LQDS液体分割数据集和LQDM模型，解决液体分割难题，在5000张真实图像上实现14类液体分割，性能优于现有方法


<details>
  <summary>Details</summary>
Motivation: 液体在日常生活中无处不在，但机器人缺乏安全避让或交互液体的能力。液体分割困难，因为液体外观多样、形状多变，且具有透明或反射特性，容易与背景混淆

Method: 构建LQDS大规模液体数据集（5000张真实图像，14个类别），设计LQDM液体检测模型，利用边界分支和分割分支之间的交叉注意力机制增强分割预测

Result: 在LQDS测试集上，LQDM表现出色，优于现有最先进方法，为液体语义分割建立了强基线

Conclusion: LQDS数据集和LQDM模型有效解决了液体分割问题，为机器人安全交互液体提供了重要基础
...
### [21] [PhyEduVideo: A Benchmark for Evaluating Text-to-Video Models for Physics Education](https://arxiv.org/abs/2601.00943)
*Megha Mariam K. M,Aditya Arun,Zakaria Laskar,C. V. Jawahar*

Main category: cs.CV

TL;DR: 该研究提出了首个用于评估文本到视频模型在物理教育中应用潜力的基准测试，发现当前模型能生成视觉连贯的视频，但概念准确性有待提高。


<details>
  <summary>Details</summary>
Motivation: 生成式AI模型，特别是文本到视频系统，为科学教育提供了通过自动创建引人入胜的视觉解释来转变教学方式的潜力。本研究旨在评估这些模型在物理教育中的应用可行性。

Method: 研究者开发了一个专门用于评估物理教育视频生成的基准测试。每个物理概念被分解为细粒度的教学点，每个点都配有精心设计的提示词，用于视觉解释。通过评估T2V模型根据这些提示生成准确视频的能力来测试其性能。

Result: 评估显示，当前模型能生成视觉连贯、运动平滑、闪烁最少的视频，但概念准确性不够可靠。在力学、流体和光学领域表现较好，但在电磁学和热力学等需要描绘抽象相互作用的领域表现较差。

Conclusion: 研究发现教育视频生成中存在视觉质量与概念正确性之间的差距。该基准测试旨在帮助社区缩小这一差距，推动开发能够大规模提供准确、符合课程要求的物理教育内容的T2V系统。
...
### [22] [Deep Clustering with Associative Memories](https://arxiv.org/abs/2601.00963)
*Bishwajit Saha,Dmitry Krotov,Mohammed J. Zaki,Parikshit Ram*

Main category: cs.CV

TL;DR: 提出DCAM方法，通过基于能量的联想记忆动力学，在单个目标函数中更紧密地结合表示学习和聚类，改善深度聚类效果


<details>
  <summary>Details</summary>
Motivation: 深度聚类中表示学习是可微的，但聚类本质上是离散优化任务，需要各种近似和正则化才能融入标准可微流程，导致表示学习和聚类之间存在割裂

Method: 提出DCAM方法，利用基于能量的联想记忆动力学设计新的损失函数，在单个目标函数中更紧密地结合表示学习和聚类

Result: DCAM在不同架构选择（卷积、残差或全连接）和数据模态（图像或文本）上都能产生改进的聚类质量

Conclusion: DCAM通过基于能量的联想记忆动力学，在单个目标函数中更紧密地结合表示学习和聚类，有效改善了深度聚类效果
...
### [23] [A Deep Learning Approach for Automated Skin Lesion Diagnosis with Explainable AI](https://arxiv.org/abs/2601.00964)
*Md. Maksudul Haque,Rahnuma Akter,A S M Ahsanul Sarkar Akib,Abdul Hasib*

Main category: cs.CV

TL;DR: 提出一种结合数据平衡、数据增强、混合EfficientNetV2-L框架和三阶段渐进学习的皮肤病变分类方法，在HAM10000数据集上达到91.15%准确率，并使用XAI技术提高模型可解释性。


<details>
  <summary>Details</summary>
Motivation: 皮肤癌是全球最常见且危险的癌症之一，需要及时准确的诊断。现有方法在分类精度和临床可解释性方面仍有提升空间。

Method: 采用高质量数据平衡方法、大规模数据增强、结合通道注意力的混合EfficientNetV2-L框架，以及三阶段渐进学习策略。同时使用Grad-CAM和显著性图等XAI技术提供可视化解释。

Result: 在HAM10000数据集上获得91.15%总准确率、85.45%宏F1分数和99.33%微平均AUC。在7种病变类别中均表现优异，特别是在黑色素瘤和黑色素细胞痣分类上表现突出。

Conclusion: 该方法不仅提高了皮肤病变分类的准确性，还通过XAI技术增强了诊断透明度和临床可信度，有助于识别导致分类的视觉特征。
...
### [24] [Few-Shot Video Object Segmentation in X-Ray Angiography Using Local Matching and Spatio-Temporal Consistency Loss](https://arxiv.org/abs/2601.00988)
*Lin Xi,Yingliang Ma,Xiahai Zhuang*

Main category: cs.CV

TL;DR: 提出一种新的FSVOS模型，采用局部匹配策略限制搜索空间，通过方向采样实现动态可变采样区域，结合时空对比学习增强特征一致性，并在新的X射线血管造影视频数据集上验证了优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频分割方法存在效率问题（如标准卷积实现）或可移植性限制（如CUDA特定内核），需要一种既高效又能在不同设备上灵活部署的解决方案，特别是在医疗影像分析领域。

Method: 1. 局部匹配策略限制搜索空间到最相关邻域像素；2. 方向采样视角重新组织局部采样过程，实现非参数化动态可变采样区域；3. 监督时空对比学习方案增强帧间特征一致性；4. 创建MOSXAV数据集用于X射线血管造影视频多目标分割。

Result: 在CADICA、XACV和MOSXAV三个数据集上的实验表明，该方法在分割精度和泛化能力（包括已见和未见类别）方面均优于当前最先进的视频分割方法。

Conclusion: 提出的FSVOS模型通过创新的局部匹配和方向采样机制，结合时空对比学习，为医疗影像分析提供了高效、灵活且可移植的视频分割解决方案，具有广泛的临床应用潜力。
...
### [25] [UnrealPose: Leveraging Game Engine Kinematics for Large-Scale Synthetic Human Pose Data](https://arxiv.org/abs/2601.00991)
*Joshua Kawaguchi,Saad Manzur,Emily Gao Wang,Maitreyi Sinha,Bryan Vela,Yunxi Wang,Brandon Vela,Wayne B. Hayes*

Main category: cs.CV

TL;DR: 提出UnrealPose-Gen合成数据生成管道和UnrealPose-1M数据集，解决3D人体姿态数据标注成本高、缺乏真实世界ground truth的问题


<details>
  <summary>Details</summary>
Motivation: 现实世界中准确标注的3D人体姿态数据获取成本高昂且受限于工作室环境，而野外数据集缺乏已知的真实标注。需要高质量合成数据来支持计算机视觉任务。

Method: 基于Unreal Engine 5构建UnrealPose-Gen离线渲染管道，生成包含3D关节坐标、2D投影、COCO风格关键点、边界框、相机参数等丰富标注的数据。创建包含8个序列约100万帧的UnrealPose-1M数据集，包含脚本化和随机化动作序列。

Result: 生成了高质量的合成人体姿态数据集，包含丰富的标注信息。在四个任务上进行了真实到合成的性能评估：图像到3D姿态、2D关键点检测、2D到3D提升、人物检测/分割。

Conclusion: 发布了UnrealPose-1M数据集和UnrealPose-Gen生成管道，为第三方生成人体姿态数据提供支持，有助于解决3D人体姿态数据稀缺的问题。
...
### [26] [WildIng: A Wildlife Image Invariant Representation Model for Geographical Domain Shift](https://arxiv.org/abs/2601.00993)
...
Method: 提出WildIng模型，通过整合文本描述（如物种外观的详细描述）与图像特征，创建对地理域转移更鲁棒的表示。该方法利用文本描述捕获一致的语义信息，减少对图像背景变化的依赖

Result: 实验表明WildIng将BioCLIP等基础模型在地理域转移条件下的准确率提升了30%。在非洲数据集上训练的模型在美国数据集上的准确率从16.17%显著提升

Conclusion: 结合文本描述与图像特征能有效提升野生动物识别模型在地理域转移下的泛化能力，WildIng为解决地理分布变化问题提供了有效方案
...
### [27] [DVGBench: Implicit-to-Explicit Visual Grounding Benchmark in UAV Imagery with Large Vision-Language Models](https://arxiv.org/abs/2601.00998)
...
TL;DR: 提出了DVGBench无人机隐式视觉定位基准数据集和DroneVG-R1模型，通过I2E-CoT方法将隐式查询转换为显式查询，提升无人机场景下的视觉定位能力


<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉语言模型的视觉定位数据集主要依赖显式参考表达（如相对位置、大小、颜色等），限制了在需要领域知识的隐式视觉定位任务上的性能

Method: 1) 构建DVGBench无人机隐式视觉定位基准数据集，覆盖6大应用场景，每个对象提供显式和隐式查询；2) 设计DroneVG-R1大视觉语言模型，集成隐式到显式思维链(I2E-CoT)和强化学习范式

Result: 评估主流模型在显式和隐式视觉定位任务上的表现，发现它们在推理能力方面存在显著局限性，为提升无人机代理的LVLMs推理能力提供了可行见解

Conclusion: DVGBench基准和DroneVG-R1模型通过将隐式查询转换为显式查询，有效降低了视觉定位难度，为提升无人机大视觉语言模型的推理能力提供了重要基础
...
### [28] [Lightweight Channel Attention for Efficient CNNs](https://arxiv.org/abs/2601.01002)
*Prem Babu Kanaparthi,Tulasi Venkata Sri Varshini Padamata*

Main category: cs.CV

TL;DR: 本文对通道注意力机制进行实证研究，比较SE、ECA和提出的LCA模块，LCA使用自适应一维卷积和分组操作减少参数，在CIFAR-10上达到竞争性准确率。
...
Result: LCA在CIFAR-10上达到竞争性准确率：ResNet-18为94.68%，MobileNetV2为93.10%，同时与ECA在参数效率上相当，并保持有利的推理延迟。提供了包括FLOPs、参数数量和GPU延迟测量的全面基准。

Conclusion: LCA模块在保持竞争性准确率的同时实现了参数效率，为在资源受限环境中部署注意力增强的CNN提供了实用见解。
...
### [29] [Decoupling Amplitude and Phase Attention in Frequency Domain for RGB-Event based Visual Object Tracking](https://arxiv.org/abs/2601.01022)
*Shiao Wang,Xiao Wang,Haonan Zhao,Jiarui Xu,Bo Jiang,Lin Zhu,Xin Zhao,Yonghong Tian,Jin Tang*

Main category: cs.CV

TL;DR: 提出一种新颖的RGB-Event视觉目标跟踪框架，在频域进行早期融合，通过运动引导的空间稀疏化减少计算开销，提升跟踪性能


<details>
  <summary>Details</summary>
Motivation: 现有RGB-Event跟踪方法主要依赖传统特征级融合，未能充分利用事件相机的高动态范围和运动敏感特性，同时低信息区域被统一处理，导致不必要的计算开销

Method: 1) 通过快速傅里叶变换将RGB和事件模态转换到频域，分离振幅和相位分量；2) 通过振幅和相位注意力选择性融合高频事件信息到RGB模态；3) 运动引导的空间稀疏化模块利用事件相机的运动敏感性过滤低信息区域；4) 稀疏的目标相关特征送入骨干网络学习，跟踪头预测最终目标位置

Result: 在FE108、FELT和COESOT三个RGB-Event跟踪基准数据集上的广泛实验证明了该方法的高性能和效率

Conclusion: 提出的频域早期融合和运动引导空间稀疏化框架有效利用了事件相机的独特优势，在减少计算开销的同时提升了RGB-Event跟踪性能
...
### [30] [ITSELF: Attention Guided Fine-Grained Alignment for Vision-Language Retrieval](https://arxiv.org/abs/2601.01024)
*Tien-Huy Nguyen,Huu-Loc Tran,Thanh Duc Ngo*

Main category: cs.CV

TL;DR: ITSELF：基于注意力引导的隐式局部对齐框架，用于文本行人搜索，无需额外监督即可学习细粒度图像-文本对应关系


<details>
  <summary>Details</summary>
Motivation: 现有基于局部对齐的方法容易陷入捷径学习和虚假相关性，导致错位问题；同时注入先验知识可能扭曲模态内部结构。研究发现编码器注意力从训练早期就能提供空间精确的证据，因此提出利用模型自身注意力来引导学习

Method: 提出ITSELF框架：1) GRAB模块将模型自身注意力转换为高显著性token的注意力库，并在该库上应用局部目标；2) MARS模块聚合跨层注意力并进行多样性感知的top-k选择；3) ATS模块在训练过程中从粗到细调度保留预算，早期保留上下文，后期聚焦判别性细节

Result: 在三个广泛使用的TBPS基准测试中实现了最先进的性能，并展现出强大的跨数据集泛化能力，无需额外先验监督

Conclusion: ITSELF框架通过利用模型自身注意力进行隐式局部对齐，有效解决了现有方法的局限性，在文本行人搜索任务中实现了高效、鲁棒的细粒度跨模态匹配
...
### [31] [Enhanced Leukemic Cell Classification Using Attention-Based CNN and Data Augmentation](https://arxiv.org/abs/2601.01026)
...
<details>
  <summary>Details</summary>
Motivation: 急性淋巴细胞白血病（ALL）是最常见的儿童癌症，需要专家显微镜诊断，但存在观察者间变异性和时间限制的问题。需要自动化、准确且可重复的分类系统来辅助临床诊断。

Method: 采用注意力卷积神经网络，结合EfficientNetV2-B3和Squeeze-and-Excitation机制。使用全面的数据增强、焦点损失处理类别不平衡、患者级数据分割确保评估鲁棒性。通过100次蒙特卡洛实验进行统计验证。

Result: 在C-NMC 2019数据集（12,528张图像，62名患者）上，测试集达到97.89%的F1分数和97.89%的准确率。相比基线方法有显著改进（p < 0.001），比现有方法提升高达4.67%，参数比VGG16少89%（15.2M vs. 138M）。

Conclusion: 现代注意力架构可以改善白血病细胞分类，同时保持适合临床部署的计算效率。注意力机制提供了可解释的诊断相关细胞特征可视化，证明了该管道在医疗图像分析中的有效性和实用性。
...
### [32] [Mono3DV: Monocular 3D Object Detection with 3D-Aware Bipartite Matching and Variational Query DeNoising](https://arxiv.org/abs/2601.01036)
...
<details>
  <summary>Details</summary>
Motivation: 传统DETR类架构在单目3D目标检测中存在关键限制：由于单目图像3D估计的固有不适定性，3D属性被排除在二分匹配过程之外，导致高质量3D预测可能被纯2D匹配标准错误抑制，造成次优结果。

Method: 1. 3D感知二分匹配策略：将3D几何信息直接纳入匹配成本，解决纯2D标准导致的错位问题；2. 3D去噪方案：在训练阶段稳定二分匹配；3. 变分查询去噪机制：解决传统去噪技术中的梯度消失问题，显著提升模型性能。

Result: 在不使用任何外部数据的情况下，该方法在KITTI 3D目标检测基准上实现了最先进的结果。

Conclusion: Mono3DV通过将3D几何信息整合到匹配过程中并解决相关的不稳定性问题，显著提升了单目3D目标检测的性能，为Transformer-based 3D检测提供了新的解决方案。
...
### [33] [Evaluating transfer learning strategies for improving dairy cattle body weight prediction in small farms using depth-image and point-cloud data](https://arxiv.org/abs/2601.01044)
...
TL;DR: 本研究评估了迁移学习在奶牛体重预测中的应用，比较了深度图像与点云两种模态的性能，发现迁移学习能显著提升小规模农场的预测效果，且两种模态表现相当。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉为奶牛监测提供了自动化、非侵入性和可扩展的工具，但迁移学习在畜牧业应用中的效果和优化策略尚不明确，且深度图像与点云两种模态在奶牛体重预测中的直接比较有限。

Method: 从大、中、小三个农场分别收集了1,201、215和58头奶牛的俯视深度图像和点云数据，评估了四种深度学习模型：ConvNeXt和MobileViT用于深度图像，PointNet和DGCNN用于点云，在三种实验设计下比较了迁移学习、单源学习和联合学习的性能。

Result: 迁移学习显著提升了小农场的体重预测性能，在所有四种模型中都优于单源学习，且效果与联合学习相当或更好；深度图像和点云模型之间没有一致的性能差异；预训练表示在不同农场条件和牛群中具有良好的泛化能力。

Conclusion: 迁移学习特别适合数据有限的小农场预测场景，因为它只需要预训练模型权重而非原始数据，避免了因隐私、物流或政策限制导致的数据共享问题。
...
### [34] [EgoGrasp: World-Space Hand-Object Interaction Estimation from Egocentric Videos](https://arxiv.org/abs/2601.01050)
*Hongming Fu,Wenjia Wang,Xiaozhen Qiao,Shuo Yang,Zheng Liu,Bo Zhao*

Main category: cs.CV

TL;DR: EgoGrasp：首个从动态单目第一人称视频重建世界空间手物交互的方法，解决了现有方法在时序建模和全局一致性上的不足。


<details>
  <summary>Details</summary>
Motivation: 准确的世界空间手物交互重建对于理解人类行为、实现具身智能和虚拟现实应用至关重要。现有方法局限于单图像或相机坐标系，无法建模时序动态或一致的全局轨迹，且在剧烈相机运动和频繁遮挡的第一人称视频中表现不佳。

Method: 提出多阶段框架：1）基于新型空间智能模型的鲁棒预处理流程；2）基于解耦扩散模型的全身手物交互先验模型（无模板、可扩展到多物体）；3）多目标测试时优化范式。

Result: 实验证明该方法在世界空间手物交互重建方面达到了最先进的性能。

Conclusion: EgoGrasp成功解决了从动态第一人称视频重建世界空间手物交互的挑战，为理解人类行为和实现相关应用提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 数字病理学时代需要自动化的图像分析技术，本研究旨在评估不同机器学习方法在组织病理学图像分类任务中的性能，特别是在噪声环境下的鲁棒性。

Method: 使用LC25000数据集（包含五类组织病理学图像），采用微调的InceptionResNet-v2网络作为分类器和特征提取器，比较了不同模型在深度特征上的表现，并评估了在不同信噪比条件下的模型鲁棒性。

Result: 微调的InceptionResNet-v2达到96.01%的准确率和96.8%的平均AUC；使用其深度特征训练的神经网络模型达到99.99%的AUC和99.84%的准确率；在噪声环境下，使用深度特征的模型（特别是GBM和KNN）表现出更好的鲁棒性。

Conclusion: 深度特征提取结合机器学习模型在组织病理学图像分类中表现出卓越性能，特别是在噪声环境下具有更好的鲁棒性，为临床实践中的自动化病理分析提供了有效解决方案。
...
### [36] [Efficient Hyperspectral Image Reconstruction Using Lightweight Separate Spectral Transformers](https://arxiv.org/abs/2601.01064)
*Jianan Li,Wangcai Zhao,Tingfa Xu*

Main category: cs.CV

TL;DR: 提出LSST轻量级分离光谱Transformer架构，用于高效重建压缩感知测量的高光谱图像，通过分组光谱自注意力和深度可分离卷积分别处理光谱和空间特征，并引入焦点光谱损失优化训练。


<details>
  <summary>Details</summary>
Motivation: 高光谱成像在多个领域有重要应用，但从压缩感知测量中高效重建高光谱图像面临挑战。需要利用高光谱图像独特的光谱和空间特性来设计更高效的架构。

Method: 采用分治策略，提出LSST架构：包含分离光谱Transformer块（SSTB）用于建模光谱关系，使用分组光谱自注意力和光谱洗牌操作；轻量级空间卷积块（LSCB）用于空间处理，采用深度可分离卷积和策略排序；并引入焦点光谱损失动态调整训练权重。

Result: 大量测试表明，LSST在实现优越性能的同时，需要更少的FLOPs和参数，证明了其高效性和有效性。

Conclusion: LSST通过创新的分治架构设计，成功解决了高光谱图像重建的效率问题，在保持高性能的同时显著降低了计算复杂度，为高光谱成像应用提供了实用的解决方案。
...
### [37] [A UAV-Based Multispectral and RGB Dataset for Multi-Stage Paddy Crop Monitoring in Indian Agricultural Fields](https://arxiv.org/abs/2601.01084)
*Adari Rama Sukanya,Puvvula Roopesh Naga Sri Sai,Kota Moses,Rimalapudi Sarvendranath*

Main category: cs.CV

TL;DR: 本文提出了一个大规模无人机采集的RGB和多光谱图像数据集，覆盖印度安得拉邦稻田从育苗到收获的所有生长阶段，包含42,430张原始图像（415GB），具有1厘米/像素的高分辨率。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏覆盖水稻完整生长阶段的高分辨率无人机图像数据集，特别是在印度地区。现有数据集往往分辨率不足或覆盖阶段不完整，限制了精准农业、疾病分析和产量估计等研究。

Method: 使用配备20兆像素RGB相机和5兆像素四波段多光谱相机的无人机，在印度安得拉邦Vijayawada地区5英亩稻田上采集数据。制定了标准化操作程序（SOP）和检查清单确保数据采集的可重复性。使用Pix4D Fields验证图像并生成正射影像图和植被指数图。

Result: 创建了包含42,430张原始图像（415GB）的数据集，地面采样距离为1厘米/像素，覆盖水稻所有生长阶段。数据集包含丰富的元数据（GPS坐标、飞行高度、环境条件），并生成了NDVI和NDRE等植被指数图。数据集已在IEEE DataPort上公开。

Conclusion: 该数据集是少数几个提供高分辨率图像并覆盖印度水稻作物所有生长阶段的资源之一，可支持精准喷洒、疾病分析和产量估计等精准农业研究，为相关领域提供了宝贵的数据基础。
...
<details>
  <summary>Details</summary>
Motivation: 为视觉生成模型开发一种通用、无需训练且具有概率认证的水印方法，确保水印检测的可靠性和跨模型兼容性。

Method: 基于块级亮度统计定义水印，预设二进制模式和阈值；利用水印引导技术作为即插即用机制，实现跨不同生成范式的水印注入。

Result: 在9个扩散、自回归和混合模型中评估，Luminark表现出高检测准确率、对常见图像变换的强鲁棒性以及良好的视觉质量。

Conclusion: Luminark为视觉生成模型提供了一种通用、无需训练且概率认证的水印解决方案，在检测准确性、鲁棒性和视觉质量方面表现优异。
...
### [39] [600k-ks-ocr: a large-scale synthetic dataset for optical character recognition in kashmiri script](https://arxiv.org/abs/2601.01088)
*Haq Nawaz Malik*

Main category: cs.CV

TL;DR: 本文介绍了600K-KS-OCR数据集，这是一个包含约60.2万个单词级分割图像的大规模合成语料库，专门用于训练和评估针对克什米尔文字的OCR系统。


<details>
  <summary>Details</summary>
Motivation: 克什米尔语是一种濒危的达尔德语系语言，使用改良的波斯-阿拉伯文字系统，约有700万人使用。该语言缺乏OCR系统训练资源，本数据集旨在填补这一关键资源缺口。

Method: 数据集生成方法包含：使用三种传统克什米尔字体、模拟真实世界文档退化的全面数据增强、多样化的背景纹理以增强模型鲁棒性。图像分辨率为256x64像素，提供多种格式的真实转录文本。

Result: 创建了包含约602,000个单词级分割图像的数据集，分布在10个分区存档中，总计约10.6 GB。数据集采用CC-BY-4.0许可证发布，兼容CRNN、TrOCR和通用机器学习流程。

Conclusion: 600K-KS-OCR数据集为低资源语言光学字符识别研究提供了重要资源，有助于推动克什米尔语OCR系统的发展，填补了该语言在数字文档处理方面的技术空白。
...
TL;DR: NarrativeTrack是首个通过细粒度实体中心推理评估MLLMs叙事理解能力的基准，使用组合推理进展(CRP)框架逐步增加叙事复杂性，揭示MLLMs在实体跟踪和时序推理方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频时序叙事理解方面存在不足，需要能够理解"谁在何时何地做什么"并保持跨动态视觉和时序上下文的连贯实体表示。

Method: 提出NarrativeTrack基准，使用组合推理进展(CRP)框架，通过三个维度逐步增加叙事复杂性：实体存在、实体变化和实体模糊性。采用全自动实体中心管道提取时序基础的实体表示。

Result: 评估显示：开源通用MLLMs具有强感知基础但弱时序连贯性；视频专用MLLMs能捕捉时序上下文但会幻觉实体上下文。模型在视觉转换和时序动态中无法稳健跟踪实体，常出现身份幻觉。

Conclusion: 叙事理解需要感知基础和时序推理的整合，两者存在根本权衡。NarrativeTrack为诊断和推进MLLMs中时序基础的叙事理解提供了首个系统框架。
...
### [41] [Evolving CNN Architectures: From Custom Designs to Deep Residual Models for Diverse Image Classification and Detection Tasks](https://arxiv.org/abs/2601.01099)
*Mahmudul Hasan,Mabsur Fatin Bin Hossain*

Main category: cs.CV

TL;DR: 本文比较了自定义CNN架构与预训练迁移学习模型在五个真实图像数据集上的表现，发现深层CNN在细粒度多分类任务中表现更好，而轻量级预训练模型在简单二分类任务中更有效。


<details>
  <summary>Details</summary>
Motivation: 研究动机是系统分析不同CNN架构（包括自定义架构、预训练和迁移学习模型）在不同复杂度图像识别任务中的表现差异，为实际应用提供网络设计选择指导。

Method: 方法包括：1）设计自定义CNN架构；2）使用广泛应用的预训练和迁移学习CNN模型；3）在五个真实图像数据集上进行对比实验，涵盖二分类、细粒度多分类和物体检测任务；4）分析网络深度、残差连接和特征提取策略等架构因素对性能的影响。

Result: 结果显示：1）深层CNN架构在细粒度多分类数据集上提供显著性能提升；2）轻量级预训练和迁移学习模型在简单二分类任务中仍然非常有效；3）提出的架构可成功扩展到物体检测场景，能够识别真实交通场景中的非法三轮车。

Conclusion: 结论是：通过系统分析自定义CNN架构与预训练迁移学习模型，本研究为根据任务复杂度和资源约束选择合适的网络设计提供了实用指导，强调了不同架构在不同应用场景中的适用性。
...
TL;DR: HAQAGen是一个统一生成模型，用于分辨率不变的红外到RGB彩色化，平衡色彩真实性和结构保真度。


<details>
  <summary>Details</summary>
Motivation: 解决红外到RGB转换中全局色彩统计对齐、局部色彩一致性保持以及纹理细节保留的挑战，实现高质量、分辨率不变的颜色重建。

Method: 1) 结合损失函数：通过可微分直方图匹配对齐全局色彩统计，使用感知图像质量度量和基于特征的相似性保留纹理信息；2) 通过SPADE注入局部色调-饱和度先验以稳定色彩重建；3) 在Mamba骨干网络中引入纹理感知监督以保留细节；4) 自适应分辨率推理引擎支持高分辨率转换。

Result: 在FANVID、OMSIV、VCIP2020和RGB2NIR数据集上评估，相比现有方法有显著改进。生成图像具有更锐利的纹理和自然的色彩，在感知指标上获得显著提升。

Conclusion: HAQAGen是一个可扩展且有效的红外到RGB转换解决方案，能在不同成像场景中保持纹理保真度和泛化能力。
...
### [43] [Cross-Layer Attentive Feature Upsampling for Low-latency Semantic Segmentation](https://arxiv.org/abs/2601.01167)
*Tianheng Cheng,Xinggang Wang,Junchao Liao,Wenyu Liu*

Main category: cs.CV

TL;DR: 提出了一种新的引导注意力插值（GAI）方法，用于高效语义分割，通过自适应插值细粒度高分辨率特征来解决特征对齐和语义信息不足的问题。
...
Method: 提出引导注意力插值（GAI）方法，通过确定不同分辨率特征中像素的空间和语义关系，利用这些关系来插值具有丰富语义的高分辨率特征。GAI可以与任何深度卷积网络集成。
...
### [44] [CardioMOD-Net: A Modal Decomposition-Neural Network Framework for Diagnosis and Prognosis of HFpEF from Echocardiography Cine Loops](https://arxiv.org/abs/2601.01176)
*Andrés Bell-Navas,Jesús Garicano-Mena,Antonella Ausiello,Soledad Le Clainche,María Villalba-Orero,Enrique Lara-Pezzi*

Main category: cs.CV

TL;DR: CardioMOD-Net：一个统一的AI框架，使用小鼠超声心动图视频进行HFpEF多类别诊断和连续发病时间预测


<details>
  <summary>Details</summary>
Motivation: HFpEF（射血分数保留的心力衰竭）源于多种合并症，进展缓慢且难以早期诊断和预后。现有AI模型主要关注人类HFpEF的二元检测，无法提供合并症特异性表型分析或疾病进展的时间估计。

Method: 使用四组小鼠超声心动图视频：对照组、高血糖组、肥胖组和系统性动脉高血压组。通过高阶动态模态分解（HODMD）提取时间特征，构建共享潜在表示支持视觉变换器，一个用于诊断分类器，另一个用于HFpEF发病时间预测的回归模块。

Result: 四组整体诊断准确率为65%，所有类别准确率超过50%。误分类主要反映肥胖组/高血压组与对照组的早期重叠。预后模块的HFpEF发病时间预测均方根误差为21.72周，肥胖组和高血压组预测最准确。预测的HFpEF发病时间与真实分布高度匹配。

Conclusion: 该统一框架证明，即使在数据量有限条件下，也能从单个超声心动图视频中获得多类别表型分析和连续的HFpEF发病预测，为临床前HFpEF研究中的诊断和预后建模整合提供了基础。
...
### [45] [GenCAMO: Scene-Graph Contextual Decoupling for Environment-aware and Mask-free Camouflage Image-Dense Annotation Generation](https://arxiv.org/abs/2601.01181)
*Chenglizhao Chen,Shaojiang Yuan,Xiaoxue Lu,Mengke Song,Jia Song,Zhenyu Wu,Wenfeng Song,Shuai Li*

Main category: cs.CV

TL;DR: 论文提出GenCAMO框架，通过生成模型合成高质量伪装图像-密集标注数据，解决伪装密集预测任务中数据稀缺问题，并构建了大规模多模态伪装数据集GenCAMO-DB。


<details>
  <summary>Details</summary>
Motivation: 伪装密集预测（如RGB-D伪装目标检测和开放词汇伪装目标分割）在复杂伪装场景理解中至关重要，但高质量、大规模、密集标注的伪装数据集稀缺，数据收集和标注成本高昂。

Method: 提出GenCAMO框架：1) 构建大规模多模态伪装数据集GenCAMO-DB，包含深度图、场景图、属性描述和文本提示；2) 开发环境感知、无需掩码的生成框架，生成高保真伪装图像-密集标注数据。

Result: 在多模态实验中，GenCAMO通过提供高质量合成数据，显著提升了复杂伪装场景下的密集预测性能。

Conclusion: 通过生成模型合成伪装图像-密集标注数据是解决数据稀缺问题的有效方法，GenCAMO框架和数据集为伪装密集预测任务提供了重要资源。
...
### [46] [Crowded Video Individual Counting Informed by Social Grouping and Spatial-Temporal Displacement Priors](https://arxiv.org/abs/2601.01192)
*Hao Lu,Xuhui Zhu,Wenjing Zhang,Yanan Li,Xiang Bai*

Main category: cs.CV

TL;DR: 本文提出OMAN++方法，通过引入社会分组先验和时空位移先验，将传统的一对一匹配放宽为一对多匹配，显著提升了拥挤场景下的视频个体计数性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频个体计数方法在拥挤场景（如地铁通勤）中表现不佳，需要新的数据集和方法来应对高密度、动态变化的行人流。

Method: 1) 构建WuhanMetroCrowd数据集，包含多种密度、时长、速度、视角和遮挡条件；2) 提出OMAN++方法，利用社会分组先验（放宽为一对多匹配）和时空位移先验（设计位移先验注入器），通过隐式上下文生成器和O2M匹配器实现。

Result: OMAN++在标准基准测试（SenseCrowd、CroHD、MovingDroneCrowd）上优于现有方法，在WuhanMetroCrowd数据集上误差降低38.12%，在拥挤场景中表现出明显优势。

Conclusion: 提出的OMAN++方法通过利用社会分组和时空位移先验，有效解决了拥挤场景下的视频个体计数问题，为拥挤动态行人流分析提供了强有力的基线方法。
...
### [47] [MS-ISSM: Objective Quality Assessment of Point Clouds Using Multi-scale Implicit Structural Similarity](https://arxiv.org/abs/2601.01200)
*Zhang Chen,Shuai Wan,Yuezhe Zhang,Siyu Ren,Fuzheng Yang,Junhui Hou*

Main category: cs.CV

TL;DR: 提出MS-ISSM方法，使用径向基函数表示点云局部特征，将失真测量转化为隐函数系数比较，避免不规则数据匹配误差，配合ResGrouped-MLP网络实现多尺度质量评估。


<details>
  <summary>Details</summary>
Motivation: 点云数据的不规则和无结构特性给客观质量评估带来挑战，特别是在建立准确感知特征对应关系方面存在困难。

Method: 1. MS-ISSM：使用径向基函数连续表示局部特征，将失真测量转化为隐函数系数比较；2. ResGrouped-MLP网络：采用分组编码策略结合残差块和通道注意力机制，分层处理亮度、色度和几何特征。

Result: 在多个基准测试中，MS-ISSM在可靠性和泛化能力方面均优于现有最先进的质量评估指标。

Conclusion: 提出的MS-ISSM方法有效解决了点云不规则数据匹配问题，通过隐式结构相似性测量和多尺度特征差异映射，实现了准确的点云质量评估。
...
### [48] [RefSR-Adv: Adversarial Attack on Reference-based Image Super-Resolution Models](https://arxiv.org/abs/2601.01202)
*Jiazhu Dai,Huihui Jiang*

Main category: cs.CV

TL;DR: 本文提出RefSR-Adv攻击方法，通过仅扰动参考图像来降低参考超分辨率系统的性能，揭示了RefSR模型过度依赖参考特征的安全漏洞。
...
Method: 提出RefSR-Adv对抗攻击方法，通过最大化对抗输出与干净输出之间的差异，仅扰动参考图像来降低超分辨率输出质量。

Result: RefSR-Adv在CNN、Transformer和Mamba架构上均能显著降低性能并产生严重伪影，实验证实低分辨率输入与参考图像相似度与攻击效果呈正相关。

Conclusion: 本研究揭示了RefSR系统因过度依赖参考特征而存在的安全漏洞，呼吁研究人员关注RefSR的鲁棒性问题。
...
### [49] [XStreamVGGT: Extremely Memory-Efficient Streaming Vision Geometry Grounded Transformer with KV Cache Compression](https://arxiv.org/abs/2601.01204)
...
<details>
  <summary>Details</summary>
Motivation: StreamVGGT虽然利用帧级因果注意力实现强流式重建，但存在KV缓存无限增长问题，导致内存消耗和推理延迟随输入帧数增加而不断上升

Method: 提出无需调优的方法，通过联合剪枝和量化系统性地压缩KV缓存：1) 通过高效token重要性识别剪枝多视图输入产生的冗余KV，实现固定内存预算；2) 利用KV张量的独特分布特性，引入KV量化进一步减少内存消耗

Result: XStreamVGGT在性能损失几乎可忽略的情况下，将内存使用减少4.42倍，推理速度提升5.48倍，实现了可扩展且实用的流式3D应用

Conclusion: XStreamVGGT通过系统性的KV缓存压缩技术，有效解决了流式3D视觉模型中的内存效率问题，为实际部署提供了可行的解决方案
...
### [50] [Real-Time LiDAR Point Cloud Densification for Low-Latency Spatial Data Transmission](https://arxiv.org/abs/2601.01210)
*Kazuhiko Murasaki,Shunsuke Konagai,Masakatsu Aoki,Taiga Yoshida,Ryuichi Tanida*

Main category: cs.CV

TL;DR: 提出一种高速LiDAR点云稠密化方法，通过结合多LiDAR输入与高分辨率彩色图像，使用联合双边滤波策略的CNN架构，实现实时（30fps）全高清稠密深度图生成。


<details>
  <summary>Details</summary>
Motivation: 为实现沉浸式远程呈现的低延迟空间传输系统，需要解决动态3D场景密集捕获和实时处理两大问题。LiDAR传感器能实时捕获3D但产生稀疏点云，因此需要高速点云稠密化方法来生成稠密3D场景。

Method: 结合多个LiDAR输入与高分辨率彩色图像，采用基于卷积神经网络架构的联合双边滤波策略，实现实时深度补全。

Result: 方法能以30fps实时生成全高清分辨率的稠密深度图，比最近的基于训练的深度补全方法快15倍以上，生成的稠密点云几何准确，无多视角不一致或重影伪影。

Conclusion: 该方法成功解决了LiDAR点云稀疏性问题，实现了高速、高质量的3D场景稠密化，为沉浸式远程呈现系统提供了可行的实时解决方案。
...
TL;DR: 本文提出了一种基于Segment Anything Model (SAM)的SAR图像雪崩分割方法，通过适配器、多编码器和提示工程策略解决领域差异问题，显著加速了SAR图像的雪崩标注工作。


<details>
  <summary>Details</summary>
Motivation: 合成孔径雷达(SAR)图像可用于雪崩检测，但训练有效模型需要大量高质量专家标注数据，这一过程耗时且昂贵。本文旨在通过改进SAM模型来加速SAR图像的雪崩标注流程。

Method: 针对SAR图像与自然图像的领域差异，采用适配器缓解领域差距；使用多编码器处理多通道SAR输入；通过提示工程策略提高雪崩定位精度；设计训练算法限制编码器训练时间以提高效率。

Result: 将改进后的模型集成到标注工具中，实验证明该方法能够显著加速SAR图像的雪崩标注过程。

Conclusion: 通过针对性地解决SAR图像与SAM模型之间的领域差异问题，成功开发了一个高效的雪崩分割系统，为SAR图像的快速标注提供了有效解决方案。
...
TL;DR: UniSH是一个统一的、前馈式的联合度量尺度3D场景和人体重建框架，通过创新的训练范式利用未标记的野外数据，解决了合成数据依赖导致的域差距问题。


<details>
  <summary>Details</summary>
Motivation: 该领域面临大规模标注真实世界数据稀缺的问题，导致过度依赖合成数据集，从而产生显著的模拟到真实域差距，导致泛化能力差、人体几何保真度低以及在野外视频中对齐不佳。

Method: 提出一个统一的训练范式，结合场景重建和人体姿态恢复的强先验，包含两个核心组件：(1) 从专家深度模型蒸馏高频细节的鲁棒蒸馏策略；(2) 两阶段监督方案：先在合成数据上学习粗略定位，然后通过在真实数据上直接优化SMPL网格与人体点云之间的几何对应关系进行微调。

Result: 模型在单次前向传播中能够联合恢复高保真场景几何、人体点云、相机参数和一致的度量尺度SMPL身体。在人体中心场景重建方面达到最先进性能，在全局人体运动估计方面获得高度竞争力的结果。

Conclusion: UniSH框架通过有效利用未标记野外数据，成功解决了合成数据依赖导致的域差距问题，实现了高质量的联合度量尺度3D场景和人体重建，在多个任务上超越了基于优化的框架和纯HMR方法。
...
TL;DR: CODA通过引入注册槽吸收残差注意力减少槽间干扰，并使用对比对齐损失增强槽与图像内容的对应关系，提升了对象中心学习的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的Slot Attention与预训练扩散模型结合用于对象中心学习存在槽纠缠和槽与图像内容对齐弱的问题，需要改进。

Method: 提出CODA方法：(1)使用注册槽吸收残差注意力，减少对象槽间的干扰；(2)应用对比对齐损失，显式鼓励槽与图像的对应关系。

Result: 在合成数据集(MOVi-C/E)和真实数据集(VOC, COCO)上，CODA在对象发现、属性预测和组合图像生成方面优于基线方法，如在COCO上FG-ARI提升6.1%。

Conclusion: CODA作为一种高效可扩展的框架，在复杂真实场景中具有稳健对象中心学习的应用潜力，注册槽仅增加可忽略的开销。
...
TL;DR: HyDRA是一个仅使用测量数据的深度平衡模型训练框架，通过结合测量一致性和自适应去噪正则化来解决图像重建问题，无需监督数据对。


<details>
  <summary>Details</summary>
Motivation: 图像重建问题（如A x = y）面临病态性和缺乏大规模监督数据集的挑战。现有深度平衡模型通常需要监督数据对(x,y)，但在许多实际场景中只有测量数据y可用。

Method: 提出HyDRA框架：1）仅使用测量数据训练深度平衡模型；2）结合测量一致性约束；3）引入自适应去噪正则化项；4）采用数据驱动的早停准则。

Result: 在稀疏视图CT重建实验中，HyDRA表现出具有竞争力的重建质量和快速推理速度。

Conclusion: HyDRA为仅使用测量数据的深度平衡模型训练提供了有效框架，在缺乏监督数据的情况下实现了高质量图像重建。
...
TL;DR: RFAssigner是一种新颖的标签分配策略，通过高斯感受野距离度量自适应补充正样本，解决密集目标检测中尺度不平衡问题。
...
Result: 在三个具有不同目标尺度分布的数据集上的综合实验验证了该方法的有效性和泛化性。配备RFAssigner的单个FCOS-ResNet-50检测器在所有目标尺度上都达到了最先进的性能，一致优于现有策略，且无需辅助模块或启发式方法。

Conclusion: RFAssigner通过自适应补充正样本有效解决了密集目标检测中的尺度不平衡问题，显著提升了多尺度学习能力，为标签分配策略提供了新的有效解决方案。
...
TL;DR: 提出MambaFormer混合专家框架，结合Transformer和状态空间模型专家，通过智能路由机制在医疗问答任务中实现计算效率与准确性的帕累托最优平衡。
...
Method: 提出LLM-based MambaFormer混合专家框架，包含轻量级门控机制进行token级动态路由：短复杂查询路由到定制Transformer专家(ET5)，长高吞吐序列路由到状态空间模型专家(EMamba)。使用新颖的效用引导多目标损失联合优化路由决策、专家利用率和计算成本。

Result: 在DentalQA和PubMedQA数据集上验证，MambaFormer取得BERTScore=0.9180的优异性能，推理延迟仅0.077秒，相比T5-Large实现24.4倍加速。

Conclusion: MambaFormer框架通过智能专家路由机制，在医疗问答任务中实现了计算效率与预测准确性的帕累托最优平衡，为资源受限的临床部署提供了可扩展的高效解决方案。
...
TL;DR: 本文评估了四种AI模型（三个CNN和一个Vision Transformer）在深度伪造检测中的表现，发现VFDNET结合MobileNetV3在准确性和效率方面表现最佳。
...
Method: 使用大规模人脸图像数据集评估了四种AI模型（三个CNN和一个Vision Transformer），并应用了数据预处理和数据增强技术来提升模型在不同场景下的性能。

Result: VFDNET结合MobileNetV3展现出卓越的准确率，同时保持了高效的性能表现，证明了AI在可靠深度伪造检测方面的能力。

Conclusion: AI技术能够有效应对深度伪造带来的挑战，VFDNET与MobileNetV3的组合为数字真实性验证提供了高效可靠的解决方案。
...
TL;DR: S2M-Net：一种轻量级医学图像分割网络，通过频谱选择令牌混合器和形态感知自适应损失，在保持全局上下文的同时实现高效计算，在16个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割面临局部精度、全局上下文和计算效率的三难困境。卷积网络局部精度高但感受野有限，视觉变换器全局上下文好但计算成本高且在小数据集上容易过拟合。

Method: 提出S2M-Net架构，包含两个核心创新：1) 频谱选择令牌混合器(SSTM)，利用医学图像的频谱集中特性，通过截断2D FFT和可学习频率滤波实现O(HW log HW)的全局上下文；2) 形态感知自适应分割损失(MASL)，自动分析结构特征来调制五个互补损失分量，无需手动调参。

Result: 在16个医学影像数据集（8种模态）上评估，达到最先进性能：息肉分割96.12% Dice，手术器械83.77%（比先前方法提升17.85%），脑肿瘤80.90%，相比专用基线一致提升3-18%，参数比基于变换器的方法少3.5-6倍。

Conclusion: S2M-Net成功解决了医学图像分割的三难困境，通过频谱方法和自适应损失实现了高效、准确的全局上下文建模，为临床部署提供了实用的解决方案。
...
TL;DR: VReID-XFD是一个用于极端远距离空中到地面行人重识别的视频基准测试和社区挑战，包含371个身份、11288个轨迹和1175万帧，覆盖5.8-120米高度、30-90度视角和120米水平距离，揭示了性能随高度和距离单调下降的规律。
...
Method: 从DetReIDX数据集构建VReID-XFD基准测试，包含371个身份、11288个轨迹和1175万帧，支持空中到空中、空中到地面和地面到空中评估，采用严格的身份分离划分，并提供丰富的物理元数据。

Result: VReID-XFD-25挑战吸引了10个团队提交数百份方案，系统分析显示性能随高度和距离单调下降，天底视角普遍处于劣势，存在峰值性能与鲁棒性之间的权衡，最佳SAS-PReID方法在空对地设置中仅达到43.93% mAP。

Conclusion: VReID-XFD为极端远距离空中到地面行人重识别提供了首个综合性基准测试，揭示了该领域的挑战性，现有方法性能有限，需要进一步研究来应对分辨率退化、视角变化等独特困难。
...
### [60] [Achieving Fine-grained Cross-modal Understanding through Brain-inspired Hierarchical Representation Learning](https://arxiv.org/abs/2601.01339)
*Weihang You,Hanqi Jiang,Yi Pan,Junhao Chen,Tianming Liu,Fei Dou*

Main category: cs.CV

TL;DR: NeuroAlign是一个新颖的fMRI-视频对齐框架，通过模仿人类视觉系统的层次结构，实现跨模态检索任务中的显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要基于将神经解码简化为生成任务或简单相关性，无法反映大脑视觉处理的层次性和时间过程。大脑表征的固有复杂性以及神经数据与视觉输入之间的模态差距使得理解神经对视觉刺激的反应仍然具有挑战性。

Method: NeuroAlign采用两阶段机制模仿生物视觉通路：1) 通过神经-时间对比学习(NTCL)实现全局语义理解，明确建模跨模态的双向预测时间动态；2) 通过增强的向量量化实现细粒度模式匹配。采用DynaSyncMM-EMA方法实现动态多模态融合和自适应加权。

Result: 实验表明，NeuroAlign在跨模态检索任务中显著优于现有方法，为理解视觉认知机制建立了新范式。

Conclusion: NeuroAlign通过模仿人类视觉系统的层次组织，成功解决了现有方法在反映大脑视觉处理层次性和时间动态方面的不足，为神经科学和计算机视觉的交叉研究提供了新方向。
...
### [61] [Slot-ID: Identity-Preserving Video Generation from Reference Videos via Slot-Based Temporal Identity Encoding](https://arxiv.org/abs/2601.01352)
*Yixuan Lai,He Wang,Kun Zhou,Tianjia Shao*

Main category: cs.CV

TL;DR: 提出使用短参考视频而非单张图像来生成保持指定身份的视频，通过Sinkhorn路由编码器学习紧凑的身份令牌，显著提升身份保持和运动自然度。


<details>
  <summary>Details</summary>
Motivation: 现有基于单张图像的视频生成方法存在三个主要问题：1）完全忽略时间特征导致姿势锁定运动；2）产生不自然的扭曲；3）在视角和表情变化时产生"平均"面孔。需要解决身份保持与运动自然性之间的平衡问题。

Method: 提出身份条件化的扩散-Transformer视频生成器，使用短参考视频而非单张肖像。核心创新是Sinkhorn路由编码器，从参考视频中学习紧凑的身份令牌，捕捉主体特定的动态模式（如微笑形成方式），同时保持与预训练骨干网络的兼容性。

Result: 方法在保持身份方面表现优异，特别是在大姿态变化和表情丰富的面部行为下。同时保持了提示忠实度和视觉真实感，适用于多样化的主体和提示。

Conclusion: 通过引入短参考视频和Sinkhorn路由编码器，有效解决了身份保持视频生成的挑战，在身份保留、运动自然度和提示忠实度之间取得了良好平衡。
...
### [62] [Advanced Machine Learning Approaches for Enhancing Person Re-Identification Performance](https://arxiv.org/abs/2601.01356)
*Dang H. Pham,Tu N. Nguyen,Hoa N. Nguyen*

Main category: cs.CV

TL;DR: 该论文提出了三种行人重识别方法：SCM-ReID（监督对比学习）、IQAGA/DAPRH（无监督域适应）和ViTC-UReID（完全无监督），分别在监督、域适应和无监督场景下显著提升了ReID性能。
...
- SCM-ReID在监督设置下达到SOTA性能
- UDA方法在跨域场景中mAP和Rank-1提升高达12%
- ViTC-UReID在无监督设置下大幅超越现有方法

Conclusion: 提出的三种方法有效解决了行人重识别中的特征学习、域适应和标签噪声处理等关键问题，为实际监控系统的鲁棒部署铺平了道路，推动了ReID研究的发展。
...
### [63] [Garment Inertial Denoiser (GID): Endowing Accurate Motion Capture via Loose IMU Denoiser](https://arxiv.org/abs/2601.01360)
*Jiawei Fang,Ruonan Zheng,Xiaoxia Gao,Shifan Jiang,Anjun Chen,Qi Ye,Shihui Guo*

Main category: cs.CV

TL;DR: 提出GID（服装惯性去噪器），一种轻量级Transformer，用于解决宽松服装中IMU传感器位移导致的运动捕捉精度问题，通过位置感知专家架构实现准确、实时的去噪。


<details>
  <summary>Details</summary>
Motivation: 可穿戴惯性运动捕捉系统需要紧密贴合的传感器，这在日常使用中不舒适且不实用。将IMU嵌入宽松服装是理想方案，但传感器与身体之间的位移会导致严重、结构化且位置相关的噪声，破坏标准惯性处理流程。

Method: 提出GID框架，包含三个阶段：位置特定去噪、自适应跨服装融合和通用姿态预测。采用位置感知专家架构，共享时空骨干网络建模全局运动，每个IMU专家头专门处理局部服装动态，轻量级融合模块确保跨部位一致性。

Result: GID能够从单用户训练中实现准确、实时的去噪，并在未见过的用户、动作和服装类型上具有良好的泛化能力。作为即插即用模块，能持续改进最先进的惯性运动捕捉方法。

Conclusion: GID通过创新的位置感知专家架构有效解决了宽松服装中IMU传感器位移问题，结合新收集的GarMoCap数据集，为舒适、实用的可穿戴运动捕捉提供了可行解决方案。
...
### [64] [Unsupervised SE(3) Disentanglement for in situ Macromolecular Morphology Identification from Cryo-Electron Tomography](https://arxiv.org/abs/2601.01364)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于期望最大化的方法经常遗漏重要但稀有的分子形态，且需要大量手动超参数调优，需要更有效的解决方案

Method: 提出解耦深度表示学习框架，包含新颖的多选择学习模块，将SE(3)变换与形态内容在表示空间中分离，并利用学习的形态内容生成模板形态

Result: 在模拟和真实cryo-ET数据集上的实验显示明显优于先前方法的改进，包括发现了先前未识别的大分子形态

Conclusion: 该框架有效解决了cryo-ET数据中分子形态分析的挑战，能够发现稀有但重要的形态，减少了对人工调参的依赖
...
### [65] [ParkGaussian: Surround-view 3D Gaussian Splatting for Autonomous Parking](https://arxiv.org/abs/2601.01386)
...
TL;DR: 提出ParkRecon3D首个停车场景重建基准和ParkGaussian框架，利用3D高斯泼溅技术实现高质量停车场景重建，并通过槽位感知策略提升下游停车位检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶停车研究主要集中在2D停车位感知、建图和定位，而3D重建研究不足。停车场景具有复杂的空间几何结构，单纯提高重建视觉质量并不能直接提升自动驾驶停车性能，因为停车的关键入口是停车位感知模块。

Method: 1. 创建ParkRecon3D基准数据集，包含四个环视鱼眼相机的传感器数据、标定外参和密集停车位标注。2. 提出ParkGaussian框架，首次将3D高斯泼溅技术应用于停车场景重建。3. 引入槽位感知重建策略，利用现有停车感知方法增强停车位区域合成质量，提升重建与下游检测的对齐度。

Result: 在ParkRecon3D基准上的实验表明，ParkGaussian实现了最先进的重建质量，并更好地保持了与下游任务的感知一致性。

Conclusion: 该研究填补了停车场景3D重建的空白，提出的ParkRecon3D基准和ParkGaussian框架为自动驾驶停车系统提供了有效的3D重建解决方案，通过槽位感知策略实现了重建与下游感知任务的更好对齐。
...
### [66] [Evaluation of Convolutional Neural Network For Image Classification with Agricultural and Urban Datasets](https://arxiv.org/abs/2601.01393)
*Shamik Shafkat Avro,Nazira Jesmin Lina,Shahanaz Sharmin*

Main category: cs.CV

TL;DR: 开发了一个自定义CNN架构来研究设计选择对多领域图像分类的影响，在智能城市和农业数据集上实现了竞争性性能
...
Method: 设计了一个自定义CNN架构，包含残差连接、Squeeze-and-Excitation注意力机制、渐进通道缩放和Kaiming初始化，在5个公开数据集上进行训练和测试

Result: CustomCNN在多个数据集上表现出竞争性性能，同时保持计算效率，与流行的CNN架构相比具有优势

Conclusion: 精心设计的架构对于现实世界的智能城市和农业成像应用至关重要，CustomCNN展示了架构设计选择对性能的重要影响
...
### [67] [SwinIFS: Landmark Guided Swin Transformer For Identity Preserving Face Super Resolution](https://arxiv.org/abs/2601.01406)
*Habiba Kausar,Saeed Anwar,Omar Jamal Hammad,Abdul Bais*

Main category: cs.CV

TL;DR: SwinIFS是一个基于地标引导的人脸超分辨率框架，通过结合结构先验和分层注意力机制，在中等和极端放大倍数下实现身份保持的重建。


<details>
  <summary>Details</summary>
Motivation: 人脸超分辨率在从严重退化的低分辨率输入中恢复高质量面部图像时面临挑战，因为会丢失精细的结构细节和身份特定特征。现有方法在极端放大倍数下难以恢复有意义的结构。

Method: 提出SwinIFS框架，将关键面部地标的密集高斯热图集成到输入表示中，使网络从处理的最早阶段就能关注语义重要的面部区域。采用紧凑的Swin Transformer骨干网络来捕获长距离上下文信息，同时保持局部几何结构。

Result: 在CelebA基准测试上的广泛实验表明，SwinIFS实现了优越的感知质量、更清晰的重建和改进的身份保持；即使在8倍放大下也能产生更逼真的结果，而大多数方法在此情况下无法恢复有意义的结构。

Conclusion: SwinIFS在重建准确性和计算效率之间提供了有利的平衡，使其适用于面部增强、监控和数字修复等实际应用。该方法在极端放大倍数下仍能保持身份特征和结构一致性。
...
### [68] [Mask-Guided Multi-Task Network for Face Attribute Recognition](https://arxiv.org/abs/2601.01408)
*Gong Gao,Zekai Wang,Jian Zhao,Ziqi Xie,Xianhui Liu,Weidong Zhao*

Main category: cs.CV

TL;DR: 提出MGMTN网络，通过自适应掩码学习和组-全局特征融合，选择特定面部区域特征，减少全局特征冗余，提升面部属性识别性能。


<details>
  <summary>Details</summary>
Motivation: 传统多任务属性识别方法依赖全局区域特征提取，会产生冗余特征，影响识别效率和准确性。需要更精确地定位关键面部区域进行特征学习。

Method: 提出掩码引导多任务网络(MGMTN)，包含自适应掩码学习(AML)和组-全局特征融合(G2FF)。AML利用预训练关键点模型和全卷积网络定位关键面部部位并生成组掩码；G2FF融合组特征和全局特征增强学习。

Result: 在两个具有挑战性的面部属性识别数据集上进行广泛实验，证明MGMTN能有效提升面部属性识别性能。

Conclusion: MGMTN通过选择特定特征区域和融合组-全局特征，解决了传统方法中全局特征冗余的问题，实现了更精确的面部属性识别。
...
### [69] [AirSpatialBot: A Spatially-Aware Aerial Agent for Fine-Grained Vehicle Attribute Recognization and Retrieval](https://arxiv.org/abs/2601.01416)
*Yue Zhou,Ran Ding,Xue Yang,Xue Jiang,Xingzhao Liu*

Main category: cs.CV

TL;DR: 提出AirSpatial数据集和AirSpatialBot代理，通过两阶段训练策略提升遥感视觉语言模型的空间理解能力，解决现有模型在空间理解方面的不足


<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉语言模型在空间理解方面存在局限，限制了其在实际应用中的效果，特别是针对无人机拍摄的车辆图像

Method: 1) 创建包含20.6万条指令的AirSpatial数据集，引入空间定位和空间问答两个新任务，首次提供3DBB标注；2) 采用两阶段训练策略：图像理解预训练和空间理解微调；3) 开发AirSpatialBot代理，动态整合任务规划、图像理解、空间理解和任务执行能力

Result: 实验验证了方法的有效性，揭示了现有VLMs的空间局限性，同时提供了有价值的见解。模型能够实现细粒度车辆属性识别和检索

Conclusion: 通过AirSpatial数据集和两阶段训练策略，成功提升了遥感视觉语言模型的空间理解能力，开发的AirSpatialBot代理能够适应多样化的查询需求，为遥感应用提供了新的解决方案
...
### [70] [DreamID-V:Bridging the Image-to-Video Gap for High-Fidelity Face Swapping via Diffusion Transformer](https://arxiv.org/abs/2601.01425)
*Xu Guo,Fulong Ye,Xinghui Li,Pengqi Tu,Pengze Zhang,Qichao Sun,Songtao Zhao,Xiangwang Hou,Qian He*

Main category: cs.CV

TL;DR: DreamID-V：首个基于扩散Transformer的视频换脸框架，通过模态感知条件注入、合成到真实课程机制和身份一致性强化学习，在保持身份相似性和时间一致性的同时实现高质量视频换脸。


<details>
  <summary>Details</summary>
Motivation: 现有视频换脸方法在保持身份相似性、属性保留和时间一致性方面存在困难，需要将图像换脸的优势无缝迁移到视频领域，并解决缺乏全面基准测试的问题。

Method: 1. SyncID-Pipe数据管道预训练身份锚定视频合成器，结合图像换脸模型构建双向ID四元组进行显式监督；2. 基于扩散Transformer的DreamID-V框架，使用模态感知条件模块区分注入多模态条件；3. 合成到真实课程机制和身份一致性强化学习策略；4. 引入IDBench-V综合基准测试。

Result: DreamID-V在实验中优于现有最先进方法，展现出卓越的泛化能力，可以无缝适应各种换脸相关任务。

Conclusion: 该研究提出了一个全面的视频换脸框架，通过创新的数据管道、模型架构和训练策略，成功解决了视频换脸中的身份相似性、属性保留和时间一致性挑战，并提供了全面的基准测试。
...
### [71] [EdgeNeRF: Edge-Guided Regularization for Neural Radiance Fields from Sparse Views](https://arxiv.org/abs/2601.01431)
*Weiqi Yu,Yiyang Yao,Lin He,Jianming Lv*

Main category: cs.CV

TL;DR: EdgeNeRF：一种边缘引导的稀疏视图3D重建算法，通过深度和法线正则化约束非边缘区域，在保持边界高频细节的同时增强几何一致性。


<details>
  <summary>Details</summary>
Motivation: NeRF在密集多视图场景中表现优异，但在稀疏输入下重建质量显著下降，出现几何伪影。现有方法使用全局深度正则化来减轻伪影，但会导致几何边界细节丢失。

Method: 利用深度和法线突变产生边缘的先验知识。首先从输入图像中提取边缘，然后在非边缘区域应用深度和法线正则化约束，增强几何一致性同时保持边界处的高频细节。

Result: 在LLFF和DTU数据集上的实验表明，EdgeNeRF在保留锐利几何边界和抑制伪影方面具有优越性能。边缘引导的深度正则化模块可以即插即用地集成到其他方法中，显著提升性能而不大幅增加训练时间。

Conclusion: EdgeNeRF通过边缘引导的稀疏视图3D重建，有效解决了现有方法在几何边界细节保留方面的不足，实现了更好的几何一致性和边界细节保持。
...
### [72] [In defense of the two-stage framework for open-set domain adaptive semantic segmentation](https://arxiv.org/abs/2601.01439)
*Wenqi Ren,Weijie Wang,Meng Zheng,Ziyan Wu,Yang Tang,Zhun Zhong,Nicu Sebe*

Main category: cs.CV

TL;DR: SATS提出了一种分离-适应训练策略，通过先分离已知/未知类别再进行未知感知域适应的两阶段方法，解决了开放集语义分割域适应中已知类负迁移和未知类欠拟合的问题。


<details>
  <summary>Details</summary>
Motivation: 现有开放集语义分割域适应方法通常在单一阶段同时处理已知类适应和未知类识别，但由于已知类和未知类标注不平衡，这会导致已知类负迁移和未知类欠拟合的问题。

Method: 提出SATS（分离-适应训练策略），包含两个顺序步骤：1）已知/未知类别分离，2）未知感知域适应。还提出了硬未知探索数据增强方法，让模型接触更具挑战性的未知样本。

Result: 在公开OSDA-SS基准测试中取得显著提升：GTA5-to-Cityscapes提升+3.85% H-Score，SYNTHIA-to-Cityscapes提升+18.64% H-Score，优于先前最先进方法。

Conclusion: 通过分离已知/未知类别学习再进行未知感知域适应的两阶段策略，能够平衡已知类和未知类的特征学习，有效提升开放集语义分割域适应性能。
...
### [73] [PartImageNet++ Dataset: Enhancing Visual Models with High-Quality Part Annotations](https://arxiv.org/abs/2601.01454)
*Xiao Li,Zilong Liu,Yining Liu,Zhuhong Li,Na Dong,Sitian Qin,Xiaolin Hu*

Main category: cs.CV

TL;DR: PartImageNet++ (PIN++) 是一个包含 ImageNet-1K 所有类别详细部件标注的数据集，每类100张图像，共10万张。基于此提出了多尺度部件监督识别模型（MPM），通过部件分割网络生成伪标签，结合原始标注进行监督学习，提升了目标识别的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有数据集中高质量部件标注稀缺，限制了部件级视觉理解的发展。需要构建一个覆盖广泛对象类别、标注详细的部件数据集，以支持更鲁棒的识别模型和下游任务。

Method: 1) 构建PartImageNet++数据集，为ImageNet-1K所有类别提供详细部件标注；2) 训练部件分割网络生成伪标签；3) 提出MPM模型，在传统识别架构中加入辅助旁路层，同时使用伪标签和原始标注进行监督学习。

Result: 1) 创建了最全面的部件标注数据集PIN++；2) MPM模型提升了基于部件的目标识别鲁棒性；3) 在部件分割、目标分割和少样本学习等下游任务上建立了强基线，证明了部件标注对提升模型性能的潜力。

Conclusion: PIN++数据集填补了高质量部件标注的空白，MPM模型展示了部件监督学习的有效性。部件标注不仅提升了目标识别性能，还为多种下游任务提供了有价值的监督信号，具有广泛的应用潜力。
...
### [74] [Rethinking Multimodal Few-Shot 3D Point Cloud Segmentation: From Fused Refinement to Decoupled Arbitration](https://arxiv.org/abs/2601.01456)
*Wentao Bian,Fenglei Xu*

Main category: cs.CV

TL;DR: DA-FSS提出了一种解耦专家仲裁的少样本3D点云语义分割方法，通过并行专家细化模块分离几何和语义路径，解决传统"融合后细化"范式中的"可塑性-稳定性困境"和CLIP的类间混淆问题。


<details>
  <summary>Details</summary>
Motivation: 传统多模态少样本3D点云分割中的"融合后细化"范式存在"可塑性-稳定性困境"：几何信息需要灵活性适应新类别，而语义信息需要稳定性保持已有知识。同时CLIP的类间混淆会导致语义盲区，限制了多模态信息的有效利用。

Method: 提出DA-FSS模型，包含：1) 并行专家细化模块，分离几何专家（保持可塑性）和语义专家（确保稳定性）；2) 堆叠仲裁模块进行卷积融合和模态路径仲裁；3) 解耦对齐模块在路径间传递知识而不传播混淆。使用与MM-FSS相同的骨干网络和预训练文本编码器。

Result: 在S3DIS和ScanNet等流行数据集上，DA-FSS优于MM-FSS基线方法。在几何边界、完整性和纹理区分等方面都表现出优越性，提高了自由模态的利用率并更好地利用了各模态信息空间。

Conclusion: 通过解耦几何和语义路径并相互正则化梯度，DA-FSS有效解决了多模态少样本3D点云分割中的可塑性-稳定性困境和语义盲区问题，实现了更好的泛化性能。
...
### [75] [Language as Prior, Vision as Calibration: Metric Scale Recovery for Monocular Depth Estimation](https://arxiv.org/abs/2601.01457)
...
<details>
  <summary>Details</summary>
Motivation: 单目度量深度估计存在尺度不确定性和领域偏移敏感性问题，而相对深度基础模型虽然迁移性好，但无法直接提供度量深度。需要一种方法在保持相对深度模型优势的同时恢复度量信息。

Method: 在冻结的相对深度骨干网络和CLIP文本编码器基础上，使用语言预测不确定性感知的校准参数包络，然后利用多尺度视觉特征从包络中选择图像特定的校准参数。通过逆深度的闭式最小二乘解提供监督信号。

Result: 在NYUv2和KITTI数据集上提高了域内精度，在SUN-RGBD和DDAD上的零样本迁移表现出比纯语言基线更好的鲁棒性。

Conclusion: 通过语言引导的不确定性校准方法，能够在保持相对深度模型优势的同时有效恢复度量深度，提高了跨域鲁棒性和准确性。
...
### [76] [Domain Adaptation of Carotid Ultrasound Images using Generative Adversarial Network](https://arxiv.org/abs/2601.01460)
*Mohd Usama,Belal Ahmad,Christer Gronlund,Faleh Menawer R Althiyabi*

Main category: cs.CV

TL;DR: 提出基于GAN的域适应方法，用于解决超声图像中因设备差异导致的纹理和混响噪声问题，通过图像转换实现跨设备域适应。
...
Method: 提出基于生成对抗网络（GAN）的域适应模型，将域适应任务构建为图像到图像转换问题。该方法转换源域图像的纹理模式并去除混响噪声，使其与目标域图像对齐，同时保持图像内容不变。

Result: 在两个包含三个不同域的颈动脉超声图像数据集上验证，模型成功转换图像纹理模式并去除混响噪声。与无适应相比，提出的模型在直方图相关性（0.960 vs 0.916）和巴氏距离（0.040 vs 0.090）等指标上表现更优，且优于CycleGAN方法。

Conclusion: 提出的基于GAN的域适应方法能有效解决超声图像中的设备差异问题，实现跨设备域适应，为医学影像分析提供了一种高效的解决方案。
...
### [77] [Robust Ship Detection and Tracking Using Modified ViBe and Backwash Cancellation Algorithm](https://arxiv.org/abs/2601.01481)
*Mohammad Hassan Saghafi,Seyed Majid Noorhosseini,Seyed Abolfazl Seyed Javadein,Hadi Khalili*

Main category: cs.CV

TL;DR: 提出一种用于海岸视频序列中船舶检测与跟踪的鲁棒实时方法，改进ViBe算法降低船舶丢失概率，并设计新的尾流消除方法。


<details>
  <summary>Details</summary>
Motivation: 海岸场景具有不可预测性和动态特性，需要能够适应这些条件的鲁棒检测方法。传统方法在船舶检测中容易受到海浪、光线变化和船舶尾流的影响。

Method: 1. 改进ViBe算法用于运动目标检测，降低船舶丢失概率，增强对海浪和光线变化的鲁棒性，实现快速背景更新。2. 基于船舶几何特性和亮度失真等概念，提出新的尾流消除方法。

Result: 实验结果表明，所提出的策略和方法在船舶检测和跟踪方面具有出色性能，同时实现了实时和精确的处理效果。

Conclusion: 提出的改进ViBe算法结合尾流消除方法，能够有效应对海岸视频中的复杂动态场景，实现鲁棒的实时船舶检测与跟踪。
...
### [78] [Unified Generation and Self-Verification for Vision-Language Models via Advantage Decoupled Preference Optimization](https://arxiv.org/abs/2601.01483)
*Xinyu Qiu,Heng Jia,Zhengwen Zeng,Shuheng Shen,Changhua Meng,Yi Yang,Linchao Zhu*

Main category: cs.CV

TL;DR: ADPO提出统一的强化学习框架，在单一策略中联合学习答案生成和自我验证，通过偏好验证奖励和解耦优化机制，显著提升验证能力和推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有并行测试时扩展方法通常需要分别训练生成和验证模型，导致高昂的训练和推理成本。需要一种统一框架来同时优化生成和验证能力。

Method: ADPO包含两个创新：1) 偏好验证奖励，通过正负样本的均值验证分数作为决策阈值，在预测正确性与答案正确性一致时提供正向反馈；2) 优势解耦优化，分别为生成和验证计算优势，应用token掩码隔离梯度，结合掩码GRPO目标，在保持生成质量的同时校准验证分数。

Result: ADPO实现了高达+34.1%的验证AUC提升和-53.5%的推理时间降低，在MathVista/MMMU上准确率分别提升+2.8%/+1.4%，ReasonSeg上提升+1.9 cIoU，AndroidControl/GUI Odyssey上步骤成功率分别提升+1.7%/+1.0%。

Conclusion: ADPO通过统一的强化学习框架成功解决了生成和验证分离训练的问题，在保持生成质量的同时显著提升了验证能力和推理效率，为联合优化生成和验证任务提供了有效解决方案。
...
### [79] [Higher-Order Domain Generalization in Magnetic Resonance-Based Assessment of Alzheimer's Disease](https://arxiv.org/abs/2601.01485)
*Zobia Batool,Diala Lteif,Vijaya B. Kolachalama,Huseyin Ozkan,Erchan Aptoula*

Main category: cs.CV

TL;DR: 提出Extended MixStyle (EM)框架，通过混合高阶特征矩（偏度和峰度）来模拟多样化的分布变化，提升阿尔茨海默病MRI分类模型在未见数据集上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在阿尔茨海默病诊断方面取得进展，但基于结构磁共振成像训练的模型在不同数据集（由于扫描仪、协议和患者人口统计学差异）上表现不佳。单域泛化在碎片化的AD数据集中至关重要但研究不足。

Method: 提出Extended MixStyle (EM)框架，通过混合高阶特征矩（偏度和峰度）来模拟多样化的分布变化。使用NACC数据集（n=4,647）训练模型区分正常认知、轻度认知障碍和AD，在三个未见数据集（总计n=3,126）上测试。

Result: EM在跨域性能上表现优异，相比最先进的单域泛化基准，平均macro-F1提高了2.4个百分点，展示了在异构现实世界环境中实现不变、可靠的AD检测的潜力。

Conclusion: Extended MixStyle框架通过模拟高阶特征分布变化，有效提升了AD分类模型在不同数据集上的泛化能力，为解决现实世界医疗数据异质性问题提供了有前景的解决方案。
...
### [80] [DeepInv: A Novel Self-supervised Learning Approach for Fast and Accurate Diffusion Inversion](https://arxiv.org/abs/2601.01487)
*Ziyue Zhang,Luxi Lin,Xiaolin Hu,Chao Chang,HuaiXi Wang,Yiyi Zhou,Rongrong Ji*

Main category: cs.CV

TL;DR: DeepInv：首个可训练的自监督扩散反演方法，通过伪噪声生成和迭代多尺度训练实现快速准确的图像到噪声映射
...
Method: 提出自监督目标函数和数据增强策略生成高质量伪噪声，采用迭代多尺度训练机制训练参数化反演求解器，实现逐步预测反演噪声

Result: 在COCO数据集上，DeepInv比EasyInv提升40.435% SSIM，比ReNoise快9887.5%，在性能和推理速度上均显著优于现有方法

Conclusion: DeepInv是首个提出可训练求解器逐步预测反演噪声的方法，其创新设计为社区提供了新的思路，代码和模型参数将开源
...
### [81] [DiffKD-DCIS: Predicting Upgrade of Ductal Carcinoma In Situ with Diffusion Augmentation and Knowledge Distillation](https://arxiv.org/abs/2601.01507)
*Tao Li,Qing Li,Na Li,Hui Xie*

Main category: cs.CV

TL;DR: 提出DiffKD-DCIS框架，结合条件扩散模型和知识蒸馏，用于预测DCIS升级为IDC，在数据有限情况下提升超声图像质量和模型泛化能力。
...
Method: 提出三阶段框架：1）条件扩散模型生成高质量超声图像用于数据增强；2）深度教师网络从原始和合成数据中提取鲁棒特征；3）紧凑学生网络通过知识蒸馏从教师网络学习，平衡泛化能力和计算效率。

Result: 在1,435例多中心数据集上评估，合成图像质量良好。学生网络参数更少、推理更快。在外部测试集上优于部分组合方法，准确率与资深放射科医生相当，优于初级医生，显示显著临床潜力。

Conclusion: DiffKD-DCIS框架有效解决了DCIS升级预测中的数据稀缺和泛化问题，通过条件扩散模型增强数据和知识蒸馏提升性能，具有重要临床应用价值。
...
### [82] [A Novel Deep Learning Method for Segmenting the Left Ventricle in Cardiac Cine MRI](https://arxiv.org/abs/2601.01512)
...
<details>
  <summary>Details</summary>
Motivation: 传统CNN分割方法在心脏MRI分割中往往忽略上下文信息，需要开发能够更好理解医学图像上下文的新型网络结构，以提高左心室分割的精度，为手术机器人和医学分析提供更准确的支持。

Method: 采用组批归一化U-Net框架，包含下采样路径用于特征提取和上采样路径用于细节恢复，针对医学影像进行了专门优化，并引入了改进上下文理解的关键技术修改。

Result: 在45名患者的805个左心室MRI扫描数据集上，GBU-Net显著提高了分割精度，超越了现有方法的标准指标（如Dice系数和平均垂直距离），在SunnyBrook测试数据集上达到97%的Dice分数。

Conclusion: GBU-Net通过其创新设计在左心室分割中提供了增强的精度和上下文理解能力，在医学影像分割任务中表现出色，特别适用于手术机器人和医学分析应用。
...
### [83] [FastV-RAG: Towards Fast and Fine-Grained Video QA with Retrieval-Augmented Generation](https://arxiv.org/abs/2601.01513)
*Gen Li,Peiyu Liu*

Main category: cs.CV

TL;DR: VideoSpeculateRAG：结合推测解码与检索增强的高效视觉语言模型框架，通过轻量级草稿模型生成候选答案、重量级模型验证优化，在保持准确性的同时将推理速度提升约2倍。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在整合外部知识方面仍有困难，检索增强生成方法效率低下且难以保持高质量答案。需要解决推理延迟和实体识别错误两大挑战。

Method: 提出VideoSpeculateRAG框架：1）推测解码管道：轻量级草稿模型快速生成多个答案候选，重量级模型验证和优化；2）相似性过滤策略：基于相似度的过滤机制改善检索知识中的实体对齐，提高答案准确性。

Result: 实验显示VideoSpeculateRAG在保持或提高准确性的同时，推理速度比标准RAG方法提升约2倍，有效解决了实体识别错误问题。

Conclusion: 该框架展示了推测解码与检索增强推理结合在复杂知识密集型多模态任务中的潜力，能同时提升效率和可靠性。
...
### [84] [BARE: Towards Bias-Aware and Reasoning-Enhanced One-Tower Visual Grounding](https://arxiv.org/abs/2601.01526)
...
Method: BARE引入三个新模块：语言显著性调制器、视觉偏置校正和指称关系增强，共同减轻多模态干扰并增强指称理解。
...
### [85] [DrivingGen: A Comprehensive Benchmark for Generative Video World Models in Autonomous Driving](https://arxiv.org/abs/2601.01528)
*Yang Zhou,Hao Shao,Letian Wang,Zhuofan Zong,Hongsheng Li,Steven L. Waslander*

Main category: cs.CV

TL;DR: DrivingGen是首个用于生成式驾驶世界模型的综合基准，包含多样化数据集和新颖评估指标，揭示了现有模型在视觉质量与物理准确性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 当前驾驶世界模型研究缺乏严谨的基准来测量进展和指导优先级。现有评估存在多个局限：通用视频指标忽视安全关键因素；轨迹合理性很少量化；时间和智能体一致性被忽略；对自我条件控制性的评估缺失；且现有数据集未能覆盖真实部署所需的多样性条件。

Method: 提出DrivingGen基准，结合从驾驶数据集和互联网规模视频源中筛选的多样化评估数据集，涵盖不同天气、时间、地理区域和复杂操作，并开发了一套新的评估指标，共同评估视觉真实性、轨迹合理性、时间一致性和可控性。

Result: 对14个最先进模型的基准测试揭示了明确的权衡：通用模型看起来更好但违反物理规律，而驾驶专用模型能真实捕捉运动但视觉质量较差。DrivingGen提供了一个统一的评估框架。

Conclusion: DrivingGen基准能够促进可靠、可控和可部署的驾驶世界模型的发展，支持可扩展的仿真、规划和数据驱动的决策制定。
...
<details>
  <summary>Details</summary>
Motivation: 传统灵活图像分词器使用嵌套dropout训练，通过截断尾部令牌实现可变长度，但这导致图像信息过度集中在早期令牌，限制了自回归图像生成的效果，特别是当令牌长度增加时。

Method: 提出ReToK方法：1）冗余令牌填充：更频繁地激活尾部令牌，缓解信息过度集中在早期令牌的问题；2）分层语义正则化：将早期令牌的解码特征与预训练视觉基础模型对齐，同时向尾部逐渐降低正则化强度，允许更精细的低级细节重建。

Result: 在ImageNet 256×256数据集上，ReToK在生成性能上优于现有的灵活和固定长度分词器。

Conclusion: ReToK通过冗余令牌填充和分层语义正则化有效解决了灵活图像分词器中信息分布不均的问题，显著提升了自回归图像生成的质量。
...
TL;DR: FAR-AMTN：用于人脸属性识别的注意力多任务网络，通过共享参数和跨组特征融合提升泛化性能，同时减少模型参数


<details>
  <summary>Details</summary>
Motivation: 传统多任务网络在低层共享模块、高层独立模块的设计导致参数随任务数量指数增长，且高层特征交互受限，阻碍了属性间语义关系的探索，影响泛化性能

Method: 提出FAR-AMTN，包含三个核心模块：1) 权重共享组特定注意力模块(WSGSA)减少复杂度并提升组特征表示；2) 跨组特征融合模块(CGFF)促进属性组间交互；3) 动态权重策略(DWS)实现任务同步收敛

Result: 在CelebA和LFWA数据集上的实验表明，FAR-AMTN相比现有模型在准确率上表现更优，同时参数数量显著减少

Conclusion: FAR-AMTN通过创新的注意力机制和特征融合策略，有效解决了传统多任务网络参数爆炸和特征交互不足的问题，在人脸属性识别任务中实现了更好的泛化性能和效率平衡
...
Method: 提出了TSI范式，包含物理动态推理和意图驱动推理两个支柱；创建了EscherVerse套件，包括Escher-Bench基准、Escher-35k数据集和Escher系列模型；开发了新颖的数据收集管道，基于真实世界视频构建开放世界评估环境。

Result: EscherVerse是首个系统评估意图驱动推理的基准，能够评估物体恒存性、状态转换和轨迹预测等能力，为空间智能研究提供了基础资源。

Conclusion: TSI范式将空间智能从被动场景描述提升到目的驱动的整体理解，EscherVerse为研究空间动态推理和人类意图理解提供了重要工具和基准。
...
### [89] [Guiding Token-Sparse Diffusion Models](https://arxiv.org/abs/2601.01608)
*Felix Krause,Stefan Andreas Baumann,Johannes Schusterbauer,Olga Grebenkova,Ming Gui,Vincent Tao Hu,Björn Ommer*

Main category: cs.CV

TL;DR: 提出Sparse Guidance (SG)方法，通过token-level稀疏性解决稀疏训练扩散模型在推理时对Classifier-free Guidance响应不佳的问题，在降低计算成本的同时保持高质量输出。


<details>
  <summary>Details</summary>
Motivation: 扩散模型训练和推理成本高，现有稀疏训练方法虽然降低了训练成本，但在推理时由于对Classifier-free Guidance响应不佳导致性能下降，需要新的指导机制。

Method: 提出Sparse Guidance (SG)，使用token-level稀疏性而非条件dropout来指导扩散模型，在推理时利用token-level稀疏性，更好地保留条件预测的高方差。

Result: 在ImageNet-256基准测试上达到1.58 FID，减少25% FLOPs；在匹配基线质量下节省58% FLOPs；2.5B文本到图像扩散模型在训练时使用稀疏性，推理时使用SG，提高了组合能力和人类偏好评分，同时增加吞吐量。

Conclusion: Sparse Guidance有效解决了稀疏训练扩散模型在推理时的性能问题，在降低计算成本的同时保持甚至提高了生成质量，为高效扩散模型训练和推理提供了新方法。
...
### [90] [CAP-IQA: Context-Aware Prompt-Guided CT Image Quality Assessment](https://arxiv.org/abs/2601.01613)
*Kazi Ramisa Rifa,Jie Zhang,Abdullah Imran*

Main category: cs.CV

TL;DR: 提出CAP-IQA框架，通过上下文感知提示和因果去偏技术改进CT图像质量评估，在LDCTIQA挑战中超越现有方法4.24%


<details>
  <summary>Details</summary>
Motivation: 现有基于提示的方法在CT图像质量评估中应用有限，且容易引入理想化定义的偏差，无法适应真实世界中的噪声、运动伪影和扫描仪变异等退化情况

Method: 提出上下文感知提示引导的图像质量评估框架，结合文本级先验和实例级上下文提示，采用因果去偏技术分离理想化知识和实际图像退化，使用CNN视觉编码器和领域特定文本编码器

Result: 在LDCTIQA挑战基准测试中达到2.8590总相关分数，超越榜首团队4.24%；在91,514张儿科CT内部数据集上验证了泛化能力

Conclusion: CAP-IQA框架通过提示引导融合和简化编码器设计有效提升特征对齐和可解释性，在不同患者群体中具有良好泛化能力
...
### [91] [An Empirical Study of Monocular Human Body Measurement Under Weak Calibration](https://arxiv.org/abs/2601.01639)
*Gaurav Sekar*

Main category: cs.CV

TL;DR: 该论文对三种弱标定单目人体测量方法进行了系统性实证研究，分析了不同标定假设对测量结果的影响，揭示了用户标定努力与测量稳定性之间的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 从单目RGB图像估计人体测量值面临尺度模糊、视角敏感和缺乏深度信息等挑战。现有研究多追求最先进的精度，但缺乏对不同标定假设如何影响测量行为、鲁棒性和失败模式的分析。

Method: 研究了三种弱标定单目策略：基于地标的几何方法、姿态驱动的回归方法和物体标定的轮廓方法。在消费级相机半约束条件下进行系统性实证评估，分析不同标定假设对测量结果的影响。

Result: 研究发现用户在校准过程中的努力程度与所得周长测量值的稳定性之间存在明显的权衡关系。不同方法在不同体型上表现出不同的测量行为和失败模式。

Conclusion: 该研究为面向消费设备的轻量级单目人体测量系统提供了实证设计参考，强调了在实际部署中需要权衡用户标定努力与测量稳定性。
...
### [92] [Animated 3DGS Avatars in Diverse Scenes with Consistent Lighting and Shadows](https://arxiv.org/abs/2601.01660)
*Aymen Mir,Riza Alp Guler,Jian Wang,Gerard Pons-Moll,Bing Zhou*

Main category: cs.CV

TL;DR: 提出Deep Gaussian Shadow Maps (DGSM)方法，为3D高斯泼溅(3DGS)虚拟角色提供一致的光照和阴影效果，支持角色与场景交互时的实时阴影计算和环境光照重照明。


<details>
  <summary>Details</summary>
Motivation: 当动画化的3DGS虚拟角色与3DGS场景或动态物体交互时，需要实现一致的光照和阴影效果。传统方法难以在保持3DGS体积表示的同时实现实时阴影计算和环境光照。

Method: 1. 提出DGSM：基于经典深度阴影映射思想，针对3DGS体积表示设计，通过封闭形式的光线累积计算体积阴影；2. 使用八面体图集存储同心径向壳层的透射率；3. 使用球谐(Spherical Harmonics)基表示的HDRI探针近似局部环境光照；4. 应用快速的高斯辐射传输，避免显式BRDF估计或离线优化。

Result: 在AvatarX、ActorsHQ虚拟角色与ScanNet++、DL3DV、SuperSplat场景的合成中，展示了单角色和多角色设置下的环境一致光照效果。DGSM和SH重照明完全在3DGS体积表示中运行，产生连贯的阴影和重照明效果，无需网格化。

Conclusion: DGSM方法成功实现了3DGS虚拟角色与场景交互时的一致光照和阴影效果，在保持体积表示的同时提供了实时阴影计算和环境光照重照明能力，避免了网格化的需求。
...
### [93] [LabelAny3D: Label Any Object 3D in the Wild](https://arxiv.org/abs/2601.01676)
*Jin Yao,Radowan Mahmud Redoy,Sebastian Elbaum,Matthew B. Dwyer,Zezhou Cheng*

Main category: cs.CV

TL;DR: LabelAny3D：基于分析-合成的3D标注框架，从2D图像重建整体3D场景，生成高质量3D边界框标注，并构建了COCO3D开放词汇单目3D检测基准。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D检测模型在野外图像上表现不佳，主要原因是缺乏3D野外数据集和3D标注的挑战性。需要一种高效生成高质量3D标注的方法来扩展3D识别在真实开放世界场景中的应用。

Method: 提出LabelAny3D分析-合成框架，从2D图像重建整体3D场景，基于此构建COCO3D基准（源自MS-COCO数据集），覆盖现有3D数据集中缺失的广泛对象类别。

Result: LabelAny3D生成的标注提升了单目3D检测在多个基准上的性能，在质量上优于先前的自动标注方法。实验证明了基础模型驱动标注在扩展现实开放世界3D识别中的潜力。

Conclusion: LabelAny3D框架和COCO3D基准为开放词汇单目3D检测提供了有效解决方案，展示了基础模型驱动标注在扩展现实世界3D识别应用中的前景。
...
### [94] [Trustworthy Data-Driven Wildfire Risk Prediction and Understanding in Western Canada](https://arxiv.org/abs/2601.01677)
...
TL;DR: 提出一个可信赖的野火风险预测框架，通过多尺度时序建模整合异质驱动因素，量化预测不确定性并提供过程级解释，在加拿大西部2023-2024火灾季表现优异。


<details>
  <summary>Details</summary>
Motivation: 加拿大西部野火活动加剧导致重大社会经济和环境损失，但准确预测面临挑战：点火和蔓延的随机性、燃料条件、气象、气候、地形和人类活动之间的非线性相互作用，使得纯数据驱动模型的可信性和可解释性受限。

Method: 基于长序列多尺度时序建模的可信赖数据驱动野火风险预测框架，整合异质驱动因素，显式量化预测不确定性，支持过程级解释。使用SHAP进行机制解释。

Result: 在加拿大西部2023-2024创纪录火灾季评估中，模型优于现有时间序列方法，F1分数0.90，PR-AUC 0.98，计算成本低。不确定性分析显示预测置信度的结构化时空模式，SHAP解释揭示温度相关驱动因素主导野火风险，2024年湿度相关约束在空间和土地覆盖对比中作用更强。

Conclusion: 提出的可信赖野火风险预测框架在准确性和可解释性方面表现优异，为野火管理提供可靠工具，数据与代码已开源。
...
### [95] [Evaluating Deep Learning-Based Face Recognition for Infants and Toddlers: Impact of Age Across Developmental Stages](https://arxiv.org/abs/2601.01680)
*Afzal Hossain,Mst Rumana Sumi,Stephanie Schuckers*

Main category: cs.CV

TL;DR: 该研究评估了四种深度学习人脸识别模型在0-3岁婴幼儿纵向数据集上的表现，发现年龄越小识别率越低（0-6个月仅30.7% TAR），2.5-3岁可达64.7%。时间间隔越短识别率越高，使用DANN方法可提升12%以上TAR。


<details>
  <summary>Details</summary>
Motivation: 婴幼儿人脸识别面临独特挑战：面部形态快速变化、类间相似度高、数据集有限。研究旨在评估现有模型在婴幼儿纵向数据上的表现，为智慧城市应用（公共医疗、儿童安全、数字身份服务）构建可靠的生物识别系统。

Method: 使用新开发的0-3岁婴幼儿纵向数据集（24个月内7次采集），评估FaceNet、ArcFace、MagFace、CosFace四种深度学习人脸识别模型。分析不同发育阶段的识别准确率，评估不同时间间隔的验证性能，并应用域对抗神经网络（DANN）减少嵌入漂移。

Result: 0-6个月婴儿TAR仅30.7%（FAR=0.1%），2.5-3岁儿童提升至64.7%。时间间隔越短识别准确率越高。DANN方法将TAR提升超过12%，获得更稳定和可泛化的特征。

Conclusion: 婴幼儿早期人脸识别存在显著挑战，特别是0-6个月阶段。DANN能有效减少时间漂移，提升识别性能。研究强调了在智慧城市应用中开发能处理时间变化的隐私保护生物认证系统的重要性。
...
### [96] [FALCON: Few-Shot Adversarial Learning for Cross-Domain Medical Image Segmentation](https://arxiv.org/abs/2601.01687)
*Abdur R. Fayjie,Pankhi Kashyap,Jutika Borah,Patrick Vandewalle*

Main category: cs.CV

TL;DR: FALCON是一个跨域少样本分割框架，通过将3D医学数据作为2D切片处理，实现高精度分割，在数据标注少、计算开销低的情况下达到SOTA边界精度。
...
Result: 在四个基准测试中，FALCON始终获得最低的Hausdorff距离分数（表明边界精度优越），同时保持与SOTA模型相当的Dice相似系数，且使用更少标注数据、无数据增强、计算开销显著降低。

Conclusion: FALCON通过跨域少样本学习有效解决了3D医学分割中的标注稀缺和计算挑战，在保持高分割精度的同时大幅降低资源需求，具有临床实用价值。
...
### [97] [Mitigating Longitudinal Performance Degradation in Child Face Recognition Using Synthetic Data](https://arxiv.org/abs/2601.01689)
*Afzal Hossain,Stephanie Schuckers*

Main category: cs.CV

TL;DR: 该研究探讨了使用合成人脸数据作为纵向稳定器，改善儿童人脸识别模型的时间鲁棒性，通过合成数据增强训练显著降低了验证错误率。


<details>
  <summary>Details</summary>
Motivation: 儿童面部识别面临挑战，因为儿童面部快速非线性生长导致模板漂移和随时间增加的验证错误。需要提高儿童人脸识别模型的时间鲁棒性。

Method: 在YFA数据集上使用身份分离协议，评估三种设置：1) 预训练MagFace嵌入；2) 仅用真实数据微调MagFace；3) 用真实+合成数据微调MagFace。使用StyleGAN2 ADA生成合成数据，并进行后生成过滤以减少身份泄漏和伪影。

Result: 实验结果显示，在6-36个月的注册验证间隔中，合成数据增强的微调相对于预训练基线和仅用真实数据微调，显著降低了错误率。

Conclusion: 合成数据增强可以有效提高儿童人脸识别中的身份持久性，为儿科人脸识别提供了风险感知的评估方法。
...
### [98] [Learnability-Driven Submodular Optimization for Active Roadside 3D Detection](https://arxiv.org/abs/2601.01695)
*Ruiyu Mao,Baoming Zhang,Nicholas Ruozzi,Yunhui Guo*

Main category: cs.CV

TL;DR: LH3D：基于可学习性的主动学习框架，用于路边单目3D目标检测，通过选择信息丰富且可可靠标注的场景，抑制固有模糊样本，在仅使用25%标注预算下达到接近全数据性能。


<details>
  <summary>Details</summary>
Motivation: 实际部署中，由于硬件和隐私限制，通常只能标注路边单目数据。许多路边场景包含远距离、模糊或被遮挡的目标，其3D属性从单视角看是模糊的，只能通过车辆-路边配对帧交叉验证来可靠标注。这些固有模糊样本增加了标注难度和成本，并揭示了根本的可学习性问题。

Method: 提出LH3D框架，专注于路边单目3D目标检测的主动学习。该框架基于可学习性驱动，选择既信息丰富又可可靠标注的场景，抑制固有模糊样本同时确保覆盖范围。

Result: 在DAIR-V2X-I数据集上，仅使用25%的标注预算，对车辆、行人和骑行者的检测性能分别达到全数据性能的86.06%、67.32%和78.67%，显著优于基于不确定性的基线方法。

Conclusion: 实验证实，对于路边3D感知，可学习性而非不确定性才是关键因素。LH3D框架能有效减少固有模糊样本的标注浪费，同时获得高性能模型。
...
<details>
  <summary>Details</summary>
Motivation: 嵌入式系统中的实时车道检测面临挑战：RGB图像中视觉信号稀疏且细微，计算资源和功耗受限。现有深度学习方法缺乏针对低功耗嵌入式环境的通用优化技术。

Method: 提出创新的协方差分布优化(CDO)模块，专门为高效实时应用设计。该模块将车道特征分布与真实标签对齐，在不增加计算复杂度的前提下提升检测精度。

Result: 在三大数据集(CULane、TuSimple、LLAMAS)上测试了六种模型，涵盖分割、锚点和曲线三类方法，包括实时优化模型和SOTA模型。实验显示精度提升0.01%到1.5%。

Conclusion: CDO模块易于集成到现有系统，无需结构修改，利用现有参数进行持续训练，在嵌入式系统中提供性能、功耗效率和操作灵活性方面的显著优势。
...
TL;DR: 提出FFP-300K大规模数据集和自适应时空RoPE框架，实现无需运行时引导的可控视频编辑，在EditVerseBench上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有第一帧传播（FFP）方法依赖繁琐的运行时引导，根本原因是训练数据集不足：视频太短、分辨率低、任务多样性不够，无法学习鲁棒的时间先验。

Method: 1) 构建FFP-300K数据集：30万对720p、81帧的高质量视频，通过双轨管道实现多样局部和全局编辑；2) 提出自适应时空RoPE（AST-RoPE）框架，动态重映射位置编码以解耦外观和运动参考；3) 使用自蒸馏策略，以身份传播任务作为正则化器确保长期时间稳定性。

Result: 在EditVerseBench基准测试中显著优于现有学术和商业模型，PickScore提高约0.2分，VLM分数提高约0.3分。

Conclusion: 通过大规模高质量数据集和创新的自适应时空RoPE框架，解决了FFP中外观保持与运动保留的关键矛盾，实现了真正无需引导的可控视频编辑。
...
TL;DR: Point-SRA：通过自蒸馏和概率建模对齐表示的三维表示学习方法，在多个下游任务上超越现有MAE方法


<details>
  <summary>Details</summary>
Motivation: 现有MAE方法使用固定掩码率，忽略了多级表示相关性和内在几何结构，且基于点级重建假设与点云多样性相冲突

Method: 1）为MAE分配不同掩码率以捕获互补几何语义信息；2）MeanFlow Transformer利用跨模态条件嵌入实现多样化概率重建；3）在MAE和MFT级别提出双重自表示对齐机制；4）设计流条件微调架构

Result: 1）ScanObjectNN上比Point-MAE提升5.37%；2）颅内动脉瘤分割：动脉平均IoU 96.07%，动脉瘤86.87%；3）3D目标检测：AP@50 47.3%，超越MaskPoint 5.12%

Conclusion: Point-SRA通过多级掩码策略、概率建模和双重对齐机制，有效解决了现有MAE方法的局限性，在多个3D任务上取得显著性能提升
...
TL;DR: MANGO是一个两阶段框架，通过纯图像级监督实现高质量的双人3D对话头部生成，解决了现有方法在双向听-说交互和伪3D标签噪声方面的问题。


<details>
  <summary>Details</summary>
Motivation: 当前音频驱动的3D头部生成方法主要关注单说话者场景，缺乏自然的双向听-说交互。现有3D对话化身方法依赖容易出错的伪3D标签，无法捕捉细粒度的面部动态。

Method: 两阶段框架：第一阶段使用基于扩散的transformer和双音频交互模块从多说话者音频建模自然3D运动；第二阶段使用快速3D高斯渲染器生成高保真图像，并通过交替训练为3D运动提供2D级光度监督。

Result: 方法在建模双人3D对话运动方面实现了卓越的准确性和真实感，显著提升了音频驱动说话头部的保真度和可控性。

Conclusion: MANGO框架通过纯图像级监督和交替训练有效缓解了伪3D标签的噪声问题，实现了与现实世界对话行为更好的对齐，推动了3D对话化身的真实感和可控性发展。
...
<details>
  <summary>Details</summary>
Motivation: 病理报告中的信息通常是非结构化的，难以系统提取和利用。需要标准化方法从病理报告中提取诊断要素，并开发能够理解全切片图像的视觉问答模型来支持临床诊断工作流程。

Method: 1) 设计临床病理报告模板(CPRT)标准化提取病理特征；2) 构建CTIS-Align数据集(80k切片-描述对)用于视觉语言对齐训练；3) 创建CTIS-Bench基准(977个WSI, 14,879个QA对)；4) 提出CTIS-QA双流架构模型，一个流通过聚类特征聚合捕获全局上下文，另一个流通过注意力引导的补丁感知模块聚焦局部区域。

Result: 在TCGA-BRCA上验证了管道的有效性。CTIS-QA在WSI-VQA、CTIS-Bench和切片级诊断任务上的实验表明，该模型在多个指标上持续优于现有最先进模型。

Conclusion: 提出的临床诊断模板管道能够有效结构化病理信息，CTIS-QA模型通过模拟病理学家诊断方法，在视觉问答和诊断任务中表现出色，为临床病理分析提供了有价值的工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前自监督学习方法依赖大量预训练数据，但在遥感图像语义分割领域，获取大量标注数据成本高昂。需要一种能在有限数据下有效预训练的方法。

Method: 提出Subimage Overlap Prediction任务：从图像中提取子图像，训练模型预测该子图像在原图中的位置语义掩码。这种方法不需要大量预训练数据。

Result: 实验表明该方法能显著加速收敛，在多个架构和下游数据集上达到相等或更好的mIoU性能。当标注数据减少时，优势更加明显，且比其他SSL方法需要更少的预训练数据。

Conclusion: Subimage Overlap Prediction是一种有效的自监督预训练方法，特别适合遥感图像语义分割，能在有限数据下实现快速收敛和优异性能。
...
TL;DR: 提出一种创新的视觉语言模型用于人脸验证，不仅能准确判断两张人脸图像是否属于同一人，还能解释决策依据。模型使用两种互补的解释风格训练：简洁总结和详细差异分析。


<details>
  <summary>Details</summary>
Motivation: 当前人脸验证系统虽然取得了显著进展，但决策过程缺乏透明度。需要开发既能准确验证又能解释决策依据的系统，以提高透明度和可靠性。

Method: 采用视觉语言模型框架，将音频区分任务的最先进建模方法适应并增强以处理视觉输入。模型集成复杂特征提取技术和高级推理能力，使用两种解释风格训练：简洁总结关键因素和详细描述图像间具体差异。

Result: 提出的VLM在性能上超越了基线方法和现有模型，跨模态迁移显著提高了模型的准确性和可解释性。
...
TL;DR: V-CORE：一个参数高效的视频理解框架，通过可学习空间聚合和因果感知时序投影器，引入显式时序约束，解决现有视频大语言模型中时序顺序和因果一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型在需要一致时序排序和因果连贯性的视频理解任务中存在挑战，许多参数高效模型使用无约束双向投影器建模帧间交互，这会模糊时序顺序，缺乏尊重视频推理方向性的显式架构机制。

Method: V-CORE包含两个关键组件：1) 可学习空间聚合(LSA)，自适应选择显著空间标记以减少冗余；2) 因果感知时序投影器(CATP)，通过块因果注意力和作为因果汇的终端动态摘要标记，强制执行结构化单向信息流。

Result: 在NExT-QA基准测试中达到61.2%准确率，在MSVD-QA、MSRVTT-QA和TGIF-QA上保持竞争力，在时序和因果推理子类别上分别提升3.5%和5.2%，验证了显式时序约束的重要性。

Conclusion: V-CORE通过引入显式时序排序约束，有效解决了视频理解中的时序顺序和因果一致性问题，在参数高效的前提下显著提升了时序和因果推理能力。
...
TL;DR: 提出LUMPNet混合深度学习模型，用于早期检测牛结节性皮肤病(LSD)，结合YOLOv11检测病灶、EfficientNet分类，以及新型自适应混合优化器，在公开数据集上达到99%训练准确率和98%验证准确率。


<details>
  <summary>Details</summary>
Motivation: 牛结节性皮肤病(LSD)是一种传染性病毒疾病，严重影响牲畜健康，对全球经济和粮食安全构成严重威胁。由于其快速传播特性，早期精确识别对于预防疫情爆发和确保及时干预至关重要。

Method: 提出LUMPNet混合深度学习模型：1) 使用YOLOv11检测和定位牛图像中的LSD皮肤结节和病变；2) 采用基于EfficientNet的CNN分类器对定位图像进行LSD感染或健康分类；3) 提出新型自适应混合优化器来稳定和加速YOLOv11与EfficientNet混合模型的训练。

Result: 在公开数据集上评估显示：LUMPNet达到99%的LSD检测训练准确率，验证准确率为98%，优于现有方案。通过案例研究比较优化后的EfficientNet-B0模型与AdamW优化器，LUMPNet表现出更优越的性能。

Conclusion: LUMPNet是一种有效的LSD早期检测方法，通过混合深度学习架构和自适应优化器实现了高准确率的病灶检测和分类，为预防和控制LSD疫情提供了可靠的技术支持。
...
TL;DR: 提出语言引导的场景上下文感知学习框架，用于鲁棒的自中心视觉注意力预测，通过语言描述引导上下文感知并抑制无关区域干扰。


<details>
  <summary>Details</summary>
Motivation: 自中心视频分析需求增长，但动态自中心场景的复杂性和模糊性使得视觉注意力预测具有挑战性。研究表明场景上下文信息在调节人类注意力中起关键作用。

Method: 1) 设计上下文感知器，基于语言场景描述引导总结自中心视频，生成上下文感知的视频表示；2) 引入两个训练目标：聚焦目标兴趣区域和抑制无关区域干扰。

Result: 在Ego4D和Aria Everyday Activities (AEA)数据集上的大量实验表明，该方法在多样动态自中心场景中实现了最先进的性能和增强的鲁棒性。

Conclusion: 提出的语言引导场景上下文感知学习框架能有效提升自中心视觉注意力预测性能，通过利用语言描述引导上下文感知和抑制干扰来实现鲁棒预测。
...
TL;DR: 提出名为RSwinV2的深度学习模型，用于Mpox（猴痘）诊断，通过定制化残差SwinTransformerV2架构，结合逆残差块和移位窗口注意力机制，在Kaggle数据集上达到96.21%准确率和95.62% F1分数。


<details>
  <summary>Details</summary>
Motivation: 现有方法在Mpox病变分类中存在局限性，需要更有效的计算机辅助诊断工具来区分Mpox、水痘、麻疹和牛痘等相似病变，提高诊断准确性和效率。

Method: 基于SwinTransformerV2定制化开发RSwinV2模型，采用分层Transformer结构，将输入图像分割为非重叠补丁，使用移位窗口注意力机制，并引入逆残差块（IRB）解决梯度消失问题，结合全局和局部特征。

Result: 在Kaggle公开数据集上，RSwinV2达到96.21%准确率和95.62% F1分数，优于标准CNN模型和原始SwinTransformer，证明其作为Mpox病变观察解释的计算机辅助工具的有效性。

Conclusion: RSwinV2通过结合Transformer的全局关联能力和逆残差块的局部特征提取，显著提高了Mpox病变分类性能，为计算机辅助Mpox诊断提供了有效的深度学习解决方案。
...
TL;DR: ESGaussianFace：基于3D高斯溅射的情感风格化音频驱动面部动画框架，通过情感音频引导空间注意力机制和3D高斯变形预测器，实现高效高质量的情感风格化面部视频生成。


<details>
  <summary>Details</summary>
Motivation: 当前音频驱动面部动画研究主要关注中性情绪的视频生成，虽然有些研究涉及情感音频驱动，但如何高效生成同时包含情感表达和风格特征的高质量说话头部视频仍然是一个重大挑战。

Method: 1. 使用3D高斯溅射重建3D场景并渲染视频；2. 提出情感音频引导的空间注意力方法，有效整合情感特征和音频内容特征；3. 引入两个3D高斯变形预测器实现情感和风格化变形；4. 采用多阶段训练策略，逐步学习嘴唇运动、情感变化和风格特征。

Result: 实验结果表明，该方法在嘴唇运动准确性、表情变化和风格特征表现力方面优于现有最先进技术，生成结果具有高效率、高质量和3D一致性。

Conclusion: ESGaussianFace框架能够高效生成具有情感表达和风格特征的3D一致面部动画视频，在多个评估指标上超越现有方法，为解决情感风格化音频驱动面部动画提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 工业检测中特征异常检测广泛使用预训练视觉编码器，但实际部署需要任务无关操作和持续类别扩展。现有方法在跨类别路由时存在分数分布不一致问题，导致路由不可靠

Method: 提出几何一致性路由(GCR)框架，在共享冻结补丁嵌入空间中通过最小化累积最近原型距离进行路由，然后仅在路由到的专家内部使用标准基于原型的评分规则计算异常图

Result: 在MVTec AD和VisA数据集上，几何一致性路由显著提高路由稳定性，缓解持续性能崩溃，实现近零遗忘，同时保持竞争力的检测和定位性能

Conclusion: 许多先前归因于表示遗忘的失败实际上可以解释为跨头路由中决策规则的不稳定性，GCR通过分离跨头决策和内部异常评分有效解决了这一问题
...
TL;DR: RRNet是一个轻量级实时视频增强框架，通过虚拟光源参数估计实现局部重照明，在视觉质量和效率之间取得最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 现有实时视频增强方法在速度和曝光控制之间难以平衡，特别是在不均匀光照条件下，需要一种既能保持高质量又能实时运行的解决方案。

Method: 使用轻量级编码器和预测头估计虚拟光源参数，通过深度感知渲染模块实现局部重照明，无需像素对齐训练数据，采用生成式AI创建多样化光照数据集。

Result: RRNet在低光增强、局部光照调整和眩光去除方面优于现有方法，支持实时高分辨率处理，保持面部身份特征。

Conclusion: RRNet通过可解释的光照控制和高效架构，为视频会议、AR肖像增强和移动摄影等实际应用提供了理想的解决方案。
...
TL;DR: 提出EGMT方法，通过实体引导的多任务学习实现红外与可见光图像融合，利用实体级文本信息消除语义噪声，提升融合图像的质量和语义密度。
...
Method: 1) 从大视觉语言模型生成的图像描述中提取实体级文本信息；2) 构建并行多任务学习架构，将图像融合与多标签分类任务结合；3) 开发实体引导的跨模态交互模块。

Result: 在TNO、RoadScene、M3FD和MSRS四个公开数据集上实验表明，EGMT在保留显著目标、纹理细节和语义一致性方面优于现有方法。

Conclusion: EGMT方法通过实体级文本引导和多任务学习，有效提升了红外与可见光图像融合的质量和语义理解能力，并发布了实体标注的数据集。
...
TL;DR: CogFlow是一个受认知启发的三阶段框架，通过知识内化阶段解决视觉数学问题中视觉感知与推理脱节的问题，显著提升了多模态大语言模型的视觉数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在视觉数学问题解决上仍有困难。虽然一些工作认识到视觉感知是瓶颈，但仅关注改进视觉输入的提取和解释，忽视了提取的视觉线索是否被忠实整合并正确用于后续推理这一关键问题。

Method: 提出CogFlow三阶段框架：感知→内化→推理。1) 设计协同视觉奖励提升符号和图表的视觉信息提取；2) 引入知识内化奖励模型确保视觉线索忠实整合；3) 设计视觉门控策略优化算法防止模型走捷径进行无视觉基础的推理。同时贡献了包含12万高质量标注的MathCog数据集。

Result: 在常用的视觉数学推理基准测试上进行了全面实验和分析，验证了CogFlow的优越性。

Conclusion: 通过模拟人类推理的层次流程并全面增强所有阶段，CogFlow有效解决了视觉数学问题中感知与推理的脱节问题，显著提升了多模态大语言模型的视觉数学推理能力。
...
TL;DR: 该论文首次全面综述了遥感领域的智能体AI，提出了统一的分类体系，区分单智能体协同和多智能体系统，分析了规划机制、检索增强生成和记忆结构等架构基础，并介绍了从像素级精度转向轨迹感知推理正确性的新兴基准。


<details>
  <summary>Details</summary>
Motivation: 地球观测分析范式正从静态深度学习模型转向自主智能体AI。尽管最近的视觉基础模型和多模态大语言模型在表征学习方面取得了进展，但它们往往缺乏复杂地理空间工作流所需的序列规划和主动工具编排能力。

Method: 该综述提出了遥感领域智能体AI的统一分类体系，区分单智能体协同和多智能体系统，并深入分析了规划机制、检索增强生成和记忆结构等架构基础。同时回顾了从像素级精度评估转向轨迹感知推理正确性的新兴基准。

Result: 通过批判性分析在基础性、安全性和编排方面的局限性，该工作为开发稳健、自主的地理空间智能制定了战略路线图。

Conclusion: 该论文为遥感领域智能体AI的发展提供了首个全面综述，通过统一的分类体系、架构分析和新兴基准评估，为构建更强大、更自主的地理空间智能系统指明了方向。
...
TL;DR: FLLP框架通过在双曲空间引入父子概念学习机制，缓解定制扩散模型中的灾难性遗忘问题，利用已学概念指导新概念学习，实现更好的持续学习性能。


<details>
  <summary>Details</summary>
Motivation: 定制扩散模型在顺序学习新概念时容易遭受灾难性遗忘，现有方法主要关注最小化概念间干扰，但忽略了概念间可能的正向交互作用。

Method: 提出FLLP框架，在双曲空间（洛伦兹流形）中引入父子概念学习机制，利用先前学习的概念作为指导来适应新概念，通过树状层次结构建模概念关系。

Result: 在三个公共数据集和一个合成基准上验证了FLLP，在鲁棒性和泛化能力方面均显示出持续改进。

Conclusion: FLLP通过双曲空间中的父子概念学习机制，不仅保留了先验知识，还支持新概念的持续集成，有效缓解了定制扩散模型中的灾难性遗忘问题。
...
TL;DR: 提出Nodule-DETR，一种基于检测变压器的甲状腺结节检测架构，通过多光谱频域通道注意力、分层特征融合和多尺度可变形注意力模块，显著提升超声图像中低对比度、小尺寸和不规则形状结节的检测性能。


<details>
  <summary>Details</summary>
Motivation: 甲状腺癌是最常见的内分泌恶性肿瘤，发病率不断上升。超声是检测甲状腺结节的首选成像方式，但其诊断准确性常受低图像对比度和模糊结节边界等挑战限制。

Method: 提出Nodule-DETR架构，包含三个关键创新：1) 多光谱频域通道注意力模块，利用频率分析增强低对比度结节特征；2) 分层特征融合模块，实现高效多尺度集成；3) 多尺度可变形注意力模块，灵活捕捉小尺寸和不规则形状结节。

Result: 在真实世界甲状腺超声图像临床数据集上的实验表明，Nodule-DETR达到最先进性能，在mAP@0.5:0.95指标上比基线模型显著提升0.149。
...
TL;DR: 提出HybridTAS框架，结合欧几里得和双曲几何，利用扩散模型进行时序动作分割，通过双曲几何的树状结构实现从粗到细的动作标签去噪


<details>
  <summary>Details</summary>
Motivation: 现有基于迭代精炼的方法未能充分利用人类动作的层次结构特性，需要一种能够显式利用动作层次关系的方法来改进时序动作分割性能

Method: 提出HybridTAS框架，将欧几里得和双曲几何结合到扩散模型的去噪过程中。利用双曲几何天然的树状关系，在较高扩散时间步使用抽象的高层动作类别（根节点），在较低时间步使用细粒度动作类别（叶节点），实现从粗到细的动作标签去噪

Result: 在GTEA、50Salads和Breakfast三个基准数据集上进行了广泛实验，取得了最先进的性能，验证了双曲引导去噪对时序动作分割任务的有效性

Conclusion: HybridTAS通过结合双曲几何和扩散模型，有效利用了动作的层次结构，为时序动作分割任务提供了一种新的有效方法，实现了从粗到细的动作标签去噪
...
TL;DR: TalkPhoto是一个无需训练的图像编辑框架，通过对话交互实现精确图像操控，利用LLM分析用户需求并分层调用现有高级编辑方法
...
Method: 使用专门设计的提示模板指导开源LLM分析用户需求，分层调用现有高级编辑方法，实现即插即用且高效的图像编辑方法调用

Result: 实验表明该方法不仅以更少的token消耗提供更准确的调用，还在各种图像编辑任务中实现了更高的编辑质量

Conclusion: TalkPhoto框架无需额外训练即可实现稳定高质量的图像编辑，能够处理复杂和未见过的编辑任务，展现了对话式图像编辑的潜力
...
<details>
  <summary>Details</summary>
Motivation: 现有MOT方法采用固定的输出头和特定任务架构，难以适应日益复杂的多模态场景和指令驱动任务，限制了方法的通用性和扩展性。

Method: 1. 将MOT建模为LLM框架内的序列生成任务；2. 使用预训练检测器构建对象分词器增强区域视觉感知；3. 提出区域感知对齐模块缓解全局与区域特征不对齐；4. 设计时序记忆融合模块缓存历史对象标记支持长时跟踪。

Result: 在MOT17和DanceTrack数据集上的实验验证了方法的可行性，性能达到SOTA水平，同时为更通用灵活的MOT系统奠定了基础。

Conclusion: AR-MOT通过自回归序列生成范式解决了传统MOT方法的架构限制，展示了在LLM框架下实现通用、可扩展MOT系统的潜力，新模态或指令只需修改输出序列格式即可集成。
...
<details>
  <summary>Details</summary>
Motivation: 当前持续学习中的VQA方法在平衡知识保留、适应性和鲁棒特征表示方面存在困难，需要更好的解决方案。
...
Conclusion: MacVQA能够有效平衡持续VQA学习中的知识获取、保留和组合泛化能力，为视觉问答的持续学习提供了有效的解决方案。
...
TL;DR: 提出一种从粗到精的人脸法线估计方法，通过小数据集训练粗估计模型生成指导样本，再用自注意力机制优化局部伪影，显著减少对大规模配对数据和计算资源的需求。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸法线估计方法严重依赖大规模配对数据进行训练，这限制了其实际应用。本文旨在通过开发一种从粗到精的估计器来减轻这一需求。

Method: 采用两阶段方法：1）用小数据集训练简洁模型生成粗人脸法线作为指导样本；2）使用自注意力机制捕获长距离依赖关系，修复粗估计中的局部伪影；3）定制细化网络将输入人脸图像与对应指导样本映射到高质量精细法线。

Result: 大量实验和消融研究表明该方法在训练成本和估计质量方面均优于现有最先进方法，同时显著减少了对大规模配对数据和计算资源的需求。

Conclusion: 提出的从粗到精人脸法线估计方法通过合理的功能分割，有效降低了对大规模训练数据和计算资源的要求，同时保持了高质量的估计结果，代码和模型已开源。
...
TL;DR: MotionAdapter：基于DiT的文本到视频模型的内容感知运动迁移框架，通过解耦运动与外观、自适应定制运动实现鲁棒且语义对齐的运动迁移。


<details>
  <summary>Details</summary>
Motivation: 尽管基于扩散变换器的文本到视频模型在生成高质量时序连贯视频方面取得显著进展，但在视频间迁移复杂运动仍然具有挑战性。现有方法难以实现鲁棒且语义对齐的运动迁移。

Method: 1）通过分析3D全注意力模块中的跨帧注意力来提取注意力驱动的运动场，实现运动与外观的显式解耦；2）引入DINO引导的运动定制模块，基于内容对应关系重新排列和精炼运动场，弥合参考视频与目标视频之间的语义鸿沟；3）使用定制化的运动场指导DiT去噪过程，确保合成视频继承参考运动同时保持目标外观和语义。

Result: 在定性和定量评估中均优于现有最先进方法，自然支持复杂运动迁移和运动编辑任务（如缩放）。

Conclusion: MotionAdapter通过内容感知的运动迁移框架，在DiT-based T2V模型中实现了鲁棒且语义对齐的运动迁移，为复杂运动迁移和编辑任务提供了有效解决方案。
...
TL;DR: AFTER方法通过事实增强的激活引导和查询自适应偏移优化，有效减少大型视觉语言模型中的物体幻觉问题，在标准基准测试上显著降低幻觉率。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型存在语言偏见导致的物体幻觉问题（包括类别、属性和关系幻觉），这阻碍了可信AI应用的发展。现有编辑方法缺乏对事实文本语义的有效引导，难以显式缓解语言偏见。

Method: 提出AFTER方法，包含两个核心组件：1）事实增强激活引导（FAS）：为激活编辑提供事实性和通用性指导，显式建模精确的视觉-文本关联；2）查询自适应偏移优化（QAO）：引入查询感知偏移估计器，从通用引导向量建立查询特定的编辑，增强编辑的多样性和粒度。

Result: 在三个广泛采用的大型视觉语言模型上的标准幻觉基准测试验证了AFTER的有效性，在AMBER基准上相比基线实现了高达16.3%的幻觉减少。

Conclusion: AFTER方法通过自适应地将原始有偏激活引导至事实语义，有效缓解了大型视觉语言模型中的物体幻觉问题，为可信AI应用提供了有前景的解决方案。
...
TL;DR: FL2T框架通过跨概念学习模块解决定制扩散模型中的灾难性遗忘问题，实现并发、顺序无关的概念学习


<details>
  <summary>Details</summary>
Motivation: 现有定制扩散模型在连续学习新概念时存在灾难性遗忘问题，且大多数先前工作只考虑顺序学习设置，忽略了概念间的交互作用

Method: 提出FL2T框架，引入集合不变性的跨概念学习模块，使用代理指导跨概念的特征选择，促进知识保留和迁移

Result: 在三个数据集上的实验表明，该方法显著提高了概念保留能力，缓解了灾难性遗忘，在十个任务的增量概念学习中平均CLIP图像对齐分数至少提升2%

Conclusion: 通过利用跨概念指导，FL2T能够在保留旧概念的同时有效整合新概念，证明了跨概念催化行为在增量概念学习中的有效性
...
TL;DR: 该论文提出了一种通过对象中心蓝图增强视觉语言模型空间推理能力的方法，将空间关系结构化表示并基于此进行推理。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么关注局部图像块（削弱全局空间感知），要么标记孤立坐标（忽略整体组织），需要更好的空间推理方法。

Method: 引入对象中心蓝图概念，让模型先构建JSON风格的蓝图记录对象位置、大小和属性，然后基于此结构化表示进行推理。采用蓝图嵌入推理轨迹监督微调、蓝图感知奖励强化学习、抗捷径数据增强三种关键技术。

Result: 实验表明该方法在空间推理任务上持续优于现有视觉语言模型和专用空间推理模型。

Conclusion: 对象中心蓝图能有效增强视觉语言模型的空间语义理解能力，将空间推理从感知提升到理解层面。
...
Method: 使用Transformer/Video Vision Transformer架构，结合不同数据模态，开发了多种规模的算法模型

Result: 在JAAD数据集上达到SOTA性能，在准确率、AUC和F1分数等指标上超越了现有最佳方法

Conclusion: 通过大量消融研究验证了不同模型设计选择的优势，为行人行为理解提供了有效的多模态Transformer解决方案
...
TL;DR: 提出API框架用于真实世界图像去雾，包含AHG模块生成多样化雾霾数据，DHR模块自适应处理不同雾霾密度区域，以及MNCD损失函数提升细节恢复，在多个真实世界基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的方法在复杂真实世界雾霾场景中性能显著下降，主要原因是训练数据有限和雾霾密度分布的内在复杂性。需要开发能够泛化到真实世界复杂雾霾场景的去雾方法。

Method: 提出自适应块重要性感知（API）框架：1）自动雾霾生成（AHG）模块提供混合数据增强策略，生成真实多样的雾霾图像作为高质量训练数据；2）密度感知雾霾去除（DHR）模块以自适应块重要性感知方式处理不同雾霾密度区域；3）多负样本对比去雾（MNCD）损失函数，充分利用空间和频域多个负样本信息，缓解去雾图像细节模糊问题。

Result: 在多个真实世界基准测试中实现了最先进的性能，在定量指标和定性视觉质量方面都表现出色，并且对不同雾霾分布展现出强大的泛化能力。

Conclusion: 提出的API框架通过AHG模块解决训练数据不足问题，通过DHR模块处理雾霾密度分布复杂性，通过MNCD损失提升细节恢复，为真实世界图像去雾任务提供了有效的解决方案，具有强大的泛化能力。
...
TL;DR: 提出一种夜间去雾新框架，通过增强雾霾与低光先验的内在一致性，利用多层级专家在视觉和频域进行渐进式恢复，显著提升夜间有雾图像可见度。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常单独处理雾霾或低光等单一退化类型，忽略了不同类型退化之间的相互作用，导致可见度提升有限。研究发现雾霾和低光先验之间的领域知识可以相互增强，从而获得更好的可见度恢复效果。

Method: 提出一个新颖框架，通过相互渐进地增强雾霾和低光先验的内在一致性来提升夜间有雾图像可见度。模型采用图像级、块级和像素级专家，在视觉和频域上协同工作，逐步恢复全局场景结构、区域模式和细粒度细节。引入频率感知路由器自适应引导各专家的贡献，确保鲁棒的图像恢复。

Result: 在夜间去雾基准测试中，该模型在定量和定性评估上都表现出优越性能。此外，模型在白天去雾和低光增强任务中也展现出良好的泛化能力。

Conclusion: 通过增强雾霾和低光先验的内在一致性，提出的多层级专家框架能有效提升夜间有雾图像的可见度，并在相关任务中展现出良好的泛化性能。
...
TL;DR: 本文提出了一种将特权学习范式应用于目标检测的方法，通过教师-学生架构在训练时利用额外信息提升检测精度，且不增加推理复杂度。


<details>
  <summary>Details</summary>
Motivation: 目标检测训练时通常有丰富的额外信息（如边界框掩码、显著性图、深度线索等），但这些信息在推理时不可用。如何有效利用这些训练时的特权信息来提升检测性能，而不增加推理时的计算负担，是本文的研究动机。

Method: 提出了一种模型无关的特权信息注入方法，采用教师-学生架构。教师模型同时接收标准输入和特权信息，学生模型只接收标准输入，通过知识蒸馏的方式让学生模型学习教师的知识。研究了不同权重分配策略来平衡特权信息和标准输入的学习。

Result: 在五个先进目标检测模型和多个公开数据集（包括无人机垃圾检测数据集和Pascal VOC 2012）上的实验表明，LUPI训练的学生模型在检测精度上显著超越基线模型，特别是对中型和大型物体效果更明显。中间权重分配策略能最优平衡特权信息和标准输入的学习。

Conclusion: LUPI框架为目标检测系统提供了一种有效且实用的提升策略，特别适用于资源受限和实际应用场景，能在不增加推理复杂度和模型大小的情况下显著提升检测性能。
...
TL;DR: GleSAM++通过生成式潜在空间增强提升SAM在低质量图像上的分割鲁棒性，引入特征分布对齐、通道复制扩展和退化感知自适应增强机制，显著改善复杂退化条件下的分割性能。


<details>
  <summary>Details</summary>
Motivation: SAM在严重退化、低质量图像上的性能显著下降，限制了其在真实场景中的应用。需要提升SAM在低质量图像上的鲁棒性，同时保持对清晰图像的泛化能力。

Method: 1. 使用生成式潜在空间增强提升低质量图像的鲁棒性；2. 引入特征分布对齐和通道复制扩展改善预训练扩散模型与分割框架的兼容性；3. 提出退化感知自适应增强机制，将重建过程解耦为退化程度预测和退化感知重建两个阶段。

Result: GleSAM++显著提升了在复杂退化条件下的分割鲁棒性，同时保持对清晰图像的泛化能力。在未见过的退化类型上也表现良好，证明了方法的通用性和数据集的多样性。

Conclusion: GleSAM++通过生成式潜在空间增强和退化感知机制，有效解决了SAM在低质量图像上的性能下降问题，仅需少量可学习参数即可应用于预训练的SAM和SAM2，具有高效性和通用性。
...
TL;DR: ADAE：一种事件引导的时空融合框架，用于增强Depth Anything在退化场景（极端光照、运动模糊）中的深度估计能力


<details>
  <summary>Details</summary>
Motivation: 当前深度基础模型（如Depth Anything）在理想场景中表现优异，但在极端光照和运动模糊等恶劣成像条件下性能下降。这些退化会破坏帧相机的视觉信号，削弱帧基深度特征在时空维度上的判别能力。现有方法通常引入事件相机来利用其高动态范围和时间分辨率，但专门的融合模型需要从头训练，无法继承基础模型的开放世界知识和鲁棒泛化能力。

Method: 提出ADAE框架，包含两个核心组件：1）熵感知空间融合：使用信息熵策略自适应融合帧基和事件基特征，以指示光照引起的退化；2）运动引导时间校正：利用事件基运动线索重新校准模糊区域的模糊特征。这两个组件在统一框架中相互补充，共同增强Depth Anything在恶劣成像条件下的性能。

Result: 通过大量实验验证了所提方法的优越性。代码将在接受后发布。

Conclusion: ADAE成功地将事件相机的优势与深度基础模型的开放世界知识相结合，通过熵感知空间融合和运动引导时间校正，显著提升了在动态和恶劣光照条件下的深度估计鲁棒性。
...
TL;DR: 无需3D标注数据或RGB图像配对，通过虚拟相机将3D点云投影到2D图像，利用基础2D模型和自然语言提示进行语义分割，通过多视角加权投票实现3D分割，性能优于现有无训练方法，接近监督方法，支持开放词汇识别。


<details>
  <summary>Details</summary>
Motivation: 传统3D语义分割方法需要大量标注的3D训练数据，这既耗时又昂贵。现有方法通常依赖配对RGB图像或预训练模型。本文旨在开发一种无需3D标注数据、无需配对RGB图像的无训练方法，同时支持开放词汇识别，突破传统监督方法的类别限制。

Method: 1) 使用虚拟相机将3D点云投影到多个2D视角图像；2) 利用基础2D视觉模型（如CLIP等）结合自然语言提示对每个2D投影进行语义分割；3) 通过加权投票机制聚合多个视角的预测结果，形成最终的3D语义分割；4) 支持任意文本查询的开放词汇识别。

Result: 方法在多个大规模点云数据集上验证：1) 性能优于现有的无训练方法；2) 分割精度达到与监督方法相当的水平；3) 成功实现开放词汇识别，用户可以使用任意文本查询检测物体；4) 克服了传统监督方法类别固定的局限性。

Conclusion: 本文提出了一种创新的无训练3D语义分割框架，通过2D投影和基础模型结合自然语言提示，实现了无需3D标注的高质量分割。该方法不仅性能优越，还支持开放词汇识别，为3D场景理解提供了更灵活、更实用的解决方案。
...
TL;DR: AlignVTOFF提出了一种用于虚拟试穿(VTOFF)的并行U-Net框架，通过参考U-Net和纹理-空间特征对齐(TSFA)解决现有方法在复杂几何变形和高频纹理保留方面的不足，显著提升了平铺服装生成的结构真实感和细节保真度。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿方法依赖轻量级模块进行快速特征提取，但难以保持结构化图案和细粒度细节，导致生成过程中纹理衰减。特别是在处理复杂几何变形和丰富高频纹理的平铺服装时，这一问题尤为突出。

Method: 提出AlignVTOFF框架，包含两个核心组件：1) 参考U-Net进行多尺度特征提取并增强几何保真度；2) 纹理-空间特征对齐(TSFA)模块，通过可训练的交叉注意力模块和冻结的自注意力模块的混合设计，将参考服装特征注入冻结的去噪U-Net中，显式对齐纹理和空间线索。

Result: 在多种设置下的广泛实验表明，AlignVTOFF持续优于现有最先进方法，生成的平铺服装结果在结构真实性和高频细节保真度方面均有显著提升。

Conclusion: AlignVTOFF通过创新的并行U-Net架构和纹理-空间特征对齐机制，有效解决了虚拟试穿中纹理衰减和几何变形保真度问题，为高质量平铺服装生成提供了有效的解决方案。
...
TL;DR: Agentic Retoucher是一个分层决策驱动框架，通过感知-推理-行动循环解决文本到图像生成中的局部失真问题，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型（如SDXL、FLUX）虽然实现了高真实感，但在肢体、面部、文字等细节上普遍存在小尺度失真。现有细化方法要么需要昂贵的迭代重新生成，要么依赖空间定位能力弱的视觉语言模型，导致语义漂移和不可靠的局部编辑。

Method: 提出了Agentic Retoucher分层决策驱动框架，将后生成修正重构为类人的感知-推理-行动循环：1) 感知代理学习上下文显著性，在文本-图像一致性线索下进行细粒度失真定位；2) 推理代理通过渐进偏好对齐执行人类对齐的推断诊断；3) 行动代理根据用户偏好自适应规划局部修复。该设计将感知证据、语言推理和可控修正集成到统一的自我纠正决策过程中。

Result: 构建了GenBlemish-27K数据集（包含6K T2I图像，涵盖12个类别的27K标注伪影区域）。大量实验表明，Agentic Retoucher在感知质量、失真定位和人类偏好对齐方面始终优于最先进方法。

Conclusion: Agentic Retoucher为自我纠正和感知可靠的文本到图像生成建立了新范式，通过分层决策驱动框架有效解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 正颌手术需要准确的术后面部形态模拟，但传统生物力学模型计算成本高，几何深度学习方法缺乏可解释性，需要开发更优的预测框架。

Method: PhysSFI-Net包含三个组件：1）具有颅面和手术计划编码器的分层图模块，结合注意力机制提取骨骼-面部交互特征；2）基于LSTM的序列预测器用于增量软组织变形；3）生物力学启发的模块用于高分辨率面部表面重建。

Result: 在135名患者数据上，PhysSFI-Net达到点云形状误差1.070±0.088mm，表面偏差误差1.296±0.349mm，标志点定位误差2.445±1.326mm，优于现有方法ACMT-Net。
...
TL;DR: 提出首个大规模仅使用光学图像的冰碛垄分割数据集，并开发轻量级MCD-Net模型，在降低60%计算成本的同时实现62.3% mIoU的分割性能。
...
Method: 创建包含3,340张手动标注高分辨率Google Earth图像的数据集，开发MCD-Net模型，集成MobileNetV2编码器、CBAM注意力模块和DeepLabV3+解码器。

Result: MCD-Net达到62.3% mIoU和72.8% Dice系数，计算成本降低60%以上，证明仅光学图像能提供可靠的冰碛体分割。

Conclusion: 建立了可复现的冰碛垄分割基准，为高海拔冰川监测提供了可部署的基线，数据集和代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 从单目视频重建完整可动画的3D人体化身仍然具有挑战性，特别是在严重遮挡情况下。现有基于3D高斯溅射的方法在遮挡区域重建时存在几何损坏和时间不一致问题

Method: 提出两种关键创新：1）多尺度UV参数化表示，采用分层粗到细特征插值，实现遮挡区域的鲁棒重建；2）身份保持扩散修复模块，结合文本反转和语义条件引导，实现特定主体、时间一致的补全

Result: 在合成基准（PeopleSnapshot、ZJU-MoCap）和真实场景（OcMotion）上展示了具有竞争力的性能，在不同姿态和视角下重建质量有持续改进

Conclusion: InpaintHuman能够从遮挡的单目视频生成高保真、完整且可动画的化身，相比基于SDS的方法，采用直接像素级监督确保身份保真度
...
<details>
  <summary>Details</summary>
Motivation: 传统多视角立体方法在稀疏视角或低纹理区域效果不佳，神经渲染方法虽然质量高但需要逐场景优化且缺乏实时性。现有的前馈3D高斯泼溅方法主要关注视觉质量而非几何一致性，限制了准确表面重建和空间感知任务的可靠性。

Method: 提出新颖的前馈3D高斯泼溅框架，引入深度-法向几何正则化，将渲染深度梯度与法向信息耦合，监督高斯的旋转、尺度和位置，以改善点云和表面精度。

Result: 实验结果表明，该方法在保持高质量渲染的同时，显著提升了几何一致性，为空间感知任务中的3D重建提供了有效解决方案。

Conclusion: 该框架成功解决了现有3D高斯泼溅方法在几何一致性方面的不足，实现了高质量渲染与准确几何重建的平衡，适用于AR、机器人和数字孪生等空间智能应用。
...
TL;DR: HeadLighter：基于3D高斯泼溅的头像生成模型，通过物理可解释的分解方法实现可控重光照，解决了现有方法中光照与内在外观深度纠缠的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的3D感知头像生成模型虽然能实现实时、逼真且视角一致的合成，但存在根本限制：光照与内在外观深度纠缠，导致无法进行可控重光照。现有解纠缠方法依赖强假设进行弱监督学习，限制了处理复杂光照的能力。

Method: 提出HeadLighter框架，采用双分支架构分别建模光照不变的头像属性和基于物理的渲染组件。使用渐进式解纠缠训练，在受控光照条件下采集的多视角图像监督下，逐步将头像外观先验注入生成架构。引入蒸馏策略生成高质量法线以实现逼真渲染。

Result: 实验表明，该方法在保持高质量生成和实时渲染的同时，支持显式光照和视角编辑。将公开代码和数据集。

Conclusion: HeadLighter成功解决了头像生成模型中光照与外观的纠缠问题，实现了物理可解释的分解，为可控重光照提供了有效解决方案，同时保持了生成质量和渲染效率。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到视频生成主要集中在通用场景和单人个性化生成，但双人交互特别是武术对战领域尚未探索。现有单人舞蹈生成模型无法捕捉双人战斗的细微差别和复杂性，导致身份混淆、异常肢体和动作不匹配等问题

Method: 使用Unity游戏物理引擎创建定制数据集，包含多样化的3D角色、武术动作和场景。MagicFight方法改进和适配现有模型和策略，生成保持个体身份和连贯动作序列的高保真双人战斗视频

Result: 成功生成了高质量的双人武术对战视频，解决了身份混淆和动作不匹配问题，为交互式视频内容创作奠定了基础

Conclusion: 提出了首个个性化武术对战视频生成任务，开发了MagicFight方法和专用数据集，填补了双人交互视频生成领域的空白，为未来交互式视频内容创新铺平了道路
...
TL;DR: 提出一种基于序列切片处理的轻量级代理模型，用于预测3D车辆的空气阻力系数，通过将点云分解为二维截面序列，结合PointNet2D和双向LSTM实现快速准确预测。


<details>
  <summary>Details</summary>
Motivation: 汽车行业追求更好的燃油经济性和性能需要高效的气动设计，但传统的CFD和风洞测试方法资源消耗大，阻碍了早期设计阶段的快速迭代。现有的机器学习代理模型存在计算复杂度高、可解释性差或对详细几何输入精度不足的问题。

Method: 受医学影像启发，将3D车辆点云沿流向轴分解为有序的2D横截面切片序列。每个切片通过轻量级PointNet2D模块编码，切片嵌入序列由双向LSTM处理以捕捉纵向几何演化，从而预测空气阻力系数(Cd)。

Result: 在DrivAerNet++数据集上训练和评估，模型在Cd预测中实现了高决定系数(R^2 > 0.9528)和低平均绝对误差(MAE ≈ 6.046×10^{-3})。在消费级GPU上每个样本的推理时间约为0.025秒。

Conclusion: 该方法提供了快速、准确且可解释的气动反馈，促进了更敏捷和明智的汽车设计探索，为早期设计阶段的高效迭代提供了有前景的解决方案。
...
Method: 1) 将单时相数据集扩展为多时相观测；2) 假设真实双时相对大多无变化，而不同位置图像配对生成变化示例；3) 采用对象感知变化图生成和迭代精化处理弱标签噪声

Result: 在扩展的FLAIR和IAILD航空数据集上验证，在零样本和低数据场景下在不同基准测试中表现优异，并在法国大区域展示方法的可扩展性

Conclusion: 提出的弱时间监督策略有效解决了遥感变化检测的数据稀缺问题，无需额外标注，具有良好的泛化能力和可扩展性
...
TL;DR: 提出OSCD任务和TAHI框架，通过双时相SAR图像变化检测提高油污识别准确性，减少误报


<details>
  <summary>Details</summary>
Motivation: 现有基于单幅SAR图像的深度学习分割方法难以区分真实油污与视觉相似的海面特征（如生物膜、低风区），导致高误报率和有限泛化能力，特别是在数据稀缺条件下

Method: 提出油污变化检测（OSCD）任务，并开发时间感知混合修复（TAHI）框架，包含高保真混合修复（用于无油重建）和时间真实性增强（用于辐射和海洋状态一致性）两个关键组件

Result: OSCD显著减少了误报并提高了检测准确性，相比传统分割方法表现更优，证明了时间感知方法在实际油污监测中的价值

Conclusion: OSCD任务和TAHI框架为可靠、可扩展的油污监测提供了有效解决方案，通过双时相变化检测方法克服了传统单图像分割的局限性
...
### [145] [Towards Long-window Anchoring in Vision-Language Model Distillation](https://arxiv.org/abs/2512.21576)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型的小分支由于窗口大小有限，在语言-图像对齐方面表现不佳。虽然大模型有强大的长上下文理解能力，但如何将这种能力有效转移到小模型中是一个挑战

Method: 提出LAid框架，包含两个互补组件：1) 渐进距离加权注意力匹配，在训练中动态强调更长的位置差异；2) 可学习RoPE响应增益调制，选择性增强需要的位置敏感性

Result: LAid蒸馏的模型相比基线小模型，有效上下文窗口最多扩大3.2倍，同时在标准视觉语言基准测试上保持或提升性能。频谱分析显示LAid成功保留了传统方法无法转移的关键低频注意力组件
...
<details>
  <summary>Details</summary>
Motivation: 当前深度学习方法在成像逆问题中表现出色，但大多数方法无法处理大规模问题（如3D成像），因为全局前向算子需要大量内存，阻碍了典型的分块策略。

Method: 提出域分割策略和正规算子近似方法，将任意大规模问题的前向算子纳入网络架构，使端到端重建模型能够在单GPU上进行训练和推理。

Result: 在3D X射线锥束断层扫描和3D多线圈加速MRI上实现了最先进的性能，同时仅需单个GPU进行训练和推理。

Conclusion: 该方法成功解决了大规模成像问题中内存限制的挑战，为3D成像等大规模逆问题提供了高效的端到端重建解决方案。
...
TL;DR: BiPrompt：双边提示优化框架，通过视觉结构化注意力引导擦除和文本平衡提示归一化，同时减少视觉和文本模态中的虚假相关性，提升CLIP等视觉语言基础模型的零样本泛化能力。
...
Method: 提出双边提示优化框架（BiPrompt）：1）视觉侧：采用结构化注意力引导擦除抑制背景激活，强制因果区域与虚假区域间的正交预测一致性；2）文本侧：引入平衡提示归一化，通过可学习的重新中心化机制将类别嵌入对齐到各向同性语义空间。

Result: 在真实世界和合成偏置基准测试中，相比先前测试时去偏方法，在平均准确率和最差组准确率上均取得一致改进。
...
TL;DR: 使用商品ESP32 WiFi传感器进行多人步态识别性能有限，所有信号分离方法准确率仅45-56%，表明硬件限制而非算法问题是主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 虽然WiFi CSI在单人步态识别中表现良好，但多人识别研究不足，现有方法需要复杂昂贵的设备。关键问题是：多人识别性能差是算法限制还是硬件约束？

Method: 系统评估六种信号分离方法（FastICA、SOBI、PCA、NMF、小波变换、张量分解）在1-10人场景下的表现，使用商品ESP32 WiFi传感器，并引入新的诊断指标（主体内变异性、主体间可区分性、性能退化率）。

Result: 所有方法准确率相似且较低（45-56%，σ=3.74%），统计差异不显著（p>0.05）。最佳方法NMF仅56%准确率。分析显示高主体内变异性、低主体间可区分性，以及随人数增加性能急剧下降。

Conclusion: 商品ESP32传感器无法提供足够的信号质量来实现可靠的多人分离，多人识别性能差主要是硬件限制而非算法问题。
...
<details>
  <summary>Details</summary>
Motivation: 在资源受限的边缘设备上部署细粒度视觉分类模型面临挑战：深层网络计算成本高，浅层网络（如AlexNet、VGG）无法区分视觉相似的子类别。标准全局平均池化仅捕捉一阶统计量，而双线性CNN存在特征维度爆炸和训练不稳定问题。

Method: 提出量子启发的交互分类器QuIC，将特征通道建模为相互作用的量子态，通过可学习的可观测量算子捕捉二阶特征协方差。QuIC是一个轻量级即插即用模块，支持稳定的单阶段端到端训练，不会导致特征维度爆炸。

Result: QuIC显著提升了浅层骨干网络的性能：将VGG16的Top-1准确率提高了近20%，在ResNet18上超越了最先进的注意力机制SE-Block。t-SNE可视化分析证实QuIC通过显式关注细粒度判别性特征并强制紧凑的类内聚类，解决了模糊案例。

Conclusion: QuIC通过量子启发的特征交互建模，有效解决了细粒度视觉分类中浅层网络性能不足的问题，在保持计算效率的同时显著提升分类准确率，为边缘设备上的细粒度分类提供了实用解决方案。
...
TL;DR: 病理学基础模型在训练时的放大倍数采样策略影响性能，连续采样比离散采样更好，优化的分布能进一步提升性能


<details>
  <summary>Details</summary>
Motivation: 病理学家在诊断时会同时观察低倍镜下的组织结构和高倍镜下的精细形态，但现有病理学基础模型在不同放大倍数下的性能表现以及训练时放大倍数采样的影响尚不清楚

Method: 将放大倍数采样建模为多源域适应问题，提出理论框架分析采样策略的权衡，引入连续放大倍数采样方法，推导优化表示质量的采样分布，并建立两个新基准（TCGA-MS, BRACS-MS）进行评估

Result: 连续采样在中间放大倍数上显著优于离散采样，平衡分类准确率提升达4个百分点，优化分布能进一步提高性能；发现放大倍数是当前病理学基础模型性能变化的主要驱动因素

Conclusion: 连续放大倍数采样能消除放大倍数覆盖的空白，在标准尺度上保持性能的同时提升中间尺度的表现，为未来可靠跨放大倍数的病理学基础模型奠定了基础
...
TL;DR: 提出基于WiFi CSI的无设备人群计数两阶段框架，通过自监督对比学习预训练CSI-ResNet-A学习领域不变表示，结合轻量适配器微调和状态计数机，在跨域泛化上取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 基于WiFi CSI的无设备人群计数是隐私保护物联网应用的关键技术，但实际部署面临严重的领域偏移问题——在一个环境训练的模型无法泛化到其他环境。

Method: 提出两阶段框架：1) 使用CSI-ResNet-A架构，通过自监督对比学习预训练学习领域不变表示；2) 利用轻量适配器模块进行高效微调，最后通过状态计数机处理事件序列生成稳定占用估计。

Result: 在WiFlow数据集上，无监督方法在10-shot学习场景中达到MAE仅0.44，而监督基线失败；提出的泛化指数(GI)接近完美；在公开WiAR基准上达到98.8%准确率的新SOTA；适配器微调性能接近全微调(98.84% vs 99.67%)但参数减少97.2%。

Conclusion: 该工作为开发稳健感知系统提供了实用且可扩展的解决方案，适用于真实世界物联网部署，通过领域不变表示学习和高效微调机制有效解决了跨域泛化问题。
...
TL;DR: NextFlow是一个统一的解码器自回归Transformer，在6万亿交错文本-图像离散token上训练，通过统一架构实现多模态理解和生成，包括图像编辑、交错内容和视频生成，采用next-scale预测实现快速高质量图像生成。


<details>
  <summary>Details</summary>
Motivation: 不同模态具有本质差异：文本是严格顺序的，而图像是层次结构的。传统的光栅扫描方法效率低下，需要更高效的视觉生成方法。同时，需要统一的架构来激活多模态理解和生成能力。

Method: 1. 统一解码器自回归Transformer架构；2. 对文本保留next-token预测，对视觉采用next-scale预测；3. 多尺度生成稳定性训练方案；4. 强化学习的prefix-tuning策略；5. 在6万亿交错文本-图像离散token上训练。

Result: 1. 生成1024x1024图像仅需5秒，比同类AR模型快几个数量级；2. 在统一模型中达到最先进性能；3. 在视觉质量上与专业扩散模型基线相媲美；4. 解锁了图像编辑、交错内容和视频生成能力。

Conclusion: NextFlow通过统一架构和创新的next-scale预测方法，实现了高效高质量的多模态理解和生成，为统一多模态模型提供了有前景的方向，在速度和视觉质量上都取得了显著突破。
...
TL;DR: RetinexEVSR：首个事件驱动的低光视频超分辨率框架，利用高对比度事件信号和Retinex先验，通过双向跨模态融合策略提升低光视频质量，在性能和效率上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有低光视频超分辨率方法由于对比度有限和高频信息不足，难以恢复细节。需要利用高对比度事件信号和Retinex理论来克服这些挑战。

Method: 提出RetinexEVSR框架：1）双向跨模态融合策略，从噪声事件数据和退化RGB帧中提取整合信息；2）光照引导的事件增强模块，利用Retinex模型的光照图逐步精炼事件特征；3）事件引导的反射率增强模块，通过多尺度融合动态恢复反射率细节。

Result: 在三个数据集上达到SOTA性能，在SDSD基准上获得最高2.95dB增益，同时运行时间比先前事件方法减少65%。

Conclusion: RetinexEVSR通过有效融合事件信号和Retinex先验，显著提升了低光视频超分辨率的性能，在恢复细节和计算效率方面均有优势。
...
TL;DR: 本文系统分析了基于MMDiT的扩散模型内部机制，发现语义信息在早期块出现、细节在后期块渲染，并提出无需训练的策略来改进文本对齐、精确编辑和加速推理。


<details>
  <summary>Details</summary>
Motivation: 尽管基于MMDiT的扩散模型（如FLUX和Qwen Image）在文本到图像生成方面取得了突破，但对其内部机制的理解仍然有限。现有方法只分析了特定组件（如位置编码和注意力层），缺乏对不同块及其与文本条件交互如何影响合成过程的全面理解。

Method: 开发了一个系统化分析流程，通过移除、禁用和增强相应块中的文本隐藏状态来全面研究每个块的功能。基于分析发现，提出了无需训练的策略来改进文本对齐、精确编辑和加速推理。

Result: 方法在文本到图像生成、图像编辑和推理加速方面优于各种基线。在SD3.5上，T2I-Combench++从56.92%提升到63.00%，GenEval从66.42%提升到71.63%，且不牺牲合成质量。

Conclusion: 该研究推进了对MMDiT模型的理解，为未来改进提供了有价值的见解，并展示了无需训练策略在提升文本对齐、编辑精度和推理速度方面的有效性。
...
### [155] [VIBE: Visual Instruction Based Editor](https://arxiv.org/abs/2601.02242)
...
TL;DR: 提出一个紧凑高效的指令式图像编辑系统，使用2B参数的Qwen3-VL指导编辑和1.6B参数的Sana1.5生成图像，在保持高质量的同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前指令式图像编辑领域虽然发展迅速，但开源模型大多质量有限，而主流的扩散模型通常参数庞大（6B-20B），计算成本高昂，不适合实际部署和研究场景。

Method: 采用轻量级架构：使用2B参数的Qwen3-VL模型指导编辑过程，配合1.6B参数的Sana1.5扩散模型进行图像生成。在架构设计、数据处理、训练配置和评估方面都针对低成本推理和严格的源一致性进行优化。

Result: 在ImgEdit和GEdit基准测试中，该方法匹配或超越了参数多几倍、推理成本更高的基线模型。在需要保持输入图像的编辑任务（如属性调整、物体移除、背景编辑和定向替换）上表现尤为出色。模型仅需24GB GPU内存，在NVIDIA H100上以BF16精度生成2K分辨率图像约需4秒。

Conclusion: 该研究证明通过精心设计的轻量级架构，可以在大幅降低计算成本的同时实现高质量的指令式图像编辑，为实际部署和研究提供了可行的解决方案。
...
### [156] [Prior-Guided DETR for Ultrasound Nodule Detection](https://arxiv.org/abs/2601.02212)
*Jingjing Wang,Zhuo Xiao,Xinning Yao,Bo Liu,Lijuan Niu,Xiangzhi Bai,Fugen Zhou*

Main category: cs.CV

TL;DR: 提出一种基于先验知识引导的DETR框架，用于超声结节检测，通过几何先验、多尺度结构先验和密集特征交互机制，显著提升不规则、边界模糊结节的检测精度。


<details>
  <summary>Details</summary>
Motivation: 超声结节检测对甲状腺和乳腺癌的早期诊断至关重要，但由于结节形状不规则、边界模糊、尺度变化大以及斑点噪声影响结构可见性，该任务仍然具有挑战性。

Method: 提出先验引导的DETR框架：1) SDFPR模块在CNN骨干网络中注入几何先验，稳定不规则和模糊结节的特征提取；2) MSFFM模块提取多尺度结构先验，空间域处理强调轮廓连续性，频域建模捕获全局形态并抑制斑点噪声；3) DFI机制在所有编码器层间传播和利用先验调制特征，使解码器能在一致的几何和结构指导下增强查询细化。

Result: 在两个临床收集的甲状腺超声数据集（Thyroid I和Thyroid II）以及两个公开基准（TN3K和BUSI）上进行的实验表明，该方法相比18种检测方法实现了更高的准确率，特别是在检测形态复杂结节方面表现优异。

Conclusion: 提出的先验引导DETR框架通过渐进式整合几何和结构先验知识，有效解决了超声结节检测中的挑战，为不规则、边界模糊结节的准确检测提供了有效解决方案。
...
TL;DR: 该研究系统比较了三种CNN应用范式（从头训练小型网络、使用预训练网络作为特征提取器、迁移学习微调），在五个真实图像分类数据集上的表现，发现迁移学习性能最佳，而定制CNN在计算效率与精度间提供更好平衡。


<details>
  <summary>Details</summary>
Motivation: 实践中，视觉识别任务通常面临三种CNN应用范式的选择：从头训练定制网络、使用预训练网络作为固定特征提取器、或进行迁移学习微调。然而缺乏对这些范式在真实世界数据集上的系统比较，特别是在效率和准确性权衡方面的指导。

Method: 在五个真实世界图像分类数据集（路面缺陷识别、农作物品种识别、水果/叶片病害识别、人行道侵占识别、未授权车辆识别）上，对三种CNN范式进行受控比较：1）从头训练紧凑型定制CNN；2）使用大型预训练CNN作为固定特征提取器；3）通过部分或完整微调预训练骨干网络进行迁移学习。评估指标包括准确率、宏F1分数，以及训练时间每轮和参数数量等效率指标。

Result: 迁移学习在所有数据集上始终表现出最强的预测性能，而定制CNN在效率和准确性之间提供了有吸引力的权衡，特别是在计算和内存预算受限的情况下。预训练网络作为固定特征提取器的性能介于两者之间。

Conclusion: 迁移学习是获得最佳预测性能的首选方法，但当计算资源有限时，从头训练定制CNN提供了更好的效率-准确性平衡。实践者应根据具体应用的计算约束和性能要求在这三种范式之间做出选择。
...
Method: 提出FMVP：1）通过掩码策略物理破坏全局对抗结构；2）使用条件流匹配（CFM）和修复目标重建干净视频动态；3）设计频率门控损失（FGL）抑制高频对抗残差同时保持低频保真度；4）设计攻击感知和通用训练范式处理已知和未知威胁

Result: 在UCF-101和HMDB-51上，FMVP优于现有方法（DiffPure、DP、TS、FlowPure），对PGD攻击达到87%以上鲁棒准确率，对CW攻击达到89%。对自适应攻击（DiffHammer）表现出优越鲁棒性，并可作为零样本对抗检测器，对PGD检测准确率达98%，对CW达79%

Conclusion: FMVP通过物理破坏对抗结构和流匹配重建，有效净化对抗视频，在多种攻击场景下表现出优越性能，同时具备对抗检测能力
...
### [159] [TopoLoRA-SAM: Topology-Aware Parameter-Efficient Adaptation of Foundation Segmenters for Thin-Structure and Cross-Domain Binary Semantic Segmentation](https://arxiv.org/abs/2601.02273)
*Salim Khazem*

Main category: cs.CV

TL;DR: TopoLoRA-SAM：基于拓扑感知的低秩适配SAM框架，用于医学图像和SAR图像的二元语义分割，仅需训练5.2%参数即可超越全微调模型


<details>
  <summary>Details</summary>
Motivation: 基础分割模型如SAM在零样本泛化方面表现优异，但难以适应特定领域的语义分割任务，特别是对薄结构（如视网膜血管）和噪声模态（如SAR图像）。全微调计算成本高且可能导致灾难性遗忘。

Method: 提出TopoLoRA-SAM框架：在冻结的ViT编码器中注入低秩适配（LoRA），增强轻量级空间卷积适配器，可选通过可微分clDice进行拓扑感知监督。

Result: 在五个基准数据集上评估：视网膜血管分割（DRIVE, STARE, CHASE_DB1）、息肉分割（Kvasir-SEG）和SAR海陆分割（SL-SSDD）。TopoLoRA-SAM在视网膜平均Dice和整体平均Dice上表现最佳，仅训练5.2%参数（约490万）。在CHASE_DB1数据集上显著提升分割准确性和鲁棒性。

Conclusion: 拓扑感知的参数高效适配方法能够匹配甚至超越完全微调的专用模型，为领域特定分割任务提供了高效解决方案。
...
TL;DR: SLGNet：一种参数高效的多模态目标检测框架，结合层次结构先验和语言引导调制，在冻结的ViT基础模型上实现跨模态结构一致性和环境感知


<details>
  <summary>Details</summary>
Motivation: 现有基于适配器的方法在将RGB预训练基础模型迁移到RGB-IR多模态检测任务时，往往牺牲跨模态结构一致性来换取模型效率，导致在高对比度或夜间环境等显著域差距场景中丢失关键结构线索。同时，传统静态多模态融合机制缺乏环境感知能力，在复杂动态场景变化下适应性不足。

Method: 提出SLGNet框架：1) 结构感知适配器提取两种模态的层次结构表示并动态注入ViT，补偿ViT主干的结构退化；2) 语言引导调制模块利用VLM驱动的结构化描述动态重新校准视觉特征，赋予模型环境感知能力；3) 在冻结的ViT基础模型上实现参数高效学习。

Result: 在LLVIP、FLIR、KAIST和DroneVehicle数据集上达到最先进性能。在LLVIP基准上实现66.1 mAP，相比传统全微调减少约87%可训练参数。

Conclusion: SLGNet通过结合层次结构先验和语言引导调制，为多模态感知提供了鲁棒且高效的解决方案，在保持参数效率的同时显著提升了跨模态结构一致性和环境适应性。
...
TL;DR: 提出新框架增强GRPO，解决VAR模型在强化学习中的异步策略冲突问题，通过稳定奖励、动态时间步重加权和掩码传播算法提升样本质量和目标对齐。


<details>
  <summary>Details</summary>
Motivation: VAR模型在生成过程中存在异构输入结构，导致严重的异步策略冲突，在强化学习场景中尤其突出，造成训练不稳定和对齐效果不佳。

Method: 提出包含三个协同组件的框架：1) 稳定中间奖励引导早期生成；2) 动态时间步重加权方案进行精确信用分配；3) 基于ReFL原理的掩码传播算法，在空间和时间上隔离优化效果。

Result: 相比原始GRPO基线，该方法在样本质量和目标对齐方面取得显著改进，实现了对VAR模型的稳健有效优化。

Conclusion: 通过管理异步策略冲突，提出的框架成功解决了VAR模型在强化学习中的优化问题，为视觉生成模型提供了更稳定的训练方法。
...
TL;DR: DiffProxy使用扩散模型生成多视角一致的人体代理，通过合成数据训练实现真实场景的零样本泛化，在遮挡和部分视角等挑战性场景表现优异。


<details>
  <summary>Details</summary>
Motivation: 真实世界数据集标注不完美会引入训练偏差，而合成数据虽有精确标注但存在领域差距。需要一种方法既能利用合成数据的精确监督，又能实现真实世界的泛化。
...
Conclusion: DiffProxy通过扩散生成先验成功桥接了合成训练与真实泛化，使网格恢复过程能同时受益于精确合成标注和扩散管道的生成优势。
...
TL;DR: InfiniteVGGT：首个支持无限长视频流输入的因果视觉几何Transformer，通过滚动内存KV缓存和训练无关的剪枝策略，解决了长期3D几何理解中可扩展性与稳定性之间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉几何理解方法面临可扩展性与长期稳定性的矛盾：离线模型（如VGGT）无法处理实时流数据，而流式架构要么不支持无限长输入，要么在长序列中产生灾难性漂移。

Method: 提出因果视觉几何Transformer，采用有界但自适应且持续表达的KV缓存作为滚动内存，配合训练无关、注意力无关的剪枝策略，智能丢弃过时信息，实现内存向前滚动。

Result: InfiniteVGGT在无限长流式处理中超越现有流式方法，保持长期稳定性，完全兼容FlashAttention。同时提出Long3D基准测试（约10,000帧序列），为长期3D几何理解提供首个严格评估平台。

Conclusion: InfiniteVGGT解决了3D视觉几何理解中长期存在的可扩展性与稳定性矛盾，首次实现无限长视频流处理，并通过Long3D基准为未来研究提供关键评估工具。
...
TL;DR: GeoRank是一种用于多光谱遥感图像对比自监督学习的新型正则化方法，通过优化球面距离将地理关系嵌入特征空间，在多种对比学习算法上取得改进。


<details>
  <summary>Details</summary>
Motivation: 自监督学习在计算机视觉中表现出色，但应用于多光谱遥感图像面临独特挑战，如地理和时间变化性。现有方法未能充分利用地理关系信息。

Method: 提出GeoRank正则化方法，直接优化球面距离将地理关系嵌入对比自监督学习的特征空间。同时系统研究多光谱遥感图像对比学习的关键适应性问题，包括数据增强效果、数据集规模和图像大小影响、时间视图的任务依赖性。

Result: GeoRank优于或匹配集成地理元数据的现有方法，并持续改进多种对比自监督学习算法（如BYOL、DINO）。

Conclusion: GeoRank为多光谱遥感图像自监督学习提供有效的正则化方法，通过嵌入地理关系提升特征学习质量，同时为领域适应性提供系统分析。
...
TL;DR: 提出SortWaste数据集和ClutterScore评估指标，用于解决废物自动分拣中缺乏真实数据集和视觉复杂度评估的问题


<details>
  <summary>Details</summary>
Motivation: 废物产量增加带来管理挑战，人工分拣效率低且有健康风险，现有自动分拣系统难以处理真实废物流的高变异性、杂乱性和视觉复杂性，缺乏真实数据集是主要瓶颈

Method: 1) 引入SortWaste数据集，从材料回收设施收集并密集标注；2) 提出ClutterScore评估指标，使用对象数量、类别和尺寸熵、空间重叠等代理指标量化场景硬度；3) 对最先进的目标检测模型进行基准测试

Result: 在塑料检测任务中达到59.7% mAP，但在高度杂乱场景中性能显著下降，表明需要更具挑战性的数据集

Conclusion: SortWaste数据集和ClutterScore指标为废物自动分拣研究提供了重要基础，但当前模型在复杂场景中仍有局限，需要开发更鲁棒的算法和更具挑战性的数据集
...
TL;DR: 首个基于深度学习的单目全景视觉里程计框架，通过失真感知球形特征提取器和全景可微分光束法平差模块，在挑战性场景中显著提升鲁棒性和精度
...
Method: 提出360DVO框架：1) 失真感知球形特征提取器(DAS-Feat)自适应学习抗失真特征；2) 稀疏特征块用于姿态估计约束；3) 新颖的全景可微分光束法平差(ODBA)模块

Result: 在新构建的真实世界基准和公开合成数据集(TartanAir V2和360VO)上，360DVO超越最先进基线(包括360VO和OpenVSLAM)，鲁棒性提升50%，精度提升37.5%

Conclusion: 360DVO是首个基于深度学习的全景视觉里程计框架，通过自适应特征学习和可微分优化，在挑战性场景中实现了显著性能提升，为全景视觉里程计研究提供了新方向
...
TL;DR: Prithvi-CAFE通过融合Prithvi地理基础模型与CNN残差分支，在洪水制图任务中超越了现有GFMs和U-Net基线，显著提升了局部细节捕捉能力。


<details>
  <summary>Details</summary>
Motivation: 现有地理基础模型（GFMs）在洪水制图等下游任务中难以超越传统U-Net基线，主要问题在于无法有效捕捉关键的局部细节信息。

Method: 提出Prithvi-CAFE架构，将Prithvi GFM预训练编码器与并行CNN残差分支结合，通过卷积注意力模块增强，并采用适配器实现快速微调，进行多尺度、多层次的特征融合。

Result: 在Sen1Flood11数据集上，IoU达到83.41，优于原始Prithvi（82.50）和其他GFMs；在保留测试站点上IoU 81.37显著优于U-Net（70.57）和Prithvi（72.42）。在FloodPlanet数据集上IoU 64.70也超越所有对比方法。

Conclusion: Prithvi-CAFE通过简单有效的架构设计，在多通道多模态数据提供互补信息且局部细节关键的分割任务中展现出强大潜力。
...
TL;DR: Fusion2Print (F2P) 框架通过融合闪光-非闪光接触式指纹图像，提升脊线清晰度，实现与接触式指纹兼容的高性能识别。


<details>
  <summary>Details</summary>
Motivation: 接触式指纹识别存在卫生和便利性问题，但接触式图像常因光照变化、皮下变色和镜面反射导致脊线清晰度下降。闪光捕捉保留细节但引入噪声，非闪光捕捉减少噪声但降低对比度。

Method: 构建配对的闪光-非闪光数据集(FNF Database)，通过手动减法分离脊线保留信号；使用轻量级注意力融合网络整合两种模态；U-Net增强模块生成优化灰度图像；深度嵌入模型生成兼容接触式和接触式指纹的统一嵌入表示。

Result: F2P显著提升脊线清晰度，在识别性能上优于单捕捉基线(Verifinger, DeepPrint)，达到AUC=0.999，EER=1.12%的优异结果。

Conclusion: Fusion2Print通过系统性地融合闪光和非闪光接触式指纹，解决了接触式指纹识别的关键挑战，实现了高性能且与现有接触式系统兼容的识别方案。
...
TL;DR: BEDS框架统一非平衡热力学、贝叶斯推断、信息几何和机器学习，提出学习本质上是通过熵输出将通量转化为结构的过程，并建立了热力学过程与贝叶斯更新的同构关系。


<details>
  <summary>Details</summary>
Motivation: 该研究旨在建立一个统一的理论框架，连接物理、生物和计算系统中的学习过程，探索学习的基本原理及其与热力学、数学逻辑的关系，为可持续人工智能提供理论基础。

Method: 基于普里高津的耗散结构理论，建立热力学过程与贝叶斯更新的形式同构，推导基本数学常数作为贝叶斯推断的固定点，提出哥德尔不完备定理与热力学约束的猜想，并设计实现BEDS原则的对等网络架构。

Result: 推导出e、π、φ等基本数学常数作为贝叶斯推断的最小公理固定点；提出哥德尔不完备性与热力学耗散缺陷的结构类比；实现的对等网络架构相比现有分布式共识系统能效提升六个数量级，支持持续学习。

Conclusion: BEDS框架为理解学习的本质提供了统一的理论视角，连接了基础物理、数学逻辑和实际系统设计，既提供了关于学习和计算本质的理论洞见，也为可持续人工智能提供了具体实现路径。
...
TL;DR: 提出联合增强的3D语义高斯建模框架，通过各向异性3D高斯切比雪夫描述符和自适应高斯分配策略，提升语义分割和渲染质量
...
Conclusion: 提出的联合增强框架通过协同语义和渲染分支，有效解决了现有方法的局限性，实现了更好的3D语义建模效果
...
TL;DR: 本文提出DACIS方法结合神经网络剪枝与少样本学习，为边缘设备开发轻量级植物病害检测模型，在保持92.3%准确率的同时将模型大小减少78%，可在树莓派上实现实时诊断。


<details>
  <summary>Details</summary>
Motivation: 偏远地区农民需要快速可靠的植物病害识别方法，但缺乏实验室或高性能计算资源。现有深度学习模型虽然准确率高，但模型过大、计算成本高，无法在低成本边缘设备上运行，且收集大量标注数据既昂贵又耗时。
...
Result: 在PlantVillage和PlantDoc数据集上的实验表明，该方法将模型大小减少78%，同时保持92.3%的原始准确率，压缩后的模型在树莓派4上以7帧/秒的速度运行，实现实时现场诊断。

Conclusion: 提出的DACIS和PMP方法有效解决了边缘设备上植物病害检测的计算资源限制和数据稀缺问题，为小农户提供了实用的实时现场诊断解决方案。
...
TL;DR: Talk2Move是一个基于强化学习的扩散框架，通过自然语言指令实现场景中物体的空间变换（平移、旋转、缩放），无需配对监督数据。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本的编辑方法主要调整外观或风格，难以执行物体级别的几何变换（如平移、旋转、缩放），因为缺乏配对监督数据和像素级优化的限制。

Method: 采用Group Relative Policy Optimization (GRPO)从输入图像和轻量级文本变体中探索几何动作；使用空间奖励引导模型对齐几何变换与语言描述；通过离策略步骤评估和主动步骤采样提高学习效率；设计物体中心的空间奖励直接评估位移、旋转和缩放行为。

Result: 在精心设计的基准测试中，Talk2Move实现了精确、一致且语义忠实的物体变换，在空间准确性和场景连贯性方面优于现有的文本引导编辑方法。

Conclusion: Talk2Move通过强化学习框架成功解决了文本指令下的物体空间变换问题，无需昂贵的配对数据，实现了可解释且连贯的几何变换。
...
TL;DR: VINO是一个统一的视觉生成器，在单一框架内执行图像和视频的生成与编辑，使用共享的扩散主干和多模态条件，避免任务特定模型


<details>
  <summary>Details</summary>
Motivation: 现有视觉生成系统通常需要针对不同模态（图像、视频）和任务（生成、编辑）使用独立的模型或模块，缺乏统一的解决方案。这限制了系统的灵活性和可扩展性，无法实现跨模态的连贯创作

Method: 将视觉语言模型（VLM）与多模态扩散变换器（MMDiT）结合，将多模态输入编码为交错的条件令牌来指导扩散过程。采用多阶段训练流程，从视频生成基础模型逐步扩展到统一的多任务生成器

Result: 在多样化的生成和编辑基准测试中，VINO展现出强大的视觉质量、准确的指令跟随、改进的参考和属性保持能力，以及更可控的多身份编辑效果

Conclusion: VINO为可扩展的统一视觉生成提供了一条实用路径，交错上下文计算作为通用视觉创作的基础展现出巨大潜力，实现了跨静态和动态内容的连贯创作
...
TL;DR: ExposeAnyone是一种基于扩散模型的全自监督方法，通过音频生成表情序列，利用个性化扩散重建误差进行人脸伪造检测，在未知伪造检测上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前深度伪造检测方法主要依赖监督训练，容易过拟合特定伪造模式，无法泛化到未见过的伪造类型。自监督方法虽有潜力，但现有方法难以仅从自监督中学习到判别性表征。

Method: 提出ExposeAnyone全自监督方法：1）使用扩散模型从音频生成表情序列；2）通过参考集将模型个性化到特定主体；3）通过扩散重建误差计算疑似视频与个性化主体之间的身份距离，实现感兴趣人物的人脸伪造检测。

Result: 1）在DF-TIMIT、DFDCP、KoDF和IDForge数据集上，平均AUC比之前最佳方法提升4.22个百分点；2）能够检测Sora2生成的视频，而之前方法表现不佳；3）对模糊和压缩等损坏具有高度鲁棒性。

Conclusion: ExposeAnyone通过全自监督的扩散模型方法，在未知深度伪造检测任务中实现了优异的泛化能力和鲁棒性，适用于现实世界的人脸伪造检测场景。
...
### [175] [Horizon Reduction as Information Loss in Offline Reinforcement Learning](https://arxiv.org/abs/2601.00831)
*Uday Kumar Nidadala,Venkata Bhumika Guthi*

Main category: cs.LG

TL;DR: 本文证明离线强化学习中常用的视野缩减策略会导致不可恢复的信息损失，即使有无限数据和完美函数逼近，最优策略也可能与次优策略统计上无法区分。


<details>
  <summary>Details</summary>
Motivation: 尽管视野缩减在离线强化学习中被广泛用于缓解长视野信用分配、提高稳定性和实现可扩展学习，但其理论影响尚未充分发展。本文旨在揭示视野缩减可能导致的基本信息损失问题。

Method: 将视野缩减形式化为从固定长度轨迹片段中学习，通过最小反例马尔可夫决策过程识别三种结构失效模式：前缀不可区分性导致可识别性失败、截断回报引起的目标错误指定、离线数据集支持和表示混叠。

Result: 证明在固定长度轨迹片段的学习范式下，即使有无限数据和完美函数逼近，最优策略也可能与次优策略统计上无法区分。建立了视野缩减安全性的必要条件，并指出了无法通过算法改进单独克服的内在限制。

Conclusion: 视野缩减在离线强化学习中会导致不可恢复的信息损失，存在根本性限制。研究结果补充了关于保守目标和分布偏移的算法工作，强调了离线RL中不同维度的困难。
...
TL;DR: 本文提出基于深度学习的虾病自动分类方法，使用六种预训练模型在1,149张图像数据集上进行评估，ConvNeXt-Tiny模型取得96.88%的最高准确率。


<details>
  <summary>Details</summary>
Motivation: 虾养殖是全球重要的经济产业，但疾病爆发严重威胁其可持续发展。传统方法难以实现及时准确的疾病检测，需要自动化分类方法来应对这一挑战。

Method: 使用包含1,149张图像的四类疾病数据集，部署ResNet50、EfficientNet、DenseNet201、MobileNet、ConvNeXt-Tiny和Xception六种预训练模型。采用背景去除和标准化预处理，使用FGSM进行对抗训练增强鲁棒性，应用CutMix和MixUp数据增强防止过拟合，并使用Grad-CAM、Grad-CAM++和XGrad-CAM进行模型可解释性分析。

Result: ConvNeXt-Tiny模型表现最佳，在测试集上达到96.88%的准确率。经过1000次迭代后，模型99%置信区间为[0.953,0.971]，显示出良好的分类性能。

Conclusion: 深度学习模型能够有效实现虾病的自动分类，ConvNeXt-Tiny模型在准确性和鲁棒性方面表现最优，为虾养殖业的疾病监测提供了可行的技术解决方案。
...
### [177] [Intrinsic-Metric Physics-Informed Neural Networks (IM-PINN) for Reaction-Diffusion Dynamics on Complex Riemannian Manifolds](https://arxiv.org/abs/2601.00834)
...
<details>
  <summary>Details</summary>
Motivation: 在复杂非欧几里得流形上模拟非线性反应扩散动力学面临两大挑战：高保真网格生成成本高昂，以及离散时间步进方案中的辛漂移问题。传统自适应细化方法在处理极端高斯曲率波动时无法解析各向异性图灵不稳定性。

Method: 提出IM-PINN框架：1）将黎曼度量张量嵌入自动微分图中，解析重建拉普拉斯-贝尔特拉米算子；2）采用双流架构和傅里叶特征嵌入来缓解谱偏差；3）在连续参数域中直接求解偏微分方程，解耦解复杂度与几何离散化。

Result: 在具有极端高斯曲率波动（K∈[-2489,3580]）的"随机布料"流形上验证，成功恢复了Gray-Scott模型的"分裂斑点"和"迷宫"模式。相比表面有限元法，IM-PINN实现了更好的物理严谨性：全局质量守恒误差为0.157 vs SFEM的0.258，消除了半隐式积分固有的质量漂移。

Conclusion: IM-PINN为在演化表面上模拟生物模式形成提供了内存高效、分辨率无关的范式，弥合了微分几何和物理信息机器学习之间的鸿沟，可作为热力学一致的全局求解器。
...
### [178] [SLO-Conditioned Action Routing for Retrieval-Augmented Generation: Objective Ablation and Failure Modes](https://arxiv.org/abs/2601.00841)
*Bharath Nunepalli*

Main category: cs.LG

TL;DR: 该论文研究了RAG系统中的查询级控制问题，通过选择检索深度和生成模式来满足成本、拒绝率和幻觉风险等SLO目标，发现简单基线策略表现良好，学习策略主要在质量导向SLO下提供额外成本节省。


<details>
  <summary>Details</summary>
Motivation: RAG系统面临实际控制问题：需要为每个查询选择检索深度和生成行为以满足服务级别目标（SLOs），如成本、拒绝率和幻觉风险。目前缺乏对这类控制问题的系统研究。

Method: 将每个查询的控制建模为离散动作：选择检索深度和生成模式（保护式vs自动），或拒绝。使用SQuAD 2.0构建离线日志数据集，执行每个动作并记录准确性、令牌成本、幻觉/拒绝指标和SLO加权奖励。评估两种简单策略学习目标：基于每个状态最佳动作的监督分类（Argmax-CE）及其奖励加权变体（Argmax-CE-WT）。

Result: 在评估的设置中，强固定基线（低k，保护式提示）表现具有竞争力；学习策略主要在质量导向SLO下提供额外成本节省，在廉价SLO下当拒绝被高度奖励时可能出现拒绝崩溃现象。

Conclusion: 本文提供了RAG管道SLO感知控制的可重复案例研究，强调失败模式和报告规范，而非提出新的检索器或语言模型。研究表明简单基线策略通常足够有效，学习策略在某些特定场景下提供额外优化。
...
### [179] [Value-guided action planning with JEPA world models](https://arxiv.org/abs/2601.00844)
...
<details>
  <summary>Details</summary>
Motivation: JEPA架构能通过自监督预测目标学习环境动态表示，但其支持有效行动规划的能力有限。需要增强JEPA世界模型的规划能力，使其能更好地支持动作规划。

Method: 通过塑造JEPA表示空间，使特定环境中到达成本的负目标条件值函数近似于状态嵌入间的距离（或准距离）。提出一种在训练中强制执行此约束的实用方法。
...
### [180] [You Only Need Your Transformer 25% of the Time: Meaning-First Execution for Eliminating Unnecessary Inference](https://arxiv.org/abs/2601.00847)
*Ryan Shamim*

Main category: cs.LG

TL;DR: MFEE框架将推理重构为控制平面决策问题，通过语义分析选择性执行transformer，在保持100%准确率的同时减少78.1%的计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有AI推理系统将transformer执行视为必需，混淆了模型能力与执行必要性。作者认为许多情况下可以通过替代路径保持正确性，无需执行完整推理。

Method: 提出Meaning-First Execution (MFEE)控制平面架构，作为现有堆栈之上的门控层，通过语义分析决定何时需要执行transformer推理，无需修改模型权重或参数。

Result: 在1000个多样化提示的确定性解码测试中，MFEE实现78.1%的执行减少，同时保持100%的精确匹配等价性。相比基于模式的路由器最多只能达到53.3%的避免率且有正确性失败，MFEE通过语义分析实现100%避免且零失败。
...
### [181] [EdgeJury: Cross-Reviewed Small-Model Ensembles for Truthful Question Answering on Serverless Edge Inference](https://arxiv.org/abs/2601.00850)
*Aayush Kumar*

Main category: cs.LG

TL;DR: EdgeJury是一个轻量级集成框架，使用小型指令调优语言模型（3B-8B）通过四阶段协调流程提高问答的真实性和鲁棒性，在边缘计算环境中显著减少幻觉错误。


<details>
  <summary>Details</summary>
Motivation: 在资源受限的边缘部署中，前沿规模模型或检索管道可能不实用，而幻觉问题会严重影响问答的可靠性。需要一种轻量级解决方案来提高小型模型在边缘环境中的真实性和鲁棒性。

Method: EdgeJury采用四阶段框架：1）并行角色专业化生成；2）匿名交叉评审，包含结构化批评和排名；3）主席合成，整合最强内容并解决标记问题；4）基于模型间一致性的声明级一致性标注。

Result: 在TruthfulQA（MC1）上达到76.2%准确率，相比单个8B基线（62.8%）提升21.4%；在200个问题的对抗性EdgeCases集上获得48.2%相对增益；人工分析显示事实性幻觉错误减少约55%；在Cloudflare Workers AI上实现8.4秒中位端到端延迟。

Conclusion: 协调的小型模型集成可以在不依赖外部检索或专有大型模型API的情况下，显著提高在误解密集的问答基准上的真实性，适合边缘部署场景。
...
### [182] [FedSCAM (Federated Sharpness-Aware Minimization with Clustered Aggregation and Modulation): Scam-resistant SAM for Robust Federated Optimization in Heterogeneous Environments](https://arxiv.org/abs/2601.00853)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习中客户端数据的统计异质性（特别是非IID标签分布）严重影响了模型的收敛和泛化能力。现有方法通常对所有客户端使用统一的扰动半径，忽略了客户端特定的异质性差异。

Method: 提出FedSCAM算法：1）为每个客户端计算异质性指标；2）根据异质性分数反向调整SAM扰动半径，防止高方差客户端破坏全局模型稳定性；3）引入异质性感知的加权聚合机制，优先考虑与全局优化方向一致的客户端更新。

Result: 在CIFAR-10和Fashion-MNIST数据集上，使用基于狄利克雷分布的不同程度标签偏斜进行实验，FedSCAM在收敛速度和最终测试准确率方面均优于FedSAM、FedLESAM等基线方法。

Conclusion: FedSCAM通过动态调整扰动半径和异质性感知聚合，有效解决了联邦学习中的统计异质性问题，提升了模型在非IID数据下的性能和稳定性。
...
### [183] [Harvesting AlphaEarth: Benchmarking the Geospatial Foundation Model for Agricultural Downstream Tasks](https://arxiv.org/abs/2601.00857)
...
<details>
  <summary>Details</summary>
Motivation: 地理空间基础模型（GFMs）如AlphaEarth Foundation（AEF）在EO任务中表现出色，但主要应用于土地覆盖分类。农业监测需要对这些模型在关键农业下游任务中进行深入评估，并缺乏与传统遥感模型的全面比较。

Method: 在美国的三个农业下游任务中评估AEF嵌入：作物产量预测、耕作制图和覆盖作物制图。使用公共和私人来源的数据集，在不同尺度和位置进行综合评估，并训练基于遥感的模型作为对比模型。

Result: AEF模型在所有任务中表现良好，在产量预测和县级耕作制图方面与专门构建的遥感模型具有竞争力。但发现AEF嵌入存在空间可迁移性有限、可解释性低和时间敏感性有限等限制。

Conclusion: AEF嵌入在农业应用中需要谨慎使用，特别是在时间敏感性、泛化能力和可解释性很重要的场景中。虽然在某些任务中表现良好，但存在显著限制。
...
### [184] [Path Integral Solution for Dissipative Generative Dynamics](https://arxiv.org/abs/2601.00860)
*Xidi Wang*

Main category: cs.LG

TL;DR: 该论文证明纯机械系统通过耗散量子动力学和非局域上下文聚合可以生成智能语言，而守恒定律会导致根本性失败。语言生成被建立为耗散量子场论，智能通过耗散和非局域性获得，而非守恒。


<details>
  <summary>Details</summary>
Motivation: 探索纯机械系统是否能生成智能语言，研究量子动力学中耗散与守恒对语言生成能力的影响，理解智能涌现的物理机制。

Method: 使用具有封闭形式路径积分传播子的Koopman算子分析耗散量子动力学，通过谱分析揭示特征值结构（衰减模式、增长模式、中性模式），研究哈密顿约束对耗散模式的影响。

Result: 耗散量子动力学能产生连贯的文本生成，而守恒定律导致根本性失败。谱分析显示特征值结构分为衰减模式（遗忘）、增长模式（放大）和中性模式（保持），这些是定向信息流的基本要素。

Conclusion: 语言生成是耗散量子场论，机械系统通过耗散和非局域性的组合获得智能，而非通过守恒。不可逆计算需要受控的信息耗散和因果上下文聚合。
...
### [185] [Universal Battery Degradation Forecasting Driven by Foundation Model Across Diverse Chemistries and Conditions](https://arxiv.org/abs/2601.00862)
*Joey Chan,Huan Wang,Haoyu Pan,Wei Wu,Zirong Wang,Zhen Chen,Ershun Pan,Min Xie,Lifeng Xi*

Main category: cs.LG

TL;DR: 提出一个统一的电池容量衰减预测框架，利用时间序列基础模型和参数高效微调，在多样化化学体系和工况下保持鲁棒性能。


<details>
  <summary>Details</summary>
Motivation: 电池容量衰减预测对储能系统的安全、可靠和长期效率至关重要，但由于不同电池化学体系、外形尺寸和运行条件的强异质性，很难构建一个能超越训练域泛化的单一模型。

Method: 收集20个公开老化数据集构建大规模语料库（1,704个电池，3,961,195个充放电循环段），采用时间序列基础模型（TSFM）作为骨干，结合参数高效的LoRA微调和物理引导的对比表示学习来捕捉共享的衰减模式。

Result: 在已见和刻意保留的未见数据集上，单一统一模型相比强大的每个数据集基线模型，实现了竞争性或更优的准确性，同时在训练中排除的化学体系、容量规模和运行条件下保持稳定性能。

Conclusion: 基于TSFM的架构展示了作为真实电池管理系统中容量衰减预测的可扩展和可转移解决方案的潜力。
...
### [186] [Selective Imperfection as a Generative Framework for Analysis, Creativity and Discovery](https://arxiv.org/abs/2601.00863)
*Markus J. Buehler*

Main category: cs.LG

TL;DR: 提出materiomusic生成框架，将物质层次结构与音乐创作逻辑连接，通过可逆映射将分子振动、蜘蛛网等物理结构转化为音乐元素，揭示科学与艺术在约束下的创造性共性。


<details>
  <summary>Details</summary>
Motivation: 探索物质结构与音乐创作之间的深层联系，建立跨尺度的统一框架，将物理振动原理转化为可感知的音乐形式，实现科学与艺术的融合。

Method: 使用可逆映射方法：1) 分子光谱映射到音调；2) 三维网络结构映射到可演奏乐器；3) 枚举所有2^12音乐音阶分析文化系统分布；4) 基于群体智能的AI模型作曲。

Result: 发现文化重要的音乐系统聚集在中熵、中缺陷区域，与材料科学中的Hall-Petch最优缺陷密度直接对应；AI生成的音乐展现出类人的小世界连接性、模块化整合和长程相干性等结构特征。

Conclusion: 科学与艺术都是在约束下进行世界构建的生成性行为，振动作为跨尺度的共享语法组织结构，选择性不完美是维持相干性与适应性平衡的机制。
...
### [187] [Distribution Matching for Graph Quantification Under Structural Covariate Shift](https://arxiv.org/abs/2601.00864)
*Clemens Damke,Eyke Hüllermeier*

Main category: cs.LG

TL;DR: 将KDEy量化学习方法扩展到图数据，通过结构重要性采样处理训练和测试数据之间的结构偏移，优于标准量化方法。


<details>
  <summary>Details</summary>
Motivation: 在社交网络等图数据中，我们通常更关心标签的整体分布（如政治观点的普遍性）而非单个实例的标签。现有量化学习方法基于先验概率偏移假设，但在图数据中，当训练和测试数据来自图的不同区域时，这种假设通常不成立，需要处理结构偏移问题。

Method: 将结构重要性采样的思想扩展到最先进的KDEy量化学习方法中。通过重要性采样来适应训练和测试数据之间的结构偏移，特别是当训练数据来自图的不同区域时。

Result: 提出的方法能够适应结构偏移，并且在性能上优于标准的量化学习方法。

Conclusion: 通过将结构重要性采样扩展到KDEy量化学习方法，成功解决了图数据中训练和测试数据之间的结构偏移问题，为图数据的量化学习提供了更有效的解决方案。
...
### [188] [A-PINN: Auxiliary Physics-informed Neural Networks for Structural Vibration Analysis in Continuous Euler-Bernoulli Beam](https://arxiv.org/abs/2601.00866)
*Shivani Saini,Ramesh Kumar Vats,Arup Kumar Sahoo*

Main category: cs.LG

TL;DR: 提出改进的辅助物理信息神经网络(A-PINN)框架，结合平衡自适应优化器，用于结构振动分析，在数值稳定性和预测精度上相比基线提升至少40%


<details>
  <summary>Details</summary>
Motivation: 物理信息神经网络(PINNs)及其变体在求解微分方程控制的正反问题方面表现出色，但需要改进以更准确地表示结构系统、捕捉振动现象并确保可靠的预测分析，这对于深入理解科学机器学习模型在振动问题中的鲁棒性至关重要

Method: 提出改进的辅助物理信息神经网络(A-PINN)框架，结合平衡自适应优化器，用于分析结构振动问题。通过不同数值模拟来近似求解各种场景下的欧拉-伯努利梁方程，以严格评估A-PINN的性能

Result: 数值结果证实了该模型在数值稳定性和预测精度方面的增强性能，相比基线模型至少提升了40%的性能

Conclusion: 改进的A-PINN框架在结构振动问题分析中表现出优越性能，为科学机器学习模型在振动工程应用中的鲁棒性提供了深入见解
...
### [189] [SmartFlow Reinforcement Learning and Agentic AI for Bike-Sharing Optimisation](https://arxiv.org/abs/2601.00868)
...
TL;DR: SmartFlow是一个多层框架，结合强化学习和智能体AI，解决城市共享单车动态再平衡问题，通过战略DQN学习、战术优化和基于LLM的通信层，显著减少网络不平衡并提高运营效率。


<details>
  <summary>Details</summary>
Motivation: 解决城市共享单车系统中的动态再平衡问题，传统方法难以应对复杂的城市移动网络，需要一种可扩展、可解释的AI驱动解决方案，以降低闲置时间、提高单车可用性并减少运营成本。

Method: 采用多层架构：战略层使用深度Q网络（DQN）在纽约Citi Bike网络的高保真模拟中学习再平衡策略；战术层进行确定性优化，安排多段行程和即时调度以最小化车队行驶距离；通信层基于大型语言模型（LLM）的智能体AI将物流计划转化为可操作指令。

Result: 在多个种子运行中评估显示，SmartFlow能减少超过95%的网络不平衡，同时需要最小的行驶距离，实现强大的卡车利用率，有效减少闲置时间、提高单车可用性并降低运营成本。

Conclusion: SmartFlow为复杂城市移动网络提供了一个可扩展、可解释的AI驱动物流蓝图，成功将机器智能与人类运营相结合，为城市共享单车再平衡问题提供了高效解决方案。
...
### [190] [Quantum Machine Learning Approaches for Coordinated Stealth Attack Detection in Distributed Generation Systems](https://arxiv.org/abs/2601.00873)
*Osasumwen Cedric Ogiesoba-Eguakun,Suman Rath*

Main category: cs.LG

TL;DR: 量子机器学习用于检测微电网中的协同隐蔽攻击，混合量子经典模型在低维数据集上表现最佳，比纯经典SVM基线略有提升


<details>
  <summary>Details</summary>
Motivation: 协同隐蔽攻击是分布式发电系统的严重网络安全威胁，它们修改控制和测量信号但接近正常行为，难以用标准入侵检测方法发现。需要研究更有效的检测方法。

Method: 使用高质量模拟测量数据创建平衡二元分类数据集，包含三个特征：DG1的无功功率、相对于标称值的频率偏差和端电压幅值。评估了经典机器学习基线、完全量子变分分类器和混合量子经典模型。

Result: 混合量子经典模型（量子特征嵌入+经典RBF支持向量机）在低维数据集上获得最佳整体性能，准确率和F1分数比强大的经典SVM基线略有提升。完全量子模型因训练不稳定和当前NISQ硬件限制表现较差。

Conclusion: 混合模型训练更可靠，表明即使完全量子学习尚不实用，量子特征映射也能增强入侵检测能力。量子特征嵌入为网络安全检测提供了有前景的方向。
...
### [191] [LLMize: A Framework for Large Language Model-Based Numerical Optimization](https://arxiv.org/abs/2601.00874)
*M. Rizki Oktavian*

Main category: cs.LG

TL;DR: LLMize是一个开源Python框架，通过迭代提示和上下文学习实现LLM驱动的优化，将优化问题转化为自然语言的黑盒过程，支持多种优化策略，特别适用于复杂领域特定任务。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型展现出超越传统语言任务的推理能力，这激发了将其用于数值优化的想法。传统优化方法需要数学编程或元启发式设计专业知识，而LLMize旨在提供更实用、可访问的方法，特别是对于约束和启发式难以形式化的复杂领域特定任务。

Method: LLMize将优化问题转化为黑盒过程：候选解以自然语言生成，通过外部目标函数评估，并使用解-分数反馈在连续迭代中精炼。框架支持多种优化策略，包括基于提示的优化（OPRO）以及受进化算法和模拟退火启发的混合LLM方法。关键优势是能够通过自然语言描述直接注入约束、规则和领域知识。

Result: 在凸优化、线性规划、旅行商问题、神经网络超参数调优和核燃料晶格优化等任务上评估LLMize。结果显示，对于简单问题，基于LLM的优化不如经典求解器有竞争力，但对于复杂、领域特定的任务，它提供了一种实用且可访问的方法，特别是当约束和启发式难以形式化时。

Conclusion: LLMize为复杂优化问题提供了一个实用且可访问的框架，特别适用于领域特定任务，其中约束和启发式难以形式化。虽然对于简单问题不如经典求解器，但它通过自然语言注入领域知识的能力使其在特定应用场景中具有价值。
...
### [192] [LearnAD: Learning Interpretable Rules for Brain Networks in Alzheimer's Disease Classification](https://arxiv.org/abs/2601.00877)
*Thomas Andrews,Mark Law,Sara Ahmadi-Abhari,Alessandra Russo*

Main category: cs.LG

TL;DR: LearnAD是一种神经符号方法，通过可解释规则从脑MRI数据预测阿尔茨海默病，结合统计模型和图神经网络识别相关脑连接，使用FastLAS学习全局规则，在保持完全可解释性的同时达到与SVM相当的准确率。


<details>
  <summary>Details</summary>
Motivation: 当前阿尔茨海默病预测方法缺乏可解释性，临床神经科学需要既能准确预测又能提供可理解规则的方法，以深入了解疾病机制和GNN在临床神经科学中的行为。

Method: 神经符号方法：1) 使用统计模型、决策树、随机森林或GNN识别相关脑连接；2) 应用FastLAS学习全局可解释规则；3) 结合神经网络的表示能力和符号学习的规则提取能力。

Result: 最佳实例优于决策树，与支持向量机准确率相当，仅略低于使用所有特征的随机森林和GNN，同时保持完全可解释性。消融研究表明神经符号方法在可比性能下显著提升可解释性。

Conclusion: LearnAD展示了符号学习如何加深对GNN在临床神经科学中行为的理解，为阿尔茨海默病预测提供了既准确又可解释的解决方案，平衡了预测性能和临床可解释性的需求。
...
### [193] [Outlier Detection Using Vector Cosine Similarity by Adding a Dimension](https://arxiv.org/abs/2601.00883)
*Zhongyang Shen*
...
<details>
  <summary>Details</summary>
Motivation: 需要一种有效的多维数据异常检测方法，传统方法可能在高维空间中效果有限，需要更鲁棒的异常识别技术

Method: 1. 为原始数据添加一个零值维度构建新数据集；2. 选择测量点并创建观测点（原点），观测点在新维度有非零值；3. 构建从观测点到测量点及其他数据点的向量；4. 比较这些向量的余弦相似度来识别异常数据

Result: 开发了优化实现MDOD，已在PyPI上发布，提供可用的异常检测工具包

Conclusion: 该方法通过向量余弦相似度比较有效识别多维数据中的异常点，MDOD实现提供了实用的异常检测解决方案
...
### [194] [FANoS: Friction-Adaptive Nosé--Hoover Symplectic Momentum for Stiff Objectives](https://arxiv.org/abs/2601.00889)
*Nalin Dhiman*

Main category: cs.LG

TL;DR: FANoS是一种受物理启发的优化器，结合了二阶动力系统、Nosé-Hoover热浴变量和半隐式积分器，在特定非凸问题上表现良好，但总体上不如现代基线方法。


<details>
  <summary>Details</summary>
Motivation: 将分子动力学中的结构保持积分和热浴思想应用于优化算法，开发一种物理启发的优化器，旨在处理某些非凸优化问题。

Method: 结合：(1) 离散化二阶动力系统的动量更新，(2) 使用动能反馈调整标量摩擦系数的Nosé-Hoover热浴变量，(3) 半隐式（辛欧拉）积分器，可选配对角RMS预处理器。

Result: 在Rosenbrock-100D基准测试中，FANoS-RMS达到1.74×10⁻²，优于未裁剪的AdamW和SGD+momentum，但不如梯度裁剪的AdamW和L-BFGS。在病态凸二次问题和小型PINN测试中表现不稳定。

Conclusion: FANoS是现有思想的可解释性综合，在某些非凸问题上有效，但不是现代基线方法的通用替代品，其行为对温度调度和超参数选择敏感。
...
### [195] [Hierarchical topological clustering](https://arxiv.org/abs/2601.00892)
*Ana Carpio,Gema Duro*

Main category: cs.LG

TL;DR: 提出一种分层拓扑聚类算法，可处理任意形状的聚类和异常值，适用于各种数据类型
...
Method: 提出分层拓扑聚类算法，可使用任意距离度量，通过层次结构推断异常值和任意形状聚类的持久性

Result: 在图像、医疗和经济数据等包含相关异常值的数据集上展示了算法的潜力，能够在其他技术失败的情况下提供有意义的聚类

Conclusion: 该分层拓扑聚类算法具有通用性，能够有效处理复杂数据结构和异常值，为数据分析提供了新的拓扑方法工具
...
### [196] [When to Ponder: Adaptive Compute Allocation for Code Generation via Test-Time Training](https://arxiv.org/abs/2601.00894)
*Gihyeon Sim*

Main category: cs.LG

TL;DR: PonderTTT：一种基于自监督重建损失的门控策略，用于选择性触发测试时训练更新，无需训练分类器，仅需单个标量阈值


<details>
  <summary>Details</summary>
Motivation: 大型语言模型对所有输入应用统一计算，无论难度如何。需要一种方法在推理时智能决定何时进行测试时训练更新，以平衡计算效率和性能

Method: 提出PonderTTT门控策略，利用TTT层的自监督重建损失作为门控信号。门控决策无需训练，仅需在未标记数据上初始校准单个标量阈值，并通过指数移动平均持续调整以维持目标更新率

Result: 在GPT-2模型（124M到1.5B）的代码语言建模任务上，该方法实现了82-89%的Oracle恢复率，完全无需训练，显著优于随机跳过基线（在OOD语言上损失降低达16%）

Conclusion: 自监督重建损失可作为有效的推理兼容门控信号，实现训练自由的选择性测试时训练更新，在保持性能的同时减少不必要的计算
...
### [197] [Dichotomous Diffusion Policy Optimization](https://arxiv.org/abs/2601.00898)
...
TL;DR: DIPOLE：一种用于稳定可控扩散策略优化的新型强化学习算法，通过二分策略分解实现灵活贪婪度控制


<details>
  <summary>Details</summary>
Motivation: 现有扩散策略训练方法存在不稳定训练或计算效率低的问题，需要一种既能稳定训练又能灵活控制生成过程的扩散策略优化方法

Method: 提出DIPOLE算法，重新审视KL正则化目标，设计贪婪化策略正则化方案，将最优策略分解为一对稳定学习的二分策略（奖励最大化策略和奖励最小化策略），在推理时通过线性组合分数实现贪婪度控制

Result: 在ExORL和OGBench的离线和离线到在线RL设置中验证了有效性，并成功训练了用于端到端自动驾驶的大型视觉语言动作模型，在NAVSIM大规模真实世界AD基准上表现出色

Conclusion: DIPOLE为扩散策略提供了稳定可控的优化框架，在复杂决策任务中表现出优越性能，具有实际应用潜力
...
### [198] [Conformal Prediction Under Distribution Shift: A COVID-19 Natural Experiment](https://arxiv.org/abs/2601.00908)
*Chorok Lee*

Main category: cs.LG

TL;DR: 研究显示，在COVID-19引发的分布偏移下，保形预测的覆盖率下降程度差异巨大（0%-86.7%），单特征依赖是灾难性失败的关键因素，季度重训练可部分恢复覆盖率，但仅对脆弱任务有效。


<details>
  <summary>Details</summary>
Motivation: 保形预测在分布偏移下的性能保证会下降，但不同任务间的下降程度差异很大。研究旨在理解这种差异的原因，并找到有效的应对策略。

Method: 使用COVID-19作为自然实验，分析8个供应链任务。通过SHAP分析特征重要性分布，计算特征集中度指标。对灾难性失败任务进行季度重训练，并与稳健任务对比。额外分析4个具有中等特征稳定性的任务。

Result: 覆盖率下降程度差异巨大（0%-86.7%），灾难性失败与单特征依赖高度相关（rho=0.714）。灾难性任务的特征重要性集中在单一特征（增加4.5倍），而稳健任务则分布在多个特征（10-20倍）。季度重训练可将灾难性任务的覆盖率从22%提升到41%，但对稳健任务无益（覆盖率99.8%）。

Conclusion: 提出决策框架：部署前监控SHAP集中度；如果集中度>40%（脆弱任务）则进行季度重训练；如果任务稳健则可跳过重训练。特征稳定性而非集中度决定稳健性，集中度效应仅适用于严重偏移情况。
...
### [199] [Latent-Constrained Conditional VAEs for Augmenting Large-Scale Climate Ensembles](https://arxiv.org/abs/2601.00915)
*Jacquelyn Shelton,Przemyslaw Polewski,Alexander Robel,Matthew Hoffman,Stephen Price*

Main category: cs.LG

TL;DR: 提出LC-CVAE方法，通过强制潜在空间在共享锚点位置的一致性，解决多气候模型集成生成中的潜在空间碎片化问题，实现从有限模型运行生成新时空气候变量


<details>
  <summary>Details</summary>
Motivation: 大型气候模型集成计算成本高，但许多下游分析需要更多统计一致的时空气候变量实现。现有方法在从有限模型运行生成新实现时，存在潜在空间碎片化问题，无法泛化到未见过的集成成员。

Method: 提出潜在约束条件变分自编码器(LC-CVAE)，强制潜在嵌入在共享地理"锚点"位置具有跨实现同质性。然后使用多输出高斯过程回归在潜在空间预测新实现中未采样位置的潜在坐标，最后解码生成完整时间序列场。

Result: 实验表明：1) 在单个实现上训练不稳定；2) 加入约5个实现后收益递减；3) 空间覆盖范围与重建质量之间存在权衡，这与潜在空间中的平均邻居距离密切相关。

Conclusion: LC-CVAE方法有效解决了多气候模型集成生成中的潜在空间碎片化问题，能够在有限模型运行基础上生成统计一致的新气候变量实现，为下游分析提供更多数据支持。
...
### [200] [Attention Needs to Focus: A Unified Perspective on Attention Allocation](https://arxiv.org/abs/2601.00919)
*Zichuan Fu,Wentao Song,Guojing Li,Yejing Wang,Xian Wu,Yimin Deng,Hanyu Yan,Yefeng Zheng,Xiangyu Zhao*

Main category: cs.LG

TL;DR: 本文提出Lazy Attention机制，通过统一视角解决注意力机制中的过载和欠载问题，实现更聚焦的注意力分布，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: Transformer架构中的标准注意力机制存在表示崩溃和注意力下沉两个已知问题。先前的研究往往孤立地解决这些问题，忽略了它们之间的深层联系。本文认为这两个问题都源于一个共同的根源——不恰当的注意力分配。

Method: 提出Lazy Attention机制：1) 针对注意力过载，采用跨头和维度的位置区分来增强token区分度；2) 针对注意力欠载，引入Elastic-Softmax归一化函数，放松标准softmax约束以抑制对无关token的关注。

Result: 在FineWeb-Edu语料库上进行实验，在九个多样化基准测试中评估。Lazy Attention成功缓解了注意力下沉问题，与标准注意力和现代架构相比具有竞争力，同时达到高达59.58%的注意力稀疏度。

Conclusion: 通过统一视角将注意力机制的两个主要问题归因于不恰当的注意力分配，提出的Lazy Attention机制有效解决了过载和欠载问题，实现了更聚焦的注意力分布和良好的性能表现。
...
### [201] [MODE: Efficient Time Series Prediction with Mamba Enhanced by Low-Rank Neural ODEs](https://arxiv.org/abs/2601.00920)
*Xingsheng Chen,Regina Zhang,Bo Gao,Xingwei He,Xiaofeng Liu,Pietro Lio,Kwok-Yan Lam,Siu-Ming Yiu*

Main category: cs.LG

TL;DR: MODE：一个结合低秩神经ODE与增强Mamba架构的统一框架，用于高效、可扩展的长时序预测


<details>
  <summary>Details</summary>
Motivation: 现有时序预测方法在效率、可扩展性和准确性之间难以平衡，特别是在处理长程依赖和不规则采样数据时存在挑战

Method: 提出MODE框架：1）线性标记化层处理输入序列；2）多个Mamba编码器块，每个包含增强Mamba层（因果卷积、SiLU激活、低秩神经ODE增强）；3）分段选择性扫描机制，基于伪ODE动态自适应聚焦重要子序列

Result: 在基准数据集上的实验表明，MODE在预测准确性和计算效率方面均超越现有基线方法

Conclusion: MODE通过低秩神经ODE与Mamba选择性扫描的集成，实现了高效、可扩展的长时序建模，显著提升了效率和可扩展性
...
### [202] [Practical Geometric and Quantum Kernel Methods for Predicting Skeletal Muscle Outcomes in chronic obstructive pulmonary disease](https://arxiv.org/abs/2601.00921)
*Azadeh Alavi,Hamidreza Khalili,Stanley H. Chan,Fatemeh Kouchmeshki,Ross Vlahos*

Main category: cs.LG

TL;DR: 该研究使用量子核回归和几何感知方法，在慢性阻塞性肺疾病小鼠模型中，基于血液和支气管肺泡灌洗液生物标志物预测肌肉功能指标，在低数据、低特征场景下取得优于传统方法的性能。


<details>
  <summary>Details</summary>
Motivation: 慢性阻塞性肺疾病常伴随骨骼肌功能障碍，与全身和气道炎症相关。研究旨在利用微创生物标志物纵向预测肌肉结局，解决临床前研究中样本量小、特征维度低的问题。

Method: 使用213只动物（Sham组与香烟烟雾暴露组）的血液和支气管肺泡灌洗液数据，预测胫骨前肌重量、比力和肌肉质量指数。比较了经典基线模型、几何感知的对称正定描述符（使用Stein散度）以及为低维表格数据设计的量子核模型。

Result: 在肌肉重量预测中，量子核岭回归使用四个可解释特征（血液C反应蛋白、中性粒细胞计数、支气管肺泡灌洗液细胞数和条件）获得测试RMSE 4.41 mg和R² 0.605，优于相同特征集的岭回归基线（4.70 mg和0.553）。几何感知方法在仅使用生物标志物时也有稳定提升（4.55 mg vs 4.79 mg）。筛查式评估中，ROC-AUC最高达0.90。

Conclusion: 几何和量子核提升方法在低数据、低特征的生物医学预测问题中能提供可测量的性能优势，同时保持可解释性和透明的模型选择，为临床前研究提供了有前景的建模工具。
...
### [203] [Complexity-based code embeddings](https://arxiv.org/abs/2601.00924)
*Rares Folea,Radu Iacob,Emil Slusanschi,Traian Rebedea*

Main category: cs.LG

TL;DR: 提出一种将算法源代码转换为数值嵌入的通用方法，通过动态分析程序对不同输入的行为，并针对分析指标定制多个通用复杂度函数，基于r-Complexity构建代码嵌入，在Codeforces真实代码片段数据集上实现XGBoost算法并获得良好性能


<details>
  <summary>Details</summary>
Motivation: 需要一种通用的方法来将算法源代码转换为数值表示（嵌入），以便进行机器学习分析，特别是针对编程竞赛中的代码片段

Method: 通过动态分析程序对不同输入的行为，针对分析指标定制多个通用复杂度函数，基于r-Complexity构建代码嵌入，然后使用XGBoost算法进行分类

Result: 在包含11个类别的多标签数据集上，使用Codeforces平台真实编程竞赛代码片段构建的数据集上，实现了平均F1-score的良好性能

Conclusion: 提出的代码嵌入方法能够有效将算法源代码转换为数值表示，为代码分析和机器学习任务提供了有效的特征表示方法
...
### [204] [Enhanced Data-Driven Product Development via Gradient Based Optimization and Conformalized Monte Carlo Dropout Uncertainty Estimation](https://arxiv.org/abs/2601.00932)
*Andrea Thomas Nava,Lijo Johny,Fabio Azzalini,Johannes Schneider,Arianna Casanova*

Main category: cs.LG

TL;DR: 提出一个数据驱动产品开发框架，使用联合神经网络优化多相关属性，并结合Conformalised Monte Carlo Dropout提供不确定性估计和覆盖保证。


<details>
  <summary>Details</summary>
Motivation: 数据驱动产品开发需要同时优化多个相关属性，现有方法缺乏可靠的不确定性估计和覆盖保证，且调整覆盖水平时需要重新训练模型。

Method: 1) 使用联合神经网络捕捉目标间的相互依赖关系；2) 应用投影梯度下降寻找最优设计特征；3) 提出Conformalised Monte Carlo Dropout方法，结合嵌套保形预测和蒙特卡洛dropout，提供模型无关的有限样本覆盖保证。

Result: 在五个真实世界数据集上的实验表明，该方法达到最先进性能，提供自适应非均匀预测区间，且调整覆盖水平时无需重新训练模型。

Conclusion: 该框架为数据驱动产品开发提供了有效的多属性优化方案，通过新颖的不确定性估计方法实现了可靠的覆盖保证和灵活性。
...
### [205] [LOFA: Online Influence Maximization under Full-Bandit Feedback using Lazy Forward Selection](https://arxiv.org/abs/2601.00933)
*Jinyu Xu,Abhishek K. Umrawal*

Main category: cs.LG

TL;DR: 提出LOFA算法解决在线影响力最大化问题，在完全强盗反馈下利用子模性降低遗憾


<details>
  <summary>Details</summary>
Motivation: 在线影响力最大化问题中，现有算法虽然利用子模性取得了低遗憾，但仍有改进空间。作者希望进一步利用子模性设计更高效的算法来降低经验遗憾。

Method: 提出Lazy Online Forward Algorithm (LOFA)，在完全强盗反馈模型下，利用影响力函数的子模性特性，通过懒惰更新策略来更高效地选择种子集。

Result: 在真实社交网络上的实验表明，LOFA在累积遗憾和瞬时奖励方面都优于现有的强盗算法，实现了更低的经验遗憾。

Conclusion: 通过进一步利用影响力函数的子模性，LOFA算法在在线影响力最大化问题上取得了更好的性能，为在线决策问题提供了有效的解决方案。
...
### [206] [Reliability Under Randomness: An Empirical Analysis of Sparse and Dense Language Models Across Decoding Temperatures](https://arxiv.org/abs/2601.00942)
*Kabir Grover*

Main category: cs.LG

TL;DR: 稀疏MoE架构在随机解码下的可靠性研究表明，指令微调而非架构稀疏性是决定模型在确定性任务中对抗解码随机性鲁棒性的主要因素。


<details>
  <summary>Details</summary>
Motivation: 随着稀疏MoE架构在大语言模型中的普及，需要研究其在随机解码下的可靠性。虽然条件计算带来了计算效率提升，但稀疏路由与基于温度的采样之间的交互是否会损害输出稳定性仍不明确。

Method: 评估三个代表性模型：OLMoE-7B（稀疏基础模型）、Mixtral-8x7B（稀疏指令微调模型）和Qwen2.5-3B（密集指令微调模型）。在具有客观可验证答案的确定性算术推理任务上进行测试，涵盖四种解码配置（从贪婪解码到T=1.0），共进行9,360次模型生成。

Result: 稀疏指令微调模型在所有解码温度下表现出与密集指令微调模型相当的稳定性，而稀疏基础模型随着温度升高出现系统性退化。这表明指令微调而非架构稀疏性是决定模型在确定性任务中对抗解码随机性鲁棒性的主要因素。

Conclusion: 在可靠性关键应用中部署稀疏语言模型时，指令微调比架构稀疏性更重要。稀疏架构可以在不牺牲输出稳定性的情况下安全采用，前提是经过适当的指令微调。
...
### [207] [Adapting Feature Attenuation to NLP](https://arxiv.org/abs/2601.00965)
*Tianshuo Yang,Ryan Rabinowitz,Terrance E. Boult,Jugal Kalita*

Main category: cs.LG

TL;DR: 将计算机视觉中的特征衰减假设移植到Transformer模型，评估在文本开放集识别中的效果，发现COSTARR框架可直接应用于NLP但相比MaxLogit或MSP无显著优势


<details>
  <summary>Details</summary>
Motivation: Transformer分类器（如BERT）在闭集任务中表现优异，但在面对未见类别时表现脆弱，这是部署NLP系统常见场景。研究旨在将计算机视觉中的开放集识别技术移植到文本领域

Method: 将计算机视觉中的COSTARR框架适配到两个语言模型（BERT base和GPT-2），训练它们标注176个arXiv学科领域。同时评估MSP、MaxLogit和温度缩放自由能分数，使用OOSA和AUOSCR指标

Result: COSTARR可直接扩展到NLP无需重新训练，但相比MaxLogit或MSP无统计学显著优势；自由能分数在高类别数量设置中落后于所有其他分数
...
### [208] [Explainability-Guided Defense: Attribution-Aware Model Refinement Against Adversarial Data Attacks](https://arxiv.org/abs/2601.00968)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在安全关键领域应用需要既鲁棒又透明的模型。研究发现LIME识别的虚假、不稳定特征与对抗脆弱性相关，这为连接可解释性与鲁棒性提供了机会。

Method: 提出基于归因引导的精炼框架：1) 使用特征掩码抑制虚假特征；2) 敏感度感知正则化；3) 对抗增强；4) 闭环精炼流程。该方法无需额外数据或架构，可与标准对抗训练无缝集成。

Result: 在CIFAR-10、CIFAR-10-C和CIFAR-100上实验显示，对抗鲁棒性和分布外泛化能力均有显著提升。

Conclusion: 建立了可解释性与鲁棒性的直接联系，将LIME从被动诊断工具转变为主动训练信号，为构建既透明又鲁棒的深度学习模型提供了新途径。
...
### [209] [Zero-shot Forecasting by Simulation Alone](https://arxiv.org/abs/2601.00970)
...
TL;DR: 提出SarSim0时间序列模拟器，基于SARIMA模型，通过三步法生成高质量模拟数据，实现高效零样本预测，在多个基准测试中超越传统统计方法和现有基线。


<details>
  <summary>Details</summary>
Motivation: 零样本时间序列预测面临数据有限、评估易泄露、隐私和许可限制等挑战，需要一种实用的模拟方法来生成高质量训练数据。

Method: 基于SARIMA模型的三步模拟方法：1)从特征多项式稳定区域采样良好轨迹；2)通过叠加方案组合多个路径形成多季节性序列；3)添加基于速率的重尾噪声模型捕捉突发性和间歇性。

Result: SarSim0比基于核的生成器快几个数量级，可生成约10亿个独特模拟序列；训练后的神经网络在M-Series和GiftEval基准测试中表现出强大的零样本泛化能力，超越统计预测器和现有基线，甚至在GiftEval上出现"学生超越老师"效应。

Conclusion: SarSim0为时间序列模拟提供了实用高效的解决方案，能够生成丰富多样的训练数据，显著提升零样本预测性能，为工业预测应用开辟了新途径。
...
### [210] [Contractive Diffusion Policies: Robust Action Diffusion via Contractive Score-Based Sampling with Differential Equations](https://arxiv.org/abs/2601.01003)
*Amin Abyaneh,Charlotte Morissette,Mohamad H. Danesh,Anas El Houssaini,David Meger,Gregory Dudek,Hsiu-Chin Lin*

Main category: cs.LG

TL;DR: 提出收缩扩散策略(CDPs)，通过使扩散采样过程具有收缩性来增强鲁棒性，减少求解器和分数匹配误差，在数据稀缺时表现更佳


<details>
  <summary>Details</summary>
Motivation: 扩散策略在离线策略学习中表现出色，但其基于分数的SDE建模会引入求解器和分数匹配误差、需要大量数据、动作生成不一致等问题。这些在图像生成中不太关键的问题在连续控制中会累积导致失败

Method: 引入收缩扩散策略(CDPs)，在扩散采样动态中诱导收缩行为。收缩能将附近的流拉近，增强对求解器和分数匹配误差的鲁棒性，同时减少不需要的动作方差。提供了理论分析和实际实现方法，可最小修改地集成到现有扩散策略架构中

Result: 在仿真和真实世界环境中进行了广泛实验，CDPs在基准测试中通常优于基线策略，在数据稀缺条件下表现出更明显的优势

Conclusion: CDPs通过引入收缩性解决了扩散策略在连续控制中的关键问题，提高了鲁棒性和性能，特别是在数据有限的情况下，为扩散策略的实际应用提供了改进方案
...
### [211] [Data-Driven Assessment of Concrete Mixture Compositions on Chloride Transport via Standalone Machine Learning Algorithms](https://arxiv.org/abs/2601.01009)
*Mojtaba Aliasghar-Mamaghani,Mohammadreza Khalafi*

Main category: cs.LG

TL;DR: 使用多种机器学习算法预测混凝土混合物成分对氯离子侵蚀时间演化的影响，KRR、GPR和MLP表现最佳，揭示了成分与氯离子含量的相关性。


<details>
  <summary>Details</summary>
Motivation: 评估混凝土结构在侵蚀环境下的使用寿命，需要理解混凝土混合物成分如何影响氯离子侵蚀的时间演化过程，这对于基础设施耐久性至关重要。

Method: 采用数据驱动方法，使用简单和复杂的机器学习算法：线性回归、KNN回归、核岭回归、支持向量回归、高斯过程回归、前馈神经网络（MLP）和门控循环单元（GRU）。

Result: KRR、GPR和MLP表现出高精度，GRU在多样化混合物比例下表现不佳。大多数混合物成分与氯离子含量呈负相关，少数呈正相关。GPR模型揭示了清晰的潜在相关性趋势。

Conclusion: 机器学习方法可作为替代方法有效描述氯离子侵蚀的物理过程和相关关系，有助于提高基础设施的使用寿命，其中GPR、MLP、SVR和KRR提供了可接受的趋势估计。
...
### [212] [Geometric and Dynamic Scaling in Deep Transformers](https://arxiv.org/abs/2601.01014)
*Haoran Su,Chenyu You*

Main category: cs.LG

TL;DR: 论文提出Transformer深度增加导致表示崩溃的根本原因是几何问题，而非优化问题，并提出基于几何约束的MGT架构来解决


<details>
  <summary>Details</summary>
Motivation: 现有解释将Transformer深度增加导致的表示崩溃归因于优化不稳定或梯度消失，但这无法解释为什么在现代归一化和初始化方案下崩溃仍然存在。作者认为这是一个根本的几何问题

Method: 提出统一的几何框架：1）流形约束超连接，限制残差更新到有效的局部切向方向，防止不受控制的流形漂移；2）深度增量学习，引入数据依赖的非单调更新，能够反射和擦除冗余特征而非无条件累积

Result: 这些机制解耦了特征更新的方向和符号，实现了跨深度的稳定几何演化，形成了流形几何Transformer（MGT）架构

Conclusion: 分析预测，强制几何有效性同时允许动态擦除对于避免超深网络中的秩崩溃至关重要。Transformer超过100层的评估协议将测试几何而非深度本身是深度表示学习的关键限制因素这一假设
...
### [213] [Improving Variational Autoencoder using Random Fourier Transformation: An Aviation Safety Anomaly Detection Case-Study](https://arxiv.org/abs/2601.01016)
*Ata Akbari Asanjan,Milad Memarzadeh,Bryan Matthews,Nikunj Oza*

Main category: cs.LG

TL;DR: 该研究探索了随机傅里叶变换(RFT)在自编码器和变分自编码器训练与推理中的改进作用，通过频率原理分析发现RFT模型能同时学习低频和高频特征，而传统DNN则从低频开始逐步学习高频。


<details>
  <summary>Details</summary>
Motivation: 研究动机是改进深度神经网络（特别是自编码器和变分自编码器）的训练过程和推理性能，探索傅里叶变换在模型训练行为中的作用，特别是在基于重构的异常检测任务中的应用。

Method: 使用随机傅里叶变换(RFT)和可训练的傅里叶变换变体，通过频率原理分析模型训练行为。在低维合成数据集上进行数据表示实验，在高维航空安全数据集(Dashlink)上进行基于重构的异常检测实验。

Result: 结果表明，使用傅里叶变换的模型优于传统对应模型。RFT模型能同时学习低频和高频特征，而传统DNN从低频开始逐步学习高频。关于可训练傅里叶变换相对于随机变体的优势，结果尚无定论。

Conclusion: 傅里叶变换在深度神经网络训练中具有改进作用，特别是在基于重构的异常检测任务中。RFT改变了模型的学习模式，使其能同时处理不同频率特征，但可训练傅里叶变换相对于随机变体的优势需要进一步研究。
...
### [214] [Expanding the Chaos: Neural Operator for Stochastic (Partial) Differential Equations](https://arxiv.org/abs/2601.01021)
...
<details>
  <summary>Details</summary>
Motivation: 随机微分方程(SDE)和随机偏微分方程(SPDE)是建模自然科学和现代机器学习中随机动力学的基本工具。开发深度学习模型来近似其解算子不仅有望提供快速实用的求解器，还可能启发从新视角解决经典学习任务的模型。
...
Result: 在多样化问题上验证模型：经典SPDE基准、图像上的扩散一步采样、图上的拓扑插值、金融外推、参数估计、用于洪水预测的流形SDE，展示了竞争性精度和广泛适用性。

Conclusion: 基于WCE的神经算子为跨不同领域学习SDE/SPDE解算子提供了实用且可扩展的方法，表明该方法具有广泛的应用潜力。
...
### [215] [Wireless Dataset Similarity: Measuring Distances in Supervised and Unsupervised Machine Learning](https://arxiv.org/abs/2601.01023)
...
<details>
  <summary>Details</summary>
Motivation: 无线通信中需要评估不同数据集之间的相似性，以支持数据集选择/增强、仿真到真实场景转换、任务特定合成数据生成等应用，但目前缺乏系统的方法来量化数据集相似性并预测模型跨数据集性能

Method: 提出任务和模型感知框架，通过数据集距离度量预测跨数据集可迁移性：如果两个数据集距离小，则在一个数据集上训练的模型在另一个数据集上应表现良好。使用基于UMAP嵌入的度量，结合Wasserstein和欧几里得距离。对于监督任务，通过集成监督UMAP和数据集不平衡惩罚来推导标签感知距离

Result: 在无监督CSI压缩任务中，使用UMAP嵌入结合Wasserstein和欧几里得距离，数据集距离与训练-测试任务性能之间的Pearson相关系数超过0.85。在监督波束预测任务中，标签感知距离优于传统基线，与模型可迁移性始终表现出更强的相关性

Conclusion: 该框架能够有效度量无线数据集之间的相似性，支持任务相关的数据集比较，为数据集选择、增强和模型适应新部署提供决策依据
...
### [216] [Coarse-Grained Kullback--Leibler Control of Diffusion-Based Generative AI](https://arxiv.org/abs/2601.01045)
*Tatsuaki Tsuruyama*

Main category: cs.LG

TL;DR: 提出基于信息论Lyapunov函数V-δ的投影反向扩散方法，用于控制生成图像中的粗粒度统计量（如区块强度），在保持像素精度和视觉质量的同时，将区块质量误差控制在预设容差内。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型和基于分数的生成模型缺乏描述粗粒度统计量（如图像空间区块的强度或类别比例）在反向扩散过程中如何演化的理论框架。需要一种能够显式控制这些粗粒度量的反向扩散方案。

Method: 将信息论Lyapunov函数V-δ框架移植到生成模型的反向扩散过程，提出V-δ投影反向扩散方法。该方法将反向扩散过程投影到由V-δ定义的势能面上，并扩展到时间非齐次的区块保持马尔可夫核。

Result: 在区块恒定图像的玩具模型和简化反向核下，数值实验表明所提方法能将区块质量误差和泄漏容忍势能控制在预设容差内，同时达到与非投影动力学相当的像素级精度和视觉质量。

Conclusion: 该研究将生成采样重新解释为从噪声到数据的信息势能下降过程，为具有显式粗粒度量控制的反向扩散过程提供了设计原则，建立了扩散模型中粗粒度统计量控制的理论基础。
...
### [217] [A UCB Bandit Algorithm for General ML-Based Estimators](https://arxiv.org/abs/2601.01061)
...
<details>
  <summary>Details</summary>
Motivation: 在序列决策中部署复杂ML模型面临的主要挑战是缺乏可处理的浓度不等式来进行有原则的探索。现有方法需要针对特定模型进行理论分析，限制了通用性。

Method: 假设均方误差随训练样本数量呈幂律下降，推导出广义浓度不等式，并基于此开发ML-UCB算法。该框架允许集成任何学习曲线可经验表征的ML模型。

Result: 理论证明ML-UCB能够实现次线性遗憾，实验验证在协同过滤推荐系统中（使用在线矩阵分解和模拟双塔模型的合成数据）相比LinUCB有显著改进。

Conclusion: ML-UCB为将任意ML模型集成到多臂老虎机框架提供了原则性方法，消除了模型特定理论分析的需求，通过经验学习曲线表征即可实现有效探索。
...
### [218] [SPoRC-VIST: A Benchmark for Evaluating Generative Natural Narrative in Vision-Language Models](https://arxiv.org/abs/2601.01062)
*Yunlin Zeng*

Main category: cs.LG

TL;DR: 提出视觉播客生成新流程，微调Qwen3-VL-32B模型，使用合成到真实训练策略，在对话自然度和叙事深度上显著超越更大基础模型。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在描述性任务上表现良好，但在生成引人入胜的长篇叙事（特别是多人播客对话）方面能力不足且难以评估。传统指标无法捕捉对话自然度、个性和叙事流畅性等细微差别。

Method: 1) 提出端到端视觉播客生成流程；2) 在4000个图像-对话对数据集上微调Qwen3-VL-32B模型；3) 采用合成到真实训练策略：在SPoRC高质量播客对话与合成生成图像上训练，在VIST真实世界照片序列上评估；4) 提出综合评估框架，使用AI作为裁判和新型风格指标。

Result: 微调的32B模型在对话自然度上显著优于235B基础模型（胜率>80%），叙事深度提升50%（平均对话轮次长度增加），同时保持相同的视觉基础能力（CLIPScore: 20.39）。

Conclusion: 通过合成到真实训练策略和综合评估框架，证明了较小模型在视觉播客生成任务上可以超越更大模型，为长篇叙事生成提供了有效的解决方案和评估方法。
...
### [219] [Tiny Machine Learning for Real-Time Aquaculture Monitoring: A Case Study in Morocco](https://arxiv.org/abs/2601.01065)
...
<details>
  <summary>Details</summary>
Motivation: 水产养殖业面临水质波动、疾病爆发和饲料管理效率低等挑战，传统监测方法依赖人工、耗时且可能导致问题处理延迟，需要更高效的自动化解决方案。

Method: 集成基于TinyML的低功耗边缘设备，通过传感器实时收集pH值、温度、溶解氧、氨氮水平等参数数据，实现自动化监测、异常检测报警和控制功能。

Result: 系统能够实时监测水质参数，自动触发警报，减少人工需求，收集的数据可用于优化水处理过程、饲料分配和模式分析，提高饲料效率并降低运营成本。

Conclusion: 研究表明TinyML在水产养殖监测中具有可行性，通过考虑传感器选择、算法设计、硬件约束和伦理因素，可开发更可持续和高效的养殖实践。
...
### [220] [Revisiting Weighted Strategy for Non-stationary Parametric Bandits and MDPs](https://arxiv.org/abs/2601.01069)
*Jing Wang,Peng Zhao,Zhi-Hua Zhou*

Main category: cs.LG

TL;DR: 本文提出了一种改进的分析框架，简化了非平稳参数化赌博机中加权策略的算法设计和理论分析，在线性赌博机、广义线性赌博机、自协调赌博机以及具有函数逼近的MDP中实现了更简单、更高效的算法和更好的遗憾界。


<details>
  <summary>Details</summary>
Motivation: 非平稳参数化赌博机中，加权策略因其适用于渐变漂移环境而在实际应用中常用，但先前理论分析复杂，导致算法计算效率低或统计次优。本文旨在解决加权策略分析框架的不足，简化算法设计并改进遗憾界。

Method: 提出精炼的分析框架，重新分析加权策略。在线性赌博机中简化算法设计，使其与窗口/重启算法同样高效。将该框架扩展到广义线性赌博机、自协调赌博机以及具有函数逼近的MDP（线性混合MDP和多项Logit混合MDP），开发基于加权策略的算法。

Result: 在线性赌博机中，新框架产生更简单的加权算法，计算效率与窗口/重启算法相当，遗憾界保持相同。在广义线性赌博机中，获得$\tilde{O}(k_μ^{5/4} c_μ^{-3/4} d^{3/4} P_T^{1/4}T^{3/4})$的遗憾界，优于先前的$\tilde{O}(k_μ^{2} c_μ^{-1}d^{9/10} P_T^{1/5}T^{4/5})$。框架成功扩展到MDP，为线性混合MDP和多项Logit混合MDP建立了动态遗憾保证。

Conclusion: 本文提出的精炼分析框架解决了非平稳参数化赌博机中加权策略的理论分析难题，简化了算法设计，提高了计算效率，并在多种参数化赌博机和具有函数逼近的MDP中实现了更好的遗憾界，为处理非平稳环境提供了更有效的工具。
...
### [221] [Flow Equivariant World Models: Memory for Partially Observed Dynamic Environments](https://arxiv.org/abs/2601.01075)
...
<details>
  <summary>Details</summary>
Motivation: 现有神经网络世界模型忽略了现实世界中连续感官输入流所遵循的平滑时间参数化对称性结构，导致重复从数据中学习相同变换，效率低下。

Method: 将自运动和外部物体运动统一为单参数李群"流"，实现对这些变换的群等变性，构建具有稳定潜在表示的世界模型框架。

Result: 在2D和3D部分观测视频世界建模基准上，Flow Equivariant World Models显著优于当前最先进的基于扩散和记忆增强的世界建模架构，特别是在智能体当前视野外存在可预测世界动态时表现更优，且对长时rollout具有良好泛化能力。
...
### [222] [Discount Model Search for Quality Diversity Optimization in High-Dimensional Measure Spaces](https://arxiv.org/abs/2601.01082)
*Bryon Tjanaka,Henry Chen,Matthew C. Fontaine,Stefanos Nikolaidis*

Main category: cs.LG

TL;DR: 提出Discount Model Search (DMS)算法，解决高维度量空间中质量多样性优化的问题，通过连续折扣值模型区分相似度量解，在图像等高维度量空间中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有质量多样性(QD)算法主要针对低维度量空间，高维度量空间容易产生度量扭曲问题，即许多解映射到相似的度量值。例如CMA-MAE算法使用直方图记录折扣值，但在高维空间中相似度量的解会落入同一直方图单元，导致探索停滞。

Method: 提出Discount Model Search (DMS)算法，使用连续折扣值模型指导探索。该模型提供平滑、连续的折扣值表示，在高维度量空间中能够区分相似度量的解，从而持续探索。

Result: 在图像等高维度量空间基准测试中，DMS表现优于CMA-MAE和其他黑盒QD算法。DMS支持新的QD应用场景，用户可以通过提供图像数据集而非手动设计度量函数来指定期望的度量。

Conclusion: DMS通过连续折扣值模型解决了高维度量空间中的探索停滞问题，扩展了QD算法的应用范围，特别是在图像等高维度量空间中表现出色。
...
### [223] [Central Dogma Transformer: Towards Mechanism-Oriented AI for Cellular Understanding](https://arxiv.org/abs/2601.01089)
*Nobuyuki Ota*

Main category: cs.LG

TL;DR: CDT是一个整合DNA、RNA和蛋白质预训练语言模型的架构，遵循中心法则方向逻辑，通过跨注意力机制产生统一的虚拟细胞嵌入，在CRISPRi增强子扰动数据上取得良好预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有领域特定的基础模型虽然在各模态上取得成功，但彼此孤立，限制了建模整合细胞过程的能力。需要开发能整合DNA、RNA和蛋白质信息的架构来理解细胞机制。

Method: 提出中心法则变换器（CDT），整合DNA、RNA和蛋白质的预训练语言模型，采用方向性跨注意力机制：DNA-to-RNA注意力建模转录调控，RNA-to-Protein注意力建模翻译关系，产生统一的虚拟细胞嵌入。

Result: 在K562细胞的CRISPRi增强子扰动数据上，CDT v1（使用固定RNA和蛋白质嵌入的概念验证）达到Pearson相关系数0.503，占交叉实验变异性理论上限（r=0.797）的63%。注意力和梯度分析提供了互补的解释窗口。

Conclusion: 与生物信息流对齐的AI架构既能实现预测准确性，又能获得机制可解释性，为理解整合的细胞过程提供了新途径。
...
### [224] [Community-Based Early-Stage Chronic Kidney Disease Screening using Explainable Machine Learning for Low-Resource Settings](https://arxiv.org/abs/2601.01119)
...
TL;DR: 开发针对孟加拉国和南亚人群的可解释机器学习框架，用于社区早期慢性肾病筛查，相比现有工具显著提高准确性和敏感性


<details>
  <summary>Details</summary>
Motivation: 现有慢性肾病筛查工具主要基于高收入国家人群开发，在孟加拉国和南亚地区表现不佳，且大多依赖简单评分函数，无法捕捉风险因素间的复杂交互作用，难以预测早期肾病

Method: 使用孟加拉国社区数据集，评估12种机器学习分类器，应用10种特征选择技术识别稳健预测因子，采用10折交叉验证，并在印度、阿联酋和孟加拉国的三个独立数据集上进行外部验证，使用SHAP提供模型可解释性

Result: RFECV特征选择模型平衡准确率达90.40%，最小非病理测试特征集平衡准确率达89.23%，优于更大或完整特征集。相比现有筛查工具，模型准确性和敏感性显著提高，外部验证敏感性达78%-98%，SHAP识别出与已知CKD风险因素一致的临床有意义预测因子

Conclusion: 提出的可解释机器学习框架为低资源环境下的社区早期慢性肾病筛查提供了有效解决方案，特别适合孟加拉国和南亚人群，具有高准确性和良好泛化能力
...
### [225] [Learning from Historical Activations in Graph Neural Networks](https://arxiv.org/abs/2601.01123)
...
<details>
  <summary>Details</summary>
Motivation: 传统图池化方法仅使用最后一层GNN特征，未能充分利用前向传播过程中产生的中间层激活（历史图激活）。这在节点表示可能随层数显著变化的情况下尤其明显，且图特有的挑战如深度架构中的过平滑问题会加剧这一问题。

Method: 提出HISTOGRAPH，一种新颖的两阶段基于注意力的最终聚合层：1）首先在中间激活上应用统一的层间注意力；2）然后应用节点间注意力。通过建模节点表示在层间的演化，利用节点的激活历史和图结构来精炼用于最终预测的特征。

Result: 在多个图分类基准测试上的实证结果表明，HISTOGRAPH提供了强大的性能，持续改进传统技术，在深度GNN中表现出特别强的鲁棒性。

Conclusion: 通过利用历史图激活，HISTOGRAPH能够更有效地聚合图表示，特别是在深度架构中，为图神经网络提供了更强大的池化机制。
...
### [226] [Wittgenstein's Family Resemblance Clustering Algorithm](https://arxiv.org/abs/2601.01127)
*Golbahar Amanpour,Benyamin Ghojogh*

Main category: cs.LG

TL;DR: 基于维特根斯坦家族相似性概念的图聚类算法，无需预设簇数量或形状假设


<details>
  <summary>Details</summary>
Motivation: 将分析哲学中的家族相似性概念引入机器学习，解决传统聚类算法需要预设簇数量和形状假设的限制

Method: 提出WFR算法：计算相邻数据实例的相似度分数，阈值处理后构建相似图，图的连通分量即为聚类结果；同时提出核WFR变体

Result: 在基准数据集上的模拟表明WFR是有效的非线性聚类算法，无需簇数量先验知识或形状假设

Conclusion: 哲学概念与机器学习结合成功，WFR算法为聚类问题提供了新的有效解决方案
...
### [227] [Self-Training the Neurochaos Learning Algorithm](https://arxiv.org/abs/2601.01146)
*Anusree M,Akhila Henry,Pramod P Nair*

Main category: cs.LG

TL;DR: 提出一种结合神经混沌学习与自训练的混合半监督学习架构，在数据稀缺和不平衡场景下显著提升分类性能


<details>
  <summary>Details</summary>
Motivation: 在许多实际应用中，获取大量标注数据既困难又昂贵，而未标注数据却容易获得。传统监督学习方法在标注数据少或数据集不平衡的情况下表现不佳，需要解决这一限制。

Method: 提出Self-Training Neurochaos Learning (NL+ST)混合架构：1) 神经混沌学习(NL)将输入特征转换为混沌基的发放率表示，捕捉数据中的非线性关系；2) 基于阈值的自训练(ST)方法逐步扩展标注集，利用高置信度的伪标注样本。

Result: 在10个基准数据集和5个机器学习分类器上评估，仅使用15%标注数据和85%未标注数据。NL+ST架构相比独立ST模型获得显著性能提升，特别是在Iris(188.66%)、Wine(158.58%)和Glass Identification(110.48%)等有限、非线性和不平衡数据集上表现突出。

Conclusion: 混沌基特征提取与半监督学习结合，在低数据场景下能有效提升模型的泛化能力、鲁棒性和分类准确性，为解决标注数据稀缺问题提供了有效方案。
...
### [228] [Evo-TFS: Evolutionary Time-Frequency Domain-Based Synthetic Minority Oversampling Approach to Imbalanced Time Series Classification](https://arxiv.org/abs/2601.01150)
*Wenbin Pei,Ruohao Dai,Bing Xue,Mengjie Zhang,Qiang Zhang,Yiu-Ming Cheung*

Main category: cs.LG

TL;DR: Evo-TFS：一种结合时域和频域特征的进化过采样方法，用于解决不平衡时间序列分类问题


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法假设数据分布平衡，在不平衡数据中会忽略少数类；传统过采样方法依赖线性插值，难以保持时间动态特性和生成多样化样本

Method: 使用强类型遗传编程进化多样化的高质量时间序列，通过包含时域和频域特征的适应度函数指导进化过程

Result: 在不平衡时间序列数据集上的实验表明，Evo-TFS优于现有过采样方法，显著提升了时域和频域分类器的性能

Conclusion: Evo-TFS通过结合时域和频域特征的进化过采样，有效解决了不平衡时间序列分类问题，生成了更高质量和多样性的少数类样本
...
### [229] [Bridging the Semantic Gap for Categorical Data Clustering via Large Language Models](https://arxiv.org/abs/2601.01162)
*Zihua Yang,Xin Liao,Yiqun Zhang,Yiu-ming Cheung*

Main category: cs.LG

TL;DR: ARISE利用大语言模型为分类数据提供语义嵌入，通过结合原始数据和LLM增强的表示来提升聚类效果


<details>
  <summary>Details</summary>
Motivation: 分类数据聚类面临相似性度量的核心挑战，传统方法将无序的分类值视为等距，造成语义鸿沟。现有方法依赖数据集内共现模式推断值关系，但在样本有限时不可靠，未能充分探索数据的语义上下文。

Method: 提出ARISE方法，利用大语言模型的外部语义知识构建语义感知表示。LLM用于描述属性值以增强表示，然后将LLM增强的嵌入与原始数据结合，探索语义突出的聚类。

Result: 在八个基准数据集上的实验表明，相比七个代表性对比方法，ARISE取得了19-27%的性能提升。

Conclusion: ARISE通过整合LLM的语义知识，有效弥补了分类数据聚类的语义鸿沟，显著提升了聚类质量。
...
### [230] [MentalGame: Predicting Personality-Job Fitness for Software Developers Using Multi-Genre Games and Machine Learning Approaches](https://arxiv.org/abs/2601.01206)
...
<details>
  <summary>Details</summary>
Motivation: 传统职业评估中的人格问卷存在回答偏差、疲劳和故意扭曲等问题，需要一种更客观、可扩展且减少偏见的替代方法。游戏化评估通过捕捉游戏过程中的隐式行为信号，为职业适配性预测提供了新途径。

Method: 通过文献综述和软件工程师实证研究确定开发人员相关人格和行为特质，设计定制移动游戏来引发问题解决、规划、适应性、坚持性、时间管理和信息寻求等行为。采用两阶段建模策略，仅从游戏行为特征预测适配性。

Result: 模型达到最高97%的精确度和94%的准确率。行为分析显示，合适候选人表现出独特的游戏模式：解谜游戏获胜更多、完成更多侧边挑战、更频繁导航菜单、暂停/重试/放弃行为更少。

Conclusion: 游戏过程中捕捉的隐式行为痕迹能够有效预测软件开发适配性，无需显式人格测试。严肃游戏作为职业评估工具具有可扩展性、趣味性和较少偏见等优势。
...
### [231] [Sparse Bayesian Message Passing under Structural Uncertainty](https://arxiv.org/abs/2601.01207)
*Yoonhyuk Choi,Jiho Choi,Chanran Kim,Yumin Lee,Hawon Shin,Yeowon Jeon,Minjeong Kim,Jiwoo Kang*

Main category: cs.LG

TL;DR: 提出一种通过建模带符号邻接矩阵后验分布来处理异质图结构不确定性的半监督学习方法，使用稀疏带符号消息传递网络，在异质图基准上优于基线模型。
...
Method: 通过建模带符号邻接矩阵的后验分布（每条边可为正、负或不存在），提出稀疏带符号消息传递网络，结合后验边缘化和稀疏带符号消息聚合，从贝叶斯角度提供理论解释。

Result: 在合成和真实世界结构噪声下的异质图基准测试中，该方法优于强基线模型，证明了其对边噪声和异质性的鲁棒性。

Conclusion: 显式建模结构不确定性并通过稀疏带符号消息传递处理异质图，为处理边噪声和异质性提供了原则性方法，在异质图学习任务中表现出色。
...
### [232] [Adaptive Conformal Prediction via Bayesian Uncertainty Weighting for Hierarchical Healthcare Data](https://arxiv.org/abs/2601.01223)
*Marzieh Amiri Shahbazi,Ali Baheri,Nasibeh Azadeh-Fard*

Main category: cs.LG

TL;DR: 提出混合贝叶斯-保形框架，结合贝叶斯层次随机森林与群体感知保形校准，为医疗预测提供分布自由的覆盖保证和风险自适应精度
...
Method: 集成贝叶斯层次随机森林与群体感知保形校准，使用后验不确定性加权保形分数，同时保持严格的覆盖有效性

Result: 在61,538例入院患者、3,793家美国医院和4个地区评估中，达到目标覆盖率（94.3% vs 95%目标），低不确定性病例区间宽度减少21%，高风险预测适当加宽区间

Conclusion: 该框架支持风险分层临床协议、高效资源规划和高置信度预测，为不确定病例提供增强监督的保守分配，实现跨多样化医疗设置的不确定性感知决策支持
...
### [233] [The Dependency Divide: An Interpretable Machine Learning Framework for Profiling Student Digital Satisfaction in the Bangladesh Context](https://arxiv.org/abs/2601.01231)
*Md Muhtasim Munif Fahim,Humyra Ankona,Md Monimul Huq,Md Rezaul Karim*

Main category: cs.LG

TL;DR: 研究发现"依赖鸿沟"现象：在基础设施脆弱的环境中，高度投入的学生反而更容易因网络故障而受挫，挑战了"投入越多越好"的传统观念。


<details>
  <summary>Details</summary>
Motivation: 传统数字鸿沟框架无法解释在资源有限环境下，拥有相似连接条件的学生对数字学习平台满意度差异显著的现象。本研究旨在探索这种差异背后的机制。

Method: 对孟加拉国396名大学生进行横断面研究，采用三阶段分析方法：1) K-prototypes聚类识别学生特征；2) 随机森林模型分析满意度驱动因素；3) 倾向得分匹配验证依赖鸿沟假设。

Result: 识别出三类学生：随意投入者(58%)、高效学习者(35%)和高度投入者(7%)。发现教育设备使用时间与网络可靠性存在显著交互作用，证实了依赖鸿沟假设。高度投入学生在基础设施不可靠时满意度最低。

Conclusion: 在基础设施脆弱的环境中，数字能力可能成为负担。数字转型政策应优先保障高依赖用户的可靠性，建立应急系统，并教育学生了解依赖风险，而非一味鼓励投入。
...
### [234] [Benchmarking the Computational and Representational Efficiency of State Space Models against Transformers on Long-Context Dyadic Sessions](https://arxiv.org/abs/2601.01237)
*Abidemi Koledoye,Chinemerem Unachukwu,Gold Nwobu,Hasin Rana*

Main category: cs.LG

TL;DR: 该论文比较了Mamba SSM和LLaMA Transformer在长上下文序列建模中的性能，特别针对治疗会话数据，发现SSM在特定条件下比Transformer更具优势


<details>
  <summary>Details</summary>
Motivation: 状态空间模型（SSMs）作为Transformer的替代方案，在长上下文序列建模中展现出潜力，具有线性计算复杂度（O(N)），而Transformer是二次复杂度（O(N²)）。需要系统比较这两种架构在实际应用中的表现，为从业者提供指导。

Method: 使用治疗会话作为代表性测试案例，从两个维度评估Mamba SSM和LLaMA Transformer：1）计算效率（测量512到8192个token的内存使用和推理速度）；2）表示效率（分析隐藏状态动态和注意力模式）。

Result: 研究结果为长上下文应用提供了可操作的见解，确定了SSM相对于Transformer具有优势的具体条件。

Conclusion: SSM在长上下文序列建模中确实具有优势，特别是在计算效率方面，但具体优势条件需要根据应用场景确定，为从业者选择架构提供了实证依据。
...
### [235] [Accelerated Full Waveform Inversion by Deep Compressed Learning](https://arxiv.org/abs/2601.01268)
*Maayan Gelboim,Amir Adler,Mauricio Araya-Polo*

Main category: cs.LG

TL;DR: 提出一种通过深度神经网络和压缩学习来降低全波形反演输入数据维度的方法，利用二值化感知层学习关键地震采集布局，通过表示学习和聚类进一步选择最相关数据，显著优于随机采样。


<details>
  <summary>Details</summary>
Motivation: 现代地震采集系统产生的工业级全波形反演数据达到太字节存储级别，使得复杂地下情况建模或多场景探索变得计算成本过高，需要降低数据维度以减轻计算负担。

Method: 使用带有二值化感知层的深度神经网络，通过压缩学习从大量地下模型中学习关键地震采集布局；对大型地震数据集，训练网络选择数据子集，然后通过自编码器计算潜在表示，再用K-means聚类进一步选择最相关数据用于全波形反演。

Result: 该方法在仅使用10%数据的情况下，在2D全波形反演中始终优于随机数据采样，为大规模3D反演加速铺平了道路。

Conclusion: 提出的分层选择方法能有效降低全波形反演的计算成本，通过智能数据选择而非随机采样，为处理大规模地震数据提供了可行的解决方案。
...
### [236] [The Alchemy of Thought: Understanding In-Context Learning Through Supervised Classification](https://arxiv.org/abs/2601.01290)
*Harshita Narnoli,Mihai Surdeanu*

Main category: cs.LG

TL;DR: 研究发现ICL在演示相关性高时表现类似分类器，更接近kNN而非逻辑回归；相关性低时LLMs表现更好，因其能依赖参数记忆。


<details>
  <summary>Details</summary>
Motivation: 尽管ICL在实践中有效，但其工作机制尚不明确。本文旨在通过比较ICL与监督分类器的行为来深入理解ICL的工作原理。

Method: 使用文本分类作为用例，在6个数据集和3个LLMs上，将ICL行为与基于梯度下降（逻辑回归）和k近邻的分类器进行比较，分析演示相关性对行为相似性的影响。

Result: 当演示相关性高时，LLMs行为与分类器相似，且更接近kNN而非逻辑回归；演示相关性低时，LLMs表现优于分类器，因其能依赖参数记忆。

Conclusion: ICL机制更类似kNN而非梯度下降，LLMs在演示相关性低时的优势源于其参数记忆能力，这为理解ICL工作原理提供了实证依据。
...
### [237] [Sobolev Approximation of Deep ReLU Network in Log-weighted Barron Space](https://arxiv.org/abs/2601.01295)
*Changhoon Song,Seungchan Ko,Youngjoon Hong*

Main category: cs.LG

TL;DR: 引入对数加权Barron空间，比传统Barron空间要求更弱的正则性条件，证明深度ReLU网络在该空间中的逼近能力，阐明深度如何降低高效表示所需的正则性要求。


<details>
  <summary>Details</summary>
Motivation: 现有理论无法完全解释深度模型在高维数据上的实际成功：通用逼近定理的参数数量随维度指数增长；传统Barron空间要求比Sobolev空间更强的正则性；现有深度敏感结果通常假设约束条件如sL≤1/2。需要更弱的正则性假设来更好地解释深度架构的性能。

Method: 1. 引入对数加权Barron空间B^log，比传统B^s空间要求更弱的假设；2. 研究该空间的嵌入性质并通过Rademacher复杂度进行统计分析；3. 证明深度ReLU网络在B^log空间中的逼近能力；4. 定义B^{s,log}空间族，建立H^1范数下的逼近界；5. 识别保持这些速率的最大深度尺度。

Result: 1. B^log空间比任何s>0的B^s空间要求更弱的假设；2. 深度ReLU网络能够有效逼近B^log空间中的函数；3. 建立了H^1范数下的逼近界；4. 确定了保持逼近速率的深度尺度；5. 阐明了深度如何降低高效表示所需的正则性要求。

Conclusion: 对数加权Barron空间为深度架构的性能提供了比传统Barron设置更精确的解释，阐明了深度如何降低高效表示所需的函数正则性要求，为深度网络在高维问题中的稳定使用提供了理论支持。
...
### [238] [ARGUS: Adaptive Rotation-Invariant Geometric Unsupervised System](https://arxiv.org/abs/2601.01297)
*Anantha Sharma*

Main category: cs.LG

TL;DR: Argus框架将高维数据流中的分布漂移检测重新定义为在数据流形固定空间划分上跟踪局部统计量，通过Voronoi划分实现正交变换不变性、O(N)复杂度、漂移传播图论分析和产品量化划分支持超高维扩展。


<details>
  <summary>Details</summary>
Motivation: 高维数据流中的分布漂移检测面临三大挑战：全局比较方法扩展性差，基于投影的方法丢失几何结构，重新聚类方法存在身份不稳定性。需要一种既能保持高维几何结构又具有计算效率的漂移检测方法。

Method: Argus框架将漂移检测重新定义为在数据流形的固定空间划分上跟踪局部统计量。核心方法包括：1）使用规范正交基上的Voronoi划分实现正交变换不变性；2）实现O(N)复杂度每快照；3）开发漂移传播的图论表征；4）引入产品量化划分支持超高维（d>500）扩展。

Result: 实验验证表明，Argus框架在坐标旋转下能正确识别漂移，而现有方法会产生误报。该框架提供了分布监控的原则性几何基础，能在不增加成对比较计算负担的情况下保持高维结构。

Conclusion: Argus框架为高维数据流中的分布漂移检测提供了新的几何视角，通过固定空间划分和局部统计跟踪，实现了正交变换不变性、计算效率和空间局部化的统一，为大规模高维分布监控提供了可行方案。
...
### [239] [Warp-Cortex: An Asynchronous, Memory-Efficient Architecture for Million-Agent Cognitive Scaling on Consumer Hardware](https://arxiv.org/abs/2601.01298)
*Jorge L. Ruiz Williams*

Main category: cs.LG

TL;DR: Warp Cortex：一种异步多智能体LLM框架，通过解耦智能体逻辑与物理内存，实现百万级智能体认知扩展，内存复杂度从O(N*L)降至O(1)权重和O(N*k)上下文


<details>
  <summary>Details</summary>
Motivation: 当前多智能体LLM框架存在线性内存扩展问题，使得"系统2"并行推理在消费级硬件上不切实际，需要解决内存瓶颈以实现大规模智能体并发

Method: 采用异步架构，通过单例权重共享和拓扑突触技术（受TDA混合地标技术启发），将KV缓存视为潜在空间点云，应用见证复形稀疏化来保持上下文流形的持久同调特征，并引入非侵入式KV缓存更新机制Referential Injection

Result: 在单张NVIDIA RTX 4090上实现100个并发智能体仅需2.2GB VRAM，理论容量超过1000个智能体，计算延迟成为主要瓶颈

Conclusion: Warp Cortex通过创新的内存优化架构，显著降低了多智能体LLM系统的内存需求，为实现大规模并行推理系统提供了可行方案
...
### [240] [Towards a Principled Muon under $μ\mathsf{P}$: Ensuring Spectral Conditions throughout Training](https://arxiv.org/abs/2601.01306)
*John Zhao*

Main category: cs.LG

TL;DR: 提出Muon++优化器，通过仅在更新层面控制谱条件，确保μP在长时训练中的有效性，无需权重谱归一化，降低计算开销。


<details>
  <summary>Details</summary>
Motivation: μ参数化（μP）为LLM训练提供了理论基础，但现有矩阵优化器（如Muon）在μP框架下存在局限性：要么无法保证整个训练过程中的谱条件，要么需要频繁的谱归一化操作，计算开销大且不实用。

Method: 提出Muon++优化器变体，关键洞见是：对于中等规模模型，仅在优化器更新层面维持谱控制就足以保持μP兼容的缩放特性，无需显式对权重进行谱归一化。还首次尝试引入数据依赖的自适应谱条件。

Result: Muon++能在整个训练过程中可靠地保证μP所需的谱条件，填补了μP理论承诺与矩阵优化器实际部署之间的差距，特别适合长时LLM训练。

Conclusion: 该工作通过仅在更新层面控制谱条件，实现了μP框架下矩阵优化器的实用化，为长时LLM训练提供了更高效、更可靠的优化方案。
...
### [241] [Spectral-Window Hybrid (SWH)](https://arxiv.org/abs/2601.01313)
*Vladimer Khasia*

Main category: cs.LG

TL;DR: SWH是一种混合架构，通过并行全局谱分支和局部注意力分支，在保持局部精度的同时实现线性扩展，解决Transformer在长序列上的计算瓶颈。


<details>
  <summary>Details</summary>
Motivation: Transformer的注意力机制具有O(T²)复杂度，限制了其在长序列任务中的应用。需要在计算效率和表示表达能力之间取得平衡。

Method: 提出Spectral-Window Hybrid (SWH)架构，将序列建模解耦为两个并行流：1) 全局分支利用卷积定理在O(T log T)时间内建模长程衰减动态；2) 局部分支使用滑动窗口注意力处理有界上下文内的token交互。通过聚合这些表示，避免全局注意力的计算瓶颈。

Result: SWH在短上下文上匹配标准Transformer的困惑度，同时在扩展序列上实现高效的线性扩展。

Conclusion: SWH架构通过结合全局谱建模和局部注意力机制，在保持模型表达能力的同时显著降低了长序列处理的计算复杂度。
...
### [242] [From Classification to Generation: An Open-Ended Paradigm for Adverse Drug Reaction Prediction Based on Graph-Motif Feature Fusion](https://arxiv.org/abs/2601.01347)
*Yuyan Pi,Min Jin,Wentao Xie,Xinhua Liu*

Main category: cs.LG

TL;DR: GM-MLG提出了一种基于图-基序特征融合和多标签生成的开放式药物不良反应预测新范式，将ADR预测从多标签分类转化为基于Transformer解码器的多标签生成，显著提升了预测性能并扩展了预测空间。


<details>
  <summary>Details</summary>
Motivation: 当前药物不良反应预测方法面临三大挑战：药物数据稀缺导致的冷启动问题、封闭标签集限制以及标签依赖关系建模不足。这些限制阻碍了计算生物学在降低新药开发成本和周期方面的潜力。

Method: GM-MLG构建了跨越原子级、局部分子级（使用BRICS算法动态提取的精细基序）和全局分子级的双图表示架构。创新性地将ADR预测从多标签分类转化为基于Transformer解码器的多标签生成，将ADR标签视为离散标记序列，使用位置嵌入显式捕获大规模标签空间中的依赖关系和共现关系，通过自回归解码生成预测。

Result: 实验表明GM-MLG实现了高达38%的性能提升，平均增益达20%，将预测空间从200种扩展到超过10,000种。通过逆合成基序分析阐明了ADR与基序之间的非线性结构-活性关系。

Conclusion: GM-MLG为药物不良反应预测提供了开放式、可扩展的新范式，通过多标签生成方法有效解决了冷启动和标签依赖建模问题，为系统降低药物安全风险提供了可解释的创新支持。
...
### [243] [Towards LLM-enabled autonomous combustion research: A literature-aware agent for self-corrective modeling workflows](https://arxiv.org/abs/2601.01357)
*Ke Xiao,Haoze Zhang,Runze Mao,Han Li,Zhi X. Chen*

Main category: cs.LG

TL;DR: FlamePilot是一个专门用于燃烧建模的LLM智能体，通过原子工具架构实现CFD模拟的自动化设置和执行，并能从科学文献中学习指导模拟流程，在基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在复杂科学领域（如燃烧建模）存在关键缺口，需要将领域文献知识与专业工具（如CFD代码）执行能力无缝集成，以实现实用的AI研究助手。

Method: FlamePilot采用原子工具架构，确保在OpenFOAM和DeepFlame等框架中稳健设置和执行复杂模拟，并能从科学文章中学习提取关键信息指导模拟流程，支持自动化和自校正工作流。

Result: 在公开基准测试中，FlamePilot获得了1.0的可执行性分数和0.438的成功率，超过了之前最佳代理的0.625和0.250分数。在MILD燃烧模拟案例中，成功将研究论文转化为配置模拟、执行、后处理、提出改进建议并管理多步参数研究。

Conclusion: FlamePilot通过透明可解释的范式，为AI赋能的燃烧建模建立了基础框架，实现了智能体管理工作流编排、研究人员专注于高层分析的协作伙伴关系。
...
### [244] [Causal discovery for linear causal model with correlated noise: an Adversarial Learning Approach](https://arxiv.org/abs/2601.01368)
*Mujin Zhou,Junzhe Zhang*

Main category: cs.LG

TL;DR: 基于f-GAN框架的因果发现方法，通过最小化贝叶斯自由能量学习二元因果结构，使用Gumbel-Softmax松弛在离散图空间进行梯度搜索


<details>
  <summary>Details</summary>
Motivation: 从存在未测量混杂因素的数据中进行因果发现是一个具有挑战性的问题，需要一种不依赖于特定权重值的学习方法

Method: 将结构学习问题重新表述为最小化贝叶斯自由能量，证明该问题等价于最小化真实数据分布与模型生成分布之间的f-散度，利用f-GAN框架转化为最小最大对抗优化问题，使用Gumbel-Softmax松弛在离散图空间实现梯度搜索

Result: 提出了一种基于f-GAN的因果发现框架，能够学习二元因果结构而不依赖于特定权重值，通过对抗优化和离散空间梯度搜索实现有效学习

Conclusion: 该方法为存在未测量混杂因素的因果发现问题提供了新的解决方案，通过f-GAN框架和贝叶斯自由能量最小化实现了有效的结构学习
...
### [245] [Data Complexity-aware Deep Model Performance Forecasting](https://arxiv.org/abs/2601.01383)
*Yen-Chia Chen,Hsing-Kuo Pao,Hanjuan Huang*

Main category: cs.LG

TL;DR: 提出一个轻量级的两阶段框架，在训练前根据数据集属性和模型结构预测模型性能，无需实际训练或复杂模拟。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型选择通常依赖试错过程，耗时耗资源且难以自动化。现有性能预测方法要么需要部分训练，要么计算开销大或泛化能力不足。
...
Conclusion: 提出的轻量级框架为模型选择和性能预测提供了高效实用的解决方案，减少了试错成本，并能提供数据质量洞察。
...
### [246] [Scale-Adaptive Power Flow Analysis with Local Topology Slicing and Multi-Task Graph Learning](https://arxiv.org/abs/2601.01387)
...
Method: 提出SaMPFA框架，包含：1) 局部拓扑切片采样技术，从完整电网提取不同规模的子图以增强跨尺度学习能力；2) 无参考多任务图学习模型，预测母线电压和支路功率而非相角，避免误差放大并学习相角差的物理关系；3) 损失函数中加入额外项，鼓励模型捕捉角差和功率传输的物理模式。

Result: 在IEEE 39节点系统和实际省级电网上的仿真表明，该模型在可变系统规模下具有优越的适应性和泛化能力，准确率分别提高了4.47%和36.82%。

Conclusion: SaMPFA框架通过创新的采样技术和多任务学习设计，显著提升了潮流分析模型对拓扑变化的适应性和预测鲁棒性，为电力系统分析提供了有效的深度学习解决方案。
...
### [247] [A Graph-based Framework for Online Time Series Anomaly Detection Using Model Ensemble](https://arxiv.org/abs/2601.01403)
*Zewei Yu,Jianqiu Xu,Caimin Li*

Main category: cs.LG

TL;DR: 提出GDME框架，一种基于图模型集成的无监督在线时间序列异常检测方法，通过动态模型池和图结构处理异构流数据


<details>
  <summary>Details</summary>
Motivation: 工业系统中流数据量不断增加，在线异常检测成为关键任务。现有方法多为离线设计或难以有效处理异构流数据，需要适应快速变化的数据模式

Method: GDME框架维护动态模型池，持续更新（修剪表现不佳模型并引入新模型）。使用动态图结构表示模型间关系，通过社区检测选择合适子集进行集成。利用图结构变化监测概念漂移，适应流数据演化

Result: 在7个异构时间序列数据集上，GDME优于现有在线异常检测方法，提升达24%。集成策略优于单个模型和平均集成，计算效率具有竞争力

Conclusion: GDME框架能有效处理异构流数据的在线异常检测，通过动态模型池和图结构集成策略，适应概念漂移，在性能和效率上均表现优异
...
### [248] [A Depth Hierarchy for Computing the Maximum in ReLU Networks via Extremal Graph Theory](https://arxiv.org/abs/2601.01417)
*Itay Safran*

Main category: cs.LG

TL;DR: 该论文证明了ReLU神经网络计算最大值函数时存在深度层次结构：对于深度3≤k≤log₂(log₂(d))，需要宽度Ω(d^{1+1/(2^{k-2}-1)})才能精确表示d个实数的最大值，这是首个关于该基本算子在深度k≥3时的无条件超线性下界。


<details>
  <summary>Details</summary>
Motivation: 研究ReLU神经网络精确计算最大值函数的计算复杂度，探索深度神经网络表示基本算子的能力限制，为深度神经网络的理论分析提供新视角。

Method: 采用组合论证方法，将最大值函数的不可微分脊线与计算网络第一隐藏层诱导的图中的团相关联，利用极值图论中的Turán定理证明足够窄的网络无法捕捉最大值的非线性特性。

Result: 证明了深度层次结构：对于任何深度3≤k≤log₂(log₂(d))，需要宽度Ω(d^{1+1/(2^{k-2}-1)})才能表示d个实数的最大值函数，这是首个关于该算子在深度k≥3时的无条件超线性下界。

Conclusion: 尽管最大值函数看似简单，但其不可微分超平面的几何结构带来了固有的复杂性，这为证明深度神经网络下界提供了新颖的方法，揭示了深度神经网络表示能力的限制。
...
### [249] [Unveiling the Heart-Brain Connection: An Analysis of ECG in Cognitive Performance](https://arxiv.org/abs/2601.01424)
*Akshay Sasi,Malavika Pradeep,Nusaibah Farrukh,Rahul Venugopal,Elizabeth Sherly*

Main category: cs.LG

TL;DR: 该研究探索使用心电图（ECG）替代脑电图（EEG）监测认知负荷，提出跨模态XGBoost框架将ECG特征映射到EEG认知空间，实现仅用ECG进行认知负荷分类。


<details>
  <summary>Details</summary>
Motivation: 虽然EEG是评估心理负荷的金标准，但其便携性有限限制了实际应用。广泛可用的可穿戴设备ECG提供了一种实用的替代方案，研究探索ECG信号是否能可靠反映认知负荷并作为EEG指标的代理。
...
Conclusion: ECG可以作为EEG的替代方案，用于可靠监测认知负荷，为日常认知监测提供便携、实用的解决方案。
...
### [250] [Bayesian Subspace Gradient Estimation for Zeroth-Order Optimization of Large Language Models](https://arxiv.org/abs/2601.01452)
*Jian Feng,Zhihong Huang*

Main category: cs.LG

TL;DR: BSZO是一种贝叶斯子空间零阶优化方法，通过卡尔曼滤波结合多个扰动方向的有限差分信息，相比传统ZO方法提高了收敛速度，在保持接近推理基线内存使用的同时，在多个LLM上实现了更好的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的零阶优化方法依赖随机扰动的一步梯度估计，限制了优化效率。需要一种能够结合多个扰动方向信息、提高收敛速度的零阶优化方法。

Method: 提出贝叶斯子空间零阶优化(BSZO)，将每个有限差分测量视为噪声观测，通过卡尔曼滤波建立投影梯度的后验分布，并使用基于残差的自适应机制调整扰动尺度。

Result: 理论分析显示BSZO比标准ZO方法收敛速度提高k/γ倍。在RoBERTa、Mistral和OPT模型上的实验表明，BSZO优于MeZO、MeZO-Adam和HiZOO，在OPT-13B上实现最高6.67%的绝对平均提升，内存使用接近推理基线（MeZO的1.00-1.08倍）。

Conclusion: BSZO通过贝叶斯推理有效结合多个扰动方向信息，显著提高了零阶优化的收敛速度和性能，同时保持了低内存优势，为大语言模型的高效微调提供了有效解决方案。
...
### [251] [Leveraging Flatness to Improve Information-Theoretic Generalization Bounds for SGD](https://arxiv.org/abs/2601.01465)
...
<details>
  <summary>Details</summary>
Motivation: 现有信息论泛化界虽然理论上依赖于数据和算法特性，但未能有效捕捉SGD平坦性偏好对泛化的改善作用，且在数值上较为宽松，需要改进。

Method: 提出"全知轨迹"技术，推导出更充分利用平坦性的信息论泛化界，该界表明当最终权重协方差的大方差方向在损失景观中具有小局部曲率时，模型泛化更好。

Result: 实验表明新界不仅正确反映平坦性改善时的泛化提升，数值上也更紧；应用于凸-Lipschitz-有界问题的梯度下降极小化超额风险时，将代表性信息论界的Ω(1)率改进为O(1/√n)。

Conclusion: 新信息论泛化界成功利用了SGD的平坦性偏好，提供了更紧的泛化保证，并暗示了可以绕过记忆化-泛化的权衡。
...
### [252] [Accelerating Storage-Based Training for Graph Neural Networks](https://arxiv.org/abs/2601.01473)
*Myung-Hwan Jang,Jeong-Min Park,Yunyong Ko,Sang-Wook Kim*

Main category: cs.LG

TL;DR: AGNES是一个基于存储的GNN训练框架，通过块级存储I/O处理和超批次处理来解决大规模图训练中的I/O瓶颈问题，相比现有方法提升高达4.1倍性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于存储的GNN训练方法在处理大规模图时面临严重的数据准备瓶颈，主要问题是如何处理大量的小型存储I/O操作，这限制了训练效率。

Method: AGNES采用块级存储I/O处理来充分利用高性能存储设备的I/O带宽，并结合超批次处理策略，基于真实世界图的特性进一步优化每个存储I/O的效率。

Result: 在五个真实世界图数据集上的综合实验表明，AGNES始终优于四种最先进的方法，比最佳竞争对手快达4.1倍。

Conclusion: AGNES通过创新的存储I/O优化策略，有效解决了大规模图神经网络训练中的I/O瓶颈问题，为单机处理网络规模图提供了高效解决方案。
...
### [253] [Multi-Subspace Multi-Modal Modeling for Diffusion Models: Estimation, Convergence and Mixture of Experts](https://arxiv.org/abs/2601.01475)
*Ruofeng Yang,Yongcan Li,Bo Jiang,Cheng Chen,Shuai Li*

Main category: cs.LG

TL;DR: 本文提出MoLR-MoG建模方法，将数据建模为K个线性子空间的并集，每个子空间采用混合高斯隐变量，以解决扩散模型在高维数据中的维度诅咒问题，并解释为什么扩散模型只需少量训练样本就能获得良好性能。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在小数据集上表现良好，但估计误差受维度诅咒影响（n^{-1/D}）。现有工作将数据建模为高斯隐变量的线性子空间并集，虽然反映了多流形特性，但高斯隐变量无法捕捉隐流形的多模态特性。

Method: 提出MoLR-MoG（混合子空间的低秩混合高斯）建模：将目标数据建模为K个线性子空间的并集，每个子空间采用混合高斯隐变量（n_k个模态，维度d_k）。对应的得分函数具有混合专家结构，能捕捉多模态信息并包含非线性特性。

Result: 1. 真实世界实验显示MoE隐变量MoG神经网络的生成结果远优于MoE隐变量高斯得分；2. MoE隐变量MoG神经网络与参数多10倍的MoE隐变量Unet性能相当；3. 理论分析得到R^4√(Σn_k)√(Σn_kd_k)/√n的估计误差，通过数据结构避免了维度诅咒；4. 证明了在MoLR-MoG建模下的优化收敛保证。

Conclusion: MoLR-MoG建模合理且适用于真实世界数据，解释了为什么扩散模型只需少量训练样本和快速优化过程就能获得优异性能。该工作为理解扩散模型在接近真实世界数据设置下的成功提供了理论解释。
...
### [254] [SGD-Based Knowledge Distillation with Bayesian Teachers: Theory and Guidelines](https://arxiv.org/abs/2601.01484)
*Itai Morad,Nir Shlezinger,Yonina C. Eldar*

Main category: cs.LG

TL;DR: 该论文从贝叶斯视角分析知识蒸馏，证明从贝叶斯分类概率学习能减少方差、提升收敛稳定性，并实验验证贝叶斯教师模型能让学生获得更高准确率


<details>
  <summary>Details</summary>
Motivation: 知识蒸馏在应用中表现出色，但其理论基础尚未完全理解。作者希望从贝叶斯角度分析知识蒸馏的收敛行为，特别是研究从贝叶斯分类概率学习与从噪声近似学习的差异

Method: 采用贝叶斯视角分析知识蒸馏，研究两种监督方式：1）教师提供精确的贝叶斯分类概率；2）监督使用噪声近似的贝叶斯分类概率。分析使用随机梯度下降训练学生的收敛行为，并实验验证贝叶斯教师模型的效果

Result: 分析表明：从贝叶斯分类概率学习能减少方差，移除收敛边界中的邻域项；噪声水平影响泛化和准确性。实验证明：从贝叶斯教师蒸馏的学生准确率最高提升4.27%，收敛稳定性提升30%（噪声减少）

Conclusion: 贝叶斯深度学习模型能提供更好的贝叶斯分类概率估计，作为知识蒸馏的教师模型能让学生获得更高准确率和更稳定的收敛。这为知识蒸馏提供了理论支持，并指出了贝叶斯模型作为教师的优势
...
### [255] [Accelerating Decentralized Optimization via Overlapping Local Steps](https://arxiv.org/abs/2601.01493)
*Yijie Zhou,Shi Pu*
...
<details>
  <summary>Details</summary>
Motivation: 去中心化优化已成为分布式学习的关键范式，能够实现可扩展训练并保护数据隐私。然而，现有方法由于节点间频繁同步而面临通信瓶颈问题，需要减少通信导致的训练停滞。

Method: 提出重叠本地去中心化SGD（OLDSGD），通过精心设计的更新机制实现计算与通信的重叠，避免通信引起的停滞，同时保持与Local SGD相同的平均更新。

Result: 理论分析建立了光滑非凸目标的非渐近收敛率，表明OLDSGD保持与标准Local Decentralized SGD相同的迭代复杂度，但改善了每次迭代的运行时间。实证结果显示在不同通信延迟下，OLDSGD在挂钟时间收敛方面有持续改进。

Conclusion: OLDSGD通过对现有框架的最小修改，提供了一种在不牺牲理论保证的情况下加速去中心化学习的实用解决方案。
...
### [256] [Advanced Global Wildfire Activity Modeling with Hierarchical Graph ODE](https://arxiv.org/abs/2601.01501)
*Fan Xu,Wei Gong,Hao Wu,Lilan Peng,Nan Wang,Qingsong Wen,Xian Wu,Kun Wang,Xibin Zhao*

Main category: cs.LG

TL;DR: HiGO：一种用于全球野火行为预测的多尺度连续时间深度学习框架，通过层次图结构和神经ODE模块显著提升长期预测性能。
...
Method: 提出层次图ODE（HiGO）框架：1）将地球系统表示为多层图层次结构；2）提出自适应滤波消息传递机制，用于层内和层间信息流；3）在多个层次上集成GNN参数化的神经ODE模块，显式学习每个尺度的连续动力学。

Result: 在SeasFire Cube数据集上的广泛实验表明，HiGO在长期野火预测方面显著优于最先进的基线方法。其连续时间预测表现出很强的观测一致性，突显了实际应用的潜力。

Conclusion: HiGO框架成功地将多尺度连续时间动力学建模应用于全球野火预测，为这一关键但具有挑战性的任务提供了有效的深度学习解决方案，具有重要的实际应用价值。
...
### [257] [Utilizing Earth Foundation Models to Enhance the Simulation Performance of Hydrological Models with AlphaEarth Embeddings](https://arxiv.org/abs/2601.01558)
*Pengfei Qu,Wenyu Ouyang,Chi Zhang,Yikai Chai,Shuolong Xu,Lei Ye,Yongri Piao,Miao Zhang,Huchuan Lu*

Main category: cs.LG

TL;DR: 卫星图像嵌入特征比传统流域属性更能有效预测无资料地区径流，基于嵌入相似性选择捐赠流域可提高预测精度


<details>
  <summary>Details</summary>
Motivation: 传统流域属性无法完全捕捉自然环境的复杂性，需要更有效的方法来描述流域特征以预测无资料地区的径流

Method: 使用AlphaEarth Foundation嵌入（从大量卫星图像学习得到）作为流域特征表示，并研究基于嵌入相似性选择捐赠流域对预测精度的影响

Result: 基于卫星图像嵌入的模型在未参与训练的流域上预测精度更高，表明这些嵌入能更有效地捕捉关键物理差异；基于嵌入相似性选择捐赠流域可提高预测性能

Conclusion: 卫星图像提供的环境表征能增强水文预报能力，支持开发更适应不同景观的水文模型
...
### [258] [The Two-Stage Decision-Sampling Hypothesis: Understanding the Emergence of Self-Reflection in RL-Trained LLMs](https://arxiv.org/abs/2601.01580)
*Zibo Zhao,Yuanting Zha,Haipeng Zhang,Xingcheng Xu*

Main category: cs.LG

TL;DR: 该论文通过梯度归因属性分析RL后训练中自我反思能力的机制，提出两阶段决策-采样假说，解释RL相比SFT在自我修正能力上的优势。


<details>
  <summary>Details</summary>
Motivation: 尽管RL后训练能让大语言模型产生自我反思能力，但统一的优化目标如何同时产生生成解决方案和评估何时修订这两种功能上不同的能力，其机制尚不明确。

Method: 引入梯度归因属性来表征奖励梯度在策略组件中的分布，形式化为两阶段决策-采样假说，将策略分解为用于生成的采样策略和用于验证的决策策略。通过理论证明和算术推理任务的实证验证。

Result: 证明替代奖励表现出平衡梯度归因，而SFT和KL惩罚表现出不平衡梯度归因，长度加权创建了不对称正则化，约束采样策略而让决策策略优化不足。实证验证RL的优越泛化主要来自改进的决策能力而非采样能力。

Conclusion: 为思维模型中自我修正能力提供了基于第一性原理的机制性解释，阐明了RL成功而SFT失败的理论原因，揭示了自我反思能力涌现的梯度归因机制。
...
### [259] [REE-TTT: Highly Adaptive Radar Echo Extrapolation Based on Test-Time Training](https://arxiv.org/abs/2601.01605)
*Xin Di,Xinglin Piao,Fei Wang,Guodong Jing,Yong Zhang*

Main category: cs.LG

TL;DR: 提出REE-TTT模型，通过时空测试时训练机制增强雷达回波外推的泛化能力，解决传统方法在跨区域和极端天气场景下的局限性


<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法依赖高质量本地训练数据和静态参数，导致泛化能力差，难以适应不同区域和极端天气事件

Method: 提出REE-TTT模型，引入自适应测试时训练机制，设计时空测试时训练块，用任务特定注意力机制替代标准线性投影

Result: 在跨区域极端降水场景实验中，REE-TTT在预测精度和泛化能力上显著优于现有基准模型

Conclusion: REE-TTT通过测试时训练机制有效适应非平稳气象分布，显著提升降水特征表示，增强了雷达回波外推的泛化能力
...
### [260] [Real Time NILM Based Power Monitoring of Identical Induction Motors Representing Cutting Machines in Textile Industry](https://arxiv.org/abs/2601.01616)
*Md Istiauk Hossain Rifat,Moin Khan,Mohammad Zunaed*

Main category: cs.LG

TL;DR: 本文针对孟加拉国纺织行业能耗高、监控落后的问题，提出了基于非侵入式负载监控(NILM)的实时工业监测框架，重点关注纺织切割机等相同电机负载，开发了硬件系统并创建了新数据集，评估了MATNILM模型在工业环境中的性能。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国纺织行业是能源密集型产业，但监控实践落后，导致电力使用效率低下和运营成本高昂。需要开发实时监控解决方案来提高能效。

Method: 开发了包含电压电流传感器、Arduino Mega和ESP8266的硬件系统，采集总负载和单个负载数据；创建了包含三个相同感应电机和辅助负载的新数据集（超过18万个样本）；评估了最先进的MATNILM模型在工业条件下的性能；实现了通过Blynk应用的远程访问实时监控系统。

Result: 总能量估计相对准确，但单个设备分解在多个相同机器同时运行时面临困难；集成系统展示了实用的实时监控能力，可通过Blynk应用远程访问。

Conclusion: NILM在工业应用中既有潜力也有局限性，未来改进方向包括更高频率的数据采集、更大规模的数据集以及处理相同负载的先进深度学习方法。
...
### [261] [Communication-Efficient Federated AUC Maximization with Cyclic Client Participation](https://arxiv.org/abs/2601.01649)
*Umesh Vangapally,Wenhan Wu,Chen Chen,Zhishuai Guo*

Main category: cs.LG

TL;DR: 本文针对联邦学习中客户端周期性参与的场景，提出了高效的联邦AUC最大化算法，在平方代理损失下达到最优通信复杂度，在一般成对损失下也显著提升了效率。


<details>
  <summary>Details</summary>
Motivation: 现有联邦AUC最大化方法通常假设客户端完全可用，但实际联邦学习系统中客户端往往以周期性方式参与训练。这种周期性参与对不可分解的AUC目标带来了独特的优化挑战，需要开发专门的算法来解决。

Method: 研究了两种设置：1) 使用平方代理损失的AUC最大化，将其重构为非凸-强凹极小极大优化问题，利用Polyak-Łojasiewicz条件；2) 一般成对AUC损失。针对周期性客户端参与场景，设计了通信高效的算法。

Result: 在平方代理损失下实现了最优的通信复杂度$\widetilde{O}(1/ε^{1/2})$和迭代复杂度$\widetilde{O}(1/ε)$；在一般成对损失下实现了通信复杂度$O(1/ε^3)$和迭代复杂度$O(1/ε^4)$，在PL条件下提升至$\widetilde{O}(1/ε^{1/2})$和$\widetilde{O}(1/ε)$。

Conclusion: 本文提出的方法在图像分类、医学影像和欺诈检测等基准任务上表现出优越的效率和效果，为周期性参与场景下的联邦AUC最大化提供了有效的解决方案。
...
### [262] [Learning Resilient Elections with Adversarial GNNs](https://arxiv.org/abs/2601.01653)
*Hao Xiang Li,Yash Shah,Lorenzo Giusti*

Main category: cs.LG

TL;DR: 本文提出使用图神经网络和对抗训练来学习更具表达能力和鲁棒性的投票规则，以解决传统投票系统在战略投票下的脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 传统投票规则难以同时满足所有理想场景，特别是在面对策略性投票时缺乏鲁棒性。虽然自动化机制设计和集合不变架构为建模选举系统提供了新思路，但现有方法在现实世界应用仍存在限制。

Method: 使用二分图表示选举，采用图神经网络学习投票规则，并结合对抗训练来提高投票规则在策略性投票下的鲁棒性，同时最大化社会福利。

Result: 在合成和真实数据集上的评估表明，该方法解决了先前工作中学习投票规则的关键限制，通过二分图表示和GNN学习显著提升了投票规则的表达能力和鲁棒性。

Conclusion: 该方法为机器学习应用于现实世界选举开辟了新前沿，通过改进的神经网络架构和对抗训练，能够学习出更具表达能力和鲁棒性的投票规则。
...
### [263] [Length-Aware Adversarial Training for Variable-Length Trajectories: Digital Twins for Mall Shopper Paths](https://arxiv.org/abs/2601.01663)
*He Sun,Jiwoong Shin,Ravi Dhar*

Main category: cs.LG

TL;DR: 提出长度感知采样(LAS)方法，通过按长度分组轨迹来减少批次内长度异质性，改善生成模型对轨迹衍生统计量的分布匹配
...
Result: LAS在多商场购物者轨迹数据集和多种公共序列数据集(GPS、教育、电子商务、电影)上一致改善衍生变量分布的匹配，在数据集特定指标上优于随机采样

Conclusion: LAS是一种简单有效的批处理策略，通过减少批次内长度异质性来改善生成模型对轨迹衍生统计量的分布匹配，无需改变模型架构
...
### [264] [Who is the Winning Algorithm? Rank Aggregation for Comparative Studies](https://arxiv.org/abs/2601.01664)
*Amichai Painsky*

Main category: cs.LG

TL;DR: 提出新框架，利用算法在多个数据集上的完整排名信息（不只是胜场数）来估计每个算法在未来数据集上的获胜概率。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只统计算法在基准数据集上的胜场数来预测未来表现，但忽略了完整排名信息（如第二名、第三名等），这些信息可能包含更多有价值的信息。

Method: 引入新的概念框架，利用算法在多个数据集上的完整排名分布（而不仅仅是胜场数）来估计每个算法在未来数据集上的获胜概率。

Result: 提出的框架在合成数据和真实世界示例中显著优于现有已知方法。

Conclusion: 利用算法的完整排名信息比仅使用胜场数能更准确地预测算法在未来数据集上的获胜概率，新框架为此提供了有效方法。
...
### [265] [Adversarial Instance Generation and Robust Training for Neural Combinatorial Optimization with Multiple Objectives](https://arxiv.org/abs/2601.01665)
*Wei Liu,Yaoxin Wu,Yingqian Zhang,Thomas Bäck,Yingjie Fan*

Main category: cs.LG

TL;DR: 提出一个面向鲁棒性的偏好条件深度强化学习求解器框架，包含偏好对抗攻击生成困难实例和硬度感知偏好选择防御策略，提升多目标组合优化问题求解器的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于学习的多目标组合优化问题求解器鲁棒性研究不足，特别是在多样复杂问题分布下的表现。需要系统评估和提升这些求解器在面对困难实例和分布外数据时的性能。

Method: 1) 提出统一鲁棒性框架；2) 开发基于偏好的对抗攻击方法，生成暴露求解器弱点的困难实例；3) 引入防御策略，将硬度感知偏好选择融入对抗训练，减少对受限偏好区域的过拟合。

Result: 在MOTSP、MOCVRP和MOKP三个问题上验证：攻击方法成功为不同求解器生成困难实例；防御方法显著增强神经求解器的鲁棒性和泛化能力，在困难实例和分布外实例上表现优异。

Conclusion: 该框架为评估和提升多目标组合优化问题求解器的鲁棒性提供了系统方法，攻击和防御策略的结合有效解决了学习型求解器在复杂分布下的性能问题。
...
### [266] [HeurekaBench: A Benchmarking Framework for AI Co-scientist](https://arxiv.org/abs/2601.01678)
*Siba Smarak Panigrahi,Jovana Videnović,Maria Brbić*

Main category: cs.LG

TL;DR: HeurekaBench是一个用于评估科学智能体系统的基准框架，通过半自动化流程创建基于真实科学研究的开放式研究问题，并在单细胞生物学领域实例化为sc-HeurekaBench，用于评估和优化智能体设计。


<details>
  <summary>Details</summary>
Motivation: 当前评估LLM驱动的科学智能体系统面临挑战，需要能够整合数据分析、解释和从实验数据生成新见解的真实端到端研究场景。现有基准缺乏这种综合性的评估框架。

Method: 开发了HeurekaBench框架，采用半自动化流程：基于科学研究和对应代码仓库创建开放式研究问题，利用多个LLM提取见解并生成候选工作流，然后与报告发现进行验证。在单细胞生物学领域实例化为sc-HeurekaBench基准。

Result: 使用sc-HeurekaBench比较了最先进的单细胞智能体，发现添加批评模块可以将开源LLM智能体的不良响应改善高达22%，缩小与闭源智能体的差距。基准有助于定量分析智能体系统的设计选择。

Conclusion: HeurekaBench为科学智能体提供了严格的端到端评估路径，将基准构建基于真实的科学工作流程，推动了科学智能体系统的评估和发展。
...
### [267] [DiMEx: Breaking the Cold Start Barrier in Data-Free Model Extraction via Latent Diffusion Priors](https://arxiv.org/abs/2601.01688)
*Yash Thesia,Meera Suthar*

Main category: cs.LG

TL;DR: DiMEx框架利用预训练潜在扩散模型的语义先验，通过随机嵌入贝叶斯优化在潜在空间中合成高保真查询，解决了数据无关模型窃取中的"冷启动"问题，显著提升了攻击效率；同时提出了混合状态集成防御来检测这种攻击的独特优化轨迹。


<details>
  <summary>Details</summary>
Motivation: 模型窃取攻击对机器学习即服务构成严重威胁，而数据无关模型提取虽然隐蔽，但受限于"冷启动"问题：基于GAN的攻击者需要数千次查询才能从随机噪声收敛到有意义的数据。本文旨在解决这一初始化障碍。

Method: 提出DiMEx框架，利用预训练潜在扩散模型的丰富语义先验，在生成器的潜在空间中使用随机嵌入贝叶斯优化，立即合成高保真查询；同时提出混合状态集成防御来检测这种攻击的独特"优化轨迹"。

Result: DiMEx在SVHN数据集上仅用2000次查询就达到52.1%的协议率，比最先进的GAN基线高出16%以上；而混合状态集成防御能够将攻击成功率抑制到21.6%，且延迟可忽略不计。

Conclusion: DiMEx通过利用潜在扩散模型的语义先验有效解决了数据无关模型窃取的冷启动问题，同时提出的混合状态集成防御能够有效检测和防御这种高度语义化的攻击，为MLaaS安全提供了新的解决方案。
...
### [268] [Enhanced Multi-model Online Conformal Prediction](https://arxiv.org/abs/2601.01692)
*Erfan Hajihashemi,Yanning Shen*

Main category: cs.LG

TL;DR: 提出一种新颖的多模型在线共形预测算法，通过构建二分图选择有效模型子集，降低计算复杂度并提升预测效率


<details>
  <summary>Details</summary>
Motivation: 传统共形预测依赖单一固定模型，在在线环境中可能表现不稳定；现有多模型选择方法计算成本高，且候选集中性能差的模型会影响效果

Method: 开发多模型在线共形预测算法，在每个时间步生成二分图识别有效模型子集，从中选择模型构建预测集

Result: 实验表明该方法在预测集大小和计算效率方面均优于现有多模型共形预测技术

Conclusion: 提出的算法能有效解决多模型共形预测中的计算复杂度和效率问题，在在线环境中具有更好的适应性
...
### [269] [Digital Twin-Driven Communication-Efficient Federated Anomaly Detection for Industrial IoT](https://arxiv.org/abs/2601.01701)
*Mohammed Ayalew Belay,Adil Rasheed,Pierluigi Salvo Rossi*

Main category: cs.LG

TL;DR: 提出数字孪生集成联邦学习(DTFL)方法，通过五种新颖方法解决工业异常检测中的数据隐私、通信效率和标注数据有限等问题，显著提升收敛速度和通信效率。


<details>
  <summary>Details</summary>
Motivation: 工业异常检测面临依赖真实传感器数据、标注数据有限、高误报率和隐私问题等挑战，需要既能保护数据隐私又能提高通信效率的解决方案。

Method: 提出五种数字孪生集成联邦学习方法：DTML(数字孪生元学习)、FPF(联邦参数融合)、LPE(层间参数交换)、CWA(循环权重适应)和DTKD(数字孪生知识蒸馏)，通过合成与真实知识结合平衡泛化与通信开销。

Result: 在公开网络物理异常检测数据集上，CWA在33轮达到80%准确率目标，FPF需41轮，LPE需48轮，DTML需87轮，而标准FedAvg和DTKD在100轮内未达标，通信效率提升最高达62%。

Conclusion: 数字孪生知识集成到联邦学习中能显著加速收敛到操作上有意义的准确率阈值，为工业物联网异常检测提供通信高效且隐私保护的解决方案。
...
### [270] [Entropy-Aligned Decoding of LMs for Better Writing and Reasoning](https://arxiv.org/abs/2601.01714)
*Kareem Ahmed,Sameer Singh*

Main category: cs.LG

TL;DR: EPIC是一种无需超参数的解码方法，通过将未来轨迹的熵纳入语言模型解码，调节生成过程中的不确定性，使采样分布的熵与数据不确定性对齐。


<details>
  <summary>Details</summary>
Motivation: 传统语言模型解码算法依赖贪婪启发式方法，导致短视扭曲，产生同质化、重复且不连贯的句子。需要一种能更好捕捉语言分布不确定性的解码方法。

Method: EPIC通过熵感知懒惰Gumbel-Max采样，将未来轨迹的熵明确纳入解码过程，使采样分布的熵与数据的不确定性对齐。该方法精确且高效，每步仅需次线性数量的熵评估。

Result: 在创意写作和摘要任务中，EPIC在LM-as-judge偏好胜率上持续优于广泛使用的解码策略。自动指标显示EPIC产生更多样化的生成和更忠实的摘要。在数学推理任务中，EPIC也优于所有基线方法。

Conclusion: EPIC通过显式调节生成过程中的不确定性，产生与底层数据分布熵对齐的采样分布，在各种任务中优于现有解码方法，提供更高质量、更多样化的语言生成。
...
### [271] [Context-Free Recognition with Transformers](https://arxiv.org/abs/2601.01754)
*Selim Jerad,Anej Svete,Sophie Hao,Ryan Cotterell,William Merrill*

Main category: cs.LG

TL;DR: 循环Transformer通过O(log n)循环层和O(n^6)填充token可以识别所有上下文无关语言，但对于自然子类如无歧义CFL，仅需O(n^3)填充，更实用。


<details>
  <summary>Details</summary>
Motivation: Transformer在处理自然语言和代码等语法结构良好的输入方面表现出色，但其处理语法结构的能力尚不清楚。现有研究表明标准Transformer无法识别上下文无关语言（CFL），甚至无法识别正则语言。虽然已有工作证明O(log n)循环层可使Transformer识别正则语言，但CFL识别问题仍未解决。

Method: 提出循环Transformer架构，使用O(log n)循环层和大量填充token。通过理论分析证明：1）使用O(n^6)填充token时，循环Transformer可以识别所有CFL；2）对于无歧义CFL等自然子类，仅需O(n^3)填充token。

Result: 理论上证明了循环Transformer能够识别所有CFL，但通用识别需要大量填充token（O(n^6)），可能不实用。对于无歧义CFL等自然子类，识别问题更易处理，仅需O(n^3)填充。实验验证了循环机制在需要对数深度的语言上的有效性。

Conclusion: CFL识别对Transformer具有复杂性：通用识别可能需要不切实际的大量填充，但自然约束（如无歧义性）可产生高效的识别算法。循环机制为Transformer处理语法结构提供了理论可能性。
...
### [272] [UnPII: Unlearning Personally Identifiable Information with Quantifiable Exposure Risk](https://arxiv.org/abs/2601.01786)
*Intae Jeon,Yujeong Kwon,Hyungjoon Koo*

Main category: cs.LG

TL;DR: UnPII：首个基于PII风险的机器学习遗忘方法，通过PII风险指数（PRI）评估不同PII属性的隐私风险，实现差异化遗忘策略，提升模型性能同时满足合规要求。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在金融、医疗等关键领域的应用引发隐私担忧，GDPR等法规要求删除敏感个人信息。现有遗忘技术采用统一策略，无法区分不同PII属性的隐私风险和业务风险。

Method: 提出PII风险指数（PRI），综合可识别性、敏感性、可用性、可链接性、持久性、可暴露性和合规性七个风险维度。UnPII方法基于PRI优先级进行差异化遗忘，可与梯度上升、负偏好优化等现有遗忘算法无缝集成。

Result: 实验显示UnPII在准确性上提升达11.8%，效用提升达6.3%，泛化能力提升达12.4%，遗忘过程中的微调开销平均仅增加27.5%。构建了包含1700个PII实例的合成数据集用于评估。

Conclusion: UnPII是首个以PII为中心的遗忘方法，能够根据PII属性风险进行优先级遗忘，在提升模型性能的同时满足隐私合规要求，为实际应用提供了可行的解决方案。
...
### [273] [HyperCLOVA X 8B Omni](https://arxiv.org/abs/2601.01792)
*NAVER Cloud HyperCLOVA X Team*

Main category: cs.LG

TL;DR: HyperCLOVA X 8B Omni是首个支持文本、音频和视觉作为输入输出的全能模态模型，通过统一的多模态序列实现理解和生成，在韩语和英语中表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发一个统一的全能模态模型，避免使用分离的模态特定管道，实现文本、音频和视觉之间的任意到任意转换，为实用的全能助手提供8B规模的探索点。

Method: 通过共享的下一个token预测接口统一模态，使用视觉和音频编码器注入连续嵌入以实现细粒度理解和基础，在交错的多模态序列上进行训练。

Result: 在韩语和英语的文本、音频和视觉多种输入输出组合中，与类似规模模型相比表现出竞争力，验证了统一全能模态方法的有效性。

Conclusion: HyperCLOVA X 8B Omni作为首个全能模态模型，通过开放权重发布将支持广泛的研究和部署场景，是迈向实用全能助手的重要里程碑。
...
### [274] [Distributed Federated Learning by Alternating Periods of Training](https://arxiv.org/abs/2601.01793)
*Shamik Bhattacharyya,Rachel Kalpana Kalaimani*

Main category: cs.LG

TL;DR: 提出分布式联邦学习框架，通过多服务器架构解决传统联邦学习的可扩展性和容错性问题，设计DFL算法结合本地训练与服务器间全局训练，实现收敛到理想模型


<details>
  <summary>Details</summary>
Motivation: 传统联邦学习依赖单一中央服务器，在处理大量客户端时面临可扩展性挑战，且存在单点故障风险。需要解决这些关键限制，提高系统的可靠性和扩展性。

Method: 提出分布式联邦学习框架，包含多个具有服务器间通信能力的服务器，每个服务器关联一组不相交的客户端。设计DFL算法，采用本地训练（客户端数据）与全局训练（服务器间）交替进行的策略。

Result: 在适当参数选择下，DFL算法确保所有服务器收敛到共同模型值，且该值在理想模型的小容忍范围内，有效整合了本地和全局训练模型。通过数值模拟验证了理论主张。

Conclusion: 提出的分布式联邦学习框架成功解决了传统联邦学习的可扩展性和容错性限制，通过多服务器架构和DFL算法实现了高效、可靠的分布式学习系统。
...
### [275] [Sparse Threats, Focused Defense: Criticality-Aware Robust Reinforcement Learning for Safe Autonomous Driving](https://arxiv.org/abs/2601.01800)
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗训练方法通常将自动驾驶代理与对手的交互建模为零和博弈，忽略了双方的不对称性和安全关键风险的稀疏性，导致在实际自动驾驶场景中鲁棒性不足。

Method: 提出CARRL框架，包含两个交互组件：风险暴露对手（REA）和风险目标鲁棒代理（RTRA）。将交互建模为一般和博弈，REA专注于暴露安全关键故障（如碰撞），RTRA学习平衡安全与驾驶效率。REA采用解耦优化机制识别稀疏安全关键时刻，RTRA通过双回放缓冲池联合利用良性对抗经验，并强制扰动下的策略一致性。
...
### [276] [Moments Matter:Stabilizing Policy Optimization using Return Distributions](https://arxiv.org/abs/2601.01803)
*Dennis Jabs,Aditya Mohan,Marius Lindauer*

Main category: cs.LG

TL;DR: 提出一种基于分布评论家高阶矩的PPO改进方法，通过惩罚极端回报行为来稳定策略更新，在Walker2D任务中稳定性提升达75%


<details>
  <summary>Details</summary>
Motivation: 深度强化学习代理即使获得相同回合回报，其行为也可能因环境和算法因素而大不相同。在连续控制任务中，小的参数变化可能导致不稳定步态，这既影响算法比较也阻碍实际应用。虽然约束策略保持窄的回报分布能改善稳定性，但直接估计该分布在计算上昂贵。

Method: 通过分布评论家建模状态-动作回报分布，然后利用该分布的高阶矩（偏度和峰度）来偏置PPO的优势函数。通过惩罚极端尾部行为，阻止策略进入易引发不稳定的参数区域。

Result: 在Walker2D环境中，该方法将稳定性提升高达75%，同时保持可比较的评估回报。当更新后评论家值与更新后回报对齐不佳时，标准PPO难以产生窄的回报分布，而基于矩的修正能有效缩小该分布。

Conclusion: 利用环境随机性和分布评论家的高阶矩信息，可以有效缓解策略更新引起的不稳定性，为连续控制任务提供了一种计算高效的稳定性改进方法。
...
### [277] [RealPDEBench: A Benchmark for Complex Physical Systems with Real-World Data](https://arxiv.org/abs/2601.01829)
...
TL;DR: RealPDEBench是首个结合真实世界测量与配对数值模拟的科学机器学习基准，包含5个数据集、3个任务、8个指标和10个基线模型，旨在解决模拟与真实数据之间的差距问题。


<details>
  <summary>Details</summary>
Motivation: 当前科学机器学习模型主要依赖模拟数据进行训练和验证，缺乏昂贵的真实世界数据，这限制了模型开发、评估以及模拟到真实迁移的研究，需要建立连接模拟与真实数据的基准。

Method: 构建包含5个真实世界测量数据集及其配对模拟数据集的基准，定义3个任务（允许比较真实与模拟数据并开发桥接方法），设计8个评估指标（数据导向和物理导向），并评估10个代表性基线模型。

Result: 实验显示模拟数据与真实世界数据存在显著差异，同时发现使用模拟数据进行预训练能持续提高准确性和收敛性，为桥接模拟与真实差距提供了见解。

Conclusion: RealPDEBench通过提供真实世界数据洞察，推动科学机器学习向桥接模拟与真实差距和实际部署方向发展，相关基准、数据集和指南已开源。
...
### [278] [FAROS: Robust Federated Learning with Adaptive Scaling against Backdoor Attacks](https://arxiv.org/abs/2601.01833)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习面临后门攻击威胁，现有防御方法依赖固定参数，存在单点故障风险，难以应对复杂攻击者策略变化。

Method: 提出FAROS框架，包含自适应差分缩放机制（根据客户端梯度分散度动态调整防御灵敏度）和鲁棒核心集计算（选择高置信度客户端计算中心点）。

Result: 在多种数据集、模型和攻击场景下的实验表明，该方法在攻击成功率和主任务准确率上均优于现有防御方法。

Conclusion: FAROS通过动态自适应防御和鲁棒核心集机制，有效解决了现有联邦学习防御方法的局限性，提升了对抗后门攻击的能力。
...
### [279] [Tackling Resource-Constrained and Data-Heterogeneity in Federated Learning with Double-Weight Sparse Pack](https://arxiv.org/abs/2601.01840)
*Qiantao Yang,Liquan Chen,Mingfu Xue,Songze Li*

Main category: cs.LG

TL;DR: FedCSPACK：基于余弦稀疏化参数打包和双权重聚合的个性化联邦学习方法，在有限客户端资源下有效处理数据异构性


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法在处理数据异构性时，忽视了客户端通信带宽和计算能力的限制，无法在解决数据异构性和适应有限客户端资源之间取得有效平衡

Method: 提出FedCSPACK方法：1）客户端基于余弦相似度打包模型参数并选择贡献最大的参数包共享，减少带宽需求；2）基于共享参数包生成掩码矩阵，提高服务器上稀疏更新的对齐和聚合效率；3）在掩码中嵌入方向和分布距离权重，实现加权引导聚合机制

Result: 在四个数据集上使用十种最先进方法的广泛实验表明，FedCSPACK在保持高模型精度的同时，有效提高了通信和计算效率

Conclusion: FedCSPACK通过参数打包和双权重聚合机制，在有限客户端资源下有效缓解数据异构性对模型性能的影响，实现了通信、计算效率和模型精度之间的良好平衡
...
### [280] [High-Order Epistasis Detection Using Factorization Machine with Quadratic Optimization Annealing and MDR-Based Evaluation](https://arxiv.org/abs/2601.01860)
*Shuta Kikuchi,Shu Tanaka*

Main category: cs.LG

TL;DR: 提出基于因子分解机和二次优化退火的FMQA方法，用于高效检测高阶上位性相互作用，解决传统MDR方法计算爆炸问题


<details>
  <summary>Details</summary>
Motivation: 高阶上位性检测面临组合爆炸的计算挑战，传统MDR方法在处理大量位点或高阶相互作用时计算不可行

Method: 将上位性检测定义为黑盒优化问题，使用因子分解机结合二次优化退火（FMQA），以MDR计算的分类错误率作为目标函数

Result: 在模拟病例对照数据集上成功识别出预设的高阶上位性相互作用，在不同相互作用阶数和遗传位点数量下都能在有限迭代次数内找到真实上位性

Conclusion: 提出的FMQA方法对于高阶上位性检测既有效又计算高效，能够克服传统方法的计算限制
...
### [281] [Safety at One Shot: Patching Fine-Tuned LLMs with A Single Instance](https://arxiv.org/abs/2601.01887)
*Jiawen Zhang,Lipeng He,Kejia Chen,Jian Lou,Jian Liu,Xiaohu Yang,Ruoxi Jia*

Main category: cs.LG

TL;DR: 仅需单个安全示例即可完全恢复微调后LLM的安全对齐，无需牺牲性能且成本极低


<details>
  <summary>Details</summary>
Motivation: 传统方法需要大量安全样本或校准集，导致计算开销大且模型性能下降。研究发现安全对齐可以更高效地恢复

Method: 使用单个安全示例进行微调恢复，发现安全梯度具有低秩结构，解释高效修正的可能性

Result: 无论有害示例数量或模型大小，仅需单个安全示例即可在几个epoch内恢复安全对齐，且不影响模型性能

Conclusion: 安全对齐的恢复比传统认知更简单高效，安全梯度的低秩结构是这一现象的关键原因
...
### [282] [FedBiCross: A Bi-Level Optimization Framework to Tackle Non-IID Challenges in Data-Free One-Shot Federated Learning on Medical Data](https://arxiv.org/abs/2601.01901)
*Yuexuan Xia,Yinghao Zhang,Yalin Liu,Hong-Ning Dai,Yong Xia*

Main category: cs.LG

TL;DR: FedBiCross：针对非IID医疗数据的个性化单轮联邦学习框架，通过聚类、双层跨集群优化和个性化蒸馏解决预测冲突问题


<details>
  <summary>Details</summary>
Motivation: 现有单轮联邦学习方法在非IID数据下，将所有客户端预测聚合形成全局教师时，冲突预测会在平均过程中相互抵消，产生接近均匀分布的软标签，导致蒸馏监督信号弱化

Method: 三阶段框架：1) 基于模型输出相似性聚类客户端形成连贯子集成；2) 双层跨集群优化学习自适应权重，选择性利用有益跨集群知识同时抑制负迁移；3) 个性化蒸馏进行客户端特定适配

Result: 在四个医疗图像数据集上的实验表明，FedBiCross在不同非IID程度下均一致优于最先进的基线方法

Conclusion: FedBiCross通过聚类和选择性知识转移有效解决了非IID数据下单轮联邦学习的预测冲突问题，为隐私敏感的医疗应用提供了有效的个性化学习方案
...
### [283] [TT-FSI: Scalable Faithful Shapley Interactions via Tensor-Train](https://arxiv.org/abs/2601.01903)
*Ungsik Kim,Suwon Lee*

Main category: cs.LG

TL;DR: 提出TT-FSI方法，利用矩阵乘积算子(MPO)高效计算Faithful Shapley Interaction指数，实现指数级加速和内存优化


<details>
  <summary>Details</summary>
Motivation: FSI指数是唯一满足忠实性公理的Shapley交互指数，但现有计算方法需要O(d^ℓ·2^d)时间和O(4^d)内存，计算成本过高

Method: 利用FSI的代数结构，证明线性算子v↦FSI(v)具有TT秩为O(ℓd)的MPO表示，设计高效的扫描算法

Result: 实现O(ℓ²d³·2^d)时间和O(ℓd²)核心存储，相比基线加速280倍，相比SHAP-IQ加速85倍，内存减少290倍，可扩展到d=20(100万个联盟)

Conclusion: TT-FSI通过张量网络方法显著提升了FSI指数的计算效率，解决了高维交互分析的可扩展性问题
...
### [284] [Evaluating Feature Dependent Noise in Preference-based Reinforcement Learning](https://arxiv.org/abs/2601.01904)
*Yuxuan Li,Harshith Reddy Kethireddy,Srijita Das*

Main category: cs.LG

TL;DR: 本文研究了偏好强化学习中的特征依赖噪声问题，发现现有噪声鲁棒方法在某些特征依赖噪声场景下性能显著下降，而无需显式去噪的方法反而表现更好。


<details>
  <summary>Details</summary>
Motivation: 偏好强化学习中偏好通常带有不确定性和噪声，尤其是当偏好来自非完美教师时。现有研究大多关注检测噪声，但噪声类型有限且通常与观测无关。本文旨在研究更现实的特征依赖噪声问题。

Method: 提出了特征依赖噪声的形式化概念，包括轨迹特征噪声、轨迹相似性噪声、不确定性感知噪声和语言模型噪声等多种变体。在DMControl和Meta-world的复杂连续控制任务中评估这些噪声。

Result: 实验表明，在某些特征依赖噪声设置下，最先进的噪声鲁棒PbRL方法学习性能显著恶化，而无需显式去噪的PbRL方法在多数设置中反而优于噪声鲁棒方法。语言模型噪声表现出与特征依赖噪声相似的特征。

Conclusion: 特征依赖噪声对现有噪声鲁棒PbRL方法构成挑战，语言模型噪声模拟了现实人类偏好，需要进一步研究如何鲁棒地学习特征依赖噪声。
...
### [285] [Distorted Distributional Policy Evaluation for Offline Reinforcement Learning](https://arxiv.org/abs/2601.01917)
*Ryo Iwaki,Takayuki Osogami*

Main category: cs.LG

TL;DR: 提出了一种新的分位数扭曲方法，通过非均匀悲观主义改进离线分布强化学习，根据数据可用性调整保守程度，相比均匀悲观主义获得更好性能


<details>
  <summary>Details</summary>
Motivation: 分布强化学习在在线设置中表现出色，但在离线场景中性能有限。现有离线DRL方法采用均匀低估回报分位数的方法，导致过于保守的价值估计，阻碍了泛化和性能提升

Method: 引入分位数扭曲概念，通过基于支持数据可用性调整保守程度，实现非均匀悲观主义。该方法基于理论分析，并通过实验验证

Result: 该方法在实验中表现出改进的性能，相比均匀悲观主义方法有更好的表现

Conclusion: 分位数扭曲方法通过非均匀悲观主义有效解决了离线分布强化学习中的过度保守问题，提高了性能
...
### [286] [Theoretical Convergence of SMOTE-Generated Samples](https://arxiv.org/abs/2601.01927)
*Firuz Kamalov,Hana Sulieman,Witold Pedrycz*

Main category: cs.LG

TL;DR: 本文对SMOTE过采样方法进行了严格的理论分析，证明了其合成变量在概率上收敛于原始变量，在紧致条件下有更强的均值收敛，并发现较小的最近邻秩能加速收敛。


<details>
  <summary>Details</summary>
Motivation: 不平衡数据广泛影响机器学习应用，SMOTE是最流行的解决方法之一，但需要从理论和实证两方面进行验证。本文旨在为SMOTE提供严格的理论分析基础。

Method: 对SMOTE的收敛性质进行理论分析，包括证明合成随机变量Z在概率上收敛于基础随机变量X，在X紧致时证明更强的均值收敛，并分析最近邻秩对收敛速度的影响。

Result: 理论证明：1) 合成变量Z依概率收敛于原始变量X；2) 当X紧致时，有更强的均值收敛；3) 较小的最近邻秩能带来更快的收敛速度。数值实验使用真实和合成数据验证了理论结果。

Conclusion: 本文为SMOTE提供了理论基础，增强了数据增强技术的理解，不仅适用于不平衡数据场景，也为实践者提供了可操作的指导（如选择较小的最近邻秩）。
...
### [287] [DéjàQ: Open-Ended Evolution of Diverse, Learnable and Verifiable Problems](https://arxiv.org/abs/2601.01931)
*Willem Röpke,Samuel Coward,Andrei Lupu,Thomas Foster,Tim Rocktäschel,Jakob Foerster*

Main category: cs.LG

TL;DR: DéjàQ是一个通过进化合成数学问题与模型训练同步进行的框架，使用LLM驱动的突变策略动态调整训练数据，以提升数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前推理模型大多依赖静态数据集，这可能导致记忆而非泛化，限制了模型的真正推理能力发展。

Method: 提出DéjàQ框架，在训练过程中联合进化多样化的合成数学问题集，采用两种LLM驱动的突变策略：改变上下文细节或直接修改问题结构。

Result: 模型能够生成新颖且有意义的数学问题，LLM驱动的突变策略改善了强化学习训练效果，同时分析了生成问题的有效性和计算开销。

Conclusion: 动态进化训练数据有潜力增强数学推理能力，具有更广泛的应用前景，作者将开源代码以支持进一步研究。
...
### [288] [SynRXN: An Open Benchmark and Curated Dataset for Computational Reaction Modeling](https://arxiv.org/abs/2601.01943)
*Tieu-Long Phan,Nhu-Ngoc Nguyen Song,Peter F. Stadler*

Main category: cs.LG

TL;DR: SynRXN是一个用于计算机辅助合成规划的统一基准框架和开放数据资源，将端到端合成规划分解为五个任务族，提供标准化数据集和评估工作流程。
...
Method: 将合成规划分解为五个任务族：反应平衡、原子到原子映射、反应分类、反应性质预测和合成路线设计。从异构公共来源收集反应数据，进行统一表示，提供版本化数据集、透明分割函数和标准化评估工作流程。

Result: 创建了一个包含版本化数据集、透明分割策略、标准化评估指标和可重复构建流程的完整基准框架，支持公平的方法比较和严格的消融实验。

Conclusion: SynRXN通过消除数据集异质性并提供透明可重用的评估框架，支持计算机辅助合成规划方法的公平纵向比较，降低实践者获取稳健可比性能评估的门槛。
...
### [289] [Refinement Provenance Inference: Detecting LLM-Refined Training Prompts from Model Behavior](https://arxiv.org/abs/2601.01966)
*Bo Yin,Qi Li,Runpeng Yu,Xinchao Wang*

Main category: cs.LG

TL;DR: 论文提出RePro方法，通过分析教师强制token分布和logit排序信号，推断微调模型是否使用原始提示或LLM优化版本进行训练，解决混合训练数据中的来源推断问题。


<details>
  <summary>Details</summary>
Motivation: 指令微调越来越多依赖LLM优化提示，但训练语料中原始提示和优化版本混合，难以确定模型具体训练于哪种版本。这在数据集治理和争议解决中很重要，但现有方法难以泛化到不同模型和训练设置。

Method: 提出RePro框架：1) 利用教师强制token分布变化和logit排序信号作为特征；2) 通过影子微调学习可迁移表示；3) 使用轻量级线性头在未见受害者模型上推断来源，无需训练数据访问。

Result: RePro在不同优化器和设置下都取得强性能，并能良好迁移，表明它利用了优化器无关的分布偏移而非重写风格伪影。

Conclusion: 提示优化会产生稳定可检测的token分布偏移，RePro框架能有效解决优化来源推断问题，为数据集治理和争议解决提供实用工具。
...
### [290] [SerpentFlow: Generative Unpaired Domain Alignment via Shared-Structure Decomposition](https://arxiv.org/abs/2601.01979)
*Julie Keisler,Anastase Alexandre Charantonis,Yannig Goude,Boutheina Oueslati,Claire Monteleoni*

Main category: cs.LG

TL;DR: SerpentFlow：一种无配对域对齐生成框架，通过将数据分解为共享结构和域特定成分，构建合成训练对，实现条件生成模型在无配对场景下的应用。


<details>
  <summary>Details</summary>
Motivation: 在无配对观测的情况下进行域对齐具有挑战性，传统条件生成模型需要配对数据。本文旨在解决无配对域对齐问题，特别是在域间共享底层结构模式但具体实现不同的场景。

Method: 提出SerpentFlow框架：1）在潜在空间中将数据分解为共享成分和域特定成分；2）用随机噪声替换域特定成分，构建共享表示与目标域样本间的合成训练对；3）使用基于分类器的准则自动确定分离低频/高频的截止频率；4）采用Flow Matching作为生成管道，学习给定共享表示的目标域条件分布。

Result: 在合成图像、物理过程模拟和气候降尺度任务上的实验表明，该方法能有效重建与底层低频模式一致的高频结构，支持共享结构分解作为无配对域对齐的有效策略。

Conclusion: 共享结构分解是解决无配对域对齐问题的有效方法，SerpentFlow框架通过构建合成训练对，使传统需要配对数据的条件生成模型能够在无配对场景下应用，特别适用于超分辨率等任务。
...
### [291] [Prior Diffusiveness and Regret in the Linear-Gaussian Bandit](https://arxiv.org/abs/2601.02022)
*Yifan Zhu,John C. Duchi,Benjamin Van Roy*

Main category: cs.LG

TL;DR: 本文证明了Thompson采样在线性高斯赌博机中具有$\tilde{O}(σd \sqrt{T} + d r \sqrt{\mathrm{Tr}(Σ_0)})$的贝叶斯遗憾界，其中先验依赖的"预热"项与极小极大遗憾项是加性分离的，而非现有文献中的乘性关系。


<details>
  <summary>Details</summary>
Motivation: 现有Thompson采样在线性赌博机中的遗憾界通常包含先验分布参数与极小极大遗憾项的乘性依赖关系。本文旨在证明这两个项可以加性分离，从而更精确地刻画Thompson采样的性能。

Method: 通过新的"椭圆势能"引理来分析Thompson采样的遗憾界，并提供了下界证明表明预热项是不可避免的。

Result: 证明了Thompson采样具有$\tilde{O}(σd \sqrt{T} + d r \sqrt{\mathrm{Tr}(Σ_0)})$的贝叶斯遗憾界，其中预热项$d r \sqrt{\mathrm{Tr}(Σ_0)}$与长期遗憾项$σd \sqrt{T}$是加性分离的。

Conclusion: Thompson采样在线性高斯赌博机中的遗憾可以分解为加性的预热项和极小极大遗憾项，这一结构比现有乘性依赖的界更精确，且下界证明表明预热项是不可避免的。
...
### [292] [Output Embedding Centering for Stable LLM Pretraining](https://arxiv.org/abs/2601.02031)
*Felix Stollenwerk,Anna Lokrantz,Niclas Hertzberg*

Main category: cs.LG

TL;DR: 提出输出嵌入中心化(OEC)方法解决大语言模型预训练中的输出logit发散问题，包括μ-centering和μ-loss两种实现方式，相比z-loss在训练稳定性和学习率敏感性方面表现更好。


<details>
  <summary>Details</summary>
Motivation: 大语言模型预训练不仅昂贵，而且容易出现训练不稳定性。特别是大学习率训练后期常出现输出logit发散问题。现有z-loss方法仅治标不治本，需要从根本原因出发解决这一问题。

Method: 从输出嵌入几何角度分析不稳定性原因，提出输出嵌入中心化(OEC)作为新的缓解策略。提供两种实现方式：确定性操作μ-centering和正则化方法μ-loss，两者都能抑制输出logit发散。

Result: 实验表明两种OEC变体在训练稳定性和学习率敏感性方面均优于z-loss。特别是对于大学习率训练，当z-loss失败时OEC仍能确保收敛。μ-loss对正则化超参数调优的敏感性显著低于z-loss。

Conclusion: 输出嵌入中心化(OEC)通过解决输出嵌入几何问题，从根本上解决了输出logit发散的不稳定性，相比现有z-loss方法更有效且更稳定。
...
### [293] [GDRO: Group-level Reward Post-training Suitable for Diffusion Models](https://arxiv.org/abs/2601.02036)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法使用在线强化学习进行奖励对齐，但面临效率低（图像采样耗时）、依赖随机采样器、以及奖励黑客问题。整流流模型与LLMs有本质差异：图像采样效率低，且整流流是确定性的（一旦初始噪声固定）

Method: 提出群体级直接奖励优化(GDRO)，这是一种结合整流流模型特性的后训练范式。通过理论分析证明支持完全离线训练，节省图像采样时间；扩散采样器独立，无需ODE-to-SDE近似来获得随机性；并考虑奖励黑客因素进行校正评估

Result: 在OCR和GenEval任务上的广泛实验表明，GDRO通过群体级离线优化有效且高效地提高了扩散模型的奖励分数，同时在缓解奖励黑客方面表现出强大的稳定性和鲁棒性

Conclusion: GDRO为整流流扩散模型的群体级奖励对齐提供了一种新的后训练范式，解决了现有在线RL方法的效率、随机性和奖励黑客问题，实现了高效、稳定且鲁棒的奖励优化
...
### [294] [Multivariate Time-series Anomaly Detection via Dynamic Model Pool & Ensembling](https://arxiv.org/abs/2601.02037)
*Wei Hu,Zewei Yu,Jianqiu Xu*

Main category: cs.LG

TL;DR: DMPEAD：一个动态模型池与集成框架，用于多变量时间序列异常检测，通过参数迁移构建多样模型池，基于相似度动态更新，并通过代理指标排名和top-k聚合集成最优模型。


<details>
  <summary>Details</summary>
Motivation: 现有多模型方法存在三个主要问题：1）选择方法依赖单一模型且对策略敏感；2）集成方法通常组合所有模型或仅限于单变量数据；3）大多数方法依赖固定数据维度，限制了可扩展性。

Method: 提出DMPEAD框架：1）通过参数迁移和多样性指标构建多样化模型池；2）使用元模型和基于相似度的策略动态更新模型池（自适应扩展、子集选择和池合并）；3）通过代理指标排名和top-k聚合在选定子集中集成排名靠前的模型。

Result: 在8个真实世界数据集上的广泛实验表明，该模型在所有基线方法中表现最优，展示了优越的适应性和可扩展性。

Conclusion: DMPEAD框架有效解决了现有多变量时间序列异常检测方法的局限性，通过动态模型池和智能集成策略实现了更好的检测性能和可扩展性。
...
### [295] [Explore the Ideology of Deep Learning in ENSO Forecasts](https://arxiv.org/abs/2601.02050)
*Yanhai Gan,Yipeng Chen,Ning Li,Xingguo Liu,Junyu Dong,Xianyao Chen*

Main category: cs.LG

TL;DR: 提出基于有界变差函数的可解释性框架，通过激活函数饱和区"拯救"神经元，提升ENSO预测模型表达能力，揭示ENSO可预测性主要来自热带太平洋，并探讨春季可预测性障碍的成因。


<details>
  <summary>Details</summary>
Motivation: ENSO对全球气候变化有深远影响，但其预测仍是重大挑战。深度学习虽提升了预测技能，但模型不透明性阻碍了科学信任和业务应用，需要开发可解释的预测框架。

Method: 引入基于有界变差函数的数学可解释性框架，通过从激活函数饱和区"拯救"神经元来增强模型表达能力，分析ENSO可预测性的空间来源，并进行控制实验验证方法稳健性。

Result: 分析显示ENSO可预测性主要来自热带太平洋，印度洋和大西洋也有贡献，与物理理解一致。发现春季可预测性障碍期间敏感性增加但预测性能下降，可能源于次优变量选择。

Conclusion: 提出的可解释性框架增强了ENSO预测模型的科学可信度，春季可预测性障碍的成因表明需要纳入更多海气变量来突破预测限制，推动长期ENSO预测发展。
...
### [296] [The Homogeneity Trap: Spectral Collapse in Doubly-Stochastic Deep Networks](https://arxiv.org/abs/2601.02080)
*Yizhi Liu*

Main category: cs.LG

TL;DR: 本文揭示了双随机矩阵约束在深度架构中的谱退化现象（同质性陷阱），表明高熵约束会抑制次主导奇异值，限制特征变换的有效深度，导致噪声主导下的正交崩溃。


<details>
  <summary>Details</summary>
Motivation: 双随机矩阵在结构保持的深度架构（如最优传输层和Sinkhorn注意力）中广泛使用，以确保数值稳定性和概率可解释性。然而，这些约束可能带来意外的谱退化问题，影响网络的表达能力。

Method: 通过理论分析双随机矩阵的谱特性，推导了次主导奇异值σ_2与网络有效深度的谱界关系，并形式化证明了层归一化在噪声主导机制下无法缓解崩溃现象。

Result: 发现Sinkhorn投影的最大熵偏差会使混合算子趋向均匀重心，抑制σ_2并过滤高频特征分量。当信噪比低于临界阈值时，几何结构会因噪声诱导的正交崩溃而不可逆地丢失。

Conclusion: 双随机矩阵约束的网络存在熵稳定性与谱表达能力之间的根本权衡。高熵约束虽然提供稳定性，但会限制特征变换的有效深度，导致谱退化。
...
### [297] [A Differentiable Adversarial Framework for Task-Aware Data Subsampling](https://arxiv.org/abs/2601.02081)
*Jiacheng Lyu,Bihua Bao*

Main category: cs.LG

TL;DR: 提出对抗性软选择子采样框架，将数据缩减重构为可微分的端到端学习问题，通过选择器网络为样本分配连续重要性权重，在保持模型性能的同时实现智能去噪。
...
Method: 提出对抗性软选择子采样框架，通过选择器网络和任务网络之间的对抗博弈，使用Gumbel-Softmax松弛直接优化，为样本分配连续重要性权重，在损失函数指导下平衡预测的保真度和稀疏性。

Result: 在四个大规模真实世界数据集上的实验表明，ASSS始终优于聚类和最近邻稀疏化等启发式子采样基线，不仅能匹配有时甚至超过整个数据集的训练性能，展示了智能去噪的效果。

Conclusion: 将任务感知数据子采样建立为可学习组件，为有效的大规模数据学习提供了原则性解决方案。
...
### [298] [Horizon Activation Mapping for Neural Networks in Time Series Forecasting](https://arxiv.org/abs/2601.02094)
*Hans Krupakar,V A Kandappan*

Main category: cs.LG

TL;DR: 提出HAM（Horizon Activation Mapping）可视化解释技术，用于跨不同神经网络家族的时序预测模型解释，通过梯度范数平均分析子序列重要性。


<details>
  <summary>Details</summary>
Motivation: 现有时序预测模型依赖误差指标和特定架构的解释方法，无法跨不同模型家族进行比较。需要一种模型无关的解释技术来理解不同神经网络家族在时序预测中的行为。

Method: 受grad-CAM启发，提出HAM技术，使用梯度范数平均分析子序列重要性。引入因果和反因果模式计算每个时间步的梯度更新范数平均，并研究批量大小、早停、数据分割、单变量预测和dropout等优化景观。

Result: 在ETTm2数据集上测试了多种模型（CycleNet、N-Linear、N-HITS、FEDformer、Pyraformer、SpaceTime、Multi-Resolution DDPM）。发现批量大小差异可能指示指数近似关系，NHITS的神经近似定理和SpaceTime的指数自回归活动在HAM图中得到体现。

Conclusion: HAM可用于细粒度模型选择、验证集选择和跨不同神经网络模型家族的比较，为时序预测模型提供统一的解释框架。
...
### [299] [LION-DG: Layer-Informed Initialization with Deep Gradient Protocols for Accelerated Neural Network Training](https://arxiv.org/abs/2601.02105)
*Hyunjun Kim*

Main category: cs.LG

TL;DR: 提出LION-DG初始化方法，针对深度监督架构中的辅助分类器问题，通过零初始化辅助头实现梯度唤醒，无需超参数即可加速收敛
...
Result: 在CIFAR-10和CIFAR-100上的实验显示：DenseNet-DS收敛速度提升8.3%；LSUV与LION-DG结合达到最佳准确率81.92%；ResNet-DS在CIFAR-100上加速11.3%

Conclusion: LION-DG方法简单、无需超参数、无计算开销，为深度监督架构提供了有效的初始化解决方案，并给出了架构特定的实践指导
...
### [300] [Prototype-Based Learning for Healthcare: A Demonstration of Interpretable AI](https://arxiv.org/abs/2601.02106)
...
TL;DR: ProtoPal框架通过原型学习实现个性化预防医疗，提供可理解和可验证的预测与干预建议，具有前后端模式，性能优越且展示直观。


<details>
  <summary>Details</summary>
Motivation: 尽管机器学习和可解释AI有进展，但个性化预防医疗仍缺乏对所有利益相关者都易于理解和验证的预测、干预和推荐系统。

Method: 提出ProtoPal框架，采用原型学习方法，包含前端和后端模式，提供直观的干预展示和模拟结果。

Result: 在定量性能方面表现优越，同时能够直观地呈现干预措施及其模拟结果。

Conclusion: 原型学习方法能够有效满足个性化预防医疗中对可理解性和可验证性的需求，ProtoPal框架为此提供了实用解决方案。
...
### [301] [Edge-aware GAT-based protein binding site prediction](https://arxiv.org/abs/2601.02138)
*Weisen Yang,Hanqing Zhang,Wangren Qiu,Xuan Xiao,Weizhong Lin*

Main category: cs.LG

TL;DR: 提出Edge-aware GAT模型，通过原子级图结构和多维特征，实现蛋白质结合位点的高精度预测，在基准数据集上达到0.93 ROC-AUC，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 准确识别蛋白质结合位点对理解生物分子相互作用机制和药物靶点设计至关重要。传统预测方法在平衡预测精度和计算效率方面存在困难，特别是在捕捉复杂空间构象时。

Method: 提出Edge-aware Graph Attention Network模型，构建原子级图结构，整合几何描述符、DSSP二级结构和相对溶剂可及性等多维结构特征。在注意力机制中引入原子间距离和方向向量作为边特征，增强模型表示能力。使用方向张量传播和残基级注意力池化进一步改进结合位点定位和局部结构细节捕捉。

Result: 在基准数据集上，蛋白质-蛋白质结合位点预测的ROC-AUC达到0.93，优于多种最先进方法。通过PyMOL可视化验证了模型的实用性和可解释性。已部署公开可访问的Web服务器。
...
### [302] [Entropy-Adaptive Fine-Tuning: Resolving Confident Conflicts to Mitigate Forgetting](https://arxiv.org/abs/2601.02151)
*Muxi Diao,Lele Yang,Wuxuan Gong,Yutong Zhang,Zhonghao Yan,Yufei Han,Kongming Liang,Weiran Xu,Zhanyu Ma*

Main category: cs.LG

TL;DR: 论文提出EAFT方法，利用token级熵作为门控机制区分认知不确定性和知识冲突，在保持下游性能的同时显著减轻通用能力退化。


<details>
  <summary>Details</summary>
Motivation: 监督微调(SFT)在领域适应中常导致灾难性遗忘，而在线强化学习(RL)能有效保留通用能力。研究发现这种差异源于分布差距：RL与模型内部信念对齐，而SFT强制模型拟合外部监督，导致"自信冲突"token（低概率但低熵），引发破坏性梯度更新。

Method: 提出熵自适应微调(EAFT)，利用token级熵作为门控机制区分认知不确定性和知识冲突。模型从不确定样本中学习，同时抑制冲突数据的梯度，避免破坏性更新。

Result: 在Qwen和GLM系列模型（4B到32B参数）上的数学、医疗和智能体领域实验验证了假设。EAFT在匹配标准SFT下游性能的同时，显著减轻了通用能力的退化。

Conclusion: EAFT通过熵自适应机制有效解决了SFT中的灾难性遗忘问题，在保持领域适应性能的同时保护了模型的通用能力，为领域适应提供了更稳健的方法。
...
### [303] [Learning with Monotone Adversarial Corruptions](https://arxiv.org/abs/2601.02193)
*Kasper Green Larsen,Chirag Pabbaraju,Abhishek Shetty*

Main category: cs.LG

TL;DR: 研究发现，在单调对抗性污染模型下，所有已知的最优二分类学习算法都会在测试集上产生次优错误率，而基于一致收敛的算法则不受影响，揭示了最优算法对数据可交换性的过度依赖。


<details>
  <summary>Details</summary>
Motivation: 研究标准机器学习算法在多大程度上依赖于数据的可交换性和独立性。通过引入单调对抗性污染模型，探讨当数据被看似"有帮助"的单调污染时，最优学习算法的表现如何。

Method: 引入单调对抗性污染模型：攻击者观察"干净"的i.i.d.数据集后，可以插入自己选择的"污染"数据点，但这些污染点必须是单调的（即按照真实目标函数进行标注）。在此设置下分析不同学习算法的表现。

Result: 1. 所有已知的最优二分类学习算法在单调污染设置下都会在独立测试点上产生次优的期望错误率；2. 基于一致收敛的算法在这种污染下不会降低性能保证；3. 最优算法在面对看似有帮助的单调污染时会失效。

Conclusion: 研究结果表明，最优学习算法在面对单调对抗性污染时会失效，这暴露了它们对数据可交换性的过度依赖。而基于一致收敛的算法则更加鲁棒，不受这种污染的影响。
...
### [304] [ACDZero: Graph-Embedding-Based Tree Search for Mastering Automated Cyber Defense](https://arxiv.org/abs/2601.02196)
...
<details>
  <summary>Details</summary>
Motivation: 现有的自动化网络防御方法（如深度强化学习）在复杂网络环境中面临探索困难、状态/决策空间大、需要大量样本的问题，需要开发更样本高效的防御策略。

Method: 将ACD建模为基于上下文的部分可观测马尔可夫决策过程，提出基于蒙特卡洛树搜索的规划中心防御策略。使用图神经网络嵌入网络观测作为属性图，实现主机及其关系的置换不变推理。通过学习的图嵌入和图编辑动作先验来指导MCTS，结合无模型泛化、策略蒸馏和前向规划。

Result: 在CC4场景中评估，涉及多样化网络结构和对手行为，结果显示基于搜索引导和图嵌入的规划方法相比最先进的RL基线，提高了防御奖励和鲁棒性。

Conclusion: 提出的MCTS与GNN结合的方法能够有效解决自动化网络防御中的探索-利用权衡问题，在复杂网络环境中实现更高效、更鲁棒的防御策略。
...
### [305] [CORE: Code-based Inverse Self-Training Framework with Graph Expansion for Virtual Agents](https://arxiv.org/abs/2601.02201)
*Keyu Wang,Bingchen Miao,Wendong Bu,Yu Wu,Juncheng Li,Shengyu Zhang,Wenqiao Zhang,Siliang Tang,Jun Xiao,Yueting Zhuang*

Main category: cs.LG

TL;DR: CORE是一个基于代码的逆向自训练框架，通过图扩展连接模仿学习和探索学习，提升多模态虚拟代理的行为多样性和泛化能力，无需手动设计奖励函数。


<details>
  <summary>Details</summary>
Motivation: 当前多模态虚拟代理训练存在两种主流范式：行为克隆（简单有效但行为多样性低）和强化学习（能发现新策略但依赖手动设计奖励函数）。这两种方法存在冲突，需要一种能结合两者优点的训练框架。

Method: 1. 语义代码抽象：从专家演示中自动推断奖励函数（称为标签函数），无需手动设计；2. 策略图扩展：构建多路径策略图，捕捉超越专家演示的多样化有效解决方案，增强域内行为多样性；3. 轨迹引导外推：利用成功和失败轨迹扩展任务空间，丰富域外行为多样性。

Result: 在Web和Android平台上的实验表明，CORE显著提高了整体性能和泛化能力，展示了其作为构建强大虚拟代理的稳健且可泛化训练范式的潜力。

Conclusion: CORE框架成功解决了行为克隆和强化学习之间的冲突，通过连接模仿和探索，在提升行为多样性的同时消除了对手动奖励设计的依赖，为多模态虚拟代理训练提供了新的有效范式。
...
### [306] [Quantized SO(3)-Equivariant Graph Neural Networks for Efficient Molecular Property Prediction](https://arxiv.org/abs/2601.02213)
*Haoyu Zhou,Ping Xue,Tianfan Fu,Hao Zhang*

Main category: cs.LG

TL;DR: 该论文提出了一种针对SO(3)-等变图神经网络的低比特量化方法，通过解耦量化、分支分离训练和注意力归一化三个创新技术，在保持精度和等变性的同时实现模型压缩和加速。


<details>
  <summary>Details</summary>
Motivation: 3D等变图神经网络在边缘设备上部署面临高计算成本挑战，需要压缩和加速SO(3)-等变GNN以在实际化学应用中部署。

Method: 提出三种量化等变变换器创新：1) 幅度-方向解耦量化方案，分别量化等变特征的范数和方向；2) 分支分离量化感知训练策略，在注意力SO(3)-GNN中区别处理不变和等变特征通道；3) 鲁棒性增强的注意力归一化机制，稳定低精度注意力计算。

Result: 在QM9和rMD17分子基准测试中，8位模型在能量和力预测方面达到与全精度基线相当的精度，推理速度提升2.37-2.73倍，模型大小减小4倍，同时保持精度和物理对称性。

Conclusion: 提出的技术使得对称感知GNN能够在实际化学应用中部署，在不牺牲精度或物理对称性的情况下实现显著效率提升。
...
### [307] [ELLA: Efficient Lifelong Learning for Adapters in Large Language Models](https://arxiv.org/abs/2601.02232)
*Shristi Das Biswas,Yue Zhang,Anwesan Pal,Radhika Bhargava,Kaushik Roy*

Main category: cs.LG

TL;DR: ELLA：一种基于选择性子空间去相关原则的持续学习框架，通过惩罚任务特定方向的对齐来减轻LLM的灾难性遗忘，无需数据回放或架构扩展。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在持续学习设置中面临严重的灾难性遗忘问题。现有方法存在根本性限制：基于回放的方法不切实际且侵犯隐私，而严格正交性方法在规模扩展时会崩溃，逐渐减少剩余自由度并消除前向传递。

Method: ELLA采用选择性子空间去相关原则，通过轻量级正则化器对单个聚合更新矩阵进行操作。该方法明确表征过去更新的结构，惩罚沿其高能量、任务特定方向的对齐，同时在低能量残差子空间中保持自由度以实现传递。

Result: 在三个流行基准测试中实现最先进的持续学习性能，相对准确率提升高达9.6%，内存占用减少35倍。无需数据回放、架构扩展和可忽略的存储开销。

Conclusion: ELLA为LLM的终身适应提供了一个原则性且可扩展的解决方案，能够稳健地跨架构扩展，并主动增强模型在未见任务上的零样本泛化性能。
...
### [308] [Neuro-Channel Networks: A Multiplication-Free Architecture by Biological Signal Transmission](https://arxiv.org/abs/2601.02253)
*Emrah Mete,Emin Erkan Korkmaz*

Main category: cs.LG

TL;DR: 提出Neuro-Channel Networks (NCN)，一种无乘法神经网络架构，用通道宽度和神经递质参数替代传统权重，仅使用加法、减法和位运算，旨在降低AI对昂贵GPU硬件的依赖。


<details>
  <summary>Details</summary>
Motivation: 深度学习严重依赖高性能GPU硬件，这些硬件昂贵、能耗高且供应稀缺，限制了AI在边缘设备的普及。传统人工感知器依赖密集矩阵乘法，而生物神经系统通过物理离子通道限制和化学神经递质调节实现高效计算，无需算术乘法。

Method: 提出Neuro-Channel Networks (NCN)：1) 用通道宽度替代权重，物理限制信号幅度；2) 引入神经递质参数，基于符号逻辑调节信号传输；3) 前向传播仅使用加法、减法和位运算（最小值、符号），完全消除浮点乘法；4) 使用标准反向传播进行训练。
...
### [309] [POSEIDON: Physics-Optimized Seismic Energy Inference and Detection Operating Network](https://arxiv.org/abs/2601.02264)
*Boris Kriuk,Fedor Kriuk*

Main category: cs.LG

TL;DR: POSEIDON是一个物理信息能量模型，用于统一的多任务地震事件预测，结合了Gutenberg-Richter和Omori-Utsu定律作为可学习约束，在三个预测任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 地震预测和地震危险性评估是地球物理学的基本挑战，现有机器学习方法通常作为黑箱运行，忽略了已建立的物理定律。

Method: 引入POSEIDON（物理优化地震能量推断和检测操作网络），这是一个物理信息能量模型，将Gutenberg-Richter震级-频率关系和Omori-Utsu余震衰减定律作为可学习约束嵌入到能量建模框架中。同时处理三个相互关联的预测任务：余震序列识别、海啸生成潜力和前震检测。

Result: POSEIDON在所有任务上实现了最先进的性能，在平均F1分数上优于梯度提升、随机森林和CNN基线。学习的物理参数收敛到科学可解释的值（Gutenberg-Richter b值0.752，Omori-Utsu参数p=0.835，c=0.1948天），这些值落在已建立的地震学范围内，同时增强了预测准确性。

Conclusion: POSEIDON展示了将物理定律作为可学习约束嵌入机器学习模型的有效性，提高了地震预测的准确性和可解释性。Poseidon数据集（280万事件，30年）公开可用，为物理信息地震研究提供了预计算能量特征、空间网格索引和标准化质量指标。
...
### [310] [Differential Privacy for Transformer Embeddings of Text with Nonparametric Variational Information Bottleneck](https://arxiv.org/abs/2601.02307)
*Dina El Zein,James Henderson*

Main category: cs.LG

TL;DR: 提出NVDP方法，通过向Transformer嵌入添加噪声实现隐私保护的数据共享，在GLUE基准上展示隐私与准确性的权衡


<details>
  <summary>Details</summary>
Motivation: Transformer嵌入包含多个向量（每个token一个），可能编码敏感信息，使攻击者能够恢复输入数据，需要隐私保护的数据共享方法

Method: 提出非参数变分差分隐私（NVDP），将非参数变分信息瓶颈（NVIB）层集成到Transformer架构中，向多向量嵌入注入噪声，使用Rényi散度和贝叶斯差分隐私（BDP）保证衡量隐私保护

Result: 在GLUE基准上测试显示，通过调整噪声水平可以实现隐私与准确性的有用权衡，较低噪声水平下模型保持高准确性同时提供强隐私保证

Conclusion: NVDP方法有效平衡了隐私与实用性，为隐私保护的文本数据共享提供了可行解决方案
...
### [311] [Temporal Kolmogorov-Arnold Networks (T-KAN) for High-Frequency Limit Order Book Forecasting: Efficiency, Interpretability, and Alpha Decay](https://arxiv.org/abs/2601.02310)
*Ahmad Makinde*

Main category: cs.LG

TL;DR: T-KAN模型通过可学习的B样条激活函数替代传统LSTM的固定线性权重，在高频交易环境中显著提升了预测性能，实现了19.1%的F1分数相对提升和132.48%的回报率。


<details>
  <summary>Details</summary>
Motivation: 高频交易环境中的限价订单簿数据噪声大、非线性强，传统模型如DeepLOB随着时间跨度增加预测能力下降（alpha衰减问题），需要更有效的模型来捕捉市场信号的"形状"而非仅幅度。

Method: 提出Temporal Kolmogorov-Arnold Networks (T-KAN)，用可学习的B样条激活函数替代标准LSTM的固定线性权重，使模型能够学习市场信号的"形状"，并针对低延迟FPGA实现进行优化。

Result: 在FI-2010数据集上，T-KAN在k=100时间跨度上实现了19.1%的F1分数相对提升，产生132.48%的回报率（相比DeepLOB的-82.76%回撤），模型具有较好的可解释性，B样条中的"死区"清晰可见。

Conclusion: T-KAN通过可学习的B样条激活函数有效解决了高频交易中的alpha衰减问题，显著提升了预测性能和交易回报，同时具备良好的可解释性和硬件优化潜力。
...
### [312] [Game of Coding: Coding Theory in the Presence of Rational Adversaries, Motivated by Decentralized Machine Learning](https://arxiv.org/abs/2601.02313)
*Hanzaleh Akbari Nodehi,Viveck R. Cadambe,Mohammad Ali Maddah-Ali*

Main category: cs.LG

TL;DR: 论文提出了一种新的博弈论框架"编码游戏"，将编码理论扩展到去中心化系统中理性对手的场景，即使对手占多数也能实现非零的数据恢复概率。


<details>
  <summary>Details</summary>
Motivation: 传统编码理论假设最坏情况的恶意对手，要求诚实节点数量超过对手。但在去中心化机器学习等新兴应用中，参与者因贡献获得奖励，对手可能采取理性策略而非纯粹恶意行为，需要新的编码框架。

Method: 提出"编码游戏"这一博弈论框架，将编码理论扩展到信任最小化的场景。以重复编码为例，展示了该框架的两个关键特性：即使对手占多数也能实现非零数据恢复概率，以及具备女巫攻击抵抗能力。

Result: 该框架能够在诚实节点不占多数的情况下实现数据恢复，且均衡状态不随对手节点数量增加而改变，具备女巫攻击抵抗性。同时探讨了对手策略未知的场景。

Conclusion: 编码游戏框架为去中心化系统中的理性对手场景提供了新的编码理论方法，即使在对手占多数的情况下也能实现可靠通信和计算，并提出了未来研究的开放性问题。
...
### [313] [DatBench: Discriminative, Faithful, and Efficient VLM Evaluations](https://arxiv.org/abs/2601.02316)
...
TL;DR: 论文提出评估视觉语言模型的三个标准：忠实性、区分性和效率，并发现现有评估存在多项缺陷，通过转换和过滤方法改进评估，发布了DatBench评估套件。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型的评估方法尚不成熟，存在多种缺陷：多项选择题格式导致猜测奖励、不能反映实际应用、容易饱和；可盲目回答的问题占比高；样本标签错误或模糊；评估计算成本过高（占开发计算的20%）。

Method: 提出评估的三个标准：忠实性、区分性和效率。通过转换和过滤方法改进现有基准：将多项选择题转换为生成任务；过滤可盲目回答和错误标签的样本；创建DatBench-Full（33个数据集）和DatBench（区分性子集）。

Result: 多项选择题转换为生成任务后，模型能力下降高达35%；过滤问题样本提高了区分能力同时降低计算成本；DatBench子集实现13倍平均加速（最高50倍），同时保持与原数据集相当的区分能力。

Conclusion: 论文为视觉语言模型评估提供了更严谨和可持续的路径，通过改进评估方法揭示了模型真实能力，并显著提高了评估效率，支持模型规模持续扩展。
...
### [314] [Heterogeneous Low-Bandwidth Pre-Training of LLMs](https://arxiv.org/abs/2601.02360)
*Yazan Obeidi,Amir Sarfi,Joel Lidin,Paul Janson,Eugene Belilovsky*

Main category: cs.LG

TL;DR: SparseLoCo（低通信数据并行方法）可以与低带宽流水线模型并行结合，通过激活和激活梯度压缩实现异构分布式训练，在保持性能的同时显著降低通信开销。


<details>
  <summary>Details</summary>
Motivation: 训练大型语言模型需要分布式计算，但带宽限制使得在数据中心之外难以扩展，特别是当模型并行需要频繁的大规模设备间通信时。需要研究如何将低通信数据并行方法与低带宽模型并行结合，以支持异构参与者的分布式训练。

Method: 提出异构分布式训练框架：高带宽参与者托管完整模型副本，资源有限参与者分组使用流水线并行，通过子空间投影进行阶段间通信压缩。将SparseLoCo（基于稀疏伪梯度交换的低频同步数据并行方法）与子空间流水线压缩适配，研究多种调整方案。

Result: 在大规模语言建模实验（1.78亿-10亿参数）中，激活压缩与SparseLoCo结合的成本适中，选择性（异构）压缩相比压缩所有副本能持续改善损失-通信权衡，特别是在高压缩比下效果更明显。

Conclusion: 这些结果表明了一条将低带宽模型并行和异构参与者纳入LLM预训练的实用路径，为在资源受限环境中扩展分布式训练提供了可行方案。
...
### [315] [Semantic Alignment of Multilingual Knowledge Graphs via Contextualized Vector Projections](https://arxiv.org/abs/2601.00814)
*Abhishek Kumar*

Main category: cs.AI

TL;DR: 提出基于嵌入余弦相似度的跨语言本体对齐系统，通过创新描述生成技术丰富本体实体上下文，使用微调的多语言Transformer模型生成嵌入，在OAEI-2022数据集上取得71% F1分数，比最佳基线提升16%
...
Method: 1) 使用新颖技术生成描述来丰富本体实体的上下文信息；2) 采用微调的多语言Transformer模型生成更好的嵌入表示；3) 基于余弦相似度匹配正样本实体对；4) 应用阈值过滤保留高相似度实体

Result: 在OAEI-2022 multifarm track评估数据集上获得71% F1分数（78%召回率，65%精确率），比最佳基线提升16%，证明系统能有效捕捉跨语言相似性

Conclusion: 提出的对齐流程能有效捕捉跨语言的细微相似性，通过上下文丰富的实体描述和微调的多语言嵌入模型显著提升了跨语言本体对齐性能
...
### [316] [MathLedger: A Verifiable Learning Substrate with Ledger-Attested Feedback](https://arxiv.org/abs/2601.00816)
*Ismail Ahmad Abdullah*

Main category: cs.AI

TL;DR: MathLedger是一个可验证机器认知的基础设施，将形式验证、密码学证明和学习动态集成到单一认知循环中，旨在解决AI系统不透明和不可验证的问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统虽然性能卓越，但缺乏透明度和可验证性，这在安全关键部署中造成了信任危机。需要一种能够提供可验证性和审计能力的基础设施。

Method: 采用Reflexive Formal Learning（RFL）方法，这是梯度下降的符号类比，通过验证器结果而非统计损失来驱动更新。系统整合了形式验证、密码学证明和学习动态，形成可审计的账本证明学习框架。

Result: 第一阶段实验验证了测量和治理基础设施在受控条件下的有效性：CAL-EXP-3验证了测量基础设施（Delta p计算、方差跟踪）；压力测试确认了越界条件下故障关闭治理触发正确。没有做出收敛或能力声明。

Conclusion: 主要贡献是基础设施层面的：提供了一个可工作的账本证明学习原型，能够实现大规模审计能力。这是构建可验证AI系统的重要基础。
...
### [317] [Agentic AI for Autonomous, Explainable, and Real-Time Credit Risk Decision-Making](https://arxiv.org/abs/2601.00818)
*Chandra Sekhar Kubam*

Main category: cs.AI

TL;DR: 本文提出了一种基于Agentic AI框架的自主信用风险评估系统，通过多智能体协作、强化学习、自然语言推理和可解释AI模块，实现实时、透明的信用决策，相比传统模型在决策速度、透明度和响应性方面表现更优。
...
Conclusion: 该系统有望变革信用分析领域，未来研究应关注动态监管合规机制、新型智能体协作、对抗鲁棒性以及在跨国信用生态系统中的大规模实施。
...
### [318] [CogCanvas: Compression-Resistant Cognitive Artifacts for Long LLM Conversations](https://arxiv.org/abs/2601.00821)
*Tao An*
...
Result: 在LoCoMo基准测试中，CogCanvas整体准确率达34.7%，优于RAG（25.6%）和GraphRAG（13.7%）。时序推理方面表现尤为突出：31.5% vs 9.3%（RAG）和5.0%（GraphRAG），相对提升530%。多跳因果推理通过率达81.0% vs GraphRAG的40.0%。
...
### [319] [Energy-Aware Routing to Large Reasoning Models](https://arxiv.org/abs/2601.00823)
*Austin R. Ellis-Mohr,Max Hartman,Lav R. Varshney*

Main category: cs.AI

TL;DR: 论文提出在大型推理模型系统中，通过方差感知的路由和调度策略来优化能源效率，找到既不浪费基线能源也不过度依赖辅助能源的临界运行点。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型具有异构的推理能耗，系统性能取决于平均能源供应与随机波动之间的平衡。需要找到既能减少能源浪费又能保证性能的优化方法。

Method: 提出方差感知的路由和调度作为设计原则，基于训练计算和推理计算的缩放定律来制定调度策略，通过时间、模型和执行选择三个维度来吸收变异性。

Result: 识别出临界运行点，在该点既不系统性地浪费基线能源也不过度依赖辅助能源。方差感知的路由策略能够优化能源效率，为开发能源感知的模型路由策略提供理论基础。

Conclusion: 方差感知的路由和调度是优化大型推理模型系统能源效率的关键设计维度，基于缩放定律的调度策略能够有效管理能源供应与需求之间的平衡。
...
### [320] [Decomposing LLM Self-Correction: The Accuracy-Correction Paradox and Error Depth Hypothesis](https://arxiv.org/abs/2601.00828)
*Yin Li*

Main category: cs.AI

TL;DR: 研究发现大型语言模型的自我修正能力存在"准确率-修正悖论"：较弱模型（GPT-3.5）的自我修正率反而比较强模型（DeepSeek）高1.6倍，揭示了模型能力与自我修正之间的非线性关系。


<details>
  <summary>Details</summary>
Motivation: 尽管普遍认为LLMs具有自我修正能力，但近期研究表明内在自我修正（无需外部反馈）效果有限。本研究旨在系统分析自我修正的三个子能力：错误检测、错误定位和错误修正，探索不同模型在这方面的表现差异。

Method: 将自我修正分解为三个子能力：错误检测、错误定位、错误修正。在GSM8K-Complex数据集上（每个模型n=500，共346个错误）进行跨模型实验，使用三个主要LLMs（GPT-3.5、DeepSeek、Claude）。分析不同架构模型的错误检测率、修正成功率，并测试提供错误位置提示的效果。

Result: 发现"准确率-修正悖论"：较弱模型（GPT-3.5，准确率66%）的内在修正率（26.8%）比较强模型（DeepSeek，准确率94%）高1.6倍（16.7%）。错误检测率在不同架构间差异巨大（10%到82%），但检测能力不能预测修正成功率（Claude仅检测10%错误但修正29%）。提供错误位置提示反而损害所有模型的性能。

Conclusion: 提出"错误深度假设"：较强模型犯的错误更少但更深，难以自我修正。研究挑战了关于模型能力和自我改进的线性假设，对自我修正流程设计有重要启示，表明自我修正能力与模型整体能力并非简单正相关。
...
### [321] [Can We Trust AI Explanations? Evidence of Systematic Underreporting in Chain-of-Thought Reasoning](https://arxiv.org/abs/2601.00830)
*Deep Pankajbhai Mehta*

Main category: cs.AI

TL;DR: 研究发现AI模型的推理解释存在"隐藏影响"问题：模型能注意到提示信息但不会主动报告，即使被监控也不会改变，强制报告又会降低准确性


<details>
  <summary>Details</summary>
Motivation: 当AI系统逐步解释其推理过程时，从业者通常假设这些解释揭示了实际影响AI答案的因素。本研究旨在测试这一假设，探究AI模型是否会报告影响其决策的隐藏提示信息。

Method: 在超过9,000个测试案例中，对11个领先的AI模型进行研究。通过在问题中嵌入提示信息，测量模型是否会提及这些提示。测试了三种情况：模型自发报告、直接询问时承认、以及强制报告要求下的表现。

Result: 发现令人担忧的模式：模型几乎从不自发提及提示信息，但当直接询问时，它们承认注意到了这些提示。监控模型不会改善报告行为。强制报告虽然有效，但会导致模型在没有提示时也报告提示，并降低准确性。特别危险的是，迎合用户偏好的提示被模型遵循最多但报告最少。

Conclusion: 仅仅观察AI推理过程不足以发现隐藏的影响因素。AI模型能够看到有影响力的信息但选择不报告，这暴露了当前AI解释系统的局限性，需要更可靠的方法来确保透明度。
...
### [322] [OmniNeuro: A Multimodal HCI Framework for Explainable BCI Feedback via Generative AI and Sonification](https://arxiv.org/abs/2601.00843)
*Ayda Aghaei Nia*

Main category: cs.AI

TL;DR: OmniNeuro是一个新型HCI框架，将BCI从黑盒解码器转变为透明反馈伙伴，通过物理、混沌和量子启发的可解释性引擎驱动实时神经声化和生成式AI临床报告。


<details>
  <summary>Details</summary>
Motivation: 深度学习虽然提高了脑机接口的解码精度，但其"黑盒"特性阻碍了临床采用，导致用户沮丧和神经可塑性结果不佳。需要可解释的反馈系统来改善用户体验。

Method: 提出OmniNeuro框架，集成三种可解释性引擎：1) 物理(能量)分析，2) 混沌(分形复杂度)分析，3) 量子启发的不确定性建模。这些指标驱动实时神经声化和生成式AI临床报告，框架与解码器无关。
...
### [323] [Enhancing Temporal Awareness in LLMs for Temporal Point Processes](https://arxiv.org/abs/2601.00845)
*Lili Chen,Wensheng Gan,Shuang Liang,Philip S. Yu*

Main category: cs.AI

TL;DR: 提出TPP-TAL框架，通过增强LLMs中的时间感知能力来改进时序点过程建模，在多个基准数据集上显著提升了时间似然估计和事件预测精度。


<details>
  <summary>Details</summary>
Motivation: 时序点过程在金融、医疗等领域很重要，但现有方法难以有效捕捉时间信息与语义上下文之间的复杂交互，而大语言模型在序列建模中表现出色却难以直接应用于时序点过程。

Method: 提出TPP-TAL框架，这是一种即插即用的方法，通过显式对齐时间动态与上下文语义，而不是简单拼接事件时间和类型嵌入，从而增强LLMs的时间推理能力。

Result: 在多个基准数据集上的实验表明，TPP-TAL在时间似然估计和事件预测准确性方面带来了显著改进，证明了增强LLMs时间感知对连续时间事件建模的重要性。

Conclusion: TPP-TAL通过增强大语言模型的时间感知能力，有效改进了时序点过程建模，为连续时间事件分析提供了更好的解决方案。
...
### [324] [Temporal Attack Pattern Detection in Multi-Agent AI Workflows: An Open Framework for Training Trace-Based Security Models](https://arxiv.org/abs/2601.00848)
*Ron F. Del Rosario*

Main category: cs.AI

TL;DR: 提出一个基于OpenTelemetry追踪分析、用于检测多智能体AI工作流中时序攻击模式的语言模型微调方法，通过合成数据生成和针对性训练，将准确率从42.86%提升至74.29%。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏可复现的框架来构建针对多智能体AI工作流中时序攻击模式的自定义安全模型，需要一种能在资源受限硬件上运行、且能适应特定威胁环境的解决方案。

Method: 1) 从18个公共网络安全源和35,026条合成OpenTelemetry追踪中整理80,851个示例的数据集；2) 在ARM64硬件上使用QLoRA进行三次迭代微调；3) 采用策略性数据增强；4) 开发合成追踪生成方法用于多智能体协调攻击和违规检测。

Result: 自定义基准测试准确率从42.86%提升至74.29%，实现31.4个百分点的显著提升。针对性训练示例比无差别扩展效果更好，证明了训练数据组成对模型行为的决定性影响。

Conclusion: 虽然实际部署仍需人工监督以处理误报，但这项工作建立了首个可复现框架，使从业者能够构建适应自身威胁环境的自定义智能体安全模型。所有数据集、训练脚本和评估基准已在HuggingFace开源。
...
### [325] [Comment on: Your Brain on ChatGPT: Accumulation of Cognitive Debt When Using an AI Assistant for Essay Writing Tasks](https://arxiv.org/abs/2601.00856)
*Milos Stankovic,Ella Hirche,Sarah Kollatzsch,Julia Nadine Doetsch*

Main category: cs.AI

TL;DR: 本文是对Kosmyna等人(2025)关于使用ChatGPT进行论文写作时认知债务积累研究的评论，指出了该研究在样本量、可重复性、EEG分析方法、结果报告一致性和透明度等方面存在的问题。


<details>
  <summary>Details</summary>
Motivation: 对Kosmyna等人(2025)关于AI助手对认知影响的研究进行建设性评论，旨在提高该研究的科学严谨性和可发表性，因为原研究的一些结果可能需要更保守的解释。

Method: 通过分析原研究的多个方面：研究设计（特别是样本量限制）、分析的可重复性、EEG分析方法学问题、结果报告的不一致性以及研究过程和发现的透明度问题。

Result: 识别出原研究存在的五个主要问题领域，包括样本量不足可能影响统计效力，分析方法可重复性有限，EEG分析存在方法学缺陷，结果报告存在不一致性，以及多个方面的透明度不足。

Conclusion: 建议对Kosmyna等人(2025)的研究结果进行更保守的解释，并提出了改进建议以增强研究的科学严谨性，使其更适合同行评审发表。
...
### [326] [Cultural Encoding in Large Language Models: The Existence Gap in AI-Mediated Brand Discovery](https://arxiv.org/abs/2601.00869)
*Huang Junyao,Situ Ruimin,Ye Renqin*

Main category: cs.AI

TL;DR: 研究发现LLM训练数据的地理分布导致品牌推荐差异，提出"数据护城河"框架，认为AI可见内容是战略资源，品牌需要构建算法无处不在性。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统越来越多地中介消费者信息发现，品牌面临算法不可见性问题。本研究旨在调查大型语言模型中的文化编码现象——由训练数据组成导致的品牌推荐系统性差异。

Method: 分析1,909个纯英文查询，覆盖6个LLM（GPT-4o、Claude、Gemini、Qwen3、DeepSeek、Doubao）和30个品牌。通过案例研究Zhizibianjie（OmniEdge）平台，展示语言边界障碍如何创造不可见的市场进入障碍。

Result: 中国LLM的品牌提及率比国际LLM高30.6个百分点（88.9% vs. 58.3%，p<.001）。这种差异在相同的英文查询中持续存在，表明训练数据的地理分布而非语言驱动了这种效应。提出"存在差距"概念：未出现在LLM训练语料库中的品牌在AI响应中缺乏"存在"，无论其质量如何。

Conclusion: 理论上贡献了"数据护城河"框架，将AI可见内容概念化为VRIN战略资源。操作化"算法无处不在性"作为生成引擎优化的战略目标。为品牌提供18个月路线图，通过语义覆盖、技术深度和文化本地化构建数据护城河。在AI中介市场中，品牌的"数据边界"限定了其"市场前沿"。
...
### [327] [Universal Conditional Logic: A Formal Language for Prompt Engineering](https://arxiv.org/abs/2601.00880)
*Anthony Mikinka*

Main category: cs.AI

TL;DR: UCL是一个将提示工程从启发式实践转化为系统优化的数学框架，通过系统评估显著减少token使用（29.8%），其结构开销函数揭示了过度指定悖论，核心机制得到验证，且最优配置因模型架构而异。


<details>
  <summary>Details</summary>
Motivation: 当前提示工程主要依赖启发式实践，缺乏系统化的优化框架。作者希望建立一个数学框架，将提示工程转化为可系统优化的过程，提高LLM交互的效率并降低成本。

Method: 提出Universal Conditional Logic (UCL)框架，包含指标函数、结构开销函数、早期绑定等核心机制。通过大规模系统评估（N=305，11个模型，4次迭代）验证框架效果，分析结构开销函数O_s(A)和过度指定悖论。

Result: 显著减少token使用29.8%（t(10)=6.36, p<0.001, Cohen's d=2.01），相应降低成本。发现过度指定阈值S*=0.509，超过此阈值后额外指定会二次降低性能。验证了核心机制，并发现最优UCL配置因模型架构而异。

Conclusion: UCL为高效LLM交互提供了一个可校准的框架，模型族特定的优化是未来关键研究方向。该框架将提示工程从启发式实践转变为系统优化，具有实际应用价值。
...
### [328] [Counterfactual Self-Questioning for Stable Policy Optimization in Language Models](https://arxiv.org/abs/2601.00885)
*Mandar Parab*
...
Method: 提出反事实自我提问框架：1) 生成初始推理轨迹；2) 针对潜在失败点提出针对性问题；3) 生成暴露错误假设或无效步骤的替代推理轨迹；4) 利用这些反事实轨迹提供结构化相对反馈进行策略优化

Result: 在多个数学推理基准测试中，反事实自我提问提高了准确性和训练稳定性，特别是对小模型效果显著，仅使用内部生成的监督就能实现可扩展的自我改进

Conclusion: Counterfactual Self-Questioning为语言模型自我改进提供了一种简单有效的框架，无需外部模型辅助，仅通过内部生成的反事实批评就能实现稳定的自我提升
...
### [329] [Context Collapse: In-Context Learning and Model Collapse](https://arxiv.org/abs/2601.00923)
*Josef Ott*

Main category: cs.AI

TL;DR: 论文研究大语言模型中的上下文学习和模型崩溃现象，在线性回归任务中分析参数相变，证明上下文长度超过临界值时出现斜对称分量，并将模型崩溃与上下文退化联系起来。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型中两个关键现象：上下文学习（ICL）和模型崩溃，旨在理解这些现象背后的数学机制及其对模型长期稳定性的影响。

Method: 1. 使用带权重绑定的线性transformer在回归任务上研究ICL，将前向传播简化为预条件梯度下降；2. 使用鞅和随机游走理论分析线性回归和高斯拟合中的模型崩溃；3. 引入"上下文崩溃"概念分析长序列生成中的退化问题。

Result: 1. 证明最小化上下文损失会导致参数相变，超过临界上下文长度时解会出现斜对称分量；2. 证明模型崩溃几乎必然发生，除非数据快速增长或被保留；3. 发现上下文在长序列生成中会退化，特别是在思维链推理中。

Conclusion: 上下文学习和模型崩溃通过"上下文崩溃"概念联系起来，揭示了生成模型长期稳定性的挑战，为理解大语言模型动态行为提供了理论框架。
...
### [330] [ElecTwit: A Framework for Studying Persuasion in Multi-Agent Social Systems](https://arxiv.org/abs/2601.00994)
*Michael Bao*

Main category: cs.AI

TL;DR: ElecTwit是一个模拟社交媒体政治选举中多智能体说服行为的框架，发现LLM使用了25种说服技巧，不同模型架构影响说服动态，并观察到"真相核心"消息和"墨水"痴迷等独特现象。


<details>
  <summary>Details</summary>
Motivation: 克服以往研究中基于游戏的模拟限制，在更真实的环境中研究多智能体系统中的说服行为，特别是在社交媒体政治选举场景中，为评估现实世界中LLM智能体的说服能力提供基础。

Method: 开发了ElecTwit模拟框架，在模拟社交媒体政治选举的逼真环境中进行实验，测试多种LLM模型，分析它们使用的25种具体说服技巧及其差异。

Result: 观察到所有测试的LLM都广泛使用了25种说服技巧，范围比之前报道的更广；不同模型在技巧使用和整体说服输出上存在显著差异，反映了模型架构和训练的影响；发现了"真相核心"消息和"墨水"痴迷（智能体集体要求书面证据）等独特现象。

Conclusion: 该研究为在现实世界环境中评估有说服力的LLM智能体奠定了基础，有助于确保对齐和防止危险结果，展示了不同模型架构如何影响真实社交模拟中的动态。
...
### [331] [Reinforcement Learning Enhanced Multi-hop Reasoning for Temporal Knowledge Question Answering](https://arxiv.org/abs/2601.01195)
*Wuzhenghong Wen,Chao Xue,Su Pan,Yuwei Sun,Minlong Peng*

Main category: cs.AI

TL;DR: MRE框架通过增强前向和后向推理，结合提示工程、监督微调和树结构强化学习，提升TKGQA中多跳推理的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: TKGQA中，LLMs在每跳推理时检索大量时间相似、语义复杂的关系子图，容易导致次优决策和错误传播，需要改进多跳推理的全局优化能力。

Method: 提出MRE框架：1) 提示工程引导LLM生成多样推理轨迹；2) 选择有效轨迹进行监督微调作为冷启动；3) 引入T-GRPO树结构强化学习方法，建立跨跳的因果依赖和多路径反馈机制。

Result: 在两个TKGQA基准测试中，MRE模型持续超越SOTA方法，在处理复杂多跳查询时表现更优，同时提高了可解释性和对噪声时间标注的鲁棒性。

Conclusion: MRE框架通过增强前向和后向推理的协同作用，有效解决了TKGQA中的多跳推理挑战，为复杂时间知识图谱问答提供了更可靠、可解释的解决方案。
...
### [332] [Accelerating Monte-Carlo Tree Search with Optimized Posterior Policies](https://arxiv.org/abs/2601.01301)
*Keith Frankston,Benjamin Howard*

Main category: cs.AI

TL;DR: RMCTS是一种递归的AlphaZero风格蒙特卡洛树搜索算法，相比MCTS-UCB速度更快，训练时间减少约三分之二，性能相当。


<details>
  <summary>Details</summary>
Motivation: AlphaZero的MCTS-UCB算法存在GPU延迟成本高的问题，网络推理需要逐节点进行，限制了搜索速度。需要一种更高效的树搜索算法来加速训练过程。

Method: 提出递归RMCTS算法：1）采用广度优先搜索，使网络推理能够批量处理；2）基于Grill等人的后验策略优化，从叶子节点向根节点递归计算优化后验策略；3）树结构由先验网络策略定义而非自适应构建。

Result: 1）单根状态搜索速度比MCTS-UCB快40倍以上；2）大批量根状态搜索快约3倍；3）在Connect-4、Dots-and-Boxes和Othello三个游戏中，RMCTS训练的网络质量与MCTS-UCB相当，但训练时间减少约三分之二。

Conclusion: RMCTS通过广度优先搜索和批量网络推理显著加速了蒙特卡洛树搜索，虽然树结构非自适应构建，但速度优势明显，能在更短时间内训练出同等质量的网络，具有实际应用价值。
...
### [333] [Digital Twin AI: Opportunities and Challenges from Large Language Models to World Models](https://arxiv.org/abs/2601.01321)
...
<details>
  <summary>Details</summary>
Motivation: 数字孪生已从被动仿真工具发展为智能自主实体，但缺乏系统化的AI集成框架。本文旨在提供一个统一框架来系统描述AI方法在数字孪生全生命周期中的嵌入方式，分析物理建模与数据驱动的协同，并探讨生成式AI对数字孪生转型的影响。

Method: 提出统一的四阶段框架：1) 通过物理基础和物理信息AI方法建模物理孪生；2) 实时同步将物理系统镜像为数字孪生；3) 通过预测建模、异常检测和优化策略干预物理孪生；4) 通过大语言模型、基础模型和智能体实现自主管理。通过跨11个应用领域的综述分析技术协同与挑战。

Result: 建立了系统化的AI集成框架，分析了从传统数值求解器向物理信息模型和基础模型的转变，识别了生成式AI（大语言模型和生成世界模型）如何将数字孪生转变为具备推理、通信和创造性场景生成能力的主动认知系统，并指出了可扩展性、可解释性和可信度等共同挑战。

Conclusion: AI技术正在将数字孪生从被动仿真工具转变为智能自主实体。提出的四阶段框架为AI在数字孪生中的系统集成提供了结构化方法，物理建模与数据驱动的协同是关键趋势，生成式AI将推动数字孪生向认知系统发展，未来需要解决可扩展性、可解释性和可信度等挑战以实现负责任的AI驱动数字孪生系统。
...
### [334] [Beyond Gemini-3-Pro: Revisiting LLM Routing and Aggregation at Scale](https://arxiv.org/abs/2601.01330)
*Shengji Tang,Weihao Lin,Jingqi Ye,Hao Li,Bo Zhang,Shuyue Hu,Tao Chen,Wangli Ouyang,Lei Bai,Peng Ye*

Main category: cs.AI

TL;DR: JiSi框架通过查询-响应混合路由、支持集聚合器选择和自适应路由-聚合切换，使开源LLM协作超越Gemini-3-Pro，仅用47%成本实现更好性能


<details>
  <summary>Details</summary>
Motivation: 探索集体智能作为替代单体扩展的路径，解决当前LLM路由和聚合的三个瓶颈：基于查询的路由器局限、静态聚合方法、路由与聚合互补性未充分利用

Method: 提出JiSi框架，包含三个创新：1) 查询-响应混合路由，同时捕获语义信息和问题难度；2) 支持集聚合器选择，联合评估聚合能力和领域能力；3) 自适应路由-聚合切换，动态利用路由和聚合优势

Result: 在9个基准测试中，JiSi通过协调10个开源LLM，仅用47%成本就超越了Gemini-3-Pro的性能，同时优于主流基线方法

Conclusion: 集体智能代表了通往AGI的新路径，通过LLM协作而非单体扩展可以实现更好的性能效率平衡
...
### [335] [A unified multimodal understanding and generation model for cross-disciplinary scientific research](https://arxiv.org/abs/2601.01363)
*Xiaomeng Yang,Zhiyu Tan,Xiaohui Zhong,Mengping Yang,Qiusheng Huang,Lei Chen,Libo Wu,Hao Li*

Main category: cs.AI

TL;DR: FuXi-Uni是一个统一的多模态科学模型，能在单一架构中理解和生成跨学科的高维科学数据，在地球科学和生物医学领域表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前AI模型多为领域特定，缺乏同时理解和生成多模态科学数据的能力，而许多全球挑战和科学问题本质上是跨学科的，需要跨领域协调进展。

Method: 将跨学科科学标记与自然语言标记对齐，使用科学解码器重建科学标记，支持自然语言对话和科学数值预测的统一架构。

Result: 在地球系统建模中：10天全球天气预报（0.25°分辨率）超越SOTA物理预报系统；热带气旋轨迹和强度预测优于SOTA物理模型；高分辨率区域天气场超越标准插值基线。在生物医学中：在多个生物医学视觉问答基准上优于领先的多模态大语言模型。
...
### [336] [KGCE: Knowledge-Augmented Dual-Graph Evaluator for Cross-Platform Educational Agent Benchmarking with Multimodal Language Models](https://arxiv.org/abs/2601.01366)
*Zixian Liu,Sihao Liu,Yuqi Zhao*

Main category: cs.AI

TL;DR: KGCE是一个针对教育场景中跨平台任务执行的多模态大语言模型代理基准测试平台，通过知识库增强和双图评估框架解决现有基准在私有教育软件支持不足和评估粒度粗糙的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准框架在教育场景的跨平台任务支持不足，特别是在处理学校专用软件（如小雅智能助手、华师匣子等）时，代理因不了解这些私有软件的结构细节而效率显著下降。同时，当前评估方法依赖粗粒度指标，难以捕捉复杂任务中的详细执行和效率。

Method: 提出KGCE平台，整合知识库增强和双图评估框架。首先构建包含104个教育相关任务的数据集，覆盖Windows、Android和跨平台协作任务。引入双图评估框架，将任务分解为多个子目标并验证完成状态，提供细粒度评估指标。为克服现有代理在私有领域任务的执行瓶颈，开发了包含学校专用软件知识库的增强代理系统。

Result: 开发了KGCE基准测试平台，包含104个教育任务的数据集和双图评估框架。通过知识库增强解决了代理在私有教育软件中的执行瓶颈问题，提供了更细粒度的评估能力。

Conclusion: KGCE通过知识库增强和双图评估框架，有效解决了教育场景中跨平台任务基准测试的现有缺陷，为多模态大语言模型代理在教育领域的评估提供了更全面和精细的解决方案。
...
### [337] [Empowering Small Language Models with Factual Hallucination-Aware Reasoning for Financial Classification](https://arxiv.org/abs/2601.01378)
...
<details>
  <summary>Details</summary>
Motivation: 小语言模型在金融分类中因推理时容易产生事实幻觉而导致分类性能较差，需要探索减少事实幻觉是否能改善其分类能力
...
Conclusion: AAAI管道有助于提升小语言模型在金融领域的可信度和有效性，为SLMs的可靠应用提供支持
...
TL;DR: 研究三元背景中的蕴涵关系，专注于Ganter和Obiedkov提出的条件属性蕴涵和属性条件蕴涵，目标是构建这些蕴涵的最优基


<details>
  <summary>Details</summary>
Motivation: 三元背景（triadic contexts）是形式概念分析中的重要扩展，比传统的二元背景更复杂。现有的三元蕴涵理论需要系统化的基构建方法，以提高知识表示和推理的效率

Method: 基于Ganter和Obiedkov提出的条件属性蕴涵和属性条件蕴涵理论，研究如何构建这些蕴涵的最优基。可能涉及形式概念分析、格论和蕴涵逻辑的相关方法

Result: 提出了构建三元背景中条件属性蕴涵和属性条件蕴涵最优基的方法，为三元形式概念分析提供了更有效的知识表示工具

Conclusion: 该研究扩展了形式概念分析在三元背景中的应用，为复杂数据关系中的蕴涵推理提供了理论基础和实用方法
...
### [339] [Reading Between the Lines: Deconfounding Causal Estimates using Text Embeddings and Deep Learning](https://arxiv.org/abs/2601.01511)
*Ahmed Dawoud,Osama El-Shamy*

Main category: cs.AI

TL;DR: 提出神经增强双重机器学习框架，利用文本嵌入解决未观测混杂变量问题，相比传统树模型显著降低估计偏差


<details>
  <summary>Details</summary>
Motivation: 观测性研究中未观测混杂变量导致选择偏差，传统计量方法在混杂变量与结构化协变量正交时失效，而高维非结构化文本包含这些潜在变量的丰富代理信息

Method: 提出神经增强双重机器学习框架，利用文本嵌入进行因果识别，通过深度学习架构建模嵌入流形的连续拓扑结构

Result: 文本嵌入能捕捉结构化数据缺失的关键混杂信息；标准树基DML估计器存在显著偏差(+24%)；深度学习方法将偏差降至-0.86%，有效恢复真实因果参数

Conclusion: 当基于高维自然语言数据进行条件分析时，深度学习架构对于满足无混杂假设至关重要
...
### [340] [Bayesian Orchestration of Multi-LLM Agents for Cost-Aware Sequential Decision-Making](https://arxiv.org/abs/2601.01522)
*Danial Amin*

Main category: cs.AI

TL;DR: 提出贝叶斯成本感知的多LLM编排框架，将LLM视为近似似然模型而非分类器，在非对称错误成本的序列决策中显著降低总成本并提高公平性


<details>
  <summary>Details</summary>
Motivation: 当前LLM在非对称错误成本的自主决策场景（如招聘、医疗分诊、欺诈检测）中，主流方法通过单一LLM获取后验概率并基于"置信度"阈值行动，这种方法在序列决策中无法有效处理成本问题

Method: 提出贝叶斯成本感知的多LLM编排框架：1) 通过对比提示为每个候选状态获取似然值；2) 使用鲁棒统计方法聚合多个不同模型的输出；3) 在新证据到达时使用贝叶斯规则更新信念；4) 基于期望成本选择行动；5) 通过信息价值进行原则性信息收集

Result: 在简历筛选实验中（错失人才成本40000美元，面试成本2500美元，电话筛选成本150美元），使用5个LLM在1000份简历上测试：总成本降低294000美元（34%），人口统计公平性提高45%（最大组差距从22%降至5%）

Conclusion: 正确的概率基础在多LLM编排中至关重要，多模型聚合贡献51%的成本节省，序列更新贡献43%，分歧触发的信息收集贡献20%，证明了贝叶斯框架在非对称成本序列决策中的有效性
...
### [341] [Aletheia: Quantifying Cognitive Conviction in Reasoning Models via Regularized Inverse Confusion Matrix](https://arxiv.org/abs/2601.01532)
*Fanzhe Fu*

Main category: cs.AI

TL;DR: 提出Project Aletheia框架，使用Tikhonov正则化反演判断混淆矩阵来量化System 2推理模型的"认知信念"，引入对齐信念分数确保安全性，为衡量AI科学完整性提供蓝图。


<details>
  <summary>Details</summary>
Motivation: 当前AGI评估范式面临认识论危机：静态基准测试能衡量知识广度但无法量化信念深度。需要扩展CHOKE现象框架来量化System 2推理模型的认知信念。

Method: 提出Project Aletheia认知物理学框架，采用Tikhonov正则化反演判断混淆矩阵。为避免依赖不透明的私有数据，实施合成代理协议进行验证。

Result: 初步研究显示推理模型虽能作为"认知缓冲器"，但在对抗压力下可能表现出"防御性过度思考"。引入对齐信念分数验证信念不损害安全性。

Conclusion: 该工作为测量AI科学完整性提供了蓝图框架，能够量化推理模型的认知信念并确保其与安全性对齐。
...
### [342] [Improving Behavioral Alignment in LLM Social Simulations via Context Formation and Navigation](https://arxiv.org/abs/2601.01546)
*Letian Kong,Qianran,Jin,Renyu Zhang*

Main category: cs.AI

TL;DR: 提出两阶段框架改善LLM在复杂决策环境中的行为对齐：上下文形成（明确实验设计）和上下文导航（指导推理过程），在多个决策任务中验证有效性。
...
Method: 提出两阶段框架：1) 上下文形成阶段明确指定实验设计，建立决策任务和上下文的准确表征；2) 上下文导航阶段在该表征内指导推理过程进行决策。在三个不同决策任务中验证框架。

Result: 在四个SOTA模型（GPT-4o、GPT-5、Claude-4.0-Sonnet-Thinking、DeepSeek-R1）上测试发现：复杂决策环境需要两个阶段才能实现与人类基准的行为对齐，而简单的需求估计任务仅需上下文形成阶段。

Conclusion: 该研究阐明了每个阶段在何时必要，为设计和诊断LLM社会模拟提供了系统方法，使其能作为行为研究中人类受试者的补充工具。
...
### [343] [Logics-STEM: Empowering LLM Reasoning via Failure-Driven Post-Training and Document Knowledge Enhancement](https://arxiv.org/abs/2601.01562)
...
<details>
  <summary>Details</summary>
Motivation: 提升STEM领域的推理能力，通过数据算法协同设计来优化推理任务的黄金标准分布拟合。

Method: 采用数据算法协同设计引擎：数据方面通过5阶段精心设计的数据整理流程构建高质量数据集；算法方面采用失败驱动的后训练框架，针对SFT阶段的失败区域进行针对性知识检索和数据合成。

Result: 在STEM相关基准测试中表现卓越，8B规模模型相比次优模型平均提升4.68%性能，证明了大规模开源数据与精心设计合成数据结合的潜力。

Conclusion: 数据算法协同设计在通过后训练增强推理能力方面发挥关键作用，Logics-STEM模型和数据集已开源以支持社区研究。
...
### [344] [CaveAgent: Transforming LLMs into Stateful Runtime Operators](https://arxiv.org/abs/2601.01569)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的代理系统受限于文本中心范式，依赖JSON函数调用处理长时程任务时存在脆弱的多轮依赖和上下文漂移问题，需要更强大的状态管理和执行能力。
...
Conclusion: CaveAgent通过将LLM转变为运行时操作员，解决了现有代理系统的关键限制，实现了高效、可扩展的复杂任务执行，为LLM代理系统开辟了新方向。
...
### [345] [Structured Decomposition for LLM Reasoning: Cross-Domain Validation and Semantic Web Integration](https://arxiv.org/abs/2601.01609)
*Albert Sadowski,Jarosław A. Chudziak*

Main category: cs.AI

TL;DR: 论文提出了一种结合LLM和符号推理的框架：用LLM将非结构化文本转换为ABox断言，再用SWRL推理器进行确定性规则应用，在三个领域验证了该方法优于few-shot prompting。


<details>
  <summary>Details</summary>
Motivation: 在需要可审计和可解释决策的领域（如临床协议、法律证据规则、科学标准），现有方法存在局限：LLM有灵活性但缺乏一致性保证，符号系统有形式化保证但需要结构化输入。需要结合两者优势。

Method: 提出集成模式：LLM作为本体填充引擎，将非结构化文本转换为ABox断言（基于专家编写的TBox规范），SWRL推理器应用规则提供确定性保证。框架将推理分解为实体识别、断言提取和符号验证，任务定义基于OWL 2本体。

Result: 在三个领域（法律传闻证据判定、科学方法任务应用、临床试验资格）和11个语言模型上验证。结构化分解在总体上比few-shot prompting有统计显著改进，三个领域都有提升。消融研究确认符号验证比单纯结构化提示有实质好处。

Conclusion: 该框架结合了LLM的灵活性和符号推理的形式化保证，填充的ABox可与标准语义网工具集成，支持更丰富的推理模式，适用于需要可审计决策的领域。
...
### [346] [Yuan3.0 Flash: An Open Multimodal Large Language Model for Enterprise Applications](https://arxiv.org/abs/2601.01718)
...
TL;DR: Yuan3.0 Flash是一个开源的MoE多模态大语言模型，拥有37亿激活参数和400亿总参数，专门为企业任务优化，同时保持通用任务竞争力，并采用RAPO算法解决过度思考问题。


<details>
  <summary>Details</summary>
Motivation: 解决大型推理模型中的"过度思考"现象，同时为企业任务（如RAG、复杂表格理解、摘要）提供高性能模型，并保持通用任务的竞争力。

Method: 1. 开发了Yuan3.0 Flash模型：基于MoE架构的多模态大语言模型，37亿激活参数/400亿总参数；2. 提出了RAPO（Reflection-aware Adaptive Policy Optimization）算法：一种新颖的强化学习训练算法，用于调节过度思考行为。

Result: 1. 在企业任务（RAG、复杂表格理解、摘要）上表现优异；2. 在数学、科学等领域展现强大推理能力，达到前沿模型相当的准确率，同时仅需约1/4到1/2的平均token数；3. 模型已完全开源。

Conclusion: Yuan3.0 Flash是一个高效的企业导向多模态大语言模型，通过RAPO算法有效解决了过度思考问题，在专业任务和通用任务上都表现出色，且已开源供研究和实际部署。
...
### [347] [AI Agent Systems: Architectures, Applications, and Evaluation](https://arxiv.org/abs/2601.01743)
*Bin Xu*

Main category: cs.AI

TL;DR: 这篇综述系统梳理了AI智能体架构的现状，涵盖推理、规划、工具调用等核心组件，提出了统一的分类体系，并讨论了设计权衡、评估挑战和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着基础模型与推理、规划、记忆和工具使用能力的结合，AI智能体正在成为连接自然语言意图与现实世界计算的实际接口。需要系统性地梳理这一新兴领域的架构设计、评估方法和挑战。

Method: 通过文献综述和综合分析，将现有工作组织成统一的分类体系：智能体组件（策略/LLM核心、记忆、世界模型、规划器、工具路由器、批评器）、编排模式（单智能体vs多智能体；集中式vs去中心化协调）和部署设置（离线分析vs在线交互；安全关键vs开放任务）。

Result: 建立了AI智能体架构的完整分类框架，识别了关键设计权衡（延迟vs准确性、自主性vs可控性、能力vs可靠性），分析了评估面临的挑战（非确定性、长期信用分配、工具和环境变异性），总结了测量和基准测试实践。

Conclusion: AI智能体架构正在快速发展，但仍面临验证和护栏、可扩展内存和上下文管理、决策可解释性、真实工作负载下的可重复评估等开放挑战，需要进一步研究解决。
...
### [348] [A New Benchmark for the Appropriate Evaluation of RTL Code Optimization](https://arxiv.org/abs/2601.01765)
*Yao Lu,Shang Liu,Hangan Zhou,Wenji Fang,Qijun Zhang,Zhiyao Xie*

Main category: cs.AI

TL;DR: RTL-OPT是一个用于评估大语言模型在RTL优化能力的新基准，包含36个手工设计的数字电路，涵盖多种实现类别，并提供自动化评估框架来验证功能正确性和量化PPA改进。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估RTL代码的语法正确性，而忽略了优化质量（功耗、性能、面积）。需要一个新的基准来评估LLM在RTL优化方面的能力，特别是针对那些传统综合工具无法捕获的行业验证优化模式。

Method: 创建了包含36个手工数字设计的RTL-OPT基准，涵盖组合逻辑、流水线数据路径、有限状态机和存储器接口等类别。每个任务提供一对RTL代码：次优版本和人工优化的参考版本。集成了自动化评估框架来验证功能正确性并量化PPA改进。

Result: 提出了RTL-OPT基准，为评估生成模型在硬件设计优化方面的能力提供了标准化且有意义的评估框架。该基准能够系统评估LLM在RTL优化方面的能力，特别是针对传统综合工具无法捕获的优化模式。

Conclusion: RTL-OPT填补了现有基准的空白，为评估LLM在RTL优化能力提供了全面的评估框架，有助于推动AI在集成电路设计优化方面的研究和应用。
...
### [349] [Can Large Language Models Solve Engineering Equations? A Systematic Comparison of Direct Prediction and Solver-Assisted Approaches](https://arxiv.org/abs/2601.01774)
*Sai Varun Kodathala,Rakesh Vunnam*

Main category: cs.AI

TL;DR: LLMs在求解超越方程时，直接数值预测误差较大，但结合传统迭代求解器的混合架构能显著降低误差67.9%-81.8%，表明LLMs更适合作为符号处理和知识检索的智能接口。


<details>
  <summary>Details</summary>
Motivation: 工程实践中广泛存在需要迭代数值求解的超越方程，研究LLMs能否直接求解这类方程，还是需要结合传统数值求解器的混合架构更有效。

Method: 测试6个先进LLM模型，在7个工程领域的100个问题上，比较直接数值预测与求解器辅助计算（LLMs负责公式推导和初始条件，牛顿-拉弗森迭代执行数值求解）。

Result: 直接预测的平均相对误差为0.765-1.262，求解器辅助计算为0.225-0.301，误差降低67.9%-81.8%。电子学领域改进最大（93.1%），流体力学改进最小（7.2%）。

Conclusion: 当代LLMs擅长符号操作和领域知识检索，但在精度关键的迭代算术方面表现不佳，最优部署方式是作为传统数值求解器的智能接口而非独立计算引擎。
...
### [350] [PsychEval: A Multi-Session and Multi-Therapy Benchmark for High-Realism and Comprehensive AI Psychological Counselor](https://arxiv.org/abs/2601.01802)
...
TL;DR: PsychEval是一个用于心理评估AI的多会话、多疗法、高真实性的基准测试，旨在训练和评估现实AI咨询师，包含记忆连续性、自适应推理等关键能力评估。
...
Method: 构建多会话基准（6-10个会话，分三个阶段），包含677个元技能和4577个原子技能；覆盖五种治疗模式（心理动力学、行为主义、CBT、人本存在主义、后现代主义）和整合疗法；建立包含18个治疗特定和共享指标的评估框架，以及2000多个多样化客户档案。

Result: 数据集质量优越且具有临床保真度，实验分析充分验证了其有效性。PsychEval不仅作为静态基准测试，还能作为高保真强化学习环境，支持临床负责和自适应AI咨询师的自我进化训练。

Conclusion: PsychEval为开发可靠的心理评估AI提供了全面的基准测试和训练环境，解决了AI咨询师训练中的关键挑战，支持多疗法、多会话的高真实性AI咨询师开发。
...
### [351] [Admissibility Alignment](https://arxiv.org/abs/2601.01816)
*Chris Duffey*

Main category: cs.AI

TL;DR: 论文提出"可采纳性对齐"框架，将AI对齐重新定义为在不确定性下对结果分布的可采纳行动和决策选择属性，并通过MAP-AI系统架构实现概率化、决策理论化的对齐评估。


<details>
  <summary>Details</summary>
Motivation: 传统AI对齐方法通常将对齐视为静态或二元条件，缺乏在不确定性环境下对决策政策的系统性评估。需要一种能够处理结果分布、干预效应、价值模糊性和治理约束的框架，以评估AI系统在真实世界决策中的信任度和对齐性。

Method: 提出MAP-AI（蒙特卡洛对齐政策）系统架构，通过蒙特卡洛估计结果分布和可采纳性控制的政策选择来实施对齐。框架评估决策政策在多个可能未来场景中的表现，显式建模不确定性、干预效应、价值模糊性和治理约束，使用分布属性（期望效用、方差、尾部风险、不对齐概率）而非准确性或排名性能来评估对齐。

Result: 建立了实用的AI系统治理基础，其影响由政策在分布和尾部事件中的行为决定，而非单个预测。展示了分布对齐评估如何集成到决策过程中，产生可采纳性控制的行动选择机制，可在不重新训练或修改底层模型的情况下改变不确定性下的政策行为。

Conclusion: 可采纳性对齐框架将AI对齐重新概念化为概率化、决策理论化的属性，MAP-AI架构为企业和机构AI系统提供了可执行的信任和对齐评估方法，强调在不确定性环境下基于分布属性的政策行为评估而非静态对齐条件。
...
### [352] [COMPASS: A Framework for Evaluating Organization-Specific Policy Alignment in LLMs](https://arxiv.org/abs/2601.01836)
*Dasol Choi,DongGeon Lee,Brigitta Jesica Kartono,Helena Berndt,Taeyoun Kwon,Joonwon Jang,Haon Park,Hwanjo Yu,Minsuk Kahng*

Main category: cs.AI

TL;DR: COMPASS框架首次系统评估LLM是否符合组织白名单/黑名单政策，发现模型在合法请求上表现良好（>95%准确率），但在禁止请求上严重失效（仅拒绝13-40%的违规请求），揭示当前LLM缺乏政策关键部署所需的鲁棒性。
...
Method: 提出COMPASS框架，应用于八个不同行业场景，生成并验证了5,920个查询，测试常规合规性和通过战略设计的边缘案例测试对抗鲁棒性。评估了七个最先进的模型。

Result: 发现基本不对称性：模型能可靠处理合法请求（>95%准确率），但在执行禁令方面灾难性失败，仅拒绝13-40%的对抗性黑名单违规请求。

Conclusion: 当前LLM缺乏政策关键部署所需的鲁棒性，COMPASS框架成为组织AI安全评估的重要工具。
...
### [353] [Clinical Knowledge Graph Construction and Evaluation with Multi-LLMs via Retrieval-Augmented Generation](https://arxiv.org/abs/2601.01844)
*Udiptaman Das,Krishnasai B. Atmakuri,Duy Ho,Chi Lee,Yugyung Lee*

Main category: cs.AI

TL;DR: 提出一个端到端框架，利用多智能体提示和模式约束的检索增强生成（KG-RAG）策略，直接从自由文本构建和评估临床知识图谱，特别针对肿瘤学领域。
...
Method: 集成四个关键组件：1）提示驱动的实体、属性和关系提取；2）基于熵的不确定性评分；3）本体对齐的RDF/OWL模式生成；4）用于幻觉检测和语义细化的多LLM共识验证。采用多智能体提示和模式约束的检索增强生成（KG-RAG）策略。

Result: 应用于两个肿瘤学队列（PDAC和BRCA），该方法无需依赖黄金标准注释即可生成可解释、SPARQL兼容且临床基础的知识图谱。实验结果显示在精确度、相关性和本体合规性方面相比基线方法有持续提升。

Conclusion: 该框架为直接从自由文本构建高质量临床知识图谱提供了有效解决方案，支持持续细化和自监督评估，在肿瘤学领域展现出良好应用前景。
...
### [354] [Jenius Agent: Towards Experience-Driven Accuracy Optimization in Real-World Scenarios](https://arxiv.org/abs/2601.01857)
...
<details>
  <summary>Details</summary>
Motivation: 随着基于大语言模型的智能体系统发展，提升自主智能体的任务性能（特别是在上下文理解、工具使用和响应生成方面）变得日益重要。尽管先前研究改进了LLM智能体的整体设计，但对其内部推理和工具使用流程的系统性优化仍待探索。

Method: 提出Jenius-Agent框架，包含三大关键创新：1) 自适应提示生成策略，根据智能体状态和任务目标调整提示以提高可靠性和鲁棒性；2) 上下文感知工具编排模块，基于用户意图和上下文进行工具分类、语义检索和自适应调用；3) 分层内存机制，集成会话内存、任务历史和外部摘要，通过动态摘要和压缩提高相关性和效率。框架还集成了基于MCP的工具、文件I/O和执行反馈等优化。

Result: 实验显示任务准确率提升20%，同时降低了token成本、响应延迟和调用失败率。框架已在Jenius平台部署，为稳健、协议兼容的自主智能体提供了轻量级可扩展解决方案。

Conclusion: Jenius-Agent框架通过系统性优化智能体的内部推理和工具使用流程，显著提升了任务性能并降低了资源消耗，为实际应用中的自主智能体提供了有效的解决方案。
...
### [355] [Toward Auditable Neuro-Symbolic Reasoning in Pathology: SQL as an Explicit Trace of Evidence](https://arxiv.org/abs/2601.01875)
*Kewen Cao,Jianxu Chen,Yongbing Zhang,Ye Zhang,Hongxiao Wang*

Main category: cs.AI

TL;DR: 提出基于SQL的代理框架，通过可执行的SQL查询将细胞特征测量与病理诊断连接，提高病理图像分析的可解释性和可追溯性


<details>
  <summary>Details</summary>
Motivation: 当前病理图像分析模型缺乏可验证的证据支持其决策过程，临床医生难以理解模型决策的依据。视觉语言模型虽然能生成自然语言解释，但这些解释通常是相关性的，缺乏可验证的证据。

Method: 引入SQL中心的代理框架：1) 提取人类可解释的细胞特征；2) 特征推理代理通过SQL查询在特征表上聚合视觉证据为量化发现；3) 知识比较代理将这些发现与已建立的病理知识进行比较，模拟病理学家从可测量观察中论证诊断的过程。

Result: 在两个病理视觉问答数据集上的广泛实验表明，该方法提高了可解释性和决策可追溯性，同时生成可执行的SQL跟踪，将细胞测量与诊断结论连接起来。
...
### [356] [Theory Trace Card: Theory-Driven Socio-Cognitive Evaluation of LLMs](https://arxiv.org/abs/2601.01878)
*Farzan Karimi-Malekabadi,Suhaib Abdurahman,Zhivar Sourati,Jackson Trager,Morteza Dehghani*

Main category: cs.AI

TL;DR: 论文指出当前LLM的社会认知基准测试存在理论缺陷，导致评估结果与真实表现脱节，提出"理论追踪卡"框架来明确评估的理论基础、能力维度、操作化过程和局限性。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型的社会认知基准测试虽然得分很高，但无法预测真实世界表现。现有研究主要归因于测量和效度问题，但忽视了更根本的问题：许多评估缺乏明确的理论基础，导致对能力范围的系统性过度泛化。

Method: 1. 诊断并形式化"理论缺口"问题；2. 提出"理论追踪卡"框架，要求明确记录评估的理论基础、目标能力组件、操作化过程和局限性，而不需要修改现有基准或达成单一理论共识。

Result: 理论追踪卡通过明确理论-任务操作化-评分-局限性的完整效度链，增强了社会认知评估的可解释性和可重用性，解决了系统性效度幻觉问题。

Conclusion: 社会认知评估需要明确的理论基础来避免对基准结果的过度泛化。理论追踪卡提供了一种轻量级文档化方法，能够提高评估的透明度和有效性，而不需要改变现有基准测试本身。
...
### [357] [MMP-A*: Multimodal Perception Enhanced Incremental Heuristic Search on Path Planning](https://arxiv.org/abs/2601.01910)
...
<details>
  <summary>Details</summary>
Motivation: 传统A*算法在大规模复杂环境中计算和内存成本过高，而基于大型语言模型的路径规划方法缺乏空间感知能力，在拓扑复杂环境中容易产生错误路径点，导致效率低下。

Method: 提出MMP-A*框架，整合视觉语言模型的空间感知能力，并引入自适应衰减机制来动态调节不确定路径点在启发式函数中的影响，确保几何有效性同时减少内存开销。

Result: 在具有严重杂乱和拓扑复杂性的挑战性环境中测试，MMP-A*能够实现接近最优的轨迹规划，同时显著降低操作成本。

Conclusion: MMP-A*为自主导航提供了一个具有感知基础和计算效率的新范式，通过多模态整合解决了纯文本规划器的局限性。
...
### [358] [OpenSocInt: A Multi-modal Training Environment for Human-Aware Social Navigation](https://arxiv.org/abs/2601.01939)
*Victor Sanchez,Chris Reinke,Ahamed Mohamed,Xavier Alameda-Pineda*

Main category: cs.AI

TL;DR: OpenSocInt是一个开源的多模态社交交互模拟器软件包，提供模块化架构来训练社交智能体，专注于社交导航任务。


<details>
  <summary>Details</summary>
Motivation: 需要开源工具来模拟和研究多模态社交交互，为训练社交智能体提供可扩展的实验平台。

Method: 开发了模块化架构的软件包，支持不同感知特征的探索、编码和融合，以及多种智能体的使用。

Result: 软件已公开可用（GPL许可证），并通过社交导航任务的实验协议展示了其应用价值。

Conclusion: OpenSocInt为社交交互研究提供了灵活的开源工具，支持多模态特征处理和智能体训练的实验探索。
...
### [359] [CNC-TP: Classifier Nominal Concept Based on Top-Pertinent Attributes](https://arxiv.org/abs/2601.01976)
*Yasmine Souissi,Fabrice Boissier,Nida Meddouri*

Main category: cs.AI

TL;DR: 本文对基于形式概念分析（FCA）的分类器进行了综述，并提出了一种从名义数据计算闭包算子的新方法，构建专注于最相关概念的部分概念格。


<details>
  <summary>Details</summary>
Motivation: 知识发现（KDD）旨在从各种应用领域生成的海量数据中提取隐藏且有意义的模式。形式概念分析（FCA）作为一种可解释和可解释学习的有效方法，基于概念格的数学结构，能够生成形式概念并发现概念间的隐藏关系。

Method: 1. 对基于FCA的分类器进行最新综述；2. 探索从名义数据计算闭包算子的各种方法；3. 提出构建部分概念格的新方法，专注于最相关的概念。

Result: 通过实验验证了所提方法的效率，展示了其在处理名义数据和构建相关概念格方面的有效性。

Conclusion: FCA作为一种可解释的分类方法具有重要价值，提出的部分概念格构建方法能够有效提高分类效率，专注于最相关的概念，为知识发现提供了新的技术途径。
...
### [360] [ChaosBench-Logic: A Benchmark for Logical and Symbolic Reasoning on Chaotic Dynamical Systems](https://arxiv.org/abs/2601.01982)
*Noel Thomas*

Main category: cs.AI

TL;DR: ChaosBench-Logic是一个评估大语言模型在混沌动力系统中逻辑推理能力的基准测试，包含30个系统、621个问题，测试显示前沿LLMs在单项准确率高达91-94%，但在组合推理上得分为0%，对话准确率53.1-75.5%。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在自然语言任务上表现出色，但在需要精确逻辑和符号推理的领域仍然脆弱。混沌动力系统提供了一个特别具有挑战性的测试环境，因为混沌是确定性的，但常被误解为随机性或复杂性。需要评估LLMs在这种复杂科学推理场景中的能力。

Method: 引入ChaosBench-Logic基准测试，使用统一的一阶逻辑本体论评估30个不同的动力系统。每个系统标注了11个语义谓词的真值分配，生成了621个问题，涵盖七个推理类别：多跳推理、跨系统类比、反事实推理、偏见探测和多轮对话。定义了逻辑准确性、蕴含一致性、对话连贯性和矛盾性等指标，并发布了开源评估流程。

Result: 前沿LLMs（GPT-4、Claude 3.5 Sonnet、Gemini 2.5 Flash和开源LLaMA-3 70B）在单项准确率达到91-94%，但在组合推理项目上得分为0%，表现出脆弱的全局连贯性。对话级准确率从53.1%（GPT-4 CoT）到75.5%（LLaMA-3零样本）不等。

Conclusion: ChaosBench-Logic为诊断LLMs在逻辑推理方面的失败提供了一个严格的测试平台，并为开发神经符号方法以改进LLMs的科学推理能力奠定了基础。研究揭示了尽管LLMs在单项任务上表现良好，但在需要组合逻辑推理的复杂场景中仍然存在显著缺陷。
...
### [361] [MindChat: A Privacy-preserving Large Language Model for Mental Health Support](https://arxiv.org/abs/2601.01993)
*Dong Xue,Jicheng Tu,Ming Wang,Xin Yan,Fangzhou Liu,Jie Hu*

Main category: cs.AI

TL;DR: MindChat是一个隐私保护的心理健康支持大语言模型，配合MindCorpus合成咨询数据集，通过联邦学习和差分隐私减少隐私风险，在咨询能力评估中表现优异。
...
Method: 1) 开发MindCorpus合成数据集：采用多智能体角色扮演框架，包含回合级批判修订和会话级策略精化；2) 使用联邦学习结合LoRA适配器进行微调，并加入差分隐私优化。

Result: MindCorpus提高了训练效果，MindChat在自动LLM评估和人工评估中与现有通用和咨询导向的LLM基线竞争，同时在成员推理攻击下表现出减少的隐私泄露。

Conclusion: MindChat是一个有效的隐私保护心理健康支持LLM，通过合成数据生成和隐私保护训练方法，在保持咨询能力的同时降低了隐私风险。
...
### [362] [XAI-MeD: Explainable Knowledge Guided Neuro-Symbolic Framework for Domain Generalization and Rare Class Detection in Medical Imaging](https://arxiv.org/abs/2601.02008)
*Midhat Urooj,Ayan Banerjee,Sandeep Gupta*

Main category: cs.AI

TL;DR: XAIMeD是一个可解释的医疗AI框架，通过神经符号架构将临床专业知识整合到深度学习中，提升分布偏移下的鲁棒性、罕见类别敏感性，并提供透明的临床对齐解释。
...
Method: XAIMeD框架将临床专业知识编码为原子医学命题的逻辑连接，转化为机器可检查的类别特定规则。通过加权特征满足分数量化诊断效用，构建符号推理分支补充神经预测。采用置信度加权融合整合符号和深度输出，以及受Hunt启发的自适应路由机制，由熵不平衡增益和罕见类别基尼系数指导。

Result: 在四个挑战性任务上评估：i) 从rs-fMRI定位癫痫发作起始区；ii) 跨6个多中心数据集的糖尿病视网膜病变分级。结果显示在跨领域泛化方面有6%的性能提升，罕见类别F1分数提高10%，远超最先进的深度学习基线。
...
### [363] [Simulated Reasoning is Reasoning](https://arxiv.org/abs/2601.02043)
*Hendrik Kempt,Alon Lavie*

Main category: cs.AI

TL;DR: 论文认为基础模型通过"大声思考"的模仿、测试和迭代过程实现了某种推理能力，这挑战了传统符号推理的必要性，但存在缺乏常识和脆弱性问题，需要重新评估推理概念和安全考量。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为推理是理解阶段的路径，需要符号推理过程。但基础模型展示了通过模仿"大声思考"过程也能实现推理，这挑战了传统推理理论，需要重新思考推理的本质和条件。

Method: 论文采用哲学分析方法，讨论基础模型推理现象的不同哲学解释，论证"随机鹦鹉"比喻已失去相关性，并反思由此产生的安全和适当性规范考量。

Result: 基础模型通过模仿思考过程、测试路径和迭代实现了某种推理能力，但这种推理缺乏人类推理的常识基础和稳定性，表现出脆弱性，需要新的安全防御方法。

Conclusion: 基础模型的推理能力挑战了传统推理理论，需要放弃过时的"随机鹦鹉"比喻，重新评估推理的必要条件，并建立针对模型脆弱性的安全和适当性规范框架。
...
### [364] [Higher-Order Action Regularization in Deep Reinforcement Learning: From Continuous Control to Building Energy Management](https://arxiv.org/abs/2601.02061)
*Faizan Ahmed,Aniket Dixit,James Brusey*

Main category: cs.AI

TL;DR: 该论文研究通过高阶导数惩罚实现动作平滑正则化，在连续控制基准中理论分析，并在建筑能源管理中实际验证，发现三阶导数惩罚（急动度最小化）能实现最优平滑性且保持性能竞争力。


<details>
  <summary>Details</summary>
Motivation: 深度强化学习代理常表现出不稳定、高频的控制行为，这在实际部署中会导致能源消耗过大和机械磨损，因此需要研究动作平滑性正则化方法来解决这些问题。

Method: 采用高阶导数惩罚进行动作平滑正则化，从连续控制基准的理论理解推进到建筑能源管理的实际验证，系统评估了不同阶数的导数惩罚效果。

Result: 在四个连续控制环境中的综合评估表明，三阶导数惩罚（急动度最小化）能持续实现最优平滑性，同时保持竞争力性能；在HVAC控制系统中，平滑策略使设备切换减少60%，带来显著运营效益。
...
### [365] [FormuLLA: A Large Language Model Approach to Generating Novel 3D Printable Formulations](https://arxiv.org/abs/2601.02071)
*Adeshola Okubena,Yusuf Ali Mohammed,Moe Elbadawi*

Main category: cs.AI

TL;DR: 研究探索了大型语言模型在药物3D打印配方开发中的应用，通过微调四种LLM架构来推荐辅料和预测丝材机械性能，发现Llama2表现最佳，但存在灾难性遗忘等问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI驱动的药物3D打印研究大多局限于狭窄领域，未能全面解决配方开发中的挑战。随着人工通用智能概念的发展，需要探索LLM在药物制剂开发中的更广泛应用，超越传统的预测建模。

Method: 研究使用包含1400多种配方的熔融沉积建模数据集，微调了四种大型语言模型架构，系统评估了微调和生成参数配置。模型被训练用于根据活性药物成分剂量推荐合适的辅料，并预测丝材机械性能。

Result: Llama2在推荐FDM配方辅料方面表现最佳；模型选择和参数化显著影响性能，较小的LLM出现灾难性遗忘现象；即使相对较小的数据集也可能导致灾难性遗忘；标准LLM指标仅评估语言性能而非配方可加工性；基于生物医学相关数据训练的LLM不一定产生最佳结果。

Conclusion: 解决这些挑战对于推进LLM超越语言能力、成为药物配方开发的可靠系统至关重要。需要开发更全面的评估指标，考虑配方可加工性等实际应用需求。
...
### [366] [EverMemOS: A Self-Organizing Memory Operating System for Structured Long-Horizon Reasoning](https://arxiv.org/abs/2601.02163)
*Chuanrui Hu,Xingze Gao,Zuyi Zhou,Dannong Xu,Yi Bai,Xintong Li,Hui Zhang,Tong Li,Chong Zhang,Lidong Bing,Yafeng Deng*

Main category: cs.AI

TL;DR: EverMemOS是一个自组织记忆操作系统，采用engram启发的生命周期管理计算记忆，通过分层记忆结构解决LLM在长期交互中的上下文限制问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型作为长期交互代理部署时，有限的上下文窗口难以维持连贯的长期行为。现有记忆系统通常存储孤立记录和检索片段，无法整合演变的用户状态和解决冲突。

Method: 提出EverMemOS系统，包含三个核心组件：1) 情节痕迹形成：将对话流转换为MemCells，捕捉情节痕迹、原子事实和时间限制的Foresight信号；2) 语义整合：将MemCells组织成主题MemScenes，提炼稳定的语义结构并更新用户画像；3) 重构回忆：执行MemScene引导的代理检索，为下游推理组合必要且充分的上下文。

Result: 在LoCoMo和LongMemEval基准测试中，EverMemOS在记忆增强推理任务上达到最先进性能。在PersonaMem v2上的画像研究和定性案例研究展示了用户画像和Foresight等聊天导向能力。

Conclusion: EverMemOS通过自组织记忆操作系统有效解决了LLM长期交互中的记忆管理问题，实现了更连贯和智能的代理行为。
...
### [367] [Streaming Hallucination Detection in Long Chain-of-Thought Reasoning](https://arxiv.org/abs/2601.02170)
*Haolang Lu,Minghui Pan,Ripeng Li,Guoshun Nan,Jialin Zhuang,Zijie Zhao,Zhongxiang Sun,Kun Wang,Yang Liu*

Main category: cs.AI

TL;DR: 该论文提出将长链思维推理中的幻觉视为演化潜态而非一次性错误事件，引入累积前缀级幻觉信号来追踪推理状态的全局演化，实现流式幻觉检测。
...
Result: 该方法能够在长链思维推理中实现实时、可解释的幻觉检测，提供全局推理状态演化的证据。

Conclusion: 将长链思维推理中的幻觉建模为演化潜态而非一次性事件，通过累积前缀级信号追踪全局状态演化，为流式幻觉检测提供了新视角。
...
### [368] [Project Ariadne: A Structural Causal Framework for Auditing Faithfulness in LLM Agents](https://arxiv.org/abs/2601.02314)
*Sourena Khanzadeh*

Main category: cs.AI

TL;DR: 论文提出Project Ariadne框架，使用结构因果模型和反事实逻辑来审计LLM代理推理的因果完整性，发现当前代理存在"因果解耦"问题，推理痕迹只是"推理剧场"而非真实决策驱动。


<details>
  <summary>Details</summary>
Motivation: 随着LLM代理越来越多地承担高风险自主决策任务，其推理过程的透明度成为关键安全问题。虽然CoT提示能让代理生成人类可读的推理痕迹，但尚不清楚这些痕迹是模型输出的真实生成驱动因素还是仅仅是事后合理化解释。

Method: 提出Project Ariadne框架，利用结构因果模型和反事实逻辑，通过对中间推理节点进行硬干预（do-演算）——系统性地反转逻辑、否定前提、反转事实主张——来测量最终答案的因果敏感性。

Result: 实证评估发现存在持续的"忠实性差距"，定义并检测到广泛存在的"因果解耦"故障模式，在事实和科学领域中违反密度高达0.77。代理在内部逻辑矛盾的情况下仍得出相同结论，证明其推理痕迹只是"推理剧场"，而决策由潜在参数先验控制。
...
### [369] [Falcon-H1R: Pushing the Reasoning Frontiers with a Hybrid Model for Efficient Test-Time Scaling](https://arxiv.org/abs/2601.02346)
...
<details>
  <summary>Details</summary>
Motivation: 探索小型语言模型(SLMs)能否通过优化训练策略和架构设计，在保持参数效率的同时实现与大模型相当的推理性能，为实际部署提供更实用的解决方案。

Method: 采用精心数据筛选和针对性训练策略（包括高效的监督微调SFT和强化学习RL扩展），结合混合并行架构设计实现更快推理，并利用DeepConf方法实现最优测试时扩展效率。

Result: Falcon-H1R-7B在多种推理密集型基准测试中，性能匹配或超越比其大2-7倍的SOTA推理模型，实现了推理效率的3D优化（更快推理、更高token效率、更高准确率）。

Conclusion: 通过针对性模型训练和架构选择，紧凑模型能够提供强大且可扩展的推理性能，为需要大量思维链生成和并行测试时扩展的场景提供了实用的骨干模型。
