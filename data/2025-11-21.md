...
Motivation: 现有基于文本指导的多任务虚拟试穿框架存在语义鸿沟和数据稀缺两大挑战，需要构建一个能灵活处理复杂任务的通用框架。

Method: 提出MLLM引导的语义对齐模块，通过多模态输入和可学习查询捕获跨模态语义关系；采用两阶段渐进训练策略和自合成管道从有限数据中学习复杂任务。

Result: UniFit支持多服装和模型间试穿等多种任务，在实验中达到了最先进的性能。

Conclusion: UniFit通过语义对齐和有效训练策略成功构建了通用虚拟试穿框架，解决了现有方法的局限性。
...
TL;DR: 提出了EfficientSAM3模型，通过渐进式分层蒸馏(PHD)方法将SAM3的能力转移到轻量级学生模型中，实现设备端概念分割和跟踪。
...
Method: 采用三阶段渐进式分层蒸馏：1)编码器蒸馏通过提示循环训练对齐图像特征；2)时序内存蒸馏用紧凑的Perceiver模块替换密集内存；3)端到端微调在SAM3 PCS数据上优化完整流程。

Result: 使用RepViT、TinyViT和EfficientViT骨干网络生成学生变体，在流行的VOS数据集上实现强大的性能-效率权衡。

Conclusion: EfficientSAM3实现了设备端概念分割和跟踪，同时保持与教师模型行为的高度保真度。
...
TL;DR: 提出了四个改进模型基于6D物体姿态估计的新方法：动态非均匀密集采样、多假设推理机制、迭代优化和遮挡训练增强，在遮挡场景下显著提升精度和速度。


<details>
  <summary>Details</summary>
Motivation: 解决现有6D姿态估计方法在遮挡场景下容易出错的问题，传统多阶段流水线在遮挡条件下早期错误会传播，导致性能下降。

Method: 动态非均匀密集采样策略聚焦可见区域计算；多假设推理机制保留多个置信度排序的姿态候选；迭代优化逐步提升精度；遮挡训练增强提升鲁棒性；提出新的加权可见性评估指标。

Result: 在ICBIN数据集上精度提升超过5%，在BOP数据集上提升超过2%，推理速度提升约3倍。

Conclusion: 所提方法有效解决了遮挡场景下的6D姿态估计问题，显著提升了精度和效率，为机器人和增强现实应用提供了更可靠的解决方案。
...
TL;DR: 提出了一种通过风格迁移生成合成历史地图的方法，解决历史地图分析中训练数据不足的问题，并评估了该方法在语义分割任务中的有效性。


<details>
  <summary>Details</summary>
Motivation: 历史地图分析面临标注数据稀缺的挑战，特别是对于特定制图领域的同质地图语料库。人工创建高质量训练数据耗时费力，而现有合成数据缺乏真实性和多样性。

Method: 通过将原始历史地图语料库的制图风格迁移到矢量数据上，生成无限数量的合成历史地图。提出了自动深度生成方法和手动随机退化技术来模拟历史地图扫描中的视觉不确定性和噪声。
...
TL;DR: Box6D是一种针对仓库场景中存储箱的类别级6D姿态估计方法，使用单一RGB-D观测，通过快速二分搜索推断箱子尺寸，利用类别CAD模板而非实例特定模型估计姿态，并通过深度合理性过滤和早停策略降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 解决仓库自动化、拣选等场景中，现有6D姿态估计方法在灵活性、准确性和实用性方面的不足：基于模型的方法需要精确CAD模型且迁移性差；无模型方法在挑战条件下易失败；类别级方法过于通用且忽略环境先验。

Method: 从单次RGB-D观测中，通过快速二分搜索推断箱子尺寸，使用类别CAD模板进行姿态估计，结合深度合理性过滤和早停策略来拒绝不合理假设。

Result: 在真实仓库场景和公共基准测试中，Box6D实现了竞争性或更优的6D姿态精度，同时将推理时间减少约76%。

Conclusion: Box6D方法在保持高精度的同时显著提升了计算效率，适用于工业环境中的存储箱6D姿态估计任务。
...
TL;DR: 提出一种两阶段自改进范式，通过生成文本推理来弥合视觉语言模型在领域特定视频分类中的语义差距，无需新标注即可提升模型性能
...
Conclusion: 自生成推理是一种有效且标注高效的范式，可用于将VLMs适配到领域特定的视频分析任务
...
<details>
  <summary>Details</summary>
Motivation: CLIP的单标签单粒度对齐在医学影像等复杂领域效果有限，因为医学图像通常对应多个高级标签和不同粒度的标注

Method: 使用结构化多标签监督，整合多粒度文本描述，引入带点约束的软标签监督，采用平滑KL散度确保跨粒度一致性

Result: 在构建的大规模多粒度数据集上预训练，在多个数据集的下游任务中优于其他最先进方法

Conclusion: MGLL作为即插即用模块，有效提升了多标签和跨粒度对齐性能，在医学影像等领域具有应用价值
...
TL;DR: 提出一种从3D心脏超声体积中自动选择标准2D视图的方法，结合深度学习视图分类器和解剖标志启发式算法，重建标准超声心动图视图，验证准确率达96%。
...
Method: 应用深度学习视图分类器和基于解剖标志的下游启发式算法，结合心脏病专家提供的启发式方法，从3D超声体积重建标准2D视图。

Result: 在来自2家医院的1,600个视频中验证准确率达96%，提取的2D视频能保持空间校准和诊断特征，支持AI模型检测心脏异常和生成临床级测量。

Conclusion: 该方法使医生能够在保持常规2D视图格式的同时，受益于3D扫描的速度和可用性，为临床实践提供了可行的解决方案。
...
TL;DR: Click2Graph是首个交互式全景视频场景图生成框架，通过单次用户提示（如点击或边界框）实现跨时间的目标分割跟踪、自主发现交互对象，并预测三元组形成时间一致的场景图。
...
Method: 引入动态交互发现模块生成主体条件化对象提示，以及语义分类头进行联合实体和谓词推理。从单次用户提示出发，实现目标分割跟踪、交互对象发现和三元组预测。

Result: 在OpenPVSG基准测试中，Click2Graph为用户引导的全景视频场景图生成建立了坚实基础，展示了如何将人类提示与全景定位和关系推理相结合。

Conclusion: Click2Graph通过结合人类提示、全景定位和关系推理，实现了可控且可解释的视频场景理解，为交互式视频场景图生成开辟了新途径。
...
TL;DR: InfoCLIP提出了一种基于信息论的方法，通过互信息目标来稳定CLIP在开放词汇语义分割任务中的微调过程，避免过拟合并保持预训练的多模态对齐能力。
...
Method: 采用信息论视角，通过两个基于互信息的新目标：1) 压缩预训练CLIP的像素-文本模态对齐以减少噪声；2) 最大化预训练CLIP与微调模型对齐知识之间的互信息，转移适合分割任务的紧凑局部语义关系。

Result: 在多个基准测试上的广泛评估验证了InfoCLIP在增强CLIP微调用于开放词汇语义分割方面的有效性，展示了其在非对称转移中的适应性和优越性。

Conclusion: InfoCLIP通过信息论方法成功解决了CLIP在分割任务微调中的过拟合问题，有效保持了多模态对齐能力，在开放词汇语义分割中表现出色。
...
TL;DR: 提出一种基于一致性正则化的多任务学习方法，通过可微分的BI-RADS形态特征来缓解乳腺超声肿瘤分割与分类任务间的破坏性干扰，在外部验证中显著提升了分割性能。
...
Method: 提出新颖的一致性正则化方法，包含可微分的BI-RADS启发形态特征，用于缓解分割与分类任务间的破坏性干扰。在BrEaST数据集上训练，在三个外部数据集上进行验证。
...
Conclusion: 所提出的一致性正则化方法有效缓解了多任务学习中的破坏性干扰，显著提升了乳腺超声肿瘤分割的泛化性能，在严格的外部验证中表现出色。
...
Method: 检测引导的生成式框架：提取精炼的ROI级特征，使用BART生成器以从粗到细的顺序生成语义token，涵盖类别层次结构和属性值对，支持属性条件识别。

Result: 在大型专有电商数据集和开源数据集上的实验表明，该方法显著优于现有相似性流水线和多阶段分类系统，实现了更强的细粒度识别和更一致的统一推理。
...
TL;DR: 本文提出了一种公平感知的演示选择方法(FADS)，通过聚类采样构建人口统计平衡且语义相关的演示，以改善多模态大语言模型在医学图像推理中的公平性，无需微调即可减少性别、种族和民族相关的差异。
...
Method: 提出公平感知演示选择(FADS)，通过聚类采样构建人口统计平衡且语义相关的演示，解决传统演示选择策略因选择样本中人口统计不平衡而无法确保公平性的问题。

Result: 在多个医学影像基准测试中，FADS一致地减少了性别、种族和民族相关的差异，同时保持了强大的准确性。

Conclusion: 公平感知的上下文学习为公平的医学图像推理提供了一种高效且可扩展的解决方案，展示了其作为可扩展和数据高效解决方案的潜力。
...
TL;DR: 提出一种基于图表示的方法来改善长尾分布视觉数据集中的OOD检测性能，通过图卷积网络优化特征空间，显著降低了误报率并提高了尾部类别的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 在长尾分布数据集上，现有的OOD检测方法存在高误报率和尾部类别分类准确率低的问题，需要开发更有效的解决方案。

Method: 使用预训练模型特征空间初始化图结构，引入高斯化处理激活层分布偏差，然后通过图卷积网络优化图表示，构建适合长尾OOD检测的特征空间。

Result: 在CIFAR10-LT、CIFAR100-LT和ImageNet-LT三个基准测试中，该方法在误报率和尾部类别分类准确率方面大幅优于现有最先进方法。

Conclusion: 基于图表示的方法能有效解决长尾分布数据集中的OOD检测挑战，显著提升检测性能和尾部类别的识别能力。
...
TL;DR: 提出了一种序列级优化框架，生成可在整个行走视频中保持有效的可打印对抗纹理，用于衬衫、裤子和帽子，在数字和物理环境中都能持续隐藏人体检测。
...
Method: 将产品图像映射到UV空间并转换为紧凑的调色板和控点参数化，使用物理基础的人体-衣物管道模拟运动、多角度相机视角、布料动力学和光照变化，通过期望变换目标与时序加权优化控点以最小化检测置信度。

Result: 实验显示具有强大稳定的隐蔽性、对视角变化的高鲁棒性和优越的跨模型可迁移性，物理服装在室内外录制中实现可靠抑制。

Conclusion: 该方法证实了在真实世界中的可行性，为可穿戴对抗攻击提供了有效的序列级解决方案。
...
TL;DR: 该论文提出了一种用于单步图像超分辨率的混合秩（MoR）架构，通过将LoRA中的每个秩视为独立专家，结合退化估计模块和负载均衡损失，实现了对复杂真实世界退化样本的自适应处理。


<details>
  <summary>Details</summary>
Motivation: 现有真实世界图像超分辨率方法主要依赖基于LoRA微调的扩散模型，但这些密集模型难以自适应捕捉复杂退化样本的异质性特征，也无法在同等计算预算下实现输入间的知识共享。

Method: 提出混合秩架构，将LoRA中的每个秩作为独立专家；采用细粒度专家划分策略；开发基于CLIP嵌入的退化估计模块；引入零专家槽和退化感知的负载均衡损失。

Result: 综合实验验证了该框架的有效性和最先进的性能表现。

Conclusion: MoR架构成功解决了真实世界图像超分辨率中自适应处理复杂退化样本的挑战，实现了灵活的知识重组和优化的计算资源分配。
...
TL;DR: 提出一种基于深度学习的激光散斑模式材料分类技术，用于监控激光切割过程，即使在激光颜色改变时也能保持高精度分类。


<details>
  <summary>Details</summary>
Motivation: 激光切割过程中产生大量粉尘、烟雾和气溶胶，对环境和工作健康构成风险。需要实时监控切割过程和识别材料类型以确保安全高效切割。

Method: 训练卷积神经网络（CNN）在激光散斑模式数据集上识别不同材料类型，解决传统方法在激光颜色改变时面临的分类问题。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在稀疏视图设置下存在监督稀缺和过拟合问题，限制了其在有限视角覆盖下的应用。

Method: 提出课程引导框架，为每个真实视角（教师）生成多组不同扰动水平的学生视图，通过深度相关性和协同正则化进行训练，并基于多信号指标评估和保留最佳学生视图。

Result: 在合成和真实稀疏视图场景中，CuriGS在渲染保真度和几何一致性方面优于现有最先进方法。

Conclusion: CuriGS通过课程引导的学生视图生成和渐进式训练策略，有效解决了稀疏视图3D重建的挑战，显著提升了3D高斯泼溅在稀疏视图下的性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决卫星图像在空间分辨率上的限制，特别是在现代农业微地块管理中的不足，通过跨模态学习弥合卫星与无人机传感之间的差距。

Method: 使用近似配准的卫星-无人机图像对数据集，训练模型学习不同传感模态之间的细粒度光谱-空间对应关系。

Result: 生成的无人机类似表示在产量和氮素预测等多个下游任务中持续优于真实卫星图像。

Conclusion: 跨模态对应学习有潜力在农业监测中弥合卫星和无人机传感之间的差距。
...
TL;DR: 提出一个基于大语言模型(LLMs)的框架来解决食品识别中的领域偏移、长尾分布和细粒度分类挑战，通过生成食品标题和成分，将文本和图像投影到共享嵌入空间进行对齐。
...
Method: 利用LLMs解析食品图像生成标题和成分，将生成的文本和来自不同领域的食品图像投影到共享嵌入空间以最大化配对相似度，最后使用对齐后的多模态特征进行识别。

Result: 在两个食品数据集上，该方法在长尾数据分布、领域适应和细粒度分类方面均优于现有专门方法。

Conclusion: 基于LLMs的简单框架能有效解决食品识别中的多重挑战，在多个任务上表现出色。
...
TL;DR: AMS-KV是一种针对视觉自回归模型(VAR)的尺度自适应KV缓存策略，通过优先存储压缩尺度和局部尺度的KV，显著减少KV缓存使用量84.83%并降低自注意力延迟60.48%。


<details>
  <summary>Details</summary>
Motivation: 在视觉自回归模型中，随着尺度数量增加，KV缓存内存呈指数级增长，严重限制了模型的可扩展性。现有的KV缓存研究主要集中在语言模型，而针对基于下一尺度预测的VAR变换器的KV缓存设计尚未充分探索。

Method: 通过系统研究发现：(1)局部尺度token对生成质量贡献显著；(2)为最粗糙尺度分配少量内存可稳定多尺度图像生成；(3)不同层间存在KV相似性差异。基于这些观察，提出AMS-KV策略，优先存储压缩尺度和局部尺度的KV，并通过尺度间相似性分析识别缓存需求层。

Result: AMS-KV将KV缓存使用量减少高达84.83%，自注意力延迟降低60.48%。在基线VAR-d30模型在批次大小为128时出现内存不足的情况下，AMS-KV能够稳定扩展到批次大小256，并提高吞吐量。

Conclusion: AMS-KV通过尺度自适应的KV缓存策略有效解决了VAR模型中KV缓存内存爆炸的问题，显著提升了模型的可扩展性和计算效率，为大规模视觉生成任务提供了可行的解决方案。
...
TL;DR: LiSTAR是一个直接在传感器原生几何上操作的生成式世界模型，通过混合圆柱-球面表示和时空注意力机制，解决了4D LiDAR数据合成中的几何保真度、时间稀疏性和动态场景复杂性挑战。


<details>
  <summary>Details</summary>
Motivation: 合成高保真且可控的4D LiDAR数据对于创建可扩展的自动驾驶仿真环境至关重要，但由于传感器独特的球面几何、点云的时间稀疏性和动态场景的复杂性，这项任务具有挑战性。

Method: 提出混合圆柱-球面表示来保持数据保真度；使用时空注意力与射线中心变换器来捕捉稀疏时间数据的复杂动态；采用4D点云对齐体素布局和离散掩码生成框架进行可控合成。

Result: 在4D LiDAR重建、预测和条件生成方面达到最先进性能：生成MMD降低76%，重建IoU提高32%，预测L1中值误差降低50%。

Conclusion: LiSTAR为创建真实可控的自动驾驶系统仿真提供了强大的新基础，在4D LiDAR数据合成方面实现了显著的性能提升。
...
Method: 采用三阶段解耦架构：1）分层文本引导帧采样模拟人类注意力；2）推理模型生成空间线索和显式推理链；3）使用SAM2和XMem进行分割传播，并通过任务难度感知机制自适应控制推理长度。

Result: 在多个基准测试上的广泛评估表明，VideoSeg-R1在复杂视频推理和分割任务中实现了最先进的性能。

Conclusion: VideoSeg-R1成功将强化学习引入视频推理分割，通过显式推理链和自适应机制在效率和准确性上取得显著提升。
...
TL;DR: SpectralTrain是一个通用的、架构无关的训练框架，通过结合课程学习和基于PCA的光谱降采样，显著提高高光谱图像分类的训练效率，减少2-7倍训练时间，同时保持良好精度。
...
Method: 集成课程学习与基于PCA的光谱降采样，逐步引入光谱复杂度同时保留关键信息，实现高效的光谱-空间模式学习。框架独立于特定架构、优化器或损失函数。

Result: 在三个基准数据集上的实验显示，训练时间减少2-7倍，精度损失较小到中等，在云分类等气候遥感任务中表现出潜力。
...
TL;DR: Rad-GS是一个基于4D雷达-相机融合的SLAM系统，使用3D高斯作为可微分空间表示，专为千米级户外环境设计。它结合雷达点云和多普勒信息来指导动态物体掩码，利用非同步图像帧优化3D高斯表示，并通过八叉树结构和高斯基元管理策略降低噪声和内存消耗。


<details>
  <summary>Details</summary>
Motivation: 传统基于相机或LiDAR的3D高斯方法在户外大规模环境中面临动态物体干扰和内存消耗大的问题。Rad-GS旨在利用4D毫米波雷达的优势，实现鲁棒的户外大规模场景重建。

Method: 1) 结合原始雷达点云、多普勒信息和几何增强点云来指导图像中的动态物体掩码；2) 利用非同步图像帧全局优化3D高斯表示；3) 采用全局八叉树结构和目标高斯基元管理策略来抑制噪声并减少内存消耗。

Result: 实验表明Rad-GS在性能上可与基于相机或LiDAR的传统3D高斯方法相媲美，成功实现了千米级真实世界场景重建，验证了使用4D毫米波雷达进行鲁棒户外建图的可行性。

Conclusion: Rad-GS证明了4D毫米波雷达在户外大规模场景重建中的潜力，通过雷达-相机融合和优化的3D高斯表示，能够有效处理动态物体、提升定位精度并降低内存消耗。
...
TL;DR: 本文提出了T2T-VICL方法，通过生成和选择最佳描述不同低层视觉任务差异的文本提示，构建首个跨任务视觉上下文学习数据集，并结合感知评分推理与评估指标实现跨任务VICL。
...
Method: 设计文本提示生成和选择机制来隐式描述不同低层视觉任务的差异，构建跨任务VICL数据集，并提出结合感知评分推理与评估指标的新型推理框架。

Result: 在9个跨任务场景中取得顶级结果，在另外10个场景中取得次优性能，成功解锁了VLMs中跨任务VICL的潜力。

Conclusion: T2T-VICL方法有效实现了跨任务视觉上下文学习，证明了视觉语言模型在不同视觉任务间进行知识迁移的可行性。
...
TL;DR: 提出一种改进4D高斯泼溅的方法，通过椭圆误差聚类和分组4D高斯泼溅技术，解决动态场景重建中的像素对应模糊和动态区域密度不足问题，显著提升时间一致性和渲染质量。
...
Method: 1) 椭圆误差聚类和误差校正泼溅添加：精确定位动态区域并初始化拟合泼溅；2) 分组4D高斯泼溅：改进泼溅与动态对象映射的一致性。通过缺失颜色和遮挡类型分类，应用基于跨视图颜色一致性的反向投影或前景分割进行针对性校正。

Result: 在Neural 3D Video和Technicolor数据集上的评估表明，该方法显著改善了时间一致性，在Technicolor光场数据集上PSNR提高了0.39dB，达到最先进的感知渲染质量。
...
TL;DR: DCS-LDM是一种新的视觉生成范式，通过构建层次化、尺度无关的潜在空间，将信息复杂度与尺度解耦，支持固定潜在表示下任意分辨率和帧率的解码，实现灵活的计算质量权衡。
...
Method: 构建层次化、尺度无关的潜在空间，通过多级token建模样本复杂度，支持在固定潜在表示下解码到任意分辨率和帧率，并采用渐进式由粗到细的生成范式。
...
TL;DR: VTinker提出了一种新的视频帧插值方法，通过引导流上采样和纹理映射来解决高分辨率运动估计中的模糊和伪影问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法在低分辨率预测双向流后使用上采样会导致流边缘模糊或马赛克，且无法充分捕捉高分辨率下的精细像素运动，导致插值帧出现重影和不连续。

Method: VTinker包含两个核心组件：引导流上采样(GFU)使用输入帧作为指导来减轻双线性上采样流的模糊细节；纹理映射生成中间代理帧作为线索，从输入帧选择清晰纹理块进行映射。

Result: 大量实验表明VTinker在视频帧插值任务中达到了最先进的性能。

Conclusion: VTinker通过改进流上采样和纹理映射策略，有效解决了高分辨率视频帧插值中的运动估计挑战，显著提升了插值质量。
...
TL;DR: 提出PiN-CLIP方法，通过在特征空间注入正激励噪声来抑制捷径敏感性，增强AI生成图像检测的泛化能力，在42种生成模型上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测方法在分布外泛化方面存在挑战，主要原因是训练过程中利用了虚假捷径，而小的特征空间扰动可以缓解捷径主导问题。

Method: 提出PiN-CLIP，联合训练噪声生成器和检测网络，通过交叉注意力融合视觉和类别语义特征构建正激励噪声，注入特征空间微调视觉编码器。

Result: 在包含42种不同生成模型的开放世界数据集上，方法实现了新的SOTA性能，平均准确率比现有方法提升5.4个百分点。

Conclusion: 正激励噪声能够有效抑制捷径敏感方向，放大稳定的取证线索，从而提取更鲁棒和泛化的伪影表示，显著提升AI生成图像检测的泛化能力。
...
TL;DR: 提出了一种用于压缩视频质量增强的预训练退化表示学习模块和分层终止机制，解决了现有盲方法缺乏空间细节和计算资源分配不均的问题。


<details>
  <summary>Details</summary>
Motivation: 现有压缩视频质量增强方法依赖已知量化参数，但在实际传输或转码场景中QP可能未知；当前盲方法只能获取全局退化信息，缺乏空间细节，且不同压缩级别需要不同的计算资源。

Method: 使用预训练退化表示学习模块提取高维多尺度退化表示来指导伪影去除；引入分层终止机制根据压缩级别动态调整伪影减少阶段的数量。

Result: 在QP=22时，PSNR相比现有最佳盲方法提升110%（从0.31dB到0.65dB）；分层终止机制在QP=22时的推理时间比QP=42减少一半。

Conclusion: 所提出的方法在压缩视频质量增强方面显著优于现有盲方法，同时通过自适应计算资源分配提高了效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D目标检测器存在训练与推理之间的差距，即训练时缺乏空间可靠性和排序感知，而推理时使用基于排序的预测选择，这阻碍了模型学习与推理行为对齐的表示。

Method: SR3D包含两个组件：1) 空间优先最优传输分配，动态强调位置良好且空间可靠的样本；2) 秩感知自适应自蒸馏方案，通过自蒸馏范式自适应注入排序感知。

Result: 在ScanNet V2和SUN RGB-D数据集上的大量实验表明，SR3D有效缩小了训练-推理差距，在保持实时速度的同时显著优于现有方法的准确性。

Conclusion: SR3D框架成功解决了3D目标检测中的训练-推理不一致问题，通过空间优先和秩感知机制实现了更准确的实时检测性能。
...
TL;DR: 提出了一个新的高分辨率水体变化检测数据集HSRW-CD和一个空间语义与连续性感知注意力模块SSCP，该模块通过多语义空间注意力、结构关系感知全局注意力和通道自注意力来提升水体变化检测性能。
...
Method: 首先构建HSRW-CD数据集，然后设计SSCP注意力模块，包含MSA、SRGA和CSA三个组件，分别增强空间语义、提取空间结构和计算通道相似性。

Result: 在HSRW-CD和Water-CD数据集上的大量实验验证了SSCP的有效性和泛化能力。

Conclusion: 提出的HSRW-CD数据集和SSCP模块显著提升了水体变化检测的性能，能够更好地利用空间语义和结构信息。
...
TL;DR: LEGO-SLAM是首个在3D高斯泼溅SLAM系统中实现实时开放词汇语义映射的框架，通过场景自适应编码器将语言特征压缩到16维，显著减少内存占用并加速渲染，同时支持语言引导的剪枝和闭环检测。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS SLAM系统缺乏开放词汇语义理解能力，而集成语言特征面临内存占用大、渲染开销高的问题，静态模型方法对未知环境缺乏适应性。

Method: 提出场景自适应编码器-解码器，将高维语言嵌入蒸馏到16维紧凑特征空间；采用语言引导剪枝策略减少60%以上高斯点；利用映射特征进行语言基础的闭环检测。

Result: 在保持竞争性映射质量和跟踪精度的同时，以15 FPS实现开放词汇能力，显著减少内存占用和渲染开销。

Conclusion: LEGO-SLAM成功解决了3DGS SLAM中的语义集成挑战，为高级机器人交互提供了实时、紧凑且自适应的语义映射解决方案。
...
TL;DR: 提出了RGE方法，通过将MLLMs的生成推理能力融入嵌入提取过程来提升多模态表示质量，在MMEB基准上比非推理基线提升4.9%的多模态检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将嵌入提取视为直接编码步骤，忽略了MLLMs具备的生成推理能力，这种能力可能被用来提升表示质量。

Method: 提出RGE方法，保留MLLMs的生成推理过程并与对比训练结合，首先让模型基于指令执行结构化推理生成，然后在推理展开后提取表示。

Result: 在MMEB基准测试中，推理引导的条件化使多模态检索性能比非推理基线提升4.9%。

Conclusion: 显式推理能够有效提升嵌入质量，将推理过程融入嵌入提取是提升多模态表示的有效方法。
...
TL;DR: PPCL是一个专门为扩散变换器设计的可插拔剪枝框架，通过层间蒸馏实现参数减少50%，关键指标退化小于3%，适用于资源受限环境。
...
Method: 首先通过线性探测和一阶微分趋势分析识别冗余层区间，然后提出可插拔的师生交替蒸馏方案，在单一训练阶段集成深度和宽度剪枝。

Result: 在多个多模态扩散变换器架构上，PPCL实现了参数量减少50%，关键客观指标退化小于3%，同时保持高质量图像生成能力。

Conclusion: PPCL框架能够实现高压缩比的同时保持性能，适合资源受限环境部署。
...
TL;DR: Video2Layout框架通过连续边界坐标重建空间布局，解决了现有网格地图方法在细粒度空间推理上的局限性，显著提升了多模态大语言模型的空间智能能力。
...
Method: 采用两阶段框架：首先在监督微调阶段使用AI2THOR模拟器构建高质量数据集，学习从视觉输入到精确边界坐标的映射；然后通过强化微调阶段增强模型在真实世界的泛化能力。

Result: 在QVS-Bench和主流空间推理基准测试中，V2LO-7B模型相比基于网格地图训练的模型平均提升了4.92%，验证了方法的优越性。

Conclusion: Video2Layout通过连续边界坐标表示有效提升了多模态大语言模型的空间推理能力，解决了自然语言描述空间关系时的模糊性问题。
...
TL;DR: 提出Simba框架，将点云补全中的逐点变换回归重新定义为分布学习问题，结合对称性先验和扩散模型，解决过拟合和噪声敏感性问题。


<details>
  <summary>Details</summary>
Motivation: 解决点云补全中同时保留输入细节和确保全局结构完整性的挑战，特别是现有回归方法容易过拟合和噪声敏感的问题。

Method: 将逐点变换回归重新定义为分布学习问题，结合对称性先验和扩散模型的生成能力，使用分层Mamba架构实现高保真上采样。

Result: 在PCN、ShapeNet和KITTI基准测试中验证了方法的state-of-the-art性能。

Conclusion: Simba框架通过分布学习有效解决了点云补全中的过拟合和噪声敏感性问题，实现了优异的性能。
...
TL;DR: 提出LNG-SWR方法，通过层间噪声注入学习频率偏置先验，结合选择性小波重构实现频率自适应，提升医学图像分割模型在分布偏移和扰动下的鲁棒性。
...
Method: 在训练时向多个层注入小幅度零均值噪声，学习频率偏置先验；应用先验引导的选择性小波重构，抑制噪声敏感频带，增强方向性结构和形状线索，稳定边界响应，同时保持频谱一致性。
...
TL;DR: 本文提出了一种新型的verbose-text诱导攻击(VTIA)，通过两阶段框架向良性图像注入不可察觉的对抗性扰动，以最大化视觉语言模型的输出token长度。
...
Method: 采用两阶段框架：1) 对抗性提示搜索，使用强化学习策略自动识别能诱导LLM产生冗长输出的对抗性提示；2) 视觉对齐扰动优化，在输入图像上制作对抗性样本，最大化扰动图像视觉嵌入与对抗性提示之间的相似度。

Result: 在四个流行的视觉语言模型上的综合实验表明，该方法在有效性、效率和泛化能力方面取得了显著优势。
...
TL;DR: EvoVLA是一个自监督的视觉语言动作框架，通过阶段对齐奖励、基于姿态的对象探索和长时程记忆三个组件，解决了VLA模型在多步任务中的阶段幻觉问题，显著提升了长时程机器人操作的成功率和样本效率。
...
Result: 在Discoverse-L基准测试中，EvoVLA将平均任务成功率提高了10.2个百分点，达到69.2%，样本效率提高1.5倍，阶段幻觉从38.5%降至14.8%。真实世界部署在物理机器人上达到54.6%的平均成功率，比OpenVLA-OFT高出11个百分点。

Conclusion: EvoVLA通过解决阶段幻觉问题，显著提升了VLA模型在长时程机器人操作中的性能，实现了有效的仿真到真实世界迁移和强泛化能力。
...
TL;DR: 本文提出RF-CLIP方法，通过模拟人类注意力分散-重聚焦行为，过滤CLIP中的干扰标记，提升其密集预测性能，在八个基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有方法虽然利用CLIP的视觉-语言对齐取得了不错效果，但很少从可解释性机制角度研究CLIP在密集预测方面的性能边界。研究发现CLIP会像人类分心一样，将大量注意力资源从目标区域转移到无关标记上。

Method: 提出RF-CLIP方法，这是一种无需训练的方法，通过识别和过滤维度特定的过度激活产生的干扰标记，将注意力从干扰标记重新聚焦到目标区域，从而改进CLIP的多模态对齐粒度。

Result: 在八个基准测试中达到了最先进的性能，同时保持了较高的推理效率。

Conclusion: 通过模拟人类注意力重聚焦行为，可以有效提升CLIP在开放词汇语义分割中的密集预测能力，证明了过滤干扰标记对改进多模态对齐的重要性。
...
TL;DR: Mantis是一个新颖的VLA框架，通过解耦视觉预测来提升模型性能，在LIBERO基准测试中达到96.7%的成功率，超越了现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型直接预测高维视觉状态会分散模型容量并带来高昂训练成本，而压缩视觉状态又会产生信息瓶颈。此外，现有方法由于忽视语言监督而存在理解和推理能力不足的问题。

Method: 提出解耦视觉预测(DVF)方法，使用元查询和扩散Transformer(DiT)头将视觉预测从主干网络中解耦。通过残差连接提供当前视觉状态，简单的下一状态预测目标使元查询自动捕捉描述视觉轨迹的潜在动作，从而促进显式动作的学习。

Result: 在人类操作视频、机器人演示和图像-文本对上进行预训练后，Mantis在LIBERO基准测试中达到96.7%的成功率，收敛速度快。真实世界评估显示其在指令跟随、未见指令泛化和推理能力方面优于π_{0.5}模型。
...
TL;DR: 提出了一种新的无监督域自适应可见光-红外行人重识别方法DSLGA，通过两阶段训练策略解决跨域和跨模态的差异问题，在真实场景中取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有VI-ReID算法在公共数据集上表现良好，但在真实世界应用中由于数据差异而表现不佳，需要解决无标注新样本的域自适应问题。

Method: 设计了两阶段模型DSLGA：预训练阶段使用域共享学习策略缓解跨域模态差异；微调阶段使用渐进对齐策略通过聚类到整体的方式处理域内跨模态对齐。

Result: 大量实验表明该方法显著优于现有的VI-ReID域自适应方法，甚至在某些设置下超过有监督方法。

Conclusion: 提出的DSLGA方法有效解决了UDA-VI-ReID中的跨域和跨模态挑战，为真实世界应用提供了可行解决方案。
...
Method: 使用连接的模板，联合变形所有子结构以匹配患者特定解剖结构，明确保持内部边界并强制平滑无伪影的表面。

Result: 在心脏、海马体和肺部重建中实现了高几何精度、正确拓扑结构，即使在有限或噪声训练数据下也能保持稳健性能。
...
TL;DR: VLA-Fool首次系统研究了具身视觉语言动作模型在多模态对抗攻击下的脆弱性，揭示了即使是轻微的多模态扰动也会导致显著的行为偏差。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单模态扰动，忽视了跨模态错位对具身推理和决策的根本影响，而VLA模型在现实多模态和黑盒条件下的对抗鲁棒性尚未得到充分探索。

Method: VLA-Fool统一了三种多模态对抗攻击：基于梯度和提示的文本扰动、补丁和噪声的视觉扰动、以及故意破坏感知与指令语义对应的跨模态错位攻击，并开发了首个自动构建的语义引导提示框架。

Result: 在LIBERO基准测试中使用微调的OpenVLA模型进行实验，发现即使是轻微的多模态扰动也会导致显著的行为偏差，证明了具身多模态对齐的脆弱性。

Conclusion: 具身VLA模型在多模态对抗攻击下表现出显著的脆弱性，跨模态错位攻击尤其有效，这凸显了提升多模态对齐鲁棒性的重要性。
...
Method: ICCE方法包含三个阶段：1) 在冻结骨干网络上训练多个聚类头产生多样化聚类；2) 使用聚类集成技术整合冲突结果；3) 用共识聚类结果作为伪标签训练图像分类器

Result: 在10个图像分类基准上达到最先进性能：CIFAR10 99.3%、CIFAR100 89%、ImageNet 70.4%，首次在ImageNet上超过70%准确率

Conclusion: ICCE通过聚类集成策略显著提升了无监督图像分类性能，缩小了与有监督方法的差距
...
Method: 引入MIDA任务和数据集，评估12个最先进的MLLMs，设计Social Chain-of-Thought推理管道和Dynamic Social Epistemic Memory模块。

Result: 即使是GPT-4o等强大模型也难以可靠地区分真假，模型无法有效将语言与多模态社交线索关联，缺乏建模他人知识、信念或意图的能力。

Conclusion: 该框架在挑战性任务上实现了性能提升，为构建具有真正类人社交推理能力的MLLMs开辟了有前景的新路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有跨模态目标跟踪方法通常将并行RGB和NIR分支连接到共享骨干网络，限制了模态特定特征的全面提取，无法解决目标漂移问题，特别是在存在不可靠输入时。

Method: 部署三个专用流：RGB帧由视觉编码器处理，NIR帧通过NIR门控适配器与视觉编码器结合进行细化，无效模态使用一致性轨迹预测模块利用时空线索估计目标运动，还包含动态模板重建和相似性对齐损失。

Result: 在最新基准测试中实现最先进性能，精度率和成功率分别提升7.2%和4.3%，同时保持65帧/秒的实时跟踪速度。

Conclusion: SwiTrack通过状态切换框架和专用流设计，有效解决了跨模态目标跟踪中的特征提取和目标漂移问题，显著提升了跟踪性能。
...
TL;DR: 提出一种基于MLP和Memory-Block的神经网络方法，从稀疏传感器输入生成完整全身运动，在移动HMD上实现72FPS的高性能实时追踪


<details>
  <summary>Details</summary>
Motivation: 现有AR/VR系统主要通过头显和控制器追踪头部和手部，导致3D全身重建不完整，需要从有限传感器输入生成完整全身运动

Method: 使用多层感知机(MLP)骨干网络，增强残差连接和新型Memory-Block组件，通过可训练代码向量表示缺失传感器数据，结合历史稀疏信号提高时间一致性，采用多任务学习框架

Result: 方法显著优于现有技术，大幅减少预测误差，在移动HMD上达到72FPS，改善了精度与运行时间的权衡

Conclusion: 所提出的MLP+Memory-Block方法能够高效地从稀疏传感器输入生成准确且时间一致的全身运动，适用于AR/VR应用
...
TL;DR: TetraSDF是一个精确的解析网格化框架，用于处理由ReLU MLP与多分辨率四面体位编码器组成的SDF表示，能够生成与学习等值面高度一致的自洽网格。


<details>
  <summary>Details</summary>
Motivation: 从神经SDF中提取与零级集完全匹配的网格仍然具有挑战性：基于采样的方法引入离散化误差，而连续分段仿射解析方法仅适用于普通ReLU MLP。

Method: 使用四面体位编码器，其重心插值保持全局CPWA结构，从而能够在编码器诱导的多面体复合体中跟踪ReLU线性区域；采用源自编码器度量的固定解析输入预处理器减少方向偏差并稳定训练。

Result: 在多个基准测试中，TetraSDF在SDF重建精度上匹配或超越现有基于网格的编码器，其解析提取器产生高度自洽的网格，忠实于学习的等值面，同时具有实用的运行时和内存效率。
...
<details>
  <summary>Details</summary>
Motivation: 开发高效的实时场景理解系统，特别是针对辅助导航应用，需要克服VGGT的高内存需求，同时保持时空一致性和丰富的上下文推理能力。

Method: 使用滑动窗口处理图像流并对齐子地图，利用VGGT跟踪头聚合2D语义实例掩码为3D对象，存储时间戳和实例级身份以实现时间一致性和环境变化检测。

Result: 在知名基准测试和专门设计的辅助导航数据集上评估，结果表明该框架适用于现实世界场景。

Conclusion: 提出的VGGT框架能够有效实现快速时空场景理解，支持接近实时的辅助导航应用，并在实际场景中表现出良好的适用性。
...
Method: 使用CNN、ViT和GNN构建混合框架，采用GAN进行数据增强平衡类别分布，结合自监督对比预训练从有限标注数据中学习更多特征。

Result: 在多个基准数据集上获得99.33%的准确率、精确率、召回率和F1分数。

Conclusion: 该框架能够实现局部、全局和关系特征表示，具有高可解释性和适应性，可在边缘设备上实时高效部署，减少对除草剂的过度依赖，提供可扩展的可持续精准农业解决方案。
...
TL;DR: Texture3dgs是一个针对移动GPU优化的3D高斯泼溅算法实现，通过新颖的排序算法和内存优化，在移动设备上实现了显著的性能提升和内存使用减少。


<details>
  <summary>Details</summary>
Motivation: 将3D高斯泼溅算法部署到移动设备上具有数据隐私、离线操作和更快响应等优势，但移动GPU的二维纹理缓存优化是关键挑战。

Method: 开发了针对二维内存优化的新型排序算法，改进了变量布局设计，并对3DGS算法的其他步骤进行了加速优化。

Result: Texture3dgs在排序和整体3D场景重建方面分别实现了4.1倍和1.7倍的加速，同时内存使用减少了1.6倍。
...
<details>
  <summary>Details</summary>
Motivation: 视觉基础模型虽然在下游任务中表现出强大的泛化能力，但其表征通常被下采样14x/16x，限制了在像素级应用中的直接使用。现有特征上采样方法依赖数据集特定重训练或重型隐式优化，限制了可扩展性和泛化性。

Method: 通过简单的每图像优化学习各向异性高斯核，结合空间和范围线索，有效桥接高斯溅射和联合双边上采样。学习到的核作为通用边缘感知算子，可在架构和模态间无缝迁移。
...
TL;DR: SAEs可被理解为嵌入空间中的主题模型，作者提出了SAE-TM框架，将SAE特征作为可重用的主题原子，在文本和图像数据集上生成更连贯的主题，支持跨模态的大规模主题分析。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器（SAEs）在嵌入分析中的作用和实用价值存在争议，作者希望从主题模型的角度重新理解SAEs，探索其在跨模态主题分析中的潜力。
...
Conclusion: SAEs可作为跨模态大规模主题分析的有效工具，SAE-TM框架为理解和应用SAEs提供了新的理论视角和实践方法。
...
TL;DR: BioBench是一个生态学视觉基准测试，解决了ImageNet在科学图像评估中的不足，包含9个任务、4个生物分类界、6种采集模态的310万张图像。


<details>
  <summary>Details</summary>
Motivation: ImageNet-1K线性探针转移准确率已无法准确预测科学图像上的性能表现，在生态学任务中仅能解释34%的方差，且对30%以上准确率超过75%的模型排名错误。

Method: 开发BioBench基准测试，统一9个公开的应用驱动任务，涵盖4个生物分类界和6种采集模态，提供Python API下载数据、在冻结骨干网络上拟合轻量级分类器，并报告类别平衡的宏F1分数。

Result: BioBench为生态学计算机视觉提供了新的性能信号，ViT-L模型在A6000 GPU上6小时内即可完成评估。

Conclusion: BioBench不仅解决了ImageNet在生态学领域的评估缺陷，还为任何领域构建可靠的AI-for-science基准测试提供了模板方法。
...
Method: 提出潜在颜色扩散方法，包括几何感知颜色点云VAE和多控制扩散变换器(DiT)，完全使用3D数据从头训练。引入原生几何控制，通过位置嵌入和几何潜在变量为DiT提供直接3D空间信息。

Result: NaTex在纹理一致性和对齐方面表现出强大性能，显著优于先前方法，并展现出强大的泛化能力，适用于材质生成、纹理优化、部件分割和纹理化等下游应用。

Conclusion: NaTex通过原生3D纹理生成范式有效解决了传统2D多视图方法的局限性，在纹理质量和应用灵活性方面取得了显著进步。
...
<details>
  <summary>Details</summary>
Motivation: 现有的混合方法虽然性能强大但计算成本高，限制了在实时场景中的实用性。需要开发既高效又有效的解决方案。

Method: 集成三个可解释先验：1）自适应白平衡缓解波长相关颜色衰减；2）小波增强块进行多频带分解捕获全局结构和精细纹理；3）梯度感知模块使用可学习门控Sobel算子显式保护边缘结构。

Result: 在基准数据集上的广泛实验表明，WWE-UIE以显著更少的参数和FLOPs实现了有竞争力的恢复质量，能够在资源受限平台上进行实时推理。

Conclusion: WWE-UIE通过集成可解释先验，在保持高质量恢复的同时实现了高效性，为水下图像增强的实际应用提供了可行的解决方案。
...
Method: 提出端到端多尺度Siamese框架，融合轻量级骨干网络和冻结DINOv3特征构建语义丰富的特征金字塔；使用空间-光谱差分transformer解码器利用多尺度绝对差异作为变化先验；最后通过可学习形态学模块优化上采样logits以恢复清晰边界。
...
<details>
  <summary>Details</summary>
Motivation: 现有FSR方法受限于固定上采样尺度和对输入尺寸变化的敏感性，无法灵活处理不同分辨率和尺度的人脸超分辨率任务。

Method: 使用2D深度特征、局部相对坐标和上采样比例预测RGB值；局部频率估计模块捕获高频纹理信息；全局坐标调制模块利用先验人脸结构知识。

Result: 定量和定性评估表明ARASFSR在各种输入尺寸和上采样尺度下都优于现有最先进方法。

Conclusion: ARASFSR方法在任意分辨率和尺度的人脸超分辨率任务中表现出强大的鲁棒性和有效性。
...
Result: 实验结果表明，仅使用30%标注数据，该方法能同时提高mIoU和时序一致性，确保地形物体的稳定定位。
...
TL;DR: 提出了一种实时相机定位方法，通过在预捕获的彩色LiDAR点云中渲染合成视图，建立2D-3D对应关系，实现无漂移、正确度量尺度的相机跟踪。
...
Method: 从LiDAR点云渲染合成视图，使用神经渲染技术缩小合成图像与真实图像的域差距，建立2D-3D对应关系，提供在线渲染匹配和预构建定位两种实时变体。

Result: 在ScanNet++数据集上表现优异，优于现有SLAM流水线，实现无漂移的相机跟踪和正确的度量尺度。
...
TL;DR: MOMNet是一种无需对齐的深度超分辨率框架，通过多阶匹配机制从错位的RGB图像中自适应检索相关信息，解决了实际场景中RGB-D数据难以严格对齐的问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界中由于硬件限制和校准漂移，RGB-D数据难以严格对齐，导致现有基于对齐假设的方法性能下降。需要开发无需对齐的深度超分辨率方法。

Method: 提出多阶匹配网络(MOMNet)，包含多阶匹配机制(零阶、一阶、二阶匹配)和多阶聚合模块，通过多阶先验作为提示，选择性从RGB向深度传递特征。

Result: 大量实验表明MOMNet实现了最先进的性能，并展现出出色的鲁棒性。

Conclusion: MOMNet通过多阶匹配和聚合策略，有效解决了RGB-D错位问题，为实际应用中的深度超分辨率提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖整体特征进行配对比较，但忽略了细粒度差异对鲁棒离线签名验证的重要性。此外，基于transformer的骨干网络可能自然模糊局部细节，影响验证性能。

Method: 提出DetailSemNet模型，通过局部结构匹配来提升验证准确性。引入细节语义集成器，利用特征解缠和重缠技术，专门增强复杂细节并扩展判别语义。

Result: 在离线签名验证的领先基准测试中，模型持续优于近期方法，以明显优势达到最先进结果。在跨数据集测试场景中展现出卓越的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于CLIP的CZSL方法主要依赖图像编码器获得的全局语义表示，但这种表示能力有限，无法完全解缠属性和对象，限制了在未见组合上的泛化能力。

Method: 设计了门控交叉注意力机制，从CLIP高层图像编码块中捕获细粒度语义特征，并通过多空间解缠技术实现属性和对象语义的分离。

Result: 在MIT-States、UT-Zappos和C-GQA三个流行基准测试中，CAMS在封闭世界和开放世界设置下都达到了最先进的性能。

Conclusion: CAMS通过语义特征提取和多空间解缠有效提升了组合零样本学习的泛化能力，为属性-对象组合识别提供了新的解决方案。
...
TL;DR: 提出了一种基于稀疏刚性标记点(RBM)的新型运动捕捉方法，通过深度学习回归模型直接估计SMPL参数，在保持高精度的同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统基于标记点的光学运动捕捉系统存在设置复杂、标记点识别模糊等问题，限制了其可扩展性和实用性。

Method: 引入刚性标记点(RBM)作为基本单元，提供明确的6自由度数据；开发基于深度学习的回归模型，在测地线损失下直接估计SMPL参数。

Result: 端到端方法在性能上媲美基于优化的方法，计算量减少一个数量级以上；在AMASS数据集上训练达到最先进的姿态估计精度；实际Vicon系统验证了方法的可行性。
...
TL;DR: 提出一种几何引导的自监督环视深度估计方法，通过圆柱投影建立跨视图一致性，改善多相机重叠区域的深度估计一致性
...
Method: 基于相机内外参，将各图像的3D点投影到共享圆柱体上建立邻域关系，使用非学习的空间注意力机制聚合跨图像特征来预测最终深度图

Result: 在DDAD和nuScenes数据集上评估，相比最先进方法提高了跨图像深度估计的一致性和整体深度精度

Conclusion: 该方法能够预测密集、度量且跨视图一致的深度，为360°视野的3D感知提供低成本解决方案
...
TL;DR: 提出两种结合ViT和GNN的图分割模型，用于腹腔镜胆囊切除术中的肝脏解剖结构分割，在遮挡、长距离依赖和精细几何结构识别方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 准确识别肝脏解剖结构对预防腹腔镜胆囊切除术并发症至关重要。现有深度学习模型在处理遮挡、长距离依赖和罕见结构的精细几何特征方面存在困难。

Method: 提出两种分割模型：1) 静态k-NN图+GCNII，实现稳定的长距离信息传播；2) 动态可微分图生成器+GAT，支持自适应拓扑学习。在Endoscapes-Seg50和CholecSeg8k基准上评估。

Result: 相比最先进基线，mIoU提升7-8%，mDice提升6%，在细薄、罕见和安全关键结构上产生解剖学一致的预测。

Conclusion: 图分割方法提升了手术场景分割的性能和解剖一致性，结合ViT的全局上下文和图关系推理，提高了可解释性和可靠性，为更安全的腹腔镜和机器人辅助手术铺平道路。
...
TL;DR: 本文提出了一种语言驱动的属性泛化（LDAG）架构，通过利用目标类别的固有属性语言描述来构建鲁棒的支持策略，以解决少样本分割中支持图像参考不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要从支持图像中挖掘参考作为元指导，但由于视觉表示的类内差异，从支持图像中提取的元信息无法为未训练类别提供准确指导。本文认为支持图像的参考可能不是必需的，关键在于为训练和未训练类别提供无偏的元指导。
...
Conclusion: 通过语言驱动的属性泛化方法，可以有效解决少样本分割中支持图像参考不足的问题，为训练和未训练类别提供无偏的元指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有垃圾检测数据集缺乏对垃圾容器溢出的监控，特别是在垃圾车拍摄图像中的容器跟踪和溢出分割任务研究不足。

Method: 构建StreetView-Waste数据集，为三个任务提供基线模型，并提出启发式跟踪方法和基于几何先验的分割框架。

Result: 微调的目标检测器在容器检测上表现良好，但跟踪方法计数误差大；提出的启发式方法将平均绝对计数误差降低79.6%；几何感知策略在轻量模型上将分割mAP@0.5提升27%。

Conclusion: StreetView-Waste为城市垃圾管理的真实世界感知系统研究提供了具有挑战性的基准。
...
TL;DR: VLA-Pruner是一种针对视觉-语言-动作模型的令牌剪枝方法，通过双层级重要性标准和自适应令牌选择策略，在保持语义理解和动作执行能力的同时显著提升计算效率。
...
Method: 提出双层级重要性标准：视觉-语言预填充注意力用于语义相关性，通过时间平滑估计的动作解码注意力用于动作重要性。基于此采用自适应双层级令牌选择策略，在给定计算预算下保留紧凑且信息丰富的视觉令牌。

Result: 实验表明VLA-Pruner在多种VLA架构和多样化机器人任务上实现了最先进的性能。

Conclusion: VLA-Pruner是一种通用即插即用的VLA专用令牌剪枝方法，能够有效平衡语义理解和动作执行需求，显著提升VLA模型的实时部署能力。
...
TL;DR: LLaVA³通过多视角2D图像增强VLM的3D场景理解能力，无需微调，在3D视觉问答和语言定位任务中优于现有2D方法。


<details>
  <summary>Details</summary>
Motivation: 由于3D训练数据有限，而2D数据集丰富，需要开发仅使用2D图像就能提升3D场景理解能力的方法。

Method: 受立体主义画家启发，通过场景的中间多视角3D重建为每个对象生成全方位视觉表示，用这些表示向VLM描述3D场景。
...
TL;DR: 提出了一种基于敏感度剪枝的3D高斯分布压缩方法，能够在保持视觉保真度和前景细节的同时，实现比现有方法更高的压缩比。


<details>
  <summary>Details</summary>
Motivation: 3D高斯分布场景表示虽然能实现实时渲染，但复杂场景包含数百万个高斯分布，导致存储和内存需求过高，限制了在资源受限设备上的应用。现有剪枝方法在高压缩比下会导致视觉质量严重下降。

Method: 提出基于二阶近似的敏感度剪枝评分，计算每个高斯分布空间参数对训练视图重建误差的影响。采用多轮剪枝-优化管道，可直接应用于预训练模型而无需改变训练流程。

Result: 在剪除90%高斯分布后，渲染速度提升3.56倍，同时保持更显著的前景信息，在Mip-NeRF 360、Tanks & Temples和Deep Blending数据集上获得更高的图像质量指标。

Conclusion: 该方法能够在极高压缩比下保持视觉质量，显著提升了3D高斯分布表示在资源受限设备上的实用性。
...
TL;DR: FastSurfer-CC是一个高效、全自动的胼胝体形态测量框架，能够自动识别正中矢状面、分割胼胝体和穹窿，提取多种形态指标，在亨廷顿病研究中显示出优于现有工具的性能。


<details>
  <summary>Details</summary>
Motivation: 胼胝体是脑老化和神经系统疾病研究的关键结构，但在公开可用的工具中缺乏全面自动化的分析流程，需要开发一个高效的自动化框架来填补这一空白。

Method: 开发FastSurfer-CC框架，自动识别正中矢状面切片，分割胼胝体和穹窿，定位前后连合以标准化头部位置，生成厚度分布和细分，提取8个形态指标用于统计分析。

Result: FastSurfer-CC在各项任务上均优于现有专门工具，并在亨廷顿病患者与健康对照的比较中检测到了现有最先进方法未能发现的统计学显著差异。

Conclusion: FastSurfer-CC提供了一个高效、全自动的胼胝体形态测量解决方案，在神经科学研究中具有重要应用价值，特别是在疾病检测方面表现出优越性能。
...
TL;DR: 该论文研究了在遮挡场景下的视频预测问题，通过引入点流和深度图信息来改进视频预测模型在遮挡和背景运动中的表现。


<details>
  <summary>Details</summary>
Motivation: 视频预测是机器人和世界建模等下游应用的基础任务，但遮挡仍然是视频预测中的固有挑战。作者假设提供明确的运动信息（通过点流）和几何结构信息（通过深度图）能够帮助视频预测模型在遮挡和背景运动场景中表现更好。
...
Conclusion: 通过引入点流和深度信息，视频预测模型在遮挡场景下的性能得到显著提升，特别是在处理背景运动方面表现更优。
...
<details>
  <summary>Details</summary>
Motivation: 当前微机器人姿态估计方法严重依赖大型高质量显微镜图像数据集，但由于微机器人制造复杂性和标注工作量大，这些数据难以获取且成本高昂。数字孪生系统为模拟到真实数据增强提供了有前景的路径，但现有技术难以复现复杂的光学显微镜现象。

Method: 提出了一种物理信息深度生成学习框架，首次将基于波动光学的物理渲染和深度对齐集成到生成对抗网络(GAN)中，用于高效合成高保真显微镜图像。

Result: 与纯AI驱动方法相比，结构相似性指数(SSIM)提高了35.6%，同时保持实时渲染速度(0.022秒/帧)。基于合成数据训练的位姿估计器在俯仰/滚转精度上达到93.9%/91.9%，仅比完全使用真实数据训练的估计器低5.0%/5.4%。

Conclusion: 该框架能够泛化到未见过的位姿，无需额外训练数据即可实现数据增强和对新型微机器人配置的鲁棒位姿估计。
...
<details>
  <summary>Details</summary>
Motivation: 乳腺DCE-MRI在肿瘤筛查和评估中很重要，但不同采集协议和个体因素导致组织外观差异大，使自动肿瘤分割面临挑战。

Method: 使用特征线性调制(FiLM)层将采集时间信息融入模型，根据特定采集序列调制模型特征，充分利用每个成像研究中可变数量的图像。

Result: 在大型公共多中心乳腺DCE-MRI数据集上训练，评估显示引入相位采集时间知识改善了肿瘤分割性能和模型泛化能力。
...
TL;DR: 提出了一种联合室内场景建图和天花板相机注册的新方法，通过移动代理的RGB-D相机和同步的CMC视频，实现场景布局与相机位姿的联合优化。


<details>
  <summary>Details</summary>
Motivation: 解决天花板相机在室内场景中注册困难的问题，传统手动注册效率低成本高，而自动视觉定位在视觉模糊时效果不佳。

Method: 使用配备头戴式RGB-D相机的移动代理遍历场景，同步采集CMC视频，通过时间戳关联轨迹，构建因子图进行联合优化。

Result: 实验结果表明该方法不仅能在一个统一框架中有效完成两项任务，还能联合提升它们的性能。

Conclusion: 为下游位置感知应用提供了一个可靠的工具，实现了场景建图和CMC注册的协同增强。
...
<details>
  <summary>Details</summary>
Motivation: 由于动态、非结构化的动作和录制环境的变化，开发稳健的数据集一直是计算机视觉分析格斗运动的主要瓶颈。

Method: 从20个公开YouTube对练视频中提取击打片段，手动分割和标注以确保精确的时间边界和类别一致性，涵盖各种运动风格、摄像机角度和运动员体型。

Result: 创建了一个包含6,915个高质量击打片段的数据集，分为六种击打类型，为实时视觉动作识别研究提供了丰富的基准。

Conclusion: 该数据集旨在加速拳击及相关领域中运动分析、自动化教练和性能评估的进展，特别适用于低资源和不受约束环境下的研究。
...
TL;DR: SemCLIP提出了一种新的CLIP对比损失函数，通过结合释义和否定来增强视觉-语言模型的语义鲁棒性，在保持CLIP性能的同时显著提高了对否定文本的处理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的CLIP模型在处理否定和释义文本时表现不一致，因为否定会以最小的词汇变化彻底改变含义，而释义可能产生完全不同但含义相同的文本表达，这对改进视觉-语言模型的评估和对齐提出了挑战。
...
Conclusion: SemCLIP能够实现对语义变换的显著鲁棒性，在保持CLIP性能的同时提高了对否定文本的处理能力，并扩展到下游任务中表现更好。
...
TL;DR: 提出了一种用于体操裁判的鲁棒多摄像头体操运动员跟踪方法，通过结合体操领域知识解决传统多摄像头三角测量在有限摄像头和复杂场景下的跟踪失败问题。


<details>
  <summary>Details</summary>
Motivation: 体操运动员跟踪面临独特挑战：场地空间限制导致摄像头数量有限；光照变化、背景复杂、服装差异和遮挡等因素导致某些视角检测失败，通常只有两个相对视角能提供有效检测，这使得传统多摄像头三角测量难以准确确定3D轨迹。

Method: 提出级联数据关联范式：当交叉视角检测足够时使用三角测量生成3D轨迹候选；当检测不足时采用射线-平面相交方法，利用体操运动员3D中心在表演过程中通常位于预定义垂直平面的领域知识，生成共面3D轨迹候选来补偿不确定轨迹。

Result: 通过大量实验验证了方法的鲁棒性，在挑战性场景中优于现有方法。该系统已成功应用于最近的体操世锦赛，并获得国际体操联合会的高度认可。

Conclusion: 通过将体操领域知识融入跟踪解决方案，提出的级联数据关联方法有效解决了多摄像头体操运动员跟踪中的挑战，显著减少了跟踪失败，在实际应用中表现出色。
...
Method: 实现Horn-Schunck算法的多分辨率版本，使用双线性插值和延拓技术来改进算法精度和收敛性能。

Result: 研究了这些组合策略在帧间运动估计中的有效性，特别是在不同图像条件下的表现。

Conclusion: 多分辨率Horn-Schunck算法结合插值和延拓技术能够有效提高光流计算的精度和收敛性，在变化图像条件下具有良好的运动估计效果。
...
TL;DR: 提出一种两阶段检测框架，通过对比学习提取图像嵌入特征，结合k-NN分类器进行小样本学习，显著提升合成图像检测和溯源能力，无需频繁重新训练。
...
Method: 第一阶段使用监督对比学习训练视觉深度学习模型提取图像嵌入特征；第二阶段使用k-NN分类器在小样本学习范式下进行分类。

Result: 仅需每类150张图像，检测准确率达到91.3%，相比现有方法提升5.2个百分点；溯源任务中AUC和OSCR分别提升14.70%和4.27%。

Conclusion: 该框架为应对不断发展的生成式AI提供了强大、可扩展的取证溯源系统，无需进行耗时的重新训练。
...
### [85] [EOGS++: Earth Observation Gaussian Splatting with Internal Camera Refinement and Direct Panchromatic Rendering](https://arxiv.org/abs/2511.16542)
*Pierrick Bournez,Luca Savant Aira,Thibaud Ehret,Gabriele Facciolo*

Main category: cs.CV

TL;DR: EOGS++是地球观测高斯溅射框架的扩展，针对卫星图像优化，直接在原始高分辨率全色数据上操作，无需外部预处理。通过集成光流技术和束调整，改进相机姿态估计，并引入早期停止和TSDF后处理，提升重建质量和几何精度。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射作为NeRF的替代方案，在地球观测中展现出训练时间短且重建质量好的优势。但原始EOGS框架依赖外部预处理和优化工具，限制了其效率和准确性。

Method: 1. 直接在原始高分辨率全色数据上操作，避免外部预处理
2. 在训练过程中嵌入束调整，利用光流技术改进相机姿态估计
3. 引入早期停止和TSDF后处理技术

Result: 在IARPA 2016和DFC2019数据集上，EOGS++在重建质量和效率方面达到最先进水平，优于原始EOGS方法和其他基于NeRF的方法。建筑物重建的平均MAE误差从1.33改善到1.19。

Conclusion: EOGS++通过直接处理原始数据、集成束调整和引入多项改进，显著提升了地球观测3D重建的质量和效率，同时保持了高斯溅射的计算优势。
...
### [86] [Progressive Supernet Training for Efficient Visual Autoregressive Modeling](https://arxiv.org/abs/2511.16546)
*Xiaoyue Chen,Yuling Shi,Kaiyuan Li,Huandong Wang,Yong Li,Xiaodong Gu,Xinlei Chen,Mingbao Lin*

Main category: cs.CV

TL;DR: VARiant通过尺度-深度非对称依赖分析，在VAR模型中采用权重共享的子网络架构，实现单一模型支持从高质量到极致效率的灵活部署，显著降低内存消耗和加速推理。


<details>
  <summary>Details</summary>
Motivation: VAR模型的多尺度渐进生成导致累积KV缓存带来巨大内存开销，限制了实际部署。研究发现VAR模型存在尺度-深度非对称依赖特性：早期尺度对网络深度极其敏感，而后期尺度对深度减少具有鲁棒性。

Method: 从30层VAR-d30网络中通过等距采样选择多个子网络（16层到2层），早期尺度使用完整网络处理，后期尺度使用子网络。子网络与完整网络共享权重，实现单一模型内的灵活深度调整。采用渐进式训练策略解决权重共享带来的优化冲突。
...
Conclusion: VARiant的单模型架构支持零成本运行时深度切换，提供了从高质量到极致效率的灵活部署选项，满足多样化应用场景需求。
...
### [87] [Lite Any Stereo: Efficient Zero-Shot Stereo Matching](https://arxiv.org/abs/2511.16555)
...
Method: 设计了紧凑而富有表现力的骨干网络、精心构建的混合成本聚合模块，以及在大规模数据上的三阶段训练策略来弥合模拟到真实的差距。

Result: 在四个广泛使用的真实世界基准测试中排名第一，准确性与最先进的非先验方法相当或更好，同时计算成本不到1%。

Conclusion: 证明了超轻量模型可以同时实现强大的泛化能力和高效率，为高效立体匹配设定了新标准。
...
### [88] [Adaptive Guided Upsampling for Low-light Image Enhancement](https://arxiv.org/abs/2511.16623)
*Angela Vivian Dcosta,Chunbo Song,Rafael Radkowski*

Main category: cs.CV

TL;DR: 提出了自适应引导上采样（AGU）方法，用于低光照图像的高效上采样，能同时优化多个图像质量特性如降噪和锐化。
...
Method: 基于多参数优化，从少量图像对中学习低光照与明亮图像特征之间的关联，使用机器学习方法学习这些特征。

Result: AGU能够使用低质量、低分辨率输入实时渲染高质量图像，在低光照场景下优于现有最先进方法。

Conclusion: AGU方法有效解决了低光照图像引导上采样的挑战，通过多参数优化实现了高质量的实时图像渲染。
...
TL;DR: NutriScreener是一个基于检索增强的多姿态图注意力网络，结合CLIP视觉嵌入、类别增强知识检索和上下文感知，从儿童图像中实现稳健的营养不良检测和人体测量预测，解决泛化性和类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 儿童营养不良仍然是全球危机，现有的筛查方法劳动密集且难以扩展，阻碍了早期干预。

Method: 采用检索增强的多姿态图注意力网络，结合CLIP视觉嵌入、类别增强知识检索和上下文感知技术。

Result: 在临床研究中，医生对其准确性和效率分别评分4.3/5和4.6/5；在2141名儿童数据上训练测试，达到0.79召回率、0.82 AUC，人体测量RMSE显著降低；跨数据集结果显示使用人口统计匹配知识库可提升25%召回率和减少3.5cm RMSE。
...
### [90] [Solving Spatial Supersensing Without Spatial Supersensing](https://arxiv.org/abs/2511.16655)
...
TL;DR: POMA-3D是首个从点图学习的自监督3D表示模型，通过视图到场景对齐策略将2D先验知识转移到3D，并使用POMA-JEPA架构确保多视图几何一致性，在仅使用几何输入的情况下支持多种3D理解任务。
...
Method: 提出POMA-3D模型，使用点图编码3D坐标到结构化2D网格，设计视图到场景对齐策略，引入POMA-JEPA联合嵌入预测架构确保多视图几何一致性，并构建ScenePoint数据集支持大规模预训练。

Result: POMA-3D在3D问答、具身导航、场景检索和具身定位等任务中表现优异，仅使用几何输入即可作为强大的3D理解骨干网络。

Conclusion: POMA-3D通过点图方式成功实现了3D场景理解，为解决3D表示学习中的数据稀缺和预训练先验不足问题提供了有效途径。
...
### [92] [Dataset Distillation for Pre-Trained Self-Supervised Vision Models](https://arxiv.org/abs/2511.16674)
*George Cazenavette,Antonio Torralba,Vincent Sitzmann*

Main category: cs.CV

TL;DR: 提出了一种用于预训练视觉模型线性分类器训练的数据集蒸馏方法Linear Gradient Matching，通过优化合成图像使其在预训练特征提取器中产生的线性分类器梯度与真实数据相似。


<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法主要针对随机初始化模型的训练，而当前最先进的视觉方法越来越多地基于大型预训练自监督模型而非从头训练，因此需要研究如何为这些预训练模型蒸馏数据集。

Method: Linear Gradient Matching方法优化合成图像，使其通过预训练特征提取器后，在线性分类器中产生的梯度与真实数据产生的梯度相似。

Result: 该方法产生的合成数据在所有真实图像基线中表现最佳，且能跨预训练视觉模型泛化，例如使用DINO骨干网络蒸馏的数据集可以训练出具有竞争力的CLIP线性分类器。

Conclusion: 该方法在细粒度分类任务中特别有效，并为模型可解释性提供了有价值的工具，能够预测两个模型嵌入空间的相似性以及模型对对抗数据集中虚假相关性的敏感性。
...
TL;DR: TimeViper是一个混合视觉语言模型，采用Mamba-Transformer混合架构处理长视频，提出TransV模块压缩视觉token信息，可处理超过10,000帧的时长视频。


<details>
  <summary>Details</summary>
Motivation: 处理长视频需要高效的模型架构和有效的长时序上下文处理机制，解决视觉token冗余问题。

Method: 采用混合Mamba-Transformer骨干网络，结合状态空间模型效率和注意力机制表达能力，提出TransV token信息转移模块压缩视觉token。
...
TL;DR: 使用生成式AI技术创建合成烟雾数据集，探索无监督域适应方法进行烟雾分割，并通过风格迁移、GAN和图像抠图提升合成数据真实性，以解决烟雾检测数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 早期野火检测至关重要，但烟雾检测缺乏大规模标注数据集限制了深度学习模型的潜力。

Method: 利用生成式AI合成带标注的烟雾数据集，采用无监督域适应方法进行烟雾分割，并集成风格迁移、GAN和图像抠图技术提升数据真实性。

Result: 通过生成合成数据和域适应方法，能够有效弥合合成数据与真实数据之间的差距。
...
TL;DR: 提出了SAM2S模型，通过DiveMem记忆机制、时序语义学习和抗模糊学习，增强SAM2在手术视频分割中的长时跟踪和零样本泛化能力，在SA-SV基准上达到80.42平均J&F分数。
...
Method: 构建SA-SV手术iVOS基准数据集；提出SAM2S模型，包含DiveMem记忆机制、时序语义学习和抗模糊学习三个核心组件。

Result: 在SA-SV基准上，SAM2S达到80.42平均J&F分数，比原始SAM2提升17.10点，比微调SAM2提升4.11点，同时保持68FPS实时推理。

Conclusion: SAM2S在手术视频分割中实现了显著的性能提升，具有良好的长时跟踪能力和零样本泛化性能。
...
TL;DR: 本文针对长尾分布下的2D目标检测问题，在LVISv1数据集上提出改进的BAGS框架和度量学习方法，实现了24.5%的mAP，达到新的SOTA性能。
...
Method: 使用两阶段Faster R-CNN架构，改进Balanced Group Softmax框架缓解类别不平衡；探索度量学习生成特征嵌入，结合k-NN分类器提升稀有类别分类性能。

Result: 在LVISv1数据集上达到24.5%的mAP，超越此前24.0%的基准，在长尾目标检测中取得新的SOTA。

Conclusion: 所提出的方法能有效提升长尾目标检测性能，特别是对稀有类别的检测效果有明显改善。
...
TL;DR: SAM 3D是一个从单张图像生成3D物体重建的模型，能够预测几何形状、纹理和布局，在自然图像中表现优异，特别是在遮挡和场景杂乱的情况下。


<details>
  <summary>Details</summary>
Motivation: 解决自然图像中由于遮挡和场景杂乱导致的3D重建困难，以及视觉识别线索依赖上下文的问题。

Method: 采用人机协同标注流程获取物体形状、纹理和姿态数据，结合合成预训练和真实世界对齐的多阶段训练框架。

Result: 在真实世界物体和场景的人类偏好测试中，相比近期工作获得至少5:1的胜率。
...
TL;DR: SurvAgent是一个用于多模态生存预测的分层思维链增强多智能体系统，通过整合病理图像和基因数据进行可解释的癌症预后分析。
...
Method: 采用两阶段方法：1) WSI-基因CoT增强案例库构建，通过分层分析病理图像和基因分层分析；2) 基于二分法的多专家智能体推理，通过RAG检索相似案例并整合多模态报告。
...
TL;DR: TRIM是一种后训练方法，通过时间轨迹缩减和空间实例掩码去噪来加速3D高斯扩散模型的推理，同时保持输出质量并支持推理时缩放。
...
Method: 提出轻量级选择器模型评估潜在高斯基元质量以实现早期轨迹缩减，并引入实例掩码去噪来剪枝可学习高斯基元中的冗余背景区域。

Result: 实验表明TRIM显著提高了3D生成的效率和质量。

Conclusion: TRIM在不影响输出质量的情况下有效加速了3D高斯扩散模型的推理过程。
...
### [101] [Teacher-Guided One-Shot Pruning via Context-Aware Knowledge Distillation](https://arxiv.org/abs/2511.16653)
*Md. Samiul Alim,Sharjil Khan,Amrijit Biswas,Fuad Rahman,Shafin Rahman,Nabeel Mohammed*

Main category: cs.CV

TL;DR: 提出了一种教师引导的剪枝框架，将知识蒸馏与重要性评分估计紧密结合，实现一次性全局剪枝，在保持性能的同时高效消除冗余权重


<details>
  <summary>Details</summary>
Motivation: 解决非结构化剪枝需要迭代训练-剪枝-再训练循环带来的计算开销问题

Method: 在重要性评分计算中利用教师模型的梯度信号来识别关键参数，采用一次性全局剪枝策略，剪枝后进行稀疏感知的再训练

Result: 在CIFAR-10、CIFAR-100和TinyImageNet等基准测试中，在高稀疏度下实现了最小性能损失，优于EPG和EPSD等先进方法
...
TL;DR: 提出了一种新的3D层次语义分割框架，通过主分支和辅助判别分支解决多层级冲突和类别不平衡问题，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D层次语义分割方法忽视了多标签学习中的多层级冲突问题，以及跨多个层级的类别不平衡问题，导致模型性能受主要类别主导。

Method: 采用后期解耦的3DHS框架，使用多个解码器配合粗到细的层次引导和一致性；引入基于语义原型的双分支监督机制，学习类别判别性特征并进行相互监督。

Result: 在多个数据集和骨干网络上进行的广泛实验表明，该方法实现了最先进的3DHS性能，其核心组件可作为即插即用增强模块改进先前方法。
...
### [103] [Thinking-while-Generating: Interleaving Textual Reasoning throughout Visual Generation](https://arxiv.org/abs/2511.16671)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉生成方法仅在生成前或生成后进行文本推理，缺乏在生成过程中实时的多模态交互，限制了生成结果的上下文感知和语义丰富性。

Method: 开发了TwiG框架，在视觉内容逐步生成过程中交织文本推理，指导即将生成的局部区域并反思已合成的内容。研究了零样本提示、监督微调（基于TwiG-50K数据集）和强化学习（TwiG-GRPO策略）三种实现方法。
...
Conclusion: PartUV通过部件对齐的UV展开方法有效解决了AI生成网格的问题，实现了更少的图表、更短的接缝和低失真，并支持新的应用如部件特定的多瓦片打包。
...
TL;DR: TriDiff-4D是一个基于扩散模型的4D生成管道，通过三平面重定位生成高质量、时间一致的4D虚拟形象，支持骨架驱动的任意长度4D生成，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有4D生成方法存在的时空不一致性、几何不一致性、感知伪影、运动不规则性、高计算成本和动态控制有限等基本限制问题。

Method: 采用自回归策略生成任意长度的4D序列，每个3D帧通过单一扩散过程合成。首先从文本提示生成规范3D虚拟形象和对应运动序列，然后使用第二个扩散模型根据运动序列为虚拟形象添加动画。

Result: 实验结果表明TriDiff-4D显著优于现有方法，通过消除优化过程将生成时间从数小时减少到数秒，同时大幅改善了复杂运动的生成质量，具有高保真外观和准确的3D几何。

Conclusion: TriDiff-4D在时间一致性、运动准确性、计算效率和视觉保真度方面表现出色，能够从大规模3D和运动数据集中显式学习3D结构和运动先验，实现高质量的骨架驱动4D生成。
...
TL;DR: SceneDesigner是一个用于多物体9自由度姿态控制的图像生成方法，通过分支网络和CNOCS表示实现精确灵活的姿态操控，解决了现有方法在可控性和质量上的限制。


<details>
  <summary>Details</summary>
Motivation: 现有的可控图像生成方法在多物体9D姿态（位置、大小、方向）同时控制方面存在挑战，存在可控性有限和质量下降的问题，需要开发更全面的多物体姿态控制方案。

Method: 提出SceneDesigner方法，在预训练基础模型上加入分支网络，使用新的CNOCS映射表示9D姿态信息，采用两阶段训练策略（包括强化学习微调），并在推理时使用解耦物体采样技术。

Result: 广泛的定性和定量实验表明，SceneDesigner在可控性和质量方面显著优于现有方法，能够有效处理复杂多物体场景的姿态控制。

Conclusion: SceneDesigner通过创新的网络架构、表示方法和训练策略，成功实现了准确灵活的多物体9D姿态控制，为可控图像生成提供了有效的解决方案。
...
Method: 构建包含合成和真实世界图像序列的基准测试，设计多样化的可验证任务，评估六个最先进的视频模型在四个关键维度上的表现。

Result: 评估显示不同视频模型在结构化、空间、模式和物理推理方面存在明显差异，并与强图像模型进行比较，分析了常见幻觉行为以及视频时长对帧链推理的影响。
...
TL;DR: 提出一种简单RL方法，让视觉语言模型根据任务难度自动切换快慢思维模式，在保持高性能的同时显著提高计算效率


<details>
  <summary>Details</summary>
Motivation: 现有视觉推理模型无论问题简单复杂都追求冗长推理链，导致计算成本过高，而人类会根据问题难度分配认知资源

Method: 两阶段方法：第一阶段根据模型输出长度标记快慢思维数据，第二阶段使用GRPO和思维模式标签训练模型发展双模式思维

Result: DualMindVLM显著超越基础模型，性能与最先进视觉推理模型相当，同时保持极高的token效率

Conclusion: 该方法成功实现了根据任务难度自动切换思维模式，为视觉语言模型提供了更高效的推理机制
...
TL;DR: 提出EvoLMM框架，通过两个协作代理（提议者和解决者）实现大型多模态模型的无监督自我进化，无需人工标注数据或奖励模型，在数学推理基准上获得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有大型多模态模型训练依赖人工标注数据或外部奖励模型，限制了自主性和可扩展性。本研究旨在以完全无监督方式提升模型推理能力。

Method: 从单一骨干模型实例化两个协作代理：提议者生成多样化的图像相关问题，解决者通过内部一致性解决问题，通过持续自我奖励过程进行学习。

Result: 以Qwen2.5-VL为基础模型，在ChartQA、MathVista和MathVision等多模态数学推理基准上获得约3%的稳定提升，仅使用原始训练图像。

Conclusion: EvoLMM为完全无监督环境下自我改进的大型多模态模型研究提供了简单有效的基准方法。
...
TL;DR: NoPo-Avatar：从单张或稀疏图像重建可动画3D人体化身，无需任何姿态输入，解决了传统方法依赖准确姿态估计的问题
...
Result: 在THuman2.0、XHuman和HuGe100K数据集上的实验表明，在实际设置（无真实姿态）中优于现有基线，在实验室设置（有真实姿态）中达到可比结果
...
### [112] [Sensorium Arc: AI Agent System for Oceanic Data Exploration and Interactive Eco-Art](https://arxiv.org/abs/2511.15997)
...
Result: 开发了一个能够以海洋视角生成融合科学见解与生态诗意的响应，实现人机生态系统交互的原型系统。

Conclusion: 展示了对话式AI代理在调解人类对环境数据的感知方面的潜力，提出了人机生态系统交互的新范式。
...
### [113] [Majority Rules: LLM Ensemble is a Winning Approach for Content Categorization](https://arxiv.org/abs/2511.15714)
*Ariel Kamen,Yakov Kamen*

Main category: cs.AI

TL;DR: 提出了一个集成大语言模型（eLLM）框架，通过整合多个模型来解决单模型在文本分类中的不一致性、幻觉、类别膨胀和误分类问题，在F1分数上比最强单模型提升高达65%。


<details>
  <summary>Details</summary>
Motivation: 单个大语言模型在文本分类中存在不一致性、幻觉、类别膨胀和误分类等弱点，需要一种更可靠和稳健的解决方案。

Method: 采用集成学习框架，整合十个最先进的大语言模型，基于集体决策的数学模型建立原则性聚合标准，在IAB分层分类法下进行零样本评估。

Result: eLLM框架在8,660个人工标注样本上显著提升性能，达到接近人类专家水平的分类准确率，同时提高了鲁棒性和准确性。
...
### [114] [Graph-Memoized Reasoning: Foundations Structured Workflow Reuse in Intelligent Systems](https://arxiv.org/abs/2511.15715)
*Yash Raj Singh*
...
### [115] [MACIE: Multi-Agent Causal Intelligence Explainer for Collective Behavior Understanding](https://arxiv.org/abs/2511.15716)
...
<details>
  <summary>Details</summary>
Motivation: 随着多智能体强化学习系统在安全关键应用中的使用，理解智能体决策原因和集体行为实现方式变得至关重要。现有可解释AI方法在多智能体环境中存在不足。

Method: 使用结构因果模型、干预反事实和Shapley值，通过干预归因分数分析个体因果贡献，通过协同指标分离集体效应与个体贡献，并生成自然语言叙述的行动解释。

Result: 在四种MARL场景中评估显示：准确的结果归因（平均φ_i=5.07，标准差<0.05），在合作任务中检测到正向涌现（协同指数高达0.461），计算效率高（CPU上每个数据集0.79秒）。

Conclusion: MACIE独特地结合了因果严谨性、涌现量化和多智能体支持，同时保持实际应用的可行性，代表了向可解释、可信赖和可问责的多智能体AI迈出的一步。
...
### [116] [How Modality Shapes Perception and Reasoning: A Study of Error Propagation in ARC-AGI](https://arxiv.org/abs/2511.15717)
...
<details>
  <summary>Details</summary>
Motivation: 当前指令优先系统将网格转换为自然语言或DSL规则执行，但缺乏对编码如何影响模型感知以及如何区分指令错误和执行错误的系统分析。假设不同模态会带来感知瓶颈。

Method: 使用加权集合分歧度量和两阶段推理流程，在九种文本和图像模态中分离感知与推理，测试不同表示方式对模型感知的影响。

Result: 结构化文本能精确定位坐标，图像能捕捉2D形状但对分辨率敏感，文本和图像结合可提升约8个感知点和0.20中位相似度。
...
### [117] [ToolMind Technical Report: A Large-Scale, Reasoning-Enhanced Tool-Use Dataset](https://arxiv.org/abs/2511.15718)
*Chen Yang,Ran Le,Yun Xing,Zhenwei An,Zongchao Chen,Wayne Xin Zhao,Yang Song,Tao Zhang*

Main category: cs.AI

TL;DR: ToolMind是一个大规模、高质量的LLM智能体数据集，包含16万合成数据和20万增强开源数据，通过多智能体框架生成，采用细粒度轮次级验证来确保数据质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要在轨迹层面验证正确性，忽略了轮次级错误会在训练中传播并降低模型性能的问题。高质量轨迹数据的稀缺阻碍了更强LLM智能体的发展。

Method: 基于参数相关性构建函数图，使用多智能体框架模拟真实用户-助手-工具交互，并采用细粒度轮次级过滤来移除错误或次优步骤。
...
### [118] [Chain of Summaries: Summarization Through Iterative Questioning](https://arxiv.org/abs/2511.15719)
...
Conclusion: CoS为网站维护者提供了使内容更易于LLM访问的吸引人选择，同时保留人工监督的可能性。
...
### [119] [Automated Hazard Detection in Construction Sites Using Large Language and Vision-Language Models](https://arxiv.org/abs/2511.15720)
*Islem Sahraoui*
...
Result: 尽管模型规模较小，但Molmo 7B和Qwen2 VL 2B在某些提示配置下表现出竞争力，证明了低资源多模态系统在规则感知安全监控中的可行性。

Conclusion: 多模态AI框架能够有效结合文本和视觉数据识别建筑安全隐患，轻量级开源模型在特定场景下可替代专有模型，为资源受限环境下的安全监控提供了可行方案。
...
### [120] [Spatial Reasoning in Multimodal Large Language Models: A Survey of Tasks, Benchmarks and Methods](https://arxiv.org/abs/2511.15722)
*Weichen Liu,Qiyao Xue,Haoming Wang,Xiangyu Yin,Boyuan Yang,Wei Gao*

Main category: cs.AI

TL;DR: 本文提出从认知角度构建空间智能分类法，按推理复杂度组织任务，连接多个认知功能，分析现有基准、评估方法和提升空间能力的技术。


<details>
  <summary>Details</summary>
Motivation: 现有调查通常基于输入模态分类，但空间能力不仅取决于输入格式。本文旨在从认知角度提供更原则性的跨任务比较，揭示当前模型能力与人类推理之间的关键差距。

Method: 引入基于认知视角的分类法，按推理复杂度组织空间智能任务，将现有基准映射到该分类法中，并分析评估指标和方法论。

Result: 建立了认知视角的空间智能分类框架，明确了当前模型在空间推理方面的局限性，识别了训练型和推理型方法的互补机制。

Conclusion: 通过从认知角度分析任务、基准和最新进展，为研究人员提供对该领域的全面理解和未来研究的可行方向。
...
### [121] [Uncertainty-Resilient Multimodal Learning via Consistency-Guided Cross-Modal Transfer](https://arxiv.org/abs/2511.15741)
*Hyo-Jeong Jang*

Main category: cs.AI

TL;DR: 提出了一种基于一致性引导跨模态迁移的不确定性弹性多模态学习框架，通过跨模态语义一致性实现鲁棒表示学习，在噪声和不完整监督下提升模型稳定性。


<details>
  <summary>Details</summary>
Motivation: 多模态学习面临数据噪声、低质量标签和模态异质性等不确定性挑战，特别是在人机交互场景中数据质量和语义可靠性差异显著，需要开发对不确定性具有弹性的学习方法。

Method: 通过将异质模态投影到共享潜在空间，利用跨模态语义一致性进行鲁棒表示学习，减少模态差距并揭示支持不确定性估计的结构关系，不依赖大规模高质量标注。

Result: 在多模态情感识别基准测试中，一致性引导跨模态迁移显著提升了模型稳定性、判别能力和对噪声或不完整监督的鲁棒性，潜在空间分析显示即使在挑战性条件下也能捕获可靠的跨模态结构。
...
### [122] [Build AI Assistants using Large Language Models and Agents to Enhance the Engineering Education of Biomechanics](https://arxiv.org/abs/2511.15752)
*Hanzhi Yan,Qin Lu,Xianqiao Wang,Xiaoming Zhai,Tianming Liu,He Li*

Main category: cs.AI

TL;DR: 该论文提出使用RAG和MAS技术增强LLM在生物力学教育任务中的表现，通过双模块框架解决概念问题和计算问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在领域特定应用中存在知识鸿沟，在需要多步推理的复杂问题上表现下降，需要开发专门的教育助手来增强本科生物力学课程学习。

Method: 构建双模块框架：1) 使用检索增强生成(RAG)提高概念判断题的准确性和逻辑一致性；2) 构建多智能体系统(MAS)解决需要多步推理和代码执行的计算问题。

Result: RAG显著提升了LLM在概念问题上的表现和稳定性，MAS能够执行多步推理、推导方程、执行代码并为计算任务生成可解释的解决方案。
...
### [123] [Multi-Agent LLM Orchestration Achieves Deterministic, High-Quality Decision Support for Incident Response](https://arxiv.org/abs/2511.15755)
*Philip Drammeh*

Main category: cs.AI

TL;DR: 多智能体编排相比单智能体方法，在事故响应中实现了100%可操作建议率，质量方差为零，是LLM事故响应生产就绪的必要条件。


<details>
  <summary>Details</summary>
Motivation: 现有单智能体LLM方法生成模糊、不可用的建议，无法满足生产系统事故响应的需求。

Method: 开发MyAntFarm.ai框架，通过348次控制试验比较单智能体副驾驶与多智能体系统在相同事故场景下的表现。

Result: 多智能体编排实现100%可操作建议率（单智能体仅1.7%），行动特异性提高80倍，解决方案正确性提高140倍，且质量方差为零。

Conclusion: 多智能体编排不是性能优化，而是LLM事故响应生产就绪的必要条件，引入了决策质量新指标。
...
### [124] [Identifying the Supply Chain of AI for Trustworthiness and Risk Management in Critical Applications](https://arxiv.org/abs/2511.15763)
*Raymond K. Sheh,Karen Geappen*

Main category: cs.AI

TL;DR: 本文提出了一种AI供应链风险分类法，以解决当前AI风险评估中缺乏对数据源、预训练模型、代理等供应链环节系统性评估的问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI风险评估主要关注算法偏见和模型幻觉等问题，但缺乏对AI系统供应链（包括数据源、预训练模型、代理、服务等）的系统性风险评估，这在关键应用领域尤为危险。

Method: 通过调查当前AI风险评估和管理现状，提出专门针对AI供应链实体的分类法，帮助利益相关者系统性地识别和评估组织AI系统的依赖关系。

Result: 开发了一个AI供应链实体分类法，使非AI专家也能"考虑正确的问题"，并系统性地盘点组织AI系统的依赖关系。
...
TL;DR: 比较基于规则的NLP方法和大型语言模型在波兰电子健康记录信息提取中的表现，发现规则方法在年龄性别提取上更准确，而LLM在药物识别上表现更好，建议采用混合方法
...
Method: 使用波兰康复医院电子健康记录，比较基于规则的低计算量方法和LLM在提取人口统计信息、临床发现和处方药物方面的表现，并评估文本标准化缺失和翻译导致的信息损失
...
TL;DR: IMACT-CXR是一个基于多智能体的交互式胸部X光解读教学系统，整合了空间标注、视线分析、知识检索和图像推理功能，通过AutoGen工作流实现智能教学。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够统一空间标注、视线分析、知识检索和图像推理的胸部X光解读教学系统，为医学实习生提供全面的学习支持。

Method: 使用多智能体架构，同时处理学习者的边界框、视线样本和文本观察，通过专门智能体评估定位质量、生成苏格拉底式指导、检索PubMed证据、推荐相似病例，并触发视觉语言推理模型。采用贝叶斯知识追踪维护技能掌握度估计。

Result: 系统展示了响应式教学流程，具有有限延迟、精确的答案泄露控制和可扩展性。初步评估显示在定位和诊断推理方面优于基线方法。

Conclusion: IMACT-CXR成功实现了整合多种功能的胸部X光解读教学系统，为实时住院医师部署提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI系统难以应对现实世界决策中的相互关联挑战，如多目标优化、主动学习、长期规划和空间推理，而现有基准测试无法全面评估这些综合决策能力。

Method: 开发了Mini Amusement Parks模拟器，统一了环境建模、不确定性下的长期后果预测和复杂业务战略运营等挑战，并提供人类基准和LLM智能体评估。

Result: 人类在简单模式下表现优于LLM智能体6.5倍，中等模式下优于9.8倍，揭示了智能体在长期优化、样本效率学习、空间推理和世界建模方面的持续弱点。
...
Method: 提出模态基础推理蒸馏框架，让模型学习生成与音频相关的推理链，真正基于声学特征而非产生脱节的推理。
...
<details>
  <summary>Details</summary>
Motivation: 理解激活引导如何改变语言模型内部机制以提升心智理论能力，特别是探究情感处理与分析推理在其中的作用

Method: 使用对比激活加法(CAA)引导Gemma-3-4B模型，在1000个BigToM信念场景上评估，通过线性探针分析45种认知行为的激活差异

Result: 心智理论任务准确率从32.5%提升至46.7%，情感感知(+2.23)和情感评估(+2.20)激活增强，而质疑(-0.78)和收敛思维(-1.59)等分析过程被抑制
...
TL;DR: 提出了一种基于细粒度不确定性信号的自校正框架，通过强化学习来检测和缓解大语言模型在多步推理中的幻觉问题


<details>
  <summary>Details</summary>
Motivation: 传统方法仅依赖最终答案正确性，无法有效检测推理过程中的幻觉和不忠实推理，需要更细粒度的实时检测机制

Method: 利用自我评估置信度对齐和词元级熵峰值作为不确定性信号，设计复合奖励函数，通过强化学习策略引导模型生成更稳定准确的推理轨迹

Result: 实验表明该方法提高了最终答案准确性和推理校准度，消融实验验证了各信号组件的独立贡献

Conclusion: 该方法不仅改善了结果正确性，还增强了中间推理步骤的一致性和忠实性，使模型更具内省能力
...
TL;DR: JudgeBoard是一个新的评估框架，直接查询模型判断答案正确性，无需额外答案比较。提出MAJ多智能体评判框架，通过多个SLM协作达到LLM级别的评判准确度。


<details>
  <summary>Details</summary>
Motivation: 现有LLM-as-a-judge框架依赖与参考答案或其他候选答案的间接比较，难以完全自动化，对推理输出的细粒度评估支持有限。

Method: 构建任务特定评估排行榜，使用基于准确率的排名和Elo评分系统。提出MAJ框架，利用多个具有不同推理特征的SLM通过协作审议来近似LLM级别的评判准确度。

Result: 实验显示SLM和LLM在独立评判任务中存在显著性能差距，但MAJ框架显著提高了SLM的可靠性和一致性。在MATH数据集上，使用较小模型的MAJ表现与较大模型相当甚至更好。
...
TL;DR: KRAL是一个低成本、可扩展、保护隐私的范式，通过教师模型推理自动提炼知识和推理轨迹，使用启发式学习进行半监督数据增强，并利用智能强化学习共同增强医学知识和推理能力，显著优于传统方法。
...
Method: 使用答案到问题的反向生成自动提炼知识和推理轨迹，采用启发式学习进行半监督数据增强（减少约80%手动标注需求），利用智能强化学习共同增强医学知识和推理能力。
...
TL;DR: 提出了一种结合语义漂移分析和金丝雀基线比较的双重检测系统，用于实时识别被植入后门的大型语言模型，在官方测试模型上达到92.5%准确率。
...
Conclusion: 该方法无需修改模型，首次提供了实用的LLM后门检测解决方案，填补了AI部署中的关键安全漏洞。
...
TL;DR: LLMs在获取正确证据后仍存在推理错误，特别是在临床环境中。研究使用WET指南作为测试平台，发现即使提供权威信息，错误仍然存在。提出了评估推理准确性、一致性和忠实度的框架。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在检索证据后推理能力不足的问题，特别是在需要严格遵循结构化协议的临床环境中，确保输出与指南一致。

Method: 使用Written Exposure Therapy (WET)指南作为测试平台，评估模型对临床医生审核问题的响应，提出评估推理准确性、一致性和忠实度的框架。

Result: 即使提供权威段落，模型错误仍然存在。检索增强生成(RAG)可以约束输出，但安全部署需要像评估检索一样严格评估推理。

Conclusion: 在临床环境中部署LLMs时，需要同时严格评估检索和推理能力，RAG有潜力但不足以确保安全使用。
...
### [135] [MUSEKG: A Knowledge Graph Over Museum Collections](https://arxiv.org/abs/2511.16014)
...
Method: 构建类型化属性图，链接对象、人物、组织和视觉或文本标签，通过符号-神经集成统一博物馆数据，支持自然语言查询。
...
TL;DR: 提出SpellForger游戏，玩家通过自然语言提示创建自定义法术，探索AI作为核心游戏玩法共创工具的应用


<details>
  <summary>Details</summary>
Motivation: 探索AI作为游戏核心玩法共创工具的应用，提供个性化和创造性的游戏体验

Method: 使用监督训练的BERT模型解释玩家提示，将文本描述映射到法术预设并平衡参数，Unity引擎开发，Python AI后端

Result: 预期交付功能原型，实时生成法术，应用于引人入胜的游戏循环，验证AI作为直接游戏机制的使用

Conclusion: AI可以作为直接游戏机制，通过自然语言交互实现玩家创造力的核心体验
...
TL;DR: 提出了一种新的约束规划模型，用于解决考虑最小批次大小的串行批次调度问题，该模型不依赖预定义的虚拟批次集合，而是直接对机器上相同家族作业的序列进行推理，实现了更紧凑的公式化。


<details>
  <summary>Details</summary>
Motivation: 现有CP模型依赖预定义的虚拟批次集合，这会导致维度灾难并增加问题复杂性。在半导体制造等实际应用中，最小批次大小是常见要求，需要更有效的解决方案。

Method: 使用关键对齐参数直接对机器上相同家族作业的序列进行推理，构建紧凑的CP模型，并通过定制搜索阶段和增强约束传播器的推理级别来进一步改进。

Result: 在近5000个实例上的计算实验表明，所提模型在最多100个作业的小到中型实例上表现优越，在最多500个作业、10个家族和10台机器的大规模实例上，能找到比现有方法好25%的解决方案。

Conclusion: 提出的新CP模型在串行批次调度问题中显著优于现有方法，特别是在处理最小批次大小约束时表现出色，为实际工业应用提供了更有效的解决方案。
...
### [138] [Artificial Intelligence and Accounting Research: A Framework and Agenda](https://arxiv.org/abs/2511.16055)
*Theophanis C. Stratopoulos,Victor Xiaoqi Wang*

Main category: cs.AI

TL;DR: 论文提出了一个AI-会计研究的二维分类框架，分析了GenAI和LLMs如何改变会计研究，指出人类研究者在判断力、创造力和理论深度方面的优势，并建议改革博士教育。
...
Method: 提出研究重点（会计中心vsAI中心）和方法论（AI方法vs传统方法）的二维分类框架，应用于IJAIS特刊和主要会计期刊的论文分析。

Result: 发现GenAI虽然民主化某些研究能力，但通过提高对高阶贡献的期望加剧了竞争，人类在判断力、创造力和理论深度方面仍具价值。

Conclusion: 会计研究者应通过战略定位和合作发挥专业优势，改革博士教育以培养比较优势并建立AI素养。
...
### [139] [A Hybrid Proactive And Predictive Framework For Edge Cloud Resource Management](https://arxiv.org/abs/2511.16075)
...
Method: 设计混合架构，将CNN-LSTM模型的时间序列预测嵌入到基于多智能体深度强化学习的编排器状态空间中，使AI管理器能够预见未来并做出更好的长期规划决策。

Result: 测试表明该系统显著优于传统方法，能够有效解决复杂决策问题，同时平衡成本节约、系统健康和应用程序性能等多个目标。

Conclusion: 通过将预测能力直接嵌入强化学习智能体，实现了更智能的资源管理，在成本节约和性能保障之间找到最佳平衡点，为系统提供平滑的前进路径。
...
### [140] [SkyRL-Agent: Efficient RL Training for Multi-turn LLM Agent](https://arxiv.org/abs/2511.16108)
...
TL;DR: SkyRL-Agent是一个高效的多轮长视野智能体训练评估框架，通过异步调度和工具集成优化，训练出SA-SWE-32B软件工程智能体，在SWE-Bench上达到39.4% Pass@1，成本降低2倍以上，并能泛化到其他任务。


<details>
  <summary>Details</summary>
Motivation: 解决多轮长视野智能体训练中的效率问题，提供统一的训练评估框架，降低训练成本并提升性能。

Method: 采用优化的异步流水线调度器（1.55倍加速），结合基于AST的代码搜索工具增强训练配方，使用纯强化学习从Qwen3-32B训练软件工程智能体。

Result: SA-SWE-32B在SWE-Bench Verified上达到39.4% Pass@1，相比之前模型成本降低2倍以上，并能有效泛化到Terminal-Bench、BrowseComp-Plus和WebArena等其他任务。

Conclusion: SkyRL-Agent框架通过高效调度和工具集成显著提升了智能体训练效率和性能，展示了在软件工程及其他智能体任务上的强大泛化能力。
...
### [141] [Multidimensional Rubric-oriented Reward Model Learning via Geometric Projection Reference Constraints](https://arxiv.org/abs/2511.16139)
*Yongnan Jin,Xurui Li,Feng Cao,Liucun Gao,Juanjuan Yao*

Main category: cs.AI

TL;DR: 提出了MR-RML框架，通过GPRC技术解决LLM在医疗领域应用的三大对齐挑战，显著提升了医疗基准测试性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在医疗实践中的关键对齐问题：静态评估与动态临床需求的脱节、难以适应多源医学标准、传统奖励模型无法捕捉多维医疗质量标准。

Method: MR-RML框架包含三个核心创新：维度-场景-学科医学标准系统、独立多维奖励模型、几何投影参考约束，将医学认知逻辑转化为数学正则化。

Result: 在Healthbench基准测试中，相比基础模型Qwen-32B性能提升45%（全子集）和85%（困难子集），在开源LLM中达到SOTA水平。

Conclusion: MR-RML框架有效解决了医疗LLM的对齐问题，为医学AI的实际应用提供了可行方案。
...
### [142] [FOOTPASS: A Multi-Modal Multi-Agent Tactical Context Dataset for Play-by-Play Action Spotting in Soccer Broadcast Videos](https://arxiv.org/abs/2511.16183)
*Jeremie Ochin,Raphael Chekroun,Bogdan Stanciulescu,Sotiris Manitsaris*

Main category: cs.AI

TL;DR: 提出了FOOTPASS数据集，这是首个在足球比赛中进行多模态、多智能体战术背景下的逐场比赛动作识别基准，旨在利用计算机视觉输出和足球战术知识自动生成可靠的逐场比赛数据流。


<details>
  <summary>Details</summary>
Motivation: 当前的动作识别方法不足以构建可靠的逐场比赛数据，通常只能辅助而非完全自动化标注。同时，战术建模、轨迹预测和表现分析等研究都依赖于比赛状态和逐场比赛数据，这促使利用战术知识作为先验来支持基于计算机视觉的预测。

Method: 引入FOOTPASS数据集，支持开发基于球员中心的动作识别方法，这些方法既利用计算机视觉任务（如跟踪、识别）的输出，也利用足球的先验知识（包括长时程的战术规律性）。

Result: 创建了首个针对整个足球比赛的逐场比赛动作识别基准，为数据驱动的体育分析提供了基础。

Conclusion: FOOTPASS数据集能够实现更自动化和可靠的逐场比赛数据提取，这些数据流构成了数据驱动体育分析的重要输入。
...
TL;DR: 本文提出可解释的自动化算法设计愿景，将LLM驱动的算法发现与系统性基准测试相结合，揭示算法性能背后的原因，实现从盲目搜索到可解释、特定类别算法设计的转变。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的自动化算法设计主要关注性能，但缺乏对算法为何有效、哪些组件重要以及设计选择如何与问题结构相关的理解。

Method: 构建三支柱框架：(i) LLM驱动的算法变体发现，(ii) 可解释基准测试以归因性能到组件和超参数，(iii) 问题类别描述符连接算法行为与景观结构。

Result: 形成发现、解释和泛化相互加强的闭环知识循环，实现从盲目搜索到可解释算法设计的转变。
...
### [144] [Multi-Agent Collaborative Reward Design for Enhancing Reasoning in Reinforcement Learning](https://arxiv.org/abs/2511.16202)
...
### [145] [ChemLabs on ChemO: A Multi-Agent System for Multimodal Reasoning on IChO 2025](https://arxiv.org/abs/2511.16205)
*Xu Qiang,Shengyuan Bai,Leqing Chen,Zijing Liu,Yu Li*

Main category: cs.AI

TL;DR: ChemO是一个基于国际化学奥林匹克竞赛的新基准，通过评估等效重构和结构化视觉增强解决化学多模态推理挑战，ChemLabs多智能体框架实现了超越人类金牌水平的性能。


<details>
  <summary>Details</summary>
Motivation: 化学领域缺乏像数学和物理那样的高级AI推理基准，特别是化学特有的多模态符号语言带来了独特挑战。

Method: 提出ChemO基准，包含评估等效重构（AER）将视觉输出问题转化为可计算格式，结构化视觉增强（SVE）分离视觉感知与化学推理能力；开发ChemLabs分层多智能体框架，模拟人类专家协作。

Result: 在顶级多模态模型上的实验显示，SVE与多智能体系统结合带来显著性能提升，最佳配置得分93.6/100，超过估计的人类金牌阈值。

Conclusion: ChemO基准和ChemLabs框架为自动化化学问题解决设立了新的最先进水平，成功解决了化学多模态推理的挑战。
...
TL;DR: 提出自动化流水线从教育文档中提取高质量QA和VQA对，结合布局感知OCR和LLM语义解析，为LLM训练提供真实教育内容替代合成数据。


<details>
  <summary>Details</summary>
Motivation: 现有指令调优和RL数据集成本高且依赖合成样本导致幻觉和多样性有限，而教科书等教育材料包含大量高质量QA内容但难以转化为AI可用的监督数据。

Method: 结合布局感知OCR和基于LLM的语义解析，自动化提取教育文档中的QA和VQA对，解决原始PDF文档到AI就绪监督数据的转换难题。

Result: 实验表明该方法能生成准确、对齐且低噪声的QA/VQA对，适用于多种文档类型。

Conclusion: 该方法实现了真实世界教育内容的大规模利用，为改进推理导向的LLM训练提供了实用的合成数据替代方案。
...
### [147] [Revisiting Fairness-aware Interactive Recommendation: Item Lifecycle as a Control Knob](https://arxiv.org/abs/2511.16248)
*Yun Lu,Xiaoyu Shi,Hong Xie,Chongjun Xia,Zhenhui Gong,Mingsheng Shang*

Main category: cs.AI

TL;DR: 该论文通过引入物品生命周期作为新的控制维度，提出了LHRL框架来平衡推荐系统的公平性和准确性。研究发现短视频平台物品生命周期呈现压缩的三阶段模式，并设计了分层强化学习框架来动态协调公平与用户参与度。


<details>
  <summary>Details</summary>
Motivation: 重新审视公平感知的交互式推荐系统，发现传统四阶段生命周期模型不适用于短视频平台，需要新的方法来动态协调公平性和推荐准确性。

Method: 提出LHRL框架：包含PhaseFormer（结合STL分解和注意力机制的轻量级编码器）用于阶段检测，以及两级HRL代理（高层策略施加阶段感知的公平约束，低层策略优化即时用户参与度）。

Result: 在多个真实交互推荐数据集上的实验表明，LHRL显著提高了公平性和用户参与度。将生命周期感知奖励集成到现有RL模型中也持续带来性能提升。

Conclusion: 生命周期感知方法在推荐系统中具有通用性和实用价值，能够有效协调长期公平和短期效用。
...
TL;DR: 提出了一个多意图科学问答基准MuISQA，并设计了一个意图感知检索框架，通过LLM假设答案、分解意图特定查询，使用RRF重排序来提升多意图问题的证据覆盖率和检索准确性。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统通常面向单一意图，导致多意图科学问题证据覆盖不完整，需要从多个来源获取证据并进行多跳推理。

Method: 使用LLM假设潜在答案，将其分解为意图特定查询，为每个意图检索支持段落，然后通过RRF聚合和重排序以平衡不同意图的覆盖并减少冗余。
...
### [149] [Distributed Agent Reasoning Across Independent Systems With Strict Data Locality](https://arxiv.org/abs/2511.16292)
...
<details>
  <summary>Details</summary>
Motivation: 探索多个组织如何在无需共享敏感数据的情况下进行安全合作，通过分布式代理系统实现隐私保护的跨组织协作。

Method: 使用Orpius平台进行多代理编排、工具执行和隐私保护通信。代理通过OperationRelay调用交换简洁的自然语言摘要，每个代理仅操作自己的本地数据，使用HMAC假名化令牌保护患者身份。

Result: 成功演示了诊所、保险公司和专科医生网络之间的分布式推理可行性，系统能够在不暴露患者身份的情况下完成覆盖评估和适当性推荐。

Conclusion: 该原型证明了分布式多代理系统在保持数据本地化的同时实现跨组织协作的可行性，为未来去中心化多代理系统的研究提供了架构模式和通信流程参考。
...
### [150] [OpenMMReasoner: Pushing the Frontiers for Multimodal Reasoning with an Open and General Recipe](https://arxiv.org/abs/2511.16334)
...
Method: 两阶段训练方法：1) SFT阶段构建87.4万样本的冷启动数据集，进行严格的逐步验证；2) RL阶段使用7.4万样本跨多个领域进一步优化和稳定推理能力。
...
### [151] [Reducing Instability in Synthetic Data Evaluation with a Super-Metric in MalDataGen](https://arxiv.org/abs/2511.16373)
*Anna Luiza Gomes da Silva,Diego Kreutz,Angelo Diniz,Rodrigo Mansilha,Celso Nobre da Fonseca*

Main category: cs.AI

TL;DR: 提出了一种用于评估Android恶意软件合成数据质量的Super-Metric，该指标整合了8个度量标准，生成单一加权分数，比传统指标更稳定和一致。


<details>
  <summary>Details</summary>
Motivation: Android恶意软件领域中，由于现有指标的不稳定性和缺乏标准化，评估合成数据质量一直是一个持续存在的挑战。

Method: 在MalDataGen中集成了一个Super-Metric，该指标聚合了四个保真度维度上的八个度量标准，生成单一加权分数。
...
### [152] [An Agent-Based Framework for the Automatic Validation of Mathematical Optimization Models](https://arxiv.org/abs/2511.16383)
*Alexander Zadorojniy,Segev Wasserkrug,Eitan Farchi*

Main category: cs.AI

TL;DR: 提出了一种基于智能体的优化模型自动验证方法，将软件测试技术应用于优化建模领域，通过多个智能体协作生成测试API、测试用例和模型变异来验证LLM生成的优化模型是否正确。
...
Conclusion: 该智能体集成方法能够有效验证优化模型的正确性，为LLM生成的优化模型提供了可靠的验证机制。
...
### [153] [CorrectHDL: Agentic HDL Design with LLMs Leveraging High-Level Synthesis as Reference](https://arxiv.org/abs/2511.16395)
...
### [154] [Trustworthy AI in the Agentic Lakehouse: from Concurrency to Governance](https://arxiv.org/abs/2511.16402)
*Jacopo Tagliabue,Federico Bianchi,Ciro Greco*

Main category: cs.AI

TL;DR: 论文提出Bauplan设计，通过重新实现数据湖中的数据和计算隔离，为智能体工作流提供正确性和可信性保证。


<details>
  <summary>Details</summary>
Motivation: 大多数企业认为智能体不够可信来处理生产数据，传统数据湖不适合智能体访问模式，需要解决基础设施问题。

Method: 提出Bauplan设计，重新实现数据和计算隔离，采用类似数据库MVCC的操作类比，但针对解耦、多语言环境进行优化。

Result: 实现了自修复管道的参考实现，将智能体推理与正确性和可信性保证无缝结合。

Conclusion: 通过先解决基础设施问题，设计面向智能体的数据湖架构，可以为可信的智能体工作流奠定基础。
...
### [155] [Pharos-ESG: A Framework for Multimodal Parsing, Contextual Narration, and Hierarchical Labeling of ESG Report](https://arxiv.org/abs/2511.16417)
*Yan Chen,Yu Zou,Jialei Zeng,Haoran You,Xiaorui Zhou,Aixi Zhong*

Main category: cs.AI

TL;DR: 提出了Pharos-ESG框架，通过多模态解析将ESG报告转换为结构化表示，解决了ESG报告阅读顺序混乱和结构不明确的问题，并发布了首个大规模ESG报告数据集Aurora-ESG。


<details>
  <summary>Details</summary>
Motivation: ESG报告作为评估企业ESG表现的核心媒介，存在阅读顺序混乱、布局不规则以及内容冗长、结构不明确的问题，难以进行大规模理解。

Method: 采用基于布局流的阅读顺序建模、目录锚点引导的层次感知分割，以及将视觉元素上下文转换为连贯自然语言的多模态聚合管道。

Result: 在标注基准测试中，Pharos-ESG始终优于专用文档解析系统和通用多模态模型。

Conclusion: 该框架为金融治理和决策中的ESG整合提供了更好的支持，并发布了首个涵盖中国大陆、香港和美国市场的大规模ESG报告数据集。
...
### [156] [TOFA: Training-Free One-Shot Federated Adaptation for Vision-Language Models](https://arxiv.org/abs/2511.16423)
*Li Zhang,Zhongxuan Han,XiaoHua Feng,Jiaming Zhang,Yuyuan Li,Linbo Jiang,Jianan Lin,Chaochao Chen*

Main category: cs.AI

TL;DR: 提出了TOFA框架，一种无需训练的one-shot联邦视觉语言模型适应方法，通过视觉和文本双管道处理数据异构性，无需额外训练资源。


<details>
  <summary>Details</summary>
Motivation: 现有联邦VLM适应方法通信成本高且易受攻击，需要开发轻量级one-shot方法。但当前方法面临多模态信息利用不足、数据异构性处理不系统、需要额外训练资源等问题。

Method: TOFA使用视觉和文本双管道：视觉管道通过分层贝叶斯模型学习个性化类别原型分布；文本管道评估并全局对齐本地文本提示；引入自适应权重校准机制平衡个性化和鲁棒性。

Result: 在9个数据集的各种联邦设置下进行广泛实验，证明了TOFA方法的有效性。

Conclusion: TOFA是一种高效、轻量级的训练免费联邦VLM适应框架，能有效处理数据异构性，无需额外训练资源。
...
### [157] [From generative AI to the brain: five takeaways](https://arxiv.org/abs/2511.16432)
*Claudius Gros*

Main category: cs.AI

TL;DR: 生成式AI的进步源于明确的生成原则而非晦涩算法，这些原则可能在大脑中同样存在，对认知神经科学很重要。机器学习研究提供了神经信息处理系统的特征，文中讨论了五个例子说明神经科学能从ML研究中学习。


<details>
  <summary>Details</summary>
Motivation: 探讨生成式AI中的生成原则是否也存在于大脑中，以及机器学习研究如何为神经科学提供新的见解和特征描述。

Method: 分析生成式AI的生成原则，并讨论五个机器学习研究中的特征：世界建模的缺陷、思维过程生成、注意力机制、神经缩放定律和量化。

Result: 识别出生成原则在AI和大脑中的潜在相似性，以及机器学习研究为理解神经信息处理系统提供的五个重要特征。

Conclusion: 神经科学应该从机器学习研究中学习，特别是关于生成原则和神经信息处理系统的特征，这有助于更好地理解大脑的认知功能。
...
### [158] [PersonaDrift: A Benchmark for Temporal Anomaly Detection in Language-Based Dementia Monitoring](https://arxiv.org/abs/2511.16445)
*Joy Lai,Alex Mihailidis*

Main category: cs.AI

TL;DR: PersonaDrift是一个用于评估机器学习方法检测痴呆症患者日常沟通行为渐进变化的合成基准，模拟了60天的交互日志，重点关注情感扁平化和话题偏离两种变化模式。


<details>
  <summary>Details</summary>
Motivation: 痴呆症患者的沟通行为会逐渐变化（如表达减少、重复、话题偏离），但现有计算工具难以追踪这种渐进性行为漂移。

Method: 基于护理人员访谈创建合成用户模型，模拟60天交互日志，注入渐进性变化（情感扁平化和话题偏离），评估多种异常检测方法（统计方法、序列模型、监督分类器）。

Result: 情感扁平化在基线变异度低的用户中可用简单统计模型检测，而语义漂移需要时序建模和个性化基线。个性化分类器在所有任务中都优于通用分类器。

Conclusion: 个性化行为背景对检测痴呆症患者沟通行为变化至关重要，PersonaDrift为这一新兴应用领域提供了可扩展的评估框架。
...
### [159] [Utilizing Large Language Models for Zero-Shot Medical Ontology Extension from Clinical Notes](https://arxiv.org/abs/2511.16548)
...
Method: 利用预训练大语言模型的强大语言理解和广泛生物医学知识，CLOZE有效识别疾病相关概念并捕捉复杂的分层关系，通过自动移除受保护健康信息确保患者隐私。

Result: 实验结果表明，CLOZE提供了一个准确、可扩展且保护隐私的本体扩展框架，在生物医学研究和临床信息学中具有广泛下游应用潜力。

Conclusion: CLOZE框架能够显著增强现有本体的覆盖范围和实用性，为生物医学研究和临床应用提供支持。
...
### [160] [Consciousness in Artificial Intelligence? A Framework for Classifying Objections and Constraints](https://arxiv.org/abs/2511.16582)
...
<details>
  <summary>Details</summary>
Motivation: 为数字意识可能性的辩论提供结构和工具，澄清计算功能主义挑战与数字意识挑战之间的区别。

Method: 开发了基于Marr层级的分类框架，将挑战按粒度层级和力度程度（1-3级）分类，并应用于14个文献案例。

Result: 建立了系统性的分类体系，能够清晰区分不同类型的意识挑战，为相关辩论提供了结构化分析工具。

Conclusion: 该框架有助于澄清数字意识辩论中的概念混淆，为未来讨论提供了更精确的分析基础。
...
### [161] [Formal Abductive Latent Explanations for Prototype-Based Networks](https://arxiv.org/abs/2511.16588)
*Jules Soria,Zakaria Chihani,Julien Girard-Satabin,Alban Grastien,Romain Xu-Darme,Daniela Cancila*

Main category: cs.AI

TL;DR: 本文提出了一种名为Abductive Latent Explanations (ALEs)的形式化方法，用于改进基于案例推理网络的可解释性，通过中间表示层的充分条件来保证预测结果，结合了案例推理模型的固有可解释性和形式化XAI的保证。


<details>
  <summary>Details</summary>
Motivation: 基于案例推理网络虽然被设计为"固有可解释"，但其解释有时会产生误导，特别是在安全关键场景中，不同实例可能导致相同解释但不同预测结果，这限制了其实际应用价值。

Method: 提出了ALE形式化方法，基于三种不同范式开发了无需求解器且可扩展的算法，在标准图像分类和细粒度图像分类任务上验证了方法的可行性。

Result: 在多个数据集上的实验表明，ALE方法能够生成具有形式化保证的解释，提高了基于案例推理网络解释的可靠性和实用性。

Conclusion: ALE方法成功地将案例推理模型的固有可解释性与形式化XAI的保证相结合，为解决基于案例推理网络中解释误导问题提供了有效解决方案。
...
### [162] [D-GARA: A Dynamic Benchmarking Framework for GUI Agent Robustness in Real-World Anomalies](https://arxiv.org/abs/2511.16590)
...
<details>
  <summary>Details</summary>
Motivation: 现有GUI代理训练和评估数据集多为静态理想化环境，无法反映真实世界中的复杂性和不可预测性，特别是异常情况的存在，这阻碍了开发具有人类水平能力的智能代理。

Method: 提出D-GARA框架，引入多种真实世界异常类型（如权限对话框、电池警告、更新提示等），构建包含嵌入式异常的Android应用基准测试集，支持模块化扩展新任务和异常类型。

Result: 实验表明，在异常丰富的环境中，现有最先进的GUI代理性能显著下降，凸显了鲁棒性学习的必要性。

Conclusion: D-GARA框架有效揭示了GUI代理在真实异常环境中的脆弱性，为开发更鲁棒的GUI代理提供了重要评估工具，支持社区进一步研究。
...
### [163] [You Only Forward Once: An Efficient Compositional Judging Paradigm](https://arxiv.org/abs/2511.16600)
*Tianlong Zhang,Hongwei Xue,Shilin Yan,Di Wu,Chen Xu,Yunyun Yang*

Main category: cs.AI

TL;DR: YOFO是一种基于模板的多模态大语言模型评判方法，通过单次前向传播同时验证多个结构化要求，实现数量级的速度提升并保持可解释性。
...
### [164] [Bridging VLMs and Embodied Intelligence with Deliberate Practice Policy Optimization](https://arxiv.org/abs/2511.16602)
...
TL;DR: 提出了DPPO训练框架，通过元认知"Metaloop"动态交替监督微调和强化学习，解决具身智能的数据瓶颈和算法效率问题，在有限数据下实现自动弱点识别和针对性资源分配。
...
Method: DPPO框架，包含元认知"Metaloop"训练机制，动态交替进行监督微调（能力扩展）和强化学习（技能精炼），实现自动弱点识别和针对性资源分配。

Result: 训练出的Pelican-VL 1.0模型相比基础模型性能提升20.3%，在100B参数规模上超越开源模型10.6%。

Conclusion: DPPO是首个系统性解决数据和资源瓶颈的框架，使社区能够高效构建多功能具身智能体。
...
### [165] [MedBayes-Lite: Bayesian Uncertainty Quantification for Safe Clinical Decision Support](https://arxiv.org/abs/2511.16625)
*Elias Hossain,Md Mehedi Hasan Nipu,Maleeha Sheikh,Rajib Rana,Subash Neupane,Niloofar Yousefi*

Main category: cs.AI

TL;DR: MedBayes-Lite是一个轻量级的贝叶斯增强框架，用于提升基于transformer的临床语言模型的可靠性，通过不确定性量化产生不确定性感知的预测，无需重新训练或架构修改。
...
Conclusion: 该框架在医疗AI系统中实现了可靠的不确定性传播，提高了可解释性，证明了其有效性。
...
### [166] [Enhancing Forex Forecasting Accuracy: The Impact of Hybrid Variable Sets in Cognitive Algorithmic Trading Systems](https://arxiv.org/abs/2511.16657)
*Juan C. King,Jose M. Amigo*

Main category: cs.AI

TL;DR: 实现了一个基于人工智能的高频算法交易系统，专门用于EUR-USD货币对，整合了基本面和技术面分析特征，评估了预测准确性和交易盈利能力。


<details>
  <summary>Details</summary>
Motivation: 研究旨在确定在EUR-USD高频交易中，基本面分析和技术面分析哪类特征能提供更可靠和更有利可图的交易信号预测能力。

Method: 整合了全面的输入特征集：欧元区和美国的关键宏观经济基本面变量（如GDP、失业率），以及技术分析变量（指标、振荡器、斐波那契水平、价格背离）。使用标准机器学习指标评估预测准确性，并通过历史数据回测模拟评估交易盈利能力和风险。

Result: 通过机器学习指标和回测模拟量化了算法的预测准确性和交易盈利能力，进行了比较分析。
...
### [167] [Cognitive Foundations for Reasoning and Their Manifestation in LLMs](https://arxiv.org/abs/2511.16660)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂问题上表现良好但在简单变体上失败，表明它们通过不同于人类推理的机制获得正确输出。研究旨在理解模型与人类推理的根本差异，为开发基于认知机制的模型奠定基础。

Method: 构建包含28个认知元素的分类法，分析17个模型的17万条推理轨迹和54条人类思维轨迹，进行元分析研究1598篇LLM推理论文，开发测试时推理指导方法。

Result: 发现人类使用层次嵌套和元认知监控，模型依赖浅层前向链式推理；研究社区关注易量化行为而忽视元认知控制；开发的推理指导方法在复杂问题上性能提升高达60%。

Conclusion: 通过连接认知科学和LLM研究，为开发基于认知机制而非脆弱捷径或记忆的模型奠定了基础，为改进模型能力和大规模测试人类认知理论开辟了新方向。
...
### [168] [Extending Test-Time Scaling: A 3D Perspective with Context, Batch, and Turn](https://arxiv.org/abs/2511.15738)
*Chao Yu,Qixin Tan,Jiaxuan Gao,Shi Yu,Hong Lu,Xinting Yang,Zelai Xu,Yu Wang,Yi Wu,Eugene Vinitsky*

Main category: cs.LG

TL;DR: 提出了3D测试时扩展框架，通过上下文长度、批处理和轮次三个维度的扩展来增强推理模型的测试时性能，突破了传统上下文长度扩展的限制。


<details>
  <summary>Details</summary>
Motivation: 测试时扩展（如R1和o1模型）受限于基础模型的有限上下文长度，远小于训练时消耗的token数量，需要探索新的测试时增强方法。

Method: 提出三维测试时扩展框架：上下文扩展（增加推理上下文长度）、批处理扩展（并行采样提高准确率）、轮次扩展（迭代自优化提升推理质量）。

Result: 每个维度都表现出测试时扩展效应但容量有限；三个维度结合显著提升IOI、IMO、CPHO等挑战性基准的推理性能，并能从人类偏好反馈中获益；该框架可扩展到具身学习领域。

Conclusion: 多维度测试时扩展能够有效突破单一上下文长度扩展的限制，显著提升推理模型的性能，并在具身学习等开放领域具有应用潜力。
...
### [169] [Connecting the Dots: A Machine Learning Ready Dataset for Ionospheric Forecasting Models](https://arxiv.org/abs/2511.15743)
...
TL;DR: 提出了一个用于电离层预报的开放数据集和建模流程，整合了多种电离层和太阳层观测数据，并训练了时空机器学习模型来预测垂直总电子含量。


<details>
  <summary>Details</summary>
Motivation: 电离层业务预报面临观测稀疏、各层耦合复杂以及GNSS、通信、航空安全和卫星运营对及时准确预测的需求增长等挑战。

Method: 整合太阳动力学观测站数据、太阳辐照度指数、太阳风参数、地磁活动指数、全球电离层TEC地图等多种数据源，构建统一的机器学习就绪数据集，并训练时空机器学习架构。

Result: 创建了一个新颖的异构数据集，支持物理和数据驱动建模，能够预测安静和地磁活跃条件下的垂直TEC。

Conclusion: 该工作提供了一个全面的数据集和建模流程，不仅支持电离层动力学研究，还支持更广泛的日地相互作用探索，有助于科学研究和业务预报工作。
...
### [170] [TB or Not TB: Coverage-Driven Direct Preference Optimization for Verilog Stimulus Generation](https://arxiv.org/abs/2511.15767)
...
<details>
  <summary>Details</summary>
Motivation: 硬件设计验证是最耗时耗资源的阶段，生成有效的测试激励既关键又费力，而LLMs为此提供了新的解决方案。

Method: 使用覆盖率驱动的直接偏好优化(CD-DPO)微调LLM，基于PairaNet数据集（包含根据仿真覆盖率标注的高/低质量测试平台对）进行偏好训练。

Result: 在CVDP CID12基准测试中，TB or not TB优于开源和商业基线，代码覆盖率提升高达77.27%。

Conclusion: 覆盖率驱动的偏好优化对基于LLM的硬件验证非常有效，能够生成最大化验证覆盖率的激励。
...
### [171] [TopoReformer: Mitigating Adversarial Attacks Using Topological Purification in OCR Models](https://arxiv.org/abs/2511.15807)
...
<details>
  <summary>Details</summary>
Motivation: 现有防御方法存在模型特定、计算昂贵、影响未扰动输入性能等问题，且对未见或自适应攻击仍然脆弱。需要一种能有效缓解对抗性扰动而不损害文本结构的方法。
...
Conclusion: TopoReformer通过拓扑特征有效缓解对抗性扰动，保持文本结构完整性，提供了一种模型无关的鲁棒防御解决方案。
...
### [172] [Beyond Tsybakov: Model Margin Noise and $\mathcal{H}$-Consistency Bounds](https://arxiv.org/abs/2511.15816)
*Mehryar Mohri,Yutao Zhong*

Main category: cs.LG

TL;DR: 提出了新的Model Margin Noise (MM噪声)假设，该假设比Tsybakov噪声条件更弱，并在此条件下推导了增强的H-一致性边界，适用于二元和多类分类问题。


<details>
  <summary>Details</summary>
Motivation: 现有的Tsybakov噪声条件可能过于严格，需要更弱的噪声假设来获得更好的理论保证。MM噪声假设依赖于假设与贝叶斯分类器之间的差异，而非分布的内在最小边界。

Method: 引入MM噪声假设，该假设被Tsybakov噪声条件蕴含但可能在其失效时仍然成立。基于此假设推导增强的H-一致性边界，并实例化到常见的替代损失函数族。

Result: 获得了与Mao等人相同的有利指数但假设更弱的增强H-一致性边界，在中间噪声水平下平滑插值于线性和平方根机制之间。

Conclusion: MM噪声假设为分类问题提供了更弱且实用的噪声条件，能够获得改进的理论保证，扩展了现有H-一致性边界理论。
...
### [173] [Attention-Based Feature Online Conformal Prediction for Time Series](https://arxiv.org/abs/2511.15838)
*Meiyi Zhu,Caili Guo,Chunyan Feng,Osvaldo Simeone*

Main category: cs.LG

TL;DR: AFOCP是一种改进的在线共形预测方法，通过在特征空间操作和使用注意力机制，显著减小预测区间大小，同时保持覆盖保证。


<details>
  <summary>Details</summary>
Motivation: 标准在线共形预测存在两个关键限制：在输出空间使用简单的非共形分数，以及统一处理所有历史观测值来估计分位数。

Method: AFOCP引入两个关键创新：1) 在预训练神经网络的特征空间操作，利用学习到的表示构建更紧凑的预测集；2) 使用注意力机制自适应加权历史观测值，有效处理非平稳性和分布偏移。

Result: 实验表明AFOCP相比标准OCP可将预测区间大小减少高达88%，同时保持目标覆盖水平。
...
### [174] [Transparent Early ICU Mortality Prediction with Clinical Transformer and Per-Case Modality Attribution](https://arxiv.org/abs/2511.15847)
*Alexander Bakumenko,Janine Hoelscher,Hudson Smith*

Main category: cs.LG

TL;DR: 提出了一种轻量级、透明的多模态集成方法，结合生理时间序列数据和临床笔记，用于ICU患者院内死亡风险预测，具有多级可解释性。
...
Result: 在MIMIC-III基准测试中，多模态集成比最佳单模型表现更好（AUPRC 0.565 vs. 0.526；AUROC 0.891 vs. 0.876），且保持良好校准的预测。
...
### [175] [discretize_distributions: Efficient Quantization of Gaussian Mixtures with Guarantees in Wasserstein Distance](https://arxiv.org/abs/2511.15854)
...
<details>
  <summary>Details</summary>
Motivation: 为控制与验证管道提供高效的高斯混合分布离散化工具，支持网络物理系统的应用需求。

Method: 实现了最先进的高斯混合模型量化方法，扩展了可扩展性，并集成了sigma-point方法等补充量化策略，提供模块化接口。

Result: 在各种高维、大规模和退化高斯混合模型示例上的基准测试表明，该包能以低计算成本产生准确的近似。

Conclusion: discretize_distributions包提供了高效准确的高斯混合分布离散化解决方案，适用于网络物理系统的控制与验证应用。
...
### [176] [GLOBE: Accurate and Generalizable PDE Surrogates using Domain-Inspired Architectures and Equivariances](https://arxiv.org/abs/2511.15856)
*Peter Sharpe*

Main category: cs.LG

TL;DR: GLOBE是一种用于齐次PDE的新型神经代理模型，结合边界元方法和等变机器学习，在AirFRANS数据集上相比基线模型实现了200倍的精度提升，具有紧凑的参数规模和任意点推理能力。
...
Method: 使用可学习的类格林函数核从边界面到目标点进行叠加表示，通过多尺度分支和通信超层组合，构建具有平移、旋转和宇称等变性、离散化不变性和单位不变性的架构。

Result: 在AirFRANS数据集上，相比参考基线模型平均平方误差降低约200倍，相比次优模型降低约50倍；在稀缺数据分割上，速度和压力场误差降低100倍以上，表面压力误差降低600倍以上。
...
### [177] [Global Resolution: Optimal Multi-Draft Speculative Sampling via Convex Minimization](https://arxiv.org/abs/2511.15898)
*Rahul Krishna Thomas,Arka Pal*

Main category: cs.LG

TL;DR: 本文提出了一种新的多草稿推测采样算法，通过将最优传输问题重新表述为最大流问题，并利用多面体理论将其简化为最多V个变量的凸优化问题，实现了90%的接受率和低于100ms的开销。


<details>
  <summary>Details</summary>
Motivation: 推测采样通过使用廉价的草稿模型来减少自回归解码的延迟，但传统的多草稿方法中寻找最优传输解的计算复杂度很高（V^n变量），难以实际应用。

Method: 将最优传输线性规划问题重新表述为最大流问题，应用多面体理论将其简化为最多V个变量的凸优化问题，开发了可调精度的算法。

Result: 实现了90%的接受率，每个生成token的开销低于100ms，且与目标模型分布的偏差可忽略不计。

Conclusion: 该方法首次实现了高效的多草稿推测采样，显著提升了解码效率，为实际应用提供了可行的解决方案。
...
### [178] [Unified all-atom molecule generation with neural fields](https://arxiv.org/abs/2511.15906)
...
Method: 使用神经场将分子表示为连续原子密度，采用基于分数的生成模型，并利用计算机视觉文献中的现代架构。这种模态无关的表示允许单一统一模型在多样化原子系统上训练。

Result: FuncBind在生成小分子、大环肽和抗体互补决定区环方面取得了有竞争力的计算机模拟性能，并通过重新设计两个共晶结构的CDR H3环生成了体外新型抗体结合物。

Conclusion: FuncBind提供了一个统一的框架，能够处理各种分子系统，包括非标准氨基酸，并引入了新的大环肽生成数据集和基准测试。
...
### [179] [AccelOpt: A Self-Improving LLM Agentic System for AI Accelerator Kernel Optimization](https://arxiv.org/abs/2511.15915)
...
Result: 在Trainium 1上平均峰值吞吐量从49%提升到61%，在Trainium 2上从45%提升到59%；使用开源模型时，与Claude Sonnet 4效果相当但成本降低26倍。
...
### [180] [Breaking the Bottleneck with DiffuApriel: High-Throughput Diffusion LMs with Mamba Backbone](https://arxiv.org/abs/2511.15927)
*Vaibhav Singh,Oleksiy Ostapenko,Pierre-André Noël,Torsten Scholak*

Main category: cs.LG

TL;DR: DiffuApriel是一个基于双向Mamba骨干的掩码扩散语言模型，结合扩散目标与线性时间序列建模，在保持性能的同时实现了比基于Transformer的扩散模型高达4.4倍的推理吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 基于扩散的语言模型虽然前景广阔，但依赖Transformer骨干网络导致推理效率受限，主要由于二次注意力计算和KV缓存开销。

Method: 使用双向Mamba骨干网络构建掩码扩散语言模型，结合扩散目标与线性时间序列建模；还提出了混合变体DiffuApriel-H，交错使用注意力和Mamba层。

Result: DiffuApriel在保持与基于Transformer的扩散模型相当性能的同时，在长序列上实现了高达4.4倍的推理吞吐量提升（1.3B模型）；混合变体提供高达2.6倍的吞吐量改进，同时平衡全局和局部上下文建模。
...
### [181] [iLTM: Integrated Large Tabular Model](https://arxiv.org/abs/2511.15941)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习的进展尚未完全应用于表格数据领域，梯度提升决策树(GBDT)仍然是实际应用中的默认选择，需要开发能够超越GBDT性能的深度表格模型。

Method: 提出iLTM集成架构，结合树衍生嵌入、维度无关表示、元训练超网络、多层感知机和检索机制，在1800多个异构分类数据集上进行预训练。

Result: iLTM在表格分类和回归任务中表现一致优越，从小型数据集到大型高维任务都能取得优异性能，轻量微调后能迁移到回归目标，匹配或超越强基线方法。

Conclusion: iLTM通过弥合基于树的方法和神经方法之间的差距，为表格基础模型提供了一个新框架，实现稳健、适应性强且可扩展的表格学习。
...
### [182] [Self-supervised and Multi-fidelity Learning for Extended Predictive Soil Spectroscopy](https://arxiv.org/abs/2511.15965)
...
TL;DR: 提出了一种基于潜在空间嵌入的自监督机器学习框架，用于多保真度学习和扩展的预测性土壤光谱分析，通过变分自编码器在大规模MIR光谱库上预训练，实现NIR到MIR光谱转换，提升土壤属性预测性能。


<details>
  <summary>Details</summary>
Motivation: 利用大规模MIR光谱库的预测能力，结合低成本便携式NIR扫描仪，通过自监督学习构建统一的光谱潜在空间，解决当前NIR库中土壤属性代表性不足的问题。

Method: 使用变分自编码器在大规模MIR光谱库上进行自监督预训练，生成压缩潜在空间；利用配对NIR-MIR光谱子集训练NIR编码器，实现光谱转换；下游机器学习模型映射光谱、预测光谱和潜在空间嵌入到九个土壤属性。

Result: 与基线模型相比，所提出的SSML及其嵌入在所有土壤属性预测任务中获得了相似或更好的准确性；光谱转换（NIR到MIR）的预测性能虽未达到原始MIR光谱水平，但优于或相当于仅使用NIR的模型。

Conclusion: 统一的光谱潜在空间能够有效利用更大、更多样化的MIR数据集，预测当前NIR库中代表性不足的土壤属性，为便携式NIR扫描仪提供了增强的预测能力。
...
### [183] [Machine Learning Epidemic Predictions Using Agent-based Wireless Sensor Network Models](https://arxiv.org/abs/2511.15982)
*Chukwunonso Henry Nwokoye,Blessing Oluchi,Sharna Waldron,Peace Ezzeh*

Main category: cs.LG

TL;DR: 该研究使用基于代理的SEIRV模型和机器学习算法预测无线传感器网络中的病毒传播，发现随机森林、XGBoost等算法在感染和恢复节点预测方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 无线传感器网络中缺乏流行病学数据，难以构建稳健的模型来预测和缓解病毒威胁，需要开发有效的预测方法。

Method: 采用基于代理的SEIRV数学模型，使用NetLogo和Python生成流行病合成数据集，应用多种机器学习算法作为回归问题进行预测。

Result: 预测效果良好，训练集R^2值高达0.997-1.000，验证集为0.971-0.999。随机森林、XGBoost、决策树和k近邻算法表现最佳。

Conclusion: 基于代理的SEIRV模型结合机器学习算法能够有效预测无线传感器网络中的病毒传播动态，为网络安全威胁建模提供了可行方案。
...
### [184] [Descend or Rewind? Stochastic Gradient Descent Unlearning](https://arxiv.org/abs/2511.15983)
*Siqiao Mu,Diego Klabjan*

Main category: cs.LG

TL;DR: 本文证明了随机R2D和D2D机器遗忘算法在强凸、凸和非凸损失函数下的(ε, δ)认证遗忘保证，通过分析扰动梯度系统实现遗忘。


<details>
  <summary>Details</summary>
Motivation: 机器遗忘算法旨在无需从头重新训练就能移除选定训练数据对模型的影响，但现有的随机版本缺乏理论支持，特别是在非凸函数上。

Method: 通过分析扰动或有偏梯度系统（收缩、半收缩或扩张），使用最优耦合技术结合松弛高斯机制来证明遗忘保证。

Result: D2D在强凸函数上能获得比R2D更严格的保证，而R2D在凸和非凸设置下也能实现遗忘，因为它通过逆转累积扰动使遗忘模型更接近重新训练模型。

Conclusion: 两种算法在不同场景下各有优势：D2D适用于强凸函数，R2D适用于凸和非凸函数，为机器遗忘提供了理论保障。
...
### [185] [Synergizing Deconfounding and Temporal Generalization For Time-series Counterfactual Outcome Estimation](https://arxiv.org/abs/2511.16006)
...
<details>
  <summary>Details</summary>
Motivation: 从时间序列观测中估计反事实结果对于有效决策至关重要，但由于反事实轨迹从未被观察到，且混杂因素随时间演变并在每一步扭曲估计，这仍然是一个重大挑战。

Method: SGA使用迭代治疗无关聚类识别细粒度子治疗组，通过对齐这些组实现更好的分布匹配；RTM通过在训练期间用高斯噪声随机替换输入协变量，促进时间泛化。
...
### [186] [Physics-Guided Inductive Spatiotemporal Kriging for PM2.5 with Satellite Gradient Constraints](https://arxiv.org/abs/2511.16013)
...
TL;DR: SPIN是一个用于PM2.5高分辨率制图的时空物理引导推理网络，通过将卫星AOD数据重新定义为损失函数中的空间梯度约束，克服了传统方法因数据缺失和反演偏差带来的限制。


<details>
  <summary>Details</summary>
Motivation: 传统PM2.5制图方法受限于地面监测网络的空间稀疏性，以及卫星AOD数据因云层覆盖和夜间观测导致的严重非随机缺失问题。

Method: 提出SPIN框架，通过并行图核显式建模物理平流和扩散过程，将AOD重新定义为损失函数中的空间梯度约束而非直接输入。

Result: 在京津冀及周边地区验证，SPIN实现了9.52 ug/m³的平均绝对误差，在无监测区域也能生成连续且物理合理的污染场。

Conclusion: SPIN为细粒度环境管理提供了一个稳健、低成本、全天候的解决方案。
...
### [187] [CARE: Turning LLMs Into Causal Reasoning Expert](https://arxiv.org/abs/2511.16016)
*Juncheng Dong,Yiling Liu,Ahmed Aloui,Vahid Tarokh,David Carlson*

Main category: cs.LG

TL;DR: 论文提出CARE框架，通过监督微调教LLMs有效利用因果发现算法输出，显著提升因果推理能力


<details>
  <summary>Details</summary>
Motivation: LLMs在因果发现任务中主要依赖变量语义而忽略观测数据，无法有效利用传统因果发现算法提供的统计信息

Method: CARE框架：先用传统因果发现算法处理观测数据生成输出，然后通过监督微调教LLMs有效利用这些算法线索

Result: 微调后的Qwen2.5-1.5B模型显著优于传统因果发现算法和参数多千倍的最先进LLMs

Conclusion: CARE框架成功提升了LLMs的因果推理能力，使其能够有效结合自身知识和外部算法线索
...
### [188] [HGCN2SP: Hierarchical Graph Convolutional Network for Two-Stage Stochastic Programming](https://arxiv.org/abs/2511.16027)
...
<details>
  <summary>Details</summary>
Motivation: 解决两阶段随机规划问题中场景数量多导致求解困难的问题。现有方法依赖聚类或蒙特卡洛采样，未能深入整合场景信息，且忽略了场景顺序对求解时间的重大影响。

Method: 开发HGCN2SP模型，采用分层图结构编码每个场景并建模层次关系。在强化学习范式下训练，利用求解器反馈。策略网络配备分层图卷积网络进行特征编码，以及基于注意力的解码器进行场景选择和排序。

Result: 在两个经典两阶段随机规划问题上的评估表明，HGCN2SP能够在短时间内提供高质量决策。模型在处理大规模实例时表现出卓越的泛化能力，即使在训练阶段未见过的变量或场景数量显著增加的情况下。

Conclusion: HGCN2SP通过深度整合场景信息和优化求解顺序，有效解决了大规模两阶段随机规划问题的计算挑战，展现出强大的实用性和泛化能力。
...
### [189] [Agent0: Unleashing Self-Evolving Agents from Zero Data via Tool-Integrated Reasoning](https://arxiv.org/abs/2511.16043)
...
<details>
  <summary>Details</summary>
Motivation: 现有的LLM智能体训练依赖人工数据，限制了可扩展性；而现有的自我进化框架受限于模型固有能力和单轮交互，难以发展涉及工具使用或动态推理的复杂课程。

Method: 建立两个从同一基础LLM初始化的智能体之间的共生竞争：课程智能体提出日益挑战的前沿任务，执行智能体学习解决这些任务。通过外部工具集成增强执行能力，反过来推动课程智能体构建更复杂的工具感知任务。
...
### [190] [Change-of-Basis Pruning via Rotational Invariance](https://arxiv.org/abs/2511.16061)
...
<details>
  <summary>Details</summary>
Motivation: 结构化剪枝的有效性取决于重要性在表示空间中的分布。基变换剪枝通过正交线性变换将重要性集中在某些维度上，但标准深度学习架构对此类变换不具固有不变性。

Method: 引入TSRA激活函数家族，该函数在其两个激活子空间内对正交线性变换具有不变性，使得基变换能够合并到周围权重中而不增加参数。

Result: 在CIFAR-10上的VGG-16实验中，CoB+TSRA框架在固定比例结构化剪枝下，相比TSRA基线在所有剪枝比例下都提高了精度，并将可靠剪枝前沿从约30%扩展到70%参数。在基于阈值的剪枝策略下，能够剪除90-96%参数，同时保持1-6%的精度下降。
...
### [191] [Gauge-Equivariant Graph Networks via Self-Interference Cancellation](https://arxiv.org/abs/2511.16062)
*Yoonhyuk Choi,Chong-Kwon Kim*

Main category: cs.LG

TL;DR: 提出GESC模型，通过投影干扰机制和相位感知门控解决GNN在异配图上的性能问题，在多个图基准测试中优于现有方法


<details>
  <summary>Details</summary>
Motivation: 解决图神经网络在异配图上由于自增强和相位不一致信号导致的性能下降问题

Method: 使用U(1)相位连接和秩-1投影替代加性聚合，引入符号和相位感知门控来调节邻居影响

Result: 在多样化的图基准测试中持续优于最新的最先进模型

Conclusion: GESC提供了一种统一的、干扰感知的消息传递视角，有效提升了GNN在异配图上的性能
...
### [192] [ILoRA: Federated Learning with Low-Rank Adaptation for Heterogeneous Client Aggregation](https://arxiv.org/abs/2511.16069)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习中的LoRA方法在客户端异构性下面临三个关键问题：随机初始化导致子空间不对齐、不同秩参数聚合产生偏差、非IID数据加剧客户端漂移，这些问题影响模型性能和泛化能力。

Method: 提出ILoRA框架，包含三个核心创新：QR正交初始化确保客户端在一致子空间开始；拼接QR聚合机制通过拼接和分解融合异质秩更新；AdamW优化器配合秩感知控制变量校正本地更新。

Result: 理论收敛保证支持，在视觉和NLP基准测试上的广泛实验表明，ILoRA相比现有联邦LoRA方法持续实现更优的准确性和收敛稳定性。

Conclusion: ILoRA通过统一的框架有效解决了联邦LoRA中的关键挑战，为异构客户端环境下的参数高效微调提供了可靠解决方案。
...
### [193] [A Mathematical Framework for Custom Reward Functions in Job Application Evaluation using Reinforcement Learning](https://arxiv.org/abs/2511.16073)
*Shreyansh Jain,Madhav Singhvi,Shreya Rahul Jain,Pranav S,Dishaa Lokesh,Naren Chittibabu,Akash Anandhan*

Main category: cs.LG

TL;DR: 本文提出了一种基于小型语言模型的两步微调方法，通过GRPO强化学习优化简历评估模型，解决了传统ATS系统过于依赖关键词匹配的问题，最终在测试数据上达到91%的准确率。


<details>
  <summary>Details</summary>
Motivation: 传统申请人跟踪系统(ATS)过于依赖关键词匹配，会因微小的语义不匹配而拒绝有才华的候选人，需要更精细的简历评估方法。

Method: 采用两步微调过程：首先进行监督微调(SFT)建立基线模型，然后使用GRPO强化学习在自定义多组件奖励函数下优化模型，通过反复精炼奖励函数和训练超参数解决奖励黑客问题。

Result: 最终模型在未见测试数据上达到91%准确率，对'SELECTED'类别的召回率为0.85，精确度为1.0，显示出优秀的候选人识别能力和可靠性。

Conclusion: 经过适当执行的两步微调程序可以有效优化小型语言模型，使其能够进行精细且类似人类的候选人评分，克服传统ATS和简单RL使用的局限性。
...
### [194] [L-JacobiNet and S-JacobiNet: An Analysis of Adaptive Generalization, Stabilization, and Spectral Domain Trade-offs in GNNs](https://arxiv.org/abs/2511.16081)
*Huseyin Goksu*

Main category: cs.LG

TL;DR: 提出了两种基于雅可比多项式的谱GNN模型：L-JacobiNet（自适应版本）和S-JacobiNet（静态稳定版本），发现[-1,1]域提供更好的数值稳定性，而[0,∞)域更适合异质性建模，且ChebyNet的主要问题是稳定性而非静态特性。


<details>
  <summary>Details</summary>
Motivation: 解决传统谱GNN（如ChebyNet）因静态低通滤波器设计导致的异质性和过平滑问题，探索自适应正交多项式滤波器作为解决方案。

Method: 引入两种在[-1,1]域操作的模型：L-JacobiNet（可学习alpha、beta形状参数的自适应ChebyNet泛化）和S-JacobiNet（LayerNorm稳定的静态ChebyNet基线），并与[0,∞)域的AOPF模型进行比较分析。

Result: 发现[0,∞)域在建模异质性方面更优，而[-1,1]域（雅可比）在高K值（K>20）时提供更好的数值稳定性；静态S-JacobiNet在5个基准数据集中的4个上优于自适应L-JacobiNet。
...
### [195] [AssayMatch: Learning to Select Data for Molecular Activity Models](https://arxiv.org/abs/2511.16087)
*Vincent Fan,Regina Barzilay*

Main category: cs.LG

TL;DR: AssayMatch是一个用于药物发现的数据选择框架，通过数据归因方法量化每个训练实验对模型性能的贡献，利用文本描述嵌入来筛选更同质的训练数据，提高模型预测能力。


<details>
  <summary>Details</summary>
Motivation: 药物发现中机器学习模型的性能高度依赖训练数据质量。由于数据集规模限制，模型通常从多个来源聚合生物活性数据，但这会因实验协议差异引入显著噪声。

Method: 利用数据归因方法量化训练实验对模型性能的贡献，通过微调基于文本的实验描述嵌入来捕获实验间的兼容性，在测试时使用这些嵌入对所有可用训练数据进行排序选择。
...
### [196] [Mitigating Estimation Bias with Representation Learning in TD Error-Driven Regularization](https://arxiv.org/abs/2511.16090)
*Haohui Chen,Zhiyong Chen,Aoxiang Liu,Wentuo Fang*

Main category: cs.LG

TL;DR: 提出了一种基于双演员-评论家框架的增强方法，通过凸组合策略实现可调节的偏差控制，结合增强的状态和动作表示来提升性能。


<details>
  <summary>Details</summary>
Motivation: 连续控制中的确定性策略梯度算法存在价值估计偏差问题，双评论家可以减少偏差但双演员的探索潜力尚未充分挖掘。

Method: 基于TDDR框架，提出三种凸组合策略（对称和不对称），通过单一超参数控制偏差谱系，并在演员和评论家网络中集成增强的状态和动作表示。

Result: 广泛实验表明该方法持续优于基准方法，证明了可调节偏差的价值，并揭示了高估和低估在不同环境中可以被不同地利用。

Conclusion: 通过灵活偏差控制和增强表示学习，双演员-评论家框架在连续控制任务中表现出色，高估和低估都可以根据环境特性被有效利用。
...
### [197] [HybSpecNet: A Critical Analysis of Architectural Instability in Hybrid-Domain Spectral GNNs](https://arxiv.org/abs/2511.16101)
*Huseyin Goksu*

Main category: cs.LG

TL;DR: 提出HybSpecNet混合域GNN，通过结合稳定的ChebyNet分支和自适应的KrawtchoukNet分支，解决了谱图神经网络中"稳定性vs适应性"的权衡问题，并采用"延迟融合"架构避免不稳定性传播。


<details>
  <summary>Details</summary>
Motivation: 谱图神经网络面临"稳定性vs适应性"的基本权衡：有限域[-1,1]的滤波器（如ChebyNet）稳定但适应性差，无法处理异配图；半无限域[0,∞)的滤波器（如KrawtchoukNet）自适应性强但在高多项式阶数K时数值不稳定。需要设计能同时保证稳定性和适应性的架构。

Method: 提出HybSpecNet混合架构：1）结合稳定的ChebyNet分支和自适应的KrawtchoukNet分支；2）发现简单拼接融合会导致"不稳定性毒化"问题；3）提出"延迟融合"架构，完全隔离梯度路径，防止NaN/Inf梯度破坏训练。

Result: 实验表明：1）简单混合架构在低K时表现良好，但在K=25时发生灾难性崩溃；2）延迟融合架构在K=30时仍保持完美稳定性，同时在所有图类型上保持SOTA性能。

Conclusion: 本研究识别了混合GNN设计中的关键架构陷阱，并提供了稳健的架构解决方案，成功统一了谱图神经网络的稳定性和适应性。
...
### [198] [Pathlet Variational Auto-Encoder for Robust Trajectory Generation](https://arxiv.org/abs/2511.16105)
*Yuanbo Tang,Yan Tang,Zixuan Zhang,Zihui Zhao,Yang Li*

Main category: cs.LG

TL;DR: 提出基于路径片段表示的概率图模型，用于生成稳健可解释的轨迹，在噪声数据上表现优异，效率显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹生成模型在鲁棒性和可解释性方面不足，限制了在真实噪声数据上的应用和下游任务的可信度。

Method: 基于路径片段表示的深度生成模型，包含变分自编码器和线性解码器，同时学习路径片段表示和轨迹片段字典。

Result: 在两个真实轨迹数据集上相对基线提升35.4%和26.3%，节省64.8%时间和56.5%GPU内存。

Conclusion: 该模型能有效学习噪声数据分布，生成的轨迹可用于轨迹预测和数据去噪等下游任务，具有显著效率优势。
...
### [199] [An Interpretability-Guided Framework for Responsible Synthetic Data Generation in Emotional Text](https://arxiv.org/abs/2511.16132)
*Paula Joy B. Martinez,Jose Marie Antonio Miñoza,Sebastian C. Ibañez*

Main category: cs.LG

TL;DR: 提出了基于SHAP解释性指导的LLM合成数据生成框架，用于解决社交媒体情感识别训练数据获取成本高昂的问题。该方法在充足种子数据下能匹配真实数据性能，但合成文本在词汇丰富度和表达复杂性方面存在局限。
...
Method: 引入基于SHAP解释性指导的框架，利用Shapley Additive Explanations为LLM合成数据生成提供原则性指导。

Result: 在充足种子数据下，SHAP指导方法能匹配真实数据性能，显著优于朴素生成方法，并大幅改善代表性不足情感类别的分类效果。但语言分析显示合成文本词汇丰富度降低，个人化和时间复杂性表达较少。

Conclusion: 这项工作既提供了负责任合成数据生成的实用框架，也批判性地指出了其局限性，强调可信AI的未来需要在合成效用和真实世界真实性之间权衡取舍。
...
### [200] [Labels Matter More Than Models: Quantifying the Benefit of Supervised Time Series Anomaly Detection](https://arxiv.org/abs/2511.16145)
...
Method: 提出STAND作为流线型监督基线，在五个公共数据集上进行系统比较实验，评估监督和无监督范式在有限标签预算下的表现。
...
### [201] [Enhancing Nuclear Reactor Core Simulation through Data-Based Surrogate Models](https://arxiv.org/abs/2511.16148)
*Perceval Beja-Battais,Alain Grossetête,Nicolas Vayatis*

Main category: cs.LG

TL;DR: 本文提出两种替代模拟方案的数据驱动代理模型，用于增强核反应堆堆芯模拟，显著降低计算时间（最高可达1000倍）。


<details>
  <summary>Details</summary>
Motivation: 为匹配可再生能源快速增长，核电站需要提高灵活性，通过数据驱动模拟方案改进模型预测控制方法。

Method: 从一组非线性刚性常微分方程出发，引入两种代理模型作为替代模拟方案，包括数据驱动和物理信息模型。

Result: 两种模型都能快速整合复杂动态，计算时间大幅减少（最高1000倍）。

Conclusion: 数据驱动和物理信息模型都能有效提升核反应堆模拟效率，为核电站灵活性改进提供支持。
...
### [202] [Achieving Skilled and Reliable Daily Probabilistic Forecasts of Wind Power at Subseasonal-to-Seasonal Timescales over France](https://arxiv.org/abs/2511.16164)
*Eloi Lindas,Yannig Goude,Philippe Ciais*

Main category: cs.LG

TL;DR: 开发了一个将ECMWF次季节到季节天气预报转换为1-46天风电功率预测的管道框架，通过后处理改善预报偏差和离散度不足问题，相比气候基准提升50%预测技能。


<details>
  <summary>Details</summary>
Motivation: 准确可靠的风电功率预测对电网稳定性、供需平衡和市场风险管理至关重要。虽然短期天气预报已广泛用于短期可再生能源预测，但涉及更长预测周期的预报仍需研究。

Method: 提出了一个预测管道，将ECMWF次季节到季节天气预报转换为1-46天日分辨率的风电功率预测，包括对结果功率集合进行后处理以解决天气预报的偏差和离散度不足问题。

Result: 该方法在连续排序概率技能评分和集合均方误差方面比气候基准提升50%，在15-46天预测周期内提供近乎完美的预测校准。

Conclusion: 该框架成功地将次季节到季节天气预报转化为具有良好技能和校准的风电功率预测，为长期风电预测提供了有效解决方案。
...
### [203] [CausalMamba: Interpretable State Space Modeling for Temporal Rumor Causality](https://arxiv.org/abs/2511.16191)
...
<details>
  <summary>Details</summary>
Motivation: 现有谣言检测模型难以捕捉传播动态的复杂性和揭示错误信息传播的潜在因果机制，缺乏可解释性。

Method: 集成Mamba序列建模、图卷积网络(GCN)和通过NOTEARS的可微分因果发现，学习推文时间序列和回复结构的联合表示，同时发现潜在因果图。

Result: 在Twitter15数据集上取得有竞争力的分类性能，能够进行反事实干预分析，移除顶级因果节点显著改变图连通性。
...
### [204] [A Switching Framework for Online Interval Scheduling with Predictions](https://arxiv.org/abs/2511.16194)
*Antonios Antoniadis,Ali Shahheidar,Golnoosh Shahkarami,Abolfazl Soltani*

Main category: cs.LG

TL;DR: 该论文提出了SemiTrust-and-Switch框架，用于结合预测和经典算法的在线区间调度，在保证鲁棒性的同时利用机器学习预测提升性能。


<details>
  <summary>Details</summary>
Motivation: 研究在线区间调度问题，在不可撤销设置下，目标是最大化接受区间的总长度且不重叠。希望通过机器学习预测来提升算法性能，同时保持对预测错误的鲁棒性。

Method: 提出了SemiTrust-and-Switch统一框架，结合预测算法和经典算法，适用于确定性和随机算法。还设计了随机算法，在预测质量和鲁棒性之间平滑插值。

Result: 该框架能够平衡一致性（预测准确时的性能）和鲁棒性（对抗性输入下的性能）。提供了下界证明，证明该框架在特定设置下的紧致性。

Conclusion: SemiTrust-and-Switch框架为学习增强的在线区间调度提供了有效解决方案，能够优雅地处理预测质量变化，同时保持性能保证。
...
### [205] [Causal Synthetic Data Generation in Recruitment](https://arxiv.org/abs/2511.16204)
*Andrea Iommi,Antonio Mastropietro,Riccardo Guidotti,Anna Monreale,Salvatore Ruggieri*

Main category: cs.LG

TL;DR: 提出了一种基于因果生成模型的合成数据生成方法，专门用于解决招聘领域因数据隐私限制导致的数据稀缺问题，通过生成保留因果关系的合成数据来评估候选人排名的公平性。


<details>
  <summary>Details</summary>
Motivation: 招聘领域由于简历包含敏感信息（性别、残疾状况、年龄等），公开数据集稀缺，这阻碍了开发公平透明的机器学习模型，特别是需要大量数据的排序算法。

Method: 使用两个因果生成模型：一个建模职位需求，另一个建模简历信息。每个模型都基于领域专业知识构建的因果图进行结构化，用于生成合成数据集并在引入特定偏见的受控场景下评估候选人排名的公平性。

Result: 该方法能够生成保留数据底层因果关系的合成数据集，为评估候选人排名算法的公平性提供了可控的环境。
...
### [206] [Towards Overcoming Data Scarcity in Nuclear Energy: A Study on Critical Heat Flux with Physics-consistent Conditional Diffusion Model](https://arxiv.org/abs/2511.16207)
*Farah Alsafadi,Alexandra Akins,Xu Wu*

Main category: cs.LG

TL;DR: 本文研究了扩散模型在核能应用中克服数据稀缺问题的有效性，通过生成与临界热通量数据集统计相似的高保真合成样本，显著增强了训练数据的规模和多样性。


<details>
  <summary>Details</summary>
Motivation: 核能应用中实验数据往往有限、昂贵或难以获取，深度生成建模为克服数据稀缺问题提供了有力途径，能够生成统计上类似于训练数据的高保真合成样本。

Method: 开发了扩散模型和条件扩散模型，利用涵盖商业核反应堆广泛运行条件的临界热通量公共数据集，生成任意数量的合成样本用于数据增强。条件扩散模型能够根据用户指定的热工水力条件生成目标数据。

Result: 结果表明，扩散模型和条件扩散模型都能成功生成真实且物理一致的临界热通量数据。不确定性量化显示条件扩散模型在数据增强方面非常有效，同时保持可接受的不确定性水平。

Conclusion: 扩散模型特别是条件扩散模型，在核能应用中能够有效克服数据稀缺问题，生成高质量、物理一致的合成数据，为下游机器学习模型提供更丰富和多样化的训练数据。
...
### [207] [Mind the Gap: Bridging Prior Shift in Realistic Few-Shot Crop-Type Classification](https://arxiv.org/abs/2511.16218)
*Joana Reuss,Ekaterina Gikalo,Marco Körner*

Main category: cs.LG

TL;DR: 提出Dirichlet Prior Augmentation (DirPA)方法，通过在少样本学习中模拟目标域未知的标签分布偏斜来解决农业数据中的类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现实农业分布通常存在严重的类别不平衡，遵循长尾分布。标记的作物分类数据集稀缺且获取成本高，训练集常被人工平衡化，导致训练和测试标签分布不匹配，影响真实世界的泛化能力。

Method: 将真实世界分布建模为Dirichlet分布的随机变量，在少样本学习期间执行先验增强，模拟目标域的未知标签分布偏斜。

Result: 实验表明DirPA成功转移了决策边界，并通过作为动态特征正则化器稳定了训练过程。

Conclusion: DirPA方法有效解决了农业数据中的类别不平衡问题，提高了模型在真实世界条件下的泛化性能。
...
### [208] [Real-Time Inference for Distributed Multimodal Systems under Communication Delay Uncertainty](https://arxiv.org/abs/2511.16225)
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的非阻塞推理方法依赖参考模态范式，需要等待一个模态完全接收后才能处理，且依赖昂贵的离线分析。异构数据流的不确定通信延迟挑战了推理过程的时间流。

Method: 采用自适应时间窗口整合(TWIs)来动态调整异构数据流的随机延迟模式，放宽参考模态要求，构建通信延迟感知框架，实现精度-延迟权衡的细粒度控制。

Result: 在音频-视觉事件定位任务上的实验表明，相比最先进方法，该方法对网络动态具有更优越的适应性。

Conclusion: 提出的神经启发式非阻塞推理范式通过自适应时间窗口整合，能够有效应对异构数据流的随机延迟，实现鲁棒的实时推理性能。
...
### [209] [Deep SOR Minimax Q-learning for Two-player Zero-sum Game](https://arxiv.org/abs/2511.16226)
*Saksham Gautam,Lakshmi Mandal,Shalabh Bhatnagar*

Main category: cs.LG

TL;DR: 提出了一种适用于高维状态-动作空间的深度连续过松弛极小极大Q学习算法，结合深度神经网络作为函数逼近器，在两人零和博弈中实现了更快的收敛。
...
Method: 开发了深度连续过松弛极小极大Q学习算法，使用深度神经网络作为函数逼近器，并证明了算法的有限时间收敛性。

Result: 数值实验表明，所提方法在收敛速度上优于现有的Q学习算法，消融研究展示了不同连续过松弛参数值的影响。

Conclusion: 该算法成功将连续过松弛技术扩展到高维两人零和博弈的函数逼近设置中，实现了更快的收敛性能。
...
### [210] [Pass@k Metric for RLVR: A Diagnostic Tool of Exploration, But Not an Objective](https://arxiv.org/abs/2511.16231)
*Yang Yu*

Main category: cs.LG

TL;DR: 本文分析了pass@k指标作为强化学习优化目标的问题，指出它本质上是对pass@1的正向重加权，在需要探索时提供消失的学习信号，可能导致探索崩溃。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的多步推理能力评估中，pass@k指标被广泛采用，不仅作为评估标准，还直接作为强化学习的优化目标。本文旨在分析这一做法的有效性。

Method: 通过数学分析pass@k目标函数，推导其梯度，证明它是pass@1的逐样本正向重加权，并分析探索崩溃的动态过程。

Result: 发现pass@k目标在需要探索的关键区域提供消失的学习信号，随着策略概率质量集中，pass@k与pass@1之间的差距会缩小。

Conclusion: pass@k作为诊断工具有用，但不适合直接作为优化目标。应使用明确鼓励高效探索的机制来改进推理任务的强化学习。
...
### [211] [GeoPTH: A Lightweight Approach to Category-Based Trajectory Retrieval via Geometric Prototype Trajectory Hashing](https://arxiv.org/abs/2511.16258)
*Yang Xu,Zuliang Yang,Kai Ming Ting*

Main category: cs.LG

TL;DR: 提出GeoPTH，一种轻量级、非学习的基于原型的轨迹哈希方法，用于高效类别轨迹检索，在准确性和效率上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹相似性检索方法存在计算成本高（传统指标）或训练成本高且不稳定（学习方法）的问题，需要一种轻量高效的替代方案。

Method: 使用代表性轨迹原型作为锚点构建数据依赖的哈希函数，通过Hausdorff度量将新轨迹映射到最近原型，实现高效哈希过程。

Result: 实验表明GeoPTH的检索准确性与传统指标和先进学习方法相当，显著优于简单二值化方法，且在效率上始终优于所有竞争对手。
...
### [212] [Graph Diffusion Counterfactual Explanation](https://arxiv.org/abs/2511.16287)
...
<details>
  <summary>Details</summary>
Motivation: 图结构数据的机器学习模型预测准确但缺乏解释性，反事实解释能提供预测改变的替代场景，但图领域在此方面研究较少，因为图是离散且非欧几里得的对象。

Method: 结合离散扩散模型和分类器自由引导，构建图扩散反事实解释框架。

Result: 经验证明该方法能可靠地为离散分类目标和连续属性生成分布内且结构差异最小的反事实。

Conclusion: 提出的框架成功解决了图数据反事实解释生成的挑战，为图模型提供了有效的解释工具。
...
### [213] [Optimizing Operation Recipes with Reinforcement Learning for Safe and Interpretable Control of Chemical Processes](https://arxiv.org/abs/2511.16297)
*Dean Brandner,Sergio Lucia*

Main category: cs.LG

TL;DR: 提出一种利用操作配方中专家知识的新方法，通过强化学习优化配方参数和线性控制器，实现优化操作，相比传统方法数据需求更少、约束处理更好、可解释性更强。


<details>
  <summary>Details</summary>
Motivation: 化工过程最优操作对节能、资源和成本节约至关重要，但传统强化学习方法面临硬约束满足、数据需求大等挑战，而详细动态模型又计算复杂，导致许多过程依赖手动定义的操作配方和简单线性控制器，性能次优且灵活性有限。

Method: 利用操作配方中嵌入的专家知识，通过强化学习优化这些配方参数及其底层线性控制器，获得优化的操作配方。
...
### [214] [Learning-Enhanced Observer for Linear Time-Invariant Systems with Parametric Uncertainty](https://arxiv.org/abs/2511.16318)
*Hao Shu*
...
Conclusion: 现代学习机制可以作为传统观测器设计的有效补充，在不确定系统中实现更准确和鲁棒的状态估计。
...
### [215] [Beyond Generative AI: World Models for Clinical Prediction, Counterfactuals, and Planning](https://arxiv.org/abs/2511.16333)
*Mohammad Areeb Qazi,Maryam Nadeem,Mohammad Yaqub*

Main category: cs.LG

TL;DR: 本文综述了医疗领域的世界模型，这些模型学习预测动态以支持多步推演、反事实评估和规划。文章调查了三个领域的近期工作：医学影像诊断、电子病历疾病进展建模、机器人手术规划，并提出了L1-L4能力评估框架。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型缺乏物理基础和时序推理能力，无法满足临床决策支持需求。世界模型能够学习多模态、时序一致、动作条件的表征，反映医疗的物理和因果结构。

Method: 通过系统文献综述，分析医疗世界模型在三个主要领域的应用：医学影像诊断（如纵向肿瘤模拟、JEPA风格预测表征学习）、电子病历疾病进展建模（生成式事件预测）、机器人手术规划（动作条件引导控制）。

Result: 大多数系统达到L1（时序预测）和L2（动作条件预测）水平，较少达到L3（反事实推演）和罕见达到L4（规划控制）。识别了限制临床可靠性的关键差距。

Conclusion: 提出了将生成主干（Transformer、扩散模型、VAE）与因果/机械基础整合的研究议程，以开发临床稳健的预测优先世界模型，为医疗安全决策提供支持。
...
### [216] [Improving Iterative Gaussian Processes via Warm Starting Sequential Posteriors](https://arxiv.org/abs/2511.16340)
*Alan Yufei Dong,Jihao Andreas Lin,José Miguel Hernández-Lobato*

Main category: cs.LG

TL;DR: 提出一种改进迭代高斯过程可扩展性的方法，通过利用小系统的已知解来加速大线性系统的求解器收敛
...
Method: 利用包含在大系统中的较小系统的已知解来改进大型线性系统的求解器收敛性，适用于增量数据添加的任务

Result: 在容忍度求解时实现了加速，并在固定计算预算下提高了贝叶斯优化性能

Conclusion: 该方法为增量数据添加任务提供了有效的GP可扩展性改进方案
...
### [217] [Are Foundation Models Useful for Bankruptcy Prediction?](https://arxiv.org/abs/2511.16375)
*Marcin Kostrzewa,Oleksii Furman,Roman Furman,Sebastian Tomczak,Maciej Zięba*

Main category: cs.LG

TL;DR: 对基础模型在企业破产预测任务中的首次系统性评估，发现XGBoost和CatBoost等传统机器学习方法在所有预测时间范围内都优于基础模型。


<details>
  <summary>Details</summary>
Motivation: 尽管基础模型在金融应用中显示出潜力，但其在企业破产预测方面的有效性尚未与既定方法进行系统比较。

Method: 使用Llama-3.3-70B-Instruct和TabPFN模型，在包含超过100万家公司记录的维谢格拉德集团数据集上进行评估，并与经典机器学习基线进行对比。

Result: XGBoost和CatBoost在所有预测时间范围内始终优于基础模型。基于LLM的方法存在概率估计不可靠的问题，TabPFN虽然与简单基线竞争，但计算成本高且性能提升不显著。
...
### [218] [Optimal Fairness under Local Differential Privacy](https://arxiv.org/abs/2511.16377)
*Hrad Ghoukasian,Shahab Asoodeh*

Main category: cs.LG

TL;DR: 提出一种优化本地差分隐私机制设计的方法，在保护敏感属性隐私的同时减少数据不公平性，从而改善下游分类的公平性。


<details>
  <summary>Details</summary>
Motivation: 研究如何在本地差分隐私机制设计中平衡隐私保护与公平性，解决现有方法在隐私保护下公平性表现不佳的问题。

Method: 首先为二元敏感属性推导出闭式最优机制，然后为多值属性开发可处理的优化框架，建立隐私感知预处理与分类公平性的理论联系。

Result: 经验验证表明，该方法在多种数据集和公平性指标上持续优于现有LDP机制，在保持接近非私有模型精度的同时显著降低数据不公平性。

Conclusion: 本地差分隐私可作为有原则且有效的预处理公平性干预技术，在保护敏感属性隐私的同时实现更优的精度-公平性权衡。
...
### [219] [Collaborative Management for Chronic Diseases and Depression: A Double Heterogeneity-based Multi-Task Learning Method](https://arxiv.org/abs/2511.16398)
...
Conclusion: 为整合身心健康护理提供计算解决方案，并为协作慢性疾病管理在治疗前、治疗中和治疗后阶段提供设计原则。
...
### [220] [FreqFlow: Long-term forecasting using lightweight flow matching](https://arxiv.org/abs/2511.16426)
*Seyed Mohamad Moghadas,Bruno Cornelis,Adrian Munteanu*

Main category: cs.LG

TL;DR: FreqFlow是一个在频域中使用条件流匹配进行确定性多元时间序列预测的新框架，相比基于扩散的模型，参数更少（仅89k），计算效率更高，且能实现单次确定性采样。


<details>
  <summary>Details</summary>
Motivation: 现有的基于去噪扩散的生成模型在捕捉复杂数据分布方面表现优异，但存在计算开销大、迭代采样过程限制实时部署的问题，且在处理高维、非平稳和多尺度周期性模式时表现脆弱。

Method: 将预测问题转换到频域，通过单个复数值线性层学习幅度和相位偏移，利用复数乘法高效捕捉时间动态。将多元时间序列信号分解为趋势、季节性和残差分量，流匹配机制专门用于残差学习以提高长期预测准确性。

Result: 在真实世界的交通速度、流量和流量数据集上的广泛实验表明，FreqFlow实现了最先进的预测性能，平均RMSE改进7%，同时比现有方法显著更快且参数效率更高。
...
### [221] [Generative Modeling of Clinical Time Series via Latent Stochastic Differential Equations](https://arxiv.org/abs/2511.16427)
...
Method: 使用潜在神经随机微分方程建模潜在动态，具有模态依赖的发射模型，通过变分推理进行状态估计和参数学习。
...
### [222] [A Comparison Between Decision Transformers and Traditional Offline Reinforcement Learning Algorithms](https://arxiv.org/abs/2511.16475)
...
<details>
  <summary>Details</summary>
Motivation: 传统离线RL算法在平衡探索与利用方面面临挑战，特别是在不同奖励密度的环境中。决策变换器将离线RL重新定义为序列建模问题，在多个基准测试中表现出色，需要系统比较其与传统方法的性能差异。

Method: 在ANT连续控制环境中进行实证分析，比较决策变换器与CQL、IQL等传统离线RL算法在不同奖励密度（密集vs稀疏）和数据质量下的表现。

Result: 决策变换器对奖励密度变化较不敏感，在稀疏奖励场景中表现优异；IQL在密集奖励和高质量数据下表现更好；CQL在不同数据质量下表现均衡；决策变换器性能方差较低但计算资源需求显著更高。

Conclusion: 序列建模方法更适合奖励结构不确定或数据质量混合的场景，而基于价值的方法在密集奖励和高质量演示数据场景中仍具竞争力。
...
### [223] [Limitations of Scalarisation in MORL: A Comparative Study in Discrete Environments](https://arxiv.org/abs/2511.16476)
*Muhammad Sa'ood Shah,Asad Jeewa*

Main category: cs.LG

TL;DR: 该研究比较了多目标强化学习中的标量化方法和多策略方法，发现标量化函数在复杂环境中难以准确逼近帕累托前沿，而内循环多策略算法更具鲁棒性和通用性。


<details>
  <summary>Details</summary>
Motivation: 标量化函数在多目标强化学习中被广泛使用，但在复杂不确定环境中往往难以准确逼近帕累托前沿，限制了其决策能力。本研究旨在深入探讨标量化方法在多目标设置中的局限性。
...
Conclusion: 与标量化方法相比，内循环多策略算法可能提供更可持续和可泛化的方法，在动态和不确定环境中促进智能决策。
...
### [224] [Correlation-Aware Feature Attribution Based Explainable AI](https://arxiv.org/abs/2511.16482)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI模型日益复杂和高风险应用对透明度需求的增加，现有全局归因方法存在计算成本高、在相关输入下缺乏稳定性、难以扩展到大型异构数据集的问题。

Method: ExCIR使用鲁棒中心化（减去中位数或中均值）后量化特征与模型输出的符号对齐共变，并引入BlockCIR进行分组归因，将相关特征集作为单一单元评分。

Result: 在文本、表格、信号和图像数据集上，ExCIR与现有基准方法和完整模型具有可信的一致性，提供稳定的top-k排名，并通过子集评估减少运行时间。
...
### [225] [Large Language Model-Based Reward Design for Deep Reinforcement Learning-Driven Autonomous Cyber Defense](https://arxiv.org/abs/2511.16483)
...
Method: 创建多个攻防智能体角色，反映智能体行为的异质性，首先为大语言模型提供网络模拟环境上下文信息，生成LLM指导的奖励设计，然后在DRL驱动的攻防模拟环境中使用这些奖励结构来学习网络防御策略集合。

Result: 结果表明LLM指导的奖励设计可以产生针对多样化对抗行为的有效防御策略。

Conclusion: LLM指导的奖励设计能够有效生成网络防御策略，应对多样化的对抗行为。
...
### [226] [ODE-ViT: Plug & Play Attention Layer from the Generalization of the ViT as an Ordinary Differential Equation](https://arxiv.org/abs/2511.16501)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型视觉模型计算资源需求大、存储成本高且决策过程难以理解，需要开发更高效、可解释的模型架构。

Method: 基于残差网络与常微分方程的联系，将Vision Transformer重新表述为满足良好条件和稳定动态的ODE系统，并提出教师-学生框架指导ODE-ViT的连续轨迹。

Result: 在CIFAR-10和CIFAR-100上实现稳定、可解释且具有竞争力的性能，参数减少一个数量级，超越先前基于ODE的Transformer方法，教师-学生框架使性能提升10%以上。

Conclusion: ODE-ViT提供了一种高效、可解释的视觉Transformer替代方案，通过ODE框架和知识蒸馏策略实现了参数效率和性能的平衡。
...
### [227] [Loss Functions Robust to the Presence of Label Errors](https://arxiv.org/abs/2511.16512)
...
<details>
  <summary>Details</summary>
Motivation: 现有的标签错误检测方法需要能够抵抗标签错误的模型，但获取这样的模型通常需要在被污染的数据上训练，这是一个挑战。受Focal Loss启发，希望通过调整损失函数来改进这一问题。

Method: 提出了两种新颖而简单的损失函数：一种降低困难样本的权重，另一种完全忽略这些可能包含标签错误的困难样本。

Result: 在人工污染的数据上的实验结果显示，相比传统的分类交叉熵和Focal Loss基线方法，新方法在检测标签错误方面的F1分数有所提升。
...
### [228] [Saving Foundation Flow-Matching Priors for Inverse Problems](https://arxiv.org/abs/2511.16520)
...
<details>
  <summary>Details</summary>
Motivation: 基础流匹配模型理论上可作为逆问题的通用先验，但实际表现不如领域特定甚至无训练先验，需要解锁其潜力。
...
### [229] [Dynamic Participation in Federated Learning: Benchmarks and a Knowledge Pool Plugin](https://arxiv.org/abs/2511.16523)
*Ming-Lun Lee,Fu-Shiang Yang,Cheng-Kuan Lin,Yan-Ann Chen,Chih-Yu Lin,Yu-Chee Tseng*

Main category: cs.LG

TL;DR: 提出了首个专门针对动态客户端参与场景的联邦学习基准框架，并开发了KPFL方法来缓解动态参与带来的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习研究大多假设客户端持续参与，忽视了实际中客户端动态加入和退出的场景，且缺乏专门针对动态参与联邦学习的基准测试框架。

Method: 开发了可配置数据分布、参与模式和评估指标的基准框架，并提出KPFL方法，通过共享知识池、双年龄和数据偏差加权以及生成知识蒸馏来维持模型稳定性。

Result: 基准测试显示动态参与导致FL模型性能显著下降，而KPFL能有效提高模型鲁棒性和泛化能力。

Conclusion: 动态客户端参与对联邦学习性能有重大影响，KPFL方法能有效缓解这一问题，为动态参与联邦学习提供了实用解决方案。
...
### [230] [FairLRF: Achieving Fairness through Sparse Low Rank Factorization](https://arxiv.org/abs/2511.16549)
*Yuanbo Guo,Jun Xia,Yiyu Shi*

Main category: cs.LG

TL;DR: 提出了一种基于奇异值分解(SVD)的公平性增强方法FairLRF，通过选择性移除酉矩阵中的偏差诱导元素来减少群体差异，在保持模型性能的同时提升公平性。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医疗诊断等敏感领域应用时，需要确保公平性同时保持高性能。现有偏差缓解方法要么计算成本高，要么导致模型准确率大幅下降，限制了在资源受限环境中的实用性。

Method: 利用奇异值分解(SVD)分析权重矩阵，发现酉矩阵中的元素对基于敏感属性的群体偏差贡献不均等。提出FairLRF方法，选择性移除这些偏差诱导元素来减少群体差异。

Result: 实验表明该方法优于传统的低秩分解方法和最先进的公平性增强技术。消融研究分析了主要超参数对处理模型性能的影响。
...
### [231] [Broad stochastic configuration residual learning system for norm-convergent universal approximation](https://arxiv.org/abs/2511.16550)
...
<details>
  <summary>Details</summary>
Motivation: 随机学习网络的普遍逼近性质通常基于概率测度收敛而非更严格的范数收敛，这使得网络对随机参数选择高度敏感。BRLS作为随机学习模型也面临这一问题，其迭代误差在随机参数选择不当时无法满足范数收敛。

Method: 在BRLS框架基础上，提出BSCRLS算法，采用新型监督机制自适应约束随机参数范围。提出了三种增量式BSCRLS算法版本以满足不同网络更新需求。

Result: 在公开数据集上进行太阳能板灰尘检测实验，与13种深度和广度学习算法比较，实验结果表明BSCRLS算法的有效性和优越性。

Conclusion: BSCRLS算法通过自适应约束随机参数范围，解决了BRLS普遍逼近性质的局限性，基于范数收敛证明了其普遍逼近定理，实验验证了该算法的有效性。
...
### [232] [Toward Valid Generative Clinical Trial Data with Survival Endpoints](https://arxiv.org/abs/2511.16551)
*Perrine Chassat,Van Tuan Nguyen,Lucas Ducrot,Emilie Lanoy,Agathe Guilloux*

Main category: cs.LG

TL;DR: 提出一种变分自编码器方法，用于生成包含混合类型协变量和生存结果的合成控制臂，解决临床试验中患者招募困难和成本高昂的问题。


<details>
  <summary>Details</summary>
Motivation: 临床试验面临患者群体分散、招募缓慢和成本不可持续等挑战，特别是在肿瘤学和罕见病领域。现有基于GAN的方法数据需求大、不稳定且依赖强假设。

Method: 使用变分自编码器(VAE)在统一潜变量框架中联合生成混合类型协变量和生存结果，不假设独立删失。

Result: 在合成和真实试验数据集上，该方法在保真度、实用性和隐私指标上优于GAN基线，但揭示了I类错误和功效的系统性错误校准。
...
### [233] [Boosting Predictive Performance on Tabular Data through Data Augmentation with Latent-Space Flow-Based Diffusion](https://arxiv.org/abs/2511.16571)
*Md. Tawfique Ihsan,Md. Rakibul Hasan Rafi,Ahmed Shoyeb Raihan,Imtiaz Ahmed,Abdullahil Azeem*

Main category: cs.LG

TL;DR: 提出了基于潜在空间和树驱动扩散的少数类过采样方法，使用条件流匹配和梯度提升树作为向量场学习器，在异构表格数据中有效处理严重类别不平衡问题。
...
Method: 提出了三种变体：PCAForest（线性PCA嵌入）、EmbedForest（学习非线性嵌入）和AttentionForest（注意力增强嵌入），每种方法都将基于梯度提升树的流与解码器耦合回原始特征空间。

Result: 在11个来自医疗、金融和制造领域的数据集上，AttentionForest实现了最佳平均少数类召回率，同时保持竞争性的精度、校准和分布相似性。PCAForest和EmbedForest以更快的生成速度达到相似效用。
...
### [234] [ECPv2: Fast, Efficient, and Scalable Global Optimization of Lipschitz Functions](https://arxiv.org/abs/2511.16575)
*Fares Fourati,Mohamed-Slim Alouini,Vaneet Aggarwal*

Main category: cs.LG

TL;DR: ECPv2是一个可扩展的全局优化算法，针对Lipschitz连续函数进行优化，解决了ECP算法的高计算成本和保守行为问题，通过自适应下界、Worst-m记忆机制和固定随机投影等创新，在保持理论保证的同时显著提升性能。
...
Method: ECPv2引入三个关键技术：自适应下界避免无效接受区域，Worst-m记忆机制限制与固定大小历史评估子集的比较，固定随机投影加速高维距离计算。

Result: ECPv2在保持ECP无遗憾保证和最优有限时间边界的同时，以高概率扩展接受区域。在广泛的高维非凸优化问题基准测试中，ECPv2始终匹配或优于最先进的优化器，并显著减少运行时间。

Conclusion: ECPv2是一个理论可靠且实际高效的全局优化算法，通过创新机制解决了ECP的局限性，在保持理论保证的同时显著提升了计算效率和性能表现。
...
### [235] [Almost Sure Convergence Analysis of Differentially Private Stochastic Gradient Methods](https://arxiv.org/abs/2511.16587)
*Amartya Mukherjee,Jun Liu*

Main category: cs.LG

TL;DR: 证明了差分隐私随机梯度下降(DP-SGD)及其动量变体在标准光滑性假设下几乎必然收敛，为差分隐私优化提供了更强的理论保证。


<details>
  <summary>Details</summary>
Motivation: 现有对DP-SGD的理论分析通常只建立期望收敛或高概率收敛，但缺乏对单条轨迹几乎必然收敛的研究，这限制了对其长期行为的理解。

Method: 在标准步长衰减条件下，通过分析DP-SGD及其动量变体(DP-SHB和DP-NAG)，使用能量构造方法证明几乎必然收敛性。

Result: 证明了DP-SGD在非凸和强凸设置下都几乎必然收敛，动量变体在精心设计的能量构造下也能获得类似保证。

Conclusion: 尽管存在隐私引起的扭曲，DP-SGD算法在凸和非凸情况下都保持路径稳定性，为差分隐私优化提供了更强的理论基础。
...
### [236] [gfnx: Fast and Scalable Library for Generative Flow Networks in JAX](https://arxiv.org/abs/2511.16592)
*Daniil Tiapkin,Artem Agarkov,Nikita Morozov,Ian Maksimov,Askar Tsyganov,Timofei Gritsaev,Sergey Samsonov*

Main category: cs.LG

TL;DR: gfnx是一个基于JAX的快速可扩展的GFlowNets训练和评估包，相比PyTorch实现实现了显著的加速效果。


<details>
  <summary>Details</summary>
Motivation: 现有的GFlowNets实现（如torchgfn）在性能上存在瓶颈，需要更高效的实现来加速研究和应用。

Method: 使用JAX框架实现GFlowNets核心训练目标，提供多种环境（超网格、序列生成、分子生成等）和标准化评估指标。

Result: 在CPU序列生成环境中实现55倍加速，GPU贝叶斯网络结构学习中实现80倍加速。

Conclusion: gfnx提供了多样化的基准测试，旨在标准化GFlowNets的实证评估并加速其研究和应用。
...
### [237] [Toward Artificial Palpation: Representation Learning of Touch on Soft Bodies](https://arxiv.org/abs/2511.16596)
...
Method: 开发模拟环境和收集真实世界数据集，使用配备触觉传感器的机器人收集触诊序列，训练预测物体不同位置感官读数的模型。
...
### [238] [Stabilizing Policy Gradient Methods via Reward Profiling](https://arxiv.org/abs/2511.16629)
*Shihab Ahmed,El Houcine Bergou,Aritra Dutta,Yue Wang*

Main category: cs.LG

TL;DR: 提出了一个通用的奖励分析框架，可与任何策略梯度算法无缝集成，通过基于高置信度性能估计的选择性策略更新来解决梯度估计高方差问题。


<details>
  <summary>Details</summary>
Motivation: 策略梯度方法虽然有效，但由于梯度估计的高方差，常常导致不可靠的奖励改进和收敛缓慢，性能表现不理想。

Method: 开发了一个通用的奖励分析框架，能够选择性地基于高置信度性能估计来更新策略，该框架可与任何策略梯度算法集成。
...
### [239] [Evolution Strategies at the Hyperscale](https://arxiv.org/abs/2511.16652)
...
TL;DR: EGGROLL是一种基于进化策略的优化算法，通过低秩学习来扩展无反向传播优化，适用于数十亿参数的大型神经网络架构，显著降低计算和内存成本。


<details>
  <summary>Details</summary>
Motivation: 传统进化策略在扩展到大规模神经网络时，由于生成矩阵扰动和批量矩阵乘法的计算和内存成本过高而变得不可行。需要一种方法在保持ES优势的同时降低这些成本。

Method: 使用低秩矩阵扰动AB⊤（其中r远小于m,n）替代全秩扰动E，将辅助存储从mn减少到r(m+n)，前向传播成本从O(mn)降低到O(r(m+n))。

Result: 理论分析显示低秩更新以O(1/r)的快速速率收敛到全秩更新。实验表明EGGROLL在强化学习、LLM推理和整数数据类型非线性循环语言模型预训练中表现良好。

Conclusion: EGGROLL成功解决了大规模进化策略的计算瓶颈，在不牺牲性能的前提下显著提高了效率，为大规模神经网络的无反向传播优化提供了可行方案。
...
### [240] [Taming the Long-Tail: Efficient Reasoning RL Training with Adaptive Drafter](https://arxiv.org/abs/2511.16665)
...
<details>
  <summary>Details</summary>
Motivation: 当前推理型大语言模型的强化学习训练存在效率瓶颈，响应生成长尾分布导致少数极长响应占据大部分执行时间，浪费资源并增加成本。

Method: TLT系统包含两个协同组件：自适应草稿模型（在空闲GPU上持续训练以保持与目标模型对齐）和自适应执行引擎（维护内存高效CUDAGraph池并自适应选择推测解码策略）。

Result: 评估显示TLT实现了超过1.7倍的端到端RL训练加速，保持了模型精度，并产生高质量草稿模型作为免费副产品。

Conclusion: TLT成功解决了推理RL训练中的效率问题，通过自适应推测解码实现了显著的训练加速，同时保持了模型质量。
