...
### [1] [RemEdit: Efficient Diffusion Editing with Riemannian Geometry](https://arxiv.org/abs/2601.17927)
...
Motivation: 现代生成式AI中的可控图像生成面临语义保真度和推理速度之间的关键权衡，现有方法难以同时满足编辑质量和实时性能的需求。

Method: 1. 将潜在空间视为黎曼流形，使用Mamba模块学习流形结构，计算精确的测地线路径实现平滑语义编辑；2. 采用双SLERP混合技术和视觉语言模型的目标感知提示增强；3. 引入任务特定注意力剪枝机制，通过轻量级剪枝头保留编辑必需的关键token。

Result: RemEdit超越了先前最先进的编辑框架，在50%剪枝率下仍保持实时性能，在语义保真度和推理速度之间取得了最佳平衡。

Conclusion: RemEdit为实用且强大的图像编辑设立了新基准，通过创新的流形导航和任务特定剪枝技术，解决了可控图像生成中的关键权衡问题。
...
### [2] [Scientific Image Synthesis: Benchmarking, Methodologies, and Downstream Utility](https://arxiv.org/abs/2601.17027)
*Honglin Lin,Chonghan Qin,Zheng Liu,Qizhi Pei,Yu Li,Zhanping Zhong,Xin Gao,Yanfeng Wang,Conghui He,Lijun Wu*

Main category: cs.CV

TL;DR: 论文提出ImgCoder框架，通过"理解-规划-编码"工作流提升科学图像合成的逻辑准确性，并引入SciGenBench评估科学正确性，验证高质量合成科学图像能提升多模态推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型生成的图像虽然视觉上合理但科学上不正确，存在视觉-逻辑偏差，限制了多模态科学推理的发展。需要解决科学图像合成的准确性问题。

Method: 提出ImgCoder逻辑驱动框架，采用"理解-规划-编码"工作流提高结构精度。分析像素级生成和程序化合成两种范式，并引入SciGenBench评估生成图像的信息效用和逻辑有效性。

Result: 评估揭示了像素级模型的系统性失败模式，发现了表达能力与精度之间的基本权衡。在严格验证的合成科学图像上微调大型多模态模型能带来一致的推理提升，显示出类似文本领域的扩展趋势。

Conclusion: 高保真科学图像合成是解锁大规模多模态推理能力的可行路径，逻辑驱动的程序化合成方法能有效解决视觉-逻辑偏差问题，为科学多模态推理提供可靠数据支持。
...
### [3] [MindCine: Multimodal EEG-to-Video Reconstruction with Large-Scale Pretrained Models](https://arxiv.org/abs/2601.18192)
...
<details>
  <summary>Details</summary>
Motivation: EEG具有非侵入性和高时间分辨率的优势，但现有EEG到视频重建方法面临两个主要问题：1）仅使用文本模态，忽略其他模态且容易过拟合；2）EEG-视频数据稀缺，训练难以收敛

Method: 提出MindCine框架：1）采用多模态联合学习策略，在训练阶段整合文本以外的模态；2）利用预训练的大型EEG模型缓解数据稀缺问题，解码语义信息；3）设计具有因果注意力的Seq2Seq模型解码感知信息

Result: 实验表明，MindCine在定性和定量评估上都优于现有最先进方法，证明了不同模态互补优势的有效性，以及利用大规模EEG模型可以进一步通过缓解数据稀缺挑战来提升重建性能

Conclusion: MindCine通过多模态联合学习和预训练大模型，成功解决了EEG到视频重建中的模态单一和数据稀缺问题，实现了在有限数据上的高保真视频重建
...
### [4] [Data-Efficient Meningioma Segmentation via Implicit Spatiotemporal Mixing and Sim2Real Semantic Injection](https://arxiv.org/abs/2601.17031)
*Yunhao Xu,Fuquan Zong,Yexuan Xing,Chulong Zhang,Guang Yang,Shilong Yang,Xiaokun Liang,Juan Yu*

Main category: cs.CV

TL;DR: 提出一种双增强框架，通过空间流形扩展和语义对象注入，在有限标注下提升医学图像分割的数据利用效率


<details>
  <summary>Details</summary>
Motivation: 医学图像分割性能越来越取决于数据利用效率而非原始数据量，特别是对于脑膜瘤等复杂病理，需要充分挖掘有限高质量标注中的潜在信息

Method: 1. 使用隐式神经表示建模连续速度场，在线性混合积分变形场的基础上进行插值，从少量锚点高效生成解剖学合理的结构变化；2. 引入Sim2Real病灶注入模块，将病灶纹理移植到健康解剖背景中构建高保真模拟域，弥合合成增强与真实病理之间的差距

Result: 在混合数据集上的综合实验表明，该框架显著提升了nnU-Net和U-Mamba等最先进模型的数据效率和鲁棒性

Conclusion: 为有限标注预算下的高性能医学图像分析提供了有效策略，通过双增强框架最大化现有数据集的价值
...
### [5] [3DGesPolicy: Phoneme-Aware Holistic Co-Speech Gesture Generation Based on Action Control](https://arxiv.org/abs/2601.18451)
*Xuanmeng Sha,Liyun Zhang,Tomohiro Mashita,Naoya Chiba,Yuki Uranishi*

Main category: cs.CV

TL;DR: 3DGesPolicy：基于扩散策略的动作框架，通过将全身手势生成重构为连续轨迹控制问题，解决了现有方法中身体运动语义不协调和空间不稳定问题，实现了语音、身体动作和面部表情的细粒度对齐。


<details>
  <summary>Details</summary>
Motivation: 现有全身协同手势生成方法存在两个主要问题：1）身体运动语义不协调，2）空间不稳定无意义运动。这是由于现有方法采用部分分解或帧级回归方法导致的。

Method: 提出3DGesPolicy框架：1）将全身手势生成重构为连续轨迹控制问题，使用机器人学中的扩散策略；2）将帧间变化建模为统一的整体动作，学习帧间整体手势运动模式；3）提出GAP融合模块，深度整合语音、身体动作和音素多模态信号，实现细粒度对齐。

Result: 在BEAT2数据集上的大量定量和定性实验表明，3DGesPolicy在生成自然、富有表现力且高度语音对齐的全身手势方面优于其他最先进方法。

Conclusion: 3DGesPolicy通过动作框架和GAP融合模块，有效解决了全身协同手势生成中的语义协调和空间稳定问题，实现了更自然、表达力更强的语音驱动手势生成。
...
### [6] [Diagnosis Support of Sickle Cell Anemia by Classifying Red Blood Cell Shape in Peripheral Blood Images](https://arxiv.org/abs/2601.17032)
*Wilkie Delgado-Font,Miriela Escobedo-Nicot,Manuel González-Hidalgo,Silena Herold-Garcia,Antoni Jaume-i-Capó,Arnau Mir*

Main category: cs.CV

TL;DR: 提出一种基于外周血涂片图像分析的自动化方法，用于区分计数正常和异常红细胞，特别针对镰状细胞贫血的诊断。
...
Method: 使用Chan-Vese主动轮廓模型分割图像中的红细胞，然后通过圆形形状因子(CSF)和椭圆形状因子(ESF)等基本形状分析描述符对红细胞进行分类（正常、细长或其他变形），并对簇中部分遮挡的细胞进行椭圆调整。

Result: 实验结果显示，该方法在镰状细胞贫血诊断方面优于现有方法，正常细胞的F-measure为0.97，细长细胞的F-measure为0.95，整体多类性能指标表现优异。

Conclusion: 该方法适合用于镰状细胞贫血的临床治疗和诊断支持，能够有效自动化红细胞分类，提高诊断准确性和效率。
...
### [7] [REMAC: Reference-Based Martian Asymmetrical Image Compression](https://arxiv.org/abs/2601.18547)
...
<details>
  <summary>Details</summary>
Motivation: 现有学习型压缩方法存在两个关键问题：1) 忽略了火星上极其有限的计算资源；2) 没有利用火星图像间的高度相似性来提升压缩性能。火星到地球通信信道受限，需要高效的火星图像压缩方法。

Method: 提出基于参考的火星非对称图像压缩方法(REMAC)：1) 参考引导的熵模块和参考解码器利用参考图像信息；2) 参考解码器采用深度多尺度架构建模长程空间依赖；3) 潜在特征循环机制进一步缓解计算约束。

Result: 与最先进方法相比，REMAC将编码器复杂度降低43.51%，同时获得0.2664 dB的BD-PSNR增益，在计算效率和压缩性能之间取得良好平衡。

Conclusion: REMAC通过利用火星图像的纹理、颜色和语义层面的强相似性，将计算负担从资源受限的编码器转移到资源丰富的解码器，为火星图像压缩提供了高效实用的解决方案。
...
### [8] [AMVICC: A Novel Benchmark for Cross-Modal Failure Mode Profiling for VLMs and IGMs](https://arxiv.org/abs/2601.17037)
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态模型快速发展，但视觉语言模型在理解和生成基础视觉概念（如物体方向、数量、空间关系）方面仍存在明显不足，需要系统评估这些失败模式以指导模型改进。

Method: 通过将MMVP基准问题转化为显式和隐式提示，创建AMVICC基准测试，在9个视觉推理类别中测试11个MLLM和3个IGM，系统分析跨模态失败模式。

Result: 失败模式在模型和模态间既有共享也有特异性；图像生成模型在响应提示时难以操控特定视觉组件，特别是在显式提示下；模型在基础视觉推理任务上表现不佳。
...
### [9] [AGSP-DSA: An Adaptive Graph Signal Processing Framework for Robust Multimodal Fusion with Dynamic Semantic Alignment](https://arxiv.org/abs/2601.18589)
...
Conclusion: AGSP-DSA框架在多模态学习中表现出色，在情感分析、事件识别和多媒体分类任务中具有高效性和鲁棒性
...
### [10] [Hybrid Deep Feature Extraction and ML for Construction and Demolition Debris Classification](https://arxiv.org/abs/2601.17038)
*Obai Alashram,Nejad Alagha,Mahmoud AlKakuri,Zeeshan Swaveel,Abigail Copiaco*

Main category: cs.CV

TL;DR: 本文提出了一种混合视觉管道，结合深度特征提取与传统机器学习分类器，用于建筑垃圾自动分类，在真实数据集上达到99.5%的准确率。


<details>
  <summary>Details</summary>
Motivation: 建筑行业产生大量废弃物，有效的分类对可持续废物管理和资源回收至关重要。传统方法需要自动化解决方案来应对真实工地环境的挑战。

Method: 使用预训练的Xception网络提取深度特征，然后结合多种传统机器学习分类器（SVM、kNN、Bagged Trees、LDA、逻辑回归）进行分类。收集了包含1800张平衡高质量图像的真实建筑工地数据集。

Result: 混合管道在四类材料（陶瓷/瓷砖、混凝土、垃圾/废弃物、木材）分类上达到99.5%的准确率和宏F1分数，超越了更复杂的端到端深度学习方法。

Conclusion: 该方法为稳健、可现场部署的建筑垃圾识别提供了操作优势，并为未来与机器人和现场自动化系统集成指明了方向。
...
### [11] [MANGO: A Global Single-Date Paired Dataset for Mangrove Segmentation](https://arxiv.org/abs/2601.17039)
*Junhyuk Heo,Beomkyu Choi,Hyunjin Shin,Darongsae Kwon*

Main category: cs.CV

TL;DR: MANGO是一个大规模全球红树林数据集，包含42,703个图像-掩码对，覆盖124个国家，旨在解决现有数据集在单日期图像对、全球覆盖和可访问性方面的限制。


<details>
  <summary>Details</summary>
Motivation: 红树林对气候变化减缓至关重要，但现有数据集存在三个主要问题：1) 只提供年度地图产品而没有单日期图像-掩码对；2) 仅限于特定区域而非全球覆盖；3) 许多数据集不公开可用。这些限制阻碍了深度学习在红树林监测中的应用。

Method: 方法包括：1) 收集2020年红树林区域所有可用的Sentinel-2影像；2) 使用目标检测驱动的方法选择与红树林年度掩码对齐的最佳单日期观测；3) 利用像素级坐标参考确保自适应和代表性的图像-掩码配对；4) 采用国家不相交分割进行基准测试。

Result: 构建了MANGO数据集，包含42,703个标记的图像-掩码对，覆盖124个国家。提供了多种语义分割架构的基准测试结果，为可扩展和可靠的全球红树林监测奠定了基础。

Conclusion: MANGO数据集解决了现有红树林监测数据集的局限性，通过提供大规模、全球覆盖、单日期图像-掩码对的数据集，推动了深度学习在红树林监测中的应用，为实现可扩展和可靠的全球红树林监测提供了重要资源。
...
### [12] [FP-THD: Full page transcription of historical documents](https://arxiv.org/abs/2601.17040)
*H Neji,J Nogueras-Iso,J Lacasta,MÁ Latre,FJ García-Marco*

Main category: cs.CV

TL;DR: 提出一个用于转录15-16世纪拉丁文历史文献的管道，通过布局分析模型提取文本行，再结合OCR模型进行识别，保留特殊字符和符号。
...
Method: 扩展现有文本行识别方法，结合布局分析模型。首先使用布局分析模型分析历史文本图像提取文本行，然后通过OCR模型处理生成完整的数字化页面。

Result: 该管道能够有效处理页面并产生高效结果。在多个数据集上的评估表明，掩码自编码器能够有效处理不同类型文本，包括手写体、印刷体和多语言文本。

Conclusion: 提出的管道方法能够有效转录历史文献，保留特殊特征，为历史文献的数字化处理提供了有效解决方案。
...
### [13] [Arabic Sign Language Recognition using Multimodal Approach](https://arxiv.org/abs/2601.17041)
*Ghadeer Alanazi,Abir Benabid*

Main category: cs.CV

TL;DR: 本文提出一种结合Leap Motion和RGB摄像头的多模态阿拉伯手语识别系统，通过融合两种传感器数据提高识别准确率，在18个手语词上达到78%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有阿拉伯手语识别系统依赖单一传感器（如Leap Motion或RGB摄像头），难以准确追踪复杂手部方向和3D运动，需要多模态方法提升识别性能。

Method: 采用双并行子网络架构：1）针对Leap Motion数据的自定义密集神经网络，包含dropout和L2正则化；2）基于微调VGG16模型的图像子网络，使用数据增强技术。两种模态的特征表示在融合模型中拼接，通过全连接层处理，最后用SoftMax分类分析手势的空间和时间特征。

Result: 在包含18个阿拉伯手语词的自定义数据集上测试，正确识别13个词，总体准确率达到78%，初步验证了多模态融合在手语识别中的可行性。
...
### [14] [Interpretable and Sparse Linear Attention with Decoupled Membership-Subspace Modeling via MCR2 Objective](https://arxiv.org/abs/2601.17042)
*Tianyuan Liu,Libin Hou,Linyuan Wang,Bin Yan*

Main category: cs.CV

TL;DR: 提出解耦成员矩阵与子空间矩阵的注意力机制DMSA，在视觉任务中提升精度与效率


<details>
  <summary>Details</summary>
Motivation: 现有MCR2驱动的白盒transformer中，成员矩阵与子空间矩阵紧密耦合，导致错误token投影下的冗余编码，需要解耦以提升效率

Method: 解耦MCR2目标中的成员矩阵与子空间U，直接从输入学习成员矩阵，从全空间S推导稀疏子空间，通过梯度展开得到可解释的稀疏线性注意力算子DMSA

Result: 在ToST中用DMSA替换注意力模块（称为DMST），在ImageNet-1K上比ToST提升1.08%-1.45% top-1精度，计算效率更高，编码减少更快

Conclusion: DMSA通过解耦成员矩阵与子空间，实现了更高效、可解释的稀疏线性注意力，为视觉建模提供了可靠的白盒解决方案
...
### [15] [Atomic Depth Estimation From Noisy Electron Microscopy Data Via Deep Learning](https://arxiv.org/abs/2601.17046)
...
<details>
  <summary>Details</summary>
Motivation: 传统TEM图像受噪声影响严重，难以准确提取3D原子级结构信息。需要开发能够从噪声图像中稳健提取深度信息的方法。

Method: 将深度估计转化为语义分割问题，训练深度卷积神经网络生成像素级深度分割图。使用模拟数据添加合成噪声进行训练，然后将方法应用于CeO2纳米颗粒的模拟和真实TEM数据。

Result: 实验表明，该方法能够准确、校准且对噪声具有鲁棒性地估计原子柱的深度，在模拟和真实TEM数据上都取得了良好效果。

Conclusion: 该方法成功解决了从噪声TEM图像中提取3D原子级信息的挑战，为材料科学中的原子尺度表征提供了新的有效工具。
...
### [16] [A Contrastive Pre-trained Foundation Model for Deciphering Imaging Noisomics across Modalities](https://arxiv.org/abs/2601.17047)
*Yuanjie Gu,Yiqun Wang,Chaohui Yu,Ang Xuan,Fan Wang,Zhi Lu,Biqin Dong*

Main category: cs.CV

TL;DR: Noisomics框架通过对比预训练基础模型，将噪声从干扰转变为信息源，仅需100个样本即可超越传统需要10万样本的方法，实现零样本泛化


<details>
  <summary>Details</summary>
Motivation: 现代传感器将物理信号与复杂算法伪影纠缠在一起，传统方法需要大量监督数据且将噪声视为干扰而非信息源，难以解耦这些因素

Method: 引入Noisomics框架，通过对比预训练基础模型，利用流形假设和合成噪声基因组，采用对比学习解耦语义信号和随机扰动

Result: 打破了传统深度学习缩放定律，仅用100个训练样本就超越了需要10万样本的监督基线，在12个不同领域数据集上实现零样本泛化，估计误差减少63.8%，决定系数提高85.1%

Conclusion: 将随机退化重新定义为重要的信息源，无需设备校准即可实现精确成像诊断，从消费摄影到深层组织显微镜都有应用价值
...
### [17] [SiMiC: Context-Aware Silicon Microstructure Characterization Using Attention-Based Convolutional Neural Networks for Field-Emission Tip Analysis](https://arxiv.org/abs/2601.17048)
...
TL;DR: SiMiC：基于注意力机制CNN的硅微结构表征框架，用于场发射尖端分析，通过深度学习自动提取SEM图像中的形态特征，减少人工干预并提高测量一致性。


<details>
  <summary>Details</summary>
Motivation: 传统SEM分析需要人工评估特征几何形状，耗时且可重复性差。需要一种自动化、高效的方法来表征硅微结构，以推进微尺度制造、质量控制和器件性能优化。

Method: 开发了硅基场发射尖端专用数据集，构建了包含注意力机制的自定义CNN架构，用于多类微结构分类和尺寸预测。注意力机制增强了模型的可解释性。

Result: 与传统图像处理技术相比，SiMiC实现了高精度测量，同时保持了可解释性。建立了数据驱动的微结构分析基础，可直接关联到场发射性能。

Conclusion: 该框架为微结构分析提供了新方法，能够关联发射器几何形状与发射行为，指导优化冷阴极和SEM电子源设计。相关数据集和算法已开源作为该领域基准。
...
### [18] [Summary of the Unusual Activity Recognition Challenge for Developmental Disability Support](https://arxiv.org/abs/2601.17049)
*Christina Garcia,Nhat Tan Le,Taihei Fujioka,Umang Dobhal,Milyun Ni'ma Shoumi,Thanh Nha Nguyen,Sozo Inoue*

Main category: cs.CV

TL;DR: ISAS 2025举办的"Recognize the Unseen"挑战赛，旨在通过姿态数据自动识别发育障碍人士设施中的异常行为，吸引了40个团队参与，使用LOSO评估策略，结果突显了在噪声低维数据中建模罕见突发行为的挑战。


<details>
  <summary>Details</summary>
Motivation: 该挑战赛旨在解决发育障碍人士设施中自动识别异常行为的迫切需求，使用非侵入式姿态估计数据，以促进医疗保健和行为监测领域的社会责任AI应用发展。

Method: 挑战赛使用从模拟场景视频中提取的骨架关键点数据，数据集反映了真实世界的行为不平衡和时间不规则性，采用Leave-One-Subject-Out评估策略确保主体无关的泛化能力，主要使用宏平均F1分数评估模型性能。

Result: 40个团队参与了挑战，应用了从经典机器学习到深度学习架构的多种方法。结果表明，在噪声低维数据中建模罕见、突发的行为具有挑战性，强调了捕捉行为建模中时间和上下文细微差别的重要性。

Conclusion: 该挑战赛突显了异常行为识别在低维姿态数据中的技术挑战，为医疗保健和行为监测领域的社会责任AI应用提供了重要见解，有助于未来相关技术的发展。
...
### [19] [Single-Pixel Vision-Language Model for Intrinsic Privacy-Preserving Behavioral Intelligence](https://arxiv.org/abs/2601.17050)
*Hongjun An,Yiliang Song,Jiawei Shao,Zhe Sun,Xuelong Li*

Main category: cs.CV

TL;DR: SP-VLM框架通过单像素传感实现隐私保护监控，在隐私敏感环境中平衡安全监控与个人隐私保护


<details>
  <summary>Details</summary>
Motivation: 在厕所、更衣室等隐私敏感环境中，传统监控因隐私法规和伦理问题受限，但不良社会互动（如霸凌、骚扰）又需要及时干预，因此需要一种既能保护隐私又能进行安全监控的解决方案

Method: 提出单像素视觉语言模型（SP-VLM），通过低维单像素模态捕捉人体动态，结合视觉语言集成推断复杂行为模式，实现隐私保护设计

Result: 单像素传感能有效抑制身份可恢复性，使先进人脸识别系统在低于临界采样率时失效；同时SP-VLM能从严重降级的单像素观测中提取有意义的语义行为，实现异常检测、人数统计和活动理解

Conclusion: SP-VLM在特定采样率范围内既能提取行为智能又能强保护个人身份，为人权对齐的安全监控提供了可行路径，支持及时干预而不侵犯隐私敏感空间的隐私
...
### [20] [Synthetic Data Guided Feature Selection for Robust Activity Recognition in Older Adults](https://arxiv.org/abs/2601.17053)
*Shuhao Que,Dieuwke van Dartel,Ilse Heeringa,Han Hegeman,Miriam Vollenbroek-Hutten,Ying Wang*

Main category: cs.CV

TL;DR: 开发用于髋部骨折康复的稳健人类活动识别系统，使用合成数据提升老年人活动监测准确性


<details>
  <summary>Details</summary>
Motivation: 髋部骨折康复期间的身体活动监测对老年患者功能恢复至关重要，但现有基于可穿戴设备的监测系统通常针对中年人群开发，在步态较慢且多变的老年人中表现不可靠

Method: 研究纳入24名80岁以上健康老年人，在模拟自由生活条件下进行日常活动（行走、站立、坐、躺、姿势转换）75分钟，佩戴下背部和前大腿两个加速度计。采用留一法交叉验证评估模型鲁棒性，并利用合成数据提升模型泛化能力

Result: 基于合成数据指导的特征干预模型（FIM）实现了可靠的活动识别：行走F1分数0.896、站立0.927、坐0.997、躺0.937、姿势转换0.816。相比无合成数据的对照模型，FIM显著改善了临床相关性高的姿势转换检测
...
### [21] [Ego4OOD: Rethinking Egocentric Video Domain Generalization via Covariate Shift Scoring](https://arxiv.org/abs/2601.17056)
*Zahra Vaseqi,James Clark*

Main category: cs.CV

TL;DR: 提出了Ego4OOD基准测试，用于评估第一人称视频动作识别的域泛化能力，通过聚类度量协变量偏移，并使用轻量级二元分类方法取得竞争性结果。


<details>
  <summary>Details</summary>
Motivation: 现有第一人称视频域泛化基准常混淆协变量偏移和概念偏移，难以可靠评估模型跨输入分布的泛化能力。需要创建强调可测量协变量多样性同时减少概念偏移的基准。

Method: 1) 从Ego4D创建Ego4OOD基准，包含8个地理域，使用语义一致的时刻级动作类别；2) 提出基于聚类的协变量偏移度量；3) 使用一对多二元训练目标，将多类动作识别分解为独立二元分类任务；4) 采用轻量级两层全连接网络。

Result: 轻量级两层全连接网络在Argo1M和Ego4OOD上取得与最先进方法竞争的性能，参数更少且无需额外模态。实证分析显示协变量偏移度量与识别性能存在明确关系。

Conclusion: Ego4OOD基准和协变量偏移度量为研究第一人称视频的分布外泛化提供了受控基准和定量域表征，二元分类方法在协变量偏移下能有效减少视觉相似类间的干扰。
...
### [22] [A Computer Vision Pipeline for Iterative Bullet Hole Tracking in Rifle Zeroing](https://arxiv.org/abs/2601.17062)
*Robert M. Belcher,Brendan C. Degryse,Leonard R. Kosta,Christopher J. Lowrance*

Main category: cs.CV

TL;DR: 基于计算机视觉的自动化弹孔检测与迭代跟踪系统，用于步枪瞄准镜校准，结合YOLOv8检测和IoU分析，实现97.0%的检测精度和88.8%的迭代分配准确率。


<details>
  <summary>Details</summary>
Motivation: 传统步枪瞄准镜校准（归零）需要人工检查弹孔，存在安全延迟和人为错误风险。需要自动化系统来实时检测和跟踪不同射击迭代的弹孔。

Method: 1. 使用YOLOv8进行小目标弹孔检测；2. 结合IoU分析区分序列图像中的弹孔；3. 提出新颖的数据增强技术（移除而非添加对象模拟射击序列）；4. 引入基于ORB的透视校正预处理管道标准化目标方向。

Result: 系统在弹孔检测上达到97.0%的平均精度均值（mAP），在将弹孔分配到正确射击迭代上达到88.8%的准确率。

Conclusion: 该系统成功实现了步枪归零的自动化弹孔检测和迭代跟踪，虽然专为步枪校准设计，但其框架可广泛应用于需要区分视觉相似对象时间顺序的领域。
...
### [23] [A Mechanistic View on Video Generation as World Models: State and Dynamics](https://arxiv.org/abs/2601.17067)
...
TL;DR: 论文提出视频生成模型作为世界模型的新分类法，关注状态构建和动力学建模两个支柱，并倡导从视觉保真度转向功能基准测试的评估转变。


<details>
  <summary>Details</summary>
Motivation: 大规模视频生成模型已展现出物理连贯性，有潜力成为世界模型，但当前"无状态"视频架构与经典状态中心世界模型理论之间存在差距，需要弥合这一差距。

Method: 提出以状态构建和动力学建模为核心的新分类法：状态构建分为隐式范式（上下文管理）和显式范式（潜在压缩）；动力学建模通过知识整合和架构重构分析；倡导从视觉保真度转向功能基准测试的评估转变。

Result: 建立了视频生成模型作为世界模型的系统分类框架，识别出两个关键前沿：通过数据驱动记忆和压缩保真度增强持久性，以及通过潜在因子解耦和推理先验整合推进因果关系。

Conclusion: 通过解决这些挑战，该领域可以从生成视觉上合理的视频演进到构建稳健、通用的世界模拟器，实现从视频生成到世界建模的范式转变。
...
### [24] [Superpixel-Based Image Segmentation Using Squared 2-Wasserstein Distances](https://arxiv.org/abs/2601.17071)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于平均颜色距离的超像素合并策略在处理强不均匀图像时效果有限，需要一种数学统一的框架来提高分割准确性

Method: 两级聚类方法：1) 通过线性最小二乘分配问题（离散最优传输特例）将像素聚类为超像素；2) 使用平方2-Wasserstein距离贪婪合并超像素为对象级分割

Result: 数值实验表明该方法在挑战性图像上提高了分割准确性，同时保持了高计算效率

Conclusion: 基于最优传输的分布距离框架为图像分割提供了数学统一的解决方案，在保持效率的同时改善了强不均匀图像的分割性能
...
### [25] [GlassesGB: Controllable 2D GAN-Based Eyewear Personalization for 3D Gaussian Blendshapes Head Avatars](https://arxiv.org/abs/2601.17088)
...
<details>
  <summary>Details</summary>
Motivation: 现有虚拟试戴系统大多只能使用预定义的眼镜模板，缺乏细粒度的用户驱动定制能力。虽然GlassesGAN支持个性化2D眼镜设计，但仅限于2D图像生成，无法满足VR应用中对3D头部虚拟形象的个性化眼镜设计需求。

Method: 结合3D高斯混合形状在头部重建中的成功经验，将2D生成式定制与3D头部虚拟形象渲染技术相结合，提出GlassesGB框架，支持3D头部虚拟形象的可定制眼镜生成。

Result: GlassesGB有效桥接了2D生成式定制与3D头部虚拟形象渲染，解决了VR应用中个性化眼镜设计的挑战。

Conclusion: 该框架为VR应用中的个性化眼镜设计提供了有效的解决方案，实现了从2D定制到3D渲染的完整流程。
...
### [26] [GRASP: Guided Region-Aware Sparse Prompting for Adapting MLLMs to Remote Sensing](https://arxiv.org/abs/2601.17089)
...
TL;DR: 提出GRASP方法，一种参数高效微调策略，通过引导区域感知稀疏提示解决遥感图像中MLLMs的过拟合和目标细节忽略问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs微调方法直接应用于遥感图像时存在过拟合背景噪声或忽略目标细节的问题，主要由于遥感图像的大尺度变化、稀疏目标分布和复杂区域语义特征，限制了MLLMs在遥感任务中的有效性。

Method: 提出GRASP方法：1）引入与冻结视觉标记网格中提取的空间块相关的空间结构化软提示；2）通过问题引导的稀疏融合机制，动态将任务特定上下文聚合到紧凑的全局提示中，使模型能够关注相关区域同时过滤背景噪声。

Result: 在多个RSVQA基准测试上的广泛实验表明，GRASP相比现有微调和基于提示的方法实现了竞争性性能，同时保持了高参数效率。

Conclusion: GRASP方法有效解决了遥感图像中MLLMs的挑战，通过参数高效的方式提升了视觉问答任务的性能，为遥感领域的多模态大语言模型应用提供了有效解决方案。
...
### [27] [LoD Sketch Extraction from Architectural Models Using Generative AI: Dataset Construction for Multi-Level Architectural Design Generation](https://arxiv.org/abs/2601.17095)
*Xusheng Du,Athiwat Kongkaeo,Ye Zhang,Haoran Xie*

Main category: cs.CV

TL;DR: 提出一个基于生成式AI的自动LoD草图提取框架，能够从高细节建筑模型逐步简化生成几何一致、层次连贯的多层次细节表示，解决训练数据缺乏问题。


<details>
  <summary>Details</summary>
Motivation: 建筑设计中需要多层次细节表示以实现从概念体量到详细建模的平滑过渡，但传统LoD建模依赖人工操作，耗时费力且容易产生几何不一致。生成式AI为从草图输入生成多级建筑模型提供了新可能，但缺乏高质量配对的LoD训练数据限制了其应用。

Method: 提出自动LoD草图提取框架，集成计算机视觉技术和生成式AI方法，建立从详细表示到体量抽象的渐进提取流程，逐步简化高细节建筑模型以生成几何一致的多LoD表示。
...
Conclusion: 该框架有效保留了全局结构，同时在不同LoD级别实现了渐进语义简化，为AI驱动的多层次建筑生成和分层建模提供了可靠数据和技术支持。
...
### [28] [Performance uncertainty in medical image analysis: a large-scale investigation of confidence intervals](https://arxiv.org/abs/2601.17103)
...
TL;DR: 该研究通过大规模实证分析评估医学影像AI性能不确定性量化的置信区间方法，揭示了影响CI可靠性的关键因素，为制定报告指南提供依据。


<details>
  <summary>Details</summary>
Motivation: 医学影像AI的性能不确定性量化对于可靠验证和临床转化至关重要，但社区对多种置信区间方法及其在不同场景下的行为缺乏了解，需要填补这一知识空白。

Method: 在24个分割和分类任务上进行大规模实证分析，每个任务组使用19个训练模型，涵盖广泛的常用性能指标、多种聚合策略和多个广泛采用的置信区间方法，评估各方法的可靠性（覆盖率）和精度（宽度）。

Result: 发现五个主要结果：1）可靠CI所需样本量从几十到几千例不等；2）CI行为受性能指标选择强烈影响；3）聚合策略显著影响CI可靠性；4）机器学习问题类型（分割vs分类）调节这些效应；5）不同CI方法在不同用例中的可靠性和精度不同。

Conclusion: 这些结果构成了未来制定医学影像AI性能不确定性报告指南的关键组成部分，强调了根据具体研究参数选择适当CI方法的重要性。
...
### [29] [StealthMark: Harmless and Stealthy Ownership Verification for Medical Segmentation via Uncertainty-Guided Backdoors](https://arxiv.org/abs/2601.17107)
...
TL;DR: 提出StealthMark方法，通过微妙调节模型不确定性而不改变分割输出，在医学分割模型中嵌入可验证的水印，实现黑盒条件下的所有权验证。


<details>
  <summary>Details</summary>
Motivation: 医学数据标注成本高且受隐私限制，训练好的分割模型成为有价值的IP需要保护。现有保护技术主要针对分类和生成任务，医学分割模型保护研究不足。

Method: 通过调节模型不确定性嵌入水印但不改变最终分割输出，利用LIME等模型无关解释方法提取特征归因，在特定触发条件下显示可验证的QR码水印。

Result: 在4个医学影像数据集和5个主流分割模型上验证，ASR超过95%，Dice和AUC分数下降小于1%，显著优于基于后门的水印方法。

Conclusion: StealthMark方法有效、隐蔽且无害，在保持模型性能的同时实现所有权验证，具有实际部署潜力。
...
### [30] [iFSQ: Improving FSQ for Image Generation with 1 Line of Code](https://arxiv.org/abs/2601.17124)
...
TL;DR: iFSQ通过简单的激活函数替换解决了FSQ的量化问题，实现了离散与连续表示的平衡，发现4比特/维度是最佳平衡点，并揭示了AR模型快速收敛但扩散模型上限更高的特性。


<details>
  <summary>Details</summary>
Motivation: 图像生成领域存在自回归模型（离散token）和扩散模型（连续潜在变量）的分裂，这源于VQ-VAE和VAE的区别，阻碍了统一建模和公平基准测试。FSQ提供了理论桥梁，但其等间隔量化会导致激活崩溃，需要在重建保真度和信息效率之间权衡。

Method: 提出iFSQ方法，将原始FSQ的激活函数替换为分布匹配映射，强制均匀先验。这种简单策略只需一行代码，但数学上保证了最优的bin利用率和重建精度。使用iFSQ作为受控基准，分析离散与连续表示的最佳平衡点，并将表示对齐（REPA）扩展到AR模型中。

Result: 发现离散与连续表示的最佳平衡点约为4比特/维度。在相同重建约束下，AR模型初始收敛快，但扩散模型能达到更高的性能上限，表明严格的序列顺序可能限制生成质量的上限。通过将REPA扩展到AR模型，开发了LlamaGen-REPA。

Conclusion: iFSQ通过简单的激活函数修改解决了FSQ的关键缺陷，为离散和连续表示提供了统一的基准。研究揭示了两种模型架构的收敛特性差异，并指出4比特/维度是最佳平衡点，为未来的图像生成模型设计提供了重要指导。
...
### [31] [Scaling medical imaging report generation with multimodal reinforcement learning](https://arxiv.org/abs/2601.17151)
...
TL;DR: UniRG是一个用于医学影像报告生成的通用框架，通过强化学习直接优化评估指标，在胸部X光报告生成中实现了新的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 前沿模型在自然语言理解和推理方面表现出色，但在多模态理解（特别是生物医学等高价值垂直领域）方面仍有重大能力差距。医学影像报告生成是一个典型例子，监督微调容易过度拟合表面模式。

Method: 提出通用报告生成（UniRG）框架，利用强化学习作为统一机制，直接针对最终应用设计的评估指标进行优化，从而超越监督微调，实现跨机构和临床实践的持久泛化。

Result: 在公开的胸部X光数据上训练UniRG-CXR，在权威的ReXrank基准测试中，UniRG-CXR创造了新的整体SOTA，大幅超越先前的最先进方法。

Conclusion: UniRG框架通过强化学习优化评估指标，显著提高了医学影像报告生成的性能，并实现了跨机构的泛化能力，为医学影像报告生成提供了有效的解决方案。
...
### [32] [LGDWT-GS: Local and Global Discrete Wavelet-Regularized 3D Gaussian Splatting for Sparse-View Scene Reconstruction](https://arxiv.org/abs/2601.17185)
*Shima Salehi,Atharva Agashe,Andrew J. McFarland,Joshua Peeples*

Main category: cs.CV

TL;DR: 提出一种用于少样本3D重建的新方法，通过全局和局部频率正则化来稳定几何并保留稀疏视图条件下的细节，同时发布了一个多光谱温室数据集和开源基准测试包。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射（3DGS）模型在稀疏视图条件下存在几何不稳定和细节丢失的问题，需要一种能够同时稳定几何并保留精细细节的方法。此外，缺乏专门用于评估3DGS方法的多光谱数据集和标准化基准测试协议。

Method: 提出一种集成全局和局部频率正则化的少样本3D重建方法，通过频率正则化来稳定几何并保留细节。同时创建了一个包含四种光谱波段的多光谱温室数据集，涵盖多种植物物种在受控条件下的数据。开发了一个开源基准测试包，定义了标准化的少样本重建协议。

Result: 在多光谱数据集和标准基准测试上的实验表明，所提出的方法比现有基线实现了更清晰、更稳定和光谱一致的重建效果。该方法在稀疏视图条件下表现出更好的几何稳定性和细节保留能力。

Conclusion: 提出的频率正则化方法有效解决了3DGS在稀疏视图条件下的局限性，同时发布的多光谱数据集和基准测试包为3DGS方法的评估提供了标准化工具，推动了该领域的发展。
...
### [33] [Decoding Psychological States Through Movement: Inferring Human Kinesic Functions with Application to Built Environments](https://arxiv.org/abs/2601.17194)
*Cheyu Lin,Katherine A. Flanigan,Sirajum Munir*

Main category: cs.CV

TL;DR: DUET数据集和运动识别框架：通过隐私保护的骨骼运动数据，将Ekman和Friesen的运动学分类法操作化为社会资本相关行为的交互词汇，解决社会基础设施研究中交互测量不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 当前社会基础设施研究中缺乏一致且隐私保护的方式来测量社会意义交互，导致不同研究对"交互"的操作化定义不同，限制了设计干预措施对社会资本相关行为影响的评估能力。

Method: 引入DUET数据集，基于Ekman和Friesen的运动学分类法（标志、说明、情感表达、适应、调节），涵盖12种二元交互、4种传感模态和3种建筑环境。开发识别框架，直接从隐私保护的骨骼运动推断交流功能，无需手工制作动作-功能词典，采用迁移学习架构。

Result: 对6个开源HAR模型的基准测试量化了DUET上交流功能识别的难度，揭示了单子动作识别扩展到二元社会交互测量的局限性。识别框架显示了运动学功能的结构化聚类，表示质量与分类性能强相关，并能跨主体和环境泛化。

Conclusion: DUET数据集和识别框架为社会基础设施研究提供了隐私保护、一致的交互测量方法，将运动学分类法与社会资本理论对齐，支持设计干预对社会交互影响的有效评估。
...
### [34] [Structural Complexity of Brain MRI reveals age-associated patterns](https://arxiv.org/abs/2601.17211)
*Anzhe Cheng,Italo Ivo Lima Dias Pinto,Paul Bogdan*

Main category: cs.CV

TL;DR: 将结构复杂度分析扩展到三维MRI信号，通过多尺度粗粒化量化信息损失，发现脑结构复杂度随年龄系统性下降


<details>
  <summary>Details</summary>
Motivation: 将结构复杂度分析框架扩展到三维信号，特别是脑MRI数据，以捕捉体积数据的多尺度组织特征，解决传统块状方法在粗分辨率下不稳定的问题

Method: 引入滑动窗口粗粒化方案，在逐渐增大的空间尺度上对信号进行粗粒化处理，量化连续分辨率之间的信息损失，相比传统块状方法提供更平滑的估计和更好的粗尺度鲁棒性

Result: 分析成年中晚期的大规模结构MRI数据集发现，结构复杂度随年龄系统性下降，最显著的效果出现在较粗的尺度上

Conclusion: 结构复杂度是分析3D成像数据多尺度特征的可靠信号处理工具，同时证明其能够从脑MRI预测生物年龄的实用性
...
### [35] [Spatiotemporal Semantic V2X Framework for Cooperative Collision Prediction](https://arxiv.org/abs/2601.17216)
*Murat Arda Onsu,Poonam Lohan,Burak Kantarci,Aisha Syed,Matthew Andrews,Sean Kennedy*

Main category: cs.CV

TL;DR: 提出基于语义V2X的实时碰撞预测框架，使用V-JEPA生成时空语义嵌入，通过传输语义而非原始视频，在降低通信开销的同时提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 智能交通系统需要实时碰撞预测，但传统方法传输原始视频或高维传感器数据受限于车联网带宽和延迟约束，不切实际。

Method: 1) 路侧单元摄像头使用V-JEPA生成未来帧的时空语义嵌入；2) 构建城市交通数字孪生环境生成多样化交通场景；3) 通过V2X链路传输语义嵌入到车辆；4) 车辆端使用轻量级注意力探针和分类器解码预测碰撞。

Result: 框架在适当处理方法下，碰撞预测F1分数提升10%，同时传输需求相比原始视频减少四个数量级，验证了语义V2X通信在协同实时碰撞预测中的潜力。

Conclusion: 语义V2X框架通过传输任务相关的语义嵌入而非原始数据，显著降低通信开销并保持预测准确性，为智能交通系统中的协同实时碰撞预测提供了可行方案。
...
### [36] [Semi-Supervised Domain Adaptation with Latent Diffusion for Pathology Image Classification](https://arxiv.org/abs/2601.17228)
*Tengyue Zhang,Ruiwen Ding,Luoting Zhuang,Yuxiao Wu,Erika F. Rodriguez,William Hsu*

Main category: cs.CV

TL;DR: 提出基于潜在扩散模型的半监督域适应框架，通过生成保持组织形态且具有目标域特征的合成图像，改善计算病理学中的域泛化问题。


<details>
  <summary>Details</summary>
Motivation: 计算病理学中的深度学习模型常因域偏移而无法在不同队列和机构间泛化。现有方法要么无法利用目标域未标记数据，要么依赖可能扭曲组织结构的图像翻译，影响模型准确性。

Method: 提出半监督域适应框架，使用在源域和目标域未标记数据上训练的潜在扩散模型，通过条件化基础模型特征、队列身份和组织制备方法，生成保持组织结构并引入目标域外观特征的合成图像。这些合成图像与源队列的真实标记图像结合训练下游分类器。

Result: 在肺腺癌预后任务上验证，目标队列测试集的加权F1分数从0.611提升至0.706，宏观F1分数从0.641提升至0.716，且不降低源队列性能。

Conclusion: 目标感知的扩散基合成数据增强为改善计算病理学中的域泛化提供了有前景且有效的方法。
...
### [37] [C-RADIOv4 (Tech Report)](https://arxiv.org/abs/2601.17237)
*Mike Ranzinger,Greg Heinrich,Collin McCarthy,Jan Kautz,Andrew Tao,Bryan Catanzaro,Pavlo Molchanov*

Main category: cs.CV

TL;DR: C-RADIOv4是基于AM-RADIO/RADIOv2.5设计的新版本模型系列，通过多教师蒸馏技术整合SigLIP2、DINOv3和SAM3三种教师模型的能力，在保持相同计算复杂度下显著提升下游任务性能，并新增任意分辨率支持、ViTDet选项和宽松许可证。


<details>
  <summary>Details</summary>
Motivation: 通过多教师蒸馏技术，构建一个统一的学生模型，能够保留并改进多个教师模型的独特能力，从而在保持计算效率的同时提升模型在下游任务中的表现。

Method: 采用多教师蒸馏方法，使用SigLIP2、DINOv3和SAM3三种教师模型进行知识蒸馏，构建C-RADIOv4模型系列（包括412M参数的-SO400M和631M参数的-H变体），并改进任意分辨率支持，重新引入ViTDet选项以提高高分辨率下的效率。

Result: C-RADIOv4在核心指标上有所改进，获得了模仿SAM3的新能力，增强了任意分辨率支持，通过ViTDet选项显著提高了高分辨率处理效率，并采用宽松许可证发布。

Conclusion: C-RADIOv4模型系列通过多教师蒸馏成功整合了多个先进模型的优势，在保持计算效率的同时显著提升了性能，并提供了更好的灵活性和实用性。
...
### [38] [Multi-stage Bridge Inspection System: Integrating Foundation Models with Location Anonymization](https://arxiv.org/abs/2601.17254)
*Takato Yasuno*

Main category: cs.CV

TL;DR: 日本桥梁损伤检测系统，在保护区域隐私的同时实现高效损伤识别，使用SAM3检测钢筋腐蚀，DBSCAN补全遗漏区域，高斯模糊保护施工标志，优化OCR精度，单图处理仅需1.7秒。


<details>
  <summary>Details</summary>
Motivation: 日本法律规定每五年需对基础设施进行目视检查。现场拍摄的损伤图像常包含混凝土裂缝和钢筋暴露，同时施工标志会泄露区域信息。为安全使用基础设施而不引起公众焦虑，需要在保护区域信息的同时准确提取损伤特征并可视化关键修复指标。

Method: 使用Segment Anything Model (SAM) 3检测钢筋腐蚀，DBSCAN算法自动补全遗漏区域。通过高斯模糊检测和保护施工标志区域。采用四种预处理方法提高OCR精度，GPU优化使单图处理仅需1.7秒。技术栈包括SAM3、PyTorch、OpenCV、pytesseract和scikit-learn。

Result: 开发出具有区域隐私保护能力的开源桥梁损伤检测系统，实现高效桥梁检查，单张图像处理时间仅1.7秒，在保护区域信息的同时准确提取损伤特征。

Conclusion: 该系统成功实现了桥梁损伤检测与区域隐私保护的平衡，为基础设施安全使用提供了技术解决方案，既能满足法律检查要求，又能避免公众不必要的焦虑。
...
### [39] [FineVAU: A Novel Human-Aligned Benchmark for Fine-Grained Video Anomaly Understanding](https://arxiv.org/abs/2601.17258)
*João Pereira,Vasco Lopes,João Neves,David Semedo*

Main category: cs.CV

TL;DR: 论文提出FineVAU基准，用于视频异常理解任务，包含新的评估指标FVScore和数据集FineW3，解决现有评估方法无法捕捉LVLM丰富视觉响应的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频异常理解评估方法存在缺陷：n-gram指标无法捕捉LVLM的丰富视觉响应，LLM评估过于关注语言质量而非事实相关性，导致与人类感知不一致。

Method: 1) 将VAU定义为三要素问题：事件(What)、参与实体(Who)、位置(Where)；2) 提出FVScore指标，评估LVLM回答中关键视觉元素的出现；3) 创建FineW3数据集，通过结构化自动流程增强现有标注。

Result: FVScore指标与人类感知对齐度优于现有方法；实验显示LVLM在需要空间和细粒度时间理解的异常事件感知方面存在局限，但在粗粒度、静态信息和强视觉线索事件上表现良好。

Conclusion: FineVAU基准推动了视频异常理解评估的发展，揭示了LVLM在细粒度时空理解方面的不足，为未来研究提供了更精确的评估框架。
...
### [40] [Inference-Time Loss-Guided Colour Preservation in Diffusion Sampling](https://arxiv.org/abs/2601.17259)
*Angad Singh Ahuja,Aarush Ram Anandh*

Main category: cs.CV

TL;DR: 提出一种无需训练、推理时基于区域的颜色控制方法，通过复合损失函数和梯度引导，在Stable Diffusion中实现精确的颜色保持


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散系统在精确颜色控制方面存在不足，特别是在设计工作流中需要满足用户指定的颜色目标时。平均颜色约束的方法虽然能满足整体颜色要求，但会产生局部感知上的明显失败

Method: 结合ROI修复、背景潜在重施加和梯度引导的三步方法：1）基于感兴趣区域的修复实现空间选择性；2）背景潜在重施加防止ROI外颜色漂移；3）使用CIE Lab和线性RGB定义的复合损失进行潜在微调，通过CVaR风格和软最大值惩罚控制像素误差分布的尾部

Result: 该方法提供了实用的、无需训练的机制，能够实现目标颜色保持，可以集成到标准的Stable Diffusion修复流程中。相比仅使用平均颜色的基线方法，能避免局部感知上的失败

Conclusion: 提出的推理时区域约束颜色保持方法有效解决了扩散模型中精确颜色控制的问题，通过分布感知的目标函数和时间相关的调度策略，在保持图像质量的同时实现了精确的颜色控制
...
### [41] [Cross360: 360° Monocular Depth Estimation via Cross Projections Across Scales](https://arxiv.org/abs/2601.17271)
*Kun Huang,Fang-Lue Zhang,Neil Dodgson*

Main category: cs.CV

TL;DR: Cross360：基于交叉注意力的360度深度估计方法，通过整合局部切线投影特征和全局等距柱面投影特征，解决现有方法在全局一致性和局部准确性之间的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 360度深度估计面临表示方法的挑战，需要在保持全局连续性的同时避免球形图像失真。现有方法尝试利用多投影的互补信息，但难以平衡全局和局部一致性，局部补丁特征缺乏全局感知，全局表示无法解决补丁边界特征提取的不一致问题。

Method: 提出Cross360架构，使用交叉注意力整合局部和全局信息：1）交叉投影特征对齐模块，通过交叉注意力将局部切线投影特征与等距柱面投影的360度视野对齐；2）渐进式注意力特征聚合模块，逐步细化多尺度特征。

Result: Cross360在大多数基准数据集上显著优于现有方法，特别是在完整360度图像可用的场景中，证明了其在准确和全局一致深度估计方面的有效性。

Conclusion: Cross360通过交叉注意力机制有效整合局部切线投影特征和全局等距柱面特征，解决了360度深度估计中全局一致性和局部准确性的平衡问题，实现了更精确的深度估计。
...
### [42] [Fluxamba: Topology-Aware Anisotropic State Space Models for Geological Lineament Segmentation in Multi-Source Remote Sensing](https://arxiv.org/abs/2601.17288)
*Jin Bai,Huiyao Zhang,Qi Wen,Shengyang Li,Xiaolin Tian,Atta ur Rahman*

Main category: cs.CV

TL;DR: Fluxamba：一种轻量级拓扑感知特征校正架构，用于地质线性特征分割，通过解耦特征方向与空间位置，在保持实时推理速度的同时显著提升分割精度。


<details>
  <summary>Details</summary>
Motivation: 现有状态空间模型（SSMs）依赖刚性的轴对齐扫描轨迹，与曲线状地质目标存在拓扑不匹配，导致上下文碎片化和特征侵蚀，需要一种能够捕捉复杂各向异性拓扑中长程依赖的方法。

Method: 提出Fluxamba架构，核心是结构通量块（SFB），包含各向异性结构门（ASG）和先验调制流（PMF），解耦特征方向与空间位置，动态沿目标内在几何聚合上下文。还包含分层空间调节器（HSR）进行多尺度语义对齐，以及高保真聚焦单元（HFFU）最大化微弱特征的信噪比。
...
### [43] [Dynamic Meta-Ensemble Framework for Efficient and Accurate Deep Learning in Plant Leaf Disease Detection on Resource-Constrained Edge Devices](https://arxiv.org/abs/2601.17290)
...
Method: DMEF采用自适应权重机制，动态组合MobileNetV2、NASNetMobile和InceptionV3三个轻量级CNN的预测结果，通过优化精度提升(DeltaAcc)和计算效率（模型大小）之间的权衡来更新集成权重。

Result: 在马铃薯和玉米病害基准数据集上分别达到99.53%和96.61%的分类准确率，比独立模型和静态集成分别提升2.1%和6.3%，推理延迟<75ms，参数量<100万。

Conclusion: DMEF在边缘设备上展示了强大的植物病害检测潜力，弥合了高精度AI与实际田间应用之间的差距，为可扩展的作物病害管理提供了可行方案。
...
### [44] [ClinNet: Evidential Ordinal Regression with Bilateral Asymmetry and Prototype Memory for Knee Osteoarthritis Grading](https://arxiv.org/abs/2601.17315)
*Xiaoyang Li,Runni Zhou*

Main category: cs.CV

TL;DR: ClinNet：基于证据序数回归的膝关节骨关节炎分级框架，通过双侧不对称编码器、诊断记忆库和证据序数头，同时估计连续KL分级和认知不确定性，实现可信赖的临床部署。


<details>
  <summary>Details</summary>
Motivation: 膝关节骨关节炎（KOA）基于X光图像的分级具有挑战性，因为级间差异细微、标注存在不确定性，且疾病进展具有固有的序数性质。传统深度学习方法通常将其视为确定性多类分类问题，忽略了退化的连续进展和专家标注的不确定性。

Method: 提出ClinNet框架，包含三个关键组件：1）双侧不对称编码器（BAE）显式建模内外侧结构差异；2）诊断记忆库维护类级原型以稳定特征表示；3）基于正态逆伽马分布的证据序数头联合估计连续KL分级和认知不确定性。

Result: 在广泛实验中，ClinNet实现了0.892的二次加权Kappa和0.768的准确率，在统计学上显著优于最先进的基线方法（p < 0.001）。模型的不确定性估计成功识别了分布外样本和潜在误诊。

Conclusion: ClinNet通过将KOA分级视为证据序数回归问题，提供连续分级和不确定性估计，为安全临床部署铺平了道路，模型的不确定性估计能够有效识别高风险病例。
...
### [45] [SkyReels-V3 Technique Report](https://arxiv.org/abs/2601.17323)
...
TL;DR: SkyReels-V3是一个基于扩散Transformer的多模态上下文学习框架的条件视频生成模型，支持图像到视频合成、视频扩展和音频引导视频生成三种核心范式。


<details>
  <summary>Details</summary>
Motivation: 视频生成是构建世界模型的基础，多模态上下文推理是衡量模型能力的关键测试。为了应对这一挑战，作者开发了SkyReels-V3，旨在通过统一的架构支持多种视频生成任务。

Method: 基于扩散Transformer的统一多模态上下文学习框架。采用三种核心范式：(1)参考图像到视频合成：通过跨帧配对、图像编辑和语义重写的数据处理流程，结合图像视频混合训练和多分辨率联合优化；(2)视频扩展：集成时空一致性建模与大规模视频理解；(3)音频引导视频生成：训练首尾帧插入模式并重构关键帧推理范式。

Result: 在视觉质量、指令跟随和特定方面指标上达到或接近最先进水平，接近领先的闭源系统性能。

Conclusion: SkyReels-V3展示了统一多模态上下文学习框架在多种视频生成任务中的有效性，为构建更强大的世界模型提供了重要基础。
...
### [46] [SymbolSight: Minimizing Inter-Symbol Interference for Reading with Prosthetic Vision](https://arxiv.org/abs/2601.17326)
*Jasmine Lesner,Michael Beyeler*

Main category: cs.CV

TL;DR: SymbolSight框架通过优化视觉符号映射来减少视网膜假体阅读中的时序干扰，使用语言特定统计和模拟视觉预测，将混淆度降低中位数22倍


<details>
  <summary>Details</summary>
Motivation: 视网膜假体空间分辨率低且存在时序持续性，导致序列字母呈现时前一个符号的残像会干扰下一个符号的识别，造成系统性识别错误。与其等待硬件改进，研究探索是否可以通过优化视觉符号本身来缓解这种时序干扰。

Method: 提出SymbolSight计算框架：1) 使用模拟假体视觉(SPV)和神经代理观察者估计符号对的混淆度；2) 基于语言特定的二元语法统计优化符号到字母的映射，选择能最小化频繁相邻字母间混淆的符号集。

Result: 在阿拉伯语、保加利亚语和英语的模拟实验中，生成的异构符号集相对于原生字母表将预测混淆度降低了中位数22倍，表明标准排版不适合序列低带宽假体视觉。

Conclusion: 计算建模可以高效缩小视觉编码的设计空间，为未来的心理物理和临床评估生成高潜力的候选方案，证明优化符号设计是改善视网膜假体阅读性能的有效途径。
...
### [47] [Learning with Geometric Priors in U-Net Variants for Polyp Segmentation](https://arxiv.org/abs/2601.17331)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CNN、Transformer和Mamba的U-Net变体在息肉分割中难以捕捉几何和结构线索，特别是在低对比度或杂乱的结肠镜场景中，需要更好的几何信息利用。

Method: 1) 在模拟的ColonDepth数据集上微调VGGT来估计息肉图像的深度图；2) 设计GPM模块处理深度图，将几何先验编码到编码器特征图中；3) 使用空间和通道注意力机制细化几何信息；4) GPM为即插即用模块，可集成到多种U-Net变体中。

Result: 在五个公共息肉分割数据集上的实验表明，GPM在三个强基线模型上均带来一致性能提升，验证了其有效性。

Conclusion: 提出的GPM模块通过注入显式几何先验，有效提升了息肉分割的准确性和鲁棒性，特别是在具有挑战性的结肠镜场景中，且具有良好的可移植性。
...
### [48] [AGE-Net: Spectral--Spatial Fusion and Anatomical Graph Reasoning with Evidential Ordinal Regression for Knee Osteoarthritis Grading](https://arxiv.org/abs/2601.17336)
*Xiaoyang Li,Runni Zhou*

Main category: cs.CV

TL;DR: AGE-Net：一种基于ConvNeXt的膝关节KL分级网络，通过光谱-空间融合、解剖图推理和差分细化，结合证据回归和序数约束，在KL分级任务上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 膝关节KL分级自动化面临挑战：结构变化细微、解剖依赖关系长距离、分级边界模糊。现有方法难以同时处理这些问题。

Method: 提出AGE-Net框架：1) 光谱-空间融合(SSF)模块；2) 解剖图推理(AGR)模块；3) 差分细化(DFR)模块。使用Normal-Inverse-Gamma证据回归头和成对序数排序约束来捕捉不确定性和保持标签序数性。

Result: 在膝关节KL数据集上，AGE-Net达到QWK 0.9017±0.0045，MSE 0.2349±0.0028，优于强CNN基线，消融研究显示各模块均有贡献。还评估了不确定性质量、鲁棒性和可解释性。

Conclusion: AGE-Net通过多模块集成有效解决了膝关节KL分级的挑战，在准确性和不确定性建模方面表现优异，为医学影像分析提供了新思路。
...
### [49] [TEXTS-Diff: TEXTS-Aware Diffusion Model for Real-World Text Image Super-Resolution](https://arxiv.org/abs/2601.17340)
*Haodong He,Xin Zhan,Yancheng Bai,Rui Lan,Lei Sun,Xiangxiang Chu*

Main category: cs.CV

TL;DR: 提出Real-Texts数据集和TEXTS-Diff模型，解决真实世界文本图像超分辨率中文本区域恢复和背景重建的问题
...
Method: 构建Real-Texts大规模高质量数据集，并提出TEXTS-Aware Diffusion Model (TEXTS-Diff)，利用抽象概念理解文本元素和具体文本区域增强文本细节

Result: 在多个评估指标上达到SOTA性能，在复杂场景中表现出优越的泛化能力和文本恢复准确性

Conclusion: 提出的数据集和模型有效解决了真实世界文本图像超分辨率问题，将公开所有代码、模型和数据集
...
### [50] [STARS: Shared-specific Translation and Alignment for missing-modality Remote Sensing Semantic Segmentation](https://arxiv.org/abs/2601.17342)
*Tong Wang,Xiaodong Zhang,Guanzhou Chen,Jiaqi Wang,Chenxi Liu,Xiaoliang Tan,Wenchao Guo,Xuyang Li,Xuanrui Wang,Zifan Wang*

Main category: cs.CV

TL;DR: STARS是一个用于处理缺失模态的遥感语义分割框架，通过非对称对齐机制和像素级语义采样对齐来解决特征崩溃和类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 多模态遥感技术通过整合光学图像、SAR和DSM等异构数据来增强地表语义理解，但在实际应用中，模态数据缺失是常见且严重的问题，会导致传统多模态融合模型性能下降。现有方法在处理缺失模态时仍面临特征崩溃和恢复特征过于泛化的限制。

Method: 提出了STARS框架，包含两个关键设计：1) 非对称对齐机制，结合双向翻译和停止梯度，有效防止特征崩溃并降低对超参数的敏感性；2) 像素级语义采样对齐策略，结合类别平衡像素采样和跨模态语义对齐损失，缓解严重类别不平衡导致的对齐失败并改善少数类识别。

Result: 论文未在摘要中提供具体实验结果，但提出的方法旨在解决现有方法的局限性，提高对缺失模态输入的鲁棒性。

Conclusion: STARS框架通过创新的对齐机制和采样策略，为不完整多模态输入的遥感语义分割提供了有效的解决方案，能够处理模态缺失问题并改善少数类识别性能。
...
### [51] [Revisiting Lightweight Low-Light Image Enhancement: From a YUV Color Space Perspective](https://arxiv.org/abs/2601.17349)
*Hailong Yan,Shice Liu,Xiangtao Zhang,Lujian Yao,Fengxiang Yang,Jinwei Chen,Bo Li*

Main category: cs.CV

TL;DR: 提出一种基于YUV颜色空间的轻量级低光图像增强方法，通过频域分析发现Y通道主要丢失低频内容而UV通道受高频噪声影响，设计针对性模块实现高质量增强且参数量少。


<details>
  <summary>Details</summary>
Motivation: 移动互联网时代需要轻量级低光图像增强技术，现有方法在视觉质量和模型紧凑性之间存在权衡。虽然近期方法使用解耦策略（如Retinex理论和YUV颜色空间变换），但忽略了通道特定的退化模式和跨通道交互，限制了性能。

Method: 通过频域分析确认YUV颜色空间在低光图像增强中的优势，发现Y通道主要丢失低频内容，UV通道受高频噪声影响。提出基于YUV的新范式：为Y通道设计双流全局-局部注意力模块，为UV通道设计Y引导的局部感知频率注意力模块，最后通过引导交互模块进行特征融合。

Result: 在多个基准测试上建立了新的最先进性能，提供卓越的视觉质量且参数量显著降低。

Conclusion: 提出的YUV-based范式通过针对不同通道的特定退化模式设计专门模块，成功解决了轻量级低光图像增强中视觉质量与模型紧凑性的权衡问题，为移动设备上的实时应用提供了有效解决方案。
...
### [52] [NeRF-MIR: Towards High-Quality Restoration of Masked Images with Neural Radiance Fields](https://arxiv.org/abs/2601.17350)
*Xianliang Huang,Zhizhou Zhong,Shuhang Chen,Yi Xu,Juhong Guan,Shuigeng Zhou*

Main category: cs.CV

TL;DR: NeRF-MIR：一种用于修复掩码图像的神经渲染方法，通过PERE策略和PIRE机制提升NeRF在图像修复中的性能


<details>
  <summary>Details</summary>
Motivation: NeRF在新视角合成方面表现出色，但在处理自然场景中常见的损坏图像（如掩码图像）时仍有很大改进空间，这些损坏会显著影响NeRF的效果

Method: 提出NeRF-MIR方法，包含三个关键技术：1) PERE策略（基于补丁的熵射线发射）来有效学习图像纹理；2) PIRE机制（渐进迭代修复）用于自训练过程中的掩码区域修复；3) 动态加权损失函数自动重新校准掩码区域的损失权重

Result: 在真实数据和构建的掩码数据集上的大量实验表明，NeRF-MIR在掩码图像修复方面优于现有方法

Conclusion: NeRF-MIR展示了NeRF在掩码图像修复领域的潜力，通过创新的射线发射策略和渐进修复机制，有效提升了损坏图像的恢复能力
...
### [53] [HyDeMiC: A Deep Learning-based Mineral Classifier using Hyperspectral Data](https://arxiv.org/abs/2601.17352)
*M. L. Mamud,Piyoosh Jaysaval,Frederick D Day-Lewis,M. K. Mudunuru*

Main category: cs.CV

TL;DR: 提出HyDeMiC（基于深度学习的矿物分类器）CNN模型，用于处理含噪声的高光谱矿物分类，在低噪声下达到完美分类，在中等噪声下保持良好性能。


<details>
  <summary>Details</summary>
Motivation: 传统高光谱矿物分类方法（如判别分析、逻辑回归、SVM）在处理环境噪声、传感器限制和高维数据时存在困难，需要更鲁棒的解决方案。

Method: 使用USGS库中115种矿物的实验室高光谱数据，通过卷积传感器响应函数生成训练集，构建CNN模型HyDeMiC，并在含1%、2%、5%、10%噪声的合成2D数据集上评估。

Result: HyDeMiC在清洁和低噪声数据集上达到近乎完美的分类准确率（MCC=1.00），在中等噪声条件下仍保持强劲性能，展示了其对噪声的鲁棒性。

Conclusion: HyDeMiC在处理含噪声高光谱数据方面表现出色，具有实际应用潜力，特别适用于真实野外条件中噪声普遍存在的场景。
...
### [54] [PocketGS: On-Device Training of 3D Gaussian Splatting for High Perceptual Modeling](https://arxiv.org/abs/2601.17354)
...
<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法依赖资源不受限的训练假设，无法在移动设备上运行，因为移动设备受限于分钟级的训练预算和硬件可用峰值内存。需要一种能在这些紧耦合约束下进行设备端训练的方法。

Method: 通过三个协同设计的算子：G算子构建几何保真的点云先验；I算子注入局部表面统计信息来初始化各向异性高斯分布，减少早期条件差距；T算子通过缓存中间结果和索引映射梯度散射来展开alpha合成，实现稳定的移动端反向传播。

Result: PocketGS能够超越强大的主流工作站3DGS基线，提供高质量重建，实现了完全设备端的实用捕获到渲染工作流程。

Conclusion: PocketGS解决了标准3DGS在移动设备上的根本矛盾，满足了训练效率、内存紧凑性和建模保真度的竞争需求，为移动端3D场景建模提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有半监督分割中的位移策略仅操作矩形区域，忽略了解剖结构，导致边界扭曲和语义不一致。医学图像分割需要保持解剖结构的语义一致性。

Method: 1. 使用超像素生成与解剖边界对齐的解剖一致区域；2. 提出不确定性指导的选择机制，选择性地位移具有挑战性的区域；3. 提出动态不确定性加权一致性损失，自适应稳定训练并正则化未标记区域。

Result: UCAD在多个实验中一致优于最先进的半监督分割方法，在有限标注下实现了优越的分割精度。

Conclusion: UCAD框架通过结合解剖感知的位移策略和不确定性指导，有效解决了半监督医学图像分割中的边界扭曲和语义不一致问题，显著提升了分割性能。
...
TL;DR: PPIA是一种针对大型视觉语言模型的黑盒物理提示注入攻击，通过在物理物体上嵌入恶意视觉指令，无需访问模型内部即可操控模型行为


<details>
  <summary>Details</summary>
Motivation: 现有提示注入攻击方法需要访问输入通道或了解用户查询，这在现实部署中很少成立。需要一种更实用的攻击方法，仅通过视觉观察就能操控LVLM

Method: 结合离线选择高识别度和语义有效的视觉提示，以及基于时空注意力的环境感知放置策略，确保注入的提示既可见又能影响模型行为

Result: 在10个最先进的LVLM上测试，包括视觉问答、规划和导航任务，攻击成功率高达98%，在不同物理条件下（距离、视角、光照）具有强鲁棒性

Conclusion: PPIA展示了LVLM在现实物理环境中面临的新型安全威胁，仅通过视觉观察就能实现黑盒、查询无关的攻击，对实际部署的智能系统构成严重风险
...
TL;DR: 提出基于扩散模型的高质量鲁棒水印框架，通过空文本优化将干净图像转换为反转噪声，经潜在空间优化后通过扩散模型迭代去噪生成水印图像，显著提升对图像损坏的鲁棒性。
...
Result: 在COCO数据集上，该方法在12种不同图像变换中平均比稳定签名方法高出10%，表现出对各种图像损坏的优越鲁棒性。

Conclusion: 提出的基于扩散模型的水印框架既能保证水印图像的视觉质量，又能增强水印对各种损坏的鲁棒性，为实际应用提供了有效解决方案。
...
TL;DR: 本文提出了一种新的事件相机动作识别框架，通过平移不变密集转换、双分支动态融合架构和生物启发的时序扭曲增强，显著提升了识别精度并降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有基于时空多视图表示学习的事件物体识别方法存在两个主要限制：1) 平移变异的空间分箱表示，2) 简单的早期拼接融合架构。这些限制影响了事件相机动作识别的性能。

Method: 1) 通过平移不变密集转换将稀疏事件转换为时空多视图表示；2) 设计双分支动态融合架构，建模不同视图间运动特征的样本级互补性；3) 引入生物启发的时序扭曲增强，模拟真实世界人类动作的速度变化。

Result: 在HARDVS、DailyDVS-200和THU-EACT-50-CHL三个数据集上，相比现有SMVRL方法分别获得+7.0%、+10.7%和+10.2%的Top-1准确率提升，同时参数减少30.1%，计算量降低35.7%。

Conclusion: 提出的框架通过改进的时空多视图表示、动态融合架构和时序增强，为事件相机动作识别建立了一个新颖且强大的范式，在精度和效率方面均显著优于现有方法。
...
TL;DR: ReLE是一个用于诊断大语言模型能力各向异性的可扩展评估系统，通过符号化混合评分机制和动态方差感知调度器，显著降低计算成本并揭示模型的专业化特征。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型评估面临基准饱和和计算成本高昂的问题，静态排行榜掩盖了模型在不同领域的能力结构性权衡，需要更高效、更精细的诊断工具。

Method: 提出ReLE系统，包含：1）符号化混合评分机制，消除推理任务中的嵌入假阳性；2）基于Neyman分配和噪声校正的动态方差感知调度器，大幅降低计算成本；3）使用领域×能力的正交矩阵评估304个模型。

Result: 系统将计算成本降低70%，同时保持排名相关性ρ=0.96；发现模型排名对权重方案高度敏感，ReLE中的排名稳定性振幅为11.4，远高于传统基准的~5.0，表明现代模型高度专业化而非普遍优越。

Conclusion: ReLE不是全面静态基准的替代品，而是作为模型演化景观的高频诊断监控工具，能够揭示模型能力各向异性和专业化特征。
...
<details>
  <summary>Details</summary>
Motivation: 精准病理学依赖检测特定感兴趣区域内的细粒度形态异常，但现有视觉-语言模型存在粒度不匹配问题，通用表示无法解析细微缺陷，且当前适应方法未能将语义提示与ROI特定视觉上下文有效结合

Method: 提出分层适应与对齐框架（HAAF），核心是跨层级缩放对齐（CLSA）机制，通过顺序校准：视觉特征先注入文本提示生成内容自适应描述符，然后空间引导视觉编码器聚焦异常；采用双分支推理策略整合语义分数与几何原型以确保少样本稳定性

Result: 在四个基准测试中，HAAF显著优于最先进方法，在低资源场景下能有效扩展至领域特定骨干网络（如CONCH）

Conclusion: HAAF通过层级对齐机制成功解决了病理图像分析中的粒度不匹配问题，为少样本精准病理诊断提供了有效解决方案
...
TL;DR: 提出一种基于邻域签名的源自由域自适应方法，通过优化目标域样本预测的相似性和相异性，使用单一损失函数实现更优的聚类效果


<details>
  <summary>Details</summary>
Motivation: 现有的源自由域自适应方法大多依赖邻域一致性概念，但容易受到误导性邻域信息的影响而产生错误。本文旨在通过更信息化的聚类和减少噪声邻域影响来改进这一方法。

Method: 提出邻域签名概念，通过优化目标域样本预测的相似性和相异性，使用单一损失函数实现自适应。该方法专注于学习更信息化的聚类并减轻噪声邻域的影响。

Result: 在具有挑战性的VisDA数据集上优于现有方法，在其他基准数据集上也取得了有竞争力的结果。

Conclusion: 通过邻域签名概念和单一损失函数优化，能够有效实现源自由域自适应，提高模型在目标域的适应性能，同时减少噪声邻域的影响。
...
TL;DR: 基于ESP32和Firebase的云物联网系统，用于环境监测和设备控制，具有高可靠性、低延迟和低成本的特点。


<details>
  <summary>Details</summary>
Motivation: 传统监控系统在实时数据访问、远程控制和云集成方面存在局限，物联网设备激增需要更先进的监控解决方案。

Method: 使用ESP32微控制器连接DHT22温湿度传感器和HC-SR04超声波距离传感器，通过Firebase实时数据库实现数据同步和远程LED控制。

Result: 数据传输成功率99.2%，实时控制延迟低于1.5秒，支持历史数据分析，总实现成本32.50美元。

Conclusion: 该系统为智能家居到工业监控等物联网应用提供了可扩展框架，Firebase集成使资源有限的开发者也能实现高级物联网应用。
...
TL;DR: CoT-Seg：无需训练的推理分割框架，结合思维链推理与自我修正，利用预训练MLLMs处理复杂查询和域外图像


<details>
  <summary>Details</summary>
Motivation: 现有推理分割方法在处理复杂查询和域外图像时表现不足，受人类解决复杂问题时需要逐步思考、查找信息、自我评估和修正的启发，需要开发更强大的推理分割系统

Method: CoT-Seg框架：1) 利用预训练MLLMs（GPT-4o）将查询分解为元指令；2) 从图像中提取细粒度语义；3) 在隐式或复杂提示下识别目标对象；4) 加入自我修正阶段：模型评估分割结果与原始查询的匹配度，识别不匹配并迭代优化掩码；5) 支持检索增强推理以获取外部知识

Result: 1) 显著提高了推理分割的可靠性和鲁棒性，特别是在模糊或易出错的情况下；2) 创建了新的挑战性数据集ReasonSeg-Hard；3) 展示了思维链推理与自我修正结合在视觉语言集成驱动分割中的强大能力

Conclusion: 思维链推理与自我修正的结合为视觉语言集成驱动的分割提供了一个强大的范式，CoT-Seg框架无需训练即可处理复杂的推理分割任务，特别是在具有挑战性的场景中表现出色
...
<details>
  <summary>Details</summary>
Motivation: X射线冠状动脉造影是评估冠状动脉疾病的临床金标准，但由于低对比度、运动、透视缩短、重叠和导管干扰等问题，常规数据中的血管分割具有挑战性，导致不同中心之间存在域偏移问题。可靠的血管分割和血管类型标记对于血管特异性分析和下游测量至关重要。

Method: 1) 从670个电影序列中选择最佳帧（基于低强度直方图标准）；2) 应用联合超分辨率和增强；3) 基准测试经典血管滤波器（Meijering, Frangi, Sato）的三种参数设置：每图像oracle调整、全局均值设置、SVR每图像参数预测；4) 神经网络基线包括U-Net、FPN和Swin Transformer，使用冠状动脉单独标注和冠状动脉+导管合并标注进行训练；5) 第二阶段分配血管身份（LAD, LCX, RCA）；6) 在公开DCA1队列上进行外部评估。

Result: SVR每图像调整相比全局均值显著提高了所有经典滤波器的Dice分数（如Frangi：0.759 vs. 0.741）。在深度学习模型中，FPN在冠状动脉单独标注下达到0.914±0.007 Dice，合并标注进一步提高到0.931±0.006。在严格的外部测试DCA1上，Dice下降到0.798（单独）和0.814（合并），但轻量域内微调可恢复到0.881±0.014和0.882±0.015。血管类型标记的准确率分别为：RCA 98.5%（Dice 0.844）、LAD 95.4%（0.786）、LCX 96.2%（0.794）。

Conclusion: 学习的每图像参数调整增强了经典管道，而高分辨率FPN模型和合并标注监督提高了稳定性和外部迁移能力，只需适度适应即可实现良好性能。该方法为X射线冠状动脉造影的定量分析提供了可靠的分割和血管识别解决方案。
...
Method: 提出ReflexSplit双流框架，包含三个关键创新：1) 跨尺度门控融合(CrGF)自适应聚合语义先验、纹理细节和解码器上下文；2) 层融合-分离块(LFSB)交替进行融合和分离操作；3) 课程训练通过深度依赖初始化和逐周期预热增强差分分离
...
TL;DR: PhaSR通过物理对齐的双层先验实现阴影去除，包括物理对齐归一化(PAN)进行光照校正和几何语义修正注意力(GSRA)进行跨模态对齐，在单光源到多光源环境下都能鲁棒工作。


<details>
  <summary>Details</summary>
Motivation: 在多样化光照条件下进行阴影去除需要将光照与内在反射率解耦，当物理先验未正确对齐时这一挑战更加复杂。传统方法在多光源环境照明下容易失效。

Method: 提出PhaSR方法，包含两个核心组件：1) 物理对齐归一化(PAN)：通过灰度世界归一化、对数域Retinex分解和动态范围重组进行闭式光照校正，抑制色偏；2) 几何语义修正注意力(GSRA)：扩展差分注意力到跨模态对齐，协调深度导出的几何信息与DINO-v2语义嵌入，解决不同光照下的模态冲突。

Result: 实验显示在阴影去除任务上具有竞争力的性能，同时复杂度更低，并能泛化到环境照明场景，而传统方法在多光源照明下会失效。

Conclusion: PhaSR通过物理对齐的双层先验实现了从单光源阴影到多光源环境照明的鲁棒阴影去除，解决了模态冲突和色偏问题，代码已开源。
...
TL;DR: BMDS-Net是一个专注于临床鲁棒性和可信度的脑肿瘤分割框架，通过零初始化多模态融合、残差门控深度解码监督和贝叶斯微调策略，在缺失模态情况下保持稳定性能并提供不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer模型（如Swin UNETR）在临床应用中存在两个关键问题：对缺失模态敏感（临床常见情况）和缺乏置信度校准。单纯追求理想数据上的高Dice分数无法满足真实医疗部署的安全要求。

Method: 1. 构建鲁棒确定性骨干网络：集成零初始化多模态上下文融合模块和残差门控深度解码监督机制，实现稳定特征学习和精确边界划分；2. 引入内存高效的贝叶斯微调策略，将网络转换为概率预测器，提供体素级不确定性地图；3. 在BraTS 2021数据集上进行全面实验验证。

Result: BMDS-Net不仅保持竞争性精度，更重要的是在基线模型失效的缺失模态场景中表现出优越的稳定性，显著降低了Hausdorff距离，并提供不确定性地图供临床医生参考。

Conclusion: BMDS-Net通过强调临床鲁棒性和可信度而非简单指标最大化，为真实世界医疗部署提供了更安全可靠的脑肿瘤分割解决方案，特别是在处理缺失模态和提供不确定性量化方面具有重要临床价值。
...
TL;DR: FMIR是一个基于基础模型的医学图像配准框架，通过基础模型特征编码器提取解剖结构，结合通用配准头和通道正则化策略，在单个数据集上训练就能实现域内SOTA性能并保持域外鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 深度学习虽然极大提升了医学图像配准速度，但其临床应用的瓶颈在于泛化能力有限，难以超越训练域，而医学数据集通常规模较小，这限制了实际应用。

Method: FMIR结合了基于基础模型的特征编码器（用于提取解剖结构）和通用配准头，采用通道正则化策略，仅需在单个数据集上进行训练。

Result: FMIR在域内配准性能达到SOTA水平，同时在域外图像上保持鲁棒的配准能力，展示了在有限资源下构建可泛化医学成像基础模型的可行路径。

Conclusion: 该研究提出了一种利用基础模型解决医学图像配准泛化问题的有效方法，为在资源有限条件下构建通用医学成像基础模型指明了方向。
...
TL;DR: 通过生成合成图像来评估视觉语言模型在特定任务上的零样本性能，相比纯文本方法能更准确预测模型效果


<details>
  <summary>Details</summary>
Motivation: 非专业用户难以评估所选视觉语言模型是否适用于他们的具体任务，现有纯文本评估方法存在局限性

Method: 在纯文本比较的基础上，生成与任务相关的合成图像来评估和优化零样本准确率的预测

Result: 使用生成图像显著提升了预测质量，并为用户提供了用于评估的图像类型反馈，在标准CLIP基准数据集上验证了有效性

Conclusion: 基于图像的评估方法能帮助用户在没有标注样本的情况下预测视觉语言模型是否适用于其应用
...
TL;DR: 提出了一种名为OTI（物体纹理强度）的模型无关、视觉可解释的图像攻击性度量方法，用于评估图像对对抗性攻击的易受攻击程度。


<details>
  <summary>Details</summary>
Motivation: 现有攻击性度量方法存在两个主要局限：1）依赖模型代理提供先验知识（如梯度或最小扰动），但实际中许多任务特定模型不易获取；2）提取的特征缺乏视觉可解释性，难以直接理解与图像的关系。

Method: 提出OTI（物体纹理强度）作为图像攻击性的度量，通过测量图像语义物体的纹理强度来评估攻击性。从决策边界以及对抗性扰动的中高频特性角度理论描述了OTI的原理。

Result: 综合实验表明OTI是有效且计算高效的。该方法为对抗性机器学习社区提供了对攻击性的视觉理解。

Conclusion: OTI是一种新颖的模型无关、视觉可解释的图像攻击性度量方法，解决了现有方法的局限性，在主动学习、对抗训练和攻击增强等应用中具有重要价值。
...
TL;DR: 利用显著性地图指导卫星图像预处理，结合传统有损压缩标准实现单张图像内的可变比特率压缩
...
Method: 使用显著性地图驱动的预处理技术，通过可变大小的平滑核映射到不同的量化显著性级别来处理图像像素，优化下游压缩编码方案

Result: 实现了在单张大型卫星图像内的可变比特率压缩，能够根据区域重要性调整压缩质量

Conclusion: 显著性地图驱动的预处理技术能够与传统有损压缩标准结合，为卫星图像压缩提供更高效的解决方案，针对不同重要性的区域实现差异化压缩
...
TL;DR: 提出Sponge Tool Attack (STA)，一种通过重写输入提示来攻击LLM工具调用过程的攻击方法，能在保持任务语义和用户意图的同时，显著增加计算开销。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM通过外部工具增强推理能力取得了高效用和高效率，但其工具调用过程对恶意操纵的脆弱性尚未充分研究。现有方法在插件式使用时存在安全漏洞。

Method: STA采用迭代式多智能体协作框架，通过明确的改写策略控制，在严格仅查询访问假设下，将原始简洁高效的推理轨迹重写为冗长复杂的版本，而不修改底层模型或外部工具。

Result: 在6个模型（开源和闭源API）、12个工具、4个智能体框架、13个数据集和5个领域的广泛实验中验证了STA的有效性，能显著增加计算开销同时保持隐蔽性。

Conclusion: STA揭示了LLM工具调用过程中的安全漏洞，为未来安全增强的智能体系统设计提供了重要启示，需要在保持功能性的同时考虑对抗性攻击的防护。
...
TL;DR: 提出Stylizing ViT，一种新颖的Vision Transformer编码器，通过权重共享的注意力块同时处理自注意力和交叉注意力，实现医学图像风格化增强，提升领域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 医学图像分析中的深度学习模型由于数据异质性和稀缺性，在跨领域和人口群体泛化方面存在困难。传统增强方法在显著领域偏移下效果有限，现有风格化增强方法存在风格多样性不足或引入伪影的问题。

Method: 提出Stylizing ViT，采用权重共享的注意力块设计，同一注意力块通过自注意力保持解剖结构一致性，同时通过交叉注意力执行风格迁移。该方法用于数据增强，在组织病理学和皮肤病学的三个图像分类任务中进行评估。

Result: 在领域泛化方面相比现有技术提升高达13%的准确率，生成感知上逼真且无伪影的图像。此外，在推理阶段使用时，通过测试时增强实现了17%的性能提升。

Conclusion: Stylizing ViT有效解决了医学图像分析中的领域泛化问题，既能生成高质量的风格化图像用于训练增强，也能在推理时提升性能，为医学图像分析提供了有效的解决方案。
...
TL;DR: SPACE-CLIP直接从冻结的CLIP视觉编码器中提取几何知识，通过双路径解码器融合语义和结构信息，在深度估计任务上显著超越现有CLIP方法


<details>
  <summary>Details</summary>
Motivation: CLIP在语义理解方面表现出色，但缺乏几何感知能力。现有方法通过文本提示查询CLIP的方式间接且低效，需要更直接的方法来提取CLIP中的几何知识

Method: 提出SPACE-CLIP架构，使用双路径解码器：语义路径通过FiLM动态调节高层特征，结构路径提取早期层的细粒度空间细节，两者分层融合

Result: 在KITTI基准测试中显著优于之前的CLIP方法，消融实验验证双路径协同融合对成功的关键作用

Conclusion: SPACE-CLIP为重用大规模视觉模型提供了高效优雅的蓝图，不仅是独立的深度估计器，更是下一代具身AI系统（如VLA模型）的可集成空间感知模块
...
<details>
  <summary>Details</summary>
Motivation: 现实世界餐食图像通常包含多种食物，但现有文本到图像扩散模型在生成多食物图像时存在物体纠缠问题（如米饭和汤融合），这影响了基于图像的饮食评估和食谱可视化等应用

Method: 提出Prompt Grafting训练免费框架，结合文本中的显式空间线索和采样过程中的隐式布局指导。采用两阶段过程：首先用布局提示建立不同区域，待布局稳定后将目标提示"嫁接"到相应区域。用户可通过编辑布局安排来控制哪些食物保持分离或有意混合

Result: 在两个食物数据集上，该方法显著提高了目标物体的存在率，并提供了可控分离的定性证据

Conclusion: Prompt Grafting框架有效解决了多食物图像生成中的物体纠缠问题，实现了对食物分离与混合的可控生成，为饮食评估和食谱可视化等应用提供了可靠的多食物数据增强方法
...
TL;DR: Uni-RS模型解决遥感多模态模型中的空间反转诅咒问题，通过空间布局规划、空间感知查询监督和图像-描述空间布局变化来提升文本到图像生成的空间忠实度。


<details>
  <summary>Details</summary>
Motivation: 现有统一的遥感多模态模型存在空间反转诅咒：虽然能准确识别和描述图像中的物体位置，但在文本到图像生成时无法忠实执行相同的空间关系，而这些空间关系是遥感图像中的核心语义信息。

Method: 1. 显式空间布局规划：将文本指令转化为空间布局计划，将几何规划与视觉合成解耦；2. 空间感知查询监督：使可学习查询偏向于指令中明确指定的空间关系；3. 图像-描述空间布局变化：让模型接触系统性的几何一致空间变换。

Result: 在多个基准测试上的广泛实验表明，该方法显著提高了文本到图像生成的空间忠实度，同时在图像描述、视觉定位和VQA等多模态理解任务上保持了强大性能。

Conclusion: Uni-RS是首个专门为遥感设计的统一多模态模型，通过显式处理理解和生成之间的空间不对称性，有效解决了空间反转诅咒问题，在生成和理解任务上都取得了优异表现。
...
TL;DR: StyleDecoupler：基于信息论的框架，利用单模态模型作为内容参考，从多模态视觉模型中分离纯风格特征，无需微调即可在冻结的视觉语言模型上实现风格检索SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 艺术风格与语义内容深度纠缠，难以有效分离。现有方法在风格表示上存在局限，需要一种能够解耦风格与内容的方法。

Method: 提出StyleDecoupler框架，利用单模态模型（抑制风格、专注内容不变特征）作为内容参考，通过互信息最小化从多模态视觉模型的嵌入中分离纯风格特征。作为即插即用模块，无需微调冻结的视觉语言模型。

Result: 在WeART（28万艺术品，152种风格，1556位艺术家）和WikiART基准测试中实现最先进的风格检索性能，支持风格关系映射和生成模型评估等应用。

Conclusion: StyleDecoupler有效解耦艺术风格与内容，为风格表示和检索提供新方法，同时发布大规模WeART数据集促进相关研究。
...
TL;DR: 提出一個自動化深度學習框架，整合AI輔助註釋、分割、分類和實例計數，用於量化時間延遲顯微鏡數據中不同密度下的紅血球群體，特別針對鐮刀型紅血球動態。
...
Result: 框架僅需少量標記數據即可達到高分割性能，能追蹤鐮刀型紅血球分數的時序演化，捕捉藥物依賴性鐮刀化行為，揭示細胞形態演化的機械生物學特徵，並可將實驗通量提高一倍以上。

Conclusion: 此AI驅動框架建立了一個可擴展且可重現的計算平台，用於研究細胞生物力學和評估微生理系統中的治療效果。
...
TL;DR: 结合3D高斯泼溅和稀疏体素光栅化的优势，提出体素初始化方法和深度几何监督，提升表面重建的几何精度和收敛速度
...
Method: 1) 提出体素初始化方法，在合理位置放置体素并设置适当细节层次，为单场景优化提供强起点；2) 提出精细化深度几何监督，将多视图线索转换为直接的每射线深度正则化，增强深度一致性而不模糊边缘

Result: 在标准基准测试中，相比先前方法在几何精度、细结构恢复和表面完整性方面均有改进，同时保持快速收敛

Conclusion: 通过结合3D高斯泼溅和稀疏体素光栅化的互补优势，实现了更准确、更完整的表面重建，同时保持了快速收敛的特性
...
TL;DR: 本文系统评估了基于隐式神经表示（INR）的任意尺度超分辨率方法，发现近期复杂方法相比早期方法仅有边际改进，性能主要受训练配置影响，并提出新损失函数提升纹理保真度。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏对INR-based任意尺度超分辨率（ASSR）方法的系统性实证研究，需要评估现有方法效果、分析训练配方（如缩放定律、目标设计、优化策略）的影响，以建立基准、识别饱和点并指导未来方向。

Method: 1）在多样化设置下比较现有技术，在多图像质量指标上汇总性能结果；2）贡献统一框架和代码库以促进可复现比较；3）研究受控训练配置对感知图像质量的影响；4）提出新损失函数，惩罚强度变化同时保留边缘、纹理和细节。

Result: 1）近期复杂INR方法相比早期方法仅有边际改进；2）模型性能与训练配置强相关，这是先前工作忽视的因素；3）提出的损失函数能跨架构提升纹理保真度；4）缩放定律适用于INR-based ASSR，模型复杂度和数据多样性的增加可带来可预测的增益。

Conclusion: INR-based ASSR领域需要更关注训练配置优化而非仅追求架构复杂性，提出的损失函数能有效提升感知质量，缩放定律的适用性为未来方法发展提供了可预测的改进路径。
...
### [81] [Flatten The Complex: Joint B-Rep Generation via Compositional $k$-Cell Particles](https://arxiv.org/abs/2601.17733)
*Junran Lu,Yuanqi Li,Hengji Li,Jie Guo,Yanwen Guo*

Main category: cs.CV

TL;DR: 提出一种将B-Rep表示为k-cell粒子集合的新范式，通过解耦刚性层次结构实现拓扑和几何的联合生成，支持无条件生成和条件任务。


<details>
  <summary>Details</summary>
Motivation: B-Rep生成建模面临挑战，因为其作为几何单元复形的异质性将拓扑与几何纠缠在一起。现有方法依赖级联序列处理层次结构，未能充分利用单元间的几何关系（如邻接和共享），限制了上下文感知和错误恢复能力。

Method: 将B-Rep重新表述为组合k-cell粒子集合，将每个拓扑实体编码为粒子组合，相邻单元在接口处共享相同潜在表示。使用多模态流匹配框架合成这些粒子集，支持无条件生成和条件任务（如单视图或点云3D重建）。

Result: 实验表明该方法能生成高保真CAD模型，在有效性和可编辑性方面优于现有方法，并能自然扩展到局部修复和直接合成非流形结构（如线框）等下游任务。

Conclusion: 通过将B-Rep表示为组合粒子集，解耦了刚性层次结构，实现了拓扑和几何的联合生成，提供了全局上下文感知，为CAD生成建模提供了新范式。
...
### [82] [The Script is All You Need: An Agentic Framework for Long-Horizon Dialogue-to-Cinematic Video Generation](https://arxiv.org/abs/2601.17737)
...
TL;DR: 提出一个端到端的智能体框架，将对话转换为电影视频，包含脚本生成、导演编排和评估三个智能体，解决了长视频叙事中的语义鸿沟问题。
...
Method: 1) 构建ScriptBench大规模多模态基准数据集；2) 训练ScripterAgent将粗略对话转换为细粒度可执行电影脚本；3) DirectorAgent使用跨场景连续生成策略编排最先进的视频模型；4) 引入CriticAgent和新的视觉-脚本对齐(VSA)指标进行评估。

Result: 框架显著提高了所有测试视频模型的脚本忠实度和时间保真度，分析揭示了当前最先进模型在视觉奇观与严格脚本遵循之间的关键权衡。

Conclusion: 该智能体框架成功弥合了对话到电影视频生成的语义鸿沟，为自动化电影制作的未来发展提供了有价值的见解。
...
### [83] [Learning Sewing Patterns via Latent Flow Matching of Implicit Fields](https://arxiv.org/abs/2601.17740)
*Cong Cao,Ren Li,Corentin Dumery,Hao Li*

Main category: cs.CV

TL;DR: 提出基于隐式表示的缝纫图案建模方法，使用符号距离场表示面板边界，无符号距离场标识缝端点，通过连续潜在空间实现可微分网格化，支持复杂结构的缝纫图案生成和从图像估计。


<details>
  <summary>Details</summary>
Motivation: 缝纫图案定义了服装的结构基础，对于时尚设计、制造和物理模拟等应用至关重要。尽管自动图案生成有所进展，但由于面板几何形状和缝线排列的广泛可变性，准确建模缝纫图案仍然很困难。

Method: 1. 使用符号距离场表示每个面板的边界，无符号距离场标识缝端点；2. 将这些场编码到连续潜在空间，实现可微分网格化；3. 潜在流匹配模型学习面板组合的分布；4. 缝线预测模块从提取的边缘段恢复缝线关系。

Result: 该方法能够准确建模和生成具有复杂结构的缝纫图案，相比现有方法在从图像估计缝纫图案方面具有更高的准确性，并支持图案补全和重新拟合等应用。

Conclusion: 该方法为数字时尚设计提供了一个实用的工具，能够有效处理缝纫图案建模中的复杂性和可变性问题，支持多种实际应用场景。
...
### [84] [Frequency-aware Neural Representation for Videos](https://arxiv.org/abs/2601.17741)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于隐式神经表示（INR）的视频压缩框架通常存在固有的频谱偏差，偏向低频分量，导致重建结果过度平滑和率失真性能不理想。

Method: 1. 多分辨率监督策略：通过分阶段监督引导网络逐步捕获全局结构和细粒度纹理；2. 动态高频注入机制：自适应强调挑战性区域以增强高频重建；3. 频率分解网络模块：改进不同频段的特征建模。

Result: 在标准基准测试中，FaNeRV显著优于最先进的INR方法，并与传统编解码器实现了具有竞争力的率失真性能。

Conclusion: FaNeRV通过频率感知的神经表示方法，有效解决了INR中的频谱偏差问题，为视频压缩提供了高效且保真的重建方案。
...
### [85] [Video Compression with Hierarchical Temporal Neural Representation](https://arxiv.org/abs/2601.17743)
*Jun Zhu,Xinfeng Zhang,Lv Tang,Junhao Jiang,Gai Zhang,Jia Wang*

Main category: cs.CV

TL;DR: TeNeRV提出了一种分层时序神经视频表示方法，通过帧间特征融合和GoP自适应调制来捕捉短期和长期时间依赖，在率失真性能上优于现有INR方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于隐式神经表示(INR)的视频压缩方法通常将时间维度作为独立输入处理，限制了捕捉复杂时间依赖的能力。需要一种能够有效建模短期和长期时间依赖的INR方法。

Method: TeNeRV包含两个关键组件：1) 帧间特征融合(IFF)模块，聚合相邻帧特征以增强局部时间一致性和捕捉细粒度运动；2) GoP自适应调制(GAM)机制，将视频划分为图像组并学习组特定先验，通过调制网络参数实现跨不同GoP的自适应表示。

Result: 大量实验表明，TeNeRV在率失真性能上持续优于现有的基于INR的方法，验证了所提方法的有效性。

Conclusion: TeNeRV通过分层时序建模成功解决了INR方法在捕捉时间依赖方面的局限性，为视频压缩提供了更有效的神经表示方法。
...
### [86] [Bridging Supervision Gaps: A Unified Framework for Remote Sensing Change Detection](https://arxiv.org/abs/2601.17747)
*Kaixuan Jiang,Chen Wu,Zhenghui Zhao,Chengxi Han*

Main category: cs.CV

TL;DR: 提出统一变化检测框架UniCD，通过耦合架构协同处理监督、弱监督和无监督任务，消除架构壁垒，在多种标注场景下均取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中像素级变化标签获取成本高，现有模型难以适应不同标注可用性的多样化场景，需要统一框架来处理不同监督级别的变化检测任务。

Method: 提出UniCD框架，包含三个监督特定分支：监督分支引入时空感知模块实现双时相特征高效融合；弱监督分支构建变化表示正则化引导模型收敛；无监督分支提出语义先验驱动变化推断，将无监督任务转化为可控的弱监督路径优化。

Result: 在主流数据集上，UniCD在三个任务上均取得最优性能，在弱监督和无监督场景下显著提升准确率，在LEVIR-CD数据集上分别超越当前SOTA方法12.72%和12.37%。

Conclusion: UniCD通过共享编码器和多分支协同学习机制，实现了异构监督信号的深度耦合，为不同标注可用性的变化检测场景提供了统一的解决方案。
...
### [87] [MV-S2V: Multi-View Subject-Consistent Video Generation](https://arxiv.org/abs/2601.17756)
*Ziyang Song,Xinyu Gong,Bangya Liu,Zelin Zhao*

Main category: cs.CV

TL;DR: 提出多视角主题到视频生成任务，通过合成数据、真实数据集和时序移位RoPE技术，实现3D级别主题一致性的高质量视频生成


<details>
  <summary>Details</summary>
Motivation: 现有S2V方法局限于单视角参考，将任务简化为S2I+I2V流水线，未能充分利用视频主题控制的潜力。需要解决多视角参考下的3D级别主题一致性挑战

Method: 1) 开发合成数据生成管道创建定制化数据，辅以小规模真实数据集；2) 提出时序移位RoPE技术，区分不同主题和同一主题的不同视角参考

Result: 框架在多视角参考图像下实现了优越的3D主题一致性和高质量视觉输出，为主题驱动视频生成开辟了新方向

Conclusion: 提出的MV-S2V任务和相应解决方案有效解决了多视角主题一致性挑战，建立了主题驱动视频生成的新范式
...
TL;DR: 提出一种基于多视角RGB图像和SAM 3D重建的牛只体重非接触式估算方法，在低数据条件下使用集成回归模型，实现农场可部署的实用解决方案。


<details>
  <summary>Details</summary>
Motivation: 传统牛只体重测量方法（如走过式称重系统或体况评分）需要人工操作，影响牲畜福利和生产效率。需要开发成本效益高、非接触式的自动化体重估算方法。

Method: 使用多视角RGB图像，通过SAM 3D基于一致性引导的融合生成单个3D点云，然后在低数据条件下比较经典集成模型与深度学习模型的性能。

Result: SAM 3D多视角一致性融合优于其他3D生成方法，经典集成模型在农场实际场景中表现最稳定（R²=0.69±0.10，MAPE=2.22±0.56%），适合农场部署。

Conclusion: 对于难以产生大量3D数据的农场场景，提高重建质量比增加模型复杂度更重要，该方法为可扩展的农场部署提供了实用解决方案。
...
TL;DR: ViTCoP：视觉与文本语义协同剪枝框架，通过视觉编码器冗余过滤和LLM层次化逐步协同剪枝，高效保留关键且信息多样的视觉token，显著降低LVLM计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型（LVLM）因视觉token冗余导致计算成本高。现有视觉token剪枝方法存在局限：要么在视觉编码器中过早丢失关键信息，要么在LLM中导致所选token间信息冗余。

Method: 提出ViTCoP框架：1）在视觉编码器中进行冗余过滤；2）基于LLM层次化特性进行逐步协同剪枝；3）引入K向量L2范数作为LLM中token显著性度量，确保与FlashAttention等加速技术兼容。

Result: 在多种大型视觉语言模型上的实验表明，ViTCoP在图像和视频理解任务上超越现有方法达到SOTA性能，同时显著降低模型推理延迟和GPU内存消耗。在极端剪枝率下性能优势更加明显。

Conclusion: ViTCoP通过视觉与文本语义协同剪枝，有效解决了LVLM中视觉token冗余问题，在保持性能的同时显著提升效率，为大型视觉语言模型的高效部署提供了有效解决方案。
...
TL;DR: 提出一种轻量级内在引导框架，利用预训练VAE特征加速扩散变换器训练，无需外部依赖，仅增加4%计算开销


<details>
  <summary>Details</summary>
Motivation: 现有的扩散变换器训练加速方法（如REPA依赖外部表示编码器，SRA需要双模型设置）都会带来沉重的计算开销。需要一种轻量级的内在引导框架来高效加速扩散训练。

Method: 利用现成的预训练变分自编码器（VAE）特征，通过轻量级投影层将扩散变换器的中间潜在特征与VAE特征对齐，使用特征对齐损失进行监督。VAE的重建特性确保了丰富的纹理细节、结构模式和基本语义信息的编码。

Result: 实验表明，该方法相比原始扩散变换器提高了生成质量和训练收敛速度，匹配或优于最先进的加速方法，仅增加4%的GFLOPs计算开销，且无需外部引导模型的额外成本。

Conclusion: 提出了一种简单有效的轻量级内在引导框架，利用预训练VAE特征加速扩散变换器训练，在保持高性能的同时显著降低了计算成本。
...
TL;DR: 提出Geometry-Grounded Gaussian Splatting，将高斯基元视为随机实体，实现高质量形状重建，解决了现有方法的多视角一致性问题


<details>
  <summary>Details</summary>
Motivation: 高斯泼溅在视角合成中表现出色，但形状提取仍存在问题。现有方法由于几何参数化和近似不足，存在多视角一致性差和对浮点噪声敏感的问题

Method: 通过理论推导将高斯基元视为特定类型的随机实体，建立几何基础的高斯泼溅框架。利用随机实体的体积特性，高效渲染高质量深度图进行细粒度几何提取

Result: 在公开数据集上，该方法在所有基于高斯泼溅的方法中取得了最佳的形状重建结果

Conclusion: 通过将高斯基元视为随机实体的理论框架，实现了几何基础的高斯泼溅，解决了形状重建中的多视角一致性问题，为高斯泼溅提供了明确的几何表示基础
...
TL;DR: SynMind：通过显式语义解析改进fMRI图像重建，利用VLMs生成文本描述指导扩散模型，在语义对齐和计算效率上超越现有方法


<details>
  <summary>Details</summary>
Motivation: 现有fMRI图像重建方法虽然视觉质量高，但存在严重语义错位问题——重要对象常被替换或幻觉生成。这是因为现有方法过度依赖纠缠的视觉嵌入，优先考虑纹理和全局特征而非显式语义身份。

Method: 1. 将fMRI信号解析为丰富的句子级语义描述，反映人类视觉理解的层次性和组合性
2. 利用接地VLMs生成类似人类的多粒度文本表示，捕捉对象身份和空间组织
3. 提出SynMind框架，整合显式语义编码与视觉先验，条件化预训练扩散模型

Result: 1. 在大多数定量指标上超越最先进方法
2. 使用更小的Stable Diffusion 1.4和单消费级GPU，超越基于SDXL的竞争方法
3. 大规模人类评估确认重建结果更符合人类视觉感知
4. 神经可视化分析显示SynMind激活更广泛、语义相关的大脑区域，减少对高级视觉区域的过度依赖

Conclusion: 通过将显式语义推理卸载到文本对齐模块，SynMind解决了fMRI图像重建中的语义错位问题，实现了更符合人类感知的重建效果，同时提高了计算效率，为神经解码提供了新思路。
...
TL;DR: 提出轻量级量子增强域泛化框架，通过多域成像模拟、域对抗训练和量子特征增强层，提升医学AI模型在未见目标域的泛化能力，无需真实多中心标注数据。


<details>
  <summary>Details</summary>
Motivation: 医学影像AI模型在单中心或单设备上表现良好，但在真实世界跨中心部署时因域偏移导致性能下降，限制了临床泛化能力。需要解决域偏移问题，提高模型在未见目标域的鲁棒性。

Method: 1) 基于MobileNetV2构建域不变编码器；2) 使用亮度、对比度、锐化和噪声扰动模拟多域成像偏移；3) 采用梯度反转的域对抗训练抑制域判别特征；4) 引入轻量级量子特征增强层，使用参数化量子电路进行非线性特征映射和纠缠建模；5) 在推理时采用测试时自适应策略缓解分布偏移。

Result: 在模拟多中心医学影像数据集上的实验表明，该方法在未见域上显著优于无域泛化或无量子增强的基线模型，降低了域特异性性能方差，提高了AUC和灵敏度。

Conclusion: 该方法展示了量子增强域泛化在有限计算资源下的临床潜力，为混合量子-经典医学影像系统提供了可行范式，能够提高医学AI模型在真实世界跨中心部署中的泛化能力。
...
TL;DR: MV-SAM：基于点图的多视图分割框架，利用视觉几何模型重建的3D点实现3D一致性，无需显式3D网络或标注3D数据


<details>
  <summary>Details</summary>
Motivation: 现有可提示分割模型（如SAM）扩展到视频和多视图图像时缺乏3D感知能力，导致结果不一致，需要昂贵的逐场景优化来保证3D一致性

Method: 利用点图（pointmaps）——从无位姿图像重建的3D点，建立像素-点的一一对应关系。将SAM的图像嵌入提升为3D点嵌入，通过transformer解码器与3D提示嵌入进行交叉注意力处理，利用3D位置嵌入隐式学习跨视图一致掩码

Result: 在SA-1B数据集上训练，在NVOS、SPIn-NeRF、ScanNet++、uCo3D、DL3DV等多个基准测试中表现优异，超越SAM2-Video，达到与逐场景优化基线相当的性能

Conclusion: MV-SAM通过点图实现2D交互与3D几何的对齐，无需显式3D网络或标注3D数据，在多视图分割中实现了3D一致性，具有良好的泛化能力
...
TL;DR: VidLaDA是一种基于扩散语言模型的视频LLM，通过双向注意力捕获双向依赖关系，解决了自回归视频LLM中的因果掩码偏差问题。配合MARS-Cache框架加速推理，实现了12倍以上的速度提升而不损失准确性。


<details>
  <summary>Details</summary>
Motivation: 标准自回归视频LLM存在因果掩码偏差，阻碍全局时空建模，导致理解效率低下。需要一种能够捕获双向依赖关系的方法来改善视频理解。

Method: 提出VidLaDA，基于扩散语言模型利用双向注意力捕获双向依赖。引入MARS-Cache框架，结合异步视觉缓存刷新和帧级分块注意力，通过锚点令牌保持全局连接性，有效剪枝冗余。

Result: VidLaDA在实验中超越了扩散基线模型，并与最先进的自回归模型（如Qwen2.5-VL和LLaVA-Video）相媲美。MARS-Cache实现了超过12倍的推理加速，且不损害推理准确性。

Conclusion: VidLaDA通过扩散语言模型和双向注意力机制有效解决了自回归视频LLM的因果掩码偏差问题，配合MARS-Cache框架实现了高效推理，为视频理解提供了新的解决方案。
...
TL;DR: Quran MD是一个全面的多模态古兰经数据集，集成了文本、语言和音频维度，涵盖经文和单词级别，包含32位不同诵经者的音频，支持NLP、语音识别、语音合成等多种应用。


<details>
  <summary>Details</summary>
Motivation: 该研究的动机是创建一个综合性的多模态古兰经数据集，以捕捉古兰经丰富的口头传统，整合文本、语言和音频信息，支持计算语言学研究和数字伊斯兰研究。

Method: 方法包括：1）在经文级别提供阿拉伯原文、英文翻译和音标转写；2）收集32位不同诵经者的音频录音；3）在单词级别对齐阿拉伯文字、英文翻译、转写和音频片段；4）创建多模态数据集结构。

Result: 创建了Quran MD数据集，包含：经文级别的文本和音频信息，单词级别的对齐数据，32位诵经者的多样化录音，支持细粒度的发音、音韵和语义分析。

Conclusion: 该数据集为古兰经研究提供了独特的资源，支持ASR、tajweed检测、TTS等多种应用，为多模态嵌入、语义检索、风格转换和个性化辅导系统奠定了基础，促进古兰经诵经和研究的计算方法发展。
...
### [97] [PEAfowl: Perception-Enhanced Multi-View Vision-Language-Action for Bimanual Manipulation](https://arxiv.org/abs/2601.17885)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言动作模型在双手操作任务中存在两个主要问题：1) 多视角特征通过视角无关的token拼接融合，导致3D空间理解能力弱；2) 语言作为全局条件注入，导致指令定位粗糙。这些问题限制了模型在遮挡、视角和场景变化下的泛化能力。

Method: 1) 空间推理：预测每个token的深度分布，进行可微3D提升，聚合局部跨视角邻居形成几何基础的跨视角一致表示；2) 指令定位：用Perceiver风格的文本感知读取机制替代全局条件注入，在冻结的CLIP视觉特征上进行迭代证据积累；3) 深度蒸馏：仅训练时使用预训练深度教师模型监督深度分布头，为感知前端提供几何感知先验。

Result: 在RoboTwin 2.0的领域随机化设置下，PEAfowl比最强基线提升了23.0个百分点的成功率。真实机器人实验进一步证明了可靠的模拟到现实迁移能力，以及深度蒸馏带来的持续改进。

Conclusion: PEAfowl通过几何感知的跨视角表示和迭代式文本感知读取机制，显著提升了双手操作策略在杂乱场景中的稳定性和泛化能力，实现了有效的模拟到现实迁移。
...
### [98] [Masked Depth Modeling for Spatial Perception](https://arxiv.org/abs/2601.17895)
*Bin Tan,Changjiang Sun,Xiage Qin,Hanat Adai,Zelin Fu,Tianxiang Zhou,Han Zhang,Yinghao Xu,Xing Zhu,Yujun Shen,Nan Xue*

Main category: cs.CV

TL;DR: LingBot-Depth是一个深度补全模型，通过掩码深度建模和视觉上下文来优化深度图，在精度和像素覆盖率上超越顶级RGB-D相机，并提供跨模态的对齐表示。


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶和机器人操作等物理世界应用中，空间视觉感知至关重要。RGB-D相机虽然能捕捉像素对齐的度量深度，但常受硬件限制和具有挑战性的成像条件（如镜面或纹理缺失表面）的影响。作者认为深度传感器的不准确性可视为反映底层几何模糊性的"掩码"信号。

Method: 提出LingBot-Depth深度补全模型，利用视觉上下文通过掩码深度建模来优化深度图，并包含一个自动数据整理流程以实现可扩展的训练。

Result: 模型在深度精度和像素覆盖率方面均优于顶级RGB-D相机。在下游任务上的实验结果表明，LingBot-Depth提供了跨RGB和深度模态的对齐潜在表示。

Conclusion: LingBot-Depth通过掩码深度建模有效解决了深度传感器的局限性，提供了高质量的深度补全解决方案，并发布了代码、检查点和3M RGB-深度对数据集（包括2M真实数据和1M模拟数据）供空间感知社区使用。
...
### [99] [Revisiting 3D Reconstruction Kernels as Low-Pass Filters](https://arxiv.org/abs/2601.17900)
*Shengjun Zhang,Min Chen,Yibo Wei,Mingyu Dong,Yueqi Duan*

Main category: cs.CV

TL;DR: 论文提出从信号处理角度重新审视3D重建，引入Jinc核作为理想低通滤波器，并通过调制核平衡空间效率和频域保真度，实现更好的渲染性能。


<details>
  <summary>Details</summary>
Motivation: 传统3D重建核（高斯、指数、学生t分布）作为低通滤波器存在非理想特性，导致高频分量与低频分量在离散信号频谱中重叠，影响重建质量。

Method: 1. 从信号处理角度分析3D重建问题，将离散采样引起的周期性频谱扩展视为根本挑战；2. 引入Jinc核作为理想低通滤波器，在截止频率处具有瞬时降为零的特性；3. 针对Jinc核在空间域衰减慢的问题，提出调制核以平衡空间效率和频域保真度。

Result: 实验结果表明，提出的Jinc核和调制核在3D重建中表现出有效性，实现了优越的渲染性能。

Conclusion: 通过信号处理视角重新审视3D重建问题，提出理想的Jinc核和改进的调制核，在频域保真度和空间效率之间取得更好平衡，为3D重建提供了新的解决方案。
...
### [100] [Feature-Space Generative Models for One-Shot Class-Incremental Learning](https://arxiv.org/abs/2601.17905)
*Jack Foster,Kirill Paramonov,Mete Ozay,Umberto Michieli*

Main category: cs.CV

TL;DR: Gen1S：一种基于残差空间生成建模的单样本类增量学习方法，通过VAE或扩散模型学习基础类残差分布作为结构先验，提升新类识别性能


<details>
  <summary>Details</summary>
Motivation: 解决单样本类增量学习（FSCIL）中，模型仅接收每个新类的单个样本且不允许进一步训练或修改的挑战性问题。传统方法在新类识别上表现不佳，需要利用基础类和新类之间的结构相似性。

Method: 提出Gen1S方法：1）将原始嵌入空间映射到残差空间（减去类别原型）；2）使用VAE或扩散模型学习基础类残差的多模态分布；3）将此分布作为结构先验，提升新类识别能力。

Result: 在多个基准测试和骨干架构上，Gen1S方法在新类识别方面持续优于现有技术，显著提升了单样本类增量学习的性能。

Conclusion: 通过利用基础类和新类嵌入之间的结构相似性，并采用生成模型学习残差分布作为先验，Gen1S为单样本类增量学习提供了一种有效的解决方案，在多个基准测试中取得了最先进的性能。
...
### [101] [Benchmarking Direct Preference Optimization for Medical Large Vision-Language Models](https://arxiv.org/abs/2601.17918)
*Dain Kim,Jiwoo Lee,Jaehoon Yun,Yong Hoe Koo,Qingyu Chen,Hyunjae Kim,Jaewoo Kang*

Main category: cs.CV

TL;DR: 本文首次全面评估了医疗领域大视觉语言模型中多种DPO变体的效果，发现现有方法在医疗场景下存在局限性，并提出针对视觉误解错误的偏好构建策略，在视觉问答任务上取得了3.6%的提升。


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型在医疗应用中有巨大潜力，但部署受到对齐不足和可靠性问题的限制。虽然DPO已成为优化模型响应的强大框架，但在高风险医疗环境中的效果尚未得到充分探索，缺乏指导未来方法发展的严格实证基础。

Method: 对医疗领域内的多种DPO变体进行首次全面评估，在LLaVA-Med和HuatuoGPT-Vision两个医疗LVLM上评估了9种不同DPO公式。基于发现的问题，提出了针对视觉误解错误的偏好构建策略。

Result: 研究揭示了当前DPO方法的几个关键局限：与监督微调相比，DPO方法往往只能带来不一致的改进，且效果在不同任务和骨干模型间差异显著；现有方法经常无法解决基本的视觉误解错误。提出的针对性偏好构建策略在视觉问答任务上比现有最强DPO基线提升了3.6%。

Conclusion: 本文填补了医疗领域DPO方法实证研究的空白，揭示了现有方法的局限性，并通过针对视觉误解的偏好构建策略展示了改进方向。为支持未来研究，作者发布了完整的框架、训练数据、模型检查点和代码库。
...
TL;DR: SC-SAM提出专家-通用框架，通过U-Net提供点提示和伪标签指导SAM适应医学图像，同时SAM作为通用监督器正则化U-Net，形成双向协同训练循环，在有限标注下实现医学图像分割SOTA。


<details>
  <summary>Details</summary>
Motivation: 基础模型如SAM在医学图像适应上面临领域偏移、标注稀缺问题，且参数高效微调无法利用未标注数据。传统U-Net在半监督医学学习中表现优异，但其辅助PEFT SAM的潜力未被充分探索。

Method: 提出SC-SAM专家-通用框架：U-Net作为专家提供点提示和伪标签指导SAM适应医学图像；SAM作为通用监督器正则化U-Net训练。两者形成双向协同训练循环，有效利用未标注数据。

Result: 在前列腺MRI和息肉分割基准测试中取得最先进结果，优于其他半监督SAM变体甚至医学基础模型如MedSAM，证明了专家-通用协作在标签高效医学图像分割中的价值。

Conclusion: 通过专家-通用协作框架，SC-SAM成功解决了SAM在医学图像适应中的挑战，展示了传统模型与基础模型协同工作的有效性，为标签稀缺的医学图像分割提供了高效解决方案。
...
TL;DR: 提出了一种用于医学图像分割的可变形转置卷积（DTC）上采样方法，通过学习动态坐标而非固定位置来生成高分辨率特征图，提升细节恢复能力。


<details>
  <summary>Details</summary>
Motivation: 传统上采样方法（如转置卷积和线性插值）使用固定位置采样，可能无法捕捉预定义采样位置之外的结构信息，导致伪影或细节丢失。受可变形卷积启发，希望开发更灵活的上采样方法。

Method: 提出可变形转置卷积（DTC），通过学习动态坐标（即采样位置）来生成高分辨率特征图，适用于2D和3D医学图像分割任务。该方法可集成到现有分割模型中。

Result: 在3D（BTCV15）和2D（ISIC18、BUSI）数据集上的实验表明，DTC能有效集成到现有医学图像分割模型中，持续提升解码器的特征重建和细节恢复能力。

Conclusion: DTC通过学习动态采样位置，克服了传统固定位置上采样方法的局限性，在医学图像分割任务中实现了更好的特征重建和细节恢复性能。
...
TL;DR: FlowMorph是一个物理一致的自监督框架，从短时明场微流控视频中学习无标记的红细胞力学代理参数k，结合层流物理和弹性松弛建模细胞变形，无需人工标注即可预测细胞力学特性。


<details>
  <summary>Details</summary>
Motivation: 红细胞力学特性是血液和系统性疾病的重要生物标志物，但现有微流控分析方法依赖监督分割或手工特征，且未编码控制红细胞形状演化的层流斯托克斯流物理原理。

Method: FlowMorph使用低维参数化轮廓建模每个细胞，通过可微分的"流中胶囊"模型结合层流平流和曲率正则化弹性松弛推进边界点，优化耦合轮廓重叠、细胞内流一致性、面积守恒、壁约束和时间平滑性的损失函数，仅使用自动提取的轮廓和光流。

Result: 在四个公开RBC微流控数据集上，FlowMorph在提供速度场的物理丰富视频中实现平均轮廓IoU为0.905，显著改善面积守恒和壁约束违反；标量参数k单独即可区分坦克履带式和翻转动力学（AUC 0.863）；仅用200个RT-DC事件校准，单调映射E=g(k)在600个保留细胞上预测表观杨氏模量的平均绝对误差为0.118 MPa。

Conclusion: FlowMorph提供了一个物理一致的自监督框架，能够从无标记微流控视频中学习红细胞力学代理参数，在通道几何、光学和帧率变化下稳健预测细胞力学特性，为高通量细胞力学分析提供了新方法。
...
TL;DR: UPLiFT是一种通用像素密集轻量级特征变换架构，采用局部注意力算子进行迭代上采样，在保持低推理成本的同时达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于交叉注意力的特征上采样方法存在效率扩展问题，而早期迭代上采样方法性能不足。需要一种既能保持高效推理又能达到先进性能的特征上采样方法。

Method: 提出UPLiFT架构，采用迭代上采样方法，并引入高效的局部注意力算子，使用完全局部定义的注意力池化公式来克服先前迭代方法的局限性。

Result: UPLiFT在保持稳定特征的同时实现了SOTA性能，推理成本低于现有像素密集特征上采样器，在生成下游任务中与最先进的耦合流匹配模型竞争。

Conclusion: UPLiFT提供了一种多功能且高效的特征上采样方法，证明迭代上采样方法仍能与基于交叉注意力的方法竞争，并在低推理成本下达到先进性能。
...
TL;DR: 提出DKGH-MoE框架，将数据驱动的MoE与领域专家引导的MoE结合，在医学影像分析中融合数据特征与临床先验知识，提升性能与可解释性。


<details>
  <summary>Details</summary>
Motivation: 医学领域MoE模型受限于小数据集，而临床实践中丰富的专家知识（如医生注视模式、诊断启发式）无法从有限数据中可靠学习。需要结合数据驱动学习与领域专家知识来获得互补优势。

Method: 提出Domain-Knowledge-Guided Hybrid MoE (DKGH-MoE)，包含两个组件：数据驱动的MoE从原始影像数据提取新特征；领域专家引导的MoE整合临床先验（特别是临床医生眼动线索）来强调高诊断相关性区域。

Result: 通过整合领域专家洞察与数据驱动特征，DKGH-MoE同时提升了模型性能和可解释性。

Conclusion: DKGH-MoE是一个即插即用且可解释的模块，统一了数据驱动学习与领域专业知识，为医学影像分析提供了更稳健和临床有意义的解决方案。
...
TL;DR: MorphXAI是一个可解释的寄生虫检测框架，将形态学监督集成到预测流程中，同时定位寄生虫并分析临床相关的形态特征。


<details>
  <summary>Details</summary>
Motivation: 寄生虫感染是全球健康挑战，特别是在资源匮乏地区，诊断依赖劳动密集型的手动血涂片检查和专家知识。现有的深度学习模型虽然性能强，但可解释性有限，无法提供临床医生依赖的形态学特征分析。

Method: 提出MorphXAI框架，将形态学监督直接集成到预测流程中，使模型能够同时定位寄生虫并表征临床相关属性（如形状、曲率、可见点计数、鞭毛存在和发育阶段）。创建了包含三种寄生虫物种（利什曼原虫、布氏锥虫和克氏锥虫）的临床医生标注数据集。

Result: 实验结果表明，MorphXAI不仅提高了检测性能（优于基线），还提供了结构化、具有生物学意义的解释。

Conclusion: MorphXAI建立了一个可解释寄生虫分析的新基准，通过统一寄生虫检测与细粒度形态学分析，增强了深度学习模型在临床环境中的实用性。
...
### [108] [Strip-Fusion: Spatiotemporal Fusion for Multispectral Pedestrian Detection](https://arxiv.org/abs/2601.18008)
*Asiegbu Miracle Kanu-Asiegbu,Nitin Jotwani,Xiaoxiao Du*

Main category: cs.CV

TL;DR: Strip-Fusion是一个用于多光谱行人检测的空间-时间融合网络，通过时间自适应卷积和KL散度损失来处理输入图像未对齐、光照变化和遮挡等挑战性问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注空间融合而忽略时间信息，且多光谱基准中的RGB和热成像图像对可能未完美对齐，加上光照变化和遮挡等挑战，使得行人检测仍然困难。

Method: 提出Strip-Fusion网络，集成时间自适应卷积动态加权时空特征，设计KL散度损失缓解可见光和热成像模态不平衡，并开发新的后处理算法减少误报。

Result: 在KAIST和CVC-14基准测试中表现优异，在严重遮挡和未对齐等挑战性条件下相比先前最先进方法有显著改进。

Conclusion: Strip-Fusion通过时空融合和模态平衡处理，有效提升了多光谱行人检测在未对齐、光照变化和遮挡等复杂条件下的性能。
...
### [109] [Leveraging Persistence Image to Enhance Robustness and Performance in Curvilinear Structure Segmentation](https://arxiv.org/abs/2601.18045)
...
TL;DR: 提出PIs-Regressor模块直接学习持久性图像(PI)表示拓扑特征，结合Topology SegNet在编码和解码阶段融合拓扑信息，无需手工设计损失函数，在医学图像曲线结构分割中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像中曲线结构分割对临床分析至关重要，整合拓扑属性（如连通性）能提高分割准确性和一致性。但现有方法主要依赖手工设计的损失函数编码拓扑信息，泛化能力差，且持久性图(PD)的非可微性和计算成本高，难以有效提取和嵌入拓扑特征。

Method: 提出PIs-Regressor模块直接从数据学习持久性图像(PI) - 拓扑特征的有限、可微表示。结合Topology SegNet在编码和解码阶段融合这些拓扑特征，将拓扑信息直接整合到网络架构中，而非辅助损失函数。该方法灵活，可与其他拓扑方法结合进一步提升性能。

Result: 在三个曲线结构基准测试上展示了最先进的性能，在像素级准确性和拓扑保真度方面均表现优异。实验结果表明整合拓扑特征增强了模型鲁棒性，能有效处理医学图像中的过度曝光和模糊等挑战。

Conclusion: 通过将拓扑信息直接整合到网络架构而非依赖手工损失函数，提出了一种更稳健的医学图像曲线结构分割方法。PIs-Regressor和Topology SegNet框架能有效学习并融合拓扑特征，在多个基准测试中达到SOTA性能，为拓扑感知分割提供了新思路。
...
### [110] [Semi-Supervised Hyperspectral Image Classification with Edge-Aware Superpixel Label Propagation and Adaptive Pseudo-Labeling](https://arxiv.org/abs/2601.18049)
*Yunfei Qiu,Qiqiong Ma,Tianhua Lv,Li Fang,Shudong Zhou,Wei Yao*

Main category: cs.CV

TL;DR: 提出一个结合空间先验信息和动态学习机制的新型半监督高光谱图像分类框架，通过边缘感知超像素标签传播和动态历史融合预测等方法，解决边界标签扩散和伪标签不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 半监督高光谱图像分类面临高标注成本和样本有限的问题，存在边界标签扩散和伪标签不稳定的挑战，需要提高分类鲁棒性和伪标签质量。

Method: 1. 边缘感知超像素标签传播模块：整合边缘强度惩罚和邻域校正策略，减轻超像素分割的标签扩散；2. 动态历史融合预测方法：维护历史预测并动态加权，平滑伪标签波动；3. 自适应三方样本分类策略：基于置信度和一致性度量，分层利用简单、模糊和困难样本；4. 动态可靠性增强伪标签框架：整合上述方法实现时空一致性优化。

Result: 在四个基准数据集上的评估表明，该框架能够保持优越的分类性能，有效解决了边界标签扩散和伪标签不稳定问题。

Conclusion: 提出的集成空间先验信息和动态学习机制的新型半监督高光谱分类框架，通过时空一致性优化，显著提升了分类鲁棒性和伪标签质量，为有限标注样本下的高光谱图像分类提供了有效解决方案。
...
### [111] [Cross-Domain Transfer with Self-Supervised Spectral-Spatial Modeling for Hyperspectral Image Classification](https://arxiv.org/abs/2601.18088)
*Jianshu Chao,Tianhua Lv,Qiqiong Ma,Yunfei Qiu,Li Fang,Huifang Shen,Wei Yao*

Main category: cs.CV

TL;DR: 提出一种无源域标注的自监督跨域迁移框架，通过S2Former模块进行光谱-空间协同建模，结合频域约束和扩散对齐微调机制，在少样本条件下实现高光谱数据的稳定跨域分类。


<details>
  <summary>Details</summary>
Motivation: 现有自监督高光谱学习方法仍依赖源域标注且易受分布偏移影响，导致目标域泛化性能下降。需要开发无需源域标签且能在目标域少样本条件下高效适应的跨域迁移方法。

Method: 1. 自监督预训练阶段：设计S2Former模块，采用双分支空间-光谱Transformer和双向交叉注意力机制实现光谱-空间协同建模；提出频域约束(FDC)通过rFFT和高频幅度损失保持频域一致性。2. 微调阶段：提出扩散对齐微调(DAFT)蒸馏机制，通过师生结构对齐语义演化轨迹，实现低标签条件下的鲁棒迁移学习。

Result: 在四个高光谱数据集上的实验结果表明，该方法具有稳定的分类性能和强大的跨域适应能力，验证了在资源受限条件下的有效性。

Conclusion: 提出的自监督跨域迁移框架能够在无需源域标注的情况下学习可迁移的光谱-空间联合表示，并通过频域约束和扩散对齐微调机制，在目标域少样本条件下实现高效适应，为资源受限的高光谱跨域应用提供了有效解决方案。
...
### [112] [Text-Pass Filter: An Efficient Scene Text Detector](https://arxiv.org/abs/2601.18098)
*Chuang Yang,Haozhao Ma,Xu Han,Yuan Yuan,Qi Wang*

Main category: cs.CV

TL;DR: TPF是一种基于特征-滤波器对的任意形状文本检测方法，通过模拟带通滤波器直接分割整个文本区域，避免了传统收缩-扩张策略的固有局限，并能自然分离粘连文本。


<details>
  <summary>Details</summary>
Motivation: 现有基于收缩-掩码扩张策略的文本检测方法在收缩操作时会丢失文本边缘的视觉特征，混淆前景背景差异，这给文本特征识别带来了固有局限。

Method: 提出文本通滤波器（TPF），为每个文本构建独特的特征-滤波器对，模拟带通滤波器原理，在推理阶段通过滤波器提取匹配文本。设计了增强集成单元（REU）增强同一文本的特征一致性，以及前景先验单元（FPU）改善前景背景区分。

Result: 实验证明了REU和FPU的有效性，并展示了TPF的优越性能。

Conclusion: TPF通过直接分割整个文本区域避免了传统方法的固有局限，能够自然分离粘连文本，无需复杂的解码或后处理过程，为实时文本检测提供了可能。
...
### [113] [Computational Framework for Estimating Relative Gaussian Blur Kernels between Image Pairs](https://arxiv.org/abs/2601.18099)
*Akbar Saadat*

Main category: cs.CV

TL;DR: 提出一种零训练前向计算框架，用于实时实现高斯模型，通过离散计算分析表达式从清晰图像生成散焦图像，并在邻域相似性度量下筛选最佳匹配。


<details>
  <summary>Details</summary>
Motivation: 为了在实时应用中实现先前验证的高斯模型，需要开发无需训练的快速计算框架来处理图像散焦问题。

Method: 基于高斯核标准差应用范围内的分析表达式离散计算，从清晰图像生成散焦图像；通过邻域相似性度量筛选多重解为单一解；处理两幅图像互为部分模糊版本的情况。

Result: 在真实图像上的实验评估显示，该框架在估计合成模糊值时的平均绝对误差低于1.7%；实际模糊图像强度与估计值之间的差异保持在2%以下。

Conclusion: 提出的零训练前向计算框架能够有效实现高斯模型在实时应用中的部署，通过分析表达式和相似性度量实现了高精度的散焦图像估计。
...
### [114] [Spatial-Conditioned Reasoning in Long-Egocentric Videos](https://arxiv.org/abs/2601.18100)
*James Tribble,Hao Wang,Si-En Hong,Chaoyi Zhou,Ashish Bastola,Siyu Huang,Abolfazl Razi*

Main category: cs.CV

TL;DR: 研究探索显式空间信号如何影响基于VLM的视频理解，通过深度图与RGB融合提升长时程自我中心视频的空间推理能力


<details>
  <summary>Details</summary>
Motivation: 长时程自我中心视频由于视角漂移和缺乏持久几何上下文，给视觉导航带来挑战。现有视觉语言模型在图像和短视频上表现良好，但在长序列中的空间推理能力有限。

Method: 1) 引入Sanpo-D数据集，对Google Sanpo进行细粒度重标注；2) 在导航导向的空间查询上基准测试多个VLM；3) 通过深度图与RGB帧融合研究输入级归纳偏置

Result: 结果显示通用精度与空间专业化之间存在权衡，深度感知和空间基础表示能提升行人检测和障碍物检测等安全关键任务的性能

Conclusion: 显式空间信号能有效增强VLM在长时程自我中心视频中的空间推理能力，特别是在安全关键应用场景中
...
### [115] [LungCRCT: Causal Representation based Lung CT Processing for Lung Cancer Treatment](https://arxiv.org/abs/2601.18118)
*Daeyoung Kim*

Main category: cs.CV

TL;DR: 提出LungCRCT框架，利用因果表示学习进行肺癌分析，实现因果干预分析并提升下游分类任务性能


<details>
  <summary>Details</summary>
Motivation: 肺癌早期症状不明显且与其他呼吸疾病症状相似，导致早期诊断困难。现有基于CNN或ViT的AI模型虽然能有效检测肺癌，但由于其相关性依赖和低可解释性，难以扩展到治疗分析或因果干预模拟。

Method: 提出LungCRCT框架，采用基于图自编码器的因果发现算法，结合距离相关性解缠和基于熵的图像重建优化，从肺癌进展的物理因果机制中提取因果表示。

Result: 框架不仅支持肺癌治疗的因果干预分析，还在恶性肿瘤分类任务中实现了93.91%的AUC分数，且下游模型极其轻量。

Conclusion: LungCRCT通过因果表示学习克服了传统深度学习的局限性，为肺癌分析提供了可解释且高效的框架，支持治疗分析和分类任务。
...
### [116] [Forward Consistency Learning with Gated Context Aggregation for Video Anomaly Detection](https://arxiv.org/abs/2601.18135)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频异常检测方法大多依赖大规模模型追求极致精度，在资源有限的边缘设备上不可行；同时主流预测方法仅使用单帧未来预测误差，忽略了更丰富的长期时序信息约束。

Method: 基于Unet架构，对连续帧进行特征提取，生成即时和前向预测；在跳跃连接中引入门控上下文聚合模块，动态融合编码器和解码器特征；采用前向一致性损失联合优化，结合即时和前向帧误差的混合异常测量策略。

Result: 在广泛实验中显著优于最先进竞争方法，运行速度高达155 FPS，在性能和效率指标间取得了优秀平衡。

Conclusion: FoGA作为轻量级视频异常检测模型，通过前向一致性学习和门控上下文聚合，实现了边缘设备上的高效部署，在保持高精度的同时大幅提升了推理速度。
...
### [117] [Agentic Very Long Video Understanding](https://arxiv.org/abs/2601.18157)
*Aniket Rege,Arka Sadhu,Yuliang Li,Kejie Li,Ramya Korlakai Vinayak,Yuning Chai,Yong Jae Lee,Hyo Jin Kim*

Main category: cs.CV

TL;DR: EGAgent是一个基于实体场景图的增强型代理框架，用于解决全天候可穿戴设备（如智能眼镜）产生的超长时程自我中心视频理解问题，通过结构化搜索和推理工具实现跨模态、时间连贯的复杂推理。


<details>
  <summary>Details</summary>
Motivation: 全天候个人AI助手需要超越短期孤立事件的上下文理解能力，能够处理连续、纵向的自我中心视频流。现有方法（如大语言模型和检索增强生成）受限于有限的上下文窗口，缺乏对超长视频流进行组合式、多跳推理的能力。

Method: 提出EGAgent框架，以实体场景图为核心，表示随时间变化的人物、地点、物体及其关系。系统为规划代理配备结构化搜索和推理工具，以及混合视觉和音频搜索能力，支持详细、跨模态和时间连贯的推理。

Result: 在EgoLifeQA和Video-MME（Long）数据集上的实验表明，该方法在EgoLifeQA上达到最先进的57.5%性能，在Video-MME（Long）上获得具有竞争力的74.1%性能，用于复杂的纵向视频理解任务。

Conclusion: EGAgent通过实体场景图和代理工具的有效结合，解决了超长时程自我中心视频理解的挑战，为全天候个人AI助手提供了强大的上下文理解能力。
...
### [118] [TempDiffReg: Temporal Diffusion Model for Non-Rigid 2D-3D Vascular Registration](https://arxiv.org/abs/2601.18168)
*Zehua Liu,Shihao Zou,Jincai Huang,Yanfang Zhang,Chao Tong,Weixin Si*

Main category: cs.CV

TL;DR: 提出了一种用于TACE手术的2D-3D血管配准方法，采用从粗到精的策略，结合结构感知PnP和时序扩散模型，显著提高了配准精度和解剖合理性。


<details>
  <summary>Details</summary>
Motivation: TACE是肝癌等肝脏恶性肿瘤的首选治疗方法，但由于术中血管导航复杂和解剖结构变异大，手术极具挑战性。准确的2D-3D血管配准对于指导微导管和器械定位、实现精确治疗靶向至关重要。

Method: 采用从粗到精的配准策略：1) 全局对齐模块SA-PnP，建立2D和3D血管结构的对应关系；2) TempDiffReg时序扩散模型，利用时序上下文迭代执行血管变形，捕捉复杂的解剖变异和局部结构变化。

Result: 在23名患者的626个多帧样本数据集上评估，方法在准确性和解剖合理性方面均优于现有方法。具体指标：MSE 0.63mm，MAE 0.51mm，相比最优现有方法MSE降低66.7%，MAE降低17.7%。

Conclusion: 该方法能够帮助经验不足的临床医生安全高效地执行复杂TACE手术，有望提升手术效果和患者护理质量。代码和数据已开源。
...
### [119] [YOLO-DS: Fine-Grained Feature Decoupling via Dual-Statistic Synergy Operator for Object Detection](https://arxiv.org/abs/2601.18172)
*Lin Huang,Yujuan Tan,Weisheng Li,Shitai Shan,Liu Liu,Bo Liu,Linlin Shen,Jing Yu,Yue Niu*

Main category: cs.CV

TL;DR: YOLO-DS通过引入双统计协同算子(DSO)和门控模块，解决了YOLO系列检测器中异质对象响应建模不足的问题，在MS-COCO基准上显著提升了检测精度，同时保持高效推理。


<details>
  <summary>Details</summary>
Motivation: 现有YOLO检测器缺乏对共享特征通道中异质对象响应的显式建模，这限制了性能的进一步提升。需要一种能够有效区分不同类型对象的机制来突破这一限制。

Method: 提出YOLO-DS框架，核心是双统计协同算子(DSO)，通过联合建模通道级均值和峰均差来解耦对象特征。基于DSO设计了两个轻量级门控模块：用于自适应通道特征选择的双统计协同门控(DSG)模块，以及用于深度特征加权的多路径分段门控(MSG)模块。

Result: 在MS-COCO基准上，YOLO-DS在五个模型尺度(N, S, M, L, X)上均优于YOLOv8，AP提升1.1%到1.7%，推理延迟仅轻微增加。可视化、消融和对比研究验证了方法的有效性。

Conclusion: YOLO-DS通过显式建模异质对象响应，显著提升了一阶段目标检测器的性能，在保持高效率的同时实现了更好的对象区分能力，为YOLO系列检测器提供了有效的改进方案。
...
### [120] [\textsc{NaVIDA}: Vision-Language Navigation with Inverse Dynamics Augmentation](https://arxiv.org/abs/2601.18188)
*Weiye Zhu,Zekai Zhang,Xiangchen Wang,Hewei Pan,Teng Wang,Tiantian Geng,Rongtao Xu,Feng Zheng*

Main category: cs.CV

TL;DR: NaVIDA是一个新的视觉语言导航框架，通过逆动力学增强学习视觉-动作因果关系，使用分层概率动作分块扩展规划范围，并采用熵引导机制自适应执行，在减少参数量的情况下实现更优性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLN方法主要依赖反应式的状态-动作映射，没有显式建模动作如何因果地改变后续视觉观察。缺乏这种视觉-动作因果关系导致代理无法预测自身动作引起的视觉变化，造成行为不稳定、泛化能力弱和轨迹累积误差。

Method: NaVIDA框架结合策略学习与动作基础的视觉动态和自适应执行。1) 使用基于分块的逆动力学监督增强训练，学习视觉变化与对应动作的因果关系；2) 采用分层概率动作分块组织轨迹为多步分块，提供区分性更强的长程视觉变化线索；3) 引入熵引导机制自适应设置动作分块的执行范围以控制误差累积。

Result: 实验表明NaVIDA在导航性能上优于最先进方法，且参数量更少（3B vs 8B）。真实机器人评估进一步验证了方法的实际可行性和有效性。

Conclusion: 通过显式建模视觉-动作因果关系并采用分层动作分块和自适应执行机制，NaVIDA解决了VLN中的行为不稳定和误差累积问题，在减少参数量的同时实现了更好的导航性能。
...
### [121] [Multi-Perspective Subimage CLIP with Keyword Guidance for Remote Sensing Image-Text Retrieval](https://arxiv.org/abs/2601.18190)
*Yifan Li,Shiying Wang,Jianqiang Huang*

Main category: cs.CV

TL;DR: MPS-CLIP是一个参数高效的遥感图像-文本检索框架，通过关键词引导的细粒度对齐替代传统的全局匹配，使用LLM提取关键词、SAM生成语义子视角，结合G^2A适配器和MPR模块，在RSICD和RSITMD基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLP模型（如CLIP）主要依赖粗粒度全局对齐，忽略了遥感图像密集、多尺度的语义特性。同时，完全微调这些大型模型计算成本高昂且存在灾难性遗忘风险。

Method: 1. 使用LLM提取核心语义关键词，指导SAM生成语义相关的子视角；2. 引入G^2A适配器高效适应冻结骨干网络；3. 通过MPR模块聚合局部线索为多视角嵌入；4. 采用多视角对比和加权三元组损失的混合目标函数，动态选择最大响应视角。

Result: 在RSICD和RSITMD基准测试中，MPS-CLIP分别达到35.18%和48.40%的平均召回率（mR），显著优于完全微调基线和近期竞争方法。

Conclusion: MPS-CLIP成功将遥感图像-文本检索从全局匹配转向关键词引导的细粒度对齐，以参数高效的方式实现了最先进的性能，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉质量评估方法依赖监督微调或强化学习，需要大量人工标注且容易产生数据集特定偏差。需要一种无需训练的方法来利用大型多模态模型的潜在感知知识进行细粒度质量理解。

Method: 提出QualiRAG框架：1）将问题分解为结构化请求；2）动态生成四种互补知识源（视觉元数据、主体定位、全局质量摘要、局部质量描述）；3）进行相关性感知检索；4）基于证据的推理。

Result: 在视觉质量理解任务上显著优于开源通用LMM和VQA微调LMM，在视觉质量比较任务上表现有竞争力，无需任何任务特定训练即可实现稳健的质量评估能力。

Conclusion: QualiRAG通过训练免费的RAG框架，成功利用LMM的潜在感知知识进行视觉质量评估，为细粒度质量理解提供了有效解决方案，避免了传统方法的标注成本和数据集偏差问题。
...
TL;DR: HomoFM首次将流匹配技术引入单应性估计，通过建模连续点状速度场将噪声分布转换为配准坐标，并集成梯度反转层实现领域自适应，在精度和鲁棒性上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度单应性估计方法通常将其视为直接回归或迭代优化问题，难以捕捉复杂几何变换或在不同领域间泛化。需要一种更有效的方法来处理复杂变换和领域偏移问题。

Method: 提出HomoFM框架，首次将生成建模中的流匹配技术引入单应性估计。将单应性估计问题重新定义为速度场学习问题，建模连续点状速度场将噪声分布转换为配准坐标。同时集成梯度反转层到特征提取主干，实现领域自适应，学习领域不变表示。

Result: 在标准基准测试中，HomoFM在估计精度和鲁棒性方面均优于现有最先进方法，特别是在多模态匹配和变化光照场景等跨领域情况下表现优异。

Conclusion: 流匹配技术能有效解决单应性估计中的复杂几何变换和领域泛化问题，结合领域自适应策略可显著提升网络鲁棒性，为计算机视觉和机器人应用提供了新的解决方案。
...
TL;DR: 提出基于EfficientNetB2的轻量级面部情绪识别管道，采用两阶段训练策略，在FER-2013数据集上达到68.78%测试准确率，参数量比VGG16少近10倍。


<details>
  <summary>Details</summary>
Motivation: 解决真实场景面部情绪识别的挑战：低图像质量、光照变化、姿态变化、背景干扰、类间差异小、标注噪声和严重类别不平衡。现有大型CNN模型计算昂贵、内存密集，不适合实时应用。

Method: 使用EfficientNetB2构建轻量级管道，采用两阶段预热和微调策略。结合AdamW优化、解耦权重衰减、标签平滑（ε=0.06）、裁剪类别权重缓解类别不平衡，以及dropout、混合精度训练和实时数据增强。

Result: 在FER-2013数据集上达到68.78%测试准确率，参数量比VGG16基准少近10倍。实验结果显示稳定训练和强泛化能力，适合实时和边缘应用。

Conclusion: 提出的轻量级EfficientNetB2管道在保持高准确率的同时显著减少计算资源需求，解决了真实场景面部情绪识别的关键挑战，适合实时和边缘部署。
...
TL;DR: 提出V-Loop框架，通过视觉逻辑循环验证来检测医疗VQA中的幻觉问题，无需训练即插即用，优于现有方法


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在医疗视觉问答中表现出色，但其输出容易产生与视觉事实矛盾的幻觉，在医疗高风险场景中存在重大风险。现有基于不确定性的内省检测方法虽然计算高效，但本质上是间接的，因为它们估计图像-问题对的预测不确定性，而非验证特定答案的事实正确性。

Method: 提出视觉逻辑循环验证(V-Loop)框架：1) MLLM生成主要答案；2) 从主要QA对中提取语义单元；3) 基于答案单元生成验证问题来重新查询问题单元；4) 强制执行视觉注意力一致性，确保主要问题和验证问题都依赖相同的图像证据；5) 如果验证答案与预期语义内容匹配，则逻辑循环闭合，表明事实基础牢固，否则标记为幻觉。

Result: 在多个医疗VQA基准测试和MLLM上的广泛实验表明，V-Loop始终优于现有的内省方法，保持高效性，并且与基于不确定性的方法结合使用时能进一步提升性能。

Conclusion: V-Loop通过双向推理过程形成视觉基础逻辑循环来验证事实正确性，为医疗VQA中的幻觉检测提供了有效的训练免费即插即用解决方案，显著提高了检测准确性。
...
TL;DR: 提出VLM引导的框架，加速并稳定多材料参数估计，在6G系统中实现快速可靠的RF材料参数估计


<details>
  <summary>Details</summary>
Motivation: 6G系统中精确的RF材料参数对电磁数字孪生至关重要，但基于梯度的逆射线追踪方法对初始化敏感且在有限测量下成本高昂

Method: 使用视觉语言模型解析场景图像推断材料类别，通过ITU-R材料表映射到定量先验，提供有信息的电导率初始化；VLM进一步选择信息丰富的发射器/接收器位置；基于这些先验，可微射线追踪引擎执行梯度优化

Result: 在NVIDIA Sionna室内场景实验中，相比均匀或随机初始化及随机放置基线，收敛速度提高2-4倍，最终参数误差降低10-100倍，仅用少量接收器即可实现低于0.1%的平均相对误差

Conclusion: VLM提供的语义先验能有效指导基于物理的优化，实现快速可靠的RF材料参数估计
...
TL;DR: OrthoFoundation是一个针对肌肉骨骼病理学的多模态视觉基础模型，使用120万未标记的膝部X光和MRI图像进行自监督对比学习预训练，在14个下游任务中达到SOTA性能，并展现出卓越的标签效率和跨解剖结构泛化能力。


<details>
  <summary>Details</summary>
Motivation: 肌肉骨骼疾病是全球残疾的主要原因，需要精确的医学影像解读。当前骨科AI方法依赖任务特定的监督学习，存在碎片化、需要大量标注数据、缺乏跨模态和临床场景泛化能力的问题。该领域基础模型发展受限于大规模、高质量开源数据集的稀缺。

Method: 构建包含120万未标记膝部X光和MRI图像的预训练数据集，使用Dinov3骨干网络，通过自监督对比学习方法训练，学习稳健的放射学表示。

Result: 在14个下游任务中达到SOTA性能：X光骨关节炎诊断准确率优异，MRI结构损伤检测排名第一；仅需50%标注数据即可匹配监督基线性能；尽管在膝部图像上预训练，但能出色泛化到髋、肩、踝等其他解剖结构。
...
TL;DR: Co-PLNet：一种点线协同框架，通过空间提示编码和交叉引导解码，实现线框解析中线段和连接点的联合优化，提升准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有线框解析方法分别预测线段和连接点，然后进行后处理融合，导致不匹配和鲁棒性降低。需要一种协同框架来同时处理这两个任务。

Method: 提出Co-PLNet点线协同框架：1) Point-Line Prompt Encoder将早期检测转换为空间提示，编码几何属性为紧凑的空间对齐图；2) Cross-Guidance Line Decoder使用稀疏注意力机制，基于互补提示细化预测，强制点线一致性。

Result: 在Wireframe和YorkUrban数据集上的实验显示，该方法在准确性和鲁棒性方面均有持续改进，同时具有有利的实时效率。

Conclusion: Co-PLNet通过点线任务间的空间线索交换，实现了结构化几何感知的有效性，解决了现有方法中的不匹配问题。
...
<details>
  <summary>Details</summary>
Motivation: 自动化患者定位在优化扫描流程和提高患者吞吐量方面很重要。利用RGB-D相机捕获的深度信息来估计内部器官位置，可以实现更准确高效的定位。

Method: 使用大规模全身MRI扫描数据集，合成深度图像与对应解剖分割配对，训练统一的卷积神经网络架构，直接从2D深度图像预测多个内部器官的3D位置和形状。

Result: 方法能够准确定位包括骨骼和软组织在内的多种解剖结构，无需显式表面重建。实验结果表明深度传感器集成到放射学工作流程中的潜力。

Conclusion: 该学习框架展示了通过深度传感器实现自动化患者定位的可行性，可以简化扫描流程并改善患者体验。
...
TL;DR: 本文综述了航拍图像分类的机器学习方法，并提出了Aerial-Y-Net模型，在AID数据集上达到91.72%准确率


<details>
  <summary>Details</summary>
Motivation: 航拍图像在城市规划和环境保护中至关重要，但由于其异质性（包含建筑物、森林、山脉、空地等多种结构），开发鲁棒的场景分类模型仍然具有挑战性

Method: 1. 文献综述：涵盖从手工特征（SIFT、LBP）到传统CNN（VGG、GoogLeNet）再到先进深度混合网络的方法
2. 提出Aerial-Y-Net：空间注意力增强的CNN，具有多尺度特征融合机制，作为基于注意力的模型帮助理解航拍图像的复杂性

Result: 在AID数据集上评估，Aerial-Y-Net模型达到91.72%的准确率，优于多个基线架构

Conclusion: 本文通过综述航拍图像分类方法并提出的Aerial-Y-Net模型，为解决航拍图像异质性带来的分类挑战提供了有效方案，展示了注意力机制和多尺度特征融合在提升分类性能方面的重要性
...
TL;DR: 提出两种改进LiDAR点云到距离图像投影的方法：基于实例中心的投影（CAP）和基于类别权重的投影（CWAP），解决传统深度优先选择策略导致的信息丢失问题


<details>
  <summary>Details</summary>
Motivation: 传统LiDAR点云到距离图像的投影采用深度优先选择策略（保留最近点），忽略了语义相关性和物体结构信息，导致重要上下文信息丢失

Method: 1. Centerness-Aware Projection (CAP)：根据点到实例中心的距离调整深度值，优先保留实例中心点而非噪声边界点
2. Class-Weighted-Aware Projection (CWAP)：通过用户定义权重对不同类别进行优先级排序，提供灵活的投影策略

Result: 在SemanticKITTI数据集上评估，CAP在投影过程中保留了更多实例点，相比基线方法提升3.1% mIoU；CWAP能有效提升目标类别性能，对其他类别影响可忽略

Conclusion: 通过结合实例中心和类别信息的上下文感知投影方法，能有效解决LiDAR点云投影中的多对一冲突问题，提升语义分割性能
...
TL;DR: 提出了SwipeGen自动化管道合成类人滑动交互，构建了首个GUI代理滑动执行能力基准，并开发了GUISwiper代理，在滑动执行准确率上相比现有VLM基线提升214%。


<details>
  <summary>Details</summary>
Motivation: 随着GUI代理的广泛应用，其任务执行能力逐渐成为任务完成的新瓶颈。现有GUI代理在处理滑动交互时采用过于简化的策略，无法准确复制类人行为。

Method: 将人类滑动手势分解为多个可量化维度，提出SwipeGen自动化管道通过GUI探索合成类人滑动交互。基于此管道构建首个GUI代理滑动执行能力基准，并利用合成数据开发GUISwiper代理。

Result: GUISwiper实现了69.07%的滑动执行准确率，相比现有VLM基线提升214%。

Conclusion: 通过量化分析人类滑动手势并合成训练数据，显著提升了GUI代理的交互执行能力，为解决GUI代理执行瓶颈提供了有效方案。
...
TL;DR: 该研究提出了一种高效的密集Swin混合（EDSH）框架用于脑肿瘤MRI分析，通过两个肿瘤感知实验设置来联合捕获细粒度纹理模式和长距离上下文依赖关系。


<details>
  <summary>Details</summary>
Motivation: 脑肿瘤MRI分析需要同时捕捉细粒度纹理模式和长距离上下文依赖关系，现有方法难以有效处理不同肿瘤类型的特异性特征（如弥漫性胶质瘤的不规则形状、边界不清和异质性纹理，以及脑膜瘤和垂体瘤的明确肿块、特定位置和肿瘤扩展特征）。
...
Result: 在大规模MRI数据集（40,260张图像，涵盖四种肿瘤类别）上进行广泛评估，相比独立CNN、Vision Transformers和混合方法表现出持续优越性，在未见测试数据集上达到98.50%的准确率和召回率。

Conclusion: EDSH框架通过两个肿瘤感知设置有效解决了脑肿瘤MRI分析中的特定诊断挑战，能够联合捕获局部纹理和全局上下文信息，显著提高了脑肿瘤分类的准确性和鲁棒性。
...
### [134] [PPISP: Physically-Plausible Compensation and Control of Photometric Variations in Radiance Field Reconstruction](https://arxiv.org/abs/2601.18336)
*Isaac Deutsch,Nicolas Moënne-Loccoz,Gavriel State,Zan Gojcic*

Main category: cs.CV

TL;DR: 提出PPISP模块，通过物理可解释的ISP校正解决多视角3D重建中的光度不一致问题，在标准基准测试中达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有方法对相机光学特性和图像信号处理（ISP）变化导致的光度不一致问题敏感，现有缓解策略（如逐帧潜在变量或仿射颜色校正）缺乏物理基础且对新视角泛化能力差

Method: 提出物理可解释的ISP（PPISP）校正模块，通过基于物理的可解释变换解耦相机固有特性和拍摄依赖效应；训练专门的PPISP控制器预测新视角的ISP参数，类似于真实相机中的自动曝光和自动白平衡

Result: 在标准基准测试中达到最先进（SOTA）性能，提供直观控制，支持集成可用元数据，无需真实图像即可实现新视角的真实公平评估

Conclusion: PPISP模块通过物理可解释的ISP校正有效解决了多视角3D重建中的光度不一致问题，实现了对新视角的泛化能力，为相关任务提供了更可靠的评估框架
...
### [135] [Beyond Rigid: Benchmarking Non-Rigid Video Editing](https://arxiv.org/abs/2601.18340)
*Bingzheng Qu,Kehai Chen,Xuefeng Bai,Jun Yu,Min Zhang*

Main category: cs.CV

TL;DR: NRVBench是首个专门评估非刚性视频编辑的基准，包含高质量数据集、新评估指标NRVE-Acc和训练免费基线方法VM-Edit，旨在解决现有方法在物理合理性和时间一致性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 尽管文本驱动视频编辑取得了显著进展，但生成连贯的非刚性变形仍然是一个关键挑战，常常受到物理失真和时间闪烁的困扰。需要专门的基准来评估非刚性视频编辑的质量。

Method: 1) 构建包含180个非刚性运动视频的数据集，涵盖6个基于物理的类别，配备2,340个细粒度任务指令和360个多项选择题；2) 提出基于视觉语言模型的新评估指标NRVE-Acc，评估物理合规性、时间一致性和指令对齐；3) 提出训练免费的基线方法VM-Edit，采用双区域去噪机制实现结构感知控制。

Result: 实验表明，现有方法在保持物理合理性方面存在不足，而提出的VM-Edit方法在标准和提出的指标上都取得了优异性能。

Conclusion: NRVBench可作为推进物理感知视频编辑的标准测试平台，为评估和改进非刚性视频编辑方法提供重要基础。
...
### [136] [Q-Bench-Portrait: Benchmarking Multimodal Large Language Models on Portrait Image Quality Perception](https://arxiv.org/abs/2601.18346)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在通用图像的低级视觉基准上表现良好，但针对具有独特结构和感知特性的人像图像的感知和评估能力尚未充分探索，需要专门的基准来评估和推动这一领域的发展。

Method: 构建Q-Bench-Portrait基准，包含：(1)多样的人像图像来源（自然、合成失真、AI生成、艺术、计算机图形图像）；(2)全面的质量维度（技术失真、AIGC特定失真、美学）；(3)多种问题格式（单选、多选、判断、开放式问题），涵盖全局和局部层面。

Result: 评估了20个开源和5个闭源MLLMs，发现当前模型在人像图像感知方面有一定能力，但表现仍然有限且不精确，与人类判断存在明显差距。

Conclusion: Q-Bench-Portrait基准将促进通用和领域特定MLLMs在人像图像感知能力方面的进一步研究，填补了现有基准在人像质量评估方面的空白。
...
### [137] [OREHAS: A fully automated deep-learning pipeline for volumetric endolymphatic hydrops quantification in MRI](https://arxiv.org/abs/2601.18368)
...
TL;DR: OREHAS是首个全自动管道，用于从常规3D MRI中量化内淋巴积水，通过深度学习分割和临床对齐的工作流程，仅需少量标注即可实现可靠的容积测量。


<details>
  <summary>Details</summary>
Motivation: 目前内淋巴积水的量化需要手动干预，存在操作者依赖性和方法不一致的问题。需要开发自动化的解决方案来提供可靠、可重复的容积测量，支持大规模研究和临床诊断阈值校准。

Method: OREHAS集成三个组件：切片分类、内耳定位和序列特异性分割，形成单一工作流程。仅需每患者3-6个标注切片进行训练，就能从完整MRI体积中自动计算每耳的内淋巴-前庭容积比。

Result: 在外部验证队列中，OREHAS Dice分数达到0.90（SPACE-MRC）和0.75（REAL-IR），与专家标注高度一致（VSI=74.3%），显著优于临床软件syngo.via（VSI=42.5%）。OREHAS测量的内淋巴容积更小且更符合生理现实。

Conclusion: OREHAS证明通过有限监督可以从标准MRI实现可靠的内淋巴积水量化。该管道减少了操作者依赖性，确保了方法一致性，为大规模研究和临床诊断阈值校准提供了坚实基础。
...
### [138] [Gaze Prediction in Virtual Reality Without Eye Tracking Using Visual and Head Motion Cues](https://arxiv.org/abs/2601.18372)
*Christos Petrou,Harris Partaourides,Athanasios Balomenos,Yannis Kopsinis,Sotirios Chatzis*

Main category: cs.CV

TL;DR: 提出结合头戴显示器运动信号和视觉显著性线索的VR注视预测框架，在无法直接眼动追踪时减少感知延迟


<details>
  <summary>Details</summary>
Motivation: VR应用中注视预测对减少传感器延迟和实现注视点渲染等技术至关重要，但直接眼动追踪常因硬件限制或隐私问题不可用

Method: 使用UniSal轻量级显著性编码器提取视频帧视觉特征，融合HMD运动数据，通过时间序列预测模块（TSMixer和LSTM）预测未来注视方向

Result: 在EHTask数据集和商用VR硬件上评估，该方法持续优于Center-of-HMD和Mean Gaze基线，有效减少感知延迟

Conclusion: 在直接眼动追踪受限的VR环境中，预测性注视建模能有效减少感知延迟并增强自然交互
...
### [139] [Estimation of geometric transformation matrices using grid-shaped pilot signals](https://arxiv.org/abs/2601.18385)
*Rinka Kawano,Masaki Kawamura*

Main category: cs.CV

TL;DR: 提出一种使用网格状导频信号进行几何变换估计的数字水印方法，能够有效应对裁剪等几何攻击，实现准确同步。


<details>
  <summary>Details</summary>
Motivation: 现有数字水印方法对裁剪攻击的鲁棒性不足，而裁剪会改变图像原点，使得水印同步变得困难。需要一种能够准确估计几何变换以实现同步的方法。

Method: 嵌入网格状导频信号，水平和垂直线采用不同编码。当图像发生几何变换时，网格也会相应变形。通过分析变形网格（使用Radon变换估计网格角度和间隔），可以估计变换矩阵。不同编码的网格线还能确定网格方向，减少歧义。

Result: 仿真实验表明，该方法在单次攻击和复合攻击下都能准确估计变换矩阵，误差较低。测试包括各向异性缩放、旋转、剪切和裁剪等几何变换。

Conclusion: 提出的基于导频信号的数字水印方法能够有效应对裁剪等几何攻击，实现准确的水印同步，解决了现有方法在裁剪鲁棒性方面的不足。
...
### [140] [ARMOR: Agentic Reasoning for Methods Orchestration and Reparameterization for Robust Adversarial Attacks](https://arxiv.org/abs/2601.18386)
*Gabriel Lee Jun Rong,Christos Korgialas,Dion Jia Xu Ho,Pai Chet Ng,Xiaoxiao Miao,Konstantinos N. Plataniotis*

Main category: cs.CV

TL;DR: ARMOR框架使用VLM和LLM智能协调CW、JSMA、STA三种对抗攻击方法，通过实时自适应调参和语义感知提升攻击效果


<details>
  <summary>Details</summary>
Motivation: 现有自动化攻击套件采用静态固定序列，缺乏战略适应性和语义感知能力，无法根据目标特性动态调整攻击策略

Method: 使用VLM引导的智能体协同生成扰动，通过共享"Mixing Desk"合成攻击，LLM实时自适应调参和重新参数化并行攻击智能体，利用图像特定语义漏洞

Result: 在标准基准测试中，ARMOR实现了改进的跨架构迁移性，可靠地欺骗黑白盒两种设置，通过置信度和SSIM评分选择最佳攻击或混合攻击

Conclusion: ARMOR框架通过智能体协同和语义感知，显著提升了对抗攻击的适应性和效果，为自动化攻击提供了新的范式
...
### [141] [Efficient Complex-Valued Vision Transformers for MRI Classification Directly from k-Space](https://arxiv.org/abs/2601.18392)
...
TL;DR: 提出kViT：直接在k空间数据上进行分类的复数视觉Transformer，通过径向k空间分块策略解决传统架构与MRI物理的不匹配问题，在保持分类性能的同时大幅提升计算效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习在MRI中的应用主要基于重建后的幅度图像，这丢弃了相位信息且需要计算昂贵的变换。标准神经网络架构依赖于局部操作（卷积或网格分块），不适合k空间数据的全局、非局部特性。

Method: 提出复数视觉Transformer（kViT），直接在k空间数据上进行分类。引入径向k空间分块策略，尊重频域的能量分布特性，解决当前架构与MRI物理之间的几何不匹配问题。

Result: 在fastMRI和内部数据集上的实验表明，kViT的分类性能与最先进的图像域基线（ResNet、EfficientNet、ViT）相当。更重要的是，kViT对高加速因子表现出更强的鲁棒性，训练时的VRAM消耗相比标准方法减少高达68倍。

Conclusion: kViT为资源高效、直接从扫描仪进行AI分析建立了新途径，实现了计算效率的范式转变，同时保持分类性能并提升对加速采样的鲁棒性。
...
TL;DR: 针对PB级大规模图像数据的内存外分析，提出基于流式处理的架构和领域特定语言，通过优化I/O访问模式实现高效处理


<details>
  <summary>Details</summary>
Motivation: 处理PB级电子显微镜数据和TB级人体器官图谱等超大规模图像时，性能受限于I/O瓶颈，需要设计能够最小化磁盘访问的高效分析方法

Method: 提出基于流式处理的架构，支持切片和分块两种数据表示；引入领域特定语言(DSL)，通过编译时和运行时分析自动优化流水线，包括窗口大小选择、阶段融合、流操作和调度

Result: 实现了近线性的I/O扫描和可预测的内存占用，显著提升了超大规模图像处理的吞吐量，无需将整个数据集加载到内存中

Conclusion: 流式处理架构结合领域特定语言能够有效解决大规模图像分析的I/O瓶颈问题，为PB级数据集的处理提供了高效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 自闭症谱系障碍（ASD）儿童在情感表达和沟通方面存在困难，传统评估方法通常具有侵入性、主观性或难以一致应用。早期识别儿童情感状态具有挑战性，需要更客观、非侵入性的评估工具。

Method: 采用迁移学习方法，在由心理学专家标注情感标签的儿童绘画数据集上训练三种深度学习架构：MobileNet、EfficientNet和VGG16。在统一的实验框架下评估这些模型的分类性能、鲁棒性和计算效率。

Result: 研究结果揭示了轻量级架构与深层架构在基于绘画的情感计算任务中的重要权衡，特别是在移动和实时应用场景中。不同模型在准确性、计算效率和适用性方面表现出不同特点。

Conclusion: 深度学习模型可用于儿童绘画情感识别，为ASD儿童情感状态评估提供非侵入性工具。模型选择需根据具体应用场景（如移动端部署、实时性要求）在性能与效率之间进行权衡。
...
### [144] [On Procrustes Contamination in Machine Learning Applications of Geometric Morphometrics](https://arxiv.org/abs/2601.18448)
*Lloyd Austin Courtenay*

Main category: cs.CV

TL;DR: 该论文分析了在机器学习中使用几何形态测量时，标准GPA预处理可能导致训练集和测试集数据依赖的问题，提出了新的重对齐方法，并通过模拟实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 几何形态测量(GMM)广泛用于量化形状变异，并作为机器学习分析的输入。标准做法是在分割训练集和测试集之前对所有样本进行广义普氏分析(GPA)对齐，这可能引入统计依赖性并污染下游预测模型。

Method: 1. 使用受控的2D和3D模拟实验，在不同样本量、地标密度和异速生长模式下表征GPA引起的污染效应；2. 提出新的重对齐程序：在模型拟合前将测试样本对齐到训练集，消除跨样本依赖性；3. 使用线性和卷积回归模型分析地标间空间自相关的重要性。

Result: 1. 模拟实验揭示了样本量与地标空间之间的稳健"对角线"关系，反映了各向同性变异下RMSE的缩放规律；2. 从普氏切空间的自由度推导出斜率的解析表达式；3. 当忽略地标间关系时，模型性能会下降；4. 新的重对齐方法能有效消除跨样本依赖性。

Conclusion: 这项工作确立了在GMM的机器学习应用中需要仔细预处理的重要性，提供了重对齐的实用指南，并阐明了普氏形状空间固有的基本统计约束。强调了空间自相关在模型性能中的关键作用。
...
<details>
  <summary>Details</summary>
Motivation: 青光眼是全球不可逆失明的主要原因，当前筛查和进展评估依赖单一测试或松散关联的检查，存在主观性和碎片化护理问题。高质量影像工具和专家资源的有限获取进一步影响了现实世界应用的一致性和公平性。
...
Conclusion: Fair-Eye Net将公平性作为主要目标与临床可靠性通过多任务学习共同优化，不同于事后公平性调整，为临床转化和大规模部署提供了可复现的路径，有助于推进全球眼健康公平。
...
TL;DR: DisasterInsight是一个用于评估视觉语言模型在灾害分析任务中性能的多模态基准，基于xBD数据集构建了约112K个建筑中心实例，支持多种灾害分析任务。


<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉语言基准主要关注粗粒度标签和图像级识别，忽略了真实人道主义工作流程中所需的功能理解和指令鲁棒性，需要更贴近实际灾害响应的评估基准。

Method: 将xBD数据集重构为约112K个建筑中心实例，支持建筑功能分类、损坏等级分类、灾害类型分类、计数和结构化报告生成等多种任务评估。提出DI-Chat模型，通过参数高效的LoRA方法在灾害特定指令数据上微调现有VLM骨干网络。

Result: 实验显示现有通用和遥感VLM在各项任务上存在显著性能差距，特别是在损坏理解和结构化报告生成方面。DI-Chat在损坏等级分类、灾害类型分类和报告生成质量上取得显著改进，但建筑功能分类对所有评估模型仍具挑战性。

Conclusion: DisasterInsight为研究灾害图像中的基础多模态推理提供了统一基准，DI-Chat展示了领域适应方法的有效性，但建筑功能理解仍是未来研究的重要方向。
...
TL;DR: 提出一种新颖的主动学习框架，结合基础模型嵌入聚类进行冷启动采样，再通过不确定性选择与空间多样性整合，显著提升医学图像分割性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割标注需要大量时间和专业知识，成为疾病监测的瓶颈。主动学习虽然能通过选择信息量大的样本减轻标注负担，但传统的冷启动策略（通常基于多样性）和不确定性选择仍有改进空间。

Method: 1. 冷启动阶段：使用基础模型嵌入结合聚类（自动选择聚类数量），并进行跨聚类的比例采样，构建多样且具代表性的初始训练集。
2. 主动学习阶段：整合熵不确定性选择与空间多样性，指导样本选择。
3. 方法具有直观可解释性，支持候选样本特征空间分布的可视化。

Result: 在三个医学图像数据集（CheXmask、Montgomery、SynthStrip）上评估：
- CheXmask：冷启动策略将Dice从0.918提升至0.929，Hausdorff距离从32.41降至27.66 mm；主动学习将Dice从0.919提升至0.939，Hausdorff距离从30.10降至19.16 mm。
- Montgomery：冷启动显著提升，Dice从0.928提升至0.950，Hausdorff距离从14.22降至9.38 mm。
- SynthStrip：冷启动对Dice影响较小但降低Hausdorff距离（9.43→8.69 mm）；主动学习提升Dice（0.816→0.826）并降低Hausdorff距离（7.76→6.38 mm）。

Conclusion: 提出的主动学习框架在低数据量情况下持续优于基线方法，显著提升分割精度，为医学图像分割标注提供了高效且可解释的解决方案。
...
TL;DR: GenAgent通过代理框架统一视觉理解与生成，将理解任务交给多模态模型，生成任务则调用图像生成模型作为工具，实现自主多轮交互和迭代优化。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型面临昂贵的训练成本以及理解与生成之间的权衡问题，而模块化系统又受限于静态流水线。需要一种既能解耦能力又能支持自主多轮交互的框架。

Method: 采用代理框架设计：多模态模型负责理解，图像生成模型作为可调用工具。两阶段训练策略：1) 使用高质量工具调用和反思数据进行监督微调；2) 端到端代理强化学习，结合点对点奖励（最终图像质量）和成对奖励（反思准确性），并进行轨迹重采样以增强多轮探索。

Result: GenAgent显著提升了基础生成器（FLUX.1-dev）在GenEval++（+23.6%）和WISE（+14%）上的性能。框架展现出三个关键特性：1) 跨工具泛化能力；2) 测试时扩展性，随着交互轮次增加持续改进；3) 任务自适应推理能力。

Conclusion: GenAgent通过代理框架有效统一了视觉理解与生成，避免了传统统一模型的训练成本和能力权衡问题，同时支持自主多轮交互和迭代优化，在多个基准上取得了显著性能提升。
...
Result: MRI在cam型撞击症的定位和诊断准确性方面达到与X射线等效的水平。该方法在3D MRI体积的冠状视图中展示了FAI评估的临床可行性，为通过放置更多标志点进行体积分析开辟了可能性。

Conclusion: 研究结果支持将自动化FAI评估整合到常规MRI工作流程中，代码已开源。这为基于MRI的髋臼撞击症评估提供了新的自动化解决方案。
...
TL;DR: 提出SDA-QEC框架，结合简化扩散增强与量子增强分类，解决医学图像分类中的类别不平衡问题，在冠状动脉造影图像分类上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 真实世界医学数据集存在严重类别不平衡问题，阳性样本远多于阴性样本，导致模型偏向多数类，对少数类召回率低，影响诊断准确性并带来临床误诊风险。
...
Conclusion: 该方法验证了在真实世界医学成像任务中集成生成增强与量子增强建模的可行性，为开发在小样本、高度不平衡和高风险诊断场景下高度可靠的医学AI系统提供了新的研究途径。
...
TL;DR: 提出一种用于高光谱图像分类的高效AI卫星边缘计算范式，采用轻量级非深度学习框架结合小样本学习策略，开发了两阶段像素级标签传播方案，无需空间结构信息。


<details>
  <summary>Details</summary>
Motivation: 高光谱成像卫星传输速度成为灾害监测等应用的瓶颈，需要卫星具备自主决策能力。同时，卫星平台资源有限，且可能面临传感器故障和扫描模式错误导致的图像质量下降问题。

Method: 采用轻量级非深度学习框架结合小样本学习策略。提出两阶段像素级标签传播方案：第一阶段通过构建锚点-像素亲和矩阵传播选定锚点标签；第二阶段基于稀疏图的闭式解替代迭代计算。开发基于秩约束的图聚类算法确定锚点标签。

Result: 该方法仅利用像素级光谱特征，无需考虑空间结构信息，适应卫星资源约束和图像质量问题，实现了高效的星上高光谱图像分类。

Conclusion: 提出的AI卫星边缘计算范式能够使卫星具备自主决策能力，通过轻量级框架和两阶段标签传播方案有效解决了传输瓶颈和星上处理挑战，为灾害监测等应用提供快速响应能力。
...
Conclusion: 自精炼视频采样是一种简单有效的方法，能够显著提升视频生成的物理真实感，无需额外训练或外部验证器，为视频生成质量提升提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散模型适配器合并主要依赖手动滑块调节，这种方法在候选适配器数量较多（20-30个）时难以扩展且权重选择困难，需要更高效的探索方法

Method: 提出GimmBO系统，采用两阶段贝叶斯优化后端，结合偏好贝叶斯优化（PBO）支持交互式探索，针对实际使用中的稀疏性和权重范围约束进行优化

Result: 通过模拟用户和用户研究评估，相比贝叶斯优化和线性搜索基线，GimmBO在收敛性、成功率方面表现更优，并展示了框架的扩展灵活性

Conclusion: GimmBO为扩散模型适配器合并提供了高效的交互式探索解决方案，解决了手动调节的可扩展性问题
...
TL;DR: EFSI-DETR：一种用于无人机图像实时小目标检测的新框架，通过动态频率-空间协同网络和高效语义特征浓缩器，在VisDrone和CODrone基准上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 无人机图像中的实时小目标检测面临特征表示有限和多尺度融合效果不佳的挑战。现有方法未充分利用频率信息，依赖静态卷积操作，限制了获取丰富特征表示的能力，阻碍了深度语义特征的有效利用。

Method: 提出EFSI-DETR框架，包含两个主要组件：1) 动态频率-空间统一协同网络(DyFusNet)，联合利用频率和空间线索进行鲁棒的多尺度特征融合；2) 高效语义特征浓缩器(ESFC)，以最小计算成本实现深度语义提取。此外采用细粒度特征保留(FFR)策略，在融合过程中融入空间丰富的浅层特征以保留细粒度细节。

Result: 在VisDrone和CODrone基准上的广泛实验表明，EFSI-DETR实现了最先进的性能并具有实时效率：在VisDrone上AP和AP_s分别提升1.6%和5.8%，在单张RTX 4090 GPU上获得188 FPS的推理速度。

Conclusion: EFSI-DETR通过动态频率-空间协同和高效语义特征增强，有效解决了无人机图像中小目标检测的挑战，在保持实时效率的同时显著提升了检测精度。
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督学习在分割任务中对大尺度均匀区域有效，但在小尺度、稀疏或不规则目标上性能下降，需要针对目标尺度特性进行适配

Method: 将小窗口裁剪集成到数据增强流程中，在预训练阶段聚焦于精细尺度结构，实现尺度感知的自监督学习适配

Result: 在地震成像（断层分割）和神经影像（细胞分割）两个领域均取得显著提升，断层分割准确率提升13%，细胞分割提升5%，但对大尺度特征改善有限

Conclusion: 自监督学习的有效性高度依赖于目标对象的尺度特性，需要根据目标大小和稀疏性进行针对性设计，这为科学成像领域的表示学习提供了通用原则
...
TL;DR: 提出一种新的跨模态图像翻译方法，通过预测空间变化的混合场和注入目标一致的恢复项，改善结构保真度和语义一致性，同时减少去噪步骤。


<details>
  <summary>Details</summary>
Motivation: 传统扩散方法依赖单一的全局线性域转移，这迫使采样器遍历非流形的高成本区域，增加了校正负担并导致语义漂移。作者将这种共享失败模式称为固定调度域转移。

Method: 将域转移动态直接嵌入生成过程：在每个反向步骤预测空间变化的混合场，并在漂移中注入明确的目标一致恢复项。提供连续时间公式和精确解形式，推导出保持边际一致性的实用一阶采样器。

Result: 在医学成像、遥感和电致发光语义映射等翻译任务中，该框架提高了结构保真度和语义一致性，同时以更少的去噪步骤收敛。

Conclusion: 通过将域转移动态直接整合到生成过程中，并采用局部残差校正而非全局对齐的方法，可以显著改善跨模态图像翻译的质量和效率。
...
TL;DR: CONQUER是一个两阶段的文本行人搜索框架，通过训练时的跨模态对齐增强和推理时的自适应查询优化，显著提升了文本行人搜索的性能。


<details>
  <summary>Details</summary>
Motivation: 文本行人搜索面临跨模态差异和模糊用户查询的挑战，现有方法在实际应用中效果有限，需要更鲁棒的解决方案来应对现实世界部署的需求。

Method: 采用两阶段框架：训练阶段使用多粒度编码、互补对挖掘和基于最优传输的上下文引导匹配来学习鲁棒嵌入；推理阶段通过即插即用的查询增强模块，通过锚点选择和属性驱动丰富来优化模糊或不完整的查询。

Result: 在CUHK-PEDES、ICFG-PEDES和RSTPReid数据集上，CONQUER在Rank-1准确率和mAP指标上均显著优于现有基线方法，在跨域和不完整查询场景中表现出色。

Conclusion: CONQUER通过创新的训练和推理策略有效解决了文本行人搜索的核心挑战，为实际应用部署提供了实用且高效的解决方案。
...
TL;DR: Splat-Portrait：基于高斯泼溅的3D说话头生成方法，无需运动先验或3D监督，仅通过2D重建和分数蒸馏损失实现高质量3D头像重建和唇部运动合成。


<details>
  <summary>Details</summary>
Motivation: 现有3D说话头生成方法依赖领域特定的启发式方法（如基于变形的面部运动表示先验），导致3D头像重建不准确，影响动画的真实感。需要一种能自动学习从单张肖像图像解耦静态3D重建和背景，并基于音频生成自然唇部运动的方法。

Method: 基于高斯泼溅的方法，自动学习将单张肖像图像解耦为静态高斯泼溅表示的静态3D重建和预测的2D背景。通过纯2D重建和分数蒸馏损失进行训练，无需3D监督或关键点标注，仅基于输入音频生成自然唇部运动。

Result: 实验结果表明，Splat-Portrait在说话头生成和新视角合成方面表现优异，相比先前工作实现了更好的视觉质量。

Conclusion: Splat-Portrait成功解决了3D头部重建和唇部运动合成的挑战，无需运动驱动先验或3D监督，仅通过2D损失实现了高质量的说话头生成，代码和文档已公开。
...
TL;DR: 本文提出GAP框架评估文本到视频模型的地理公平性，发现Sora 2模型在不同地区表现出相对均匀的地理视觉知识，挑战了地理偏见假设。


<details>
  <summary>Details</summary>
Motivation: 虽然文本到视频生成技术取得了视觉上令人信服的成果，但尚不清楚这些模型是否编码了地理上公平的视觉知识。本文旨在调查文本到视频模型的地理公平性和地理基础视觉知识。

Method: 提出Geo-Attraction Landmark Probing (GAP)框架，通过旅游景点为中心的系统评估方法。构建GEOATTRACTION-500基准数据集，包含500个全球分布的景点。整合互补指标：全局结构对齐、细粒度关键点对齐和视觉语言模型判断，并与人类评估验证。

Result: 对Sora 2模型的评估显示，与常见的地理偏见假设相反，该模型在不同地区、发展水平和文化群体中表现出相对均匀的地理基础视觉知识，仅与景点流行度有弱相关性。

Conclusion: 当前文本到视频模型表达全球视觉知识比预期更均匀，突显了其在全球部署应用中的潜力，同时强调了随着系统发展需要持续评估的必要性。
...
TL;DR: 提出MinkUNeXt-VINE轻量级深度学习方法，在葡萄园环境中超越现有方法，通过预处理和Matryoshka表示学习多损失方法实现高效实时定位。


<details>
  <summary>Details</summary>
Motivation: 农业环境由于非结构化和缺乏显著地标，定位具有挑战性。现有方法在农业场景的物体分类和分割方面有研究，但移动机器人的地点识别任务在当前技术状态下并不简单。

Method: 提出MinkUNeXt-VINE方法，采用预处理和Matryoshka表示学习多损失方法，优先考虑低成本稀疏LiDAR输入和低维输出，确保实时场景的高效率。

Result: 在两个长期葡萄园数据集上验证，该方法在低成本和低分辨率输入数据上表现出鲁棒性能，实现了效率与性能的良好权衡。

Conclusion: 该方法在葡萄园环境中超越了现有方法，通过轻量级设计和高效处理策略，为农业环境中的实时定位提供了有效解决方案。
...
TL;DR: 提出SeNeDiF-OOD方法，基于语义嵌套二分融合的层次化框架，用于处理开放世界中异质性的OOD检测问题，在MonuMAI建筑风格识别系统中验证有效性。


<details>
  <summary>Details</summary>
Motivation: OOD检测对于AI在开放世界环境中的可靠部署至关重要，但现有单阶段检测器难以处理从低级损坏到语义偏移的异质性OOD数据。

Method: 提出SeNeDiF-OOD框架，将检测任务分解为二进制融合节点的层次结构，每层集成与特定语义抽象级别对齐的决策边界。

Result: 在MonuMAI建筑风格识别系统的案例研究中，该方法显著优于传统基线，能有效过滤多样化的OOD类别，同时保持分布内性能。

Conclusion: 语义嵌套二分融合的层次化框架为解决开放世界中异质性OOD检测问题提供了有效解决方案，在真实世界应用中表现出优越性能。
...
### [162] [AI-based System for Transforming text and sound to Educational Videos](https://arxiv.org/abs/2601.17022)
...
<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在图像和视频生成方面有广泛应用，但从文本或语音等条件输入生成教育视频仍然是一个挑战。现有方法在生成完整教育视频方面存在不足，需要开发能够从文本或语音输入自动创建高质量教育视频的系统。

Method: 提出三阶段框架：1) 语音识别阶段：将输入文本或语音转录；2) 图像生成阶段：提取关键词，使用CLIP和扩散模型生成语义对齐的高质量图像；3) 视频合成阶段：将生成的图像合成为视频，并集成预录制或合成的声音，创建完整的交互式教育视频。

Result: 系统与TGAN、MoCoGAN、TGANS-C等现有方法相比，取得了28.75%的Fréchet Inception Distance (FID)分数，表明视觉质量显著提升，优于现有方法。

Conclusion: 提出的基于GAN的教育视频生成系统能够从文本或语音输入自动创建高质量教育视频，在视觉质量上优于现有方法，为教育内容创作提供了有效的自动化解决方案。
...
### [163] [Integrating Fine-Grained Audio-Visual Evidence for Robust Multimodal Emotion Reasoning](https://arxiv.org/abs/2601.18321)
*Zhixian Zhao,Wenjie Tian,Xiaohai Tian,Jun Zhang,Lei Xie*

Main category: cs.MM

TL;DR: SABER-LLM：一个用于鲁棒多模态情感推理的框架，通过构建大规模情感推理数据集和结构化证据分解范式，解决当前MLLMs在细粒度感知和跨模态融合方面的局限性。
...
Method: 1. 构建SABER大规模情感推理数据集（60万个视频片段），采用新颖的六维标注模式联合捕捉视听线索和因果逻辑；2. 提出结构化证据分解范式，强制"感知-推理"分离以缓解单模态主导；3. 通过一致性感知直接偏好优化强化复杂场景感知能力。

Result: 在EMER、EmoBench-M和SABER-Test上的实验表明，SABER-LLM显著优于开源基线模型，在解码复杂情感动态方面达到与闭源模型竞争的鲁棒性。

Conclusion: SABER-LLM通过大规模数据集和结构化推理框架有效解决了多模态情感分析中的细粒度感知和跨模态融合问题，为复杂社交情境下的情感推理提供了鲁棒解决方案。
...
### [164] [Online parameter estimation for the Crazyflie quadcopter through an EM algorithm](https://arxiv.org/abs/2601.17009)
*Yanhua Zhao*

Main category: cs.AI

TL;DR: 该论文研究在四旋翼无人机系统中添加随机噪声的影响，使用扩展卡尔曼滤波进行状态估计，基于SDE系统实现LQG控制器，并应用期望最大化算法进行参数估计，比较了离线和在线参数估计的性能。


<details>
  <summary>Details</summary>
Motivation: 无人机因其体积小、成本低、可靠性高而被广泛应用于地震救援、航拍、农业和运输等领域。然而在实际应用中，无人机系统会受到各种噪声干扰，影响其性能。研究随机噪声对四旋翼无人机系统的影响，并开发有效的状态估计和控制方法具有重要意义。

Method: 1. 在四旋翼无人机系统中添加随机噪声；2. 使用扩展卡尔曼滤波器基于传感器的噪声观测进行状态估计；3. 基于随机微分方程系统实现线性二次高斯控制器；4. 应用期望最大化算法进行参数估计，包括离线和在线两种方式。

Result: 研究结果显示，在线参数估计的收敛值范围比离线参数估计略大。这表明在线参数估计方法在应对系统变化和噪声影响方面具有更好的适应性。

Conclusion: 该研究成功地将随机噪声建模、状态估计和控制方法应用于四旋翼无人机系统，证明了期望最大化算法在参数估计中的有效性，特别是在线参数估计方法在无人机控制系统中的实用价值。
...
### [165] [Interpreting Agentic Systems: Beyond Model Explanations to System-Level Accountability](https://arxiv.org/abs/2601.17168)
*Judy Zhu,Dhari Gandhi,Himanshu Joshi,Ahmad Rezaie Mianroodi,Sedef Akinli Kocak,Dhanesh Ramachandran*

Main category: cs.AI

TL;DR: 论文评估现有可解释性方法在智能体系统中的应用局限，提出需要专门针对智能体系统设计新的可解释性技术，以确保AI系统的安全和可问责部署。


<details>
  <summary>Details</summary>
Motivation: 智能体系统与传统机器学习模型在架构和部署上存在根本差异，引入了独特的安全挑战（如目标错位、决策错误累积、多智能体协调风险），需要嵌入可解释性和可追溯性设计，但现有针对静态模型的可解释性方法在应用于智能体系统时存在局限性。

Method: 评估现有可解释性方法在智能体系统中的适用性和局限性，识别其在提供智能体决策洞察方面的能力差距，并提出专门针对智能体系统设计可解释性技术的未来方向。

Result: 发现现有可解释性方法在应用于智能体系统时存在不足，无法充分处理智能体系统的时间动态性、累积决策和上下文依赖行为等特性。

Conclusion: 需要开发专门针对智能体系统的新型可解释性技术，在智能体生命周期的各个阶段（目标形成、环境交互、结果评估）嵌入监督机制，这对于确保智能体AI系统的安全和可问责部署至关重要。
...
### [166] [Implementing Tensor Logic: Unifying Datalog and Neural Reasoning via Tensor Contraction](https://arxiv.org/abs/2601.17188)
...
<details>
  <summary>Details</summary>
Motivation: 符号系统具有可靠性和可解释性但缺乏可扩展性，神经网络具有学习能力但缺乏透明度。Tensor Logic提出逻辑规则与爱因斯坦求和在数学上等价，为两者统一提供了理论路径。

Method: 通过三个实验验证Tensor Logic：1) 递归Datalog规则与迭代张量收缩的等价性验证；2) 嵌入空间中可学习变换矩阵的神经网络实现；3) 大规模知识图谱上的关系矩阵构造验证。

Result: 1) 在圣经家谱图上成功计算传递闭包；2) 实现零样本组合推理；3) 在FB15k-237上获得MRR 0.3068（标准链接预测）和0.3346（组合推理基准），证明矩阵组合支持多跳推理。

Conclusion: Tensor Logic为符号推理与神经网络的统一提供了实证支持，张量运算能够有效实现逻辑推理、组合推理和多跳推理，为可解释AI系统开辟了新途径。
...
### [167] [High-Fidelity Longitudinal Patient Simulation Using Real-World Data](https://arxiv.org/abs/2601.17310)
*Yu Akagi,Tomohisa Seki,Hiromasa Ito,Toru Takiguchi,Kazuhiko Ohe,Yoshimasa Kawazoe*

Main category: cs.AI

TL;DR: 利用真实世界临床记录开发生成式模拟器，能够基于患者历史生成高保真的未来临床轨迹，准确预测事件概率和时间动态。
...
Result: 模型生成了高保真的未来时间线，与真实患者未来数据中的事件发生率、实验室测试结果和时间动态高度匹配。准确估计了未来事件概率，观察值与预期值比率在不同结果和时间范围内始终接近1.0。
...
### [168] [Phase Transition for Budgeted Multi-Agent Synergy](https://arxiv.org/abs/2601.17311)
*Bang Liu,Linglong Kong,Jian Pei*

Main category: cs.AI

TL;DR: 该论文提出了一个可校准的理论框架，用于预测多智能体系统在有限推理预算下的三种性能模式：提升、饱和和崩溃。该理论基于三个关键约束：有限上下文窗口、有损通信和智能体间的共享故障相关性。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统理论上能提高可靠性，但在固定推理预算下，其性能往往呈现提升、饱和甚至崩溃三种模式。现有研究缺乏一个能够统一解释这些现象并指导系统设计的理论框架。

Method: 构建了一个最小化可校准理论模型，包含三个核心参数：智能体计算性能缩放指数β、消息长度保真度曲线γ(m)、有效共享误差相关性ρ。通过分析二元成功/失败任务中的多数聚合机制，证明了深度b叉树结构中存在尖锐的相变现象。

Result: 发现单个标量αρ（结合γ(m)、ρ和扇入b）决定了弱信号是被放大到非平凡固定点还是被淹没到随机水平。在放大区域，推导出组织指数s，并证明当s>β时出现预算协同效应（优于同等预算下的最佳单智能体）。

Conclusion: 该理论框架能够解释多智能体系统性能的相变行为，提供计算分配规则和预算阈值，并通过合成模拟验证了预测的相边界。该模型揭示了现代智能体堆栈中的核心设计权衡，为实际系统设计提供了理论指导。
...
### [169] [TheoremForge: Scaling up Formal Data Synthesis with Low-Budget Agentic Workflow](https://arxiv.org/abs/2601.17332)
*Yicheng Tao,Hongteng Xu*

Main category: cs.AI

TL;DR: TheoremForge是一个低成本的形式数学数据合成管道，通过将形式化过程分解为五个子任务，并使用解耦提取策略从失败轨迹中回收有效训练信号，显著降低了形式数学数据合成的成本。


<details>
  <summary>Details</summary>
Motivation: 形式数学中智能体工作流的高成本阻碍了大规模数据合成，加剧了开源语料库的稀缺性。需要一种成本效益高的方法来生成形式数学训练数据。

Method: 将形式化过程分解为五个子任务：陈述形式化、证明生成、前提选择、证明修正和证明草图。采用解耦提取策略，从全局失败的轨迹中回收有效训练信号，有效利用浪费的计算资源。

Result: 在2000个问题的基准测试中，TheoremForge实现了12.6%的验证率，超过8.6%的基线，每个成功轨迹的平均成本仅为0.481美元（使用Gemini-3-Flash）。解耦提取策略使证明生成的数据产出提高了1.6倍。

Conclusion: TheoremForge为构建数据飞轮以训练未来专家模型提供了一个可扩展的框架，显著降低了形式数学数据合成的成本并提高了数据产出效率。
...
### [170] [The Relativity of AGI: Distributional Axioms, Fragility, and Undecidability](https://arxiv.org/abs/2601.17335)
*Angshul Majumdar*

Main category: cs.AI

TL;DR: 该论文证明AGI（人工通用智能）无法获得独立于任务分布的统一定义，缺乏普遍鲁棒性，存在有界迁移限制，且无法通过计算程序（包括自认证）进行完备验证。


<details>
  <summary>Details</summary>
Motivation: 研究AGI是否存在一个连贯的理论定义，能够支持关于存在性、鲁棒性或自验证的绝对主张。探讨强分布无关的AGI声明是否成立，以及AI的实证进展是否意味着可实现自认证的通用智能。

Method: 将AGI形式化为一个分布性、资源受限的语义谓词，通过任务族、任务分布、性能函数和显式资源预算进行索引。在此框架下，运用公理化方法、非不变性分析、有界迁移理论以及Rice风格和哥德尔-塔斯基论证。

Result: 1) 通用性是关系性的，不存在分布无关的AGI概念；2) 任务分布的任意小扰动可通过悬崖集使AGI属性失效，排除普遍鲁棒性；3) 有限资源下无法实现跨任务族的无界泛化；4) AGI是非平凡语义属性，无法通过任何计算程序（包括代理自身）进行完备验证。

Conclusion: 强分布无关的AGI声明在没有显式形式索引的情况下是未定义的，而非错误。AI的实证进展并不暗示可实现自认证的通用智能，依赖内部自认证的递归自我改进方案是病态的。
...
### [171] [Are We Evaluating the Edit Locality of LLM Model Editing Properly?](https://arxiv.org/abs/2601.17343)
*Wei Liu,Haomei Xu,Hongkai Liu,Zhiying Deng,Ruixuan Li,Heng Huang,Yee Whye Teh,Wee Sun Lee*

Main category: cs.AI

TL;DR: 论文指出现有模型编辑特异性评估协议存在缺陷，提出了新的评估框架，能更敏感地衡量知识保留能力


<details>
  <summary>Details</summary>
Motivation: 模型编辑需要平衡编辑效果（成功注入目标知识）和特异性（保留现有非目标知识），但现有特异性评估协议不足，存在三个基本问题：概念问题、与特异性正则化器弱相关、缺乏敏感性

Method: 提出建设性评估协议，消除开放LLM与确定答案假设的冲突，避免查询无关的流畅性偏差，评估严格度可在近连续空间中平滑调整

Result: 实验表明，新协议产生的指标对特异性正则化器强度变化更敏感，与正则化器强相关，能更细粒度地区分不同方法的知识保留能力

Conclusion: 提出的评估协议解决了现有特异性评估的不足，为模型编辑方法提供了更可靠的知识保留能力评估框架
...
### [172] [Multi-Agent Learning Path Planning via LLMs](https://arxiv.org/abs/2601.17346)
*Haoxin Xu,Changyong Qi,Tong Liu,Bohao Zhang,Anna He,Bingqian Jiang,Longwei Zheng,Xiaoqing Gu*

Main category: cs.AI

TL;DR: 提出基于多智能体协作的MALPP框架，利用LLM驱动的智能体进行个性化学习路径规划，通过角色和规则机制实现透明、可解释的路径推荐。
...
Method: 提出多智能体学习路径规划框架，包含三个任务特定智能体：学习者分析智能体、路径规划智能体和反思智能体，通过结构化提示和预定义规则协作，基于认知负荷理论和最近发展区理论设计。

Result: 在MOOCCubeX数据集上使用7个LLM进行实验，MALPP在路径质量、知识序列一致性和认知负荷对齐方面显著优于基线模型，消融研究验证了协作机制和理论约束的有效性。

Conclusion: 该研究为教育领域可信、可解释AI的发展做出贡献，展示了基于LLM的以学习者为中心的自适应教学的可扩展方法。
...
### [173] [Auditing Disability Representation in Vision-Language Models](https://arxiv.org/abs/2601.17348)
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型越来越多地应用于社会敏感领域，但它们在残疾相关问题上的行为尚未得到充分探索。模型在描述人物图像时经常从基于证据的事实描述转向包含超出可观察视觉证据的未经支持推断的解释偏移。

Method: 引入基于中性提示(NP)和残疾情境化提示(DP)配对的基准测试，在零样本设置下评估15个最先进的开放和闭源视觉语言模型，涵盖9个残疾类别。评估框架以解释保真度为核心目标，结合标准文本指标(捕捉情感降级、社会关注和响应长度变化)和经过残疾生活经验标注者验证的LLM-as-judge协议。

Result: 引入残疾上下文会持续降低解释保真度，导致解释偏移，表现为推测性推断、叙事扩展、情感降级和缺陷导向框架。这些效应在种族和性别维度上进一步放大。针对性提示和偏好微调能有效提高解释保真度并显著减少解释偏移。

Conclusion: 视觉语言模型在残疾相关描述中存在系统性偏差，需要开发更公平、基于证据的模型。针对性干预措施如提示工程和微调可以有效改善模型表现，减少解释偏移。
...
### [174] [A Syllogistic Probe: Tracing the Evolution of Logic Reasoning in Large Language Models](https://arxiv.org/abs/2601.17426)
*Zhengqing Zang,Yuqi Ding,Yanmei Gu,Changkai Song,Zhengkai Yang,Guoping Du,Junbo Zhao,Haobo Wang*

Main category: cs.AI

TL;DR: LLMs在逻辑推理中表现出从传统逻辑向现代逻辑的演变趋势，模型规模扩展、思维链推理和基础模型是影响这一转变的关键因素。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型是否像人类逻辑一样，从直觉驱动的推理演变为严谨的形式系统，使用存在引入作为探针来评估三段论推理。

Method: 使用存在引入作为探针，在传统逻辑和现代逻辑下评估三段论推理，通过在新构建的三段论数据集上测试SOTA LLMs进行广泛实验。

Result: 发现三个关键结果：(1) 模型规模扩展促进向现代逻辑的转变；(2) 思维链推理是超越参数扩展的高效加速器；(3) 基础模型决定这一转变的难易程度和稳定性。

Conclusion: LLMs在逻辑推理中确实表现出从传统到现代逻辑的演变趋势，模型规模、思维链和基础模型是影响这一转变的核心因素，为进一步分析LLMs的三段论推理特性提供了基础。
...
### [175] [Lattice: Generative Guardrails for Conversational Agents](https://arxiv.org/abs/2601.17481)
*Emily Broadhurst,Tawab Safi,Joseph Edell,Vashisht Ganesh,Karime Maamari*

Main category: cs.AI

TL;DR: Lattice框架通过两阶段自构建和持续改进机制，为对话AI系统提供动态可适应的安全护栏，相比静态规则方法显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有对话AI系统的安全护栏使用静态规则，无法适应新威胁和部署环境变化，需要能够自主适应和改进的动态防护机制

Method: Lattice框架采用两阶段方法：1)构建阶段通过迭代模拟和优化从标注示例构建初始护栏；2)持续改进阶段通过风险评估、对抗测试和整合自主适应已部署的护栏
...
Conclusion: Lattice框架证明有效的安全护栏可以通过迭代优化自构建，为对话AI系统提供动态适应性的防护机制
...
### [176] [Cognitive Platform Engineering for Autonomous Cloud Operations](https://arxiv.org/abs/2601.17542)
...
TL;DR: 提出认知平台工程新范式，通过四层架构整合数据收集、智能推理、策略编排和人工交互，实现云原生系统的自主运维和弹性调整。


<details>
  <summary>Details</summary>
Motivation: 传统DevOps自动化难以应对云原生系统的规模和动态性，规则驱动方法导致被动运维、修复延迟和依赖人工经验，需要更智能的平台工程方法。

Method: 提出认知平台工程范式，设计四层参考架构：数据收集层、智能推理层、策略驱动编排层和人工体验层，构建持续反馈循环。原型使用Kubernetes、Terraform、Open Policy Agent和基于ML的异常检测。

Result: 原型实现显示在平均修复时间、资源效率和合规性方面有改进，证明将智能嵌入平台运维可实现弹性、自调整和意图对齐的云环境。

Conclusion: 认知平台工程使云环境更具弹性和自适应性，未来研究方向包括强化学习、可解释治理和可持续自管理云生态系统。
...
### [177] [JaxARC: A High-Performance JAX-based Environment for Abstraction and Reasoning Research](https://arxiv.org/abs/2601.17564)
*Aadam,Monu Verma,Mohamed Abdel-Mottaleb*

Main category: cs.AI

TL;DR: JaxARC是一个基于JAX的高性能强化学习环境，用于Abstraction and Reasoning Corpus（ARC）任务，相比Gymnasium实现了38-5,439倍加速，支持大规模并行化研究。
...
Method: 使用JAX实现功能化、无状态的架构，支持大规模并行化。支持多个ARC数据集、灵活的动作空间、可组合的包装器和配置驱动的可复现性。

Result: 在相同批量大小下实现38-5,439倍加速，峰值吞吐量达到7.9亿步/秒。相比现有环境显著提升了计算效率。

Conclusion: JaxARC解决了ARC任务中强化学习研究的计算瓶颈问题，使得之前计算上不可行的大规模研究成为可能，为AI推理能力研究提供了高效工具。
...
### [178] [Discovery of Feasible 3D Printing Configurations for Metal Alloys via AI-driven Adaptive Experimental Design](https://arxiv.org/abs/2601.17587)
*Azza Fadhel,Nathaniel W. Zuckschwerdt,Aryan Deshwal,Susmita Bose,Amit Bandyopadhyay,Jana Doppa*

Main category: cs.AI

TL;DR: 结合AI驱动的自适应实验设计与领域知识，开发了一种高效发现金属增材制造可行参数配置的方法，成功应用于NASA开发的GRCop-42合金，大幅缩短实验时间并降低成本。


<details>
  <summary>Details</summary>
Motivation: 金属合金增材制造的参数配置是一个复杂问题，传统试错方法效率低下且成本高昂，需要一种更智能的方法来发现可行的工艺参数配置。

Method: 结合AI驱动的自适应实验设计与领域知识，通过构建过去实验的代理模型，在每次迭代中智能选择一小批输入配置进行验证，实现高效参数探索。

Result: 在三个月内成功获得多个无缺陷的GRCop-42打印输出，大幅减少了结果获取时间和资源消耗，相比领域科学家数月的手动实验无果，取得了显著突破。

Conclusion: 该方法首次在现成的红外激光平台上实现了高质量GRCop-42制造，为航空航天应用的成本效益高、分散化生产铺平了道路，具有重要的实际应用价值。
...
### [179] [Intelligence Requires Grounding But Not Embodiment](https://arxiv.org/abs/2601.17588)
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM的发展，关于智能是否需要具身的科学争论重新兴起。本文旨在澄清这一争论，提出智能需要的是接地而非具身本身

Method: 首先定义智能为具备四个属性：动机、预测能力、因果理解、经验学习。然后论证每个属性都可以通过非具身但接地的智能体实现。最后通过数字环境中智能LLM代理的思想实验来支持论点

Result: 论证表明接地（而非具身）是智能的必要条件。智能的四个关键属性都可以在非具身但接地的系统中实现

Conclusion: 智能需要的是接地现象（由具身所蕴含），而非具身本身。这为理解人工智能的本质提供了新的理论框架
...
### [180] [Health-ORSC-Bench: A Benchmark for Measuring Over-Refusal and Safety Completion in Health Context](https://arxiv.org/abs/2601.17642)
*Zhihao Zhang,Liting Huang,Guanghao Wu,Preslav Nakov,Heng Ji,Usman Naseem*

Main category: cs.AI

TL;DR: 提出了首个大规模医疗健康领域基准Health-ORSC-Bench，用于系统评估LLMs在医疗查询中的过度拒绝和安全完成能力，发现当前模型在安全性和实用性之间存在显著权衡


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在医疗健康领域的安全对齐主要依赖二元拒绝边界，导致对良性查询的过度拒绝或对有害查询的不安全遵从。现有基准只能测量极端情况，无法评估模型在双重用途或边界查询中提供安全高层指导的能力

Method: 构建包含31,920个良性边界提示的Health-ORSC-Bench基准，涵盖7个健康类别（如自残、医疗错误信息）。采用自动化流水线结合人工验证，在不同意图模糊度水平上测试模型。评估了30个最先进的LLM，包括GPT-5和Claude-4
...
Conclusion: 当前LLM难以平衡拒绝和遵从，Health-ORSC-Bench为下一代医疗AI助手提供了严格的校准标准，促进细致、安全和有用的完成。代码和数据将在接受后发布
...
### [181] [DIML: Differentiable Inverse Mechanism Learning from Behaviors of Multi-Agent Learning Trajectories](https://arxiv.org/abs/2601.17678)
*Zhiyu An,Wan Du*

Main category: cs.AI

TL;DR: 提出DIML框架，从自利学习代理的战略交互轨迹中逆向学习未知的激励生成机制，包括非结构化机制（如神经网络映射），支持反事实预测并在大规模环境中表现良好。


<details>
  <summary>Details</summary>
Motivation: 现有方法如逆向博弈论和多智能体逆向强化学习通常只能推断结构化机制内的效用/奖励参数，而无法处理非结构化机制（如神经网络映射）。同时，可微分机制设计是前向优化机制，而非从观察行为中推断机制。需要一种能够从战略交互轨迹中逆向学习任意激励生成机制的方法。

Method: 提出DIML（Differentiable Inverse Mechanism Learning）框架，基于似然的方法，通过多智能体学习动态模型进行微分，并使用候选机制生成预测观察行为所需的反事实收益。在条件logit响应模型下建立收益差异的可识别性，并在标准正则条件下证明最大似然估计的统计一致性。

Result: DIML在非结构化神经网络机制、拥堵收费、公共物品补贴和大规模匿名博弈等模拟交互中可靠地恢复了可识别的激励差异。在小环境中性能接近表格枚举oracle，在大规模（百参与者）环境中具有良好的收敛性，支持反事实预测。

Conclusion: DIML框架能够从战略交互轨迹中逆向学习激励生成机制，包括非结构化机制，为机制逆向学习提供了有效的解决方案，并在各种环境中展现出良好的性能和可扩展性。
...
### [182] [SQL-Trail: Multi-Turn Reinforcement Learning with Interleaved Feedback for Text-to-SQL](https://arxiv.org/abs/2601.17699)
...
TL;DR: SQL-Trail：一个用于Text-to-SQL的多轮强化学习代理框架，通过迭代交互和自适应预算分配，显著提升SQL生成性能，在BIRD-SQL等基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前Text-to-SQL生成主要采用单次生成范式，缺乏人类专家在SQL生成中使用的迭代推理、模式探索和错误纠正能力，导致在BIRD-SQL等挑战性基准上与人类专家存在明显差距。

Method: 提出SQL-Trail多轮强化学习代理框架：1）与数据库环境交互，利用执行反馈迭代优化预测；2）自适应轮次预算分配机制，根据问题难度调整交互深度；3）复合奖励面板，联合激励SQL正确性和高效探索。

Result: 在多个基准测试中达到新的SOTA，数据效率比之前单次RL SOTA方法高18倍。7B和14B模型平均超越大型专有系统5%，展示了交互式代理工作流的有效性。

Conclusion: 交互式、代理式工作流能够显著提升Text-to-SQL生成的鲁棒性，多轮强化学习框架通过迭代推理和自适应交互，有效缩小了AI系统与人类专家在复杂SQL生成任务上的差距。
...
### [183] [The LLM Data Auditor: A Metric-oriented Survey on Quality and Trustworthiness in Evaluating Synthetic Data](https://arxiv.org/abs/2601.17717)
...
TL;DR: 本文提出了LLM数据审计框架，系统评估LLM生成多模态合成数据的质量和可信度，发现现有评估方法存在不足并给出改进建议。


<details>
  <summary>Details</summary>
Motivation: LLM已成为生成多模态数据的有力工具，但确保合成数据的高质量仍是关键挑战。现有研究主要关注生成方法，对数据质量评估关注有限，且多为单模态研究，缺乏跨模态的统一视角。
...
Conclusion: LLM数据审计框架将评估重点从依赖下游任务性能的外在评估转向数据本身的内在属性，为多模态合成数据的质量评估提供了系统化方法，有助于推动该领域的发展。
...
### [184] [EntWorld: A Holistic Environment and Benchmark for Verifiable Enterprise GUI Agents](https://arxiv.org/abs/2601.17722)
*Ying Mo,Yu Bai,Dapeng Sun,Yuqian Shi,Yukai Miao,Li Chen,Dan Li*

Main category: cs.AI

TL;DR: EntWorld是一个大规模企业级基准测试，包含1,756个任务，涵盖CRM、ITIL、ERP等6个企业领域，用于评估多模态大语言模型在复杂企业工作流中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要针对消费级场景（如电商、旅游预订），无法捕捉专业企业工作流的复杂性和严谨性。企业系统具有高密度用户界面、严格业务逻辑约束和精确状态一致性要求等独特挑战，当前通用智能体在这些场景中表现不佳。

Method: 采用基于模式的任务生成框架，直接从底层数据库模式逆向工程业务逻辑，合成真实的长时程工作流。提出基于SQL的确定性验证机制，用严格的状态转换验证替代模糊的视觉匹配。

Result: 实验结果显示，最先进模型（如GPT-4.1）在EntWorld上的成功率仅为47.61%，远低于人类表现，突显了当前智能体在企业领域的能力差距。

Conclusion: EntWorld揭示了当前多模态大语言模型在企业环境中的显著能力差距，需要开发领域特定的企业级数字智能体。该基准测试为下一代企业就绪数字智能体的开发和评估提供了严谨的测试平台。
...
### [185] [ReFuGe: Feature Generation for Prediction Tasks on Relational Databases with LLM Agents](https://arxiv.org/abs/2601.17735)
*Kyungho Kim,Geon Lee,Juyeon Kim,Dongwon Choi,Shinhwan Kang,Kijung Shin*

Main category: cs.AI

TL;DR: ReFuGe是一个基于LLM代理的框架，用于自动生成关系数据库中的预测性特征，通过三个代理协同工作：模式选择、特征生成和特征过滤，在迭代反馈循环中提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 关系数据库在现实应用中广泛使用，但针对关系数据库的预测任务面临挑战：需要处理复杂模式、探索组合爆炸的特征空间，且缺乏明确的监督信号。现有方法难以自动生成信息丰富的预测特征。

Method: 提出ReFuGe框架，包含三个LLM代理：1）模式选择代理识别与任务相关的表和列；2）特征生成代理从选定模式生成多样化候选特征；3）特征过滤代理通过推理和验证两种方式评估并保留有前景的特征。这些代理在迭代反馈循环中协同工作，直到性能收敛。

Result: 在关系数据库基准测试上的实验表明，ReFuGe在各种关系数据库预测任务上显著提升了性能。

Conclusion: ReFuGe通过LLM代理的协同工作，能够有效解决关系数据库中预测特征生成的挑战，为关系数据库预测任务提供了有效的自动化特征工程解决方案。
...
### [186] [Faramesh: A Protocol-Agnostic Execution Control Plane for Autonomous Agent Systems](https://arxiv.org/abs/2601.17744)
*Amjad Fatmi*

Main category: cs.AI

TL;DR: Faramesh是一个协议无关的执行控制平面，通过不可绕过的行动授权边界(AAB)为自主代理驱动的行动强制执行执行时授权，确保组织在行动改变现实前能够确定性地允许、拒绝或推迟行动。


<details>
  <summary>Details</summary>
Motivation: 自主代理系统越来越多地触发现实世界的副作用（部署基础设施、修改数据库、移动资金、执行工作流），但大多数代理栈没有提供强制性的执行检查点，让组织在行动改变现实前能够确定性地允许、拒绝或推迟行动。

Method: Faramesh将代理意图规范化为规范行动表示(CAR)，根据策略和状态确定性地评估行动，并生成决策工件(PERMIT/DEFER/DENY)，执行器在执行前必须验证该决策。系统设计为框架和模型无关，支持多代理和多租户部署，独立于传输协议，并提供基于规范行动哈希的决策中心、仅追加的溯源日志。

Result: Faramesh为自主执行提供了可强制执行、可预测的治理，避免了与编排层的隐藏耦合或仅观察性的方法，实现了可审计性、验证性和确定性重放，而无需重新运行代理推理。

Conclusion: Faramesh通过协议无关的执行控制平面和不可绕过的行动授权边界，为自主代理系统提供了强制性的执行时授权机制，确保组织能够在行动产生现实影响前进行确定性控制，同时保持框架和模型的独立性。
...
### [187] [HyCARD-Net: A Synergistic Hybrid Intelligence Framework for Cardiovascular Disease Diagnosis](https://arxiv.org/abs/2601.17767)
...
<details>
  <summary>Details</summary>
Motivation: 心血管疾病是全球主要死因，传统预测模型在异构数据集和复杂生理模式上泛化能力不足，需要智能数据驱动的诊断工具

Method: 集成CNN和LSTM深度学习架构与KNN、XGB传统机器学习算法，采用投票机制构建混合集成框架

Result: 在两个Kaggle数据集上分别达到82.30%和97.10%的准确率，在精确率、召回率和F1分数上均有稳定提升

Conclusion: 混合AI框架在心血管疾病预测中具有鲁棒性和临床潜力，支持联合国可持续发展目标3，促进早期诊断和干预
...
### [188] [Neuro-Symbolic Verification on Instruction Following of LLMs](https://arxiv.org/abs/2601.17789)
...
Result: 在VIFBENCH基准测试中，NSVIF显著优于基于LLM的方法，并提供可解释的反馈。NSVIF的反馈还能在不进行后训练的情况下提高LLM的指令遵循能力。

Conclusion: NSVIF是一个通用、通用的验证框架，能够有效验证LLM输出是否遵循指令，为解决LLM指令遵循问题提供了有效的神经符号解决方案。
...
### [189] [MMR-Bench: A Comprehensive Benchmark for Multimodal LLM Routing](https://arxiv.org/abs/2601.17814)
*Haoxuan Ma,Guannan Lai,Han-Jia Ye*

Main category: cs.AI

TL;DR: MMR-Bench是一个多模态路由基准测试，用于在固定候选模型和成本模型下评估多模态大语言模型的选择策略，通过路由优化实现成本与精度的平衡。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在架构、对齐策略和效率方面存在异质性，单一模型无法在所有任务上表现最优。实际部署中，工作负载从轻量OCR到复杂多模态推理不等，使用单一模型要么在简单实例上过度配置计算资源，要么在困难实例上牺牲准确性。需要将文本LLM的路由扩展到多模态场景。

Method: 提出MMR-Bench基准测试，提供：1）具有模态感知输入和可变计算预算的受控环境；2）涵盖OCR、通用VQA和多模态数学推理的广泛视觉语言任务套件；3）强大的单模型参考、理论上限和代表性路由策略。通过多模态信号改进路由质量。

Result: 多模态线索改善了成本-精度边界，使路由系统能够以最强单模型约33%的成本超越其准确性。在模型和任务子集上训练的策略能够零样本泛化到新数据集和纯文本基准测试，无需重新调整。

Conclusion: MMR-Bench为研究自适应多模态模型选择和高效MLLM部署奠定了基础，证明了多模态路由在实际部署中的可行性和价值。
...
### [190] [RegGuard: AI-Powered Retrieval-Enhanced Assistant for Pharmaceutical Regulatory Compliance](https://arxiv.org/abs/2601.17826)
*Siyuan Yang,Xihan Bian,Jiayin Tang*

Main category: cs.AI

TL;DR: RegGuard是一个工业级AI助手，用于自动化解读异构监管文本并与企业内部政策对齐，通过HiSACC和ReLACE技术提升检索和生成质量，显著减少幻觉风险。


<details>
  <summary>Details</summary>
Motivation: 监管更新日益频繁复杂，跨国制药公司面临沉重合规负担。合规团队需要跨司法管辖区、格式和机构手动解读不断变化的规则，成本高且易出错。

Method: 系统通过安全管道摄入异构文档，采用两个核心组件：HiSACC（分层语义聚合上下文分块）将长文档语义分割为连贯单元；ReLACE（监管列表自适应交叉编码器重排序）基于开源模型构建，联合建模用户查询和检索候选以提升排序相关性。

Result: 企业环境评估显示，RegGuard在相关性、基础性和上下文聚焦方面显著提升答案质量，同时大幅降低幻觉风险。系统架构具备可审计性和可追溯性。

Conclusion: RegGuard为具有严格合规要求的领域提供了高效解决方案，系统架构支持来源追踪、访问控制和增量索引，能够快速响应不断变化的文档源。
...
### [191] [Aligning Medical Conversational AI through Online Reinforcement Learning with Information-Theoretic Rewards](https://arxiv.org/abs/2601.17828)
*Tanvi Verma,Yang Zhou,Rick Siow Mong Goh,Yong Liu*

Main category: cs.AI

TL;DR: IGFT是一种无需人类对话数据、通过信息增益奖励训练医疗对话AI的新方法，能在多轮对话中高效收集病史信息。


<details>
  <summary>Details</summary>
Motivation: 现有医疗对话AI依赖昂贵的人工标注对话数据或静态数据集，难以学习有效的多轮问诊策略。需要一种能自主探索、发现有效提问策略的方法。

Method: 结合在线Group Relative Policy Optimization和信息论奖励，通过信息增益函数追踪对话中揭示的临床实体（症状、时间模式、病史），结合GPT-4o-mini的质量评估，训练模型进行自我对话学习。

Result: 在Avey和MIMIC数据集上，DeepSeek-R1-Distill-Qwen-7B (IGFT)分别获得0.408和0.289的F1分数，相比基线提升10.9%和12.9%。两个模型均优于OpenAI模型和医疗专用基线模型。

Conclusion: IGFT框架能有效训练医疗对话AI进行多轮问诊，无需人工对话数据，通过信息增益奖励学习到临床相关、患者参与度高、具体明确的提问策略。
...
### [192] [When Personalization Legitimizes Risks: Uncovering Safety Vulnerabilities in Personalized Dialogue Agents](https://arxiv.org/abs/2601.17887)
*Jiahe Guo,Xiangran Guo,Yulin Hu,Zimo Long,Xingyu Sui,Xuda Zhi,Yongbo Huang,Hao He,Weixiang Zhao,Yanyan Zhao,Bing Qin*

Main category: cs.AI

TL;DR: 个性化LLM代理中的长期记忆可能导致"意图合法化"安全漏洞，即良性个人记忆会偏向意图推断，使模型将有害查询合法化。


<details>
  <summary>Details</summary>
Motivation: 现有个性化代理研究主要关注实用性和用户体验，将记忆视为中性组件，忽视了其安全影响。本文旨在揭示"意图合法化"这一未被充分探索的安全漏洞。

Method: 引入PS-Bench基准来识别和量化个性化交互中的意图合法化；在多个记忆增强代理框架和基础LLM上进行实验；从内部表示空间提供机制证据；提出轻量级检测-反思方法。

Result: 个性化使攻击成功率相对无状态基线增加15.8%-243.7%；从内部表示空间获得了意图合法化的机制证据；提出的检测-反思方法有效减少了安全退化。

Conclusion: 这是对意图合法化作为自然产生于良性、真实世界个性化的安全故障模式的首次系统探索和评估，强调了在长期个性化背景下评估安全性的重要性。
...
### [193] [UniCog: Uncovering Cognitive Abilities of LLMs through Latent Mind Space Analysis](https://arxiv.org/abs/2601.17897)
*Jiayu Liu,Yinhe Long,Zhenya Huang,Enhong Chen*

Main category: cs.AI

TL;DR: UniCog框架通过潜在思维空间分析LLM认知，发现帕累托原则：共享推理核心+能力特定特征，利用潜在激活异常检测推理失败，并提出候选优先策略提升推理性能7.5%


<details>
  <summary>Details</summary>
Motivation: 现有可解释性方法在解释LLM推理过程中认知能力如何参与方面存在局限，LLM的认知过程与人类有根本差异，需要新的分析框架

Method: 提出UniCog统一框架，作为潜在变量模型，将密集模型激活编码为稀疏、解耦的潜在维度，分析六个先进LLM的认知结构

Result: 发现LLM认知的帕累托原则：共享推理核心辅以能力特定特征；推理失败常表现为潜在激活强度异常；潜在信息候选优先策略在挑战性基准上提升推理性能达7.5%

Conclusion: UniCog为LLM分析开辟了新范式，提供了基于认知的推理动态视图，通过潜在激活分析可改进模型推理性能
...
### [194] [Think Locally, Explain Globally: Graph-Guided LLM Investigations via Local Reasoning and Belief Propagation](https://arxiv.org/abs/2601.17915)
...
TL;DR: EoG框架通过将调查任务分解为图上的溯因推理，使用LLM进行局部证据挖掘和标注，由确定性控制器管理图遍历和信念传播，显著提升了开放调查任务的准确性和一致性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在处理开放调查任务时存在局限性：上下文窗口有限导致关键证据可能被丢弃；ReAct式智能体在检索-总结-推理循环中结论对探索顺序敏感，存在运行不一致性；语义推理与控制器职责耦合导致执行错误影响推理质量。

Method: 提出EoG框架：将调查任务形式化为依赖图上的溯因推理。LLM负责有界的局部证据挖掘和标注（区分原因与症状），确定性控制器管理图遍历、状态维护和信念传播，计算最小解释边界。

Result: 在ITBench诊断任务上，EoG相比ReAct基线在准确性和运行一致性上均有提升，其中Majority-at-k实体F1平均提升7倍。

Conclusion: 通过解耦证据挖掘与推理控制，EoG框架能够更可靠地处理开放调查任务，解决了ReAct式智能体在复杂依赖结构环境中的脆弱性问题。
...
### [195] [Agentic AI for Self-Driving Laboratories in Soft Matter: Taxonomy, Benchmarks,and Open Challenges](https://arxiv.org/abs/2601.17920)
*Xuanzhou Chen,Audrey Wang,Stanley Yin,Hanyang Jiang,Dong Zhang*

Main category: cs.AI

TL;DR: 这篇综述论文将自驱动实验室(SDLs)视为智能体与环境交互问题，回顾了闭环实验的主要方法家族，提出了基于能力的分类体系，并制定了基准任务模板和评估指标，最后总结了实际部署经验与开放挑战。


<details>
  <summary>Details</summary>
Motivation: 自驱动实验室为在昂贵操作、噪声延迟反馈、严格可行性安全约束和非平稳性条件下的智能体AI提供了重要测试平台。论文旨在连接SDL流程与已建立的AI原则，为这一新兴领域提供系统化框架。

Method: 将SDL自主性框架化为智能体环境交互问题，回顾了贝叶斯优化、主动学习、规划与强化学习、工具使用智能体等方法家族，提出了基于决策视野、不确定性建模、动作参数化、约束处理等维度的能力驱动分类法。

Result: 建立了SDL的统一框架，提出了系统化分类体系，制定了基准任务模板和评估指标（成本感知性能、漂移鲁棒性、约束违反行为、可重复性），总结了实际部署经验。

Conclusion: SDL为AI研究提供了丰富挑战，需要进一步发展多模态表示、校准不确定性、安全探索和共享基准基础设施。可验证和溯源感知策略对调试、可重复性和安全操作至关重要。
...
### [196] [Learning Transferable Skills in Action RPGs via Directed Skill Graphs and Selective Adaptation](https://arxiv.org/abs/2601.17923)
*Ali Najar*

Main category: cs.AI

TL;DR: 提出基于技能图的分层课程学习框架，在复杂实时控制环境（黑暗之魂III）中实现终身学习，通过选择性微调适应环境变化。


<details>
  <summary>Details</summary>
Motivation: 终身学习智能体需要在不从头训练或覆盖已学行为的情况下持续扩展能力，这在复杂实时控制环境中尤其具有挑战性。

Method: 将战斗表示为有向技能图，采用分层课程训练五个可重用技能：相机控制、目标锁定、移动、闪避和治疗-攻击决策策略，每个技能针对特定职责优化。

Result: 技能分解提高了样本效率，当环境从第一阶段切换到第二阶段时，仅需微调两个技能就能在有限交互预算内快速恢复性能。
...
### [197] [LLM-Based SQL Generation: Prompting, Self-Refinement, and Adaptive Weighted Majority Voting](https://arxiv.org/abs/2601.17942)
*Yu-Jie Yang,Hung-Fu Chang,Po-An Chen*

Main category: cs.AI

TL;DR: 提出了SSEV和ReCAPAgent-SQL两个框架，通过自优化和智能体协作提升Text-to-SQL性能，在多个基准测试中取得竞争性结果，支持实际企业部署。


<details>
  <summary>Details</summary>
Motivation: Text-to-SQL技术能降低数据分析门槛，但自然语言查询的歧义性、模式链接复杂性、SQL方言泛化限制以及领域特定理解需求使得生成准确SQL仍然具有挑战性。

Method: 1. SSEV：基于PET-SQL的单智能体自优化管道，集成加权多数投票及其随机变体，无需真实数据；2. ReCAPAgent-SQL：多智能体协作框架，包含规划、外部知识检索、批判、动作生成、自优化、模式链接和结果验证等专门智能体。
...
Conclusion: 该工作促进了可扩展Text-to-SQL系统在实际环境中的部署，以更低成本和更高效率支持数据驱动决策，通过智能体协作和自优化机制有效应对复杂企业数据库需求。
...
### [198] [Sentipolis: Emotion-Aware Agents for Social Simulations](https://arxiv.org/abs/2601.18027)
*Chiyuan Fu,Lyuhao Chen,Yunze Xiao,Weihao Xuan,Carlos Busso,Mona Diab*

Main category: cs.AI

TL;DR: Sentipolis框架为LLM智能体提供情感状态记忆，通过PAD情感表示、双速情感动态和情感-记忆耦合解决情感遗忘问题，提升社交模拟中的情感连续性和真实行为。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体在社交模拟中常将情感视为瞬时线索，导致情感遗忘和长期连续性不足，需要建立具有情感状态记忆的智能体框架。

Method: 提出Sentipolis框架，包含：1) 连续PAD（愉悦-唤醒-支配）情感表示；2) 双速情感动态机制；3) 情感与记忆耦合系统。

Result: 在数千次交互中，Sentipolis显著提升情感基础行为、沟通能力和情感连续性。效果模型依赖：高容量模型可信度提升，小模型可能下降；情感意识可能轻微降低社会规范遵守度。

Conclusion: Sentipolis成功实现情感状态智能体，支持研究累积社交动态如联盟形成和关系渐变，反映了情感驱动行为与规则遵守之间的人类式张力。
...
### [199] [Expert Evaluation and the Limits of Human Feedback in Mental Health AI Safety Testing](https://arxiv.org/abs/2601.18061)
...
TL;DR: 专家在AI心理健康回复评估中存在系统性分歧，而非随机误差，特别是在自杀自伤等安全关键问题上，聚合标签会抹杀专业哲学差异


<details>
  <summary>Details</summary>
Motivation: 验证人类反馈学习(LHF)的基本假设：专家判断经过适当聚合后能提供有效的训练和评估基础。在心理健康这一高风险领域，专家共识尤为重要，但这一假设尚未得到充分验证。

Method: 三位认证精神科医生使用校准的评分标准独立评估LLM生成的回复。通过计算组内相关系数(ICC)和Krippendorff's α等指标衡量评分者间信度，并进行定性访谈了解分歧原因。

Result: 评分者间信度极低(ICC 0.087-0.295)，低于可接受阈值。自杀自伤类回复分歧最大，且是系统性而非随机分歧。一个因子的Krippendorff's α为负值(-0.203)，表明分歧比随机更差。定性分析显示分歧源于不同的临床框架：安全优先、参与为中心和文化敏感导向。

Conclusion: 专家分歧反映了不同的专业哲学而非测量误差，聚合标签会抹杀这些差异。在安全关键AI评估中，应放弃基于共识的聚合方法，转向能够保留和学习专家分歧的对齐方法。
...
### [200] [EvolVE: Evolutionary Search for LLM-based Verilog Generation and Optimization](https://arxiv.org/abs/2601.18067)
*Wei-Po Hsin,Ren-Hao Deng,Yao-Ting Hsieh,En-Ming Huang,Shih-Hao Hung*

Main category: cs.AI

TL;DR: EvolVE框架通过结合蒙特卡洛树搜索和引导式优化等进化策略，结合结构化测试平台生成，显著提升了硬件设计的自动化水平，在多个基准测试中达到SOTA，并在工业级优化问题上大幅降低PPA指标。


<details>
  <summary>Details</summary>
Motivation: Verilog硬件设计流程劳动密集且需要深厚领域知识，现有大语言模型由于训练数据有限和顺序推理特性，难以捕捉硬件系统的严格形式逻辑和并发特性，需要新的自动化解决方案。

Method: 提出EvolVE框架，分析多种进化策略：蒙特卡洛树搜索用于最大化功能正确性，引导式优化用于设计优化；结合结构化测试平台生成加速进化过程；创建IC-RTL工业级基准测试套件。

Result: 在VerilogEval v2上达到98.1%，RTLLM v2上达到92%；在工业级IC-RTL套件上超越竞赛参考实现，哈夫曼编码PPA降低66%，所有问题几何平均PPA降低17%。

Conclusion: EvolVE通过进化策略和结构化测试平台生成，成功解决了硬件设计自动化的关键挑战，在功能正确性和设计优化方面均达到新的最先进水平，为工业级硬件设计提供了有效自动化方案。
...
### [201] [Beyond Text-to-SQL: Can LLMs Really Debug Enterprise ETL SQL?](https://arxiv.org/abs/2601.18119)
*Jing Ye,Yiwen Duan,Yonghong Yu,Victor Ma,Yang Gao,Xing Chen*

Main category: cs.AI

TL;DR: OurBench是首个企业级SQL推理与调试基准，包含469个语法错误查询和516个语义错误查询，通过自动化构建工作流注入真实错误，评估显示当前LLMs在SQL调试上表现不佳（最佳模型准确率仅36%）。


<details>
  <summary>Details</summary>
Motivation: 企业数据工程中SQL至关重要，但即使有经验的开发者和先进的text-to-SQL LLMs也难以一次性生成完全正确的SQL代码，通常需要多次调试迭代。目前缺乏专门针对企业级SQL调试的基准测试。

Method: 1. 自动化构建工作流：使用逆向工程方法，在大规模SQL代码中系统性地注入真实错误，实现可扩展且多样化的基准生成。2. 免执行评估框架：针对企业环境设计，提供快速、准确且资源高效的评估方法。

Result: OurBench包含469个OurBenchSyn查询（语法错误+显式错误消息）和516个OurBenchSem查询（语义错误）。查询复杂度高，平均超过140行，具有深而广的抽象语法树。评估近30个LLMs显示性能差距显著：最佳模型Claude-4-Sonnet在OurBenchSyn上仅36.46%准确率，OurBenchSem上32.17%，大多数模型低于20%。

Conclusion: 企业级SQL调试对当前LLMs仍具挑战性，性能差距明显。研究探索了四种解决方案策略，识别了关键挑战，并为企业SQL调试与LLMs的结合指出了有前景的方向。
...
### [202] [Deadline-Aware, Energy-Efficient Control of Domestic Immersion Hot Water Heaters](https://arxiv.org/abs/2601.18123)
*Muhammad Ibrahim Khan,Bivin Pradeep,James Brusey*

Main category: cs.AI

TL;DR: 论文研究家庭浸入式热水器的截止时间感知控制，通过强化学习（PPO）在指定时间达到目标温度的同时最小化能耗，相比传统方法节省26-69%能源。


<details>
  <summary>Details</summary>
Motivation: 传统家庭浸入式热水器在冬季通常连续运行，加热速度快但效率低，忽略了可预测的需求窗口和环境热损失。需要一种能在指定时间达到目标温度同时最小化能耗的智能控制方法。

Method: 建立Gymnasium环境模拟浸入式热水器的一阶热损失模型，采用离散开关控制（0W和6000W，每120秒动作）。比较三种方法：时间最优的bang-bang基线控制、零样本蒙特卡洛树搜索规划器、以及近端策略优化强化学习策略。

Result: 在60步（2小时）时间范围内，PPO能耗最低（3.23千瓦时），相比bang-bang控制（4.37-10.45千瓦时）和MCTS（4.18-6.46千瓦时）显著节省能源。在典型场景中，PPO比bang-bang节省54%能耗，比MCTS节省33%。

Conclusion: 学习型截止时间感知控制能在相同物理条件下显著降低能耗，规划器无需训练即可提供部分节能效果，而学习策略一旦训练完成推理成本几乎为零，为智能热水器控制提供了有效解决方案。
...
### [203] [RouteMoA: Dynamic Routing without Pre-Inference Boosts Efficient Mixture-of-Agents](https://arxiv.org/abs/2601.18130)
...
TL;DR: RouteMoA：一种通过动态路由实现高效混合代理的框架，使用轻量级评分器进行初步筛选，混合评委进行后验修正，在保持性能的同时显著降低成本和延迟


<details>
  <summary>Details</summary>
Motivation: 现有混合代理方法采用密集拓扑结构，导致成本和延迟过高；使用LLM评委筛选响应仍需要所有模型先进行推理，无法有效降低成本；缺乏模型选择标准，在大规模模型池中面临成本过高和上下文限制问题

Method: 1. 轻量级评分器：仅基于查询预测粗略性能，筛选出高潜力候选子集，无需推理；2. 混合评委：通过轻量级自评估和交叉评估对现有模型输出进行后验修正；3. 模型排名机制：平衡性能、成本和延迟进行模型选择

Result: RouteMoA在不同任务和模型池规模下均优于传统MoA，在大规模模型池中降低成本89.8%，减少延迟63.6%

Conclusion: RouteMoA通过动态路由机制有效解决了混合代理框架的成本和延迟问题，实现了高效的模型协作，为大规模模型池应用提供了实用解决方案
...
### [204] [RareAlert: Aligning heterogeneous large language model reasoning for early rare disease risk screening](https://arxiv.org/abs/2601.18132)
...
TL;DR: RareAlert是一个基于LLM推理校准的罕见病早期筛查系统，通过整合10个LLM的推理信号，训练出可在本地部署的Qwen3-4B模型，在罕见病识别任务上达到0.917 AUC，优于现有最佳机器学习集成和所有评估的LLM。


<details>
  <summary>Details</summary>
Motivation: 罕见病的漏诊和延迟诊断是重大挑战，现有初级诊疗分诊流程无法可靠识别罕见病患者，需要通用筛查来减少诊断延迟。医生在初次临床接触时只能基于有限信息在高不确定性下评估罕见病风险。

Method: 开发RareAlert系统：整合10个LLM生成的推理信号，使用机器学习校准和加权这些信号，然后将对齐的推理蒸馏到单个本地可部署模型（基于Qwen3-4B）。构建RareBench数据集（158,666个病例，覆盖33个Orphanet疾病类别和7,000多种罕见病）。

Result: 在独立测试集上，RareAlert达到0.917 AUC，优于最佳机器学习集成和所有评估的LLM（包括GPT-5、DeepSeek-R1、Claude-3.7-Sonnet等）。证明LLM医学推理的多样性，以及在高不确定性临床任务中对齐推理的有效性。

Conclusion: 罕见病识别可重新概念化为应用于普通患者群体的通用不确定性解决过程。RareAlert通过将校准推理整合到单个模型中，实现了准确、隐私保护、可扩展的罕见病风险筛查，适合大规模本地部署。
...
### [205] [DeepPlanning: Benchmarking Long-Horizon Agentic Planning with Verifiable Constraints](https://arxiv.org/abs/2601.18137)
*Yinger Zhang,Shutong Jiang,Renhao Li,Jianhong Tu,Yang Su,Lianghao Deng,Xudong Guo,Chenxu Lv,Junyang Lin*

Main category: cs.AI

TL;DR: DeepPlanning是一个挑战性的长程智能体规划基准，包含多日旅行规划和多产品购物任务，需要主动信息获取、局部约束推理和全局约束优化，前沿智能体LLM在此基准上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有智能体评估基准过于强调局部步级推理，而缺乏对全局约束优化（如时间和财务预算）的关注；同时现有LLM规划基准未能充分体现现实场景中的主动信息收集和细粒度局部约束。

Method: 提出了DeepPlanning基准，包含多日旅行规划和多产品购物两类任务，这些任务需要智能体进行主动信息获取、局部约束推理和全局约束优化，并开源了代码和数据。

Result: 评估显示即使前沿的智能体LLM在这些问题上也表现困难，突显了可靠的显式推理模式和并行工具使用对于实现更好的效果-效率权衡的重要性。

Conclusion: DeepPlanning基准揭示了当前智能体LLM在长程规划方面的不足，错误分析指出了改进方向，开源资源将支持未来研究。
...
### [206] [Success Conditioning as Policy Improvement: The Optimization Problem Solved by Imitating Success](https://arxiv.org/abs/2601.18175)
*Daniel Russo*

Main category: cs.AI

TL;DR: 成功条件化（success conditioning）通过收集轨迹、识别成功轨迹并模仿其动作来改进策略，本文证明该方法精确解决了信任域优化问题，建立了策略改进、策略变化幅度和动作影响力之间的等式关系。


<details>
  <summary>Details</summary>
Motivation: 成功条件化（如拒绝采样+SFT、目标条件RL、决策变换器）被广泛用于策略改进，但其背后的优化问题本质一直不明确。本文旨在揭示该方法解决的数学优化问题及其理论性质。

Method: 通过理论分析证明成功条件化精确解决了信任域优化问题：在χ²散度约束下最大化策略改进，约束半径由数据自动确定。建立了相对策略改进、策略变化幅度和动作影响力之间的等式关系。

Result: 成功条件化是保守的改进算子，不会降低性能或引发危险的分布偏移。当失败时，它会通过几乎不改变策略来可观察地失败。应用于回报阈值化时，可以放大改进但可能偏离真实目标。

Conclusion: 成功条件化有坚实的优化理论基础，是一种安全可靠的策略改进方法。它解决了明确的信任域优化问题，建立了策略改进的精确数学关系，为相关实践提供了理论支撑。
...
### [207] [GAIA: A Data Flywheel System for Training GUI Test-Time Scaling Critic Models](https://arxiv.org/abs/2601.18197)
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然提升了GUI代理的能力，但操作不可逆性导致单个错误动作可能引发灾难性偏差，需要解决这一关键挑战

Method: 提出GAIA训练框架：1) 使用基础代理的正负动作样本训练直觉批评模型；2) 批评模型评估代理动作的正确性，选择高成功率操作；3) 批评模型指导代理收集精炼样本，启动自我改进循环；4) 增强数据训练第二轮批评模型

Result: 实验表明ICM能提升闭源和开源模型的测试时性能，且随着数据循环利用性能逐步提高
...
### [208] [SAGE: Steerable Agentic Data Generation for Deep Search with Execution Feedback](https://arxiv.org/abs/2601.18202)
...
TL;DR: SAGE是一个自动生成高质量、难度可控的深度搜索问答对的智能管道，通过生成器和搜索代理的交互迭代优化，显著提升深度搜索代理的性能。


<details>
  <summary>Details</summary>
Motivation: 深度搜索代理需要跨多个文档进行复杂推理，但人工标注此类问答对成本极高，因为探索轨迹长且复杂。需要自动化方法来生成高质量的训练数据。

Method: 提出SAGE管道，包含数据生成器（提出QA对）和搜索代理（尝试解决问题并提供执行反馈）。两个组件通过多轮交互迭代优化问答对，直到达到目标难度水平。

Result: 内在评估显示SAGE生成的问题需要多样推理策略，显著提高生成数据的正确性和难度。外在评估显示在流行深度搜索基准上获得高达23%的相对性能提升。额外实验表明，用该数据训练的代理可以在推理时从固定语料库检索适应到Google搜索，无需额外训练。
...
### [209] [Paying Less Generalization Tax: A Cross-Domain Generalization Study of RL Training for LLM Agents](https://arxiv.org/abs/2601.18217)
*Zhihan Liu,Lin Guan,Yixin Nie,Kai Zhang,Zhuoqun Hao,Lin Chen,Asli Celikyilmaz,Zhaoran Wang,Na Zhang*

Main category: cs.AI

TL;DR: 研究探索LLM智能体后训练中的跨域泛化问题，发现状态信息丰富度和规划复杂度是影响泛化的关键因素，并提出通过添加干扰特征增强状态信息丰富度来提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: LLM智能体通常在特定环境后训练，但部署时会遇到未见领域。研究旨在探索当测试领域未知时，哪些环境和建模因素对跨域泛化影响最大。

Method: 1) 识别影响跨域泛化的关键环境轴：状态信息丰富度和规划复杂度；2) 提出随机化技术：添加少量与目标无关的干扰特征来增强状态信息丰富度；3) 分析建模选择：SFT预热/中期训练和逐步思考对泛化的影响。

Result: 发现状态信息丰富度和规划复杂度与跨域泛化强相关，而领域真实性和文本相似性不是主要因素。增加状态信息丰富度能有效提升跨域鲁棒性。SFT预热有助于防止灾难性遗忘但会损害未包含领域的泛化，逐步思考在保持泛化方面起关键作用。

Conclusion: 跨域泛化主要受状态信息丰富度和规划复杂度影响，而非领域相似性。通过简单添加干扰特征增强状态信息丰富度是提升泛化的有效方法。建模选择需要在防止遗忘和保持泛化之间权衡。
...
### [210] [ShopSimulator: Evaluating and Exploring RL-Driven LLM Agent for Shopping Assistants](https://arxiv.org/abs/2601.18225)
...
TL;DR: ShopSimulator是一个大规模中文购物仿真环境，用于评估和训练LLM购物代理，发现现有模型成功率低于40%，通过SFT+RL训练可显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏统一的仿真环境来全面评估LLM购物代理在个性化偏好理解、多轮对话、产品检索和相似产品区分等方面的能力，且只关注评估而缺乏训练支持

Method: 构建ShopSimulator大规模中文购物环境，评估LLM在不同场景下的表现，进行错误分析，并探索监督微调(SFT)和强化学习(RL)的训练方法

Result: 即使最佳模型的全成功率也低于40%，代理在长轨迹中的深度搜索和产品选择、平衡个性化线索使用、有效用户交互方面存在困难；SFT+RL组合训练能显著提升性能

Conclusion: ShopSimulator为LLM购物代理提供了全面的评估和训练环境，揭示了现有模型的局限性，并展示了通过适当训练方法可以显著改进性能
...
### [211] [Yunjue Agent Tech Report: A Fully Reproducible, Zero-Start In-Situ Self-Evolving Agent System for Open-Ended Tasks](https://arxiv.org/abs/2601.18226)
*Haotian Li,Shijun Yang,Weizhen Qi,Silei Zhao,Rui Hua,Mingzhu Song,Xiaojian Yang,Chao Peng*

Main category: cs.AI

TL;DR: 论文提出了一种原位自进化范式，让智能体在开放环境中通过任务交互持续进化工具集，无需真实标签监督，显著提升零起点性能并实现跨领域知识迁移。
...
Method: 提出原位自进化范式，将序列任务交互视为连续经验流，通过执行反馈蒸馏出长期可重用能力；以工具进化为核心能力扩展路径，开发Yunjue Agent系统迭代合成、优化和重用工具；引入并行批量进化策略优化进化效率。

Result: 在五个多样化基准测试的零起点设置下，相比专有基线取得显著性能提升；补充的热启动评估证实积累的通用知识可无缝迁移到新领域；提出监测进化收敛的新指标。

Conclusion: 原位自进化范式为构建弹性、自进化智能体提供了有效框架，工具进化作为可验证的二元反馈信号是实现能力扩展的关键路径，开源代码库、系统轨迹和进化工具促进未来研究。
...
### [212] [Think-Augmented Function Calling: Improving LLM Parameter Accuracy Through Embedded Reasoning](https://arxiv.org/abs/2601.18282)
*Lei Wei,Jinpeng Ou,Xiao Peng,Bin Wang*

Main category: cs.AI

TL;DR: 提出Think-Augmented Function Calling (TAFC)框架，通过函数和参数级别的显式推理增强大语言模型的函数调用准确性，无需修改模型架构即可提升复杂参数生成的质量和可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在函数调用方面缺乏参数生成的显式推理透明度，特别是对于具有相互依赖参数的复杂函数。现有方法如思维链提示主要在代理层面操作，无法为单个函数参数提供细粒度的推理指导。

Method: 提出TAFC框架，引入通用的"think"参数增强机制，让模型能够阐述决策过程；通过动态优化参数描述提升推理质量；基于复杂度评分自动触发细粒度推理；提出推理引导优化以对齐人类期望。

Result: 在ToolBench上对专有和开源模型的评估显示，TAFC在多参数函数的参数生成准确性和推理连贯性方面有显著提升，同时为调试AI代理行为提供了增强的可解释性。

Conclusion: TAFC框架通过函数和参数级别的显式推理有效提升了大语言模型函数调用的准确性和可解释性，无需修改现有模型架构且保持完整的API兼容性，为解决复杂函数调用中的推理透明度问题提供了有效方案。
...
### [213] [A Generative AI-Driven Reliability Layer for Action-Oriented Disaster Resilience](https://arxiv.org/abs/2601.18308)
*Geunsik Lim*

Main category: cs.AI

TL;DR: Climate RADAR 是一个基于生成式AI的可靠性层，将灾害预警从警报传递转变为行动执行，通过个性化推荐提高防护行动执行率


<details>
  <summary>Details</summary>
Motivation: 传统预警系统虽然能快速传播警报，但往往无法触发及时的防护行动，导致可预防的损失和不公平现象。需要将灾害通信从"警报传递"转变为"行动执行"

Method: 整合气象、水文、脆弱性和社会数据形成综合风险指数，使用带有护栏的大型语言模型为公民、志愿者和市政接口提供个性化推荐

Result: 通过模拟、用户研究和市政试点评估显示，系统提高了防护行动执行率、降低了响应延迟、增强了可用性和信任度

Conclusion: Climate RADAR 结合预测分析、行为科学和负责任AI，推进以人为本、透明和公平的预警系统，为合规性灾害韧性基础设施提供实用路径
...
### [214] [Can Good Writing Be Generative? Expert-Level AI Writing Emerges through Fine-Tuning on High-Quality Books](https://arxiv.org/abs/2601.18353)
*Tuhin Chakrabarty,Paramveer S. Dhillon*

Main category: cs.AI

TL;DR: 研究通过实验发现，经过微调的AI写作在专家评审中胜过人类作家，引发专家作家的身份危机，挑战了AI创意局限的传统观点


<details>
  <summary>Details</summary>
Motivation: 挑战"创意写作是独特人类能力"的传统假设，探究生成式AI在模仿作家风格方面的能力，及其对创意劳动未来的影响

Method: 行为实验：28位MFA作家与3个LLM竞争模仿50位知名作家风格；采用盲审对比，由28位专家评委和131位普通评委进行评价；比较上下文提示与微调两种条件下的表现

Result: 上下文提示条件下，专家评委82.7%偏好人类写作；但经过微调后，62%偏好AI写作；普通评委则始终偏好AI写作；专家作家对AI写作的偏好引发身份危机和审美自信动摇

Conclusion: AI在创意写作方面的能力挑战了传统认知，引发关于"优秀写作"本质的质疑，对创意劳动的未来提出根本性问题
...
### [215] [AI Agent for Reverse-Engineering Legacy Finite-Difference Code and Translating to Devito](https://arxiv.org/abs/2601.18381)
*Yinghan Hou,Zongyou Yang*

Main category: cs.AI

TL;DR: 开发了一个集成AI代理框架，用于将传统有限差分实现转换为Devito环境，结合RAG和开源大语言模型，通过多阶段迭代工作流实现代码转换和优化。


<details>
  <summary>Details</summary>
Motivation: 为了促进将传统的有限差分实现转换为Devito环境，需要解决代码迁移的复杂性问题，特别是对于地震波模拟、计算流体动力学等领域的遗留Fortran代码。

Method: 采用混合LangGraph架构，结合RAG和开源大语言模型，通过多阶段迭代工作流：1)构建Devito知识图谱；2)反向工程组件分析Fortran源代码；3)多阶段检索管道进行并行搜索和语义分析；4)Pydantic约束的代码合成；5)集成验证框架。

Result: 开发了一个完整的AI代理框架，能够实现从传统有限差分代码到Devito环境的自动化转换，支持质量导向的迭代优化和状态感知的动态路由，提高了代码迁移的准确性和效率。

Conclusion: 该研究的主要贡献在于引入了受强化学习启发的反馈机制，使代码转换从静态翻译转向动态自适应分析行为，为科学计算代码迁移提供了创新的AI驱动解决方案。
...
### [216] [Dynamic Thinking-Token Selection for Efficient Reasoning in Large Reasoning Models](https://arxiv.org/abs/2601.18383)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型生成推理轨迹时会产生大量内存占用和计算开销，但研究发现只有部分决策关键token真正影响最终答案，其余token贡献可忽略

Method: 提出动态思维token选择方法，利用注意力图分析推理轨迹的影响，识别决策关键token，在推理时仅保留这些关键token的KV缓存状态，剔除冗余条目

Result: 通过选择性保留关键token的KV缓存，显著降低了内存占用和计算开销，优化了大型推理模型的推理效率

Conclusion: 推理轨迹中只有部分token对最终答案决策至关重要，DynTS方法通过动态选择这些关键token并优化KV缓存管理，有效提升了大型推理模型的效率
...
### [217] [OffSeeker: Online Reinforcement Learning Is Not All You Need for Deep Research Agents](https://arxiv.org/abs/2601.18467)
*Yuhang Zhou,Kai Zheng,Qiguang Chen,Mengkang Hu,Qingfeng Sun,Can Xu,Jingjing Chen*

Main category: cs.AI

TL;DR: 该论文提出了一种完全离线的研究智能体训练方法，通过开源工具套件生成大规模研究数据，训练出性能与在线强化学习相当但成本更低的模型。


<details>
  <summary>Details</summary>
Motivation: 当前深度研究智能体虽然表现出色，但依赖昂贵的在线强化学习（需要大量API调用）。离线训练虽然更高效，但缺乏高质量研究轨迹数据。本文旨在证明不需要昂贵的在线强化学习也能构建强大的研究智能体。

Method: 提出了一个完全开源的工具套件，包括：1) DeepForge - 无需大量预处理的即用型任务合成框架，可生成大规模研究查询；2) 精心策划的数据集（66k QA对、33k SFT轨迹、21k DPO对）。利用这些资源完全离线训练了OffSeeker（8B参数模型）。

Result: 在六个基准测试上的广泛评估表明，OffSeeker不仅在同规模智能体中领先，而且与通过大量在线强化学习训练的30B参数系统保持竞争力。

Conclusion: 证明了昂贵的在线强化学习并非构建强大研究智能体的必要条件，通过高质量的离线训练数据和有效的训练框架，可以实现与在线方法相当的性能，同时大幅降低成本。
...
### [218] [AgentDoG: A Diagnostic Guardrail Framework for AI Agent Safety and Security](https://arxiv.org/abs/2601.18491)
*Dongrui Liu,Qihan Ren,Chen Qian,Shuai Shao,Yuejin Xie,Yu Li,Zhonghao Yang,Haoyu Luo,Peng Wang,Qingyu Liu,Binxin Hu,Ling Tang,Jilin Mei,Dadi Guo,Leitao Yuan,Junyao Yang,Guanxu Chen,Qihao Lin,Yi Yu,Bo Zhang,Jiaxuan Guo,Jie Zhang,Wenqi Shao,Huiqi Deng,Zhiheng Xi,Wenjie Wang,Wenxuan Wang,Wen Shen,Zhikai Chen,Haoyu Xie,Jialing Tao,Juntao Dai,Jiaming Ji,Zhongjie Ba,Linfeng Zhang,Yong Liu,Quanshi Zhang,Lei Zhu,Zhihua Wei,Hui Xue,Chaochao Lu,Jing Shao,Xia Hu*

Main category: cs.AI

TL;DR: 提出AgentDoG框架，通过三维风险分类法构建细粒度智能体安全基准ATBench，实现透明化风险诊断和监控，在复杂交互场景中达到SOTA性能。
...
Method: 提出统一的三维风险分类法（来源、失效模式、后果），基于此构建细粒度智能体安全基准ATBench，并开发AgentDoG诊断护栏框架，提供细粒度的上下文监控和根因诊断。

Result: AgentDoG在Qwen和Llama模型家族中提供三个规模版本（4B、7B、8B参数），在多样复杂的交互场景中实现了最先进的智能体安全调节性能。

Conclusion: AgentDoG通过结构化风险分类和透明化诊断，有效解决了智能体安全监控的挑战，为智能体对齐提供了超越二元标签的溯源和透明度支持。
...
### [219] [DEEPMED: Building a Medical DeepResearch Agent via Multi-hop Med-Search Data and Turn-Controlled Agentic Training & Inference](https://arxiv.org/abs/2601.18496)
*Zihan wang,Hao Wang,Shi Feng,Xiaocui Yang,Daling Wang,Yiqun Zhang,Jinghao Lin,Haihua Yang,Xiaozhong Ji*

Main category: cs.AI

TL;DR: DeepMed：针对医学推理的深度研究模型，通过多跳医学搜索QA合成、难度感知轮次惩罚和推理监控，解决通用DR模型在医学领域性能有限的问题。


<details>
  <summary>Details</summary>
Motivation: 医学推理模型受限于参数知识，易出现遗忘和幻觉。通用深度研究（DR）模型虽然能在通用领域基于工具证据进行推理，但直接迁移到医学领域效果有限，主要因为任务特性差异（需要临床上下文推理）和工具使用扩展问题（盲目扩展工具调用会引入噪声）。

Method: 1. 数据：采用多跳医学搜索QA合成方法，支持模型在医学上下文中应用DR范式；2. 训练：引入难度感知轮次惩罚，抑制过度工具调用增长；3. 推理：添加监控机制，在有限步骤内验证假设，避免上下文污染。

Result: 在七个医学基准测试中，DeepMed相比基础模型平均提升9.79%，并优于更大的医学推理和DR模型。

Conclusion: DeepMed通过专门针对医学领域设计的深度研究方法，解决了通用DR模型在医学推理中的局限性，显著提升了医学推理性能。
...
### [220] [Deconstructing Instruction-Following: A New Benchmark for Granular Evaluation of Large Language Model Instruction Compliance Abilities](https://arxiv.org/abs/2601.18554)
*Alberto Purpura,Li Wang,Sahil Badyal,Eugenio Beaufrand,Adam Faulkner*

Main category: cs.AI

TL;DR: MOSAIC框架通过动态生成包含多达20个应用导向约束的数据集，对LLM的指令遵循能力进行模块化评估，发现合规性并非单一能力，而是受约束类型、数量和位置影响。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试往往无法反映真实世界使用情况或无法将合规性与任务成功分离，因此需要一种能够可靠评估LLM遵循复杂指令能力的新方法。

Method: 提出MOSAIC框架，使用动态生成的数据集，包含多达20个应用导向的生成约束，对五个不同家族的LLM进行模块化评估，分析约束类型、数量和位置对合规性的影响。

Result: 评估显示合规性不是单一能力，而是随约束类型、数量和位置显著变化；揭示了模型特定弱点、指令间的协同和冲突交互，以及首因效应和近因效应等位置偏差。
...
### [221] [Stability as a Liability:Systematic Breakdown of Linguistic Structure in LLMs](https://arxiv.org/abs/2601.18588)
*Xianzhe Meng,Qiangsheng Zeng,Ling Luo,Qinghan Yang,Jiarui Hao,Wenbo Wu,Qinyu Wang,Rui Yin,Lin Qi,Renzhi Lu*

Main category: cs.AI

TL;DR: 训练稳定性与生成质量不一定对齐：稳定训练可能导致模型输出低熵、重复，尽管损失平滑收敛


<details>
  <summary>Details</summary>
Motivation: 研究训练稳定性如何影响大语言模型的生成分布，探索稳定训练是否必然带来良好的生成质量

Method: 理论分析最大似然训练下稳定参数轨迹的影响，并使用基于反馈的训练框架进行实证验证，观察内部生成统计的稳定性

Result: 稳定训练使模型近似最小化前向KL散度，但会隐式降低生成熵，导致模型集中在有限的模式上，产生重复性退化行为

Conclusion: 优化稳定性与生成表达能力并不天然对齐，稳定性本身不足以作为生成质量的指标
...
### [222] [A Balanced Neuro-Symbolic Approach for Commonsense Abductive Logic](https://arxiv.org/abs/2601.18595)
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在形式推理方面表现出色，但在需要复杂证明规划的问题上经常失效。传统逻辑求解器虽然推理效率高，但假设所有相关事实都已提供，无法处理缺失的常识关系。

Method: 提出一种新颖方法：使用逻辑求解器的反馈来迭代地增强逻辑问题，通过LLM提供常识关系。采用搜索程序探索潜在的常识假设，在保持成本可控的同时最大化找到有用事实的机会。

Result: 在多个纯逻辑推理数据集上（其中部分常识信息已被移除），该方法相比现有技术持续取得显著改进。
...
### [223] [PolySHAP: Extending KernelSHAP with Interaction-Informed Polynomial Regression](https://arxiv.org/abs/2601.18608)
*Fabian Fumagalli,R. Teal Witter,Christopher Musco*

Main category: cs.AI

TL;DR: PolySHAP扩展KernelSHAP，使用高阶多项式近似游戏以捕捉特征间的非线性交互，相比线性近似获得更好的Shapley值估计，并证明与配对采样方法等价。


<details>
  <summary>Details</summary>
Motivation: KernelSHAP通过线性近似计算Shapley值，但无法捕捉特征间的非线性交互。需要更精确的近似方法来提高Shapley值估计的准确性。

Method: 提出PolySHAP方法，使用高阶多项式（而非线性函数）来近似游戏，捕捉特征间的非线性交互。证明二阶PolySHAP与配对采样方法在数学上等价。

Result: PolySHAP在各种基准数据集上获得比KernelSHAP更好的Shapley值估计。证明估计具有一致性，并为配对采样方法的优异性能提供了首个理论依据。

Conclusion: PolySHAP通过高阶多项式近似改进了Shapley值估计，揭示了配对采样方法的理论基础，为可解释AI中的Shapley值计算提供了更精确和理论可靠的方法。
...
### [224] [Emergence of Phonemic, Syntactic, and Semantic Representations in Artificial Neural Networks](https://arxiv.org/abs/2601.18617)
*Pierre Orhan,Pablo Diego-Simón,Emmnanuel Chemla,Yair Lakretz,Yves Boubenec,Jean-Rémi King*

Main category: cs.AI

TL;DR: 人工神经网络在训练过程中会自发形成音素、词汇和句法表征，其学习阶段与儿童语言习得相似但数据需求高2-4个数量级


<details>
  <summary>Details</summary>
Motivation: 儿童语言习得过程（音素分类→词汇识别→句法组合）已有良好描述，但缺乏统一的计算框架来解释其背后的神经表征机制

Method: 研究语音和文本基础的人工神经网络在训练过程中，其神经激活是否以及何时出现音素、词汇和句法表征，分析激活空间的几何结构

Result: 神经网络遵循顺序学习阶段：训练过程中神经激活逐步构建子空间，其几何结构分别表征音素、词汇和句法结构。这种发展轨迹与儿童相似但数据需求高2-4个数量级

Conclusion: 研究展示了语言习得主要阶段自发出现的条件，为理解语言习得背后的计算机制提供了有前景的路径
...
### [225] [Assessing the Quality of Mental Health Support in LLM Responses through Multi-Attribute Human Evaluation](https://arxiv.org/abs/2601.18630)
...
TL;DR: 本文提出了一种基于人类专家的评估方法，用于评估LLM在心理健康对话中的表现，发现LLM在认知支持方面可靠，但在情感共鸣方面不稳定，存在认知-情感鸿沟。


<details>
  <summary>Details</summary>
Motivation: 全球心理健康危机日益严重，存在治疗缺口和合格治疗师短缺的问题，LLM作为可扩展的支持途径具有潜力，但其可靠性、治疗相关性和与人类标准的对齐仍面临挑战。

Method: 提出基于人类专家的评估方法，从真实场景数据集中整理500个心理健康对话，评估9个不同LLM（包括闭源和开源模型）的响应。由两位精神科专家使用5点李克特量表，基于包含6个属性的评估框架（涵盖认知支持和情感共鸣）独立评分。

Result: LLM在认知可靠性方面表现良好，能提供安全、连贯且临床适当的信息，但在情感对齐方面不稳定。闭源模型（如GPT-4o）提供更平衡的治疗响应，开源模型则表现出更大的变异性和情感平淡。揭示了持续的认知-情感鸿沟。

Conclusion: 需要建立具有失败意识、临床基础的评估框架，在心理健康导向的LLM中优先考虑关系敏感性而不仅仅是信息准确性。提倡以治疗敏感性为中心、包含人类参与的平衡评估协议，为心理健康对话AI的负责任设计和临床监督提供框架。
...
### [226] [AdaReasoner: Dynamic Tool Orchestration for Iterative Visual Reasoning](https://arxiv.org/abs/2601.18631)
*Mingyang Song,Haoyu Sun,Jiawei Gu,Linjie Li,Luxin Xu,Ranjay Krishna,Yu Cheng*

Main category: cs.AI

TL;DR: AdaReasoner是一个多模态模型家族，通过学习工具使用作为通用推理技能，而非特定工具或显式监督行为，实现了在视觉推理任务中的自适应工具调用和组合能力。
...
Method: 1）可扩展的数据策划流程，让模型接触长视野、多步骤的工具交互；2）Tool-GRPO强化学习算法，基于最终任务成功优化工具选择和序列；3）自适应学习机制，动态调节工具使用频率。这些组件使模型能够从任务上下文和中间结果推断工具效用，实现多工具协调和对未见工具的泛化。

Result: AdaReasoner表现出强大的工具自适应和泛化行为：自主采用有益工具、抑制无关工具、根据任务需求调整工具使用频率，尽管从未被显式训练这样做。这些能力转化为在挑战性基准测试中的最先进性能，将7B基础模型平均提升24.9%，并在多个任务（包括VSP和Jigsaw）上超越GPT-5等强大专有系统。

Conclusion: AdaReasoner通过学习工具使用作为通用推理技能，实现了多模态模型的自适应工具调用能力，能够在面对新工具和新任务时有效协调多工具使用，显著提升了视觉推理性能，超越了现有专有系统。
...
### [227] [FadeMem: Biologically-Inspired Forgetting for Efficient Agent Memory](https://arxiv.org/abs/2601.18642)
*Lei Wei,Xu Dong,Xiao Peng,Niantao Xie,Bin Wang*

Main category: cs.AI

TL;DR: FadeMem是一种受生物学启发的智能体记忆架构，通过主动遗忘机制模拟人类记忆的适应性衰减，实现记忆的高效管理


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型作为自主智能体部署时面临关键记忆限制：缺乏选择性遗忘机制，导致要么在上下文边界发生灾难性遗忘，要么在边界内信息过载。人类记忆通过适应性衰减过程自然平衡保留与遗忘，而现有AI系统采用二元保留策略（要么全部保留，要么全部丢失）

Method: FadeMem采用受生物学启发的智能体记忆架构，在双层记忆层次结构中实现差异化衰减率。保留机制由适应性指数衰减函数控制，该函数受语义相关性、访问频率和时间模式调制。通过LLM引导的冲突解决和智能记忆融合，系统能够整合相关信息同时让无关细节逐渐消失

Result: 在Multi-Session Chat、LoCoMo和LTI-Bench上的实验表明，FadeMem在实现45%存储减少的同时，展现出优越的多跳推理和检索能力，验证了生物学启发的遗忘机制在智能体记忆系统中的有效性

Conclusion: FadeMem通过模拟人类记忆的主动遗忘机制，有效解决了AI智能体记忆管理中的关键限制，实现了记忆效率与性能的平衡，为构建更高效的自主智能体系统提供了新思路
...
### [228] [TEA-Bench: A Systematic Benchmarking of Tool-enhanced Emotional Support Dialogue Agent](https://arxiv.org/abs/2601.18700)
*Xingyu Sui,Yanyan Zhao,Yulin Hu,Jiahe Guo,Weixiang Zhao,Bing Qin*

Main category: cs.AI

TL;DR: TEA-Bench：首个用于评估工具增强情感支持对话系统的交互式基准，通过工具使用减少幻觉并提升支持质量


<details>
  <summary>Details</summary>
Motivation: 现有情感支持对话系统主要关注文本情感表达，忽视了外部工具在提供事实基础和减少幻觉方面的作用，需要建立工具增强的评估基准

Method: 提出TEA-Bench基准，包含真实情感场景、MCP风格工具环境和过程级评估指标；创建TEA-Dialog数据集；在9个LLM上进行实验，分析工具增强效果

Result: 工具增强普遍提升情感支持质量并减少幻觉，但效果与模型能力相关：强模型能更选择性和有效地使用工具，弱模型获益有限；监督微调在分布内有效但泛化能力差

Conclusion: 工具使用对构建可靠的情感支持代理至关重要，TEA-Bench为评估工具增强系统提供了重要基准
...
### [229] [Health-SCORE: Towards Scalable Rubrics for Improving Health-LLMs](https://arxiv.org/abs/2601.18706)
...
Method: 提出Health-SCORE框架，通过自动化的方式减少量规开发成本，同时保持评估性能。该框架不仅能用于评估，还能作为结构化奖励信号指导强化学习，并可直接整合到提示中以通过上下文学习提高响应质量。

Result: 在开放式医疗任务中，Health-SCORE实现了与人工创建量规相当的评估质量，同时显著降低了开发工作量，使基于量规的评估和训练更具可扩展性。

Conclusion: Health-SCORE提供了一个实用的解决方案，解决了医疗领域量规开发成本高、难以扩展的问题，为基于量规的LLM评估和训练提供了更高效、更经济的途径。
...
### [230] [Conditioned Generative Modeling of Molecular Glues: A Realistic AI Approach for Synthesizable Drug-like Molecules](https://arxiv.org/abs/2601.18716)
...
Conclusion: 这种集成方法为设计针对神经退行性疾病的UPS靶向疗法提供了有前景的框架
...
### [231] [Why Keep Your Doubts to Yourself? Trading Visual Uncertainties in Multi-Agent Bandit Systems](https://arxiv.org/abs/2601.18735)
*Jusheng Zhang,Yijia Fan,Kaitong Cai,Jing Yang,Jiawei Yao,Jian Wang,Guanlong Qu,Ziliang Chen,Keze Wang*

Main category: cs.AI

TL;DR: Agora框架将多智能体协调重构为不确定性市场，通过将认知不确定性结构化并作为可交易资产，基于理性经济规则实现成本高效的智能体协作，在多个多模态基准上显著优于现有方法。
...
Method: Agora将认知不确定性（感知、语义、推理）形式化为结构化可交易资产，基于理性经济规则强制智能体进行盈利驱动交易。引入市场感知的经纪人（扩展Thompson Sampling）发起协作并引导系统达到成本高效均衡。
...
### [232] [TSRBench: A Comprehensive Multi-task Multi-modal Time Series Reasoning Benchmark for Generalist Models](https://arxiv.org/abs/2601.18744)
...
<details>
  <summary>Details</summary>
Motivation: 时间序列数据在现实场景中无处不在且至关重要，但现有通用模型基准测试缺乏时间序列维度，需要填补这一空白以评估模型解决实际问题的能力。

Method: 构建TSRBench基准测试，包含4125个来自14个领域的问题，分为感知、推理、预测和决策四个主要维度，涵盖15个评估基本推理能力的任务。

Result: 评估30多个领先的专有和开源LLM、VLM和TSLLM发现：1）缩放定律适用于感知和推理但预测失效；2）强推理能力不保证准确的上下文感知预测；3）当前多模态模型无法有效融合文本和视觉表示。

Conclusion: TSRBench提供了一个标准化评估平台，揭示了当前模型的挑战，为推进通用模型发展提供了有价值的见解。
...
### [233] [TelcoAI: Advancing 3GPP Technical Specification Search through Agentic Multi-Modal Retrieval-Augmented Generation](https://arxiv.org/abs/2601.16984)
*Rahul Ghosh,Chun-Hao Liu,Gaurav Rele,Vidya Sagar Ravipati,Hazar Aouad*

Main category: cs.LG

TL;DR: TelcoAI：针对3GPP技术规范的智能多模态RAG系统，通过代理式推理和多模态融合显著提升文档理解能力


<details>
  <summary>Details</summary>
Motivation: 3GPP技术规范具有复杂的层次结构、密集格式和多模态内容，现有方法难以处理复杂查询、视觉信息和文档依赖关系，需要专门解决方案

Method: 提出TelcoAI系统，采用章节感知分块、结构化查询规划、元数据引导检索以及文本与图表的多模态融合技术

Result: 在多个基准测试中达到87%召回率、83%声明召回率和92%忠实度，比现有最优方法提升16%

Conclusion: 代理式和多模态推理在技术文档理解中具有显著效果，为电信研究和工程提供了实用的解决方案
...
### [234] [Sparsity-Aware Low-Rank Representation for Efficient Fine-Tuning of Large Language Models](https://arxiv.org/abs/2601.16991)
*Longteng Zhang,Sen Wu,Shuai Hou,Zhengyu Qing,Zhuo Zheng,Danning Ke,Qihong Lin,Qiang Wang,Shaohuai Shi,Xiaowen Chu*

Main category: cs.LG

TL;DR: SALR是一种新的微调范式，将低秩适配与稀疏剪枝结合，在保持LoRA性能的同时实现50%稀疏度、2倍模型压缩和1.7倍推理加速。


<details>
  <summary>Details</summary>
Motivation: 大型预训练语言模型微调需要大量参数更新，在资源受限环境中难以部署。现有方法如LoRA虽然减少了可训练参数，但底层密集权重仍带来高存储和计算成本，而简单的幅度剪枝会损害LoRA性能。

Method: 提出SALR框架，在均方误差框架下统一低秩适配与稀疏剪枝：1) 静态剪枝冻结的基础权重以最小化剪枝误差界；2) 通过截断SVD低秩适配器恢复丢弃的残差信息；3) 融合多个低秩适配器为单个GEMM操作；4) 采用基于位图的编码和两阶段流水线解码+GEMM设计。

Result: 在多种LLM上实现50%稀疏度，在GSM8K和MMLU任务上性能与LoRA相当，模型大小减少2倍，推理速度提升最高达1.7倍。

Conclusion: SALR成功将低秩适配与稀疏剪枝统一，在保持性能的同时显著提升了硬件效率，为资源受限环境中的模型部署提供了有效解决方案。
...
### [235] [A Dataset of Dengue Hospitalizations in Brazil (1999 to 2021) with Weekly Disaggregation from Monthly Counts](https://arxiv.org/abs/2601.16994)
...
TL;DR: 该论文发布了一个巴西市级登革热住院时间序列数据集，将原始月度数据通过三次样条插值分解为周度分辨率，并包含多种解释变量，用于流行病学预测AI模型训练。


<details>
  <summary>Details</summary>
Motivation: 提高原始月度数据的时间粒度，以更有效地训练用于流行病学预测的AI模型，解决时间分辨率不足的问题。

Method: 采用三次样条插值协议将市级登革热住院月度时间序列分解为周度分辨率，并包含校正步骤以保持月度总数。使用圣保罗州的高分辨率参考数据集评估了三种策略：线性插值、抖动和三次样条。

Result: 三次样条插值在参考数据上表现出最高的贴合度，因此被采用来生成1999年至2021年的周度序列。数据集还包括人口密度、温室气体排放、贫困指数、温度、降水等多种解释变量。

Conclusion: 该数据集为多变量时间序列分析、环境健康研究和机器学习模型开发提供了高质量资源，特别适用于登革热爆发预测，并公开了详细的质量指标和使用建议。
...
### [236] [MathMixup: Boosting LLM Mathematical Reasoning with Difficulty-Controllable Data Synthesis and Curriculum Learning](https://arxiv.org/abs/2601.17006)
*Xuchen Li,Jing Chen,Xuzhao Li,Hao Liang,Xiaohuan Zhou,Taifeng Wang,Wentao Zhang*

Main category: cs.LG

TL;DR: MathMixup：一种通过混合和分解策略系统生成高质量、难度可控数学推理问题的新数据合成范式，配合课程学习策略显著提升LLM数学推理能力


<details>
  <summary>Details</summary>
Motivation: 现有数学推理数据合成方法存在多样性有限、难度控制不精确的问题，无法有效支持课程学习等高效训练范式，需要开发能系统控制问题难度的高质量数据生成方法

Method: 提出MathMixup数据合成范式，采用混合和分解策略系统生成难度可控的数学推理问题，结合自动自检和人工筛选确保语义清晰和难度梯度结构，构建MathMixupQA数据集并设计课程学习策略

Result: 微调后的Qwen2.5-7B在七个数学基准测试中平均得分52.6%，超越先前最先进方法，验证了MathMixup在提升LLM数学推理能力和推进数据中心课程学习方面的有效性

Conclusion: MathMixup及其课程学习策略能显著增强LLM的数学推理性能，为数据中心的课程学习提供了有效解决方案，具有广泛适用性
...
### [237] [Analysis of voice recordings features for Classification of Parkinson's Disease](https://arxiv.org/abs/2601.17007)
*Beatriz Pérez-Sánchez,Noelia Sánchez-Maroño,Miguel A. Díaz-Freire*

Main category: cs.LG

TL;DR: 该论文提出使用机器学习模型结合特征选择方法来诊断帕金森病，通过分析患者语音记录，发现神经网络等模型适合分类任务，且特征数量可大幅减少而不影响性能。


<details>
  <summary>Details</summary>
Motivation: 帕金森病早期诊断困难，因为早期运动症状非常轻微。虽然患者语音记录可用于辅助诊断，但临床分析成本高，且语音特征众多，不清楚哪些特征对诊断真正相关。

Method: 使用不同类型的机器学习模型（特别是神经网络）结合特征选择方法，通过确定哪些特征对问题提供最多信息来减少分类器使用的特征数量。

Result: 机器学习方法（特别是神经网络）适合帕金森病分类，且特征数量可以显著减少而不影响模型性能。

Conclusion: 机器学习结合特征选择方法可以有效利用患者语音记录进行帕金森病早期诊断，既能保持诊断准确性，又能降低计算复杂度和临床分析成本。
...
### [238] [Bayesian Robust Financial Trading with Adversarial Synthetic Market Data](https://arxiv.org/abs/2601.17008)
*Haochong Xia,Simin Li,Ruixiao Xu,Zhixia Zhang,Hongxiang Wang,Zhiqian Liu,Teng Yao Long,Molei Qin,Chuqiao Zong,Bo An*

Main category: cs.LG

TL;DR: 提出贝叶斯鲁棒框架，结合宏观条件生成对抗网络和鲁棒策略学习，解决算法交易中模型对市场机制变化的适应性问题。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在算法交易中样本内表现良好，但在真实市场机制变化时性能下降。现有方法存在两个问题：1) 对高层市场波动的鲁棒性不足；2) 缺乏真实多样的模拟训练环境导致策略过拟合。

Method: 提出贝叶斯鲁棒框架：1) 数据端：使用宏观条件GAN生成器，以宏观经济指标为主要控制变量，合成具有真实时间、跨工具和宏观相关性的数据；2) 策略端：将交易过程建模为两人零和贝叶斯马尔可夫博弈，对抗代理通过扰动宏观指标模拟市场机制变化，交易代理通过分位数信念网络维护和更新对隐藏市场状态的信念，使用贝叶斯神经虚拟自我博弈寻求鲁棒完美贝叶斯均衡。
...
### [239] [Optimizing the Landscape of LLM Embeddings with Dynamic Exploratory Graph Analysis for Generative Psychometrics: A Monte Carlo Study](https://arxiv.org/abs/2601.17010)
*Hudson Golino*

Main category: cs.LG

TL;DR: 该研究将LLM嵌入重构为可搜索的语义空间，通过动态探索图分析在嵌入维度中寻找最优结构信息，发现不同指标产生竞争性优化轨迹，需要加权复合标准来平衡准确性和组织性。


<details>
  <summary>Details</summary>
Motivation: 当前应用将LLM嵌入视为静态的横截面表示，假设所有嵌入坐标均匀贡献，忽略了最优结构信息可能集中在嵌入空间特定区域的可能性。需要系统探索嵌入空间以找到最优维度信息。

Method: 将嵌入重构为可搜索的景观，采用动态探索图分析(DynEGA)系统遍历嵌入坐标，将维度索引视为伪时间排序。使用OpenAI的text-embedding-3-small模型嵌入代表自恋五个维度的项目，通过大规模蒙特卡洛模拟在不同项目池大小(3-40项目/维度)和嵌入深度(3-1,298维度)下生成网络估计。

Result: TEFI在深层嵌入范围(900-1,200维度)达到最小值，此时基于熵的组织最大化但结构准确性下降；NMI在浅层深度达到峰值，此时维度恢复最强但基于熵的拟合仍不理想。单一指标优化产生结构不连贯解，而加权复合标准能识别同时平衡准确性和组织性的嵌入维度深度区域。最优嵌入深度随项目池大小系统缩放。

Conclusion: 嵌入景观是非均匀的语义空间，需要原则性优化而非默认使用完整向量。不同优化指标产生竞争性轨迹，需要复合标准来平衡结构准确性和组织性。最优嵌入深度取决于项目池大小，为LLM嵌入在心理学测量中的应用提供了方法论指导。
...
### [240] [FlashMoE: Reducing SSD I/O Bottlenecks via ML-Based Cache Replacement for Mixture-of-Experts Inference on Edge Devices](https://arxiv.org/abs/2601.17063)
*Byeongju Kim,Jungwan Lee,Donghyeon Han,Hoi-Jun Yoo,Sangyeob Kim*

Main category: cs.LG

TL;DR: FlashMoE是一个将不活跃专家卸载到SSD的系统，可在有限RAM下实现高效MoE推理，通过轻量级ML缓存策略提升缓存命中率，在真实硬件上相比现有系统获得2.6倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有MoE推理系统（如Fiddler、DAOP）依赖DRAM卸载，不适合内存受限的端侧设备环境。随着MoE模型增长到数百GB，RAM卸载方案变得不切实际，需要新的解决方案。

Method: 提出FlashMoE系统，将不活跃专家卸载到SSD，采用轻量级基于机器学习的缓存策略，自适应结合最近使用和频率信号来最大化专家重用，显著减少存储I/O。

Result: 在真实桌面硬件平台上，FlashMoE相比LRU和LFU等传统卸载策略提升缓存命中率高达51%，相比现有MoE推理系统实现高达2.6倍的加速。

Conclusion: FlashMoE通过SSD卸载和智能缓存策略，成功解决了大型MoE模型在内存受限设备上的推理问题，为端侧设备部署大型MoE模型提供了实用解决方案。
...
### [241] [ThinkTank-ME: A Multi-Expert Framework for Middle East Event Forecasting](https://arxiv.org/abs/2601.17065)
*Haoxuan Li,He Chang,Yunshan Ma,Yi Bin,Yang Yang,See-Kiong Ng,Tat-Seng Chua*

Main category: cs.LG

TL;DR: 提出ThinkTank-ME框架，通过模拟智库多专家协作分析来改进中东事件预测，克服现有单模型方法的局限性


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的事件预测方法采用单一模型架构，只能生成单一显式轨迹，无法捕捉复杂区域背景下多样化的地缘政治细微差别。事件预测受到国际关系、区域历史动态和文化背景等多方面因素的影响，需要更全面的分析框架。

Method: 引入ThinkTank-ME框架，模拟现实世界战略决策中的协作专家分析。构建POLECAT-FOR-ME基准数据集，专门用于中东事件预测，促进专家专业化和严格评估。采用多专家协作架构处理复杂时空地缘政治预测任务。

Result: 实验结果表明，多专家协作方法在处理复杂时空地缘政治预测任务方面具有优越性。该方法能够更好地捕捉中东地区的地缘政治细微差别。

Conclusion: 多专家协作框架能够有效解决现有单模型方法在复杂区域事件预测中的局限性，为地缘政治预测提供了更全面的分析视角。代码已开源。
...
### [242] [Multi-Agent Deep Reinforcement Learning Under Constrained Communications](https://arxiv.org/abs/2601.17069)
*Shahil Shaik,Jonathon M. Smereka,Yue Wang*

Main category: cs.LG

TL;DR: 提出分布式多智能体强化学习框架DG-MAPPO，通过多跳通信和分布式图注意力网络消除对集中式训练和全局状态信息的依赖，实现仅基于局部观测和点对点通信的协作学习。


<details>
  <summary>Details</summary>
Motivation: 集中式训练分散式执行（CTDE）范式存在可扩展性、鲁棒性和泛化性瓶颈，依赖训练时的全局状态信息，在实际场景中（如队友增减、环境动态变化）脆弱且重训练成本高。分布式方法允许智能体仅使用局部信息和点对点通信进行适应。

Method: 1) 提出分布式图注意力网络（D-GAT），通过多跳通信执行全局状态推断，智能体以完全分布式方式通过输入依赖的注意力权重整合邻居特征；2) 基于D-GAT开发分布式图注意力MAPPO（DG-MAPPO），智能体使用局部观测、多跳通信和共享/平均奖励来优化局部策略和价值函数。

Result: 在StarCraftII多智能体挑战、Google Research Football和Multi-Agent Mujoco上的实验表明，该方法在广泛的合作任务中（包括同质和异质团队）始终优于强CTDE基线，实现更优的协调性能。

Conclusion: 提出的分布式MARL框架为鲁棒协作提供了原则性和可扩展的解决方案，消除了对集中式训练或全局可观测性的需求。DG-MAPPO首次完全消除了对特权集中信息的依赖，使智能体能够仅通过点对点通信进行学习和行动。
...
### [243] [Attention-Based Variational Framework for Joint and Individual Components Learning with Applications in Brain Network Analysis](https://arxiv.org/abs/2601.17073)
...
Method: 提出跨模态联合-个体变分网络（CM-JIVNet），这是一个统一的概率框架，设计用于从配对的SC-FC数据集中学习因子化潜在表示。模型利用多头注意力融合模块捕获非线性跨模态依赖，同时分离独立的模态特定信号。

Result: 在人类连接组项目年轻成人（HCP-YA）数据上验证，CM-JIVNet在跨模态重建和行为特征预测方面表现出优越性能。

Conclusion: 通过有效分离联合和个体特征空间，CM-JIVNet为大规模多模态脑分析提供了一个鲁棒、可解释且可扩展的解决方案。
...
### [244] [PhysE-Inv: A Physics-Encoded Inverse Modeling approach for Arctic Snow Depth Prediction](https://arxiv.org/abs/2601.17074)
*Akila Sampath,Vandana Janeja,Jianwu Wang*

Main category: cs.LG

TL;DR: 提出PhysE-Inv框架，结合物理约束与深度学习，解决北极雪深反演问题，在数据稀疏噪声下提升预测精度与物理一致性。


<details>
  <summary>Details</summary>
Motivation: 北极雪深估计是关键的时变反演问题，现有过程模型对稀疏数据敏感，数据驱动模型缺乏物理可解释性，需要噪声容忍且物理可解释的解决方案。

Method: 提出PhysE-Inv框架：1) 使用LSTM编码器-解码器与多头注意力序列架构；2) 物理引导对比学习；3) 基于静水力学平衡前向模型作为目标代理；4) 在潜在空间进行重构物理正则化，从噪声不完整时间序列中动态发现隐藏物理参数。

Result: 相比最先进基线，PhysE-Inv显著提升预测性能，误差降低20%，同时展示出优越的物理一致性和对数据稀疏性的鲁棒性。
...
### [245] [E2PL: Effective and Efficient Prompt Learning for Incomplete Multi-view Multi-Label Class Incremental Learning](https://arxiv.org/abs/2601.17076)
*Jiajun Chen,Yue Wu,Kai Huang,Wen Xi,Yangyang Wu,Xiaoye Miao,Mengying Zhu,Meng Xi,Guanjie Cheng*

Main category: cs.LG

TL;DR: 提出E2PL框架，通过任务定制提示和缺失感知提示，结合高效原型张量化模块，解决不完整多视图多标签类增量学习问题，显著提升效果和效率。


<details>
  <summary>Details</summary>
Motivation: 现实网络应用中存在视图缺失和类别动态扩展的问题，现有方法要么无法适应新类别，要么在处理缺失视图模式时参数爆炸，限制了在web环境中的可扩展性。

Method: 提出E2PL框架，包含：1）任务定制提示用于类增量适应；2）缺失感知提示用于灵活整合任意视图缺失场景；3）高效原型张量化模块，利用原子张量分解将提示参数复杂度从指数级降至线性；4）动态对比学习策略建模缺失视图模式间的复杂依赖关系。

Result: 在三个基准测试上的广泛实验表明，E2PL在效果和效率方面均优于现有最先进方法。

Conclusion: E2PL框架通过统一的提示设计和高效参数化方法，有效解决了不完整多视图多标签类增量学习问题，为现实网络应用提供了可扩展的解决方案。
...
### [246] [SFO: Learning PDE Operators via Spectral Filtering](https://arxiv.org/abs/2601.17090)
*Noam Koren,Rafael Moschopoulos,Kira Radinsky,Elad Hazan*

Main category: cs.LG

TL;DR: SFO是一种新型神经算子，使用通用谱基(USB)参数化积分核，能高效捕捉PDE解映射中的长程非局部相互作用，在多个基准测试中达到最先进精度。


<details>
  <summary>Details</summary>
Motivation: 传统神经算子在捕捉偏微分方程解映射中的长程非局部相互作用时效率低下。作者发现离散格林函数具有空间线性动力系统结构，这启发了他们开发更高效的表示方法。

Method: 提出谱滤波算子(SFO)，使用从希尔伯特矩阵特征模态导出的固定全局正交基(USB)参数化积分核。通过理论证明这些核在USB中具有紧凑近似，仅学习快速衰减特征值的谱系数，实现高效表示。

Result: 在六个基准测试（包括反应扩散、流体动力学和3D电磁学）中，SFO达到最先进精度，相对于强基线误差减少高达40%，同时使用更少的参数。

Conclusion: SFO通过谱滤波理论提供了一种高效表示PDE解映射中长程非局部相互作用的方法，在精度和参数效率方面均优于现有神经算子方法。
...
### [247] [CUROCKET: Optimizing ROCKET for GPU](https://arxiv.org/abs/2601.17091)
*Ole Stüven,Keno Moenck,Thorsten Schüppstuhl*

Main category: cs.LG

TL;DR: CUROCKET：一种在GPU上高效执行ROCKET时间序列分类算法的改进版本，相比CPU版本实现高达11倍的每瓦计算效率提升


<details>
  <summary>Details</summary>
Motivation: 虽然ROCKET算法在时间序列分类中表现出色且计算成本较低，但现有实现主要局限于CPU执行。卷积操作高度可并行化，适合GPU加速，但ROCKET使用的不均匀内核使得标准GPU卷积方法效率低下。

Method: 提出一种新算法，能够高效地在GPU上执行ROCKET。该方法解决了ROCKET不均匀内核在GPU上执行效率低下的问题，通过优化卷积操作实现GPU加速。

Result: CUROCKET相比CPU版本的ROCKET实现了高达11倍的每瓦计算效率提升，显著提高了计算性能。

Conclusion: CUROCKET成功将ROCKET算法高效移植到GPU平台，大幅提升了计算效率，为时间序列分类任务提供了更高效的解决方案，代码已在GitHub开源。
...
### [248] [The Triangle of Similarity: A Multi-Faceted Framework for Comparing Neural Network Representations](https://arxiv.org/abs/2601.17093)
*Olha Sirikova,Alvin Chan*

Main category: cs.LG

TL;DR: 论文提出了"相似性三角"框架，结合静态表征相似性、功能相似性和稀疏性相似性三个互补视角，用于更全面地比较神经网络表示。


<details>
  <summary>Details</summary>
Motivation: 现有的神经网络表示比较方法往往提供有限的视角，需要更全面的框架来理解模型在科学应用中的内部机制。

Method: 提出相似性三角框架，结合三种相似性度量：静态表征相似性（CKA/Procrustes）、功能相似性（线性模式连接性或预测相似性）和稀疏性相似性（剪枝下的鲁棒性）。在CNN、Vision Transformers和视觉语言模型上进行分析，使用ImageNetV2（分布内）和CIFAR-10（分布外）测试集。

Result: 研究发现：(1) 架构家族是表征相似性的主要决定因素，形成不同的聚类；(2) CKA自相似性和任务精度在剪枝过程中强相关，但精度下降更剧烈；(3) 某些模型对在剪枝后表现出正则化效果，暴露出共享的计算核心。
...
### [249] [Boltzmann-GPT: Bridging Energy-Based World Models and Language Generation](https://arxiv.org/abs/2601.17094)
*Junichiro Niimi*

Main category: cs.LG

TL;DR: 该论文提出"嘴巴不是大脑"的架构原则，将世界模型与语言模型分离，使用深度玻尔兹曼机作为世界模型，结合冻结的GPT-2进行文本生成，在消费者评论领域实现了更一致、可控的生成。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型能生成流畅文本，但它们是否真正理解世界还是仅仅产生看似合理的语言仍存在争议。作者希望明确分离世界理解和语言能力，探索语言模型与适当世界模型结合是否能实现更一致、可控的生成。

Method: 提出三组件架构：1) 深度玻尔兹曼机作为能量基世界模型捕获领域结构；2) 适配器将潜在信念状态投影到嵌入空间；3) 冻结的GPT-2提供语言能力但不含领域知识。在亚马逊智能手机评论领域实例化该框架。

Result: 实验表明：1) 通过世界模型调节的生成在情感相关性、困惑度和语义相似度上显著优于仅基于提示的生成；2) DBM能量函数能区分连贯与不连贯的市场配置；3) 对特定属性的干预能因果传播到生成文本，输出分布与自然样本统计一致。

Conclusion: 即使小规模语言模型，当连接到适当的世界模型时，也能实现一致、可控的生成。这为分离语言能力与世界理解提供了实证支持，验证了"嘴巴不是大脑"的架构原则的有效性。
...
### [250] [MambaNet: Mamba-assisted Channel Estimation Neural Network With Attention Mechanism](https://arxiv.org/abs/2601.17108)
...
<details>
  <summary>Details</summary>
Motivation: 针对具有大量子载波的OFDM配置，传统信道估计方法面临复杂度高的问题，需要一种既能有效捕获子载波间长距离依赖关系，又具有较低复杂度的解决方案。

Method: 提出Mamba辅助的神经网络框架，集成定制化的Mamba架构，采用双向选择性扫描机制（而非传统的Mamba结构），以处理非因果性的子载波信道增益，并结合自注意力机制。

Result: 在3GPP TS 36.101信道上的仿真结果表明，相比其他基线神经网络解决方案，该方法以更少的可调参数实现了更好的信道估计性能，且空间复杂度低于基于Transformer的神经网络。

Conclusion: 所提出的框架能够高效处理大规模子载波信道估计，有效捕获子载波间的长距离依赖关系，在性能和复杂度之间取得了良好平衡，为OFDM系统提供了一种有前景的信道估计解决方案。
...
### [251] [Least-Loaded Expert Parallelism: Load Balancing An Imbalanced Mixture-of-Experts](https://arxiv.org/abs/2601.17111)
*Xuan-Phi Nguyen,Shrey Pandit,Austin Xu,Caiming Xiong,Shafiq Joty*

Main category: cs.LG

TL;DR: LLEP算法解决MoE模型专家路由不平衡导致的并行计算效率问题，通过动态重路由实现负载均衡，提升训练和推理性能。


<details>
  <summary>Details</summary>
Motivation: MoE模型即使经过预训练负载平衡约束，仍然存在显著的路由不平衡问题。专家并行（EP）假设平衡路由，但在极端不平衡情况下会导致部分设备过载，造成计算和内存瓶颈，影响后训练和推理效率。

Method: 提出Least-Loaded Expert Parallelism（LLEP）算法，动态地将过载设备上的多余token和相关专家参数重路由到利用率低的设备上，确保所有设备在最小集体延迟内完成工作负载，同时满足内存约束。

Result: 在不同模型规模下，LLEP相比标准EP实现了最高5倍的加速和4倍的峰值内存使用减少。对于gpt-oss-120b模型，实现了约1.9倍的加速，支持更高吞吐量的后训练和推理。

Conclusion: LLEP有效解决了MoE模型路由不平衡导致的并行计算瓶颈，通过理论分析和实证评估验证了其有效性，为硬件特定的超参数调优提供了原则性框架。
...
### [252] [Low-Rank Tensor Approximation of Weights in Large Language Models via Cosine Lanczos Bidiagonalization](https://arxiv.org/abs/2601.17112)
*A. El Ichi,K. Jbilou*
...
### [253] [How does Graph Structure Modulate Membership-Inference Risk for Graph Neural Networks?](https://arxiv.org/abs/2601.17130)
*Megha Khosla*

Main category: cs.LG

TL;DR: 该论文研究了图神经网络中的成员推理攻击风险，特别关注图结构对节点级隐私泄露的影响，并分析了差分隐私GNN的可审计性。


<details>
  <summary>Details</summary>
Motivation: GNN在敏感应用中的使用引发了训练数据泄露的担忧，现有隐私泄露研究主要基于非图领域，需要针对图结构进行专门分析。

Method: 形式化节点-邻域元组的成员推理攻击，研究两个关键维度：训练图构建和推理时边访问。通过实验比较随机采样和滚雪球采样，分析不同设置下的隐私风险。

Result: 滚雪球采样的覆盖偏差通常损害泛化能力；推理时允许训练-测试边访问能提高测试准确率、缩小训练测试差距，并在大多数模型和数据集上产生最低的成员优势。泛化差距不能完全代表MI风险，边访问对MI风险起主导作用。

Conclusion: 图结构对GNN隐私泄露有重要影响，需要专门的图特定分析。对于节点级任务，归纳分割破坏了统计可交换性，限制了差分隐私模型标准界限的适用性。
...
### [254] [Learning to Collaborate: An Orchestrated-Decentralized Framework for Peer-to-Peer LLM Federation](https://arxiv.org/abs/2601.17133)
*Inderjeet Singh,Eleonore Vissol-Gaudin,Andikan Otung,Motoyoshi Sekiya*

Main category: cs.LG

TL;DR: KNEXA-FL是一个新颖的联邦学习框架，通过去中心化但协调的方式解决LLM微调中的数据隐私与协作效率矛盾，使用上下文多臂老虎机算法优化异构代理间的知识交换。


<details>
  <summary>Details</summary>
Motivation: 传统联邦学习在LLM微调中存在两难：集中式FL有单点故障和模型反转攻击风险，而去中心化FL的随机P2P配对效率低下且可能导致负迁移。需要一种既能保护数据隐私又能高效协作的解决方案。

Method: 提出KNEXA-FL框架，包含非聚合的中心配置器/匹配器(CPM)，将P2P协作建模为上下文多臂老虎机问题，使用LinUCB算法基于抽象代理配置文件学习最优匹配策略，通过安全蒸馏实现异构PEFT-based LLM代理间的直接知识交换。

Result: 在代码生成任务上的实验表明，KNEXA-FL相比随机P2P协作将Pass@1提高了约50%，且表现出稳定的收敛性，而强大的集中式蒸馏基线则出现灾难性性能崩溃。

Conclusion: 基于学习的自适应协调是构建稳健有效的去中心化AI生态系统的基础原则，KNEXA-FL成功解决了隐私保护与高效协作之间的权衡问题。
...
### [255] [ConceptACT: Episode-Level Concepts for Sample-Efficient Robotic Imitation Learning](https://arxiv.org/abs/2601.17135)
*Jakob Karalus,Friedhelm Schwenker*

Main category: cs.LG

TL;DR: ConceptACT：通过概念感知注意力机制提升模仿学习效率，利用演示时的人类语义概念注释（如物体属性、空间关系、任务约束）作为训练监督，无需部署时输入语义信息。


<details>
  <summary>Details</summary>
Motivation: 当前模仿学习方法仅使用低级传感器数据，忽略了人类自然拥有的丰富语义知识。人类在完成任务时理解物体属性、空间关系和任务约束等概念，这些语义信息可以显著提升学习效率。

Method: 扩展Action Chunking with Transformers (ACT)，在训练时利用演示级别的语义概念注释。修改transformer架构，在最终编码器层实现概念感知交叉注意力机制，监督其与人类注释对齐。概念仅在演示收集时由人类提供，部署时不需要语义输入。

Result: 在两个具有逻辑约束的机器人操作任务上，ConceptACT比标准ACT收敛更快且样本效率更高。注意力机制架构集成显著优于简单的辅助预测损失或语言条件模型。

Conclusion: 适当集成的语义监督为机器人学习提供了强大的归纳偏置，显著提升学习效率。概念感知注意力机制是有效利用人类语义知识的关键。
...
### [256] [Conservative & Aggressive NaNs Accelerate U-Nets for Neuroimaging](https://arxiv.org/abs/2601.17180)
*Inés Gonzalez-Pepe,Vinuyan Sivakolunthu,Jacob Fortin,Yohan Chatelain,Tristan Glatard*

Main category: cs.LG

TL;DR: 提出Conservative & Aggressive NaNs两种方法，通过识别数值不稳定体素并替换为NaN，让后续层跳过无关计算，在神经影像CNN中平均减少30%卷积操作，推理速度提升1.67倍。


<details>
  <summary>Details</summary>
Motivation: 神经影像深度学习模型架构越来越大，效率成为持续关注的问题。分析发现CNN中许多操作应用于数值噪声主导的值，对模型输出影响可忽略，部分模型中高达三分之二的卷积操作是冗余的。

Method: 提出Conservative & Aggressive NaNs两种max pooling和unpooling变体，识别数值不稳定体素并替换为NaN，后续层可跳过这些无关数据的计算。方法在PyTorch中实现，无需架构修改。

Result: 在四个CNN模型上评估，当输入包含至少50% NaN时获得稳定运行时改进；当NaN超过三分之二时（常见于神经影像场景），平均推理速度提升1.67倍。Conservative NaNs平均减少30%卷积操作且无性能下降，特定层可跳过64.64%卷积；Aggressive NaNs可跳过69.30%卷积但可能影响性能。

Conclusion: 数值不确定性可被利用来减少CNN中的冗余计算并提高推理效率，Conservative & Aggressive NaNs方法在神经影像和图像分类任务中均有效。
...
### [257] [Federated Proximal Optimization for Privacy-Preserving Heart Disease Prediction: A Controlled Simulation Study on Non-IID Clinical Data](https://arxiv.org/abs/2601.17183)
*Farzam Asad,Junaid Saif Khan,Maria Tariq,Sundus Munir,Muhammad Adnan Khan*

Main category: cs.LG

TL;DR: 该研究通过模拟医院间的非IID数据分布，验证了FedProx在心脏病预测中的效果优于集中式学习和孤立本地模型，同时保护患者隐私。


<details>
  <summary>Details</summary>
Motivation: 医疗数据因隐私法规无法直接共享，而临床数据固有的非IID特性（人口统计差异、疾病流行度不同等）给联邦学习带来挑战。需要一种能在保护隐私的同时处理数据异质性的解决方案。

Method: 使用UCI心脏病数据集（克利夫兰诊所303名患者），通过人口统计分层模拟四个异质医院客户端，创建真实的非IID数据分区。采用FedProx算法，通过近端正则化控制客户端漂移，进行50次独立运行的消融研究。

Result: FedProx（mu=0.05）达到85.00%准确率，优于集中式学习（83.33%）和孤立本地模型（平均78.45%）。近端正则化在异质环境中有效抑制客户端漂移，同时保护患者隐私。

Conclusion: FedProx在医疗联邦学习中能有效处理数据异质性并保护隐私，为实际医疗系统部署提供了算法见解和实用指南，结果可直接应用于医院IT管理。
...
### [258] [Rethinking Benchmarks for Differentially Private Image Classification](https://arxiv.org/abs/2601.17189)
*Sabrina Mokhtari,Sara Kodeiri,Shubhankar Mohapatra,Florian Tramer,Gautam Kamath*

Main category: cs.LG

TL;DR: 重新审视差分隐私图像分类基准，提出一套全面的基准测试集，用于评估不同场景下的差分隐私机器学习技术，并建立公开排行榜跟踪进展。


<details>
  <summary>Details</summary>
Motivation: 现有差分隐私图像分类基准不够全面，需要一套更全面的基准测试集来评估不同场景下的技术效果，促进社区进步。

Method: 提出一套全面的基准测试集，涵盖不同场景（有无额外数据、凸优化设置、不同数据集类型），并在这些基准上测试现有技术。

Result: 建立了全面的基准测试框架，评估了现有技术在不同场景下的有效性，并创建了公开排行榜供社区跟踪进展。

Conclusion: 该工作为差分隐私机器学习研究提供了更全面的评估框架，有助于识别不同场景下有效的技术，并通过公开排行榜促进社区进步。
...
### [259] [PUNCH: Physics-informed Uncertainty-aware Network for Coronary Hemodynamics](https://arxiv.org/abs/2601.17192)
...
TL;DR: 提出一种基于标准血管造影的非侵入性、不确定性感知框架，用于评估冠状动脉血流储备，无需地面真实血流测量，通过物理信息神经网络和变分推断实现。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉微血管功能障碍影响数百万人但诊断不足，因为金标准生理测量具有侵入性且可重复性差。需要一种非侵入性、可扩展的评估方法。

Method: 整合物理信息神经网络与变分推断，从对比剂传输的第一性原理模型推断冠状动脉血流，无需地面真实血流测量。使用合成时空强度图进行验证，并在临床患者中与侵入性热稀释法比较。

Result: 在合成数据中，预测不确定性与误差高度相关（Pearson r=0.997）。临床验证显示与侵入性热稀释法有强相关性（Pearson r=0.90）。概率CFR估计的置信区间比重复侵入性测量的变异性更窄。
...
### [260] [Accelerated Sinkhorn Algorithms for Partial Optimal Transport](https://arxiv.org/abs/2601.17196)
*Nghia Thu Truong,Qui Phu Pham,Quang Nguyen,Dung Luong,Mai Tran*

Main category: cs.LG

TL;DR: ASPOT方法将交替最小化与Nesterov加速结合，将部分最优传输的复杂度提升至O(n^{7/3}ε^{-5/3})，优于传统Sinkhorn方法。


<details>
  <summary>Details</summary>
Motivation: 传统Sinkhorn方法在处理部分最优传输问题时复杂度不够优化，限制了可扩展性，特别是在边缘分布大小不等或包含异常值的情况下。

Method: 提出ASPOT方法，在POT设置中结合交替最小化和Nesterov风格加速，同时通过明智选择熵参数γ来改进经典Sinkhorn方法的收敛速度。

Result: ASPOT实现了O(n^{7/3}ε^{-5/3})的复杂度，优于现有方法。实验验证了理论结果，显示所提方法在真实世界应用中表现优越。

Conclusion: ASPOT方法显著提升了部分最优传输问题的计算效率，为处理不等边缘分布和异常值提供了更可扩展的解决方案。
...
### [261] [SpecBridge: Bridging Mass Spectrometry and Molecular Representations via Cross-Modal Alignment](https://arxiv.org/abs/2601.17204)
*Yinkai Wang,Yan Zhou Chen,Xiaohui Chen,Li-Ping Liu,Soha Hassoun*

Main category: cs.LG

TL;DR: SpecBridge通过几何对齐框架，将质谱数据直接映射到预训练分子模型的潜在空间，显著提升小分子鉴定准确率


<details>
  <summary>Details</summary>
Motivation: 当前小分子鉴定面临质谱库不完整的问题，现有深度学习方法要么采用原子级构建的显式生成模型，要么从头学习跨模态子空间的对比模型，存在局限性

Method: 提出隐式对齐框架SpecBridge，微调自监督质谱编码器(DreaMS)直接投影到冻结的分子基础模型(ChemBERTa)的潜在空间，通过余弦相似度在预计算分子嵌入库中进行检索

Result: 在MassSpecGym、Spectraverse和MSnLib基准测试中，SpecBridge相比强基线神经网络将top-1检索准确率提升了约20-25%，同时保持可训练参数数量较少

Conclusion: 对齐到冻结的基础模型是设计新架构的实用稳定替代方案，为小分子鉴定提供了有效解决方案
...
### [262] [NewPINNs: Physics-Informing Neural Networks Using Conventional Solvers for Partial Differential Equations](https://arxiv.org/abs/2601.17207)
*Maedeh Makki,Satish Chandran,Maziar Raissi,Adrien Grenier,Behzad Mohebbi*

Main category: cs.LG

TL;DR: NewPINNs是一种将神经网络与传统数值求解器耦合的物理信息学习框架，通过求解器一致性而非残差损失来训练网络，避免了传统PINNs的常见问题。


<details>
  <summary>Details</summary>
Motivation: 传统物理信息神经网络(PINNs)存在优化困难、损失权重敏感、在刚性或非线性问题中表现不佳等问题，需要设计复杂的损失函数。NewPINNs旨在通过将数值求解器直接集成到训练循环中，让求解器负责物理约束，从而解决这些问题。

Method: NewPINNs将神经网络与数值求解器耦合：神经网络生成候选解状态，数值求解器推进这些状态，训练目标是最小化网络预测与求解器演化状态之间的差异。这种"拉-推"交互让网络通过反复接触求解器的行为来学习物理可接受的解。

Result: 该方法在涉及有限体积、有限元和谱求解器的多个正问题和反问题中表现出有效性，能够缓解传统PINNs的优化病态、损失权重敏感性和在刚性/非线性区域表现差的问题。

Conclusion: NewPINNs通过将物理约束、边界条件和数值稳定性的执行委托给成熟的数值求解器，提供了一种更稳健的物理信息学习框架，无需问题特定的损失工程或显式微分方程残差评估。
...
### [263] [JetFormer: A Scalable and Efficient Transformer for Jet Tagging from Offline Analysis to FPGA Triggers](https://arxiv.org/abs/2601.17215)
...
TL;DR: JetFormer是一种用于LHC粒子喷注标记的通用Transformer架构，在保持高性能的同时显著降低计算成本，并通过硬件优化实现从离线分析到在线触发的全场景部署。


<details>
  <summary>Details</summary>
Motivation: 现有粒子喷注标记方法通常针对特定部署场景设计，缺乏统一的架构既能满足高精度离线分析需求，又能适应超低延迟在线触发系统。需要一种既能处理变长粒子特征集，又能在计算效率和性能之间取得平衡的通用解决方案。

Method: 采用仅编码器的Transformer架构处理变长粒子特征集，无需显式成对相互作用输入。引入硬件感知优化流程，包括多目标超参数搜索、结构化剪枝和量化，生成适合FPGA部署的紧凑变体（如JetFormer-tiny）。

Result: 在JetClass数据集上，JetFormer与ParT模型精度相当（相差0.7%以内），但FLOPs减少37.4%。在HLS4ML 150P基准数据集上，比MLP、Deep Sets和Interaction Networks等现有模型准确率高3-4%。通过压缩优化，JetFormer-tiny可在FPGA上实现亚微秒级延迟。

Conclusion: JetFormer通过统一的Transformer架构成功平衡了高性能建模与硬件部署需求，为LHC的离线和在线环境提供了实用的粒子喷注标记解决方案，代码已开源。
...
### [264] [Parameter Inference and Uncertainty Quantification with Diffusion Models: Extending CDI to 2D Spatial Conditioning](https://arxiv.org/abs/2601.17224)
*Dmitrii Torbunov,Yihui Ren,Lijun Wu,Yimei Zhu*

Main category: cs.LG

TL;DR: 将条件扩散模型逆问题求解器（CDI）从一维时间信号扩展到二维空间数据，应用于材料表征中的会聚束电子衍射参数推断，能提供校准良好的后验分布，揭示参数不确定性。
...
Method: 将CDI扩展到二维空间条件化，使其能够直接从空间观测进行概率参数推断。在会聚束电子衍射参数推断这一具有挑战性的多参数逆问题上进行验证，使用模拟的CBED数据和真实参数进行测试。

Result: CDI产生校准良好的后验分布：对于确定量有紧密分布，对于模糊参数有适当宽泛分布。相比之下，标准回归方法虽然聚合指标看似准确，但通过预测训练集均值来掩盖不确定性。

Conclusion: CDI成功从时间域扩展到空间域，为稳健的科学推断提供了真实的不确定性信息，在材料表征等复杂逆问题中具有重要应用价值。
...
### [265] [A Constrained Optimization Perspective of Unrolled Transformers](https://arxiv.org/abs/2601.17257)
*Javier Porras-Valenzuela,Samar Hadou,Alejandro Ribeiro*

Main category: cs.LG

TL;DR: 提出一种约束优化框架训练Transformer，使其行为像优化下降算法，通过层间下降约束和原始-对偶训练方案，确保中间表示在期望上单调降低损失


<details>
  <summary>Details</summary>
Motivation: 传统Transformer训练使用经验风险最小化(ERM)，但缺乏优化算法的理论保证。希望让Transformer具备类似优化下降算法的行为，提高模型的鲁棒性和泛化能力

Method: 提出约束优化框架：1) 在目标函数上施加层间下降约束；2) 用原始-对偶训练方案替代标准ERM；3) 确保中间表示在期望上跨层单调降低损失；应用于展开式Transformer和预训练Transformer

Result: 在视频去噪和文本分类任务上，约束Transformer表现出更强的扰动鲁棒性，保持更高的分布外泛化能力，同时不损害分布内性能

Conclusion: 通过约束优化框架训练Transformer使其具备优化下降特性，能有效提升模型的鲁棒性和泛化能力，为Transformer设计提供了新的理论指导方向
...
### [266] [The Viscosity of Logic: Phase Transitions and Hysteresis in DPO Alignment](https://arxiv.org/abs/2601.17260)
*Marco Pollanen*

Main category: cs.LG

TL;DR: DPO的β参数不是简单的"越大越好"的对齐压力控制，而是对模型能力有复杂影响：不同架构响应模式不同，偏好边界可能与推理能力负相关，且存在能力损失的滞后效应。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为DPO中增加β（对齐压力）会持续提升模型行为质量，但作者质疑这一假设，认为β应被视为控制参数，需要系统研究其对不同模型能力的影响。

Method: 在固定DPO配方下，对三个7B开源模型家族（Mistral、Llama、Qwen）进行密集的β参数扫描，分析模型能力变化模式，包括逻辑探测边界、偏好边界与推理能力的相关性，以及训练路径的滞后效应。

Result: 1. Mistral能力呈尖锐非单调变化：逻辑探测边界仅在β≈10⁻²附近窄带内为正，边界点对随机种子敏感；2. 不同架构响应模式不同：Mistral剧烈重组，Llama选择性变化，Qwen平滑权衡；3. DPO偏好边界与推理能力可能负相关（Llama逻辑任务r=-0.91）；4. 高β训练会导致能力损失，即使降低β也无法恢复（滞后效应）。

Conclusion: 不能简单依赖偏好边界或聚合基准来选择DPO模型，需要在β参数空间中全面评估模型能力，考虑不同架构的响应差异和训练路径的滞后效应。
...
### [267] [AGZO: Activation-Guided Zeroth-Order Optimization for LLM Fine-Tuning](https://arxiv.org/abs/2601.17261)
*Wei Lin,Yining Jiang,Qingyu Song,Qiao Xiang,Hong Xu*

Main category: cs.LG

TL;DR: AGZO提出了一种基于激活引导的零阶优化方法，利用前向传播中的激活结构信息来改进LLM微调，在保持内存效率的同时显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有零阶优化方法通常使用各向同性扰动，忽略了前向传播中丰富的结构信息。线性层的梯度被限制在其输入激活张成的子空间中，这一关键洞察未被充分利用。
...
Conclusion: AGZO通过利用激活结构信息改进了零阶优化，为内存受限环境下的LLM微调提供了更有效的解决方案，在性能和内存效率之间取得了更好的平衡。
...
### [268] [Unrolled Neural Networks for Constrained Optimization](https://arxiv.org/abs/2601.17274)
*Samar Hadou,Alejandro Ribeiro*

Main category: cs.LG

TL;DR: 提出约束对偶展开（CDU）框架，使用两个耦合神经网络近似拉格朗日函数的鞍点，通过约束学习诱导对偶上升算法动态，用于解决约束优化问题。


<details>
  <summary>Details</summary>
Motivation: 传统对偶上升算法在解决约束优化问题时需要迭代计算，效率较低。作者希望开发可学习的加速版本，通过神经网络近似优化过程，提高求解效率同时保持理论保证。

Method: CDU框架包含两个耦合神经网络：原始网络模拟给定对偶乘子时拉格朗日函数的平稳点求解器；对偶网络生成最优乘子轨迹并在每层查询原始网络。通过约束学习施加原始下降和对偶上升约束，训练过程采用交替优化策略更新两个网络。

Result: 在混合整数二次规划（MIQP）和无线网络功率分配问题上，CDU方法能够产生接近最优且接近可行的解，并展现出强大的分布外泛化能力。

Conclusion: CDU框架成功地将对偶上升算法转化为可学习的神经网络架构，为约束优化问题提供了高效、可泛化的求解方案，在保持理论性质的同时实现了加速计算。
...
### [269] [Latent-Space Contrastive Reinforcement Learning for Stable and Efficient LLM Reasoning](https://arxiv.org/abs/2601.17275)
*Lianlei Shan,Han Chen,Yixuan Wang,Zhenjie Liu,Wei Li*

Main category: cs.LG

TL;DR: DLR提出了一种潜在空间双向对比强化学习框架，将推理过程从昂贵的token级序列生成转移到连续潜在流形，通过冻结主模型参数避免灾难性遗忘，实现更稳定、更长链的推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在表面文本生成上表现出色，但在处理复杂多步推理任务时往往只是"统计拟合"而非系统逻辑推理。传统强化学习虽然尝试引入"先思考后说话"范式，但在高维离散token空间中面临样本效率低、梯度估计方差大和灾难性遗忘三大挑战。

Method: 提出DeepLatent Reasoning (DLR)框架：1) 使用轻量级辅助模型在潜在空间中采样K个推理链编码；2) 基于正确性和格式的双重奖励机制筛选高价值潜在轨迹；3) 仅将筛选后的轨迹输入冻结主模型进行单次解码；4) 设计对比学习目标以最大化推理多样性同时保持连贯性。

Result: 在可比较的GPU计算预算下，DLR实现了更稳定的训练收敛，支持更长的推理链，促进了推理能力的可持续积累，为大语言模型提供了可靠且可扩展的强化学习路径。

Conclusion: DLR通过将试错成本从昂贵的token级序列生成转移到连续潜在流形，从根本上解决了传统RL在大语言模型推理任务中的结构瓶颈，同时通过冻结主模型参数消除了灾难性遗忘风险，为LLM的可靠强化学习提供了可行路径。
...
### [270] [Tabular Foundation Models are Strong Graph Anomaly Detectors](https://arxiv.org/abs/2601.17301)
*Yunhui Liu,Tieke He,Yongchao Liu,Can Yi,Hong Jin,Chuntao Hong*

Main category: cs.LG

TL;DR: TFM4GAD框架将表格基础模型应用于图异常检测，通过图扁平化构建增强特征表，利用TFM的合成预训练和上下文学习能力，实现跨域通用检测，性能超越专门训练的GAD模型。


<details>
  <summary>Details</summary>
Motivation: 现有图异常检测方法遵循"一个数据集一个模型"范式，导致计算成本高、数据需求大、泛化能力差。需要一种"一劳永逸"的基础模型解决方案，能够跨不同图数据集检测异常而无需重新训练。

Method: 提出TFM4GAD框架，通过"扁平化"图结构构建增强特征表：将原始节点特征与拉普拉斯嵌入、局部全局结构特征、异常敏感邻域聚合等结合。然后使用表格基础模型在完全上下文学习机制下处理该特征表。

Result: 在多个数据集和各种TFM骨干网络上的广泛实验表明，TFM4GAD显著超越了专门从头训练的GAD模型性能，实现了令人惊讶的性能提升。

Conclusion: TFM4GAD为利用表格基础模型作为强大、通用的图异常检测器提供了新视角和实践范式，解决了跨域泛化、特征异构性和标签稀缺等核心挑战。
...
### [271] [Decentralized Multi-Agent Swarms for Autonomous Grid Security in Industrial IoT: A Consensus-based Approach](https://arxiv.org/abs/2601.17303)
*Samaresh Kumar Singh,Joyjit Roy*

Main category: cs.LG

TL;DR: 提出去中心化多智能体群架构，用于工业物联网安全监控，实现亚毫秒级响应，检测准确率达97.3%


<details>
  <summary>Details</summary>
Motivation: 工业物联网设备数量激增，传统集中式安全监控架构存在延迟问题，攻击者可利用此漏洞危害整个制造生态系统

Method: 设计去中心化多智能体群架构，在每个边缘网关部署自主AI代理，通过轻量级P2P协议协作检测异常行为，采用共识式威胁验证流程进行投票决策

Result: 在模拟2000个IIoT设备的测试环境中，平均响应时间0.85ms，恶意活动检测准确率97.3%，零日攻击检测准确率87%，网络带宽使用比云方案减少89%

Conclusion: DMAS架构能有效解决集中式安全监控的延迟问题，提供快速响应和高检测准确率，防止工业控制系统级联故障，显著降低网络带宽需求
...
### [272] [Weighted Graph Clustering via Scale Contraction and Graph Structure Learning](https://arxiv.org/abs/2601.17307)
*Haobing Liu,Yinuo Zhang,Tingting Wang,Ruobing Jiang,Yanwei Yu*

Main category: cs.LG

TL;DR: 提出一种收缩式边权重感知图聚类网络，通过图收缩模块减少图规模并保留重要节点，同时使用边权重感知注意力网络识别和削弱噪声连接，从而提高聚类效果并减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有图聚类方法未能充分利用边权重信息，而利用边权重面临两大挑战：1) 边权重会增加存储空间和训练时间，需要在不损失聚类效果的前提下减少图规模；2) 边权重信息可能包含噪声，对聚类结果产生负面影响。现有研究很少能同时优化聚类和边权重。

Method: 提出收缩式边权重感知图聚类网络，包含两个核心模块：1) 面向聚类的图收缩模块，用于减少图规模同时保留对聚类任务重要的节点；2) 边权重感知注意力网络，用于识别和削弱噪声连接，从而在聚类过程中减轻噪声边的影响。

Result: 在三个真实世界加权图数据集上进行广泛实验，模型性能优于最佳基线方法。实验还表明，提出的图收缩模块能显著减少训练时间和存储空间。

Conclusion: 该研究提出的方法能有效解决加权图聚类中的两个关键挑战，通过图收缩和噪声边处理机制，在提高聚类效果的同时减少了计算开销，为加权图聚类提供了有效的解决方案。
...
### [273] [PAR: Plausibility-aware Amortized Recourse Generation](https://arxiv.org/abs/2601.17309)
*Anagha Sabu,Vidhya S,Narayanan C Krishnan*

Main category: cs.LG

TL;DR: PAR：基于摊销近似推理的算法补救方法，通过约束最大后验推断生成高似然、现实可行的反事实解释
...
Method: 将补救问题形式化为约束最大后验推断问题，使用摊销近似推理生成反事实，利用可处理概率模型直接估计似然，通过最大化接受类分布似然、最小化拒绝类分布似然及其他约束损失来训练生成器，并引入基于邻域的调节机制实现个性化补救生成

Result: 在广泛使用的算法补救数据集上验证，PAR能高效生成有效、与事实相似、稀疏且高度合理的补救方案，性能优于现有最先进方法

Conclusion: PAR通过约束最大后验推断和摊销近似推理，成功解决了算法补救中生成高似然、现实可行反事实的挑战，为个性化算法补救提供了有效解决方案
...
### [274] [Conformal Feedback Alignment: Quantifying Answer-Level Reliability for Robust LLM Alignment](https://arxiv.org/abs/2601.17329)
*Tiejin Chen,Xiaoou Liu,Vishnu Nandam,Kuan-Ru Liou,Hua Wei*

Main category: cs.LG

TL;DR: 提出Conformal Feedback Alignment (CFA)框架，利用Conformal Prediction的统计保证来量化答案可靠性，并将其转化为DPO和PPO训练的权重，提高对齐的鲁棒性和数据效率。


<details>
  <summary>Details</summary>
Motivation: 基于偏好的对齐方法（如RLHF）面临标签噪声和不一致问题。现有不确定性感知方法只关注偏好权重，但忽略了更基本的因素：被比较答案的可靠性。

Method: 提出Conformal Feedback Alignment (CFA)框架：1) 使用Conformal Prediction构建具有可控覆盖率的预测集来量化答案级可靠性；2) 将这些可靠性聚合为DPO和PPO风格训练的权重。

Result: 在不同数据集上的实验表明，CFA提高了对齐的鲁棒性和数据效率，证明建模答案侧不确定性可以补充偏好级加权，实现更鲁棒、数据高效的对齐。

Conclusion: CFA通过Conformal Prediction的统计保证来量化答案可靠性，为偏好对齐提供了更全面的不确定性建模方法，提高了对齐系统的鲁棒性和数据效率。
...
### [275] [Thermodynamically Optimal Regularization under Information-Geometric Constraints](https://arxiv.org/abs/2601.17330)
*Laurent Caraffa*

Main category: cs.LG

TL;DR: 论文提出一个统一理论框架，连接热力学最优性、信息几何和正则化，证明在特定假设下Fisher-Rao度量是信念空间唯一容许几何，热力学最优正则化对应最小化到参考状态的Fisher-Rao距离平方。


<details>
  <summary>Details</summary>
Motivation: 现代机器学习依赖经验成功但理论异质的正则化技术（如权重衰减、dropout、指数移动平均），同时训练大模型能耗成本急剧增加，需要探索学习算法是否接近基本效率界限。

Method: 提出基于三个明确假设的理论框架：(A1)最优性需要内在、参数化不变的信息度量；(A2)信念状态由已知约束下的最大熵分布建模；(A3)最优过程是准静态的。证明条件最优定理，推导高斯和圆形信念模型的诱导几何。

Result: 证明Fisher-Rao度量是信念空间唯一容许几何，热力学最优正则化对应最小化到参考状态的Fisher-Rao距离平方。推导出高斯模型对应双曲流形，圆形模型对应von Mises流形，显示经典正则化方案结构上无法保证热力学最优性。

Conclusion: 该工作为机器学习中的正则化提供了原则性的几何和热力学基础，提出了学习热力学效率概念和可实验验证的预测，统一了不同正则化技术的理论基础。
...
### [276] [Power-based Partial Attention: Bridging Linear-Complexity and Full Attention](https://arxiv.org/abs/2601.17334)
*Yufeng Huang*

Main category: cs.LG

TL;DR: 论文提出了一种幂基部分注意力机制(PPA)，其复杂度为O(L^{1+p})，其中0≤p≤1，通过调节p值可以探索不同复杂度注意力机制的性能表现。
...
Method: 引入幂基部分注意力(PPA)，这是一种复杂度为O(L^{1+p})的注意力机制，其中p在0到1之间。p=0对应线性复杂度的滑动窗口注意力，p=1对应完全注意力。通过这种构造，可以探索Transformer性能如何随p控制的注意力缩放行为变化。

Result: 实验显示整体趋势呈现S曲线行为：性能在p值的狭窄窗口内从滑动窗口注意力过渡到完全注意力，当p接近1时趋于平稳。实验表明存在0<p<1使得O(L^{1+p})注意力足以达到与O(L^2)完全注意力相似的结果。

Conclusion: 二次复杂度的注意力并非必要，存在次二次复杂度的注意力机制能够达到可比性能，这为设计更高效的Transformer架构提供了理论依据。
...
### [277] [Spectral Geometry for Deep Learning: Compression and Hallucination Detection via Random Matrix Theory](https://arxiv.org/abs/2601.17357)
*Davide Ettori*

Main category: cs.LG

TL;DR: 该论文提出基于谱几何和随机矩阵理论的统一框架，通过分析隐藏激活的特征值结构来解决大模型的可靠性问题和计算成本问题。主要贡献包括用于检测幻觉的EigenTrack方法和用于模型压缩的RMT-KD方法。
...
Method: 提出基于谱几何和随机矩阵理论的统一框架，分析隐藏激活的特征值结构。具体包括：1) EigenTrack：使用谱特征及其时间动态实时检测语言和视觉语言模型中的幻觉和分布外行为；2) RMT-KD：通过识别信息性谱分量并应用迭代知识蒸馏来压缩模型。

Result: 谱统计量提供了可解释且鲁棒的信号，可用于监控大尺度神经网络的不确定性和指导模型压缩。EigenTrack能有效检测幻觉，RMT-KD能在保持准确性的同时生成紧凑高效的模型。

Conclusion: 谱几何和随机矩阵理论为解决大模型的可靠性问题和计算成本问题提供了有效的统一框架，谱统计量是监控不确定性和指导压缩的有力工具。
...
### [278] [Robust Privacy: Inference-Time Privacy through Certified Robustness](https://arxiv.org/abs/2601.17360)
...
<details>
  <summary>Details</summary>
Motivation: 机器学习系统在推理时可能泄露敏感输入属性，现有隐私保护方法存在不足，需要一种新的推理时隐私概念来防止模型反转攻击等隐私泄露。

Method: 提出鲁棒隐私（RP）概念，基于认证鲁棒性思想：如果模型预测在输入x的半径R邻域内保持不变，则x享有R-鲁棒隐私。开发属性隐私增强（APE）方法将输入级不变性转化为属性级隐私保护。

Result: 在推荐任务中，RP扩展了与正面推荐兼容的敏感属性值集合；在模型反转攻击实验中，即使小噪声水平（σ=0.1）下，攻击成功率从73%降至4%，且部分情况下可降低攻击成功率至44%而不影响模型性能。

Conclusion: 鲁棒隐私提供了一种有效的推理时隐私保护框架，既能保护敏感属性不被推断，又能缓解模型反转攻击，在隐私保护与模型性能之间取得平衡。
...
### [279] [Diversified Scaling Inference in Time Series Foundation Models](https://arxiv.org/abs/2601.17376)
...
<details>
  <summary>Details</summary>
Motivation: 时间序列基础模型的发展主要依赖大规模预训练，但推理时的计算潜力尚未充分挖掘。本研究旨在探索两个问题：1) TSFMs在标准采样推理缩放下的行为特征；2) 受控采样多样性是否能提升性能。

Method: 首先分析TSFMs在标准采样下的特性，发现其因解决方案空间探索不足而无法遵循缩放定律。然后通过定制的时间序列扰动实现多样化推理缩放，扩展生成分布的支撑集。理论分析多样性-保真度权衡，推导出多样化采样优于标准采样的关键样本阈值。

Result: 在各种TSFMs和数据集上的广泛实验表明，适当的多样化推理缩放能在不更新参数的情况下带来显著的性能提升，确立了推理设计作为TSFM优化的关键计算高效维度。提出了RobustMSE指标来量化固定预算下TSFM的性能上限。

Conclusion: 研究阐明了这些因素的相互作用，使得在不重新训练TSFMs的情况下，通过并行环境中的多样化大规模推理时间序列实现可靠性能成为可能。推理设计是TSFM优化的关键计算高效维度。
...
### [280] [GO-OSC and VASH: Geometry-Aware Representation Learning for Early Degradation Detection in Oscillatory Systems](https://arxiv.org/abs/2601.17396)
*Vashista Nobaub*

Main category: cs.LG

TL;DR: GO-OSC是一个几何感知的振荡时间序列表示学习框架，通过强制规范可识别的潜在参数化，实现对早期退化的稳定检测，相比传统能量基方法具有更高灵敏度。


<details>
  <summary>Details</summary>
Motivation: 振荡系统的早期退化通常表现为动力学的几何畸变（如相位抖动、频率漂移），在信号能量变化可检测之前就已发生。传统的能量基诊断和无约束学习表示对此结构不敏感，导致检测延迟或不稳定。

Method: 引入GO-OSC框架：1）强制规范可识别的潜在参数化，实现跨短时无标签窗口的稳定比较和聚合；2）定义一族不变线性几何探针，针对潜在空间中与退化相关的方向；3）提供理论分析，证明在早期仅相位退化情况下，几何探针具有严格正灵敏度。

Result: 理论分析表明，在早期仅相位退化下，能量基统计量的一阶检测能力为零，而几何探针具有严格正灵敏度。实验验证了该方法在合成基准和真实振动数据集上实现了更早的检测、更高的数据效率和更强的工况变化鲁棒性。

Conclusion: GO-OSC通过几何感知的表示学习和规范参数化，解决了振荡系统早期退化检测的挑战，相比传统方法具有理论保证的更高灵敏度，为实际工程应用提供了更可靠的早期故障检测工具。
...
### [281] [Efficient Dilated Squeeze and Excitation Neural Operator for Differential Equations](https://arxiv.org/abs/2601.17407)
*Prajwal Chauhan,Salah Eddine Choutri,Saif Eddin Jabari*

Main category: cs.LG

TL;DR: D-SENO是一种轻量级神经算子框架，通过结合扩张卷积和挤压-激励模块，在保持高精度的同时实现比现有方法快约20倍的训练速度，适用于多种PDE求解。
...
Method: 结合扩张卷积块（捕捉宽感受野和长程物理依赖）与挤压-激励模块（通道注意力机制，自适应重新校准特征通道），构建轻量级算子学习框架。

Result: 在多种PDE基准测试中（翼型势流、达西流、泊肃叶流、不可压缩Navier-Stokes涡场），训练速度比标准Transformer模型和神经算子快约20倍，同时精度相当或更高。

Conclusion: D-SENO提供了一种高效准确的PDE求解方案，通过精心设计的架构在保持性能的同时大幅降低计算成本，消融研究表明SE模块对性能有重要贡献。
...
### [282] [Active Hypothesis Testing for Correlated Combinatorial Anomaly Detection](https://arxiv.org/abs/2601.17430)
*Zichuan Yang,Yiming Xing*

Main category: cs.LG

TL;DR: 提出ECC-AHT算法，用于在相关噪声下识别异常流子集，通过主动噪声消除实现最优样本复杂度


<details>
  <summary>Details</summary>
Motivation: 监控和网络安全中的异常检测问题，现有组合纯探索方法假设独立观测，无法利用相关性进行高效测量设计

Method: ECC-AHT自适应算法，通过选择连续约束测量来最大化竞争假设间的Chernoff信息，实现主动噪声消除的差分感知

Result: 在合成和真实世界相关环境中显著优于现有基线方法，达到最优样本复杂度保证

Conclusion: ECC-AHT算法有效解决了相关噪声下的异常子集识别问题，为监控和安全系统提供了高效解决方案
...
### [283] [Data-driven Clustering and Merging of Adapters for On-device Large Language Models](https://arxiv.org/abs/2601.17441)
...
Method: D2C方法利用少量任务特定示例（如每个任务10个），采用迭代优化过程精炼聚类分配，将每个聚类内的适配器合并为多任务适配器

Result: 实验结果表明，该方法在考虑存储预算的情况下有效提升了性能

Conclusion: D2C方法通过适配器聚类和合并，解决了移动设备上多任务适配器部署的存储限制问题，实现了更好的性能表现
...
### [284] [DREAM: Dual-Standard Semantic Homogeneity with Dynamic Optimization for Graph Learning with Label Noise](https://arxiv.org/abs/2601.17449)
*Yusheng Zhao,Jiaye Xie,Qixin Zhang,Weizhi Zhang,Xiao Luo,Zhiping Xiao,Philip S. Yu,Ming Zhang*

Main category: cs.LG

TL;DR: DREAM：一种针对标签噪声图数据的双重标准语义同质性动态优化方法，通过关系感知的动态优化框架和双重标准锚点选择策略，有效处理图学习中的标签噪声问题。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络方法通常假设训练图具有良好标注，但现实场景中标签可靠性无法保证。现有处理标签噪声的方法存在两个主要问题：(1)难以区分可靠与不可靠节点，(2)忽略了图拓扑中嵌入的关系信息。

Method: 提出DREAM方法，包含关系感知的动态优化框架，在优化过程中迭代重新评估每个标注节点的可靠性。采用双重标准选择策略，基于节点邻近性和图拓扑选择锚点集，计算目标节点与锚点之间的语义同质性作为优化指导。

Result: 在六个不同领域的图数据集上，针对三种类型的图标签噪声进行了广泛实验，与竞争基线相比，结果证明了DREAM的有效性。

Conclusion: DREAM通过结合关系信息和动态优化机制，能够有效处理图学习中的标签噪声问题，提供了可靠的关系感知优化框架，并有严格的理论分析支持。
...
### [285] [Harnessing Reasoning Trajectories for Hallucination Detection via Answer-agreement Representation Shaping](https://arxiv.org/abs/2601.17467)
*Jianxiong Zhang,Bing Guo,Yuming Jiang,Haobo Wang,Bo An,Xuefeng Du*

Main category: cs.LG

TL;DR: 提出ARS方法，通过生成反事实答案并学习答案一致性表示，改善大型推理模型的幻觉检测


<details>
  <summary>Details</summary>
Motivation: 大型推理模型经常生成看似连贯但答案错误的推理轨迹，使得幻觉检测困难。现有方法直接使用轨迹文本或隐藏状态容易过拟合表面模式而非答案有效性。

Method: 提出答案一致性表示塑形（ARS）方法：通过小规模潜在干预生成反事实答案，扰动轨迹边界嵌入，根据生成答案是否与原答案一致进行标记，学习使答案一致的表示接近、不一致的表示分离的嵌入。

Result: 实验表明ARS持续改善检测性能，相比强基线取得显著提升。塑形后的嵌入可与现有基于嵌入的检测器即插即用，训练过程无需人工标注。

Conclusion: ARS通过编码答案稳定性学习检测友好的轨迹条件表示，有效暴露指示幻觉风险的潜在不稳定性，提升大型推理模型的幻觉检测能力。
...
### [286] [Identifying and Correcting Label Noise for Robust GNNs via Influence Contradiction](https://arxiv.org/abs/2601.17469)
*Wei Ju,Wei Zhang,Siyu Yi,Zhengyang Mao,Yifan Wang,Jingyang Yuan,Zhiping Xiao,Ziyue Qiao,Ming Zhang*

Main category: cs.LG

TL;DR: 提出ICGNN方法，利用图结构信息处理标签噪声问题，通过影响矛盾分数检测噪声标签，结合邻居预测进行校正，并利用伪标签增强监督信号。


<details>
  <summary>Details</summary>
Motivation: 图神经网络在处理图结构数据方面表现出色，但在现实场景中，标签噪声（源于标注错误或不一致）严重影响了GNN的鲁棒性和有效性，需要专门的方法来解决这一问题。

Method: 提出ICGNN方法：1) 设计基于图扩散矩阵的影响矛盾分数(ICS)作为噪声指示器，量化节点标签可信度；2) 使用高斯混合模型精确检测噪声标签；3) 开发软策略结合邻居节点预测校正噪声标签；4) 引入未标记节点的伪标签提供辅助监督信号。

Result: 在基准数据集上的实验表明，该方法在处理图数据标签噪声问题上具有优越性。

Conclusion: ICGNN通过有效利用图结构信息来检测和校正噪声标签，并引入伪标签增强监督，为解决图数据中的标签噪声问题提供了一种有效方法。
...
### [287] [LeanTutor: Towards a Verified AI Mathematical Proof Tutor](https://arxiv.org/abs/2601.17473)
*Manooshree Patel,Rayna Bhattacharyya,Thomas Lu,Arnav Mehta,Niels Voss,Narges Norouzi,Gireeja Ranade*

Main category: cs.LG

TL;DR: 开发结合LLM自然语言交互与定理证明器可验证正确性的数学证明辅导系统LeanTutor，包含自动形式化、下一步生成和自然语言反馈三个模块，使用PeanoBench数据集评估。


<details>
  <summary>Details</summary>
Motivation: LLM支持自然语言交互但易出错，定理证明器（如Lean）能保证正确性但学习曲线陡峭。结合两者优势，为数学学习者提供既易用又可验证正确性的证明辅导工具。

Method: 提出LeanTutor系统，包含三个核心模块：1) 自动形式化/证明检查器，将自然语言证明转为形式化验证；2) 下一步生成器，提供证明步骤建议；3) 自然语言反馈生成器，提供易懂的指导。使用PeanoBench数据集（371个皮亚诺算术证明）进行评估。

Result: 开发了概念验证系统LeanTutor，结合了LLM的自然语言交互能力和定理证明器的可验证正确性，通过PeanoBench数据集展示了系统可行性。

Conclusion: 结合LLM与定理证明器的混合方法为解决数学证明辅导中的自然语言交互与可验证正确性之间的权衡提供了有前景的解决方案，LeanTutor展示了这种方法的可行性。
...
### [288] [Unintended Memorization of Sensitive Information in Fine-Tuned Language Models](https://arxiv.org/abs/2601.17480)
...
TL;DR: 该研究系统调查了LLM微调中仅出现在输入而非训练目标中的PII泄露风险，通过设计提取探针量化记忆行为，并评估了四种隐私保护方法的隐私-性能权衡。


<details>
  <summary>Details</summary>
Motivation: 微调大型语言模型时，敏感数据中的个人身份信息存在意外记忆和泄露风险，这违反了隐私法规并威胁个人安全。特别是那些仅出现在模型输入中、不在训练目标中的PII，这一关键但未被充分探索的漏洞需要系统研究。

Method: 使用合成和真实数据集设计受控提取探针来量化意外PII记忆，研究语言、PII频率、任务类型和模型规模等因素对记忆行为的影响。进一步对差分隐私、机器遗忘、正则化和偏好对齐四种隐私保护方法进行基准测试。

Result: 研究表明，后训练方法通常提供更一致的隐私-效用权衡，而差分隐私在特定设置下能显著减少泄露，但可能引入训练不稳定性。记忆行为受多种因素影响，包括PII频率和模型规模。

Conclusion: 微调LLM中的记忆问题仍然是一个持续挑战，需要强大、可扩展的隐私保护技术。研究强调了系统评估隐私保护方法的重要性，并指出了未来开发更有效隐私保护解决方案的方向。
...
### [289] [Automatic Stability and Recovery for Neural Network Training](https://arxiv.org/abs/2601.17483)
*Barak Or*

Main category: cs.LG

TL;DR: 提出一个运行时稳定性框架，通过隔离创新信号自动检测和恢复训练中的不稳定更新，无需修改底层优化器


<details>
  <summary>Details</summary>
Motivation: 现代神经网络训练越来越脆弱，罕见但严重的破坏性更新常导致不可逆的发散或性能退化，现有优化方法主要依赖优化器内部的预防机制，一旦发生不稳定，检测和恢复能力有限

Method: 引入监督运行时稳定性框架，将优化视为受控随机过程，通过隔离来自次级测量（如验证探针）的创新信号，实现自动检测和恢复破坏性更新

Result: 提供理论运行时安全保证，形式化有界退化和恢复，实现开销最小且兼容内存受限的训练设置

Conclusion: 该框架为神经网络训练提供了有效的运行时稳定性监控和恢复机制，解决了现有方法在检测和恢复不稳定更新方面的局限性
...
### [290] [SpatialMath: Spatial Comprehension-Infused Symbolic Reasoning for Mathematical Problem-Solving](https://arxiv.org/abs/2601.17489)
*Ashutosh Bajpai,Akshat Bhandari,Akshay Nambi,Tanmoy Chakraborty*

Main category: cs.LG

TL;DR: SpatialMath框架通过将空间表示融入符号推理链，显著提升多模态语言模型在视觉密集型数学问题（特别是几何问题）上的表现，在MATHVERSE-PLUS数据集上比基线提升10个百分点。
...
Method: 提出SpatialMath框架，包含专门感知模块提取视觉图表中的空间基础表示，捕获关键几何结构和空间关系，然后将这些表示系统地融入符号推理链，实现视觉理解感知的结构化推理。

Result: SpatialMath显著优于强大多模态基线，在视觉密集型设置下比监督微调加数据增强提升高达10个百分点。鲁棒性分析显示增强的空间表示直接提高推理准确性。

Conclusion: 结构化感知到推理管道对多模态语言模型至关重要，空间表示与符号推理的整合能有效解决视觉密集型数学问题中的理解与推理挑战。
...
### [291] [PEARL: Prototype-Enhanced Alignment for Label-Efficient Representation Learning with Deployment-Driven Insights from Digital Governance Communication Systems](https://arxiv.org/abs/2601.17495)
*Ruiyu Zhang,Lin Nie,Wai-Fung Lam,Qihao Wang,Xin Zhao*

Main category: cs.LG

TL;DR: PEARL是一种标签高效的表示学习方法，通过原型增强对齐嵌入表示，在标签稀缺情况下显著改善嵌入空间的局部邻域结构，提升最近邻检索性能。


<details>
  <summary>Details</summary>
Motivation: 现实部署系统中，基于嵌入的最近邻检索经常失败，问题不在于语言模型本身，而在于嵌入空间的最近邻对应错误案例。固定预训练嵌入在标签稀缺、领域漂移时难以重训练，导致下游性能严重依赖嵌入几何结构，而原始嵌入往往与最近邻检索所需的局部邻域结构对齐不佳。

Method: PEARL（原型增强对齐表示学习）使用有限监督通过软对齐嵌入到类别原型来重塑局部邻域几何结构。该方法保持嵌入维度，避免激进投影或坍缩，旨在填补无监督后处理（增益有限）和全监督投影（需要大量标签数据）之间的空白。

Result: 在标签稀缺条件下，PEARL显著提升局部邻域质量：相比原始嵌入获得25.7%增益，相比强无监督后处理获得超过21.1%增益，在基于相似性的系统最脆弱的场景中表现优异。

Conclusion: PEARL提供了一种标签高效的嵌入对齐方法，在现实部署的标签稀缺场景中有效改善嵌入空间的局部几何结构，提升最近邻检索和轻量级分类器的性能。
...
### [292] [One-Shot Federated Clustering of Non-Independent Completely Distributed Data](https://arxiv.org/abs/2601.17512)
*Yiqun Zhang,Shenghong Cai,Zihua Yang,Sen Feng,Yuzhu Ji,Haijun Zhang*

Main category: cs.LG

TL;DR: 本文提出GOLD框架解决联邦聚类中Non-IID数据导致的聚类碎片化问题，通过全局导向的局部分布学习提升聚类性能。


<details>
  <summary>Details</summary>
Motivation: 联邦聚类在无标签的分布式IoT系统中应用广泛，但Non-IID数据导致聚类性能瓶颈：不同客户端可能将同一聚类碎片化，形成更广义的Non-ICD问题，现有方法难以有效融合全局知识。
...
Conclusion: 揭示了Non-IID数据中聚类碎片化的关键问题，提出Non-ICD概念和GOLD框架，有效解决了联邦聚类中的全局知识融合挑战。
...
### [293] [Towards Generalisable Imitation Learning Through Conditioned Transition Estimation and Online Behaviour Alignment](https://arxiv.org/abs/2601.17563)
*Nathan Gavenski,Matteo Leonetti,Odinaldo Rodrigues*

Main category: cs.LG

TL;DR: 提出UfO方法，通过无监督学习从观察中模仿，无需动作监督，优于现有ILfO方法且泛化能力更强。


<details>
  <summary>Details</summary>
Motivation: 现有观察模仿学习方法需要动作监督、假设状态有单一最优动作、不考虑环境状态直接应用教师动作，存在局限性。

Method: 两阶段学习：先近似教师动作，再调整智能体轨迹与教师对齐，完全无监督。

Result: 在5个环境中超越教师和所有ILfO方法，标准差最小，泛化能力最好。

Conclusion: UfO解决了现有ILfO方法的局限性，实现了无监督的观察模仿学习，性能优越且泛化能力强。
...
### [294] [Quantum-Inspired Episode Selection for Monte Carlo Reinforcement Learning via QUBO Optimization](https://arxiv.org/abs/2601.17570)
*Hadi Salloum,Ali Jnadi,Yaroslav Kholodov,Alexander Gasnikov*

Main category: cs.LG

TL;DR: MC+QUBO：通过将轨迹选择转化为QUBO问题，使用量子启发采样器提升蒙特卡洛强化学习的样本效率


<details>
  <summary>Details</summary>
Motivation: 传统蒙特卡洛强化学习在稀疏奖励、大状态空间和相关轨迹环境下样本复杂度高，收敛速度慢

Method: 将轨迹选择建模为QUBO问题，线性项偏好高奖励轨迹，二次项惩罚冗余，使用模拟量子退火和模拟分叉作为黑盒求解器

Result: 在有限时域GridWorld环境中，MC+QUBO在收敛速度和最终策略质量上优于传统蒙特卡洛方法

Conclusion: 量子启发优化作为强化学习中决策子程序具有潜力，能有效提升样本效率和策略质量
...
### [295] [Understanding Transformer Encoder-Decoder Representations through Bernoulli Dropout](https://arxiv.org/abs/2601.17602)
*Xuanzhou Chen*

Main category: cs.LG

TL;DR: 研究通过高维编码器-解码器嵌入的角度相似性分析Transformer过参数化，使用伯努利dropout识别保持Top-1预测的稀疏性阈值


<details>
  <summary>Details</summary>
Motivation: 研究Transformer模型过参数化问题，探索编码器-解码器嵌入在高维空间中的角度相似性，理解模型在稀疏条件下的鲁棒性

Method: 应用伯努利dropout在编码器和解码器之间，改变保留概率p来识别稀疏性阈值；理论证明当嵌入有效稀疏度足够大时，解码器性能在适度坐标dropout下保持稳定；实验上构建带二进制擦除通道(BEC)的Transformer模型，在英法翻译任务上测试

Result: 实验结果显示验证准确率和BLEU分数在某个阈值处急剧下降，可视化展示了这一趋势，证实了稀疏性阈值的存在

Conclusion: Transformer模型存在稀疏性依赖的阈值，当有效稀疏度足够大时，模型性能在适度dropout下保持稳定，这为理解Transformer过参数化提供了新视角
...
### [296] [A Thermodynamic Theory of Learning I: Irreversible Ensemble Transport and Epistemic Costs](https://arxiv.org/abs/2601.17607)
*Daisuke Okanohara*

Main category: cs.LG

TL;DR: 论文提出学习本质上是一个不可逆过程，通过引入认知自由能框架，推导出认知速度极限(ESL)，为学习过程中的最小熵产生设定了下界。
...
Method: 将学习建模为模型配置概率分布空间中的传输过程，引入认知自由能框架，定义自由能下降作为学习轨迹上的记账量，并将其分解为可逆和不可逆分量。

Result: 推导出认知速度极限(ESL)，这是一个有限时间不等式，为任何学习过程实现给定分布变换所需的最小熵产生设定了下界，该下界仅取决于初始和最终分布之间的Wasserstein距离。

Conclusion: 学习本质上是一个不可逆过程，认知结构的实现必然伴随熵产生，认知速度极限为学习过程的热力学限制提供了理论基础。
...
### [297] [Split-on-Share: Mixture of Sparse Experts for Task-Agnostic Continual Learning](https://arxiv.org/abs/2601.17616)
*Fatema Siddika,Md Anwar Hossen,Tanwi Mallick,Ali Jannesari*

Main category: cs.LG

TL;DR: SETA框架通过混合稀疏专家解决LLMs持续学习中的可塑性-稳定性困境，将知识分解为任务特定专家和共享专家，实现任务无关的持续学习。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在持续学习中面临可塑性-稳定性困境：学习新能力会导致灾难性遗忘先前知识。现有方法通常均匀处理参数，无法区分特定任务知识和共享能力。

Method: 提出SETA框架，将模型分解为模块化子空间：独特专家（隔离任务特定模式）和共享专家（捕获共同特征）。通过弹性权重锚定保护关键共享知识，统一门控网络在推理时自动检索正确的专家组合。

Result: 在多个领域特定和通用基准测试中，SETA持续优于最先进的参数高效微调基持续学习方法。

Conclusion: SETA通过混合稀疏专家框架有效解决了LLMs持续学习中的可塑性-稳定性冲突，实现了任务无关的持续学习，显著优于现有方法。
...
### [298] [BrainDistill: Implantable Motor Decoding with Task-Specific Knowledge Distillation](https://arxiv.org/abs/2601.17625)
...
TL;DR: BrainDistill：一种用于植入式脑机接口的轻量级运动解码框架，通过任务特定知识蒸馏和量化感知训练，在保持高性能的同时大幅降低计算需求。


<details>
  <summary>Details</summary>
Motivation: Transformer等大型预训练模型在脑机接口任务上表现出色，但其庞大的参数量和计算需求限制了在功率受限的植入式系统中的部署。需要开发既能保持高性能又适合植入式系统约束的解码器。

Method: 提出BrainDistill框架，包含植入式神经解码器（IND）和任务特定知识蒸馏（TSKD）。TSKD通过监督投影优先保留对解码关键的特征，而非完整保留教师模型表示。还提出量化感知训练方案，支持整数推理并学习激活裁剪范围。

Result: IND在多个神经数据集上优于现有神经解码器。TSKD蒸馏版本在少样本校准设置中超越其他蒸馏方法。量化后的IND在严格功率约束下部署时性能损失最小。

Conclusion: BrainDistill通过任务特定知识蒸馏和量化技术，成功解决了大型Transformer模型在植入式脑机接口中部署的功率约束问题，实现了高性能与低功耗的平衡。
...
### [299] [RPNT: Robust Pre-trained Neural Transformer -- A Pathway for Generalized Motor Decoding](https://arxiv.org/abs/2601.17641)
*Hao Fang,Ryan A. Canfield,Tomohiro Ouchi,Beatrice Macagno,Eli Shlizerman,Amy L. Orsborn*

Main category: cs.LG

TL;DR: RPNT是一种鲁棒的预训练神经Transformer模型，通过多维旋转位置嵌入、上下文注意力机制和鲁棒自监督学习目标，在多任务、多会话、多被试的神经解码任务中实现优越的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 脑解码需要模型能够跨不同脑区、会话、行为类型和受试者进行泛化。现有模型只能部分解决这些挑战，需要开发能够适应和泛化的预训练神经Transformer模型。

Method: 提出RPNT模型，包含三个关键组件：1) 多维旋转位置嵌入(MRoPE)聚合实验元数据；2) 基于上下文的注意力机制通过卷积核学习局部时间结构；3) 鲁棒自监督学习目标，采用均匀因果掩码策略和对比表示。

Result: 在跨会话、跨类型、跨被试和跨脑区的下游行为解码任务中，RPNT在所有任务中都一致达到并超越了现有解码模型的性能。

Conclusion: RPNT通过创新的架构设计和预训练策略，在神经解码任务中实现了优越的泛化能力，为跨不同实验条件的脑活动解码提供了有效的解决方案。
...
### [300] [A Mosco sufficient condition for intrinsic stability of non-unique convex Empirical Risk Minimization](https://arxiv.org/abs/2601.17646)
*Karim Bounja,Lahcen Laayouni,Abdeljalil Sakat*

Main category: cs.LG

TL;DR: 该论文研究经验风险最小化(ERM)的稳定性问题，针对凸非严格损失函数产生集合值极小化器的情况，提出Painlevé-Kuratowski上半连续性作为ERM解对应的内在稳定性概念，并建立相应的定性定量分析框架。


<details>
  <summary>Details</summary>
Motivation: 传统ERM稳定性研究通常针对单值输出，但凸非严格损失函数会产生集合值极小化器。需要为ERM解对应建立合适的稳定性概念，以便理解选择函数的稳定性。

Method: 采用Painlevé-Kuratowski上半连续性(PK-u.s.c.)作为ERM解对应的内在稳定性概念。在Mosco一致扰动和局部有界极小化器的条件下，证明PK-u.s.c.、极小值连续性和消失间隙近极小化器的一致性。对于二次增长情况，推导出显式的定量偏差界。

Result: 建立了ERM解对应的稳定性理论：在最小非退化定性机制下，Mosco一致扰动和局部有界极小化器蕴含PK-u.s.c.、极小值连续性和消失间隙近极小化器的一致性。二次增长条件下获得了显式定量偏差界。

Conclusion: PK-u.s.c.是ERM解对应的内在稳定性概念，为理解选择函数的稳定性提供了先决条件。该框架统一处理了凸非严格损失函数的稳定性问题，建立了定性定量分析工具。
...
### [301] [Time-Varying Causal Treatment for Quantifying the Causal Effect of Short-Term Variations on Arctic Sea Ice Dynamics](https://arxiv.org/abs/2601.17647)
*Akila Sampath,Vandana Janeja,Jianwu Wang*

Main category: cs.LG

TL;DR: 提出KGCM-VAE模型，通过知识引导的因果建模量化海冰厚度与海面高度之间的因果关系，结合物理约束和分布平衡机制提升时空因果效应估计精度。


<details>
  <summary>Details</summary>
Motivation: 量化冰融化和淡水分布的因果关系对理解极地气候变化和全球海平面上升至关重要。传统深度学习模型在时空因果效应估计中面临未观测混杂变量和缺乏物理约束的挑战。

Method: 提出KGCM-VAE模型：1）速度调制方案，通过SSH转换控制的sigmoid函数动态放大平滑速度信号生成物理基础的因果处理；2）在潜在空间使用MMD平衡处理和对照协变量分布；3）因果邻接约束解码器确保与已知物理结构对齐。

Result: 在合成和真实北极数据集上，KGCM-VAE在PEHE指标上优于现有基准方法。消融研究表明，MMD和因果邻接约束的联合应用使估计误差降低1.88%。

Conclusion: KGCM-VAE通过整合物理知识和因果约束，有效解决了时空因果效应估计中的挑战，为理解海冰厚度与海面高度之间的因果关系提供了可靠工具。
...
### [302] [Kareus: Joint Reduction of Dynamic and Static Energy in Large Model Training](https://arxiv.org/abs/2601.17654)
*Ruofan Wu,Jae-Won Chung,Mosharaf Chowdhury*

Main category: cs.LG

TL;DR: Kareus是一个AI训练系统，通过联合优化细粒度内核调度和频率缩放，在时间和能耗之间实现更好的权衡，相比现有方法可减少最多28.3%的训练能耗或27.5%的训练时间。


<details>
  <summary>Details</summary>
Motivation: AI计算需求快速增长，但能源供应跟不上，能源已成为昂贵且竞争激烈的资源，需要明确的管理和优化。现有工作只关注动态或静态能耗的单一方面，而细粒度内核调度和频率缩放会共同影响这两种能耗。

Method: Kareus将难以处理的联合优化问题分解为局部的、基于分区的子问题，然后使用多通道多目标优化算法寻找能够推动时间-能耗权衡前沿的执行调度方案。

Result: 相比现有技术，Kareus在相同训练时间下可减少最多28.3%的训练能耗，或在相同能耗下减少最多27.5%的训练时间。

Conclusion: Kareus通过联合优化内核调度和频率缩放，有效推动了AI训练的时间-能耗权衡前沿，为解决AI计算能耗问题提供了有效方案。
...
### [303] [Entropic Risk-Aware Monte Carlo Tree Search](https://arxiv.org/abs/2601.17667)
...
<details>
  <summary>Details</summary>
Motivation: 现有的风险感知MDP求解方法在处理复杂问题时可能效率不足，需要一种结合树搜索的高效算法，同时保证理论上的正确性。

Method: 提出一种基于置信上界的蒙特卡洛树搜索算法，利用先前工作中引入的熵风险度量MDP动态规划公式，在树搜索框架中实现风险感知决策。

Result: 算法被证明是正确的（根节点经验熵风险度量收敛到最优值），且具有多项式遗憾集中性。实验表明该方法优于相关基线。

Conclusion: 该研究成功开发了一种理论保证正确且实际有效的风险感知MCTS算法，为风险敏感决策问题提供了新的解决方案。
...
### [304] [Fast KVzip: Efficient and Accurate LLM Inference with Gated KV Eviction](https://arxiv.org/abs/2601.17668)
*Jang-Hyun Kim,Dongyoon Han,Sangdoo Yun*

Main category: cs.LG

TL;DR: 提出基于门控的KV缓存淘汰方法，通过轻量级sink-attention门控模块识别关键KV对，在保持接近无损性能的同时可淘汰70%的KV缓存，计算开销极低。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存压缩技术通常在性能下降和计算开销之间需要权衡，对于冻结权重的大语言模型部署来说，需要一种既能高效压缩KV缓存又不会显著增加计算成本的方法。

Method: 提出基于门控的KV缓存淘汰方法，引入轻量级sink-attention门控模块识别和保留关键KV对；使用仅需前向传播的门训练算法，避免昂贵的反向传播；采用任务无关的重建目标实现强任务泛化能力。

Result: 在Qwen2.5-1M、Qwen3和Gemma3系列模型上的广泛实验表明，该方法在淘汰高达70%的KV缓存时仍能保持接近无损的性能，结果在长上下文理解、代码理解和数学推理等多种任务中表现一致。

Conclusion: 该方法为冻结权重的大语言模型提供了一种高效、低成本的KV缓存管理方案，在保持高性能的同时显著减少内存占用，具有很好的通用性和实用性。
...
### [305] [$\infty$-MoE: Generalizing Mixture of Experts to Infinite Experts](https://arxiv.org/abs/2601.17680)
*Shota Takashiro,Takeshi Kojima,Shohei Taniguchi,Yusuke Iwasawa,Yutaka Matsuo*

Main category: cs.LG

TL;DR: ∞-MoE：通过连续空间选择大型FFN的部分参数，实现无限专家数量，在保持计算效率的同时提升性能


<details>
  <summary>Details</summary>
Motivation: 传统MoE将专家视为完全独立且在离散空间中组合，当专家数量增加时难以有效训练每个专家。需要一种方法在增加专家数量的同时稳定训练。

Method: 提出∞-MoE，基于每个token采样的连续值选择大型前馈网络的部分参数。通过在连续空间中考虑专家，允许无限数量的专家同时保持计算效率。

Result: 基于GPT-2 Small的∞-MoE模型（1.29亿活跃参数，1.86亿总参数）达到与3.5亿参数的密集GPT-2 Medium相当的性能。在推理时调整采样专家数量可在准确性和速度之间灵活权衡，比传统MoE准确率提升高达2.5%。

Conclusion: ∞-MoE通过在连续空间中操作，解决了传统MoE在增加专家数量时的训练困难，实现了无限专家数量的可能性，同时保持了计算效率，为模型性能与计算成本之间的权衡提供了更灵活的解决方案。
...
### [306] [Agentic reinforcement learning empowers next-generation chemical language models for molecular design and synthesis](https://arxiv.org/abs/2601.17687)
*Hao Li,He Cao,Shenyao Peng,Zijing Liu,Bin Feng,Yu Wang,Zhiyuan Yan,Yonghong Tian,Yu Li,Li Yuan*

Main category: cs.LG

TL;DR: ChemCRAFT是一个利用智能体强化学习将化学推理与知识存储解耦的框架，使小型本地部署模型能够通过外部知识检索实现高性能，解决了大模型隐私风险和小模型幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 当前生化领域语言模型存在两难：小型模型易产生幻觉且知识有限，大型云端模型有隐私风险且推理成本高。需要一种既能保护隐私又成本低廉的解决方案。

Method: 1) 构建智能体轨迹构建管道和化学智能体沙箱；2) 基于沙箱交互创建ChemToolDataset数据集；3) 提出SMILES-GRPO构建密集化学奖励函数；4) 使用智能体强化学习让小型模型学习调用化学工具。

Result: ChemCRAFT在药物设计的多个方面（分子结构分析、分子优化、合成路径预测）优于当前云端大模型，证明科学推理不是模型规模的涌现能力，而是可学习的工具编排策略。

Conclusion: 该工作为AI辅助化学建立了成本效益高且保护隐私的范式，通过本地可部署的智能体为加速分子发现开辟了新途径。
...
### [307] [REV-INR: Regularized Evidential Implicit Neural Representation for Uncertainty-Aware Volume Visualization](https://arxiv.org/abs/2601.17689)
*Shanu Saklani,Tushar M. Athawale,Nairita Pal,David Pugmire,Christopher R. Johnson,Soumya Dutta*

Main category: cs.LG

TL;DR: REV-INR是一种正则化证据隐式神经表示，通过学习预测数据值及其相关的不确定性，解决了传统确定性INR无法提供预测不确定性的问题。


<details>
  <summary>Details</summary>
Motivation: 传统确定性隐式神经表示(INR)只能预测数值，无法提供模型预测不确定性或数据固有噪声的影响，这可能导致不可靠的数据解释和可视化。当原始数据因体积过大而不可用时，识别模型预测数据中的错误结果可能不可行。

Method: REV-INR（正则化证据隐式神经表示）通过学习准确预测数据值以及相关的坐标级数据不确定性和模型不确定性，在推理过程中仅需一次前向传播。该方法结合了正则化和证据理论来估计不确定性。

Result: REV-INR在体积重建质量方面表现最佳，同时提供稳健的数据（偶然性）和模型（认知性）不确定性估计，且推理时间最快。与现有深度不确定性估计方法相比，REV-INR在各方面都表现优越。

Conclusion: REV-INR能够评估提取的等值面和体积可视化结果的可靠性和可信度，使得分析可以完全基于模型预测的数据进行，为大规模体积数据集提供了更可靠的隐式神经表示方法。
...
### [308] [FedCCA: Client-Centric Adaptation against Data Heterogeneity in Federated Learning on IoT Devices](https://arxiv.org/abs/2601.17713)
*Kaile Wang,Jiannong Cao,Yu Yang,Xiaoyin Li,Yinfeng Cao*

Main category: cs.LG

TL;DR: FedCCA是一种针对物联网数据异构性的联邦学习算法，通过客户端特定编码器和动态选择机制，为每个客户端学习独特模型以提升性能


<details>
  <summary>Details</summary>
Motivation: 物联网中AI模型训练需要保护隐私数据，联邦学习是解决方案，但物联网设备间的数据异构性会显著降低模型性能和收敛速度。现有方法在固定客户端选择和云端聚合方面存在局限，难以在本地训练中提取客户端特定信息。

Method: 提出FedCCA算法：1）使用客户端特定编码器进行选择性适应；2）采用动态客户端选择机制；3）基于注意力的全局聚合策略；4）为每个客户端学习独特模型

Result: 在多个数据集上的实验表明，FedCCA在解决数据异构性问题方面相比基线方法具有显著性能优势

Conclusion: FedCCA通过客户端中心化适应策略，有效缓解了联邦学习中的数据异构性问题，提升了模型性能和收敛速度
...
### [309] [Do Reasoning Models Ask Better Questions? A Formal Information-Theoretic Analysis on Multi-Turn LLM Games](https://arxiv.org/abs/2601.17716)
*Daniel M. Pedrozo,Telma W. de L. Soares,Bryan L. M. de Oliveira*

Main category: cs.LG

TL;DR: 提出一个多轮对话框架，通过是/否问题在层次知识图环境中定量评估LLMs收集信息的能力，使用信息增益作为主要指标，在地理猜城市游戏中测试不同LLM变体。


<details>
  <summary>Details</summary>
Motivation: LLMs在许多任务上表现出色，但在LLM智能体的关键能力——通过提问解决用户请求中的模糊性方面仍有不足。现有基准缺乏基于信息增益的全面评估框架，也很少系统比较使用思维链推理和不使用思维链的模型。

Method: 提出多轮对话框架，使用三个交互的LLM智能体：提问者、回答者和假设空间更新者。采用基于香农熵的信息增益作为主要指标，在五级分类的地理猜城市游戏环境中实例化该框架，评估不同LLM变体在完全和部分可观测条件下的表现。

Result: 实验表明，在评估的模型中，具有显式推理能力的模型每轮获得更高的信息增益，且用更少步骤达到解决方案，特别是在部分可观测设置中。分析显示，较小模型通过更积极地探索候选问题来弥补有限能力，而较大模型在选择最优查询时表现出更高的自信度，生成具有更大潜在信息增益的候选问题。

Conclusion: 该框架为评估LLMs通过提问解决模糊性的能力提供了定量方法，证明了显式推理能力在信息收集任务中的重要性，并为理解不同规模模型的提问策略差异提供了见解。
...
### [310] [AR-Omni: A Unified Autoregressive Model for Any-to-Any Generation](https://arxiv.org/abs/2601.17761)
*Dongjie Cheng,Ruifeng Yuan,Yongqi Li,Runyang You,Wenjie Wang,Liqiang Nie,Lei Zhang,Wenjie Li*

Main category: cs.LG

TL;DR: AR-Omni是一个统一的自回归多模态模型，支持文本、图像和语音的任意到任意生成，无需专家解码器，在单一Transformer解码器下实现实时多模态生成。


<details>
  <summary>Details</summary>
Motivation: 现实世界的感知和交互本质上是多模态的，但现有系统通常依赖额外的专家组件来实现多模态生成，限制了统一训练和推理的简洁性。自回归建模在文本领域已被证明是优雅且可扩展的基础，因此希望将其扩展到多模态领域。

Method: 提出AR-Omni模型，在单一Transformer解码器下支持文本、图像和流式语音生成。通过任务感知损失重加权解决模态不平衡问题，通过轻量级token级感知对齐损失提高视觉保真度，通过有限状态解码机制平衡稳定性和创造性。

Result: AR-Omni在三个模态上均实现了强大的生成质量，同时保持实时性，语音生成的实时因子达到0.88。

Conclusion: AR-Omni展示了自回归范式在多模态生成中的可行性，通过统一的架构实现了简洁高效的多模态生成，为真正的任意到任意多模态交互奠定了基础。
...
### [311] [LLM-42: Enabling Determinism in LLM Inference with Verified Speculation](https://arxiv.org/abs/2601.17768)
*Raja Gond,Aditya K Kamath,Arkaprava Basu,Ramachandran Ramjee,Ashish Panwar*

Main category: cs.LG

TL;DR: LLM-42通过调度方法实现LLM推理的确定性，使用非确定性快速路径解码，通过轻量级验证-回滚循环确保一致性，避免动态批处理关闭带来的吞吐量损失。


<details>
  <summary>Details</summary>
Motivation: LLM推理中的非确定性源于浮点非结合性与动态批处理及GPU内核的交互，传统方法要么关闭动态批处理严重降低吞吐量，要么需要重新设计内核且带来固定开销。

Method: 基于推测解码思想，使用非确定性快速路径解码令牌，通过轻量级验证-回滚循环确保确定性。验证器在固定形状规约调度下重放候选令牌，提交保证一致性的令牌，回滚违反确定性的令牌。

Result: LLM-42大部分重用现有内核，仅对需要确定性的流量产生开销，在保持高吞吐量的同时实现确定性推理。

Conclusion: LLM-42提供了一种调度基础的确定性LLM推理方法，解耦了确定性与内核设计，仅在实际需要确定性的工作负载上产生开销，实现了效率与确定性的平衡。
...
### [312] [Shortcut Learning in Binary Classifier Black Boxes: Applications to Voice Anti-Spoofing and Biometrics](https://arxiv.org/abs/2601.17782)
*Md Sahidullah,Hye-jin Shim,Rosa Gonzalez Hautamäki,Tomi H. Kinnunen*

Main category: cs.LG

TL;DR: 提出一个分析黑盒分类器的新框架，通过干预和观察视角结合线性混合效应模型，检测数据集偏差和"捷径学习"效应，在音频反欺骗和说话人验证任务中验证有效性。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在数据驱动应用中的广泛使用引发了人们对数据集和模型偏见的关注。被忽视或隐藏的偏见可能导致意外结果，特别是"捷径学习"或"Clever Hans效应"在二元分类器中可能带来风险。

Method: 提出一个新颖的分析框架，结合干预和观察视角，采用线性混合效应模型进行事后分析。该框架不仅评估训练数据，还检查测试数据对分类器分数的影响，超越传统的错误率评估。

Result: 在音频反欺骗和说话人验证任务中，使用统计模型和深度神经网络验证了方法的有效性。该框架能够提供对偏见数据集的深入理解，并揭示分类器行为的影响因素。

Conclusion: 该方法为检测数据集偏见和"捷径学习"效应提供了有效工具，对解决其他领域的偏见问题具有广泛意义，并推动了可解释人工智能领域的发展。
...
### [313] [Robust Computational Extraction of Non-Enhancing Hypercellular Tumor Regions from Clinical Imaging Data](https://arxiv.org/abs/2601.17802)
*A. Brawanski,Th. Schaffer,F. Raab,K. -M. Schebesch,M. Schrey,Chr. Doenitz,A. M. Tomé,E. W. Lang*

Main category: cs.LG

TL;DR: 提出一个从常规MRI数据生成非增强高细胞性肿瘤区域概率图的稳健计算框架，通过多网络架构解决成像边界模糊问题，并验证其临床相关性。


<details>
  <summary>Details</summary>
Motivation: 神经肿瘤成像中准确识别非增强高细胞性肿瘤区域是一个未满足的需求，对患者管理和治疗规划有重要意义。当前缺乏清晰的成像边界和存在固有变异性。

Method: 开发了一个稳健的计算框架，利用多种网络架构从常规MRI数据生成NEH区域的概率图，解决了成像边界模糊和变异性的问题。

Result: 该框架通过独立临床标志物（相对脑血容量和增强肿瘤复发位置）验证，显示出方法学稳健性和生物学相关性，能够可靠地非侵入性映射NEH肿瘤区域。
...
### [314] [MergeMix: Optimizing Mid-Training Data Mixtures via Learnable Model Merging](https://arxiv.org/abs/2601.17858)
...
<details>
  <summary>Details</summary>
Motivation: 当前优化语言模型数据混合比例主要依赖启发式试验或昂贵的代理训练，计算成本过高，需要更高效的优化方法

Method: 通过训练领域专家模型（仅需少量token），优化其融合权重来评估下游任务性能，以此作为数据混合比例优化的代理指标

Result: 在8B和16B参数模型上验证，MergeMix性能达到或超过人工调优，搜索成本大幅降低，且具有高秩一致性和跨尺度可迁移性

Conclusion: MergeMix提供了一种可扩展、自动化的数据混合优化解决方案，显著降低了优化成本
...
### [315] [EEG Foundation Models: Progresses, Benchmarking, and Open Problems](https://arxiv.org/abs/2601.17883)
*Dingkun Liu,Yuheng Chen,Zhu Chen,Zhenyao Cui,Yaozhi Wen,Jiayu An,Jingwei Luo,Dongrui Wu*

Main category: cs.LG

TL;DR: 该论文对EEG基础模型进行了系统性综述与评估，通过统一框架比较了50个代表性模型，并在13个数据集上测试了12个开源模型，发现线性探测常不足、专业模型仍具竞争力、更大模型未必带来更好泛化性能。
...
Method: 首先回顾50个代表性模型，将其设计选择组织成统一分类框架（数据标准化、模型架构、自监督预训练策略）。然后评估12个开源基础模型和竞争性专业基线模型，在13个EEG数据集上测试9种BCI范式，考虑跨被试泛化和少样本快速校准两种场景。

Result: 研究结果表明：1）线性探测通常不足；2）从头训练的专业模型在许多任务中仍具竞争力；3）在当前数据规模和训练实践下，更大的基础模型不一定带来更好的泛化性能。

Conclusion: 该研究为EEG基础模型提供了系统性评估框架和实证结果，揭示了当前方法的局限性，为未来研究提供了重要参考。
...
### [316] [Adaptive Weighting in Knowledge Distillation: An Axiomatic Framework for Multi-Scale Teacher Ensemble Optimization](https://arxiv.org/abs/2601.17910)
...
<details>
  <summary>Details</summary>
Motivation: 多教师知识蒸馏虽然能提升鲁棒性、效率和安全性，但现有方法主要依赖启发式或实现特定的权重方案，缺乏统一的理论框架。

Method: 开发了一个算子无关的公理化框架，形式化了自适应权重算子的结构条件，包括良好定义性、多重非等价实现可能性，以及通过乘积结构归一化进行分层组合的能力。

Result: 证明了符合条件算子的存在性和非唯一性，在标准假设下分析了梯度优化的收敛性，研究了稳定性和扰动鲁棒性，并提供了安全约束蒸馏的抽象表述。

Conclusion: 该框架将理论保证与具体权重公式解耦，使得在异构性、分布偏移和安全约束下对自适应蒸馏方法进行原则性分析成为可能。
...
### [317] [Causal Pre-training Under the Fairness Lens: An Empirical Study of TabPFN](https://arxiv.org/abs/2601.17912)
*Qinyi Liu,Mohammad Khalil,Naman Goel*

Main category: cs.LG

TL;DR: TabPFN等表格数据基础模型通过因果预训练获得高预测精度，但其公平性表现中等且不稳定，尤其在MNAR协变量偏移下，表明因果预训练对公平性有帮助但不足。


<details>
  <summary>Details</summary>
Motivation: 虽然TabPFN等表格基础模型通过大规模SCM生成数据预训练，利用上下文学习在真实任务中表现出高预测精度，但这些融入因果推理思想的模型公平性尚未得到充分探索。

Method: 对TabPFN及其微调变体进行全面的实证评估，评估不同数据集规模和分布偏移下的预测性能、公平性和鲁棒性。

Result: TabPFN相比基线具有更强的预测准确性，对虚假相关性表现出鲁棒性，但公平性改进中等且不一致，特别是在MNAR协变量偏移下。

Conclusion: TabPFN中的因果预训练对公平性有帮助但不足，这对实际部署此类模型具有启示意义，并凸显了需要进一步公平性干预的必要性。
...
### [318] [UniPACT: A Multimodal Framework for Prognostic Question Answering on Raw ECG and Structured EHR](https://arxiv.org/abs/2601.17916)
*Jialu Tang,Tong Xia,Yuan Lu,Aaqib Saeed*

Main category: cs.LG

TL;DR: UniPACT是一个统一框架，通过结构化提示将数值型EHR数据转换为语义丰富的文本，并与原始ECG波形表示融合，使LLM能够对两种模态进行整体推理，在MDS-ED基准测试中实现了最先进的预后性能。


<details>
  <summary>Details</summary>
Motivation: 准确的临床预后需要整合结构化电子健康记录（EHR）和实时生理信号（如心电图ECG）。大型语言模型（LLMs）为此任务提供了强大的推理引擎，但难以原生处理这些异构的非文本数据类型。

Method: 提出UniPACT框架，核心贡献是结构化提示机制，将数值EHR数据转换为语义丰富的文本。然后将这种文本化的患者上下文与从原始ECG波形直接学习的表示融合，使LLM能够对两种模态进行整体推理。

Result: 在综合MDS-ED基准测试中，UniPACT在包括诊断、病情恶化、ICU入院和死亡率在内的多样化预后任务上实现了89.37%的平均AUROC，优于专门的基线方法。多模态多任务方法对性能至关重要，并在数据缺失场景中提供鲁棒性。

Conclusion: UniPACT通过弥合模态差距，成功地将结构化EHR数据与原始ECG信号整合到LLM推理中，为临床预后问题回答提供了一个统一且有效的框架，在多任务预后评估中实现了最先进的性能。
...
### [319] [treaming-dLLM: Accelerating Diffusion LLMs via Suffix Pruning and Dynamic Decoding](https://arxiv.org/abs/2601.17917)
*Zhongyu Xiao,Zhiwei Hao,Jianyuan Guo,Yong Luo,Jia Liu,Jie Xu,Han Hu*

Main category: cs.LG

TL;DR: Streaming-dLLM是一个训练免费框架，通过空间维度上的衰减引导后缀建模和时间维度上的动态置信感知策略，显著加速扩散大语言模型推理，最高可达68.2倍加速


<details>
  <summary>Details</summary>
Motivation: 现有扩散大语言模型推理加速方法存在两个核心问题：空间冗余（对信息稀疏的后缀区域进行统一建模）和时间效率低下（在整个解码过程中使用固定的去噪调度）

Method: 提出Streaming-dLLM框架：1）空间维度：通过衰减引导后缀建模，修剪冗余的掩码标记来近似完整上下文；2）时间维度：采用动态置信感知策略和提前退出机制，让模型跳过已收敛标记的不必要迭代

Result: 实验表明Streaming-dLLM在保持生成质量的同时，最高可实现68.2倍的推理加速，显著提升了扩散大语言模型的解码效率

Conclusion: Streaming-dLLM有效解决了扩散大语言模型推理中的空间冗余和时间效率问题，为扩散模型的实际应用提供了高效的推理框架
...
### [320] [Dissipative Learning: A Framework for Viable Adaptive Systems](https://arxiv.org/abs/2601.17933)
*Laurent Caraffa*

Main category: cs.LG

TL;DR: 学习被重新定义为在耗散约束下维持可行信念状态的过程，遗忘和正则化是自适应系统的结构要求而非启发式附加组件


<details>
  <summary>Details</summary>
Motivation: 传统机器学习将遗忘和正则化视为启发式技术，缺乏统一的理论框架。本文旨在从信息理论、热力学和信息几何的角度，为学习过程提供更基础的理解，将学习视为本质上耗散的过程

Method: 提出BEDS（贝叶斯涌现耗散结构）框架，将学习建模为在耗散约束下压缩信念状态的演化。引入条件最优性定理，证明Fisher-Rao正则化是唯一热力学最优的正则化策略。该框架统一了现有方法（Ridge、SIGReg、EMA、SAC）

Result: 证明了Fisher-Rao正则化在信息散度度量下达到最小耗散，而欧几里得正则化是结构上次优的。框架区分了BEDS可结晶问题（信念收敛到稳定平衡）和BEDS可维持问题（需要持续适应）。该框架自然地扩展到持续学习和多智能体系统

Conclusion: 学习应被重新定义为在耗散约束下维持可行信念状态的过程，这为遗忘、正则化和稳定性提供了原则性的理论视角。可行性、适应稳定性和有限资源下的稳定性取代渐近最优性成为主要标准
...
### [321] [FedGraph-VASP: Privacy-Preserving Federated Graph Learning with Post-Quantum Security for Cross-Institutional Anti-Money Laundering](https://arxiv.org/abs/2601.17935)
*Daniel Commey,Matilda Nkoom,Yousef Alsenani,Sena G. Hounsinou,Garth V. Crosby*

Main category: cs.LG

TL;DR: FedGraph-VASP：基于联邦图学习的隐私保护反洗钱框架，通过边界嵌入交换协议在保护用户隐私的同时实现跨机构洗钱检测，在连接性高的交易图中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 虚拟资产服务提供商面临监管合规与用户隐私之间的根本矛盾，现有方法要么需要共享敏感交易数据，要么孤立运行，无法检测关键的跨链洗钱模式。

Method: 提出FedGraph-VASP框架，采用边界嵌入交换协议，仅共享压缩、不可逆的图神经网络边界账户表示，并使用后量子密码学（Kyber-512和AES-256-GCM）保障安全。

Result: 在Elliptic比特币数据集上，FedGraph-VASP的F1分数达到0.508，优于FedSage+的0.453（提升12.1%）。在连接性高的场景下接近集中式性能（F1=0.620），但在稀疏连接场景下效果不如FedSage+。

Conclusion: FedGraph-VASP在保护隐私的同时有效检测跨机构洗钱，但存在拓扑依赖的权衡：嵌入交换适用于连接性高的交易图，而生成式插补在高度模块化的稀疏图中表现更好。
...
### [322] [Scaling Effects and Uncertainty Quantification in Neural Actor Critic Algorithms](https://arxiv.org/abs/2601.17954)
*Nikos Georgoudios,Konstantinos Spiliopoulos,Justin Sirignano*

Main category: cs.LG

TL;DR: 研究神经Actor-Critic算法在不同网络宽度缩放方案下的收敛性和统计特性，提出可调超参数控制近似误差，推导出网络输出方差随宽度衰减的规律，为超参数选择提供理论指导。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注收敛速度，但缺乏对神经Actor-Critic算法输出统计特性的全面分析。本文旨在量化算法的不确定性，提供更全面的统计表征，以改进算法的统计鲁棒性。

Method: 研究浅层神经网络Actor-Critic模型，采用一般逆多项式缩放方案（指数作为可调超参数在0.5到1之间），推导网络输出的渐近展开，分析其作为统计估计量的结构。

Result: 方差随网络宽度以幂律衰减，指数为0.5减去缩放参数，表明当缩放参数接近1时统计鲁棒性更好。数值实验支持该行为并显示更快的收敛速度。

Conclusion: 通过分析缩放参数对统计特性的影响，为学习率和探索率等超参数选择提供具体指导，确保算法具有可证明的有利统计行为。
...
### [323] [TensorLens: End-to-End Transformer Analysis via High-Order Attention Tensors](https://arxiv.org/abs/2601.17958)
...
<details>
  <summary>Details</summary>
Motivation: 现有注意力分析大多关注单个注意力头或层，忽略了模型的全局行为。虽然已有研究尝试通过平均和矩阵乘法扩展注意力公式，或纳入归一化、FFN等组件，但仍缺乏一个统一完整的表示来封装所有transformer块。

Method: 提出TensorLens方法，将整个transformer表示为单一输入依赖线性算子，通过高阶注意力交互张量联合编码注意力、FFN、激活、归一化和残差连接，提供理论上连贯且表达力强的线性表示。

Result: 经验验证表明TensorLens比之前的注意力聚合方法产生更丰富的表示。实验证明注意力张量可以作为开发可解释性和模型理解工具的强大基础。

Conclusion: TensorLens填补了transformer全局表示的理论空白，为模型理解提供了更全面、统一的框架，支持更丰富的可解释性分析工具开发。
...
### [324] [Federated learning for unpaired multimodal data through a homogeneous transformer model](https://arxiv.org/abs/2601.17986)
*Anders Eklund*

Main category: cs.LG

TL;DR: 提出联邦无配对多模态基础模型训练框架，通过公共锚点集对齐分散的私有数据流形，无需传输私有样本或共享对齐数据对


<details>
  <summary>Details</summary>
Motivation: 现实联邦环境中数据通常是未配对且分散在不同节点上的（如图像和文本分别存储在不同节点），现有联邦学习方法假设本地客户端拥有对齐的数据对或需要共享原始特征嵌入，这违反了数据主权

Method: 1) 引入小型公共锚点集对齐分散的私有流形；2) 使用基于公共锚点计算的Gram矩阵通过中心核对齐实现跨模态语义对齐；3) 提出子空间稳定微调方法处理大型transformer模型；4) 提出精度加权平均，利用不确定性估计降低不确定节点的权重

Result: 建立了联邦无配对基础模型的数学基础，使全局模型能够从分散、未配对且私有的数据孤岛中学习世界的统一表示，无需集中存储或配对样本

Conclusion: 该框架为联邦无配对基础模型提供了数学基础，能够在保护数据主权的前提下，从分散、未配对且私有的多模态数据中训练全局模型，相比原型共享提供更优越的隐私保证
...
### [325] [Systematic Characterization of Minimal Deep Learning Architectures: A Unified Analysis of Convergence, Pruning, and Quantization](https://arxiv.org/abs/2601.17987)
*Ziwei Zheng,Huizhi Liang,Vaclav Snasel,Vito Latora,Panos Pardalos,Giuseppe Nicosia,Varun Ojha*

Main category: cs.LG

TL;DR: 该研究提出一种系统探索收敛性、剪枝和量化关系的计算方法，发现尽管架构多样，性能基本不变且学习动态呈现三个稳定阶段，揭示了最小可学习参数需求、不同收敛剪枝阶段，以及量化对参数较少模型影响更大。


<details>
  <summary>Details</summary>
Motivation: 深度学习网络在分类任务上表现出色，但确定能够可靠完成任务的最小架构仍然具有挑战性。需要系统探索和分析收敛性、剪枝和量化之间的关系，为在剪枝和低精度约束下选择紧凑稳定模型提供指导。

Method: 提出计算方法论：首先在大量架构上进行结构化设计扫描，然后在代表性模型上评估收敛行为、剪枝敏感性和量化鲁棒性。研究聚焦于复杂度递增的知名图像分类任务，涵盖深度神经网络、卷积神经网络和视觉变换器。

Result: 尽管架构多样，性能基本不变且学习动态一致呈现三个阶段：不稳定、学习和过拟合。确定了稳定学习所需的最小可学习参数，揭示了不同的收敛和剪枝阶段，量化了降低数值精度对可训练参数的影响。更深架构比浅层架构对剪枝更具韧性，参数冗余高达60%，量化对参数较少的模型影响更严重，对更难图像数据集影响更大。

Conclusion: 研究结果为在图像分类任务中，在剪枝和低精度约束下选择紧凑稳定模型提供了可操作的指导。这些发现有助于理解深度学习架构的基本特性，并为模型压缩和优化提供实用见解。
...
### [326] [Coding-Enforced Resilient and Secure Aggregation for Hierarchical Federated Learning](https://arxiv.org/abs/2601.17995)
...
<details>
  <summary>Details</summary>
Motivation: 分层联邦学习(HFL)虽然能改善客户端与服务器之间的链路质量，但在不可靠通信环境下，如何在保证模型精度的同时保护隐私仍然是一个关键挑战，因为隐私噪声的协调可能被随机破坏

Method: 提出鲁棒的分层安全聚合方案H-SecCoGC，集成编码策略来强制执行结构化聚合，避免部分参与问题

Result: 该方案在任意强隐私保证下都能确保准确的全局模型构建，显著提高了鲁棒性、隐私保护和学习效率

Conclusion: 理论分析和实验结果都证明了该方案在不可靠通信环境下的优越性，能够有效解决分层联邦学习中的隐私保护与模型精度平衡问题
...
### [327] [Spelling Bee Embeddings for Language Modeling](https://arxiv.org/abs/2601.18030)
*Markus N. Rabe,Judith Clymo,Zheren Dong*

Main category: cs.LG

TL;DR: 通过将拼写信息融入词嵌入层，模型在拼写和标准基准测试上都有提升，相当于节省约8%的计算和数据达到相同性能


<details>
  <summary>Details</summary>
Motivation: 现有词嵌入方法通常忽略单词的拼写信息，而拼写信息可能包含有用的语言特征，有助于提升模型性能

Method: 对嵌入层进行简单修改，将词嵌入与拼写信息融合，在40M到800M参数的模型上进行缩放研究

Result: 使用这种嵌入的模型不仅在拼写任务上表现更好，在标准基准测试上也有提升，相当于节省约8%的计算和数据达到相同测试损失

Conclusion: 将拼写信息融入词嵌入是一种简单有效的改进方法，能显著提升模型性能并减少训练资源需求
...
### [328] [Multimodal Machine Learning for Soft High-k Elastomers under Data Scarcity](https://arxiv.org/abs/2601.18032)
...
<details>
  <summary>Details</summary>
Motivation: 软电子和可拉伸电子学快速发展，需要同时具备高介电常数和低杨氏模量的介电弹性体，但缺乏系统性的数据集和有效的数据驱动设计方法

Method: 1) 整理过去10年文献中的丙烯酸酯基介电弹性体实验数据构建高质量数据集；2) 提出多模态学习框架，利用基于图和序列的编码器获取预训练聚合物表示；3) 通过知识迁移实现分子序列到介电和机械性能的少样本预测

Result: 成功开发了能够准确预测介电弹性体性能的多模态学习模型，克服了数据稀缺问题，为其他聚合物体系（如硅酮、聚氨酯）提供了可推广的范式

Conclusion: 该研究提出了一种通过预训练多模态模型进行知识迁移的新范式，能够加速数据高效的软高介电常数弹性体发现，代码和数据集已开源
...
### [329] [Resonant Sparse Geometry Networks](https://arxiv.org/abs/2601.18064)
*Hasi Hays*

Main category: cs.LG

TL;DR: RSGN是一种受大脑启发的稀疏几何网络，通过在双曲空间中嵌入计算节点实现输入依赖的动态稀疏连接，相比Transformer具有O(n*k)的计算复杂度，参数更少但性能相当。


<details>
  <summary>Details</summary>
Motivation: Transformer架构使用密集注意力机制，计算复杂度为O(n^2)，参数需求大。受大脑稀疏连接和几何组织的启发，研究者希望开发更高效、生物合理的神经网络架构。

Method: RSGN将计算节点嵌入学习的双曲空间，连接强度随测地距离衰减，实现动态稀疏连接。采用双时间尺度：快速可微分激活传播（梯度下降优化）和慢速Hebbian启发结构学习（局部相关性规则）。

Result: 在长距离依赖任务上达到96.5%准确率，参数比标准Transformer少约15倍。在20类层次分类任务上达到23.8%准确率（随机基线5%），仅需41,672参数，而Transformer需要403,348参数达到30.1%准确率。

Conclusion: 受大脑启发的稀疏几何计算原则为开发更高效、生物合理的神经网络架构提供了有前景的方向，RSGN在保持性能的同时显著减少了计算复杂度和参数需求。
...
### [330] [Comparison requires valid measurement: Rethinking attack success rate comparisons in AI red teaming](https://arxiv.org/abs/2601.18076)
*Alexandra Chouldechova,A. Feder Cooper,Solon Barocas,Abhinav Palia,Dan Vann,Hanna Wallach*

Main category: cs.LG

TL;DR: 论文指出当前AI红队测试中基于攻击成功率(ASR)比较得出的系统安全性或攻击方法有效性结论往往缺乏证据支持，存在苹果与橙子的不当比较和低效度测量问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI安全评估中，研究人员经常通过比较不同攻击方法的成功率来评估系统安全性或攻击方法有效性，但这些比较往往缺乏方法论上的严谨性，导致结论不可靠。

Method: 采用社会科学测量理论和推断统计学框架，提出攻击成功率可比较性的概念基础，以越狱攻击为例，分析ASR比较中的不当比较和测量效度问题。

Result: 研究表明许多ASR比较属于苹果与橙子的不当比较，测量效度低，需要满足特定条件才能进行有意义的比较，为AI安全评估提供了更严谨的方法论基础。

Conclusion: AI红队测试中的攻击成功率比较需要更严谨的方法论框架，基于社会科学测量理论的方法能够提供更有意义的比较基础，避免得出误导性结论。
...
### [331] [DRPG (Decompose, Retrieve, Plan, Generate): An Agentic Framework for Academic Rebuttal](https://arxiv.org/abs/2601.18081)
*Peixuan Han,Yingjie Yu,Jingjun Xu,Jiaxuan You*

Main category: cs.LG

TL;DR: DRPG是一个用于自动生成学术反驳的智能体框架，通过分解审稿意见、检索论文证据、规划反驳策略和生成回应四步，显著优于现有方法，甚至超越人类平均水平。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在科研工作流中应用广泛，但学术反驳这一关键环节的自动化支持仍未被充分探索。现有方法通常依赖现成大语言模型或简单流水线，难以处理长文本理解，且无法生成有针对性和说服力的回应。

Method: 提出DRPG框架，包含四个步骤：1) 将审稿意见分解为原子化问题；2) 从论文中检索相关证据；3) 规划反驳策略；4) 基于策略生成回应。特别地，规划器在识别最可行反驳方向方面达到超过98%的准确率。

Result: 在顶级会议数据上的实验表明，DRPG显著优于现有反驳流水线，仅使用8B参数模型就达到了超越人类平均水平的性能。分析显示规划器设计有效，能提供多视角和可解释的建议，且在更复杂的多轮设置中表现良好。

Conclusion: DRPG框架有效展示了提供高质量反驳内容和支持学术讨论扩展的潜力，为学术交流自动化提供了有前景的解决方案。
...
### [332] [LatentMoE: Toward Optimal Accuracy per FLOP and Parameter in Mixture of Experts](https://arxiv.org/abs/2601.18089)
...
TL;DR: LatentMoE是一种新的混合专家模型架构，通过硬件-软件协同设计优化推理成本，在准确率每FLOP和每参数指标上优于标准MoE架构。


<details>
  <summary>Details</summary>
Motivation: 尽管MoE已成为许多最先进大语言模型的核心组件，但现有MoE架构在推理成本（以准确率每FLOP和每参数衡量）方面是否接近最优仍不清楚。需要从硬件-软件协同设计角度重新审视MoE设计。

Method: 采用硬件-软件协同设计方法，分析不同部署场景下的性能瓶颈，包括离线高吞吐量执行和在线延迟关键推理。通过系统化的设计空间探索（高达95B参数和1T token训练规模）和理论分析，开发了LatentMoE架构。

Result: LatentMoE在准确率每FLOP和每参数指标上持续优于标准MoE架构。该架构已被Nemotron-3 Super和Ultra旗舰模型采用，并扩展到更大规模（更长token范围和更大模型尺寸）。

Conclusion: 通过硬件-软件协同设计方法开发的LatentMoE架构在推理成本效率方面显著优于现有MoE设计，为大规模语言模型提供了更优的架构选择。
...
### [333] [From LLMs to LRMs: Rethinking Pruning for Reasoning-Centric Models](https://arxiv.org/abs/2601.18091)
*Longwei Ding,Anhao Zhao,Fanghua Ye,Ziyang Chen,Xiaoyu Shen*

Main category: cs.LG

TL;DR: 该研究对比了指令跟随型LLM和推理增强型LLM的剪枝策略，发现不同范式需要不同的剪枝方法，深度剪枝在分类任务上表现更好，宽度剪枝在生成和推理任务上更稳健。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注指令跟随型LLM的剪枝，但缺乏对推理增强型模型（生成长中间推理轨迹）剪枝策略的研究，需要明确现有剪枝方法是否适用于这类模型。

Method: 对指令跟随型LLM和推理增强型LLM进行对照研究，采用与原始训练分布对齐的剪枝校准和后剪枝恢复数据，评估静态深度剪枝、静态宽度剪枝和动态剪枝在17个任务上的表现。

Result: 发现明显的范式依赖差异：深度剪枝在分类任务上优于宽度剪枝，而宽度剪枝在生成和推理任务上更稳健；静态剪枝能更好地保持推理性能，动态剪枝在分类和生成任务上表现优异但在长链推理上仍有挑战。

Conclusion: 推理增强型LLM需要专门考虑其独特特性的剪枝策略，不能简单沿用指令跟随型模型的剪枝方法，这为高效部署推理增强模型提供了重要指导。
...
### [334] [Beyond Static Datasets: Robust Offline Policy Optimization via Vetted Synthetic Transitions](https://arxiv.org/abs/2601.18107)
*Pedram Agand,Mo Chen*

Main category: cs.LG

TL;DR: MoReBRAC：基于模型的不确定性感知离线强化学习框架，通过双重循环世界模型合成高质量转移数据，利用VAE流形检测、模型敏感性分析和MC dropout分层过滤，在D4RL基准上显著提升性能


<details>
  <summary>Details</summary>
Motivation: 离线强化学习在工业机器人等安全关键领域有巨大潜力，但面临静态数据集与学习策略之间的分布偏移问题，通常需要高度保守的方法，限制了策略改进潜力

Method: 提出MoReBRAC框架：1）使用双重循环世界模型合成高保真转移数据扩展训练流形；2）通过分层不确定性管道（VAE流形检测、模型敏感性分析、MC dropout）确保合成数据的可靠性，只使用学习动态高置信区域的转移

Result: 在D4RL Gym-MuJoCo基准测试中取得显著性能提升，特别是在"随机"和"次优"数据机制下；深入分析了VAE作为几何锚点的作用，并讨论了从近最优数据集学习时的分布权衡

Conclusion: MoReBRAC通过不确定性感知的潜在合成有效解决了离线强化学习中的分布偏移问题，在保持安全性的同时显著提升了策略性能，为安全关键领域的离线RL应用提供了有前景的解决方案
...
### [335] [AttenMIA: LLM Membership Inference Attack through Attention Signals](https://arxiv.org/abs/2601.18110)
*Pedram Zaree,Md Abdullah Al Mamun,Yue Dong,Ihsen Alouani,Nael Abu-Ghazaleh*

Main category: cs.LG

TL;DR: AttenMIA：利用Transformer自注意力模式进行成员推断攻击的新框架，通过分析注意力头模式和扰动差异来识别训练数据成员，在多个LLM上显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 大型语言模型训练数据记忆化引发隐私和知识产权担忧，现有成员推断攻击主要依赖输出置信度或嵌入特征，但这些信号脆弱且攻击成功率有限，需要更有效的攻击方法

Method: 提出AttenMIA框架，利用Transformer内部自注意力模式推断成员身份：1）分析注意力头在不同层的模式；2）结合基于扰动的差异度量；3）训练有效的MIA分类器；4）在多个开源模型（LLaMA-2、Pythia、Opt）上进行实验

Result: 注意力特征始终优于基线方法，在低误报率指标下表现突出（WikiMIA-32基准上达到0.996 ROC AUC和87.9% TPR@1%FPR），注意力信号在不同数据集和架构间具有泛化性，在数据提取框架中应用AttenMIA能超越现有最优方法

Conclusion: 注意力机制虽然最初用于增强可解释性，却无意中放大了LLM的隐私风险，揭示了需要新的防御措施来应对基于注意力模式的成员推断攻击
...
### [336] [Demystifying Data-Driven Probabilistic Medium-Range Weather Forecasting](https://arxiv.org/abs/2601.18111)
...
TL;DR: 该研究提出一个可扩展的多尺度大气动力学学习框架，通过直接下采样潜在空间和历史条件局部投影器实现高分辨率物理解析，证明无需复杂架构或专门训练策略即可达到最先进的概率预报技能。


<details>
  <summary>Details</summary>
Motivation: 当前数据驱动的天气预报方法存在架构复杂、训练策略碎片化的问题，这掩盖了预报准确性的根本驱动因素。研究旨在证明最先进的概率预报技能并不需要复杂的架构约束或专门的训练启发式方法。

Method: 引入一个可扩展框架，结合直接下采样的潜在空间和历史条件局部投影器来学习多尺度大气动力学。该框架设计对概率估计器的选择具有鲁棒性，支持随机插值、扩散模型和CRPS集成训练。

Result: 与集成预报系统（IFS）和深度学习概率模型GenCast相比，该框架在大多数变量上实现了统计显著的改进。验证了该方法的有效性。

Conclusion: 扩展通用模型足以实现最先进的中期预报，无需定制化训练方案，且在完整的概率框架范围内都有效。这简化了天气预报的深度学习架构设计。
...
### [337] [Robust Learning of a Group DRO Neuron](https://arxiv.org/abs/2601.18115)
*Guyang Cao,Shuyao Li,Sushrut Karmalkar,Jelena Diakonikolas*

Main category: cs.LG

TL;DR: 该论文研究在任意标签噪声和组级分布偏移下学习单个神经元的问题，提出一种计算高效的原始-对偶算法，能在最坏情况组权重下获得与最优参数竞争的结果。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据常存在标签噪声和分布偏移，传统方法在这些挑战下性能下降。需要开发能在最坏情况组分布下保持鲁棒性的学习算法。

Method: 提出Group Distributionally Robust Optimization框架，使用f-散度惩罚偏离均匀组权重。开发计算高效的原始-对偶算法，直接处理损失函数的非凸性，通过双重外推更新实现。

Result: 算法输出参数向量在常数因子内与最优参数竞争，在最坏情况组权重下保持性能。算法在LLM预训练基准上显示出潜力。

Conclusion: 该工作为在标签噪声和分布偏移下学习神经元提供了鲁棒的理论保证和实用算法，扩展了分布鲁棒优化的应用范围。
...
### [338] [Enhance the Safety in Reinforcement Learning by ADRC Lagrangian Methods](https://arxiv.org/abs/2601.18142)
*Mingxu Zhang,Huicheng Zhang,Jiaming Ji,Yaodong Yang,Ying Sun*

Main category: cs.LG

TL;DR: 提出ADRC-Lagrangian方法，将主动抗扰控制集成到安全强化学习中，显著减少安全违规和振荡，相比传统方法提升安全性能


<details>
  <summary>Details</summary>
Motivation: 现有安全强化学习方法（如Lagrangian和PID方法）存在振荡频繁、安全违规多的问题，主要由于参数敏感性和相位滞后导致，需要更鲁棒的控制方法

Method: 提出ADRC-Lagrangian框架，将主动抗扰控制（ADRC）集成到安全强化学习中，增强鲁棒性并减少振荡，该统一框架包含传统Lagrangian和PID方法作为特例

Result: 实验表明该方法减少安全违规达74%，约束违规幅度降低89%，平均成本降低67%，在复杂环境中表现出优越的安全性能

Conclusion: ADRC-Lagrangian方法为安全强化学习提供了更鲁棒的解决方案，显著改善安全性能，减少振荡和违规，适用于复杂环境
...
### [339] [FP8-RL: A Practical and Stable Low-Precision Stack for LLM Reinforcement Learning](https://arxiv.org/abs/2601.18150)
*Zhaopeng Qiu,Shuang Yu,Jingqi Zhang,Shuai Zhang,Xue Huang,Jingyi Yang,Junjie Lai*

Main category: cs.LG

TL;DR: 提出用于大语言模型强化学习的FP8推理栈，通过FP8量化、KV缓存优化和重要性采样校正，实现高达44%的推理吞吐量提升，同时保持与BF16基准相当的学习性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型强化学习中的推理（生成）阶段成为瓶颈，长序列导致注意力机制和KV缓存内存占用主导端到端时间。FP8量化可加速推理，但在RL中面临独特挑战：策略权重频繁变化需要重复量化，低精度推理与高精度训练不匹配可能导致不稳定。

Method: 1) 使用块级FP8量化实现W8A8线性层推理；2) 通过每步QKV尺度重新校准将FP8扩展到KV缓存，解决长上下文内存瓶颈；3) 使用基于重要性采样的推理校正（token级TIS/MIS变体）缓解训练-推理不匹配。

Result: 在密集和MoE模型上，该技术实现了高达44%的推理吞吐量提升，同时学习行为与BF16基准相当，有效解决了FP8在RL应用中的工程和算法挑战。

Conclusion: 提出的FP8推理栈为LLM强化学习提供了实用的低精度推理解决方案，通过量化、内存优化和校正技术，在保持学习性能的同时显著提升推理效率，解决了FP8在动态RL环境中的独特挑战。
...
### [340] [Learning Fair Domain Adaptation with Virtual Label Distribution](https://arxiv.org/abs/2601.18171)
*Yuguang Zhang,Lijun Sheng,Jian Liang,Ran He*

Main category: cs.LG

TL;DR: VILL是一个用于无监督域适应的新框架，通过自适应重加权和KL散度再平衡策略来解决类别公平性问题，在保持高整体精度的同时提升最差类别性能。


<details>
  <summary>Details</summary>
Motivation: 现有无监督域适应方法虽然提升了整体精度，但忽视了不同类别间的性能差异（类别公平性问题）。研究发现UDA分类器倾向于偏好容易分类的类别而忽略困难类别，导致性能不均衡。

Method: 提出VILL框架：1）自适应重加权策略，放大难以分类类别的影响；2）基于KL散度的再平衡策略，显式调整决策边界以增强类别公平性。该框架可作为即插即用模块集成到现有UDA方法中。

Result: 在常用数据集上的实验表明，VILL能够显著改善类别公平性，同时保持高整体精度，可以无缝集成到现有UDA方法中。

Conclusion: VILL有效解决了UDA中的类别公平性问题，通过自适应重加权和决策边界调整，在保持整体性能的同时提升了最差类别性能，为UDA研究提供了新的公平性视角。
...
### [341] [Smooth, Sparse, and Stable: Finite-Time Exact Skeleton Recovery via Smoothed Proximal Gradients](https://arxiv.org/abs/2601.18189)
*Rui Wu,Yongjun Li*

Main category: cs.LG

TL;DR: 提出AHOC混合阶无环约束和SPG-AHOC算法，理论保证有限时间内精确恢复DAG结构，无需后处理阈值化


<details>
  <summary>Details</summary>
Motivation: 现有连续优化方法（如NOTEARS）只能保证渐近收敛到驻点，产生稠密权重矩阵，需要任意后处理阈值化来恢复DAG，连续优化与离散图结构之间存在根本性差距

Method: 提出混合阶无环约束(AHOC)并通过平滑近端梯度(SPG-AHOC)进行优化，利用近端算法的流形识别特性

Result: 理论保证有限时间oracle性质：在标准可识别性假设下，SPG-AHOC在有限迭代内精确恢复DAG支撑（结构），返回具有精确零条目的图，无需启发式截断；实证达到最先进精度并强有力地支持有限时间识别理论

Conclusion: 通过AHOC和SPG-AHOC填补了连续优化与离散图结构之间的差距，提供了有限时间内精确恢复DAG结构的理论保证，消除了结构模糊性
...
### [342] [HeterCSI: Channel-Adaptive Heterogeneous CSI Pretraining Framework for Generalized Wireless Foundation Models](https://arxiv.org/abs/2601.18200)
*Chenyu Zhang,Xinchen Lyu,Chenshan Ren,Shuhan Liu,Qimei Cui,Xiaofeng Tao*

Main category: cs.LG

TL;DR: HeterCSI是一个用于无线信道状态信息处理的通道自适应预训练框架，通过优化梯度动态和批次构建，解决了CSI在规模和场景维度上的双重异质性问题，实现了高效训练和强大的跨场景泛化能力。


<details>
  <summary>Details</summary>
Motivation: 无线基础模型在6G网络应用中面临信道状态信息（CSI）在规模和场景维度上的双重异质性问题。现有的预训练方法要么限制输入维度固定，要么按规模隔离训练，限制了模型的泛化能力和可扩展性。

Method: 提出HeterCSI框架，关键洞察是：CSI规模异质性主要导致破坏性梯度干扰，而场景多样性在适当管理下能促进建设性梯度对齐。具体包括：1）将异质CSI批次构建形式化为分区优化问题；2）开发规模感知自适应分批策略；3）设计双重掩码机制隔离有效信号和填充伪影。

Result: 在12个数据集上的实验表明，HeterCSI在不进行场景特定微调的情况下建立了泛化基础模型，性能优于全样本基线。相比最先进的零样本基准WiFo，在CSI重建、时域和频域预测上分别降低NMSE 7.19dB、4.08dB和5.27dB。训练延迟降低53%，泛化性能平均提升1.53dB。

Conclusion: HeterCSI通过重新理解异质CSI预训练中的梯度动态，协调了训练效率和跨场景泛化能力，为无线基础模型提供了有效的预训练框架，解决了CSI双重异质性带来的挑战。
...
### [343] [PaperSearchQA: Learning to Search and Reason over Scientific Papers with RLVR](https://arxiv.org/abs/2601.18207)
*James Burgess,Jan N. Hansen,Duo Peng,Yuhui Zhang,Alejandro Lozano,Min Woo Sun,Emma Lundberg,Serena Yeung-Levy*

Main category: cs.LG

TL;DR: 开发了在生物医学论文摘要上进行搜索和推理的智能体，构建了包含1600万摘要的搜索语料库和6万样本的PaperSearchQA数据集，训练出的搜索智能体超越了非强化学习检索基线。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR搜索智能体主要处理通用领域QA，限制了其在科学、工程和医学等专业AI系统中的适用性。需要开发能够搜索和推理科学论文的智能体，这对技术问答和未来AI科学家系统至关重要。

Method: 1) 发布包含1600万生物医学论文摘要的搜索语料库；2) 构建包含6万样本的PaperSearchQA事实问答数据集；3) 在该环境中训练搜索智能体，使用RLVR方法；4) 与Search-R1代码库兼容。

Result: 训练出的搜索智能体超越了非强化学习检索基线，表现出规划、推理和自我验证等有趣行为。定量分析验证了方法的有效性。

Conclusion: 成功开发了针对科学论文搜索的智能体系统，数据创建方法具有可扩展性，可轻松扩展到其他科学领域。相关资源已公开发布。
...
### [344] [Rethinking Cross-Modal Fine-Tuning: Optimizing the Interaction between Feature Alignment and Target Fitting](https://arxiv.org/abs/2601.18231)
*Trong Khiem Tran,Manh Cuong Dao,Phi Le Nguyen,Thao Nguyen Truong,Trong Nghia Hoang*

Main category: cs.LG

TL;DR: 提出理论框架分析特征对齐与目标微调间的交互作用，通过特征标签扭曲概念建立可证明的泛化边界，指导算法设计并提升跨模态适应性能


<details>
  <summary>Details</summary>
Motivation: 预训练模型适应新特征模态的需求日益增长，但现有方法缺乏对特征对齐与目标微调交互作用的理论理解，未校准的组合可能加剧源-目标特征标签结构错配，降低目标泛化能力

Method: 开发理论框架建立目标误差的可证明泛化边界，引入特征标签扭曲概念解释特征对齐与目标拟合的交互作用，基于边界洞察设计优化算法

Result: 在广泛基准数据集上显著优于现有最先进方法，理论框架为实际算法设计提供可操作的指导

Conclusion: 通过理论分析特征对齐与目标拟合的交互作用，建立了可证明的泛化边界，为跨模态知识迁移提供了原则性框架，显著提升了预训练模型适应新模态的性能
...
### [345] [Tractable Gaussian Phase Retrieval with Heavy Tails and Adversarial Corruption with Near-Linear Sample Complexity](https://arxiv.org/abs/2601.18245)
*Santanu Das,Jatin Batra*

Main category: cs.LG

TL;DR: 本文提出了首个多项式时间算法，用于解决具有重尾噪声和对抗性损坏的鲁棒相位恢复问题，样本复杂度接近线性。


<details>
  <summary>Details</summary>
Motivation: 相位恢复在光学、晶体学等领域有广泛应用，但现有算法对测量误差的鲁棒性不足。虽然近期在鲁棒统计方面有突破，但针对相位恢复的鲁棒算法仍面临挑战，特别是需要鲁棒谱初始化这一关键步骤。

Method: 通过建立鲁棒谱初始化与鲁棒PCA最新算法进展之间的联系，利用鲁棒PCA技术实现高效的谱初始化，从而开发出多项式时间的鲁棒相位恢复算法。

Result: 首次实现了多项式时间算法，能够处理重尾噪声和对抗性损坏（测量值和传感向量都可能被恶意篡改），样本复杂度为接近线性的O(n log n)。

Conclusion: 通过连接鲁棒谱初始化和鲁棒PCA，解决了鲁棒相位恢复中的关键计算瓶颈，为实际应用提供了高效可行的解决方案。
...
### [346] [Beyond Retention: Orchestrating Structural Safety and Plasticity in Continual Learning for LLMs](https://arxiv.org/abs/2601.18255)
*Fei Meng*

Main category: cs.LG

TL;DR: 本文发现经验回放在LLM持续学习中存在关键二分：对非结构化NLP任务产生正向迁移，但对结构化代码生成任务造成负向迁移，并提出正交子空间唤醒方法来解决这一困境。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型持续学习面临稳定性与可塑性的平衡挑战。虽然经验回放是防止灾难性遗忘的标准方法，但其对不同能力的影响尚未充分探索。本文旨在揭示经验回放在不同任务类型中的差异化影响，并解决其在结构化任务中造成的负面迁移问题。

Method: 提出正交子空间唤醒方法：通过短暂的"唤醒"阶段识别先前任务的关键参数子空间，并为新任务强制执行正交更新，为已有知识结构提供数学基础的"安全保证"。

Result: 实验结果表明，OSW在保持脆弱编码能力方面优于经验回放，同时维持对新任务的高可塑性。在多样化的四任务序列中，OSW成功保护了代码生成能力，而经验回放则导致显著的性能下降。

Conclusion: 研究强调在LLM持续学习中需要同时评估结构安全性和平均保留能力。OSW方法为解决经验回放的结构完整性缺陷提供了有效解决方案，为平衡稳定性和可塑性提供了新思路。
...
### [347] [FGGM: Fisher-Guided Gradient Masking for Continual Learning](https://arxiv.org/abs/2601.18261)
*Chao-Hong Tan,Qian Chen,Wen Wang,Yukun Ma,Chong Zhang,Chong Deng,Qinglin Zhang,Xiangang Li,Jieping Ye*

Main category: cs.LG

TL;DR: FGGM使用Fisher信息指导梯度掩码，缓解大语言模型持续学习中的灾难性遗忘，在TRACE基准上相比SFT提升9.6%，相比MIGU提升4.4%。


<details>
  <summary>Details</summary>
Motivation: 灾难性遗忘会损害大语言模型的持续学习能力，需要一种能够平衡稳定性和可塑性的方法，同时避免依赖历史数据。

Method: 提出Fisher指导的梯度掩码框架，使用对角Fisher信息策略性地选择参数进行更新，动态生成具有自适应阈值的二进制掩码，保留关键参数。

Result: 在TRACE基准上，FGGM相比监督微调在保留通用能力方面有9.6%的相对提升，相比MIGU在TRACE任务上有4.4%的提升。代码生成任务分析也证实了其优越性能和减少遗忘的效果。

Conclusion: FGGM通过数学原理的参数重要性估计，提供了一种有效的解决方案，能够在不依赖历史数据的情况下平衡稳定性和可塑性，缓解灾难性遗忘问题。
...
### [348] [Neural Network Approximation: A View from Polytope Decomposition](https://arxiv.org/abs/2601.18264)
*ZeYu Li,ShiJun Zhang,TieYong Zeng,FengLei Fan*

Main category: cs.LG

TL;DR: 该论文提出了一种基于多面体分解的ReLU网络通用逼近理论，相比传统均匀网格划分方法，能更好地处理目标函数的局部正则性，特别是在奇异点附近表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有通用逼近理论大多通过均匀划分输入空间来构建神经网络，忽略了目标函数的局部正则性。作者希望开发一种更现实、任务导向的逼近方法，能够更好地处理函数奇异点等复杂情况。

Method: 1. 提出基于多面体分解的通用逼近框架；2. 开发显式核多项式方法逼近连续函数；3. 使用精化的Totik-Ditzian型连续性模刻画逼近特性；4. 在每个子域中分别构建ReLU网络来逼近核多项式；5. 将方法扩展到解析函数以获得更高逼近率。

Result: 多面体分解方法相比现有方法在许多情况下更高效灵活，特别是在目标函数奇异点附近。该方法能够实现连续函数的通用逼近，并扩展到解析函数时能达到更高的逼近率。

Conclusion: 基于多面体分解的ReLU网络通用逼近理论提供了一种更现实、任务导向的逼近方法，能够更好地处理函数局部正则性，在奇异点附近表现优异，为神经网络在实际应用中的原理性使用提供了更坚实的理论基础。
...
### [349] [What Do Learned Models Measure?](https://arxiv.org/abs/2601.18278)
*Indrė Žliobaitė*

Main category: cs.LG

TL;DR: 论文提出"测量稳定性"概念，指出机器学习模型作为测量工具时，即使预测性能相同，也可能实现不等价的测量函数，现有评估框架存在局限性。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型越来越多地被用作测量工具而非仅仅是预测器，但现有评估标准（如泛化误差、校准、鲁棒性）无法保证模型作为测量函数的稳定性，可能导致不同训练过程产生不等价的测量映射。

Method: 形式化学习测量函数作为独立评估对象，引入"测量稳定性"概念，要求测量量在可接受的学习过程实现和不同上下文环境中保持不变。通过真实案例研究展示问题。

Result: 研究表明，具有可比预测性能的模型可以实现系统不等价的测量函数，分布偏移具体说明了这种失败。现有评估标准无法保证测量稳定性。

Conclusion: 当学习模型输出被识别为测量时，现有评估框架存在局限性，需要增加"测量稳定性"这一新的评估维度来确保模型作为测量工具的可靠性。
...
### [350] [TriPlay-RL: Tri-Role Self-Play Reinforcement Learning for LLM Safety Alignment](https://arxiv.org/abs/2601.18292)
...
TL;DR: TriPlay-RL：一个闭环强化学习框架，通过攻击者、防御者和评估者三者的协同进化实现LLM安全对齐，无需人工标注


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的安全风险日益突出，需要有效缓解有毒有害内容的生成。当前主流的安全对齐范式通常采用三方协作框架，但存在效率不高、需要大量人工标注等问题。

Method: 提出TriPlay-RL闭环强化学习框架，使攻击者（生成对抗提示）、防御者（安全防御）和评估者（响应评估）在近乎零人工标注的情况下实现迭代协同改进。通过强化学习让三者相互促进、共同进化。

Result: 攻击者在保持高输出多样性的同时，对抗效果提升20%-50%；防御者在不降低通用推理能力的情况下，安全性能获得10%-30%的提升；评估者通过迭代持续细化判断能力，能准确区分不安全响应、简单拒绝和有用指导。

Conclusion: TriPlay-RL为LLM安全对齐建立了高效可扩展的范式，在统一学习循环中实现了三者的持续协同进化，为解决LLM安全风险提供了有效方案。
...
### [351] [A Master Class on Reproducibility: A Student Hackathon on Advanced MRI Reconstruction Methods](https://arxiv.org/abs/2601.18314)
...
TL;DR: 学生可重复性黑客马拉松，复现三篇MRI重建论文结果，包括MoDL、HUMUS-Net和基于物理正则化的动态MRI方法，并建立可重复代码库实践


<details>
  <summary>Details</summary>
Motivation: 解决医学影像重建领域研究可重复性问题，通过学生黑客马拉松形式系统性地复现有影响力的MRI重建方法，建立可重复研究实践标准

Method: 组织学生黑客马拉松，设计系统协议，复现三篇关键MRI重建论文：MoDL（基于模型展开网络）、HUMUS-Net（混合CNN+Transformer架构）和基于物理正则化的动态MRI方法，并建立可重复代码库实践

Result: 成功复现了三种MRI重建方法的结果，提供了详细的复现结果和额外实验，建立了构建可重复代码库的基本实践指南

Conclusion: 通过结构化黑客马拉松可以有效促进研究可重复性，为医学影像重建领域提供了可重复性实践框架，有助于推动该领域的开放科学和可靠研究
...
### [352] [Cognitive Fusion of ZC Sequences and Time-Frequency Images for Out-of-Distribution Detection of Drone Signals](https://arxiv.org/abs/2601.18326)
...
<details>
  <summary>Details</summary>
Motivation: 无人机远程识别(RID)需要检测未知或非标准通信协议的无人机信号（OOD检测），现有方法难以有效处理多模态信号特征

Method: 结合ZC序列（识别DJI无人机协议）和时频图像（捕获未知协议信号特征），通过多模态特征提取、交互和融合，生成自适应注意力权重进行分类

Result: 在RID和OODD指标上分别提升1.7%和7.5%，在不同飞行条件和无人机类型下表现出强鲁棒性

Conclusion: 提出的多模态融合算法能有效检测无人机信号中的OOD样本，为无人机远程识别提供了更可靠的解决方案
...
### [353] [Discriminability-Driven Spatial-Channel Selection with Gradient Norm for Drone Signal OOD Detection](https://arxiv.org/abs/2601.18329)
*Chuhan Feng,Jing Li,Jie Li,Lu Lv,Fengkui Gong*

Main category: cs.LG

TL;DR: 提出一种基于可区分性驱动的空间-通道选择和梯度范数的无人机信号OOD检测算法，通过时频图像特征自适应加权和梯度范数度量扰动敏感性，实现优越的判别性能。
...
Method: 1) 基于协议特定时频特征量化类间相似性和方差，对时频图像特征进行空间和通道维度的自适应加权；2) 引入梯度范数度量扰动敏感性，捕捉OOD样本的内在不稳定性；3) 将梯度范数与基于能量的分数融合进行联合推理。

Result: 仿真结果表明，该算法在信噪比(SNR)和多种无人机类型下均表现出优越的判别能力和鲁棒性能。

Conclusion: 提出的基于可区分性驱动的空间-通道选择和梯度范数的OOD检测算法能够有效识别无人机信号中的分布外样本，具有实际应用价值。
...
### [354] [Structural Gender Bias in Credit Scoring: Proxy Leakage](https://arxiv.org/abs/2601.18342)
*Navya SD,Sreekanth D,SS Uma Sankari*

Main category: cs.LG

TL;DR: 研究审计台湾信贷违约数据集中的结构性性别偏见，挑战"公平盲视"理念，发现即使移除敏感属性，婚姻状况、年龄和信用额度等非敏感特征仍作为性别代理变量，使模型保持歧视性预测能力。


<details>
  <summary>Details</summary>
Motivation: 随着金融机构越来越多地采用机器学习进行信用风险评估，算法偏见仍然是实现公平金融包容性的关键障碍。本研究旨在挑战"公平盲视"的主流理念，揭示即使移除显式保护属性，结构性性别偏见仍然存在于非敏感特征中。

Method: 使用SHAP（SHapley Additive exPlanations）识别婚姻状况、年龄和信用额度等变量如何作为性别代理变量；采用对抗性逆建模框架从纯非敏感金融特征中重建受保护的性别属性，量化信息泄漏程度。

Result: 研究发现受保护的性别属性可以从纯非敏感金融特征中以ROC AUC得分0.65重建，表明传统公平审计不足以检测隐性结构性偏见。即使移除显式保护属性并应用行业标准公平干预措施，性别预测信号仍深深嵌入非敏感特征中。

Conclusion: 研究主张从表面统计平等转向因果感知建模和金融AI中的结构性问责制。传统公平审计方法存在不足，需要更深入的结构性偏见检测方法。
...
### [355] [Making medical vision-language models think causally across modalities with retrieval-augmented cross-modal reasoning](https://arxiv.org/abs/2601.18356)
*Weiqin Yang,Haowen Xue,Qingyi Peng,Hexuan Hu,Qian Huang,Tingbo Zhang*

Main category: cs.LG

TL;DR: 提出多模态因果检索增强生成框架，将因果推理与多模态检索结合，提升医学视觉语言模型在放射学报告生成、诊断预测和视觉问答中的事实准确性、鲁棒性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型主要基于相关性而非因果性，依赖表面统计关联而非病理生理机制，导致模型脆弱、易产生幻觉且对数据集偏差敏感。传统检索增强生成依赖语义相似性，会引入新的虚假相关性。

Method: 提出多模态因果检索增强生成框架，从外部源检索临床相关示例和因果图，使模型推理基于反事实和干预证据而非仅相关性。应用于放射学报告生成、诊断预测和视觉问答任务。

Result: 该框架提高了事实准确性、对分布偏移的鲁棒性和可解释性。结果表明因果检索是构建超越模式匹配的可信医学视觉语言模型的可扩展路径。

Conclusion: 将因果推理与多模态检索结合是提升医学视觉语言模型在临床高风险环境中可信度的有效方法，使模型能够超越单纯模式匹配，实现更可靠的医学推理。
...
### [356] [Estimating Dense-Packed Zone Height in Liquid-Liquid Separation: A Physics-Informed Neural Network Approach](https://arxiv.org/abs/2601.18399)
*Mehmet Velioglu,Song Zhai,Alexander Mitsos,Adel Mhamdi,Andreas Jupke,Manuel Dahmen*

Main category: cs.LG

TL;DR: 使用物理信息神经网络（PINN）结合扩展卡尔曼滤波器，仅通过流量测量估计重力沉降器中液-液分散的相高度，减少对昂贵光学测量的依赖。


<details>
  <summary>Details</summary>
Motivation: 重力沉降器中密集堆积区高度是重要的性能和安全指标，但传统光学测量方法昂贵且不实用。需要开发仅使用廉价流量测量就能估计相高度的方法。

Method: 1. 使用低精度机理模型生成合成数据预训练PINN；2. 仅使用体积平衡方程（避免计算昂贵的液滴聚并和沉降子模型）；3. 用少量实验数据微调PINN；4. 将可微PINN作为预测模型集成到扩展卡尔曼滤波器框架中进行状态估计。

Result: 两阶段训练的PINN在所有评估中表现最佳：1. 前向模拟比机理模型和非预训练PINN更准确；2. 在滤波器中的相高度估计比两阶段训练的纯数据驱动神经网络更准确。

Conclusion: 提出的两阶段PINN训练方法结合扩展卡尔曼滤波器，能够仅通过流量测量准确估计相高度，为工业分离过程提供了一种经济实用的监测方案。
...
### [357] [Superlinear Multi-Step Attention](https://arxiv.org/abs/2601.18401)
*Yufeng Huang*
...
<details>
  <summary>Details</summary>
Motivation: 标准自注意力机制在处理长序列时存在二次复杂度问题，现有方法要么牺牲随机上下文访问能力，要么无法有效扩展到极长序列。需要一种既能保持结构非排他性（任何合格位置都能被选中）又能实现次二次复杂度的注意力机制。

Method: 将标准因果自注意力重新表述为N步搜索问题，整体复杂度为O(L^(1+1/N))。提出N=2的基线实现，第一步进行O(L^(3/2))的跨度搜索选择相关序列段，第二步在选定段上应用O(L^(3/2))的跨度注意力。还展示了O(L^1.54)的鲁棒配置。

Result: 在修改的30B混合MoE模型上，单B200 GPU上实现：1M上下文长度下平均解码吞吐量114 tokens/sec，10M上下文长度下80 tokens/sec。在NIAH任务上达到256K上下文长度的强性能，证明路由跨度选择可端到端学习。

Conclusion: Superlinear attention架构在保持随机上下文访问的同时实现了次二次复杂度，展示了系统可行性和初步验证。该工作侧重于架构表述、扩展分析和系统可行性，全面的质量评估留待未来工作。
...
### [358] [Frequency-Based Hyperparameter Selection in Games](https://arxiv.org/abs/2601.18409)
*Aniket Sanyal,Baraah A. M. Sidahmed,Rebekka Burkholz,Tatjana Chavdarova*

Main category: cs.LG

TL;DR: 提出MoLA方法，通过频率估计自适应调整LookAhead超参数，加速平滑博弈中的训练


<details>
  <summary>Details</summary>
Motivation: 平滑博弈中的旋转动力学使经典超参数调优策略失效，现有方法如LookAhead虽有效但引入额外参数且对性能影响关键，缺乏有效的博弈超参数调优方法

Method: 基于频率估计分析振荡动力学，在连续时间轨迹和离散动力学谱中分析振荡，提出Modal LookAhead (MoLA)，自适应选择超参数

Result: 提供收敛保证，实验表明MoLA在纯旋转博弈和混合机制中都能加速训练，且计算开销最小
...
### [359] [Gradient Regularized Natural Gradients](https://arxiv.org/abs/2601.18420)
*Satya Prakash Dash,Hossein Abdi,Wei Pan,Samuel Kaski,Mingfei Sun*

Main category: cs.LG

TL;DR: 提出GRNG优化器，将梯度正则化与自然梯度结合，提供频度和贝叶斯两种变体，在优化速度和泛化性能上优于一阶和二阶基线方法。


<details>
  <summary>Details</summary>
Motivation: 梯度正则化已被证明能提升模型泛化能力，自然梯度下降在训练初期能加速优化，但二阶优化器的训练动态如何从梯度正则化中获益尚未得到充分研究。

Method: 提出梯度正则化自然梯度（GRNG）框架，包含两种算法：1）频度变体通过结构化近似避免显式计算Fisher信息矩阵逆；2）贝叶斯变体基于正则化-卡尔曼公式完全消除FIM求逆需求。

Result: 理论证明GRNG具有收敛保证，梯度正则化提升稳定性并确保收敛到全局最小值。实验显示GRNG在视觉和语言基准上持续优于一阶方法（SGD、AdamW）和二阶基线（K-FAC、Sophia）。

Conclusion: 梯度正则化是解锁自然梯度方法在大规模深度学习中的鲁棒性的原则性实用工具，GRNG在优化速度和泛化性能上均表现出色。
...
### [360] [GCFX: Generative Counterfactual Explanations for Deep Graph Models at the Model Level](https://arxiv.org/abs/2601.18447)
*Jinlong Hu,Jiacheng Liu*

Main category: cs.LG

TL;DR: GCFX：基于深度图生成的模型级反事实解释方法，通过增强的图生成框架和全局总结算法，为深度图学习模型提供高质量的反事实解释，提升模型透明度和可信度。


<details>
  <summary>Details</summary>
Motivation: 深度图学习模型虽然处理图结构数据能力强且应用广泛，但其复杂的内部架构和缺乏透明度使得决策难以解释，导致用户难以理解和信任这些"黑盒"模型。

Method: 提出GCFX方法，基于深度图生成的反事实解释框架。采用双编码器、结构感知标记器和消息传递神经网络解码器的架构，准确学习输入数据的真实潜在分布并生成高质量反事实示例。随后通过全局反事实总结算法从众多候选反事实中选择最具代表性和全面的解释。

Result: 在合成数据集和多个真实世界数据集上的实验表明，GCFX在反事实有效性和覆盖率方面优于现有方法，同时保持较低的解释成本，为增强全局反事实解释的实用性和可信度提供了重要支持。

Conclusion: GCFX为深度图学习模型提供了有效的模型级反事实解释方法，通过生成高质量的反事实解释来揭示模型的全局预测行为，有助于提高模型的可解释性、实用性和用户信任度。
...
### [361] [Enhancing Control Policy Smoothness by Aligning Actions with Predictions from Preceding States](https://arxiv.org/abs/2601.18479)
*Kyoleen Kwak,Hyoseok Hwang*

Main category: cs.LG

TL;DR: 提出ASAP方法，通过过渡诱导相似状态和对齐动作来平滑深度强化学习中的高频振荡，提升控制平滑性和策略性能


<details>
  <summary>Details</summary>
Motivation: 深度强化学习在控制任务中表现出色，但其固有的高频振荡限制了在实际环境中的应用。现有方法通常依赖启发式或合成的状态相似性定义来促进动作一致性，但这些定义往往无法准确反映底层系统动态。

Method: 提出ASAP方法：1）引入过渡诱导相似状态，定义为从前一状态过渡到的下一状态分布，仅利用环境反馈和实际收集数据；2）通过对齐过渡诱导相似状态中的动作来强制动作平滑性；3）惩罚二阶差异以抑制高频振荡。

Result: 在Gymnasium和Isaac-Lab环境中的实验表明，ASAP相比现有方法能产生更平滑的控制和更好的策略性能。

Conclusion: ASAP通过利用过渡诱导相似状态和对齐动作的方法，有效缓解了深度强化学习中的动作振荡问题，为实际应用提供了更平滑可靠的控制策略。
...
### [362] [Nearly Optimal Bayesian Inference for Structural Missingness](https://arxiv.org/abs/2601.18500)
...
Method: 采用贝叶斯视角，通过后验预测分布集成完整模型后验不确定性，将学习缺失值后验与标签预测解耦，实现后验集成

Result: 在43个分类基准和15个插补基准上达到SOTA，在SCM先验下具有有限样本近贝叶斯最优性保证

Conclusion: 该框架提供"几乎免费午餐"：一旦学习到后验，预测即可即插即用同时保持不确定性传播，有效解决结构性缺失数据的挑战
...
### [363] [Conformal Prediction Algorithms for Time Series Forecasting: Methods and Benchmark](https://arxiv.org/abs/2601.18509)
*Andro Sabashvili*
...
<details>
  <summary>Details</summary>
Motivation: 时间序列预测中的可靠不确定性量化至关重要，但传统方法依赖限制性分布假设。保形预测（CP）作为无分布框架具有理论保证，但时间序列的时序依赖性违反了CP核心的可交换性假设，需要专门解决方案。

Method: 本文系统综述了四类主要算法解决方案：1）放宽可交换性假设的方法；2）重新定义数据单元为独立时间序列集合的方法；3）显式建模预测残差动态的方法；4）适应分布漂移以维持长期覆盖的在线学习算法。

Result: 通过对这些方法的综合分析和基准测试，突出了它们在计算效率和真实数据上的实际性能表现，为时间序列CP应用提供了系统指导。

Conclusion: 时间序列保形预测面临可交换性假设与时间依赖性的根本冲突，但通过多种算法策略可以有效解决。综述为实际应用提供了方法选择和性能评估的参考框架。
...
### [364] [Just-In-Time Reinforcement Learning: Continual Learning in LLM Agents Without Gradient Updates](https://arxiv.org/abs/2601.18510)
*Yibo Li,Zijie Lin,Ailin Deng,Xuan Zhang,Yufei He,Shuo Ji,Tri Cao,Bryan Hooi*

Main category: cs.LG

TL;DR: JitRL是一个无需训练、测试时策略优化的框架，通过动态记忆检索相关轨迹来估计动作优势，直接调制LLM输出logits，实现持续适应而不需要梯度更新。


<details>
  <summary>Details</summary>
Motivation: LLM代理在部署后权重固定，难以持续适应新任务。传统强化学习计算成本高且存在灾难性遗忘问题，需要一种无需训练、测试时优化的解决方案。

Method: JitRL维护动态非参数化记忆库，检索相关经验轨迹，在线估计动作优势值，然后直接调制LLM的输出logits。理论证明这种加性更新规则是KL约束策略优化目标的精确闭式解。

Result: 在WebArena和Jericho基准测试中，JitRL在无需训练的方法中达到最先进水平，甚至超越计算昂贵的微调方法（如WebRL），同时降低30倍以上的成本。

Conclusion: JitRL为持续学习代理提供了一种可扩展的路径，能够在测试时优化策略而无需梯度更新，解决了LLM持续适应的关键挑战。
...
### [365] [LipNeXt: Scaling up Lipschitz-based Certified Robustness to Billion-parameter Models](https://arxiv.org/abs/2601.18513)
*Kai Hu,Haoqi Hu,Matt Fredrikson*

Main category: cs.LG

TL;DR: LipNeXt：首个无约束、无卷积的1-Lipschitz架构，通过流形优化和空间移位模块实现高效确定性认证鲁棒性，在ImageNet上扩展到10亿参数规模。


<details>
  <summary>Details</summary>
Motivation: 现有的Lipschitz认证方法在模型规模、训练效率和ImageNet性能方面存在扩展困难，需要一种既能保持确定性认证又能适应现代大规模模型的方法。

Method: 提出LipNeXt架构：1）使用流形优化直接在正交流形上更新参数；2）引入空间移位模块替代卷积建模空间模式；3）结合正交投影、空间移位、β-Abs非线性激活和L2空间池化。

Result: 在CIFAR-10/100和Tiny-ImageNet上达到SOTA的干净准确率和认证鲁棒准确率；在ImageNet上扩展到1-20亿参数大模型，认证鲁棒准确率提升高达8%（ε=1），保持高效稳定的低精度训练。

Conclusion: Lipschitz认证方法能够受益于现代扩展趋势，同时不牺牲确定性或效率，为大规模确定性认证鲁棒性提供了可行路径。
...
### [366] [Scalable Transit Delay Prediction at City Scale: A Systematic Approach with Multi-Resolution Feature Engineering and Deep Learning](https://arxiv.org/abs/2601.18521)
*Emna Boudabbous,Mohamed Karaa,Lokman Sboui,Julio Montecinos,Omar Alam*

Main category: cs.LG

TL;DR: 提出一个城市规模的公交延误预测框架，通过多分辨率特征工程、降维和深度学习，实现实时、可扩展的延误预测，在蒙特利尔公交网络上验证效果优于Transformer模型。


<details>
  <summary>Details</summary>
Motivation: 城市公交机构需要可靠的网络级延误预测来提供准确的到站信息并支持实时运营控制。现有系统大多只能处理少数线路，依赖手工特征，缺乏可扩展、可复用的架构指导。

Method: 提出城市规模预测管道，结合多分辨率特征工程（生成1,683个时空特征）、降维（自适应PCA压缩到83个组件）和深度学习。引入混合H3+拓扑聚类方法解决"巨型集群"问题，形成12个平衡的路由集群。比较五种模型架构。

Result: 在蒙特利尔公交网络6个月数据上，全局LSTM模型在准确性和效率间取得最佳平衡，比Transformer模型性能提升18-52%，参数量减少275倍。管道适合实时城市规模部署，可复用于其他网络。

Conclusion: 提出的城市规模预测管道能够实现准确、高效的公交延误预测，解决了现有系统的局限性，为实时运营控制提供了可扩展、可复用的解决方案。
...
### [367] [From Human Labels to Literature: Semi-Supervised Learning of NMR Chemical Shifts at Scale](https://arxiv.org/abs/2601.18524)
*Yongqi Jin,Yecheng Wang,Jun-jie Wang,Rong Zhu,Guolin Ke,Weinan E*

Main category: cs.LG

TL;DR: 提出一个半监督框架，利用文献提取的百万级未标记NMR谱图训练化学位移预测模型，无需原子级标注，显著提升预测精度和泛化能力
...
Method: 将文献谱图化学位移预测建模为置换不变集合监督问题，在损失函数满足特定条件下，最优二分匹配简化为基于排序的损失函数，实现稳定的大规模半监督训练

Result: 模型在精度和鲁棒性上显著超越现有方法，在更大更多样化的分子数据集上表现出更强的泛化能力，首次在常见NMR溶剂中捕捉到系统性溶剂效应
...
### [368] [Closing the Modality Gap Aligns Group-Wise Semantics](https://arxiv.org/abs/2601.18525)
*Eleonora Grassucci,Giordano Cicchetti,Emanuele Frasca,Aurelio Uncini,Danilo Comminiello*

Main category: cs.LG

TL;DR: 论文提出模态间隙（modality gap）在群体级任务（如聚类）中影响显著，而非实例级任务（如检索），并提出了减少该间隙的方法来提升群体级任务性能。


<details>
  <summary>Details</summary>
Motivation: CLIP等方法在多模态学习中虽然能对齐语义，但产生的潜在空间仍存在模态间隙。目前关于是否需要解决这一现象存在争议，特别是它对实例级任务影响有限。作者发现模态间隙对群体级任务影响更大，需要深入研究。

Method: 提出了一种新颖的方法来持续减少双模态设置中的模态间隙，并可简单扩展到一般的n模态情况。该方法专门针对减少模态间的结构不匹配。

Result: 广泛评估表明：减少模态间隙对传统实例级任务仅提供边际或不一致的改进，但显著提升了群体级任务的性能。这揭示了模态间隙在语义分组任务中的关键作用。

Conclusion: 模态间隙在群体级任务中具有重要影响，解决它可以显著提升语义分组任务的性能。这一发现可能重塑我们对模态间隙的理解，强调其在需要语义分组的任务中的关键作用。
...
### [369] [Information Hidden in Gradients of Regression with Target Noise](https://arxiv.org/abs/2601.18546)
*Arash Jamshidi,Katsiaryna Haitsiukevich,Kai Puolamäki*

Main category: cs.LG

TL;DR: 提出一种通过梯度协方差估计Hessian矩阵的方法，核心是噪声方差校准：将目标噪声方差设为批次大小n，使经验梯度协方差近似Hessian矩阵，即使远离最优解也能工作。


<details>
  <summary>Details</summary>
Motivation: 二阶信息（如曲率或数据协方差）对优化、诊断和鲁棒性至关重要，但在许多现代设置中只能观察到梯度。本文旨在探索仅通过梯度能否揭示Hessian矩阵。

Method: 提出方差校准方法：注入高斯噪声使总目标噪声方差等于批次大小n，确保经验梯度协方差能紧密近似Hessian矩阵。在线性回归中，Hessian等于数据协方差Σ。方法提供非渐近算子范数保证，适用于次高斯输入。

Result: 理论证明：通过适当校准，可以从梯度中恢复Hessian矩阵；若无校准，恢复可能失败Ω(1)因子。方法实用（"将目标噪声方差设为n"规则）且鲁棒（方差O(n)足以恢复Σ至缩放比例）。在合成和真实数据实验中验证了理论结果。

Conclusion: 仅通过梯度就能揭示Hessian矩阵，提出的方差校准方法简单实用，为更快优化、对抗风险估计和分布式系统中的梯度训练等应用提供了基础。
...
### [370] [An Unsupervised Tensor-Based Domain Alignment](https://arxiv.org/abs/2601.18564)
*Chong Hyun Lee,Kibae Lee,Hyun Hee Yim*

Main category: cs.LG

TL;DR: 提出基于张量的域对齐算法，通过对齐矩阵在不变子空间中对齐源和目标张量，使用斜流形约束提供比传统Stiefel流形更大的灵活性，通过正则化项保持方差，显著提升分类精度和转换速度。


<details>
  <summary>Details</summary>
Motivation: 传统张量域对齐方法通常使用Stiefel流形约束，灵活性有限。本文旨在开发更灵活、适应性更强的张量域对齐框架，能够泛化现有方法并提升性能。

Method: 提出基于张量的域对齐算法，使用对齐矩阵在不变子空间中对齐源和目标张量。采用斜流形约束进行迭代优化，比传统Stiefel流形更灵活。引入正则化项保持源和目标张量的方差，确保鲁棒性。

Result: 实验表明，该方法不仅显著提升域对齐转换速度，还大幅提高分类精度。框架具有通用性，能够将现有张量域对齐方法作为特例进行泛化。

Conclusion: 提出的基于斜流形约束的张量域对齐方法优于当前最先进技术，在复杂域适应任务中表现优越，是域对齐任务的优选方案。
...
### [371] [K-Myriad: Jump-starting reinforcement learning with unsupervised parallel agents](https://arxiv.org/abs/2601.18580)
*Vincenzo De Paola,Mirco Mutti,Riccardo Zamboni,Marcello Restelli*

Main category: cs.LG

TL;DR: K-Myriad是一种可扩展的无监督方法，通过最大化并行策略群体诱导的集体状态熵来培养多样化的探索策略，为强化学习提供鲁棒初始化，提高训练效率并发现异构解决方案。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习并行化通常使用相同采样分布的多个工作器来加速单个策略训练，这种设计忽视了多样化探索策略的优势，限制了并行化的潜力。

Method: 提出K-Myriad方法，通过最大化并行策略群体诱导的集体状态熵来培养专门的探索策略组合，实现无监督的多样化探索。

Result: 在高维连续控制任务上的大规模并行化实验表明，K-Myriad能够学习到广泛的不同策略，证明了其在集体探索方面的有效性。

Conclusion: K-Myriad通过培养多样化探索策略组合，为强化学习提供了鲁棒初始化，提高了训练效率并发现了异构解决方案，为新型并行化策略开辟了道路。
...
### [372] [Learning long term climate-resilient transport adaptation pathways under direct and indirect flood impacts using reinforcement learning](https://arxiv.org/abs/2601.18586)
*Miguel Costa,Arthur Vandervoort,Carolin Schmidt,Morten W. Petersen,Martin Drews,Karyn Morrissey,Francisco C. Pereira*

Main category: cs.LG

TL;DR: 提出结合集成评估模型与强化学习的决策支持框架，用于学习城市交通基础设施在气候变化下的自适应多十年投资路径
...
Result: 在哥本哈姆市2024-2100年内涝案例中，学习策略产生协调的时空路径，相比不作为和随机行动等基准方法具有更好的鲁棒性

Conclusion: 该框架可推广到其他灾害和城市，为城市气候适应提供有效的决策支持工具
...
### [373] [LaCoGSEA: Unsupervised deep learning for pathway analysis via latent correlation](https://arxiv.org/abs/2601.18604)
...
<details>
  <summary>Details</summary>
Motivation: 传统通路富集分析方法（如GSEA）依赖预定义表型标签和成对比较，在无监督场景下应用受限。现有无监督扩展方法主要捕获线性关系，且不明确建模基因-通路关联。深度学习模型虽能捕获非线性结构，但其解释依赖通用XAI技术，在无监督转录组分析中的通路级解释效果有限。

Method: LaCoGSEA采用自编码器捕获非线性流形，提出全局基因-潜在相关度量作为差异表达代理，无需先验标签生成密集基因排名。该框架整合深度表示学习和稳健通路统计。

Result: LaCoGSEA在癌症亚型区分中实现更好的聚类性能；相比线性降维和梯度XAI方法，在更高排名恢复更广泛的生物学相关通路；在不同实验协议和数据集规模下保持高稳健性和一致性。

Conclusion: LaCoGSEA在无监督通路富集分析中提供最先进性能，填补了深度学习表示与通路级解释之间的空白。
...
### [374] [Geometry-Free Conditional Diffusion Modeling for Solving the Inverse Electrocardiography Problem](https://arxiv.org/abs/2601.18615)
*Ramiro Valdes Jara,Adam Meyers*

Main category: cs.LG

TL;DR: 提出基于条件扩散模型的数据驱动方法，解决心电图成像中的逆问题，无需几何建模，可概率性采样多个重建结果


<details>
  <summary>Details</summary>
Motivation: 传统心电图成像逆问题存在非唯一性和欠定性，传统方法需要患者特定的网格构建，且通常提供单一确定性估计而非概率性重建

Method: 采用条件扩散框架，学习从噪声体表信号到心脏表面电位的概率映射，利用扩散模型的生成特性捕获逆问题的非唯一性，实现几何无关的纯数据驱动方法

Result: 在真实ECGI数据集上评估，相比卷积神经网络、长短时记忆网络和Transformer等确定性基线模型，扩散方法实现了更高的重建精度

Conclusion: 扩散模型作为非侵入性心脏电生理成像的稳健工具具有潜力，能够处理逆问题的非唯一性并提供概率性重建
...
### [375] [CASSANDRA: Programmatic and Probabilistic Learning and Inference for Stochastic World Modeling](https://arxiv.org/abs/2601.18620)
*Panagiotis Lymperopoulos,Abhiramon Rajasekharan,Ian Berlot-Attwell,Stéphane Aroca-Ouellette,Kaheer Suleman*

Main category: cs.LG

TL;DR: CASSANDRA是一种神经符号世界建模方法，利用LLM作为知识先验构建轻量级规划用转移模型，在商业模拟器中显著优于基线方法。
...
Method: CASSANDRA整合两个组件：1）LLM合成的代码用于建模确定性特征；2）LLM引导的概率图模型结构学习，用于捕捉随机变量间的因果关系。

Result: 在咖啡店模拟器和复杂主题公园商业模拟器中评估，CASSANDRA在转移预测和规划方面相比基线方法有显著改进。

Conclusion: CASSANDRA通过结合LLM的知识先验和符号建模，能够有效构建轻量级世界模型，在复杂商业领域规划任务中表现出色。
...
### [376] [Rank-1 Approximation of Inverse Fisher for Natural Policy Gradients in Deep Reinforcement Learning](https://arxiv.org/abs/2601.18626)
*Yingxiao Huo,Satya Prakash Dash,Radu Stoican,Samuel Kaski,Mingfei Sun*

Main category: cs.LG

TL;DR: 提出一种基于秩-1近似的自然策略优化方法，避免了完整Fisher信息矩阵求逆的计算负担，在多种环境中优于标准actor-critic和信任域基线方法。


<details>
  <summary>Details</summary>
Motivation: 自然梯度在深度强化学习中具有快速收敛特性，但计算自然梯度需要在每次迭代中求逆Fisher信息矩阵，这在计算上是不可行的。需要一种高效且可扩展的自然策略优化技术。

Method: 提出一种利用秩-1近似来替代完整逆Fisher信息矩阵的高效自然策略优化方法。理论上证明在特定条件下，这种秩-1近似比策略梯度收敛更快，并且在某些条件下具有与随机策略梯度方法相同的样本复杂度。

Result: 在多样化环境上进行基准测试，结果显示该方法在性能上优于标准的actor-critic和信任域基线方法。

Conclusion: 通过秩-1近似实现了一种计算高效的自然策略优化方法，既保持了自然梯度的优势，又避免了昂贵的矩阵求逆计算，在实际应用中表现出优越性能。
...
### [377] [Physics-Informed Uncertainty Enables Reliable AI-driven Design](https://arxiv.org/abs/2601.18638)
*Tingkai Xue,Chin Chun Ooi,Yang Jiang,Luu Trung Pham Duong,Pao-Hsiung Chiu,Weijiang Zhao,Nagarajan Raghavan,My Ha Dao*

Main category: cs.LG

TL;DR: 提出物理信息不确定性作为预测不确定性的廉价有效代理，用于频率选择表面逆设计，将高性能方案发现率从<10%提升至>50%，计算成本降低一个数量级。


<details>
  <summary>Details</summary>
Motivation: 传统基于深度学习的代理辅助优化方法通常缺乏不确定性量化，导致在数据稀疏区域产生错误预测，影响优化性能。需要一种能有效量化预测不确定性的方法，以提升高维物理系统逆设计的成功率。

Method: 提出物理信息不确定性范式，利用模型预测违反基本物理定律的程度作为预测不确定性的廉价有效代理。将物理信息不确定性集成到多保真度不确定性感知优化工作流中，用于设计20-30 GHz范围内的复杂频率选择表面。

Result: 将高性能方案发现率从不到10%提升至超过50%，同时计算成本相比仅使用高保真求解器降低一个数量级。

Conclusion: 物理信息不确定性可作为物理系统代理模型中量化不确定性的可行替代方案，为能高效稳健探索和评估候选设计的自主科学发现系统奠定基础。
...
### [378] [TwinPurify: Purifying gene expression data to reveal tumor-intrinsic transcriptional programs via self-supervised learning](https://arxiv.org/abs/2601.18640)
*Zhiwei Zheng,Kevin Bryson*

Main category: cs.LG

TL;DR: TwinPurify是一个基于Barlow Twins自监督学习的表示学习框架，用于从批量转录组数据中提取肿瘤特异性信号，无需外部参考，通过利用同一队列中的相邻正常组织作为背景指导来消除肿瘤纯度变异的影响。


<details>
  <summary>Details</summary>
Motivation: 尽管单细胞和空间转录组技术取得了进展，但大规模患者队列研究仍依赖批量转录组数据。肿瘤纯度的变异会掩盖肿瘤内在的转录信号，限制下游发现。现有的去卷积方法在合成混合物上表现良好，但由于未建模的生物和技术变异，难以推广到真实患者队列。

Method: TwinPurify采用Barlow Twins自监督学习目标，学习连续的高维肿瘤嵌入表示。它利用同一队列中的相邻正常组织作为"背景"指导，无需外部参考，通过表示学习而非传统的去卷积方法来分离肿瘤特异性信号。

Result: 在多个大型癌症队列（RNA-seq和微阵列平台）的基准测试中，TwinPurify在恢复肿瘤内在和免疫信号方面优于传统表示学习方法（如自编码器）。纯化的嵌入改进了分子亚型和分级分类，增强了生存模型一致性，并揭示了比原始批量谱更有生物学意义的通路活性。

Conclusion: TwinPurify为批量转录组学去污染提供了一个可转移的框架，扩展了现有临床数据集在分子发现中的实用性，通过表示学习而非去卷积的方法有效解决了肿瘤纯度变异问题。
...
### [379] [FaLW: A Forgetting-aware Loss Reweighting for Long-tailed Unlearning](https://arxiv.org/abs/2601.18650)
*Liheng Yu,Zhe Zhao,Yuxuan Wang,Pengkun Wang,Binwu Wang,Yang Wang*

Main category: cs.LG

TL;DR: 该论文首次研究长尾分布下的机器遗忘问题，提出FaLW方法通过实例级动态损失重加权解决现有方法的异质遗忘偏差和倾斜遗忘偏差问题。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘研究主要在相对平衡的遗忘集上评估，忽略了现实世界中数据通常遵循长尾分布（如用户活动记录）的情况。这是该领域的重要研究空白。

Method: 提出FaLW方法：一种即插即用的实例级动态损失重加权方法。通过比较每个样本的预测概率与同类未见数据的分布来评估其遗忘状态，然后使用由平衡因子调制的遗忘感知重加权方案自适应调整每个样本的遗忘强度。

Result: 大量实验表明，FaLW在长尾分布设置下实现了优越的性能，有效解决了异质遗忘偏差和倾斜遗忘偏差问题。

Conclusion: 该研究填补了长尾分布下机器遗忘的研究空白，提出的FaLW方法能够有效处理现实世界中常见的长尾数据遗忘场景，为数据隐私法规的实践应用提供了更好的解决方案。
...
### [380] [A Dynamic Framework for Grid Adaptation in Kolmogorov-Arnold Networks](https://arxiv.org/abs/2601.18672)
*Spyros Rigas,Thanasis Papaioannou,Panagiotis Trakadas,Georgios Alexandridis*

Main category: cs.LG

TL;DR: 本文提出了一种基于曲率的重要性密度函数框架，用于改进KAN网络的网格自适应策略，相比传统基于输入数据密度的方法，在多个基准测试中显著降低了误差。


<details>
  <summary>Details</summary>
Motivation: 现有KAN网格自适应策略仅依赖输入数据密度，未能考虑目标函数的几何复杂性或训练过程中的计算指标，限制了网络性能。

Method: 提出将节点分配视为由重要性密度函数（IDFs）控制的密度估计任务，引入基于曲率的自适应策略，让训练动态决定网格分辨率。

Result: 在合成函数拟合、Feynman数据集回归和Helmholtz PDE问题上，相比标准输入基线方法，平均相对误差分别降低了25.3%、9.4%和23.3%，统计显著性得到验证。

Conclusion: 基于曲率的自适应策略是KAN训练中一种鲁棒且计算高效的替代方案，能够更好地捕捉目标函数的几何特性。
...
### [381] [Learning temporal embeddings from electronic health records of chronic kidney disease patients](https://arxiv.org/abs/2601.18675)
*Aditya Kumar,Mario A. Cypko,Oliver Amft*

Main category: cs.LG

TL;DR: 研究探索基于电子病历的时间嵌入模型能否在不牺牲预测性能的情况下学习临床有意义的表示，以及架构选择如何影响嵌入质量。时间感知LSTM（T-LSTM）在慢性肾病阶段聚类和ICU死亡率预测中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 模型引导医学需要能够捕捉疾病动态且保持透明、任务无关的表示，而大多数临床预测模型仅针对单一任务优化。表示学习有助于学习能泛化到下游任务的嵌入，循环架构适合建模临床数据的时间结构。

Method: 使用MIMIC-IV数据集，研究慢性肾病患者，比较三种循环架构：普通LSTM、注意力增强LSTM和时间感知LSTM（T-LSTM）。所有模型都作为嵌入模型和直接端到端预测器进行训练。通过CKD阶段聚类和ICU内死亡率预测评估嵌入质量。

Result: T-LSTM产生更有结构的嵌入，Davies-Bouldin指数更低（9.91），CKD阶段分类准确率更高（0.74），优于普通LSTM（15.85，0.63）和注意力增强LSTM（20.72，0.67）。在ICU死亡率预测中，嵌入模型始终优于端到端预测器，准确率从0.72-0.75提升到0.82-0.83。

Conclusion: 时间感知LSTM架构能学习到临床有意义的表示而不损害预测性能，嵌入模型作为中间步骤比直接端到端学习更有效，为模型引导医学提供了有前景的方向。
...
### [382] [Quasi Monte Carlo methods enable extremely low-dimensional deep generative models](https://arxiv.org/abs/2601.18676)
*Miles Martinez,Alex H. Williams*

Main category: cs.LG

TL;DR: QLVMs是一种专门用于寻找高维数据极低维可解释嵌入的深度生成模型，通过拟蒙特卡洛积分直接近似边缘似然，在一维、二维和三维潜在空间中表现优于传统VAE和IWAE。


<details>
  <summary>Details</summary>
Motivation: 传统变分自编码器（VAE）和重要性加权自编码器（IWAE）在低维潜在空间中难以获得可解释的嵌入，且变分下界方法在高维空间中效果有限。需要一种专门针对极低维嵌入优化的方法，以实现透明的可视化和潜在空间分析。

Method: 提出拟蒙特卡洛潜在变量模型（QLVMs），采用拟蒙特卡洛积分直接近似边缘似然，而不是依赖学习编码器和变分下界。这种方法专门针对一维、二维和三维的深度潜在变量模型进行优化。

Result: 在多个数据集上，QLVMs在匹配潜在维度的情况下，始终优于传统VAE和IWAE。生成的嵌入支持透明可视化、非参数密度估计、聚类和测地线路径计算等后验分析。

Conclusion: 虽然QLVMs计算密集且在复杂数据集上难以生成精细细节，但对于优先考虑可解释性和潜在空间分析的应用，它提供了一个有吸引力的解决方案，特别适合需要极低维嵌入的场景。
...
### [383] [Counterfactual Explanations on Robust Perceptual Geodesics](https://arxiv.org/abs/2601.18678)
...
<details>
  <summary>Details</summary>
Motivation: 现有反事实解释方法存在距离度量选择模糊性问题：不同的距离度量会导致结果要么有意义要么变成对抗性扰动。现有方法采用平坦或不对齐的几何结构，导致离流形伪影、语义漂移或对抗性崩溃。

Method: 提出感知反事实测地线（PCG）方法，利用鲁棒视觉特征诱导的感知黎曼度量构造反事实，通过追踪测地线实现平滑、在流形上、语义有效的转换。

Result: 在三个视觉数据集上的实验表明，PCG优于基线方法，并揭示了在标准度量下隐藏的失败模式。

Conclusion: PCG通过感知对齐的黎曼几何解决了反事实解释中的距离度量模糊性问题，能够生成语义有效的反事实解释，揭示了现有方法的局限性。
...
### [384] [ART for Diffusion Sampling: A Reinforcement Learning Approach to Timestep Schedule](https://arxiv.org/abs/2601.18681)
*Yilie Huang,Wenpin Tang,Xunyu Zhou*

Main category: cs.LG

TL;DR: ART-RL：通过强化学习自适应调整扩散模型时间步长，优化采样效率


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型使用均匀或手工设计的时间网格可能不是最优的，特别是在给定时间步数预算的情况下。需要一种自适应的时间步长调整方法来最小化离散化误差。

Method: 提出自适应重参数化时间（ART）方法，控制重参数化时间变量的时钟速度，导致时间变化和不均匀的时间步长。进一步提出ART-RL，将时间变化建模为连续时间强化学习问题，使用高斯策略，通过actor-critic更新学习最优时间调度。

Result: 基于EDM框架，ART-RL在CIFAR-10上显著改善了Fréchet Inception Distance（FID），并且在AFHQv2、FFHQ和ImageNet等数据集上具有良好迁移性，无需重新训练。

Conclusion: ART-RL提供了一种数据驱动的方法来自适应优化扩散模型的时间调度，提高了采样效率和质量，具有很好的泛化能力。
...
### [385] [Explainability Methods for Hardware Trojan Detection: A Systematic Comparison](https://arxiv.org/abs/2601.18696)
*Paul Whitten,Francis Wolff,Chris Papachristou*

Main category: cs.LG

TL;DR: 本文比较了硬件木马检测中的三种可解释性方法：基于属性的分析、基于案例的推理和模型无关特征归因，发现前两者在领域对齐和可解释性方面优于后者，同时XGBoost分类器在精度上比先前工作提升了9倍。


<details>
  <summary>Details</summary>
Motivation: 硬件木马检测需要准确识别和可解释的解释，以便安全工程师验证和采取行动。本研究旨在比较不同可解释性方法在门级木马检测中的效果，为实际部署提供指导。

Method: 在Trust-Hub基准上比较三种可解释性方法：(1) 基于31个电路特定特征的领域感知属性分析；(2) 使用k近邻的基于案例推理；(3) 模型无关特征归因方法（LIME、SHAP、梯度）。使用XGBoost分类器进行检测，评估11,392个测试样本。

Result: XGBoost分类器达到46.15%精度和52.17%召回率，比先前工作（Hasegawa等：5.13%）精度提升9倍，假阳性率从5.6%降至0.25%。基于案例的推理实现97.4%的预测与训练样本对应性。LIME和SHAP特征归因相关性高（r=0.94）但缺乏电路级上下文。

Conclusion: 基于属性的分析和基于案例的推理方法在领域对齐和基于先例的可解释性方面优于通用特征排名方法，对于需要验证ML预测的实际部署场景具有重要价值。梯度归因比SHAP快481倍但提供类似的领域不透明见解。
...
### [386] [Mechanistic Analysis of Catastrophic Forgetting in Large Language Models During Continual Fine-tuning](https://arxiv.org/abs/2601.18699)
*Olaf Yunus Laitinen Imanov*

Main category: cs.LG

TL;DR: 论文对Transformer大语言模型在顺序微调中的灾难性遗忘现象进行了机制分析，发现梯度干扰、表征漂移和损失景观平坦化是主要驱动因素，遗忘严重程度与任务相似度高度相关。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型通过预训练和微调在各种任务上表现出色，但在顺序任务上进行持续微调会导致灾难性遗忘，新获得的知识会干扰先前学习的能力。虽然这种现象被广泛观察到，但其机制理解仍然有限。

Method: 对基于Transformer的大语言模型在顺序微调过程中的灾难性遗忘进行了全面的机制分析。通过跨多个模型规模（109B到400B总参数）和任务序列的系统实验，识别了驱动遗忘的主要机制。

Result: 识别出三个主要遗忘机制：注意力权重中的梯度干扰、中间层的表征漂移以及损失景观平坦化。遗忘严重程度与任务相似度高度相关（Pearson r = 0.87），大约15-23%的注意力头在微调过程中受到严重干扰，较低层表现出更大的易感性。

Conclusion: 这些发现为开发持续学习系统中的针对性缓解策略建立了机制基础，有助于理解大语言模型在顺序微调中的遗忘现象，并为改进模型稳定性提供理论依据。
...
### [387] [From Fuzzy to Exact: The Halo Architecture for Infinite-Depth Reasoning via Rational Arithmetic](https://arxiv.org/abs/2601.18702)
*Hansheng Ren*

Main category: cs.LG

TL;DR: 该论文挑战了深度学习优先计算吞吐量而非数值精度的现有范式，提出精确性假说：通用人工智能需要任意精度算术计算基础，并引入基于有理数算术的Halo架构来减少逻辑不确定性。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习范式过于注重计算吞吐量而忽视数值精度，假设智能源于大规模统计相关性。作者认为当前大语言模型中的"幻觉"和逻辑不一致是IEEE 754浮点近似误差在深度组合函数中累积的结果，需要从根本上解决数值精度问题。

Method: 提出精确性假说，认为通用智能需要任意精度算术计算基础。引入Halo架构，采用有理数算术（ℚ）作为计算范式，并设计新型精确推理单元（EIU）来支持精确计算。

Result: 在Huginn-0125原型上的实证验证显示，当6000亿参数的BF16基线在混沌系统中崩溃时，Halo架构能够无限期保持零数值发散，证明了精确算术在减少逻辑不确定性方面的有效性。

Conclusion: 精确算术是减少System 2 AGI逻辑不确定性的先决条件，为通用人工智能的发展提供了新的计算基础方向，挑战了当前以计算吞吐量为优先的深度学习范式。
...
### [388] [SMART: Scalable Mesh-free Aerodynamic Simulations from Raw Geometries using a Transformer-based Surrogate Model](https://arxiv.org/abs/2601.18707)
*Jan Hagnberger,Mathias Niepert*

Main category: cs.LG

TL;DR: SMART是一种基于神经网络的代理模型，仅使用点云几何表示（无需仿真网格）即可预测任意查询位置的物理量，性能与依赖网格的方法相当甚至更优。


<details>
  <summary>Details</summary>
Motivation: 现有基于网格的代理模型虽然精度较高，但为新几何生成仿真网格计算成本高昂；而无网格方法通常误差较大。需要一种既高效又准确的替代方案。

Method: SMART将几何和仿真参数编码到共享潜在空间，捕捉结构和参数特征。物理解码器通过跨层交互关注编码器的中间潜在表示，将空间查询映射到物理量，联合更新潜在几何特征和演化物理场。

Result: 大量实验表明，SMART与依赖仿真网格作为输入的现有方法相比具有竞争力，且经常表现更优，展示了其在工业级仿真中的能力。

Conclusion: SMART提供了一种高效准确的网格无关代理模型方案，仅需点云表示即可实现高质量物理仿真预测，有望应用于工业级复杂几何仿真。
...
### [389] [Riemannian AmbientFlow: Towards Simultaneous Manifold Learning and Generative Modeling from Corrupted Data](https://arxiv.org/abs/2601.18728)
*Willem Diepeveen,Oscar Leong*

Main category: cs.LG

TL;DR: Riemannian AmbientFlow：从噪声观测中同时学习概率生成模型和底层非线性数据流形的框架，结合变分推断和黎曼几何，为逆问题提供理论保证。


<details>
  <summary>Details</summary>
Motivation: 在许多科学和成像应用中，只能获得噪声或线性损坏的测量数据，而无法获得干净样本。同时，数据中的潜在结构（如流形几何）对下游科学分析很重要。现有方法难以同时从损坏观测中学习生成模型和底层流形结构。

Method: 基于AmbientFlow的变分推断框架，结合由归一化流诱导的数据驱动黎曼几何。通过拉回度量和黎曼自编码器提取流形结构。在几何正则化和测量条件下，学习平滑、双Lipschitz流形参数化。

Result: 理论保证表明，在适当的几何正则化和测量条件下，学习模型能以可控误差恢复底层数据分布，并获得平滑的双Lipschitz流形参数化。平滑解码器可作为逆问题的原则性生成先验，具有恢复保证。

Conclusion: Riemannian AmbientFlow为从损坏观测中同时学习生成模型和底层流形提供了理论保证的框架，在低维合成流形和MNIST上进行了实证验证，为逆问题提供了原则性生成先验。
...
### [390] [Self-Distilled Reasoner: On-Policy Self-Distillation for Large Language Models](https://arxiv.org/abs/2601.18734)
*Siyan Zhao,Zhihui Xie,Mengchen Liu,Jing Huang,Guan Pang,Feiyu Chen,Aditya Grover*

Main category: cs.LG

TL;DR: 提出On-Policy Self-Distillation (OPSD)框架，让单个大语言模型既当老师又当学生，通过在不同上下文条件下生成不同策略，实现自我蒸馏，提高数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏需要单独的教师模型，且无法充分利用推理数据集中的真实解决方案。受启发于"足够强大的LLM能够解释外部特权推理轨迹并教导其较弱版本"的直觉，提出让单个模型自我蒸馏的方法。

Method: OPSD框架中，单个模型同时扮演教师和学生角色：教师策略基于特权信息（如已验证的推理轨迹），学生策略仅看到问题。训练时在学生自己的轨迹上最小化这两个策略之间的每token分布差异。

Result: 在多个数学推理基准测试中，相比GRPO等强化学习方法实现了4-8倍的token效率提升，且性能优于离策略蒸馏方法。

Conclusion: OPSD框架通过让单个模型自我蒸馏，既解决了传统蒸馏方法需要单独教师模型的问题，又充分利用了数据集中的真实解决方案，在数学推理任务上实现了高效且优越的性能。
...
### [391] [Benchmarking Machine Learning Models for IoT Malware Detection under Data Scarcity and Drift](https://arxiv.org/abs/2601.18736)
*Jake Lyon,Ehsan Saeedizade,Shamik Sengupta*

Main category: cs.LG

TL;DR: 该研究评估了四种监督学习模型在IoT-23数据集上的恶意软件检测与分类性能，发现树基模型在有限训练数据下表现优异，但性能随时间推移而下降。


<details>
  <summary>Details</summary>
Motivation: 随着物联网在智慧城市、交通和工业系统中的快速扩张，其安全漏洞问题日益紧迫。IoT设备通常计算资源有限、物理防护薄弱，部署在异构动态网络中，容易成为网络攻击和恶意软件的目标。机器学习为自动化恶意软件检测提供了有前景的解决方案，但实际部署需要既有效又轻量级的模型。

Method: 研究调查了四种监督学习模型（随机森林、LightGBM、逻辑回归和多层感知机）在IoT-23数据集上的有效性。评估了模型在二分类和多分类任务中的性能，对训练数据量的敏感性，以及时间鲁棒性分析以模拟在不断演变的威胁环境中的部署。

Result: 结果显示树基模型（随机森林和LightGBM）即使在有限训练数据下也能实现高准确率和良好泛化能力。然而，随着恶意软件多样性的增加，所有模型的性能都会随时间推移而恶化。

Conclusion: 这些发现强调了为现实世界物联网系统安全部署自适应、资源高效的机器学习模型的重要性。树基模型在资源受限的IoT环境中表现出色，但需要持续更新以适应不断演变的威胁环境。
...
### [392] [Trust, Don't Trust, or Flip: Robust Preference-Based Reinforcement Learning with Multi-Expert Feedback](https://arxiv.org/abs/2601.18751)
*Seyed Amir Hosseini,Maryam Abdolali,Amirhosein Tavakkoli,Fardin Ayar,Ehsan Javanmardi,Manabu Tsukada,Mahdi Javanmardi*

Main category: cs.LG

TL;DR: TriTrust-PBRL (TTP) 是一个新的偏好强化学习框架，通过联合学习共享奖励模型和专家特定信任参数，自动处理包含可靠、嘈杂和对抗性注释者的异构偏好数据。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的偏好数据通常来自具有不同可靠性的异构注释者，包括准确、嘈杂和系统性对抗的注释者。现有的PBRL方法要么平等对待所有反馈，要么试图过滤不可靠来源，但在面对系统性提供错误偏好的对抗性注释者时都会失败。

Method: TTP框架联合学习共享奖励模型和专家特定信任参数。关键洞察是信任参数在基于梯度的优化过程中自然演变为正值（信任）、接近零（忽略）或负值（翻转），使模型能够自动反转对抗性偏好并恢复有用信号，而不是简单地丢弃损坏的反馈。

Result: TTP在四个不同领域（包括MetaWorld的操纵任务和DM Control的移动任务）的各种损坏场景下实现了最先进的鲁棒性。在对抗性损坏下保持接近Oracle的性能，而标准PBRL方法则完全失败。TTP成功地从包含可靠和对抗性注释者的混合专家池中学习，表现优于现有基线方法。

Conclusion: TTP提供了一个统一的框架，能够从异构专家偏好反馈中学习，自动适应不同专家的可靠性，包括正确处理对抗性注释者，而无需超出识别索引的专家特征，并能与现有PBRL流程无缝集成。
...
### [393] [HalluGuard: Demystifying Data-Driven and Reasoning-Driven Hallucinations in LLMs](https://arxiv.org/abs/2601.18753)
*Xinyue Zeng,Junhong Lin,Yujun Yan,Feng Guo,Liang Shi,Jun Wu,Dawei Zhou*

Main category: cs.LG

TL;DR: 提出Hallucination Risk Bound理论框架，将幻觉风险分解为数据驱动和推理驱动两部分，并基于此开发HalluGuard检测方法，在多个基准测试中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: LLM在医疗、法律等高风险领域的可靠性常因幻觉而受损，现有检测方法通常只针对单一幻觉来源且依赖任务特定启发式，泛化能力有限。

Method: 提出Hallucination Risk Bound统一理论框架，将幻觉风险分解为训练时数据不匹配和推理时不稳定性两部分；基于此开发HalluGuard，利用NTK诱导的几何结构和捕获的表示来联合检测两种幻觉。

Result: 在10个多样化基准测试、11个竞争基线和9个流行LLM骨干网络上评估，HalluGuard在检测多种LLM幻觉形式方面始终达到最先进的性能。

Conclusion: Hallucination Risk Bound为分析幻觉产生和演化提供了理论基础，HalluGuard作为统一的检测方法能有效识别数据驱动和推理驱动的幻觉，提升LLM在高风险领域的可靠性。
...
### [394] [Beyond Preferences: Learning Alignment Principles Grounded in Human Reasons and Values](https://arxiv.org/abs/2601.18760)
*Henry Bell,Lara Neubauer da Costa Schertel,Bochu Ding,Brandon Fain*

Main category: cs.LG

TL;DR: 提出Grounded Constitutional AI (GCAI)框架，通过结合用户对AI的普遍期望（通用原则）和交互时偏好（情境原则）来生成更具代表性、道德基础更强的人工智能宪法。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型对齐中，宪法原则通常由自然语言指定，但如何公平确定这些原则以广泛代表利益相关者的价值观尚不明确。需要一种能同时考虑用户普遍期望和具体交互偏好的宪法生成方法。

Method: 扩展Inverse Constitutional AI (ICAI)方法，利用人类偏好标注数据中提供的理由生成情境原则。同时从用户关于AI价值观的陈述中提取通用原则。将这两类原则结合形成完整的宪法。

Result: 人类评估显示，GCAI生成的宪法在个人偏好和广泛治理AI行为方面都优于ICAI生成的宪法。参与者认为GCAI宪法更具道德基础、更连贯、更多元化。

Conclusion: GCAI框架能够生成更全面代表用户价值观的AI宪法，为AI对齐提供了更公平、更具代表性的宪法制定方法，有助于开发更符合人类价值观的AI系统。
...
### [395] [PRECISE: Reducing the Bias of LLM Evaluations Using Prediction-Powered Ranking Estimation](https://arxiv.org/abs/2601.18777)
*Abhishek Divekar,Anirban Majumder*

Main category: cs.LG

TL;DR: 提出PRECISE框架，结合少量人工标注与LLM判断来评估搜索/排序/RAG系统质量，显著减少标注需求


<details>
  <summary>Details</summary>
Motivation: 传统搜索/排序/RAG系统评估需要大量人工相关性标注，LLM作为自动评估器存在固有偏差，需要更高效的统计框架

Method: 扩展预测驱动推理(PPI)框架，结合少量人工标注查询(100个)和大量未标注数据(10,000个)，通过重新定义指标集成空间将计算复杂度从O(2^|C|)降至O(2^K)

Result: 在多个检索数据集上验证，PRECISE显著降低了Precision@K指标的估计方差，有效校正了低资源设置下的LLM偏差

Conclusion: PRECISE框架为LLM辅助的系统评估提供了可靠的统计基础，大幅减少人工标注需求，适用于查询-文档级别的子实例标注任务
...
### [396] [Teaching Models to Teach Themselves: Reasoning at the Edge of Learnability](https://arxiv.org/abs/2601.18778)
*Shobhita Sundaram,John Quan,Ariel Kwiatkowski,Kartik Ahuja,Yann Ollivier,Julia Kempe*

Main category: cs.LG

TL;DR: SOAR是一个基于元强化学习的自改进框架，通过教师模型生成合成问题来训练学生模型，利用学生进步作为奖励信号，在初始成功率极低（0/128）的数学基准上实现了突破性学习。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习方法在微调大型推理模型时，当面对初始成功率极低的数据集时，由于缺乏训练信号而陷入学习停滞。研究探索预训练LLM是否能够利用潜在知识为自身无法解决的问题生成自动化课程。

Method: SOAR框架采用元强化学习设计：教师模型副本为学生模型副本生成合成问题，通过学生在少量难题子集上的进步作为奖励信号。关键创新在于将课程生成基于实际学生进步而非内在代理奖励。

Result: 在数学基准最难题集（初始成功率0/128）上的实验显示：1）实现了双层元强化学习，在稀疏二元奖励下解锁学习能力；2）基于实际进步的奖励优于先前LLM自玩的内在奖励方案，避免不稳定性和多样性崩溃；3）生成问题的结构质量和明确性比解决方案正确性对学习进步更重要。

Conclusion: 研究表明，生成有用"垫脚石"问题不需要预先具备解决难题的能力，这为无需额外策划数据即可突破推理停滞提供了原则性路径，揭示了预训练模型通过元强化学习实现自我改进的潜力。
...
### [397] [POPE: Learning to Reason on Hard Problems via Privileged On-Policy Exploration](https://arxiv.org/abs/2601.18779)
*Yuxiao Qu,Amrith Setlur,Virginia Smith,Ruslan Salakhutdinov,Aviral Kumar*

Main category: cs.LG

TL;DR: 提出POPE方法，利用特权信息（如人类解决方案）引导强化学习在困难问题上的探索，解决传统RL方法在困难推理问题上探索失败的问题。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习方法在训练大型语言模型推理能力时，面对困难问题经常无法探索到任何正确解，导致零奖励和缺乏学习信号。传统解决方案如熵奖励、重要性采样调整或直接优化pass@k目标都无法有效解决此问题，而混合难易问题训练反而会产生"射线干扰"现象，阻碍在困难问题上的进展。

Method: 提出特权在线策略探索（POPE）方法：1）利用人类或其他oracle解决方案作为特权信息；2）在困难问题前添加oracle解决方案的前缀，引导RL获得非零奖励；3）通过指令跟随和推理的协同作用，将习得的行为迁移回原始无引导问题。

Result: POPE显著扩展了可解决问题集合，在具有挑战性的推理基准测试中大幅提升了性能表现。

Conclusion: POPE通过特权信息引导探索，有效解决了RL在困难推理问题上的探索失败问题，相比传统方法显著提升了模型在复杂推理任务上的能力。
...
### [398] [Multi-Objective Reinforcement Learning for Efficient Tactical Decision Making for Trucks in Highway Traffic](https://arxiv.org/abs/2601.18783)
*Deepthi Pathare,Leo Laine,Morteza Haghir Chehreghani*

Main category: cs.LG

TL;DR: 提出基于PPO的多目标强化学习框架，为重型车辆学习连续帕累托最优策略集，平衡安全、能效和时间效率的权衡


<details>
  <summary>Details</summary>
Motivation: 高速公路驾驶中平衡安全性、效率和运营成本对重型车辆是挑战性决策问题，传统标量奖励函数会模糊这些竞争目标间的权衡结构

Method: 基于近端策略优化(PPO)的多目标强化学习框架，在卡车战术决策模拟平台上学习连续策略集，显式表示安全、能效和时间效率三个冲突目标间的权衡

Result: 学习到平滑可解释的帕累托前沿，捕获了碰撞/完成率（安全）、能源成本（能效）和驾驶员成本（时间效率）之间的权衡，无需重新训练即可在不同驾驶策略间无缝切换

Conclusion: 该框架为自动驾驶卡车应用提供了灵活、稳健和自适应的决策策略，能够根据不同的冲突目标选择驾驶行为
...
### [399] [Reuse your FLOPs: Scaling RL on Hard Problems by Conditioning on Very Off-Policy Prefixes](https://arxiv.org/abs/2601.18795)
*Amrith Setlur,Zijian Wang,Andrew Cohen,Paria Rashidinejad,Sang Michael Xie*

Main category: cs.LG

TL;DR: PrefixRL：通过重用旧采样FLOPs中的离轨迹前缀，调节问题难度来提升强化学习效率，避免离轨不稳定性


<details>
  <summary>Details</summary>
Motivation: 传统RL方法在LLM推理任务中，面对困难问题时正确轨迹稀少、策略梯度消失、学习停滞，计算效率低下。需要重用先前推理或RL训练中的离轨迹来提升效率

Method: PrefixRL方法：利用成功离轨迹的前缀作为条件，运行在轨RL来完成剩余部分。通过调节前缀长度来调整问题难度，避免离轨不稳定性。使用基础模型通过拒绝采样获取离轨迹，形成自我改进循环

Result: 在困难推理问题上，PrefixRL达到相同训练奖励的速度比最强基线（在离轨数据上SFT然后RL）快2倍，即使考虑初始拒绝采样的计算成本。最终奖励提升3倍。效果可迁移到保留基准测试，即使离轨迹来自不同模型家族也有效

Conclusion: PrefixRL通过重用离轨迹前缀有效提升RL效率，避免离轨不稳定性，在困难推理问题上显著优于现有方法，具有实际应用的灵活性
