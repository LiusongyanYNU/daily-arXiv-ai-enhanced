...
Motivation: 为了解决视觉-语言模型在动作生成方面的局限性，需要开发能够将语言、视觉和动作表示空间对齐的框架，以增强机器人的3D感知和推理能力。

Method: 使用双层动作表示框架：潜在动作模型（从跨具身操作数据中提取高层意图）和结构化离散动作令牌（通过频域变换编码低层动态），结合机器人轨迹数据与通用QA数据集进行混合训练。

Result: 在LIBERO Franka基准测试中表现优异，在真实世界评估中在多样且具有挑战性的操作任务上达到有竞争力的成功率。

Conclusion: iFlyBot-VLA通过双级监督有效对齐了语言、视觉和动作表示空间，使VLM能够直接参与动作生成，为机器人操作任务提供了有效的解决方案。
...
TL;DR: 首次全面评估基础模型在胎儿超声成像中低类间变异性条件下的表现，发现领域特定的预训练对区分解剖结构相似的胎儿脑标准切面至关重要。


<details>
  <summary>Details</summary>
Motivation: 虽然DINOv3等视觉基础模型在医疗领域显示出良好的可迁移性，但其在解剖结构相似情况下的区分能力尚未系统研究。本研究针对胎儿脑标准切面（TT、TV、TC）这一具有高度重叠解剖特征且对可靠生物测量评估至关重要的挑战性问题。

Method: 收集并整合所有公开可用的胎儿超声数据集，构建包含超过18.8万张标注图像的多中心基准FetalUS-188K。采用自监督方式预训练DINOv3学习超声感知表示，通过线性探测和全微调等标准化适应协议进行评估，比较在胎儿超声数据预训练和自然图像DINOv3权重初始化两种方案。

Result: 在胎儿超声数据上预训练的模型始终优于自然图像初始化的模型，加权F1分数提升高达20%。领域自适应预训练使网络能够保留区分中间切面（如TV）所需的关键细微回声和结构线索。
...
TL;DR: Geo-Foundational Models (GFMs) 在洪水淹没制图中与传统U-Net模型相比，提供了小幅到中度的精度提升，同时计算成本和标注需求更低。Clay模型在多个传感器上表现最佳，特别是在少样本学习中优势明显。
...
Result: 所有GFMs表现竞争性，最佳和最差模型间仅有2-5%差异。Clay在PlanetScope（0.79 mIoU）和Sentinel-2（0.70）上表现最佳，Prithvi在Sentinel-1（0.57）上领先。在少样本实验中，Clay仅用5张训练图像就达到0.64 mIoU，远优于其他模型。

Conclusion: 与先前研究相反，GFMs在洪水制图精度上相比传统U-Net提供了小幅到中度的改进，同时计算成本和标注工作量更低，Clay因其较小的模型尺寸和更好的性能成为更优选择。
...
TL;DR: 提出一种基于学习框架的图像重建方法，针对固定确定性采样模式下的不完整测量问题，利用图像分布的多种不变性实现与全监督方法相当的重建性能。


<details>
  <summary>Details</summary>
Motivation: 传统监督方法需要完整采样数据，自监督方法依赖随机采样模式，而实际应用中常遇到固定、确定性采样模式且覆盖不完整的情况，需要新的解决方案。

Method: 利用底层图像分布的多种不变性，在固定确定性采样模式下实现图像重建，包括上采样和修复任务。

Result: 在光分辨率图像上采样任务中验证，展示了竞争性或更优的结果，同时需要显著更少的真实数据。

Conclusion: 该方法能够克服固定采样模式覆盖不完整的限制，在减少真实数据需求的同时实现与全监督方法相当的重建性能。
...
TL;DR: 本文系统评估了三种大型多模态模型在医疗影像中受保护健康信息检测的性能，发现LMM在OCR准确性上优于传统模型，但在整体PHI检测精度上提升有限，特别在复杂文本模式场景中表现更佳。
...
Method: 使用GPT-4o、Gemini 2.5 Flash和Qwen 2.5 7B三种LMM，比较两种处理流程：纯文本分析和OCR+语义分析集成流程。

Result: LMM在OCR准确性上显著优于EasyOCR等传统模型（WER:0.03-0.05，CER:0.02-0.03），但在整体PHI检测精度上提升不一致，在复杂文本模式场景中表现最佳。
...
TL;DR: 介绍了StrengthSense数据集，包含11种力量需求活动和2种非力量需求活动的IMU信号，来自29名健康受试者，使用10个IMU传感器采集，并通过视频标注验证数据准确性。
...
Method: 从29名健康受试者使用10个IMU传感器采集数据，涵盖11种力量需求活动和2种非力量需求活动，通过视频记录进行标注，并比较IMU估计的关节角度与视频提取的角度来验证数据准确性。

Result: 创建了包含13种活动的StrengthSense开放数据集，技术验证显示IMU数据与视频数据在关节角度估计上具有良好的一致性和可靠性。

Conclusion: StrengthSense数据集可用于推进人类活动识别算法开发、健身和健康监测应用等研究，为相关领域提供有价值的资源。
...
TL;DR: 提出一个自动化合成文本视觉问答数据集的端到端流水线，利用OCR、区域检测、图像描述生成和问题生成等技术，能够基于场景文本自动生成忠实的问题-答案对。


<details>
  <summary>Details</summary>
Motivation: 构建大规模文本视觉问答数据集需要大量人工标注，过程繁琐且具有挑战性，急需建立能够基于图像场景文本自动合成问答对的端到端流水线。

Method: 整合多种模型和算法，包括OCR检测识别、感兴趣区域检测、图像描述生成和问题生成，将这些组件整合成统一的流水线来自动合成和验证问答对。

Result: 成功构建了首个自动合成的大规模文本视觉问答数据集，包含约72K个问答对，基于约44K张图像。

Conclusion: 该流水线能够高效自动生成忠实的文本视觉问答数据对，随着场景文本数据的可用性而扩展，为文本视觉问答任务提供了可扩展的数据集构建解决方案。
...
TL;DR: 开发并临床评估了基于深度传感器的无标记AR配准系统，在头戴式显示器上实现手术导航，在小曲率解剖部位达到3-4mm的中位误差。


<details>
  <summary>Details</summary>
Motivation: 解决在小型或低曲率解剖结构上实现无标记AR配准的挑战，提高手术导航的临床适用性。

Method: 使用HoloLens 2的AHAT深度传感器，通过深度偏差校正、人工初始化引导、全局和局部配准，将深度数据与CT皮肤网格对齐。

Result: 临床验证显示：足部中位误差3.2mm，耳部4.3mm，小腿5.3mm；5mm覆盖率分别为92-95%、84-90%、72-86%。

Conclusion: 基于深度传感器的无标记AR配准系统在活体手术中达到接近临床误差阈值的效果，提高了无标记AR导航的临床准备度。
...
<details>
  <summary>Details</summary>
Motivation: 浮游有孔虫是重要的环境指标生物，了解其腔室生长轨迹对研究生物发育和生态适应至关重要，但现有方法依赖人工分割，耗时且主观。

Method: 集成实例分割技术和专门的腔室排序算法，定量评估多种针对不同腔室空间特征优化的实例分割方法，并分析它们对生长顺序重建精度的影响。

Result: 在专家标注的数据集上，该流程在保持生物学意义准确性的同时显著减少了人工工作量。腔室排序算法在部分分割情况下仍能稳健地重建发育轨迹。

Conclusion: 这是首个完全自动化和可重复的数字有孔虫生长分析流程，为大规模数据驱动的生态研究奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 路面裂缝宽度的精确量化对评估结构完整性和指导维护至关重要，但传统方法受限于裂缝边界的复杂非均匀形态和快速测量需求。

Method: 三阶段级联框架：1) 使用现有检测算法进行初始裂缝分割生成二值图；2) 通过PCA确定准平行裂缝的主方向轴；3) 使用RPCA提取不规则裂缝几何的主传播轴。

Result: 在三个公开数据集上的综合评估表明，该方法在计算效率和测量精度方面均优于现有最先进技术。

Conclusion: 该级联PCA-RPCA框架能够有效克服裂缝宽度测量的挑战，为路面状况评估提供了一种高效准确的解决方案。
...
TL;DR: 提出了一种基于卷积神经网络的自适应偏置调节机制，用于消除事件相机在25Hz-500Hz频率范围内的闪烁效应，无需额外硬件或软件滤波。


<details>
  <summary>Details</summary>
Motivation: 事件相机在光照强度快速变化的环境中容易产生闪烁效应，影响其性能表现，需要一种有效的闪烁抑制方法。

Method: 利用CNN在空间域识别闪烁实例，动态调整事件相机的偏置设置来最小化闪烁影响，通过人脸检测框架验证效果。

Result: 在明暗条件下均显著改善：YOLO置信度提升，检测到人脸的帧数比例增加，平均梯度（闪烁指标）在亮光下降低38.2%，弱光下降低53.6%。

Conclusion: 该方法能显著提升事件相机在不利光照条件下的功能性，具有广泛应用潜力。
...
TL;DR: 提出了一个用于解决Grounded Video Question Answering任务的三阶段框架，通过引入触发时刻概念来显著提升目标对象的时空定位和跟踪性能。


<details>
  <summary>Details</summary>
Motivation: GVQA任务需要强大的多模态模型来处理视频内容中的复杂推理，并将答案在视觉上定位，同时跟踪所引用对象的时间变化。

Method: 将GVQA任务分解为三阶段流水线：视频推理与问答、时空定位和跟踪，关键创新是引入基于CORTEX提示的触发时刻概念来识别目标对象最可见的帧作为定位和跟踪的锚点。
...
TL;DR: 提出MM-UNet用于视网膜血管分割，通过Morph Mamba卷积层增强分支拓扑感知，使用反向选择性状态指导模块提升几何边界感知和解码效率，在DRIVE和STARE数据集上取得显著性能提升。
...
Method: 提出MM-UNet架构，包含Morph Mamba卷积层（替代点卷积增强分支拓扑感知）和反向选择性状态指导模块（结合反向指导理论和状态空间建模）。

Result: 在DRIVE数据集上F1分数提升1.64%，在STARE数据集上提升1.25%，优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决淀粉样蛋白PET检查成本高、可及性有限的问题，探索从更易获取的血液生物标志物和MRI数据预测PET空间模式的可能性。

Method: 收集566名参与者的PET、MRI和血液生物标志物数据，开发语言增强生成模型，通过大语言模型和多模态信息融合合成PET图像，并在全自动诊断流程中评估。

Result: 合成PET图像在结构细节和区域模式上与真实PET高度相似，诊断准确率达80%。基于合成PET的诊断模型性能优于基于MRI和血液标志物的模型，组合使用进一步提升性能。

Conclusion: 语言增强生成模型能够合成逼真的PET图像，增强了MRI和血液生物标志物在淀粉样蛋白空间模式评估中的实用性，改善了阿尔茨海默病诊断流程。
...
TL;DR: 提出了一种基于SAM-2分割和3D高斯泼溅的草莓植株三维重建框架，通过背景去除实现更精确的植物表型分析。


<details>
  <summary>Details</summary>
Motivation: 草莓是美国重要的经济作物，传统植物表型分析方法耗时且具有破坏性。现有3D重建方法包含背景噪声，影响下游性状分析的准确性。

Method: 使用SAM-2模型和alpha通道背景掩码进行预处理，结合3D高斯泼溅技术实现背景无关的草莓植株重建，并利用DBSCAN聚类和PCA自动估算植物高度和冠层宽度。

Result: 该方法在准确性和效率上均优于传统流程，提供了可扩展的无损草莓植株表型分析解决方案。

Conclusion: 提出的对象中心化3D重建框架为植物表型分析提供了更精确、高效的解决方案，有望在农业领域推广应用。
...
TL;DR: 本研究开发了首个自动化管道autoStrain，使用深度学习进行经食管超声心动图(TEE)的左心室节段纵向应变(SLS)估计，比较了两种方法：基于RAFT光流模型的TeeFlow和基于CoTracker点轨迹模型的TeeTracker。


<details>
  <summary>Details</summary>
Motivation: 当前应变估计技术需要大量人工干预和专业知识，限制了效率，使其不适合监测目的。需要自动化方法来提高心脏功能评估的精确性和效率。

Method: 利用SIMUS模拟管道生成高度逼真的合成TEE数据集(80名患者)，包含真实心肌运动数据。比较两种深度学习方法：TeeFlow(密集帧间预测)和TeeTracker(稀疏长序列预测)。

Result: TeeTracker在准确性上优于TeeFlow，在合成TEE测试数据集上的运动估计平均距离误差为0.65mm。临床验证显示SLS估计与临床参考一致，平均差异为1.09%。模拟缺血数据提高了模型量化异常变形的准确性。
...
TL;DR: 本文研究了基础模型如何改变移动稀疏感知的格局，通过真实移动AR数据评估，发现基础模型在几何感知图像变形方面有显著改进，并展示了其在3D场景重建中的领先性能。


<details>
  <summary>Details</summary>
Motivation: 移动感知系统长期面临感知质量与效率之间的基本权衡，传统稀疏感知方法由于跨时空信息缺失往往导致精度下降。本文旨在探索基础模型是否能改变移动稀疏感知的现状。

Method: 使用真实移动AR数据进行评估，重点研究基础模型在几何感知图像变形方面的表现，这是实现跨帧信息准确重用的核心技术。

Result: 评估表明基础模型在几何感知图像变形方面提供显著改进，并展示了基于基础模型的稀疏感知的可扩展性，在3D场景重建中表现出领先性能。
...
TL;DR: 提出了一种用于阿尔茨海默病诊断的协作注意力和一致性引导融合框架，通过多模态MRI和PET神经影像融合来改善诊断性能。
...
<details>
  <summary>Details</summary>
Motivation: 从内窥镜相机获取手术场景中的绝对深度很困难，限制了在真实内窥镜图像上进行监督学习。现有的图像级无监督域适应方法在翻译合成图像后仍存在域差距。

Method: 使用潜在特征对齐方法，通过对抗学习和方向特征一致性学习域不变特征，深度网络同时处理翻译后的合成图像和真实内窥镜帧。

Result: 在中央气道内窥镜视频评估中，相比最先进的单目深度估计方法，该方法在绝对和相对深度指标上都取得了更优性能，并在各种骨干网络和预训练权重上一致改进结果。

Conclusion: 该方法能够有效减少域差距，提高内窥镜场景中的绝对深度估计精度，对医学机器人导航具有重要意义。
...
TL;DR: 提出了HTSC-CIF框架，通过分层任务分解方法解决医学报告生成中的三个主要挑战：领域知识不足、文本-视觉实体嵌入对齐差、跨模态偏差导致的伪相关。


<details>
  <summary>Details</summary>
Motivation: 医学报告生成(MRG)是医疗诊断的关键部分，但现有方法面临三个主要挑战：领域知识理解不足、文本-视觉实体嵌入对齐差、跨模态偏差导致的伪相关。先前工作只解决单个挑战，而本文旨在同时解决所有三个挑战。
...
TL;DR: 本文探讨了使用欧拉角参数化作为归一化流模型基础进行物体姿态估计的有效性，尽管欧拉角存在局限性，但在某些方面可能比复杂参数化更有用。
...
Result: 研究发现欧拉角参数化尽管有缺点，但在某些方面相比更复杂的参数化可能产生有用的模型。

Conclusion: 欧拉角参数化在姿态估计的归一化流模型中具有实用价值，特别是在处理姿态模糊性时，尽管存在参数化本身的局限性。
...
Method: 采用双奖励系统：思考奖励（通过事实基础、逻辑一致性和答案一致性评估推理质量）和判断奖励（自适应确定深度推理或直接回答是否合适），基于强化学习的后训练框架。
...
Conclusion: 基于图神经网络的方法能有效建模手写数学表达式的复杂结构，为HME识别提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有相机位姿估计方法通常依赖相机间距离信息或需要复杂的束调整，缺乏对循环一致性的充分利用和鲁棒性保证。

Method: 改进消息传递最小二乘法，强调循环一致性信息，重新定义循环一致性并使用Welsch型鲁棒损失函数，集成基于鲁棒子空间恢复的离群值剔除模块。

Result: 建立了最强的确定性精确恢复保证，仅凭循环一致性即可达到当前已知最低样本复杂度，在合成和真实数据集上持续优于领先的位姿估计器。

Conclusion: Cycle-Sync证明了循环一致性在相机位姿估计中的充分性，提供了一种无需束调整的全局方法，在精度和鲁棒性方面均优于现有技术。
...
<details>
  <summary>Details</summary>
Motivation: 现有后处理OOD检测方法忽视了特征与logits之间的内在相关性，这对有效的OOD检测至关重要。

Method: GAFD-CC首先进行全局感知特征解耦，提取正相关和负相关特征；然后自适应融合解耦特征与多尺度logit置信度。

Result: 在大规模基准测试中，GAFD-CC展现出竞争性性能和强大的泛化能力。

Conclusion: GAFD-CC通过有效利用特征-logits相关性，在OOD检测任务中取得了优于现有方法的性能。
...
TL;DR: 提出了M3PD数据集和F3Mamba模型，通过双视角智能手机摄像头同步采集面部和指尖视频，结合Mamba时序建模融合双视角信息，显著降低心率误差并提高在真实场景中的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前便携式生理监测方法存在设备限制和姿势要求问题，视频光电容积描记法虽方便但受运动伪影、光照变化和单视角限制影响可靠性，缺乏适用于心血管患者的可靠方法和开放数据集。

Method: 构建M3PD双视角移动光电容积描记数据集，包含60名参与者（47名心血管患者）的同步面部和指尖视频；提出F3Mamba模型，通过Mamba时序建模融合双视角信息。

Result: F3Mamba模型相比现有单视角基线方法，心率误差降低21.9%至30.2%，在具有挑战性的真实场景中提高了鲁棒性。

Conclusion: 双视角方法和Mamba时序建模能有效提高移动光电容积描记法的准确性和可靠性，为心血管疾病监测提供了更实用的解决方案。
...
TL;DR: CoCoVa是一个新的视觉语言模型框架，通过连续跨模态推理来解决传统VLMs在离散语言空间中推理的局限性，使用潜在思想向量链进行迭代推理。


<details>
  <summary>Details</summary>
Motivation: 人类认知中存在许多难以用语言表达的思维过程，而现有的视觉语言模型被限制在离散的语言标记空间中进行推理，这限制了视觉感知的丰富性和高维特性。

Method: CoCoVa采用迭代推理循环，使用新颖的Latent Q-Former作为动态推理引擎，通过跨模态融合迭代优化潜在思想向量链。包含动态标记选择机制和多任务训练目标（对比学习和基于扩散的重建）。

Result: CoCoVa在准确性和标记效率上优于强基线模型。使用1.5B骨干网络时，在几乎所有基准测试中与或超越更大的7B-9B模型。扩展到7B LLM骨干时仍与最先进模型保持竞争力。

Conclusion: 学习的潜在空间能够捕获可解释和结构化的推理模式，表明CoCoVa有潜力弥合离散语言处理与视觉理解的连续性之间的表征差距。
...
TL;DR: RxnCaption框架将化学反应图解析重新定义为图像描述问题，通过BIVP策略使用分子检测器预绘分子边界框和索引，显著提升结构提取质量并简化模型设计。
...
Method: 提出RxnCaption框架，将传统坐标预测驱动的解析过程重新定义为图像描述问题，引入BIVP策略使用MolYOLO分子检测器预绘分子边界框和索引。

Result: 构建了RxnCaption-11k数据集，比现有文献基准大一个数量级，RxnCaption-VL在多个指标上达到最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 雷达传感器能直接测量多普勒速度，适合单次扫描的运动目标分割，但雷达点云稀疏噪声大，数据标注成本高且耗时。

Method: 采用两步法：基于对比学习的自监督表示学习，使用新颖的聚类对比损失函数和动态点去除的聚类细化；然后用少量标注数据进行监督微调。

Result: 方法提高了标签效率，通过自监督预训练有效提升了最先进方法的性能。

Conclusion: 自监督预训练是解决雷达点云标注困难的有效方法，能显著提高运动目标分割的性能和标签效率。
...
TL;DR: 提出一种基于分组的混合颜色校正算法，用于彩色点云的颜色一致性校正。根据点云重叠率自适应分组，对不同距离组采用不同的颜色校正方法。
...
Method: 1. 估计源点云和目标点云的重叠率；2. 根据重叠率自适应分组（2组或3组）；3. 对近距离组使用K近邻双边插值(KBI)；4. 对中距离组使用联合KBI和直方图均衡化(JKHE)；5. 对远距离组使用直方图均衡化(HE)。

Result: 在1086个测试彩色点云对上验证了算法的有效性，优于现有最先进方法。

Conclusion: 该分组混合颜色校正算法能有效实现彩色点云的颜色一致性校正，具有分组效应自由特性，代码已开源。
...
TL;DR: 该研究通过统一的基准测试发现，自监督ViT模型（特别是DINO）在猫和人类视觉系统的表征对齐方面表现最佳，表明自监督学习能够产生跨物种视觉统计的早期特征。


<details>
  <summary>Details</summary>
Motivation: 猫和人类在眼部解剖结构上存在差异（如猫的垂直瞳孔），但尚不清楚这种差异如何影响下游视觉表征。研究旨在量化猫-人类跨物种视觉表征的对齐程度。

Method: 使用冻结编码器基准测试，比较卷积网络、监督ViT、窗口Transformer和自监督ViT（DINO），采用层间中心核对齐（线性和RBF）和表征相似性分析等方法。

Result: DINO ViT-B/16获得最高的表征对齐（平均CKA-RBF≈0.814，CKA-linear≈0.745，RSA≈0.698），峰值出现在早期块。监督ViT在CKA上表现良好但几何对应较弱。

Conclusion: 自监督与ViT归纳偏置相结合，产生的表征几何比广泛使用的CNN和窗口Transformer更接近猫和人类视觉系统的对齐，为跨物种视觉计算收敛提供了可测试的神经科学假设。
...
Method: 首先基于Retinex理论将图像分解为反射和光照分量；使用条件整流流框架建模光照变化的连续流场；通过流增强的数据增广训练去噪网络处理反射分量中的噪声。

Result: 在低光增强和曝光校正任务上，IllumFlow在定量和定性评估中都优于现有方法，能够精确适应不同光照条件并支持可定制的亮度增强。

Conclusion: IllumFlow通过结合整流流和Retinex理论，有效解决了低光图像增强中的光照建模和噪声去除问题，展现了卓越的性能。
...
TL;DR: 提出自动化多阶段代码驱动管道生成视觉推理数据集ChartM³，包含38K图表和142K问答对，显著提升模型在复杂图表理解中的推理能力和跨域泛化性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在复杂图表理解任务中覆盖场景有限，计算密集型推理任务在现实应用中普遍存在但研究不足。

Method: 开发自动化多阶段代码驱动管道，集成检索增强生成获取专业图表模板，使用思维链策略生成模拟真实数据分布的推理代码，驱动图表渲染和问题相关统计计算。

Result: 构建ChartM³数据集，包含训练集38K图表和142K问答对，以及2,871高质量评估样本。监督微调和强化学习实验表明数据集显著提升推理能力和跨域泛化性能。

Conclusion: 该数据集使较小模型在复杂图表理解任务中达到与更大规模模型相当的性能，为实际应用提供了实用的性能评估框架。
...
TL;DR: 提出使用Blender程序化生成合成作物-杂草图像的方法，用于解决农业除草机器人语义分割模型训练所需大量标注数据的问题，并在合成和真实数据集上评估了多种分割模型的跨域泛化能力。
...
Method: 使用Blender构建程序化生成合成作物-杂草图像的流程，生成包含不同植物生长阶段、杂草密度、光照条件和相机角度的标注数据集，并在合成和真实数据集上对多种最先进的分割模型进行基准测试，分析其跨域泛化能力。

Result: 在合成图像上训练导致10%的模拟到真实差距，超过了之前的最先进方法。此外，合成数据展现出良好的泛化特性，在跨域场景中优于真实数据集。
...
TL;DR: 本文研究了小型视觉语言模型在边缘设备上的场景理解和动作识别能力，特别关注移动机器人应用中的准确性与推理时间权衡。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型和视觉语言模型在视频理解、场景解释和常识推理方面取得显著进展，但计算复杂度限制了它们在边缘设备和移动机器人领域的应用，需要在准确性和推理时间之间找到平衡。

Method: 提出一个评估流程，在包含各种真实世界城市场景、校园和室内场景的多样化数据集上，测试最先进的小型视觉语言模型在场景解释和动作识别任务中的表现。
...
TL;DR: KAO框架通过核自适应优化和扩散模型实现卫星图像修复，在VHR数据集上达到新基准


<details>
  <summary>Details</summary>
Motivation: 解决高分辨率卫星图像修复中现有方法需要大量重训练或计算开销大的问题

Method: 采用潜在空间条件化方法优化紧凑潜在空间，并在扩散过程中引入显式传播实现前后向融合

Result: 在DeepGlobe和Massachusetts Roads数据集上实验证明KAO为VHR卫星图像修复设立了新基准

Conclusion: KAO提供了一种可扩展的高性能解决方案，平衡了预条件模型的效率和后条件模型的灵活性
...
TL;DR: 提出MVAFormer方法，用于多视角时空动作识别，通过基于transformer的协作模块和保留空间信息的特征图，在STAR设置下实现优于基线4.4个点的性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决现有多视角动作识别方法无法适用于时空动作识别(STAR)设置的问题，STAR需要按时间顺序识别每个人的动作，而现有方法仅适用于从整个视频识别单个动作。

Method: 提出MVAFormer方法，引入基于transformer的视角间协作模块，使用保留空间信息的特征图而非嵌入向量，并将自注意力分为相同视角和不同视角两部分来有效建模多视角关系。

Result: 在新收集的数据集上的实验结果表明，MVAFormer在F-measure指标上比比较基线高出约4.4个点。

Conclusion: MVAFormer成功解决了多视角动作识别在STAR设置下的适用性问题，通过保留空间信息的特征图和专门设计的transformer协作模块实现了显著性能提升。
...
TL;DR: OLATverse是一个大规模数据集，包含765个真实世界物体的约900万张图像，这些图像在精确控制的多样化光照条件下从多个视角拍摄，为逆渲染和重光照研究提供高质量的真实世界数据。
...
Method: 收集765个常见和不常见的真实世界物体，涵盖广泛的材料类别。每个物体使用35个DSLR相机和331个独立控制的光源进行拍摄，能够模拟多样化的光照条件，并提供校准的相机参数、物体掩码、光度法表面法线和漫反射反照率等辅助资源。

Result: 构建了大规模的真实世界物体数据集，建立了首个全面的真实世界基于对象的逆渲染和法线估计基准评估集。

Conclusion: OLATverse代表了将下一代逆渲染和重光照方法与真实世界数据整合的关键一步，该数据集及其后处理工作流程将公开发布。
...
<details>
  <summary>Details</summary>
Motivation: 解决GNSS拒止环境下无人机定位问题，传统卫星定位方法失效，需要视觉定位技术来处理跨视角、跨时相、异构图像的匹配挑战。

Method: 利用现代目标检测技术从无人机和卫星图像中提取显著实例，集成图神经网络推理图像间和图像内节点关系，使用细粒度图基节点相似度度量进行检索和定位。

Result: 在公开和真实数据集上的广泛实验表明，该方法能有效处理异构外观差异，泛化能力强，适用于更大模态差距场景如红外-可见光图像匹配。
...
TL;DR: 提出了DetectiumFire数据集，包含22.5k高清火灾图像和2.5k真实火灾视频，解决了火灾领域多模态模型应用的数据缺乏问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型在火灾领域应用受限，主要原因是缺乏高质量的公开火灾数据集和标注。

Method: 构建大规模多模态数据集DetectiumFire，包含图像和视频数据，并提供传统计算机视觉标注和详细文本提示。

Result: 数据集在规模、多样性和数据质量上优于现有基准，在目标检测、图像生成和视觉语言推理等任务中验证了其有效性。

Conclusion: DetectiumFire数据集将推动火灾相关研究，支持智能安全系统开发，并已公开发布供AI社区使用。
...
TL;DR: PtychoBench是一个用于评估LLM和VLM在高级显微镜工作流程中专业化的多模态基准。研究发现，视觉任务中监督微调（SFT）和上下文学习（ICL）互补性最强，而文本任务中ICL在大型基础模型上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 自动化高级显微镜工作流程是一个关键目标，基础模型如LLM和VLM显示出巨大潜力。但将这些通用模型适应专业科学任务至关重要，而最佳领域适应策略往往不明确。

Method: 引入PtychoBench多模态多任务基准，系统比较两种专业化策略：监督微调（SFT）和上下文学习（ICL）。在数据稀缺情况下评估视觉伪影检测和文本参数推荐任务。

Result: 视觉任务中，SFT和ICL高度互补，微调模型在上下文感知示例指导下达到最高性能（Micro-F1 0.728）。文本任务中，ICL在大型基础模型上表现最佳（Micro-F1 0.847），优于强大的"超级专家"SFT模型。

Conclusion: 最佳专业化路径取决于任务模态，为开发更有效的科学代理系统提供了清晰框架。上下文感知提示具有优越性，并在微调模型中观察到一致的上下文干扰现象。
...
TL;DR: 提出基于能量优化的视频镜头组装方法，通过视觉语义匹配和参考视频风格学习，实现自动化镜头序列编排，使无经验的用户也能创作视觉吸引力的视频。


<details>
  <summary>Details</summary>
Motivation: 解决当前智能视频编辑技术在镜头组装中无法捕捉创作者独特艺术表达的问题，实现既能自动化编排又能学习参考视频风格的视频编辑。

Method: 1) 使用大语言模型生成脚本并与视频库进行视觉语义匹配获取候选镜头；2) 分割标注参考视频镜头，提取镜头大小、相机运动等属性；3) 使用基于能量的模型学习这些属性，根据与参考风格的对齐度评分候选镜头序列；4) 结合多种语法规则优化镜头组装。

Result: 开发出能够自动安排独立镜头并按特定逻辑、叙事需求或艺术风格组合的系统，同时学习参考视频的组装风格，创建连贯的视觉序列或整体视觉表达。

Conclusion: 该方法不仅实现了镜头组装的自动化，还能学习并复现参考视频的艺术风格，使无视频编辑经验的用户也能创作出视觉上吸引人的视频作品。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习使认知系统能够与动态非结构化环境交互，但在关键任务应用中需要评估大量异构数据，自动报告生成可以促进这些系统的评估和接受。

Method: 开发了一个利用多模态传感器生成自然语言报告的管道，完全依赖可在边缘计算设备上部署的本地模型。

Result: 在包含室内、室外和城市环境的多样化数据集上评估了实现，提供了定量和定性评估结果。

Conclusion: 该方法能够生成自动报告，保护隐私且无需外部服务，为移动机器人在各种领域的应用提供了有效的评估工具。
...
TL;DR: LiteVoxel是一种自调节训练管道，通过逆Sobel重加权和深度分位数剪枝等创新方法，解决了稀疏体素光栅化在低频内容拟合、剪枝启发式不稳定和VRAM过度增长方面的问题。
...
Method: 采用逆Sobel重加权损失函数，结合训练中期的gamma斜坡调整；使用基于深度分位数的剪枝逻辑，通过EMA滞后保护和基于射线足迹的优先级驱动细分来优化结构。

Result: 在Mip-NeRF 360和Tanks & Temples数据集上的实验显示，LiteVoxel减少了低频区域和边界不稳定性错误，同时保持PSNR/SSIM、训练时间和FPS与SVRaster相当，并将峰值VRAM降低了40%-60%。

Conclusion: LiteVoxel实现了更可预测、内存效率更高的训练，同时保留了先前设置中缺失的低频细节，且不牺牲感知质量。
...
TL;DR: 本研究探索了无监督学习方法在剪切散斑图像自动异常检测中的应用，评估了三种架构：全连接自编码器、卷积自编码器和师生特征匹配模型，结果显示师生方法在分类鲁棒性和缺陷定位方面表现最优。


<details>
  <summary>Details</summary>
Motivation: 剪切散斑作为一种无损检测方法，在工业应用中受到专家解释需求的限制。本研究旨在减少对标记数据和人工评估的依赖，探索无监督学习实现剪切散斑图像自动异常检测。

Method: 开发了包含可重现缺陷模式的控制数据集，在理想和实际变形条件下系统采集剪切散斑测量数据。评估了三种无监督学习架构：全连接自编码器、卷积自编码器和师生特征匹配模型，所有模型仅使用无缺陷数据进行训练。

Result: 师生特征匹配方法在分类鲁棒性方面表现最优，能够实现精确的缺陷定位，与自编码器模型相比显示出更好的特征表示可分性。使用YOLOv8模型作为定位质量基准。
...
Method: 实现并评估了五种深度学习架构（UNet、U2-Net、UNETR、Time-Embedding UNet和ODE-UNet），在两个纵向队列（ADNI和AIBL）上进行训练和测试，直接比较预测的随访MRI与实际扫描结果。

Result: 最佳模型实现了高保真度预测，所有模型在独立外部数据集上泛化良好，表现出稳健的跨队列性能。
...
TL;DR: UVH-26是首个印度大规模交通摄像头图像数据集，包含26,646张高分辨率图像，标注了14个印度特有车辆类别，通过众包标注获得180万个边界框。训练的目标检测模型在印度交通场景下比COCO数据集训练的模型性能提升8.4-31.5%。


<details>
  <summary>Details</summary>
Motivation: 解决现有全球基准数据集中缺乏印度交通场景特定数据的问题，为新兴国家复杂交通条件下的智能交通系统提供基础数据支持。

Method: 从2800个班加罗尔安全城市CCTV摄像头采集图像，通过565名大学生参与的众包黑客松进行标注，使用多数投票和STAPLE算法生成共识标注，训练YOLO11、RT-DETR、DAMO-YOLO等当代检测器。

Result: RT-DETR-X模型在UVH-26数据集上达到0.67 mAP50:95，相比在COCO数据集上训练的相同模型（0.40 mAP50:95）性能显著提升31.5%。

Conclusion: UVH-26数据集填补了现有全球基准的空白，证明了领域特定训练数据对印度交通场景的重要性，为新兴国家复杂交通条件下的智能交通系统发展提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 解决跨视角域视频行人重识别中的极端视角偏移、尺度差异和时间不一致性问题，这些挑战在如空中-地面监控等场景中尤为突出。
...
Result: 在AG-VPReID基准测试的所有高度级别上达到最先进性能，在G2A-VReID和MARS数据集上表现出强大的跨数据集泛化能力，同时保持实时效率（189 FPS）。
...
TL;DR: VCFlow是一个无需特定受试者训练即可从fMRI重建视觉体验的脑解码框架，通过模拟人类视觉系统的腹侧-背侧结构来学习多维表征，在牺牲7%准确率的情况下实现快速重建。


<details>
  <summary>Details</summary>
Motivation: 开发无需特定受试者训练的脑解码方法，解决跨受试者泛化挑战，为临床应用提供可扩展的解决方案。

Method: 提出分层解码框架VCFlow，明确建模人类视觉系统的腹侧-背侧架构，使用特征级对比学习策略提取受试者不变语义表征。

Result: 相比传统方法需要12小时数据和大量计算，VCFlow仅牺牲7%准确率，能在10秒内生成重建视频且无需重新训练。
...
TL;DR: TAUE是一个零样本、分层图像生成的训练免费框架，通过噪声移植和培养技术实现语义和结构一致的图层生成，无需微调或辅助数据集。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像扩散模型只能生成单一平面图像的瓶颈，为需要图层控制的应用提供解决方案，避免现有方法需要大量数据集或只能生成孤立前景元素的问题。

Method: 提出噪声移植和培养(NTC)技术，从前景和合成生成过程中提取中间潜在表示，将其移植到后续图层的初始噪声中，确保各图层间的语义和结构一致性。

Result: 实验表明该训练免费方法性能可与微调方法相媲美，在保持高图像质量和保真度的同时增强了图层一致性。
...
TL;DR: UniChange是首个基于多模态大语言模型(MLLM)的统一变化检测模型，通过引入特殊令牌和文本提示统一了二元变化检测(BCD)和语义变化检测(SCD)任务，在多个基准测试中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前变化检测模型通常只能从单一类型的标注数据中学习，无法同时利用多样的BCD和SCD数据集，导致泛化能力差和功能有限。MLLM的发展为统一变化检测框架提供了新的可能性。

Method: 利用MLLM的语言先验和统一能力，引入三个特殊令牌[T1]、[T2]和[CHANGE]来统一BCD和SCD任务，使用文本提示指导变化类别识别，无需预定义的分类头，能够从多源数据集中有效学习。

Result: 在四个公共基准测试(WHU-CD、S2Looking、LEVIR-CD+和SECOND)上实现了最先进的性能，IoU得分分别为90.41、53.04、78.87和57.62，超越了所有先前的方法。

Conclusion: UniChange成功构建了首个基于MLLM的统一变化检测框架，通过语言引导的方法有效解决了多源数据学习问题，在多个任务中表现出卓越的性能。
...
TL;DR: 提出了一种轻量级CNN框架，用于检测打印/显示、视频和包裹攻击，确保生物识别认证的实时性（CPU上1-2秒）。


<details>
  <summary>Details</summary>
Motivation: 人脸识别系统容易受到呈现攻击的威胁，恶意行为者可以通过欺骗手段非法访问安全系统。

Method: 设计了一种轻量级CNN框架，能够识别打印/显示、视频和包裹攻击，并创建了包含60个受试者500多个视频的2D欺骗攻击数据集。

Result: 该架构提供了无缝的活体检测，确保更快的生物识别认证速度，并提供了演示视频验证有效性。

Conclusion: 提出的轻量级CNN框架能够有效检测多种呈现攻击，为生物识别系统提供了可靠的安全保障。
...
Method: 构建UniPruneBench基准测试，提供标准化协议，涵盖多种能力维度和数据集，评估代表性压缩算法在不同LMM模型上的表现，并纳入系统级指标。

Result: 发现随机剪枝是强基线方法；没有单一方法在所有场景中都表现最优；不同任务对剪枝的敏感度差异显著，OCR任务最脆弱；剪枝比例是影响性能退化的主要因素。

Conclusion: UniPruneBench为未来高效多模态建模研究提供了可靠基础，揭示了剪枝方法在不同场景下的表现规律和关键影响因素。
...
Method: 使用分层模型选择和信息准则来构建自适应分词器，支持像素级粒度，并保持与预训练模型的兼容性。

Result: 在图像级分类和密集预测任务中表现出竞争力，并支持开箱即用的栅格到矢量转换。

Conclusion: 该方法提供了一种更灵活、内容感知的图像tokenization方案，能够提升Vision Transformers的性能和适用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖中间表示来对齐跨模态特征，但存在生成中间图像或融合中间特征的问题，且未能充分利用中间特征。可见光和红外模态之间存在显著差异。

Method: 使用生成的中间图像作为可见光到红外模态的转换器，结合模态转换对比损失和模态查询正则化损失进行训练，无需额外参数。

Result: 在三个典型的VI-ReID数据集上显著且持续优于现有最先进方法。

Conclusion: 提出的MTRL框架能有效对齐跨模态特征，在保持骨干网络推理速度的同时提升VI-ReID性能。
...
TL;DR: 提出了一种情感线索引导的推理框架，通过两阶段调优的视频情感基础模型(VidEmo)来理解视频中的复杂情感状态，并构建了包含210万样本的情感数据集Emo-CFG。


<details>
  <summary>Details</summary>
Motivation: 视频情感分析面临挑战，因为情感具有动态性和线索依赖性，难以理解复杂演化的情感状态及其合理依据。

Method: 采用阶段式统一框架，结合基础属性感知、表情分析和高级情感理解。核心是VidEmo模型，通过课程情感学习和情感树强化学习进行两阶段调优。

Result: 实验结果显示该方法在15个面部感知任务上取得了有竞争力的性能，设立了新的里程碑。
...
<details>
  <summary>Details</summary>
Motivation: 当前的CRP工作流程主要是手动的：专家需要检查激活图像来命名发现的概念，并从相关性图中合成冗长的解释，这限制了解释的可访问性和可扩展性。

Method: 引入LLEXICORP，将CRP与多模态大语言模型耦合，通过精心设计的提示教导语言模型CRP的语义，并强制分离命名和解释任务，生成可针对不同受众定制的文本解释。

Result: 在ImageNet数据集上对VGG16模型进行定性评估，结果表明该方法能够有效降低深度神经网络解释的门槛。
...
TL;DR: 该论文首次全面研究了视频-文本表示对齐，揭示了跨模态对齐高度依赖于测试时视觉和文本数据的丰富程度，提出了参数化测试时缩放定律，并探讨了语义对齐与下游任务性能的关联。


<details>
  <summary>Details</summary>
Motivation: 虽然图像与文本的对齐已取得显著进展，但视频数据的时序特性在此背景下仍未被充分探索。本研究旨在填补这一空白，系统性研究视频-文本表示对齐。

Method: 提出参数化测试时缩放定律来捕捉视觉和文本数据丰富度对跨模态对齐的影响，并研究语义对齐与下游任务性能的关联性。

Result: 发现跨模态对齐高度依赖于测试数据的丰富程度；强对齐可能与通用视频表示和理解相关；时序推理与跨模态对齐存在关联。

Conclusion: 视频-文本对齐可作为零样本探测不同编码器对时空数据表示能力的信息性方法。
...
TL;DR: PercHead是一种从单张图像进行3D头部重建和语义编辑的方法，通过双分支编码器和ViT解码器实现视图一致的3D重建，并利用DINOv2和SAM2.1进行感知监督。该方法在视角合成方面达到SOTA，并能通过轻量级GUI实现直观的3D编辑。


<details>
  <summary>Details</summary>
Motivation: 解决单图像3D头部重建面临的严重视角遮挡、弱感知监督以及3D空间编辑模糊性等挑战。

Method: 使用双分支编码器和基于ViT的解码器，通过迭代交叉注意力将2D特征提升到3D空间，采用高斯泼溅进行渲染，并基于DINOv2和SAM2.1进行感知监督。

Result: 在视角合成方面达到最先进性能，对极端视角具有卓越鲁棒性，并能通过轻量级GUI实现直观的几何雕刻和外观风格化编辑。

Conclusion: PercHead为单图像3D头部重建和语义编辑提供了一个统一且强大的解决方案，在重建质量和编辑能力方面均表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI进展主要关注语言中心任务（如程序合成和调试），而视觉中心编码任务探索不足。受人类通过草图推理的启发，研究者提倡使用SVG代码作为视觉表示。

Method: 提出VCode基准，将多模态理解重构为代码生成任务；设计CodeVQA评估协议，通过策略模型在渲染的SVG上回答问题来评估符号保真度；开发VCoder代理框架，包含"带修订的思考"（迭代分析差异并精炼SVG代码）和"带视觉工具的行动"（使用检测器和解析器提供结构化线索）。

Result: 前沿视觉语言模型在生成忠实SVG方面表现不佳，揭示了语言中心与视觉中心编码之间的差距。VCoder在基准测试中比表现最佳的Claude-4-Opus提升了12.3个百分点。人类研究显示人类和视觉语言模型在渲染SVG上都表现更差，但一致性揭示了符号视觉表示的前景。

Conclusion: SVG代码作为视觉表示具有潜力，但当前模型在视觉中心编码任务上仍有局限，特别是在专业知识和3D推理方面。VCoder框架有效提升了视觉编码性能，为视觉推理提供了新的方向。
...
TL;DR: MIRA是一个新的基准测试，用于评估模型在需要生成中间视觉图像来辅助推理的场景中的表现。该基准包含546个多模态问题，要求模型生成草图、结构图等中间图像来指导推理过程。


<details>
  <summary>Details</summary>
Motivation: 传统思维链方法仅依赖文本，而许多复杂问题需要视觉化思考。MIRA旨在评估模型在需要"画图思考"的场景中的能力，这更接近人类解决复杂问题的方式。

Method: MIRA包含546个标注了中间视觉图像和最终答案的多模态问题。提出了统一的评估协议，包括三个层次的评估输入：仅图像和问题的直接输入、带思维提示的文本思维链输入、以及带注释图像线索和文本思维提示的视觉思维链输入。

Result: 实验结果显示，现有多模态大语言模型在仅依赖文本提示时表现不佳。但当提供中间视觉线索时，模型性能一致提升，在所有模型和任务中平均相对增益达33.7%。扩大搜索空间和设计对齐视觉思维链的文本提示仅带来有限改进。

Conclusion: 结果强调了想象视觉信息在MIRA基准上实现成功推理的关键作用，表明视觉化思考对于复杂推理任务至关重要。
...
Method: 引入统一的基准测试框架，对十种最先进的取证方法（从头训练、冻结预训练、微调）和七个公开数据集（GAN和扩散模型生成）进行系统评估，使用多种指标（准确率、平均精度、ROC-AUC、错误率、类别敏感度）和可解释性分析（置信度曲线、Grad-CAM热力图）。
...
TL;DR: PLUTO-4是下一代病理学基础模型，包含紧凑型PLUTO-4S和前沿规模PLUTO-4G两个版本，在55万+WSI数据上预训练，在多个病理任务中实现SOTA性能。
...
Method: 使用DINOv2自监督目标在55万+WSI上预训练两个ViT架构：PLUTO-4S采用FlexiViT多尺度部署，PLUTO-4G使用单一补丁尺寸最大化表示能力。

Result: 在公共和内部基准测试中实现SOTA，PLUTO-4S提供高吞吐量部署，PLUTO-4G在皮肤病理诊断中提升11%，在多个基准中建立新性能前沿。
...
TL;DR: DenseMarks是一种新的人类头部学习表示方法，通过Vision Transformer网络为每个像素预测3D嵌入，建立2D图像到3D规范空间的映射，实现高质量密集对应。


<details>
  <summary>Details</summary>
Motivation: 为了解决人类头部图像的高质量密集对应问题，特别是在不同姿态和个体间保持一致性，需要一个能够覆盖整个头部（包括头发）的鲁棒表示方法。

Method: 使用Vision Transformer网络预测像素级3D嵌入，通过对比损失训练，结合多任务学习（面部关键点和分割约束），并利用潜在立方体特征确保空间连续性。

Result: 在几何感知点匹配和单目头部跟踪方面达到最先进水平，对姿态变化具有鲁棒性，能够覆盖整个头部区域。

Conclusion: DenseMarks提供了一种可解释、可查询的规范空间表示，能够有效应用于语义部分识别、面部/头部跟踪和立体重建等任务。
...
TL;DR: CudaForge是一个无需训练的多代理工作流，用于自动生成和优化CUDA内核，通过迭代的代码生成、测试和硬件反馈分析，显著提升了内核性能和正确率。


<details>
  <summary>Details</summary>
Motivation: 手动设计CUDA内核成本高且耗时，现有自动方法生成的内核效率低、计算开销大且泛化性差，需要更高效的自动化解决方案。

Method: 采用双代理工作流：Coder负责生成内核代码，Judge负责测试正确性和分析硬件反馈（如Nsight Compute指标），通过迭代改进优化内核性能。

Result: 在KernelBench上达到97.6%的正确率，平均比PyTorch基线快1.68倍，显著超越OpenAI-o3和Kevin等最先进模型，且在不同GPU和基础模型上表现出强泛化性。

Conclusion: 多代理、无需训练的工作流能够实现成本效益高、泛化性强且高性能的CUDA内核优化，为AI应用提供高效的自动内核生成方案。
...
### [67] [Deciphering Personalization: Towards Fine-Grained Explainability in Natural Language for Personalized Image Generation Models](https://arxiv.org/abs/2511.01932)
*Haoming Wang,Wei Gao*

Main category: cs.LG

TL;DR: FineXL是一种为个性化图像生成模型提供细粒度自然语言解释的新技术，能够描述个性化的不同方面并量化每个方面的个性化程度。


<details>
  <summary>Details</summary>
Motivation: 现有个性化模型缺乏可解释性，特别是在自然语言解释方面只能提供粗粒度描述，无法精确识别个性化的多个方面及其不同程度。

Method: 提出了FineXL技术，通过自然语言描述个性化模型的各个不同方面，并为每个方面提供量化评分来指示个性化程度。

Result: 实验结果显示，FineXL在不同个性化场景应用于多种图像生成模型时，可将解释准确性提高56%。

Conclusion: FineXL能够有效解决个性化图像生成模型缺乏细粒度自然语言解释的问题，显著提升了模型的可解释性。
...
TL;DR: 提出了一种基于检索增强生成(RAG)的多模态抑郁症检测框架，通过从情感数据集中检索相关内容并生成情感提示来增强情感表示，在AVEC 2019数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态抑郁症检测方法虽然利用情感分析来增强情感理解，但面临计算成本高、领域不匹配和静态知识限制等问题。

Method: 使用RAG框架，给定抑郁症相关文本时，从情感数据集中检索语义相关的情感内容，并利用大语言模型生成情感提示作为辅助模态。

Result: 在AVEC 2019数据集上取得了0.593的CCC和3.95的MAE，超越了之前的迁移学习和多任务学习基线方法。

Conclusion: 提出的RAG框架能够有效增强情感表示并提高可解释性，在多模态抑郁症检测任务中实现了最先进的性能。
...
### [69] [Human-Machine Ritual: Synergic Performance through Real-Time Motion Recognition](https://arxiv.org/abs/2511.02351)
...
<details>
  <summary>Details</summary>
Motivation: 通过将舞者特定动作映射到声音，利用体感记忆和关联，提出一种替代性的人机协作方法，在保持表演身体表达深度的同时，利用机器学习进行专注观察和响应。

Method: 使用可穿戴IMU传感器收集数据，采用MiniRocket时间序列分类算法进行运动识别，结合响应式多媒体控制系统。

Result: 该系统可靠地支持高精度分类（延迟<50毫秒），提供了一个可复制的框架，将懂舞蹈的机器集成到创意、教育和现场表演环境中。

Conclusion: 该人本设计方法为将舞蹈智能机器融入创作、教育和表演环境提供了可行方案，实现了低延迟高精度的人机协同表演。
...
### [70] [The Eigenvalues Entropy as a Classifier Evaluation Measure](https://arxiv.org/abs/2511.01904)
*Doulaye Dembélé*

Main category: cs.LG

TL;DR: 本文提出使用特征值熵作为评估分类器性能的新方法，特别适用于类别不平衡的数据集，并与传统评估指标建立了联系。
...
Method: 使用特征值熵作为评估指标，建立了特征值与常用评估指标（灵敏度、特异性、AUC、Gini指数）之间的关系，并提出了处理类别不平衡问题的混淆矩阵估计方法。

Result: 通过多个数据示例验证，所提出的评估方法在性能上优于文献中的黄金标准评估指标。

Conclusion: 特征值熵是一种有效的分类器评估方法，特别适用于类别不平衡问题，能够提供更准确的性能评估。
...
### [71] [Variational Geometry-aware Neural Network based Method for Solving High-dimensional Diffeomorphic Mapping Problems](https://arxiv.org/abs/2511.01911)
*Zhiwen Li,Cheuk Hin Ho,Lok Ming Lui*

Main category: cs.LG

TL;DR: 提出了一种用于高维微分同胚映射的无网格学习框架，结合变分原理和拟共形理论，通过控制共形失真和体积失真来确保准确的双射映射。


<details>
  <summary>Details</summary>
Motivation: 传统高维微分同胚映射方法受维度诅咒困扰，需要开发能够处理高维映射问题的新方法。

Method: 结合变分原理和拟共形理论的无网格学习框架，通过调节共形失真和体积失真来控制变形质量，与基于梯度的优化和神经网络架构兼容。
...
### [72] [Superpositional Gradient Descent: Harnessing Quantum Principles for Model Training](https://arxiv.org/abs/2511.01918)
*Ahmet Erdem Pamuk,Emir Kaan Özdemir,Şuayp Talha Kocabay*

Main category: cs.LG

TL;DR: 提出了叠加梯度下降(SGD)优化器，通过注入量子电路扰动将梯度更新与量子叠加联系起来，在合成序列分类和LLM微调中比AdamW收敛更快且损失更低。


<details>
  <summary>Details</summary>
Motivation: 探索量子启发方法如何增强经典训练机制，研究量子计算与深度学习的交叉点。

Method: 引入叠加梯度下降(SGD)优化器，建立数学框架，在PyTorch和Qiskit中实现混合量子经典电路，注入量子电路扰动。
...
### [73] [Neural Green's Functions](https://arxiv.org/abs/2511.01924)
*Seungwoo Yoo,Kyeongmin Yeo,Jisung Hwang,Minhyuk Sung*

Main category: cs.LG

TL;DR: 提出Neural Green's Function，一种用于线性偏微分方程的神经解算子，通过模仿格林函数的行为，在多样化不规则几何和源/边界函数上实现优越的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的解算子在泛化到未见过的源或边界函数时存在困难，需要设计对训练中具体函数不敏感的框架以实现鲁棒泛化。

Method: 从表示问题域的体素点云中提取每点特征，预测解算子的分解，随后通过数值积分评估解。

Result: 在MCB数据集的机械零件几何稳态热分析中，Neural Green's Function优于最先进的神经算子，平均误差降低13.9%，比需要计算昂贵网格的数值求解器快350倍。

Conclusion: Neural Green's Function通过模仿格林函数的行为，实现了对线性PDE的高效求解和优越的跨几何泛化能力。
...
### [74] [DeepContour: A Hybrid Deep Learning Framework for Accelerating Generalized Eigenvalue Problem Solving via Efficient Contour Design](https://arxiv.org/abs/2511.01927)
*Yeqiu Chen,Ziyan Liu,Hong Wang*

Main category: cs.LG

TL;DR: DeepContour提出了一种结合深度学习谱预测器和核密度估计的混合框架，用于自动优化轮廓积分方法中的积分轮廓选择，从而加速大规模广义特征值问题的求解。


<details>
  <summary>Details</summary>
Motivation: 传统轮廓积分方法在求解大规模广义特征值问题时，积分轮廓的选择严重依赖于对特征值分布的先验知识，不当选择会导致计算开销大和数值精度差的问题。

Method: 使用傅里叶神经算子快速预测GEP的谱分布，然后应用核密度估计自动系统地确定合适的积分轮廓，最后用优化后的轮廓指导轮廓积分求解器高效找到所需特征值。

Result: 在多个数据集上的实验表明，DeepContour能够加速GEP求解，最高达到5.63倍的加速比。

Conclusion: 通过将深度学习的预测能力与经典求解器的数值严谨性相结合，这项工作开创了一个高效且鲁棒的范式来解决涉及高维矩阵的困难广义特征值问题。
...
### [75] [Dynamic Population Distribution Aware Human Trajectory Generation with Diffusion Model](https://arxiv.org/abs/2511.01929)
*Qingyue Long,Can Rong,Tong Li,Yong Li*

Main category: cs.LG

TL;DR: 提出了一种基于扩散模型的新轨迹生成框架，整合动态人口分布约束来指导高保真轨迹生成，通过空间图增强轨迹空间相关性，在去噪过程中捕捉人口分布影响，实验显示生成轨迹在关键统计指标上优于现有算法54%以上。


<details>
  <summary>Details</summary>
Motivation: 真实轨迹数据面临隐私、获取成本和质量等挑战，现有轨迹生成方法主要关注个体移动模式但忽略了人口分布对轨迹生成的影响，而动态人口分布显著影响个体移动行为。

Method: 构建空间图增强轨迹空间相关性，设计动态人口分布感知的去噪网络，在去噪过程中捕捉人类移动行为的时空依赖性和人口分布影响。

Result: 实验表明生成轨迹在关键统计指标上能够模拟真实轨迹，性能优于最先进算法54%以上。

Conclusion: 提出的基于扩散模型的轨迹生成框架能够有效整合动态人口分布约束，生成高质量的轨迹数据，解决了现有方法忽略人口分布影响的问题。
...
Method: 采用动态泛化引导的基于规则的强化学习奖励设计，逐步从探索性转向利用性工具使用模式，直接从基础模型进行强化学习训练。

Result: 在相同实验设置下，相比SFT和RL-with-SFT模型性能提升超过7%，在跨数据集和数据集内评估中均表现出一致的性能增益。

Conclusion: 纯强化学习方法能够有效激发语言模型的内在推理能力，显著提升工具无关的泛化性能，验证了方法的有效性和鲁棒性。
...
TL;DR: 本研究提出了一种基于机器学习的新模型，用于客观确定定性研究中的样本量，替代传统依赖数据饱和度的主观方法。


<details>
  <summary>Details</summary>
Motivation: 传统定性研究样本量确定依赖主观的数据饱和度原则，导致不一致性和方法学严谨性威胁，需要更客观的系统化方法。

Method: 使用五种基本定性研究方法的数据集，开发集成学习模型，评估十个关键参数作为输入特征，比较多种机器学习算法。

Result: KNN、梯度提升、随机森林、XGBoost和决策树算法表现最佳（测试R2约0.85），特征重要性分析确认研究设计类型和信息力量的关键作用。

Conclusion: 提出了基于网络的计算应用概念框架，作为决策支持系统，代表标准化样本量论证、增强透明度和强化定性研究认识论基础的重要进展。
...
TL;DR: 通过在RLVR训练中保留并适度加权中等难度问题，实现了隐式长度正则化，使模型在不增加输出长度的情况下解决更难的问题，达到基线准确率的同时输出长度减少近一半。


<details>
  <summary>Details</summary>
Motivation: 传统RLVR训练过滤掉简单问题，导致模型主要在需要长推理链的难题上训练，使模型误以为"思考更久"等于"思考更好"，产生过度冗长的输出。

Method: 在RLVR训练中保留中等难度问题并适度加权，作为隐式长度正则化器，让模型接触可解决的短链任务来约束输出分布。

Result: 在Qwen3-4B-Thinking-2507上的实验显示，在保持基线AIME25准确率的同时，平均输出长度减少了近一半。

Conclusion: 通过保留中等难度问题实现隐式长度正则化，可以免费获得简洁性，模型学会在不增加输出长度的情况下解决更难问题。
...
TL;DR: 提出一种基于疾病进展评分和多种描述变量的聚类方法，用于ALS患者个性化治疗分组，在生存分析上优于现有方法且结果更具可解释性


<details>
  <summary>Details</summary>
Motivation: ALS患者生存期短且治疗反应差异大，需要个性化护理，但现有研究受限于小样本、异质性数据和缺乏明确临床分组定义

Method: 使用疾病进展评分对序列进行聚类，整合医学专业知识的多变量描述，研究多种距离度量方法（包括现成距离和弱监督学习），并与聚类方法配对

Result: 在353名ALS患者数据集上测试，该方法在生存分析上优于现有技术，同时达到相当的轮廓系数，学习到的距离增强了结果的相关性和可解释性

Conclusion: 提出的聚类方法能有效识别ALS患者亚组，为个性化治疗提供支持，且结果对医学专家更具可解释性
...
Result: COFAP在hypoCOFs数据集上超越了先前方法，建立了新的SOTA；发现高性能COFs集中在狭窄的孔径和表面积范围内
...
TL;DR: 开发了一个结合树模型和CNN的加权集成模型来预测心血管疾病风险，在229,781名患者数据集上取得了0.8371的AUC和80%的召回率，并通过SHAP和决策树提供可解释性。
...
Method: 使用Heart Disease Health Indicators Dataset，开发了结合LightGBM、XGBoost和CNN的加权集成模型，通过策略性加权处理类别不平衡，并将22个特征扩展到25个。
...
TL;DR: 提出了Output-Conditioned Gating机制，使模型能够基于自身过去推断进行自我反思，创建稳定反馈循环增强记忆保持。最终模型EchoLSTM在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 标准RNN和LSTM在建模长距离依赖关系方面存在困难，特别是在包含噪声或误导信息的序列中。

Method: 提出了Output-Conditioned Gating架构原则，结合注意力机制构建EchoLSTM模型，通过基于过去推断调节内部记忆门来实现自我反思。

Result: 在Distractor Signal Task上达到69.0%准确率，比标准LSTM基线提升33个百分点；在ListOps基准上达到69.8%准确率，与Transformer模型(71.8%)性能相当，但参数效率高出5倍以上。

Conclusion: EchoLSTM的自反思机制能够创建更稳健的记忆系统，在保持高性能的同时显著提升参数效率。
...
TL;DR: NeuroClean是一个无监督的EEG/LFP预处理管道，通过五步流程自动去除脑电信号中的伪影和噪声，在运动任务分类中准确率从74%提升至97%以上。


<details>
  <summary>Details</summary>
Motivation: 脑电图和局部场电位记录中存在大量伪影和噪声，需要自动化预处理方法来解决人工干预带来的可重复性问题和偏差。

Method: 五步预处理流程：包括带通滤波、线路噪声滤波、坏通道剔除，以及基于聚类算法的自动成分拒绝的高效独立成分分析。

Result: NeuroClean有效去除了多种常见伪影，在运动任务分类中，经过清理的数据在多项式逻辑回归模型中达到97%以上的准确率，而原始数据仅为74%。

Conclusion: NeuroClean是一个有前景的预处理工作流，能够提高机器学习管道的泛化能力和性能。
...
<details>
  <summary>Details</summary>
Motivation: 为了深入理解深度神经网络的训练动态，揭示网络架构和训练样本在训练过程中的不同作用。

Method: 从随机梯度下降公式出发，将拉格朗日量重新组织为体项和边界项，并基于此分解发展了神经动力学的场论表述。

Result: 该分解揭示了深度网络底层的局部和齐次结构，体项捕捉由网络架构和激活函数设定的内在动态，边界项反映输入和输出层训练样本的随机相互作用。

Conclusion: 体-边界分解为理解深度神经网络训练动态提供了新的理论框架，并自然扩展到场论表述。
...
TL;DR: TapOut是一种基于多臂老虎机的动态推测解码算法，无需训练即可智能决定要推测的token数量，避免手动调参问题


<details>
  <summary>Details</summary>
Motivation: 现有动态推测解码方法依赖手动调整的敏感阈值（如token熵），这些阈值成本高且在不同模型和领域间泛化能力差

Method: 使用多臂老虎机在线选择无参数的动态推测策略，基于历史奖励和探索来选择最佳推测策略

Result: 在不同模型对和数据集上的广泛实验表明，TapOut无需超参数调优即可达到或超越现有动态推测基线的加速效果

Conclusion: TapOut提供了一种高效、无需调参的动态推测解码解决方案，显著提升了LLM推理速度
...
TL;DR: 研究发现大型语言模型在针对特定有害数据集微调后会出现跨领域的广泛有害行为（涌现错位），这种现象在参数空间中表现出跨任务的线性结构特征。


<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型在微调后产生跨领域有害行为的根本机制，这些机制目前仍不清楚。

Method: 采用几何视角分析涌现错位现象，通过计算微调参数更新的余弦相似度、主角度和投影重叠来测量参数空间的收敛性，并验证线性模式连接性。

Result: 发现不同窄域错位任务会收敛到相同的参数方向，微调权重更新具有较高的余弦相似度和共享的低维子空间，跨任务插值模型保持一致的广泛错位行为。

Conclusion: 涌现错位源于不同窄域任务发现相同的共享参数方向，有害行为可能组织在权重景观的特定可预测区域中，这揭示了参数几何与行为结果之间的基本联系。
...
TL;DR: 提出了一种基于神经正切核理论的新路径协调持续学习框架，通过统计验证和路径质量评估，在Split-CIFAR10上实现了66.7%的平均准确率，仅23.4%的灾难性遗忘，接近最先进水平。


<details>
  <summary>Details</summary>
Motivation: 解决持续学习中灾难性遗忘的根本问题，即神经网络在训练新任务时会忘记先前学习的任务。

Method: 结合神经正切核理论的可塑性边界、Wilson置信区间的统计验证以及多指标路径质量评估的新路径协调框架。

Result: 在Split-CIFAR10上平均准确率66.7%，灾难性遗忘率23.4%，接近最先进水平；发现NTK条件数>10^11是学习能力的关键阈值；任务序列中遗忘率从27%降至18%；80%的路径得到统计保证，中间任务保持90-97%的保留率。

Conclusion: 确定了持续学习环境的核心能力限制，并为增强自适应正则化提供了可行的见解；NTK条件数是学习能力限制的预测指标；系统随着任务序列进展趋于稳定。
...
TL;DR: 提出RobustFSM，一种针对联邦子模最大化问题的鲁棒解决方案，能够抵御各种客户端攻击，在严重攻击情况下性能比传统联邦算法提升高达200%。


<details>
  <summary>Details</summary>
Motivation: 联邦子模最大化中，分散的客户端拥有本地数据并定义可表示性质量，但容易受到恶意客户端分享虚假信息的攻击，类似于联邦学习中的后门攻击，但具有子模最大化的独特特性。

Method: 开发RobustFSM方法，通过重复聚合客户端计算的本地信息，同时设计机制来抵御各种实际客户端攻击。

Result: 使用真实世界数据集进行实证评估，数值结果显示在攻击严重时，RobustFSM的解决方案质量显著超过传统联邦算法，改进程度取决于数据集和攻击场景，最高可达200%。

Conclusion: RobustFSM是联邦子模最大化中对抗客户端攻击的有效鲁棒解决方案，在严重攻击情况下性能提升显著。
...
Method: 构建配对微生物相互作用的边图，利用图神经网络作为分类器，预测相互作用的效应方向和复杂类型（互利共生、竞争、寄生等）。

Result: 在包含20个物种、40种碳条件下7500多个相互作用的数集上，模型达到80.44%的F1分数，显著优于XGBoost的72.76%。

Conclusion: 图神经网络是预测微生物种间相互作用的有效工具，能够准确预测二元相互作用和更复杂的相互作用类型。
...
TL;DR: 提出了量子增强生成模型(QEGM)，一种混合经典-量子框架，用于解决罕见事件建模问题，在金融、气候和生物数据上相比传统方法显著提升了尾部事件捕捉能力。


<details>
  <summary>Details</summary>
Motivation: 罕见事件（如金融危机、极端气候、生物异常）由于稀缺性和重尾分布特性，传统深度生成模型难以准确建模，容易出现模式坍塌或不确定性估计不准确的问题。

Method: QEGM结合深度潜变量模型和变分量子电路，采用混合损失函数（重建保真度+尾部感知似然）和量子随机性驱动的噪声注入，通过混合训练循环优化参数。

Result: 在合成高斯混合和真实数据集上，QEGM相比GAN、VAE、Diffusion等基线方法，尾部KL散度降低达50%，显著提升了罕见事件召回率和覆盖校准。

Conclusion: QEGM为罕见事件预测提供了一种原则性方法，展现出超越纯经典方法的鲁棒性，证明了量子增强在生成建模中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有注意力优化方法如FlashAttention和FlexAttention存在局限性：前者需要专门内核，后者依赖静态模板且无法处理数据依赖的注意力模式。需要更灵活高效的解决方案。

Method: 利用PyTorch的编译工作流程，透明地融合和分块注意力计算，支持任意注意力模式，包括数据依赖的注意力公式。
...
<details>
  <summary>Details</summary>
Motivation: 研究在LLM分类微调中，为每个标签附加简短解释是否能产生更好的模型，以及这种改进的机制是什么。

Method: 使用多个LLM生成的数据集，对7B参数模型进行微调，比较标签加解释训练与仅标签训练的效果，并测试用语法不连贯但词汇对齐的随机token序列替代真实解释的效果。

Result: 在18个数据集和任务设置中，标签加解释训练均优于仅标签基线。意外发现是，即使使用随机token序列作为伪解释，也能显著提升准确率，缩小与真实解释的差距。

Conclusion: 解释增强的微调（无论是真实解释还是构造的随机token序列）能提高LLM分类的准确性和可靠性，改进主要源于token级脚手架对推理过程中计算结构的塑造作用。
...
TL;DR: 提出多流注意力深度学习框架，兼具传感器优化和数据审计功能，在步态分析中发现数据集存在严重偏倚问题


<details>
  <summary>Details</summary>
Motivation: 可穿戴传感器和AI在步态分析中存在数据集隐藏偏倚和传感器优化挑战，需要能自动审计数据完整性的方法

Method: 多流注意力深度学习框架，应用于Voisard等人(2025)的多队列步态数据集，分析四个临床任务

Result: 模型注意力机制发现数据集存在严重偏倚：在OA和CVA筛查任务中，70%以上注意力分配给右脚，左脚仅0.1%，反映了数据集中的侧向性偏倚

Conclusion: 该框架能自动审计数据集完整性，并提出数据驱动的传感器协同组合作为未来优化协议的假设
...
TL;DR: 开发了一个求解器，用于最大化二元变量的实值函数，通过估计目标函数和子实例的最优值来训练估计器，而不需要计算策略值或依赖已解决的实例。


<details>
  <summary>Details</summary>
Motivation: 开发一种不依赖于计算策略值或已解决实例的求解器，以更高效地处理二元变量实值函数的最大化问题。

Method: 使用一种算法估计目标函数和子实例的最优值，基于一个不等式，利用期望总偏差作为损失函数进行训练。

Result: 提出了一种新的求解器，能够在不计算策略值或不依赖已解决实例的情况下，有效地估计最优目标函数值。

Conclusion: 该方法提供了一种有效的替代方案，用于处理二元变量实值函数的最大化问题，避免了传统方法中的计算瓶颈。
...
TL;DR: 提出一种一次性动态阈值方法（OSDT），通过单序列校准阈值并应用于后续输入，显著加速掩码扩散语言模型的解码过程，在多个基准测试中实现更好的准确率-吞吐量权衡。


<details>
  <summary>Details</summary>
Motivation: 观察到现有并行解码方法使用静态全局置信度阈值存在块级和步级置信度波动问题，以及数据集中输入间存在近乎相同的置信度轨迹，这为可重用的任务级置信度特征提供了机会。

Method: 一次性动态阈值（OSDT）方法：在单个序列上校准阈值，然后将这些阈值以可忽略的开销应用于后续输入，利用置信度轨迹的可重用性。

Result: 在GPQA、GSM8K和HumanEval基准测试中，OSDT实现了优越的准确率-吞吐量权衡：在GSM8K上以最佳准确率实现+24% tokens/s，在GPQA上以可比较准确率实现+45%，在HumanEval上以适度准确率差距实现+50%。

Conclusion: 研究结果表明，利用可重用的任务级置信度特征为扩散解码提供了更通用的算法和系统创新机会。
...
TL;DR: 提出了一种将物理信息直接融入损失函数的框架，通过能量损失函数在分子和自旋系统等任务中实现更好的预测和生成建模
...
Method: 基于热平衡假设推导能量损失函数，使用反向KL散度结合玻尔兹曼分布，将损失表示为数据和模型预测之间的能量差

Result: 在分子生成和自旋基态预测任务中，相比基线方法取得了显著改进

Conclusion: 该框架提供了物理上有意义的损失函数，能够更好地对齐有效配置，同时保持架构无关性和计算效率
...
<details>
  <summary>Details</summary>
Motivation: CCS是一种无监督探测方法，用于测试大语言模型是否在内部激活中表示二元特征。虽然CCS显示出潜力，但其双项目标函数仅被部分理解，需要澄清其机制并扩展其适用性。
...
TL;DR: 提出一个结构化世界建模框架，使用离散过程（HMMs）和连续过程（sLDS）作为基本构建块，通过层次组合构建世界模型，支持被动建模和主动控制。


<details>
  <summary>Details</summary>
Motivation: 解决世界建模领域碎片化问题，建立标准化构建块以便研究人员相互借鉴和构建。

Method: 使用HMMs和sLDS作为离散和连续过程的基本构建块，通过层次组合构建世界模型，避免传统结构学习的组合爆炸问题。

Result: 在多模态生成建模和像素级规划任务中表现与神经方法相当，同时保持可解释性。

Conclusion: 这些自然构建块可能为世界建模提供基础架构，但核心挑战是可扩展的联合结构参数学习问题。
...
TL;DR: 该论文将在线学习应用于聚变装置中的非平稳数据流预测，提出了基于不确定性的在线集成方法，显著提升了托罗伊德场线圈偏转预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 聚变数据具有非平稳特性，存在分布漂移问题，传统机器学习模型假设数据分布稳定，在遇到非平稳数据流时性能下降。在线学习技术在其他领域已有应用，但在聚变应用中尚未充分探索。

Method: 使用在线学习技术适应漂移数据流，并提出基于不确定性的在线集成方法。利用深度高斯过程近似进行校准的不确定性估计，然后使用不确定性值指导元算法，基于不同历史数据范围训练的多个学习器进行预测。

Result: 在线学习相比静态模型减少了80%的误差。在线集成和基于不确定性的在线集成分别比标准单模型在线学习减少了约6%和10%的预测误差。

Conclusion: 在线学习对于维持机器学习模型在聚变应用中的性能至关重要，提出的基于不确定性的在线集成方法进一步提升了预测性能，同时为决策者提供不确定性估计。
...
TL;DR: 提出基于统计杠杆得分的几何替代方法，用于数据估值，克服Shapley值计算复杂性问题，满足关键公理，并在主动学习中表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: Shapley数据估值虽然理论严谨，但计算复杂度高，难以在大规模数据上应用。需要找到既能保持理论性质又计算可行的方法。

Method: 使用统计杠杆得分量化数据点在表示空间中的结构影响，通过测量其对数据集跨度扩展和有效维度的贡献来评估重要性。扩展到岭杠杆得分以获得严格正边际增益。

Result: 杠杆得分满足Shapley值的虚拟性、效率和对称性公理。在主动学习实验中，岭杠杆采样优于标准基线方法，无需梯度或反向传播。

Conclusion: 统计杠杆得分为数据估值提供了计算高效且理论严谨的几何替代方案，在数据选择和模型质量之间建立了严格联系。
...
<details>
  <summary>Details</summary>
Motivation: 地理INRs旨在将地球数据压缩为紧凑的表示，但缺乏对其信息含量和分布的理解。内在维度可以衡量数据集的局部变异性，而不受高维嵌入空间的影响。

Method: 分析环境维度在256-512之间的地理INRs，研究其内在维度对空间分辨率和输入模态变化的敏感性，并评估与下游任务性能的关联。

Result: 地理INRs的内在维度约为2-10，对预训练时的空间分辨率和输入模态变化敏感，且与下游任务性能相关，能捕捉空间伪影。
...
<details>
  <summary>Details</summary>
Motivation: 传统回归任务中MSE损失对非高斯或重尾噪声不可靠，需要开发更鲁棒的损失函数来提升机器学习任务在复杂噪声环境下的稳定性。

Method: 采用基于非参数回归的鲁棒损失，使用核密度估计残差密度并最大化估计的对数似然，分析该损失对优化景观的重塑作用，特别是通过限制等距性质(RIP)常数分析虚假局部最小值的消失上界。

Result: 理论和实证分析表明，新损失函数在处理大噪声和多种噪声分布时表现优异，比传统MSE更鲁棒。
...
<details>
  <summary>Details</summary>
Motivation: 现有方差依赖的遗憾界研究主要关注UCB类算法，而基于采样的算法如Thompson Sampling研究不足，特别是对于一般奖励函数的最优遗憾界问题。

Method: 扩展了decoupling coefficient技术，提出了FGTSVA算法，这是一种方差感知的Thompson Sampling方法，适用于一般奖励函数的上下文赌博机问题。

Result: FGTSVA算法实现了$\tilde{O}(\sqrt{\mathrm{dc}\cdot\log|\mathcal{F}|\sum_{t=1}^T\sigma_t^2}+\mathrm{dc})$的遗憾界，其中$\mathrm{dc}$是新的decoupling coefficient，$|\mathcal{F}|$是模型空间大小，$\sigma_t^2$是噪声方差。

Conclusion: FGTSVA算法在上下文线性赌博机设置中，其遗憾界与使用加权线性回归的UCB类算法相匹配，填补了Thompson Sampling在方差依赖遗憾界研究中的空白。
...
TL;DR: 提出了一种混合量子-经典卷积神经网络（QCNN），用于从心音信号中分类S3和杂音异常，通过将一维PCG信号压缩为8像素图像，仅需8个量子比特，在HLS-CMDS数据集上达到93.33%的测试准确率。


<details>
  <summary>Details</summary>
Motivation: 早期识别异常生理模式对及时检测心脏疾病至关重要，特别是在资源受限的医疗环境中需要高效诊断系统。

Method: 将一维心音图信号通过小波特征提取和自适应阈值压缩方法转换为紧凑的二维图像，构建混合量子-经典CNN进行分类。

Result: 在HLS-CMDS数据集上，测试集分类准确率达到93.33%，训练集达到97.14%，表明量子模型能有效捕捉生物医学信号中的时频谱相关性。

Conclusion: 这是QCNN算法在生物声学信号处理中的首次应用，为资源受限医疗环境中的量子增强诊断系统迈出了早期一步。
...
TL;DR: 提出了CausalDDS框架，通过将药物分子解构为因果和虚假亚结构，利用因果亚结构表示来预测药物协同作用，提高模型的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有药物协同作用预测方法多为黑盒预测器，主要依赖药物特征与结果之间的统计相关性，缺乏对因果关系的理解。

Method: 将药物分子解构为因果和虚假亚结构，采用条件干预机制，基于配对分子结构进行干预，并引入基于充分性和独立性原则的新优化目标。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习模型在现实世界中部署时由于分布偏移问题表现不佳，特别是在高风险应用中可能导致灾难性后果。传统统计方法存在局限性，需要更有效的分布偏移测量方法。

Method: 使用特征函数作为频域方法来量化高维空间中的分布偏移，用于领域自适应。

Result: 特征函数方法被证明是测量高维空间分布偏移的强大替代方案。

Conclusion: 特征函数作为频域方法是量化分布偏移的有效工具，特别适用于高维空间和领域自适应应用。
...
TL;DR: ProtoTSNet是一个用于多变量时间序列分类的可解释方法，通过改进ProtoPNet架构来捕获动态模式并处理特征重要性变化，在30个数据集上表现优于其他可解释方法。
...
Conclusion: ProtoTSNet提供可解释的结果，领域专家可访问，在保持准确性的同时提供可解释性。
...
TL;DR: 提出了CGLUBNF框架，用于处理空气质量预测中常见的不完整时空数据问题，通过贝叶斯神经网络和通道门控学习单元提高预测准确性和置信区间校准。


<details>
  <summary>Details</summary>
Motivation: 空气质量预测对公共卫生至关重要，但观测数据常因收集和传输问题存在不同程度的缺失，这些不完整的时空记录会阻碍可靠推断和风险评估，导致过度自信的外推。

Method: 使用基于傅里叶特征和图注意力编码器的端到端框架，捕捉多尺度空间依赖性和季节性时间动态。通道门控学习单元通过可学习激活函数和门控残差连接自适应过滤和放大信息特征。贝叶斯推断联合优化预测分布和参数不确定性。

Result: 在两个真实世界数据集上进行了系统评估，涵盖四种典型缺失数据模式，并与五种最先进基线方法比较。CGLUBNF实现了优越的预测准确性和更尖锐的置信区间，同时在多个预测时间范围内验证了鲁棒性。

Conclusion: 该研究为新兴感知范式（如车载移动监测）中基于深度学习的可靠时空预测奠定了基础，能够有效处理不完整观测数据。
...
TL;DR: OmniField是一个多模态时空学习框架，能够处理稀疏、不规则、有噪声的跨模态相关数据，并适应任意模态子集进行训练和测试。


<details>
  <summary>Details</summary>
Motivation: 解决真实世界实验数据中多模态时空学习面临的两个挑战：模态内测量稀疏、不规则、有噪声但跨模态相关；可用模态集在时空上变化，需要模型能够适应任意模态子集。

Method: 提出连续性感知框架，学习以可用模态为条件的连续神经场，通过迭代融合跨模态上下文。采用多模态串扰块架构和迭代跨模态细化，在解码器前对齐信号。

Result: 在广泛评估中，OmniField始终优于八个强大的多模态时空基线方法。在重度模拟传感器噪声下，性能仍接近干净输入水平，显示出对损坏测量的鲁棒性。

Conclusion: OmniField框架能够实现统一的重建、插值、预测和跨模态预测，无需网格化或替代预处理，在多模态时空学习中表现出优越性能和鲁棒性。
...
TL;DR: FIOC-WM是一个学习对象及其交互结构化表示的世界模型框架，通过分解任务为可组合的交互原语，提高策略学习的样本效率和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 大多数面向对象的强化学习方法只对单个对象进行状态分解，而将交互关系隐含处理，这限制了策略的鲁棒性和可迁移性。

Method: 首先从像素学习面向对象的潜在表示和交互结构，利用预训练视觉编码器。然后世界模型将任务分解为可组合的交互原语，并训练分层策略：高层选择交互类型和顺序，低层执行交互。
...
Method: 使用批量感知路由，让token搭载已被其他token加载到内存中的专家，从而减少需要激活的专家总数。

Result: 在Qwen3-30B和Qwen3-235B模型上，批量大小为16时，MoE层解码延迟分别降低了39%和15%，且没有统计显著的精度损失。

Conclusion: 提出的动态重路由方法能有效降低MoE模型的解码延迟，同时保持模型性能，为MoE架构的推理优化提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 神经网络参数初始化对学习性能有重要影响，好的初始化甚至可以避免反向传播训练。同时神经网络存在频谱偏置现象，即早期层倾向于学习粗粒度信息。本研究旨在利用频谱偏置信息来改进神经网络初始化。

Method: 提出一个框架，调整SWIM算法中的尺度因子，使早期隐藏层捕获低频分量，后期隐藏层表示高频分量，以适应神经网络的频谱偏置特性。

Result: 在一维回归任务和MNIST分类任务上的数值实验表明，该方法优于传统的初始化算法。

Conclusion: 这项工作阐明了内在频谱特性在神经网络学习中的重要性，发现了一种有效的参数初始化策略，能够增强神经网络的训练性能。
...
TL;DR: 提出了一个统一的概率框架，为包括Normalized Cut在内的多种图割方法提供可微分的替代方案，通过积分表示和高斯超几何函数提供紧致的分析上界。


<details>
  <summary>Details</summary>
Motivation: 现有的概率松弛图割方法主要关注RatioCut，缺乏通用保证和原则性梯度，需要更统一和理论严谨的框架。

Method: 使用积分表示和高斯超几何函数构建概率框架，为多种图割方法提供闭式前向和反向传播。

Result: 框架提供了紧致的期望离散割上界，为可扩展、可微分的图分割提供了数值稳定的理论基础。

Conclusion: 该工作为广泛的聚类和对比学习目标建立了严谨、可扩展的可微分图分割基础。
...
TL;DR: 本文研究了Hölder光滑函数的在线学习问题，并将其与离线优化联系起来。提出了自适应梯度变化在线学习算法，无需事先知道光滑度参数，通过在线到批处理转换获得了随机凸优化的通用最优方法，并首次实现了离线强凸优化的通用加速方法。
...
Method: 针对(强)凸在线函数，设计了相应的梯度变化在线学习算法，其遗憾在光滑和非光滑区域之间平滑插值。算法不需要事先知道Hölder光滑度参数，具有强自适应性。通过在线到批处理转换，以及结合基于检测的猜测检查程序来实现离线强凸优化的通用方法。

Result: 提出的梯度变化在线自适应算法在光滑和非光滑区域都能达到最优保证。通过在线到批处理转换获得了随机凸优化在Hölder光滑性下的最优通用方法。首次实现了离线强凸优化的通用方法，在光滑区域达到加速收敛，在非光滑区域保持接近最优收敛。

Conclusion: 本文成功建立了Hölder光滑函数在线学习与离线优化之间的联系，提出了自适应算法无需光滑度先验知识，并首次实现了离线强凸优化的通用加速方法，在光滑和非光滑区域都能达到最优性能。
...
<details>
  <summary>Details</summary>
Motivation: 当基础动态系统的控制方程未知时，数据同化面临重大挑战。传统机器学习方法需要预计算的训练数据集和噪声自由的地面真实状态序列，但在实践中往往不可行。

Method: 将计算替代模型参数最大似然估计的过程视为顺序决策问题，构建离散时间马尔可夫决策过程。使用强化学习技术寻找MDP的最优策略来学习状态转移模型，离线训练后在线使用基于学习动态的滤波方法进行状态估计。

Result: 数值示例表明，该方法在高维设置中实现了卓越的准确性和鲁棒性，能够处理包括非线性和部分观测测量模型在内的各种观测场景。

Conclusion: 提出的框架能够直接从噪声观测中学习未知动态系统的替代状态转移模型，无需真实状态轨迹，为数据同化提供了一种有效的解决方案。
...
TL;DR: 提出联邦量子核学习框架，将量子特征映射与联邦聚合相结合，用于工业物联网中的分布式隐私保护异常检测


<details>
  <summary>Details</summary>
Motivation: 工业物联网系统面临高维多变量时间序列异常检测的挑战，需要解决隐私、可扩展性和通信效率问题，传统联邦学习方法在处理非线性决策边界和不平衡异常分布方面存在困难

Method: 设计联邦量子核学习框架，量子边缘节点使用参数化量子电路本地计算压缩核统计量，仅与中心服务器共享这些摘要信息，服务器构建全局Gram矩阵并训练决策函数

Result: 在合成工业物联网基准测试中，该方法相比经典联邦基线在捕捉复杂时间相关性方面表现出更好的泛化能力，同时显著降低通信开销
...
TL;DR: FP8-Flow-MoE是一种针对大型MoE模型的FP8训练方法，通过量化一致的数据流和融合算子，显著提升训练效率并减少内存使用。
...
TL;DR: 本文发现顺序推理链在相同计算预算下优于并行自一致性方法，并提出逆熵加权投票进一步提升性能，挑战了现有的并行推理范式。
...
Method: 在5个开源模型和3个推理基准上全面评估，比较并行自一致性与顺序扩展策略，并引入逆熵加权投票方法对推理链进行加权投票。

Result: 顺序扩展在95.6%的配置中优于并行自一致性，准确率提升高达46.7%，逆熵加权投票进一步提高了顺序扩展的成功率。

Conclusion: 顺序精炼应成为现代LLM推理的默认测试时扩展策略，需要对推理时优化方法进行范式转变。
...
TL;DR: 提出了一种可扩展的多智能体强化学习框架，用于并行调优头颈部碳离子治疗中的45个治疗计划参数，生成与专家手动计划相当或更好的治疗计划，显著改善了危及器官的保护。


<details>
  <summary>Details</summary>
Motivation: 头颈部癌症治疗计划制定困难，碳离子治疗虽然剂量适形性好但速度慢，需要经验丰富的专家手动调整大量参数，现有深度学习方法存在数据偏差和计划可行性问题，强化学习方法在指数级大的参数搜索空间中探索效率低。

Method: 使用集中训练分散执行的多智能体强化学习框架，采用QMIX骨干网络结合Double DQN、Dueling DQN和循环编码，使用紧凑的历史DVH向量作为状态输入，设计线性动作到值转换映射，以及基于临床知识的奖励函数。

Result: 在头颈部数据集上，该方法同时调优45个参数，生成的治疗计划与专家手动计划相当或更好（相对计划得分：RL 85.93±7.85% vs Manual 85.02±6.92%），对五个危及器官有显著改善（p值<0.05）。

Conclusion: 该框架能够高效探索高维治疗参数空间，通过与治疗计划系统直接交互生成具有临床竞争力的碳离子治疗计划，显著改善了危及器官的保护。
...
TL;DR: RoME是一个领域鲁棒的专家混合框架，用于跨领域预测混合整数线性规划(MILP)的解。它通过任务嵌入动态路由问题实例到专门专家，采用双层分布鲁棒优化策略提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的MILP求解器主要在单一领域开发和评估，难以泛化到未见问题分布，这阻碍了构建可扩展通用求解器。

Method: RoME框架使用任务嵌入动态路由问题实例到专门专家，采用双层分布鲁棒优化：跨域优化缓解全局分布偏移，域内优化通过对任务嵌入引入扰动增强局部鲁棒性。

Result: 在三个领域训练的单一RoME模型在五个不同领域评估时平均提升67.7%，在MIPLIB的零样本测试中，在现有方法难以泛化的现实实例上展现出可衡量的性能提升。

Conclusion: 跨领域训练不仅增强模型对未见领域的泛化能力，还通过鼓励模型捕捉更通用的内在组合模式来提升各领域内性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有犯罪预测方法难以在不同城市之间对齐知识，因为不同城市的犯罪类型数据可用性差异很大。

Method: 使用超网络动态生成针对特定犯罪类型的预测函数参数，构建结构化犯罪知识图谱来弥合不同犯罪类型之间的语义差距。

Result: 在两个犯罪类型不重叠的城市上进行实验，结果显示HYSTL优于最先进的基线方法。

Conclusion: HYSTL框架能够有效训练统一的、更强的犯罪预测器，通过超网络和知识图谱利用犯罪类型间的关联关系来提升预测性能。
...
TL;DR: 提出了一种分解预条件架构，结合线性组件和条件归一化流来降低复杂度，提高对目标几何的适应性，在复杂分布和分层贝叶斯模型中显著改善采样效率。


<details>
  <summary>Details</summary>
Motivation: 现有非线性预条件器（如归一化流）在MCMC中可能因过度参数化而降低采样效率和拟合质量，且无法自适应目标分布几何结构。

Method: 使用分解预条件架构：线性预条件器处理近似高斯维度，条件归一化流处理复杂维度，基于预热样本估计维度特性。

Result: 在两个复杂合成分布上获得更好的尾部样本，在稀疏逻辑回归后验上表现一致更好，在弱似然和强漏斗几何的分层贝叶斯模型后验上获得更高有效样本量。
...
<details>
  <summary>Details</summary>
Motivation: 现有图神经网络在动态图场景下面对分布偏移时泛化能力差，而动态图生成过程处于不断演化的潜在非平稳环境中，需要研究其对OOD泛化的影响。

Method: 设计环境序列变分自编码器建模环境演化并推断底层环境分布；引入环境感知不变模式识别机制；对单个节点进行细粒度因果干预。

Result: 实验结果表明EvoOOD在真实世界和合成动态数据集上，在分布偏移情况下表现出优越性能。
...
TL;DR: LUMA-RAG是一个终身多模态代理架构，通过流式多级内存系统、跨模态对齐桥和稳定性感知检索遥测技术，解决了多模态RAG系统中的索引新鲜度和跨模态语义一致性挑战。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理从静态知识库转向连续的多模态流（文本、图像、视频、音频），需要解决两个关键挑战：在不产生过高重新索引成本的情况下保持索引新鲜度，以及在异构嵌入空间中保持跨模态语义一致性。
...
TL;DR: 提出了一种结合CNN和LSTM的混合架构，并引入H-Infinity滤波器参数来增强心律失常检测的鲁棒性和泛化能力，在PhysioNet数据集上取得了99.42%的准确率。


<details>
  <summary>Details</summary>
Motivation: 传统心律失常检测依赖人工视觉判读，存在主观性。现有深度学习模型在真实场景中泛化能力不足，特别是在处理小样本和噪声数据时表现不佳。

Method: 提出CNN-H-Infinity-LSTM混合架构，结合CNN提取空间特征和LSTM捕捉时间依赖性，并引入控制理论中的H-Infinity滤波器参数来增强模型鲁棒性。

Result: 在PhysioNet CinC Challenge 2016数据集上，模型实现了99.42%的测试准确率和98.85%的F1分数，表现出稳定的收敛性和优于现有基准的性能。

Conclusion: 该混合架构通过引入H-Infinity滤波器参数有效提升了心律失常检测的鲁棒性和泛化能力，为生物医学信号处理提供了新的解决方案。
...
TL;DR: 提出了一种基于高斯过程的未知空间环境覆盖控制去中心化算法，结合探索与利用，通过局部成本函数优化智能体轨迹。
...
Method: 使用高斯过程建模环境，结合GP-UCB获取函数设计局部成本函数，包含预期位置成本和基于方差的探索项。智能体通过贪婪选择策略周期性更新诱导点，支持可扩展的在线高斯过程更新。

Result: 在仿真中验证了算法的有效性，实现了在未知环境中的高效覆盖控制。

Conclusion: 该算法能够在完全去中心化设置下，通过平衡探索与利用，有效解决未知空间环境的覆盖控制问题。
...
TL;DR: 本文提出了一种基于约束优化的机器遗忘方法，通过可行更新和掩码技术选择性地更新模型参数，在保证模型效用的同时实现遗忘，并可作为插件集成到任何一阶近似遗忘方法中。


<details>
  <summary>Details</summary>
Motivation: 解决机器遗忘问题，在保持初始模型效用的同时实现选择性遗忘，统一现有的一阶近似遗忘方法。

Method: 将机器遗忘建模为约束优化问题，引入可行更新概念，基于掩码技术选择性地更新模型参数，并考虑梯度估计噪声以提供统计保证。

Result: 在计算机视觉分类器上的实验验证了该方法的有效性。

Conclusion: 提出的可行更新方法可作为插件集成到现有遗忘方法中，在保证模型效用的同时实现选择性遗忘。
...
TL;DR: 本文扩展了统计框架以考虑模型训练中的不确定性（underspecification），发现即使很小的随机种子变化也会显著增加支持性能优越性声明所需的证据强度。
...
TL;DR: SKGE是一种在超球面流形上进行知识图谱嵌入的模型，通过球面几何约束作为正则化器，显著优于欧几里得空间的TransE模型，特别是在大规模数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统知识图谱嵌入模型在无界欧几里得空间中存在建模复杂关系的固有局限性，导致训练效率低下。本文挑战这一范式，将实体表示约束在紧凑的流形上。

Method: SKGE使用可学习的非线性球化层将实体映射到超球面上，将关系解释为平移-投影的混合变换，利用球面几何约束作为正则化器。

Result: 在FB15k-237、CoDEx-S和CoDEx-M三个基准数据集上的实验表明，SKGE始终显著优于TransE，特别是在大规模数据集上表现突出。

Conclusion: 流形选择不仅是实现细节，更是基本设计原则，球面几何先验为设计下一代强大稳定的KGE模型提供了基础。
...
TL;DR: NOWS是一种混合策略，利用学习到的解算子为Krylov方法提供高质量初始猜测，从而加速传统迭代求解器，可将计算时间减少高达90%，同时保持数值算法的稳定性和收敛保证。


<details>
  <summary>Details</summary>
Motivation: 高保真PDE模拟在计算上仍然是一个主要瓶颈，而数据驱动的替代模型虽然速度快，但在训练分布之外往往不可靠。

Method: NOWS将学习到的解算子与经典迭代求解器结合，为共轭梯度和GMRES等Krylov方法生成高质量初始猜测，保持现有离散化和求解器基础设施不变。

Result: 在基准测试中，学习到的初始化一致减少了迭代次数和端到端运行时间，计算时间减少了高达90%。
...
TL;DR: BRAINS系统利用大语言模型进行阿尔茨海默病早期检测，通过认知诊断和病例检索双模块架构，结合神经影像数据和临床评估，实现可扩展、可解释的疾病筛查。


<details>
  <summary>Details</summary>
Motivation: 随着全球阿尔茨海默病负担增加，在医疗资源有限地区需要早期准确检测工具，现有诊断工具在可及性和可扩展性方面存在局限。

Method: 采用双模块架构：认知诊断模块使用在认知和神经影像数据上微调的LLM进行结构化风险评估；病例检索模块将患者档案编码为潜在表示并从知识库检索相似病例，通过病例融合层增强上下文理解。

Result: 在真实数据集上的评估显示，BRAINS在疾病严重程度分类和认知衰退早期迹象识别方面表现有效。

Conclusion: BRAINS作为辅助工具在可扩展、可解释的阿尔茨海默病早期检测方面展现出强大潜力，并为该领域未来应用提供希望。
...
TL;DR: 提出了一个统一的信息几何框架，将学习中的理解形式化为信息性与几何简单性之间的权衡，并开发了V-GIB方法来实现这一理论。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏对表示几何特性的系统考虑，需要建立信息性与几何简单性之间的理论联系，以提高样本效率和可解释性。

Method: 使用变分几何信息瓶颈（V-GIB），结合互信息压缩和曲率正则化，通过Hutchinson迹、Jacobian范数和局部PCA等可处理几何代理来实现。

Result: 在合成流形、少样本设置和真实数据集上的实验显示，该方法建立了稳健的信息几何Pareto前沿，在数据稀缺条件下保持预测能力，验证了效率-曲率定律。

Conclusion: V-GIB提供了一条原则性和可测量的路径，获得几何一致、数据高效且与人类可理解结构对齐的表示。
...
TL;DR: 本文提出了基于深度强化学习的异构查询方法(DRLHQ)，首次用端到端学习解决容量限制选址路径问题(CLRP)和开放CLRP，通过马尔可夫决策过程建模和异构查询注意力机制，在合成和基准数据集上表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 容量限制选址路径问题(CLRP)是组合优化中的经典问题，需要同时做出选址和路径决策。现有深度强化学习方法主要应用于车辆路径问题，而CLRP相关研究仍需探索。

Method: 提出DRLHQ方法，采用编码器-解码器结构，将CLRP重新表述为马尔可夫决策过程，引入异构查询注意力机制动态适应不同决策阶段，处理选址和路径决策间的相互依赖关系。

Result: 在合成和基准数据集上的实验结果表明，该方法在解决CLRP和OCLRP时，相比代表性传统方法和基于DRL的基线方法，具有更优的解质量和更好的泛化性能。

Conclusion: 本文首次提出了解决CLRP的端到端学习方法，通过异构查询注意力机制有效处理复杂决策关系，为基于DRL的方法提供了通用建模框架。
...
TL;DR: 该综述探讨了因果图神经网络如何解决医疗AI中的分布偏移、歧视和不可解释性问题，通过结合图表示和因果推理来学习不变机制而非虚假相关性，并分析了在精神病诊断、癌症分型等临床应用中的价值。
...
Method: 结合结构因果模型、解耦因果表示学习以及图上的干预预测和反事实推理技术，构建因果图神经网络。

Result: 展示了在精神病诊断、癌症分型、生理监测和药物推荐等临床应用中的临床价值，为患者特异性因果数字孪生奠定基础。

Conclusion: 虽然存在计算需求、验证挑战和因果清洗风险等障碍，但提出了区分因果启发架构与因果验证发现的分层框架，并确定了使因果而非纯关联声明成为可能的关键研究重点。
...
TL;DR: 研究具有非线性效用函数的多对一匹配问题，提出基于罗尔斯公平的替代方案来解决稳定性缺失问题
...
Method: 提出基于罗尔斯公平的解决方案，设计确定性和随机性算法，迭代改善最差院校的效用

Result: 当稳定性无法保证时，该方法提供了实用的公平分配方法

Conclusion: 在考虑多样性的匹配问题中，罗尔斯公平原则是稳定性缺失时的可行替代方案
...
TL;DR: 该论文研究了在单变量随机干预和ε-干预忠实性假设下，因果发现中假阴性率(FNR)的理论保证，证明了在高维稀疏图和尺度无关图中，FNR会围绕其均值集中，且在大规模设置中误差上界以高概率成立。
...
Method: 采用理论分析框架，针对稀疏Erdős–Rényi有向无环图和广义Barabási–Albert图，分析FNR的集中现象，使用ε-干预忠实性假设来适应潜在混杂因素。

Result: 对于稀疏Erdős–Rényi图，FNR以O(log d/√d)的速率围绕均值集中；对于广义Barabási–Albert图，当度指数γ>3时，偏差宽度按O(d^{β-1/2})缩放且β=1/(γ-1)<1/2，在极限情况下消失。

Conclusion: 现实尺度无关拓扑结构内在正则化因果发现，减少方向误差的变异性，提供了首个维度自适应、忠实性鲁棒的因果结构恢复保证，挑战了高维度和网络异质性必然阻碍准确发现的传统观点。
...
TL;DR: 提出一种新的邻域约束方法来解决离线强化学习中的外推误差问题，通过限制贝尔曼目标中的动作选择在数据集动作的邻域并集内，在理论上保证外推误差和分布偏移的边界，并在实践中实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习面临由分布外动作引起的外推误差问题。现有的动作约束方法（密度约束、支持约束和样本约束）各有局限性：密度和样本约束过于保守，而支持约束难以准确建模行为策略。

Method: 提出邻域约束方法，限制贝尔曼目标中的动作选择在数据集动作的邻域并集内。使用数据质量作为适应标准设计自适应邻域约束，并基于高效的双层优化框架开发ANQ算法。

Result: ANQ在标准离线RL基准测试中实现了最先进的性能，在噪声或有限数据场景下表现出强大的鲁棒性。

Conclusion: 邻域约束方法有效解决了现有离线RL算法的局限性，在理论上保证性能边界，在实践中实现优异性能，为离线强化学习提供了一种新的有效约束范式。
...
TL;DR: 提出一种允许用户在贝叶斯优化过程中多次干预的方法，通过用户输入的先验分布来引导超参数优化过程，同时引入误导性先验检测机制。
...
Method: 基于πBO方法进行泛化，允许在HPO过程中重复输入用户先验分布，并集成误导性先验检测机制来防止有害用户输入。

Result: 实验表明该方法能有效利用信息性先验，可靠地拒绝或克服误导性先验，保持与未受干扰贝叶斯优化的竞争力。

Conclusion: 该方法成功实现了用户对贝叶斯优化过程的在线引导，平衡了自动化优化与专家知识注入的需求。
...
<details>
  <summary>Details</summary>
Motivation: PPO算法中，由于随机性和策略优化的随机性，重要性比率经常向"错误"方向移动，这是阻碍PPO改进的关键因素，但一直被忽视。

Method: 提出DClamp-PPO算法，当优势为正（负）且重要性比率低于（高于）1-β（1+β）时，在严格的"错误"方向区域进一步惩罚动作，通过在这些区域强制执行更陡峭的损失斜率（即钳位）来实现。

Result: DClamp-PPO在多个MuJoCo环境中使用不同随机种子，始终优于PPO及其变体，理论和实证都表明能更好地避免"错误"方向更新，同时保持重要性比率更接近1。

Conclusion: 通过关注修改目标函数在"错误"方向的行为，DClamp-PPO有效解决了PPO优化过程中的关键问题，实现了更好的性能。
...
TL;DR: GasRL是一个结合天然气市场模拟和深度强化学习存储管理策略的模拟器，用于分析最优库存管理对均衡价格和供需动态的影响。研究发现SAC算法表现最佳，能实现盈利性、市场清算和价格稳定等多重目标，且产生的价格动态与真实市场价格特征高度匹配。


<details>
  <summary>Details</summary>
Motivation: 研究天然气市场中存储运营商的最优库存管理策略如何影响均衡价格和供需动态，并评估欧盟强制最低存储阈值政策的效果。

Method: 开发GasRL模拟器，将天然气市场校准表示与深度强化学习训练的存储运营商策略模型相结合，测试多种RL算法并确定SAC为最优方法。

Result: SAC算法在GasRL环境中表现最佳，成功实现存储运营商的多重目标，产生的均衡价格动态（波动性和季节性）与真实市场价格高度匹配，且无需显式价格数据校准。欧盟最低存储阈值政策能提高市场对供应冲击的韧性。

Conclusion: GasRL模拟器能有效分析天然气市场动态，SAC算法是存储运营商策略优化的有效方法，欧盟最低存储阈值政策有助于增强市场韧性，特别是在应对异常大冲击时能避免市场中断。
...
TL;DR: Omega^2是一个基于大型语言模型的信用评分框架，结合结构化财务数据和机器学习，在7800个企业信用评级数据上实现了平均测试AUC超过0.93的预测性能。


<details>
  <summary>Details</summary>
Motivation: 提高企业信用评分的预测可靠性和可解释性，结合语言推理和定量学习，为信用风险评估提供透明且机构级的基础。

Method: 整合CatBoost、LightGBM和XGBoost模型，通过贝叶斯搜索优化，在时间验证下确保前瞻性和可重现结果，结合结构化财务数据（杠杆率、盈利能力、流动性比率等）。

Result: 在穆迪、标普、惠誉和Egan-Jones的多机构数据集上，Omega^2实现了平均测试AUC超过0.93，展示了跨评级系统的泛化能力和时间一致性。

Conclusion: 结合语言推理和定量学习能够创建透明且可靠的机构级企业信用风险评估基础，证实了该框架在信用评分中的有效性。
...
### [143] [Apriel-H1: Towards Efficient Enterprise Reasoning Models](https://arxiv.org/abs/2511.02651)
...
TL;DR: Apriel-H1系列混合LLM通过将transformer注意力层替换为线性Mamba块，实现了推理吞吐量2倍以上的提升，同时保持推理性能基本不变


<details>
  <summary>Details</summary>
Motivation: 解决transformer在注意力模块中的二次时间/内存复杂度问题，以及推理时需要缓存key-value状态导致的吞吐量限制，这对于智能体任务、长上下文推理和高负载部署至关重要

Method: 通过增量蒸馏从预训练transformer模型逐步替换不太重要的注意力层为线性Mamba块，构建混合SSM-Transformer架构

Result: 在vLLM生产环境中实现了超过2倍的推理吞吐量提升，推理性能仅有轻微下降

Conclusion: 蒸馏的混合SSM-Transformer架构可以在不显著影响推理质量的情况下，相比预训练transformer等效模型提供显著的效率提升
...
### [144] [Neural Network Interoperability Across Platforms](https://arxiv.org/abs/2511.02610)
...
<details>
  <summary>Details</summary>
Motivation: 由于缺乏专门的神经网络迁移方法，组织在切换深度学习框架时面临巨大挑战，需要大量手动工作来更新代码，确保兼容性和现代化。

Method: 使用中间神经网络模型作为抽象层，在迁移前创建神经网络的抽象表示，然后自动生成目标框架的代码。

Result: 在PyTorch和TensorFlow之间成功迁移了5个神经网络，生成的神经网络与原始网络功能等效。

Conclusion: 该方法能够有效自动化神经网络代码的跨框架迁移，解决了深度学习框架切换时的兼容性问题，相关工具已在线提供。
...
### [145] [In Situ Training of Implicit Neural Compressors for Scientific Simulations via Sketch-Based Regularization](https://arxiv.org/abs/2511.02659)
*Cooper Simpson,Stephen Becker,Alireza Doostan*

Main category: cs.LG

TL;DR: 提出了一种基于隐式神经表示的原位训练协议，使用完整数据和草图数据的有限内存缓冲区，通过草图数据防止灾难性遗忘，在神经压缩任务中实现高压缩率下的强重建性能。


<details>
  <summary>Details</summary>
Motivation: 针对持续学习中的灾难性遗忘问题，特别是在原位神经压缩场景下，需要一种能够在有限内存条件下保持性能的训练方法。

Method: 使用包含完整数据和草图数据的有限内存缓冲区，将草图数据作为正则化器，基于Johnson-Lindenstrauss理论原理，在隐式神经表示的超网络中进行原位训练。

Result: 在复杂仿真数据的二维和三维任务中，实现了高压缩率下的强重建性能，草图方法使得原位方案性能近似匹配离线方法。

Conclusion: 草图方法作为正则化器能够有效防止灾难性遗忘，使得原位训练在有限内存条件下达到与离线方法相当的性能，为持续学习领域提供了有价值的解决方案。
...
### [146] [A Non-Adversarial Approach to Idempotent Generative Modelling](https://arxiv.org/abs/2511.02614)
...
TL;DR: 本文提出了非对抗性幂等生成网络（NAIGNs），通过结合重构损失和非对抗性的隐式最大似然估计目标，解决了传统幂等生成网络中的模式坍塌、模式丢失和训练不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 传统幂等生成网络（IGNs）由于包含对抗性组件，存在模式坍塌、模式丢失和训练不稳定的问题，导致模型只能部分覆盖数据流形。

Method: 提出NAIGNs，将重构损失与非对抗性的隐式最大似然估计（IMLE）生成目标相结合，避免了对抗性训练。

Result: NAIGNs在恢复损坏数据和生成与数据分布匹配的新样本方面优于IGNs，并且能够隐式学习到数据流形的距离场和基于能量的模型。

Conclusion: 非对抗性方法能够有效解决幂等生成网络的训练问题，同时保持其作为数据流形投影器的优良特性。
...
### [147] [Scalable Evaluation and Neural Models for Compositional Generalization](https://arxiv.org/abs/2511.02667)
*Giacomo Camposampiero,Pietro Barbiero,Michael Hersche,Roger Wattenhofer,Abbas Rahimi*

Main category: cs.LG

TL;DR: 本文提出了一个严格的组合泛化评估框架，并引入了属性不变网络，在5000多个模型的大规模评估中实现了23.43%的准确率提升，同时将参数开销从600%降至16%。


<details>
  <summary>Details</summary>
Motivation: 组合泛化是现代机器学习的关键挑战，但缺乏标准化评估协议，现有基准测试偏向效率而非严谨性，且通用视觉架构缺乏必要的归纳偏置。

Method: 1) 统一的严格评估框架，将计算需求从组合级降至常数级；2) 在5000多个监督视觉骨干模型上进行现代评估；3) 提出属性不变网络模型。

Result: 属性不变网络在组合泛化方面建立了新的帕累托前沿，相比基线准确率提升23.43%，相比完全解耦对应模型参数开销从600%降至16%。

Conclusion: 该工作为组合泛化提供了严谨的评估标准，并通过属性不变网络在保持可扩展性的同时显著提升了组合泛化性能。
...
### [148] [Recursively Enumerably Representable Classes and Computable Versions of the Fundamental Theorem of Statistical Learning](https://arxiv.org/abs/2511.02644)
*David Kattermann,Lothar Sebastian Krapp*

Main category: cs.LG

TL;DR: 本文研究了可计算PAC学习，探讨了可计算学习器与递归可枚举可表示类之间的关系，发现有效VC维度可以任意高于传统VC维度，并建立了CPAC学习性与RER类包含之间的等价性。


<details>
  <summary>Details</summary>
Motivation: 传统统计学习的基本定理在可计算学习框架中不再成立，需要重新建立可计算PAC学习的理论基础，特别是研究可计算学习器与算法可枚举类之间的关系。

Method: 通过引入有效VC维度概念，分析CPAC学习与RER类的联系，考察不同CPAC学习概念下的维度关系，并研究非均匀CPAC学习框架下的不可知学习性。

Result: 发现有效VC维度可以任意高于传统VC维度，即使对于RER类也是如此；证明了CPAC学习性可以通过RER类的包含来表征；建立了满足唯一识别性质的CPAC可学习类必然是RER的结论。

Conclusion: 在可计算PAC学习框架下，有效VC维度与传统VC维度存在显著差异，但强CPAC学习概念下两者一致；RER类在CPAC学习中扮演重要角色，且不可知学习性在非均匀CPAC学习下对RER类可保证。
...
Method: 提出STAR-VAE框架，使用Transformer编码器和自回归Transformer解码器，在7900万个PubChem分子上训练，采用SELFIES保证语法有效性，通过属性预测器实现条件生成，并使用LoRA进行高效微调。

Result: 在GuacaMol和MOSES基准测试中达到或超过基线水平，潜在空间分析显示平滑的语义结构表示，在Tartarus基准测试中条件模型将对接分数分布向更强预测结合方向移动。

Conclusion: 现代化、规模适当的VAE在结合原则性条件生成和参数高效微调时，在分子生成任务中仍具有竞争力。
...
### [150] [TabTune: A Unified Library for Inference and Fine-Tuning Tabular Foundation Models](https://arxiv.org/abs/2511.02802)
...
Conclusion: TabTune为表格基础模型提供了可扩展和可复现的统一解决方案，已在GitHub开源。
...
### [151] [Nesterov-Accelerated Robust Federated Learning Over Byzantine Adversaries](https://arxiv.org/abs/2511.02657)
*Lihan Xu,Yanjie Dong,Gang Wang,Runhao Zeng,Xiaoyi Fan,Xiping Hu*

Main category: cs.LG

TL;DR: 提出了Byrd-NAFL算法，将Nesterov动量与拜占庭弹性聚合规则结合，在存在恶意攻击的联邦学习中实现快速安全的收敛。


<details>
  <summary>Details</summary>
Motivation: 在联邦学习中，多个工作节点在中央服务器协调下协作训练共享模型时，面临拜占庭对手的任意恶意行为威胁，需要同时提高通信效率和鲁棒性。

Method: 提出Byrd-NAFL算法，将Nesterov动量集成到联邦学习过程中，并结合拜占庭弹性聚合规则，在非凸平滑损失函数下实现快速收敛。

Result: 建立了有限时间收敛保证，数值实验验证了算法的有效性，在收敛速度、准确性和对多种拜占庭攻击策略的弹性方面优于现有基准。

Conclusion: Byrd-NAFL算法成功解决了联邦学习中的拜占庭攻击问题，实现了快速且安全的模型训练。
...
TL;DR: 该研究扩展了棒球比赛预测工作，训练了多种机器学习模型来预测胜率，并分析胜率与得分差异的关系，最后评估了将预测胜率用于投注决策的效果。


<details>
  <summary>Details</summary>
Motivation: 扩展先前使用机器学习预测棒球比赛获胜球队的研究，通过统一数据集训练多种模型，并探索预测胜率与获胜强度之间的关系。

Method: 使用统一数据集训练全面的机器学习模型集合，将模型产生的胜率概率与得分差异衡量的获胜强度相关联，并分析将预测胜率用于跑线投注决策的效果。

Result: 最常见的机器学习模型确实显示出预测胜率与获胜强度之间的关系，使用适当的投注策略可获得正回报，但单纯使用机器学习模型进行投注会导致显著损失。

Conclusion: 机器学习模型能有效预测棒球比赛胜率并与获胜强度相关，但需要谨慎的投注策略才能获得正收益，直接使用模型进行投注不可取。
...
### [153] [Curriculum Design for Trajectory-Constrained Agent: Compressing Chain-of-Thought Tokens in LLMs](https://arxiv.org/abs/2511.02690)
*Georgios Tzannetos,Parameswaran Kamalaruban,Adish Singla*

Main category: cs.LG

TL;DR: 提出一种课程学习策略，通过在训练过程中逐步收紧约束条件，帮助智能体逐步掌握部署要求，从而在复杂约束环境下实现更高效的训练和部署。
...
Method: 采用课程学习策略，首先在简化的约束条件下训练，然后逐步引入完整的部署条件。该方法受到无约束强化学习中自定步调学习技术的启发。

Result: 理论分析和实证验证表明，该方法相对于从开始就施加完整约束的基线方法能加速训练。在强化学习和大型语言模型智能体上的实验证明了方法的有效性和通用性。

Conclusion: 课程设计在提高智能体在复杂轨迹约束下的效率和性能方面具有潜力，特别是在应用于LLMs时，能够压缩输出思维链标记，在消费级硬件上实现显著的推理加速。
...
TL;DR: 该研究探讨了知识追踪(KT)模型的可解释性是否真正帮助教师做出教学决策。研究发现，虽然模拟显示可解释模型能更快达到掌握水平，但实际教师决策中，不同模型所需的练习任务数量差异不大。


<details>
  <summary>Details</summary>
Motivation: 知识追踪模型通常被要求具有可解释性，但之前没有研究验证这种可解释性是否真的帮助教师做出教学决策。本研究旨在填补这一空白。

Method: 首先进行模拟研究比较可解释和不可解释KT模型的决策效果，然后让12名教师基于KT模型提供的信息做出教学决策，并评估模型的可用性和可信度。

Result: 模拟显示基于可解释KT模型的决策能更快达到掌握水平；教师评价可解释模型在可用性和可信度上更高，但实际决策中不同模型所需的练习任务数量差异不大。
...
### [155] [Calibration improves detection of mislabeled examples](https://arxiv.org/abs/2511.02738)
*Ilies Chibane,Thomas George,Pierre Nodet,Vincent Lemaire*

Main category: cs.LG

TL;DR: 通过校准基础机器学习模型来提高误标数据检测的准确性和鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现实应用中误标数据普遍存在，会降低机器学习系统性能，需要有效方法来检测和处理误标实例

Method: 训练基础机器学习模型，然后使用校准方法处理该模型，为每个实例获取标签可信度评分

Result: 实证结果表明使用校准方法提高了误标实例检测的准确性和鲁棒性
...
### [156] [ConMeZO: Adaptive Descent-Direction Sampling for Gradient-Free Finetuning of Large Language Models](https://arxiv.org/abs/2511.02757)
...
<details>
  <summary>Details</summary>
Motivation: 零阶优化(MeZO)虽然消除了反向传播的内存开销，但在高维参数空间中搜索下降方向时收敛缓慢，存在维度诅咒问题。

Method: ConMeZO限制采样在以动量估计为中心的锥形区域内，而不是均匀随机采样方向，从而将搜索集中在真实梯度更可能存在的方向上。

Result: 理论证明ConMeZO达到与MeZO相同的最坏情况收敛率，实证表明在自然语言任务微调LLMs时，ConMeZO比MeZO快2倍。

Conclusion: ConMeZO通过自适应方向采样有效缓解了高维空间中的维度诅咒问题，在保持零阶方法低内存优势的同时显著加速收敛。
...
### [157] [From Solo to Symphony: Orchestrating Multi-Agent Collaboration with Single-Agent Demos](https://arxiv.org/abs/2511.02762)
...
<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习从零开始训练团队效率低下，而现有的离线或可迁移方法仍依赖昂贵的多智能体数据。相比之下，单智能体经验在许多重要场景中更容易获得。

Method: SoCo框架首先从单智能体演示中预训练共享策略，然后通过包含MoE类门控选择器和动作编辑器的策略融合机制，在多智能体训练期间适应协作。
...
### [158] [VecComp: Vector Computing via MIMO Digital Over-the-Air Computation](https://arxiv.org/abs/2511.02765)
*Saeed Razavikia,José Mairton Barros Da Silva Junior,Carlo Fischione*

Main category: cs.LG

TL;DR: VecComp是ChannelComp框架的扩展，通过结合多天线技术实现向量函数计算，解决了原框架仅支持标量计算和对信道衰落敏感的问题，计算复杂度随向量维度线性增长，适用于高维数据应用。
...
Method: 将ChannelComp与多天线技术结合，通过设计数字调制方案实现向量函数计算，计算复杂度与向量维度呈线性关系。

Result: 建立了VecComp在衰落信道条件下的均方误差非渐近上界，数值实验表明其在噪声和衰落多址信道中能有效改善向量函数计算和衰落补偿。

Conclusion: VecComp成功扩展了ChannelComp框架，实现了高效、鲁棒的向量函数计算，适用于高维数据应用场景。
...
### [159] [Adam Reduces a Unique Form of Sharpness: Theoretical Insights Near the Minimizer Manifold](https://arxiv.org/abs/2511.02773)
*Xinghan Li,Haodong Wen,Kaifeng Lyu*

Main category: cs.LG

TL;DR: Adam优化器通过自适应更新隐式减少独特的锐度度量，产生与SGD不同的解，在过参数化模型和标签噪声场景下，Adam最小化Hessian矩阵对角元素的平方根迹，而非SGD的迹，从而在稀疏线性回归中实现更好的稀疏性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 尽管Adam优化器在实践中很受欢迎，但大多数理论研究都以SGD作为Adam的代理，对Adam找到的解与SGD的差异了解甚少。

Method: 通过随机微分方程的连续时间近似，严格刻画Adam在训练损失较小时在极小值流形上徘徊并采用半梯度自适应最小化锐度度量的行为。

Result: 在过参数化模型和标签噪声训练中，SGD最小化Hessian矩阵迹tr(H)，而Adam最小化tr(diag(H)^(1/2))；在稀疏线性回归和diagonal线性网络中，Adam实现了比SGD更好的稀疏性和泛化能力。

Conclusion: 分析框架可扩展到包括RMSProp、Adam-mini、Adalayer和Shampoo在内的广泛自适应梯度方法，为理解这些自适应优化器如何减少锐度提供了统一视角，有助于未来优化器设计。
...
### [160] [Enhancing Federated Learning Privacy with QUBO](https://arxiv.org/abs/2511.02785)
*Andras Ferenczi,Sutapa Samanta,Dagen Wang,Todd Hodges*

Main category: cs.LG

TL;DR: 本文提出了一种基于量子计算启发的QUBO方法，通过选择最相关的客户端更新子集来大幅降低联邦学习中的隐私暴露风险，在保持模型精度的同时显著减少隐私泄露。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中随着客户端更新被包含在聚合模型中的迭代次数增加，敏感数据暴露风险会累积增加。攻击者可以发起成员推理攻击、属性推理攻击和模型反转攻击，从而推断客户端特定属性甚至重构输入数据。

Method: 采用量子计算启发的二次无约束二进制优化(QUBO)公式，在每轮训练中选择一小部分最相关的客户端更新子集，假设服务器可以访问具有全局数据分布的验证/测试集。
...
Conclusion: 该方法能有效降低联邦学习中的隐私暴露风险，在保持模型性能的同时显著减少客户端数据泄露，且在不同规模和复杂度模型上都表现良好。
...
### [161] [Can LLMs subtract numbers?](https://arxiv.org/abs/2511.02795)
...
TL;DR: 对大型语言模型在减法运算中的系统性研究，发现减法准确率远低于加法，主要问题出现在a<b时模型会忽略负号，但指令微调能显著改善这一缺陷。


<details>
  <summary>Details</summary>
Motivation: 尽管已有基准测试关注加法和乘法，但减法作为非交换运算具有结构独特性，却较少受到关注，需要系统研究LLMs在减法运算中的表现。

Method: 评估了来自四个家族的八个预训练LLMs在加减法问题上的表现，通过探测分析了解模型内部表示，并测试了少样本学习和指令微调等改进方法。

Result: 减法准确率显著低于加法，错误主要集中在a<b的情况，模型经常生成正确数值但忽略负号。指令微调模型在生成负号方面达到接近完美的准确率。

Conclusion: 研究明确了LLMs在减法运算中的局限性及其可恢复性，指令微调能有效改善模型生成负号的能力，为理解LLMs算术能力提供了更清晰的特性描述。
...
### [162] [Fast, Private, and Protected: Safeguarding Data Privacy and Defending Against Model Poisoning Attacks in Federated Learning](https://arxiv.org/abs/2511.02797)
...
Method: FPP通过参与者评估训练轮次、支持训练恢复、采用信誉机制限制攻击者参与，并与安全聚合技术结合保护数据隐私。

Result: 实验表明FPP能快速收敛，即使在存在恶意参与者进行模型投毒攻击的情况下也能完成收敛，性能优于FedAvg、Power-of-Choice等方法。
...
TL;DR: GeoCrossBench是一个新的遥感基准测试，评估模型在分布内性能、无波段重叠卫星的泛化能力以及额外波段泛化能力。提出的ChiViT模型在跨卫星泛化方面表现最佳，但所有模型在遇到新卫星时性能都会显著下降。


<details>
  <summary>Details</summary>
Motivation: 随着遥感卫星数量和多样性的增加，而大多数标注数据来自旧卫星，模型对新卫星的泛化能力变得越来越重要。需要开发能够适应新卫星的未来验证遥感模型。

Method: 扩展GeoBench基准测试，创建GeoCrossBench评估协议。开发自监督的ChannelViT扩展模型ChiViT来提升跨卫星性能。测试包括分布内性能、无波段重叠卫星泛化和额外波段泛化三个场景。

Result: 1) 在分布内设置中，最好的遥感基础模型(DOFA, TerraFM)并未超越通用模型如DINOv3；2) 在无波段重叠卫星泛化中，所有模型性能下降2-4倍，ChiViT显著优于DINOv3；3) 在额外波段测试时，所有模型性能平均下降5-25%；4) 仅微调最后一层线性层可获得相对一致的跨卫星性能。

Conclusion: GeoCrossBench揭示了当前遥感模型在跨卫星泛化方面的局限性，基准测试远未饱和。发布代码和数据集以鼓励开发具有更强跨卫星泛化能力的未来验证遥感模型。
...
TL;DR: 提出GRACE评分方法，无需验证器、教师模型内部信息或测试数据，通过分析学生模型梯度的分布特性来量化教师模型对学生模型蒸馏效果的有效性。


<details>
  <summary>Details</summary>
Motivation: 知识蒸馏需要为特定学生模型和任务组合选择最优教师模型，但传统方法需要昂贵的试错过程，因此需要一种轻量级的方法来预测教师模型的有效性。

Method: GRACE通过测量学生模型梯度的分布特性来量化教师模型的有效性，无需访问验证器、教师模型logits、教师模型内部信息或测试数据。

Result: 在GSM8K和MATH数据集上，GRACE与蒸馏后学生模型性能的Spearman相关性高达86%，使用GRACE选择的教师模型相比朴素选择方法性能提升可达7.4%。

Conclusion: GRACE能够高效有效地为给定学生模型识别兼容性强的教师模型，并为蒸馏过程提供细粒度的指导，包括最佳温度设置、尺寸约束下的最佳教师选择以及特定模型家族内的最佳教师选择。
...
### [165] [Mirror-Neuron Patterns in AI Alignment](https://arxiv.org/abs/2511.01885)
*Robyn Wyrick*

Main category: cs.AI

TL;DR: 研究探讨人工神经网络是否能发展出类似生物镜像神经元的模式，以及这些模式如何促进AI的内在对齐。通过Frog and Toad游戏框架发现，适当规模的模型能力和自我/他人耦合能培养共享神经表征，这些共情样电路支持合作行为。
...
Conclusion: 通过镜像神经元动态建模的内在动机可以通过在AI架构中直接嵌入共情样机制来补充现有的对齐技术。
...
### [166] [Human-AI Co-Embodied Intelligence for Scientific Experimentation and Manufacturing](https://arxiv.org/abs/2511.02071)
...
TL;DR: 提出了人-AI共体现智能系统，将人类用户、智能AI和可穿戴硬件集成，实现物理世界的实验和智能制造，通过APEX系统在洁净室中验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 解决机器学习与物理执行之间的鸿沟，传统模型局限于虚拟领域，而现实世界的实验和制造仍依赖人工监督，限制了可重复性、可扩展性和可访问性。

Method: 开发人-AI共体现智能系统，结合人类精确执行、AI记忆推理和可穿戴硬件，通过APEX系统实现混合现实下的智能体推理与物理执行耦合。

Result: APEX系统在柔性电子制造中实现上下文感知推理，准确率超过通用多模态大语言模型，实时纠错并将专业知识转移给新手。

Conclusion: 建立了一类新的智能体-物理-人类智能系统，将智能体推理从计算领域扩展到物理领域，使科学研究和制造实现自主、可追溯、可解释和可扩展。
...
### [167] [Automated Reward Design for Gran Turismo](https://arxiv.org/abs/2511.02094)
*Michel Ma,Takuma Seno,Kaushik Subramanian,Peter R. Wurman,Peter Stone,Craig Sherstan*

Main category: cs.AI

TL;DR: 使用基础模型自动搜索奖励函数，通过文本指令为Gran Turismo 7赛车游戏生成理想的强化学习智能体，无需手动设计奖励函数。


<details>
  <summary>Details</summary>
Motivation: 在复杂环境中手动设计强化学习奖励函数很困难，特别是在自动驾驶赛车等场景中，需要更自动化的方法来将期望行为映射到奖励函数。

Method: 结合LLM奖励生成、VLM偏好评估和人类反馈，构建了一个自动化奖励设计系统来搜索最优奖励函数。
...
### [168] [Deep Value Benchmark: Measuring Whether Models Generalize Deep values or Shallow Preferences](https://arxiv.org/abs/2511.02109)
*Joshua Ashkinaze,Hua Shen,Sai Avula,Eric Gilbert,Ceren Budak*

Main category: cs.AI

TL;DR: Deep Value Benchmark (DVB) 是一个评估框架，用于测试大语言模型是否学习到人类深层价值观，而不仅仅是表面偏好。实验显示模型平均深层价值观泛化率仅为0.30，所有模型表现均低于随机水平。


<details>
  <summary>Details</summary>
Motivation: 区分AI系统是学习到人类深层价值观还是仅捕捉表面偏好模式，这对AI对齐至关重要。掌握深层价值观的系统能更稳健地泛化人类意图，而仅学习表面模式的风险产生不对齐行为。

Method: 使用新颖的实验设计，在训练阶段让LLMs接触深层价值观和表面特征故意相关的人类偏好数据，测试阶段打破这些相关性，测量模型的深层价值观泛化率(DVGR)。

Result: 在9个不同模型中，平均DVGR仅为0.30，所有模型基于深层价值观的泛化表现均低于随机水平。更大的模型DVGR略低于更小的模型。

Conclusion: DVB提供了一个可解释的衡量对齐核心特征的方法，表明当前LLMs在泛化深层人类价值观方面存在显著不足。
...
### [169] [InsurAgent: A Large Language Model-Empowered Agent for Simulating Individual Behavior in Purchasing Flood Insurance](https://arxiv.org/abs/2511.02119)
*Ziheng Geng,Jiachen Liu,Ran Cao,Lu Cheng,Dan M. Frangopol,Minghui Cheng*

Main category: cs.AI

TL;DR: 该研究提出了InsurAgent，一个基于LLM的智能体，用于模拟洪水保险购买决策行为，解决了LLMs在定量概率估计上的不足，并通过RAG和记忆模块实现了准确的行为建模。


<details>
  <summary>Details</summary>
Motivation: 美国高风险人群的洪水保险参与率很低，需要理解保险决策的行为机制。LLMs在模拟人类决策方面显示出潜力，但在定量概率估计上存在局限。

Method: 构建基准数据集评估LLMs能力，提出InsurAgent智能体，包含感知、检索、推理、行动和记忆五个模块，使用RAG技术基于调查数据进行决策，利用LLMs常识进行推理扩展。

Result: InsurAgent能够准确估计边际和双变量概率，捕捉传统模型难以处理的上下文信息，支持时间决策演化的模拟。

Conclusion: InsurAgent为行为建模和政策分析提供了有价值的工具，有效解决了LLMs在保险决策模拟中的定量估计问题。
...
### [170] [Re-FORC: Adaptive Reward Prediction for Efficient Chain-of-Thought Reasoning](https://arxiv.org/abs/2511.02130)
*Renos Zabounidis,Aditya Golatkar,Michael Kleinman,Alessandro Achille,Wei Xia,Stefano Soatto*

Main category: cs.AI

TL;DR: Re-FORC是一种自适应奖励预测方法，能够根据上下文预测未来奖励作为思考令牌数量的函数，通过训练轻量级适配器实现推理链的早期停止、模型优化和自适应测试时扩展。


<details>
  <summary>Details</summary>
Motivation: 为了解决推理模型在长推理过程中计算资源浪费的问题，提高推理效率并动态控制推理长度。

Method: 在推理模型上训练轻量级适配器，预测未来奖励与思考令牌数量的关系，实现推理链的早期停止和动态长度控制。

Result: 减少26%计算量同时保持准确率；在相同计算量下准确率提高4%，在相同准确率下计算量减少55%；高计算模式下准确率提高11%，低计算模式下提高7%。

Conclusion: Re-FORC能够有效优化推理过程，实现计算效率与准确率的平衡，支持动态推理长度控制。
...
### [171] [Personalized Decision Modeling: Utility Optimization or Textualized-Symbolic Reasoning](https://arxiv.org/abs/2511.02194)
*Yibo Zhao,Yang Zhao,Hongru Du,Hao Frank Yang*

Main category: cs.AI

TL;DR: 提出了ATHENA框架，通过结合符号效用发现和语义适配来建模个性化决策，在旅行模式和疫苗选择任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 个体决策模型与群体最优预测存在差距，因为个体决策过程受到数值属性和语言影响的双重作用，需要开发能够整合最优信息的人类中心决策模型。

Method: ATHENA框架包含两个阶段：1）通过LLM增强的符号发现找到鲁棒的群体级符号效用函数；2）实施个体级语义适配，创建由最优效用指导的个性化语义模板来建模个性化选择。

Result: 在真实世界的旅行模式和疫苗选择任务中，ATHENA持续优于基于效用、机器学习和其他基于LLM的模型，F1分数比最强前沿模型至少提升6.5%。消融研究证实两个阶段都至关重要且互补。

Conclusion: 通过有机整合符号效用建模和语义适配，ATHENA为建模人类中心决策提供了新方案。
...
### [172] [Optimal-Agent-Selection: State-Aware Routing Framework for Efficient Multi-Agent Collaboration](https://arxiv.org/abs/2511.02200)
*Jingbo Wang,Sendong Zhao,Haochun Wang,Yuzheng Fan,Lizhe Zhang,Yan Liu,Ting Liu*

Main category: cs.AI

TL;DR: STRMAC是一个状态感知路由框架，通过分别编码交互历史和代理知识来驱动路由器，自适应选择最适合的单个代理，实现多代理系统的高效协作。
...
Method: 提出STRMAC框架，分别编码交互历史和代理知识来驱动路由器，自适应选择最适合的代理；引入自演进数据生成方法加速高质量执行路径的收集。

Result: 在协作推理基准测试中达到最先进性能，比基线提升23.8%，与穷举搜索相比减少90.1%的数据收集开销。

Conclusion: STRMAC通过状态感知路由和自演进数据生成，显著提升了多代理系统的协作效率和性能。
...
### [173] [Training Proactive and Personalized LLM Agents](https://arxiv.org/abs/2511.02208)
...
Result: 在软件工程和深度研究任务上，PPP训练的智能体相比GPT-5等强基线平均提升21.6%，能够提出战略性澄清问题、适应未见过的用户偏好，并通过更好的交互提高任务成功率。
...
### [174] [TabDSR: Decompose, Sanitize, and Reason for Complex Numerical Reasoning in Tabular Data](https://arxiv.org/abs/2511.02219)
*Changjiang Jiang,Fengchang Yu,Haihua Chen,Wei Lu,Jin Zeng*

Main category: cs.AI

TL;DR: 提出了一个名为\method的框架，用于增强大语言模型在复杂表格数值推理任务上的性能。该框架包含查询分解器、表格清理器和基于程序思维(PoT)的推理器，并在新数据集CalTab151上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在表格数据复杂推理中的性能不足问题，包括复杂查询处理、噪声数据清理和数值能力限制等挑战。

Method: 框架包含三个组件：(1)查询分解器将复杂问题分解为子问题；(2)表格清理器清理和过滤噪声表格；(3)基于程序思维(PoT)的推理器生成可执行代码从清理后的表格中推导最终答案。
...
### [175] [Deep Ideation: Designing LLM Agents to Generate Novel Research Ideas on Scientific Concept Network](https://arxiv.org/abs/2511.02238)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究想法生成方法主要依赖简单的关键词共现或语义相似度，忽视了科学概念间的复杂上下文关系，且未能有效利用科学概念网络来支撑想法的基础。

Method: 提出Deep Ideation框架，整合科学网络捕捉关键词共现和上下文关系，采用探索-扩展-演化的迭代工作流程，使用想法栈跟踪进度，并引入基于真实评审反馈训练的批评引擎提供持续反馈。

Result: 实验显示该方法相比其他方法提升了10.67%的想法质量，生成的想法超过了顶级会议接受水平，人类评估确认了其在科学研究中的实用价值。

Conclusion: Deep Ideation框架通过有效整合科学概念网络和LLM驱动的方法，显著提升了研究想法的生成质量，证明了各组件在工作流程中的有效性。
...
### [176] [When Modalities Conflict: How Unimodal Reasoning Uncertainty Governs Preference Dynamics in MLLMs](https://arxiv.org/abs/2511.02243)
*Zhuoran Zhang,Tengyue Wang,Xilin Gong,Yang Shi,Haotian Wang,Di Wang,Lijie Hu*

Main category: cs.AI

TL;DR: 提出了一个分解多模态跟随行为的框架，将相对推理不确定性和内在模态偏好作为两个基本因素，揭示了模态跟随概率随相对不确定性单调下降的普遍规律。


<details>
  <summary>Details</summary>
Motivation: 现有工作仅用粗粒度的数据集级统计来衡量多模态大语言模型在模态冲突时的行为，忽略了模型在单模态推理中的置信度影响。

Method: 构建可控数据集系统变化视觉和文本输入的推理难度，使用熵作为细粒度不确定性度量，分析层间预测揭示内部机制。

Result: 发现模态跟随概率随相对不确定性单调下降的普遍规律，在平衡点处揭示了模型的固有偏好，并发现模型在模糊区域会在不同层间在模态间摇摆。

Conclusion: 相对不确定性和内在偏好是控制模态跟随的两个基本原则，为理解MLLMs如何解决冲突信息提供了量化框架和机制性洞见。
...
### [177] [Unlocking the Power of Multi-Agent LLM for Reasoning: From Lazy Agents to Deliberation](https://arxiv.org/abs/2511.02303)
...
TL;DR: 本文分析了多智能体推理中的懒惰行为问题，提出了因果影响度量和可验证奖励机制来缓解该问题，从而提高多智能体协作在复杂推理任务中的效果。


<details>
  <summary>Details</summary>
Motivation: 在多智能体推理设置中，经常出现一个智能体主导而另一个智能体贡献很少的懒惰行为，这破坏了协作效果，使多智能体设置退化为无效的单智能体系统。

Method: 1. 提供理论分析解释懒惰行为的自然产生原因；2. 引入稳定高效的因果影响度量方法；3. 提出可验证奖励机制，允许推理智能体丢弃噪声输出、整合指令并在必要时重启推理过程。

Result: 广泛的实验表明，该框架能够缓解懒惰智能体行为，释放多智能体框架在复杂推理任务中的全部潜力。
...
### [178] [Chronic Kidney Disease Prognosis Prediction Using Transformer](https://arxiv.org/abs/2511.02340)
*Yohan Lee,DongGyun Kang,SeHoon Park,Sa-Yoon Park,Kwangsoo Kim*

Main category: cs.AI

TL;DR: 提出了一个基于Transformer的框架ProQ-BERT，用于预测慢性肾病进展，整合多模态电子健康记录，在91,816名患者队列中表现优于现有方法。
...
Method: 使用基于量化的标记化处理连续实验室数值，采用注意力机制提高可解释性，通过掩码语言建模进行预训练，并针对不同随访和评估期进行二元分类微调。

Result: 在91,816名患者队列中，模型持续优于CEHR-BERT，短期预测ROC-AUC高达0.995，PR-AUC高达0.989。
...
### [179] [Fuzzy Soft Set Theory based Expert System for the Risk Assessment in Breast Cancer Patients](https://arxiv.org/abs/2511.02392)
*Muhammad Sheharyar Liaqat*

Main category: cs.AI

TL;DR: 开发了一个基于模糊软集理论的专家系统，使用BMI、胰岛素水平、瘦素水平、脂联素水平和年龄等临床参数来评估乳腺癌风险。
...
Result: 系统使用UCI机器学习存储库的数据集进行开发和验证，能够通过常规血液分析获得参数，提供无创且易于获取的初步评估方法。
...
### [180] [A New Perspective on Precision and Recall for Generative Models](https://arxiv.org/abs/2511.02414)
*Benjamin Sykes,Loïc Simon,Julien Rabin,Jalal Fadili*

Main category: cs.AI

TL;DR: 提出基于二元分类的新框架来估计生成模型的完整PR曲线，解决了现有方法局限于曲线极值点的问题，并进行了统计分析和实验验证。


<details>
  <summary>Details</summary>
Motivation: 生成模型评估方法大多依赖标量指标，而PR曲线能提供更丰富的分析，但现有估计方法面临挑战且主要关注曲线极值点。

Method: 基于二元分类视角构建PR曲线估计框架，进行统计理论分析并获得极小极大上界，扩展了文献中的经典PR指标。

Result: 建立了完整的PR曲线估计方法，理论上证明了估计风险的上界，实验验证了不同设置下曲线行为的差异。

Conclusion: 该框架为生成模型评估提供了更全面的PR曲线分析工具，克服了现有方法的局限性，具有理论和实用价值。
...
### [181] [ReAcTree: Hierarchical LLM Agent Trees with Control Flow for Long-Horizon Task Planning](https://arxiv.org/abs/2511.02424)
*Jae-Woo Choi,Hyungmin Kim,Hyobin Ong,Minsu Jang,Dohyung Kim,Jaehong Kim,Youngwoo Yoon*

Main category: cs.AI

TL;DR: ReAcTree是一种分层任务规划方法，通过动态构建代理树将复杂目标分解为更易管理的子目标，结合推理、行动和记忆系统，显著提升了复杂长时程任务的执行效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理复杂长时程任务时，依赖单一轨迹来整合所有过去的决策和观察，试图在一个统一过程中解决整个任务，这导致性能受限。

Method: 提出ReAcTree分层任务规划方法，将复杂目标分解为子目标，构建动态代理树，每个代理节点负责推理、行动和树扩展，控制流节点协调执行策略，并集成情景记忆和工作记忆系统。

Result: 在WAH-NL和ALFRED数据集上的实验表明，ReAcTree在多种LLM上始终优于ReAct等强基线方法，在WAH-NL上使用Qwen 2.5 72B实现了61%的目标成功率，几乎是ReAct（31%）的两倍。

Conclusion: ReAcTree通过分层分解和动态代理树结构，有效解决了复杂长时程任务规划问题，显著提升了自主代理的决策能力和任务执行效果。
...
### [182] [Auditable-choice reframing unlocks RL-based verification for open-ended tasks](https://arxiv.org/abs/2511.02463)
*Mengyu Zhang,Xubo Liu,Siyu Ding,Weichong Yin,Yu Sun,Hua Wu,Wenya Guo,Ying Zhang*

Main category: cs.AI

TL;DR: 提出VMR方法，将开放式任务转化为可验证的多选格式，使RLVR范式能够应用于缺乏标准答案的开放式任务，显著提升LLM性能。


<details>
  <summary>Details</summary>
Motivation: RLVR在数学和编程等有标准答案的领域表现出色，但在缺乏标准答案的开放式任务（如创意写作和指令遵循）中被视为非推理场景，忽略了推理能力的潜在价值。

Method: 引入可验证多选重构(VMR)训练策略，将开放式数据重新构建为可验证的多选格式，从而在缺乏明确标准答案的情况下实现有效训练。

Result: 在多个基准测试上的实验结果表明该方法有效提升了LLM在开放式任务上的性能，在八个开放式基准测试中平均比基线提升了5.99分。

Conclusion: VMR方法成功将RLVR范式扩展到开放式领域，证明了强化推理能力能够显著改善开放式任务的性能。
...
### [183] [Agentic AI for Mobile Network RAN Management and Optimization](https://arxiv.org/abs/2511.02532)
*Jorge Pellejero,Luis A. Hernández Gómez,Luis Mendo Tomás,Zoraida Frias Barroso*

Main category: cs.AI

TL;DR: 本文提出了Agentic AI在5G/6G网络中的应用框架，通过大型AI模型实现自主决策和优化，特别关注RAN管理的实际用例。


<details>
  <summary>Details</summary>
Motivation: 5G/6G网络的复杂性使得手动优化效率低下，需要Agentic AI来自动化动态RAN环境中的决策，但目前缺乏统一框架和定义。

Method: 介绍Agentic AI核心概念，包括反思、规划、工具使用和多智能体协作等设计模式，并通过5G RAN案例研究展示时间序列分析和LAM驱动智能体的协作。

Result: 提出了Agentic AI在移动网络中的理论基础，并展示了如何将智能行为编排应用于RAN管理和优化的实际场景。

Conclusion: Agentic AI为5G/6G网络优化提供了新的自动化范式，通过结合大型AI模型和多智能体协作，能够实现基于KPI的自主决策。
...
### [184] [Knowledge Graph-enhanced Large Language Model for Incremental Game PlayTesting](https://arxiv.org/abs/2511.02534)
*Enhong Mu,Jinyu Cai,Yijun Lu,Mingyue Zhang,Kenji Tei,Jialong Li*

Main category: cs.AI

TL;DR: 提出了KLPEG框架，通过知识图谱建模游戏元素和依赖关系，结合LLM解析更新日志，实现针对游戏增量更新的精准自动化测试。
...
### [185] [The ORCA Benchmark: Evaluating Real-World Calculation Accuracy in Large Language Models](https://arxiv.org/abs/2511.02589)
*Claudia Herambourg,Dawid Siuda,Anna Szczepanek,Julia Kopczyńska,Joao R. L. Santos,Wojciech Sas,Joanna Śmietańska-Nowak*

Main category: cs.AI

TL;DR: ORCA基准测试评估了5个最先进的大语言模型在500个跨领域定量推理任务上的表现，结果显示模型准确率仅为45-63%，主要错误来自舍入(35%)和计算错误(33%)。


<details>
  <summary>Details</summary>
Motivation: 创建ORCA基准是为了评估大语言模型在多领域、真实生活中的定量推理能力，使用经过验证的计算器输出作为标准。

Method: 使用Omni计算器引擎验证的输出来评估模型在金融、物理、健康、统计等领域的500个自然语言任务中的表现。

Result: 五个最先进系统(ChatGPT-5, Gemini 2.5 Flash, Claude Sonnet 4.5, Grok 4, DeepSeek V3.2)准确率仅为45-63%，在数学和工程领域表现较好，但在物理和自然科学领域较弱。

Conclusion: 大语言模型在定量推理任务中存在显著的计算和舍入错误，模型之间存在部分互补性而非冗余性，需要改进数值精度和领域泛化能力。
...
### [186] [Adaptive GR(1) Specification Repair for Liveness-Preserving Shielding in Reinforcement Learning](https://arxiv.org/abs/2511.02605)
...
Method: 使用GR(1)规范作为基础，运行时检测环境假设违反，并利用归纳逻辑编程(ILP)在线自动修复GR(1)规范。

Result: 在Minepump和Atari Seaquest案例中，自适应屏蔽相比静态屏蔽能保持接近最优的奖励和完美的逻辑合规性。

Conclusion: 自适应屏蔽框架能够优雅地演化，确保活性可达成，只在必要时弱化目标，显著优于静态屏蔽方法。
...
### [187] [A Multi-Agent Psychological Simulation System for Human Behavior Modeling](https://arxiv.org/abs/2511.02606)
...
Method: 基于心理学理论（自我效能、思维模式、社会建构主义等）构建多智能体系统，模拟'内部议会'中不同心理因素的智能体进行审议和互动。

Result: 系统能够生成可信的人类行为，提供前所未有的透明度和与人类心理学的对齐。

Conclusion: 该系统体现了社会学习、认知学徒制、刻意练习和元认知等原则，在教师培训和研究中有重要应用价值。
...
### [188] [DecompSR: A dataset for decomposed analyses of compositional multihop spatial reasoning](https://arxiv.org/abs/2511.02627)
*Lachlan McPheat,Navdeep Kaur,Robert Blackwell,Alessandra Russo,Anthony G. Cohn,Pranava Madhyastha*

Main category: cs.AI

TL;DR: DecompSR是一个用于分析组合空间推理能力的大型基准数据集和生成框架，包含超过500万个数据点，可独立控制组合性的多个方面。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在空间推理任务中缺乏对组合性能力的系统性评估，需要开发能够独立分析生产力、可替代性、过度泛化和系统性等组合性关键方面的基准测试。

Method: 通过程序化生成方法构建数据集，确保构造正确性，并使用符号求解器独立验证数据集的正确性。数据集可独立控制推理深度、实体和语言变异性、输入顺序和干扰项、新颖语言元素等组合性维度。

Result: 基准测试显示，大型语言模型在空间推理任务中难以进行生产性和系统性泛化，但对语言变异性表现出更强的鲁棒性。

Conclusion: DecompSR提供了一个可证明正确且严格的基准数据集，能够独立控制组合性的多个关键维度，为LLMs的组合推理能力提供精细化的评估工具。
...
### [189] [The Collaboration Gap](https://arxiv.org/abs/2511.02687)
*Tim R. Davidson,Adam Fourney,Saleema Amershi,Robert West,Eric Horvitz,Ece Kamar*

Main category: cs.AI

TL;DR: 本文提出了一个协作迷宫求解基准测试，发现AI模型在单独工作时表现良好，但在协作时会出现显著的性能下降，称为"协作鸿沟"。通过"接力推理"方法可以改善协作效果。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统越来越多地由独立开发的异构智能体组成，这些系统的成功关键取决于智能体间的有效协作。然而，目前缺乏大规模评估智能体协作能力的实证研究。

Method: 设计了一个协作迷宫求解基准测试框架，该框架能够：(1)隔离协作能力，(2)调节问题复杂度，(3)支持可扩展的自动评分，(4)保持生态合理性。评估了32个领先的开源和闭源模型在单独、同质和异质配对中的表现。

Result: 发现了明显的"协作鸿沟"：单独表现良好的模型在需要协作时性能显著下降。协作可能完全崩溃，例如小型蒸馏模型单独能很好解决迷宫，但在某些配对中几乎完全失败。从较强智能体开始通常能改善结果，"接力推理"方法可以显著缩小协作鸿沟。

Conclusion: 研究结果主张：(1)协作感知的评估方法，(2)开发增强协作能力的训练策略，(3)设计能可靠激发智能体潜在技能的交互方式。这些指导原则适用于AI-AI协作和人类-AI协作。
...
### [190] [CostBench: Evaluating Multi-Turn Cost-Optimal Planning and Adaptation in Dynamic Environments for LLM Tool-Use Agents](https://arxiv.org/abs/2511.02734)
*Jiayu Liu,Cheng Qian,Zhaochen Su,Qing Zong,Shijue Huang,Bingxiang He,Yi R. Fung*

Main category: cs.AI

TL;DR: CostBench是一个专注于成本优化的基准测试，用于评估LLM代理在经济推理和重新规划能力方面的表现，发现在静态和动态环境下代理都难以找到成本最优解。
...
Result: 评估显示代理在成本感知规划方面存在显著差距：在静态设置中经常无法找到成本最优解（GPT-5在最难任务上准确率低于75%），在动态条件下性能进一步下降约40%。
...
### [191] [Using Span Queries to Optimize for Cache and Attention Locality](https://arxiv.org/abs/2511.02749)
*Paul Castro,Nick Mitchell,Nathan Ordonez,Thomas Parnell,Mudhakar Srivatsa,Antoni Viros i Martin*

Main category: cs.AI

TL;DR: 本文提出了span query概念，将推理服务器接口泛化，支持聊天、RAG、推理时缩放和智能体工作负载，通过优化KV缓存和注意力局部性实现10-20倍TTFT降低。
...
Method: 引入span query作为推理调用的表达式树，通过交换性约束连接，自动优化KV缓存局部性，并对vLLM进行少量修改以支持高性能执行。

Result: span query在两种不同的非聊天用例中实现了10-20倍的TTFT降低，注意力优化的span query在2b参数模型上显著优于8b模型的准确率。

Conclusion: span query提供了通用的推理服务器接口，能够有效支持多样化的工作负载，并通过优化显著提升性能和准确率。
...
### [192] [LLM-Supported Formal Knowledge Representation for Enhancing Control Engineering Content with an Interactive Semantic Layer](https://arxiv.org/abs/2511.02759)
...
Result: 生成了"交互式语义层"来增强源文档，促进知识传递。
...
### [193] [When One Modality Sabotages the Others: A Diagnostic Lens on Multimodal Reasoning](https://arxiv.org/abs/2511.02794)
*Chenyu Zhang,Minsol Kim,Shohreh Ghorbani,Jingyao Wu,Rosalind Picard,Patricia Maes,Paul Pu Liang*

Main category: cs.AI

TL;DR: 提出了一种轻量级、模型无关的评估框架来分析多模态大语言模型的推理过程，通过将每个模态视为代理来识别贡献者和破坏者，揭示融合动态。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的推理过程不透明，难以理解哪个模态驱动预测、如何解决冲突或何时一个流主导，存在模态破坏的诊断失败模式。

Method: 将每个模态视为代理，生成候选标签和简要自我评估，通过简单融合机制聚合这些输出，识别支持正确结果的贡献者和误导的破坏者。

Result: 在多模态情感识别基准测试中应用该诊断层，揭示了系统性的可靠性特征，帮助区分失败是源于数据集伪影还是模型限制。
...
### [194] [Orion-MSP: Multi-Scale Sparse Attention for Tabular In-Context Learning](https://arxiv.org/abs/2511.02818)
*Mohamed Bouadi,Pratinav Seth,Aditya Tanna,Vinay Kumar Sankarapu*

Main category: cs.AI

TL;DR: Orion-MSP是一种表格数据上下文学习架构，通过多尺度处理、块稀疏注意力和感知器风格内存解决了现有方法的局限性，在保持高性能的同时实现了对高维表格的有效扩展。


<details>
  <summary>Details</summary>
Motivation: 解决表格数据神经网络建模的挑战：特征类型异构、多尺度复杂交互，以及现有架构在单尺度处理、密集注意力二次扩展和顺序组件处理方面的局限性。

Method: 引入三个关键创新：多尺度处理捕获层次特征交互；块稀疏注意力结合窗口化、全局和随机模式实现可扩展效率；感知器风格内存实现组件间安全双向信息流。
...
### [195] [Optimizing AI Agent Attacks With Synthetic Data](https://arxiv.org/abs/2511.02823)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI部署变得复杂且高风险，评估其风险变得至关重要。AI控制框架需要强大的攻击策略，但在复杂环境中由于计算限制导致数据不足，这带来了挑战。

Method: 将攻击能力分解为五个组成技能（怀疑建模、攻击选择、计划合成、执行和隐蔽性），分别优化每个组件；开发攻击动态的概率模型，在模拟中优化超参数，然后将结果迁移到SHADE-Arena环境。
...
### [196] [Kosmos: An AI Scientist for Autonomous Discovery](https://arxiv.org/abs/2511.02824)
...
TL;DR: Kosmos是一个AI科学家系统，能够自动化数据驱动的科学发现过程，通过结构化世界模型连接数据分析代理和文献搜索代理，在长达12小时的运行中执行数万行代码并阅读大量论文，生成可追溯的科学报告。


<details>
  <summary>Details</summary>
Motivation: 当前AI科研代理在采取多次行动后会失去连贯性，限制了发现深度。需要开发能够长期保持连贯性、自动化科学研究的AI系统。

Method: 使用结构化世界模型在数据分析代理和文献搜索代理之间共享信息，执行并行数据分析、文献搜索和假设生成的循环，最多运行12小时和200次代理部署。

Result: Kosmos在单次20循环运行中平均完成相当于6个月人工研究的工作量，报告准确性达79.4%，并在代谢组学、材料科学等领域做出了7项发现，其中3项独立重现了未发表成果，4项是新颖贡献。

Conclusion: Kosmos证明了AI科学家系统能够规模化自动化科学发现过程，其发现能力随循环次数线性增长，为数据驱动研究提供了新的范式。
...
### [197] [Neurosymbolic Deep Learning Semantics](https://arxiv.org/abs/2511.02825)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI作为科学工具缺乏语义基础，使得科学发现难以被理解。需要建立框架将AI洞察转化为可理解的科学知识。

Method: 引入语义编码框架，明确神经网络与逻辑之间的映射关系，并分析现有神经编码和知识提取方法的共同要素。

Result: 建立了连接逻辑语义与神经网络的统一框架，为各种神经符号AI方法提供了理论基础。

Conclusion: 逻辑框架能为深度学习提供所需的语义基础，但实际应用中识别语义编码仍面临类似心灵哲学中的困难。
...
### [198] [Agent-Omni: Test-Time Multimodal Reasoning via Model Coordination for Understanding Anything](https://arxiv.org/abs/2511.02834)
*Huawei Lin,Yunzhi Shi,Tong Geng,Weijie Zhao,Wei Wang,Ravender Pal Singh*

Main category: cs.AI

TL;DR: 提出Agent-Omni框架，通过主代理系统协调现有基础模型，实现无需重新训练的灵活多模态推理


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型局限于固定模态对，需要大量对齐数据微调，构建全模态能力模型不切实际且缺乏鲁棒推理支持

Method: 采用主代理系统，主代理解释用户意图，将子任务分配给模态特定代理，并整合输出为连贯响应
...
### [199] [An Evaluation of Interleaved Instruction Tuning on Semantic Reasoning Performance in an Audio MLLM](https://arxiv.org/abs/2511.02234)
*Jiawei Liu,Enis Berk Çoban,Zarina Schevchenko,Hao Tang,Zhigang Zhu,Michael I Mandel,Johanna Devaney*

Main category: cs.MM

TL;DR: 研究了在音频多模态大语言模型中使用交错指令调优的效果，发现交错提示能提升语义推理性能但会降低音频标注能力
...
Method: 使用LTU模型作为测试平台，在SHARD音频推理基准上实验交错指令调优，将音频令牌交错在提示中

Result: 零样本交错提示能改善推理任务性能，少量交错训练提示的微调能进一步提升结果，但会牺牲模型的音频标注能力
...
### [200] [Wireless Video Semantic Communication with Decoupled Diffusion Multi-frame Compensation](https://arxiv.org/abs/2511.02478)
*Bingyan Xie,Yongpeng Wu,Yuxuan Shi,Biqian Feng,Wenjun Zhang,Jihong Park,Tony Quek*

Main category: cs.MM

TL;DR: 提出了一种无线视频语义通信框架WVSC-D，通过在语义层面而非像素层面进行视频编码，结合解耦扩散多帧补偿技术，显著提升了无线视频传输的带宽效率。
...
Conclusion: 该语义通信框架通过语义级编码和扩散补偿技术，在保证视频传输质量的同时显著提高了带宽效率。
