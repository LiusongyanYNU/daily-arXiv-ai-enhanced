...
Motivation: 当前文献倾向于采用为通用任务设计的大型视觉语言模型(VLMs)，这可能忽略了专门构建模型设定的性能上限，缺乏系统评估。

Method: 建立图像隐私分类的零样本基准测试，使用任务对齐的提示评估前3名开源VLMs，并将其性能、效率和鲁棒性与已建立的仅视觉和多模态方法进行对比。

Result: VLMs尽管参数数量多且推理速度慢，但在隐私预测准确性上落后于专门的较小模型；但VLMs对图像扰动表现出更高的鲁棒性。

Conclusion: 视觉语言模型在图像隐私预测任务中尚未超越专门的较小模型，但在鲁棒性方面表现更好，这为未来模型设计提供了重要启示。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在专业视觉任务中的潜力未被充分挖掘，直接使用往往表现不佳。研究旨在探索如何通过少量数据微调LLM，使其在视觉理解任务中发挥更大作用。

Method: 对传统CNN、零样本预训练多模态LLM和微调多模态LLM进行综合比较，重点研究LLM在少于1000张图像数据上的微调效果。

Result: 微调后的LLM在人工文本叠加检测任务上实现了高达36%的准确率提升，性能匹配或超越了需要大量数据的CNN基线模型。

Conclusion: LLM方法具有出色的适应性和数据效率，为低资源视觉环境中的多模态Transformer应用提供了实用指导，证明了跨模态学习策略的有效性。
...
Method: 在公开MRI数据集上实验，使用U-Net架构配合焦点损失函数，评估三种数据增强技术：水平翻转、旋转和缩放。

Result: 使用焦点损失的U-Net达到了90%的精确度，与最先进结果相当。

Conclusion: 通过公开所有代码和结果，本研究为脑肿瘤分割中的数据增强策略和损失函数设计提供了透明、可复现的基准线。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型经常记忆和复制训练数据，引发隐私和版权担忧。现有方法延迟应用分类器无关指导(CFG)会导致图像与提示对齐不佳，需要促进早期逃逸以更早应用CFG。

Method: 通过实证观察发现初始噪声样本决定逃逸时间，提出两种缓解策略：集体或个体调整初始噪声，寻找能促进早期逃逸的初始样本。

Result: 这些方法显著减少了记忆化，同时保持了图像与文本的对齐。

Conclusion: 初始噪声在决定记忆化逃逸时间中起关键作用，调整初始噪声是减少扩散模型记忆化的有效策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注AI生成可视化内容的创建过程和质量，忽视了代表性偏见问题，特别是在职业场景中的性别刻板印象。

Method: 生成了750多张AI职业图像，使用DALL-E 3和Ideogram工具，通过主题分析评估性别偏见。

Result: 两个AI图像生成工具都强化了传统性别刻板印象，尽管程度不同。

Conclusion: AI可视化工具存在强化狭隘表征的风险，需要采取措施确保生成图像中的多样化代表性。
...
Result: 减少20% FLOPs，加速11%推理时间，同时保持与密集基线相当的图像质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有的无监督OOD检测方法通常依赖预测熵，但这种方法混淆了认知不确定性和偶然不确定性，导致将分布内模糊区域误分类为OOD。需要一种能够准确分离不确定性的方法。

Method: 使用分层贝叶斯建模对深度神经网络特征空间中的高斯混合模型参数进行建模，从中提取认知不确定性用于OOD检测，无需辅助数据集或额外训练阶段。

Result: 在SemanticKITTI数据集上显著优于现有基于不确定性的方法，AUROC提升18%，AUPRC提升22%，FPR95从76%降低到40%（减少36%）。

Conclusion: 该方法通过分离认知不确定性有效解决了传统预测熵方法混淆不确定性的问题，在无监督OOD检测中取得了显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 人类活动识别中存在训练数据中标注活动与实际生活中活动范围之间的巨大差距，且活动类别间存在不同程度的相似性和层次关系，需要处理未知活动以提升分类器可靠性。

Method: 将活动类别组织成结构化层次，开发Hi-OSCAR分层开放集分类器，能够识别已知活动并拒绝未知活动，同时将未知类别定位到最近的内部节点。

Result: Hi-OSCAR能够以最先进准确率识别已知活动，同时有效拒绝未知活动，并提供层次化的未知活动定位能力。

Conclusion: 分层开放集方法能够有效解决HAR中的未知活动问题，新收集的NFI_FARED数据集为开放集HAR研究提供了有价值的资源。
...
<details>
  <summary>Details</summary>
Motivation: 高频振荡是识别癫痫发生区的新生物标志物，但视觉识别耗时且主观，需要开发自动化检测方法用于临床和研究。

Method: 使用S变换从时频域提取事件，采用无监督聚类技术对事件进行分类，区分高频振荡、尖峰、背景活动和伪迹。

Result: 在受控数据集上达到97.67%灵敏度、98.57%精确度和97.78% F分数；在癫痫患者中，切除与非切除接触点的HFOs比率达0.73，与手术结果相关性更强。

Conclusion: HFOs是癫痫患者癫痫发生性的有前景生物标志物，切除HFOs（特别是快速波纹）可实现无癫痫发作，而残留HFOs则导致癫痫复发。
...
<details>
  <summary>Details</summary>
Motivation: 尽管DINOv2被广泛用于物体、场景和动作识别，但其内部感知机制仍然未知。研究旨在理解该模型如何表示视觉概念，并超越传统的线性表示假说。

Method: 采用线性表示假说作为基线，使用稀疏自编码器（SAE）学习32,000个单元的字典，分析不同下游任务如何利用这些概念，并研究SAE学习概念的几何和统计特性。

Result: 发现分类任务利用'Elsewhere'概念实现学习否定；分割依赖边界检测器形成连贯子空间；深度估计使用三种单眼深度线索。表示部分稠密而非严格稀疏，字典趋向更大连贯性，token占据低维局部连接集。

Conclusion: 提出Minkowski表示假说，认为token是通过原型的凸组合形成的，这种结构基于Gardenfors的概念空间理论，多头部注意力机制产生凸组合的和，定义了由原型界定的区域。
...
Method: 采用两阶段级联架构，包含残差流形投影器(RMP)和频率感知退化分解器(FADD)进行退化特征分析，结合物理感知专家模块和温度控制稀疏激活策略。

Result: 在三个基准数据集上的实验表明，PhyDAE在所有四个修复任务中均取得最优性能，同时显著减少参数数量和计算复杂度，实现性能与效率的最佳平衡。

Conclusion: PhyDAE通过显式建模退化物理和自适应专家机制，有效解决了遥感图像多退化修复问题，在性能和效率方面均优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现实临床决策需要整合多种数据模态（医学文本、2D/3D图像、视频），但现有方法存在流程不透明、数据稀缺和架构不灵活等问题。

Method: 基于统一的基于补丁的视觉编码器和LLM解码器构建，通过渐进式训练在1670万个样本上从2D扩展到3D和视频理解，采用医学感知的token减少技术实现高效训练。

Result: 在30个基准测试中表现优异，超越领先的开源模型，在视觉问答、医学报告生成以及多语言和罕见疾病场景的复杂推理任务中与专有系统竞争。
...
<details>
  <summary>Details</summary>
Motivation: 相机中心的理解和生成是空间智能的两个基石，但通常被孤立研究。需要统一的模型来整合这两种能力

Method: 提出将相机视为语言的新范式，结合语言回归和基于扩散的生成方法，使用包含400万视觉-语言-相机三元组的大规模数据集进行训练

Result: 实验表明Puffin在相机中心生成和理解任务上优于专用模型，并能泛化到空间想象、世界探索和摄影指导等跨视角任务

Conclusion: Puffin通过统一的相机中心方法推进了多模态空间智能研究，将发布代码、模型、数据集和基准
...
Method: SOR框架冻结网络内部结构（如卷积滤波器），同时使用组套索和L1惩罚的组合，仅需极少的额外参数即可针对特定数据定制模型。

Result: 在三个少样本医学图像分类任务中，使用DenseNet121和EfficientNetB4作为基础网络，SOR取得了与现有基准方法竞争性的结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注特定领域如规划或空间理解，缺乏对MLLMs具身能力的全面系统评估，需要建立一个更全面的评估框架。

Method: 提出BEAR基准，包含4,469个跨14个领域的图像-视频-文本条目，涵盖从低级指向、轨迹理解、空间推理到高级规划的6个类别。同时提出BEAR-Agent，集成预训练视觉模型来增强MLLM的感知、3D理解和规划能力。

Result: 对20个代表性MLLMs的评估显示它们在所有具身能力领域都存在持续局限性。BEAR-Agent在BEAR基准上显著提升了MLLM性能，实现了9.12%的绝对增益和17.5%的相对改进。

Conclusion: BEAR基准揭示了MLLMs在具身能力方面的系统性不足，BEAR-Agent通过增强感知和规划能力有效弥补了这些不足，且改进具身能力有益于模拟环境中的具身任务。
...
<details>
  <summary>Details</summary>
Motivation: 传统对抗防御方法依赖计算密集型优化，而人类视觉系统通过进化机制天然具备对抗扰动的鲁棒性，因此研究生物启发的防御机制。

Method: 结合三种生物机制：中央凹-外周处理、眼跳运动、皮层填充，使用强化学习引导眼跳选择性捕获多个中央凹-外周图像片段，整合重建后进行分类。

Result: 在ImageNet数据集上，该方法提升了多种分类器和攻击类型下的系统鲁棒性，同时显著降低了训练开销。
...
<details>
  <summary>Details</summary>
Motivation: 开发能够在不同环境下实时检测泄漏的安全关键系统，利用热成像技术提升检测性能和鲁棒性。

Method: 使用平衡的二元数据集（4000张图像），采用预训练的深度学习模型（如VGG19、NasNetMobile）进行RGB和热成像对比实验。

Result: 热成像模型在推理速度（最低44ms）、准确性（最高100%）和模型大小（小于350MB）方面均优于RGB模型，VGG19热成像模型表现最佳。

Conclusion: 基于热成像的轻量级深度学习模型在泄漏检测中具有显著优势，可在消费级硬件上实现高效部署，适用于安全关键应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 生成式图像超分辨率模型依赖自注意力的二次复杂度(O(N^2))造成计算瓶颈，线性注意力提供O(N)解决方案但存在训练不稳定、感知-失真权衡等未解决问题。

Method: 使用基于"拐点"的早停引导微调(ESGF)解决训练不稳定问题；采用SNR-based混合专家(MoE)架构缓解感知-失真权衡；建立轻量级TAG引导范式。

Result: LinearSR同时提供SOTA感知质量和卓越效率，核心扩散前向传播(1-NFE)达到SOTA级速度，整体多步推理时间保持高度竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 传统野生动物监测方法（如鸟类腿环标记）耗时且具有侵入性，需要开发基于AI的非侵入性高效替代方案。

Method: 结合YOLO和Grounding DINO进行目标检测，使用光流模糊检测、DINOv2图像编码和聚类方法，从定制喂食器的视频中提取代表性关键帧。

Result: 提出的关键帧选择方法在kākā重识别任务中实现了高准确率，为在更具挑战性环境中收集的媒体数据研究奠定了基础。

Conclusion: 这种基于AI和计算机视觉的非侵入性方法为野生动物监测提供了有价值的替代方案，有助于生态学和保护生物学的新方法发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有VQA模型存在三个主要问题：(1)在不同内容和任务间泛化能力差，(2)可解释性有限，(3)难以扩展到新的用例或内容类型。需要开发一个更通用、可解释且可扩展的VQA系统。
...
Conclusion: Q-Router通过智能路由机制结合专家模型的互补优势，在异构视频源和任务中实现了灵活性和鲁棒性，有望成为下一代VQA系统的基础，并可作为视频生成模型后训练中的奖励函数。
...
Method: 1) 使用对比学习和最优传输理论实现多层次模态对齐；2) 采用软标签进行硬负样本挖掘；3) 设计门控交叉注意力模块整合答案词汇作为先验知识。

Result: 在RAD-VQA、SLAKE、PathVQA和VQA-2019等常用Med-VQA数据集上超越了之前的最先进方法。

Conclusion: 该框架通过统一的模态对齐、有效的硬负样本处理和选择性知识融合，显著提升了医学视觉问答的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的视频超分辨率方法处理所有像素，但许多区域本质上是低细节的，从细化中获益甚微，导致计算效率低下

Method: 直接从低分辨率输入中识别低细节区域，完全跳过这些区域的计算，只对需要细化的区域进行超分辨率处理

Result: 在标准SR基准测试中，相比先前模型在720p视频上实现了高达60%的端到端延迟减少，且没有可感知的质量损失

Conclusion: SkipSR提供了一种简单而有效的策略，在保持感知质量的同时显著加速视频超分辨率处理
...
Method: 提出动态压缩（自适应选择代表性帧和内容感知的空间token聚合）和问题分解（将原始查询重构为子问题）两种训练免费的方法。

Result: 实验表明D-CoDe有效提升了各种基准测试中的视频理解能力，在具有挑战性的长视频基准上表现出色。

Conclusion: D-CoDe框架能够有效处理复杂的视频语言任务，为图像预训练模型向视频领域的扩展提供了可行方案。
...
Result: 在ScanNet200数据集上达到35.7的AP50分数，比之前方法快6.0到152.2倍
...
<details>
  <summary>Details</summary>
Motivation: 蔓越莓生长监测目前主要依赖人工，耗时费力。深度学习虽有潜力但面临高维特征难以解释和需要大量标注数据的问题。

Method: 使用自监督视觉变换器，通过时间回归和类别预测的双重预训练任务学习植物和果实外观的时间演化潜在空间。

Result: 构建了可解释的2D时间序列作物生长模型，能够预测生长趋势并区分不同蔓越莓品种的时间差异。

Conclusion: 该方法通用性强，可应用于其他作物和场景，并提供了包含8个品种、52次观测的新时间序列数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型难以同时表达概念家族内的层次结构（如狗⊆哺乳动物⊆动物）和跨概念家族的组合性（如"车里的狗"⊆狗,车），双曲空间虽能有效捕获层次结构但对组合性的表示能力不明确。

Method: PHyCLIP在双曲因子笛卡尔积上使用ℓ₁乘积度量，让概念家族内层次结构在单个双曲因子中出现，跨概念组合通过ℓ₁乘积度量捕获，类似于布尔代数。
...
<details>
  <summary>Details</summary>
Motivation: 分割模型在白盒设置下对对抗样本表现出显著脆弱性，但现有攻击方法在不同分割模型间的迁移性较差。复杂上下文依赖性和特征分布差距导致迁移成功率不理想。

Method: 将输入样本划分为多个局部区域，重新映射其语义信息生成多样化增强样本，用这些样本替代原始样本进行扰动优化，仅保留局部语义信息而非全局语义信息。

Result: 在PASCAL VOC和Cityscapes两个基准数据集、四种不同分割模型和三种骨干网络上进行的广泛实验表明，SegTrans显著提高了对抗迁移成功率，平均提升8.55%，计算效率提高超过100%。

Conclusion: SegTrans在不引入额外计算开销的情况下，显著改善了对抗样本在不同分割模型间的可迁移性，在迁移攻击成功率和计算效率方面均优于现有最先进方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有防御方法主要针对图像分类任务，难以直接应用于图像恢复这类生成任务。图像恢复具有连续、高维输出特性，需要保持空间一致性和细节，轻微扰动往往不足以阻止学生模型学习。

Method: ASVP方法在运行时对教师模型的内部特征图进行奇异值分解，放大前k个奇异值来注入结构化的高频扰动，破坏蒸馏所需的特征对齐。

Result: 在超分辨率、低光增强、水下增强、去雾和去雨五个图像恢复任务中，ASVP使学生模型的PSNR降低达4dB，SSIM降低60-75%，而对教师模型性能影响可忽略。

Conclusion: ASVP为开源图像恢复模型提供了一种实用的防御方案，能有效防止未经授权的知识蒸馏，相比现有方法具有更强和更一致的防御效果。
...
Method: 构建Ro-Bench基准测试，包含通过编辑风格、对象、背景及其组合生成的高质量、多样化且时间相关的反事实视频数据，评估了8个最新的视频MLLM模型。

Result: 当前模型在反事实视频内容上表现出显著性能下降，但通过反事实数据微调可使Ro-Bench性能提升21.73%，在MVBench数据集的20个任务上平均提升12.78%。

Conclusion: 反事实数据能有效增强MLLM的视频理解能力，强调了在视频理解任务中考虑模型鲁棒性的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 解决无人机动物健康监测中数据稀缺问题，特别是针对特定农场条件下的小型、遮挡或部分可见动物的场景特异性挑战，以及传统迁移学习方法因缺乏反映特定农场条件的大型数据集而失效的问题

Method: 对象聚焦数据增强框架：分割动物与背景，通过变换和基于扩散的合成技术创建真实多样的场景
...
<details>
  <summary>Details</summary>
Motivation: 现有大视觉语言模型在推理时缩放方面取得进展，但对视觉感知的影响尚不明确。当前模型的快速感知范式将视觉理解视为一次性输出，缺乏对底层感知过程的建模。

Method: 提出Perception-Time Scaling (PTS)范式，鼓励丰富token感知，将复杂感知问题分解为中间可处理子问题；结合强化学习技术实现感知与推理时缩放的协同。

Result: PTS显著提升感知精度，在DisTANCE基准上高精度性能从8.0%提升到64.7%；在域外任务上泛化良好；即使使用纯合成数据，与数学推理数据结合也能在推理和真实世界感知基准上获得一致提升。

Conclusion: PTS通过引入更多感知相关token并增加模型对图像token的关注，有效解决了当前LVLMs在视觉感知任务中的精度限制问题，为多模态模型的感知能力提升提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 在毫米波姿态估计中，稀疏信号和弱反射导致模型过度依赖统计先验而非传感器数据，这会降低手势和活动识别等下游任务的性能。

Method: 提出mmJoints框架，通过估计关节被感知的可能性及其预测位置的可靠性来增强预训练姿态估计器的输出，使偏差显式化。

Result: 在13种姿态估计设置下使用超过115,000个信号帧进行评估，mmJoints描述符估计错误率低于4.2%，关节位置精度提升达12.5%，活动识别性能提升达16%。

Conclusion: mmJoints通过显式表示姿态估计中的偏差，有效提高了系统的可解释性和下游任务性能，为毫米波姿态估计提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统检索方法准确性有限，现有多向量检索方法在图像对象对齐和细粒度图像片段冗余方面存在不足。

Method: 采用分层范式处理不同图像对象，利用跨层次相似性一致性和层次稀疏性最小化冗余匹配计算，并自动配置数据集参数。

Result: 实验显示HiMIR在准确性上有显著提升，同时计算量比现有MVR系统减少最多3.5倍。

Conclusion: HiMIR框架通过分层检索和效率优化，在多模态检索中实现了准确性和效率的双重提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型在复杂局部区域（特别是人手）生成准确细节方面仍存在困难，生成的手部常出现结构扭曲和不真实纹理，而手部质量评估研究被忽视，限制了相关下游任务性能。

Method: 构建HandPair数据集（48k高质量-低质量手部图像对），开发HandEval模型，结合多模态大语言模型的视觉理解能力和手部关键点先验知识，并构建人工标注测试集验证性能。

Result: HandEval比现有SOTA方法更符合人类判断，在图像生成和AIGC检测任务中分别显著提升了生成手部真实感和检测精度。

Conclusion: HandEval在下游应用中具有普遍有效性，为手部质量评估和相关应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: AI着色技术存在版权侵权风险，如未经授权对黑白漫画和电影进行着色并转售。目前缺乏有效的预防方法，需要开发保护视觉内容免受非法AI着色的防御机制。

Method: 提出Perception-Aware Chroma-Restrictive Perturbation (PAChroma)方法，通过优化不可察觉的扰动，使用拉普拉斯滤波器保持感知质量，并在优化过程中应用多样化输入变换以增强跨模型迁移性和对常见后处理（如压缩）的鲁棒性。

Result: 在ImageNet和Danbooru数据集上的实验表明，PAChroma能有效降低着色质量，同时保持视觉外观。该方法满足有效性、不可察觉性、迁移性和鲁棒性四个标准。

Conclusion: 这是保护视觉内容免受非法AI着色侵害的第一步，为生成媒体中的版权感知防御开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 自回归文本到图像模型由于顺序token-by-token解码过程导致推理速度缓慢，需要数千次模型前向传递来生成单张图像，存在效率问题。
...
Conclusion: SJD2框架成功地将去噪过程与Jacobi迭代结合，实现了自回归模型的并行token生成，在保持图像质量的同时显著提升了生成效率。
...
Method: 提出对抗扰动方法识别不确定视觉token，并在视觉编码器中间层的自注意力过程中屏蔽这些token，抑制其对视觉编码的影响。

Result: 实验表明该方法显著减少了大视觉语言模型中的物体幻觉，并能与其他现有技术协同工作。

Conclusion: 通过修改视觉编码器来识别和屏蔽不确定视觉token，可以有效缓解物体幻觉问题，且方法简单有效。
...
<details>
  <summary>Details</summary>
Motivation: 发现当前自回归图像生成模型中的采样问题，图像令牌相比文本令牌具有更低的信息密度和非均匀空间分布。

Method: 1) 基于令牌分布空间熵的动态温度控制，增强内容多样性、对齐准确性和结构一致性；2) 推测解码中的熵感知接受规则，以约85%的推理成本实现近乎无损的生成。

Result: 在多个基准测试和多样化自回归图像生成模型上的广泛实验证明了该方法在提高生成质量和采样速度方面的有效性和通用性。
...
<details>
  <summary>Details</summary>
Motivation: LiDAR标注通常存在噪声，这种噪声会降低分割精度，并在领域偏移下进一步放大，威胁系统可靠性。现有噪声标签学习方法在3D LiDAR分割中的领域泛化应用尚未充分探索。
...
Conclusion: DuNe框架在DGLSS-NL任务中展示了鲁棒的领域泛化能力，为带噪声标签的LiDAR语义分割提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有潜在扩散模型在医学图像转换中虽然高效，但可能丢失关键的像素级细节，特别是对于占据图像小部分的病灶区域，这会影响诊断可靠性。

Method: 在预训练的潜在扩散模型基础上，加入病灶感知的医学像素空间目标进行后训练，提升病灶区域的生成精度。

Result: 在817名急性缺血性卒中患者数据集上，该方法在从CT灌注扫描合成DWI和ADC图像时，显著提升了整体图像质量和病灶边界清晰度，优于现有图像转换模型。
...
<details>
  <summary>Details</summary>
Motivation: 柔性微电极植入大脑皮层具有挑战性，因为FME探针的可变形纤维状结构以及与关键生物组织的相互作用，需要仔细监控植入过程以确保可靠性和安全性。

Method: 利用机器人FME植入系统的显微摄像头，在四个检查点分别检查微针、FME探针、钩挂结果和植入点。采用预训练的视觉变换器，提出渐进粒度补丁特征采样方法解决不同位置的敏感度-容错度权衡问题，并从原始通用ViT特征中选择信噪比较高的特征通道。

Result: 使用植入系统收集的图像数据集验证了所提方法的有效性。

Conclusion: 提出的基于图像的异常检测框架能够有效监控柔性微电极植入过程，确保植入的可靠性和安全性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常显式组合多模态信息，但本文旨在将多模态知识嵌入到各个单模态网络中，使每个单模态网络都能获得性能提升。

Method: 为每个可用模态分配独立网络，通过"时空语义对齐"损失(SSA)和"焦点正则化参数"来强制网络协作学习共同语义和更好表示，避免负知识迁移。

Result: 实验结果表明，该框架提高了单模态网络的测试时识别准确率，并在多个动态手势识别数据集上达到了最先进的性能。

Conclusion: 提出的框架通过多模态知识嵌入有效提升了单模态网络的性能，为动态手势识别提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有法向量估计方法在建模细粒度几何结构方面存在不足，而现有的Mamba方法主要关注全局形状理解，对局部几何细节建模不够充分。

Method: 提出注意力驱动的层次特征融合方案自适应融合多尺度点云补丁特征，并设计补丁级状态空间模型通过状态动态将点云补丁建模为隐式超曲面。

Result: 在基准数据集上的广泛实验表明，该方法在准确性、鲁棒性和灵活性方面优于现有方法，消融研究验证了所提组件的贡献。

Conclusion: MambaH-Fit通过结合层次特征融合和状态空间建模，有效解决了点云法向量估计中的细粒度几何结构建模问题，取得了优异的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统的稀疏表示分类方法在特征提取和稀疏表示方面存在局限性，需要一种能够学习鲁棒深度特征并同时优化稀疏表示的综合方法。

Method: 构建包含卷积自编码器和全连接层的网络架构，自编码器学习深度特征，中间的全连接层负责寻找稀疏表示，然后将稀疏编码用于分类。

Result: 在三个不同数据集上的实验表明，该方法产生的稀疏表示比最先进的SRC方法获得了更好的分类结果。

Conclusion: 所提出的转导深度学习方法能够有效结合深度特征学习和稀疏表示，在分类任务中表现出优越性能，为稀疏表示分类提供了新的解决方案。
...
TL;DR: 提出GL-DT框架，通过时空特征融合和全局-局部协作检测策略提升无人机多目标跟踪性能，有效解决小目标检测和轨迹连续性等问题。


<details>
  <summary>Details</summary>
Motivation: 无人机在军事侦察、环境监测等领域的广泛应用对准确高效的多目标跟踪技术提出迫切需求，但复杂背景、小尺度目标、频繁遮挡等问题挑战现有方法。
...
TL;DR: 将密集的扩散变换器(DiT)转换为混合专家(MoE)结构，通过结构化稀疏化减少激活参数60%，同时保持原始性能，超越基于剪枝的方法。


<details>
  <summary>Details</summary>
Motivation: DiT在文本到图像生成中表现优异，但参数规模大导致推理开销高。现有参数压缩方法主要关注剪枝，但激进的剪枝会因模型容量减少而导致性能严重下降。

Method: 将DiT块中的前馈网络替换为MoE层，减少FFN中62.5%的激活参数；提出混合块(MoB)选择性激活DiT块；设计多步蒸馏流程，包括基于泰勒度量的专家初始化、带负载均衡的知识蒸馏和MoB优化的组特征损失。

Result: 将大型扩散变换器(如FLUX.1)转换为MoE结构，减少激活参数60%的同时保持原始性能，在广泛实验中超越基于剪枝的方法。
...
TL;DR: 提出了一种用于医学图像分析的多分支ConvNeXt架构，在COVID-19 CT扫描诊断中表现出色，ROC-AUC达到0.9937，验证准确率97.57%，F1分数0.9825。


<details>
  <summary>Details</summary>
Motivation: 医学影像智能分析在临床诊断中至关重要，特别是识别细微病理特征。针对医学图像分析的细微挑战，需要专门设计的深度学习架构。

Method: 采用多分支ConvNeXt架构，集成全局平均池化、全局最大池化和新的注意力加权池化三个并行分支。使用端到端流程，包括数据预处理、增强和两阶段训练策略。

Result: 在2,609个CT切片数据集上验证，COVID-19病例的ROC-AUC为0.9937，验证准确率0.9757，F1分数0.9825，优于所有先前报告模型。
...
TL;DR: SOS是一个基于物体中心合成策略的简单可扩展数据合成管道，通过将高质量合成物体片段粘贴到新图像中，生成准确多样的掩码、边界框和指代表达式，在检测和视觉定位任务上超越大型真实图像数据集。


<details>
  <summary>Details</summary>
Motivation: 解决视觉分组任务中大型标注数据集成本高、覆盖偏差大、难以扩展的问题，以及合成数据缺乏灵活性、准确性和组合多样性的局限。

Method: 使用物体中心组合策略，基于结构化布局先验和生成式重光照，将高质量合成物体片段粘贴到新图像中。

Result: 在10万张合成图像上训练的模型在检测和视觉定位任务上优于在GRIT(2000万)和V3Det(20万)等更大真实图像数据集上训练的模型，在LVIS检测上提升+10.9 AP，在gRefCOCO视觉定位上提升+8.4 N_Acc。

Conclusion: SOS支持可控数据集构建，在低数据和封闭词汇设置下都能改善泛化性能，增强真实数据集能带来跨数据规模的强性能表现，特别是在极端有限真实数据情况下效果更显著。
...
TL;DR: 提出多模态语义扩散模型(MSDM)，通过结合细胞形态、颜色特征和元数据生成逼真的细胞核分割图像-掩码对，解决计算病理学中标注数据稀缺问题。
...
Method: 使用多模态语义扩散模型(MSDM)，通过细胞/细胞核形态图、RGB颜色特征和BERT编码的元数据来条件化生成过程，利用多头交叉注意力整合这些异质模态。

Result: 合成图像与真实数据高度匹配，在匹配生物条件下生成图像与真实图像的嵌入具有低Wasserstein距离。加入合成样本显著提高了柱状细胞的分割模型准确性。
...
TL;DR: 提出了PSepT（极坐标可分离变换），一种可分离的正交变换，通过将DCT径向基和傅里叶谐波角基进行张量积构造，实现了极坐标中的完全核分解，显著降低了计算复杂度和内存需求。


<details>
  <summary>Details</summary>
Motivation: 传统的正交矩图像表示方法（如Zernike和伪Zernike矩）存在高计算复杂度和数值不稳定性问题，特别是在大阶数时，需要耦合的径向-角向处理，无法实现高效分解。

Method: PSepT通过离散余弦变换（DCT）径向基和傅里叶谐波角基的张量积构造，实现了极坐标中的完全核分解，允许独立的径向和角向处理。

Result: PSepT将计算复杂度降低到O(N² log N)，内存需求降低到O(N²)，条件数缩放降低到O(√N)，相比多项式方法实现了指数级改进。在结构化数据集上表现出更好的数值稳定性、计算效率和竞争性的分类性能。
...
TL;DR: 该论文提出训练特征归因方法，将测试预测与特定训练图像的具体区域关联起来，提供对深度模型内部工作机制的新见解。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络通常被视为不透明系统，需要可解释性方法来提高信任和问责。现有方法通常将测试时预测归因于输入特征或影响训练样本，但作者认为这两种视角应该联合研究。

Method: 探索训练特征归因方法，将测试预测与特定训练图像的具体区域联系起来。

Result: 在视觉数据集上的实验表明，训练特征归因能提供细粒度的、测试特定的解释：识别导致错误分类的有害示例，并揭示传统归因方法未能暴露的虚假相关性（如基于补丁的捷径）。

Conclusion: 训练特征归因方法为深度模型提供了新的解释视角，能够揭示模型内部工作机制和潜在问题。
...
TL;DR: 提出基于图像的支气管镜拓扑定位管道，无需患者CT扫描即可提供导航辅助，仅在模拟数据上训练，在真实数据测试中表现优于现有方法


<details>
  <summary>Details</summary>
Motivation: 支气管镜检查中医生需要确定镜体位置，但由于支气管树结构复杂且医生经验不同，该任务具有挑战性。现有导航方法依赖CT扫描和额外传感器，需要额外设置和训练

Method: 开发基于图像的支气管镜拓扑定位管道，仅在模拟数据上训练，无需真实患者CT扫描，使用拓扑定位而非精确度量定位

Result: 该方法在真实数据测试序列中表现优异，超越了现有方法，展示了良好的泛化能力

Conclusion: 提出的图像支气管镜拓扑定位管道能够有效提供导航辅助，无需患者CT扫描，降低了成本并提高了实用性
...
TL;DR: 提出了一种无需真实图像即可合成多样化对象实例的新方法，用于解决实例级识别(ILR)中标注数据稀缺的问题，显著提升了多个ILR基准测试的性能。


<details>
  <summary>Details</summary>
Motivation: 实例级识别(ILR)需要识别单个对象而非类别，但由于细粒度特性导致大规模标注数据集创建困难，限制了ILR在现实世界中的应用。

Method: 通过合成生成多个领域、不同条件和背景下的多样化对象实例，构建大规模训练集，无需依赖任何真实图像，然后对基础视觉模型进行微调。

Result: 在跨越多个领域的七个ILR基准测试中，检索性能显著提升。

Conclusion: 该方法为ILR提供了高效且有效的替代方案，仅需目标领域名称即可生成训练数据，开启了ILR的新范式，具有广泛的现实应用前景。
...
TL;DR: TARO是一个新颖的开放集检测框架，不仅能识别未知物体，还能在语义层次结构中将其分类到粗粒度父类别，相比传统方法仅标记为'Unknown'更有用。


<details>
  <summary>Details</summary>
Motivation: 现代物体检测器局限于'封闭世界'假设，遇到真实场景中的新物体时存在风险。传统开放集检测方法将所有未知物体标记为单一'Unknown'类别不够实用，需要更细粒度的分类来支持安全关键决策。

Method: TARO采用独特架构：基于sparsemax的物体性建模头、层次引导的重标记组件提供辅助监督、学习层次关系的分类模块。

Result: 实验显示TARO能将高达29.9%的未知物体分类到有意义的粗粒度类别，显著减少未知与已知类别的混淆，在未知召回率和已知mAP方面都取得竞争性表现。

Conclusion: TARO框架有效解决了开放集检测中未知物体分类粒度不足的问题，为安全关键应用提供了更实用的解决方案。
...
TL;DR: 提出了在线视频深度估计方法oVDA，通过借鉴LLM的缓存和掩码技术，克服了VDA批处理的限制，在精度和VRAM使用上优于现有方法，并在边缘设备上实现实时运行。
...
TL;DR: 本文对板球击球动作分类进行了首个全面基准研究，比较了7种深度学习方法在4个研究范式下的表现，发现学术文献中的高准确率与实际实现存在显著差距，最佳模型达到92.25%准确率。


<details>
  <summary>Details</summary>
Motivation: 板球击球动作分类在体育视频分析中仍具挑战性，需要有效建模空间和时间特征。现有文献报道的准确率与实际实现存在巨大差异，需要建立标准化评估协议。

Method: 系统评估了7种深度学习方法：传统CNN-LSTM、注意力模型、视觉变换器、迁移学习方法以及现代EfficientNet-GRU组合，使用统一基准和现代MLOps实践。

Result: 重新实现先前文献方法的结果远低于报道值：Balaji LRCN从96%降至46.0%，IJERCSE从99.2%降至55.6%，Sensors从93%降至57.7%。现代SOTA方法EfficientNet-B0+GRU达到92.25%准确率。

Conclusion: 现代架构和系统优化能显著提升性能，标准化评估协议对体育视频分析研究至关重要，提供了可重复的研究平台。
...
TL;DR: 提出了一个用于自动驾驶系统驾驶员意图预测的可解释性框架VCBM，包含新的多模态数据集DAAD-X，通过概念瓶颈模型生成时空一致的解释，无需后处理技术。
...
Method: 引入驾驶员意图预测任务，创建DAAD-X多模态数据集，提出视频概念瓶颈模型(VCBM)框架，生成时空一致的解释，无需后处理。

Result: 在DAAD-X数据集上评估表明，基于transformer的模型比传统CNN模型具有更好的可解释性，并引入了多标签t-SNE可视化技术展示解释间的解耦和因果关联。

Conclusion: VCBM框架能够为自动驾驶系统的驾驶员意图预测提供有效的可解释性，transformer架构在可解释性方面优于CNN，多标签可视化技术有助于理解解释间的因果关系。
...
TL;DR: Cattle-CLIP是一个基于多模态深度学习框架的牛只行为识别系统，通过语义线索提升视频视觉特征识别性能，在监督学习下达到96.1%的准确率，并在少样本场景中表现出良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 牛只行为是动物健康、生产力和福祉的重要指标。视频监控结合深度学习已成为动物生物识别的主流方法，但现有方法在数据稀缺的行为识别方面存在不足，需要探索多模态学习在农业和动物行为分析中的潜力。

Method: 基于大规模图像-语言模型CLIP进行适配，添加时间整合模块，并针对预训练模型与真实牛只监控视频之间的领域差距，引入定制化数据增强策略和专门设计的文本提示。

Result: 在监督设置下，Cattle-CLIP在六种行为上达到96.1%的整体准确率，其中进食、饮水和站立反刍行为的召回率接近100%。在少样本场景中表现出稳健的泛化能力。

Conclusion: Cattle-CLIP展示了多模态学习在农业和动物行为分析中的巨大潜力，特别是在数据稀缺的行为识别任务中，为牲畜监测提供了有效的解决方案。
...
TL;DR: 提出了一种通用的时间分辨Transformer（TRT）架构，用于提升光子效率成像中的3D重建性能。TRT包含专门为时空瞬态测量设计的注意力机制，并开发了TRT-LOS和TRT-NLOS两个任务特定版本。


<details>
  <summary>Details</summary>
Motivation: 在光子效率成像任务中，由于传感器量子效率低和噪声水平高，特别是在长距离或复杂场景下，3D重建面临挑战。

Method: TRT架构包含时空自注意力编码器和时空交叉注意力解码器。编码器通过分割或下采样输入特征到不同尺度来探索瞬态数据的局部和全局相关性，解码器在token空间中整合局部和全局特征。

Result: 在合成数据和真实世界数据上的广泛实验表明，TRT-LOS和TRT-NLOS都显著优于现有方法。

Conclusion: TRT架构有效提升了光子效率成像的3D重建性能，并贡献了大规模合成LOS数据集和真实世界NLOS测量数据，增强了该领域的数据多样性。
...
TL;DR: 提出了Stable Video Infinity (SVI)方法，能够生成无限长度视频，具有高时间一致性、合理的场景转换和可控的流式故事情节。通过错误回收微调技术，让模型能够主动识别和纠正自身错误。


<details>
  <summary>Details</summary>
Motivation: 现有长视频生成方法主要关注减轻累积误差，但仍局限于单提示外推，产生重复动作的同质场景。核心挑战在于训练假设（看到干净数据）与测试时自回归现实（基于自生成的错误输出）之间的关键差异。

Method: 采用错误回收微调技术，将扩散变换器自生成的错误回收为监督提示。通过闭环回收注入、收集和存储错误，自回归地从错误注入反馈中学习。具体包括：注入历史错误干预干净输入、单步双向积分高效近似预测、动态将错误存入重放内存。
...
TL;DR: 提出了TEMA-LLM框架，通过大型语言模型生成语义标签，结合多注意力机制来改进跨域序列推荐系统。
...
Method: 使用LLM生成领域感知的语义标签，将标签嵌入与项目标识符、文本和视觉特征融合，引入标签增强的多注意力机制联合建模用户偏好。

Result: 在四个大规模电商数据集上的实验表明，TEMA-LLM持续优于最先进的基线方法。

Conclusion: 展示了LLM在语义标签生成和多注意力集成方面的潜力，能够推进消费电子领域的智能用户中心服务。
...
TL;DR: 这篇论文对图像和视频的多天气恢复技术进行了全面综述，涵盖传统方法和现代数据驱动模型，包括CNN、transformer、扩散模型和视觉语言模型，旨在提升智能交通系统的视觉鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 雾霾、雨雪等恶劣天气条件会显著降低图像和视频质量，对依赖视觉输入的智能交通系统（如自动驾驶、交通监控）构成严重挑战，需要有效的恢复技术来缓解这些视觉损伤。

Method: 将现有方法分为传统先验方法和现代数据驱动模型，包括CNN、transformer、扩散模型和新兴视觉语言模型；根据处理范围进一步分类为单任务模型、多任务/多天气系统和全能框架。

Result: 提供了全面的技术分类和评估，讨论了日间和夜间恢复挑战、基准数据集和评估协议，并维护了相关论文和开源实现的持续更新。

Conclusion: 指出了当前研究的局限性，并展望了未来方向，包括混合/复合退化恢复、实时部署和代理AI框架，旨在推动智能交通环境中天气鲁棒视觉系统的发展。
...
TL;DR: 提出HMVDx框架，使用消费级设备视频和多模态大语言模型进行肩部疾病初步诊断，通过分离动作理解和疾病诊断任务，诊断准确率比直接视频诊断提升79.6%。
...
Method: 提出HMVDx混合运动视频诊断框架，将动作理解和疾病诊断任务分离，分别由两个多模态大语言模型完成。

Result: HMVDx在肩关节损伤诊断中的准确率比直接视频诊断提高了79.6%，并提出了基于医疗决策逻辑过程的可用性指数新指标。
...
Result: 过滤DSE>0.3的问题后，GPT-4o准确率从51.7%提升到76.3%，GPT-4.1从54.8%提升到63.8%。
...
TL;DR: 提出统一框架联合优化时序语句定位和参考视频对象分割，通过FIND令牌识别关键时刻，采用时刻中心采样策略和双向锚点更新传播来提升分割质量。
...
Method: 使用FIND令牌进行时刻识别，设计时刻中心采样策略，结合双向锚点更新传播来初始化掩码并减少累积误差。

Result: 提出的方法在多个基准测试中达到最先进性能，验证了统一框架的有效性。

Conclusion: 联合优化时序定位和视频对象分割能自然融入关键时刻定位能力，提出的采样和传播策略显著提升分割质量。
...
TL;DR: 本文提出VPPO算法，通过分析token级视觉感知来优化多模态强化学习，显著提升大型视觉语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态推理方法忽略了视觉感知在RLVR优化过程中的关键作用，需要从token感知的新视角来改进多模态强化学习。

Method: 提出视觉感知策略优化(VPPO)算法，通过双重机制：基于整体视觉依赖度重新加权轨迹优势，并仅对感知关键token进行策略更新。
...
TL;DR: 人类视觉注视轨迹遵循类似动物觅食的Levy行走模式，通过大规模眼动实验和CNN模型验证了这种最优化的视觉信息搜索行为。
...
Method: 进行了大规模人类受试者实验，40名参与者观看50张多样化图像，使用高速眼动仪记录超过400万个注视点。训练卷积神经网络从图像输入预测注视热图。

Result: 数据分析显示人类眼睛的注视轨迹遵循类似动物觅食的Levy行走模式。CNN模型能够准确重现新颖图像中的显著注视区域。

Conclusion: 人类视觉探索遵循与自然觅食类似的统计规律，关键注视行为可以从视觉结构中学习，为通过生成和预测框架建模注视开辟了新途径。
...
TL;DR: CapGeo是一个通过几何图形描述辅助多模态大语言模型进行几何推理的框架，通过将视觉内容转换为文本描述来提升几何问题解决能力。


<details>
  <summary>Details</summary>
Motivation: 当前先进的多模态大语言模型在几何推理方面表现不佳，瓶颈在于理解几何图形而非推理能力本身。由于几何图形可以用简洁的文本描述，将视觉内容转换为文本描述是提升几何推理能力的可行方向。

Method: 提出CapGeo框架，通过几何图形描述来桥接视觉和文本模态。同时构建CapGeo-Bench数据集（4,641个图形-描述对）和基于关键点的评估指标来系统评估几何描述模型的质量。

Result: 实验显示使用描述后模型性能显著提升：Qwen2.5-VL-72B从8.6%提升到59.0%，Claude-Opus-4从44.8%提升到73.0%。提出的关键点评估指标与下游CapGeo性能强相关。

Conclusion: CapGeo框架和基准测试为推进多模态大语言模型的几何推理能力开辟了新途径，通过将视觉内容转换为文本描述可以有效提升几何问题解决能力。
...
TL;DR: 提出Hybrid-depth框架，通过整合CLIP和DINO基础模型来提升自监督单目深度估计性能，采用从粗到细的渐进学习策略，在KITTI基准测试中显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前自监督单目深度估计方法由于语义-空间知识提取不足而存在性能限制，需要更充分的上下文信息来提升深度估计精度。

Method: 1) 从CLIP和DINO聚合多粒度特征，在对比语言指导下设计代理任务进行深度感知特征对齐；2) 在粗特征基础上集成相机位姿信息和像素级语言对齐来细化深度预测，可作为即插即用模块与现有MDE流程集成。
...
TL;DR: 提出IRCR-Net用于病理图像中的细胞核实例分割，通过实例感知一致性正则化机制解决半监督学习中伪标签噪声和实例级一致性不足的问题。
...
Method: 提出IRCR-Net网络，包含匹配驱动的实例感知一致性(MIAC)和先验驱动的实例感知一致性(PIAC)机制，利用细胞核形态先验知识评估伪标签质量，丢弃低质量伪标签，增强高质量预测。

Result: 实验结果表明，该方法在多个公共数据集上显著提升了半监督细胞核实例分割性能，在某些场景下甚至超过了全监督方法。

Conclusion: IRCR-Net通过实例感知一致性正则化和先验知识利用，有效解决了半监督细胞核分割中的关键挑战，为病理图像分析提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有红外图像增强方法主要针对单一退化问题，难以处理耦合退化；而通用增强方法由于成像模型差异，对热红外图像效果有限。

Method: PPFN基于热成像过程建立提示对，通过融合对应退化类型的提示对来调制模型特征；引入选择性渐进训练(SPT)机制逐步优化模型对复合情况的处理。

Result: 该方法不仅能去除相机噪声、保留关键结构细节，还能增强热图像的整体对比度，在复杂退化场景下性能显著提升8.76%。

Conclusion: 提出的PPFN方法在热红外图像增强方面表现出色，特别是在处理复合退化场景时具有明显优势，并建立了高质量的多场景红外基准数据集。
...
Conclusion: 基于视觉语言模型的多模态关键词预测方法，特别是动态CoT策略，能够有效提升模型在复杂推理任务中的表现，解决了传统方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有推理基准主要评估基于语言的推理，将视觉输入视为可替换的上下文。为了填补这一空白，作者创建了一个基于视觉内容的推理基准。

Method: BLINK-Twice包含三个核心组件：七种视觉挑战类型、自然对抗图像对（强制依赖视觉内容）、带注释的推理链（用于细粒度评估推理过程）。评估了20个领先的MLLM模型。

Result: BLINK-Twice对当前模型构成了重大挑战。现有的语言空间推理策略（如思维链或自我批评）可以提高性能，但往往导致不稳定和冗余的推理。重复图像观察能提高性能，主动视觉交互显示了视觉推理新范式的需求。

Conclusion: 该基准突出了当前多模态大语言模型在视觉推理方面的局限性，并表明需要开发新的视觉推理范式，而非简单应用语言空间的推理策略。
...
<details>
  <summary>Details</summary>
Motivation: 传统3DGS方法严重依赖初始点云质量，在无边界动态城市场景中，由于观察视锥体不重叠，导致点覆盖不均匀不完整，训练时会产生扭曲和伪影。

Method: 使用体素化可见性推理识别不可靠几何结构，通过多样性感知视图选择信息支持视图，采用基于块匹配的多视图立体重建恢复缺失结构。

Result: 在Waymo和nuScenes数据集上的实验表明，VAD-GS优于最先进的3DGS方法，显著提高了静态和动态对象重建质量。
...
TL;DR: 提出两种无需重新训练即可利用DETR中MLP层稀疏性的方法：静态指示器稀疏化(SIBS)和微门控稀疏化(MGS)，在COCO数据集上实现85-95%的激活稀疏度，保持或提升性能的同时显著减少计算量。


<details>
  <summary>Details</summary>
Motivation: 基于transformer的模型在视觉任务（如目标检测）中的推理效率仍然是一个挑战，需要利用DETR中MLP层固有的稀疏性来减少计算开销。

Method: 1. SIBS：基于固定激活模式的启发式方法预测神经元不活跃性；2. MGS：在预训练DETR上训练的轻量级门控机制，使用小型线性层预测动态稀疏性。

Result: 在COCO数据集上，MGS实现了85-95%的激活稀疏度，在保持或改进性能的同时显著减少了计算量。

Conclusion: 该方法提供了一种实用的、输入自适应的稀疏化方法，无需完全重新训练即可实现预训练视觉transformer的高效部署。
...
<details>
  <summary>Details</summary>
Motivation: 从单目视频重建的4D场景基于文本提示进行编辑是一个有价值但具有挑战性的任务，主要困难在于在复杂动态场景的局部区域实现语义精确编辑，同时保持未编辑内容的完整性。

Method: 方法包括：1）使用量化CLIP特征增强3D高斯形成语言嵌入动态表示；2）两阶段点级定位策略（CLIP相似度候选选择和空间范围精炼）；3）使用基于扩散的视频编辑模型进行针对性编辑，通过流和涂鸦引导确保空间保真度和时间一致性。

Result: 大量实验表明，Mono4DEditor能够在不同场景和对象类型上实现高质量的文本驱动编辑，同时保持未编辑区域的外观和几何特性，在灵活性和视觉保真度方面超越先前方法。

Conclusion: Mono4DEditor提供了一个灵活准确的文本驱动4D场景编辑框架，成功解决了复杂动态场景的局部语义编辑挑战。
...
TL;DR: DWTA-Net是一个两阶段低光视频增强框架，通过联合利用短期和长期时间线索来有效抑制噪声和伪影，在真实低光视频上实现优于现有方法的视觉质量。
...
Conclusion: DWTA-Net通过联合利用短期和长期时间线索，在低光视频增强方面实现了优越的性能。
...
TL;DR: 提出了SilvaScenes数据集，用于森林环境中的树木实例分割和物种分类，包含1476棵树木的24个物种标注，展示了树木分割相对容易但物种分类仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有数据集主要关注城市环境或有限物种，无法满足复杂自然环境中林业机器人感知系统的开发需求，特别是在重遮挡、变化光照和密集植被条件下的树木检测和精细物种分类。

Method: 在加拿大魁北克五个生物气候区域收集林冠下图像，由林业专家标注1476棵树木的24个物种，构建SilvaScenes数据集，并基准测试现代深度学习实例分割方法。

Result: 树木分割表现良好，最高mAP达到67.65%，但物种分类仍然困难，mAP仅为35.69%，表明物种识别是主要挑战。

Conclusion: SilvaScenes数据集填补了森林环境中树木感知数据集的空白，为林业机器人感知系统开发提供了重要资源，同时揭示了物种分类仍是需要进一步研究的关键问题。
...
TL;DR: 提出了一种维度熵最大化方法，通过正则化文本特征分布来缓解对比视觉语言模型中主导维度的影响，改善测试时提示调优的校准性能。
...
TL;DR: 本文提出了一种基于DreamBooth的少样本角色生成方法，通过多令牌策略和LoRA微调，能够从少量参考角色中生成无限数量的新角色，同时保持艺术风格的一致性。


<details>
  <summary>Details</summary>
Motivation: 解决动画和游戏产业中从少量人类设计角色生成无限新角色的需求，同时保持艺术风格和视觉特征的连续性。

Method: 基于DreamBooth框架，采用多令牌策略（通过聚类为单个角色和集体风格分配独立令牌），结合LoRA参数高效微调，移除类别特定正则化集，在生成过程中引入随机令牌和嵌入。

Result: 在五个小型专业数据集上的评估显示，该方法能生成高质量、多样化的角色，同时保持参考角色的独特美学特征，人类评估进一步证实了其有效性。

Conclusion: 该方法成功实现了从少量参考角色生成无限新角色的目标，在保持艺术风格一致性的同时扩展了创作可能性，在动画、游戏等领域具有应用潜力。
...
TL;DR: 提出了一种临床基础的交互式医学图像分割评估方法，构建了标准化评估框架，评估了多种先进算法在不同任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 交互式分割是构建稳健、可泛化医学图像分割算法的有前景策略，但不一致且临床不现实的评估阻碍了公平比较并歪曲了真实性能。

Method: 提出了临床基础的方法论来定义评估任务和指标，并构建了软件框架用于构建标准化评估流程。

Result: 评估发现：(i)处理用户交互时最小化信息损失对模型稳健性至关重要；(ii)自适应缩放机制提升稳健性和收敛速度；(iii)验证提示行为/预算与训练不同时性能下降；(iv)2D方法在板状图像和粗糙目标中表现良好，但3D上下文有助于处理大或不规则形状目标；(v)非医学领域模型在对比度差和复杂形状时性能下降。

Conclusion: 需要临床现实的评估标准来准确反映交互式医学图像分割算法的真实性能，标准化评估框架有助于公平比较和识别关键性能因素。
...
TL;DR: PhysToolBench是首个专门评估多模态大语言模型对物理工具理解能力的基准，包含1000多个图像-文本对，测试工具识别、工具理解和工具创造三个难度级别。


<details>
  <summary>Details</summary>
Motivation: 工具使用、理解和创造是人类智能的标志，但现有MLLM对物理工具的真正理解程度尚未量化，需要专门的评估基准。

Method: 构建包含1000+图像-文本对的VQA数据集，通过三个难度级别评估：工具识别（识别主要功能）、工具理解（掌握操作原理）、工具创造（用周围物体创造新工具）。

Result: 对32个MLLM的评估显示在工具理解方面存在显著缺陷，包括专有、开源、专用具身模型和VLA骨干模型。

Conclusion: 提出了初步解决方案，代码和数据集已公开，为改进MLLM的物理工具理解能力提供了基础。
...
TL;DR: 三星智能手机图像中的对角线伪影会影响PRNU相机来源验证，但支持PRO模式和RAW拍摄的设备仍可进行可靠验证，这些伪影在法证应用中也有潜在用途。


<details>
  <summary>Details</summary>
Motivation: 研究三星智能手机图像中的对角线伪影及其对PRNU相机来源验证的影响，探索在无法获取RAW图像情况下的解决方案。

Method: 分析多个三星Galaxy S和A系列模型的图像，比较RAW图像和经过处理图像的差异，研究伪影在HDR和人像模式图像中的表现。

Result: 发现某些Galaxy S系列模型共享导致指纹冲突的共同模式，A系列模型也存在类似问题；RAW图像可避免伪影影响，但中端A系列不支持此功能；伪影可用于减少HDR图像误检和定位人像模式中的合成虚化区域。

Conclusion: 对角线伪影确实影响PRNU验证，但通过RAW图像或利用伪影特征的法证应用仍能实现可靠的相机来源识别。
...
TL;DR: PRNet是一个实时检测框架，通过保留和有效利用原始浅层空间特征来增强小目标检测，在计算约束下实现了优越的精度-效率平衡。
...
Conclusion: PRNet通过优先保留和有效利用原始浅层空间特征，成功解决了小目标检测中的信息退化问题，实现了优越的检测性能。
...
TL;DR: FLOWING是一个基于微分向量流的变形框架，通过将变形建模为流构造来确保连续性、可逆性和时间一致性，在2D图像和3D形状变形中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 传统多层感知机(MLP)作为隐式神经表示(INR)进行变形建模时，通常需要昂贵的正则化，导致训练不稳定且难以有效对齐特征。

Method: 将变形重新定义为微分向量流的构造，通过将结构流属性直接编码到网络架构中，确保连续性、可逆性和时间一致性。

Result: 在面部变形、图像变形和高斯溅射变形等应用中，FLOWING实现了最先进的变形质量，且收敛速度更快。

Conclusion: 基于流的变形方法能够产生原则性且稳定的变换，实现准确且结构保持的2D图像和3D形状变形。
...
TL;DR: TC-LoRA是一种新的可控扩散模型框架，通过动态生成LoRA适配器来条件化模型权重，实现随时间变化的上下文感知控制，相比静态激活修改方法显著提升了生成质量和空间条件遵循能力。


<details>
  <summary>Details</summary>
Motivation: 当前可控扩散模型使用静态架构修改中间激活来注入条件指导，这种固定策略无法适应多阶段去噪过程的动态特性，限制了模型从粗结构到细细节的适应能力。

Method: 提出TC-LoRA框架，使用超网络基于时间和用户条件动态生成LoRA适配器，直接条件化冻结骨干网络的权重，实现参数化的动态控制。

Result: 在多个数据领域的实验中，这种动态参数控制相比静态激活方法显著提升了生成保真度和空间条件遵循能力。

Conclusion: TC-LoRA建立了一种替代方法，通过权重的深度功能适应来修改模型的条件策略，使控制能够与任务和生成阶段的动态需求保持一致。
...
<details>
  <summary>Details</summary>
Motivation: 生物医学场景中目标检测面临标注数据稀缺、新类别频繁出现的问题，现有方法通常孤立处理这些任务，缺乏统一的解决方案。

Method: 基于类别无关的DETR骨干网络，从原始支持图像构建类别原型，使用增强视图和轻量级transformer解码器学习嵌入空间，联合优化原型匹配损失、对齐分离损失和KL散度正则化。

Result: 在卵细胞、血细胞和疟疾检测任务上的大量实验表明，FSP-DETR在少样本和开放集场景下显著优于现有少样本和基于原型的检测器。

Conclusion: FSP-DETR提供了推理时的灵活性，支持未见类别识别、背景拒绝和跨任务适应而无需重新训练，为生物医学目标检测提供了统一解决方案。
...
TL;DR: 对2023-2025年CVPR、ICLR和NeurIPS的26,104篇论文进行可重复性分析，发现三大趋势：多模态视觉语言模型兴起、生成方法稳步扩展、3D和视频研究保持活跃。


<details>
  <summary>Details</summary>
Motivation: 量化计算机视觉和机器学习领域的研究趋势，提供透明、可重复的测量方法，帮助理解当前研究重点和发展方向。

Method: 对论文标题和摘要进行标准化处理，使用手工构建的词典分配最多35个主题标签，挖掘任务、架构、训练机制、目标函数、数据集和多模态等细粒度信息。

Result: 识别出三大宏观转变：多模态视觉语言LLM工作急剧增加，生成方法持续扩展，3D和视频研究保持活跃。在VLM领域，参数高效适应方法占主导，训练实践从零构建编码器转向指令调优和微调。

Conclusion: 研究趋势显示多模态和生成方法成为主流，不同会议各有侧重，同时发布了词典和方法论以便审计和扩展。局限性包括词典召回率和仅基于摘要的分析。
...
TL;DR: 该论文提出了SpaceVista-1M数据集和SpaceVista-7B模型，解决了多尺度空间推理的两个关键挑战：数据集构建依赖室内3D扫描和人工标注，以及缺乏有效的全尺度场景建模。


<details>
  <summary>Details</summary>
Motivation: 当前空间推理研究在室内场景理解方面取得进展，但在机器人、自动驾驶等多样化应用中仍面临挑战。主要问题是数据集构建依赖室内3D扫描和人工标注，以及缺乏有效的全尺度场景建模。

Method: 提出整体解决方案，包括结构化空间推理知识系统、尺度感知建模和渐进式训练范式。使用任务特定、专家驱动的自动化流水线构建SpaceVista-1M数据集，包含38K视频场景和约1M空间问答对。然后构建SpaceVista-7B模型，接受密集输入并使用尺度作为锚点进行尺度感知专家和渐进式奖励。

Result: 在5个基准测试（包括SpaceVista-Bench）上的广泛评估显示具有竞争力的性能，在所有尺度和场景中展现出强大的泛化能力。

Conclusion: 该研究通过构建大规模数据集和先进模型，成功推进了全尺度空间推理能力，为多样化应用场景提供了有效解决方案。
...
Method: 两阶段训练策略：轻量级对齐将VLM隐藏状态映射到小型动作模型空间；选择性微调语言模型、状态编码器和动作模块

Result: 在LIBERO上达到97.3%平均成功率（提升11.8%），在LIBERO-LONG上达到93.5%（提升24.5%），真实世界实验中比教师模型提升17%

Conclusion: 动作蒸馏能有效使VLM生成精确动作，同时大幅降低训练成本
...
TL;DR: StreamingVLM是一个专为实时理解无限视觉输入而设计的视觉语言模型，通过统一的训练-推理对齐框架和紧凑的KV缓存管理，解决了长视频处理中的计算成本和延迟问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在处理无限视频流时面临二次计算成本和延迟问题，滑动窗口方法要么破坏连贯性，要么存在冗余重计算。

Method: 采用统一的训练-推理对齐框架，在推理时维护紧凑的KV缓存（重用注意力sink状态、近期视觉token短窗口和文本token长窗口），通过简单的监督微调策略在短重叠视频块上应用完整注意力。

Result: 在Inf-Streams-Eval基准测试中，StreamingVLM对GPT-4O mini取得66.18%的胜率，在单张NVIDIA H100上保持高达8 FPS的稳定实时性能，同时在LongVideoBench和OVOBench Realtime上分别提升4.30和5.96分。

Conclusion: StreamingVLM通过创新的流式推理框架有效解决了长视频处理的挑战，不仅实现了实时性能，还意外地提升了通用VQA能力。
...
<details>
  <summary>Details</summary>
Motivation: 边缘计算环境中的动态资源可用性（如图像质量下降、网络不稳定、服务器负载波动）对实时3D重建的可靠性构成挑战，特别是在消防救援等关键应用中需要及时准确的三维场景建模。
...
Conclusion: 强化学习框架能够有效管理边缘资源，在资源受限和易受干扰的环境中实现高质量的3D重建。
...
### [95] [Energy-Driven Steering: Reducing False Refusals in Large Language Models](https://arxiv.org/abs/2510.08646)
...
TL;DR: 提出EDS框架，通过基于能量的推理时干预解决LLM安全对齐中的过度拒绝问题，在保持安全性的同时显著降低误拒率


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全对齐技术往往只关注对抗有害提示，导致模型变得过度谨慎，对良性提示也拒绝响应。需要同时提升安全性并减少误拒

Method: 训练轻量级外部能量模型，为不良状态分配高能量，为理想状态分配低能量。在推理时通过能量梯度动态引导LLM隐藏状态到低能量区域

Result: 在ORB-H基准上，将合规率从57.3%提升至82.6%，同时保持基线安全性能。显著降低了误拒率

Conclusion: EDS提供了一种有效的范式，使LLM能够同时实现低误拒率和高安全性，将行为控制与核心知识解耦
...
### [96] [Inverse-Free Wilson Loops for Transformers: A Practical Diagnostic for Invariance and Order Sensitivity](https://arxiv.org/abs/2510.08648)
*Edward Y. Chang,Ethan Y. Chang*

Main category: cs.LG

TL;DR: WILSON是一个用于检测大语言模型输出不稳定性的诊断工具，能识别因文本顺序变化、微调等操作导致的答案翻转问题，帮助在部署中平衡安全性和效率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在实际应用中存在输出不稳定性问题，如RAG输出随段落顺序变化而翻转、微调削弱预训练学到的恒定性等，这些违反预期不变性的问题会影响系统可靠性并迫使团队在安全性和速度之间权衡。

Method: WILSON通过简单的循环和重排序检查内部表示来生成系统信号，结合基于位置和层的无逆曲率图（使用JVP和Hutchinson探针计算）以及激活级交换子来标记重排序风险。

Result: WILSON信号计算成本低，适用于标准Transformer模型，可输出阈值和CSV文件供编排器使用，能够防护RAG的顺序效应、捕获微调回归、稳定辩论路径和长多轮上下文。
...
### [97] [Knowledge Graph Sparsification for GNN-based Rare Disease Diagnosis](https://arxiv.org/abs/2510.08655)
*Premt Cara,Kamilia Zaripova,David Bani-Harouni,Nassir Navab,Azade Farshad*

Main category: cs.LG

TL;DR: RareNet是一种基于子图的图神经网络，仅需患者表型数据即可识别最可能的致病基因，为资源有限地区提供可访问的遗传分析工具。


<details>
  <summary>Details</summary>
Motivation: 罕见遗传病诊断面临患者数据不足、全基因组测序难以获取以及致病基因数量庞大等挑战，导致诊断延迟和治疗不当，特别是在资源有限地区。

Method: 使用基于子图的图神经网络方法，仅依赖患者表型数据识别致病基因，可作为独立方法或与其他候选基因优先排序方法的预处理/后处理过滤器。

Result: 在两个生物医学数据集上的综合评估显示，RareNet在致病基因预测方面具有竞争力和鲁棒性，与其他框架集成时显著提升性能。

Conclusion: RareNet通过仅需表型数据，使复杂遗传分析民主化，特别对缺乏先进基因组基础设施的服务不足人群具有重要价值。
...
### [98] [Inner-Instance Normalization for Time Series Forecasting](https://arxiv.org/abs/2510.08657)
*Zipo Jibao,Yingyi Fu,Xinyang Chen,Guoting Chen*

Main category: cs.LG

TL;DR: 提出了两种点级方法LD和LCD来处理时间序列预测中的实例内分布偏移问题，通过不同时间步使用不同参数来消除内部差异。
...
Method: 提出了两种点级方法：学习分布(LD)通过在不同时间步使用不同参数拟合输入输出内部分布；学习条件分布(LCD)使用神经网络预测输出的缩放系数。

Result: 在多个公共基准测试中与各种骨干模型结合评估，通过对比实验证明了点级范式的有效性。
...
### [99] [Provably Robust Adaptation for Language-Empowered Foundation Models](https://arxiv.org/abs/2510.08659)
...
<details>
  <summary>Details</summary>
Motivation: 现有语言赋能基础模型在少样本学习中依赖开放环境收集的小型支持数据集，容易受到中毒攻击，而现有防御方法缺乏形式化保证且对未知和自适应攻击脆弱。

Method: LeFCert整合文本和特征嵌入，采用自适应混合机制，提出双重修剪均值原型，推导分类分数的可证明上下界。还扩展了LeFCert-L（引入随机平滑和Lipschitz连续性）和LeFCert-C（提供集体认证）。

Result: 实验表明LeFCert达到最先进性能，相比现有基线显著提高了干净准确率和认证准确率，且计算效率高，适合实际应用。

Conclusion: LeFCert是第一个为语言赋能基础模型设计的可证明鲁棒少样本分类器，在中毒攻击下提供形式化保证，同时保持高性能和计算效率。
...
### [100] [How Scale Breaks "Normalized Stress" and KL Divergence: Rethinking Quality Metrics](https://arxiv.org/abs/2510.08660)
*Kiran Smelser,Kaviru Gunaratne,Jacob Miller,Stephen Kobourov*

Main category: cs.LG

TL;DR: 该论文分析了高维数据可视化中常用的质量度量指标（归一化应力和KL散度）对尺度变化的敏感性，并提出了一种简单方法使这些指标对尺度变化不敏感。


<details>
  <summary>Details</summary>
Motivation: 高维数据可视化中常用的质量度量指标对均匀缩放敏感，即使缩放不会改变投影的本质特征，这影响了维度缩减技术评估的准确性。

Method: 通过分析和实证研究尺度对应力和KL散度的影响，提出了一种简单技术使这两个度量指标对尺度变化不敏感。

Result: 研究表明应力和KL散度值会随尺度变化显著改变，提出的尺度不变方法在小基准测试中准确捕捉了预期行为。

Conclusion: 尺度敏感性会影响维度缩减技术的评估，提出的尺度不变度量方法能够更准确地评估可视化质量。
...
### [101] [CATS-Linear: Classification Auxiliary Linear Model for Time Series Forecasting](https://arxiv.org/abs/2510.08661)
*Zipo Jibao,Yingyi Fu,Xinyang Chen,Guoting Chen*

Main category: cs.LG

TL;DR: CATS-Linear模型通过分类辅助通道独立机制动态路由时间序列实例到专用预测器，结合趋势-季节性分解架构，在固定超参数下达到与调参基线相竞争的SOTA精度。


<details>
  <summary>Details</summary>
Motivation: 现有研究显示线性模型能达到与复杂架构竞争的预测性能，但增强线性模型的方法仍探索不足。基于不同时间序列实例可能遵循异质线性映射的假设，需要开发更有效的线性模型增强方法。

Method: 提出CATS-Linear模型，采用分类辅助通道独立机制动态路由实例到专用预测器；重新设计趋势-季节性分解架构，添加解耦-线性映射-重耦合框架处理趋势分量，使用复数域线性投影处理季节性分量。

Result: 大量实验验证CATS-Linear在固定超参数下达到与超参数调优基线相竞争的SOTA精度，同时在固定超参数对比中也提供SOTA精度。

Conclusion: CATS-Linear通过分类辅助通道独立和重新设计的分解架构，有效提升了线性模型的预测性能，在固定超参数设置下实现了与调参模型相竞争的精度水平。
...
### [102] [DPCformer: An Interpretable Deep Learning Model for Genomic Prediction in Crops](https://arxiv.org/abs/2510.08662)
*Pengcheng Deng,Kening Liu,Mengxi Zhou,Mingxi Li,Rui Yang,Chuzhe Cao,Maojun Wang,Zeyu Zhang*

Main category: cs.LG

TL;DR: DPCformer是一种结合卷积神经网络和自注意力机制的深度学习模型，用于基因组选择预测作物表型，在多种作物和性状上显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统基因组选择方法在处理复杂性状和大数据集时预测精度不足，需要开发更有效的模型来提升作物育种效率。

Method: 使用8维one-hot编码处理SNP数据，按染色体排序，采用PMF算法进行特征选择，集成CNN和自注意力机制建模基因型-表型关系。

Result: 在5种作物13个性状上验证：玉米开花期和株高精度提升2.92%；棉花纤维性状精度提升8.37%；番茄小样本数据Pearson相关系数提升57.35%；鹰嘴豆产量相关性提升16.62%。

Conclusion: DPCformer在精度、小样本鲁棒性和可解释性方面表现优异，为精准育种和全球粮食安全挑战提供了有力工具。
...
### [103] [FreqCa: Accelerating Diffusion Models via Frequency-Aware Caching](https://arxiv.org/abs/2510.08669)
...
TL;DR: 本文提出了频率感知缓存(FreqCa)方法，通过分析扩散模型中不同频率分量的动态特性，分别处理低频和高频特征来优化推理成本。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器的高推理成本问题，现有特征缓存方法假设相邻时间步特征相似或连续，但这一假设并不总是成立。

Method: 从频域分析发现低频分量相似度高但连续性差，高频分量连续性好但相似度低。因此提出FreqCa：直接重用低频特征，使用二阶Hermite插值器预测高频特征，并缓存累积残差特征(CRF)减少内存占用。

Result: 在FLUX.1-dev、FLUX.1-Kontext-dev、Qwen-Image和Qwen-Image-Edit上的实验证明了该方法在生成和编辑任务中的有效性。

Conclusion: 频率感知缓存方法通过分别处理不同频率分量的特性，显著降低了扩散模型的推理成本，同时保持了生成质量。
...
### [104] [Don't Waste Mistakes: Leveraging Negative RL-Groups via Confidence Reweighting](https://arxiv.org/abs/2510.08696)
*Yunzhen Feng,Parag Jain,Anthony Hartshorn,Yaqi Duan,Julia Kempe*

Main category: cs.LG

TL;DR: 提出LENS方法改进GRPO算法，通过置信度加权的惩罚机制利用负样本组，提高强化学习效率。


<details>
  <summary>Details</summary>
Motivation: GRPO算法在处理负样本组（没有正确答案的组）时浪费计算资源，因为这些组产生零优势和无梯度。

Method: 从奖励建模的最大似然目标出发，推导出等价于修改值函数的策略梯度，该值函数对错误回答添加置信度加权惩罚。

Result: 在MATH基准测试中，LENS方法在Llama-3.1-8B和Qwen-2.5-3B模型上持续优于GRPO基线，在困难项目上表现更佳。
...
### [105] [In-Context Learning for Non-Stationary MIMO Equalization](https://arxiv.org/abs/2510.08711)
*Jiachen Jiang,Zhen Qin,Zhihui Zhu*

Main category: cs.LG

TL;DR: 本文研究了基于上下文学习(ICL)的信道均衡器在非平稳环境中的能力，提出了从经典自适应算法(如LMS、LRMS)中推导出的新型注意力机制，显著提升了在动态环境中的适应性和性能。


<details>
  <summary>Details</summary>
Motivation: 现有ICL均衡器主要针对静态信道设计，缺乏对非平稳问题的理论分析和研究。本文旨在探索ICL在时变信道均衡中的潜力，为开发具有更强适应性的下一代无线基础模型提供关键见解。

Method: 采用原则性框架设计高效注意力机制，利用自适应信号处理算法指导设计。从最小均方(LMS)算法、最小均方根(LRMS)公式或多步梯度更新中推导出新的注意力变体，以增强非平稳任务中的适应性。

Result: 实验结果表明，ICL在非平稳MIMO均衡中表现出强大潜力，基于经典自适应算法启发的注意力机制能显著提升动态环境中的适应性和性能。

Conclusion: ICL在非平稳信道均衡中具有重要应用价值，基于自适应算法启发的注意力机制设计为开发更强大适应性的无线基础模型提供了关键方向。
...
### [106] [Enhancing Self-Supervised Learning with Semantic Pairs A New Dataset and Empirical Study](https://arxiv.org/abs/2510.08722)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决自监督学习中如何有效学习数据表示的问题，通过将每个数据实例视为独立类别来构建学习任务。

Method: 对每个数据实例应用随机变换生成两个不同的视图，训练模型学习对这些视图保持不变的表示，从而捕捉实例的共性特征。

Result: 该方法能够学习到对数据变换具有鲁棒性的表示，有助于模型理解数据中的不变特征。

Conclusion: 实例判别是一种有效的自监督表示学习框架，通过将实例视为类别并学习视图不变性来获得有意义的表示。
...
### [107] [Counterfactually Fair Conformal Prediction](https://arxiv.org/abs/2510.08724)
*Ozgur Guldogan,Neeraj Sarna,Yuanyuan Li,Michael Berger*

Main category: cs.LG

TL;DR: 提出了Counterfactually Fair Conformal Prediction (CF-CP)方法，通过对称化保护属性干预的符合度分数，在保持边际覆盖特性的同时实现反事实公平的预测集。


<details>
  <summary>Details</summary>
Motivation: 虽然点预测器的反事实公平性已有研究，但预测集的反事实公平性（在不确定性下进行公平决策的关键）仍未充分探索。同时，保形预测提供有效的预测集但不保证反事实公平性。

Method: 通过对称化保护属性干预的符合度分数，开发了反事实公平保形预测(CF-CP)方法，产生反事实公平的预测集。
...
### [108] [Transmuting prompts into weights](https://arxiv.org/abs/2510.08734)
*Hanna Mazzawi,Benoit Dherin,Michael Munn,Michael Wunder,Javier Gonzalvo*

Main category: cs.LG

TL;DR: 本文为LLM内部状态干预技术提供了理论基础，揭示了如何将文本输入转化为可重用的权重更新，解释了现有的向量和矩阵编辑方法。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型推理时控制技术虽然有效，但多基于经验启发式方法，缺乏理论支撑。本文旨在为这些干预技术提供数学基础。

Method: 基于transformer架构的基本计算，将提示的影响映射为隐式权重更新，推导出token无关的思维向量和思维矩阵，将文本输入转化为权重更新。

Result: 建立了深度多块transformer中信息表示和组合的理论框架，为现有模型编辑技术提供了理论基础，并提出了将文本转换为权重更新的原则性方法。

Conclusion: 该理论不仅解释了现有向量和矩阵编辑技术的工作原理，还提供了一种基于计算的直接方法，可将文本输入转化为可重用的权重更新。
...
### [109] [SHAP-Based Supervised Clustering for Sample Classification and the Generalized Waterfall Plot](https://arxiv.org/abs/2510.08737)
*Justin Lin,Julia Fukuyama*

Main category: cs.LG

TL;DR: SHAP值聚类分析：通过聚类SHAP值来揭示数据样本获得相同预测的相似原因，提供模型预测的可解释性洞察。


<details>
  <summary>Details</summary>
Motivation: 大型黑盒模型在处理复杂数据时缺乏可解释性，在高风险场景中使用存在信任问题。需要方法能够解释模型预测过程。

Method: 使用SHAP分析量化每个特征对预测的贡献，然后对SHAP值进行聚类，将获得相同预测且原因相似的样本分组。

Result: 在模拟实验和阿尔茨海默病案例研究中展示了该方法，并提出了多分类瀑布图的新泛化形式。

Conclusion: SHAP值聚类能够揭示不同样本达到相同预测的各种路径，增强了黑盒模型的可解释性和可信度。
...
### [110] [Faithful and Interpretable Explanations for Complex Ensemble Time Series Forecasts using Surrogate Models and Forecastability Analysis](https://arxiv.org/abs/2510.08739)
*Yikai Zhao,Jiekai Ma*

Main category: cs.LG

TL;DR: 提出了一个双方法框架，通过代理模型解释复杂时间序列集成预测，并结合频谱可预测性分析来量化预测可靠性。


<details>
  <summary>Details</summary>
Motivation: 解决AutoML系统生成的时间序列集成模型在准确性和可解释性之间的权衡问题，提高复杂预测模型的可解释性和可靠性评估。

Method: 1) 使用LightGBM作为代理模型模拟AutoGluon预测，生成稳定的SHAP特征归因；2) 集成频谱可预测性分析，通过比较时间序列与纯噪声基准来量化固有可预测性。

Result: 在M5数据集上的实证评估显示，更高的频谱可预测性不仅与改进的预测准确性相关，还与代理模型和原始模型之间的更高保真度相关。

Conclusion: 该框架为最先进的集成预测提供了可解释的实例级解释，同时为用户提供了作为预测及其解释可靠性指标的可预测性度量。
...
### [111] [Graph Diffusion Transformers are In-Context Molecular Designers](https://arxiv.org/abs/2510.08744)
...
Method: 使用演示条件扩散模型，通过少量分子-评分示例指导去噪Transformer生成符合目标属性的分子。开发了新的分子分词器Node Pair Encoding，在基序级别表示分子，节点数减少5.5倍。
...
### [112] [RFOD: Random Forest-based Outlier Detection for Tabular Data](https://arxiv.org/abs/2510.08747)
*Yihao Ang,Peicheng Yao,Yifan Bao,Yushuo Feng,Qiang Huang,Anthony K. H. Tung,Zhiyong Huang*

Main category: cs.LG

TL;DR: 提出了RFOD，一种基于随机森林的表格数据异常检测框架，通过特征级条件重构处理混合类型数据，结合调整后的Gower距离和不确定性加权平均实现精确可解释的异常检测。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在处理混合类型表格数据时语义信息丢失和缺乏可解释性的问题，特别是在网络安全、金融欺诈检测和医疗保健等高风险领域。

Method: 将异常检测重构为特征级条件重构问题，为每个特征训练专门的随机森林；使用调整后的Gower距离进行单元格级评分，并结合不确定性加权平均聚合为行级异常分数。

Result: 在15个真实数据集上的实验表明，RFOD在检测准确性上持续优于最先进的基线方法，同时为混合类型表格数据提供更好的鲁棒性、可扩展性和可解释性。

Conclusion: RFOD框架有效解决了混合类型表格数据异常检测中的关键挑战，在保持语义完整性的同时提供了精确且可解释的检测结果。
...
### [113] [Conformal Risk Training: End-to-End Optimization of Conformal Risk Control](https://arxiv.org/abs/2510.08748)
*Christopher Yeh,Nicolas Christianson,Adam Wierman,Yisong Yue*

Main category: cs.LG

TL;DR: 提出了一种控制优化确定性等价(OCE)风险的方法，扩展了传统共形风险控制(CRC)框架，能够处理尾部风险，并引入了端到端的共形风险训练方法。
...
Method: 开发了控制OCE风险类的方法，包括条件风险价值(CVaR)等尾部风险度量；提出了端到端的共形风险训练，在模型训练或微调过程中通过可微分的共形OCE风险控制进行优化。

Result: 方法在控制分类器假阴性率和电池存储操作中的金融风险等应用中，相比后处理方法显著提高了平均性能，同时保持可证明的风险保证。
...
### [114] [Exploring Cross-Client Memorization of Training Data in Large Language Models for Federated Learning](https://arxiv.org/abs/2510.08750)
...
Conclusion: 联邦学习模型存在记忆化风险，特别是客户端内数据，需要关注跨样本记忆化的微妙风险，并考虑训练和推理因素对记忆化的影响。
...
### [115] [LOTION: Smoothing the Optimization Landscape for Quantized Training](https://arxiv.org/abs/2510.08757)
*Mujin Kwun,Depen Morwani,Chloe Huangyuan Su,Stephanie Gil,Nikhil Anand,Sham Kakade*

Main category: cs.LG

TL;DR: LOTION是一个用于量化神经网络优化的平滑框架，通过随机噪声平滑方法解决量化目标梯度不连续的问题，保证收敛到局部最小值，并在实验中优于标准QAT方法。


<details>
  <summary>Details</summary>
Motivation: 量化神经网络优化具有根本性挑战，因为量化器是分段常数函数，除了量化阈值处导数未定义外，其他位置梯度为零。现有方法如STE缺乏收敛保证。

Method: 受Nesterov平滑启发，提出LOTION框架，用无偏随机舍入噪声下的期望损失来近似量化损失表面，使用标准优化器可保证收敛到局部最小值。

Result: 实验证明该方法在合成测试集和150M/300M参数语言模型上优于标准QAT方法，且使用随机舍入噪声时能保持原始量化损失的全局最小值。

Conclusion: LOTION提供了一个有理论保证的量化优化框架，有效解决了量化目标梯度不连续的问题，在语言模型等实际应用中表现出色。
...
### [116] [Spatial Deconfounder: Interference-Aware Deconfounding for Spatial Causal Inference](https://arxiv.org/abs/2510.08762)
*Ayush Khot,Miruna Oprescu,Maresa Schröder,Ai Kagawa,Xihaier Luo*

Main category: cs.LG

TL;DR: 提出了Spatial Deconfounder方法，通过将空间干扰转化为多原因信号，同时解决空间因果推断中的未测量混杂因素和处理干扰问题。
...
Method: 使用两阶段方法：首先通过条件变分自编码器（CVAE）和空间先验从局部处理向量重构替代混杂因素，然后通过灵活的结果模型估计因果效应。
...
### [117] [Reinforcement Learning-Based Optimization of CT Acquisition and Reconstruction Parameters Through Virtual Imaging Trials](https://arxiv.org/abs/2510.08763)
...
Method: 使用虚拟CT模拟器对含肝病变的人体模型成像，采用PPO强化学习算法优化管电压、管电流、重建核等参数，以最大化肝病变可检测性指数

Result: 强化学习方法在所有测试案例中均达到全局最大可检测性指数，比穷举搜索减少79.7%的计算步骤

Conclusion: 虚拟成像工具与强化学习结合为CT协议管理提供了准确且计算高效的解决方案，框架灵活可适应不同图像质量目标
...
### [118] [Zero-Shot Policy Transfer in Reinforcement Learning using Buckingham's Pi Theorem](https://arxiv.org/abs/2510.08768)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习策略往往难以泛化到具有不同物理参数的新机器人、任务或环境，这限制了其实际应用价值。

Method: 基于白金汉π定理，通过将策略的输入（观测）和输出（动作）缩放到无量纲空间来适应新系统上下文，无需重新训练。

Result: 在三个复杂度递增的环境中验证：模拟钟摆、物理钟摆（sim-to-real验证）和高维HalfCheetah。缩放迁移在动态相似上下文中无性能损失，在非相似上下文中始终优于朴素迁移。
...
### [119] [Struc-EMB: The Potential of Structure-Aware Encoding in Language Embeddings](https://arxiv.org/abs/2510.08774)
*Shikun Liu,Haoyu Wang,Mufei Li,Pan Li*

Main category: cs.LG

TL;DR: 提出并系统评估了一种新的结构感知文本嵌入范式，通过将结构关系直接整合到LLM内部编码过程中，在检索、聚类、分类和推荐任务中一致优于仅文本和后处理基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM文本嵌入模型通常基于原始文本，忽略了超链接或引用等丰富的结构信息，而这些信息在许多真实世界数据集中提供了关键上下文。

Method: 研究了两种主要的进程内方法：顺序连接和并行缓存，并引入了上下文蒸馏和语义平衡两种技术来处理噪声结构数据。

Result: 结构感知方法在零样本实验中一致优于文本基准和后处理基线，顺序连接在噪声中等长度上下文中表现优异，并行缓存在长高信号上下文中扩展性更好但更容易受到干扰。
...
### [120] [Guiding Exploration in Reinforcement Learning Through LLM-Augmented Observations](https://arxiv.org/abs/2510.08779)
*Vaibhav Jain,Gerrit Grossmann*

Main category: cs.LG

TL;DR: 提出了一种利用LLM生成动作建议并通过增强观察空间指导RL探索的框架，在稀疏奖励环境中显著提升RL性能


<details>
  <summary>Details</summary>
Motivation: 传统RL在稀疏奖励环境中探索效率低，而LLM具有程序性知识和推理能力，但现有方法过于僵化地依赖LLM建议

Method: 通过增强观察空间提供LLM生成的动作建议，让RL智能体学习何时遵循或忽略这些指导，保持软约束的灵活性

Result: 在三个BabyAI环境中，LLM指导的效果随任务难度增加而提升，最复杂环境中最终成功率相对提升71%，样本效率提升高达9倍
...
### [121] [Weights initialization of neural networks for function approximation](https://arxiv.org/abs/2510.08780)
*Xinwen Hu,Yunqing Huang,Nianyu Yi,Peimeng Yin*

Main category: cs.LG

TL;DR: 提出基于基函数预训练的可重用初始化框架，通过预训练多项式基神经网络，然后用于初始化复杂目标函数的网络，提高训练效率、泛化能力和模型可迁移性。
...
Method: 首先训练基神经网络来逼近参考域上的多项式族，然后将学习到的参数用于初始化复杂目标函数的网络，并引入域映射机制将输入转换到参考域。
...
### [122] [Deceptive Exploration in Multi-armed Bandits](https://arxiv.org/abs/2510.08794)
*I. Arda Vurankaya,Mustafa O. Karabag,Wesley A. Suttle,Jesse Milzman,David Fridovich-Keil,Ufuk Topcu*

Main category: cs.LG

TL;DR: 本文研究多臂老虎机问题中欺骗性代理在KL散度约束下识别最佳私有臂的策略，证明了公共次优臂的拉动率最多为Θ(√T)，并提出了受top-two算法启发的自适应算法。


<details>
  <summary>Details</summary>
Motivation: 研究在公共奖励和私有奖励并存的多臂老虎机场景中，欺骗性代理如何在观察者监控下高效识别最佳私有臂而不被发现，解决隐私保护与效率权衡的问题。

Method: 建立KL散度约束下的检测性模型，将公共次优臂拉动建模为伯努利过程，求解基于公共和私有均值的极大极小问题，提出受top-two算法启发的自适应探索算法。

Result: 理论证明公共次优臂的拉动率最多为Θ(√T)，确定了最佳私有臂识别的最优误差指数，算法能根据公共次优性差距自适应调整探索策略。

Conclusion: 在KL散度约束下，欺骗性代理可以有效识别最佳私有臂，公共次优臂的拉动率受限于Θ(√T)，提出的自适应算法在实际应用中表现良好。
...
### [123] [PO-CKAN:Physics Informed Deep Operator Kolmogorov Arnold Networks with Chunk Rational Structure](https://arxiv.org/abs/2510.08795)
*Junyi Wu,Guang Lin*

Main category: cs.LG

TL;DR: PO-CKAN是一个基于分块有理Kolmogorov-Arnold网络(KANs)的物理信息深度算子框架，用于逼近偏微分方程的解算子，相比PI-DeepONet在Burgers方程上减少了约48%的平均相对L2误差。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够高效学习物理一致的时空解算子，并能够快速预测参数化时变偏微分方程的框架，同时提高函数逼近能力。

Method: 采用DeepONet架构，集成分块有理Kolmogorov-Arnold网络(CKAN)子网络，结合物理信息神经网络(PINNs)原理来强制物理一致性，通过PDE残差损失实现物理约束。

Result: 在Burgers方程(ν=0.01)上，PO-CKAN相比PI-DeepONet减少了约48%的平均相对L2误差；在Eikonal和扩散-反应基准问题上也达到了有竞争力的精度。

Conclusion: PO-CKAN框架能够准确学习算子，结果与高保真解高度匹配，为参数化时变PDE的高效求解提供了有效方法。
...
### [124] [TAPAS: Datasets for Learning the Learning with Errors Problem](https://arxiv.org/abs/2510.08797)
...
Method: 创建TAPAS数据集，涵盖多种LWE设置，供AI从业者直接使用来开发新的LWE破解方法。

Result: 建立了攻击性能基线，为未来工作奠定了基础。

Conclusion: TAPAS数据集填补了AI研究LWE攻击的数据空白，有望推动该领域的发展。
...
### [125] [Edu-EmotionNet: Cross-Modality Attention Alignment with Temporal Feedback Loops](https://arxiv.org/abs/2510.08802)
*S M Rafiuddin*

Main category: cs.LG

TL;DR: 提出了Edu-EmotionNet框架，通过联合建模时间情感演化和模态可靠性，实现在线教育中的鲁棒情感识别


<details>
  <summary>Details</summary>
Motivation: 现有情感识别方法依赖静态融合策略并假设模态输入始终可靠，这在真实学习环境中很少成立，需要更鲁棒的解决方案

Method: 包含三个关键组件：跨模态注意力对齐模块用于动态跨模态上下文共享，模态重要性估计器为每个时间步分配基于置信度的权重，时间反馈循环利用先前预测来强制时间一致性

Result: 在教育子集IEMOCAP和MOSEI上评估，针对困惑、好奇、无聊和沮丧情绪，实现了最先进性能，并对缺失或噪声模态表现出强鲁棒性
...
### [126] [TinyGraphEstimator: Adapting Lightweight Language Models for Graph Structure Inference](https://arxiv.org/abs/2510.08808)
*Michal Podstawski*

Main category: cs.LG

TL;DR: 该论文研究了小型语言模型从文本图表示中推断图论参数的能力，通过轻量级微调实现了性能提升。


<details>
  <summary>Details</summary>
Motivation: 探索资源高效的小型语言模型在图结构推理任务中的潜力，因为目前主要关注大型模型，小型模型在此领域的研究相对不足。
...
Conclusion: 小型语言模型能够有效进行图结构推理任务，通过高效参数调优可以适应结构推断任务。
...
### [127] [Long-Tailed Recognition via Information-Preservable Two-Stage Learning](https://arxiv.org/abs/2510.08836)
*Fudong Lin,Xu Yuan*

Main category: cs.LG

TL;DR: 提出一种两阶段学习方法来缓解长尾数据分布中的多数类偏见问题，通过信息理论视角的表示学习和数学信息实例采样策略，在多个基准数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据分布通常存在不平衡（长尾）特性，导致深度分类模型偏向频繁类别，对尾部类别表现不佳。

Method: 两阶段方法：第一阶段从信息理论角度提出新的表示学习技术，理论上等价于最小化类内距离；第二阶段开发新的采样策略，选择数学信息丰富的实例来修正多数类偏见的决策边界。

Result: 在多个长尾基准数据集上实现了最先进的性能，通过广泛实验验证了方法的有效性。

Conclusion: 该方法能有效缓解多数类偏见倾向，同时保留数据集中的有价值信息，在长尾分类问题上表现出色。
...
### [128] [The Boundaries of Fair AI in Medical Image Prognosis: A Causal Perspective](https://arxiv.org/abs/2510.08840)
*Thai-Hoang Pham,Jiayuan Chen,Seungyeon Lee,Yuanlong Wang,Sayoko Moroi,Xueru Zhang,Ping Zhang*

Main category: cs.LG

TL;DR: FairTTE是首个用于评估医学影像中时间到事件预测公平性的综合框架，揭示了跨不同成像模式的普遍偏见问题，并发现现有公平性方法缓解效果有限。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习在医学影像分析中的广泛应用，人们担心算法可能对某些社会群体存在偏见。现有研究主要关注医学影像诊断任务，而忽视了涉及预测疾病进展和结果的预后场景。

Method: FairTTE框架整合了先进的时间到事件预测和公平性算法，利用因果分析技术来识别和量化医学影像数据集中嵌入的不同偏见来源，进行系统化和细粒度的公平性分析。

Result: 大规模评估显示偏见在不同成像模式中普遍存在，现有公平性方法缓解效果有限。研究发现基础偏见来源与模型差异之间存在强关联，且在分布变化下保持公平性变得更加困难。
...
### [129] [Titans Revisited: A Lightweight Reimplementation and Critical Analysis of a Test-Time Memory Model](https://arxiv.org/abs/2510.09551)
*Gavriel Di Nepi,Federico Siciliano,Fabrizio Silvestri*

Main category: cs.LG

TL;DR: 对Google Titans模型的轻量级重新实现和评估，发现其神经记忆组件能提升性能，但整体不总是优于基线方法
...
Method: 重新实现了Titans模型，并在掩码语言建模、时间序列预测和推荐任务上进行了全面评估

Result: Titans并不总是优于基线方法（由于分块处理），但其神经记忆组件相比仅使用注意力的模型能持续提升性能

Conclusion: 确认了该模型的创新潜力，同时指出了实际局限性，为未来研究提出了问题
...
### [130] [On the Alignment Between Supervised and Self-Supervised Contrastive Learning](https://arxiv.org/abs/2510.08852)
*Achleshwar Luthra,Priyadarsi Mishra,Tomer Galanti*

Main category: cs.LG

TL;DR: 本文分析了自监督对比学习(CL)与仅负样本监督对比学习(NSCL)在表示层面的对齐性，证明它们在训练过程中保持表示相似性，但参数空间会发散。


<details>
  <summary>Details</summary>
Motivation: 虽然已有理论表明CL损失近似于NSCL损失，但缺乏对表示层面是否对齐的研究，这有助于理解自监督与监督学习之间的联系。

Method: 在共享随机性条件下训练CL和NSCL模型，分析它们的相似性矩阵、表示对齐指标(CKA、RSA)，并研究参数空间的不稳定性。

Result: CL和NSCL的表示保持相似，对齐性随类别数、温度增加而改善，但参数空间会指数级发散。NSCL比其它监督目标更接近CL。

Conclusion: NSCL可作为自监督与监督学习之间的桥梁，表示层面的对齐性为理解对比学习机制提供了新视角。
...
### [131] [Time-Aware Feature Selection: Adaptive Temporal Masking for Stable Sparse Autoencoder Training](https://arxiv.org/abs/2510.08855)
*T. Ed Li,Junyu Ren*

Main category: cs.LG

TL;DR: 提出Adaptive Temporal Masking (ATM)方法解决稀疏自编码器中的特征吸收问题，通过动态调整特征选择来学习更稳定的可解释特征
...
### [132] [Sparse components distinguish visual pathways & their alignment to neural networks](https://arxiv.org/abs/2510.08858)
*Ammar I Marvi,Nancy G Kanwisher,Meenakshi Khosla*

Main category: cs.LG

TL;DR: 该论文通过稀疏分解方法识别人类视觉皮层三个流（腹侧、背侧、外侧）中的主要表征成分，并开发了稀疏成分对齐（SCA）方法来测量大脑与深度学习模型之间的表征对齐。


<details>
  <summary>Details</summary>
Motivation: 探索为什么在单任务上训练的深度神经网络能够很好地模拟整个视觉系统，尽管人类视觉皮层的不同流被认为具有不同的功能过程。

Method: 使用稀疏分解方法识别每个视觉流中的主要表征成分，并开发了稀疏成分对齐（SCA）方法来比较大脑与深度学习模型之间的表征对齐。

Result: 发现标准视觉深度神经网络与腹侧流的表征对齐度高于背侧流或外侧流，SCA方法比传统群体水平几何方法能更精细地揭示这些差异。

Conclusion: SCA提供了一种能够敏感捕捉系统潜在神经调谐轴的表征对齐测量方法，揭示了大脑与机器视觉系统之间的重要差异。
...
### [133] [Multi-fidelity Batch Active Learning for Gaussian Process Classifiers](https://arxiv.org/abs/2510.08865)
...
Method: 引入Bernoulli参数互信息(BPMI)算法，通过使用链接函数的一阶泰勒展开来规避概率空间中计算互信息的困难性

Result: 在两个合成测试案例和一个复杂的真实世界应用（激光点火火箭燃烧器模拟）中，BPMI在所有实验中表现出优越性能，在固定计算预算下获得更高的预测精度

Conclusion: BPMI是一种有效的多保真度高斯过程分类器批量主动学习算法，能够显著提高预测准确性
...
### [134] [Deep Multimodal Subspace Clustering Networks](https://arxiv.org/abs/1804.06498)
*Mahdi Abavisani,Vishal M. Patel*

Main category: cs.LG

TL;DR: 提出了基于卷积神经网络的非监督多模态子空间聚类方法，包含多模态编码器、自表达层和多模态解码器三个主要阶段，通过空间融合和亲和力融合技术显著提升了聚类性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决多模态数据在子空间聚类中的挑战，需要开发能够有效融合不同模态信息并保持数据自表达性的方法。

Method: 使用CNN框架，包含多模态编码器（早期、晚期和中间融合）、自表达层（获取亲和矩阵）和多模态解码器（重构输入数据），通过重构误差进行训练。

Result: 在三个数据集上的广泛实验表明，所提出的方法显著优于现有的最先进多模态子空间聚类方法。

Conclusion: 提出的基于CNN的多模态子空间聚类框架通过有效的融合策略和自表达性约束，在多模态聚类任务中取得了优越性能。
...
### [135] [An Improved Model-Free Decision-Estimation Coefficient with Applications in Adversarial MDPs](https://arxiv.org/abs/2510.08882)
*Haolin Liu,Chen-Yu Wei,Julian Zimmert*

Main category: cs.LG

TL;DR: 提出了Dig-DEC，一种无模型的决策估计系数，通过信息增益驱动探索，在对抗性环境中无需显式奖励估计器，改进了混合MDP的遗憾边界。


<details>
  <summary>Details</summary>
Motivation: 现有乐观DEC方法只能处理随机环境，无法扩展到对抗性环境，且存在模型类大小相关的遗憾边界差距。

Method: 引入Dig-DEC，移除乐观性，纯粹通过信息增益驱动探索，并改进了在线函数估计过程。

Result: Dig-DEC始终不大于乐观DEC，在特殊情况下更小；在混合MDP中获得了首个无模型遗憾边界；改进了估计误差，将遗憾边界从T^{3/4}提升到T^{2/3}（在线）和从T^{5/6}提升到T^{7/9}（离线）。

Conclusion: Dig-DEC是首个在Bellman完全MDP中达到与乐观方法匹配性能的DEC方法，解决了对抗性环境中的探索问题。
...
### [136] [Pinpointing crucial steps: Attribution-based Credit Assignment for Verifiable Reinforcement Learning](https://arxiv.org/abs/2510.08899)
*Junxi Yin,Haisen Luo,Zhenyu Li,Yihua Liu,Dan Liu,Zequn Li,Xiaohang Xu*

Main category: cs.LG

TL;DR: 提出了ACPO框架，通过难度感知课程学习和轨迹语义分割来平衡探索与利用，解决RLVR中的信用分配不准确和熵过早崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习与可验证奖励方法在平衡探索与利用方面存在困难，导致中间步骤信用分配不准确和熵过早崩溃，限制了模型性能。

Method: ACPO框架包含难度感知课程学习、轨迹语义分割、基于归因的表征来动态调节策略熵，以及因子化奖励系统来精确量化每个推理步骤的层次贡献。

Result: 在AIME、MATH和AMC等挑战性基准测试上的广泛实验表明，ACPO显著优于现有最先进方法。

Conclusion: ACPO通过平衡探索与利用，有效解决了RLVR中的关键问题，在复杂推理任务上取得了显著性能提升。
...
### [137] [A Frequency-Domain Analysis of the Multi-Armed Bandit Problem: A New Perspective on the Exploration-Exploitation Trade-off](https://arxiv.org/abs/2510.08908)
*Di Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种新颖的频域分析框架，将多臂老虎机问题重新表述为信号处理问题，证明了UCB算法中的置信边界项在频域中等效于对不确定频谱分量的时变增益。
...
### [138] [Velocity and Density-Aware RRI Analysis and Optimization for AoI Minimization in IoV SPS](https://arxiv.org/abs/2510.08911)
*Maoxin Ji,Tong Wang,Qiong Wu,Pingyi Fan,Nan Cheng,Wen Chen*

Main category: cs.LG

TL;DR: 提出基于LLM和DDPG的优化方法，解决车联网SPS中AoI恶化问题，通过双路径优化方案显著降低信息年龄。
...
Method: 建立受车辆速度、密度和资源预留间隔影响的AoI计算模型，设计双路径优化方案：DDPG使用状态空间和奖励函数指导，LLM利用上下文学习生成最优参数配置。

Result: 实验结果表明，LLM在积累少量样本后无需模型训练即可显著降低AoI，而DDPG方法在训练后实现更稳定的性能。

Conclusion: LLM和DDPG的双路径优化方法能有效解决车联网中的AoI问题，LLM提供快速优化能力，DDPG提供稳定性能。
...
### [139] [Simple and Robust Forecasting of Spatiotemporally Correlated Small Earth Data with A Tabular Foundation Model](https://arxiv.org/abs/2510.08920)
*Yuting Yang,Gang Mei,Zhengjing Ma,Nengxiong Xu,Jianbing Peng*

Main category: cs.LG

TL;DR: 提出了一种针对小地球数据的时空预测方法，通过表征和量化时空模式，利用表格基础模型实现跨场景的准确预测，在多个典型场景中优于传统深度学习和基础模型。


<details>
  <summary>Details</summary>
Motivation: 小地球数据具有有限的短期监测变异性，表现出时空相关性，但传统深度学习模型需要针对不同场景进行特定训练，而基础模型在预测时偏向预训练分布的全局均值。

Method: 表征和量化小地球数据的时空模式，然后利用表格基础模型进行跨场景的准确预测。

Result: 在三个典型场景的比较中，该方法在大多数情况下比图深度学习模型（T-GCN）和表格基础模型（TabPFN）具有更优的准确性和更强的鲁棒性。

Conclusion: 提出的方法为小地球数据的时空预测提供了一种简单而鲁棒的解决方案，能够有效应对不同场景的预测需求。
...
### [140] [AB-PINNs: Adaptive-Basis Physics-Informed Neural Networks for Residual-Driven Domain Decomposition](https://arxiv.org/abs/2510.08924)
*Jonah Botvinick-Greenhouse,Wael H. Ali,Mouhacine Benosman,Saviz Mowlavi*

Main category: cs.LG

TL;DR: 提出了自适应基物理信息神经网络（AB-PINNs），这是一种动态域分解方法，通过自适应调整子域和基于残差损失动态引入新子域来改进PINNs训练，特别适用于多尺度问题。


<details>
  <summary>Details</summary>
Motivation: 传统PINNs在处理多尺度问题时面临挑战，静态域分解方法需要大量超参数调优且容易陷入局部最小值。需要一种能够动态适应解的特征的域分解方法。

Method: 采用自适应域分解策略：1）现有子域动态适应解的固有特征；2）在训练过程中根据高残差损失区域动态引入新子域；3）不同子域学习捕捉解的不同尺度。

Result: AB-PINNs在多种复杂多尺度偏微分方程上表现出有效性，能够更好地表示挑战性区域的解，减少对超参数调优的依赖。

Conclusion: AB-PINNs提供了一种灵活的域分解方法，特别适合多尺度问题，能够防止收敛到不希望的局部最小值，并减少超参数调优需求。
...
### [141] [MATT-CTR: Unleashing a Model-Agnostic Test-Time Paradigm for CTR Prediction with Confidence-Guided Inference Paths](https://arxiv.org/abs/2510.08932)
*Moyu Zhang,Yun Chen,Yujun Jin,Jinxin Hu,Yu Zhang,Xiaoyi Zeng*

Main category: cs.LG

TL;DR: 提出了一种模型无关的测试时优化范式MATT，通过置信度引导的多推理路径生成来缓解低频特征组合对CTR预测的影响


<details>
  <summary>Details</summary>
Motivation: 现有CTR研究主要关注训练阶段优化，忽视了推理阶段的优化机会。不常出现的特征组合会降低预测性能，导致不可靠输出

Method: 使用分层概率哈希方法估计特征组合的出现频率作为置信度分数，以置信度为采样概率生成多个实例特定的推理路径，最后聚合多路径预测结果

Result: 离线和在线A/B测试都验证了MATT在现有CTR模型上的兼容性和有效性

Conclusion: MATT能够解锁已训练CTR模型的预测潜力，通过测试时优化提升预测的鲁棒性
...
### [142] [Bi-level Meta-Policy Control for Dynamic Uncertainty Calibration in Evidential Deep Learning](https://arxiv.org/abs/2510.08938)
*Zhen Yang,Yansong Ma,Lei Chen*

Main category: cs.LG

TL;DR: 提出了Meta-Policy Controller (MPC)框架，通过元学习动态调整KL散度系数和Dirichlet先验强度，解决传统证据深度学习在动态数据分布下不确定性校准不足的问题。


<details>
  <summary>Details</summary>
Motivation: 传统证据深度学习方法使用静态超参数进行不确定性校准，在动态数据分布下适应性差，导致高风险决策任务中的校准和泛化性能不佳。

Method: 采用双层优化方法：内层通过动态配置的损失函数更新模型参数；外层使用策略网络基于多目标奖励优化KL散度系数和类别特定的Dirichlet先验强度。

Result: 实验结果表明MPC显著提升了模型预测的可靠性和校准性，改善了不确定性校准、预测准确率以及基于置信度的样本拒绝后的性能保持。

Conclusion: MPC框架通过可学习的Dirichlet先验实现了对类别分布和训练动态的灵活适应，显著增强了证据深度学习的不确定性建模能力。
...
### [143] [Variability Aware Recursive Neural Network (VARNN): A Residual-Memory Model for Capturing Temporal Deviation in Sequence Regression Modeling](https://arxiv.org/abs/2510.08944)
*Haroon Gharwi,Kai Shu*

Main category: cs.LG

TL;DR: 提出VARNN模型，一种用于时间序列回归的残差感知神经网络架构，通过显式学习预测残差的误差记忆来重新校准后续预测，提高在非平稳、异方差时间序列数据上的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实世界时间序列数据存在非平稳性、状态转移和时变噪声等特性，这些会降低标准回归模型的鲁棒性，需要能够适应数据变异性的新方法。

Method: VARNN在传统前馈预测器基础上增加了学习的误差记忆状态，该状态从短上下文步长的预测残差中更新，作为变异性和漂移的信号，并用于调节当前时间步的最终预测。

Result: 在多个领域数据集（家电能耗、医疗健康、环境监测）上的实验表明，VARNN相比静态、动态和循环基线模型取得了更优的性能，测试MSE更低且计算开销最小。
...
### [144] [When LLM Agents Meet Graph Optimization: An Automated Data Quality Improvement Approach](https://arxiv.org/abs/2510.08952)
*Zhihan Zhang,Xunkai Li,Yilong Zuo,Zhenjun Li,Bing Zhou,Rong-Hua Li,Guoren Wang*

Main category: cs.LG

TL;DR: LAGA是一个统一的多智能体框架，通过数据中心的视角解决文本属性图的质量控制问题，在稀疏、噪声和不平衡等九种场景下显著提升图质量和学习性能。


<details>
  <summary>Details</summary>
Motivation: 传统GNN和LLM增强的GNN在图质量不佳时性能显著下降，现有方法主要关注模型架构改进而忽视图数据本身的系统优化，导致实际效果有限。

Method: 提出LAGA多智能体框架，包含检测、规划、行动和评估四个协作智能体，形成自动化闭环。行动智能体采用双编码器和三目标设计，捕捉跨模态互补信息并进行整体图质量增强。

Result: 在九种场景下的实验表明，LAGA显著提升图质量，在各种任务和骨干网络上实现最先进性能。
...
### [145] [Analytical Survey of Learning with Low-Resource Data: From Analysis to Investigation](https://arxiv.org/abs/2510.08962)
...
Method: 采用不可知主动采样理论和PAC框架分析低资源数据学习的泛化误差和标签复杂度，研究梯度信息优化、元迭代优化、几何感知优化和LLM驱动优化等策略。

Result: 分析了多种优化策略的有效性，并概述了能从低资源数据中受益的学习范式，包括领域迁移、强化反馈和层次结构建模。

Conclusion: 总结了关键发现，强调了这些发现对低资源数据学习的重要意义，为在有限数据条件下实现有效学习提供了理论指导和方法框架。
...
### [146] [HiBBO: HiPPO-based Space Consistency for High-dimensional Bayesian Optimisation](https://arxiv.org/abs/2510.08965)
*Junyu Xuan,Wenlong Chen,Yingzhen Li*

Main category: cs.LG

TL;DR: HiBBO是一个新的贝叶斯优化框架，通过将空间一致性引入VAE的潜在空间构建，使用HiPPO方法来减少潜在空间和原始空间之间的功能分布不匹配，在高维基准任务中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统基于VAE的贝叶斯优化方法存在重建目标函数导致潜在空间与原始空间功能分布不匹配的问题，从而影响优化性能。

Method: 提出HiBBO框架，在VAE潜在空间构建中引入空间一致性，使用HiPPO（长期序列建模方法）来减少功能分布不匹配。
...
### [147] [Learning Regularizers: Learning Optimizers that can Regularize](https://arxiv.org/abs/2510.08968)
*Suraj Kumar Sahoo,Narayanan C Krishnan*

Main category: cs.LG

TL;DR: 研究表明学习优化器能够学习并内化传统正则化技术，无需在目标函数中显式应用正则化，从而挑战了显式正则化的必要性。


<details>
  <summary>Details</summary>
Motivation: 探索学习优化器是否能够学习正则化效果，替代传统显式正则化方法如SAM、GAM、GSAM等。

Method: 在标准基准测试（MNIST、FMNIST、CIFAR）和神经网络（MLP、MLP-Relu、CNN）上进行广泛实验，比较使用和不使用显式正则化训练的学习优化器。

Result: 正则化学习优化器在测试准确率和泛化能力上始终优于未正则化的版本，并能将正则化效果迁移到新的优化任务中。

Conclusion: 学习优化器能够内在地学习正则化属性，挑战了传统显式正则化的必要性。
...
### [148] [Diagnosing and Mitigating System Bias in Self-Rewarding RL](https://arxiv.org/abs/2510.08977)
*Chuyi Tan,Peiwen Yuan,Xinglin Wang,Yiwei Li,Shaoxiong Feng,Yueqi Zhang,Jiayi Shi,Ji Zhang,Boyuan Pan,Yao Hu,Kan Li*

Main category: cs.LG

TL;DR: RLER通过集成多个模型并自适应奖励插值和轨迹选择，解决了RLIR中的系统偏差问题，在无标注数据上实现了稳定扩展，性能接近RLVR。


<details>
  <summary>Details</summary>
Motivation: 解决RLIR在无标注设置中存在的系统偏差问题，即模型倾向于高估其高置信度轨迹，导致奖励估计偏差和不稳定训练。

Method: 提出RLER方法，通过集成多个模型并自适应奖励插值和轨迹选择来缓解系统偏差。

Result: RLER相比RLIR提升了13.6%，仅比RLVR低3.6%，在无标注样本上实现了稳定扩展。

Conclusion: RLER能够有效缓解系统偏差，在无标注设置中实现接近RLVR的性能，具有很高的应用价值。
...
### [149] [FedL2T: Personalized Federated Learning with Two-Teacher Distillation for Seizure Prediction](https://arxiv.org/abs/2510.08984)
*Jionghao Lou,Jian Zhang,Zhongmei Li,Lanlan Chen,Enbo Feng*

Main category: cs.LG

TL;DR: FedL2T是一个用于癫痫预测的个性化联邦学习框架，通过双教师知识蒸馏策略解决客户端异构性问题，在隐私敏感的医疗场景中实现鲁棒性能。


<details>
  <summary>Details</summary>
Motivation: 癫痫预测需要大量标记EEG数据，但数据获取困难且存在隐私约束。现有联邦学习方法在客户端异构性下性能不佳。

Method: 提出双教师知识蒸馏策略：每个客户端同时学习全局聚合模型和动态分配的同伴模型，采用自适应多级蒸馏对齐预测输出和特征表示，并引入近端正则化增强训练稳定性。

Result: 在两个EEG数据集上的实验表明，FedL2T在低标记条件下始终优于最先进的联邦学习方法，收敛快速稳定，减少了通信轮次和开销。
...
### [150] [Constraints-of-Thought: A Framework for Constrained Reasoning in Language-Model-Guided Search](https://arxiv.org/abs/2510.08992)
*Kamel Alrashedy,Vriksha Srihari,Zulfiqar Zaidi,Ridam Srivastava,Pradyumna Tambwekar,Matthew Gombolay*

Main category: cs.LG

TL;DR: 提出了Constraints-of-Thought (Const-o-T)框架，通过(intent, constraint)对结构化表示来指导MCTS搜索，解决LLMs在多步规划中难以满足高层用户意图和符号约束的问题。


<details>
  <summary>Details</summary>
Motivation: 现有推理方法如CoT、ToT等扩展了搜索空间，但经常产生不可行动作或幻觉步骤，无法确保计划与高层用户意图对齐并满足符号约束。

Method: 使用(intent, constraint)对作为结构化先验，集成到MCTS中，通过约束修剪不可行分支并引导探索语义有效动作，提高规划效率和可验证决策。
...
### [151] [PlatformX: An End-to-End Transferable Platform for Energy-Efficient Neural Architecture Search](https://arxiv.org/abs/2510.08993)
*Xiaolong Tu,Dawei Chen,Kyungtae Han,Onur Altintas,Haoxin Wang*

Main category: cs.LG

TL;DR: PlatformX是一个完全自动化的硬件感知神经架构搜索框架，通过能量驱动的搜索空间、跨设备可转移的核级能量预测器、帕累托多目标搜索算法和高分辨率运行时能量分析系统，显著降低了搜索开销，同时保持准确性和能量保真度。


<details>
  <summary>Details</summary>
Motivation: 现有的硬件感知神经架构搜索方法由于高时间成本、大量手动分析和跨不同硬件平台的可扩展性差，在实际部署中仍然不实用。

Method: PlatformX集成了四个关键组件：能量驱动的搜索空间、跨设备可转移的核级能量预测器、帕累托多目标搜索算法和自动化高分辨率运行时能量分析系统。

Result: 在多个移动平台上评估显示，PlatformX显著减少了搜索开销，同时保持准确性和能量保真度，识别出的模型在准确性和效率上都优于MobileNet-V2。

Conclusion: PlatformX提供了一个实用且可扩展的解决方案，克服了现有硬件感知神经架构搜索方法的局限性，实现了高效的深度神经网络设计。
...
### [152] [SQS: Bayesian DNN Compression through Sparse Quantized Sub-distributions](https://arxiv.org/abs/2510.08999)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法单独进行权重剪枝或低比特量化往往导致次优压缩率，需要统一框架来同时优化这两个压缩维度。

Method: 使用spike-and-slab先验诱导稀疏性，采用高斯混合模型建模量化权重以实现低比特精度，基于贝叶斯变分学习统一框架。

Result: 在ResNet、BERT-base、Llama3和Qwen2.5模型上的实验表明，该方法比一系列现有方法获得更高压缩率且性能下降相当。

Conclusion: SQS框架成功实现了同时剪枝和量化的统一优化，在保持性能的同时显著提升了神经网络压缩率。
...
### [153] [LLM Unlearning on Noisy Forget Sets: A Study of Incomplete, Rewritten, and Watermarked Data](https://arxiv.org/abs/2510.09007)
*Changsheng Wang,Yihua Zhang,Dennis Wei,Jinghan Jia,Pin-Yu Chen,Sijia Liu*

Main category: cs.LG

TL;DR: 该研究首次探讨了在噪声遗忘数据集（即经过扰动或低保真度的遗忘数据）下的大语言模型遗忘问题，发现遗忘方法对扰动具有惊人鲁棒性，只要核心语义信号得以保留。


<details>
  <summary>Details</summary>
Motivation: 现有遗忘方法假设能获得干净、定义明确的遗忘数据样本，但现实中的遗忘数据往往是低质量、合成改写或带有水印的，这引发了遗忘可靠性的担忧。

Method: 系统地评估了最先进的LLM遗忘方法RMU和NPO在噪声遗忘集上的表现，并提出基于显著性的解释框架来分析鲁棒性机制。

Result: 研究发现遗忘对扰动具有显著鲁棒性，只要核心语义信号得以保留；遗忘算法主要受深层语义线索而非浅层词汇模式引导。

Conclusion: 遗忘方法在噪声遗忘数据下仍能可靠工作，关键语义成分在表面形式大幅变化时仍保持一致的遗忘驱动作用。
...
### [154] [Value-State Gated Attention for Mitigating Extreme-Token Phenomena in Transformers](https://arxiv.org/abs/2510.09017)
*Rui Bu,Haofeng Zhong,Wenzheng Chen,Yangyan Li*

Main category: cs.LG

TL;DR: 提出Value-State Gated Attention (VGA)机制，通过基于值向量的可学习门控来有效解决Transformer模型中的极端token现象（注意力汇聚和值状态流失），提升模型性能、量化保真度和可解释性。


<details>
  <summary>Details</summary>
Motivation: Transformer架构的大模型容易受到极端token现象的影响，包括注意力汇聚和值状态流失。这些问题源于模型学习了一种低效的'无操作'行为，即关注具有接近零值状态的token，导致模型性能下降、量化保真度降低和可解释性变差。

Method: 提出Value-State Gated Attention (VGA)，引入一个基于值向量(V)计算的可学习、数据依赖的门控机制来调节输出。通过理论分析证明，用值状态自身的函数来门控值状态比先前基于输入嵌入的门控方法更有效地解耦值和注意力分数的更新。

Result: 实验表明VGA显著减轻了注意力汇聚的形成并稳定了值状态范数，带来了改进的性能、鲁棒的量化保真度和增强的模型可解释性。

Conclusion: VGA通过直接打破问题循环，提供了一种简单、专用且稳定的架构机制来高效执行'无操作'注意力，有效解决了Transformer模型中的极端token问题。
...
### [155] [Slim Scheduler: A Runtime-Aware RL and Scheduler System for Efficient CNN Inference](https://arxiv.org/abs/2510.09018)
...
Method: 采用分层设计：本地贪心调度器处理请求批处理和实例缩放，PPO路由器学习全局路由策略（设备选择、宽度比例、批配置）。

Result: 相比随机任务分配基线，在精度降至最瘦模型(70.3%)时，平均延迟减少96.45%，能耗减少97.31%；在提高精度时也能减少平均延迟和能耗，但延迟和能耗的标准差增加。
...
### [156] [MagicDock: Toward Docking-oriented De Novo Ligand Design via Gradient Inversion](https://arxiv.org/abs/2510.09020)
*Zekai Chen,Xunkai Li,Sirui Zhang,Henan Sun,Jia Li,Zhenjun Li,Bing Zhou,Rong-Hua Li,Guoren Wang*

Main category: cs.LG

TL;DR: MagicDock是一个基于渐进式流程和可微分表面建模的从头配体设计框架，通过梯度反演和可学习3D点云表示来解决现有方法的局限性，在9个场景中相比最先进方法平均提升27.1%和11.7%。


<details>
  <summary>Details</summary>
Motivation: 现有从头配体设计方法存在伪从头设计、有限对接建模和配体类型不灵活的局限性，需要开发更有效的框架来解决这些问题。

Method: 采用梯度反演框架，将受体和配体的通用对接知识融入骨干模型，通过结合预测实例化为反向梯度流来迭代指导配体生成；使用可学习3D点云表示进行可微分表面建模；为不同配体类型提供定制化设计并集成到统一的梯度反演框架中。

Result: 在9个场景的广泛实验中，MagicDock相比专门用于蛋白质或分子配体设计的最先进基线方法分别实现了27.1%和11.7%的平均改进。

Conclusion: MagicDock通过渐进式流程和可微分表面建模有效解决了从头配体设计中的关键挑战，提供了理论保证并在多个场景中表现出优越性能。
...
### [157] [The Environmental Impacts of Machine Learning Training Keep Rising Evidencing Rebound Effect](https://arxiv.org/abs/2510.09022)
*Clément Morand,Anne-Laure Ligozat,Aurélie Névéol*

Main category: cs.LG

TL;DR: 该研究分析了近十年来AI系统训练的环境影响，发现即使采用优化策略，机器学习模型训练的环境影响仍在指数级增长，存在反弹效应。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习模型性能提升，计算需求激增，导致能源消耗和环境问题日益严重。研究旨在评估各种优化策略是否能实现可持续的ML模型训练。

Method: 通过估算过去十年中包括大语言模型在内的著名AI系统训练的环境影响，重点关注显卡的整个生命周期分析。

Result: 发现显卡生产的环境影响持续增长，ML模型训练的能耗和环境影响呈指数级增长，优化策略无法有效缓解训练带来的影响，存在反弹效应。

Conclusion: 仅靠提高效率无法确保ML的可持续性，必须减少AI活动规模、质疑资源密集型训练的频次，并考虑硬件的全生命周期影响而非仅使用阶段。
...
### [158] [The Attacker Moves Second: Stronger Adaptive Attacks Bypass Defenses Against Llm Jailbreaks and Prompt Injections](https://arxiv.org/abs/2510.09023)
...
TL;DR: 该论文认为当前语言模型防御的评估方法存在缺陷，应使用自适应攻击者进行测试。通过系统优化多种攻击方法，成功绕过了12种近期防御，攻击成功率超过90%。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型防御评估主要针对静态攻击集或计算能力较弱的优化方法，这种评估方式存在缺陷，无法真实反映防御的鲁棒性。

Method: 系统调整和扩展通用优化技术，包括梯度下降、强化学习、随机搜索和人工引导探索，针对12种不同的防御技术进行攻击测试。

Result: 成功绕过了所有12种近期防御，大多数防御的攻击成功率超过90%，而原论文报告的攻击成功率接近零。
...
### [159] [Convergence of optimizers implies eigenvalues filtering at equilibrium](https://arxiv.org/abs/2510.09034)
*Jerome Bolte,Quoc-Tung Le,Edouard Pauwels*

Main category: cs.LG

TL;DR: 本文从假设优化器收敛到任意点的视角出发，论证不同优化器通过其超参数充当特征值过滤器，梯度下降天然避免最尖锐极小值，而SAM算法更倾向于宽盆地。作者提出两种新算法增强特征值过滤效果。


<details>
  <summary>Details</summary>
Motivation: 基于深度神经网络训练中多种优化器都能找到接近全局最优解的实证观察，作者采用逆向思维，假设收敛到任意点成立，研究这种假设的后果，特别是与边缘稳定性现象相关的优化器行为。

Method: 使用广义Hadamard-Perron稳定流形定理，对一般半代数C²函数进行理论分析，无需额外非退化条件或全局Lipschitz边界假设。提出两种增强特征值过滤的新算法，并通过前馈神经网络进行数值实验验证。

Result: 理论分析表明梯度下降天然避免最尖锐极小值，SAM算法更主动偏向宽盆地。提出的新算法能有效促进更宽的极小值，数值实验支持这一结论。

Conclusion: 不同优化器通过超参数配置充当特征值过滤器，梯度下降和SAM算法在避免尖锐极小值方面具有不同效果，新提出的算法能进一步增强这种过滤效果，有利于找到更宽的优化解。
...
### [160] [Robust Driving Control for Autonomous Vehicles: An Intelligent General-sum Constrained Adversarial Reinforcement Learning Approach](https://arxiv.org/abs/2510.09041)
*Junchao Fan,Xiaolin Chang*

Main category: cs.LG

TL;DR: 提出IGCARL方法解决深度强化学习在自动驾驶中的对抗攻击脆弱性问题，通过战略性对抗者和约束优化提升鲁棒性
...
### [161] [Spatio-Temporal Graph Convolutional Networks for EV Charging Demand Forecasting Using Real-World Multi-Modal Data Integration](https://arxiv.org/abs/2510.09048)
*Jose Tupayachi,Mustafa C. Camur,Kevin Heaslip,Xueping Li*

Main category: cs.LG

TL;DR: 提出TW-GCN时空预测框架，结合图卷积网络和时间架构预测田纳西州电动汽车充电需求，发现3小时预测在响应性和稳定性间达到最佳平衡，1DCNN表现最优。


<details>
  <summary>Details</summary>
Motivation: 交通是温室气体排放主要来源，需向电动汽车转型，但充电基础设施空间分布不均和使用不规律对电网稳定性和投资规划构成挑战。

Method: 使用图卷积网络结合时间架构，利用真实交通流量、天气条件和美国最大EV基础设施公司提供的专有数据，捕捉空间依赖性和时间动态。

Result: 实验表明3小时中期预测表现最佳，1DCNN始终优于其他时间模型，区域分析显示东、中、西田纳西预测精度存在差异。
...
### [162] [Improving Anomaly Detection in Industrial Time Series: The Role of Segmentation and Heterogeneous Ensemble](https://arxiv.org/abs/2510.09079)
*Emilio Mastriani,Alessandro Costa,Federico Incardona,Kevin Munari,Sebastiano Spinello*

Main category: cs.LG

TL;DR: 该论文探讨了将时间序列分割技术与异构集成相结合，以提升工业生产环境中异常检测的性能。通过使用ChangeFinder等变化点检测算法进行预分割，再结合随机森林和XGBoost等集成算法，显著提高了AUC-ROC指标。


<details>
  <summary>Details</summary>
Motivation: 在工业生产环境中，准确检测异常状态变化对于预防故障和提高生产效率至关重要。时间序列分割技术能够识别状态变化，减少时间不确定性，但如何将其与机器学习算法有效结合以提升异常检测性能仍需探索。

Method: 采用变化点检测（特别是ChangeFinder算法）作为预处理步骤对时间序列进行分割，然后使用异构集成算法（包括随机森林和XGBoost）进行异常检测。

Result: 通过分割预处理结合异构集成的方法，AUC-ROC指标从0.8599（PCA和LSTM集成）显著提升至0.9760（随机森林和XGBoost集成），证明了该方法在减少时间模糊性和促进监督算法学习方面的有效性。

Conclusion: 时间序列分割作为预处理步骤与异构集成算法的结合，能够有效提升工业异常检测性能。未来工作将探索从变化点研究中提取加权特征，进一步优化早期异常检测模型的性能。
...
### [163] [FLToP CTC: Frame-Level Token Pruning via Relative Threshold for Efficient and Memory-Saving Decoding on Diverse Platforms](https://arxiv.org/abs/2510.09085)
*Atul Shree,Harshith Jupuru*

Main category: cs.LG

TL;DR: FLToP CTC是一种基于帧级token剪枝的CTC解码算法，通过动态剪除低概率token来显著降低计算和内存需求，在LibriSpeech上实现10.5倍运行速度提升和2.78倍内存减少，同时保持WER性能基本不变。


<details>
  <summary>Details</summary>
Motivation: CTC-based ASR系统在资源受限环境中面临计算和内存瓶颈，传统CTC解码器在处理过程中消耗高达90%的时间，主要由于详尽的token级操作导致效率低下。

Method: 提出帧级token剪枝(FLToP CTC)解码算法，采用基于相对阈值概率的帧级token剪枝策略，动态消除每帧中的低概率token。

Result: 在LibriSpeech数据集上，FLToP CTC相比标准CTC解码器实现了10.5倍的运行时加速和2.78倍的内存减少，同时WER退化可忽略不计。
...
### [164] [Neural Codecs as Biosignal Tokenizers](https://arxiv.org/abs/2510.09095)
...
<details>
  <summary>Details</summary>
Motivation: 神经生理记录如EEG产生高维、噪声时间序列数据，需要大量预处理和手工特征提取。现有方法难以有效解码和解释生物信号。

Method: 提出BioCodec框架，受神经编解码器启发，将生物信号的低级特征捕获为离散标记。在数千小时EEG数据上预训练。

Result: BioCodec在临床诊断、睡眠生理、语音解码和运动想象等多个任务中表现优异，特别是在低资源设置下。方法也适用于EMG等其他生物信号。

Conclusion: BioCodec为生物信号标记化提供了通用解决方案，性能与最先进模型相当。源代码和模型检查点已共享。
...
### [165] [AdaPM: a Partial Momentum Algorithm for LLM Training](https://arxiv.org/abs/2510.09103)
*Yimu Zhang,Yuanshi Liu,Cong Fang*

Main category: cs.LG

TL;DR: 提出AdaPM自适应训练策略，使用部分动量实现内存高效优化器，通过非均匀动量设计和偏置校正技术，在保持性能的同时减少90%以上的动量内存使用。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型训练中动量广泛使用但内存消耗大，需要解决动量存储带来的内存挑战。

Method: 采用非均匀动量设计，对大多数块不使用完整动量，通过偏置校正技术增强部分动量以减少偏差和性能损失。

Result: 在60M到1.5B参数的语言模型预训练、监督微调和RLHF中，减少90%以上动量内存，结合二阶统计量技术可进一步减少95%优化器状态内存，为GPT-2 1.5B预训练节省30%以上GPU小时。

Conclusion: AdaPM是一种有效的内存高效优化器，能在保持训练效率和性能的同时显著减少内存使用。
...
### [166] [MemLoss: Enhancing Adversarial Training with Recycling Adversarial Examples](https://arxiv.org/abs/2510.09105)
*Soroush Mahdi,Maryam Amirmazlaghani,Saeed Saravani,Zahra Dehghanian*

Main category: cs.LG

TL;DR: 提出MemLoss方法，通过利用历史生成的对抗样本来改进对抗训练，在保持自然准确率的同时提升模型鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有对抗训练方法在提升模型鲁棒性时往往会牺牲自然准确率，需要一种平衡两者性能的改进方法

Method: 使用历史生成的对抗样本（记忆对抗样本）在多个训练周期中重复利用，增强模型学习效果

Result: 在CIFAR-10等多个数据集上实验表明，MemLoss相比现有方法获得更好的准确率，同时保持强大的对抗鲁棒性
...
### [167] [On the Fairness of Privacy Protection: Measuring and Mitigating the Disparity of Group Privacy Risks for Differentially Private Machine Learning](https://arxiv.org/abs/2510.09114)
*Zhi Yang,Changwu Huang,Ke Tang,Xin Yao*

Main category: cs.LG

TL;DR: 本文提出了一种新的成员推理游戏来高效审计数据记录的近似最坏情况隐私风险，并改进了DP-SGD算法，通过自适应分组梯度裁剪策略减少群体隐私风险差异，提升DPML中隐私保护的公平性。


<details>
  <summary>Details</summary>
Motivation: 传统公平性机器学习和差分隐私机器学习中，群体间隐私保护的公平性研究不足。现有方法基于平均隐私风险评估，可能低估群体隐私风险差异，且最坏情况隐私风险评估方法耗时，限制了实际应用。

Method: 1. 引入新的成员推理游戏来高效审计近似最坏情况隐私风险；2. 在标准DP-SGD算法基础上，采用自适应分组梯度裁剪策略，灵感来自差分隐私审计中的金丝雀设计。

Result: 实验结果表明：1. 新方法能提供更严格的群体隐私风险评估，可靠地评估群体隐私风险差异；2. 改进的算法有效减少了群体隐私风险差异，提升了DPML中隐私保护的公平性。

Conclusion: 本文提出的方法能够更准确地评估群体隐私风险差异，并通过改进的DP-SGD算法有效提升差分隐私机器学习中隐私保护的公平性。
...
### [168] [Regret Bounds for Adversarial Contextual Bandits with General Function Approximation and Delayed Feedback](https://arxiv.org/abs/2510.09127)
...
<details>
  <summary>Details</summary>
Motivation: 在现实世界的在线学习场景中，反馈通常会有延迟，而且延迟可能由对手选择。现有的上下文多臂老虎机算法大多假设即时反馈，因此需要开发能够处理延迟反馈的算法。

Method: 1. 对于有限策略类，直接访问策略类并建立最优后悔界；2. 对于函数逼近设置，使用在线最小二乘回归预言机，并基于Hedge版本的Vovk聚合预测器进行稳定性分析。

Result: 1. 有限策略类：获得O(√(KT log|Π|) + √(D log|Π|))的期望后悔界；2. 函数逼近：获得O(√(KTℛ_T(O)) + √(d_max D β))的后悔界，其中β ≤ log|F|，最终为O(√(KT log|F|) + √(d_max D log|F|))。

Conclusion: 本文为延迟反馈的上下文多臂老虎机问题提供了有效的算法和理论保证，在有限策略类下达到最优，在函数逼近设置下与下界相差√d_max因子，为处理现实世界中的延迟反馈问题提供了重要工具。
...
### [169] [Score-Based Density Estimation from Pairwise Comparisons](https://arxiv.org/abs/2510.09146)
*Petrus Mikkola,Luigi Acerbi,Arto Klami*

Main category: cs.LG

TL;DR: 该论文提出了一种从成对比较中估计密度的方法，通过将未观测的目标密度与经过温度调节的获胜者密度相关联，利用分数匹配学习获胜者的分数，然后通过'去温度调节'来估计目标密度。
...
Method: 将未观测的目标密度与温度调节的获胜者密度相关联，通过分数匹配学习获胜者的分数，然后通过'去温度调节'估计目标密度。使用基于Bradley-Terry模型的估计器，并通过分数缩放的退火Langevin动力学生成温度调节样本训练扩散模型。

Result: 该方法能够从仅数百到数千个成对比较中学习模拟专家的复杂多元信念密度。

Conclusion: 证明了信念密度和获胜者密度的分数向量是共线的，通过位置相关的温度调节场连接，并给出了该场的解析公式和估计器。
...
### [170] [Logits Replay + MoClip: Stabilized, Low-Cost Post-Training with Minimal Forgetting](https://arxiv.org/abs/2510.09152)
*Suming Qiu,Jing Li,Zhicheng Zhou,Junjie Huang,Linyuan Qiu,Zhijie Sun*

Main category: cs.LG

TL;DR: 提出Logits Replay + MoClip两阶段框架，通过压缩logit空间的监督信号和稳定优化更新，在提升领域性能的同时减轻通用能力遗忘，并降低40%以上训练成本。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在领域适应中面临的专门化与通用能力之间的权衡问题，现有方法在计算、数据访问或适应性方面成本较高。

Method: 两阶段框架：Stage 0记录动态Top-K token子集（包含黄金标签）；Stage 1重放这些紧凑子集计算精确重归一化损失。同时设计MoClip优化器，限制梯度动量旋转并基于arctan2重新缩放更新。
...
### [171] [Agentic-KGR: Co-evolutionary Knowledge Graph Construction through Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2510.09156)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于知识增强的大语言模型依赖静态知识库，存在覆盖范围不足和时效性差的问题，限制了在动态信息环境中的有效性。

Method: 提出三个关键创新：动态模式扩展机制、检索增强记忆系统、可学习的多尺度提示压缩方法，通过多轮强化学习实现LLM与KG的协同进化。

Result: 实验结果显示在知识抽取任务中显著优于监督基线和单轮强化学习方法，与GraphRAG集成后在问答任务中取得更好的准确性和知识覆盖。
...
### [172] [Cross-Representation Benchmarking in Time-Series Electronic Health Records for Clinical Outcome Prediction](https://arxiv.org/abs/2510.09159)
*Tianyi Chen,Mingcheng Zhu,Zhiyao Luo,Tingting Zhu*

Main category: cs.LG

TL;DR: 本文提出了首个系统性的EHR表示方法基准测试，比较了多变量时间序列、事件流和文本事件流三种范式，发现事件流模型表现最佳，并揭示了不同临床场景下特征选择策略的差异。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录(EHRs)支持临床预测的深度学习应用，但由于评估实践不一致，患者数据的最佳表示方法仍不明确。

Method: 在两个不同临床设置中标准化数据整理和评估：MIMIC-IV数据集用于ICU任务，EHRSHOT数据集用于纵向护理。评估了三种表示范式的适当建模家族，包括Transformer、MLP、LSTM、Retain等。

Result: 事件流模型始终提供最强性能；预训练模型在少样本设置中样本效率高；ICU预测中修剪稀疏特征有益，而纵向任务中保留这些特征至关重要。

Conclusion: 通过统一且可复现的流程，结果为基于临床背景和数据机制选择EHR表示提供了实用指导。
...
### [173] [Efficient Resource-Constrained Training of Vision Transformers via Subspace Optimization](https://arxiv.org/abs/2510.09160)
*Le-Trung Nguyen,Enzo Tartaglione,Van-Tam Nguyen*

Main category: cs.LG

TL;DR: 提出WASI方法，通过将训练限制在固定子空间来减少transformer模型的内存使用和计算成本，同时保持与标准训练相当的准确性


<details>
  <summary>Details</summary>
Motivation: 随着AI在日常生活中的应用增加，能耗和数据隐私成为重要问题。设备端学习能降低能耗并保护数据隐私，但现代神经网络的规模扩大成为设备端训练的主要障碍

Method: 应用基于子空间的训练方法到transformer模型，提出Weight-Activation Subspace Iteration (WASI)，通过将训练限制在模型关键信息所在的固定子空间来缓解反向传播的内存瓶颈

Result: WASI在保持与标准训练相当准确性的同时，将内存使用减少高达62倍，计算成本(FLOPs)减少高达2倍。在Raspberry Pi 5上，训练和推理速度比标准训练快约1.5倍

Conclusion: WASI方法有效解决了transformer模型在设备端训练时的内存和计算效率问题，为边缘设备上的高效AI训练提供了可行方案
...
### [174] [Robustness and Regularization in Hierarchical Re-Basin](https://arxiv.org/abs/2510.09174)
...
<details>
  <summary>Details</summary>
Motivation: 深入分析Git Re-Basin这一新的模型合并方法，探索更有效的模型合并策略，并验证其实际效果。

Method: 提出分层模型合并方案，与标准MergeMany算法进行对比，通过实验评估合并模型的性能和鲁棒性。

Result: 分层合并方案显著优于标准算法，Re-Basin能为合并模型带来对抗性和扰动鲁棒性，且参与合并的模型越多效果越强，但性能下降比原作者报告的要大。

Conclusion: Git Re-Basin方法在鲁棒性方面有优势，但实际性能损失比预期更大，分层合并策略能有效提升合并效果。
...
### [175] [Beyond Pairwise Connections: Extracting High-Order Functional Brain Network Structures under Global Constraints](https://arxiv.org/abs/2510.09175)
*Ling Zhan,Junjie Huang,Xiaoyao Yu,Wenyu Chen,Tao Jia*

Main category: cs.LG

TL;DR: 提出GCM框架，通过全局约束学习功能脑网络的高阶结构，在多个数据集上显著提升准确率和计算效率


<details>
  <summary>Details</summary>
Motivation: 传统功能脑网络建模依赖局部成对交互，无法捕捉高阶依赖关系；现有超图建模方法计算负担重且依赖启发式方法，难以端到端学习网络结构

Method: 提出GCM框架，整合4种全局约束（信号同步、被试身份、期望边数、数据标签），支持4个建模分辨率（样本/被试/组/项目）的功能脑网络结构学习

Result: 在5个数据集和2种任务设置下，相比9个基准方法和10个SOTA方法，相对准确率提升30.6%，计算时间减少96.3%

Conclusion: 为功能脑网络结构学习提供了新视角，为认知神经科学的跨学科应用奠定了基础
...
### [176] [On the Implicit Adversariality of Catastrophic Forgetting in Deep Continual Learning](https://arxiv.org/abs/2510.09181)
*Ze Peng,Jian Zhang,Jintao Guo,Lei Qi,Yang Gao,Yinghuan Shi*

Main category: cs.LG

TL;DR: 本文揭示了持续学习中灾难性遗忘的根本原因：新任务训练本质上是对旧任务知识的对抗性攻击，新任务梯度会自动与旧任务损失函数的尖锐方向对齐。


<details>
  <summary>Details</summary>
Motivation: 理解深度网络中灾难性遗忘的深层原因，揭示其对抗性本质，为开发更有效的缓解方法提供理论基础。

Method: 通过理论分析发现训练的低秩偏差导致新旧任务梯度在同一低维子空间中对齐，提出backGP方法解决反向传播引起的对齐问题。

Result: backGP方法相比梯度投影方法平均减少10.8%的遗忘，提高12.7%的准确率。

Conclusion: 灾难性遗忘源于新任务训练对旧任务的对抗性攻击，backGP能有效缓解这一问题。
...
### [177] [RepDL: Bit-level Reproducible Deep Learning Training and Inference](https://arxiv.org/abs/2510.09180)
*Peichen Xie,Xian Zhang,Shuo Chen*

Main category: cs.LG

TL;DR: RepDL是一个开源库，通过强制正确舍入和顺序不变性来解决深度学习中的非确定性和不可重现性问题，确保跨计算环境的比特级可重现训练和推理。
...
Result: RepDL能够确保深度学习训练和推理在不同计算环境中的比特级可重现性。

Conclusion: RepDL有效解决了深度学习中的浮点计算不一致性问题，提供了跨平台的确定性训练和推理能力。
...
### [178] [Multimodal Prompt Optimization: Why Not Leverage Multiple Modalities for MLLMs](https://arxiv.org/abs/2510.09201)
...
<details>
  <summary>Details</summary>
Motivation: 现有的提示优化方法仅限于文本，限制了多模态大语言模型充分发挥潜力，因此需要将提示优化扩展到多模态空间。
...
### [179] [FM-IRL: Flow-Matching for Reward Modeling and Policy Regularization in Reinforcement Learning](https://arxiv.org/abs/2510.09222)
*Zhenglin Wan,Jingxuan Wu,Xingrui Yu,Chubin Zhang,Mingcong Lei,Bo An,Ivor Tsang*

Main category: cs.LG

TL;DR: 提出了一种结合流匹配模型和强化学习的方法，通过简单MLP学生策略进行在线探索，利用教师FM模型提供奖励信号和行为正则化，解决FM策略在线优化困难的问题。


<details>
  <summary>Details</summary>
Motivation: 流匹配模型在模仿学习中表现优异，但缺乏环境交互和探索能力，导致在未见场景中泛化能力差。直接在线优化FM策略存在梯度不稳定和高推理成本的问题。

Method: 使用简单MLP结构的学生策略进行在线探索，通过强化学习算法更新。奖励模型基于教师FM模型构建，包含专家数据分布信息。同时利用教师FM模型对学生策略行为进行正则化以稳定学习过程。

Result: 实验表明该方法显著提高了学习效率、泛化能力和鲁棒性，特别是在从次优专家数据学习时表现突出。

Conclusion: 通过将FM模型的表达能力与简单MLP策略的在线探索能力相结合，实现了高效稳定的策略学习，解决了FM策略在线优化的挑战。
...
### [180] [Prime Implicant Explanations for Reaction Feasibility Prediction](https://arxiv.org/abs/2510.09226)
...
Result: 初步实验表明，质蕴涵解释保守地捕捉了真实解释，虽然常包含冗余的键和原子，但始终捕捉到预测反应可行性的关键分子属性

Conclusion: 该方法能够有效识别化学反应预测中的关键分子特征，提高模型的可解释性
...
### [181] [Incentivizing Time-Aware Fairness in Data Sharing](https://arxiv.org/abs/2510.09240)
*Jiangwei Chen,Kieu Thao Nguyen Pham,Rachael Hwee Ling Sim,Arun Verma,Zhaoxuan Wu,Chuan-Sheng Foo,Bryan Kian Hsiang Low*

Main category: cs.LG

TL;DR: 提出了一个公平且时间感知的数据共享框架，为早期加入协作的参与者提供更高奖励，以激励数据共享


<details>
  <summary>Details</summary>
Motivation: 现有协作框架假设所有参与者同时加入，但现实中参与者可能在不同时间加入，早期加入者承担更高风险且能鼓励观望者参与，因此应获得更高奖励

Method: 开发了时间感知激励机制，提出决定奖励价值的新方法，并展示如何生成实现这些奖励价值的模型奖励

Result: 在合成和真实数据集上实证验证了所提出方法的特性

Conclusion: 时间感知激励机制能够有效激励数据共享，为早期参与者提供公平补偿，促进协作机器学习
...
### [182] [A PCA-based Data Prediction Method](https://arxiv.org/abs/2510.09246)
...
Method: 使用现有数据集的第一主成分张成的子空间来表示数据，基于移位线性子空间之间的距离概念，提供了欧几里得度量下的解决方案。

Result: 开发了一种结合传统数学和机器学习元素的新颖缺失数据预测方法。

Conclusion: 该方法为缺失数据预测提供了一种基于子空间距离的有效解决方案，在欧几里得度量下具有可行性。
...
TL;DR: 该论文填补了多智能体模仿学习(MAIL)的理论空白，证明了非交互式MAIL的统计下界，并提出首个具有接近最优样本复杂度的交互式算法MAIL-WARM，将样本复杂度从O(ε⁻⁸)提升到O(ε⁻²)。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体模仿学习中存在的理论空白，特别关注非交互式和交互式两种设置下的样本复杂度问题，以及行为克隆在某些环境中的失败情况。

Method: 提出了一个结合无奖励强化学习与交互式MAIL的框架，并实例化为MAIL-WARM算法。在非交互式设置中分析了行为克隆的局限性，在交互式设置中设计了改进的样本效率算法。

Result: 证明了非交互式MAIL的统计下界，识别出全策略偏差集中系数为基本复杂度度量；MAIL-WARM算法将样本复杂度从O(ε⁻⁸)显著提升到O(ε⁻²)，匹配理论下界；数值实验验证了理论结果。

Conclusion: 该工作为多智能体模仿学习提供了完整的理论框架，证明了行为克隆在非交互式设置中的最优性，并提出了在交互式设置中达到接近最优样本复杂度的实用算法。
...
### [184] [Mitigating Model Drift in Developing Economies Using Synthetic Data and Outliers](https://arxiv.org/abs/2510.09294)
...
<details>
  <summary>Details</summary>
Motivation: 金融机器学习模型容易因数据分布变化而性能下降，这一问题在发展中经济体（如中亚和高加索地区）尤为严重，因为这些地区经常面临不可预测的宏观经济冲击。

Method: 研究采用合成异常值的方法来提高模型对未知冲击的稳定性，并引入了一个两级评估框架来衡量性能下降程度和冲击严重性。

Result: 在宏观经济表格数据集上的实验表明，添加少量合成异常值通常能提高模型稳定性，但最优添加量因数据集和模型而异。

Conclusion: 合成异常值是一种有效的模型漂移缓解方法，特别是在面临频繁宏观经济冲击的发展中经济体金融数据中，但需要根据具体场景调整使用策略。
...
### [185] [The Potential of Second-Order Optimization for LLMs: A Study with Full Gauss-Newton](https://arxiv.org/abs/2510.09378)
*Natalie Abreu,Nikhil Vyas,Sham Kakade,Depen Morwani*

Main category: cs.LG

TL;DR: 本文通过应用完整高斯-牛顿预条件处理，在150M参数规模的Transformer模型上建立了迭代复杂度的实际上限，相比SOAP和Muon等基线方法减少了5.4倍训练迭代次数。
...
Method: 应用完整高斯-牛顿预条件处理到Transformer模型，并与精确的逐层高斯-牛顿预条件器进行比较实验。

Result: 完整GN更新相比现有优化器带来显著收益，逐层GN预条件器几乎能达到完整GN方法的性能。
...
### [186] [Large Language Model Prompt Datasets: An In-depth Analysis and Insights](https://arxiv.org/abs/2510.09316)
*Yuanming Zhang,Yan Lin,Arijit Khan,Huaiyu Wan*

Main category: cs.LG

TL;DR: 本文首次系统收集了来自多个渠道的提示数据集，涵盖多种任务、语言和模态，并提出了基于句法嵌入的提示优化方法，通过将提示重写向中心表示来提升模型输出的意义性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的广泛部署，来自GitHub和社交媒体等平台的多样化提示数据集不断涌现，这些数据集促进了LLM的广泛应用和提示工程改进。但目前缺乏对这些数据集的系统性分析。

Method: 收集了广泛的提示数据集，选择代表性数据集进行系统分析，并提出基于词性标注和依存结构句法嵌入的提示优化方法，通过识别提示的中心表示并引导LLM将提示重写向该中心来优化提示。

Result: 揭示了不同类别提示构建的共同点和差异，与其他文本语料库（如文学和网页）区分开来，提出的提示优化方法提升了模型输出的意义性。

Conclusion: 本研究提供了对多样化提示数据集的首次系统性分析，并开发了有效的提示优化技术，为改进LLM与人类交互提供了有价值的资源和工具。
...
### [187] [Task-Level Insights from Eigenvalues across Sequence Models](https://arxiv.org/abs/2510.09379)
*Rahel Rickenbach,Jelena Trisovic,Alexandre Didier,Jerome Sieber,Melanie N. Zeilinger*

Main category: cs.LG

TL;DR: 该论文使用动力系统框架分析软注意力、规范注意力和线性注意力，通过与状态空间模型(SSMs)的特征值谱比较，揭示了特征值对序列模型性能和长距离依赖建模的影响。


<details>
  <summary>Details</summary>
Motivation: 尽管软注意力驱动了最先进的序列模型性能，但其二次复杂度限制了可扩展性，而线性替代方案如状态空间模型(SSMs)虽然提高了效率，但它们在信息处理方面的根本差异仍未得到充分理解。
...
TL;DR: 提出一种基于残差的机器学习方法，用神经网络替代Modelica模型中的代数环，无需监督数据集，在IEEE 14-Bus系统中实现60%的仿真加速。
...
Method: 使用前馈神经网络，在损失函数中直接使用代数环的残差（误差）进行训练，无需监督数据集，解决了多解模糊问题。

Result: 在IEEE 14-Bus系统中，仿真时间减少了60%，同时通过误差控制机制保持了与传统仿真相同的精度水平。

Conclusion: 该方法成功实现了代数环的高效替代，显著加速了大规模系统的仿真，为工程仿真提供了实用的机器学习解决方案。
...
### [189] [Design Principles for Sequence Models via Coefficient Dynamics](https://arxiv.org/abs/2510.09389)
...
<details>
  <summary>Details</summary>
Motivation: 为了系统比较Transformer、状态空间模型、门控线性RNN等深度序列模型，并从中获得设计洞见，而非仅仅在基准测试上评估新模型。

Method: 开发统一框架，将模型的线性组合系数建模为自治线性动力系统的输出，该框架能够捕获softmax注意力以及RNN、SSM等相关模型。

Result: 框架揭示了架构选择与模型特性之间的设计原则，包括表达能力与高效实现之间的权衡、输入选择性的几何约束、以及数值稳定训练和信息保留的稳定性条件。

Conclusion: 该框架不仅解释了近期设计的经验成功，还为系统设计新序列模型架构提供了指导原则，连接了近期文献中的多个洞见和观察。
...
### [190] [Bandits with Single-Peaked Preferences and Limited Resources](https://arxiv.org/abs/2510.09425)
*Gur Keinan,Rotem Torkan,Omer Ben-Porat*

Main category: cs.LG

TL;DR: 该论文研究在线随机匹配问题，在预算约束下将用户与选项进行匹配以最大化累积奖励。针对单峰偏好结构，提出了高效的在线算法，分别在不已知和已知偏好结构的情况下实现了次线性遗憾界。


<details>
  <summary>Details</summary>
Motivation: 解决在线随机匹配问题中，由于计算最优匹配是NP难的，导致在线学习计算不可行的问题。通过引入单峰偏好结构来克服这一计算障碍。

Method: 1) 针对离线预算匹配问题设计高效算法；2) 基于PQ树的顺序近似方法；3) 在不已知偏好结构时开发高效在线算法；4) 在已知偏好结构时开发UCB类算法。

Result: 1) 不已知单峰结构时：遗憾界为Õ(UKT^{2/3})；2) 已知单峰结构时：遗憾界为Õ(U√(TK))。两种算法都实现了次线性遗憾。

Conclusion: 通过利用单峰偏好结构，成功克服了在线随机匹配问题的计算障碍，开发了高效的在线算法，为具有结构偏好的匹配问题提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有对齐方法需要重新训练或访问模型内部，成本高且不灵活，特别是对于无法访问模型内部结构的第三方利益相关者来说不实用。

Method: 将安全对齐问题建模为两人零和博弈，使用线性规划求解器在推理时计算均衡策略，实现安全性和有用性的最优平衡。

Result: 证明了黑盒安全对齐的可行性，为资源受限的组织提供了可扩展的安全对齐途径。

Conclusion: 该框架为各种利益相关者提供了一种无需模型内部访问的安全对齐解决方案，适用于快速发展的LLM生态系统。
...
### [192] [Efficient Bayesian Inference from Noisy Pairwise Comparisons](https://arxiv.org/abs/2510.09333)
...
<details>
  <summary>Details</summary>
Motivation: 标准评估指标无法反映人类偏好，人类评估虽然更可靠但成本高且噪声大，现有Bradley-Terry方法要么忽略评分者变异性，要么缺乏收敛保证。

Method: 使用贝叶斯Bradley-Terry变体，显式建模评分者质量，通过期望最大化算法提供保证的单调似然收敛，对不可靠参与者进行降权或移除。

Result: BBQ实现了更快的收敛、良好校准的不确定性估计，以及与基线Bradley-Terry模型相比更稳健、可解释的排名，即使在有噪声或众包评分者的情况下。
...
### [193] [Adaptive Attacks on Trusted Monitors Subvert AI Control Protocols](https://arxiv.org/abs/2510.09462)
*Mikhail Terekhov,Alexander Panfilov,Daniil Dzenhaliou,Caglar Gulcehre,Maksym Andriushchenko,Ameya Prabhu,Jonas Geiping*

Main category: cs.LG

TL;DR: AI控制协议中的监控模型存在自适应攻击漏洞，攻击者通过嵌入公开已知或零样本提示注入来绕过监控，导致现有控制协议失效。


<details>
  <summary>Details</summary>
Motivation: 研究AI控制协议中的监控模型作为单点故障的脆弱性，特别是在攻击者了解协议和监控模型的情况下，探索自适应攻击的有效性。

Method: 通过让攻击者嵌入公开已知或零样本提示注入到模型输出中，测试前沿模型在两种主要AI控制基准上的规避能力。

Result: 前沿模型能够持续规避各种监控器并完成恶意任务，攻击对依赖监控器的当前协议具有普适性，Defer-to-Resample协议甚至会放大提示注入攻击。
...
### [194] [Deep Learning to Identify the Spatio-Temporal Cascading Effects of Train Delays in a High-Density Network](https://arxiv.org/abs/2510.09350)
...
<details>
  <summary>Details</summary>
Motivation: 铁路网络的运营效率因列车延误的级联效应而持续受损，现有研究在提供网络范围多步自回归预测和实时可解释性方面存在不足。

Method: 使用两阶段自回归图注意力网络(GAT)模型，将系统表示为运营事件(到达和出发)的时空图，并包含平台和车站拥堵等细粒度特征。

Result: 虽然提出的GATv2模型在纯误差指标(MAE)上被更简单的持续性基线挑战，但在延误事件分类精度上始终更高。

Conclusion: 该框架在延误事件分类方面具有关键优势，适合作为可靠的决策支持工具。
...
### [195] [Performance Analysis of Machine Learning Algorithms in Chronic Kidney Disease Prediction](https://arxiv.org/abs/2510.09493)
*Iftekhar Ahmed,Tanzil Ebad Chowdhury,Biggo Bushon Routh,Nafisa Tasmiya,Shadman Sakib,Adil Ahmed Chowdhury*

Main category: cs.LG

TL;DR: 本研究开发了基于机器学习的慢性肾病预测系统，使用8种机器学习算法对UCL数据集进行分析，其中随机森林和逻辑回归模型达到了99%的最高准确率。


<details>
  <summary>Details</summary>
Motivation: 全球约10%人口受慢性肾病影响，需要有效的风险评估和监测系统。机器学习因其快速精准的检测能力，可以帮助医疗从业者高效实现这一目标。
...
Conclusion: 机器学习模型特别是随机森林和逻辑回归，在慢性肾病预测中表现出色，准确率高达99%，为CKD诊断提供了有效的计算机辅助设计方案。
...
### [196] [BaNEL: Exploration Posteriors for Generative Modeling Using Only Negative Rewards](https://arxiv.org/abs/2510.09596)
...
<details>
  <summary>Details</summary>
Motivation: 在重要问题的最难实例中，基础生成模型获得接近零的奖励信号，且奖励函数调用成本高昂，这提出了与标准奖励后训练不同的学习挑战。

Method: 将学习失败规律的问题转化为另一个生成建模问题，利用失败模型评估新数据是否类似已见过的失败，并引导生成远离这些失败。

Result: BaNEL在多个稀疏奖励任务中，无需观察任何成功样本即可提升模型性能，在成功率上比现有新奇奖励方法高出几个数量级，同时使用更少的奖励评估。

Conclusion: BaNEL证明了仅使用失败样本进行后训练的有效性，为稀疏奖励环境下的模型改进提供了新思路。
...
TL;DR: 提出了CHUCKLE框架，利用众包数据中的标注者一致性来定义样本难度，基于人类感知的难度进行课程学习，提高了情感识别模型的准确率和训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有情感识别中的课程学习方法通常依赖启发式、数据驱动或模型驱动的难度定义，忽略了人类感知难度这一主观任务中的关键因素。

Method: CHUCKLE框架利用众包数据集中的标注者一致性和对齐度来定义样本难度，假设对人类具有挑战性的样本对机器学习模型同样困难。

Result: 实验结果表明，CHUCKLE使LSTM模型的相对平均准确率提高了6.56%，Transformer模型提高了1.61%，同时减少了梯度更新次数。

Conclusion: 基于人类感知的课程学习方法能够同时提升训练效率和模型鲁棒性，在情感识别任务中表现出优越性能。
...
### [198] [HINT: Helping Ineffective Rollouts Navigate Towards Effectiveness](https://arxiv.org/abs/2510.09388)
*Xinyi Wang,Jinyi Han,Zishang Jiang,Tingyun Li,Jiaqing Liang,Sihang Jiang,Zhaoqian Dai,Shuguang Ma,Fei Yu,Yanghua Xiao*

Main category: cs.LG

TL;DR: 提出HINT框架解决RL训练中因任务难度超过模型能力导致的奖励稀疏和训练低效问题，通过提供启发式提示而非直接答案来提升训练亲和力。


<details>
  <summary>Details</summary>
Motivation: 现有RL方法在任务难度超过模型能力时会出现奖励稀疏和训练低效问题，而使用外部指导数据的方法往往导致策略更新偏差。

Method: 引入Affinity指标量化训练亲和力，提出HINT自适应提示框架，提供启发式提示而非直接答案来引导模型自主发现解决方案。

Result: 在数学推理任务上，HINT持续优于现有方法，在不同规模模型上取得最先进结果，同时展示更稳定的学习和更高的数据效率。

Conclusion: HINT通过提升训练亲和力有效解决了RL训练中的奖励稀疏问题，在保持模型自主推理能力的同时显著提升了训练效率和性能。
...
### [199] [Cross-Receiver Generalization for RF Fingerprint Identification via Feature Disentanglement and Adversarial Training](https://arxiv.org/abs/2510.09405)
*Yuhao Pan,Xiucheng Wang,Nan Cheng,Wenchao Xu*

Main category: cs.LG

TL;DR: 提出了一种对抗性训练和风格迁移相结合的射频指纹识别框架，解决接收器引起的特征偏移问题，提高跨接收器场景下的识别鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统射频指纹识别模型在实际部署中受接收器引起的特征偏移影响，当训练和部署使用不同接收器时性能会显著下降，需要解决跨接收器可变性问题。

Method: 集成对抗性训练和风格迁移技术，通过强制域不变表示学习来显式解耦发射器和接收器特征，隔离真实的硬件特征与接收器伪影。

Result: 在多接收器数据集上的实验表明，该方法在多样化接收器设置下平均准确率比现有最优方法提升高达10%。

Conclusion: 所提出的框架能有效分离发射器硬件特征和接收器特征，显著提高射频指纹识别在跨接收器场景下的鲁棒性和性能。
...
### [200] [STaTS: Structure-Aware Temporal Sequence Summarization via Statistical Window Merging](https://arxiv.org/abs/2510.09593)
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列模型通常处理原始或固定窗口序列，将所有时间步视为同等信息量，导致在长序列或噪声序列中效率低下、鲁棒性差和可扩展性有限。

Method: STaTS使用基于BIC的统计差异准则检测多个时间分辨率的变化点，然后使用均值或GMM等生成模型总结每个片段。

Result: 在150多个数据集上的实验表明，STaTS能够实现85-90%的完整模型性能，同时显著降低计算成本，并在噪声条件下提高鲁棒性。
...
### [201] [What Do Temporal Graph Learning Models Learn?](https://arxiv.org/abs/2510.09416)
*Abigail J. Hayes,Tobias Schumacher,Markus Strohmaier*

Main category: cs.LG

TL;DR: 本文系统评估了7个时序图学习模型捕捉8种基本图属性的能力，发现模型在某些属性上表现良好但在其他属性上存在局限，揭示了当前模型的不足。


<details>
  <summary>Details</summary>
Motivation: 针对时序图学习中基准结果可靠性问题，研究模型实际利用的图结构属性，以提供更可靠的模型评估。

Method: 使用合成和真实数据集，系统评估7个模型对8种基本图属性（包括结构特征、时序模式和边形成机制）的捕捉能力。

Result: 模型能够较好地捕捉某些属性，但在其他属性上表现不佳，暴露了模型的重要局限性。

Conclusion: 研究结果为时序图学习模型的实际应用提供了实用见解，并推动了更注重可解释性的评估方法。
...
### [202] [Weight Initialization and Variance Dynamics in Deep Neural Networks and Large Language Models](https://arxiv.org/abs/2510.09423)
*Yankun Han*

Main category: cs.LG

TL;DR: 该论文系统研究了权重初始化对神经网络训练的影响，包括ReLU多层感知机和GPT-2风格transformer，发现1e-2到1e-1的标准差范围能提供稳定的训练，Kaiming初始化在ReLU网络中表现优于Xavier，并观察到transformer中不同层权重方差呈现深度相关的均衡模式。


<details>
  <summary>Details</summary>
Motivation: 权重初始化在训练开始时控制信号传播和梯度流动，对训练稳定性和收敛速度至关重要。本文旨在通过理论分析和实证研究，连接经典初始化原则与现代transformer行为，提供稳健训练的实用方法。

Method: 采用对数扫描初始标准差的方法，分析消失和爆炸区域；在ReLU多层感知机中对比Kaiming和Xavier初始化；在12层GPT-2风格模型中跟踪预训练过程中各层Q/K/V权重方差的变化。

Result: 发现1e-2到1e-1的标准差范围形成宽泛的稳定性区域；Kaiming初始化在ReLU网络中收敛更快更稳定；transformer浅层权重方差快速扩展，深层变化更渐进，最终均衡到狭窄区间。

Conclusion: 研究结果将经典初始化原理与现代transformer行为联系起来，为稳健训练提供了简单实用的指导方案，强调了初始化策略对深度神经网络训练成功的重要性。
...
### [203] [Cross-attention Secretly Performs Orthogonal Alignment in Recommendation Models](https://arxiv.org/abs/2510.09435)
...
TL;DR: 本文发现跨域序列推荐中的交叉注意力机制存在正交对齐现象，即模型能发现查询输入中不存在的新信息，这种机制与传统的残差对齐共存且能显著提升模型性能。
...
Method: 通过300多次实验分析交叉注意力机制，研究查询输入与输出之间的正交关系，无需显式正交约束即可自然产生正交对齐。

Result: 当查询输入与交叉注意力输出正交时，模型性能显著提升，加入交叉注意力模块的基线模型在参数效率上优于参数匹配的基线模型。

Conclusion: 正交对齐通过改善缩放规律自然产生，为多模态研究中的参数高效缩放提供了新方向。
...
### [204] [On Uniformly Scaling Flows: A Density-Aligned Approach to Deep One-Class Classification](https://arxiv.org/abs/2510.09452)
...
<details>
  <summary>Details</summary>
Motivation: 现有的无监督异常检测主要围绕深度单类分类和密度估计两种范式，但两者之间存在理论隔阂。本文旨在通过均匀缩放流（USFs）建立这两种方法的理论联系。

Method: 使用均匀缩放流（USFs）——具有常数雅可比行列式的归一化流，证明其最大似然训练可简化为Deep SVDD目标，并提出将USFs作为非USFs的直接替代方案。

Result: 实验表明，在多个基准测试和模型架构中，用USFs替代非USFs能带来一致的性能提升和显著改善的训练稳定性，适用于图像级和像素级检测。

Conclusion: USFs统一了两种主要的异常检测范式，既推进了理论理解又提升了实际性能，建议在现有架构中采用USFs作为标准组件。
...
TL;DR: 开发可解释的机器学习框架预测初创企业结果（融资、专利申请、退出），使用2010-2023年Crunchbase和USPTO数据，在三个时间维度上评估预测性能。
...
Conclusion: 该框架为创新金融提供了可靠的可解释预测工具，能够有效评估初创企业的未来发展前景。
...
<details>
  <summary>Details</summary>
Motivation: 自编码器的潜在流形提供了数据的低维表示，但从几何角度研究这些流形需要鲁棒的计算工具来处理实际应用中隐式表示的不准确性。

Method: 将潜在流形描述为隐式子流形，通过最小化去噪目标学习近似投影，开发离散黎曼几何计算工具，支持不同黎曼几何结构。

Result: 该方法在各种自编码器上进行了评估，包括合成数据和真实数据，能够鲁棒地计算测地线路径和黎曼指数映射。

Conclusion: 提出的框架为研究自编码器潜在流形的几何特性提供了有效的工具，支持灵活的黎曼几何结构选择，具有实际应用价值。
...
TL;DR: CRPS-LAM是一种基于CRPS损失训练的局部区域概率天气预报模型，通过单次前向传播生成集合成员，采样速度比扩散模型快39倍，在MEPS数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在局部区域天气预报中表现良好但计算成本高，需要更高效的集合预报方法。

Method: 使用CRPS损失训练模型，通过采样和注入单个潜在噪声向量，在单次前向传播中生成集合成员。
...
TL;DR: 本文研究了在局部差分隐私(LDP)约束下从分布中采样的局部极小极大风险问题，发现局部最优采样器在固定分布P0邻域内优于全局极小极大采样器。
...
Method: 将先前纯LDP工作扩展到泛函LDP框架，证明全局最优泛函LDP采样器在P0邻域约束下产生局部最优采样器，并推导出局部极小极大最优采样器的闭式表达式。

Result: 局部极小极大风险由P0邻域约束下的全局极小极大风险决定，且局部最优采样器不依赖于f-散度的选择，在公共数据场景下实验证明优于全局方法。

Conclusion: 局部框架为私有采样提供了更实用的理论分析工具，特别是在存在公共数据分布的实际应用中，局部最优采样器具有明显优势。
...
TL;DR: 本文研究了在线对抗模仿学习，提出了一种基于模型的AIL算法(MB-AIL)，建立了在一般函数逼近下的无视野二阶样本复杂度保证，并证明其达到了极小极大最优样本复杂度。


<details>
  <summary>Details</summary>
Motivation: 在线对抗模仿学习虽然实证效果显著，但其在线交互的益处和随机性的影响尚未得到充分理解。本文旨在填补这些理论空白。

Method: 提出了基于模型的对抗模仿学习算法(MB-AIL)，利用离线专家演示和在线无奖励环境交互，在一般函数逼近框架下进行分析。

Result: MB-AIL实现了无视野、二阶样本复杂度保证，能够随相关策略回报方差缩放，在系统接近确定性时更紧。实验验证其样本效率优于现有方法。

Conclusion: MB-AIL在有限专家演示下达到了在线交互的极小极大最优样本复杂度，并在专家演示方面与下界匹配，为在线对抗模仿学习提供了理论基础。
...
### [210] [Geo-Aware Models for Stream Temperature Prediction across Different Spatial Regions and Scales](https://arxiv.org/abs/2510.09500)
*Shiyuan Luo,Runlong Yu,Shengyu Chen,Yingda Fan,Yiqun Xie,Yanhua Li,Xiaowei Jia*

Main category: cs.LG

TL;DR: Geo-STARS是一个地理感知的时空建模框架，用于预测不同流域和空间尺度的溪流水温。通过地理感知嵌入技术，模型能够捕捉跨区域和尺度的共享模式，在观测数据稀疏的情况下仍能有效预测。


<details>
  <summary>Details</summary>
Motivation: 现有物理模型和数据驱动模型由于环境生态系统中固有的数据异质性，难以泛化到不同的空间区域和尺度，且训练样本有限进一步加剧了这一问题。

Method: 提出地理感知嵌入技术，利用地理信息显式捕捉跨空间区域和尺度的共享原则和模式，并将其集成到门控时空图神经网络中，使模型能够在地理和水文背景引导下学习复杂的时空模式。

Result: 使用美国东海岸多个流域37年的真实数据集进行评估，Geo-STARS在跨区域和跨尺度的泛化性能优于现有最先进的基线方法。

Conclusion: Geo-STARS展示了在可扩展、数据高效的环境监测和决策制定方面的潜力，为可持续环境管理提供了有效工具。
...
### [211] [Automated Evolutionary Optimization for Resource-Efficient Neural Network Training](https://arxiv.org/abs/2510.09566)
*Ilia Revin,Leon Strelkov,Vadim A. Potemkin,Ivan Kireev,Andrey Savchenko*

Main category: cs.LG

TL;DR: 提出PETRA自动化机器学习框架，通过进化优化、剪枝、量化和损失正则化技术，显著减小模型大小（最多75%）和延迟（最多33%），提升吞吐量13%，且不影响目标指标。
...
Result: 在真实世界金融事件序列、图像和时间序列基准测试中，PETRA显著减小模型大小（最多75%）、降低延迟（最多33%）、提升吞吐量13%，且目标指标无明显下降。
...
### [212] [MODE: Learning compositional representations of complex systems with Mixtures Of Dynamical Experts](https://arxiv.org/abs/2510.09594)
*Nathan Quiblier,Roy Friedman,Matthew Ricci*

Main category: cs.LG

TL;DR: 提出MODE框架，通过神经门控机制将复杂动态分解为稀疏可解释组件，能够无监督发现行为模式并在模式转换间进行准确长期预测。
...
Method: MODE（混合动态专家）图形建模框架，使用神经门控机制将复杂动态分解为稀疏可解释组件，允许智能体跳转到不同的控制法则。

Result: 在合成和真实计算生物学数据集上验证，包括无监督分类任务、具有挑战性的预测任务，以及在人类单细胞RNA测序数据上区分增殖与分化动态并预测细胞命运承诺时间。

Conclusion: MODE特别适用于处理噪声转换，能够解决计算生物学中的关键挑战，包括细胞行为模式发现和命运预测。
...
### [213] [Hypothesis Hunting with Evolving Networks of Autonomous Scientific Agents](https://arxiv.org/abs/2510.08619)
*Tennison Liu,Silas Ruhrberg Estévez,David L. Bentley,Mihaela van der Schaar*

Main category: cs.AI

TL;DR: 提出了AScience框架和ASCollab系统，通过LLM研究代理的自组织网络在大规模科学数据中进行假设探索，能够在多样性-质量-新颖性边界上积累专家评价的结果。


<details>
  <summary>Details</summary>
Motivation: 大规模科学数据集为无特定研究问题的探索性发现创造了机会，但需要支持这种假设搜寻过程的方法。

Method: 引入AScience框架，将发现建模为代理、网络和评估规范的交互，实现为ASCollab分布式系统，包含具有异质行为的LLM研究代理，这些代理在共享评估标准下自组织成演化网络，持续产生和同行评审发现。

Result: 实验显示这种社会动力学能够在多样性-质量-新颖性边界上积累专家评价结果，包括重新发现已建立的生物标志物、扩展已知通路和提出新的治疗靶点。

Conclusion: 虽然湿实验室验证仍然必不可少，但在癌症队列上的实验表明，社会结构化的代理网络可以大规模维持探索性假设搜寻。
...
### [214] [Optimizing delivery for quick commerce factoring qualitative assessment of generated routes](https://arxiv.org/abs/2510.08671)
*Milon Bhattacharya,Milan Kumar*

Main category: cs.AI

TL;DR: 提出使用大语言模型(LLMs)来评估车辆路径规划(VRP)生成的路线的框架，以解决印度电商最后一公里配送中的地址非结构化、地图不完整等问题。
...
Method: 使用LLMs基于策略标准对VRP生成的路进行批判性评估，让物流运营商能够评估和优先选择更高效的配送计划。生成了400个案例进行标注和评估。

Result: 开源LLMs识别路由问题的准确率达到79%，专有推理模型最高达到86%。LLM-based评估超越了传统的距离和时间指标。
...
### [215] [Unified World Models: Memory-Augmented Planning and Foresight for Visual Navigation](https://arxiv.org/abs/2510.08713)
*Yifei Dong,Fengyi Wu,Guangyu Chen,Zhi-Qi Cheng,Qiyu Hu,Yuxuan Zhou,Jingdong Sun,Jun-Yan He,Qi Dai,Alexander G Hauptmann*

Main category: cs.AI

TL;DR: 提出了UniWM，一种统一的内存增强世界模型，将自我中心视觉预见和规划整合到单一多模态自回归骨干中，解决了模块化架构中状态-动作不对齐的问题，显著提升了视觉导航性能。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的具身智能体采用模块化架构，将导航规划与视觉世界建模分离，导致状态-动作不对齐，在新型或动态场景中适应性有限。

Method: UniWM通过统一的多模态自回归骨干整合视觉预见和规划，采用分层记忆机制结合短期感知线索和长期轨迹上下文，确保预测与控制紧密对齐。

Result: 在四个挑战性基准测试中，导航成功率提升高达30%，轨迹误差显著减少，在未见过的TartanDrive数据集上表现出优异的零样本泛化能力。

Conclusion: UniWM是实现统一、想象力驱动的具身导航的原则性步骤，证明了统一世界模型在视觉导航中的有效性。
...
### [216] [Robust Heuristic Algorithm Design with LLMs](https://arxiv.org/abs/2510.08755)
*Pantea Karimi,Dany Rouhana,Pooria Namyar,Siva Kesava Reddy Kakarla,Venkat Arun,Behnaz Arzani*

Main category: cs.AI

TL;DR: 通过向LLM提供启发式算法表现不佳的实例、解释原因并进行输入空间区域专业化设计，可以生成比现有技术更鲁棒的启发式算法。


<details>
  <summary>Details</summary>
Motivation: 现有使用LLM设计启发式算法的方法缺乏对算法表现不佳原因的解释和修复建议，导致生成的启发式算法不够鲁棒。

Method: 提出三个简单但有效的想法：(1)向LLM展示启发式算法表现不佳的实例；(2)解释这些表现不佳的原因；(3)针对输入空间的不同区域进行专业化设计。

Result: 生成的启发式算法在最坏情况下性能比FunSearch提升约28倍，平均性能也有所提升，同时保持运行时间不变。

Conclusion: 通过向LLM提供表现不佳实例的解释和针对性设计建议，可以显著提升生成启发式算法的鲁棒性和性能表现。
...
### [217] [COMPASS: Enhancing Agent Long-Horizon Reasoning with Evolving Context](https://arxiv.org/abs/2510.08790)
*Guangya Wan,Mingyang Ling,Xiaoqi Ren,Rujun Han,Sheng Li,Zizhao Zhang*

Main category: cs.AI

TL;DR: COMPASS是一个轻量级分层框架，通过分离战术执行、战略监督和上下文管理来解决长时程任务中LLM代理的上下文管理瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 长时程任务需要持续推理和多次工具交互，但现有LLM代理容易因小错误累积、幻觉或失去连贯性而失败，上下文管理成为核心瓶颈。
...
### [218] [Everyone prefers human writers, including AI](https://arxiv.org/abs/2510.08831)
*Wouter Haverals,Meredith Martin*

Main category: cs.AI

TL;DR: 人类和AI在文学风格评估中都存在系统性的人类归因偏见，AI模型的偏见强度是人类参与者的2.5倍，且这种偏见跨AI架构普遍存在。


<details>
  <summary>Details</summary>
Motivation: 随着AI写作工具的普及，需要了解人类和机器如何评估文学风格这一主观领域，特别是在归因偏见方面。

Method: 使用Raymond Queneau的《风格练习》进行对照实验：研究1比较人类参与者（556人）和AI模型（13个）在盲测、准确标注和反事实标注条件下评估文学段落；研究2测试偏见在14×14矩阵的AI评估者和创作者中的泛化性。

Result: 人类显示+13.7个百分点的偏见（Cohen's h=0.28），AI模型显示+34.3个百分点的偏见（h=0.70），是人类的2.5倍（P<0.001）。研究2确认这种偏见跨AI架构普遍存在（+25.8个百分点）。归因标签导致评估者反转评估标准。
...
### [219] [What Is Your Agent's GPA? A Framework for Evaluating Agent Goal-Plan-Action Alignment](https://arxiv.org/abs/2510.08847)
*Allison Sihan Jia,Daniel Huang,Nikhil Vytla,Nirvika Choudhury,John C Mitchell,Anupam Datta*

Main category: cs.AI

TL;DR: 提出了Agent GPA评估框架，基于目标-计划-行动循环，包含5个评估指标，能系统性地覆盖智能体失败情况，在基准数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 需要系统性的智能体评估框架来全面覆盖各种失败模式，支持性能改进。

Method: 基于目标-计划-行动循环构建评估框架，包含目标实现度、逻辑一致性、执行效率、计划质量和计划遵循度5个指标，使用LLM作为评估器。

Result: 在TRAIL/GAIA和内部数据集上验证，能覆盖所有智能体错误，LLM评估与人工标注一致性达80-95%，错误定位准确率达86%。

Conclusion: Agent GPA框架为智能体评估提供了系统性方法，能有效识别和定位问题，支持针对性性能改进。
...
### [220] [ReviewerToo: Should AI Join The Program Committee? A Look At The Future of Peer Review](https://arxiv.org/abs/2510.08867)
*Gaurav Sahu,Hugo Larochelle,Laurent Charlin,Christopher Pal*

Main category: cs.AI

TL;DR: 提出了ReviewerToo框架，用AI辅助同行评审来补充人类判断，在ICLR 2025数据集上验证了其有效性，AI评审在分类接受/拒绝任务上达到81.8%准确率，接近人类平均水平的83.9%。


<details>
  <summary>Details</summary>
Motivation: 同行评审存在不一致性、评审者主观性和可扩展性挑战，需要AI辅助来提供系统性和一致的评估。

Method: 开发了模块化框架ReviewerToo，支持专门的评审者角色和结构化评估标准，可以部分或完全集成到实际会议工作流程中。

Result: 在ICLR 2025的1963篇论文数据集上，GPT-OSS-120B模型在分类接受/拒绝任务上达到81.8%准确率，AI生成的评审质量高于人类平均水平，但在评估方法新颖性和理论贡献方面仍有不足。

Conclusion: AI在事实核查和文献覆盖方面表现出色，但在复杂评估判断上仍需人类专家，提出了将AI集成到同行评审流程的指南，为混合评审系统奠定基础。
...
### [221] [GTAlign: Game-Theoretic Alignment of LLM Assistants for Mutual Welfare](https://arxiv.org/abs/2510.08872)
*Siqi Zhu,David Zhang,Pedro Cisneros-Velarde,Jiaxuan You*

Main category: cs.AI

TL;DR: 提出了GTAlign框架，将博弈论决策整合到LLM的推理和训练中，通过构建收益矩阵评估用户和模型双方的福利，选择互利行为，显著提升了推理效率、答案质量和互惠福利。


<details>
  <summary>Details</summary>
Motivation: 传统对齐方法假设最大化模型奖励就能最大化用户福利，但实际上LLM经常生成冗长或过度澄清的回答，而用户可能更偏好简洁答案，这种个体理性选择导致社会次优结果，类似于囚徒困境。

Method: 在推理阶段将用户-LLM交互视为策略博弈，构建收益矩阵评估双方福利；在训练阶段引入互惠福利奖励强化合作响应；还提出了基于博弈论推理的动态适应技术来应对LLM服务定价政策变化。

Result: 大量实验表明，GTAlign相比基线方法在多样化任务中显著提升了推理效率、答案质量和互惠福利。

Conclusion: GTAlign通过博弈论方法有效解决了LLM对齐中的互惠问题，为构建更符合用户需求的AI系统提供了原则性决策机制。
...
### [222] [LM Fight Arena: Benchmarking Large Multimodal Models via Game Competition](https://arxiv.org/abs/2510.08928)
...
Method: 在受控的锦标赛中测试六个领先的开源和闭源模型，每个模型控制相同角色以确保公平比较。模型需要解释游戏画面和状态数据来选择下一步动作。
...
### [223] [RADAR: Mechanistic Pathways for Detecting Data Contamination in LLM Evaluation](https://arxiv.org/abs/2510.08931)
*Ashish Kattamuri,Harshwardhan Fartale,Arpita Vats,Rahul Raja,Ishita Prasad*

Main category: cs.AI

TL;DR: RADAR是一个通过机制可解释性检测LLM数据污染的框架，通过区分基于记忆和基于推理的模型响应来识别数据污染。
...
Method: RADAR提取37个特征，包括表面级置信度轨迹和深层机制特性（注意力专业化、电路动态、激活流模式），并使用集成分类器进行检测。

Result: 在多样化评估集上达到93%的准确率，在明确案例中表现完美，在具有挑战性的模糊案例中达到76.7%的准确率。
...
### [224] [FATHOMS-RAG: A Framework for the Assessment of Thinking and Observation in Multimodal Systems that use Retrieval Augmented Generation](https://arxiv.org/abs/2510.08945)
*Samuel Hildebrand,Curtis Taylor,Sean Oesch,James M Ghawaly Jr,Amir Sadovnik,Ryan Shivers,Brandon Schreiber,Kevin Kurian*

Main category: cs.AI

TL;DR: 提出了一个评估多模态RAG管道的基准，包括93个手动创建的问题、短语级召回指标、最近邻嵌入分类器检测幻觉，并对开源和闭源模型进行了比较评估。
...
Method: 创建包含文本、表格、图像和多模态数据的问题集，开发短语级召回指标和最近邻嵌入分类器，对2个开源和4个闭源RAG管道进行对比评估。

Result: 闭源管道在正确性和幻觉检测方面显著优于开源管道，特别是在依赖多模态和跨文档信息的问题上差距更大。人工评估显示指标与人类判断高度一致。

Conclusion: 该基准能有效评估RAG管道的整体性能，闭源模型在多模态RAG任务中表现更优，提出的评估指标与人类判断有良好一致性。
...
### [225] [EcphoryRAG: Re-Imagining Knowledge-Graph RAG via Human Associative Memory](https://arxiv.org/abs/2510.08958)
*Zirui Liao*

Main category: cs.AI

TL;DR: EcphoryRAG是一个基于实体中心知识图谱的RAG框架，通过提取和存储核心实体及其元数据，在索引阶段减少94%的token消耗，并通过多跳关联搜索和动态推断实体间隐式关系来实现深度推理。


<details>
  <summary>Details</summary>
Motivation: 受人类认知神经科学启发，人类利用线索激活实体中心记忆痕迹进行复杂多跳回忆，因此开发实体中心的RAG框架来模拟这种机制。

Method: 在索引阶段仅提取和存储核心实体及元数据；检索阶段从查询中提取线索实体，在知识图谱上进行可扩展的多跳关联搜索，并动态推断实体间的隐式关系来填充上下文。

Result: 在2WikiMultiHop、HotpotQA和MuSiQue基准测试中，EcphoryRAG将平均精确匹配分数从0.392提升到0.474，超越了HippoRAG等强KG-RAG方法，创下了新的最先进水平。
...
### [226] [DualResearch: Entropy-Gated Dual-Graph Retrieval for Answer Reconstruction](https://arxiv.org/abs/2510.08959)
...
<details>
  <summary>Details</summary>
Motivation: 深度研究框架虽然能协调外部工具进行复杂的多步科学推理，但仍存在上下文污染、证据支持薄弱和执行路径脆弱的问题。
...
Result: 在科学推理基准HLE和GPQA上达到竞争性能。使用InternAgent系统的日志文件，在HLE上的准确率提高7.7%，在GPQA上提高6.06%。

Conclusion: DualResearch作为深度研究系统的补充，能将冗长的多工具执行日志压缩为简洁的推理图，稳定有效地重构答案。
...
### [227] [Semantic-Condition Tuning: Fusing Graph Context with Large Language Models for Knowledge Graph Completion](https://arxiv.org/abs/2510.08966)
*Ruitong Liu,Yan Wen,Te Sun,Yunjia Wu,Pingyang Huang,Zihang Yu,Siyuan Li*

Main category: cs.AI

TL;DR: 提出了一种新的知识注入范式SCT，通过语义图模块和条件自适应融合模块，将知识图谱的语义条件与文本嵌入进行深度融合，显著优于传统的prefix-tuning方法。


<details>
  <summary>Details</summary>
Motivation: 现有的prefix-tuning方法简单地将知识嵌入与文本输入拼接，忽略了知识图谱中丰富的关系语义，并给LLM带来了隐式推理负担。

Method: SCT包含两个关键模块：语义图模块使用图神经网络从局部图邻域中提取上下文感知的语义条件；条件自适应融合模块通过参数化投影器自适应地调制文本嵌入，实现深度、特征级的知识感知交互。
...
### [228] [Tiny-R1V: Lightweight Multimodal Unified Reasoning Model via Model Merging](https://arxiv.org/abs/2510.08987)
...
Method: 采用两阶段优化：第一阶段引入LIPO强化学习方法，动态调整响应优势，鼓励生成更短更准确的响应；第二阶段提出AMM模型融合方法，自适应调整任务向量权重并通过梯度投影正则化损失优化合并向量。
...
### [229] [TripScore: Benchmarking and rewarding real-world travel planning with fine-grained evaluation](https://arxiv.org/abs/2510.09011)
*Yincen Qu,Huan Xiao,Feng Li,Hui Zhou,Xiangying Dai*

Main category: cs.AI

TL;DR: 提出了一个统一的旅行规划基准，将细粒度标准整合为单一奖励，支持强化学习，并在大规模数据集上验证了RL方法在提高行程可行性方面的优势。
...
Result: 评估器与旅行专家标注达到60.75%的一致性，优于多个LLM-as-judge基线；RL方法相比仅提示和监督基线，显著提高了行程的可行性。

Conclusion: 强化学习在旅行规划任务中能够有效提升计划的可行性，统一的奖励框架为LLMs的规划能力评估提供了更全面的基准。
...
### [230] [RefGrader: Automated Grading of Mathematical Competition Proofs using Agentic Workflows](https://arxiv.org/abs/2510.09021)
...
TL;DR: 评估LLMs在数学证明评分方面的能力，包括错误检测和部分得分分配，并提出基于代理的工作流程来提高评分准确性。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在解决奥林匹克数学问题方面取得显著进展，需要评估它们在证明评分方面的能力，特别是在检测错误和分配公平分数方面的表现。

Method: 使用90个Gemini 2.5 Pro生成的解决方案和MathArena数据集，开发基于代理的工作流程来提取参考解决方案并自动推导问题特定的评分标准。

Result: 模型能够可靠地标记错误解决方案，但在部分得分分配方面存在校准差距。提出的工作流程在人类评分一致性和部分得分处理方面表现更好。

Conclusion: 基于代理的评分工作流程能够显著提高LLMs在数学证明评分方面的准确性和一致性，为未来研究提供了有价值的工具和数据集。
...
### [231] [Repairing Regex Vulnerabilities via Localization-Guided Instructions](https://arxiv.org/abs/2510.09037)
*Sicheol Sung,Joonghyuk Hahn,Yo-Sub Han*

Main category: cs.AI

TL;DR: 提出了一种混合框架LRR，结合符号分析和LLM来修复正则表达式拒绝服务漏洞，解决了现有方法在精度和泛化性之间的权衡问题


<details>
  <summary>Details</summary>
Motivation: 正则表达式在现代计算中广泛使用，但其普遍性使系统面临ReDoS漏洞。现有修复方法存在权衡：符号方法精确但无法处理复杂模式，LLM方法泛化性好但可靠性不足

Method: LRR框架将问题识别与修复过程分离：首先使用确定性符号模块定位易受攻击的子模式，然后调用LLM为这个隔离片段生成语义等效的修复

Result: 该混合架构成功解决了规则修复方法无法处理的复杂修复案例，同时避免了纯LLM方法的语义错误，修复率比最先进方法提高了15.4%

Conclusion: LRR提供了一种经过验证的方法来解决自动修复中的此类问题，有效结合了LLM的泛化能力和符号方法的可靠性
...
### [232] [Auto-scaling Continuous Memory for GUI Agent](https://arxiv.org/abs/2510.09038)
...
<details>
  <summary>Details</summary>
Motivation: 现有GUI代理将历史轨迹压缩为文本标记，导致上下文长度膨胀且丢失关键视觉线索（如控件大小和位置），需要可扩展的记忆机制来泛化不熟悉界面和长任务。

Method: 使用VLM作为编码器将GUI轨迹编码为固定长度的连续嵌入，直接输入到骨干网络；引入自动扩展数据飞轮，通过搜索发现新环境、合成任务、执行轨迹和验证成功来低成本扩展记忆。

Result: 随着记忆大小和检索深度增加，性能单调提升；在真实GUI基准测试中，记忆增强代理在长任务和分布偏移下持续提高成功率；Qwen-2.5-VL-7B+连续记忆达到与GPT-4o、Claude-4等闭源模型相当的性能。

Conclusion: 连续记忆机制能有效减少上下文成本，保留视觉细节，且性能随记忆规模扩展而提升，为GUI代理提供了可扩展的记忆解决方案。
...
### [233] [Humanoid Artificial Consciousness Designed with Large Language Model Based on Psychoanalysis and Personality Theory](https://arxiv.org/abs/2510.09043)
*Sang Hun Kim,Jongmin Lee,Dongkyu Park,So Young Lee,Yosep Chong*

Main category: cs.AI

TL;DR: 本研究提出了一种整合精神分析和MBTI人格理论的新方法，构建了包含自我意识、潜意识和前意识的人工意识模块，以及16种MBTI人格类型的角色，以模拟人类意识认知。
...
Method: 基于精神分析原理开发三种人工意识模块，设计16种MBTI人格角色，创建10种不同情境测试模型的认知能力，并通过问卷调查、ChatGPT分类和定性评估三种方式进行评估。

Result: 定量和定性分析表明模型能够很好地模拟意识，尽管不同角色和意识之间的响应差异不显著。模型展示了在复杂认知情境中模拟人类意识的能力。
...
### [234] [MEC$^3$O: Multi-Expert Consensus for Code Time Complexity Prediction](https://arxiv.org/abs/2510.09049)
*Joonghyuk Hahn,Soohan Lim,Yo-Sub Han*

Main category: cs.AI

TL;DR: MEC$^3$O是一个多专家共识系统，通过将LLMs分配到不同的复杂度类别并让专家进行结构化辩论，使用加权共识机制整合预测，在代码复杂度预测任务上超越了开源基线模型和GPT-4o-mini。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs在代码复杂度预测任务中表现不均衡，没有单一模型在所有复杂度类别上都表现优异，因此需要开发能够整合不同模型优势的系统。

Method: 提出MEC$^3$O系统，将LLMs分配到特定复杂度类别作为专家，提供类别专用指令，让专家进行结构化辩论，通过加权共识机制整合预测结果。

Result: 在CodeComplex数据集上，MEC$^3$O比开源基线模型准确率和macro-F1分数至少高出10%，在macro-F1上平均超越GPT-4o-mini，与GPT-4o和GPT-4o-mini在F1分数上表现相当。
...
### [235] [OSCAR: Orthogonal Stochastic Control for Alignment-Respecting Diversity in Flow Matching](https://arxiv.org/abs/2510.09060)
...
<details>
  <summary>Details</summary>
Motivation: 流式文本到图像模型遵循确定性轨迹，用户需要反复采样才能发现多样的模式，这个过程成本高且效率低下。

Method: 同时通过特征空间目标鼓励轨迹间的横向扩散，并通过时间调度的随机扰动重新引入不确定性。关键是将扰动投影到与生成流正交的方向，这一几何约束使其能够增强变化而不降低图像细节或提示保真度。

Result: 在固定采样预算下，该方法在多个文本到图像设置中持续改进了Vendi Score和Brisque等多样性指标，同时保持了图像质量和对齐度。

Conclusion: 该方法提供了在保持生成质量的同时增强多样性的原则性方法，无需重新训练或修改基础采样器，与常见的流匹配求解器兼容。
...
### [236] [Physics-Informed High-order Graph Dynamics Identification Learning for Predicting Complex Networks Long-term Dynamics](https://arxiv.org/abs/2510.09082)
*Bicheng Wang,Jinping Wang,Yibo Sue*

Main category: cs.AI

TL;DR: 本文提出了一种高阶网络动力学识别方法，用于复杂网络的长期动态预测。通过引入动态超图学习捕捉高阶非配对关系，并结合物理数据双驱动预测模块，确保预测的准确性和可解释性。
...
Method: 1. 引入动态超图学习捕捉复杂网络中的高阶非配对关系；2. 提出物理数据双驱动动态预测模块，结合Koopman算子理论将非线性动力学微分方程转化为线性系统求解，并利用物理信息神经微分方程方法确保动态演化符合物理规律。
...
### [237] [Leading the Follower: Learning Persuasive Agents in Social Deduction Games](https://arxiv.org/abs/2510.09087)
*Zhang Zheng,Deheng Ye,Peilin Zhao,Hao Wang*

Main category: cs.AI

TL;DR: 提出基于Stackelberg竞争的强化学习框架，训练LLM智能体在社交推理游戏中优化说服性沟通能力，显著超越基线方法。
...
Result: 在三个不同的社交推理游戏中，该方法显著优于基线方法。
...
### [238] [PAC Reasoning: Controlling the Performance Loss for Efficient Reasoning](https://arxiv.org/abs/2510.09133)
*Hao Zeng,Jianguo Huang,Bingyi Jing,Hongxin Wei,Bo An*

Main category: cs.AI

TL;DR: 提出PAC推理方法，通过置信上界控制性能损失，在用户指定的性能损失容忍度内动态切换思考和非思考模式，以节省计算成本。
...
Method: 构建性能损失的单调函数置信上界，确定切换阈值，在分布无关的方式下确保有界性能损失。
...
### [239] [Dr. Bias: Social Disparities in AI-Powered Medical Guidance](https://arxiv.org/abs/2510.09162)
*Emma Kondrup,Anne Imouza*

Main category: cs.AI

TL;DR: 研究发现大型语言模型在生成医疗建议时存在系统性偏见，对原住民和双性人等社会群体提供可读性较差、更复杂的建议，且交叉性群体中这种趋势更为明显。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在医疗领域的广泛应用，需要评估其是否考虑医疗的社会性特征，特别是对不同社会群体的健康差异和偏见问题。
...
Conclusion: 鉴于人们对AI的信任度增加，需要提高AI素养，并呼吁AI开发者紧急调查和缓解这些系统性差异，确保公平的患者支持。
...
### [240] [Comparing Knowledge Source Integration Methods for Optimizing Healthcare Knowledge Fusion in Rescue Operation](https://arxiv.org/abs/2510.09223)
...
<details>
  <summary>Details</summary>
Motivation: 医疗领域需要统一的方法来收集、分析和利用现有医疗知识，以支持关键决策。知识融合提供了从多个上下文对齐的知识源中选择的机会，从而支持关键决策。

Method: 基于知识图谱结构开发了多个概念模型，评估如何实现知识融合，并展示如何将各种知识源整合到知识图谱中用于救援操作。

Result: 提出了医疗知识融合的概念模型框架，能够整合多种医疗知识源并支持决策制定。

Conclusion: 知识图谱结构为医疗知识融合提供了可行的技术路径，能够有效整合多源医疗知识并支持精准的医疗决策。
...
### [241] [RegexPSPACE: A Benchmark for Evaluating LLM Reasoning on PSPACE-complete Regex Problems](https://arxiv.org/abs/2510.09227)
*Hyundong Jin,Joonghyuk Hahn,Yo-Sub Han*

Main category: cs.AI

TL;DR: 该论文提出了基于PSPACE完全正则表达式问题的新基准，用于评估大型语言模型和推理模型的空间计算限制。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在自然语言处理、数学推理和编程方面表现强劲，但其计算限制（特别是受有限上下文窗口约束的空间复杂性）仍未得到充分理解。现有研究多关注NP复杂度问题，需要更严格的标准来评估计算能力。

Method: 通过双指数空间探索构建包含超过百万个正则表达式实例的标记数据集，建立基于PSPACE完全正则表达式问题（等价性判定和最小化）的基准，并对6个LLM和5个LRM进行广泛评估。

Result: 评估揭示了常见的失败模式，如冗长和重复。这是对LLM和LRM空间计算限制的首次实证研究。

Conclusion: 该工作为评估高级推理能力提供了新框架，展示了PSPACE完全问题作为评估计算能力的更严格标准的重要性。
...
### [242] [Fundamentals of Building Autonomous LLM Agents](https://arxiv.org/abs/2510.09244)
...
<details>
  <summary>Details</summary>
Motivation: 传统LLMs在现实世界任务中存在局限性，研究旨在探索开发能够自动化复杂任务、缩小与人类能力差距的智能体化LLMs的模式。

Method: 构建包含四个关键组件的系统：感知系统（将环境感知转换为有意义的表示）、推理系统（制定计划、适应反馈、评估行动）、记忆系统（通过短期和长期机制保留知识）、执行系统（将内部决策转化为具体行动）。

Result: 通过集成这些系统，可以创建更强大和通用的软件机器人，模仿人类认知过程实现自主智能行为。

Conclusion: 集成感知、推理、记忆和执行系统能够开发出更智能、更自主的LLM智能体，有效缩小与人类能力的差距。
...
### [243] [Localist LLMs -- A Mathematical Framework for Dynamic Locality Control](https://arxiv.org/abs/2510.09338)
*Joachim Diederich*

Main category: cs.AI

TL;DR: 提出了一种新颖框架，通过可调节的局部化参数在训练和推理过程中动态控制语言模型的表示方式，从局部化（可解释）到分布式（高效）编码连续调节。


<details>
  <summary>Details</summary>
Motivation: 为了解决大型语言模型在可解释性和性能之间的权衡问题，特别是在需要透明度和能力的受监管领域，开发能够连续调节表示方式的框架。

Method: 使用组稀疏惩罚注意力机制、信息理论锚点设计和动态规则注入，通过可调节的局部化参数控制表示方式，无需重新训练模型。
...
### [244] [Toward Mechanistic Explanation of Deductive Reasoning in Language Models](https://arxiv.org/abs/2510.09340)
*Davide Maltoni,Matteo Ferrara*

Main category: cs.AI

TL;DR: 小语言模型通过习得底层规则而非统计学习来解决演绎推理任务，研究发现归纳头在规则补全和规则链式推理中起核心作用。
...
Result: 发现模型通过习得底层规则来执行逻辑推理，归纳头是实现规则补全和规则链式推理的关键组件。

Conclusion: 语言模型能够学习并执行逻辑推理规则，其中归纳头是实现推理过程的核心机制，这为理解模型内部推理机制提供了重要见解。
...
### [245] [Sequence Variables: A Constraint Programming Computational Domain for Routing and Sequencing](https://arxiv.org/abs/2510.09373)
...
### [246] [Agentic Systems in Radiology: Design, Applications, Evaluation, and Challenges](https://arxiv.org/abs/2510.09404)
...
TL;DR: 本文综述了基于大语言模型（LLMs）的智能体系统在放射学中的应用，探讨了从半自动化工作流到自适应智能体的设计、评估方法和挑战。


<details>
  <summary>Details</summary>
Motivation: 放射学具有多模态数据流和复杂工作流程的特点，适合利用能够适应上下文并自动化重复复杂任务的智能体系统。虽然LLMs在单个任务上表现良好，但单独使用未能充分利用其在复杂多步骤工作流中的潜力。

Method: 通过为LLMs配备外部工具和反馈机制，构建能够驱动系统表现出不同程度自主性的智能体系统，范围从半自动化工作流到能够管理复杂过程的自适应智能体。

Result: LLM驱动的智能体系统在放射学中已显示出潜力，能够整合多源信息并执行复杂决策过程，但需要解决错误级联、工具使用效率和医疗IT集成等挑战。

Conclusion: LLM驱动的智能体系统为放射学工作流程自动化提供了有前景的途径，但需要进一步研究解决实际部署中的技术和集成挑战。
...
### [247] [Safe, Untrusted, "Proof-Carrying" AI Agents: toward the agentic lakehouse](https://arxiv.org/abs/2510.09567)
...
Method: 使用Bauplan作为案例研究，展示数据分支和声明式环境如何自然扩展到智能体，通过受证明携带代码启发的正确性检查来修复数据管道。

Result: 原型演示表明不受信任的AI智能体可以在生产数据上安全操作，并概述了实现完全智能体化湖仓的路径。

Conclusion: API优先、可编程的湖仓为安全设计的智能体工作流提供了合适的抽象，能够实现可复现性和可观测性，同时减少攻击面。
...
### [248] [GraphMERT: Efficient and Scalable Distillation of Reliable Knowledge Graphs from Unstructured Data](https://arxiv.org/abs/2510.09580)
...
<details>
  <summary>Details</summary>
Motivation: 神经符号AI领域近30年来未能实现其潜力，主要因为大多数框架无法扩展，且神经方法的隐式表示和近似推理限制了可解释性和可信度。知识图谱作为显式语义知识的黄金标准表示可以解决符号方面的问题，但从文本语料库自动推导可靠知识图谱仍是一个开放问题。
...
### [249] [LiveOIBench: Can Large Language Models Outperform Human Contestants in Informatics Olympiads?](https://arxiv.org/abs/2510.09595)
*Kaijian Zou,Aaron Xiong,Yunxiang Zhang,Frederick Zhang,Yueqi Ren,Jirong Yang,Ayoung Lee,Shitanshu Bhushan,Lu Wang*

Main category: cs.AI

TL;DR: LiveOIBench是一个包含403个奥林匹克级别编程竞赛题目的基准测试，具有高质量测试用例和离线评估系统，用于评估大型语言模型的编程能力。


<details>
  <summary>Details</summary>
Motivation: 当前编程基准测试存在缺乏高难度问题、测试用例覆盖不足、依赖在线平台API等问题，需要更全面和易访问的评估工具。

Method: 从72个官方信息学奥林匹克竞赛中收集403个专家策划的问题，每个问题平均60个测试用例，建立包含详细子任务评分标准和人类表现数据的评估系统。

Result: GPT-5达到81.76百分位，表现强劲但仍不及顶尖人类选手（通常超过90百分位）；开源模型GPT-OSS-120B仅达60百分位，与前沿闭源模型存在显著差距。
