...
Method: 提出显著认知解耦(SED)方法，将专家构建的二部图注入模型，解耦视图-疾病关联，并通过强化学习引导模型沿临床步骤进行偏好选择。同时构建了首个大规模胎儿超声报告数据集FetalSigma-1M。

Result: FetalMind在所有孕周阶段均优于开源和闭源基线，平均增益+14%，在关键病症上的准确率提高61.2%，同时保持高效、稳定和可扩展性。

Conclusion: FetalMind通过临床工作流引导的SED方法和大规模数据集，成功解决了胎儿超声领域的独特挑战，为医疗AI在胎儿超声诊断中的应用提供了有效解决方案。
...
Method: 1) 设计不依赖领域显式知识的提示模板，使用LLM增强文本多义性描述；2) 使用超图适配器构建文本和图像间的多边连接，校正同义语义的正负匹配误差。

Result: 在Flickr30K和MS-COCO基准测试中，文本到图像检索提升16.8%，图像到文本检索提升40.1%，在语义对齐任务中达到新的最先进性能。

Conclusion: OS-HGAdapter通过熵增强对齐和超图适配器有效解决了跨模态检索不平衡问题，显著提升了检索性能，为多媒体内容理解提供了有效解决方案。
...
Method: 扩展3DGUT支持复杂相机模型，通过自动分块策略支持任意旋转LiDAR模型和基于射线的剔除，设计因子化3D高斯表示和锚定策略解决跨传感器不一致性。

Result: 相比现有方法，相机和深度误差降低达40%，渲染速度比光线追踪方法快10-20倍，比基于光栅化的方法快1.5-10倍，在两个自动驾驶数据集上达到或超过最先进方法的保真度。

Conclusion: SimULi首次实现了任意相机模型和LiDAR数据的实时渲染，解决了多传感器模拟中的关键挑战，为自动驾驶等应用的严格测试提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前手术AI研究主要关注理解正在发生的事件，而非预测未来事件。现有方法针对孤立任务，缺乏统一方法，且基于未来特征预测的方法难以在不同手术上下文和程序中泛化。

Method: 提出SurgFUTR框架，采用教师-学生架构：视频片段通过Sinkhorn-Knopp聚类压缩为状态表示；教师网络学习当前和未来片段，学生网络仅从当前视频预测未来状态，由Action Dynamics模块指导。

Result: 在四个数据集和三个手术程序上的实验显示一致改进。跨程序转移验证了泛化能力。

Conclusion: 通过将手术未来预测重构为状态变化学习，SurgFUTR实现了跨手术程序的通用预测能力，在短期和长期预测任务上均表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习植物病害诊断系统在不同环境条件下性能下降严重，主要原因是训练数据多样性不足和领域差异

Method: 基于度量学习的目标感知度量学习与优先级采样框架，利用少量目标域标注样本提升模型泛化能力

Result: 在包含223,073张叶片图像的大规模数据集上，仅使用每个病害10个目标域样本，TMPS相比基线方法在宏F1分数上分别提升7.3和3.6个百分点

Conclusion: TMPS框架能有效利用少量目标域样本显著提升植物病害诊断模型在未见领域的鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型依赖标注图像数据集的问题，充分利用大量未标注图像数据，实现零标签学习

Method: 提出统一视觉语言对齐框架ViZer，在训练过程中主动对齐视觉和语言表示特征，无需依赖人工或合成标注数据集

Result: 在SmolVLM-Base和Qwen2-VL上应用ViZer，生成更接地气和描述性的图像描述，在定性评估中表现优势

Conclusion: ViZer为视觉语言任务的零标签适应提供了实用起点，能够有效提升图像描述质量而无需额外标注
...
<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型在采样过程中存在细节不足、伪影控制不佳和提示遵循度不够的问题，需要一种无需重新训练就能提升图像质量的采样器级指导方法。

Method: 开发了ZeResFDG模块，包含三个核心组件：频率解耦指导、能量重缩放和零投影；结合轻量级频谱EMA进行模式切换，以及QSilk微颗粒稳定器进行推理时稳定。

Result: 在SD/SDXL采样器上，该方法在中等指导尺度下显著改善了图像清晰度、提示遵循度和伪影控制，同时在高分辨率下产生自然的高频微纹理。

Conclusion: CADE 2.5提供了一种无需重新训练的有效采样器级指导方案，能够显著提升扩散模型的图像生成质量，特别是在细节增强和稳定性方面。
...
Method: 使用共享编码器和路由编码器池，通过轻量级路由器基于文本提示和共享视觉特征的交叉注意力动态选择最优编码器，采用双熵正则化和辅助损失训练路由器。

Result: 仅使用1个共享+1个路由编码器的SCOPE模型优于同时使用4个额外编码器的模型，同时减少24-49%的计算量。
...
Method: 提出SVAGFormer基线框架，该框架适配了最先进的视觉语言模型，用于联合空间和时间定位。
...
<details>
  <summary>Details</summary>
Motivation: 现有T2V生成模型在创建视觉吸引人的视频方面取得进展，但在生成需要多个事件逻辑递进的连贯序列叙事方面存在困难，且现有基准主要关注视觉质量指标，缺乏对扩展序列中叙事连贯性的评估。

Method: 开发了SeqBench基准，包含精心设计的320个提示数据集，涵盖各种叙事复杂度，包含2560个人工标注视频；设计了基于动态时序图的自动评估指标，能高效捕捉长距离依赖和时序排序。

Result: 基于DTG的指标与人工标注表现出强相关性；系统评估揭示了当前T2V模型的关键局限：在多动作序列中无法保持一致的物体状态、多物体场景中产生物理上不合理的结果、难以保持顺序动作之间的现实时序和排序关系。
...
<details>
  <summary>Details</summary>
Motivation: 人类运动具有多样性和语义丰富性，同时受到周围场景的影响。现有运动生成方法孤立地处理运动语义或场景感知，因为构建同时具有丰富文本-运动覆盖和精确场景交互的大规模数据集极具挑战性。

Method: 使用两个适应阶段：1）中间帧生成阶段引入关键帧层来调节运动潜在空间；2）场景感知中间帧生成阶段添加场景条件层，通过交叉注意力自适应查询局部上下文来注入场景几何信息。

Result: 实验结果表明SceneAdapt有效地将场景感知注入文本到运动模型，并进一步分析了这种感知出现的机制。

Conclusion: SceneAdapt通过利用不相交数据集和中间帧生成作为代理任务，成功地将场景感知能力注入文本条件运动模型，解决了同时处理运动语义和场景交互的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督新颖视图合成方法的预测位姿缺乏可迁移性——相同的位姿集在不同3D场景中会产生不同的相机轨迹，无法实现真正的视图合成。

Method: 结合成对位姿估计与输入输出增强方案，联合实现相机位姿与场景内容的解耦，无需3D归纳偏置或多视图几何概念。

Result: XFactor显著优于先前的无位姿NVS变换器，潜在位姿与现实世界位姿高度相关，实现了真正的位姿可迁移性。

Conclusion: XFactor证明了无需显式SE(3)位姿参数化，仅通过自监督学习就能实现真正可迁移的新颖视图合成。
...
<details>
  <summary>Details</summary>
Motivation: 现有的学习方法在训练时使用梯度幅值损失，但这种方法丢失了方向信息，导致监督模糊和边缘保真度不理想。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割模型在不同数据集间部署时，由于域偏移（包括图像外观的风格差异和人群相关的解剖结构内容差异）导致性能下降，特别是在海马体分割中处理年轻健康人群到临床痴呆患者的转移时。

Method: 结合z-归一化进行高效风格协调，采用双向可变形图像配准策略，配准网络与分割网络和判别器网络联合训练，生成解剖学上合理的变换以对齐源图像到目标域。

Result: 在合成数据集和三个MRI海马体数据集上的综合评估显示，该方法优于现有基线。从年轻健康人群转移到临床痴呆患者时，Dice得分相对标准增强方法提升高达15%，在内容偏移较大的场景中增益最显著。

Conclusion: 该框架在多样化人群中进行准确海马体分割方面具有高效性，特别适用于处理显著内容偏移的跨域分割任务。
...
<details>
  <summary>Details</summary>
Motivation: 扩散概率模型在生成任务中取得了显著进展，但经常产生与现实世界知识冲突的幻觉样本。目前缺乏系统量化此类幻觉的方法，阻碍了解决这一挑战的进展。

Method: 构建包含ToyShape、SimObject和RealHand的CountHalluSet数据集套件，开发标准化评估协议，系统研究不同采样条件（求解器类型、ODE求解器阶数、采样步数、初始噪声）对计数幻觉水平的影响。

Result: 发现不同的采样条件显著影响计数幻觉水平，并揭示FID等广泛使用的图像质量指标无法一致地捕捉计数幻觉。
...
Method: 1. 引入时空特征记忆库(SFM)缓存空间注意力处理的关键图像标记；2. 提出特征最相似传播(FMP)方法，将前一帧的最相关标记传播到后续帧；3. SFM更新算法持续刷新缓存特征；4. 利用交叉注意力图自动提取感兴趣实例的掩码。

Result: 大量实验表明，Edit-Your-Interest在效率和视觉保真度方面均优于最先进的方法，验证了其优越的有效性和实用性。

Conclusion: 该方法通过轻量化的时空特征管理和自动掩码提取，实现了高效、高质量的视频编辑，在保持时间一致性和背景完整性的同时显著降低了计算开销。
...
<details>
  <summary>Details</summary>
Motivation: 随着AR/VR技术融入日常生活，需要能够从自我中心视角理解人类社交动态的AI。当前LLM缺乏社交意识，导致持续、不合时宜的响应，干扰自然对话并影响用户专注度。

Method: 创建了包含13,500个社交视频-问题对的EgoSocial数据集，并提出EgoSoD方法，该方法整合多模态上下文线索（音频和视觉）到社交思维图中，动态建模参与者和互动。

Result: 实验显示现有OLLM在检测干预时机方面表现不佳（Gemini 2.5 Pro仅为14.4%）。EgoSoD方法在干预时机检测上比Phi-4提升45.6%，比Gemini 2.5 Pro提升9.9%；在整体社交互动性能上分别提升20.4%和6.9%。

Conclusion: EgoSoD方法通过多模态上下文建模能够有效检测社交动态和干预时机，显著提升了AI助手在社交环境中的表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有的自动驾驶规划器评估指标（如EPDMS）在复杂场景中缺乏上下文感知能力，无法与人类判断有效对齐。

Method: 开发了DriveCritic数据集（包含需要上下文判断的挑战性场景和成对人类偏好标注）和DriveCritic模型（基于VLM，通过监督学习和强化学习两阶段训练，整合视觉和符号上下文来评估轨迹对）。
...
Method: 使用变分原理(VP)网格生成方法构建具有指定雅可比行列度和旋度的无折叠网格，在微分同胚群中生成逆变换而非图像空间操作。

Result: 在OASIS-1数据集的150个脑部扫描配准中，基于35个感兴趣区域的Dice分数评估显示，VPreg在Dice分数、变换规则性以及逆映射的准确性和一致性方面均优于ANTs-SyN、Freesurfer-Easyreg和FSL-Fnirt等先进方法。

Conclusion: VPreg通过变分原理网格生成方法实现了高质量的微分同胚图像配准，在配准精度、变换规则性和逆映射质量方面均优于现有方法，为计算解剖学和形态计量学提供了可靠工具。
...
<details>
  <summary>Details</summary>
Motivation: 研究3D CNN和LSTM在实时美国手语识别中的性能差异，为开发辅助技术提供专业基准，平衡识别精度和实时操作要求。

Method: 在包含50个类别、1200个ASL手势的数据集上，评估3D CNN和LSTM架构的准确性、计算效率和延迟性能，使用相似的训练条件进行比较。

Result: 3D CNN达到92.4%的识别准确率，但每帧处理时间比LSTM多3.2%；LSTM保持86.7%的准确率，资源消耗显著更低；混合3D CNN-LSTM模型表现良好。

Conclusion: 上下文相关的架构选择对于实际实现至关重要，需要在边缘计算环境中权衡识别精度和实时操作要求。
...
Result: 将容量限制从100比特提升到500比特，准确率达到2000比特中仅1比特错误，视觉质量达到31.47 dB PSNR和0.13 LPIPS。

Conclusion: 通过新颖的感知设计在多模态潜在表示中的应用，证明了在隐写术中实现高容量、高准确性和良好视觉质量的有效性。
...
Method: 使用字典学习将TEM信号的内在物理特性（如指数衰减和平滑性）编码为字典驱动先验，在训练时集成到模型中，在测试时通过最小化基于字典一致性和信号一阶变化的自监督损失来动态适应新环境。

Result: 大量实验结果表明，该方法在TEM去噪性能上显著优于现有TEM去噪方法和TTA方法。

Conclusion: 提出的DP-TTA方法通过利用TEM信号的内在物理特性作为先验知识，有效提升了模型在不同地理环境下的泛化能力和去噪性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统边缘资源管理方法强调通信吞吐量或通用学习性能，不适用于专门优化高斯泼溅质量的需求。需要解决如何评估不同客户端视图贡献的异构性，同时避免因果困境的问题。

Method: 采用两阶段方法：1) 使用特征域聚类(FDC)和传输时间最小化(PTTM)从每个客户端采样代表性图像作为导频数据；2) 基于导频评估，使用基于惩罚交替主化最小化(PAMM)算法的联合客户端选择和功率控制(JCSPC)框架优化资源分配。

Result: 实验表明该方法在真实数据集上显著优于现有基准，能够以低采样率(如10%)准确预测高斯泼溅目标函数，在视图贡献和通信成本之间实现了良好平衡。

Conclusion: 提出的STT-GS策略有效解决了边缘高斯泼溅中的资源管理问题，通过智能采样和资源分配机制，在保证重建质量的同时优化了通信效率。
...
Method: 提取输入图像的分割、图形和深度特征，引入可变形多级融合机制融合这三种多级特征，并融入SAM知识蒸馏来增强预测精度。
...
<details>
  <summary>Details</summary>
Motivation: AI会议的快速增长给同行评审系统带来压力，导致评审工作量大、专业知识不匹配、评价标准不一致、评审质量差等问题，现有改进措施往往造成更多困惑。

Method: 开发Paper Copilot系统，创建跨计算机科学会议的持久数字评审档案，构建开放数据集，并对ICLR多年评审进行大规模实证分析。

Result: 提供了基础设施和数据集，支持对同行评审演化的可重复研究，帮助社区追踪变化、诊断问题模式。

Conclusion: 这些资源有助于社区基于证据改进评审系统，建立更稳健、透明和可靠的同行评审机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么过度简化风格多样性，要么忽略区域运动风格差异（如上身与下身），且未能让运动风格动态适应语音节奏和情感变化

Method: 将身体分为不同区域编码局部运动风格，使用分区感知注意力块让节奏和情感线索精确指导每个身体区域

Result: 实验结果表明该方法优于现有方法，生成了更自然和富有表现力的3D人体运动序列

Conclusion: MimicParts框架能够有效捕捉细粒度区域风格差异，并让运动风格动态适应语音变化，显著提升了运动生成的真实感
...
Method: 从第一性原理重新审视VP和VPT设计，将其概念化为统一的提示式适应(PA)框架，提供可学习、生成式和不可学习提示的分类体系，并按注入粒度组织。

Result: 建立了PA方法的系统分类，考察了PA在医学影像、3D点云、视觉语言任务等领域的应用，以及在测试时适应和可信AI中的作用。

Conclusion: 这是首个专门针对PA方法学及其应用的全面综述，旨在为研究人员提供清晰的路线图来理解和探索PA相关研究的演进格局。
...
<details>
  <summary>Details</summary>
Motivation: 工业表面缺陷检测面临极端的前景-背景不平衡、缺陷稀疏性、长尾尺度分布和低对比度等挑战。现有模型虽然像素重叠指标（如mIoU）表现良好，但在样本级决策稳定性不足，特别是对于稀疏或细长缺陷。根本原因是优化目标与质量决策粒度不匹配。

Method: 基于共享编码器架构的多任务学习框架，联合学习样本级缺陷分类和像素级掩码定位。样本级监督调节特征分布，在梯度层面持续提升小缺陷和低对比度缺陷的召回率，分割分支保留边界和形状细节以增强样本级决策稳定性。

Result: 在两个基准数据集上的实验表明，该方法显著提高了样本级决策的可靠性和缺陷定位的完整性。

Conclusion: 提出的样本中心多任务学习框架和评估套件有效解决了工业缺陷检测中样本级决策稳定性的问题，通过联合优化分类和分割任务，提升了小缺陷和低对比度缺陷的检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的视觉语言模型在理解否定时存在严重的肯定性偏见，特别是在描述性目标检测任务中表现不佳，这限制了模型在真实世界检测应用中的实用性。

Method: 提出了两个主要贡献：(1) CoVAND数据集，使用系统化的思维链和VQA流程生成高质量的实例接地否定数据；(2) NegToMe模块，通过文本令牌合并直接解决肯定性偏见的架构原因，将否定词与属性词合并为连贯的语义短语。

Result: 该方法在具有挑战性的否定基准测试中显著提升了性能，将NMS-AP在OVDEval上提高了+10.8分，降低了假阳性率，并证明了在最新视觉语言模型上的泛化能力。

Conclusion: 这项工作在解决真实世界检测应用中的否定理解问题方面迈出了关键一步，为视觉语言模型的否定理解能力提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖大型语言模型的静态文本描述，缺乏对实时目标状态变化的适应性且容易产生幻觉，需要更灵活的多模态跟踪解决方案。

Method: 使用显式提示将空间运动信息转换为自然语言描述，隐式提示结合伪词和可学习描述符构建个性化知识表示，并通过CLIP文本编码器进行动态调整，同时设计了判别性特征增强器。

Result: 在MOT17、MOT20和DanceTrack数据集上的大量实验表明，EPIPTrack在多样化场景中优于现有跟踪器，展现出强大的适应性和优越性能。

Conclusion: EPIPTrack通过显式和隐式提示的动态调整机制，有效解决了多模态跟踪中的适应性问题和幻觉问题，为实时目标跟踪提供了鲁棒的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管VLA模型在机器人学习方面取得了革命性进展，但其对抗鲁棒性仍未得到充分探索。本文旨在填补这一空白，研究VLA模型的对抗脆弱性并提出相应的保护措施。

Method: 提出了EDPA攻击方法，通过优化两个目标：(1)破坏视觉与文本潜在表示的语义对齐；(2)最大化对抗样本与干净样本潜在表示的差异。同时提出了针对视觉编码器的对抗性微调防御方案。

Result: 在LIBERO机器人仿真基准测试中，EDPA显著提高了最先进VLA模型的任务失败率，而提出的防御方法有效缓解了这种性能下降。

Conclusion: 本文揭示了VLA模型在对抗攻击下的脆弱性，并提出了有效的攻击和防御方法，为构建更安全的机器人系统提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 由于在真实环境中收集和标注无人机数据成本高昂且困难，需要创建大规模、多样化的数据集来支持计算机视觉算法在无人机应用中的发展。

Method: 基于SynDrone和FlyAware数据集，FlyAwareV2引入了多模态数据（RGB、深度、语义标签）、通过单目深度估计计算真实样本的深度图、提供RGB和多模态语义分割的基准测试，以及研究合成到真实的域适应。

Result: FlyAwareV2提供了丰富的注释和环境多样性，成为无人机基于3D城市场景理解研究的宝贵资源。

Conclusion: FlyAwareV2通过其多模态数据和环境多样性，有效解决了无人机数据收集的挑战，为城市场景理解任务提供了重要的数据集支持。
...
Method: 引入SketchSem3D基准数据集，包含基于草图的SemanticKITTI和KITTI-360子集；提出CymbaDiff方法，通过结构化空间排序、圆柱连续性和垂直层次捕获来增强空间连贯性。

Result: 在SketchSem3D上的广泛实验表明，CymbaDiff在语义一致性、空间真实性和跨数据集泛化方面表现优异。

Conclusion: SketchSem3D为户外3D语义场景生成提供了标准化评估基准，CymbaDiff方法有效解决了空间连贯性问题，推动了该领域的发展。
...
Method: 设计stem-encoder-decoder结构：stem网络使用大卷积核扩大感受野提取头部细节；编码器部分采用条件通道加权和多分支局部融合块以低计算量融合多尺度特征；顶部添加特征金字塔网络缓解不完全融合问题。

Result: 在三个基准测试上验证，模型在NVIDIA GTX 1080Ti上达到381.7 FPS，在NVIDIA Jetson TX1上达到71.9 FPS，推理速度最快，同时保持竞争力的准确率。

Conclusion: 该网络适用于嵌入式系统上的超实时人群计数，实现了最快的推理速度，为智能安防、城市规划等领域的实际应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管VideoLLMs在视频问答方面取得进展，但其内部如何提取和传播视频与文本信息的机制仍未被充分探索。

Method: 使用机制可解释性技术分析VideoLLMs的内部信息流，研究不同VideoQA任务中的一致模式。

Result: 发现时间推理的四个阶段：早期到中间层跨帧交互、中间层视频-语言集成、中后期层生成答案。通过选择有效信息路径，可以在抑制58%注意力边的情况下保持性能。

Conclusion: 这些发现揭示了VideoLLMs执行时间推理的机制，为提高模型可解释性和下游泛化提供了实用见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有端到端多模态模型使用不同的编码器和解码器处理不同模态，这种分离阻碍了多模态的联合表示学习。

Method: 采用基于Mamba的多步选择扩散模型，通过统一的变分自编码器逐步生成和细化模态特定信息，实现编码和解码的统一。
...
Method: 构建了MMLongCite基准，包含8个不同任务，涵盖6个上下文长度区间，整合了文本、图像和视频等多种模态。

Result: 评估发现最先进的大视觉语言模型在处理长多模态上下文时忠实度有限，并分析了上下文长度和关键内容位置对模型忠实度的影响。

Conclusion: 需要改进大视觉语言模型在长多模态上下文中的忠实度，MMLongCite为这一研究方向提供了重要基准。
...
Conclusion: MaskDCPT提供了一种有效的图像恢复预训练方法，显著提升性能并释放了UIR-2.5M数据集供研究使用。
...
TL;DR: 提出一个自动文档分类系统，能够检测图像中的文本内容并将文档分类为发票、报告、信件和表格四类，支持离线图像和实时摄像头捕获。


<details>
  <summary>Details</summary>
Motivation: 解决实际应用中遇到的挑战，如不同光照条件、任意方向、弯曲或部分遮挡文本、低分辨率和远距离文本等问题，实现无约束成像场景下的文档分类。
...
Conclusion: 所提出的方法在无约束成像场景下对于混合来源的文档分类是有效的，具有实际应用价值。
...
Method: 使用结构因果模型(SCM)重新形式化3D-NCD问题，提出因果表示和推理联合学习方法。首先分析基础类别表示中的隐藏混杂因子，设计消除混杂因子的因果表示原型，然后使用图结构建模基础类别和新类别原型之间的因果关系。

Result: 在3D和2D NCD语义分割上的大量实验和可视化结果证明了该方法的优越性。

Conclusion: 通过因果表示学习和推理，能够有效解决点云分割中的新类发现问题，显著提升分割性能。
...
TL;DR: 本文提出了一种基于GPU并行计算的SfM方法，通过加速标准SfM流程中的关键阶段，在大规模场景下实现了比COLMAP快约40倍的速度，同时保持或提高了重建精度。


<details>
  <summary>Details</summary>
Motivation: 传统SfM方法（如COLMAP、GLOMAP）在处理大规模场景时存在计算开销大、灵活性有限的问题，而基于深度学习的SfM方法（如VGGSfM、VGGT）又受限于GPU内存无法处理数千张输入图像。

Method: 利用GPU并行计算加速标准SfM流程的每个关键阶段，基于稀疏感知的bundle adjustment优化技术，在统一的全局SfM框架中同时加速BA和GP。

Result: 在不同规模的数据集上（包括5000张图像的大规模场景）进行实验，相比COLMAP实现了约40倍的速度提升，同时重建精度相当或更好。

Conclusion: 该方法成功释放了GPU并行计算的潜力，解决了大规模SfM场景下的速度和精度权衡问题。
...
### [45] [Language as a Label: Zero-Shot Multimodal Classification of Everyday Postures under Data Scarcity](https://arxiv.org/abs/2510.13364)
*MingZe Tang,Jubal Chandy Jacob*

Main category: cs.CV

TL;DR: 研究发现，在视觉语言模型的零样本分类中，对于高性能模型，简单提示词比详细描述效果更好，存在'提示词过拟合'现象；而低性能模型则能从详细提示词中受益。


<details>
  <summary>Details</summary>
Motivation: 探索提示词设计对视觉语言模型在识别视觉相似类别（如人体姿态）时的影响，特别是在数据稀缺条件下的零样本分类性能。

Method: 在285张COCO数据集图像上评估OpenCLIP、MetaCLIP 2和SigLip等现代VLM模型，采用三层次提示词设计，系统性地增加语言细节。

Result: 高性能模型（MetaCLIP 2和OpenCLIP）使用最简单提示词时效果最佳（准确率68.8%），添加描述性细节会显著降低性能（降至55.1%）；低性能模型（SigLip）在模糊类别上能从详细提示词中获益。

Conclusion: 提示词设计对VLM性能有显著影响，高性能模型存在'提示词过拟合'现象，简单提示词效果更好；而低性能模型需要更详细的提示词来改善分类效果。
...
TL;DR: 本文探讨大型多模态模型（特别是GPT-4o）在理解视觉趣味性概念方面的能力，通过比较分析评估人类评估与模型预测的一致性，并利用模型标注数据训练学习排序模型。


<details>
  <summary>Details</summary>
Motivation: 探索大型多模态模型是否能够捕捉视觉趣味性概念，以及人类评估与模型预测之间的对齐程度，这对于理解人类兴趣具有重要意义。

Method: 通过比较分析研究人类评估与GPT-4o预测的一致性，利用模型标注图像对的有趣程度作为训练数据，将知识蒸馏到学习排序模型中。

Result: 研究发现人类与GPT-4o在视觉趣味性评估上存在部分对齐，GPT-4o在捕捉该概念方面表现优于现有最先进方法，能够有效标注图像对的有趣程度。

Conclusion: 大型多模态模型能够有效理解视觉趣味性概念，为深入理解人类兴趣提供了新的途径，通过知识蒸馏可以构建更高效的学习排序模型。
...
<details>
  <summary>Details</summary>
Motivation: 基于经验观察发现，当RAFT等光流估计管道的其他网络部分充分训练后，代价体积的重要性会降低。代价体积的计算和空间复杂度限制了处理速度和输入帧分辨率。

Method: 引入一种训练策略，在训练过程中逐步移除代价体积，创建了三种不同计算预算的模型。

Result: 最准确的模型达到最先进精度，同时比可比模型快1.2倍、内存占用低6倍；最快的模型能够在仅使用500MB GPU内存的情况下以20FPS处理全高清帧。

Conclusion: 该训练策略成功实现了在保持准确性的同时显著提升光流估计的速度和效率，证明了代价体积在训练后期可以被有效移除。
...
TL;DR: 提出了一种基于热成像的隐蔽武器检测新方法DEF-YOLO，并创建了首个大规模热成像隐蔽武器检测数据集TICW。通过引入可变形卷积和焦点损失等技术，在保持实时性的同时提高了检测精度。


<details>
  <summary>Details</summary>
Motivation: 现有隐蔽武器检测技术存在分辨率低、隐私问题等限制。为了提供实时、低成本且保护隐私的解决方案，选择热成像技术，但缺乏基准数据集。

Method: 基于YOLOv8架构改进，在SPPF层引入可变形卷积以利用多尺度特征；在骨干网络和颈部网络提取低、中、高级特征；使用焦点损失解决类别不平衡问题。

Result: 提出的DEF-YOLO方法在热成像隐蔽武器检测任务中建立了新的性能基准，在保持速度和吞吐量的同时提高了检测精度。

Conclusion: 该工作为热成像隐蔽武器检测提供了首个大规模数据集和有效的检测方法，为解决该领域的挑战提供了实用解决方案。
...
Method: 提出Group-VQ进行分组优化码本，每组独立优化，组内联合优化；同时引入训练后码本重采样方法。

Result: 在各种图像重建实验中，Group-VQ在重建指标上表现更好，训练后码本采样方法实现了码本大小调整的灵活性。

Conclusion: Group-VQ改善了码本利用率和重建性能之间的权衡，训练后码本重采样提供了调整码本大小的灵活性。
...
TL;DR: 提出了一个针对渲染视频的无参考质量评估方法和数据集，解决了现有方法对渲染视频评估不准确的问题


<details>
  <summary>Details</summary>
Motivation: 现有无参考视频质量评估方法主要针对相机拍摄视频，直接应用于渲染视频会产生偏差，因为渲染视频更容易出现时间伪影

Method: 创建了一个大规模渲染视频数据集，包含多种3D场景和渲染设置，并设计了专门针对渲染视频的无参考质量评估指标，同时考虑图像质量和时间稳定性

Result: 提出的指标在渲染视频上表现优于现有无参考视频质量评估方法，能够有效评估超采样方法和实时渲染中的帧生成策略

Conclusion: 渲染视频需要专门的质量评估方法，提出的数据集和指标为渲染视频质量评估提供了有效工具，能够更好地反映实际应用场景
...
TL;DR: 提出UniME-V2模型，利用MLLM的语义理解能力增强多模态嵌入学习，通过MLLM-as-a-Judge机制评估查询-候选对语义对齐，生成软语义匹配分数用于硬负样本挖掘和训练。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以捕捉候选样本间的细微语义差异，负样本缺乏多样性，嵌入表示在区分假负样本和硬负样本方面能力有限。

Method: 1) 通过全局检索构建潜在硬负样本集；2) 引入MLLM-as-a-Judge机制评估语义对齐并生成软语义匹配分数；3) 利用匹配分数进行硬负样本挖掘和训练；4) 提出UniME-V2-Reranker重排序模型。

Result: 在MMEB基准测试和多个检索任务上进行了全面实验，在所有任务上平均达到了最先进的性能。

Conclusion: 该方法通过MLLM增强的语义理解和软标签训练，显著提升了多模态嵌入模型的判别能力，在多个任务上实现了最优性能。
...
TL;DR: DepthVLA是一个增强空间推理能力的视觉-语言-动作模型，通过集成预训练深度预测模块来提升空间感知能力，在真实世界和模拟环境中都显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型在需要精确空间推理的任务上表现不佳，因为它们继承了VLMs有限的空间推理能力，且需要大量动作数据预训练来在3D空间中定位，这降低了训练效率且空间理解仍然不准确。

Method: 采用混合变换器设计，统一了VLM、深度变换器和动作专家，通过完全共享注意力形成端到端模型，显式地通过预训练深度预测模块融入空间感知。

Result: 在真实世界任务中达到78.5%进展（vs. 65.0%），在LIBERO模拟器中达到94.9%（vs. 93.6%），在Simpler模拟器中达到74.8%（vs. 58.8%），全面超越最先进方法。

Conclusion: DepthVLA通过显式融入空间感知，有效提升了VLA模型的空间推理能力，在多种环境中都取得了显著性能提升。
...
### [53] [Leveraging 2D Priors and SDF Guidance for Dynamic Urban Scene Rendering](https://arxiv.org/abs/2510.13381)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于3DGS的动态城市场景方法需要相机和LiDAR数据、真实3D分割和运动数据，限制了应用范围。本文旨在通过结合2D对象无关先验和SDF表示来放松这些要求。

Method: 将符号距离函数(SDF)与3D高斯泼溅(3DGS)集成，利用两者的优势创建更鲁棒的对象表示。采用统一优化框架，增强3DGS的几何精度并改进SDF内的变形建模。

Result: 在城市场景中无需LiDAR数据即可在渲染指标上达到最先进性能。结合LiDAR时，在重建和生成新视图方面表现更佳，且无需真实3D运动标注。支持场景分解和组合等编辑任务。

Conclusion: 该方法通过结合SDF和3DGS，实现了对动态场景更精确和自适应的表示，显著降低了对额外数据的需求，同时保持了高渲染质量。
...
### [54] [Modeling Cultural Bias in Facial Expression Recognition with Adaptive Agents](https://arxiv.org/abs/2510.13557)
...
TL;DR: 本文提出了一个基于代理的流式基准测试，研究文化组成和渐进模糊如何影响面部表情识别的鲁棒性。结果显示不同文化群体在模糊条件下的性能退化曲线存在不对称性。


<details>
  <summary>Details</summary>
Motivation: 现有面部表情识别评估通常假设同质数据和高质量图像，但实际应用中需要处理文化差异和感知退化条件。本文旨在研究文化组成和视觉退化如何共同影响面部表情识别的鲁棒性。

Method: 使用基于代理的流式基准测试，每个代理在冻结的CLIP特征空间中运行，配备轻量级残差适配器。代理在5x5网格上移动交互，环境提供按sigma调度的Gaussian模糊输入。研究单文化群体（西方、亚洲）和混合环境（平衡5/5，不平衡8/2、2/8）以及不同空间接触结构。

Result: 结果显示文化群体间存在不对称退化曲线：JAFFE（亚洲）群体在低模糊时保持较高性能但在中间阶段急剧下降，而KDEF（西方）群体退化更均匀。混合群体呈现中间模式，平衡混合缓解早期退化但不平衡设置在高模糊下放大主要群体弱点。

Conclusion: 文化组成和交互结构显著影响面部表情识别在感知条件恶化时的鲁棒性，量化了这些因素如何塑造FER系统的退化模式。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有WiFi手势识别方法在泛化性和语义表达能力上的不足，特别是CSI信号的领域敏感性和缺乏高级手势抽象的问题

Method: 采用双路径CSI编码（CSI-Ratio相位序列和多普勒频谱图）、多尺度语义编码器、语义感知软监督方案和鲁棒双蒸馏策略

Result: 在Widar3.0基准测试中，GLSDA在域内和跨域手势识别任务中均优于现有方法，同时显著减小模型大小和推理延迟
...
<details>
  <summary>Details</summary>
Motivation: 现有基准不足以评估视觉语言模型的空间推理能力，特别是人类空间认知的基本方面——内在动态空间推理。

Method: 基于认知分类法构建统一基准，将任务分为四个基本象限：内在静态、内在动态、外在静态和外在动态空间推理。开发可扩展的自动化流水线生成多样且可验证的空间推理问题。

Result: 创建了包含559个评估VQA对和12K+训练VQA对的Spatial-DISE数据集。对28个最先进VLMs的评估显示，当前模型在特别是多步骤多视图空间推理方面与人类能力存在显著差距。
...
TL;DR: 提出了Mask-GRPO方法，首次将基于GRPO的强化学习应用于被忽视的掩码生成模型范式，通过重新定义转移概率将去掩码过程建模为多步决策问题。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法主要针对扩散模型或自回归模型，忽略了掩码生成模型这一重要替代方案。

Method: 重新定义转移概率，将去掩码过程建模为多步决策问题，采用去除KL约束、应用缩减策略和过滤低质量样本等策略增强方法。

Result: 在标准文本到图像基准测试和偏好对齐方面显著提升了基础模型Show-o的性能，优于现有最先进方法。

Conclusion: Mask-GRPO成功将强化学习应用于掩码生成模型，为该范式提供了有效的训练方法，并在多个指标上取得了优越性能。
...
TL;DR: Patch-Adapter是一个用于高分辨率文本引导图像修复的有效框架，能够实现4K+分辨率，同时保持精确的内容一致性和提示对齐。


<details>
  <summary>Details</summary>
Motivation: 现有方法受限于较低分辨率，随着分辨率和纹理复杂度的增加，图像修复中的内容一致性和提示对齐这两个关键挑战变得更加突出。

Method: 采用两阶段适配器架构：双上下文适配器学习掩码和非掩码区域在降低分辨率下的连贯性以建立全局结构一致性；参考补丁适配器实现补丁级注意力机制进行全分辨率修复，通过自适应特征融合保持局部细节保真度。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视频掩码自回归模型存在慢启动问题（缺乏早期采样的结构化全局先验）以及在空间和时间维度上的误差累积问题。

Method: 提出画布机制（模糊的下一帧全局预测）作为掩码生成的起点，提供早期采样的全局结构；引入组合式无分类器引导联合扩大空间（画布）和时间条件；采用基于噪声的画布增强来提高鲁棒性。
...
### [60] [CoDS: Enhancing Collaborative Perception in Heterogeneous Scenarios via Domain Separation](https://arxiv.org/abs/2510.13432)
*Yushan Han,Hui Zhang,Honglei Zhang,Chuntao Ding,Yuanzhouhan Cao,Yidong Li*

Main category: cs.CV

TL;DR: 提出了CoDS方法，通过领域分离解决自动驾驶中异构场景下的协作感知特征差异问题，在保持高效推理的同时提升检测精度。


<details>
  <summary>Details</summary>
Motivation: 现有协作感知方法假设所有智能体使用相同编码器，但在实际异构场景中，不同车辆配备不同模型，导致特征差异。现有方法对领域差异敏感且推理效率低。

Method: 使用轻量级空间-通道调整器(LSCR)和基于领域分离的分布对齐(DADS)两个特征对齐模块，结合领域对齐互信息(DAMI)损失函数，采用全卷积架构确保高效推理。

Result: CoDS在异构场景中有效缓解特征差异，在检测精度和推理效率之间取得良好平衡。

Conclusion: CoDS通过领域分离策略成功解决了异构协作感知中的特征差异问题，为实际部署提供了高效可靠的解决方案。
...
TL;DR: 提出MVCustom框架，解决多视角生成与个性化定制难以统一的问题，通过特征场表示和扩散模型实现相机姿态控制和几何一致性的个性化定制。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：多视角生成模型不支持几何一致的个性化定制，而个性化定制模型缺乏明确的视角控制，难以将两者统一。

Method: 使用特征场表示学习主体身份和几何，结合文本到视频扩散主干和密集时空注意力，在推理阶段引入深度感知特征渲染和一致性感知潜在补全技术。

Result: MVCustom是唯一能同时实现忠实多视角生成和个性化定制的框架，在实验中表现出色。

Conclusion: MVCustom成功解决了多视角定制任务，实现了相机姿态控制和个性化定制的高质量统一。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法主要在2D像素上操作，难以分离对物理场景属性的选择性。需要表征神经元对形状、姿态和光照等3D场景属性的选择性。

Method: 使用可微分渲染管道优化可变形网格，用径向基函数参数化网格变形，学习最大化神经元响应的偏移和尺度，同时强制几何规律性。

Result: 应用于猴子V4区域模型，能够探测神经元对姿态和光照等可解释3D因素的选择性。

Conclusion: 该方法将逆向图形学与系统神经科学连接起来，提供了一种超越传统像素方法的、基于物理基础的3D刺激来探测神经选择性的途径。
...
### [63] [Near-Infrared Hyperspectral Imaging Applications in Food Analysis -- Improving Algorithms and Methodologies](https://arxiv.org/abs/2510.13452)
...
<details>
  <summary>Details</summary>
Motivation: 探索近红外高光谱成像在食品质量分析中的潜力，特别是比较基于卷积神经网络和偏最小二乘法的建模方法在不同分析场景下的性能差异。

Method: 通过四项研究验证五个研究假设，使用CNN和PLS模型进行比较分析，包括2D CNN增强光谱卷积层的方法，以及开发了两个开源Python包来加速PLS建模和交叉验证。

Result: CNN在联合空间-光谱分析中优于PLS，特别是在需要化学和物理视觉信息的参数建模中。增强光谱卷积层的2D CNN能改善预测性能。PLS在分析样品化学参数平均含量时表现相当，是推荐方法。

Conclusion: 近红外高光谱成像结合CNN在食品质量分析中具有优势，特别是在需要空间分布信息时。PLS在平均含量分析中仍是有效方法。开发的开源工具可加速相关研究。
...
### [64] [Multi-Scale High-Resolution Logarithmic Grapher Module for Efficient Vision GNNs](https://arxiv.org/abs/2510.13740)
*Mustafa Munir,Alex Zhang,Radu Marculescu*

Main category: cs.CV

TL;DR: 提出了LogViG模型，使用对数可扩展图构建(LSGC)方法来改进视觉图神经网络，通过限制长距离连接数量来提升性能，在图像分类和语义分割任务中超越了现有ViG、CNN和ViT架构。


<details>
  <summary>Details</summary>
Motivation: 现有视觉图神经网络(ViG)的图构建方法(如KNN)在大图像上计算昂贵，而SVGA等方法的固定步长尺度会导致过度压缩和错过长距离连接的信息增益。

Method: 提出对数可扩展图构建(LSGC)方法，并构建LogViG混合CNN-GNN模型，引入高分辨率分支并在高/低分辨率分支间融合特征，形成多尺度高分辨率视觉GNN网络。
...
### [65] [VIST3A: Text-to-3D by Stitching a Multi-view Reconstruction Network to a Video Generator](https://arxiv.org/abs/2510.13454)
*Hyojun Go,Dominik Narnhofer,Goutam Bhat,Prune Truong,Federico Tombari,Konrad Schindler*

Main category: cs.CV

TL;DR: VIST3A是一个文本到3D生成的通用框架，通过将文本到视频生成器与3D重建解码器结合，实现了高质量的文本到3D场景生成。


<details>
  <summary>Details</summary>
Motivation: 结合现代潜在文本到视频模型的生成能力和近期前馈3D重建系统的几何能力，创建一个强大的3D场景生成器。

Method: 使用模型缝合技术将文本到视频生成器与3D解码器连接，并通过直接奖励微调进行对齐，确保生成的潜在表示可解码为一致的3D几何。

Result: 所有测试的配对组合都显著优于之前输出高斯泼溅的文本到3D模型，同时通过选择合适的3D基础模型，VIST3A还能实现高质量的文本到点云图生成。
...
### [66] [RECODE: Reasoning Through Code Generation for Visual Question Answering](https://arxiv.org/abs/2510.13756)
*Junhong Shen,Mu Cai,Bo Hu,Ameet Talwalkar,David A Ross,Cordelia Schmid,Alireza Fathi*

Main category: cs.CV

TL;DR: RECODE是一个基于代码的视觉推理框架，通过将图像反渲染为可执行代码来实现可验证的多模态推理，显著提升了在图表和几何图形等结构化视觉任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在结构化视觉推理中缺乏验证机制的问题，像素级感知无法确保推理的准确性。

Method: 提出RECODE框架：首先生成多个候选程序来重现输入图像，然后通过批评器选择最忠实的重建，并迭代优化代码，将模糊的感知任务转化为可验证的符号问题。

Result: 在CharXiv、ChartQA、Geometry3K等视觉推理基准测试中，RECODE显著优于不使用代码或仅使用代码进行辅助线绘制或裁剪的方法。
...
### [67] [Through the Lens of Doubt: Robust and Efficient Uncertainty Estimation for Visual Place Recognition](https://arxiv.org/abs/2510.13464)
...
<details>
  <summary>Details</summary>
Motivation: 视觉位置识别系统在跨环境、光照、季节和视角变化时面临挑战，关键应用如SLAM中的闭环检测需要鲁棒的位置匹配不确定性估计。

Method: 提出了三种训练免费的不确定性度量：相似性分布(SD)通过候选分数分离度量匹配独特性；比率扩展(RS)评估高分位置间的竞争模糊性；统计不确定性(SU)结合SD和RS提供统一度量。

Result: 在9种最先进VPR方法和6个基准数据集上的综合评估表明，这些度量在区分正确和错误VPR匹配方面表现出色，始终优于现有方法，计算开销可忽略。

Conclusion: 该方法无需额外模型训练或架构修改，可部署于实时机器人应用，在各种环境条件下提高了精确率-召回率性能。
...
### [68] [Scaling Vision Transformers for Functional MRI with Flat Maps](https://arxiv.org/abs/2510.13768)
*Connor Lane,Daniel Z. Kaplan,Tanishq Mathew Abraham,Paul S. Scotti*

Main category: cs.CV

TL;DR: 将4D fMRI数据转换为2D活动平铺图视频，使用时空掩码自编码器训练Vision Transformers，发现性能随数据量呈幂律增长，模型支持跨被试状态解码和跨状态特质解码。
...
Method: 将4D fMRI数据转换为2D活动平铺图视频，使用时空掩码自编码器框架在Human Connectome Project的2.3K小时fMRI数据上训练Vision Transformers。

Result: 掩码fMRI建模性能随数据集大小按严格幂律缩放提升，下游分类显示模型学习到丰富表示，支持跨被试状态解码和跨状态特质解码。

Conclusion: 成功构建了fMRI基础模型，为fMRI数据分析提供了有效工具，项目持续进行开放科学研究。
...
### [69] [ExpressNet-MoE: A Hybrid Deep Neural Network for Emotion Recognition](https://arxiv.org/abs/2510.13493)
*Deeptimaan Banerjee,Prateek Gothwal,Ashis Kumer Biswas*

Main category: cs.CV

TL;DR: 提出ExpressNet-MoE混合深度学习模型，结合CNN和MoE框架，通过多尺度特征提取和自适应专家网络选择，在多个数据集上实现优越的情感识别性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的面部情感识别面临头部位置变化、遮挡、光照变化和人口多样性等挑战，现有模型在参与度检测等应用中表现有限。

Method: 结合CNN和MoE框架，包含多个CNN特征提取器、MoE模块进行自适应特征选择，以及残差网络骨干进行深度特征学习。

Result: 在多个数据集上取得优异性能：AffectNet(v7) 74.77%、AffectNet(v8) 72.55%、RAF-DB 84.29%、FER-2013 64.66%。

Conclusion: 模型具有高度适应性，可用于开发实际场景中的端到端情感识别系统。
...
### [70] [High Semantic Features for the Continual Learning of Complex Emotions: a Lightweight Solution](https://arxiv.org/abs/2510.13534)
*Thibault Geoffroy,gauthier Gerspacher,Lionel Prevost*

Main category: cs.CV

TL;DR: 该论文提出使用面部动作单元作为非瞬态特征来改进增量学习中的复杂情绪识别，有效缓解灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 解决增量学习中因学习新任务而导致对旧任务灾难性遗忘的问题，特别是在复杂情绪识别领域。

Method: 使用面部动作单元作为非瞬态、高度语义化的特征，先学习基本情绪，然后增量学习复杂情绪，构建轻量级模型。

Result: 在CFEE数据集上达到0.75的准确率，优于浅层和深度卷积神经网络提取的特征，且模型内存占用小。

Conclusion: 面部动作单元是有效的非瞬态特征，能够显著提升增量学习在复杂情绪识别中的性能，同时保持模型轻量化。
...
### [71] [Bee: A High-Quality Corpus and Full-Stack Suite to Unlock Advanced Fully Open MLLMs](https://arxiv.org/abs/2510.13795)
*Yi Zhang,Bolin Ni,Xin-Sheng Chen,Heng-Rui Zhang,Yongming Rao,Houwen Peng,Qinglin Lu,Han Hu,Meng-Hao Guo,Shi-Min Hu*

Main category: cs.CV

TL;DR: 提出了Honey-Data-15M数据集和HoneyPipe数据整理流程，通过高质量数据训练出性能领先的完全开源多模态大语言模型Bee-8B
...
Method: 构建包含约1500万问答对的Honey-Data-15M数据集，采用多重清理技术和新颖的双层（短和长）思维链增强策略，开发HoneyPipe数据整理流程和DataStudio框架

Result: 训练的Bee-8B模型在完全开源MLLMs中达到新的最先进水平，性能可与甚至超越InternVL3.5-8B等半开源模型竞争
...
TL;DR: 提出了一个神经参数化的3D乳房形状模型和基于该模型的低成本单目RGB视频3D表面重建流程，能够在2毫米误差范围内恢复准确的乳房几何形状。


<details>
  <summary>Details</summary>
Motivation: 解决现有商业3D乳房扫描方案成本高昂且需要专用硬件的问题，提供一种无需专用硬件或专有软件的低成本替代方案。

Method: 结合现成的运动恢复结构流程和参数化乳房模型，使用局部神经符号距离函数将隐式乳房域分解为多个小区域进行建模。

Result: 提出的liRBSM模型在重建质量上显著优于iRBSM，能够恢复高质量的3D乳房几何形状，误差小于2毫米，处理时间少于6分钟。
...
### [73] [Generative Universal Verifier as Multimodal Meta-Reasoner](https://arxiv.org/abs/2510.13804)
...
Method: 1) 构建ViVerBench基准评估视觉结果；2) 设计自动化流水线构建大规模验证数据并训练OmniVerifier-7B；3) 提出OmniVerifier-TTS顺序测试时扩展范式进行迭代优化。

Result: OmniVerifier-7B在ViVerBench上提升8.3分，OmniVerifier-TTS在T2I-ReasonBench和GenEval++上分别提升3.7和4.3分，优于现有并行测试时扩展方法。

Conclusion: 通过可靠的视觉验证，OmniVerifier推进了生成过程中的可靠反思和可扩展测试时优化，向更可信可控的下一代推理系统迈进。
...
TL;DR: 该论文比较了GPU和FPGA在视觉SLAM特征检测中的性能，发现传统特征检测器（FAST、Harris）在GPU上表现更好，而学习型检测器（SuperPoint）在FPGA上具有更好的运行性能和能效。


<details>
  <summary>Details</summary>
Motivation: 特征检测是SLAM中耗时但关键的模块，当前需要在功耗受限平台（如无人机）上部署，需要比较不同硬件加速方案（GPU vs FPGA）的性能差异。

Method: 在Nvidia Jetson Orin和AMD Versal SoC平台上，比较GPU和FPGA加速的FAST、Harris和SuperPoint特征检测器在视觉SLAM流水线中的性能。

Result: 传统检测器在GPU上表现更优，而SuperPoint在FPGA上实现3.1倍运行性能提升和1.4倍能效提升。FPGA加速的V-SLAM在部分数据集上帧率更高，但GPU版本精度普遍更好。

Conclusion: 硬件加速特征检测可减少全局束调整调用频率而不损失精度，为不同应用场景下的硬件选择提供了指导。
...
TL;DR: XD-RCDepth是一种轻量级雷达-相机融合深度估计架构，参数减少29.7%但保持精度，通过两种知识蒸馏策略提升性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中深度估计至关重要，雷达-相机融合在恶劣条件下通过提供互补几何线索来增强鲁棒性。

Method: 提出两种知识蒸馏策略：解释性对齐蒸馏将教师的显著性结构转移到学生网络；深度分布蒸馏将深度回归重新表述为离散化区间的软分类。

Result: 相比直接训练，平均绝对误差(MAE)降低7.97%，在nuScenes和ZJU-4DRadarCam数据集上实现实时效率的竞争性精度。

Conclusion: XD-RCDepth在显著减少参数的同时保持了深度估计性能，通过知识蒸馏策略有效缓解了模型压缩带来的性能损失。
...
TL;DR: 提出了一个高多样性的无人机RGB-IR图像数据集ATR-UMOD，并开发了基于提示引导的条件感知动态融合方法PCDF，用于自适应多模态目标检测。


<details>
  <summary>Details</summary>
Motivation: 现有数据集难以充分捕捉真实世界复杂成像条件，限制了全天候无人机目标检测的性能。

Method: 提出PCDF方法，通过将成像条件编码为文本提示，利用任务特定的软门控变换自适应重新分配多模态贡献，并包含条件解耦模块确保无标注时的实用性。

Result: 在ATR-UMOD数据集上的实验验证了PCDF方法的有效性。

Conclusion: ATR-UMOD数据集和PCDF方法为复杂条件下的多模态目标检测提供了有效解决方案。
...
TL;DR: AVAR-Net是一个轻量级音频-视觉异常识别框架，通过融合音频和视频特征，结合多阶段时间卷积网络来建模跨模态关系，在真实环境中实现鲁棒的异常识别。
...
Method: 使用Wav2Vec2提取音频特征，MobileViT提取视觉特征，通过早期融合机制结合模态，采用多阶段时间卷积网络(MTCN)学习融合表示中的长期时间依赖关系。
...
TL;DR: 这篇系统综述使用PRISMA方法分析了39篇医学图像增强研究，发现低对比度和噪声是最常见问题，MRI和多模态成像研究最多，而组织病理学等专业模态研究不足。传统数学方法占主导，深度学习应用较少，图像质量评估主要使用无参考指标。


<details>
  <summary>Details</summary>
Motivation: 医学图像常存在噪声、伪影和低对比度等问题，限制了诊断潜力。需要强大的预处理、去噪算法和先进的增强方法来提高医学图像质量和可解释性，支持早期检测、准确诊断和有效治疗规划。

Method: 采用PRISMA方法的系统文献综述，分析了39篇同行评审研究，调查医学图像增强的关键挑战、最新进展和评估指标。

Result: 39项研究中，29项使用传统数学方法，9项关注深度学习技术，1项探索混合方法。图像质量评估方面，18项研究同时使用参考和无参考指标，9项仅使用参考指标，12项仅使用无参考指标，共引入65个IQA指标，主要为无参考类型。

Conclusion: 该综述突出了当前医学图像增强的局限性、研究空白和未来发展方向，指出需要更多关注深度学习方法和专业成像模态的研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于DINOv2的异常检测器在对抗攻击下的脆弱性和不确定性校准问题尚未得到充分研究，这限制了其在安全关键应用中的可信部署。

Method: 在AnomalyDINO基础上，通过附加轻量级线性头进行白盒梯度攻击，并使用Platt缩放对异常分数进行后处理以改善不确定性估计。

Result: 对抗攻击导致F1、AUROC、AP和G-mean指标显著下降，表明微小扰动可改变特征空间中的最近邻关系；校准后验在对抗样本上产生更高预测熵，有效检测攻击并降低校准误差。

Conclusion: 对抗鲁棒性和不确定性量化是异常检测系统可信部署的必备能力，研究揭示了DINOv2基检测器的具体漏洞并建立了评估协议和基准。
...
TL;DR: 提出了一种上下文精确的图像超分辨率框架，通过局部-全局上下文感知注意力和分布感知条件机制，在保持结构一致性的同时生成高质量图像。
...
Result: 在多个超分辨率基准测试中表现出色，能够生成高保真、感知准确的图像重建结果。

Conclusion: 该方法能够有效缓解伪影问题，确保真实细节恢复，在图像超分辨率任务中取得了显著效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有基础模型在图像编辑方面取得显著进展，但扩展到3D编辑仍面临挑战。直接替换现有工作流中的图像编辑模块计算成本高，且闭源API的限制和成本使得迭代编辑策略不实用。

Method: 提出EditCast3D管道，使用视频生成基础模型从单个第一帧传播编辑到整个数据集，引入视图选择策略识别一致且重建友好的视图，采用前馈重建避免昂贵的细化过程。

Result: 在常用3D编辑数据集上评估，与最先进的3D编辑基线相比，EditCast3D展现出卓越的编辑质量和高效性。

Conclusion: EditCast3D为将基础模型集成到3D编辑管道中提供了一个可扩展且通用的范例，代码已开源。
...
TL;DR: OmniGaze是一个半监督3D视线估计框架，通过利用大规模无标注数据和伪标签策略，解决了现有方法在跨域泛化方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前3D视线估计方法由于标注数据稀缺和多样性不足，难以在不同数据域间泛化。

Method: 构建多样化的无标注面部图像集，采用伪标签策略和奖励模型评估伪标签可靠性，结合视觉嵌入和语义线索计算置信度分数。

Result: 在五个数据集上实现了最先进的性能，在域内和跨域设置下均表现优异，并在四个未见数据集上展示了强大的零样本泛化能力。

Conclusion: OmniGaze不仅是一个有效的视线估计方法，还可作为可扩展的数据引擎，显著提升了3D视线估计的泛化性能。
...
TL;DR: NTIRE 2025低光图像增强挑战赛综述，展示参赛方案和最终结果，旨在寻找在多样挑战条件下能产生更亮、更清晰、视觉吸引力更强图像的有效网络。


<details>
  <summary>Details</summary>
Motivation: 识别在多样挑战条件下能够产生更亮、更清晰、视觉吸引力更强图像的有效网络，推动低光图像增强技术的发展。

Method: 通过竞赛形式，收集来自762名注册参与者和28个有效提交团队的解决方案，进行综合评估和分析。

Result: 展示了低光图像增强领域的最先进进展，证明了该技术的显著进步。

Conclusion: NTIRE 2025挑战赛成功推动了低光图像增强技术的发展，展示了该领域的显著进步和未来潜力。
...
<details>
  <summary>Details</summary>
Motivation: 开放域视觉实体识别面临固定标签集、训练期间未见实体、长尾分布等挑战，需要解决有限监督、视觉模糊性和语义消歧等问题。

Method: 使用知识引导的对比学习框架，将视觉和文本输入抽象到概念层面，利用实体描述、类型层次和关系上下文来支持零样本实体识别。

Result: 在OVEN基准测试中，最小模型在未见实体上的准确率比现有最优方法提高了10.5%，且模型规模小了35倍。
...
TL;DR: FlashWorld是一个快速生成3D场景的生成模型，比现有方法快10-100倍，同时保持更好的渲染质量。它通过双模式预训练和跨模式后训练，结合了多视图导向和3D导向方法的优势。


<details>
  <summary>Details</summary>
Motivation: 传统多视图导向方法生成多视图图像再重建3D，速度慢且质量受限。3D导向方法直接生成3D表示但视觉质量较差。需要结合两种方法的优势，实现快速且高质量的3D场景生成。

Method: 1. 双模式预训练：基于视频扩散模型先验，训练同时支持多视图导向和3D导向的多视图扩散模型；2. 跨模式后训练蒸馏：通过匹配3D导向模式到高质量多视图模式的分布，提升视觉质量并减少去噪步骤；3. 利用大规模单视图图像和文本提示增强泛化能力。

Result: 实验证明FlashWorld在生成速度上比先前工作快10-100倍，同时具有更优的渲染质量，能够有效处理分布外输入。

Conclusion: FlashWorld通过创新的双模式训练框架，成功实现了快速、高质量的3D场景生成，在速度和视觉质量上都超越了现有方法。
...
TL;DR: 提出了一种基于去噪扩散桥模型（DDBMs）的新方法，用于从病理图像生成健康反事实图像，该方法在保留个体解剖特征的同时选择性去除病理区域。
...
Conclusion: DDBM方法能够有效生成保留患者解剖特征的健康反事实图像，在医学影像分析中具有重要应用价值。
...
TL;DR: 提出了Risk-adaptive Activation Steering (RAS)方法，通过重新构建查询来增强对安全关键图像区域的跨模态注意力，实现准确的风险评估，并自适应地引导激活以生成安全且有用的响应。


<details>
  <summary>Details</summary>
Motivation: 现代AI模型在应对恶意多模态查询时存在安全漏洞，传统安全对齐方法成本高昂，而推理时对齐方法存在过度拒绝良性查询和推理速度慢的问题。

Method: 重新构建查询以增强对安全关键图像区域的跨模态注意力，进行风险评估，并根据风险自适应地引导激活生成响应。

Result: 在多个多模态安全和效用基准测试中，RAS显著降低了攻击成功率，保持了通用任务性能，并提高了推理速度。

Conclusion: RAS方法有效解决了多模态安全对齐的挑战，在保证安全性的同时提升了模型性能和推理效率。
...
TL;DR: 该研究开发了一种基于学习的骨架化与图连接算法，用于从脑部血管图像中提取Willis环中心线，并在TopCoW数据集上验证了其解剖准确性和特征鲁棒性。


<details>
  <summary>Details</summary>
Motivation: Willis环是脑部重要的动脉网络，传统骨架化技术难以可靠提取其中心线，且公开的中心线数据集稀缺，需要开发更准确的自动化分析方法。

Method: 结合U-Net骨架化和A*图连接算法，从MRA和CTA图像中提取中心线图，并开发基线算法进行中心线和特征提取。

Result: 算法在测试集上实现高精度图拓扑重建（F1=1），节点平均欧氏距离小于一个体素，半径、长度等特征相对误差中位数低于5%，皮尔逊相关系数高于0.95。

Conclusion: 基于学习的骨架化与图连接方法能有效提取解剖合理的中心线，强调了超越简单体素测量、评估解剖准确性和特征鲁棒性的重要性，数据集和算法已公开以支持进一步研究。
...
TL;DR: LiFMCR是一个新颖的多微透镜阵列光场相机配准数据集，包含两个高分辨率Raytrix R32光场相机的同步图像序列和Vicon运动捕捉系统记录的高精度6自由度位姿，为多相机光场配准方法提供了严格的评估基准。


<details>
  <summary>Details</summary>
Motivation: 现有光场数据集局限于单相机设置且通常缺乏外部真实值，无法支持多相机光场配准方法的严格评估。

Method: 提供了两种互补的配准方法：基于RANSAC的跨视图点云3D变换估计和从单光场图像估计外参6自由度位姿的光场PnP算法，两者都显式集成了光场相机模型。

Result: 实验显示与真实值有很强的对齐性，支持可靠的多视角光场处理。

Conclusion: LiFMCR数据集及其配准方法为多相机光场配准提供了可靠的基准和解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 低场MRI更便宜、更易获取且更安全，但分辨率低、信噪比差。合成高场MRI可减少对昂贵采集的依赖，但现有方法存在临床保真度差距，需要保持解剖保真度、增强细节并弥合图像对比度差异

Method: 提出循环自监督扩散框架，在循环一致性约束下重新制定基于扩散的合成。包含切片间隙感知网络通过对比学习对齐切片不一致性，以及局部结构校正网络通过掩码和扰动补丁的自重建增强局部特征恢复

Result: 在跨场合成任务中达到最先进性能（PSNR 31.80±2.70 dB，SSIM 0.943±0.102，LPIPS 0.0864±0.0689）。相比原始低场MRI，保留了精细解剖结构（左脑白质误差从12.1%降至2.1%，皮层从4.2%降至3.7%）

Conclusion: CSS-Diff能够合成在定量上可靠且在解剖学上一致的图像，解决了低场到高场MRI合成的临床保真度问题
...
TL;DR: UniCalli是一个统一的扩散框架，用于解决中国书法计算复制的挑战，通过联合训练识别和生成任务，在保持字符结构的同时提升页面级美学质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么只能生成高质量的单字但忽略页面级美学（如连笔和间距），要么在页面合成时牺牲书法正确性。需要一种统一的方法来同时解决这两个问题。

Method: 采用统一的扩散框架，联合训练识别和生成任务；使用非对称加噪和栅格化框图提供空间先验；在合成、标注和未标注数据的混合数据集上进行训练。

Result: 模型在生成质量上达到最先进水平，具有优越的连笔连续性和布局保真度，同时识别能力也更强；框架成功扩展到其他古文字，包括甲骨文和埃及象形文字。

Conclusion: UniCalli通过联合训练识别和生成任务，实现了书法复制的突破，在保持字符结构的同时提升了页面级美学质量，具有良好的泛化能力。
...
TL;DR: InteractiveOmni是一个开源的多模态大语言模型（4B-8B参数），集成了视觉、音频编码器和语音解码器，支持多轮音视频交互，在轻量级模型中表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发一个轻量级但功能全面的多模态模型，支持音视频理解和语音生成，提供智能的多轮交互体验。

Method: 采用多阶段训练策略：预训练实现多模态理解，后训练加入语音对话和音视频交互；精心构建多轮训练数据集；集成视觉编码器、音频编码器、大语言模型和语音解码器。

Result: 在多项基准测试中显著优于领先的开源模型，特别是在长期记忆能力方面；InteractiveOmni-4B在通用基准上与Qwen2.5-Omni-7B相当，仅用50%模型大小就能达到InteractiveOmni-8B 97%的性能。

Conclusion: InteractiveOmni为下一代智能交互系统提供了一个可访问、开源的基础，在图像、音频、视频理解和语音生成任务中达到了同类模型的最先进水平。
...
TL;DR: Uni-MMMU是一个评估统一多模态模型视觉理解和生成能力双向协同的基准测试，涵盖8个推理领域，通过耦合任务揭示两种能力如何相互增强。
...
Method: 构建包含科学、编程、数学、谜题等8个推理领域的综合性基准测试，每个任务都双向耦合，要求模型利用概念理解指导视觉合成，或利用生成作为分析推理的认知支架。

Result: 通过对最先进的统一模型、仅生成模型和仅理解模型的广泛评估，揭示了显著的性能差异和跨模态依赖性。

Conclusion: 该基准为推进统一模型提供了可靠基础，揭示了视觉理解和生成能力何时以及如何相互增强的新见解。
...
TL;DR: 提出了AVC框架，通过CLIP模型检索语义最相关的历史图像，并在没有足够相关图像时自适应限制先前视觉信息的影响，实现更好的故事延续图像生成。


<details>
  <summary>Details</summary>
Motivation: 解决故事延续中有效利用先前视觉上下文同时确保与当前文本输入语义对齐的挑战，避免误导或不相关信息的影响。

Method: 使用CLIP模型检索语义对齐的历史图像，在扩散过程的早期阶段自适应限制先前视觉信息的影响，并使用LLM重新标注噪声数据集以增强文本监督。

Result: 定量结果和人工评估显示，AVC在连贯性、语义一致性和视觉保真度方面优于强基线，特别是在先前视觉与当前输入冲突的挑战性情况下。

Conclusion: AVC框架能够有效利用视觉上下文，同时避免不相关信息的影响，在故事延续任务中实现了更好的性能。
...
Method: 利用扩散过程初始化种子的相关性，通过哈希函数集成到噪声采样过程中，确保从内容中恢复有效种子不可行，并使用密码学零知识证明来保护种子隐私

Result: 在多个最先进的图像和视频扩散模型上验证了NoisePrints，展示了仅使用种子和输出即可进行高效验证，无需模型权重

Conclusion: NoisePrints提供了一种实用且可扩展的水印方案，有效解决了扩散模型内容生成的版权保护问题
...
TL;DR: 提出了GS-Reasoner模型，通过双路径池化机制构建统一的3D表示，无需外部模块即可实现3D视觉定位和空间推理，并创建了GCoT数据集来连接这两个任务。
...
Result: GS-Reasoner在无需外部模块的情况下实现了与最先进模型相当的3D视觉定位性能，并显著提升了空间推理能力，达到最先进水平。

Conclusion: GS-Reasoner是首个无需外部模块即可实现自回归定位的3D LLM，为3D空间推理建立了统一且自包含的框架，证明了定位能力对空间推理的重要促进作用。
...
Method: 将视频表示为轨迹场，每个像素对应一个连续3D轨迹函数。Trace Anything模型通过单次前向传播为每个像素预测一组控制点，参数化B样条轨迹。

Result: 在轨迹场估计新基准上达到最先进性能，在现有点跟踪基准上表现有竞争力；具有显著效率优势；展现出目标条件操作、运动预测和时空融合等涌现能力。

Conclusion: Trajectory Field表示法和Trace Anything模型为视频动态建模提供了有效解决方案，实现了高效准确的轨迹预测，并展现出多种应用潜力。
...
TL;DR: VisCoP通过向视觉编码器添加可学习的视觉探针，实现大视觉语言模型的高效领域自适应，在跨视图、跨模态和跨任务场景中表现优异，同时保留源域知识。


<details>
  <summary>Details</summary>
Motivation: 现有领域自适应方法在微调不同VLM组件时，往往导致有限的领域特定特征学习或灾难性遗忘先前能力，需要更有效的适应策略。

Method: 提出视觉上下文探测(VisCoP)，通过向VLM视觉编码器添加少量可学习视觉探针，实现高效领域特定适应，最小化对预训练参数的修改。

Result: 在跨视图、跨模态和跨任务三个挑战性领域自适应设置中，VisCoP持续优于现有适应策略，在目标域获得优越性能并有效保留源域知识。

Conclusion: VisCoP提供了一种高效的大视觉语言模型领域自适应方法，能够在适应新领域的同时保持原有能力。
...
TL;DR: PhysMaster通过物理表征学习和强化学习优化，提升视频生成模型的物理合理性，使其能够生成符合物理定律的视频。
...
Method: 基于图像到视频任务，设计PhysEncoder从输入图像中提取物理信息作为额外条件。采用强化学习与人类反馈，通过直接偏好优化(DPO)端到端优化物理表征。

Result: PhysMaster在简单代理任务上证明了其能力，并具有广泛的物理场景泛化性，能够统一处理各种物理过程。

Conclusion: PhysMaster提供了一个通用的即插即用解决方案，可用于物理感知的视频生成和更广泛的应用。
...
TL;DR: 提出了Rule-Intent Distinction (RID)框架，一种低计算成本的元提示技术，用于在零样本设置下引导LLM进行人类对齐的异常处理，显著提升决策质量。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在作为智能体推理引擎时存在的"规则刚性"问题——对显式规则的僵化遵循导致决策与人类常识和意图不一致，这是构建可信自主智能体的重要障碍。

Method: RID框架通过结构化认知模式，让模型能够解构任务、分类规则、权衡冲突结果并证明最终决策，是一种零样本元提示技术。

Result: 在包含20个需要细致判断场景的基准测试中，RID框架达到95%的人类对齐分数，显著优于基线(80%)和思维链提示(75%)，并产生更高质量、意图驱动的推理。

Conclusion: RID框架提供了一种实用、易用且有效的方法，能够引导LLM从字面指令遵循转向灵活、目标导向的推理，为构建更可靠和实用的AI智能体铺平道路。
...
TL;DR: DeepPlanner是一个端到端的强化学习框架，通过基于熵的令牌级优势塑造和样本级优势加权，有效增强深度研究代理的规划能力。
...
Method: 提出DeepPlanner框架：1）使用基于熵的术语塑造令牌级优势，为高熵令牌分配更大更新；2）选择性加权规划密集型rollouts的样本级优势。

Result: 在七个深度研究基准测试中的广泛实验表明，DeepPlanner提高了规划质量，并在显著降低的训练预算下实现了最先进的结果。

Conclusion: DeepPlanner通过系统优化规划阶段，有效增强了语言模型的规划能力，在多个基准测试中取得了优异表现。
...
TL;DR: Sentinel是首个用于在语义、规划和轨迹三个层次上形式化评估基于LLM的具身智能体物理安全性的框架，通过时态逻辑精确规范安全要求，并采用多级验证管道进行系统化安全检测。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖启发式规则或主观的LLM判断，缺乏对具身智能体物理安全性的形式化评估框架，无法精确指定状态不变性、时间依赖性和时序约束等安全要求。

Method: 1) 语义层：将自然语言安全要求形式化为时态逻辑公式，并检测LLM智能体对这些要求的理解是否与时态逻辑公式一致；2) 规划层：验证LLM智能体生成的高层行动计划和子目标是否符合时态逻辑公式，在执行前检测不安全计划；3) 轨迹层：将多个执行轨迹合并为计算树，并针对物理细节的时态逻辑规范进行高效验证，进行最终安全检查。

Result: 在VirtualHome和ALFRED环境中评估多个基于LLM的具身智能体，实验表明Sentinel能够发现先前方法忽略的安全违规，并提供对失败模式的深入理解。

Conclusion: 通过将物理安全基于时态逻辑并在多个层次应用验证方法，Sentinel为在物理环境中系统化评估基于LLM的具身智能体提供了严谨基础，能够暴露被先前方法忽视的安全违规并提供失败模式洞察。
...
TL;DR: 提出一种基于微调大语言模型的框架，从交通事故文本叙述中自动推断驾驶员危险行为，提高分类的有效性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 两车事故占道路事故约70%，但人工编码驾驶员危险行为数据存在不一致性和劳动密集型问题，限制了大规模数据库的可靠性。

Method: 使用MTCF五年两车事故数据，在详细事故叙述上微调Llama 3.2 1B模型，并与随机森林、XGBoost、CatBoost和神经网络等传统机器学习分类器进行基准比较。

Result: 微调后的LLM总体准确率达到80%，优于所有基线模型，在数据不平衡场景中表现尤为突出。通过概率推理方法分析模型输出变化，发现分心驾驶显著增加"一般不安全驾驶"概率，双方分心最大化"双方驾驶员采取危险行为"概率，青少年驾驶员显著提高"速度和停车违规"概率。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界的时间序列分析需要超越表面趋势，揭示驱动因素。现有LLM方法大多忽视深层推理潜力，仅使用数值回归能力。

Method: 将时间序列分析重新定义为推理任务，利用LLMs整合多模态输入，优先考虑因果结构和可解释性。

Result: 该方法使时间序列分析更接近人类对齐的理解，在复杂现实环境中提供透明和上下文感知的洞察。

Conclusion: 需要重新思考LLMs在时间序列分析中的角色，将其作为推理工具而非简单回归模型，以实现更透明和可解释的分析。
...
TL;DR: 提出PBRR框架，通过从人类偏好中学习修正项来自动修复人工设计的奖励函数，解决奖励函数错配问题，比从头学习奖励函数更高效。


<details>
  <summary>Details</summary>
Motivation: 人工设计的奖励函数经常与真实目标错配，导致奖励黑客行为；而从头学习奖励函数成本高昂。

Method: PBRR框架：通过目标探索策略和新的偏好学习目标，学习一个加性的、状态转移相关的修正项来修复代理奖励函数。

Result: 在表格域中证明PBRR的累积遗憾与现有偏好学习RL方法相当；在奖励黑客基准测试中，PBRR始终优于基线方法，学习高性能策略所需偏好数量显著减少。

Conclusion: PBRR通过有效修正代理奖励函数，以更少的人类偏好实现了更好的策略性能，解决了奖励错配问题。
...
TL;DR: 本文提出了一个情感认知框架，通过整合欲望生成和目标管理，使LLM智能体能够模拟人类情感决策过程，显著提升了智能体在模拟社会互动中的生态效度和人类行为相似性。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的智能体在情感认知方面存在严重局限：无法模拟连接虚拟与现实服务所需的有限理性，缺乏经过实证验证的情感与决策架构整合机制。

Method: 构建包含欲望生成和目标管理的情感认知框架，建模LLM智能体的完整决策过程，包括状态演化、欲望生成、目标优化、决策生成和行动执行。

Result: 实验表明，采用该框架的智能体不仅表现出与其情感状态一致的行为，而且在比较评估中展现出更高的生态效度，决策结果显著更接近人类行为模式。

Conclusion: 所提出的情感认知框架成功实现了LLM智能体与人类之间的情感对齐，为构建更真实的社会模拟系统提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然链式思维提示和深度推理能提升复杂任务性能，但对所有问题应用深度推理计算成本过高，需要更高效的解决方案。

Method: 使用小型LLM生成初始答案，大型LLM进行验证，仅在答案错误时触发深度推理，避免不必要的计算开销。

Result: 实验结果显示，在简单问题上大型LLM的计算成本降低超过50%，准确率损失可忽略，复杂任务性能保持稳定。

Conclusion: 该互补代理系统在保持性能的同时显著降低了计算成本，为LLM应用提供了更高效的推理策略。
...
TL;DR: 提出了Jericho测试时学习基准(J-TTL)来衡量AI代理在测试时学习复杂技能的能力，并开发了EvoTest进化框架，通过进化代理系统配置来提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理无法在测试时学习复杂技能，在陌生环境中表现不佳，这严重限制了其实际应用价值。

Method: 提出了EvoTest进化测试时学习框架，包含执行游戏的Actor代理和分析游戏记录以提出改进配置的Evolver代理，通过改写提示、更新记忆、调整超参数和学习工具使用例程来改进系统。
...
TL;DR: 提出了一种基于效用的分析模型，用于自动驾驶车辆的环境感知系统，通过YOLOv8s目标检测模型和感知服务效用测量模块，评估不同优化器在nuScense数据集上的性能表现。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要准确感知道路上的多个物体并预测驾驶员感知以控制车辆运动，当前需要开发能够理解驾驶环境的感知系统模型。

Method: 使用自定义数据集（包含摩托车手、三轮车等独特物体），采用YOLOv8s进行目标检测，并开发了基于模型性能值的感知服务效用测量模块。
...
TL;DR: 提出了SAJA框架，一种在状态和动作层面联合攻击多智能体深度强化学习模型的方法，相比单独攻击更有效且隐蔽。


<details>
  <summary>Details</summary>
Motivation: 现有研究只关注状态或动作的单方面攻击，没有充分利用两者协同效应，简单组合效果不佳。

Method: SAJA框架包含两个阶段：状态攻击阶段使用多步梯度上升方法结合actor和critic网络计算对抗状态；动作攻击阶段基于扰动状态使用critic网络生成最终对抗动作，并添加正则化项增强效果。

Result: 在MPE环境中测试表明，SAJA比单独状态或动作攻击更有效且隐蔽，现有防御方法无法抵御其攻击。

Conclusion: 状态和动作的联合攻击比单独攻击更具威胁，现有防御方法需要改进以应对此类协同攻击。
...
<details>
  <summary>Details</summary>
Motivation: 传统合理化方法存在模式崩溃问题，即预测器能正确预测但生成器持续输出崩溃模式，现有研究缺乏统一考虑。

Method: 从博弈论视角分析合作合理化，提出PORAT方法，通过渐进式策略干预解决合作博弈过程中的均衡问题。

Result: 在9个真实数据集和2个合成设置上验证，PORAT相比现有最优方法性能提升达8.1%。

Conclusion: PORAT能有效解决合理化中的模式崩溃问题，引导模型达到更优的博弈均衡状态。
...
Result: LLMs在生成的因果步骤数量和粒度上存在差异，虽然它们对生成的因果链具有自一致性和信心，但主要基于关联模式匹配而非真正的因果推理。人类评估确认了生成链的逻辑连贯性和完整性。

Conclusion: 该研究为推进论证情境中隐式机制性因果推理的未来工作奠定了坚实基础，包括基线因果链发现方法、诊断评估见解和带有因果链的基准数据集。
...
Method: 基于众包QoE数据，在单个天线级别进行覆盖分析后聚合到站点级别，应用OC-SVM算法将决策超平面建模为有效覆盖边界。

Result: 该框架能准确映射移动网络覆盖，特别是在复杂城市环境中突出显示信号不足的细粒度区域。

Conclusion: 所提出的框架在移动网络覆盖映射和弱信号点识别方面具有显著效果，为网络优化提供了有效工具。
...
TL;DR: 提出了Confidence-as-a-Reward (CRew)方法，利用模型对最终答案的token级置信度作为奖励代理，无需训练即可在数学推理任务中超越现有无训练奖励方法，甚至优于大多数训练过的奖励模型。
...
Method: 系统研究CRew方法，使用模型对最终答案的token级置信度作为奖励代理，特别适用于封闭式任务。基于此提出CRew-DPO训练策略，结合置信度分数和正确性信号构建偏好数据。

Result: 在MATH500和RewardMATH基准测试中，CRew优于现有无训练奖励方法，甚至超过大多数训练过的奖励模型。CRew分数与模型实际推理性能强相关，并能有效过滤高质量训练数据。CRew-DPO进一步提升了模型判断能力，持续优于现有自训练方法。

Conclusion: 置信度作为奖励是一个简单而强大的无训练方法，在数学推理任务中表现优异，且能有效支持数据过滤和模型训练，为提升LLM推理能力提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI在金融服务行业应用时，历史机器学习指标往往无法很好地泛化到GenAI工作负载，且广泛使用的基准测试难以适用于工业场景，需要解决这些性能评估障碍。

Method: 开发了一个风险评估框架，更好地结合领域专家评估和机器学习指标，考虑选择特定指标时的各种独特风险。

Result: 提出了一个能够改进领域专家和机器学习指标应用的框架，解决了GenAI在金融服务中的性能评估问题。

Conclusion: 该风险评估框架有助于克服生成式AI在金融服务行业应用中的性能测量障碍，促进更好的模型评估实践。
...
<details>
  <summary>Details</summary>
Motivation: 随着语言模型快速发展，其推理过程可能变得难以理解，影响可解释性和监督。需要确保强模型的解决方案对弱代理和人类保持可理解性

Method: 引入串联训练：在强化学习中随机从冻结的弱模型采样token，而不是仅从强模型采样。只有当强模型的行动和推理过程能被弱模型继续时，才能成功完成rollout

Result: 在GSM8K数学推理任务中，串联训练可靠地教会模型放弃专业术语，适应较弱合作伙伴的语言，同时保持高任务准确率

Conclusion: 该方法为构建可由弱代理审计的AI系统提供了有前景的途径，对人与AI协作和多智能体通信具有重要意义
...
<details>
  <summary>Details</summary>
Motivation: 基于逻辑的模型可用于构建机器学习分类器的验证工具，这些分类器在法律领域通过案例推理预测案件结果
...
TL;DR: 提出了一种基于人类赋权最大化的辅助语言模型调优方法Empower，该方法仅需离线文本数据，无需额外人工反馈，能显著提高人类在编程任务中的成功率。


<details>
  <summary>Details</summary>
Motivation: 现有辅助代理方法往往鼓励代理独立完成任务而非真正协助人类，且需要昂贵的人工反馈。需要一种更有效的自我监督方法来训练真正协助人类的AI代理。

Method: 提出Empower方法，通过最大化人类在环境中的赋权（影响期望变化的能力）来调优语言模型，仅使用离线文本数据进行自监督训练。

Result: 用户研究中78%的参与者偏好Empower助手（p=0.015），接受率提高31%，建议减少38%。在编程环境中，Empower使模拟程序员在挑战性编码问题上的成功率平均提高192%。
...
TL;DR: 本文提出了一种基于控制理论的安全护栏方法，用于预防AI代理在交互过程中可能造成的下游危害，如财务或物理伤害，通过实时监控和主动修正风险输出来替代传统的标记-阻断式护栏。


<details>
  <summary>Details</summary>
Motivation: 当前AI安全护栏主要依赖基于标记数据集和人工标准的输出分类，这种方法对新的危险情况脆弱，且检测到不安全条件后通常只是拒绝行动，这并非总是安全选择。

Method: 将AI代理安全形式化为序列决策问题，借鉴安全关键控制理论，在AI模型的世界潜在表示中构建预测性护栏，实时监控AI系统输出并主动修正风险输出为安全输出，采用模型无关方式，通过安全关键强化学习进行大规模训练。

Result: 在模拟驾驶和电子商务场景中的实验表明，控制理论护栏能可靠地引导LLM代理避免灾难性后果（从碰撞到破产），同时保持任务性能。

Conclusion: 控制理论护栏为当前的标记-阻断式护栏提供了一种原则性的动态替代方案，能够更有效地处理AI代理在现实世界交互中的安全风险。
...
<details>
  <summary>Details</summary>
Motivation: 在LLM推理系统需要为每个证明步骤提供充分支持的挑战性开放环境中，需要强大的验证器来捕捉步骤级错误，但目前缺乏合适的评估基准。
...
Conclusion: Hard2Verify为评估前沿步骤级验证器提供了重要基准，揭示了当前验证器性能的差距，并为改进验证能力提供了分析基础。
...
<details>
  <summary>Details</summary>
Motivation: 脉冲神经网络在神经形态硬件上具有能效优势，但在需要快速适应和长期记忆的持续学习任务中表现不佳，特别是面对稳定性-可塑性困境。

Method: LT-Gate模型让每个脉冲神经元并行跟踪快慢两个时间尺度的信息，通过学习的门控机制局部调整其影响，并引入方差跟踪正则化来稳定放电活动。

Result: 在挑战性时序分类基准测试中达到约51%的最终准确率，优于Hebbian持续学习基线的约46%和先前SNN方法，且完全兼容神经形态硬件。

Conclusion: 多时间尺度门控能显著增强SNN的持续学习能力，缩小脉冲网络与传统深度网络在终身学习任务上的差距。
...
<details>
  <summary>Details</summary>
Motivation: 伪对齐是许多时间序列大语言模型面临的普遍挑战，导致其性能不如线性模型或随机初始化的骨干网络，但社区对此问题的原因讨论有限。

Method: 通过深入研究伪对齐的根源，将其与LLM中的锥效应联系起来，并提出了TimeSUP技术，通过增加时间序列流形使其更接近语言嵌入的固有维度，从而缓解伪对齐问题。

Result: TimeSUP在长期预测性能上持续优于最先进的LLM4TS方法和其他轻量级基线，并且可以无缝集成到四个现有的LLM4TS管道中，显著提升预测性能。

Conclusion: 伪对齐源于预训练LLM组件中的锥效应与时间序列数据固有低维流形的相互作用，TimeSUP通过调整时间序列流形维度有效解决了这一问题，在保持各模态独特特征的同时学习其共性。
...
<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中的类增量学习挑战，包括统计异质性、任务序列长度扩展性、隐私保护以及减少遗忘问题。

Method: 使用Cardinality-Agnostic Task Encoder生成高斯分布的任务嵌入，通过2-Wasserstein距离测量任务间差距并强制任务分离，避免直接传输潜在嵌入以保护隐私。
...
TL;DR: 该论文展示了在振荡器伊辛机(OIMs)上实现平衡传播(EP)的方法，将机器学习优化与物理系统的能量下降过程统一起来，在MNIST和Fashion-MNIST数据集上取得了竞争性准确率，同时保持了在硬件约束下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 利用物理系统自然执行能量下降的特性来加速机器学习，特别是将振荡器伊辛机的GHz频率动态与能量基模型的优化和梯度下降联系起来，同时内在噪声对应朗之万动力学，支持采样和优化。

Method: 在振荡器伊辛机上实现平衡传播(EP)，将优化过程统一到单一总能量景观上的下降，实现无需全局反向传播的局部学习规则。

Result: 在MNIST数据集上达到约97.2±0.1%的准确率，在Fashion-MNIST上达到约88.0±0.1%的准确率，在参数量化和相位噪声等实际硬件约束下保持鲁棒性。

Conclusion: 振荡器伊辛机是神经形态学习的快速、节能基底，能量基模型可能在直接执行其优化的物理硬件上找到实际实现。
...
TL;DR: 该论文开发了首个在状态对抗MDP中修剪认证鲁棒性的理论框架，证明了元素级修剪只能收紧鲁棒性界限，并通过三阶段遗憾分解揭示了性能-鲁棒性权衡边界。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的强化学习策略需要在对抗扰动下保持可靠性，而过参数化的深度RL代理增加了成本和脆弱性。虽然修剪在监督学习中已被证明能提高鲁棒性，但其在对抗RL中的作用尚不清楚。

Method: 为高斯和分类策略开发理论框架，证明元素级修剪能收紧认证鲁棒性界限。提出三阶段遗憾分解方法，分离干净任务性能、修剪引起的性能损失和鲁棒性增益。在连续控制基准上评估幅度和微修剪调度。

Result: 在所有任务中，修剪在中等稀疏度水平下一致地发现可复现的"甜蜜点"，鲁棒性显著改善而不损害（有时甚至增强）干净性能。

Conclusion: 修剪不仅是压缩工具，更是实现鲁棒RL的结构性干预手段。
...
TL;DR: 提出了一个评估电子健康记录基础模型隐私记忆风险的框架，包含嵌入层和生成层的记忆探测方法，旨在区分模型泛化和有害记忆。


<details>
  <summary>Details</summary>
Motivation: 基础模型在大型去标识化电子健康记录上的训练具有临床应用前景，但其记忆患者信息的能力引发了重要的隐私担忧。

Method: 引入一套黑盒评估测试，包括在嵌入层和生成层探测记忆的方法，并开发开源工具包以促进医疗AI中的隐私评估。

Result: 在公开可用的电子健康记录基础模型上验证了该方法，并发布了开源工具包。

Conclusion: 该框架有助于评估和减轻电子健康记录基础模型中的隐私记忆风险，特别是在易受伤害的亚组中保护患者隐私。
...
Method: 统一了注意力增强特征融合、基于Grad-CAM++的局部解释和Reveal-to-Revise反馈循环，用于偏见检测和缓解。

Result: 在MNIST多模态扩展数据集上达到93.2%分类准确率、91.6% F1分数和78.1%解释保真度（IoU-XAI），优于单模态和非可解释基线方法。

Conclusion: 这项工作在性能、透明度和公平性之间架起了桥梁，为敏感领域中的可信AI提供了一条实用路径。
...
TL;DR: 提出了一种新的拒绝推断框架CI-EX，通过离群点检测识别被拒绝客户的分布，并为最接近接受人群分布的拒绝个体分配标签，在保持AUC竞争力的同时显著提升了拒绝推断特定指标的性能。


<details>
  <summary>Details</summary>
Motivation: 传统拒绝推断方法假设被拒绝客户的行为可以从接受客户中推断，但忽略了两个人群之间可能存在的分布差异，导致盲目推断问题。

Method: 提出置信内点外推框架(CI-EX)，迭代使用离群点检测模型识别被拒绝客户样本分布，基于监督分类模型的概率为最接近接受人群分布的拒绝个体分配标签。

Result: 在两个大型真实信用数据集上的实验表明，CI-EX在拒绝推断特定指标（如Kickout和Area under the Kickout）上始终优于现有方法，同时在大多数实验中保持AUC的竞争力。
...
Method: 通过理论证明LID是去噪分数匹配损失的下界，从而将其作为LID估计器，并与隐式分数匹配损失和FLIPD方法建立联系。
...
<details>
  <summary>Details</summary>
Motivation: 当前生成模型的遗忘指标基于参考响应或分类器输出进行评估，而非核心目标：未学习模型是否与从未见过不需要数据的模型行为无法区分。这种特定参考方法存在系统性盲点。

Method: 提出功能性对齐分布等价(FADE)指标，通过比较未学习模型和参考模型在生成样本上的双向似然分配来测量分布相似性，捕获整个输出分布的功能性对齐。

Result: 在TOFU和UnlearnCanvas基准测试中，传统指标接近最优的方法未能实现分布等价，许多模型在遗忘后反而离黄金标准更远。
...
TL;DR: CSI-4CAST是一个混合深度学习架构，通过整合CNN残差、自适应校正层、ShuffleNet块和Transformer，有效捕获CSI预测中的局部和长程依赖关系，在计算效率和预测精度上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在应对实际非高斯噪声、跨不同信道条件的泛化能力和计算效率方面存在局限，需要开发更鲁棒和高效的CSI预测方法。

Method: 提出混合深度学习架构CSI-4CAST，整合4个关键组件：CNN残差、自适应校正层、ShuffleNet块和Transformer，以捕获CSI预测中的局部和长程依赖关系。

Result: CSI-4CAST在88.9%的TDD场景和43.8%的FDD场景中优于基线模型，计算量比最强基线LLM4CP减少5倍和3倍，同时建立了包含30万样本的CSI-RRG基准数据集。

Conclusion: CSI-4CAST在CSI预测精度和计算效率方面均取得显著提升，建立的CSI-RRG基准为未来研究提供了标准化评估平台，推动了鲁棒高效CSI预测方法的发展。
...
TL;DR: 该论文提出了ExtremBench基准数据集，用于评估LLMs在数学极值问题求解方面的推理能力，发现现有数学基准可能无法全面评估数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 理解LLMs推理能力的来源和机制，特别是数学极值求解这一关键抽象能力，该能力在规划、控制和资源分配等应用中至关重要。

Method: 构建ExtremBench基准数据集，包含93个标准化极值求解问题，源自中国数学奥林匹克不等式练习，并对多个先进开源模型家族进行广泛评估。

Result: 发现LLMs的极值求解推理能力与当前数学基准（如AIME25和MATH-500）的评估结果不一致，有些模型在通用数学推理方面表现良好但在极值求解方面较差，反之亦然。
...
TL;DR: 提出了AMORE框架，一种自适应多输出算子网络，用于解决刚性系统时间积分中的计算成本问题，通过自适应损失函数和约束满足技术来预测热化学状态。


<details>
  <summary>Details</summary>
Motivation: 刚性系统的时间积分是燃烧、高超声速等反应输运系统的主要计算成本来源，需要开发能够缓解刚度挑战的策略。

Method: 开发AMORE框架，包含多输出预测算子和自适应损失函数；提出可逆分析映射来精确满足质量分数约束；采用两步训练方法；在(n-1)维空间进行DeepONet训练。

Result: 在合成气（12状态）和GRI-Mech 3.0（24个活跃状态）两个示例中验证了模型的有效性和适用性。

Conclusion: AMORE是一个通用框架，提出的DeepONet将成为未来CFD研究中加速湍流燃烧模拟的骨干技术。
...
TL;DR: 提出了一种用于单细胞扰动建模的多阶段强化学习算法，通过自然梯度更新和PPO优化来避免Waddington景观中的局部最优问题，显著提升了单细胞测序数据的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动框架在细胞命运决策的Waddington景观中容易陷入局部最优，导致轨迹陷入虚假谱系或不合理的分化结果，需要设计良好的初始化方法来逃离局部最优并收敛到正确的谱系。

Method: 采用多阶段强化学习算法：第一阶段使用Fisher向量积和共轭梯度求解器计算显式自然梯度更新，通过KL信任域约束提供安全的曲率感知初始策略；第二阶段应用带裁剪代理的近端策略优化(PPO)，利用小批量效率优化策略。

Result: 该方法在单细胞RNA测序(scRNA-seq)和单细胞ATAC测序(scATAC-seq)扰动分析中显著提高了泛化性能。

Conclusion: 提出的多阶段强化学习算法通过精心设计的初始化策略，能够有效避免局部最优问题，在单细胞扰动建模中表现出优越的泛化能力。
...
TL;DR: 提出一个端到端的机器学习工作流，用于在工业环境下进行自动化超声波焊缝检测，解决数据稀缺和信号干扰问题。


<details>
  <summary>Details</summary>
Motivation: 自动化超声波焊缝检测面临训练数据有限和环境干扰导致信号失真的挑战，现有方法难以在工业环境下实现端到端检测。

Method: 使用基于兰姆波理论的降阶亥姆霍兹模型生成数据集，结合扩散模型进行分布对齐，采用U-Net进行分割和反演，通过迁移学习优化模型。

Result: 该集成框架能够处理现实世界中的分布外测量数据，为真实数据上的自动化焊缝检测提供端到端解决方案。

Conclusion: 提出的工作流成功解决了焊缝检测中的数据稀缺和信号干扰问题，实现了工业环境下的自动化检测。
...
TL;DR: 本文通过信息论视角重新审视Koopman学习，提出了一种平衡简洁性和表达性的信息论拉格朗日公式，解决了深度架构中有限维子空间识别的挑战。


<details>
  <summary>Details</summary>
Motivation: Koopman算子的无限维特性使得识别合适的有限维子空间具有挑战性，特别是在深度架构中。作者认为这些困难源于次优的表示学习，其中潜在变量未能平衡表达性和简洁性。

Method: 提出基于信息论的拉格朗日公式，利用潜在互信息促进简洁性，同时使用冯·诺依曼熵维持表达性以防止模式崩溃。开发了新的算法来平衡这一权衡。

Result: 在多种动力系统上验证了该方法，相比现有Koopman学习方法表现出改进的性能，并可视化学习到的流形，观察结果与理论预测一致。

Conclusion: 通过信息论框架平衡简洁性和表达性，可以获得稳定且可解释的Koopman表示，在多种动力系统上优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率操作模型存在持续偏差，特别是在模拟极端事件和统计分布方面；而理想化模型能精确校准特定动力学特征，但不同模型之间存在学科界限。需要结合两者的优势来改进地球系统模拟。

Method: 使用重新配置的潜在数据同化技术，构建桥接模型层次结构的可解释AI框架，利用理想化模型的稀疏输出来增强操作模型。

Result: 桥接模型继承了操作模型的高分辨率和全面变量，同时通过理想化模型的针对性改进实现了全局精度提升，显著纠正了CMIP6中厄尔尼诺时空模式的模拟偏差。

Conclusion: 该框架超越了黑盒校正，提供了物理洞察力，支持有效的物理辅助数字孪生和不确定性量化，同时强调了推动理想化模型发展和建模社区间交流的重要性。
...
TL;DR: 该论文基于SGD提出了IAGD和NRSGD两种优化器，IAGD使用牛顿插值法加速收敛，NRSGD通过噪声正则化防止过拟合，在CIFAR数据集上的实验证明了这两种方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 改进传统SGD优化器的收敛速度和过拟合问题，通过插值加速和噪声正则化技术来提升深度学习模型的训练效果。

Method: IAGD利用二阶牛顿插值法加速梯度下降过程，假设迭代间梯度相关性；NRSGD在优化过程中向梯度添加受控噪声来实现正则化。

Result: 在CIFAR-10和CIFAR-100数据集上的对比实验表明，IAGD和NRSGD相比Keras包中的经典优化器表现更好，证明了这两种改进方法的有效性。

Conclusion: IAGD和NRSGD是SGD的可行改进方法，能够有效加速收敛并防止过拟合，在深度学习优化中具有应用潜力。
...
Method: 利用GPM CORRA数据集、地球静止卫星数据和全球NWP数据，以约5公里空间分辨率和15分钟时间分辨率预测未来12小时降水。

Result: 显著优于行业标准小时预报，在数据稀疏地区表现优于美国的高分辨率NWP模型，关键指标如CSI和FSS均有显著提升。

Conclusion: 该模型能在1分钟内生成预报，已部署到Google搜索，是减少全球预报质量差异和整合稀疏高分辨率卫星观测的重要进展。
...
Method: 引入基于权重的结构化公式，明确捕捉流数据的时变目标函数特性。分析均匀权重和折扣权重两种策略，在梯度下降更新下推导跟踪误差的紧界。

Result: 均匀权重下跟踪误差以𝒪(1/t)速率渐近消失，折扣权重下存在由折扣因子和每步梯度更新次数控制的非零误差下限。

Conclusion: 理论分析揭示了不同权重策略对跟踪误差的影响，数值模拟验证了理论发现，为流数据学习中的时变优化提供了理论指导。
...
TL;DR: 该论文研究了KL正则化在博弈论环境中的理论优势，提出了OMG和SOMG算法，在矩阵博弈和马尔可夫博弈中实现了对数级后悔，且后悔与KL正则化强度β成反比。
...
Method: 提出了OMG算法用于矩阵博弈（基于最佳响应采样和乐观奖励），以及SOMG算法用于马尔可夫博弈（使用最佳响应采样和超乐观奖励概念）。

Result: 两种算法都实现了对数级后悔，后悔与KL正则化强度β成反比，同时保持了标准的√T后悔界。

Conclusion: KL正则化在博弈论环境中确实能提供理论上的样本效率优势，所提出的算法在正则化和非正则化设置下都能达到最优后悔界。
...
TL;DR: 提出新的绝对聚类指数来评估聚类的紧凑性和分离性，用于确定真实聚类数量


<details>
  <summary>Details</summary>
Motivation: 现有聚类有效性指标通常是相对的，依赖于数据结构和算法比较，难以确定真正的聚类

Method: 定义每个聚类的紧凑性函数和聚类对之间的邻点集，计算紧凑性和分离性指数

Result: 在合成和真实数据集上验证了新指数的性能，与其他常用聚类有效性指标进行了比较

Conclusion: 新提出的绝对聚类指数能有效评估聚类质量并确定最优聚类数量
...
TL;DR: NeuroRVQ是一种基于码本的脑电信号大模型，通过多尺度特征提取、分层残差向量量化和相位振幅感知损失函数，解决了现有脑电信号分词器无法保留高频动态的问题，实现了高保真重构和更好的下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有脑电信号分词器无法保留高频动态，限制了信号重构的保真度，阻碍了脑电基础模型的性能提升。

Method: 提出NeuroRVQ模型，包含：多尺度特征提取模块捕获全频段神经频谱；分层残差向量量化码本实现高分辨率编码；相位振幅感知损失函数进行高效训练。

Result: NeuroRVQ实现了更低的信号重构误差，在各种下游任务中优于现有脑电大模型。

Conclusion: NeuroRVQ分词器为基于码本的通用脑电模型建立了强大先验，推动了神经解码、生成建模和多模态生物信号集成的发展。
...
TL;DR: 提出了一种基于Transformer的无监督深度学习框架，用于大规模MU-MISO系统的下行链路波束成形，通过离线训练实现实时推理，在动态通信环境中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决大规模多用户MISO系统中波束成形的计算复杂性和实时性挑战，传统优化方法如WMMSE计算量大，难以适应动态环境。
...
Method: 使用门控循环单元(GRU)自动学习时间模式(如广告存量效应和滞后)，通过有向无环图学习渠道间统计依赖和因果结构，并基于Hill方程实现饱和曲线建模。

Result: 实现了数据驱动设计、多区域建模、鲁棒统计方法、全面响应曲线分析和14+交互式可视化仪表板等关键创新。

Conclusion: DeepCausalMMM通过结合深度学习和因果推断，显著提升了营销组合建模的能力，能更好地捕捉复杂营销动态并优化预算分配。
...
TL;DR: 提出稀疏三角传输映射方法，利用周期性边界条件下晶格图的条件独立性结构，通过单调整流神经网络实现，在保持表达性的同时实现线性时间复杂度。
...
Method: 使用稀疏三角传输映射，利用晶格图的条件独立性结构，通过单调整流神经网络实现，支持站点并行评估，时间复杂度与晶格大小N成线性关系。

Result: 在二维φ^4模型上验证了方法的有效性，分析了节点标记对稀疏性和性能的影响，并与混合蒙特卡洛和RealNVP等方法进行了比较。

Conclusion: 提出的稀疏三角传输映射框架在保持表达性的同时实现了计算效率，为大规模晶格场理论采样提供了可行方案。
...
TL;DR: 本文证明了在有限精度对数宽度设置下，掩码扩散模型与多项式填充循环变换器等价，能够解决所有CoT增强变换器可解的问题，并在某些问题类别上比CoT变换器更高效。


<details>
  <summary>Details</summary>
Motivation: 探索掩码扩散模型的计算能力和并行性限制，将其与现有的推理框架（链式思维和填充循环变换器）进行理论对比。

Method: 在有限精度对数宽度设置下，建立掩码扩散模型与多项式填充循环变换器的等价关系，并与CoT增强变换器进行能力对比分析。

Result: 证明了掩码扩散模型与多项式填充循环变换器在计算能力上等价，能够解决所有CoT变换器可解的问题，并在某些问题类别（如正则语言）上具有更高的效率。

Conclusion: 掩码扩散模型不仅具备与现有推理框架相当的计算能力，其并行生成特性还使其在某些推理任务中比序列生成的CoT方法更高效。
...
TL;DR: CoDa-FL：面向集群和依赖感知的联邦学习框架，通过基于EMD的客户端聚类和依赖任务分配，减少多任务联邦学习在移动边缘计算环境中的总完成时间。


<details>
  <summary>Details</summary>
Motivation: 解决移动边缘计算环境中联邦学习在多任务依赖设置下的客户端选择问题，旨在减少各种学习任务的总完成时间。
...
Conclusion: CoDa-FL通过推导簇内EMD与训练轮数之间的直接关系，简化了最优解的获取过程，有效提升了联邦学习在移动边缘计算环境中的性能。
...
TL;DR: 本文通过随机批次方法研究连续时间模型中的dropout正则化，构建了无偏估计器，建立了轨迹级和分布级的收敛性分析，并在训练过程中分析了最优控制和梯度下降的偏差。
...
Method: 使用随机批次方法构建无偏估计器，在时间区间上对神经元进行批次采样，建立轨迹收敛性和分布稳定性分析，并进行伴随分析和梯度下降迭代偏差分析。

Result: 在轨迹层面建立了线性收敛率，在分布层面获得了总变差误差为h^{1/2}的稳定性，在单层神经ODE上验证了理论预测的收敛率和正则化效果。

Conclusion: 该方法提供了连续时间dropout的理论基础，展示了良好的计算效率和正则化性能，为深度连续时间模型的正则化设计提供了理论指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有程序表示方法存在两难：静态表示高效但无法反映程序在代码变换下的行为，动态表示能洞察性能瓶颈但开销大且不确定。需要超越这种权衡。

Method: 通过用多样化优化序列探测程序的中间表示，量化静态特征变化来生成程序行为谱。使用产品量化将连续反应向量离散化，然后用多任务Transformer模型PQ-BERT学习行为代码的上下文语法。

Result: 在两个编译器优化任务上的实验表明，该方法优于最先进的静态基线方法。

Conclusion: 提出的准动态框架成功超越了静态和动态表示之间的权衡，为程序表示提供了新思路。
...
TL;DR: 提出了一种高效实用的构建完全等变图神经网络的理论框架，通过规范形式和全秩可转向基集实现完全性，显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有等变GNN通过更深架构、增加体阶或可转向特征维度来实现完全性，但计算成本高且缺乏多项式时间解决方案。

Method: 基于理论证明，使用几何图的规范形式（完全标量函数）和全秩可转向基集构建完全等变GNN，并在EGNN和TFN模型上实现高效算法。

Result: 实验结果表明，该模型仅需几层就能实现优越的完全性和优秀性能，显著减少计算开销同时保持强大实用效果。

Conclusion: 提出的理论框架能够高效构建完全等变GNN，在保持实用性的同时大幅降低计算复杂度。
...
TL;DR: 提出跨模态互补性假设(CCH)：当教师和学生表征间的互信息超过学生表征与标签间的互信息时，跨模态知识蒸馏有效。通过理论验证和实验证实，为跨模态KD提供了理论框架和实践指导。


<details>
  <summary>Details</summary>
Motivation: 跨模态知识蒸馏技术虽然应用广泛，但缺乏理论理解导致效果不稳定。需要建立理论框架来解释和指导实践。

Method: 提出跨模态互补性假设(CCH)，在联合高斯模型中进行理论验证，并在图像、文本、视频、音频和癌症组学等多模态数据集上进行实证检验。

Result: 理论分析和实验结果表明，当教师-学生表征互信息大于学生-标签互信息时，跨模态KD能有效提升性能。

Conclusion: 建立了跨模态知识蒸馏的理论框架，提供了基于CCH准则选择最优教师模态的实用指南，以改善较弱模态的性能。
...
TL;DR: CleverCatch是一个知识引导的弱监督模型，通过将结构化领域专业知识集成到神经网络架构中，在共享嵌入空间中对齐规则和数据样本，从而改进医疗欺诈检测的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 医疗欺诈检测面临标记数据有限、欺诈手段不断演变和医疗记录高维度等挑战。传统监督方法受限于极端标签稀缺，而纯无监督方法往往无法捕捉临床意义上的异常。

Method: 该方法在神经网络架构中集成结构化领域专业知识，在共享嵌入空间中对齐规则和数据样本。通过在代表合规和违规的合成数据上联合训练编码器，学习能够泛化到复杂真实数据集的软规则嵌入。

Result: 在大型真实数据集上的实验表明，CleverCatch优于四种最先进的异常检测基线方法，AUC平均提高1.3%，召回率平均提高3.4%。消融研究进一步验证了专家规则的补充作用。
...
TL;DR: 比较Ising和QUBO编码在玻尔兹曼机学习中的差异，发现QUBO编码会导致Fisher信息矩阵更病态，使SGD收敛更慢，而自然梯度下降能解决此问题。


<details>
  <summary>Details</summary>
Motivation: 研究不同变量编码（Ising的{-1,+1}和QUBO的{0,1}）如何影响玻尔兹曼机学习的信息几何和收敛性能。

Method: 在固定模型、采样器和步长的控制协议下，通过Fisher信息矩阵分析两种编码的统计特性差异，比较SGD和NGD的收敛表现。

Result: QUBO编码产生更大的交叉项，导致FIM有更多小特征值方向，谱熵更低，使SGD收敛更慢；NGD因重参数化不变性而表现相似。

Conclusion: 对于SGD训练，Ising编码提供更各向同性的曲率和更快收敛；QUBO编码可通过中心化/缩放或NGD预处理来缓解曲率问题。
...
Method: 使用新提出的截断影响函数(TIF)评估数据质量，开发两种计算更简单的评分函数，并将它们组合以抵消不同的误差源。
...
<details>
  <summary>Details</summary>
Motivation: 传统GNN在领域自适应中表现不佳，主要原因是未能区分处理全局和局部模式，导致多层GNN后局部细节被破坏。通过频谱分析发现低频组件编码领域不变的全局模式，高频组件捕获领域特定的局部细节。

Method: 提出FracNet，包含两个协同模块：将原始图分解为高频和低频组件，执行频率感知的领域自适应，并结合对比学习框架改善领域自适应的模糊边界问题。

Result: 大量实验表明，FracNet相比最先进方法有显著改进。

Conclusion: FracNet通过频谱分析和对比学习的结合，有效解决了图神经网络在领域自适应中的结构分布偏移问题，并提供了理论证明支持其优越性。
...
TL;DR: 应用超网络+适配器架构到视角感知分类，在减少参数的同时保持与专用模型相当的仇恨言论和毒性检测性能
...
Method: 采用超网络+适配器的现有架构，应用于视角感知分类任务

Result: 在仇恨言论和毒性检测任务上能与专用模型竞争，同时使用显著更少的参数

Conclusion: 该解决方案是架构无关的，可应用于广泛的基模型
...
### [159] [BlendFL: Blended Federated Learning for Handling Multimodal Data Heterogeneity](https://arxiv.org/abs/2510.13266)
*Alejandro Guerra-Manzanares,Omar El-Herraoui,Michail Maniatakos,Farah E. Shamout*

Main category: cs.LG

TL;DR: BlendFL是一个新颖的联邦学习框架，融合了横向和纵向联邦学习的优势，能够处理多模态数据异构性，支持去中心化推理，并在医疗和金融等隐私敏感领域表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决现实世界中多模态数据异构性问题，现有联邦学习框架在非理想设置下效果有限，无法应对客户端间模态和样本分布不均的情况。

Method: 提出BlendFL框架，同步融合横向和纵向联邦学习原则，支持去中心化推理，并引入BlendAvg自适应全局模型聚合策略。

Result: 在三个分类任务上的实验表明，BlendFL在多模态和单模态分类中均优于现有方法，收敛速度更快。

Conclusion: BlendFL在处理多模态数据异构性方面具有巨大潜力，特别适用于医疗和金融等隐私敏感领域的协作学习。
...
### [160] [To Steer or Not to Steer? Mechanistic Error Reduction with Abstention for Language Models](https://arxiv.org/abs/2510.13290)
*Anna Hedström,Salim I. Amoukou,Tom Bewley,Saumitra Mishra,Manuela Veloso*

Main category: cs.LG

TL;DR: MERA是一个通过选择性干预来减少语言模型错误的框架，能优化干预方向和强度，在无法自信修正时选择弃权。
...
Method: 通过优化干预方向和校准干预时机与强度，在无法自信修正时选择弃权，可与其他干预技术结合使用。

Result: 在多个数据集和语言模型上的实验显示，MERA能安全有效地纠正错误且不降低性能，优于现有基线方法。

Conclusion: MERA是一个通用且高效的机制性激活干预方法，可提升现有干预技术的性能。
...
### [161] [Federated Conditional Conformal Prediction via Generative Models](https://arxiv.org/abs/2510.13297)
*Rui Xu,Sihong Xie*

Main category: cs.LG

TL;DR: 提出了Fed-CCP方法，通过生成模型实现联邦学习中的条件保形预测，解决客户端数据分布差异问题


<details>
  <summary>Details</summary>
Motivation: 标准保形预测假设i.i.d.数据，但在联邦学习场景中客户端数据分布差异显著，现有方法只能保证边际覆盖，无法反映输入条件不确定性
...
### [162] [Km-scale dynamical downscaling through conformalized latent diffusion models](https://arxiv.org/abs/2510.13301)
*Alessandro Brusaferri,Andrea Ballarino*

Main category: cs.LG

TL;DR: 本文提出了一种结合生成扩散模型和保形预测框架的动态降尺度方法，通过后处理扩散模型的样本来获得条件分位数估计，从而提供具有有限样本边际有效性的局部自适应预测区间。


<details>
  <summary>Details</summary>
Motivation: 生成扩散模型在动态降尺度任务中缺乏有限样本保证，导致网格点级别的不确定性估计校准不佳，影响了在操作环境中的可靠性。

Method: 在降尺度流程中增加保形预测框架，将扩散模型的样本进行后处理得到条件分位数估计，并整合到保形化分位数回归过程中。

Result: 在意大利ERA5再分析数据上的实验表明，该方法相比扩散模型基线显著改善了网格点级别不确定性估计的覆盖率和稳定的概率评分。

Conclusion: 保形化生成模型为高分辨率气象场的概率降尺度提供了更可信的方法。
...
### [163] [Isolation-based Spherical Ensemble Representations for Anomaly Detection](https://arxiv.org/abs/2510.13311)
*Yang Cao,Sikun Yang,Hao Tian,Kai He,Lianyong Qi,Ming Liu,Yujiu Yang*

Main category: cs.LG

TL;DR: 提出ISER方法，通过使用超球体半径作为局部密度特征的代理，扩展了现有的基于隔离的方法，同时保持线性时间和恒定空间复杂度。
...
Conclusion: ISER方法有效解决了现有异常检测方法的关键挑战，并在多个数据集上展现出卓越性能。
...
<details>
  <summary>Details</summary>
Motivation: 在大规模非结构化离散空间中，贝叶斯优化由于缺乏梯度而难以高效最大化获取函数，需要一种可扩展的替代方法。

Method: 基于Thompson采样的ToSFiT方法，通过直接参数化候选解产生最大奖励的概率，利用提示条件化大型语言模型的先验知识，并逐步调整它们逼近后验分布。

Result: 在FAQ响应优化、热稳定蛋白质搜索和量子电路设计三个任务中验证，在线微调显著提高了样本效率，对计算效率影响可忽略。

Conclusion: ToSFiT方法通过变分Thompson采样实现了与标准方法相当的理论保证，并在实践中展示了优越的性能。
...
### [165] [RockNet: Distributed Learning on Ultra-Low-Power Devices](https://arxiv.org/abs/2510.13320)
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习在信息物理系统中的普及，需要将训练从云端转移到设备端，但超低功耗微控制器的有限计算资源使训练变得困难。

Method: 设计分布式学习方法，集成机器学习和无线通信，利用所有设备进行分布式训练，结合高效的无线多跳通信协议克服通信瓶颈。
...
TL;DR: 提出了Generalist框架，通过将整体泛化目标分解为多个子任务，每个任务分配给专门的基学习器，然后插值参数形成全局学习器，缓解了对抗训练中自然精度下降和跨攻击鲁棒性差的问题。


<details>
  <summary>Details</summary>
Motivation: 对抗训练虽然是最有效的防御方法，但在实际应用中存在两个主要限制：自然精度显著下降，以及在不同范数约束下生成的攻击之间鲁棒性转移不佳。

Method: 将整体泛化目标分解为多个子任务，每个子任务分配给专门的基学习器；在训练后期插值基学习器参数形成全局学习器；定期将全局参数重新分配给基学习器以防止优化轨迹偏离共享目标。
...
### [167] [When In Doubt, Abstain: The Impact of Abstention on Strategic Classification](https://arxiv.org/abs/2510.13327)
*Lina Alkarmi,Ziyuan Huang,Mingyan Liu*

Main category: cs.LG

TL;DR: 本文研究在战略分类中引入弃权机制的影响，证明最优弃权策略能确保分类器效用不低于非弃权设置，并能抑制代理的操纵行为。


<details>
  <summary>Details</summary>
Motivation: 算法决策系统易受战略代理的操纵，而分类器弃权机制已被证明能提高准确性，但尚未在战略分类背景下系统研究其影响。

Method: 使用Stackelberg博弈模型，其中主体（分类器）首先宣布决策策略，战略代理随后操纵特征以获取期望结果，重点关注二元分类器。

Result: 最优弃权策略确保主体效用不劣于非弃权设置，且弃权能增加操纵成本，特别是对资质较差的代理，从而抑制操纵行为。

Conclusion: 弃权是减少算法决策系统中战略行为负面影响的宝贵工具，既能提高准确性又能抑制操纵。
...
### [168] [A New Perspective on Transformers in Online Reinforcement Learning for Continuous Control](https://arxiv.org/abs/2510.13367)
...
<details>
  <summary>Details</summary>
Motivation: 尽管transformer在离线或基于模型的强化学习中表现出色，但在在线无模型强化学习中探索不足，主要因为其对训练设置和模型设计决策的敏感性。

Method: 研究关键设计问题：如何条件化输入、在actor和critic之间共享组件、以及如何切割序列数据进行训练，探索稳定的架构和训练策略。

Result: 实验表明这些策略在完全和部分可观测任务中都能实现有竞争力的性能，适用于向量和图像两种设置。
...
### [169] [Kernel Representation and Similarity Measure for Incomplete Data](https://arxiv.org/abs/2510.13352)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法要么丢弃不完整数据，要么进行插值预处理，导致信息丢失和相似性估计偏差。

Method: 引入数据依赖分箱结合邻近分配，将数据投影到高维稀疏表示中；采用级联回退策略处理缺失值分布估计。

Result: 在12个真实世界不完整数据集上的聚类任务中表现出优越性能，同时保持线性时间复杂度。

Conclusion: 邻近核方法能够有效处理不完整数据的相似性计算问题，优于现有方法。
...
TL;DR: 提出一种无监督的低秩方法来校正停车位GPS点的误差，并将其与道路对齐，解决高层建筑导致的GPS漂移问题。


<details>
  <summary>Details</summary>
Motivation: 停车位的准确GPS定位对城市管理至关重要，但高层建筑和低成本GPS设备会导致定位误差，需要一种无监督的方法来校正大量停车位中的错误GPS点。

Method: 利用停车位与道路平行的物理约束，提出无监督低秩方法，在统一框架中同时进行GPS点误差校正和对齐。

Result: 实验证明该方法能有效解决GPS点误差问题，对任何类型的GPS误差都表现良好。

Conclusion: 该方法简单有效，能够解决停车位GPS定位的实际问题，数据集和代码已开源。
...
### [171] [Neural Sum-of-Squares: Certifying the Nonnegativity of Polynomials with Transformers](https://arxiv.org/abs/2510.13444)
*Nico Pelleriti,Christoph Spiegel,Shiwei Liu,David Martínez-Rubio,Max Zimmer,Sebastian Pokutta*

Main category: cs.LG

TL;DR: 提出首个学习增强算法来验证多项式非负性，通过Transformer预测最小单项式基，显著减少SDP问题规模，实现100倍加速


<details>
  <summary>Details</summary>
Motivation: 验证多项式非负性是NP难问题，传统SOS方法计算昂贵，SDP维度随单项式基规模平方增长，需要降低基规模的方法

Method: 训练Transformer模型预测几乎最小的单项式基，包含三个关键组件：高效生成1亿+SOS多项式训练数据、设计训练Transformer架构、系统回退机制确保正确终止

Result: 在200多个基准数据集上验证，相比最先进求解器实现100倍以上加速，能够解决竞争方法失败的实例

Conclusion: 该方法为提升SOS编程的实际可扩展性提供了新见解，学习增强方法可显著改进多项式非负性验证效率
...
### [172] [DistilCLIP-EEG: Enhancing Epileptic Seizure Detection Through Multi-modal Learning and Knowledge Distillation](https://arxiv.org/abs/2510.13497)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习癫痫检测方法仅依赖单模态EEG信号，忽略了多模态信息的潜在优势，需要开发能整合多种信息源的方法。

Method: 基于CLIP框架，使用Conformer架构的EEG编码器和可学习BERT作为文本编码器，在共享潜在空间进行跨模态表示学习，并通过知识蒸馏训练轻量学生模型。

Result: 在TUSZ、AUBMC和CHB-MIT数据集上，师生模型准确率均超过97%，F1分数稳定高于0.94，学生模型参数量减少约41.9%。

Conclusion: 该模型在保持高性能的同时显著降低复杂度，为资源受限环境部署轻量级癫痫检测模型奠定了基础。
...
### [173] [Contrastive Learning-Based Dependency Modeling for Anomaly Detection in Cloud Services](https://arxiv.org/abs/2510.13368)
*Yue Xing,Yingnan Deng,Heyao Liu,Ming Wang,Yun Zi,Xiaoxuan Sun*

Main category: cs.LG

TL;DR: 提出一种结合对比学习的依赖建模和异常检测方法，通过构建依赖图、提取时空特征，使用对比学习增强正常与异常模式的可分离性，在云服务环境中实现稳定可靠的异常检测。


<details>
  <summary>Details</summary>
Motivation: 解决云服务环境中复杂依赖关系和多样化异常模式的挑战，传统方法难以有效处理多维度特征和噪声干扰。
...
TL;DR: 本文研究了在局部差分隐私模型下，基于KL正则化的人类反馈强化学习（RLHF）的离线和在线设置，提出了相应的算法并分析了性能边界。
...
Method: 在离线设置中采用悲观主义原则设计算法，在在线设置中采用乐观主义原则设计算法，并引入KL正则化和局部差分隐私约束。

Result: 离线设置中获得了$\tilde{O}(1/[(e^\epsilon-1)^2 n])$的次优性间隙，并证明其最优性；在线设置中获得了$O(d_{\mathcal{F}}\log(N_{\mathcal{F}}\cdot T)/(e^\epsilon-1)^2)$的对数遗憾边界。

Conclusion: 首次在理论上研究了带隐私保护的KL正则化RLHF问题，提出的算法在离线和在线设置中都取得了理论保证，并为无隐私的在线KL正则化RLHF提供了首个分析。
...
### [175] [Prediction Markets with Intermittent Contributions](https://arxiv.org/abs/2510.13385)
*Michael Vitali,Pierre Pinson*

Main category: cs.LG

TL;DR: 提出一个基于预测市场的合作框架，允许独立代理在考虑历史表现、适应时变条件的情况下自由进出市场，通过稳健回归模型优化预测组合并处理缺失提交。


<details>
  <summary>Details</summary>
Motivation: 虽然数据可用性和准确预测需求增加，但利益相关者之间的合作常受数据所有权和竞争利益限制。现有合作博弈框架存在局限，需要更通用的市场机制。

Method: 设计预测市场机制，使用稳健回归模型学习最优预测组合，处理缺失提交，并引入考虑样本内外表现的收益分配机制。

Result: 在模拟和真实数据案例研究中证明了所提市场设计的有效性和适应性。

Conclusion: 该预测市场框架能够在保护数据隐私和竞争利益的同时，促进多代理间的有效合作预测。
...
### [176] [Going with the Flow: Approximating Banzhaf Values via Graph Neural Networks](https://arxiv.org/abs/2510.13391)
*Benjamin Kempinski,Tal Kachman*

Main category: cs.LG

TL;DR: 提出基于图神经网络的方法来近似计算网络流博弈中的Banzhaf值，相比传统精确计算和蒙特卡洛采样方法，实现了数量级的速度提升和零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统Banzhaf值计算方法在超过20个代理时因指数复杂度变得不可行，蒙特卡洛方法样本复杂度高且无法在不同网络配置间迁移知识，难以应用于大规模动态系统。

Method: 将问题构建为图级预测任务，使用三种先进GNN架构（GAT、GINE、EdgeConv）直接从网络拓扑和控制结构中学习代理影响力的通用模式。

Result: 在包含200,000个图的大规模合成数据集上，训练后的GNN模型实现了高保真度的Banzhaf值近似，相比精确和采样方法有数量级的速度提升，并展现出强大的零样本泛化能力。

Conclusion: 这项工作确立了GNN作为复杂网络系统可扩展合作博弈论分析的实用工具，能够准确预测具有不同结构特性的新网络中的Banzhaf值，无需重新训练。
...
### [177] [K-Merge: Online Continual Merging of Adapters for On-device Large Language Models](https://arxiv.org/abs/2510.13537)
*Donald Shenaj,Ondrej Bohdal,Taha Ceritli,Mete Ozay,Pietro Zanuttigh,Umberto Michieli*

Main category: cs.LG

TL;DR: 提出了一种在设备存储有限情况下，在线持续合并LoRA适配器的数据无关且计算高效的方法，以支持增量任务部署。
...
Method: 采用数据无关且计算高效的策略，在新LoRA可用时选择和合并适配器，仅存储有限数量的适配器。

Result: 在真实世界任务上的广泛实验表明，该方法在遵守设备存储和计算限制的同时，优于其他替代策略。

Conclusion: 提出的方法有效解决了设备端在线持续合并LoRA的问题，在资源受限环境下保持多任务性能。
...
### [178] [Assessing the robustness of heterogeneous treatment effects in survival analysis under informative censoring](https://arxiv.org/abs/2510.13397)
*Yuxin Wang,Dennis Frauen,Jonas Schweisthal,Maresa Schröder,Stefan Feuerriegel*

Main category: cs.LG

TL;DR: 提出一个假设宽松的框架，用于评估生存分析中条件平均处理效应(CATE)估计在存在删失偏差时的稳健性，通过部分识别方法推导CATE的信息边界。


<details>
  <summary>Details</summary>
Motivation: 临床研究中高达一半的患者可能因副作用或其他原因提前退出，当退出是信息性时（即依赖于生存时间），会引入删失偏差，导致治疗效果估计出现偏差。

Method: 使用部分识别方法推导CATE的信息边界，开发一种新的元学习器，可以利用任意机器学习模型估计边界，具有双重稳健性和准oracle效率的理论特性。
...
### [179] [SWIR-LightFusion: Multi-spectral Semantic Fusion of Synthetic SWIR with {Thermal} IR {(LWIR/MWIR)} and RGB](https://arxiv.org/abs/2510.13404)
...
Method: 使用对比度增强技术从LWIR数据合成SWIR图像，构建包含RGB、LWIR和合成SWIR的多模态融合框架，采用优化的编码器-解码器神经网络架构，包含模态特定编码器和softmax门控融合头。

Result: 在多个公开数据集和私有真实数据集上的实验表明，该框架提升了融合图像质量（对比度、边缘清晰度、结构保真度），同时保持实时性能，显著优于现有方法。

Conclusion: 该方法在恶劣能见度条件下具有实际应用潜力，为监控和自动驾驶系统提供了有效的场景理解解决方案。
...
### [180] [Message Passing on the Edge: Towards Scalable and Expressive GNNs](https://arxiv.org/abs/2510.13615)
*Pablo Barceló,Fabian Jogl,Alexander Kozachinskiy,Matthias Lanzinger,Stefan Neumann,Cristóbal Rojas*

Main category: cs.LG

TL;DR: 提出了EB-1WL（基于边的颜色细化测试）和EB-GNN架构，该架构在消息传递中显式使用三角形，比1-WL更具表达能力，且在实际任务中保持近线性时间和内存复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有GNN架构在表达能力上存在局限，而之前提出的更具表达能力的架构通常计算复杂度较高。本文旨在开发既具有更强表达能力又保持高效计算的GNN架构。

Method: 受Chiba和Nishizeki经典三角形计数算法启发，提出了EB-1WL测试和EB-GNN架构，在消息传递过程中显式利用三角形结构信息。

Result: EB-1WL比1-WL显著更具表达能力；EB-GNN在保持近线性时间和内存复杂度的同时，大幅优于简单MPNN，并与任务专用GNN保持竞争力。

Conclusion: EB-GNN是一种高效通用的GNN架构，在保持计算效率的同时提供了更强的表达能力，为图学习任务提供了实用的解决方案。
...
### [181] [Optimizing Storage Overhead of User Behavior Log for ML-embedded Mobile Apps](https://arxiv.org/abs/2510.13405)
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习模型在移动应用中的普及，记录用户行为数据带来了巨大的存储成本，导致系统响应性下降和更多应用卸载。需要在不影响模型性能的前提下提高存储效率。

Method: 1) 将特征级冗余数据消除建模为超图中的最大权重匹配问题，提出分层算法；2) 使用虚拟哈希属性设计将异构行为分配到少量日志文件中；3) 设计增量更新机制以适应动态用户行为模式。
...
### [182] [Time Series Foundation Models: Benchmarking Challenges and Requirements](https://arxiv.org/abs/2510.13654)
*Marcel Meyer,Sascha Kaltenpoth,Kevin Zalipski,Oliver Müller*

Main category: cs.LG

TL;DR: 本文分析了时间序列基础模型（TSFMs）评估中存在的多重挑战，包括基准数据集代表性不足、缺乏时空评估、信息泄露风险以及外部冲击导致的全局模式记忆问题，呼吁开发更稳健的评估方法。


<details>
  <summary>Details</summary>
Motivation: 随着时间序列基础模型的发展，其零样本预测能力带来了新的评估挑战。类似于大语言模型，TSFMs评估面临数据完整性、基准数据集代表性和信息泄露等问题，需要建立更可靠的评估框架。

Method: 通过调查现有TSFM评估实践，识别出数据分区混淆、时空评估缺失、数据集重叠和信息泄露等关键问题，并分析这些问题的具体表现和影响。

Result: 研究发现现有评估方法存在广泛的数据分区混淆问题，导致性能估计膨胀和全局知识错误传递到局部时间序列。评估中还缺乏对时空维度的考量，且存在因数据集重叠导致的信息泄露风险。

Conclusion: 需要开发稳健的评估方法学，避免在大语言模型和传统时间序列基准测试中已观察到的陷阱，建议研究社区设计新的原则性方法，如基于真正样本外未来数据的评估，以确保TSFM评估的完整性。
...
### [183] [When Embedding Models Meet: Procrustes Bounds and Applications](https://arxiv.org/abs/2510.13406)
*Lucas Maystre,Alvaro Ortega Gonzalez,Charles Park,Rares Dolga,Tudor Berariu,Yu Zhao,Kamil Ciosek*

Main category: cs.LG

TL;DR: 本文研究如何通过正交变换对齐两个嵌入模型，提出基于Procrustes后处理的方法，使不同嵌入模型能够互操作，并在模型重训练、文本检索和多模态搜索等应用中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 解决相似数据上分别训练的嵌入模型之间缺乏互操作性的问题，这在模型重训练、部分模型升级和多模态搜索等实际应用中带来挑战。

Method: 使用Procrustes后处理方法，通过正交变换对齐两个嵌入集合，保持每个嵌入空间的几何结构。

Result: 实验证明该方法在三个应用中有效：跨重训练的兼容性维护、不同模型组合进行文本检索、改进多模态搜索（达到最先进性能）。

Conclusion: 通过正交变换可以有效地对齐嵌入模型，实现模型间的互操作性，同时保持各自的几何特性。
...
### [184] [Axial Neural Networks for Dimension-Free Foundation Models](https://arxiv.org/abs/2510.13665)
*Hyunsu Kim,Jonggeon Park,Joan Bruna,Hongseok Yang,Juho Lee*

Main category: cs.LG

TL;DR: 提出了维度无关的轴向神经网络(XNN)架构，用于处理不同维度物理数据的预训练基础模型，解决了传统方法在处理变维度PDE数据时的效率问题。


<details>
  <summary>Details</summary>
Motivation: 基础模型在AI领域的出现显著推进了通用学习能力，但在物理数据（如PDE解）上的训练面临维度变化的挑战。传统方法要么固定最大维度，要么为不同维度使用单独编码器，导致效率低下。

Method: 提出轴向神经网络(XNN)架构，受Deep Sets和图神经网络等参数共享结构启发。XNN能够泛化到不同张量维度，同时保持计算效率。将现有PDE基础模型转换为轴向神经网络，并在三种训练场景下评估性能。

Result: 实验表明XNNs与原始模型性能相当，并在未见维度上表现出更好的泛化能力，凸显了多维预训练对基础模型的重要性。

Conclusion: XNN架构有效解决了物理基础模型中维度变化带来的挑战，为多维PDE数据的预训练提供了高效解决方案。
...
### [185] [Modeling Adoptive Cell Therapy in Bladder Cancer from Sparse Biological Data using PINNs](https://arxiv.org/abs/2510.13431)
*Kayode Olumoyin,Katarzyna Rejniak*

Main category: cs.LG

TL;DR: 本文提出了一种改进的物理信息神经网络框架，应用于肿瘤学领域，通过嵌入微分方程建模的动力学系统规律和生物学约束，能够在稀疏数据条件下学习组合疗法在肿瘤微环境中的时变相互作用。


<details>
  <summary>Details</summary>
Motivation: 在肿瘤学中，实验数据通常稀疏且仅包含少数时间点的肿瘤体积数据。传统方法难以从有限数据中准确学习组合疗法的时变相互作用动力学。

Method: 扩展物理信息神经网络，将观察到的生物学约束作为正则化项嵌入损失函数，引导网络在稀疏训练样本下找到合理解并具有良好的泛化能力。

Result: 算法能够求解ODE并获得ODE模型参数的时变形式，在间歇性治疗动力学学习中表现出色，通过MSE、MAE和MAPE等指标验证了强收敛性。

Conclusion: 改进的PINN框架能够在肿瘤学稀疏数据条件下有效学习组合疗法的动力学行为，为肿瘤治疗建模提供了有前景的计算工具。
...
### [186] [Hybrid Interval Type-2 Mamdani-TSK Fuzzy System for Regression Analysis](https://arxiv.org/abs/2510.13437)
*Ashish Bhatia,Renato Cordeiro de Amorim,Vito De Feo*

Main category: cs.LG

TL;DR: 提出了一种结合Mamdani系统可解释性和TSK模型精度的新型模糊回归方法，通过混合规则结构和双重支配类型来平衡准确性与可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统回归方法难以处理现实数据中的不确定性和模糊性，而深度学习缺乏可解释性且容易在小数据集上过拟合。模糊系统能处理不确定性，但Mamdani和TSK系统在可解释性与准确性之间存在权衡。

Method: 开发了一种混合模糊回归方法，结合了Mamdani系统的可解释性和TSK模型的精度，采用包含模糊和清晰组件的混合规则结构以及双重支配类型。

Result: 在6个基准数据集上的评估显示，该方法在4个数据集上获得了最佳模糊方法得分，在2个数据集上优于不透明模型，在1个数据集上获得最佳总体得分，RMSE改进范围从0.4%到19%。
...
### [187] [Simplicial Embeddings Improve Sample Efficiency in Actor-Critic Agents](https://arxiv.org/abs/2510.13704)
*Johan Obando-Ceron,Walter Mayor,Samuel Lavoie,Scott Fujimoto,Aaron Courville,Pablo Samuel Castro*

Main category: cs.LG

TL;DR: 提出使用单纯形嵌入来加速强化学习训练，通过几何归纳偏置产生稀疏离散特征，提高样本效率和最终性能，且不影响运行速度。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模环境并行化方法虽然能加速训练时间，但仍需要大量环境交互才能达到理想性能水平。结构良好的表示可以改善深度强化学习的泛化能力和样本效率。

Method: 提出单纯形嵌入：轻量级表示层，将嵌入约束为单纯形结构。这种几何归纳偏置产生稀疏离散特征，稳定critic自举并增强策略梯度。应用于FastTD3、FastSAC和PPO算法。

Result: 在多种连续和离散控制环境中，单纯形嵌入一致提高了样本效率和最终性能，且没有损失运行速度。

Conclusion: 单纯形嵌入是一种有效的表示学习方法，能够提升强化学习算法的性能而不增加计算开销。
...
### [188] [$L_2$-Regularized Empirical Risk Minimization Guarantees Small Smooth Calibration Error](https://arxiv.org/abs/2510.13450)
...
<details>
  <summary>Details</summary>
Motivation: 概率预测的校准对于可靠的机器学习至关重要，但标准训练程序如何产生良好校准的模型尚不清楚。

Method: 基于优化误差、正则化强度和Rademacher复杂度建立平滑校准误差的有限样本泛化界，并在再生核希尔伯特空间中实例化该理论。

Result: 实验证实了理论保证，表明L2正则化ERM可以提供良好校准的模型，无需提升或后处理重新校准。

Conclusion: 标准L2正则化经验风险最小化本身就具有内在的校准特性，无需额外的校准技术。
...
### [189] [Towards Blackwell Optimality: Bellman Optimality Is All You Can Get](https://arxiv.org/abs/2510.13476)
...
<details>
  <summary>Details</summary>
Motivation: 虽然平均收益最优性是MDP中常用的性能度量，但它过于渐进。结合即时损失度量形成了偏差最优性层次结构，本文旨在识别具有此类最优性顺序的策略。

Method: 为每个最优性顺序构建了具有渐近误差的学习算法，并提供了可追踪的停止规则，当与学习算法结合时，在可能的情况下会在有限时间内触发停止。

Result: 确定了具有唯一Bellman最优策略的MDP类别，这类MDP的识别算法可以在有限时间内停止，且不依赖于所考虑的最优性顺序。

Conclusion: 本文提供了在偏差最优性层次结构中识别策略的有效方法，包括学习算法和停止规则，特别适用于具有唯一Bellman最优策略的MDPs。
...
### [190] [Tahakom LLM guidelines and receipts: from pre-training data to an Arabic LLM](https://arxiv.org/abs/2510.13481)
...
TL;DR: 本文探讨了开发阿拉伯语大语言模型面临的独特挑战，重点关注数据整理、分词器设计和评估方法，并提出了系统性解决方案。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在自然语言处理领域取得了显著进展，但为阿拉伯语开发LLM面临特殊挑战，需要专门的方法论。

Method: 采用系统方法进行阿拉伯语预训练数据集的收集和过滤，评估不同分词器设计对模型性能的影响，并提出改进现有阿拉伯语评估框架的方法。

Result: 开发了针对阿拉伯语的数据整理和分词器设计方法，提出了评估框架的纠正方法，并公开分享了数据和开发方法。

Conclusion: 通过系统化方法解决了阿拉伯语LLM开发的关键挑战，为阿拉伯语语言建模的进步做出了贡献，并促进了透明和协作开发。
...
TL;DR: 该研究首次系统性地分析了强化学习在大型语言模型中的扩展规律，提出了可预测的RL扩展框架，并开发了最佳实践方法ScaleRL。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习已成为训练大型语言模型的核心方法，但该领域缺乏类似预训练的可预测扩展方法，无法在计算预算快速增长的背景下评估算法改进效果。

Method: 进行了超过40万GPU小时的大规模系统研究，拟合RL训练的S形计算-性能曲线，消融分析各种常见设计选择对渐近性能和计算效率的影响。

Result: 发现：(1)不同方法产生不同的渐近性能；(2)损失聚合、归一化、课程学习和离策略算法等细节主要影响计算效率而非渐近性能；(3)稳定可扩展的方法遵循可预测的扩展轨迹。

Conclusion: 提出了最佳实践方法ScaleRL，成功在10万GPU小时的单一RL运行中预测验证性能，为RL训练提供了科学的扩展分析框架和接近预训练可预测性的实用方法。
...
TL;DR: 提出了一种基于信息论的"不可战胜"对抗攻击方法，通过率失真理论随机改变智能体对转移核的观测，使其在训练中获得极少的地面真实信息。


<details>
  <summary>Details</summary>
Motivation: 现有确定性对抗攻击可被智能体通过逆转攻击来防御，需要研究更强大的攻击方法来评估RL系统的鲁棒性。

Method: 采用率失真信息论方法，随机改变智能体对转移核或其他属性的观测，限制其获取地面真实信息。

Result: 推导了接收方智能体奖励遗憾的信息论下界，并展示了攻击对最先进的基于模型和无模型算法的影响。

Conclusion: 信息论方法可扩展到其他类型的对抗攻击，如状态观测攻击，为RL系统安全性评估提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 传统主题模型在医学文本上表现不佳，特别是在医疗领域某些主题文档数量较少的情况下。

Method: 使用原型网络构建主题模型，通过计算输入数据点与原型表示之间的距离来进行预测，特别适合低数据量或少样本学习场景。

Result: 与文献中使用的两个主题模型基线相比，ProtoTopic在主题连贯性和多样性方面都有所提升。
...
<details>
  <summary>Details</summary>
Motivation: 扩展传统在线凸优化(OCO)到多目标场景，考虑K个不同的损失函数序列，需要平衡跟踪所有序列的性能。

Method: 结合Hedge算法和在线梯度下降(OGD)的简单算法，用于i.i.d.输入设置。

Result: 在i.i.d.模型下，提出的算法期望最小-最大遗憾为O(√T log K)。

Conclusion: 成功将单目标OCO扩展到多目标场景，通过简单算法实现了接近最优的遗憾界。
...
<details>
  <summary>Details</summary>
Motivation: 联邦持续学习需要在保护隐私的同时学习新任务且不遗忘旧知识，现有方法在性能、隐私保护和通信效率之间难以平衡。

Method: 提出DOLFIN方法，结合Vision Transformers和低秩适配器(LoRA)，使用DualGradient Projection Memory防止遗忘，在联邦环境中高效稳定学习新任务。
...
TL;DR: 本文研究了针对MMLU基准测试的选择性对抗攻击，发现即使细微的语义等效扰动也能显著改变语言模型的相对排名，挑战了排行榜评估的公平性和透明度。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估基准容易受到语义等效对抗扰动的影响，但现有研究主要关注影响多个模型的通用攻击，而缺乏针对特定模型的选择性攻击研究。

Method: 使用TextAttack框架中的经典攻击方法，开发选择性评估协议，设计定制约束来提高攻击选择性，并提出了基于代理LLM的选择性扰动生成流程。

Result: 实验证明选择性对抗攻击确实存在，能够实质性改变模型相对排名，影响评估的公平性和可重复性。
...
TL;DR: ArtNet是一种新型的人工网表生成器，通过生成具有关键拓扑特征的多样化训练数据，解决机器学习模型泛化能力不足和设计技术协同优化中数据缺乏的问题。


<details>
  <summary>Details</summary>
Motivation: 在先进节点中，功耗、性能和面积的优化变得高度复杂。机器学习和设计技术协同优化面临训练数据缺乏和设计流程周转时间长的限制。

Method: 提出ArtNet人工网表生成器，复制关键拓扑特征，生成更接近目标参数的逼真人工数据集，支持更广泛的设计空间探索。

Result: 在CNN基DRV预测中，ArtNet数据增强使F1分数比仅使用原始数据集提高0.16。在DTCO应用中，ArtNet生成的mini-brains与目标全规模块设计的PPA匹配度达到97.94%。
...
Method: EEGChaT引入通道聚合令牌(CATs)来聚合跨通道信息，并利用改进的Attention Rollout技术计算可解释的通道重要性分数。

Result: 在DuIN数据集上评估，EEGChaT与现有分类模型集成可显著提高解码准确性，获得高达17%的绝对增益，且通道权重与人工选择通道有显著重叠。

Conclusion: EEGChaT是高维SEEG分析中有效且可泛化的通道选择解决方案，既提供增强性能又提供神经信号相关性的见解。
...
TL;DR: 提出了一种物理增强的多任务高斯过程框架，用于建模高维时空动态系统，通过几何感知的多任务高斯过程和物理正则化方案，在3D心脏电动力学建模任务中显著提升了预测精度。
...
Method: 开发了物理增强的多任务高斯过程框架，包括几何感知的多任务高斯过程模型来捕捉时空结构和任务间依赖关系，并通过基于物理的正则化方案将控制物理定律纳入模型。

Result: 在3D心脏电动力学建模任务中，数值实验表明该方法通过有效整合领域特定的物理约束和几何先验，显著优于现有方法的预测精度。

Conclusion: 所提出的P-M-GP框架能够有效建模复杂几何域上的时空动态系统，通过结合物理知识和数据驱动方法，在保持物理一致性的同时提高了预测性能。
...
TL;DR: 提出一种基于任务算术和神经正切核的创新方法，用于快速从模型中移除客户端的影响，解决现有知识移除方法需要多轮通信导致模型不可用的问题。
...
Result: 该方法能够满足特定的效能和时间要求，避免了传统方法中模型不可用的问题。

Conclusion: 提出的方法为联邦学习中的客户端贡献移除提供了一种高效解决方案，解决了现有方法的通信延迟和模型不可用问题。
...
TL;DR: 本文为经典非线性降维方法（如t-SNE、Isomap、LLE）构建了神经解码器架构和扩散生成过程，首次实现了双向映射，但在CelebA数据集上的实验表明，这种改造方法在重建质量和生成样本质量上都不如端到端优化的自编码器。


<details>
  <summary>Details</summary>
Motivation: 经典非线性降维技术虽然擅长创建低维嵌入用于可视化，但缺乏将嵌入映射回原始高维空间的能力，这限制了它们在生成应用中的使用。

Method: 构建了神经解码器架构来实现双向映射，并实现了在学习的流形空间上直接操作的扩散生成过程。

Result: 解码器能成功重建数据，但质量不如端到端优化的自编码器；流形约束的扩散产生低质量样本，表明经典NLDR嵌入的离散稀疏特性不适合生成模型所需的连续插值。

Conclusion: 为主要用于可视化和分析的NLDR方法添加生成能力存在固有挑战，经典NLDR嵌入的特性与生成模型的要求不匹配。
...
TL;DR: 本文提出了基于门电路的量子储层计算（MTS-QRC）方法，用于多变量时间序列预测，在NISQ硬件上实现了与经典方法相媲美的性能，并发现设备噪声在某些情况下可起到正则化作用。


<details>
  <summary>Details</summary>
Motivation: 现有量子储层计算研究主要针对单变量信号，且忽视了近期硬件的实际约束。本文旨在开发适用于多变量时间序列预测且适应当前量子硬件限制的量子储层计算方法。

Method: 采用基于门电路的量子储层计算架构，配对注入和记忆量子比特，使用Trotter化的最近邻横向场Ising演化，针对当前设备连接性和深度进行优化。

Result: 在Lorenz-63和ENSO数据集上分别获得0.0087和0.0036的均方误差，在Lorenz上与经典储层计算性能相当，在两个数据集上都优于学习的RNN。在IBM Heron R2上，MTS-QRC在现实深度下保持精度，在ENSO上甚至优于无噪声模拟器。

Conclusion: 研究证明了基于门电路的量子储层计算在NISQ硬件上用于多变量时间序列预测的实用性，并发现硬件噪声可通过集中特征方向的方差来对线性读出起正则化作用，这为系统研究硬件噪声何时以及如何有益于QRC读出提供了动机。
...
TL;DR: 该论文表明几种流行的基于二元奖励的大语言模型强化学习算法可以看作是正确回答概率的单调变换上的随机梯度上升


<details>
  <summary>Details</summary>
Motivation: 揭示不同强化学习算法在数学上的统一性，理解它们在优化目标上的本质联系

Method: 将拒绝采样算法和GRPO算法等流行方法建模为正确回答概率的单调变换上的随机梯度上升

Result: 发现拒绝采样算法对应对数变换，GRPO算法对应反正弦平方根变换

Conclusion: 不同强化学习算法本质上是优化相同目标的不同单调变换形式，这为理解算法间关系提供了统一框架
...
TL;DR: 提出了一种基于分布校准的类别不平衡分类方法RCS，通过使用多数类和中间类的高斯混合成分加权参数来估计少数类分布参数，有效缓解过泛化问题。


<details>
  <summary>Details</summary>
Motivation: 解决类别不平衡问题中分类器偏向多数类的偏差问题，传统方法仅使用多数类分布近似少数类统计会导致过泛化。

Method: 使用编码器-解码器网络保持不平衡数据结构，通过分布校准策略从编码器提取的特征向量生成合成样本，利用邻域数据点分布校准参数。

Result: 在多种图像、文本和表格数据集上的实验结果表明，该方法相比多个基线和最先进技术实现了更优越的分类性能。

Conclusion: RCS方法通过分布校准有效解决了类别不平衡问题，在多个领域数据集上表现出色，为不平衡分类提供了稳健的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 对角预条件器作为二阶优化器的近似，在深度学习训练中显示出加速潜力。Adam和Gauss-Newton是两种主要方法，但它们在基选择和梯度噪声影响方面的比较尚不明确。

Method: 在二次目标和逻辑回归上分析四种象限情况，比较不同基选择下的优化器性能，并研究全批次和随机设置下的行为差异。

Result: 无论基选择如何，在全批次设置下存在Adam优于GN^{-1}和GN^{-1/2}的情况；在随机设置下，对于高斯数据假设的线性回归，Adam与GN^{-1/2}行为相似。

Conclusion: 理论分析表明Adam和GN方法在不同设置下各有优势，这些结果在凸和非凸目标上的实证研究得到支持。
...
TL;DR: 提出了InfoRM和IBL框架来解决RLHF中的奖励过优化问题，通过信息瓶颈原则过滤偏好无关信息，并使用分布级正则化来缓解奖励错误泛化。


<details>
  <summary>Details</summary>
Motivation: RLHF在语言模型对齐中面临奖励过优化的挑战，主要障碍包括奖励模型对虚假特征的过拟合以及RL优化中缺乏合适的正则化约束。

Method: 基于信息瓶颈原则的InfoRM奖励建模框架，以及IBL分布级正则化方法，通过Mahalanobis距离检测异常响应并惩罚偏离。

Result: 在多种LLM和数据集上的实验证实了方法的有效性，InfoRM和IBL能够显著缓解奖励过优化问题，MOP指标可可靠地量化奖励过优化程度。

Conclusion: 提出的信息论框架和正则化方法共同推进了RLHF技术的发展，为解决奖励过优化问题提供了有效的理论和实践方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统LLM剪枝方法依赖贪婪启发式算法，忽略了权重交互，导致剪枝效果不佳。本文旨在解决这一组合优化问题。

Method: 采用凸松弛技术将组合约束转化为可优化问题，使用Frank-Wolfe算法求解，最后通过取整获得近似最优解。

Result: 该方法显著减少了逐层剪枝误差，在GPT架构上优于现有基线方法，且保持内存效率。

Conclusion: 提出的方法通过理论保证获得原组合问题的近似解，为LLM剪枝提供了更有效的解决方案。
...
TL;DR: 该论文评估了深度学习模型在千米尺度天气降尺度任务中的表现，发现现有模型在地理泛化和物理一致性方面存在不足，并提出使用功率谱密度损失函数来改善这些问题。


<details>
  <summary>Details</summary>
Motivation: 千米尺度天气数据对实际应用至关重要，但传统天气模拟计算成本高。深度学习模型提供了更快的替代方案，但其可靠性仍受质疑，因为现有评估主要使用标准机器学习指标而非大气物理学的洞察。

Method: 对最先进的深度学习模型进行基准测试，引入基于物理的诊断方法来评估性能，特别关注地理泛化和物理一致性。提出使用功率谱密度损失函数来改善小尺度物理结构的重建。

Result: 实验表明，即使像CorrDiff这样的模型在训练区域表现良好，但在泛化到其他地理区域（如伊比利亚、摩洛哥或斯堪的纳维亚）时表现不佳。模型还无法准确捕捉从预测速度场导出的二阶变量（如散度和涡度）。

Conclusion: 当前深度学习天气降尺度模型在地理泛化和物理一致性方面存在显著挑战。提出的功率谱密度损失函数能够经验性地改善地理泛化能力，但需要更多工作来确保物理一致性预测。
...
TL;DR: 该论文提出了一个针对块马尔可夫决策过程的两阶段强化学习算法，通过先学习潜在结构再基于结构进行优化，实现了O(√T+n)的遗憾界，优于之前的O(√T+n²)结果，并证明了该算法的渐近最优性。


<details>
  <summary>Details</summary>
Motivation: 高维状态和动作空间使得强化学习在许多实际应用中不实用，但许多环境具有可利用的结构。块马尔可夫决策过程建模了具有大观测空间但转移动态完全由潜在状态决定的问题，通过利用聚类方法恢复潜在结构可以加速学习。

Method: 采用两阶段强化学习算法：第一阶段通过随机探索学习潜在结构，第二阶段切换到基于已发现结构的乐观引导策略。

Result: 算法在可聚类的BMDPs类上实现了O(√T+n)的遗憾界，其中T是时间步数，n是观测空间基数，这显著优于之前最好的O(√T+n²)结果。

Conclusion: 该算法在所研究的BMDPs类上达到了渐近最优性，证明了准确估计潜在状态确实可以有效加速学习过程。
...
TL;DR: 提出一种渐进式多保真度代理模型，能够顺序整合不同类型的数据，通过双连接系统利用不同数据集之间的相关性，同时确保每个级别对前一级进行加法修正而不改变它。


<details>
  <summary>Details</summary>
Motivation: 高精度数据获取成本高昂且耗时，而低精度数据虽然容易获取但精度不足。实际应用中数据可能来自不同模态且非同时可用，这给建模带来了挑战。

Method: 使用定制编码器顺序整合不同类型数据，通过神经网络进行多保真度回归。采用双连接系统：所有编码输入的连接和最终输出的加法连接，实现从低到高保真度的信息流动。

Result: 在数值基准测试和实际案例研究中，该方法能够可靠地整合多模态数据并提供准确预测，在时间和参数变化泛化时保持性能。

Conclusion: 该渐进式多保真度代理模型能够有效利用多模态数据，在整合新输入数据时防止性能下降，并基于可用输入自动调整预测。
...
TL;DR: 提出了TGPS，一种基于张量高斯过程的PDE求解器，通过一维GP建模和交替最小二乘法，显著提高了训练效率和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习PDE求解器存在效率低和可扩展性差的问题：神经网络求解器依赖随机训练效率低，高斯过程求解器在处理大量配置点时存在可扩展性问题。

Method: 使用张量分解将高维GP建模为一维GP的组合，采用部分冻结策略和牛顿法线性化非线性项，开发交替最小二乘法进行高效训练。

Result: 在多个基准PDE上的实验表明，该方法相比现有方法实现了更高的精度和效率。

Conclusion: TGPS方法通过张量分解和高效训练策略，成功解决了现有PDE求解器的效率和可扩展性问题，具有理论保证和实际优势。
...
Method: 采用模态特定编码器处理不同输入类型，通过基于Transformer的融合模块学习统一表示，支持在预训练和推理期间灵活使用任何可用模态子集。

Result: 在56个城市的41个任务评估中，UrbanFusion在位置编码方面优于先前的基础模型，支持推理时的多模态输入，并对训练期间未见区域具有良好的泛化能力。

Conclusion: UrbanFusion通过随机多模态融合技术实现了强大的地理空间数据整合能力，为不同数据可用性场景提供了广泛适用性。
...
TL;DR: T3former是一个新颖的拓扑时序Transformer，通过滑动窗口拓扑和谱描述符作为主要token，结合描述符注意力机制，在时序图分类任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 时序图分类在网络安全、脑连接分析等领域至关重要，但现有方法存在时间信息丢失、长程依赖处理困难、过平滑和过压缩等问题，需要更好的解决方案。

Method: 使用滑动窗口拓扑和谱描述符作为主要token，通过专门的描述符注意力机制进行集成，避免刚性离散化，保持时间保真度。

Result: 在动态社交网络、脑功能连接数据集和交通网络等多个基准测试中实现了最先进的性能，并在时间和结构扰动下具有理论稳定性保证。

Conclusion: 结合拓扑和谱洞察的方法能够推进时序图学习的前沿，T3former展示了这种方法的强大能力。
