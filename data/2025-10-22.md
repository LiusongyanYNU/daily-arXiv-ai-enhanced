...
Method: 部署自主智能体动态调优训练参数，采用非平稳多臂老虎机算法，结合双速率指数移动平均平滑和混合精度训练，通过复合奖励函数平衡准确性、稀有类别性能和训练稳定性。

Result: 在Pascal VOC上达到97.4 mAP（优于PAT-T的96.2），COCO上92.8 mAP（优于HSQ-CvN的92.0），VG-256上60.9 mAP，具有加速收敛和强大的跨域泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决情感计算领域的西方中心偏见，开发面向非西方文化背景的个性化娱乐系统。

Method: 收集241名参与者观看10场专业漫才表演的多模态数据，使用k-means聚类识别观众类型，进行个体层面分析和自动幽默分类。

Result: 识别出三种观众类型：稳定高欣赏者(72.8%)、低可变下降者(13.2%)和可变改善者(14.0%)；发现正向观看顺序效应；自动幽默分类未发现类型间差异。

Conclusion: 该数据集支持开发文化感知的情感AI和个性化娱乐系统，特别适用于非西方语境。
...
Method: CoIDO采用轻量级插件评分器，仅在小规模随机样本上训练来学习候选集分布，通过同方差不确定性公式平衡重要性和多样性。

Result: 使用20%随机数据训练CoIDO评分器后，在LLaVA-1.5-7B模型的十个下游任务中，选出的20%子集平均达到全数据微调98.2%的性能。

Conclusion: CoIDO框架能显著降低计算成本，同时保持模型性能，为大规模多模态模型训练提供了高效的数据选择解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 胶质母细胞瘤患者对治疗反应存在高度异质性，传统方法需要至少两个月才能通过MRI观察到治疗效果，早期预测治疗反应对个性化医疗至关重要。

Method: 使用潜在扩散模型，结合治疗前MRI和肿瘤定位的串联条件，以及基于生存信息的无分类器引导来增强生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 随着生成式AI和大型语言模型的快速发展，AI生成的图像越来越逼真，难以与人类创作图像区分，这对数字内容认证构成了挑战。需要验证数字数据的完整性和来源，以维护数字媒体的信任和合法性。

Method: 开发了一个基于图像嵌入的检测框架，使用五个基准嵌入模型处理多样化的AI生成和人类创作图像数据集，通过向量相似度分析图像在嵌入空间中的聚类特征。

Result: 实验证明该方法具有鲁棒性，中度到高度扰动对嵌入签名影响较小，扰动后的图像仍与其原始版本保持高度相似性匹配。
...
<details>
  <summary>Details</summary>
Motivation: 反馈信号在对话中至关重要，但个体差异显著影响其表达，现有情感识别方法未能充分处理这种多尺度行为特征。

Method: 使用级联多尺度个体标准化网络，在帧级和序列级提取个体归一化特征，并引入隐式数据增强模块处理数据分布偏差。
...
<details>
  <summary>Details</summary>
Motivation: 现有的流匹配模型需要专门的步长嵌入才能实现轨迹跳过，但这与现有模型不兼容，重新训练成本几乎与预训练相当。

Method: 基于速度场的自蒸馏方法，无需步长嵌入，通过在线自引导蒸馏快速学习，可在不到一个A100天的训练时间内生成3步Flux模型。

Result: 该方法能够生成高效的少步流模型而不损失质量，首次实现了数十亿参数扩散模型的少样本蒸馏（如10个文本-图像对），以几乎零成本提供最先进的性能。

Conclusion: 该方法不仅提供高效的后训练蒸馏方案，还可整合到预训练阶段本身，产生天生学习高效少步流的模型，为大规模扩散模型提供了经济高效的加速方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统的人类活动识别方法在水下环境中效果不佳，特别是潜水员佩戴的传感器与自主水下航行器(AUV)之间的无线通信信号衰减严重。需要一种新方法来监测潜水员安全，特别是识别可能导致医疗紧急情况的异常行为。

Method: 利用计算机视觉从3D人体关节点流生成高保真运动数据，创建"伪IMU"。将分类器集成到AUV上，通过模拟遇险场景进行实验验证。

Result: 实验证明了该方法在机器人监测和潜水员安全方面的实用性和有效性，能够识别如心脏骤停等医疗紧急情况的异常行为信号。

Conclusion: 该混合方法成功解决了水下环境中的活动识别挑战，为潜水员安全监测提供了一种有效的技术方案，特别是在传统传感器通信受限的情况下。
...
Method: 收集新的配对RGB和X射线数据，进行分层拟合以对齐RGB和X射线辐射高斯泼溅，提出X射线参考损失确保内部结构一致性。
...
<details>
  <summary>Details</summary>
Motivation: 解决零样本2D目标检测和分割中模型泛化能力不足的问题，特别是在未见过的3D物体上实现准确的2D检测和分割。

Method: 使用3D未见物体的2D多视角模板渲染和查询图像的2D目标提议；集成类别和补丁嵌入，使用广义平均池化归一化；采用结合绝对和相对相似度的联合相似度度量；通过不确定性感知目标先验优化相似度得分。

Result: 在BOP Challenge 2025中，无需额外训练或微调，在Classic Core、H3和Industrial三个赛道均排名第一，达到最先进的性能水平。

Conclusion: MUSE提供了一个强大且可泛化的零样本2D目标检测和分割框架，证明了其在挑战性场景中的鲁棒性和有效性。
...
<details>
  <summary>Details</summary>
Motivation: 光学音乐识别领域面临真实标注数据稀缺的问题，特别是在处理手写历史乐谱时。类似的手写文本识别领域已证明合成数据有助于训练更好的识别模型。

Method: 实现了一个音乐符号级的生成对抗网络(GAN)来生成逼真的手写风格符号，然后使用Smashcima排版软件将这些符号组装成完整的乐谱。

Result: 系统评估了生成样本的视觉保真度，发现生成的符号具有高度的真实感，在合成乐谱生成方面取得了显著进展。

Conclusion: 该方法为光学音乐识别提供了有效的合成数据生成解决方案，生成的符号质量高，有助于训练更好的识别架构。
...
<details>
  <summary>Details</summary>
Motivation: 性别分类系统经常继承并放大训练数据中的人口统计不平衡问题。现有数据集存在严重的交叉性代表性不足，导致模型产生偏见，特别是对女性和少数种族群体的误分类率更高。

Method: 首先审计了五个广泛使用的性别分类数据集，然后在两个相对平衡的数据集(UTKFace和FairFace)上训练MobileNetV2分类器。为了对抗数据引起的偏见，构建了BalancedFace数据集，通过混合FairFace和UTKFace的图像，并补充其他集合的图像来填补缺失的人口统计空白，确保189个年龄、种族和性别交叉子组的平等代表性。

Result: 在BalancedFace上训练的标准分类器，相比最佳现有数据集，将种族子组间的最大真阳性率差距减少了50%以上，并将平均差异影响分数向理想值1.0拉近了63%，同时整体准确率损失最小。

Conclusion: 研究结果强调了以数据为中心的干预措施的深远价值，并为公平性别分类研究提供了公开可用的资源。BalancedFace数据集能够显著改善性别分类模型的公平性表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D弱监督语义分割方法存在伪标签质量低和3D几何先验利用不足的问题，这成为开发高性能模型的技术瓶颈。

Method: 设计类感知标签细化模块生成平衡准确的伪标签，开发几何感知标签细化组件整合3D几何约束过滤低置信度伪标签，提出标签更新策略结合自训练扩展标签覆盖范围。

Result: 在ScanNet和S3DIS基准上达到最先进性能，在无监督设置下展现出卓越的泛化能力，通过鲁棒设计保持竞争性准确率。

Conclusion: 该方法通过有效整合3D几何先验和迭代标签优化，成功解决了3D弱监督语义分割中的关键挑战，为开发高性能模型提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分析中深度学习分割算法存在内在数据偏见，基于种族和性别等敏感属性的性能差异公平性问题日益紧迫，需要评估分割模型在不同人口亚组中的表现。

Method: 使用包含四个种族-性别亚组的数据集，评估UNesT、nnU-Net、CoTr和ANTs模型在伏隔核分割中的性能，采用手动标注金标准进行训练和测试，通过公平性度量指标和线性混合模型分析人口变量对分割准确性和体积的影响。

Result: 在种族匹配训练和测试条件下，ANTs和UNesT的分割准确性显著提高，而nnU-Net表现出与人口统计匹配无关的稳健性能。使用有偏见模型时，手动分割观察到的性别效应仍然存在，但种族效应在除一个模型外均消失。

Conclusion: 分割模型的性能存在种族相关的偏见，种族匹配训练能改善某些模型的表现，而nnU-Net显示出更好的公平性。模型偏见会影响基于分割的体积分析结果，特别是种族效应的检测。
...
<details>
  <summary>Details</summary>
Motivation: 在线学习环境中的参与度检测对于改善学生学习成果和个性化教学至关重要。

Method: 使用双流架构，通过EfficientNetV2提取面部裁剪和完整视频帧的空间特征，然后使用LSTM和Transformer编码器进行时间建模，并应用针对性数据增强技术。

Result: 在DAiSEE数据集上，ViBED-Net with LSTM变体达到73.43%准确率，超越了现有最先进方法。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统对罕见、分布外场景中的语义异常高度脆弱，现有视觉语言模型方法存在性能不可靠、依赖昂贵专有模型的问题，限制了实际部署。

Method: 采用结构化推理框架，包含结构化场景描述提取和多模态评估两阶段流程，在四个语义层（街道、基础设施、可移动对象、环境）进行系统分析。

Result: 在真实驾驶场景中达到89.6%召回率和88.0%准确率，显著优于非结构化基线。微调的7B参数开源模型Qwen2.5VL达到90.8%召回率和93.8%准确率，超越所有评估模型。
...
<details>
  <summary>Details</summary>
Motivation: 红棕榈螨侵染已成为严重问题，导致生产力下降和经济损失，需要准确早期识别受侵染植物以实现有效管理。

Method: 采用TriggerNet框架集成Grad-CAM、RISE、FullGrad和TCAV生成视觉解释，使用CNN、EfficientNet、MobileNet、ViT、ResNet50、InceptionV3等深度学习模型和Random Forest、SVM、KNN等机器学习分类器进行植物分类，利用Snorkel自动标注疾病类别。

Result: 研究使用11种植物物种的RGB图像数据集，将植物分为健康、黄斑、红褐色青铜化和丝网四个疾病类别进行训练和评估。

Conclusion: 该框架为植物分类和疾病检测提供了可解释的AI解决方案，特别针对红棕榈螨侵染问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在几何推理方面存在困难，需要一种能够生成空间感知的3D相机轨迹和自然语言摘要的方法，用于房地产和旅游应用的自动化专业质量视频创建。

Method: 使用扩散过程生成受已知相机位姿约束的平滑视频轨迹，将3D信息整合到视觉语言模型中，使用3D高斯泼溅技术沿轨迹渲染新视图。

Result: 实验表明，将3D相机轨迹整合到文本生成过程中比独立处理每个任务的方法性能更好，提出了新的联合评估指标。

Conclusion: 该方法能够为房地产和旅游应用创建自动化、专业质量的视频，无需专业知识或专业设备。
...
<details>
  <summary>Details</summary>
Motivation: 现有的个性化图像生成模型缺乏对多源图像中特定部分组合的显式控制能力，无法根据文本指令精确组合不同图像中的对象部分。

Method: 首先基于464个独特（部分，主体）对的语义原子构建数据集，生成37k提示并使用高保真文本到图像模型合成相应图像。训练带有部分条件引导的自定义扩散先验模型，通过部分条件引导来增强图像条件特征，强制语义一致性和空间布局。

Result: 人类评估和提出的PartEval指标显示，Chimera在部分对齐和组合准确性方面比其他基线方法高出14%，在视觉质量方面高出21%。

Conclusion: Chimera模型能够有效实现多源图像中特定部分的组合生成，在部分对齐、组合准确性和视觉质量方面均优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 微塑料污染对生态系统和人类健康构成威胁，但其微小尺寸使得检测和分类困难，传统方法需要人工分析且难以大规模应用。

Method: 使用扫描电子显微镜（SEM）成像结合基于YOLO的机器学习目标检测方法，专注于具有对称重复图案背景的过滤场景。

Result: 研究发现不同YOLO模型在检测任务中表现存在差异，预处理优化对检测质量有重要影响。

Conclusion: 机器学习在微塑料检测中具有潜力，但仍面临专家标注数据有限等挑战，需要进一步研究解决。
...
<details>
  <summary>Details</summary>
Motivation: 传统ViT将输入图像划分为统一大小的图像块，不考虑图像内容，导致高分辨率图像输入序列过长。APT旨在通过自适应调整图像块大小来解决这个问题。

Method: APT使用多种不同大小的图像块，在图像更均匀的区域使用更大的块，在更复杂的区域使用更小的块，从而减少总输入token数量。

Result: APT在ViT-L和ViT-H上分别实现了40%和50%的吞吐量提升，同时保持下游性能；在视觉问答、目标检测和语义分割等密集视觉任务中，训练和推理速度提升高达30%。

Conclusion: APT通过自适应图像块划分有效减少了ViT的输入序列长度，显著提升了训练和推理效率，同时不损失模型性能，且可与已微调的ViT兼容，收敛迅速。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS在3D重建领域带来了根本性变革，通过显式建模3D高斯集合实现了高效的光栅化，但存在内存占用高、光照效果被直接烘焙到表示中以及次生射线效果支持有限等问题。

Method: 从泼溅公式出发，探索解决3DGS局限性的主要方法，包括表面重建、虚拟形象建模、动画和内容生成等应用。

Result: 3DGS通过体积泼溅实现了与常见图形管线的无缝集成，具备实时渲染能力，适用于前馈式管线。

Conclusion: 3D高斯泼溅技术为3D重建领域带来了重大突破，其高效的渲染特性和对前馈管线的适应性使其在多个应用领域具有广阔前景。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型部署成本高，小模型需要昂贵微调才能缩小性能差距。研究如何在低资源、预算受限环境中有效适配视觉语言模型。

Method: 基于上下文学习框架，提出在线上下文蒸馏方法，包含跨模态演示选择策略、教师测试时缩放减少噪声、学生不确定性条件化动态填充演示池等技术。

Result: ICD方法显著提升小模型性能(最高33%)，仅需少量教师标注(低至4%)，能与教师模型的零样本性能竞争。

Conclusion: 上下文蒸馏为低资源环境下的视觉语言模型部署提供了高效解决方案，在有限计算预算下优于微调方法。
...
Method: 开发了全面的攻击策略分类，包括连接中断、中继/重放干扰、内容欺骗和多连接伪造。提出了SafeCoop防御框架，整合语义防火墙、语言-感知一致性检查和多源共识，通过代理转换函数实现跨帧空间对齐。
...
Method: 开发World-in-World平台，提供统一在线规划策略和标准化动作API，构建四个闭环环境来评估不同世界模型。

Result: 发现三个意外结果：视觉质量不保证任务成功，可控性更重要；后训练数据扩展比升级预训练视频生成器更有效；增加推理时间计算可显著提升闭环性能。

Conclusion: 提出了首个具身场景下世界模型的数据扩展定律，强调需要超越视觉质量来评估世界模型的具身效用。
...
<details>
  <summary>Details</summary>
Motivation: 现有立体视觉重建方法在月球环境下表现不佳，因为月球缺乏纹理、光照变化复杂且轨道轨迹特殊，而现有深度学习模型无法直接迁移到月球条件。

Method: 引入LunarStereo数据集（首个月球立体图像对开源数据集），通过光线追踪基于高分辨率地形和反射率模型生成，并基于此数据集对MASt3R模型进行微调以适应月球领域。

Result: 在合成和真实月球数据上的广泛实验验证了方法的有效性，在3D表面重建和相对位姿估计方面相比零样本基线有显著改进。

Conclusion: 该方法为地外环境中的鲁棒跨尺度泛化铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 解决拥挤场景中由于严重遮挡和高度动态、上下文相关的运动模式导致的异常检测挑战，现有方法难以适应不同人群密度且缺乏可解释的异常指标。

Method: 结合头部检测和密集光流提取个体速度，使用分层聚类将速度分类为语义运动类别（停止、慢速、正常、快速），并采用百分位异常评分系统衡量与学习到的正常模式的偏差。

Result: 实验证明该框架在密集拥挤环境中能够有效实时检测各种异常运动模式。

Conclusion: VelocityNet框架在拥挤场景异常检测方面表现出色，能够适应不同人群密度并提供可解释的异常指标。
...
<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型难以同时生成诊断文本和像素级分割掩码，限制了临床应用的实用性。需要开发能够同时提供两种模态输出的辅助系统。

Method: 首先创建RadDiagSeg-D数据集，整合异常检测、诊断和多目标分割任务；然后基于该数据集开发RadDiagSeg-M模型，支持联合异常检测、诊断和灵活分割。

Result: RadDiagSeg-M在多目标文本和掩码生成任务的所有组件上都表现出强劲性能，建立了有竞争力的基准。
...
<details>
  <summary>Details</summary>
Motivation: 解决甲状腺微小癌射频消融中超声视频分割的挑战，包括低对比度、探头引起的运动和热相关伪影，以及SAM-2模型在介入性超声中帧独立设计导致的不稳定预测和时间漂移问题。

Method: 在SAM-2基础上引入置信度加权的指数移动平均指针到记忆库中，提供跨帧的稳定肿瘤潜在原型，保持时间一致性同时快速适应清晰证据的出现。

Result: 在PTMC-RFA数据集上，EMA-SAM将maxDice从0.82提升到0.86，maxIoU从0.72提升到0.76，假阳性减少29%。在外部基准测试中，Dice分数比SAM-2持续提高2-5个百分点，同时仅增加<0.1%的FLOPs，保持约30FPS的实时吞吐量。
...
<details>
  <summary>Details</summary>
Motivation: 现有安全评估方法将视觉和语言输入分开处理，忽略了联合解释的风险，且无法清晰区分明显有害内容和边界案例。

Method: 使用多阶段流程和真实世界图像构建大规模基准测试，包含8,187个样本，涵盖15种危害类别和17种安全模式。

Result: 模型在需要联合图像-文本推理时性能大幅下降，34%的错误发生在单独模态分类正确的情况下。指令框架可以减少边界内容的过度阻止，但会降低对不安全内容的拒绝率。
...
<details>
  <summary>Details</summary>
Motivation: 现有去偏方法受限于样本表示多样性，而简单的生成增强往往保留原有偏见。研究发现仅增加罕见类数据是次优的，因为实例频率不能完全反映模型需求，且现有布局到图像合成缺乏保真度和控制能力。

Method: 引入表示分数(RS)来诊断频率之外的表示差距，指导创建无偏布局；用精确的视觉蓝图替代模糊文本提示，并采用生成对齐策略促进检测器和生成器之间的通信。

Result: 显著缩小了代表性不足物体组的性能差距，大/罕见实例的mAP比基线提高4.4/3.6，在生成图像的布局准确性上比现有L2I合成模型高出15.9 mAP。

Conclusion: 该生成式去偏框架通过表示分数诊断和高质量合成策略，有效解决了物体检测中的偏见问题，显著提升了罕见物体的检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 研究长上下文压缩的可行性，探索历史文档压缩和LLM记忆遗忘机制，同时提供高效的OCR解决方案。

Method: 采用DeepEncoder编码器保持高分辨率输入下的低激活，实现高压缩比，结合DeepSeek3B-MoE-A570M解码器进行OCR识别。

Result: 10倍压缩比下OCR精度97%，20倍压缩比下精度60%；在OmniDocBench上仅用100视觉token超越GOT-OCR2.0，用少于800视觉token超越MinerU2.0；单A100-40G每日可处理20万+页面。

Conclusion: DeepSeek-OCR在长上下文压缩方面展现出巨大潜力，为历史文档处理和LLM记忆机制研究提供了新方向，同时具有高实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 解决3D物体分类中合成数据训练与真实稀疏噪声LiDAR扫描之间的域差距问题，当前方法在合成数据上训练无法泛化到室外场景，而仅用真实数据训练又缺乏语义多样性。

Method: 提出生成大规模对象级三元组数据集（点云、图像、文本描述），并采用课程学习的数据混合策略，先在语义丰富的合成CAD数据上训练，再逐步适应真实世界扫描特性。

Result: 仅需在每批次中引入1.5%的真实世界样本，就能在nuScenes基准上提升零样本准确率27%，在nuScenes和TruckScenes上达到最先进性能，比之前最佳方法提升19.3%。

Conclusion: 有效的域适应而非大规模真实世界标注是实现鲁棒开放词汇3D感知的关键，证明了混合训练策略的高效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的语义高斯泼溅方法存在两个主要问题：(1) 预处理过程中单个掩码的上下文线索不足；(2) 从2D模型融合多视角特征时存在不一致性和细节缺失。
...
Conclusion: 该方法在3D场景理解及其在实际部署中展现出强大的鲁棒性和泛化能力，标志着该领域的重要进展。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频3D人体网格恢复方法在欧几里得空间中学习网格特征，难以准确捕捉人体层次结构（如躯干-肢体-手指），导致重建错误。

Method: 设计时间运动先验提取模块，分别从3D姿态序列和图像特征序列提取时间运动特征并融合；提出双曲空间优化学习策略，利用时间运动先验辅助学习，在双曲空间中分别使用3D姿态和姿态运动信息优化网格特征；提出双曲网格优化损失以确保学习稳定性。

Result: 在大型公开数据集上的广泛实验结果表明，该方法相比大多数最先进方法具有优越性。

Conclusion: 双曲空间学习能有效捕捉人体层次结构，结合时间运动先验可重建出准确平滑的3D人体网格。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大规模视觉语言模型在自然场景理解方面取得了显著成功，但在水下环境中的应用仍未被充分探索。水下图像存在严重的光衰减、颜色失真和悬浮粒子散射等独特挑战，且需要海洋生态系统和生物分类学的专业知识。

Method: 构建了包含15,003张高分辨率水下图像的UWBench数据集，涵盖海洋、珊瑚礁和深海栖息地等多样化水生环境。每张图像都包含人工验证的标注，包括15,281个物体指代表达和124,983个问答对。基于该数据集建立了三个基准：详细图像描述、视觉定位和视觉问答。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D人体网格恢复方法未能充分利用潜在信息（如人体运动、形状对齐），导致肢体错位和局部细节不足的问题，且基于注意力机制的网格顶点与姿态节点交互建模计算成本高昂。
...
Conclusion: 该方法通过有效挖掘潜在信息和设计低维交互策略，成功解决了现有方法在复杂场景下的肢体错位和细节不足问题，同时大幅降低了计算成本。
...
Method: 1. 因果时间缩减：基于相邻帧变化和token显著性选择token，对每帧施加固定预算，仅处理紧凑的视觉token子集而非全部；2. 在线量化内存：以4位格式存储token，按需检索相关组并反量化，保持活动kv缓存有界。

Result: 实现15.7倍kv缓存压缩，峰值内存降低1.2倍，TTFT速度提升2倍。在离线基准测试中平均准确率达63.8%，在RVS上达到55.8%/3.7，在无需训练的方法中保持最先进精度。
...
<details>
  <summary>Details</summary>
Motivation: 解决3D人脸身份保持编辑的挑战，包括多视角一致性、高分辨率编辑的灵活性，以及减少对大规模属性标注数据集的依赖。

Method: 利用3D感知深度生成模型和2D肖像编辑技术，通过少量标记图像估计潜在空间编辑方向，使用现有带掩码的人脸数据集生成合成图像。

Result: 实验结果表明，仅需10个或更少的标记图像即可实现3D感知属性编辑，并展示了编辑的线性特性，通过顺序编辑实现一次性风格化。

Conclusion: 该方法能够高效地进行少样本身份保持的3D人脸编辑，减轻了对大规模标注数据的依赖，并保持了编辑的3D一致性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的单目深度估计方法在相对深度预测方面表现出色，但由于单图像场景中的尺度模糊性，估计绝对度量深度仍然具有挑战性。

Method: 将深度估计重新定义为逆问题，利用预训练的潜在扩散模型结合RGB图像条件，并引入立体几何约束来学习尺度和位移参数，实现准确的深度恢复。

Result: 广泛的实验表明，该方法在室内、室外和复杂环境中都能匹配或超越最先进的方法，特别是在涉及半透明和镜面表面的挑战性场景中表现优异。

Conclusion: 该训练免费的解决方案能够无缝集成到现有的基于扩散的单目深度估计框架中，无需重新训练即可显著提升度量深度估计的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学MLLMs仅依赖内部知识进行推理，在遇到超出训练范围的病例时会产生幻觉推理和事实错误。虽然最近出现了代理检索增强生成方法，但仅限于单模态LLMs，忽略了推理和检索过程中的关键视觉信息。

Method: 设计了两阶段强化学习策略，通过定制奖励刺激模型同时利用视觉诊断发现和文本临床信息进行有效检索。进一步提出置信度驱动的图像重检索方法，在检测到低预测置信度时进行测试时扩展。

Result: 在各种公共医学基准测试中，Med-RwR相比基线模型有显著改进，证明了通过外部知识整合增强推理能力的有效性。在提出的超声心动图基准测试中，尽管训练语料中超声心动图数据稀缺，仍实现了8.8%的性能提升。

Conclusion: Med-RwR框架通过主动检索外部知识有效增强了多模态大语言模型的医学推理能力，并展现出对不熟悉领域的显著泛化能力。
...
Method: 通过实验研究不同分辨率训练和推理对MLLM性能的影响，包括原生分辨率、分辨率不匹配和混合分辨率训练策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶世界模型存在状态模态有限、视频序列短、动作控制不精确和缺乏奖励意识等问题，需要开发一个能同时解决这三个维度的统一框架。
...
Conclusion: OmniNWM成功构建了一个全面的自动驾驶世界模型，统一解决了状态、动作和奖励三个核心维度的问题，为自动驾驶系统提供了有效的仿真和评估平台。
...
Method: ATED是一个训练免费的令牌级集成框架，在推理过程中动态计算基于不确定性的权重来反映每个模型在每个解码步骤的可靠性，并集成不同的解码路径以提高上下文基础和语义一致性。

Result: 在标准幻觉检测基准测试中，ATED显著优于现有最先进方法，在不影响流畅性或相关性的情况下减少了幻觉。

Conclusion: 研究结果突出了自适应集成的优势，并为提高大型视觉语言模型在高风险应用中的鲁棒性指明了有前景的方向。
...
TL;DR: 本文提出了ATTBHFA-Net，一种基于注意力机制的Bhattacharyya-Hellinger特征聚合网络，通过结合Bhattacharyya系数和Hellinger距离来比较和聚合特征概率分布，以解决灾难图像分类中数据稀缺和类间相似性的问题。


<details>
  <summary>Details</summary>
Motivation: 灾难场景下的视觉识别面临数据稀缺、类内变化大和类间相似性高的挑战，而现有的少样本学习方法主要基于通用基准数据集，缺乏针对遥感灾难图像的有效解决方案。

Method: 提出ATTBHFA-Net网络，线性结合Bhattacharyya系数和Hellinger距离来比较特征概率分布，Bhattacharyya系数增强类间可分性，Hellinger距离正则化同类对齐，并提出了基于Bhattacharyya-Hellinger距离的对比损失函数。

Result: 在四个少样本学习基准和两个灾难图像数据集上的实验表明，ATTBHFA-Net相比现有方法具有优越的有效性和泛化能力。

Conclusion: ATTBHFA-Net通过概率分布层面的对比学习机制，有效解决了灾难图像少样本分类中的挑战，为灾难救援中的视觉识别提供了有力工具。
...
TL;DR: 提出了一种用于自动驾驶街景外推的四阶段管道，包括伪LiDAR初始化、2D-SDF几何先验、生成伪真值监督和数据驱动适配网络，在RealADSim-NVS基准测试中获得第一名。


<details>
  <summary>Details</summary>
Motivation: 解决当前新视角合成方法在原始轨迹之外产生扭曲和不一致图像的问题，实现逼真的街景外推，为自动驾驶闭环仿真提供支持。

Method: 四阶段管道：1) 数据驱动的伪LiDAR点云初始化；2) 使用降维2D-SDF建模道路表面的几何先验；3) 利用生成先验为外推视角创建伪真值监督；4) 数据驱动适配网络去除时间特定伪影。

Result: 在RealADSim-NVS基准测试中获得最终得分0.441，在所有参与者中排名第一。

Conclusion: 该方法通过结合几何先验和生成先验，有效解决了街景外推的挑战，在自动驾驶仿真中实现了高质量的视图外推效果。
...
TL;DR: 提出了一种基于大规模网络数据的生成式预训练模型，用于面部知识学习，通过自监督任务训练，在面部下游任务和编辑任务中表现优异。
...
Method: 利用网络爬取的人脸图像和文本进行预训练，采用掩码图像/语言建模和图像-文本匹配等自监督任务，在生成阶段使用图像-文本匹配损失进行可控生成。

Result: 模型在属性分类和表情识别等面部下游任务中达到与最先进预训练模型相当的性能，并适用于面部属性编辑、表情操控、口罩移除和图像修复等多种编辑任务。

Conclusion: 该方法通过利用大规模网络数据解决了面部预训练模型的数据标注瓶颈，展示了在面部理解和生成任务中的良好泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本学习方法将深度视觉特征构成的视觉空间作为嵌入空间，但实例在视觉空间中的离散分布使得数据结构不明显。作者认为优化视觉空间对于更有效地嵌入语义向量至关重要。

Method: 提出了两种策略：1）视觉原型方法，为每个视觉类别学习一个视觉原型，用原型特征代替离散视觉特征；2）在中间嵌入空间中优化视觉特征结构，设计了一个基于多层感知机的框架算法来学习共同中间嵌入空间并使视觉数据结构更显著。

Result: 在四个基准数据集上的广泛实验评估表明，优化视觉空间对零样本学习有益，提出的基于原型的方法实现了新的最先进性能。

Conclusion: 优化视觉空间对零样本学习至关重要，视觉原型方法能够有效提升零样本学习的性能，达到了新的最佳效果。
...
Method: 引入动态自适应焦点采样机制逐步关注与问题最相关的音视频片段；提出偏好感知策略独立建模各模态贡献；使用双路径对比损失强化跨维度一致性和互补性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的DPO方法虽然避免了REINFORCE算法提高了训练稳定性，但仍面临sigmoid函数非线性估计不准确和离线数据集多样性有限的挑战。

Method: Diffusion-DRO基于逆强化学习，将偏好学习转化为排序问题，简化训练目标为去噪公式，并整合离线专家演示和在线策略生成的负样本。

Result: 实验表明Diffusion-DRO在多种挑战性和未见提示上提升了生成质量，在定量指标和用户研究中均优于现有最佳基线方法。
...
TL;DR: GroupHOI是一个用于人类-物体交互检测的框架，通过几何邻近性和语义相似性来传播上下文信息，解决了现有方法忽视群体行为的问题。
...
Method: 使用可学习的邻近性估计器根据边界框的空间特征将人类和物体分组，在每个组内通过自注意力计算软对应关系来聚合和分发上下文线索，并在基于transformer的交互解码器中增强局部上下文线索。
...
TL;DR: FeatureFool是一种针对视频模型的零查询黑盒攻击方法，通过利用DNN提取的特征信息直接改变干净视频的特征空间，无需与模型进行多轮交互。


<details>
  <summary>Details</summary>
Motivation: 现有黑盒对抗攻击需要多轮模型交互和大量查询，这在现实世界中不实用且难以扩展到新兴的Video-LLMs。视频领域缺乏直接利用特征映射来改变干净视频特征空间的攻击方法。

Method: 提出FeatureFool攻击方法，通过从DNN中提取信息来改变干净视频的特征空间，实现零查询攻击，无需依赖迭代交互。

Result: FeatureFool对传统视频分类器的攻击成功率超过70%，且无需任何查询。利用特征映射的迁移性，还能生成有害内容绕过Video-LLM识别。生成的对抗视频在SSIM、PSNR和时间一致性方面质量很高，攻击几乎不可察觉。
...
Result: 在多个ICA数据集上的实验表明，CM-SSA显著优于最先进的方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗性剪枝方法在分数空间优化时容易陷入尖锐局部最小值，导致掩码选择不稳定，降低模型鲁棒性
...
TL;DR: 该研究提出了一种基于共形几何特征熵的局部表面表示方法，用于阿尔茨海默病的自动诊断，在ADNI数据集上达到98.62%的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病与显著的皮质萎缩相关，几何表面模型对于分析解剖结构的3D形状至关重要，需要开发准确诊断AD的新方法。

Method: 使用ADNI的160名参与者T1加权MRI数据，通过Freesurfer重建皮质表面模型，计算面积失真、共形因子和高斯曲率等几何属性，应用香农熵创建特征向量，训练多种分类器。

Result: MLP和逻辑回归分类器表现最佳，准确率和F1分数均达到98.62%，统计显著性检验证实了性能差异。

Conclusion: 共形几何特征的熵为皮质形态测量提供了强大而稳健的度量，高分类准确率表明该方法在AD研究和诊断中具有重要应用潜力。
...
TL;DR: 提出贝叶斯全连接张量网络方法，通过概率框架和层次稀疏先验解决传统张量分解方法在超光谱-多光谱图像融合中的结构破坏和参数调优问题


<details>
  <summary>Details</summary>
Motivation: 现有张量分解方法存在数据向量化破坏空间-光谱结构、因子张量排列约束严格、需要大量手动参数调优以及对噪声和空间退化鲁棒性有限等问题

Method: 提出贝叶斯FCTN方法，使用层次稀疏先验建立因子张量间联系，通过变分贝叶斯推理和EM算法进行参数估计

Result: 在广泛实验中，BFCTN实现了最先进的融合精度和强鲁棒性，在复杂实际场景中具有实用适用性

Conclusion: BFCTN方法有效解决了传统张量分解方法的局限性，在保持空间-光谱结构的同时显著减少了手动参数调优需求
...
Result: 场地检测模型达到99.5% mAP50精度和0.999精确率；球体检测模型通过迁移学习达到99.18% mAP50、0.968精确率和0.978召回率；成功在多个板球比赛视频中实现轨迹建模。
...
TL;DR: ScaleNet是一种高效的ViT模型扩展方法，通过在预训练模型基础上插入共享参数的附加层，实现快速模型扩展，显著减少训练成本。
...
Method: 收集真实用户交互数据构建ImageGem数据集，利用用户偏好标注训练偏好对齐模型，研究检索模型和视觉语言模型在个性化图像检索和生成模型推荐中的性能，并提出在潜在权重空间中编辑定制扩散模型以对齐个人用户偏好的端到端框架。

Result: 使用ImageGem数据集中的用户偏好标注能够训练出更好的偏好对齐模型，并为个性化图像检索和生成模型推荐提供了性能评估基准。
...
TL;DR: RISE是一个检索自增强的伪装目标检测范式，通过利用整个训练数据集为单个图像生成伪标签来训练COD模型，无需人工标注。


<details>
  <summary>Details</summary>
Motivation: 现有COD方法主要依赖图像级建模或基于标注的优化，难以利用数据集级上下文信息或需要大量人工标注工作。

Method: 使用聚类-检索策略构建环境和伪装目标的原型库，然后通过多视图KNN检索生成鲁棒的伪掩码。

Result: 在多个数据集上的实验表明，RISE在无监督和基于提示的方法中达到了最先进的性能。

Conclusion: RISE证明了利用数据集级信息生成高质量伪标签的有效性，为无监督COD提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法生成3D CT扫描计算成本高，需要开发能够在单GPU上生成高质量256x256x256体积图像的方法，同时通过解剖掩码精确控制输出特征。

Method: 使用潜在扩散模型，以3D解剖掩码（肺部和结节区域）为条件，在单中端GPU上生成1mm各向同性分辨率的体积图像。

Result: 实验表明仅使用结节掩码会导致解剖学上不正确的输出，强调需要结合全局肺部结构进行准确的条件合成。该方法支持生成具有不同属性肺结节或无结节的多样化CT体积。

Conclusion: 该方法为训练AI模型或医疗专业人员提供了有价值的工具，能够生成高质量、计算成本低的3D胸部CT扫描，同时精确控制解剖特征。
...
TL;DR: 提出VFM-VAE直接集成视觉基础模型到潜在扩散模型中，避免了蒸馏方法的语义偏差问题，通过多尺度潜在融合和渐进分辨率重建实现高质量重建，并引入SE-CKNNA指标加速训练收敛。
...
Method: 设计VFM-VAE变分自编码器，采用多尺度潜在融合和渐进分辨率重建模块，引入SE-CKNNA分析表示动态，开发联合tokenizer-diffusion对齐策略。

Result: 仅用80个epoch达到gFID 2.20（10倍加速），640个epoch后达到gFID 1.62，性能显著优于现有方法。

Conclusion: 直接VFM集成是潜在扩散模型的更优范式，在性能和效率上都表现出色。
...
Method: 基于高斯泼溅的两阶段优化框架：第一阶段在正交相机坐标系学习视频HDR高斯表示，第二阶段将视频高斯转换到世界空间并联合优化世界高斯和相机位姿，同时提出时间亮度正则化策略增强HDR外观的时间一致性。

Result: 在新建的评估基准上，Mono4DGS-HDR在渲染质量和速度上显著优于现有方法改编的替代方案。
...
TL;DR: 提出了一种结合视觉语言模型和检索增强生成的方法，用于零样本车辆品牌型号识别，避免大规模重新训练，通过文本推理实现对新车型的快速适应。
...
Method: 集成视觉语言模型与检索增强生成：VLM将车辆图像转换为描述性属性，与文本特征数据库进行比较检索相关条目，结合描述形成提示，语言模型推理出品牌型号。

Result: 实验表明，该方法比CLIP基线提高了近20%的识别率。

Conclusion: RAG增强的语言模型推理在智能城市应用中具有可扩展VMMR的潜力，能够避免大规模重新训练并通过添加新车型文本描述实现快速更新。
...
TL;DR: 开发了DWaste平台，这是一个基于计算机视觉的实时垃圾分类系统，可在资源受限的智能手机和边缘设备上运行，支持离线功能。研究发现轻量级目标检测模型在准确性和资源消耗之间达到最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 便利包装的普及导致大量垃圾产生，高效的垃圾分类对于可持续垃圾管理至关重要。需要开发能在资源受限设备上实时运行的垃圾分类解决方案。
...
Result: EfficientNetV2S分类器准确率最高（约96%），但延迟高（约0.22秒）且碳排放高；轻量级目标检测模型性能良好（最高77% mAP），推理速度极快（约0.03秒），模型尺寸小（<7MB）。模型量化可减少75%的模型大小和VRAM使用。

Conclusion: 成功实现了"更环保AI"模型，支持在边缘设备上进行实时、可持续的垃圾分类，轻量级目标检测模型在准确性和效率方面表现最佳。
...
TL;DR: 提出了一种基于射线对齐的模板物体姿态估计方法，使用扩散变换器架构将无姿态查询图像与带姿态模板对齐，在多个基准数据集上取得有竞争力的结果。


<details>
  <summary>Details</summary>
Motivation: 传统基于模板的物体姿态估计方法在检索错误模板时会导致姿态预测不准确，需要改进模板匹配机制。

Method: 将姿态估计重新表述为射线对齐问题，使用扩散变换器架构学习多姿态模板的观察方向与非姿态查询图像的对齐，采用物体中心相机射线重新参数化旋转，并扩展尺度不变平移估计到密集平移偏移。

Result: 在多个基准数据集上的广泛实验表明，该方法在未见物体姿态估计方面与最先进方法相比具有竞争力。

Conclusion: 提出的射线对齐公式和扩散变换器架构能够有效利用模板的几何先验来指导准确的查询姿态推断，无需修改网络架构的由粗到精训练策略进一步提升了性能。
...
Method: 使用GBlobs作为网络输入特征，这是一种专门设计的局部点云特征描述符，能够有效规避几何捷径，迫使网络学习鲁棒的对象中心表示。

Result: 该方法显著提升了模型的泛化能力，在挑战中展现了卓越的性能表现。

Conclusion: GBlobs特征描述符通过消除位置偏差，成功解决了3D目标检测中的几何捷径问题，为不同LiDAR配置下的泛化检测提供了有效解决方案。
...
TL;DR: 提出了Occluded nuScenes数据集，这是nuScenes基准的扩展，为相机、雷达和LiDAR提供参数化遮挡，支持在可控和可重复的恶劣条件下评估感知模型。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶数据集虽然包含传感器噪声和环境变化，但缺乏可控、参数化和可重复的多模态传感器退化，限制了系统评估感知和融合架构在明确定义恶劣条件下的性能。

Method: 为相机模态发布了完整版和迷你版，包含四种遮挡类型（两种来自公共实现，两种新设计）；为雷达和LiDAR提供了参数化遮挡脚本，每种实现三种退化类型，支持灵活可重复生成遮挡数据。
...
TL;DR: Kaleido是一个主题到视频生成框架，通过专门的数据构建流程和参考旋转位置编码技术，解决了多主题一致性和背景解耦问题，显著提升了视频生成的质量。
...
Method: 提出了专门的数据构建流程，包括低质量样本过滤和多样化数据合成，以及参考旋转位置编码技术来稳定处理多参考图像。

Result: 在多个基准测试上的广泛实验表明，Kaleido在一致性、保真度和泛化能力方面显著优于先前的方法。

Conclusion: Kaleido在主题到视频生成领域取得了重要进展，为解决多主题一致性和背景解耦问题提供了有效解决方案。
...
TL;DR: 提出CovMatch框架，通过跨协方差对齐和模态内特征分布正则化，实现可扩展的多模态数据集蒸馏，在仅使用500个合成图像-文本对的情况下显著提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法为降低计算成本而冻结文本编码器，但这严重限制了跨模态语义对齐，成为性能扩展的瓶颈。

Method: CovMatch框架同时优化两个编码器，通过跨协方差对齐真实和合成特征，并在每个模态内正则化特征分布。

Result: 在Flickr30K和COCO数据集上，CovMatch优于现有多模态蒸馏方法，使用500个合成对时检索准确率绝对提升达6.8%。

Conclusion: CovMatch通过联合优化编码器和跨模态对齐策略，实现了更强大的跨模态对齐和性能提升，解决了多模态数据集蒸馏的关键挑战。
...
TL;DR: 3DThinker是一个无需3D先验输入就能在推理过程中进行3D心理建模的框架，通过两阶段训练有效利用图像中的几何信息进行3D空间关系推理。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在3D空间关系理解方面存在局限，主要依赖文本或2D视觉线索，难以处理需要3D空间想象的任务。
...
Conclusion: 3DThinker框架首次实现了无需3D先验输入的3D心理建模推理，在3D空间关系理解方面取得了显著进展。
...
Result: 在CNN和ViT基准模型上的实验表明，该方法能显著减少模型大小，对性能影响最小，且无需微调。

Conclusion: 该方法在模型压缩与性能保持之间提供了最佳权衡，优于现有方法。
...
TL;DR: ε-Seg是一种基于分层变分自编码器的语义分割方法，专为电子显微镜生物图像设计，能够在极稀疏标签（仅0.05%图像数据）下实现竞争性分割性能。


<details>
  <summary>Details</summary>
Motivation: 电子显微镜图像中的生物结构语义分割在生命科学中仍具挑战性，数据复杂度高，即使人类观察者也难以处理。需要开发能在极稀疏标签下有效工作的分割方法。

Method: 采用分层变分自编码器，结合中心区域掩码、稀疏标签对比学习、高斯混合模型先验和无聚类标签预测。中心区域掩码和修复损失学习鲁棒嵌入，对比学习和GMM先验塑造潜在空间，MLP分割头直接预测类别标签。

Result: 在2个密集EM生物组织数据集上展示了竞争性稀疏监督分割结果，并在荧光显微镜数据上验证了方法适用性，在有限训练标签下仍能有效工作。

Conclusion: ε-Seg能够在极稀疏标签条件下，对复杂生物图像数据实现竞争性的语义分割性能，为解决生物图像分析中的标签稀缺问题提供了有效方案。
...
TL;DR: 提出了一种新的矩阵量化方法BQQ，使用二元二次表达式替代传统的一阶量化方法，在保持紧凑数据格式的同时提高表达能力。在矩阵压缩和视觉Transformer后训练量化中表现出色。
...
Result: 在矩阵压缩基准测试中，BQQ在内存效率和重构误差之间实现了优于传统方法的平衡。在后训练量化中，在ImageNet数据集上，2位量化下比最先进方法分别提升2.2%（基于校准）和59.1%（数据无关场景）。

Conclusion: 二元二次表达式在高效矩阵近似和神经网络压缩方面具有显著效果，BQQ方法展示了其在保持紧凑格式的同时提供强大表达能力的优势。
...
<details>
  <summary>Details</summary>
Motivation: 传统变化检测方法需要大量标注数据，而主动学习能够通过少量用户交互获得高质量标注，但现有方法在样本增强方面存在局限性。

Method: 使用可逆网络将非线性输入空间映射到潜在空间，在潜在空间进行线性数据增强，然后将增强后的数据映射回输入空间用于训练变化检测模型。

Result: 实验结果表明，该方法相比相关工作具有更优越的性能表现。

Conclusion: 提出的可逆网络增强方法有效提升了主动学习框架下卫星图像变化检测的准确性和效率。
...
TL;DR: 本文研究了影响历史笔迹识别端到端深度学习方法的多种因素，发现大多数模型在零样本场景下泛化能力差，但找到了一种简单设计能达到顶尖性能的配置。


<details>
  <summary>Details</summary>
Motivation: 历史笔迹识别面临笔迹风格多样性、文档退化和标记样本有限等挑战，传统方法依赖手工特征提取，而端到端方法在真实文档级场景中泛化能力不足。

Method: 探索了不同的预处理方法、骨干网络架构和后处理策略组合，包括文本分割、补丁采样和特征聚合等技术。

Result: 大多数配置因低层视觉特征捕捉弱、补丁表示不一致和对内容噪声敏感而表现不佳，但发现一种端到端设置能达到与顶尖系统相当的结果。

Conclusion: 研究揭示了构建鲁棒端到端系统的关键挑战，并为历史文档笔迹识别的设计选择提供了重要见解。
...
TL;DR: 本文提出Mixture-of-Groups Attention (MoGA)，一种高效的稀疏注意力机制，通过轻量级可学习的token路由器精确匹配token，解决了DiT在生成长视频时因全注意力二次方复杂度带来的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 基于DiT的长视频生成受限于全注意力机制的二次方复杂度扩展，而注意力本身具有高度冗余性，输出主要由少量query-key对主导。现有稀疏方法依赖分块粗估计，其精度-效率权衡受限于块大小。

Method: 提出MoGA稀疏注意力机制，使用轻量级可学习的token路由器精确匹配token，无需分块估计。通过语义感知路由实现有效的长程交互，作为无核方法可与现代注意力栈（如FlashAttention和序列并行）无缝集成。

Result: 基于MoGA开发了高效的长视频生成模型，能够端到端生成分钟级、多镜头、480p、24fps的视频，上下文长度约58万token。在各种视频生成任务上的综合实验验证了方法的有效性。

Conclusion: MoGA通过精确的token路由机制有效解决了长视频生成的注意力瓶颈，实现了高效的长序列处理能力，为长视频生成提供了可行的解决方案。
...
TL;DR: UniGenBench++是一个统一的文本到图像生成语义评估基准，包含600个分层组织的提示，涵盖5个主要主题和20个子主题，提供中英文长短版本，通过多模态大语言模型进行可靠评估。
...
Method: 构建包含600个提示的分层基准，涵盖5个主要主题和20个子主题，提供中英文长短版本，利用Gemini-2.5-Pro多模态大语言模型开发评估流程，并训练离线评估模型。

Result: 通过对开源和闭源文本到图像模型的全面基准测试，系统揭示了它们在不同方面的优势和弱点。

Conclusion: UniGenBench++为文本到图像生成提供了全面、多语言、细粒度的语义评估基准，有助于系统评估模型性能。
...
TL;DR: 提出了VC2L框架，使用单一视觉Transformer在像素空间处理文本、图像及其组合，无需OCR或模态融合策略，通过片段级对比学习对齐多模态文档片段。


<details>
  <summary>Details</summary>
Motivation: 现有对比视觉语言模型在处理复杂网页文档时能力有限，特别是当文本和图像交错、松散对齐或以视觉形式嵌入时。

Method: VC2L将所有输入渲染为图像，在像素空间统一处理，采用片段级对比学习目标对齐连续多模态片段，利用文档固有连贯性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于transformer的动作识别方法在场景相关数据集上表现良好，但在运动敏感数据集上表现不佳，因为缺乏精细的运动建模设计。观察到传统动作识别中广泛使用的cost volume与自注意力中的亲和矩阵高度相似，但具有更强的运动建模能力。

Method: 提出显式运动信息挖掘模块(EMIM)，以cost volume风格构建理想的亲和矩阵，其中关键候选标记集以滑动窗口方式从下一帧中基于查询的相邻区域采样。构建的亲和矩阵用于聚合上下文信息进行外观建模，并转换为运动特征进行运动建模。

Result: 在四个广泛使用的数据集上验证了方法的运动建模能力，特别是在运动敏感数据集Something-Something V1 & V2上优于现有最先进方法。

Conclusion: 通过将cost volume的有效运动建模特性以统一简洁的方式集成到现有transformer中，显著提升了动作识别模型在运动敏感任务上的性能。
...
TL;DR: PLANA3R是一个无需相机位姿的度量三维重建框架，使用平面基元从无位姿的双视图图像重建室内场景，无需3D平面标注即可学习平面结构。


<details>
  <summary>Details</summary>
Motivation: 利用室内场景固有的几何规律性，通过紧凑的平面基元表示来解决度量三维重建问题，避免对3D平面标注的依赖。

Method: 使用视觉Transformer提取稀疏平面基元，估计相对相机位姿，通过平面渲染进行几何学习监督，在高分辨率深度和法线图上传播梯度。

Result: 在多个室内场景数据集上验证了方法的有效性，展示了在跨域室内环境中的强泛化能力，支持3D表面重建、深度估计和相对位姿估计等任务。

Conclusion: 基于平面3D表示的方法不仅能实现准确的度量重建，还具备精确平面分割的能力，为大规模室内场景重建提供了可扩展的解决方案。
...
TL;DR: 提出了一种针对自回归图像生成模型的KV缓存压缩框架，通过识别空间局部性和语义汇聚现象，将注意力头分为两类进行自适应压缩，显著降低了内存使用和计算需求。


<details>
  <summary>Details</summary>
Motivation: 自回归图像生成模型如Janus-Pro虽然能生成高质量图像，但由于大量视觉令牌导致内存占用高且计算需求不断增长。虽然KV缓存压缩在语言建模中已被广泛研究，但在图像生成领域仍未被充分探索。

Method: 识别了空间局部性和涌现语义汇聚现象，提出将注意力头自适应解耦为两种类型：对于空间局部性头，保持短期的最近令牌窗口；对于语义汇聚头，策略性地保留一组高度关注的紧凑令牌集。

Result: 实验表明该方法实现了5倍的内存使用减少和6.6倍的总体吞吐量加速，仅带来最小的视觉质量损失。

Conclusion: 该方法使得在资源受限的硬件上实现高效的原生自回归图像生成成为可能。
...
TL;DR: 提出了ColIAGS框架，改进3D高斯泼溅技术用于结肠镜检查，通过改进的外观建模和几何建模解决动态光照问题，提升渲染质量和几何重建精度


<details>
  <summary>Details</summary>
Motivation: 传统3DGS假设静态光照，无法处理结肠镜场景中的动态光照变化，导致重建质量下降，需要改进光照建模方法

Method: 引入改进的外观建模包含两种光照衰减因子，使用高维视图嵌入增强几何属性预测，利用余弦嵌入隐式生成光照衰减解决方案

Result: 在标准基准测试中，ColIAGS在新视角合成和几何重建方面表现优异，显著降低深度MSE，渲染保真度优于现有方法

Conclusion: ColIAGS框架成功解决了结肠镜场景中的动态光照问题，实现了高质量的3D重建和视图合成
...
TL;DR: 提出SEAL框架，利用自然层次结构解决广义类别发现问题，通过层次语义引导的软对比学习和跨粒度一致性模块，在细粒度数据集上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有广义类别发现方法依赖单层语义或手动设计的抽象层次结构，限制了泛化性和可扩展性。
...
Conclusion: SEAL通过利用自然层次结构有效解决了广义类别发现问题，显著提升了性能表现。
...
TL;DR: 本研究利用地理空间基础模型评估城市热岛效应，通过微调模型预测未来气候情景下的地表温度，并展示其在数据稀缺地区支持气候适应性城市规划的潜力。
...
Method: 建立城市热模式的实证基准，量化绿地的降温效应，并以此评估模型准确性。随后对基础模型进行微调，预测未来气候情景下的地表温度，并通过模拟修复展示其实际应用价值。

Result: 基础模型能够准确评估城市热岛缓解策略，在数据稀缺地区表现出强大的预测能力，为气候适应性城市规划提供支持。

Conclusion: 地理空间基础模型为评估城市热岛缓解策略提供了有力工具，特别是在数据稀缺地区，有助于建设更具气候适应性的城市。
...
Conclusion: UltraGen首次实现了原生高分辨率视频生成，通过创新的注意力架构设计解决了计算瓶颈问题，为高分辨率视频生成提供了有效解决方案。
...
TL;DR: 提出了一种新颖的"叛逆学生"框架，通过故意让空间分支与光谱教师网络产生分歧，学习互补的空间特征，实现无需场景重训练的超光谱异常检测。


<details>
  <summary>Details</summary>
Motivation: 现有超光谱异常检测方法需要针对每个场景重新训练或调整参数，效率较低。本文旨在开发一种能够一次性训练、通用部署的方法，通过整合光谱和空间线索来提高检测性能。

Method: 采用两阶段学习策略：首先通过反向蒸馏训练光谱增强网络获取背景光谱表示；然后训练空间网络（叛逆学生）使用解相关损失强制特征正交性，同时保持重建保真度以避免无关噪声。

Result: 在HAD100基准测试上的广泛实验显示，相比多个基线方法有显著改进，且计算开销最小，验证了互补学习范式的有效性和通用性。

Conclusion: 提出的叛逆学生框架成功实现了光谱和空间背景特征的增强，当与传统检测器结合时，能够实现无需参数调整和重训练的异常检测，具有很好的实用价值。
...
TL;DR: ProCLIP提出了一种基于课程学习的渐进式视觉-语言对齐框架，通过知识蒸馏和对比学习将CLIP图像编码器与基于LLM的嵌入器对齐，解决了CLIP文本编码器输入长度限制和多语言支持不足的问题。


<details>
  <summary>Details</summary>
Motivation: 原始CLIP文本编码器存在77个token的最大输入长度限制，无法有效处理长文本和进行细粒度语义理解，且缺乏多语言支持，限制了其应用范围。

Method: 采用课程学习策略：1) 从CLIP文本编码器向LLM嵌入器进行知识蒸馏；2) 通过图像-文本对比调优进一步对齐，使用自蒸馏正则化防止过拟合；3) 在表示继承和对比调优阶段使用实例语义对齐损失和嵌入结构对齐损失。

Result: 该方法有效对齐了CLIP图像编码器与LLM嵌入器，避免了直接对比学习可能破坏CLIP固有视觉-语言对齐的问题，充分利用了预训练知识。

Conclusion: ProCLIP框架成功解决了CLIP在处理长文本和多语言任务时的局限性，同时保持了其原有的视觉-语言对齐能力。
...
TL;DR: 本文提供了一种基于几何论证和模式匹配基本原理的直观推导方法，用于构建d维可操纵卷积神经网络，并提出使用插值核的新实现方式。


<details>
  <summary>Details</summary>
Motivation: 许多现有论文采用抽象群论方法，本文旨在提供更直观的推导方式来理解可操纵卷积神经网络。

Method: 基于几何论证和模式匹配基本原理进行推导，提出使用插值核构建可操纵卷积层的新方法。

Result: 提供了对Clebsch-Gordan分解和球谐基函数出现的直观解释，新方法相比现有实现具有改进，对噪声数据更鲁棒。

Conclusion: 本文提供了一种更直观的推导方法，并通过插值核改进了可操纵卷积层的实现，提高了对噪声数据的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 结核病在资源有限和偏远地区仍是严重健康问题，早期检测对治疗至关重要，但缺乏专业放射科医生，需要AI驱动的筛查工具。开发可靠AI模型面临高质量数据集获取成本高的问题。
...
TL;DR: SAM 2++是一个统一的视频跟踪模型，能够在任意粒度（掩码、边界框、点）上进行目标跟踪，通过任务特定提示、统一解码器和任务自适应内存机制实现多任务统一。


<details>
  <summary>Details</summary>
Motivation: 现有跟踪器通常针对单一任务设计，依赖定制化模块，导致泛化能力有限且模型冗余。需要开发能够处理不同粒度跟踪任务的统一框架。

Method: 1) 设计任务特定提示将不同任务输入编码为通用提示嵌入；2) 使用统一解码器将多样任务结果统一为预输出形式；3) 引入任务自适应内存机制统一不同粒度的内存；4) 构建包含三个粒度丰富标注的大规模数据集Tracking-Any-Granularity。

Result: 在多个基准测试上的综合实验表明，SAM 2++在不同粒度的多样化跟踪任务中均达到了新的最先进水平。

Conclusion: SAM 2++建立了一个统一且鲁棒的跟踪框架，能够有效处理任意粒度的视频跟踪任务，解决了现有方法泛化能力有限和模型冗余的问题。
...
TL;DR: 提出统一的层次掩码框架，揭示模型架构与注意力掩码构建的等价性，并基于此开发M3Dphormer模型，通过多级掩码和双注意力计算实现可扩展的图Transformer。


<details>
  <summary>Details</summary>
Motivation: 现有图Transformer依赖特定交互的复杂架构设计，缺乏灵活性。需要统一框架来捕捉多样节点交互。

Method: 提出层次掩码框架，将架构设计转化为注意力掩码构造。开发M3Dphormer模型，包含三个理论基础的层次掩码、双级专家路由机制和基于局部稀疏度的双注意力计算方案。

Result: 在多个基准测试中达到最先进性能，验证了统一框架和模型设计的有效性。

Conclusion: 层次掩码框架为图Transformer提供了统一建模范式，M3Dphormer通过自适应集成多级交互信息实现了优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习中领域偏移和标签异质性阻碍全局模型的泛化，需要有效在多领域联邦设置中微调CLIP模型。
...
TL;DR: SeeTok提出了一种基于视觉的文本处理方法，将文本渲染为图像，利用预训练的多模态大语言模型进行解读，相比传统的子词分词方法，在多个语言任务中表现相当或更好，同时显著减少了计算开销。


<details>
  <summary>Details</summary>
Motivation: 人类通过视觉识别文字的形状、布局和模式来阅读，而现代大语言模型依赖子词分词，这种方法对低资源语言过度分割，产生冗长且无语言学意义的序列，增加了计算负担。

Method: SeeTok将文本渲染为图像（视觉文本），利用预训练的多模态大语言模型来解读这些图像，复用从大规模多模态训练中学到的OCR和文本-视觉对齐能力。

Result: 在三个不同的语言任务中，SeeTok与子词分词器表现相当或更好，同时需要4.43倍更少的token，减少70.5%的FLOPs，在跨语言泛化、对排版噪声的鲁棒性和语言层次结构方面有额外优势。

Conclusion: SeeTok标志着从符号化分词向人类视觉阅读的转变，朝着更自然和认知启发的语言模型迈出了一步。
...
TL;DR: DP²O-SR是一个用于真实图像超分辨率的框架，利用预训练文本到图像扩散模型的感知多样性，通过混合奖励信号和分层偏好优化来提升感知质量，无需人工标注。


<details>
  <summary>Details</summary>
Motivation: 利用T2I扩散模型的随机性产生的感知质量范围，通过偏好优化来提升真实图像超分辨率性能，避免昂贵的人工标注成本。

Method: 结合全参考和无参考图像质量评估模型构建混合奖励信号，构建多偏好对，采用分层偏好优化根据模型容量自适应调整训练策略。

Result: 在扩散和流式T2I骨干网络上实验表明，DP²O-SR显著提升感知质量，并在真实世界基准上具有良好的泛化能力。

Conclusion: DP²O-SR有效利用感知多样性，通过智能偏好优化策略显著提升真实图像超分辨率的性能，为生成模型对齐提供新思路。
...
Method: 构建DSI-Bench基准，包含九个解耦的观察者和物体运动模式，采用时空对称设计减少偏见，系统评估模型对自身运动和物体运动的推理能力。

Result: 评估了14个视觉语言模型和专家模型，发现关键局限性：模型经常混淆观察者和物体运动，存在语义偏见，无法准确推断动态场景中的相对关系。
...
TL;DR: GAR（Grasp Any Region）是一个区域级多模态大语言模型，通过RoI对齐特征重放技术，能够利用全局上下文进行精确感知、建模多提示交互，并实现高级组合推理。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在理解复杂场景时存在困难，需要细粒度分析细节和对象间关系。虽然区域级MLLMs是很有前景的方向，但先前方法通常孤立地理解给定区域，忽略了关键的全局上下文。

Method: 引入GAR模型，采用有效的RoI对齐特征重放技术，支持：(1)利用必要的全局上下文进行精确感知；(2)建模多个提示之间的交互；(3)实现高级组合推理来回答关于任何区域的特定自由形式问题。

Result: GAR-1B在DLC-Bench上超越DAM-3B +4.5，保持最先进的字幕能力；在GAR-Bench-VQA上甚至超越InternVL3-78B；零样本GAR-8B在VideoRefer-BenchQ上超越领域内VideoRefer-7B，显示其能力可轻松迁移到视频。

Conclusion: GAR通过结合全局上下文和多提示交互建模，实现了从被动描述到主动对话的范式转变，在区域级视觉理解方面表现出色，且能力可扩展到视频领域。
...
TL;DR: PRISM模型结合了基于分数的扩散预处理、动态图编码器和物理正则化预测头，用于多变量时间序列的长期预测，在六个基准测试中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 解决多变量时间序列长期预测中的三大挑战：(i) 对异质信号进行去噪，(ii) 跟踪时变的跨序列依赖关系，(iii) 在长预测周期内保持稳定性和物理合理性。

Method: 耦合基于分数的扩散预处理器、动态相关阈值图编码器，以及通过通用物理惩罚正则化的预测头。

Result: 在六个标准基准测试中，PRISM实现了持续的最先进性能，在MSE和MAE指标上取得了显著提升。

Conclusion: 证明了所诱导的视界动态在温和条件下的收缩性，并推导了图块的Lipschitz边界，解释了模型的鲁棒性。
...
Conclusion: 基于执行的验证比单纯依赖语义相似性为工具选择提供了更可靠的基础，能够在实际应用中实现更稳健的代理性能。
...
Result: 在XJTU-SY和PRONOSTIA数据集上，CARLE优于多种先进方法，特别是在动态条件下表现更佳。

Conclusion: CARLE框架在RUL预测中展现出优异的性能、鲁棒性和泛化能力，通过可解释性分析增强了模型的透明度和可信度。
...
TL;DR: 提出了一种物理感知深度学习框架，用于构建稀薄、含激波微喷嘴流动的快速准确代理模型


<details>
  <summary>Details</summary>
Motivation: 需要开发能够捕捉参数依赖性、嵌入物理知识并处理高梯度区域的深度学习模型来模拟复杂流动问题

Method: 集成三个关键组件：融合DeepONet算子学习架构、物理引导的特征空间（嵌入激波对齐坐标系）和两阶段课程策略（强调高梯度区域）

Result: 首先在经典粘性Burgers方程上验证了框架的有效性，该方程表现出平流陡化和激波状梯度

Conclusion: 该框架具有通用性和归纳偏置，能够有效处理含激波的复杂流动问题
...
<details>
  <summary>Details</summary>
Motivation: 深度学习开源模型激增，模型融合虽能结合各模型优势，但现有方法存在参数冲突问题，导致特定领域任务性能下降。

Method: 提出路由器框架MIN-Merging，选择性融合最重要神经元以减少参数冲突。
...
TL;DR: 提出了一种联邦遗忘方法用于大语言模型，通过任务特定适配器学习和分层合并策略来解决连续异构遗忘需求中的域间和域内干扰问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在现实应用中面临隐私安全和知识移除需求，但现有机器遗忘方法难以处理连续异构的遗忘需求以及涉及分散敏感数据的场景。

Method: 采用联邦遗忘框架，通过任务特定适配器学习解耦遗忘和保留过程，并使用分层合并策略来缓解目标冲突，实现稳健可适应的遗忘更新。

Result: 在WMDP、MUSE和TOFU基准测试上的综合实验表明，该方法能有效处理异构遗忘请求，同时相比基线方法保持强大的LLM效用。

Conclusion: 该方法为LLMs提供了一种可扩展且保护隐私的联邦遗忘解决方案，能够平衡遗忘和保留性能的困境。
...
TL;DR: 提出了一个统一的基准测试框架，用于系统评估长上下文LLM训练中的注意力机制优化方法，包括内核级优化和上下文并行策略。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer注意力机制存在二次计算和内存成本问题，阻碍长上下文训练。先前研究缺乏系统性评估，算子级比较不完整，上下文并行策略框架特定且性能分析不清晰。

Method: 构建统一基准测试框架，集成代表性注意力内核和上下文并行机制，提供模块化可扩展接口。评估维度包括注意力掩码模式和序列长度/分布式规模。

Result: 在最多96个GPU集群上进行全面实验，实现可重复比较，揭示方法特定权衡，为长上下文LLM训练中注意力机制的设计和部署提供实用指导。

Conclusion: 该基准测试填补了系统性评估空白，能够有效指导长上下文训练中注意力优化方法的选择和应用。
...
TL;DR: L-MoE将混合专家(MoE)与低秩适配(LoRA)相结合，提出了一种轻量级可端到端训练的框架，用任务专用的低秩适配器替代传统MoE中的密集前馈网络专家。


<details>
  <summary>Details</summary>
Motivation: 结合MoE架构的稀疏激活特性和LoRA的参数高效微调优势，创建一个既高效又模块化的语言模型框架。

Method: 使用轻量级门控网络动态组合多个LoRA适配器，通过可微分路由机制计算加权平均参数，实现端到端联合训练。

Result: 提出了L-MoE的数学框架，实现了参数高效的MoE模型，支持动态技能组合和端到端训练。

Conclusion: L-MoE为构建更高效、可扩展和专用的语言模型提供了新路径。
...
TL;DR: 该论文提出使用大语言模型(LLMs)自动生成针对自动性能调优问题的优化算法，通过提供问题描述和搜索空间特征来生成专门的优化策略，并在多个真实应用中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统自动调优依赖固定的优化算法，但不同调优任务需要不同的优化策略，设计通用有效的优化器具有挑战性。

Method: 开发了一个框架，通过向LLMs提供问题描述和搜索空间特征来生成专门的优化算法，并进行迭代改进。

Result: 在4个真实应用和6个硬件平台上评估，LLM生成的优化器平均性能提升30.7%和14.6%，最佳生成算法平均比现有最优优化器提升72.4%。

Conclusion: LLM生成的优化算法能够匹敌甚至超越人工设计的算法，为自动性能调优提供了新的有效方法。
...
Conclusion: SBVFL使得在医疗、金融、制造、航空航天、网络安全和国防等敏感领域的实用、隐私保护的VFL成为可能。
...
<details>
  <summary>Details</summary>
Motivation: 为地球观测领域的神经压缩和表示学习提供标准化评估框架，解决预训练偏差问题，促进社区驱动的评估标准发展。

Method: 基于固定大小的任务无关嵌入表示，构建包含评估流程、隐藏任务挑战模式和评分系统的基准框架，并发布SSL4EO-S12-downstream数据集支持可复现性。

Result: 在2025年CVPR EARTHVISION研讨会上进行了公开挑战，并与最先进的基础模型进行了消融实验，验证了框架的有效性。

Conclusion: NeuCo-Bench为地球观测及其他领域的神经嵌入标准化评估迈出了重要一步，提供了社区驱动的评估解决方案。
...
TL;DR: 提出了一种基于预测可靠性评估的后处理校准框架，通过语义相似性将样本分为正确和错误两组，分别应用标准保序回归和欠置信正则化保序回归，在无需重新训练模型的情况下同时提升校准质量和不确定性感知决策能力。
...
Method: 使用基于邻近性的保形预测按特征空间语义相似性将校准样本分层为推定正确和推定错误两组，对正确预测应用标准保序回归，对错误预测应用欠置信正则化保序回归以降低置信度。

Result: 在CIFAR-10和CIFAR-100数据集上的实验表明，该方法减少了自信错误预测，在期望校准误差方面与保序回归和焦点损失基线具有竞争力。

Conclusion: 该工作通过实例级自适应性连接了校准和不确定性量化，提供了一种实用的后处理解决方案，无需模型重新训练即可同时改善概率对齐和不确定性感知决策。
...
TL;DR: 提出了一种针对扩散模型的数据遗忘方法，通过选择性关注特定时间-频率范围来改善遗忘效果和生成质量，避免了传统方法在所有时间步平均遗忘导致的质量下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型数据遗忘方法往往导致生成质量下降或不完全遗忘，这是因为它们在所有扩散时间步上平均进行遗忘操作，而实际上遗忘在不同时间和频率上的影响是不均衡的。

Method: 采用时间-频率选择性方法，根据模型和场景选择性地在特定时间-频率范围内进行训练，将这种方法应用于基于梯度的和偏好优化的目标，以及图像级和文本到图像任务。

Result: 通过选择性时间-频率方法，实现了更高美学质量和更低噪声的样本生成，在多种设置下验证了改进效果。

Conclusion: 该方法为扩散模型数据遗忘提供了更清晰的理解和实用策略，同时改进了评估和遗忘性能。
...
TL;DR: COMPASS是一个无需外部监督的测试时奖励机制，通过双校准答案奖励和决定性路径奖励来增强语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习方法依赖人工标注的偏好数据或标记数据集，存在可扩展性瓶颈，需要探索在无标签数据上的自主学习。

Method: COMPASS包含两个互补组件：DCAR通过置信度和可信度校准建立可靠的伪标签，DPR直接优化推理过程质量。

Result: 实验表明COMPASS在多样化推理任务和模型架构上均取得显著且一致的性能提升。

Conclusion: COMPASS为大语言模型从连续经验中学习提供了更可扩展的方向。
...
TL;DR: 提出了Param-CPD框架，通过神经后验估计将混沌时间序列映射到参数空间，然后在参数轨迹上应用标准变化点检测算法，显著提高了检测性能。


<details>
  <summary>Details</summary>
Motivation: 在混沌时间序列中检测机制转换很困难，因为观测空间信号与内在变异性纠缠在一起，需要找到更清晰的检测信号。

Method: 两阶段框架：首先使用基于模拟推理训练的神经后验估计器对控制参数进行贝叶斯推断，然后在生成的参数轨迹上应用标准变化点检测算法。

Result: 在Lorenz-63模型中，相比观测空间基线方法，Param-CPD提高了F1分数，减少了定位误差，降低了误报率，并在容忍度、窗口长度和噪声方面表现出稳健性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态理解和生成模型通常独立发展，缺乏统一的强化学习框架来协同提升理解与生成能力，并促进它们之间的交互。

Method: 提出了统一的强化学习框架UniRL-Zero，定义了六种统一模型强化学习场景，为统一理解和生成模型的强化学习提供系统性基准。

Result: 开发了一个统一的模型框架，能够同时提升多模态理解和多媒体生成能力，并促进两者之间的有益交互。

Conclusion: UniRL-Zero为统一理解和生成模型的强化学习提供了系统性的基准框架，有望推动多模态AI模型的协同发展。
...
TL;DR: 本文分析了Transition Matching(TM)相比Flow Matching(FM)的优势条件，证明在单峰高斯分布中TM能达到更低的KL散度，在计算预算固定时收敛更快，在模式分离良好的高斯混合分布中TM表现更优。


<details>
  <summary>Details</summary>
Motivation: 虽然Flow Matching是许多先进生成模型的基础，但最近研究表明Transition Matching能用更少的采样步骤达到更高质量。本文旨在回答TM何时以及为何优于FM的问题。

Method: 首先在单峰高斯分布场景下理论证明TM的KL散度优势，分析收敛速度；然后扩展到高斯混合分布，识别局部单峰性机制；最后通过高斯分布实验和图像视频生成应用验证理论结果。

Result: TM在单峰高斯分布中严格优于FM，收敛更快；在高斯混合分布中，当模式分离良好且方差不可忽略时TM表现更优；但当目标方差趋近于零时，TM优势减弱。

Conclusion: TM在目标分布具有良好分离模式和非可忽略方差时优于FM，这为选择生成模型提供了理论指导。
...
TL;DR: 提出A-DATSC模型，结合深度子空间聚类生成器和质量验证判别器，用于处理多变量时空数据的复杂非线性流形聚类问题，在多个真实数据集上表现优于现有方法。
...
Method: A-DATSC模型包含基于U-Net的生成器（使用TimeDistributed ConvLSTM2D层保持时空完整性）和图注意力变换器的自表达网络，捕获局部空间关系、全局依赖以及短长程相关性。

Result: 在三个真实世界多变量时空数据集上的实验表明，A-DATSC相比最先进的深度子空间聚类模型实现了显著优越的聚类性能。

Conclusion: A-DATSC通过注意力引导的深度对抗时序子空间聚类方法，有效解决了现有模型在时空数据聚类中的局限性，为复杂非线性流形上的数据聚类提供了有效解决方案。
...
TL;DR: 该论文系统评估了8种概率深度学习模型在神经活动预测中的表现，发现多个深度学习模型在多个预测时间范围内优于传统统计方法，最佳模型能够提供长达1.5秒的有意义预测。


<details>
  <summary>Details</summary>
Motivation: 神经活动预测对于理解神经系统和实现闭环控制至关重要，但深度学习在该领域的应用仍然有限，需要系统评估深度学习模型在神经活动预测中的有效性。

Method: 系统评估了8种概率深度学习模型（包括2个基础模型）和4种经典统计模型、2种基线方法，使用小鼠皮层自发神经活动的宽场成像数据进行对比。

Result: 多个深度学习模型在多个预测时间范围内持续优于传统方法，最佳模型能够产生长达1.5秒的有意义预测。
...
<details>
  <summary>Details</summary>
Motivation: 解决科学机器学习中从稀疏、跨域传感器数据预测不可观测物理量的核心问题，突破现有方法依赖密集、共位输入输出场和短时间上下文的限制。

Method: 引入时空算子网络(STONe)，在传感器和目标流形之间定义非线性算子，无需迭代递归即可在长时间预测范围内保持稳定，不要求域对齐或自回归传播。

Result: 在23年全球中子数据上训练，STONe实现了准确的180天预测，具有毫秒级推理延迟，证明了算子学习可以超越共享域设置进行泛化。

Conclusion: 该框架建立了跨域算子推断的一般原则，为物理、气候和能源系统中复杂时空场的实时预测提供了可行方案。
...
Conclusion: ACIA为稳健的反因果学习提供了理论保证，建立了训练和未见环境之间性能差距的紧密界限。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖固定发散正则化，导致固有困境：强正则化保留模型能力但限制奖励优化，弱正则化能更好对齐但风险不稳定或奖励黑客。

Method: ADRPO根据优势估计自动调整正则化强度：对高价值样本减少正则化，对差样本应用更强正则化，使策略能根据数据质量在探索和积极利用间导航。

Result: 在文本到图像生成中超越DPO和ORW-CFM-W2方法；2B参数SD3模型在属性绑定、语义一致性等方面超越4.8B和12B模型；在LLM和多模态推理中增强现有RL方法，7B模型超越Gemini 2.5 Pro和GPT-4o Audio。

Conclusion: ADRPO为不同生成架构和模态的探索-利用挑战提供了有效的即插即用解决方案，通过主动探索能够逃离局部最优。
...
TL;DR: SPACeR框架结合预训练tokenized自回归运动模型和自强化学习，在自动驾驶仿真中实现高效、人类化的多智能体行为模拟。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在计算成本高、推理速度慢或行为偏离人类规范的问题，需要既能保持人类驾驶分布特性又具备RL可扩展性的解决方案。

Method: 使用预训练tokenized自回归运动模型作为中心化参考策略，通过似然奖励和KL散度指导去中心化自强化学习，将策略锚定在人类驾驶分布中。

Result: 在Waymo仿真智能体挑战中表现优异，推理速度比大型生成模型快10倍，参数规模小50倍，同时在闭环规划评估中有效测试自动驾驶策略质量。
...
TL;DR: AC-Flow是一个用于微调流式生成模型的演员-评论家框架，通过奖励塑形、双稳定性机制和可扩展的评论家加权方案，解决了现有方法在中间反馈学习中的信用分配问题和训练不稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有的流式生成模型在基于中间反馈进行微调时面临挑战，特别是连续时间流匹配模型。大多数方法仅从结果奖励学习，难以解决信用分配问题，而通过直接回归累积奖励学习评论家的方法在在线设置中经常面临训练不稳定和模型崩溃。

Method: AC-Flow框架包含三个关键创新：(1) 奖励塑形提供标准化学习信号，实现稳定的中间值学习和梯度控制；(2) 双稳定性机制结合优势裁剪防止破坏性策略更新，以及预热阶段让评论家在影响演员前成熟；(3) 可扩展的广义评论家加权方案扩展传统奖励加权方法，同时通过Wasserstein正则化保持模型多样性。

Result: 在Stable Diffusion 3上的广泛实验表明，AC-Flow在文本-图像对齐任务中实现了最先进的性能，并能泛化到未见的人类偏好模型。即使使用计算高效的评论家模型，也能在不损害生成质量、多样性或稳定性的情况下稳健地微调流模型。

Conclusion: AC-Flow提供了一个稳健的演员-评论家框架，成功解决了流式生成模型在中间反馈微调中的关键挑战，实现了稳定的训练和优异的性能表现。
...
### [125] [R2L: Reliable Reinforcement Learning: Guaranteed Return & Reliable Policies in Reinforcement Learning](https://arxiv.org/abs/2510.18074)
*Nadir Farhi*

Main category: cs.LG

TL;DR: 提出了一种可靠的强化学习方法，通过最大化累积回报超过预设阈值的概率来确保策略的可靠性，可将该问题转化为标准RL问题使用现有算法求解。


<details>
  <summary>Details</summary>
Motivation: 传统RL算法专注于最大化期望回报，但许多实际应用（如路由、资源分配、风险决策）需要确保高成功率而非仅高平均性能的策略。

Method: 通过状态增强表示将可靠RL问题重新表述为标准RL问题，允许使用现有RL算法（如Q-learning、Dueling Double DQN）而无需全新框架。

Result: 理论证明两种表述的等价性，数值实验在可靠路由问题上验证了该方法能有效平衡效率和可靠性。

Conclusion: 可靠RL方法在随机和安全关键环境中具有实际应用潜力，能提供性能保证的策略。
...
### [126] [Batch Distillation Data for Developing Machine Learning Anomaly Detection Methods](https://arxiv.org/abs/2510.18075)
...
TL;DR: 构建了一个用于化学过程异常检测的机器学习实验数据库，包含119个实验数据，涵盖正常和异常操作，提供多种传感器数据和非常规数据源。


<details>
  <summary>Details</summary>
Motivation: 解决机器学习在化学过程异常检测中缺乏公开实验数据的问题，促进先进ML方法的发展。

Method: 建立实验室规模的间歇蒸馏装置，进行119个实验，涵盖各种操作条件和混合物，包括正常和故意诱导的异常实验。

Result: 创建了包含时间序列传感器数据、测量不确定性估计、在线NMR谱、视频音频记录的结构化数据库，并开发了异常注释本体。

Conclusion: 该数据库为开发先进的基于ML的异常检测方法铺平了道路，特别支持可解释和可解释的ML方法以及异常缓解方法的发展。
...
### [127] [MEG-GPT: A transformer-based foundation model for magnetoencephalography data](https://arxiv.org/abs/2510.18080)
*Rukuang Huang,Sungjun Cho,Chetan Gohil,Oiwi Parker Jones,Mark Woolrich*

Main category: cs.LG

TL;DR: MEG-GPT是一个基于Transformer的基础模型，使用时间注意力和下一时间点预测来建模大规模脑动力学。它通过数据驱动的分词器处理连续MEG数据，在大型MEG数据集上训练后能生成具有现实时空特性的数据，并在下游解码任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统方法无法捕捉脑磁图(MEG)等模态中复杂的时空模式，而深度学习在语言和视觉领域的成功表明基础模型在大规模应用中的潜力。

Method: 开发了MEG-GPT Transformer基础模型，使用时间注意力和下一时间点预测，并引入数据驱动的分词器来处理连续MEG信号而不损失时间分辨率。

Result: 模型能生成具有现实时空特性的数据，在下游解码任务中表现优异：跨会话准确率从0.54提升到0.59，跨被试准确率从0.41提升到0.49，并能通过微调进一步提升性能。
...
<details>
  <summary>Details</summary>
Motivation: LLMs表现出强但浅层的对齐：在助手轮开始时直接拒绝有害查询，但一旦有害内容开始生成，这种保护就会崩溃。需要确保在任意生成深度的安全性。

Method: 提出Any-Depth Alignment (ADA)，基于观察到对齐集中在助手头部令牌中。通过在生成过程中重新引入这些令牌，诱导模型重新评估有害性并恢复拒绝。

Result: 在多种开源模型上，ADA实现了接近100%的拒绝率对抗对抗性预填充攻击，将主要对抗提示攻击的平均成功率降低到3%以下，同时保持良性任务的实用性。
...
### [129] [Provably Optimal Reinforcement Learning under Safety Filtering](https://arxiv.org/abs/2510.18082)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习在复杂任务中应用时缺乏正式的安全保证，限制了其在安全关键环境中的使用。安全过滤器虽然常用，但被认为会牺牲性能和阻碍学习过程。

Method: 定义了安全关键马尔可夫决策过程(SC-MDP)和过滤MDP，通过理论证明在过滤MDP中学习是安全的，且标准RL收敛性仍然适用。

Result: 在Safety Gymnasium上的实验验证了训练过程中零违规，最终性能达到或超过无过滤基准。
...
TL;DR: 开发并评估结合结构化临床数据和非结构化文本信息的机器学习模型，用于ICU住院死亡率早期预测，相比仅使用结构化数据，AUC从0.753提升至0.918。
...
TL;DR: 提出了潜在离散扩散模型（LDDMs），通过结合离散标记的掩码扩散和潜在嵌入的连续扩散，解决了传统掩码去噪器在少步生成中联合结构弱化的问题。
...
Method: 提出了两种LDDM变体：FUJI-LDDMs（完全联合去噪）和SEQ-LDDMs（顺序解析潜在和离散链），并推导了ELBO风格的目标函数。

Result: 在无条件生成指标上相比最先进的掩码离散扩散基线有所改进，在较低采样预算下特别有效。

Conclusion: LDDMs通过潜在通道提供更柔和的信号和跨标记依赖关系，能够有效解决歧义问题，在少步生成中表现优异。
...
TL;DR: Rectified Flows在确定性训练下存在记忆化问题，即使插值线相交也会记忆任意训练配对，而注入噪声可以恢复泛化能力。
...
Result: 证明在确定性训练下，即使插值线相交，Rectified Flow也会在推理时重现训练时的特定配对，而注入小噪声可以恢复泛化。

Conclusion: 确定性插值会导致记忆化，而噪声注入是恢复Rectified Flows泛化能力的关键机制。
...
TL;DR: 提出了核心注意力解耦技术，通过将核心注意力计算与其他模型层分离并在专用设备上执行，解决了长上下文训练中的负载不平衡问题。
...
Method: 将核心注意力分解为token级任务，调度到专用注意力服务器上，动态重新批处理任务以均衡计算而不牺牲内核效率。

Result: 在512个H200 GPU和512k token上下文长度下，端到端训练吞吐量提升1.35倍，消除了数据并行和流水线并行拖尾，实现了近乎完美的计算和内存平衡。
...
TL;DR: HyperDiffusionFields (HyDiF) 是一个将3D分子构象建模为连续场的框架，使用分子方向场(MDF)表示分子结构，通过超网络生成分子特定神经隐式场，并采用扩散模型实现生成功能。
...
Method: 使用分子方向场(MDF)表示分子结构，通过超网络生成分子特定神经隐式场(MNFs)，采用去噪扩散模型训练超网络，支持掩码扩散机制实现条件生成。

Result: 该方法支持分子生成、分子修复等任务，能够进行空间细粒度特征提取用于分子性质预测，并能扩展到更大的生物分子。
...
TL;DR: 本文基于自注意力与核主成分分析(PCA)的联系，重新审视PCA基础理论，提出多种新颖的DC框架公式化方法，揭示了同时迭代与QR算法的联系，并开发了新的PCA算法和鲁棒核PCA变体。


<details>
  <summary>Details</summary>
Motivation: 受自注意力机制与核主成分分析之间新发现的联系启发，重新审视PCA的基本理论，探索更优化的算法框架。

Method: 采用差异凸(DC)优化框架，提出多种PCA的新公式化方法，包括核化版本和样本外应用，并建立了同时迭代与QR算法的优化视角联系。

Result: 证明了PCA类问题的核化能力和样本外适用性，揭示了同时迭代是DC算法的一个实例，开发了新的PCA算法并在实验中与先进方法进行比较。

Conclusion: 通过DC框架为PCA提供了新的理论洞察和算法视角，建立了传统数值方法与优化理论的联系，并提出了可核化的鲁棒PCA变体。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于正则化的方法需要正则化强度趋近于零来逼近纳什均衡，这在实际中常导致学习不稳定。

Method: 固定较大的正则化强度以保证鲁棒性，通过迭代优化参考策略实现收敛，开发了Nash Policy Gradient (NashPG)算法。

Result: 理论证明该方法在两人零和博弈中保证严格单调改进并收敛到精确纳什均衡；实验显示在经典基准游戏上达到可比或更低的可剥削性，在大型领域如Battleship和No-Limit Texas Hold'em中持续获得更高的Elo评分。

Conclusion: NashPG方法在保持策略梯度方法通用性的同时，仅依赖当前和参考策略，能够稳定收敛到精确纳什均衡，并在大规模不完全信息博弈中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然能生成流畅文本，但其内部推理过程不透明且难以控制。稀疏自编码器虽然能暴露与人类概念对齐的潜在特征，但这些特征脆弱且被动，缺乏系统性推理或模型控制机制。

Method: ActivationReasoning框架包含三个阶段：(1) 发现潜在表示：识别潜在概念表示并组织成字典；(2) 激活命题：在推理时检测激活概念并映射到逻辑命题；(3) 逻辑推理：应用逻辑规则进行推理，推断高阶结构、组合新概念并引导模型行为。

Result: 在多个任务上评估AR：多跳推理(PrOntoQA)、抽象和鲁棒性(Rail2Country)、自然语言推理(ProverQA)、上下文敏感安全性(BeaverTails)。AR在所有任务中都能随着推理复杂性稳健扩展，泛化到抽象和上下文敏感任务，并在不同模型骨干间迁移。

Conclusion: 在潜在激活中建立逻辑结构不仅提高了透明度，还实现了结构化推理、可靠控制以及与期望行为的对齐，为更可靠和可审计的AI提供了路径。
...
TL;DR: 提出一种多阶段集成框架，通过HJB方程学习最优成本函数和相应的最优控制信号，无需使用稳定项，提供单一或集成控制策略。
...
Method: 采用多阶段集成框架学习最优成本函数和最优控制信号，不使用稳定项，支持单一控制信号或集成控制策略。

Result: 在稳态时不变双状态连续非线性系统的闭环控制中成功验证，能够处理噪声扰动系统状态和变化的初始条件。
...
Result: 该框架在集中训练和分散执行范式下，能够实现自适应隐蔽合作，同时满足能量和移动性约束。
...
TL;DR: 提出了P-GAP方法，通过投影梯度对齐扰动进行零阶优化，实现快速LLM微调，减少参数扰动和方差，加速收敛


<details>
  <summary>Details</summary>
Motivation: 现有零阶优化方法在梯度估计中存在高方差问题，导致收敛慢且性能不佳，特别是在大规模模型上

Method: 首先估计低维梯度空间，然后在投影梯度方向对齐扰动，减少扰动参数数量和方差

Result: 在LLM实验中，P-GAP在分类任务上准确率提升6%，生成任务上准确率提升12%，训练迭代减少81%，GPU时间减少70%
...
TL;DR: 提出了ACTG-ARL框架，通过分层方法和锚定强化学习改进差分隐私下的合成文本生成质量与控制能力
...
Method: 采用分层框架：特征学习+条件文本生成，结合DP表格合成器和DP微调条件生成器；提出锚定RL方法提升指令跟随能力

Result: 在强隐私保证下，合成文本质量提升20% MAUVE，条件生成器控制能力显著增强

Conclusion: ACTG-ARL框架有效解决了DP合成文本生成的质量和控制问题，为隐私保护语言模型训练提供了可行方案
...
TL;DR: 论文认为当前AI/ML社会影响力研究的评审标准过度强调同时实现部署和方法创新，这不利于更广泛的社会影响力研究生态系统的可持续发展。


<details>
  <summary>Details</summary>
Motivation: 当前AI/ML社会影响力研究的评审标准倾向于同时认可部署和新方法创新的项目，这可能导致研究人员忽视仅在一个方面（应用或方法）做出贡献但更能满足合作伙伴需求的项目。

Method: 提出立场性观点，主张研究者和评审者需要：1）采用更广泛的社会影响力概念，超越部署本身；2）对已部署系统的影响力进行更严格的评估。

Result: 通过分析当前评审标准的局限性，提出了改进建议，旨在促进更可持续的社会影响力研究生态系统。

Conclusion: 呼吁AI/ML社会影响力研究社区采纳更包容的评审标准，认可单一方面的贡献，并加强对已部署系统实际影响力的评估。
...
TL;DR: 本文提出了一个名为RULE的鲁棒多模态实体对齐框架，用于解决多模态知识图谱中存在的双重噪声对应问题，包括实体-属性层和跨图谱层的噪声对应。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态实体对齐方法通常假设实体内部和跨图谱对应关系是完美的，但在真实世界的多模态知识图谱中，由于依赖专家标注，往往存在双重噪声对应问题。

Method: RULE框架通过两阶段原则估计实体内部和跨图谱对应关系的可靠性，在属性融合时减轻实体内部噪声的影响，在消除跨图谱差异时防止对噪声对应关系的过拟合，并包含一个对应推理模块来发现跨图谱的属性-属性连接。

Result: 在五个基准数据集上的广泛实验表明，与七种最先进方法相比，RULE在应对双重噪声对应问题上具有显著优势。
...
TL;DR: 该研究提出了条件缩放定律和搜索框架，用于在保持准确性的同时优化大语言模型的推理效率，通过训练200多个模型验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型变得越来越强大和广泛部署，推理成本已成为紧迫问题，但模型准确性和推理效率之间的权衡尚未得到充分探索。

Method: 引入条件缩放定律，将Chinchilla框架与架构信息相结合，并开发搜索框架来识别同时具备推理效率和准确性的架构。训练了200多个模型（80M到3B参数，8B到100B训练token）来拟合该定律。

Result: 条件缩放定律可靠地预测了最优架构选择，所得模型优于现有开源基线。在相同训练预算下，优化架构相比LLaMA-3.2实现了高达2.1%的准确率提升和42%的推理吞吐量提升。

Conclusion: 通过系统研究关键架构因素对推理成本和准确性的影响，提出的条件缩放定律和搜索框架能够有效识别推理高效且准确的模型架构。
...
TL;DR: 提出NTKMTL方法，利用神经正切核理论分析多任务学习中的训练动态，通过谱分析平衡多个任务的收敛速度来解决任务不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 多任务学习中任务不平衡是一个主要挑战，虽然平衡不同任务的收敛速度是有效方法，但在复杂的MTL系统中准确表征多个任务的训练动态和收敛速度极具挑战性。
...
Conclusion: NTKMTL方法通过NTK理论有效解决了MTL中的任务不平衡问题，在保持竞争力的同时提高了训练效率。
...
TL;DR: 提出了Customized-GRPO框架，通过Synergy-Aware Reward Shaping和Time-Aware Dynamic Weighting解决主题驱动图像生成中身份保真度与提示遵循度之间的权衡问题，有效缓解了竞争性退化。


<details>
  <summary>Details</summary>
Motivation: 解决主题驱动图像生成模型在身份保真度和提示遵循度之间的基本权衡问题，发现简单的GRPO应用会导致竞争性退化，因为静态权重的线性奖励聚合会产生冲突的梯度信号，并与扩散过程的时间动态不匹配。

Method: 提出Customized-GRPO框架，包含两个关键创新：(1) Synergy-Aware Reward Shaping - 非线性机制，明确惩罚冲突奖励信号并放大协同信号，提供更清晰、更决定性的梯度；(2) Time-Aware Dynamic Weighting - 通过早期优先提示遵循、后期优先身份保真度，使优化压力与模型时间动态对齐。

Result: 广泛实验表明，该方法显著优于朴素GRPO基线，成功缓解了竞争性退化。模型实现了更优的平衡，生成既保留关键身份特征又准确遵循复杂文本提示的图像。

Conclusion: Customized-GRPO通过奖励整形和动态加权机制，有效解决了主题驱动图像生成中的权衡问题，为平衡身份保真度和提示遵循度提供了创新解决方案。
...
TL;DR: 提出了TOT理论框架，用于处理存在未知分布漂移的在线时间序列预测问题，通过引入潜变量来提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 在线时间序列预测中存在未知的分布漂移，即潜变量会影响从历史观测到未来观测的映射关系，需要开发自动化的预测方法。

Method: 提出TOT理论框架，证明引入潜变量可以降低贝叶斯风险；设计通过最小相邻观测识别潜变量的方法；开发模型无关的蓝图，使用时序解码器匹配观测变量分布，并用两个独立噪声估计器分别建模潜变量的因果推断和观测变量的混合过程。

Result: 在合成数据上的实验结果支持理论主张；在多个基线模型上的插件实现显示在多个基准测试中普遍提升性能。

Conclusion: TOT框架能够有效处理在线时间序列预测中的分布漂移问题，通过引入潜变量提升预测性能，具有实际应用价值。
...
Method: pretc算法从随机探索开始，然后基于估计的奖励函数选择最优波束；prgreedy在线估计并选择当前最佳波束。两种算法都将每条路径参数视为黑盒，基于历史采样奖励进行最优估计。

Result: 在合成DeepMIMO数据集和真实DeepSense6G数据集上的实验表明，两种算法在各种信道环境下的多种场景中都优于现有方法。
...
Conclusion: CHiLD框架能够唯一确定分层潜在变量的联合分布，成功解决了分层时间序列建模的关键挑战。
...
TL;DR: 研究嵌入维度如何影响Transformer在排序任务中内部世界模型的形成，发现更大嵌入维度产生更忠实、一致和鲁棒的内部表示，并揭示了两种一致的内部机制。


<details>
  <summary>Details</summary>
Motivation: 探究Transformer在强化学习排序任务中如何形成内部世界模型，以及嵌入维度对这种内部表示质量的影响。

Method: 使用强化学习训练Transformer执行冒泡排序式的相邻交换操作，通过数百次实验分析不同嵌入维度下的内部表示机制。

Result: 模型在很小嵌入维度下也能达到高准确率，但更大维度产生更结构化的内部表示和更好的可解释性；发现注意力权重矩阵最后一行单调编码全局排序，选择的交换操作与编码值的最大相邻差异对齐。
...
TL;DR: 提出了灵活证据深度学习(F-EDL)，通过预测灵活的Dirichlet分布来扩展传统证据深度学习，提高不确定性量化的表达能力和泛化性能。


<details>
  <summary>Details</summary>
Motivation: 传统证据深度学习(EDL)虽然计算高效，但其Dirichlet分布的严格假设限制了在复杂或未知场景中的鲁棒性，需要更灵活的不确定性表示方法。

Method: F-EDL扩展EDL，预测灵活的Dirichlet分布（Dirichlet分布的泛化形式）来建模类别概率，提供更具表达力和适应性的不确定性表示。
...
Method: TCCM学习从数据分布到固定目标（原点）的时间条件收缩向量场，无需在训练和推理期间求解ODE，使用单步偏差评分策略量化与预期收缩行为的偏离。

Result: 在ADBench基准测试中，TCCM在检测精度和推理成本之间取得了良好平衡，在高维和大规模数据集上表现优于现有最先进方法。

Conclusion: TCCM提供了一种轻量级、可扩展且可解释的异常检测方法，具有理论鲁棒性保证，特别适用于高维和大规模表格数据场景。
...
TL;DR: 本文分析了在无折扣（γ=1）总奖励无限时域MDP中的策略梯度方法，通过将状态分类为循环状态和瞬态状态，并使用瞬态访问测度替代经典状态访问测度。
...
Method: 基于两个关键洞察：(1) MDP状态分为循环状态和瞬态状态的分类在所有为每个动作分配严格正概率的策略下是不变的；(2) 用新的瞬态访问测度替代经典状态访问测度。

Result: 为无折扣期望总奖励无限时域MDP提供了策略梯度方法的理论分析框架。

Conclusion: 提出的瞬态访问测度和状态分类方法为无折扣设置下的策略梯度理论分析提供了可行途径。
...
TL;DR: 该论文研究了可计算通用在线学习，探讨了理论上的学习可能性与计算机程序实现之间的关系，发现在通用在线学习中可学习性并不自动意味着可计算可学习性。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习理论中的学习性表征往往将学习视为抽象数学对象，而忽略了关键问题：学习何时可以作为计算机程序实现？本文旨在解决通用在线学习中的可计算性问题。

Method: 研究通用在线学习模型，其中对手可以改变主意，只要保持与给定假设类的局部一致性。要求学习者在有限错误次数内使用可计算策略。分析了可计算通用在线学习、不可知变体和适当变体的可学习性。

Result: 证明通用在线学习并不蕴含可计算通用在线学习，即使假设类在可计算性理论角度相对简单。给出了不可知变体可计算通用在线学习的精确表征，并确定了适当变体可学习的确切条件。

Conclusion: 研究结果为在线二元分类和相关归纳推理问题提供了更现实的视角，揭示了理论可学习性与实际可计算实现之间的重要区别。
...
<details>
  <summary>Details</summary>
Motivation: Deep Ensembles等方法虽然不确定性量化性能强，但计算和内存成本高，难以扩展到大型模型。需要开发既高效又保持强大UQ性能的方法。

Method: 提出Hydra Ensembles：1）剪枝注意力头创建多样化集成成员；2）通过新的多头注意力和分组全连接层合并成员；3）形成紧凑模型，推理速度接近单网络。

Result: 在图像和文本分类任务中，各种架构上均一致优于Deep Ensembles。在ImageNet-1k零样本分类中，无需额外训练即超越现有最优方法。

Conclusion: Hydra Ensembles提供了一种高效的不确定性量化解决方案，在保持强大性能的同时显著降低了计算和内存开销，为安全关键应用中的深度神经网络部署提供了实用工具。
...
TL;DR: 提出FGP预训练方法，通过重构信息流代理来训练神经网络架构编码器，无需专用模型结构即可捕获信息流，显著提升编码器性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于信息流的神经网络架构编码器虽然性能优秀，但由于复杂的专用模型结构导致处理速度缓慢，需要一种既能捕获信息流又保持高效的方法。

Method: FGP预训练方法训练编码器重构信息流代理（flow surrogate），这是一种表示神经网络架构信息流的替代表示，无需专用模型结构。

Result: 实验显示FGP将编码器性能提升高达106%（Precision-1%指标），相比仅使用监督学习的相同编码器有显著改进。

Conclusion: FGP方法有效解决了基于信息流的编码器速度慢的问题，通过预训练方式捕获信息流，显著提升神经网络架构编码器的性能。
...
TL;DR: 提出了GraphRTA框架，用于解决无监督开放集图域自适应问题，通过图重编程和模型重编程来识别目标图中的已知类和未知类。
...
Method: GraphRTA框架通过修改目标图结构和节点特征进行图重编程，同时通过剪枝领域特定参数进行模型重编程，并在分类器中添加未知类维度。

Result: 在多个公共数据集上的综合实验表明，该模型相比现有最优基线方法取得了令人满意的性能。

Conclusion: GraphRTA能够有效解决开放集图域自适应问题，无需手动设置阈值即可识别未知类，代码和数据集已公开。
...
TL;DR: 本研究系统评估了20种GNN专家多样化策略，在14个节点分类基准上构建了200多个集成变体，揭示了训练最大多样化专家的机制性见解。


<details>
  <summary>Details</summary>
Motivation: 单个GNN的性能受限于真实图数据的异质性，而集成多个具有不同泛化模式的GNN可以显著提升性能，但缺乏对专家级多样化技术的系统研究。

Method: 评估了20种多样化策略，包括随机重新初始化、超参数调优、架构变化、方向性建模和训练数据划分，在14个节点分类基准上构建了200多个集成变体。
...
TL;DR: 该论文研究了具有指数函数幂次激活函数的浅层神经网络的逼近特性，重点关注在Barron函数空间中逼近率对维度和函数光滑度的依赖性。
...
Method: 分析ReLU^k激活函数的逼近率，证明在ℓ^1有界系数或不足够光滑条件下无法达到最优逼近率，并在各种范数下建立Barron空间和Sobolev空间函数的最优逼近率。

Result: 确认了维度灾难的存在，阐明了浅层神经网络逼近能力的局限性，为激活函数和网络结构的选择提供了理论依据。
...
<details>
  <summary>Details</summary>
Motivation: 弱监督学习通常使用粗粒度的聚合信号而非实例标签。本文研究每个训练样本是包含m个正例的n元组，但只观察到每组的计数m的情况，这在图像分类和多重实例测量等场景中很常见。

Method: 通过将元组生成过程与潜在实例边际联系起来，推导出闭式无偏风险估计器，并扩展到可变元组大小和计数。引入ReLU修正来提高有限样本稳定性，同时保持渐近正确性。

Result: 在转换为NTMP任务的基准测试中，该方法持续优于代表性弱监督基线，在精度-召回率和F1得分权衡方面表现良好，在类别先验不平衡和不同元组配置下保持鲁棒性。

Conclusion: 研究表明，仅通过计数监督可以通过理论严谨且实际稳定的目标有效利用，证明了计数监督在弱监督学习中的有效性。
...
TL;DR: 提出了MAGDrop正则化方法，通过基于梯度和动量动态调整dropout率来增强DNN泛化能力，理论推导了更紧的PAC-Bayes泛化界，在MNIST和CIFAR-10上取得了优于基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络虽然性能优异，但由于高容量容易过拟合，需要更有效的正则化方法来提高泛化能力。

Method: MAGDrop方法基于当前梯度和累积动量动态调整激活函数的dropout率，在非凸优化环境中增强稳定性。

Result: 在MNIST上达到99.52%测试准确率，CIFAR-10上达到90.63%，比标准dropout和自适应梯度正则化方法提升1-2%，泛化差距分别为0.48%和7.14%。

Conclusion: 该工作连接了理论洞察和实际进展，为增强DNN泛化提供了鲁棒框架，适用于高风险应用。
...
TL;DR: 提出了约束质量传输（CMT）框架，通过约束KL散度和熵衰减来改善玻尔兹曼生成器的采样效率，在多个基准测试中表现优于现有方法


<details>
  <summary>Details</summary>
Motivation: 解决高维多模态概率分布采样中的模式崩溃和质量传输问题，特别是玻尔兹曼生成器在采样物理系统时的挑战

Method: 引入约束质量传输（CMT）框架，在连续步骤间约束KL散度和熵衰减，增强分布重叠，减轻质量传输问题
...
<details>
  <summary>Details</summary>
Motivation: 现有方法采用解耦的时空学习范式，削弱了时空信息交互并导致模型复杂度高，需要一种更高效的异构时序图学习范式。

Method: 提出动态注意力机制，保留历史图快照的注意力信息来指导后续计算；利用大语言模型获取节点类型的隐式属性作为先验知识。

Result: 相比最先进基线方法，SE-HTGNN实现了高达10倍的加速，同时保持了最佳的预测精度。

Conclusion: SE-HTGNN通过集成时序建模和空间学习的新范式，在异构时序图上实现了高效且准确的表示学习。
...
TL;DR: 该论文对知识图谱上的公平感知图神经网络进行了基准测试研究，发现知识图谱在公平性与准确性权衡方面表现出与现有数据集不同的趋势，且性能受多种因素影响。


<details>
  <summary>Details</summary>
Motivation: 图神经网络在处理图结构数据时会产生关于敏感属性的偏见预测，但现有研究尚未在知识图谱上评估公平感知GNN，而知识图谱是推荐系统等应用中的重要图结构。

Method: 从YAGO、DBpedia和Wikidata三个知识图谱生成新图，这些图比现有公平性研究使用的数据集更大。在不同GNN骨干网络和早停条件下对预处理和过程中处理方法进行基准测试。

Result: 发现知识图谱显示出与现有数据集不同的趋势：在公平感知GNN中公平性指标与预测准确性之间的权衡更清晰；性能不仅受公平感知GNN方法影响，还受GNN骨干网络和早停条件影响；预处理方法通常改善公平性指标，而过程中处理方法改善预测准确性。

Conclusion: 知识图谱在公平感知GNN研究中表现出独特特性，需要专门考虑GNN骨干网络和训练策略对公平性的影响，预处理和过程中处理方法在公平性和准确性方面各有优势。
...
TL;DR: 提出不确定安全评论家(USC)方法，通过不确定性感知的调制和精炼来平衡强化学习的安全性和性能，在保持竞争力的同时减少约40%的安全违规。


<details>
  <summary>Details</summary>
Motivation: 现有方法在强化学习安全探索中存在平衡问题：严格的安全约束会损害任务性能，而优先奖励的方法则频繁违反安全约束，产生平坦的成本梯度阻碍策略改进。

Method: 引入不确定安全评论家(USC)，在评论家训练中集成不确定性感知的调制和精炼，在不确定和高成本区域集中保守性，同时在安全区域保持锐利梯度。

Result: USC减少约40%的安全违规，同时保持竞争力或更高的奖励，预测与真实成本梯度之间的误差减少约83%。
...
TL;DR: COPPOL是一种基于共形预测的感知到策略学习方法，为语义分割提供有限样本安全保证，生成带有严格漏检边界的校准危险地图，用于下游强化学习规划。
...
Method: 将分布无关的有限样本安全保证集成到语义分割中，生成带有严格漏检边界的校准危险地图，这些地图为下游RL规划产生风险感知成本场。

Result: 在两个卫星基准测试中，COPPOL将危险覆盖率提高了最多6倍，实现近乎完全的不安全区域检测，同时将导航中的危险违规减少了约50%，且在分布偏移下保持鲁棒性。

Conclusion: COPPOL方法通过共形预测为感知系统提供有限样本安全保证，显著提升了危险检测能力和导航安全性，同时保持对分布变化的鲁棒性。
...
TL;DR: DILAB团队开发的多语言电商搜索系统在最终排行榜中排名第5，整体得分0.8819，在跨语言和跨领域任务中表现出稳定且高性能的结果。
...
Method: 采用多阶段流水线，包括数据精炼、轻量级预处理和自适应建模。数据精炼阶段增强数据集一致性和类别覆盖，语言标记和噪声过滤提高输入质量。在建模阶段探索多种架构和微调策略，使用精选验证集优化超参数以平衡QC和QI任务性能。

Result: 系统在最终排行榜中排名第5，整体得分0.8819，在评估指标上表现出稳定且高性能的结果。

Conclusion: 该框架在跨语言和跨领域任务中展现出鲁棒性和适应性，突显了系统性数据管理和迭代评估对多语言搜索系统的有效性。
...
TL;DR: 本文提出了一种新的ROC分析方法，用于处理医疗警报系统等应用中存在的精度约束、容量限制和不对称成本问题，定义了部分VOROS指标来更好地评估分类器性能。


<details>
  <summary>Details</summary>
Motivation: 传统ROC分析无法捕捉医疗警报系统部署中的关键因素，如最小精度约束（避免误报疲劳）、预测阳性数量上限（医院人员容量限制）以及假阳性和假阴性的不对称成本。

Method: 展示了满足给定精度和容量约束的分类器子集如何在ROC空间中表示为可行区域，建立了该区域的几何结构，定义了部分较差分类器面积作为性能指标，并提出了部分VOROS指标。

Result: 在MIMIC-IV数据集上使用生命体征历史预测死亡风险的实验中，证明这种成本感知指标在医院警报应用中比替代方法更适合对分类器进行排序。

Conclusion: 提出的部分VOROS方法能够有效处理医疗警报系统中的约束条件和不对称成本问题，为这类应用的分类器评估提供了更合适的性能指标。
...
<details>
  <summary>Details</summary>
Motivation: 由于教师模型可能来自不可信方，知识蒸馏可能带来意外的安全风险，现有LLM后门方法选择的触发词在常规语境中很少出现，低估了知识蒸馏的安全风险。

Method: 提出T-MTB技术，构建由多个特定词组成的复合后门触发器，这些词在预期的蒸馏数据集中经常单独出现，使中毒教师保持隐蔽，同时在蒸馏过程中这些词的单独存在为后门转移到学生模型提供了足够信号。

Result: 使用T-MTB在两种攻击场景（越狱和内容调控）和四个LLM模型家族中证明了可转移后门的安全风险，现有后门大多不会转移到学生模型，但T-MTB能够实现后门转移。

Conclusion: 知识蒸馏从后门教师模型存在严重安全风险，T-MTB技术能够构建可转移的后门，需要重新评估知识蒸馏的安全假设。
...
TL;DR: RAISE框架通过统一评分系统评估AI模型在可解释性、公平性、鲁棒性和可持续性四个维度的表现，发现不同模型在不同责任维度存在权衡，没有单一模型在所有维度都表现最优。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统进入高风险领域，评估需要超越预测准确性，扩展到可解释性、公平性、鲁棒性和可持续性等多维度责任考量。

Method: 提出RAISE框架，量化模型在四个责任维度的表现并聚合成整体责任分数，评估了MLP、Tabular ResNet和Transformer三种深度学习模型在金融、医疗和社会经济数据集上的表现。
...
<details>
  <summary>Details</summary>
Motivation: 传统特征选择方法容易过早收敛，难以捕捉复杂的高维特征关系，特别是在高维数据集中需要更精细的特征选择策略。

Method: 使用遗传算法，采用偏置初始化方案和比率引导变异机制，结合帕累托多目标优化来最大化预测准确性和特征互补性。

Result: 在18个基准数据集上的实验表明，HeFS能够一致识别被忽视的信息特征，在胃癌分类、药物毒性预测和计算机科学应用等挑战性领域表现优于现有方法。

Conclusion: HeFS框架有效改进了特征选择结果，能够发现互补特征并提升分类性能，为高维数据分析提供了更精细的解决方案。
...
TL;DR: 提出了一种改进图神经网络结构鲁棒性的方法，用高效的部分求解器替代复杂求解器来检测对抗攻击。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络对抗鲁棒性检测方法依赖复杂的约束求解器，计算成本高。需要更高效的检测技术。

Method: 使用多项式时间运行但可能不完整的高效部分求解器来替代混合整数规划等复杂求解器。

Result: 在多种GNN变体和数据集上评估的RobLight工具显示，该方法在结构鲁棒性方面优于现有技术。

Conclusion: 通过使用高效的部分求解器，可以在图神经网络的结构鲁棒性检测中取得比现有技术更好的效果。
...
TL;DR: Unrolled-SINDy是一种改进SINDy方法稳定性的技术，通过解耦数值时间步长与数据采样率，解决了稀疏时间采样数据下的偏微分方程发现问题。


<details>
  <summary>Details</summary>
Motivation: 传统SINDy方法在处理时间稀疏采样数据时存在局限性，由于局部截断误差较大而无法找到正确的方程参数。

Method: 采用展开方案，通过迭代闭式方法或梯度下降方案实现，支持不同数值格式（欧拉法、RK4）。

Result: 实验表明该方法在传统SINDy和先进的噪声鲁棒iNeuralSINDy上均有效，能够解决非展开方法无法处理的问题。

Conclusion: Unrolled-SINDy提供了一种简单有效的方法来改进显式方法在PDE发现中的稳定性，扩展了SINDy方法的应用范围。
...
<details>
  <summary>Details</summary>
Motivation: 需要在保持良好预测性能的同时提高模型的可解释性，使机器学习模型更易于理解和解释。

Method: 使用称为修正（rectification）的方法来实现提升树到决策树的蒸馏过程。

Result: 实证研究表明该方法提供了有趣的结果，与通过重新训练模型实现的蒸馏方法相比表现良好。

Conclusion: 修正方法为提升树蒸馏为决策树提供了一种有效的途径，在预测性能和可解释性之间达成了可接受的折衷。
...
TL;DR: 该论文研究了在Next Symbol Prediction (NSP)设置下语言的可学习性，该设置提供比传统分类更丰富的标签信息，但证明了学习DFA和布尔公式等概念类仍然是计算困难的。
...
Method: 通过构造使几乎所有额外标签变得无信息化，将传统学习问题归约到具有NSP标签的学习问题。

Result: 在密码学假设下，学习DFA在NSP设置中仍然是计算困难的。

Conclusion: 尽管NSP设置提供了更丰富的标签信息，但对于某些概念类（如DFA和布尔公式）的学习仍然保持计算困难性。
...
TL;DR: 本文研究了Transformer在马尔可夫函数学习中的上下文学习能力，揭示了单层线性自注意力模型的全局最优解形式，证明了恢复最优参数的NP-hard性质，并提出了多层模型执行预条件梯度下降的新解释。


<details>
  <summary>Details</summary>
Motivation: 现有理论研究主要关注线性回归任务，为了理解Transformer在建模动态驱动函数时的上下文学习机制，需要研究马尔可夫函数学习。

Method: 通过结构化上下文学习设置，分析损失景观以揭示底层优化行为，包括推导单层线性自注意力模型的全局最优解闭式表达式，证明参数恢复的NP-hard性质，以及解释多层模型作为预条件梯度下降。

Result: 获得了单层线性自注意力模型的全局最优解闭式表达式，证明了恢复最优参数的NP-hard性质，提出了多层模型执行预条件梯度下降的新解释，并通过简化Transformer进行了数值验证。

Conclusion: 单层线性自注意力在表示结构化动态函数方面存在根本限制，而多层模型能够通过预条件梯度下降优化多个目标，这为理解Transformer的上下文学习机制提供了新的理论视角。
...
TL;DR: 该研究提出了一种结合作物模拟模型和机器学习优势的混合方法，通过将作物产量建模为时间性水资源短缺的函数，并引入物理约束的损失函数，实现了高精度且可解释的作物产量预测。


<details>
  <summary>Details</summary>
Motivation: 传统作物模拟模型具有物理可解释性但性能较差，而机器学习模型性能强大但缺乏物理一致性。本研究旨在结合两者的优势，开发既准确又符合作物生长物理原理的产量预测模型。

Method: 将作物产量建模为时间性水资源短缺的函数，预测作物干旱胁迫和对水资源短缺的敏感性。采用物理约束的损失函数，结合多光谱卫星影像、气象数据和精细尺度产量数据，并基于深度集成方法处理模型不确定性。

Result: 该方法在作物产量预测中超越了LSTM和Transformer等最先进模型，决定系数(R²)达到0.82，同时保持了高可解释性。

Conclusion: 该方法为行业、政策制定者和农民在气候变化背景下构建更具韧性的农业提供了决策支持，成功实现了物理一致性和预测准确性的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有对话动态模型往往无法泛化到单个群体之外，且试图用通用公式描述发言行为可能不适合所有群体。

Method: 开发了先前对话模型的泛化版本，基于个体特征（人格特质）和过往发言行为来预测任何群体中个体的发言轮次，特别能够学习个体发言倾向如何基于上次发言时间而变化。

Result: 在合成和真实对话数据上验证了该方法，结果表明先前的行为模型可能不总是现实的。

Conclusion: 数据驱动但理论基础扎实的方法能够更好地描述真实的群体互动，证明了先前行为模型的局限性。
...
TL;DR: 该研究提出了一种用于心电和心音信号分类的轻量级卷积神经网络，在资源受限的医疗边缘设备上实现了三个数量级的内存和计算成本降低，同时保持竞争力准确率。


<details>
  <summary>Details</summary>
Motivation: 心血管疾病早期检测很重要，但现有深度学习方法在资源受限设备上运行困难。需要开发高效、准确的分类模型以在医疗边缘设备上进行实时监测。

Method: 使用早期融合的卷积神经网络对同步的心电图和心音图记录进行二元分类，在Physionet Challenge 2016数据集上训练验证。

Result: 模型内存占用和计算成本比现有技术降低了三个数量级，同时保持了竞争性的准确率。在微控制器和实验传感器设备上的能耗分析证实了设备端推理比持续数据流更节能。

Conclusion: 提出的轻量级模型在医疗边缘设备上可行，能实现高效的心血管监测，为可穿戴医疗设备提供了实用的解决方案。
...
TL;DR: 该论文首次对推理大语言模型(RLLM)的服务性能进行了全面研究，揭示了RLLM与传统LLM在服务行为上的显著差异，并评估了现有推理优化技术对RLLM的有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管RLLM在解决复杂推理任务方面表现出色，但其服务性能和行为尚未被充分探索，这可能阻碍RLLM在真实场景中的部署和应用。

Method: 首先进行试点研究比较RLLM与传统LLM的服务性能，然后调查现有推理优化技术对RLLM的有效性，最后使用Gamma分布建模的真实世界工作负载进行评估。

Result: 发现RLLM具有显著的内存使用和波动、拖尾请求、自适应运行时间和领域偏好等特征；模型量化和推测解码能提高系统效率且对精度影响小，而前缀缓存和KV缓存量化可能降低小RLLM的精度或性能。

Conclusion: 研究为RLLM推理服务提供了重要见解，有助于研究社区和产业界推进RLLM推理服务的发展。
...
TL;DR: 提出了一种任务无关的多教师蒸馏框架，通过多数投票目标函数实现，无需任务特定标签或先验知识，在文本、视觉和分子建模中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有嵌入模型因架构、损失函数、输入模态和数据集的不同而捕获输入的不同方面，多教师蒸馏能利用这种多样性但通常局限于特定任务。

Method: 基于多数投票目标函数的任务无关蒸馏框架，该函数受学生与教师嵌入间互信息的约束，消除了对任务特定标签的依赖。

Result: 在文本、视觉模型和分子建模中的评估表明，该方法有效利用教师多样性，产生的表示在分类、聚类和回归等下游任务中表现更好。

Conclusion: 该方法成功实现了任务无关的多教师蒸馏，训练并发布了最先进的嵌入模型，提升了多种模态的下游性能。
...
TL;DR: 提出了贝叶斯价值函数和BOLA算法，解决传统RL在多步预测场景下的维度灾难问题，证明了在非完美预测下的样本效率


<details>
  <summary>Details</summary>
Motivation: 传统RL基于单步转移模型，而现实应用中多步预测能提供决策优势，但直接嵌入会导致状态空间指数爆炸和维度灾难

Method: 提出贝叶斯价值函数、Bellman-Jensen Gap分析方法和BOLA两阶段算法（离线贝叶斯价值学习+轻量级在线适应）

Result: 理论证明BOLA在非完美预测下仍保持样本效率，在合成MDP和真实风能存储控制问题上验证有效

Conclusion: 贝叶斯价值函数和BOLA算法为多步预测增强的RL提供了理论工具和实用解决方案
...
TL;DR: OmniCast是一个统一多时间尺度天气预报的概率模型，使用VAE编码和扩散变换器生成未来潜在状态，避免了自回归方法的误差累积问题。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的深度学习方法在中期天气预报中表现良好，但在长期次季节到季节(S2S)尺度上由于自回归方法的误差累积而表现不佳。

Method: OmniCast包含VAE编码器和扩散变换器两个组件：VAE将原始天气数据编码到低维潜在空间，扩散变换器基于初始条件生成未来潜在token序列。训练时随机掩码未来token，推理时通过迭代解掩码生成完整序列。

Result: OmniCast在中期范围与领先概率方法竞争且快10-20倍，在S2S尺度上在准确性、物理基础和概率指标方面达到最先进性能，并能生成长达100年的稳定预测。

Conclusion: OmniCast通过联合时空采样和低维潜在空间建模，成功统一了多时间尺度天气预报，解决了自回归方法的误差累积问题。
...
TL;DR: 本文提出M-AUPO算法用于在线偏好强化学习，通过最大化提供子集内的平均不确定性来选择多个动作，在Plackett-Luce排名反馈模型下实现了与子集大小直接相关的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有偏好强化学习理论工作大多只关注成对比较，少数研究探索了多比较和排名反馈，但它们的性能保证未能随反馈长度增加而改善甚至可能恶化，尽管可获得更丰富的信息。

Method: 采用Plackett-Luce模型进行动作子集的排名反馈，提出M-AUPO算法通过最大化提供子集内的平均不确定性来选择多个动作。

Result: M-AUPO实现了$\tilde{\mathcal{O}}\left( \frac{d}{T} \sqrt{ \sum_{t=1}^T \frac{1}{|S_t|}} \right)$的次优性差距，其中较大子集直接导致性能提升，且避免了先前工作中对未知参数范数的指数依赖。

Conclusion: 这是偏好强化学习中首个理论结果，明确显示样本效率随子集大小而改善，并建立了近乎匹配的下界$\Omega \left( \frac{d}{K \sqrt{T}} \right)$。
...
TL;DR: 提出了STEAM方法，用于生成医疗治疗效应分析的合成数据，满足因果推断的三个关键要求，并在复杂数据生成过程中优于现有生成模型。


<details>
  <summary>Details</summary>
Motivation: 真实医疗数据因监管障碍难以获取，合成数据可促进医疗分析和因果推断方法开发，但现有方法未考虑下游治疗效应分析的特殊挑战。

Method: 建立了合成数据应满足的三个标准：保持协变量分布、治疗分配机制和结果生成机制，并基于此提出评估指标和STEAM方法。

Result: STEAM在所有评估指标上达到最先进性能，特别是在真实数据生成过程复杂度增加时表现更优。

Conclusion: STEAM方法能有效生成适用于治疗效应分析的合成数据，为医疗因果推断提供了有价值的工具。
...
<details>
  <summary>Details</summary>
Motivation: 分数梯度下降虽然能加速优化过程，但在收敛行为和超参数选择方面存在显著挑战，特别是在非凸设置中难以调度超参数。

Method: 提出L2O-CFGD方法，通过元学习动态调整Caputo分数梯度下降的超参数，利用历史依赖特性进行优化。

Result: 元学习调度方法优于通过广泛搜索找到的静态超参数CFGD，在某些任务中性能可与完全黑盒元学习优化器相媲美。

Conclusion: L2O-CFGD可作为研究人员识别高性能超参数的有力工具，并为利用分数微分的历史依赖性提供见解。
...
TL;DR: CAGE是一种新的量化感知训练方法，通过曲率感知的梯度修正来减少量化引起的精度损失，在Llama风格模型上W4A4量化下比现有方法多恢复10%以上的损失。


<details>
  <summary>Details</summary>
Motivation: 当前的低比特量化感知训练与原生训练之间存在较大的精度差距，需要新的方法来弥补这一差距。

Method: CAGE在直通估计器梯度基础上添加曲率感知修正项，基于多目标优化视角平衡损失最小化和量化约束，利用局部曲率信息进行梯度修正。

Result: 在800M参数的Llama风格模型预训练中，CAGE在W4A4量化下比异常值处理方法多恢复超过10%的量化引起的损失。

Conclusion: 曲率感知梯度修正能够弥补当前异常值处理方法之外的性能差距，为量化训练提供了理论保证和实用实现。
...
TL;DR: SB-SETM是一种创新的在线主题模型，通过合并连续文档批次形成的模型来处理数据流，能够自动推断每个时间步的活动主题数量，并在俄乌战争新闻语料上表现优异。


<details>
  <summary>Details</summary>
Motivation: 在线主题模型能够处理持续演化的数据流，与真实场景更契合，但由于面临额外挑战，相比离线模型受到的关注较少。
...
TL;DR: 该论文提出了一种连续时间神经网络模型，统一了多种生物可信学习算法，消除了阶段分离需求。研究发现学习依赖于时间重叠，突触更新需要输入和误差信号在时间上重合，且稳健学习要求突触可塑性时间尺度超过刺激持续时间1-2个数量级。


<details>
  <summary>Details</summary>
Motivation: 生物学习在连续时间中展开，但大多数算法模型依赖离散更新和分离的推理与学习阶段。研究旨在开发连续时间神经模型，统一生物可信学习算法并消除阶段分离需求。

Method: 通过连续时间神经网络模型，将SGD、FA、DFA、KP等学习规则作为动力学极限情况自然涌现。通过分析和模拟研究学习的时间依赖性特征。

Result: 模拟显示这些连续时间网络在生物时间尺度上稳定学习，即使存在时间不匹配和积分噪声。学习强度随输入与误差延迟接近刺激持续时间而线性下降。稳健学习要求突触可塑性时间尺度超过刺激持续时间1-2个数量级。

Conclusion: 对于典型皮层刺激（数十毫秒），功能性可塑性窗口应在几秒范围内，这一可测试预测表明秒级资格迹是生物电路中误差驱动学习所必需的。
...
TL;DR: 研究发现AttnLRP中的双线性传播规则违反实现不变性公理，且绕过softmax层的CP-LRP与LOO有更好对齐，表明双线性分解敏感性和softmax传播误差共同影响LRP在Transformer中近似LOO的能力。


<details>
  <summary>Details</summary>
Motivation: LOO特征重要性评估计算成本高，而LRP作为高效替代方法在现代Transformer中的公理正确性尚未充分验证。

Method: 分析AttnLRP的双线性传播规则违反实现不变性公理，并通过CP-LRP诊断基线评估绕过softmax层传播的效果。

Result: 双线性传播规则在注意力层中违反实现不变性；绕过softmax层仅通过value矩阵传播相关性可显著改善与LOO的对齐。

Conclusion: 双线性分解敏感性和softmax传播误差是LRP在Transformer中难以准确近似LOO的主要原因。
...
TL;DR: 这篇综述提供了迭代优化的统一视角，连接经典理论与神经网络训练和生物学习。虽然基于梯度的方法主导机器学习，但零阶优化方法通过函数评估和随机性提供更轻量级的替代方案，在神经网络中能达到与反向传播竞争的性能，并为理解生物学习机制提供了数学原理框架。


<details>
  <summary>Details</summary>
Motivation: 统一理解迭代优化在AI和生物学习中的作用，探索零阶优化作为梯度方法的替代方案，特别是在高维设置中解决计算需求问题，并为生物学习机制提供数学解释。

Method: 通过分类优化方法（从一阶、二阶到高阶梯度方法，再到零阶方法），分析这些方法如何适应神经网络训练的独特挑战和学习动态，并将这些见解应用于理解生物学习过程。

Result: 现代零阶方法能够有效近似梯度，在神经网络模型中达到与反向传播竞争的性能；零阶优化的随机探索和反馈引导适应机制与生物学习的关键机制平行，为理解大脑学习提供了数学原理视角。

Conclusion: 零阶优化范式将大脑的内在噪声视为计算资源，不仅加深了对自然智能的理解，还对神经形态硬件设计具有重要影响，有助于开发利用硬件内在噪声的快速、节能AI系统。
...
TL;DR: 提出了一种简单、自监督的在线监督微调（OSFT）范式，用于提升大语言模型的推理能力。该方法通过模型自我生成响应并立即进行微调，无需奖励机制，仅需单次默认训练即可实现高效推理能力提升。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法通常需要复杂的奖励机制和多次训练迭代，而OSFT旨在提供一种更简单、高效的替代方案，利用模型在预训练阶段学到的潜在知识来提升推理能力。

Method: 采用在线监督微调范式，模型生成自己的响应后立即进行微调。该方法奖励无关，默认仅需单次训练，通过促进模型自身在预训练阶段学到的偏好（潜在知识）来提升推理能力。

Result: 在具有挑战性的数学推理任务上，OSFT的下游性能与GRPO等需要可验证奖励的强化学习方法相当。消融研究进一步证明了OSFT的效率和鲁棒性。

Conclusion: OSFT为更复杂的、基于奖励的训练范式提供了一种高效且有前景的替代方案，能够有效利用模型的潜在知识来提升推理能力。
...
TL;DR: 提出了一种搜索自博弈（SSP）方法，通过让LLM同时扮演任务提出者和问题解决者，自动生成具有明确真实答案的深度搜索查询，实现无监督的强化学习训练。


<details>
  <summary>Details</summary>
Motivation: 现有的基于可验证奖励的强化学习（RLVR）需要大量人工标注的任务查询和真实答案，这限制了RL在智能体场景下的扩展性。虽然已有任务合成方法，但难以控制生成任务的难度以提供有效的RL训练优势。

Method: 采用自博弈训练框架，LLM同时作为任务提出者和问题解决者。任务提出者生成具有明确定义真实答案且难度递增的深度搜索查询，问题解决者处理这些查询并输出答案预测。通过检索增强生成（RAG）确保每个查询都有准确真实答案。

Result: 实验结果表明，SSP在各种基准测试上都能显著提升搜索智能体的性能，无需任何监督，适用于从零开始和持续RL训练设置。

Conclusion: SSP方法通过自博弈实现了可扩展的智能体RLVR训练，有效解决了传统方法对人工标注的依赖问题，为搜索智能体的自主进化提供了新途径。
...
TL;DR: BO4Mob是一个用于高维贝叶斯优化的新基准框架，专注于解决大型城市道路网络中起点-终点出行需求估计的挑战性问题。


<details>
  <summary>Details</summary>
Motivation: 从有限的交通传感器数据中估计起点-终点出行需求是一个困难的逆优化问题，特别是在现实世界的大规模交通网络中。这个问题涉及在高维连续空间中进行优化，其中每个目标评估都是计算昂贵、随机且不可微分的。

Method: BO4Mob包含基于美国加州圣何塞真实道路网络的五个场景，输入维度最高达到10,100维。这些场景利用高分辨率、开源的交通模拟，包含现实的非线性和随机动态。

Result: 通过评估三种最先进的贝叶斯优化算法和两种非贝叶斯基线方法，展示了该基准的实用性。
...
TL;DR: 提出了一种基于纯值函数的连续控制框架，通过结构最大化Q函数实现高效稳定的学习，无需学习单独的actor网络，在受限动作空间环境中表现优于传统actor-critic方法。


<details>
  <summary>Details</summary>
Motivation: 传统基于值的算法只能用于离散动作空间，在连续动作空间中计算不可行；而actor-critic方法虽然流行但训练不稳定。

Method: 采用结构最大化Q函数的架构和算法选择，构建纯值函数的Q学习框架，无需单独的actor网络。

Result: 在标准仿真任务中表现出与最先进基线相当的性能和样本效率，在受限动作空间环境中优于传统actor-critic方法。

Conclusion: 纯值函数框架为连续控制提供了稳定高效的替代方案，特别适用于动作空间受限的环境。
...
TL;DR: 提出一个反事实推理框架，用于个性化风险评估和干预分析，应用于COVID-19后遗症对心力衰竭患者的影响研究。


<details>
  <summary>Details</summary>
Motivation: 反事实推理为替代干预下的假设结果提供了数学框架，连接因果推理和预测建模，旨在识别可操作路径以改善患者预后。

Method: 结合正则化预测建模与反事实搜索，使用精确枚举和基于优化的方法（包括NICE和MOC算法）来探索高维干预空间。

Result: 在2700多名确诊SARS-CoV-2感染且有心力衰竭史的患者中，模型表现出强大的区分性能（AUROC：0.88），并生成可解释的患者特异性反事实。

Conclusion: 反事实推理可以形式化为预测函数的优化问题，为复杂生物医学系统中的个性化推理提供严谨、可解释且计算高效的方法。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型通过后训练适应新任务时存在灾难性遗忘问题，需要找到减轻这种现象的指导原则。

Method: 在Llama和Qwen等模型家族上，系统比较SFT和RL的遗忘模式，并通过将语言模型建模为两个分布的混合来简化分析。

Result: 实验显示RL在所有测试任务（指令遵循、通用知识和算术推理）中都比SFT产生更少的遗忘，且目标任务性能相当或更好。RL的模式寻求特性使其能够保持先验知识。

Conclusion: RL对遗忘的鲁棒性源于其使用on-policy数据，这为使用近似on-policy数据来高效减轻遗忘提供了实用启示。
...
### [198] [Activation Manifold Projection: Liberating Task-Specific Behaviors from LLM Architectures](https://arxiv.org/abs/2510.17902)
*Al Kari*

Main category: cs.AI

TL;DR: CAST框架通过激活空间映射实现不同LLM架构间LoRA适配器的零样本迁移，解决了模型架构锁定问题。


<details>
  <summary>Details</summary>
Motivation: 现有微调方法（如LoRA）导致任务特定行为被锁定在源模型架构中，现有权重空间迁移方法脆弱且间接。

Method: 学习轻量级双向投影头，在目标模型激活流和源模型潜在空间之间建立非线性映射，应用冻结的LoRA行为核。

Result: CAST迁移的适配器在目标模型上达到完全重训练LoRA性能的85-95%，优于现有权重空间迁移技术。

Conclusion: CAST为模型互操作性建立了新标准，能够真正实现标准LoRA适配器的零样本跨架构迁移。
...
### [199] [Beyond More Context: Retrieval Diversity Boosts Multi-Turn Intent Understanding](https://arxiv.org/abs/2510.17940)
*Zhiming Lin*
...
<details>
  <summary>Details</summary>
Motivation: 解决任务导向聊天机器人中多轮意图理解面临的问题：token预算紧张、上下文嘈杂，现有检索流程过于强调相关性而忽视集合级多样性和混淆因素。

Method: 开发多样性感知检索框架，选择上下文示例以平衡意图覆盖和语言多样性，并与标准LLM解码器集成；评估时强制预算匹配提示和随机化位置。

Result: 在MultiWOZ 2.4和SGD数据集上，在相同token预算下实现了联合目标准确率的显著提升，超越了强LLM/DST基线，在K=4到7范围内保持一致性改进。
...
### [200] [FABRIC: Framework for Agent-Based Realistic Intelligence Creation](https://arxiv.org/abs/2510.17995)
...
<details>
  <summary>Details</summary>
Motivation: 收集智能体数据需要人类标注者参与，成本高昂、耗时且难以扩展，因此需要一种可扩展的合成数据生成方法。
...
Conclusion: 该框架为手动收集提供了可复现的、仅使用LLM的替代方案，从而推动能够进行稳健工具使用的智能体LLM的发展。
...
### [201] [OPTAGENT: Optimizing Multi-Agent LLM Interactions Through Verbal Reinforcement Learning for Enhanced Reasoning](https://arxiv.org/abs/2510.18032)
...
Conclusion: 有效的智能体通信和多智能体协作结构优化对提升复杂推理能力至关重要，提出的强化学习方法能有效改善多智能体系统的性能。
...
### [202] [Subject-Event Ontology Without Global Time: Foundations and Execution Semantics](https://arxiv.org/abs/2510.18040)
*Alexander Boldachev*

Main category: cs.AI

TL;DR: 提出了一种基于主体-事件的本体论形式化方法，用于建模不依赖全局时间的复杂动态系统，通过事件固定、因果排序、可执行本体等核心原则确保确定性。


<details>
  <summary>Details</summary>
Motivation: 解决复杂动态系统中对全局时间的依赖问题，为分布式系统、微服务架构等场景提供无需全局时钟的建模方法。

Method: 基于九个公理(A1-A9)的形式化框架，包括事件固定、因果排序、声明式数据流机制、模型作为认知过滤器等核心原则，并在boldsea系统中实现。

Result: 开发了可执行本体工作流引擎boldsea系统，实现了理论构造在BSL语言中的具体应用，确保了历史的单调性、因果关系的无环性和可追溯性。

Conclusion: 该形式化方法适用于分布式系统、微服务架构、DLT平台和多视角场景，为处理不同主体间冲突事实提供了理论基础和实用工具。
...
### [203] [CompactPrompt: A Unified Pipeline for Prompt Data Compression in LLM Workflows](https://arxiv.org/abs/2510.18043)
*Joong Ho Choi,Jiayang Zhao,Jeel Shah,Ritvika Sonawane,Vedant Singh,Avani Appalla,Will Flanagan,Filipe Condessa*

Main category: cs.AI

TL;DR: CompactPrompt是一个端到端的提示压缩管道，通过合并硬提示压缩和轻量级文件级数据压缩，在保持输出质量的同时将总令牌使用量和推理成本降低高达60%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在代理工作流中运行时会产生高昂成本，因为需要链接冗长提示并处理丰富数据流。

Method: 使用自信息评分和基于依赖关系的短语分组从提示中修剪低信息令牌，同时对附加文档应用n-gram缩写和数值列的均匀量化。

Result: 在TAT-QA和FinQA基准数据集上，总令牌使用量和推理成本降低高达60%，同时保持输出质量（Claude-3.5-Sonnet和GPT-4.1-Mini的准确率下降小于5%）。
...
### [204] [Planned Diffusion](https://arxiv.org/abs/2510.18087)
*Daniel Israel,Tian Jin,Ellie Cheng,Guy Van den Broeck,Aditya Grover,Suvinay Subramanian,Michael Carbin*

Main category: cs.AI

TL;DR: Planned diffusion是一种结合自回归和扩散模型的混合方法，通过两阶段生成（先自回归规划，后并行扩散生成）来优化文本生成的速度-质量权衡。
...
Method: 两阶段方法：1）自回归规划阶段生成短规划，将输出分解为独立的小片段；2）扩散生成阶段并行生成这些片段。

Result: 在AlpacaEval的805个指令跟随提示上，实现了速度-质量的Pareto最优权衡，相比自回归生成获得1.27x到1.81x加速，仅损失0.87%到5.4%的胜率。

Conclusion: Planned diffusion扩展了速度-质量的Pareto前沿，提供了更快、高质量文本生成的实用路径，其规划机制简洁可靠，可通过简单运行时参数灵活控制质量-延迟权衡。
...
### [205] [SMaRT: Select, Mix, and ReinvenT -- A Strategy Fusion Framework for LLM-Driven Reasoning and Planning](https://arxiv.org/abs/2510.18095)
*Nikhil Verma,Manasa Bharadwaj,Wonjun Jang,Harmanpreet Singh,Yixiao Wang,Homa Fashandi,Chul Lee*

Main category: cs.AI

TL;DR: SMaRT框架通过融合多种推理策略，克服单一策略的局限性，在推理、规划和决策任务中实现更优性能
...
Conclusion: SMaRT重新定义了LLM驱动的决策制定，开创了跨策略校准的新范式，为推理系统解锁了更优结果并推进了自我精化方法学的边界
...
### [206] [Measuring Reasoning in LLMs: a New Dialectical Angle](https://arxiv.org/abs/2510.18134)
*Soheil Abbasloo*

Main category: cs.AI

TL;DR: 提出了SIEV框架，基于辩证法来评估语言模型的推理过程，而不仅仅是最终答案的正确性，揭示了当前最先进模型在推理能力上的显著缺陷。


<details>
  <summary>Details</summary>
Motivation: 当前的语言模型评估主要关注答案的正确性，但很少揭示产生这些答案的推理过程。作者认为推理应该被看作一个动态的、思想碰撞和演化的过程。

Method: 基于辩证法理论（正题、反题、合题），开发了SIEV结构化框架，评估模型解决张力、整合不同观点和进行高阶推理的能力。

Result: 在GSM和MMLU等饱和基准测试上，SIEV评估显示GPT-5-chat等最先进模型存在显著的推理缺陷，在GSM上得分下降超过40分。
...
### [207] [Learning from Generalization Patterns: An Evaluation-Driven Approach to Enhanced Data Augmentation for Fine-Tuning Small Language Models](https://arxiv.org/abs/2510.18143)
...
TL;DR: PaDA-Agent是一种评估驱动的数据增强方法，通过发现验证数据中的失败模式并制定针对性数据增强策略，显著提升小语言模型在领域特定任务上的性能。
...
Method: 提出PaDA-Agent（模式引导数据增强代理），通过协调操作简化小语言模型的数据增强过程。该方法从验证数据中通过评估发现失败模式，并制定针对性数据增强策略，直接减少泛化差距。
...
### [208] [Annotating the Chain-of-Thought: A Behavior-Labeled Dataset for AI Safety](https://arxiv.org/abs/2510.18154)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于文本推理步骤的安全监控方法可能遗漏细微的有害模式，且可能被隐藏不安全推理的模型规避，需要更细粒度的监控手段。

Method: 构建包含推理序列和句子级安全行为标注的数据集，提取引导向量来检测和影响模型激活中的安全行为。

Result: 展示了该数据集的实用性，提取的表征能够检测和引导模型激活中的安全行为。

Conclusion: 激活级技术有潜力改善推理过程中的安全监督，为AI安全研究提供了重要工具。
...
### [209] [LLM-Based Multi-Agent System for Simulating and Analyzing Marketing and Consumer Behavior](https://arxiv.org/abs/2510.18155)
*Man-Lin Chu,Lucian Terhorst,Kadin Reed,Tom Ni,Weiwei Chen,Rongyu Lin*

Main category: cs.AI

TL;DR: 提出一个基于大语言模型的多智能体仿真框架，用于模拟消费者决策和社会动态，替代传统的基于规则的方法。


<details>
  <summary>Details</summary>
Motivation: 传统的事后分析和基于规则的智能体模型难以捕捉人类行为和社会互动的复杂性，需要更先进的仿真工具来在真实部署前测试营销策略。
...
### [210] [Saber: An Efficient Sampling with Adaptive Acceleration and Backtracking Enhanced Remasking for Diffusion Language Model](https://arxiv.org/abs/2510.18165)
...
TL;DR: 提出Saber算法，一种无需训练的新型采样方法，用于扩散语言模型在代码生成中平衡推理速度与输出质量


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在代码生成中存在推理速度与输出质量的关键权衡，减少采样步骤会导致性能灾难性下降

Method: Saber算法基于两个关键洞察：1）随着代码上下文建立可自适应加速；2）需要回溯机制来反转生成的标记
...
### [211] [AgentChangeBench: A Multi-Dimensional Evaluation Framework for Goal-Shift Robustness in Conversational AI](https://arxiv.org/abs/2510.18170)
*Manik Rana,Calissa Man,Anotida Expected Msiiwa,Jeffrey Paine,Kevin Zhu,Sunishchal Dev,Vasu Sharma,Ahan M R*

Main category: cs.AI

TL;DR: AgentChangeBench是一个专门评估工具增强语言模型代理在对话中目标变化适应能力的基准测试，包含2,835个任务序列和四个评估指标，揭示了传统准确率指标无法捕捉的代理韧性差异。
...
Method: 构建包含2,835个任务序列的AgentChangeBench基准，在三个企业领域评估代理对中期对话目标变化的适应能力，使用四个互补指标：任务成功率、工具使用效率、工具调用冗余率和目标变化恢复时间。

Result: 评估显示前沿模型在动态目标适应方面存在显著差异：GPT-4o在机票预订任务中达到92.2%恢复率，而Gemini降至48.6%；零售任务参数有效性接近完美但冗余率超过80%，揭示重大效率问题。

Conclusion: 高原始准确率不代表在动态目标下的鲁棒性，明确测量恢复时间和冗余率对于评估代理韧性至关重要，AgentChangeBench为诊断和改进企业环境中代理韧性提供了可复现的测试平台。
...
### [212] [Local Coherence or Global Validity? Investigating RLVR Traces in Math Domains](https://arxiv.org/abs/2510.18176)
*Soumya Rani Samineni,Durgesh Kalwar,Vardaan Gangal,Siddhant Bhambri,Subbarao Kambhampati*

Main category: cs.AI

TL;DR: 本文研究了基于可验证奖励的强化学习（RLVR）后训练对大型语言模型推理过程的影响，发现RL后训练提高了推理轨迹的局部连贯性，但未必能保证最终答案的正确性或推理的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法通常对所有标记一视同仁，仅基于最终答案正确性评估性能，却声称RL后训练能改善推理轨迹。这促使我们研究RL后训练对未直接激励的中间标记的影响。

Method: 使用GRPO算法在Qwen-2.5-0.5B模型和GSM8K数据集上进行实验，引入基于一阶逻辑的轨迹连贯性度量来捕捉推理步骤的一致性，区分轨迹有效性和轨迹连贯性。

Result: RL后训练总体上提高了轨迹连贯性，在基础模型失败但RL模型成功的问题上提升最显著。但RL增强了局部连贯性却未必产生有效或正确的解决方案。

Conclusion: 声称RL改善推理的主张需要谨慎审视，因为改善的轨迹连贯性可能无法转化为完全有效的数学证明，局部连贯性的提升不能保证最终答案正确性。
...
### [213] [FST.ai 2.0: An Explainable AI Ecosystem for Fair, Fast, and Inclusive Decision-Making in Olympic and Paralympic Taekwondo](https://arxiv.org/abs/2510.18193)
...
Method: 集成基于图卷积网络的姿势动作识别、通过置信集进行认知不确定性建模、可视化解释覆盖层，以及支持人机协作的交互式仪表板。

Result: 在比赛数据上的实验验证显示，决策审查时间减少85%，裁判对AI辅助决策的信任度达到93%。

Conclusion: 该框架建立了一个透明且可扩展的流程，用于可信赖的数据驱动裁判和运动员评估，代表了在体育中实现公平、负责任和以人为本的AI的一步。
...
### [214] [A Definition of AGI](https://arxiv.org/abs/2510.18212)
...
<details>
  <summary>Details</summary>
Motivation: 由于缺乏对AGI的具体定义，当前专业AI与人类水平认知之间的差距难以衡量。本文旨在通过量化框架来明确这一差距。

Method: 基于Cattell-Horn-Carroll认知理论，将通用智能分解为10个核心认知领域（包括推理、记忆、感知等），并采用成熟的人类心理测量工具来评估AI系统。
...
### [215] [ssToken: Self-modulated and Semantic-aware Token Selection for LLM Fine-tuning](https://arxiv.org/abs/2510.18250)
*Xiaohan Qin,Xiaoxing Wang,Ning Liao,Cancheng Zhang,Xiangdong Zhang,Mingquan Feng,Jingzhi Wang,Junchi Yan*

Main category: cs.AI

TL;DR: 提出ssToken方法，通过自调制和语义感知的token选择，解决了现有token级数据选择方法需要额外参考模型和仅依赖损失信息的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有token级选择方法存在两个关键限制：(1)需要训练或访问额外参考模型，(2)仅依赖损失信息进行token选择，无法很好保留语义重要但损失指标不偏好的token。

Method: ssToken利用历史模型计算当前模型的逐token损失差异作为自调制信号，并引入基于注意力的语义感知token重要性估计指标，提供互补的语义信息。

Result: 实验表明自调制选择和语义感知选择单独使用都优于全数据微调，两者结合的ssToken实现协同增益，超越现有token级选择方法，在保持训练效率的同时提升性能。

Conclusion: ssToken通过自调制和语义感知的token选择，有效解决了现有方法的局限性，在多个模型家族和规模上实现了性能提升和训练效率的平衡。
...
### [216] [Illusions of reflection: open-ended task reveals systematic failures in Large Language Models' reflective reasoning](https://arxiv.org/abs/2510.18254)
*Sion Weatherhead,Flora Salim,Aaron Belbasis*

Main category: cs.AI

TL;DR: 本文研究发现当前大语言模型的'反思'能力与人类反思推理在功能上不等价，模型在开放但有规则约束的任务中自我修正效果有限，经常重复相同的约束违反错误。


<details>
  <summary>Details</summary>
Motivation: 测试大语言模型在开放但有规则约束的真实任务中的反思和自我修正能力，探究其是否具有人类式的主动、目标驱动的监控机制。

Method: 在8个前沿模型上进行科学测试项目生成任务，要求模型首先生成有效项目，然后基于自身批评进行修订，评估反思前后的表现变化。

Result: 首次生成表现差（平均约1个有效项目），反思后仅有轻微改善（也约1个）。模型经常重复相同的约束违反，表明'修正收益'主要来自偶然生成有效项目而非错误检测和原则性修复。

Conclusion: 当前LLM的'反思'缺乏人类那种主动、目标驱动的监控机制，无法在首次尝试时就尊重约束。可靠性能需要外部结构来强制执行约束。
...
### [217] [Genesis: Evolving Attack Strategies for LLM Web Agent Red-Teaming](https://arxiv.org/abs/2510.18314)
*Zheng Zhang,Jiarui He,Yuchen Cai,Deheng Ye,Peilin Zhao,Ruili Feng,Hao Wang*

Main category: cs.AI

TL;DR: Genesis是一个针对网络代理攻击的新型框架，通过遗传算法和动态策略发现来持续改进攻击效果，在各种网络任务中表现优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型代理越来越多地自动化复杂网络任务，它们在提高生产力的同时也引入了新的安全风险。现有红队方法主要依赖手动构建的攻击策略或离线训练的静态模型，难以捕捉网络代理的行为模式，无法在不同环境中泛化。

Method: 提出Genesis框架，包含三个模块：攻击者（使用遗传算法和混合策略表示生成对抗注入）、评分器（评估目标网络代理的响应提供反馈）、策略师（从交互日志中动态发现有效策略并构建持续增长的战略库）。

Result: 在各种网络任务上的广泛实验表明，该框架能够发现新颖策略，并持续优于现有的攻击基线方法。

Conclusion: Genesis框架通过动态策略发现和持续改进机制，有效解决了网络代理攻击中的泛化问题，为自动化安全测试提供了新的解决方案。
...
### [218] [Earth AI: Unlocking Geospatial Insights with Foundation Models and Cross-Modal Reasoning](https://arxiv.org/abs/2510.18318)
...
TL;DR: 提出了Earth AI系列地理空间AI模型和智能推理系统，通过三个关键领域的基础模型和Gemini驱动的推理引擎，解决地理数据分析中的挑战。


<details>
  <summary>Details</summary>
Motivation: 地理数据具有巨大潜力，但其海量性、多样性、不同分辨率和稀疏性给深入分析带来挑战，需要新的AI方法来解锁对地球的新见解。

Method: 构建了三个关键领域的基础模型（行星尺度影像、人口、环境），并开发了Gemini驱动的智能推理引擎，能够联合推理多个基础模型和大型地理数据源。

Result: 在严格基准测试中展示了基础模型的强大能力和新颖特性，验证了模型组合使用的互补价值。在真实危机场景基准测试中，智能代理能够提供关键及时的洞察。

Conclusion: Earth AI系统有效弥合了原始地理数据与可操作理解之间的差距，为地理空间推理提供了强大的解决方案。
...
### [219] [ShortcutBreaker: Low-Rank Noisy Bottleneck with Global Perturbation Attention for Multi-Class Unsupervised Anomaly Detection](https://arxiv.org/abs/2510.18342)
...
TL;DR: 提出了ShortcutBreaker框架，通过低秩噪声瓶颈和全局扰动注意力机制解决多类无监督异常检测中的身份捷径问题，在多个数据集上取得优异性能


<details>
  <summary>Details</summary>
Motivation: 多类无监督异常检测需要统一模型检测多类异常，但Transformer架构存在身份捷径问题，即直接复制输入到输出，导致正常和异常样本的重构误差差异变小，难以区分

Method: 1. 基于矩阵秩不等式设计低秩噪声瓶颈(LRNB)，将高维特征投影到低秩潜在空间，防止平凡身份复制；2. 利用ViT的全局建模能力，引入全局扰动注意力防止解码器中的信息捷径

Result: 在四个基准数据集上测试：MVTec-AD(99.8%)、ViSA(98.9%)、Real-IAD(90.6%)、Universal Medical(87.8%)，图像级AUROC均优于现有MUAD方法

Conclusion: ShortcutBreaker通过有效防止身份捷径，在多类无监督异常检测任务中取得了显著性能提升，在不同场景下均优于现有方法
...
### [220] [Memory-Augmented State Machine Prompting: A Novel LLM Agent Framework for Real-Time Strategy Games](https://arxiv.org/abs/2510.18395)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法中LLM智能体在实时策略游戏中存在的幻觉问题和碎片化决策挑战，统一结构化动作与长期战术连贯性。

Method: 结合自然语言驱动的状态机架构和轻量级记忆模块，通过提示引导LLM模拟有限状态机和行为树，跨决策周期保存战略变量。

Result: 在星际争霸II中对战最强内置AI(Lv7)达到60%胜率，远超基线方法(0%)，案例研究显示方法保持LLM语义理解能力同时解决"知行差距"。

Conclusion: 建立了结合神经和符号AI在复杂决策中的新范式，通过严格状态-动作映射实现可解释性和类似FSM的可靠性。
...
### [221] [Heterogeneous Adversarial Play in Interactive Environments](https://arxiv.org/abs/2510.18407)
*Manjie Xu,Xinyi Yang,Jiayu Zhan,Wei Liang,Chi Zhang,Yixin Zhu*

Main category: cs.AI

TL;DR: 提出了HAP框架，通过对抗性自动课程学习实现教师-学生互动，解决传统自博弈在非对称学习场景中的不足


<details>
  <summary>Details</summary>
Motivation: 传统自博弈框架在零和竞争环境中依赖智能体对称性，但在开放式学习场景中无法处理固有的非对称性。人类教学系统展示了非对称教学框架的优势，需要将其操作化到人工智能系统中

Method: HAP框架将师生互动形式化为极小极大优化，任务生成教师和问题解决学生通过对抗动态共同进化。建立双向反馈系统，教师根据实时学习者表现重新校准任务复杂度

Result: 在多任务学习领域的实验验证表明，该框架达到SOTA基线性能水平，同时生成的课程提高了人工智能体和人类受试者的学习效能
...
### [222] [Deep Learning-Based Control Optimization for Glass Bottle Forming](https://arxiv.org/abs/2510.18412)
...
<details>
  <summary>Details</summary>
Motivation: 在玻璃瓶制造中，精确控制成型机器对确保质量和减少缺陷至关重要，需要优化实际生产环境中的成型过程。

Method: 使用来自实际制造工厂的运行数据，通过神经网络基于当前生产设置预测参数变化的影响，并设计专门的逆推机制来确定实现所需玻璃料滴特性的最优机器设置。
...
### [223] [Med-VRAgent: A Framework for Medical Visual Reasoning-Enhanced Agents](https://arxiv.org/abs/2510.18424)
*Guangfu Guo,Xiaoqian Lu,Yue Feng*

Main category: cs.AI

TL;DR: 提出了Med-VRAgent框架，结合视觉引导、自我奖励和蒙特卡洛树搜索，通过PPO微调提升医学视觉语言模型的推理能力


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在医学推理中存在幻觉、模糊描述、逻辑不一致和定位差的问题

Method: 基于视觉引导和自我奖励范式，结合蒙特卡洛树搜索，使用PPO目标对VLM进行微调

Result: 在多个医学VQA基准测试中优于现有方法

Conclusion: Med-VRAgent有效提升了医学视觉推理能力
...
### [224] [Automated urban waterlogging assessment and early warning through a mixture of foundation models](https://arxiv.org/abs/2510.18425)
*Chenxu Zhang,Fuxiang Huang,Lei Zhang*

Main category: cs.AI

TL;DR: 提出了UWAssess框架，使用基础模型自动识别监控图像中的积水区域并生成结构化评估报告，通过半监督微调和思维链提示策略解决标注数据稀缺问题。
...
Method: 采用基础模型驱动框架，结合半监督微调策略和思维链提示策略，利用多基础模型协作框架。
...
### [225] [AlphaOPT: Formulating Optimization Programs with Self-Improving LLM Experience Library](https://arxiv.org/abs/2510.18428)
...
TL;DR: AlphaOPT是一个自改进的经验库系统，让LLM能够从有限的演示（甚至只有答案）和求解器反馈中学习，无需标注推理轨迹或参数更新。
...
Result: 实验显示AlphaOPT随数据增加而稳定改进（从100到300训练项，性能从65%提升到72%），在仅使用答案训练时，在OptiBench数据集上比最强基线高出7.7%。

Conclusion: AlphaOPT能够高效地从有限演示中学习，通过更新库而非模型权重实现持续扩展，使知识显式化且可解释。
...
### [226] [PlanU: Large Language Model Decision Making through Planning under Uncertainty](https://arxiv.org/abs/2510.18442)
*Ziwei Deng,Mian Deng,Chenjing Liang,Zeming Gao,Chennan Ma,Chenxing Lin,Haipeng Zhang,Songzhu Mei,Cheng Wang,Siqi Shen*

Main category: cs.AI

TL;DR: PlanU是一种基于LLM的规划方法，通过将蒙特卡洛树搜索中的节点回报建模为分位数分布来捕捉不确定性，并使用带好奇心的上置信界分数来平衡探索与利用。


<details>
  <summary>Details</summary>
Motivation: LLM在不确定性下的决策任务中表现不佳，特别是在具有随机状态转换的环境中。现有方法要么只处理LLM不确定性而忽略环境不确定性，要么不适用于需要与环境交互的多步决策任务。

Method: 将蒙特卡洛树搜索中的节点回报建模为分位数分布，使用一组分位数表示回报分布。引入带好奇心的上置信界分数来估计MCTS节点的不确定性，平衡探索与利用。

Result: 通过广泛实验证明了PlanU在基于LLM的不确定性决策任务中的有效性。

Conclusion: PlanU通过结合分位数分布和UCC分数，有效解决了LLM决策中的不确定性挑战，在随机环境中表现出色。
...
### [227] [CircuitSeer: Mining High-Quality Data by Probing Mathematical Reasoning Circuits in LLMs](https://arxiv.org/abs/2510.18470)
*Shaobo Wang,Yongliang Miao,Yuancheng Liu,and Qianli Ma,Ning Liao,Linfeng Zhang*

Main category: cs.AI

TL;DR: CircuitSeer是一种基于模型内部注意力机制的数据选择方法，通过识别推理任务中激活的关键注意力头来量化数据复杂度，仅用10%数据就能超越全数据集训练效果。


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法依赖昂贵的外部模型或启发式方法，而本文转向研究模型内部机制，发现复杂推理任务会激活稀疏的专用注意力头形成核心推理电路。

Method: 提出CircuitSeer方法，通过测量数据对关键推理电路的影响来量化推理复杂度，基于模型内部注意力机制进行数据选择。

Result: 在4个模型和9个数据集上的实验显示，CircuitSeer方法仅用10%数据就能使Qwen2.5-Math-7B模型在平均Pass@1指标上比全数据集训练提升1.4个百分点。

Conclusion: CircuitSeer证明了利用模型内部机制进行数据选择的有效性，能够在显著减少计算成本的同时提升模型性能。
...
### [228] [Probabilistic Modeling of Intentions in Socially Intelligent LLM Agents](https://arxiv.org/abs/2510.18476)
*Feifan Xia,Yuyang Fang,Defang Li,Yantong Xie,Weikang Li,Yang Li,Deguo Xia,Jizhou Huang*

Main category: cs.AI

TL;DR: 提出了一种用于多轮社交对话中LLM代理的概率意图建模框架，通过维护对伙伴潜在意图的信念分布并动态更新，提升对话策略的适应性


<details>
  <summary>Details</summary>
Motivation: 为LLM代理在多轮社交对话中提供更好的意图理解和适应性，解决不确定性环境下的对话策略问题

Method: 基于上下文先验初始化伙伴潜在意图的信念分布，通过似然估计在每轮对话后动态更新该分布，为策略提供额外上下文基础
...
### [229] [LAFA: Agentic LLM-Driven Federated Analytics over Decentralized Data Sources](https://arxiv.org/abs/2510.18477)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM代理的分析框架假设集中式数据访问，缺乏隐私保护；而联邦分析(FA)虽然支持隐私保护计算，但缺乏自然语言输入支持，需要结构化查询。

Method: 引入分层多代理架构：粗粒度规划器分解复杂查询为子查询，细粒度规划器将每个子查询映射为FA操作的有向无环图，优化器代理重写和合并多个DAG以消除冗余操作。

Result: 实验表明LAFA持续优于基线提示策略，实现更高的执行计划成功率，并大幅减少资源密集型FA操作。
...
### [230] [StarBench: A Turn-Based RPG Benchmark for Agentic Multimodal Decision-Making and Information Seeking](https://arxiv.org/abs/2510.18483)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在真实客户端环境中，将原始屏幕截图映射到时间一致的低级动作并决定何时寻求指导的能力仍是一个开放挑战。

Method: 设计了两种评估机制：直接控制（仅接收截图并输出低级操作原语）和工具辅助控制（允许使用检测器和OCR输出）。还包括询问或行动诊断来测量代理何时选择请求指导。

Result: 结果显示在直接控制机制下感知到控制的保真度存在显著差距，而明智的信息寻求与改进的成功率相关。
...
### [231] [AndroidControl-Curated: Revealing the True Potential of GUI Agents through Benchmark Purification](https://arxiv.org/abs/2510.18488)
*Ho Fai Leung,Xiaoyan Xi,Fei Zuo*

Main category: cs.AI

TL;DR: 研究发现现有GUI代理基准测试AndroidControl存在缺陷，通过改进得到AndroidControl-Curated基准，在该基准上模型性能提升15%达到75%。同时开发了仅需60美元训练成本的3B参数模型Magma-R1，性能媲美235B参数模型。


<details>
  <summary>Details</summary>
Motivation: 解决现有虚拟助手依赖刚性API的问题，以及纠正现有基准测试对GUI代理能力的低估，推动设备端虚拟助手的发展。

Method: 识别AndroidControl基准的模糊性和事实错误，通过严格净化流程改进为AndroidControl-Curated基准；使用2.4k精选样本在H20 GPU上训练60小时开发Magma-R1模型。

Result: 在改进后的基准上，最先进模型在复杂任务上的成功率接近75%（提升15%）；Magma-R1模型仅3B参数但性能与Qwen3-VL-235B相当，训练成本仅约60美元。

Conclusion: 设备端GUI代理的实际能力被现有基准低估，通过改进基准和高效模型训练，证明了设备端虚拟助手比预想的更接近实际部署。
...
### [232] [Crucible: Quantifying the Potential of Control Algorithms through LLM Agents](https://arxiv.org/abs/2510.18491)
...
Method: 开发了Crucible代理，采用LLM驱动的多级专家模拟方法来调优算法，并定义了形式化的指标来定量评估调优潜力。

Result: 在从经典控制任务到复杂计算机系统的广泛案例研究中验证了Crucible的有效性，并在实际部署中验证了其发现。实验结果表明Crucible能够系统量化不同算法的可调优空间。
...
### [233] [Counterfactual Reasoning for Steerable Pluralistic Value Alignment of Large Language Models](https://arxiv.org/abs/2510.18526)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地服务于不同文化背景的用户，需要超越平均原则（如HHH）来与多元化的人类价值观对齐。现有方法在处理细粒度价值目标时面临两个挑战：价值观复杂性和价值观可引导性。

Method: 提出COUPLE框架，使用结构因果模型(SCM)来表征特征间的复杂相互依赖关系和优先级，以及高层价值维度与行为之间的因果关系。应用反事实推理来生成与任何期望价值目标对齐的输出。

Result: 在两个具有不同价值体系的数据集上评估COUPLE，结果表明COUPLE在各种类型价值目标上都优于其他基线方法。

Conclusion: COUPLE通过显式因果建模不仅提升了性能，还提供了更好的可解释性，能够有效处理多元化价值观对齐的挑战。
...
### [234] [Physics-guided Emulators Reveal Resilience and Fragility under Operational Latencies and Outages](https://arxiv.org/abs/2510.18535)
*Sarth Dubey,Subimal Ghosh,Udit Bhatia*

Main category: cs.AI

TL;DR: 开发了一个具有操作弹性的洪水预报系统模拟器，结合长短时记忆网络和松弛水平衡约束，在数据延迟、缺失或不一致的情况下仍能保持稳定性能。


<details>
  <summary>Details</summary>
Motivation: 当前水文和洪水预报模型大多在理想数据条件下评估，强调准确性而非操作弹性。需要开发在真实操作环境中可靠的预报系统。

Method: 使用长短时记忆网络与松弛水平衡约束耦合的方法，构建了五种不同信息可用性场景的架构，从完整历史预报到数据延迟和中断情况。

Result: 在美国5000多个流域测试，包括印度高度调控河流，模拟器能复现GloFAS水文核心功能，在信息质量下降时性能平滑退化。跨不同水文气候和管理体制的迁移产生性能降低但仍保持物理一致性。
...
### [235] [SOCIA-Nabla: Textual Gradient Meets Multi-Agent Orchestration for Automated Simulator Generation](https://arxiv.org/abs/2510.18551)
*Yuncheng Hua,Sion Weatherhead,Mehdi Jafari,Hao Xue,Flora D. Salim*

Main category: cs.AI

TL;DR: SOCIA-Nabla是一个端到端的智能框架，将模拟器构建视为文本计算图中的代码实例优化，通过LLM驱动的智能体节点和基于损失的循环（代码合成→执行→评估→代码修复）实现自动优化。


<details>
  <summary>Details</summary>
Motivation: 将脆弱的提示管道转换为可重现、约束感知的模拟器代码生成，统一多智能体编排与损失对齐的优化视图，减少专家工作量。

Method: 在文本计算图中嵌入专门的LLM驱动智能体作为节点，工作流管理器执行损失驱动的循环，优化器执行文本梯度下降(TGD)，人类仅在任务规范确认时参与。

Result: 在三个CPS任务（用户建模、口罩采用、个人移动性）中实现了最先进的整体准确率。

Conclusion: SOCIA-Nabla通过将代码本身作为可训练对象，实现了跨领域和模拟粒度的可扩展模拟器代码生成，将脆弱的提示管道转换为可重现的约束感知系统。
...
### [236] [Extracting alignment data in open models](https://arxiv.org/abs/2510.18554)
...
Method: 使用高质量嵌入模型来衡量字符串之间的语义相似性，而非传统的字符串匹配方法，因为后者会严重低估可提取的数据量。

Result: 发现模型容易重现后训练阶段使用的数据，这些数据可用于训练基础模型，恢复相当部分的原始性能。近似字符串匹配会严重低估（保守估计10倍）可提取的数据量。

Conclusion: 这项工作暴露了提取对齐数据的潜在风险，并引发了对蒸馏实践下游影响的讨论：由于模型会重现其训练集的某些方面，蒸馏可被视为间接使用模型原始数据集进行训练。
...
### [237] [QuantEvolve: Automating Quantitative Strategy Discovery through Multi-Agent Evolutionary Framework](https://arxiv.org/abs/2510.18569)
...
Method: 使用与投资者偏好对齐的特征图来维持多样化的有效策略集，并整合假设驱动的多智能体系统，通过迭代生成和评估来系统性地探索策略空间。

Result: 实证结果显示QuantEvolve优于传统基线方法，验证了其有效性。

Conclusion: 该框架能够产生多样化、复杂的策略，适应市场状态变化和个体投资需求，并发布了进化策略数据集以支持未来研究。
...
### [238] [VAR: Visual Attention Reasoning via Structured Search and Backtracking](https://arxiv.org/abs/2510.18619)
*Wei Cai,Jian Zhao,Yuchen Yuan,Tianle Zhang,Ming Zhu,Haichuan Tang,Chi Zhang,Xuelong Li*

Main category: cs.AI

TL;DR: 提出VAR框架，将多模态推理重构为结构化搜索过程，通过可追溯证据基础和回溯机制解决MLLM的幻觉问题，在多个基准测试中达到SOTA性能。
...
Method: 将推理过程分解为可追溯证据基础和基于搜索的思维链生成两个阶段，引入回溯机制进行自我修正，使用包含语义和几何自验证组件的多面奖励函数。

Result: VAR-7B模型在幻觉和安全基准测试中创下新纪录，显著优于现有开源模型，与领先专有系统性能相当。

Conclusion: VAR框架通过结构化搜索和自验证机制有效解决了MLLM的幻觉问题，为多模态推理提供了更可靠的方法。
...
### [239] [Leveraging Association Rules for Better Predictions and Better Explanations](https://arxiv.org/abs/2510.18628)
*Gilles Audemard,Sylvie Coste-Marquis,Pierre Marquis,Mehdi Sabiri,Nicolas Szczepanski*
...
Conclusion: 结合数据挖掘得到的关联规则可以有效提升树模型的预测能力和解释质量，为分类任务提供更好的性能和可解释性。
...
### [240] [Comparative Expressivity for Structured Argumentation Frameworks with Uncertain Rules and Premises](https://arxiv.org/abs/2510.18631)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注抽象的不确定性论证模型，但缺乏对这些抽象模型的具体实例化研究。本文旨在填补这一空白，通过将论证的不确定性基于其组成部分（规则和前提）来研究结构化模型。

Method: 将论证的不确定性基于其组成部分（规则和前提）进行结构化建模，引入新的表达能力概念来处理抽象和结构化形式主义，比较抽象论证框架（包括不完整框架和依赖扩展）与结构化模型（ASPIC+）的表达能力。

Result: 提出了能够处理抽象和结构化形式主义的表达能力概念，并给出了负面和正面的表达能力结果，比较了抽象模型和结构化模型在不确定性论证中的表达能力差异。

Conclusion: 本文通过研究抽象和结构化不确定性论证模型的表达能力，为形式论证中定性不确定性的建模提供了理论基础，并展示了不同类型模型在表达能力方面的特点和局限性。
...
### [241] [Query Decomposition for RAG: Balancing Exploration-Exploitation](https://arxiv.org/abs/2510.18633)
*Roxana Petcu,Kenton Murray,Daniel Khashabi,Evangelos Kanoulas,Maarten de Rijke,Dawn Lawrie,Kevin Duh*

Main category: cs.AI

TL;DR: 本文提出了一种基于多臂老虎机框架的检索增强生成系统，通过动态选择信息量最大的子查询来平衡检索的广度与噪声控制，显著提升了文档检索精度和下游生成任务性能。


<details>
  <summary>Details</summary>
Motivation: 解决RAG系统中在分解复杂查询时面临的权衡问题：既要检索足够广泛以捕获所有相关材料，又要限制检索以避免过多噪声和计算成本。
...
Conclusion: 基于老虎机学习框架的查询分解和文档检索方法能够有效平衡检索的广度与精度，显著提升RAG系统的整体性能。
...
### [242] [Sherlock Your Queries: Learning to Ask the Right Questions for Dialogue-Based Retrieval](https://arxiv.org/abs/2510.18659)
...
<details>
  <summary>Details</summary>
Motivation: 信息检索中的用户查询往往存在歧义，现有对话式交互检索系统缺乏明确的策略来询问最具信息量的问题，导致效率低下。

Method: 使用强化学习训练智能体生成二进制问题序列，通过对话策略逐步缩小搜索范围，避免对大规模标注数据的依赖。

Result: 在结构化任务中性能与强基线相当，接近二分搜索的理论最优值；在非结构化任务中显著优于基线，展示了高效信息寻求对话策略的学习能力。

Conclusion: SherlockLLM是一个稳健高效的解决方案，能够学习高度有效的信息寻求对话策略，在具有挑战性的非结构化任务中表现突出。
...
### [243] [Seg the HAB: Language-Guided Geospatial Algae Bloom Reasoning and Segmentation](https://arxiv.org/abs/2510.18751)
...
Method: 整合GeoSAM辅助人工评估进行高质量分割掩码标注，并在NASA的CAML数据集上微调视觉语言模型进行严重程度预测
...
### [244] [Decoding Funded Research: Comparative Analysis of Topic Models and Uncovering the Effect of Gender and Geographic Location](https://arxiv.org/abs/2510.18803)
...
<details>
  <summary>Details</summary>
Motivation: 需要了解不断演变的研究趋势以及影响这些趋势的人口和地理因素，特别是在促进公平、多样性和包容性的背景下，以优化国家科学投资。

Method: 分析了2005-2022年NSERC资助的研究提案，比较了LDA、STM和BERTopic三种主题建模方法，并开发了COFFEE算法用于BERTopic的协变量效应估计。

Result: 所有模型都能有效识别核心科学领域，但BERTopic表现最佳，能够识别更细粒度、更连贯和新兴的主题（如人工智能的快速发展）。COFFEE协变量分析确认了各省的研究专业化模式，并揭示了跨学科的一致性别主题模式。

Conclusion: 这些发现为资助机构制定更公平和有效的资助策略提供了坚实的实证基础，从而提升科学生态系统的效率。
...
### [245] [EVER: Edge-Assisted Auto-Verification for Mobile MR-Aided Operation](https://arxiv.org/abs/2510.18224)
...
<details>
  <summary>Details</summary>
Motivation: 解决混合现实中用户是否遵循MR指导的精确快速自动验证问题，现有方法无法处理物理对象与虚拟对象之间的差异。

Method: 利用分割模型和渲染管道处理物理与虚拟帧的独特属性，采用基于IoU指标的阈值策略进行验证，将计算密集型任务卸载到边缘服务器。

Result: 在公开数据集和自定义数据集上评估，EVER在100毫秒内达到90%以上的验证准确率，显著快于人类平均反应时间，且仅消耗极少额外计算资源和能量。

Conclusion: EVER系统能够高效准确地验证混合现实操作，为MR辅助操作提供了实用的自动验证解决方案。
...
### [246] [How2Compress: Scalable and Efficient Edge Video Analytics via Adaptive Granular Video Compression](https://arxiv.org/abs/2510.18409)
*Yuheng Wu,Thanh-Tung Nguyen,Lucas Liebe,Quang Tau,Pablo Espinosa Campos,Jinghan Cheng,Dongman Lee*

Main category: cs.MM

TL;DR: How2Compress是一个简单有效的视频压缩框架，通过宏块级别的精细质量控制提升压缩效率，在H.264编解码器上实现高达50.4%的比特率节省。


<details>
  <summary>Details</summary>
Motivation: 现有视频压缩框架未能充分利用现代基于块的视频编解码器支持的精细质量控制，导致压缩效率未被充分挖掘。

Method: 提出How2Compress框架，在宏块级别进行精确的精细质量控制，可作为即插即用模块无缝集成到现有边缘视频分析流水线中。

Result: 实验结果显示，How2Compress在多种真实场景下实现高达50.4%的比特率节省，性能比基线方法提升高达3.01倍，且不影响分析精度。

Conclusion: How2Compress展示了在实际应用中提升视频压缩效率的有效性和实用性。
...
### [247] [DeLoad: Demand-Driven Short-Video Preloading with Scalable Watch-Time Estimation](https://arxiv.org/abs/2510.18459)
*Tong Liu,Zhiwei Fan,Guanyan Peng,Haodan Zhang,Yucheng Zhang,Zhen Wang,Pengjin Xie,Liang Liu*

Main category: cs.MM

TL;DR: DeLoad是一个针对短视频流媒体的预加载框架，通过动态任务大小调整、多维观看时间估计和深度强化学习优化，显著提升了用户体验质量并降低了带宽消耗。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个关键问题：(1)下载任务大小无法适应动态条件；(2)观看时间预测模型难以在规模化部署中可靠运行。这些问题影响了短视频平台的用户体验和带宽效率。

Method: 提出DeLoad框架，包含动态任务大小调整、实用的多维观看时间估计方法，以及使用深度强化学习训练的自适应下载范围决策代理。

Result: 离线测试显示QoE指标提升34.4%到87.4%。在实际商业平台部署后，用户观看时间增加0.09%，同时减少卡顿事件和3.76%的带宽消耗。

Conclusion: DeLoad通过创新的动态预加载策略，成功平衡了用户体验和带宽效率，为短视频流媒体平台提供了有效的技术解决方案。
...
### [248] [PIRA: Pan-CDN Intra-video Resource Adaptation for Short Video Streaming](https://arxiv.org/abs/2510.18606)
*Chunyu Qiao,Tong Liu,Yucheng Zhang,Zhiwei Fan,Pengjin Xie,Zhen Wang,Liang Liu*

Main category: cs.MM

TL;DR: PIRA是一个动态CDN资源选择算法，在短视频播放中实时优化用户体验质量和成本，通过控制理论方法在视频内平衡QoE和成本。


<details>
  <summary>Details</summary>
Motivation: CDN资源选择对维持用户体验质量和控制流量成本至关重要，但现有研究在短视频流媒体背景下对成本约束下的高QoE维持问题研究不足。

Method: 提出PIRA算法，通过数学模型整合QoE和成本，采用视频内控制理论CDN资源选择方法，并运用状态空间剪枝和自适应参数调整来高效解决高维优化问题。

Result: 在涉及45万用户的两周大规模生产实验中，PIRA相比生产基线实现了2.1%的启动延迟降低、15.2%的重缓冲时间缩短和10%的平均单位流量成本降低。

Conclusion: PIRA在大规模环境下有效平衡了用户体验和财务成本，证明了其在动态网络条件下优化CDN资源选择的实用性。
