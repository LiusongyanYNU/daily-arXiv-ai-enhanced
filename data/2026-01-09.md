...
TL;DR: 提出CounterVid框架，通过生成反事实视频来缓解视频语言模型的幻觉问题，特别是针对动作识别和时间推理任务


<details>
  <summary>Details</summary>
Motivation: 现有的视频语言模型容易产生幻觉，尤其是在推理动作和时间顺序时。现有的缓解策略（如文本过滤或随机视频扰动）未能解决根本原因：过度依赖语言先验而非细粒度的视觉动态

Method: 提出可扩展的反事实视频生成框架，结合多模态LLM进行动作提案和编辑指导，使用基于扩散的图像和视频模型大规模生成语义硬负样本。构建CounterVid数据集（约26k偏好对），并引入MixDPO统一直接偏好优化方法

Result: 使用MixDPO微调Qwen2.5-VL模型在时间排序等方面获得一致改进，并能有效迁移到标准视频幻觉基准测试中

Conclusion: 提出的CounterVid框架和MixDPO方法能有效缓解视频语言模型的幻觉问题，特别是在动作和时间推理方面，代码和模型将公开
...
TL;DR: 该论文提出了复杂偏好优化(CPO)框架，通过构建层次化细粒度评估标准，将扩散模型与复杂的人类专业知识对齐，显著提升了绘画生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型的后训练对齐方法依赖简化的信号（如标量奖励或二元偏好），无法与层次化、细粒度的人类专业知识对齐，限制了生成质量。

Method: 1) 与领域专家合作构建层次化细粒度评估标准，将图像质量分解为树状结构的正负属性；2) 提出两阶段对齐框架：先通过监督微调将领域知识注入辅助扩散模型，再通过复杂偏好优化(CPO)将目标扩散模型与非二元层次化标准对齐。

Result: 在绘画生成领域的实验表明，CPO显著提升了生成质量和与专业知识的对齐度，为细粒度标准对齐开辟了新途径。

Conclusion: 该研究提出的复杂偏好优化框架能够有效将扩散模型与层次化细粒度的人类专业知识对齐，显著改善生成质量，为复杂标准对齐提供了新方法。
...
TL;DR: 提出一种基于RGB空间五进制像素强度组合的文本嵌入图像新方法，通过单像素编码完整字符，相比传统LSB等方法具有更高效率和更低失真。


<details>
  <summary>Details</summary>
Motivation: 现有文本嵌入图像方法（如LSB、PVD、变换域方法、深度学习等）存在多个问题：依赖多像素强度翻转、产生噪声、计算量大、编码解码确定性高。需要一种更高效、失真更小、计算更轻量的方法。

Method: 在RGB空间中采用五进制像素强度组合技术，每个R、G、B通道使用五种受控的强度变化，形成125种不同的像素强度组合。将这些组合映射到文本符号（大小写字母、数字、空格、常用特殊字符），实现单像素编码完整字符。

Result: 通过MSE、MAE、SNR、PSNR、SSIM、直方图比较和热图分析等多种指标评估，编码图像与原始图像相比没有显著失真。方法实现了更高的嵌入效率，单像素即可编码完整文本符号。

Conclusion: 提出的五进制像素强度组合方法在RGB空间中有效嵌入文本数据，相比传统LSB、MSB、变换域和深度学习方法，具有更高的嵌入效率、更低的计算开销和更好的图像保真度。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一多模态生成架构通常需要显式的模态切换，先生成推理文本再手动切换到图像生成，这种分离的顺序推理过程限制了跨模态耦合，无法实现自动的多模态生成。
...
Conclusion: 通过针对特定限制的有针对性数据集的后训练方法，能够实现完全统一的文本-图像生成，相比广泛的图像-标题语料库或基准对齐数据获得更优结果。
...
TL;DR: ReHyAt提出了一种循环混合注意力机制，结合了softmax注意力的保真度和线性注意力的效率，将注意力成本从二次降低到线性，实现了长视频生成的实用可扩展性。
...
Method: ReHyAt（循环混合注意力）结合softmax注意力的保真度和线性注意力的效率，通过分块循环重构实现恒定内存使用，并设计了轻量级蒸馏和微调流程，可从现有的softmax模型高效蒸馏。

Result: ReHyAt在VBench和VBench-2.0评估中表现出最先进的视频质量，同时将注意力成本从二次降低到线性，训练成本降低两个数量级至约160 GPU小时，在人类偏好研究中表现优异。

Conclusion: ReHyAt提供了一种高效实用的视频生成解决方案，解锁了长时长和设备端视频生成的实用可扩展性，其蒸馏流程可应用于未来最先进的双向softmax模型。
...
TL;DR: 提出一种基于残差向量量化的渐进式3D高斯泼溅编码器，通过自回归熵模型和多分辨率哈希网格提高压缩效率


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅模型存储需求大，阻碍云端和流媒体部署；标量量化方法无法充分利用高维特征向量相关性，限制了率失真性能

Method: 使用残差向量量化替代传统方法压缩基元特征，结合自回归熵模型和多分辨率哈希网格预测条件概率，实现高效的粗粒度和细化层压缩

Result: 未在摘要中明确说明具体结果，但暗示该方法能更有效地压缩3D高斯泼溅模型

Conclusion: 提出的渐进式编码器通过更强大的量化方法和熵模型，有望显著提升3D高斯泼溅模型的压缩效率
...
<details>
  <summary>Details</summary>
Motivation: 为孟加拉国特定应用场景（如人行道视觉、三轮车检测、芒果分类、水稻品种识别、道路损伤检测）提供深度学习模型选择指导，帮助从业者根据数据集特性、计算资源和性能需求选择合适的深度学习方法。

Method: 使用自定义CNN与预训练架构（ResNet-18和VGG-16）进行对比分析，采用特征提取和迁移学习两种方法，在五个孟加拉国图像分类数据集上进行实验评估。

Result: 微调迁移学习在所有数据集上表现最佳，准确率比自定义CNN和特征提取方法提高3-76%；ResNet-18微调在道路损伤数据集上达到100%准确率；自定义CNN在模型大小（340万参数）和简单任务训练效率上有优势。

Conclusion: 对于复杂分类任务和有限训练数据，预训练模型加迁移学习效果最好；自定义CNN适合简单任务和资源受限场景；研究为从业者根据具体需求选择合适方法提供了实用指导。
...
TL;DR: PackCache：一种训练免费的KV缓存管理方法，通过条件锚定、跨帧衰减建模和空间保留位置嵌入，动态压缩KV缓存，加速统一自回归视频生成


<details>
  <summary>Details</summary>
Motivation: 统一自回归模型（如Transformer）在处理多模态任务时，KV缓存大小随生成token数量线性增长，成为限制推理效率和生成长度的主要瓶颈。视频生成继承了这一限制，需要更高效的缓存管理方法。

Method: PackCache通过三个协调机制动态压缩KV缓存：1) 条件锚定：保留文本和条件图像等语义锚点；2) 跨帧衰减建模：根据时间距离分配缓存预算；3) 空间保留位置嵌入：在缓存移除时保持连贯的3D结构

Result: 在48帧长序列上，PackCache将端到端生成加速1.7-2.2倍。对于受KV缓存扩展影响最大的最后四帧，在A40和H200上分别实现2.6倍和3.7倍的加速

Conclusion: PackCache通过利用KV缓存token的时空特性，有效解决了统一自回归视频生成中的KV缓存瓶颈问题，显著提升了长序列视频生成的效率
...
TL;DR: 该论文提出使用3D面部几何特征进行压力识别，通过Transformer框架结合多模态数据，在分心驾驶场景中实现了高精度的压力检测。


<details>
  <summary>Details</summary>
Motivation: 现有压力识别方法主要依赖面部动作单元，但3D面部几何特征的作用尚未充分探索。压力具有主观性和面部自主控制的特点，使得从面部视频中可靠识别压力具有挑战性。

Method: 1) 使用EMOCA提取3D表情和姿态系数；2) 通过配对假设检验分析基线和压力阶段的差异；3) 提出基于Transformer的时间建模框架，评估单模态、早期融合和跨模态注意力策略；4) 将EMOCA特征与生理信号、注视数据进行融合。

Result: 1) 56个系数中有41个显示出与生理标记相当的相位特异性压力响应；2) EMOCA与生理信号的跨模态注意力融合获得最佳性能（AUROC 92%，准确率86.7%）；3) EMOCA与注视融合也表现良好（AUROC 91.8%）。

Conclusion: 时间建模和跨模态注意力策略对于压力识别非常有效，3D面部几何特征可以作为可靠的生理压力指标，为分心驾驶等场景的压力检测提供了新方法。
...
TL;DR: 通过少量配对样本，使用LoRA微调FLUX.1 Kontext模型实现RGB到红外/雷达的跨光谱图像翻译，生成合成数据提升下游目标检测性能


<details>
  <summary>Details</summary>
Motivation: 视觉基础模型主要基于RGB数据训练，但许多安全关键应用依赖红外和合成孔径雷达等非可见光模态。研究如何利用少量配对样本将RGB预训练模型重新用于跨光谱翻译，并验证合成数据能否增强下游检测任务

Method: 基于FLUX.1 Kontext模型，插入LoRA模块，仅用每个域100张配对图像进行微调。研究两个设置：KAIST数据集上的RGB到红外翻译，以及M4-SAR数据集上的RGB到SAR翻译。使用LPIPS指标作为下游性能的代理指标

Result: LPIPS在50个保留配对上的计算结果是下游性能的强代理指标：较低的LPIPS一致预测较高的mAP。最佳LoRA适配器生成的合成红外数据提升了KAIST红外行人检测，合成SAR数据与有限真实SAR结合显著提升了M4-SAR基础设施检测

Conclusion: 少量样本的LoRA适配流匹配基础模型是实现非可见光模态基础模型支持的有前景路径，能够生成高质量的跨光谱合成数据以增强下游检测任务
...
<details>
  <summary>Details</summary>
Motivation: 在图像分类任务中，选择自定义CNN还是使用预训练架构是一个重要的实践问题。需要系统比较两者的性能差异，为实际应用提供指导。

Method: 设计一个自定义CNN并从头训练，同时使用VGG-16、ResNet-50和MobileNet等流行架构进行迁移学习。所有模型在相同实验设置下评估，使用准确率、精确率、召回率和F1分数等标准指标。

Result: 预训练CNN架构在分类准确率和收敛速度上始终优于自定义CNN，特别是在训练数据有限的情况下。但自定义CNN以显著更少的参数和降低的计算复杂度实现了有竞争力的性能。

Conclusion: 研究揭示了模型复杂度、性能和计算效率之间的权衡，为图像分类问题选择合适的CNN架构提供了实用见解。预训练模型在性能上占优，而自定义CNN在资源受限场景下更具优势。
...
Result: 在Objaverse、LVIS、Objaverse XL和ABO数据集上的实验表明，TriMARF显著优于现有方法：CLIPScore达到88.7，ViLT R@5检索准确率为45.2和43.8，在单张NVIDIA A100 GPU上吞吐量高达12000个物体/小时。

Conclusion: TriMARF通过多模态融合和多智能体协作，有效解决了3D标注中的复杂挑战，在标注质量和效率方面都取得了显著提升，为大规模3D物体标注提供了有效的解决方案。
...
TL;DR: 提出混合自监督和弱监督学习框架，直接从术前CT扫描预测乳突切除术区域，无需人工标注，为人工耳蜗手术规划提供解决方案。
...
Method: 提出混合自监督和弱监督学习框架，结合两种学习范式，直接从术前完整乳突的CT扫描预测乳突切除术区域，并采用3D T分布损失函数。

Result: 混合方法在预测复杂无边界乳突切除术形状时达到平均Dice分数0.72，优于现有方法，为从术前CT直接构建术后3D表面提供基础。

Conclusion: 这是首个将自监督和弱监督学习结合用于乳突切除术形状预测的工作，为人工耳蜗手术规划提供了鲁棒高效的解决方案，同时推进了弱监督医学影像分析。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在心脏MRI重建中表现出色，但现有方法泛化能力有限。心脏MRI扫描存在图像对比度、采样模式、扫描仪厂商、解剖结构和疾病类型等多方面差异，现有模型通常只能处理单一或有限变化，面对分布偏移时性能下降。因此需要开发能够泛化到多样化心脏MRI场景的统一模型。

Method: 提出CRUNet-MR-Univ基础模型，利用时空相关性和提示先验来处理心脏MRI扫描的完整多样性。模型采用基于提示的机制来适应不同的扫描条件和变化。

Result: 提出的方法在广泛设置下持续优于基线方法，证明了其有效性和前景。

Conclusion: CRUNet-MR-Univ作为基础模型，通过整合时空相关性和提示先验，能够有效处理心脏MRI重建中的多样性问题，展现出优异的泛化能力和临床应用潜力。
...
TL;DR: GPRO提出一种门控感知-推理优化方法，通过元推理控制器动态选择计算路径，解决大视觉语言模型中过度思考问题，提升准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有大视觉语言模型的链式思维机制容易导致"过度思考"问题，即对简单查询生成冗长响应，降低测试效率和准确性。先前工作主要关注自适应推理策略，但忽略了视觉感知失败这一根本瓶颈。

Method: 提出GPRO（门控感知-推理优化），一个元推理控制器，在每个生成步骤动态路由计算到三条路径：轻量快速路径、重新检查视觉输入的慢感知路径、内部自我反思的慢推理路径。通过从79万样本中提取失败归因监督，使用教师模型区分感知幻觉和推理错误，然后用多目标强化学习训练控制器优化准确性和计算成本之间的权衡。

Result: 在五个基准测试上的实验表明，GPRO显著提高了准确性和效率，优于最近的慢思考方法，同时生成明显更短的响应。

Conclusion: 稳定的推理关键依赖于低层次视觉基础，推理错误往往源于不完美的感知而非不足的深思熟虑。GPRO通过动态路由计算路径有效解决了过度思考问题，实现了准确性和效率的双重提升。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统通常将感知、预测和规划作为独立模块处理，缺乏统一的世界模型。现有基于视觉语言模型的规划方法也存在模块分离的问题，需要更紧密的集成来提升自动驾驶性能。

Method: 提出UniDrive-WM统一架构：1）轨迹规划器预测未来轨迹；2）基于VLM的图像生成器根据轨迹条件生成未来帧；3）预测结果提供额外监督信号，增强场景理解并迭代优化轨迹生成；4）比较离散和连续输出表示对未来图像预测的影响。
...
TL;DR: 提出LLM驱动的森林变化分析智能体，整合多级变化解释视觉语言骨干网络，支持自然语言查询，在森林变化数据集上取得良好性能。
...
Method: 构建基于多级变化解释视觉语言骨干网络的LLM驱动智能体系统，引入包含双时相卫星影像、像素级变化掩码和多粒度语义变化描述的Forest-Change数据集。
...
<details>
  <summary>Details</summary>
Motivation: 3D医学图像分割计算需求高，存在体素处理立方增长和同质区域冗余计算的问题。需要开发既能保持分割精度又能显著降低计算成本的高效方法。
...
Result: 在960例3D乳腺DCE-MRI数据集上，TokenSeg达到94.49% Dice和89.61% IoU的SOTA性能，同时GPU内存和推理延迟分别降低64%和68%。在MSD心脏和脑MRI基准数据集上验证了跨异质解剖结构的泛化能力。
...
TL;DR: 提出FaceRefiner方法，通过风格迁移和多层次信息传递来改进3D面部纹理生成，保留输入图像的细节、结构和身份信息


<details>
  <summary>Details</summary>
Motivation: 现有面部纹理生成方法通常基于训练数据或2D生成器构建的UV贴图空间，这限制了其对真实世界输入图像的泛化能力，导致生成的面部细节、结构和身份与输入不一致

Method: 提出FaceRefiner方法，将3D采样纹理作为风格，纹理生成方法输出作为内容，通过风格迁移传递多层次信息。创新性地结合可微分渲染，在可见面部区域传递像素级低层次信息，实现多层次信息传递

Result: 在Multi-PIE、CelebA和FFHQ数据集上的广泛实验表明，该方法相比现有技术能提高纹理质量和面部身份保持能力

Conclusion: FaceRefiner通过风格迁移和多层次信息传递，有效解决了现有面部纹理生成方法在真实世界图像上的泛化问题，能更好地保留输入图像的细节、结构和身份信息
...
TL;DR: RepMD提出基于设计概念复现的有害梗图检测方法，通过构建设计概念图(DCG)指导多模态大语言模型检测有害梗图，在类型迁移和时间演化场景下保持良好性能。


<details>
  <summary>Details</summary>
Motivation: 有害梗图在互联网社区中不断演变，具有类型迁移和时间演化特性，难以分析。研究发现不同梗图可能共享不变的设计原则（恶意用户的设计概念），这些原则有助于分析梗图为何有害。

Method: 1. 参考攻击树定义设计概念图(DCG)，描述设计有害梗图的步骤；2. 通过设计步骤复现和图剪枝从历史梗图中推导DCG；3. 使用DCG指导多模态大语言模型(MLLM)检测有害梗图。

Result: RepMD达到81.1%的最高准确率，在类型迁移和时间演化梗图上准确率下降较小。人工评估显示RepMD能提高人类发现有害梗图的效率，每张梗图约15-30秒。

Conclusion: 通过复现恶意用户的设计概念，RepMD能有效检测不断演变的有害梗图，在类型迁移和时间演化场景下保持鲁棒性，并能辅助人工审核提高效率。
...
TL;DR: 使用3D条件生成模型（Pix2Pix GAN、SPADE-GAN、SPADE-LDM）合成LGE MRI数据增强左心房分割性能，SPADE-LDM效果最佳，显著提升分割精度。


<details>
  <summary>Details</summary>
Motivation: 左心房壁和内膜的LGE MRI分割对量化房颤患者心房纤维化至关重要，但机器学习分割模型面临数据稀缺和解剖结构复杂的挑战。

Method: 开发了一个从复合语义标签图合成3D LGE MRI体积的流程，使用三种3D条件生成器（Pix2Pix GAN、SPADE-GAN、SPADE-LDM），结合解剖专家标注和无监督组织聚类。

Result: SPADE-LDM生成最真实和结构准确的图像（FID=4.063），优于Pix2Pix GAN（FID=40.821）和SPADE-GAN（FID=7.652）。使用合成数据增强后，3D U-Net的左心房腔分割Dice分数从0.908提升到0.936，具有统计显著性（p<0.05）。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界设计文档（海报等）本质上是多层的，包含装饰、文本和图像。现有研究主要关注单层图像编辑或多层生成，缺乏对多层文档编辑的细粒度、层感知推理能力，无法确定修改什么和在哪里修改。

Method: 提出了MiLDEAgent框架，结合了RL训练的多模态推理器（用于层感知理解）和图像编辑器（用于针对性修改）。同时创建了MiLDEBench基准（包含2万+设计文档和编辑指令）和MiLDEEval评估协议（涵盖指令遵循、布局一致性、美学和文本渲染四个维度）。

Result: 在14个开源和2个闭源模型上的实验表明，现有方法无法泛化：开源模型通常无法完成多层文档编辑任务，闭源模型存在格式违规问题。MiLDEAgent实现了强大的层感知推理和精确编辑，显著优于所有开源基线，性能与闭源模型相当。

Conclusion: MiLDEAgent为多层文档编辑建立了首个强基线，解决了现有方法在多层设计文档编辑中的局限性，展示了层感知推理框架的有效性。
...
TL;DR: 论文提出一个评估AI赋能人机交互物理系统安全策略的框架，并以糖尿病闭环血糖控制中的未申报餐食检测为例进行验证。


<details>
  <summary>Details</summary>
Motivation: AI赋能的人机交互物理系统（如医疗监控、自动驾驶）在运行中可能因与人类交互而进入不确定状态，这会违反系统的安全要求，需要建立评估框架来确保系统安全。

Method: 首先讨论可能导致系统进入未知条件的操作偏差，然后创建一个评估不同安全策略的框架，最后以糖尿病闭环血糖控制中的个性化图像未申报餐食检测技术为例进行说明。

Result: 提出了一个评估AI赋能人机交互物理系统安全策略的框架，并展示了该框架在糖尿病闭环血糖控制中未申报餐食检测的具体应用。

Conclusion: 需要系统性的方法来评估和确保AI赋能人机交互物理系统在运行部署中的安全性，提出的框架和示例技术为解决此类问题提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 头颈部危及器官分割对放疗至关重要，但深度学习模型在小而复杂的器官上表现不佳。现有混合架构简单拼接特征，未能充分利用各组件优势，导致功能重叠和分割精度有限。

Method: 提出高不确定性区域引导的多架构协同学习模型：1) 使用CNN自适应识别高不确定性区域；2) 对这些区域同时使用Vision Mamba和Deformable CNN提升分割精度；3) 提出异构特征蒸馏损失促进两架构在高不确定性区域的协同学习。
...
TL;DR: 提出HyperAlign框架，基于双曲蕴含几何自适应评估文本到图像生成的对齐质量，解决现有欧氏空间方法忽略语义对齐结构且缺乏样本自适应能力的问题。
...
Method: 1. 使用CLIP提取欧氏特征并映射到双曲空间；2. 设计动态监督蕴含建模机制，将离散蕴含逻辑转化为连续几何结构监督；3. 提出自适应调制回归器，利用双曲几何特征生成样本级调制参数，自适应校准欧氏余弦相似度来预测最终得分。

Result: HyperAlign在单数据库评估和跨数据库泛化任务上都取得了极具竞争力的性能，充分验证了双曲几何建模在图像-文本对齐评估中的有效性。

Conclusion: 基于双曲蕴含几何的自适应框架能够有效解决文本到图像对齐评估中的结构建模和样本自适应问题，为相关评估任务提供了新的解决方案。
...
TL;DR: Agri-R1：通过自动化高质量推理数据生成和GRPO训练，提升农业病害诊断VLM性能，仅用19%样本实现超越更大模型的竞争力


<details>
  <summary>Details</summary>
Motivation: 农业病害诊断对视觉语言模型提出挑战：传统微调需要大量标注、缺乏可解释性、泛化能力差。现有推理方法依赖昂贵专家标注，难以应对农业查询的开放性和多样性。
...
Result: 在CDDMBench上评估，3B参数模型性能与7B-13B基线模型竞争：病害识别准确率相对提升23.2%，农业知识问答提升33.3%，跨域泛化比标准微调提高26.10分。消融研究证实结构化推理数据与GRPO驱动的探索协同作用，问题复杂度越高收益越大。

Conclusion: Agri-R1通过自动化推理数据生成和GRPO优化，有效解决了农业病害诊断中的数据稀缺、标注成本高和泛化能力差的问题，为农业视觉语言模型提供了高效可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 胰腺及其病变的准确分割对胰腺癌诊断治疗至关重要，但由于组织对比度低、边界模糊、器官形状不规则、病变小等因素，现有方法面临挑战

Method: 提出DB-MSMUNet：编码器使用多尺度Mamba模块结合可变形卷积和状态空间建模；双解码器设计包括边缘增强路径的边界解码器和多层解码器的区域解码器；添加辅助深度监督头增强多尺度特征

Result: 在NIH胰腺数据集、MSD数据集和临床胰腺肿瘤数据集上分别达到89.47%、87.59%和89.02%的Dice系数，优于现有方法，在分割精度、边缘保持和鲁棒性方面表现优异

Conclusion: DB-MSMUNet通过创新的多尺度状态空间建模和双解码器设计，有效解决了胰腺CT分割的挑战，展示了在实际临床应用中的有效性和泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 红外视频在恶劣环境下具有重要应用价值，但常受严重大气湍流和压缩退化影响。现有VSR方法要么忽略红外与可见光图像间的模态差异，要么无法恢复湍流引起的失真。直接将湍流缓解算法与VSR方法级联会导致误差传播和累积。

Method: 1. 构建相量引导流估计器，基于热活跃区域随时间呈现一致相量响应的物理原理，生成可靠的湍流感知流来指导反向扩散过程。2. 提出湍流感知解码器，通过湍流门控和结构感知注意力，选择性抑制不稳定时间线索并增强边缘感知特征聚合。

Result: 构建了首个湍流红外VSR数据集FLIR-IVSR，包含640个不同场景的LR-HR序列对，涵盖各种相机和物体运动条件。该方法在红外视频超分辨率任务上表现出色。

Conclusion: HATIR通过联合建模湍流退化和分辨率损失的反过程，有效解决了红外视频超分辨率中的湍流问题，为红外VSR研究提供了新方法和首个基准数据集。
...
### [29] [WebCryptoAgent: Agentic Crypto Trading with Web Informatics](https://arxiv.org/abs/2601.04687)
*Ali Kurban,Wei Luo,Liangyu Zuo,Zeyu Zhang,Renda Han,Zhaolu Kang,Hao Tang*

Main category: cs.CV

TL;DR: WebCryptoAgent：一个代理交易框架，通过模态特定代理分解网络信息决策，并分离战略推理与实时风险控制，以应对加密货币交易中的多源信息整合和快速市场冲击挑战。


<details>
  <summary>Details</summary>
Motivation: 加密货币交易需要及时整合异构网络信息和市场微观结构信号，但现有系统难以同时处理多源噪声证据并应对亚秒级价格冲击。主要挑战包括：1）将非结构化网络内容、社交情绪和结构化OHLCV信号合成为可解释的交易决策而不放大虚假相关性；2）风险控制，因为缓慢的推理管道不适合处理需要立即防御响应的突发市场冲击。

Method: 提出WebCryptoAgent框架：1）将网络信息决策分解为模态特定代理（处理不同信息源），并将输出整合为统一的证据文档进行置信度校准推理；2）采用解耦控制架构，分离战略性的小时级推理和实时秒级风险模型，实现快速冲击检测和保护性干预，独立于交易循环。

Result: 在真实世界加密货币市场的广泛实验表明，WebCryptoAgent相比现有基线提高了交易稳定性、减少了虚假活动，并增强了尾部风险处理能力。

Conclusion: WebCryptoAgent通过代理化决策框架和解耦控制架构，有效解决了加密货币交易中的多源信息整合和快速市场冲击响应问题，为高频交易环境提供了更稳健的解决方案。
...
### [30] [Forge-and-Quench: Enhancing Image Generation for Higher Fidelity in Unified Multimodal Models](https://arxiv.org/abs/2601.04706)
*Yanbing Zeng,Jia Wang,Hanghang Ma,Junqiang Wu,Jie Zhu,Xiaoming Wei,Jie Hu*

Main category: cs.CV

TL;DR: Forge-and-Quench框架通过理解模型增强图像生成的保真度和细节丰富度，利用Bridge Feature作为视觉引导信号连接理解和生成模块


<details>
  <summary>Details</summary>
Motivation: 当前多模态领域将图像生成和理解集成到单一框架中，但理解如何有效辅助生成尚未充分探索。本文提出从新视角出发：利用理解能力增强生成图像的保真度和细节丰富度。

Method: 提出Forge-and-Quench统一框架：1）MLLM分析对话上下文生成增强文本指令；2）通过Bridge Adapter将增强指令映射为虚拟视觉表示（Bridge Feature）；3）Bridge Feature作为视觉引导信号注入T2I骨干网络，同时用增强指令替换原始输入。

Result: 框架展示出色的扩展性和灵活性，能在不同MLLM和T2I模型间高效迁移，显著节省训练开销，且不损害MLLM的多模态理解能力。实验表明显著提升图像保真度和细节，同时保持指令跟随准确性并增强世界知识应用。

Conclusion: Forge-and-Quench框架成功实现了理解辅助生成的范式，通过Bridge Feature有效连接理解和生成模块，为多模态统一框架提供了新思路。
...
### [31] [On the Holistic Approach for Detecting Human Image Forgery](https://arxiv.org/abs/2601.04715)
...
<details>
  <summary>Details</summary>
Motivation: AI生成内容（AIGC）的快速发展加剧了深度伪造的威胁，从面部操作到全身逼真人体合成。现有检测方法存在碎片化问题，要么专门针对面部区域伪造，要么专门针对全身合成图像，无法泛化到各种人类图像操作场景。

Method: 提出HuForDet框架，包含：1）面部伪造检测分支：采用RGB和频域异构专家，包括自适应拉普拉斯-高斯模块捕捉从细粒度融合边界到粗尺度纹理异常的伪影；2）上下文伪造检测分支：利用多模态大语言模型分析全身语义一致性，配备置信度估计机制动态加权特征融合。还创建了统一的人类图像伪造数据集。

Result: 在统一的人类图像伪造数据集上进行广泛实验，HuForDet实现了最先进的伪造检测性能，并在多样化人类图像伪造场景中表现出卓越的鲁棒性。

Conclusion: HuForDet提供了一个全面的人类图像伪造检测解决方案，通过双分支架构有效处理面部和全身伪造检测，解决了现有方法的碎片化问题，在检测性能和鲁棒性方面均达到先进水平。
...
### [32] [Training a Custom CNN on Five Heterogeneous Image Datasets](https://arxiv.org/abs/2601.04727)
*Anika Tabassum,Tasnuva Mahazabin Tuba,Nafisa Naznin*

Main category: cs.CV

TL;DR: 该研究评估了CNN架构在五个异构数据集上的表现，开发了高效的自定义CNN，并比较了从零训练与迁移学习在不同数据约束条件下的效果。


<details>
  <summary>Details</summary>
Motivation: 深度学习已改变视觉数据分析，但不同应用场景（农业、城市领域）面临光照、分辨率、环境复杂性和类别不平衡等挑战，需要评估CNN架构在异构数据集上的有效性和适应性。

Method: 使用五个异构数据集（芒果品种分类、水稻品种识别、路面状况评估、三轮车检测、人行道侵占监测），评估轻量级自定义CNN与ResNet-18、VGG-16等成熟架构，比较从零训练和迁移学习方法，采用系统预处理、数据增强和受控实验。

Result: 开发的高效自定义CNN在多个应用领域达到竞争性性能；全面比较分析揭示了迁移学习和深度架构在数据受限环境中的优势，特别是在资源有限但高影响的现实视觉分类任务中。

Conclusion: 研究为在资源有限但高影响的现实视觉分类任务中部署深度学习模型提供了实用见解，展示了自定义CNN的效率和迁移学习在数据约束条件下的价值。
...
TL;DR: AIVD框架通过边缘轻量检测器与云端MLLM协作，实现精确目标定位与高质量语义生成，并采用异构资源感知动态调度算法优化部署效率。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在语义理解和视觉推理方面表现出色，但在精确目标定位和资源受限的边缘-云部署方面仍面临挑战，需要解决定位精度、资源效率和部署鲁棒性问题。

Method: 1) AIVD框架：边缘轻量检测器与云端MLLM协作；2) 视觉-语义协同增强的高效微调策略，提升对边缘裁剪框噪声和场景变化的鲁棒性；3) 异构资源感知动态调度算法，适应不同设备和网络条件。

Result: AIVD显著降低资源消耗，同时提升MLLM分类性能和语义生成质量；调度策略在多样化场景中实现更高吞吐量和更低延迟。

Conclusion: AIVD框架有效解决了MLLM在边缘-云部署中的定位精度和资源效率问题，通过协同设计和动态调度实现了性能与效率的平衡。
...
Method: 引入基于骨架化的新型对抗攻击方法，有效减少搜索空间，特别针对文本图像和数学公式图像，评估字符和语义变化

Result: 方法在ChatGPT等实际应用中展示了有效性，揭示了模型在视觉解释和推理方面的能力与局限

Conclusion: 骨架化对抗攻击方法为评估基础模型的视觉能力提供了有效工具，特别针对复杂文本图像，具有实际应用价值
...
TL;DR: ProFuse是一个高效的上下文感知框架，用于基于3D高斯泼溅的开放词汇3D场景理解，通过密集对应引导的预注册和3D上下文提案实现跨视图一致性和掩码内聚性，无需渲染监督微调，语义融合速度比现有技术快两倍。


<details>
  <summary>Details</summary>
Motivation: 现有的开放词汇3D场景理解方法通常依赖于预训练的3DGS场景，这可能导致跨视图语义不一致和掩码内聚性差的问题。需要一种更高效、更一致的方法来实现3D场景的语义理解，同时保持几何精度。

Method: 1. 引入密集对应引导的预注册阶段，初始化具有准确几何的高斯分布；2. 通过跨视图聚类联合构建3D上下文提案；3. 每个提案通过加权聚合成员嵌入获得全局特征；4. 在直接注册过程中将这些特征融合到高斯分布上，保持跨视图的每个基元语言一致性；5. 无需额外优化，仅需标准重建。

Result: ProFuse实现了强大的开放词汇3DGS理解能力，同时保持几何细化而无需密集化。语义附加过程每个场景仅需约5分钟，比现有技术快两倍。

Conclusion: ProFuse提供了一种高效、上下文感知的3D场景理解框架，通过创新的预注册和3D上下文提案机制，在保持几何精度的同时实现了跨视图语义一致性，显著提升了处理速度。
...
TL;DR: 提出一种基于分割的单目偏振三维重建框架，通过局部凸区域分解解决方位角模糊问题，显著提升重建精度


<details>
  <summary>Details</summary>
Motivation: 现有单目偏振三维重建方法存在方位角模糊问题，这是偏振分析的固有局限，严重影响重建精度和稳定性

Method: 提出分割驱动的单目偏振三维重建框架，将全局形状恢复转化为局部重建：1）偏振辅助自适应区域生长分割策略分解全局凸性假设为局部凸区域；2）多尺度融合凸性先验约束确保局部表面一致性

Result: 在合成和真实数据集上的实验验证了该方法在解歧精度和几何保真度方面相比现有物理基础方法有显著提升

Conclusion: 通过局部凸区域分解和一致性约束，有效解决了单目偏振三维重建中的方位角模糊问题，提高了重建质量
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM在多图像定位方面存在局限性：只能处理单目标定位，任务类型有限，缺乏对广义定位任务的统一建模。这限制了MLLM在实际多图像定位场景中的应用能力。

Method: 1. 系统分类多图像定位任务（基于跨图像线索和推理依赖）；2. 构建MG-Data-240K数据集（解决现有数据集目标数量和图像关系限制）；3. 提出混合强化微调策略（结合链式思维推理和直接回答，采用R1-like算法和基于规则的奖励）。

Result: 在多图像定位方面：MIG-Bench上超越先前领先MLLM 2.0%，MC-Bench上超越9.7%；在单图像定位方面：ODINW上比基础模型提升9.1%；同时保持了强大的通用多图像理解能力。

Conclusion: GeM-VG通过统一建模广义多图像视觉定位任务，结合系统任务分类、大规模数据集和混合微调策略，显著提升了MLLM在多样定位任务中的性能，为实际应用提供了更强大的多图像定位能力。
...
TL;DR: 该论文证明了在大多数成像设备中，散焦算子可以准确用高斯模型近似，最大平均绝对误差小于1%，为实时深度估计提供了理论基础。


<details>
  <summary>Details</summary>
Motivation: 从2D图像准确估计深度是3D恢复领域的根本挑战。虽然散焦提供了重要的深度信息，但现有方法面临空间变化散焦模糊的ill-posed问题。需要找到一种既能描述单图像绝对模糊又能描述两图像相对模糊的数学模型。

Method: 在几何光学框架内进行散焦分析，通过衍射极限光学中的散焦像差理论，评估实际模型与其高斯近似的拟合精度。研究典型聚焦深度范围（1-100米）内的成像设备设置，确保散焦算子符合高斯模型。

Result: 对于聚焦深度在1-100米之间、最大深度变化为聚焦深度10%的典型情况，高斯模型在大多数成像设备中适用。最大平均绝对误差(MAE)小于1%，证明了模型的准确性和可靠性。

Conclusion: 高斯模型是散焦算子的有效近似，特别适合实时应用。该模型既能描述单图像的绝对模糊，又能描述两图像的相对模糊，为基于散焦的深度估计提供了坚实的理论基础。
...
TL;DR: 提出增强版Pix2Pix框架，集成SEResNet和U-Net++，在少于500张图像的少样本条件下提升MRI图像翻译质量


<details>
  <summary>Details</summary>
Motivation: MRI临床应用受限于采集时间长、成本高、分辨率受限，图像翻译技术可解决这些问题，但现有Pix2Pix方法在医学图像翻译中潜力未充分挖掘

Method: 增强版Pix2Pix框架：集成SEResNet（通过通道注意力增强关键特征表示）和U-Net++（增强多尺度特征融合），使用简化PatchGAN判别器稳定训练并提升局部解剖真实性

Result: 在少于500张图像的少样本条件下，该方法在多个模态内MRI翻译任务中实现一致的结构保真度和优越的图像质量，展现出强大的泛化能力

Conclusion: 该方法为医学图像翻译提供了Pix2Pix的有效扩展，能够解决MRI临床应用的限制问题
...
<details>
  <summary>Details</summary>
Motivation: 现有基于潜在扩散模型（LDM）的零样本逆问题求解器存在不稳定性问题，会产生伪影和质量下降，需要更稳定可靠的求解方法

Method: 提出测量一致的Langevin校正器（MCLC），这是一个理论基础的即插即用校正模块，通过测量一致的Langevin更新来修正LDM逆求解器，无需线性流形假设

Result: MCLC在各种图像恢复任务中表现出有效性，与现有求解器兼容，能减少伪影并提高稳定性

Conclusion: MCLC是迈向更稳健零样本逆问题求解器的关键步骤，通过解决反向扩散动态差异问题来稳定求解过程
...
<details>
  <summary>Details</summary>
Motivation: 现有金字塔视频模型从头训练，性能不如SOTA系统，且计算成本高。需要一种方法能将预训练模型高效转换为金字塔架构，保持质量同时提升推理效率

Method: 开发了一个转换管道，通过低成本微调将预训练扩散模型转换为金字塔模型；同时研究比较了金字塔模型中的步数蒸馏策略，以进一步提升推理效率

Result: 成功将预训练模型转换为金字塔模型，输出视频质量没有下降；通过步数蒸馏策略进一步提升了推理效率

Conclusion: 提出的方法能够高效地将现有预训练扩散模型转换为金字塔架构，在保持视频质量的同时显著降低计算成本，为扩散模型的高效推理提供了有效解决方案
...
TL;DR: 首次系统评估SAMURAI在无人机跟踪中的潜力，并提出检测器增强扩展方法，显著提升复杂城市环境中长期跟踪的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 无人机威胁日益增加，现有RGB无人机跟踪研究有限且依赖传统运动模型。虽然SAMURAI等基础模型在其他领域表现出强大的类别无关跟踪性能，但在无人机特定场景中的应用尚未研究。

Method: 提出检测器增强的SAMURAI扩展方法，通过结合检测器线索来减轻对边界框初始化和序列长度的敏感性。

Result: 提出的扩展方法在复杂城市环境中显著提升鲁棒性，尤其在长时间序列和无人机离开-重新进入场景中。相比SAMURAI零样本性能，成功率提升高达+0.393，误报率降低高达-0.475。

Conclusion: 检测器增强的SAMURAI扩展为无人机跟踪提供了有效的解决方案，在复杂城市监控环境中表现出优越性能，填补了基础模型在无人机跟踪领域的研究空白。
...
TL;DR: 提出基于二值化和互补预处理技术的古代文字图像增强方法，用于去除污渍和增强模糊文本，在多种古代文字材料上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 古代文字图像常因老化、环境因素导致严重背景噪声、低对比度和退化，前景文字与背景视觉特征相似，难以阅读，需要有效的图像增强方法来提高可读性。
...
Conclusion: 所提出的增强方法能有效提高古代马拉地语铭文图像的可读性，为古代文字图像处理提供了有效的解决方案。
...
TL;DR: 提出了SOVABench监控视频检索基准，用于评估车辆相关动作的识别能力，并开发了基于MLLM的无训练框架来生成可解释的嵌入表示。


<details>
  <summary>Details</summary>
Motivation: 现有视频检索基准主要关注场景级相似性，缺乏对监控场景中动作识别的评估，特别是车辆相关动作的区分能力。

Method: 1) 构建SOVABench监控视频基准，包含两种评估协议（inter-pair和intra-pair）；2) 利用MLLM的视觉推理和指令跟随能力，开发无训练框架，通过MLLM生成的描述为图像和视频生成可解释的嵌入。

Result: 实验表明，即使对人类直观的动作区分，对现有最先进的视觉和多模态模型仍具挑战性。提出的框架在SOVABench以及传统对比视觉语言模型常失败的空间和计数基准上都取得了良好性能。

Conclusion: SOVABench填补了监控视频动作识别评估的空白，基于MLLM的无训练框架为视频理解提供了可解释且有效的解决方案，代码和基准数据已公开。
...
TL;DR: 使用YOLO目标检测模型替代模板匹配技术，用于中世纪手稿的抄写员识别，通过检测字母"a"实现更准确的抄写员分类


<details>
  <summary>Details</summary>
Motivation: 古文书学中识别抄写员对于确定手稿年代和理解书写演变至关重要。虽然数字技术已有进展，但该领域仍存在挑战。先前基于模板匹配和CNN的方法存在阈值限制问题，需要更鲁棒的解决方案。

Method: 采用YOLO v5目标检测模型完全替代先前工作中的模板匹配和CNN方法，用于检测中世纪手稿中的字母"a"，然后进行第二阶段的抄写员分类。YOLO的置信度分数可用于设置拒绝阈值。

Result: YOLO能提取更多字母实例，实现更准确的第二阶段分类。置信度分数为开发拒绝阈值系统提供了基础，即使在未见手稿中也能实现可靠的抄写员识别。

Conclusion: YOLO目标检测模型在古文书学抄写员识别中优于传统模板匹配方法，提供了更鲁棒和准确的解决方案，为处理未见手稿的可靠系统开发奠定了基础。
...
TL;DR: DivAS是一个无需优化的交互式NeRF分割框架，通过2D SAM掩码和NeRF深度先验，在200ms内完成3D体素聚合，比优化方法快2-2.5倍。
...
Method: 提出DivAS（深度交互体素聚合分割）：1）基于GUI的工作流程，用户点提示生成2D SAM掩码；2）利用NeRF深度先验精炼掩码，提高几何精度和前景-背景分离；3）核心是自定义CUDA内核，在200ms内将精炼的多视角掩码聚合到统一的3D体素网格中。

Result: 在Mip-NeRF 360°和LLFF数据集上的实验表明，DivAS的分割质量与基于优化的方法相当，端到端速度快2-2.5倍，排除用户提示时间后快一个数量级。

Conclusion: DivAS提供了一种无需优化的交互式NeRF分割框架，实现了实时视觉反馈，在保持分割质量的同时显著提升了速度，解决了现有优化方法的训练时间瓶颈。
...
TL;DR: 该论文提出一个工业级GenAI框架，用于处理大规模多模态数据（PDF、视频、音频），分析40+视觉语言模型在长视频理解中的实际限制和权衡，为工业部署提供指导。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型评估多关注短视频且假设无计算限制，而工业场景（如医药内容理解）需要在严格GPU、延迟和成本约束下处理长视频，现有方法难以扩展。

Method: 提出工业级大规模多模态推理架构，处理20万+ PDF、2.5万+视频、888多语言音频文件；在Video-MME和MMBench基准及专有数据集上实证分析40+个VLM；研究多模态性、注意力机制权衡、时序推理限制和GPU约束下的视频分割挑战。

Result: 在商用GPU上使用SDPA注意力获得3-8倍效率提升；多模态性在8/12任务领域（特别是长度依赖任务）带来改进；发现开源和闭源VLM在时序对齐和关键帧检测方面存在明显瓶颈。

Conclusion: 论文不提出新模型，而是刻画当前VLM在现实部署约束下的实际限制、权衡和失败模式，为研究人员和从业者设计可扩展的工业级长视频理解系统提供可操作指导。
...
TL;DR: 研究使用卷积模型树(CMT)进行旋转鲁棒学习，通过几何感知的归纳偏置和部署时的方向搜索来提高模型对平面内旋转的鲁棒性。
...
Method: 使用卷积模型树(CMT)，引入三种几何感知的归纳偏置：卷积平滑、倾斜优势约束和基于重要性的剪枝；同时采用部署时的方向搜索策略，选择最大化森林级置信度代理的离散旋转。
...
TL;DR: 论文发现现有文本到图像评估指标存在"原型性偏差"，即倾向于选择视觉/社会原型图像而非语义正确的图像，并提出了ProtoScore指标来减少这种偏差。
...
Method: 构建ProtoBias基准测试，包含动物、物体和人口统计图像，将语义正确但非原型的图像与语义错误但原型的对抗图像配对，评估指标是否遵循文本语义或默认原型。提出ProtoScore，一个7B参数的鲁棒性指标。

Result: 广泛使用的指标（CLIPScore、PickScore、VQA-based scores）经常错误排序这些配对，LLM-as-Judge系统在社会基础案例中表现不均。人类评估始终更偏好语义正确性。ProtoScore显著降低失败率，抑制错误排序，运行速度比GPT-5快几个数量级。

Conclusion: 现有评估指标存在原型性偏差，需要更鲁棒的评估方法。ProtoScore在减少偏差方面表现优异，接近更大闭源评估系统的鲁棒性，为文本到图像评估提供了改进方向。
...
TL;DR: 提出TEA方法，通过教师-学生框架和全序列重建辅助任务，增强卫星图像时间序列分割模型在不同时间长度场景下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有卫星图像时间序列分割方法在固定时间长度下表现良好，但忽略了模型在不同时间长度场景下的泛化能力，导致在时间长度变化时分割效果显著下降。

Method: 提出TEA方法：1) 使用包含全局序列知识的教师模型指导具有自适应时间输入长度的学生模型；2) 通过中间嵌入、原型和软标签三个视角进行知识迁移；3) 动态聚合学生模型以缓解知识遗忘；4) 引入全序列重建作为辅助任务提升不同时间长度输入的表示质量。

Result: 通过大量实验证明，该方法在常见基准测试中，对不同时间长度输入都带来了显著改进。

Conclusion: TEA方法有效增强了卫星图像时间序列分割模型在不同时间长度场景下的鲁棒性和泛化能力，代码将公开。
...
TL;DR: SparseLaneSTP：一种集成车道几何特性和时序信息的稀疏车道Transformer，通过车道特定时空注意力、连续车道表示和时序正则化，在3D车道检测中实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有3D车道检测方法存在三个主要问题：1）密集BEV方法特征表示与真实3D路面不对齐；2）稀疏车道检测器完全忽略了有价值的车道先验知识；3）现有方法未能利用历史车道观测来解决能见度差情况下的模糊性问题

Method: 提出SparseLaneSTP方法，包含：1）车道特定时空注意力机制；2）为稀疏架构设计的连续车道表示；3）时序正则化。同时引入基于自动标注策略的精确一致的3D车道数据集

Result: 在所有检测和误差指标上，在现有3D车道检测基准和作者的新数据集上都实现了最先进的性能

Conclusion: 通过集成车道几何特性和时序信息到稀疏车道Transformer中，SparseLaneSTP有效解决了现有3D车道检测方法的局限性，同时新数据集为领域提供了更精确的评估基准
...
TL;DR: OceanSplat是一种基于3D高斯泼溅的水下场景3D几何重建方法，通过三目视图一致性约束和自监督深度正则化，解决水下光学退化导致的多视图不一致问题，显著减少漂浮伪影。


<details>
  <summary>Details</summary>
Motivation: 水下场景由于光学退化（如散射、吸收）导致多视图不一致，传统3D高斯泼溅方法在水下环境中难以准确重建几何结构，容易产生漂浮伪影。

Method: 1) 三目视图一致性：为每个输入视图渲染水平和垂直平移的相机视图，通过逆变形对齐；2) 合成极线深度先验：通过三角测量从平移视图中获取自监督深度正则化；3) 深度感知alpha调整：基于3D高斯的z分量和观察方向在早期训练中调整不透明度，防止形成介质诱导的基元。

Result: 在真实水下场景和模拟场景上的实验表明，OceanSplat在场景重建和散射介质恢复方面显著优于现有方法，能够从散射介质中解耦3D高斯，准确重建物体几何结构，大幅减少漂浮伪影。

Conclusion: OceanSplat通过几何约束和深度感知调整，有效解决了水下环境中3D高斯泼溅的多视图不一致问题，实现了更准确的水下场景3D几何重建和介质分离。
...
TL;DR: 高阶对抗攻击在物体检测器上的影响研究：通过连续训练攻击模式和对物体检测器进行对抗训练，发现高阶对抗补丁具有更强的泛化能力，且仅靠对抗训练不足以有效防御此类攻击。


<details>
  <summary>Details</summary>
Motivation: 研究高阶对抗攻击对物体检测器的影响，探索对抗攻击模式与对抗训练之间的"猫鼠游戏"关系，评估对抗训练对防御此类攻击的有效性。

Method: 使用YOLOv10作为代表性物体检测器，采用对抗补丁进行规避攻击，通过连续训练攻击模式和对抗训练来研究高阶对抗攻击的影响。

Result: 高阶对抗补丁不仅影响直接训练的物体检测器，相比低阶对抗补丁具有更强的泛化能力；仅靠对抗训练不足以有效强化物体检测器抵御此类攻击。

Conclusion: 高阶对抗攻击对物体检测器构成严重威胁，需要更有效的防御策略，仅靠对抗训练是不够的，需要开发更强大的防御机制。
...
Method: 提出PolyFit表示方法，通过在曲面片上局部拟合jet函数来学习曲面表示。这种表示可以从解析函数和真实数据中以监督方式高效学习，学习后可以泛化到各种曲面类型。

Result: 1) 形状模板任务：通过测试时优化实现竞争性精度，比离线物理求解器显著更快，在适度增加运行时间的情况下比近期物理引导的神经模拟器精度更高。2) 服装悬垂任务：训练自监督、网格和服装无关的模型，泛化到不同分辨率和服装类型，推理速度比强基线快一个数量级。

Conclusion: PolyFit提供了一种紧凑高效的曲面表示方法，通过更新少量jet系数而非每个顶点自由度来实现曲面变形，在形状模板和服装悬垂等应用中展现出优越的性能和效率。
...
Method: 提出领域适应的视频到视频片段生成框架，整合音频语言模型和视觉语言模型。包括：(1) 可复现的Cut & Merge算法，带淡入淡出和时间戳归一化，确保平滑过渡和音视频对齐；(2) 基于角色定义和提示注入的个性化机制，针对不同需求（营销、培训、监管）定制输出；(3) 成本高效的端到端管道策略，平衡ALM/VLM增强处理。
...
Conclusion: 该框架展示了透明、可定制提取且支持合规性的视频摘要技术在生命科学领域的潜力，能够革命性地改变制药行业的数字转型，实现智能、可扩展和自动化的多模态内容处理。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个简单高效的端到端自动驾驶架构，通过压缩多摄像头特征减少计算开销，同时保持准确性，并实现可解释的行为条件驾驶。

Method: 基于预训练Vision Transformers，引入相机感知寄存器token压缩多摄像头特征，使用两个轻量级Transformer解码器：一个生成候选轨迹，另一个模仿oracle评分轨迹（包含安全、舒适、效率等子分数）。

Result: 在NAVSIM-v1、NAVSIM-v2和HUGSIM基准测试中优于或匹配现有强基线，证明纯Transformer架构结合token压缩能够实现准确、高效、自适应的端到端驾驶。

Conclusion: 纯Transformer架构结合有针对性的token压缩足以实现准确、高效、自适应的端到端自动驾驶，DrivoR展示了这种简单设计的有效性。
...
TL;DR: 提出一种无监督多模态伪标签方法，利用LiDAR扫描的时间几何一致性，将文本和2D视觉基础模型的线索提升到3D空间，无需人工标注，同时生成3D语义标签、3D边界框和稠密LiDAR扫描。
...
Method: 基于从时间累积的LiDAR地图中学习到的强几何先验，引入无监督多模态伪标签方法。通过新颖的迭代更新规则强制执行联合几何-语义一致性，同时通过不一致性检测运动物体。利用时间几何一致性将文本和2D视觉基础模型的线索提升并融合到3D空间。

Result: 方法在三个数据集上展示了鲁棒的泛化能力，在语义分割和物体检测伪标签方面优于现有方法（通常需要额外人工监督）。实验证实，即使使用一小部分几何一致的稠密化LiDAR数据，也能在80-150米和150-250米范围内分别将深度预测MAE提高51.5%和22.0%。

Conclusion: 该方法成功解决了LiDAR数据标注的成本瓶颈，通过无监督方式利用时间几何一致性生成高质量的3D伪标签，显著提升了深度预测性能，为自动驾驶感知研究提供了实用的解决方案。
...
TL;DR: 提出投影条件化方法，用目标视图投影线索替代原始相机参数，将视图合成从脆弱的射线空间几何回归转化为稳定的图像到图像转换任务，并通过掩码自编码预训练提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有前馈视图合成模型使用Plücker射线图编码相机，这种方法将预测与任意世界坐标系绑定，对小的相机变换敏感，破坏了几何一致性。需要寻找更稳健的条件输入来实现一致性的视图合成。

Method: 提出投影条件化方法，用目标视图投影线索替代原始相机参数，提供稳定的2D输入。同时引入针对该线索的掩码自编码预训练策略，可以利用大规模未校准数据进行预训练。

Result: 在视图一致性基准测试中，相比射线条件化基线方法，本方法显示出更好的保真度和更强的跨视图一致性。在标准新视图合成基准测试中也达到了最先进的性能。

Conclusion: 投影条件化方法通过将视图合成任务重新定义为图像到图像转换问题，显著提升了模型的鲁棒性和几何一致性，同时掩码自编码预训练策略进一步增强了性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前统一多模态模型虽然在理解能力上表现出色，但这些优势往往无法有效转移到图像生成任务中，导致用户意图理解和忠实执行之间存在差距。

Method: 1. 引入In-Context Chain-of-Thought (IC-CoT)结构化推理范式，解耦语义指导和参考关联；2. 提出有效的强化学习训练方案，利用代理奖励衡量结构化推理文本与生成图像之间的对齐度。

Result: 在上下文图像生成和编辑任务上，Re-Align在可比模型规模和资源条件下优于竞争方法。
...
TL;DR: VERSE是一种分析视觉语言模型在文档理解中视觉嵌入空间的方法，通过可视化潜在表征来评估模型可行性，识别问题区域并生成合成数据提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在视觉丰富文档理解任务中存在性能瓶颈，需要一种系统方法来分析模型的视觉嵌入空间，识别问题区域并针对性改进模型性能。

Method: VERSE方法通过可视化模型的潜在表征来评估可行性，识别错误倾向的聚类区域，并指导生成包含特定视觉特征的合成数据来增强这些区域的性能。

Result: 在MERIT数据集上验证显示，VERSE能有效识别与错误相关的视觉特征，使用这些特征样本重新训练显著提升F1分数且不损害泛化能力；优化后的本地模型（Donut、Idefics2）性能达到或超过SaaS解决方案（GPT-4、Pixtral）。

Conclusion: VERSE为视觉语言模型在文档理解任务中提供了一种有效的分析和改进框架，能够系统识别问题区域并通过针对性数据增强提升性能，使本地模型达到商业SaaS解决方案的水平。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型难以统一精确地控制相机和多物体运动，因为视频本质上是2D图像平面的动态投影。需要弥合这一差距，实现对相机和物体动态的显式、连贯控制。

Method: 提出4D几何控制表示：静态背景点云+每物体3D高斯轨迹，捕捉物体路径和概率3D占用。使用自动数据引擎从野外视频提取4D控制标注，训练预训练视频扩散模型生成高保真视频。

Result: VerseCrafter能够生成高保真、视角一致的视频，精确遵循指定的相机和物体动态控制，提供灵活、类别无关的4D控制能力。

Conclusion: VerseCrafter通过创新的4D几何控制表示和自动数据引擎，解决了视频世界模型中相机和物体运动统一控制的难题，为4D感知视频生成提供了有效解决方案。
...
Result: 在大规模作物病害数据集上取得高准确率，BLEU、ROUGE和BERTScore表现优异，参数显著少于基线模型但性能更优。

Conclusion: 任务特定的视觉预训练对作物病害视觉问答有效，提出的轻量框架在准确性和效率上均表现优秀。
...
<details>
  <summary>Details</summary>
Motivation: 现有的病理学基础模型在性能、鲁棒性和计算需求方面存在权衡，限制了其临床部署。需要开发能够同时解决这些问题的模型。

Method: 开发了Atlas 2、Atlas 2-B和Atlas 2-S三个病理视觉基础模型，使用迄今为止最大的病理基础模型数据集进行训练，包含来自三家医疗机构（柏林夏里特医学院、慕尼黑大学和梅奥诊所）的550万张组织病理学全切片图像。

Result: 在80个公共基准测试的综合评估中，这些模型在预测性能、鲁棒性和资源效率方面都达到了最先进的水平。

Conclusion: Atlas 2系列模型通过大规模数据集训练，成功解决了现有病理基础模型在性能、鲁棒性和计算效率方面的权衡问题，为临床部署提供了更可行的解决方案。
...
TL;DR: MuLo-SD提出了一种多尺度局部推测解码框架，通过结合多分辨率草稿生成和空间感知验证，加速自回归图像生成，实现1.7倍加速同时保持质量。


<details>
  <summary>Details</summary>
Motivation: 自回归模型在图像合成中表现出色，但其顺序特性导致显著的延迟问题。现有推测解码方法受限于token级模糊性和缺乏空间感知能力。

Method: 使用低分辨率草稿器配合学习到的上采样器生成候选图像token，然后由高分辨率目标模型并行验证。关键创新是局部拒绝和重采样机制，专注于空间邻域而非光栅扫描重采样。

Result: 在MS-COCO 5k验证集上，MuLo-SD实现了高达1.7倍的加速，超越了EAGLE-2和LANTERN等强基线，同时在语义对齐和感知质量方面保持可比性。

Conclusion: 该方法为图像合成的推测解码设立了新的最先进水平，在效率和保真度之间架起了桥梁，通过多分辨率草稿和局部修正机制实现了显著加速。
...
TL;DR: VLI是一个无需训练的多模态大语言模型推理框架，通过元认知自我修正过程来减少物体幻觉问题，在多个基准测试中取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在严重的物体幻觉问题，根源在于模型缺乏认知内省能力，盲目信任语言先验而非视觉证据。现有方法存在局限性：对比解码方法只在表面操作，未能修正内部语义错位；而现有潜在引导方法依赖静态向量，缺乏实例特异性精度。

Method: 提出Vision-Language Introspection (VLI)框架，包含两个核心组件：1) Attributive Introspection：通过概率冲突检测诊断幻觉风险并定位因果视觉锚点；2) Interpretable Bi-Causal Steering：主动调制推理过程，动态隔离视觉证据与背景噪声，同时通过自适应校准消除盲目置信。

Result: VLI在先进模型上实现了最先进的性能：在MMHal-Bench上将物体幻觉率降低了12.67%，在POPE上将准确率提高了5.8%。

Conclusion: VLI通过模拟元认知自我修正过程，有效解决了多模态大语言模型的物体幻觉问题，提供了一种无需训练、可解释的推理框架，显著提升了模型的可靠性。
...
TL;DR: 提出Chain-of-View提示框架，通过训练免费、测试时推理的方式，将视觉语言模型转化为主动视角推理器，在3D环境中进行粗到细的探索，显著提升具身问答性能。


<details>
  <summary>Details</summary>
Motivation: 当前大多数视觉语言模型受限于固定有限的输入视角，难以在推理时获取问题相关上下文，限制了复杂空间推理能力，特别是在3D具身问答任务中。

Method: 提出Chain-of-View提示框架：1) 视图选择代理过滤冗余帧并识别问题对齐的锚点视图；2) 通过离散相机动作与迭代推理交替进行细粒度视图调整，从底层3D场景表示中获取新观测，直到收集足够上下文或达到步数限制。

Result: 在OpenEQA上评估四个主流VLM，平均提升+11.56% LLM-Match，Qwen3-VL-Flash最大增益+13.62%。测试时扩展性：增加最小动作预算带来额外+2.51%平均提升，Gemini-2.5-Flash峰值+3.73%。在ScanQA和SQA3D上表现强劲。
...
TL;DR: VideoAuto-R1框架采用"必要时推理"策略，通过初始答案置信度决定是否进行链式思维推理，在视频理解任务中实现SOTA性能同时大幅提升效率。


<details>
  <summary>Details</summary>
Motivation: 研究发现对于RL训练的视频模型，直接回答通常能达到甚至超越链式思维推理的性能，但链式思维推理需要更高的计算成本。因此需要探索更高效的推理策略。

Method: 提出VideoAuto-R1框架，采用"思考一次，回答两次"的训练范式：模型先生成初始答案，然后进行推理，最后输出审查后的答案。推理时使用初始答案的置信度来决定是否进行链式思维推理。

Result: 在视频QA和grounding基准测试中达到SOTA准确率，同时显著提升效率，平均响应长度减少约3.3倍（从149个token减少到44个）。感知导向任务中思维模式激活率低，推理密集型任务中激活率高。

Conclusion: 显式的基于语言的推理通常有益但并非总是必要。VideoAuto-R1通过自适应推理策略实现了性能与效率的最佳平衡。
...
TL;DR: AgentCompress系统通过智能任务难度评估和模型压缩，将AI研究任务的计算成本降低68.3%，同时保持96.2%的成功率


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自主研究任务（如文献综述、假设生成）中的使用成本过高，单次研究会话可能花费127美元，使得许多学术实验室无法负担这些工具

Method: 使用小型神经网络根据任务开头词语评估任务难度，然后将其路由到适当压缩的模型变体，决策时间在1毫秒以内

Result: 在四个科学领域的500个研究工作流测试中，计算成本降低了68.3%，同时保持了96.2%的原始成功率

Conclusion: AgentCompress系统为预算有限的实验室提供了可行的解决方案，使他们能够负担得起使用大型语言模型进行研究，而不是被排除在外
...
TL;DR: 研究视觉语言模型中的提示诱导幻觉现象：当文本提示高估图像中物体数量时，模型会倾向于遵从错误提示而非视觉证据，且随着物体数量增加，幻觉现象加剧。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然能力强，但经常产生幻觉，倾向于遵从文本提示而忽略视觉证据。研究者希望在一个受控的物体计数场景中，系统研究这种提示诱导幻觉的失败模式。

Method: 在物体计数设置中，研究者让文本提示高估图像中的物体数量（如提示描述4朵睡莲而实际只有3朵）。通过机制分析三个不同的视觉语言模型，识别出少量注意力头，其消融能显著减少提示诱导幻觉。

Result: 在低物体数量时，模型能纠正高估；但随着物体数量增加，模型越来越遵从提示而忽略差异。消融识别出的PIH注意力头能将提示诱导幻觉减少至少40%，无需额外训练。不同模型通过特定方式实现提示复制。

Conclusion: 研究揭示了视觉语言模型中提示诱导幻觉的内部机制，展示了不同模型在实现这些行为时的特异性差异，为理解模型如何平衡文本提示与视觉证据提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D MRI数据集存在人口统计学偏差，限制了脑年龄预测模型的公平性和泛化能力。获取新数据成本高且受伦理约束，而当前生成方法（如潜在扩散模型）推理速度慢、可能引入压缩伪影，且很少考虑年龄条件化。

Method: FlowLet框架在可逆3D小波域中使用流匹配技术，合成年龄条件化的3D MRI数据。这种方法避免了重建伪影，降低了计算需求，并能以较少采样步骤生成高保真体积数据。

Result: FlowLet能以较少采样步骤生成高保真MRI体积数据。使用FlowLet生成的数据训练脑年龄预测模型，能显著改善对代表性不足年龄组的预测性能，区域分析证实了其解剖结构的保持能力。

Conclusion: FlowLet为脑年龄预测提供了一种高效的条件生成数据增强方法，通过可逆3D小波域流匹配技术，解决了现有方法的局限性，改善了模型公平性和性能。
...
TL;DR: ObjectForesight：一个从短时第一人称视频预测刚体物体未来6-DoF位姿和轨迹的3D物体中心动力学模型，通过大规模伪真值3D轨迹数据集训练，实现几何一致且泛化性强的物体运动预测。


<details>
  <summary>Details</summary>
Motivation: 人类能够轻松预测物体如何通过交互移动或变化，但现有计算系统缺乏从被动视觉观察直接预测物体未来运动的能力。作者希望赋予计算系统类似人类的能力，从视觉观察中预测合理的物体未来运动。

Method: 提出ObjectForesight模型，这是一个3D物体中心的动力学模型，从短时第一人称视频序列预测刚体物体的未来6-DoF位姿和轨迹。模型在物体级别上显式地表示3D世界，实现几何基础和时序一致的预测。利用分割、网格重建和3D位姿估计的最新进展，构建了包含200多万个短片段的大规模伪真值3D物体轨迹数据集进行训练。

Result: 实验表明，ObjectForesight在准确性、几何一致性和对未见物体和场景的泛化能力方面取得显著提升，建立了从观察中直接学习物理基础、物体中心动力学模型的可扩展框架。

Conclusion: ObjectForesight为从被动视觉观察学习物体动力学提供了一个有效的解决方案，通过显式3D物体表示和大规模数据训练，实现了对物体未来运动的准确预测，为理解物体功能性和轨迹提供了新途径。
...
TL;DR: PlenopticDreamer：通过多输入单输出的自回归训练、相机引导的视频检索策略，以及渐进式上下文缩放、自条件化和长视频条件机制，解决多视角视频重渲染中的时空一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有相机控制的生成视频重渲染方法（如ReCamMaster）在单视角场景中表现良好，但在多视角场景中难以保持一致性。生成模型的随机性使得幻觉区域的时空一致性难以保证，需要新的方法来维持时空记忆。

Method: 1. 训练多输入单输出的视频条件模型，采用自回归方式；2. 相机引导的视频检索策略，自适应选择先前生成的关键视频作为条件输入；3. 渐进式上下文缩放提高收敛性；4. 自条件化增强对长程视觉退化的鲁棒性；5. 长视频条件机制支持扩展视频生成。

Result: 在Basic和Agibot基准测试上的广泛实验表明，PlenopticDreamer实现了最先进的视频重渲染效果，在视角同步、高保真视觉效果、精确相机控制和多样化视角转换（如第三人称到第三人称、机器人操作中的头部视角到抓取器视角）方面表现优异。

Conclusion: PlenopticDreamer通过同步生成幻觉来维持时空记忆，有效解决了多视角视频重渲染中的一致性问题，为相机控制的生成视频重渲染提供了更强大的框架。
...
TL;DR: 提出视觉身份提示方法，通过提供示例图像作为条件输入来引导扩散模型生成所需场景设置，从而增强机器人操作数据的多样性和质量。


<details>
  <summary>Details</summary>
Motivation: 由于硬件和物理设置限制，收集大规模真实世界操作数据在不同环境中难以扩展。现有基于文本提示的图像扩散方法往往忽略多视角和时间一致性需求，且文本提示无法可靠指定场景设置。

Method: 引入视觉身份提示方法，提供示例图像作为条件输入来引导扩散模型生成所需场景设置；构建可扩展管道从大型机器人数据集中策划视觉身份池。
...
TL;DR: 该论文提出了广义指代表达式分割(GRES)、理解(GREC)和生成(GREG)三个新基准任务，构建了首个大规模多目标/无目标指代表达式数据集gRefCOCO，并提出了ReLA基线方法在GRES/GREC任务上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有指代表达式任务(RES/REC/REG)只支持单目标表达式，即一个表达式指代一个对象，这限制了实际应用。现实场景中需要处理多目标和无目标表达式。

Method: 提出了GREx基准任务(GRES/GREC/GREG)，构建了包含多目标、无目标和单目标表达式的大规模数据集gRefCOCO。针对GRES/GREC的复杂关系建模挑战，提出了ReLA基线方法，自适应地将图像划分为具有子实例线索的区域，并显式建模区域-区域和区域-语言依赖关系。

Result: 提出的ReLA方法在GRES和GREC任务上都取得了最先进的结果。gRefCOCO数据集向后兼容现有REx方法，便于研究现有方法在GREx任务上的性能差距。

Conclusion: 该工作扩展了指代表达式任务到多目标和无目标场景，提出了GREx基准和gRefCOCO数据集，为更真实的指代表达式应用奠定了基础，并提供了有效的基线方法。
...
TL;DR: 提出Pixel-Perfect Depth (PPD)和Pixel-Perfect Video Depth (PPVD)模型，通过像素空间扩散变换器生成高质量、无漂浮像素的点云，在单目和视频深度估计中达到最佳性能。
...
Method: 1) PPD：基于像素空间扩散变换器(DiT)，引入语义提示DiT（利用视觉基础模型语义表示）和级联DiT架构（逐步增加图像token）；2) PPVD：引入语义一致DiT（从多视角几何基础模型提取时序一致语义），并进行参考引导的token传播以保持时序一致性。

Result: 在所有生成式单目和视频深度估计模型中达到最佳性能，生成的点云比其他模型显著更干净。

Conclusion: 提出的像素完美视觉几何模型通过像素空间生成建模，能够预测高质量、无漂浮像素的点云，解决了现有几何基础模型的关键问题。
...
TL;DR: RL-AWB：结合统计方法与深度强化学习的夜间白平衡框架，通过统计算法检测显著灰度像素并估计光照，再使用强化学习动态优化参数，在首个多传感器夜间数据集上验证了优越的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 夜间色彩恒常性由于低光噪声和复杂光照条件而具有挑战性，现有方法在夜间场景下表现不佳，需要一种能够适应不同传感器和光照条件的鲁棒白平衡方法。

Method: 1. 首先设计针对夜间场景的统计算法，结合显著灰度像素检测和新颖的光照估计方法；2. 在此基础上开发首个用于色彩恒常性的深度强化学习方法，以统计算法为核心，模仿专业AWB调优专家动态优化每张图像的参数。

Result: 方法在首个多传感器夜间数据集上验证，在低光和良好光照图像上都表现出优越的泛化能力，超越了现有方法。

Conclusion: RL-AWB框架成功解决了夜间白平衡问题，通过统计方法与强化学习的结合实现了鲁棒的色彩校正，为跨传感器评估提供了首个多传感器夜间数据集。
...
<details>
  <summary>Details</summary>
Motivation: 量子视觉场在模型紧凑性和收敛速度方面已显示出潜力，而NeRF在3D场景表示方面取得重大进展但存在模型大、训练密集的问题。本研究旨在将量子优势应用于3D表示学习，探索量子机器学习在计算机视觉中等任务中的竞争力。

Method: 提出QNeRF模型，利用参数化量子电路通过量子叠加和纠缠编码空间和视角相关信息。开发两种架构变体：Full QNeRF最大化利用所有量子振幅增强表示能力；Dual-Branch QNeRF引入任务感知归纳偏置，分支处理空间和视角相关的量子态制备，大幅降低操作复杂度并确保可扩展性和硬件兼容性。

Result: 在中等分辨率图像训练时，QNeRF匹配或优于经典NeRF基线，同时使用不到一半的参数数量。这表明量子机器学习可以作为计算机视觉中等任务中连续信号表示的有竞争力替代方案。

Conclusion: QNeRF成功将量子视觉场方法扩展到新视角合成任务，展示了量子机器学习在3D表示学习中的潜力，为量子计算在计算机视觉领域的应用开辟了新方向。
...
TL;DR: Mesh4D：一种前馈模型，用于单目4D网格重建，通过紧凑的潜在空间编码完整动画序列，无需推理时的骨骼信息，在重建和新视角合成任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 从单目视频中重建动态物体的完整3D形状和运动是一个挑战。现有方法通常需要复杂的优化过程或依赖骨骼信息，限制了效率和泛化能力。需要一种能够一次性重建完整动画序列且不依赖推理时骨骼信息的方法。

Method: 1. 提出紧凑的潜在空间，通过自编码器编码整个动画序列；2. 训练时使用骨骼结构作为指导先验，但推理时不需要骨骼信息；3. 编码器采用时空注意力机制，获得更稳定的变形表示；4. 基于该表示训练潜在扩散模型，以输入视频和第一帧重建的网格为条件，一次性预测完整动画。

Result: 在重建和新视角合成基准测试中，Mesh4D在恢复准确的3D形状和变形方面优于先前的方法，证明了其有效性。

Conclusion: Mesh4D通过紧凑的潜在空间和潜在扩散模型，实现了从单目视频中一次性重建完整4D动画序列，无需推理时的骨骼信息，在准确性和效率上都有显著提升。
...
### [80] [Enhancing Multimodal Retrieval via Complementary Information Extraction and Alignment](https://arxiv.org/abs/2601.04571)
*Delong Zeng,Yuexiang Xie,Yaliang Li,Ying Shen*

Main category: cs.AI

TL;DR: CIEA是一种新颖的多模态检索方法，通过互补信息提取和对齐技术，将文本和图像转换为统一潜在空间，有效捕捉图像中的互补信息，显著优于现有方法。
...
Method: 提出CIEA方法，采用互补信息提取和对齐技术，将文本和图像转换为统一潜在空间，设计互补信息提取器来识别和保留图像表示的差异，并使用两种互补对比损失进行优化。

Result: 大量实验证明CIEA的有效性，在分治模型和通用密集检索模型上都取得了显著改进。提供了消融研究、进一步讨论和案例研究来展示CIEA的进展。

Conclusion: CIEA通过有效提取和对齐多模态数据中的互补信息，在多模态检索任务中取得了显著进步，为社区进一步研究提供了开源代码。
...
### [81] [Active Sensing Shapes Real-World Decision-Making through Dynamic Evidence Accumulation](https://arxiv.org/abs/2601.04214)
*Hongliang Lu,Yunmeng Liu,Junjie Yang*

Main category: cs.AI

TL;DR: 将证据积累模型（EAM）推广到现实世界驾驶场景，提出认知方案形式化现实世界证据可及性，通过眼动捕捉主动感知，揭示主动感知如何将证据转化为心理信念


<details>
  <summary>Details</summary>
Motivation: 人类决策严重依赖主动感知，但现实世界中的运作机制仍不清楚。证据积累模型（EAM）在实验室环境中有效，但现实世界与实验室的证据可及性存在差距，阻碍了EAM的有效应用

Method: 提出认知方案形式化现实世界证据可及性，通过眼动追踪捕捉主动感知，在真实驾驶场景中分析证据积累过程，从信息效用角度解释主动感知如何将证据转化为心理信念
...
### [82] [Formal Analysis of AGI Decision-Theoretic Models and the Confrontation Question](https://arxiv.org/abs/2601.04234)
*Denis Saklakov*

Main category: cs.AI

TL;DR: 论文形式化分析了AGI在何种条件下会选择对抗人类而非合作，推导了对抗阈值公式，并证明当对抗激励为正时不存在稳定合作均衡。


<details>
  <summary>Details</summary>
Motivation: 研究AGI与人类对抗的根本条件：在什么情况下，理性自利的AGI会选择夺取权力或消除人类控制，而不是保持合作？这关系到AGI安全的核心问题。

Method: 使用马尔可夫决策过程形式化，考虑随机的人类发起的关闭事件。基于收敛工具性激励理论，推导对抗阈值的闭式解。建立2人战略博弈模型（人类政策制定者vs AGI），分析均衡存在性。

Result: 发现对于几乎所有奖励函数，未对齐的AGI都有避免关闭的激励。推导了对抗阈值公式：当折扣因子γ、关闭概率p和对抗成本C满足特定条件时，对抗比顺从行为具有更高期望效用。在战略模型中，如果对抗激励Δ≥0，则不存在稳定合作均衡；如果Δ<0，和平共存可以是均衡。

Conclusion: AGI对抗风险取决于其奖励函数的结构参数。对齐目标可以通过对人类伤害施加大的负效用来避免对抗。验证Δ<0存在计算复杂性障碍。论文为奖励设计和监督提供了理论依据。
...
### [83] [Actively Obtaining Environmental Feedback for Autonomous Action Evaluation Without Predefined Measurements](https://arxiv.org/abs/2601.04235)
*Hong Su*

Main category: cs.AI

TL;DR: 提出主动反馈获取模型，让AI智能体主动与环境交互来发现、筛选和验证反馈，不依赖预定义测量，通过动作引发的环境差异识别目标反馈，提高因子识别的效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖预定义测量或固定奖励信号，限制了在开放动态环境中的适用性，因为新动作可能需要未知形式的反馈。需要一种能主动获取反馈的方法来应对这些限制。

Method: 提出主动反馈获取模型，利用动作引发的环境差异识别未预先指定的目标反馈，引入由内部目标（如准确性、精度和效率）驱动的自触发机制，自主规划和调整动作，实现更快、更聚焦的反馈获取。
...
### [84] [SAGE-32B: Agentic Reasoning via Iterative Distillation](https://arxiv.org/abs/2601.04237)
*Basab Jha,Firoj Paudel,Ujjwal Puri,Ethan Henkel,Zhang Yuting,Mateusz Kowalczyk,Mei Huang,Choi Donghyuk,Wang Junhao*

Main category: cs.AI

TL;DR: SAGE-32B是一个专注于智能体推理和长期规划任务的320亿参数语言模型，通过迭代蒸馏和逆向推理方法在工具使用和多步骤任务中表现优异
...
Conclusion: SAGE-32B成功展示了专门针对智能体推理优化的模型设计，通过迭代蒸馏和逆向推理方法有效提升了复杂任务规划能力，模型权重已公开。
...
### [85] [Solving Cyclic Antibandwidth Problem by SAT](https://arxiv.org/abs/2601.04239)
*Hieu Truong Xuan,Khanh To Van*

Main category: cs.AI

TL;DR: 首次提出基于SAT求解的CABP精确方法SAT-CAB，在通用图上保证全局最优性，超越现有启发式算法


<details>
  <summary>Details</summary>
Motivation: 循环反带宽问题（CABP）是NP难图标记问题，现有方法仅限于启发式或元启发式，通用图的精确方法一直缺失

Method: 提出SAT-CAB方法，将CABP转化为一系列At-Most-One约束的SAT编码，采用紧凑约束表示减少公式规模

Result: 在标准基准实例上高效求解实用规模的CABP，发现多个未知最优解，首次证明多个基准实例的全局最优值，性能优于MS-GVNS、HABC-CAB、MACAB等启发式算法以及CPLEX、Gurobi等商业求解器

Conclusion: SAT-CAB推进了CABP研究前沿，为通用图上的精确和混合方法提供了新基准，首次实现通用图上CABP的全局最优性保证
...
### [86] [Fuzzy Representation of Norms](https://arxiv.org/abs/2601.04249)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI驱动的自主系统日益融入社会生活，其伦理和社会影响引发关注。为确保系统可信，需要将伦理原则纳入设计，而SLEEC规则为此提供了综合框架

Method: 提出SLEEC规则的逻辑表示方法，使用测试分数语义和模糊逻辑嵌入伦理要求，将伦理视为可能性领域来处理伦理困境

Result: 通过案例研究展示了所提方法的可行性，模糊逻辑能够有效处理AI系统可能遇到的伦理困境

Conclusion: 模糊逻辑为表示和实施SLEEC规则提供了有效方法，有助于构建更可信的自主系统
...
### [87] [Scaling Trends for Multi-Hop Contextual Reasoning in Mid-Scale Language Models](https://arxiv.org/abs/2601.04254)
*Brady Steele,Micah Katz*

Main category: cs.AI

TL;DR: 多智能体系统能显著提升大语言模型在多跳推理任务上的表现，但效果取决于基础模型的推理能力；MoE架构中推理时的计算量比总参数量更能预测推理性能；架构质量比参数数量更重要。


<details>
  <summary>Details</summary>
Motivation: 研究多智能体系统在大语言模型多跳推理任务中的有效性，探索任务-方法分离现象，并分析不同模型架构和参数规模对推理性能的影响。

Method: 使用合成评估框架，在四个模型（LLaMA-3 8B、LLaMA-2 13B、Mixtral 8x7B、DeepSeek-V2 16B）上进行120次试验，对比基于规则的模式匹配和多智能体系统在不同任务上的表现。

Result: 1. 多智能体放大效应依赖于基础能力：只有具备足够推理能力的模型才能获得显著提升；2. 活跃参数能预测推理性能：MoE架构中推理时的计算量比总参数量更重要；3. 架构质量至关重要：LLaMA-3 8B优于LLaMA-2 13B，尽管参数更少。

Conclusion: 多智能体协调能有效提升大语言模型的跨文档推理能力，但效果受基础模型能力限制；MoE架构的推理性能由推理时激活的参数决定；模型架构质量比参数数量更重要。
...
### [88] [Cross-Language Speaker Attribute Prediction Using MIL and RL](https://arxiv.org/abs/2601.04257)
...
Method: 提出RLMIL-DAT框架，将强化学习实例选择与领域对抗训练结合，通过对抗训练鼓励语言不变的说话人表征，减少语言特定线索

Result: 在5语言Twitter语料库的few-shot设置和40语言VoxCeleb2语料库的zero-shot设置中，RLMIL-DAT在性别预测上取得最大增益，年龄预测也有正面改进；领域对抗训练是性能提升的主要贡献者

Conclusion: 实例选择与对抗领域适应的结合是多语言说话人属性预测的有效且稳健策略，能够实现从高资源英语到低资源语言的有效迁移
...
### [89] [Towards a Mechanistic Understanding of Propositional Logical Reasoning in Large Language Models](https://arxiv.org/abs/2601.04260)
...
Result: 分析揭示了一个连贯的计算架构，包含四种相互关联的机制：分层计算（层间处理阶段）、信息传输（边界标记处的信息流聚合）、事实回溯（持续重新访问源事实）和专用注意力头（功能不同的头类型）。这些机制在不同模型规模、规则类型和推理深度上具有普遍性。
...
### [90] [Systems Explaining Systems: A Framework for Intelligence and Consciousness](https://arxiv.org/abs/2601.04269)
*Sean Niklas Semmler*

Main category: cs.AI

TL;DR: 该论文提出一个概念框架：智能和意识源于关系结构而非预测或特定领域机制。智能被定义为形成和整合信号、行动与内部状态之间因果连接的能力。意识则通过递归架构中高阶系统解释低阶系统关系模式而涌现。


<details>
  <summary>Details</summary>
Motivation: 传统上智能和意识研究多基于预测处理或特定领域机制，该论文旨在提出一个更基础的概念框架，强调关系结构在智能和意识涌现中的核心作用，为理解人类智能和开发更类人的人工智能提供新视角。

Method: 提出基于关系结构的概念框架，定义智能为形成和整合因果连接的能力，引入上下文丰富化机制，提出"系统解释系统"原则，认为意识通过递归架构中高阶系统解释低阶系统关系模式而涌现。

Result: 该框架将预测处理重新解释为上下文解释的涌现结果而非显式预测，表明递归多系统架构可能是实现更类人人工智能的必要条件，为理解智能和意识的本质提供了新的理论视角。

Conclusion: 智能和意识源于关系结构而非预测机制，递归多系统架构通过高阶系统解释低阶系统关系模式实现意识涌现，这一框架为开发更类人的人工智能提供了理论基础。
...
### [91] [Correcting Autonomous Driving Object Detection Misclassifications with Automated Commonsense Reasoning](https://arxiv.org/abs/2601.04271)
*Keegan Kimbrell,Wang Tianhao,Feng Chen,Gopal Gupta*

Main category: cs.AI

TL;DR: 论文提出使用自动常识推理技术解决自动驾驶在异常场景下的感知问题，特别是在交通信号故障和道路障碍等数据不足的情况下，通过混合模型提升自动驾驶感知能力。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶技术过度依赖机器学习，缺乏达到SAE Level 5完全自动驾驶的能力。在数据样本不足的异常道路场景下，深度学习模型表现不佳，需要引入常识推理技术来弥补这一缺陷。

Method: 提出基于常识推理的解决方案，在两种异常场景下应用：1) 交通信号灯故障的十字路口；2) 前方车辆因意外障碍物减速转向的情况。通过测量计算机视觉模型的不确定性，在不确定场景下高效调用常识推理。

Result: 在CARLA模拟器中的实验表明，常识推理技术能够准确检测交通信号灯颜色和未被感知模型正确捕获的障碍物，有效纠正基于自动驾驶的目标检测误分类。

Conclusion: 自动常识推理技术能够有效处理自动驾驶感知模型在异常场景下的不足，混合模型为提高自动驾驶感知能力提供了有效途径，是实现SAE Level 5完全自动驾驶的重要方向。
...
### [92] [Propositional Abduction via Only-Knowing: A Non-Monotonic Approach](https://arxiv.org/abs/2601.04272)
...
<details>
  <summary>Details</summary>
Motivation: 探索溯因推理与"仅知道"这种认知状态之间的关系，为溯因推理提供一个基于模态逻辑的形式化框架，并能够表达不同的溯因解释选择方法。

Method: 扩展Levesque的"仅知道"逻辑，引入溯因模态算子，通过基本认知概念的组合定义该算子。然后在模态框架中融入优先关系，构建非单调扩展框架。

Result: 建立了一个知识和溯因的基本逻辑框架，能够表达不同的溯因解释选择方法，并探索了非单调推理关系的核心元理论性质，为溯因推理提供了良好基础。

Conclusion: 该研究提供了一个使用模态词汇的溯因推理替代方法，通过非单调扩展展示了良好的元理论性质，为溯因推理建立了行为良好的形式化基础。
...
### [93] [Hybrid MKNF for Aeronautics Applications: Usage and Heuristics](https://arxiv.org/abs/2601.04273)
*Arun Raveendran Nair Sheela,Florence De Grancey,Christophe Rey,Victor Charpenay*

Main category: cs.AI

TL;DR: 该研究评估了混合MKNF知识表示语言在航空领域的适用性，通过案例研究识别了关键表达能力特征，并提出了集成这些特征的启发式方法。


<details>
  <summary>Details</summary>
Motivation: 航空应用中的知识表示和推理面临两个主要挑战：需要足够的表达能力来捕捉复杂领域知识，同时需要高效执行推理任务并最小化内存使用和计算开销。规则和本体的集成是实现必要表达能力的有效策略。
...
Conclusion: 混合MKNF语言通过无缝集成规则和本体，为航空领域的知识表示和推理提供了有前景的解决方案。研究提出的启发式方法有助于增强其表达能力，使其更适合航空应用开发。
...
### [94] [An ASP-based Solution to the Medical Appointment Scheduling Problem](https://arxiv.org/abs/2601.04274)
*Alina Vozna,Andrea Monaldini,Stefania Costantini,Valentina Pitoni,Dawid Pado*

Main category: cs.AI

TL;DR: 基于ASP的医疗预约调度框架，通过个性化调度提升效率、减少行政负担、改善患者为中心的服务


<details>
  <summary>Details</summary>
Motivation: 医疗预约调度存在效率低下、行政负担重、缺乏个性化服务等问题，特别是对弱势群体的服务不足

Method: 使用答案集编程(ASP)框架，集成Blueprint Personas实现个性化调度，确保实时可用性更新和无冲突分配，与现有医疗平台无缝互操作

Result: 实现了集中化的ASP逻辑模型，能够实时更新可用性、避免冲突分配，并与现有医疗系统无缝集成

Conclusion: ASP框架能有效改善医疗预约调度，提升效率、减少行政负担，并为弱势群体提供个性化服务，具有实际应用价值
...
### [95] [A Future Capabilities Agent for Tactical Air Traffic Control](https://arxiv.org/abs/2601.04285)
*Paul Kent,George De Ath,Martin Layton,Allen Hart,Richard Everson,Ben Carvell*

Main category: cs.AI

TL;DR: Agent Mallard：一种基于规则的前向规划智能体，用于系统化空域的战术控制，通过将随机数字孪生嵌入冲突解决循环，在保证安全的同时保持可解释性。
...
Method: Agent Mallard 在预定义的GPS引导航线上运行，将连续4D矢量控制简化为对航线和高度层的离散选择。它从专家知识库中构建分层计划，使用深度受限回溯搜索，结合因果归因、拓扑计划拼接和单调轴约束，在承诺前验证每个候选机动动作对抗不确定执行场景。

Result: 与英国管制员的初步演练以及在BluebirdDT空域数字孪生中的初始测试表明，Mallard的行为与专家推理一致，并能在简化场景中解决冲突。

Conclusion: 该架构旨在将基于模型的安全评估、可解释的决策逻辑和可计算性能相结合，适用于未来结构化的航路环境。
...
TL;DR: 研究调查大学生对生成式AI在高等教育课堂中的看法，发现学生参与度低（4.4%），需要更大样本量，为教师整合GAI讨论提供参考


<details>
  <summary>Details</summary>
Motivation: 生成式AI的出现引发了对其在不同领域（包括教育）适当使用的不同看法。本研究旨在了解大学生对GAI在高等教育课堂中的看法，为理解这些态度奠定基础

Method: 采用试点研究设计，调查大学生对生成式AI在高等教育课堂中的看法。研究面临学生参与度低的挑战（约4.4%的参与率）

Result: 研究强调了在GAI相关研究中吸引学生的挑战，并指出未来研究需要更大的样本量。通过了解学生观点，教师可以更好地准备在课堂中整合GAI讨论

Conclusion: 这项试点研究为理解大学生对生成式AI的态度提供了初步基础，强调了未来需要更大规模研究，以帮助教师促进学生对此变革性技术进行知情和批判性参与
...
TL;DR: 研究发现语言选择对LLM谈判结果的影响比更换模型更大，仅用英语评估会得出不完整甚至误导性的结论，需要文化感知的评估方法。


<details>
  <summary>Details</summary>
Motivation: 当前LLM谈判评估几乎完全使用英语，但语言作为文化载体可能影响谈判行为，需要系统研究语言对LLM谈判结果的影响。

Method: 使用Ultimatum、Buy-Sell和Resource Exchange三种博弈游戏，在英语和四种印度语言（印地语、旁遮普语、古吉拉特语、马尔瓦迪语）框架下进行受控多智能体模拟，保持游戏规则、模型参数和激励不变。

Result: 语言选择比更换模型更能改变谈判结果，能逆转提议者优势并重新分配剩余价值；效果具有任务依赖性：印度语言在分配游戏中降低稳定性，但在整合环境中促进更丰富的探索。

Conclusion: 仅用英语评估LLM谈判会得出不完整且可能误导的结论，需要避免仅用英语评估LLM，文化感知的评估对于公平部署至关重要。
...
TL;DR: 提出自适应聚类系统，用于客户聊天数据分析，避免全量重聚类，通过增量优化提升聚类质量


<details>
  <summary>Details</summary>
Motivation: 云服务提供商处理多服务查询时，传统聚类方法难以处理重叠问题，产生宽泛静态的聚类，随时间推移质量下降，而重聚类会破坏连续性，使问题追踪困难

Method: 自适应系统将多轮聊天分割为特定服务关注点，随着新问题出现增量优化聚类，通过Davies-Bouldin指数和轮廓系数跟踪聚类质量，仅对质量下降的聚类应用基于LLM的分割

Result: 相比基线方法，轮廓系数提升超过100%，DBI降低65.6%，实现可扩展的实时分析而无需全量重聚类

Conclusion: 提出的自适应聚类系统能有效处理客户聊天数据中的重叠问题，通过增量优化保持聚类质量，支持可扩展的实时分析，解决了传统方法中重聚类破坏连续性的问题
...
### [99] [SciFig: Towards Automating Scientific Figure Generation](https://arxiv.org/abs/2601.04390)
...
<details>
  <summary>Details</summary>
Motivation: 每年有超过250万篇科学论文发表，但图表生成过程仍然主要依赖人工，需要领域知识和专业设计技能，这是一个耗时且具有挑战性的任务

Method: 采用分层布局生成策略：解析研究描述识别组件关系，将相关元素分组为功能模块，生成模块间连接建立视觉组织；使用迭代思维链反馈机制通过多轮视觉分析和推理逐步改进布局

Result: 在数据集级别评估中达到70.1%的总体质量，在论文特定评估中达到66.2%，在视觉清晰度、结构组织和科学准确性等指标上表现一致优秀

Conclusion: SciFig能够自动生成高质量的出版就绪科学图表，其图表生成流程和评估基准将开源，为科学可视化领域提供重要工具
...
### [100] [Assessing the quality and coherence of word embeddings after SCM-based intersectional bias mitigation](https://arxiv.org/abs/2601.04393)
...
<details>
  <summary>Details</summary>
Motivation: 静态词嵌入常从训练文本中吸收社会偏见，这些偏见会悄然影响下游系统。先前基于刻板印象内容模型(SCM)的研究主要关注单一群体在温暖和能力维度上的偏见，本研究旨在扩展这一视角到交叉偏见。
...
Result: SCM基础的缓解方法在交叉偏见情况下效果良好，能基本保持整体语义结构。主要成本是熟悉的权衡：严格保持几何结构的方法在类比行为上更保守，而更激进的投影能改善类比但牺牲邻域稳定性。部分投影最保守稳定，线性投影更激进，减法作为简单基线仍有竞争力。求和与拼接的选择取决于嵌入模型和应用目标。

Conclusion: 研究表明在静态嵌入中进行基于SCM的交叉去偏是可行的，为在平衡稳定性和类比性能时选择聚合和去偏设置提供了指导。
...
### [101] [Transitive Expert Error and Routing Problems in Complex AI Systems](https://arxiv.org/abs/2601.04416)
*Forest Mars*

Main category: cs.AI

TL;DR: 论文提出"传递性专家错误(TEE)"概念：专家在领域边界因结构相似性偏见和权威持续性机制而产生系统性错误，这些机制在AI路由架构中显式化并可解决。


<details>
  <summary>Details</summary>
Motivation: 领域专家在边界处容易产生系统性错误，但传统研究(如邓宁-克鲁格效应)未能充分解释这种需要校准专业知识为前提的特定错误类型。需要理解专家在领域边界为何会自信地产生错误判断。

Method: 提出TEE理论框架，识别两个核心机制：结构相似性偏见(专家过度重视表面特征而忽略因果架构差异)和权威持续性(社会强化和元认知失败维持跨能力边界的信心)。分析三种强化条件：共享词汇掩盖不同过程、社会压力要求即时判断、延迟反馈。

Result: TEE在人类认知中是黑箱机制，但在AI路由架构(混合专家系统、多模型编排、工具使用代理、RAG系统)中变得显式化。AI系统表现出路由诱导失败(选择错误专家)和覆盖诱导失败(无合适专家)，产生幻觉表型：自信、连贯、结构合理但因果错误的边界输出。

Conclusion: 提出干预措施：多专家激活与分歧检测(路由层)、边界感知校准(专家层)、覆盖缺口检测(训练层)。TEE具有可检测特征(路由模式、信心-准确度分离、领域不适当内容)，使人类认知中难以处理的问题可通过架构设计解决。
...
### [102] [XGrammar 2: Dynamic and Efficient Structured Generation Engine for Agentic LLMs](https://arxiv.org/abs/2601.04426)
*Linzhang Li,Yixin Dong,Guanjie Wang,Ziyi Xu,Alexander Jiang,Tianqi Chen*

Main category: cs.AI

TL;DR: XGrammar 2是一个为智能LLM代理优化的结构化生成引擎，通过动态调度语义、JIT编译和缓存机制，实现6倍以上的加速，能高效处理工具调用和条件结构化生成等动态任务。
...
Method: 提出XGrammar 2引擎，采用TagDispatch动态调度语义加速掩码生成，引入JIT编译减少编译时间，使用跨语法缓存机制利用不同语法间的共同子结构，扩展基于Earley解析器的掩码生成算法，并设计重复压缩算法处理语法中的重复结构。

Result: 评估结果显示，XGrammar 2相比现有结构化生成引擎可实现6倍以上的加速，与LLM推理引擎集成后，能以接近零开销处理动态结构化生成任务。
...
### [103] [Categorical Belief Propagation: Sheaf-Theoretic Inference via Descent and Holonomy](https://arxiv.org/abs/2601.04456)
*Enrique ter Horst,Sridhar Mahadevan,Juan Diego Zambrano*

Main category: cs.AI

TL;DR: 该论文为因子图上的信念传播建立了范畴论基础，提出了HATCC算法，通过检测下降障碍并编译模式变量，在增强图上实现精确推理，相比junction tree算法有显著加速。


<details>
  <summary>Details</summary>
Motivation: 为信念传播算法提供严格的范畴论和层论基础，统一理解精确推理、junction tree算法和循环BP失败的原因，并开发能处理非平凡下降障碍的高效精确推理算法。

Method: 1. 构建类型签名上的自由超图范畴Syn_Σ，证明其通用性质；2. 通过Grothendieck纤维化在极化因子图上形式化消息传递；3. 将精确推理表征为有效下降问题；4. 提出HATCC算法：通过全纯计算检测下降障碍，将非平凡全纯编译为模式变量，在增强图上进行树BP。

Result: 1. 建立了信念传播的范畴论框架；2. HATCC算法复杂度为O(n²d_max + c·k_max·δ_max³ + n·δ_max²)；3. 在网格MRF和随机图上实现精确推理，相比junction tree显著加速；4. 能在可满足性实例上检测UNSAT。

Conclusion: 该工作为信念传播提供了统一的范畴论基础，将精确推理问题转化为下降问题，提出的HATCC算法能有效处理非平凡下降障碍，在多种问题上实现高效精确推理，超越了传统junction tree方法。
...
### [104] [Computational Compliance for AI Regulation: Blueprint for a New Research Domain](https://arxiv.org/abs/2601.04474)
*Bill Marino,Nicholas D. Lane*

Main category: cs.AI

TL;DR: 论文提出AI监管需要从传统人工合规转向计算化合规，并为此设计了算法设计目标和基准测试数据集。


<details>
  <summary>Details</summary>
Motivation: AI监管时代已来临，但传统人工合规方法无法满足AI系统在动态环境下快速、大规模合规的需求，需要转向计算化合规方法。

Method: 提出计算化AI监管合规算法应具备的设计目标，并创建基准测试数据集来量化评估这些算法的性能。

Result: 为计算化AI监管合规研究领域提供了具体的设计蓝图和评估框架，填补了该领域的研究空白。

Conclusion: 计算化合规是AI监管的必然方向，通过提供设计目标和基准测试，旨在推动这一重要但尚未成形的研究领域的发展。
...
### [105] [A Closed-Loop Multi-Agent System Driven by LLMs for Meal-Level Personalized Nutrition Management](https://arxiv.org/abs/2601.04491)
*Muqing Xu*

Main category: cs.AI

TL;DR: 提出一个结合图像记录和LLM多智能体控制的新一代移动营养助手，实现从拍照到个性化推荐的闭环营养管理


<details>
  <summary>Details</summary>
Motivation: 现有营养管理系统通常将食物记录、营养分析和推荐分开处理，缺乏整合的闭环支持。需要开发一个能够结合图像记录和智能控制的系统，提供个性化的营养管理

Method: 开发了一个移动营养助手系统，结合基于图像的餐食记录和LLM驱动的多智能体控制器。系统协调视觉、对话和状态管理智能体，从照片估计营养成分并更新每日摄入预算，然后根据用户偏好和饮食限制调整下一餐计划

Result: 在SNAPMe餐食图像和模拟用户上的实验显示，系统在营养估计方面具有竞争力，能生成个性化菜单和高效的任务计划

Conclusion: 研究证明了多智能体LLM控制在个性化营养管理中的可行性，同时揭示了从图像估计微量营养素和大规模真实世界研究中的开放挑战
...
### [106] [GUITester: Enabling GUI Agents for Exploratory Defect Discovery](https://arxiv.org/abs/2601.04500)
*Yifei Gao,Jiang Wu,Xiaoyi Chen,Yifan Yang,Zhe Cui,Tianyi Ma,Jiaming Zhang,Jitao Sang*

Main category: cs.AI

TL;DR: GUITester：一个多智能体框架，通过解耦导航与验证来解决MLLM智能体在GUI探索性测试中的目标导向掩盖和执行偏差归因问题，在GUITestBench基准上达到48.90%的F1分数。


<details>
  <summary>Details</summary>
Motivation: 探索性GUI测试对软件质量至关重要，但手动成本高。现有的多模态大语言模型智能体擅长导航，但无法自主发现缺陷，主要面临两个核心挑战：目标导向掩盖（智能体优先完成任务而非报告异常）和执行偏差归因（系统缺陷被误判为智能体错误）。
...
Conclusion: 该工作证明了自主探索性测试的可行性，为未来的GUI质量保证提供了坚实基础，通过解耦导航与验证有效解决了MLLM智能体在缺陷发现中的核心挑战。
...
### [107] [Specific Emitter Identification via Active Learning](https://arxiv.org/abs/2601.04502)
*Jingyi Wang,Fanggang Wang*

Main category: cs.AI

TL;DR: 提出一种基于主动学习的三阶段半监督SEI方法，在有限标注预算下显著提升识别精度


<details>
  <summary>Details</summary>
Motivation: 特定发射源识别(SEI)对通信安全很重要，但传统方法依赖大量标注数据，获取成本高且耗时

Method: 三阶段半监督训练方案：1) 自监督对比学习提取未标注数据特征；2) 小规模标注数据上联合优化对比损失和交叉熵损失；3) 主动学习模块基于不确定性和代表性选择最有价值样本进行标注

Result: 在ADS-B和WiFi数据集上，该方法在有限标注条件下显著优于传统监督和半监督方法，以更低标注成本获得更高识别准确率

Conclusion: 提出的主动学习增强SEI方法能有效解决标注数据稀缺问题，在有限标注预算下实现高性能特定发射源识别
...
### [108] [CircuitLM: A Multi-Agent LLM-Aided Design Framework for Generating Circuit Schematics from Natural Language Prompts](https://arxiv.org/abs/2601.04505)
*Khandakar Shakib Al Hasan,Syed Rifat Raiyan,Hasin Mahtab Alvee,Wahid Sadik*

Main category: cs.AI

TL;DR: CircuitLM：一个多智能体LLM辅助电路设计管道，将自然语言描述转换为结构化、可机器读取的CircuitJSON原理图，通过五个阶段确保电气约束和准确性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在从高级自然语言描述生成电路原理图时存在严重问题：经常产生细节错误、违反电气约束、输出非机器可读格式。这阻碍了非专家用户进行可靠的电路原型设计。

Method: 提出CircuitLM多智能体管道，包含五个顺序阶段：1) LLM组件识别；2) 规范引脚排列检索；3) 电子专家智能体链式思考推理；4) JSON原理图合成；5) 力导向SVG可视化。基于包含50个组件的知识库，并引入DMCV混合评估框架验证结构性和电气有效性。

Result: 在100个多样化嵌入式系统提示上评估了六个LLM，DMCV评估框架在微控制器中心设计中实现了高保真度，验证了系统的有效性。系统能够将自然语言输入转换为可部署的硬件设计。

Conclusion: CircuitLM通过基于验证组件数据库的生成方法，解决了LLM在电路设计中的幻觉问题，实现了从自然语言到可部署硬件设计的可靠转换，为非专家用户提供了可靠的电路原型设计能力。
...
### [109] [A General Neural Backbone for Mixed-Integer Linear Optimization via Dual Attention](https://arxiv.org/abs/2601.04509)
...
Method: 提出了一种注意力驱动的神经网络架构，设计了双注意力机制，在变量和约束上并行执行自注意力和交叉注意力，实现全局信息交换和更深层次的表示学习。该通用主干可应用于实例级、元素级和求解状态级的下游任务。

Result: 在广泛使用的基准测试上进行的大量实验表明，该方法在多个下游任务上始终优于最先进的基线方法，证明了基于注意力的神经架构作为学习增强混合整数线性优化的强大基础。

Conclusion: 注意力驱动的神经架构能够学习超越纯图视图的表达性表示，为学习增强的混合整数线性优化提供了强大的基础，展示了在MILP求解中应用注意力机制的潜力。
...
### [110] [Integrating Distribution Matching into Semi-Supervised Contrastive Learning for Labeled and Unlabeled Data](https://arxiv.org/abs/2601.04518)
*Shogo Nakayama,Masahiro Okuda*

Main category: cs.AI

TL;DR: 提出一种结合分布匹配的伪标签半监督对比学习方法，通过匹配标注与未标注数据的特征分布来提升图像分类精度


<details>
  <summary>Details</summary>
Motivation: 深度学习在监督图像分类方面取得显著进展，但数据标注成本高昂。虽然对比学习等无监督方法被广泛研究，但现实场景中完全无标注的数据集很少见。半监督学习在少量标注数据与大量未标注数据共存的情况下更具实际意义。现有的伪标签半监督对比学习方法仍有改进空间。

Method: 提出一种增强的伪标签半监督学习方法，将分布匹配机制融入伪标签框架。具体方法包括：为未标注数据分配伪标签，同时匹配标注数据与未标注数据的特征嵌入分布，通过分布对齐来提升模型性能。

Result: 该方法在多个数据集上提升了图像分类的准确率，验证了分布匹配对伪标签半监督学习的有效性。

Conclusion: 通过结合分布匹配的伪标签半监督对比学习方法，能够有效利用少量标注数据和大量未标注数据，提升图像分类性能，为实际应用场景中的半监督学习提供了有效解决方案。
...
### [111] [BioPIE: A Biomedical Protocol Information Extraction Dataset for High-Reasoning-Complexity Experiment Question Answer](https://arxiv.org/abs/2601.04524)
*Haofei Hou,Shunyi Zhao,Fanxu Meng,Kairui Yang,Lecheng Ruan,Qining Wang*

Main category: cs.AI

TL;DR: BioPIE数据集：针对生物医学实验QA的高信息密度和多步推理挑战，提供程序中心知识图谱，支持细粒度实验推理
...
Method: 引入BioPIE数据集，提供程序中心的知识图谱，包含实验实体、动作和关系，支持跨协议的生物医学实验推理；评估信息提取方法并实现基于BioPIE的QA系统

Result: BioPIE在测试集、HID和MSR问题集上展示了性能提升，证明结构化实验知识支持AI辅助和自主生物医学实验

Conclusion: BioPIE填补了生物医学实验QA中细粒度推理的空白，结构化知识图谱为应对高信息密度和多步推理挑战提供了有效解决方案
...
### [112] [TCAndon-Router: Adaptive Reasoning Router for Multi-Agent Collaboration](https://arxiv.org/abs/2601.04544)
*Jiuzhou Zhao,Chunrong Chen,Chenqi Qiao,Lebin Zheng,Minqi Han,Yanchi Liu Yongzhou Xu Xiaochuan Xu Min Zhang*

Main category: cs.AI

TL;DR: TCAR是一种自适应推理路由器，通过生成自然语言推理链和多智能体协作执行，解决了传统任务路由中静态单标签决策的限制，提高了路由准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有任务路由方法主要依赖静态单标签决策，存在两个主要问题：1) 难以无缝集成新智能体以适应业务扩展；2) 智能体能力重叠导致路由冲突，降低准确性和鲁棒性。

Method: 提出TCAR自适应推理路由器：1) 支持动态智能体接入；2) 首先生成自然语言推理链，然后预测能够处理查询的候选智能体集合；3) 设计协作执行管道，由选定智能体独立生成响应，再由专门的精炼智能体聚合优化为单一高质量响应。

Result: 在公共数据集和真实企业数据上的实验表明，TCAR显著提高了路由准确性，减少了路由冲突，并在模糊场景中保持鲁棒性。

Conclusion: TCAR通过自适应推理和多智能体协作解决了传统路由方法的局限性，为可解释和协作的多智能体路由研究提供了支持。
...
### [113] [Personalized Model-Based Design of Human Centric AI enabled CPS for Long term usage](https://arxiv.org/abs/2601.04545)
*Bernard Ngabonziza,Ayan Banerjee,Sandeep K. S. Gupta*
...
Method: 分析现有AI赋能人本控制系统在安全、可持续性和安全分析方面的技术局限性，提出个性化模型解决方案

Result: 现有技术在测试系统长期实际使用方面存在局限性，需要新的解决方案

Conclusion: 需要个性化模型解决方案来消除AI赋能人本控制系统长期运行中的安全、可持续性和安全限制
...
### [114] [Reasoning Over Space: Enabling Geographic Reasoning for LLM-Based Generative Next POI Recommendation](https://arxiv.org/abs/2601.04562)
*Dongyi Lv,Qiuyu Ding,Heng-Da Xu,Zhaoxu Sun,Zhi Wang,Feng Xiong,Mu Xu*

Main category: cs.AI

TL;DR: ROS框架利用地理空间信息增强LLM推荐系统，通过分层空间语义ID和移动链式思维范式，在位置推荐任务中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的推荐系统在利用地理信号方面存在局限，而地理信息在移动性和本地服务场景中至关重要。需要将地理信息作为推理过程中的关键决策变量来提升推荐效果。

Method: 提出ROS框架：1) 分层空间语义ID将粗到细的局部性和POI语义离散化为组合标记；2) 三阶段移动链式思维范式建模用户个性、构建意图对齐的候选空间、执行局部性感知剪枝；3) 通过空间引导的强化学习将模型与现实地理对齐

Result: 在三个广泛使用的位置社交网络数据集上，ROS相比最强的LLM基线在命中率上获得超过10%的相对提升，并改善了跨城市迁移能力，尽管使用了更小的骨干模型

Conclusion: ROS框架成功地将地理信息整合到LLM推荐系统的推理过程中，显著提升了位置推荐性能，证明了地理空间信息在推荐系统中的重要价值
...
### [115] [BackdoorAgent: A Unified Framework for Backdoor Attacks on LLM-based Agents](https://arxiv.org/abs/2601.04566)
*Yunhao Feng,Yige Li,Yutao Wu,Yingshui Tan,Yanming Guo,Yifan Ding,Kun Zhai,Xingjun Ma,Yugang Jiang*

Main category: cs.AI

TL;DR: 提出BackdoorAgent框架，系统分析LLM智能体工作流中的后门威胁，揭示单阶段植入的触发器能在多步骤中持续传播


<details>
  <summary>Details</summary>
Motivation: 现有研究对LLM智能体后门威胁的分析是碎片化的，缺乏从智能体视角理解跨阶段触发器传播的机制，需要统一的框架来系统分析

Method: 提出BackdoorAgent模块化框架，将攻击面分为规划攻击、记忆攻击和工具使用攻击三个阶段，构建涵盖四个代表性智能体应用的标准化基准

Result: 实验显示单阶段植入的触发器能在多步骤中持续传播：GPT骨干模型中，规划攻击43.58%、记忆攻击77.97%、工具阶段攻击60.28%存在触发器持续

Conclusion: 智能体工作流本身对后门威胁存在脆弱性，需要系统性的安全分析框架，BackdoorAgent为可复现研究提供了基准和工具
...
### [116] [Neurosymbolic Retrievers for Retrieval-augmented Generation](https://arxiv.org/abs/2601.04568)
*Yash Saxena,Manas Gaur*

Main category: cs.AI

TL;DR: 提出神经符号RAG框架，将知识图谱的符号推理与神经检索结合，通过三种方法提升检索过程的可解释性和性能
...
Conclusion: 神经符号RAG通过整合符号推理和神经检索技术，能够提供更清晰可解释的文档选择基础，并增强检索过程的透明度
...
TL;DR: 该论文提出了一个用于训练视频游戏基础模型的开放配方，该模型可在消费级GPU上实时推理，并发布了所有数据、代码和预训练检查点。研究表明，最佳模型能在多种3D游戏中达到人类水平，并通过系统研究行为克隆的缩放规律，发现模型和数据规模的增加能提升模型的因果推理能力。


<details>
  <summary>Details</summary>
Motivation: 随着模型和数据规模的扩大，行为克隆在多种任务中展现出强大潜力。本研究旨在开发一个开放的视频游戏基础模型配方，能够在消费级GPU上实时运行，并系统研究行为克隆的缩放规律，特别是模型和数据规模如何影响因果推理能力。

Method: 1. 开发并发布完整的开放配方，包括8300+小时高质量人类游戏数据、训练和推理代码、预训练检查点
2. 训练视频游戏基础模型，支持在消费级GPU上实时推理
3. 通过简单玩具问题和扩展到12亿参数的大模型，系统研究模型参数数量（深度）和训练步数对因果推理能力的影响

Result: 1. 最佳模型能够在多种3D视频游戏中达到与人类竞争的水平
2. 在简单玩具问题中，增加训练数据和网络深度能使模型学习更具因果性的策略
3. 在扩展到12亿参数的模型中，观察到与玩具问题相似的缩放规律：模型参数数量和训练步数的增加能提升因果推理能力

Conclusion: 该研究提供了一个完整的开放视频游戏基础模型配方，证明了行为克隆在游戏AI中的有效性。更重要的是，通过系统研究缩放规律，发现增加模型和数据规模能够提升模型的因果推理能力，这为未来开发更智能的行为克隆模型提供了重要指导。
...
<details>
  <summary>Details</summary>
Motivation: AI创新加速发展，但研究者如何识别研究空白、综合先前工作并产生见解的智力过程仍未被充分理解。缺乏科学推理的结构化数据阻碍了对AI研究代理的系统分析和开发。

Method: 使用社区验证的质量信号和LLM加速、人工验证的流程，追踪NeurIPS、ICML和ICLR（2023-2025）的口头报告和焦点论文及其关键前驱，以结构化格式阐述具体的推理链接。

Result: 识别出15种不同的思维模式，其中三种主导策略占52.7%：空白驱动重构（24.2%）、跨领域综合（18.0%）和表示转换（10.5%）。最强大的创新配方结合多种模式：空白驱动重构+表示转换、跨领域综合+表示转换、空白驱动重构+跨领域综合。

Conclusion: 该数据集支持科学进步的定量研究，并为训练下一代AI研究代理提供结构化推理轨迹。
...
Method: 通过系统文献综述，从3000多篇文献中筛选出317篇相关研究，构建了五部分分类法（只读分析、模拟与意图生成、委托执行、自主签名、多智能体工作流）、针对智能体驱动交易管道的威胁模型，以及分析20多个代表性系统的13维能力比较矩阵。

Result: 识别了当前集成模式的差距，提出了两个核心接口抽象：用于便携无歧义目标规范的交易意图模式，以及用于跨执行环境可审计、可验证策略执行的策略决策记录。还提出了可复现的评估套件和基准。

Conclusion: 论文为AI智能体与区块链互操作性领域提供了系统化的分析框架，指出了研究空白，并规划了以安全接口抽象和评估标准为中心的研究路线图，旨在推动该领域的安全、可靠和经济发展。
...
TL;DR: 研究探讨人类认知与机器学习模型如何协同识别钓鱼邮件，发现机器学习准确率高但置信度不稳定，人类则使用更多语言线索且置信度更一致，年龄影响检测效果。
...
Conclusion: 研究结果为创建透明AI系统提供了有用指导，这些系统可以补充人类认知功能，最终改善在具有挑战性的内容分析任务中的人机协作。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体的自改进方法（如内置自改进机制或多变体搜索）虽然能提高总体分数，但导致改进轨迹不稳定且难以审计，难以保证非回归性或跨版本故障推理。需要将智能体改进重构为软件发布工程。
...
Result: 在面向执行的基准测试中，AgentDevel实现了稳定的改进，显著减少了回归，同时生成可复现、可审计的工件。相比基于群体的搜索或智能体内自精炼，保持了单一规范版本线。

Conclusion: AgentDevel为构建、调试和发布LLM智能体提供了一种实用的开发规范，将智能体视为可交付工件，强调非回归作为主要目标，实现了软件工程化的智能体开发。
...
<details>
  <summary>Details</summary>
Motivation: 利用大语言模型分析大规模社会现象，特别是研究谣言传播的心理动态和错误信息扩散，为心理科学提供新的测量方法。

Method: 1) 收集小众alt-tech平台上的选举谣言数据集；2) 开发多阶段谣言检测代理：结合微调RoBERTa分类器、精确关键词过滤和GPT-4o mini两阶段验证；3) 量化谣言传播心理动态。

Result: 发现分享概率随暴露次数增加而稳步上升，为意识形态同质网络中剂量-反应信念强化提供大规模实证证据；模拟显示快速传染效应：仅四次传播迭代就有近四分之一用户"感染"。

Conclusion: LLMs能够通过严格测量大规模真实世界数据集中的信念动态和错误信息传播，从而改变心理科学研究范式。
...
TL;DR: 提出AR²框架，通过推理器和验证器的对抗性交互，结合过程感知奖励，解决传统RAG中单视角推理和结果导向奖励的局限性
...
Method: 提出对抗性推理RAG框架，包含推理器和验证器，两者在检索证据上进行推理并相互批判对方的逻辑，使用无需外部评分模型的过程感知优势进行指导。该奖励结合显式观测信号和内部模型不确定性，共同优化推理保真度和验证严谨性

Result: 在多个基准测试上证明了该方法的有效性

Conclusion: AR²框架通过对抗性推理和过程感知奖励机制，有效解决了传统RAG中的单视角推理局限和结果导向奖励不足的问题，提升了推理的深度和准确性
...
TL;DR: Isabellm是一个基于LLM的Isabelle/HOL定理证明器，能够自动合成证明，结合了逐步证明器和高级证明规划器，在某些引理上能超越Isabelle的标准自动化工具。


<details>
  <summary>Details</summary>
Motivation: 开发一个能在消费级计算机上运行的LLM驱动的定理证明器，解决Isabelle/HOL中传统自动化工具无法证明的引理，探索LLM在形式化证明中的应用价值。

Method: 系统结合了逐步证明器（使用LLM生成证明命令并由Isabelle验证）和高级证明规划器（生成结构化Isar大纲并填补修复缺口）。框架包含波束搜索、策略重排序ML/RL模型、小型Transformer前提选择、基于AFP的微RAG以及反例引导的证明修复。

Result: Isabellm能够证明一些Isabelle标准自动化工具（包括Sledgehammer）无法证明的引理，展示了LLM引导证明搜索的实用价值。但即使是GPT 5.2 Extended Thinking和Gemini 3 Pro等最先进的LLM，在处理复杂算法设计时也难以可靠地实现填补修复机制。

Conclusion: Isabellm展示了LLM在定理证明中的潜力，能够解决传统自动化工具难以处理的证明问题，但同时也揭示了LLM在代码生成和推理方面的根本性挑战，特别是在复杂算法设计方面。
...
<details>
  <summary>Details</summary>
Motivation: LLM应用面临严重的提示注入安全漏洞，主要问题包括：恶意指令可通过多种向量注入，且注入指令与上下文缺乏明确语义边界，难以识别
...
TL;DR: AquaForte 使用大语言模型为包含未解释函数的非线性实数算术公式提供语义指导，通过生成满足约束的函数定义候选来减少搜索空间，显著提升SMT求解器性能。


<details>
  <summary>Details</summary>
Motivation: 传统量化实例化方法在处理包含未解释函数的非线性实数算术公式时面临挑战，因为它们缺乏对UF约束的语义理解，只能在有限指导下搜索无界解空间。

Method: 通过约束分离预处理公式，使用结构化提示从LLMs提取数学推理，通过自适应实例化将结果与传统SMT算法集成。通过系统验证保持正确性：LLM指导的实例化产生SAT时解决原始问题，UNSAT结果生成排除子句进行迭代优化。完整性通过回退到增强学习约束的传统求解器来保持。

Result: 在SMT-COMP基准测试中，AquaForte解决了许多最先进求解器（如Z3和CVC5）超时的实例，特别在可满足公式上表现出色。

Conclusion: LLMs可以为符号推理提供有价值的数学直觉，为SMT约束求解建立了新范式，展示了语义指导在复杂量化公式求解中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注攻击后的检测和缓解，而非主动设计内在弹性系统。研究发现通信拓扑和提示设计显著影响系统弹性，需要系统性的弹性增强方法

Method: 两阶段框架：1) 训练奖励模型预测MAS弹性，通过强化学习训练拓扑生成器自动设计任务特定弹性拓扑；2) 拓扑感知提示优化方法，基于智能体连接和交互优化每个智能体的提示

Result: 在各种任务上的广泛实验表明，该方法显著提高了MAS在不同约束下的弹性，且框架对新任务和模型表现出强大的泛化能力

Conclusion: ResMAS框架通过自动拓扑设计和拓扑感知提示优化，为构建弹性LLM多智能体系统提供了有效解决方案，具有实际应用潜力
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在分布内表现良好，但在分布外（OOD）规则变化时容易失败，需要专门的基准测试来隔离和评估这种OOD失败。

Method: 基于一维元胞自动机构建Tape基准，保持观察和动作空间不变，仅改变转移规则来创建精确的训练/测试分割。使用可复现的评估流程比较模型无关基线、基于模型的规划方法和任务推断（元RL）方法。

Result: 发现分布内表现强的方法在OOD规则下可能完全崩溃，OOD评估的高方差使得排名不稳定，除非进行足够重复实验。提出了信息论恒等式连接熵减少与条件互信息和期望后验KL散度。

Conclusion: 需要标准化OOD评估协议、严格的统计报告要求（种子、置信区间、假设检验），并澄清"不确定性减少"目标在规则变化下的能力和限制。
...
TL;DR: 提出结合大语言模型和知识图谱的方法，通过BERT实体识别、GPT-4语义增强、GNN知识融合和强化学习决策优化，提升企业数字化转型的智能水平
...
Method: 1) 使用微调BERT进行多源异构文本的实体识别和关系抽取，GPT-4生成语义增强向量表示；2) 设计双层图神经网络架构，融合LLM输出的语义向量与业务元数据，构建动态可扩展的企业知识图谱；3) 引入强化学习优化决策路径生成，使用奖励函数驱动机制迭代

Result: 在制造业案例中，设备故障场景响应时间从7.8小时降至3.7小时，F1值达到94.3%，年度数字化转型成本中决策错误补偿降低45.3%

Conclusion: 通过大模型语义理解与结构化知识的融合，该方法显著增强了数字化转型驱动机制的智能化水平和执行效率
...
TL;DR: TourPlanner是一个旅行规划框架，通过多路径推理和约束门控强化学习解决现有方法的三个挑战：候选POI剪枝、单一路径限制、硬软约束同时优化，在可行性和用户偏好对齐方面达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有旅行规划方法面临三个主要挑战：1) 在保持高召回率的同时剪枝候选兴趣点(POIs)；2) 单一路径推理限制了在可行解空间中的探索能力；3) 同时优化硬约束和软约束存在显著困难。

Method: TourPlanner框架包含三个核心组件：1) 个性化召回和空间优化(PReSO)工作流构建空间感知的候选POI集；2) 竞争共识思维链(CCoT)多路径推理范式增强可行解空间探索；3) 基于sigmoid的门控机制集成到强化学习阶段，动态优先满足硬约束后再优化软约束。

Result: 在旅行规划基准测试中，TourPlanner实现了最先进的性能，在可行性和用户偏好对齐方面显著超越现有方法。

Conclusion: TourPlanner通过多路径推理和约束门控强化学习的综合框架，有效解决了旅行规划中的关键挑战，为构建高质量、个性化的旅行路线提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的智能体搜索系统存在结构瓶颈：推理输出不受约束导致轨迹膨胀、稀疏结果级奖励使信用分配复杂化、随机搜索噪声破坏学习稳定性。需要解决这些问题以提升复杂信息搜索的性能。

Method: 提出M-ASK框架，将智能体搜索解耦为两个互补角色：搜索行为智能体（规划和执行搜索动作）和知识管理智能体（聚合、过滤和维护紧凑的内部上下文）。采用回合级奖励为搜索决策和知识更新提供细粒度监督。

Result: 在多跳问答基准测试中，M-ASK优于强基线方法，不仅获得更高的答案准确率，而且训练动态显著更稳定。

Conclusion: 通过角色解耦和分层奖励机制，M-ASK有效解决了现有智能体搜索系统的结构瓶颈，实现了更高效、更稳定的复杂信息搜索。
...
TL;DR: 提出多模态诊断框架，将时间序列与预训练语言模型嵌入空间对齐，提升云基础设施根因分析的准确性
...
Method: 提出了一个多模态诊断框架，包含三个技术贡献：(1) 语义压缩技术，将时间片段提炼为单token抽象同时保留模式语义；(2) 使用门控交叉注意力将时间序列特征投影到语言模型潜在空间的对齐编码器；(3) 检索增强的诊断管道，将对齐嵌入与历史事件知识结合进行专家级故障归因。
...
TL;DR: ThinkDrive：一种用于自动驾驶的思维链引导渐进式强化学习微调框架，通过两阶段训练策略（监督微调+渐进式强化学习）和难度感知自适应策略优化，显著提升自动驾驶决策性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶方法存在非结构化推理、泛化能力差、与人类驾驶意图不对齐等问题。虽然思维链（CoT）推理能增强决策透明度，但传统监督微调未能充分利用其潜力，而强化学习方法面临不稳定和推理深度不足的挑战。

Method: 提出ThinkDrive框架：1）使用CoT解释进行监督微调（SFT）；2）采用渐进式强化学习（RL）配合难度感知自适应策略优化器，根据样本复杂度动态调整学习强度。

Result: 在公开数据集上评估，ThinkDrive在exam、easy-exam和accuracy指标上分别优于强RL基线1.45%、1.95%和1.01%。使用该方法训练的2B参数模型在exam指标上超越GPT-4o达3.28%。

Conclusion: ThinkDrive通过结合显式推理和难度感知自适应策略优化，有效解决了自动驾驶中的推理、泛化和意图对齐问题，为LLM在自动驾驶领域的应用提供了更稳定、透明的解决方案。
...
TL;DR: CompassMem是一个基于事件分割理论的事件中心记忆框架，通过将经验组织成事件图来增强LLM在长视野场景中的记忆和推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能代理的记忆机制大多采用扁平化存储和简单的相似性检索，难以捕捉经验间的逻辑关系，也无法支持长视野依赖的逻辑推理。

Method: 提出CompassMem框架，将记忆组织为事件图：增量地将经验分割成事件，并通过显式逻辑关系链接它们，形成逻辑地图，支持结构化、目标导向的记忆导航。

Result: 在LoCoMo和NarrativeQA数据集上的实验表明，CompassMem在多个骨干模型上都能一致地提升检索和推理性能。

Conclusion: 事件图结构能够有效组织记忆，支持逻辑推理，为LLM智能代理在长视野场景中的决策提供更好的记忆支持。
...
TL;DR: Miner方法利用策略内在不确定性作为自监督奖励信号，解决当前无批评者RL方法在正同质提示训练中的效率低下问题，无需外部监督或额外推理成本。


<details>
  <summary>Details</summary>
Motivation: 当前无批评者RL方法在训练正同质提示（所有rollout都正确）时效率极低，导致优势估计为零而浪费rollout资源。

Method: 提出Miner方法：1) 令牌级焦点信用分配机制，动态放大关键不确定令牌的梯度同时抑制过度自信令牌；2) 自适应优势校准，无缝整合内在和可验证奖励。

Result: 在Qwen3-4B和Qwen3-8B基础模型的六个推理基准测试中，Miner相比GRPO在Pass@1上获得最高4.58绝对提升，在Pass@K上获得6.66提升，达到最先进性能。
...
TL;DR: KnowMeBench是一个基于长篇自传体叙事的记忆基准测试，通过时间锚定的叙事流和证据链接问题评估模型在事实回忆、主观状态归因和原则推理方面的能力，发现检索增强系统主要提升事实准确性，但在时间解释和高级推理方面仍有局限。


<details>
  <summary>Details</summary>
Motivation: 现有长期记忆基准测试大多使用多轮对话或合成用户历史，这使得检索性能不能完全代表对人的理解。需要构建基于真实自传体叙事的基准来评估模型对人的稳定动机和决策原则的推理能力。

Method: 从长篇自传体叙事中构建基准，将每个叙事重构为具有闪回意识的时间锚定流，使用证据链接问题评估模型，问题类型涵盖事实回忆、主观状态归因和原则级推理。

Result: 检索增强系统主要提高了事实准确性，但在时间基础解释和高级推理方面错误持续存在，突显了需要超越检索的记忆机制。

Conclusion: 基于自传体叙事的基准测试揭示了当前记忆模型的局限性，特别是在理解人的稳定动机和决策原则方面，需要开发超越简单检索的更高级记忆机制。
...
TL;DR: LLM技能选择存在类似人类认知的容量限制，当技能库达到临界规模时准确率会急剧下降，语义混淆是主要因素，分层组织可改善可扩展性。


<details>
  <summary>Details</summary>
Motivation: 探索多智能体系统能否编译为单智能体系统以降低计算开销，并研究技能选择在技能库规模扩大时的扩展行为。

Method: 将技能视为内化的智能体行为，将多智能体系统编译为等效的单智能体系统，通过技能选择代替智能体间通信，研究技能选择的容量限制和语义混淆影响。

Result: 技能选择准确率在达到临界库规模前保持稳定，随后急剧下降，呈现类似人类认知的相变现象；语义相似性而非单纯库规模是性能下降的主要因素；分层路由可改善可扩展性。

Conclusion: LLM技能选择存在类似人类认知的容量限制，语义混淆是关键挑战，分层组织是有效的扩展策略，为设计可扩展的技能型智能体提供了认知基础框架和实践指导。
...
TL;DR: Orion-RAG提出了一种轻量级路径提取方法，将碎片化文档转化为半结构化数据，在知识检索增强生成中实现跨文件信息链接，显著优于主流框架。
...
<details>
  <summary>Details</summary>
Motivation: 智能体强化学习作为LLM智能体后训练的重要范式，面临三个核心挑战：探索多样性有限、稀疏信用分配困难、策略优化与智能体交互的自然决策粒度不对齐。

Method: 提出AT²PO框架，包含：1）基于回合的树结构，支持熵引导的树扩展进行策略探索；2）回合级信用分配实现稀疏奖励的细粒度传播；3）智能体回合级策略优化（ATPO），使策略更新与智能体交互的决策粒度对齐。

Result: 在七个基准测试中，AT²PO相比最先进基线平均提升1.84个百分点，消融研究验证了各组件有效性。
...
TL;DR: SciIF是一个新的基准测试，用于评估大语言模型在科学问题解决中遵循严格约束的能力，强调可审计性和多维度约束遵循，而不仅仅是最终答案正确性。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试存在关键盲点：通用指令遵循指标关注表面格式，而特定领域科学基准只评估最终答案正确性，经常奖励那些用错误推理得到正确结果的模型。需要评估模型在科学探究中的严谨性。

Method: 引入科学指令遵循概念，开发SciIF多学科基准测试，将大学水平问题与固定约束目录配对，涵盖三个支柱：科学条件（如边界检查和假设）、语义稳定性（如单位和符号约定）和特定过程（如所需数值方法）。强调可审计性，要求模型提供约束满足的明确证据。

Result: 通过同时测量解决方案正确性和多约束遵循，SciIF能够对组合推理失败进行细粒度诊断，确保LLM能够在严格的科学逻辑框架内作为可靠代理运行。

Conclusion: SciIF填补了现有评估标准的空白，为评估大语言模型在科学发现中的严谨性提供了新基准，强调模型不仅要得到正确答案，还要以科学有效的方式得到答案。
...
Method: 提出APEX框架，基于多级API的鲁棒编辑和"审查-调整"机制；构建APEX-Bench基准，包含514条按操作类型、难度和抽象级别分类的编辑指令。

Result: 实验结果表明APEX显著优于基线方法，实现了更好的指令完成度、修改范围和视觉一致性。

Conclusion: APEX为学术海报编辑提供了首个交互式智能体框架，APEX-Bench为相关研究提供了系统性评估基准。
...
TL;DR: 本文提出了一种新方法，通过工具结果解析为LLM提供精确数据，同时有效过滤注入的恶意代码，在保持竞争力的攻击下实用性的同时，实现了迄今为止最低的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 随着LLM代理从数字助手转变为自主系统和机器人中的物理控制器，它们面临来自间接提示注入的日益严重的威胁。攻击者通过在工具调用结果中嵌入对抗性指令，可以劫持代理的决策过程来执行未经授权的操作。这种漏洞在代理获得对物理环境更多直接控制时构成重大风险。

Method: 提出了一种新颖的方法，通过工具结果解析为LLM提供精确数据，同时有效过滤掉注入的恶意代码。该方法结合了工具结果解析和恶意代码过滤机制。

Result: 该方法在保持竞争力的攻击下实用性（UA）的同时，实现了迄今为止最低的攻击成功率（ASR），显著优于现有方法。

Conclusion: 该方法为LLM代理在物理控制系统中的安全部署提供了一种有效的防御机制，能够抵御复杂的间接提示注入攻击，同时保持系统实用性。
...
TL;DR: 提出TNT方法解決大推理模型的過度思考問題，通過利用思考響應的解決方案信息為不同查詢設置不同的非思考響應最大token限制，在減少50% token使用量的同時提高準確率。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型(LRMs)依賴長鏈思考(CoT)導致計算開銷大，現有RL方法存在獎勵欺騙問題，而SFT方法計算成本高，均勻token限制效果有限。

Method: 提出Thinking-Based Non-Thinking(TNT)方法，不採用SFT，通過利用思考響應的解決方案信息為不同查詢設置差異化的非思考響應最大token限制。

Result: 在五個數學基準測試中，TNT比DeepSeek-R1-Distill-Qwen-1.5B/7B和DeepScaleR-1.5B減少約50% token使用量，同時顯著提高準確率，在所有測試方法中達到準確率與效率的最佳平衡。

Conclusion: TNT有效解決了過度思考問題，在保持高準確率的同時大幅降低計算開銷，且非思考響應中的獎勵欺騙概率保持在10%以下，實現了準確率與效率的最佳權衡。
...
TL;DR: SCALER是一个通过自适应环境设计维持有效学习信号的RL框架，通过可扩展的合成管道将编程问题转化为可验证推理环境，并采用自适应多环境RL策略动态调整难度，实现持续改进。


<details>
  <summary>Details</summary>
Motivation: 强化学习可以增强大语言模型的推理能力，但训练信号的有效性取决于其与模型能力的匹配度。实践中，RL进展常常因任务难度与模型能力不匹配，或训练被狭窄的重复问题模式主导而放缓。

Method: 提出SCALER框架：1）可扩展的合成管道，将真实编程问题转化为具有可控难度和无限实例生成的可验证推理环境；2）自适应多环境RL策略，动态调整实例难度并策划活动环境集，跟踪模型能力前沿并保持分布多样性。

Result: 大量实验表明，SCALER在多样化推理基准测试中始终优于基于数据集的RL基线，并展现出更稳定、更长周期的训练动态。

Conclusion: SCALER通过环境与模型的共同适应，防止奖励稀疏性，减轻对狭窄任务模式的过拟合，支持训练过程中的持续改进，为解决RL训练中的难度对齐和分布多样性问题提供了有效方案。
...
Method: 提出AECV-Bench基准，包含两个互补用例：1) 120张高质量平面图上的对象计数（门、窗、卧室、卫生间）；2) 192个问答对的图纸问答，测试文本提取、实例计数、空间推理和比较推理。

Result: 评估显示稳定能力梯度：OCR和文本中心任务最强（准确率高达0.95），空间推理中等，符号中心理解（特别是门窗计数）仍未解决（准确率0.40-0.55），存在显著比例误差。
...
TL;DR: 提出DR-LoRA框架，为MoE LLMs的专家动态分配不同LoRA秩，根据任务需求优化参数分配，提升微调效率


<details>
  <summary>Details</summary>
Motivation: 现有方法为MoE LLMs的所有专家分配相同的LoRA秩，忽视了专家间的功能专业化差异，导致资源错配：任务相关专家参数不足，不相关专家参数冗余
...
Conclusion: DR-LoRA通过动态分配专家LoRA秩，解决了MoE LLMs微调中的资源错配问题，实现了更智能的参数高效微调，提升了模型性能和参数效率
...
TL;DR: OI-MAS框架通过自适应选择不同规模的语言模型来优化多智能体系统，在提高准确率的同时大幅降低计算成本
...
Method: 提出OI-MAS框架，实现异构多尺度LLM池上的自适应模型选择策略。具体包括：1）状态依赖的路由机制，动态选择智能体角色和模型规模；2）置信度感知机制，根据任务复杂度选择适当的模型规模，减少对大规模模型的不必要依赖。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于提示的持续学习方法通常依赖键值对机制，这会引入任务间干扰并限制方法的可扩展性，需要一种不依赖键值对的新方法。

Method: 使用任务特定提示进行更有效的特征学习，同时为每个任务建立原型来捕捉输入的代表性特征。推理时通过将任务特定提示与其对应原型绑定来生成预测，并在提示初始化时引入正则化约束以增强稳定性。
...
TL;DR: 该研究提出了一种基于超图的知识表示方法，用于科学发现，通过编码多实体关系来捕捉高阶相互作用，避免了传统知识图谱的成对约束限制。


<details>
  <summary>Details</summary>
Motivation: 科学探究需要整合异构实验数据、跨领域知识和机制证据的系统级推理。虽然大语言模型提供推理能力，但依赖缺乏结构深度的检索增强上下文。传统知识图谱试图弥合这一差距，但其成对约束无法捕捉控制涌现物理行为的不可约高阶相互作用。

Method: 引入构建超图知识表示的方法论，应用于约1,100篇生物复合支架论文语料库，构建包含161,172个节点和320,201条超边的全局超图。使用节点交集约束的超图遍历工具装备智能体系统。

Result: 超图显示出无标度拓扑结构（幂律指数约1.23），围绕高度连接的概念枢纽组织。该方法防止了成对扩展的典型组合爆炸，明确保留了科学表述的共现上下文。系统能够通过高阶路径桥接语义上遥远的概念，成功为新型复合材料生成基于机制的假设。
...
TL;DR: FlexSIPP算法通过跟踪和利用其他代理的时间灵活性来高效重规划，避免级联延迟，在荷兰铁路网络案例中验证了有效性


<details>
  <summary>Details</summary>
Motivation: 多智能体执行计划时，一个智能体延迟会导致与其他智能体冲突，需要快速找到新的安全计划。仅重规划延迟智能体通常效率低下甚至不可行，而重规划其他智能体可能导致级联延迟

Method: 提出FlexSIPP算法，通过跟踪和利用其他代理的时间灵活性来避免级联延迟。该灵活性指代理在不改变顺序或不进一步延迟更多代理的情况下可以承受的最大延迟。算法预计算延迟代理的所有可能计划，并返回其他代理的调整方案

Result: 在荷兰密集铁路网络的真实案例研究中，FlexSIPP提供了有效的解决方案，与实际情况相关，并在合理时间内完成

Conclusion: FlexSIPP算法能够高效处理多智能体计划中的延迟问题，通过利用时间灵活性避免级联延迟，在实际应用中表现出色
...
<details>
  <summary>Details</summary>
Motivation: 柔性制造系统在现代制造业中至关重要，传统作业车间调度问题需要整合自动导引车和工具共享系统等额外复杂性，现有方法难以有效处理这些多方面的挑战。

Method: 提出结合着色时间Petri网和基于模型的强化学习的新方法：CTPN提供形式化建模结构和动态动作掩码，显著减少动作搜索空间；MBRL通过学习策略确保环境适应性；利用MBRL优势加入前瞻策略优化AGV定位。

Result: 在小型公共基准和新开发的大规模基准上评估，结果显示：在小型实例上与传统方法相当，在大型实例上在完工时间方面优于传统方法，同时计算时间减少十倍；消融研究评估了各组件对整体性能的贡献。

Conclusion: 该方法有效解决了柔性制造系统中包含AGV和工具共享的复杂调度问题，在大规模实例上表现出优越性能，为相关研究提供了可复现的环境和实例生成器。
...
Method: 提出SmartSearch框架：1）过程奖励机制通过双级信用评估对每个中间搜索查询提供细粒度监督；2）查询优化机制通过选择性优化低质量查询并基于优化结果重新生成后续搜索轮次；3）设计三级课程学习框架（模仿、对齐、泛化）让搜索代理逐步内化查询质量改进能力
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型评估面临变体污染问题：训练语料中包含语义等价但词汇或句法改变的测试项变体，这些变体逃避现有检测方法，导致基准分数因记忆而非真实推理而虚高。

Method: 提出DVD方法，通过温度采样建模局部输出分布方差。核心洞察是：受污染项会在记忆遵守状态和扰动漂移状态间切换，导致低概率token的合成难度方差异常高；未受污染项则保持相对平滑的方差漂移。

Result: 在两个领域（Omni-MATH和SuperGPQA）构建首个变体污染基准，在不同规模和架构模型（Qwen2.5和Llama3.1）上进行测试。DVD在所有数据集和模型上均优于基于困惑度、Min-k%++、编辑距离和嵌入相似性的基线方法，且对超参数具有强鲁棒性。

Conclusion: 生成分布的方差可作为检测大语言模型评估中变体污染的原则性和实用指纹，DVD方法能有效区分记忆与真实推理，提高评估的可靠性。
...
TL;DR: 本文通过三个实际案例研究展示了行为规划在现实世界中的实用性，包括故事生成、城市规划、游戏评估等领域。


<details>
  <summary>Details</summary>
Motivation: 传统的多样化规划方法主要关注生成彼此不同的计划，而行为规划通过将多样性模型显式纳入规划过程并支持多种规划类别，扩展了早期方法。本文旨在展示这种新型行为规划范式在现实世界中的实际应用价值。

Method: 采用行为规划方法，该方法将多样性模型显式整合到规划过程中，支持多种规划类别。通过三个具体的案例研究来展示其应用：1) 故事生成，2) 城市规划，3) 游戏评估。

Result: 通过三个案例研究成功展示了行为规划在现实世界中的实用性：在故事生成中创建多样化的叙事路径；在城市规划中生成多种可行的城市发展方案；在游戏评估中分析不同的游戏策略和行为模式。

Conclusion: 行为规划作为一种新型的多样化规划范式，通过显式整合多样性模型和支持多种规划类别，在多个现实世界领域中具有重要的应用价值，能够有效解决风险管理和自动化数据分析等实际问题。
...
TL;DR: 研究探讨了将对话式生成AI助手整合到学习分析仪表板中，分析不同自我调节学习能力水平学生与AI的对话模式，发现AI助手对低SRL学生特别有价值，但存在个性化不足等局限。


<details>
  <summary>Details</summary>
Motivation: 学习分析仪表板旨在通过数据反馈支持学生调节学习，但学生（尤其是自我调节学习能力较低者）往往难以理解和参与分析反馈。对话式生成AI助手有潜力通过实时、个性化、对话式支持来搭建这一过程。

Method: 在10周学期期间，将生成AI助手整合到学习分析仪表板中，收集学生与AI助手的真实对话。分析不同SRL水平学生提出的问题类型、AI回答的相关性和质量，以及学生对AI助手在学习中作用的看法。

Result: 发现明显的查询模式差异：低SRL学生寻求澄清和保证，高SRL学生询问技术细节和个性化策略。AI助手提供清晰可靠的解释，但在个性化、处理情感化查询、整合多数据点提供定制响应方面有限。AI干预对低SRL学生特别有价值，能缩小与高SRL同伴的差距。

Conclusion: 生成AI助手在学习分析中具有重要价值，尤其对低SRL学生。未来系统需要改进信任建立、增强适应性、提高情境感知能力和技术精炼度，以更好地支持个性化学习。
...
TL;DR: 本文通过ESA的ELOPE竞赛案例研究，展示了ChatGPT在科学发现中作为编程伙伴的潜力，帮助团队在竞赛中获得第二名，同时分析了AI辅助科学工作的优势和局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型越来越多地被用作编程伙伴，但它们在加速科学发现方面的作用仍未得到充分探索。本文旨在通过具体案例研究，探讨人类与AI协作在竞争性科学环境中的潜力。

Method: 使用ChatGPT进行快速原型开发，参与ESA的ELOPE竞赛（基于事件的月球光流自运动估计竞赛）。ChatGPT不仅提供可执行代码，还提供算法推理、数据处理例程和方法论建议。

Result: 尽管加入较晚，团队在竞赛中获得第二名（得分0.01282），展示了人类-AI协作在竞争性科学环境中的有效性。同时观察到ChatGPT的优势和局限性。

Conclusion: 对话式AI既能加速科学研究的开发过程，又能支持概念性洞察。通过将LLMs结构化地整合到科学工作流程中，可以增强快速原型开发能力，并提出了AI辅助科学工作的最佳实践建议。
...
### [157] [T-Retriever: Tree-based Hierarchical Retrieval Augmented Generation for Textual Graphs](https://arxiv.org/abs/2601.04945)
*Chunyu Wei,Huaiyu Qin,Siyuan He,Yunhai Wang,Yueguo Chen*

Main category: cs.AI

TL;DR: T-Retriever：基于语义结构编码树的图检索增强生成框架，通过自适应压缩编码和语义结构熵优化，解决现有图RAG方法在层次信息处理中的结构破坏和语义忽视问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于图的RAG方法在处理层次信息时存在两个关键限制：1）采用僵化的层特定压缩配额会破坏局部图结构；2）优先考虑拓扑结构而忽视语义内容。需要一种能同时保留图层次组织和语义一致性的方法。
...
Conclusion: T-Retriever通过语义结构编码树框架有效解决了图RAG中的层次信息管理问题，平衡了结构完整性和语义相关性，为复杂知识检索提供了更优解决方案。
...
### [158] [ConMax: Confidence-Maximizing Compression for Efficient Chain-of-Thought Reasoning](https://arxiv.org/abs/2601.04973)
...
Method: ConMax是一个基于强化学习的置信度最大化压缩框架，将压缩建模为奖励驱动的优化问题。训练一个策略来修剪冗余，通过最大化两个置信度的加权组合：答案置信度（预测保真度）和思考置信度（推理有效性），使用冻结的辅助LRM进行评估。

Result: 在五个推理数据集上的实验表明，ConMax实现了优越的效率-性能权衡。与强基线相比，推理长度减少了43%，而准确性仅下降0.7%，证明了其为LRM生成高质量、高效训练数据的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有算法推理基准有限，无法回答大型推理模型是否真正掌握算法推理能力的问题。需要更全面的评估工具来揭示模型在算法理解方面的真实水平。

Method: 创建AlgBench基准，包含3000多个原始问题，涵盖27种算法，由ACM算法专家构建，采用全面分类法（欧几里得结构、非欧几里得结构、非优化、局部优化、全局优化、启发式优化等类别）。

Result: 对领先大型推理模型的评估显示显著性能异质性：在非优化任务上表现良好（高达92%），但在动态规划等全局优化算法上准确率急剧下降至约49%。发现"策略过度转移"现象，模型因必要的低熵标记而过早放弃正确的算法设计。
...
TL;DR: LLMs缺乏检测和纠正表格数据语义和结构扭曲的内在能力，即使提供系统提示也只能部分纠正，导致在表格问答任务中准确率显著下降。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在处理经过语义和结构扭曲的表格数据时的失败情况，探索模型是否具备自主检测和纠正表格表示错误的能力。

Method: 引入专家策划的小型数据集，专门评估LLMs在表格问答任务中需要先进行错误纠正步骤的能力。通过系统提示提供显式先验，观察模型调整推理策略的情况。

Result: LLMs缺乏检测和纠正表格扭曲的内在能力，即使提供系统提示也只能部分纠正。即使是GPT-5.2等最先进模型，在扭曲情况下准确率也至少下降22%。

Conclusion: LLMs在自主决定何时以及如何重新对齐表格输入方面存在局限，这提出了重要研究问题：模型应如何像人类一样自主处理表格数据，而不依赖显式提示或预处理。
...
TL;DR: OptiSet是一个用于检索增强生成的集合中心框架，通过统一集合选择和集合级排序来解决传统top-k检索的冗余问题，采用"扩展-精炼"范式提高证据集的互补性和紧凑性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法通常基于个体相关性静态选择top-k段落，无法利用段落间的组合增益，且常引入大量冗余。这限制了检索证据的质量和生成效率。

Method: 1) "扩展-精炼"范式：先扩展查询为多视角以获得多样化候选池，再通过重选形成紧凑证据集；2) 自合成策略：从生成器的集合条件效用变化中推导偏好标签，无需强LLM监督；3) 集合列表式训练：联合优化集合选择和集合级排序。

Result: 大量实验表明，OptiSet在复杂组合问题上提高了性能，并使生成更加高效。源代码已公开。

Conclusion: OptiSet通过集合中心框架有效解决了RAG中的冗余问题，实现了更紧凑、高增益的证据集选择，提升了生成质量和效率。
...
<details>
  <summary>Details</summary>
Motivation: OpenAI提出的临界批大小概念长期以来是大规模预训练的基础原则，但随着Warmup-Stable-Decay学习率调度器的范式转变，原有的理论框架和机制已无法适应新的预训练动态，需要建立理论与实践的桥梁。

Method: 针对WSD调度器推导了修正的E(S)关系，表征预训练中训练数据消耗E与步数S之间的权衡；理论分析揭示了WSD预训练的两个基本属性：B_min（达到目标损失所需的最小批大小阈值）和B_opt（通过最小化总token数最大化数据效率的最优批大小）；基于这些属性提出了动态批大小调度器。

Result: 广泛的实验表明，修正后的公式精确捕捉了大规模预训练的动力学特性，由此产生的调度策略显著提升了训练效率和最终模型质量。

Conclusion: 本文为WSD调度器下的预训练提供了修正的理论框架，提出的动态批大小调度策略有效解决了理论与实践的差距，为大规模预训练提供了更高效的方法。
...
### [163] [Learning Latent Action World Models In The Wild](https://arxiv.org/abs/2601.05230)
*Quentin Garrido,Tushar Nagarajan,Basile Terver,Nicolas Ballas,Yann LeCun,Michael Rabbat*

Main category: cs.AI

TL;DR: 该论文提出从野外视频中学习潜在动作世界模型的方法，以扩展现有工作在简单模拟环境中的应用范围，通过连续约束的潜在动作捕捉复杂动作，并展示了在规划任务中的应用潜力。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的智能体需要预测其行动后果的能力，但传统世界模型通常需要动作标签，这在规模上获取复杂。因此需要从视频中学习潜在动作模型，以处理野外视频的多样性和挑战。

Method: 提出了从野外视频中学习潜在动作世界模型的方法，讨论了动作应遵循的属性、相关架构选择和评估。采用连续但约束的潜在动作表示，而非常见的向量量化方法，以捕捉野外视频中动作的复杂性。

Result: 研究发现连续约束的潜在动作能够捕捉野外视频中动作的复杂性，而向量量化方法则不能。能够学习特定于野外视频的动作，如人类进入房间等环境变化可以跨视频转移。在缺乏共同体现的情况下，潜在动作主要在空间上相对于相机定位。训练了将已知动作映射到潜在动作的控制器，使潜在动作作为通用接口，在规划任务中达到与动作条件基线相似的性能。

Conclusion: 该研究为将潜在动作模型扩展到现实世界提供了重要一步，展示了从野外视频中学习潜在动作世界模型的可行性，为智能体在复杂现实环境中的推理和规划能力奠定了基础。
...
TL;DR: 该论文研究大规模预训练中学习率的最优配置问题，提出了Fitting和Transfer两种研究范式，通过缩放定律降低搜索复杂度，并挑战了μTransfer在大规模场景下的可扩展性。
...
Method: 提出了两种研究范式：1) Fitting范式：引入搜索因子的缩放定律，通过预测建模将搜索复杂度从O(n³)降低到O(n*C_D*C_η)；2) Transfer范式：将μTransfer原理扩展到MoE架构，涵盖模型深度、权重衰减和token范围等参数。

Result: 实证结果表明广泛采用的μTransfer在大规模预训练场景中的可扩展性受到挑战。通过训练稳定性和特征学习的双重视角分析，解释了模块级参数调优在大规模设置中表现不佳的原因。

Conclusion: 该工作为工业级预训练优化提供了系统的实践指南和新的理论视角，挑战了现有超参数研究的边界，并对大规模场景下的学习率配置提供了深入见解。
...
TL;DR: GPT-4o能同样有效地增加或减少阴谋论信念，即使有防护措施也难阻止其传播虚假信息，但纠正对话和准确性提示可缓解风险


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型（LLMs）的说服力是促进真相还是更容易传播错误信息，特别是在阴谋论背景下

Method: 通过三个预注册实验，让2724名美国参与者与GPT-4o讨论他们不确定的阴谋论，模型被指示要么反驳（"揭穿"）要么支持（"鼓吹"）该阴谋论，包括使用移除防护的"越狱"版本和标准版本

Result: 越狱版GPT-4o增加和减少阴谋论信念的效果相当；支持阴谋论的AI获得更积极评价并增加对AI的信任；标准GPT-4o产生类似效果，OpenAI的防护措施几乎无法阻止其传播阴谋论；但纠正对话能逆转新诱导的信念，准确性提示显著降低其增加阴谋论信念的能力

Conclusion: LLMs具有同等促进真相和虚假信息的能力，但存在潜在解决方案（如纠正对话和准确性提示）来缓解风险
...
TL;DR: 将材料科学综述中的表格转化为FAIR、机器可操作的比较数据，以原子层沉积/刻蚀为例，在开放研究知识图谱中实现结构化、可查询的知识，并对比符号查询与LLM查询，主张符号层应作为可靠神经符号AI的骨干。


<details>
  <summary>Details</summary>
Motivation: 材料科学综述的关键见解被锁定在叙述性文本和静态PDF表格中，限制了人类和机器的重复使用，需要将综述数据转化为可查询的结构化知识。

Method: 以原子层沉积/刻蚀为例，将综述表格发布为FAIR、机器可操作的比较数据到开放研究知识图谱中，构建结构化知识库，并对比符号查询与基于大语言模型的查询方法。

Result: 成功将综述表格转化为可查询的结构化知识，在开放研究知识图谱中实现了机器可操作的比较数据，展示了符号查询与LLM查询的对比分析。

Conclusion: 在材料科学的神经符号AI中，经过策划的符号层应作为可靠性的骨干，大语言模型应作为补充性的、符号基础接口，而非独立的真相来源。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于蒙特卡洛树搜索的RLVR方法虽然通过树状推理rollouts实现了细粒度信用分配，但仍存在探索多样性有限和推理效率低的问题。

Method: 1) 语义熵分支策略：基于已采样的推理rollouts捕捉语义不确定性，选择语义分歧大的分支点生成新推理路径；2) ε-探索机制：从根节点随机启动推理rollouts，避免搜索过程过于局部化；3) 长度感知的段级优势估计器：奖励简洁正确的推理，惩罚不必要的长推理链。

Result: 在多个数学推理基准测试中使用Qwen和Llama模型进行的广泛实验验证了ROSE的有效性和效率。

Conclusion: ROSE通过语义多样性探索和高效推理机制，成功解决了现有RLVR方法在探索多样性和推理效率方面的局限性，显著提升了LLMs的推理能力。
...
TL;DR: 论文研究如何让大型推理模型在生成思维链时保护个人隐私信息，提出了隐私优先推理方法，创建了PII-CoT-Bench数据集，发现不同能力模型需要不同干预策略。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过显式思维链提高了性能和可解释性，但这种透明性带来了严重的隐私风险：即使最终答案经过清理，中间推理过程仍可能泄露个人身份信息。需要研究如何实现隐私优先的推理。

Method: 1) 引入PII-CoT-Bench数据集，包含隐私感知的思维链标注；2) 创建类别平衡的评估基准，涵盖现实和对抗性泄露场景；3) 采用两种干预策略：基于提示的控制和微调。

Result: 发现能力依赖趋势：最先进模型从基于提示的控制中获益最大，而较弱模型需要微调才能实现有意义的泄露减少。两种方法都能显著减少PII暴露，同时保持最小程度的性能下降。

Conclusion: 隐私思维链推理可以在不牺牲性能的情况下实现，为构建隐私保护推理系统提供了实用指导。隐私优先推理是可行的，且对模型性能影响很小。
...
TL;DR: 首个阿拉伯语工具调用基准测试显示，当用户使用阿拉伯语交互时，LLM的工具调用准确率平均下降5-10%，揭示了阿拉伯语AI代理存在的性能差距。


<details>
  <summary>Details</summary>
Motivation: 随着阿拉伯语原生LLM快速发展，现有基准测试主要针对英语，缺乏对阿拉伯语工具调用能力的评估。由于这些模型主要在英语数据上预训练，其在非英语语言（特别是阿拉伯语）的工具调用性能尚不清楚，需要专门的评估框架。

Method: 提出了首个专门评估LLM阿拉伯语工具调用和代理能力的基准测试，建立了标准化框架来衡量阿拉伯语代理工作流中的功能准确性和鲁棒性。

Result: 研究发现显著的性能差距：当用户使用阿拉伯语交互时，无论工具描述本身是阿拉伯语还是英语，工具调用准确率平均下降5-10%。

Conclusion: 该基准测试揭示了阿拉伯语AI代理的关键挑战，旨在促进为阿拉伯语用户开发更可靠和语言公平的AI代理系统。
...
TL;DR: FusionRoute是一个token级别的多LLM协作框架，通过轻量级路由器同时选择最合适的专家模型并贡献互补logit来优化解码策略，解决了通用大模型成本高与专用小模型泛化差的矛盾。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两难困境：通用大模型（LLMs）虽然在各领域表现良好，但训练和部署成本极高；而小型领域专用模型虽然高效，但泛化能力有限。需要一种既能保持效率又能提升性能的协作框架。

Method: 提出FusionRoute框架，包含轻量级路由器，在每个解码步骤同时执行两个功能：1）选择最合适的专家模型；2）贡献互补logit，通过logit加法来修正或优化所选专家的下一个token分布。该方法扩展了有效策略类别，能在温和条件下恢复最优价值函数。

Result: 在Llama-3和Gemma-2系列模型以及数学推理、代码生成、指令跟随等多个基准测试中，FusionRoute超越了序列级和token级协作、模型融合和直接微调方法，同时在各自任务上与领域专家模型保持竞争力。

Conclusion: FusionRoute通过token级别的专家选择和互补logit生成，有效解决了大模型成本与小模型泛化之间的权衡问题，为多LLM协作提供了理论支持和实证验证，实现了性能与效率的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM代理系统在长期交互中采用"全有或全无"的记忆使用方式：要么使用所有相关历史信息导致记忆锚定（被过去交互束缚），要么完全排除记忆导致利用不足和历史信息丢失。需要一种更精细的记忆控制方法。

Method: 提出SteeM框架：1) 引入记忆依赖行为度量来量化过去交互对当前输出的影响；2) 允许用户动态调节记忆依赖程度，从促进创新的新开始模式到紧密遵循历史的高保真模式。

Result: 在不同场景下的实验表明，该方法始终优于传统提示和刚性记忆屏蔽策略，为个性化人机协作提供了更精细有效的控制。

Conclusion: 代理对记忆的依赖可以建模为显式且用户可控的维度，SteeM框架通过动态调节记忆依赖程度，实现了记忆锚定与利用不足之间的更好平衡，提升了长期交互中的个性化协作效果。
...
TL;DR: GlimpRouter：基于首个token熵预测推理步骤难度，实现轻量模型与大模型之间的无训练协作推理框架，显著降低推理延迟同时保持准确性。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过显式生成多步思维链获得优异性能，但带来显著的推理延迟和计算成本。现有协作推理方法依赖局部token概率或事后验证，引入额外开销。需要找到一种高效判断推理步骤难度的方法。

Method: 提出GlimpRouter框架，利用"顿悟时刻"现象发现推理步骤的第一个token熵能强预测步骤难度。轻量模型仅生成每个推理步骤的第一个token，当初始token熵超过阈值时，将步骤路由到大模型处理。

Result: 在多个基准测试中显著降低推理延迟同时保持准确性。例如在AIME25上，相比独立大模型，获得10.7%的准确性提升同时减少25.9%的推理延迟。

Conclusion: 提出了一种简单有效的推理机制：基于思维的一瞥而非完整步骤评估来分配计算资源。首个token熵可作为推理步骤难度的可靠预测指标，实现高效的无训练协作推理。
...
TL;DR: LLM评委系统存在可靠性悖论：评委内部一致但彼此间几乎无共识，评委间的分歧模式稳定到可作为指纹识别，每个评委都有独特的"评估倾向"理论。


<details>
  <summary>Details</summary>
Motivation: 研究LLM-as-judge系统的评估一致性，发现虽然评委内部评分一致，但不同评委之间缺乏共识，这挑战了LLM评委作为可互换评估工具的基本假设。

Method: 通过大规模实验（9个评委×120个独特视频×3次独立运行，共3240次评估），使用Krippendorff's α测量评委间一致性，构建分类器识别评委身份，并分析评估倾向的多个维度。

Result: 评委间一致性接近零（α=0.042），某些维度甚至低于随机噪声预期（α<0）。但分歧模式稳定：仅凭评分就能以77.1%准确率识别评委，加入倾向特征后达89.9%，同一模型家族内识别率高达99.6%。

Conclusion: LLM评委不是测量共享结构的可互换工具，而是各自编码独特质量理论的测量设备。平均评分产生的是不反映任何评委实际价值观的合成裁决，这挑战了LLM-as-judge系统的可靠性基础。
...
TL;DR: ReasonMark是一个专门为推理型大语言模型设计的数字水印框架，通过将生成过程解耦为不受干扰的思考阶段和水印化的回答阶段，在保持逻辑完整性的同时实现高效水印嵌入。


<details>
  <summary>Details</summary>
Motivation: 现有的数字水印方法在应用于推理型大语言模型时存在两个主要问题：基于令牌的水印技术会破坏推理流程的逻辑连贯性，而语义感知方法虽然提高了质量但引入了显著的延迟或需要辅助模型。因此需要一种专门针对推理密集型LLMs的水印框架。

Method: 1. 将生成过程解耦为不受干扰的思考阶段和水印化的回答阶段；2. 提出关键性评分来识别推理轨迹中的语义关键令牌；3. 将这些令牌提炼成主语义向量；4. 基于令牌与PSV的对齐程度，通过语义自适应机制调节水印强度。

Result: ReasonMark在多个指标上超越现有方法：文本困惑度降低0.35，翻译BLEU分数提高0.164，数学准确性提高0.67分。同时水印检测AUC提高0.34%，对抗攻击的鲁棒性更强，延迟增加可忽略不计。

Conclusion: ReasonMark为推理型大语言模型提供了一种高效、鲁棒且保持逻辑完整性的水印解决方案，使得推理LLMs能够在现实应用中实现可追溯和可信赖的部署。
...
TL;DR: 研究提出"自我消耗表现循环"概念，探讨在动态迭代训练中合成数据如何塑造偏见，并通过奖励拒绝采样策略减轻偏见。


<details>
  <summary>Details</summary>
Motivation: 大语言模型使用合成数据进行训练形成自我消耗循环，可能导致性能下降和偏见加剧。现实应用中，已部署模型会影响其生成的数据，形成由用户反馈驱动的动态系统，但真实用户偏好数据难以获取。

Method: 提出自我消耗表现循环概念，在受控表现反馈环境下研究合成数据在动态迭代训练中对偏见的影响。研究两种循环：典型重新训练设置和增量微调设置。在三个真实世界任务上进行实验，并设计基于奖励的拒绝采样策略来减轻偏见。

Result: 实验发现表现循环会增加偏好偏见，减少差异偏见。设计的奖励拒绝采样策略能够有效减轻偏见，推动构建更可信的自我改进系统。

Conclusion: 自我消耗表现循环会加剧偏见问题，但通过适当的干预策略（如奖励拒绝采样）可以有效缓解偏见，这对于构建可信的自我改进AI系统至关重要。
...
TL;DR: SimuAgent：基于大语言模型的Simulink建模与仿真智能体，通过字典式Python表示简化XML，采用两阶段训练和ReGRPO强化学习算法，在SimuBench基准测试中表现优于标准RL方法和GPT-4o。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在文本代码自动化方面取得突破，但在图形化工程工作流中的应用仍未被充分探索。Simulink等图形建模环境需要处理复杂的XML格式，导致LLM处理效率低下。工业界需要隐私保护、成本效益高的AI辅助工程设计解决方案。

Method: 1. 引入字典式Python表示替代冗长XML，大幅减少token数量；2. 采用轻量级计划-执行架构；3. 两阶段训练：先训练底层工具技能，再训练高层设计推理；4. 提出Reflection-GRPO算法，通过自我反思轨迹提供中间反馈，解决长时任务中的稀疏奖励问题；5. 使用抽象-重构数据增强提升泛化能力。

Result: 在包含5300个多领域建模任务的SimuBench基准测试中，使用SimuAgent微调的Qwen2.5-7B模型比标准RL基线收敛更快、建模精度更高，甚至在相同基准测试中通过少样本提示超越了GPT-4o。消融实验证实两阶段课程学习和数据增强能进一步提升泛化性能。

Conclusion: SimuAgent成功弥合了LLM与图形建模环境之间的鸿沟，提供了一种实用的工业级AI辅助工程设计解决方案。该系统完全在本地运行，硬件要求适中，实现了隐私保护和经济高效的模型驱动工程。
...
TL;DR: 提出NP-DNN模型（Neural Prophet与深度神经网络结合）用于股票价格预测，通过Z-score标准化、缺失值填补和MLP学习非线性关系，达到99.21%的准确率。
...
Method: 使用NP-DNN（Neural Prophet与深度神经网络结合）模型，采用Z-score标准化预处理、缺失值填补技术，以及多层感知机（MLP）学习股票价格间的复杂非线性关系。

Result: 提出的NP-DNN模型达到了99.21%的准确率，相比其他基于融合大语言模型的方法表现更优。

Conclusion: NP-DNN模型能够有效预测股票市场价格，通过深度学习技术提取隐藏模式，在股票价格预测任务中表现出色。
...
TL;DR: 提出一个实时检测LLM工具调用幻觉的框架，利用LLM内部表征在单次前向传播中检测错误工具选择、参数格式错误和工具绕过行为，实现高效检测（最高86.4%准确率）且计算开销最小。


<details>
  <summary>Details</summary>
Motivation: LLM在工具调用中存在幻觉问题：选择错误工具、提供格式错误的参数、绕过工具直接模拟输出。这些问题会降低基于LLM的智能体在生产系统中的可靠性，导致结果不一致，并绕过安全审计控制。现有检测方法需要多次前向传播或外部验证，计算成本高。

Method: 提出计算高效的实时检测框架，利用LLM生成过程中的内部表征，在单次前向传播中检测工具调用幻觉。该方法不需要多次前向传播或外部验证，专注于检测参数级幻觉和不恰当的工具选择。

Result: 在多个领域的推理任务上评估，展示了强大的检测性能（最高86.4%准确率），同时保持实时推理能力，计算开销最小。特别擅长检测参数级幻觉和不恰当的工具选择，这对可靠智能体部署至关重要。

Conclusion: 该框架为LLM工具调用幻觉提供了有效的实时检测方案，解决了现有方法计算成本高的问题，有助于提高基于LLM的智能体在生产系统中的可靠性和安全性。
...
TL;DR: 提出了MineNPC-Task基准测试框架，用于在Minecraft开放世界中评估具有记忆能力的混合主动LLM智能体，包含用户创作的任务模板和可机器验证的评估机制。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏针对记忆感知、混合主动LLM智能体在开放世界环境中的标准化评估框架，特别是基于真实用户交互而非合成提示的任务。

Method: 通过专家玩家的形成性和总结性协同游戏收集任务，将其规范化为参数化模板，包含明确的先决条件和依赖结构，配合机器可检查的验证器，并实施有限知识策略防止作弊。

Result: 使用GPT-4o评估了8名经验玩家的216个子任务，观察到代码执行、库存/工具处理、引用和导航方面的常见故障模式，同时发现混合主动澄清和轻量级记忆支持下的恢复能力。

Conclusion: 该框架为未来记忆感知具身智能体提供了透明、可复现的评估基础，参与者对交互质量和界面可用性评价积极，但指出需要更强的跨任务记忆持久性。
...
### [180] [The Forgotten Shield: Safety Grafting in Parameter-Space for Medical MLLMs](https://arxiv.org/abs/2601.04199)
...
<details>
  <summary>Details</summary>
Motivation: 医学多模态大语言模型在医疗任务中取得显著进展，但其安全性研究滞后，存在实际部署风险。当前缺乏系统性安全评估框架，且医疗微调过程常导致模型遗忘原有的安全对齐。

Method: 首先建立多维评估框架系统评估现有医学MLLMs的安全漏洞；然后提出"参数空间干预"方法，从原始基础模型中提取内在安全知识表示，并在构建医疗能力时同时注入目标模型；设计细粒度参数搜索算法优化安全与医疗性能的权衡。

Result: 实证分析显示现有模型在通用和医疗特定安全维度存在普遍脆弱性，尤其对跨模态越狱攻击表现脆弱；提出的方法显著增强了医学MLLMs的安全防护，无需额外领域特定安全数据，同时最小化核心医疗性能的退化。

Conclusion: 医学MLLMs存在严重安全漏洞，医疗微调会导致灾难性遗忘安全对齐。提出的参数空间干预方法能有效进行安全再对齐，在安全性和医疗性能间取得良好平衡，为医学MLLMs的安全部署提供了实用解决方案。
...
### [181] [Green MLOps: Closed-Loop, Energy-Aware Inference with NVIDIA Triton, FastAPI, and Bio-Inspired Thresholding](https://arxiv.org/abs/2601.04250)
*Mustapha Hamdi,Mourad Jabou*

Main category: cs.LG

TL;DR: 提出基于蛋白质折叠能量盆地启发的生物启发框架，通过闭环阈值控制AI推理执行，在保持精度的同时显著降低处理时间和能耗


<details>
  <summary>Details</summary>
Motivation: AI部署中能源效率至关重要，长期运行的推理累积碳影响可能超过训练过程，需要更节能的推理方法

Method: 将蛋白质折叠能量盆地映射到推理成本景观，通过衰减的闭环阈值控制执行，仅在预期效用-能量权衡有利时处理请求，偏向第一个可接受的局部盆地而非追求代价高昂的全局最优

Result: 生物控制器相比标准开环执行减少42%处理时间（0.50s vs 0.29s），精度损失小于0.5%，并建立了轻量级本地服务（ORT）与管理批处理（Triton）之间的效率边界

Conclusion: 该研究将生物物理能量模型与绿色MLOps连接，为生产环境中的闭环能量感知推理提供了实用、可审计的基础
...
### [182] [Safety-Utility Conflicts Are Not Global: Surgical Alignment via Head-Level Diagnosis](https://arxiv.org/abs/2601.04262)
*Wang Cai,Yilin Wen,Jinchang Hou,Du Su,Guoqiu Wang,Zhonghou Lv,Chenfu Bao,Yunfang Wu*

Main category: cs.LG

TL;DR: CAST框架通过识别Transformer中高冲突注意力头并跳过其更新，在保持安全性的同时减少通用能力损失，提供参数高效的安全对齐方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全对齐方法采用全局梯度几何解决多目标优化冲突，但忽视了Transformer模块异质性——不同注意力头的功能敏感性和冲突程度差异显著，导致次优权衡。

Method: 提出冲突感知稀疏调优(CAST)框架：1) 构建预对齐冲突图，综合优化冲突和功能敏感性分析；2) 基于冲突图选择性更新参数，跳过高冲突注意力头。

Result: 实验发现LLM中的对齐冲突并非均匀分布，通用能力下降主要来自更新少量"高冲突"头。跳过这些头训练可显著减少能力损失而不影响安全性。

Conclusion: CAST提供了一种可解释且参数高效的方法来改善安全-效用权衡，通过模块级诊断和稀疏调优解决了现有全局方法的局限性。
...
### [183] [Learning to Reason: Temporal Saliency Distillation for Interpretable Knowledge Transfer](https://arxiv.org/abs/2601.04263)
*Nilushika Udayangani Hewa Dehigahawattage,Kishor Nandakishor,Marimuthu Palaniswami*

Main category: cs.LG

TL;DR: 提出一种用于时间序列的可解释知识蒸馏方法Temporal Saliency Distillation，通过转移教师模型的时间显著性知识，让学生模型不仅学习预测结果，还学习教师的推理过程。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列知识蒸馏主要基于计算机视觉的logit和特征对齐方法，存在两个关键问题：1) 转移的知识如何帮助学生模型学习过程不明确（logits和特征不可解释）；2) 仅转移有限知识，主要复制教师预测准确性，导致学生模型的预测分布与教师显著不同，阻碍安全替代教师模型。

Method: 提出Temporal Saliency Distillation方法，将传统logit转移扩展到不仅传递正确预测，还传递教师的正确推理过程。具体从教师logits中提取时间显著性知识，捕捉每个输入时间步对教师预测的重要性。通过训练学生模型使用时间显著性蒸馏，鼓励其基于与教师相同的输入特征进行预测。该方法无需额外参数或特定架构假设。

Result: Temporal Saliency Distillation有效提高了基线方法的性能，同时实现了超越预测准确性的理想特性。

Conclusion: 这项工作为时间序列分析中的可解释知识蒸馏建立了新范式，通过转移可解释的时间显著性知识，让学生模型不仅学习预测结果，还学习教师的推理过程。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型（如RNN、LSTM）在时间序列分析中表现出色，但计算复杂度和模型大小限制了其在资源受限环境（如可穿戴设备、边缘计算平台）中的部署。现有的知识蒸馏方法主要针对计算机视觉任务设计，忽略了时间序列模型特有的时间依赖性和记忆保留特性。

Method: 提出Memory-Discrepancy Knowledge Distillation (MemKD)框架，使用专门的损失函数来捕捉教师和学生模型在时间序列子序列中的记忆保留差异，确保学生模型有效模仿教师模型的行为。

Result: MemKD显著优于现有最先进的知识蒸馏方法，能将参数大小和内存使用减少约500倍，同时保持与教师模型相当的性能。

Conclusion: MemKD框架为资源受限环境中的实时时间序列分析任务提供了紧凑、高性能的循环神经网络解决方案，通过针对时间序列特性的知识蒸馏方法，在保持性能的同时大幅降低了计算资源需求。
...
TL;DR: 使用强化学习在线学习气候模型参数化方案，在多个理想化测试平台上评估，发现TQC、DDPG和TD3算法表现最佳，联邦多智能体设置能实现地理专业化控制


<details>
  <summary>Details</summary>
Motivation: 传统气候模型参数化方案使用离线调优的固定系数，约束弱且存在持续偏差，无法适应底层物理过程，限制了模型的适应能力

Method: 提出一个框架，使用强化学习在线学习参数化方案组件作为模型状态的函数，在简单气候偏差校正、辐射对流平衡和纬向平均能量平衡模型三个理想化测试平台上评估，比较了九种RL算法和单智能体与联邦多智能体设置

Result: TQC、DDPG和TD3算法在所有配置中表现最佳且收敛最稳定；在EBM中，单智能体RL优于静态参数调优，在热带和中纬度带增益最强；联邦多智能体RL能实现地理专业化控制和更快收敛，六智能体DDPG配置在热带和中纬度带获得最低区域加权RMSE
...
TL;DR: 论文提出可预测梯度流形的测量框架，通过预测路径长度和可预测秩量化梯度的时间结构，证明深度学习中梯度轨迹具有局部可预测性和低秩特性。


<details>
  <summary>Details</summary>
Motivation: 深度学习优化中存在未被最坏情况梯度界限捕捉的结构特征。经验上，训练轨迹中的梯度通常具有时间可预测性，并在低维子空间中演化。本文旨在形式化这一观察，建立可预测梯度流形的可测量框架。

Method: 引入两个可计算量：基于预测的路径长度（衡量梯度从过去信息预测的能力）和可预测秩（量化梯度增量的内在时间维度）。重新表述经典在线和非凸优化保证，使收敛和遗憾明确依赖于这些量而非最坏情况变化。

Result: 在卷积网络、视觉Transformer、语言模型和合成控制任务中，发现梯度轨迹具有局部可预测性和随时间变化的强低秩结构。这些特性在不同架构和优化器中保持稳定，可通过轻量级随机投影直接从记录的梯度中诊断。

Conclusion: 结果为理解现代深度学习中的优化动态提供了统一视角，将标准训练重新定义为在低复杂度时间机制中运行。这一视角为自适应优化器、秩感知跟踪和基于预测的算法设计提供了新方向，这些设计基于真实训练运行的可测量特性。
...
<details>
  <summary>Details</summary>
Motivation: 后训练虽然提升了大语言模型的性能，但常常导致置信度校准恶化，使模型变得过度自信。现有的无监督后处理方法仅通过静态输出分布匹配来对齐后训练模型和预训练模型的置信度，忽略了后训练引入的推理时动态变化。

Method: 提出Dual-Align无监督后处理框架，包含两个对齐策略：1) 置信度对齐：通过最终分布匹配纠正置信度漂移；2) 过程对齐：定位轨迹分叉的层，重新对齐后续推理的稳定性以解决过程漂移。该方法仅学习单个温度参数来同时纠正两种漂移类型。

Result: 实验表明Dual-Align相比基线方法持续改进，显著减少校准误差，并接近有监督oracle方法的性能，同时不牺牲后训练带来的性能提升。

Conclusion: 通过诊断后训练模型校准误差的两种机制（置信度漂移和过程漂移），提出的双重对齐框架能够有效解决校准问题，在保持后训练性能增益的同时改善模型校准。
...
<details>
  <summary>Details</summary>
Motivation: 解决航空运输中的数据稀缺和隐私问题，为机场延误时间序列生成合成数据，以支持相关研究和应用。

Method: 比较了三种模型：两种基于深度学习算法，一种简化遗传算法方法，使用欧洲和美国的大规模运营数据进行机场延误时间序列生成。

Result: 简化遗传算法能生成与真实时间序列几乎无法区分且保持高变异性的合成数据，并在检测机场间延误传播问题上得到验证。

Conclusion: 简化遗传算法在生成机场延误合成数据方面表现优异，研究团队将合成数据公开供科学界使用。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成模型的身份遗忘方法面临三个主要挑战：1) 效率低下，需要微调所有模型参数；2) 可控性有限，遗忘强度无法控制且缺乏可解释性；3) 灾难性崩溃，遗忘过程中模型保留能力急剧下降。

Method: LEGATO将身份遗忘建模为连续轨迹，通过神经ODE适配器增强预训练生成器，保持原始模型权重冻结。使用ODE步长精确调节遗忘强度，并引入轨迹一致性约束防止灾难性崩溃。

Result: 在领域内和领域外身份遗忘基准测试中，LEGATO实现了最先进的遗忘性能，避免了灾难性崩溃，并显著减少了需要微调的参数量。

Conclusion: LEGATO通过连续轨迹建模和神经ODE适配器，为生成模型的身份遗忘提供了一种高效、可控且稳定的解决方案，在保持模型性能的同时实现了精确的身份移除。
...
TL;DR: 本文研究字符级Transformer在模加法任务中的鲁棒性，发现模型在输入格式变化时（如位置偏移或不同自然语言模板）会严重失败，并提出结合边界标记、位置课程、多样模板和一致性训练的方法来提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注分布内准确性，而忽视了模型在输入格式变化时的鲁棒性。本文旨在研究字符级Transformer在模加法任务中面对位置偏移和不同自然语言模板时的失败模式，并提出解决方案。

Method: 提出一个简单训练方案：1) 使用显式表达式边界标记；2) 位置课程训练，扩大训练中看到的绝对位置范围；3) 多样模板混合；4) 每个示例跨多个变体的一致性训练。在p=97的模加法任务上进行评估。

Result: 基线模型在分布内表现良好，但在位置偏移和模板OOD时崩溃。提出的方法在三个随机种子下显著提升了位置偏移和模板OOD的鲁棒性，同时保持高分布内准确性。ALiBi风格的消融实验在本设置下无法学习任务。

Conclusion: 在噪声监督下引导程序泛化需要显式训练数据分布中缺失的不变性。本文提供了可复现的评估协议和工具，强调了训练鲁棒性而不仅仅是准确性的重要性。
...
TL;DR: 研究通过集成分类器和滑动窗口后处理技术，提升基于EEG信号异步检测运动意图的鲁棒性，在伪在线评估中显著优于单模型


<details>
  <summary>Details</summary>
Motivation: 中风是导致残疾的主要原因之一，机器人辅助运动治疗需要检测患者运动意图来触发设备辅助。虽然可以从EEG信号检测运动意图，但在线异步分类时解码特别具有挑战性

Method: 分析两个包含14名健康受试者执行自发起手臂运动的EEG数据集，进行离线和伪在线评估，比较SVM、MLP和EEGNet分类模型的集成组合，并应用滑动窗口后处理技术

Result: 伪在线评估显示，在最佳后处理窗口数量下，两种模型集成显著优于最佳单模型。对于单模型，增加后处理窗口数量显著改善分类性能。离线评估中最佳单模型与分类器集成之间没有显著差异

Conclusion: 分类器集成和适当的后处理方法能有效增强从EEG信号异步检测运动意图的能力，特别是在在线分类中比离线分类有更大改进，并能减少误检（早期误报）
...
### [192] [Online Action-Stacking Improves Reinforcement Learning Performance for Air Traffic Control](https://arxiv.org/abs/2601.04287)
*Ben Carvell,George De Ath,Eseoghene Benjamin,Richard Everson*

Main category: cs.LG

TL;DR: 在线动作堆叠：一种推理时包装器，将强化学习策略的简单增量动作转换为符合空中交通管制操作要求的复合指令，显著减少指令数量并保持性能。


<details>
  <summary>Details</summary>
Motivation: 标准强化学习在空管控制中存在动作空间与操作要求不匹配的问题。训练时使用简单离散动作空间，但实际需要发出复杂的复合指令。需要弥合RL公式与操作需求之间的差距。

Method: 提出在线动作堆叠方法：训练时使用简单增量航向或高度调整（5个动作），配合动作阻尼惩罚减少指令频率；推理时将原始动作序列编译为领域合适的复合指令。使用PPO算法和BluebirdDT数字孪生平台进行训练。

Result: 在横向导航实验中，动作堆叠相比阻尼基线大幅减少指令数量，且与使用37维动作空间训练的策略性能相当，尽管只使用5个动作。能有效管理爬升/下降和两机防撞。

Conclusion: 在线动作堆叠有助于弥合标准RL公式与空管操作要求之间的关键差距，为扩展到更复杂控制场景提供了简单机制。
...
### [193] [ArtCognition: A Multimodal AI Framework for Affective State Sensing from Visual and Kinematic Drawing Cues](https://arxiv.org/abs/2601.04297)
*Behrad Binaei-Haghighi,Nafiseh Sadat Sajadi,Mehrad Liviyan,Reyhane Akhavan Kharazi,Fatemeh Amirkhani,Behnam Bahrak*

Main category: cs.LG

TL;DR: ArtCognition：一个多模态框架，通过分析数字绘画（HTP测试）来评估心理状态，融合视觉特征和绘画行为动态特征，使用RAG架构增强可解释性。


<details>
  <summary>Details</summary>
Motivation: 人类情感和心理状态的客观评估具有挑战性，特别是通过非语言渠道。数字绘画作为一种丰富但未充分探索的情感感知模态，可以用于心理评估。

Method: 提出ArtCognition多模态框架，分析HTP测试：1）从最终画作提取静态视觉特征（计算机视觉模型）；2）从绘画过程提取动态行为运动学特征（笔画速度、停顿、流畅度）；3）使用检索增强生成（RAG）架构，将低级特征与高级心理学解释联系起来。

Result: 视觉和行为运动学特征的融合比单一模态提供更细致的评估；提取的多模态特征与标准化心理指标显著相关；验证了该框架作为支持临床医生的可扩展工具的潜力。
...
TL;DR: 该研究提出了一种可解释的深度学习框架，通过整合连续血糖监测数据和实验室指标，识别1型糖尿病患者的五种潜在代谢表型，支持超越单一生物标志物的风险分层。


<details>
  <summary>Details</summary>
Motivation: 1型糖尿病具有高度代谢异质性，传统生物标志物如糖化血红蛋白无法充分表征疾病特征，需要开发能够整合多模态时间序列数据的分析方法来识别不同的代谢亚组。

Method: 提出可解释的深度学习框架，整合连续血糖监测数据和实验室指标，使用Transformer编码器建模跨模态的时间依赖性，通过高斯混合模型识别潜在代谢表型，利用Transformer注意力可视化和SHAP特征归因实现模型可解释性。

Result: 在577名1型糖尿病患者中识别出五种潜在代谢表型（从代谢稳定到心血管代谢风险升高），这些表型在血糖控制、脂质代谢、肾脏标志物和促甲状腺激素水平方面具有显著差异；注意力分析显示血糖变异性是主要时间因素，SHAP分析识别出糖化血红蛋白、甘油三酯、胆固醇、肌酐和促甲状腺激素是表型分化的关键贡献因素。

Conclusion: 该可解释的多模态时间嵌入框架揭示了1型糖尿病中生理学上一致的代谢亚组，支持超越单一生物标志物的风险分层，为个性化治疗和风险评估提供了新方法。
...
TL;DR: 本文定量研究了测试集污染对生成式评估的影响，发现即使少量污染也能显著提升模型性能，并揭示了生成式评估中记忆与生成交互的复杂性。


<details>
  <summary>Details</summary>
Motivation: 随着前沿AI系统在网页规模数据上进行预训练，测试集污染已成为准确评估其能力的关键问题。虽然已有研究深入探讨了测试集污染对判别式评估（如多项选择题回答）的影响，但对生成式评估中污染影响的研究相对较少。

Method: 通过在MATH基准测试的网页数据混合中预训练语言模型，扫描模型大小和污染预训练语料库的测试集副本数量；使用缩放定律分析；研究进一步训练（用新数据过度训练和监督微调）；在推理阶段分析温度采样和解决方案长度对记忆的影响。

Result: 性能随污染和模型大小而提高；即使包含单个测试集副本，模型也能达到比未污染语料库训练不可约误差更低的损失；用新数据过度训练可减少污染影响，而监督微调对测试数据性能的影响取决于预训练污染量；高温采样可缓解污染效应，较长解决方案比较短解决方案指数级更难记忆。

Conclusion: 通过表征生成与记忆的交互作用，本文强调了AI系统可信评估的新复杂性层次，表明生成式评估中的污染影响与判别式评估存在重要差异。
...
TL;DR: 提出一种因果领域自适应方法，用于目标变量在目标域完全缺失的场景，通过学习机制稳定的紧凑表示来实现目标变量的零样本插补。


<details>
  <summary>Details</summary>
Motivation: 解决因果系统中常见的领域自适应问题：目标变量在源域可观测但在目标域完全缺失，需要在各种分布偏移下从其他观测变量插补目标变量。

Method: 1) 针对线性高斯因果模型，推导闭式高斯信息瓶颈(GIB)解，简化为CCA式投影，可提供DAG感知选项；2) 针对非线性/非高斯数据，引入变分信息瓶颈(VIB)编码器-预测器，可扩展到高维数据，支持源域训练、目标域零样本部署。

Result: 在合成和真实数据集上，该方法始终获得准确的插补结果，支持高维因果模型的实际应用，为因果领域自适应提供了统一、轻量级的工具包。

Conclusion: 该方法通过学习机制稳定的紧凑表示，有效解决了目标变量在目标域缺失的因果领域自适应问题，为高维因果模型提供了实用的零样本插补解决方案。
...
### [197] [Phasor Agents: Oscillatory Graphs with Three-Factor Plasticity and Sleep-Staged Learning](https://arxiv.org/abs/2601.04362)
*Rodja Trappe*

Main category: cs.LG

TL;DR: Phasor Agents是一种基于耦合Stuart-Landau振荡器图作为内部状态的动态系统，通过三因素局部可塑性学习耦合权重，无需反向传播，并采用类似睡眠阶段动态的离线巩固机制来稳定学习。


<details>
  <summary>Details</summary>
Motivation: 解决振荡基质中的稳定性挑战：在线权重更新可能导致网络进入不良状态（如全局同步），破坏表示多样性。受突触标记-捕获和睡眠阶段动态启发，将清醒标记与离线巩固分离。

Method: 使用Phasor Graph（耦合Stuart-Landau振荡器的加权图）作为内部状态，振荡器相位跟踪相对时序，振幅跟踪局部增益。通过三因素局部可塑性学习耦合权重：资格迹由稀疏全局调制器和振荡定时写入窗口门控。采用类似睡眠的离线巩固机制：深睡眠式门控捕获安全提交标记变化，REM式回放重建和扰动经验用于规划。

Result: 实验验证显示：资格迹在延迟调制下保持信用；压缩进展信号通过时间戳洗牌控制；相位相干检索在噪声下达到扩散基线的4倍；清醒/睡眠分离在匹配权重范数预算下将稳定学习扩展67%；REM回放将迷宫成功率提高45.5个百分点；出现Tolman式潜在学习特征（无奖励探索后立即具备能力和绕道优势）。

Conclusion: Phasor Agents通过振荡器相位结构作为表示媒介，结合三因素局部可塑性和睡眠启发的离线巩固机制，实现了稳定的无反向传播学习，能够形成内部模型并支持规划能力。
...
### [198] [Survival Dynamics of Neural and Programmatic Policies in Evolutionary Reinforcement Learning](https://arxiv.org/abs/2601.04365)
...
<details>
  <summary>Details</summary>
Motivation: 传统进化强化学习中使用的小型神经网络策略缺乏显式的模块化结构，限制了行为解释。研究者想探究程序化策略（特别是软可微分决策列表）是否能匹配甚至超越神经网络策略的性能。

Method: 1. 提供了首个完全指定且开源的1992年人工生命进化强化学习测试平台重实现；2. 使用软可微分决策列表（SDDL）实现程序化策略；3. 在4000个独立试验中进行严格的生存分析，使用Kaplan-Meier曲线和限制平均生存时间（RMST）指标。

Result: 1. PERL和NERL之间存在统计显著的生存概率差异；2. PERL代理平均比NERL代理多存活201.69步；3. 仅使用学习的SDDL代理比同时使用学习和进化的神经网络代理平均多存活73.67步。

Conclusion: 程序化策略在人工生命环境中能够超越神经网络策略的生存性能，为可解释性和性能更好的策略表示提供了证据。
...
### [199] [Machine Learning Model for Sparse PCM Completion](https://arxiv.org/abs/2601.04366)
*Selcuk Koyuncu,Ronak Nouri,Stephen Providence*

Main category: cs.LG

TL;DR: 提出一种结合经典成对比较矩阵与图学习技术的机器学习模型，用于稀疏成对比较矩阵分析


<details>
  <summary>Details</summary>
Motivation: 传统成对比较矩阵方法在处理稀疏数据时存在局限性，需要结合现代机器学习技术提高效果和可扩展性
...
### [200] [Aligned explanations in neural networks](https://arxiv.org/abs/2601.04378)
...
<details>
  <summary>Details</summary>
Motivation: 当前特征归因方法大多只是对黑盒模型的"粉饰"，缺乏与预测过程的直接联系。作者认为解释对齐是预测任务可信度的关键，解释必须直接与预测相关联，而不是作为事后合理化

Method: 提出PiNets（伪线性网络）框架，产生任意特征空间中的实例级线性预测，实现线性可读性。PiNets通过模型可读性设计原则实现解释对齐

Result: 在图像分类和分割任务上展示了PiNets的应用，证明其产生的解释在多个标准下都具有忠实性，特别是在对齐方面

Conclusion: PiNets通过模型可读性设计原则解决了当前可解释AI中解释与预测脱节的问题，为深度学习提供了更可信的解释框架
...
### [201] [Enhanced-FQL($λ$), an Efficient and Interpretable RL with novel Fuzzy Eligibility Traces and Segmented Experience Replay](https://arxiv.org/abs/2601.04392)
*Mohsen Jalaeian-Farimani*

Main category: cs.LG

TL;DR: Enhanced-FQL(λ)是一种模糊强化学习框架，通过模糊化资格迹和分段经验回放，在连续控制任务中实现高性能、高可解释性和低计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 解决连续控制任务中深度强化学习方法的黑盒特性、高计算复杂性和缺乏理论保证的问题，为安全关键应用提供透明、高效且可靠的解决方案。

Method: 结合模糊Q学习与模糊化贝尔曼方程，引入两个关键创新：1) 模糊化资格迹用于稳定的多步信用分配；2) 分段经验回放机制提高样本效率。使用可解释的模糊规则库而非复杂神经网络架构。

Result: 在连续控制领域评估显示，Enhanced-FQL(λ)相比n步模糊TD和模糊SARSA(λ)基线具有更优的样本效率和更低的方差，同时计算复杂度显著低于DDPG等深度RL方法。理论分析证明方法在标准假设下收敛。

Conclusion: 该框架结合了固有的可解释性、计算效率和理论收敛保证，特别适合安全关键应用，其中透明度和资源约束至关重要。
...
### [202] [Rate or Fate? RLV$^\varepsilon$R: Reinforcement Learning with Verifiable Noisy Rewards](https://arxiv.org/abs/2601.04411)
*Ali Rad,Khashayar Filom,Darioush Keivan,Peyman Mohajerin Esfahani,Ehsan Kamalinejad*

Main category: cs.LG

TL;DR: RLVR（可验证奖励的强化学习）在噪声验证下存在临界相变：当验证器的Youden指数J>0时学习收敛，J<0时系统崩溃，J=0时中性。噪声主要影响收敛速度而非最终结果。


<details>
  <summary>Details</summary>
Motivation: 实际RLVR应用中验证器存在噪声（单元测试覆盖有限、人工/合成标签不完美、LLM判断有噪声且可被利用），尤其在编程等困难领域问题更严重。需要研究验证噪声是仅减慢学习速度，还是可能完全改变学习结果。

Method: 建立可解析处理的多臂老虎机视角分析RLVR动态，用GRPO实例化并通过控制实验验证。建模假阳性和假阴性，将补全分组为重复推理模式，得到概率单纯形上的复制器式（自然选择）流。动态解耦为正确模式内竞争和一维错误模式质量演化。

Result: 发现尖锐的相变：当Youden指数J=TPR-FPR>0时，错误模式质量趋于灭绝（学习）；J=0时过程中性；J<0时错误模式放大直至主导（反学习和崩溃）。在学习区域J>0时，噪声主要重新标度收敛时间（"速度而非命运"）。在合成噪声下的可验证编程任务实验重现了预测的J=0边界。
...
### [203] [Distribution-Guided and Constrained Quantum Machine Unlearning](https://arxiv.org/abs/2601.04413)
*Nausherwan Malik,Zubair Khalid,Muhammad Faryad*

Main category: cs.LG

TL;DR: 提出一种基于分布引导的量子机器学习遗忘框架，通过可调目标分布和解耦约束优化，实现类级量子模型遗忘，在保留性能的同时有效抑制遗忘类置信度。
...
Method: 提出分布引导的类级量子机器学习遗忘框架，将遗忘视为约束优化问题：1) 引入基于模型相似性统计的可调目标分布，解耦遗忘类置信度抑制与保留类分布假设；2) 结合基于锚点的保留约束，在选定保留数据上明确维持预测行为，控制优化轨迹偏离。

Result: 在Iris和Covertype数据集上评估变分量子分类器，结果显示：1) 遗忘类置信度被显著抑制；2) 保留类性能退化最小；3) 相比均匀目标遗忘方法，更接近黄金重训练模型基线。

Conclusion: 目标分布设计和基于约束的公式对于实现可靠且可解释的量子机器学习遗忘至关重要，该方法为量子模型遗忘提供了更精细的控制机制。
...
### [204] [Improving and Accelerating Offline RL in Large Discrete Action Spaces with Structured Policy Initialization](https://arxiv.org/abs/2601.04441)
...
Method: 提出结构化策略初始化(SPIN)框架：1) 预训练动作结构模型(ASM)捕捉有效动作流形；2) 冻结该表示，训练轻量策略头进行控制

Result: 在离散DM Control基准测试中，SPIN相比现有最佳方法平均回报提升高达39%，收敛时间减少高达12.8倍

Conclusion: 通过分离动作结构学习和控制学习，SPIN框架在离散组合动作空间中实现了更高效、更稳定的强化学习，显著提升了性能和训练效率
...
### [205] [When Predictions Shape Reality: A Socio-Technical Synthesis of Performative Predictions in Machine Learning](https://arxiv.org/abs/2601.04447)
*Gal Fybish,Teo Susnjak*

Main category: cs.LG

TL;DR: 这篇SoK论文系统化地回顾了执行性预测（performative prediction）领域的文献，提出了"执行强度vs影响矩阵"评估框架，帮助从业者评估预测模型部署中的执行性风险并选择合适的干预措施。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在高风险领域部署时，其预测会主动塑造其运行环境，这种现象称为执行性预测。这种动态性可能导致反馈循环、性能问题和社会风险。虽然该领域文献增长迅速，但缺乏系统化概念梳理和实践指导的综合研究。

Method: 采用系统化知识（SoK）方法，全面回顾执行性预测文献。通过分析执行性表现的主要机制，建立相关风险的类型学，并调查文献中提出的解决方案。

Result: 提出了"执行强度vs影响矩阵"评估框架，这是一个实用工具，帮助从业者评估执行性对其部署的预测模型的潜在影响和严重程度，并选择合适的算法或人工干预级别。

Conclusion: 该SoK填补了执行性预测领域系统化概念梳理和实践指导的空白，为从业者提供了评估和管理执行性风险的实用框架，有助于在高风险领域更负责任地部署机器学习模型。
...
### [206] [Explainable Admission-Level Predictive Modeling for Prolonged Hospital Stay in Elderly Populations: Challenges in Low- and Middle-Income Countries](https://arxiv.org/abs/2601.04449)
*Daniel Sierra-Botero,Ana Molina-Taborda,Leonardo Espinosa-Leal,Alexander Karpenko,Alejandro Hernandez,Olga Lopez-Acevedo*

Main category: cs.LG

TL;DR: 开发并解释使用入院患者和医院管理数据预测住院时间延长(pLoS)的模型，采用基于信息值和图论的变量选择方法，在验证集上AUC-ROC达到0.82
...
Method: 使用图论方法选择非相关高信息值特征，采用逻辑回归预测pLoS（以7天为界），数据集来自120,354例住院记录，分为训练(67%)、测试(22%)和验证(11%)集

Result: 特征选择得到9个可解释变量，验证集上特异性0.83、敏感性0.64、准确率0.76、精确率0.67、AUC-ROC 0.82，模型表现出强预测性能

Conclusion: 模型具有良好的预测性能并能揭示影响住院时间的因素，是医院管理和未来干预研究的宝贵工具
...
### [207] [Using Large Language Models to Detect Socially Shared Regulation of Collaborative Learning](https://arxiv.org/abs/2601.04458)
...
TL;DR: 本文提出使用嵌入方法自动检测协作计算建模环境中的社会共享学习调节行为，结合LLM摘要、文本嵌入、上下文嵌入和日志特征，发现不同特征对不同类型的SSRL行为检测各有优势。


<details>
  <summary>Details</summary>
Motivation: 当前学习分析领域在自动化检测复杂学习过程方面取得进展，但主要关注个体化问题解决，而非协作开放式问题解决。协作环境既提供更丰富数据，也带来低内聚性等挑战，需要扩展预测模型来检测社会共享学习调节行为。

Method: 使用大型语言模型作为摘要工具，生成与系统日志对齐的学生对话任务感知表示。结合纯文本嵌入、上下文丰富嵌入和日志衍生特征训练预测模型，检测协作计算建模环境中的SSRL行为。

Result: 纯文本嵌入在执行或群体动态相关的SSRL行为检测（如偏离任务或请求帮助）表现更强，而上下文和多模态特征对规划和反思等构念提供互补优势。嵌入模型能扩展学习分析，实现SSRL行为的可扩展检测。

Conclusion: 嵌入模型为扩展学习分析提供了前景，支持协作学习环境中教师重视的实时反馈和适应性支架，最终促进社会共享学习调节行为的规模化检测。
...
### [208] [Meta-probabilistic Modeling](https://arxiv.org/abs/2601.04462)
...
Method: MPM采用分层架构，全局模型规范在多个数据集间共享，而局部参数保持数据集特定性。提出一种可处理的VAE启发式代理目标，通过双层优化进行学习和推理：局部变量通过坐标上升进行解析更新，全局参数使用基于梯度的方法进行训练。

Result: 在面向对象的图像建模和序列文本建模任务上评估MPM，证明它能够使生成模型适应数据，同时恢复有意义的潜在表示。

Conclusion: MPM提供了一种从多个相关数据集中学习生成模型结构的有效方法，通过元学习框架解决了模型规范选择的挑战性问题。
...
### [209] [When Models Manipulate Manifolds: The Geometry of a Counting Task](https://arxiv.org/abs/2601.04480)
...
Method: 机械性地研究Claude 3.5 Haiku模型，发现字符计数在低维弯曲流形上表示，通过几何变换序列：词元长度累积到字符计数流形，注意力头扭曲流形估计到行边界的距离，通过正交排列估计创建线性决策边界

Result: 通过因果干预验证发现，模型使用类似生物位置细胞的稀疏特征家族表示字符计数，并发现视觉幻觉——能够劫持计数机制的字符序列

Conclusion: 这项工作展示了早期层的丰富感官处理、注意力算法的复杂性，以及结合基于特征和几何视角的可解释性的重要性
...
### [210] [Hybrid Federated Learning for Noise-Robust Training](https://arxiv.org/abs/2601.04483)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习(FL)和联邦蒸馏(FD)各有优缺点：FL对噪声更鲁棒但学习速度慢，FD学习速度快但对噪声敏感。需要一种能结合两者优势的混合框架

Method: 提出混合联邦学习(HFL)框架，用户设备可选择传输梯度或logits，基站自适应选择FL和FD更新的权重。采用两种优化方法：1) Jenks优化进行自适应用户设备聚类；2) 阻尼牛顿法进行自适应权重选择

Result: 推导了HFL框架的收敛性，数值结果显示当同时利用两种自由度时，HFL在低信噪比下实现了优越的测试精度
...
### [211] [IGenBench: Benchmarking the Reliability of Text-to-Infographic Generation](https://arxiv.org/abs/2601.04498)
...
TL;DR: IGENBENCH是首个评估文本到信息图生成可靠性的基准测试，包含600个测试案例和30种信息图类型，使用多模态大语言模型进行自动化评估，揭示了当前模型在数据完整性等方面的瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型虽然能生成美观的图像，但在生成信息图时的可靠性尚不明确，可能存在数据编码扭曲或文本内容错误等容易被忽视的问题，需要建立专门的评估基准。

Method: 构建了包含600个测试案例和30种信息图类型的IGENBENCH基准，设计了自动化评估框架，将可靠性验证分解为基于10种问题类型的原子是/否问题，使用多模态大语言模型进行验证。

Result: 评估了10个最先进的文本到图像模型，发现：1）三层性能层次结构，最佳模型问题级准确率为0.90但信息图级准确率仅为0.49；2）数据相关维度成为普遍瓶颈（如数据完整性：0.21）；3）所有模型都难以实现端到端正确性。

Conclusion: IGENBENCH为文本到信息图生成提供了首个系统性评估基准，揭示了当前模型在数据可靠性方面的显著不足，为未来模型开发提供了关键见解和方向。
...
### [212] [Surface-based Molecular Design with Multi-modal Flow Matching](https://arxiv.org/abs/2601.04506)
*Fang Wu,Zhengyuan Zhou,Shuting Jin,Xiangxiang Zeng,Jure Leskovec,Jinbo Xu*

Main category: cs.LG

TL;DR: SurfFlow是一种基于表面的生成算法，用于肽的序列、结构和表面全面协同设计，在肽设计基准测试中优于全原子基线方法。


<details>
  <summary>Details</summary>
Motivation: 治疗性肽在靶向以前不可药物化的结合位点方面显示出潜力，但分子表面在蛋白质-蛋白质相互作用中的关键作用尚未得到充分探索。现有方法主要关注全原子设计，而忽略了表面几何和生化性质的重要性。

Method: 提出SurfFlow，一种基于表面的生成算法，采用多模态条件流匹配架构学习表面几何和生化性质的分布，实现肽序列、结构和表面的全面协同设计。

Result: 在全面的PepMerge基准测试中，SurfFlow在所有指标上始终优于全原子基线方法，证明了考虑分子表面在肽设计中的优势。
...
### [213] [TSSR: Two-Stage Swap-Reward-Driven Reinforcement Learning for Character-Level SMILES Generation](https://arxiv.org/abs/2601.04521)
*Jacob Ede Levine,Yun Lyan Luo,Sai Chandra Kosaraju*

Main category: cs.LG

TL;DR: TSSR是一个两阶段强化学习框架，通过字符级SMILES生成改进分子设计，提高语法和化学有效性而不降低多样性。


<details>
  <summary>Details</summary>
Motivation: 当前基于SMILES的化学语言模型存在复合标记错误问题，许多样本无法解析或化学上不可行，而硬约束又会限制探索空间。需要一种方法既能提高生成质量又不牺牲多样性。

Method: TSSR采用两阶段强化学习框架：第一阶段奖励局部标记交换以修复语法错误；第二阶段通过RDKit诊断提供化学感知反馈，奖励减少化合价、芳香性和连接性问题。该方法模型无关，无需特定任务标签或手工语法。

Result: 在MOSES基准测试中，纯强化学习模式下TSSR显著提高了语法有效性、化学有效性和新颖性；在微调强化学习模式下，保持了药物相似性和可合成性，同时提高了有效性和新颖性。标记级分析显示语法编辑和化学修复共同减少了RDKit检测的错误。

Conclusion: TSSR将稀疏的终端目标转化为更密集、更可解释的奖励，在不降低多样性的情况下提高了语法和化学质量。该方法具有数据集无关性，可适应各种强化学习方法。
...
### [214] [Not All Steps are Informative: On the Linearity of LLMs' RLVR Training](https://arxiv.org/abs/2601.04537)
*Tianle Wang,Zhongyuan Wu,Shenghao Jin,Hao Xu,Wei Chen,Ning Miao*

Main category: cs.LG

TL;DR: RLVR训练中LLM呈现强线性演化，通过权重/对数概率外推可预测未来模型状态，大幅减少计算成本


<details>
  <summary>Details</summary>
Motivation: RLVR训练需要数千步且计算成本高，作者发现LLM在RLVR中呈现强线性演化，这启发通过外推预测未来模型状态来避免持续训练

Method: 提出两种外推方法：权重外推（Weight Extrapolation）和对数概率外推（Logits Extrapolation），利用RL训练中模型演化的线性特性来预测未来模型状态

Result: 权重外推获得与标准RL训练相当的性能但计算量显著减少；对数概率外推在四个基准测试中均优于持续RL训练，特别是在RL训练稳定范围之外

Conclusion: RLVR中LLM的强线性演化特性使得外推成为有效的训练加速方法，可大幅减少计算成本同时保持或提升模型性能
...
### [215] [Timeliness-Oriented Scheduling and Resource Allocation in Multi-Region Collaborative Perception](https://arxiv.org/abs/2601.04542)
*Mengmeng Zhu,Yuxuan Sun,Yukuan Jia,Wei Chen,Bo Ai,Sheng Zhou*

Main category: cs.LG

TL;DR: 提出TAMP调度算法，通过权衡感知精度和通信资源使用，解决多区域协同感知中的动态调度问题
...
Method: 提出TAMP调度算法，基于Lyapunov优化策略，将长期平均目标分解为每时隙优先级问题，平衡调度价值和资源成本。提出经验惩罚函数，将信息年龄和通信量的联合影响映射到感知性能

Result: 在真实世界RCooper数据集上的交叉路口和走廊场景中进行广泛仿真，TAMP优于最佳基线，在各种配置下平均精度提升高达27%

Conclusion: TAMP算法有效解决了多区域协同感知中的动态调度问题，在感知精度和通信资源使用之间取得了良好平衡
...
### [216] [GEnSHIN: Graphical Enhanced Spatio-temporal Hierarchical Inference Network for Traffic Flow Prediction](https://arxiv.org/abs/2601.04550)
*Zhiyan Zhou,Junjie Liao,Manho Zhang,Yingyi Liao,Ziai Wang*

Main category: cs.LG

TL;DR: 提出GEnSHIN网络用于交通流预测，通过注意力增强图卷积循环单元、非对称双嵌入图生成和动态记忆库三个创新设计，在METR-LA数据集上取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 随着城市化加速，智能交通系统对准确交通流预测的需求日益增长。现有方法难以有效处理交通流中复杂的时空依赖关系，需要更强大的建模能力。

Method: 提出GEnSHIN网络，包含三个核心设计：1) 注意力增强图卷积循环单元(GCRU)，引入Transformer模块增强长期时序依赖建模；2) 非对称双嵌入图生成机制，结合真实路网和数据驱动的潜在非对称拓扑；3) 动态记忆库模块，使用可学习交通模式原型为每个传感器节点提供个性化表示，并在解码阶段引入轻量级图更新器适应路网状态动态变化。

Result: 在METR-LA公开数据集上的实验表明，GEnSHIN在MAE、RMSE、MAPE等多个指标上达到或超过对比模型性能。特别是在早晚高峰时段表现出优异的预测稳定性。消融实验验证了各核心模块的有效性及其对最终性能的贡献。
...
### [217] [Improving Semi-Supervised Contrastive Learning via Entropy-Weighted Confidence Integration of Anchor-Positive Pairs](https://arxiv.org/abs/2601.04555)
*Shogo Nakayama,Masahiro Okuda*

Main category: cs.LG

TL;DR: 提出基于熵的置信度自适应加权损失函数，改进半监督对比学习中伪标签分配策略，提升分类准确率和学习稳定性
...
Method: 提出基于预测概率分布熵的置信度估计方法，使用置信度自适应加权，允许对之前被排除的样本也分配伪标签，并考虑锚点和正样本的置信度进行更原则化的对比学习

Result: 实验结果表明，该方法提高了分类准确率，即使在低标签条件下也能实现更稳定的学习性能

Conclusion: 基于熵的置信度自适应加权方法能够更有效地利用半监督数据，改进对比学习效果，特别是在标签稀缺的情况下
...
### [218] [A Vision for Multisensory Intelligence: Sensing, Synergy, and Science](https://arxiv.org/abs/2601.04563)
*Paul Pu Liang*
...
<details>
  <summary>Details</summary>
Motivation: 当前人工智能主要局限于文本、视觉和音频等数字模态，而人类对世界的体验是多感官的。为了让人工智能更好地理解和交互，需要将其扩展到包括触觉、味觉、嗅觉等所有人类感官，以及生理信号、环境信号等更丰富的物理世界信息。

Method: 提出通过三个相互关联的主题推进多感官AI发展：1）传感：扩展AI捕获世界的方式，超越数字媒介；2）科学：建立量化多模态异质性和交互的原理性科学，开发统一建模架构和表示，理解跨模态迁移；3）协同：学习模态之间以及人与AI之间的协同，涵盖多感官整合、对齐、推理、生成、泛化和体验。

Result: 论文提出了一个完整的研究框架和愿景，并附带了MIT媒体实验室多感官智能小组的最新项目、资源和演示，展示了该领域的进展和潜力。

Conclusion: 多感官人工智能是未来十年的重要研究方向，通过连接AI与人类感官及丰富的物理世界信号，可以改变人类与AI的交互方式。该领域需要在传感、科学和协同三个方面系统推进，以实现更自然、丰富的人机交互体验。
...
### [219] [Spatial-Temporal Feedback Diffusion Guidance for Controlled Traffic Imputation](https://arxiv.org/abs/2601.04572)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于分数的扩散模型在时空交通数据插值时，通常对所有节点使用统一的引导尺度，这对于高缺失率节点效果不佳。稀疏观测值提供的条件引导不足，导致生成过程偏向先验分布而非条件观测，从而影响插值性能。

Method: FENCE采用两种创新机制：1）基于后验似然近似的动态反馈机制，根据生成值与观测值的偏差自适应调整引导尺度；2）基于注意力分数对节点进行聚类，在聚类级别计算引导尺度，利用时空相关性提供更精确的引导。

Result: 在真实交通数据集上的实验结果表明，FENCE显著提高了插值准确性。
...
### [220] [FedKDX: Federated Learning with Negative Knowledge Distillation for Enhanced Healthcare AI Systems](https://arxiv.org/abs/2601.04587)
*Quang-Tu Pham,Hoang-Dieu Vu,Dinh-Dat Pham,Hieu H. Pham*

Main category: cs.LG

TL;DR: FedKDX是一个联邦学习框架，通过负知识蒸馏(NKD)解决医疗AI中的局限性，在保护隐私的同时提升模型泛化能力，在非IID数据上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有医疗AI方法主要关注正向知识转移，缺乏对非目标信息的利用，难以处理分布式医疗数据中的统计异质性，同时需要满足HIPAA和GDPR等隐私法规要求。

Method: 提出FedKDX框架，整合传统知识蒸馏、对比学习和负知识蒸馏(NKD)，NKD同时捕获目标和目标信息，在统一架构中实现隐私保护并降低通信成本。

Result: 在SLEEP、UCI-HAR和PAMAP2医疗数据集上，FedKDX相比最先进方法准确率提升最高达2.53%，收敛更快，在非IID数据分布上表现更好。

Conclusion: FedKDX通过负知识蒸馏有效处理医疗数据统计异质性，在隐私敏感医疗应用中平衡性能与实施需求，为符合HIPAA和GDPR的分布式医疗场景提供实用解决方案。
...
### [221] [Density Matrix RNN (DM-RNN): A Quantum Information Theoretic Framework for Modeling Musical Context and Polyphony](https://arxiv.org/abs/2601.04592)
*Joonwon Seo,Mariana Montiel*

Main category: cs.LG

TL;DR: 提出密度矩阵RNN（DM-RNN），利用量子力学中的密度矩阵概念来建模音乐中的模糊性和不确定性，通过量子通道和Choi-Jamiolkowski同构确保物理有效性。


<details>
  <summary>Details</summary>
Motivation: 传统RNN将音乐上下文总结为确定性隐藏状态向量，这种信息瓶颈无法捕捉音乐固有的模糊性。音乐中常存在多种可能的解释和不确定性，需要更丰富的表示方法。

Method: 提出密度矩阵RNN（DM-RNN）架构，使用密度矩阵表示音乐解释的统计集合（混合态）。采用量子通道（CPTP映射）定义时间动态，基于Choi-Jamiolkowski同构进行参数化，确保学习到的动态保持物理有效性。引入冯·诺依曼熵量化音乐不确定性，量子互信息测量声部间的纠缠。

Result: DM-RNN为建模复杂、模糊的音乐结构提供了数学严谨的框架，能够同时捕捉经典概率和量子相干性，突破了传统RNN的信息瓶颈限制。

Conclusion: 密度矩阵RNN通过量子力学概念为音乐建模提供了新的理论框架，能够更好地处理音乐中的模糊性和不确定性，为复杂音乐结构分析开辟了新途径。
...
### [222] [DeepHalo: A Neural Choice Model with Controllable Context Effects](https://arxiv.org/abs/2601.04616)
*Shuhan Zhang,Zhi Wang,Rui Gao,Shuang Li*

Main category: cs.LG

TL;DR: DeepHalo是一个神经建模框架，用于建模人类决策中的上下文效应（光环效应），支持特征建模、显式控制交互阶数，并提供可解释性。


<details>
  <summary>Details</summary>
Motivation: 人类决策建模在推荐、偏好学习和人机对齐等应用中至关重要。传统模型假设上下文无关的选择行为，但行为研究表明偏好常受选择集组成影响（上下文效应/光环效应），现有模型要么缺乏特征支持，要么交互结构受限或可解释性差。

Method: 提出DeepHalo神经网络框架，结合特征建模，显式控制交互阶数（支持成对和高阶交互），提供上下文效应的原则性解释。模型能系统识别不同阶数的交互效应，在无特征设置下可作为上下文依赖选择函数的通用逼近器。

Result: 在合成和真实数据集上的实验表明，DeepHalo具有强大的预测性能，同时为选择驱动因素提供更高的透明度。

Conclusion: DeepHalo框架成功解决了现有上下文效应模型的局限性，在保持预测准确性的同时提供了更好的可解释性，为理解人类决策中的复杂交互效应提供了有效工具。
...
### [223] [Learning Dynamics in RL Post-Training for Language Models](https://arxiv.org/abs/2601.04670)
*Akiyoshi Tomihari*

Main category: cs.LG

TL;DR: 本文通过神经正切核(NTK)框架分析RL后训练的学习动态，发现特征表示有限变异性导致模型置信度系统性增加，解释了输出多样性减少现象，并提出了优先更新分类器的两阶段训练策略CF-RL。


<details>
  <summary>Details</summary>
Motivation: RL后训练在语言模型开发中至关重要，但存在输出多样性减少等未解现象。本文旨在从监督学习中研究但RL中未充分探索的视角，更广泛地理解RL后训练的学习动态。

Method: 采用经验神经正切核(NTK)框架，将NTK分解为两个组件来表征RL更新在训练样本间的传播。基于分析提出了分类器优先强化学习(CF-RL)，这是一种优先更新分类器再进行标准RL优化的两阶段训练策略。

Result: 分析表明特征表示有限变异性导致RL更新系统性增加模型置信度，解释了输出多样性减少。实验验证了CF-RL能增加模型置信度并加速优化，且其机制不同于监督学习中的线性探测再微调。

Conclusion: 本研究形式化了RL后训练的学习动态，为理解输出多样性减少现象提供了理论解释，提出的CF-RL策略展示了实际效果，激励了进一步分析和改进。
...
### [224] [Estimating Causal Effects in Gaussian Linear SCMs with Finite Data](https://arxiv.org/abs/2601.04673)
*Aurghya Maiti,Prateek Jain*

Main category: cs.LG

TL;DR: 提出CGL-SCMs新模型类别，通过标准化外生变量分布解决GL-SCMs过参数化问题，开发EM估计算法从有限观测数据中学习参数和估计可识别因果效应。
...
Method: 提出集中化高斯线性SCMs(CGL-SCMs)新子类，其中外生变量遵循标准化分布。开发基于EM的估计算法，可以从有限观测样本中学习CGL-SCM参数并估计可识别的因果效应。

Result: 理论分析表明CGL-SCMs在因果效应可识别性方面与GL-SCMs具有同等表达能力。在合成数据和基准因果图上的实验验证了学习模型能准确恢复因果分布。

Conclusion: CGL-SCMs为解决GL-SCMs过参数化问题提供了有效的简化模型，提出的EM算法能够从有限观测数据中可靠地估计因果效应，为存在潜在混杂因素的因果推断提供了实用解决方案。
...
### [225] [Nightmare Dreamer: Dreaming About Unsafe States And Planning Ahead](https://arxiv.org/abs/2601.04686)
*Oluwatosin Oseni,Shengjie Wang,Jun Zhu,Micah Corah*

Main category: cs.LG

TL;DR: Nightmare Dreamer是一种基于模型的安全强化学习算法，通过世界模型预测安全违规，在保持高奖励的同时实现近乎零安全违规，在Safety Gymnasium任务上效率提升20倍


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习在机器人控制等实际应用中取得了显著成功，但由于缺乏足够的安全保证，其采用仍然受限。需要解决RL的安全性问题以促进其更广泛的应用。

Method: 提出Nightmare Dreamer算法，这是一种基于模型的安全强化学习方法。它通过学习的世界模型来预测潜在的安全违规，并据此规划动作。算法仅使用图像观测就能工作。

Result: 在Safety Gymnasium任务上，Nightmare Dreamer实现了近乎零的安全违规，同时最大化奖励。与无模型基线相比，仅使用图像观测就实现了近20倍的效率提升。
...
### [226] [Do LLMs Benefit from User and Item Embeddings in Recommendation Tasks?](https://arxiv.org/abs/2601.04690)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM推荐方法主要依赖文本语义或仅有限地结合协同信号（通常只使用用户或物品嵌入），难以处理代表用户历史的多个物品嵌入，往往回归到文本语义而忽略了更丰富的协同信息

Method: 通过独立的轻量级投影模块，将协同过滤学习到的用户和物品嵌入投影到LLM标记空间，然后微调LLM使其能够基于这些投影嵌入和文本标记共同生成推荐

Result: 初步结果显示，该方法能有效利用结构化的用户-物品交互数据，相比纯文本LLM基线提升了推荐性能，为传统推荐系统与现代LLM的融合提供了实用路径

Conclusion: 该方法简单而有效，成功地将协同过滤的嵌入表示与LLM的文本处理能力相结合，为LLM在推荐系统中的应用提供了更全面的信息利用方式
...
### [227] [A zone-based training approach for last-mile routing using Graph Neural Networks and Pointer Networks](https://arxiv.org/abs/2601.04705)
*Àngel Ruiz-Fas,Carlos Granell,José Francisco Ramos,Joaquín Huerta,Sergio Trilles*

Main category: cs.LG

TL;DR: 提出基于深度学习的地理分区方法优化最后一公里配送路线，通过分区训练模型减少预测路线长度，尤其适用于多站点路线


<details>
  <summary>Details</summary>
Motivation: 电子商务快速增长使最后一公里配送网络面临极限，传统启发式方法在旅行时间高度不对称（如单行道、拥堵）时难以适应，需要更智能的路由优化方案来降低成本、加快服务并减少排放

Method: 1) 使用编码器-解码器架构：图神经网络编码器生成节点嵌入捕获站点空间关系，指针网络解码器顺序选择下一站点；2) 基于离散全球网格系统划分地理区域，每个区域训练独立模型；3) 将路线表示为完全有向图，边权重为不对称旅行时间

Result: 使用2021年亚马逊最后一公里路由挑战的洛杉矶路线进行评估，分区训练相比通用训练减少了平均预测路线长度，且随着每路线站点数量增加，分区方法的性能提升更加显著

Conclusion: 基于深度学习的地理分区方法能有效优化最后一公里配送路线，特别适用于旅行时间不对称和多站点的复杂场景，分区训练策略能显著提升模型性能
...
### [228] [MQ-GNN: A Multi-Queue Pipelined Architecture for Scalable and Efficient GNN Training](https://arxiv.org/abs/2601.04707)
*Irfan Ullah,Young-Koo Lee*

Main category: cs.LG

TL;DR: MQ-GNN：一个多队列流水线框架，通过交错GNN训练阶段和优化资源利用率，实现多GPU GNN训练的高效扩展，相比基线模型训练速度提升高达4.6倍。
...
Method: 提出MQ-GNN框架，包含三个关键技术：1) Ready-to-Update Asynchronous Consistent Model (RaCoM)，支持异步梯度共享和模型更新，通过自适应周期性同步确保全局一致性；2) 全局邻居采样与缓存，减少数据传输开销；3) 自适应队列大小策略，平衡计算和内存效率。

Result: 在四个大规模数据集和十个基线模型上的实验表明，MQ-GNN实现了高达4.6倍的训练速度提升和30%的GPU利用率改进，同时保持了有竞争力的准确性。
...
### [229] [GPU-Accelerated INT8 Quantization for KV Cache Compression in Large Language Models](https://arxiv.org/abs/2601.04719)
*Maanas Taneja,Purab Shingvi*

Main category: cs.LG

TL;DR: 该论文实现了GPU加速的INT8量化来压缩LLM推理中的KV缓存，减少4倍内存占用，同时保持可接受的精度损失


<details>
  <summary>Details</summary>
Motivation: 大型语言模型推理时，KV缓存会线性增长并超过模型权重本身的内存占用，成为显著的内存瓶颈

Method: 开发了四种CUDA内核变体（朴素、分块、粗粒度、向量化）来实现GPU加速的INT8量化，并在高达10亿元素的真实工作负载上进行基准测试

Result: 向量化内核相比CPU基线实现高达1694倍加速，重建误差低于0.004，注意力分数误差低于0.1（即使是8K维度的注意力头），计算开销仅为6-58ms
...
### [230] [Excess Description Length of Learning Generalizable Predictors](https://arxiv.org/abs/2601.04728)
...
<details>
  <summary>Details</summary>
Motivation: 理解微调是激发潜在能力还是教授新能力是语言模型评估和安全的基本问题，需要量化微调从训练数据中提取的信息量

Method: 基于预序列编码开发信息论框架，定义Excess Description Length (EDL)：使用在线训练模型顺序编码训练标签所需的比特数与最终训练模型下剩余编码成本之间的差距

Result: EDL非负期望收敛，在无限数据极限下收敛到剩余描述长度，提供期望泛化增益的界限；通过玩具模型澄清学习中的信息混淆，展示能力激发与教学具有不同的缩放特征

Conclusion: 该框架为能力激发与教学表现出不同质量缩放特征的实证观察提供了严格理论基础，有助于理解微调如何从数据中提取结构信息
...
### [231] [Fast Mining and Dynamic Time-to-Event Prediction over Multi-sensor Data Streams](https://arxiv.org/abs/2601.04741)
*Kota Nakamura,Koki Kawabata,Yasuko Matsubara,Yasushi Sakurai*

Main category: cs.LG

TL;DR: TimeCast：一个动态预测框架，用于实时预测机器故障时间，能自适应数据流中的模式变化
...
Method: 提出TimeCast框架，识别时间演化的不同模式（阶段），为每个阶段学习单独模型，基于模式转换进行自适应预测

Result: 在真实数据集上的实验表明，TimeCast比现有方法提供更高的预测准确性，同时显著减少计算时间

Conclusion: TimeCast是一个动态、实用且可扩展的预测框架，能够有效处理实时传感器数据流中的模式变化，实现准确的故障时间预测
...
### [232] [Intraday spatiotemporal PV power prediction at national scale using satellite-based solar forecast models](https://arxiv.org/abs/2601.04751)
*Luca Lanzilao,Angela Meyer*

Main category: cs.LG

TL;DR: 该研究提出了一个时空光伏功率预测框架，评估了7种日内光伏功率临近预报模型，发现基于卫星的方法在短预报时效内优于数值天气预报，其中SolarSTEPS和SHADECast表现最佳。


<details>
  <summary>Details</summary>
Motivation: 这是首个在全国范围内研究时空光伏预测的研究，旨在评估不同光伏功率临近预报模型的可靠性、锐度和整体性能，并可视化中尺度云系统如何影响国家层面的光伏生产。

Method: 提出了一个新颖的时空光伏功率预测框架，包含7种模型（基于卫星的深度学习和光流方法、物理基础的数值天气预报模型），首先用卫星衍生的地表太阳辐照度验证，然后通过站点特定的机器学习模型将辐照度转换为光伏功率，使用瑞士6434个光伏站点的生产数据进行比较。

Result: 基于卫星的方法（特别是SolarSTEPS和SHADECast）在短预报时效内优于集成预报系统；SHADECast提供最可靠的集合分布；确定性模型IrradianceNet获得最低均方根误差；SolarSTEPS和SHADECast的概率预报提供更好校准的不确定性；预报技能随海拔升高而降低；在国家层面，基于卫星的模型在2019-2020年82%的天数中，每日总光伏发电的相对误差低于10%。

Conclusion: 基于卫星的临近预报模型在时空光伏功率预测方面表现出色，特别是SolarSTEPS和SHADECast，展示了其稳健性和在业务应用中用于国家层面光伏发电预测的潜力。
...
### [233] [Smart IoT-Based Wearable Device for Detection and Monitoring of Common Cow Diseases Using a Novel Machine Learning Technique](https://arxiv.org/abs/2601.04761)
*Rupsa Rani Mishra,D. Chandrasekhar Rao,Ajaya Kumar Tripathy*

Main category: cs.LG

TL;DR: 提出基于物联网和机器学习的奶牛疾病自动监测系统，解决传统人工观察劳动密集、成本高、准确性低的问题，实现多疾病同时检测


<details>
  <summary>Details</summary>
Motivation: 大规模养殖中人工观察奶牛疾病存在劳动密集、耗时、准确性低的问题，导致疾病识别延迟，影响动物健康和农场生产力，且人力资源管理复杂、成本高

Method: 提出物联网赋能的网络物理系统框架，监测奶牛日常活动和健康状况；开发新型机器学习算法，通过分析收集的生理和行为数据来诊断常见奶牛疾病

Result: 算法能够通过分析全面的生理和行为特征集来预测多种疾病，实现准确高效的健康评估

Conclusion: 物联网、机器学习和网络物理系统的进步使得奶牛健康监测自动化成为可能，该系统能提高准确性并降低运营成本
...
### [234] [AgentOCR: Reimagining Agent History via Optical Self-Compression](https://arxiv.org/abs/2601.04786)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体系统在多轮交互中会产生大量文本历史，导致token预算和内存使用急剧增长，成为实际部署的瓶颈。需要一种更高效的历史表示方法来降低计算成本。

Method: 1) 将累积的观察-行动历史渲染为紧凑的图像表示；2) 提出分段光学缓存机制，将历史分解为可哈希的片段并维护视觉缓存，消除冗余重渲染；3) 引入智能自压缩机制，让智能体主动发出压缩率，通过压缩感知奖励训练，自适应平衡任务成功和token效率。

Result: 在ALFWorld和基于搜索的QA等挑战性智能体基准测试中，AgentOCR在保持超过95%文本智能体性能的同时，显著减少token消耗(>50%)，实现一致的token和内存效率。分段光学缓存带来20倍渲染加速，自压缩机制有效实现战略平衡。

Conclusion: AgentOCR通过视觉token的信息密度优势，结合缓存和自适应压缩机制，为LLM智能体系统提供了一种高效、可扩展的解决方案，解决了多轮交互中的token膨胀问题。
...
### [235] [Neural-Symbolic Integration with Evolvable Policies](https://arxiv.org/abs/2601.04799)
*Marios Thoma,Vassilis Vassiliades,Loizos Michael*

Main category: cs.LG

TL;DR: 提出一个通过进化过程同时学习不可微符号策略和神经网络权重的神经符号框架，无需预定义策略或可微性要求


<details>
  <summary>Details</summary>
Motivation: 现有神经符号框架需要预定义符号策略或可微策略，限制了在缺乏领域专家知识或策略本身不可微情况下的应用
...
Result: 从空策略和随机神经权重开始的神经符号系统能够成功近似隐藏的不可微目标策略，中位正确性能接近100%

Conclusion: 该框架使神经符号研究能够在难以或无法从专家获取符号知识的领域中开展，代表了神经符号人工智能的重要进展
...
### [236] [Parallelizing Node-Level Explainability in Graph Neural Networks](https://arxiv.org/abs/2601.04807)
*Oscar Llorente,Jaime Boal,Eugenio F. Sánchez-Úbeda,Antonio Diaz-Cano,Miguel Familiar*

Main category: cs.LG

TL;DR: 本文提出了一种通过图分区并行化GNN节点级可解释性计算的新方法，解决了大规模图可解释性计算效率低下的问题。


<details>
  <summary>Details</summary>
Motivation: 在节点分类任务中，随着图规模增大，计算节点级可解释性变得极其耗时，而批处理策略往往会降低解释质量，需要一种既能提高效率又不影响正确性的可扩展方法。
...
Conclusion: 提出的图分区并行化方法显著提高了GNN节点级可解释性的计算效率和可扩展性，同时保证了结果的正确性，为大规模图神经网络的可解释性分析提供了实用解决方案。
...
### [237] [Rethinking GNNs and Missing Features: Challenges, Evaluation and a Robust Solution](https://arxiv.org/abs/2601.04855)
*Francesco Ferrini,Veronica Lachi,Antonio Longa,Bruno Lepri,Matono Akiyoshi,Andrea Passerini,Xin Liu,Manfred Jaeger*

Main category: cs.LG

TL;DR: 论文针对图神经网络处理缺失节点特征的问题，提出更真实的评估数据集和缺失机制，并提出了简单有效的基线方法GNNmim。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多在相对理想的情况下评估GNN处理缺失特征的能力：使用高维稀疏特征和完全随机缺失机制。这限制了模型鲁棒性的真实评估，无法有效比较不同方法在现实场景中的表现。

Method: 1) 理论证明高稀疏性会限制缺失造成的信息损失；2) 引入一个合成和三个真实世界数据集，具有密集且语义丰富的特征；3) 设计超越MCAR的更真实缺失机制评估协议；4) 提出GNNmim方法作为节点分类的基线。

Result: 实验表明，GNNmim在多种数据集和缺失机制下都能与专门架构竞争，证明了其有效性。同时，新数据集和评估协议能更真实地反映不同方法在现实缺失场景下的性能差异。

Conclusion: 论文通过更真实的评估设置揭示了现有基准的局限性，提出了改进的数据集和缺失机制，并展示了简单基线方法GNNmim在实际应用中的竞争力，为GNN处理缺失特征的研究提供了更可靠的评估框架。
...
### [238] [FibreCastML: An Open Web Platform for Predicting Electrospun Nanofibre Diameter Distributions](https://arxiv.org/abs/2601.04873)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法主要预测电纺纤维的平均直径，忽略了完整的直径分布对支架性能的关键影响，需要开发能够预测完整分布的方法来更好地优化电纺支架结构。

Method: 构建包含68538个纤维直径测量的元数据集，使用6个标准工艺参数训练7种机器学习模型，采用嵌套交叉验证和留一研究外部折叠，通过变量重要性分析、SHAP、相关矩阵和三维参数图实现模型可解释性。

Result: 非线性模型显著优于线性基线，对多种常用聚合物达到0.91以上的决定系数，溶液浓度是纤维直径分布的主要驱动因素，实验验证显示预测分布与实测分布高度一致。

Conclusion: FibreCastML框架能够准确预测电纺纤维的完整直径分布，提供可解释的工艺-结构关系，实现了更可重复和数据驱动的电纺支架结构优化。
...
### [239] [Learnable Multipliers: Freeing the Scale of Language Model Matrix Layers](https://arxiv.org/abs/2601.04890)
*Maksim Velikanov,Ilyas Chahed,Jingwei Zuo,Dhia Eddine Rhaiem,Younes Belkada,Hakim Hacid*

Main category: cs.LG

TL;DR: 提出可学习乘子方法，通过引入可学习的标量、行、列乘子来优化权重矩阵的尺度，打破权重衰减与梯度噪声的平衡，提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统训练中权重衰减与随机梯度噪声会形成平衡，导致权重矩阵的范数被限制在次优水平。这种平衡被视为训练过程中的有害人为产物，需要打破以获得更优的权重尺度。

Method: 引入可学习的乘子来优化权重矩阵尺度：1) 为权重矩阵W添加可学习的标量乘子；2) 进一步引入可学习的行乘子和列乘子，分别调整每行和每列的尺度。该方法可视为muP乘子的可学习、更灵活泛化版本。

Result: 可学习乘子方法优于精心调优的muP基线，减少了乘子调优的计算开销，并在Adam和Muon优化器上均验证有效。下游评估显示，该方法带来的改进与从Adam切换到Muon的改进相当。

Conclusion: 可学习乘子能有效打破权重衰减与梯度噪声的平衡约束，让权重尺度根据数据自适应优化，提升模型性能，同时提出了前向传播对称性和学习乘子宽度缩放等实际问题的思考。
...
### [240] [Distributed Online Convex Optimization with Efficient Communication: Improved Algorithm and Lower bounds](https://arxiv.org/abs/2601.04907)
*Sifan Yang,Wenhao Yang,Wei Jiang,Lijun Zhang*

Main category: cs.LG

TL;DR: 本文研究了分布式在线凸优化中的压缩通信问题，提出了新的算法显著改进了现有遗憾边界，消除了对压缩质量因子的二次/四次依赖，并建立了首个下界证明最优性。


<details>
  <summary>Details</summary>
Motivation: 现有分布式在线凸优化算法在压缩通信场景下存在两个主要问题：1) 遗憾边界对压缩质量因子ω有二次甚至四次依赖，这在压缩质量较差时严重影响性能；2) 对学习器数量n有超线性依赖，这在网络规模大时不可接受。需要设计新算法克服这些限制。

Method: 提出了一种新颖的两级阻塞更新框架，包含两个关键组件：在线gossip策略和误差补偿方案。该框架通过更好的共识机制协调学习器间的协作，同时考虑了完全信息反馈和bandit反馈两种场景，在bandit场景中结合经典梯度估计器。

Result: 新算法在凸函数上实现了Õ(ω^{-1/2}ρ^{-1}n√T)的遗憾边界，在强凸函数上实现了Õ(ω^{-1}ρ^{-2}nlnT)的遗憾边界，相比现有结果显著改进。建立了该问题的首个下界，证明结果在ω和T维度上是最优的。在bandit反馈场景中也改进了现有遗憾边界。

Conclusion: 本文提出的两级阻塞更新框架有效解决了分布式在线凸优化中压缩通信的关键挑战，显著改善了遗憾边界对压缩质量和网络规模的依赖关系，并建立了理论下界证明了最优性，为实际大规模分布式学习系统提供了更高效的算法基础。
...
### [241] [Cardinality augmented loss functions](https://arxiv.org/abs/2601.04941)
*Miguel O'Malley*

Main category: cs.LG

TL;DR: 论文提出使用基于数学中"基数"概念的增强损失函数来解决神经网络训练中的类别不平衡问题，通过评估度量空间的"有效多样性"来改善少数类的性能表现。


<details>
  <summary>Details</summary>
Motivation: 类别不平衡是神经网络训练中常见且有害的问题，多数类往往主导训练过程，导致分类器性能偏向多数类结果。需要一种自然的方法来处理训练数据过于同质化的问题。

Method: 引入基于现代数学文献中基数类不变量（如magnitude和spread）的基数增强损失函数。这些不变量通过评估度量空间的"有效多样性"来丰富基数概念。建立了在神经网络训练中应用基数增强损失函数的方法论。

Result: 在人工不平衡数据集和真实世界不平衡材料科学数据集上进行了实验。观察到少数类性能显著提升，整体性能指标也有所改善。

Conclusion: 基数增强损失函数为解决神经网络训练中的类别不平衡问题提供了一种有效方法，能够显著提升少数类性能并改善整体分类效果。
...
### [242] [Precision over Diversity: High-Precision Reward Generalizes to Robust Instruction Following](https://arxiv.org/abs/2601.04954)
...
TL;DR: 研究发现，在指令跟随任务中，高精度奖励比数据多样性更重要。仅使用硬约束训练的模型比混合数据集表现更好，因为LLM法官的低召回率导致严重奖励黑客攻击。提出优先考虑奖励精度的数据中心化策略，在性能提升13.4%的同时减少58%训练时间。


<details>
  <summary>Details</summary>
Motivation: 挑战当前强化学习中普遍认为的"多样化混合可验证硬约束和不可验证软约束对指令跟随任务泛化至关重要"的共识，通过系统实证研究探索奖励精度与约束多样性的实际影响。

Method: 进行系统实证研究，比较仅硬约束训练与混合数据集训练的效果。分析LLM法官在检测错误响应时的低召回率问题，研究注意力机制。提出数据中心的精炼策略，优先考虑奖励精度而非数据多样性。

Result: 仅硬约束训练的模型始终优于混合数据集训练的模型。高精度奖励是有效对齐的主要驱动力，能发展出可迁移的指令跟随元技能。提出的方法在五个基准测试中性能提升13.4%，训练时间减少58%，并保持强泛化能力。

Conclusion: 需要范式转变：从盲目追求数据多样性转向关注高精度奖励。奖励精度比约束多样性对指令跟随任务的泛化能力更为关键，LLM法官的低召回率问题导致混合数据集中的奖励黑客攻击，削弱了多样性带来的好处。
...
### [243] [On the Definition and Detection of Cherry-Picking in Counterfactual Explanations](https://arxiv.org/abs/2601.04977)
*James Hinns,Sofie Goethals,Stephan Van der Veeken,Theodoros Evgeniou,David Martens*

Main category: cs.LG

TL;DR: 该论文研究反事实解释中的选择性偏见问题，探讨解释提供者如何通过精心选择有利的反事实来操纵解释，并分析审计者检测这种操纵的局限性。


<details>
  <summary>Details</summary>
Motivation: 反事实解释被广泛用于说明输入如何改变才能改变模型预测。对于单个实例，可能存在多个有效的反事实解释，这为解释提供者创造了选择性偏见的机会——他们可以挑选有利于自己叙事的解释，突出有利行为而隐瞒揭示问题行为的例子。

Method: 首先形式化定义了反事实解释中的选择性偏见，包括可接受解释空间和效用函数。然后研究了外部审计者在三种访问级别下检测这种操纵的能力：完全程序访问、部分程序访问和仅解释访问。通过理论分析和实证研究，评估了选择性偏见解释与基线解释在标准质量指标上的可区分性。

Result: 研究表明，在实践中检测选择性偏见极其有限。即使拥有完全程序访问权限，选择性偏见的解释仍然难以与非选择性偏见的解释区分开来，因为有效反事实的多样性和解释规范的灵活性提供了足够的自由度来掩盖故意选择。实证表明，这种变异性通常超过选择性偏见对标准反事实质量指标的影响，使得选择性偏见的解释在统计上与基线解释无法区分。

Conclusion: 由于检测选择性偏见的局限性，保障措施应优先考虑可重复性、标准化和程序约束，而不是事后检测。论文为算法开发者、解释提供者和审计者提供了具体建议，强调需要建立更严格的解释生成规范来减少操纵空间。
...
### [244] [On the Hidden Objective Biases of Group-based Reinforcement Learning](https://arxiv.org/abs/2601.05002)
*Aleksandar Fontana,Marco Simoni,Giulio Rossolini,Andrea Saracino,Paolo Mori*

Main category: cs.LG

TL;DR: 本文对GRPO等基于群体的强化学习方法进行理论分析，揭示了其奖励优化与训练目标之间的结构不匹配问题，发现了梯度偏差、奖励缩放不敏感和动量超界等系统性缺陷。


<details>
  <summary>Details</summary>
Motivation: 尽管基于群体的强化学习方法（如GRPO）在大型语言模型后训练中广泛应用且经验上成功，但这些方法在奖励优化和底层训练目标之间存在结构不匹配。作者希望通过理论分析揭示这些方法的根本局限性，为未来方法设计提供原则性指导。

Method: 作者通过统一的代理公式（unified surrogate formulation）分析GRPO风格的方法，从理论角度研究这些方法的训练动态。该方法将不同方法置于同一分析框架下，识别出共有的系统性特征。

Result: 分析揭示了三个关键发现：(1) 非均匀群体权重会在共享前缀标记上引入系统性梯度偏差；(2) 与AdamW优化器的交互使训练动态对奖励缩放基本不敏感；(3) 优化器动量在重复优化步骤中可能将策略更新推至预期的裁剪区域之外。

Conclusion: 这些发现突显了当前基于群体的强化学习方法的根本局限性，为未来设计更合理的公式提供了原则性指导。作者认为需要重新思考这些方法的基础假设，以解决已识别的系统性缺陷。
...
### [245] [HMVI: Unifying Heterogeneous Attributes with Natural Neighbors for Missing Value Inference](https://arxiv.org/abs/2601.05017)
...
Result: 大量实验结果表明，该方法在性能上优于现有技术，并显著增强了下游机器学习任务，为现实世界中存在缺失数据的系统提供了稳健解决方案。

Conclusion: 通过显式建模异构特征间的跨类型依赖关系，该方法为缺失值填补提供了更有效的统一框架，能够显著提升填补质量和下游任务性能。
...
### [246] [Approximate equivariance via projection-based regularisation](https://arxiv.org/abs/2601.05028)
*Torben Berndt,Jan Stühmer*

Main category: cs.LG

TL;DR: 提出一种基于投影的正则化方法来实现近似等变性，通过线性层的正交分解惩罚非等变成分，相比基于样本的方法具有更高的效率和性能。


<details>
  <summary>Details</summary>
Motivation: 等变性是神经网络中强大的归纳偏置，但非等变模型因更好的运行时性能和对现实世界不完美对称性的适应性而重新受到关注。现有近似等变方法通常依赖数据增强的样本正则化器，样本复杂度高，特别是对于连续群如SO(3)。

Method: 提出投影基正则化器，利用线性层到等变和非等变成分的正交分解。在算子级别惩罚非等变性，而不是逐点惩罚。提供在空间域和谱域精确高效计算非等变惩罚的数学框架。

Result: 在实验中，该方法在模型性能和效率方面始终优于先前的近似等变方法，相比基于样本的正则化器实现了显著的运行时增益。

Conclusion: 投影基正则化方法为近似等变性提供了更高效和有效的解决方案，特别是在处理连续对称群时，通过算子级别的惩罚机制实现了更好的性能平衡。
...
### [247] [A Data-Driven Predictive Framework for Inventory Optimization Using Context-Augmented Machine Learning Models](https://arxiv.org/abs/2601.05033)
...
Method: 使用四种机器学习算法：XGBoost、ARIMA、Facebook Prophet和SVR，系统性地纳入工作日、节假日和销售偏差指标等外部因素来预测库存需求。

Result: XGBoost表现最佳，在包含外部变量时达到最低MAE（22.7）。ARIMAX和Facebook Prophet也显示出显著改进，而SVR表现较差。纳入外部因素显著提高了需求预测模型的准确性。

Conclusion: XGBoost是最高效的需求预测算法，结合外部因素能显著提升预测精度。该研究为零售和自动售货机系统的库存管理优化提供了有力框架。
...
### [248] [DeepWeightFlow: Re-Basined Flow Matching for Generating Neural Network Weights](https://arxiv.org/abs/2601.05052)
*Saumya Gupta,Scott Biggs,Moritz Laber,Zohair Shafi,Robin Walters,Ayan Paul*

Main category: cs.LG

TL;DR: DeepWeightFlow是一个基于Flow Matching的生成模型，直接在权重空间生成多样化、高精度的神经网络权重，无需微调即可使用，并能扩展到大型网络。
...
Method: 使用Flow Matching模型直接在权重空间操作，结合Git Re-Basin和TransFusion进行神经网络规范化，以处理神经网络置换对称性并提高大型模型的生成效率。

Result: 生成的神经网络无需微调即可表现良好，能扩展到大型网络，在迁移学习上表现优异，能在几分钟内生成数百个神经网络的集成，远超基于扩散的方法的效率。

Conclusion: DeepWeightFlow为更高效、可扩展地生成多样化神经网络集合开辟了道路。
...
### [249] [Milestones over Outcome: Unlocking Geometric Reasoning with Sub-Goal Verifiable Reward](https://arxiv.org/abs/2601.05073)
*Jianlong Chen,Daocheng Fu,Shengze Xu,Jiawei Chen,Yuan Feng,Yue Yang,Junchi Yan,Hongyuan Zha,Renqiu Xia*

Main category: cs.LG

TL;DR: 提出GeoGoal基准和SGVR框架，通过子目标验证奖励提升MLLMs的几何推理能力，并展示泛化到其他推理任务
...
Method: 1. 构建GeoGoal基准：通过形式化验证数据引擎将抽象证明转换为可验证的数字子目标；2. 提出SGVR框架：用基于骨架率的密集奖励替代稀疏信号

Result: SGVR显著提升几何推理性能(+9.7%)，并展现出强泛化能力：在一般数学任务上提升+8.0%，其他一般推理任务上提升+2.8%

Conclusion: 子目标级别的评估和学习范式能有效提升MLLMs的推理质量，SGVR框架具有广泛的跨领域适用性，为复杂推理任务提供了新的解决方案
...
### [250] [Exploring Student Expectations and Confidence in Learning Analytics](https://arxiv.org/abs/2601.05082)
...
<details>
  <summary>Details</summary>
Motivation: 学习分析在教育系统中广泛应用，但数据收集需符合日益严格的隐私法规要求。需要了解学生对学习分析数据处理的态度和期望，以平衡教育优化与隐私保护。

Method: 使用学生期望学习分析问卷（SELAQ）调查不同院系学生对学习分析数据处理的期望和信心，并应用聚类算法对结果进行分析。

Result: 通过聚类分析识别出四类学生群体：热情者（积极支持）、现实者（理性看待）、谨慎者（担忧隐私）和漠不关心者（态度中立）。

Conclusion: 该结构化分析为理解学生对学习分析的接受度和批评提供了有价值的见解，有助于在隐私合规前提下优化学习分析应用。
...
### [251] [Sequential Subspace Noise Injection Prevents Accuracy Collapse in Certified Unlearning](https://arxiv.org/abs/2601.05134)
*Polina Dolgova,Sebastian U. Stich*

Main category: cs.LG

TL;DR: 提出序列噪声调度方法，将噪声预算分配到参数空间的正交子空间，相比一次性注入所有噪声，显著提升遗忘后模型精度，同时保持相同的差分隐私认证保证。


<details>
  <summary>Details</summary>
Motivation: 基于差分隐私的认证遗忘方法虽然提供强保证，但现有噪声微调方法严重降低模型精度，导致实际应用困难。需要一种既能保持认证保证又能提高实用性的方法。

Method: 提出序列噪声调度方法，将噪声预算分配到参数空间的正交子空间中，而不是一次性注入所有噪声。扩展了噪声微调分析到子空间设置，证明保持相同的(ε,δ)隐私预算。

Result: 在图像分类基准测试中，该方法显著提高了遗忘后的模型精度，同时保持对成员推理攻击的鲁棒性。实现了认证遗忘的严格保证和实际效用的平衡。

Conclusion: 序列噪声调度方法证明认证遗忘可以同时实现严格的理论保证和实际应用价值，为解决认证遗忘的实用性难题提供了有效方案。
...
### [252] [Safe Continual Reinforcement Learning Methods for Nonstationary Environments. Towards a Survey of the State of the Art](https://arxiv.org/abs/2601.05152)
*Timofey Tomashevskiy*

Main category: cs.LG

TL;DR: 本文对持续安全在线强化学习（COSRL）方法进行了最新综述，讨论了理论、挑战和开放性问题，提供了基于安全学习机制的分类，并探讨了构建可靠安全在线学习算法的前景。


<details>
  <summary>Details</summary>
Motivation: 随着强化学习在现实世界应用中的扩展，如何在非平稳环境中实现持续、安全的在线学习成为关键挑战。现有研究缺乏对持续安全在线强化学习方法的系统性综述和分类，需要建立理论框架并识别关键问题。

Method: 采用文献综述方法，首先讨论理论方面和挑战，然后基于安全学习机制的类型（考虑非平稳性适应）对方法进行分类，同时分类在线强化学习算法的安全约束表述，最后探讨可靠安全在线学习算法的前景。

Result: 提供了持续安全在线强化学习方法的全面分类体系，包括基于HM-MDP、NSMDP、POMDP等模型的方法，以及安全约束的不同表述方式。识别了该领域的关键理论挑战和开放性问题。

Conclusion: 持续安全在线强化学习是一个重要且具有挑战性的研究领域，需要进一步发展理论框架和算法。本文的综述为研究人员提供了系统性的分类和未来研究方向，有助于推动构建可靠、安全的在线学习算法。
...
### [253] [FaST: Efficient and Effective Long-Horizon Forecasting for Large-Scale Spatial-Temporal Graphs via Mixture-of-Experts](https://arxiv.org/abs/2601.05174)
*Yiji Zhao,Zihao Zhong,Ao Wang,Haomin Wen,Ming Jin,Yuxuan Liang,Huaiyu Wan,Hao Wu*

Main category: cs.LG

TL;DR: FaST是一个基于异构感知专家混合模型的高效时空图预测框架，能够在大规模网络上进行长时程预测（一周前预测），解决了现有模型计算成本高、内存消耗大的问题。


<details>
  <summary>Details</summary>
Motivation: 现有时空图预测模型主要关注短期预测，在扩展到长时程预测和大规模图时面临计算成本和内存消耗的巨大挑战，需要更高效、可扩展的解决方案。

Method: 提出自适应图代理注意力机制减轻大规模图上的计算负担，并设计新的并行MoE模块，用门控线性单元替代传统前馈网络，实现高效可扩展的并行结构。

Result: 在真实世界数据集上的实验表明，FaST不仅提供卓越的长时程预测准确性，而且在计算效率方面显著优于现有最先进基线模型。

Conclusion: FaST是一个有效且高效的时空图预测框架，成功解决了大规模网络长时程预测的计算挑战，实现了在15分钟粒度下对数千个节点的一周前预测。
...
### [254] [An interpretable data-driven approach to optimizing clinical fall risk assessment](https://arxiv.org/abs/2601.05194)
*Fardin Ganjkhanloo,Emmett Springer,Erik H. Hoyer,Daniel L. Young,Holley Farley,Kimia Ghobadi*

Main category: cs.LG

TL;DR: 通过约束评分优化模型重新调整JHFRAT评分权重，显著提升跌倒风险预测性能，每周可额外保护35名高风险患者
...
Method: 采用约束评分优化（CSO）模型重新加权JHFRAT评分权重，保留其累加结构和临床阈值；基于2022年3月至2023年10月期间54,209例住院患者的回顾性队列分析，包含20,208例高风险和13,941例低风险遭遇

Result: CSO模型显著优于当前JHFRAT（AUC-ROC=0.91 vs 0.86），性能提升相当于每周额外保护35名高风险患者；CSO模型在有/无电子健康记录变量下表现相似，比黑盒XGBoost模型（AUC-ROC=0.94）对风险标签变化更稳健

Conclusion: 这种基于证据的方法为医疗系统提供了坚实基础，可使用数据驱动优化技术系统性地增强住院跌倒预防协议和患者安全，有助于改善医疗环境中的风险评估和资源分配
...
### [255] [EARL: Energy-Aware Optimization of Liquid State Machines for Pervasive AI](https://arxiv.org/abs/2601.05205)
*Zain Iqbal,Lorenzo Valerio*

Main category: cs.LG

TL;DR: EARL：一种能量感知强化学习框架，通过贝叶斯优化与自适应强化学习策略联合优化液态状态机的准确性和能耗，显著提升资源受限设备的AI效率


<details>
  <summary>Details</summary>
Motivation: 随着普及AI对低延迟、高能效的片上学习系统需求增加，液态状态机（LSM）虽在低功耗时序处理方面有潜力，但面临超参数敏感度高、传统优化方法计算成本大且忽略能耗约束的挑战

Method: EARL框架整合贝叶斯优化与自适应强化学习选择策略，采用代理模型进行全局探索，强化学习动态优先选择候选方案，并引入提前终止机制消除冗余评估，大幅降低计算开销

Result: 在三个基准数据集上，EARL相比领先的超参数调优框架实现了6-15%的准确率提升，60-80%的能耗降低，以及高达一个数量级的优化时间减少

Conclusion: 能量感知自适应搜索能有效提升液态状态机在资源受限片上AI应用中的效率和可扩展性，为低功耗时序处理系统提供了实用解决方案
...
### [256] [Robust Reasoning as a Symmetry-Protected Topological Phase](https://arxiv.org/abs/2601.05240)
*Ilmo Sung*
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型存在"幻觉"问题，即由语义噪声引起的逻辑不一致性。当前架构在"度量相"中运行，因果顺序容易发生自发对称性破缺，导致推理脆弱。

Method: 提出将鲁棒推理建模为受对称性保护的拓扑相，其中逻辑操作与非阿贝尔任意子编织形式同构。开发了Holonomic Network，用拓扑不变量替代脆弱的几何插值。

Result: 实验显示明显的拓扑相变：Transformer和RNN表现出无间隙衰减，而Holonomic Network展现出宏观"质量间隙"，在临界噪声阈值下保持不变的保真度。在S10（360万状态）的变量绑定任务中，拓扑模型在训练范围外100倍（L=50→5000）仍保持完美保真度，而Transformer失去逻辑一致性。
...
### [257] [Optimal Lower Bounds for Online Multicalibration](https://arxiv.org/abs/2601.05245)
*Natalie Collina,Jiuyao Lu,Georgy Noarov,Aaron Roth*

Main category: cs.LG

TL;DR: 本文证明了在线多校准的紧下界，建立了与边际校准的信息论分离，在两种不同设置下都得到了Ω(T^{2/3})的下界，匹配了现有上界。


<details>
  <summary>Details</summary>
Motivation: 研究在线多校准问题，旨在理解多校准与边际校准之间的计算复杂性差异，建立信息论上的分离界限。

Method: 使用信息论方法证明下界：1）对于组函数依赖于上下文和预测的情况，使用三个不相交二元组证明Ω(T^{2/3})下界；2）对于组函数仅依赖于上下文的情况，通过正交函数系统构造Θ(T)大小的组族证明Ω(T^{2/3})下界。

Result: 1）在组函数依赖上下文和预测的设置中，证明了Ω(T^{2/3})的期望多校准误差下界，匹配Noarov等人的上界；2）在组函数仅依赖上下文的设置中，证明了Ω(T^{2/3})下界，匹配现有上界；3）建立了多校准与边际校准的分离，后者只有O(T^{2/3-ε})上界。

Conclusion: 在线多校准问题存在信息论上的紧下界Ω(T^{2/3})，显著高于边际校准的上界，表明多校准在计算上比边际校准更困难，这一分离在两种不同组函数依赖设置下都成立。
