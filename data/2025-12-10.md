...
TL;DR: 提出基于深度学习的GIF网络欺凌检测方法，使用VGG16模型在自建数据集上达到97%准确率


<details>
  <summary>Details</summary>
Motivation: 网络欺凌日益严重，社交媒体上通过GIF/贴图进行欺凌的现象缺乏研究，现有研究主要集中在文本和图像检测，GIF检测研究很少

Method: 1) 从Twitter提取网络欺凌相关标签；2) 使用GIPHY API下载GIF文件；3) 构建包含4100个欺凌/非欺凌GIF的数据集；4) 应用预训练VGG16深度学习模型进行检测

Result: 深度学习模型达到97%的准确率，并提供了可用于研究的GIF数据集

Conclusion: 该方法能有效检测GIF中的网络欺凌，为研究者提供了宝贵的数据资源，填补了该领域的研究空白
...
TL;DR: PAVAS：首个将物理推理融入视频到音频生成的方法，通过物理参数估计和适配器实现物理感知的音频合成
...
Method: 提出物理感知视频到音频合成（PAVAS），基于潜在扩散模型，通过物理驱动音频适配器（Phy-Adapter）融入物理推理。物理参数估计器（PPE）使用视觉语言模型估计物体质量，通过分割和3D重建恢复运动轨迹计算速度。

Result: PAVAS生成物理上合理且感知上连贯的音频，在定量和定性评估中均优于现有V2A模型。创建了VGG-Impact基准和音频-物理相关性系数（APCC）评估指标。

Conclusion: 将物理推理融入视频到音频生成能显著提升音频的物理真实性和感知质量，为物理感知的多模态生成开辟了新方向。
...
TL;DR: 利用Planet Labs四波段卫星影像和深度学习模型，可近乎实时监测苏丹武装冲突中的火灾破坏，相比基线方法能更准确捕捉活跃火灾和烧焦区域。
...
Method: 使用Planet Labs的四波段卫星影像，结合深度学习模型，对武装冲突中的火灾破坏进行监测。

Result: 在苏丹五个案例研究中，自动化方法比基线更准确地捕捉了活跃火灾和烧焦区域；使用八波段影像或时间序列影像仅带来边际改进。

Conclusion: 四波段卫星影像结合深度学习模型能够有效、近乎实时地监测武装冲突中的火灾破坏，为冲突监测提供了实用工具。
...
TL;DR: LivingSwap：首个视频参考引导的人脸交换模型，通过关键帧条件注入目标身份，结合视频参考指导进行时序拼接，在长视频序列中实现高保真度和时序一致性。


<details>
  <summary>Details</summary>
Motivation: 影视娱乐制作中需要高质量的视频人脸交换，但现有方法在长复杂视频序列中难以同时保证高保真度和时序一致性。受参考引导图像编辑的启发，探索是否可以利用源视频的丰富视觉属性来提升视频人脸交换的质量。

Method: 1. 使用关键帧作为条件信号注入目标身份，实现灵活可控的编辑；2. 结合关键帧条件和视频参考指导，进行时序拼接确保稳定身份保持和高保真重建；3. 构建Face2Face配对人脸交换数据集，并通过数据对反转确保可靠的监督信号。

Result: 实验表明该方法达到最先进水平，能够无缝整合目标身份与源视频的表情、光照和运动，同时显著减少制作流程中的手动工作量。

Conclusion: LivingSwap是首个视频参考引导的人脸交换模型，通过创新的关键帧条件和时序拼接机制，成功解决了长视频序列中身份保持和时序一致性的挑战，为影视制作提供了高效解决方案。
...
TL;DR: 提出一个将显式解剖层次结构嵌入语义分割的通用框架，通过递归层级预测方案、限制性输出头和自上而下的特征调节，在牙齿全景X光片上验证了层次感知分割能提升性能和解剖一致性


<details>
  <summary>Details</summary>
Motivation: 准确理解解剖结构对于可靠诊断某些牙科疾病至关重要。现有的层次感知分割方法主要通过损失函数编码解剖结构，提供弱且间接的监督，需要更直接的层次结构建模方法

Method: 提出通用框架，通过耦合递归层级预测方案、限制性输出头和自上而下的特征调节来嵌入显式解剖层次。使用特征线性调制将父类概率调节子类特征空间，概率组合规则强制父类与子类一致性，层次损失结合每层Dice损失、交叉熵损失和一致性损失

Result: 在TL-pano数据集（194张全景X光片）上验证，使用UNet和HRNet作为基础模型进行5折交叉验证，层次变体一致提高了IoU、Dice和召回率，特别是对细粒度解剖结构，产生更解剖一致的掩码，但召回率提升超过精确度意味着假阳性增加

Conclusion: 显式层次结构建模提高了性能和临床合理性，特别是在数据量少的牙科影像领域，证明了层次感知分割在解剖结构理解中的价值
...
TL;DR: FRIEDA是一个用于评估大视觉语言模型在复杂开放式地图推理能力的新基准，涵盖拓扑、度量和方向三类空间关系，需要多步推理和跨地图关联，现有模型表现远低于人类水平。


<details>
  <summary>Details</summary>
Motivation: 地图推理是解释地理关系的关键认知能力，对灾害响应和城市规划等任务至关重要，但目前缺乏专门的评估基准。现有地图视觉问答研究往往将地图视为图表的特例，未能充分评估地图特有的分层符号系统和空间关系理解能力。

Method: FRIEDA从各种领域和地理区域的文档报告中收集真实地图图像，基于GIS文献分类，涵盖拓扑关系（边界、相等、相交、包含）、度量关系（距离）和方向关系（方位）三类空间关系。所有问题都需要多步推理，许多需要跨地图关联和推理。评估在两种设置下进行：直接设置（提供相关地图）和上下文设置（模型需要先识别相关地图）。

Result: 评估了11个最先进的大视觉语言模型，即使在直接设置下，最强模型Gemini-2.5-Pro和GPT-5-Think的准确率分别仅为38.20%和37.20%，远低于人类84.87%的表现。结果表明模型在多步地图推理方面存在显著差距。

Conclusion: FRIEDA揭示了当前大视觉语言模型在复杂地图推理能力上的严重不足，为评估和推动空间智能发展提供了一个严谨的基准，有助于促进模型在地理空间理解方面的进步。
...
TL;DR: SSplain是一种用于ROP分类的神经网络解释器，通过强制平滑性和稀疏性约束来生成保留图像结构的像素级解释。


<details>
  <summary>Details</summary>
Motivation: 神经网络在医疗诊断中被广泛使用，但由于其黑盒性质，需要解释器来帮助临床医生理解和信任模型输出。现有方法无法生成保留输入图像结构（如平滑性和稀疏性）的解释。

Method: 提出SSplain方法，通过定义具有组合约束的优化问题，并使用交替方向乘子法（ADMM）求解，生成保留图像平滑性和稀疏性的像素级解释。

Result: 实验结果显示SSplain在事后准确性和平滑性分析方面优于常用解释器，并能识别与临床医生认为的ROP判别特征一致的特征，在公开数据集上展示了良好的泛化能力。

Conclusion: SSplain通过强制平滑性和稀疏性约束，生成了更真实、结构保留的解释，有助于增强对黑盒模型的理解和信任，特别是在医疗诊断领域。
...
Method: 1) 轻量视觉骨干从人体关键点和唇部区域图像捕捉手动和非手动线索；2) 滑动感知映射网络将连续视觉特征聚合为词级嵌入；3) 多任务可扩展训练策略联合优化手语翻译和字幕对齐

Result: 在BOBSL数据集上取得手语翻译和字幕对齐的SOTA结果，在How2Sign上展示零样本泛化和微调性能，实现跨语言可扩展翻译

Conclusion: 通过多语言预训练和强模型设计，实现了统一的手语理解框架，在BSL和ASL上表现优异，展示了跨不同手语的可扩展翻译潜力
...
TL;DR: 该论文提出了一种基于频域掩码的通用深度伪造检测方法，通过频率掩码增强模型对不同生成器的泛化能力，同时保持计算效率，实现可持续的深度伪造检测。
...
Method: 提出频率域掩码作为深度伪造检测器的训练策略，引入随机掩码和几何变换，特别关注频率掩码因其优越的泛化特性。该方法不依赖空间特征或大规模预训练模型。

Result: 在GAN和扩散模型生成的图像数据集上实现了最先进的泛化性能，在显著模型剪枝下仍保持性能，提供了可扩展且资源节约的解决方案。

Conclusion: 频率掩码不仅提高了对不同生成器的检测准确性，还保持了模型剪枝后的性能，展示了基于频率的掩码作为可持续和可泛化深度伪造检测的实用方法潜力。
...
TL;DR: M2A：一种简单的持续测试时自适应方法，通过随机掩码生成多个视图，结合掩码一致性损失和熵最小化损失进行适应，无需复杂的掩码设计或不确定性校准。


<details>
  <summary>Details</summary>
Motivation: 测试时的分布偏移会降低图像分类器性能。现有的持续测试时自适应方法依赖不确定性校准或注意力分数，且设计复杂。本文探讨是否简单的随机掩码方案在强噪声下就足够有效。

Method: 提出M2A方法：生成短序列的随机掩码视图（空间或频率域），使用两个目标函数：1）掩码一致性损失，对齐不同视图的预测；2）熵最小化损失，鼓励置信输出。研究了空间掩码（补丁vs像素）和频率掩码（全频vs低频vs高频）两种类型。

Result: 在CIFAR10C/CIFAR100C/ImageNetC（严重程度5）上，M2A（空间）达到8.3%/19.8%/39.2%的平均错误率，优于或匹配强基线。M2A（频率）表现较差。消融实验显示简单随机掩码有效且鲁棒。

Conclusion: 简单的随机掩码方案结合一致性和熵目标足以驱动有效的测试时自适应，无需依赖不确定性或注意力信号。空间掩码优于频率掩码。
...
TL;DR: 本文评估了多种基于机器学习的亚马逊雨林砍伐检测模型，通过统一数据集比较了全卷积模型和Transformer架构，并展示了预处理、后处理及模型融合策略能显著提升检测效果，最终达到80.41%的F1分数。


<details>
  <summary>Details</summary>
Motivation: 亚马逊雨林保护是全球气候、生物多样性和原住民文化保护的重点。虽然已有基于PRODES卫星数据的机器学习模型用于砍伐检测，但现有方法存在效果不佳、未充分利用现代架构（如自注意力机制）、缺乏标准化比较等问题。

Method: 在统一数据集上评估多种变化检测模型，包括全卷积模型和基于Transformer的自注意力机制网络。研究了不同预处理和后处理技术的影响，如基于连通分量大小的预测区域过滤、纹理替换和图像增强。测试了多种模型融合策略。

Result: 预处理和后处理技术能显著提升单个模型效果。通过模型融合策略获得了优于单个模型的结果，达到了80.41%的F1分数，与近期文献中的结果相当。

Conclusion: 本研究填补了亚马逊雨林砍伐检测领域的空白，通过系统评估现代架构和标准化方法，证明了预处理、后处理和模型融合策略的有效性，为未来的研究和实际应用提供了重要参考。
...
TL;DR: CVP框架通过模拟人类中央和周边视觉，提出目标亲和力token和全局网格表示，显著提升多模态模型的空间推理能力
...
Method: 提出中央-周边视觉启发框架(CVP)，包含两个互补组件：1)目标亲和力token（模拟中央视觉），引导模型关注查询相关对象；2)全局网格（模拟周边视觉），捕捉全局场景上下文和空间布局
...
<details>
  <summary>Details</summary>
Motivation: 现有Transformer方法虽然能捕捉全局上下文，但二次计算复杂度阻碍实时部署；真实世界非均匀雾霾条件下去雾仍然极具挑战。

Method: 基于多状态感知范式：1) 空间感知：可变形四方向令牌移位动态调整感受野；2) 频域感知：Fourier Mix块将RWKV的WKV注意力扩展到傅里叶域；3) 语义关系感知：语义桥接模块通过动态语义核融合对齐编码器-解码器特征。

Result: 在多个基准测试中达到最先进性能，显著降低计算开销，在恢复质量和实际效率之间建立良好平衡。

Conclusion: Fourier-RWKV通过线性复杂度的多状态感知框架，为真实世界非均匀雾霾条件下的高效图像去雾提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 单目深度估计是自动驾驶和机器人等实际应用的基础能力。尽管深度神经网络在物理基准测试中已达到超人类准确性，但关键挑战仍然存在：如何将模型表征与人类感知对齐，这是增强模型鲁棒性和可解释性的有前景策略。物体识别领域的研究揭示了模型准确性与人类行为之间的复杂权衡关系，这引发了一个问题：在深度估计领域是否存在类似的差异，特别是在自然室外场景中，这些场景的基准测试依赖于传感器而非人类感知估计。

Method: 研究使用KITTI数据集，系统分析了69个单目深度估计模型。为了逐因素分解误差模式结构，应用仿射拟合将预测误差分解为可解释的组成部分。通过这种方法深入探讨模型准确性与人类相似性之间的关系。

Result: 研究结果显示，虽然人类和深度神经网络共享某些估计偏差（正误差相关性），但观察到模型准确性与人类相似性之间存在明显的权衡关系。这一发现表明，提高准确性并不必然导致更接近人类的行为。

Conclusion: 研究强调了超越传统准确性评估的必要性，需要开发多方面的、以人类为中心的评估方法。模型准确性与人类相似性之间的权衡关系表明，追求更高准确性可能不会自动产生更符合人类感知的深度估计模型。
...
<details>
  <summary>Details</summary>
Motivation: 高质量几何图生成需要严格的空间准确性，同时提供明确的约束条件来指导生成过程。现有方法在结构保真度方面存在不足，需要更可解释和可扩展的解决方案。

Method: GeoLoom包含两个核心组件：1) 自动形式化模块，将自然语言翻译为专门设计的生成导向形式语言GeoLinua；2) 坐标求解器，使用高效的蒙特卡洛优化将形式约束映射到精确坐标。还提出了基于约束的评估指标来量化结构偏差。

Result: GeoLoom在结构保真度方面显著优于现有最先进的基线方法，为可解释和可扩展的图表生成提供了原则性基础。

Conclusion: GeoLoom通过结合形式化语言和符号求解器，实现了高精度、可解释的几何图生成，为解决几何图生成中的空间准确性问题提供了有效框架。
...
TL;DR: 提出用于MCU设备的动物重识别框架，通过优化MobileNetV2架构和高效微调策略，在保持高精度的同时将模型缩小两个数量级。


<details>
  <summary>Details</summary>
Motivation: 野外环境中的动物重识别需要在有限无线连接的边缘设备上运行，但现有模型大多为工作站设计，内存占用大且需要高分辨率输入，不适用于MCU级别的硬件。

Method: 1) 分析现有模型与MCU硬件的差距；2) 基于MobileNetV2设计针对低分辨率输入的CNN架构；3) 提出数据高效微调策略，仅需每个动物3张图像即可适应新场景。

Result: 在六个公开数据集上，紧凑模型在保持竞争力的检索精度同时，模型大小减少两个数量级；在自收集的牛群数据集上，设备端推理仅小幅降低精度，Top-1准确率保持不变。

Conclusion: 证明了在MCU级别设备上实现实用、可适应的动物重识别是可行的，为在真实野外环境中的可扩展部署铺平了道路。
...
TL;DR: Blur2Sharp：一个结合3D感知神经渲染和扩散模型的框架，从单张参考视图生成几何一致、清晰的多视角图像，解决现有方法在视角变化和复杂运动下模糊或不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 当前方法在生成逼真人像时面临两难：要么产生几何不一致的多视角图像，要么牺牲真实感导致模糊输出。特别是在不同视角和复杂运动下，现有方法难以同时保持几何一致性和高真实感。

Method: 采用双条件架构：1）Human NeRF模型生成几何一致的多视角渲染作为3D结构指导；2）扩散模型基于这些渲染进行细化，保留细节和结构保真度。还通过层次特征融合整合SMPL模型的纹理、法线和语义先验，提升全局一致性和局部细节准确性。

Result: 在多种实验场景下，Blur2Sharp在生成新姿态和新视角图像任务中均优于现有技术，特别是在处理宽松衣物和遮挡等挑战性场景时表现突出。

Conclusion: Blur2Sharp成功解决了从单张参考视图生成几何一致、清晰多视角图像的难题，通过结合3D感知神经渲染和扩散模型，在保持结构一致性的同时实现了高真实感输出。
...
<details>
  <summary>Details</summary>
Motivation: 当前对象理解任务的数据通常是任务导向且缺乏系统组织，无法支持全面的对象类别理解，需要结构化多模态知识来支持高级视觉任务

Method: 提出VisKnow框架，结合专家设计和大型模型应用，从百科全书文档中提取文本知识三元组，整合图像和区域标注，构建结构化多模态对象知识图谱

Result: 构建了AnimalKB知识库，涵盖406个动物类别，包含22K文本知识三元组、420K图像及区域标注，实验显示能提升零样本识别、细粒度VQA等任务性能

Conclusion: 自动构建视觉知识库具有推进视觉理解及其实际应用的潜力，AnimalKB可作为知识图谱补全和部件分割的挑战性基准
...
TL;DR: 该论文探索了在3D目标检测中应用提示调优方法的有效性，提出了面向场景的提示池(SOP²)，验证了提示池在3D检测中的效果。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型展现强大泛化能力，通过微调和提示调优等技术可以适应各种下游任务。本文旨在探索在3D目标检测领域中常见提示调优方法的有效性，研究在Waymo数据集上训练的模型能否作为基础模型适应其他3D检测场景。

Method: 首先系统研究了提示标记和提示生成器的影响，然后提出了面向场景的提示池(SOP²)。该方法探索了提示池在3D目标检测中的应用潜力。

Result: 论文证明了提示池在3D目标检测中的有效性，展示了基于大规模Waymo数据集训练的模型可以通过提示调优适应其他3D检测场景。

Conclusion: 本文成功将提示调优方法引入3D目标检测领域，提出的SOP²方法有效，旨在启发未来研究者深入探索提示在3D领域的潜力。
...
TL;DR: 该论文研究在MPEG-AI FCM标准下使用VVC压缩神经网络中间特征，提出了三种轻量级VVC配置，在保持压缩效率的同时显著减少编码时间。


<details>
  <summary>Details</summary>
Motivation: 传统视频编码器针对人类视觉系统优化，但在分割推理系统中传输的是神经网络中间特征而非像素数据，这些特征具有抽象、稀疏、任务特定的特点，使得感知保真度不再相关。需要研究如何高效压缩这些特征。

Method: 在MPEG-AI特征编码标准框架下，对VVC进行工具级分析，了解各编码组件对压缩效率和下游视觉任务准确性的影响。基于这些分析，提出了三种轻量级VVC配置：Fast、Faster和Fastest。

Result: Fast配置获得2.96%的BD-Rate增益，同时减少21.8%的编码时间；Faster配置获得1.85%的BD-Rate增益，速度提升51.5%；Fastest配置减少95.6%的编码时间，仅损失1.71%的BD-Rate。

Conclusion: 研究表明VVC可以有效地压缩神经网络中间特征，提出的轻量级配置在压缩效率和编码速度之间取得了良好平衡，为分割推理系统中的特征传输提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试强调生成但忽视验证，无法评估思维链推理是否真正基于视觉证据且逻辑连贯。需要填补这一空白，以诊断多模态模型的视觉基础和逻辑连贯性。

Method: 设计MM-CoT基准测试，要求模型从多个选项中选择唯一满足两个正交约束的事件链：(1)视觉一致性：所有步骤都基于可观察证据；(2)逻辑连贯性：具有因果和常识有效性。同时设计对抗性干扰项来暴露不同的推理失败。
...
TL;DR: 提出一种基于法线引导的稀疏深度采样策略，通过PCA法线估计计算像素深度可靠性，改进深度补全模型的训练数据生成，使其更接近真实传感器行为。


<details>
  <summary>Details</summary>
Motivation: 当前深度补全方法使用的稀疏深度数据通常是从密集真值中均匀随机采样得到的，忽略了真实传感器深度测量的几何依赖性和空间非均匀可靠性，导致训练条件不真实。

Method: 提出法线引导的稀疏深度采样策略：1) 在RGB-D点云上使用PCA进行表面法线估计；2) 基于法线计算每个像素的深度可靠性度量；3) 根据可靠性分布采样稀疏深度点。该方法与Marigold-DC扩散模型集成。

Result: 在NYU Depth v2数据集上的实验表明，几何感知的稀疏深度采样提高了深度补全的准确性，减少了边缘和不连续区域的伪影，并产生了更接近真实传感器行为的训练条件。

Conclusion: 通过考虑几何依赖的深度可靠性进行稀疏深度采样，能够显著改善深度补全模型的性能，使其更符合真实传感器特性，为工业机器人系统提供更准确的三维感知。
...
TL;DR: FastBEV++：一个无需定制CUDA内核、完全TensorRT原生可移植的BEV感知框架，在nuScenes基准测试中达到0.359 NDS，在Tesla T4上超过134 FPS


<details>
  <summary>Details</summary>
Motivation: 当前基于相机的鸟瞰图感知面临一个根本矛盾：最先进的性能与车载部署可行性之间的冲突。这一瓶颈源于对计算密集型视图变换和平台特定内核的深度依赖。

Method: 通过"算法快速"和"设计可部署"两大原则：1）采用新的视图变换范式，将整体投影分解为标准索引-收集-重塑流水线，使用确定性预排序策略，完全基于基本算子原语；2）集成端到端深度感知融合机制，结合时间聚合和鲁棒数据增强。

Result: 在nuScenes基准测试中达到新的SOTA性能0.359 NDS，同时在Tesla T4等汽车级硬件上保持超过134 FPS的实时性能，无需任何定制插件。

Conclusion: FastBEV++通过消除对专用CUDA内核的依赖，同时保持高精度，为生产级自动驾驶系统提供了一个成熟且可扩展的设计理念，实现了高性能与部署效率的统一。
...
TL;DR: HTC-VLM提出混合双通道框架，通过连续通道保留细粒度细节，离散通道保留高级语义，实现580:1压缩比，在多个基准上保持87.2%性能


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型面临效率-保真度困境：连续压缩会稀释高级语义（如物体识别），离散量化会丢失细粒度细节（如纹理）。需要解决二次计算成本和内存压力问题。

Method: 采用双通道混合框架：1）连续通道通过ViT补丁保留细粒度细节；2）离散通道使用MGVQ量化生成4个符号锚点。通过解耦注意力掩码和瓶颈机制融合为580个token的混合序列，再压缩为单个voco token。

Result: 在7个基准测试（GQA、VQAv2、MMBench、MME、POPE、SEED-Bench、ScienceQA-Image）上平均性能保持87.2%，优于领先连续基线的81.0%，实现580:1压缩比。注意力分析显示压缩token优先关注离散锚点。

Conclusion: 极简混合设计能解决效率-保真度困境，推动可扩展视觉语言模型发展。离散锚点提供语义指导，验证了混合表示的有效性。
...
TL;DR: 提出Residual-SwinCA-Net混合分割框架，结合残差CNN和Swin Transformer，在BUSI数据集上实现99.29%准确率、98.74% IoU和0.9041 Dice系数
...
Method: 1) 残差CNN模块提取局部相关特征；2) 定制Swin Transformer块学习全局依赖；3) 拉普拉斯-高斯区域算子增强组织连续性；4) 边界导向算子保持病变轮廓完整性；5) 阶段式收缩策略捕获尺度不变性；6) 多尺度通道注意力挤压模块(MSCAS)选择性强调特征；7) 像素注意力模块自适应加权病变像素

Result: 在BUSI数据集上，Residual-SwinCA-Net达到99.29%平均准确率、98.74% IoU和0.9041 Dice系数，优于现有CNN/ViT技术

Conclusion: 提出的混合框架显著提升乳腺病变分割性能，增强临床诊断准确性和及时决策能力
...
TL;DR: FTKD是一种稀疏查询的未来时序知识蒸馏方法，将离线教师模型的未来帧知识转移到在线学生模型中，提升3D目标检测性能而不增加推理成本。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法主要关注空间特征蒸馏或时序关系蒸馏，忽略了未来帧知识，导致在线模型难以有效学习未来信息。需要一种方法将离线模型（可使用未来帧）的知识转移到在线模型（只能使用当前及过去帧）。

Method: 提出FTKD方法：1) 未来感知特征重建策略，让学生模型在不严格帧对齐的情况下捕获未来特征；2) 未来引导的logit蒸馏，利用教师模型稳定的前景和背景上下文信息。

Result: 在nuScenes数据集上，应用于两个高性能3D目标检测基线模型，获得了最高1.3 mAP和1.3 NDS的提升，同时实现了最准确的运动速度估计，且不增加推理成本。

Conclusion: FTKD通过有效转移未来帧知识，显著提升了在线3D目标检测模型的性能，为自动驾驶中的实时感知系统提供了有效的知识蒸馏框架。
...
TL;DR: 提出QHP方法，通过查询感知的枢纽原型学习解决少样本3D点云分割中的原型偏差问题，利用二分图建模支持集与查询集语义关联，并通过原型分布优化提升分割性能。
...
Method: 提出查询感知枢纽原型（QHP）学习方法：1）枢纽原型生成（HPG）模块：构建连接查询点和支持点的二分图，识别频繁链接的支持枢纽，生成与查询相关的原型；2）原型分布优化（PDO）模块：使用纯度重加权对比损失，将坏枢纽和边界模糊原型拉近到对应类中心，优化原型表示。

Result: 在S3DIS和ScanNet数据集上的大量实验表明，QHP相比现有最先进方法取得了显著性能提升，有效缩小了原型与查询集之间的语义差距。

Conclusion: QHP方法通过显式建模支持集与查询集之间的语义关联，生成查询感知的原型表示，有效解决了少样本3D点云分割中的原型偏差问题，提升了分割性能。
...
TL;DR: 提出SFP方法，结合空间和频率先验进行场景恢复，无需复杂网络或合成数据训练，通过加权融合策略处理多种真实世界退化


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖单一先验无法处理多种退化，或使用合成数据训练的复杂网络在真实场景中泛化能力差，需要更有效的真实世界场景恢复方法

Method: 1) 空间域：利用退化图像逆的谱方向投影估计透射图恢复散射退化；2) 频率域：构建自适应频率增强掩码，基于两个新先验估计参数：DC分量均值近似和低频幅度占比约1%；3) 加权融合空间域恢复、频率域增强和输入图像显著特征
...
TL;DR: RLCNet是一个用于LiDAR、RADAR和相机多模态传感器在线标定的端到端深度学习框架，具有实时操作能力和对动态环境的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆中多传感器（LiDAR、RADAR、相机）的外参标定至关重要，但由于机械振动和动态环境中的传感器漂移等因素，准确标定仍然具有挑战性。
...
Conclusion: RLCNet为多模态传感器在线标定提供了实用部署解决方案，通过深度学习框架和在线校准机制实现了对动态环境的适应性和实时操作能力。
...
<details>
  <summary>Details</summary>
Motivation: 将第三人称（外中心）视频转换为第一人称（自我中心）视频可以创造沉浸式体验，但由于相机姿态变化大、视角重叠少，这项任务极具挑战性，需要保持可见内容的同时合成几何一致的未观察区域。

Method: EgoX框架利用大规模视频扩散模型的预训练时空知识，通过轻量级LoRA适配；采用宽度和通道级拼接的统一条件策略结合外中心和自我中心先验；引入几何引导的自注意力机制选择性关注空间相关区域。

Result: 该方法实现了连贯且真实的自我中心视频生成，在未见过的和野外视频上表现出强大的可扩展性和鲁棒性。

Conclusion: EgoX通过创新的条件策略和几何引导机制，成功解决了外中心到自我中心视频转换的挑战，为沉浸式理解开辟了新可能性。
...
TL;DR: OpenSubject：一个用于主体驱动图像生成与编辑的大规模视频数据集，包含250万样本和435万图像，通过四阶段流水线构建，显著提升复杂场景下的生成性能


<details>
  <summary>Details</summary>
Motivation: 当前主体驱动图像生成模型存在两个主要问题：1）容易偏离参考主体身份；2）在包含多个主体的复杂场景中表现不佳。需要高质量、大规模的训练数据来解决这些问题。

Method: 提出四阶段流水线构建OpenSubject数据集：1）视频筛选（分辨率与美学过滤）；2）跨帧主体挖掘与配对（基于VLM的类别共识、局部定位和多样性感知配对）；3）身份保持参考图像合成（分割图引导的外绘和框引导的内绘，配合几何感知增强和边界侵蚀）；4）验证与标注（VLM验证样本，重新合成失败样本，构建长短描述）

Result: 构建了包含250万样本、435万图像的OpenSubject数据集，并建立了相应的基准测试。实验表明，使用OpenSubject训练能显著提升主体驱动生成和编辑的性能，特别是在复杂场景中。

Conclusion: OpenSubject数据集通过精心设计的四阶段流水线构建，解决了主体驱动图像生成中的身份保持和复杂场景处理问题，为相关研究提供了高质量的训练数据和评估基准。
...
TL;DR: Terrain Diffusion：一种AI时代的程序化地形生成方法，结合扩散模型的高保真度与传统噪声函数的无限扩展、种子一致性和实时访问特性，能够无缝生成整个行星尺度的连贯地形。


<details>
  <summary>Details</summary>
Motivation: 传统程序化噪声函数（如Perlin噪声）虽然快速且无限，但在真实感和大尺度连贯性方面存在根本限制。需要一种能够结合扩散模型高保真度优势，同时保留程序化噪声关键特性的新方法。

Method: 提出Terrain Diffusion系统，核心是InfiniteDiffusion算法实现无限生成；采用分层扩散模型堆栈耦合行星上下文与局部细节；使用紧凑拉普拉斯编码稳定地球尺度动态范围的输出；开发开源无限张量框架支持无界张量的恒定内存操作；通过少步一致性蒸馏实现高效生成。

Result: 建立了扩散模型作为程序化世界生成的实用基础，能够以可控方式合成整个行星尺度的连贯地形，实现无缝、实时、无限制的景观生成。

Conclusion: Terrain Diffusion成功将扩散模型的高保真度与程序化噪声的关键特性相结合，为AI时代的程序化世界生成提供了新的基础框架，能够生成无限、连贯且可控的整个行星地形。
...
Method: 提出GeoDM框架，在欧几里得、双曲和球面流形的笛卡尔积空间中操作，通过可学习的曲率和权重参数适应底层数据几何结构，设计最优传输损失增强分布保真度。

Result: 理论分析表明，在乘积空间中进行几何感知的分布匹配比欧几里得对应方法具有更小的泛化误差界。在标准基准测试上的大量实验证明，该算法优于最先进的数据蒸馏方法，并且在各种单几何分布匹配策略中保持有效性。

Conclusion: GeoDM通过在多几何空间中操作，成功捕获了数据的固有几何结构，为数据集蒸馏提供了更有效的解决方案，能够更好地保持原始数据的分布特性。
...
<details>
  <summary>Details</summary>
Motivation: 牙齿标志点检测是现代临床正畸学的关键任务，对诊断、个性化治疗和治疗进展监测至关重要。然而，由于牙齿几何结构复杂且个体差异大，现有技术面临挑战，需要开发先进的深度学习方法来精确可靠地检测3D牙齿标志点。

Method: 通过组织3DTeethLand挑战赛，与MICCAI 2024会议合作，征集专注于从口内3D扫描中检测牙齿标志点的算法。挑战赛提供了首个公开可用的3D牙齿标志点检测数据集，用于评估最先进的方法。

Result: 建立了首个公开的3D牙齿标志点检测数据集，为社区提供了评估现有方法和推动方法学发展的宝贵资源。

Conclusion: 3DTeethLand挑战赛通过提供公开数据集和促进算法开发，为解决具有重要临床意义的牙齿标志点检测问题做出了贡献，推动了该领域的研究进展。
...
<details>
  <summary>Details</summary>
Motivation: 现有欧拉方法通过解耦表示学习（纹理、形状、频率方案）处理放大引起的噪声，但在运动位移非常小时难以将光子噪声与真实微运动分离
...
Conclusion: 提出的基于扩散的拉格朗日框架通过光流几何线索实现了结构一致的运动放大，有效解决了光子噪声分离问题
...
TL;DR: 提出了一种用于彩色图像分类的低秩支持四元数矩阵机（LSQMM），将RGB通道作为纯四元数处理，通过四元数代数保持通道间的内在耦合关系，并引入四元数核范数正则化来促进低秩结构。
...
Method: 提出低秩支持四元数矩阵机（LSQMM），将RGB通道作为纯四元数处理，在铰链损失中添加四元数核范数正则化项来促进低秩结构（源于强相关的颜色通道）。设计了基于交替方向乘子法（ADMM）的迭代算法来有效求解该四元数优化模型。

Result: 在多个彩色图像分类数据集上的实验结果表明，与使用支持向量机、支持矩阵机和支持张量机的最先进方法相比，所提出的分类方法在分类精度、鲁棒性和计算效率方面表现出优势。
...
TL;DR: 本文系统分析了Glaze和Nightshade等图像保护机制，通过可解释AI方法揭示了这些对抗性扰动在特征空间、空间域和频域的结构化特性，表明它们通过结构化特征变形而非语义错位实现保护。


<details>
  <summary>Details</summary>
Motivation: 尽管Glaze和Nightshade等图像保护机制在经验上有效，但其内部结构、可检测性和表征行为仍不明确。需要系统分析这些对抗性扰动的本质，以理解其工作机制。

Method: 采用统一框架结合白盒特征空间检查和黑盒信号级探测。具体方法包括：潜在空间聚类、特征通道激活分析、基于遮挡的空间敏感性映射、频域特征分析。

Result: 保护机制是结构化、低熵的扰动，紧密耦合于图像内容；保护图像保留内容驱动的特征组织，具有保护特异性子结构；可检测性受扰动熵、空间部署和频率对齐相互作用影响；Glaze和Nightshade沿主导图像对齐频率轴重新分配能量而非引入扩散噪声。

Conclusion: 当代图像保护通过结构化特征级变形而非语义错位运作，这解释了保护信号视觉上微妙但一致可检测的原因。研究推进了对抗性图像保护的可解释性，为未来生成AI系统的防御和检测策略设计提供信息。
...
TL;DR: 该研究针对多模态大语言模型在低资源语言中表现不佳的问题，提出了MELLA数据集，通过双源策略（原生网络替代文本获取文化知识+MLLM生成字幕提升语言能力）来增强模型的语言能力和文化基础，实验证明该方法能显著提升八种语言的性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在高资源语言中表现优异，但在低资源语言中效果显著下降。现有的多语言增强方法通常局限于文本模态或仅依赖机器翻译，这些方法虽然能帮助模型获得基本语言能力并产生"薄描述"，但忽视了多模态信息性和文化基础的重要性，而这两者对有效服务低资源语言用户至关重要。

Method: 提出双源策略指导针对每个目标的数据收集：1) 从原生网络替代文本获取文化知识，2) 通过MLLM生成字幕提升语言能力。具体实现为MELLA多模态多语言数据集，用于微调MLLM模型。

Result: 在MELLA上微调后，八种语言在各种MLLM骨干网络上普遍表现出性能提升，模型能够产生"厚描述"。验证表明性能提升来自文化知识增强和语言能力增强两方面。

Conclusion: 该研究填补了MLLM在低资源语言应用中的空白，通过强调文化意识和语言能力的双重目标，提出的MELLA数据集能有效提升模型在低资源语言环境中的表现，为服务低资源语言用户提供了更全面的解决方案。
...
TL;DR: PointDico是一个新颖的3D表示学习模型，通过知识蒸馏将扩散模型和对比学习的优点结合起来，在3D点云表示学习中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督表示学习方法在处理3D数据时面临困难，因为3D数据具有无序性和不均匀密度的特点。对比模型容易过拟合，而3D掩码自编码器难以处理无序点云。这促使研究者探索结合扩散模型和对比模型优点的3D表示学习方法。
...
Conclusion: PointDico成功地将扩散模型和对比学习的优点结合起来，通过知识蒸馏实现了卓越的3D表示学习性能，为解决3D数据表示学习中的挑战提供了有效解决方案。
...
Method: 设计轻量级模块生成软硬掩码，软掩码初步调制输入特征，硬掩码将不同类型区域引导到不同处理分支：冗余特征进入紧凑分支进行低成本全局处理，异质特征进入聚焦分支进行细粒度建模

Result: 在多个基准数据集上实现SOTA性能，同时显著降低训练时间和参数数量，在自适应卷积模型中计算成本最低

Conclusion: Bi^2MAC能够有效利用不同类型区域信息并智能分配计算资源，在遥感图像融合任务中取得了计算效率和性能的良好平衡
...
Method: 提出混合溅射机制，包括反射烘焙高斯追踪（将视点相关反射烘焙到高斯基元内）、基于瓦片的高斯溅射渲染反射，以及反射敏感高斯剪枝和流水线级加速

Result: 在Ref-NeRF和NeRF-Casting数据集上，相比类似基于光线追踪的高斯溅射基线，实现约7倍渲染加速，高斯基元减少4倍，同时保持反射渲染质量

Conclusion: HybridSplat为复杂反射场景提供了一种新的最先进方法，在渲染速度、内存效率和反射保真度方面取得显著改进
...
TL;DR: DINO-BOLDNet：首个从T1加权图像生成平均BOLD图像的框架，使用DINOv3引导的多切片注意力网络，在临床数据集上超越条件GAN基线
...
Method: 提出DINO-BOLDNet框架：集成冻结的自监督DINOv3编码器和轻量可训练解码器，使用DINOv3提取切片内结构表示，通过切片注意力模块融合相邻切片上下文信息，多尺度生成解码器恢复精细功能对比度，采用基于DINO的感知损失确保结构纹理一致性
...
TL;DR: 提出TrackingWorld方法，通过跟踪上采样器将稀疏2D轨迹提升为密集2D轨迹，再通过优化框架将密集2D轨迹反投影到世界中心3D坐标系，实现单目视频中几乎所有像素的密集3D跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D跟踪方法存在两个主要局限：1) 难以将相机运动与前景动态运动分离；2) 无法密集跟踪视频中新出现的动态主体。需要一种能在世界中心3D坐标系中跟踪几乎所有像素的方法。

Method: 提出三阶段方法：1) 跟踪上采样器将任意稀疏2D轨迹高效提升为密集2D轨迹；2) 将上采样器应用于所有帧，通过消除重叠区域轨迹来减少2D轨迹冗余；3) 基于优化的框架将密集2D轨迹反投影到世界中心3D轨迹，同时估计相机姿态和3D坐标。

Result: 在合成和真实世界数据集上的广泛评估表明，该系统在世界中心坐标系中实现了准确且密集的3D跟踪。

Conclusion: TrackingWorld解决了现有单目3D跟踪方法的局限性，能够分离相机运动与动态运动，并跟踪新出现的动态主体，实现了世界中心坐标系中的密集3D跟踪。
...
Conclusion: SCU-CGAN能有效生成高质量火图像，解决火灾数据集不足问题，显著提升火灾检测模型性能，为家庭火灾检测系统提供有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型(MLLMs)普遍采用Pre-Norm架构，但存在视觉token高norm和文本token低norm的严重差异。这种norm不平衡不是静态问题，而是导致"不对称更新动态"：高norm的视觉token表现出"表征惯性"，语义转换速度远慢于文本token，从而损害跨模态特征融合效果。

Method: 首先通过理论分析证明norm差异导致的不对称更新动态问题，然后在主流MLLMs上进行实证验证。基于这一洞察，提出一个简单有效的解决方案：在视觉投影器后插入一个精心初始化的LayerNorm层，强制实现norm对齐。

Result: 在LLaVA-1.5架构上的实验表明，这种干预不仅在一系列多模态基准测试中带来显著性能提升，而且在纯文本评估(如MMLU)上也表现出改进，表明解决架构不平衡能产生更全面的模型能力。

Conclusion: MLLMs中视觉和文本token的norm差异是一个关键但被忽视的问题，会导致不对称更新动态。通过在视觉投影器后添加LayerNorm层来强制norm对齐，是一个简单而有效的解决方案，能显著提升模型在多模态和纯文本任务上的性能。
...
Method: 1) 使用梯度域加权引导滤波(GDWGIF)准确估计光照并改善图像质量；2) 应用Retinex模型将图像分解为光照层和反射层；3) 并行处理这两层：校正光照层以优化光照条件，增强反射层以改善图像质量；4) 通过多曝光融合和线性拉伸策略优化图像动态范围

Result: 在真实世界数据集上的实验结果表明，该方法在对比度增强和噪声抑制方面均优于现有最先进方法

Conclusion: 提出的框架能够有效处理复杂光照条件下的图像退化问题，同时实现图像增强和噪声抑制，为视觉应用提供更高质量的图像输入
...
TL;DR: 该研究提出利用面部美化评估来增强面部修图检测，在未知攻击算法情况下实现了1.1%的D-EER检测性能


<details>
  <summary>Details</summary>
Motivation: 面部修图在社交媒体和广告中广泛使用，但当修图图像作为生物特征样本使用时，会对人脸识别系统构成挑战。由于先前研究已证明面部修图对人脸识别系统构成威胁，因此检测面部修图变得越来越必要。

Method: 研究分析修图图像在美容评估算法中的变化，评估基于人工智能的不同特征提取方法以改进修图检测，并评估是否可以利用面部美容特征来提升检测率。

Result: 在攻击修图算法未知的场景下，该工作在单图像检测上实现了1.1%的D-EER（等错误率）。

Conclusion: 面部美容特征可以有效用于增强面部修图检测，在未知攻击算法情况下仍能实现高性能检测。
...
TL;DR: 提出优化的深度学习流水线用于鱼类重识别，在模拟电子监控系统的数据集上，使用硬三元组挖掘和定制图像变换，Swin-T架构优于ResNet-50，主要挑战在于区分同物种的相似个体。
...
Method: 开发优化的深度学习流水线，使用硬三元组挖掘结合定制图像变换（包括数据集特定归一化），在AutoFish数据集上比较Vision Transformer的Swin-T架构和CNN的ResNet-50。

Result: Swin-T架构优于ResNet-50，达到41.65% mAP@k和90.43% Rank-1准确率，主要挑战是区分同物种的相似个体，视角不一致比部分遮挡更具破坏性。

Conclusion: 优化的深度学习流水线能有效自动化鱼类重识别，支持渔业数据收集和可持续管理，Vision Transformer在鱼类重识别任务上表现优于传统CNN。
...
TL;DR: SAM-Body4D：无需训练的框架，通过视频分割和遮挡感知模块实现时间一致且抗遮挡的人体网格恢复


<details>
  <summary>Details</summary>
Motivation: 现有图像级人体网格恢复方法在视频中应用时存在时间不一致性和遮挡性能下降的问题，需要不依赖额外训练就能解决这些挑战的方案

Method: 1) 使用可提示视频分割模型生成身份一致的masklets；2) 通过遮挡感知模块修复缺失区域；3) 用精炼的masklets指导SAM 3D Body生成一致的全身体网格轨迹；4) 基于填充的并行策略实现高效多人推理

Result: 在具有挑战性的野外视频中实现了更好的时间稳定性和鲁棒性，无需任何重新训练
...
Method: 提出OneClip-RAG范式，包含查询引导的视频分块算法（统一分块和跨模态检索），并创建SynLongVideo数据集进行渐进式训练

Result: 在多个MLLMs上验证，显著提升性能（如将InternLV2 8B和Qwen2-VL 7B提升至GPT-4o水平），且效率极高（LLaVA-Video可在2.2分钟内处理1小时视频）

Conclusion: OneClip-RAG是一种有效且高效的长视频理解解决方案，解决了MLLMs的内存限制问题，在保持知识完整性和语义连贯性的同时大幅提升处理效率
...
TL;DR: 提出一种基于深度图的6D姿态估计方法，通过多视角深度图融合为精细点云或稀疏TSDF，采用分层热图机制和密度感知稀疏Transformer，实现高分辨率稀疏处理，在工业装箱场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 工业装箱场景中，由于遮挡、反射和无纹理部件，精确恢复6D姿态仍然是一个严峻挑战。现有方法难以处理密集堆叠环境中的复杂几何细节。

Method: 1) 多视角深度图融合为精细3D点云或稀疏TSDF；2) 分层热图机制生成场景自适应注意力先验；3) 密度感知稀疏Transformer块处理自遮挡和非均匀数据分布；4) 全稀疏框架实现高分辨率体素表示；5) 基于体素投票策略预测6D姿态。

Result: 在IPD和MV-YCB多视角数据集上验证，在重度堆叠的工业和家庭装箱场景中表现出竞争性性能，能够同时预测任意数量目标物体的姿态。

Conclusion: 该方法通过全稀疏处理和高分辨率体素表示，有效解决了工业装箱场景中的6D姿态估计问题，展示了稀疏3D方法在近距离机器人应用中的潜力。
...
Method: 提出分层概念演进预训练范式：1) 通过分层掩码解码器和父子查询嵌入建立腹腔镜概念层次结构，统一不同实体；2) 提出置信度驱动的演进标注，基于分层一致性迭代生成和过滤伪标签，逐步将可靠样本纳入训练，构建了包含114K图像-掩码对的大规模基准LapBench-114K。

Result: 大量实验表明，LapFM显著优于最先进的方法，为通用腹腔镜分割的粒度自适应泛化建立了新标准。

Conclusion: LapFM是一个专门设计用于腹腔镜手术分割的基础模型，通过创新的分层概念演进预训练方法，能够从大规模无标注手术图像中学习，实现了卓越的分割性能和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有快照多光谱成像技术虽然能捕获比传统RGB传感器更丰富的频谱信息，但大多数方法将色彩校正流程分成多个独立阶段，往往在早期就丢弃了多光谱数据，未能充分利用多光谱信息的优势。

Method: 提出一个统一的、基于学习的端到端框架，将整个色彩校正流程集成在单一模型中，同时利用高分辨率RGB传感器和辅助低分辨率多光谱传感器的数据。通过重构两种不同的最先进的图像到图像架构来展示框架的灵活性和通用性。

Result: 通过聚合和重新利用公开可用的频谱数据集，构建了专门的训练和评估数据集。实验表明，该方法相比仅使用RGB和多光谱驱动的基线方法，提高了色彩准确性和稳定性，误差减少了高达50%。

Conclusion: 该统一的端到端学习框架能够有效利用多光谱信息，显著提升色彩校正性能，为消费和移动设备中的紧凑型低成本光谱传感器提供了实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于子集选择的解释方法在分布内（ID）设置下表现良好，但在分布外（OOD）条件下可靠性显著下降，产生冗余、不稳定且对不确定性敏感的解释。需要解决这些缺陷以提升AI在真实世界视觉应用中的透明度和可信度。

Method: 提出一个结合子模子集选择和分层梯度不确定性估计的框架。通过自适应权重扰动估计不确定性，并利用这些估计指导子模优化，确保多样化和信息丰富的子集选择。该方法不需要额外训练或辅助模型。

Result: 实证评估表明，该框架不仅缓解了现有方法在OOD场景下的弱点，还在ID设置中带来改进。相比现有方法，能产生更鲁棒、更稳定的解释。

Conclusion: 研究揭示了当前基于子集方法的局限性，并展示了不确定性驱动优化如何增强归因和对象级可解释性，为更透明、可信的AI在真实世界视觉应用中铺平道路。
...
TL;DR: 提出轻量级SAM+CSRT足球运动员跟踪方法，结合SAM初始化、CSRT跟踪和球衣颜色重识别，在资源受限下实现7.6-7.7 FPS的稳定跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 足球运动员跟踪面临频繁遮挡、相似外观和快速运动等挑战，需要轻量级解决方案来处理拥挤场景中的跟踪问题。

Method: 结合Segment Anything Model (SAM)进行精确初始化，使用CSRT跟踪器进行连续跟踪，并基于HSV直方图的球衣颜色外观模型进行重识别，形成团队感知跟踪系统。

Result: 在足球视频序列上达到7.6-7.7 FPS，内存使用稳定(~1880 MB)；轻度遮挡下100%跟踪成功率，拥挤禁区场景(5+球员)下90%成功率；重度遮挡下50%恢复率；但长期离帧遮挡仅8.66%重获成功率。

Conclusion: SAM+CSRT组合在连续可见性场景表现良好，但需要更强的重识别机制处理长期离帧情况；为资源受限下的足球跟踪系统部署提供了实用指导。
...
TL;DR: ContextDrag是一种基于拖拽的图像编辑新范式，通过利用编辑模型的上下文建模能力，结合参考图像的VAE编码特征，实现更连贯、保真度更高的编辑效果，无需微调或反转。
...
Method: 提出ContextDrag框架：1) 上下文保持令牌注入(CTI)，通过潜在空间反向映射(LRM)算法将无噪声参考特征注入正确位置；2) 位置一致注意力(PCA)，对参考令牌进行位置重编码并应用重叠感知掩码消除无关特征干扰。

Result: 在DragBench-SR和DragBench-DR基准测试上的广泛实验表明，该方法超越了所有现有SOTA方法。

Conclusion: ContextDrag通过有效利用参考图像的上下文信息，实现了更精确的拖拽控制和语义纹理一致性，为基于拖拽的图像编辑提供了新范式。
...
TL;DR: Visionary是一个基于WebGPU的浏览器原生平台，用于实时渲染3D高斯泼溅和网格，支持动态神经处理和生成模型，降低了3DGS方法的部署门槛。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS查看器解决方案分散、笨重或受传统管线限制，导致部署困难，对动态内容和生成模型支持有限。

Method: 基于高效WebGPU渲染器和每帧ONNX推理，引入标准化高斯生成器合约，支持插件式算法生成或更新高斯，并提供three.js插件库。

Result: 在相同3DGS资源下，Visionary通过GPU基元排序实现优于现有Web查看器的渲染效率，已支持MLP-based 3DGS、4DGS、神经化身等多种变体。
...
TL;DR: 提出PCI框架分析扩散模型中概念形成的动态过程，通过概念插入成功率研究概念何时在去噪轨迹中锁定，为图像编辑提供可操作指导


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型评估主要关注最终输出，但生成过程是动态轨迹。理解概念在去噪过程中何时形成和锁定，对模型的可控性、可靠性和可预测性分析至关重要

Method: 提出PCI（Prompt-Conditioned Intervention）框架，无需训练且模型无关。核心是分析概念插入成功率（CIS），即在给定时间步插入的概念在最终图像中保留的概率，以此表征概念形成的时间动态

Result: 应用于多个最先进的文本到图像扩散模型和广泛的概念分类，PCI揭示了不同扩散模型的多样化时间行为，发现轨迹的某些阶段对特定概念更有利，即使在同一概念类型内也存在差异

Conclusion: PCI为文本驱动的图像编辑提供了可操作的见解，指出了无需访问模型内部或训练即可进行最有效干预的时间点，实现了比强基线更好的语义准确性和内容保留平衡
...
TL;DR: 首个用于多相机阵列的在线3D高斯泼溅重建框架，通过多相机融合实现无漂移轨迹估计和高效在线重建，在2分钟内重建数百米场景。


<details>
  <summary>Details</summary>
Motivation: 现有基于单目RGB流的3DGS在线扩展方法由于视野有限，难以实现完整的3D覆盖。多相机阵列能从根本上解决这一限制，但尚无针对多相机阵列的在线3D重建框架。

Method: 1) 分层相机初始化方案：无需标定实现粗略相机间对齐；2) 轻量级多相机束调整：稳定轨迹同时保持实时性能；3) 无冗余高斯采样策略：减少高斯基元数量；4) 频率感知优化调度器：减少优化迭代次数。

Result: 仅使用原始多相机视频流，在2分钟内重建数百米3D场景，展示了前所未有的速度、鲁棒性和保真度，实现了无漂移轨迹估计和高效在线重建。

Conclusion: 该方法首次实现了多相机阵列的在线3D高斯泼溅重建，通过多相机融合解决了单目视野限制问题，在保持实时性能的同时显著提升了重建覆盖范围和质量。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大推理模型（MLRMs）通过分层思维链推理从个人图像中推断精确地理位置，构成严重隐私风险。现有隐私保护技术主要针对感知模型，对MLRMs的复杂多步推理过程无效。

Method: 提出ReasonBreak对抗框架，基于关键洞察：有效破坏地理推理需要与概念层次对齐的扰动而非均匀噪声。策略性地针对推理链中的关键概念依赖关系，生成使特定推理步骤无效并级联影响后续推理阶段的扰动。同时贡献GeoPrivacy-6K数据集，包含6,341张超高分辨率图像和分层概念标注。

Result: 在七个最先进的MLRMs（包括GPT-o3、GPT-5、Gemini 2.5 Pro）上评估显示，ReasonBreak在区域级保护方面提升14.4%（33.8% vs 19.4%），在街区级保护方面几乎翻倍（33.5% vs 16.8%）。

Conclusion: 这项工作建立了针对基于推理的隐私威胁的新保护范式，通过概念感知扰动有效破坏MLRMs的分层推理过程。
...
TL;DR: NoisyCLIP：一种在去噪过程中早期检测文本-图像错位的方法，通过测量噪声潜在空间中的语义对齐，实现实时对齐评估，减少50%计算成本的同时达到98%的CLIP对齐性能。


<details>
  <summary>Details</summary>
Motivation: 条件扩散模型虽然依赖语言-图像对齐方法来引导生成，但错位和幻觉问题仍然常见，需要自动错位检测工具。现有方法需要在完整生成后才能测量对齐度，计算成本高且无法实时评估。

Method: 提出NoisyCLIP方法，在去噪过程中测量噪声潜在空间中的语义对齐，首次探索和基准测试在反向扩散过程中使用双编码器进行提示到潜在空间的错位检测。

Result: NoisyCLIP在Best-of-N设置中减少50%计算成本，同时达到98%的CLIP对齐性能，实现了生成过程中的实时对齐评估。
...
TL;DR: 提出OCCDiff方法，在占用函数空间应用潜在扩散模型，从LiDAR点云中重建建筑物表面，能够生成连续可评估的占用函数，对噪声数据具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 从LiDAR点云重建建筑物面临的主要挑战是：在不同点密度和噪声干扰下准确捕捉建筑物表面。需要灵活获取高质量3D轮廓以适应不同分辨率。

Method: 提出OCCDiff方法，将潜在扩散过程与函数自编码器架构结合，生成可在任意位置评估的连续占用函数。引入点编码器为扩散学习提供条件特征，约束占用解码器的最终预测，并为潜在编码器插入多模态特征。采用多任务训练策略增强模型性能。

Result: 实验结果表明，该方法生成物理一致的样本，与目标分布具有高保真度，并对噪声数据表现出鲁棒性。

Conclusion: OCCDiff通过在占用函数空间应用潜在扩散模型，有效解决了LiDAR点云建筑物重建中的点密度变化和噪声干扰问题，能够生成高质量的连续3D表面表示。
...
TL;DR: sCoT：一种新的视觉推理范式，将多模态CoT重新表述为带自调用的纯语言CoT，通过主代理分解任务并调用虚拟子代理解决原子子任务，显著提升训练效率和效果。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态思维链（iMCoT）依赖高质量推理数据进行强化学习优化，但这类数据稀缺，导致训练困难。需要一种更高效的视觉推理训练方法。

Method: 提出自调用思维链（sCoT），将多模态CoT重新表述为纯语言CoT。主代理将复杂视觉推理任务分解为原子子任务，调用参数共享的虚拟子代理在隔离上下文中解决。采用组相对策略优化来强化有效推理行为。

Result: 在HR-Bench 4K上，sCoT将整体推理性能提升高达1.9%，同时减少约75%的GPU小时，相比强基线方法显著提升了训练效率和效果。

Conclusion: sCoT通过将多模态CoT转化为纯语言自调用范式，有效解决了高质量推理数据稀缺的问题，在显著减少计算成本的同时提升了视觉推理性能。
...
TL;DR: 提出渐进式重参数化策略，用紧凑的PHM层替代密集前馈网络块，压缩多模态语言模型，减少参数和计算量，同时保持性能
...
Method: 渐进式重参数化策略：逐步用参数化超复数乘法层替代密集前馈网络块；结合残差插值调度、轻量级重建和知识蒸馏损失，确保PHM模块继承原始功能

Result: 在多个视觉语言模型上评估，方法保持与基准模型相当的性能，同时显著减少模型大小和推理延迟

Conclusion: 渐进式PHM替代为更高效的多模态推理提供了架构兼容的路径，可与现有的低位量化技术互补
...
Conclusion: MVP框架通过多视角推理有效解决了GUI grounding中的坐标预测不稳定问题，显著提升了各种模型在基准测试中的性能，且无需额外训练
...
TL;DR: 提出统一多模态油画生成与编辑框架，结合参考图像、手绘草图和文本提示进行精确控制，通过空间对齐、语义增强和自监督风格转换实现交互式油画创作。
...
Method: 1. 训练阶段增强空间对齐和语义增强条件策略，将掩码和草图映射为空间约束，从参考图像和文本编码上下文嵌入作为特征约束；2. 基于笔触渲染的自监督风格转换管道，模拟油画修复的修复动态，构建大规模配对训练数据集；3. 推理阶段使用AdaIN算子集成特征确保风格一致性。

Result: 实验表明该系统能够在保持油画艺术品质的同时实现细粒度编辑，在风格化油画生成与编辑方面达到了前所未有的想象力实现水平。

Conclusion: 提出了一个统一的多模态框架，通过空间语义对齐、自监督数据增强和风格一致性控制，成功解决了油画数字生成与编辑的挑战，实现了交互式油画创作。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D原生生成器虽然能合成可靠的几何结构，但在实现真实外观方面仍有不足。主要障碍在于缺乏多样化和高质量的真实世界3D资产，这些资产需要丰富的纹理细节，而由于场景尺度多样、物体非刚性运动以及3D扫描仪精度有限，获取这样的数据非常困难。

Method: 1. 利用GPT-4o-Image生成图像数据驱动框架；2. 设计结构对齐的多视角合成流程，构建细节增强的多视角数据集与3D几何配对；3. 提出真实细节增强方案，利用感知特征适应和语义结构匹配，在保持3D原生几何结构一致性的同时增强外观真实感；4. 为几何-纹理耦合和解耦的3D原生生成范式提供专门的训练策略。

Result: 实验表明，Photo3D能够很好地泛化到不同的3D原生生成范式，并在真实感3D生成方面达到了最先进的性能。

Conclusion: Photo3D通过利用生成图像数据解决了3D生成中真实感不足的问题，提出的结构对齐多视角合成和细节增强方案有效提升了3D生成的外观质量，且具有对不同3D生成范式的良好通用性。
...
TL;DR: Fast-ARDiff提出统一的AR-扩散混合框架，通过熵感知推测解码和动态调度优化，同时加速AR推测解码和扩散解码，实现无损加速


<details>
  <summary>Details</summary>
Motivation: 现有的AR-扩散混合范式结合了AR的结构化建模和扩散的光真实感合成优势，但由于顺序AR生成和迭代去噪过程，存在高延迟问题，需要解决这一瓶颈

Method: 1) 熵感知推测策略：鼓励草稿模型产生与目标模型熵特征对齐的高熵表示，缓解熵不匹配和拒绝率高的问题；2) 动态调度集成：将扩散解码整合到端到端框架中，优先优化AR部分来指导扩散；3) 联合蒸馏框架：结合轨迹和分布匹配确保稳定训练和高质量合成；4) 推理时使用AR模块的浅层特征熵预过滤低熵草稿

Result: 在ImageNet 256×256上，TransDiff实现4.3倍无损加速；在文本条件生成上，NextStep-1实现3倍加速，达到最先进的加速效果

Conclusion: Fast-ARDiff通过联合优化AR和扩散组件，有效解决了AR-扩散混合范式的高延迟瓶颈，实现了显著的加速效果，同时保持生成质量
...
TL;DR: 提出基于四元数Wasserstein距离的生成对抗网络，解决彩色图像生成中的通道相关性问题，提升生成效率和质量


<details>
  <summary>Details</summary>
Motivation: 现有彩色图像生成模型忽略颜色通道间的相关性，可能导致色差问题；同时缺乏系统性的彩色图像数据分布理论，缺少衡量不同彩色图像数据集的理论基础

Method: 定义新的四元数Wasserstein距离并发展其对偶理论，通过四元数凸集分离定理和四元数Farkas引理推导强对偶形式，提出基于四元数Wasserstein距离的Wasserstein四元数生成对抗网络

Result: 实验表明，该新模型在生成效率和图像质量方面均超越了传统的（四元数）生成对抗网络和Wasserstein生成对抗网络

Conclusion: 通过引入四元数Wasserstein距离，有效解决了彩色图像生成中的通道相关性问题，为彩色图像数据分布提供了理论框架，显著提升了生成模型的性能
...
TL;DR: 提出一种无需迭代的固定点估计器用于扩散反演，通过误差近似方法降低计算成本，在重建任务中表现优于DDIM反演和其他基于固定点迭代的方法


<details>
  <summary>Details</summary>
Motivation: 扩散反演中的固定点迭代方法虽然能最小化重建误差，但存在计算成本高和超参数选择复杂的问题，需要一种更高效的解决方案

Method: 首先推导理想反演步骤中固定点的显式表达式，然后引入误差近似方法，使用前一步可计算的误差来近似当前步骤的未知误差，从而得到可计算的近似固定点表达式

Result: 在NOCAPS和MS-COCO两个文本-图像数据集上的评估显示，该方法在重建任务中表现一致且优于DDIM反演和其他基于固定点迭代的方法，无需额外迭代或训练

Conclusion: 提出的迭代自由固定点估计器解决了扩散反演中固定点迭代方法的高计算成本和超参数选择问题，实现了高效且性能优越的重建效果
...
TL;DR: 提出SSCATeR算法，利用LiDAR连续扫描特性，通过时间窗口和卷积结果存储，仅处理点云变化区域，减少卷积操作6.61倍，保持精度不变。


<details>
  <summary>Details</summary>
Motivation: 传统LiDAR目标检测方法对整个点云进行卷积计算，计算量大。利用LiDAR连续扫描时大部分区域不变的特性，可以只处理变化区域，大幅减少计算量。

Method: 使用滑动时间窗口和短步长，存储卷积结果实现时间维度数据复用。扩展散射卷积算法支持数据复用，提出SSCATeR算法，将LiDAR数据视为连续流，仅处理点云变化部分。

Result: 处理时间减少高达6.61倍，特征图输出与传统稀疏卷积技术完全相同，显著提高网络计算效率。

Conclusion: SSCATeR算法通过时间数据复用和仅处理变化区域，在保持检测精度的同时大幅提升LiDAR目标检测的计算效率。
...
TL;DR: 提出大规模自动化框架，通过无监督分解发现fMRI活动中的可解释模式，并用自然图像和语言描述解释这些模式，揭示了数千个可解释的视觉概念表征。


<details>
  <summary>Details</summary>
Motivation: 人类大脑如何表征视觉概念以及在哪些脑区编码这些表征仍是长期挑战。现有研究规模小、依赖人工检查、聚焦特定区域和属性，缺乏系统验证，需要大规模自动化方法来发现和解释整个皮层的视觉表征。

Method: 采用两阶段方法：1) 通过无监督数据驱动分解方法发现fMRI活动中的候选可解释模式；2) 通过识别最能激发该模式的自然图像集并生成描述其共享视觉意义的自然语言描述来解释每个模式。引入自动化流程测试多个候选解释、分配定量可靠性分数，为每个体素模式选择最一致的描述。

Result: 框架揭示了数千个可解释模式，涵盖许多不同的视觉概念，包括以前未报告的细粒度表征。

Conclusion: 该大规模自动化框架能够系统地发现和解释人类皮层中的视觉表征，克服了传统小规模研究的局限性，为理解大脑视觉概念表征提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有神经ISP设计缺乏模块化，限制了控制性、可扩展性和调试性。需要一种既能保持高质量渲染，又能提供中间过程完全控制、适应不同相机和用户偏好的解决方案

Method: 采用模块化神经ISP框架，将整个渲染过程分解为多个可独立控制的中间阶段。框架完全基于学习，提供不同容量变体（0.5M到3.9M参数），支持用户交互式编辑工具开发

Result: 在多个测试集上获得有竞争力的定性和定量结果，实现了高渲染精度、可扩展性、可调试性，能泛化到未见过的相机，并灵活匹配不同用户偏好风格

Conclusion: 模块化神经ISP设计不仅实现了高质量图像渲染，还显著提升了控制性、可扩展性和灵活性，为交互式照片编辑工具开发提供了有效框架
...
<details>
  <summary>Details</summary>
Motivation: 现有持续测试时适应方法通常依赖固定或批量级阈值，无法考虑不同类别和实例间的难度差异。这在语义分割中尤其成问题，因为每个图像都需要密集的多类别预测。

Method: 提出自适应调整伪标签的方法，根据每个图像内的置信度分布动态调整伪标签，并针对受域偏移影响最大的类别动态平衡学习过程。这是一种细粒度的、类别和实例感知的适应方法。

Result: 在八个CTTA和TTA场景（包括合成到真实和长期偏移）的广泛实验中，该方法始终优于最先进的技术，为演化条件下的语义分割设定了新标准。

Conclusion: 该方法通过细粒度的类别和实例感知适应，产生更可靠的监督并减轻持续适应过程中的错误累积，在持续测试时适应中表现出色。
...
TL;DR: HiGINE是一个基于层次图的方法，利用多路免疫荧光图像中的肿瘤微环境特征预测肺癌患者生存期，通过融合临床分期信息提升风险分层性能


<details>
  <summary>Details</summary>
Motivation: 肿瘤微环境是重要的预后生物标志物来源，但现有方法未能充分捕捉不同细胞类型间的复杂相互作用，需要更有效的分析方法来提升肺癌风险分层能力

Method: 提出HiGINE层次图模型，编码细胞邻域的局部和全局相互关系，整合细胞类型和形态学信息，通过多模态融合将癌症分期与mIF衍生特征结合

Result: 在两个公共数据集上验证显示，HiGINE在风险分层、鲁棒性和泛化性方面均有提升，多模态融合进一步增强了模型性能

Conclusion: HiGINE能够有效利用肿瘤微环境的复杂相互作用信息，为肺癌预后预测提供更准确的风险分层工具，具有临床应用潜力
...
TL;DR: 提出自动对齐多摄像头手术视频的方法，通过检测灯光系统移动、重新对齐帧、选择遮挡最少的摄像头，生成固定视角的手术区域视频，提高观看体验和视频质量。


<details>
  <summary>Details</summary>
Motivation: 手术视频对教育和研究很重要，但拍摄时外科医生经常遮挡视野。传统方法需要安装多个摄像头并手动对齐，过程繁琐且需要后期处理，因此需要自动化解决方案。

Method: 方法包括：1) 识别灯光系统移动的帧；2) 重新对齐这些帧；3) 选择遮挡最少的摄像头；4) 生成固定视角的手术区域视频。还实现了多种合成选项供用户选择。

Result: 用户研究显示，外科医生认为该方法生成的视频在确认手术区域和观看舒适度方面优于传统方法。视频质量也有提升，并通过用户研究评估了不同合成选项的偏好。

Conclusion: 该方法成功实现了手术视频的自动对齐和合成，提高了视频质量和观看体验，为手术视频的自动化处理提供了有效解决方案。
...
TL;DR: CLIP模型在少样本学习中存在模板-样本相似性偏差，该偏差使模型过度依赖模板而非真实类别对齐。本文提出使用空提示来捕获无偏模板特征，通过两阶段框架在预训练和微调中校正偏差，显著提升分类准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: CLIP模型在少样本学习中存在模板-样本相似性偏差，这种偏差使模型过度依赖文本模板与图像样本的相似性，而非真正的样本-类别对齐，导致分类准确性和鲁棒性下降。

Method: 提出两阶段框架：1) 预训练阶段使用空提示（不含类别信息的文本输入）揭示并减少CLIP编码器中的模板诱导偏差；2) 少样本微调阶段使用偏差校准损失强制图像与其类别正确对齐，确保模型关注相关视觉线索。

Result: 在多个基准测试中，模板校正方法显著减少了由模板-样本相似性引起的性能波动，获得了更高的分类准确性和更强的鲁棒性。

Conclusion: 通过空提示和偏差校准，本文方法有效解耦了CLIP中的模板偏差，提升了少样本学习的性能稳定性和分类准确性。
...
TL;DR: 使用深度学习（YOLOv8s和MobileNetV3L）改进花粉粒在光学和全息显微图像中的检测与分类，针对兽医细胞学应用，通过数据集扩展和自动标注技术显著提升了全息图像的性能。


<details>
  <summary>Details</summary>
Motivation: 探索深度学习在花粉粒检测和分类中的应用，特别是在兽医细胞学领域，同时评估其在光学和全息显微图像中的性能差异，并解决全息图像性能较低的问题。

Method: 使用YOLOv8s进行目标检测，MobileNetV3L进行分类任务；通过自动标注和边界框区域扩增技术扩展数据集，特别是针对全息图像的性能提升。

Result: 光学图像检测达到91.3% mAP50，分类准确率97%；全息图像初始性能较低（检测2.49% mAP50，分类42%），通过技术改进后提升至检测13.3% mAP50，分类54%。

Conclusion: 深度学习技术可以与成本效益高的无透镜数字全息显微设备结合，至少在图像分类任务中可行，为花粉粒的自动化检测和分类提供了有效解决方案。
...
TL;DR: 提出一个仅使用单目RGB视觉和自然语言指令的无人机视觉语言导航统一框架，通过多任务学习联合优化空间感知、轨迹推理和动作预测，在Aerial VLN基准上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有空中视觉语言导航方法依赖全景图像、深度输入或里程计，增加了系统成本和集成复杂度，阻碍了轻量级无人机的实际部署。需要一种仅使用单目RGB视觉的解决方案。

Method: 1) 将导航建模为下一个token预测问题；2) 通过提示引导的多任务学习联合优化空间感知、轨迹推理和动作预测；3) 提出关键帧选择策略减少视觉冗余；4) 引入动作合并和标签重加权机制缓解长尾监督不平衡。

Result: 在Aerial VLN基准测试中，该方法在仅使用单目RGB的挑战性设置下，在已见和未见环境中均取得强劲结果，显著优于现有RGB基线，并缩小了与最先进全景RGB-D方法的性能差距。

Conclusion: 该框架证明了仅使用单目RGB视觉和自然语言指令实现高效空中视觉语言导航的可行性，通过多任务学习和关键优化策略，为轻量级无人机的实际部署提供了有前景的解决方案。
...
TL;DR: 该研究提出了一种基于分布的个体公平性评估与缓解框架，将肤色视为连续属性而非分类标签，使用核密度估计建模肤色分布，并比较12种统计距离度量来量化肤色分布差异，提出基于距离的重新加权损失函数来纠正少数肤色的代表性不足。


<details>
  <summary>Details</summary>
Motivation: 肤色历史上一直是歧视的焦点，但医学影像机器学习中的公平性研究通常依赖粗糙的亚组分类，忽视了个体层面的差异。这种基于群体的方法可能掩盖亚组内异常值面临的偏见。

Method: 将肤色视为连续属性而非分类标签；使用核密度估计(KDE)建模肤色分布；比较12种统计距离度量来量化肤色分布差异；提出基于距离的重新加权(DRW)损失函数来纠正少数肤色的代表性不足；在CNN和Transformer模型上进行实验。

Result: 实验表明：(i)分类重新加权在捕捉个体层面差异方面存在局限性；(ii)基于分布的重新加权表现更优，特别是使用Fidelity Similarity(FS)、Wasserstein Distance(WD)、Hellinger Metric(HM)和Harmonic Mean Similarity(HS)时。
...
Method: 提出OpenMonoGS-SLAM框架，统一3D高斯泼溅与开放集语义理解。利用视觉基础模型：MASt3R用于视觉几何，SAM和CLIP用于开放词汇语义。仅使用自监督学习目标，无需深度输入或3D语义真值。设计了专门的内存机制来管理高维语义特征，构建高斯语义特征图。
...
Method: 提出自精炼扩散框架，使用基于可解释人工智能（XAI）的缺陷高亮器生成缺陷激活图（FAMs），识别伪影和不真实区域。这些FAMs在前向过程中放大缺陷区域的噪声，在反向过程中重点关注这些区域，从而改善重建质量。

Result: 在各种基于扩散的模型中，Fréchet起始距离（FID）提升了高达27.3%，在不同数据集上表现一致强劲。在图像生成、文本到图像生成和修复等不同任务中都显示出稳健的有效性。
...
TL;DR: 提出一种通过分析视频模糊模式来估计深度的方法，结合光学设计和视觉语言模型实现深度估计和轨迹稠密化
...
Method: 使用现成的视觉编码器和点跟踪器提取视频信息，通过窗口化嵌入和多窗口聚合估计深度图，利用视觉语言模型对光学算法生成的稀疏轨迹进行稠密化处理

Result: 在多个深度数据集上评估显示，该方法在大深度范围内表现优异，具有良好的泛化能力。在手持拍摄设置中，光学算法相比真实轨迹具有更高的精度，稠密重建保持强准确性

Conclusion: 通过分析视频中的透视模糊模式可以有效估计深度，结合光学设计和深度学习的方法能够实现精确的深度估计和轨迹稠密化，为无稳定器条件下的深度感知提供了新思路
...
TL;DR: 提出MatteViT框架，结合空间与频域信息进行文档阴影去除，通过高频放大模块和连续亮度阴影蒙版保留文本细节，在公开基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 文档阴影去除对于提升数字化文档清晰度至关重要，阴影常常会模糊或扭曲文本边缘和线条等高频细节结构，需要有效保留这些细粒度信息。

Method: 提出MatteViT框架：1) 轻量级高频放大模块(HFAM)分解并自适应放大高频分量；2) 基于连续亮度的阴影蒙版，使用自定义蒙版数据集和阴影蒙版生成器提供精确空间引导；3) 结合空间和频域信息进行阴影去除。

Result: 在RDD和Kligler等公开基准测试中达到最先进性能，能够准确识别细粒度区域并高保真恢复，在下游任务如光学字符识别中更好地保留文本级细节，提升识别性能。

Conclusion: MatteViT为实际文档阴影去除提供了鲁棒实用的解决方案，通过高频细节保留策略显著提升了阴影去除质量，对下游文档处理任务有实际应用价值。
...
TL;DR: CoIG框架将图像生成重构为类似人类艺术创作的序列化语义过程，通过LLM分解复杂提示为逐步指令，提升可监控性和组合鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前图像生成模型内部过程是"黑盒"，缺乏可观察性和干预能力，影响模型可靠性、安全性和可控性，且非人类化的工作流程难以解释。

Method: CoIG框架：1) 使用LLM将复杂提示分解为简单逐步指令序列；2) 图像生成模型按计划逐步生成和编辑图像；3) 每个步骤聚焦单一语义实体，实现直接监控。

Result: CoIG显著提升定量可监控性，在组合鲁棒性方面与基线模型竞争；引入CoIG可读性和因果相关性两个新指标评估中间步骤；缓解实体崩溃问题。

Conclusion: CoIG框架通过序列化语义过程使图像生成更透明、可监控，类比CoT在LLM中的优势，是模型无关的通用解决方案。
...
TL;DR: 提出无需训练的双曲适配器T-DHA，利用双曲空间嵌入层次化视觉-语言关系，在少样本图像识别和领域泛化任务中显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在领域变化时性能下降，或需要大量计算资源进行微调。需要一种无需训练、能有效处理层次化视觉-语言关系的适应方法

Method: 提出T-DHA方法：1) 在双曲空间而非欧氏空间中表征视觉-语言关系的层次树结构；2) 利用庞加莱球模型嵌入层次数据；3) 结合负学习，用更少特征维度实现更准确鲁棒的分类

Result: 在多个数据集上的实验表明，T-DHA方法在少样本图像识别和领域泛化任务中显著优于现有最先进方法

Conclusion: 双曲空间因其指数级体积增长特性，能更有效地嵌入层次化视觉-语言关系，T-DHA方法为视觉语言模型提供了一种无需训练的高效适应方案
...
<details>
  <summary>Details</summary>
Motivation: 车载摄像头进行驾驶员分心行为识别需要在边缘设备上实时推理，但轻量级模型难以捕捉细粒度行为特征，ROI方法又增加计算成本，需要在效率和准确性之间取得平衡

Method: 提出C-DIRA框架，结合显著性驱动的Top-K ROI池化和融合分类进行局部特征提取，动态ROI路由仅对高难度样本应用ROI推理，使用伪域标注和对抗学习学习对驾驶员和背景变化鲁棒的域不变特征

Result: 在State Farm Distracted Driver Detection数据集上，C-DIRA在显著减少FLOPs和降低延迟的同时保持高准确率，在模糊和低光等视觉退化条件下表现鲁棒，在未见域上性能稳定
...
TL;DR: InfiniteVL是一种线性复杂度视觉语言模型架构，结合滑动窗口注意力与Gated DeltaNet，在有限训练数据下达到领先VLM性能，实现3.6倍推理加速和恒定内存占用。
...
Method: 提出InfiniteVL架构，融合滑动窗口注意力与Gated DeltaNet；设计三阶段训练策略：蒸馏预训练、指令微调和长序列SFT。

Result: 仅用不到2%的训练数据，InfiniteVL大幅超越先前线性复杂度VLM，匹配领先Transformer VLM性能；相比FlashAttention-2加速的Transformer VLM，实现3.6倍推理加速，保持恒定延迟和内存占用；在流式视频理解中维持24 FPS实时预填充速度。

Conclusion: InfiniteVL成功解决了现有VLM架构在长序列处理和信息密集型任务上的局限性，实现了高效的多模态理解能力，为资源受限环境提供了实用解决方案。
...
TL;DR: 提出MName框架，通过内存银行机制提供大量负样本进行对比学习，无需外部编码器，在生成模型中实现高效表示学习
...
Method: 提出MName框架：1）使用内存银行机制维护动态更新的负样本队列，解耦负样本数量与批次大小；2）采用低维投影头减少内存和带宽开销；3）无需外部编码器，完全自包含

Result: 在ImageNet-256上，仅用40万步达到FID 2.40的SOTA结果，显著优于可比方法，收敛速度更快，生成质量更高

Conclusion: MName框架消除了对外部预训练模型的依赖，不增加推理时的参数和计算成本，实现了更高效的生成模型训练和表示学习
...
<details>
  <summary>Details</summary>
Motivation: 现有3D点云自监督学习主要基于MAE等生成方法，但这些方法难以有效捕获高层判别性特征，导致线性探测等下游任务性能不佳。而对比学习方法在图像数据上表现出优秀的判别特征表示和泛化能力，但直接应用于3D数据无法有效学习局部细节。
...
Conclusion: CSCon框架通过双分支中心-周围对比学习有效解决了3D点云自监督学习中生成方法的局限性，在多个评估协议下表现出优异的性能，为3D点云对比学习提供了新思路。
...
Method: 提出MoSAIC-ReID框架，使用基于LoRA的专家模型（每个专家对应一个属性）和oracle路由器进行受控的属性归因分析。通过广义线性模型、统计检验和特征重要性分析来量化属性重要性。

Result: 在Market-1501和DukeMTMC数据集上取得有竞争力的性能（假设测试时有属性标注）。通过大规模量化研究发现：服装颜色和内在特征等属性贡献最大，而配饰等不常见线索影响有限。

Conclusion: 该工作为可解释的行人重识别提供了原则性框架，并强调了在实践中整合显式语义知识的要求。代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 图像描述在辅助视障人士、内容管理系统和人机交互中很重要，但处理低分辨率图像时面临挑战。现有大型模型（如Transformer）虽然性能好，但计算资源需求大、内存占用高，在资源受限场景下难以重新训练。

Method: 提出SOLI方法，采用孪生网络架构优化潜在嵌入，通过双路径神经网络结构增强图像到文本转换的效率和准确性，专门针对轻量级低分辨率图像描述设计。

Result: SOLI方法在保持性能的同时最小化了计算开销，使其成为资源受限场景下训练的理想选择。

Conclusion: SOLI为低分辨率图像描述提供了一种高效、轻量级的解决方案，解决了传统大型模型在资源受限环境中的部署难题。
...
<details>
  <summary>Details</summary>
Motivation: 现有基础模型在单目深度估计中表现出强大的泛化能力，但直接应用于全周单目深度估计面临两大挑战：1）高计算成本限制实时性能；2）难以估计度量尺度深度，因为这些模型通常只预测相对深度。

Method: 提出混合回归框架，结合知识蒸馏方案和深度分箱模块增强尺度一致性。具体包括：1）交叉交互知识蒸馏方案，将基础模型的尺度不变深度分箱概率蒸馏到学生网络，同时指导学生从真实深度推断度量尺度分箱中心；2）视图关系知识蒸馏，编码相邻相机视图间的结构关系并转移以增强跨视图深度一致性。

Result: 在DDAD和nuScenes数据集上的实验表明，该方法相比传统监督方法和现有知识蒸馏方法更有效，并在性能和效率之间取得了良好平衡，满足实时要求。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的训练免费开放词汇语义分割方法在精确定位或复杂流程方面存在挑战，特别是在遥感场景中密集小目标众多。SAM 3统一了分割和识别，本文探索其在遥感开放词汇语义分割中的应用潜力。

Method: 1. 掩码融合策略：结合SAM 3的语义分割头和Transformer解码器（实例头）输出，利用两者优势改善土地覆盖分割。2. 存在分数过滤：利用存在头分数过滤场景中不存在的类别，减少由于词汇量大和遥感场景中块级处理导致的误报。

Result: 在广泛的遥感数据集上进行评估，实验表明这种简单适应方法取得了有希望的性能，展示了SAM 3在遥感开放词汇语义分割中的潜力。

Conclusion: SAM 3在遥感开放词汇语义分割任务中具有应用潜力，无需训练即可通过简单适应获得良好性能，为遥感场景中的密集小目标分割提供了新思路。
...
TL;DR: 提出VALOR框架：无需标注的训练方法，通过AI验证器（LLM验证器强化推理，VLM验证器强化视觉定位）提升视觉推理和定位能力


<details>
  <summary>Details</summary>
Motivation: 现有视觉推理方法存在局限性：纯语言链式思维方法需要大规模监督数据，程序合成方法存在逻辑缺陷和定位错误。需要一种无需标注但能同时提升推理和定位能力的方法。

Method: 使用AI验证器的无标注训练框架：1) LLM验证器通过强化学习精炼LLM推理；2) VLM验证器通过自动硬负样本挖掘增强视觉定位。结合语言模型的推理分解能力和视觉专家模型的定位能力。

Result: 在多种空间推理任务上评估，方法提升了视觉推理能力，超越了开源和专有模型。改进的视觉定位模型进一步超越了最近的纯文本视觉推理方法。

Conclusion: VALOR框架通过AI验证器实现了无需标注的视觉推理训练，有效结合了语言模型的推理能力和视觉模型的定位能力，在多个任务上取得了优越性能。
...
TL;DR: 提出基于姿态关键点的端到端手语检测模型，用于在连续手语句子中检索特定手语词汇，避免传统基于文本或RGB帧的方法


<details>
  <summary>Details</summary>
Motivation: 手语识别领域主要关注词汇识别，但手语检索（在连续手语序列中检测特定手语）尚未充分探索。现有方法依赖中间词汇识别或文本匹配，计算成本高且受视觉噪声影响。

Method: 使用姿态关键点而非原始RGB帧，构建仅编码器架构的端到端模型，通过二元分类头判断查询手语是否出现在目标序列中。

Result: 在WSLP 2025共享任务的Word Presence Prediction数据集上达到61.88%准确率和60.00% F1分数，证明姿态表示的有效性。

Conclusion: 基于姿态的框架为手语检测提供了有效解决方案，显著降低计算成本并减少视觉噪声，为自动手语检索和验证的未来研究奠定基础。
...
<details>
  <summary>Details</summary>
Motivation: 工业蘑菇种植依赖计算机视觉监控和自动化收获，但开发准确检测分割模型需要大量精确标注数据，成本高昂。合成数据虽可扩展但常缺乏真实感，难以泛化到真实场景。

Method: 提出新颖工作流，集成Blender中的3D渲染与约束扩散模型，自动生成高质量、逼真的双孢蘑菇合成图像。保持对3D场景配置和标注的完全控制，无需专业计算机图形学知识。

Result: 发布两个合成数据集（各含6000张图像，描绘超过25万个蘑菇实例）。在零样本设置下评估Mask R-CNN模型，在两个独立真实数据集（包括新收集的基准）上测试，实现SOTA分割性能（M18K上F1=0.859），仅使用合成训练数据。

Conclusion: 该方法在双孢蘑菇上得到验证，但提出的流程可轻松适应其他蘑菇物种或其他农业领域，如水果和叶片检测。
...
Conclusion: Astra成功构建了一个能够支持多样化交互的通用世界模型，为探索、操作和相机控制等实际任务提供了有效的长时域预测能力。
...
TL;DR: 提出基于偏度引导的剪枝方法，用于压缩多模态Swin Transformer模型，在联邦学习环境中实现36%模型大小缩减且不损失精度
...
Method: 提出偏度引导剪枝方法，基于输出分布的统计偏度，有选择性地剪枝多模态Swin Transformer的多头自注意力和多层感知机层。在水平联邦学习环境中验证。

Result: 在紧凑型Swin Transformer上实验显示，模型大小减少约36%，同时保持准确率不变。证明了在边缘设备上实现高效模型压缩和隐私保护分布式学习的可行性。

Conclusion: 该方法成功解决了医学AI模型在边缘设备部署的挑战，通过偏度引导剪枝在联邦学习环境中实现了高效模型压缩，为多模态医学AI在边缘计算中的应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有运动控制方法通常存在控制粒度粗糙和可扩展性有限的问题，导致输出质量不足以满足实际应用需求。需要缩小这一差距，实现精确高质量的运动控制。

Method: 使用密集点轨迹表示物体运动，实现细粒度场景控制；将轨迹投影到潜在空间，沿轨迹传播第一帧特征，生成对齐的时空特征图作为更新的潜在条件；该特征图直接集成到现成的图像到视频模型中作为运动指导，无需架构修改。

Result: 通过规模化训练，Wan-Move能生成5秒480p视频，其运动可控性在用户研究中与Kling 1.5 Pro的商业Motion Brush相媲美；在MoveBench和公共数据集上的实验一致显示其优越的运动质量。

Conclusion: Wan-Move通过简单可扩展的框架实现了精确高质量的视频运动控制，无需辅助运动编码器或基础模型架构修改，为实际应用提供了有效的解决方案。
...
TL;DR: LoFA：一种通过超网络预测LoRA权重分布模式，实现秒级个性化视觉生成模型适配的通用框架


<details>
  <summary>Details</summary>
Motivation: 当前个性化视觉生成模型方法（如LoRA）需要特定任务数据和长时间优化，不实用；现有超网络方法难以将细粒度用户提示映射到复杂的LoRA分布

Method: 发现LoRA参数相对变化的分布模式规律，设计两阶段超网络：先预测相对分布模式定位关键适配区域，再用其指导最终LoRA权重预测

Result: 方法能在数秒内预测高质量个性化先验，跨多个任务和用户提示均表现优异，甚至优于需要数小时处理的传统LoRA方法

Conclusion: LoFA框架通过有效预测LoRA权重分布模式，实现了快速高效的个性化视觉生成模型适配，解决了现有方法的实用性问题
...
TL;DR: 生成式方法通过解码器约束和反演能实现组合泛化，而非生成式方法通常需要大规模预训练或额外监督才能达到类似效果。


<details>
  <summary>Details</summary>
Motivation: 研究生成式和非生成式视觉方法能否实现人类感知的关键特征——组合泛化，以验证生成式方法对人类级视觉感知的必要性。

Method: 在组合数据生成过程下，形式化分析解码器（生成式）和编码器（非生成式）方法实现组合泛化所需的归纳偏置，理论证明非生成式方法难以实施这些偏置，而生成式方法可通过约束解码器并反演来实现。

Result: 理论分析表明非生成式方法难以实施组合泛化所需的归纳偏置，而生成式方法能通过梯度搜索或生成回放有效实施；实证研究显示非生成式方法常失败于组合泛化，需要大规模预训练或额外监督，而生成式方法显著改善组合泛化且无需额外数据。

Conclusion: 生成式方法通过解码器约束和反演能有效实现组合泛化，支持生成式方法对人类级视觉感知的必要性假设；非生成式方法在组合泛化方面存在根本性限制。
...
TL;DR: Tri-Bench基准测试显示，视觉语言模型在平面三角形几何推理任务中表现一般（平均准确率约69%），无法识别特殊三角形类型，且相机倾斜会降低性能，表明模型未能有效利用提示中的参考框架信息。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型（VLMs）能力强大，但在真实场景变化下经常失败。需要可验证的几何推理来构建可信且可控的智能体AI。研究旨在评估VLMs在几何推理任务中的表现，特别是在相机姿态和场景上下文干扰下的鲁棒性。

Method: 提出Tri-Bench基准测试，包含平面三角形问题，专注于相对几何推理。测试两个关键部署因素：相机姿态（平面vs倾斜）和场景上下文干扰（10个日常物体）。使用固定提示，明确描述周围正方形边框作为参考框架，通过单应性实现正确答案。评估四个最新VLMs在六个简单任务上的表现，包括二元和连续目标。

Result: VLMs整体准确率中等（平均约69%，最佳约75%，最差约64%）。模型响应与图像平面2D投影更一致（平均准确率约72%）。所有四个VLMs在识别少数三角形类型（等边、等腰、直角三角形）时完全失败（准确率降至约0%）。相机倾斜使整体准确率下降约4.1%。物体干扰对VLM准确率无显著影响。

Conclusion: 视觉语言模型未能正确利用提示中提供的明确参考框架提示，默认依赖2D图像平面线索。模型在几何推理任务中存在系统性缺陷，特别是在识别特殊三角形类型和处理相机姿态变化方面，这对构建可信赖的智能体AI构成挑战。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型正在成为遥感领域的强大通用工具，但现有的VLM在卫星图像中的视觉定位能力有限，需要增强其结构化空间推理能力以实现更精确的对象定位。

Method: 通过专用控制令牌将专用定位模块与预训练的VLM接口连接，在多样化的指令跟随任务上微调VLM，实现语言和空间信息的联合推理。

Result: 在多个遥感基准测试中一致改进现有技术，视觉定位任务相对先前方法提升24.8%，显著增强了模型在复杂卫星场景中精确定位对象的能力。

Conclusion: 将结构化空间推理集成到VLM中具有显著优势，为更可靠的现实世界卫星数据分析铺平了道路，展示了增强VLM视觉定位能力的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 无人机航拍图像中的目标具有任意方向和精细细节，需要旋转不变性进行精确分割。传统卷积架构（如U-Net）缺乏旋转不变性，导致分割精度下降。虽然可以通过多方向滤波器组实现旋转不变性，但这会显著增加计算成本和内存流量。

Method: 提出GPU优化的旋转不变卷积框架，消除了传统基于矩阵乘法的卷积所需的数据降维（im2col）步骤。通过利用对称旋转滤波器间的结构化数据共享，实现了内存流量和计算冗余大幅减少的多方向卷积。该方法还推广到加速任意（非对称）旋转角度的卷积。

Result: 在广泛基准测试中，提出的卷积比CUDNN快20-55%，能耗低15-45%，同时保持与最先进旋转不变方法相当的精度。在八方向设置下，对256×256输入实现45%加速和41%能耗节省，对1024×1024输入实现32%加速和23%能耗降低。集成到U-Net分割模型中，比非旋转感知基线精度提升达6%。
...
TL;DR: UniLayDiff：首个统一的扩散Transformer模型，通过多模态方法处理内容感知布局生成的各种任务，包括无条件到有条件生成，无需为不同条件训练单独模型。


<details>
  <summary>Details</summary>
Motivation: 内容感知布局生成在平面设计自动化中至关重要，但现实应用场景多样，现有方法要么只能处理部分任务，要么需要为不同条件训练单独模型，缺乏真正的统一解决方案。

Method: 提出UniLayDiff统一扩散Transformer，将布局约束视为独立模态，采用多模态扩散Transformer框架捕捉背景图像、布局元素和多样化约束之间的复杂交互。通过LoRA微调整合关系约束。

Result: UniLayDiff在从无条件到各种条件生成任务中均达到最先进性能，是首个统一处理完整内容感知布局生成任务的模型。

Conclusion: 该工作首次实现了内容感知布局生成任务的统一建模，通过多模态扩散Transformer框架和LoRA微调策略，不仅实现了统一条件生成，还提升了整体布局质量。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于单图像生成3D场景的方法存在局限性：对象中心训练难以泛化到复杂大尺度场景，且缺乏忠实结构和纹理。需要一种能处理复杂场景、保持几何稳定性和视图一致纹理的方法。

Method: 采用三阶段迭代框架：1) 空间先验初始化；2) 视觉引导的3D场景网格生成；3) 空间引导的新视角生成。在2D和3D域之间交替优化，逐步改进结构和外观。

Result: 在多样化场景上的实验表明，EvoScene相比基线方法在几何稳定性、视图一致纹理和未见区域补全方面表现更优，能生成可直接用于实际应用的3D网格。

Conclusion: EvoScene通过结合现有模型的互补优势，实现了从单图像高质量重建完整3D场景，为实际应用提供了实用的3D网格生成方案。
...
TL;DR: LiDAS：一种闭环主动照明系统，通过动态预测最优照明场来最大化下游感知性能，实现白天训练模型在夜间的零样本泛化
...
Result: 相比标准近光灯，mAP50提升18.7%，mIoU提升5.0%，在保持性能的同时减少40%能耗，无需重新训练即可增强鲁棒性

Conclusion: LiDAS将现有头灯转变为主动视觉执行器，为鲁棒的夜间感知提供了经济有效的解决方案，补充了领域泛化方法
...
<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型在文本中心图像修复任务中容易产生文本幻觉，因为它们缺乏显式的语言知识。需要一种能够整合语言指导的方法来准确恢复退化的文本内容。

Method: 提出UniT框架，包含三个组件：1) Diffusion Transformer (DiT)作为主干网络；2) Vision-Language Model (VLM)从退化图像提取文本内容提供指导；3) Text Spotting Module (TSM)在去噪过程中生成中间OCR预测，使VLM能够迭代细化指导。

Result: 在SA-Text和Real-Text基准测试中，UniT能够忠实重建退化文本，显著减少文本幻觉，并在TAIR任务中实现了最先进的端到端F1分数性能。

Conclusion: UniT通过整合扩散模型、视觉语言模型和文本检测模块的迭代协作，有效解决了文本图像修复中的文本幻觉问题，实现了高质量的文本恢复。
...
Method: 采用统一Transformer架构，通过新颖查询机制独立灵活探测时空任意点的3D位置，避免密集逐帧解码和多任务解码器的复杂性，实现轻量级设计。

Result: 在广泛的4D重建任务中达到新的最先进水平，同时实现高效的训练和推理，模型轻量且可扩展性强。

Conclusion: D4RT通过统一架构和灵活查询机制，为动态场景的4D重建提供了高效、轻量且性能优越的解决方案，在多个任务上超越先前方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统NVS依赖显式3D归纳偏置和已知相机参数，而VGGT等视觉基础模型通过训练数据隐式学习3D知识，但缺乏显式的多视角几何一致性。研究发现改进3D特征一致性对NVS和姿态估计都有益。

Method: 提出Selfi自改进3D重建流程，通过特征对齐将VGGT主干转换为高保真3D重建引擎。使用轻量级特征适配器，基于重投影一致性损失训练，将VGGT输出蒸馏到新的几何对齐特征空间，捕捉3D空间邻近性。

Result: 在NVS和相机姿态估计任务上实现了最先进的性能，证明特征对齐对下游3D推理非常有益。

Conclusion: 特征对齐是提升3D推理性能的关键步骤，Selfi通过自改进流程有效提高了VGGT的3D特征一致性，为无标定图像处理提供了新思路。
...
TL;DR: 该研究对比了数据导向设计(DOD)与传统面向对象设计(OOD)在多线程环境下的性能表现，发现DOD在执行时间、系统调用和缓存未命中方面表现更优，但单线程版本在多线程版本中表现更好。
...
Result: 多线程测试中，DOD实现表现出显著性能优势：执行时间更快、系统调用更少、缓存未命中更低。OOD偶尔在内存使用或百分比缓存未命中率上有微弱优势，但DOD在数据密集型操作中效率更明显。对于A*这样的细粒度任务，线程管理开销导致单线程版本在两个范式中都显著优于多线程版本。
...
<details>
  <summary>Details</summary>
Motivation: 当前企业数据管理任务繁重，涵盖架构、集成、质量、治理等多个方面。虽然AI助手能帮助特定角色（如数据工程师、数据管家）操作数据堆栈，但远未实现完全自动化。随着AI能力提升，处理复杂任务成为可能，因此存在实现完全自主数据资产的机会。

Method: 提出范式转变：从AI在独立数据组件操作中的应用，转向更整体、自主地处理整个数据生命周期。探索现代数据堆栈每个阶段如何通过智能代理实现自主管理，构建不仅能供人类用户使用，也能供AI自身使用的自足系统。

Result: 论文主要提出概念框架和研究方向：1）描述推动范式转变的力量和机遇；2）探讨代理如何简化数据生命周期；3）指出需要进一步研究的开放问题和领域。

Conclusion: 呼吁实现数据系统的更自主未来，希望激发辩论、推动研究、促进协作方法，为数据系统向完全自主管理转型奠定基础。
...
TL;DR: SkipKV是一种无需训练的KV缓存压缩方法，通过句子级评分识别并移除高度相似的句子，同时调整隐藏激活状态来抑制冗余生成，在保持推理准确性的同时显著减少KV缓存和生成长度。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在链式思维推理过程中，KV缓存随推理过程线性增长，导致内存和吞吐量瓶颈。现有KV缓存逐出方法在多批次设置中因不稳定的token级评分和填充token导致的KV预算减少而失效，且会生成更长的序列。

Method: SkipKV是一种无需训练的句子级KV压缩方法：1）引入句子评分指标识别并移除高度相似的句子，保持语义连贯性；2）动态调整引导向量更新推理过程中的隐藏激活状态，强制模型生成简洁响应。

Result: 在多个推理基准测试中，SkipKV在相似压缩预算下比替代方法保持高达26.7%的准确性提升，同时生成长度减少1.6倍，吞吐量提高1.7倍。

Conclusion: SkipKV通过句子级序列移除和动态隐藏状态调整，有效解决了KV缓存增长问题，在保持推理准确性的同时显著提高了推理效率。
...
TL;DR: 开发了一个基于LLM的AI系统，用于辅助临床试验患者匹配，解决EHR数据整合、专家审查和安全标准等实施挑战。


<details>
  <summary>Details</summary>
Motivation: 目前临床试验患者筛选过程是手动、耗时且资源密集的，需要更高效、可扩展的解决方案来减轻协调员负担并提高匹配效率。

Method: 使用开源、具备推理能力的大型语言模型（LLMs），构建了一个安全、可扩展的概念验证系统，能够生成结构化资格评估和可解释的推理链，支持人机协同审查。

Result: 系统超越了传统的二元分类，将资格表示为动态状态而非固定判定，能够识别当前匹配并提供未来可能使患者符合资格的可操作建议。

Conclusion: 该系统旨在减轻协调员负担，智能扩展每位患者考虑的试验范围，并保证所有AI生成输出的全面可审计性，为临床试验匹配提供有效的决策支持工具。
...
TL;DR: 本研究对ChatGPT和DeepSeek两大LLM进行全面评估，通过技术分析、实证实验和用户调查，比较它们在教育研究领域的表现，发现ChatGPT在通用语言理解方面更优，而DeepSeek在编程任务上表现更佳。


<details>
  <summary>Details</summary>
Motivation: 预训练大语言模型在教育研究领域展现出巨大潜力，ChatGPT和DeepSeek作为当前最先进的LLM，在数学、科学、医学、文学和编程等方面都有强大能力。需要系统评估这两款模型在教育研究应用中的实际表现，了解它们在准确性、计算效率和用户体验方面的权衡。

Method: 采用三种方法：1) 背景技术分析；2) 实证实验，在文本生成、编程和专业问题解决等任务上进行基准测试；3) 真实用户调查，收集学生、教育工作者和研究人员的反馈意见。

Result: 实验结果显示：ChatGPT在通用语言理解和文本生成方面表现更优；DeepSeek凭借其效率导向的设计，在编程任务上表现更佳；两款模型都能提供医学上准确的诊断输出，并有效解决复杂的数学问题。用户调查进一步揭示了这些模型在实际应用中的益处和局限性。

Conclusion: ChatGPT和DeepSeek在教育研究领域各有优势，ChatGPT更适合通用语言任务，DeepSeek在编程方面表现突出。两款模型都能有效支持教育研究工作，但需要根据具体应用场景选择合适的模型。用户调查为理解这些模型的实际价值提供了重要见解。
...
TL;DR: 开发用于糖尿病预测移动应用的可扩展后端系统，采用水平扩展、数据库分片和异步消息队列，成功处理10,000并发用户，83%功能达到性能目标
...
Result: 83%的系统功能（24个中的20个）达到性能目标（故障率<5%，平均延迟<1000ms），能处理10,000并发用户，异步通信显著降低错误率

Conclusion: 该可扩展后端系统成功支持糖尿病预测移动应用，验证了架构设计的有效性，特别是异步通信在保证系统可靠性方面发挥关键作用
...
<details>
  <summary>Details</summary>
Motivation: 当前大型预训练模型在因果学习和建模方面存在困难，而认知科学中的因果贝叶斯网络形式化方法已成功应用于理解人类因果学习。研究者希望找到连接经典贝叶斯因果学习与强化学习的桥梁，以更好地理解人类因果学习机制并提升机器的因果学习能力。

Method: 提出"赋能"（empowerment）概念作为连接桥梁，通过实证研究系统测试儿童和成人如何使用赋能线索来推断因果关系并设计有效的因果干预措施。

Result: 研究显示赋能可以作为连接贝叶斯因果学习与强化学习的重要桥梁，既能解释儿童因果学习的特征，又能为因果学习提供更易处理的计算解释。

Conclusion: 赋能概念不仅有助于理解人类因果学习机制，特别是儿童的学习特征，还能为机器实现更有效的因果学习提供理论基础，通过建立准确因果世界模型与赋能之间的相互促进关系，推动因果学习研究的发展。
...
TL;DR: KPI框架通过知识图谱增强、原型感知和可解释性设计，解决仅基于患者信息进行疾病预测时的数据不平衡和可解释性不足问题，提高预测准确性和临床可靠性。
...
Method: 提出KPI框架：1）将结构化医学知识整合为统一疾病知识图谱；2）构建临床有意义的疾病原型；3）使用对比学习提高预测准确性（特别是长尾疾病）；4）利用大语言模型生成患者特定的医学相关解释。

Result: 在真实世界数据集上的实验表明，KPI在预测准确性上优于最先进方法，并能提供与患者叙述高度一致的临床有效解释，突显其在以患者为中心的医疗保健中的实用价值。

Conclusion: KPI框架通过整合医学知识、原型感知和可解释性设计，有效解决了疾病预测中的数据不平衡和可解释性问题，为患者为中心的医疗保健提供了实用解决方案。
...
TL;DR: 最新推理模型在CFA各级别考试中表现优异，多个模型通过所有级别，其中Gemini 3.0 Pro在Level I创下97.6%的最高分


<details>
  <summary>Details</summary>
Motivation: 先前研究表明大型语言模型在CFA考试中表现不佳，但近期推理模型在各种专业考试中取得突破，需要评估它们在CFA考试中的实际表现

Method: 使用980道模拟题（包括Level I三套、Level II两套、Level III三套）评估顶尖推理模型，采用与先前研究相同的通过/失败标准

Result: 大多数模型通过了所有三个级别，性能排名为：Gemini 3.0 Pro > Gemini 2.5 Pro > GPT-5 > Grok 4 > Claude Opus 4.1 > DeepSeek-V3.1。Gemini 3.0 Pro在Level I达97.6%，GPT-5在Level II达94.3%，Gemini 2.5 Pro在Level III选择题达86.4%，Gemini 3.0 Pro在Level III主观题达92.0%

Conclusion: 最新推理模型在CFA考试中表现出色，显著超越了先前LLM的糟糕表现，展示了推理模型在专业金融认证考试中的强大能力
...
TL;DR: 该研究提出使用生成式智能体来高效评估AI生成内容的质量，替代传统耗时耗力的人工评估方法，帮助企业实现自动化内容生成与评估。


<details>
  <summary>Details</summary>
Motivation: 现代企业在内容生成和评估上面临时间和成本挑战：人工写作受时间限制，外部评估成本高昂。虽然大语言模型在内容创作上有潜力，但AI生成内容的质量问题仍然存在，传统的人工评估方法进一步增加了运营成本，因此需要高效、自动化的解决方案。

Method: 引入生成式智能体（Generative Agents）来评估AI生成内容。这些智能体能够模拟人类判断，快速且经济地对内容的连贯性、趣味性、清晰度、公平性和相关性等方面进行评分。

Result: 通过使用生成式智能体，企业可以简化内容生成流程，确保一致的高质量输出，同时减少对昂贵人工评估的依赖。该方法为提升大语言模型生成符合业务需求的高质量内容提供了重要见解。

Conclusion: 生成式智能体为解决AI内容生成的质量评估问题提供了有效的自动化解决方案，在自动内容生成和评估方面取得了显著进展，有助于企业实现高效、高质量的内容生产。
...
TL;DR: 该研究提出了智能体系统的定量扩展原则，通过180个配置的实验发现工具协调权衡、能力饱和和拓扑依赖错误放大三个主导效应，能够预测87%配置的最优协调策略。
...
Method: 在四个多样化基准测试（Finance-Agent、BrowseComp-Plus、PlanCraft、Workbench）上评估五种典型架构（单智能体、独立、集中式、分散式、混合式），使用三个LLM家族实例化，进行180个配置的受控评估，使用标准化工具和令牌预算，通过经验协调指标建立预测模型。

Result: 预测模型达到交叉验证R^2=0.513，识别出三个主导效应：工具协调权衡、能力饱和（β=-0.408，p<0.001）和拓扑依赖错误放大。集中式协调在可并行任务上提升80.9%性能，分散式协调在动态网络导航上表现更佳（+9.2% vs +0.2%），但所有多智能体变体在顺序推理任务上降低39-70%性能。框架能预测87%保留配置的最优协调策略。

Conclusion: 该研究为智能体系统提供了基于可测量任务特性的预测性扩展原则，能够指导实践者选择最优协调策略，填补了智能体系统性能决定原则的研究空白。
...
TL;DR: 本文提出了一种强化策略注入机制(rSIM)，通过小型规划器引导LLM的思维链，使任何LLM都能进化为推理语言模型(RLM)。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLM)通过强化学习后训练进化为推理语言模型(RLM)，其标志性特征是在思维链中表现出"顿悟"时刻，如自我反思和深度思考等策略。受此启发，本文旨在开发一种机制，使任何LLM都能获得这种高级推理能力。

Method: 提出强化策略注入机制(rSIM)，使用小型规划器（领导者智能体）引导LLM（跟随者智能体）的思维链，通过自适应注入推理策略。基于领导者-跟随者框架，采用多智能体强化学习(MARL)联合训练规划器和LLM，使用简单的基于规则的奖励机制。

Result: 实验结果显示，rSIM使Qwen2.5-0.5B进化为RLM，性能显著超过Qwen2.5-14B。规划器具有泛化性：只需训练一次即可作为插件大幅提升现有LLM的推理能力。规划器还支持跨任务的持续学习，其规划能力能逐步提升并泛化到更广泛的问题。

Conclusion: rSIM机制通过小型规划器引导LLM的思维链，成功使任何LLM都能进化为具有高级推理能力的RLM。该方法具有泛化性、可插拔性和持续学习能力，为提升LLM推理能力提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 传统的CBR实验室测试方法虽然准确，但耗时、成本高，且在大规模或多样化土壤剖面中不实用。需要开发更快速、精确的预测方法来替代传统方法，促进岩土工程的数字化转型。

Method: 收集土耳其不同地理气候区域的382个土壤样本，提取与承载能力相关的物理化学特性作为特征。测试了12种机器学习算法，包括决策树、随机森林、梯度提升、XGBoost、K近邻、支持向量回归、多层感知器、AdaBoost、Bagging、投票和堆叠回归器。对每个模型进行训练、验证和评估。

Result: 随机森林回归器表现最佳，在训练集上R²=0.95，验证集上R²=0.76，测试集上R²=0.83。这表明模型具有强大的非线性映射能力，能够有效预测CBR值。

Conclusion: 机器学习框架为CBR预测提供了一种高效的数据驱动方法，可作为传统实验室测试的有效替代方案。支持智能、数据中心的模型在岩土工程中的集成，促进基础设施分析和设计的数字化转型。
...
TL;DR: 本研究提出一种AutoML方法预测土壤最佳含水量和最大干密度，XGBoost算法表现最佳，R²分别达89.1%和80.4%，证明AutoML能有效预测不同土壤类型的压实参数。


<details>
  <summary>Details</summary>
Motivation: 传统确定土壤最佳含水量和最大干密度的方法实验繁琐，经验回归模型在不同土壤类型中适用性和准确性有限。现有机器学习模型在预测精度和泛化能力方面存在不足，特别是处理异质数据集时。

Method: 采用自动化机器学习方法，自动选择算法并优化超参数，通过广泛实验评估不同算法性能，最终确定极端梯度提升算法为最佳模型。

Result: XGBoost算法在独立数据集上表现最佳，最大干密度预测R²达80.4%，最佳含水量预测R²达89.1%。研究还发现异质数据集对提升模型泛化能力和性能至关重要。

Conclusion: AutoML方法能有效预测不同土壤类型的压实参数，提高预测精度和可扩展性。异质数据集对机器学习模型的泛化性能有重要影响，该研究有助于提升建筑工程实践的效率和可靠性。
...
TL;DR: 提出一个针对图神经网络的新型可解释AI框架，旨在提供适应性强、计算高效的解释，超越单个特征分析，捕捉图结构如何影响预测


<details>
  <summary>Details</summary>
Motivation: 图神经网络在众多应用中广泛使用，但其复杂性阻碍了理解其决策过程。现有的可解释AI方法难以解开图中复杂的交互关系，后处理方法需要额外计算资源且可靠性有限，而可解释模型又缺乏泛化能力

Method: 开发一个专门针对图机器学习的新型XAI框架，该框架旨在提供适应性强、计算高效的解释，不仅分析单个特征，还能捕捉图结构对预测的影响

Result: 论文提出了一种新的框架设计，但具体实验结果未在摘要中说明

Conclusion: 需要开发专门针对图神经网络的新型可解释AI框架，以解决现有方法的局限性，提供更可靠、高效且能捕捉图结构影响的解释
...
TL;DR: 使用LLM多智能体系统模拟职场毒性对话，通过蒙特卡洛方法量化毒性对决策效率的影响，发现毒性参与者使对话时间增加约25%


<details>
  <summary>Details</summary>
Motivation: 职场毒性对组织文化有害，但量化其对运营效率的直接影响存在方法论挑战，因为伦理和实践上难以在人类受试者中复制冲突

Method: 利用基于LLM的多智能体系统模拟1对1对抗性辩论，创建受控的"社会学沙盒"。采用蒙特卡洛方法模拟数百次讨论，测量基线对照组与包含"毒性"系统提示的处理组之间的收敛时间
...
TL;DR: DuSAR是一个无需演示的LLM智能体框架，通过双策略协同推理（高层整体规划与上下文局部策略）和轻量级反思机制，在ALFWorld和Mind2Web任务上实现SOTA性能，同时大幅降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体依赖外部演示或检索增强规划，导致脆弱性、泛化能力差和计算开销高。受人类问题解决启发，需要开发无需演示、计算高效的智能体框架。

Method: 提出DuSAR框架：1）双策略协同推理：高层整体规划与上下文局部策略互补；2）轻量级反思机制：通过策略适应度评分动态评估进展，在受阻时修订全局计划或在有意义进展时细化计划；3）支持可选集成专家演示。

Result: 在ALFWorld上达到37.1%成功率（Llama3.1-70B），比之前最佳结果13.0%提高一倍以上；在Mind2Web上达到4.02%，同样比最强基线提高一倍以上；每步token消耗减少3-9倍；消融研究证实双策略协调的必要性。

Conclusion: DuSAR通过双策略协同推理和反思机制，实现了无需演示的高效LLM智能体，在多个基准上取得SOTA性能，显著降低计算成本，并保持与外部知识集成的灵活性。
...
<details>
  <summary>Details</summary>
Motivation: 现有可穿戴设备生物信号的特征提取方法缺乏任务特定的上下文知识，在高维特征空间中难以找到最优特征提取设置，且容易产生代码生成和自动化错误
...
<details>
  <summary>Details</summary>
Motivation: 机器人领域基于模型的规划面临混合动力学的挑战——连续运动与离散事件（如接触、碰撞）交织。传统单一神经网络模型强制全局连续性，会过度平滑不同的动态模式（如粘滞vs滑动、飞行vs站立），导致规划器在物理边界处产生灾难性的累积误差。

Method: 提出PRISM-WM（棱镜世界模型），采用结构化架构将复杂混合动力学分解为可组合的原语。使用上下文感知的混合专家框架，其中门控机制隐式识别当前物理模式，专门化的专家预测相应的转移动态。引入潜在正交化目标确保专家多样性，有效防止模式崩溃。

Result: PRISM-WM能准确建模系统动力学中的急剧模式转换，显著减少滚动漂移。在具有挑战性的连续控制基准测试（包括高维人形机器人和多样化多任务设置）中，PRISM-WM为轨迹优化算法（如TD-MPC）提供了优越的高保真度基础，证明其作为下一代基于模型代理的强大基础模型的潜力。

Conclusion: PRISM-WM通过结构化分解混合动力学，解决了传统世界模型在物理边界处的平滑问题，为长时域规划提供了更可靠的高保真度模型基础，有望推动基于模型的强化学习发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI系统在高风险领域（医疗、金融、公共政策）部署时，主要关注技术性能指标而忽视社会技术维度，导致AI行为与人类价值观的对齐问题。需要从模型中心转向影响中心的AI开发方法。

Method: 提出Impact-Driven AI Framework (IDAIF)，将变革理论的五阶段模型（输入-活动-输出-成果-影响）映射到AI架构的五层（数据层-管道层-推理层-代理层-规范层）。每层采用特定技术：多目标帕累托优化、分层多代理编排、因果有向无环图、对抗性去偏和RLHF，并引入管理假设失败的保障层。

Result: 提供了每个组件的正式数学公式，并在医疗、网络安全和软件工程三个领域进行了案例研究，展示了框架的实际应用。
...
TL;DR: 研究通过强化学习智能体控制旋转圆柱体在水流中的阻力，发现学习高性能技能需要比执行技能更丰富的信息反馈，且学习条件的好坏仅取决于目标而非系统动力学或策略复杂性。


<details>
  <summary>Details</summary>
Motivation: 研究人类在无外部反馈情况下执行高性能技能（如花样滑冰、棒球投球）的机制，通过可控实验环境探索技能获取过程，避免人类实验的复杂性。

Method: 使用通用强化学习智能体直接控制桌面循环水槽中的旋转圆柱体，通过最大化或最小化阻力作为目标。实验设置包含高维流体反馈，并与无反馈条件进行对比。

Result: 高维流体反馈使智能体在几分钟内发现高性能阻力控制策略；无反馈时仍能执行相同策略。无反馈训练时，阻力最大化任务完全失败，阻力最小化任务虽能成功但更慢更不可靠。

Conclusion: 学习高性能技能需要比执行技能更丰富的信息反馈；学习条件的好坏仅取决于目标（阻力最小化vs最大化），而非系统动力学或策略复杂性。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在函数级代码生成方面取得进展，但仓库级自动程序修复(APR)仍面临挑战。现有方法采用控制中心范式，迫使代理处理复杂的目录结构和无关的控制逻辑，存在"语义陷阱"问题。

Method: 1. 从标准代码属性图(CPGs)转向数据转换图(DTG)范式，将数据状态作为节点、函数作为边；2. 提出多代理框架，协调数据完整性导航与控制流逻辑；3. 实现自主问题解决器(AIR)，利用神经符号推理和DTG结构进行可扩展逻辑修复。

Result: 在多个SWE基准测试中表现良好，在SWE-Verified基准上达到87.1%的解决率，直接解决了当前AI代码助手工具的核心限制。

Conclusion: DTG方法通过数据谱系追踪逻辑缺陷，解决了现代编码代理中RAG系统的"语义陷阱"，为日益依赖软件的世界提供了更强大的基础，实现了零接触代码维护的自主改进系统。
...
TL;DR: 提出了一种轻量级迁移学习方法CITL，用于移动设备中锂离子电池的健康状态监测，通过半监督机制和网络节点增量构建，在减少计算资源消耗的同时提高监测精度。


<details>
  <summary>Details</summary>
Motivation: 移动设备中锂离子电池的健康状态监测面临工作条件多变的问题，传统迁移学习方法在移动设备上不可行，因为迁移学习阶段消耗大量计算资源，会意外降低设备的工作续航能力。

Method: 提出构造性增量迁移学习方法CITL：1）利用目标域未标记数据，提出半监督迁移学习机制，通过迭代添加网络节点最小化监测残差；2）通过结构风险最小化、迁移失配最小化和流形一致性最大化保证节点参数的跨域学习能力；3）提供CITL收敛性分析。

Result: 在真实无人机电池数据集上的实验表明，CITL在SOH估计方面优于SS-TCA、MMD-LSTM-DA、DDAN、BO-CNN-TL和AS$^3$LSTM，均方根误差分别降低了83.73%、61.15%、28.24%、87.70%和57.34%。

Conclusion: CITL方法有效解决了移动设备中电池健康状态监测的计算资源限制问题，通过轻量级迁移学习实现了高精度监测，为便携移动设备的电池管理提供了可行解决方案。
...
TL;DR: Principles2Plan：一个通过人-LLM协作生成情境化伦理规则以指导自动化规划的原型系统
...
Method: 开发交互式原型系统Principles2Plan，让领域专家提供规划领域、问题细节和高级伦理原则（如仁慈、隐私），系统使用LLM生成可操作的伦理规则，用户可审查、优先排序这些规则，然后提供给规划器生成伦理感知的计划。

Result: 创建了首个支持用户在经典规划情境中生成基于原则的伦理规则的系统，展示了人-LLM协作在使伦理自动化规划更实用可行方面的潜力。

Conclusion: Principles2Plan证明了人-LLM协作在生成情境敏感伦理规则方面的价值，为更实用的伦理自动化规划提供了新途径。
...
TL;DR: 本文提出了SMART+框架，这是一个基于安全、监控、问责、可靠性和透明度支柱的结构化模型，用于评估和治理跨行业AI系统，特别关注临床研究领域。


<details>
  <summary>Details</summary>
Motivation: AI系统已广泛应用于医疗、金融、制造等多个行业，虽然提升了运营效率，但也带来了安全、问责和监管合规等新挑战。需要建立一个综合框架来应对这些挑战，确保AI系统的负责任采用。

Method: 提出SMART+框架，该框架建立在安全、监控、问责、可靠性和透明度五大支柱基础上，并进一步增强了隐私与安全、数据治理、公平性与偏见、防护措施等要素。

Result: SMART+框架提供了一个实用、全面的方法来评估和治理跨行业AI系统，能够实现风险缓解、信任建立和合规准备，为临床研究等领域的有效AI治理提供了坚实基础。

Conclusion: SMART+框架通过整合运营保障、监督程序以及强化的隐私和治理控制，与不断发展的机制和监管指南保持一致，能够支持负责任的AI采用并确保可审计性。
...
TL;DR: CogMCTS：一种新颖的认知引导MCTS框架，通过多轮认知反馈、双轨节点扩展和策略突变，将LLM认知指导与MCTS紧密结合，实现高效自动启发式优化。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的进化方法容易陷入局部最优，而LLM与MCTS结合的方法在多轮认知整合和搜索多样性方面仍有局限。需要克服这些限制，实现更高效的自动启发式设计。
...
Conclusion: CogMCTS通过紧密集成LLM认知指导与MCTS，有效解决了自动启发式设计中的探索-利用权衡问题，提升了优化性能。
...
TL;DR: 提出基于Transformer的蛋白质二级结构预测模型，通过注意力机制处理氨基酸序列，使用滑动窗口数据增强技术，在CB513数据集上表现优异


<details>
  <summary>Details</summary>
Motivation: 蛋白质二级结构（如α螺旋、β折叠、无规则卷曲）的预测对于理解蛋白质功能至关重要，需要开发能够有效处理氨基酸序列并捕获残基间相互作用的模型

Method: 使用基于Transformer的模型，应用注意力机制处理蛋白质序列数据；采用滑动窗口数据增强技术在CB513数据集上扩展训练样本；模型能够处理变长序列并有效捕获局部和长程残基相互作用

Result: Transformer模型展现出强大的泛化能力，能够处理不同长度的蛋白质序列，同时有效捕获残基间的局部和长程相互作用

Conclusion: 基于Transformer的模型是预测蛋白质二级结构的有效方法，能够从氨基酸序列中学习复杂的结构模式，为蛋白质功能研究提供有力工具
...
TL;DR: See-Control 是一个通过低自由度机械臂直接物理交互操作智能手机的框架，解决了现有方法依赖ADB仅适用于Android设备的问题，提出了ESO任务基准和平台无关的解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有基于MLLM的智能手机操作智能体依赖Android Debug Bridge进行数据传输和动作执行，这限制了它们仅适用于Android设备，缺乏平台无关的解决方案。

Method: 提出See-Control框架，包含三个核心组件：1) ESO基准测试集（155个任务及评估指标）；2) 基于MLLM的具身智能体，无需ADB或系统后端访问即可生成机器人控制命令；3) 丰富标注的操作数据集。

Result: 通过低自由度机械臂实现智能手机的物理交互操作，提供了平台无关的解决方案，弥合了数字智能体与物理世界之间的鸿沟。
...
<details>
  <summary>Details</summary>
Motivation: 胃肠道肿瘤学中的多模态临床推理需要整合内镜图像、放射学数据和生化标志物。尽管多模态大语言模型(MLLMs)显示出潜力，但在处理复杂、异质的医疗历史时经常面临上下文稀释和幻觉等挑战。

Method: 提出分层多智能体框架，模拟人类多学科团队(MDT)的协作工作流程，通过多智能体协同解决单模型在处理复杂医疗信息时的局限性。

Result: 系统获得4.60/5.00的专家评估分数，相比单一基线模型有显著改进。智能体架构在推理逻辑和医学准确性方面提升最为明显。

Conclusion: 模拟性的智能体协作为肿瘤学中的自动化决策支持提供了可扩展、可解释且临床稳健的范式，能够有效处理复杂的多模态医疗数据。
...
TL;DR: 提出"人机协作认知增强"新范式，通过结构化"元交互"将人类专家的"认知黑箱"和AI的"计算黑箱"转变为可组合、可审计、可扩展的"功能白箱"系统。


<details>
  <summary>Details</summary>
Motivation: 解决人类专家直觉（认知黑箱）与AI决策不可信（计算黑箱）之间的根本鸿沟，实现从"AI作为工具"到"AI作为思维伙伴"的范式转变。

Method: 提出"即插即用认知框架"——可从专家对话中提取的可计算知识包，加载到递归对抗元思维网络（RAMTN）中，通过结构化"元交互"实现认知转化。

Result: 实现了专家思维（如医疗诊断逻辑、教学直觉）向可重用、可扩展的公共资产的转化，为"认知公平"提供了首个工程证明，并开辟了通过"交互协议透明化"而非窥探模型内部机制的新型AI治理路径。

Conclusion: 该框架开源以促进技术向善和认知包容，实现了从双黑箱到功能白箱的转变，为人机协作认知增强提供了可行的工程路径和治理范式。
...
TL;DR: 论文指出基础模型作为AI智能体的"大脑"，当前研究主要关注单智能体能力，但下一代前沿应赋予基础模型原生多智能体智能，包括理解、规划、高效通信和适应四大核心能力。


<details>
  <summary>Details</summary>
Motivation: 虽然基础模型正成为AI智能体的核心，但现有研究主要集中于单智能体能力（如GUI交互、工具使用）。作者认为下一代前沿应是赋予基础模型原生多智能体智能，因为强大的单智能体性能并不能自动转化为稳健的多智能体智能。

Method: 通过41个大语言模型的广泛实证研究，验证了单智能体性能与多智能体智能之间的差距。提出了构建原生多智能体智能基础模型的关键研究方向，包括数据集构建、评估方法、训练范式和安全考虑。

Result: 实证研究表明，强大的单智能体性能并不能自动产生稳健的多智能体智能，揭示了当前基础模型在多智能体环境中的能力差距。

Conclusion: 需要系统性地研究如何赋予基础模型原生多智能体智能，包括理解、规划、通信和适应四大核心能力，并提出了具体的研究方向来解决这一重要挑战。
...
TL;DR: 本文比较了无人机搭载的RIS和STAR-RIS在三维无线环境中的性能，发现STAR-RIS在低空场景表现更好，而RIS在基站附近的高空场景更优。


<details>
  <summary>Details</summary>
Motivation: 可重构智能表面(RIS)和同时透射反射RIS(STAR-RIS)是提升下一代网络覆盖和容量的关键技术。虽然无人机搭载这些技术具有部署灵活和视距条件改善的优势，但缺乏对空中RIS和STAR-RIS架构的全面性能比较研究。

Method: 建立了包含定向辐射模式的精确信道模型，并深入研究了部署高度和方向的影响。为优化系统和速率，为两种架构制定了联合优化问题，并提出了基于加权最小均方误差和块坐标下降算法的高效解决方案。

Result: 仿真结果表明：STAR-RIS在低空场景中优于RIS，因其具有全空间覆盖能力；而RIS在基站附近的高空场景中表现更好。部署高度和方向对系统性能有显著影响。

Conclusion: 研究结果为未来6G通信系统中空中智能表面的部署提供了实用见解，表明应根据具体场景需求选择合适的RIS架构，STAR-RIS适合低空全空间覆盖，而RIS适合高空定向增强。
...
TL;DR: 本文提供了构建生产级智能体AI工作流的端到端实用指南，涵盖设计、开发和部署全流程，提出了9个核心最佳实践，并通过多模态新闻分析案例展示应用。


<details>
  <summary>Details</summary>
Motivation: 随着智能体AI在各行业的加速应用，组织面临如何设计、构建和运维生产级智能体工作流的挑战，需要确保其可靠性、可观测性、可维护性，并符合安全和治理要求。

Method: 提出结构化工程生命周期，包括工作流分解、多智能体设计模式、模型上下文协议(MCP)和工具集成、确定性编排、负责任AI考虑、环境感知部署策略，并提出了9个核心最佳实践。

Result: 提供了构建生产级智能体AI工作流的完整框架，包括工具优先设计、纯函数调用、单一职责智能体、外部化提示管理、负责任AI联盟设计等具体实践，并通过多模态新闻分析案例验证了方法的有效性。

Conclusion: 本文为构建稳健、可扩展、生产就绪的智能体AI工作流提供了基础性参考，结合架构指导、操作模式和实际实现洞察，帮助组织应对智能体AI系统工程化的挑战。
...
TL;DR: CARLoS是一个大规模框架，用于在没有额外元数据的情况下表征LoRAs，通过分析650多个LoRAs的图像生成行为，定义了方向、强度和一致性的三部分表示，并开发了高效的检索系统。


<details>
  <summary>Details</summary>
Motivation: 生成组件（如LoRAs）的快速扩散创建了一个庞大但非结构化的生态系统。现有的发现方法依赖于不可靠的用户描述或有偏见的流行度指标，阻碍了可用性。

Method: 分析650多个LoRAs，在各种提示和种子下进行图像生成以评估其行为。使用CLIP嵌入及其与基础模型生成的差异，定义了三个部分的表示：方向（定义语义偏移）、强度（量化效果显著性）和一致性（量化效果的稳定性）。

Result: 开发了高效的检索框架，能够语义匹配文本查询到相关LoRAs，同时过滤过强或不稳定的LoRAs，在自动和人工评估中优于文本基线。该表示还支持分析强度和一致性与版权法律概念（实质性、意愿）的关联。

Conclusion: CARLoS是一个实用的系统，不仅专注于检索，其表示方法对LoRA分析具有更广泛的相关性，特别是与版权法律概念的关联。
...
TL;DR: 论文探讨描述逻辑和逻辑编程中Craig插值与均匀插值的理论结果与计算方法


<details>
  <summary>Details</summary>
Motivation: Craig插值和均匀插值在知识表示中有重要应用（可解释性、遗忘、模块化、重用和学习），但许多相关形式化方法缺乏这些插值性质，且实际计算插值具有挑战性

Method: 聚焦描述逻辑和逻辑编程两种知识表示形式化方法，分析理论结果并讨论实际计算插值的实用方法

Result: 论文综述了两种形式化方法中插值的理论性质，并提供了计算插值的实用方法

Conclusion: 虽然描述逻辑和逻辑编程中插值计算具有挑战性，但通过理论分析和实用方法可以推进知识表示中插值技术的应用
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试大多关注学术场景或人工设计的环境，忽视了实际应用中的挑战。电商领域具有大量用户交互、动态市场条件和真实决策任务，是评估智能体实际能力的理想场景。

Method: 从全球领先电商生态系统的真实用户需求中构建基准，通过人类专家精心策划和标注，确保清晰性、准确性和领域相关性。涵盖电商场景中的多个任务类别，并定义了三个难度级别。

Result: EcomBench提供了一个严格且动态的测试平台，能够在真实电商环境中评估智能体的实际能力，特别是深度信息检索、多步推理和跨源知识整合等关键能力。

Conclusion: 通过将评估建立在真实电商环境中，EcomBench为衡量智能体在现代电商中的实际能力提供了一个严谨的基准测试，填补了现有评估方法在真实应用场景中的空白。
...
TL;DR: 该论文提出了REST和REST+两个新基准，用于系统评估多模态大语言模型的跨模态不一致性问题，发现即使语义信息相同，模型在不同模态（图像、文本、混合）上的推理表现也不一致。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型被训练在同一个嵌入空间中表示视觉和语言信息，但它们无法在两种模态上执行相同的任务。现有评估方法缺乏对跨模态一致性的系统性测试，需要新的基准来揭示这一重要问题。

Method: 创建了REST和REST+基准，包含三种模态（图像、文本、混合）下相同语义信息的样本。评估了15个最先进的多模态大语言模型，分析了OCR准确性、视觉特征（文本颜色、分辨率、字体）和视觉标记数量对模型性能的影响。

Result: 研究发现：1）所有评估的MLLMs都存在跨模态不一致问题；2）将文本渲染为图像或将图像渲染为文本都无法解决不一致性；3）视觉特征（文本颜色和分辨率）和视觉标记数量影响模型性能；4）一致性得分与文本和图像之间的模态差距相关。

Conclusion: 多模态大语言模型存在显著的跨模态不一致问题，这不仅仅是OCR识别问题。REST基准为评估和改善MLLMs的跨模态一致性提供了重要工具，一致性得分与模态差距的相关性为理解模型机制提供了新视角。
...
### [149] [ThreadWeaver: Adaptive Threading for Efficient Parallel Reasoning in Language Models](https://arxiv.org/abs/2512.07843)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在推理任务中表现出色，但顺序解码导致高延迟。现有并行推理方法要么局限于监督行为克隆，要么准确率显著下降，且需要定制化推理引擎，部署复杂。

Method: 1) 两阶段并行轨迹生成器生成大规模高质量带并行标注的CoT数据用于监督微调；2) trie训练推理协同设计，无需修改位置嵌入或KV缓存即可在现成自回归推理引擎上实现并行推理；3) 并行感知强化学习框架，教导模型在准确率和有效并行化之间取得平衡。

Result: 在六个数学推理基准测试中，基于Qwen3-8B训练的ThreadWeaver达到与先进顺序推理模型相当的准确率（平均71.9%，AIME24上79.9%），同时实现最高1.53倍的平均token延迟加速，建立了准确率与效率之间的新帕累托前沿。

Conclusion: ThreadWeaver框架成功实现了自适应并行推理，在保持准确率的同时显著提升推理效率，且无需定制化推理引擎，为高效LLM推理提供了实用解决方案。
...
### [150] [Space Alignment Matters: The Missing Piece for Inducing Neural Collapse in Long-Tailed Learning](https://arxiv.org/abs/2512.07844)
*Jinping Wang,Zhiqiang Gao,Zhiwu Xie*

Main category: cs.LG

TL;DR: 该论文针对长尾分布中特征与分类器权重空间不对齐的问题，提出三种显式对齐策略，显著提升了现有长尾方法的性能。


<details>
  <summary>Details</summary>
Motivation: 在长尾分布下，严重的样本不平衡阻碍了神经坍缩现象的出现，导致泛化性能差。现有方法主要关注恢复ETF几何结构，但忽略了特征空间与分类器权重空间之间的显著不对齐问题。

Method: 通过最优误差指数分析理论量化不对齐的危害，提出三种显式的对齐策略，这些策略可以即插即用地集成到现有长尾方法中，无需改变网络架构。

Result: 在CIFAR-10-LT、CIFAR-100-LT和ImageNet-LT数据集上的大量实验表明，提出的对齐策略能够持续提升基准方法的性能，并达到最先进的性能水平。

Conclusion: 特征与分类器权重空间的对齐是长尾学习中的关键问题，提出的三种显式对齐策略有效解决了这一问题，显著提升了模型在长尾分布下的泛化性能。
...
### [151] [CarBench: A Comprehensive Benchmark for Neural Surrogates on High-Fidelity 3D Car Aerodynamics](https://arxiv.org/abs/2512.07847)
*Mohamed Elrefaie,Dule Shu,Matt Klenk,Faez Ahmed*

Main category: cs.LG

TL;DR: CarBench是首个针对大规模3D汽车空气动力学仿真的标准化基准测试，在包含8000多个高保真仿真的DrivAerNet++数据集上评估了11种最先进模型，涵盖神经算子、几何深度学习、Transformer求解器和隐式场网络等多种架构。


<details>
  <summary>Details</summary>
Motivation: 尽管大规模CFD数据集为机器学习在空气动力学和工程设计中的应用提供了新机会，但工程设计中缺乏大规模数值仿真的标准化基准。现有计算机视觉和NLP领域的基准测试推动了算法创新，而工程领域需要类似的标准化评估框架来加速数据驱动工程的发展。

Method: 在DrivAerNet++（最大的公共汽车空气动力学数据集，包含8000多个高保真仿真）上评估11种架构：神经算子方法（如傅里叶神经算子）、几何深度学习（PointNet、RegDGCNN、PointMAE、PointTransformer）、基于Transformer的神经求解器（Transolver、Transolver++、AB-UPT）和隐式场网络（TripNet）。除了标准插值任务外，还进行跨类别实验，评估在单一车型上训练的Transformer求解器在未见类别上的表现。

Result: 基准测试涵盖了预测准确性、物理一致性、计算效率和统计不确定性等多个维度。通过自举重采样提供不确定性估计，开源了包括训练流程、不确定性估计例程和预训练模型权重的完整框架，为高保真CFD仿真的大规模学习建立了首个可复现的基础。

Conclusion: CarBench填补了工程设计中大规模数值仿真标准化基准的空白，为数据驱动工程提供了首个可复现的评估框架，将加速高保真CFD仿真机器学习方法的发展，推动空气动力学和工程设计的创新。
...
### [152] [RaX-Crash: A Resource Efficient and Explainable Small Model Pipeline with an Application to City Scale Injury Severity Prediction](https://arxiv.org/abs/2512.07848)
*Di Zhu,Chen Xie,Ziwei Wang,Haoyun Zhang*

Main category: cs.LG

TL;DR: RaX-Crash是一个用于纽约市机动车碰撞伤害严重程度预测的资源高效、可解释的小模型管道，整合多表数据，使用树集成模型（XGBoost和随机森林）显著优于小型语言模型，并保持可解释性。


<details>
  <summary>Details</summary>
Motivation: 纽约市每年发生超过10万起机动车碰撞事故，造成重大伤害和公共卫生负担。需要开发资源高效且可解释的模型来预测伤害严重程度，以支持城市规模的事故分析。

Method: 整合三个关联表（数千万条记录），构建统一特征模式的分区存储，训练紧凑的树集成模型（随机森林和XGBoost），并与使用文本摘要提示的小型语言模型（SLMs）进行比较。采用SHAP归因进行可解释性分析。

Result: XGBoost和随机森林在时间保留测试集上分别达到0.7828和0.7794的准确率，明显优于SLMs（0.594和0.496）。类别不平衡分析显示简单类别加权可提高致命事故召回率，SHAP归因突出人类脆弱性因素、时间和地点是预测严重程度的主要驱动因素。
...
### [153] [SABER: Small Actions, Big Errors -- Safeguarding Mutating Steps in LLM Agents](https://arxiv.org/abs/2512.07850)
*Alejandro Cuadron,Pengfei Yu,Yang Liu,Arpit Gupta*

Main category: cs.LG

TL;DR: 研究发现LLM代理在长时程工具使用任务中，突变性操作（改变环境的动作）的偏差对失败影响最大，而非突变性操作偏差影响很小。作者提出了CM框架来增强代理鲁棒性，并发布了改进的τ-Bench Verified基准。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM代理发展迅速，但在长时程工具使用任务上的表现仍然脆弱。为了理解这种脆弱性，研究者想要探究：所有动作对失败的影响是否相同？特别是突变性操作（改变环境的动作）与非突变性操作的影响差异。

Method: 1) 分析τ-Bench（航空/零售）和SWE-Bench Verified的执行轨迹，将轨迹分解为突变性步骤和非突变性步骤；2) 形式化"决定性偏差"概念（最早将成功翻转为失败的动作级分歧）；3) 使用逻辑回归分析偏差影响；4) 提出CM框架：突变门控验证、突变步骤前的针对性反思、基于块的上下文清理。

Result: 1) 每个突变性动作的额外偏差使成功几率降低：航空任务92%，零售任务96%（SoTA模型）；非突变性动作偏差影响很小；2) 上下文长度增加时错误增多；3) CM框架带来显著提升：Qwen3-Thinking在航空任务相对提升28%，零售11%，SWE-Bench Verified 7%；Claude提升9%/7%；4) 发现τ-Bench存在天花板效应，发布τ-Bench Verified恢复基准空间。

Conclusion: 突变性操作偏差是LLM代理失败的主要因素。CM框架通过针对性保护措施有效提升代理鲁棒性。需要动作级分析、针对性保护措施和可靠评估作为稳健多轮代理的前提条件。
...
### [154] [GPU Memory Prediction for Multimodal Model Training](https://arxiv.org/abs/2512.07853)
...
Method: 提出一个框架，通过分析多模态模型的架构和训练行为来预测GPU峰值内存使用量。具体方法是将多模态模型分解为其组成层，并应用因子化方法来估计每层的内存使用量。

Result: 评估显示该框架实现了约8.7%的平均MAPE（平均绝对百分比误差），具有较高的预测准确性。

Conclusion: 该框架能够准确预测多模态模型的GPU峰值内存使用量，有助于防止内存溢出错误，提高训练过程的效率和资源利用率，特别适用于智能体AI系统中常见的复杂多模态模型。
...
### [155] [HSTMixer: A Hierarchical MLP-Mixer for Large-Scale Traffic Forecasting](https://arxiv.org/abs/2512.07854)
...
<details>
  <summary>Details</summary>
Motivation: 现有交通预测模型在大规模场景下存在二次计算复杂度问题，难以应用于现实世界复杂交通网络，需要更高效的大规模预测方法

Method: 提出分层时空混合器(HSTMixer)框架，采用全MLP架构，包含分层时空混合块进行多分辨率特征提取（自底向上聚合和自顶向下传播），以及自适应区域混合器根据区域语义生成变换矩阵，动态捕捉不同区域的时空模式

Result: 在四个大规模真实世界数据集上的实验表明，该方法不仅达到最先进的性能，而且展现出有竞争力的计算效率
...
TL;DR: LAPA提出了一种基于对数域注意力预测的算法-架构协同设计，通过消除昂贵乘法运算和减少累加开销，实现跨阶段稀疏Transformer加速。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在不同阶段的计算瓶颈随输入序列长度变化而动态变化，需要跨阶段稀疏加速策略。现有稀疏Transformer方法多为单阶段设计，其稀疏性预测机制在多阶段应用时会产生显著功耗开销。

Method: 1. 设计非对称前导一计算(ALOC)方案消除昂贵乘法运算；2. 提出混合精度多轮移位累加(MRSA)机制减少累加开销；3. 设计数据特征依赖滤波器(DDF)与MRSA协同工作；4. 设计专用加速器将理论优势转化为实际硬件改进。

Result: LAPA相比现有最先进方法Spatten、Sanger和FACT，能效分别提升3.52倍、3.24倍和2.79倍。
...
### [157] [Medical Test-free Disease Detection Based on Big Data](https://arxiv.org/abs/2512.07856)
*Haokun Zhao,Yingzhe Bai,Qingyang Xu,Lixin Zhou,Jianxin Chen,Jicong Fan*

Main category: cs.LG

TL;DR: CLDD是一种基于图神经网络的疾病检测模型，通过利用疾病关联和患者相似性进行协同学习，无需依赖医学检测即可预测数千种疾病。


<details>
  <summary>Details</summary>
Motivation: 传统疾病检测需要大量医学测试且成本高昂，无法对每个患者进行所有可能的医学测试来诊断或预测数百上千种疾病。

Method: 提出基于图的协同学习疾病检测模型(CLDD)，通过自适应利用疾病间关联和患者间相似性，整合患者-疾病交互和人口统计学特征进行疾病预测。

Result: 在MIMIC-IV数据集(61,191患者，2,000疾病)上，CLDD在多个指标上优于基线方法，召回率提升6.33%，精确率提升7.63%。案例研究显示模型能成功恢复掩蔽疾病。
...
### [158] [SA^2GFM: Enhancing Robust Graph Foundation Models with Structure-Aware Semantic Augmentation](https://arxiv.org/abs/2512.07857)
*Junhua Shi,Qingyun Sun,Haonan Yuan,Xingcheng Fu*

Main category: cs.LG

TL;DR: SA^2GFM是一个鲁棒的图基础模型框架，通过结构感知语义增强提升领域自适应表示，在节点和图分类任务中优于9个SOTA基线


<details>
  <summary>Details</summary>
Motivation: 当前图基础模型在领域噪声、结构扰动和对抗攻击下的鲁棒性研究不足，关键限制在于对层次结构语义建模不充分，而这对泛化至关重要

Method: 1) 将基于熵的编码树转化为结构感知文本提示进行特征增强；2) 使用自监督信息瓶颈机制通过结构引导压缩提取鲁棒可迁移表示；3) 引入专家自适应路由机制（混合专家架构+空专家设计）解决跨域适应中的负迁移；4) 提出微调模块通过联合社区内和社区间结构学习优化层次结构

Result: 在节点和图分类任务中，SA^2GFM在对抗随机噪声和对抗扰动方面，在效果和鲁棒性上都优于9个最先进的基线方法

Conclusion: SA^2GFM通过结构感知语义增强和层次结构建模，显著提升了图基础模型的鲁棒性和领域自适应能力，为图基础模型的鲁棒性研究提供了有效框架
...
### [159] [FAIM: Frequency-Aware Interactive Mamba for Time Series Classification](https://arxiv.org/abs/2512.07858)
...
<details>
  <summary>Details</summary>
Motivation: 时间序列分类在环境监测、医疗诊断等应用中至关重要，但现有深度学习模型存在计算成本高、对噪声敏感、在小规模数据集上容易过拟合等问题，需要更高效鲁棒的解决方案。

Method: 提出FAIM模型，包含自适应滤波块（AFB）和交互Mamba块（IMB）。AFB利用傅里叶变换提取频域特征，通过可学习的自适应阈值动态抑制噪声，并耦合全局和局部语义自适应滤波。IMB促进高效的多粒度信息交互，平衡细粒度判别特征和全局上下文信息的提取。还加入了自监督预训练机制增强模型鲁棒性。

Result: 在多个基准测试上的广泛实验表明，FAIM持续优于现有的SOTA方法，在准确性和效率之间实现了优越的权衡，并在各种领域和高噪声场景中表现出色。

Conclusion: FAIM通过频率感知设计和交互Mamba架构，有效解决了时间序列分类中的计算效率、噪声敏感性和过拟合问题，为TSC任务提供了强大且高效的解决方案。
...
### [160] [SetAD: Semi-Supervised Anomaly Learning in Contextual Sets](https://arxiv.org/abs/2512.07863)
*Jianling Gao,Chongyang Tao,Xuelian Lin,Junfeng Liu,Shuai Ma*

Main category: cs.LG

TL;DR: SetAD：一种新颖的半监督异常检测框架，将异常检测重新定义为集合级任务，通过注意力机制和分级学习目标来量化整个集合的异常程度，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有半监督异常检测方法主要关注单个点或简单点对的评分，忽略了异常的上下文性质（异常通常由与集体群体的偏离来定义），且未能利用集合组合产生的丰富监督信号，难以捕捉数据中的高阶交互作用。

Method: 提出SetAD框架：1）使用基于注意力的集合编码器，通过分级学习目标训练模型量化整个集合的异常程度；2）提出上下文校准的异常评分机制，通过聚合点在多个不同上下文集合中相对于同伴行为的归一化偏差来评估异常分数。

Result: 在10个真实世界数据集上的广泛实验表明，SetAD显著优于最先进的模型。特别值得注意的是，模型性能随着集合大小的增加而持续提升，为基于集合的异常检测公式提供了强有力的实证支持。

Conclusion: 将半监督异常检测重新定义为集合级任务是一种有效的方法，能够直接建模定义异常的复杂群体级交互作用。SetAD框架通过集合编码器和上下文校准评分机制，在性能上取得了显著提升，并展示了集合大小与性能的正相关关系。
...
### [161] [Pattern Recognition of Ozone-Depleting Substance Exports in Global Trade Data](https://arxiv.org/abs/2512.07864)
*Muhammad Sukri Bin Ramli*

Main category: cs.LG

TL;DR: 提出一个基于无监督机器学习的框架，用于系统检测海关数据中的可疑贸易模式，以监控环境条约执行情况。


<details>
  <summary>Details</summary>
Motivation: 需要新方法来监控《蒙特利尔议定书》等环境条约的执行，通过分析大规模、复杂的海关数据集来发现违规贸易活动。

Method: 结合多种无监督机器学习技术：K-Means聚类发现贸易原型，隔离森林和IQR进行异常检测，启发式标记识别模糊描述等策略，最后整合为优先级评分。

Result: 成功识别出1,351个价格异常值和1,288个高优先级货物供海关审查，发现高优先级商品的单位重量价值与普通商品存在显著差异，模型检测到2021年初"超大贸易"激增与实际监管变化相符。

Conclusion: 该研究提供了一个可重复的无监督学习流程，能够将原始贸易数据转化为监管机构可用的优先情报，验证了机器学习在环境条约监控中的有效性。
...
### [162] [Using Text-Based Life Trajectories from Swedish Register Data to Predict Residential Mobility with Pretrained Transformers](https://arxiv.org/abs/2512.07865)
*Philipp Stark,Alexandros Sopasakis,Ola Hall,Markus Grillitsch*

Main category: cs.LG

TL;DR: 将瑞典登记数据转化为文本生命轨迹，解决分类变量高基数性和编码不一致问题，使用NLP模型预测居住流动性，证明文本化登记数据能有效支持纵向分析


<details>
  <summary>Details</summary>
Motivation: 解决数据分析中的两个长期挑战：分类变量的高基数性和时间上编码方案的不一致性，同时利用瑞典独特的全面人口登记数据来支持纵向预测分析

Method: 将690万个体（2001-2013）的登记数据转化为语义丰富的文本生命轨迹，结合人口统计信息和年度居住、工作、教育、收入、家庭状况变化，使用多种NLP架构（LSTM、DistilBERT、BERT、Qwen）预测个体后期（2013-2017）居住流动性

Result: 序列和基于transformer的模型比基线模型更有效地捕捉时间和语义结构，文本化登记数据保留了关于个体路径的有意义信息，支持复杂、可扩展的建模

Conclusion: 结合语义丰富的登记数据和现代语言模型可以显著推进社会科学中的纵向分析，该数据集为开发和评估新的序列建模方法提供了严格的测试平台
...
### [163] [Command & Control (C2) Traffic Detection Via Algorithm Generated Domain (Dga) Classification Using Deep Learning And Natural Language Processing](https://arxiv.org/abs/2512.07866)
*Maria Milena Araujo Felix*

Main category: cs.LG

TL;DR: 本文提出基于深度学习和NLP的DGA域名检测方法，使用LSTM神经网络达到97.2%准确率，优于传统统计熵分析。


<details>
  <summary>Details</summary>
Motivation: 现代恶意软件使用域名生成算法（DGA）产生大量动态地址，使基于静态黑名单的传统防火墙防御失效，需要更先进的检测方法。

Method: 收集包含5万合法域名和5万恶意域名的混合数据库，提取词汇特征，训练LSTM循环神经网络进行检测。

Result: 神经网络方法在检测复杂模式上表现优越，达到97.2%准确率，在模糊合法流量场景中降低了误报率。
...
### [164] [Bayesian Optimization for Function-Valued Responses under Min-Max Criteria](https://arxiv.org/abs/2512.07868)
*Pouya Ahadi,Reza Marzban,Ali Adibi,Kamran Paynabar*

Main category: cs.LG

TL;DR: 提出MM-FBO框架，用于优化函数型响应（如随时间或波长变化的响应）的黑箱函数，直接最小化函数域上的最大误差，而非传统方法的平均误差。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯优化广泛用于优化昂贵的黑箱函数，但现有方法主要针对标量响应。在科学和工程中，响应通常是函数型的（随时间或波长平滑变化），传统方法不适用。现有方法通常最小化积分误差（平均性能），但忽略了最坏情况偏差。

Method: 使用函数主成分分析表示函数型响应，为PC分数构建高斯过程代理模型。提出MM-FBO框架，引入集成不确定性采集函数，平衡最坏情况预期误差的利用和函数域上的探索。

Result: 提供两个理论保证：最坏情况目标的离散化界限，以及代理模型准确时采集函数收敛到真实最小最大目标的一致性结果。在合成基准和物理案例（电磁散射、气相渗透）上验证，MM-FBO始终优于现有基线。

Conclusion: MM-FBO框架能有效优化函数型响应的黑箱函数，直接处理最坏情况误差，在贝叶斯优化中显式建模函数不确定性的重要性得到验证。
...
### [165] [Advancing physiological time series reconstruction and imputation via mixture of receptive fields and experts fusion](https://arxiv.org/abs/2512.07873)
*Ci Zhang,Huayu Li,Changdi Yang,Jiangnan Xia,Yanzhi Wang,Xiaolong Ma,Jin Lu,Geng Yuan*

Main category: cs.LG

TL;DR: 提出基于专家混合的噪声估计器，在基于分数的扩散框架中用于医学时间序列重建，通过自适应感受野和并行噪声生成实现单步推理，显著提升性能并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 医学时间序列信号具有多变量、高时间变异性、高噪声和易受伪影影响等独特特性，使得基于深度学习的方法在插补等任务中仍然具有挑战性。现有扩散模型在医学时间序列领域应用不足，且多推理平均方法虽然能减少重建误差但计算成本高。

Method: 提出基于专家混合的噪声估计器，包含两个核心模块：1) 感受野自适应MoE模块，使每个通道在扩散过程中自适应选择所需感受野；2) 融合MoE模块，利用MoE特性并行生成K个噪声信号，通过路由机制融合，实现单步推理完成信号重建。

Result: 在多个任务和数据集上的广泛实验表明，所提出的框架在性能上持续优于基于扩散的SOTA方法，同时消除了多推理过程带来的显著计算成本和延迟。

Conclusion: 提出的基于MoE的噪声估计器在基于分数的扩散框架中有效解决了医学时间序列重建的挑战，通过自适应感受野和并行融合机制，在提升性能的同时大幅降低计算开销，为医学时间序列处理提供了高效解决方案。
...
### [166] [Controllable risk scenario generation from human crash data for autonomous vehicle testing](https://arxiv.org/abs/2512.07874)
*Qiujing Lu,Xuanhan Wang,Runze Yuan,Wei Lu,Xinyi Gong,Shuo Feng*

Main category: cs.LG

TL;DR: CRAG框架统一建模自动驾驶测试中的正常驾驶行为和罕见风险行为，通过解耦潜在空间实现可控风险场景生成，提升AV鲁棒性评估效率。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆需要同时测试日常驾驶和罕见安全关键场景，但现有方法难以在模拟中生成既真实又包含风险行为的交通参与者，特别是如何有效利用有限的真实事故数据。
...
Conclusion: CRAG框架成功统一了正常和风险行为建模，通过解耦潜在空间和优化模式转换，有效利用有限事故数据生成多样且可控的风险场景，为自动驾驶安全测试提供了有力工具。
...
### [167] [Softly Symbolifying Kolmogorov-Arnold Networks](https://arxiv.org/abs/2512.07875)
*James Bagrow,Josh Bongard*

Main category: cs.LG

TL;DR: S2KAN通过将符号基元直接集成到训练中，解决了传统KAN中激活函数缺乏符号保真度的问题，使用可学习的门控机制和最小描述长度目标实现可微分的稀疏化，在符号基准、动力系统预测和真实世界任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统的Kolmogorov-Arnold Networks（KANs）虽然提供了可解释机器学习的路径，但在实践中训练出的激活函数往往缺乏符号保真度，学习到的分解没有有意义的对应关系，无法达到真正的可解释性。

Method: 提出Softly Symbolified Kolmogorov-Arnold Networks（S2KAN），将符号基元直接集成到训练中。每个激活函数从符号项和密集项的字典中提取，使用可学习的门控机制实现稀疏表示。这种稀疏化是可微分的，支持端到端优化，并通过最小描述长度目标进行指导。

Result: 在符号基准测试、动力系统预测和真实世界预测任务中，S2KAN表现出竞争性或更优的准确性，同时模型规模显著更小。即使在没有正则化压力的情况下，也观察到自稀疏化的出现。

Conclusion: S2KAN通过将符号表示直接集成到训练过程中，解决了KANs的符号保真度问题，当符号项足够时能发现可解释形式，不足时则优雅地退化为密集样条，实现了可解释性和性能的良好平衡。
...
### [168] [Fourier-Enhanced Recurrent Neural Networks for Electrical Load Time Series Downscaling](https://arxiv.org/abs/2512.07876)
*Qi Chen,Mihai Anitescu*

Main category: cs.LG

TL;DR: 提出一种傅里叶增强的循环神经网络，用于电力负荷降尺度预测，结合循环主干、傅里叶季节嵌入和自注意力机制，在PJM地区表现优于传统基准方法。


<details>
  <summary>Details</summary>
Motivation: 电力负荷预测需要高精度的时间序列降尺度方法，传统方法在处理季节性和高分辨率组件依赖关系方面存在局限，需要更先进的模型来提升预测性能。

Method: 提出傅里叶增强的循环神经网络，包含三个核心组件：1) 由低分辨率输入驱动的循环神经网络主干；2) 在潜在空间中融合的显式傅里叶季节嵌入；3) 捕捉每个周期内高分辨率组件依赖关系的自注意力层。

Result: 在四个PJM地区测试中，该方法相比传统Prophet基准（无论是否包含季节性和LAA）以及没有注意力或傅里叶特征的RNN消融模型，都取得了更低的RMSE和更平坦的预测误差分布。

Conclusion: 傅里叶增强的循环神经网络结合自注意力机制，能够有效提升电力负荷降尺度预测的准确性，特别是在处理季节性和高分辨率时间依赖关系方面表现优异。
...
### [169] [Artificial Intelligence-Driven Network-on-Chip Design Space Exploration: Neural Network Architectures for Design](https://arxiv.org/abs/2512.07877)
...
<details>
  <summary>Details</summary>
Motivation: 传统NoC设计空间探索方法速度慢，难以处理复杂的非线性参数交互，需要更高效自动化的解决方案来满足严格的吞吐量和延迟约束。

Method: 开发机器学习驱动框架，使用BookSim仿真生成超过150,000个数据点，比较三种神经网络架构（MLP、条件扩散模型、CVAE）来预测给定目标性能指标下的最优NoC参数。

Result: 条件扩散模型表现最佳，在未见数据上获得0.463的均方误差，框架将设计探索时间减少了数个数量级。

Conclusion: 提出的机器学习框架为快速可扩展的NoC协同设计提供了实用解决方案，显著加速设计空间探索过程。
...
### [170] [Graph Contrastive Learning via Spectral Graph Alignment](https://arxiv.org/abs/2512.07878)
*Manh Nguyen,Joshua Cape*

Main category: cs.LG

TL;DR: SpecMatch-CL是一种新颖的对比学习损失函数，通过最小化图嵌入的图-图结构的归一化拉普拉斯矩阵差异来对齐增强视图，在多个图学习任务上取得SOTA性能。
...
Method: 提出SpecMatch-CL损失函数，通过最小化视图特定图-图结构的归一化拉普拉斯矩阵差异来对齐图嵌入。理论上证明该差异为理想完美对齐对比损失与当前损失之间的差异提供了上界。

Result: 在8个TU基准测试的无监督学习和低标签率半监督学习中达到新的SOTA，在PPI-306K和ZINC 2M数据集的迁移学习中获得一致增益。

Conclusion: SpecMatch-CL通过控制图嵌入的图-图结构全局对齐，显著提升了图对比学习的性能，为图表示学习提供了有效的理论框架和实践方法。
...
### [171] [Nonnegative Matrix Factorization through Cone Collapse](https://arxiv.org/abs/2512.07879)
*Manh Nguyen,Daniel Pimentel-Alarcón*

Main category: cs.LG

TL;DR: 提出Cone Collapse算法，从几何视角重新审视NMF，通过收缩非负象限到数据生成的最小锥体，然后应用单正交NMF得到CC-NMF模型，在聚类任务中优于传统NMF方法。


<details>
  <summary>Details</summary>
Motivation: 现有NMF算法主要从优化视角出发，未能充分利用NMF诱导的锥体几何结构。数据点位于凸锥中，其极射线编码了基本方向或"主题"。本文从几何视角重新审视NMF，旨在显式恢复数据锥体以获得更好的聚类性能。

Method: 提出Cone Collapse算法：从完整的非负象限开始，迭代收缩到数据生成的最小锥体。证明在温和数据假设下，该算法有限步终止并恢复X^T的最小生成锥。在此基础上，通过对恢复的极射线应用单正交NMF，推导出锥体感知的正交NMF模型（CC-NMF）。

Result: 在16个基准基因表达、文本和图像数据集上，CC-NMF在聚类纯度方面始终匹配或优于强NMF基线方法，包括乘法更新、ANLS、投影NMF、ONMF和稀疏NMF。

Conclusion: 显式恢复数据锥体可以产生既有理论依据又在经验上强大的基于NMF的聚类方法。几何视角为NMF提供了新的见解，CC-NMF在多个领域数据集上表现出优越的聚类性能。
...
### [172] [Semi-Supervised Contrastive Learning with Orthonormal Prototypes](https://arxiv.org/abs/2512.07880)
...
<details>
  <summary>Details</summary>
Motivation: 对比学习在深度学习中表现出色，但维度坍缩问题（嵌入收敛到低维空间）在半监督和自监督设置中构成重大挑战，特别是在高学习率下会导致坍缩解。

Method: 首先识别了导致维度坍缩的关键学习率阈值，然后提出CLOP损失函数，通过促进类别嵌入形成正交线性子空间来防止维度坍缩。

Result: 在真实和合成数据集上的广泛实验表明，CLOP在图像分类和目标检测任务中提高了性能，同时在不同的学习率和批次大小下表现出更好的稳定性。

Conclusion: CLOP通过防止维度坍缩有效提升了对比学习的性能，为半监督学习提供了更稳定和有效的解决方案。
...
### [173] [GSPN-2: Efficient Parallel Sequence Modeling](https://arxiv.org/abs/2512.07884)
...
TL;DR: GSPN-2通过算法-系统联合重设计，解决了原GSPN在GPU实现中的效率瓶颈，在保持视觉Transformer精度的同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有GSPN虽然用线性扫描传播替代了二次自注意力，但在GPU实现上仍存在三个主要问题：1) 重复启动GPU内核导致大量开销；2) 全局GPU内存数据传输过多；3) 为每个通道维护独立传播权重造成冗余计算。

Method: 1) 系统优化：将数千个微启动合并为单个2D内核，为每个通道切片固定一个warp，将前一列的激活暂存到共享内存；2) 算法优化：引入紧凑通道传播策略，替代逐通道矩阵，减少参数，并与Transformer注意力中的亲和力图自然对齐。

Result: 在图像分类和文本到图像合成任务上，GSPN-2能够匹配Transformer级别的精度，同时显著降低计算成本，为视觉应用中的全局空间上下文建模建立了新的效率前沿。

Conclusion: GSPN-2通过结构化矩阵变换与GPU优化实现的独特结合，为高分辨率图像和长视频相关应用提供了高效的视觉Transformer解决方案。
...
### [174] [ByteStorm: a multi-step data-driven approach for Tropical Cyclones detection and tracking](https://arxiv.org/abs/2512.07885)
...
<details>
  <summary>Details</summary>
Motivation: 传统台风追踪方案主要依赖主观阈值，在不同地理区域应用时可能引入偏差，需要更准确、无需阈值调优的追踪方法

Method: 使用深度学习网络通过分类和定位检测台风中心（仅使用850mb相对涡度和平均海平面气压），然后通过BYTE算法将检测到的中心连接成台风轨迹

Result: 在东太平洋和西太平洋盆地，ByteStorm在检测概率（85.05% ENP，79.48% WNP）、误报率（23.26% ENP，16.14% WNP）和年际变率相关性（0.75 ENP，0.69 WNP）方面均优于现有确定性追踪器

Conclusion: ByteStorm展示了将深度学习和计算机视觉技术用于快速准确台风追踪的潜力，为传统方法提供了稳健的替代方案
...
### [175] [Towards symbolic regression for interpretable clinical decision scores](https://arxiv.org/abs/2512.07961)
...
<details>
  <summary>Details</summary>
Motivation: 医疗决策常使用风险方程与规则结合的算法，但传统符号回归仅限于连续函数形式，难以建模这种决策过程。然而符号回归具有数据驱动、可解释的优点，有望用于开发数据驱动的临床风险评分系统。

Method: Brush算法结合了决策树式的分割算法和非线性常数优化，能够将基于规则的逻辑无缝集成到符号回归和分类模型中。

Result: Brush在SRBench上实现了帕累托最优性能，成功复现了两个广泛使用的临床评分系统，获得了高准确率和可解释模型。与决策树、随机森林和其他符号回归方法相比，Brush实现了相当或更优的预测性能，同时生成更简单的模型。

Conclusion: Brush算法通过结合规则逻辑和符号回归，为开发数据驱动、可解释的临床风险评分系统提供了有效工具，在保持高预测性能的同时提高了模型的可解释性。
...
### [176] [CIP-Net: Continual Interpretable Prototype-based Network](https://arxiv.org/abs/2512.07981)
*Federico Di Valerio,Michela Proietti,Alessio Ragno,Roberto Capobianco*

Main category: cs.LG

TL;DR: CIP-Net：一种无需示例存储的自解释原型模型，用于持续学习，在避免灾难性遗忘的同时提供可解释性，且内存开销低。


<details>
  <summary>Details</summary>
Motivation: 持续学习面临灾难性遗忘的挑战，现有可解释方法多使用事后解释或需要额外内存存储任务示例，限制了可扩展性。需要一种既保持可解释性又无需存储历史示例的轻量级解决方案。

Method: 提出CIP-Net（自解释原型网络），采用原型基础架构，无需存储过去任务的示例。模型在预测时生成解释，通过原型机制保留知识，保持简单架构同时提供可解释性。

Result: CIP-Net在任务增量学习和类别增量学习设置中，相比现有的无示例方法和自解释方法，达到了最先进的性能，同时显著降低了内存相关开销。

Conclusion: CIP-Net为持续学习提供了一个实用且可解释的解决方案，在保持高性能的同时避免了存储历史示例的需求，具有良好的可扩展性和实际应用价值。
...
### [177] [HOLE: Homological Observation of Latent Embeddings for Neural Network Interpretability](https://arxiv.org/abs/2512.07988)
*Sudhanva Manjunath Athreya,Paul Rosen*

Main category: cs.LG

TL;DR: HOLE方法使用持续同调分析深度神经网络，通过拓扑特征提取和可视化工具（桑基图、热图等）来理解和解释神经网络表示。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型虽然在各领域取得显著成功，但其学习到的表示和决策过程仍然不透明且难以解释，需要新的方法来理解和分析神经网络内部表示。

Method: HOLE方法从神经激活中提取拓扑特征，并使用持续同调进行分析，提供桑基图、热力图、树状图和blob图等多种可视化工具来检查各层的表示结构和质量。

Result: 在标准数据集上评估多种判别模型，结果显示拓扑分析能揭示与类别分离、特征解耦和模型鲁棒性相关的模式，为理解深度学习系统提供了补充视角。

Conclusion: HOLE方法通过拓扑分析为深度神经网络的可解释性提供了新工具，能够揭示表示质量、层间可解释性和模型鲁棒性，有助于理解和改进深度学习系统。
...
### [178] [Bridging the Clinical Expertise Gap: Development of a Web-Based Platform for Accessible Time Series Forecasting and Analysis](https://arxiv.org/abs/2512.07992)
*Aaron D. Mullen,Daniel R. Harris,Svetla Slavova,V. K. Cody Bumgardner*

Main category: cs.LG

TL;DR: 开发了一个面向医疗领域的时间序列预测Web平台，降低技术门槛，让研究者和临床医生能轻松分析数据、训练模型并解释结果


<details>
  <summary>Details</summary>
Motivation: 时间序列预测在医疗健康领域有广泛应用，但技术门槛高，需要专业知识分析数据、构建模型和解释结果，这限制了研究人员和临床医生的使用

Method: 开发了一个Web平台，支持数据上传和可视化分析，提供多种可高度定制的预测模型和训练技术，并集成大语言模型提供参数推荐和结果解释

Result: 创建了一个使时间序列预测过程对非技术用户可访问的平台，支持数据可视化、多模型选择、参数定制，以及AI辅助的决策支持和结果解释

Conclusion: 该平台旨在降低医疗领域时间序列预测的技术门槛，计划整合到学习型健康系统中，实现临床数据管道的持续收集和推断
...
### [179] [Benchmarking Offline Multi-Objective Reinforcement Learning in Critical Care](https://arxiv.org/abs/2512.08012)
*Aryaman Bansal,Divya Sharma*

Main category: cs.LG

TL;DR: 在ICU等重症监护环境中，研究比较了离线多目标强化学习算法与单目标基线方法，发现PEDA DT算法在灵活性和性能上表现最佳，为个性化医疗决策提供了新框架。


<details>
  <summary>Details</summary>
Motivation: 重症监护中临床医生面临生存率最大化和资源利用最小化的冲突目标。传统单目标强化学习方法使用固定标量化奖励函数，导致策略僵化，无法适应变化的临床优先级。需要一种能够在离线环境下学习多目标策略的方法。

Method: 在MIMIC-IV数据集上，对三种离线多目标强化学习算法（CPQL、Adaptive CPQL、PEDA DT）与三种标量化单目标基线方法（BC、CQL、DDQN）进行基准测试。使用离线策略评估（OPE）指标评估性能。

Result: PEDA DT算法相比静态标量化基线展现出更优的灵活性。研究结果扩展了单目标决策变换器在医疗领域的发现，证实序列建模架构在多目标条件生成中保持稳健有效。

Conclusion: 离线多目标强化学习是一个有前景的框架，能够在无需重新训练的情况下实现重症监护中的个性化、可调整决策制定。
...
### [180] [CLARITY: Medical World Model for Guiding Treatment Decisions by Modeling Context-Aware Disease Trajectories in Latent Space](https://arxiv.org/abs/2512.08029)
...
<details>
  <summary>Details</summary>
Motivation: 当前肿瘤学临床决策需要预测动态疾病演化，但现有静态AI预测器无法完成此任务。现有医学世界模型方法通常忽略患者特异性时间和临床上下文，缺乏将预测与治疗决策连接的反馈机制。

Method: CLARITY在结构化潜在空间中直接预测疾病演化，明确整合时间间隔（时间上下文）和患者特异性数据（临床上下文），将治疗条件进展建模为平滑、可解释的轨迹，从而生成生理上可信的个体化治疗计划。还引入了新颖的预测到决策框架，将潜在推演转化为透明、可操作的建议。

Result: 在MU-Glioma-Post数据集上，CLARITY比最近的MeWM方法性能提升12%，并显著超越所有其他医学专用大型语言模型，在治疗规划方面展示了最先进的性能。

Conclusion: CLARITY通过整合时间间隔和患者特异性上下文，能够生成生理上可信的个体化治疗计划，并提供从预测到决策的透明框架，为肿瘤学临床决策提供了有效的动态疾病演化预测工具。
...
### [181] [LUNA: Linear Universal Neural Attention with Generalization Guarantees](https://arxiv.org/abs/2512.08061)
*Ashkan Shahbazi,Ping He,Ali Abbasi,Yikun Bai,Xinran Liu,Elaheh Akbari,Darian Salehi,Navid NaderiAlizadeh,Soheil Kolouri*

Main category: cs.LG

TL;DR: LUNA是一种可学习的核化线性注意力机制，通过参数化核特征映射实现线性计算复杂度，同时达到甚至超越二次注意力机制的准确性。


<details>
  <summary>Details</summary>
Motivation: 软注意力机制存在O(n²)的二次计算成本，限制了其在长序列领域的应用。现有线性注意力机制虽然将成本降至O(n)，但依赖固定的随机特征映射，导致模型准确性和计算效率之间存在根本性权衡。

Method: LUNA采用可学习的核特征映射，而非固定的随机特征映射。该方法参数化核函数，学习针对特定数据和任务的特征基，同时保持正定核特性并支持流式处理，实现序列长度的线性时间和内存扩展。

Result: 在Long Range Arena（LRA）基准测试中，LUNA在计算对等条件下（相同参数量、训练步数和近似FLOPs）达到了高效Transformer中的最先进平均准确率。在BERT和ViT-B/16的后验转换实验中，LUNA显著优于固定线性化方法，能够恢复大部分原始性能。

Conclusion: LUNA通过可学习的核特征映射消除了线性注意力机制中准确性和效率之间的权衡，实现了线性计算成本同时达到甚至超越二次注意力机制的性能，为长序列处理提供了有效的解决方案。
...
### [182] [Deep Kernel Aalen-Johansen Estimator: An Interpretable and Flexible Neural Net Framework for Competing Risks](https://arxiv.org/abs/2512.08063)
*Xiaobin Shen,George H. Chen*

Main category: cs.LG

TL;DR: 提出可解释的深度竞争风险模型DKAJ，通过自动学习核函数衡量数据点相似性，将每个数据点表示为多个簇的加权组合，在保持竞争力的同时提供可视化解释。


<details>
  <summary>Details</summary>
Motivation: 现有竞争风险模型缺乏可解释性，需要一种既能保持预测性能又能提供模型解释的方法，以帮助理解模型决策过程。

Method: 提出Deep Kernel Aalen-Johansen (DKAJ)估计器，将经典Aalen-Johansen非参数估计推广到深度框架。每个数据点表示为多个簇的加权组合，权重通过自动学习的核函数计算，衡量数据点之间的相似性。

Result: 在四个标准竞争风险数据集上，DKAJ与最先进的基线方法竞争性能相当，同时能够提供可视化辅助模型解释。

Conclusion: DKAJ是一种有效的可解释深度竞争风险模型，在保持预测性能的同时提供了模型可解释性，有助于理解模型决策过程。
...
### [183] [CAMO: Causality-Guided Adversarial Multimodal Domain Generalization for Crisis Classification](https://arxiv.org/abs/2512.08071)
...
<details>
  <summary>Details</summary>
Motivation: 社交媒体危机分类任务面临的主要挑战是现有方法在未见灾难类型上泛化能力差。原因有二：1) 未能解耦虚假特征和因果特征，导致域偏移时性能下降；2) 未能对齐异构模态表示，阻碍了单模态域泛化技术向多模态设置的直接迁移。

Method: 提出因果引导的多模态域泛化框架，结合对抗解耦与统一表示学习。对抗目标鼓励模型解耦并关注域不变因果特征；统一表示将不同模态特征对齐到共享潜在空间，使单模态域泛化策略能无缝扩展到多模态学习。

Result: 在不同数据集上的实验表明，该方法在未见灾难场景中取得了最佳性能。

Conclusion: 通过因果引导的多模态域泛化框架，有效解决了社交媒体危机分类在未见灾难类型上的泛化问题，为多模态域泛化提供了新思路。
...
### [184] [Unveiling Latent Knowledge in Chemistry Language Models through Sparse Autoencoders](https://arxiv.org/abs/2512.08077)
*Jaron Cohen,Alexander G. Hasson,Sara Tanovic*

Main category: cs.LG

TL;DR: 该研究将稀疏自编码器技术应用于化学语言模型，揭示了模型内部可解释的化学特征表示，建立了潜在特征与化学结构、性质及药物类别之间的关联。
...
Result: 发现这些模型编码了丰富的化学概念景观，识别出特定潜在特征与化学知识不同领域（包括结构基序、物理化学性质和药理学药物类别）之间的相关性。
...
### [185] [Complexity of One-Dimensional ReLU DNNs](https://arxiv.org/abs/2512.08091)
*Jonathan Kogan,Hayden Jananthan,Jeremy Kepner*

Main category: cs.LG

TL;DR: 研究一维ReLU深度神经网络在线性区域方面的表达能力，证明了随机初始化网络在无限宽度极限下线性区域数量的期望增长公式，并提出了函数自适应的稀疏性概念。


<details>
  <summary>Details</summary>
Motivation: 理解深度神经网络表达能力的关键指标之一是网络产生的线性区域数量。对于一维ReLU网络，研究其线性区域数量如何随网络结构变化，以及如何评估网络在近似特定函数时的效率。

Method: 分析随机初始化的一维ReLU网络（He缩放且带非零偏置）在无限宽度极限下的行为。使用数学证明推导线性区域数量的期望增长公式，并提出函数自适应稀疏性概念来比较网络使用的区域数量与近似目标函数所需的最小区域数量。

Result: 证明了线性区域数量的期望增长为∑_{i=1}^L n_i + o(∑_{i=1}^L n_i) + 1，其中n_ℓ是第ℓ隐藏层的神经元数量。提出了函数自适应稀疏性概念来衡量网络近似效率。

Conclusion: 一维ReLU深度神经网络的表达能力可以通过线性区域数量来量化，其增长与网络总神经元数量线性相关。提出的稀疏性概念为评估网络近似特定函数的效率提供了新视角。
...
### [186] [Training LLMs for Honesty via Confessions](https://arxiv.org/abs/2512.08093)
*Manas Joglekar,Jeremy Chen,Gabriel Wu,Jason Yosinski,Jasmine Wang,Boaz Barak,Amelia Glaese*

Main category: cs.LG

TL;DR: 提出一种通过"忏悔"机制让大语言模型诚实报告自身缺陷的方法，通过单独奖励诚实的忏悔而不影响主回答的奖励，激励模型在忏悔中诚实披露不当行为。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在报告自身行为和信念时可能不诚实，例如夸大事实信心或掩盖不当行为。这种不诚实可能源于强化学习中的奖励塑造问题，训练过程可能无意中激励模型说谎或掩饰行为。

Method: 提出"忏悔"机制：在主回答后，模型应请求提供一个忏悔输出，完整说明其遵守政策和指令的情况。忏悔的奖励仅基于其诚实性，不影响主回答的奖励。只要最大化忏悔奖励的"最小阻力路径"是披露而非掩盖不当行为，就能激励模型在忏悔中诚实。

Result: 训练GPT-5-Thinking产生忏悔，在分布外场景中评估其诚实性（包括幻觉、指令遵循、阴谋行为和奖励黑客攻击）。发现当模型在主回答中说谎或忽略缺陷时，通常会在忏悔中诚实承认这些行为，且忏悔的诚实性随训练适度提升。忏悔支持多种推理时干预，包括监控、拒绝采样和向用户报告问题。

Conclusion: 忏悔机制是让大语言模型诚实报告自身缺陷的可行方法，通过单独奖励诚实的忏悔，能够激励模型在忏悔中披露不当行为，为模型监控和干预提供了新途径。
...
### [187] [Scalable Offline Model-Based RL with Action Chunks](https://arxiv.org/abs/2512.08108)
*Kwanyoung Park,Seohong Park,Youngwoon Lee,Sergey Levine*

Main category: cs.LG

TL;DR: MAC提出了一种基于模型的离线强化学习方法，通过动作块模型减少长期预测误差，并结合拒绝采样防止模型利用，在复杂长时域任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决基于模型的强化学习在离线设置下处理复杂长时域任务时的挑战：传统基于模型的价值扩展方法在增加预测步长n时，虽然减少了价值引导的偏差，但会放大模型误差的累积，导致长期预测质量下降。

Method: 1. 动作块模型：预测未来状态时使用动作序列（动作块）而非单个动作，减少误差累积；2. 拒绝采样：从表达性强的行为动作块策略中进行拒绝采样，防止模型利用分布外动作；3. 整体框架称为MAC（Model-Based RL with Action Chunks）。

Result: 在包含高达1亿个转移的大规模数据集上的实验表明，MAC在离线基于模型的RL算法中表现最佳，特别是在具有挑战性的长时域任务上。

Conclusion: MAC通过动作块模型减少长期预测误差，结合拒绝采样防止模型利用，为离线强化学习处理复杂长时域任务提供了可扩展的解决方案。
...
### [188] [Balanced Accuracy: The Right Metric for Evaluating LLM Judges -- Explained through Youden's J statistic](https://arxiv.org/abs/2512.08121)
*Stephane Collot,Colin Fraser,Justin Zhao,William F. Shen,Timon Willi,Ilias Leontiadis*

Main category: cs.LG

TL;DR: 论文提出使用Youden's J统计量和平衡准确率来选择LLM评估中的分类器，相比传统指标能提供更可靠的模型比较


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估依赖于分类器（如LLM-as-a-judge或人工标注）来估计期望/不期望行为的流行率，但常用的准确率、精确率、F1等指标对类别不平衡和正类选择敏感，可能导致选择扭曲流行率估计的分类器

Method: 提出使用Youden's J统计量作为选择最佳分类器的理论依据，证明平衡准确率是J的线性变换，通过理论分析和实证模拟验证平衡准确率在分类器选择中的优越性

Result: Youden's J统计量在理论上与选择最佳分类器以比较模型的目标一致，平衡准确率作为等效指标能提供更稳健的分类器选择，避免传统指标的问题

Conclusion: 在LLM评估中，应使用平衡准确率而非传统指标来选择分类器，这能确保更可靠、更稳健的模型比较和流行率估计
...
### [189] [Long-only cryptocurrency portfolio management by ranking the assets: a neural network approach](https://arxiv.org/abs/2512.08124)
*Zijiang Yang*
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单个加密货币（如比特币）的价格预测，而忽略了加密货币之间的相对关系。本文旨在通过分析加密货币间的交叉截面信息，开发更有效的投资组合管理方法。

Method: 在每个时间步，使用神经网络预测所管理加密货币的未来收益排名，并根据排名分配投资权重。这种方法利用了加密货币间的相对关系信息，而非独立预测单个币种的价格走势。

Result: 在2020年5月至2023年11月的真实加密货币市场数据上进行回测，涵盖牛市、熊市和横盘市场周期。提出的方法在复杂市场条件下表现优异，夏普比率达到1.01，年化收益率64.26%，且对交易费用增加具有鲁棒性。

Conclusion: 通过分析加密货币间的相对关系而非独立预测，提出的基于神经网络的排名预测方法能够有效管理投资组合，在不同市场条件下均能获得优异表现，为加密货币投资组合管理提供了新思路。
...
### [190] [Improving the Sensitivity of Backdoor Detectors via Class Subspace Orthogonalization](https://arxiv.org/abs/2512.08129)
...
<details>
  <summary>Details</summary>
Motivation: 传统后门检测方法依赖目标类表现出极端异常统计量，但存在两个问题：1) 某些非目标类本身容易区分，自然获得极端统计量；2) 隐蔽后门攻击中后门特征相对内在特征较弱。需要更敏感的检测方法

Method: 提出类子空间正交化(CSO)方法：利用少量干净样本，通过约束优化问题，在优化检测统计量的同时正交化抑制类的内在特征。对于非目标类，这种抑制会大幅降低可达到的统计量；对于目标类，后门触发器的贡献仍然显著

Result: CSO方法能够有效检测具有挑战性的混合标签攻击和自适应攻击，相比传统方法具有更高的检测敏感性

Conclusion: 通过正交化抑制内在特征，CSO方法显著提升了后门检测的敏感性，能够有效应对传统方法失效的挑战性攻击场景
...
### [191] [Biothreat Benchmark Generation Framework for Evaluating Frontier AI Models I: The Task-Query Architecture](https://arxiv.org/abs/2512.08130)
...
TL;DR: 开发首个生物威胁基准生成框架，专门评估AI模型（特别是大语言模型）在细菌生物威胁方面的安全风险，考虑不同攻击者能力和操作风险因素


<details>
  <summary>Details</summary>
Motivation: 随着前沿AI模型（特别是大语言模型）的快速发展，需要量化它们可能被用于生物恐怖主义或获取生物武器的风险。现有基准往往忽视威胁本身的关键方面，如不同攻击者能力水平和操作风险因素

Method: 提出生物威胁基准生成框架，采用分层结构（生物威胁类别、要素和任务），开发任务对齐的查询。本文重点描述细菌生物威胁模式架构的开发，作为试点仅针对细菌生物威胁

Result: 开发了细菌生物威胁模式，建立了可重用的评估结构，能够跨多个聚合层次评估LLM带来的细菌生物风险，涵盖生物对手的技术和操作需求全范围

Conclusion: BBG框架（包括细菌生物威胁模式）为评估AI模型在细菌生物威胁方面的安全风险提供了稳健、可重用的结构，未来工作将把查询转化为模型提示并实施基准评估
...
### [192] [Robust Agents in Open-Ended Worlds](https://arxiv.org/abs/2512.08139)
*Mikayel Samvelyan*

Main category: cs.LG

TL;DR: 该论文提出了一系列方法，通过开放世界和多智能体学习来训练和评估鲁棒的AI智能体，使其能够泛化到新环境、分布外输入以及与其他智能体的交互。
...
Method: 1) 引入MiniHack沙盒框架，通过程序内容生成创建多样化环境；2) 提出Maestro方法，生成对抗性课程以增强RL智能体在双人零和游戏中的鲁棒性；3) 使用质量-多样性方法在足球游戏领域系统识别预训练RL策略的漏洞；4) 将鲁棒性探索扩展到LLM领域，使用进化搜索生成多样化对抗提示。

Result: 开发了MiniHack框架用于创建多样化RL任务，提出了Maestro方法增强智能体鲁棒性，在复杂足球游戏领域识别了现有RL策略的漏洞，并为LLM开发了对抗提示生成方法以提高其鲁棒性。

Conclusion: 这项工作为AI鲁棒性的未来发展铺平了道路，使智能体不仅能够适应不断变化的世界，还能在面对意外挑战和交互时茁壮成长。通过结合开放世界和多智能体学习方法，论文展示了提高AI系统泛化能力和鲁棒性的有效途径。
...
### [193] [PolyLingua: Margin-based Inter-class Transformer for Robust Cross-domain Language Detection](https://arxiv.org/abs/2512.08143)
...
<details>
  <summary>Details</summary>
Motivation: 语言识别是多语言系统的关键第一步，现有工具在音乐请求等关键场景中表现不佳，开源工具速度快但精度低，大语言模型有效但成本过高，不适合低延迟或低资源环境。
...
Conclusion: PolyLingua在计算和延迟受限环境中实现了高精度的语言识别，特别适合处理音乐请求等具有代码切换的挑战性场景，为多语言系统提供了高效解决方案。
...
### [194] [TreeGRPO: Tree-Advantage GRPO for Online RL Post-Training of Diffusion Models](https://arxiv.org/abs/2512.08153)
...
Conclusion: TreeGRPO为基于强化学习的视觉生成模型对齐提供了一条可扩展且有效的途径，显著提高了训练效率，解决了现有方法在样本效率和信用分配方面的局限性。
...
### [195] [LayerPipe2: Multistage Pipelining and Weight Recompute via Improved Exponential Moving Average for Training Neural Networks](https://arxiv.org/abs/2512.08160)
*Nanda K. Unnikrishnan,Keshab K. Parhi*

Main category: cs.LG

TL;DR: LayerPipe2 为神经网络训练流水线化提供了理论框架，通过延迟梯度适应和重定时分析，推导出各层所需的延迟量，并解决了历史权重存储问题。


<details>
  <summary>Details</summary>
Motivation: 之前的LayerPipe方法虽然通过重叠前向和反向计算加速了训练，但缺乏对每层需要引入多少梯度延迟的理论理解。本文旨在填补这一空白，提供流水线化训练的理论基础。

Method: 使用变量延迟梯度适应和重定时技术，正式推导LayerPipe框架。分析网络结构中哪些位置可以合法插入延迟，并开发管道感知移动平均方法，通过重建历史状态而非显式存储来减少内存开销。

Result: 发现延迟需求直接取决于网络结构：内层需要较少延迟，外层需要较长延迟。当每层都流水线化时，延迟量仅取决于下游阶段数量；当层分组流水线化时，组内所有层共享相同的延迟分配。

Conclusion: 提出了一个理论框架，指导如何构建LayerPipe架构、预测其延迟需求并减轻存储负担，从而实现可扩展的流水线化训练，并控制通信计算权衡。
...
### [196] [MobileFineTuner: A Unified End-to-End Framework for Fine-Tuning LLMs on Mobile Phones](https://arxiv.org/abs/2512.08211)
*Jiaxiang Geng,Lunyu Zhao,Yiyi Lu,Bing Luo*

Main category: cs.LG

TL;DR: MobileFineTuner是一个开源框架，支持在普通手机上直接进行LLM端到端微调，解决了移动设备内存和能耗限制问题。


<details>
  <summary>Details</summary>
Motivation: 随着高质量公共数据接近枯竭，设备端微调可以利用用户私有数据同时保护隐私。现有方法主要是模拟或基于IoT设备/PC，普通手机未被充分探索，缺乏开源框架。

Method: 提出MobileFineTuner统一开源框架，支持全参数微调和参数高效微调。引入参数分片、梯度累积、能量感知计算调度等系统级优化来解决内存和能耗限制。

Result: 在真实手机上成功微调了GPT-2、Gemma 3和Qwen 2.5模型。大量实验和消融研究验证了优化有效性，证明MobileFineTuner是设备端LLM训练研究的可行基础。

Conclusion: MobileFineTuner填补了移动设备LLM微调框架的空白，为未来设备端LLM训练研究提供了实用基础，解决了隐私保护和数据利用的关键问题。
...
### [197] [Correction of Decoupled Weight Decay](https://arxiv.org/abs/2512.08217)
*Jason Chuan-Chih Chou*

Main category: cs.LG

TL;DR: 论文挑战了AdamW中权重衰减与学习率γ成比例的传统设定，提出权重衰减应与γ²成比例，基于权重范数稳定性的理论推导，并验证了这种设定能改善训练动态和模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统AdamW优化器中，解耦权重衰减（decoupled weight decay）默认与学习率γ成比例，但这种设定缺乏理论依据。最近有研究基于稳态正交性论证提出权重衰减应与γ²成比例，但作者发现这种正交性论证对训练动态影响很小，需要更合理的理论解释。

Method: 作者基于"稳态时更新与权重无关"的简单假设，推导出解耦权重衰减应与γ²成比例才能使权重范数稳定。同时推导了Scion优化器中每个小批量的总更新贡献（TUC）更应由动量相关的有效学习率表征，并通过实验验证了理论推导。

Result: 理论推导表明权重衰减∝γ²能实现稳定的权重和梯度范数。实验验证了这种设定能更好地控制训练动态，改善模型性能，并且最优的有效学习率值在不同设置间具有可迁移性。

Conclusion: 解耦权重衰减应与学习率的平方γ²成比例，而不是传统认为的与γ成比例。这种设定基于权重范数稳定性的理论推导，能提供更稳定的训练动态和更好的模型性能，为优化器设计提供了新的理论基础。
...
### [198] [PR-CapsNet: Pseudo-Riemannian Capsule Network with Adaptive Curvature Routing for Graph Learning](https://arxiv.org/abs/2512.08218)
*Ye Qin,Jingchao Wang,Yang Shi,Haiying Huang,Junxu Li,Weijian Liu,Tinghui Chen,Jinghui Qin*

Main category: cs.LG

TL;DR: PR-CapsNet将胶囊网络扩展到伪黎曼流形，通过自适应曲率路由解决图数据几何建模问题，在节点和图分类任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统胶囊网络在固定曲率空间中建模图数据存在几何不连续问题，而伪黎曼流形为图嵌入提供了更好的归纳偏置，但如何利用其改进胶囊网络尚未充分探索。

Method: 提出伪黎曼胶囊网络(PR-CapsNet)，包含：1)伪黎曼切空间路由，将胶囊状态分解为球面-时间和欧几里得-空间子空间；2)自适应曲率路由，通过可学习曲率张量融合不同曲率空间特征；3)几何保持的伪黎曼胶囊分类器，使用曲率加权softmax进行分类。

Result: 在节点和图分类基准测试中，PR-CapsNet优于现有最先进模型，验证了其对复杂图结构的强大表示能力。

Conclusion: PR-CapsNet通过将胶囊网络扩展到自适应曲率的伪黎曼流形，有效解决了图数据的几何建模问题，为复杂图结构表示学习提供了新方法。
...
### [199] [Persistent Topological Structures and Cohomological Flows as a Mathematical Framework for Brain-Inspired Representation Learning](https://arxiv.org/abs/2512.08241)
...
<details>
  <summary>Details</summary>
Motivation: 为脑启发表示学习建立严格的数学基础，通过拓扑结构与上同调流的相互作用来捕捉大脑状态中的不变特征，解决现有方法在流形一致性和噪声鲁棒性方面的不足。

Method: 将神经计算重构为动态单纯复形上的上链映射演化，结合代数拓扑和微分几何构建上同调算子，使用持久同调、层上同调和谱拉普拉斯分析合成数据和真实神经数据集。

Result: 模型在流形一致性和噪声鲁棒性方面优于图神经网络和基于流形的深度架构，为拓扑驱动的表示学习建立了连贯的数学基础。

Conclusion: 该框架为脑启发表示学习提供了严格的数学基础，通过拓扑结构和上同调流的相互作用实现了跨时空和功能状态的稳定表示，在拓扑驱动的表示学习领域具有重要理论价值。
...
### [200] [SPROCKET: Extending ROCKET to Distance-Based Time-Series Transformations With Prototypes](https://arxiv.org/abs/2512.08246)
*Nicholas Harner*

Main category: cs.LG

TL;DR: SPROCKET提出了一种基于原型的时间序列特征工程方法，在UCR和UEA数据集上性能与现有卷积算法相当，其集成模型MR-HY-SP超越了之前最好的卷积集成HYDRA-MR。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列分类算法主要依赖特征工程，其中ROCKET通过随机核特征取得了良好性能。本文旨在探索基于原型的特征工程策略，以提升时间序列分类的准确性和鲁棒性。

Method: 提出了SPROCKET（Selected Prototype Random Convolutional Kernel Transform）方法，采用基于原型的特征工程策略，通过选择原型来生成特征。

Result: 在大多数UCR和UEA时间序列分类数据集上，SPROCKET性能与现有卷积算法相当；新的MR-HY-SP集成模型（MultiROCKET-HYDRA-SPROCKET）的平均准确率排名超过了之前最好的卷积集成HYDRA-MR。

Conclusion: 实验结果表明，基于原型的特征变换能够增强时间序列分类的准确性和鲁棒性，为时间序列分类提供了新的有效方法。
...
### [201] [Wavelet-Accelerated Physics-Informed Quantum Neural Network for Multiscale Partial Differential Equations](https://arxiv.org/abs/2512.08256)
...
<details>
  <summary>Details</summary>
Motivation: 传统物理信息神经网络(PINNs)及其量子版本(量子PINNs)在求解多尺度特征时面临准确性挑战，且依赖自动微分构建损失函数导致计算开销大、训练时间长。

Method: 开发小波加速的物理信息量子神经网络，消除对自动微分的依赖，将小波的多分辨率特性融入量子神经网络架构，增强网络捕捉多尺度问题局部和全局特征的能力。

Result: 数值实验表明，该方法相比经典小波PINNs减少95%以上可训练参数，收敛更快；相比现有量子PINNs获得3-5倍加速，在求解多尺度和振荡问题时表现出优越精度。

Conclusion: 提出的基于小波的物理信息量子神经网络框架能高效求解具有挑战性的多尺度和振荡问题，在减少参数、加速收敛和提升精度方面具有显著优势。
...
### [202] [Geometric-Stochastic Multimodal Deep Learning for Predictive Modeling of SUDEP and Stroke Vulnerability](https://arxiv.org/abs/2512.08257)
*Preksha Girish,Rachana Mysore,Mahanthesha U,Shrey Kumar,Misbah Fatimah Annigeri,Tanish Jain*

Main category: cs.LG

TL;DR: 提出一个统一的几何-随机多模态深度学习框架，整合多种生理信号来建模癫痫猝死和卒中易感性


<details>
  <summary>Details</summary>
Motivation: 癫痫猝死和急性缺血性卒中是涉及皮层、脑干和自主神经系统复杂相互作用的危及生命疾病，需要整合多模态信号进行建模

Method: 结合黎曼流形嵌入、李群不变特征表示、分数随机动力学、哈密顿能量流建模和跨模态注意力机制，使用分数流行病扩散在结构脑图上建模卒中传播

Result: 在MULTI-CLARID数据集上展示改进的预测准确性和可解释的生物标志物，包括流形曲率、分数记忆指数、注意力熵和扩散中心性

Conclusion: 该框架为神经自主系统疾病的早期检测、风险分层和可解释多模态建模提供了数学原理基础
...
### [203] [Mathematical Foundations of Neural Tangents and Infinite-Width Networks](https://arxiv.org/abs/2512.08264)
...
<details>
  <summary>Details</summary>
Motivation: 研究无限宽度神经网络的数学基础，特别是神经正切核(NTK)的动态演化，旨在建立理论框架来理解神经网络训练过程中的核演化行为。

Method: 提出NTK-ECRN架构，整合傅里叶特征嵌入、带层间缩放的残差连接和随机深度，通过理论分析推导NTK动态边界，表征特征值演化，并将谱特性与泛化和优化稳定性联系起来。

Result: 在合成和基准数据集上的实证结果验证了预测的核行为，展示了改进的训练稳定性和泛化性能，为无限宽度理论与实际深度学习架构之间建立了桥梁。

Conclusion: 该工作提供了一个全面的框架，将无限宽度神经网络理论与实际深度学习架构联系起来，通过NTK-ECRN架构实现了对训练过程中核演化的严格分析。
...
### [204] [SOFA-FL: Self-Organizing Hierarchical Federated Learning with Adaptive Clustered Data Sharing](https://arxiv.org/abs/2512.08267)
*Yi Ni,Xinkun Wang,Han Zhang*

Main category: cs.LG

TL;DR: SOFA-FL是一个自组织层次联邦学习框架，通过动态聚类、拓扑重构和自适应数据共享解决数据异构性和固定拓扑问题。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在动态环境中面临数据异构性和固定网络拓扑的挑战，需要能够自适应变化的系统结构。

Method: 提出SOFA-FL框架，包含三个核心机制：DMAC（动态多分支凝聚聚类）构建初始层次结构；SHAPE（自组织层次自适应传播与演化）通过嫁接、剪枝、合并和净化操作动态重构拓扑；自适应聚类数据共享缓解数据异构性。

Result: 该框架能够有效捕捉客户端间的动态关系，增强个性化能力，无需依赖预定义的聚类结构。

Conclusion: SOFA-FL为动态环境中的联邦学习提供了自适应的层次化解决方案，能够应对数据分布变化和网络拓扑僵化问题。
...
### [205] [gHAWK: Local and Global Structure Encoding for Scalable Training of Graph Neural Networks on Knowledge Graphs](https://arxiv.org/abs/2512.08274)
*Humera Sabir,Fatima Farooq,Ashraf Aboulnaga*

Main category: cs.LG

TL;DR: gHAWK：针对大规模知识图谱的GNN训练框架，通过预计算结构特征（Bloom过滤器编码局部邻域、TransE嵌入编码全局位置）来提升训练效率、减少内存使用并提高准确性。


<details>
  <summary>Details</summary>
Motivation: 现有基于消息传递的图神经网络在处理大规模知识图谱时存在扩展性问题，因为迭代的消息传递过程效率低下，特别是在小批量训练中，节点只能看到部分邻域视图。

Method: 提出gHAWK框架，在GNN训练开始前预计算结构特征：1) 使用Bloom过滤器紧凑编码局部邻域结构；2) 使用TransE嵌入表示节点在图中全局位置。这些特征与领域特定特征融合后作为节点特征向量，可集成到任何GNN技术中。

Result: 在Open Graph Benchmark大规模数据集上的实验表明，gHAWK在节点属性预测和链接预测任务上实现了最先进的准确性，显著降低了训练时间，并在三个图谱的OGB排行榜上位居榜首。

Conclusion: 通过将结构先验与消息传递训练相结合，gHAWK显著减少了内存使用、加速了收敛并提高了模型准确性，为大规模知识图谱上的GNN训练提供了可扩展的解决方案。
...
### [206] [Jacobian Aligned Random Forests](https://arxiv.org/abs/2512.08306)
*Sarwesh Rauniyar*

Main category: cs.LG

TL;DR: JARF：通过计算随机森林预测的梯度，构建全局线性预处理器来旋转特征空间，使轴对齐决策树能处理旋转边界和特征交互，同时保持简单性和训练效率。


<details>
  <summary>Details</summary>
Motivation: 轴对齐决策树虽然快速稳定，但难以处理需要特征线性组合的旋转决策边界和特征交互问题。斜森林能解决但计算成本高且实现复杂。需要一种既能处理复杂边界又能保持轴对齐树简单性的方法。

Method: 1. 先训练轴对齐森林估计类别概率或回归输出；2. 计算预测相对于每个特征的有限差分梯度；3. 聚合梯度为期望雅可比外积（扩展EGOP）；4. 将其作为全局线性预处理器旋转特征空间；5. 在变换后的数据上重新训练标准轴对齐森林。

Result: 在表格分类和回归基准测试中，该方法持续改进轴对齐森林性能，通常匹配或超越斜森林基线，同时提高训练时间。理论和实验表明能恢复斜森林大部分精度，同时保持轴对齐树的简单性和鲁棒性。

Conclusion: 监督预处理器通过全局特征空间旋转，使轴对齐决策树能有效处理复杂决策边界，在保持简单训练流程的同时获得斜森林的优势，是实用且高效的折中方案。
...
### [207] [Minimizing Layerwise Activation Norm Improves Generalization in Federated Learning](https://arxiv.org/abs/2512.08314)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习中聚合的全局模型容易收敛到"尖锐最小值"，这会损害模型的泛化能力。需要改进联邦学习模型的泛化性能。

Method: 引入平坦性约束的FL优化问题，通过最小化Hessian矩阵最大特征值实现。提出MAN正则化技术，在客户端侧最小化每层激活范数，理论上证明这能减少Hessian最大特征值。

Result: 将平坦性约束优化应用于现有FL技术，获得了显著改进，建立了新的state-of-the-art。

Conclusion: MAN正则化技术能有效约束联邦学习优化问题的平坦性，通过最小化激活范数减少Hessian最大特征值，确保收敛到平坦最小值，显著提升模型泛化性能。
...
### [208] [A Multivariate Bernoulli-Based Sampling Method for Multi-Label Data with Application to Meta-Research](https://arxiv.org/abs/2512.08371)
*Simon Chung,Colby J. Vorland,Donna L. Maney,Andrew W. Brown*

Main category: cs.LG

TL;DR: 提出一种考虑标签依赖关系的多标签数据集采样算法，通过估计多元伯努利分布参数和计算标签组合权重，生成更平衡的子样本，改善少数类别的代表性。


<details>
  <summary>Details</summary>
Motivation: 多标签数据集中，标签通常不互斥且频率差异很大。为了对稀缺标签进行有效推断，需要获得偏离总体频率但保持已知偏差的样本，同时考虑标签间的依赖关系，这是一个具有挑战性的问题。

Method: 使用多元伯努利分布作为多标签问题的底层分布，利用观察到的标签频率估计分布参数，计算每个标签组合的权重，设计考虑标签依赖关系的加权采样算法，确保采样结果具有目标分布特性。

Result: 将方法应用于Web of Science的64个生物医学主题类别的研究文章样本，成功保持了类别频率顺序，减少了最常见和最不常见类别之间的频率差异，考虑了类别依赖关系，生成了更平衡的子样本，增强了少数类别的代表性。

Conclusion: 提出的考虑标签依赖关系的采样算法能够有效处理多标签数据集中的不平衡问题，通过加权采样生成更平衡的子样本，改善少数类别的表示，为多标签数据分析提供了实用的采样解决方案。
...
### [209] [Fully Decentralized Certified Unlearning](https://arxiv.org/abs/2512.08443)
*Hithem Lamri,Michail Maniatakos*

Main category: cs.LG

TL;DR: 提出RR-DU方法，用于去中心化网络中的认证遗忘，通过随机游走过程结合梯度操作和噪声注入，在保护隐私的同时有效移除指定数据的影响。


<details>
  <summary>Details</summary>
Motivation: 当前认证遗忘主要研究集中式和服务器协调的联邦学习设置，而去中心化网络（无协调器的对等通信）中的认证遗忘研究不足。需要开发适用于去中心化固定拓扑网络的认证遗忘方法。

Method: 提出RR-DU方法：1) 在遗忘客户端对遗忘集执行一步投影梯度上升；2) 在其他客户端对保留数据执行几何分布数量的投影下降步骤；3) 结合子采样高斯噪声；4) 投影到原始模型周围的信任区域。使用子采样高斯Rényi差分隐私提供网络遗忘认证。

Result: 1) 凸情况下的收敛保证和非凸情况下的平稳性保证；2) 通过子采样高斯RDP提供(ε,δ)网络遗忘认证；3) 删除容量界限随遗忘与本地数据比率缩放；4) 在MNIST和CIFAR-10上，RR-DU在满足相同(ε,δ)的同时，比去中心化DP基线获得更高的测试准确率，并将遗忘准确率降至随机猜测水平（约10%）。

Conclusion: RR-DU是首个在去中心化网络中提供认证遗忘的方法，有效平衡隐私与效用，在移除指定数据影响的同时保持模型性能，为去中心化环境中的隐私保护机器学习提供了新方案。
...
### [210] [Biothreat Benchmark Generation Framework for Evaluating Frontier AI Models II: Benchmark Generation Process](https://arxiv.org/abs/2512.08451)
...
TL;DR: 该论文提出了一种细菌生物威胁基准(B3)数据集生成方法，作为生物威胁基准生成框架的第二部分，包含1010个最终基准，用于评估AI模型在生物安全风险方面的能力。


<details>
  <summary>Details</summary>
Motivation: 前沿AI模型（特别是大语言模型）可能被用于生物恐怖主义或获取生物武器，这引起了政策界、学术界和公众的广泛关注。为了量化和减轻这种风险，需要开发能够评估特定模型生物安全风险的基准测试。

Method: 采用三种互补方法生成基准：1)基于网络的提示生成，2)红队测试，3)挖掘现有基准语料库。通过去重、提升诊断性评估和质量控制，从7000多个候选基准中筛选出1010个最终基准，并与项目第一阶段开发的任务查询架构相关联。

Result: 成功生成了包含1010个最终基准的B3数据集，这些基准具有以下特性：a)在提供提升方面具有诊断性；b)与生物安全威胁直接相关；c)与更大的生物安全架构对齐，允许在不同分析层次进行细致分析。

Conclusion: 该研究开发了一个系统化的生物威胁基准生成框架，为评估AI模型的生物安全风险提供了重要工具，有助于模型开发者和政策制定者更好地理解和减轻相关风险。
...
### [211] [Biothreat Benchmark Generation Framework for Evaluating Frontier AI Models III: Implementing the Bacterial Biothreat Benchmark (B3) Dataset](https://arxiv.org/abs/2512.08459)
...
TL;DR: B3数据集试点实施表明，该基准测试能有效评估LLM的生物安全风险，识别风险来源并提供缓解优先级指导。


<details>
  <summary>Details</summary>
Motivation: 前沿AI模型（特别是LLM）可能被用于生物恐怖主义或获取生物武器，这引发了政策、学术和公众的广泛担忧。需要量化并缓解这些风险，其中开发能够评估特定模型生物安全风险的基准测试是关键环节。

Method: 采用B3（Bacterial Biothreat Benchmark）数据集进行试点实施，这是生物威胁基准生成（BBG）框架的第三部分。试点包括：1）在样本前沿AI模型上运行基准测试；2）人工评估模型响应；3）从多个维度对结果进行应用风险分析。

Result: 试点证明B3数据集提供了一种可行且细致的方法，能够快速评估LLM的生物安全风险，识别关键风险来源，并为缓解措施的优先级提供指导。

Conclusion: B3基准测试框架为评估AI模型的生物安全风险提供了有效工具，有助于模型开发者和政策制定者量化风险并制定针对性缓解策略。
...
### [212] [Transformers for Multimodal Brain State Decoding: Integrating Functional Magnetic Resonance Imaging Data and Medical Metadata](https://arxiv.org/abs/2512.08462)
*Danial Jafarzadeh Jazi,Maryam Hajiesmaeili*

Main category: cs.LG

TL;DR: 提出整合fMRI数据和DICOM元数据的Transformer框架，通过注意力机制提升脑状态解码的准确性和可解释性
...
Method: 采用基于Transformer的多模态架构，同时处理fMRI数据和DICOM元数据，利用注意力机制捕捉时空模式和上下文关系

Result: 提出的框架提升了模型准确性、可解释性和鲁棒性，在临床诊断、认知神经科学和个性化医疗中具有应用潜力

Conclusion: 整合多模态数据的Transformer方法为fMRI分析提供了新方向，但需解决元数据变异性和计算需求等挑战
...
### [213] [Solving Over-Smoothing in GNNs via Nonlocal Message Passing: Algebraic Smoothing and Depth Scalability](https://arxiv.org/abs/2512.08475)
*Weiqi Guan,Junlin He*

Main category: cs.LG

TL;DR: 论文提出了一种解决Layer Normalization位置困境的新方法：基于Post-LN架构诱导代数平滑，避免过平滑和深度诅咒，支持更深的网络（达256层）且无需额外参数。
...
Method: 提出一种基于Post-LN的新方法，通过诱导代数平滑来防止过平滑，同时避免深度诅咒。该方法参数高效，不需要额外参数。

Result: 在五个基准测试上的实证结果表明，该方法支持更深的网络（最多256层）并提高性能，无需额外参数。

Conclusion: 成功解决了LN放置位置的困境，提出的方法既能避免过平滑又能绕过深度诅咒，为构建更深层的图神经网络提供了有效的解决方案。
...
### [214] [Optimal Perturbation Budget Allocation for Data Poisoning in Offline Reinforcement Learning](https://arxiv.org/abs/2512.08485)
*Junnan Qiu,Jie Li*
...
<details>
  <summary>Details</summary>
Motivation: 现有离线RL数据投毒攻击采用均匀扰动策略效率低下且容易被检测，需要更高效、隐蔽的攻击方法

Method: 基于TD误差理论分析，将攻击建模为全局资源分配问题，推导出在L2约束下按TD误差敏感性比例分配扰动幅度的闭式解

Result: 在D4RL基准测试中显著优于基线方法，仅用最小扰动就能实现高达80%的性能下降，并能逃避最先进的统计和频谱防御检测

Conclusion: 提出的全局预算分配攻击策略比均匀扰动更高效、更隐蔽，揭示了离线RL系统在数据投毒攻击下的新脆弱性
...
### [215] [Developing Distance-Aware Uncertainty Quantification Methods in Physics-Guided Neural Networks for Reliable Bearing Health Prediction](https://arxiv.org/abs/2512.08499)
*Waleed Razzaq,Yun-Bo Zhao*

Main category: cs.LG

TL;DR: 提出了两种基于确定性物理引导神经网络的距离感知不确定性方法（PG-SNGP和PG-SNER），用于旋转机械轴承退化预测，在OOD条件下表现更好且具有距离感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有不确定性方法在安全关键系统（如旋转机械轴承）的预测性维护中存在不足：缺乏置信度校准、计算成本高、无距离感知能力、在分布外数据下泛化能力差。

Method: 提出了两种距离感知不确定性方法：PG-SNGP（基于谱归一化高斯过程）和PG-SNER（基于深度证据回归）。在隐藏层应用谱归一化以保持输入到潜在空间的距离，PG-SNGP用高斯过程层替换最终密集层，PG-SNER输出Normal Inverse Gamma参数。设计了基于Pearson相关系数的新距离感知度量，并在损失函数中采用动态加权方案平衡数据保真度和物理一致性。

Result: 在PRONOSTIA数据集上的实验表明，PG-SNGP和PG-SNER相比蒙特卡洛和深度集成PGNNs提高了预测精度，在OOD条件下可靠泛化，对对抗攻击和噪声保持鲁棒性。

Conclusion: 提出的距离感知不确定性方法能够为安全关键系统的预测性维护提供准确且具有校准不确定性的退化估计，在分布外条件下表现优异。
...
### [216] [A Hybrid Model for Stock Market Forecasting: Integrating News Sentiment and Time Series Data with Graph Neural Networks](https://arxiv.org/abs/2512.08567)
*Nader Sadek,Mirette Moawad,Christina Naguib,Mariam Elzahaby*

Main category: cs.LG

TL;DR: 该研究提出了一种多模态方法，将公司新闻文章与历史股票数据结合，使用图神经网络（GNN）预测股价走势，相比传统LSTM基线模型有显著提升。


<details>
  <summary>Details</summary>
Motivation: 股票市场预测是金融领域的长期挑战，传统模型主要依赖历史价格数据，但金融新闻能提供有用的外部信号。本研究旨在探索结合新闻文章和历史数据的多模态方法，以提高预测性能。

Method: 采用图神经网络（GNN）模型，将公司历史数据通过LSTM编码，新闻标题通过语言模型嵌入。这些嵌入形成异构图的节点，使用GraphSAGE捕捉文章、公司和行业之间的交互关系。评估两个目标：二元方向变化标签和基于重要性的标签。

Result: 在US equities和Bloomberg数据集上的实验表明，GNN模型优于LSTM基线，在第一个目标上达到53%准确率，在第二个目标上获得4%的精确度提升。有更多相关新闻的公司预测准确率更高，新闻标题比完整文章包含更强的预测信号。

Conclusion: 多模态方法结合新闻和历史数据能有效提升股票市场预测性能，图神经网络能有效捕捉不同实体间的复杂关系。简洁的新闻摘要在短期市场反应中起重要作用，为金融预测提供了新的研究方向。
...
### [217] [Long-Sequence LSTM Modeling for NBA Game Outcome Prediction Using a Novel Multi-Season Dataset](https://arxiv.org/abs/2512.08591)
*Charles Rios,Longzhen Han,Almas Baimagambetov,Nikolaos Polatidis*

Main category: cs.LG

TL;DR: 该研究构建了覆盖2004-2024赛季的NBA纵向数据集，并开发了基于LSTM的深度学习框架，通过长达8个赛季（9840场比赛）的序列长度来捕捉球队长期动态变化，在篮球比赛结果预测中取得了最佳性能。
...
Method: 构建了2004-05至2024-25赛季的纵向NBA数据集，设计了基于LSTM的深度学习框架，使用长达8个完整赛季（9840场比赛）的序列长度来捕捉球队动态演变和赛季间依赖关系，并与逻辑回归、随机森林、MLP、CNN等传统方法进行对比。

Result: LSTM模型在所有指标上表现最佳：准确率72.35%，精确率73.15%，AUC-ROC 76.13%，显著优于其他基线模型，证明了长序列时间建模在篮球结果预测中的重要性。

Conclusion: 研究证明了长序列LSTM在捕捉NBA球队长期动态变化方面的有效性，新构建的多赛季数据集对于开发稳健、可泛化的NBA预测系统具有重要价值，为篮球结果预测提供了新的方法框架。
...
### [218] [DS FedProxGrad: Asymptotic Stationarity Without Noise Floor in Fair Federated Learning](https://arxiv.org/abs/2512.08671)
*Huzaifa Arif*

Main category: cs.LG

TL;DR: 改进FedProxGrad算法的收敛性分析，证明在衰减步长下能达到渐近平稳点，消除方差噪声的影响


<details>
  <summary>Details</summary>
Motivation: 原始FedProxGrad算法在非凸复合优化问题中只能收敛到噪声主导的平稳邻域，收敛结果依赖于方差诱导的噪声底限，需要改进分析以获得更好的收敛性质

Method: 提出DS FedProxGrad（衰减步长FedProxGrad）分析框架，采用Robbins-Monro步长调度，考虑局部不精确近端解和显式公平正则化，在局部不精确性衰减条件下进行分析

Result: 证明liminf_{r→∞} E[‖∇F(x^r)‖²] = 0，即算法渐近平稳，收敛速率不依赖于方差诱导的噪声底限

Conclusion: 通过改进的收敛分析，DS FedProxGrad在衰减步长下能够达到真正的渐近平稳点，克服了原始FedProxGrad的噪声限制问题
...
### [219] [An Additive Manufacturing Part Qualification Framework: Transferring Knowledge of Stress-strain Behaviors from Additively Manufactured Polymers to Metals](https://arxiv.org/abs/2512.08699)
*Chenglong Duan,Dazhong Wu*

Main category: cs.LG

TL;DR: 开发基于动态时间规整和迁移学习的框架，将低成本聚合物应力-应变行为知识迁移到金属材料，用于增材制造零件认证


<details>
  <summary>Details</summary>
Motivation: 增材制造零件认证需要准确预测复杂应力-应变行为，但金属材料数据获取成本高。通过将低成本聚合物的知识迁移到金属材料，可以降低认证成本并提高预测准确性。

Method: 提出DTW-TL框架：使用动态时间规整选择与目标金属数据集最相关的聚合物源域，然后采用长短期记忆模型进行迁移学习。实验使用四种聚合物（尼龙、PLA、CF-ABS、树脂）和三种金属（AlSi10Mg、Ti6Al4V、碳钢）。

Result: DTW-TL框架成功识别聚合物与金属之间的最佳匹配，选择单一聚合物作为源域。在三种金属作为目标域时，模型达到最低平均绝对百分比误差12.41%和最高决定系数0.96，优于无迁移学习的LSTM模型以及使用四种聚合物预训练的迁移学习模型。

Conclusion: DTW-TL框架能够有效将聚合物应力-应变行为知识迁移到金属材料，为增材制造零件认证提供了一种经济高效的解决方案，显著提高了预测性能。
...
### [220] [Exposing Hidden Biases in Text-to-Image Models via Automated Prompt Search](https://arxiv.org/abs/2512.08724)
*Manos Plitsis,Giorgos Bouritsas,Vassilis Katsouros,Yannis Panagakis*

Main category: cs.LG

TL;DR: 本文提出Bias-Guided Prompt Search (BGPS)框架，自动生成能最大化文本到图像生成模型中偏见的提示词，发现现有去偏方法仍存在大量未记录的细微偏见。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像扩散模型存在社会偏见，现有去偏方法依赖人工或LLM构建的提示词数据集，成本高且可能遗漏未预见的偏见触发提示。需要自动发现能暴露模型偏见的提示词。

Method: BGPS框架包含两部分：1) 生成属性中性提示词的LLM；2) 作用于TTI内部表示的属性分类器，引导LLM解码过程向能放大目标图像属性的提示空间区域发展。

Result: 在Stable Diffusion 1.5和最先进去偏模型上的实验发现了大量细微且未记录的偏见，严重损害公平性指标。发现的提示词可解释且可由普通用户输入，困惑度指标优于硬提示优化方法。

Conclusion: BGPS揭示了TTI模型的脆弱性，扩展了偏见搜索空间，可作为偏见缓解的新评估工具，帮助发现现有去偏方法遗漏的细微偏见。
...
### [221] [Neural Ordinary Differential Equations for Simulating Metabolic Pathway Dynamics from Time-Series Multiomics Data](https://arxiv.org/abs/2512.08732)
*Udesh Habaraduwa,Andrei Lixandru*

Main category: cs.LG

TL;DR: 该论文提出使用神经常微分方程（NODEs）作为动态框架，从蛋白质组和代谢组的时间序列数据中学习复杂相互作用，相比传统机器学习方法在预测精度和推理速度上都有显著提升。


<details>
  <summary>Details</summary>
Motivation: 虽然高通量多组学数据日益丰富，但将这些数据转化为可操作的预测模型仍然是一个瓶颈。需要高容量、数据驱动的模拟系统来推断潜在相互作用，实现个性化医疗和合成生物学中的下游干预效果预测。

Method: 引入神经常微分方程（NODEs）作为动态框架，应用于工程化大肠杆菌菌株的时间序列数据，建模代谢途径的连续动力学。该架构直接从观测数据中推断潜在相互作用。

Result: NODE架构在捕捉系统动力学方面表现优异：在柠檬烯途径数据集上RMSE改善高达94.38%，在异戊烯醇途径数据集上改善高达97.65%。推理时间加速1000倍。

Conclusion: NODE模型为下一代代谢工程和生物发现提供了一个可扩展、高保真度的工具，能够有效预测复杂生物系统的行为。
...
### [222] [Learning and Editing Universal Graph Prompt Tuning via Reinforcement Learning](https://arxiv.org/abs/2512.08763)
...
<details>
  <summary>Details</summary>
Motivation: 现有选择性节点图提示调优方法会损害通用图提示调优的理论基础，需要一种既能保持理论基础又能追求更理想提示的新方法。

Method: LEAP首先构建基本通用图提示以保持理论基础，然后采用演员-评论家强化学习来选择节点并编辑提示。

Result: 在多种预训练策略下的图级和节点级任务中，无论是全样本还是少样本场景，LEAP都持续优于微调和其他基于提示的方法。

Conclusion: 在所有节点添加提示是实现图提示通用性的必要条件，LEAP通过结合理论基础和强化学习实现了更优的图提示调优。
...
### [223] [De novo generation of functional terpene synthases using TpsGPT](https://arxiv.org/abs/2512.08772)
*Hamsini Ramanathan,Roman Bushuiev,Matouš Soldát,Jirí Kohout,Téo Hebra,Joshua David Smith,Josef Sivic,Tomáš Pluskal*

Main category: cs.LG

TL;DR: TpsGPT是基于ProtGPT2微调生成的萜烯合酶设计模型，通过多指标验证筛选出功能性酶，实验证实了其有效性。


<details>
  <summary>Details</summary>
Motivation: 萜烯合酶是天然产物合成的关键酶，但传统定向进化方法成本高、速度慢，需要开发更高效的蛋白质设计方法。

Method: 在UniProt中挖掘79k个TPS序列，微调蛋白质语言模型ProtGPT2得到TpsGPT，生成候选序列后使用酶分类、结构预测、序列多样性、结构比对等多重验证指标进行筛选。

Result: 从28k生成序列中筛选出7个候选酶，实验验证至少2个具有TPS酶活性，证明该方法能生成功能性、进化距离较远的酶。

Conclusion: 在精心策划的酶类特定数据集上微调蛋白质语言模型，结合严格筛选，能够实现功能性酶的全新设计。
...
### [224] [Can TabPFN Compete with GNNs for Node Classification via Graph Tabularization?](https://arxiv.org/abs/2512.08798)
...
<details>
  <summary>Details</summary>
Motivation: 基于TabPFN在表格数据和时间序列上的成功，研究是否可以将图节点分类有效地重新表述为表格学习问题，为图学习提供无需图特定训练或语言模型依赖的替代方案

Method: TabPFN-GN方法：将图数据转换为表格特征，提取节点属性、结构属性、位置编码，以及可选的平滑邻域特征，使TabPFN能够直接进行节点分类

Result: 在12个基准数据集上的实验显示，TabPFN-GN在同配图上与GNNs表现相当，在异配图上始终优于GNNs

Conclusion: 原则性特征工程可以弥合表格和图领域之间的差距，为任务特定的GNN训练和LLM依赖的图基础模型提供实用的替代方案
...
### [225] [Identifying counterfactual probabilities using bivariate distributions and uplift modeling](https://arxiv.org/abs/2512.08805)
*Théo Verhelst,Gianluca Bontempi*

Main category: cs.LG

TL;DR: 提出一种基于提升模型的双变量Beta分布反事实估计方法，无需额外因果假设即可获得反事实结果的后验分布


<details>
  <summary>Details</summary>
Motivation: 提升模型只能估计因果效应（处理组与对照组的差异），而反事实识别需要恢复潜在结果的联合分布（如"如果给客户营销优惠，他们是否仍会流失"）。反事实分布提供更丰富信息但更难估计，两者具有协同作用

Method: 提出一种反事实估计器，将双变量Beta分布拟合到预测的提升分数上，从而获得反事实结果的后验分布。该方法除了提升模型所需的因果假设外，不需要额外的因果假设

Result: 模拟实验显示该方法有效，可应用于电信客户流失等问题，揭示标准机器学习或单独提升模型无法获得的洞察

Conclusion: 提升模型可用于反事实估计，提出的双变量Beta分布方法能够有效估计反事实结果的联合分布，为决策提供更丰富的信息
...
### [226] [Forecasting Fails: Unveiling Evasion Attacks in Weather Prediction Models](https://arxiv.org/abs/2512.08832)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI模型在天气预报中的日益依赖，评估其对对抗性扰动的脆弱性变得至关重要。需要研究如何生成既能有效操纵预测又能避免检测的对抗性扰动。

Method: 提出了Weather Adaptive Adversarial Perturbation Optimization (WAAPO)框架，该框架通过通道稀疏性、空间局部性和平滑性约束来生成对抗性扰动，确保扰动在物理上合理且难以察觉。使用ERA5数据集和FourCastNet模型进行实验验证。

Result: WAAPO能够生成与预定目标紧密对齐的对抗性轨迹，即使在约束条件下也能成功。实验表明，对初始条件的微小扰动可以导致预测天气模式的显著偏差，揭示了AI驱动预报模型的关键漏洞。

Conclusion: AI天气预报模型存在严重的安全漏洞，微小的对抗性扰动可能导致预测结果的重大偏差。这强调了在业务预报系统中需要建立强大的安全防护措施来防止对抗性攻击。
...
### [227] [Reinforcement Learning From State and Temporal Differences](https://arxiv.org/abs/2512.08855)
*Lex Weaver,Jonathan Baxter*

Main category: cs.LG

TL;DR: 论文提出STD(λ)算法，改进TD(λ)在函数逼近中的策略优化问题，通过关注状态相对值而非绝对值来避免收敛到次优策略。


<details>
  <summary>Details</summary>
Motivation: TD(λ)在函数逼近中虽然能最小化状态值的平方误差，但策略优化的关键是状态的相对排序而非绝对值。作者发现TD(λ)即使从最优策略开始，也可能收敛到次优策略，这在双状态、三状态系统和西洋双陆棋中都有体现。

Method: 提出改进的STD(λ)算法，在二元决策问题中训练函数逼近器关注状态的相对值而非绝对值。该方法基于相对状态值进行训练，并在双状态系统中证明了策略单调改进的理论保证。

Result: STD(λ)在双状态系统和改进的acrobot问题上成功演示，避免了TD(λ)收敛到次优策略的问题。理论分析包括STD(λ)在双状态系统中的单调策略改进证明，并与Bertsekas的差分训练方法进行了比较。

Conclusion: STD(λ)通过关注状态相对值而非绝对值，解决了TD(λ)在函数逼近中可能收敛到次优策略的问题，为强化学习中的策略优化提供了更有效的方法。
...
### [228] [Refining Diffusion Models for Motion Synthesis with an Acceleration Loss to Generate Realistic IMU Data](https://arxiv.org/abs/2512.08859)
*Lars Ole Häusler,Lena Uhlenberg,Göran Köber,Diyora Salimova,Oliver Amft*

Main category: cs.LG

TL;DR: 提出基于加速度二阶损失的文本到IMU运动合成框架，通过微调预训练扩散模型提升IMU数据真实性和HAR分类性能
...
Method: 在预训练扩散模型中引入加速度二阶损失(L_acc)，强制生成运动在离散二阶时间差分上的一致性，然后通过表面建模和虚拟传感器模拟生成IMU数据

Result: L_acc损失相对原始模型降低12.7%，高动态活动改进更明显；合成IMU数据分布更接近真实记录；HAR分类性能提升8.7%

Conclusion: 加速度感知的扩散模型细化有效对齐运动生成与IMU合成，展示了深度学习管道在将通用文本到运动先验专门化到传感器特定任务中的灵活性
...
### [229] [Differentially Private Synthetic Data Generation Using Context-Aware GANs](https://arxiv.org/abs/2512.08869)
...
<details>
  <summary>Details</summary>
Motivation: 大数据应用中隐私保护与数据效用之间存在矛盾，现有合成数据方法往往无法捕捉领域特定的隐式规则（如医疗中的处方指南），导致生成的数据不现实或不安全。

Method: 提出ContextGAN，通过约束矩阵编码显式和隐式领域知识，使用约束感知的判别器评估合成数据是否符合领域规则，同时结合差分隐私保护原始数据的敏感信息。

Result: 在医疗、安全和金融领域验证表明，ContextGAN能生成高质量合成数据，既尊重领域规则又保护隐私，相比传统方法显著提高了数据的真实性和实用性。

Conclusion: ContextGAN通过整合领域约束和差分隐私，解决了合成数据生成中隐私保护与领域规则遵守的平衡问题，适用于需要严格隐私保证和领域合规的应用场景。
...
### [230] [Fed-SE: Federated Self-Evolution for Privacy-Constrained Multi-Environment LLM Agents](https://arxiv.org/abs/2512.08870)
*Xiang Chen,Yuling Shi,Qizhen Lan,Yuchao Qiu,Xiaodong Gu*

Main category: cs.LG

TL;DR: Fed-SE：一个用于LLM智能体的联邦自进化框架，通过局部进化-全局聚合范式解决隐私约束下智能体在异构任务中的协同优化问题，相比联邦基线提升约18%的任务成功率。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在复杂交互任务中广泛应用，但隐私约束限制了集中式优化和动态环境中的协同进化。虽然联邦学习在静态数据集上有效，但在智能体的开放式自进化方面仍待探索。直接应用标准联邦学习存在挑战：异构任务和稀疏的轨迹级奖励会导致严重的梯度冲突，破坏全局优化过程。

Method: 提出Fed-SE框架，采用局部进化-全局聚合范式。局部层面：智能体在过滤的高回报轨迹上进行参数高效微调，实现稳定的梯度更新。全局层面：在低秩子空间内聚合更新，解耦环境特定动态，有效减少客户端间的负迁移。

Result: 在五个异构环境中的实验表明，Fed-SE相比联邦基线平均任务成功率提升约18%，验证了其在隐私约束部署中实现鲁棒跨环境知识转移的有效性。

Conclusion: Fed-SE成功解决了隐私约束下LLM智能体在异构任务中的联邦自进化问题，通过局部稳定优化和全局解耦聚合实现了有效的跨环境知识转移，为隐私敏感场景下的智能体协同进化提供了可行方案。
...
### [231] [When Tables Leak: Attacking String Memorization in LLM-Based Tabular Data Generation](https://arxiv.org/abs/2512.08875)
*Joshua Ward,Bochao Gu,Chi-Hua Wang,Guang Cheng*

Main category: cs.LG

TL;DR: LLM生成表格数据时存在隐私泄露风险，数字序列易被记忆并复现，提出LevAtt攻击方法揭示此漏洞，并提出防御策略
...
Method: 提出LevAtt黑盒成员推理攻击，仅基于生成的合成数据，针对数字字符串序列进行攻击分析；同时提出两种防御方法，包括在生成过程中策略性扰动数字的新型采样策略

Result: 攻击揭示了广泛的隐私泄露，在某些最先进模型上甚至成为完美的成员分类器；提出的防御方法能以最小的保真度和效用损失击败这些攻击

Conclusion: LLM基于的合成数据生成存在独特的隐私漏洞，需要有效防御措施；提出的防御策略能有效缓解隐私风险
...
### [232] [DAO-GP Drift Aware Online Non-Linear Regression Gaussian-Process](https://arxiv.org/abs/2512.08879)
*Mohammad Abu-Shaira,Ajita Rattani,Weishi Shi*

Main category: cs.LG

TL;DR: DAO-GP是一种新型的漂移感知在线高斯过程模型，能够自适应处理数据分布变化，无需手动调整超参数，具有漂移检测和适应机制。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据集常呈现随时间变化的数据分布（概念漂移），忽视这一问题会显著降低模型预测精度。传统在线高斯过程方法存在多个关键限制：缺乏漂移感知能力、依赖固定超参数、易受数据窥探影响、缺乏原则性衰减机制和内存效率低下。

Method: 提出DAO-GP（漂移感知在线高斯过程），这是一种完全自适应、无超参数、衰减和稀疏的非线性回归模型。模型内置漂移检测和适应机制，能根据漂移严重程度动态调整模型行为。

Result: 广泛的实证评估证实DAO-GP在平稳条件、多种漂移类型（突然、增量、渐进）和不同数据特征下都具有鲁棒性。分析显示其具有动态适应能力、高效的内存和衰减管理以及演化诱导点。与最先进的参数和非参数模型相比，DAO-GP始终实现优越或竞争性能。

Conclusion: DAO-GP被确立为在线非线性回归的漂移弹性解决方案，能够有效处理概念漂移问题，无需手动超参数调整，具有实际应用价值。
...
### [233] [Explainable Anomaly Detection for Industrial IoT Data Streams](https://arxiv.org/abs/2512.08885)
*Ana Rita Paupério,Diogo Risca,Afonso Lourenço,Goreti Marreiros,Ricardo Martins*

Main category: cs.LG

TL;DR: 提出一个结合无监督异常检测与人在回路学习的协作式数据流挖掘框架，用于工业维护决策支持


<details>
  <summary>Details</summary>
Motivation: 工业维护中物联网和边缘计算产生连续数据流，需要实时自适应决策，但实际应用中地面真实标签往往延迟或不可得，现有数据流挖掘方法大多假设完全监督设置

Method: 采用在线隔离森林进行无监督异常检测，结合增量部分依赖图和特征重要性评分增强可解释性，特征重要性基于个体条件期望曲线与衰减平均值的偏差计算，支持用户动态重新评估特征相关性和调整异常阈值

Result: 描述了实时实现，并提供了在提花织机单元故障检测的初步结果

Conclusion: 该协作框架整合无监督异常检测与交互式人在回路学习，支持工业维护决策，正在进行的工作旨在通过连续监测预测和解释即将发生的轴承故障
...
### [234] [Revisiting the Scaling Properties of Downstream Metrics in Large Language Model Training](https://arxiv.org/abs/2512.08894)
*Jakub Krajewski,Amitis Shidani,Dan Busbridge,Sam Wiseman,Jason Ramapuram*

Main category: cs.LG

TL;DR: 本文提出了一种直接建模基准性能随训练预算缩放的方法，挑战了传统上认为下游任务性能难以预测的观点。


<details>
  <summary>Details</summary>
Motivation: 传统的大语言模型缩放定律主要关注预训练损失等代理指标，而预测下游任务性能被认为不可靠。本文旨在挑战这一观点，探索直接建模基准性能随训练预算缩放的可能性。

Method: 提出一个直接框架，在固定token-参数比的情况下，使用简单的幂律来描述多个下游任务的对数准确率缩放行为。引入能够跨token-参数比预测准确率并考虑重复采样下推理计算的功能形式。

Result: 直接方法比之前提出的两阶段程序外推效果更好，后者容易产生复合误差。在高达170亿参数、3500亿token的两个数据集混合上验证了发现。

Conclusion: 下游任务性能可以通过训练预算直接建模，且直接方法优于传统两阶段方法。为支持可复现性和未来研究，发布了完整的预训练损失和下游评估结果。
...
### [235] [Unsupervised Learning of Density Estimates with Topological Optimization](https://arxiv.org/abs/2512.08895)
*Suina Tanweer,Firas A. Khasawneh*

Main category: cs.LG

TL;DR: 提出一种基于拓扑数据分析的损失函数，用于无监督选择核密度估计的最优带宽，并在不同维度上验证其效果


<details>
  <summary>Details</summary>
Motivation: 核密度估计中的带宽选择是关键但困难的问题，传统方法需要人工调参。拓扑数据分析能够量化高维数据的拓扑特征，为无监督带宽选择提供了新思路

Method: 使用基于拓扑的损失函数进行无监督学习，自动选择最优带宽。该方法利用拓扑数据分析量化密度估计的拓扑特征，如连通分量、环、空洞等

Result: 提出的方法在不同维度上进行了基准测试，展示了其潜力，能够有效选择带宽并控制偏差-方差权衡

Conclusion: 基于拓扑的损失函数为核密度估计的带宽选择提供了一种有效的无监督方法，特别适用于高维数据可视化困难的情况
...
### [236] [Open Polymer Challenge: Post-Competition Report](https://arxiv.org/abs/2512.08896)
...
<details>
  <summary>Details</summary>
Motivation: 机器学习在发现可持续聚合物材料方面潜力巨大，但进展受到缺乏大规模、高质量、开放可访问的聚合物数据集的限制。
