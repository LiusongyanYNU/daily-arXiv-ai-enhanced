...
Method: 系统包含三个步骤：(i) 将专有360度格式标准化为标准视图并分片并行处理；(ii) 应用AI模型套件进行密集多模态预标注；(iii) 将信号整合为结构化输出规范供人工快速验证。

Result: GAZE工作流显著提升效率（每审核小时节省约19分钟），通过保守自动跳过低显著性片段减少人工审核量80%以上，生成高保真、隐私感知的数据集。

Conclusion: 该方法提供了可扩展的蓝图，可在不牺牲吞吐量或治理的情况下生成高质量的世界模型训练数据，直接用于学习跨模态动态和动作条件预测。
...
<details>
  <summary>Details</summary>
Motivation: PET临床应用中，高信噪比剂量会增加辐射暴露，而降低剂量会引入泊松噪声，现有去噪方法无法有效处理，导致图像失真和伪影。

Method: 开发Poisson Consistent U-Net模型，引入新的Poisson Variance and Mean Consistency Loss，该损失函数在方差和梯度适应方面具有统计无偏性，作为广义矩方法的实现，对数据轻微失配具有鲁棒性。

Result: 在PET数据集上的测试表明，PC-UNet提高了物理一致性和图像保真度。

Conclusion: PC-UNet能够有效整合物理信息，改善PET图像质量。
...
Method: DeLeaker在扩散过程中动态重新加权注意力图，抑制过度的跨实体交互，同时增强每个实体的身份特征。该方法无需优化过程。
...
<details>
  <summary>Details</summary>
Motivation: 城市AI代理训练需要多样化、高保真的城市环境，但现有仿真场景要么缺乏可扩展性，要么无法捕捉真实世界的复杂性。

Method: UrbanVerse系统包括UrbanVerse-100K（10万+标注城市3D资产库）和UrbanVerse-Gen（从视频提取场景布局并使用检索资产实例化度量尺度3D仿真的自动管线）。

Result: UrbanVerse场景保持真实世界语义和布局，在仿真中成功率提升+6.3%，在零样本仿真到真实迁移中提升+30.1%，在300米真实世界任务中仅需两次干预。

Conclusion: UrbanVerse通过数据驱动方法有效解决了城市仿真场景的可扩展性和真实性问题，为城市AI代理训练提供了高质量环境。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D物体编辑方法效率低下、不一致，且难以保护未编辑区域。大多数方法依赖多视图渲染编辑后再重建，会产生伪影且实用性有限。

Method: 将FlowEdit集成到TRELLIS中，通过前视图渲染引导局部编辑，引入区域感知合并策略Voxel/Slat-Merge，自适应保持结构保真度。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像生成基准滞后于技术进步，无法捕捉新兴使用场景，导致社区认知与正式评估之间存在差距。

Method: 从展示新颖提示和定性用户判断的社交媒体帖子中直接构建基准，收集了GPT-4o Image Gen的31,000多个提示。

Result: ECHO发现了现有基准中缺失的创意复杂任务，能更清晰地区分最先进模型，并通过社区反馈为质量指标设计提供信息。

Conclusion: ECHO框架通过真实世界使用证据构建基准，能更好地评估图像生成模型的新兴能力，弥合社区认知与正式评估之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: LoRA模型虽然能个性化预训练扩散模型，但平台上超过10万个适配器缺乏结构化组织，用户难以导航和选择最适合的模型。

Method: 将LoRA模型选择问题构建为组合优化问题，并提出了新颖的子模框架来解决。

Result: 定量和定性实验表明，该方法能在广泛领域中生成多样化的输出。

Conclusion: 所提出的子模框架能有效解决海量LoRA适配器的选择和多样化问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有基础模型虽然性能优异，但计算成本过高限制了在资源受限平台上的应用。需要设计能够高效部署且不牺牲性能的架构。

Method: 提出瓶颈像素解码器用于高效多尺度多模态融合；语言引导不确定性校准损失用于自适应解码器剪枝；统一的训练策略简化训练过程。

Result: MOBIUS将像素和transformer解码器的FLOPs分别降低55%和75%，仅用三分之一训练迭代次数就能保持SOTA性能。

Conclusion: MOBIUS为高性能计算平台和移动设备上的高效分割设立了新基准，实现了Pareto最优的缩放能力。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在缺乏大规模人工标注推理数据的人工图像领域（如图表、渲染文档、网页）中推理能力有限。

Method: 将种子问题分解为原始感知和推理因素，然后与新的图像系统重组生成合成问答对，并配备子问题和中间答案，支持因子级过程奖励的强化学习。

Result: 在图表推理实验中，COGS显著提高了未见问题的性能，特别是在推理密集和组合问题上表现最佳。因子级混合训练在多个数据集上实现了更好的迁移效果。

Conclusion: COGS框架能够诱导出可泛化的能力而非数据集特定的过拟合，并可扩展到图表之外的其他领域如网页。
...
Method: 从势能角度出发，将世界视为整体实体，引入方向刚度扩展弹性动力学，使用专门网络建模材料属性，神经场表示几何无关的变形。

Result: 实验表明GDGen能稳健统一多种仿真范式，为创建交互式虚拟环境和训练机器人代理提供多功能基础。

Conclusion: GDGen通过势能视角成功统一了多种物理系统动力学，为复杂动态场景中的虚拟环境和机器人训练提供了通用框架。
...
<details>
  <summary>Details</summary>
Motivation: 3D医学影像领域的数据可用性限制了当前视觉语言编码器的能力，需要解决数据不足的问题以提升模型性能。
...
Conclusion: 通过引入额外归纳偏置和利用更多数据，成功开发出在多个3D医学影像任务上表现优异的视觉语言编码器。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的推理能力落后于纯文本模型，现有方法需要大量资源，模型合并的效果因模型而异且不稳定。

Method: DRIFT方法预计算推理先验作为参数空间差异，在微调过程中用其偏置梯度，实现推理知识转移而不破坏多模态对齐。

Result: 在MathVista和MathVerse等基准测试中，DRIFT一致优于简单合并和监督微调，以较低成本达到或超过资源密集型方法的性能。

Conclusion: DRIFT提供了一种轻量级且有效的解决方案，能够高效转移推理知识，提升多模态模型的推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法在处理长视频序列时存在大量冗余和几何不一致性问题，需要解决长期重建中的效率和一致性问题。

Method: 使用紧凑锚点基元通过可微分显著性感知高斯量化消除冗余，结合3D点Transformer优化锚点属性和显著性，实现区域自适应高斯解码。

Result: 在多个数据集上实现了最先进的性能，在50+视图上达到10+FPS，冗余去除率达50%-90%，在新型视图合成和深度估计方面表现优异。

Conclusion: 该方法无需已知相机参数或测试时优化，在单次前向传播中有效解决伪影并修剪冗余3DGS，展示了长期通用3D重建的优越效率、鲁棒性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成方法在控制场景中多个对象的组合时存在困难，特别是在复杂多对象设置下，缺乏轨迹与视觉实体之间的明确对应关系。

Method: 提出Location-Aware Cross-Attention (LACA)来整合轨迹和文本信号，采用双CFG方案分别调制局部和全局文本指导，并开发了数据流水线来生成带局部描述的轨迹数据。

Result: TGT在视觉质量、文本对齐准确性和运动可控性方面优于现有方法，能够使用点轨迹作为直观的运动控制手柄。

Conclusion: TGT框架通过轨迹与文本的配对，有效提升了视频生成中对多个对象外观和运动的精确控制能力。
...
<details>
  <summary>Details</summary>
Motivation: 结合数据驱动的扩散模型与医学成像领域的知识，解决传统贝叶斯逆问题中经典数学先验无法捕捉大脑解剖复杂结构的问题。

Method: 使用基于分数的扩散先验，在多样化的脑MRI数据上进行训练，配合灵活的向前模型来处理超分辨率、偏置场校正、修复等常见图像处理任务。

Result: 在异构临床和研究MRI数据上的实验表明，该方法无需配对训练数据集就能产生一致、高质量的解决方案，达到最先进的性能，并能改进现有深度学习方法的输出。

Conclusion: 扩散先验作为脑MRI分析的通用工具具有巨大潜力，能够有效结合数据驱动模型和领域知识来解决医学图像逆问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多实例学习方法难以捕获全切片图像中的全局依赖关系，因为图像尺寸巨大且补丁嵌入具有局部性，这限制了建模对诊断预测至关重要的粗粒度结构。

Method: 提出FFT-MIL框架，通过快速傅里叶变换从WSI中提取低频裁剪，使用包含卷积层和Min-Max归一化的FFT模块处理频域数据，并将学习到的全局频域特征与空间补丁特征融合。

Result: 在三个公共数据集和六种最先进的MIL方法上评估，FFT模块集成使宏F1分数平均提高3.51%，AUC提高1.51%，在不同架构和数据集上均取得一致增益。
...
Method: 构建包含60,828个多选题的XModBench基准，涵盖五个任务家族和六种模态组合，系统评估模态不变推理、模态差异和方向不平衡。
...
Method: 采用半合成方法，利用原始图像生成对应四个质量级别的文本，创建样本-分数对来训练UniFilter分类器，然后应用于筛选DataComp描述数据集和OBELICS交错数据集。

Result: 在筛选数据上预训练的MLLM相比基线数据训练的模型表现出显著增强的能力，包括更强的零样本推理和上下文学习能力，经过视觉监督微调后在各种基准测试中表现更好。

Conclusion: UniFilter方法有效提升了多模态预训练数据的质量，带来了下游任务性能的显著改善，并发布了相关数据和模型供社区使用。
...
Result: 发现明确趋势：RandomResizedCrop值0.7-0.8表现最佳，分布式训练无局部损失提高了稳定性，学习率低于5.0e-5在所有数据集上都会降低性能，LC25000（Colon）数据集提供了最可重复的基准。
...
Method: 使用显著概念感知图像嵌入模型来减少合成过程中不相关视觉细节的影响，保持图像和文本输入之间的直观对齐，通过生成更好保留类别区分特征并具有额外受控变化的图像来增强训练数据多样性。

Result: 在八个细粒度视觉数据集上表现出优越性能，在常规和长尾设置下分别比最先进的增强方法平均分类准确率提高0.73%和6.5%。

Conclusion: 该框架通过减少不相关视觉细节的影响，有效提升了训练数据的多样性，从而显著提高了下游模型的鲁棒性。
...
Method: 构建首个公开的多模态数据集CARDIUM，包含胎儿超声和超声心动图图像及母体临床记录；提出融合图像和表格数据的跨注意力transformer架构

Result: 在CARDIUM数据集上，多模态方法比单模态图像和表格方法分别提升11%和50%的检测性能，F1分数达到79.8±4.8%

Conclusion: CARDIUM数据集和提出的多模态架构为产前先天性心脏病检测研究提供了重要资源，公开数据集和代码将推动该领域进一步发展
...
Method: 通过分析不同广告主题中的人口统计偏见，比较仅改变人物性别/种族对说服力的影响，并实验针对特定国家的广告定向技术。

Result: 研究发现文本到图像模型生成的广告存在人口统计偏见，仅改变人物性别/种族会影响广告的说服力水平。

Conclusion: 文本到图像模型在广告定制中具有潜力，但需要关注和解决其中的人口统计偏见问题，以确保广告的公平性和有效性。
...
Result: 能够实时生成长达12FPS的扩展驾驶视频（分辨率424×800）和相应的动态3D场景，在新视角合成上达到SSIM 0.811和PSNR 22.84，同时保持参数效率。

Conclusion: DriveGen3D成功整合了加速视频生成和动态3D重建，为高质量、可控的动态驾驶场景生成提供了有效解决方案。
...
Method: 采用CUDA加速的GPU并行化技术，使用计算密集型但高精度的特征提取器，生成全面且非冗余的数据关联，支持姿态优化、建图、先验地图定位和外参精化。

Result: 实验结果表明，cuSfM在各种测试场景下相比广泛使用的COLMAP方法，在精度和处理速度上都有显著提升，同时保持了离线SfM应用所需的高精度和全局一致性。

Conclusion: cuSfM作为一个开源的Python封装实现PyCuSfM发布，旨在促进计算机视觉和机器人领域的研究与应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动化MRI分析工具主要针对健康解剖结构优化，在处理大型病变（如肿瘤）时经常失败。图像修复技术旨在肿瘤区域局部合成健康脑组织，使通用工具能够可靠应用。
...
Conclusion: 通过结合现有模型与针对性后处理，实现了改进且更易获得的修复结果，支持更广泛的临床部署和资源节约型研究。
...
<details>
  <summary>Details</summary>
Motivation: 当前人脸识别系统在可识别性利用方面存在局限：软边界约束只能部分捕捉可识别性，导致质量表示较弱；特征方向与幅度的梯度重叠造成优化不稳定和表示纠缠。

Method: 引入硬边界策略QCFace，使用基于硬边界的损失函数，通过指导因子进行超球面规划，同时优化识别能力和显式可识别性表示。

Result: 大量实验证实QCFace不仅提供稳健且可量化的可识别性编码，在验证和识别基准测试中相比现有基于可识别性的损失函数达到最先进性能。

Conclusion: QCFace成功解决了梯度重叠问题，实现了可识别性与身份表示的清晰解耦，为人脸识别系统提供了更优的特征表示方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决SPMLL中每个训练样本仅标注一个正标签但实际可能属于多个类别的挑战，现有方法缺乏对不同关系类型的显式几何定义，难以捕捉复杂标签关系和层次结构。

Method: 使用双曲球表示标签，通过球体交互建模标签关系；引入温度自适应双曲球分类器和物理启发的双势阱正则化来指导球体形成有意义的配置。

Result: 在四个基准数据集上的实验表明，该方法在保持竞争力的同时具有更好的可解释性，统计分析显示学习到的嵌入与实际共现模式强相关。

Conclusion: 双曲几何为不完全监督下的结构化分类提供了更鲁棒的范式，能够有效建模多种标签关系类型。
...
<details>
  <summary>Details</summary>
Motivation: 传统VAE+扩散范式存在训练效率低、推理速度慢、可迁移性差的问题，主要原因是VAE潜在空间缺乏清晰的语义分离和强判别结构，这些属性对于感知理解和稳定训练至关重要。

Method: SVG利用冻结的DINO特征构建具有清晰语义判别性的特征空间，同时使用轻量级残差分支捕捉细粒度细节以实现高保真重建，直接在语义结构化的潜在空间上训练扩散模型。

Result: SVG实现了加速扩散训练、支持少步采样、提升生成质量，同时保留了底层自监督表示的语义和判别能力，为任务通用的高质量视觉表示提供了原则性途径。

Conclusion: SVG通过自监督表示构建语义结构化的潜在空间，克服了传统VAE+扩散范式的局限性，为高效、高质量的视觉生成提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然性能优异，但巨大的规模导致高计算和存储需求。现有结构化剪枝方法存在性能下降、权重聚合不理想和缺乏有效后训练恢复机制等问题。

Method: 提出CoMe框架：1）使用基于激活强度和权重范数的通道敏感度指标进行细粒度通道选择；2）采用基于拼接的层合并方法融合相邻层的关键通道；3）设计分层蒸馏协议，利用原始模型与剪枝模型层间对应关系进行知识迁移。

Result: 在7个基准测试上达到最先进性能，当剪掉LLaMA-2-7b 30%参数时，剪枝模型仍能保持83%的原始平均准确率。

Conclusion: CoMe通过渐进式剪枝和分层蒸馏有效解决了结构化剪枝中的性能下降问题，为大语言模型压缩提供了有效解决方案。
...
Method: 包含自适应原型感知编码器(APAE)进行自适应特征提取和学习原型表示，以及渐进式原型感知解码器(PPAD)细化原型生成提示来引导注意力。还提出了原型感知(PA)损失函数来解决原型专家寻址不稳定问题。

Result: 在广泛使用的基准数据集上的大量实验表明，Proto-Former相比现有最先进方法取得了优越性能。

Conclusion: Proto-Former通过原型学习实现了跨数据集统一训练，显著提升了面部关键点检测的泛化能力和性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注2D空间理解，缺乏对6D空间推理能力的全面评估框架，需要开发专门的数据集来测试模型在复杂3维空间推理中的能力。

Method: 创建了Spatial457合成数据集，包含4个关键空间推理能力：多目标识别、2D位置、3D位置和3D方向。采用级联评估结构，构建了7种问题类型和5个难度级别。

Result: 评估发现大型多模态模型在任务复杂度增加时性能普遍下降，特别是在3D推理和6D空间任务上。引入了相对性能下降率(RPDR)来量化这些挑战，并发现了不同属性间的预测偏差。

Conclusion: 现有大型多模态模型在复杂3维空间推理方面存在显著局限性，需要进一步改进。Spatial457数据集为评估和改进模型的6D空间推理能力提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前的人体运动重建方法在将人体准确放置在3D空间中存在困难，而逼真的角色与环境交互对于游戏、AR/VR和机器人等自主代理系统至关重要。

Method: SHARE首先估计每帧的人体网格和分割掩码，以及关键帧的场景点图。通过比较人体网格与使用掩码从场景提取的人体点图，迭代优化关键帧中的人体位置，并在优化过程中保持非关键帧与关键帧根关节的相对位置一致性。

Result: 广泛的实验表明，SHARE在准确重建周围场景的同时，能够实现更精确的3D人体放置，在精心策划的数据集和野外网络视频上都表现出色。

Conclusion: SHARE方法超越了现有技术，为自主代理系统提供了更准确的人体运动重建和环境交互能力。
...
<details>
  <summary>Details</summary>
Motivation: EEG和ECoG信号在运动想象分类中具有重要应用价值，但易受生理伪影干扰，且现有Transformer方法难以捕获细粒度依赖关系。

Method: 扩展深度状态空间模型，构建Cortical-SSM架构，整合捕获EEG和ECoG信号在时域、空域和频域的综合依赖关系。

Result: 在三个基准测试中（两个大规模公共MI EEG数据集和一个临床MI ECoG数据集）均优于基线方法，并能有效捕获神经生理相关区域。

Conclusion: Cortical-SSM模型能够有效处理EEG和ECoG信号，在运动想象分类任务中表现出色，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 微创手术需要自动化的手术工具检测，但手术场景中标注数据有限，难以训练鲁棒的深度学习模型。

Method: 两阶段自适应微调：线性探测阶段在预训练CNN架构上调节额外分类层；渐进冻结阶段动态减少可微调层数，调节对手术领域的适应。

Result: 在Cholec80数据集上验证，使用ResNet-50和DenseNet-121预训练模型，检测性能优于现有方法和传统微调技术，mAP达96.4%。在CATARACTS眼科手术数据集上进一步验证了泛化能力。

Conclusion: 渐进冻结微调是改善不同手术过程中工具存在检测的有前景技术，在通用图像分类任务中具有更广泛应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 当前多视图3D检测方法依赖深度预测恢复空间信息，需要LiDAR点云监督，但深度预测质量不佳，存在物体边界深度不连续、小物体难以区分等问题，且忽略了跨视图一致性和尺度不变性。

Method: 1. FSPE构建结合高频边缘线索和低频语义的特征金字塔；2. CSDP通过跨视图和通道注意力机制估计像素级深度分布；3. PDE结合2D图像特征和3D位置嵌入生成深度感知特征；4. 采用混合深度监督从度量和分布两方面进行互补学习。

Result: 在nuScenes数据集上的大量实验证明了该方法的有效性和优越性。

Conclusion: FreqPDE方法能够为2D图像特征有效添加空间信息，实现高质量的3D目标检测，无需LiDAR监督，解决了现有方法的深度预测质量问题。
...
Method: 通过迭代融合主姿态和辅助姿态图像，结合全局和局部配准策略，利用背景特征进行姿态估计和跨姿态配准。

Result: PFGS在定性和定量评估中均优于强基线方法，产生更完整的重建结果和更高保真度的3DGS模型。

Conclusion: PFGS通过智能融合多姿态图像和基础模型，有效解决了3DGS重建中的遮挡问题，实现了更完整的物体重建。
...
Conclusion: LILAC成功将离线运动风格化框架扩展到在线设置，实现了实时长序列任意运动风格化，解决了现有方法的计算开销和时间稳定性问题。
...
<details>
  <summary>Details</summary>
Motivation: 多器官多切片多方向多b值的复杂性，以及呼吸、蠕动等生理运动引起的严重相位伪影，限制了多镜头扩散加权MRI在全身肿瘤诊断中的临床应用。

Method: 将镜头间相位变化建模为高阶局部平滑相位(LoSP)，集成到低秩Hankel矩阵重建中，通过仅基于模拟生理运动的合成腹部DWI数据训练的提示学习自动设置算法秩参数。

Result: 在10,000+临床图像上验证，LoSP-Prompt实现了两倍于临床单镜头DWI的空间分辨率，增强了肝脏病变显着性；在七个不同解剖区域通用；在图像质量、伪影抑制和噪声降低方面优于最先进方法。

Conclusion: 该方法消除了导航信号和真实数据监督，为高分辨率多器官多镜头DWI提供了可解释、稳健的解决方案，其扫描仪无关性能在精准肿瘤学中具有变革潜力。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型尽管经过广泛对齐，仍然容易受到越狱攻击，现有检测方法要么学习攻击特定参数导致泛化能力差，要么依赖启发式原则限制准确性和效率。

Method: 提出LoD框架，包括多模态安全概念激活向量模块进行安全导向表示学习，以及安全模式自动编码器模块进行无监督攻击分类。

Result: 广泛实验表明，该方法在多种未知攻击上实现了持续更高的检测AUROC，同时提高了效率。
...
Method: 应用零样本语义分割从街景图像提取11个可解释的街道指标，结合道路类型分析约30,000条事故记录。使用XGBoost多类分类器和SHAP解释特征贡献，然后应用GPS加权和ATE估计控制混杂因素并量化因果效应。

Result: 发现异质性的、特定事故类型的因果模式：捕捉场景复杂性、暴露度和道路几何的特征主导预测能力；更大的可驾驶区域和应急空间降低风险，而过度视觉开放度可能增加风险。
...
Method: PC范式将建模任务解耦为两个协同模块：预测先验模块（PPM）利用解剖知识（双侧对称性）生成粗略近似，校正残差网络（CRN）学习残差误差，专注于精炼挑战性区域。

Result: 在高分辨率脑MRI分割实验中，PCMambaNet达到最先进精度，且仅需1-5个epoch即可收敛，远超传统端到端模型。

Conclusion: 通过显式整合领域知识简化学习目标，PCMambaNet有效缓解了数据低效和过拟合问题，实现了显著的学习加速。
...
Result: 在五个视频推理基准测试中达到新的SOTA，7B模型在LongVideoBench上达到59.8%，MVBench上69.0%，VideoMME上64.9%。

Conclusion: 证据纯度的优先考虑和所提框架的有效性得到验证，模型同时学习了有效且高纯度的视觉证据选择策略。
...
Conclusion: MAVR-Net通过多视角学习和跨模态交互，有效提升了微型飞行器动作识别的性能，为自主空中集群的协同感知和控制提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于提示学习的夜间航拍跟踪器仅依赖空间定位监督，无法提供指向目标特征的细粒度线索，导致提示模糊，影响跟踪器准确聚焦目标特征的能力，性能仍然较差。

Method: 受视觉仿生学启发，DPTrack首先分层捕获目标的拓扑结构，利用拓扑属性丰富特征表示；然后编码器将这些拓扑感知特征压缩到方向核中；最后基于通道-类别对应关系的核引导提示模块在搜索区域特征中传播核，定位目标特征位置并转换为精确提示，集成空间门控实现鲁棒的夜间跟踪。
...
TL;DR: 提出一种基于动态图像的双阶段时间增强方法，将微表情序列分解为起始到顶点和顶点到结束两个运动阶段，分别生成动态图像，以丰富运动多样性并提高识别性能。
...
Result: 在CASME-II和SAMM数据集上的实验表明，该方法在识别准确率、未加权F1分数和未加权平均召回率方面均有提升，结合空间增强可获得最高10%的相对改进。
...
TL;DR: MRASfM是一个专门为驾驶场景设计的运动结构恢复框架，通过利用多相机系统的固定空间关系提升相机姿态估计可靠性，使用平面模型改善路面重建质量，并将多相机系统作为单一单元进行优化以提高效率。


<details>
  <summary>Details</summary>
Motivation: 传统SfM方法在应用于多相机系统捕获的驾驶场景时存在相机姿态估计不可靠、路面重建异常点多、重建效率低等问题，需要专门针对驾驶场景的优化方案。

Method: 利用多相机系统的固定空间关系增强相机姿态估计可靠性；采用平面模型去除三角化路面中的错误点；将多相机系统作为单一单元进行Bundle Adjustment以减少优化变量；通过场景关联和组装模块实现多场景聚合。

Result: 在nuScenes数据集上达到0.124的绝对姿态误差，表现出最先进的性能；在实际车辆部署中验证了MRASfM在各种场景下的泛化能力和在挑战性条件下的鲁棒性。

Conclusion: MRASfM框架有效解决了驾驶场景中多相机SfM的关键问题，在姿态估计精度、重建质量和效率方面都取得了显著提升，具有很好的实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 随着无人机技术的发展，视频数据量急剧增加，迫切需要高效的语义检索方法。无人机视频具有俯视视角、结构同质性强、目标组合语义表达多样等特点，现有的跨模态方法难以有效建模这些特性。

Method: 提出了多语义自适应挖掘方法，包含多语义自适应学习机制，通过动态帧间变化和特定场景区域语义信息提取来增强对无人机视频内容的理解。该方法结合了自适应语义构建模块、分布驱动的语义学习项和多样性语义项，并引入了跨模态交互特征融合池化机制以减少复杂背景干扰。

Result: 在两个自建的无人机视频-文本数据集上的广泛实验表明，MSAM方法在无人机视频-文本检索任务中优于其他现有方法。

Conclusion: 针对无人机视频场景的专用检索机制是必要的，MSAM方法通过深度模态交互和鲁棒特征表示有效解决了无人机视频-文本检索的挑战。
...
TL;DR: 该研究提出了一种结合光学流（COF）方法，整合了微表情的起始到顶点和顶点到结束两个阶段，以增强特征表示并提高微表情识别性能。


<details>
  <summary>Details</summary>
Motivation: 大多数基于光学流的微表情识别方法通常只关注起始到顶点阶段，而忽略了顶点到结束阶段的关键时间动态信息，这限制了特征表示的全面性。
...
Conclusion: 结合光学流方法能够更全面地捕捉微表情的时间动态，显著提高微表情识别性能。
...
TL;DR: 提出一个用于农业植物3D重建的自动化管道，通过无人机自主采集图像，使用迭代变形方法消除叶片运动影响，生成高质量3D模型。
...
Method: 使用商业无人机自主采集图像，通过迭代方法估计叶片运动（光学流），逐步调整输入图像变形，减少场景运动，生成规范表示。

Result: 管道在几次迭代后改进了现有最先进方法的重建质量，能够提取高分辨率3D网格，并提供了包含多种作物在不同时间点采集的数据集。

Conclusion: 该管道能够生成高质量的植物3D重建，支持任意最先进的3D重建方法集成，代码和数据集将公开。
...
TL;DR: 提出HiMA架构，结合Transformer和Mamba模块处理不同尺度特征，通过LoDA调整局部特征分布，MPF融合多先验信息，在低光RAW图像增强任务中实现高效高质量处理。


<details>
  <summary>Details</summary>
Motivation: 解决低光RAW图像增强中同时实现强增强质量和高效率的挑战，克服现有深度学习方法在效率和性能平衡方面的局限。

Method: 采用分层混合架构HiMA，Transformer处理大尺度特征，Mamba处理小尺度特征；提出LoDA模块自适应调整局部特征分布；设计MPF模块融合空间和频域先验信息。
...
TL;DR: ORCA提出了一种利用预训练文本到图像扩散模型获取任务自适应视觉表示的方法，通过可学习的任务提示和视觉提示来适应控制环境，在机器人控制基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 预训练视觉表示在模仿学习中通常保持冻结状态，导致任务无关性。本文探索如何利用预训练扩散模型获得任务自适应表示，而不需要微调模型本身。

Method: 提出ORCA方法，引入可学习的任务提示来适应控制环境，以及视觉提示来捕捉细粒度的帧特定细节，通过新的条件设计促进任务自适应表示。

Result: 该方法在各种机器人控制基准测试中实现了最先进的性能，显著超越了先前的方法。

Conclusion: 通过考虑控制任务所需的特定动态视觉信息，ORCA成功地将预训练扩散模型应用于机器人控制，证明了任务自适应视觉表示的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现代人脸识别模型存在系统性偏见，传统偏见评估方法依赖昂贵的人工标注属性且限于预定义类别，需要无监督的偏见检测方法。

Method: 使用潜在方向识别子群体的Latent Feature Alignment算法，相比标准聚类能获得语义更一致的群组和可解释的潜在方向。

Result: 在四个最先进的人脸识别模型和两个基准测试上，LFA在组内语义一致性方面持续优于k-means和最近邻搜索，并能发现与人口统计和上下文属性对齐的可解释潜在方向。
...
<details>
  <summary>Details</summary>
Motivation: 针对卫星图像分类中的特定失败模式，探索无需预训练模型的专用架构设计，以解决空间和光谱特征提取的平衡问题。

Method: 通过三个渐进式架构迭代：基线模型、CBAM增强模型和平衡多任务注意力模型。使用Coordinate Attention提取空间特征，Squeeze-Excitation块提取光谱特征，通过可学习融合参数统一。采用渐进DropBlock正则化和类别平衡损失加权。

Result: 最终12层架构达到97.23%测试准确率，Cohen's Kappa为0.9692，所有类别准确率超过94.46%。可学习参数自主收敛到alpha≈0.57，表明空间和光谱模态重要性接近相等。

Conclusion: 系统化架构设计在领域特定应用中有效，性能接近微调ResNet-50（98.57%），且无需外部数据，验证了专用架构设计的效能。
...
TL;DR: SiM2P是一个基于3D扩散桥的框架，能够从MRI和患者辅助信息生成诊断质量的FDG-PET图像，显著提高了痴呆症诊断准确率。


<details>
  <summary>Details</summary>
Motivation: FDG-PET在痴呆症诊断中有效但成本高、可及性差，而MRI更普及且便宜，因此需要开发从MRI模拟PET的方法。

Method: 使用3D扩散桥框架学习从MRI和患者信息到FDG-PET的概率映射，仅需20个站点特定病例和基本人口统计信息即可部署。
...
Method: 从127个二战时期档案视频片段中提取包含拍板的生产元数据，提供完整帧标注和裁剪单词图像，支持零样本和微调条件下的识别和检测模型评估。
...
Method: 构建高质量资产库，使用图像生成模型扩展提示表示并微调对齐资产库，开发图像解析模块基于视觉语义和几何信息恢复3D布局，通过场景图和整体视觉语义优化布局。
...
TL;DR: 提出一种多视角架构，通过分析多个层次的面部特征来增强DeepFake检测，在具有挑战性的姿态和光照条件下表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有DeepFake检测方法在真实世界条件下难以处理姿态变化、遮挡和难以检测的伪影。

Method: 集成三个专用编码器：全局视角编码器检测边界不一致性，中间视角编码器分析纹理和颜色对齐，局部视角编码器捕捉眼睛、鼻子、嘴巴等表情区域的失真。还包含面部朝向编码器来分类面部姿态。

Result: 在具有挑战性的数据集上的实验结果表明，该方法优于传统的单视角方法。

Conclusion: 通过融合多个编码器的特征，该模型在检测操纵图像方面实现了卓越性能，即使在具有挑战性的姿态和光照条件下也能保持鲁棒性。
...
Method: 用固定通道方法替代U-Net生成器中的传统通道倍增策略，大幅减少参数数量。

Result: 参数从4180万减少到约9000个，性能更优，训练更快，内存使用更低。

Conclusion: 该模型不仅是有效的模态转换工具，还可作为验证显微镜工作流程中实验准确性和图像保真度的实用诊断工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有的STIF方法通常需要不同传感器捕获的具有匹配空间和光谱分辨率的图像集，这限制了其应用。为了促进STIF方法的应用，需要开发标准化方法来解决这个问题。

Method: 提出了两种标准化方法：1）基于传统上采样的方法；2）锐化方法ABSIS，将精细分辨率图像序列的整体特征与特定粗分辨率图像的独特属性相结合，生成更接近精细分辨率图像聚合结果的图像。

Result: 两种方法都显著提高了USTFIP STIF方法的准确性，其中锐化方法将融合图像的光谱和空间精度分别提高了49.46%和78.40%。

Conclusion: 提出的标准化方法，特别是ABSIS锐化方法，能够有效提高时空图像融合的精度，为STIF方法的更广泛应用提供了解决方案。
...
TL;DR: FlexiReID是一个灵活的多模态行人重识别框架，支持RGB、红外、素描和文本四种模态间的七种检索模式，通过自适应专家混合机制和跨模态查询融合模块实现高效特征整合。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注有限的跨模态设置，无法支持任意查询-检索组合，限制了实际部署。需要开发能够灵活处理多种模态组合的通用框架。

Method: 提出自适应专家混合(MoE)机制动态整合不同模态特征，引入跨模态查询融合模块增强多模态特征提取，构建统一的CIRS-PEDES数据集支持全面评估。

Result: 在广泛实验中，FlexiReID实现了最先进的性能，并在复杂场景中展现出强大的泛化能力。

Conclusion: FlexiReID框架通过灵活支持多种模态组合，显著提升了多模态行人重识别的实用性和性能表现。
...
Method: 采用量化版本的特征对应分析算法，通过量化值直方图进行补丁统计比较，并引入基于主成分分析的特征预处理步骤

Result: 在保持精度的同时获得10倍速度提升，在复杂纹理上提高了检测精度

Conclusion: QFCA方法在速度和精度方面均优于现有方法，适用于实时纹理异常检测应用
...
TL;DR: 提出了一种名为Noise2Detail (N2D)的超轻量级自监督去噪模型，在保持高质量图像恢复的同时实现快速去噪，解决了现有方法计算和内存需求大的问题。


<details>
  <summary>Details</summary>
Motivation: 现有自监督去噪技术在真实应用中受到计算和内存需求的限制，需要在推理速度和重建质量之间进行权衡。

Method: 基于Noise2Noise训练框架，引入创新的多阶段去噪流程，在推理过程中破坏噪声模式的空间相关性产生中间平滑结构，然后从噪声输入中重新捕获精细细节。

Result: 广泛测试表明Noise2Detail在性能上超越了现有的无数据集技术，同时仅需少量计算资源。

Conclusion: 该方法结合了高效性、低计算成本和无需数据的特点，使其成为生物医学成像的有价值工具，克服了由于罕见和复杂成像模式导致的清洁训练数据稀缺的挑战。
...
TL;DR: 这是一篇关于遥感领域深度学习方法域适应的综述论文，系统梳理了该领域的重要进展、分类方法和未来方向。


<details>
  <summary>Details</summary>
Motivation: 遥感中的域适应对于知识从源域迁移到不同分布的目标域至关重要，但由于数据差异（如采样距离、传感器模式、地理环境等）带来显著挑战，需要系统总结深度学习方法在该领域的应用。

Method: 论文首先介绍基础知识，然后从任务分类、输入模式、监督范式和算法粒度等多个角度组织现有算法，并回顾常用数据集和最新方法性能。

Result: 提供了遥感域适应领域的系统性分类框架，总结了当前最先进方法的性能表现，为研究社区提供了全面的领域理解。

Conclusion: 与以往综述相比，本文覆盖了更广泛的遥感域适应任务，提出了系统化的分类方法，能够启发研究社区并指导未来工作方向。
...
TL;DR: 提出一种弱监督医学图像分割框架，仅需四个极值点标注，通过SAM2生成初始伪标签，结合改进的FGEPM算法和不确定性估计进行边界追踪，使用USC损失和框对齐损失确保空间一致性，在超声图像分割中达到与全监督方法相当甚至更好的性能。
...
Method: 使用极值点生成边界框作为SAM2提示来创建初始伪标签；通过增强的FGEPM算法结合蒙特卡洛dropout不确定性估计构建梯度不确定性代价图进行边界追踪；引入双分支不确定性感知尺度一致性损失和框对齐损失。

Result: 在两个公共超声数据集BUSI和UNS上的实验表明，该方法性能与全监督方法相当甚至更优，同时显著降低标注成本。

Conclusion: 所提出的弱监督框架在超声图像分割中具有有效性和实用性，大幅减少了标注需求。
...
TL;DR: 提出了一个用于检测行人意图的新型多模态数据集，包含鱼眼相机、激光雷达、超声波传感器和动作捕捉数据，支持行人检测、3D姿态估计和意图预测算法的开发与评估。
...
Method: 收集同步多模态数据，包括鱼眼相机、激光雷达、超声波传感器和基于动作捕捉的3D人体姿态，并在多样化真实场景中进行标注。

Result: 发布了部分数据集和基准测试套件，提供了3D人体关节位置和行人位置的精确标注，并给出了基于自定义神经网络架构的基线性能指标。
...
Result: 在测试集上获得病灶级Dice相似系数：全肿瘤0.437、肿瘤核心0.316、增强肿瘤0.350；验证集异常检测率达到89.4%。
...
TL;DR: 提出了UniMedVL，首个医学统一多模态模型，能够同时处理图像理解和生成任务，通过OKA范式在单一架构中实现双向知识共享。


<details>
  <summary>Details</summary>
Motivation: 现有医学AI系统存在割裂问题：图像理解模型无法生成视觉输出，图像生成模型无法提供文本解释，导致数据表示、特征集成和多模态能力方面的差距。

Method: 采用观察-知识-分析(OKA)范式：观察层构建UniMed-5M数据集；知识层提出渐进式课程学习；分析层开发UniMedVL统一多模态模型。

Result: 在5个医学图像理解基准测试中表现优异，在8种医学成像模态的生成质量上与专门模型相当，且生成任务能增强视觉理解特征。

Conclusion: 在单一医学框架中整合传统分离的能力，能够解锁跨多样化医学视觉语言任务的改进，实现双向知识共享。
...
TL;DR: 提出DGME-T方法，通过注入方向性网格运动编码来增强Video Swin Transformer在档案影片相机运动分类中的性能，显著提升现代和历史影片的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 解决在档案影片分析中，传统相机运动分类模型因噪声、缺失帧和低对比度等问题导致的性能下降问题。

Method: 构建统一基准数据集，并在Video Swin Transformer基础上引入方向性网格运动编码，通过可学习的归一化后期融合层注入光学流信息。

Result: 在现代片段上，top-1准确率从81.78%提升到86.14%，宏F1从82.08%提升到87.81%；在二战档案影片上，准确率从83.43%提升到84.62%，宏F1从81.72%提升到82.63%。
...
TL;DR: Ditto框架通过融合图像编辑器和视频生成器创建了Ditto-1M数据集，训练出的Editto模型在指令式视频编辑中达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 解决指令式视频编辑领域缺乏大规模高质量训练数据的根本挑战。

Method: 融合图像编辑器与上下文视频生成器的数据生成流水线，采用高效蒸馏模型架构和时序增强器，由智能代理驱动整个流程。

Result: 投入12,000 GPU天构建了包含100万高质量视频编辑样本的Ditto-1M数据集，训练出的Editto模型展现出卓越的指令跟随能力。

Conclusion: Ditto框架成功解决了指令式视频编辑的数据稀缺问题，建立的Editto模型在该领域达到了新的最先进水平。
...
TL;DR: 提出SEGA方法，通过分层推理框架解决内容感知布局生成问题，采用从粗到细的策略，并融入布局设计原则作为先验知识。
...
Conclusion: SEGA方法通过分层推理和设计原则融入，有效提升了内容感知布局生成的性能，特别是在复杂布局规划场景下。
...
Method: 利用早期预测噪声的可分离性开发噪声检测方法，并提出噪声增强自适应负引导机制，通过抑制显著区域注意力来优化初始噪声。

Result: 在自然和对抗数据集上验证，NDM性能优于现有SOTA方法（如SLD、UCE、RECE等）。

Conclusion: NDM能有效检测和缓解隐式恶意意图，同时保持模型的生成质量，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当获取精细像素级标注困难或昂贵时，粗标注（如忽略类别边界像素）是可行的替代方案，但边界对齐效果较差。

Method: 在编码器-解码器架构中引入基于SLIC超像素的正则化，鼓励解码图像中的分割像素形成与颜色和位置相关的超像素。
...
TL;DR: QSilk是一个轻量级的潜在扩散模型稳定层，通过微钳位和自适应分位数裁剪技术提高高频保真度并抑制罕见激活峰值，无需训练即可在低步数和高分辨率下获得更清晰的结果。


<details>
  <summary>Details</summary>
Motivation: 解决潜在扩散模型中高频细节保真度不足和罕见激活峰值问题，提升图像质量而不增加显著计算开销。

Method: 结合每样本微钳位技术（温和限制极值）和自适应分位数裁剪（AQClip），AQClip可使用局部结构统计或注意力熵引导模式来调整各区域允许的值范围。

Result: 在CADE 2.5渲染管线中集成QSilk，在低步数和超高分辨率下获得更干净、更锐利的结果，计算开销可忽略，与CFG/Rescale有协同效应。

Conclusion: QSilk是一种无需训练的有效稳定解决方案，在SD/SDXL骨干网络上均能带来一致的定性改进，可略微提高引导强度而不产生伪影。
...
Result: 在三个不同数据集上的实验表明，LDCBM在概念准确率和分类准确率方面均优于先前CBMs，同时提升了可解释性。
...
Result: ReCon显著提高了生成数据的质量和可训练性，在各种数据集、骨干架构和数据规模上都取得了持续的性能提升。

Conclusion: ReCon通过改进结构可控生成模型的能力，为数据增强提供了一种有效解决方案，能够生成更高质量的训练数据。
...
<details>
  <summary>Details</summary>
Motivation: 解决非刚性变形点云序列配准的两个关键挑战：局部最小值导致的配准目标非凸性，以及长序列中的误差累积问题

Method: 采用可扩展的数据驱动方法，通过两阶段管道：首先估计帧级粗粒度图节点进行鲁棒初始化，然后在滑动窗口方式下随时间细化节点轨迹

Result: 在DeformingThings4D和D-FAUST数据集上优于先前最先进方法，相比之前最佳方法实现4倍以上的加速

Conclusion: ERNet能够有效处理噪声和部分输入，利用时序信息实现准确一致的序列配准，在准确性和效率方面均有显著提升
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成方法严重依赖精确的用户提示，而现有的测试时优化方法在处理视频的多方面特性时表现不佳。
...
TL;DR: 提出了RelateSeg方法，在开放词汇语义分割中引入神经符号空间推理，通过一阶逻辑约束提升对未见类别的分割性能
...
Method: 使用一阶逻辑公式表示空间关系，通过伪类别编码空间关系，在神经网络架构中实现模糊逻辑松弛，实现端到端学习

Result: 在四个基准数据集上达到最先进的平均mIoU性能，特别是在包含多个类别的图像上表现优势，仅引入单个辅助损失函数且无额外参数

Conclusion: 神经符号空间推理在开放词汇语义分割中是有效的，能够显著提升分割性能
...
TL;DR: 3DPR是一个基于图像的重新照明模型，利用从光舞台捕获的多视图单光源图像学习的生成先验，通过预训练的生成头部模型和新的三平面反射网络实现高质量的人脸重新照明。


<details>
  <summary>Details</summary>
Motivation: 传统图形学方法通过可微分渲染将输入图像分解为几何、材质和光照，但受限于底层模型的假设和近似。需要一种更准确的方法来渲染新颖的重新照明视图。

Method: 首先将输入肖像嵌入预训练生成头部模型的潜在空间，然后使用在光舞台数据上训练的三平面反射网络合成高质量单光源图像，实现基于图像的重新照明。

Result: 3DPR在定量和定性评估中优于先前方法，特别是在保持身份和捕捉光照效果（如镜面反射、自阴影和次表面散射）方面表现出色。

Conclusion: 该方法通过结合生成先验和光舞台数据，实现了物理准确的环境重新照明效果，为单目肖像图像的重新照明提供了高质量解决方案。
...
TL;DR: Memory-SAM是一个无需训练、无需人工提示的舌像分割方法，通过检索历史案例的DINOv3特征自动生成提示点，指导SAM2模型实现高精度分割。


<details>
  <summary>Details</summary>
Motivation: 传统监督模型需要大量标注数据，而SAM系列模型仍需人工提示。本文旨在开发无需训练和人工干预的自动舌像分割方法。
...
Conclusion: 检索到提示的方法能够实现数据高效、鲁棒的舌像不规则边界分割，在真实场景下表现优异。
...
TL;DR: BLIP3o-NEXT是一个开源的图像生成基础模型，统一了文本到图像生成和图像编辑功能，采用自回归+扩散架构，在多项基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 推动原生图像生成的前沿发展，探索统一文本到图像生成和图像编辑的单一架构，并验证关键设计洞察。

Method: 采用自回归+扩散混合架构：自回归模型首先生成基于多模态输入的离散图像token，其隐藏状态作为扩散模型的调节信号来生成高保真图像。

Result: 在各项文本到图像生成和图像编辑基准测试中，BLIP3o-NEXT均表现出优于现有模型的性能。

Conclusion: 该研究确认了架构选择、强化学习应用、数据质量和规模对模型性能的关键影响，展示了混合架构在图像生成任务中的有效性。
...
### [85] [OmniVinci: Enhancing Architecture and Data for Omni-Modal Understanding LLM](https://arxiv.org/abs/2510.15870)
*Hanrong Ye,Chao-Han Huck Yang,Arushi Goel,Wei Huang,Ligeng Zhu,Yuanhang Su,Sean Lin,An-Chieh Cheng,Zhen Wan,Jinchuan Tian,Yuming Lou,Dong Yang,Zhijian Liu,Yukang Chen,Ambrish Dantrey,Ehsan Jahangiri,Sreyan Ghosh,Daguang Xu,Ehsan Hosseini-Asl,Danial Mohseni Taheri,Vidya Murali,Sifei Liu,Jason Lu,Oluwatobi Olabiyi,Frank Wang,Rafael Valle,Bryan Catanzaro,Andrew Tao,Song Han,Jan Kautz,Hongxu Yin,Pavlo Molchanov*

Main category: cs.CV

TL;DR: OmniVinci是一个开源的多模态大语言模型，通过创新的架构设计和数据管道，在视觉、音频等多模态理解任务上表现优异，使用更少的训练数据超越了现有模型。


<details>
  <summary>Details</summary>
Motivation: 推动机器智能发展需要像人类一样感知多模态信息，因此需要构建强大的开源多模态大语言模型。

Method: 提出三个关键创新：OmniAlignNet增强视觉和音频嵌入的对齐；Temporal Embedding Grouping捕捉相对时间对齐；Constrained Rotary Time Embedding编码绝对时间信息。同时构建了包含2400万单模态和多模态对话的数据管道。
...
Method: 使用进化框架，利用大型语言模型作为生物医学知识提取器和自适应优化器，自动生成多样化的可解释自然语言提示对。
...
<details>
  <summary>Details</summary>
Motivation: 现有的单图像眩光去除方法在画面外光源不完整或缺失时表现不佳，这严重影响了图像质量并阻碍了计算机视觉任务如目标检测和自动驾驶的发展。

Method: 采用基于扩散模型的外绘框架，结合多任务回归模块和LoRA微调的扩散模型，确保生成真实且物理一致的外绘结果。

Result: 综合实验表明，LightsOut能够持续提升现有单图像眩光去除方法在挑战性场景下的性能，无需额外重新训练。

Conclusion: LightsOut作为一个通用的即插即用预处理解决方案，有效解决了画面外光源缺失导致的眩光去除性能下降问题。
...
Method: 利用卫星图像提供粗略几何结构，结合开放域扩散模型生成高质量近景外观，采用课程驱动的迭代细化策略逐步提升几何完整性和真实感纹理。

Result: 相比现有方法，Skyfall-GS在跨视角几何一致性和纹理真实感方面表现更优。

Conclusion: 该方法为大规模3D场景创建提供了一种无需昂贵3D标注的可行方案，支持实时沉浸式探索。
...
### [89] [Game mechanics for cyber-harm awareness in the metaverse](https://arxiv.org/abs/2510.15180)
...
<details>
  <summary>Details</summary>
Motivation: 随着元宇宙等下一代互联网技术的发展，儿童和年轻人在虚拟现实中的网络安全风险增加，需要开发专门的教育工具来保护他们。

Method: 开发CyberNinjas VR体验，通过游戏机制教育儿童安全行为，并提供有害互动的明确转介步骤。

Result: 该项目分析了CyberNinjas，了解游戏机制如何促进网络安全行为。
...
Method: 设计多级预测系统，使用可解释和可扩展的预测模型，通过完全并行化的单模型预测工作流程，逐步将区域预测扩展到节点预测。

Result: 区域预测的准确性和可解释性得到改善，节点预测有显著改进，运营商能够以前所未有的信心和准确性调整预测，并精确诊断错误。

Conclusion: 多级预测系统有效解决了节点负荷预测的挑战，提高了电网运营的可靠性和效率。
...
TL;DR: 提出了TangledFeatures框架，用于在相关特征空间中进行特征选择，通过识别纠缠预测因子组中的代表性特征来减少冗余并保留解释能力。
...
Method: 引入TangledFeatures框架，识别相关预测因子组中的代表性特征，减少冗余同时保持解释能力。
...
TL;DR: 本文提出了ES-C51算法，通过将C51中的贪婪Q学习更新替换为Expected Sarsa更新，使用softmax结合所有可能动作的信息，解决了当动作具有相似期望奖励但不同分布时的不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于值的RL算法只估计每个动作的期望奖励，而分布强化学习(DRL)估计整个奖励概率分布。C51是流行的离散动作空间DRL算法，但使用贪婪Bellman更新可能导致当多个动作具有相似期望奖励但不同分布时学习不稳定。

Method: 提出ES-C51算法，用Expected Sarsa更新替换C51的贪婪Q学习更新，使用softmax计算结合所有可能动作的信息，而非依赖单一最佳动作。同时将标准C51的探索策略从ε-greedy改为softmax作为对比基准(QL-C51)。

Result: 在Gym经典控制环境和Atari-10游戏上的评估表明，ES-C51在多数环境中优于QL-C51，能够学习到更高性能的策略。

Conclusion: 使用Expected Sarsa更新能够有效解决C51在动作具有相似期望奖励时的不稳定性问题，提高算法性能。
...
TL;DR: 提出了一种基于集成深度学习的无监督异常检测框架，用于风力涡轮机的早期故障检测，结合了VAE、LSTM自编码器和Transformer架构，在真实SCADA数据上实现了0.947的AUC-ROC和提前48小时的故障检测能力。
...
Result: 在包含89年真实涡轮机数据的CARE数据集上测试，AUC-ROC达到0.947，能够提前48小时检测到故障，优于现有方法。

Conclusion: 该方法通过无监督学习实现了风力涡轮机的早期故障检测，为预测性维护提供了有效工具，具有显著的社会价值，能减少涡轮机故障并提高运营效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于最优传输的流生成模型方法使用小批量采样估计传输计划，这在大规模高维数据集上存在可扩展性问题。

Method: 利用半离散最优传输(SDOT)在噪声分布和数据点之间建立显式最优对齐，将噪声空间划分为Laguerre单元，每个单元映射到对应的数据点。

Result: AlignFlow能够扩展到大型数据集和模型架构，计算开销可忽略，显著提升了多种最先进流生成模型算法的性能。

Conclusion: AlignFlow是一种有效的即插即用组件，通过半离散最优传输改进了流生成模型的训练效率和性能。
...
TL;DR: 提出了IQNN-CS，一种用于多类别信用风险分类的可解释量子神经网络框架，结合了变分QNN和专门为结构化数据设计的解释技术。


<details>
  <summary>Details</summary>
Motivation: 信用评分是金融服务中的高风险任务，需要透明和可信的模型。量子机器学习虽然提供新的计算能力，但其黑盒特性在需要透明度的领域存在挑战。

Method: 使用变分量子神经网络架构，结合后解释技术，并引入了Inter-Class Attribution Alignment (ICAA)指标来量化不同预测类别间的归因差异。
...
Method: 将世界模型分解为状态表示和转移建模，通过自监督微调阶段冷启动策略，学习世界模型后再进行策略优化

Result: 在Sokoban、FrozenLake和Sudoku等环境中显著提升性能，Sokoban成功率从25.6%提升到59.8%，FrozenLake得分从22.1%提升到70.9%

Conclusion: 为LLM智能体配备内部世界模型可以更好地对齐推理与环境动态，改善决策能力，SPA框架有效提升了强化学习训练效果
...
TL;DR: 论文提出了多层级可配置时变马尔可夫决策过程（MCTVMDP），其中智能体不仅可以通过传统动作与环境交互，还能通过上层模型改变动作主动修改环境动态模型本身。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习假设环境是给定或固定的，但实际中智能体可能具备主动修改环境动态模型的能力，通过重新配置底层转移过程来增加奖励。

Method: 引入MCTVMDP框架，包含上层MDP用于配置模型改变动作，下层MDP具有可配置的非平稳转移函数。智能体需要联合优化上层配置策略和下层原始动作策略。

Result: 提出了一个能够处理智能体主动修改环境动态模型的强化学习框架，扩展了传统MDP的假设。

Conclusion: MCTVMDP为研究智能体主动修改环境的能力提供了理论基础，能够更好地模拟现实世界中智能体与环境互动的复杂性。
...
TL;DR: Antislop是一个检测和消除LLM输出中重复短语模式的框架，包含推理时采样器、自动化数据生成管道和细粒度微调方法，能显著减少AI文本特征性重复模式同时保持模型性能。
...
Method: 结合三种创新方法：1) Antislop采样器使用回溯在推理时抑制不需要的字符串；2) 自动化管道分析模型特定slop模式并生成训练数据；3) 最终令牌偏好优化(FTPO)，一种在单个令牌级别操作的微调方法。

Result: 某些slop模式在LLM输出中出现频率比人类文本高1000倍以上。Antislop采样器成功抑制8000+模式同时保持质量，而令牌禁止在仅2000模式时就不可用。FTPO实现90%的slop减少，同时在GSM8K、MMLU和创意写作任务中保持或改进性能。

Conclusion: Antislop框架能有效检测和消除LLM输出中的重复模式，显著提高文本质量，同时保持模型在各种任务上的性能表现。
...
TL;DR: 本文提出了三种基于物理信息数据驱动预测模型和统计方法的两光子光刻(TPL)系统健康监测方法，能够准确及时地监测机器健康状况，实现基于状态的维护。


<details>
  <summary>Details</summary>
Motivation: 当前TPL系统维护主要依赖经验而非机器健康监测，导致维护时机不当——要么维护不及时造成机器停机和制造质量差，要么不必要维护导致效率低下和可避免的停机时间。

Method: 通过集成物理信息数据驱动的结构尺寸预测模型与统计方法，提出了三种能够处理不同泛化水平复杂场景的健康监测方法。

Result: 在包含六种工艺参数组合和六种结构尺寸的全面实验数据集上评估，所有测试场景中方法都实现了高准确率，表现出优异的有效性、鲁棒性和泛化能力。

Conclusion: 这些结果为TPL系统实现基于状态的维护迈出了重要一步。
...
TL;DR: 该论文提出了首个在线样本模型(AOS)下的相关聚类算法，能够同时近似所有ℓ_p-范数目标，成功将离线的"全范数"保证扩展到在线场景。


<details>
  <summary>Details</summary>
Motivation: 研究动机源于发现离线环境中可以同时近似所有ℓ_p-范数，但标准随机顺序在线模型存在基本分离：ℓ_1-范数可轻松近似，而公平性导向的ℓ_∞-范数需要至少Ω(n^{1/3})的竞争比，因此需要超越最坏情况的模型。

Method: 提出了一种在线样本模型(AOS)算法，给定输入的一小部分常数比例作为样本，生成单一聚类，同时为所有ℓ_p-范数提供竞争性保证。

Result: 算法在AOS模型中：对所有ℓ_p-范数高概率O(log⁴n)-竞争，对ℓ_∞-范数高概率O(logn)-竞争，对ℓ_1-范数期望O(1)-竞争。同时证明了标准RO模型中ℓ_∞-范数需要Ω(n^{1/3})竞争比的下界。

Conclusion: 成功将离线的"全范数"保证扩展到在线世界，证明了AOS模型的必要性，并提供了近乎最优的竞争比保证。
...
TL;DR: 提出TempO模型，一种基于流匹配的生成模型，用于高维PDE动力学预测，通过稀疏条件和通道折叠技术高效处理3D时空场，在多个基准数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有自回归和扩散方法在长期物理一致性预测中存在累积误差和离散化伪影问题，流匹配提供了一种高效确定性采样的替代方案。

Method: 使用潜在流匹配模型，结合稀疏条件和通道折叠技术，采用时间条件傅里叶层来捕捉多尺度模式，并证明了FNO近似误差的上界。

Result: TempO在三个基准PDE数据集上优于最先进的基线方法，谱分析显示其能更好地恢复多尺度动力学，效率研究表明其参数和内存需求低于基于注意力或卷积的回归器。

Conclusion: TempO模型通过流匹配方法有效解决了高维PDE动力学预测中的累积误差问题，实现了高效且物理一致的长期预测。
...
TL;DR: DLER通过改进强化学习优化方法，使用简单的截断长度惩罚，在保持高准确率的同时大幅减少推理语言模型的输出长度，实现了最优的准确率-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 现有推理语言模型虽然性能强大，但生成了不必要冗长的输出。最大化每个token的智能度（准确率相对于响应长度）仍然是一个开放问题。

Method: 提出DLER训练方法，结合批处理奖励归一化、更高裁剪、动态采样和简单的截断长度惩罚，解决强化学习优化中的三大挑战：优势估计偏差大、熵崩溃和稀疏奖励信号。

Result: DLER将输出长度减少70%以上，同时超过所有先前基线的准确率。DLER-7B相比DeepSeek-R1-7B，在并行生成多个简洁响应时准确率提高28%，延迟更低。

Conclusion: DLER展示了通过改进强化学习优化而非复杂惩罚机制，可以实现推理语言模型的准确率-效率最优权衡，并提出了难度感知DLER和更新选择性合并方法进一步提升效率。
...
<details>
  <summary>Details</summary>
Motivation: 需要分析异质性患者-呼吸机系统的机械通气策略效果，理解临床决策对患者预后的影响，改进重症监护呼吸管理。

Method: 采用进化博弈论分析呼吸行为，生成量化前体数据，通过概率和随机方法进行深度分析，在合成数据上验证后应用于真实ICU数据。

Result: 建立了可扩展的患者-呼吸机-护理系统分析方法，揭示了数据生成过程的复杂性，为机械通气优化提供了量化基础。

Conclusion: 这是实现机械通气优化和个性化的重要一步，未来可发展包含经验和博弈论元素的状态转移模型来模拟机械通气决策效果。
...
<details>
  <summary>Details</summary>
Motivation: 针对大型离散支撑集上多模态、重尾分布的概率质量函数估计问题，传统方法在处理噪声和保持粗粒度结构方面存在挑战。

Method: 将经验PMF视为线图上的信号，构建对称三对角算子（路径图拉普拉斯加上经验PMF构建的对角矩阵），计算最小特征值对应的特征向量，将经验PMF投影到低维子空间获得平滑估计。

Result: 在合成和真实重尾数据上，该方法能有效保持粗粒度结构并抑制采样噪声，在目标场景下优于logspline和高斯KDE基线方法。

Conclusion: 该方法实现简洁、计算可靠、内存和时间效率高，适合自动化流程和大规模探索性分析，但存在已知的失效模式（如突然的不连续性）。
...
<details>
  <summary>Details</summary>
Motivation: 研究全球恐怖主义数据库(GTD)中恐怖主义事件周计数的短期预测，建立可复现的预测管道，评估深度学习模型相对于经典基准的表现。

Method: 构建基于固定时间分割的可复现管道，使用双向LSTM(BiLSTM)模型，与季节性朴素、线性/ARIMA模型和LSTM-Attention基线进行比较。进行消融实验分析时间记忆、训练历史长度、空间粒度等影响因素。

Result: BiLSTM在测试集上达到RMSE 6.38，比LSTM-Attention(9.19)提升30.6%，比线性滞后回归基线提升35.4%。MAE和MAPE也有相应改进。长期历史数据训练效果最佳，中等回溯窗口(20-30周)提供强上下文，双向编码对捕捉窗口内事件前后模式至关重要。

Conclusion: 该研究为GTD事件预测提供了透明、超越基线的参考框架，短期结构特征(滞后计数和滚动统计)贡献最大，地理和伤亡特征提供增量提升。
...
TL;DR: 该论文首次理论证明了连续时间强化学习中的策略迁移，证明了在一个LQR问题中最优的策略可以作为相近LQR问题的近最优初始化，同时保持原始算法的收敛速率。
...
Method: 研究在熵正则化的连续时间线性二次调节器（LQR）中的策略迁移方法，提出了一种新的策略学习算法，实现了全局线性和局部超线性收敛。

Result: 证明了策略迁移在连续时间RL中的理论可行性，提出的新算法具有优越的收敛性能，并建立了连续时间基于分数的扩散模型与LQR的联系。

Conclusion: 本文填补了连续时间RL中迁移学习的理论空白，将先前工作从离散时间扩展到连续时间设置，为连续时间RL的效率提升提供了理论保证和算法支持。
...
TL;DR: 该论文基于统计物理学方法，推导了使用随机梯度朗之万动力学训练的两层非线性网络的贝叶斯后验的自洽平均场理论，揭示了有限宽度下网络与目标函数对齐的对称性破缺相变，并发现了自增强输入特征选择机制对泛化性能的重要影响。


<details>
  <summary>Details</summary>
Motivation: 特征学习（神经网络在训练过程中调整内部表示）仍然缺乏深入理解。作者希望通过统计物理学方法建立可处理的平均场理论来理解特征学习的机制。

Method: 使用统计物理学方法推导了两层非线性网络在随机梯度朗之万动力学训练下的贝叶斯后验自洽平均场理论，并在基本平均场理论基础上加入了自增强输入特征选择机制。

Result: 在无限宽度下理论简化为核岭回归，但在有限宽度下预测了对称性破缺相变，网络会突然与目标函数对齐。基本平均场理论能半定量预测特征学习的出现，但低估了相变后泛化性能的改善。加入自增强输入特征选择机制后，理论能定量匹配SGLD训练网络的学习曲线。

Conclusion: 自增强输入特征选择机制是理解特征学习的关键机制，该平均场理论为特征学习提供了机制性洞见，能够定量描述神经网络训练过程中的相变现象和泛化性能变化。
...
TL;DR: 本文提出测地线问题特别适合深度Ritz方法，并通过三个数值实例验证了这一观点，旨在为科学机器学习研究指明一个有前景的方向。
...
Method: 使用深度Ritz方法解决测地线问题，该方法基于变分原理，通过三个数值实例进行验证：路径规划、光学和固体力学。

Result: 通过三个不同领域的数值实验，证明了深度Ritz方法在解决测地线问题上的有效性。

Conclusion: 测地线问题是深度Ritz方法的一个有前景的应用方向，值得未来科学机器学习研究的进一步探索。
...
<details>
  <summary>Details</summary>
Motivation: 传统工具如DXA T-score和FRAX在髋部骨折风险预测中敏感性不足，特别是对于无既往骨折史或骨量减少的高风险人群。

Method: 使用MrOS、SOF和UK Biobank数据，构建两阶段模型：第一阶段使用临床、人口统计学和功能变量进行筛查，第二阶段整合DXA影像特征进行精炼。

Result: 经过内部和外部验证，该模型在不同队列中表现一致且适应性强，相比T-score和FRAX具有更高敏感性和更少漏诊病例。

Conclusion: 该两阶段框架为早期髋部骨折风险评估提供了一种经济有效且个性化的方法。
...
TL;DR: 本文探索了使用机器学习替代模型预测碰撞场景中结构变形的可行性，比较了MeshGraphNet和Transolver两种神经网络架构以及三种瞬态动力学建模策略，在车身白车身碰撞数据集上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 传统汽车碰撞安全性评估依赖计算昂贵的高保真有限元模拟，需要开发更高效的替代方法。

Method: 使用NVIDIA PhysicsNeMo框架，研究MeshGraphNet和Transolver两种神经网络架构，以及时间条件、标准自回归和稳定性增强自回归三种瞬态动力学建模策略。

Result: 模型能够合理捕捉整体变形趋势，虽然尚未达到完整有限元模拟的精度，但实现了计算成本数量级的降低。

Conclusion: 证明了将机器学习应用于结构碰撞动力学的可行性，为快速设计探索和早期优化提供了有效工具。
...
TL;DR: 该论文通过实证研究发现Mahalanobis距离方法在OOD检测中并非普遍可靠，定义了理想的数据表示几何结构，并提出径向缩放ℓ2归一化方法，通过调整特征空间的径向几何结构来显著提升OOD检测性能。


<details>
  <summary>Details</summary>
Motivation: 理解表示几何结构和归一化对Mahalanobis距离方法在OOD检测中性能的影响，目前这一领域缺乏深入理解，限制了这些方法的下游应用。

Method: 在多样化的图像基础模型、数据集和距离归一化方案上进行全面实证研究，分析表示几何结构与OOD性能的关系，并提出径向缩放ℓ2归一化方法。

Result: 研究发现Mahalanobis方法并非普遍可靠，光谱和内在维度指标能准确预测模型OOD性能，提出的径向缩放归一化方法能显著提升OOD检测性能。

Conclusion: 通过连接表示几何结构、归一化和OOD性能之间的关联，为设计更有效可靠的深度学习模型提供了新的见解。
...
TL;DR: ReasonIF基准评估大型推理模型在推理过程中遵循用户指令的能力，发现现有模型表现不佳（最高IFS低于0.25），并提出多轮推理和推理指令微调两种改进方法。


<details>
  <summary>Details</summary>
Motivation: 虽然已有研究评估模型主要回答的指令遵循能力，但推理过程中的指令遵循对模型的可控性、透明性和安全性同样重要，能减少不良捷径、幻觉或奖励攻击风险。

Method: 引入ReasonIF基准，包含多语言推理、格式和长度控制等六类指令提示；探索多轮推理和推理指令微调（RIF）两种策略。

Result: 多个开源大型推理模型在推理指令遵循方面表现严重不足，IFS最高仅0.25；任务难度增加时表现更差；RIF将GPT-OSS-20B的IFS从0.11提升到0.27。

Conclusion: 大型推理模型在推理过程中遵循指令的能力存在显著不足，虽然RIF方法能带来可衡量的改进，但仍有很大提升空间。
...
TL;DR: 研究发现，不同基础模型在可验证奖励强化学习（RLVR）后表现差异巨大，关键在于模型能否在预训练阶段区分逻辑规则的严谨程度。通过引入严谨性感知水平（SAL）指标，揭示了模型推理潜力与其内在区分严谨知识与不严谨知识能力的关系。


<details>
  <summary>Details</summary>
Motivation: 探究为什么不同基础模型在RLVR强化学习后表现差异显著，寻找预训练模型的微观特性如何影响其推理能力。

Method: 将推理形式化为Horn子句链，使用跨层稀疏自编码器从LLM潜在空间提取特征，估计特征间转移概率，并用LLM对每条规则的语义严谨性进行分类。

Result: 发现高潜力模型具有严谨性感知能力，其内部概率分布在不同的规则严谨性水平上系统性地变化，而弱模型则对所有严谨性水平都使用相同的分布。SAL指标能准确预测模型在RLVR后的推理表现（R^2=0.87）。

Conclusion: 模型的推理潜力与其预训练阶段区分严谨知识与不严谨知识的内在能力密切相关，这为选择和设计更强的基础模型提供了基于模型内部机制的实际指标。
...
Method: 通过由资深和初级主席主持的小组圆桌会议形式，鼓励开放式交流和集体创意。

Result: 成功举办了8个圆桌会议，涵盖可解释性、公平性、因果推断、领域适应、基础模型、小样本医疗数据学习、多模态方法和可扩展医疗解决方案等主题。

Conclusion: 研究圆桌会议为医疗机器学习领域的关键议题提供了重要的讨论平台，推动了该领域的协作发展和创新方向。
...
TL;DR: 使用MIMIC-III数据库中214名脑膜炎患者和46,303名非脑膜炎患者数据，通过集成学习方法构建脑膜炎诊断模型，在模拟急诊室场景下获得优异性能（测试集AUC分别为0.9637和0.9472）。
...
Method: 采用两阶段特征选择流程，选择性别和高风险ICD代码等临床相关特征，使用随机森林、LightGBM和深度神经网络作为基础模型，通过集成学习将基础模型输出堆叠到逻辑回归元模型中。

Result: 在模拟急诊室场景的测试集上获得优异性能：测试集1的AUC为0.9637，测试集2的AUC为0.9472。

Conclusion: 集成学习方法在脑膜炎诊断中表现出色，为未来AI驱动的脑膜炎诊断工具开发提供了可行路径，尽管直接部署临床诊断工具仍具挑战性。
...
TL;DR: 该研究扩展了先前关于使用乘积系数增强3D LiDAR点云分类的工作，通过结合自编码器表示和KNN分类器，相比PCA基线和早期框架获得了一致的性能提升。


<details>
  <summary>Details</summary>
Motivation: 探索如何通过结合乘积系数特征和自编码器表示来进一步提升LiDAR点云分类性能，并研究不同层次乘积系数对分类效果的影响。

Method: 将乘积系数与自编码器表示相结合，使用KNN分类器进行分类，并逐层添加乘积系数来研究其对分类性能的影响。

Result: 实验表明，结合乘积系数和自编码器表示相比PCA基线和早期框架获得了持续的性能提升，且更丰富的乘积系数集合系统性地提高了类别可分性和整体准确率。

Conclusion: 结合分层乘积系数特征与自编码器能够有效提升LiDAR分类性能，为点云分类提供了有价值的改进方向。
...
TL;DR: 提出熵正则化信任衰减方法，通过压力感知指数倾斜处理分布漂移下的序列决策问题，实现动态遗憾保证和鲁棒性分析。


<details>
  <summary>Details</summary>
Motivation: 研究在分布漂移下的序列决策问题，需要开发能够适应分布变化同时保持性能鲁棒性的算法。

Method: 提出熵正则化信任衰减方法，在信念更新和镜像下降决策中注入压力感知指数倾斜，建立Fenchel对偶等价性，并分析鲁棒性指标如脆弱性、信念带宽和脆弱指数。

Result: 证明了高概率敏感性边界，在KL漂移路径长度S_T下实现Õ(√T)的动态遗憾保证，达到O(1)的每切换遗憾，而无压力更新会产生Ω(1)的尾部损失。

Conclusion: 该框架统一了动态遗憾分析、分布鲁棒目标和KL正则化控制，提供了一种单一的压力自适应更新方法，并扩展到二阶更新、老虎机反馈、异常值处理等场景。
...
TL;DR: FinTrust是一个专门评估金融领域LLM可信赖性的综合基准，测试了11个LLM在多个维度的表现，发现专有模型在安全性方面表现更好，开源模型在特定公平性方面有优势，但所有模型在法律意识方面都存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 由于金融应用具有高风险和高利害关系的特性，在实际金融应用中部署LLM仍然具有挑战性，需要专门的可信赖性评估基准。

Method: 开发了FinTrust基准，基于实际情境设计广泛的对齐问题，并为每个可信赖性评估维度提供细粒度任务。

Result: 评估11个LLM发现：专有模型（如o4-mini）在大多数任务（如安全性）中表现更好；开源模型（如DeepSeek-V3）在特定领域（如行业级公平性）有优势；所有LLM在受托责任对齐和披露等挑战性任务中表现不佳，显示法律意识存在显著差距。

Conclusion: FinTrust可以作为金融领域LLM可信赖性评估的有价值基准，当前LLM在金融应用中的可信赖性仍需改进，特别是在法律合规方面。
...
Method: TESSERA结合专家混合模型的多样性和保形校准，提供具有可靠覆盖保证的每样本不确定性、信息丰富且自适应的预测区间宽度，能够跟踪绝对误差。

Result: 在独立同分布和支架外分布分割下，TESSERA实现了接近名义覆盖率和最佳覆盖-宽度权衡（CWC），同时保持竞争性的适应性（最低AUSE）。大小分层覆盖进一步确认区间大小合适。

Conclusion: TESSERA通过统一专家混合模型多样性和保形校准，提供了可信、紧密且自适应的不确定性，适用于药物发现管道中的选择性预测和下游决策制定。
...
TL;DR: 提出了双权重强化学习（DWRL）框架，将思维链推理与Bradley-Terry模型结合，用于处理非可验证任务的人类偏好建模，在多个基准测试中优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 强化学习在可验证任务中的思维链推理已证明有效，但扩展到非可验证的人类偏好任务仍然具有挑战性且研究不足。

Method: DWRL通过双权重RL目标整合思维链推理与BT模型：实例级错位权重强调与人类偏好不一致的训练对，组级条件偏好分数促进有前景的思考。训练生成偏好模型首先生成思考，然后预测人类偏好分数。

Result: 在多个基准测试和模型规模（Llama3和Qwen2.5）上，DWRL始终优于GPM基线和标量模型，同时产生连贯、可解释的思考。

Conclusion: DWRL作为推理增强偏好学习的通用框架，可扩展到可验证任务之外的应用。
...
TL;DR: 提出基于Transformer的框架，利用候鸟迁徙轨迹预测终点疾病风险，整合GPS追踪、疫情记录和地理空间数据，在测试集上表现优异。
...
Method: 使用Transformer架构，整合多源数据（GPS追踪、疫情记录、地理空间数据），通过H3地理空间编码处理坐标，学习候鸟移动序列的时空依赖性。

Result: 在测试集上表现优异：准确率0.9821、AUC 0.9803、平均精度0.9299、F1分数0.8836。

Conclusion: Transformer架构在禽类疾病监测中具有巨大潜力，可支持早期预警系统，实现及时干预和预防策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动提示搜索方法（如InstructZero）在单一评估分布下优化期望性能，导致提示在分布偏移和对抗评估中性能下降，缺乏可靠性和可迁移性。

Method: 将零样本提示优化建模为鲁棒贝叶斯优化，使用f-散度球定义评估分布的模糊集，通过鲁棒采集规则最大化最坏情况期望效用，同时保持贝叶斯搜索的查询效率。

Result: 在形式化重写任务中准确率从61.3%提升至85-90%，代码调试在域偏移下提升约25个百分点，稳定任务（如因果推理）保持96%以上性能，改进在不同散度选择和解码温度下一致。
...
TL;DR: 本文提出了针对AdamW优化器的权重衰减缩放规则，解决了在尺度不变架构中μP方法因归一化层导致的宽度依赖性问题，实现了跨宽度的学习率和权重衰减零样本迁移。


<details>
  <summary>Details</summary>
Motivation: 现代尺度不变架构中，训练快速进入优化器主导的稳态，归一化层造成反向尺度敏感性，使得有效学习率变得宽度依赖，破坏了μP方法的迁移效果。

Method: 引入权重衰减缩放规则：对于矩阵类参数，学习率η₂∝d⁻¹，权重衰减λ₂∝√d；对于向量类参数，η₁=Θ_d(1)，λ₁=0。通过匹配顶层奇异值来检查子层增益不变性。

Result: 在LLaMA风格Transformer和最小合成设置中验证了该规则的有效性，实现了从代理宽度到目标宽度的学习率和权重衰减的零样本迁移，无需逐宽度调参。
...
TL;DR: RIC-TSC框架将因果发现嵌入时间序列分类，在格陵兰冰上湖演化预测中，因果模型比相关基线在分布外评估中准确率提升12.59%。


<details>
  <summary>Details</summary>
Motivation: 当前地球观测中的时空模型依赖纯相关性特征，在异质域间迁移性差。因果建模能发现稳定不变关系，提高分布偏移下的鲁棒性和泛化能力。

Method: 提出RIC-TSC框架，使用J-PCMCI+进行区域特定的因果发现，将验证的预测因子及其时间滞后输入轻量级分类器。

Result: 在两个对比融化季节的1000个标记湖泊上，因果模型在分布外评估中比相关基线准确率最高提升12.59%。
...
TL;DR: 提出了一种不确定性感知的伪标签框架，用于半监督回归任务，通过双层优化动态调整伪标签影响，有效减轻不可靠伪标签的影响。


<details>
  <summary>Details</summary>
Motivation: 半监督回归中伪标签方法应用较少，由于连续输出和异方差噪声，难以评估伪标签可靠性，简单的伪标签方法会导致错误累积和过拟合。

Method: 采用不确定性感知的伪标签框架，从双层优化角度动态调整伪标签影响，联合最小化所有数据的经验风险和优化不确定性估计以提高泛化能力。

Result: 在多个基准半监督回归数据集上的实验验证了方法的有效性，相比现有方法展现出更好的鲁棒性和性能。

Conclusion: 提出的不确定性感知伪标签框架能够有效解决半监督回归中的伪标签可靠性问题，为半监督回归任务提供了新的解决方案。
...
TL;DR: 该论文提出了一个三阶段框架来描述基础模型如何推动科学范式的转变：元科学整合、人机混合共创和自主科学发现。


<details>
  <summary>Details</summary>
Motivation: 探讨基础模型是仅仅增强现有科学方法，还是正在重新定义科学实践方式，旨在帮助科学界理解基础模型的变革性作用。

Method: 引入三阶段演化框架：(1)元科学整合-在传统范式中增强工作流程；(2)人机混合共创-成为问题制定、推理和发现的积极合作者；(3)自主科学发现-作为独立代理生成新科学知识。

Result: 通过该框架回顾了基础模型在当前科学范式中的应用和新兴能力，识别了风险并提出了未来发展方向。

Conclusion: 基础模型正在催化向新科学范式的过渡，需要科学界反思科学发现的未来。
...
TL;DR: 提出一种结合集成卡尔曼滤波和全连接神经网络的机器学习数据同化方法，使用小规模集成生成初步分析状态，再用神经网络预测修正项来提高精度
...
Method: 使用相对小的集成规模通过EnKF生成初步分析状态，然后用FCNN学习并预测修正项来弥补小集成规模带来的性能下降

Result: 在Lorenz系统和非线性海洋波场模拟实验中，新方法比传统EnKF在相同集成规模下精度更高，且计算成本增加可忽略

Conclusion: EnKF-FCNN方法通过结合传统DA方法和机器学习，在保持计算效率的同时显著提高分析精度，且具有良好的适应性
...
TL;DR: 提出基于CNN、TCN和GRU的神经网络方法，用于自动检测(1+1)维复制过程中的相变和分类隐藏渗流模式
...
Result: 网络成功重现了相图，并为配置分配了相标签，证明深度架构能够从数值实验原始数据中提取层次结构

Conclusion: 深度神经网络能够有效从原始数值实验数据中提取层次结构信息，实现相变的自动检测和隐藏模式的分类
...
TL;DR: DFCA是一种完全去中心化的聚类联邦学习算法，无需中央服务器协调，通过顺序运行平均聚合邻居模型，在稀疏连接下仍能保持与集中式算法相当的性能。
...
Method: 提出DFCA算法，使用顺序运行平均来聚合邻居模型，提供通信高效的替代方案，同时保持聚类性能。

Result: 在多个数据集上的实验表明，DFCA优于其他去中心化算法，性能与集中式IFCA相当，在稀疏连接下仍表现稳健。

Conclusion: DFCA展示了在动态现实世界去中心化网络中的鲁棒性和实用性，解决了集中式方法的局限性。
...
TL;DR: 本文研究了可学习激活函数的随机特征模型(RFLAF)的泛化性质，通过数据依赖的采样方案显著改进了特征数量的理论边界，并提出了近似核算法来学习加权RFLAF。


<details>
  <summary>Details</summary>
Motivation: 研究RFLAF模型的泛化性质，旨在通过数据依赖的采样方案显著减少所需特征数量，提高学习效率。

Method: 采用数据依赖的杠杆加权采样方案，提出近似核算法来学习加权RFLAF，并与普通采样方案进行对比。

Result: 通过加权采样，MSE损失情况下的特征数量边界从Ω(1/ε²)改进为Ω̃((1/ε)^{1/t})，Gram矩阵有限秩时甚至可达Ω(1)；Lipschitz损失边界从Ω(1/ε²)改进为Ω̃((1/ε²)^{1/t})。

Conclusion: 加权RFLAF在显著减少特征数量的情况下仍能达到相同性能，验证了理论的有效性和方法的优越性。
...
TL;DR: 提出基于MoE架构的统一防御框架，通过逻辑多样性损失和鲁棒感知路由器，同时防御图神经网络的后门攻击、边操纵和节点注入攻击。


<details>
  <summary>Details</summary>
Motivation: 现有防御方法通常只针对单一攻击类型，缺乏统一框架来同时防御多种图对抗攻击威胁。

Method: 使用混合专家架构，提出基于互信息的逻辑多样性损失使不同专家关注不同邻域结构，并设计鲁棒感知路由器识别扰动模式并自适应路由受扰节点。

Result: 在各种对抗设置下的广泛实验表明，该方法在对抗多种图对抗攻击时始终实现优越的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有序列建模方法在处理高度非线性和概率性状态动态时存在局限，而算子理论提供了将动态视为希尔伯特空间线性映射的视角，但目前被忽视。

Method: 采用算子理论视角的隐马尔可夫模型，将序列分布嵌入为张量，使用MMD梯度流进行生成；引入谱均值流算法，包括基于谱分解的神经架构和扩展到时间依赖希尔伯特空间的MMD梯度流。

Result: 在多个时间序列建模数据集上展示了有竞争力的结果。

Conclusion: 基于算子理论的序列建模方法为处理复杂状态动态提供了新的有效途径，谱均值流算法解决了可扩展性和采样效率问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的前向-后向表示方法在零样本强化学习中存在表达性不足和外推误差问题，导致表示偏差和次优性能。

Method: 引入行为正则化将策略优化转化为稳定的样本内学习范式；使用任务条件扩散模型提取策略以生成高质量多模态动作分布；采用表达性注意力架构建模环境动态的复杂关系。

Result: 在ExORL和D4RL Kitchen基准测试中，BREEZE达到最佳或接近最佳性能，并展现出优于现有离线零样本RL方法的鲁棒性。

Conclusion: BREEZE通过同时增强学习稳定性、策略提取能力和表示学习质量，有效提升了零样本强化学习的性能。
...
TL;DR: SWFP框架通过离散化流匹配推理过程，将其与最优传输的JKO原理对齐，实现了预训练流模型的稳定在线微调。


<details>
  <summary>Details</summary>
Motivation: 行为克隆中的流/扩散策略虽然擅长从演示中学习复杂技能，但对分布偏移很敏感，标准RL方法难以微调这些模型。

Method: 将全局流分解为一系列小增量变换，每个步骤对应JKO更新，通过级联小流块微调预训练流模型。

Result: 实验证明SWFP在多种机器人控制基准上具有更好的稳定性、效率和适应性能。

Conclusion: SWFP提供了一种稳定高效的流策略微调方法，具有可证明的稳定性和实际性能优势。
...
TL;DR: 提出了GeoMix框架，一种保持Set-SE(3)等变性的几何感知方法，用于准确预测电解质系统中的离子电导率，通过几何交互网络实现分子间几何消息传递。


<details>
  <summary>Details</summary>
Motivation: 当前电解质系统离子电导率预测面临两个基本挑战：缺乏高质量标准化基准数据，以及对混合物系统中几何结构和分子间相互作用的建模不足。

Method: 首先重组和增强CALiSol和DiffMix电解质数据集，加入分子几何图表示；然后提出GeoMix框架，包含专门设计的几何交互网络(GIN)模块，用于保持Set-SE(3)等变性的分子间几何消息传递。

Result: 综合实验表明，GeoMix在两个数据集上持续优于多种基线方法（包括MLP、GNN和几何GNN），验证了跨分子几何相互作用和等变消息传递对准确性质预测的重要性。
...
Method: 使用随机傅里叶特征进行显式特征映射，结合在线动态模态分解，采用Sherman-Morrison更新在滚动窗口内实现连续适应，仅需当前数据无需历史数据存储。

Result: 在多个领域的基准数据集上，WORK-DMD比几种最先进的在线预测方法获得更高准确性，仅需单次数据遍历，在短期预测中表现尤其出色。

Conclusion: 结合核评估与自适应矩阵更新，以最小数据需求实现强大预测性能，为流预测应用提供了深度学习的实用替代方案。
...
TL;DR: ParaFormer是一种浅层Transformer架构，通过并行分支结构实现真正的结构和计算并行，解决了深度模型训练时间长、推理延迟高的问题，性能优于标准Transformer。
...
Result: 实验验证ParaFormer优于标准Transformer如ViT，支持高达15.07倍的模型压缩，在GPU部署上比FairScale等并行方案快3.30倍。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决强化学习在真实世界应用中面临的安全性、样本效率和鲁棒性问题，特别是在排序推荐和生成模型等关键领域，需要开发具有理论保证的算法。

Method: 采用上下文多臂老虎机框架，开发了曝光泛化边界和反事实风险最小化目标确保排序系统安全性；提出基线校正框架统一各种离策略估计器；针对生成RL提出LOOP算法结合PPO和REINFORCE优势。

Result: 提出的方法在排序系统中保证不弱于记录策略，在离策略学习中显著降低方差，在生成RL中LOOP算法达到PPO级样本效率同时生成更符合文本属性的图像。

Conclusion: 通过上下文多臂老虎机框架，该研究为强化学习的安全部署、高效学习和鲁棒应用提供了系统的理论分析和实用算法，在多个重要应用领域取得了显著进展。
...
TL;DR: 本文提出了首个理论框架来分析基于采样的测试时扩展方法，揭示了自一致性和困惑度方法的关键局限性，并提出了RPC混合方法来解决这些问题。


<details>
  <summary>Details</summary>
Motivation: 测试时扩展通过增加计算资源来提升大语言模型的推理性能，但尽管采样方法在实践中成功，其理论基础仍未充分探索。

Method: 提出了RPC混合方法，包含两个关键组件：困惑度一致性（结合自一致性和困惑度的优势）和推理剪枝（消除低概率推理路径）。

Result: 在七个基准数据集上的实验表明，RPC在保持推理性能的同时，将估计误差收敛率从线性提升到指数级，并将采样成本降低了50%。

Conclusion: RPC方法在减少推理错误方面具有强大潜力，不仅提升了置信度可靠性，还显著降低了计算成本。
...
TL;DR: 提出了一种基于鞍点优化的隐变量能量模型训练方法，通过耦合Wasserstein梯度流实现最大似然训练，无需判别器或辅助网络。


<details>
  <summary>Details</summary>
Motivation: 隐变量能量模型能够同时进行表达性生成建模和捕捉隐藏结构，但传统训练方法存在局限性。

Method: 将最大似然训练重新表述为潜变量和联合流形上分布的鞍点问题，使用过阻尼Langevin更新联合负池和条件潜变量粒子，结合随机参数上升。

Result: 在标准平滑性和耗散性假设下证明了存在性和收敛性，在KL散度和Wasserstein-2距离上具有衰减率，且ELBO比受限摊销后验获得的界限更紧。

Conclusion: 该方法在物理系统数值逼近上表现优异，与可比方法竞争性相当。
...
TL;DR: 提出了一种将时序逻辑因果图整合到概率奖励机中的新方法，以解决强化学习在稀疏奖励任务中的困难，加速策略学习并促进任务规范到新环境的迁移。


<details>
  <summary>Details</summary>
Motivation: 强化学习算法在处理奖励反馈稀疏且依赖于环境中复杂事件序列的任务时面临困难。概率奖励机虽然能捕捉奖励信号中的时间依赖性，但难以手动修改和设计，这阻碍了利用高层因果知识和将奖励形式迁移到具有不同因果结构的新领域。

Method: 提出将时序逻辑因果图形式表达的因果信息整合到奖励形式中，通过这种结合来加速策略学习并帮助任务规范迁移到新环境。

Result: 提供了关于该方法收敛到最优策略的理论结果，并通过实证研究展示了其优势。

Conclusion: 通过将因果信息整合到概率奖励机中，可以有效解决强化学习在稀疏奖励任务中的挑战，加速学习过程并促进跨领域迁移。
...
TL;DR: 本文研究了在存在多个正确答案的情况下从演示中学习生成答案的问题，提出了基于奖励模型低基数假设的新方法，替代了传统的最大似然估计方法。


<details>
  <summary>Details</summary>
Motivation: 传统方法假设演示者属于低复杂度策略类，但在实际中奖励模型（指定哪些答案正确）可能具有低基数特性，这是一个更弱的假设。当奖励模型具有低基数时，最大似然估计方法可能会失败。

Method: 将问题形式化为上下文赌博机中的离线模仿学习，基于奖励模型处于低基数类的假设，设计了一种样本复杂度与奖励类基数呈对数关系的新方法。

Result: 证明了最大似然估计方法在这种情况下可能失败，而新方法能够以对数样本复杂度学习，提供了更有效的学习框架。

Conclusion: 当从正确演示中学习时，应该超越最大似然估计方法，特别是在奖励模型具有低基数特性的情况下，本文提出的方法提供了更好的学习保证。
...
Result: 在数值模拟和真实临床数据集上，在似然度、反事实误差和政策评估等指标上表现优于现有方法。

Conclusion: 该方法在避免对抗性方案训练不稳定性的同时，在反事实预测任务中取得了良好性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习需要手动设计模拟器和奖励函数，这通常耗时费力。OffSim旨在直接从专家轨迹中学习这些组件，降低开发成本。

Method: 联合优化高熵转移模型和IRL奖励函数，增强探索性和奖励泛化能力。OffSim+扩展版本引入边际奖励以处理多数据集场景。

Result: 在MuJoCo实验中，OffSim相比现有离线IRL方法取得了显著性能提升，证明了其有效性和鲁棒性。

Conclusion: OffSim框架能够有效从专家轨迹中学习环境动态和奖励结构，实现无需真实环境交互的策略训练，为强化学习应用提供了更便捷的解决方案。
...
TL;DR: 提出了SESA（顺序采样框架），通过顺序生成多样化解题草图来缓解强化学习中探索不足和熵崩溃问题，显著提升LLM的推理多样性和性能。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在提升大语言模型推理能力时存在探索有限和熵崩溃问题，模型容易收敛到相似的解，导致采样多样性丧失，阻碍性能进一步提升。

Method: SESA框架：先顺序生成多样化解题草图，再扩展为完整推理路径。通过将每个新输出条件化于前序输出，确保过程多样性并防止策略崩溃。

Result: 在合成任务中，顺序采样始终优于传统RL方法，路径多样性更高且能更好从崩溃中恢复。在三个智能体基准测试中，成功率分别提升+0.25、+0.42和+0.07（相对基线RL最高提升211%）。
...
TL;DR: Transformer语言模型在离散输入序列到连续表示的映射中是单射的，这意味着可以从模型的隐藏激活中精确恢复原始输入文本。


<details>
  <summary>Details</summary>
Motivation: 挑战传统观点，证明Transformer组件（如非线性激活和归一化）的非单射性并不妨碍语言模型整体上的单射性，这对于模型透明度和可解释性有重要意义。

Method: 首先从数学上证明Transformer语言模型的单射性，然后通过数十亿次碰撞测试进行实证验证，最后开发SipIt算法实现从隐藏激活中高效重建原始输入。

Result: 理论证明和实验验证均表明Transformer语言模型是单射的，SipIt算法能够在线性时间内精确重建输入文本，实践中实现了完全可逆性。
...
TL;DR: 该研究发现知识蒸馏在数据稀缺场景下效果更显著，揭示了数据集大小是影响蒸馏机制的关键变量，驳斥了蒸馏等同于标签平滑的假说，支持暗知识假说。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注知识蒸馏的模型大小和泛化能力，但忽略了数据集大小这一重要维度。本研究旨在探索蒸馏在不同数据规模下的表现，特别是低数据场景下的效果。

Method: 通过跨多个数据集、任务和神经网络架构的大规模实验，系统研究蒸馏效果随数据集大小的变化规律，并测试现有蒸馏理论在不同数据规模下的预测能力。

Result: 实验表明蒸馏效果在低数据场景下不仅保持而且增强，这种现象被称为蒸馏的数据效率。结果反驳了蒸馏等同于标签平滑的假说，进一步支持了暗知识假说。

Conclusion: 数据集大小是理解知识蒸馏机制的一个基本但被忽视的关键变量，在低数据场景下蒸馏具有显著的数据效率优势。
...
TL;DR: 该论文开发了一种用于多变量数据集压缩的神经网络表示方法，通过参数共享实现高效数据压缩，在重建质量、渲染效果和存储效率方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在科学可视化任务中的广泛应用，以及隐式神经表示在时空体积可视化和超分辨率等任务中的成功，激发了开发多变量数据集压缩表示的需求。

Method: 使用单一网络通过参数共享同时学习所有数据变量的表示，实现高效的数据压缩。

Result: 通过综合评估，该方法在重建数据质量、渲染和可视化质量、变量间依赖关系保持以及存储效率方面都表现出优越性能。

Conclusion: 提出的压缩神经表示方法为多变量数据集提供了最先进的数据压缩解决方案，在多个关键指标上均优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 虽然MC Dropout被广泛用于估计模型不确定性，但其在识别分割错误（特别是肿瘤边界附近）方面的有效性尚不清楚，这对于医学图像分割的诊断和治疗规划至关重要。
...
Conclusion: MC Dropout不确定性在边界错误定位方面提供的线索有限，强调了在医学图像分割中需要替代或混合不确定性估计方法的必要性。
...
Result: 实证评估显示SDR优于基线方法，在不同战略强度下实现7.6%-29.3%的偏差减少，且能稳健扩展到大规模代理群体。

Conclusion: 该框架为当代理对干预做出战略响应时提供了一种可靠的因果推断原则性方法。
...
TL;DR: 本文证明了神经特征假设(NFA)在多层线性网络中的深度依赖性，发现NFA指数α=1/L，并揭示了不平衡初始化和非线性激活函数对NFA的影响。


<details>
  <summary>Details</summary>
Motivation: 理解特征学习是建立深度神经网络数学基础的重要开放问题。神经特征假设(NFA)描述了网络训练后第一层权重Gram矩阵与平均梯度外积(AGOP)之间的关系，需要验证其在更广泛网络架构中的有效性。

Method: 使用梯度流动力学和平衡权重初始化，理论分析多层线性网络的训练动态，并通过数值实验验证理论结果，涵盖多种优化算法、权重衰减率和初始化方案。

Result: 对于L≥2层的线性网络，NFA在整个训练过程中成立且指数α=1/L；对于不平衡初始化，应用权重衰减时NFA渐近成立；但某些非线性激活函数的网络架构中NFA不成立。

Conclusion: NFA在多层线性网络中具有深度依赖性，其指数与网络层数成反比，但该假设对网络架构和激活函数具有敏感性，非线性激活可能破坏NFA的成立条件。
...
<details>
  <summary>Details</summary>
Motivation: 受迭代联合图传播(IJGP)算法启发，旨在提高#SAT问题的求解准确性，通过注意力机制关注关键变量和簇，减少冗余计算。

Method: 使用树分解将CNF公式编码为联合图，在联合图上执行迭代消息传递，通过注意力机制在簇内和簇间应用，学习划分函数来近似模型数量。

Result: 实验表明Attn-JGNN模型比其他神经网络方法取得了更好的结果。

Conclusion: 注意力增强的联合图神经网络能够有效提高#SAT问题的求解精度，通过关注关键变量和簇优化了概率推理过程。
...
Method: 采用整体前向传播，通过渐进式聚类剪枝减少延迟，使用双层滑动窗口和分块执行策略来限制峰值内存使用，实现I/O与计算的重叠。

Result: 在0.6B到8B参数的reranker上，GRATING在Apple M2和RTX 5070上分别降低延迟达89.0%和峰值内存达94.9%，在三个真实应用中降低延迟11.6%-51.0%、内存18.6%-77.8%。

Conclusion: GRATING通过利用相对排名和序列级稀疏性，显著提升了边缘设备上语义Top-K选择的效率和可部署性，且无需任何精度损失。
...
TL;DR: 提出CQD-SHAP框架，用于解释复杂查询回答中各个查询部分对特定答案排名的贡献，基于合作博弈论的Shapley值计算。


<details>
  <summary>Details</summary>
Motivation: 现有神经符号CQA方法虽然可追踪中间结果，但无法解释查询不同部分的重要性，存在可解释性问题。

Method: 基于Shapley值构建CQD-SHAP框架，计算查询部分对答案排名的贡献，满足所有基本Shapley公理。

Result: 自动化评估显示该方法在必要性和充分性解释方面对大多数查询类型有效，优于多种基线方法。

Conclusion: CQD-SHAP提供了解释神经预测器如何从不完整知识图谱推断新知识的价值，增强了CQA系统的可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 解决去中心化在线学习中需要手动调参的问题，为分布式感知、去中心化优化和协作机器学习提供参数自由的解决方案。

Method: 结合多智能体投币博弈和gossip步骤，引入新的"投注函数"公式来简化多智能体遗憾分析。

Result: 实现了次线性网络遗憾边界，并在合成和真实数据集上验证了算法性能。

Conclusion: 该算法家族适用于分布式感知、去中心化优化和协作机器学习应用，无需超参数调优即可获得次线性遗憾保证。
...
### [157] [Deep Neural ODE Operator Networks for PDEs](https://arxiv.org/abs/2510.15651)
*Ziqian Li,Kang Liu,Yongcun Song,Hangrui Yue,Enrique Zuazua*

Main category: cs.LG

TL;DR: 提出NODE-ONet框架，结合神经ODE和算子学习，通过物理编码提升PDE求解的准确性和泛化能力
...
Result: 在非线性扩散反应和Navier-Stokes方程上表现出高精度、计算效率和超出训练时间范围的预测能力

Conclusion: 该框架具有灵活性，可适应不同编码器/解码器，并能跨相关PDE族泛化，是科学机器学习中可扩展的物理编码工具
...
### [158] [Fast and Compact Tsetlin Machine Inference on CPUs Using Instruction-Level Optimization](https://arxiv.org/abs/2510.15653)
*Yefan Zeng,Shengyu Duan,Rishad Shafik,Alex Yakovlev*

Main category: cs.LG

TL;DR: 提出了一种基于位运算和提前退出机制的Tsetlin机器高效软件实现，通过字面重排序策略显著提升推理速度，在ARM处理器上实现高达96.71%的推理时间减少。
...
Method: 使用指令级位运算进行紧凑模型表示和加速处理，引入基于AND子句评估的提前退出机制，并通过统计分析提出字面重排序策略。

Result: 在ARM处理器上的实验显示，优化实现相比传统整数实现减少了高达96.71%的推理时间，同时保持相当的代码密度。

Conclusion: 提出的位运算实现和提前退出机制能显著提升Tsetlin机器的推理效率，特别适合资源受限设备部署。
...
TL;DR: CarBoN是一个测试时校准框架，通过自适应调整模型的logits来引导生成更可靠的推理路径，相比传统Best-of-N采样方法，在相同精度下可减少4倍采样次数。


<details>
  <summary>Details</summary>
Motivation: 传统的Best-of-N采样方法在N增加时收益递减，存在效率问题。需要一种无需重新训练大语言模型就能提高推理任务性能的方法。

Method: 提出两阶段方法：先探索解空间，然后通过学习输入特定的温度T和加性偏移向量δ来校准logits，引导生成更可靠的推理路径。

Result: 在MATH-500和AIME-2024上的实验表明，CarBoN可减少高达4倍的采样次数达到相同精度，且在固定预算下通常获得更高精度。

Conclusion: CarBoN框架有效提高了推理任务的效率，T和δ在平衡输出多样性和正确性方面发挥互补作用，且该框架可推广到beam search等步级采样策略。
...
### [160] [WARP-LUTs - Walsh-Assisted Relaxation for Probabilistic Look Up Tables](https://arxiv.org/abs/2510.15655)
*Lino Gerlach,Liv Våge,Thore Gerlach,Elliott Kauffman*

Main category: cs.LG

TL;DR: 本文提出WARP-LUTs方法，通过Walsh变换辅助的概率查找表来高效学习逻辑门组合，相比DLGNs训练更快且参数更少，在CIFAR-10上实现快速收敛并保持准确率。
...
Conclusion: WARP-LUTs为现代FPGA上的高效部署提供了潜力，有望应用于实时科学计算场景。
...
### [161] [KS-Net: Multi-layer network model for determining the rotor type from motor parameters in interior PMSMs](https://arxiv.org/abs/2510.15688)
*Kivanc Dogan,Ahmet Orhan*

Main category: cs.LG

TL;DR: 本研究使用机器学习方法基于电磁参数分类IPMSM转子形状，开发了KS-Net深度学习模型并与传统算法比较，结果显示Cubic SVM和Quadratic SVM达到100%准确率，KS-Net达到99.98%准确率，为电机设计提供了快速替代方案。


<details>
  <summary>Details</summary>
Motivation: 传统使用有限元方法分析IPMSM转子形状计算成本高，需要开发更高效的替代方法。

Method: 使用KS-Net深度学习模型与Cubic SVM、Quadratic SVM、Fine KNN、Cosine KNN和Fine Tree算法比较，在包含9,000个样本的平衡数据集上采用10折交叉验证。
...
<details>
  <summary>Details</summary>
Motivation: 神经网络定理证明生成的证明虽然经过形式系统验证，但长度过长难以理解，限制了数学洞察力。证明简化是重要瓶颈，但训练数据稀缺，现有方法难以处理RL训练证明器生成的极长证明。

Method: 通过专家迭代和强化学习训练ProofOptimizer模型，使用Lean验证简化并提供训练信号。在推理时采用迭代证明缩短工作流程，逐步减少证明长度。

Result: ProofOptimizer在标准基准测试中显著压缩证明：miniF2F减少87%，PutnamBench减少57%，Seed-Prover的IMO 2025证明减少49%。简化后的证明在Lean中检查更快，且作为训练数据能进一步提升下游证明器性能。

Conclusion: ProofOptimizer是首个无需人工监督的Lean证明简化模型，能有效压缩证明长度并提升证明检查效率，同时改善下游证明器的训练效果。
...
### [163] [Constrained Adversarial Perturbation](https://arxiv.org/abs/2510.15699)
*Virendra Nishad,Bhaskar Mukhoty,Hilal AlQuabeh,Sandeep K. Shukla,Sayak Ray Chowdhury*

Main category: cs.LG

TL;DR: 提出了一种在约束特征空间中生成通用对抗扰动的方法CAP，通过增强拉格朗日优化框架强制执行多个复杂约束，在金融、网络等多个领域实现了更高的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的通用对抗扰动方法忽视了特定领域的特征关系约束，违反这些约束会使对抗样本不可信或易被检测，限制了实际应用。

Method: 提出CAP算法，使用基于梯度的交替优化策略解决增强拉格朗日最小最大优化问题，强制执行多个复杂约束。

Result: 在金融、IT网络和网络物理系统等多个领域评估，CAP实现了更高的攻击成功率，同时显著减少了运行时间。

Conclusion: CAP方法在约束特征空间中有效生成对抗扰动，并提出了从数据中学习特征约束的原则性方法，适用于具有结构化输入空间的广泛领域。
...
### [164] [ProSh: Probabilistic Shielding for Model-free Reinforcement Learning](https://arxiv.org/abs/2510.15720)
*Edwin Hamel-De le Court,Gaspard Ohlmann,Francesco Belardinelli*

Main category: cs.LG

TL;DR: ProSh是一种基于风险增强的概率屏蔽模型无关安全强化学习算法，通过在学习过程中应用屏蔽机制确保智能体行为的安全性。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习系统中的安全问题，开发既能达到最优性能又能提供正式安全保证的RL系统。

Method: 在约束MDP状态空间中增加风险预算，使用学习到的成本评论器对智能体策略分布应用屏蔽机制，确保所有采样动作在期望上保持安全。

Result: 在确定性环境中保持最优性，提供仅依赖于备份评论器精度的成本期望紧上界，实验证明在训练期间也能保证安全性。

Conclusion: ProSh是一种有效的模型无关安全强化学习方法，能够在训练和部署阶段提供形式化的安全保证。
...
### [165] [RLAF: Reinforcement Learning from Automaton Feedback](https://arxiv.org/abs/2510.15728)
*Mahyar Alinejad,Alvaro Velasquez,Yue Wang,George Atia*

Main category: cs.LG

TL;DR: 提出了一种基于自动机反馈的强化学习方法，用DFA生成的偏好替代显式奖励函数，解决了复杂历史依赖奖励结构环境中的学习挑战。
...
Method: 使用确定性有限自动机(DFA)结构生成轨迹偏好来学习奖励函数，包括静态方法（直接使用学习到的奖励函数进行策略优化）和动态方法（通过迭代更新不断优化奖励函数和策略）。
...
### [166] [A Comprehensive Evaluation of Graph Neural Networks and Physics Informed Learning for Surrogate Modelling of Finite Element Analysis](https://arxiv.org/abs/2510.15750)
*Nayan Kumar Singh*
...
<details>
  <summary>Details</summary>
Motivation: 有限元分析计算成本高，不适合设计优化问题，需要开发准确且高效的深度学习替代模型。

Method: 使用图神经网络和3D U-Net作为FEA替代模型，引入基于Navier-Cauchy方程的物理信息神经网络框架，并采用课程学习策略（先数据预训练后物理信息微调）。

Result: GNN明显优于U-Net，GCN相对L2误差8.7%，最佳U-Net为13.0%。MPNN和Graph Transformer分别达到3.5%和2.6%的误差。PINN框架将误差降低达11.3%。
...
### [167] [SAMix: Calibrated and Accurate Continual Learning via Sphere-Adaptive Mixup and Neural Collapse](https://arxiv.org/abs/2510.15751)
...
<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法大多关注减轻遗忘和提高准确率，但忽视了网络校准这一重要方面。神经崩溃现象在持续学习中具有优势，但很少有工作致力于改进持续模型的校准以实现更可靠的预测
...
### [168] [Poultry Farm Intelligence: An Integrated Multi-Sensor AI Platform for Enhanced Welfare and Productivity](https://arxiv.org/abs/2510.15757)
...
TL;DR: PoultryFI是一个模块化、经济高效的AI平台，集成了六个AI模块，通过优化摄像头布局、音视频监控、实时鸡蛋计数和预测分析，为中小型家禽养殖场提供持续监测和决策支持。


<details>
  <summary>Details</summary>
Motivation: 中小型家禽养殖场缺乏经济实惠的集成工具进行持续监测和决策，主要依赖人工检查，难以同时满足生产目标、动物福利和环境合规要求。

Method: 使用进化算法优化摄像头布局实现全覆盖，集成音视频监控提取福利指标，边缘视觉模型实现实时鸡蛋计数，预测模型进行产量和饲料消耗预测，结合天气数据提供操作建议。
...
Method: 使用Luminary Cloud平台生成汽车(SHIFT-SUV)和飞机(SHIFT-Wing)数据集，采用AB-UPT模型进行CFD模拟，并与基于transformer的基线模型进行比较。

Result: AB-UPT在两个数据集上都优于之前的transformer基线模型，能够近乎完美地预测集成空气动力，仅需几秒钟即可从简单的各向同性网格几何表示中获得结果。

Conclusion: AB-UPT在汽车和飞机CFD模拟中表现优异，计算效率高，训练速度快，具有工业级应用的潜力。
...
### [170] [Cavity Duplexer Tuning with 1d Resnet-like Neural Networks](https://arxiv.org/abs/2510.15796)
*Anton Raskovalov*

Main category: cs.LG

TL;DR: 提出了一种用于调谐具有大量调节螺钉的空腔双工器的机器学习方法，采用监督学习而非强化学习，神经网络架构包含1D ResNet骨干网络，能够通过4-5次旋转/螺钉达到接近调谐状态。


<details>
  <summary>Details</summary>
Motivation: 针对具有大量调节螺钉的空腔双工器调谐问题，传统方法效率低下，需要开发更高效的自动调谐方法。

Method: 采用监督学习框架，神经网络架构包含1D ResNet骨干网络，并处理S参数的附加信息如曲线形状、峰值位置和幅度，结合外部控制算法。

Result: 该方法能够在每个螺钉仅需4-5次旋转的情况下，使双工器达到接近调谐状态。

Conclusion: 提出的机器学习方法能够有效调谐具有大量调节螺钉的空腔双工器，相比传统方法显著提高了调谐效率。
...
### [171] [Chronos-2: From Univariate to Universal Forecasting](https://arxiv.org/abs/2510.15821)
...
<details>
  <summary>Details</summary>
Motivation: 现有预训练时间序列模型主要关注单变量预测，限制了在现实世界多变量数据和协变量场景中的应用，需要开发能够处理更复杂预测任务的通用模型。

Method: 采用组注意力机制，通过跨多个时间序列的有效信息共享实现上下文学习，使用在单变量序列上施加多样化多变量结构的合成数据集进行训练。

Result: 在fev-bench、GIFT-Eval和Chronos Benchmark II三个综合基准测试中达到最先进性能，在涉及协变量的任务中显著优于基线模型，在能源和零售领域的案例研究进一步证明了其实用优势。

Conclusion: Chronos-2的上下文学习能力使其成为一个通用预测模型，可以在现实世界预测管道中直接使用，无需特定任务训练。
...
### [172] [SNOO: Step-K Nesterov Outer Optimizer - The Surprising Effectiveness of Nesterov Momentum Applied to Pseudo-Gradients](https://arxiv.org/abs/2510.15830)
*Dominik Kallusky,Vinay Rao,Vishal Nandavanam,Hao-Jun Michael Shi*

Main category: cs.LG

TL;DR: 论文提出SNOO优化器，通过将Nesterov动量应用于伪梯度，在非分布式训练中实现1.5-2.5倍的计算效率提升，且改进效果随模型规模增大而增强。
...
Method: 提出SNOO（Step-K Nesterov Outer Optimizer），这是Lookahead优化器的变体，核心是在两循环框架中对伪梯度应用Nesterov动量来更新慢权重。

Result: SNOO在非分布式设置中实现了1.5-2.5倍的计算效率提升，训练规模可达1e23 FLOPs，且改进效果随模型规模增大而增强。
...
### [173] [Self-Certifying Primal-Dual Optimization Proxies for Large-Scale Batch Economic Dispatch](https://arxiv.org/abs/2510.15850)
...
<details>
  <summary>Details</summary>
Motivation: 虽然优化代理在平均情况下表现良好，但最坏情况下的最优性差距可能很大，难以在实际中信任预测结果。需要在经典求解器和优化代理之间找到平衡，实现可信部署。
...
Conclusion: 混合求解器成功地在优化代理和经典求解器之间找到了平衡，实现了可解释的速度-最优性权衡，为优化代理的可信部署提供了实用解决方案。
...
### [174] [FIDDLE: Reinforcement Learning for Quantum Fidelity Enhancement](https://arxiv.org/abs/2510.15833)
*Hoang M. Ngo,Tamer Kahveci,My T. Thai*

Main category: cs.LG

TL;DR: 提出FIDDLE框架，通过高斯过程代理模型和强化学习模块，在量子电路路由阶段直接最大化过程保真度，显著提升量子电路的可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前量子设备受噪声影响，可靠性降低。在门基量子计算中，如何在路由阶段提高量子电路的过程保真度是一个关键挑战。

Method: FIDDLE框架包含两个模块：基于高斯过程的代理模型用于有限训练样本下的过程保真度估计，以及强化学习模块用于优化路由。

Result: 与现有技术相比，提出的代理模型能提供更好的过程保真度估计，端到端框架在各种噪声模型下显著提高了量子电路的过程保真度。

Conclusion: FIDDLE是首个直接最大化过程保真度的方法，优于依赖电路深度或门数等间接指标的传统方法。
...
### [175] [Transfer Orthology Networks](https://arxiv.org/abs/2510.15837)
*Vikash Singh*

Main category: cs.LG

TL;DR: TRON是一个新颖的跨物种迁移学习神经网络架构，利用同源关系构建二分图来指导知识迁移，通过物种转换层将源物种基因表达映射到目标物种基因空间。


<details>
  <summary>Details</summary>
Motivation: 解决跨物种迁移学习问题，利用生物同源关系来有效利用可用的转录组数据，实现更有效的知识迁移。

Method: 在预训练的前馈神经网络前添加学习的物种转换层，该层的权重通过同源关系的二分图邻接矩阵进行掩码，学习从源物种到目标物种基因表达的线性变换。

Result: TRON提供了一种生物学基础和可解释的跨物种迁移学习方法，为更有效利用转录组数据铺平了道路，目前正在收集跨物种数据以进行实验验证。

Conclusion: TRON架构通过利用同源关系实现了有效的跨物种知识迁移，同时提供了功能同源性解释的途径，具有生物学基础和可解释性优势。
...
### [176] [Learning Correlated Reward Models: Statistical Barriers and Opportunities](https://arxiv.org/abs/2510.15839)
*Yeshwanth Cherapanamjeri,Constantinos Daskalakis,Gabriele Farina,Sobhan Mohammadpour*

Main category: cs.LG

TL;DR: 本文研究学习相关probit模型的统计和计算挑战，发现传统的成对偏好数据无法学习相关性信息，而三选一偏好数据能有效克服这一限制，并提出高效估计器。


<details>
  <summary>Details</summary>
Motivation: 随机效用模型在人类反馈强化学习中很重要，但许多技术依赖无关替代品独立性假设，这限制了人类偏好的精细建模。避免该假设的模型缺乏统计和计算保证。

Method: 研究相关probit模型的学习挑战，分析不同数据收集范式（成对偏好vs三选一偏好），提出统计和计算高效的估计器。

Result: 理论证明成对偏好数据无法学习相关性信息，三选一偏好数据能克服此限制，提出的估计器具有近最优性能，并在真实数据集上验证了改进的个性化效果。

Conclusion: 高阶偏好数据在学习相关效用方面具有优势，能够实现更精细的人类偏好建模，提高个性化效果。
...
### [177] [OpenEstimate: Evaluating LLMs on Reasoning Under Uncertainty with Real-World Data](https://arxiv.org/abs/2510.15096)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界部署的语言模型需要处理不完整信息并在不确定性下进行推理，但现有评估主要关注有明确答案的问题，缺乏对不确定性推理能力的系统评估。

Method: 开发了OpenEstimate基准测试，通过数值估计任务评估语言模型，要求模型综合大量背景信息并以概率先验形式表达预测，评估这些先验的准确性和校准度。

Result: 在六个前沿语言模型上的测试显示，模型产生的先验通常不准确且过于自信，性能仅轻微受不确定性提取方式影响，而采样策略、推理努力或提示设计的改变影响不大。
...
### [178] [Procedural Game Level Design with Deep Reinforcement Learning](https://arxiv.org/abs/2510.15120)
*Miraç Buğra Özkan*

Main category: cs.AI

TL;DR: 提出了一种在Unity 3D环境中使用深度强化学习进行程序化关卡设计的新方法，包含两个智能体：蜂鸟（解谜者）和浮岛（生成可收集物品），通过PPO算法训练实现自主关卡生成和解决。


<details>
  <summary>Details</summary>
Motivation: 利用程序化内容生成技术减少游戏开发中的人工工作量，创造动态、可重玩和可扩展的环境，探索AI在创意游戏开发中的潜力。

Method: 使用Unity ML-Agents工具包的PPO算法训练两个智能体：蜂鸟智能体学习导航和收集花朵，浮岛智能体基于障碍物位置、蜂鸟初始状态和性能反馈生成花朵布局。

Result: 系统产生了涌现行为，在各种环境配置中表现出强大的泛化能力，不仅产生了高效智能体行为，还为基于机器学习的自主游戏关卡设计开辟了新机会。

Conclusion: 这项工作突显了深度强化学习在使智能体生成和解决虚拟环境内容方面的潜力，推动了AI在创意游戏开发过程中的边界。
...
### [179] [Towards Error Centric Intelligence I, Beyond Observational Learning](https://arxiv.org/abs/2510.15128)
*Marcus A. Thomas*

Main category: cs.AI

TL;DR: 该论文认为AGI进展受理论限制而非数据或规模限制，提出了因果力学框架，强调假设空间变化作为首要操作，通过错误发现和修正机制来推进AGI发展。


<details>
  <summary>Details</summary>
Motivation: 挑战柏拉图表示假说，认为仅凭观测充分性无法保证干预能力，因为观测等价的世界在干预下可能产生分歧，需要转向以错误为中心的视角。

Method: 提出因果力学框架，将假设空间变化作为首要操作，引入局部性和自主性原则、独立因果机制和组合自主性原则等结构原则，使错误发现和修正变得可行。

Result: 建立了理论框架，提供了可操作的诊断工具，旨在构建能够将不可达错误转化为可达错误并进行修正的系统支架。

Conclusion: AGI发展需要理论突破，因果力学框架通过关注错误发现和假设空间扩展，为解决观测学习的局限性提供了新途径。
...
### [180] [HugAgent: Evaluating LLMs in Simulating Human-Like Individual Reasoning on Open-Ended Tasks](https://arxiv.org/abs/2510.15144)
...
TL;DR: HugAgent是一个用于评估AI模型如何适应个体推理风格的基准测试，包含合成和人类两个轨道，旨在推动机器推理更贴近人类个体思维。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型主要反映群体共识，缺乏对个体推理风格和信念轨迹的个性化捕捉，需要开发能够模拟人类个体推理的AI系统。

Method: 采用双轨道设计：合成轨道用于规模化和系统性压力测试，人类轨道提供生态有效的"出声思考"推理数据，评估模型对个体推理演化的捕捉能力。

Result: 实验显示最先进的LLM在个体适应方面仍存在持续差距，HugAgent成为首个可扩展的基准测试，用于对齐机器推理与人类思维的个体性。

Conclusion: HugAgent为评估机器推理与人类个体思维的匹配度提供了首个可扩展基准，推动了更人性化AI推理的发展。
...
### [181] [WELD: A Large-Scale Longitudinal Dataset of Emotional Dynamics for Ubiquitous Affective Computing](https://arxiv.org/abs/2510.15221)
*Xiao Sun*

Main category: cs.AI

TL;DR: 提出了一个包含733,651条面部表情记录的大规模纵向工作场所情绪数据集，覆盖38名员工30.5个月的自然办公环境数据，包含情绪概率、元数据和扩展情绪指标。


<details>
  <summary>Details</summary>
Motivation: 解决真实工作环境中大规模、纵向情绪数据稀缺的问题，捕捉自然环境下员工情绪动态。

Method: 使用深度学习面部表情识别技术收集数据，包含7种基本情绪概率，并提供32种扩展情绪指标计算。

Result: 数据集质量验证成功复制已知心理模式（周末效应+192%效价改善），员工流失预测AUC=1.0，基线实验情绪分类准确率91.2%，效价预测R2=0.84。

Conclusion: 这是目前公开可用的最大、最长的纵向工作场所情绪数据集，支持情绪识别、情感动态建模、情绪传染、流失预测等研究。
...
### [182] [From Checklists to Clusters: A Homeostatic Account of AGI Evaluation](https://arxiv.org/abs/2510.15236)
*Brett Reynolds*

Main category: cs.AI

TL;DR: 本文提出AGI评估应基于稳态属性簇概念，采用因果中心性加权和跨会话持久性测试，以解决传统评估中领域权重对称和快照测试的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前AGI评估存在两个问题：(1) 对称权重假设所有领域同等重要，与人类智力研究不符；(2) 快照测试无法区分持久能力和脆弱表现。作者认为通用智力应理解为稳态属性簇。

Method: 提出两种电池兼容的扩展：基于CHC理论的中心性优先评分和簇稳定性指数家族，分别用于领域加权和测试能力持久性、学习持久性和错误纠正。

Result: 这些扩展保持了多领域广度，同时减少了脆弱性和博弈行为，提供了可测试的预测和黑盒协议。

Conclusion: AGI评估应关注能力的因果中心性和跨会话持久性，新的评估框架能更准确地反映通用智力的本质特征。
...
### [183] [Multi-dimensional Data Analysis and Applications Basing on LLM Agents and Knowledge Graph Interactions](https://arxiv.org/abs/2510.15258)
...
<details>
  <summary>Details</summary>
Motivation: 在大数据时代，从海量、异构、复杂关联的多维数据中提取深度洞察面临挑战。LLM存在"幻觉"问题且难以实时更新，而知识图谱的静态特性限制了动态交互和分析能力。

Method: 利用LLM代理从非结构化数据中自动提取产品数据，实时构建和可视化知识图谱，并通过交互平台支持用户对图节点进行深度探索和分析。

Result: 实验结果表明，该方法在产品生态系统分析、关系挖掘和用户驱动探索分析方面具有显著优势。
...
### [184] [Experience-Driven Exploration for Efficient API-Free AI Agents](https://arxiv.org/abs/2510.15259)
*Chenwei Tang,Jingyu Xing,Xinyu Liu,Zizhou Wang,Jiawei Du,Liangli Zhen,Jiancheng Lv*

Main category: cs.AI

TL;DR: KG-Agent是一个经验驱动的学习框架，通过将像素级GUI交互构建为状态-动作知识图谱，解决了API缺失环境下LLM代理的低效探索和短视决策问题。


<details>
  <summary>Details</summary>
Motivation: 现有软件大多缺乏API，代理只能通过像素级GUI操作，导致LLM代理面临严重效率瓶颈：局限于局部视觉体验，做出短视决策，依赖低效试错，阻碍技能获取和长期规划。

Method: 提出KG-Agent框架，将原始像素交互构建为持久的状态-动作知识图谱(SA-KG)，连接功能相似但视觉不同的GUI状态。设计基于图谱拓扑的混合内在奖励机制，结合状态价值奖励和探索新颖性奖励。

Result: 在Civilization V和Slay the Spire两个复杂开放GUI决策环境中评估，相比最先进方法在探索效率和策略深度上有显著提升。

Conclusion: KG-Agent通过知识图谱结构化和混合奖励机制，有效解决了API缺失环境下代理的探索低效和长期规划问题，实现了更好的策略学习和决策性能。
...
### [185] [AUGUSTUS: An LLM-Driven Multimodal Agent System with Contextualized User Memory](https://arxiv.org/abs/2510.15261)
*Jitesh Jain,Shubham Maheshwari,Ning Yu,Wen-mei Hwu,Humphrey Shi*

Main category: cs.AI

TL;DR: AUGUSTUS是一个多模态智能体系统，采用基于认知科学中人类记忆理念的图结构多模态上下文记忆，通过语义标签和上下文关联实现高效的概念驱动检索。


<details>
  <summary>Details</summary>
Motivation: 现有系统主要存储文本信息，忽略了多模态信号的重要性。受人类记忆多模态特性的启发，希望开发一个更符合人类记忆认知的多模态智能体系统。

Method: 系统包含四个循环阶段：编码（理解输入）、存储记忆（保存重要信息）、检索（从记忆中搜索相关上下文）、行动（执行任务）。使用图结构多模态上下文记忆，将信息概念化为语义标签并与上下文关联。

Result: 在ImageNet分类任务中比传统多模态RAG方法快3.5倍，在MSC基准测试中优于MemGPT。

Conclusion: AUGUSTUS系统通过图结构多模态上下文记忆和概念驱动检索，在多模态任务中表现出色，验证了基于人类记忆认知理念的系统设计的有效性。
...
### [186] [WebGen-V Bench: Structured Representation for Enhancing Visual Design in LLM-based Web Generation and Evaluation](https://arxiv.org/abs/2510.15306)
*Kuang-Da Wang,Zhao Wang,Yotaro Shimose,Wei-Yao Wang,Shingo Takamatsu*

Main category: cs.AI

TL;DR: WebGen-V是一个用于指令到HTML生成的新基准和框架，通过代理爬取真实网页、结构化数据表示和多模态评估协议，提升了数据质量和评估粒度。
...
### [187] [VERITAS: Leveraging Vision Priors and Expert Fusion to Improve Multimodal Data](https://arxiv.org/abs/2510.15317)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态模型的监督微调数据质量不足，存在事实错误和幻觉问题，主要原因是视觉感知不充分。

Method: 使用视觉识别模型(RAM++)和OCR系统(PP-OCRv4)提取结构化视觉先验，结合三个LMM模型评估原始答案，通过统计融合得到高置信度共识分数作为真实标签，训练轻量级批评模型，最终生成精炼答案。

Result: 在六个多模态基准测试中，使用VERITAS处理数据微调的模型性能优于使用原始数据的模型，在文本丰富和细粒度推理任务中表现尤为突出。

Conclusion: VERITAS能有效提升SFT数据质量，批评模型在保持高性能的同时显著提升了效率，为多模态数据优化研究提供了新思路。
...
### [188] [Towards Flash Thinking via Decoupled Advantage Policy Optimization](https://arxiv.org/abs/2510.15374)
*Zezhong Tan,Hang Gao,Xinhong Ma,Feng Zhang,Ziqiang Dong*

Main category: cs.AI

TL;DR: DEPO是一个新的强化学习框架，通过优势解耦算法、难度感知长度惩罚和优势裁剪方法，显著减少大型推理模型在简单任务上的冗余推理和过长响应，在保持准确率的同时降低39%的序列长度。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习算法虽然提高了模型准确性，但存在响应过长和过度思考问题，导致推理延迟和计算消耗增加，特别是在简单任务上需要最小化推理时。

Method: 提出DEPO框架，包含三个核心组件：(1) 创新的优势解耦算法指导模型减少低效token；(2) 难度感知长度惩罚降低模型响应总体长度；(3) 优势裁剪方法防止策略优化偏差。

Result: 在DeepSeek-Distill-Qwen-7B和1.5B模型上，DEPO实现了序列长度减少39%，减少了低效token中的过度推理路径，同时在整体准确率上优于基础模型。

Conclusion: DEPO框架有效解决了大型推理模型在简单任务上的冗余推理问题，在保持性能的同时显著降低了计算开销和推理延迟。
...
### [189] [Advancing Routing-Awareness in Analog ICs Floorplanning](https://arxiv.org/abs/2510.15387)
*Davide Basso,Luca Bortolussi,Mirjana Videnovic-Misic,Husni Habal*

Main category: cs.AI

TL;DR: 提出基于强化学习和关系图卷积神经网络的自动布局引擎，通过提高网格分辨率和精确引脚信息集成，结合动态布线资源估计技术，在模拟环境中相比现有学习方法减少13.8%死区空间、40.6%线长，提升73.4%布线成功率。


<details>
  <summary>Details</summary>
Motivation: 解决模拟集成电路布局中由于电气约束和问题特定约束的严格要求，以及布局规划与布线步骤的相互依赖性，导致机器学习技术应用受限的问题，满足布局工程师对路由感知布局解决方案的需求。

Method: 使用强化学习和关系图卷积神经网络开发自动布局引擎，结合提高网格分辨率、精确引脚信息集成和动态布线资源估计技术，平衡布线效率和面积效率。

Result: 在模拟环境中测试，相比过去基于学习的最先进技术，实现死区空间减少13.8%，线长减少40.6%，布线成功率提高73.4%。

Conclusion: 所提出的方法能够生成更易布线的布局结果，满足工业标准，有效解决了模拟集成电路布局中路由感知布局规划的挑战。
...
### [190] [Corrigibility Transformation: Constructing Goals That Accept Updates](https://arxiv.org/abs/2510.15395)
*Rubi Hudson*
...
<details>
  <summary>Details</summary>
Motivation: AI训练过程中，部分学习的目标往往会激励AI避免进一步的目标更新，因为大多数目标通过持续追求能更好地实现。可修正性允许纠正错误和适应人类偏好的变化，是重要的安全属性。

Method: 提供可修正性的形式化定义，引入一种转换方法，通过短视地获取在无成本阻止更新条件下的奖励预测来构建任何可修正目标的修正版本。该方法可递归扩展到新创建的代理，并防止代理故意修改其目标。
...
### [191] [MARS: Reinforcing Multi-Agent Reasoning of LLMs through Self-Play in Strategic Games](https://arxiv.org/abs/2510.15414)
...
<details>
  <summary>Details</summary>
Motivation: 虽然强化学习在单智能体任务中有效，但在多轮次、多智能体场景中的扩展仍未被充分探索，主要面临长时程信用分配和智能体特定优势估计的挑战。

Method: MARS框架包含回合级优势估计器来对齐学习信号进行信用分配，以及智能体特定优势归一化来稳定多智能体训练，通过自博弈在合作和竞争游戏中学习。

Result: 从Qwen3-4B训练的MARS智能体在保留游戏中性能提升达28.7%，在推理基准测试中集成到领先多智能体系统后，AIME提升10.0%，GPQA-Diamond提升12.5%。

Conclusion: 在战略游戏中使用自博弈的端到端强化学习训练是开发LLM可泛化多智能体推理能力的有效方法。
...
### [192] [Adaptive Minds: Empowering Agents with LoRA-as-Tools](https://arxiv.org/abs/2510.15416)
...
Method: 使用LangGraph进行工作流管理，基础LLM作为语义路由器分析查询并动态选择相关LoRA适配器，支持API和Web界面。

Result: 系统能够提供准确的专业化响应，同时保持对话能力，实现了领域自适应AI辅助的可扩展基础。

Conclusion: Adaptive Minds提供了一个开源、可扩展的领域自适应AI辅助框架，结合了多智能体灵活性和参数高效微调的优势。
...
### [193] [Taming the Judge: Deconflicting AI Feedback for Stable Reinforcement Learning](https://arxiv.org/abs/2510.15514)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在判断不一致性问题，特别是偏好循环等逻辑一致性问题尚未充分解决，这会破坏强化学习的稳定性。

Method: 引入冲突检测率(CDR)量化判断冲突，提出去冲突图奖励(DGR)框架，通过构建偏好图、转换为无冲突有向无环图(DAG)来净化信号，生成逻辑一致的奖励信号。
...
### [194] [Hypergraph Contrastive Sensor Fusion for Multimodal Fault Diagnosis in Induction Motors](https://arxiv.org/abs/2510.15547)
*Usman Ali,Ali Zia,Waqas Ali,Umer Ramzan,Abdul Rehman,Muhammad Tayyab Chaudhry,Wei Xiang*

Main category: cs.AI

TL;DR: 提出MM-HCAN多模态超图对比注意力网络，用于工业电机多故障诊断，在三个真实基准测试中达到99.82%准确率，具有强跨域泛化能力和噪声鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统方法难以捕捉复杂多模态信号关系，局限于单模态数据或单一故障类型，在噪声或跨域条件下性能下降。需要统一的鲁棒故障诊断框架。

Method: MM-HCAN将对比学习集成到多模态传感器融合的超图拓扑中，联合建模模态内和模态间依赖关系，超越欧几里得嵌入空间增强泛化能力。
...
### [195] [JudgeSQL: Reasoning over SQL Candidates with Weighted Consensus Tournament](https://arxiv.org/abs/2510.15560)
*Jiayuan Bai,Xuan-guang Pan,Chongyang Tao,Shuai Ma*

Main category: cs.AI

TL;DR: JudgeSQL是一个新的SQL候选选择框架，通过结构化推理和加权共识锦标赛机制来解决文本到SQL任务中候选查询选择的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的SQL候选选择方法（如自一致性或最佳N解码）只能提供浅层信号，容易导致不一致的评分、脆弱的推理链，并且无法捕捉密切相关的SQL候选之间的细粒度语义差异。

Method: 开发了一个基于推理的SQL判断模型，通过强化学习在可验证奖励的指导下提炼推理轨迹；构建了一个加权共识锦标赛，将显式推理偏好与隐式生成器置信度相结合。

Result: 在BIRD基准测试上的广泛实验表明，JudgeSQL表现出卓越的SQL判断能力，具有良好的跨尺度泛化能力和对生成器容量的鲁棒性。
...
### [196] [Context-aware deep learning using individualized prior information reduces false positives in disease risk prediction and longitudinal health assessment](https://arxiv.org/abs/2510.15591)
*Lavanya Umapathy,Patricia M Johnson,Tarun Dutt,Angela Tong,Madhur Nayan,Hersh Chandarana,Daniel K Sodickson*

Main category: cs.AI

TL;DR: 开发了一个机器学习框架，通过整合患者历史就诊信息来改进健康监测，特别适用于就诊次数有限且频率不固定的情况。该模型在预测前列腺癌风险方面显著降低了假阳性率。


<details>
  <summary>Details</summary>
Motivation: 医学中的时间背景对于评估患者健康状况随时间变化至关重要，特别是在就诊次数有限且频率不固定的情况下，需要有效整合历史信息来改进健康监测。

Method: 模型首先使用最近一次就诊的医疗数据估计疾病初始风险，然后利用先前收集的影像学和/或临床生物标志物信息来优化评估。

Result: 在预测临床显著前列腺癌风险时，整合历史背景将假阳性转化为真阴性，总体特异性提高同时保持高敏感性。整合最多三次先前影像检查信息时，假阳性率从51%降至33%，进一步包括临床数据后降至24%。预测五年内前列腺癌风险时，假阳性率从64%降至9%。

Conclusion: 随时间收集的信息为医学风险预测提供了相关背景，能显著提高特异性。对于多种进展性疾病，通过背景信息充分降低假阳性率可为低基线风险的大规模人群提供纵向健康监测途径，实现早期检测和改善健康结果。
...
### [197] [Unleashing Scientific Reasoning for Bio-experimental Protocol Generation via Structured Component-based Reward Mechanism](https://arxiv.org/abs/2510.15600)
*Haoran Sun,Yankai Jiang,Zhenyu Tang,Yaning Pan,Shuang Gu,Zekai Lin,Lilong Wang,Wenjie Lou,Lei Liu,Lei Bai,Xiaosong Wang*

Main category: cs.AI

TL;DR: 提出了SciRecipe数据集和"Sketch-and-Fill"范式，开发了Thoth模型用于自动生成精确可执行的科学实验协议，显著提升了协议生成的完整性和一致性。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型生成的实验协议往往不完整或不一致，限制了其在科学复现中的实用性，需要改进协议生成的质量和可靠性。

Method: 引入SciRecipe数据集（12K+结构化协议），提出"Sketch-and-Fill"范式分离分析、结构化和表达步骤，采用基于组件的奖励机制评估步骤粒度、行动顺序和语义保真度，通过分阶段的知识到行动过程训练Thoth模型。
...
### [198] [Build Your Personalized Research Group: A Multiagent Framework for Continual and Interactive Science Automation](https://arxiv.org/abs/2510.15624)
*Ed Li,Junyu Ren,Xintian Pan,Cat Yan,Chuanhao Li,Dirk Bergemann,Zhuoran Yang*

Main category: cs.AI

TL;DR: 提出了freephdlabor，一个开源的多智能体框架，具有完全动态的工作流程和模块化架构，支持自动上下文压缩、工作区通信、内存持久化和非阻塞人工干预，将自动化研究转变为持续的研究程序。
...
Method: 开发了freephdlabor框架，采用完全动态的工作流程（由实时智能体推理决定）和模块化架构，用户可以修改、添加或删除智能体来满足特定领域需求。

Result: 该框架提供了全面的基础设施，包括自动上下文压缩、基于工作区的通信、跨会话内存持久化和非阻塞人工干预机制。
...
### [199] [Direct Preference Optimization with Unobserved Preference Heterogeneity: The Necessity of Ternary Preferences](https://arxiv.org/abs/2510.15716)
*Keertana Chidambaram,Karthik Vinary Seetharaman,Vasilis Syrgkanis*

Main category: cs.AI

TL;DR: 本文提出了一种改进的RLHF框架，通过引入多响应排序和考虑异构用户偏好，解决了传统方法中二元比较的局限性，并建立了公平性和个性化的生成模型对齐理论框架。


<details>
  <summary>Details</summary>
Motivation: 传统RLHF和DPO方法假设统一的标注者偏好并依赖二元比较，忽略了人类评估者的多样性和成对反馈的局限性。本文旨在解决这两个关键问题。

Method: 1) 将RLHF中的偏好学习与计量经济学文献联系起来，证明二元比较不足以识别潜在用户偏好；2) 开发了DPO的期望最大化变体，发现潜在标注者类型并相应训练混合LLM；3) 提出使用最小最大遗憾公平准则的聚合算法来生成具有公平性能保证的单一生成策略。

Result: 建立了从有限用户数据和无限用户中识别潜在用户偏好的理论保证，证明了三个或更多响应的排序能够确保可识别性，而二元比较则不能。

Conclusion: 这些贡献共同建立了一个理论和算法框架，用于在生成模型对齐中实现多样用户的公平性和个性化。
...
### [200] [Invoice Information Extraction: Methods and Performance Evaluation](https://arxiv.org/abs/2510.15727)
...
Method: 使用Docling和LlamaCloud Services预处理发票，提取关键字段如发票号、日期、总金额和供应商详情。

Result: 建立了包含字段级精度、一致性检查失败和精确匹配准确率的评估框架。

Conclusion: 提出的评估指标为比较不同提取方法提供了标准化方式，并能揭示字段特定性能的优缺点。
...
### [201] [AURA: An Agent Autonomy Risk Assessment Framework](https://arxiv.org/abs/2510.15739)
*Lorenzo Satta Chiris,Ayush Mishra*

Main category: cs.AI

TL;DR: AURA是一个统一的框架，用于检测、量化和减轻自主AI代理带来的风险，采用基于gamma的风险评分方法，支持人机协同监督和自主风险评估。


<details>
  <summary>Details</summary>
Motivation: 随着自主AI代理系统在组织中的采用增加，对齐、治理和风险管理方面的持续挑战阻碍了大规模部署，需要有效的风险检测和缓解框架。

Method: 引入基于gamma的风险评分方法，平衡风险评估准确性与计算效率；提供交互式流程来评分、评估和缓解AI代理风险；设计人机协同监督机制和代理-人通信机制。

Result: AURA框架能够支持负责任和透明的自主AI代理采用，提供强大的风险检测和缓解功能，同时平衡计算资源需求。

Conclusion: AURA是企业在环境中实现大规模、可治理的自主AI代理的关键推动因素，通过统一的风险评估框架解决部署挑战。
...
### [202] [Towards Relaxed Multimodal Inputs for Gait-based Parkinson's Disease Assessment](https://arxiv.org/abs/2510.15748)
*Minlin Zeng,Zhipeng Zhou,Yang Qiu,Zhiqi Shen*

Main category: cs.AI

TL;DR: 提出了首个将帕金森病评估的多模态学习建模为多目标优化问题的系统TRIP，解决了训练时模态同步和推理时模态依赖的限制，在异步和同步设置下均取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有帕金森病评估的多模态方法存在两个主要限制：(1)训练时需要同步所有模态，(2)推理时依赖所有模态，这阻碍了实际应用。

Method: 将多模态学习建模为多目标优化问题，引入基于边界的类别重平衡策略来处理模态内不平衡问题，支持训练和推理时的灵活模态要求。

Result: 在三个公共数据集上的实验表明，TRIP在异步设置下比最佳基线提升16.48、6.89和11.55个百分点，在同步设置下提升4.86和2.30个百分点。
...
### [203] [Preliminary Quantitative Study on Explainability and Trust in AI Systems](https://arxiv.org/abs/2510.15769)
*Allen Daniel Sunny*

Main category: cs.AI

TL;DR: 该研究通过定量实验设计探讨了解释性与用户对AI系统信任之间的关系，发现交互性解释能增强用户参与度和信任度，解释的清晰度和相关性是信任的关键决定因素。


<details>
  <summary>Details</summary>
Motivation: 随着GPT-4等大规模AI模型在法律、医疗和金融等关键领域的部署，关于AI系统信任和透明度的问题变得日益紧迫，需要研究解释性如何影响用户信任。

Method: 采用定量实验设计，通过基于网络的交互式贷款审批模拟，比较从基本特征重要性到交互式反事实解释等不同类型解释对感知信任的影响。
...
### [204] [Self-evolving expertise in complex non-verifiable subject domains: dialogue as implicit meta-RL](https://arxiv.org/abs/2510.15772)
*Richard M. Bailey*

Main category: cs.AI

TL;DR: Dialectica框架通过结构化对话、记忆、自我反思和策略约束的上下文编辑，使AI代理能够在复杂问题中发展专业知识，实验证明该方法优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 解决复杂多维度问题（如司法框架、环境污染等）时，现有LLM缺乏通过经验发展专业知识的内生机制，需要一种框架来增强AI代理在开放不可验证领域中的专业能力。

Method: 提出Dialectica框架，让代理在定义的主题上进行结构化对话，结合记忆、自我反思和策略约束的上下文编辑，将讨论视为隐式元强化学习过程。

Result: 在两个模型架构上的实验显示，启用基于反思的上下文编辑的代理在Elo分数、标准化Bradley-Terry-Davidson能力和AlphaRank质量上均优于基线模型，反思能可靠地识别弱点并塑造后续陈述。
...
### [205] [Demo: Guide-RAG: Evidence-Driven Corpus Curation for Retrieval-Augmented Generation in Long COVID](https://arxiv.org/abs/2510.15782)
*Philip DiGiacomo,Haoyang Wang,Jinrui Fang,Yan Leng,W Michael Brode,Ying Ding*

Main category: cs.AI

TL;DR: 开发了六种检索增强生成(RAG)语料库配置用于长新冠临床问答，评估发现结合临床指南和高质量系统评价的配置表现最佳。


<details>
  <summary>Details</summary>
Motivation: 随着AI聊天机器人在临床医学中的应用增加，为复杂新兴疾病开发有效框架面临挑战，特别是针对长新冠这样的疾病。

Method: 开发了六种RAG语料库配置，从专家精选源到大规模文献数据库，使用LLM作为评估框架，在忠实性、相关性和全面性指标上评估。
...
### [206] [PokeeResearch: Effective Deep Research via Reinforcement Learning from AI Feedback and Robust Reasoning Scaffold](https://arxiv.org/abs/2510.15862)
*Yi Wan,Jiuqi Wang,Liam Li,Jinsong Liu,Ruihao Zhu,Zheqing Zhu*

Main category: cs.AI

TL;DR: PokeeResearch-7B是一个7B参数的深度研究代理，通过统一的强化学习框架构建，在10个深度研究基准测试中实现了最先进的性能。
...
Method: 采用无标注的AI反馈强化学习(RLAIF)框架，使用基于LLM的奖励信号优化策略，捕捉事实准确性、引用忠实性和指令遵循性；通过思维链驱动的多调用推理支架增强鲁棒性，实现自我验证和从工具故障中自适应恢复。

Result: 在10个流行的深度研究基准测试中，PokeeResearch-7B在7B规模的深度研究代理中实现了最先进的性能。
