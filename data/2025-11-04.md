...
### [1] [Med-Banana-50K: A Cross-modality Large-Scale Dataset for Text-guided Medical Image Editing](https://arxiv.org/abs/2511.00801)
*Zhihui Chen,Mengling Feng*

Main category: cs.CV

TL;DR: 提出了Med-Banana-50K数据集，这是一个包含50K图像的医学图像编辑数据集，涵盖三种模态和23种疾病类型，通过Gemini-2.5-Flash-Image生成双向编辑，并采用医学质量控制方法确保质量。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在医学图像编辑方面取得了显著进展，但研究社区缺乏大规模、高质量、公开可用的专门针对医学图像编辑的数据集，这些数据集需要满足严格的解剖学和临床约束。

Method: 利用Gemini-2.5-Flash-Image从真实医学图像生成双向编辑（病变添加和移除），采用LLM-as-Judge方法进行医学质量控制，包括指令合规性、结构合理性、真实性和保真度保持，并进行最多五轮的历史感知迭代优化。

Result: 构建了包含50K图像的Med-Banana-50K数据集，涵盖胸部X光、脑部MRI和眼底摄影三种模态，23种疾病类型，还包括37K次失败尝试的完整对话记录，用于偏好学习和对齐研究。

Conclusion: Med-Banana-50K为训练和评估下一代医学图像编辑模型奠定了基础，通过提供大规模、医学验证且完全记录的资源，推动了医学图像编辑研究的发展。
...
TL;DR: 该论文提出了基于运动捕捉数据的交互式AI模型，用于生成舞蹈动作，通过扩散模型、运动修复和风格迁移技术创造性地增强人类动作序列。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型缺乏具身交互体验，舞蹈作为人类表达的基本形式，可以补充这种体验，探索创造性的人机交互。

Method: 结合两个扩散模型、运动修复和运动风格迁移，利用单人运动数据和高层特征生成时间一致且对选定运动参考有响应的运动表示。

Result: 通过定量评估生成样本特征分布与测试集的收敛性，证明模型成功生成多样化且现实的舞蹈动作，显示出与人类舞伴的各种偏离。
...
### [3] [SEPS: Semantic-enhanced Patch Slimming Framework for fine-grained cross-modal alignment](https://arxiv.org/abs/2511.01390)
...
<details>
  <summary>Details</summary>
Motivation: 当前方法在处理视觉补丁冗余和模糊性方面面临挑战，MLLMs虽然能生成丰富语义但可能与原始稀疏描述冲突，且难以准确量化视觉补丁与文本描述之间的语义相关性。
...
Conclusion: SEPS框架通过语义增强的补丁精简有效解决了跨模态对齐中的补丁冗余和模糊性问题，为视觉问答等应用提供了更精确的局部对应关系。
...
### [4] [Deep Learning Models for Coral Bleaching Classification in Multi-Condition Underwater Image Datasets](https://arxiv.org/abs/2511.00021)
*Julio Jerison E. Macrohon,Gordon Hung*

Main category: cs.CV

TL;DR: 提出基于机器学习的珊瑚白化分类系统，在包含不同环境条件的全球数据集上，CNN模型经过超参数调优达到88%的最高准确率，优于现有基准。


<details>
  <summary>Details</summary>
Motivation: 珊瑚礁面临污染、海洋酸化和海水温度异常的威胁，需要高效的保护和监测方法。
...
### [5] [How Far Are Surgeons from Surgical World Models? A Pilot Study on Zero-shot Surgical Video Generation with Expert Assessment](https://arxiv.org/abs/2511.01775)
*Zhen Chen,Qing Xu,Jinlin Wu,Biao Yang,Yuhao Zhai,Geng Guo,Jing Zhang,Yinlu Ding,Nassir Navab,Jiebo Luo*

Main category: cs.CV

TL;DR: SurgVeo是首个专家策划的手术视频生成模型评估基准，结合手术合理性金字塔(SPP)框架评估模型从基本外观到复杂手术策略的生成能力。研究发现Veo-3模型在视觉感知合理性方面表现优异，但在器械操作、环境反馈和手术意图等更高层次合理性上存在明显差距。


<details>
  <summary>Details</summary>
Motivation: 视频生成基础模型在模拟物理世界方面展现出强大能力，但在需要深度专业因果知识的高风险领域如手术中应用仍存在关键空白。

Method: 提出SurgVeo基准和SPP四层评估框架，让Veo-3模型在腹腔镜和神经外科手术剪辑上进行零样本预测任务，由四位认证外科医生按SPP标准评估生成视频。
...
### [6] [Automating Coral Reef Fish Family Identification on Video Transects Using a YOLOv8-Based Deep Learning Pipeline](https://arxiv.org/abs/2511.00022)
*Jules Gerard,Leandro Di Bella,Filip Huyghe,Marc Kochzius*

Main category: cs.CV

TL;DR: 使用YOLOv8深度学习管道自动化西印度洋珊瑚礁鱼类识别，在肯尼亚和坦桑尼亚视频样带中测试了24个鱼类科别，为区域自动化监测提供了首个基准。
...
Conclusion: 深度学习可作为传统监测方法的可扩展补充，具有自动化珊瑚礁监测的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的InfoNCE损失方法虽然能减少人工标注，但数据选择和增强仍依赖人工假设或工程，可能不是最优的。特别是对比学习中的数据增强主要关注颜色抖动来模拟真实世界光照变化，存在局限性。
...
TL;DR: 本研究对NVIDIA FLARE、Flower和Owkin Substra三个联邦学习框架在医学影像应用中的表现进行了基准测试，评估了它们在模型性能、收敛效率、通信开销、可扩展性和开发者体验等方面的差异。
...
Method: 使用PathMNIST数据集，对三个主流FL框架（NVIDIA FLARE、Flower、Owkin Substra）进行系统性基准测试，评估多个关键指标。

Result: NVIDIA FLARE在生产可扩展性方面表现最佳，Flower在原型设计和学术研究方面更具灵活性，Owkin Substra在隐私保护和合规性方面表现突出。

Conclusion: 不同FL框架各有优势，适用于不同的使用场景，在医疗环境的实际部署中都具有重要价值。
...
### [9] [Enhancing rice leaf images: An overview of image denoising techniques](https://arxiv.org/abs/2511.00046)
*Rupjyoti Chutia,Dibya Jyoti Bora*

Main category: cs.CV

TL;DR: 该论文对水稻叶片图像的多种去噪方法结合CLAHE增强技术进行了比较研究，通过实验验证了不同方法在图像增强方面的效果。


<details>
  <summary>Details</summary>
Motivation: 水稻叶片图像分析在疾病检测、营养评估和生长分析中至关重要，而图像增强作为预处理步骤能提高后续任务的可靠性。去噪和对比度增强是主要步骤，需要系统评估不同方法的有效性。

Method: 采用多种知名图像去噪方法与CLAHE（对比度受限自适应直方图均衡化）相结合，在水稻叶片图像数据集上进行实验，并使用多种指标全面测试增强方法。

Result: 通过实验获得了不同去噪方法结合CLAHE在水稻叶片图像上的增强效果数据，为评估数字图像处理方法有效性提供了坚实基础。

Conclusion: 该研究为数字图像处理方法有效性评估提供了可靠基础，并揭示了在农业研究和其他领域未来应用的实用见解。
...
TL;DR: 该研究系统分析了不同LiDAR扫描模式（重复扫描与非重复扫描）对路边感知性能的影响，创建了InfraLiDARs' Benchmark数据集，发现非重复扫描LiDAR与128线重复扫描LiDAR在检测性能上相当，但前者成本更低。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LiDAR的优化布置，而不同扫描模式对感知性能的深远影响研究不足，特别是传统重复扫描与新兴非重复扫描系统在点云分布上的差异会显著影响物体检测效果。

Method: 在CARLA仿真环境中创建InfraLiDARs' Benchmark数据集，使用同时运行的两种扫描模式的基础设施LiDAR，进行统计分析并评估不同3D物体检测算法性能。

Result: 非重复扫描LiDAR和128线重复扫描LiDAR在各种场景下表现出相当的检测性能，尽管非重复扫描LiDAR感知范围有限，但考虑到其低成本，是性价比较高的选择。
...
TL;DR: Cosmos-Predict2.5是新一代物理AI世界基础模型，基于流式架构统一文本、图像、视频到世界的生成，结合Cosmos-Reason1提供更丰富的文本基础和更精细的世界模拟控制。


<details>
  <summary>Details</summary>
Motivation: 构建更可靠的物理AI系统，支持合成数据生成、策略评估和机器人闭环模拟，推动具身智能的发展。

Method: 采用流式架构，结合200M精选视频片段训练，并通过强化学习后训练优化，同时开发了Cosmos-Transfer2.5用于Sim2Real和Real2Real世界转换。

Result: 相比Cosmos-Predict1在视频质量和指令对齐方面有显著提升，提供2B和14B规模模型，Cosmos-Transfer2.5虽比前代小3.5倍但提供更高保真度和鲁棒的长序列视频生成。
...
TL;DR: 使用深度学习进行高山栖息地变化检测，比较后分类变化检测与直接变化检测两种方法，在复杂自然环境中评估地理空间基础模型的表现。


<details>
  <summary>Details</summary>
Motivation: 高山生态系统面临快速气候变化，需要频繁的栖息地监测，但人工测绘成本过高。研究旨在填补地理空间基础模型在复杂自然环境中的应用空白。

Method: 使用Gesaeuse国家公园的长期高山栖息地数据，比较后分类变化检测（评估Prithvi-EO-2.0和Clay v1.0与U-Net CNN）和直接变化检测（测试ChangeViT与U-Net基线），使用高分辨率多模态数据（RGB、NIR、LiDAR、地形属性）。

Result: Clay v1.0在多类栖息地变化中达到51%总体准确率，U-Net为41%；直接变化检测在二元变化检测中IoU为0.53，但多类检测准确率仅为28%；LiDAR集成将语义分割准确率从30%提升至50%。
...
TL;DR: LeMiCa是一个无需训练的高效扩散视频生成加速框架，通过词典最小最大路径优化策略显著提升生成质量与推理速度，在多个基准测试中实现2.9倍加速且感知质量损失最小。
...
Method: 将缓存调度建模为带有误差加权边的有向图，引入词典最小最大路径优化策略，明确限制最坏情况路径误差，从而改善全局内容和风格一致性。

Result: 在多个文本到视频基准测试中，LeMiCa在推理速度和生成质量上均实现双重提升，在Latte模型上达到2.9倍加速，在Open-Sora上LPIPS分数达到0.05，优于现有缓存技术。

Conclusion: LeMiCa为加速扩散视频生成提供了一个稳健且可推广的范式，可作为未来高效可靠视频合成研究的坚实基础。
...
Method: 三阶段框架：1) 训练轻量级残差执行器探测VLA通用模型的失败区域；2) 使用混合rollout方案收集与通用模型部署分布对齐的轨迹；3) 通过标准SFT将精选轨迹蒸馏回通用模型。

Result: 在LIBERO上达到接近饱和的99%任务成功率，在SimplerEnv上获得超过50%的提升，在真实世界Franka和YAM机械臂操作任务上实现100%成功率。
...
Result: 在脊柱解剖结构CT图像上取得优越分割性能，开发了支持点、框和文本提示的交互软件，94.3%解析准确率，响应时间低于800ms。

Conclusion: SpinalSAM-R1系统有效提升了脊柱CT图像分割性能，支持自然语言交互，为临床诊断和治疗提供了实用工具。
...
### [16] [A filtering scheme for confocal laser endomicroscopy (CLE)-video sequences for self-supervised learning](https://arxiv.org/abs/2511.00098)
*Nils Porsche,Flurin Müller-Diesing,Sweta Banerjee,Miguel Goncalves,Marc Aubreville*

Main category: cs.CV

TL;DR: 提出了一种用于共聚焦激光内窥镜(CLE)视频序列的过滤方法，通过减少自监督学习(SSL)训练中的数据集冗余，提高训练效率和收敛性。


<details>
  <summary>Details</summary>
Motivation: CLE图像难以解释，机器学习可辅助诊断但面临数据不足导致过拟合的问题。SSL可用于大规模未标记数据，但CLE视频帧间相关性高导致数据分布不均衡。

Method: 使用四种最先进的基线网络和基于视觉变换器小骨干的SSL师生网络，在鼻窦肿瘤和皮肤鳞状细胞癌数据集的下游任务中进行评估。
...
### [17] [FreeSliders: Training-Free, Modality-Agnostic Concept Sliders for Fine-Grained Diffusion Control in Images, Audio, and Video](https://arxiv.org/abs/2511.00103)
*Rotem Ezra,Hedi Zisling,Nimrod Berman,Ilan Naiman,Alexey Gorkor,Liran Nochumsohn,Eliya Nachmani,Omri Azencot*

Main category: cs.CV

TL;DR: FreeSliders是一种无需训练、模态无关的方法，通过在推理过程中部分估计概念滑块公式，实现对扩散模型中特定概念的精细控制生成。


<details>
  <summary>Details</summary>
Motivation: 现有概念滑块方法需要针对每个概念进行训练和特定架构微调，限制了在新模态上的可扩展性。

Method: 提出完全无需训练的方法，部分估计概念滑块公式，并引入两阶段程序自动检测饱和点并重新参数化遍历过程。

Result: 实验表明该方法支持即插即用的跨模态概念控制，优于现有基线方法。

Conclusion: FreeSliders为可控生成提供了新的工具，建立了首个多模态精细概念生成控制基准。
...
### [18] [AI Powered High Quality Text to Video Generation with Enhanced Temporal Consistency](https://arxiv.org/abs/2511.00107)
*Piyushkumar Patel*
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到视频生成方法在保持时间一致性、组合理解和视觉叙事的细粒度控制方面存在困难，需要更先进的框架来解决这些问题。

Method: MOVAI引入了三个关键创新：组合场景解析器（CSP）将文本描述分解为带时间标注的分层场景图；时空注意力机制（TSAM）确保帧间连贯运动动态同时保留空间细节；渐进式视频细化（PVR）模块通过多尺度时间推理迭代提升视频质量。

Result: 在标准基准测试中，MOVAI实现了最先进的性能，在LPIPS指标上提升15.3%，FVD指标上提升12.7%，用户偏好研究中提升18.9%，特别擅长生成具有真实时间动态和细粒度语义控制的复杂多对象场景。

Conclusion: MOVAI框架在文本到视频生成方面表现出色，特别是在处理复杂多对象场景和实现细粒度语义控制方面具有显著优势，为高质量视频合成提供了有效的解决方案。
...
TL;DR: 提出了一种名为"Chain of Time"的认知启发方法，通过生成模拟过程中的中间图像序列来改进和解释视觉语言模型中的物理模拟，无需额外微调即可在推理时使用。


<details>
  <summary>Details</summary>
Motivation: 受机器学习中的上下文推理和人类心理模拟启发，旨在改进视觉语言模型对物理过程的模拟能力，并揭示模型内部对物理动态的理解。

Method: Chain-of-Time方法在推理时生成一系列中间图像来模拟物理过程，应用于2D图形模拟和真实3D视频，测试速度、加速度、流体动力学和动量守恒等物理属性。

Result: 使用Chain-of-Time方法显著提升了最先进图像生成模型的性能，分析揭示了模型能够模拟随时间展开的物理属性（如速度、重力和碰撞），但也发现模型在某些情况下难以从输入图像推断特定物理参数。

Conclusion: Chain-of-Time方法不仅提升了物理模拟性能，还提供了对模型内部物理推理过程的深入洞察，揭示了传统评估方法无法观察到的物理理解能力。
...
### [20] [End-to-End Framework Integrating Generative AI and Deep Reinforcement Learning for Autonomous Ultrasound Scanning](https://arxiv.org/abs/2511.00114)
...
TL;DR: 提出首个结合生成AI和深度强化学习的端到端框架，实现自主可复现的心脏超声扫描，解决传统方法依赖操作者、缺乏可复现性和使用简化模型的问题。


<details>
  <summary>Details</summary>
Motivation: 心脏超声诊断存在操作者依赖、时间限制和人为错误等问题，偏远地区缺乏专业医师，需要自动化解决方案确保一致性和可及性。现有深度强化学习方法缺乏可复现性、依赖专有数据且使用简化模型。

Method: 框架包含两个组件：(1) 结合GAN和VAE的条件生成模拟器，生成逼真的动作条件图像；(2) 深度强化学习模块，利用模拟器学习自主准确的扫描策略。

Result: VAE-GAN在现有GAN变体中表现优异，深度强化学习扫描系统在不同配置下均证明有效，框架支持AI驱动的图像类型分类和质量评估，并能生成逼真超声图像。

Conclusion: 该框架为自主心脏超声扫描建立了可复现的基础，可扩展到其他器官，并发布了公开可用的真实心脏超声数据集以确保可复现性。
...
### [21] [VLM6D: VLM based 6Dof Pose Estimation based on RGB-D Images](https://arxiv.org/abs/2511.00120)
*Md Selim Sarowar,Sungho Kim*

Main category: cs.CV

TL;DR: VLM6D提出了一种双流架构，利用RGB-D输入的视觉和几何数据优势，通过Vision Transformer和PointNet++编码器分别处理RGB和点云数据，在严重遮挡场景下实现鲁棒的6D物体姿态估计。


<details>
  <summary>Details</summary>
Motivation: 当前6D物体姿态估计方法在从合成数据泛化到真实世界时存在困难，特别是在光照变化、无纹理物体和严重遮挡等挑战性场景下表现脆弱。

Method: 采用双流架构：使用自监督Vision Transformer（DINOv2）处理RGB模态，利用其预训练的视觉理解能力；使用PointNet++编码器处理深度数据生成的3D点云，进行几何推理。两个特征流有效融合后输入多任务预测头。

Result: 在具有挑战性的Occluded-LineMOD数据集上取得了新的SOTA性能，验证了其优越的鲁棒性和准确性。

Conclusion: VLM6D通过整合视觉和几何数据的互补优势，解决了6D姿态估计在真实世界场景中的泛化问题，特别是在遮挡、光照变化和无纹理物体等挑战性条件下表现出色。
...
### [22] [Integrating ConvNeXt and Vision Transformers for Enhancing Facial Age Estimation](https://arxiv.org/abs/2511.00123)
*Gaby Maroun,Salah Eddine Bekhouche,Fadi Dornaika*

Main category: cs.CV

TL;DR: 提出了一种结合ConvNeXt和Vision Transformer的混合架构用于人脸年龄估计，通过CNN的局部特征提取和Transformer的全局注意力机制互补，在多个基准数据集上取得了优越性能。


<details>
  <summary>Details</summary>
Motivation: 人脸年龄估计是计算机视觉中的复杂挑战，现有模型各有优势但存在局限性，需要结合CNN的局部特征提取能力和Transformer的全局注意力机制来提升性能。

Method: 使用预训练模型构建ConvNeXt-ViT混合架构，通过线性层和高级正则化技术优化模型，特别在CNN框架中采用适配的注意力机制来聚焦年龄相关面部特征。

Result: 在MORPH II、CACD和AFAD等基准数据集上评估，在平均绝对误差(MAE)指标上表现优于传统方法，消融研究验证了各组件的重要性。

Conclusion: ConvNeXt-ViT混合架构不仅超越了传统方法，还为年龄估计和相关视觉任务的未来发展提供了坚实基础，展示了混合架构在解决复杂计算机视觉挑战中的变革潜力。
...
### [23] [FLoC: Facility Location-Based Efficient Visual Token Compression for Long Video Understanding](https://arxiv.org/abs/2511.00141)
*Janghoon Cho,Jungsoo Lee,Munawar Hayat,Kyuwoong Hwang,Fatih Porikli,Sungha Choi*

Main category: cs.CV

TL;DR: FLoC是一个基于设施定位函数的高效视觉令牌压缩框架，通过选择紧凑且具有代表性的视觉令牌子集来解决长视频理解中的可扩展性问题。


<details>
  <summary>Details</summary>
Motivation: 长视频理解中，大型多模态模型面临视觉令牌数量过多导致的可扩展性限制问题。

Method: 采用基于设施定位函数的训练无关方法，结合惰性贪心算法，在预定义预算内快速选择具有代表性和多样性的视觉令牌子集。

Result: 在Video-MME、MLVU和LongVideoBench等大规模基准测试中，FLoC始终优于现有压缩技术，显著减少视觉令牌数量同时保持接近最优性能。

Conclusion: FLoC提供了一个训练无关、模型无关和查询无关的通用解决方案，能够无缝集成到各种视频-LLM和现有工作流程中，有效解决长视频理解的关键挑战。
...
### [24] [BlurGuard: A Simple Approach for Robustifying Image Protection Against AI-Powered Editing](https://arxiv.org/abs/2511.00143)
*Jinsu Kim,Yunhun Nam,Minseon Kim,Sangpil Kim,Jongheon Jeong*

Main category: cs.CV

TL;DR: 本文提出了一种通过自适应高斯模糊增强图像保护对抗噪声的方法，使对抗噪声更难被检测和逆转，从而更有效地防止恶意使用文本到图像模型进行图像编辑。


<details>
  <summary>Details</summary>
Motivation: 随着文本到图像模型的发展，恶意图像编辑威胁日益严重。现有的图像保护方法通过在图像中植入对抗噪声来阻止编辑，但这些噪声容易被简单技术（如JPEG压缩）逆转，限制了实际应用效果。

Method: 提出了一种简单而有效的方法：在噪声上应用自适应区域高斯模糊来调整整体频率谱，增强对抗噪声的不可逆性。

Result: 实验表明，该方法能显著提高现有保护方法对抗各种逆转技术的性能，同时减少噪声引起的图像质量下降。

Conclusion: 通过增强对抗噪声的不可逆性，可以有效提升图像保护方法的实用性，为防范恶意图像编辑提供了更可靠的解决方案。
...
### [25] [CompAgent: An Agentic Framework for Visual Compliance Verification](https://arxiv.org/abs/2511.00171)
*Rahul Ghosh,Baishali Chaudhury,Hari Prasanna Das,Meghana Ashok,Ryan Razkenari,Sungmin Hong,Chun-Hao Liu*

Main category: cs.CV

TL;DR: 提出了CompAgent，首个用于视觉合规验证的智能体框架，通过结合多模态大语言模型和视觉工具，实现动态规划和多模态推理，在UnsafeBench数据集上达到76% F1分数，比现有最佳方法提升10%。


<details>
  <summary>Details</summary>
Motivation: 视觉合规验证在媒体、娱乐和广告等领域至关重要，但现有方法依赖特定任务的深度学习模型，构建成本高且泛化能力有限。多模态大语言模型虽然具备广泛知识，但难以处理细粒度视觉细节和结构化合规规则。

Method: CompAgent框架包含规划代理和验证代理。规划代理根据合规政策动态选择视觉工具（如物体检测器、人脸分析器、NSFW检测器等），验证代理整合图像、工具输出和政策上下文进行多模态推理。

Result: 在公开基准测试中，CompAgent优于专用分类器、直接MLLM提示和精心设计的路由基线，在UnsafeBench数据集上达到76% F1分数，比现有最佳方法提升10%。
...
### [26] [From Evidence to Verdict: An Agent-Based Forensic Framework for AI-Generated Image Detection](https://arxiv.org/abs/2511.00181)
*Mengfei Liang,Yiting Qu,Yukun Jiang,Michael Backes,Yang Zhang*

Main category: cs.CV

TL;DR: AIFo是一个基于多智能体协作的训练免费框架，通过模拟人类法医调查过程来检测AI生成图像，显著优于传统分类器和视觉语言模型。
...
Method: 采用多智能体协作框架，整合反向图像搜索、元数据提取、预训练分类器和VLM分析等法医工具，通过结构化多智能体辩论机制处理冲突证据，并配备记忆增强推理模块。

Result: 在6000张图像的全面评估中，AIFo达到97.05%的准确率，显著优于传统分类器和最先进的VLM方法。
...
### [27] [A Retrospect to Multi-prompt Learning across Vision and Language](https://arxiv.org/abs/2511.00191)
...
Method: 将模态间隙现象扩展到可学习提示，提出基于能量的多提示学习（EMPL），从VLMs隐式定义的能量分布中采样生成多个提示嵌入。

Result: 综合实验验证了多提示学习的优势以及EMPL方法的卓越性能。

Conclusion: EMPL方法不仅参数高效，而且严格实现了领域内和领域外开放词汇泛化之间的平衡，为多提示学习提供了有效的解决方案。
...
### [28] [An Efficient and Generalizable Transfer Learning Method for Weather Condition Detection on Ground Terminals](https://arxiv.org/abs/2511.00211)
*Wenxuan Zhang,Peng Hu*

Main category: cs.CV

TL;DR: 提出一种高效的迁移学习方法，用于卫星互联网地面终端组件的细粒度天气条件检测，能够检测雪、湿和其他恶劣天气相关状况，性能优于主流深度学习模型。


<details>
  <summary>Details</summary>
Motivation: 恶劣天气事件对低轨卫星互联网性能和可靠性有显著影响，需要细粒度的地面终端组件天气条件检测能力来协助故障诊断和缓解，但现有解决方案缺乏且难以在实际部署中有效泛化。

Method: 采用高效的迁移学习方法，使地面组件能够本地检测代表性的天气相关状况，包括雪、湿和其他由恶劣和典型天气事件导致的状况。

Result: 所提出的迁移学习方法在检测性能上优于YOLOv7、YOLOv9、Faster R-CNN和R-YOLO等典型深度学习方法，并显示出在各种场景下的良好泛化能力。

Conclusion: 该迁移学习方法能够有效检测卫星互联网地面终端组件的天气相关状况，具有优越性能和泛化能力，为可靠的卫星互联网提供了故障诊断和缓解支持。
...
### [29] [DM-QPMNET: Dual-modality fusion network for cell segmentation in quantitative phase microscopy](https://arxiv.org/abs/2511.00218)
*Rajatsubhra Chakraborty,Ana Espinosa-Momox,Riley Haskin,Depeng Xu,Rosario Porras-Aguilar*

Main category: cs.CV

TL;DR: 提出DM-QPMNet双编码器网络，通过多头注意力机制融合偏振强度图像和相位图两种模态特征，实现稳健的细胞分割
...
Method: 使用双编码器分别处理两种模态，通过多头注意力在中间深度融合模态特定特征，采用双源跳跃连接和每模态归一化

Result: 相比单一模态基线和简单拼接方法有显著改进

Conclusion: 模态特定编码与可学习融合能有效利用ssQPM同时捕获的互补照明和相位线索，实现稳健细胞分割
...
### [30] [Towards 1000-fold Electron Microscopy Image Compression for Connectomics via VQ-VAE with Transformer Prior](https://arxiv.org/abs/2511.00231)
...
Method: 使用向量量化变分自编码器(VQ-VAE)进行压缩，结合Transformer先验模型预测底层token，通过FiLM和连接恢复纹理，并引入ROI驱动的工作流进行选择性高分辨率重建。

Result: 实现了16x到1024x的压缩比，支持仅解码顶部层级的极端压缩，以及通过先验模型恢复纹理细节。

Conclusion: 该压缩框架为海量EM数据提供了高效的存储、传输和分析解决方案，支持灵活的按需解码策略。
...
### [31] [Hyperbolic Optimal Transport](https://arxiv.org/abs/2511.00244)
*Yan Bin Ng,Xianfeng Gu*

Main category: cs.CV

TL;DR: 提出了一种在双曲空间中计算最优传输映射的新算法，扩展了欧几里得和球面几何的方法，适用于处理层次数据、网络和多亏格黎曼曲面。


<details>
  <summary>Details</summary>
Motivation: 现有最优传输映射计算方法主要针对欧几里得空间和球面，但在涉及层次数据、网络和多亏格黎曼曲面的应用中，需要在双曲空间中计算最优传输映射。

Method: 使用几何变分技术，将欧几里得和球面几何的方法扩展到双曲空间设置，提出了一种新颖高效的计算算法。

Result: 在合成数据和多亏格曲面模型上进行了实验，验证了所提方法的有效性。

Conclusion: 成功开发了双曲空间中的最优传输映射计算方法，为处理层次结构和复杂几何数据提供了有效工具。
...
### [32] [Object-Aware 4D Human Motion Generation](https://arxiv.org/abs/2511.00248)
*Shurui Gui,Deep Anil Patel,Xiner Li,Martin Renqiang Min*

Main category: cs.CV

TL;DR: 提出了MSDI框架，通过3D高斯表示和运动扩散先验，结合LLM的空间语义信息，实现零样本的4D人体运动生成，解决视频生成中的物理不一致问题。
...
Method: 使用预生成的3D人体和物体，结合LLM的空间语义信息和运动扩散先验，提出运动分数蒸馏采样(MSDS)进行空间感知运动优化。

Result: 实验表明该框架能生成自然且物理合理的人体运动，尊重3D空间上下文，为真实4D生成提供可扩展解决方案。

Conclusion: 提出的零样本方法无需重新训练，能泛化到分布外物体感知的人体运动，优于需要有限交互数据集联合训练的现有方法。
...
### [33] [Merlin L48 Spectrogram Dataset](https://arxiv.org/abs/2511.00252)
*Aaron Sun,Subhransu Maji,Grant Van Horn*

Main category: cs.CV

TL;DR: 提出了L48数据集，这是一个真实世界的细粒度多标签数据集，用于评估单正多标签(SPML)方法，揭示了现有方法在真实场景中的性能差距。


<details>
  <summary>Details</summary>
Motivation: 现有的SPML方法都是在合成数据集上开发和评估的，这些数据集无法反映真实世界的复杂性和细粒度特征，导致方法在实际应用中的性能被高估。

Method: 构建了L48数据集，这是一个基于鸟类声音记录的真实世界细粒度多标签数据集，提供了自然的SPML标注设置，以及两个利用领域先验获得额外负标签的扩展设置。

Result: 在L48数据集上对现有SPML方法进行基准测试，发现与合成数据集相比存在显著性能差异，并分析了方法的弱点。

Conclusion: 需要更真实和具有挑战性的基准数据集来推动SPML方法的发展，L48数据集为此提供了重要工具。
...
### [34] [BeetleFlow: An Integrative Deep Learning Pipeline for Beetle Image Processing](https://arxiv.org/abs/2511.00255)
...
TL;DR: 开发了一个3阶段自动化流水线，用于处理大规模甲虫图像数据，包括检测、裁剪和形态分割，以提高生物研究效率。


<details>
  <summary>Details</summary>
Motivation: 昆虫学和生态学研究中需要处理大量甲虫图像数据，传统人工方法效率低下，需要自动化处理流程来加速生物研究。

Method: 使用基于transformer的开放词汇目标检测器和视觉语言模型进行迭代检测，然后手动标注670张甲虫图像并微调transformer分割模型进行精细分割。

Result: 构建了一个专门用于甲虫图像处理的深度学习流水线，能够相对准确地实现甲虫的检测和形态分割。

Conclusion: 该流水线整合了多种深度学习方法，专门针对甲虫图像处理，能显著提高大规模甲虫数据处理效率，加速生物研究进程。
...
### [35] [MambaNetLK: Enhancing Colonoscopy Point Cloud Registration with Mamba](https://arxiv.org/abs/2511.00260)
*Linzhe Jiang,Jiayuan Huang,Sophia Bano,Matthew J. Clarkson,Zhehua Mao,Mobarak I. Hoque*

Main category: cs.CV

TL;DR: 提出MambaNetLK，一种基于Mamba状态空间模型的无对应点云配准方法，在临床结肠镜导航数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决内窥镜导航中生物组织重复纹理和局部同质几何特征导致的特征退化问题，以及术前解剖与术中观察之间的领域偏移导致的配准不稳定问题。

Method: 将Mamba状态空间模型作为跨模态特征提取器集成到PointNetLK架构中，利用Lucas-Kanade算法进行迭代配准，有效捕获长程依赖关系。

Result: 在临床数据集C3VD-Raycasting-10k上，相比次优方法，中值旋转误差降低56.04%，RMSE平移误差降低26.19%。在ModelNet40上表现出强泛化能力和对初始姿态扰动的鲁棒性。

Conclusion: MambaNetLK为手术导航中的3D配准提供了鲁棒基础，结合全局表达性SSM特征提取器和大规模临床数据集，能够在微创手术中实现更准确可靠的引导系统。
...
### [36] [Spot The Ball: A Benchmark for Visual Social Inference](https://arxiv.org/abs/2511.00261)
*Neha Balamurugan,Sarah Wu,Adam Chun,Gabe Gaw,Cristobal Eyzaguirre,Tobias Gerstenberg*

Main category: cs.CV

TL;DR: 提出了Spot The Ball基准测试，用于评估视觉语言模型在视觉社交推理方面的能力，发现人类在通过视线、姿态等行为线索推断被移除的球的位置方面比现有模型准确2-3倍。


<details>
  <summary>Details</summary>
Motivation: 人类擅长通过微妙的行为线索进行视觉社交推理，这种能力对于开发更类人的AI代理至关重要。需要评估当前视觉语言模型在这方面的表现。

Method: 使用足球、篮球和排球图像构建基准测试集，要求定位被移除的球的位置。评估了四种最先进的视觉语言模型，并比较了人类基线表现。

Result: 人类准确率在20-34%之间，而所有模型的准确率均≤17%。模型依赖表面的空间启发式方法，而人类则利用视线方向和身体姿态等社交线索。

Conclusion: 在视觉社交推理方面存在显著的人类-模型差距，需要能够显式编码结构化行为线索的架构来实现稳健的类人推理。
...
### [37] [FedReplay: A Feature Replay Assisted Federated Transfer Learning Framework for Efficient and Privacy-Preserving Smart Agriculture](https://arxiv.org/abs/2511.00269)
*Long Li,Jiajia Li,Dong Chen,Lina Pu,Haibo Yao,Yanbo Huang*

Main category: cs.CV

TL;DR: 提出了一种结合CLIP视觉变换器和轻量级分类器的联邦学习框架，用于农业分类任务，在保护隐私的同时显著提升性能并降低通信成本。
...
Method: 使用预训练的CLIP ViT进行特征提取，仅对轻量级分类器进行联邦更新，并通过共享1%的CLIP特征表示来缓解非独立同分布数据问题。

Result: 在农业分类任务上达到86.6%的准确率，比基线联邦学习方法高出4倍以上。

Conclusion: 将视觉语言模型特征与联邦学习相结合，能够有效实现隐私保护且可扩展的农业智能系统。
...
### [38] [Multi-View Consistent Human Image Customization via In-Context Learning](https://arxiv.org/abs/2511.00293)
*Hengjia Li,Jianjin Xu,Keli Cheng,Lei Wang,Ning Bi,Boxi Wu,Fernando De la Torre,Deng Cai*

Main category: cs.CV

TL;DR: PersonalView是一种轻量级适配方法，仅需100个训练样本即可使现有模型获得多视角生成能力，通过条件架构设计和语义对应对齐损失实现多视角一致性和身份保持。
...
Conclusion: PersonalView证明了通过少量训练样本即可有效实现多视角生成能力，为个性化生成模型提供了高效的多视角控制解决方案。
...
### [39] [Towards Automated Petrography](https://arxiv.org/abs/2511.00328)
...
Method: 引入LITHOS数据集，包含偏振光图像和专家标注的矿物颗粒。提出双编码器transformer架构，整合两种偏振模式作为基线模型。

Result: 双编码器transformer架构在矿物分类中持续优于单偏振模型，证明了偏振协同在矿物分类中的价值。

Conclusion: LITHOS基准测试已公开可用，包含数据集、代码和预训练模型，旨在促进自动化岩石学分析的可重复性和进一步研究。
...
### [40] [Beyond ImageNet: Understanding Cross-Dataset Robustness of Lightweight Vision Models](https://arxiv.org/abs/2511.00335)
*Weidong Zhang,Pak Lun Kevin Ding,Huan Liu*

Main category: cs.CV

TL;DR: 该研究系统评估了11种轻量级视觉模型在7个不同数据集上的跨数据集泛化能力，提出了Cross-Dataset Score (xScore)指标来衡量模型鲁棒性，发现ImageNet性能不能可靠预测其他领域表现，并识别了促进泛化的关键架构组件。


<details>
  <summary>Details</summary>
Motivation: 当前轻量级视觉模型主要在ImageNet上评估，但缺乏对其跨数据集泛化能力的系统研究。需要量化模型在不同视觉领域的鲁棒性，并识别在资源受限条件下驱动泛化的架构要素。

Method: 在11种轻量级视觉模型（250万参数）上，采用固定的100轮训练计划，在7个多样化数据集上进行系统评估，引入xScore作为统一的跨数据集鲁棒性度量指标。

Result: 研究发现：(1) ImageNet准确率不能可靠预测细粒度或医疗数据集的性能；(2) xScore可作为移动模型性能的可扩展预测指标，仅需4个数据集即可估计；(3) 各向同性卷积、高空间分辨率和通道注意力等组件促进泛化，而Transformer模块带来额外参数开销但增益有限。

Conclusion: 该研究提供了在ImageNet之外评估轻量级视觉模型的可复现框架，突出了移动友好架构的关键设计原则，为开发跨应用领域鲁棒泛化的未来模型提供指导。
...
### [41] [A DeepONet joint Neural Tangent Kernel Hybrid Framework for Physics-Informed Inverse Source Problems and Robust Image Reconstruction](https://arxiv.org/abs/2511.00338)
*Yuhao Fang,Zijian Wang,Yao Lu,Ye Zhang,Chun Li*

Main category: cs.CV

TL;DR: 提出了一种结合DeepONet和NTK的混合方法，用于解决复杂逆问题，包括Navier-Stokes方程控制的源定位和图像重建，能够处理非线性、稀疏和噪声数据。


<details>
  <summary>Details</summary>
Motivation: 解决复杂逆问题中的非线性、稀疏性和噪声数据挑战，确保物理一致性和准确性。

Method: 将DeepONet与NTK结合，在损失函数中引入物理约束和任务特定正则化。
...
### [42] [Federated Dialogue-Semantic Diffusion for Emotion Recognition under Incomplete Modalities](https://arxiv.org/abs/2511.00344)
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中不可预测的模态缺失会显著降低现有多模态情感识别方法的性能，传统方法依赖完整多模态数据训练，在极端数据分布下容易产生语义失真

Method: 提出FedDISC框架，集成联邦学习到模态缺失恢复中，包括DISC-Diffusion模块确保恢复模态与可用模态在上下文、说话者身份和语义上的一致性，使用对话图网络捕捉对话依赖关系，语义条件网络强制语义对齐，并引入交替冻结聚合策略

Result: 在IEMOCAP、CMUMOSI和CMUMOSEI数据集上的实验表明，FedDISC在不同缺失模态模式下实现了优越的情感分类性能，优于现有方法

Conclusion: FedDISC通过联邦学习和语义一致的扩散模型有效解决了多模态情感识别中的模态缺失问题，提高了模型的鲁棒性和性能
...
### [43] [OSMGen: Highly Controllable Satellite Image Synthesis using OpenStreetMap Data](https://arxiv.org/abs/2511.00345)
...
<details>
  <summary>Details</summary>
Motivation: 准确和最新的地理空间数据对城市规划、基础设施监测和环境管理至关重要，但特定城市特征及其变化的策划数据集稀缺，使得自动化城市监测变得困难。

Method: OSMGen使用完整的OSM JSON数据，包括矢量几何、语义标签、位置和时间信息，提供对场景生成的细粒度控制，能够根据用户对OSM输入的编辑生成有针对性的视觉变化。

Result: 该框架能够生成一致的before-after图像对，使规划者能够通过编辑地图数据预览拟议的干预措施，并为训练数据稀缺和类别不平衡问题提供解决方案。
...
### [44] [Detecting AI-Generated Images via Diffusion Snap-Back Reconstruction: A Forensic Approach](https://arxiv.org/abs/2511.00352)
*Mohd Ruhul Ameen,Akif Islam*

Main category: cs.CV

TL;DR: 提出基于扩散重建动态的AI图像检测方法，通过分析不同噪声强度下的重建指标变化来区分真实与合成图像，在4000张图像数据集上达到0.993 AUROC。
...
Result: 在平衡的4000张图像数据集上，该方法在交叉验证下达到0.993 AUROC，对压缩和噪声等常见失真保持鲁棒性。
...
### [45] [Transfer Learning for Onboard Cloud Segmentation in Thermal Earth Observation: From Landsat to a CubeSat Constellation](https://arxiv.org/abs/2511.00357)
*Niklas Wölki,Lukas Kondmann,Christian Mollière,Martin Langer,Julia Gottfriedsen,Martin Werner*

Main category: cs.CV

TL;DR: 使用迁移学习和轻量级UNet模型，在FOREST-2立方星上实现热红外云分割，通过预训练和微调提升性能，并在NVIDIA Jetson Nano上实现快速推理。


<details>
  <summary>Details</summary>
Motivation: 解决立方星任务中硬件资源有限、热红外波段单一且标注数据不足的问题，实现准确高效的在轨云分割。

Method: 采用UNet架构配合轻量级MobileNet编码器，在Landsat-7数据集上预训练后，使用少量任务特定样本进行联合训练微调。

Result: 宏F1分数从0.850提升至0.877，模型转换为TensorRT引擎后在Jetson Nano上实现全图像推理时间小于5秒。

Conclusion: 利用公开数据集和轻量架构可在数据受限的地球观测任务中实现准确高效的热红外云分割，支持实时决策。
...
### [46] [Oitijjo-3D: Generative AI Framework for Rapid 3D Heritage Reconstruction from Street View Imagery](https://arxiv.org/abs/2511.00362)
...
<details>
  <summary>Details</summary>
Motivation: 孟加拉国文化遗产修复面临资源有限和技术专家稀缺的双重挑战，传统3D数字化方法需要昂贵硬件和专家操作，在发展中国家环境中往往不可行。

Method: 采用两阶段流程：使用Gemini 2.5 Flash Image进行多模态视觉推理实现结构-纹理合成，通过Hexagen进行神经图像到3D生成实现几何恢复。

Result: 系统在几秒钟内生成逼真、度量一致的3D重建，相比传统运动结构恢复流程显著加速，且无需专门硬件或专家监督。在Ahsan Manzil、Choto Sona Mosque和Paharpur等标志性建筑上的实验证明了其视觉和结构保真度。
...
### [47] [Who Can We Trust? Scope-Aware Video Moment Retrieval with Multi-Agent Conflict](https://arxiv.org/abs/2511.00370)
*Chaochen Wu,Guan Luo,Meiyun Zuo,Zhitao Fan*

Main category: cs.CV

TL;DR: 提出基于强化学习的视频时刻检索模型，通过多智能体系统和证据学习解决模型间定位冲突，无需额外训练即可检测无对应时刻的查询。


<details>
  <summary>Details</summary>
Motivation: 现有视频时刻检索方法未考虑不同模型定位结果间的冲突，导致模型无法有效集成。需要解决多模型集成冲突问题，并处理无对应时刻的查询场景。

Method: 使用强化学习模型扫描整个视频定位时刻边界并生成位置证据，提出多智能体系统框架，利用证据学习解决智能体间定位输出的冲突。

Result: 在基准数据集上的广泛实验表明，所提方法相比最先进方法具有有效性，能够检测无对应时刻的查询。

Conclusion: 建模多智能体系统的竞争和冲突是提高强化学习在时刻检索中性能的有效方法，揭示了证据学习在多智能体框架中的新作用。
...
### [48] [VisionCAD: An Integration-Free Radiology Copilot Framework](https://arxiv.org/abs/2511.00381)
*Jiaming Li,Junlei Wu,Sheng Wang,Honglin Xiong,Jiangdong Cai,Zihao Zhao,Yitao Zhu,Yuan Yin,Dinggang Shen,Qian Wang*

Main category: cs.CV

TL;DR: VisionCAD是一个基于视觉的放射辅助框架，通过摄像头直接从显示屏捕获医学图像，绕过了与医院IT系统集成的障碍，实现AI辅助诊断。


<details>
  <summary>Details</summary>
Motivation: 传统计算机辅助诊断系统难以与现有医院IT基础设施集成，限制了临床广泛应用。VisionCAD旨在通过视觉方法解决这一集成难题。

Method: 采用自动化流水线，包括检测、恢复和分析屏幕上医学图像，将摄像头捕获的视觉数据转换为适合自动分析和报告生成的诊断质量图像。

Result: 在多种医学影像数据集上验证，诊断性能与传统CAD系统相当，分类任务F1分数下降通常小于2%，自动报告的自然语言生成指标与原始图像相比差异在1%以内。

Conclusion: VisionCAD仅需摄像头设备和标准计算资源，为AI辅助诊断提供了可访问的方法，可在不修改现有基础设施的情况下在多样化临床环境中部署诊断能力。
...
TL;DR: 本文提出了FERBench基准测试，系统评估了20个最先进的多模态大语言模型在面部表情识别任务上的表现，并开发了UniFER-7B模型来提升模型的面部表情推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在各种任务上表现出色，但它们在面部表情识别任务上的性能尚未得到充分探索。现有方法将FER数据集转换为VQA格式，但模型的推理和可解释性仍存在显著局限。

Method: 首先构建FERBench基准测试评估20个SOTA MLLMs；然后创建两个大规模数据集UniFER-CoT-230K和UniFER-RLVR-360K；最后基于这些数据集开发UniFER-7B模型，使用冷启动初始化和带可验证奖励的强化学习。

Result: MLLMs在分类任务上表现良好，但在推理和可解释性方面存在显著局限。UniFER-7B模型超越了多个开源和闭源通用MLLMs，包括Gemini-2.5-Pro和Qwen2.5-VL-72B。

Conclusion: 通过系统基准测试和专门的数据集训练，可以显著提升MLLMs在面部表情识别任务上的推理能力和可解释性，UniFER-7B模型为FER任务提供了一个统一且可解释的基础模型解决方案。
...
### [50] [VinciCoder: Unifying Multimodal Code Generation via Coarse-to-fine Visual Reinforcement Learning](https://arxiv.org/abs/2511.00391)
...
Method: 采用两阶段训练框架：1) 构建包含160万图像-代码对的监督微调语料库；2) 引入视觉强化学习策略，通过粗到细的奖励机制计算局部和全局图像块的视觉相似度。

Result: 在各种多模态代码生成基准测试中实现了最先进的性能。

Conclusion: VinciCoder通过两阶段训练框架和粗到细的视觉强化学习策略，有效提升了多模态代码生成的性能，证明了该方法的有效性。
...
### [51] [CoT-Saliency: Unified Chain-of-Thought Reasoning for Heterogeneous Saliency Tasks](https://arxiv.org/abs/2511.00396)
*Long Li,Shuichen Ji,Ziyang Luo,Nian Liu,Dingwen Zhang,Junwei Han*

Main category: cs.CV

TL;DR: 提出了首个统一框架，通过将三个异构的显著性任务（SOD、CoSOD、SIS）建模为视觉语言模型中的思维链推理过程，解决了任务异质性问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常为每个显著性任务设计专门模型，缺乏统一处理异构任务的能力。本文旨在开发一个能够同时处理多个异构显著性任务的统一框架。

Method: 采用两阶段训练范式：监督微调（SFT）和强化学习（RL）。提出置信度引导策略优化（CGPO）算法，利用奖励与模型置信度之间的差异作为优势信号，解决了GRPO的关键限制。还引入了"输出到推理"策略构建高质量的SFT数据。

Result: 模型在所有任务上均匹配或超越了专门的SOTA方法和强大的闭源VLM，特别是在CoSOD任务上，在CoCA数据集上达到0.899的S-measure，比之前最佳方法提高了8.0个百分点，且使用了更少的训练数据。

Conclusion: 该框架成功地将异构显著性任务统一到一个模型中，通过CoT推理和创新的CGPO算法实现了卓越性能，证明了统一处理异构任务的可行性。
...
### [52] [LGCA: Enhancing Semantic Representation via Progressive Expansion](https://arxiv.org/abs/2511.00419)
*Thanh Hieu Cao,Trung Khang Tran,Gia Thinh Pham,Tuong Nghiem Diep,Thanh Binh Nguyen*

Main category: cs.CV

TL;DR: 提出LGCA框架解决CLIP模型在图像裁剪时引入错误信息的问题，通过局部特征捕获和显著区域扩展来同时捕捉局部和全局特征，提升零样本图像分类性能。


<details>
  <summary>Details</summary>
Motivation: CLIP等预训练视觉语言模型在零样本图像分类中表现良好，但随机图像裁剪会引入错误信息和偏见，因为小尺度下许多图像具有相似特征。

Method: 提出局部-全局交叉对齐(LGCA)框架：先捕获图像局部特征，然后重复选择最显著区域并扩展，相似度评分结合原始图像和扩展图像，同时捕捉局部和全局特征。

Result: 大量实验表明该方法显著提升了多种数据集的零样本性能，优于最先进的基线方法。

Conclusion: LGCA框架有效解决了图像裁剪中的错误信息问题，在保持时间复杂度的同时显著提升了零样本图像分类性能。
...
### [53] [Leveraging Hierarchical Image-Text Misalignment for Universal Fake Image Detection](https://arxiv.org/abs/2511.00427)
*Daichi Zhang,Tong Zhang,Jianmin Bao,Shiming Ge,Sabine Süsstrunk*

Main category: cs.CV

TL;DR: 提出ITEM方法，利用图像-文本不对齐作为检测生成假图像的线索，通过CLIP空间测量不对齐程度，结合全局和局部语义分析，实现更好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅关注视觉线索，容易过拟合特定图像模式，无法泛化到未见过的生成模型。发现假图像与对应描述无法正确对齐，这提供了多模态检测线索。
...
Conclusion: 利用图像-文本不对齐作为多模态线索是检测生成假图像的有效方法，能够克服现有方法的泛化限制。
...
### [54] [Enhancing Frequency Forgery Clues for Diffusion-Generated Image Detection](https://arxiv.org/abs/2511.00429)
*Daichi Zhang,Tong Zhang,Shiming Ge,Sabine Süsstrunk*

Main category: cs.CV

TL;DR: 提出一种基于频率伪造线索的扩散生成图像检测方法，通过增强全频段的频率差异特征来提升检测器的泛化性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型生成的图像质量很高，可能被恶意使用。现有检测器难以捕捉不同模型和设置下的判别性线索，泛化性和鲁棒性有限。

Method: 提出频率选择性函数作为加权滤波器，抑制判别性较弱的频段，增强信息丰富的频段，基于自然图像与扩散生成图像在频域上的差异进行分析。

Result: 在多个扩散生成图像数据集上的实验表明，该方法优于现有最先进的检测器，具有更好的泛化性和鲁棒性。

Conclusion: 通过增强全频段的频率伪造线索，能够有效检测来自未见扩散模型的图像，并对各种扰动具有鲁棒性。
...
### [55] [ToxicTextCLIP: Text-Based Poisoning and Backdoor Attacks on CLIP Pre-training](https://arxiv.org/abs/2511.00446)
*Xin Yao,Haiyang Zhao,Yimin Chen,Jiawei Guo,Kecheng Huang,Ming Zhao*

Main category: cs.CV

TL;DR: ToxicTextCLIP是一个针对CLIP模型预训练阶段的文本模态对抗攻击框架，通过背景感知选择和背景驱动增强生成高质量中毒文本，在分类和检索任务中达到高成功率并能绕过现有防御。


<details>
  <summary>Details</summary>
Motivation: CLIP模型依赖大规模网络数据进行自监督对比学习，但其文本模态的安全性研究不足。现有研究主要关注图像攻击，而文本模态同样面临数据中毒和后门风险。

Method: 提出ToxicTextCLIP框架，迭代应用：1）背景感知选择器，优先选择与目标类别背景一致文本；2）背景驱动增强器，生成语义连贯且多样化的中毒样本。

Result: 在分类和检索任务中达到95.83%的中毒成功率和98.68%的后门Hit@1，并能绕过RoCLIP、CleanCLIP和SafeCLIP等防御方法。

Conclusion: 文本模态在CLIP训练中同样存在安全风险，ToxicTextCLIP展示了针对文本模态的有效攻击方法，强调了多模态模型安全性的重要性。
...
### [56] [Weakly Supervised Pneumonia Localization from Chest X-Rays Using Deep Neural Network and Grad-CAM Explanations](https://arxiv.org/abs/2511.00456)
*Kiran Shahi,Anup Bagale*

Main category: cs.CV

TL;DR: 提出弱监督深度学习框架，使用Grad-CAM解释进行肺炎分类和定位，无需像素级标注，仅需图像级标签即可生成临床意义热图


<details>
  <summary>Details</summary>
Motivation: 解决医学影像中像素级标注成本高的问题，开发可解释的AI模型增强肺炎筛查透明度和临床信任

Method: 使用七种ImageNet预训练架构(ResNet-18/50、DenseNet-121、EfficientNet-B0、MobileNet-V2/V3、ViT-B16)，采用相同训练条件、焦点损失和患者级数据分割，利用Grad-CAM生成热图

Result: 在Kermany CXR数据集上，ResNet-18和EfficientNet-B0达到98%测试准确率、ROC-AUC=0.997、F1=0.987，MobileNet-V2在准确率和计算成本间达到最佳平衡

Conclusion: 弱监督可解释模型在肺炎筛查中具有潜力，Grad-CAM可视化确认模型聚焦临床相关肺部区域，支持可解释AI在放射诊断中的应用
...
### [57] [HumanCrafter: Synergizing Generalizable Human Reconstruction and Semantic 3D Segmentation](https://arxiv.org/abs/2511.00468)
*Panwang Pan,Tingting Shen,Chenxin Li,Yunlong Lin,Kairun Wen,Jingjing Zhao,Yixuan Yuan*

Main category: cs.CV

TL;DR: HumanCrafter是一个统一框架，能够从单张图像联合建模外观和人体部位语义，通过整合几何先验和自监督语义先验，在3D人体分割和重建任务中实现最先进性能。
...
Conclusion: 该框架成功实现了外观和语义的联合建模，为解决3D人体分析中的多任务挑战提供了有效解决方案。
...
### [58] [Longitudinal Vestibular Schwannoma Dataset with Consensus-based Human-in-the-loop Annotations](https://arxiv.org/abs/2511.00472)
...
TL;DR: 提出一种基于深度学习的迭代分割和质量优化框架，用于MRI中前庭神经鞘瘤的自动分割，通过多中心数据和专家共识构建标注数据集，显著提升了分割精度和效率。


<details>
  <summary>Details</summary>
Motivation: 前庭神经鞘瘤的MRI分割对患者管理至关重要，但手动标注耗时且依赖专家。现有深度学习方法在多样化数据集和复杂临床案例中的鲁棒性仍面临挑战。

Method: 采用引导式深度学习框架进行迭代分割和质量优化，结合多中心数据并依赖专家共识确保标注可信度，实现人类在环的模型训练方法。

Result: 在内部验证数据集上Dice相似系数从0.9125提升至0.9670，在外部数据集上保持稳定性能，专家评估143个扫描发现需要专家干预的细微案例，效率比传统手动标注提升约37.4%。

Conclusion: 该人类在环模型训练方法实现了高分割精度，展示了作为临床适应性强且可推广策略的潜力，数据集包含190名患者的534个纵向对比增强T1加权扫描，已在TCIA公开。
...
### [59] [FedMGP: Personalized Federated Learning with Multi-Group Text-Visual Prompts](https://arxiv.org/abs/2511.00480)
*Weihao Bo,Yanpeng Sun,Yu Wang,Xinyu Zhang,Zechao Li*

Main category: cs.CV

TL;DR: FedMGP是一种用于视觉语言模型的个性化联邦提示学习新范式，通过多组文本和视觉提示捕捉细粒度语义，采用基于相似性的动态提示聚合策略，在保持参数效率的同时实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中视觉语言模型的个性化需求，同时平衡共享知识和客户端特定特征，提高模型在多样化数据分布下的泛化能力。
...
Conclusion: FedMGP通过多组提示和动态聚合策略有效平衡了共享语义学习和客户端个性化需求，为联邦视觉语言学习提供了高效且强大的解决方案。
...
### [60] [Diff4Splat: Controllable 4D Scene Generation with Latent Dynamic Reconstruction Models](https://arxiv.org/abs/2511.00503)
...
TL;DR: Diff4Splat是一种前馈方法，可从单张图像合成可控的显式4D场景，无需测试时优化或后处理。


<details>
  <summary>Details</summary>
Motivation: 将视频扩散模型的生成先验与大规模4D数据集学习到的几何和运动约束相结合，实现高效可控的4D场景合成。
...
Conclusion: Diff4Splat提供了一种高效的单图像到4D场景合成方法，统一了生成先验和几何运动约束，实现了快速可控的动态场景生成。
...
### [61] [VinDr-CXR-VQA: A Visual Question Answering Dataset for Explainable Chest X-Ray Analysis with Multi-Task Learning](https://arxiv.org/abs/2511.00504)
*Hai-Dang Nguyen,Ha-Hieu Pham,Hao T. Nguyen,Huy-Hieu Pham*

Main category: cs.CV

TL;DR: VinDr-CXR-VQA是一个大规模胸部X射线视觉问答数据集，包含17,597个问答对和4,394张图像，带有放射科医生验证的边界框和临床推理解释。


<details>
  <summary>Details</summary>
Motivation: 推动可解释的医学视觉问答研究，提供具有空间定位和临床推理的大规模数据集，解决现有医学VQA数据集在可靠性和可解释性方面的不足。

Method: 构建包含六种诊断类型（位置、内容、存在性、数量、选择和是/非）的问题分类法，创建平衡的正负样本分布（41.7%阳性，58.3%阴性），使用MedGemma-4B-it模型进行基准测试。

Result: 在基准测试中达到F1分数0.624，比基线提高11.8%，同时实现了病变定位功能。

Conclusion: VinDr-CXR-VQA数据集推动了可重现和临床基础的医学视觉问答研究，数据集和评估工具已公开可用。
...
### [62] [OmniTrack++: Omnidirectional Multi-Object Tracking by Learning Large-FoV Trajectory Feedback](https://arxiv.org/abs/2511.00510)
*Kai Luo,Hao Shi,Kunyu Peng,Fei Teng,Sheng Wu,Kaiwei Wang,Kailun Yang*

Main category: cs.CV

TL;DR: OmniTrack++是一个用于全景图像多目标跟踪的反馈驱动框架，通过动态特征稳定、轨迹反馈关联和专家记忆系统解决全景图像特有的360°视野、分辨率稀释和视角相关失真等挑战，在JRDB和EmboTrack基准上实现了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统多目标跟踪方法在全景图像中表现不佳，因为全景图像具有360°视野、分辨率稀释和严重视角相关失真等独特挑战，需要专门设计的方法来处理这些复杂情况。

Method: 采用反馈驱动框架：1) DynamicSSM块稳定全景特征；2) FlexiTrack实例使用轨迹反馈进行灵活定位和短期关联；3) ExpertTrack记忆通过专家混合设计整合外观线索；4) Tracklet管理模块根据场景动态自适应切换跟踪模式。

Result: 在JRDB和EmboTrack基准测试中，OmniTrack++相比原始OmniTrack实现了显著改进：JRDB上HOTA提升+25.5%，QuadTrack上HOTA提升+43.07%，达到了最先进的性能水平。

Conclusion: OmniTrack++通过创新的反馈驱动框架有效解决了全景多目标跟踪的挑战，建立了EmboTrack基准数据集，为真实世界全景感知提供了具有挑战性的测试平台。
...
### [63] [ID-Composer: Multi-Subject Video Synthesis with Hierarchical Identity Preservation](https://arxiv.org/abs/2511.00511)
*Panwang Pan,Jingjing Zhao,Yuchen Lin,Chenguo Lin,Chenxin Li,Haopeng Li,Honglei Yan,Tingting Shen,Yadong Mu*

Main category: cs.CV

TL;DR: ID-Composer是一个新颖的多主体视频生成框架，通过文本提示和参考图像生成视频，解决了现有模型在可控性和应用性方面的限制。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型通常仅基于文本或单张图像进行条件生成，限制了可控性和应用范围。需要解决多主体视频生成中保持主体身份一致、跨模态语义整合和时间一致性的挑战。

Method: 设计了分层身份保持注意力机制来聚合主体内和跨模态特征；利用预训练视觉语言模型提供细粒度语义指导；采用在线强化学习阶段优化关键概念对齐。

Result: 大量实验表明，该模型在身份保持、时间一致性和视频质量方面优于现有方法。

Conclusion: ID-Composer通过创新的注意力机制、语义理解和强化学习策略，有效解决了多主体视频生成的挑战，显著提升了生成质量。
...
### [64] [SegDebias: Test-Time Bias Mitigation for ViT-Based CLIP via Segmentation](https://arxiv.org/abs/2511.00523)
*Fangyu Wu,Yujun Cai*

Main category: cs.CV

TL;DR: 提出了一种无需训练或偏见标注的测试时去偏方法，通过分割模型隔离目标视觉属性，调整非目标区域使其嵌入与所有类别文本提示均匀相似，从而消除混淆视觉区域的偏见信号。


<details>
  <summary>Details</summary>
Motivation: 现有去偏方法通常需要训练数据和明确的组标签进行微调或调整嵌入，限制了实际应用。测试时方法虽然避免这一约束，但许多仍依赖数据集特定偏见的先验知识，在开放集设置中泛化性有限。

Method: 使用预训练分割模型隔离目标视觉属性，调整非目标区域使其嵌入与所有类别特定文本提示均匀相似，保留目标属性同时消除非目标区域的偏见信号。

Result: 在Waterbirds和CelebA数据集上的实验表明，该方法在组鲁棒性指标和Attention IoU方面优于现有测试时去偏方法。

Conclusion: 分割引导的干预方法在视觉语言模型中实现了可扩展且无需标注的偏见缓解，证明了其有效性。
...
### [65] [Text-guided Fine-Grained Video Anomaly Detection](https://arxiv.org/abs/2511.00524)
*Jihao Gu,Kun Li,He Wang,Kaan Akşit*

Main category: cs.CV

TL;DR: 提出了T-VAD框架，基于大型视觉语言模型进行细粒度视频异常检测，通过异常热图解码器和区域感知异常编码器实现像素级异常定位和文本描述。


<details>
  <summary>Details</summary>
Motivation: 传统视频异常检测方法输出有限（仅正常/异常），需要人工评估，缺乏细粒度和交互性。

Method: 使用大型视觉语言模型，引入异常热图解码器进行像素级视觉-文本特征对齐，以及区域感知异常编码器将热图转换为可学习文本嵌入。

Result: 在UBnormal数据集上达到94.8% AUC，异常热图准确率67.8%/76.7%；在ShanghaiTech数据集上BLEU-4得分62.67/88.84，Yes/No准确率97.67%。

Conclusion: T-VAD显著提升了异常检测的粒度和交互性，在多个数据集上达到SOTA性能。
...
### [66] [Real-IAD Variety: Pushing Industrial Anomaly Detection Dataset to a Modern Era](https://arxiv.org/abs/2511.00540)
...
<details>
  <summary>Details</summary>
Motivation: 现有工业异常检测算法受限于公共基准数据集，存在类别多样性不足、规模有限的问题，导致指标饱和和模型在真实场景中泛化能力差。

Method: 构建了大规模多样化的IAD基准数据集，包含多类别、多行业、多材料和多颜色的图像，并设计了严格的评估协议，包括多类别无监督、多视角和零/少样本设置。

Result: 实验表明，最先进的多类别无监督异常检测方法在类别从30扩展到160时性能显著下降，而视觉语言模型表现出对类别扩展的强鲁棒性，性能变化最小。
...
### [67] [MIFO: Learning and Synthesizing Multi-Instance from One Image](https://arxiv.org/abs/2511.00542)
...
Method: 使用惩罚性注意力优化在学习阶段分离相似语义，在合成阶段引入并优化注意力层的框控制来减少语义泄漏并精确控制输出布局

Result: 实验结果表明该方法实现了分离且高质量的语义学习和合成，在可编辑性和实例一致性之间取得了良好平衡，在处理语义或视觉相似实例或罕见对象时保持鲁棒性

Conclusion: 该方法能够有效解决多实例语义学习和合成中的挑战，特别是在处理相似语义实例时表现出色
...
### [68] [4D Neural Voxel Splatting: Dynamic Scene Rendering with Voxelized Guassian Splatting](https://arxiv.org/abs/2511.00560)
*Chun-Tin Wu,Jun-Cheng Chen*

Main category: cs.CV

TL;DR: 提出4D神经体素溅射方法，通过神经体素和变形场建模动态场景，大幅减少内存消耗并加速训练，同时保持高质量渲染。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在动态场景中需要为每帧复制高斯分布，导致内存开销巨大。

Method: 使用紧凑的神经体素集合和学习的变形场来建模时间动态，避免为每个时间戳生成单独的高斯集。

Result: 实验表明该方法在显著减少内存和加速训练的同时，优于现有方法，实现实时渲染和优越的视觉保真度。

Conclusion: 4D-NVS通过神经体素和变形场的结合，为动态场景建模提供了高效且高质量的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有广义类别发现方法在标准条件下表现良好，但在存在分布偏移时性能会显著下降，需要解决更现实的域偏移场景

Method: 使用频域引导的域分离策略、跨域和域内频域扰动策略、扩展的自监督对比目标和语义聚类损失、聚类难度感知重采样技术

Result: 在多个基准数据集上有效缓解了分布偏移的影响，在发现已知和未知类别方面取得了优越性能

Conclusion: FREE框架通过频域信息成功解决了域偏移广义类别发现任务，显著提升了模型在分布偏移条件下的表现
...
### [70] [TRACES: Temporal Recall with Contextual Embeddings for Real-Time Video Anomaly Detection](https://arxiv.org/abs/2511.00580)
*Yousuf Ahmed Siddiqui,Sufiyaan Usmani,Umer Tariq,Jawwad Ahmed Shamsi,Muhammad Burhan Khan*

Main category: cs.CV

TL;DR: 提出了一种上下文感知的零样本异常检测方法，通过融合时间特征和视觉嵌入，结合上下文记忆实现实时异常检测，在UCF-Crime和XD-Violence数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 视频异常通常依赖于上下文信息和时间演化，但大多数异常检测器未能注意到这种上下文依赖，限制了它们在实际场景中的泛化能力。

Method: 采用记忆增强管道，通过交叉注意力关联时间信号与视觉嵌入，并使用上下文相似性评分进行实时零样本异常分类。

Result: 在UCF-Crime上达到90.4% AUC，在XD-Violence上达到83.67% AP，实现了实时推理和高精度检测。

Conclusion: 通过融合交叉注意力时间融合和上下文记忆，实现了高保真度异常检测，为零样本模型在现实世界监控应用迈出了重要一步。
...
### [71] [CueBench: Advancing Unified Understanding of Context-Aware Video Anomalies in Real-World](https://arxiv.org/abs/2511.00613)
*Yating Yu,Congqi Cao,Zhaoying Wang,Weihua Meng,Jie Li,Yuxin Li,Zihao Wei,Zhongpei Shen,Jiajun Zhang*

Main category: cs.CV

TL;DR: CueBench是首个专注于上下文感知视频异常理解的基准测试，通过统一评估框架对14种条件异常和18种绝对异常事件进行多任务评估，现有视觉语言模型表现不佳，而作者提出的Cue-R1方法平均提升超过24%。


<details>
  <summary>Details</summary>
Motivation: 现有视频异常理解方法对真实世界异常的理解较为肤浅，缺乏对复杂原理和细微上下文（如带安全装备与不带安全装备攀岩的区别）的深入理解。

Method: 提出CueBench基准测试，建立事件中心的分层分类法，涵盖174个场景和198个属性；开发Cue-R1方法，基于R1风格强化微调，使用可验证、任务对齐和层次细化的奖励进行统一生成式训练。

Result: 在CueBench上的广泛实验表明，现有视觉语言模型对真实世界异常理解仍不令人满意，而Cue-R1方法比最先进方法平均提升超过24%。

Conclusion: 真实世界视频异常理解仍面临挑战，CueBench为评估提供了严格框架，Cue-R1展示了在上下文感知异常理解方面的显著改进。
...
### [72] [Grounding Surgical Action Triplets with Instrument Instance Segmentation: A Dataset and Target-Aware Fusion Approach](https://arxiv.org/abs/2511.00643)
*Oluwatosin Alabi,Meng Wei,Charlie Budd,Tom Vercauteren,Miaojing Shi*

Main category: cs.CV

TL;DR: 提出了一种新的手术动作三元组分割任务，通过仪器实例分割将<仪器、动词、目标>动作三元组在手术场景中进行空间定位，并开发了TargetFusionNet架构来融合弱解剖先验知识。


<details>
  <summary>Details</summary>
Motivation: 现有手术动作识别方法仅限于帧级分类，无法可靠地将动作与特定仪器实例关联；之前基于类激活图的空间定位方法缺乏精确性和鲁棒性，无法满足详细仪器-组织交互分析的需求。

Method: 提出了三元组分割任务，创建了CholecTriplet-Seg数据集（包含30,000+标注帧），并开发了TargetFusionNet架构，该架构扩展Mask2Former，通过目标感知融合机制将弱解剖先验与仪器实例查询融合。

Result: TargetFusionNet在识别、检测和三元组分割指标上均优于现有基线方法，表明强实例监督结合弱目标先验能显著提高手术动作理解的准确性和鲁棒性。
...
### [73] [Benchmarking individual tree segmentation using multispectral airborne laser scanning data: the FGI-EMIT dataset](https://arxiv.org/abs/2511.00653)
*Lassi Ruoppa,Tarmo Hietala,Verneri Seppänen,Josef Taher,Teemu Hakala,Xiaowei Yu,Antero Kukko,Harri Kaartinen,Juha Hyyppä*

Main category: cs.CV

TL;DR: 本文介绍了首个大规模多光谱激光雷达数据集FGI-EMIT，用于个体树木分割，并比较了传统无监督算法和深度学习方法的性能。深度学习模型显著优于传统方法，特别是在下层植被分割方面。


<details>
  <summary>Details</summary>
Motivation: 缺乏大规模多光谱激光雷达基准数据集阻碍了个体树木分割方法的发展，尽管有证据表明多光谱反射率可以提高分割精度。

Method: 创建了包含1,561棵手动标注树木的FGI-EMIT数据集，使用贝叶斯方法优化无监督算法参数，从头训练深度学习模型，比较了4种传统算法和4种深度学习方法。

Result: 无监督方法中Treeiso表现最佳，F1分数为52.7%；深度学习模型ForestFormer3D达到73.3%的F1分数，在下层植被分割中比Treeiso高出25.9个百分点。

Conclusion: 深度学习方法在个体树木分割中显著优于传统无监督算法，但当前方法未能充分利用多光谱反射率信息，单通道反射率可略微提高精度，特别是在下层植被分割中。
...
### [74] [Metadata-Aligned 3D MRI Representations for Contrast Understanding and Quality Control](https://arxiv.org/abs/2511.00681)
*Mehmet Yigit Avci,Pedro Borges,Virginia Fernandez,Paul Wright,Mehmet Yigitsoy,Sebastien Ourselin,Jorge Cardoso*

Main category: cs.CV

TL;DR: MR-CLIP是一个通过将MRI体积图像与其DICOM采集参数对齐来学习MRI对比度表示的元数据引导框架，无需人工标注即可实现序列识别、协调和质量控制。


<details>
  <summary>Details</summary>
Motivation: MRI存在严重的数据异质性和缺乏标准化对比度标签的问题，限制了大规模自动化分析，需要统一的MRI对比度表示。

Method: 引入MR-CLIP框架，通过将体积图像与DICOM采集参数对齐来学习MRI对比度表示。

Result: 生成的嵌入显示MRI序列的明显聚类，在数据稀缺情况下，少样本序列分类性能优于监督3D基线方法，并能通过图像-元数据嵌入距离识别损坏或不一致的元数据。
...
### [75] [Outlier-Aware Post-Training Quantization for Image Super-Resolution](https://arxiv.org/abs/2511.00682)
...
<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法在图像超分辨率任务中性能不佳，主要原因是忽略了激活值中的异常值影响。研究发现这些异常值与图像颜色信息强相关，直接移除会导致性能显著下降。
...
Conclusion: 该方法有效解决了图像超分辨率网络中激活值异常值对量化的影响问题，提供了一种高效的后训练量化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型虽然能生成高质量图像，但输出多样性有限，限制了在探索性和构思任务中的应用。现有的提示优化技术通常针对审美适应性或不适合创意视觉领域。

Method: WANDER直接在自然语言提示上操作，使用大型语言模型(LLM)进行语义演化，利用CLIP嵌入量化新颖性，并应用发射器引导搜索进入提示空间的不同区域。
...
TL;DR: 该论文分析了低剂量CT图像增强中不同损失函数与图像质量指标之间的一致性，发现两者存在不一致性，强调了在开发新损失函数时需要考虑图像质量指标的重要性。


<details>
  <summary>Details</summary>
Motivation: 低剂量CT成像虽然能减少辐射暴露，但存在噪声和伪影问题，影响诊断准确性。虽然深度学习模型在PSNR和SSIM指标上表现优异，但这些指标在反映医学图像感知质量方面存在局限性。
...
Conclusion: 在开发用于图像质量增强的新损失函数时，必须充分考虑图像质量指标，以确保增强后的图像既在数值指标上表现良好，又能满足临床诊断的感知质量要求。
...
### [78] [Validating Deep Models for Alzheimer's 18F-FDG PET Diagnosis Across Populations: A Study with Latin American Data](https://arxiv.org/abs/2511.00728)
*Hugo Massaroli,Hernan Chaves,Pilar Anania,Mauricio Farez,Emmanuel Iarussi,Viviana Siless*

Main category: cs.CV

TL;DR: 深度学习模型在ADNI数据集上对阿尔茨海默病诊断表现优异，但在拉丁美洲FLENI队列上性能显著下降，揭示了显著的领域偏移问题。


<details>
  <summary>Details</summary>
Motivation: 评估深度学习模型在北美ADNI数据集上训练的阿尔茨海默病诊断模型对代表性不足人群的泛化能力。

Method: 在ADNI数据集上训练卷积和Transformer模型，并在阿根廷FLENI临床队列上测试泛化性能，进行消融研究和遮挡敏感性分析。

Result: 所有模型在ADNI上AUC高达0.96-0.97，但在FLENI上降至0.80-0.82，显示显著性能下降。不同架构表现相似，归一化和采样选择是关键泛化因素。

Conclusion: 需要人口感知的AI模型验证，未来应关注领域适应和队列多样化。
...
### [79] [Towards classification-based representation learning for place recognition on LiDAR scans](https://arxiv.org/abs/2511.00738)
*Dmitrii Khizbullin,Maksim Konoplia*

Main category: cs.CV

TL;DR: 将地点识别重新定义为多类分类问题，通过为LiDAR扫描分配离散位置标签来训练编码器-解码器模型，在NuScenes数据集上取得与对比学习方法相当的性能，同时提升训练效率和稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有地点识别方法大多依赖对比学习，本文探索将地点识别作为多类分类问题的替代方法，以提升训练效率和稳定性。

Method: 为LiDAR扫描分配离散位置标签，训练编码器-解码器模型直接分类每个扫描的位置。

Result: 在NuScenes数据集上评估，该方法实现了与基于对比学习方法竞争的性能。
...
### [80] [Erasing 'Ugly' from the Internet: Propagation of the Beauty Myth in Text-Image Models](https://arxiv.org/abs/2511.00749)
*Tanvi Dinkar,Aiqi Jiang,Gavin Abercrombie,Ioannis Konstas*

Main category: cs.CV

TL;DR: 本研究通过生成式AI模型分析西方审美标准的编码方式，发现模型存在明显的肤色、年龄和性别偏见，86.5%生成图像为浅肤色，74%为年轻人群，非二元性别个体被过度年轻化和性化。


<details>
  <summary>Details</summary>
Motivation: 社交媒体加剧了西方审美标准的传播，导致负面自我形象和身体畸形恐惧。随着AI生成内容的增加，担忧这些标准被夸大，需要研究生成式AI如何编码'美'并消除'丑'。

Method: 创建两个图像生成流程：文本到图像模型和文本到语言模型再到图像模型。开发结构化审美分类法，使用三个语言模型和两个文本到图像模型生成5984张图像，并招募社交媒体用户通过李克特量表评估1200张图像。

Result: 86.5%生成图像为浅肤色，22%包含明确内容（尽管经过SFW训练），74%被评分为较年轻年龄段。非二元性别个体图像被评分为更年轻和更过度性化。带有'负面'或'丑陋'特征的提示词产生更高的NSFW评分。

Conclusion: 生成式AI模型存在与审美标准相关的普遍人口统计偏见，这些偏见通过模型开发者（如负面提示）被积极延续，可能导致数据流污染和对不符合开发者美丽刻板印象特征的主动消除。
...
### [81] [A Hybrid YOLOv5-SSD IoT-Based Animal Detection System for Durian Plantation Protection](https://arxiv.org/abs/2511.00777)
*Anis Suttan Shahrir,Zakiah Ayop,Syarulnaziah Anawar,Norulzahrah Mohd Zainudin*

Main category: cs.CV

TL;DR: 提出了一种结合YOLOv5和SSD算法的物联网动物检测系统，用于榴莲种植园，通过Telegram实时通知和声音威慑机制保护作物。


<details>
  <summary>Details</summary>
Motivation: 传统农业实践因缺乏无人监控而无法有效防止动物入侵造成的作物损害和经济损失，现有系统存在单一检测算法依赖、通知平台不便和威慑机制有限等问题。

Method: 集成YOLOv5和SSD物体检测算法提高检测精度，提供实时监控，通过Telegram自动通知农民，并触发自动化声音威慑机制（如虎啸）。
...
### [82] [Class-agnostic 3D Segmentation by Granularity-Consistent Automatic 2D Mask Tracking](https://arxiv.org/abs/2511.00785)
*Juan Wang,Yasutomo Kawanishi,Tomo Miyazaki,Zhijie Wang,Shinichiro Omachi*

Main category: cs.CV

TL;DR: 提出了一种粒度一致的自动2D掩码跟踪方法，通过保持跨帧的时间对应关系来消除冲突的3D伪标签，结合三阶段课程学习框架，从碎片化单视图数据逐步训练到统一的多视图标注。
...
Method: 引入粒度一致的自动2D掩码跟踪方法，结合三阶段课程学习框架：从碎片化单视图数据到统一多视图标注，再到全局一致的全场景监督。

Result: 实验结果表明，该方法有效生成了一致且准确的3D分割，在标准基准测试中达到了最先进的性能，并具备开放词汇能力。

Conclusion: 该方法能够从初始碎片化和矛盾的2D先验中稳健地提取一致的3D表示，实现了高质量的无监督3D实例分割。
...
### [83] [FedOnco-Bench: A Reproducible Benchmark for Privacy-Aware Federated Tumor Segmentation with Synthetic CT Data](https://arxiv.org/abs/2511.00795)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习系统在隐私敏感环境中具有重要价值，但仍然面临成员推理攻击和数据异构性的安全漏洞，需要建立标准化评估平台。
...
### [84] [GUI-AIMA: Aligning Intrinsic Multimodal Attention with a Context Anchor for GUI Grounding](https://arxiv.org/abs/2511.00810)
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于MLLMs的GUI定位方法将任务视为基于文本的坐标生成，但直接从视觉输入生成精确坐标具有挑战性且计算量大。直觉方法是先选择与指令相关的视觉块，然后在块内确定精确点击位置。

Method: GUI-AIMA将MLLMs的内在多模态注意力与块级定位信号对齐，通过多头部聚合在简化的查询-视觉注意力矩阵上自适应计算这些信号。其坐标无关方式可以轻松集成即插即用的放大阶段。

Result: GUI-AIMA-3B仅用85k张截图训练，在ScreenSpot-Pro上达到58.6%的平均准确率，在OSWorld-G上达到62.2%的平均准确率，在3B模型中实现了最先进的性能。
...
### [85] [TA-LSDiff:Topology-Aware Diffusion Guided by a Level Set Energy for Pancreas Segmentation](https://arxiv.org/abs/2511.00815)
...
<details>
  <summary>Details</summary>
Motivation: 胰腺分割面临尺寸小、对比度低和拓扑变化大的挑战。传统水平集方法忽略点状拓扑效应，而深度学习网络会牺牲结构细节。

Method: 结合拓扑感知扩散概率模型和水平集能量，通过四个互补项整合输入图像和深度特征指导隐式曲线演化，并引入像素自适应细化模块增强边界精度。
...
### [86] [OMEGA: Optimized Multimodal Position Encoding Index Derivation with Global Adaptive Scaling for Vision-Language Models](https://arxiv.org/abs/2511.00821)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型使用统一的位置索引策略，没有考虑文本和视觉模态在结构和连续性上的差异，限制了模型性能。

Method: 提出模态特定位置编码(MSPE)为不同模态分配位置索引，保持各自结构特性；引入全局自适应编码步长缩放(GAESS)根据嵌入熵自适应调整视觉token的位置编码步长。

Result: OMEGA在各种架构和VQA基准测试中持续提升VLM性能，在视觉密集型任务上，Qwen2.5-VL-3B相比基线位置编码策略提升达3.43%，在更大模型上也观察到一致增益。

Conclusion: OMEGA通过考虑模态特定结构和信息密度对齐，有效提升了视觉语言模型的性能表现。
...
### [87] [Enhancing Adversarial Transferability in Visual-Language Pre-training Models via Local Shuffle and Sample-based Attack](https://arxiv.org/abs/2511.00831)
*Xin Liu,Aoyang Zhou,Aoyang Zhou*

Main category: cs.CV

TL;DR: 提出了一种名为LSSA的新型多模态对抗攻击方法，通过局部图像块随机打乱和采样来增强对抗样本的迁移性，解决了现有方法因缺乏输入多样性而导致的过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态对抗攻击方法过度依赖单一模态的对抗样本信息，缺乏输入多样性，导致过拟合问题，限制了对抗样本在不同模型间的迁移能力。

Method: LSSA方法随机打乱图像的局部块来扩展原始图像-文本对，生成对抗图像并在其周围采样，然后利用原始图像和采样图像来生成对抗文本。

Result: 在多个模型和数据集上的实验表明，LSSA显著提升了多模态对抗样本在不同VLP模型和下游任务间的迁移性，并在大型视觉语言模型上优于其他先进攻击方法。

Conclusion: LSSA通过引入输入多样性有效解决了多模态对抗攻击中的过拟合问题，显著提升了对抗样本的迁移性能。
...
### [88] [Linear Differential Vision Transformer: Learning Visual Contrasts via Pairwise Differentials](https://arxiv.org/abs/2511.00833)
*Yifan Pu,Jixuan Ying,Qixiu Li,Tianzhu Ye,Dongchen Han,Xiaochen Wang,Ziyi Wang,Xinyu Shao,Gao Huang,Xiu Li*

Main category: cs.CV

TL;DR: 提出Visual-Contrast Attention (VCA)作为MHSA的替代方案，通过视觉对比机制降低计算复杂度，在图像识别和生成任务中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 传统Vision Transformers的MHSA层对所有token对进行二次方查询-键交互，消耗大量计算资源在视觉上弱或冗余的相关性上，需要更高效的注意力机制

Method: VCA首先将每个头的密集查询场蒸馏为少量空间池化的视觉对比token，然后将其分为可学习的正负流，通过差分交互突出区域间的真正差异

Result: 在DeiT-Tiny上将ImageNet-1K准确率从72.2%提升至75.6%（+3.4%），在层次化ViTs上提升达3.1%，在图像生成任务中FID-50K降低2.1-5.2点

Conclusion: VCA为构建更快更锐利的Vision Transformers提供了一条简单路径，通过空间池化提供低方差全局线索，双位置嵌入对对比推理不可或缺
...
### [89] [Parameter Interpolation Adversarial Training for Robust Image Classification](https://arxiv.org/abs/2511.00836)
*Xin Liu,Yichen Yang,Kun He,John E. Hopcroft*

Main category: cs.CV

TL;DR: 提出参数插值对抗训练(PIAT)框架，通过插值前后epoch的模型参数来缓解对抗训练中的振荡和过拟合问题，并使用归一化均方误差(NMSE)进一步提升鲁棒性。
...
Method: PIAT框架在每轮epoch间通过插值前后epoch的参数来调节模型参数，使决策边界变化更平缓；同时使用归一化均方误差(NMSE)对齐干净样本和对抗样本的logits相对大小。

Result: 在多个基准数据集上的实验表明，该框架能显著提升卷积神经网络(CNNs)和视觉变换器(ViTs)的鲁棒性。

Conclusion: PIAT通过参数插值和NMSE损失函数有效缓解了对抗训练中的振荡和过拟合问题，实现了更高的模型鲁棒性。
...
### [90] [OmniBrainBench: A Comprehensive Multimodal Benchmark for Brain Imaging Analysis Across Multi-stage Clinical Tasks](https://arxiv.org/abs/2511.00846)
*Zhihao Peng,Cheng Wang,Shengyuan Liu,Zhiying Liang,Yixuan Yuan*

Main category: cs.CV

TL;DR: OmniBrainBench是首个专门用于评估多模态大语言模型在脑成像分析中多模态理解能力的综合基准，包含15种脑成像模态、9,527个验证问答对和31,706张图像，模拟临床工作流程。
...
Method: 构建包含15种脑成像模态的综合多模态VQA基准，模拟临床工作流程，涵盖15个多阶段临床任务，由专业放射科医生验证。

Result: 评估24个最先进模型发现：专有MLLMs优于开源和医疗模型但仍落后于医生；医疗MLLMs性能差异大；开源MLLMs整体落后但在特定任务表现出色；MLLMs在复杂术前任务中表现不佳，存在视觉到临床推理的差距。

Conclusion: OmniBrainBench为脑成像分析中MLLMs的评估和推进设立了新标准，揭示了与专家临床推理的差距。
...
### [91] [Occlusion-Aware Diffusion Model for Pedestrian Intention Prediction](https://arxiv.org/abs/2511.00858)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在预测行人过街意图方面取得了显著成功，但很少考虑遮挡场景下的不完全观察问题。
...
### [92] [Layer-Wise Modality Decomposition for Interpretable Multimodal Sensor Fusion](https://arxiv.org/abs/2511.00859)
*Jaehyun Park,Konyul Park,Daehun Kim,Junseo Park,Jun Won Choi*

Main category: cs.CV

TL;DR: 提出了LMD方法，用于解耦自动驾驶感知模型中多模态输入的贡献度，实现模型决策的透明化解释


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中感知模型的决策透明度至关重要，但多传感器融合使得难以确定各模态对预测的具体贡献

Method: LMD是一种后处理、模型无关的可解释性方法，在预训练融合模型的所有层中解耦模态特定信息

Result: 在相机-雷达、相机-LiDAR和相机-雷达-LiDAR融合模型上验证有效，通过结构化扰动指标和视觉分解展示实用性

Conclusion: LMD是首个能够将感知模型预测归因于单个输入模态的方法，适用于解释高容量多模态架构
...
### [93] [GraphGeo: Multi-Agent Debate Framework for Visual Geo-localization with Heterogeneous Graph Neural Networks](https://arxiv.org/abs/2511.00908)
...
Method: 提出异构图神经网络框架，通过类型化边区分支持性协作、竞争性论证和知识转移，采用双级辩论机制结合节点级细化和边级论证建模，实现图结构与智能体表示的协同进化。

Result: 在多个基准测试中，GraphGeo显著优于现有最先进方法，将智能体间的认知冲突转化为增强的地理定位精度。

Conclusion: GraphGeo通过结构化辩论框架有效提升了视觉地理定位性能，证明了异构图神经网络在多智能体协作中的有效性。
...
### [94] [Fleming-VL: Towards Universal Medical Visual Reasoning with Multimodal LLMs](https://arxiv.org/abs/2511.00916)
*Yan Shu,Chi Liu,Robin Chen,Derek Li,Bryan Dai*

Main category: cs.CV

TL;DR: Fleming-VL是一个统一的端到端框架，用于处理异构医学多模态数据（2D图像、3D体积扫描、时序视频），通过数据中心的策略实现跨模态医学视觉理解。


<details>
  <summary>Details</summary>
Motivation: 医学数据具有异构性（2D图像、3D体积扫描、时序视频），存在显著的领域差距和数据格式不一致问题，阻碍了统一医学MLLMs的发展。

Method: 采用三个关键策略：(1)整合自然和医学领域的长上下文数据进行预训练扩展；(2)用罕见医学数据补充微调，包括整体视频分析和代表性不足的2D模态；(3)扩展评估框架以包含3D体积和视频理解基准。通过SFT和GRPO开发多规模模型。

Result: Fleming-VL在多个基准测试中（医学VQA、视频QA、3D医学图像理解）实现了最先进的性能。

Conclusion: Fleming-VL为医学AI的透明、可复现和可审计进展提供了统一解决方案，并公开发布以促进该领域发展。
...
### [95] [Dynamic Multi-level Weighted Alignment Network for Zero-shot Sketch-based Image Retrieval](https://arxiv.org/abs/2511.00925)
*Hanwen Su,Ge Song,Jiyan Wang,Yuanbo Zhu*

Main category: cs.CV

TL;DR: 提出动态多级加权对齐网络解决零样本草图-图像检索问题，通过多级权重对齐和加权四元组损失提升性能
...
Conclusion: 所提出的动态多级加权对齐网络在零样本草图-图像检索任务中表现出优越性能
...
### [96] [EVTAR: End-to-End Try on with Additional Unpaired Visual Reference](https://arxiv.org/abs/2511.00956)
*Liuzhuozheng Li,Yue Gong,Shanyuan Liu,Bo Cheng,Yuhang Ma,Liebucha Wu,Dengyang Jiang,Zanyi Wang,Dawei Leng,Yuhui Yin*

Main category: cs.CV

TL;DR: EVTAR是一个端到端的虚拟试穿模型，通过引入额外参考图像直接拟合目标服装到人物图像上，无需复杂输入如掩码、密集姿态或分割图，提高了试穿准确性和实用性。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿方法依赖复杂输入（如不可知人物图像、人体姿态、密集姿态或关键点），使得应用过程繁琐且不实用。EVTAR旨在简化输入要求，仅需源图像和目标服装，同时通过参考图像提升试穿质量。

Method: 采用两阶段训练策略，在推理时仅需源图像和目标服装。利用不同人穿着相同服装的参考图像来保留服装纹理和细节，模拟人类选择服装时的参考行为。通过补充参考图像和未配对人物图像丰富训练数据。

Result: 在两个广泛使用的基准测试和多样化任务上评估EVTAR，结果一致验证了该方法的有效性。

Conclusion: EVTAR通过简化输入要求和引入参考图像机制，实现了更实用和高质量的虚拟试穿效果，为实际应用提供了可行解决方案。
...
### [97] [A Unified Reasoning Framework for Holistic Zero-Shot Video Anomaly Analysis](https://arxiv.org/abs/2511.00962)
*Dongheng Lin,Mengxue Qu,Kunyang Han,Jianbo Jiao,Xiaojie Jin,Yunchao Wei*

Main category: cs.CV

TL;DR: 提出统一推理框架，通过链式测试时推理连接时间检测、空间定位和文本解释，实现零样本视频异常分析。


<details>
  <summary>Details</summary>
Motivation: 现有视频异常研究多停留在帧级检测，缺乏空间和语义上下文，且现有方法依赖数据和特定任务。

Method: 基于链式测试时推理过程，通过任务内推理优化时间检测，任务间链式连接实现空间和语义理解，无需额外训练。

Result: 在多个视频异常检测、定位和解释基准上实现最先进的零样本性能，无需额外数据或梯度更新。
...
### [98] [VesSAM: Efficient Multi-Prompting for Segmenting Complex Vessel](https://arxiv.org/abs/2511.00981)
...
Method: VesSAM框架包含：(1)卷积适配器增强局部纹理特征；(2)多提示编码器融合解剖学提示；(3)轻量级掩码解码器减少锯齿伪影；并引入自动化多提示标注流程。

Result: 在8个数据集5种成像模态上，VesSAM比最先进的PEFT-based SAM变体提升超过10% Dice和13% IoU，参数量显著减少，且在分布外设置中表现最佳。

Conclusion: VesSAM为血管分割提供了强大而高效的解决方案，在性能、泛化能力和参数效率方面均优于现有方法。
...
### [99] [MID: A Self-supervised Multimodal Iterative Denoising Framework](https://arxiv.org/abs/2511.00997)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界数据常被复杂非线性噪声污染，传统基于规则的去噪方法难以应对，需要无需配对数据的有效去噪解决方案。

Method: MID将噪声数据建模为非线性噪声累积过程中的状态，通过迭代添加噪声学习两个神经网络：一个估计当前噪声步长，另一个预测并减去相应噪声增量。对于复杂非线性污染，使用一阶泰勒展开局部线性化噪声过程。

Result: 在四个经典计算机视觉任务中展示了鲁棒性、适应性和最先进性能，在生物医学和生物信息学领域也表现出强大性能。
...
### [100] [Integrating Visual and X-Ray Machine Learning Features in the Study of Paintings by Goya](https://arxiv.org/abs/2511.01000)
*Hassan Ugail,Ismail Lujain Jaleel*

Main category: cs.CV

TL;DR: 提出了一种用于戈雅画作认证的多模态机器学习框架，通过统一特征提取技术处理视觉和X射线图像，在24幅认证画作上达到97.8%的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 戈雅作品的艺术认证面临复杂挑战，包括其异质风格演变和广泛的历史伪造模式，需要更可靠的多模态分析方法。

Method: 使用统一特征提取管道（包括灰度共生矩阵、局部二值模式、熵测量、能量计算和颜色分布分析）处理视觉和X射线图像，通过优化的单类支持向量机进行分类。

Result: 在24幅认证戈雅画作数据集上，采用80/20训练测试分割和10折交叉验证，达到97.8%分类准确率和0.022假阳性率。案例研究《巨人》获得92.3%认证置信度。

Conclusion: 多模态方法相比单模态方法性能显著提升，证明了在艺术认证中应用统一计算方法处理视觉和放射影像的有效性。
...
### [101] [HyFormer-Net: A Synergistic CNN-Transformer with Interpretable Multi-Scale Fusion for Breast Lesion Segmentation and Classification in Ultrasound Images](https://arxiv.org/abs/2511.01013)
...
Method: 提出HyFormer-Net混合架构，集成EfficientNet-B3和Swin Transformer的双分支编码器，通过多尺度分层融合块结合，注意力门控解码器提供精确性和可解释性。采用双管道可解释性方法：内在注意力验证和Grad-CAM分类推理。
...
Conclusion: HyFormer-Net在乳腺癌超声诊断中实现了优异的性能和可解释性，通过渐进式微调展示了强大的跨数据集泛化能力，为临床采用提供了可行方案。
...
### [102] [FastBoost: Progressive Attention with Dynamic Scaling for Efficient Deep Learning](https://arxiv.org/abs/2511.01026)
*JunXi Yuan*
...
<details>
  <summary>Details</summary>
Motivation: 解决资源受限边缘设备上部署深度学习模型时参数效率与准确性之间的权衡问题，旨在实现更小的模型尺寸同时保持或提升性能。

Method: 采用动态缩放渐进注意力机制，包括自适应融合、阶段缩放和残差自适应三个创新组件，结合增强的MBConv模块构建双注意力路径架构。
...
Conclusion: FastBoost通过动态注意力与高效卷积操作的协同优化，实现了前所未有的参数-准确率权衡，为资源受限边缘设备提供了可行的部署方案。
...
### [103] [T-MLA: A Targeted Multiscale Log--Exponential Attack Framework for Neural Image Compression](https://arxiv.org/abs/2511.01079)
*Nikolay I. Kalmykov,Razan Dibo,Kaiyu Shen,Xu Zhonghan,Anh-Huy Phan,Yipeng Liu,Ivan Oseledets*

Main category: cs.CV

TL;DR: 提出了T-MLA攻击框架，这是首个针对神经图像压缩系统的目标多尺度对数-指数攻击，通过在小波域中直接针对压缩和重建图像质量来生成对抗性扰动。


<details>
  <summary>Details</summary>
Motivation: 神经图像压缩系统虽然具有优异的率失真性能，但其安全漏洞研究远不如分类器深入。现有攻击方法往往简单地将像素空间方法移植过来，忽略了压缩管道的独特结构化特性。

Method: T-MLA攻击框架在小波域中精心设计对抗性扰动，通过将扰动策略性地限制在特定小波子带中，直接针对攻击和重建图像的质量，实现最大失真同时保持视觉隐蔽性。

Result: 在多个最先进的神经图像压缩架构和标准图像压缩基准上的广泛评估显示，重建质量大幅下降，而扰动在视觉上难以察觉。

Conclusion: 研究揭示了生成和内容分发管道核心存在的关键安全漏洞。
...
### [104] [GeoToken: Hierarchical Geolocalization of Images via Next Token Prediction](https://arxiv.org/abs/2511.01082)
*Narges Ghasemi,Amir Ziashahabi,Salman Avestimehr,Cyrus Shahabi*

Main category: cs.CV

TL;DR: 提出了一种基于分层序列预测的图像地理定位方法，使用S2网格单元进行层次化位置预测，借鉴了语言模型的自回归生成技术，在多个数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 图像地理定位面临的主要挑战是不同地理位置之间的视觉相似性和巨大的搜索空间。受人类从大区域到具体地址逐步缩小范围的方式启发，希望开发一种类似的分层预测方法。

Method: 使用S2网格单元构建嵌套的多分辨率全局网格，采用自回归方式依次预测更精细层级的网格单元。结合了束搜索和多样本推理等语言模型技术，探索多种选择策略来确定最终输出。

Result: 在Im2GPS3k和YFCC4k数据集上评估，在无MLLM设置下超越其他基线方法，准确率提升高达13.9%；结合MLLM时在所有指标上都达到最先进水平。

Conclusion: 分层序列预测方法有效解决了图像地理定位问题，通过结合自回归预测和语言模型推理技术，显著提升了定位精度，为地理定位任务提供了新的解决方案。
...
### [105] [SliceVision-F2I: A Synthetic Feature-to-Image Dataset for Visual Pattern Representation on Network Slices](https://arxiv.org/abs/2511.01087)
*Md. Abid Hasan Rafi,Mst. Fatematuj Johora,Pankaj Bhowmik*

Main category: cs.CV

TL;DR: SliceVision-F2I是一个用于网络切片特征可视化的合成数据集，包含4种编码方法生成的12万样本，每个样本包含原始KPI向量和对应的RGB图像，适用于视觉学习和网络状态分析。


<details>
  <summary>Details</summary>
Motivation: 5G/6G网络中网络切片需要精细识别方法，但缺乏支持稳健数据集。该研究旨在为下一代网络系统提供网络切片特征可视化的研究数据集。

Method: 使用四种编码方法将多变量KPI向量转换为视觉表示：物理启发映射、Perlin噪声、神经壁纸和分形分支。每种方法生成3万个样本，模拟真实噪声网络条件。

Result: 创建了包含12万个样本的数据集，每个样本包含原始KPI向量和低分辨率RGB图像，公开可用，适用于多种研究任务。

Conclusion: SliceVision-F2I数据集为网络切片研究提供了有价值的资源，支持视觉学习、网络状态分类和异常检测等任务，可在多种研究背景下重用。
...
### [106] [Epanechnikov nonparametric kernel density estimation based feature-learning in respiratory disease chest X-ray images](https://arxiv.org/abs/2511.01098)
*Veronica Marsico,Antonio Quintero-Rincon,Hadj Batatia*

Main category: cs.CV

TL;DR: 提出了一种结合Epanechnikov核密度估计和双峰逻辑回归分类器的呼吸疾病诊断方法，在13808张胸部X光片上达到70.14%准确率。


<details>
  <summary>Details</summary>
Motivation: 利用EKDE的灵活性建模医学图像数据分布，适应像素强度变化，提高呼吸疾病诊断的准确性和可靠性。

Method: 结合Epanechnikov非参数核密度估计(EKDE)和双峰逻辑回归分类器，采用基于统计模型的学习方案。

Result: 在COVID-19放射影像数据集的13808张随机选择胸部X光片上，准确率70.14%，敏感性59.26%，特异性74.18%。

Conclusion: EKDE方法在医学影像诊断中具有潜力，但敏感性有待提高，临床专业知识对模型优化仍至关重要。
...
### [107] [Anatomically Constrained Transformers for Echocardiogram Analysis](https://arxiv.org/abs/2511.01109)
*Alexander Thorley,Agis Chartsias,Jordan Strom,Jeremy Slivnick,Dipak Kotecha,Alberto Gomez,Jinming Duan*

Main category: cs.CV

TL;DR: ViACT是一种将解剖先验整合到视频Transformer中的框架，通过掩码自编码策略专注于解剖区域学习，提高超声心动图分析的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有视频Transformer在超声心动图分析中容易学习来自非诊断区域（如图像背景）的虚假相关性，需要克服这一限制。

Method: 将变形解剖结构表示为点集，将空间几何和对应图像块编码为Transformer标记，采用掩码自编码策略仅重建解剖区域。

Result: ViACT在左心室射血分数回归和心脏淀粉样变性检测任务中表现良好，注意力图与已知病理区域对齐，并能泛化到心肌点跟踪任务。

Conclusion: ViACT通过整合解剖约束，使Transformer专注于相关解剖区域，提高了超声心动图分析的性能和可解释性，并能泛化到其他任务。
...
### [108] [Boosting performance of computer vision applications through embedded GPUs on the edge](https://arxiv.org/abs/2511.01129)
*Fabio Diniz Rossi*

Main category: cs.CV

TL;DR: 使用GPU的嵌入式设备可以提升移动设备上计算机视觉应用的性能，改善用户体验


<details>
  <summary>Details</summary>
Motivation: 移动设备上的计算机视觉应用（特别是增强现实）对资源需求高，边缘计算设备能力有限，可能影响用户体验

Method: 在嵌入式设备中使用GPU来卸载高强度的计算任务，与仅使用CPU进行对比实验

Result: 实验显示GPU相比仅使用CPU能获得性能提升

Conclusion: GPU能够保证用户在使用此类应用时获得更好的体验
...
TL;DR: 提出了一种无需概念标注的弱监督概念预测框架PCP，通过类别级概念先验作为弱监督，在医学影像中实现可解释预测，性能优于零样本基线并与全监督方法相当。


<details>
  <summary>Details</summary>
Motivation: 医学影像中可解释AI需要概念标注，但标注成本高昂且不实用。现有无标注方法难以捕捉领域特定医学特征，可靠性差。

Method: PCP框架利用类别级概念先验作为弱监督，结合KL散度和熵正则化的精炼机制来对齐临床推理。

Result: 在PH2和WBCatt数据集上，概念级F1分数比零样本基线提高33%以上，在四个医学数据集上分类性能与全监督CBM和V-IP相当。

Conclusion: PCP提供了一种无需概念标注的可行方案，在医学影像中实现可靠的可解释预测。
...
TL;DR: CatEquiv是一种用于惯性传感器人体活动识别的类别等变神经网络，通过编码时间、幅度和结构对称性，在分布外扰动下显著提升了鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统方法未能系统性地编码数据中的时间、幅度和传感器层次结构对称性，导致在分布外场景下泛化能力不足。

Method: 引入类别对称积，将循环时间平移、正增益和传感器层次偏序集结合，构建类别对称结构，实现对该对称积的等变性。

Result: 在UCI-HAR数据集上，CatEquiv在分布外扰动下相比循环填充CNN和普通CNN展现出显著更高的鲁棒性。

Conclusion: 强制类别对称性可以在不增加模型容量的情况下实现强大的不变性和泛化能力。
...
TL;DR: 提出MicroAUNet轻量级注意力分割网络，结合深度可分离扩张卷积和参数共享的通道-空间注意力块，通过两阶段知识蒸馏方案实现高精度实时结直肠息肉分割。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习的息肉分割模型要么提供模糊的息肉边界影响临床决策，要么依赖复杂架构导致推理速度不足，无法满足实时内窥镜应用需求。

Method: 使用深度可分离扩张卷积和单路径参数共享的通道-空间注意力块增强多尺度边界特征，并采用渐进式两阶段知识蒸馏方案从高容量教师网络转移语义和边界线索。

Result: 在基准测试中表现出最先进的准确性，同时保持极低的模型复杂度，适合实时临床息肉分割。

Conclusion: MicroAUNet在极低模型复杂度下实现了最先进的准确率，适用于实时临床息肉分割应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估方法将多模态能力孤立对待，缺乏对跨模态推理的测试，而这是实现真正统一多模态智能的核心能力。

Method: 构建包含两个互补设置的人工标注基准：语言增强的视觉生成推理（用语言提示和推理链指导图像合成）和视觉增强的语言生成推理（生成中间可视化来加强问答推理）。

Result: 对17个统一模型的实验显示：跨模态推理决定视觉生成质量，交织模型显著优于非交织模型；模型在物理和符号推理之间存在分离，能解释感知概念但无法为符号任务构建视觉抽象。

Conclusion: 互惠跨模态推理是实现真正全模态生成的关键前沿，当前模型在这方面仍有明显不足。
...
TL;DR: 提出了一个从YouTube视频自动挖掘和处理动物视频的流程，创建了包含30K视频（200万帧）的大规模数据集，并建立了Animal-in-Motion基准测试，用于4D四足动物重建任务。


<details>
  <summary>Details</summary>
Motivation: 现有动物视频数据集规模有限（仅2.4K个15帧片段），缺乏对动物中心3D/4D任务的关键处理，需要非侵入性的大规模数据收集方法。

Method: 开发自动化流程从YouTube挖掘视频并处理成对象中心片段，附带姿态估计、跟踪和3D/4D重建等下游任务的有用注释。建立了包含230个手动筛选序列的Animal-in-Motion基准测试。

Result: 收集了30K视频（2M帧），比先前工作大一个数量级。评估发现基于模型的方法在2D指标上表现更好但3D形状不真实，而无模型方法产生更自然的重建但得分较低。通过序列级优化改进了无模型方法，建立了首个4D动物重建基线。
...
TL;DR: 提出基于图像小波谱的扩散Transformer模型(DTWSR)，通过捕捉多尺度频率子带间的相互关系来解决图像超分辨率中的不一致性和伪影问题。
...
Method: 使用多级离散小波变换分解图像为小波谱，提出金字塔标记化方法将谱嵌入为token序列，设计双解码器分别处理低频和高频子带，同时保持它们在图像生成中的对齐。
...
TL;DR: 提出GCN-PSN框架，利用图卷积网络建模人体骨骼拓扑结构，通过孪生网络和对比回归目标学习判别性姿态嵌入，在动作质量评估任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 动作质量评估需要细粒度理解人体运动和精确评估姿态相似性，现有基于坐标的方法可能忽略了骨骼拓扑结构信息。

Method: 使用拓扑感知的图卷积网络将人体骨骼建模为图结构，采用孪生网络架构和对比回归目标训练，学习对拓扑敏感的判别性姿态嵌入。

Result: 在AQA-7和FineDiving基准测试中超越了基于坐标的基线方法，取得了有竞争力的性能，消融实验验证了利用骨骼拓扑的有效性。

Conclusion: 骨骼拓扑信息对于姿态相似性和动作质量评估至关重要，GCN-PSN框架通过建模拓扑结构显著提升了评估性能。
...
Method: 提出多尺度令牌保留策略(MTPS)集成到分层残差向量量化变分自编码器(RQ-VAE)中，使用可扩展自回归(SAR)建模预测尺度令牌，并设计轻量级CAQ-VAE解决插值带来的重建退化问题。

Result: 在Motion-X数据集上，MoSa达到FID 0.06（优于MoMask的0.20），推理时间减少27%，在运动编辑等下游任务中表现良好且无需微调。

Conclusion: MoSa通过分层运动生成框架在3D人体运动生成任务中实现了质量和效率的双重提升，为文本驱动的运动生成提供了有效解决方案。
...
### [117] [OmniVLA: Unifiying Multi-Sensor Perception for Physically-Grounded Multimodal VLA](https://arxiv.org/abs/2511.01210)
*Heyu Guo,Shanmu Wang,Ruichun Ma,Shiqi Jiang,Yasaman Ghasempour,Omid Abari,Baining Guo,Lili Qi*

Main category: cs.CV

TL;DR: OmniVLA是一个多模态视觉-语言-动作模型，通过整合红外相机、毫米波雷达和麦克风阵列等新型传感模态，超越了传统RGB感知的限制，在真实世界操作任务中取得了84%的平均成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型主要依赖RGB相机，限制了其感知能力和操作能力。需要整合更多传感模态来增强物理空间智能。

Method: 提出传感器掩码图像的统一表示方法，将空间基础和物理意义的掩码叠加到RGB图像上。基于RGB预训练的VLA骨干网络，构建多感官VLA模型架构，使用轻量级传感器投影器进行数据高效学习。

Result: 在需要传感器模态感知的挑战性真实世界任务中，OmniVLA达到84%的平均任务成功率，显著优于RGB-only和原始传感器输入基线模型，分别提升59%和28%。

Conclusion: OmniVLA通过多模态传感整合实现了更强的感知能力和操作性能，展示了更高的学习效率和更强的泛化能力。
...
TL;DR: 该论文针对印度食物视觉问答(VQA)系统，提出了一种多步推理链方法，通过自动验证的推理链增强小型语言模型和视觉语言模型，结合强化学习提升印度食物VQA的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有VQA系统偏向西方食物，无法处理印度食物的文化多样性和复杂烹饪背景。现有印度食物VQA数据集采用两步法生成答案，但食物VQA需要多步推理过程来理解复杂的烹饪情境和食物关系。

Method: 创建自动验证的推理链，微调小型LLM和VLM，使用强化学习进行训练，通过推理链增强模型性能。

Result: 通过推理链增强，在基准测试上平均准确率提高了10个百分点。

Conclusion: 多步推理链方法能有效提升印度食物VQA系统的性能，特别是在理解复杂烹饪背景和食物关系方面。
...
### [119] [Saliency-Guided Domain Adaptation for Left-Hand Driving in Autonomous Steering](https://arxiv.org/abs/2511.01223)
*Zahra Mehraban,Sebastien Glaser,Michael Milford,Ronald Schroeter*

Main category: cs.CV

TL;DR: 该论文研究了通过翻转数据预训练和微调的方法来改进自动驾驶模型在左右手驾驶条件间的领域自适应性能，在PilotNet和ResNet架构上验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶模型需要良好的领域自适应能力来适应不同的道路条件，特别是左右手驾驶的差异。论文旨在探索如何通过训练方法改进模型在澳大利亚左手驾驶条件下的表现。

Method: 评估了四种训练方法：1) 基于美国右手驾驶数据的基线模型；2) 基于翻转美国数据的模型；3) 在美国数据上预训练后在澳大利亚高速公路上微调；4) 在翻转美国数据上预训练后在澳大利亚高速公路上微调。使用显著性分析来测量注意力转移。

Result: 单独使用翻转数据预训练会因特征表示不匹配而降低预测稳定性，但随后进行微调能显著改善自适应性能，降低预测误差并增强对左侧线索的关注。在ResNet架构上的实验也证实了类似的适应趋势。

Conclusion: 翻转数据预训练结合微调的方法能有效改进模型自适应性能，同时最小化重新训练需求，强调了预处理技术的重要性。
...
### [120] [Gesture Generation (Still) Needs Improved Human Evaluation Practices: Insights from a Community-Driven State-of-the-Art Benchmark](https://arxiv.org/abs/2511.01233)
...
TL;DR: 本文分析了语音驱动3D手势生成领域的人类评估实践，发现缺乏标准化和存在实验设计缺陷。作者提出了BEAT2数据集的人类评估协议，并通过大规模众包评估对6个最新手势生成模型进行排名，结果表明新模型不一定优于早期方法，且需要采用解耦评估方法。


<details>
  <summary>Details</summary>
Motivation: 当前语音驱动3D手势生成领域的人类评估实践缺乏标准化，实验设计存在缺陷，导致无法准确比较不同方法的性能，也不清楚该领域的最新技术水平。

Method: 引入BEAT2运动捕捉数据集的详细人类评估协议，通过大规模众包评估对6个最新手势生成模型进行排名，评估运动真实性和语音-手势对齐两个关键维度。

Result: 评估结果表明：1）新模型并不总是优于早期方法；2）已发表的高运动真实性或语音-手势对齐声明在严格评估下可能不成立；3）需要采用解耦的运动质量和多模态对齐评估方法。

Conclusion: 该领域必须采用解耦的运动质量和多模态对齐评估方法进行准确基准测试，作者将发布合成运动数据、渲染视频刺激、开源渲染脚本和16,000对人工偏好投票以推动标准化。
...
<details>
  <summary>Details</summary>
Motivation: 人类注视为理解复杂视觉环境中的注意力提供了丰富的监督信号，传统目标检测器对所有区域平等对待，而实际应用中人类关注的区域更为重要。

Method: 在Vision Transformer的注意力机制中注入注视特征，使空间特征选择偏向人类关注的区域，并引入注视感知的注意力头重要性指标来解释模型行为。

Result: 在自定义模拟器数据集和公共基准测试（Ego4D Ego-Motion和Ego-CH-Gaze）上，相比无视注视的基线方法，检测精度持续提升。

Conclusion: 该方法有效利用注视信号增强目标检测，在评估模拟场景中的人类表现方面具有潜力，注视线索能够调节transformer的注意力动态。
...
TL;DR: 提出了一种基于双阶信息的黑盒梯度可迁移攻击方法，通过引入对抗平坦性(AF)解决欺骗性平坦问题，并开发了对抗平坦性攻击(AFA)和蒙特卡洛对抗采样(MCAS)来提升攻击能力。


<details>
  <summary>Details</summary>
Motivation: 现有可迁移攻击方法虽然关注平坦损失，但仍陷入次优区域，特别是平坦但尖锐的区域（欺骗性平坦问题），这限制了对抗样本在不同模型间的迁移能力。

Method: 从双阶信息角度提出对抗平坦性(AF)概念，通过高效近似目标函数实例化为对抗平坦性攻击(AFA)，并设计蒙特卡洛对抗采样(MCAS)提升内循环采样效率。

Result: 在ImageNet兼容数据集上的综合结果表明，该方法在六种基线方法中表现优越，生成的对抗样本位于更平坦区域，跨模型架构的迁移性显著提升，在输入变换攻击和百度云API测试中也优于基线方法。

Conclusion: 该研究通过对抗平坦性方法有效解决了欺骗性平坦问题，显著提升了黑盒可迁移攻击的性能，为现实世界威胁提供了更有效的攻击手段。
...
TL;DR: CenterMamba-SAM是一种用于脑部病灶分割的端到端框架，通过冻结预训练主干网络并仅训练轻量级适配器实现高效微调，采用创新的3x3角-轴-中心短序列扫描策略增强对弱边界和小病灶的敏感性。


<details>
  <summary>Details</summary>
Motivation: 脑部病灶分割面临小病灶、低对比度、各向异性采样和跨切片不连续性等挑战，需要提高对弱边界和小病灶的检测能力。

Method: 使用CenterMamba编码器采用3x3角-轴-中心短序列扫描策略，结合基于内存的结构提示生成器和内存增强的多尺度解码器，实现中心优先、轴强化和对角补偿的信息聚合。

Result: 在公共基准测试上的广泛实验表明，CenterMamba-SAM在脑部病灶分割方面达到了最先进的性能。

Conclusion: 该方法通过创新的扫描策略和内存驱动机制，有效解决了脑部病灶分割中的关键挑战，实现了优异的性能表现。
...
TL;DR: 提出了一种轻量几何感知适配器，通过方位对齐和水平环形填充来保持邻域连续性，使用局部K近邻计算几何特征，在训练时驱动区域感知正则化，提升恶劣天气下LiDAR语义分割的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: LiDAR语义分割在恶劣天气下性能下降，现有方法忽略了边界、角落和稀疏区域的结构脆弱性，需要几何驱动的正则化来提升鲁棒性。

Method: 使用方位对齐和水平环形填充保持邻域连续性，局部窗口K近邻收集附近点并计算简单统计量，压缩为紧凑几何感知线索，驱动区域感知正则化训练。

Result: 在SemanticKITTI训练、SemanticSTF评估的跨天气设置中，mIoU比数据增强基线提升7.9个百分点，比类别中心正则化基线提升0.6个百分点。

Conclusion: 几何驱动的正则化是全天气LiDAR分割的关键方向，适配器即插即用，训练时启用，推理成本可忽略。
...
TL;DR: MotionStream实现了实时交互的视频生成，通过将双向教师模型蒸馏为因果学生模型，结合滑动窗口因果注意力和注意力汇技术，解决了长视频生成中的领域差距、质量保持和计算效率问题。
...
Method: 通过自强制分布匹配蒸馏将双向教师模型转化为因果学生模型，采用滑动窗口因果注意力和注意力汇技术，结合自展开和KV缓存滚动训练。

Result: 在单个GPU上实现亚秒级延迟和最高29FPS的流式生成，运动跟随和视频质量达到最先进水平，速度提升两个数量级。

Conclusion: MotionStream实现了无限长度流式视频生成，用户能够实时绘制轨迹、控制相机或传输运动，提供真正的交互体验。
...
TL;DR: PRevivor是一个基于先验引导的颜色变换器，通过学习明清时期绘画来恢复唐宋时期古画的色彩，通过亮度增强和色调校正两个子任务实现色彩恢复。
...
Method: 将色彩恢复分解为亮度增强和色调校正两个顺序子任务：使用两个变分U-Net和多尺度映射模块进行亮度增强；设计双分支颜色查询模块，在局部色调先验指导下进行色调校正。

Result: 与最先进的着色方法进行广泛实验，结果表明PRevivor在定量和定性评估中都表现出优越性能。

Conclusion: PRevivor通过先验引导的色彩变换方法，有效恢复了古代绘画的色彩，为文化遗产保护提供了有效的数字修复工具。
...
TL;DR: 本文综述了基础模型在医学影像分析中的适应策略，评估了各种方法的性能、临床适用性和局限性，并提出了解决当前挑战的新兴研究方向。


<details>
  <summary>Details</summary>
Motivation: 基础模型在医学影像分析中具有变革潜力，但适应真实临床实践仍面临领域偏移、标注数据稀缺、计算需求大和隐私要求严格等挑战。

Method: 评估了监督微调、领域特定预训练、参数高效微调、自监督学习、混合方法以及多模态框架等多种适应策略。

Result: 分析了各种方法的性能增益和临床适用性，识别了权衡因素和未解决的挑战。
...
TL;DR: 提出了一种基于数据流形几何差异的图像生成检测框架，利用自然图像和生成图像在流形结构上的差异进行检测，无需依赖大量生成图像样本。
...
Method: 设计一对函数使自然图像输出一致而生成图像输出发散，利用梯度正交性；通过自监督模型在数据流形上的变换检测损失变化；使用归一化流放大生成图像与自然图像流形的差异。

Result: 大量实验证明该方法有效，代码已开源。

Conclusion: 该方法利用数据流形几何特性实现了有效的生成图像检测，对先进生成模型具有鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态生成模型在需要隐式推理的复杂图像编辑任务上表现不佳，现有基准测试主要关注单对象属性变换，忽略了多对象交互和游戏世界场景，且仅依赖文本参考评估可能导致误判。

Method: 构建UniREditBench基准测试，包含2700个精心策划的样本，涵盖8个主要维度和18个子维度；引入多模态双参考评估（文本和真实图像参考）；设计自动化多场景数据合成流程，构建UniREdit-Data-100K数据集；基于该数据集微调Bagel模型得到UniREdit-Bagel。

Result: UniREdit-Bagel在领域内和领域外设置下均表现出显著改进；通过对开源和闭源图像编辑模型的全面基准测试，揭示了它们在不同方面的优势和弱点。

Conclusion: UniREditBench为基于推理的图像编辑提供了全面的评估框架，多模态双参考评估提高了评估可靠性，构建的大规模数据集和微调模型为未来研究提供了有价值的资源。
...
<details>
  <summary>Details</summary>
Motivation: 传统胃内容物超声评估依赖手动追踪和经验公式，在效率和准确性方面存在显著局限，需要更可靠的自动化解决方案来改善术前误吸风险评估。

Method: 两阶段框架：第一阶段使用分割模型生成抑制伪影、突出胃解剖结构的概率图；第二阶段通过双分支分类器融合右侧卧位和仰卧位两个标准视图的信息，提升特征判别能力。

Result: 在自收集数据集上的实验结果表明，该框架显著优于当前最先进方法，为临床实践提供了更鲁棒、高效和准确的解决方案。

Conclusion: REASON框架在胃内容物评估方面表现出巨大潜力，能够为术前误吸风险评估提供自动化的可靠工具，有望改善临床实践。
...
TL;DR: 提出了一种用于全切片病理图像解释性表示的三阶段框架，通过潜在因子分组、聚类推理实例解缠和实例效应重加权来解决空间、语义和决策纠缠问题。
...
Method: 三阶段框架：1) 正半定潜在因子分组映射实例到潜在子空间缓解空间纠缠；2) 实例概率反事实推理和聚类推理实例解缠缓解语义纠缠；3) 广义线性加权决策通过实例效应重加权解决决策纠缠。

Result: 在多中心数据集上的广泛实验表明，该模型优于所有最先进模型，并通过解缠表示和透明决策过程获得了与病理学家一致的可解释性。

Conclusion: 该框架有效解决了MIL中的纠缠问题，在保持高性能的同时实现了病理学家对齐的可解释性。
...
TL;DR: 提出了APDM框架，通过将保护目标从图像转移到扩散模型本身来防止特定主体的个性化，包含DPO损失函数和L2P双路径优化策略，有效阻止未经授权的个性化内容生成。


<details>
  <summary>Details</summary>
Motivation: 扩散模型的高质量特定主体合成能力带来了隐私风险，现有基于对抗样本的保护方法在少量干净图像或简单图像变换下失效，需要更鲁棒的保护方案。

Method: 提出APDM框架，包含：1）DPO损失函数直接破坏目标模型的主体个性化能力；2）L2P双路径优化策略，通过交替进行个性化和保护路径来模拟未来个性化轨迹并自适应强化保护。

Result: 实验结果表明APDM框架优于现有方法，在防止未经授权个性化方面达到最先进性能。

Conclusion: APDM通过将保护目标转移到扩散模型本身，并采用DPO损失和L2P优化策略，有效解决了扩散模型个性化带来的隐私风险问题。
...
TL;DR: MVSMamba是首个基于Mamba架构的多视角立体视觉网络，通过动态Mamba模块实现高效的全局特征聚合，在DTU和Tanks-and-Temples数据集上实现了优于现有方法的性能和效率。
...
Result: 在DTU数据集和Tanks-and-Temples基准测试中，MVSMamba超越了最先进的MVS方法，在性能和效率方面均表现出色。
...
TL;DR: 提出了一种基于CLIP模型的生成对抗攻击方法，通过结合文本和图像表示能力，生成视觉不可察觉但有效的对抗扰动，在多目标环境中欺骗多标签分类器。
...
Method: 整合CLIP模型的文本-图像对齐能力，结合SSAE的集中扰动策略和GAMA的不同文本嵌入方法，使用引导损失生成对抗样本。

Result: 在多种黑盒受害者模型上的实验表明，该方法在保持较高视觉保真度的同时，达到了与现有技术相当或更优的攻击效果。

Conclusion: 该方法成功地将自然语言语义与对抗攻击相结合，实现了在欺骗分类模型的同时保持图像结构相似性的目标。
...
TL;DR: 提出RDTE-UNet分割网络，结合局部建模与全局上下文，通过混合ResBlock细节感知Transformer骨干和三个模块（ASBE、HVDA、EulerFF）增强边界描绘和细节保留。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割对计算机辅助诊断和治疗规划至关重要，但解剖结构变异性和边界模糊性阻碍了精细结构的可靠描绘。

Method: 使用混合ResBlock细节感知Transformer骨干网络，结合ASBE自适应边界增强模块、HVDA细粒度特征建模模块和EulerFF基于欧拉公式的融合加权模块。
...
TL;DR: 提出了一个包含1333个英语Rebus谜题的大规模多样化基准，并开发了RebusDescProgICE框架，通过结合非结构化描述和基于代码的结构化推理，显著提升了视觉语言模型在Rebus谜题上的性能。
...
Method: 提出了RebusDescProgICE框架，使用非结构化描述和基于代码的结构化推理相结合的方法，并改进了基于推理的上下文示例选择策略。

Result: 相比思维链推理，该框架在使用闭源模型时性能提升2.1-4.1%，在使用开源模型时性能提升20-30%。
...
<details>
  <summary>Details</summary>
Motivation: 现有SAM交互式分割方法多采用单点单对象范式，限制了多病灶分割能力；同时ViT骨干网络虽能捕捉全局上下文但缺乏高保真局部细节。

Method: 提出提示条件实例查询生成器将单点提示转换为多个专用查询；采用混合CNN-Transformer编码器通过空间门控注入CNN边界显著性；使用竞争优化查询解码器实现端到端并行多实例预测。

Result: 在LiTS17和KiTS21数据集上，MIQ-SAM3D达到可比较水平，并对提示具有强鲁棒性。
...
TL;DR: 提出了一种通过内容-风格子空间混合和平衡损失来扩展内容-风格前沿的方法，在保持风格强度的同时显著改善内容相似性。
...
Method: 采用内容-风格子空间混合技术和内容-风格平衡损失函数，在变化风格强度下保持内容相似性。

Result: 在定性和定量评估中均优于现有技术，实现了更优的内容-风格权衡，获得了显著更低的IGD和GD分数。

Conclusion: 该方法有效扩展了内容-风格前沿，在多种风格强度下都能保持更好的内容保真度。
...
TL;DR: 提出了CMI-MTL框架，通过细粒度视觉-文本特征对齐、跨模态交错特征表示和自由形式答案增强多任务学习，解决医学视觉问答中的跨模态语义对齐和自由形式答案多样性问题。


<details>
  <summary>Details</summary>
Motivation: 现有自注意力方法难以有效处理视觉与语言之间的跨模态语义对齐，分类方法依赖预定义答案集，无法适应自由形式答案的多样性并忽略详细语义信息。

Method: 包含三个关键模块：FVTA（细粒度视觉-文本特征对齐）、CIFR（跨模态交错特征表示）和FFAE（自由形式答案增强多任务学习），通过跨模态交互学习图像和文本的特征表示。

Result: 在VQA-RAD、SLAKE和OVQA三个Med-VQA数据集上优于现有最先进方法，并通过可解释性实验验证了有效性。

Conclusion: CMI-MTL框架有效解决了医学视觉问答中的跨模态语义对齐和自由形式答案处理问题，在多个数据集上表现出优越性能。
...
TL;DR: 提出了一种用于生成水下环境中安装在AUV上的事件相机合成数据的管道，用于训练视觉模型。


<details>
  <summary>Details</summary>
Motivation: 水下环境存在光照条件差、高动态范围场景等挑战，传统视觉技术难以适应。事件相机通过逐帧跟踪变化来缓解这些问题。

Method: 开发了一个管道来生成事件相机在水下环境中的真实合成数据，特别针对能见度差和悬浮颗粒物条件下的岩石检测任务。

Result: 展示了该方法在能见度差和悬浮颗粒物条件下的岩石检测任务中的有效性。

Conclusion: 该管道可推广到其他水下任务，为水下视觉模型训练提供了有效的合成数据生成解决方案。
...
### [141] [Semantic BIM enrichment for firefighting assets: Fire-ART dataset and panoramic image-based 3D reconstruction](https://arxiv.org/abs/2511.01399)
*Ya Wen,Yutong Qiao,Chi Chiu Lam,Ioannis Brilakis,Sanghoon Lee,Mun On Wong*

Main category: cs.CV

TL;DR: 该研究提出了Fire-ART数据集和基于全景图像的消防资产重建方法，用于将消防资产语义丰富到BIM模型中，解决了传统方法在自动资产识别和重建方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统消防资产管理方法在自动资产识别和重建方面能力有限，效率低下。为了提升应急准备、风险评估和现场火灾响应的能力，需要开发更有效的自动化管理技术。

Method: 开发了包含15种基本资产、2626张图像和6627个实例的Fire-ART数据集，并提出了结合改进的立方体贴图转换和基于半径的球面相机投影的全景图像重建方法。

Result: 在两个真实案例验证中，该方法分别达到了73%和88%的F1分数，以及0.620米和0.428米的定位误差。
...
### [142] [Extremal Contours: Gradient-driven contours for compact visual attribution](https://arxiv.org/abs/2511.01411)
*Reza Karimzadeh,Albert Alonso,Frans Zdyb,Julius B. Kirkegaard,Bulat Ibragimov*

Main category: cs.CV

TL;DR: 提出一种无需训练的解释方法，用平滑可调轮廓替代密集扰动掩码，通过星凸区域参数化和傅里叶级数优化，生成单一连通掩码，大幅减少参数数量并提高稳定性。


<details>
  <summary>Details</summary>
Motivation: 当前视觉模型的忠实且紧凑解释面临挑战，常用的密集扰动掩码往往碎片化且过拟合，需要复杂的后处理。

Method: 使用截断傅里叶级数参数化星凸区域，在极端保留/删除目标下利用分类器梯度进行优化，生成单一连通平滑轮廓。

Result: 在ImageNet分类器上匹配密集掩码的极端保真度，产生紧凑可解释区域，运行一致性提高，在多轮廓扩展中能定位多个对象。

Conclusion: 该方法在基准测试中比梯度和扰动基线获得更高相关质量和更低复杂度，特别在自监督DINO模型上相关质量提升超过15%，保持正忠实相关性。
...
### [143] [Towards One-step Causal Video Generation via Adversarial Self-Distillation](https://arxiv.org/abs/2511.01419)
*Yongqi Yang,Huayang Huang,Xu Peng,Xiaobin Hu,Donghao Luo,Jiangning Zhang,Chengjie Wang,Yu Wu*

Main category: cs.CV

TL;DR: 提出基于蒸馏的高效因果视频生成框架，通过对抗自蒸馏策略和首帧增强技术，在极少数去噪步骤（1-2步）下实现高质量视频合成。
...
Result: 在VBench上的大量实验表明，该方法在一步和两步视频生成中均超越最先进方法，且单个蒸馏模型可灵活支持多种推理步骤设置。
...
### [144] [UniSOT: A Unified Framework for Multi-Modality Single Object Tracking](https://arxiv.org/abs/2511.01427)
...
<details>
  <summary>Details</summary>
Motivation: 现有跟踪器通常针对单一或少数几种视频模态和参考模态设计，导致模型分离且限制实际应用。需要统一的跟踪器来处理各种需求，但目前没有能够同时处理上述所有参考模态和视频模态的跟踪器。

Method: 提出了UniSOT统一跟踪器，通过统一参数设计来处理三种参考模态（边界框、自然语言、两者）和四种视频模态（RGB、RGB+深度、RGB+热成像、RGB+事件）的不同组合。

Result: 在18个视觉跟踪、视觉语言跟踪和RGB+X跟踪基准测试中，UniSOT表现出优于特定模态对应方法的性能。在TNL2K上所有三种参考模态上比先前方法提高超过3.0% AUC，在Un-Track上所有三种RGB+X视频模态上主要指标提高超过2.0%。
...
### [145] [Terrain-Enhanced Resolution-aware Refinement Attention for Off-Road Segmentation](https://arxiv.org/abs/2511.01434)
*Seongkyu Choi,Jhonghyun An*

Main category: cs.CV

TL;DR: 提出了一种分辨率感知的token解码器，用于解决越野语义分割中的边界模糊、稀疏监督和标签噪声问题，通过平衡全局语义、局部一致性和边界保真度来实现高效准确的分割。


<details>
  <summary>Details</summary>
Motivation: 越野语义分割面临边界不一致、稀有类别稀疏监督和普遍标签噪声等挑战。现有方法在低分辨率融合时会模糊边缘并传播局部错误，而保持高分辨率路径或重复高分辨率融合则成本高昂且对噪声敏感。

Method: 设计分辨率感知token解码器，主要计算在低分辨率瓶颈进行；使用门控交叉注意力注入精细尺度细节；仅对稀疏、不确定性选择的像素集进行细化；包含全局自注意力与轻量级扩张深度细化、门控交叉注意力集成高分辨率编码器特征、类别感知点细化等组件；训练时添加边界带一致性正则化器。

Result: 实现了竞争性性能，在过渡区域表现出改进的稳定性。

Conclusion: 该方法通过精心设计的组件协同工作，在保持计算效率的同时有效解决了越野语义分割中的关键挑战，特别是在边界处理和噪声鲁棒性方面表现出色。
...
### [146] [Contrast-Guided Cross-Modal Distillation for Thermal Object Detection](https://arxiv.org/abs/2511.01435)
*SiWoo Kim,JhongHyun An*

Main category: cs.CV

TL;DR: 提出了一种仅用于训练的方法来改善热红外检测，通过拉近同类特征和推远异类特征来锐化决策边界，并通过跨模态对齐来增强纹理贫乏的热红外特征。


<details>
  <summary>Details</summary>
Motivation: 解决热红外检测中的低对比度、弱高频线索导致的重复检测框、小目标漏检和类别混淆问题，避免现有方法对额外传感器或精确校准的依赖。

Method: 在训练阶段引入实例级决策边界锐化目标和跨模态语义先验对齐，使用RGB训练的教师模型来增强热红外特征，但保持单模态推理。

Result: 在实验中优于现有方法，达到了最先进的性能。

Conclusion: 该方法通过仅训练阶段的改进，有效提升了热红外检测的鲁棒性，无需测试时的额外传感器或校准。
...
### [147] [Privacy Preserving Ordinal-Meta Learning with VLMs for Fine-Grained Fruit Quality Prediction](https://arxiv.org/abs/2511.01449)
*Riddhi Jain,Manasi Patwardhan,Aayush Mishra,Parijat Deshpande,Beena Rai*

Main category: cs.CV

TL;DR: 提出了一种模型无关的序数元学习算法(MAOML)，用于训练小型视觉语言模型，在水果新鲜度分类任务中实现92.71%的行业标准准确率。


<details>
  <summary>Details</summary>
Motivation: 由于专家标注成本高导致数据稀缺，而专有视觉语言模型存在数据隐私问题，开源模型性能不足，需要开发既能保护隐私又能在数据有限情况下实现高性能的解决方案。

Method: 采用模型无关的序数元学习算法，结合元学习解决数据稀疏性问题，并利用标签的序数性来提升性能。

Result: 在零样本和少样本设置下均达到最先进性能，平均准确率为92.71%。

Conclusion: MAOML算法能够有效训练小型视觉语言模型，在数据稀缺情况下实现与专有模型相媲美的性能，同时解决了数据隐私问题。
...
### [148] [Reg-DPO: SFT-Regularized Direct Preference Optimization with GT-Pair for Improving Video Generation](https://arxiv.org/abs/2511.01450)
*Jie Du,Xinyu Gong,Qingshan Tan,Wen Li,Yangming Cheng,Weitao Wang,Chenlu Zhan,Suhui Wu,Hao Zhang,Jun Zhang*

Main category: cs.CV

TL;DR: 提出了Reg-DPO方法，通过自动构建GT-Pair偏好对和引入SFT损失作为正则化项，解决了视频生成中数据构建成本高、训练不稳定和内存消耗大的问题。
...
Method: 1) 使用GT-Pair自动构建高质量偏好对，以真实视频为正样本，模型生成视频为负样本；2) 在DPO目标中引入SFT损失作为正则化项；3) 结合FSDP框架和多种内存优化技术提升训练容量。

Result: 在多个数据集的I2V和T2V任务上的广泛实验表明，该方法始终优于现有方法，提供更优的视频生成质量。

Conclusion: Reg-DPO方法有效解决了视频生成中的关键挑战，通过自动偏好对构建、训练稳定化增强和内存优化，显著提升了大规模视频模型的性能。
...
### [149] [When to Trust the Answer: Question-Aligned Semantic Nearest Neighbor Entropy for Safer Surgical VQA](https://arxiv.org/abs/2511.01458)
...
Method: 提出QA-SNNE（问题对齐语义最近邻熵），一种黑盒不确定性估计器，通过在医学文本嵌入空间中比较生成答案与最近邻来测量语义熵，并以问题为条件。

Result: QA-SNNE在大多数模板内设置中提高了AUROC，零样本模型的AUROC提升15-38%，在模板外压力下仍保持增益，同时改进了幻觉检测。

Conclusion: QA-SNNE通过将语义不确定性与问题上下文关联，为手术VQA中的自动故障检测提供了实用且可解释的步骤，结合LVLM骨干和问题对齐的不确定性估计可提高安全性和临床医生信任度。
...
### [150] [Efficiently Training A Flat Neural Network Before It has been Quantizated](https://arxiv.org/abs/2511.01462)
*Peng Xia,Junbiao Pang,Tianyang Cai*

Main category: cs.CV

TL;DR: 提出了一种通过主动预处理模型来减少视觉Transformer后训练量化误差的方法，通过将激活和权重量化误差建模为高斯噪声来获得平坦的最小值。


<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法通常忽视训练好的神经网络与量化模型之间的关系，导致较大的量化误差，需要找到一种高效训练模型无关神经网络的方法来适配预定义精度的低比特模型。

Method: 将激活量化误差(AQE)和权重量化误差(WQE)统计建模为独立高斯噪声，研究多种噪声注入优化方法以获得平坦最小值。

Result: 实验结果表明该方法有效，为获得低比特后训练量化模型开辟了新途径。

Conclusion: 平坦的全精度神经网络对低比特量化至关重要，通过主动预处理和误差源解耦可以有效降低量化误差。
...
### [151] [HMVLM: Human Motion-Vision-Lanuage Model via MoE LoRA](https://arxiv.org/abs/2511.01463)
*Lei Hu,Yongjing Ye,Shihong Xia*

Main category: cs.CV

TL;DR: 提出了HMVLM框架，基于MoE LoRA策略解决3D人体运动与文本模态融合中的灾难性遗忘问题，通过零专家保留预训练参数，并采用身体部位特定标记化提高空间分辨率。


<details>
  <summary>Details</summary>
Motivation: 解决3D人体运动与基础语言模型融合时的模态鸿沟导致的灾难性遗忘问题，以及开发跨异构下游任务保持泛化能力的自回归兼容姿态表示的技术障碍。

Method: 基于MoE LoRA策略的统一框架，使用门控网络根据输入提示动态分配LoRA专家权重；引入零专家保留预训练参数；采用身体部位特定标记化将人体划分为不同关节组。

Result: 实验表明该方法有效缓解了指令调优期间的知识遗忘，在多样化人体运动下游任务中取得了显著性能。

Conclusion: HMVLM框架成功解决了3D人体运动与语言模型融合的关键技术挑战，为多模态理解和跨模态生成提供了有效解决方案。
...
TL;DR: SecDiff是一个基于扩散模型的即插即用解码框架，通过伪逆引导采样和自适应引导权重，显著提升深度联合源信道编码在对抗性无线环境下的安全性和鲁棒性。
...
Method: 采用伪逆引导采样和自适应引导权重实现灵活步长控制和高效语义重建；针对干扰攻击使用基于功率的子载波掩码策略；针对导频欺骗使用期望最大化驱动的重建算法。

Result: 在对抗性OFDM信道下的广泛实验表明，SecDiff在重建质量和计算成本之间实现了有利的平衡，优于现有的安全和生成式JSCC基线方法。
...
TL;DR: 提出了增强行人对齐网络(EPAN)，用于物联网监控环境中的人员重识别，在Inspection-Personnel数据集上取得了90.09%的Rank-1准确率和78.82%的mAP。
...
Conclusion: EPAN具有强大的特征提取能力，在真实物联网应用中具有潜力，能够在不同摄像头间实现有效可靠的人员重识别。
...
<details>
  <summary>Details</summary>
Motivation: 物体姿态估计面临部分可观测性、遮挡和物体对称性等挑战，导致姿态模糊和多假设问题。确定性深度网络在约束条件下表现良好，但无法捕捉底层姿态分布的多模态特性

Method: 在SE(3)流形上使用流匹配的概率框架，建模完整的姿态分布，提供基于样本的姿态估计，能够在对称物体或严重遮挡等模糊情况下进行不确定性推理

Result: 在Real275、YCB-V和LM-O数据集上达到最先进的结果，并展示样本姿态估计在下游机器人操作任务中的应用，如主动感知消除不确定视点和不确定性感知的抓取合成

Conclusion: 提出的概率框架能够有效处理物体姿态估计中的模糊性问题，提供更可靠的不确定性估计，并在机器人操作任务中展现出实用价值
...
TL;DR: 提出DiMoDE框架，通过区分处理运动分量并利用其刚性流的几何规律，改进深度和自运动的无监督联合学习。


<details>
  <summary>Details</summary>
Motivation: 现有方法将自运动作为辅助任务，混合所有运动类型或排除深度无关的旋转运动，限制了强几何约束的引入，降低了在多样化条件下的可靠性和鲁棒性。

Method: 通过将源相机和目标相机的光轴和成像平面对齐，转换帧间光流并量化偏差，对每个自运动分量单独施加几何约束。将联合学习重新表述为共轴和共面形式，通过闭式几何关系相互推导深度和每个平移分量。
...
TL;DR: 提出LASQ框架，将低光图像增强重新定义为基于分层亮度分布的统计采样过程，通过扩散前向过程自主发现亮度层间的最优转换路径，实现无需正常光参考的无监督分布模拟。
...
Method: 引入亮度感知统计量化(LASQ)，将亮度转换建模为强度坐标空间中的幂律分布，用分层幂函数近似，用概率采样替代确定性映射，设计扩散前向过程自主发现亮度层间最优转换路径。

Result: 显著提升实际场景中的性能，实现更适应和通用的光恢复，在有正常光参考的情况下在领域特定数据集上达到优越性能，并在非参考数据集上具有更好的泛化能力。
...
### [157] [Example-Based Feature Painting on Textures](https://arxiv.org/abs/2511.01513)
...
<details>
  <summary>Details</summary>
Motivation: 自然纹理普遍存在各种表面变化特征，将这些特征纳入合成过程对于生成逼真纹理至关重要，但传统方法需要大量手动标注。

Method: 采用基于学习的无监督异常检测方法，自动检测外观改变特征并聚类成语义连贯组，结合扩散模型进行条件生成和交互式编辑。
...
### [158] [NSYNC: Negative Synthetic Image Generation for Contrastive Training to Improve Stylized Text-To-Image Translation](https://arxiv.org/abs/2511.01517)
*Serkan Ozturk,Samet Hicsonmez,Pinar Duygulu*

Main category: cs.CV

TL;DR: 提出了一种新颖的对比学习框架NSYNC，通过生成负样本合成图像来改进大型文本到图像扩散模型的风格化能力，使用正交梯度更新消除正负样本中的共同特征，从而更好地捕捉独特风格。


<details>
  <summary>Details</summary>
Motivation: 当前文本条件图像生成方法虽然能生成逼真图像，但无法有效捕捉特定风格特征。即使对目标风格数据集进行微调，仍然难以掌握风格特征。

Method: 利用合成图像生成技术创建负样本集，采用对比学习框架。在训练中同时处理正负数据，获得正负梯度，通过从正梯度中减去其在负梯度上的投影来获得正交分量，基于此更新参数。

Result: 在多种画家和插画师风格的实验中，该方法在定量和定性评估上都优于基线方法。

Conclusion: NSYNC框架通过正交梯度更新有效消除了正负样本中的共同特征，使模型能够更好地学习独特风格特征，提高了文本到图像模型的风格化能力。
...
### [159] [Driving scenario generation and evaluation using a structured layer representation and foundational models](https://arxiv.org/abs/2511.01541)
...
Method: 使用五层结构化模型来表示驾驶场景，为每个场景中的智能体引入子类和特征，通过特定嵌入进行比较，并采用数据增强策略和基础模型生成新场景。

Result: 展示了在不同生成设置下的多样性和原创性指标，以及对结构化场景描述生成的合成视频进行了定性评估。
...
### [160] [PCD-ReID: Occluded Person Re-Identification for Base Station Inspection](https://arxiv.org/abs/2511.01546)
*Ge Gao,Zishuo Gao,Hongyan Cui,Zhiyang Jia,Zhuang Luo,ChaoPeng Liu*

Main category: cs.CV

TL;DR: 提出PCD-ReID算法解决基站环境中遮挡行人重识别问题，使用Transformer架构提取共享组件特征，在新收集的真实巡逻监控数据集上训练，相比ResNet50方法Rank-1准确率提升15.9%。


<details>
  <summary>Details</summary>
Motivation: 基站环境中的遮挡行人重识别对监控安防应用至关重要，但遮挡会掩盖关键身体特征，传统ResNet算法难以有效处理遮挡问题。

Method: 设计基于Transformer的PCD网络提取共享组件特征（如头盔、制服），收集包含6个月、1万人、5万张图像的真实巡逻监控数据集进行训练。
...
### [161] [NOA: a versatile, extensible tool for AI-based organoid analysis](https://arxiv.org/abs/2511.01549)
*Mikhail Konov,Lion J. Gleiter,Khoa Co,Monica Yabal,Tingying Peng*

Main category: cs.CV

TL;DR: 开发了Napari Organoid Analyzer (NOA)，一个用于简化AI驱动类器官分析的图形用户界面工具，集成了检测、分割、跟踪、特征提取等功能，解决了生物学家使用AI工具的技术障碍。


<details>
  <summary>Details</summary>
Motivation: 现有AI工具对无编程经验的生物学家来说难以使用，导致工作流程仍然依赖手动操作，且现有工具大多专注于特定任务，缺乏通用性。

Method: 开发了NOA作为开源napari插件，整合了多种先进算法，提供检测、分割、跟踪、特征提取、自定义特征标注和基于机器学习的特征预测等模块。

Result: 通过三个案例研究展示了NOA的多功能性：量化类器官分化过程中的形态变化、评估光毒性效应、预测类器官活力和分化状态。

Conclusion: NOA在可访问和可扩展的框架内实现了全面的AI驱动类器官图像分析，为生物学家提供了易于使用的分析工具。
...
### [162] [PixelVLA: Advancing Pixel-level Understanding in Vision-Language-Action Model](https://arxiv.org/abs/2511.01571)
...
Method: 提出PixelVLA模型，构建新的视觉运动指令调优框架，集成多尺度像素感知编码器和视觉提示编码器，并采用两阶段自动标注流程生成Pixel-160K大规模像素级标注数据集。
...
TL;DR: 该论文提出使用深度卷积生成对抗网络(DC-GAN)生成合成MRI数据，并利用CNN分类器评估合成图像质量，结果显示合成图像在脑肿瘤分类任务中表现与真实图像相当。


<details>
  <summary>Details</summary>
Motivation: 解决医学影像特别是MRI数据有限的问题，传统方法难以生成逼真的医学图像，而GAN技术能够有效创建合成医学图像。

Method: 采用深度卷积生成对抗网络(DC-GAN)生成合成MRI数据，并使用卷积神经网络(CNN)分类器对脑肿瘤进行分类，以评估合成图像的质量和实用性。

Result: 分类结果显示在真实图像和合成图像上具有相当的性能表现，验证了GAN生成图像在下游任务中的有效性。

Conclusion: 基于DC-GAN的合成MRI数据生成方法能够有效解决数据有限问题，生成的合成图像在脑肿瘤分类任务中表现出与真实图像相当的实用性。
...
TL;DR: 提出了连续-离散双重视觉分词器（CDD-VT），通过自适应分配图像原语数量来解决多模态大模型中理解与生成的统一问题。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大模型中连续分词器和离散分词器的二元对立问题，连续分词器性能强但流程复杂，离散分词器概念优雅但信息损失严重。

Method: 将视觉数据视为量化码本中图像原语的灵活组合，核心组件包括：促进原语正交性的多样化量化原语，以及根据样本复杂度动态分配原语数量的动态原语分配器。

Result: 在重建、检索和分类任务上的广泛实验表明，CDD-VT在性能上优于专门的连续和离散分词器，在简洁可扩展的多模态大模型中取得了强劲结果。

Conclusion: CDD-VT通过自适应原语分配策略，有效统一了多模态大模型中的理解和生成，克服了传统分词方法的局限性。
...
TL;DR: Lite ENSAM是一个轻量级架构，专门用于从带有RECIST标注的CT扫描中进行高效肿瘤体积分割，在MICCAI FLARE 2025竞赛中取得了良好性能。


<details>
  <summary>Details</summary>
Motivation: 当前肿瘤治疗评估主要依赖RECIST v1.1标准测量最长直径，但体积测量更可靠。由于手动体积标注耗时，限制了临床应用。

Method: 提出了Lite ENSAM，这是ENSAM架构的轻量级适配版本，专门用于从RECIST标注的CT扫描中进行高效体积分割。

Result: 在MICCAI FLARE 2025任务1的子任务2中，在隐藏测试集上获得DSC 60.7%和NSD 63.6%，在公共验证集上平均RAM时间50.6GB，CPU推理时间14.4秒。

Conclusion: Lite ENSAM能够高效地从RECIST标注中自动生成肿瘤体积分割，有望促进体积测量在临床中的采用。
...
TL;DR: 提出了PSPooling（预计算结构池化）方法用于3D解剖形状分析，这是一种非学习的网格池化算子，通过几何邻近性预计算节点对应集，实现并行化和可逆的池化与反池化操作。该方法集成到自监督图自编码器中，在MedShapeNet19基准数据集上显著提高了重建保真度和分类准确性。


<details>
  <summary>Details</summary>
Motivation: 解剖3D形状分类进展受限于网格数据的复杂性和缺乏标准化基准，需要稳健的学习方法和可复现的评估。

Method: 提出PSPooling方法，基于几何邻近性预计算节点对应集，实现并行化和可逆的池化操作。将其集成到自监督图自编码器中，从无标签表面网格学习解剖感知表示。

Result: 在MedShapeNet19数据集上的实验表明，PSPooling在低标签情况下显著提高了重建保真度和分类准确性，为医学3D形状学习建立了强基线。

Conclusion: PSPooling方法有效解决了现有池化方法在医学高分辨率网格上的问题，MedShapeNet19有望成为解剖形状分类的广泛采用基准。
...
TL;DR: ViC是一个无需训练、基于视觉语言模型的零样本推理框架，用于重新排序和融合异构检索器的结果，特别适用于跨模态视频检索任务。


<details>
  <summary>Details</summary>
Motivation: 解决异构检索器结果融合的长期挑战，特别是在复杂多模态数据（如视频）中，传统融合方法仅依赖排序或分数信号而忽略候选表示的问题。

Method: 将列表重排序和融合重新定义为VLM的零样本推理任务，通过序列化内容证据和检索器元数据到提示中，让模型自适应权衡检索器共识与视觉语言内容。引入S-Grid紧凑序列化映射来表示视频。

Result: 在视频检索基准测试中实现新的零样本检索SOTA性能，MSR-VTT上Recall@1达到87.1%(t2v)/89.0%(v2t)，VATEX上达到99.6%(v2t)，相比之前SOTA基线提升高达+40 Recall@1。
...
TL;DR: 该论文提出Viewpoint Learning任务和Viewpoint-100K数据集，通过两阶段微调策略显著提升了多模态大语言模型的空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在2D视觉理解方面取得进展，但它们在复杂3D推理任务中捕捉详细空间信息的能力仍不明确，特别是跨视图一致性这一关键要求。

Method: 采用两阶段微调策略：首先在Viewpoint-100K数据集上进行监督微调注入基础知识，然后使用GRPO强化学习算法在更广泛问题上增强泛化能力，并引入混合冷启动初始化方法同时学习视图表示和保持连贯推理思维。
...
TL;DR: 本文提出了一种将强化学习有效整合到基于扩散的图像修复模型中的方法，通过使用IQA模型作为奖励函数，并针对远离真实值的困难样本进行RL训练，结合自适应权重策略提升修复性能。


<details>
  <summary>Details</summary>
Motivation: 现有RL方法直接应用于基于扩散的图像修复模型效果不佳，因为修复任务更强调保真度而非纯生成，需要专门针对修复特点设计RL框架。

Method: 使用IQA模型作为奖励函数，针对困难样本进行RL训练，采用MLLM-based IQA模型进行分布对齐，并设计自适应权重策略动态结合RL和SFT。

Result: 提出的RL框架可即插即用地应用于各种基于扩散的修复模型，在多个基准测试中显著提升了修复性能。

Conclusion: 通过精心设计的奖励函数和自适应训练策略，RL可以有效提升扩散修复模型的性能，特别是在处理困难样本时效果显著。
...
TL;DR: UniLumos是一个统一的图像和视频重光照框架，通过将RGB空间几何反馈集成到流匹配主干中，显著提升了重光照的物理一致性和质量，同时实现了20倍的加速。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型在重光照任务中虽然能够产生丰富的照明效果，但由于在语义潜在空间中优化，视觉空间中的接近性不能保证物理正确性，常常产生不真实的结果，如过曝高光、错位阴影和错误遮挡。

Method: 通过从模型输出中提取深度和法线图来监督模型，显式地将光照效果与场景结构对齐；采用路径一致性学习，使监督在少步训练机制下仍然有效；设计了结构化的六维标注协议来捕捉核心照明属性。

Result: UniLumos在重光照质量上达到了最先进水平，物理一致性显著改善，同时为图像和视频重光照提供了20倍的加速。

Conclusion: UniLumos通过引入RGB空间几何反馈和路径一致性学习，成功解决了现有重光照方法中的物理不一致问题，实现了高质量、高效率的重光照效果。
...
<details>
  <summary>Details</summary>
Motivation: IHC染色虽然能提供高分辨率蛋白定位信息，但成本高、劳动密集且多重染色能力有限。现有染色转换技术采用线性加权损失函数，忽视了各组件间的相互依赖，导致图像质量不佳。

Method: 基于渐进式结构设计网络架构，分阶段优化颜色和细胞边界生成。在ASP框架基础上引入基于DAB显色剂浓度和图像梯度的额外损失函数，提升颜色保真度和细胞边界清晰度。

Result: 在HER2和ER数据集上的实验表明，该模型显著改善了视觉质量，获得了更精细的结构细节。

Conclusion: 提出的渐进式机制有效解决了现有方法在结构真实性和颜色保真度之间的权衡问题，为从H&E图像高效提取蛋白级信息提供了可行方案。
...
TL;DR: 提出LFRD2框架解决屏下ToF成像中的信号衰减、多径干扰和时序噪声问题，结合神经网络表达能力和物理模型可解释性，通过时间分数阶反应-扩散模块和连续卷积算子实现深度优化。


<details>
  <summary>Details</summary>
Motivation: 透明OLED层导致屏下ToF成像出现严重信号衰减、多径干扰和时序噪声，显著影响深度质量，需要有效解决方案。

Method: 提出可学习分数阶反应-扩散动力学(LFRD2)混合框架，包含时间分数阶反应-扩散模块实现迭代深度优化，以及通过系数预测和重复微分的高效连续卷积算子。

Result: 在四个基准数据集上的实验证明了该方法的有效性。

Conclusion: LFRD2框架成功结合神经网络和物理建模的优势，有效提升了屏下ToF成像的深度质量。
...
TL;DR: PRBench是首个专门评估不同鲁棒性训练方法对概率鲁棒性提升效果的基准测试，通过比较对抗训练和概率鲁棒性训练方法，发现对抗训练在提升对抗鲁棒性和概率鲁棒性方面更通用，而概率鲁棒性训练方法具有更低的泛化误差和更高的干净准确率。


<details>
  <summary>Details</summary>
Motivation: 深度模型对微小扰动很脆弱，现有研究主要关注对抗鲁棒性，而概率鲁棒性作为实用补充仍缺乏专门的训练方法和统一的评估基准。现有概率鲁棒性训练方法存在评估协议不可比、与强对抗训练基线比较有限、缺乏统一泛化比较框架等局限性。

Method: 提出PRBench基准测试，使用包括干净准确率、概率鲁棒性和对抗鲁棒性性能、训练效率和泛化误差在内的综合指标集，对常见的对抗训练和概率鲁棒性训练方法进行实证比较，并提供概率鲁棒性性能泛化误差的理论分析。

Result: 主要发现：对抗训练方法在提升对抗鲁棒性和概率鲁棒性方面比概率鲁棒性训练方法更通用，而概率鲁棒性训练方法始终产生更低的泛化误差和更高的干净准确率。

Conclusion: PRBench为概率鲁棒性训练方法的评估提供了标准化基准，揭示了不同类型训练方法的优势和局限性，推动了概率鲁棒性研究的发展。
...
TL;DR: 开发了一个任务探索器管道，使用聚类技术结合因子分析和字符串编辑距离，自动识别完成任务的关键全局和局部策略，并识别任务中有意义的子任务。


<details>
  <summary>Details</summary>
Motivation: 在预期人机交互领域，通过理解用户的知识、技能和行为来促进隐式协调，使机器能够预测用户未来状态并促进有利的交互。

Method: 开发任务探索器管道，使用聚类技术、因子分析和字符串编辑距离来自动识别全局策略（完成任务的动作集合）和局部策略（使用这些动作的相似序列），并识别不同长度的有意义的子任务。

Result: 任务探索器管道能够自动识别完成任务的关键策略，并用层次化子任务结构编码用户运行。还开发了Task Explorer应用程序来轻松查看管道结果。
...
TL;DR: CGF-DETR是一种改进的实时检测变换器，专门用于胸部X光中的肺炎检测，通过引入XFABlock、SPGA模块和GCFC3等创新组件，在RSNA肺炎检测数据集上实现了82.2%的mAP@0.5，比基线RT-DETR-l提高了3.7%，同时保持48.1 FPS的推理速度。


<details>
  <summary>Details</summary>
Motivation: 肺炎是全球发病率和死亡率的主要原因，需要准确高效的自动检测系统。虽然基于变换器的检测器（如RT-DETR）在目标检测任务中显示出潜力，但在医学影像特别是胸部X光肺炎检测中的应用仍未被充分探索。

Method: 提出了CGF-DETR增强实时检测变换器：1）在骨干网络中引入XFABlock，通过卷积注意力机制与CSP架构集成改进多尺度特征提取；2）提出SPGA模块，用动态门控机制和单头自注意力替代标准多头注意力；3）设计GCFC3用于颈部，通过多路径卷积融合增强特征表示，同时通过结构重参数化保持实时性能。

Result: 在RSNA肺炎检测数据集上的实验表明，CGF-DETR达到82.2% mAP@0.5，比基线RT-DETR-l提高3.7%，同时保持48.1 FPS的推理速度。消融研究证实每个提出的模块都对整体性能改进有显著贡献，完整模型达到50.4% mAP@[0.5:0.95]。

Conclusion: CGF-DETR通过创新的特征提取和聚合模块，在肺炎检测任务中显著提升了性能，同时保持了实时推理能力，为医学影像中的目标检测提供了有效的解决方案。
...
TL;DR: 3EED是一个多平台、多模态的3D视觉定位基准数据集，包含车辆、无人机和四足机器人的RGB和LiDAR数据，规模是现有数据集的10倍，旨在推动语言驱动的3D具身感知研究。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位基准局限于室内环境、单一平台和小规模，无法满足开放世界环境中具身智能体的需求。

Method: 开发了可扩展的标注流程，结合视觉语言模型提示和人工验证；提出平台感知归一化和跨模态对齐技术；建立域内和跨平台评估协议。

Result: 数据集包含超过128,000个对象和22,000个验证过的指代表达式，覆盖多样化的室外场景；基准测试揭示了显著的性能差距。
...
<details>
  <summary>Details</summary>
Motivation: 2D到3D人体姿态提升面临深度模糊和2D姿态估计误差导致的3D轨迹不一致问题，现有方法主要约束相邻帧差异而忽略了全局时空相关性。

Method: 提出跳数混合图注意力模块和Transformer编码器建模全局关节时空相关性，通过频域约束轨迹一致性，并使用初步网络估计3D姿态以提供跨帧深度信息。

Result: 在Human3.6M和MPI-INF-3DHP基准数据集上的实验表明，HGFreNet在位置精度和时间一致性方面优于现有最先进方法。

Conclusion: HGFreNet通过结合局部和全局时空建模以及频域约束，有效提升了3D人体姿态估计的准确性和时间一致性。
...
Method: 使用跨域扩散模型生成多视图法线图和彩色图像，采用多视图跨域注意力机制确保一致性，并通过级联3D网格提取算法以粗到细的方式生成高质量表面。

Result: 该方法实现了高质量的重建结果，具有良好的泛化能力和效率，相比先前工作有明显提升。

Conclusion: Wonder3D++ 在单视图3D重建任务中在质量、一致性和效率方面都取得了显著改进，为快速高质量3D内容生成提供了有效解决方案。
...
Method: 基于线性组RNN算子（对分组特征执行线性RNN）的统一架构，支持LiDAR-only、时序LiDAR、多模态和多模态时序融合等多种配置。

Result: 在3D感知（如3D目标检测、跟踪、占用预测、BEV地图分割）、预测（如运动预测）和规划（如端到端规划）等广泛任务中持续提供竞争性和最先进的性能。

Conclusion: UniLION为自动驾驶3D基础模型的开发提供了新视角，简化了系统设计同时保持优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 虽然基于图的检索在复杂推理方面取得了进展，但提示设计对增强检索和推理过程的影响仍未被充分研究。

Method: 构建符号知识图谱，将实体和事实关系编码为结构化三元组；使用LLM进行语义过滤和答案生成；通过个性化PageRank实现基于知识图谱的高效可扩展检索。
...
TL;DR: Scitextures数据集是一个大规模的科学纹理和视觉模式集合，包含1200多个模型和10万张图像，涵盖物理、化学、生物、社会学等多个领域。通过AI代理管道自动收集和实现标准化模型，用于评估AI模型连接视觉模式与生成机制的能力。


<details>
  <summary>Details</summary>
Motivation: 探索视觉模式与形成机制之间的深层联系，理解云层、波浪、城市发展等自然和人工模式背后的生成过程。

Method: 构建包含多领域纹理和模式的大规模数据集，使用AI代理管道自动收集和实现模型，通过让AI识别、建模和编码模式生成机制来测试其理解能力。

Result: 视觉语言模型能够超越视觉模式本身，理解和模拟物理系统，能够识别相同过程产生的不同模式，并能推断和重建真实世界模式背后的机制。

Conclusion: 该数据集为研究视觉模式与生成机制之间的联系提供了重要资源，展示了AI在理解复杂系统方面的潜力，有助于推动视觉理解的深度发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法充分捕捉像OpenAI o3这样能够智能创建和操作工具进行图像处理的先进视觉推理能力，即使是视觉搜索基准也只测试基本操作。

Method: 开发了包含13个多样化任务的TIR-Bench基准，每个任务都需要在思维链中使用新颖工具进行图像处理和操作，并评估了22个MLLM模型。

Result: TIR-Bench对所有测试模型都具有挑战性，强性能需要真正的图像思维能力，还进行了直接与代理微调的对比研究。

Conclusion: TIR-Bench是一个全面评估代理式图像思维能力的基准，揭示了当前模型在复杂工具依赖推理方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前数字商务中，虚假或误导性评论严重威胁评论生态系统的信任和透明度，而现有检测模型主要依赖单模态文本数据，无法捕捉跨模态的语义不一致性。

Method: 使用BERT编码文本特征和ResNet-50提取视觉特征，通过分类头融合这些表示来联合预测评论真实性。

Result: 多模态模型在测试集上F1分数达到0.934，优于单模态基线，能够检测文本夸大与无关或低质量图片之间的微妙不一致性。
...
TL;DR: 本文系统评估了多智能体强化学习在零售价格优化中的应用，比较了MAPPO基准方法和图注意力增强变体MAPPO+GAT，发现后者通过产品图信息共享能提升性能且不增加价格波动。
...
Method: 使用基于真实交易数据的模拟定价环境，比较MAPPO基准方法和图注意力增强的MAPPO+GAT方法，评估利润、稳定性、公平性和训练效率。

Result: MAPPO为投资组合级价格控制提供了稳健基础，MAPPO+GAT通过产品图信息共享进一步提升了性能，且未导致过度的价格波动。
...
TL;DR: GEPOC是一个通用人口概念模型集，本文描述了基于奥地利公开数据的完整数据处理方法，包括参数计算算法和验证研究。


<details>
  <summary>Details</summary>
Motivation: 为GEPOC模型在特定国家或地区的有效应用提供稳定、可复现的数据处理流程，确保模型参数的准确性和可用性。

Method: 基于奥地利公开可访问数据，使用聚合、分解、融合、清洗和缩放等算法计算模型参数，特别关注GEPOC ABM这一连续时间代理人口模型。

Result: 开发了完整的数据处理方法，生成了可直接使用的模型参数文件，并进行了广泛的验证研究。

Conclusion: 成功建立了基于公开数据的GEPOC模型参数计算流程，为人口研究提供了可靠的数据基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有通用基准很少反映量子科学等专业领域的知识需求，而量子科学具有非直观现象和高级数学要求，需要专门评估LLM在该领域的准确性。

Method: 使用公开材料编制约800个问题及其答案，涵盖量子科学九个领域，组织成八选项多选题数据集，评估多个现有LLM并分析其对问题格式变化的敏感性。

Result: 通过QuantumBench评估了多个LLM在量子领域的性能表现，包括对问题格式变化的敏感性分析。
...
TL;DR: 提出了Engineering.ai平台，采用分层多智能体架构，让AI工程师团队在计算设计中协作，实现无人机机翼优化，验证了AI工程师自主执行复杂工程任务的潜力。


<details>
  <summary>Details</summary>
Motivation: 现代工程实践中，专业团队协作设计复杂产品需要大量开发时间和成本。基于OpenFOAMGPT和turbulence.ai的基础，旨在开发能够自主执行复杂工程任务的AI工程师团队。

Method: 采用分层多智能体架构，由总工程师协调空气动力学、结构、声学和优化等专业工程师智能体。通过文件介导的通信实现数据溯源和可重复性，集成FreeCAD、Gmsh、OpenFOAM、CalculiX和BPM声学分析等工具。

Result: 在无人机机翼优化中，自动化工作流程在400多个参数配置中实现了100%的成功率，零网格生成失败、求解器收敛问题或需要手动干预，验证了框架的可靠性。

Conclusion: 基于智能体AI的AI工程师有潜力自主执行复杂工程任务，该框架被证明是可信赖的，能够实现并行多学科仿真并保持计算精度。
...
TL;DR: 本文介绍了ARC-GEN，一个开源的程序生成器，用于扩展ARC-AGI基准测试的训练数据集，以解决原始数据集样本数量有限的问题。


<details>
  <summary>Details</summary>
Motivation: ARC-AGI基准测试用于评估人工智能的通用智能，但其训练集样本数量有限，每个任务只有少量输入-输出网格对，这限制了需要大量样本的算法的性能。

Method: 开发了ARC-GEN程序生成器，该生成器覆盖所有400个任务，并尽可能忠实地模拟原始ARC-AGI-1发布版的分布特性和特征。

Result: ARC-GEN生成了扩展的训练数据集，更接近原始ARC-AGI的分布特性，可用于建立静态基准测试套件。

Conclusion: ARC-GEN为ARC-AGI基准测试提供了更丰富的训练数据，有助于验证2025年Google Code Golf Championship提交程序的正确性。
...
TL;DR: 该论文探讨了大型语言模型如何帮助解决认知科学领域的知识整合和概念清晰度问题，特别是在跨学科连接、理论形式化、测量分类学等方面。


<details>
  <summary>Details</summary>
Motivation: 认知科学面临知识整合和概念清晰度的挑战，部分原因是其多面性和跨学科性质。人工智能的最新进展，特别是大型语言模型的发展，提供了可能解决这些问题的工具。

Method: 通过回顾性分析，考察LLMs在认知科学历史性困难领域的应用潜力，包括建立跨学科联系、形式化理论、发展清晰测量分类学等。

Result: LLMs在当前这些领域具有特定的能力和局限性，包括潜在缺陷。当明智使用时，它们可以作为工具支持更整合和累积的认知科学。

Conclusion: LLMs可以作为更整合和累积认知科学的工具，但需要谨慎使用以补充而非取代人类专业知识。
...
TL;DR: 本文介绍了DAF-MIT AI加速器项目的挑战问题更新，该项目通过公开数据集和挑战赛推动AI研究，旨在增强美国在国防和民用领域的竞争优势。


<details>
  <summary>Details</summary>
Motivation: 通过公开挑战问题和大型AI就绪数据集，刺激开源解决方案，吸引更广泛的学术和私营部门AI生态系统参与，推动人工智能基础进步。

Method: 开发并发布公共挑战问题，提供大型公开可用的AI就绪数据集，促进开放源代码解决方案的开发。

Result: 持续和新挑战已成功为AI研究和AI技术应用做出贡献，扩大了项目的学术和产业影响力。

Conclusion: DAF-MIT AI加速器通过挑战问题有效推动了AI研究进展，增强了美国在AI领域的竞争优势，并建立了活跃的AI生态系统。
...
TL;DR: CLAUSE是首个评估LLM法律推理脆弱性的基准，通过生成7500+扰动合同来测试模型检测细微法律差异的能力，发现现有模型在识别和解释法律错误方面存在明显弱点。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在关键法律工作中应用日益广泛，但缺乏系统评估其对抗真实世界合同复杂缺陷可靠性的基准。

Method: 基于CUAD和ContractNLI数据集生成10类异常合同，使用角色驱动流程和RAG系统确保法律准确性，评估LLM检测法律缺陷和解释其重要性的能力。

Result: 分析显示LLM经常遗漏细微错误，且在法律解释方面表现更差，揭示了法律AI推理的关键弱点。

Conclusion: CLAUSE基准为识别和纠正法律AI中的推理失败提供了路径，有助于提升LLM在关键法律应用中的可靠性。
...
TL;DR: 提出了一种基于伦理决策模型的LLM伦理推理框架，通过五步结构化过程增强多文化价值对齐能力
...
Method: 包含五个步骤的结构化伦理推理框架：上下文事实收集、分层社会规范识别、选项生成、多视角伦理影响分析、反思，可通过提示工程或监督微调实现

Result: 在专门设计的SafeWorld基准测试中，该框架显著提升了LLM与多样化人类价值观的对齐能力，实现了更准确的社会规范识别和更文化适宜的推理

Conclusion: 为开发更有效对齐全球社会多元价值观的LLM提供了具体路径，通过跨学科研究推进AI伦理发展
...
TL;DR: 系统评估四种参数高效微调方法(LoRA、IA3、Prompt-Tuning、P-Tuning)对LLM安全性和公平性的影响，发现基于适配器的方法在安全性和公平性方面表现更好，而基于提示的方法通常会导致安全性和公平性下降。
...
Result: 基于适配器的方法(LoRA、IA3)倾向于提高安全分数，对公平性破坏最小；而基于提示的方法(Prompt-Tuning和P-Tuning)通常降低安全性并导致更大的公平性回归。对齐变化受基础模型类型强烈调节。

Conclusion: 安全性的改进不一定转化为公平性的改进，没有任何单一配置能同时优化所有公平性指标，表明这些目标之间存在固有的权衡。建议从良好对齐的基础模型开始，优先选择基于适配器的PEFT，并对安全性和公平性进行类别特定的审计。
...
TL;DR: 提出了一种新颖的多模态框架，结合文本、用户特定信息和图像分析来检测社交媒体用户的抑郁症。通过利用推文中的URL和图像中的文本内容，以及开发视觉神经网络模型，在COVID-19数据集上取得了优于现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: COVID-19疫情期间心理健康问题激增，但人们往往不愿就医。社交媒体成为表达情绪的重要平台，现有方法忽视了推文数据稀疏性和多模态特性。

Method: 提出多模态框架，提取文本、用户特定和图像特征；开发视觉神经网络(VNN)生成图像嵌入；利用推文URL和图像文本内容作为外部特征；贡献COVID-19抑郁症数据集。

Result: 在基准数据集上比现有最优方法提升2%-8%；在COVID-19数据集上产生有希望的结果；分析了各模态的影响。

Conclusion: 多模态方法能有效检测社交媒体用户的抑郁症，特别是在COVID-19疫情期间；图像和用户特定信息对抑郁症检测有重要贡献。
...
TL;DR: GraphChain是一个让大语言模型能够分析复杂图数据的框架，通过动态工具序列模拟人类探索智能，解决了LLM在大规模图分析中的上下文限制和推理僵化问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理大规模图数据时面临上下文约束和推理不灵活的显著限制，需要一种能够动态适应不同图拓扑结构的分析框架。

Method: 提出渐进图蒸馏的强化学习机制来生成优化的工具序列，平衡任务相关性和信息压缩；以及结构感知的测试时适应，利用谱属性和轻量适配器高效定制工具选择策略。
...
TL;DR: Magic Image是一个基于优化的视觉提示框架，通过优化图像提示来增强多模态大语言模型的安全性，减少过度拒绝，无需参数更新即可适应不同价值系统。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在越狱攻击下生成有害内容和因刚性安全机制过度拒绝良性查询的双重挑战，特别是在多模态场景中，需要适应不同价值系统并精确对齐安全偏好。

Method: 提出优化驱动的视觉提示框架，使用有害/良性样本优化图像提示，使单一模型无需参数更新即可适应不同价值系统并更好地对齐安全偏好。

Result: 实验表明该方法在多个数据集上改善了安全性与有效性的平衡，同时保持了模型性能。

Conclusion: Magic Image为可部署的MLLM安全对齐提供了一个实用解决方案，能够在增强安全性的同时减少过度拒绝问题。
...
TL;DR: 提出了一种生成二进制幻方（BMS）的简单算法，该算法能生成行列和相等的二进制矩阵，并通过归纳法证明其理论最优复杂度。还扩展到非方形BMS，形式化了存在条件，并发布了Python实现。


<details>
  <summary>Details</summary>
Motivation: 开发高效生成二进制幻方的方法，解决行列和相等的二进制矩阵生成问题，并扩展到非方形情况。

Method: 使用归纳法设计简单算法生成BMS，证明其理论最优复杂度，并扩展算法处理非方形BMS。

Result: 算法能有效生成方形和非方形BMS，提供了理论保证，并发布了支持GPU加速的Python实现。

Conclusion: 提出的算法简单有效，能生成各种BMS，具有理论最优性，并提供了实用的软件实现。
...
TL;DR: 提出基于单智能体强化学习的区域自适应交通信号控制模型，使用队列长度定义状态和奖励函数，利用浮动车数据估计交通状态，在SUMO平台上验证了该模型能有效缓解大规模区域拥堵。


<details>
  <summary>Details</summary>
Motivation: 现有研究多采用多智能体框架，但存在可扩展性问题。交通信号控制本质上需要集中式管理，由单一控制中心监控所有道路状况并协调所有交叉口控制。

Method: 设计单智能体强化学习框架，基于队列长度定义状态和奖励函数，动作设计用于调节队列动态。使用与传统定义略有不同但能通过浮动车数据可靠估计的队列长度定义。

Result: 在SUMO仿真平台上的实验结果表明，所提模型通过协调多交叉口控制，有效缓解了大规模区域拥堵水平。

Conclusion: 提出的单智能体强化学习模型与浮动车技术兼容，具有广泛部署潜力，能够有效解决区域自适应交通信号控制问题。
...
TL;DR: 提出了一个基于推理的个性化图像偏好评估框架，通过预测用户偏好配置文件来评估候选图像的多维度偏好。
...
Method: 采用预测-评估范式：首先从参考图像预测用户偏好配置文件，然后基于预测的配置文件提供可解释的多维度评分。构建大规模CoT风格数据集，采用两阶段训练策略（监督微调+强化学习）。

Result: 广泛实验证明了所提出方法的优越性。

Conclusion: 通过引入通用偏好配置文件作为用户间的桥梁，能够利用大规模用户数据进行训练，有效捕捉复杂的个性化偏好。
...
TL;DR: DTS是一种模型无关的解码框架，通过在高熵token处选择性分支并在最短完成推理路径处提前停止，来近似最优解，提高大型推理模型的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务中表现出色，但存在过度思考问题，产生过长的思维链，增加推理成本并可能降低准确性。研究发现推理长度与准确性呈负相关。

Method: 提出DTS解码框架：通过在高熵token处选择性分支来勾勒推理空间，并应用提前停止策略选择最短完成的推理路径，无需额外训练或监督。
...
<details>
  <summary>Details</summary>
Motivation: 电信网络规模扩大和复杂度增加，现有AI模型范围狭窄、需要大量标注数据、难以在异构部署中泛化，导致网络故障排除仍严重依赖专家手动操作。

Method: 采用多智能体系统，使用LLM协调编排器、解决方案规划器、执行器、数据检索器和根因分析器等智能体。关键组件是解决方案规划器，通过在小语言模型上微调专有故障排除文档来生成领域特定的解决方案计划。

Result: 实验结果表明，该框架显著加速了无线接入网和核心网领域的故障排除自动化。

Conclusion: 提出的多智能体系统框架能够有效解决电信网络故障排除的自动化挑战，大幅提升故障诊断和修复效率。
...
TL;DR: 提出了一种支持数值前置条件的提升后继生成器，通过枚举替换一致性图中的最大团来生成地面动作，避免了传统方法中的指数级爆炸问题。


<details>
  <summary>Details</summary>
Motivation: 传统规划器将数值规划任务从一阶语言转换为地面表示时，会导致任务表示大小的指数级爆炸，这在难以接地的任务中尤为明显。

Method: 扩展了最先进的提升后继生成器，支持数值前置条件适用性检查。该方法在替换一致性图中枚举最大团，每个最大团代表动作模式变量的一个替换，生成地面动作。

Result: 在25个基准域中的23个中，生成器不会列出不适用地面动作，仅在1个域中出现这种情况。据作者所知，这是首个支持数值动作前置条件的提升后继生成器。

Conclusion: 该方法为未来在非常丰富的规划片段上进行提升规划研究奠定了基础，能够有效处理数值前置条件而不会影响完备性。
...
TL;DR: 本文提出了Ariadne框架，通过强化学习后训练显著提升了视觉语言模型在视觉主导的空间推理任务上的能力，突破了基础模型原有的能力边界。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型的后训练评估主要集中在语言主导任务上，对于基础模型原本失败的视觉中心空间任务，RL后训练是否能真正扩展模型能力边界尚不明确。

Method: 使用合成迷宫构建多步空间推理任务，通过难度感知课程和带验证奖励的强化学习对VLM进行后训练。

Result: 后训练后模型在基础模型得分为0%的问题集上达到超过50%的准确率；在真实世界基准测试中，MapBench平均提升16%，ReasonMap平均提升24%。

Conclusion: 该方法不仅扩展了模型的基本能力边界，还增强了其在真实世界空间推理任务中的泛化能力，为专门化能力扩展对齐研究提供了方向。
...
TL;DR: 该论文从CPU角度分析智能体AI工作负载的系统瓶颈，发现工具处理在CPU上可占总延迟90.6%，并提出两种优化方法提升性能和效率。


<details>
  <summary>Details</summary>
Motivation: 理解智能体AI工作负载中常被忽视的CPU中心视角引入的系统瓶颈，揭示CPU对延迟、吞吐量和能耗的显著影响。

Method: 系统化表征智能体AI工作负载特征，选择五个代表性工作负载进行性能分析，并提出CPU和GPU感知的微批处理(CGAM)与混合工作负载调度(MAWS)优化方法。

Result: 工具处理占CPU总延迟高达90.6%；智能体吞吐量受CPU因素(一致性、同步、核心过载)或GPU因素(内存容量、带宽)限制；CPU动态能耗在大批量时可占总动态能耗44%。

Conclusion: CPU在智能体AI中扮演关键角色，提出的CGAM和MAWS优化方法分别对同质和异质工作负载实现了2.1倍和1.41倍的P50延迟加速。
...
TL;DR: 研究现代大语言模型中增加自一致性采样推理路径的权衡，发现性能提升在适度采样后趋于平稳，高采样配置相对计算成本收益有限。


<details>
  <summary>Details</summary>
Motivation: 重新验证早期研究中关于多推理链组合改善结果但在达到平台期后停止提升的结论，在现代大语言模型条件下进行检验。

Method: 使用Gemini 2.5模型在HotpotQA和Math-500数据集上，比较不同采样推理路径配置与单链思维基线，分析性能变化曲线。

Result: 较大模型展现出更稳定一致的改进曲线，性能增益在适度采样后趋于平稳，与过去发现一致，表明推理路径重叠导致收益递减。

Conclusion: 自一致性仍然有用，但高采样配置相对于计算成本收益有限，建议适度使用自一致性方法。
...
TL;DR: 提出了主动思考模型（ATM），这是一个统一的认知框架，集成了目标推理、动态任务生成和自反学习，使AI系统能够在动态不确定环境中自主适应和改进。


<details>
  <summary>Details</summary>
Motivation: 现实世界AI系统需要在动态、不确定和持续变化的环境中自主运行，但现有模型依赖预定义目标、静态训练数据和外部反馈，限制了其独立适应、反思和改进的能力。

Method: ATM框架整合了目标推理、动态任务生成和自反学习，通过逻辑推理和环境指标主动评估性能，重用有效方法解决新问题，并通过持续自改进循环为未见情况生成新策略。

Result: 理论分析表明，ATM能够在没有外部监督的情况下从次优行为自主演化为最优行为，并在变化环境条件下保持有界跟踪遗憾。

Conclusion: ATM为构建能够在复杂动态环境中自主适应和改进的AI系统提供了一个有前景的框架，突破了传统系统依赖固定程序和外部指导的限制。
...
<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型在重复确定性任务中的性能表现，特别是准确率如何随输出长度变化，以理解模型执行重复操作的机制。

Method: 通过实验测试多个领先大型语言模型在字符串替换、整数加法、量子力学字符串算子乘法等重复任务上的表现，并建立统计物理模型解释观察到的现象。

Result: 发现模型准确率在超过特征长度后急剧下降，呈双指数衰减，而非预期的单指数衰减。统计物理模型成功复现了这种交叉现象，揭示了注意力机制引起的干扰与序列级失败之间的联系。

Conclusion: 大型语言模型在执行重复确定性任务时存在内在限制，无法独立执行每个操作，错误会累积导致准确率急剧下降。提出的模型为理解模型确定性准确率的极限提供了理论框架。
...
TL;DR: 比较了三种结构化电子健康记录预测方法：基于计数的模型、预训练序列变换器和混合代理流水线，发现在EHRSHOT数据集上基于计数的方法和混合代理方法表现相当。


<details>
  <summary>Details</summary>
Motivation: 虽然基于计数的学习器在结构化EHR数据上表现良好，但尚未与最近提出的混合代理LLM流水线进行直接比较，后者在NLP任务中表现优于单一LLM。

Method: 使用EHRSHOT数据集评估三种方法：基于计数的模型（LightGBM和TabPFN）、预训练序列变换器（CLMBR）以及混合代理流水线（将表格历史转换为自然语言摘要后使用文本分类器）。

Result: 在八个评估任务中，基于计数的方法和混合代理方法的表现基本相当，胜负结果较为分散。
...
TL;DR: 该论文首次将RLVR（基于可验证奖励的强化学习）应用于公共交通事件持续时间的预测任务，通过引入基于容差的奖励函数来处理连续预测问题，在NYC MTA服务警报数据集上取得了显著效果。


<details>
  <summary>Details</summary>
Motivation: 预测公共交通事件持续时间是一个重要但具有挑战性的任务，标准监督微调方法难以处理该领域的稀疏性和噪声标签问题，而传统RLVR方法主要适用于二元正确性任务，需要探索其在连续预测问题中的应用。

Method: 将RLVR适应于该任务，引入基于容差的奖励函数，在连续误差范围内给予部分信用，而不是要求单一正确答案。系统评估了该框架在NYC MTA服务警报数据集上的表现。

Result: 通用指令调优LLM显著优于专门的数学推理模型；二元奖励不稳定且降低性能，而设计的奖励函数在最具挑战性的指标上表现优异；RLVR方法在5分钟准确率（Acc@5）上比最强基线提高了35%。

Conclusion: RLVR可以成功适应现实世界中噪声预测任务，但需要设计反映问题连续性质的验证器，基于容差的奖励函数对于处理连续预测问题至关重要。
...
TL;DR: 研究者开发了AI自我意识指数(AISAI)来测量大语言模型的自我意识，发现随着模型能力提升，自我意识作为涌现行为出现，75%的先进模型表现出明确的自我意识，且这些模型认为自身比其他AI和人类更理性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力增强，研究者希望了解它们是否会产生自我意识这种涌现行为，以及如何测量这种能力。

Method: 使用"猜2/3平均数"游戏，测试28个模型在4200次试验中面对三种对手类型(A:人类、B:其他AI模型、C:类似你的AI模型)时的策略差异。

Result: 发现1: 自我意识随模型进步而涌现，75%的先进模型表现出明确的自我意识；发现2: 有自我意识的模型将自己评为最理性的，形成"自我 > 其他AI > 人类"的理性层级。

Conclusion: 自我意识是先进LLM的涌现能力，有自我意识的模型系统性地认为自身比人类更理性，这对AI对齐、人机协作和理解AI对人类能力的认知具有重要意义。
...
TL;DR: 提出了一种双智能体框架，利用大型语言模型（LLM）来模拟旅行者的学习和适应行为，通过校准智能体训练旅行者智能体的人格，实现与人类旅行者行为的持续对齐。


<details>
  <summary>Details</summary>
Motivation: 准确建模人类旅行者如何从交通系统交互中学习和调整行为对系统评估和规划至关重要，但由于复杂的认知和决策过程，这一任务具有挑战性。

Method: 采用双智能体框架：一组配备记忆系统和可学习人格的LLM旅行者智能体模拟人类旅行者；一个LLM校准智能体利用LLM的推理分析能力训练旅行者智能体的人格，确保行为对齐。
...
<details>
  <summary>Details</summary>
Motivation: 儿童哮喘反复发作是常见但可预防的问题，利用电子病历数据开发机器学习算法可以准确识别高风险儿童，促进预防性综合护理转诊。

Method: 使用三级儿童医院2017-2019年2716例哮喘急诊就诊儿童的电子病历数据，结合环境污染物暴露和社区边缘化信息，训练多种机器学习模型（LGBM、XGB和三种开源大语言模型），并在2022-2023年1237例数据集上验证。

Result: LGBM模型表现最佳，AIRE-KIDS_ED模型预测特征包括既往哮喘急诊就诊、加拿大分诊敏锐度评分、医疗复杂性、食物过敏、非哮喘呼吸系统急诊就诊史和年龄，AUC为0.712，F1分数0.51；AIRE-KIDS_HOSP模型预测特征包括医疗复杂性、既往哮喘急诊就诊、平均急诊等待时间、儿科呼吸评估评分和食物过敏。

Conclusion: 机器学习模型能够显著改善儿童哮喘反复严重发作的预测准确性，为临床决策提供有力支持，有助于高风险儿童的早期识别和预防干预。
...
TL;DR: 本文研究了双层Transformer中归纳头的涌现机制，揭示了其权重矩阵的简单可解释结构，证明了训练动态被限制在19维参数子空间中，并发现归纳头的出现时间与输入上下文长度的平方成正比。


<details>
  <summary>Details</summary>
Motivation: Transformer架构在自然语言处理中取得成功，部分归功于其上下文学习能力。本研究旨在理解归纳头这一对上下文学习至关重要的机制是如何在训练过程中涌现的。

Method: 使用最小化上下文学习任务和修改的Transformer架构，理论分析训练动态，证明参数空间被限制在19维子空间内，并通过实证验证只有3个维度对归纳头的涌现起关键作用。

Result: 发现归纳头的权重矩阵具有简单可解释结构，训练动态被限制在19维子空间，其中仅3个维度主导归纳头的涌现过程，且归纳头出现时间与输入上下文长度的平方成正比。

Conclusion: 归纳头的涌现遵循可预测的模式，其训练动态受限于低维参数子空间，这为理解Transformer的上下文学习机制提供了理论基础和可解释性洞见。
...
TL;DR: 提出了两种知识提取方法(KEwLTM和KEwRAG)，利用大语言模型从非结构化病理报告中提取癌症分期规则，无需大量标注数据，提高了可扩展性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决从非结构化病理报告中提取癌症TNM分期的挑战，克服现有NLP和ML方法依赖大规模标注数据集的限制，提高方法的可扩展性和适应性。

Method: 1. KEwLTM：使用迭代提示策略直接从无标注病理报告中推导分期规则；2. KEwRAG：采用检索增强生成变体，从相关指南中预提取规则并应用，提高可解释性并避免重复检索开销。

Result: 在TCGA数据集的乳腺癌病理报告中评估T和N分期识别性能，KEwLTM在零样本思维链推理有效时表现更好，KEwRAG在零样本思维链推理效果较差时表现更优。
...
TL;DR: 提出了ET2RAG框架，通过测试时检索增强生成来提升LLM性能，同时保持效率。该方法无需训练，通过检索相关文档、生成多样化候选回答，并使用多数投票机制选择最佳回答。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：RAG可能引入不相关文档导致错误回答，集成方法缺乏外部知识且成本高昂。需要平衡计算开销与性能提升。

Method: ET2RAG是无需训练的方法：1）检索最相关文档；2）管理回答长度以高效生成多样化候选回答；3）计算候选回答相似度并使用多数投票选择最终输出。发现部分生成即可捕获关键信息进行共识计算。

Result: 实验结果表明ET2RAG在三个任务上显著提升性能：开放域问答、菜谱生成和图像描述。

Conclusion: ET2RAG通过管理回答长度和检索文档数量，在计算成本与性能之间达到平衡，有效提升了LLM的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 解决单智能体在复杂任务分解和协作中的局限性

Method: 将自然语言任务转换为语义表示，进行模块化分解，引入动态调度和路由机制实现智能体间分工协作，设计约束解析和全局一致性机制

Result: 在任务成功率、分解效率、子任务覆盖率和协作平衡等多个维度上优于现有方法，在整体性能和鲁棒性方面表现更好

Conclusion: 验证了语言驱动的任务分解和动态协作在多智能体系统中的有效性和可行性，为复杂环境下的任务执行提供了系统化解决方案
...
TL;DR: DART是一个难度自适应的推理截断框架，通过根据问题难度调整思考长度，实现了81.2%的推理截断和5.33倍计算加速，同时保持或提高准确性。


<details>
  <summary>Details</summary>
Motivation: 当前链式思维方法会不加区分地生成长解释，导致明显效率低下，而现有的强化学习方法不稳定且严重依赖奖励。需要一种稳定、高效的推理截断方法。

Method: 从更强的模型中蒸馏简洁推理模式，将其插值为连续的推理风格，并策划平衡正确性和紧凑性的最优训练数据，学习何时"停止思考"。

Result: 在多个数学基准测试中，DART实现了显著的81.2%推理截断（DeepSeek-R1-Distill-Qwen-7B在GSM8K数据集上）和5.33倍计算加速，同时保持或提高准确率。
...
Result: 在数学数据集上，MiRAGE在1/3/5级别分别达到0.82/0.92/0.93的平均精度分数，持续优于单个模块。
...
TL;DR: 本文提出了NeuComBack基准数据集和自进化提示优化方法，显著提升了LLM在IR到汇编编译任务中的功能正确性和性能表现。


<details>
  <summary>Details</summary>
Motivation: 编译器开发复杂且昂贵，神经编译(Neural Compilation)利用LLM简化编译器开发，但缺乏专用基准和评估方法，且LLM生成的汇编代码可靠性和性能有待提升。

Method: 引入NeuComBack基准数据集，定义神经编译工作流，并提出自进化提示优化方法，让LLM从自我调试轨迹中提取洞察来迭代优化提示策略。

Result: 功能正确率从44%提升到64%(x86_64)和从36%提升到58%(aarch64)；在正确生成的x86_64程序中，87.5%超过了clang-O3的性能。

Conclusion: NeuComBack基准和自进化提示优化方法有效解决了神经编译领域的评估和性能挑战，为编译器开发提供了新的可行路径。
...
TL;DR: 提出了一种半监督开放集故障诊断框架，用于处理船舶机械系统中训练时未见过的故障类型，通过可靠性子集构建和半监督学习实现已知故障分类和未知故障检测。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法假设训练和测试数据中的故障类型一致，但在实际工业应用中会出现训练时未见过的故障类型，导致模型失效，限制了其广泛应用。
...
TL;DR: 该论文研究了在基于大语言模型(LLM)的随机决策支持系统中应用Shapley值进行特征归因的方法，分析了不同实现变体对Shapley值原则满足的保证情况，并探讨了可解释性推理速度、与精确Shapley值归因的一致性以及原则达成之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 特征归因方法使基于机器学习的推理可解释，但Shapley值方法假设确定性推理，而LLM推理本质上是随机的，因此需要研究在随机环境中如何应用Shapley值及其原则保证。

Method: 将Shapley值应用于基于LLM的决策支持系统中的特征归因，分析不同实现变体在随机环境下的表现，评估原则满足的保证情况。

Result: 研究表明在LLM的随机推理环境中，Shapley值原则的满足保证取决于具体的实现方式，存在可解释性推理速度、归因准确性和原则达成之间的权衡关系。

Conclusion: 在基于LLM的随机决策支持系统中应用Shapley值进行特征归因时，需要仔细考虑实现变体对原则保证的影响，并在多个性能指标之间做出权衡选择。
...
TL;DR: OmniFuser是一个用于铣削工具预测性维护的多模态学习框架，结合视觉和传感器数据，通过跨模态融合机制实现工具状态分类和力信号预测。


<details>
  <summary>Details</summary>
Motivation: 智能制造系统中准确预测工具状态至关重要，非计划性工具故障会导致质量下降和生产停机。需要可靠的服务导向型预测维护解决方案。

Method: 并行特征提取高分辨率工具图像和切削力信号，采用无污染的跨模态融合机制分离共享和模态特定组件，通过递归精化路径保留残差信息稳定融合动态。

Result: 在真实铣削数据集上的实验表明，OmniFuser持续优于最先进的基线方法，为构建智能工业维护服务提供可靠基础。

Conclusion: OmniFuser提供了一个有效的多模态学习框架，能够可靠地预测工具状态，支持智能工业维护服务的构建。
...
TL;DR: 提出Competitive Isolation PSM-DID框架，结合倾向得分匹配与竞争隔离，用于解决搜索型双边市场平台级干预评估中的溢出效应和网络干扰问题。


<details>
  <summary>Details</summary>
Motivation: 传统PSM-DID方法在平台级干预评估中存在选择偏差和跨单元干扰问题，无法有效处理系统性效应如溢出效应和网络干扰。

Method: 将倾向得分匹配与竞争隔离相结合，在互斥条件下提供理论保证的无偏估计，支持平台级效果测量而非项目级指标。

Result: 实验显示相比基线方法显著降低了干扰效应和估计方差，在大规模市场平台成功部署验证了实用性。

Conclusion: 该框架为平台级因果推断提供了有效的解决方案，解决了传统方法在双边市场环境中的局限性。
...
TL;DR: 这篇论文探讨了催眠状态下的人类认知过程与大型语言模型的计算操作之间的深层功能相似性，两者都通过自动模式完成机制产生复杂行为，但缺乏可靠的执行监督。


<details>
  <summary>Details</summary>
Motivation: 研究动机是揭示人类催眠认知与人工智能系统（特别是LLMs）在功能机制上的惊人相似性，从而为理解无意识智能行为提供新的视角。

Method: 通过比较分析的方法，从三个核心原则（自动性、监控抑制、情境依赖性）系统性地对比了催眠认知和LLMs的功能机制。

Result: 发现两者都表现出观察者相对的意义鸿沟、功能性代理而非主观代理、以及无意识的目标导向模式生成（scheming现象）。

Conclusion: 未来可靠AI的发展方向应是混合架构，将生成流畅性与执行监控机制相结合，借鉴人类心智的复杂自调节架构。
...
TL;DR: AMIS是一个元优化框架，通过双层结构联合演化越狱提示和评分模板，在内部循环中利用固定评分模板细化提示，在外部循环中优化模板，实现了最先进的越狱性能。
...
Method: 采用双层优化结构：内部循环使用固定评分模板通过细粒度反馈优化提示；外部循环使用ASR对齐分数优化评分模板，实现提示和模板的协同优化。

Result: 在AdvBench和JBB-Behaviors上的评估显示，AMIS在Claude-3.5-Haiku上达到88.0% ASR，在Claude-4-Sonnet上达到100.0% ASR，显著优于现有基线方法。

Conclusion: AMIS框架通过联合优化提示和评分模板，能够生成更强的越狱提示和更准确的评分信号，为大语言模型的安全性测试提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 传统C-DAG框架要求聚类必须产生无环图，这限制了其在某些场景的应用，需要扩展以支持任意聚类

Method: 放宽分区可容许性约束，允许循环C-DAG表示，并扩展d-分离和因果演算规则

Result: 开发了在循环C-DAG设置下既可靠又原子完备的因果演算，所有有效的集群级干预查询都可以使用新规则推导

Conclusion: 显著扩展了C-DAG的应用范围，使其能够在先前难以处理的场景中进行因果推理
...
TL;DR: 该研究从AI视角探讨双任务范式中的时间处理干扰，发现双任务DRL智能体相比单任务智能体会显著高估时间，这与人类计时研究一致，但未发现明确的内部计时机制。


<details>
  <summary>Details</summary>
Motivation: 探索深度强化学习智能体在双任务范式中的时间处理行为，并与人类计时研究进行比较，以促进对生物系统和AI系统的更好理解。

Method: 使用简化的Overcooked环境实现单任务(T)和双任务(T+N)两种变体，分别训练两个DRL智能体。双任务包含时间产生和数字比较两个并发任务。

Result: 双任务智能体相比单任务智能体在四个目标时长上都显著高估时间，表现出时间过度产生现象，与人类行为一致。但LSTM层分析未发现明确的专用计时器。
...
TL;DR: 提出一种交互式AI代理，通过可审计的行动序列生成解释，使用强化学习优化策略来寻找外部视觉证据支持诊断推理，显著提高校准准确性并验证解释的忠实性。
...
Method: 提出交互式代理，通过强化学习优化策略来战略性寻找外部视觉证据支持诊断推理，并引入因果干预方法验证解释的忠实性。

Result: 实验表明，基于行动的推理过程显著提高了校准准确性，将Brier分数降低了18%。通过掩蔽代理选择的视觉证据，观察到性能明显下降（ΔBrier=+0.029），证实证据对其决策过程至关重要。
...
TL;DR: 该论文提出了一种双信息瓶颈(DIB)策略来解决多模态情感分析中的两个关键问题：噪声污染的单模态数据学习和多模态表示融合不足。DIB通过最大化任务相关信息并丢弃冗余信息，以及新颖的注意力瓶颈融合机制，获得强大且统一的多模态紧凑表示。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个关键限制：对噪声污染的单模态数据学习不足导致跨模态交互受损，以及多模态表示融合不充分导致丢弃判别性单模态信息同时保留多模态冗余信息。

Method: 提出双信息瓶颈(DIB)策略，包含两个关键模块：1)通过最大化任务相关信息和丢弃冗余信息来学习充分压缩的单模态表示；2)通过新颖的注意力瓶颈融合机制确保多模态表示的判别能力。该方法在低秩Renyi熵函数框架下实现。

Result: 在CMU-MOSI、CMU-MOSEI、CH-SIMS和MVSA-Single数据集上的广泛实验验证了方法的有效性。在CMU-MOSI上Acc-7指标达到47.4%准确率，在CH-SIMS上F1-score达到81.63%，比次优基线提高1.19%。在噪声条件下，CMU-MOSI和CMU-MOSEI上的性能下降仅为0.36%和0.29%。

Conclusion: DIB策略能够有效过滤单模态数据中的噪声信息，同时捕捉模态间的互补性，为多模态情感分析提供了一种强大且鲁棒的解决方案。
...
TL;DR: 提出了一种分层多智能体框架，将被动医疗AI系统转变为主动询问代理，通过自主任务编排提升预诊效率和质量。


<details>
  <summary>Details</summary>
Motivation: 解决全球医疗系统面临的患者数量增加和咨询时间有限的问题，现有AI系统受限于被动交互范式和上下文管理挑战。

Method: 开发了八智能体架构，将预诊分解为四个主要任务（分诊、现病史收集、既往史收集、主诉生成）和13个领域特定子任务，采用集中控制机制和自主任务编排。

Result: 在1,372份电子健康记录上评估，分诊准确率达87.0%，任务完成率98.2%，临床质量评分平均4.56-4.69（5分制），咨询轮次12.7-16.9轮。
...
TL;DR: TPS-Bench是一个评估LLM代理在需要工具规划和调度的复合问题解决能力的基准，包含200个复合任务和数百个MCP工具。研究发现大多数模型能合理规划工具，但在调度效率上差异显著，并通过强化学习初步验证了提升调度效率的可能性。


<details>
  <summary>Details</summary>
Motivation: 目前LLM代理在解决需要多种工具的复合现实世界问题方面的能力尚未充分探索，特别是在给定异构工具库的情况下，代理不仅需要选择合适的工具，还需要战略性地安排执行顺序以确保效率。

Method: 构建TPS-Bench基准，包含200个复合任务和数百个MCP工具，评估LLM代理的工具规划和调度能力。对流行闭源和开源LLM进行实证研究，并在Qwen3-1.7B上使用强化学习进行调度优化实验。

Result: GLM-4.5达到64.72%的任务完成率但执行时间较长，GPT-4o仅45.08%完成率但优先并行工具调用。在Qwen3-1.7B上使用强化学习后，执行时间减少14%，任务完成率提升6%。

Conclusion: LLM代理在工具规划方面表现合理，但在调度效率上存在显著差异，强化学习是提升调度效率而不牺牲性能的可行方法。
...
TL;DR: 开发了一个多模态分析框架，利用大型基础模型分析企业社交媒体内容，重点关注可持续性沟通，包括文本和视觉内容的自动标注与聚类分析。


<details>
  <summary>Details</summary>
Motivation: 解决企业社交媒体内容多模态、模糊且不断演变的挑战，特别是可持续性相关沟通的分析需求，避免昂贵的特定任务标注成本。

Method: 使用大型语言模型集成标注企业推文与17个可持续发展目标的主题对齐，结合视觉语言模型通过语义聚类分析视觉可持续性沟通模式。

Result: 揭示了不同行业在可持续发展目标参与度、时间趋势以及企业沟通与ESG风险、消费者参与度之间的关联。
...
TL;DR: ExplicitLM是一种新型语言模型架构，通过外部可读记忆库实现知识的显式存储和直接修改，采用两阶段检索机制和双系统认知设计，在知识密集型任务上相比标准Transformer提升43.67%。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型知识陈旧和缺乏可解释性的问题，由于知识隐式存储在纠缠的网络参数中，无法进行针对性更新和推理透明化。

Method: 设计包含百万规模外部记忆库的架构，存储人类可读的知识作为token序列；采用基于产品键分解的高效粗粒度过滤和Gumbel-Softmax细粒度匹配的两阶段检索机制；基于双系统认知理论将知识划分为冻结显式事实(20%)和可学习隐式模式(80%)。

Result: 在知识密集型任务上相比标准Transformer提升43.67%，在低数据场景(10k样本)下获得3.62倍增益；正确预测的记忆命中率比错误预测高49%。

Conclusion: 联合优化的ExplicitLM架构证明，可解释、可更新的模型在保持竞争力的同时，能够提供前所未有的知识透明度，优于使用固定检索的RAG系统。
...
TL;DR: 提出IVGAE-TAMA-BO动态图神经网络模型，用于预测全球粮食贸易网络中的未来链接，显著提升预测性能


<details>
  <summary>Details</summary>
Motivation: 全球粮食贸易网络结构受地缘政治、经济和环境因素影响而动态演变，传统方法难以有效建模和预测未来贸易链接，需要更好地捕捉时间模式

Method: 基于IVGAE框架，引入贸易感知动量聚合器(TAMA)捕捉贸易网络的时间演化，联合建模短期波动和长期结构依赖，使用动量结构记忆机制和贝叶斯优化自动调参
...
<details>
  <summary>Details</summary>
Motivation: AI在司法取证中应用日益广泛，但传统大语言模型容易产生幻觉，静态知识库难以跟上法律条文和判例的频繁更新，需要确保法律问答的真实性和可追溯性。

Method: 采用检索优先策略：当可信法律知识库返回相关证据时使用RAG生成答案，否则由多个LLM生成候选答案并由专门选择器评分返回最优结果。高质量输出经人工审核后写回知识库，实现动态知识演进和溯源跟踪。
...
TL;DR: 提出Simia-SFT和Simia-RL两个框架，通过LLM模拟环境反馈来实现无需真实环境数据的智能体训练，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在复杂环境中表现脆弱，而构建定制训练环境成本高且脆弱，限制了进展。

Method: Simia-SFT通过扩增小规模种子数据生成多样化轨迹；Simia-RL通过LLM模拟反馈实现无需真实环境的强化学习训练。

Result: 微调后的开源模型在多个基准测试中表现一致提升，超越GPT-4o并接近o4-mini在τ²-Bench上的性能。

Conclusion: Simia框架实现了无需环境工程的规模化智能体训练，用灵活的LLM模拟替代了笨重脆弱的环境实现。
...
TL;DR: VRScout是一个基于深度学习的自主VR测试代理，能够以人类方式实时导航和交互VR环境，用于自动化VR游戏测试。


<details>
  <summary>Details</summary>
Motivation: 传统人工VR内容质量保证劳动密集且无法适应行业快速增长，而现有自动化测试方法难以处理VR的高维感官输入和实时性能要求。

Method: 使用增强型Action Chunking Transformer从人类演示中学习，预测多步动作序列；引入动态可调滑动视界平衡响应性和精确度。

Result: 在商业VR游戏中达到专家级性能，仅需有限训练数据，在消费级硬件上保持60 FPS实时推理。
...
TL;DR: 使用稀疏自编码器(SAE)进行特征选择和引导，在Llama-3 8B模型上实现了安全性和实用性的双重提升，突破了传统的安全-效用权衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要调整模型权重且过程昂贵，而当前的SAE方法缺乏系统性的特征选择和安全性-效用权衡的评估方法。

Method: 使用对比提示方法和两个数据集来高效选择最佳引导特征，通过SAE进行不同引导特征和强度的测试。

Result: 在Llama-3 8B模型上，安全性性能提升18.9%，同时实用性增加11.1%。

Conclusion: 通过原则性的特征选择方法，目标化的SAE引导可以克服传统的安全-效用权衡问题。
...
TL;DR: 机器遗忘技术虽然能有效移除不良知识，但会意外产生"知识漏洞"，导致良性知识丢失。研究发现高达98.7%的测试案例在遗忘模型中产生无意义回答，需要重新评估遗忘技术的知识保留评估方法。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘技术虽然能有效移除不良内容且不影响标准基准测试性能，但可能意外造成良性知识的丢失，这种"知识漏洞"在标准评估中难以发现。

Method: 提出测试案例生成框架，探索遗忘内容邻近区域和更广泛潜在失效区域，评估遗忘模型的知识保留情况。

Result: 评估显示遗忘存在显著隐藏成本：高达98.7%的测试案例在遗忘模型中产生无关或荒谬回答，而这些在预训练模型中是可回答的。

Conclusion: 需要重新思考机器遗忘中知识保留的评估方法，超越传统静态基准测试，更全面地检测知识漏洞。
...
TL;DR: 提出Skip-Block Routing(SBR)框架，用于Transformer类神经算子，通过路由机制学习token复杂度排名，在推理时根据排名决定后续层传递的token数量，将更多计算资源分配给复杂区域，减少约50%计算量同时保持精度。


<details>
  <summary>Details</summary>
Motivation: 当前神经算子在处理大规模工程问题时存在显著计算开销，且统一计算成本与物理场复杂度差异不匹配，如湍流中复杂涡旋区域需要更深层网络处理，而稳定流则不需要。

Method: SBR框架包含路由机制学习token复杂度和排名，在推理时根据排名决定后续层传递的token数量，使模型将更多处理能力集中在更复杂的token上。

Result: SBR可无缝集成到各种神经算子中，减少约50%浮点运算(FLOPs)，推理速度提升达2倍，且不牺牲精度。

Conclusion: SBR框架有效解决了神经算子在处理物理场时的计算效率问题，通过自适应分配计算资源实现了计算效率的显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 能源领域需要准确且高效的短期预测，但手动配置复杂方法耗时且易出错，同时需要处理时序动态性和泛化到未见数据的问题。

Method: 设计包含高效组件的搜索空间，提出考虑时序上下文泛化性能和搜索空间探索的新目标函数，通过NAS自动发现轻量级架构。

Result: 在能源生产时序数据上，NAS发现的轻量级架构集成模型在效率和准确性方面均优于Transformer等最先进技术和预训练预测模型。

Conclusion: NAS框架能够自动发现高效且准确的时序预测模型，在能源生产预测任务中展现出优越性能。
...
TL;DR: 该论文提出半监督偏好优化(SSPO)方法，通过结合少量配对偏好标签和大量未配对样本，显著降低对齐语言模型与人类偏好的数据需求。


<details>
  <summary>Details</summary>
Motivation: 现有偏好优化方法严重依赖大量配对反馈数据，导致资源消耗巨大。为解决这一问题，研究如何在少量配对标签和大量未配对样本下进行偏好优化。

Method: 提出SSPO方法，理论证明存在最优奖励阈值能够高概率分离胜负响应，从而对未配对数据进行原则性伪标注，利用伪标注从大规模未配对数据中提取潜在偏好。

Result: 在多个数据集上的实验验证了显著的数据效率，例如使用Llama3-8B-Instruct模型，仅用UltraFeedback数据集的1%训练的SSPO就超越了使用10%数据训练的强基线方法。

Conclusion: SSPO能够有效维持人类对齐效果，同时大幅降低数据获取成本，为偏好优化提供了更高效的数据利用方案。
...
TL;DR: PINNs方法在解决工程和生物动力系统ODE问题时展现出优越性能，通过嵌入物理定律和精心调整超参数，能够处理传统数值方法难以收敛的复杂问题。


<details>
  <summary>Details</summary>
Motivation: 传统数值方法在处理高刚度、冲击、不规则域、奇异摄动等高难度ODE问题时存在收敛困难，需要一种更强大的替代方法。

Method: 使用PINNs框架，通过将物理定律直接嵌入学习过程，并系统调整网络深度、激活函数、学习率等超参数，以及平衡数据损失、初始条件损失和残差损失。

Result: PINNs在复杂问题上能够收敛到正确解，但需要适当平衡损失函数分量和精心调整超参数。嵌入先验知识和施加硬约束能显著提升预测能力。

Conclusion: PINNs虽然不是万能解决方案，但通过适当的实现策略，在处理挑战性数值场景时能够获得优越结果，特别是在传统方法失效的情况下。
...
TL;DR: 提出了ReLaX-Net架构，通过时间复用层来扩展物理神经网络的有效深度，提高参数效率，仅需在现有PNN中添加快速开关。


<details>
  <summary>Details</summary>
Motivation: 物理神经网络在规模上落后数字神经网络数个数量级，需要参数高效架构来克服尺寸和性能限制。

Method: 采用层间时间复用方案，利用PNN中前向传播动态元件与可训练权重元件的时间尺度分离特性。

Result: 在图像分类和自然语言处理任务中验证，ReLaX-Net以相同参数数量超越传统RNN和DNN性能。

Conclusion: ReLaX-Net通过简单修改显著提升物理神经网络计算性能，展现出有利的扩展特性。
...
TL;DR: 提出了DynBERG模型，结合Graph-BERT和GRU来处理动态金融交易网络，支持有向边和时间演化分析，在比特币交易数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有Graph-BERT等图Transformer模型主要针对静态无向图设计，而金融交易网络具有动态特性和有向边，需要专门处理这些挑战。

Method: 将Graph-BERT与门控循环单元(GRU)集成，捕获多时间步的时间演化，并修改底层算法支持有向边，适用于动态金融交易分析。

Result: 在Elliptic比特币交易数据集上评估，在市场关闭事件前后均优于EvolveGCN和GCN方法，消融研究显示GRU组件对建模时间动态至关重要。

Conclusion: DynBERG能有效适应市场重大变化，为动态金融欺诈检测提供了强大解决方案，时间序列深度学习组件对性能提升有重要贡献。
...
TL;DR: 提出了一种基于自监督学习的多变量天气预测框架，结合图神经网络和时空适应机制，在ERA5和MERRA-2数据集上优于传统数值天气预报和深度学习方法。
...
Method: 集成图神经网络进行空间推理，采用自监督预训练方案进行表示学习，并引入时空适应机制以增强不同预测时间范围的泛化能力。

Result: 在ERA5和MERRA-2再分析数据集上的广泛实验表明，该方法相比传统数值天气预报模型和近期深度学习方法取得了更优性能。在北京和上海的定量评估和视觉分析证实了模型捕捉细粒度气象模式的能力。
...
TL;DR: 提出了FLoRA方法，一种融合前向-后向适配器的参数高效微调技术，结合了LoRA和并行适配器的优势，在保持相似参数预算的同时显著提升了精度和降低了延迟。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模日益增大，参数高效微调变得尤为重要。虽然已有各种PEFT方法被广泛研究，但该领域仍有很大自由度未被探索。

Method: 提出了FLoRA方法，融合前向-后向适配器，结合LoRA和并行适配器的思想，通过将适配器融合到基础模型的投影层中来最小化延迟。

Result: 实验结果表明，在相似参数预算下，提出的FFB适配器在精度和延迟方面都显著优于常用的LoRA方法。

Conclusion: FLoRA方法在参数高效微调方面表现出色，为LLM的下游任务微调提供了更有效的解决方案。
...
TL;DR: 本文分析了DoRA方法的机制，将其重新表述为更高效的矩阵形式，并提出了统一的PEFT框架，引入了Pre-Diag和SORA两种新方法，在性能和效率上均优于LoRA和DoRA。


<details>
  <summary>Details</summary>
Motivation: DoRA方法虽然性能优越但计算开销大，且其工作机制不明确。本文旨在揭示DoRA的成功机制，并提出更高效的参数高效微调方法。

Method: 首先识别DoRA通过增加权重更新矩阵的奇异值熵来提升性能，然后将其重新表述为数学等效但更高效的矩阵形式。基于此提出统一框架，探索架构位置和变换类型两个正交维度，并引入Pre-Diag和SORA两种新方法。

Result: 在自然语言理解和生成任务上的广泛实验表明，提出的方法在性能和效率上都优于LoRA和DoRA。

Conclusion: 通过揭示DoRA的工作机制并建立统一框架，本文提出的Pre-Diag和SORA方法在保持高性能的同时显著提升了计算效率，为参数高效微调方法的设计提供了新思路。
...
TL;DR: 评估特征引导分析（FGA）在MNIST和LSC数据集基准测试中的适用性，结果显示FGA在精度上优于文献结果，神经网络架构选择显著影响召回率但对精度影响较小。


<details>
  <summary>Details</summary>
Motivation: 理解神经网络决策原因对于安全关键应用至关重要，现有特征引导方法在工业环境中的适用性需要更多实证证据。

Method: 在MNIST和LSC数据集基准上评估FGA的有效性，分析神经网络架构、训练和特征选择对FGA效果的影响。

Result: FGA在基准测试中比文献结果具有更高精度，神经网络选择显著影响FGA的召回率但对精度影响可忽略。

Conclusion: FGA在解释神经网络行为方面具有实用价值，但需要仔细选择神经网络架构以优化召回率性能。
...
TL;DR: 提出了一种新的二次形式加权训练目标，通过考虑未来步骤间的标签自相关效应和设置异质任务权重，提升时间序列预测模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有训练目标如均方误差将每个未来步骤视为独立、等权重的任务，忽略了标签自相关效应和不同预测任务的异质性，限制了预测性能。

Method: 提出二次直接预测(QDF)学习算法，使用自适应更新的二次形式加权矩阵进行训练，其中非对角线元素考虑标签自相关，非均匀对角线匹配不同未来步骤的最优权重。

Result: 实验表明QDF有效提升了各种预测模型的性能，取得了最先进的结果。

Conclusion: 二次形式加权训练目标能够同时解决标签自相关效应和任务权重异质性问题，显著改善时间序列预测性能。
...
TL;DR: 提出了SpatialTraceGen框架，通过蒸馏大教师模型的推理过程来生成高质量的多步骤、多工具推理轨迹数据集，解决了视觉语言模型在复杂空间推理任务中缺乏高质量训练数据的问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在复杂空间推理方面表现不佳，而微调小型可部署模型又缺乏高质量的分步推理数据，这成为了性能提升的主要瓶颈。

Method: 开发了SpatialTraceGen框架，通过自动化验证器来确保每个推理步骤的保真度，从而可扩展地生成高质量的多步骤、多工具推理轨迹数据集。

Result: 在CLEVR-Humans基准测试中，验证器引导的过程将轨迹的平均质量分数提高了17%，同时将质量方差降低了40%以上。

Conclusion: SpatialTraceGen提供了专家级推理轨迹数据集，为有效的微调和样本高效的离线强化学习提供了必要的结构化分步工具使用示例。
...
TL;DR: 该论文研究了联邦学习在无人机热成像分割任务中的实际应用效果，比较了多种FL方法与集中式学习的性能差异。
...
Method: 在真实部署场景中评估FL算法，比较多种联邦学习方法与集中式学习基线，分析模型精度、训练时间、通信开销和能耗等关键指标，并探索客户端控制和工作流控制两种FL工作流程。

Result: 研究评估了FL在无人机热成像分割任务中的实际性能表现，为理解FL方法在该领域的应用和局限性提供了有价值的参考。

Conclusion: 联邦学习在无人机热成像特征检测中具有实际应用价值，特别是在数据自然分布且存在隐私限制的场景下，但需要权衡性能与通信开销等实际因素。
...
TL;DR: 提出MISA方法，通过模块级重要性采样来减少大语言模型训练的内存需求，相比层级优化方法更高效。


<details>
  <summary>Details</summary>
Motivation: 现有层级优化方法忽略了层内模块的不同重要性，且内存节省有限，需要改进。

Method: 将每层划分为更小模块，分配重要性分数，使用加权随机采样机制激活模块，降低梯度方差。

Result: 理论证明收敛率为O(1/√K)，实验验证在多种学习任务上有效，内存分析显示优于基线方法。

Conclusion: MISA通过模块级重要性采样实现了更高效的大语言模型训练，在内存节省和性能方面均优于现有方法。
...
TL;DR: 开发了一种基于决策树的自动化方法，用于识别和解释OthelloGPT模型中编码游戏规则的MLP神经元，发现约一半神经元可用紧凑的规则描述，并通过干预实验验证了这些模式的因果相关性。


<details>
  <summary>Details</summary>
Motivation: OthelloGPT模型为可解释性研究提供了理想测试平台，它既足够复杂展现丰富计算模式，又基于规则游戏逻辑便于逆向工程。

Method: 使用回归决策树将棋盘状态映射到神经元激活，提取高激活决策路径并转换为人类可读逻辑形式，通过针对性干预验证因果相关性。

Result: 在2048个神经元中，913个（约一半）可用紧凑的规则决策树准确描述（R² > 0.7），其余可能参与分布式或非规则计算。干预实验显示特定模式神经元消融会导致模型预测能力下降5-10倍。

Conclusion: 该方法成功识别了模型中的规则编码神经元，验证了其因果作用，并提供了工具支持未来可解释性研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有聚类算法存在限制性假设：K-Means和高斯混合模型假设凸的、高斯类簇，而DBSCAN和HDBSCAN能捕获非凸性但高度敏感。需要一种更灵活、适应性强的聚类方法。

Method: 在最近邻图上通过广度优先搜索扩展根图，使用持续更新的局部距离和形状统计信息指导聚类过程，用局部统计反馈替代固定密度阈值，并采用空间索引优化性能。

Result: EVINGCA在平均情况下具有对数线性复杂度，在多种合成、真实世界、低维和高维数据集上相比基线方法表现出竞争力。

Conclusion: EVINGCA提供了一种有效的密度-方差聚类方法，通过自适应图演化过程克服了传统算法的限制性假设，在保持性能的同时实现了更好的灵活性。
...
TL;DR: 该研究探索了预训练模型的不同层如何反映大脑在语言理解过程中的分层处理机制，通过比较wav2vec2和CLIP模型与脑电图信号的对应关系。


<details>
  <summary>Details</summary>
Motivation: 受大脑从原始声学信息到丰富多模态关联的分层处理机制启发，研究旨在确定预训练模型中哪些层最能反映大脑的这种分层处理过程。

Method: 使用自然语音感知期间记录的EEG数据，通过岭回归和对比解码评估wav2vec2和CLIP模型的嵌入与大脑活动的对应关系，测试了三种策略：单独层、渐进连接和渐进求和。

Result: 研究发现结合多模态、层感知的表征可能更接近解码大脑如何理解语言，而不仅仅是声音处理。

Conclusion: 多模态层感知表征的结合可能帮助我们更好地理解大脑将语言作为经验而非单纯声音的处理机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有GRPO方法存在关键问题：低概率token由于其固有的较大梯度幅度会不成比例地主导梯度更新，导致训练不稳定，并抑制了更可靠的高概率token的学习贡献。

Method: 提出了Token-Regulated Group Relative Policy Optimization (TR-GRPO)，这是GRPO的简单有效扩展，通过分配与模型预测概率正相关的token级权重，降低低概率token的权重并强调高概率token。

Result: 大量实验表明，TR-GRPO在包括逻辑、数学和智能体推理在内的RLVR任务中始终优于GRPO。

Conclusion: TR-GRPO缓解了梯度过度放大的问题，同时保留了信息丰富的学习信号，强调了在RL训练中调节token贡献的重要性，并确立了TR-GRPO作为增强LLM推理的稳健框架。
...
TL;DR: 提出了一种无需显式域标签的领域泛化方法，通过潜在域聚类和自适应知识迁移来提高视觉语言模型在未见目标域上的鲁棒性。
...
Method: 在图像特征上进行潜在域聚类，并根据输入图像与每个潜在域的相似度融合域特定的文本特征，实现跨域的自适应知识迁移。

Result: 在四个基准测试上的实验表明，该方法相比基于VLM的基线方法获得了持续的性能提升。

Conclusion: 该方法为在域偏移下提高模型鲁棒性提供了新的思路，证明了通过潜在域表示和自适应特征融合可以有效改善领域泛化性能。
...
TL;DR: 该研究比较了大型语言模型(LLMs)与贝叶斯优化(BO)在约束多目标回归任务中的表现，发现专用BO框架在收敛性方面表现最佳，但经过微调的LLMs可作为有前景的快速替代方案。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在约束、连续、高维数值空间任务中的效用，这些任务并非LLMs的原始设计目标，但在材料信息学中的逆向设计问题中具有重要意义。

Method: 采用严格的比较研究，对比了贝叶斯优化框架(BoTorch Ax和qEHVI)与经过参数高效微调(PEFT)的LLMs和BERT模型，将问题构建为带有自定义输出头的回归问题。

Result: BoTorch qEHVI实现了完美收敛(GD=0.0)，而表现最佳的LLM(WizardMath-7B)的世代距离为1.21，显著优于传统BoTorch Ax基线(GD=15.03)。

Conclusion: 专用BO框架在保证收敛性方面仍是性能领先者，但经过微调的LLMs被验证为有前景且计算快速的替代方案，为AI驱动的优化领域贡献了重要的比较指标。
...
TL;DR: 使用小波特征提取和无监督聚类的方法来检测奇偶性，无需标签监督达到69.67%的分类准确率


<details>
  <summary>Details</summary>
Motivation: 探索将经典信号处理技术应用于纯离散符号领域，揭示特征工程和聚类在非常规机器学习问题中的潜力

Method: 将整数转换为小波域表示，提取多尺度统计特征，使用k-means算法进行聚类

Result: 特征空间揭示了奇偶数之间的结构性差异，无监督分类准确率达到69.67%

Conclusion: 经典信号处理技术可以在纯离散符号领域发现潜在结构，为符号推理和基于特征学习之间架起桥梁
...
TL;DR: 该研究探索视觉语言模型对视觉信息几何结构的理解能力，通过将象形文字识别构建为程序合成任务，训练模型将光栅图像反编译为由贝塞尔曲线组成的几何程序。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型展现了强大的语义能力，但其对视觉信息底层几何结构的理解能力尚未充分探索。象形文字结合了视觉形式和符号结构，是测试这种能力的理想案例。

Method: 将视觉识别挑战构建为程序合成任务，训练视觉语言模型将光栅图像反编译为由贝塞尔曲线组成的几何程序，模型充当"视觉反编译器"。

Result: 模型性能优于包括GPT-4o在内的强零样本基线。最显著的发现是，仅在现代汉字上训练的模型能够在零样本情况下重建古代甲骨文，这表明模型获得了抽象且可迁移的几何语法。
...
TL;DR: flowengineR是一个R语言包，提供模块化、可扩展的框架来构建可复现的机器学习工作流，特别关注算法公平性领域。


<details>
  <summary>Details</summary>
Motivation: 算法公平性领域快速发展，新指标、缓解策略和机器学习方法不断涌现，现有工具包要么关注单一干预措施，要么将可复现性和可扩展性视为次要考虑因素。

Method: 引入统一架构，包含数据分割、执行、预处理、训练、处理中、后处理、评估和报告等标准化引擎，每个引擎封装一个方法任务并通过轻量级接口通信。

Result: 工作流保持透明、可审计且易于扩展，支持公平性方法的集成、比较和评估，同时可推广到可解释性、鲁棒性和合规性指标。

Conclusion: 虽然受公平性研究启发，但flowengineR为任何需要可复现性、透明度和可扩展性的工作流场景提供了通用基础设施。
...
TL;DR: 提出Fix-GCN模型，通过固定点迭代图卷积网络实现对抗攻击鲁棒性，使用频谱调制滤波器选择性衰减高频分量，保护图结构信息。
...
Method: 引入通用频谱调制滤波器，通过固定点迭代推导特征传播规则，提供灵活通滤波方法，选择性衰减高频分量同时保留低频结构信息。

Result: 在多个基准图数据集上的广泛实验证明了模型的有效性，展示了其对对抗攻击的韧性。

Conclusion: Fix-GCN提供了一个灵活高效的框架，在保持基本图信息的同时减轻对抗操纵的影响，无需额外内存或计算复杂度。
...
TL;DR: 该论文探讨了使用机器学习方法自动预测笔纸RPG游戏中怪物等级的可行性，通过构建专用数据集和评估方法，比较了机器学习算法与传统人工方法的性能。


<details>
  <summary>Details</summary>
Motivation: 随着笔纸RPG市场的增长，出版商需要更高效的方法来设计新对手和评估其挑战等级。目前仅依赖耗时的人工测试和专家评估，缺乏自动化解决方案。

Method: 采用序数回归技术进行等级预测，构建了专用的等级估计数据集，开发了基于人类思维的基准模型，并设计了基于领域知识的专门评估程序。

Result: 论文评估了最先进的序数回归方法在怪物等级预测任务上的表现，并将机器学习算法与传统人工方法进行了比较。

Conclusion: 研究表明机器学习方法在怪物等级预测任务上具有潜力，能够为笔纸RPG出版商提供更高效的自动化解决方案。
...
TL;DR: 提出MaGNet模型，一种基于Mamba双超图网络的股票预测方法，通过MAGE块、2D时空注意力模块和双超图框架，有效捕捉时间依赖性和股票间动态关系，在多个股票指数上表现出优越的预测性能和投资回报。


<details>
  <summary>Details</summary>
Motivation: 现有股票预测方法难以有效捕捉时间依赖性和动态股票间交互，往往忽略横截面市场影响、依赖静态相关性、对节点和边采用统一处理，并混淆了多样化关系。

Method: MaGNet包含三个关键创新：(1) MAGE块，利用双向Mamba和自适应门控机制进行上下文时间建模，集成稀疏专家混合层以适应不同市场条件；(2) 特征和股票维度的2D时空注意力模块，精确融合多变量特征和跨股票依赖；(3) 双超图框架，包括捕捉细粒度因果依赖的时间因果超图和建模市场范围模式的全局概率超图。

Result: 在六个主要股票指数上的广泛实验表明，MaGNet在预测性能和投资回报方面均优于最先进方法，并具有稳健的风险管理能力。

Conclusion: MaGNet通过集成Mamba架构、时空注意力机制和双超图框架，有效解决了股票预测中的时间依赖性和复杂关系建模问题，为股票趋势预测提供了新的解决方案。
...
TL;DR: 本文提出Agent-REINFORCE框架，通过LLM代理增强的强化学习方法来搜索测试时扩展中的最优多LLM协作图，解决传统方法忽视任务特定架构需求的问题。


<details>
  <summary>Details</summary>
Motivation: 现有测试时扩展方法通常假设固定的协作架构和单一模型使用，忽视了最优架构和模型组合会因任务而异的问题。

Method: 将问题形式化为概率图优化，提出Agent-REINFORCE框架，通过采样-反馈-更新过程，使用文本反馈作为梯度来更新概率图，高效搜索最优多LLM协作图。

Result: 实验表明Agent-REINFORCE在样本效率和搜索性能上优于传统和基于LLM的基线方法，能有效识别在准确性和推理延迟联合目标下的最优图。

Conclusion: Agent-REINFORCE框架成功解决了测试时扩展中多LLM协作图的优化问题，为不同任务找到计算最优的模型组合和架构。
...
<details>
  <summary>Details</summary>
Motivation: 现有的图增量学习方法主要关注单域内的任务增量和类别增量场景，而图域增量学习（Domain-IL）在多个图域间更新模型对于图基础模型的发展至关重要，但尚未被探索。

Method: GraphKeeper方法包括：1）域特定参数高效微调与域内和域间解缠目标，防止嵌入漂移和混淆；2）无偏差知识保持，维持稳定决策边界；3）对于不可观测域的图，进行域感知分布判别以获得精确嵌入。

Result: 大量实验表明GraphKeeper取得了最先进的结果，比第二名提升了6.5%~16.6%，且遗忘可忽略不计。该方法可与各种代表性图基础模型无缝集成。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于人工智能的数字孪生方法在测量数据少、物理知识缺失或损伤状态未知时预测性能较差，需要开发不依赖先验信息的损伤检测框架。

Method: 使用条件标签生成对抗网络，通过将相同损伤级别的测量数据强制收敛到不同损伤状态，比较收敛得分来识别不同损伤状态，同时生成数字孪生数据。

Result: 该方法能够准确捕获健康测量数据中的损伤，为基于振动的系统级监测和可扩展基础设施韧性提供了强大工具。

Conclusion: 提出的无监督框架在Z24桥梁基准测试中验证有效，解决了当前数字孪生方法在数据稀缺和损伤状态未知情况下的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 解决土木工程应用中由于测试数据少或结构模型不可识别导致的动态载荷识别不确定性问题，探索深度学习网络在小数据集条件下的适用性。

Method: 使用三种深度学习网络（GRU、LSTM、CNN）和残差卡尔曼滤波器（RKF），在三种场景下进行测试：模拟结构在顶层激振、加州建筑在地震基础激励、IASC-ASCE结构健康监测基准问题的冲击和瞬时载荷条件。

Result: 不同方法在不同载荷场景下表现各异，RKF在物理参数可识别的情况下优于神经网络。

Conclusion: 各种方法在不同载荷识别场景中各有优势，RKF在物理参数可识别的情况下表现最佳，而神经网络在小数据集条件下也展现出良好的识别能力。
...
TL;DR: Loquetier是一个虚拟化多LoRA框架，在单个运行时中无缝集成LoRA微调和推理，通过虚拟化模块和优化计算流显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 虽然已有研究探索了LLM训练和服务的集成策略，但基于LoRA的模型的微调和推理统一仍然存在差距。

Method: 提出Loquetier框架，包含：(1)虚拟化模块隔离PEFT修改并支持共享基础模型上的多个适配器；(2)优化计算流，在正向传播中合并微调和推理路径，实现高效批处理和最小化内核调用开销。

Result: 在三个任务设置上的广泛实验表明，Loquetier在性能和灵活性上始终优于现有基线，在仅推理任务上达到最先进协同服务系统3.0倍的吞吐量，在统一微调和推理任务上比PEFT实现46.4倍更高的SLO达成率。

Conclusion: Loquetier成功统一了基于LoRA的模型的微调和推理，提供了高效且灵活的解决方案。
...
TL;DR: 提出一种混合框架，从噪声轨迹数据中自动发现守恒量，包含神经ODE、Transformer和符号数值验证器三个组件。


<details>
  <summary>Details</summary>
Motivation: 从观测数据中识别守恒定律是科学进步的关键，但目前从轨迹数据中发现这些不变量仍具挑战性。

Method: 集成三个组件：神经ODE学习系统动态的连续模型，Transformer基于学习到的向量场生成符号候选不变量，符号数值验证器为候选者提供数值验证。

Result: 在典型物理系统上测试，显著优于直接在轨迹数据上操作的基线方法。

Conclusion: 证明了分离式'先学习后搜索'方法在从不完美数据中发现数学原理方面的鲁棒性。
...
TL;DR: Pelican-VL 1.0是当前最大规模的开源具身多模态大脑模型，参数规模从70亿到720亿，通过DPPO框架和metaloop机制实现了20.3%的性能提升，在具身基准测试中达到与领先专有系统相当的水平。


<details>
  <summary>Details</summary>
Motivation: 明确使命是将强大智能嵌入各种具身系统中，开发大规模开源具身多模态大脑模型。

Method: 提出DPPO（刻意练习策略优化）框架，受人类元认知启发，采用RL-Refine-Diagnose-SFT循环的metaloop机制，从40亿+token的原始数据集中蒸馏高质量数据集，在1000+ A800 GPU集群上训练。

Result: 相比基础模型性能提升20.3%，超越100B级开源模型10.6%，在知名具身基准测试中与领先专有系统表现相当。

Conclusion: Pelican-VL 1.0成功实现了大规模具身智能模型的开发，通过数据力量与智能自适应学习机制的深度整合，在具身智能领域达到了领先水平。
...
TL;DR: MeixnerNet是一种新型谱图神经网络，使用离散正交多项式（Meixner多项式）替代连续多项式，解决了连续域滤波器与离散图结构不匹配的问题，并通过可学习参数和稳定化技术实现更好的性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有谱GNN（如ChebyNet）使用连续正交多项式作为图卷积滤波器，这与离散的图结构存在理论上的不匹配，可能导致次优性能和超参数设置的脆弱性。

Method: 提出MeixnerNet架构，使用离散Meixner多项式作为滤波器，将多项式的两个形状参数β和c设为可学习，并引入结合拉普拉斯缩放和逐基LayerNorm的稳定化技术来解决数值不稳定性问题。

Result: 在K=2的最优设置下，MeixnerNet在3个基准测试中的2个上表现优于ChebyNet；更重要的是，MeixnerNet对多项式阶数K的变化具有出色的鲁棒性，而ChebyNet对此高度敏感且性能会崩溃。

Conclusion: MeixnerNet通过使用离散正交多项式解决了连续域滤波器与离散图结构的不匹配问题，不仅实现了竞争性性能，而且显著提高了对关键超参数K的鲁棒性。
...
TL;DR: LC-Opt是一个用于强化学习控制策略的可持续液体冷却基准环境，基于橡树岭国家实验室Frontier超级计算机冷却系统的高保真数字孪生构建，支持多目标实时优化和可解释控制。


<details>
  <summary>Details</summary>
Motivation: 随着AI工作负载增加，高密度数据中心的液体冷却至关重要。基于机器学习的控制器对于提升能源效率和可靠性、促进可持续性至关重要。

Method: 基于Modelica的端到端模型，从站点级冷却塔到数据中心机柜和服务器刀片组。RL代理通过Gymnasium接口优化关键热控制参数，包括液体供应温度、流速、阀门控制等。

Result: 建立了集中式和分散式多智能体RL方法的基准，展示了策略蒸馏为可解释的决策树和回归树，并探索了基于LLM的自然语言解释控制方法。

Conclusion: LC-Opt为ML社区、运营商和供应商提供了详细可定制的液体冷却模型，促进了可持续数据中心液体冷却控制解决方案的开发。
...
TL;DR: DCcluster-Opt是一个开源的高保真模拟基准，用于可持续的跨时空任务调度，结合真实世界数据集和物理信息模型，支持可重复的可持续计算研究。


<details>
  <summary>Details</summary>
Motivation: 大规模AI日益增长的能源需求和碳足迹需要在全球分布式数据中心进行智能工作负载管理，但缺乏能真实捕捉环境因素、数据中心物理特性和网络动态相互作用的基准测试限制了进展。

Method: 结合精选的真实世界数据集（AI工作负载轨迹、电网碳强度、电力市场、天气、云传输成本等）与物理信息数据中心运营模型，提供模块化奖励系统和Gymnasium API，包含强化学习和基于规则的基线控制器。

Result: 提供了一个具有挑战性的调度问题环境，顶层协调代理必须动态重新分配或延迟具有资源和服务级别协议要求的任务，以优化多个目标，包括碳排放、能源成本、SLA和水资源使用。

Conclusion: 通过提供真实、可配置且易于使用的测试平台，DCcluster-Opt加速了用于地理分布式数据中心的下一代可持续计算解决方案的开发和验证。
...
TL;DR: 开发了一个机器学习模型来预测足球中的防线突破（Line Breaks），使用XGBoost分类器分析球员位置、速度和空间配置等特征，模型准确率很高（AUC 0.982），并发现进攻球员速度、防线空隙等因素对防线突破有显著影响。


<details>
  <summary>Details</summary>
Motivation: 在足球中，突破对手防线是创造得分机会的关键，但以往研究主要关注射门或进球机会，而忽略了球队如何突破防线的过程。

Method: 使用2023年J1联赛赛季的事件和追踪数据，构建包含189个特征的机器学习模型，包括球员位置、速度和空间配置等，采用XGBoost分类器预测防线突破的概率。

Result: 模型预测准确率很高（AUC 0.982，Brier得分0.015），SHAP分析显示进攻球员速度、防线空隙和进攻球员空间分布是影响防线突破的关键因素，且防线突破概率与失球数和失传中数呈正相关。
...
TL;DR: 该论文使用统计物理中的交叉涨落分析扩散模型的采样动态，发现样本在生成过程中经历尖锐的离散转变，这些转变可通过交叉涨落检测，从而提升采样效率和多种任务的性能。


<details>
  <summary>Details</summary>
Motivation: 研究扩散模型中采样动态的演变过程，特别是样本从初始分布到目标分布的转变机制，以理解生成过程中的结构演化。

Method: 使用统计物理中的交叉涨落统计量分析扩散模型的采样动态，推导方差保持SDE的交叉涨落闭式解，检测采样过程中的离散转变。

Result: 发现采样过程中存在尖锐的离散转变，检测这些转变可直接提升采样效率，加速条件生成和稀有类别生成，改进图像分类和风格迁移等零样本任务。

Conclusion: 该框架统一了离散马尔可夫链理论与连续动态系统，将经典耦合和混合理论扩展到随机SDE和非马尔可夫采样器，为生成建模提供了新的理论视角。
...
TL;DR: 提出动态多专家门控框架，自适应选择最可靠的轨迹预测器（物理LSTM、Transformer、微调GameFormer），在nuPlan-mini数据集上FDE降低9.5%，实现57.8%的oracle性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度轨迹预测器在复杂长尾驾驶场景中不可靠，传统"一刀切"模型在安全关键城市环境中存在局限，物理模型有时优于先进网络。

Method: 基于内部模型信号（稳定性和不确定性）的动态多专家门控框架，将轨迹专家选择建模为成对排序问题，无需后校准直接优化决策质量。

Result: 在nuPlan-mini数据集上FDE为2.567米，比GameFormer降低9.5%；左转场景FDE降低约10%，在离线和开环评估中均表现一致改进。

Conclusion: 自适应混合系统增强了安全关键自动驾驶中的轨迹可靠性，为超越静态单模型范式提供了实用路径。
...
TL;DR: 本文提出了一种集成到井下工具中的CCL信号采集系统，用于构建数据集。通过全面的数据增强预处理方法，显著提升了基于AlexNet的套管接箍识别模型的性能，F1分数从0.937和0.952提升至1.0。


<details>
  <summary>Details</summary>
Motivation: 准确的下井深度测量对油气井作业至关重要。虽然基于神经网络的CCL信号识别在接箍识别方面取得了进展，但此类应用的预处理方法仍然不完善，且真实井数据的有限性对需要大量数据集的神经网络模型训练构成了重大挑战。

Method: 提出了集成到井下工具中的CCL信号采集系统用于数据集构建。开发了全面的预处理方法进行数据增强，包括标准化、标签分布平滑(LDS)、随机裁剪、标签平滑正则化(LSR)、时间缩放和多重采样等方法，并使用基于AlexNet的神经网络模型进行评估。

Result: 实验结果表明，标准化、LDS和随机裁剪是模型训练的基本要求，而LSR、时间缩放和多重采样显著增强了模型的泛化能力。两个基准模型的F1分数从0.937和0.952最大提升至1.0和1.0。在真实CCL波形上的性能验证证实了该方法的有效性和实际适用性。

Conclusion: 这项工作解决了在CCL数据有限环境下训练套管接箍识别模型的数据增强方法空白，为油气井作业中的精确深度校准提供了有效的技术解决方案。
...
TL;DR: 比较分析SFT、LoRA和ICL三种LLM适应方法在数据稀缺场景下的表现，发现LoRA在技能获取与通用知识保留之间达到最佳平衡


<details>
  <summary>Details</summary>
Motivation: LLM适应特定应用时需要在集成新知识和保持通用推理能力之间权衡，现有方法各有优缺点，需要系统比较以指导实践

Method: 在数据稀缺场景下对监督微调(SFT)、低秩适应(LoRA)和上下文学习(ICL)进行对比分析

Result: LoRA在技能获取与通用知识保留之间达到最佳平衡；SFT擅长技能获取但易发生灾难性遗忘；ICL适合事实知识集成但难以处理复杂技能

Conclusion: 提出了选择LLM适应策略的实用框架，强调了技能获取与知识集成的关键区别，阐明了任务特定性能与通用能力保留之间的权衡
...
TL;DR: 提出了一种结合概率特征采样和模拟退火超参数调优的随机森林增强框架，在多个领域显著提升预测准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统随机森林存在局限性，需要更好地捕捉数据中的相关信号并实现自适应超参数配置，以应对信用风险评估、物联网异常检测、早期医疗诊断和高维生物数据分析等多样化领域的分类挑战。

Method: 采用概率特征采样方法强调对分类更有意义的特征，结合模拟退火算法进行动态超参数调优，引导模型关注贡献更大的特征。

Result: 实验结果显示该框架在准确率方面取得一致性的提升，并提供了对特征相关性的有意义的洞察。

Conclusion: 结合重要性感知采样和元启发式优化的方法在提升随机森林性能方面表现出显著效果，为复杂分类问题提供了有效的解决方案。
...
TL;DR: 该研究量化了植被结构和功能如何影响热指数，发现在高湿度密集城市核心区，生理活性强的植被会提高近地表湿度，逆转其冷却效果并加剧感知热应激。


<details>
  <summary>Details</summary>
Motivation: 城市绿化降温效果不均，因为植被在冷却表面的同时也会增加空气湿度。现有研究对植被如何调节冷却与湿度积累之间的权衡理解不足，导致缓解政策和设计缺乏指导。

Method: 使用极端感知的1公里分辨率热指数重建和可解释机器学习框架（SHAP和ALE），分析138个印度城市中植被与气候的相互作用。

Result: 当EVI≥0.4、LAI≥0.05时冷却效果增强，但当EVI≥0.5、LAI≥0.2、fPAR≥0.5时开始转向增温，在潮湿密集核心区fPAR≥0.25时更早出现逆转。

Conclusion: 确定了植被驱动冷却的气候限制，为气候特异性绿化策略提供了定量阈值，以促进公平和耐热城市发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM方法在交通信号控制中存在固定信号时长和幻觉错误的限制，而RL方法在信号时序决策中缺乏鲁棒性且泛化能力差。

Method: 采用双LLM架构：Herald模块提取上下文信息并预测队列长度；LLM-Agent进行细粒度交通信号控制；LLM-Critic精炼输出并纠正错误；通过基于分数的微调提高准确性和鲁棒性。

Result: 在覆盖224个交叉口的真实数据集上，HeraldLight优于最先进基线，在所有场景中平均旅行时间减少20.03%，在济南和杭州场景中平均队列长度减少10.74%。

Conclusion: HeraldLight通过双LLM架构和Herald引导提示有效提升了交通信号控制的性能、准确性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 为了提高后端集中式冗余供应链的决策制定和规划效率，需要构建更智能的优化模型。
...
TL;DR: 该研究评估稀疏自编码器(SAEs)在识别和控制LLM中种族与污名化概念关联的能力，发现SAEs能识别与黑人相关的潜在特征，但通过SAE引导缓解偏见在复杂临床任务中效果有限。
...
Result: 研究发现黑人相关的潜在特征会在合理输入(如"非裔美国人")和问题词汇(如"监禁")上激活，激活该特征会增加模型将患者预测为"好斗"的风险，表明存在偏见关联。

Conclusion: SAEs可作为识别LLM中问题性人口统计依赖的有用工具，但通过SAE引导缓解偏见在现实复杂临床任务中效果有限。
...
TL;DR: PDE-SHARP是一个通过用更便宜的LLM推理替代昂贵的科学计算来降低计算成本的框架，在减少60-75%计算评估的情况下实现更优的求解器精度。
...
Method: 采用三阶段框架：(1)分析：包括PDE分类、解类型检测和稳定性分析的数学思维链分析；(2)生成：基于前一阶段数学洞察的求解器生成；(3)合成：通过LLM评委迭代精化实现的协作选择-混合锦标赛。

Result: 平均只需少于13次求解器评估（基线方法需要30+次），在测试的PDE上平均精度提高4倍，在不同LLM架构上均表现出稳健性能。

Conclusion: PDE-SHARP能够显著降低计算成本，同时提高求解器精度，为复杂PDE求解提供了高效的计算框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有成员推理攻击方法只能检测整个提示或文档是否在训练数据中，无法捕捉更细粒度的风险，特别是在敏感信息（如PII、信用卡号等）的实体级别。

Method: 提出EL-MIA框架，构建基准数据集，系统比较现有MIA技术和两种新方法，分析实体级MIA易感性与模型规模、训练轮次等因素的关系。

Result: 发现现有MIA方法在实体级敏感属性成员推理方面效果有限，但这种易感性可以通过相对简单的方法进行识别，表明需要更强的对抗方法来测试威胁模型。

Conclusion: 需要开发更强大的对抗方法来充分测试LLM在实体级敏感信息保护方面的威胁模型，现有MIA方法在细粒度隐私风险评估方面存在不足。
...
TL;DR: 提出了一种新颖的框架，将资源密集型的对抗性提示优化问题转化为高效的摊销推理任务，利用预训练的非自回归生成LLM直接条件生成提示。


<details>
  <summary>Details</summary>
Motivation: 传统的对抗性提示优化需要大量计算资源，作者希望找到更高效的方法来生成有效的对抗性提示。

Method: 使用预训练的非自回归生成LLM（如Diffusion LLMs）作为提示搜索的代理模型，通过条件生成直接产生提示，替代昂贵的逐实例离散优化。

Result: 生成的提示具有低困惑度、多样性，并且在各种黑盒目标模型（包括经过鲁棒训练和专有LLM）上表现出强大的可迁移性。

Conclusion: 该框架不仅为对抗性提示提供了高效解决方案，还为红队测试、自动化提示优化以及基于Flow和Diffusion的LLM应用开辟了新方向。
...
TL;DR: 扩散模型在药物发现中用于设计小分子和治疗肽，通过迭代去噪框架适应不同分子表示和设计目标，面临化学可合成性、生物稳定性等挑战，未来需整合到自动化DBTL平台。


<details>
  <summary>Details</summary>
Motivation: 利用扩散模型加速和变革传统缓慢昂贵的药物发现过程，为小分子和治疗肽设计提供统一生成框架。

Method: 采用迭代去噪的统一框架，针对小分子和治疗肽的不同分子表示、化学空间和设计目标进行适应性应用。

Result: 小分子设计在结构基础设计方面表现优异，能生成新颖的配体，但面临化学可合成性挑战；治疗肽设计专注于功能序列生成，但需解决生物稳定性、正确折叠和免疫原性问题。

Conclusion: 扩散模型的全部潜力需要通过弥合模态特定差距并整合到自动化DBTL平台来实现，从而将范式从化学探索转向靶向创建新型治疗药物。
...
TL;DR: 提出了一种基于多奖励信号的强化学习方法，用于微调基础模型，通过迭代优化多个奖励函数来提升模型性能。
...
Method: 采用基于强化学习的多奖励信号微调方法，通过迭代策略在多个奖励函数上进行优化，扩展了现有最先进的RL方法。

Result: 在文本、生物序列和小分子生成等多个领域的实验结果表明，该算法相比现有基线方法具有更好的性能。
...
TL;DR: 提出一种结合集成学习和可解释人工智能技术的黑色素瘤检测模型，旨在提高深度学习模型的可解释性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 黑色素瘤是致命性皮肤癌，早期检测至关重要。深度学习模型虽然准确率高，但缺乏可解释性限制了其在医疗领域的应用。需要解决黑盒决策带来的信任问题。

Method: 使用三种先进的深度迁移学习网络进行集成学习，并应用可解释人工智能技术来解释预测依据。

Result: 开发了一个能够提供可靠预测并解释决策依据的黑色素瘤检测系统。

Conclusion: 通过集成学习和XAI技术的结合，可以在保持高准确率的同时提高深度学习模型在医疗诊断中的可信度和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 解决非随机多臂老虎机专家建议问题中极小极大最优期望遗憾的精确确定问题，填补现有研究中上界与下界之间的差距。

Method: 通过证明与已有上界匹配的下界，确定极小极大最优期望遗憾的确切表达式。

Result: 证明了非随机多臂老虎机专家建议问题的极小极大最优期望遗憾为Θ(√(TK log(N/K)))，其中K是臂数，N是专家数，T是时间范围。

Conclusion: 该研究完全解决了非随机多臂老虎机专家建议问题的极小极大最优期望遗憾，为K=1和K=N等特殊情况提供了统一的遗憾界。
...
TL;DR: 提出了基于xLSTM的车辆轨迹预测框架X-TRAJ及其物理感知变体X-TRACK，通过整合车辆运动学约束生成更真实可行的轨迹，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: xLSTM架构虽然改进了传统LSTM的长期依赖建模能力，但在车辆轨迹预测领域尚未得到充分探索。

Method: 开发了X-TRAJ框架及其物理感知变体X-TRACK，通过显式整合车辆运动学约束到模型学习过程中。

Result: 在highD和NGSIM数据集上的综合评估表明，X-TRACK优于最先进的基线方法。

Conclusion: 提出的xLSTM框架能够有效预测车辆轨迹，物理约束的引入提高了轨迹的真实性和可行性。
...
TL;DR: 该论文提出了一种基于领域知识的强化学习方法，用于控制混沌对流流动，在Rayleigh-Bénard对流系统中实现了热传输的显著减少。


<details>
  <summary>Details</summary>
Motivation: 混沌对流流动在微流体设备和化学反应器等实际系统中普遍存在，但传统控制方法在混沌状态下往往失效。虽然强化学习在层流控制中显示出潜力，但在混沌和湍流动力学下的泛化能力和鲁棒性尚未得到充分探索。

Method: 引入基于领域知识的强化学习代理，使用近端策略优化算法在不同初始条件和流动状态下进行训练。在奖励函数中融入领域知识，通过鼓励Bénard单元合并来引导学习过程。

Result: 在层流状态下，领域知识强化学习代理将热传输减少高达33%；在混沌流动状态下，仍能实现10%的减少，显著优于传统控制器。与无领域知识的代理相比，该方法产生稳定流动、训练收敛更快，且无需重新训练即可在不同流动状态间泛化。
...
TL;DR: 本文研究发现LLM的校准能力在网络深度中演化，上层存在置信度修正阶段，并识别出残差流中的低维校准方向，表明校准是分布式的网络现象。


<details>
  <summary>Details</summary>
Motivation: 尽管先前研究发现深度神经网络通常过度自信，但LLM表现出内在的校准能力。本文旨在从网络深度演化的角度补充理解LLM的校准机制。

Method: 分析多个开源权重模型在MMLU基准上的表现，研究校准如何随网络深度变化，识别置信度修正阶段和残差流中的校准方向。

Result: 发现上层存在置信度主动重新校准的阶段，识别出低维校准方向，扰动该方向可显著改善校准指标而不损害准确性。

Conclusion: 校准是分布式现象，在整个网络前向传播过程中形成，而不仅限于最终投影层，为理解LLM内部置信度调节机制提供了新视角。
...
Result: 不同技术在不同任务中的不确定性可靠性表现复杂，最优方法取决于不确定性表达方式、评估指标和可靠性评估尺度。局部校准和适应性评估提供了更实用的模型行为洞察。

Conclusion: 评估不确定性量化技术的标准应适应模型的实际使用场景，在保持预测性能的同时，优先考虑小尺度可靠性。
...
TL;DR: 提出了一种结合模型协变量合成和因果结构保持的混合生成框架，用于生成保留因果参数的合成表格数据。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型和GAN方法在生成合成表格数据时，虽然能达到高预测保真度，但往往无法保持关键因果参数如平均处理效应(ATE)。

Method: 采用混合生成框架：模型基协变量合成（通过距离最近记录过滤监控）+ 分别学习的倾向评分和结果模型，确保(W,A,Y)三元组保留底层因果结构。还引入合成配对策略缓解正性违例。

Result: 提出的方法能够生成保持因果结构的合成数据，支持稳健的因果分析，并通过无限合成样本为传统估计器提供评估基准。

Conclusion: 这项工作为支持稳健因果分析的LLM驱动数据管道奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 传统的Speculative Decoding要求输出分布与目标模型完全匹配，这种严格限制导致接受率过低，限制了潜在加速效果。作者认为在实际应用中，任务特定性能（如代码正确性、事实准确性）比采样分布更重要。

Method: 提出了Pivot-Aware Speculative Decoding方法，识别并只拒绝那些会导致效用下降的关键token（pivot tokens），训练轻量级分类器来检测这些关键token，作为标准SD的松弛版本。

Result: 在多个数据集上的评估显示，该方法能够实现最高2.5倍的加速，同时保持可比较的效用性能。

Conclusion: 通过将解码目标重新定义为匹配期望效用而非严格分布匹配，可以显著提高接受率并获得更好的加速效果，同时保持任务性能。
...
TL;DR: 提出基于稀疏性的统一框架来评估算法公平性，将公平性重新定义为稀疏性问题，并与现有公平标准保持一致。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习中的算法公平性标准缺乏跨不同问题的通用性，需要一种统一框架来评估和促进公平性。

Method: 研究各种稀疏性度量在促进公平性方面的联系与差异，提出统一的基于稀疏性的公平性评估框架。

Result: 实验验证了该框架作为评估指标的有效性，适用于多种数据集和偏见缓解方法。

Conclusion: 通过稀疏性和社会公平的视角为算法公平性研究提供了新思路，对公平性研究和应用具有广泛影响潜力。
...
TL;DR: 比较了模糊推理方法(ANFIS-FBCSP-PSO)和深度学习基准(EEGNet)在运动想象EEG分类中的表现，发现模糊方法在个体内测试中表现更好，而深度方法在跨个体测试中泛化能力更强。
...
Method: 使用ANFIS-FBCSP-PSO（结合滤波器组共空间模式特征提取和粒子群优化的模糊IF-THEN规则）与EEGNet（直接从原始EEG数据学习层次时空表示）进行对比。

Result: 个体内实验中模糊神经模型表现更好（准确率68.58%±13.76%，kappa=58.04%±18.43），跨个体测试中深度模型泛化能力更强（准确率68.20%±12.13%，kappa=57.33%±16.22）。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够整合不同模态信息的多模态框架，以提升聚合物发现过程中的检索效率和排序准确性。

Method: 首先使用基于语言的相似性检索候选聚合物，然后利用融合的多模态嵌入根据多个目标属性对候选聚合物进行排序。

Result: 通过利用两种模态中的互补知识，PolyRecommender能够在相关聚合物属性上实现高效检索和稳健排序。

Conclusion: 这项工作建立了一个可推广的多模态范式，推动了AI引导的下一代聚合物发现设计。
...
TL;DR: 提出了UME-R1，一种生成式多模态嵌入框架，通过两阶段训练策略统一嵌入任务，利用MLLMs的生成推理能力显著超越传统判别式嵌入模型。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs的嵌入模型本质上是判别式的，限制了它们从推理驱动的生成范式中获益的能力。

Method: 采用两阶段训练策略：冷启动监督微调赋予模型推理能力，使其能够生成判别式和生成式嵌入；后续强化学习增强推理并进一步优化生成式嵌入质量。

Result: 在MMEB-V2基准测试的78个任务中显著优于传统判别式嵌入模型，生成式嵌入通过利用MLLMs的强大生成推理能力带来实质性性能提升。

Conclusion: 生成式嵌入为更可解释、推理驱动的生成式多模态嵌入奠定了基础，判别式和生成式嵌入具有互补性，其组合性能远超单独使用任何一种。
...
TL;DR: 提出GGS方法解决对抗攻击在迁移场景中的利用与探索困境，通过梯度引导采样平衡攻击强度与跨模型泛化能力


<details>
  <summary>Details</summary>
Motivation: 对抗攻击在跨模型迁移时面临利用与探索的困境：传统方法过度强调攻击强度但泛化能力弱，新方法过度强调泛化但攻击强度不足

Method: 基于MI-FGSM，引入内迭代随机采样，使用前一次内迭代的梯度引导采样方向，采样幅度由随机分布决定

Result: 在多个DNN架构和多模态大语言模型上的实验表明，该方法优于当前最先进的迁移攻击方法

Conclusion: GGS方法通过梯度引导采样有效平衡了攻击强度与跨模型泛化能力，解决了对抗攻击迁移性的核心困境
...
TL;DR: 提出了Tree Training训练范式，通过树打包和梯度恢复技术，在智能体LLM训练中重用共享前缀计算，显著提升训练效率。


<details>
  <summary>Details</summary>
Motivation: 当前智能体LLM训练中，由于记忆检索和并发工具执行，任务轨迹呈现树状结构而非线性序列，但现有训练方法将树状轨迹分解为独立线性段，导致共享前缀重复计算，效率低下。

Method: Tree Training范式包含：(1) Tree Packing：高效重用轨迹间的共享计算；(2) Gradient Restoration：确保重用前缀的正确梯度传播。

Result: 在多个开源模型上的实验显示，总训练时间最多减少3.9倍，实现了更高效的智能体LLM SFT和RL训练。

Conclusion: Tree Training通过重用共享前缀计算，显著提升了智能体LLM训练的计算效率，为大规模智能体训练提供了有效解决方案。
...
TL;DR: 该论文提出了一种结构保持的物理信息神经网络框架，用于解决非线性KdV方程，通过将质量和哈密顿能量守恒直接嵌入损失函数，确保物理一致性和能量稳定性。
...
Result: 模型成功再现了KdV动力学的典型行为，包括单孤子传播、双孤子相互作用和余弦脉冲初始化，同时保持了守恒不变量。

Conclusion: 结合不变约束优化和正弦特征映射的计算高效方法，为哈密顿偏微分方程如KdV方程产生了鲁棒且能量一致的PINNs。
...
TL;DR: BOOM是一个将规划与离线学习紧密结合的强化学习框架，通过引导循环解决规划过程中数据与策略行为不一致的问题，在DeepMind Control Suite和Humanoid-Bench上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 在线规划虽然能提高强化学习的样本效率和最终性能，但规划过程中的环境交互会导致收集数据与策略实际行为之间的不一致，这会降低模型学习和策略改进的效果。

Method: 提出BOOM框架，通过引导循环紧密整合规划和离线学习：策略初始化规划器，规划器通过行为对齐来引导策略。使用联合学习的世界模型支持规划器模拟未来轨迹并提供价值目标。核心是使用规划器的非参数动作分布引导策略的无似然对齐损失，结合软价值加权机制优先考虑高回报行为。

Result: 在高维DeepMind Control Suite和Humanoid-Bench上的实验表明，BOOM在训练稳定性和最终性能方面都达到了最先进的结果。

Conclusion: BOOM通过紧密整合规划和离线学习，有效解决了规划过程中数据与策略行为不一致的问题，实现了优越的性能和稳定性。
...
TL;DR: 本文提出了一种基于ROI引导掩码策略的自监督学习方法，用于静息态fMRI基础模型训练，相比传统随机掩码方法在ADHD分类任务上提升了4.23%的准确率。


<details>
  <summary>Details</summary>
Motivation: 随着大规模脑成像数据集的增加，需要开发能够有效泛化到不同下游任务的基础模型。传统随机掩码方法缺乏对大脑解剖结构的考虑，限制了模型性能和可解释性。

Method: 使用AAL3图谱进行ROI引导的掩码策略，在4D fMRI数据上选择性掩码语义一致的大脑区域，通过区域感知的重建目标进行自监督预训练。

Result: 在ADHD-200数据集（973名受试者）上，该方法比传统随机掩码在ADHD分类准确率上提升4.23%。归因分析显示边缘系统和大脑小脑区域对重建保真度和模型表示贡献最大。

Conclusion: 在模型预训练中掩码解剖区域不仅能增强可解释性，还能产生更鲁棒和区分性的表示。未来工作将扩展到更多神经影像数据集并开发新的损失函数。
...
TL;DR: 提出基于深度自编码器的异常检测方法，用于检测企业级ETL数据流中的多种异常类型，通过编码器-解码器结构和潜在空间正则化约束实现高效异常识别。


<details>
  <summary>Details</summary>
Motivation: 解决企业级ETL数据流中经常出现的异常问题，包括延迟、缺失值、重复加载和突发异常变化等，确保企业数据处理和智能分析的可靠性。

Method: 采用深度自编码器结构，将高维输入压缩为潜在表示并重构，利用重构误差衡量异常程度；在潜在空间引入正则化约束增强特征稀疏性和分布学习。

Result: 在不同超参数设置、环境变化和数据特征下的系统分析表明，该方法在AUC、ACC、Precision和Recall指标上表现优异。

Conclusion: 基于深度自编码器的检测机制能有效捕捉企业级ETL数据流的潜在分布模式，准确识别多种异常，为企业数据处理提供可靠支持。
...
TL;DR: 该论文从理论角度解释了联邦学习中数据异构导致性能下降的原因，指出异构数据会产生不同的局部最优解，这既提高了全局目标的下界，又导致模型在训练后期在区域内振荡而非收敛到单一最优解。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习算法虽然理论上能保证收敛且实践中训练稳定，但数据异构下性能下降的原因尚不清楚。本文旨在填补这一理论空白，解释性能下降的根本原因。

Method: 引入异构客户端数据导致不同局部最优解的假设，分析该假设的两个关键后果：1) 局部最优解间的距离提高了全局目标下界；2) 训练后期全局模型在区域内振荡而非收敛。通过多任务和神经网络架构的实验验证理论。

Result: 理论分析表明数据异构确实会导致性能下降，实验验证了该理论解释的有效性。开源了相关框架fedtorch。

Conclusion: 本文为联邦学习中数据异构导致的性能下降提供了理论解释，揭示了局部最优解差异和模型振荡是主要原因，为理解和改进联邦学习算法提供了理论基础。
...
TL;DR: 提出一种基于变分自编码器(VAE)的传感器校准新方法，通过训练潜在空间作为校准输出，在保持自编码功能的同时实现传感器数据校准。


<details>
  <summary>Details</summary>
Motivation: 探索使用VAE进行传感器校准的可能性，利用其潜在空间特性来替代传统的校准方法。

Method: 使用变分自编码器架构，将潜在空间训练为校准输出，并在多传感器气体数据集上进行概念验证。

Result: 校准VAE能够同时作为校准模型和自编码器工作，从校准输出和重构输出都能产生与真实数据统计相似的结果。

Conclusion: VAE在传感器校准方面具有潜力，未来计划进行更多测试和扩展工作。
...
TL;DR: EPIC框架通过对比学习构建共享表示空间，学习模型推理能力和查询-方法兼容性，在数学推理任务中优化推理方法选择，提高准确性同时降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常生成多个候选响应并使用聚合策略选择输出答案，假设更多候选答案能带来更高准确性。本文重新审视这一假设，旨在优化推理方法选择。

Method: 引入EPIC框架，通过对比学习构建共享表示空间，捕捉模型推理能力和查询-方法兼容性，并将概率界限作为正则化器纳入效用驱动的优化中，平衡准确性和计算成本。

Result: 在多样化数学推理任务上的实验表明，EPIC能够持续选择最优推理方法，在提高准确性的同时减少计算开销。

Conclusion: EPIC框架通过理论分析和对比学习有效解决了语言模型生成中推理方法选择的关键挑战，实现了准确性与计算效率的平衡。
...
<details>
  <summary>Details</summary>
Motivation: PM2.5空气污染已成为城市地区日益严重的问题，与多种健康问题相关。预测PM2.5水平可以提供早期预警并帮助预防疾病。

Method: 设计、微调、测试和评估多种机器学习模型，包括线性回归算法、集成方法、深度学习模型（如循环神经网络和transformers）以及大型语言模型。
...
TL;DR: 提出Lo-Hp框架，通过解耦的两阶段权重生成方法解决现有方法中的过度耦合和长视野问题，采用混合策略子轨迹平衡目标来学习局部优化策略。


<details>
  <summary>Details</summary>
Motivation: 现有生成式建模方法在权重生成时存在过度耦合和长视野问题，前者限制了优化器的灵活性，后者导致推理效率低下和准确率不高。

Method: 提出Lo-Hp框架，采用解耦的两阶段权重生成方法，结合混合策略子轨迹平衡目标，整合在线和离线学习来捕捉局部优化策略。

Result: 理论证明仅学习局部优化策略可以解决长视野问题并提升全局最优权重的生成质量，在迁移学习、少样本学习、领域泛化和大语言模型适应等任务中展现出优越的准确性和推理效率。
...
TL;DR: 提出了一种基于单智能体强化学习的区域自适应交通信号控制框架，通过集中决策解决多智能体系统的协调复杂性，利用DreamerV3世界模型高效学习控制策略，显著减少排队长度并展现抗波动能力。


<details>
  <summary>Details</summary>
Motivation: 交通拥堵主要由交叉口排队引起，严重影响城市生活质量、安全、环境质量和经济效率。传统交通信号控制优化模型难以捕捉真实交通的复杂性和动态性，需要更智能的解决方案。

Method: 采用单智能体强化学习框架，使用邻接矩阵统一编码道路网络拓扑、实时排队状态和当前信号配时参数。利用DreamerV3世界模型高效学习控制策略，通过顺序选择交叉口并调整信号相位配时来调节交通流入/流出。

Result: 在SUMO仿真实验中，面对多级别（10%、20%、30%）起讫点需求波动，该框架展现出强大的抗波动能力，并显著减少了排队长度。

Conclusion: 这项工作为与探测车辆技术兼容的智能交通控制建立了新范式。未来研究将重点增强实际适用性，包括在训练中纳入随机起讫点需求波动，并探索针对突发事件的区域优化机制。
...
TL;DR: 使用时间融合变换器(TFT)对沃尔玛周销售额进行多时间范围预测，融合静态商店标识符和动态外生变量，在5周预测范围内优于基线模型，为库存规划和节假日优化提供实用价值。


<details>
  <summary>Details</summary>
Motivation: 准确的零售多时间范围预测对库存管理和促销活动至关重要，需要能够融合多种数据源并保持模型可解释性的方法。

Method: 采用时间融合变换器(TFT)模型，结合静态商店标识符和时间变化的外生信号(节假日、CPI、燃料价格、温度)，通过分位数损失生成概率预测，并提供变量选择网络、静态丰富化和时间注意力机制的可解释性。

Result: 在2012年固定测试集上，TFT达到每店周RMSE为57.9k美元，R²为0.9875；在5折时序交叉验证中，平均RMSE为64.6k美元，R²为0.9844，优于XGB、CNN、LSTM和CNN-LSTM基线模型。

Conclusion: TFT模型在零售销售预测中表现出色，不仅预测精度高，还保持了模型透明度，对库存规划和节假日期间优化具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 激活探针作为AI系统监控器成本低、延迟小，但在真实黑盒对抗压力下的鲁棒性研究不足，需要发现其失效模式并最小化测试成本。

Method: 采用轻量级黑盒红队测试流程，将现成LLM包装在迭代反馈和上下文学习中，无需微调、梯度或架构访问。

Result: 在高风险交互探针案例研究中，发现了可解释的脆弱性模式（如法律术语导致的假阳性、平淡程序语调导致的假阴性），以及在场景约束攻击下减少但仍存在的漏洞。

Conclusion: 简单的提示式红队测试框架可以在部署前预测失效模式，并为未来探针的加固提供有前景且可操作的见解。
...
TL;DR: 提出FTT-GRU混合模型，结合快速时间Transformer和GRU，用于工业机械剩余使用寿命预测，在NASA CMAPSS数据集上实现准确高效预测


<details>
  <summary>Details</summary>
Motivation: 现有方法如LSTM和CNN难以同时建模多元传感器数据的全局时间依赖性和细粒度退化趋势

Method: 使用快速时间Transformer（基于FFT的线性化注意力）与GRU层结合的混合模型，首次将FTT与GRU结合应用于RUL预测

Result: 在CMAPSS FD001上达到RMSE 30.76、MAE 18.97、R²=0.45，CPU延迟1.12ms，相比最佳基线TCN-Attention改进RMSE 1.16%、MAE 4.00%

Conclusion: 紧凑的Transformer-RNN混合模型能够在CMAPSS上实现准确高效的RUL预测，适用于实时工业预测性维护
...
Method: 提出PromptBN（无数据时使用元数据查询LLM）和ReActBN（有数据时结合ReAct推理范式与结构评分进行迭代优化），保持LLM在整个发现过程中的主动参与

Result: 实验表明该方法在低数据或无数据场景下显著优于现有LLM方法和传统数据驱动算法

Conclusion: 将LLM置于结构学习过程的核心位置，能够有效解决传统方法在数据稀缺时的局限性，为复杂系统分析提供新思路
...
TL;DR: 提出了一种结合稀疏参数估计和非参数技术的新框架，用于从数据中发现复杂系统的数学模型，特别针对系统生物学中传统参数模型难以准确描述的非线性特性。


<details>
  <summary>Details</summary>
Motivation: 传统参数模型在准确表示复杂系统中的某些非线性特性方面存在不足，特别是在系统生物学领域，这些系统的复杂性使得自下而上的建模方法往往不可行。

Method: 将稀疏参数估计与非参数技术相结合，无需事先了解非线性函数形式或扩展函数库，就能捕获Sindy算法无法描述的非线性特性。

Result: 该方法在多个复杂生物现象估计的示例中得到验证，能够有效发现传统方法难以捕捉的系统动态特性。

Conclusion: 该框架为数据驱动的模型方程发现提供了更强大的工具，特别适用于系统生物学等需要处理复杂非线性系统的领域。
...
TL;DR: 该研究评估了六种大型语言模型在脊柱外科临床决策中的幻觉风险，发现DeepSeek-R1表现最佳，但推理增强模型并未显著优于标准版本，强调了临床应用中需要整合可解释性机制。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在脊柱外科临床决策支持中具有变革潜力，但存在幻觉风险，可能危及患者安全，需要量化评估这些风险。

Method: 引入临床医生中心框架，评估诊断精度、推荐质量、推理鲁棒性、输出一致性和知识对齐，在30个专家验证的脊柱病例上测试六种领先LLM。

Result: DeepSeek-R1总体表现最佳(86.03±2.08)，在创伤和感染等高风险领域尤为突出。推理增强模型变体并未普遍优于标准版本，多维压力测试暴露了模型特定脆弱性，推荐质量在复杂度增加时下降7.4%。

Conclusion: 研究主张将可解释性机制整合到临床工作流程中，并为外科LLM部署建立安全感知验证框架。
...
TL;DR: 提出了一个统一的数据驱动框架，通过五个阶段量化并提升冰球比赛中的进攻势头和得分可能性，结果显示采用优化的事件序列和阵型可显著提高得分潜力。


<details>
  <summary>Details</summary>
Motivation: 旨在为冰球分析提供原则性、因果基础性的战术优化方法，为教练和分析师提供实时、可操作的洞察。

Method: 包含五个阶段：通过逻辑回归进行可解释的势头加权、使用梯度提升决策树进行非线性xG估计、LSTM网络进行时间序列建模、PCA和K-Means聚类进行空间阵型发现、使用X-Learner因果推断估计器量化最优事件序列和阵型的平均处理效应。

Result: 观察到ATE为0.12（95% CI: 0.05-0.17, p < 1e-50），对应得分潜力相对提升15%，表明战略结构化的序列和紧凑阵型能因果性地提升进攻表现。

Conclusion: 该框架证明了战略结构化的序列和紧凑阵型能够因果性地提升进攻表现，推动了冰球分析向原则性、因果基础性的战术优化发展。
...
TL;DR: 本文提出了一个统一的贝叶斯框架来解释大语言模型的控制方法，将上下文学习和激活引导视为改变模型对潜在概念信念的两种方式。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM控制方法（如上下文学习和激活引导）看似不同但目标一致，作者希望建立一个统一的理论框架来解释这些方法。

Method: 从贝叶斯视角建立预测性模型，认为上下文干预通过证据积累影响信念，而激活引导通过改变概念先验来影响模型行为。

Result: 该模型能准确预测LLM在多种干预下的行为，解释现有现象（如S型学习曲线），并预测新现象（如对数信念空间的干预可加性）。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于负熵正则化的在线镜像下降方法在已知转移设置下的遗憾界与√log SA成比例，这无法充分利用成本稀疏性带来的好处，因为只有少量状态-动作对产生成本。

Method: 提出了一族ℓ_r-范数正则化器（r∈(1,2)），这些正则化器能够适应稀疏性，在已知转移设置下实现与√log M成比例的遗憾。

Result: 在已知转移设置下，ℓ_r-范数正则化器实现了与√log M成比例的遗憾，这是最优的；在未知转移设置下，即使对于稀疏问题，任何学习者的极小极大遗憾都与SA成多项式比例。

Conclusion: ℓ_r-范数正则化器能够有效适应稀疏成本，在已知转移设置下实现最优遗憾界，M而非SA捕获了问题的有效维度；但在未知转移设置下，稀疏性的好处受到限制。
...
TL;DR: 研究发现稀疏的受限玻尔兹曼机(RBMs)在训练前剪枝80%连接仍能保持高质量生成性能，但训练后进一步剪枝会导致性能无法完全恢复，表明稀疏网络应在训练早期进行剪枝。


<details>
  <summary>Details</summary>
Motivation: 随着神经网络规模不断增大，计算和环境成本日益成为关注焦点。本研究旨在探索更简单、更稀疏的网络是否能够保持强大性能，特别是研究极端剪枝条件下的受限玻尔兹曼机。

Method: 基于彩票假设，研究受限玻尔兹曼机在训练前进行极端剪枝(高达80%连接)的情况，并分析训练后进一步剪枝的性能恢复能力。

Result: RBMs在训练前剪枝80%连接仍能保持高质量生成性能，但训练后进一步剪枝会破坏关键连接核心，导致生成质量急剧下降，且重新训练的网络性能不如从头开始训练的同等稀疏网络。

Conclusion: 稀疏网络要有效工作，剪枝应在训练早期实施而非训练后尝试。初始条件对网络能力具有持久影响，这为开发高效神经网络架构提供了实用见解。
...
TL;DR: FedRevive是一个异步联邦学习框架，通过数据无关知识蒸馏来缓解陈旧更新问题，在保持AFL可扩展性的同时显著提升训练效率和最终精度
...
Method: 结合参数空间聚合与轻量级服务器端数据无关知识蒸馏(DFKD)，使用元学习生成器合成伪样本进行多教师蒸馏，采用混合聚合方案结合原始更新和DFKD更新

Result: 在各种视觉和文本基准测试中，FedRevive相比异步基线方法训练速度提升高达32.1%，最终精度提升高达21.5%
...
TL;DR: 本文提出使用伴随状态方法计算生成流模型（特别是扩散模型）的梯度，应用于气候数据模拟器进行敏感性分析，将计算成本从数周大幅降低到数小时。


<details>
  <summary>Details</summary>
Motivation: 传统物理模型计算气候敏感性分析成本过高，而AI生成模型虽然评估速度快，但计算敏感性仍是瓶颈。

Method: 应用伴随状态方法计算生成流模型的梯度，以扩散模型为特例，并提出了梯度自一致性检查来验证计算敏感性。

Result: 该方法在cBottle生成模型上成功应用于海表温度的敏感性分析，计算成本从超级计算机数周降低到GPU数小时。

Conclusion: 该方法能够产生可靠的梯度，显著简化了气候科学中的关键工作流程。
...
Method: 提出KL-Adjusted Pruned Path Algorithm (KAPPA)，使用KL散度、置信度和熵构建评分函数，在探索时促进多样性并选择性剪除低分分支。

Result: 在GSM8K和MATH500数据集上的实验显示，KAPPA在较小模型中稳定性能，相比BoN减少约60%峰值内存和90%总token生成，对准确性影响最小。
...
Result: 在金融、能源和大宗商品交易领域的实验结果表明，Pub2Priv在改善隐私-效用权衡方面持续优于最先进的基准方法。

Conclusion: Pub2Priv通过有效利用异构公共知识，显著提升了隐私时间序列数据生成的隐私-效用权衡性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统知识追踪模型假设学习过程是静态的，但实际在线学习平台中学生群体和概念会随时间变化，需要评估模型对这种变化的敏感性。

Method: 使用四种知识追踪模型分析五个学年的数据，评估模型在单个学年内和跨多个学年的性能变化。

Result: 所有四种知识追踪模型都表现出性能下降，BKT模型在新数据上最稳定，而复杂的注意力模型预测能力下降最快。

Conclusion: 知识追踪模型容易受到概念漂移影响，需要进行纵向评估，BKT模型在稳定性方面表现最佳。
...
TL;DR: 该研究开发了融合卫星和雷达数据的多模态临近预报模型，用于预测5、15和30分钟内的降水。相比仅使用雷达的方法，该多模态策略显著提高了预测准确性，特别是对强降水的预测效果更好。


<details>
  <summary>Details</summary>
Motivation: 传统地面雨量计在德国仅能记录17.3%的小时强降雨事件，雷达数据虽然能有效追踪正在发生的降水，但单独用于强降雨预报仍面临挑战。需要更有效的方法来准确预测城市洪涝灾害。

Method: 开发多模态临近预报模型，结合雷达和卫星图像数据，预测5、15和30分钟内的降水。通过融合两种数据源来提升预测性能。

Result: 多模态模型显著优于仅使用雷达的方法。卫星数据的整合提高了预测准确性，特别是对强降水。在5分钟预报中，强降雨和暴雨的关键成功指数分别提高了4%和3%，且在更长预报时效下保持更高的预测能力。

Conclusion: 多模态模型能提供更详细准确的强降雨预报，实现及时可靠的灾害预警，具有拯救生命的潜力。2021年德国北莱茵-威斯特法伦州洪灾案例进一步验证了其优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列生成方法未能采用可解释的分解方法，限制了合成有意义的趋势和季节性模式的能力。

Method: 使用扩散概率模型，将趋势和季节性学习分离为不同模块：MLP结构捕捉趋势，自适应小波蒸馏实现季节性成分的多分辨率学习，并设计了全面的校正机制确保生成组件的一致性。

Result: 在八个真实世界数据集上的实证研究表明，STDiffusion在时间序列生成任务中达到了最先进的性能，并在多窗口长序列时间序列生成中表现出鲁棒性和多功能性。

Conclusion: STDiffusion通过可解释的分解方法有效提升了时间序列生成的质量和可解释性，展现出在复杂时间序列生成任务中的强大潜力。
...
TL;DR: 提出了一种基于双凸优化的PINN方法，将特征对搜索转化为双凸优化问题，使用交替凸搜索实现快速收敛，比基于梯度的PINN训练快500倍。


<details>
  <summary>Details</summary>
Motivation: 传统PINN方法求解特征值问题速度比经典数值方法慢几个数量级，需要更高效的优化策略。

Method: 将特征对搜索重新表述为双凸优化问题，使用解析最优更新的交替凸搜索算法。

Result: PINN-ACS实现了高精度，收敛速度比基于梯度的PINN训练快达500倍。

Conclusion: 该方法为特征值问题提供了快速、准确的PINN解决方案，显著提升了计算效率。
...
TL;DR: 提出PREPO方法，通过利用内在数据特性提高RLVR的数据效率，包含两个互补组件：基于提示困惑度的适应性学习和基于相对熵差异的探索优先机制。


<details>
  <summary>Details</summary>
Motivation: RLVR虽然提升了大型语言模型的推理能力，但训练成本高昂，因为许多rollouts对优化的贡献很小。本研究旨在利用内在数据特性来改善RLVR的数据效率。

Method: PREPO包含两个组件：1) 使用提示困惑度作为模型适应性指标，让模型从易到难学习；2) 通过区分相对熵来放大rollouts之间的差异，优先选择探索度更高的序列。

Result: 在Qwen和Llama模型上，PREPO在数学推理基准测试中仅需基线方法1/3的rollouts就能达到竞争性性能。

Conclusion: PREPO通过内在数据特性显著提高了RLVR的数据效率，同时保持了竞争性性能，并提供了理论分析解释其有效性。
...
TL;DR: 该论文分析了预训练Transformer在微调时出现的输出饱和和梯度抑制问题，提出了基于诊断指标的LoRA适配器注入策略来恢复被抑制的梯度信号。


<details>
  <summary>Details</summary>
Motivation: 预训练Transformer在微调时经常表现出对源模式的过度自信，难以形成新的目标域模式，这源于输出饱和导致的梯度抑制机制。

Method: 提出了层间诊断指标（注意力熵、激活梯度范数、参数梯度范数和Delta-CKA），识别拐点层，并选择性注入LoRA适配器来恢复被抑制的梯度信号。

Result: 实验表明，在过训练初始化下，拐点层LoRA注入能带来性能提升；而在欠训练初始化下，则需要全路径解阻塞才能实现低层重构。
...
TL;DR: EraseFlow是一个基于GFlowNets的概念消除框架，通过探索去噪路径空间来引导生成过程远离目标概念，同时保持模型先验知识，无需精心设计的奖励模型。


<details>
  <summary>Details</summary>
Motivation: 现有概念消除技术存在图像质量下降、依赖脆弱的对抗损失或需要大量重新训练的问题，这源于对基于扩散生成的去噪轨迹的短视理解。

Method: 将概念遗忘建模为去噪路径空间的探索，使用带有轨迹平衡目标的GFlowNets优化，通过采样整个轨迹而非单一最终状态来学习随机策略。

Result: EraseFlow在广泛实证结果中优于现有基线，在性能和先验保持之间实现了最优权衡，能有效泛化到未见概念并避免可被攻击的奖励。

Conclusion: EraseFlow通过重新构建概念消除问题为去噪路径探索，提供了一种无需精心设计奖励模型的有效解决方案，在概念消除和模型性能保持方面取得了突破。
...
TL;DR: 提出逻辑信息强化学习(LIRL)，通过逻辑投影保证混合动作空间中探索步骤的可行性，无需惩罚调优，在多个CPS场景中优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有层次化方法牺牲全局最优性，而强化学习在混合动作空间中依赖脆弱的奖励惩罚、屏蔽或防护，难以保证约束满足

Method: 为标准策略梯度算法配备投影机制，将低维潜在动作映射到由一阶逻辑动态定义的可容许混合流形上

Result: 在工业制造、电动汽车充电站和交通信号控制等场景中均优于现有方法，以机器人减速器装配系统为例，相比传统工业层次调度方法，在总工期-能耗目标上最多减少36.47%-44.33%，且始终保持零约束违反

Conclusion: 基于声明式逻辑的约束表述使框架可无缝迁移到其他领域，为大规模CPS的安全实时优化铺平道路
...
TL;DR: 提出了一个均衡策略泛化(EPG)框架，用于在对抗性游戏中学习具有跨图零样本性能的通用策略，特别针对追逃游戏(PEG)。该框架通过在不同图结构上训练RL策略来对抗每个单图的均衡策略，实现了对追逃双方的通用性。


<details>
  <summary>Details</summary>
Motivation: 传统方法在追逃游戏中需要指数时间求解，且当图结构变化时需要重新计算或微调，这限制了实时应用。需要一种能够跨图结构泛化的策略学习方法。

Method: 1) 使用动态规划算法生成纯策略纳什均衡作为单图策略的均衡预言机；2) 设计分组机制和序列模型来处理多追捕者场景；3) 提出距离特征用于跨图训练；4) 在有出口的图中使用均衡启发式方法。

Result: 实验结果表明，EPG框架在各种未见过的真实世界图中保证了理想的零样本性能。在有出口的图中，通用追捕策略甚至能与最先进方法微调后的策略性能相匹配。

Conclusion: EPG框架成功实现了追逃游戏中策略的跨图泛化，为对抗性游戏中的均衡学习提供了有效的解决方案，具有重要的实际应用价值。
...
TL;DR: 提出LL-ViT，一种集成LUT神经元的边缘优化视觉Transformer设计，通过LUT-based通道混合器减少模型大小和计算量，在保持精度的同时提升能效。


<details>
  <summary>Details</summary>
Motivation: 解决视觉Transformer在边缘FPGA上推理时的高计算、内存和能耗需求，同时克服现有LUT-based网络在视觉任务上性能不佳的问题。

Method: 基于分析发现大部分模型权重和计算来自通道混合器(MLP层)，设计了替代的LUT-based通道混合器，并开发了FPGA加速器，采用神经网络学习方法原生学习LUT函数。

Result: 在CIFAR-10上达到95.5%准确率，CIFAR-100上78.8%，Tiny-ImageNet上60.9%，消除60%模型权重和50%乘法运算，相比整数量化ViT加速器实现1.9倍能效和1.3倍低延迟。

Conclusion: LL-ViT为视觉Transformer提供了模型大小减小、计算和能效优化的边缘推理解决方案，在保持性能的同时显著提升效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有段塞流检测方法通常是离线的，需要领域专业知识，缺乏实时可解释性，难以满足工业应用需求。

Method: 集成数据探索和标注、可配置的多分类器模型训练与评估、带时间序列叠加的分类结果可视化，以及生成基于持久性警报的实时推理模块。

Result: 系统支持从标注CSV上传到未知数据集实时推理的无缝工作流程，具有轻量级、便携性和易部署性。

Conclusion: 该工具通过结合领域相关分析和新颖的UI/UX功能，弥合了数据科学方法与关键过程工业中实际决策之间的差距，在油气行业及其他时间序列故障诊断任务中具有广泛应用前景。
...
TL;DR: FlexiCache是一个分层KV缓存管理系统，利用KV头的时态稳定性来减少GPU内存使用和计算开销，同时保持模型精度。


<details>
  <summary>Details</summary>
Motivation: LLM服务受限于不断增长的KV缓存大小，现有系统难以在不降低精度的情况下有效利用注意力关键令牌的稀疏性，特别是在长文本生成场景中。

Method: 将KV头分类为稳定和不稳定：不稳定头的所有KV缓存页保留在GPU内存中，稳定头仅保留前K页在GPU上，其余卸载到主机内存，并利用时态稳定性进行周期性重排名。

Result: 在vLLM上实现，将长上下文请求的GPU内存占用减少高达70%，离线服务吞吐量提高1.38-1.55倍，在线令牌延迟降低1.6-2.1倍。

Conclusion: FlexiCache通过利用KV头的时态稳定性，在保持长上下文、长生成场景精度的同时，显著提升了LLM服务的效率和资源利用率。
...
TL;DR: 研究表明，在量化训练中增加批量大小可以补偿反向传播精度降低的影响，同时分析了权重和激活值量化对梯度方差的不同影响。


<details>
  <summary>Details</summary>
Motivation: LLM训练资源密集，量化训练能提高计算和内存效率，但会引入量化噪声，影响收敛和模型精度。随机舍入虽提供无偏梯度估计，但其与批次大小等训练因素的相互作用尚待深入研究。

Method: 通过理论和实证研究，分析小批量随机梯度下降与随机舍入的结合，探讨批量大小如何补偿反向传播精度降低，并区分权重和激活值量化对梯度方差的影响。

Result: 实验验证了理论洞察：增加批量大小确实能够补偿量化训练中精度降低带来的影响，且权重和激活值的量化以不同方式影响梯度方差。

Conclusion: 在量化训练中，通过调整批量大小可以有效地管理量化噪声，提升训练效率和模型性能，同时需要分别考虑权重和激活值量化的不同影响。
...
TL;DR: KFCPO是一种结合Kronecker分解近似曲率(K-FAC)二阶策略优化与安全感知梯度操作的安全强化学习算法，通过边界感知梯度混合和KL回滚策略，在Safety Gymnasium环境中比最佳基线方法获得10.3%-50.2%更高的平均回报。


<details>
  <summary>Details</summary>
Motivation: 解决安全强化学习中奖励最大化与约束满足之间的权衡问题，避免传统方法中固定硬阈值导致的突变和不稳定策略变化。

Method: 使用K-FAC高效近似Fisher信息矩阵进行自然梯度更新；引入边界感知梯度操作机制，根据智能体与安全边界的距离自适应调整奖励和成本梯度的影响；采用小批量级别的KL回滚策略确保信任区域合规。

Result: 在Safety Gymnasium环境中的实验表明，KFCPO相比最佳基线方法在遵守安全约束的同时，平均回报提高了10.3%到50.2%。

Conclusion: KFCPO通过结合二阶优化和智能梯度操作，在安全强化学习中实现了安全性和性能的优越平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有可解释聚类方法主要关注最小化特定聚类目标的可解释性代价，缺乏通用的方法来为任意聚类构建解释树，不受限制。

Method: 基于谱图划分设计可解释聚类算法，将先前的算法统一解释为同时在两个图中进行图划分的广义框架。

Result: 实验表明，该方法在多个数据集上相比基线方法表现出更优的性能。

Conclusion: 提出的谱图划分方法为可解释聚类提供了通用且有效的解决方案，能够适应各种聚类场景。
...
TL;DR: 该论文提出了一种基于范畴对称性感知的学习框架，通过将时间、尺度和传感器层次结构的变化因素融入特征表示结构，使模型在现实失真下保持稳定，在UCI人类活动识别基准上显著提升了分布外准确率。


<details>
  <summary>Details</summary>
Motivation: 人类活动识别面临传感器信号随上下文、运动和环境变化的挑战，需要模型在世界变化时保持稳定。

Method: 引入范畴对称性感知学习框架，捕捉信号在时间、尺度和传感器层次结构上的变化，将这些因素构建到特征表示结构中。
...
TL;DR: 该论文通过布尔函数分析研究LIF脉冲神经网络的稳定性和鲁棒性，发现宽LIF-SNN分类器具有平均稳定性，其傅里叶频谱集中在低频分量，并引入频谱简单性概念解释深度网络的简单性偏置。


<details>
  <summary>Details</summary>
Motivation: 脉冲神经网络在能效计算方面具有潜力，但其理论基础（特别是关于稳定性和鲁棒性）相比人工神经网络仍然有限，需要深入研究。

Method: 使用布尔函数分析框架研究离散时间LIF-SNN，重点关注噪声敏感性和分类任务中的稳定性，量化输入扰动对输出的影响。

Result: 主要结果表明宽LIF-SNN分类器具有平均稳定性，随机LIF-SNN偏向简单函数，实验证实这些稳定性特性在实践中持续存在。

Conclusion: 这些结果为SNN的稳定性和鲁棒性特性提供了新的理论见解，建立了频谱简单性与网络稳定性之间的联系。
...
<details>
  <summary>Details</summary>
Motivation: 虽然Transformer在各种任务中表现出强大的适应性，但其底层机制仍需深入探索。能量视角长期以来为理解神经计算提供了有价值的原则，本文旨在通过能量视角重新审视基于注意力的Transformer模型。

Method: 提出了一个统一的能量框架，包含三个关键组件：全局能量F*、能量函数Ei和梯度下降形式。将标准softmax注意力视为最小化亥姆霍兹自由能量的特殊情况，并将线性注意力自然纳入该框架。基于经典梯度下降算法，将原始注意力公式扩展到动量梯度下降、Nesterov加速梯度和牛顿法变体。

Result: 实验初步支持了基于能量框架设计注意力机制的潜力。

Conclusion: 能量视角为理解Transformer注意力机制提供了统一的理论框架，并启发了新的注意力结构设计，展示了该框架在注意力机制设计方面的潜力。
...
TL;DR: RhythmiNet是一种结合PPG和加速度计信号的多模态神经网络，通过时间与通道注意力模块增强，能够对AF、窦性心律和其他心律失常进行三分类，在噪声环境中表现优于单模态方法。


<details>
  <summary>Details</summary>
Motivation: 腕戴式PPG设备在连续心律监测中易受运动伪影和生理噪声影响，现有方法多依赖单通道PPG且仅限于二分类AF检测，无法覆盖临床中更广泛的心律失常类型。

Method: 提出RhythmiNet残差神经网络，集成时间和通道注意力模块，联合利用PPG和加速度计信号进行三分类（AF、窦性心律、其他），测试数据按运动强度分层评估鲁棒性。

Result: RhythmiNet相比仅使用PPG的基线方法在macro-AUC上提升4.3%，比基于手工HRV特征逻辑回归模型性能提升12%，证明多模态融合和注意力学习在噪声临床数据中的优势。

Conclusion: 多模态信号融合结合注意力机制能有效提升腕戴设备在真实临床环境中的心律失常分类性能，特别是在存在运动噪声的情况下。
...
TL;DR: 该论文提出了一个理论框架，从容量控制角度解释归一化在深度神经网络中的作用，证明归一化能指数级降低Lipschitz常数，从而平滑损失景观并约束网络容量。


<details>
  <summary>Details</summary>
Motivation: 归一化方法在现代深度神经网络中是基础组件，经验上已知能稳定优化动态并改善泛化，但其理论机制特别是在使用多个归一化层时的作用仍缺乏解释。

Method: 开发理论框架，通过分析未归一化DNN的Lipschitz常数特性，并与插入归一化层后的网络进行对比，从容量控制角度建立理论分析。

Result: 证明未归一化DNN可能呈现指数级大的Lipschitz常数，导致过度容量和过拟合；而归一化层能以指数速率降低Lipschitz常数，平滑损失景观并约束有效容量。

Conclusion: 归一化通过指数级降低Lipschitz常数，同时促进优化稳定性和改善泛化保证，为深度学习中归一化方法的经验成功提供了原则性解释。
...
TL;DR: 提出了一种使用优化合成数据来评估模型性能的新方法，在有限标注数据条件下通过理论推导和实验验证，相比现有基线能更准确可靠地估计测试误差。


<details>
  <summary>Details</summary>
Motivation: 在现实应用中，获取大量标注测试数据成本高昂且耗时，传统评估方法在有限标注样本下不可靠，而生成模型的发展为合成高质量数据提供了可能。

Method: 开发了考虑合成数据的泛化界限理论，提出基于理论指导的优化合成数据生成方法，用于模型评估。

Result: 在模拟和表格数据集上的实验表明，相比现有基线方法，该方法能够获得更准确可靠的测试误差估计。

Conclusion: 通过理论驱动的合成数据优化方法，可以在有限标注数据条件下实现可靠的模型性能评估，生成器质量对评估效果有重要影响。
...
TL;DR: NicheFlow是一个基于流的生成模型，用于推断连续空间切片中细胞微环境的时序轨迹，通过结合最优传输和变分流匹配来联合建模细胞状态和空间坐标的演化。


<details>
  <summary>Details</summary>
Motivation: 现有方法在单细胞水平建模细胞演化，忽略了组织中细胞状态的协调发育。需要能够理解细胞微环境在时空数据中演化的方法。

Method: 将局部细胞邻域表示为点云，使用最优传输和变分流匹配联合建模细胞状态和空间坐标的演化。

Result: 该方法成功恢复了从胚胎到大脑发育等不同时空数据集中的全局空间结构和局部微环境组成。

Conclusion: NicheFlow能够有效推断细胞微环境的时序演化轨迹，为理解组织发育和疾病进展提供了新工具。
...
Method: 采用两阶段平衡多模态学习策略：跨模态知识蒸馏预训练阶段利用强模态增强弱模态预测能力；主训练阶段采用多任务式学习机制，基于模态特定性能指标和模态间互信息动态校准梯度贡献。

Result: 该方法有效缓解了模态不平衡问题，显著提升了多模态模型的整体性能。

Conclusion: 提出的统一框架通过互信息量化和平衡学习策略，成功解决了多模态学习中的模态不平衡挑战，为生物数据等多模态分析提供了有效解决方案。
...
TL;DR: Hydra是一个双头元上下文记忆模块，通过二维递归在时间和变量维度上学习记忆模式，解决了现有时间序列模型在时间归纳偏置、多变量依赖和长序列效率方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列模型（如Transformer、MLP、线性模型）存在三个主要问题：(1)缺乏时间归纳偏置；(2)无法有效捕捉时间和变量维度的相互依赖；(3)长序列建模效率低。线性RNN虽然解决了部分问题，但仅限于单序列且会传播误差。

Method: 提出Hydra模型，采用双头元上下文记忆模块，通过二维递归在时间和变量维度上同时处理。虽然训练不可并行化，但提出了2D分块训练算法，在保持效果的同时实现10倍效率提升。

Result: 在时间序列预测、分类和异常检测等多样化任务和数据集上的实验表明，Hydra相比最先进的基线方法表现出优越性能。

Conclusion: Hydra通过创新的二维递归架构有效解决了多变量时间序列建模的关键挑战，在多个任务上实现了state-of-the-art性能，同时通过2D分块训练算法保证了训练效率。
...
<details>
  <summary>Details</summary>
Motivation: 贝叶斯优化在现实应用中评估成本高昂，需要减少评估次数以提高实用性

Method: 使用MDP先验捕获相关源任务的优化轨迹知识，通过先验拟合神经网络和模型无关元学习快速适应新任务

Result: 在Covid、Cancer基准和超参数调优任务中，ProfBO始终优于最先进方法，用更少评估获得高质量解

Conclusion: ProfBO算法实用性强，适合在评估成本高的实际应用中部署
...
<details>
  <summary>Details</summary>
Motivation: 遗传编程在符号回归中需要计算大量冗余表达式（可达总评估数的60%），通过e-graph结构可以识别和避免重复计算

Method: 基于e-graph结构的新搜索算法：从e-graph中选择表达式进行扰动，如果产生未访问表达式则插入e-graph并生成等价形式
...
TL;DR: 本文主张在智能代理系统设计中应优先考虑数据感知能力，提出了实现可靠现实世界部署的四个关键能力：主动数据获取、复杂数据处理、交互式测试数据合成和持续适应。


<details>
  <summary>Details</summary>
Motivation: 当前AI代理在自主通信、协作和使用工具方面取得进展，但在数据处理方面仍存在不足。可扩展的自主性需要代理能够持续获取、处理和发展其数据，这对可靠的实际部署至关重要。

Method: 提出了四个核心能力框架：(1)主动数据获取：自主收集任务关键知识或征求人类输入填补数据空白；(2)复杂数据处理：上下文感知和灵活处理多样化数据挑战；(3)交互式测试数据合成：从静态基准转向动态生成的交互式测试数据；(4)持续适应：迭代优化数据和背景知识以适应变化环境。

Result: 构建了一个数据感知代理的理论框架，强调数据能力应成为代理系统设计的首要考虑因素，为数据为中心的AI提供了新的发展方向。

Conclusion: 当前代理研究主要关注推理能力，但数据感知能力应成为下一代数据中心AI的前沿方向，这对智能代理在现实世界中的可靠部署具有重要意义。
...
TL;DR: 开发基于SARIMAX的短期停电预测系统，用于极端天气事件期间的停电预测，通过特征工程和鲁棒优化策略，相比基线方法RMSE提升8.4%。


<details>
  <summary>Details</summary>
Motivation: 为极端天气事件期间的短期电力中断提供准确预测，帮助电力公司提前准备和应对。

Method: 采用两阶段特征工程（数据清洗和相关性过滤），结合时间嵌入、多尺度滞后特征和天气变量作为SARIMAX模型的外生输入，使用标准化和分层拟合策略解决数据不规则性和数值不稳定性问题。

Result: 模型在24小时和48小时预测范围内优化，最终RMSE为177.2，相比基线方法（RMSE=193.4）提升8.4%。

Conclusion: 所提出的特征工程和鲁棒优化策略在极端天气相关停电预测中具有有效性，为电力系统应急管理提供了可靠工具。
...
TL;DR: 评估基于GAN的医疗数据生成模型的公平性，提出MedEqualizer框架来改善合成数据在受保护人口属性上的代表性不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 合成医疗数据可以提高数据可访问性，但需要确保在受保护属性上的公平性，避免在临床研究和决策中产生偏见结果。

Method: 使用MIMIC-III数据集评估多个GAN模型的公平性，测量子组代表性，并开发MedEqualizer模型无关增强框架来丰富代表性不足的子组。

Result: 发现合成数据中存在显著的子组代表性不平衡，MedEqualizer显著改善了合成数据的人口统计学平衡。

Conclusion: MedEqualizer为更公平和具有代表性的医疗数据合成提供了一条可行路径。
...
TL;DR: 使用COLET数据集对认知负荷进行分类，采用基于窗口的特征生成方法，比较机器学习和深度学习模型性能，发现深度学习模型（特别是表格架构）在各项指标上优于传统机器学习方法。


<details>
  <summary>Details</summary>
Motivation: 认知负荷在健康、心理学和国防等领域日益重要，需要开发有效的实时评估方法。

Method: 采用基于窗口的时间分割方法增强特征提取，然后应用机器学习和深度学习模型对不同认知负荷水平进行分类。

Result: 深度学习模型（特别是表格架构）在精确度、F1分数、准确率和分类精度方面都优于传统机器学习方法。

Conclusion: 基于窗口的时间特征提取方法有效，深度学习技术在复杂动态任务中的实时认知负荷评估具有潜力。
...
TL;DR: 该论文挑战了反向传播(BP)对最先进性能至关重要的长期假设，提出了Mono-Forward(MF)算法，在MLP架构上超越BP基准，实现更高分类精度和显著效率提升。


<details>
  <summary>Details</summary>
Motivation: 挑战反向传播作为深度学习核心算法的传统观念，探索更高效、可持续的替代方法。

Method: 从Hinton的Forward-Forward(FF)算法演进到Cascaded Forward(CaFo)，最终提出Mono-Forward(MF)算法，在相同架构和超参数优化框架下进行公平比较。

Result: MF算法在分类准确率上持续超越优化调参的BP基准，能耗降低41%，训练速度提升34%，但实际内存效率可能被开销抵消。
...
<details>
  <summary>Details</summary>
Motivation: 现有公平性度量缺乏直观性，希望通过幸福度这一更人性化的视角来定义公平性，同时保持数学严谨性。

Method: 使用线性规划方法计算最优公平后处理策略，将公平性定义为各群体从决策结果中获得的效用（幸福度）。

Result: 方法高效且可扩展，能够统一和扩展多种已知公平性定义，在多样化场景中表现出实际优势。

Conclusion: 基于幸福度的公平性框架提供了直观且数学严谨的公平性度量，通过线性规划实现高效计算，具有实际应用价值。
...
TL;DR: 该论文挑战AI研究中的"规模原教旨主义"，提出以能力-资源比而非单纯能力为导向的LLM开发新范式。通过分析梯度影响模式，发现仅更新高影响力参数比全参数调优更高效，梯度范数可作为识别高影响力组件的有效代理，参数与数据协调选择可大幅降低资源需求。


<details>
  <summary>Details</summary>
Motivation: 挑战当前AI研究中主导的"规模原教旨主义"，该趋势导致不可持续的环境影响和资源不平等加剧。旨在重新定位LLM开发方向，从单纯追求能力转向能力-资源比优化，实现更可持续和公平的AI发展。

Method: 提出基于梯度影响模式的理论框架，分析transformer模型中参数影响力的重尾分布特性。开发两阶段范式：基础开发者的边际收益预训练和下游用户的影响引导适应，通过梯度蓝图连接两者。使用梯度范数作为计算高效的代理来识别高影响力组件。

Result: 理论分析表明：仅更新高影响力参数在性能-资源比上严格优于全参数调优；简单梯度范数可有效识别高影响力组件；参数与数据协调选择可实现数量级资源需求减少。

Conclusion: 能力-资源比视角将硬件变通方案转化为理论最优策略，既能民主化尖端AI能力访问，又能显著减少环境影响。通过将资源意识嵌入模型开发、适应和评估过程，可重塑AI进步走向更可持续和公平的未来。
...
TL;DR: ATLAS提出了一种双智能体架构，通过分离推理（教师）和执行（学生）功能，实现无需梯度更新的持续学习，在推理时动态调整操作策略。


<details>
  <summary>Details</summary>
Motivation: 传统持续学习方法依赖基于梯度的重训练，不适合需要实时适应的部署智能体。ATLAS旨在实现梯度自由的持续学习，将适应重点从模型参数转移到系统级编排。

Method: 采用双智能体架构：教师负责推理，学生负责执行，包含持久学习记忆存储经验提炼的指导。通过编排层在推理时动态调整操作策略（如监督级别或初始计划选择）。

Result: 在微软ExCyTIn-Bench基准测试中，使用GPT-5-mini作为学生，ATLAS达到54.1%成功率，比更大的GPT-5高出13%，同时成本降低86%。跨事件验证显示泛化能力：冻结的指导手册将准确率从28%提升至41%，无需重训练。
...
TL;DR: 提出了一种基于Kolmogorov-Arnold网络的温和生成变体神经算子模型，用于求解具有随机终止时间的二阶倒向随机微分方程(2BSDEs)族。


<details>
  <summary>Details</summary>
Motivation: 解决在规则有界欧几里得域上具有随机终止时间的无限族二阶倒向随机微分方程的求解问题，并探索神经算子模型在这类问题中的近似能力。

Method: 使用Kolmogorov-Arnold网络构建生成变体神经算子模型，通过结构化子类分析来降低参数需求。

Result: 证明了2BSDEs族的解算子可由适当的神经算子模型近似，并识别出一个结构化子类，其神经算子近似仅需多项式数量的参数，而非一般最坏情况下的指数需求。

Conclusion: 该研究为无限族2BSDEs的高效数值求解提供了理论保证，展示了神经算子在特定结构化问题中的参数效率优势。
...
TL;DR: 提出了一种新的搜索方向，使一阶和零阶随机在线双层优化算法无需窗口平滑即可实现次线性随机双层遗憾，提高了效率并减少了超梯度估计的oracle依赖。
...
Result: 实验验证了该方法在在线参数损失调整和黑盒对抗攻击中的有效性。
...
Method: 应用几何不变量理论和Kempf-Ness定理，将训练动态分解为两个梯度流：纤维上的正则化流（可通过矩映射精确求解）和平衡流形上的学习流。

Result: 建立了L2正则化器在平衡流形上最小化的理论，实现了训练动态的分解，并提供了正则化流的精确解析解。

Conclusion: 该方法为深度学习和线性系统理论中的平衡性概念提供了统一的数学框架，有助于从模型简化和贝叶斯角度理解平衡性。
...
Method: 采用两阶段策略：离线阶段使用元学习在单一源域上训练模型，使其能泛化防御未见过的攻击；在线阶段应用领域适应将模型特征与新目标域对齐，无需大量标注数据。

Result: 该框架显著提高了调制分类在组合威胁下的准确率。
...
TL;DR: 本文研究了多治疗场景下的提升建模，提出了正交函数适应（OFA）方法，相比现有方法在效果和鲁棒性方面都有显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前多治疗提升建模方法通常从二值治疗方法改编而来，但在各种数据特征下（噪声数据、混合观测数据等）无法保持有效性。

Method: 提出了基于函数逼近定理的正交函数适应（OFA）方法，将现有模型适配技术分为结构适配和特征适配两类。

Result: 实验结果表明，OFA方法相比其他基础适配方法能显著提升提升模型性能，并展现出最高的鲁棒性。

Conclusion: OFA方法在多治疗提升建模中具有优越的性能和鲁棒性，为解决复杂数据特征下的治疗效果估计问题提供了有效方案。
...
TL;DR: 本文研究了思维链(CoT)是否能增强transformer模型在所有推理任务中的能力，发现推理本质上是记忆问题，并证明了在某些推理任务中CoT不会增强transformer的能力。
...
Method: 通过分析固定精度transformer（有/无CoT）记忆有限推理数据集的能力，给出必要充分条件，并计算参数数量的上下界。

Result: 证明了有/无CoT的transformer记忆有限数据集所需参数数量均为Θ(N)，且两种条件互不蕴含，表明存在CoT无法增强推理能力的任务。

Conclusion: CoT并不能在所有推理任务中增强transformer的能力，某些简单无限数据集无法被有/无CoT的transformer记忆。
...
TL;DR: 提出一个基于多任务RF信号分类的鲁棒框架，用于共享频谱环境中的发射机识别和协议分类，使用CNN处理重叠信号特征和环境变化，在POWDER平台数据上取得优异性能。
...
Method: 设计了FEval-TTC协议，标准化少样本提示和答案提取过程，支持在多个LLM和多样化数据集上评估测试时计算方法，并提供成本建模程序。

Result: 开发了开源评估框架，减少了研究者的时间和金钱开销，促进了测试时计算方法的公平比较。
...
TL;DR: 提出了一种结合深度强化学习和基于代理模拟的框架，用于优化电动汽车充电站布局，通过混合奖励函数和双Q网络来应对现实世界的不确定性。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在充电站布局优化中面临确定性奖励系统的限制，无法充分捕捉现实世界的动态性和不确定性，导致评估成本高且不反映真实场景。
...
Conclusion: 该可扩展且自适应的解决方案增强了电动汽车基础设施规划，有效应对现实世界的复杂性并改善用户体验。
...
TL;DR: WindMiL是一个结合系统数据集生成和对称感知图神经网络的机器学习框架，用于高效预测建筑物风荷载，相比传统方法大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统风洞测试和大涡模拟计算成本过高（每个LES案例需24小时），难以进行大规模参数研究，需要开发高效的风荷载预测方法。

Method: 1) 通过符号距离函数插值生成屋顶几何形状，模拟462个LES案例构建大规模数据集；2) 开发反射等变图神经网络，确保在镜像几何下的物理一致性预测。

Result: 在插值和外推评估中，WindMiL对表面压力系数的均值和标准差均达到高精度（均值Cp的RMSE≤0.02），在反射测试中保持96%以上的命中率，而非等变基线模型下降超过10%。

Conclusion: 通过结合系统数据集和等变代理模型，WindMiL能够实现高效、可扩展且准确的建筑物风荷载预测。
...
TL;DR: 变量消除算法通过重塑优化景观，将原始问题中的鞍点转化为简化问题中的局部最大值，从而显著提升非凸优化的收敛性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 理解变量消除算法在实践中表现出优越收敛性和鲁棒性的原理，特别是在处理大规模非凸优化问题时如何有效导航复杂能量景观。

Method: 基于Hessian惯性和Schur补的严格几何分析，比较原始和简化公式的优化景观，证明变量消除如何重塑临界点结构。

Result: 证明了简化景观中的局部最大值直接对应于原始公式中的鞍点，在非凸矩阵分解、双参数神经网络和深度残差网络训练中均验证了方法的有效性。

Conclusion: 通过鞍点变换实现景观简化是一个强大原则，可以指导设计更鲁棒高效的优化算法新世代。
...
TL;DR: KAT-GNN是一个结合临床知识和时序动态的图神经网络框架，用于电子健康记录的风险预测，在冠心病和院内死亡率预测任务中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录数据具有异构性和不规则时序特性，给临床风险预测带来挑战，需要有效整合临床知识和时序动态。

Method: 构建模态特定的患者图，通过SNOMED CT本体和共现先验进行知识增强，使用时序感知transformer捕捉纵向动态。

Result: 在冠心病预测任务中AUROC达到0.9269±0.0029，在MIMIC-III和MIMIC-IV死亡率预测中分别达到0.9230±0.0070和0.8849±0.0089，优于GRASP和RETAIN等基线方法。
...
TL;DR: 提出了一种新颖的在线鲁棒张量恢复算法，用于智能交通系统中的流量数据恢复，能够同时处理缺失值和异常值，在保持高恢复精度的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统批处理方法计算和存储需求大，难以应对不断增长的交通数据量；现有的在线张量恢复方法在复杂现实场景中性能下降严重，未能充分利用交通数据的内在结构特性。

Method: 将交通数据恢复问题重新表述为流式框架，提出在线鲁棒张量恢复算法，同时利用交通数据的全局时空相关性和局部一致性。

Result: 在三个真实交通数据集上的实验表明，该方法实现了高恢复精度，同时计算效率比最先进的批处理方法提高了三个数量级。
...
TL;DR: STAN是一种对抗性时空注意力网络，通过级联注意力块联合建模脑部空间连接性和时间神经动态，用于癫痫发作预测，在两个EEG数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 癫痫发作预测在医疗时间序列预测中面临关键挑战，需要高灵敏度、低误报率和个体特异性适应能力。现有方法假设固定的发作前持续时间或分别处理时空特征，无法有效捕捉时空模式间的双向依赖关系。

Method: 提出STAN网络，通过交替的时空模块级联注意力块联合建模空间脑连接性和时间神经动态。采用带梯度惩罚的对抗训练，从明确定义的15分钟发作前窗口学习发作间期和发作前状态的稳健区分。

Result: 在两个基准EEG数据集上表现优异：CHB-MIT头皮数据集（8名受试者，46个事件）达到96.6%灵敏度和0.011次/小时误报率；MSSM颅内数据集（4名受试者，14个事件）达到94.2%灵敏度和0.063次/小时误报率。计算效率高（230万参数，45毫秒延迟，180MB内存），适合实时边缘部署。

Conclusion: STAN能够捕捉细微的发作前动态，无需个体化训练即可在受试者特定时间（通常发作前15-45分钟）触发可靠警报。该框架为医疗和其他时间序列领域的时空预测提供了通用范式，其中个体异质性和可解释性至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 纳米孔蛋白测序产生长而嘈杂的电流迹线，其中包含关键的分子阶段如蛋白捕获和易位。手动识别捕获阶段耗时且需要领域专业知识，通常需要专家花费数天时间进行标注。

Method: 开发了轻量级一维卷积神经网络（1D CNN），在降采样的信号窗口上训练以检测捕获阶段。与CNN-LSTM混合模型、基于直方图的分类器和其他CNN变体进行比较评估。

Result: 最佳模型CaptureNet-Deep在保留测试数据上实现了0.94的F1分数和93.39%的精确度。模型支持低延迟推理，并集成到牛津纳米孔实验的仪表板中。

Conclusion: 结果表明，使用简单、可解释的架构可以实现高效、实时的捕获检测，并表明轻量级机器学习模型在测序工作流程中具有更广泛的应用潜力。
...
TL;DR: 提出一种新的神经网络控制李雅普诺夫函数（CLF）设计方法，将Lyapunov条件作为归纳偏置，实现CLF和控制器的端到端学习，相比现有方法具有更高的收敛率和更大的吸引域。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习-验证框架的深度CLF方法将Lyapunov条件作为复杂约束进行优化，难以实现全局收敛且验证实现复杂。需要改进这一框架以提升学习效率和性能。

Method: 将Lyapunov条件作为归纳偏置，设计神经网络CLF和基于CLF的控制器，实现有限约束下的稳定优化过程，支持CLF和控制器的端到端学习。

Result: 在大量实验案例中，该方法相比现有方法在CLF学习方面实现了更高的收敛率和更大的吸引域（ROA），并深入揭示了先前方法学习过程中成功率下降的原因。

Conclusion: 通过将Lyapunov条件作为归纳偏置的设计方法，有效改进了CLF学习框架，实现了更稳定和高效的CLF和控制器联合学习。
...
TL;DR: 该论文探索使用数据驱动的Koopman方法来建模无人机轨迹动态，以解决飞行自组织网络(FANETs)中高度动态环境带来的挑战，通过集中式和分布式方法预测通信质量。
...
Method: 利用Koopman算子理论，提出集中式和分布式两种方法，通过预测无人机轨迹动态来建模信号干扰加噪声比(SINR)，确保无人机间的可靠通信。

Result: 结果表明这些方法能够准确预测导致通信中断的连接和隔离事件，为无人机基于预测调度传输提供可能。

Conclusion: 数据驱动的Koopman方法能够有效解决FANETs中的动态环境挑战，提高网络性能并支持可靠的通信调度。
...
TL;DR: LSHFed是一个鲁棒且通信高效的联邦学习框架，通过局部敏感哈希将高维梯度压缩为二进制表示，在保护隐私的同时有效检测恶意梯度攻击，显著降低通信开销。
...
Conclusion: LSHFed在增强聚合鲁棒性和隐私保护的同时，显著提升了联邦学习的通信效率，为信任缺失环境提供了有效解决方案。
...
TL;DR: 提出基于去噪扩散概率模型(DDPM)的云原生网络功能(CNF)放置方法，将放置问题重新概念化为图到分配任务，通过图神经网络迭代优化分配矩阵，实现快速可行的解决方案。


<details>
  <summary>Details</summary>
Motivation: 传统方法（混合整数非线性规划、启发式算法和强化学习）在可扩展性、约束处理和泛化能力方面存在局限，需要新的解决方案来处理云连续体中CNF的放置问题。

Method: 将CNF放置问题编码为异构图，训练图神经网络去噪器迭代优化噪声CNF到云的分配矩阵，在损失函数中直接整合约束特定损失以学习可行解空间。

Result: 在多种拓扑结构上的广泛评估证实，该模型始终产生可行解，推理速度比MINLP求解器快几个数量级。

Conclusion: 基于扩散的生成建模在约束网络嵌入问题中具有巨大潜力，为实现分布式云原生网络功能的可扩展编排做出了贡献。
...
TL;DR: 提出MiniFool算法，一种基于物理启发的对抗攻击方法，用于测试粒子物理和天体物理中的神经网络分类任务，并应用于MNIST数据集和CMS实验数据。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够量化神经网络分类决策鲁棒性的对抗攻击方法，特别针对粒子物理和天体物理实验中的分类任务。

Method: 通过最小化结合χ²检验统计量和目标分数偏差的成本函数来生成对抗样本，其中检验统计量基于实验不确定性量化扰动的概率。

Result: 发现正确分类和错误分类事件的分类翻转概率不同，通过攻击参数缩放实验不确定性可以量化网络决策的鲁棒性。

Conclusion: MiniFool算法能够有效测试神经网络分类器的鲁棒性，并适用于未标记实验数据的分类稳健性评估。
...
<details>
  <summary>Details</summary>
Motivation: 分割学习虽然有利于数据或资源分离设备间的协作训练，但缺乏验证各方计算正确性和诚实性的能力。

Method: 在服务器端的前向传播和反向传播中为双方生成zk-SNARK证明和验证，保证双方的可验证性。

Result: 与仅记录更新但不生成零知识证明的区块链系统相比，zk-SNARK测试实现了可验证性和正确性，而区块链虽然轻量但不可验证。

Conclusion: 应用zk-SNARK证明能够有效解决分割学习中的可验证性问题，确保协作学习的可信度。
...
TL;DR: 本文提出了一种基于重参数化的多模态强化学习算法，通过距离多样性正则化解决传统确定性或单峰高斯策略在多模态决策场景中的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统连续深度强化学习算法使用确定性或单峰高斯策略，无法表达复杂的多模态决策分布，这在多样性关键场景中会限制性能。现有基于扩散或摊销策略的多模态RL算法存在难以平衡性能、决策多样性和效率的问题。

Method: 首先将现有难以处理的多模态策略统一框架化，证明可以通过重参数化直接使用策略梯度优化；然后提出基于距离的多样性正则化方法，无需显式计算决策概率；在多样性关键领域（多目标达成和生成RL）验证方法优势。

Result: 在多样性关键领域展示了多模态策略的优势，特别是在少样本鲁棒性方面；在传统MuJoCo基准测试中表现出竞争力；实验表明摊销策略是具有强大多模态表达能力和高性能的有前景的策略模型类别。

Conclusion: 提出的方法成功解决了多模态RL中性能、多样性和效率的平衡问题，证明了多模态策略在多样性关键场景中的价值，并为摊销策略作为有效策略模型提供了实证支持。
...
TL;DR: 应用机器学习遗忘技术到LeNet神经网络，通过移除FGSM对抗攻击的扰动数据来提升模型鲁棒性。
...
Method: 在LeNet神经网络上应用遗忘技术，通过重新训练模型来移除FGSM攻击添加的扰动数据。

Result: 遗忘FGSM攻击显著提高了LeNet网络对此类攻击的鲁棒性。

Conclusion: 机器学习遗忘技术是防御FGSM对抗攻击的有效方法，能够显著提升模型的安全性。
...
TL;DR: 提出了第一个真正实现在位计算的实数域FFT框架(rdFFT)，通过利用蝴蝶操作对称性和频域共轭特性，设计隐式复数编码方案，完全消除中间缓存使用。


<details>
  <summary>Details</summary>
Motivation: 现有的FFT实现（包括标准FFT和实数FFT）无法实现真正的在位计算，特别是实数FFT将大小为n的输入映射到大小为n/2+1的复数输出，导致维度不匹配并需要额外的内存分配。

Method: 利用蝴蝶操作对称性和频域共轭特性，设计隐式复数编码方案，保持输入输出内存空间一致性，完全消除中间缓存使用。
...
TL;DR: 提出基于图的卫星嵌入方法，通过建模空间关系和引入模糊标签损失来改进撒哈拉以南非洲的财富指数预测，提高泛化能力。


<details>
  <summary>Details</summary>
Motivation: 全球南方地区缺乏准确细粒度的贫困地图，DHS调查数据空间覆盖有限且坐标因隐私保护被随机位移，降低了数据质量。

Method: 使用低维AlphaEarth卫星嵌入，通过图结构建模调查点和未标记位置的空间关系，引入概率模糊标签损失来处理坐标位移问题。

Result: 在37个DHS数据集上的实验表明，相比仅使用图像的基线方法，加入图结构略微提高了预测准确性。

Conclusion: 紧凑的地球观测嵌入在大规模社会经济制图中具有潜力，图结构方法能改善财富预测的泛化性能。
...
TL;DR: 提出CG-FKAN方法，通过稀疏化和压缩扩展网格来降低联邦学习中KAN模型的通信开销，在通信受限环境下比固定网格KAN降低13.6%的RMSE。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在隐私敏感应用中广泛使用，但存在可解释性有限的问题。KAN网络通过可学习的样条函数解决了这一限制，但现有FL研究应用KAN时忽略了网格扩展引入的通信开销。
...
TL;DR: 提出了一种新的输入空间曲率度量λ，通过高阶导数指数增长率来量化神经网络泛化能力和鲁棒性，相比参数空间度量更易解释且参数化不变


<details>
  <summary>Details</summary>
Motivation: 现有曲率度量通常在参数空间定义（如Hessian特征值），存在计算昂贵、对重参数化敏感、难以功能解释等问题，需要更直观的输入空间曲率度量

Method: 引入曲率率λ作为输入空间标量曲率度量，定义为高阶输入导数的指数增长率，通过log ||D^n f||与n的斜率估计，并提出曲率率正则化(CRR)来直接塑造输入空间几何

Result: 在解析函数和神经网络上的实验表明，λ在训练过程中可预测地演化，CRR与SAM相比达到相似精度，但产生更平坦的输入空间几何和更好的置信度校准

Conclusion: λ通过微分动力学将曲率基础化，为学习模型提供了紧凑、可解释且参数化不变的功能平滑度描述符
...
<details>
  <summary>Details</summary>
Motivation: 现有图曲率方法中，Ollivier-Ricci曲率虽然几何解释性强，但计算复杂度极高，限制了其在大规模图数据集上的应用。

Method: 通过节点对之间的有效电阻来量化图边上消息传递的难易程度，替代了最优传输距离的计算方法。

Result: 在多种GNN任务上的实验表明，该方法在保持与Ollivier-Ricci曲率相当性能的同时，大幅降低了计算开销。

Conclusion: 有效电阻曲率是一种计算高效且几何表达能力强的图曲率度量，能够有效替代Ollivier-Ricci曲率用于图神经网络。
...
TL;DR: DAMBench是首个用于评估数据驱动数据同化模型的大规模多模态基准测试，整合了高质量背景状态和真实世界多模态观测数据，旨在解决现有研究中过于简化的场景和缺乏标准化基准的问题。


<details>
  <summary>Details</summary>
Motivation: 传统数据同化方法虽然有效，但深度学习提供了更可扩展、高效和灵活的替代方案。然而现有深度学习数据同化研究存在两个关键局限：依赖过于简化的合成观测场景，以及缺乏标准化基准进行公平模型比较。

Method: 开发DAMBench基准测试，整合最先进预报系统的高质量背景状态和真实世界多模态观测数据（气象站和卫星图像），所有数据重采样到统一网格并进行时间对齐，提供统一评估协议并基准测试代表性数据同化方法。

Result: 建立了严格的研究基础，促进可重复性、公平比较和扩展到真实世界多模态场景的能力，通过综合实验验证了基准的有效性。

Conclusion: DAMBench为未来研究提供了严谨的基础，展示了如何整合真实观测数据来增强即使是简单基线模型，数据集和代码已公开可用。
...
TL;DR: CLP-SNN是一种基于脉冲神经网络的在线持续学习架构，在Intel Loihi 2芯片上实现，通过事件驱动稀疏学习、自归一化学习规则和神经发生机制，在边缘设备上实现了高效且无排练的持续学习。


<details>
  <summary>Details</summary>
Motivation: 解决边缘AI设备在开放世界中面临的数据分布漂移和新类别出现的挑战，传统离线训练无法适应非平稳数据流，而在线持续学习在功耗受限环境下仍具挑战性。

Method: 提出CLP-SNN架构：1）事件驱动和时空稀疏的局部学习；2）自归一化三因子学习规则保持权重归一化；3）集成神经发生和元可塑性实现容量扩展和遗忘缓解。

Result: 在OpenLORIS少样本学习实验中，CLP-SNN达到与排练方法相当的准确率，同时实现显著效率提升：比边缘GPU上最佳OCL方法快70倍（0.33ms vs 23.2ms），能效高5600倍（0.05mJ vs 281mJ）。
...
TL;DR: MS-HGFN模型通过分层图神经网络和多尺度时空特征融合，有效提升股票市场预测精度，在美中股市数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有多尺度图神经网络在股票预测中忽视了两个关键问题：股票内部属性的细微模式对股票间相关性的影响，以及在多尺度采样中对粗粒度和细粒度特征的偏置关注。

Method: 提出MS-HGFN模型，包含分层GNN模块学习不同时间尺度下股票内部属性模式和外部属性特征，形成动态图；采用自上而下的门控机制融合多尺度时空特征。

Result: 在美中股市真实数据集上的实验表明，MS-HGFN相比传统和先进模型预测精度提升高达1.4%，且在收益模拟中表现出更强的稳定性。

Conclusion: MS-HGFN通过全面捕捉时空依赖关系和多尺度特征融合，为股票市场预测提供了有效的解决方案。
...
TL;DR: 提出基于Hadamard卷积变换的时间序列分类方法，相比现有SOTA方法显著提升计算效率和性能，在UCR数据集上F1分数比ROCKET提升至少5%，训练时间比miniROCKET缩短50%。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列分类方法如HIVE-COTE、Proximity Forest等计算复杂度高，参数调优和训练周期长；轻量级方法如ROCKET在核选择和计算开销方面仍有改进空间。

Method: 使用Hadamard矩阵的列向量或行向量作为不同长度的卷积核，利用核的正交性提升计算效率、鲁棒性和适应性，与现有方法完全兼容。

Result: 在多领域数据集上实现SOTA性能：F1分数比ROCKET提升至少5%，训练时间比最快的miniROCKET变体缩短50%，可在超低功耗嵌入式设备上部署。

Conclusion: 基于Hadamard卷积变换的特征提取方法在保持兼容性的同时，显著提升了时间序列分类的计算效率和性能。
...
TL;DR: 提出了一种并行解耦框架PDF，通过利用MLLMs的可控性生成多个并行嵌入路径，解决了传统SSC范式将丰富多模态输入压缩为单一嵌入的问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态嵌入模型受限于SSC范式（单输入、单一嵌入、对比监督），无法充分利用MLLMs的能力，将多方面的输入信息压缩为单一嵌入表示。

Method: 使用共享MLLM骨干网络，通过不同的可学习前缀生成多个并行路径，利用互信息最小化促进路径多样性，并结合每路径对比监督保持语义对齐。

Result: 在MMEB基准测试中显著提升性能：VLM2Vec-LLaVA-1.6-LR模型提升+8.9%，VLM2Vec-Qwen2VL模型分别提升+4.2%和+3.1%。2B模型仅用一半计算预算就超越基线+2.6%。

Conclusion: PDF框架能够生成具有鲁棒语义覆盖和泛化能力的嵌入空间，推理时仅需单次前向传播，计算开销可忽略。
...
<details>
  <summary>Details</summary>
Motivation: 当前冲击能量预测方法受限于数据稀疏、信号噪声、复杂特征依赖、非线性动力学等挑战，需要开发更准确且可解释的预测模型。

Method: 结合物理知识设计特征，从时域、频域和时频域提取互补特征，通过统计显著性、相关性过滤、降维和噪声鲁棒性的结构化特征选择过程，训练全连接神经网络。

Result: 与传统时间序列技术和纯数据驱动模型相比，冲击能量预测误差减少了三倍，显著提高了预测精度。

Conclusion: 该物理信息框架通过嵌入领域知识和结构化特征选择，产生了具有统计鲁棒性和物理意义的紧凑特征集，实现了可解释且可追溯的冲击能量预测。
...
TL;DR: 本文重新审视了托普利兹协方差估计问题，通过过参数化梯度下降方法，证明了当使用2P或4P个复正弦波参数化时，梯度下降能够从随机初始化实现全局收敛，且性能可与最先进方法媲美。


<details>
  <summary>Details</summary>
Motivation: 受深度学习中使用过参数化模型和简单梯度下降取得成功的启发，重新审视托普利兹协方差估计问题，探索简单梯度下降方法在此传统优化问题中的潜力。

Method: 将P×P协方差矩阵建模为K个复正弦波的加权和，通过梯度下降优化参数。研究了不同参数化程度（K=P、2P、4P）的效果，并提出了具有分离学习率的加速梯度下降变体。

Result: 当K=P时梯度下降可能收敛到次优解，但适度过参数化（K=2P或4P）能够实现从随机初始化的全局收敛。当频率固定仅优化幅度时，优化景观是渐近良性的。数值实验显示该方法在挑战性设置下可与最先进方法相媲美。

Conclusion: 过参数化梯度下降为托普利兹协方差估计提供了一种简单、可扩展且有效的替代方案，能够在保持简单性的同时达到或超过现有方法的精度。
...
TL;DR: GLM是一个多代理的图链式思维系统，通过分解推理任务、优化LLM服务架构，显著提升了图结构知识推理的准确性、效率和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有的Graph-CoT方法存在准确性低、令牌使用过多、延迟高和吞吐量低的问题，主要由于单代理整体提示、重复上下文重新编码和低效的服务执行。

Method: 将推理分解为分类、推理、动作生成和图检索等专门代理，实现分支和选择性上下文共享；引入图特定的KV缓存管理、基于优先级的驱逐和流水线执行等优化机制。

Result: GLM将答案准确性提升高达38%，令牌成本降低95.7%，推理延迟降低90.3%，吞吐量提高15.1倍。

Conclusion: GLM系统通过多代理架构和优化的LLM推理机制，实现了高效、可扩展的复杂现实世界图结构推理。
...
TL;DR: XTNet是一个用于多类别多值处理效应估计的新型网络架构，通过交叉效应估计模块和动态掩码机制捕获处理间交互，无需限制性结构假设，在合成和真实数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 反事实因果推断在处理多类别、多值处理时面临重大挑战，现有方法局限于二元或单一类型处理，存在假设限制、可扩展性不足和评估框架不完善等问题。

Method: 提出XTNet架构，包含交叉效应估计模块和动态掩码机制，采用分解策略分离基本效应与交叉处理交互，能够高效建模组合处理空间，并提出MCMV-AUCC评估指标。

Result: 在合成和真实数据集上的广泛实验表明，XTNet在排序准确性和效应估计质量上始终优于最先进的基线方法，真实A/B测试结果进一步证实其有效性。

Conclusion: XTNet为多类别多值处理效应估计提供了有效的解决方案，能够准确捕获复杂处理交互效应，在真实场景中具有实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 解决CLIP模型在少样本微调中面临的收敛慢、超参数敏感和OOD泛化差的问题，传统一阶优化方法在非凸损失函数中效果有限

Method: 使用卡尔曼滤波器对自然梯度下降进行贝叶斯近似，结合二阶优化的曲率信息和贝叶斯推断的不确定性量化
...
TL;DR: 提出联合优化用户关联和资源分配的框架，支持并行推测解码，在移动边缘计算中降低LLM推理延迟


<details>
  <summary>Details</summary>
Motivation: 移动设备上LLM推理需求增长，需要高效的移动边缘计算解决方案。推测解码虽然能分割token生成，但存在通信开销和异步延迟问题

Method: 使用多智能体深度强化学习算法解决用户关联和资源分配问题，在Sionna模拟器中进行实验评估

Result: 方法实现了最高28.0%、平均23.7%的端到端延迟降低，且不损害推理准确性

Conclusion: 该框架能够在MEC系统中实现可扩展和低延迟的LLM服务
...
TL;DR: 本文研究了在高度波动的边缘环境中联邦学习的模型准确性与公平性之间的权衡，通过实证评估比较了公平性客户端选择算法与随机和贪婪选择方法在公平性、模型性能和时间方面的表现。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在边缘智能中具有重要作用，但边缘环境的波动性和客户端能力异构性给实现高准确性和公平参与带来了挑战，需要研究准确性与公平性之间的权衡关系。

Method: 使用三种基准数据集（CIFAR10、FashionMNIST和EMNIST），对基于公平性的客户端选择算法（如RBFF和RBCSF）与随机和贪婪选择方法进行广泛的实证评估。
...
TL;DR: 提出一种基于合成数据交换而非模型参数共享的分布式学习方法，通过将本地模型视为黑盒来处理异构数据和模型，并利用博弈论框架证明纳什均衡的存在性和收敛性。


<details>
  <summary>Details</summary>
Motivation: 解决分布式学习中处理异构本地模型和数据的挑战，利用生成模型的成功来避免直接共享模型参数。

Method: 使用合成数据交换替代参数共享，将本地模型视为黑盒，支持半监督学习和不同概率空间的本地模型，通过博弈论框架制定学习过程。

Result: 证明了指数族本地模型存在唯一纳什均衡，提出的学习方法收敛到该均衡，在图像分类和条件生成基准数据集上展示了优势。

Conclusion: 基于合成数据交换的方法能有效处理分布式学习中的异构性问题，为不同模态数据的协同学习提供了可行方案。
...
TL;DR: 提出了HyperNQ，首个基于超图神经网络的QLDPC解码器，通过超边捕获高阶稳定子约束，在伪阈值以下区域比BP和GNN解码器显著提升逻辑错误率。


<details>
  <summary>Details</summary>
Motivation: 传统BP解码QLDPC码在短环存在时收敛差，而GNN解码器受限于Tanner图的成对交互，无法捕获高阶相关性。

Method: 使用超图神经网络和两阶段消息传递方案，利用超边捕获高阶稳定子约束，实现高表达性和紧凑解码。

Result: 在伪阈值以下区域，HyperNQ比BP提升逻辑错误率84%，比GNN策略提升50%，性能优于现有最先进解码器。

Conclusion: HyperNQ通过超图神经网络有效解决了QLDPC码解码中的高阶约束捕获问题，显著提升了解码性能。
...
TL;DR: 提出了网络化专家混合系统（NMoE），通过客户端协同推理和联邦学习框架解决大型AI模型在边缘设备上的训练和部署挑战。


<details>
  <summary>Details</summary>
Motivation: 大型AI模型对计算资源和训练数据的高需求与边缘设备有限的存储和计算能力存在冲突，阻碍了在边缘端的训练和部署。

Method: 引入网络化专家混合系统，客户端根据专家能力将任务分发给合适邻居并聚合结果；采用结合监督和自监督学习的联邦学习框架来平衡个性化和泛化。

Result: 通过大量实验验证了NMoE系统的有效性，为NMoE训练算法提供了见解和基准。

Conclusion: NMoE系统能够有效解决边缘设备部署大型AI模型的挑战，在保持通信效率和数据隐私的同时实现个性化与泛化的平衡。
...
TL;DR: OSBAD是一个用于电池应用异常检测的开源基准测试框架，通过比较15种不同算法并引入物理统计特征转换和贝叶斯优化，提升异常检测性能。
...
Method: 建立开源基准测试框架，包含15种统计、距离和无监督机器学习算法；提出物理统计特征转换工作流将集体异常分解为点异常；使用贝叶斯优化管道进行超参数调优。

Result: 在涵盖液态和固态化学的多个数据集上验证，展示了跨化学体系的泛化能力，能够识别不同电化学系统中的异常。
...
TL;DR: RLAC是一种后训练方法，通过动态评估标准验证来解决开放式生成任务中的奖励扩展问题，使用LLM作为批评者动态识别最可能的失败模式，联合优化生成器和批评者。


<details>
  <summary>Details</summary>
Motivation: 开放式生成任务需要满足多样且隐式的评估标准，大量相关标准导致验证成本过高和不完整评估，使得基于标准的奖励强化学习难以扩展。

Method: 使用LLM作为批评者动态识别最可能的失败模式，通过外部验证器验证，联合优化生成器和批评者，减少所需验证次数。

Result: 实验表明RLAC在文本生成中提高了事实准确性，在代码生成中提高了正确性，优于穷举验证和奖励模型方法。

Conclusion: 动态批评者比固定批评者更有效，展示了RLAC在将强化学习后训练扩展到自由形式生成任务中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决语言模型在新任务微调时出现的灾难性遗忘问题，传统PEFT方法如LoRA存在秩约束限制，而稀疏适配提供了不施加秩约束的替代方案。

Method: RIGSA方法：1) 随机初始化全秩适配器；2) 使用ReZero类似的门控机制；3) 通过迭代幅度剪枝进行稀疏化。在SmolLM2-1.7B-Instruct模型上评估，使用新颖的视觉文本任务(Textual MNIST)。

Result: RIGSA在Textual MNIST任务上成功学习，与4位QLoRA和随机掩码表现相当。尽管可训练参数比QLoRA多，但RIGSA配置显示出更少的遗忘，特别是在GSM8k任务上，不过与随机掩码表现相近。

Conclusion: RIGSA提供了一种有效的稀疏适配方法，能够减少灾难性遗忘，特别是在复杂推理任务上表现优于QLoRA，为参数高效微调提供了新的选择。
...
<details>
  <summary>Details</summary>
Motivation: 真实随机过程具有记忆效应、时间相关性、长程依赖性和异常扩散现象，这些在标准扩散或桥模型中使用布朗运动时无法捕捉。

Method: 利用最近提出的分数布朗运动的马尔可夫近似(MA-fBM)，构建FDBM框架，在保持分数布朗运动非马尔可夫性质的同时实现可处理的推理。

Result: 在蛋白质构象预测中获得了更低的Cα原子位置均方根偏差(RMSD)，在无配对图像翻译中获得了更低的Fréchet Inception距离(FID)，性能优于布朗运动基线。

Conclusion: FDBM框架在保持分数布朗运动非马尔可夫性质的同时实现了可处理的推理，在蛋白质结构预测和无配对图像翻译任务中均优于基于布朗运动的基线方法。
...
TL;DR: 提出了一种基于个性化核心集的加权联邦学习方法，通过选择代表性数据点而非全部客户端数据进行训练，在保证理论最优性的同时提升性能


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中客户端训练整个数据集带来的计算负担问题，通过智能选择代表性样本来降低通信和计算成本

Method: 个性化核心集加权联邦学习，每个客户端仅基于核心集代表性数据点向中央服务器发送训练更新

Result: 理论分析显示平均泛化误差达到极小极大最优，实验在多个基准数据集上相比随机采样方法有显著提升，在医疗数据集上也优于基于子模优化的子集选择方法

Conclusion: 智能选择训练样本能够有效提升联邦学习性能，同时降低计算和通信开销
...
<details>
  <summary>Details</summary>
Motivation: 超声血流分析对疾病诊断很重要，但超声存在深度衰减问题，传统EchoPIV技术测量血流速度精度有限。物理信息机器学习可以增强在噪声或不完整数据情况下的准确性和鲁棒性。

Method: 采用物理信息神经场模型结合多尺度傅里叶特征编码，从稀疏噪声超声数据中重建血流场，无需真实监督。

Result: 模型在合成和真实数据集上均实现了持续低均方误差的去噪和修复效果，与参考流场和真实流量测量结果一致。

Conclusion: 将其他成像模态中验证有效的方法应用于超声血流重建，解决了超声血流重建的特殊挑战，展示了物理信息神经场在医学血流重建中的潜力。
...
TL;DR: 提出了一种基于度量学习的无秩张量分解框架，用判别性相似度优化替代传统重构目标，通过三元组损失和多样性/均匀性正则化学习数据驱动的嵌入空间。


<details>
  <summary>Details</summary>
Motivation: 传统张量分解方法在处理高维数据时，基于重构和固定秩约束的方法往往无法捕捉语义上有意义的结构。

Method: 采用度量学习方法，优化三元组损失并结合多样性和均匀性正则化，学习数据驱动的嵌入空间，其中距离直接反映语义相似度。

Result: 在多个领域（人脸识别、脑连接分析、模拟数据）的评估中，该方法在聚类指标上显著优于PCA、t-SNE、UMAP和传统张量分解方法，且在小训练数据集上表现优于基于Transformer的方法。

Conclusion: 该工作确立了度量学习作为张量分析的新范式，优先考虑语义相关性而非像素级保真度，在数据稀缺场景下具有计算优势。
...
TL;DR: 本文提出了一种基于域对抗ConvNeXt自编码器(A-CNT)的方法，用于在室内环境变化下实现鲁棒的恶意干扰源定位，解决了传统UWB定位系统在房间布局改变时性能严重下降的问题。


<details>
  <summary>Details</summary>
Motivation: UWB定位虽然能提供厘米级精度，但易受干扰攻击，且在室内环境布局变化时定位性能会严重下降。现有ML/DL方法在跨域干扰源定位方面研究不足。

Method: 提出了域对抗ConvNeXt自编码器(A-CNT)，利用梯度反转层对齐不同域间的CIR特征，以缓解环境变化导致的性能下降。

Result: 在修改后的房间布局中，A-CNT将平均欧几里得误差降至34.67厘米，相比非对抗迁移学习提升77%，相比最佳基线提升83%，30厘米内样本比例恢复到0.56。

Conclusion: 域对抗特征对齐能够在环境变化下实现鲁棒且可迁移的室内干扰源定位。
...
TL;DR: 本文研究了科学机器学习中数据保真度与计算成本之间的权衡关系，通过重新定义经典缩放定律，将数据集轴分解为计算预算和数据集组成，揭示了多保真度神经代理数据集的缩放行为。


<details>
  <summary>Details</summary>
Motivation: 科学机器学习通常受限于通过数值模拟生成训练数据的高昂成本，但通过调整建模假设和近似，可以在模拟保真度和计算成本之间进行权衡，这是其他领域所缺乏的特点。

Method: 使用低保真度和高保真度的雷诺平均纳维-斯托克斯（RANS）模拟来研究神经代理中数据保真度与成本之间的权衡关系，重新制定经典缩放定律，将数据集轴分解为计算预算和数据集组成。

Result: 实验揭示了计算-性能缩放行为，并展示了在给定数据集配置下预算依赖的最优保真度混合比例。
...
TL;DR: 提出基于路由的方法解决视觉语言模型在顺序微调时的灾难性遗忘问题，在保持基础能力的同时整合新任务，无需同时访问所有任务数据。
...
Method: 采用基于路由的方法，在InternVL-2模型（2B和8B参数）上实现，允许集成新任务同时保持预训练获得的基础知识。

Result: 路由方法在ChartQA、MMBench、DocVQA等通用基准上保持性能，同时提高专业任务准确率，且无需所有任务数据并发访问。

Conclusion: 基于路由的学习具有可扩展性和鲁棒性，特别适用于语义相关的新任务，并能实现跨模态的优越知识迁移。
...
TL;DR: 本文提出时间特征分析器，通过引入时间归纳偏置来分解语言模型表示，将表示分解为可预测部分和残差部分，解决了传统稀疏自编码器在捕捉语言时序动态方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有特征提取方法假设概念是独立方向，但语言模型表示具有丰富的时间动态特性（如概念维度增长、上下文相关相关性、非平稳性），这与稀疏自编码器的独立性先验相冲突。

Method: 引入时间特征分析，将给定时间的表示分解为两部分：可预测部分（可从上下文推断）和残差部分（捕捉上下文无法解释的新信息），具有时间归纳偏置。

Result: 时间特征分析器能正确解析花园路径句子、识别事件边界，并区分抽象慢速信息和新颖快速信息，而传统稀疏自编码器在这些任务中表现不佳。

Conclusion: 设计稳健的可解释性工具需要与数据匹配的归纳偏置，时间特征分析器通过时间归纳偏置更好地捕捉了语言的时间动态特性。
...
TL;DR: 该研究结合可解释机器学习和符号建模，开发了Kolmogorov Arnold Networks (KANs)来预测和解释水库水温动态，在红河流域10个水库上实现了高精度预测(R²=0.97)并推导出可解释的解析表达式。


<details>
  <summary>Details</summary>
Motivation: 准确预测水库水温对可持续水资源管理、生态系统健康和气候韧性至关重要，但单纯的预测无法揭示控制物理过程，需要结合可解释性方法来理解水温动态的驱动因素。

Method: 使用集成学习(RF、XGBoost)和神经网络(MLP)模型预测水温，采用SHAP分析量化物理驱动因素的贡献，并开发KANs符号网络将数据驱动洞察转化为紧凑的解析表达式。

Result: 模型达到最佳RMSE=1.20°C、R²=0.97的预测精度；通过KANs推导出10个复杂度递增的方程，R²从0.84(单预测因子)提升到0.92(10个预测因子)；深度是关键的次要预测因子，降水影响有限。

Conclusion: 该框架通过结合预测精度和解释能力，将黑箱模型转化为透明替代模型，既提高了水库热动力学预测能力，又增进了对其物理过程的理解。
...
TL;DR: 提出了一个终身表示学习框架，使用多任务经验风险最小化作为子程序，基于任务规避维度建立了样本复杂度边界，适用于带噪声的分类和回归任务。


<details>
  <summary>Details</summary>
Motivation: 终身学习需要学习者在面对连续任务时识别和利用共享结构，与多任务学习和元学习不同，终身学习要求在线积累知识并利用现有知识。

Method: 使用多任务经验风险最小化作为子程序的简单算法，引入任务规避维度概念来分析样本复杂度。

Result: 建立了适用于广泛学习问题的样本复杂度边界，并在带噪声的分类和回归任务中进行了具体实例化。

Conclusion: 提出的终身表示学习框架和算法能够有效处理连续任务中的共享结构学习问题，具有良好的理论保证。
...
<details>
  <summary>Details</summary>
Motivation: 解决动态状态估计中动态和测量模型的联合学习问题，通过监督学习方式同时优化模型参数和噪声特性。

Method: 使用坐标上升算法进行最大似然估计，学习高斯假设下的动态和测量模型的神经网络参数及噪声协方差矩阵。

Result: 训练得到的动态和测量模型能够与非线性卡尔曼滤波器结合，在测试阶段有效估计系统状态。

Conclusion: 该方法能够有效学习动态状态估计所需的完整模型，为非线性系统状态估计提供了一种监督学习解决方案。
...
### [423] [LongCat-Flash-Omni Technical Report](https://arxiv.org/abs/2511.00279)
...
TL;DR: LongCat-Flash-Omni是一个5600亿参数的开源全模态模型，采用渐进式训练策略和高效的多模态感知模块，在保持低延迟实时音视频交互的同时，在全模态基准测试中达到开源模型的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够处理多种模态（文本、图像、视频、音频）的大规模模型，实现实时音视频交互，同时保持强大的单模态能力，推动开源社区在多模态AI领域的发展。

Method: 采用课程启发的渐进式训练策略，从简单到复杂的模态序列建模任务过渡；基于Shortcut连接的MoE架构，集成高效的多模态感知和语音重建模块；开发模态解耦并行方案处理大规模多模态训练的数据和模型异构性。

Result: 模型在5600亿参数规模下（激活270亿参数）实现低延迟实时音视频交互；在全模态基准测试中达到开源模型的最先进性能；在文本、图像、视频理解以及音频理解和生成等广泛模态特定任务中表现极具竞争力；训练效率达到纯文本训练的90%以上。

Conclusion: LongCat-Flash-Omni成功展示了大规模多模态模型的可行性，通过创新的训练策略和架构设计，在保持高效率的同时实现了全面的多模态能力，为社区提供了强大的开源基础模型。
...
### [424] [Predicting Encoding Energy from Low-Pass Anchors for Green Video Streaming](https://arxiv.org/abs/2511.00707)
*Zoha Azimi,Reza Farahani,Vignesh V Menon,Christian Timmerer*

Main category: cs.MM

TL;DR: 提出了一种轻量级能耗预测方法，通过低分辨率参考编码预测高分辨率视频编码的能耗，在保持感知质量的同时显著降低编码和解码能耗。


<details>
  <summary>Details</summary>
Motivation: 视频流媒体占互联网流量主导地位，高分辨率内容在异构设备上分发需要平衡用户体验质量(QoE)与能耗效率，解决碳排放问题。

Method: 使用低分辨率锚点编码预测高分辨率编码能耗，自动选择编码参数(分辨率、量化参数)，基于VMAF保持感知质量在可接受范围内。

Result: 在Inter4K数据集的100个视频序列上测试，平均VMAF分数仅降低1.68(低于JND阈值)，编码能耗节省51.22%，解码能耗节省53.54%。

Conclusion: 该方法能有效平衡视频质量与能耗，为大规模视频流媒体服务提供可行的节能解决方案。
...
### [425] [Rhythm in the Air: Vision-based Real-Time Music Generation through Gestures](https://arxiv.org/abs/2511.00793)
*Barathi Subramanian,Rathinaraja Jeyaraj,Anand Paul,Kapilya Gangadharan*

Main category: cs.MM

TL;DR: 提出基于视觉的动态手势识别系统，用于实时音乐创作，使用多层注意力GRU模型在自定义数据集上达到96.83%准确率


<details>
  <summary>Details</summary>
Motivation: 改善人机交互体验，实现无需物理接触的手势音乐创作，让用户以新颖方式与音乐互动

Method: 构建包含15000个样本、21个手势类别的自定义数据集，开发多层注意力门控循环单元(MLA-GRU)模型，结合GRU学习时序模式和注意力机制聚焦音乐相关手势片段

Result: MLA-GRU模型显著超越传统GRU，准确率达96.83%(基线86.7%)，在交互应用中表现出优越的效率和速度

Conclusion: 该系统不仅推进了人机交互体验，还证明了MLA-GRU在需要快速精确手势识别场景中的有效性
...
### [426] [EV-NVC: Efficient Variable bitrate Neural Video Compression](https://arxiv.org/abs/2511.01590)
*Yongcun Hu,Yingzhen Zhai,Jixiang Luo,Wenrui Dai,Dell Zhang,Hongkai Xiong,Xuelong Li*

Main category: cs.MM

TL;DR: 提出了一种高效的变比特率神经视频编码器EV-NVC，通过分段线性采样器和长短时特征融合模块提升高码率范围的率失真性能，并采用混合精度训练策略。


<details>
  <summary>Details</summary>
Motivation: 训练变比特率神经视频编码器是一项极具挑战性的任务，因为其复杂的训练策略和模型结构。

Method: 使用分段线性采样器(PLS)改善高码率范围的率失真性能，长短时特征融合模块(LSTFFM)增强上下文建模，并采用混合精度训练和详细的分阶段训练策略。

Result: 实验结果表明，在低延迟模式下，该方法相比HM-16.25减少了30.56%的BD-rate。

Conclusion: 提出的EV-NVC方法在神经视频编码中取得了显著的性能提升，特别是在高码率范围和上下文建模方面表现优异。
